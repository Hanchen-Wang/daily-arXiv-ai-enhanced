...
Motivation: 传统的标签平滑方法虽然能改善网络泛化能力，但为所有非目标类别分配相同的重要性，破坏了类别间的相互关系。

Method: 提出Label Smoothing++方法，为目标类别使用固定标签，同时让网络学习非目标类别的标签，保持类别间关系。

Result: 在多个数据集上的实验表明，该方法能有效缓解过度自信预测，同时促进类别间关系和泛化能力。

Conclusion: Label Smoothing++在保持标签平滑优势的同时，更好地保留了类别间关系，提升了模型的泛化性能。
...
Method: 开发VILOD工具，集成t-SNE图像特征投影、不确定性热图和模型状态可视化，支持用户探索数据、解释模型状态和实施多样化的样本选择策略。

Result: 实证研究表明VILOD通过交互可视化使模型状态和数据集特征更易解释，不同视觉引导标注策略能够达到与自动化不确定性采样基准相当的检测性能。

Conclusion: VILOD为目标检测的人机协作标注工作流提供了新颖工具和实证见解，使其更加透明、可管理且可能更有效。
...
<details>
  <summary>Details</summary>
Motivation: 传统知识蓄粑(KD)使用固定的平衡因子alpha来结合硬标签和软标签损失，但静态的alpha子最优，因为训练过程中硬软监督的最优平衡可能发生变化。

Method: 1) 将alpha作为可学习参数在训练中自动优化 2) 提出用学生-教师差异来动态计算alpha的公式 3) 使用MLP+注意力机制的上下文感知模块(CAM)来自适应地重新加权类别级别的教师输出。

Result: 在CIFAR-10数据集上，以ResNet-50为教师、ResNet-18为学生的实验显示，该方法比固定权重KD基线获得更高的准确率，并产生更稳定的收敛性能。

Conclusion: 自适应性知识蓄粑框架能够有效地动态调整硬软监督的平衡，提升了知识蓄粑的性能和稳定性，为知识传递提供了更优雅的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 为了提供视频到EEG信号生成的多模态数据集，支持多模态大模型的训练和对齐能力，推动情感分析、数据增帽和脑机接口应用的研究

Method: 使用自我对役图网络(SPGN)结合模型液化模型来生成个性化EEG信号，并提供视频-EEG数据对齐的工程流水线

Result: 释放包含1000个样本的新数据集，每个样本包含SEED-VD视频刺激、62通道200Hz的EEG信号以及情感标签

Conclusion: 该框架为视频-EEG对齐和多模态研究提供了新的工具，具有重要的研究和工程价值
...
<details>
  <summary>Details</summary>
Motivation: 探索网络拓扑结构对深度学习的影响，研究不同连接模式如何影响学习效率和模型性能，同时为边缘中心网络度量提供剪枝和优化的自然框架。

Method: 使用三种边缘中心网络度量（Forman-Ricci曲率、Ollivier-Ricci曲率、边缘介数中心性）对三种网络生成器（ER、WS、BA模型）生成的随机有线神经网络进行选择性剪枝，保留重要突触。

Result: FRC剪枝方法能有效简化网络，在保持与ORC相当性能的同时提供显著的计算优势，压缩后的网络在准确率、特异性和敏感性方面表现良好。

Conclusion: 计算效率更高的FRC可以作为ORC的有效替代方案进行神经网络剪枝，在保持性能的同时大幅降低计算成本，为网络压缩提供了新的有效工具。
...
<details>
  <summary>Details</summary>
Motivation: 解决手写爵士乐谱的光学音乐识别挑战，特别是现有系统无法处理的和弦识别问题，以及手写图像的高变异性问题

Method: 创建包含293份手写爵士乐谱的数据集，提供合成乐谱图像，开发专门的OMR模型，讨论特定标记化选择和使用合成乐谱及预训练模型的优势

Result: 构建了包含2021个谱表的数据集，与Humdrum **kern和MusicXML格式对齐，开发了针对爵士乐谱的OMR系统

Conclusion: 成功解决了手写爵士乐谱OMR的特定挑战，公开发布了所有代码、数据和模型，为后续研究提供了重要资源
...
<details>
  <summary>Details</summary>
Motivation: 现实世界部署中，目标识别模型面临多种域偏移（图像统计变化、姿态变化、遮挡、类别混淆）导致精度严重下降，需要开发更鲁棒的视觉理解方法

Method: 1) 构建合成数据集生成管道，生成包含4个线索（精确边界框、类别名、详细目标级描述、场景级描述）的图像标注；2) 对Llama 3.2 11B Vision Instruct进行参数高效监督调优；3) 推理时执行两阶段重新思考：首先生成四个线索，然后将其作为证据进行迭代修正

Result: 在隔离单个域偏移的鲁棒性基准测试中，RT-VLM始终超越强基线方法

Conclusion: 结构化多模态证据与显式自我批判循环的结合，是实现可靠且可迁移视觉理解的有前景途径
...
<details>
  <summary>Details</summary>
Motivation: 解决传统运动性能测量技术质昂、复杂且无法满足普通运动员需求的问题，通过智能手机技术平敏性能分析的可达性

Method: 采用自定制训练的YOLOv5模型进行羽毛球检测，结合Kalman筛波实现稳健的轨迹跟踪，通过视频基于动力学的速度估算方法进行时空缩放

Result: 开发出了一套完整的移动应用系统，能够从标准视频录制中自动计算羽毛球速度，实现了高水准性能分析的普及化

Conclusion: 该系统成功地将高级运动性能分析技术民主化，为各级别羽毛球运动员提供了可达的性能评估工具，有助于运动员技术改进和发展
...
Method: 构建大规模手写数据库（42人手写1200字符），升级OpenHandWrite_Toolbox工具箱，采用多元回归分析手写准备和执行过程

Result: 字形因素在字符、部首和笔画层面都影响手写准备和执行；语音因素在所有三个层面影响执行；语言效应呈现层级刻衰特征（字符层> 部首层> 笔画层）

Conclusion: 手写准备和执行在部首和笔画层面与语言组件密切相关，该数据库和工具箱为语言心理学和神经语言学研究提供了价值资源
...
Conclusion: 解耦预测和分类任务相比使用单一DGNN解决统一问题能提高性能，该方法还能监测瞬态状态，为高级辅助系统功能增强提供有价值见解
...
<details>
  <summary>Details</summary>
Motivation: 脑罐的异质性和强度分布不均匀使得MRI分割面临重大挑战，准确的罐叙划分对临床决策和疗效跟踪至关重要。
...
Conclusion: 硬聚类和软聚类存在计算效率与边界准确性之间的特征军交换，需根据具体应用场景选择适合的算法。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法主要集中于二元分类，在需要准确识别具体疾虫害类型的场景中存在局限性，需要更加精确的多类别分类方法

Method: 基于预训练卷积神经网络(CNN)模型，集成注意力模块，采用综合数据增强管道来缓解类别不平衡问题

Result: 在实际田间图像数据集上达到96.90%的总体准确率和0.96 F1分数，表现超过了使用同样数据集的其他方法

Conclusion: 该模型为葱植物疾虫害的精确多类别分类提供了有效解决方案，在农业实践中具有重要应用价值
...
Method: 提出基于蒸馏的方法，显式建模源和目标速度场差异，引入时间依赖的偏移项将噪声潜在向量推向目标轨迹。

Result: 实验结果表明DVRF在编辑质量、保真度和可控性方面表现优越，无需架构修改即可高效应用于文本到图像编辑任务。

Conclusion: DVRF成功弥合了基于分数的扩散优化和基于速度的整流流优化，为FlowEdit等方法提供了理论解释，是一个高效且广泛适用的编辑框架。
...
Result: 注意力增强的CNN在所有指标上均优于基线架构，其中EfficientNetB5+混合注意力表现最佳，在两个医学影像数据集上均取得显著性能提升。
...
Method: 使用自建的COCO格式数据集评估五种目标检测模型，并开发用户界面进行模型训练和评估。

Result: 结果显示了各模型在检测精度和计算效率之间的不同拼扣，为实际应用提供了选型指南。

Conclusion: 这些发现为实际太阳能板监测和维护场景中选择适当的检测方法提供了价值较高的指导。
...
<details>
  <summary>Details</summary>
Motivation: 解决实例分割中人工标注成本高、收集困难的问题，提出一种无需人类标注的高效分割方案。

Method: 1）使用MultiCut算法处理自监督特征获取粗粟面具 2）面具筛选获取高质量粗粟面具 3）设计超像素引导面具损失（硬损失+软损失）训练分割网络 4）采用自适应损失进行自训练提升面具质量

Result: 在公开数据集的实例分割和目标检测任务中，该框架表现超过了之前的最佳方法。

Conclusion: 该无督程框架能够高效地分割对象而无需人工标注，为计算机视觉应用提供了一种成本效益更高的解决方案。
...
Result: 新模型在模拟和真实细胞迁移案例中测试，通过滚动策略预测完整细胞轨迹，达到了高精度水平。

Conclusion: 该研究成功开发了一种能够准确预测细胞迁移轨迹和有丝分裂事件的综合机器学习方法，为理解细胞集体行为提供了有效工具。
...
Result: 在BraTs2020数据集（369名脑肿瘤患者的MRI扫描）上，模型取得了最佳Dice分数0.31，平均Dice分数稳定在0.285±0.015，验证集上表现一致且无过拟合迹象。

Conclusion: EMCAD在保持计算效率的同时实现了中等水平的脑肿瘤分割性能，为计算资源受限场景下的医学图像分析提供了可行的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有潜在生成模型的tokenizer在图像压缩过程中偏向优先重建低频信息，导致高频细节丢失，生成图像缺乏实感性，尤其是在纹理丰富的区域出现过度平滑和视觉假像。

Method: 提出了一种基于小波变换的频率感知变分自助编码器（FA-VAE）框架，通过明确解耦低频和高频组件的优化过程，使得方法能够同时保持全局结构和收集细节细节。

Result: 该方法能够显著提高图像重建的高频保真度，改善了细节细节的重建质量，减少了过度平滑和视觉假像，提升了感知质量。

Conclusion: 频率感知优化对于实现真实图像表征至关重要，本文提出的FA-VAE框架为现有潜在tokenizer提供了一种有效的改进方案，在内容创作、神经渲染和医学成像等应用中具有广泛的应用前景。
...
<details>
  <summary>Details</summary>
Motivation: 深度卷积神经网络虽然在计算机视觉任务中表现优异，但其计算和内存开销限制了实际部署。需要一种高效的模型压缩方法来支持边缘和移动平台的部署。

Method: 差分敏感性融合剪枝方法，通过融合梯度基础敏感性、一阶泰勒展开和KL散度的差异来计算每个满足器的差分敏感性分数。采用指数缩放机制来强调在不同指标上重要性不一致的满足器，识别结构不稳定或对模型性能关键性低的候选。

Result: 在50-70%的剪枝率下，该方法显著减少了模型复杂度，实现了超过80%的FLOPs减少。在70%剪枝时，保持了基准准确性的98.23%，超越了传统假设方法在压缩和普适化方面的表现。

Conclusion: 该方法提供了一种高效、确定性的单次过程剪枝解决方案，为深度卷积神经网络的可扩展和适应性压缩提供了有效途径，为在边缘和移动平台上的高效部署摩平了道路。
...
Conclusion: Veriserum数据集为算法开发和评估提供了可复现的基准，通过公开访问促进医学影像和计算机视觉研究的进步。
...
<details>
  <summary>Details</summary>
Motivation: 解决YOLO架构在资源突出环境（如无人机）中部署的效率问题，探索不同冻结配置对当代YOLOv8和YOLOv10架构的具体影响

Method: 系统研究多种冻结配置策略，集成梯度行为分析（L2范数）和可视化解释（Grad-CAM）来揭示不同冻结策略下的训练动态

Result: 发现没有通用的最佳冻结策略，而是依赖于数据特性。冻结backbone有助于保留通用特征，浅层冻结更适合处理极端类别不平衡。这些配置能节省28%的GPU内存，某些情况下mAP@50识别精度超过全量微调

Conclusion: 该研究提供了实证发现和实践指南，为资源突出环境中的对象检测转移学习提供了平衡的、基于证据的方法
...
<details>
  <summary>Details</summary>
Motivation: 传统四元数神经网络尝试完全在四元数域操作，计算复杂且效率低。QUAN旨在通过近似方法在保持旋转等变性的同时提高计算效率。

Method: 使用Hamilton乘积分解将四元数卷积近似为实数运算，引入独立四元数批量归一化(IQBN)保证训练稳定性，并将四元数操作扩展到空间注意力机制。

Result: 在图像分类任务中，QUAN以更少参数和更快收敛速度获得更高准确率；在目标检测任务中，相比标准CNN提高了参数效率和旋转处理能力，建立了四元数CNN在该任务的新SOTA。
...
Method: 整合六个公开数据集，统一手部姿态布局，提供描述性、时间戳标注的动作基元，总计1107小时视频，涵盖290个操作任务和600多个环境。

Result: 成功训练了语言条件化的模仿学习策略来预测灵巧的手部轨迹，验证了数据集的实用性。

Conclusion: OpenEgo旨在降低从自我中心视频学习灵巧操作的门槛，支持视觉-语言-动作学习领域的可重复研究。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法存在两个基本问题：背景高斯与前景高斯获得相同特征贡献，以及多视图语言嵌入中的视图特定噪声导致的不一致性。

Method: 提出可见性感知语言聚合(VALA)，计算每条光线的边际贡献并应用可见性感知门控保留可见高斯；使用流式加权几何中值在余弦空间中合并多视图噪声特征。

Result: VALA在参考数据集上显著提升了开放词汇定位和分割性能，一致超越现有工作。

Conclusion: 该方法以快速且内存高效的方式实现了鲁棒、视图一致的语言特征嵌入，有效解决了3D高斯语言特征蒸馏中的关键问题。
...
<details>
  <summary>Details</summary>
Motivation: 现有表示学习方法主要针对动作识别任务，基于孤立序列表示，缺乏对多尺度表示和跨序列变化的利用，需要专门针对动作分割任务的预训练方法

Method: 提出Shuffle and Warp数据增强策略生成多动作排列，引入Cross Permutation Contrasting（CPC）和Relative Order Reasoning（ROR）两个对比学习代理任务，构建Dual-Surrogate Contrastive Learning（DuoCLR）网络

Result: 在未修剪数据集上的实验显示，DuoCLR在多类和多标签动作分割任务中均显著优于最先进方法

Conclusion: 所提出的对比学习框架通过创新的数据增强和双代理任务设计，有效学习了适用于动作分割的多尺度特征表示，为动作分割任务提供了有效的预训练解决方案
...
<details>
  <summary>Details</summary>
Motivation: 解决事件监视相机在敏感性和噪声之间的批执导致的事件流不完整问题，该问题影响了运动先验知识的完整性和事件导向去模糊的效果

Method: 提出RED网络：1)随机投影策略(RPS)通过随机隐藏事件来培养网络对不完整模式的适应能力；2)解耦全波注意力机制显式建模内部运动、运动间和跨模态关联；3)互动模块增强模糊图像的运动敏感区域并向事件表征注入语义上下文

Result: 在合成和真实数据集上的实验表明，RED网络在准确性和稳健性方面都达到了最先进水平

Conclusion: 通过对事件流不完整性的明确建模和适应性训练，RED网络能够从部分受损的事件数据中提取可靠的运动信息，从而实现更稳健有效的运动去模糊
...
Method: 基于参数敏感性分析，优先量化高敏感性参数，利用未量化低敏感性参数补偿错误；通过列向聚类和行并行量化框架，采用全局共享逆Hessian矩阵更新机制

Result: 在ResNet-50和YOLOv5s上实现了20-200倍量化速度提升，均值准确性损失低于0.3%

Conclusion: 该方法有效场平衡了量化效率和模型准确性，适用于资源受限的边缘计算环境
...
<details>
  <summary>Details</summary>
Motivation: 需要从单张肖像图像生成高质量可控3D头像，现有方法在泛化能力、纹理重建质量和渲染效率方面存在不足。

Method: 采用重建与重演分离框架，基于WebSSL构建大规模单样本高斯头部生成器，使用两阶段训练策略提升泛化能力和高频纹理重建，推理时使用超轻量级高斯头像驱动。

Result: 在512x512分辨率下达到90FPS高帧率渲染，遵循缩放定律（参数规模增加性能提升），驱动效率不受影响，在定量和定性实验中超越现有最优方法。

Conclusion: 该分离框架有效解决了单图像3D头像生成的质量和效率问题，展示了良好的可扩展性和实用性。
...
<details>
  <summary>Details</summary>
Motivation: 当前Deepfake检测方法受限于现有数据集的多样性不足，无法应对真实世界场景中的挑战，包括未知的高级伪造技术、面部场景变化、真实数据丰富度和真实传播退化等问题。

Method: 构建MFFI数据集，基于四个战略维度：1）50种不同伪造方法；2）多样化面部场景；3）多样化真实数据；4）多级退化操作。包含1024K图像样本。
...
<details>
  <summary>Details</summary>
Motivation: 现有视频摘要方法主要关注帧间全局时序建模，但忽略了细粒度视觉实体（如对象）与视频主要内容的高度相关性。语言引导的视频摘要需要全面理解复杂真实世界视频的语义信息

Method: 提出递归时空图网络VideoGraph，将对象和帧分别建模为空间图和时间图的节点，通过图边表示节点间的语义关系。引入语言查询到图节点表示中以防止仅基于视觉相似性配置边，并采用递归策略优化初始图和关键帧分类

Result: 在多个通用和查询聚焦的视频摘要基准测试中，VideoGraph在监督和无监督设置下均达到了最先进的性能

Conclusion: 将视频摘要视为语言引导的时空图建模问题，通过语义关系建模和递归优化策略，能够有效提升视频摘要的质量和准确性
...
<details>
  <summary>Details</summary>
Motivation: 现有的自监督表示学习方法主要关注全局表示，但密集预测任务需要空间精度和细粒度细节，需要捕获局部语义信息

Method: 基于预训练表示进行额外自监督学习，通过PaKA目标对齐师生模型的密集特征分布，捕获统计依赖关系并匹配结构关系，同时研究专门的数据增强策略

Result: 在多个密集视觉基准测试中取得了最先进的结果

Conclusion: 提出的框架有效解决了密集表示学习问题，PaKA方法简单而有效，能够成功将语义知识迁移到密集特征空间
...
Method: 提出静态动作级剪枝（利用全局历史和局部上下文）、动态层级剪枝（基于层级重要性）、轻量级动作感知控制器（根据动作粒度调整剪枝强度）

Result: 在LIBERO基准测试中，相比OpenVLA-OFT，在NVIDIA A800上实现1.46倍加速，在NVIDIA GeForce RTX 3090上实现1.57倍加速，成功率损失可忽略不计

Conclusion: 通过结合局部和全局信息进行智能token选择，SpecPrune-VLA能够在保持模型性能的同时显著提升VLA模型的推理速度
...
<details>
  <summary>Details</summary>
Motivation: 现有概念擦除方法无法同时实现鲁棒性和有效性，特别是对于窄概念（如版权角色、名人）的擦除存在挑战，这些概念与非目标概念距离较近，需要更精细的操作。
...
Conclusion: SuMa方法成功实现了对窄概念的鲁棒擦除，同时保持了图像质量，为解决版权和法律问题提供了有效解决方案。
...
Method: 采用自监督学习构建神经网络嵌入空间，该空间能够反映树木的植被特性，基于航空高光谱图像数据。

Result: 实验结果表明，使用植被特性相关的嵌入空间构建的树木表示，在下游机器学习任务中的性能优于直接使用高光谱植被特性作为树木表示的方法。

Conclusion: 自监督学习能够有效从无标签或标签有限的高光谱图像中学习有意义的植被特性表示，为精准农业中的树木分析提供了更好的特征表示方法。
...
<details>
  <summary>Details</summary>
Motivation: 因为基于非孜加拉数据集训练的车辆检测系统在孜加拉本土车辆类型检测中表现差异，影响自动驾驶技术在发展中地区的应用。

Method: 使用包含29种孜加拉特有车辆类别的自建数据集，评测六种YOLO模型变体的检测性能，包括mAP、回归率和F1分数等指标。

Result: YOLOv11x表现最好（mAP@0.5为63.7%），但逐图推理时间最长（45.8毫秒）。中等模型在性能和速度之间取得平衡。罀罐车辆等稀有车型检测准确度极低。

Conclusion: 研究为孜加拉特定交通环境开发了基础检测系统，解决了通用模型在发展中地区的性能不足问题。
...
<details>
  <summary>Details</summary>
Motivation: 现有角色编辑方法在简单提示下表现良好，但在面对包含多层语义、时序逻辑和复杂上下文关系的长文本叙事时，往往出现编辑能力退化、语义理解偏差和身份一致性崩溃的问题。

Method: 通过感知器注意力的精细分解、引入ID损失和与扩散模型的联合动态训练，以及集成模块的离线融合策略，在最小数据润滑条件下实现编辑性注入。

Result: 在复杂叙事环境中实现了深度多层级语义编辑，同时保持身份一致性，满足长提示和高质量图像生成的需求，在IBench评估中取得了优异结果。

Conclusion: EditIDv2成功解决了长文本叙事场景下的角色编辑挑战，通过创新的架构设计和训练策略，在少量数据条件下实现了高质量的语义编辑和身份保持。
...
Method: 提出语言场景图预测(LSGA)任务，采用对象导向两阶段方法(OOTSM)：首先使用LLM预测对象的出现和消失，然后生成详细的人-物关系。将纯文本模型用于预测未来帧的场景图

Result: 在Action Genome基准测试中，短期mean-Recall(@10)提升3.4%，长期mean-Recall(@50)大幅提升21.9%，达到最先进性能

Conclusion: 基于语言模型的方法能够有效整合常识知识，显著提升场景图预测性能，特别是在长期预测方面。LSGA作为一个独立任务具有重要研究价值
...
<details>
  <summary>Details</summary>
Motivation: 高能粒子物理中的堆积干扰问题与图像去器存在类似性，研究者想借鉴物理学原理（如守恒性、局域性、隔离性）来改善神经网络在强污染条件下的稳健性。

Method: 提出了一系列堆积器去器：包含守恒约束的残差CNN、高斯噪声变种以及WIPUNet（基于UNet构架集成物理原理的网络）。在CIFAR-10和BSD500数据集上进行实验。

Result: 在高斯噪声条件下，PU去器与标准基线模型相当，而WIPUNet在高噪声水平下显示出更大的优势，说明物理受启发的偏见提供了更好的稳健性。

Conclusion: 物理受启发的归纳偏见能够在不依赖复杂SOTA模型的情况下，有效提升神经网络在强污染条件下的稳健性和性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有大语言模型和视觉语言模型在单轮任务表现优秀，但在需要深度上下文理解和复杂视觉推理的多轮交互中存在推理碎片化、上下文丢失和幻觉等挑战。

Method: 提出CAMVR框架，包含两个核心创新：1) 视觉-文本上下文记忆单元(VCMU) - 动态读写记忆网络，存储管理每轮交互的关键视觉特征、文本语义表示及其跨模态对应关系；2) 自适应视觉聚焦指导(AVFG)机制 - 利用VCMU上下文动态调整视觉编码器对相关图像区域的注意力。采用多层次推理集成策略确保响应生成与当前输入和历史上下文深度一致。

Result: 在VisDial、适配的A-OKVQA和新型多轮指令跟随(MTIF)数据集上的广泛实验表明，CAMVR始终达到最先进的性能。

Conclusion: CAMVR框架有效增强了视觉语言大模型在多轮视觉-文本推理中的鲁棒性和连贯性，解决了现有模型在多轮交互中的关键限制。
...
<details>
  <summary>Details</summary>
Motivation: 图像分割研究中的可重复性危机，特别是性能评估指标实现的不一致性导致了显著的计算差异，需要更可靠的度量实现方法。

Method: 开发了基于网格的MeshMetrics框架，通过理论分析和实证验证，提供比传统网格方法更精确的距离度量计算。

Result: MeshMetrics相比现有工具实现了更高的准确性和精度，受距离量化等离散化伪影的影响显著减小，Hausdorff距离差异可超过100mm，归一化表面距离差异达30%以上。

Conclusion: MeshMetrics作为一个开源Python包，为图像分割评估提供了更可靠的度量实现，有助于改善研究可重复性。
...
Result: 在NTU RGB+D和NTU RGB+D 120等挑战性基准测试中达到最先进或极具竞争力的性能（如NTU RGB+D X-Sub 94.1%，NTU RGB+D 120 X-Set 90.0%）

Conclusion: LVLM-VAR不仅显著提升了动作识别准确率，还通过生成自然语言解释大大增强了模型的可解释性，为视频理解任务开辟了新途径
...
<details>
  <summary>Details</summary>
Motivation: 现有方法主要依赖RGB图像的语义特征，忽视了空间结构信息在捕获视角不变特征中的重要性。无人机地理定位面临剧烈视角差异和外观变化的挑战

Method: 使用双分支特征提取框架，包含差异感知融合模块(DAFM)和联合约束交互聚合(JCIA)策略，实现深度融合和联合约束的语义与结构信息表示。提出3D地理增强技术生成潜在视角变化样本

Result: 在University-1652和SUES-200数据集上的大量实验验证了方法对复杂视角变化的鲁棒性，达到了最先进的性能

Conclusion: 通过融合RGB和法线图像的几何结构信息，结合创新的融合策略和增强技术，有效解决了跨视角地理定位中的视角差异问题，提升了定位精度
...
Result: 该方法能够使模型适应声学数据，无需手动标注或重新训练模型

Conclusion: 该框架展示了VLMs在专业生物声学领域应用的潜力，为领域特定视觉分析提供了新的解决方案
...
Method: 引入时间嵌入相似性、运动对齐变换损失和窗口时间融合模块，改进模型架构以保持空间结构

Result: 在雷达/声呐到LiDAR转换中表现出更好的时间和空间一致性，降低了绝对轨迹误差，提高了占用地图精度

Conclusion: LiDAR-BIND-T保持了即插即用的模态融合能力，同时显著增强了时间稳定性，为SLAM应用提供了更鲁棒的解决方案
...
<details>
  <summary>Details</summary>
Motivation: 解决多相机系统中视觉信息融合效率低、一致性差的问题，避免对单个相机的冗余处理，提高状态估计的准确性和鲁棒性

Method: 引入全景视觉特征模型统一多相机观测，提出外参补偿方法解决帧间不对准问题，基于因子图实现紧耦合的LiDAR-视觉-惯性系统集成

Result: 在公开数据集上的实验表明，全景视觉特征模型提高了多相机约束的质量和一致性，相比现有系统具有更高的精度和鲁棒性

Conclusion: 所提出的全景视觉特征模型和外参补偿方法有效解决了多相机系统的三角测量不一致问题，显著提升了多传感器融合里程计的性能
...
TL;DR: 这篇论文提出了一种量子化的距离适配性超分辨框架，通过空间变化逆问题模型和深度条件卷积核实现了基于场景几何结构的自适应重建策略，在室外场景中获得了领先的性能。
...
Method: 提出了一种严格的变分框架，将超分辨形容为空间变化的逆问题，通过深度条件卷积核的神经网络实现离散的梯度流动力学，并结合学习的距离适应性正则化项。

Result: 在5个标准数据集上进行综合评估，在KITTI室外场景中在2倍和4倍缩放下分别达到36.89/0.9516和30.54/0.8721的PSNR/SSIM值，超过现有方法0.44dB和0.36dB。

Conclusion: 这项工作建立了第一个理论基础坚实的距离适应性超分辨框架，在深度变化场景中实现了显著改进，同时在传统标准测试中保持了竞争力。
...
<details>
  <summary>Details</summary>
Motivation: 解决日常场景中两人交互行为的准确捕捉问题，现有工作大多只关注单人或假设身体方向/位置不变，需要建模更动态、语义一致的长期交互。

Method: 采集了241个多模态交互序列数据集InterAct，提出分层回归身体动作的扩散方法，并设计了新颖的微调机制提高嘴唇准确性。

Result: 构建了包含多样复杂动作和长期交互模式的数据集，提出的方法能够从语音有效生成两人交互的面部表情和身体动作。

Conclusion: InterAct数据集填补了长期动态交互行为数据的空白，提出的扩散方法为交互行为生成提供了有效解决方案，推动了该领域研究发展。
...
TL;DR: PARSE-VOS是一个基于大语言模型的免训练框架，通过分层粗粒度到细粒度的推理来解决视频对象分割中的语言-视觉对齐问题，在多个基准测试中达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 当前方法在处理复杂组合式描述时，依赖整体视觉-语言融合而表现不佳，特别是在外观相似但运动姿态不一致的对象对齐方面存在困难。
...
Conclusion: PARSE-VOS通过分层推理框架有效解决了复杂语言描述下的视频对象分割问题，证明了免训练方法在该任务中的有效性。
...
TL;DR: 本文提出了PictOBI-20k数据集，用于评估大型多模态模型在象形甲骨文视觉破译任务上的表现，包含2万个甲骨文和实物图像，形成1.5万个选择题。实验表明通用LMM具备初步视觉破译能力，但主要受语言先验限制而非有效利用视觉信息。


<details>
  <summary>Details</summary>
Motivation: 甲骨文破译是理解人类早期生产方式的关键，但当前方法受限于考古发掘的零散性和铭文语料有限。大型多模态模型强大的视觉感知能力为甲骨文视觉破译提供了新可能。

Method: 构建PictOBI-20k数据集，包含2万张精心收集的甲骨文和实物图像，形成超过1.5万个多选题。进行主观标注以研究人类与LMM在视觉推理中参考点的一致性。

Result: 实验表明通用LMM具备初步的视觉破译技能，但LMM并未有效利用视觉信息，大多数时候受限于语言先验。

Conclusion: 该数据集可促进未来甲骨文导向LMM的视觉注意力评估和优化，为甲骨文破译研究提供新的技术路径。
...
<details>
  <summary>Details</summary>
Motivation: 医学形态重建中，训练数据与目标形状的姿态对齐差异会导致偏差的重建结果，尤其在观察小部分形状时。这是当前方法中常被忽视的关键因素。

Method: 提出一种高效方法来调整现有模型以适应特定目标形状。该方法保持线性模型的计算效率，无需访问原始训练数据，通过简单的预处理步骤实现。

Result: 方法显著提高了重建准确性和预测方差，能够准确恢复平移对齐的模型，并对小角度旋转提供良好的近似。

Conclusion: 该方法作为插件式预处理步骤，可广泛应用于现有的形态重建流水线，具有实用价值和普遍适用性。
...
TL;DR: PointPillars改进版本，通过新的CNN网络3DPillars和二阶段检测流程，在保持高效率的同时提升了3D检测性能，缩小了与最先进方法的性能差距。


<details>
  <summary>Details</summary>
Motivation: 解决PointPillars的两大限制：1)伪图像表示无法保保精确的3D结构 2)难以采用二阶段检测流程

Method: 提出3DPillars CNN网络，通过可分离汇累模块从伪图像中提取汇累特征，以及二阶段检测流程中的稀疏场景上下文特征模块

Result: 在KITTI和Waymo Open数据集上验证了方法的有效性和效率，实现了速度与准确性的良好平衡

Conclusion: 该方法成为首个基于伪图像表示的二阶段3D检测框架，在保持PointPillars效率优势的同时显著提升了检测性能
...
### [53] [AdCare-VLM: Leveraging Large Vision Language Model (LVLM) to Monitor Long-Term Medication Adherence and Care](https://arxiv.org/abs/2505.00275)
*Md Asaduzzaman Jabin,Hanqi Jiang,Yiwei Li,Patrick Kaggwa,Eugene Douglass,Juliet N. Sekandi,Tianming Liu*

Main category: cs.CV

TL;DR: 提出了AdCare-VLM，一种基于Video-LLaVA的多模态视觉语言模型，用于通过患者视频进行药物依从性视觉问答，在TB药物监测数据集上取得了优于现有方法的性能。
...
Method: 使用806个临床专家标注的TB药物监测视频数据集，基于Video-LLaVA架构构建多模态视觉语言模型，整合视觉特征（患者面部、药物、饮水等）与医学概念，进行参数高效微调。

Result: 在LLM-TB-VQA数据集上，该方法超越了LLaVA-V1.5和Chat-UniVi等模型，绝对性能提升3.1%-3.54%，并通过消融实验和注意力可视化验证了方法的有效性。

Conclusion: AdCare-VLM成功地将视觉特征与医学概念对齐，提高了多模态交互能力，为药物依从性监测提供了一种有效的视频分析方法，具有很好的可解释性。
...
<details>
  <summary>Details</summary>
Motivation: 现有相机-雷达融合方法中，前向投影方法生成稀疏BEV特征，后向投影方法存在深度模糊问题导致误报，需要解决这些限制

Method: 使用后向投影技术，结合图像密集但不可靠的深度分布与雷达稀疏但精确的深度信息，引入包含雷达上下文信息的空间交叉注意力机制

Result: 在nuScenes数据集上实现62.4% NDS和54.0% mAP的3D目标检测性能，在后向投影相机-雷达融合方法中达到最先进水平

Conclusion: CRAB模型通过有效融合相机和雷达信息，成功解决了深度模糊问题，提升了3D目标检测和分割的性能
...
<details>
  <summary>Details</summary>
Motivation: 医疗设备制造中的视觉检测面临小样本、不平衡数据、高分辨率图像和严格监管要求的挑战，需要开发能够在这些约束下工作的深度异常检测方法。

Method: 提出了两种注意力引导的自编码器架构：第一种使用4-MS-SSIM结构相似性异常评分进行轻量级实时缺陷检测；第二种使用马哈拉诺比斯评分对降维潜在特征进行特征距离分析，用于监督监控。

Result: 第一种方法在仅有10%缺陷样本的情况下达到ACC 0.903（无监督阈值）和0.931（有监督阈值）；第二种方法达到ACC 0.722（有监督阈值），两种方法均优于重新实现的基线模型。
...
TL;DR: Probabilistic SAM是Segment Anything Model的概率扩展版本，通过引入潜在变量空间和变分训练目标，能够生成反映人类标注变异性的多样化分割掩码，特别适用于医学图像中存在标注不确定性的场景。


<details>
  <summary>Details</summary>
Motivation: 现有的可提示分割模型（如SAM）是确定性的，只能为每个提示生成单一分割结果，无法捕捉真实世界任务中固有的模糊性。这在医学成像中尤其成问题，因为标注不确定性或专家间差异可能导致多个合理的分割结果。

Method: 在SAM框架中集成先验网络和后验网络，通过潜在变量空间调制提示嵌入，使用变分目标进行训练，学习生成多样化的分割掩码分布。

Result: 在LIDC-IDRI肺结节数据集上评估表明，Probabilistic SAM能够产生与专家分歧一致的多样化输出，在不确定性感知指标上优于现有概率基线方法。
...
TL;DR: 基于MODIS卫星多谱图像的3D卷积网络实时监测沙尘暴的系统，达到0.92准确度
...
Method: 使用NASA Terra和Aqua卫星MODIS仪器的36个波段图像，通过3D卷积网络学习模式，分离沙尘与云层和地表特征

Result: 在17个独立MODIS场景中达到约0.92的准确度，均方误差为0.014，核心区域检测结果一致

Conclusion: 聚合波段和空间学习能够提供全球范围的实时沙尘警报，使用更大输入窗口或注意力模型可进一步提升边缘检测效果
...
TL;DR: 对比较多种深度学习分割模型在限限的心血管组织分析图像数据集上的表现，发现模型性能很容易受到数据分割的影响，性能排名不能真正反映临床效用。


<details>
  <summary>Details</summary>
Motivation: 心血管组织分析图像的准确分割对心血管疾病研究和诊断至关重要，但深度学习模型发展受限于标注数据稀缺。

Method: 系统性评估多种先进深度学习分割模型（U-Net、DeepLabV3+、SegFormer、SAM、MedSAM等），在限限的心血管组织学图像数据集上进行调优，使用贝叶斯搜索进行深度调参。

Result: 模型性能对数据分割极为敏感，小的性能差异更多是由统计噪声导致，而非真正算法优势。标准测试方法在低数据临床场景中存在局限性。

Conclusion: 在数据稀缺的临床环境中，传统的模型性能测试和排名方法可能无法真正反映临床效用，需要更加谨慎地评估模型的实际价值。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法通过直接拼接处理图像对，无法充分建模时间相关性和空间语义变化，限制了变化理解的视觉-语义对齐效果
...
<details>
  <summary>Details</summary>
Motivation: 现有的肌肉骨骼风险评估方法大多设计用于受控环境，依赖单一数据类型，在复杂环境中可靠性不足。需要开发能够融合多模态数据的可靠风险评估方法。

Method: 提出ViSK-GAT框架，结合残差块和轻量级Transformer块学习时空依赖关系。包含两个新模块：细粒度注意力模块（FGAM）用于跨模态特征精炼，多模态几何对应模块（MGCM）用于增强跨模态一致性。构建了包含8个风险类别的多模态数据集。

Result: 验证准确率93.55%，测试准确率93.89%，精确率93.86%，F1分数93.85%，Cohen's Kappa和Matthews相关系数均为93%。RMSE为0.1205，MAE为0.0156，在9个流行迁移学习骨干网络中表现最优。
...
<details>
  <summary>Details</summary>
Motivation: 新兴应用更关注语义保存而非像素级重建，需要跨数据分布和下游任务的鲁棒性能，传统深度学习方法无法满足这些语义压缩需求

Method: 利用多模态基础模型的零样本和表征能力，提出基于CLIP模型的语义压缩方法，将CLIP特征嵌入压缩为最小比特同时保持跨任务语义信息

Result: 在基准数据集上保持语义完整性，平均比特率约2-3×10⁻³ bpp，比主流图像压缩方法节省95%以上比特率，在极端压缩下仍具有零样本鲁棒性

Conclusion: 基于CLIP的语义压缩方法实现了极低比特率下的语义保持，展现出跨数据分布和下游任务的强大零样本鲁棒性，为语义压缩提供了新范式
...
TL;DR: AttriPrompt是一个新颖的深度提示学习框架，通过利用CLIP视觉编码器的中间层特征来增强文本语义表示，解决了现有深度文本提示方法的两个关键局限性：过度依赖对比学习目标和静态提示问题。


<details>
  <summary>Details</summary>
Motivation: 当前深度文本提示方法存在两个主要问题：1）过度依赖对比学习目标，只关注高层语义对齐而忽略细粒度特征优化；2）所有输入类别使用静态提示，缺乏内容感知适应能力。

Method: 提出AttriPrompt框架，包含属性检索模块（聚类视觉特征并从提示池中检索语义相似的提示）、双流对比学习（实现细粒度对齐）和自正则化机制（防止过拟合）。

Result: 在三个基准测试上的广泛实验显示，AttriPrompt优于最先进方法，在基础到新类别设置中实现了高达7.37%的性能提升，在跨域知识转移方面表现出色。
...
TL;DR: 通过提出保持系数采样(CPS)方法，解决了在流匹配模型中应用RL时SDE采样导致的噪声问题，提高了奖励学习效果和生成质量


<details>
  <summary>Details</summary>
Motivation: 发现SDE基于采样在流匹配模型中引入显著噪声色彩，对奖励学习过程产生负面影响，需要找到替代方案

Method: 受DDIM启发，提出保持系数采样(CPS)方法，重新定义采样过程以消除噪声色彩

Result: CPS方法成功消除了噪声色彩，实现了更准确的奖励建模，促进了Flow-GRPO和Dance-GRPO等RL优化器的更快更稳定的收敛

Conclusion: 该研究为流匹配模型的强化学习应用提供了有效的无噪声采样方案，显著提升了生成质量和学习效果
...
Conclusion: 所提出的DIFM能够有效利用原始和增强图像的互补信息，多尺度边界损失提高了对象边界的分割精度，为医学图像分割提供了有效解决方案
...
TL;DR: StripDet是一个轻量级3D目标检测框架，通过Strip Attention Block和分层骨干网络实现高效的边缘设备部署，在KITTI数据集上以0.65M参数达到79.97% mAP，比基准方法参数减少7倍


<details>
  <summary>Details</summary>
Motivation: 解决点云3D目标检测模型计算和内存需求大的问题，使其能够在边缘设备上高效运行

Method: 提出Strip Attention Block（SAB）模块，通过非对称条带卷积分解标准2D卷积来捕获长距离空间依赖；设计硬件友好的分层骨干网络，结合深度可分离卷积和简单多尺度融合策略

Result: 在KITTI数据集上，仅用0.65M参数就实现了79.97%的汽车检测mAP，超越了基准PointPillars（参数减少7倍），并且优于最近的轻量级和知识蒸馏方法
...
TL;DR: 通过神经网络实时生成渴染光效果，方法速度比现有技术提高30%，节省计算资源


<details>
  <summary>Details</summary>
Motivation: 传统渴染光效果技术依靠多次模糊和纹理采样，包含条件分支，占用大量执行时间，成为实时渲染的急需解决的急需解决的问题

Method: 提出两种神经网络基于渴染光效果方法：Neural Bloom Lighting (NBL) 和 Fast Neural Bloom Lighting (FastNBL)，分别重点关注质量和性能，通过神经网络生成亮度掩码

Result: 在多种3D场景中进行测试，FastNBL速度提升28%，NBL提升12%，同时保持高质量的渴染光效果，在亮度掩码准确性和推理速度方面都超越了标准方法

Conclusion: 该方法能够更快速地实现现实的渴染光效果，节省计算资源，对于维持高FPS环境中的沉浸感和顺畅体验至关重要，为实时渲染提供了更高的现实感
...
TL;DR: 提出一种针对医学3D影像的自监督学习方法，通过三个子任务捕获空间相关语义，提高模型可解释性，同时保持与现有方法相当的性能表现。


<details>
  <summary>Details</summary>
Motivation: 解决当前医学可视化任务中自监督学习方法主要借鉴2D视觉设计，缺乏对3D空间知识学习过程直观展示的问题，提升医学可解释性。

Method: 设计三个子任务来捕获医学3D影像中的空间相关语义，遵循可观察原则确保可解释性，并采用多粒度空间关系建模来维持训练稳定性。

Result: 实验结果表明，该方法能够提供与当前方法相当的性能，同时便于直观理解自监督学习过程。

Conclusion: 该方法通过创新的子任务设计，在保持性能的同时显著提升了医学3D影像自监督学习的可解释性，为医疗领域的数据稀缺问题提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 解决艺术风格迁移中缺乏真实监督数据的基本挑战，通过去风格化反转风格迁移过程来获得真实的监督信号
...
<details>
  <summary>Details</summary>
Motivation: 解决深度神经网络在测试数据分布与训练数据不同时的性能下降问题，传统方法在训练域有限或训练-测试领域差距大时效果不佳

Method: 提出ConstStyle方法，利用统一域捕获领域不变特征并桥接领域差距。训练时将样本映射到优化后的统一域，测试时类似投影后再进行预测

Result: 在多样化场景中 consistently 优于现有方法，当只有有限seen domains时，相比次优方法准确率提升高达19.82%

Conclusion: 通过统一域对齐训练和测试数据，ConstStyle能有效减少领域偏移影响，即使在领域差距大或seen domains少的情况下也能显著提升领域泛化性能
...
<details>
  <summary>Details</summary>
Motivation: 扩散模型在图像处理任务中表现出强大的生成能力，但如何有效处理稀疏视图CT重建中的缺失投影视图完成和全局信息建模仍是一个挑战。

Method: 提出联合训练机制和时变稀疏条件重加权引导策略，采用线性回归校正分布偏移，构建双网络并行架构进行全局校正和多子频段优化。

Result: 在公开和真实数据集上，PSNR提升2.58dB，SSIM提高2.37%，MSE降低0.236，重建图像在结构一致性、细节恢复和伪影抑制方面表现优异。

Conclusion: STRIDE方法通过创新的引导策略和网络架构设计，实现了高质量的稀疏视图CT重建，具有良好的泛化性和鲁棒性。
...
<details>
  <summary>Details</summary>
Motivation: 单目3D目标检测由于输入是单张2D图像，缺乏深度线索，深度估计问题是一个不适定问题。现有方法主要使用CNN或Transformer作为特征提取骨干网络，然后通过特定检测头预测3D参数。

Method: 采用解耦策略，注入预计算的分割信息先验，并将其直接融合到特征空间来指导检测，不扩展检测模型或联合学习先验。重点评估分割信息对现有检测流程的影响，不添加额外的预测分支。

Result: 在KITTI 3D目标检测基准测试中表现优异，在场景中的小目标（行人和骑行者）检测上优于仅依赖RGB图像特征的等效架构。

Conclusion: 证明理解输入数据可以平衡对额外传感器或训练数据的需求，分割信息的引入能有效提升单目3D检测性能。
...
TL;DR: 基于深度学习的单目风船位姿估计方法，结合动态热力图和光流信息，在SPADES-RGB和SPARK-2024数据集上实现了更好的2D关键点定位和6自由度位姿估计性能


<details>
  <summary>Details</summary>
Motivation: 现有的单图像风船位姿估计方法主要依赖静态关键点定位，没有利用空间作业中的时序信息

Method: 采用人体位姿估计的深度学习框架，结合Vision Transformer编码器的图像特征和预训练光流模型的运动线索来定位2D关键点，然后通过PnP求解器恢复6自由度位姿

Result: 在SPADES-RGB数据集上训练和评估，并在SPARK-2024的真实和合成数据上进行测试，在2D关键点定位和6自由度位姿估计方面都显示出比单图基线更好的性能

Conclusion: 该方法能够有效抓取运动动态特征，提高位姿估计的准确性，并在不同数据分布上都显示出了较好的演化能力
...
TL;DR: Khana是一个包含13.1万张印度菜肴图像的新基准数据集，涵盖80个类别，用于食物图像分类、分割和检索，填补了印度美食研究的空白。


<details>
  <summary>Details</summary>
Motivation: 全球对多样化烹饪体验的兴趣增长，但现有食物数据集缺乏对印度美食的全面覆盖，印度菜具有丰富的地区多样性和复杂制作工艺，需要专门的数据集来支持相关应用开发。

Method: 建立了印度美食的分类体系，收集了约13.1万张500x500像素的图像，涵盖80个标签类别，并评估了最先进的模型在分类、分割和检索任务上的性能。

Result: 创建了Khana数据集，为研究人员提供了全面的基准测试资源，也为开发者开发实际应用提供了有价值的资源。

Conclusion: Khana填补了印度美食研究的空白，为研究和开发提供了具有挑战性的基准，有助于推动基于印度丰富美食文化的实际应用发展。
...
Method: 使用LoRA调优的BLIP-2模型生成多样候选答案，用PolyFormer对每个答案进行空间定位，最后通过思维链推理模块评估答案是否指向相同或不同区域
...
<details>
  <summary>Details</summary>
Motivation: 现有的开放词汇目标检测预训练数据集主要基于地面自然图像，与无人机图像存在显著的域差距，导致在无人机图像上性能大幅下降。

Method: 首先提出改进的UAV-Label标注引擎，构建了包含200万实例和1800个类别的UAVDE-2M数据集以及15000张图像的UAVCAP-15k数据集；然后提出跨注意力门控增强融合(CAGE)模块，并将其集成到YOLO-World-v2架构中。

Result: 在VisDrone和SIMD数据集上的大量实验验证了所提方法在无人机图像和遥感应用中的有效性。

Conclusion: 通过构建专门的无人机图像数据集和设计CAGE融合模块，成功解决了无人机开放词汇目标检测中的域适应问题，提升了模型在无人机图像上的性能表现。
...
TL;DR: 提出了一种新颖的细粒度动态感知（FDP）框架用于微表情识别，通过帧级特征排序编码动态信息，结合局部-全局特征感知Transformer和动态图像构建任务，显著提升了识别性能。


<details>
  <summary>Details</summary>
Motivation: 微表情识别面临瞬时性、细微性和动态性挑战，现有手工特征方法需要关键帧，深度学习方法受限于小规模和低多样性训练数据。

Method: 提出FDP框架：1）使用局部-全局特征感知Transformer进行帧表示学习；2）采用排序器计算帧级特征排序分数；3）时序池化捕获动态表示；4）共享动态表示进行MER分类和动态图像构建。

Result: 在CASME II、SAMM、CAS(ME)^2和CAS(ME)^3数据集上F1分数分别比之前最佳结果提升4.05%、2.50%、7.71%和2.11%，显著优于现有方法。

Conclusion: FDP框架通过动态特征排序和辅助动态图像构建任务，有效捕获微表情的细微动作特征，缓解数据稀缺问题，在微表情识别任务上取得了state-of-the-art性能。
...
<details>
  <summary>Details</summary>
Motivation: 传统视觉-LiDAR里程计方法存在传感器不对准、未能充分利用时序信息、需要大量手动调参等问题，难以在多样传感器配置下实现高精度和强鲁棒性。

Method: 提出三个关键创新：1)稀疏查询融合实现多模态数据有效融合；2)时序交互更新模块整合时序预测位置与当前帧数据；3)时序片段训练策略结合集体平均损失机制实现全局优化。

Result: 在KITTI和Argoverse里程计数据集上的大量实验表明，DVLO4D在姿态精度和鲁棒性方面均达到最先进性能，推理时间82ms，具有实时部署潜力。

Conclusion: DVLO4D通过创新的稀疏时空融合方法有效解决了传统视觉-LiDAR里程计的局限性，在精度、鲁棒性和效率方面都表现出色，为自主系统定位提供了可靠的解决方案。
...
TL;DR: 评估通用视觉语言模型在血液报告图像分析中的表现，发现Qwen-VL-Max、Gemini 2.5 Pro和Llama 4 Maverick三种模型在100份血液报告图像上展现出临床应用潜力


<details>
  <summary>Details</summary>
Motivation: 血液报告解读对健康知识很重要，但个人往往难以准确理解，导致焦虑和问题遗漏，需要自动化分析解决方案

Method: 使用三种VLMs分析100份多样化血液报告图像，通过临床相关问题提示，用Sentence-BERT处理答案进行对比评估

Result: 通用VLMs显示出开发面向患者的初步血液报告分析工具的实际应用前景，能够直接从图像提供清晰解读
...
### [79] [Index-Preserving Lightweight Token Pruning for Efficient Document Understanding in Vision-Language Models](https://arxiv.org/abs/2509.06415)
...
Method: 使用二进制补丁级分类器移除非文本区域，通过最大池化精炼步骤恢复碎片化的文本区域以增强空间一致性。

Result: 在真实世界文档数据集上的实验表明，该方法显著降低了计算成本，同时保持了可比的准确性。

Conclusion: 该轻量级令牌剪枝框架有效地减少了VLM处理文档图像的计算要求，为高效文档理解提供了可行方案。
...
<details>
  <summary>Details</summary>
Motivation: 无人机输电线路自动化检测面临小缺陷在复杂背景下难以识别的问题，传统检测器存在下采样细节丢失、轻量级主干网络边界敏感性弱、全局上下文与局部线索融合不足等挑战。
...
TL;DR: BranchGRPO通过分支采样策略、树状优势估计和剪枝策略，在保持探索多样性的同时大幅降低GRPO训练成本，实现了更好的人类偏好对齐效果


<details>
  <summary>Details</summary>
Motivation: 解决GRPO方法在图像和视频生成模型对齐中遇到的高计算成本、过多SDE采样步骤以及稀疏奖励导致的训练不稳定问题

Method: 提出分支采样策略更新SDE采样过程，共享公共前缀计算，采用树状优势估计器包含密集过程级奖励，并通过路径和深度冗余剪枝加速收敛

Result: 在图像和视频偏好对齐实验中，对齐分数比强基线提升16%，同时训练时间减少50%

Conclusion: BranchGRPO通过效率优化和稳定性改进，在保持生成质量的同时显著降低计算成本，为生成模型对齐提供了更有效的解决方案
...
TL;DR: 提出一种多阶段图神经网络架构，通过层次池化操作来模型闭合空间内的自然对流传热过程，解决传统GNN在高分辨率图结构中捕捉长距离依赖关系的问题。


<details>
  <summary>Details</summary>
Motivation: 高保真CFD模拟需要专家物理模型、细密网格和大量计算，限制了迭代速度。需要一种能够直接从模拟数据学习热流体行为的方法，尤其是在不规则网格结构上。

Method: 设计了一种新的多阶段GNN架构，利用层次池化和解池化操作，在多个空间尺度上逐步模型从全局到局部的相互作用。在新开发的CFD数据集上进行评估，该数据集模拟了不同长宽比矩形空间内的自然对流传热。

Result: 实验结果显示，该模型在预测准确性、训练效率和减少长期错误累积方面都超过了现有最优GNN基线模型。

Conclusion: 该研究证明了多阶段GNN方法在网格基础流体动力学模拟中模拟复杂传热过程的潜力，为热设计提供了更高效的解决方案。
...
TL;DR: HDM是一个高效文本到图像扩散模型，在消费级硬件上仅需535-620美元训练成本即可实现1024x1024高质量生成，通过创新的Cross-U-Transformer架构和训练策略大幅降低计算需求。


<details>
  <summary>Details</summary>
Motivation: 解决传统文本到图像扩散模型训练成本过高的问题，为计算资源有限的个人研究者和小型组织提供高质量图像生成的民主化方案。

Method: 提出Cross-U-Transformer（XUT）架构，使用交叉注意力进行跳跃连接；采用TREAD加速、移位方形裁剪策略和渐进分辨率缩放的综合训练方案；构建仅343M参数的小型模型。

Result: 在四块RTX5090 GPU上实现竞争性的1024x1024生成质量，训练成本显著降低至535-620美元，展现出优秀的组合一致性和直观相机控制等涌现能力。

Conclusion: 证明了通过精心设计的架构和训练策略，小型模型也能实现高质量文本到图像生成，为资源有限的研究者提供了可行的替代扩展范式。
...
TL;DR: 一种通过神经网络识别边缘并整合到数学优化模型中的新题形式微空间分辨率旷光重建技术，显著提升了同质材料样品的重建质量


<details>
  <summary>Details</summary>
Motivation: 解决传统旷光重建技术在处理同质材料相位置的尖锐边缘时存在的模糊和伪影问题

Method: 训练神经网络识别子图片中的边缘，然后将训练好的网络整合到数学优化模型中，在学习预测和原始数据支持之间寻找平衡

Result: 实验数据集显示了界面尖锐度和材料均质性的显著提升，比基准算法更优

Conclusion: 该技术能够生产高质量的重建图像，展示了在提升旷光成像技术方面的潜力
...
Method: 提出MedSeqFT框架，包含两个核心组件：1) MDS选择 - 识别最能代表原始预训练分布的下游样本；2) K&G RFT - 基于LoRA的知识蒸馏方案，平衡任务特定适应和预训练知识保留。

Result: 在两个多任务数据集上的10个3D分割任务中，MedSeqFT持续优于最先进的微调策略，平均Dice提升3.0%。在未见任务(COVID-19-20和肾脏)上验证了更好的迁移性，特别是肿瘤分割。

Conclusion: MedSeqFT建立了一个有效、知识保持的序列微调范式，用于将基础模型适应不断演变的临床任务，损失景观和参数变化的可视化分析进一步证明了其鲁棒性。
...
### [86] [Interleaving Reasoning for Better Text-to-Image Generation](https://arxiv.org/abs/2509.06945)
...
TL;DR: 提出了IRG框架，通过交替文本推理和图像合成来提升文本到图像生成的质量，在多个基准测试中取得5-10个百分点的绝对提升


<details>
  <summary>Details</summary>
Motivation: 现有统一多模态理解生成模型在图像生成能力上有显著改进，但在指令遵循和细节保持方面仍与GPT-4o等理解-生成紧密耦合的系统存在差距

Method: 引入Interleaving Reasoning Generation (IRG)框架，交替进行文本推理和图像合成：先产生文本思考指导初始图像生成，然后对结果进行反思以细化细节、视觉质量和美学效果

Result: 在GenEval、WISE、TIIF、GenAI-Bench和OneIG-EN等基准测试中取得state-of-the-art性能，视觉质量和细粒度保真度有显著改善
...
<details>
  <summary>Details</summary>
Motivation: 病理图像分析对自动化肿瘤诊断至关重要，但现有视觉语言模型难以捕捉结构化病理报告所需的复杂推理，限制了其在临床环境中的应用。

Method: 提出PathoHR-Bench基准测试，并引入病理特异性视觉语言训练方案，通过生成增强和扰动样本进行多模态对比学习。

Result: 实验评估显示该方法在PathoHR-Bench和六个额外病理数据集上达到最先进性能，在细粒度病理表征方面表现优异。

Conclusion: 所提出的方法有效解决了现有视觉语言模型在病理图像分析中的局限性，显著提升了模型在复杂病理推理任务中的表现。
...
TL;DR: CARDIE是一种基于颜色和亮度内容的无监督图像聚类算法，专门针对图像增强任务设计，相比基于语义属性的聚类方法更有效


<details>
  <summary>Details</summary>
Motivation: 解决自动图像聚类在图像增强应用中受限的问题，因为传统方法难以定义对图像增强有意义的聚类

Method: 提出CARDIE无监督算法，基于颜色和亮度内容进行图像聚类；同时引入量化图像增强算法对亮度分布和局部方差影响的方法
...
<details>
  <summary>Details</summary>
Motivation: 多光谱和高光谱图像在农业、环境监测和城市规划中具有互补性，但现有方法难以同时保持空间细节和光谱保真度。多光谱图像空间分辨率高但光谱分辨率有限，高光谱图像光谱丰富但空间分辨率较低。

Method: 提出SpecSwin3D模型，采用3D移位窗口Transformer框架，输入5个多光谱波段，重建224个高光谱波段。引入级联训练策略逐步扩展光谱范围，并设计优化的波段序列来更好地捕捉成对关系。

Result: 模型达到PSNR 35.82 dB、SAM 2.40°、SSIM 0.96，比基线MHF-Net提升5.6 dB PSNR，ERGAS降低超过一半。在土地利用分类和燃烧区域分割等下游任务中表现出实用价值。

Conclusion: SpecSwin3D成功解决了多光谱到高光谱转换中空间和光谱质量保持的难题，通过Transformer架构和创新的训练策略实现了优异的性能，在多个应用场景中具有重要价值。
...
Method: 采用特征级生成对抗掩蔽机制，结合多对一知识蒸馏策略，利用视网膜基础模型和多个替代年龄编码器，实现对黑盒年龄预测模型的通用防御。
...
<details>
  <summary>Details</summary>
Motivation: 为了解决现有音视频生成模型中存在的训练效率低、标注对齐不准确等问题，开发一个能够同时生成高质量协调音频和视频的统一模型。

Method: 采用专家拼接(SoE)技术深度融合预训练的视频和音乐生成专家模型，开发在线标注管道处理训练数据并在训练过程中生成标签，避免基于文本标注的错位问题。

Result: 在约7,600小时的音视频数据上微调后，模型在环境音生成方面产生良好协调的音视频内容，在语音生成方面具有强对齐性，并发布了Verse-Bench基准数据集。

Conclusion: UniVerse-1通过创新的专家拼接技术和在线标注管道，成功实现了高质量的音视频协同生成，为多模态生成研究提供了新的解决方案和基准工具。
...
<details>
  <summary>Details</summary>
Motivation: 现有的视频场景图生成方法通常针对特定粒度（边界框级别或像素级别）设计，需要任务特定的架构和多阶段训练流程，缺乏统一的解决方案。

Method: 提出UNO框架：1）扩展slot attention机制将视觉特征分解为对象和关系slot；2）对象时序一致性学习确保跨帧表示一致性；3）动态三元组预测模块链接关系slot与对象对。

Result: 在标准边界框级别和像素级别VidSGG基准测试中达到有竞争力的性能，同时通过统一的对象中心设计提高了效率。

Conclusion: UNO证明了单一框架可以同时处理不同粒度的视频场景图生成任务，通过最大化参数共享和最小化任务特定修改实现了良好的泛化能力。
...
TL;DR: 使用自定义卷积神经网络实现自动X光片骨折检测，达到95.96%准确度，但转移学习模型表现差异


<details>
  <summary>Details</summary>
Motivation: 解决低资源场景中骨折诊断难题，免除传统影像方法的高成本、放射风险和专业解释依赖

Method: 使用自定义CNN模型，并与EfficientNetB0、MobileNetV2、ResNet50等转移学习模型进行对比，训练数据为FracAtlas数据集的4,083张X光图像

Result: 自定义CNN在FracAtlas数据集上达到95.96%准确度、0.94精准度、0.88召回率和0.91 F1分数，转移学习模型表现较差

Conclusion: 轻量级CNN在X光骨折检测中具有潜力，强调公平性能测评、多样化数据集和外部验证对临床应用的重要性
...
TL;DR: 本文通过适配IWPOD-Net网络来实现高效的文档检测和置正流水线，在保持竞争力OCR质量的同时达到更小模型和更高效率。


<details>
  <summary>Details</summary>
Motivation: 实时文档检测和置正是自动信息提取的关键步骤，但目前文献中对这一小众领域的研究较少。本文旨在开发一种既能满足OCR提取要求又比现有方案更快的高效文档检测流水线。

Method: 适配IWPOD-Net（车牌检测网络）用于文档检测，在合成ID卡数据集NBID上训练。进行数据增帽和跨数据集验证（使用MIDV数据集）。使用基于Levenshtein距离的新的OCR质量指标来评估性能。

Result: 证明即使文档置正不完美，也能达到状态前沿性能水平。本文模型比当前最优解决方案更小、效率更高，同时保持了竞争力的OCR质量。

Conclusion: 通过适配现有车牌检测网络，可以实现高效的文档检测和置正流水线。该方法在保持高质量OCR提取的同时，具有更小的模型尺寸和更高的运行效率，为实时文档信息提取提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型在3D空间关系理解方面存在局限，而现实世界的具身AI智能体（如机器人和自动驾驶汽车）依赖自我中心的多视角观测，需要更好的空间理解能力
...
Conclusion: Ego3D-Bench和Ego3D-VLM为在现实世界多视角环境中实现人类水平空间理解提供了有价值的工具，当前VLMs仍需改进空间理解能力
...
Method: 构建多阶段解决方案：数据收集、预处理，提出新颖的Skin-Prior Adaptive Vision Transformer模型进行回归，针对数据不平衡问题提出对称对比正则化方法

Result: 成功实现了从自拍面部图像估计SH和TEWL，有效减少了因数据不平衡导致的模型偏差

Conclusion: 这是首个无需物理测量即可从自拍图像进行皮肤评估的研究，弥合了计算机视觉与皮肤护理研究的鸿沟，为更广泛的实际应用提供了AI驱动的可访问皮肤分析
...
TL;DR: PAML框架通过原型感知多模态学习解决视觉定位在开放词汇场景中的挑战，包括跨模态对齐、特征融合和语义原型利用问题，在标准场景和开放词汇场景均取得优异性能。


<details>
  <summary>Details</summary>
Motivation: 当前基于transformer的视觉定位方法在标准场景表现良好，但在开放词汇场景（包含训练时未见的新物体类别）存在明显局限，主要由于三个问题：视觉-语言模态对齐不完美、跨模态特征融合不足、语义原型信息利用不充分。

Method: 提出PAML框架：1）使用ALBEF进行初始跨模态特征对齐；2）视觉判别特征编码器增强显著物体表示；3）新颖的原型发现和继承机制提取聚合多邻居语义原型；4）多阶段解码器进行综合多模态整合和边界框回归。

Result: 在五个基准数据集上的广泛实验验证了该方法，在标准场景具有竞争力，在开放词汇场景达到最先进性能。

Conclusion: PAML框架通过系统解决跨模态对齐、特征融合和语义原型利用三大挑战，有效提升了视觉定位在开放词汇场景的性能，为处理新颖物体类别提供了有效解决方案。
...
TL;DR: 这篇论文提出了视频领域的广义类别发现（Video-GCD）问题，设计了一种新的内存导向一致性对比学习框架（MCCL），通过时空线索和全局上下文来提高视频中新类别发现的准确性。


<details>
  <summary>Details</summary>
Motivation: 现有的GCD方法主要集中于静态图像，但仅靠静态视觉内容不能可靠地发现新类别。视频领域的多视角时间信息对于准确发现新类别至关重要。

Method: 提出MCCL框架，包含两个核心组件：1）一致性认知对比学习（CACL）：利用多视角时间特征估计一致性分数，加权对比损失；2）内存导向表征增强（MGRE）：通过双层内存缓冲维护特征级和logit级表征，提供全局上下文。两者形成互相增强的反馈循环。

Result: 在新建的Video-GCD测试集（包括动作识别和鸟类分类）上进行广泛实验，方法显著超过了来自图像基础设置的竞争性GCD方法，证明了时间信息对于视频中新类别发现的重要性。

Conclusion: 该研究成功将GCD扩展到视频领域，提出的MCCL框架能够有效利用时空线索和全局上下文来提高新类别发现的性能，为视频领域的开放世界学习问题提供了新的解决方案。
...
TL;DR: 提出Text4Seg和Text4Seg++框架，将图像分割转化为文本生成问题，通过语义描述符和R-RLE编码实现高效分割，无需额外解码器，在多个基准测试中达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 解决多模态大语言模型中图像分割集成困难的问题，简化分割流程，提高效率。

Method: 使用文本作为掩码的范式，提出语义描述符表示分割掩码，引入R-RLE编码压缩文本序列，开发Text4Seg和Text4Seg++框架。

Result: 在自然和遥感数据集上一致优于最先进模型，无需任务特定微调，推理速度提升3倍，文本长度减少74%。

Conclusion: 文本驱动的图像分割在多模态大语言模型框架中具有高效性、可扩展性和通用性。
...
TL;DR: MVP-FAS是一个基于CLIP的面部防欺骗框架，通过多视角注意力机制和多文本补丁对齐模块，显著提升了跨域泛化性能
...
Conclusion: MVP-FAS通过多视角和多文本策略有效提升了面部防欺骗的跨域泛化能力，为相关领域提供了新的解决方案
...
TL;DR: 这篇论文系统性分析了3D植物分割技术的挑战，提出了开源框架Plant Segmentation Studio，通过实验验证了稀疏卷积和transformer方法的效果，为植物表型学提供了数据高效的深度学习解决方案。


<details>
  <summary>Details</summary>
Motivation: 解决3D植物分割在植物表型学中面临的三大挑战：注释数据稀缺、深度网络适配困难、缺乏标准化评估协议。需要缩小算法进步与实际应用之间的差距。

Method: 1）系统综述现有3D植物数据集和深度学习分割方法。2）提出开源框架Plant Segmentation Studio（PSS）支持可复现性测试。3）进行广泛的定量实验，评估代表性网络和模拟-实际学习策略。

Result: 识别出稀疏卷积核心网络和transformer基础的实例分割方法表现最佳。模型基于和增强基于的合成数据生成方法在模拟-实际学习中具有补充作用，能够有效减少注释需求。

Conclusion: 该研究成功缩小了算法进步与实际部署之间的差距，为植物表型学领域提供了立即可用的工具和数据高效的深度学习解决方案路线图。
...
<details>
  <summary>Details</summary>
Motivation: 现有货币识别系统往往忽视可用性和真伪评估，特别是在低资源环境中，对视觉障碍用户和离线验证的支持不足，且通常忽略物理损坏和伪造问题

Method: 使用轻量级CNN模型进行面额分类，通过新颖的统一货币损坏指数(UCDI)量化损坏程度，采用基于特征的模板匹配进行假币检测

Result: 在超过82,000张标注图像的数据集上，Custom_CNN模型实现了高分类性能且参数数量少，UCDI指标提供连续的可用性评分，假币检测模块在不同成像条件下都能可靠识别伪造纸币

Conclusion: 该框架支持实时设备端推理，解决了受限环境中的关键部署挑战，证明了准确、可解释且紧凑的解决方案能够在实际环境中支持包容性货币评估
...
TL;DR: 提出了一种融合RGB和热红外成像的多模态检测框架，使用微调YOLOv8模型，通过类别重加权和轻量增强技术提升弱势道路使用者的检测性能，特别是在光线不足和恶劣天气条件下。


<details>
  <summary>Details</summary>
Motivation: 弱势道路使用者（行人、骑行者等）占全球交通死亡人数一半以上，但在光线不佳、恶劣天气和数据不平衡情况下检测仍然困难，需要提升检测可靠性。

Method: 集成RGB和热红外成像，使用微调YOLOv8模型，在KITTI、BDD100K和FLIR数据集上训练，采用类别重加权和轻量增强技术，优化分辨率和骨干网络冻结策略。
...
TL;DR: 这篇论文提出了一种轻量化混合CNN框架，用于早期精准检测也地压力，在保持高准确度的同时大幅减少计算成本。


<details>
  <summary>Details</summary>
Motivation: 传统早地压力检测方法耗时耗力，而现有深度学习模型参数量过大，不适合资源有限的农业环境。

Method: 综合ResNet、DenseNet和MobileNet的轻量化混合CNN框架，加上基于梯度范数影响函数的机器忘学机制。

Result: 计算参数减少15倍，但仍保持竞争力的准确度，在马铃薯田地空中图数据集上表现优异。

Conclusion: 该框架为精准农业提供了一种实用、可扩展且适应性强的早地监测解决方案，特别适用于资源约束环境。
...
TL;DR: 通过给视频大语言模型添加轻量级GO-Tokenizer模块，利用现成物体检测器编码紧凑物体信息，显著提升了时间敏感视频理解性能


<details>
  <summary>Details</summary>
Motivation: 时间敏感视频理解任务可以从帧内基础物体中获益，但传统文本描述方式存在远程标注和信息噪声问题

Method: 设计GO-Tokenizer模块，利用现有物体检测器实时编码紧凑的物体信息，避免文本描述带来的标记长度和噪声问题

Result: 实验结果显示，使用GO-Tokenizer预训练的模型在性能上超过原始Video-LLM和使用文本物体描述的对照组

Conclusion: GO-Tokenizer能够在不同模型、数据集和视频理解任务中广泛提升性能，为时间敏感视频理解提供了高效的物体信息编码方案
...
### [106] [Focusing by Contrastive Attention: Enhancing VLMs' Visual Reasoning](https://arxiv.org/abs/2509.06461)
*Yuyao Ge,Shenghua Liu,Yiwei Wang,Lingrui Mei,Baolong Bi,Xuanshan Zhou,Jiayu Yao,Jiafeng Guo,Xueqi Cheng*

Main category: cs.CV

TL;DR: 提出CARVE方法，通过对比注意力机制在像素级别提取任务相关视觉信号，无需训练即可显著提升视觉语言模型在复杂环境中的性能


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型在复杂视觉环境中性能下降，现有增强方法需要额外训练、依赖外部分割工具或操作粒度粗糙，忽视了模型内在的注意力能力

Method: 分析VLMs注意力模式，发现视觉复杂度与注意力熵强相关；提出CARVE方法，通过对比通用查询和任务特定查询的注意力图，在像素级别分解视觉信号为语义信号和视觉噪声

Result: CARVE方法在开源模型上实现了高达75%的性能提升，无需训练即可一致增强视觉推理性能
...
TL;DR: 这项研究提出了一种统一的深度学习网络，能够同时分析组织学图片和腺镜视频帧，以提高结直肠疾病的诊断效率和准确性。


<details>
  <summary>Details</summary>
Motivation: 传统诊断流程需要分别处理组织学图片和腺镜视频，导致效率低下和可能的诊断差异，需要一种统一的方案来改善结直肠疾病的诊断。

Method: 使用ResNet-50 CNN架构构建统一深度学习网络，集成类别平衡学习、健壮的数据增帽和检验方法。数据来源于PathMNIST数据集的组织学图片和HyperKvasir数据集的腺镜视频帧。

Result: 研究开发出了一个可解释且可重现的诊断流程，能够统一多种诊断模态，为结直肠疾病的检测提供了更高效的方案。

Conclusion: 这个统一的深度学习网络为结直肠疾病的诊断带来了重要进步，通过整合多模态数据提高了诊断的准确性和效率，具有很好的应用前景。
...
TL;DR: 提出了一种即插即用的尺度感知注意力模块(SAAM)，能够将现有的固定尺度超分辨率模型改造为支持任意尺度超分辨率的模型，具有轻量化和高性能的特点。


<details>
  <summary>Details</summary>
Motivation: 传统单图像超分辨率模型往往难以泛化到不同的尺度因子，限制了实际应用。需要一种能够适配多种尺度因子的通用解决方案。

Method: 使用轻量级的尺度自适应特征提取和上采样，结合无参数的Simple Attention Module(SimAM)进行高效指导，并采用梯度方差损失来增强图像细节的清晰度。

Result: 该方法可以无缝集成到多个最先进的SR骨干网络中，在广泛的整数和非整数尺度因子下都能实现竞争性或更优的性能，计算开销极小。

Conclusion: SAAM模块为实际应用场景提供了稳健的多尺度上采样解决方案，显著提升了超分辨率模型的实用性和泛化能力。
...
### [109] [On the Reproducibility of "FairCLIP: Harnessing Fairness in Vision-Language Learning''](https://arxiv.org/abs/2509.06535)
*Hua Chang Bakker,Stan Fris,Angela Madelon Bernardy,Stan Deutekom*

Main category: cs.CV

TL;DR: 对FairCLIP进行可复现性研究，发现原始实现与描述不一致，新实现A-FairCLIP和FairCLIP+均未能验证FairCLIP在性能和公平性方面的改善效果


<details>
  <summary>Details</summary>
Motivation: 验证Luo等人(2024)提出的FairCLIP方法的可复现性，以及评估其通过最小化Sinkhorn距离来改善CLIP模型组群公平性的效果

Method: 重现Luo等人的实验设置，开发新实现A-FairCLIP来检查设计选择，提出支持多属性的FairCLIP+，并分析距离最小化对公平性和性能的影响

Result: 证实CLIP在零检查青光眼分类任务中存在偏见，但实验结果不支持FairCLIP改善性能和公平性的方法。虽然正则化目标减小了Sinkhorn距离，但既有实现和新实现都未能提升零检查青光眼分类的表现

Conclusion: 虽然FairCLIP的正则化目标能够减小Sinkhorn距离，但这并不能有效改善CLIP模型在实际应用中的性能和公平性表现，原研究的主要断言未能得到验证
...
TL;DR: 这篇论文建立了一个自动化流水线，用于分析脑转移瘤症在立体定向放射手术后的纵向MRI数据，通过数据驱动聚类识别了5种主要增长轨迹，并使用机器学习方法预测12个月的病变响应，达到了高达0.90的AUC值。


<details>
  <summary>Details</summary>
Motivation: 脑转移瘤症是瘤症患者死亡的主要原因之一。目前供医生手动分析纵向MRI数据的工作量过大，导致随访图像无法得到注释，需要自动化方法来识别病变增长轨迹和预测治疗效果。

Method: 建立了一个自动化数据处理流水线，收集了177名患者的896个脑转移瘤症的纵向MRI数据。使用数据驱动聚类方法识别特征性轨迹，并采用梯度提升和图机器学习模型预测12个月的病变响应。

Result: 聚类分析识别出5种主要增长轨迹，每种轨迹对应不同的最终响应类别。使用治疗前和第一次随访MRI数据，梯度提升模型预测的AUC达到0.90（CI95%=0.88-0.92）；图机器学习模型也获得0.88的AUC值（CI95%=0.86-0.90）。

Conclusion: 该研究为脑转移瘤症的纵向MRI响应评估和预测提供了自动化和高精度的方法。提出的流水线有助于可扩展的数据汇总，为优化个性化谨疗的临床决策支持系统奠定了基础。
...
TL;DR: 该论文针对3D高斯泼溅在有限视角采集的大场景中的3DoF+应用场景，提出了基于球坐标的新量化方案来解决坐标量化问题，通过分析位置误差对投影误差的影响，证明了投影误差与投影点距离平方成反比的关系。


<details>
  <summary>Details</summary>
Motivation: 在大场景中，输入视角通常来自空间有限区域，重建主要适用于相同区域的新视角（3DoF+）。需要解决坐标量化问题，研究位置误差对投影误差的影响。

Method: 分析位置误差对投影误差的影响，证明投影误差与投影点距离平方成反比。基于此提出基于球坐标的新量化方案。

Result: 在著名的Garden场景上展示了所提方法的率失真性能。

Conclusion: 提出的球坐标量化方案能够有效处理3D高斯泼溅在3DoF+应用场景中的坐标量化问题，改善率失真性能。
...
### [112] [Improved Classification of Nitrogen Stress Severity in Plants Under Combined Stress Conditions Using Spatio-Temporal Deep Learning Framework](https://arxiv.org/abs/2509.06625)
*Aswini Kumar Patra*

Main category: cs.CV

TL;DR: 使用多模态影像和CNN-LSTM混合深度学习框架，在水分压力和杂草竞争复合压力环境中准确分类氧缠素压力严重程度，达到98%准确率


<details>
  <summary>Details</summary>
Motivation: 植物在自然生境中通常面临多种相互作用的生物和非生物急逼，氧缠素缺乏在与干旱和杂草竞争结合时更加关键，早期检测至关重要

Method: 提出了一种新的深度学习框架，使用RGB、多谱和两种红外波长四种成像模态，通过时间序列数据记录植物健康状况，采用CNN提取空间特征和LSTM捕捉时间依赖关系

Result: CNN-LSTM流水线达到了98%的突出准确率，显著超过了仅使用空间CNN模型的80.45%和其他机器学习方法的76%准确率

Conclusion: 该方法能够有效捕捉氧缠素缺乏、水分压力和杂草压力之间细微而复杂的相互作用，为及时和主动识别氧缠素压力严重程度提供了有力工具
...
### [113] [VQualA 2025 Challenge on Image Super-Resolution Generated Content Quality Assessment: Methods and Results](https://arxiv.org/abs/2509.06413)
...
<details>
  <summary>Details</summary>
Motivation: 现有超分辨率图像质量评估数据集未能充分涵盖最新生成式方法（如GAN和扩散模型）产生的SR图像，需要专门的数据集和挑战来评估这些新技术带来的独特伪影和感知质量。

Method: 构建ISRGen-QA数据集，重点关注生成式超分辨率方法产生的图像，组织挑战赛吸引参与者提交解决方案，最终有4个团队提交有效方案进行最终测试。

Result: 108名参与者注册，4个团队提交了有效解决方案，这些提交在ISRGen-QA数据集上表现出最先进的性能。

Conclusion: ISRGC-Q挑战赛成功建立了针对生成式超分辨率方法的质量评估基准，推动了该领域的发展，相关项目已公开可用。
...
### [114] [BioLite U-Net: Edge-Deployable Semantic Segmentation for In Situ Bioprinting Monitoring](https://arxiv.org/abs/2509.06690)
*Usman Haider,Lukasz Szemet,Daniel Kelly,Vasileios Sergis,Andrew C. Daly,Karl Mason*

Main category: cs.CV

TL;DR: 轻量级语义分割框架BioLite U-Net，专为生物打印实时监控设计，在Raspberry Pi 4B上达到92.85% mIoU和96.17% Dice分数，模型大小减少1300倍


<details>
  <summary>Details</summary>
Motivation: 解决生物打印过程中因图像数据有限和嵌入式硬件约束而导致的打印结构保真度和一致性问题，需要实时语义分割来区分噼嘴、生物墨水和背景

Method: 提出BioLite U-Net模型，采用深度分离卷积来大幅降低计算负荷，构建了787张手动标注的RGB图像数据集，包含噼嘴、生物墨水和背景三个类别

Result: 在Raspberry Pi 4B上达到92.85% mIoU和96.17% Dice分数，每帧推理时间335ms，模型大小比MobileNetV2-DeepLabV3+小1300倍，超过了MobileNet基线模型的性能

Conclusion: BioLite U-Net在分割准确性、效率和部署性方面完美平衡，极适合集成到智能闭环生物打印系统中实现实时监控
...
### [115] [MRI-Based Brain Tumor Detection through an Explainable EfficientNetV2 and MLP-Mixer-Attention Architecture](https://arxiv.org/abs/2509.06713)
*Mustafa Yurdakul,Şakir Taşdemir*

Main category: cs.CV

TL;DR: 通过结合EfficientNetV2和注意力基于MLP-Mixer的深度学习模型，实现了高准确率和可解释性的脑部脾瘤分类，达到99.50%的准确率。


<details>
  <summary>Details</summary>
Motivation: 脑部脾瘤残疾率高，需要早期诊断。MRI图像诊断需要专业知识且容易出错，因此需要自动化诊断系统。

Method: 使用Figshare数据集的3,064张T1加强MRI图像，首先评估五个CNN架构选择EfficientNetV2作为基础，然后集成注意力基于MLP-Mixer架构来提升分类性能。使用五折交叉验证和Grad-CAM可视化进行评估和解释。

Result: 模型在脑部脾瘤分类中达到了99.50%的准确率，99.47%的精确度，99.52%的召回率和99.49%的F1分数，超过了现有文献中的方法。Grad-CAM显示模型能够有效关注MRI图像中的相关区域。

Conclusion: 通过结合EfficientNetV2和注意力基于MLP-Mixer架构，开发了一个健壮的深度学习模型，为临床决策支持系统提供了高准确率和可解释性的脑部脾瘤分类方案。
...
TL;DR: Phantom-Insight是一种基于SAM和MLLM的视频伪装目标检测新方法，通过动态前景视觉令牌评分和提示网络增强SAM模型对细微纹理的适应性，采用解耦的前景-背景学习策略提高目标与背景的可分离性，在MoCA-Mask数据集上达到最先进性能。
...
TL;DR: 香港大学研究人员提出了一种基于Grounding DINO视觉-语言模型的零样本牛哺棉检测框架，无需注释数据即可实现高精度的牛哺棉检测。


<details>
  <summary>Details</summary>
Motivation: 传统的监督学习方法需要大量注释数据且对新牛种性能下降，存在扩展性和适配性问题。

Method: 利用Grounding DINO视觉-语言模型，通过自然语言提示指导检测，无需任务特定训练或注释数据。

Result: 在mAP@0.5指标上达到76.8%的精度，显示出无需注释数据的优秀性能。

Conclusion: 该框架为牛哺棉检测提供了首个实用化、产业化且无注释的解决方案，具有更好的适配性和部署便利性。
...
TL;DR: 通过感知导向的双向注意力网络(PBAN)来评估超分辨率图像质量，结合人视觉特性和双向注意机制，在多个数据集上超越现有最佳方法


<details>
  <summary>Details</summary>
Motivation: 当前完全参考(FR)图像质量评估指标在比较和评估不同超分辨率算法时存在限制，需要更有效的质量评估方法

Method: 构建三模块结构：图像编码器模块、感知导向双向注意(PBA)模块、质量预测模块。采用双向注意机制构建视觉注意，组合多尺度可变形卷积和子信息激发卷积来适应性感知形变和指导视觉感知

Result: 广泛实验证明PBAN在多个数据集上表现超越了现有的最佳质量评估方法

Conclusion: 该方法通过模拟人视觉系统特性和创新的注意机制，为超分辨率图像质量评估提供了有效的解决方案，具有良好的性能表现
...
TL;DR: 跨源点云注册新方法，通过构建最大实际数据集Cross3DReg和重叠区域预测框架，结合视觉-几何注意力匹配，显著提升了注册精度和稳健性


<details>
  <summary>Details</summary>
Motivation: 解决跨源点云注册中缺乏大规模实际数据集训练深度模型的问题，以及不同传感器导致的点云特征提取和匹配困难

Method: 构建Cross3DReg数据集，设计基于重叠区域的跨源注册框架，利用未对齐图像预测重叠区域，筛除无关点，提出视视-几何注意力导向的匹配模块融合图像和几何信息

Result: 方法实现了最先进的注册性能，相对旋转误差(RRE)和相对平移误差(RTE)分别减少63.2%和40.2%，注册回召率(RR)提高5.4%

Conclusion: 该研究为跨源点云注册领域提供了重要的数据集资源和有效的方法框架，通过重叠区域预测和多源信息融合有效提升了注册准确性和稳健性
...
### [120] [Automated Radiographic Total Sharp Score (ARTSS) in Rheumatoid Arthritis: A Solution to Reduce Inter-Intra Reader Variation and Enhancing Clinical Practice](https://arxiv.org/abs/2509.06854)
*Hajar Moradmand,Lei Ren*

Main category: cs.CV

TL;DR: 本研究开发了一个基于深度学习的自动RA评分框架ARTSS，通过多阶段处理全手X光图像来预测Total Sharp Score，有效解决了关节消失和图像序列长度变化的问题，显著降低了评分变异性和时间消耗。


<details>
  <summary>Details</summary>
Motivation: 手动评估类风湿关节炎严重程度的Total Sharp Score耗时且主观性强，存在观察者间和观察者内变异性大的问题，需要自动化解决方案来提高评分的一致性和效率。

Method: 采用四阶段深度学习框架：1) ResNet50图像预处理和重定向；2) UNet.3手部分割；3) YOLOv7关节识别；4) 多种模型(VGG16/19、ResNet50、DenseNet201、EfficientNetB0、ViT)进行TSS预测，使用3折交叉验证和外部测试集评估。

Result: 关节识别准确率达到99%，最佳模型ViT在TSS预测上获得0.87的低Huber损失，在291名未见过的测试对象上表现良好，显著降低了评分变异性。

Conclusion: ARTSS框架成功实现了RA评分的自动化，能够处理关节消失和变长序列的挑战，具有节省时间、减少评分变异性、提高放射科医生准确性等临床价值。
...
### [121] [IGAff: Benchmarking Adversarial Iterative and Genetic Affine Algorithms on Deep Neural Networks](https://arxiv.org/abs/2509.06459)
*Sebastian-Vasile Echim,Andrei-Alexandru Preda,Dumitru-Clementin Cercel,Florin Pop*

Main category: cs.CV

TL;DR: 本文提出两种基于仿射变换和遗传算法的黑盒对抗攻击方法ATA和AGA，在多个数据集和网络架构上取得了比现有方法更好的攻击效果，准确率提升最高达8.82%。


<details>
  <summary>Details</summary>
Motivation: 深度神经网络虽然性能优异但存在难以理解和脆弱性的问题，特别是在黑盒场景下对抗攻击更具挑战性，需要开发有效的攻击方法来揭示模型弱点。

Method: 提出了两种新颖的黑盒迭代对抗算法：1）ATA算法使用随机仿射变换最大化攻击评分函数；2）AGA算法结合随机噪声和仿射变换的遗传算法。在Tiny ImageNet等数据集上对ResNet-18等架构进行测试。

Result: 实验结果表明，提出的算法在图像分类任务上优于Pixle和Square Attack等类似方法，准确率提升最高达8.82%，并在全局和定向攻击配置、数据增强等场景下验证了对抗鲁棒性。

Conclusion: 研究为成功的对抗防御和攻击提供了重要见解，通过算法参数变化展示了对抗鲁棒性，所提出的黑盒对抗攻击方法在揭示深度神经网络脆弱性方面具有显著效果。
...
### [122] [Barlow-Swin: Toward a novel siamese-based segmentation architecture using Swin-Transformers](https://arxiv.org/abs/2509.06885)
...
<details>
  <summary>Details</summary>
Motivation: 传统卷积网络感知野有限，Transformer模型过于深层和计算费用高，不适合实时临床应用。需要一种轻量级但保持高精度的医学图像分割方案。

Method: 设计结合Swin Transformer编码器和U-Net解码器的端到端轻量级网络架构，通过skip路径保住空间细节。采用Barlow Twins自监督预训练编码器，然后精细调整整体模型。

Result: 在二值分割平台任务上达到竞争性的精度，同时参数数量大幅减少，推理速度显著提升。

Conclusion: 该模型为实时和资源有限的临床环境提供了一种实用的替代方案，在保持高精度的同时实现了高效率的医学图像分割。
...
### [123] [A Statistical 3D Stomach Shape Model for Anatomical Analysis](https://arxiv.org/abs/2509.06464)
*Erez Posner,Ore Shtalrid,Oded Erell,Daniel Noy,Moshe Bouhnik*

Main category: cs.CV

TL;DR: 本文提出了一种创建综合性3D胃部统计形状模型的新方法，结合了合成数据生成、参数化建模和真实CT扫描验证，为医疗应用提供了首个胃部统计形状模型。


<details>
  <summary>Details</summary>
Motivation: 现有的人体内部器官3D模型受限于数据获取困难和方法论挑战，特别是对于胃部这种复杂器官的详细建模。需要一种能够抓取自然解剖变异性的参数化模型来支持医学研究、诊断和手术规划。

Method: 设计了一个新的管道来生成合成的3D胃部模型，基于胃部形状变异性的已知研究。使用这个管道构建了合成胃部数据集，并在此基础上开发了3D统计形状模型。通过半监督对准过程使用公开数据集中的CT网格进行精炼，提升模型的普适性。

Result: 在真实胃部CT扫描的指定测试集上评估模型，证明了其健壮的普适性和拟合精度。该模型能够在低维形状空间中抗据自然解剖变异性。

Conclusion: 这项工作介绍了首个胃部的3D统计形状模型，在手术模拟、术前规划、医学教育和计算建模等领域具有应用价值。通过结合合成数据生成、参数化建模和真实世界验证，该方法代表了器官建模领域的重要进展，为个性化健康解决方案开启了新可能性。
...
### [124] [Does DINOv3 Set a New Medical Vision Standard?](https://arxiv.org/abs/2509.06467)
...
TL;DR: DINOv3作为基于自然图像训练的视觉基础模型，在医学图像任务中表现出艰强的性能，超越了部分医学特定模型，但在需深度领域专业化的场景中存在限制


<details>
  <summary>Details</summary>
Motivation: 探索前沿视觉基础模型在专业领域（如医学图像）的效果转移能力，评估DINOv3是否能够直接作为医学视觉任务的统一编码器而无需领域特定预训练

Method: 在广泛的医学图像模态上对DINOv3进行基准测试，包括2D/3D分类和分割任务，系统分析模型规模和输入分辨率的可扩展性

Result: DINOv3在多个任务上表现出艰强性能，甚至超过医学特定基础模型，但在需深度领域专业化的场景中性能下降，且在医学领域不遵循扩展定律

Conclusion: DINOv3可作为医学视觉任务的强大基准，其强大的视觉特征可作为多个复杂医学任务的稳健先验知识，为未来研究开启新方向
...
### [125] [H$_{2}$OT: Hierarchical Hourglass Tokenizer for Efficient Video Pose Transformers](https://arxiv.org/abs/2509.06956)
*Wenhao Li,Mengyuan Liu,Hong Liu,Pichao Wang,Shijian Lu,Nicu Sebe*

Main category: cs.CV

TL;DR: 通过分层的插拔式剪枝恢复框架H2OT，在保持视频3D人体姿估计精度的同时大幅提升Transformer模型的计算效率


<details>
  <summary>Details</summary>
Motivation: 解决视频姿态Transformer模型计算成本高、在资源受限设备上实用性差的问题

Method: 提出分层沙漏模型H2OT，包含令牌剪枝模块(TPM)和令牌恢复模块(TRM)，逐步剪枝冗余帧的姿态令牌并最终恢复完整序列

Result: 在多个标准数据集上验证了方法的有效性和效率，可以结合不同的VPT模型和策略

Conclusion: 维护完整姿态序列并非必要，选择少量代表性帧的姿态令牌即可同时实现高效率和高精度估计
...
TL;DR: FSG-Net是一个频率-空间协同门控网络，通过频域分析和空间注意力机制有效区分真实变化与伪变化，在多个变化检测基准上达到最先进性能。
...
Method: 提出FSG-Net框架，包含三个核心模块：1）频域中的差异感知小波交互模块（DAWIM）自适应处理不同频率分量以抑制伪变化；2）空间域中的协同时空注意力模块（STSAM）增强真实变化区域的显著性；3）轻量级门控融合单元（LGFU）利用高层语义选择性整合浅层细节特征。
...
TL;DR: 这篇论文提出了一种基于"前后监督"的弱监督分割方法，用于废弃物分类中自动识别不应存在物品，避免了大量手动标注的需求。


<details>
  <summary>Details</summary>
Motivation: 工业质量控制中人工分类废弃物依然靠人工，而全监督方法需要大量标注工作。论文提出利用操作员删除动作自然提供的隐式监督，实现弱监督分割。

Method: 提出Before-After Supervision概念，利用操作员删除前后的图像差异训练分割网络，并发布WS²多视角数据集，包含11,000张高清视频帧。

Result: 构建了稳健的端到端流水线，并在WS²数据集上对多个独刻弱监督分割方法进行了性能测试和对比。

Conclusion: 该方法为工业质量控制领域提供了一种可行的弱监督解决方案，能够减少对大量手动标注的依赖，具有实际应用价值。
...
Result: 在标准基准测试中，TIDE在生成主体忠实输出同时保持指令遵从性方面表现出优越性能，在多个定量指标上优于基线方法。

Conclusion: TIDE框架成功解决了主题驱动图像生成中的关键挑战，展示了在结构条件生成、图像到图像生成和文本图像插值等多种任务中的通用性。
...
TL;DR: 提出了一种结合深度学习和放射组学特征的混合框架，用于从纵向MRI扫描中自动分类胶质母细胞瘤治疗反应，在4类响应预测任务中达到0.81的ROC AUC和0.50的Macro F1分数。
...
Result: 在4类治疗反应预测（完全缓解、部分缓解、疾病稳定、疾病进展）中取得了0.81的平均ROC AUC和0.50的Macro F1分数。

Conclusion: 深度学习图像表示与领域特异性放射组学特征的协同结合，为神经肿瘤学中的自动化治疗反应评估提供了强大有效的解决方案。
...
TL;DR: 本文介绍了一种新的JSON格式的FMOX数据集标注文件，用于快速移动小物体跟踪。它扩充了现有FMO数据集，包含物体大小信息，并用于测试EfficientTAM模型的性能，结果显示其性能与专门为FMO设计的方法相当。


<details>
  <summary>Details</summary>
Motivation: 快速移动小物体(FMO)跟踪在计算机视觉中仍面临挑战，需要更好的数据集标注和评估方法来推动该领域的发展。

Method: 创建了FMOX JSON格式的元数据文件，扩充现有FMO数据集的真实标注信息，包含物体大小信息。使用这个新的数据集来测试EfficientTAM基础跟踪模型，并使用轨迹交并比(TIoU)指标进行性能评估。

Result: EfficientTAM模型在FMOX数据集上的表现与专门为FMO设计的方法相当，证明了该基础模型的有效性。

Conclusion: FMOX数据集标注文件为FMO跟踪领域提供了丰富的真实标注信息，支持了更好的模型评估和发展。该工作开源分享，促进了FMO处理相关机器学习流水线的发展。
...
TL;DR: 提出了一种针对场景级草图图像检索的训练方法，通过专门的预训练、编码器架构和损失函数设计，在不增加模型复杂度的情况下实现了最先进的性能，特别关注草图固有的模糊性和噪声问题。


<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注检索模型的架构改进，而忽略了真实世界草图中固有的模糊性和噪声问题。本文强调需要专门针对草图变异性的鲁棒训练目标。

Method: 采用适当的预训练策略、编码器架构和损失函数组合，设计了一种能够有效处理草图变异性的训练方法，不引入额外复杂度。

Result: 在具有挑战性的FS-COCO和广泛使用的SketchyCOCO数据集上进行大量实验，证明了方法的有效性，达到了最先进的性能水平。

Conclusion: 研究强调了训练设计在跨模态检索任务中的关键作用，同时指出需要改进场景级SBIR的评估场景，为未来研究提供了重要方向。
...
TL;DR: 这篇论文提出了RARL方法来解决增量开集识别(IOSR)中的问题，通过角度空间对齐未知表征和虚拟类交互训练，有效缓解了知识更新过程中的表征漫洒和类间混淆问题。


<details>
  <summary>Details</summary>
Motivation: 现有开集识别方法主要适用于静态场景，而真实场景中模型需要从连续数据流中增量识别新的未知类，这导致了严重的类间混淆问题。

Method: 提出保持性角度表征学习(RARL)：1)在均匀紧凑框构建的角度空间中，将未知表征对齐到非活跃原型；2)采用虚拟-内在交互(VII)训练策略，通过边界近伺虚拟类来压缩已知表征；3)分层校正策略精炼决策边界。

Result: 在CIFAR100和TinyImageNet数据集上进行了全面评估，建立了IOSR的新标准，在各种任务设置下都达到了最先进的性能。

Conclusion: RARL方法有效解决了增量开集识别中的表征漫洒和类间混淆问题，为连续学习环境下的开集识别提供了有效的解决方案。
...
Method: 采用机器学习方法学习一个降维排序，该排序近似于从一组向量排序中推导出的Condorcet排序，受投票问题启发，将不同排序视为投票者。

Result: 初步计算实验证实了学习降维映射在定义彩色图像向量值形态学算子方面的有效性。

Conclusion: 该方法为解决向量值数学形态学中的排序问题提供了一种有效的机器学习解决方案，特别适用于彩色图像处理。
...
TL;DR: CausNVS是一个自回归的多视角扩散模型，通过因果掩码和逐帧噪声训练，支持任意输入输出视角配置和顺序生成，解决了传统非自回归方法的固定视角限制和推理速度慢的问题。


<details>
  <summary>Details</summary>
Motivation: 现有的多视角扩散模型大多采用非自回归方法，限制了它们在3D世界建模中的应用，因为只能支持固定数量的视角，并且由于需要同时去噪所有帧而导致推理速度慢。
...
TL;DR: 开发基于机器学习的对象识别模型，用于自动检测野生动物制品图像，特别是象牙、穿山甲鳞片和虎制品，准确率达84.2%，并开发了手机应用实现实时检测。


<details>
  <summary>Details</summary>
Motivation: 数字市场和社交媒体中不可持续的野生动物贸易日益普遍，对生物多样性构成重大威胁。由于数字内容量巨大，需要自动化方法来检测野生动物贸易列表，特别是自动识别象牙等野生动物制品。

Method: 使用机器学习对象识别模型，训练数据来自被确认为非法销售或被当局没收的大象、穿山甲和老虎制品图像。研究了不同训练策略和损失函数的组合，为每个物种单独训练模型，同时开发了一个识别所有三个物种制品的单一模型。

Result: 最佳模型整体准确率为84.2%，其中大象制品检测准确率71.1%，穿山甲90.2%，老虎93.5%。开发的智能手机应用整体准确率达91.3%，可实时拍摄图像识别潜在违禁产品。

Conclusion: 该方法不仅适用于网络贸易监控，还可用于实体市场等场景的野生动物贸易监测，为政府当局和执法机构提供了实用的工具。
...
TL;DR: 提出了一种新型混合Swin注意力网络(HSANet)，用于低剂量CT/PET图像去噪，通过高效全局注意力模块和混合上采样模块提升性能，在保持轻量级模型的同时实现优于现有方法的去噪效果。
...
Result: 实验结果表明HSANet在去噪性能上优于现有方法，同时保持轻量级模型大小，适合标准内存GPU部署。

Conclusion: 该方法具有很高的实际临床应用价值，能够有效提升低剂量医学图像质量，同时保持计算效率。
...
TL;DR: 本文研究了位置元数据在海底图像自监督学习中的作用，在多个SSL框架和数据集上评估了位置正则化的效果，发现其能持续提升分类性能。


<details>
  <summary>Details</summary>
Motivation: 高速解释机器人收集的海底视觉图像可以提高海洋监测和探索的效率。虽然最近的研究已经建议位置元数据可以增强自监督学习，但其在不同SSL策略、模型和海底图像数据集上的好处还未充分探索。

Method: 评估了位置基于正则化对六个最先进的SSL框架的影响，包括卷积神经网络(CNN)和视觉Transformer(ViT)模型，具有不同的潜在空间维度。在三个多样化的海底图像数据集上进行了评估。

Result: 位置正则化持续改善了下游分类性能，CNN模型平均F1分数提升4.9±4.0%，ViT模型提升6.3±8.9%。位置正则化的SSL在低维潜在表征下对CNN的改善效果更为显著，而高维ViT模型显示出强大的汇总能力。

Conclusion: 研究结果突出了位置元数据对SSL正则化的价值，尤其是在使用低维潜在表征时，并证明了高维ViT模型在海底图像分析中的强大汇总能力。
...
TL;DR: 基于代表性样本取样的在线聚类框架，能够在海底自主漏洋器上实时处理连续图像数据流，并保持计算效率和稳定性


<details>
  <summary>Details</summary>
Motivation: 解决长时间海底自主漏洋器在实时图像解释方面的需求，适应环境和操作条件对图像外观的强影响，免去对完整数据集和人工标注的依赖

Method: 提出一种在线聚类框架(OCF)，通过识别和维护一组代表性样本来捐描演化的特征分布，支持动态聚类合并和分割，无需重新处理全部图像历史

Result: 在三个多样化海底图像数据集上评估，OCF在所有比较在线聚类方法中获得最高平均F1分数0.68，标准差3%，显示了其优秀的聚类能力和对调查轨迹变化的稳健性，同时保持低且有界的计算时间

Conclusion: 该框架特性有助于生成调查数据摘要和支持长期持久自主海洋探索中的信息化路径规划
...
<details>
  <summary>Details</summary>
Motivation: 传统高斯溅射方法需要准确的深度信息（RGB-D/立体相机），但深度感知范围有限，难以处理大场景。单目图像缺乏深度信息，而现有的大基础模型深度估计存在跨帧不一致、远距离不准确和纹理欺骗等问题。

Method: 利用视觉惯性SfM的准确稀疏深度来优化大基础模型的密集但粗糙深度。提出对象分割深度传播算法处理结构化对象的深度渲染，并开发动态深度细化模块处理动态对象的SfM深度问题。

Result: 在公开和定制数据集上的实验表明，VIM-GS在大场景中具有优越的渲染质量。

Conclusion: VIM-GS成功解决了大场景单目图像新视角合成的深度估计挑战，通过结合稀疏和密集深度信息实现了高质量的高斯溅射渲染。
...
<details>
  <summary>Details</summary>
Motivation: 现有工业异常合成方法存在两个关键问题：(i)合成异常缺乏精细纹理细节且与背景对齐不精确；(ii)难以生成细粒度像素级异常。需要一种能够产生高质量异常数据的方法来提升下游异常分割性能。
...
TL;DR: Cortex Synth是一个端到端可微分框架，可从单张2D图像联合合成3D骨骼几何和拓扑结构，在ShapeNet数据集上相比先前方法在MPJPE指标提升18.7%，图编辑距离提升27.3%，拓扑错误减少42%。


<details>
  <summary>Details</summary>
Motivation: 从单张2D图像重建3D骨骼结构在机器人操作、医学成像和角色绑定等领域有重要应用价值，但现有方法在几何精度和拓扑结构准确性方面存在局限。

Method: 提出包含四个协同模块的端到端框架：伪3D点云生成器、增强PointNet编码器、骨骼坐标解码器和可微分图构建网络(DGCN)，采用分层图注意力机制、可微分谱拓扑优化和对抗几何一致性训练三大创新技术。

Result: 在ShapeNet数据集上取得最先进结果，MPJPE提升18.7%，图编辑距离提升27.3%，拓扑错误减少42%。

Conclusion: 该框架的端到端可微分特性使其在机器人操作、医学成像和自动化角色绑定等领域具有广泛应用前景，显著提升了3D骨骼重建的几何精度和拓扑准确性。
...
<details>
  <summary>Details</summary>
Motivation: 解决现有轨迹引导I2V生成方法需要大量计算资源进行微调，以及零样本方法因忽略3D视角和潜在空间操作与噪声预测不对齐而导致运动不真实的问题。
...
Result: Zo3T在轨迹控制的I2V生成中显著提升了3D真实感和运动准确性，性能优于现有的基于训练和零样本方法。
...
Conclusion: Co-Seg框架通过协同组织分割和细胞核分割任务，有效提升了组织病理学图像分析性能，证明了任务间协同学习的重要性。
...
<details>
  <summary>Details</summary>
Motivation: 传统感知方法在森林环境中存在延迟、深度分辨率低、依赖光照等问题，无人机在密林中安全导航和精确数据收集面临挑战

Method: 使用结构光进行深度重建，通过调制投射结构光的波长在650-850nm范围内捕获光谱信息，开发可穿梭的RGB版本进行实际森林数据收集

Result: 深度估计RMSE比商业深度传感器提高60%，光谱准确性与参考光谱仪相似，深度数据将材料区分准确性提升超30%

Conclusion: 该系统在实验室和真实雨林环境中都表现出艺艺性能，为轻量化、集成化的无人机感知和数据收集提供了新途径
...
TL;DR: 基于转移学习的ResNet50-EfficientNet-RegNet深度学习模型在坑洞检测中达到了高准确率和计算效率，在扩展测试集上达到了98.89%的分类准确率


<details>
  <summary>Details</summary>
Motivation: 通过图像特征提取实现坑洞状况的自动识别，这对于社会发展具有重要意义

Method: 基于标准化、归一化、数据增广等预处理技术，构建了ResNet50-EfficientNet-RegNet转移学习模型，并与随机森林、MLP、SVM、LightGBM等模型进行性能对比

Result: 转移学习模型在90个测试样本上达到97.78%准确率，在900个扩展测试样本上达到98.89%准确率，识别速度和准确性都超过其他模型

Conclusion: 通过参数选择和模型优化，构建的转移学习模型在坑洞检测任务中表现出优异的性能，为自动化路面检测提供了有效解决方案
...
Result: 实验结果表明Raw2Event能够生成与真实事件相机非常相似的事件流，同时受益于更高分辨率和自动对焦能力，系统还支持用户直观的参数调优，并在树莓派上实现实时操作。
...
<details>
  <summary>Details</summary>
Motivation: 线上谜图中的黑色幽默依赖隐式、敏感和文化上下文的索引，目前缺乏多模态内容中检测黑色幽默的资源和方法。

Method: 构建了4,379个Reddit谜图数据集，提出了一种推理增强框架，先用大型视觉-语言模型生成结构化解释，通过角色转换自我循环精炼解释，然后使用三流交叉推理网络融合文本、图像和推理特征进行分类。

Result: 实验结果显示，该方法在黑色幽默检测、目标识别和强度预测三个任务上都超过了强基线方法。

Conclusion: 该研究为多模态幽默理解和内容审核提供了有价值的资源和方法，数据集、注释和代码已开源以促进进一步研究。
...
<details>
  <summary>Details</summary>
Motivation: 为了解决真实激光雷达数据采集成本高、标注困难的问题，通过数字孪生技术创建与真实数据集高度对齐的合成数据集，以增强现有基准数据集并提高深度学习模型的训练效果。

Method: 使用模拟激光雷达传感器在精确建模的数字孪生环境中合成数据，基于实际位置的几何结构、道路对齐、车道拓扑和车辆运动模式进行建模。

Result: 合成数据集与真实数据在统计和结构相似性分析中表现出高度一致性，仅使用合成数据训练的3D目标检测模型在真实数据上测试时性能优于使用真实数据训练的模型。

Conclusion: UrbanTwin数据集是首个可以替代真实激光雷达感知任务的合成数据集，能够有效增加样本量和场景多样性，数字孪生还可用于自定义场景测试。
...
Result: 在包含近370万个模型的新建数据集上训练，比较显示该方法在任意复杂对象上实现精确分割结果和强鲁棒性，达到最先进性能

Conclusion: P3-SAM为3D对象部件分割提供了完全自动化的解决方案，在复杂场景下表现出优异的性能和鲁棒性
...
TL;DR: AIM 2025高帧率非均匀运动去模糊挑战赛综述，评估了9个团队的最先进解决方案，展示了该领域的显著进展，并使用了包含挑战性运动模式的新数据集MIORe。


<details>
  <summary>Details</summary>
Motivation: 识别能够在多样化挑战性条件下通过学习复杂运动类型的代表性视觉线索来生成更清晰、视觉吸引力更强图像的有效网络。

Method: 通过竞赛形式，68名参与者注册，9个团队提交有效方案，使用新颖的MIORe数据集进行评估，该数据集包含挑战性的运动模式样本。

Result: 挑战赛展示了高帧率单图像运动去模糊领域的最先进进展，证明了该技术的显著进步。

Conclusion: 该挑战赛成功推动了非均匀运动去模糊技术的发展，MIORe数据集为未来研究提供了有价值的基准测试资源。
...
TL;DR: 通过3D生成技术建立可扩展的真实2模拟2真实系统，解决自动驾驶中稀缺场景数据生成的挑战


<details>
  <summary>Details</summary>
Motivation: 现有CG基于方法缺乏多样性且难扩展，学习基于方法受限于特定对象类别且需要大量多传感器数据

Method: 采用3D生成技术自动化资产挖掘、生成和稀有案例数据合成

Result: 提出了一种可扩展的真实2模拟2真实系统

Conclusion: 该系统能够有效解决自动驾驶中稀有场景数据生成的问题
...
TL;DR: 提出了MIORe和VAR-MIORe两个新颖的多任务数据集，通过高帧率采集和自适应帧平均技术生成一致的运动模糊，为视频恢复任务提供高质量基准


<details>
  <summary>Details</summary>
Motivation: 解决当前运动恢复基准测试的关键局限性，包括缺乏复杂运动场景、多主体交互和深度相关模糊效应的全面数据

Method: 使用1000FPS高帧率采集和专业级光学设备，基于计算的光流指标自适应平均帧来生成一致运动模糊，同时保留清晰输入用于视频帧插值和光流估计

Result: 创建了涵盖从最小到极端运动幅度的可变范围数据集，提供高分辨率可扩展的真实标注，在受控和不利条件下挑战现有算法

Conclusion: 这些数据集为各种图像和视频恢复任务的下一代研究铺平了道路，建立了首个提供明确运动幅度控制的基准
...
TL;DR: UMO是一个统一的多身份优化框架，通过多对多匹配范式和强化学习解决多参考图像下的身份一致性和身份混淆问题，显著提升了图像定制方法中的身份保真度。
...
Conclusion: UMO有效解决了多身份图像定制中的身份保真度和混淆问题，为图像定制方法提供了可扩展的身份一致性解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 传统视频年龄分级方法需要大量标注数据、泛化能力差、特征学习效率低，无法满足流媒体平台对MPAA分级标准的自动化需求

Method: 采用三种对比学习框架：实例判别、上下文对比学习和多视图对比学习。使用LRCN（CNN+LSTM）骨干网络结合Bahdanau注意力机制，测试了NT-Xent、NT-logistic和Margin Triplet等对比损失函数

Result: 在Contextual Contrastive Learning框架下取得最佳性能：88%准确率和0.8815 F1分数，能够有效区分PG-13和R级等边界案例

Conclusion: 提出的混合架构在视频年龄分级任务中表现优异，已部署为实时分类的Web应用，为流媒体平台内容合规提供了高效解决方案
...
TL;DR: Curia是一个基于大规模真实世界医学影像数据训练的基础模型，在19个放射学任务上表现优异，达到或超过放射科医生和其他基础模型的性能，并展现出跨模态和低数据场景的临床显著能力。
...
Method: 使用一家大型医院数年间的全部横断面成像输出数据（150,000次检查，130TB）训练Curia基础模型，并在新构建的19任务外部验证基准上进行评估。
...
TL;DR: 这篇论文研究了如何通过迁移学习调整通用基础模型和整合手术室多模态数据来支持手术数据科学。使用V-JEPA模型作为基础，分析了基于未标注手术视频的微调和多模态数据整合对模型性能的改善效果。


<details>
  <summary>Details</summary>
Motivation: 探索如何利用通用基础模型通过迁移学习和整合手术室多模态数据来支持手术数据科学应用，提高手术决策支持系统的性能。

Method: 使用V-JEPA作为单模态基础模型，在未标注的手术视频数据上进行微调，并通过训练独立编码器整合手术室的其他时间分辨数据流构建多模态模型。在内部肝脏手术数据集上预测住院时长和手术后并发症，在公开HeiCo数据集上识别手术阶段。

Result: 实验结果显示：基于领域特定数据的微调能够提高模型性能；在内部数据上，整合额外的时间分辨数据同样有益；在HeiCo数据集上，单模态基线模型的准确率已经与EndoVis2017挑战赛最佳性能相当，微调后还能进一步提高。

Conclusion: 研究证明手术数据科学可以利用公开的通用基础模型，领域适配和整合适当的补充数据流具有很大潜力。研究人员已开源代码和模型权重以支持进一步研究。
...
TL;DR: 本文研究了交通标志分类器对对抗性攻击的脆弱性，使用LISA交通标志数据集训练CNN模型，测试FGSM和PGD攻击效果，发现扰动幅度增加会导致分类准确率急剧下降。


<details>
  <summary>Details</summary>
Motivation: 对抗性攻击对机器学习模型构成严重威胁，但先前研究主要集中在MNIST等简单数据集上，本文旨在探究真实场景中交通标志分类器的安全性问题。

Method: 使用LISA交通标志数据集训练卷积神经网络分类47种交通标志，评估模型对Fast Gradient Sign Method (FGSM) 和 Projected Gradient Descent (PGD) 攻击的鲁棒性。

Result: 随着扰动幅度的增加，分类准确率出现急剧下降，表明模型对对抗样本具有高度敏感性。

Conclusion: 该研究为未来开发针对真实世界交通标志识别系统的防御机制奠定了基础，强调了此类系统在对抗性攻击面前的脆弱性。
...
TL;DR: 通过收集动漫风格数据集并对BiRefNet模型进行精细调整，显著提升了动漫图像背景移除的准确性


<details>
  <summary>Details</summary>
Motivation: 当前最优背景移除模型在动漫风格内容上表现异常，特别是处理头发和透明效果等复杂特征时面临挑战

Method: 收集和注释了1,228张高质量动漫图像数据集，对开源BiRefNet模型进行精细调整

Result: 动漫图像背景移除准确度从95.3%提升到99.5%（按像素准确度指标）
...
TL;DR: 提出了一种拓扑自适应变形模型，用于处理包含拓扑伪影的非刚性3D网格匹配问题，能够在ARAP和双射关联约束下对齐形状对，无需数据驱动先验即可处理高度非等距形状和拓扑伪影。
...
Result: 方法适用于高度非等距形状和包含拓扑伪影的形状，包括噪声多视角重建，在3D对齐质量上甚至优于基于大数据集训练的方法。

Conclusion: 该拓扑自适应方法能够有效处理传统假设失效的拓扑配置，为包含拓扑伪影的3D网格匹配提供了有效的解决方案。
...
TL;DR: 这篇论文提出了一种结合选择性GAN去模糊预处理和YOLOv5检测模型的自动车牌识别系统，在实时性和准确性方面取得了显著成效


<details>
  <summary>Details</summary>
Motivation: 自动车牌识别系统在智慧城市中至关重要，但面临高变化性挑战，特别是模糊车牌的处理问题，需要更有效的深度学习方案

Method: 采用选择性GAN进行去模糊预处理，结合YOLOv5对象检测架构进行车牌检测和字符分割识别，避免不必要的输入操作

Result: 检测时间仅0.026秒，车牌检测准确率95%，字符识别准确率97%，去模糊GAN预处理使检测准确率提高近40%

Conclusion: 结合YOLOv5和去模糊GAN的综合模型在准确性和实时性方面表现优异，特别适用于模糊场景的箱体输入，适合移动应用部署
...
TL;DR: 提出了一种在非欧几里得球面特征空间中进行相似性学习的方法，用于2D/3D配准，通过黎曼距离近似测地距离，提高配准精度和收敛速度


<details>
  <summary>Details</summary>
Motivation: 现有的欧几里得近似方法扭曲了流形结构并减慢了收敛速度，需要探索非欧几里得球面特征空间来更好地捕捉和拟合复杂的流形结构

Method: 使用CNN-Transformer编码器提取特征嵌入，将其投影到球面空间，在双不变SO(4)空间中使用黎曼距离近似测地距离，推理时使用完全可微的Levenberg-Marquardt优化替代梯度下降

Result: 在真实和合成数据集上的实验显示，在患者特定和患者无关场景中都表现出优越的准确性

Conclusion: 该方法提供了更具表达力和几何一致性的深度相似性度量，增强了区分细微姿态差异的能力，显著提高了2D/3D配准的性能
...
TL;DR: BIR-Adapter是一个低复杂度的盲图像恢复适配器，利用预训练扩散模型的先验知识，无需额外特征提取器训练，通过自注意力扩展和采样引导机制实现高效图像恢复。
...
Method: 从退化图像中通过模型自身提取特征，扩展自注意力机制引入退化特征，采用采样引导机制减少幻觉现象。

Result: 在合成和真实退化场景下，BIR-Adapter达到或优于最先进方法的性能，同时显著降低复杂度，并能集成到其他扩散模型中扩展应用。
...
TL;DR: FoMo4Wheat是首个针对小麦作物的视觉基础模型，通过在ImAg4Wheat数据集（250万张高分辨率图像）上进行自监督预训练，显著提升了田间视觉任务的性能表现。


<details>
  <summary>Details</summary>
Motivation: 传统基于通用领域预训练骨干网络的模型在农业视觉监测中泛化能力不足，因为精细多变的冠层结构与波动的田间条件之间存在复杂交互作用。

Method: 使用自监督学习方法在ImAg4Wheat数据集（全球30个站点10年收集的250万张小麦图像，涵盖2000+基因型和500+环境条件）上预训练小麦专用基础模型。

Result: 在10个田间视觉任务（冠层和器官级别）中，FoMo4Wheat模型一致优于基于通用数据集预训练的最先进模型，且表征具有跨作物和杂草的迁移能力。

Conclusion: 作物专用基础模型对于可靠的田间感知具有重要价值，为实现具有跨物种和跨任务能力的通用作物基础模型指明了方向。
...
TL;DR: 这篇论文系统研究了语音语言模型中离散单元表示的优化策略，探讨了模型架构、数据表示和训练稳健性对持续预训练阶段的影响。


<details>
  <summary>Details</summary>
Motivation: 为了在持续预训练阶段优化语音建模，并探索如何将现有的预训练语言模型适配到语音模态。

Method: 通过实验系统分析语音编码器、聚类粒度、聚类分布和音位对齐等因素，研究不同模型规模下的最优离散化策略。

Result: 实验显示最优离散化策略随模型容量而变化，发现了语言和超语言模式，并高亮了离散化训练与目标应用领域匹配的重要性。

Conclusion: 这项研究为语音语言模型的离散单元表示提供了系统性的优化指南，对于提高语音建模效果具有重要意义。
...
TL;DR: 提出一种基于N-Gram频率张量的新方法来检测大语言模型的幻觉现象，通过张量分解提取特征并训练二元分类器，在HaluEval数据集上取得显著改进。


<details>
  <summary>Details</summary>
Motivation: 大语言模型存在幻觉问题，影响信息的可靠性。现有检测方法如ROUGE、BERTScore等缺乏语义深度，需要更有效的幻觉检测方法。

Method: 构建N-Gram频率张量来描述语义结构，利用张量分解提取奇异值作为特征，训练多层感知机二元分类器进行幻觉检测。

Result: 在HaluEval数据集上表现出艾，显著超越传统基线方法，与最先进的LLM判断方法竞争性能相当。

Conclusion: 该方法通过张量表示提供了更有效的幻觉检测方案，为提高LLM生成内容的可靠性提供了新的视角。
...
TL;DR: SAGE是一个触发引导的动态微调框架，通过在推理时检测失败并动态更新知识，使大语言模型能够持续适应新数据


<details>
  <summary>Details</summary>
Motivation: 解决大语言模型在推理时无法持续适应和学习新数据的局限性

Method: 将复杂推理任务分解为原子子任务，包含三个模块：触发模块检测推理失败、触发缓冲模块聚类异常样本、Lora存储模块动态优化参数更新

Result: SAGE在原子推理子任务上表现出优秀的准确性、鲁棒性和稳定性

Conclusion: 该框架通过推理时的动态知识更新，有效提升了大语言模型的持续学习能力
...
TL;DR: 多机器人辩论在模型能力异质的情况下可能导致性能退步，强大模型也会因谨重而改变正确观点
...
Method: 通过一系列实验，分析不同能力模型在辩论中的交互动态和结果变化

Result: 辩论可能导致准确率随时间下降，模型经常从正确答案转向错误答案，偏向迅速达成一致而不是挑战错误推理

Conclusion: 天真地应用辩论策略可能造成性能退化，需要正确激励和设备来抵御说服力强但错误的推理
...
### [168] [Proof-Carrying Numbers (PCN): A Protocol for Trustworthy Numeric Answers from LLMs via Claim Verification](https://arxiv.org/abs/2509.06902)
*Aivin V. Solatorio*

Main category: cs.CL

TL;DR: Proof-Carrying Numbers (PCN) 是一种展示层协议，通过机械验证确保大语言模型生成数字的保真度，将数字标记为经过验证或未验证状态。


<details>
  <summary>Details</summary>
Motivation: 解决大语言模型在生成数字时可能出现的数值幻觉问题，现有方法无法保证数字保真度，需要一种能够机械验证数字准确性的机制。

Method: PCN协议将数字作为声明绑定令牌发出，验证器根据声明的策略（如精确相等、舍入、别名或容差）检查每个令牌，验证过程在渲染器而非模型中完成。

Result: PCN证明了声音性、诚实令牌下的完备性、故障关闭行为和策略精化下的单调性，实现了轻量级、模型无关的验证方案。
...
### [169] [No Translation Needed: Forecasting Quality from Fertility and Metadata](https://arxiv.org/abs/2509.05425)
*Jessica M. Lundin,Ada Zhang,David Adelani,Cody Carroll*

Main category: cs.CL

TL;DR: 使用语言类型学特征、词汇生成率和基本语言元数据，可以在不运行翻译系统的情况下准确预测GPT-4o翻译质量，在203种语言上达到高准确度。
...
Method: 采用梯度提升模型，使用词汇生成率比例、词汇计数和语言元数据（语言家族、写作系统、地区）作为特征，在FLORES-200测试集上进行预测。

Result: 模型表现优异，XX→英语R²=0.66，英语→XX R²=0.72。特征重要性分析显示，语言类型学因素在英语翻译中占主导，而词汇生成率在多样性目标语言中更重要。

Conclusion: 翻译质量受到词汇层面生成率和更广泛语言类型学的共同影响，为多语言评估和质量估计提供了新视角。
...
### [170] [Direct-Scoring NLG Evaluators Can Use Pairwise Comparisons Too](https://arxiv.org/abs/2509.05440)
*Logan Lawrence,Ashton Williamson,Alexander Shelton*

Main category: cs.CL

TL;DR: 本文提出了一种直接评分方法，利用合成摘要来模拟人工排序，解决了传统成对比评估方法无法赋予绝对分数的问题。


<details>
  <summary>Details</summary>
Motivation: 当前大语言模型被广泛用于自动评估自由形式内容，但成对比评估方法缺乏给单个摘要赋予绝对分数的能力，而这对需要间阈处理的应用场景至关重要。

Method: 提出一种直接评分方法，通过使用合成摘要来在测试时模拟成对比评分，从而能够给单个摘要赋予绝对分数。

Result: 在SummEval、TopicalChat和HANNA三个评测数据集上，该方法在样本级相关性指标上与最优成对比评估器表现相似，分别为+0.03、-0.03和+0.05。

Conclusion: 该方法能够在保持与成对比评估器相似性能的同时，提供绝对评分能力，为需要间阈处理的应用场景提供了解决方案。
...
### [171] [From Staff Messages to Actionable Insights: A Multi-Stage LLM Classification Framework for Healthcare Analytics](https://arxiv.org/abs/2509.05484)
*Hajar Sakai,Yi-En Tseng,Mohammadsadegh Mikaeili,Joshua Bosire,Franziska Jovin*

Main category: cs.CL

TL;DR: 这篇论文提出了一种基于大语言模型的多阶段框架，用于分析医院呼叫中心的员工消息，识别主题并进行多类分类，最佳模型o3达到78.4%的F1分数。


<details>
  <summary>Details</summary>
Motivation: 医院呼叫中心产生大量文本数据，但传统监督学习需要注释数据和较长训练时间，大语言模型提供了更高效的健康分析方案。

Method: 设计了多阶段LLM框架，评估了包括推理模型、通用模型和轻量模型在内的多种LLM类型，并结合数据安全措施和HIPAA合规要求。

Result: o3模型表现最佳，达到积F1分数78.4%和准确率79.2%，gpt-5以积F1分数75.3%和准确率76.2%随后。处理输出集成到可视化决策支持工具中。

Conclusion: 该方法能够高效利用员工消息数据，识别导航员培训机会，支持改善患者体验和护理质量，为健康领域提供了可行的分析解决方案。
...
### [172] [The Token Tax: Systematic Bias in Multilingual Tokenization](https://arxiv.org/abs/2509.05486)
*Jessica M. Lundin,Ada Zhang,Nihal Karim,Hamza Louzan,Victor Wei,David Adelani,Cody Carroll*

Main category: cs.CL

TL;DR: 研究表明词汇分割效率对形态复杂语言存在结构性劣势，高词汇分割率（tokens/word）可靠预测准确率下降，推理模型表现更优，词汇分割翻倍导致训练成本和时间四倍增长


<details>
  <summary>Details</summary>
Motivation: 评估词汇分割效率对非洲语言处理的影响，揭示形态复杂语言在NLP中的结构性劣势，推动更公平的语言处理技术发展

Method: 在AfriMMLU数据集（9000个多选题，5个科目，16种非洲语言）上评估10个大语言模型，分析词汇分割率与准确率的关系

Result: 词汇分割率可靠预测模型准确率，高分割率始终对应低准确率；推理模型（DeepSeek, o1）在所有语言中表现优于非推理模型；词汇分割翻倍导致训练成本和时间增加四倍

Conclusion: 需要开发形态感知的词汇分割技术、实施公平定价策略、建立多语言基准测试，以实现更公平的自然语言处理
...
### [173] [Biomedical Literature Q&A System Using Retrieval-Augmented Generation (RAG)](https://arxiv.org/abs/2509.05505)
*Mansi Garg,Lee-Chi Wang,Bhavesh Ghanchi,Sanjana Dumpala,Shreyash Kakde,Yen Chih Chen*

Main category: cs.CL

TL;DR: 基于RAG架构的生物医学文献问答系统，通过MiniLM和FAISS搜索配合精经网络Mistral-7B，在乳腺癌预计任务上显著提升了回答的事实一致性和语义相关性。


<details>
  <summary>Details</summary>
Motivation: 解决传统健康搜索引擎的不足以及公众获取生物医学研究成果的延迟问题，提供准确、有证据支撑的医疗信息访问。

Method: 采用检索增强生成(RAG)架构，整合PubMed文章、精选问答数据集和医学百科等多源数据。检索通道使用MiniLM语义嵌入和FAISS向量搜索，生成模型使用QLoRA优化的精经网络Mistral-7B-v0.3。

Result: 在乳腺癌文献预计任务上，系统在BERTScore(F1)指标上显示出显著攻劫基线模型的事实一致性和语义相关性提升。

Conclusion: 证明了RAG增强语言模型在缩小复杂生物医学文献与可访公共健康知识之间差距的潜力，为多语言适配、隐私保护推理和个性化医疗AI系统的未来研究铺平道路。
...
### [174] [Using Contrastive Learning to Improve Two-Way Reasoning in Large Language Models: The Obfuscation Task as a Case Study](https://arxiv.org/abs/2509.05553)
...
TL;DR: 这篇论文探讨了大语言模型是否真正理解概念的根本问题，提出双向推理作为测试标准，并发现了精细调整导致的认知专门化问题，最终开发了对比式精细调整方法来实现双向推理能力。


<details>
  <summary>Details</summary>
Motivation: 解决AI领域的基本问题：大语言模型是否真正理解概念还是仅仅识别模式。作者认为真正的理解应该自然允许可逆性变换。

Method: 提出双向推理概念，并开发了对比式精细调整(CFT)方法。CFT使用三类示例：保持语义的正示例、语义不同的负示例、以及前向匿名化示例，以发展深层理解而非表面模式识别。

Result: 实验表明CFT成功实现了双向推理，在保持前向任务能力的同时实现了强大的反向性能。还发现了精细调整导致的认知专门化现象：前向任务性能提升但反向推理能力显著下降。

Conclusion: 双向推理既是评估真正理解的理论框架，也是开发更强大AI系统的实践训练方法。CFT方法能够在不需显式反向训练的情况下自然发展反向能力。
...
### [175] [Ad hoc conventions generalize to new referents](https://arxiv.org/abs/2509.05566)
*Anya Ji,Claire Augusta Bergey,Ron Eliav,Yoav Artzi,Robert D. Hawkins*

Main category: cs.CL

TL;DR: 人们通过概念对齐而不是任意标签来形成新的共享命名系统，这种概念协调能够推广到未讨论过的新引用物


<details>
  <summary>Details</summary>
Motivation: 研究人们如何形成新的共享命名系统，测试两种竞争观点：是任意标签还是概念对齐

Method: 采用二元沟通研究（N=302），利用KiloGram数据集的1000张抽象图形。参与者先协调对一组图片的指引规约，然后测量对未讨论图片的描述对齐程度

Result: 发现强烈的推广效应：伙伴间相对预测标签显示凸凸增加的对齐。推广效应随视觉相似性非线性衰减（符合Shepard定律），并在不同命名能力水平下都稳健

Conclusion: 随机规约不是任意标签，而是真正的概念协调，这对参考理论和更适应性语言代理的设计具有含义
...
### [176] [Mitigating Spurious Correlations Between Question and Answer via Chain-of-Thought Correctness Perception Distillation](https://arxiv.org/abs/2509.05602)
*Hongyan Xie,Yitong Yao,Yikun Ban,Zixuan Huang,Deqing Wang,Zhenhe Wu,Haoxiang Su,Chao Wang,Shuangyong Song,Xuelong Li*

Main category: cs.CL

TL;DR: CoPeD方法通过正确性感知任务设置和加权损失函数，提升小语言模型从大语言模型生成的思维链数据中学习推理质量的能力


<details>
  <summary>Details</summary>
Motivation: 小语言模型从大语言模型生成的思维链数据中学习时，会吸收噪声推理链，导致学习到问题和答案之间的虚假相关性，影响推理质量

Method: 提出正确性感知任务设置，鼓励学生模型基于正确推理链预测答案并修正错误；提出正确性感知加权损失，根据推理链和答案的联合损失动态调整训练样本权重

Result: 在分布内和分布外基准推理数据集上均表现出有效性

Conclusion: CoPeD方法通过改进任务设置和数据利用方式，有效提升了学生模型的推理质量
...
TL;DR: Icon²提出了一种利用LLM表示空间内在调节机制来高效构建偏好数据集的新方法，通过层间方向向量编码人类偏好并过滤自合成指令，在解码时进行双向内在控制以生成具有明确对齐差异的响应对。
...
Method: 1) 提取层间方向向量编码复杂人类偏好；2) 基于内在一致性过滤自合成指令；3) 解码时应用双向内在控制来引导token表示，精确生成具有清晰对齐差异的响应对。

Result: Llama3-8B和Qwen2-7B在AlpacaEval 2.0上平均胜率提升13.89%，在Arena-Hard上提升13.45%，同时计算成本降低高达48.1%。

Conclusion: Icon²通过利用LLM表示空间的内在调节，实现了高效且定制化的偏好数据集构建，在提升对齐效果的同时显著降低了计算开销。
...
TL;DR: 本文提出了一个端到端的生成式搜索引擎优化(GSEO)框架，包括大规模内容中心基准测试CC-GSEO-Bench和多维度评估框架，以及一个多智能体系统来自动化内容优化。


<details>
  <summary>Details</summary>
Motivation: 传统排名式搜索向生成式搜索引擎的范式转变使得传统SEO指标过时，迫切需要理解和优化内容对合成答案的影响。

Method: 构建大规模内容中心基准测试CC-GSEO-Bench，提出多维度评估框架量化内容影响力，设计多智能体系统实现分析-修订-评估的协作工作流。
...
TL;DR: 该论文提出了一种基于不平衡最优传输的声学-语言表示对齐方法，将ASR中的跨模态对齐视为检测问题，通过软部分匹配处理分布不匹配和结构不对称性。


<details>
  <summary>Details</summary>
Motivation: 声学和语言表示的对齐是ASR知识迁移中的核心挑战，存在固有的结构不对称性（多对一、一对多关系）和噪声帧问题，需要高精度、高召回率的对齐方法。

Method: 提出不平衡最优传输对齐模型，显式处理分布不匹配和结构不对称性，采用软部分匹配机制，确保每个语言标记至少对应一个声学观测，同时允许灵活的声学到语言单元的概率映射。

Result: 在基于CTC的ASR系统和预训练语言模型的知识迁移实验中，该方法能够灵活控制匹配程度，有效提升了ASR性能。

Conclusion: 将声学-语言对齐视为检测问题，并通过不平衡最优传输方法处理结构不对称性，为ASR知识迁移提供了有效的跨模态对齐解决方案。
...
TL;DR: 这篇论文研究歌词情感分析，通过手动标注的数据集评估了零梳学习和细调模型在多标签情感强度预测中的表现。


<details>
  <summary>Details</summary>
Motivation: 歌词的情感内容对音乐体验和偏好有重要影响，需要研究如何准确识别和预测歌词中的多种情感。

Method: 构建了基于平均意见分数(MOS)的手动标注数据集，评估多个公开大语言模型的零梳学习能力，并细调BERT模型进行多标签情感强度预测。

Result: 实验结果显示了零梳学习和细调模型在捐描歌词情感细腻方面的相对优势和局限性。

Conclusion: 大语言模型在创造性文本情感识别中具有潜力，为情感基于音乐信息检索应用提供了模型选择策略的见解。
...
### [181] [Few-Shot Query Intent Detection via Relation-Aware Prompt Learning](https://arxiv.org/abs/2509.05635)
*Liang Zhang,Yuan Li,Shijie Zhang,Zheng Zhang,Xitong Li*

Main category: cs.CL

TL;DR: SAID是一个新颖的框架，首次统一整合文本和关系结构信息进行模型预训练，通过QueryAdapt机制在关系标记级别实现细粒度知识迁移，显著提升少样本意图检测性能。
...
Method: 提出SAID框架，统一整合文本和关系结构信息进行预训练；进一步提出QueryAdapt机制，在关系标记级别生成意图特定的关系标记，实现细粒度知识迁移。
...
### [182] [LM-Searcher: Cross-domain Neural Architecture Search with LLMs via Unified Numerical Encoding](https://arxiv.org/abs/2509.05657)
...
<details>
  <summary>Details</summary>
Motivation: 现有基于大语言模型的神经架构搜索方法严重依赖提示工程和领域特定调优，限制了其在多样化任务中的实用性和可扩展性。

Method: 提出NCode通用数值字符串表示法编码神经架构，将NAS问题重构为排名任务，使用基于剪枝的子空间采样策略生成指令调优样本训练LLM从候选池中选择高性能架构。

Result: 在领域内（如图像分类CNN）和领域外（如分割和生成的LoRA配置）任务中均取得竞争性性能。

Conclusion: LM-Searcher为灵活和可泛化的基于LLM的架构搜索建立了新范式，具有跨域应用的潜力。
...
### [183] [Cross-Question Method Reuse in Large Language Models: From Word-Level Prediction to Rational Logical-Layer Reasoning](https://arxiv.org/abs/2509.05660)
*Hong Su*
...
Method: 首先将问题和解决方案分离，然后指导LLM将解决方案适配到新的相关问题上，专注于解决方案迁移而非问题识别。该方法还扩展到仅共享部分特征或隐藏特征的情况。

Result: 实验验证表明，该范围扩展方法提高了筛选可重用解决方案的概率，从而提升了跨问题方法重用的有效性。

Conclusion: 通过分离问题和解决方案并专注于解决方案迁移，可以突破传统相似性约束，实现更广泛的跨问题方法重用。
...
### [184] [Llama-GENBA-10B: A Trilingual Large Language Model for German, English and Bavarian](https://arxiv.org/abs/2509.05668)
...
TL;DR: Llama-GENBA-10B是一个10B参数的三语基础模型，基于Llama 3.1-8B构建，支持英语、德语和巴伐利亚语，旨在解决英语中心化偏见并促进低资源语言发展。


<details>
  <summary>Details</summary>
Motivation: 解决大型语言模型中的英语中心化偏见，为德语NLP社区提供支持，并促进巴伐利亚语这一低资源语言的发展。

Method: 在Llama 3.1-8B基础上扩展到10B参数，使用164B tokens（82B英语、82B德语、80M巴伐利亚语）进行持续预训练，创建统一的分词器，优化架构和语言比例超参数，建立首个标准化三语评估套件。

Result: 模型在三语跨语言性能表现强劲，微调版本在巴伐利亚语上超越Apertus-8B-2509和gemma-2-9b，成为该语言类别最佳模型，同时在英语上优于EuroLLM，在德语上与其持平。
...
### [185] [Revealing the Numeracy Gap: An Empirical Investigation of Text Embedding Models](https://arxiv.org/abs/2509.05691)
*Ningyuan Deng,Hanyu Duan,Yixuan Tang,Yi Yang*

Main category: cs.CL

TL;DR: 文本嵌入模型在数值信息精确编码方面表现不佳，尤其在金融预计等需要数值精度的领域


<details>
  <summary>Details</summary>
Motivation: 当前文本嵌入模型的能力测试通常涉及不需要理解细微数值信息的任务，而在金融、医疗等数值关键的领域，数值精确性至关重要

Method: 使用金融上下文中的合成数据，对13个广泛使用的文本嵌入模型进行评估

Result: 文本嵌入模型通常无法准确地捕捉数值细节，在数值内容编码方面遇到困难

Conclusion: 需要加强嵌入模型的数值处理能力，以支持更好的NLP系统在数值关键领域的应用
...
### [186] [A Survey of the State-of-the-Art in Conversational Question Answering Systems](https://arxiv.org/abs/2509.05716)
*Manoj Madushanka Perera,Adnan Mahmood,Kasun Eranda Wijethilake,Fahmida Islam,Maryam Tahermazandarani,Quan Z. Sheng*

Main category: cs.CL

TL;DR: 这是一份关于对话问答系统(ConvQA)的综述性论文，详细分析了该领域的最新进展、核心组件、元学习技术和大型语言模型的应用。


<details>
  <summary>Details</summary>
Motivation: 对话问答系统在多个领域发挥着关键作用，需要维持话题的相关性和连贯性。本文通过综述形式提供对该领域现状的全面分析，以指导未来研究方向。

Method: 论文采用综述性分析方法，从三个核心组件(历史选择、问题理解、答案预测)入手，分析了加强学习、对比学习、迁移学习等元学习技术，以及RoBERTa、GPT-4、Gemini 2.0 Flash、Mistral 7B、LLaMA 3等大型语言模型的应用。

Result: 本文系统性总结了ConvQA领域的现状，包括关键数据集分析和技术进展。大型语言模型通过数据可扩展性和架构创新，显著提升了ConvQA系统的准确性和效率。

Conclusion: 该综述为ConvQA领域提供了全面的概览，明确了未来的研究方向，对推动该领域的进一步发展具有重要指导价值。
...
### [187] [Exploring Subjective Tasks in Farsi: A Survey Analysis and Evaluation of Language Models](https://arxiv.org/abs/2509.05719)
*Donya Rooein,Flor Miriam Plaza-del-Arco,Debora Nozza,Dirk Hovy*

Main category: cs.CL

TL;DR: Farsi虽然拥有1.27亿使用者和大量数字文本，但在主观性NLP任务（情感分析、情绪分析、毒性检测）中面临严重的数据可用性和质量问题，实际资源状况远不如表面显示的丰富。


<details>
  <summary>Details</summary>
Motivation: 研究Farsi语言在自然语言处理中的真实资源状况，特别是针对主观性任务的数据可用性和质量挑战，揭示表面数据丰富背后的实际困境。

Method: 回顾了110篇关于Farsi主观性任务的出版物，分析公开数据集的可用性，评估现有数据集的质量（特别是人口统计因素缺失问题），并在可用数据集上测试预测模型的稳定性。

Result: 发现公开数据集严重缺乏，现有数据集缺少关键的人口统计信息（如年龄、性别），模型预测结果在不同数据集和模型间极不稳定。

Conclusion: 单纯的数据量增长不足以显著改善语言在NLP中的前景，数据质量和多样性同样重要，Farsi的实际资源状况比表面标签所显示的更为严峻。
...
### [188] [QCSE: A Pretrained Quantum Context-Sensitive Word Embedding for Natural Language Processing](https://arxiv.org/abs/2509.05729)
*Charles M. Varmantchaonala,Niclas GÖtting,Nils-Erik SchÜtte,Jean Louis E. K. Fendji,Christopher Gies*

Main category: cs.CL

TL;DR: 量子自然语言处理新方法，通过QCSE模型利用量子计算特性学习语言上下文关系，在低资源语言上也可获得良好表现


<details>
  <summary>Details</summary>
Motivation: 利用量子计算的特有性质来处理自然语言的复杂上下文关系，并解决低资源语言数据缺乏的问题

Method: 提出QCSE预训练量子上下文敏感嵌入模型，采用5种上下文矩阵计算方法（指数衰减、正弦调制、相位移、哈希变换等）来创建词的上下文表征

Result: QCSE模型成功捐捕了上下文敏感性，并在Fulani（低资源非洲语言）和英语语料上都取得良好表现，证明了量子系统在语言表征中的优势

Conclusion: 该研究展示了量子计算在NLP领域的强大潜力，为解决实际语言挑战开启了新途径，尤其在处理低资源语言方面具有重要意义
...
### [189] [Enhancing Factual Accuracy and Citation Generation in LLMs via Multi-Stage Self-Verification](https://arxiv.org/abs/2509.05741)
*Fernando Gabriela García,Qiyang Shi,Zilin Feng*

Main category: cs.CL

TL;DR: VeriFact-CoT是一种新颖方法，通过多阶段事实验证-反思-引用集成机制，解决大语言模型在生成复杂事实敏感内容时的幻觉和缺乏可信引用问题。


<details>
  <summary>Details</summary>
Motivation: 解决大语言模型在生成复杂事实敏感内容时普遍存在的幻觉问题和缺乏可信引用来源的问题，提高生成内容的客观准确性、可信度和可追溯性。

Method: 采用多阶段的事实验证-反思-引用集成机制，使大语言模型能够批判性地自我检查和修订其中间推理步骤和最终答案。

Result: 显著提高了生成输出的客观准确性、可信度和可追溯性，使大语言模型在需要高保真度的应用中更加可靠。

Conclusion: VeriFact-CoT方法有效增强了大语言模型在科学研究、新闻报道和法律咨询等高保真度要求应用中的可靠性。
...
### [190] [LatinX: Aligning a Multilingual TTS Model with Direct Preference Optimization](https://arxiv.org/abs/2509.05863)
*Luis Felipe Chary,Miguel Arjona Ramirez*

Main category: cs.CL

TL;DR: LatinX是一个多语言文本转语音模型，专注于语音到语音翻译中保持说话人身份，通过三阶段训练（预训练、监督微调、DPO对齐）在英语和罗曼语系语言上取得良好效果


<details>
  <summary>Details</summary>
Motivation: 解决跨语言语音翻译中说话人身份保持的问题，特别是在英语和罗曼语系语言之间，重点针对葡萄牙语

Method: 12层仅解码器Transformer，三阶段训练：1）文本到音频映射预训练 2）零样本语音克隆监督微调 3）基于WER和说话人相似度指标的DPO对齐

Result: 相比微调基线，DPO版本持续降低WER并改善客观相似度。人工评估显示比XTTSv2基线具有更强的感知说话人相似性，但客观与主观测量存在差距

Conclusion: 拉丁X模型在跨语言说话人身份保持方面表现优异，未来工作需要平衡偏好信号和开发低延迟架构
...
### [191] [ZhiFangDanTai: Fine-tuning Graph-based Retrieval-Augmented Generation Model for Traditional Chinese Medicine Formula](https://arxiv.org/abs/2509.05867)
*ZiXuan Zhang,Bowen Hao,Yingjie Li,Hongzhi Yin*

Main category: cs.CL

TL;DR: 中医方前主流模型在方前生成中缺乏完整组成和详细解释。该研究提出ZhiFangDanTai框架，结合图基检索增强生成和大语言模型微调，通过知识检索和数据集优化提高方前生成质量。理论证明该方法能降低欠误率，实验结果显示性能显著提升。


<details>
  <summary>Details</summary>
Motivation: 现有中医方前生成模型存在以下问题：缺乏完整的方前组成细节、中医四气六组等具体解释、现有指令数据集内容不足。这些限制了模型输出的深度和完整性。

Method: 提出ZhiFangDanTai框架，结合GraphRAG（图基检索增强生成）和LLM微调技术。使用GraphRAG检索并综合结构化中医知识，构建增强指令数据集来改善LLM整合检索信息的能力。

Result: 在收集的数据集和临床数据集上的实验结果显示，ZhiFangDanTai在中医方前生成任务上较现有最优模型取得了显著提升。模型已开源发布。

Conclusion: 通过结合GraphRAG知识检索和LLM微调的方法，能够有效提高中医方前生成的完整性和准确性，降低模型的欠误率和幻觉现象。
...
### [192] [MedFactEval and MedAgentBrief: A Framework and Workflow for Generating and Evaluating Factual Clinical Summaries](https://arxiv.org/abs/2509.05878)
...
TL;DR: 一个可扩展的临床文本事实准确性评估框架(MedFactEval)和高质量出院摘要生成工作流(MedAgentBrief)，通过LLM雇团评估与临床专家评分达到几乎完美一致性。


<details>
  <summary>Details</summary>
Motivation: 解决LLM生成临床文本事实准确性评估的可扩展性问题，因为专家审查无法满足连续质量保障的需求。

Method: 使用MedFactEval框架：临床医生定义关键事实，LLM雇团(多个LLM多数票)评估生成摘要中的包含情况；同时提出MedAgentBrief多步高质量生成工作流。

Result: LLM雇团与7名医生评分达到几乎完美一致性(Cohen's kappa=81%)，统计学上不劣于单个人类专家(kappa=67%, P < 0.001)。
...
### [193] [Let's Roleplay: Examining LLM Alignment in Collaborative Dialogues](https://arxiv.org/abs/2509.05882)
*Abhijnan Nath,Carine Graff,Nikhil Krishnaswamy*

Main category: cs.CL

TL;DR: 本文研究了不同对齐方法对LLM代理在多轮多方协作中作为合作伙伴有效性的影响，提出了摩擦代理干预机制和反事实评估框架，发现摩擦感知方法在促进共识达成和任务结果正确性方面显著优于基线方法。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型越来越多地作为人类"协作者"融入工作流程，需要确保其在多轮交互中的行为可预测且可靠。现有的对齐技术通常在简化单用户设置下开发，未能考虑长期多方交互的动态特性。

Method: 通过角色扮演方法评估不同训练方式的摩擦代理在协作任务对话中的干预效果，提出新颖的反事实评估框架量化摩擦干预如何改变群体协作轨迹和信念对齐。
...
### [194] [Enhancing the Robustness of Contextual ASR to Varying Biasing Information Volumes Through Purified Semantic Correlation Joint Modeling](https://arxiv.org/abs/2509.05908)
*Yue Gu,Zhihao Du,Ying Shi,Shiliang Zhang,Qian Chen,Jiqing Han*

Main category: cs.CL

TL;DR: 通过精绎语义相关性聚合模型（PSC-Joint），从列表、短语和标记三个粒度聚合最相关的偏置信息，解决交叉注意力ASR模型在长偏置列表下效果下降的问题。


<details>
  <summary>Details</summary>
Motivation: 交叉注意力基于上下文ASR模型在处理不同长度偏置列表时效果差异较大，而实际上只有少量偏置信息与具体ASR中间表示最相关。

Method: 提出PSC-Joint方法，定义三种语义相关性（列表级、短语级、标记级），通过聚合模型找到交集最相关信息，并使用分组竞争筛选机制降低计算成本。

Result: 在AISHELL-1和KeSpeech数据集上，相比基线模型平均F1分数提升21.34%和28.46%，适用于不同长度偏置列表。

Conclusion: PSC-Joint通过聚合多粒度语义相关性，能够有效精准获取最相关偏置信息，提升上下文ASR模型的性能和稳定性。
...
### [195] [Accelerating Large Language Model Inference via Early-Exiting Algorithms](https://arxiv.org/abs/2509.05915)
*Sangmin Bae*

Main category: cs.CL

TL;DR: 这篇论文解决了大语言模型中适应性计算与系统效率的冲突，通过算法与架构的协同设计实现了计算效率和性能的最优平衡。


<details>
  <summary>Details</summary>
Motivation: 大语言模型部署面临高计算成本，而早期退出等适应性计算方法往往造成系统级瓶颈，反而降低批处理吞吐量。

Method: 提出高效并行解码机制、深度参数共享架构以减轻同步问题，以及轻量路由器预训练来动态分配最优递归深度。

Result: 建立了效率与性能之间的新帕罗边界，在单个模型内同时优化了适应性计算和参数效率。

Conclusion: 通过算法和架构的协同设计，成功解决了动态计算与系统效率的冲突，为大语言模型的高效部署提供了新的解决方案。
...
### [196] [KatotohananQA: Evaluating Truthfulness of Large Language Models in Filipino](https://arxiv.org/abs/2509.06065)
*Lorenzo Alfred Nery,Ronald Dawson Catignas,Thomas James Tiam-Lee*

Main category: cs.CL

TL;DR: KatotohananQA是TruthfulQA基准的菲律宾语翻译版本，用于评估LLM在低资源语言中的真实性表现。研究发现英语和菲律宾语之间存在显著性能差距，新OpenAI模型展现出较强的多语言鲁棒性，但不同问题类型存在差异。


<details>
  <summary>Details</summary>
Motivation: 当前LLM真实性基准主要针对英语，缺乏对低资源语言的评估，限制了LLM在多语言环境中的可靠应用。

Method: 将TruthfulQA基准翻译成菲律宾语(KatotohananQA)，使用二元选择框架评估7个免费专有模型，比较英语和菲律宾语版本的表现差异。

Result: 英语和菲律宾语真实性表现存在显著差距，GPT-5和GPT-5 mini展现出最佳多语言鲁棒性，不同问题类型和主题的跨语言迁移效果存在差异。
...
### [197] [Multimodal Fine-grained Context Interaction Graph Modeling for Conversational Speech Synthesis](https://arxiv.org/abs/2509.06074)
*Zhenqi Jia,Rui Liu,Berrak Sisman,Haizhou Li*

Main category: cs.CL

TL;DR: MFCIG-CSS是一个基于多模态细粒度上下文交互图的对话语音合成系统，通过构建语义和韵律两个交互图来建模词级语义和韵律的交互，显著提升了对话语音的自然韵律表达。


<details>
  <summary>Details</summary>
Motivation: 现有对话语音合成方法主要关注话语级别的交互特征建模，忽视了多模态对话历史中词级别的细粒度语义和韵律知识交互，导致生成的语音韵律不够自然。

Method: 构建两个多模态细粒度对话交互图：语义交互图和韵律交互图，编码词级语义、韵律及其对后续话语的影响，利用这些交互特征增强合成语音的自然对话韵律。

Result: 在DailyTalk数据集上的实验表明，MFCIG-CSS在韵律表达性方面优于所有基线模型。

Conclusion: 提出的多模态细粒度上下文交互图方法有效解决了对话语音合成中细粒度语义和韵律交互建模的不足，显著提升了合成语音的自然度和韵律质量。
...
### [198] [Multimodal Reasoning for Science: Technical Report and 1st Place Solution to the ICML 2025 SeePhys Challenge](https://arxiv.org/abs/2509.06079)
*Hao Liang,Ruitao Wu,Bohan Zeng,Junbo Niu,Wentao Zhang,Bin Dong*

Main category: cs.CL

TL;DR: 提出了一种caption-assisted reasoning框架，在ICML 2025 AI for Math Workshop挑战赛中获得第一名，有效解决了多模态推理中的视觉-文本模态融合问题


<details>
  <summary>Details</summary>
Motivation: 尽管文本推理已有显著进展，但最先进的模型（如GPT-3）在多模态场景中表现不佳，需要解决视觉和文本模态之间的融合挑战

Method: 采用caption-assisted reasoning框架，通过生成描述性文本来桥接视觉和文本模态，实现有效的多模态推理

Result: 在ICML 2025 AI for Math Workshop的SeePhys挑战中获得第一名，在MathVerse几何推理基准测试中验证了方法的泛化能力

Conclusion: 该方法在多模态推理中表现出色，具有有效性和鲁棒性，代码已开源供社区使用
...
### [199] [Orthogonal Low-rank Adaptation in Lie Groups for Continual Learning of Large Language Models](https://arxiv.org/abs/2509.06100)
*Kefan Cao,Shuaicheng Wu*

Main category: cs.CL

TL;DR: OLieRA方法通过引入李群理论和乘法更新，在保持LLM参数几何结构的同时应用正交约束，解决了连续学习中灾难性遗忘问题，在标准基准测试中达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有参数正则化方法如O-LoRA和N-LoRA虽然通过强制低秩子空间正交性来缓解任务干扰，但忽略了传统加法微调会破坏LLM参数的内在几何结构，限制了性能表现。

Method: 提出OLieRA方法，将李群理论引入LLM微调：利用乘法更新来保持参数几何结构，同时对任务子空间应用正交性约束。

Result: 实验表明OLieRA在标准连续学习基准测试中取得了最先进的结果，在大量任务设置中仍保持顶级性能。
...
### [200] [Benchmarking Gender and Political Bias in Large Language Models](https://arxiv.org/abs/2509.06164)
*Jinrui Yang,Xudong Han,Timothy Baldwin*

Main category: cs.CL

TL;DR: EuroParlVote是一个评估LLM在政治敏感语境中表现的新基准，通过连接欧洲议会辩论演讲和投票结果，揭示LLM在性别分类和投票预测任务中存在系统性偏见。


<details>
  <summary>Details</summary>
Motivation: 需要评估大型语言模型在政治敏感语境中的表现，特别是在涉及性别、政治立场等敏感因素时的公平性和准确性。

Method: 构建EuroParlVote数据集，包含欧洲议会辩论演讲、投票结果和议员人口统计数据，评估LLM在性别分类和投票预测两个任务上的表现。

Result: 发现LLM经常将女性议员误分类为男性，在模拟女性演讲者投票时准确性降低，政治立场上偏向中间派而低估极左和极右派，GPT-4o等专有模型在鲁棒性和公平性上优于开源模型。

Conclusion: EuroParlVote数据集为NLP在政治语境中的公平性和问责制研究提供了重要资源，揭示了LLM在政治敏感任务中存在的系统性偏见问题。
...
### [201] [Understanding the Influence of Synthetic Data for Text Embedders](https://arxiv.org/abs/2509.06184)
*Jacob Mitchell Springer,Vaibhav Adlakha,Siva Reddy,Aditi Raghunathan,Marius Mosbach*

Main category: cs.CL

TL;DR: 本文研究了合成数据在文本嵌入模型中的作用，发现其改进效果有限且存在任务间的性能权衡，挑战了合成数据能构建更鲁棒通用嵌入模型的观念。


<details>
  <summary>Details</summary>
Motivation: 当前通用文本嵌入模型的发展依赖于大量LLM生成的合成数据训练，但缺乏公开可用的合成数据集阻碍了对其泛化作用的研究。

Method: 首先复现并公开了Wang等人提出的合成数据集，然后系统分析了合成数据在哪些方面真正提升了模型泛化能力。

Result: 合成数据带来的性能提升是稀疏且高度局部化的，仅限于特定数据集；同时存在任务间的性能权衡，提升一个任务可能降低另一个任务的性能。
...
### [202] [Augmented Fine-Tuned LLMs for Enhanced Recruitment Automation](https://arxiv.org/abs/2509.06196)
*Mohamed T. Younes,Omar Walid,Khaled Shaban,Ali Hamdi,Mai Hassan*

Main category: cs.CL

TL;DR: 这篇论文提出了一种基于精调大语言模型的新题招聘自动化方法，通过合成数据集和标准化JSON格式提高了招聘任务的准确性和效率。


<details>
  <summary>Details</summary>
Motivation: 解决通用LLMs在招聘任务中的局限性，通过专门调整提高模型在招聘工作流中的准确性和效率。

Method: 使用标准化JSON格式创建合成数据集，并使用DeepSeek LLM解析简历数据以增加数据多样性。对Phi-4等模型进行专门的招聘任务精调。

Result: 精调后的Phi-4模型在F1分数上达到90.62%，在exact match、BLEU、ROUGE等各项指标上都显著超越基础模型和其他最先进LLMs。
...
### [203] [MSLEF: Multi-Segment LLM Ensemble Finetuning in Recruitment](https://arxiv.org/abs/2509.06200)
...
Method: 采用分段感知架构，集成多个微调的大型语言模型（Gemma 9B、LLaMA 3.1 8B、Phi-4 14B），使用加权投票机制，Gemini-2.5-Flash作为复杂片段的高级聚合器

Result: 在Exact Match、F1分数、BLEU、ROUGE和Recruitment Similarity等指标上取得显著提升，最佳单模型相比RS指标提升高达+7%

Conclusion: MSLEF的分段感知设计增强了模型对不同简历布局的泛化能力，使其高度适应现实招聘场景，确保精确可靠的候选人信息表示
...
### [204] [No Encore: Unlearning as Opt-Out in Music Generation](https://arxiv.org/abs/2509.06277)
*Jinju Kim,Taehan Kim,Abdul Waheed,Rita Singh*

Main category: cs.CL

TL;DR: 这篇论文探索了在文本到音乐生成模型中应用机器忘学技术，以防止侵权使用受版权保护的创作内容。


<details>
  <summary>Details</summary>
Motivation: AI音乐生成系统存在利用受版权保护内容的风险，引发道德和法律问题，需要找到方法防止这种惯性使用。

Method: 将现有的机器忘学技术应用于预训练的文本到音乐生成基线模型，分析这些方法在忘学预训练数据集时的效果，同时保持模型性能。

Result: 实验结果为音乐生成中应用忘学技术提供了初步见解，识别了遇到的挑战，为未来研究奠定了基础。

Conclusion: 这项研究为音乐生成模型的机器忘学应用开启了基础性分析，显示了该技术在解决侵权风险方面的潜力。
...
### [205] [Mask-GCG: Are All Tokens in Adversarial Suffixes Necessary for Jailbreak Attacks?](https://arxiv.org/abs/2509.06350)
*Junjie Mu,Zonghao Ying,Zhekui Fan,Zonglei Jing,Yaoyuan Zhang,Zhengmin Yu,Wenxin Zhang,Quanchen Zou,Xiangzheng Zhang*

Main category: cs.CL

TL;DR: Mask-GCG是一种改进的越狱攻击方法，通过在GCG攻击后缀中使用可学习的token掩码来识别和修剪低影响力token，减少冗余并降低计算开销，同时保持攻击成功率。


<details>
  <summary>Details</summary>
Motivation: 现有的GCG越狱攻击方法使用固定长度的后缀，其中可能存在token冗余问题，但这个问题尚未被探索。作者希望开发一种能够识别和修剪低影响力token的方法来提高攻击效率。

Method: 提出Mask-GCG方法，使用可学习的token掩码来识别后缀中有影响力的token位置，增加高影响力位置的更新概率，同时修剪低影响力位置的token，从而减少梯度空间大小和计算开销。

Result: 实验结果表明，后缀中大多数token对攻击成功有显著贡献，修剪少数低影响力token不会影响损失值或攻击成功率，揭示了LLM提示中的token冗余现象。

Conclusion: Mask-GCG提供了一种高效且可解释的越狱攻击方法，为从越狱攻击角度开发高效可解释的LLM提供了见解。
...
### [206] [PL-CA: A Parametric Legal Case Augmentation Framework](https://arxiv.org/abs/2509.06356)
*Ao Chang,Yubo Chen,Jun Zhao*

Main category: cs.CL

TL;DR: PL-CA提出参数化RAG框架，将法律知识编码到参数向量中并集成到LLM的前馈网络，缓解上下文窗口限制，同时构建专家标注的多任务法律数据集验证效果。
...
Method: 提出参数化RAG（P-RAG）框架，对语料知识进行数据增强并编码为参数向量，通过LoRA将参数化知识集成到LLM的前馈网络中。构建包含2000+专家标注实例的多任务法律数据集。
...
### [207] [Do LLMs exhibit the same commonsense capabilities across languages?](https://arxiv.org/abs/2509.06401)
*Ivan Martínez-Murillo,Elena Lloret,Paloma Moreda,Albert Gatt*

Main category: cs.CL

TL;DR: 提出了MULTICOM多语言常识生成基准，评估LLMs在英语、西班牙语、荷兰语和瓦伦西亚语中的常识生成能力，结果显示英语表现最佳，资源较少语言表现较差。


<details>
  <summary>Details</summary>
Motivation: 探索大型语言模型在多语言常识生成方面的能力，特别是在资源较少语言中的表现。

Method: 构建MULTICOM基准数据集，评估多个开源LLMs（LLaMA、Qwen、Gemma等），结合自动指标、LLM作为评判者和人工标注进行综合评估。

Result: 英语表现最优，资源较少语言（如瓦伦西亚语）表现显著较差；上下文支持对弱势语言有一定帮助但效果不一。

Conclusion: 当前LLMs在多语言常识生成方面仍存在明显局限性，特别是在资源较少语言中表现不佳。
...
### [208] [WebExplorer: Explore and Evolve for Training Long-Horizon Web Agents](https://arxiv.org/abs/2509.06501)
...
TL;DR: WebExplorer是一个8B参数的网页浏览智能体，通过创新的数据生成方法和强化学习训练，在复杂信息搜索任务中实现了最先进的性能，甚至超越了更大规模的模型。


<details>
  <summary>Details</summary>
Motivation: 现有开源网页智能体在复杂任务上的信息搜索能力有限，且缺乏透明实现，主要挑战在于缺乏具有挑战性的信息搜索数据。

Method: 提出WebExplorer系统化数据生成方法，使用基于模型的探索和迭代式长到短查询演化，创建需要多步推理和复杂网页导航的查询-答案对，然后通过监督微调和强化学习训练8B模型。

Result: WebExplorer-8B在多个信息搜索基准测试中达到同规模最佳性能，在BrowseComp-en/zh上超越WebSailor-72B，在WebWalkerQA和FRAMES上达到100B参数以下模型的最佳表现，并具有良好的泛化能力。

Conclusion: 该方法为长视野网页智能体的发展提供了实用路径，证明了通过高质量数据生成和适当训练，较小模型也能在复杂网页导航任务中表现出色。
...
### [209] [Crown, Frame, Reverse: Layer-Wise Scaling Variants for LLM Pre-Training](https://arxiv.org/abs/2509.06518)
*Andrei Baroian,Kasper Notebomer*

Main category: cs.CL

TL;DR: 这篇论文探索了Transformer模型中不同层深度的功能差异性，提出了三种新的层次缩放变体（Framed、Reverse、Crown）来重新分配FFN宽度和关注头数，在固定参数预算下实现了更好的性能。


<details>
  <summary>Details</summary>
Motivation: 传统Transformer模型使用均匀的层大小，忽视了不同深度层在功能角色和计算需求上的差异性。需要一种更有效的层次缩放方案来优化模型结构。

Method: 基于层次缩放（LWS）和剪枝技术，提出三种新的LWS变体：Framed、Reverse和Crown。通过二点或三点线性插值重新分配FFN宽度和关注头数，在预训练阶段实施。

Result: 在固定180M参数和5B标签的训练设置下，所有模型都收敛到相似的损失值，且都比相同成本的均匀基线模型表现更好，同时没有导致训练速度的显著下降。

Conclusion: 这项工作是预训练阶段层次架构设计空间的初步探索，未来需要在更大规模的标签和参数上扩展实验以全面评估其潜力。
...
### [210] [LAMDAS: LLM as an Implicit Classifier for Domain-specific Data Selection](https://arxiv.org/abs/2509.06524)
*Jian Wu,Hang Yu,Bingchang Liu,Wenjie Yang,Peng Di,Jianguo Li,Yue Zhang*

Main category: cs.CL

TL;DR: LAMDAS是一种利用预训练LLM作为隐式分类器的数据选择方法，通过将数据选择重构为单类分类问题，仅使用少量参考数据就能识别目标域候选数据，在性能和计算效率方面都优于现有方法


<details>
  <summary>Details</summary>
Motivation: 解决领域适配中高质量人工标注数据稀缺的问题，避免使用未筛选数据带来的噪声和性能下降，需要既准确又高效的数据选择方法

Method: 将预训练LLM本身作为隐式分类器，将数据选择重构为单类分类问题，使用少量参考数据集来识别属于目标域的候选数据

Result: LAMDAS不仅在使用少量数据的情况下超越了全数据训练的性能，还在各种场景下优于9个最先进的基线方法，在性能提升和计算效率之间达到了最佳平衡

Conclusion: LAMDAS提供了一种有效且高效的领域特定数据选择方法，通过利用LLM的隐式分类能力，避免了显式特征工程和计算密集型优化过程
...
### [211] [SLiNT: Structure-aware Language Model with Injection and Contrastive Training for Knowledge Graph Completion](https://arxiv.org/abs/2509.06531)
...
Method: 提出SLiNT框架：1）结构引导的邻域增强(SGNE)检索伪邻居丰富稀疏实体；2）动态硬对比学习(DHCL)通过插值正负样本来解决实体级模糊性；3）梯度解耦双重注入(GDDI)进行token级结构感知干预同时保持核心LLM参数。

Result: 在WN18RR和FB15k-237数据集上的实验表明，SLiNT相比基于嵌入和基于生成的基线方法实现了优越或竞争性的性能。

Conclusion: SLiNT证明了结构感知表示学习对于可扩展知识图谱补全的有效性，通过结合语言模型和知识图谱结构信息提升了链接预测性能。
...
### [212] [HAVE: Head-Adaptive Gating and ValuE Calibration for Hallucination Mitigation in Large Language Models](https://arxiv.org/abs/2509.06596)
*Xin Tong,Zhi Lin,Jingya Wang,Bo Jin*

Main category: cs.CL

TL;DR: HAVE是一个无需微调的参数自由解码框架，通过头自适应门控和值校准来解决LLM幻觉问题，在单次前向传播中有效减少幻觉并提升性能
...
### [213] [Guided Decoding and Its Critical Role in Retrieval-Augmented Generation](https://arxiv.org/abs/2509.06631)
*Özgür Uğur,Musa Yılmaz,Esra Şavirdi,Özay Ezerceli,Mahmut El Huseyni,Selva Taş,Reyhan Bayraktar*

Main category: cs.CL

TL;DR: 本研究比较了三种引导解码方法（Outlines、XGrammar、LM Format Enforcer）在RAG系统中的性能，分析了多轮提示设置对结构化输出生成的影响。
...
Result: 研究发现多轮交互对引导解码有显著影响，揭示了意想不到的性能变化，为特定用例的方法选择提供了依据。
...
### [214] [Modelling Intertextuality with N-gram Embeddings](https://arxiv.org/abs/2509.06637)
*Yi Xing*
...
<details>
  <summary>Details</summary>
Motivation: 互文性是文学研究中的核心概念，但缺乏可扩展的量化分析方法。本文旨在开发一种能够进行大规模分析和网络洞察的量化互文性模型。

Method: 使用n-gram嵌入进行文本对比较，计算两个文本中n-gram嵌入的成对相似度，然后平均这些结果作为整体互文性得分。

Result: 在4个已知互文性程度的文本上验证有效，并在267个多样化文本上进行了可扩展性测试，证明了方法的有效性和效率。网络分析还揭示了中心性和社区结构。

Conclusion: 该方法成功捕获和量化了互文关系，为文学研究提供了可扩展的量化分析工具，网络分析进一步验证了方法的有效性。
...
### [215] [Domain-Aware RAG: MoL-Enhanced RL for Efficient Training and Scalable Retrieval](https://arxiv.org/abs/2509.06650)
...
Method: 两阶段管道：1）混合损失持续预训练平衡领域知识和通用能力；2）组相对策略优化强化学习阶段优化查询和段落生成；采用多查询单段落后期融合策略降低计算开销

Result: 在基准数据集上显著优于基线方法，实现最先进的检索性能
...
### [216] [IntrEx: A Dataset for Modeling Engagement in Educational Conversations](https://arxiv.org/abs/2509.06652)
*Xingwei Tan,Mahathi Parvatham,Chiara Gambi,Gabriele Pergola*

Main category: cs.CL

TL;DR: IntrEx数据集首次对教师-学生互动中的趣味性和预期趣味性进行大规模标注，通过比较评级方法提高标注一致性，发现微调后的LLM在预测趣味性方面优于GPT-4o，并分析了语言认知因素对教育对话参与度的影响。
...
Method: 基于TSCC语料库构建IntrEx数据集，采用序列级标注和基于比较的评级方法（受RLHF启发），使用100多名二语学习者进行严格标注，并研究LLM预测人类趣味性判断的能力。

Result: 微调后的7B/8B参数LLM在预测趣味性方面表现优于GPT-4o等大型专有模型，证明了专门数据集在教育场景中建模参与度的潜力。

Conclusion: 研究揭示了具体性、可理解性和吸收度等语言认知因素对教育对话参与度的影响，为开发更具吸引力的教育对话系统提供了重要见解。
...
### [217] [ParCzech4Speech: A New Speech Corpus Derived from Czech Parliamentary Data](https://arxiv.org/abs/2509.06675)
*Vladislav Stankov,Matyáš Kopp,Ondřej Bojar*

Main category: cs.CL

TL;DR: ParCzech4Speech 1.0是基于ParCzech 4.0语料库处理后的版本，专门用于语音建模任务，包含2,695小时的捷克议会演讲录音和官方转录文本，通过WhisperX和Wav2Vec 2.0实现音频文本对齐
...
Result: 创建了包含2,695小时数据的语音数据集，相比ParCzech 3.0版本提取了更多数据且对齐可靠性更高，提供三种灵活变体并保持原始元数据

Conclusion: ParCzech4Speech 1.0是一个高质量、大规模、许可宽松的捷克语语音数据集，适用于自动语音识别、语音合成等多种任务，已在LINDAT和Hugging Face平台发布
...
### [218] [Will Annotators Disagree? Identifying Subjectivity in Value-Laden Arguments](https://arxiv.org/abs/2509.06704)
*Amir Homayounirad,Enrico Liscio,Tong Wang,Catholijn M. Jonker,Luciano C. Siebert*

Main category: cs.CL

TL;DR: 本文研究识别论点中人类价值主观性的方法，发现直接主观性识别比通过价值预测推断主观性效果更好，且对比损失与二元交叉熵损失的组合虽不提升性能但减少对标签主观性的依赖


<details>
  <summary>Details</summary>
Motivation: 聚合多个标注为单一真实标签可能会隐藏标注者分歧的宝贵见解，特别是在主观性起关键作用的任务中。本文旨在识别论点中人类价值动机的主观性

Method: 评估两种主要方法：通过价值预测推断主观性 vs 直接识别主观性。实验结合对比损失和二元交叉熵损失

Result: 直接主观性识别显著提高了标记主观论点的模型性能。损失函数组合虽不提升性能但减少了对每个标签主观性的依赖
...
### [219] [Anchoring Refusal Direction: Mitigating Safety Risks in Tuning via Projection Constraint](https://arxiv.org/abs/2509.06795)
*Yanrui Du,Fenglei Fan,Sendong Zhao,Jiawei Cao,Qika Lin,Kai He,Ting Liu,Bing Qin,Mengling Feng*

Main category: cs.CL

TL;DR: IFT微调会损害LLM的安全拒绝能力，研究发现拒绝方向(r-direction)在训练中会漂移。提出ProCon方法通过投影约束损失来缓解漂移，结合预热策略增强约束效果，在保持任务性能的同时显著提升安全性。
...
Method: 提出ProCon方法：1) 引入投影约束损失项，正则化训练样本隐藏状态在r-direction上的投影幅度；2) 基于早期剧烈漂移观察和数据驱动视角，采用预热策略，早期强约束并拓宽数据分布以增强约束信号。

Result: 在各种数据集、场景和LLM上的实验表明，该方法能显著缓解IFT带来的安全风险，同时保持任务性能增益。与强基线相比，始终提供更优的整体性能。分析显示ProCon有助于稳定训练过程中的r-direction。

Conclusion: ProCon方法有效解决了IFT微调中的安全风险问题，通过约束r-direction漂移来保持模型安全性。这种基于可解释性驱动的LLM内部机制探索为未来安全研究奠定了坚实基础。
...
### [220] [MachineLearningLM: Continued Pretraining Language Models on Millions of Synthetic Tabular Prediction Tasks Scales In-Context ML](https://arxiv.org/abs/2509.06806)
*Haoyu Dong,Pengkun Zhang,Mingzhe Lu,Yanzhen Shen,Guolin Ke*

Main category: cs.CL

TL;DR: MachineLearningLM是一个持续预训练框架，通过从数百万个结构因果模型合成ML任务来增强LLM的上下文学习能力，在少样本到1024样本的范围内实现单调性能提升，在表格分类任务上平均优于强基线15%，同时保持通用聊天能力。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型虽然具有广泛的世界知识和强大的通用推理能力，但在标准机器学习任务上难以通过纯上下文学习从大量样本中有效学习，需要梯度下降之外的解决方案。

Method: 使用随机森林教师模型从结构因果模型合成ML任务进行持续预训练，通过知识蒸馏将基于树的决策策略融入LLM，采用token高效的提示序列化方法，支持3-6倍更多样本和50倍吞吐量。

Result: 在Qwen-2.5-7B-Instruct基础上，使用LoRA rank 8的轻量设置，在金融、物理、生物和医疗领域的分布外表格分类任务上平均优于GPT-5-mini等强基线约15%，展现从8到1024样本的单调缩放规律，达到随机森林级别的准确率。

Conclusion: MachineLearningLM成功为通用LLM赋予了强大的上下文ML能力，同时保持了75.4%的MMLU得分，证明了在保持通用知识推理的同时增强专业任务性能的可行性。
...
### [221] [MoGU V2: Toward a Higher Pareto Frontier Between Model Usability and Security](https://arxiv.org/abs/2509.06807)
...
<details>
  <summary>Details</summary>
Motivation: 现有LLM安全方法往往导致过于保守的拒绝响应，损害了实际可用性。需要找到一种方法能够同时提升安全性和可用性，而不是在两者之间进行权衡。

Method: 提出MoGU_v2框架，在编码高可分类安全特征的层中嵌入路由器，通过双向适配机制动态分配安全优化和可用性优化变体的权重贡献。

Result: MoGU_v2在各种LLM系列上表现出强大的适应性和稳定改进，包括主流LLM、设备端LLM和推理LLM，并能通过简单数据混合策略轻松恢复安全性。

Conclusion: MoGU_v2是一个强大且多功能的解决方案，能够有效缓解现实应用中的安全风险，在安全性和可用性之间实现了帕累托前沿的推进。
...
### [222] [Saturation-Driven Dataset Generation for LLM Mathematical Reasoning in the TPTP Ecosystem](https://arxiv.org/abs/2509.06809)
*Valentin Quesnel,Damien Sileo*

Main category: cs.CL

TL;DR: 利用E-prover定理证明器从TPTP公理库生成大量逻辑严格的数据集，提供三种数学推理挑战任务，替代侞靠LLM生成数据的方法


<details>
  <summary>Details</summary>
Motivation: 解决高质量逻辑合理数据稀缺的问题，这是提升大语言模型数学推理能力的关键瓶颈

Method: 利用E-prover的饱和证明能力，从TPTP公理库汇总派生大量保证正确的定理，通过饱和公理、筛选有趣定理、生成任务的流程生成纯符号数据

Result: 构建了三种难度可控的挑战任务，通过零样本测试发现前沿模型在深层结构化推理任务上表现崩溃

Conclusion: 该框架既提供了评估LLM推理缺口的诊断工具，也为解决这些问题提供了可扩展的符号训练数据来源
...
### [223] [A Comparative Benchmark of Large Language Models for Labelling Wind Turbine Maintenance Logs](https://arxiv.org/abs/2509.06813)
*Max Malyi,Jonathan Shek,Alasdair McDonald,Andre Biscaya*

Main category: cs.CL

TL;DR: 本文提出了一个新题的可复现框架，用于对大语言模型在风力气气轮维护日志分类任务上进行基准测试，并开源代码以促进该领域的研究。


<details>
  <summary>Details</summary>
Motivation: 风力气气轮维护日志的非结构化特性纵碍了自动化分析，需要一个标准化的测试框架来评估LLM模型在这一复杂工业任务上的性能。

Method: 开发了一个可复现的开源框架，系统性评估多种商业和开源LLM模型，分析它们在可靠性、运营效率和模型检验方面的交易特性。

Result: 结果显示了清晰的性能层次结构，识别出了与标准很好对齐且可信的顶级模型。分类性能高度依赖于任务的语义模糊性，所有模型在对象性组件识别上比在解释性维护动作上更一致。

Conclusion: 由于没有模型能达到完美准确性且检验性差异很大，最有效和负责任的应用方式是人在循环系统，让LLM作为人类专家的助手来加速数据标注，提升O&M数据质量和可靠性分析。
...
### [224] [COMPACT: Common-token Optimized Model Pruning Across Channels and Tokens](https://arxiv.org/abs/2509.06836)
*Eugene Kwek,Wenpeng Yin*

Main category: cs.CL

TL;DR: COMPACT是一种联合剪枝方法，通过剪枝稀有词汇和FFN中间通道来压缩LLM，保持标准Transformer架构的同时实现高效部署。


<details>
  <summary>Details</summary>
Motivation: 为了让LLM在边缘部署、交互应用和大规模推理中更高效，需要解决现有剪枝方法的局限性：宽度剪枝破坏标准架构，深度剪枝导致精度骤降。

Method: 联合剪枝稀有词汇（缩小嵌入/解嵌入层）和基于常见词加权激活的FFN中间通道剪枝，保持标准Transformer布局。

Result: 在Qwen、LLaMA、Gemma等模型（0.5B-70B）上实现SOTA下游任务性能，显著减少参数量、GPU内存和端到端延迟。

Conclusion: COMPACT结合了深度和宽度剪枝的优点，具有部署友好性、规模适应性、免训练操作和强内存节省等优势。
...
### [225] [EPT Benchmark: Evaluation of Persian Trustworthiness in Large Language Models](https://arxiv.org/abs/2509.06838)
*Mohammad Reza Mirbagheri,Mohammad Mahdi Mirkamali,Zahra Motoshaker Arani,Ali Javeri,Amir Mahdi Sadeghzadeh,Rasool Jalili*

Main category: cs.CL

TL;DR: 这篇论文提出了EPT评测指标，专门用于评估大语言模型在波斯文化背景下的可信质性，发现主要模型在安全性方面存在显著缺陷


<details>
  <summary>Details</summary>
Motivation: 虽然大语言模型在各种语言任务上表现优异，但确保其可信质性仍是重要挑战，特别是在符合道德、文化和社会价值观方面。需要基于文化基础的评估标准来发展负责任的AI系统

Method: 研究引入EPT（波斯语可信质性评估）指标，这是一个基于文化知识的标准，用于评估LLM在六个关键方面的表现：真实性、安全性、公平性、稳健性、隐私性和道德对齐。编诒了标签数据集，并使用自动LLM基于和人工评估方法对ChatGPT、Claude、DeepSeek、Gemini、Grok、LLaMA、Mistral和Qwen等领先模型进行了评测

Result: 结果显示在安全性方面存在显著缺陷，强调了对模型行为这一关键方面进行重点关注的紧迫性。研究还提供了有关这些模型与波斯道德-文化价值观对齐的有价值见解，并指出了推进可信质和文化负责任AI的关键缺口和机遇

Conclusion: 研究强调了基于文化的评估对发展负责任AI系统的重要性。EPT指标提供了一个专门用于评估波斯语模型可信质性的标准，并发现主要模型在安全性方面存在显著缺陷。这项研究为推进可信质和文化负责任的AI提供了重要见解和方向
...
### [226] [The Majority is not always right: RL training for solution aggregation](https://arxiv.org/abs/2509.06870)
*Wenting Zhao,Pranjal Aggarwal,Swarnadeep Saha,Asli Celikyilmaz,Jason Weston,Ilia Kulikov*

Main category: cs.CL

TL;DR: 提出AggLM方法，通过强化学习训练聚合模型来整合多个候选解决方案，相比简单投票或奖励模型排序能更有效地提升语言模型的推理性能


<details>
  <summary>Details</summary>
Motivation: 当前测试时计算扩展主要依赖简单多数投票或奖励模型排序来聚合解决方案，但这种方法收益有限，需要更智能的聚合方式

Method: 使用强化学习训练聚合模型，让模型学习审查、协调和综合多个候选解决方案来生成最终正确答案，关键是通过平衡难易训练样本来提升模型能力

Result: AggLM方法在多个基准测试中优于基于规则和奖励模型的基线方法，能够有效泛化到不同模型的解决方案，且比多数投票方法需要更少的token

Conclusion: 将聚合作为显式推理技能进行学习是有效的，能够显著提升语言模型在复杂推理任务中的表现，同时提高计算效率
...
### [227] [UNH at CheckThat! 2025: Fine-tuning Vs Prompting in Claim Extraction](https://arxiv.org/abs/2509.06883)
...
TL;DR: 探索不同提示方法和上下文学习策略，用于从社交媒体文本中提取值得核查的声明，发现微调FLAN-T5模型获得最佳METEOR分数，但其他方法有时能提取更高质量声明


<details>
  <summary>Details</summary>
Motivation: 参与CheckThat! Task 2英语任务，研究如何从社交媒体段落中有效提取值得核查的声明

Method: 尝试多种提示方法和上下文学习策略，包括少样本提示和不同LLM家族的微调

Result: 微调FLAN-T5模型获得最佳METEOR分数，但其他方法有时能提取更高质量的声明（即使METEOR分数较低）

Conclusion: METEOR分数并非衡量声明提取质量的唯一标准，某些方法虽然分数较低但能产生更高质量的核查声明
...
### [228] [mmBERT: A Modern Multilingual Encoder with Annealed Language Learning](https://arxiv.org/abs/2509.06888)
*Marc Marone,Orion Weller,William Fleshman,Eugene Yang,Dawn Lawrie,Benjamin Van Durme*

Main category: cs.CL

TL;DR: mmBERT是一个基于3T多语言文本训练的编码器模型，在1800多种语言上表现优异，特别是在低资源语言分类和检索任务中显著超越前代模型。
...
Method: 使用3T多语言文本训练，引入逆掩码比率调度和逆温度采样比率，在衰减阶段添加1700多种低资源语言数据以最大化训练效果。

Result: mmBERT在分类和检索任务上显著优于前代模型，在低资源语言和高资源语言上都表现出色，性能可与OpenAI o3和Google Gemini 2.5 Pro相媲美。

Conclusion: 通过创新的训练策略和数据调度方法，mmBERT证明了编码器模型在多语言处理中的强大潜力，特别是在低资源语言处理方面取得了突破性进展。
...
TL;DR: 提出了一种基于双层优化的新方法，通过让SFT目标以最优RL策略为条件，实现监督微调与强化学习的更好协作，提升推理模型的训练效率和效果。


<details>
  <summary>Details</summary>
Motivation: 传统两阶段方法（SFT预热+RL训练）限制了监督微调与强化学习之间的交互，制约了整体效果。RL虽然能激励大语言模型的推理能力，但试错性质导致效率低下。

Method: 采用双层优化框架：下层执行RL更新同时接收SFT监督，上层显式最大化协作收益（联合训练相比单独RL的性能优势）。SFT目标以最优RL策略为条件进行元学习，指导RL优化过程。

Result: 在五个推理基准测试上的实证评估表明，该方法 consistently 优于基线方法，在效果和效率之间实现了更好的平衡。

Conclusion: 通过双层优化促进SFT与RL的协作训练，能够有效提升推理模型的学习效率和性能表现，为联合训练范式提供了新思路。
...
TL;DR: TraceRL是一个轨迹感知的强化学习框架，用于扩散语言模型的后训练，通过扩散价值模型提升训练稳定性，在数学推理和编程任务上表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有的扩散语言模型在复杂推理任务上表现不足，需要一种能够整合偏好推理轨迹的后训练方法来提升性能。

Method: 提出TraceRL框架，使用扩散价值模型增强训练稳定性，支持跨架构应用，并可通过课程学习适应不同规模模块。

Result: TraDo-4B-Instruct在数学推理任务上超越7B规模的自回归模型，TraDo-8B-Instruct在数学基准上相对Qwen2.5-7B-Instruct提升6.1%，相对Llama3.1-8B-Instruct提升51.3%。

Conclusion: TraceRL框架有效提升了扩散语言模型的推理能力，开发的开源工具包支持多样化架构的训练和部署，推动了扩散LLM的可复现研究和实际应用。
...
TL;DR: 语言模型在语用理解和生成上的语用理解能力评估，发现大型模型表现接近人类，而理性语言行为推理方法能显著提升性能


<details>
  <summary>Details</summary>
Motivation: 语言使用受语用理解影响，随着语言模型作为会话机器人的应用增多，需要理解其语用理解能力

Method: 使用Wavelength游戏框架评估语言理解和生成能力，测试直接提示、思维链提示和理性语言行为推理方法

Result: 大型语言模型在语言理解上达到类人准确度，而语言生成中思维链提示更好，理性语言行为推理方法能显著提升性能

Conclusion: 研究确认了语言模型语用理解能力的优势与限制，并证明理性语言行为推理方法的改进潜力，为理解概念表征、语言理解和社会理解提供了新方向
...
### [232] [A Unified Framework for Cultural Heritage Data Historicity and Migration: The ARGUS Approach](https://arxiv.org/abs/2509.06044)
...
TL;DR: ARGUS项目开发了一个全面的数据历史性和迁移框架，用于处理异构文化遗产数据，通过标准化、丰富化、集成和可视化等流程，将原始数据转换为符合FAIR原则的标准化格式，并在五个欧洲试点站点成功应用。


<details>
  <summary>Details</summary>
Motivation: 文化遗产保护面临多样化、多来源、多尺度数据管理的挑战，需要有效的数据处理框架来支持监测和保护工作。

Method: 采用系统化的数据处理流水线，包括标准化、数据丰富（使用既定插补技术）、集成、可视化、摄取和发布策略，利用LLM驱动的自然语言处理提升查询能力。

Result: 在五个欧洲试点站点应用表明，该框架提高了数据可访问性，增强了分析能力，并为保护工作提供了更有效的决策支持。
...
### [233] [Language Native Lightly Structured Databases for Large Language Model Driven Composite Materials Research](https://arxiv.org/abs/2509.06093)
*Yuze Liu,Zhaoyuan Zhang,Xiangsheng Zeng,Yihe Zhang,Leping Yu,Lejia Wang,Xi Yu*

Main category: cs.DB

TL;DR: 这篇论文提出了一种语言本地化的数据库系统，用于摘取和组织化匙化硝纳米纳米片聚合物热导复合材料研究中的语言基础信息，支持高保真度的RAG和智能代理应用。


<details>
  <summary>Details</summary>
Motivation: 化学材料研究依赖语言描述而非表格数据，限制了传统数据库和机器学习的利用，需要语言本地化的数据管理方案。

Method: 构建异构数据库，摘取论文中的轻结构化信息，包括制备、表征、理论计算和机理推理，通过复合检索技术进行查询。

Result: 系统能够生成准确、可验证的专业指南，支持高保真度的RAG和工具增强智能代理，产生可执行的标准操作程序。

Conclusion: 该框架为LLM驱动的材料发现提供了丰富的语言基础，有力地解决了化学材料领域语言信息利用不充分的问题。
...
### [234] [MCTuner: Spatial Decomposition-Enhanced Database Tuning via LLM-Guided Exploration](https://arxiv.org/abs/2509.06298)
*Zihan Yan,Rui Xi,Mengshu Hou*

Main category: cs.DB

TL;DR: MCTuner是一个自适应数据库参数调优框架，通过混合专家机制识别关键参数，并使用空间分解算法和贝叶斯优化高效搜索最优配置，在多个基准测试中性能提升达19.2%。
...
Method: 提出MCTuner框架：1）使用混合专家机制（MoE）和专用LLMs识别性能关键参数；2）引入首个空间分解算法，递归将空间划分为层次子空间；3）在子空间上执行贝叶斯优化搜索近最优配置。

Result: 在OLAP、OLTP和HTAP等不同基准测试中，MCTuner相比最先进方法实现了高达19.2%的性能提升，每次迭代的配置发现速度提升1.4倍。

Conclusion: MCTuner通过有效减少无效配置空间的探索，显著提高了数据库参数调优的效率和效果，为解决高维参数调优问题提供了有效解决方案。
...
### [235] [Relational Algebras for Subset Selection and Optimisation](https://arxiv.org/abs/2509.06439)
*David Robert Pratten,Luke Mathieson,Fahimeh Ramezani*

Main category: cs.DB

TL;DR: 该论文提出了统一的关系代数基础，通过关系指数操作来支持子集选择和优化查询，充完了传统关系代数的基础操作。


<details>
  <summary>Details</summary>
Motivation: 数据库社区缺乏统一的关系查询语言来处理子集选择和优化查询，现有方案存在不兼容和临时性问题，限制了用户表达能力和查询优化器的推理能力。

Method: 首先扩展关系代数支持域关系（通过特征函数定义），提供了NP完备/难问题的表达能力。其次引入解集合，一种高阶关系代数，将搜索空间表达为函数f: Base → Decision，产生|Decision|^|Base|个候选关系。最后提供结构保持的语义翻译机制。

Result: 该框架实现了最强大现有方案的表达能力，同时提供了理论清晰性和组合性质。通过多态SQL展示了这种代数在数据管理、子集选择和优化查询方面的能力。

Conclusion: 该研究为子集选择和优化查询提供了第一个统一的代数基础，充完了关系代数的基础操作集，为数据库查询语言的发展奠定了重要的理论基础。
...
### [236] [Attention of a Kiss: Exploring Attention Maps in Video Diffusion for XAIxArts](https://arxiv.org/abs/2509.05323)
...
<details>
  <summary>Details</summary>
Motivation: 受早期视频艺术家操控模拟视频信号的启发，希望通过解释性AI技术揭示生成式视频模型的内部工作机制，让艺术家能够将AI内部过程作为创作媒介

Method: 基于开源Wan模型建立工具，提取和可视化跨注意力地图，通过探索性探针和艺术案例研究进行分析

Result: 开发了一种可解释的视觉化工具，能够揭示生成式视频模型中注意力机制的时空行为特征

Conclusion: 这项工作为艺术可解释性AI领域做出了贡献，推动了注意力地图作为分析工具和艺术创作原材料的双重潜力开发
...
### [237] [Perception Graph for Cognitive Attack Reasoning in Augmented Reality](https://arxiv.org/abs/2509.05324)
...
Method: 引入Perception Graph模型，首先模拟人类从MR环境中解释关键信息的过程，然后用语义有意义的结构表示结果，计算量化感知扭曲分数

Result: 模型能够计算反映感知扭曲水平的量化分数
...
### [238] [SynDelay: A Synthetic Dataset for Delivery Delay Prediction](https://arxiv.org/abs/2509.05325)
*Liming Xu,Yunbo Long,Alexandra Brintrup*

Main category: cs.AI

TL;DR: SynDelay是一个用于交付延迟预测的合成数据集，通过先进生成模型在真实数据上训练生成，保持了真实的交付模式同时确保隐私保护，为预测建模提供了具有挑战性的测试平台。


<details>
  <summary>Details</summary>
Motivation: AI在供应链管理中的应用受到高质量开放数据集稀缺的限制，现有数据集通常是专有的、规模小或维护不一致，阻碍了可重复性和基准测试。

Method: 使用在真实数据上训练的先进生成模型生成合成数据集SynDelay，保留现实交付模式同时确保隐私，并提供基线结果和评估指标作为初始基准。

Result: SynDelay数据集虽然不完全无噪声或不一致，但为推进预测建模提供了具有挑战性和实用性的测试平台，并通过供应链数据中心公开提供。

Conclusion: SynDelay作为开放数据集促进了供应链AI研究的可重复性和基准测试，鼓励社区贡献数据集、模型和评估实践以推动该领域研究发展。
...
### [239] [MVRS: The Multimodal Virtual Reality Stimuli-based Emotion Recognition Dataset](https://arxiv.org/abs/2509.05330)
*Seyed Muhammad Hossein Mousavi,Atiye Ilanloo*

Main category: cs.AI

TL;DR: MVRS数据集是一个包含VR情绪刺激下同步采集的眼动、身体运动、EMG和GSR信号的多模态情感识别数据集，通过特征融合和分类器验证了数据质量和情感可分性。
...
Result: 成功构建了MVRS数据集，验证了数据质量和不同情绪状态的可分性。

Conclusion: MVRS数据集为多模态情感计算领域提供了有价值的资源，有助于推动情感识别技术的发展。
...
### [240] [Benchmarking Large Language Models for Personalized Guidance in AI-Enhanced Learning](https://arxiv.org/abs/2509.05346)
*Bo Yuan,Jiazi Hu*

Main category: cs.AI

TL;DR: 这篇论文通过实验比较GPT-4o、DeepSeek-V3和GLM-4.5在个性化学习辅导任务中的表现，发现GPT-4o在准确性、清晰度和可操作性方面更受偏好。


<details>
  <summary>Details</summary>
Motivation: 识别大语言模型在真实学习场景中作为智能助教的效果，填补系统性评估的空白。

Method: 使用10道混合题型的学生答题数据集，要求LLM分析知识点、推断学习技能并生成有针对性的指导。使用Gemini作为虚拟判官进行多维度对比评估。

Result: GPT-4o在准确性、清晰度、可操作性和适度方面都表现最佳，DeepSeek-V3和GLM-4.5显示出间息性优势但一致性较差。

Conclusion: LLM可以作为高级教学助手提供个性化支持，为未来LLM驱动的个性化学习研究提供了方法论指导。
...
### [241] [SasAgent: Multi-Agent AI System for Small-Angle Scattering Data Analysis](https://arxiv.org/abs/2509.05363)
*Lijie Ding,Changwoo Do*

Main category: cs.AI

TL;DR: SasAgent是一个基于大语言模型的多智能体系统，用于自动化小角散射数据分析，通过集成SasView软件工具并提供文本交互界面。


<details>
  <summary>Details</summary>
Motivation: 小角散射数据分析过程复杂且需要专业知识，传统方法效率较低。本文旨在利用大语言模型驱动的AI系统来自动化这一科学工作流程，提高SAS研究的自动化水平。

Method: 系统采用多智能体架构：一个协调器智能体解析用户提示，并分配任务给三个专业智能体（SLD计算、合成数据生成、实验数据拟合）。这些智能体使用基于SasView Python库开发的LLM友好工具，包括模型数据工具、RAG文档工具、拟合工具和SLD计算工具。系统还提供了基于Gradio的用户友好界面。

Result: 通过多样化示例验证，SasAgent能够准确解释复杂提示、计算散射长度密度、生成精确的散射数据，并以高精度拟合实验数据集。

Conclusion: 这项工作展示了LLM驱动的AI系统在简化科学工作流程和增强SAS研究自动化方面的巨大潜力，为科学数据分析提供了新的自动化解决方案。
...
### [242] [Characterizing Fitness Landscape Structures in Prompt Engineering](https://arxiv.org/abs/2509.05375)
*Arend Hintze*
...
<details>
  <summary>Details</summary>
Motivation: 当前提示工程方法将提示优化视为黑盒问题，缺乏对优化景观拓扑结构的理解，需要系统分析提示工程的适应度景观结构。

Method: 使用语义嵌入空间的自相关分析方法，在错误检测任务上比较两种提示生成策略：系统枚举（1024个提示）和新颖性驱动的多样化生成（1000个提示）。

Result: 发现不同提示生成策略产生根本不同的景观拓扑结构，系统提示生成产生平滑衰减的自相关，多样化生成呈现非单调模式，在中间语义距离处达到峰值相关性。不同错误类型的崎岖程度各不相同。

Conclusion: 研究结果为理解提示工程景观中优化的复杂性提供了实证基础，揭示了提示工程优化景观的结构特征和复杂性。
...
### [243] [Code Like Humans: A Multi-Agent Solution for Medical Coding](https://arxiv.org/abs/2509.05378)
*Andreas Motzfeldt,Joakim Edin,Casper L. Christensen,Christian Hardmeier,Lars Maaløe,Anna Rogers*

Main category: cs.AI

TL;DR: 一种新的代理框架"Code Like Humans"，使用大语言模型进行医疗编码，支持全部ICD-10编码系统，在稀有诊断编码上达到最佳性能


<details>
  <summary>Details</summary>
Motivation: 解决医疗编码中将非结构化临床笔记映射到诊断和手术代码的挑战，提供支持全部ICD-10编码系统的解决方案

Method: 开发了一种代理框架，实施了专业人员使用的官方编码指南，是首个支持全部ICD-10编码系统（超70,000个标签）的解决方案

Result: 在稀有诊断编码上达到了目前最佳性能，对于高频编码，细调的判别分类器仍保持优势（但只能处理高频编码）

Conclusion: 该框架为医疗编码领域带来了重要进展，同时还分析了系统的性能并识别出了"盲点"（系统性低估的编码），为未来工作提供了方向
...
### [244] [Murphys Laws of AI Alignment: Why the Gap Always Wins](https://arxiv.org/abs/2509.05381)
*Madhava Gaikwad*

Main category: cs.AI

TL;DR: 本文提出了Alignment Gap概念，统一解释RLHF等对齐方法的失败模式，通过KL倾斜形式化说明优化压力导致代理奖励与人类意图的偏差，并提出了Alignment Trilemma和MAPS框架来指导未来设计。


<details>
  <summary>Details</summary>
Motivation: 现有的人类偏好对齐方法（如RLHF、DPO等）虽然有效，但存在奖励黑客行为、奉承、标注者漂移和错误泛化等重复性失败模式，需要统一的理论框架来理解这些结构性限制。

Method: 使用KL倾斜形式化分析优化压力如何放大代理奖励与真实人类意图的差异，组织失败模式为AI对齐的墨菲定律目录，提出Alignment Trilemma框架，并通过小规模实证研究进行说明。

Result: 建立了Alignment Gap概念框架，系统分类了对齐失败模式，提出了权衡优化强度、价值捕获和泛化能力的Alignment Trilemma，以及实用的MAPS设计杠杆框架。
...
### [245] [From Image Generation to Infrastructure Design: a Multi-agent Pipeline for Street Design Generation](https://arxiv.org/abs/2509.05469)
*Chenguang Wang,Xiang Yan,Yilong Dai,Ziyi Wang,Susu Xu*

Main category: cs.AI

TL;DR: 一种多段系统通过自动化车道定位、提示优化和设计生成技术，直接在实际街道影像上编辑和重新设计自行车设施，提高了设计效率和现实性。


<details>
  <summary>Details</summary>
Motivation: 解决传统街道设计渲染方案制作劳动密集、效率低下的问题，提高公众参与主动交通规划的效果。

Method: 集成车道定位、提示优化、设计生成和自动化评估等模块，构建多段系统来编辑实际街道影像中的自行车设施。
...
### [246] [TreeGPT: A Novel Hybrid Architecture for Abstract Syntax Tree Processing with Global Parent-Child Aggregation](https://arxiv.org/abs/2509.05550)
*Zixi Li*

Main category: cs.AI

TL;DR: TreeGPT是一种新颖的神经架构，结合transformer注意力机制和全局父子聚合来处理AST，在程序合成任务中取得优异性能


<details>
  <summary>Details</summary>
Motivation: 传统方法仅依赖序列处理或图神经网络，无法有效捕捉抽象语法树的层次结构信息，需要一种能同时处理局部依赖和全局树结构的混合方法

Method: 采用混合设计：自注意力捕捉局部依赖 + 专用TreeFFN通过迭代消息传递建模层次树结构，核心创新是全局父子聚合机制，支持门控聚合、残差连接和双向传播

Result: 在ARC Prize 2025数据集上达到96%准确率，显著优于transformer基线(1.3%)、Grok-4(15.9%)和SOAR(52%)，仅使用150万参数

Conclusion: TreeGPT在程序合成任务中表现出色，边投影是最关键组件，边投影与门控组合实现最优性能，为处理树结构数据提供了有效解决方案
...
### [247] [OccVLA: Vision-Language-Action Model with Implicit 3D Occupancy Supervision](https://arxiv.org/abs/2509.05578)
*Ruixun Liu,Lingyu Kong,Derun Li,Hang Zhao*

Main category: cs.AI

TL;DR: OccVLA是一个新颖的多模态框架，将3D占据表示集成到统一推理过程中，无需显式3D输入即可实现自动驾驶中的精细空间理解，在nuScenes基准测试中达到最先进水平。


<details>
  <summary>Details</summary>
Motivation: 解决多模态大语言模型在3D空间理解方面的不足，特别是自动驾驶场景中缺乏鲁棒的3D空间感知能力，以及构建有效3D表示和保持精细空间细节的挑战。

Method: 提出OccVLA框架，将密集3D占据既作为预测输出又作为监督信号，直接从2D视觉输入学习精细空间结构，占据预测可视为隐式推理过程，在推理时可跳过而不增加计算开销。

Result: 在nuScenes基准测试的轨迹规划任务中达到最先进结果，在3D视觉问答任务上表现出优越性能，提供了可扩展、可解释且完全基于视觉的自动驾驶解决方案。

Conclusion: OccVLA成功解决了MLLMs在3D空间理解方面的局限性，通过创新的3D占据表示方法实现了无需昂贵手动标注的精细空间推理，为自动驾驶提供了有效的多模态解决方案。
...
### [248] [MSRFormer: Road Network Representation Learning using Multi-scale Feature Fusion of Heterogeneous Spatial Interactions](https://arxiv.org/abs/2509.05685)
*Jian Yang,Jiahui Wu,Li Fang,Hongchao Fan,Bianying Zhang,Huijie Zhao,Guangyi Yang,Rui Xin,Xiong You*

Main category: cs.AI

TL;DR: MSRFormer是一个新颖的道路网络表示学习框架，通过整合多尺度空间交互来解决道路网络的异质性和层次性挑战，在道路网络分析任务中优于基线方法。


<details>
  <summary>Details</summary>
Motivation: 城市道路网络的异质性和层次性给准确表示学习带来挑战，传统图神经网络由于同质性假设和单尺度结构关注而难以有效处理。

Method: 使用空间流卷积从大规模轨迹数据提取小尺度特征，识别尺度依赖的空间交互区域，采用图变换器捕获多尺度复杂空间依赖，通过残差连接融合特征并应用对比学习算法。
...
TL;DR: 提出了CogEdit基准测试和MIND框架，用于评估和改进多模态大语言模型的元认知知识编辑能力，在三个层次上超越了现有认知编辑方法。


<details>
  <summary>Details</summary>
Motivation: 现有知识编辑基准主要关注认知层面的修改，缺乏对更深层次元认知过程的关注，需要开发能够评估模型自我意识、边界约束和噪声鲁棒性的新基准和方法。

Method: 提出CogEdit基准测试评估三个元认知层次：反事实驱动编辑、边界约束编辑和噪声鲁棒编辑；开发MIND框架，构建元知识记忆库用于自我意识，使用博弈论交互监控知识激活，并采用标签精炼实现噪声鲁棒更新。

Result: 大量实验表明MIND框架显著优于现有认知编辑方法，在传统和元认知知识编辑基准上都取得了强劲性能。

Conclusion: 该研究填补了多模态大语言模型元认知知识编辑评估的空白，提出的MIND框架为提升模型自我意识和知识更新能力提供了有效解决方案。
...
### [250] [Hyperbolic Large Language Models](https://arxiv.org/abs/2509.05757)
*Sarang Patil,Zeyong Zhang,Yiran Huang,Tengfei Ma,Mengjia Xu*

Main category: cs.AI

TL;DR: 该论文综述了双曲几何在大语言模型中的应用，分析了四种主要技术类别，并探讨了潜在应用和未来研究方向。


<details>
  <summary>Details</summary>
Motivation: 现实世界数据具有高度非欧几里得的层次结构特性，而传统LLMs在处理这类数据时存在局限性，双曲几何能有效建模树状层次结构，因此需要系统研究双曲几何在LLMs中的应用。

Method: 提出双曲LLMs的分类法，包括四种主要技术：通过指数/对数映射的双曲LLMs、双曲微调模型、完全双曲LLMs和双曲状态空间模型。

Result: 建立了双曲LLMs的系统分类框架，总结了该领域的关键技术进展，并提供了相关论文、模型、数据集和代码的资源库。

Conclusion: 双曲几何为LLMs提供了强大的表示学习能力，特别是在处理层次结构数据方面具有优势，未来需要进一步探索其在不同领域的应用和发展新的双曲模型架构。
...
### [251] [DRF: LLM-AGENT Dynamic Reputation Filtering Framework](https://arxiv.org/abs/2509.05764)
...
<details>
  <summary>Details</summary>
Motivation: 随着生成式AI发展，基于大语言模型的多智能体系统面临性能量化困难和信誉评估机制缺失的挑战。

Method: 构建交互评分网络量化性能，设计信誉评分机制评估诚实度和能力，集成上置信界策略优化智能体选择。

Result: 实验表明DRF在逻辑推理和代码生成任务中显著提升了任务完成质量和协作效率。

Conclusion: DRF为多智能体系统处理大规模任务提供了新方法，有效解决了智能体性能量化和信誉评估问题。
...
### [252] [Decision-Focused Learning Enhanced by Automated Feature Engineering for Energy Storage Optimisation](https://arxiv.org/abs/2509.05772)
...
TL;DR: 提出AFE-DFL框架，结合自动特征工程和决策聚焦学习，在小型数据集上优化电池储能系统运营，相比传统方法降低运营成本22.9-56.5%


<details>
  <summary>Details</summary>
Motivation: 传统预测后优化方法存在预测误差传导问题，决策聚焦学习方法虽能克服但缺乏实际验证，且现实电池储能应用面临数据稀缺和变异性挑战

Method: 开发AFE-DFL框架，使用自动特征工程提取丰富特征表示，集成预测和优化过程，在真实英国房产数据集上进行验证

Result: DFL方法平均运营成本低于PTO，加入AFE后DFL性能提升22.9-56.5%，证明了DFL在实际应用中的可行性

Conclusion: 领域特定的自动特征工程能增强决策聚焦学习，减少对领域专业知识的依赖，为面临类似挑战的能源管理系统带来经济效益
...
### [253] [Chatbot To Help Patients Understand Their Health](https://arxiv.org/abs/2509.05818)
...
TL;DR: NoteAid-Chatbot是一个基于多智能体LLM和强化学习的对话AI，通过'学习即对话'框架帮助患者理解医疗信息，使用轻量级LLaMA 3.2 3B模型，无需人工标注数据就能实现清晰、相关的结构化对话。


<details>
  <summary>Details</summary>
Motivation: 患者需要具备必要的知识来积极参与自己的护理，但目前缺乏有效的工具来促进患者对医疗信息的理解。

Method: 使用多智能体大语言模型和强化学习框架，分两阶段训练：先在合成医疗对话数据上进行监督微调，然后通过基于患者理解评估的强化学习进行优化。

Result: NoteAid-Chatbot展现出关键的新兴行为（清晰性、相关性、结构化对话），在图灵测试中超越了非专家人类表现。

Conclusion: 基于PPO的奖励建模可以成功训练轻量级、领域特定的聊天机器人来处理多轮交互，整合多样化教育策略，并满足细微的沟通目标，展示了低成本RL在现实开放对话领域的应用前景。
...
### [254] [MapAgent: A Hierarchical Agent for Geospatial Reasoning with Dynamic Map Tool Integration](https://arxiv.org/abs/2509.05933)
*Md Hasebul Hasan,Mahir Labib Dihan,Mohammed Eunus Ali,Md Rizwan Parvez*

Main category: cs.AI

TL;DR: MapAgent是一个分层多智能体框架，专门用于地理空间推理任务，通过解耦规划和执行来提升LLM在地图交互和空间推理方面的能力。
...
Method: 采用分层多智能体架构：高层规划器分解复杂查询为子目标，路由到专用模块；针对地图工具设计专用地图工具代理，并行协调相关API；简单模块无需额外代理开销。
...
### [255] [Rethinking Reasoning Quality in Large Language Models through Enhanced Chain-of-Thought via RL](https://arxiv.org/abs/2509.06024)
*Haoyang He,Zihua Rong,Kun Ji,Chenyang Li,Qing Huang,Chong Xia,Lan Yang,Honggang Zhang*

Main category: cs.AI

TL;DR: 这篇论文提出了动态推理效率奖励(DRER)框架，通过精细的推理质量奖励和动态长度优势来改善大语言模型的逻辑推理能力，在Logictree数据集上达到GPT-o3-mini级别性能。


<details>
  <summary>Details</summary>
Motivation: 当前的强化学习奖励函数仅考虑答案格式和正确性，无法为推理过程提供信号，且任务特定训练无法控制逻辑深度，限制了模型真实推理能力的发挥。

Method: 提出DRER框架：(1)推理质量奖励：给能显著提高正确答案概率的推理链分配细粒度奖励；(2)动态长度优势：对偏离验证阈值的响应长度进行衰减处理，稳定训练。同时发布Logictree推理数据集用于训练和测试。

Result: 7B模型在400步训练后在Logictree上达到GPT-o3-mini级别性能，CoT增强答案的平均信心度提升30%，并在多种逻辑推理数据集和AIME24数学测试中展现良好的过适性。

Conclusion: DRER框架有效地控制了RL对CoT行为的形成，为提升大语言模型的形式推理能力提供了实用的方法。
...
### [256] [Reverse-Engineered Reasoning for Open-Ended Generation](https://arxiv.org/abs/2509.06160)
...
TL;DR: REER方法通过逆向工程从已知优质解决方案中发现潜在的深度推理过程，解决了开放创造性生成中的推理难题，超越了传统RL和蒸馏方法的局限


<details>
  <summary>Details</summary>
Motivation: 传统深度推理方法在开放性创造性生成任务中存在局限：强化学习缺乏明确奖励信号，指令蒸馏成本高昂且受限于教师模型能力

Method: 提出REER（逆向工程推理）范式，从已知优质解决方案反向计算发现潜在的逐步深度推理过程，采用可扩展的无梯度方法

Result: 构建了DeepWriting-20K大规模数据集，包含20,000条深度推理轨迹；DeepWriter-8B模型在开放生成任务中超越开源基线，性能与GPT-4o和Claude 3.5等专有模型相当甚至更优

Conclusion: REER为开放创造性生成任务提供了一种新的深度推理范式，通过逆向工程方法有效解决了传统方法的局限性，在性能上取得了显著突破
...
### [257] [From Long to Short: LLMs Excel at Trimming Own Reasoning Chains](https://arxiv.org/abs/2509.06174)
*Wei Han,Geng Zhan,Sicheng Yu,Chenyu Wang,Bryan Hooi*

Main category: cs.AI

TL;DR: EDIT方法通过约束引导生成和动态推理修剪，有效解决大型推理模型过度思考问题，在保持正确性的同时大幅缩短推理路径长度


<details>
  <summary>Details</summary>
Motivation: 大型推理模型(LRMs)在处理简单问题时容易过度思考，产生冗长复杂的推理路径，影响可解释性和用户体验

Method: 提出EDIT测试时缩放方法，通过约束引导生成同时跟踪长度和答案分布，动态选择最优平衡简洁性和正确性的推理路径

Result: 在多种模型和数据集上的实验表明，EDIT显著提高了推理效率，产生紧凑且信息丰富的输出，改善了可读性和用户体验

Conclusion: EDIT方法有效解决了LRMs在简洁性和正确性之间的平衡难题，为提升推理模型的实际应用价值提供了有效解决方案
...
### [258] [PillagerBench: Benchmarking LLM-Based Agents in Competitive Minecraft Team Environments](https://arxiv.org/abs/2509.06235)
*Olivier Schipper,Yudi Zhang,Yali Du,Mykola Pechenizkiy,Meng Fang*

Main category: cs.AI

TL;DR: 提出了PillagerBench框架用于评估Minecraft中的实时竞争多智能体系统，并开发了TactiCrafter智能体系统，该系统通过可读战术、因果依赖学习和对手策略适应，在团队对抗中表现出色。
...
Method: 开发了PillagerBench评估框架（提供可扩展API、多轮测试和基于规则的对手），并提出了TactiCrafter多智能体系统，该系统使用人类可读战术、学习因果依赖关系并适应对手策略。

Result: TactiCrafter在性能上优于基线方法，通过自我对弈展示了自适应学习能力，并在多轮游戏中表现出战略演化。
...
### [259] [Proof2Silicon: Prompt Repair for Verified Code and Hardware Generation via Reinforcement Learning](https://arxiv.org/abs/2509.06239)
...
<details>
  <summary>Details</summary>
Motivation: 解决LLM生成代码无法通过形式验证的问题，特别是在硬件和安全关键领域需要形式化验证的挑战。
...
Conclusion: 该工作展示了一个强大、可扩展的自动化流程，能够实现LLM驱动的形式化验证硬件合成，连接自然语言规范和硅实现。
...
### [260] [REMI: A Novel Causal Schema Memory Architecture for Personalized Lifestyle Recommendation Agents](https://arxiv.org/abs/2509.06269)
...
Method: 使用个人因果图谱记录用户生活事件和习惯，通过目标导向的因果遍历、外部知识增强和假设推理，检索可适应的计划模式来生成定制化行动方案，由大语言模型协调各组件

Result: 基于CSM架构的智能体相比基线LLM智能体能够提供更具情境感知和用户对齐的推荐
...
### [261] [TableMind: An Autonomous Programmatic Agent for Tool-Augmented Table Reasoning](https://arxiv.org/abs/2509.06278)
*Chuang Jiang,Mingyue Cheng,Xiaoyu Tao,Qingyang Mao,Jie Ouyang,Qi Liu*

Main category: cs.AI

TL;DR: TableMind是一个LLM驱动的表格推理代理，通过自主工具调用和代码执行实现精确数值推理，采用两阶段微调范式提升性能


<details>
  <summary>Details</summary>
Motivation: 现有文本方法在复杂数值计算方面存在不足，工具集成方法缺乏真正的自主适应性，需要开发能够自主执行多步推理的表格分析系统

Method: 采用两阶段微调：监督微调建立工具使用模式，强化微调优化多目标策略，提出Rank-Aware Policy Optimization (RAPO)算法

Result: 在多个主流基准测试中表现出优越性能，在推理准确性和计算精度方面都有显著提升

Conclusion: TableMind通过自主工具调用和代码执行有效解决了表格推理中的数值计算问题，两阶段微调方法显著提升了模型性能
...
### [262] [SFR-DeepResearch: Towards Effective Reinforcement Learning for Autonomously Reasoning Single Agents](https://arxiv.org/abs/2509.06283)
...
<details>
  <summary>Details</summary>
Motivation: 为大型语言模型配备复杂的交错推理和工具使用能力是智能体AI研究的关键，特别是在深度研究应用中需要大量搜索和推理

Method: 采用持续强化学习训练推理优化模型，使用完全合成数据，集成最小化网络爬虫和Python工具，实现动态决策的自主单智能体

Result: 最佳模型SFR-DR-20B在Humanity's Last Exam基准上达到28.7%的性能表现

Conclusion: 提出的简单RL配方能有效增强智能体技能同时保持推理能力，为自主单智能体深度研究提供了有效方法
...
### [263] [From Implicit Exploration to Structured Reasoning: Leveraging Guideline and Refinement for LLMs](https://arxiv.org/abs/2509.06284)
*Jiaxiang Chen,Zhuo Wang,Mingxi Zou,Zhucong Li,Zhijian Zhou,Song Wang,Zenglin Xu*

Main category: cs.AI

TL;DR: 提出了一个从隐式探索转向结构化推理的框架，通过指导原则和精炼机制来提升大语言模型的推理稳定性和性能
...
Method: 从成功轨迹中提取结构化推理模式，从失败中获取反思信号；在推理时遵循指导原则逐步执行，并在每一步后进行精炼以纠正错误

Result: 在BBH、GSM8K、MATH-500、MBPP、HumanEval等多个基准测试中 consistently 超越强基线模型

Conclusion: 结构化推理通过逐步执行和精炼提高了稳定性和泛化能力，指导原则具有良好的跨域迁移能力，支持跨模型协作，在效果和可扩展性上达到或超过监督微调
...
### [264] [Can AI Make Energy Retrofit Decisions? An Evaluation of Large Language Models](https://arxiv.org/abs/2509.06307)
*Lei Shu,Dong Zhao*

Main category: cs.AI

TL;DR: 评估7个大型语言模型在住宅能源改造决策中的表现，发现LLMs在技术目标上表现更好，但需要改进准确性、一致性和上下文处理能力
...
Method: 评估7个LLMs（ChatGPT、DeepSeek、Gemini、Grok、Llama和Claude）在住宅改造决策中的表现，使用包含49个州400个住宅的数据集，从准确性、一致性、敏感性和推理四个维度进行分析

Result: LLMs在许多情况下能生成有效建议，最高达到54.5%的top 1匹配率和92.8%的top 5匹配率。技术目标表现更强，模型间一致性低，对位置和建筑几何形状敏感但对技术和居住者行为不敏感

Conclusion: LLMs在能源改造决策中是有前途的助手，但需要在准确性、一致性和上下文处理方面进行改进才能可靠应用
...
### [265] [Large Language Models as Virtual Survey Respondents: Evaluating Sociodemographic Response Generation](https://arxiv.org/abs/2509.06337)
...
TL;DR: 该论文提出使用大语言模型(LLMs)模拟虚拟调查受访者的新范式，引入PAS和FAS两种模拟设置，并构建LLM-S^3基准套件来评估LLMs生成准确且人口统计一致的调查响应的能力。
...
Method: 提出Partial Attribute Simulation (PAS)和Full Attribute Simulation (FAS)两种模拟设置，使用GPT-3.5/4 Turbo、LLaMA 3.0/3.1-8B等主流LLMs，在包含11个真实世界公共数据集的LLM-S^3基准套件上进行评估。

Result: 研究揭示了预测性能的一致趋势，识别了失败模式，并证明了上下文和提示设计对模拟保真度的影响。

Conclusion: 这项工作为LLM驱动的调查模拟建立了严格的基础，为社会学研究和政策评估提供了可扩展且成本效益高的工具。
...
### [266] [Evaluating Multi-Turn Bargain Skills in LLM-Based Seller Agent](https://arxiv.org/abs/2509.06341)
*Issue Yishu Wang,Kakam Chong,Xiaofeng Wang,Xu Yan,DeXin Kong,Chen Ju,Ming Chen,Shuai Xiao,Shuguang Han,jufeng chen*

Main category: cs.AI

TL;DR: 提出了一个用于评估电商对话中卖家代理议价能力的多轮评估框架，重点关注买家意图的提取和追踪。


<details>
  <summary>Details</summary>
Motivation: 在线二手市场中，多轮议价是买卖双方互动的关键部分。LLM可以作为卖家代理在给定业务约束下与买家谈判，但需要准确追踪和理解长期谈判中的累积买家意图，这直接影响议价效果。

Method: 1) 构建大规模电商议价基准数据集（622个类别、9,892个产品、3,014个任务）；2) 基于心智理论(ToM)的轮级评估框架，包含标注的买家意图；3) 从海量对话数据中提取可靠意图的自动化流程。

Result: 开发了一个全面的评估框架，能够超越仅关注结果的指标，从对话过程中深入分析卖家代理的议价能力。

Conclusion: 该研究为LLM在电商议价场景中的应用提供了重要的评估工具和方法论，通过意图追踪和理论化评估框架，能够更准确地衡量卖家代理的谈判能力。
...
### [267] [A data-driven discretized CS:GO simulation environment to facilitate strategic multi-agent planning research](https://arxiv.org/abs/2509.06355)
*Yunzhe Wang,Volkan Ustun,Chris McGroarty*

Main category: cs.AI

TL;DR: DECOY是一个新颖的多智能体模拟器，将3D地形中的战略长期规划抽象为高级离散化模拟，同时保持低级别环境保真度。使用CS:GO作为测试平台，仅通过移动决策来准确模拟游戏玩法。


<details>
  <summary>Details</summary>
Motivation: 现代复杂多智能体交互的模拟环境需要在高度细节保真度和计算效率之间取得平衡。需要一种能够抽象战略规划同时保持环境真实性的模拟框架。

Method: 采用路径点系统简化和离散化连续状态和动作，配合基于真实CS:GO比赛数据训练的神经预测和生成模型来重建事件结果。无需显式建模瞄准和射击等低级机制。

Result: 广泛评估表明，从人类数据生成的DECOY回放与原始游戏中观察到的回放高度匹配。模拟环境能够准确重现游戏玩法。

Conclusion: DECOY提供了一个有价值的工具，可用于推进战略多智能体规划和行为生成的研究，为复杂多智能体交互提供了高效且高保真的模拟解决方案。
...
### [268] [Teaching AI Stepwise Diagnostic Reasoning with Report-Guided Chain-of-Thought Learning](https://arxiv.org/abs/2509.06409)
*Yihong Luo,Wenwu He,Zhuo-Xu Cui,Dong Liang*

Main category: cs.AI

TL;DR: DiagCoT是一个多阶段框架，通过监督微调通用视觉语言模型，仅使用自由文本报告模拟放射科医生的逐步诊断推理，显著提升了疾病分类、病理定位和报告生成性能。


<details>
  <summary>Details</summary>
Motivation: 现有的视觉语言模型在医疗诊断中缺乏放射科医生的逐步推理能力，需要一种能够从非结构化临床报告中学习结构化推理逻辑的方法。

Method: 采用三阶段方法：对比图像-报告调优实现领域对齐，思维链监督捕捉推理逻辑，强化学习调优使用临床奖励信号提高事实准确性和流畅性。

Result: 在MIMIC-CXR基准测试中，疾病分类AUC从0.52提升至0.76，病理定位mIoU从0.08提升至0.31，报告生成BLEU从0.11提升至0.33，在长尾疾病和外部数据集上优于最先进模型。
...
### [269] [Tree of Agents: Improving Long-Context Capabilities of Large Language Models through Multi-Perspective Reasoning](https://arxiv.org/abs/2509.06436)
*Song Yu,Xiaofei Xu,Ke Deng,Li Li,Lin Tian*

Main category: cs.AI

TL;DR: 提出了Tree of Agents (TOA)多智能体推理框架，通过将长输入分块处理并让智能体在树状结构中动态交换信息，有效解决LLM长上下文处理中的"中间信息丢失"问题。
...
Method: 多智能体推理框架，将输入分块由独立智能体处理，生成局部认知后沿树状路径动态交换信息进行协作推理，结合前缀哈希缓存和自适应剪枝策略。

Result: 使用紧凑的LLaMA3.1-8B模型，TOA在多种长上下文任务上显著优于多个基线，性能可与更大的商业模型（如Gemini1.5-pro）相媲美。

Conclusion: TOA框架通过多智能体协作推理有效缓解位置偏见和减少幻觉，在保持可比较API开销的同时实现了显著的性能提升。
...
### [270] [HyFedRAG: A Federated Retrieval-Augmented Generation Framework for Heterogeneous and Privacy-Sensitive Data](https://arxiv.org/abs/2509.06444)
*Cheng Qian,Hainan Zhang,Yongxin Tong,Hong-Wei Zheng,Zhiming Zheng*

Main category: cs.AI

TL;DR: HyFedRAG是一个面向混合数据模态的联邦RAG框架，通过边缘-云协作机制解决医疗领域异构隐私数据的检索问题，在保持数据隐私的同时提升检索质量和系统效率。


<details>
  <summary>Details</summary>
Motivation: 集中式RAG系统在处理异构和隐私敏感的医疗数据时面临困难，特别是在分布式医疗环境中，患者数据分布在SQL、知识图谱和临床笔记等多种格式中，且存在隐私约束限制罕见病例检索。

Method: 1) 基于Flower设计边缘-云协作RAG框架，支持查询结构化SQL数据、半结构化知识图谱和非结构化文档；2) 集成轻量级本地检索器和隐私感知LLM，提供三种匿名化工具生成去标识化摘要；3) 设计三级缓存策略（本地缓存、中间表示缓存、云推理缓存）优化延迟和减少冗余计算。

Result: 在PMC-Patients数据集上的实验结果表明，HyFedRAG在检索质量、生成一致性和系统效率方面优于现有基线方法。

Conclusion: 该框架为结构异构数据上的RAG提供了可扩展且符合隐私要求的解决方案，释放了LLM在敏感和多样化数据环境中的潜力。
...
### [271] [Accelerate Scaling of LLM Alignment via Quantifying the Coverage and Depth of Instruction Set](https://arxiv.org/abs/2509.06463)
*Chengwei Wu,Li Du,Hanyu Zhao,Yiming Ju,Jiapu Wang,Tengfei Pan*

Main category: cs.AI

TL;DR: 本文提出了一种新的指令数据选择方法，通过最大化指令深度和语义覆盖来可持续提升大语言模型在下游任务中的对齐性能。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型在下游任务中应用的增加，提高模型对齐性能和效率变得至关重要。但由于指令集分布的复杂性，当前方法无法随着指令池扩大而持续提升性能。

Method: 首先研究指令数据集分布与对齐模型性能关系的关键因素，发现指令深度和语义空间覆盖是决定性因素。然后设计指令选择算法同时最大化这两个指标。

Result: 实验结果表明，相比最先进的基线方法，该方法能以更快的速度可持续提升模型性能，实现"加速扩展"，关键因素能解释开发集上70%以上的模型损失。

Conclusion: 指令深度和语义覆盖是影响对齐模型性能的关键因素，提出的选择算法能有效实现性能的可持续提升和加速扩展。
...
### [272] [MAS-Bench: A Unified Benchmark for Shortcut-Augmented Hybrid Mobile GUI Agents](https://arxiv.org/abs/2509.06477)
...
<details>
  <summary>Details</summary>
Motivation: 当前缺乏系统评估GUI操作与快捷方式（API、深度链接等）混合智能体的基准框架，需要填补这一空白以推动更高效GUI智能体的发展。

Method: 构建包含139个跨11个真实应用复杂任务的基准，提供88个预定义快捷方式知识库，设计可通过纯GUI操作解决但能通过智能嵌入快捷方式显著加速的任务，使用7个评估指标来衡量智能体的快捷方式生成能力。

Result: 实验表明混合智能体相比纯GUI智能体实现了显著更高的成功率和效率，验证了评估方法的有效性。

Conclusion: MAS-Bench填补了关键评估空白，为创建更高效、鲁棒的智能体提供了基础平台，推动了GUI-快捷方式混合范式的发展。
...
### [273] [MORSE: Multi-Objective Reinforcement Learning via Strategy Evolution for Supply Chain Optimization](https://arxiv.org/abs/2509.06490)
*Niki Kotecha,Ehecatl Antonio del Rio Chanona*

Main category: cs.AI

TL;DR: 提出结合强化学习和多目标进化算法的方法，用于供应链动态多目标优化，通过MOEA搜索策略网络参数空间生成Pareto前沿策略，并引入CVaR进行风险敏感决策。
...
Method: 结合强化学习(RL)和多目标进化算法(MOEAs)，利用MOEA搜索策略神经网络的参数空间生成Pareto前沿策略，引入条件风险价值(CVaR)进行风险敏感决策。

Result: 在库存管理案例研究中表现出色，能够有效响应供应链动态变化，优于现有最先进方法。
...
### [274] [Scaling up Multi-Turn Off-Policy RL and Multi-Agent Tree Search for LLM Step-Provers](https://arxiv.org/abs/2509.06493)
*Ran Xin,Zeyu Zheng,Yanchen Nie,Kun Yuan,Xia Xiao*

Main category: cs.AI

TL;DR: BFS-Prover-V2系统通过多轮离线强化学习框架和规划增强的多智能体搜索架构，解决了LLM在定理证明中的训练和推理扩展问题，在数学基准测试中达到最先进性能


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在自动定理证明中面临训练时强化学习和推理时计算资源的双重扩展挑战，需要突破性能瓶颈

Method: 1) 受AlphaZero启发的多轮离线RL框架，采用自适应策略级数据过滤和周期性重训练；2) 规划增强的多智能体搜索架构，使用通用推理模型作为高层规划器分解复杂定理

Result: 在MiniF2F和ProofNet测试集上分别达到95.08%和41.4%的state-of-the-art性能

Conclusion: 该方法不仅适用于形式数学领域，还可推广到其他需要长视野多轮推理和复杂搜索的领域
...
### [275] [An AI system to help scientists write expert-level empirical software](https://arxiv.org/abs/2509.06503)
...
TL;DR: AI系统使用大语言模型和树搜索自动生成专家级科学软件，在多个领域超越人工方法


<details>
  <summary>Details</summary>
Motivation: 解决科学发现中软件开发的瓶颈问题，加速科学进步

Method: 结合大语言模型(LLM)和树搜索(TS)方法，系统性地优化质量指标并智能探索解决方案空间

Result: 在生物信息学中发现40种优于人工方法的新方法；流行病学中生成14个超越CDC模型的预测模型；在多个领域产生最先进软件
...
### [276] [CogGuide: Human-Like Guidance for Zero-Shot Omni-Modal Reasoning](https://arxiv.org/abs/2509.06641)
*Zhou-Peng Shou,Zhi-Qiang You,Fang Wang,Hai-Bo Liu*

Main category: cs.AI

TL;DR: 通过人类认知策略导向的"意图草稿"组件，提出了一种无需微调的多模态推理方案，有效抵御短便路径问题并提升上下文理解能力。
...
Method: 设计了一个插拔式三模块流水线：意图感知器、策略生成器和策略选择器，明确构建"理解-规划-选择"的认知过程。

Result: 在IntentBench、WorldSense和Daily-Omni数据集上验证了方法的普遍性和稳健收益，相比各自基准线，完整的"三模块"方案在不同推理引擎和流水线组合中均实现了一致收益，最高提升约9.51个百分点。

Conclusion: 信息论分析显示该过程能够降低条件熵并提高信息利用效率，从而抑制意外的短便路径推理，证明了"意图草稿"推理组件在零样本场景中的实用价值和可移植性。
...
### [277] [Reinforcement Learning Foundations for Deep Research Systems: A Survey](https://arxiv.org/abs/2509.06733)
*Wenjun Li,Zhi Chen,Jingru Lin,Hannan Cao,Wei Han,Sheng Liang,Zhi Zhang,Kuicai Dong,Dexun Li,Chen Zhang,Yong Liu*

Main category: cs.AI

TL;DR: 本论文是首个专注于深度研究系统强化学习基础的综述，系统化了以DeepSeek-R1为起点的RL方法在多步任务中的应用、挑战和最佳实践。


<details>
  <summary>Details</summary>
Motivation: 当前深度研究系统主要依靠SFT和DPO方法，但这些方法存在模仿偏差、暴露偏差、代理依赖性强、长期限任务信赖分配困难等限制，而强化学习能够更好地处理工具交互、探索和信赖分配问题。

Method: 论文从三个维度系统化RL在深度研究系统中的应用：(i)数据合成与管理；(ii)RL方法在程序性研究中的稳定性、样本效率、长上下文处理、奖励设计等方面；(iii)系统框架和基础设施。同时涵盖代理架构、协调机制以及评估标准。

Result: 论文总结了重复出现的模式，持续的基础设施瓶颈，并为培养健壮、透明的深度研究代理提供了实用指南。

Conclusion: 强化学习在深度研究系统中具有重要价值，能够充分利用环境反馈、支持探索和恢复行为、实现理论上的信赖分配，并减少对人类先验知识和评分偏差的依赖。本综述为该领域的研究者和实践者提供了丰富的技术见解和实践建议。
...
### [278] [VehicleWorld: A Highly Integrated Multi-Device Environment for Intelligent Vehicle Interaction](https://arxiv.org/abs/2509.06736)
*Jie Yang,Jiajun Chen,Zhangyue Yin,Shuo Chen,Yuxin Wang,Yiran Guo,Yuan Li,Yining Zheng,Xuanjing Huang,Xipeng Qiu*

Main category: cs.AI

TL;DR: 这篇论文提出了一种新的State-based Function Call (SFC)方法，通过维护显式系统状态意识和直接状态迁移，在车辆室内环境控制任务中显著超越传统函数调用方法。


<details>
  <summary>Details</summary>
Motivation: 智能车辆室对API代理提出了独特挑战，需要协调多个紧密耦合子系统。传统函数调用方法存在无状态运行、多次探索调用和错误恢复能力局限等问题。

Method: 研究者首先构建了VehicleWorld环境，包含30个模块、250个API和680个属性。通过系统分析发现直接状态预测在环境控制中更优，从而提出SFC方法，维护显式系统状态意识并实现直接状态迁移。

Result: 实验结果显示SFC方法显著超过传统FC方法，实现了更高的执行准确性和更低的延迟。

Conclusion: 论文证明了维护状态意识对于复杂系统控制的重要性，SFC方法为车辆室API代理提供了更高效的解决方案，并已开源公布所有实现代码。
...
### [279] [Another Turn, Better Output? A Turn-Wise Analysis of Iterative LLM Prompting](https://arxiv.org/abs/2509.06770)
*Shashidhar Reddy Javaji,Bhavul Gauri,Zining Zhu*

Main category: cs.AI

TL;DR: 这篇论文提出了一个评估框架，用于量化大语言模型在多轮迭代精细化任务中的表现，包括思想创造、代码生成和数学解题三个领域。


<details>
  <summary>Details</summary>
Motivation: 虽然大语言模型被广泛用于多轮工作流程，但目前缺乏明确的方法来量化迭代精细化的效果，不知道什么时候迭代有帮助什么时候反而有害。

Method: 设计了一个评估协议，在每个任务上进行12轮的受控对话，使用从模糊"改进它"反馈到有针对性导航的多种提示词，并记录每轮输出。采用领域适宜的检查方式（代码用单元测试，数学用答案等效性加推理合理性，思想创造用原创性和可行性）进行评分。

Result: 不同领域的收益差异显著：思想创造和代码在前几轮就有收益，而数学需要后期轮次的详细说明才能获得收益。模糊反馈在前几轮后常平台或造成正确性倒退，而有针对性提示能可靠地推动目标质量轴的改善。

Conclusion: 该框架和指标使得迭代过程可裁量化并在不同模型间可比较，为什么时候应该导航、停止或更换策略提供了信号。
...
### [280] [RAFFLES: Reasoning-based Attribution of Faults for LLM Systems](https://arxiv.org/abs/2509.06822)
...
<details>
  <summary>Details</summary>
Motivation: 当前的长时程多组件LLM代理系统评估方法存在局限，主要关注单一指标、端到端结果和人类偏好，难以识别系统故障的具体位置和原因。需要能够推理、探测、迭代和理解复杂逻辑的评估框架。

Method: RAFFLES采用迭代式多组件流水线架构，包含一个中央Judge系统性地调查故障，以及一组专门的Evaluators评估系统组件和Judge自身的推理质量，建立假设历史记录。

Result: 在Who&When数据集上，RAFFLES在算法生成数据集上达到43%以上的代理-步骤故障对准确率（相比之前最佳16.6%大幅提升），在手工制作数据集上超过20%（超越之前最佳8.8%）。

Conclusion: RAFFLES代表了向自主系统自动化故障检测迈出的关键一步，有望替代劳动密集型的人工审查，显著提升长时程多组件LLM代理系统的评估能力。
...
### [281] [Test-Time Scaling in Reasoning Models Is Not Effective for Knowledge-Intensive Tasks Yet](https://arxiv.org/abs/2509.06861)
*James Xu Zhao,Bryan Hooi,See-Kiong Ng*

Main category: cs.AI

TL;DR: 测试时扩展在知识密集任务中效果有限，并不肯定提高准确性，反而可能导致更多幻觉


<details>
  <summary>Details</summary>
Motivation: 评估测试时扩展在知识密集任务上的效果，因为这类任务需要高事实准确性和低幻觉率

Method: 使用12个推理模型在2个知识密集测试集上进行综合评估，分析扩展思考对幻觉行为的影响

Result: 增加测试时计算并不肯定提高准确性，反而导致更多幻觉；减少的幻觉多数来自模型选择放弃回答，而非改善事实记忆

Conclusion: 虽然扩展思考存在限制，但与不思考相比，允许思考仍然有益；扩展推理可能导致确认偏见，产生过信任的幻觉
...
### [282] [Paper2Agent: Reimagining Research Papers As Interactive and Reliable AI Agents](https://arxiv.org/abs/2509.06917)
*Jiacheng Miao,Joe R. Davis,Jonathan K. Pritchard,James Zou*

Main category: cs.AI

TL;DR: Paper2Agent是一个自动将研究论文转换为AI代理的框架，通过分析论文和代码库构建MCP服务器，使静态论文变成可交互的AI研究助手
...
Method: 使用多代理系统分析论文和代码库，构建Model Context Protocol服务器，通过迭代生成和运行测试来优化MCP，支持自然语言查询和原始论文工具调用

Result: 成功创建了基于AlphaGenome、ScanPy和TISSUE的论文代理，能够复现原始论文结果并正确执行新用户查询

Conclusion: Paper2Agent将静态论文转化为动态交互AI代理，为知识传播和AI协作生态系统奠定了基础
...
### [283] [Directly Aligning the Full Diffusion Trajectory with Fine-Grained Human Preference](https://arxiv.org/abs/2509.06942)
*Xiangwei Shen,Zhimin Li,Zhantao Yang,Shiyi Zhang,Yingfang Zhang,Donghao Li,Chunyu Wang,Qinglin Lu,Yansong Tang*

Main category: cs.AI

TL;DR: 通过提前定义噪声先验和语义相对偏好优化方法，解决了模型与人类偏好对齐中的计算成本高和需要离线调整奖励模型的问题


<details>
  <summary>Details</summary>
Motivation: 现有方法在对齐模型与人类偏好时存在计算成本高（需要多步去噪计算奖励）和需要离线调整奖励模型的问题

Method: 提出Direct-Align方法，通过提前定义噪声先验来恢复原始图像，避免后期时间步的过度优化。同时提出语义相对偏好优化(SRPO)，将奖励形式化为文本条件信号，支持圧线奖励调整

Result: 在FLUX.1.dev模型上进行精调，将人类评估的实体感和美学质量提升了3倍以上

Conclusion: 该方法有效解决了模型与人类偏好对齐的计算效率和奖励调整问题，显著提升了生成图像的质量
