...
TL;DR: nuCarla是一个基于CARLA模拟器构建的大规模BEV感知数据集，解决了端到端自动驾驶中缺乏标准化闭环测试数据的问题，提供与nuScenes格式兼容的数据和模型基准。


<details>
  <summary>Details</summary>
Motivation: 现有自动驾驶数据集主要来自真实世界的非交互条件，仅支持开环学习，缺乏标准化、大规模且经过验证的数据集来支持有意义的中间表示学习，导致闭环端到端模型性能落后于简单基于规则的基线。

Method: 在CARLA模拟器中构建nuScenes风格的大规模BEV感知数据集，具有与nuScenes格式完全兼容、数据集规模相当但类别分布更均衡、可直接用于闭环仿真部署等特点。

Result: nuCarla提供了高性能的BEV骨干网络，实现了最先进的检测结果，为闭环端到端开发提供了数据和模型基准。

Conclusion: nuCarla通过提供开放的数据和模型基准，显著加速了闭环端到端自动驾驶的发展，为可靠和安全感知的研究铺平了道路。
...
TL;DR: 提出一个框架解决开放集识别中的过度自信问题，通过扰动不确定性估计和两阶段未知检测来提升性能
...
Method: 包含两个组件：基于扰动的不确定性估计模块（通过参数扰动生成多样预测）和两阶段未知检测模块（使用学习型分类器）
...
TL;DR: 提出了一种用于少样本视频目标检测的新方法，通过选择性传播高置信度目标特征来实现时间一致性，在多个数据集上取得了显著性能提升。


<details>
  <summary>Details</summary>
Motivation: 解决传统目标检测方法需要大量训练数据的问题，以及视频中由于遮挡和外观变化导致的时间一致性挑战，同时避免使用计算复杂的区域建议方法。

Method: 采用目标感知的时间建模方法，包含过滤机制选择性传播高置信度目标特征，使用少样本训练的检测和分类头进行聚焦特征传播，无需显式目标管建议。

Result: 在5-shot设置下，AP分别提升3.7%(FSVOD-500)、5.3%(FSYTV-40)、4.3%(VidOR)和4.5%(VidVRD)，在1-shot、3-shot和10-shot配置下也表现出改进。

Conclusion: 该方法在少样本视频目标检测中实现了稳健的时间一致性和检测精度，无需依赖复杂的区域建议机制，代码已开源。
...
Method: 采用流匹配范式进行直接概率传输，收集多个SOTA模型融合结果作为伪标签，使用任务感知选择函数选择可靠伪标签，并引入融合精炼模块和持续学习机制。

Result: 在多种融合任务中取得有竞争力的性能，显著提高采样效率，同时保持轻量级模型设计。

Conclusion: 该方法为多模态图像融合提供了一种高效、可扩展的解决方案，平衡了性能、效率和模型复杂度。
...
TL;DR: 提出了一种两阶段无轨迹碰撞检测框架，通过扩散模型生成合理的未来路段地图并与实际监测地图比较来检测碰撞
...
Method: 两阶段方法：第一阶段使用Mapfusion扩散模型生成未来路段地图，第二阶段通过比较监测地图与生成地图来检测碰撞

Result: Mapfusion成功生成基于学习运动模式的真实路段演化地图，在不同采样间隔下保持鲁棒性，真实世界碰撞实验表明该方法能准确检测碰撞

Conclusion: 提出的两阶段方法在准确检测碰撞方面有效，扩散模型能够生成合理的未来路段地图用于碰撞检测
...
<details>
  <summary>Details</summary>
Motivation: 现有视觉Transformer在处理地震数据时忽略了其内在模式，无法有效捕捉地震数据中的高频和低频信息，需要专门针对地震数据特性的基础模型训练方法。

Method: 使用频谱分解分离高低频分量，采用分层希尔伯特编码表示数据，提出自适应训练策略：先关注粗粒度信息，再逐步细化到细粒度特征。

Result: 实验证明该方法在地震基础模型预训练中具有有效性和高效性。

Conclusion: 基于地震数据高低频特征的数据编码和训练策略对于提升视觉地震基础模型预训练至关重要。
...
TL;DR: 该论文开发了一种基于面部微动态分析的被动痴呆筛查方法，通过分析眨眼动态、嘴部运动、注视变化和头部微调等面部运动特征，无需语言或文本即可进行早期神经认知变化检测。


<details>
  <summary>Details</summary>
Motivation: 现有痴呆筛查方法主要依赖语音或脚本访谈，限制了在临床外的应用，且预测结果与语言和转录紧密耦合。本文旨在开发一种无需语言干预、可在自然环境中大规模应用的被动筛查方法。

Method: 通过稳定面部信号，将面部微动作转换为可解释的微动态时间序列，平滑处理后将短窗口汇总为紧凑的片段级统计量。分析运动在不同通道的分布而非幅度，使每个通道的影响透明化。

Result: 在YT DemTalk数据集上，消融实验显示注视不稳定性和嘴部/下颌动态是最具信息量的线索，轻量级分类器可获得AUROC 0.953、AP 0.961、F1分数0.851和准确率0.857的痴呆预测性能。

Conclusion: 面部微动态分析为语言无关的痴呆筛查提供了有效方法，能够在自然环境中进行大规模应用，且性能优异，具有跨设备、跨主题和跨文化的可转移性。
...
TL;DR: Gen-ViRe是一个评估视频生成模型作为推理器的基准框架，将链式帧推理分解为6个认知维度和24个子任务，填补了现有基准无法评估多步规划、算法逻辑等核心认知能力的空白。
...
Method: 基于认知科学和实际AI应用，开发了Gen-ViRe框架，通过多源数据整理、最小提示协议和混合VLM辅助评估来量化视频模型的推理能力。

Result: 实验发现最先进系统在视觉质量和实际推理深度之间存在显著差距，为推进真正世界模拟器建立了基线和诊断工具。

Conclusion: Gen-ViRe提供了首个对视频模型作为推理器的定量评估，揭示了当前模型的局限性，并为开发更强大的世界模拟器提供了指导。
...
TL;DR: 该论文提出基于排序的损失函数来解决热图姿态估计中的三个主要问题：MSE损失无法有效提升关节定位精度、热图空间和类别不平衡、以及评估指标与损失函数之间的不一致性。


<details>
  <summary>Details</summary>
Motivation: 现有热图姿态估计方法存在三个问题：(1) MSE损失对所有像素偏差同等惩罚，无法专注于锐化峰值和精确定位关节；(2) 热图存在空间和类别不平衡；(3) 评估指标(mAP)与损失函数之间存在差异。
...
Conclusion: 这是首个提出与姿态估计评估指标(mAP)对齐的损失函数的工作。基于排序的损失函数能够显著提升热图姿态估计的性能，特别是在提高置信度与定位质量相关性方面表现出色。
...
TL;DR: 提出碰撞声源分割任务(CS3)，从碰撞声音中识别并分割视觉场景中的相关物体，利用基础模型和弱监督方法在自我中心视频中实现音频条件分割。


<details>
  <summary>Details</summary>
Motivation: 受人类多感官感知能力启发，希望从碰撞声音中识别产生声音的物体，特别是在自我中心视频中，声音清晰但视觉场景杂乱、物体小、交互短暂。

Method: 使用弱监督的音频条件分割方法，结合基础模型(CLIP和SAM2)，并融入自我中心线索(如手中物体)来识别可能的碰撞声源物体。

Result: 在两个新基准测试EPIC-CS3和Ego4D-CS3上，分别比竞争基线方法在mIoU指标上提升了3倍和4.7倍。

Conclusion: 提出的CS3任务和方法能有效从碰撞声音中分割视觉场景中的声源物体，特别是在自我中心视频中表现出色。
...
TL;DR: 提出几何表示回归(GRR)范式，通过预测解耦的几何表示（射线束方向和点图）来估计相机位姿，在7-Scenes和Cambridge Landmarks数据集上达到最先进性能


<details>
  <summary>Details</summary>
Motivation: 传统绝对位姿回归(APR)模型作为黑盒直接回归6自由度位姿，容易记忆训练视图而非理解3D场景几何。需要一种几何基础的方法来提升泛化能力

Method: 将APR重新表述为逆渲染过程，预测两个解耦的几何表示：射线束方向估计相机旋转，点图估计相机平移，然后通过可微分确定性求解器恢复最终位姿

Result: 在7-Scenes和Cambridge Landmarks数据集上实现了最先进的性能，验证了建模逆渲染过程是更鲁棒的绝对位姿估计路径

Conclusion: 几何表示回归通过解耦旋转和平移预测，引入强几何先验，为通用绝对位姿估计提供了更稳健的方法
...
<details>
  <summary>Details</summary>
Motivation: 膀胱癌复发率高达78%，术后监测至关重要。多序列增强MRI是常用检测方法，但术后组织变化使诊断困难，且缺乏专门用于复发预测的多序列MRI数据集。

Method: 构建专用多序列多模态MRI数据集，提出H-CNN-ViT分层门控注意力多分支模型，独立处理各模态，根据上下文需求选择性加权全局(ViT)和局部(CNN)路径特征。

Result: 在自建数据集上，H-CNN-ViT模型达到78.6%的AUC，超越了现有最先进模型。

Conclusion: 该研究填补了膀胱癌复发预测专用数据集的空白，提出的H-CNN-ViT模型通过有效的特征融合策略显著提升了复发预测性能。
...
<details>
  <summary>Details</summary>
Motivation: CLIP的文本编码器最多只能处理77个token，限制了其在信息丰富的长文本放射学报告中的表示能力。现有的领域特定编码器虽然缓解了这个问题，但仍受限于512个token的输入长度和相对浅层的语义理解。
...
<details>
  <summary>Details</summary>
Motivation: 传统迁移学习方法在医学图像高维背景下存在局限，无法充分提取解剖特征，需要克服这些约束以提升分类性能。

Method: 集成EfficientNet和VGG19，加入伪牛顿增强层（智能调整特征权重）和稀疏诱导特征减少层（去除冗余特征），形成多层框架。

Result: 相比基线模型EfficientNet，性能显著提升：精度0.9、召回率0.861、F1分数0.88、损失0.18、准确率88.1%。

Conclusion: 该架构克服了传统迁移学习的限制，在医学图像分类中表现优异，有助于开发自动化诊断工具。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法依赖手工制作的提示词且性能不稳定，限制了在真实场景中的鲁棒性和泛化能力。作者发现现有VLM在高级决策任务中存在关键局限性。

Method: 利用VLM的强大场景理解能力丰富现有视觉基准，引入多模态交互架构融合视觉和语言特征，并设计了后处理细化模块来增强预测可靠性。
...
<details>
  <summary>Details</summary>
Motivation: 医学解剖分割领域的数据缩放规律研究不足，而解剖结构在图像间具有拓扑同构性，这启发了通过变形增强策略来改善数据缩放规律。

Method: 分析了15个语义任务和4种成像模态的缩放规律，评估了随机弹性变形和配准引导变形的效果，并提出基于图像配准生成微分同胚映射的新增广方法。

Result: 实验结果显示，配准和生成变形增强都显著提高了数据利用效率，其中生成变形方法实现了更优的性能和更快的收敛速度。

Conclusion: 这项工作为理解医学图像分割的可扩展性和拓扑变化影响提供了见解，能够以更低的标注和计算成本实现更高效的模型开发。
...
<details>
  <summary>Details</summary>
Motivation: 现有血液病理学分析方法（单任务、视觉语言、WSI优化或单细胞模型）无法提供跨复杂数字血液病理学的统一多任务多模态推理能力。

Method: 基于Hema-Former多模态模块，在视觉和文本表示层次上构建桥梁，支持检测、分类、分割、形态学、掩码语言建模和视觉问答等不同粒度的任务。
...
TL;DR: 提出首个弱监督管道用于检测短暂冲沟，利用视觉语言模型减少手动标注工作量，并发布了首个用于半监督检测的遥感图像数据集。


<details>
  <summary>Details</summary>
Motivation: 短暂冲沟是农业领域的重要侵蚀问题，但传统计算机视觉方法难以自动检测，且标注数据稀缺，限制了机器学习方法的应用。
...
Conclusion: 该方法有效解决了短暂冲沟检测中的数据稀缺问题，为土壤侵蚀监测提供了可行的弱监督解决方案。
...
TL;DR: 提出基于压缩视频运动向量的可扩展框架，用于评估生成视频的时间真实性和改进判别模型的时间推理能力。
...
Method: 从H.264和HEVC等压缩视频流中提取运动向量，使用KL、JS和Wasserstein散度量化真实视频与生成视频之间的运动统计差异，并研究MV-RGB融合方法。

Result: 实验显示Pika和SVD在熵基散度上最接近真实视频，VC2和Text2Video-Zero在MV-sum统计上表现最佳，CogVideo在所有指标上偏差最大。MV-RGB融合显著提升了分类准确率。
...
<details>
  <summary>Details</summary>
Motivation: 现有MCVT研究过分强调准确性而忽视实时性能和可扩展性，这在城市规模应用中随着摄像头数量增加而变得更具挑战性。

Method: 系统包含多个边缘设备和一个中央工作站。边缘侧处理RTSP视频流，包括目标检测、跟踪、地理映射和特征提取，只传输车辆位置和深度外观特征。中央侧基于自监督相机链接模型学习相邻相机间的时空关系进行跨摄像头关联。
...
<details>
  <summary>Details</summary>
Motivation: 评估AI系统（特别是自动驾驶车辆中的AI系统）对道路安全规范的理解能力，确保其遵守道路安全规范

Method: 使用从学校教科书收集的交通标志和道路安全规范图像构建数据集，在零样本设置下评估多模态大语言模型

Result: 初步结果显示这些模型在安全推理方面表现不佳，揭示了人类学习与模型解释之间的差距

Conclusion: 多模态大语言模型在道路安全概念理解方面存在性能差距，需要进一步研究来改进
...
TL;DR: 本文提出CuRPO方法，通过基于课程学习的策略优化来解决CoT推理在视觉定位任务中的性能下降问题，利用CoT长度和gIoU奖励作为复杂度指标来结构化训练数据。


<details>
  <summary>Details</summary>
Motivation: 研究发现强化学习微调的CoT推理在视觉定位任务中会随着CoT输出变长或复杂而性能下降，且数据集规模增大并不总是提升性能，这促使作者开发新的训练策略。

Method: 提出Curriculum-based Relative Policy Optimization (CuRPO)，使用CoT长度和广义交并比(gIoU)奖励作为复杂度指标，从简单到复杂逐步结构化训练数据。

Result: 在RefCOCO、RefCOCO+、RefCOCOg和LISA数据集上的实验显示，CuRPO优于现有方法，在RefCOCO上提升达+12.52 mAP，且在少样本学习场景下表现出色。

Conclusion: CuRPO是一种高效且鲁棒的方法，特别适用于具有模糊复杂文本描述的视觉定位任务，能够显著提升性能。
...
TL;DR: 提出基于聚类的帧选择策略，通过将视觉相似的帧分组后再划分数据集，减少信息泄漏，生成更具代表性、平衡和可靠的数据集划分


<details>
  <summary>Details</summary>
Motivation: 解决视频衍生帧数据集中存在的信息泄漏问题，提高数据集划分的质量和可靠性

Method: 使用聚类方法将视觉相似的帧分组，然后在组内进行训练集、验证集和测试集的划分

Result: 该方法能够生成更具代表性、平衡和可靠的数据集分区

Conclusion: 基于聚类的帧选择策略是减少视频数据集信息泄漏的有效方法
...
TL;DR: 通过在程序生成数据上预训练视觉Transformer，可以显著提高数据效率、收敛速度和下游任务性能，1%的程序数据相当于28%的ImageNet数据效果


<details>
  <summary>Details</summary>
Motivation: 探索通过程序生成的无视觉语义内容数据来为视觉Transformer注入通用归纳偏置，使其能够学习抽象计算先验

Method: 使用形式语法等简单算法生成无视觉语义内容的数据，在视觉Transformer的视觉补丁嵌入机制之前进行预训练，然后进行标准图像训练
...
TL;DR: 提出了一种基于标量场的细胞图像分割方法，通过训练网络学习连续标量场，使用分水岭方法进行分割，无需正则化即可获得稳健的细胞实例分割结果。
...
Method: 使用泊松偏微分方程和热方程稳态解的扩散方程作为标量场，通过最小化场残差来获得解，无需正则化，使用U-Net网络进行训练。

Result: 在公共数据集上取得了有竞争力的结果，能够实现优秀的细胞分割效果，同时具有简化实现、降低能耗和小内存占用的优势。

Conclusion: 这种新颖、简单但具有几何洞察力的方法能够实现出色的细胞分割结果，特别适合边缘计算场景。
...
TL;DR: EchoAgent是一个基于大语言模型控制的框架，通过协调专门的视觉工具实现超声心动图的自动化分析，包括时间定位、空间测量和临床解释。
...
Method: EchoAgent在大语言模型控制下协调专门的视觉工具，执行时间定位、空间测量和临床解释。关键贡献是测量可行性预测模型，用于确定解剖结构是否可可靠测量，实现自主工具选择。
...
TL;DR: 提出了CrossTrainer方法，通过发现跨运动通用的技能属性（如平衡、控制、手部位置），训练多模态语言模型为视频生成可操作的反馈和熟练度评估，在跨运动和同运动场景下相比现有技术提升达60%。


<details>
  <summary>Details</summary>
Motivation: 解决当前技能评估模型局限于单一运动、专家监督成本高且稀缺的问题，特别是针对小众运动的长尾分布挑战。

Method: 发现跨运动通用的技能属性，训练多模态语言模型生成针对新视频的可操作反馈和熟练度等级评估。

Result: 在多个数据集上验证，跨运动和同运动场景下相比现有技术提升达60%，视频表示泛化能力显著优于现有技术。

Conclusion: 通过抽象出指示人类技能的共享行为，所提出的视频表示比现有技术泛化能力更强，丰富了当前的多模态大语言模型。
...
Result: 在公共多对比度MRI数据集上，CD-DPE在重建精细细节方面优于最先进方法，在未见数据集上表现出强大的泛化能力。

Conclusion: CD-DPE方法有效解决了多对比度MRI超分辨率中的特征整合问题，具有优异的性能和泛化能力。
...
TL;DR: RISE是首个基于单静态雷达的室内场景理解基准系统，通过利用多径反射的几何线索，实现了布局重建和物体检测，在保护隐私的同时提升了室内场景理解的性能。
...
TL;DR: 开发了一个用于自动识别MRI切片解剖平面（轴位、冠状位、矢状位）的2.5D上下文感知分类器，显著提高了平面识别准确性，并在脑肿瘤检测任务中验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: MRI图像中缺少平面方向元数据会复杂化分析，增加异质数据集合并时的域偏移，并降低诊断分类器的准确性。人工识别解剖平面容易，但自动化系统难以完成此任务。

Method: 采用2.5D上下文感知模型，利用多切片信息避免单个切片的歧义性，实现稳健的特征学习。在3D切片序列和静态2D图像上训练模型。

Result: 2D参考模型准确率达到98.74%，2.5D方法提升至99.49%，错误率降低60%。在脑肿瘤检测任务中，基于不确定度评分的门控策略将准确率从97.0%提升至98.0%，误诊率降低33.3%。
...
TL;DR: LINGUAL框架通过自然语言指令和上下文学习实现医学图像分割，相比传统主动学习减少约80%的标注时间，在主动域适应任务中达到或超越基线性能。


<details>
  <summary>Details</summary>
Motivation: 医学图像分割中边界模糊和标注任务认知负荷高的问题，传统主动学习需要专家标注整个感兴趣区域，标注成本与区域大小相关。

Method: 提出LINGUAL框架：接收专家自然语言指令，通过上下文学习转换为可执行程序，自动执行相应子任务序列，无需人工干预。

Result: 在主动域适应任务中，LINGUAL达到或优于主动学习基线性能，同时估计标注时间减少约80%。

Conclusion: 语言引导为医学图像分割提供了一种高效替代方案，显著降低专家标注负担，同时保持或提升分割性能。
...
<details>
  <summary>Details</summary>
Motivation: 随着视觉生成模型的快速发展，AI生成图像检测变得至关重要。传统方法需要针对特定数据集训练检测器，而本文研究无需训练的方法，利用自监督模型而无需先验数据知识。

Method: 基于自监督模型对RandomResizedCrop增强的鲁棒性，WaRPAD首先定义基础评分函数量化图像嵌入对Haar小波分解提取的高频方向扰动的敏感性。然后将图像缩放到模型输入大小的倍数，分割成小块，计算每个块的基础评分，最后对所有块的评分取平均得到最终检测分数。

Result: 在真实数据集和23种不同生成模型生成的图像上验证，WaRPAD始终实现竞争性性能，并对测试时损坏表现出强鲁棒性。

Conclusion: WaRPAD是一种有效的无需训练AI生成图像检测方法，由于RandomResizedCrop不变性是自监督模型的常见训练方案，该方法适用于各种自监督模型。
...
TL;DR: 提出了FashionMAC框架，通过无变形直接外推分割服装的方法，实现高质量可控的时尚展示图像生成，解决了现有方法中服装纹理失真和细粒度控制不足的问题。


<details>
  <summary>Details</summary>
Motivation: 现有服装中心时尚图像生成方法存在两个关键问题：(1) 服装变形导致纹理失真，(2) 缺乏对生成模型细粒度属性的控制机制。

Method: 提出无变形的扩散框架，直接外推分割服装；引入区域自适应解耦注意力机制(RADA)和链式掩码注入策略，实现细粒度外观控制。

Result: 实验验证了该框架相比现有最先进方法的优越性能，能够高质量地保留服装细节并实现精细控制。

Conclusion: FashionMAC框架成功解决了服装细节保留和细粒度控制的问题，为电子商务应用提供了实用的时尚展示图像生成方案。
...
TL;DR: 提出了一种基于潜在扩散模型的洪水地图超分辨率方法，能够在保持精度的同时显著减少计算时间，并提高对未见区域的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 传统基于物理的水动力模型计算密集，不适用于实时大规模应用；现有卷积神经网络方法泛化能力有限。

Method: 使用潜在扩散模型对粗网格洪水地图进行超分辨率处理，结合物理信息输入增强可解释性。

Result: 潜在扩散模型显著减少了生成高保真洪水地图的计算时间，同时保持精度，并展现出优越的跨地理位置泛化能力。

Conclusion: 该方法实现了实时洪水风险管理，通过迁移学习加速对新地理区域的适应，解决了机器学习黑盒行为的局限性。
...
TL;DR: 提出一种基于无人机图像自动检测、定位和分类混凝土桥梁结构缺陷的新方法，包含显著性缺陷区域提议和YOLOX深度学习检测器两个主要阶段。


<details>
  <summary>Details</summary>
Motivation: 异常物体检测和分类是计算机视觉和模式识别中的主要挑战任务，需要开发有效的桥梁缺陷检测方法。

Method: 第一阶段使用显著性进行缺陷区域提议，第二阶段在通过边界框级亮度增强获得的显著性增强图像上使用YOLOX深度学习检测器。

Result: 在标准数据集上的实验结果证实了该框架的性能及其在准确性和计算效率方面的适用性。

Conclusion: 该方法具有在自供电检查系统中实施的巨大潜力。
...
Conclusion: SCAR克服了基于向量量化的预填充方法的语义限制和高成本，在保持灵活性和通用性的同时，显著提升了自回归模型在图像编辑任务中的性能。
...
Method: 使用高效分割解码器生成对象掩码作为语义先验，指导视觉token合并为紧凑的对象中心表示；采用质心引导排序机制恢复空间顺序。
...
TL;DR: 提出ZS-TMS框架，使用预训练生成引擎直接合成任务特定分类器的参数，仅需单张图像和文本描述即可实现1-shot分类，在医学图像分析中显著优于现有少样本学习方法。


<details>
  <summary>Details</summary>
Motivation: 解决医学图像分析中对大规模标注数据的依赖问题，特别是在罕见疾病等数据稀缺场景中，传统深度学习模型难以有效应用。

Method: 使用语义引导参数合成器(SGPS)，输入单张示例图像和临床文本描述，通过预训练生成引擎直接合成轻量级分类器的全部参数，无需任务特定训练。

Result: 在ISIC 2018皮肤病变数据集和自定义罕见疾病数据集上，1-shot和5-shot分类任务中显著优于先进的少样本和零样本学习方法，达到新的最优性能。
...
TL;DR: 开发了一个全自动深度学习管道，用于在三维CT扫描上测量肩关节盂骨缺损，该方法比手动测量更快速可靠，且超过了外科医生之间的一致性。


<details>
  <summary>Details</summary>
Motivation: 当前测量肩关节盂骨缺损的手动和半自动方法耗时且存在读者间变异性，需要开发更高效可靠的自动化测量工具。

Method: 采用多阶段算法：1) 使用U-Net自动分割肩关节盂和肱骨；2) 使用第二个网络预测肩关节盂边缘点；3) 应用PCA、投影和圆拟合计算骨缺损百分比。

Result: 自动测量与共识读数高度一致（ICC 0.84 vs 0.78），在低骨缺损和高骨缺损亚组中表现优于外科医生一致性（ICC 0.71 vs 0.63和0.83 vs 0.21）。
...
<details>
  <summary>Details</summary>
Motivation: 现有3D-LLMs在准确将语言定位到3D环境中的视觉和空间元素方面存在限制，这源于训练数据侧重于语言推理而非空间理解，导致固有的定位偏差未解决。

Method: 提出DEER-3D框架，采用"分解、诊断评估、编辑、重新训练"的工作流程。识别3D-LLM定位失败后，首先诊断具体的谓词级错误（如属性或空间关系），然后执行最小化的谓词对齐3D场景编辑（如重新着色或重新定位），生成针对性反事实监督用于迭代模型微调。

Result: 在多个3D定位和场景理解任务的基准测试中评估编辑流程，通过迭代优化在所有评估数据集上持续展示改进效果。

Conclusion: DEER-3D强调了针对性、错误驱动的场景编辑在弥合3D LLMs中语言推理能力与空间定位方面的有效性。
...
TL;DR: 提出GCA-ResUNet网络，通过集成分组坐标注意力机制来增强医学图像分割的全局建模能力，在保持计算效率的同时超越了多个先进基线模型。


<details>
  <summary>Details</summary>
Motivation: 现有U-Net风格网络难以捕获长距离依赖，而基于Transformer的变体需要大量计算资源和训练数据，需要一种既能建模全局上下文又保持计算效率的解决方案。

Method: 将分组坐标注意力(GCA)集成到ResNet-50残差块中，通过分组坐标建模联合编码通道和空间位置的全局依赖关系，增强特征表示和边界划分能力。

Result: 在Synapse数据集上Dice分数达86.11%，在ACDC数据集上达92.64%，超越了多个先进基线模型，同时保持快速推理和良好的计算效率。

Conclusion: GCA为卷积架构增强全局建模能力提供了一种实用方法，能够实现高精度且资源高效的医学图像分割。
...
TL;DR: SMGeo是一个基于Transformer的端到端可提示地理定位模型，通过点击提示实现跨视角（无人机到卫星）物体地理定位，在IoU=0.25和mIoU指标上达到领先性能。


<details>
  <summary>Details</summary>
Motivation: 解决传统多阶段"检索-匹配"管道在跨视角物体地理定位中因视角和尺度差异大、背景干扰复杂而导致的累积误差问题。

Method: 采用全Transformer架构：Swin-Transformer进行无人机和卫星图像联合特征编码；引入网格级稀疏专家混合（GMoE）捕捉跨模态和视图内依赖；使用无锚检测头通过热图监督直接预测坐标。

Result: 在无人机到卫星任务中，IoU=0.25和mIoU指标分别达到87.51%、62.50%和61.45%，显著优于DetGeo等方法。

Conclusion: SMGeo通过端到端可提示架构、网格级稀疏专家混合和无锚检测头，有效解决了跨视角地理定位的挑战，实现了领先的性能。
...
TL;DR: 提出BCE3S方法，通过二元交叉熵的三部分协同学习解决长尾识别问题，相比传统交叉熵方法能获得更好的类内紧凑性和类间分离性。
...
Conclusion: BCE3S方法不仅实现了样本特征间更高的紧凑性和分离性，还平衡了分类器的分离性，为长尾识别任务提供了有效的解决方案。
...
TL;DR: FAPE-IR是一个频率感知规划与执行框架，通过冻结多模态大语言模型分析退化图像并生成频率感知修复计划，指导基于扩散的执行器动态选择高频或低频专家进行图像修复。


<details>
  <summary>Details</summary>
Motivation: 现有的多合一图像修复方法依赖任务特定设计或潜在路由策略，难以适应现实世界中复杂多样的退化情况，需要更统一和可解释的解决方案。

Method: 使用冻结MLLM作为规划器分析退化图像并生成频率感知修复计划，通过LoRA-MoE模块在扩散执行器中动态选择高频或低频专家，结合对抗训练和频率正则化损失提升修复质量。
...
<details>
  <summary>Details</summary>
Motivation: 现有的视频编辑方法需要用户提供精确的空间位置和时间边界描述，但在用户通过语义属性或对象关系的隐式查询进行编辑时，这些要求变得不切实际。

Method: RIVER通过数字孪生表示解耦推理和生成，使用大型语言模型进行多跳推理，然后输出结构化指令指导基于扩散的编辑器执行像素级修改，训练时使用强化学习评估推理准确性和生成质量。
...
TL;DR: 提出了一种实时自监督单目深度估计方法RTS-Mono，采用轻量级编码器-解码器架构，在保持高性能的同时大幅减少计算资源消耗，可在嵌入式设备上实现49 FPS的实时推理。
...
Result: 在KITTI数据集上以极低的参数量（3M）实现了最先进性能，在低分辨率下Abs Rel和Sq Rel分别提升5.6%和9.8%，在高分辨率下Sq Rel和RMSE分别提升6.1%和1.9%。在Nvidia Jetson Orin上达到49 FPS的实时推理速度。

Conclusion: RTS-Mono成功解决了自监督单目深度估计模型在实际部署中的计算效率问题，实现了高性能与实时性的平衡。
...
TL;DR: 提出了一种基于非对称聚合和几何约束的视觉位置识别方法A²GC-VPR，通过行-列归一化平均和分离边缘校准解决特征与聚类中心分布差异问题，并引入可学习坐标嵌入增强空间感知能力。


<details>
  <summary>Details</summary>
Motivation: 现有最优传输聚合方法中Sinkhorn算法对称处理源和目标边缘分布，当图像特征和聚类中心分布差异显著时效果受限，需要非对称匹配机制。

Method: 采用行-列归一化平均与分离边缘校准实现非对称匹配，通过可学习坐标嵌入计算兼容性分数并与特征相似性融合，促进空间邻近特征分配到同一聚类。

Result: 在MSLS、NordLand和Pittsburgh数据集上的实验结果表明该方法在匹配准确性和鲁棒性方面表现优越。

Conclusion: A²GC-VPR方法通过非对称聚合和几何约束有效提升了视觉位置识别的性能，验证了处理分布差异和增强空间感知的重要性。
...
TL;DR: 提出了CViT，一种轻量级且计算高效的视觉Transformer架构，通过新颖的Cascaded-Chunk Feed Forward Network设计，在保持准确性的同时提升了参数和FLOP效率。
...
Method: 设计了Cascaded-Chunk Feed Forward Network (CCFFN)，通过分割输入特征来提高参数和FLOP效率。

Result: 在ImageNet-1K上，CViT-XL达到75.5% Top-1准确率，相比EfficientViT-M5减少15% FLOPs和3.3%能耗。CViT系列在各种模型尺寸下都表现出最低的能耗。

Conclusion: CViT架构特别适合部署在电池受限设备上，如手机和无人机，并在新的Accuracy-Per-FLOP指标下展现出顶级的计算效率。
...
TL;DR: Coffee方法通过语言描述来指定不希望学习的概念，在微调过程中防止文本到图像扩散模型学习不需要的概念，无需额外训练即可灵活修改不需要的概念描述。


<details>
  <summary>Details</summary>
Motivation: 解决文本到图像扩散模型在微调过程中学习不需要的概念的问题，这对于偏见缓解、防止恶意适应、属性解缠和扩散策略的通用微调等下游任务至关重要。

Method: 通过保持用户提示的嵌入与不需要的概念不对齐，使用语言描述来规范适应过程，无需额外训练即可灵活修改不需要的概念。

Result: 实验结果表明，Coffee能够有效防止文本到图像模型在微调过程中学习指定的不需要概念，并且优于现有方法。

Conclusion: Coffee提供了一种有效的方法来控制扩散模型的微调过程，防止学习不需要的概念，同时保持灵活性，仅通过修改文本描述即可调整不需要的概念。
...
<details>
  <summary>Details</summary>
Motivation: 现有视频系统只能检测事故是否发生，但无法深入理解事故在不同认知阶段的发展过程，缺乏对事故成因的系统分析。

Method: 四阶段框架：事件触发多视角视频采集→行人行为阶段分割→阶段特定多视角推理→分层合成与诊断推理，使用两个专门VLM模型进行阶段分割和多视角分析。
...
TL;DR: 该论文提出了ConvNN框架，将卷积和自注意力统一在k近邻聚合框架下，揭示了它们都是邻居选择和聚合的特殊情况，处于连续谱系中。


<details>
  <summary>Details</summary>
Motivation: 尽管CNN和Transformer重塑了计算机视觉，但它们通常被视为根本不同的架构。作者认为卷积和自注意力虽然表面不同，但可以在统一的k近邻框架下理解。

Method: 引入ConvNN框架，将卷积和注意力形式化为邻居选择机制：卷积基于空间邻近性选择邻居，注意力基于特征相似性选择邻居。该框架可作为卷积层和注意力层的即插即用替代品。

Result: 在CIFAR-10和CIFAR-100上的验证表明：VGG中的混合分支结合两种选择策略提高了准确率；ViT中的ConvNN优于标准注意力和其他变体。消融实验显示在谱系中间插值具有正则化效果。
...
<details>
  <summary>Details</summary>
Motivation: 现有视频时刻检索方法主要依赖粗略的时间理解和单一视觉模态，在复杂视频中性能受限，需要更精细的多模态融合方法。

Method: 提出SMART框架，集成音频和视觉特征，应用镜头感知令牌压缩选择性保留高信息令牌以减少冗余，同时优化提示设计以更好利用音频-视觉线索。

Result: 在Charades-STA数据集上R1@0.5提升1.61%，R1@0.7提升2.59%，在QVHighlights上也取得显著改进。
...
Method: 首先使用高斯场景先验的姿态回归网络回归粗略6DoF姿态，然后通过特征匹配和多模型融合进行精炼，关键创新包括无需可微分渲染的跨相关模块和多视角预测器。

Result: 在多个数据集上显著优于先前方法，将中值旋转误差降至0.2°，在移动机器人上实现2.87 FPS跟踪。

Conclusion: iGaussian通过前馈框架和直接3D高斯反演，在保持高精度的同时实现了实时性能，为机器人视觉导航提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 毫米波信号能够穿透常见遮挡物并从隐藏物体反射，这为在机器人、增强现实和物流等领域开发新的应用提供了可能。

Method: 采用三阶段流水线：提出候选几何表面、使用专门为毫米波信号设计的基于transformer的形状补全模型、执行熵引导的表面选择。该方法完全使用合成点云进行训练。

Result: 在与最先进基线的比较中，Wave-Former将召回率从54%提高到72%，同时保持85%的高精度。

Conclusion: Wave-Former展示了从合成数据训练到真实世界数据的出色泛化能力，为遮挡物体3D重建开辟了新途径。
...
TL;DR: 提出了O3SLM模型，通过构建大规模图像-草图-指令三元组数据集，解决了大型视觉语言模型在理解手绘草图方面的局限性，在多个草图任务上达到最先进性能。
...
Method: 构建了大规模图像-草图-指令三元组数据集，并基于此训练O3SLM模型，支持预训练和指令调优。

Result: 在多个草图任务（目标定位、计数、图像检索、视觉问答）上评估，O3SLM显著优于现有LVLMs，在草图理解和推理方面达到最先进性能。

Conclusion: 通过构建专门的草图-图像-指令数据集和训练O3SLM模型，成功提升了大型视觉语言模型对抽象视觉输入（特别是手绘草图）的理解能力。
...
TL;DR: 提出了RiSe框架解决多模态人脸防伪中的跨域泛化问题，通过非对称不变风险最小化和多模态协同解耦来应对表示不变风险和协同不变风险。
...
Method: RiSe框架包含两个组件：AsyIRM学习径向空间中的不变球形决策边界，MMSD通过跨样本混合和解耦增强内在的通用模态特征。

Result: 理论分析和实验验证了RiSe的有效性，在跨域性能上达到了最先进水平。

Conclusion: RiSe框架成功解决了多模态人脸防伪中的跨域泛化挑战，通过处理表示和协同不变风险实现了更好的泛化性能。
...
TL;DR: MVI-Bench是首个专门评估误导性视觉输入对大型视觉语言模型鲁棒性影响的基准测试，包含1,248个专家标注的VQA实例，并提出了细粒度鲁棒性评估指标MVI-Sensitivity。


<details>
  <summary>Details</summary>
Motivation: 现有鲁棒性基准主要关注幻觉或误导性文本输入，而忽略了误导性视觉输入对视觉理解评估的重要挑战。

Method: 基于基础视觉原语，设计了三个层次误导性视觉输入分类：视觉概念、视觉属性和视觉关系，构建了六个代表性类别和1,248个VQA实例。

Result: 对18个最先进LVLM的实证研究揭示了它们对误导性视觉输入的显著脆弱性。

Conclusion: MVI-Bench提供了可操作的见解，可指导开发更可靠和鲁棒的LVLM，基准和代码已开源。
...
TL;DR: 提出一种对象级token压缩策略，通过自适应token合并减少图像token数量，仅用10%的token即可达到原始模型96%的性能。


<details>
  <summary>Details</summary>
Motivation: 传统补丁级token化导致图像token数量二次增长，增加了计算和内存负担，且与人类视觉认知系统不匹配，导致幻觉和计算冗余。

Method: 采用对象级token合并策略进行自适应token压缩，该方法与人类视觉系统保持一致。

Result: 在多个基准测试中，平均仅使用10%的token即可达到原始模型约96%的性能，在压缩比和性能平衡方面优于相关工作。

Conclusion: 提出的对象级token压缩策略有效解决了MLLMs中图像token过多的问题，显著减少了计算负担同时保持了良好的性能。
...
TL;DR: DoGCLR是一种基于博弈论的自监督对比学习框架，用于骨架动作识别。它通过时空双权重定位机制识别关键运动区域，并采用熵驱动的优势策略管理负样本，在多个数据集上实现了最先进的性能。
...
Method: 提出基于博弈论的自监督框架DoGCLR，将正负样本构建建模为动态优势博弈；使用时空双权重定位机制识别关键运动区域；采用熵驱动的优势策略管理记忆库，保留高熵负样本并替换低熵负样本。
...
<details>
  <summary>Details</summary>
Motivation: 现有工作针对软效应退化问题开发高度专业化的模型，缺乏可扩展性且未能利用这些恢复问题的共享本质。同时，通用模型需要详细提示且在细粒度任务上往往无法实现稳健去除或保持场景身份。

Method: 利用软效应的共同本质（半透明遮挡），策划包含380万对数据的大规模数据集，采用定制训练流程微调扩散变换器，集成细粒度掩码和强度控制来学习稳健的恢复先验。

Result: UniSER在野外环境中显著优于专业模型和通用模型，实现了稳健、高保真的图像恢复效果。

Conclusion: 通过利用软效应的共享本质和大规模数据集，UniSER展示了在单一框架内处理多种图像退化问题的能力，为图像恢复领域提供了新的基础性解决方案。
...
TL;DR: 提出了UMEG-Net，一个统一的多实体图网络，用于少样本精确事件检测，通过整合人体骨架和运动特定物体关键点，结合GCN和多尺度时间移位，在有限标注数据下实现鲁棒性能。


<details>
  <summary>Details</summary>
Motivation: 精确事件检测在体育分析中至关重要，但现有方法依赖大量标注数据和像素/姿态输入，在少样本条件下表现不佳。获取大规模标注数据集实际困难。

Method: UMEG-Net整合人体骨架和运动特定物体关键点到统一图中，采用基于先进GCN和多尺度时间移位的高效时空提取模块，并通过多模态蒸馏将关键点图知识迁移到视觉表示。

Result: 在少样本设置下显著优于基线模型，仅需有限标注数据即可实现鲁棒性能。

Conclusion: UMEG-Net为少样本精确事件检测提供了可扩展且有效的解决方案，代码已公开。
...
Method: 提出码本直方图关系学习方法将预训练模型在整个数据集上建模的语义转移到语义码本中，并设计残差学习模块恢复细粒度细节以减少量化带来的重建误差。
...
Method: 提出多尺度相关性感知Transformer网络(MCFormer)，包含全局相关性模块(GCM)构建全局相似度亲和矩阵，以及局部相关性模块(LCM)挖掘对齐局部特征，通过动态记忆库整合多尺度相关特征。

Result: 在三个基准测试上的实验表明，MCFormer达到了最先进的性能。

Conclusion: MCFormer通过显式建模多尺度相关性，有效抑制了船舶重识别中异常样本的不利影响，提升了特征鲁棒性和识别性能。
...
TL;DR: 首个完全由自动驾驶模式在真实世界收集的端到端基准数据集，包含100+小时自然驾驶数据，用于评估自动驾驶车辆的真实行为安全性。


<details>
  <summary>Details</summary>
Motivation: 现有数据集多为人类驾驶模式收集，只能用于感知和预测的早期训练，无法评估自动驾驶车辆在真实世界中的行为安全性。

Method: 收集超过100小时来自市场多个量产自动驾驶车辆的自然驾驶数据，分割成32,727个关键帧，包含同步相机图像、高精度GNSS/IMU数据、车辆轨迹和详细2D标注。

Result: 数据集提供丰富的场景属性标注，使用端到端运动规划模型在自动驾驶帧上实现1.4米的平均位移误差。
...
TL;DR: Orion是一个多模态视觉代理框架，通过调用多种计算机视觉工具实现复杂的多步骤视觉工作流，在多个基准测试中达到最先进水平。


<details>
  <summary>Details</summary>
Motivation: 传统视觉语言模型只能生成描述性输出，无法执行复杂的视觉推理任务。Orion旨在通过工具驱动的智能框架，将被动视觉理解转变为主动视觉智能。

Method: 采用代理框架，整合多种专用计算机视觉工具，包括目标检测、关键点定位、全景分割、OCR和几何分析，结合神经感知与符号执行实现自主视觉推理。

Result: 在MMMU、MMBench、DocVQA和MMLongBench等基准测试中取得竞争性表现，将单块视觉语言模型扩展到生产级视觉智能。

Conclusion: Orion通过工具驱动的视觉智能框架，实现了从被动视觉理解到主动视觉推理的转变，为生产级视觉AI应用提供了可行方案。
...
TL;DR: 提出了一种基于课程学习策略的分形softmax方法，用于主动脉血管的层次语义分割，解决了血管结构中的类别不平衡问题，显著提升了分割精度和效率。


<details>
  <summary>Details</summary>
Motivation: 主动脉作为人体最大的动脉，容易发生夹层、动脉瘤和动脉粥样硬化等病理变化，需要及时干预。现有的血管分割方法往往忽视层次解剖关系，同时难以处理血管结构中固有的严重类别不平衡问题。

Method: 采用课程学习策略，结合新颖的分形softmax进行层次语义学习。受人类认知启发，该方法从简单到复杂逐步学习解剖约束。通过两阶段推理策略实现高达五倍的加速，增强临床实用性。

Result: 在验证集上，层次语义损失将nnU-Net ResEnc M的Dice得分提高了11.65%。在测试集上，所提模型比基线方法Dice得分高出5.6%。实验结果显示分割精度和效率均有显著提升。

Conclusion: 该框架在分割精度和效率方面均取得显著改进，适用于实时临床应用，为主动脉血管的精确分割提供了有效的解决方案。
...
TL;DR: Gen3d是一种从单张图像生成高质量通用3D场景的新方法，通过RGBD图像创建初始点云，维护和扩展世界模型，最终优化高斯溅射表示来完成3D场景重建。
...
Method: 从RGBD图像创建初始点云，维护和扩展世界模型，通过优化高斯溅射表示来最终完成3D场景生成。
...
### [68] [Online Data Curation for Object Detection via Marginal Contributions to Dataset-level Average Precision](https://arxiv.org/abs/2511.14197)
...
<details>
  <summary>Details</summary>
Motivation: 高质量数据已成为规模定律下进步的主要驱动力，但现有的在线采样策略很少扩展到目标检测领域，因为其结构复杂且存在领域差距。

Method: 通过建模全局分数分布，DetGain高效估计全局AP变化并计算师生贡献差距，在每个迭代中选择信息丰富的样本。该方法与架构无关且侵入性最小。

Result: 在COCO数据集上的实验显示，使用多个代表性检测器均实现了准确率的持续提升，在低质量数据下表现出强大的鲁棒性，并能与知识蒸馏技术有效结合进一步提升性能。
...
<details>
  <summary>Details</summary>
Motivation: 医学图像分割面临数据隐私和专家标注成本高的限制，联邦半监督学习(FSSL)是解决方案但存在两个挑战：伪标签可靠性依赖本地模型强度，客户端设备通常需要紧凑或异构架构。这些限制降低了伪标签质量和稳定性，而大模型虽然更准确但无法在客户端设备上训练或推理。

Method: 提出SAM-Fed框架，利用高容量分割基础模型指导轻量级客户端训练，结合双重知识蒸馏和自适应一致性机制来细化像素级监督。

Result: 在皮肤病变和息肉分割的同质和异构设置实验中，SAM-Fed始终优于最先进的FSSL方法。

Conclusion: SAM-Fed通过利用基础模型指导轻量级客户端，有效解决了联邦半监督学习中的伪标签质量和模型异构性问题，在医学图像分割任务上取得了优异性能。
...
### [70] [InstantViR: Real-Time Video Inverse Problem Solver with Distilled Diffusion Prior](https://arxiv.org/abs/2511.14208)
...
Method: 将强大的双向视频扩散模型（教师）蒸馏为因果自回归学生模型，仅需教师扩散模型和已知退化算子，无需外部配对数据。采用LeanVAE替换VAE骨干，通过教师空间正则化蒸馏方案实现低延迟潜在空间处理。
...
Method: 基于YOLOv11-Pose，集成部分卷积(PConv)和相似性感知激活模块(SimAM)，设计Light-C3k2轻量模块；在识别头中通过逐点卷积将关键点直接映射到姿势类别，采用中间监督实现姿态估计与分类的高效融合

Result: 构建包含6个姿势类别5000张图像的数据集；最小模型LSP-YOLO-n在PC上达到94.2%准确率和251FPS，模型大小仅1.9MB；在SV830C + GC030A平台上实现实时高精度推理

Conclusion: 该方法具有高效率、轻量化和可部署性特点，适用于智能教室、康复和人机交互应用
...
Result: Bapp在标准指标上表现优异：精确度98.4%、召回率96.9%、F1分数97.6%，整体准确率达到98.3%。

Conclusion: Bapp被证实为可靠、便携、易用的眼睑运动监测工具，为传统手动眨眼计数提供了有前景的替代方案，支持连续眼部健康监测和临床环境中的术后评估。
...
TL;DR: 提出了一种测量约束采样方法，用于盲人脸恢复的多样性重建，通过文本提示生成多个合理的高质量结果。


<details>
  <summary>Details</summary>
Motivation: 现有盲人脸恢复方法通常产生确定性结果，难以捕捉一对多的本质，即低质量输入可能对应多个合理的高质量重建。

Method: 将盲人脸恢复构建为测量约束的生成任务，通过粗恢复的受控退化构建逆问题，在文本到图像扩散中进行后验引导采样，包含前向测量和后向测量约束。

Result: 实验表明该方法能生成与提示对齐的结果，并优于现有盲人脸恢复方法。

Conclusion: 测量约束采样方法能够有效解决盲人脸恢复中的一对多问题，实现多样化的高质量重建。
...
TL;DR: 提出了一种自回归扩散模型用于语音驱动的3D面部动画，通过流式处理解决长音频序列的性能下降和延迟问题，实现低延迟实时生成。
...
Result: 实现了灵活处理不同长度音频的能力，达到与音频时长无关的低延迟，并开发了实时交互演示验证效果。

Conclusion: 该方法在保持高质量生成结果的同时，有效解决了长音频处理的延迟问题，适用于实时应用场景。
...
TL;DR: 提出首个针对立体匹配模型的纹理式物理对抗攻击，使用3D全局伪装纹理而非局部2D补丁，确保立体相机不同视角下的视觉一致性和攻击效果。
...
Method: 采用3D物理对抗样本与全局伪装纹理，提出3D立体匹配渲染模块处理相机视差效应，并开发新颖的融合攻击通过细粒度优化将目标无缝融入环境。

Result: 广泛评估表明，该方法能成功欺骗立体模型产生错误的深度信息，相比现有隐藏攻击具有显著增强的隐蔽性和杀伤力。

Conclusion: 该方法在立体视觉系统中实现了有效的物理对抗攻击，证明了3D全局纹理在欺骗立体深度估计模型方面的潜力。
...
TL;DR: 提出了一种主动感知策略(APS)用于3D人体运动预测，通过商空间表示显式编码运动特性，并引入辅助学习目标来加强时空建模，在多个数据集上取得了新的最先进性能。
...
Method: 设计数据感知模块将姿态投影到商空间，解耦运动几何与坐标冗余；引入网络感知模块通过恢复性学习主动学习时空依赖关系，通过掩码特定关节或注入噪声构建辅助监督信号。

Result: 在H3.6M上提升16.3%，CMU Mocap上提升13.9%，3DPW上提升10.1%，实现了新的最先进性能。

Conclusion: APS方法能够有效增强主动感知能力，是模型无关的，可以与不同的预测模型集成，显著提升了3D人体运动预测性能。
...
TL;DR: WeSTAR是一个参数高效的弱监督自训练框架，通过结构自监督、语义感知层次归一化、成对深度排序弱监督和权重正则化来增强单目深度估计基础模型在未见领域的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 尽管基础模型在单目深度估计的零样本泛化方面取得了进展，但在获得下游任务数据后，如何进一步提升这些模型的性能成为一个自然的问题。

Method: 采用密集自训练目标作为结构自监督的主要来源；引入语义感知层次归一化，利用实例级分割图进行更稳定的多尺度结构归一化；使用成对深度排序标注作为成本效益高的弱监督来指导适应过程；应用权重正则化损失来稳定训练并保留模型的可泛化知识。

Result: 在多样化和具有挑战性场景下的现实和损坏分布外数据集上进行广泛实验，证明WeSTAR持续改善泛化能力，并在广泛基准测试中达到最先进性能。
...
TL;DR: AVI是一个无需训练的视频智能框架，通过检索-感知-审查的三阶段推理过程、结构化视频知识库和开源模型集成，实现类似人类的视频理解能力。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型通常以单次处理方式处理视频，缺乏证据重访和迭代精炼能力，而基于代理的方法要么依赖昂贵的专有模型，要么需要大量强化学习训练。

Method: 提出三阶段推理过程（检索-感知-审查）、结构化视频知识库（实体图）和多粒度集成工具，使用开源模型集成（推理LLM+轻量级CV模型+VLM）。

Result: 在LVBench、VideoMME-Long、LongVideoBench和Charades-STA等基准测试中达到竞争性性能，同时提供更好的可解释性。

Conclusion: AVI框架通过系统级设计和优化，实现了无需依赖专有API或强化学习训练的高效视频理解，具有灵活性和可解释性优势。
...
### [79] [V2VLoc: Robust GNSS-Free Collaborative Perception via LiDAR Localization](https://arxiv.org/abs/2511.14247)
*Wenkai Lin,Qiming Xia,Wen Li,Xun Huang,Chenglu Wen*

Main category: cs.CV

TL;DR: 提出了一种基于LiDAR定位的GNSS-free协作感知框架，包括轻量级姿态生成器PGC和姿态感知时空对齐Transformer PASTAT，并在V2VLoc数据集上验证了在GNSS拒止环境下的优越性能。


<details>
  <summary>Details</summary>
Motivation: 传统GNSS定位在GNSS拒止环境中经常失效，导致多智能体协作感知中特征对齐困难，需要开发不依赖GNSS的鲁棒协作感知方法。

Method: 1) 轻量级姿态生成器PGC估计紧凑姿态和置信度表示；2) 姿态感知时空对齐Transformer PASTAT执行置信度感知的空间对齐并捕获时间上下文；3) 创建V2VLoc仿真数据集支持LiDAR定位和协作检测任务。

Result: 在V2VLoc数据集上的广泛实验表明，该方法在GNSS拒止条件下达到了最先进的性能。在真实世界V2V4Real数据集上的扩展实验验证了PASTAT的有效性和泛化能力。

Conclusion: 提出的GNSS-free协作感知框架能够有效解决GNSS拒止环境下的多智能体协作感知问题，通过LiDAR定位和时空对齐技术实现了鲁棒的协作感知性能。
...
### [80] [ManipShield: A Unified Framework for Image Manipulation Detection, Localization and Explanation](https://arxiv.org/abs/2511.14259)
*Zitong Xu,Huiyu Duan,Xiaoyu Wang,Zhaolin Cai,Kaiwei Zhang,Qiang Hu,Jing Liu,Xiongkuo Min,Guangtao Zhai*

Main category: cs.CV

TL;DR: 提出了ManipBench大规模图像篡改检测基准和ManipShield多模态检测模型，解决了现有基准内容多样性不足、生成模型覆盖窄、可解释性差的问题。
...
Method: 构建包含45万张篡改图像的ManipBench基准，覆盖25个最先进的图像编辑模型和12种篡改类别；提出基于多模态大语言模型的ManipShield模型，采用对比LoRA微调和任务特定解码器实现统一的检测、定位和解释。

Result: 在ManipBench和多个公共数据集上的广泛实验表明，ManipShield实现了最先进的性能，并对未见过的篡改模型表现出强大的泛化能力。

Conclusion: ManipBench基准和ManipShield模型为解决AI编辑图像检测提供了全面解决方案，具有重要的研究和应用价值。
...
TL;DR: 提出深度学习框架用于WMH分割和定位，区域WMH体积比全局病灶负荷在疾病分类中表现更好，与脑萎缩指标结合可进一步提高诊断性能


<details>
  <summary>Details</summary>
Motivation: 现有WMH分割方法大多只提供全局病灶负荷，忽视了不同白质区域的空间分布差异，而区域WMH量化可能对神经退行性疾病诊断有重要价值

Method: 深度学习框架进行WMH分割和定位，在公共数据集和ADNI队列中评估，量化解剖定义区域内的WMH负荷，并与脑结构体积结合评估诊断价值
...
### [82] [Gaussian Splatting-based Low-Rank Tensor Representation for Multi-Dimensional Image Recovery](https://arxiv.org/abs/2511.14270)
...
<details>
  <summary>Details</summary>
Motivation: 传统张量奇异值分解(t-SVD)方法存在两个关键限制：1)潜在张量近似粗糙，无法准确捕获空间局部高频信息；2)变换矩阵由固定基原子组成，无法精确捕获mode-3纤维方向的局部高频信息。

Method: 提出GSLR框架，使用定制的2D高斯泼溅生成潜在张量，1D高斯泼溅生成变换矩阵。这两种高斯泼溅在该表示框架中不可或缺且互补，具有强大的表示能力，特别是对局部高频信息。

Result: 开发了无监督的GSLR多维图像恢复模型，实验表明GSLR在多维图像恢复任务中始终优于最先进方法，特别是在捕获局部高频信息方面表现突出。

Conclusion: GSLR框架通过高斯泼溅技术有效解决了传统t-SVD方法的局限性，为多维图像提供了紧凑连续的表示，显著提升了局部高频信息的捕获能力。
...
<details>
  <summary>Details</summary>
Motivation: 真实临床环境中多模态MRI经常缺失一个或多个模态，严重影响深度学习分割模型的性能和泛化能力。

Method: 采用共享-特定编码器-解码器架构，包含两种自蒸馏策略：层次模态自蒸馏机制和渐进模态组合蒸馏方法。

Result: 在公共脑肿瘤分割基准测试中，CCSD在各种缺失模态场景下实现了最先进的性能，具有强大的泛化性和稳定性。

Conclusion: CCSD框架能够有效处理多模态MRI中的模态缺失问题，为临床诊断和治疗规划提供可靠的分割结果。
...
### [84] [Let Language Constrain Geometry: Vision-Language Models as Semantic and Spatial Critics for 3D Generation](https://arxiv.org/abs/2511.14271)
*Weimin Bai,Yubo Li,Weijian Luo,Zeqiang Lai,Yequan Wang,Wenzheng Chen,He Sun*

Main category: cs.CV

TL;DR: VLM3D是一个通用框架，利用大型视觉语言模型(VLMs)作为可微分的语义和空间评论家，通过双查询评论信号来提升文本到3D生成的质量，解决语义对齐和空间理解问题。
...
Method: 提出VLM3D框架，重新利用大型视觉语言模型作为可微分的语义和空间评论家。核心贡献是基于VLM的是/否对数概率的双查询评论信号，评估语义保真度和几何连贯性。该指导信号适用于两种范式：作为优化管道的奖励目标，以及作为前馈管道的测试时指导模块。

Result: 在标准基准测试中，VLM3D显著优于现有方法。作为测试时指导模块，它能主动引导最先进原生3D模型的迭代采样过程，纠正严重的空间错误。

Conclusion: VLM3D建立了一个原则性和可泛化的路径，将VLM丰富的、基于语言理解的语义和空间知识注入到多样化的3D生成管道中。
...
TL;DR: 本研究评估了表格特征和基于Transformer的MRI嵌入在预测阿尔茨海默病认知衰退中的互补作用，发现临床特征擅长识别高风险极端病例，而MRI嵌入对稳定性标记更敏感。


<details>
  <summary>Details</summary>
Motivation: 准确建模阿尔茨海默病认知衰退对早期分层和个性化管理至关重要，但表格预测因子捕捉细微脑变化的能力有限，需要评估影像表征的预测贡献。

Method: 引入基于动态时间规整聚类的轨迹感知标签策略，通过无监督重建训练3D Vision Transformer获取解剖保留的MRI嵌入，与传统机器学习分类器和卷积网络基线进行比较。

Result: 临床和体积特征在预测轻度和重度进展时达到约0.70的最高AUC，而ViT的MRI嵌入在区分认知稳定个体时表现最佳（AUC 0.71），但在异质性中度组中所有方法都表现不佳。

Conclusion: 临床特征擅长识别高风险极端病例，基于Transformer的MRI嵌入对稳定性标记更敏感，需要多模态融合策略来建模AD进展。
...
### [86] [Free Lunch to Meet the Gap: Intermediate Domain Reconstruction for Cross-Domain Few-Shot Learning](https://arxiv.org/abs/2511.14279)
...
<details>
  <summary>Details</summary>
Motivation: 跨域小样本学习面临语义不匹配、大域差异和数据稀缺三重挑战。现有方法主要关注通用表示，而本文尝试通过中间域代理来弥合源域和目标域之间的差距。

Method: 使用源域特征嵌入作为码本构建中间域代理，然后重建目标域特征。通过经验研究探索中间域代理的视觉风格和语义内容属性，并开发快速域对齐方法，利用这些代理作为目标域特征转换的学习指导。

Result: 在8个跨域小样本学习基准测试中，所提出的模型超越了最先进模型。

Conclusion: 通过中间域重建和目标特征转换的协同学习，该方法能够有效解决跨域小样本学习中的挑战，并在多个基准测试中取得优异性能。
...
### [87] [NeuralSSD: A Neural Solver for Signed Distance Surface Reconstruction](https://arxiv.org/abs/2511.14283)
*Zi-Chen Xi,Jiahui Huang,Hao-Xiang Chen,Francis Williams,Qun-Ce Xu,Tai-Jiang Mu,Shi-Min Hu*

Main category: cs.CV

TL;DR: 提出NeuralSSD方法，基于神经Galerkin方法从点云数据重建3D隐式表面，在ShapeNet和Matterport数据集上达到最先进效果


<details>
  <summary>Details</summary>
Motivation: 现有隐式场参数化方法缺乏确保表面与输入数据紧密拟合的显式机制，需要平衡点云信息的可靠性

Method: 提出新的能量方程平衡点云信息可靠性，引入学习三维信息的新卷积网络实现优化，基于神经Galerkin方法

Result: 在ShapeNet和Matterport等挑战性数据集上评估，在表面重建精度和泛化性方面达到最先进水平

Conclusion: NeuralSSD能够确保重建表面紧密贴合原始输入点，并从点云中推断有价值的归纳偏差，实现高精度稳定表面重建
...
TL;DR: 提出NeuralBoneReg，一种自监督的骨表面配准框架，使用3D点云作为模态无关表示，在计算机辅助骨科手术中实现跨模态骨表面配准。
...
Conclusion: NeuralBoneReg在解剖结构和模态间表现出强大的泛化能力，为CAOS提供鲁棒准确的跨模态配准。
...
TL;DR: 提出了一种用于时间多普勒全息图中动静脉分割的简单有效方法，通过结合脉冲分析特征使传统U-Net能够利用时间动态信息
...
<details>
  <summary>Details</summary>
Motivation: 自动年龄估计在年龄验证中广泛应用，但输入图像分辨率差异很大，需要研究分辨率对年龄估计准确性的具体影响。

Method: 使用IMDB-Clean数据集的1000张图像，在7种不同分辨率下处理得到7000个测试样本，使用MAE、SD和MedAE评估DeepFace和InsightFace的性能。

Result: 两个框架在224x224像素时表现最佳，DeepFace的MAE为10.83年，InsightFace为7.46年。低分辨率时MAE显著增加，过高分辨率也会降低准确性。InsightFace在所有分辨率下都比DeepFace更快。

Conclusion: 输入图像分辨率对年龄估计准确性有明显且一致的影响，224x224像素是最佳分辨率选择。
...
TL;DR: Diff-NAF是一个针对超稀疏视角多源静态CT的迭代框架，结合神经衰减场和双分支条件扩散模型，通过角度先验引导的投影合成和扩散驱动的投影细化，逐步提升重建质量。


<details>
  <summary>Details</summary>
Motivation: 多源静态CT在超稀疏视角采样下重建质量严重下降，传统方法难以处理，需要新的解决方案来提升重建质量。

Method: 结合神经衰减场和双分支条件扩散模型，采用迭代框架：先训练初始NAF，然后通过角度先验引导的投影合成生成新投影，再用扩散驱动的投影细化模块进行优化，最后将细化后的投影作为伪标签加入训练集进行下一轮迭代。
...
TL;DR: 提出了一种融合ECG电生理信息和AHA-17解剖图谱的多模态框架，通过时间感知特征融合机制实现LGE-MRI心肌瘢痕的生理一致性分割。


<details>
  <summary>Details</summary>
Motivation: LGE心脏MRI的瘢痕分割因对比度变化和成像伪影而具有挑战性，ECG信号提供的电生理信息可以补充定位瘢痕区域。

Method: 引入时间感知特征融合(TAFF)机制，根据ECG和LGE-MRI采集时间差动态加权融合特征，结合AHA-17解剖图谱先验知识。

Result: 在临床数据集上显著优于图像基线方法(nnU-Net)，瘢痕Dice分数从0.6149提升到0.8463，精确度0.9115，灵敏度0.9043。

Conclusion: 融合生理和解剖知识使模型能够"超越图像观察"，为稳健且生理基础的心肌瘢痕分割开辟了新方向。
...
<details>
  <summary>Details</summary>
Motivation: 传统口腔内扫描方法无法用于远程正畸，而基于稀疏智能手机图像的3D高斯溅射方法在口腔环境中面临大视点基线、光照不一致和镜面反射等挑战，导致姿态和几何估计不稳定，且容易丢失关键诊断细节。

Method: 提出Dental3R流程：1）几何感知配对策略(GAPS)选择高价值图像对，提高几何初始化的稳定性；2）在恢复的姿态和点云基础上，使用小波正则化目标训练3DGS模型，通过离散小波变换保持频带保真度。

Result: 在950个临床病例和195个视频测试集上验证，Dental3R能有效处理稀疏、无姿态输入，在牙齿咬合可视化方面实现优越的新视角合成质量，优于现有最先进方法。

Conclusion: Dental3R为远程正畸提供了一种稳健、高保真度的口腔内3D重建解决方案，能够从稀疏照片中恢复精细的牙齿结构细节。
...
### [94] [Zero-shot Synthetic Video Realism Enhancement via Structure-aware Denoising](https://arxiv.org/abs/2511.14719)
...
### [95] [ARC Is a Vision Problem!](https://arxiv.org/abs/2511.14761)
*Keya Hu,Ali Cy,Linlu Qiu,Xiaoman Delores Ding,Runqian Wang,Yeyin Eva Zhu,Jacob Andreas,Kaiming He*

Main category: cs.CV

TL;DR: 将ARC问题重新定义为视觉中心的图像到图像翻译任务，使用Vision Transformer架构，在仅使用ARC数据训练的情况下，在ARC-1基准上达到60.4%的准确率，显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: ARC任务本质上是视觉化的谜题，但现有研究多从语言角度处理。本文旨在从视觉中心的角度重新构建ARC问题，利用视觉先验知识。

Method: 将ARC输入表示为"画布"，作为自然图像处理，使用标准视觉架构（如ViT）进行图像到图像映射，通过测试时训练实现泛化。

Result: 在ARC-1基准上达到60.4%准确率，显著优于仅使用ARC数据训练的现有方法，与领先的LLMs性能相当，接近人类平均水平。

Conclusion: 视觉中心的方法在抽象推理任务中表现出色，证明了视觉范式在解决ARC问题上的有效性，缩小了与人类表现的差距。
...
TL;DR: 本文提出了Step by Step Network (StepsNet)来解决深度残差网络中的两个关键问题：捷径退化和宽度限制，通过通道维度特征分离和逐步增加宽度的块堆叠来释放深度网络的潜力。


<details>
  <summary>Details</summary>
Motivation: 随着网络深度增加，现有残差架构难以实现理论上的容量提升，存在捷径退化阻碍深层学习和深度-宽度权衡限制宽度的问题，需要更先进的设计来充分发挥深度网络的潜力。

Method: 提出StepsNet架构，沿通道维度分离特征，通过堆叠宽度逐渐增加的块让模型逐步学习，缓解捷径退化和宽度限制问题，作为适用于各种模型的通用宏观设计。
...
### [97] [ArchMap: Arch-Flattening and Knowledge-Guided Vision Language Model for Tooth Counting and Structured Dental Understanding](https://arxiv.org/abs/2511.14336)
...
<details>
  <summary>Details</summary>
Motivation: 现有深度学习方法依赖模态特定训练、大量标注数据和受控扫描条件，限制了跨设备的泛化能力和临床部署。原始口腔网格存在牙弓姿态变化、几何不完整和缺乏纹理线索等问题。

Method: 1. 几何感知牙弓展平模块：将原始3D网格标准化为空间对齐的连续多视图投影；2. 构建牙科知识库：编码层次化牙齿本体、牙列阶段策略和临床语义；3. 结合几何归一化与本体引导的多模态推理。

Result: 在1060个正畸前后病例上验证，在牙齿计数、解剖分区、牙列阶段分类和临床状况识别方面表现稳健，相比监督方法和VLM基线具有更高准确性、减少语义漂移和优越稳定性。

Conclusion: ArchMap证明结合几何归一化与本体引导的多模态推理为现代数字正畸中3D口腔扫描的结构化分析提供了实用且可扩展的解决方案。
...
### [98] [Silhouette-to-Contour Registration: Aligning Intraoral Scan Models with Cephalometric Radiographs](https://arxiv.org/abs/2511.14343)
*Yiyi Miao,Taoyu Wu,Ji Jiang,Tong Chen,Zhe Tang,Zhengyong Jiang,Angelos Stefanidis,Limin Yu,Jionglong Su*

Main category: cs.CV

TL;DR: DentalSCR是一个用于口腔内扫描模型与侧位头颅X光片3D-2D配准的稳定轮廓引导框架，通过构建统一解剖坐标系和优化轮廓相似性变换，在真实临床条件下实现高精度配准。
...
Method: 首先构建U-Midline Dental Axis建立统一解剖坐标系，然后通过表面DRR生成X光样投影，最后使用对称双向Chamfer距离优化2D相似性变换。

Result: 在34个临床病例上评估，显著减少了标志点误差（特别是后牙区域），下颌骨误差分布更集中，曲线级别的Chamfer和Hausdorff距离表现优异。
...
### [99] [ARC-Chapter: Structuring Hour-Long Videos into Navigable Chapters and Hierarchical Summaries](https://arxiv.org/abs/2511.14349)
*Junfu Pu,Teng Wang,Yixiao Ge,Yuying Ge,Chen Li,Ying Shan*

Main category: cs.CV

TL;DR: ARC-Chapter是首个基于百万级长视频章节数据训练的大规模视频分章模型，通过双语、时序定位和分层标注，在视频分章任务上显著超越现有最佳方法14.0% F1分数和11.3% SODA分数。
...
Result: ARC-Chapter在数据规模和标注强度上均显示出明显性能提升，在密集视频描述等下游任务上也表现出优秀的迁移能力。

Conclusion: ARC-Chapter通过大规模多模态标注和创新的评估方法，为长视频内容结构化建立了新的技术标杆，并展示了出色的泛化能力。
...
### [100] [IBGS: Image-Based Gaussian Splatting](https://arxiv.org/abs/2511.14357)
...
<details>
  <summary>Details</summary>
Motivation: 现有3DGS方法使用低阶球谐函数限制了捕捉空间变化颜色和视角相关效果（如镜面高光）的能力。现有增强方法要么使用全局纹理图（难以处理复杂场景），要么使用每高斯纹理图（存储开销大）。

Method: 提出基于图像的GS方法，将每个像素颜色建模为标准3DGS渲染的基础颜色与从邻近训练图像推断的学习残差的组合。利用高分辨率源图像进行精细细节和视角特定颜色建模。

Result: 在标准NVS基准测试中，该方法在渲染质量上显著优于先前的GS方法，能够渲染高频细节和准确的视角相关效果。

Conclusion: 该方法提供了一种高效的替代方案，在不增加存储开销的情况下，显著提升了3DGS的渲染质量，特别是对精细细节和视角相关效果的捕捉能力。
...
### [101] [Blur-Robust Detection via Feature Restoration: An End-to-End Framework for Prior-Guided Infrared UAV Target Detection](https://arxiv.org/abs/2511.14371)
*Xiaolin Wang,Houzhang Fang,Qingshan Li,Lu Wang,Yi Chang,Luxin Yan*

Main category: cs.CV

TL;DR: 提出JFD3框架，联合特征域去模糊与检测，通过双分支架构和特征一致性监督，提升红外无人机模糊目标的检测性能。
...
Method: 设计权重共享的双分支架构，清晰分支引导模糊分支；引入轻量级特征恢复网络；提出频率结构引导模块；施加特征一致性自监督损失。

Result: 在包含30,000张模拟和4,118张真实红外无人机模糊图像的IRBlurUAV基准上，JFD3实现了优越的检测性能，同时保持实时效率。

Conclusion: JFD3框架通过联合特征域去模糊与检测，有效提升了红外无人机模糊目标的检测性能，证明了特征级监督的重要性。
...
### [102] [A Quantitative Method for Shoulder Presentation Evaluation in Biometric Identity Documents](https://arxiv.org/abs/2511.14376)
*Alfonso Pedro Ridao*

Main category: cs.CV

TL;DR: 提出了一种肩部姿态评估算法，仅使用两个肩部标志点的3D坐标来量化肩部偏转和滚动角度，用于自动检查生物特征证件照片是否符合国际标准的姿势要求。


<details>
  <summary>Details</summary>
Motivation: 生物特征身份证件的国际标准要求严格遵守姿势要求，包括肩部正面展示，但目前缺乏针对这一特定属性的自动化定量评估方法。
...
### [103] [Enhancing LLM-based Autonomous Driving with Modular Traffic Light and Sign Recognition](https://arxiv.org/abs/2511.14391)
...
### [104] [BEDLAM2.0: Synthetic Humans and Cameras in Motion](https://arxiv.org/abs/2511.14394)
*Joachim Tesch,Giorgio Becherini,Prerana Achar,Anastasios Yiannakidis,Muhammed Kocabas,Priyanka Patel,Michael J. Black*

Main category: cs.CV

TL;DR: BEDLAM2.0是一个新的数据集，用于从视频中推断3D人体运动，特别是在世界坐标系中估计人体运动。它在BEDLAM数据集的基础上增加了更多样化和真实的相机、身体形状、动作、服装、头发、3D环境以及鞋子等元素。


<details>
  <summary>Details</summary>
Motivation: 从视频中推断3D人体运动是一个具有挑战性的问题，许多应用需要将人体运动估计在世界坐标系中。然而，当存在人体和相机运动时，这个问题尤其困难。该领域的进展受到缺乏具有真实人体和相机运动地面真实值的丰富视频数据的限制。

Method: 通过创建BEDLAM2.0数据集来解决这个问题，该数据集在BEDLAM的基础上引入了更多样化和真实的相机和相机运动，增加了身体形状、动作、服装、头发和3D环境的多样性和真实性，并添加了鞋子等缺失元素。

Result: BEDLAM2.0显著优于BEDLAM，特别是在训练估计世界坐标系中人体运动的方法时。比较在BEDLAM和BEDLAM2.0上训练的最先进方法，发现BEDLAM2.0显著提高了准确性。

Conclusion: BEDLAM2.0是一个重要的资源，为研究目的提供了渲染视频、地面真实身体参数和相机运动，以及3D资产和第三方链接，显著改进了3D人体姿态和运动回归器的训练效果。
...
### [105] [Stage Aware Diagnosis of Diabetic Retinopathy via Ordinal Regression](https://arxiv.org/abs/2511.14398)
...
Conclusion: 该序数回归方法在糖尿病视网膜病变检测中表现出色，为临床分级提供了高一致性
...
### [106] [Language as an Anchor: Preserving Relative Visual Geometry for Domain Incremental Learning](https://arxiv.org/abs/2511.14401)
*Shuyi Geng,Tao Zhou,Yi Zhou*

Main category: cs.CV

TL;DR: LAVA提出了一种新的领域增量学习框架，通过基于文本的参考锚点进行相对对齐，解决了现有方法在统一视觉空间中的域间干扰和语义失真问题。


<details>
  <summary>Details</summary>
Motivation: 解决领域增量学习中统一视觉空间导致的域间干扰和语义失真，以及隔离域特定参数导致的知识碎片化问题。

Method: 使用语言锚定的视觉对齐方法，通过文本参考锚点指导每个域的可视表示保持一致的相对几何结构，该结构由类别名称间的语义相似度定义。

Result: 在标准DIL基准测试中，LAVA相比现有最先进方法实现了显著的性能提升。

Conclusion: LAVA通过语言锚定的相对对齐有效解决了领域增量学习中的知识保留和知识重用问题，为跨域知识迁移提供了新思路。
...
### [107] [Cranio-ID: Graph-Based Craniofacial Identification via Automatic Landmark Annotation in 2D Multi-View X-rays](https://arxiv.org/abs/2511.14411)
*Ravi Shankar Prasad,Nandani Sharma,Dinesh Singh*

Main category: cs.CV

TL;DR: 提出Cranio-ID框架，通过YOLO-pose模型自动标注2D头骨X射线图像和光学图像中的标志点，然后使用图表示和跨模态注意力机制进行头骨到面部的匹配。
...
Method: 1) 使用YOLO-pose模型自动标注2D头骨X射线和光学图像中的标志点；2) 将标志点表示为图结构，使用跨注意力机制和最优传输框架进行跨模态图匹配。

Result: 在S2F和CUHK数据集上的实验表明，该方法在可靠性和准确性方面均有显著提升，在法医科学中的头骨到面部和素描到面部匹配中表现有效。

Conclusion: Cranio-ID框架提供了一种可靠且准确的颅骨标志点自动定位和跨模态匹配方法，在法医科学中具有应用价值。
...
### [108] [Learning to See Through a Baby's Eyes: Early Visual Diets Enable Robust Visual Intelligence in Humans and Machines](https://arxiv.org/abs/2511.14440)
*Yusen Cai,Bhargava Satya Nunna,Qing Lin,Mengmi Zhang*

Main category: cs.CV

TL;DR: 该研究通过模拟婴儿视觉发展的CATDiet方法（包括灰度到彩色、模糊到清晰、保持时间连续性），训练自监督学习模型，发现这种阶段性视觉饮食能增强模型的鲁棒性，并表现出与生物发育相似的模式。


<details>
  <summary>Details</summary>
Motivation: 探索婴儿阶段性视觉发展（低清晰度、色彩退化但时间连续）如何为机器视觉提供生态优势，以及这种发展模式是否能增强视觉智能的鲁棒性。

Method: 使用CATDiet方法训练自监督学习模型：灰度到彩色(C)、模糊到清晰(A)、保持时间连续性(T)，并在10个数据集上进行综合评估。还开发了CombDiet方法，先用CATDiet初始化再进行标准训练。

Result: 所有CATDiet变体在物体识别中表现出增强的鲁棒性，模型展现出与生物发育对齐的模式（如神经可塑性变化类似猕猴V1区突触密度，视觉悬崖反应类似婴儿）。CombDiet在物体识别和深度感知任务上优于标准自监督学习。

Conclusion: 早期婴儿视觉经验的发展进程为理解机器中鲁棒视觉智能的出现提供了一个强大的逆向工程框架，阶段性视觉饮食能有效提升模型的鲁棒性和生物对齐性。
...
### [109] [DIR-TIR: Dialog-Iterative Refinement for Text-to-Image Retrieval](https://arxiv.org/abs/2511.14449)
*Zongwei Zhen,Biqing Zeng*

Main category: cs.CV

TL;DR: DIR-TIR框架通过对话精炼和图像精炼模块实现交互式文本到图像检索，通过多轮对话逐步优化搜索，显著提升目标图像命中准确率。


<details>
  <summary>Details</summary>
Motivation: 解决传统单查询文本到图像检索方法在可控性和容错性方面的不足，通过交互式对话更好地理解用户意图。

Method: 使用对话精炼模块主动询问用户提取关键信息生成精确描述，图像精炼模块识别生成图像与用户意图的感知差距，减少视觉语义差异。

Result: 在多种图像数据集上的实验表明，该方法显著优于仅使用初始描述的基线方法，实现了更高的检索精度和更好的交互体验。

Conclusion: DIR-TIR框架通过对话驱动的渐进式精炼策略，在交互式文本到图像检索任务中提供了优越的可控性和容错性。
...
### [110] [CompEvent: Complex-valued Event-RGB Fusion for Low-light Video Enhancement and Deblurring](https://arxiv.org/abs/2511.14469)
...
<details>
  <summary>Details</summary>
Motivation: 解决低光视频去模糊的挑战，特别是在夜间监控和自动驾驶等应用中，现有方法采用分阶段策略限制了处理低光和运动模糊组合退化的效果。

Method: 提出CompEvent框架，包含复数时序对齐GRU（使用复数卷积和GRU迭代处理视频和事件流）和复数空间频率学习模块（在空间和频域进行统一复数信号处理）。

Result: 大量实验表明CompEvent在解决这一挑战性任务上优于现有最先进方法。
...
### [111] [Learning Subglacial Bed Topography from Sparse Radar with Physics-Guided Residuals](https://arxiv.org/abs/2511.14473)
*Bayu Adhi Tama,Jianwu Wang,Vandana Janeja,Mostafa Cham*

Main category: cs.CV

TL;DR: 提出了一种物理引导的残差学习框架，用于从冰面观测重建冰下床地形，通过结合物理约束和深度学习，在格陵兰冰盖区域实现了优于传统方法的精度和物理一致性。


<details>
  <summary>Details</summary>
Motivation: 准确的冰下床地形对冰盖建模至关重要，但雷达观测数据稀疏且分布不均，需要一种能够有效利用先验知识和物理约束的方法来填补数据空白。

Method: 采用DeepLabV3+解码器和ResNet-50编码器，结合多尺度质量守恒、流向总变分、拉普拉斯阻尼、厚度非负性、斜坡先验一致性以及基于置信度图的掩码Huber拟合等物理和数据项进行训练。

Result: 在两个格陵兰子区域，该方法在测试核心区域实现了高精度和高结构保真度，优于U-Net、Attention U-Net、FPN和普通CNN等方法。
...
### [112] [2D Gaussians Spatial Transport for Point-supervised Density Regression](https://arxiv.org/abs/2511.14477)
*Miao Shang,Xiaopeng Hong*

Main category: cs.CV

TL;DR: Gaussian Spatial Transport (GST) 是一个利用高斯泼溅技术实现图像坐标空间到标注图概率度量传输的新框架，通过贝叶斯概率计算传输计划，避免了传统最优传输方法在训练时的迭代计算，显著提高了效率。


<details>
  <summary>Details</summary>
Motivation: 传统最优传输方法在计算机视觉任务中需要迭代计算传输计划，训练效率较低。本文旨在开发一个更高效的传输框架，避免训练过程中的迭代计算。

Method: 提出基于高斯泼溅的方法来估计像素-标注对应关系，然后通过贝叶斯概率计算传输计划，并推导出衡量传输后差异的损失函数，将其集成到标准网络优化中。

Result: 在人群计数和关键点检测等代表性计算机视觉任务上的广泛实验验证了该方法的有效性，相比传统最优传输方案显著提高了效率。

Conclusion: GST框架通过高斯泼溅和贝叶斯概率计算，成功实现了高效的图像坐标空间到标注图的传输，避免了训练时的迭代计算，在多个视觉任务中表现出色。
...
### [113] [Segmentation-Aware Latent Diffusion for Satellite Image Super-Resolution: Enabling Smallholder Farm Boundary Delineation](https://arxiv.org/abs/2511.14481)
...
TL;DR: SEED-SR是一种新的农场边界分割方法，通过条件潜在扩散模型和地理空间基础模型，在分割感知的潜在空间中进行超分辨率，实现了20倍尺度因子的分割图生成。


<details>
  <summary>Details</summary>
Motivation: 解决小农场边界分割的挑战：高分辨率图像更新频率低，而低分辨率图像更新频率高但分辨率不足。现有Ref-SR方法会平滑关键特征且无法满足大尺度因子需求。

Method: 使用条件潜在扩散模型和大规模多光谱、多源地理空间基础模型，绕过像素空间的显式超分辨率任务，在分割感知的潜在空间中进行超分辨率。

Result: 在两个大型真实数据集上，相比基于最先进Ref-SR方法的方法，在实例分割和语义分割指标上分别实现了25.5%和12.9%的相对改进。

Conclusion: SEED-SR方法能够以前所未有的20倍尺度因子生成分割图，有效解决了小农场边界分割中的高分辨率图像更新频率问题。
...
### [114] [Enhancing End-to-End Autonomous Driving with Risk Semantic Distillaion from VLM](https://arxiv.org/abs/2511.14499)
...
Conclusion: RSD通过增强BEV特征的风险注意力表示，使自动驾驶系统能够更好地处理空间边界和风险对象，实现更接近人类驾驶的行为。
...
### [115] [Parameter Aware Mamba Model for Multi-task Dense Prediction](https://arxiv.org/abs/2511.14503)
*Xinzhuo Yu,Yunzhi Zhuge,Sitong Gong,Lu Zhang,Pingping Zhang,Huchuan Lu*

Main category: cs.CV

TL;DR: 提出了一种基于解码器的参数感知Mamba模型(PAMM)，利用状态空间模型的丰富参数增强多任务密集预测中的任务互联性，通过双状态空间参数专家和Hilbert扫描方法提升性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法主要使用卷积层和注意力机制探索任务级交互，但缺乏对任务间关系的全局建模能力。需要一种能更好捕捉任务内在特性并增强任务互联性的新方法。

Method: 采用状态空间模型参数，设计双状态空间参数专家来整合和设置任务特定参数先验，使用结构化状态空间序列模型(S4)全局整合任务先验，并通过多方向Hilbert扫描构建多角度特征序列。

Result: 在NYUD-v2和PASCAL-Context基准测试上的广泛实验证明了该方法的有效性。

Conclusion: PAMM框架通过状态空间模型参数增强了多任务密集预测中的任务互联性，在多个基准测试上表现出色。
...
TL;DR: D-PerceptCT是一种新的LDCT图像增强方法，受人类视觉系统启发，通过视觉双路径提取器和全局-局部状态空间块来增强感知相关特征，并使用深度感知相关性损失函数来强调重要特征。


<details>
  <summary>Details</summary>
Motivation: 低剂量CT图像质量较差，现有方法过度平滑导致关键细节丢失，需要一种能增强或保留感知相关特征的方法来帮助放射科医生诊断。

Method: 提出D-PerceptCT架构，包含视觉双路径提取器（集成预训练DINOv2模型的语义先验）和全局-局部状态空间块（捕获长距离信息和多尺度特征），以及基于人类对比敏感度的深度感知相关性损失函数。

Result: 在Mayo2016数据集上的实验表明，D-PerceptCT在LDCT增强方面优于现有最先进方法，能更好地保留结构和纹理信息。

Conclusion: D-PerceptCT通过模拟人类视觉系统，有效提升了LDCT图像质量，为放射科诊断提供了更好的图像支持。
...
### [117] [A Generative Data Framework with Authentic Supervision for Underwater Image Restoration and Enhancement](https://arxiv.org/abs/2511.14521)
*Yufeng Tian,Yifan Chen,Zhe Sun,Libang Chen,Mingyu Dou,Jijun Lu,Ye Zheng,Xuelong Li*

Main category: cs.CV

TL;DR: 本文提出了一种基于非配对图像翻译的生成式数据框架，构建大规模合成水下图像数据集，解决现有方法因缺乏高质量配对数据而受限的问题。


<details>
  <summary>Details</summary>
Motivation: 当前水下图像恢复和增强方法受限于高质量配对数据集的稀缺性，现有基准数据集通常依赖算法手动选择的结果，缺乏全局一致的颜色和真实监督信号。

Method: 使用空中自然图像作为明确参考目标，通过非配对图像到图像翻译将其转换为水下退化版本，构建包含6种代表性水下退化类型的合成数据集。

Result: 在6种代表性网络架构和3个独立测试集上的实验表明，使用合成数据训练的模型在颜色恢复和泛化性能上达到或优于现有基准数据集训练的模型。

Conclusion: 该研究为水下图像恢复和增强提供了可靠且可扩展的数据驱动解决方案，构建的合成数据集具有精确的真实标签，能够促进从退化水下图像到原始场景外观的准确映射学习。
...
### [118] [DeCo-VAE: Learning Compact Latents for Video Reconstruction via Decoupled Representation](https://arxiv.org/abs/2511.14530)
*Xiangchen Yin,Jiahui Yuan,Zhangchi Hu,Wenzhang Sun,Jie Chen,Xiaozhen Qiao,Hao Li,Xiaoyan Sun*

Main category: cs.CV

TL;DR: 提出DeCo-VAE方法，通过解耦视频内容为关键帧、运动和残差三个组件，分别学习专用潜在表示，实现紧凑的视频表征。
...
Method: 将视频内容分解为关键帧、运动和残差三个组件，为每个组件设计专用编码器，使用共享3D解码器保持时空一致性，并采用解耦适应策略进行稳定训练。

Result: 大量定量和定性实验表明DeCo-VAE实现了优越的视频重建性能。

Conclusion: 通过解耦视频内容并分别建模，DeCo-VAE能够实现紧凑且高效的视频潜在表示。
...
### [119] [Learning Compact Latent Space for Representing Neural Signed Distance Functions with High-fidelity Geometry Details](https://arxiv.org/abs/2511.14539)
...
Method: 采用结合泛化学习和过拟合学习策略的框架，利用两者的优势；引入新的采样策略来采样训练查询，提高训练效率并消除其他SDF影响造成的伪影。

Result: 在广泛使用的基准测试上进行了数值和视觉评估，验证了设计有效性，在表示能力和紧凑性方面优于最新方法。

Conclusion: 该方法能够用更紧凑的潜在表示恢复更多高保真几何细节，在多个SDF的表示分析中表现出优势。
...
### [120] [Interaction-Aware 4D Gaussian Splatting for Dynamic Hand-Object Interaction Reconstruction](https://arxiv.org/abs/2511.14540)
*Hao Tian,Chenyangguang Zhang,Rui Liu,Wen Shen,Xiaolin Qin*

Main category: cs.CV

TL;DR: 提出了一种无需物体先验的动态手-物体交互场景建模方法，通过交互感知的高斯表示和动态场来同时重建几何和外观，采用渐进式优化策略和显式正则化提升重建质量。


<details>
  <summary>Details</summary>
Motivation: 解决无物体先验条件下动态手-物体交互场景的几何和外观建模挑战，特别是处理相互遮挡、边缘模糊和复杂运动模式的问题。

Method: 使用交互感知的手-物体高斯表示，引入可优化参数采用分段线性假设；将手部信息融入物体变形场构建交互感知动态场；采用渐进式优化策略分步处理动态区域和静态背景；设计显式正则化确保平滑运动、物理交互真实性和光照一致性。

Result: 实验表明该方法超越了现有的动态3D-GS方法，在手-物体交互重建中达到了最先进的性能。

Conclusion: 该方法能够有效建模复杂的手-物体交互场景，无需物体先验知识，在动态重建质量方面表现优异。
...
### [121] [ForensicFlow: A Tri-Modal Adaptive Network for Robust Deepfake Detection](https://arxiv.org/abs/2511.14554)
*Mohammad Romani*

Main category: cs.CV

TL;DR: ForensicFlow是一个三模态Deepfake检测框架，融合RGB、纹理和频率证据，通过注意力机制动态整合多尺度伪造痕迹，在Celeb-DF数据集上达到97.52% AUC和92.08%准确率。


<details>
  <summary>Details</summary>
Motivation: 现有单流CNN无法捕捉空间、纹理和频域中的多尺度伪造痕迹，限制了Deepfake检测的鲁棒性和泛化能力。

Method: 提出三模态框架：RGB分支(ConvNeXt-tiny)提取全局视觉不一致性；纹理分支(Swin Transformer-tiny)检测细粒度混合伪影；频率分支(CNN+SE)识别周期性频谱噪声。使用注意力时序池化和自适应注意力融合。

Result: 在Celeb-DF(v2)数据集上，使用Focal Loss训练，达到AUC 0.9752、F1-Score 0.9408、准确率0.9208，优于单流基线方法。

Conclusion: 多模态特征融合提供了对细微伪造的优越鲁棒性，消融实验验证了分支协同作用，Grad-CAM确认了法医关注点。
...
### [122] [Explaining Digital Pathology Models via Clustering Activations](https://arxiv.org/abs/2511.14558)
...
<details>
  <summary>Details</summary>
Motivation: 传统基于显著性图的方法（如遮挡、GradCAM、相关性传播）只能突出单个切片中影响预测的区域，缺乏对模型全局行为的理解，限制了在临床实践中的应用。

Method: 采用聚类方法分析卷积神经网络模型，通过结果聚类可视化来理解模型行为，增加对模型操作的信心。

Result: 在前列腺癌检测模型上评估了该技术的性能，证明了其有效性。

Conclusion: 该聚类可解释性技术能够更好地理解模型全局行为，提高临床实践中的采用速度。
...
TL;DR: OmniZip是一个无需训练、音频引导的音视频token压缩框架，通过识别关键音频token并计算音频保留分数，动态指导视频token剪枝，实现3.42倍推理加速和1.4倍内存减少。


<details>
  <summary>Details</summary>
Motivation: 全模态大语言模型在处理音视频token序列时存在显著计算瓶颈，现有token压缩方法尚未满足多模态token联合压缩的需求。

Method: 首先识别关键音频token，计算每个时间组的音频保留分数以捕捉信息密度，动态指导视频token剪枝，并通过跨模态相似性增强音频锚点线索。在每个时间窗口使用交错时空方案压缩视频token。

Result: 实现了3.42倍推理加速和1.4倍内存减少，相比其他顶级方法保持性能且无需训练。
...
### [124] [XAttn-BMD: Multimodal Deep Learning with Cross-Attention for Femoral Neck Bone Mineral Density Estimation](https://arxiv.org/abs/2511.14604)
...
Result: 模型在Hertfordshire队列研究数据上表现优异，相比无交叉注意力的简单特征拼接，MSE降低16.7%，MAE降低6.03%，R2得分提高16.4%。在临床相关骨密度阈值下的二元分类也显示出良好的筛查性能。
...
### [125] [3D-Guided Scalable Flow Matching for Generating Volumetric Tissue Spatial Transcriptomics from Serial Histology](https://arxiv.org/abs/2511.14613)
...
<details>
  <summary>Details</summary>
Motivation: 现有的预测算法大多将每个切片独立处理而忽略3D结构，而现有的3D感知方法不可生成且扩展性差，需要一种能够处理3D结构并具有良好扩展性的方法。
...
### [126] [Fusing Biomechanical and Spatio-Temporal Features for Fall Prediction: Characterizing and Mitigating the Simulation-to-Reality Gap](https://arxiv.org/abs/2511.14620)
*Md Fokhrul Islam,Sajeda Al-Hammouri,Christopher J. Arellano,Kavan Hazeli,Heman Shakeri*

Main category: cs.CV

TL;DR: 提出了BioST-GCN双流模型，结合姿态和生物力学信息进行跌倒预测，在模拟数据上表现良好但存在模拟-现实差距问题。


<details>
  <summary>Details</summary>
Motivation: 跌倒对老年人造成严重伤害，基于视觉的跌倒预测系统需要解决真实跌倒数据稀缺的问题。

Method: 使用Biomechanical Spatio-Temporal Graph Convolutional Network (BioST-GCN)双流模型，通过交叉注意力机制融合姿态和生物力学信息。

Result: 在模拟数据集上比基线ST-GCN提升5.32%和2.91% F1分数，但零样本泛化到真实受试者时性能降至35.9%。
...
### [127] [SparseSurf: Sparse-View 3D Gaussian Splatting for Surface Reconstruction](https://arxiv.org/abs/2511.14633)
*Meiying Gu,Jiawei Zhang,Jiahe Li,Xiaohan Yu,Haonan Luo,Jin Zheng,Xiao Bai*

Main category: cs.CV

TL;DR: 提出了一种名为\net{}的方法，通过立体几何-纹理对齐和伪特征增强几何一致性来解决稀疏视图下高斯泼溅优化中的过拟合问题，提升表面重建和新视角合成的质量。


<details>
  <summary>Details</summary>
Motivation: 稀疏视图输入时，高斯泼溅优化容易过拟合，导致重建质量下降。现有方法使用扁平高斯基元结合深度正则化，但扁平高斯固有的各向异性加剧了稀疏视图下的过拟合问题。

Method: 引入立体几何-纹理对齐来连接渲染质量和几何估计，同时提出伪特征增强几何一致性，通过结合训练视图和未见视图来强制多视角几何一致性。

Result: 在DTU、BlendedMVS和Mip-NeRF360数据集上的大量实验表明，该方法达到了最先进的性能。

Conclusion: 所提出的方法能够重建更准确和详细的表面，同时保持高质量的新视角渲染，有效缓解了稀疏监督导致的过拟合问题。
...
### [128] [SLAM-AGS: Slide-Label Aware Multi-Task Pretraining Using Adaptive Gradient Surgery in Computational Cytology](https://arxiv.org/abs/2511.14639)
*Marco Acerbis,Swarnadip Chatterjee,Christophe Avenel,Joakim Lindblad*

Main category: cs.CV

TL;DR: SLAM-AGS是一个面向计算细胞学的多任务预训练框架，通过联合优化弱监督相似性目标和自监督对比目标，在低见证率下提升下游任务性能，并使用自适应梯度手术解决任务梯度冲突问题。


<details>
  <summary>Details</summary>
Motivation: 计算细胞学面临两个主要挑战：实例级标签不可靠且获取成本高，以及见证率极低。需要一种能在低监督信号下稳定学习的方法。
...
<details>
  <summary>Details</summary>
Motivation: 手动标注气道不切实际，现有U-Net方法常产生不连通组件，影响可靠生物标志物提取

Method: 三阶段框架：nnU-Net初始分割、基于骨架的断点识别和重连提议、1D卷积分类器判断真假解剖分支

Result: 在ATM'22和AeroPath数据集上优于现有3D U-Net方法，在体素级和拓扑指标上表现更好

Conclusion: RepAir能生成更完整、解剖一致的气道树，同时保持高分割精度
...
### [130] [HyMAD: A Hybrid Multi-Activity Detection Approach for Border Surveillance and Monitoring](https://arxiv.org/abs/2511.14698)
*Sriram Srinivasan,Srinivasan Aruchamy,Siva Ram Krisha Vadali*

Main category: cs.CV

TL;DR: 提出HyMAD框架，通过融合频谱和时序特征来检测和区分同时发生的地震活动（如人类入侵、动物移动和车辆行驶），解决了边境监控中地震信号复杂重叠的挑战。


<details>
  <summary>Details</summary>
Motivation: 地震传感器在边境监控中具有隐蔽性优势，但难以准确检测和区分同时发生的复杂活动，这会导致误分类和漏检，降低监控系统的可靠性。

Method: 基于深度神经网络的HyMAD架构，整合SincNet提取的频谱特征和RNN建模的时序依赖，使用自注意力层增强模态内表示，并通过跨模态融合模块实现鲁棒的多标签分类。

Result: 在真实边境监控数据集上的评估表明，该方法能够泛化到涉及人类、动物和车辆的同时活动场景，取得了有竞争力的性能。

Conclusion: HyMAD提供了一个模块化框架，可扩展地震活动识别在现实安全应用中的能力。
...
TL;DR: 提出了一种无需训练的FreeSwim方法，利用预训练的视频扩散Transformer生成更高分辨率的视频，通过内滑动窗口注意力和交叉注意力覆盖策略解决高分辨率生成中的重复内容和全局一致性问题。


<details>
  <summary>Details</summary>
Motivation: 现代基于Transformer的视频生成器中注意力机制的二次时间和内存复杂度使得超高分辨率视频的端到端训练成本过高，因此需要一种无需额外训练的方法来生成更高分辨率的视频。

Method: 采用内滑动窗口注意力机制，并设计了双路径流水线，通过交叉注意力覆盖策略让局部注意力产生的语义内容受到具有完整感受野分支的引导，同时引入交叉注意力缓存策略提高效率。

Result: 实验表明该方法能够在无需训练的情况下生成具有精细视觉细节的超高分辨率视频，在VBench基准测试中表现优于基于训练的方法，且具有竞争性或改进的效率。

Conclusion: FreeSwim方法提供了一种高效且无需训练的解决方案，能够生成高质量的超高分辨率视频，在保持视觉保真度和全局一致性的同时显著降低了计算成本。
...
TL;DR: 提出DSD框架，将传统扩散模型的编码器、解码器和扩散网络统一为单网络端到端训练，解决了联合训练中的潜在崩溃问题。


<details>
  <summary>Details</summary>
Motivation: 传统扩散模型的三部分架构计算效率低、性能次优，且无法与视觉基础模型的单网络架构统一。目标是统一这三个组件为单一端到端可训练网络。

Method: 提出扩散作为自蒸馏(DSD)框架，通过修改训练目标来稳定潜在空间，实现编码、解码和扩散的同步学习。

Result: 在ImageNet 256×256条件生成任务上取得优异性能：FID=13.44/6.38/4.25，参数量仅42M/118M/205M，训练50个epoch，无需分类器自由引导。

Conclusion: DSD首次实现了单网络端到端稳定训练，统一了编码、解码和扩散过程，显著提升了计算效率和性能。
...
TL;DR: 提出基于神经场的3D城市环境隐式表示方法，通过向量场编码视图来支持高效的城市数据分析，解决了传统3D探索中的遮挡问题和手动视角调整的低效性。


<details>
  <summary>Details</summary>
Motivation: 3D城市数据集的复杂性导致严重的遮挡问题和大量手动视角调整需求，使得大规模探索效率低下。

Method: 使用神经场构建3D环境的隐式表示，支持直接查询（视图评估指标计算）和逆向查询（避免遮挡、寻找匹配数据模式的视图）。

Result: 验证表明该方法能有效找到理想视点、分析建筑立面可见性、评估室外空间视野，并通过定量实验、真实案例和专家反馈得到证实。

Conclusion: 该方法为城市分析任务（如可视性评估、日照分析、新开发项目视觉影响评估）提供了高效的解决方案，代码和数据已公开。
...
Result: 在EngageNet基准测试中（6个特征设置中的3个）最大提升+1.21%，在DREAMS/PAFE基准测试中F1得分分别提升+0.22/+0.06，超越了现有最佳方法。

Conclusion: 通过VLMs处理标签主观性，在精炼的高可靠性子集上训练并结合课程学习策略，能够有效提升视频参与度识别的性能。
...
TL;DR: Co-Me是一种无需重新训练或微调的视觉几何Transformer加速机制，通过轻量级置信度预测器对token进行不确定性排序，选择性地合并低置信度token以减少计算量，同时保持空间覆盖范围。


<details>
  <summary>Details</summary>
Motivation: 现有的基于相似性的合并或剪枝方法在视觉几何Transformer中效果有限，需要一种能够可靠识别Transformer关注区域并实现显著加速的方法。

Method: 使用轻量级置信度预测器对token进行不确定性排序，选择性地合并低置信度token，从而减少计算复杂度。该方法可无缝应用于多视图和流式视觉几何Transformer。

Result: 在VGGT和MapAnything上分别实现了11.3倍和7.2倍的加速，使视觉几何Transformer能够应用于实时3D感知和重建任务。

Conclusion: Co-Me通过置信度引导的token合并实现了视觉几何Transformer的显著加速，且不降低性能，使其更适合实时应用场景。
...
Method: 提出了统一的强化学习策略，通过共享奖励模型联合改进图像生成和编辑；引入轻量级的编辑指令对齐阶段，显著提升编辑指令理解能力。

Result: 在GenEval和ImgEdit上分别获得0.89和4.31的总体得分，超越了BAGEL等最先进模型，性能可与GPT-Image-1等专有模型相媲美。

Conclusion: UniGen-1.5展示了在图像理解、生成和编辑方面的竞争性表现，为多模态模型的发展提供了有效的解决方案。
...
TL;DR: 评估多种水印技术对抗攻击的鲁棒性及其对文本质量和写作风格的保持能力，发现现有水印技术能保持语义但会改变写作风格，且容易受到对抗攻击，特别是回译攻击。


<details>
  <summary>Details</summary>
Motivation: 减轻大语言模型生成文本的潜在危害，通过水印技术嵌入可检测信号，但由于水印技术可能降低文本质量且易受对抗攻击，导致LLM创作者对广泛采用水印技术存在抵触。

Method: 通过比较释义和回译攻击评估多种水印技术的鲁棒性，并使用语言指标来评估水印文本在质量和写作风格方面与未水印文本的保持能力。

Result: 水印技术能够保持语义，但会偏离未水印文本的写作风格，并且容易受到对抗攻击，特别是回译攻击。

Conclusion: 当前的水印技术在保持语义方面表现良好，但在保持写作风格和对抗攻击鲁棒性方面存在不足，尤其是对回译攻击的抵抗力较弱，这限制了水印技术的广泛采用。
...
<details>
  <summary>Details</summary>
Motivation: 增强文本嵌入模型的语义推理能力，激活预训练期间学习到的语义推理潜力。

Method: 通过多次前向传递运行文本嵌入模型，获得最终的语义表示，这是一种测试时推理方法。
...
TL;DR: 本文提出了两种无需训练的QE-informed错误校正方法：一种通过选择多个LLM候选中的最高质量翻译，另一种基于QE解释替换错误子串。第一种方法在WMT 2025任务中获胜。


<details>
  <summary>Details</summary>
Motivation: 虽然联合训练QE和APE系统能提升性能，但APE系统存在过度校正问题。本文旨在探索无需训练的方法来避免过度校正，同时利用QE信息改进翻译质量。

Method: 1. QE-informed重翻译：从不同LLM生成的多个候选翻译中选择质量最高的一个
2. QE解释指导的替换：基于提供的QE解释，指导LLM替换错误子串，使用条件启发式最小化编辑次数

Result: 两种方法分别获得Delta COMET分数0.0201和-0.0108，第一种方法在子任务排行榜上获得获胜位置。

Conclusion: 无需训练的QE-informed重翻译方法在避免过度校正的同时能有效提升翻译质量，优于基于QE解释的替换方法。
...
TL;DR: 论文研究了LLM在长上下文中的"迷失在中间"现象，提出了GM-Extract基准数据集，通过两种评价指标系统评估了7-8B参数模型在多文档任务中的检索性能，并对缓解方法进行了分类和测试。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在利用长距离上下文时出现"迷失在中间"现象，这对基于检索的LLM应用构成重大挑战，需要在实际应用环境中研究这一现象的影响。

Method: 提出了GM-Extract基准数据集，使用两种评价指标（空间检索能力和语义检索能力），系统评估了7-8B参数模型在两个多文档任务上的表现，并分析了数据表示方式对性能的影响。

Result: 研究发现改变上下文窗口中数据的表示方式会显著影响检索性能，虽然没有一致观察到明显的U形曲线，但发现了清晰的性能模式，并与困惑度得分相关。缓解方法的效果高度微妙，有些情况下能提升性能，有些情况下会产生负面影响。

Conclusion: 论文提供了对LLM长上下文检索能力的全面理解，揭示了缓解策略在实际应用中的复杂效用，为改进检索型LLM应用提供了重要见解。
...
TL;DR: LLM框架通过提取结构化解释来处理包含最高级（如“最好”、“最流行”）的搜索查询，将查询分解为属性-值提示，在检索过程中生成，提升搜索性能10.9 MAP点和5.9 MRR点，并通过将解释转移到轻量级模型解决延迟问题。


<details>
  <summary>Details</summary>
Motivation: 包含最高级的搜索查询需要在多个维度比较候选结果，需要语言理解和领域知识，现有方法难以有效处理这类查询的潜在意图。

Method: 开发一个框架，通过LLM提取结构化解释或提示，将查询分解为属性-值提示，在检索过程中并发生成，并将最高级解释转移到轻量级模型以解决延迟问题。

Result: 方法在搜索性能上比基线提升10.9 MAP点和5.9 MRR点，成功解决了LLM直接重排的高延迟问题。

Conclusion: 该研究展示了如何表示和传递最高级语义，推进了检索系统中的语言理解，同时解决了实际部署的约束条件。
...
TL;DR: MoRA-RAG是一个基于知识的LLM框架，通过混合检索机制和代理分块技术，将灾后勘察报告转化为结构化数据，用于多灾害推理，准确率达94.5%，显著减少幻觉。
...
Result: 在HazardRecQA数据集上，MoRA-RAG达到94.5%准确率，比零样本LLMs提高30%，比最先进RAG系统提高10%，同时在不同LLM架构中减少幻觉，使开源LLM达到与专有模型相当的性能。
...
TL;DR: HiEAG是一个分层证据增强生成框架，通过多模态大语言模型改进外部一致性检查，在OOC虚假信息检测中超越现有SOTA方法
...
Method: 提出分层证据增强生成框架，将外部一致性检查分解为检索、重排序和重写的综合流程，使用AESP和AEGP提示技术

Result: 在不同基准数据集上的实验结果表明，HiEAG在所有样本的准确率上超越了之前的最先进方法

Conclusion: HiEAG框架通过利用MLLMs的广泛知识，有效提升了外部一致性检查能力，在OOC虚假信息检测中表现出色
...
TL;DR: 构建平衡的多标签情感数据集并开发增强分类模型，显著提升模型性能
...
Result: 相比不平衡数据训练的模型，在准确率、精确率、召回率、F1分数和AUC方面均有显著提升

Conclusion: 所提出的平衡数据集和增强分类模型方法在多标签情感分类任务中具有显著效果
...
<details>
  <summary>Details</summary>
Motivation: 推理增强视觉语言模型虽然依赖安全对齐来防止有害行为，但其暴露的思维链痕迹引入了新的攻击面。

Method: 通过段级干扰引发有害推理痕迹，并将自生成输出作为监督微调数据，采用轮次加权损失设计实现轻量级、分布一致的微调方法。
...
<details>
  <summary>Details</summary>
Motivation: 临床文本自动ICD编码任务受到诊断代码极端长尾分布的阻碍，数千个罕见和零样本ICD代码在数据集中严重不足，导致宏观F1分数偏低。

Method: 构建以罕见代码为中心的现实多标签代码集，利用真实世界共现模式、ICD描述、同义词、分类法和相似临床笔记生成结构化提示，产生90,000条合成笔记覆盖7,902个ICD代码。
...
TL;DR: HyperABSA是一个动态超图框架，通过样本特定的层次聚类构建方面-观点结构，解决了传统图方法在短文本ABSA任务中的冗余和误差传播问题。


<details>
  <summary>Details</summary>
Motivation: 传统基于图的方法只能建模成对依赖关系，需要构建多个图来捕捉不同关系视图，这引入了冗余、参数开销和融合过程中的误差传播，限制了在短文本、低资源设置下的鲁棒性。

Method: 提出动态超图框架，通过引入加速-回退截止点的层次聚类方法自适应确定粒度级别，构建样本特定的方面-观点结构。

Result: 在三个基准数据集（Lap14、Rest14、MAMS）上的实验显示，相比强图基线方法取得了一致的改进，特别是与RoBERTa骨干网络结合时获得了显著提升。
...
TL;DR: 将基于Transformer的关系提取与知识图谱匹配相结合，用于回答多项选择题，同时保持输出过程的可追溯性。通过验证生成的关系图谱与事实正确的知识图谱来评估问题句子的真实性。


<details>
  <summary>Details</summary>
Motivation: 知识图谱构建成本高，通常被视为静态数据库。但基于Transformer的关系提取方法能够动态生成知识图谱，为表示输入句子的含义提供了可能性，同时需要处理关系提取方法可能生成错误信息的问题。

Method: 提出一种方法：使用关系提取方法将问题句子转换为关系图谱，然后在封闭世界假设下与事实正确的知识图谱进行验证，从而评估问题句子的真实性并回答填空式多项选择题。

Result: 实验结果表明，该方法能正确回答约70%的问题，同时提供过程的可追溯性。问题类别对准确率有显著影响。

Conclusion: 结合关系提取和知识图谱匹配的方法能有效回答多项选择题，并保持可追溯性。问题类别是影响性能的关键因素。
...
<details>
  <summary>Details</summary>
Motivation: 解决超人类模型只能获得弱监督的问题，现有弱到强泛化方法中部分弱标签对模型有害，需要避免不必要的弱监督使用。
...
TL;DR: 提出首个基于符号语言知识的幻觉定位框架，揭示LLMs中的幻觉本质上是符号语义处理失败而非一般生成问题。


<details>
  <summary>Details</summary>
Motivation: LLMs在遇到修饰语、否定、数字、例外和命名实体等符号触发器时仍存在幻觉问题，但缺乏对这些符号幻觉起源的清晰理解，需要系统性地处理这些触发器并在模型内部定位幻觉的出现位置。

Method: 提出符号定位框架，利用符号语言和语义知识追踪幻觉在所有模型层中的发展过程，通过分析模型如何处理符号触发器，使用HaluEval和TruthfulQA对五个模型进行分析。

Result: 符号知识方法显示，这些语言元素的注意力方差在早期层（2-4层）爆炸性增长至临界不稳定状态，否定触发灾难性方差水平，表明符号语义处理从一开始就崩溃。尽管模型规模更大，幻觉率仍保持高位（Gemma变体为78.3%-83.7%），符号语义触发器在深层中注意力急剧下降。
...
TL;DR: DeepEL是一个将大语言模型(LLM)完全集成到实体链接所有阶段的框架，通过自验证机制利用全局上下文信息纠正预测，在10个基准数据集上显著优于现有方法。
...
Method: 提出DeepEL框架，将LLM集成到实体链接的所有阶段，并引入自验证机制利用全局上下文信息来纠正预测和识别实体间的连贯关系。

Result: 在10个基准数据集上，DeepEL显著优于现有最先进方法，整体F1分数平均提升2.6%，在域外数据集上提升4%。

Conclusion: 深度LLM集成能有效推进实体链接技术的前沿发展。
...
TL;DR: 提出了ArbESC+系统，这是首个用于阿拉伯语语法错误校正的多系统集成方法，通过组合多个模型的特征并使用分类器选择最佳修正，显著提升了校正性能。


<details>
  <summary>Details</summary>
Motivation: 阿拉伯语具有复杂的形态和句法结构，比其它语言更具挑战性。虽然现代神经网络模型已有很大改进，但以往方法多使用单一模型，没有充分利用多系统组合的潜力。

Method: 使用多个模型（AraT5、ByT5、mT5、AraBART等）收集修正建议，将其表示为数值特征，通过分类器确定并实施适当修正。框架还使用支持技术过滤重叠修正并评估决策可靠性。
...
TL;DR: 构建大规模音乐相关语料库(40B tokens)和领域优先的数据管道，通过参考模型软评分进行质量控制，提出MusicSimpleQA基准评估事实性，为音乐领域LLM提供可扩展的数据训练框架。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在通用任务上表现良好，但在音乐等专业领域受到限制，特别是在音乐娱乐领域，语料库规模、纯净度以及数据与训练目标的匹配至关重要。

Method: 构建大规模音乐相关自然语言语料库，采用领域优先数据管道进行过滤、清洗、去重和隐私保护，使用参考模型进行token级软评分进行质量控制，设计MusicSimpleQA基准评估事实性。

Result: 提出了一个可扩展的数据训练框架和可重用的评估工具，能够更有效地进行音乐领域的持续预训练和对齐。

Conclusion: 这项工作在语料库构建和训练目标方面都取得了进展，为音乐领域构建领域专用LLM提供了系统性的解决方案。
...
TL;DR: 提出Authentic-Dubber模型，通过检索增强的导演-演员交互学习方案实现真实电影配音，包含多模态参考素材库、情感相似性检索增强策略和渐进式图基语音生成方法，显著提升情感表现力。


<details>
  <summary>Details</summary>
Motivation: 现有配音方法模拟简化的演员直接配音流程，忽视了导演与演员之间的关键互动环节，而真实配音工作流程涉及导演指导演员内化情感上下文线索的动态协作。

Method: 构建多模态参考素材库模拟导演提供的学习素材，使用LLM深度理解多模态信号中的情感表征；提出情感相似性检索增强策略，检索与目标静默视频对齐的最相关多模态信息；开发渐进式图基语音生成方法，逐步整合检索到的多模态情感知识。

Result: 在V2C Animation基准数据集上的主观和客观评估验证了有效性，实现了情感表现力的全面改进。

Conclusion: Authentic-Dubber能够忠实复制真实的配音工作流程，通过模拟导演-演员互动显著提升了配音的情感表现质量。
...
Method: 构建包含100多种非洲英语口音的评估套件，涵盖7个应用领域，使用自发和非自发语音对话数据，对开源、闭源、单模态ASR和多模态LLM语音识别系统进行全面基准测试。

Result: 开源ASR在自发语音中表现良好但在嘈杂非母语对话中下降；多模态LLM对口音更鲁棒但在领域特定命名实体上表现不佳；专有模型在清晰语音中准确度高但随国家和领域变化显著；非洲英语微调模型在延迟和准确性方面具有竞争优势。
...
TL;DR: 本文提出了一种熵引导的训练框架来解决推理模型压缩过程中的熵冲突问题，将推理长度压缩到原始的20%同时保持或提升准确率。
...
Method: 采用熵引导的训练框架：当熵值下降时引导模型进行简洁推理；当熵值上升时在紧凑推理模式下加强探索以提高鲁棒性。分析发现熵冲突源于逻辑连接词等高熵标记在性能目标下获得更大梯度被鼓励，而压缩目标同时惩罚这些可能冗余的连接词。

Result: 在六个数学基准测试上的实验表明，该方法将推理长度压缩到原始的20%，同时保持甚至超越了基线准确率。

Conclusion: 熵引导训练框架有效解决了推理模型压缩中的熵冲突问题，实现了显著的推理长度压缩同时维持或提升性能，为实际部署提供了可行方案。
...
TL;DR: 提出了一种通过预测口头化概率分布来增强LLM置信度估计的方法，该方法要求模型考虑答案空间中的所有候选答案而非单一猜测，从而促进深度推理。


<details>
  <summary>Details</summary>
Motivation: 了解模型响应的可靠性对应用至关重要，现有研究关注生成口头化置信度并结合思维链推理，但推理策略如何影响置信度估计仍待探索。

Method: 通过预测口头化概率分布来鼓励深度推理，要求LLM考虑答案空间中的所有候选答案，并仔细分配置信度分数以满足分布要求。

Result: 该方法在不同模型和任务中均表现出优势，无论答案空间是否已知，且在强化学习后仍保持优势，其推理模式与人类期望一致。

Conclusion: 预测口头化概率分布是一种有效的置信度估计方法，能促进深度推理并在多种场景下保持优势。
...
TL;DR: AraLingBench是一个完全人工标注的基准测试，用于评估大型语言模型的阿拉伯语语言能力，包含语法、形态学、拼写、阅读理解和句法五个核心类别，共150道专家设计的选择题。
...
Method: 开发包含150道专家设计选择题的基准测试，涵盖五个语言核心类别，评估35个阿拉伯语和双语LLMs。

Result: 评估显示当前模型在表层语言能力上表现良好，但在深层语法和句法推理方面存在困难，许多模型通过记忆或模式识别而非真实理解来获得高分。

Conclusion: AraLingBench揭示了知识型基准测试高分与真正语言掌握之间的差距，为开发阿拉伯语LLMs提供了诊断框架。
...
Method: 引入ConInstruct基准测试数据集，专门评估LLMs检测和解决用户指令中冲突的能力，并进行实验分析。

Result: 专有LLMs冲突检测能力强，DeepSeek-R1和Claude-4.5-Sonnet表现最佳；但LLMs很少明确通知用户冲突或请求澄清。

Conclusion: 当前LLMs在冲突处理方面存在明显不足，这是设计指令遵循型LLMs时需要改进的重要方向。
...
TL;DR: 本文提出了一种解决LLMs在化学领域应用的tokenization瓶颈的方法，通过统一自然语言和分子结构的表示，在预训练LLM中扩展化学相关词汇并进行领域适应性训练。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在化学应用中常遇到"tokenization瓶颈"，即通用文本的分词器会将化学表示（如SMILES）分割成语义无意义的子标记，阻碍模型理解化学结构。

Method: 采用目标词汇扩展策略，在预训练LLM的词汇表中添加化学相关标记，然后在化学领域文本上进行持续预训练，以整合新的化学知识。

Result: 该方法在多个下游化学任务上表现出优越性能，证明了策略的有效性。

Conclusion: 通过统一自然语言和分子结构的表示，并采用词汇扩展和领域适应性训练，可以有效解决LLMs在化学应用中的tokenization瓶颈问题。
...
Method: 由领域专家开发，涵盖7个核心科学领域，采用原创问题设计、跨学科整合、高保真答案格式和多阶段质量控制的综合方法。
...
TL;DR: 提出了标准化上下文校准（NCC）方法来解决LLMs中的标签长度偏差问题，该方法在完整标签级别进行标准化和校准，显著提升了多个数据集和模型的性能。


<details>
  <summary>Details</summary>
Motivation: LLMs在多标签预测中存在标签偏差，特别是多token类标签导致的标签长度偏差问题，现有校准方法未能有效解决。

Method: 提出标准化上下文校准（NCC），在完整标签级别对预测进行标准化和校准，而不是单个token级别。

Result: NCC在多个数据集和模型上取得了统计显著的改进，F1分数提升高达10%，并且在少样本学习中对示例选择更不敏感，需要更少示例就能达到竞争性性能。

Conclusion: 缓解完整标签偏差对于提高LLM方法的性能和鲁棒性至关重要，特别是在现实应用中类标签自然包含多个token的场景。
...
<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注通用安全性评估，缺乏对教育场景独特安全需求的关注，而LLM在教育应用中容易受到越狱和微调攻击的威胁。

Method: 构建EduHarm基准包含五个教育场景的安全-不安全指令对；提出三阶段防护框架：安全感知注意力重对齐、分层安全判断、防御驱动双路由。

Result: 在八种越狱攻击策略下有效增强安全性，防止良性查询的过度拒绝；在三个微调攻击数据集上实现稳健防御，同时保持良性微调的效用增益。

Conclusion: TSSF框架能够同时缓解越狱和微调攻击，为教育LLM提供有效的安全保障，同时维持对良性查询的正常处理能力。
...
TL;DR: MedBench v4是一个全国性的医疗AI基准测试平台，包含70多万个专家策划的任务，涵盖24个主要和91个次要专科，评估了15个前沿模型。基础LLM平均得分54.1/100，但安全和伦理得分较低(18.4/100)。多模态模型表现更差(平均47.5/100)，而基于相同骨干的智能体显著提升性能(平均79.8/100)。


<details>
  <summary>Details</summary>
Motivation: 随着医疗大语言模型、多模态模型和智能体的发展，需要能够反映真实临床工作流程和安全约束的评估框架。现有的基准测试无法充分评估这些模型在复杂医疗场景中的表现。

Method: 构建了包含70多万个专家策划任务的云基准测试基础设施，任务经过多阶段细化和多轮临床医生评审。开放式回答使用经过人类评分校准的LLM作为评判者进行评分。

Result: 基础LLM平均得分54.1/100(最佳：Claude Sonnet 4.5，62.5/100)，但安全和伦理得分很低(18.4/100)。多模态模型表现更差(平均47.5/100；最佳：GPT-5，54.9/100)。基于相同骨干的智能体显著提升性能(平均79.8/100)，Claude Sonnet 4.5智能体达到85.3/100总分和88.9/100安全任务得分。

Conclusion: MedBench v4揭示了基础模型在多模态推理和安全性方面存在持续差距，但具有治理意识的智能体编排可以显著提升临床准备度而不牺牲能力。该平台为中国医院、开发者和政策制定者审核医疗AI提供了实用参考。
...
<details>
  <summary>Details</summary>
Motivation: 解决心理健康资源获取障碍，为研究提供合成治疗数据，通过AI技术补充现有护理资源。

Method: 系统包含三个组件：RAG对话助手提供个性化支持；合成对话生成器基于用户档案生成治疗对话；Well-being AI crew使用CrewAI生成周度自我护理计划和冥想音频。

Result: 系统在精心设计的健康场景中进行了评估，使用了自动LLM判断和人类用户研究。
...
<details>
  <summary>Details</summary>
Motivation: RL在训练LLM Agent方面具有巨大潜力但面临挑战，当前缺乏针对LLM Agent的RL方法深入探索和灵活可扩展的训练框架。
...
TL;DR: LiveRAG是一个包含895个合成问答对的公开基准数据集，用于系统评估基于检索增强生成的问答系统，源自SIGIR'2025 LiveRAG挑战赛。
...
Method: 构建包含895个合成问答对的基准数据集，包含真实答案、支持性声明，以及通过项目反应理论模型估计的难度和区分度分数。

Result: 分析显示基准问题具有多样性、难度范围广泛，并能有效区分系统能力。
...
TL;DR: 本文研究了文档级声明提取的评估方法，通过声明对齐和相似度计算来比较模型提取与人工标注的声明集，并在捷克和斯洛伐克新闻评论数据集上验证了方法的有效性。


<details>
  <summary>Details</summary>
Motivation: 文档级声明提取在事实核查领域仍面临挑战，现有的评估方法关注有限，需要建立可靠的评估框架来比较模型提取和人工标注的声明集。

Method: 提出声明对齐方法，通过识别最佳对齐和评估方法来计算两个声明集之间的相似度得分，并应用于捷克和斯洛伐克新闻评论数据集。

Result: 实验结果揭示了当前评估方法在文档级声明提取中的局限性，特别是在处理非正式语言、强本地语境和语言细微差别时的不足。

Conclusion: 需要开发更先进的评估方法，能够准确捕捉语义相似度并评估声明的关键属性，如原子性、可核查性和去语境化。
...
TL;DR: 利用历史报纸的头版预告自动收集多语言摘要数据，构建了首个希伯来语多文档摘要数据集HEBTEASESUM


<details>
  <summary>Details</summary>
Motivation: 在少数语言中高质量摘要数据稀缺，而数字化历史报纸提供了丰富的自然标注数据源

Method: 通过头版预告自动收集编辑撰写的文章摘要，开发适合不同语言资源水平的自动处理流程

Result: 在七种不同语言中验证了该现象的普遍性，并成功构建了希伯来语多文档摘要数据集

Conclusion: 历史报纸的头版预告是获取自然摘要数据的有效方法，可扩展到资源匮乏的语言
...
TL;DR: 使用电子健康记录数据动态追踪急性肾损伤患者的临床演变，识别15种不同的后AKI状态，发现3,491名患者发展为慢性肾病，建立了AKI到CKD进展的数据驱动风险评估模型。


<details>
  <summary>Details</summary>
Motivation: 急性肾损伤患者有发展为慢性肾病的高风险，但识别最高风险患者仍然具有挑战性，需要开发有效的方法来预测AKI到CKD的进展。

Method: 使用电子健康记录数据，通过纵向医疗代码和肌酐测量值聚类识别后AKI临床状态，采用多状态模型估计状态间转移概率和CKD进展风险，通过生存分析识别不同AKI亚群的CKD风险因素。

Result: 在20,699名入院时患有AKI的患者中，3,491名(17%)发展为CKD；识别出15种不同的后AKI状态，每种状态具有不同的CKD发展概率；75%的患者在研究期间保持单一状态或仅进行一次转换；确定了传统和新型CKD风险因素。
...
Result: 在传统Transformer模型中，RoBERTa与人类标注最一致；生成模型如GPT在零样本设置下与人类标注整体一致性最强；微调后的RoBERTa模型在所有基线中准确率最高。
...
TL;DR: 使用视觉语言模型作为评判者的多智能体系统能够通过将图表作为可验证检查点来改进自主科学发现，在宇宙学和天体化学案例中展示了从错误推理路径中恢复和适应新数据集的能力。


<details>
  <summary>Details</summary>
Motivation: 现有的自主科学发现系统在处理复杂数据分析和推理时容易出错，且缺乏实时纠错机制。本文旨在通过视觉语言模型作为评判者来提升系统的自我纠正能力和适应性。

Method: 将图表作为可验证检查点，使用视觉语言模型作为评判者，根据动态生成的领域特定评分标准评估图表，使智能体能够实时纠正错误并引导探索性数据分析。

Result: 在10个数据驱动发现任务的基准测试中，VLM增强系统达到0.7-0.8的通过率，显著优于仅代码（0.2-0.3）和代码加文本（0.4-0.5）的基线系统。

Conclusion: 视觉语言模型指导的多智能体系统能够有效提升自主科学发现的性能，提供可审计的推理轨迹，并在无需人工干预的情况下从错误中恢复和适应新数据集。
...
TL;DR: RareSeek R1是一个专门用于罕见病诊断的AI系统，通过领域专业语料库和临床验证推理集开发，在多中心电子健康记录和公共基准测试中达到最先进水平，性能与经验丰富的医生相当。
...
Result: 在多中心电子健康记录叙述和公共基准测试中，RareSeek R1达到最先进准确率，具有稳健的泛化能力，在噪声或重叠表型下保持稳定性。增强检索在叙述与优先变异配对时效果最佳。人类研究显示性能与经验丰富的医生相当。

Conclusion: 这项工作推进了以叙述为先、知识整合的推理范式，缩短诊断历程，并提供可审计、临床可转化的决策支持，透明推理凸显了支持正确诊断的决定性非表型证据（中位数23.1%）。
...
TL;DR: 提出了一个理解NLP数据集中注释偏见的综合框架，包括指令偏见、注释者偏见以及情境和文化偏见，并介绍了检测方法和缓解策略。


<details>
  <summary>Details</summary>
Motivation: NLP数据集中的注释偏见是开发多语言大语言模型的主要挑战，特别是在文化多样性的环境中。偏见会扭曲模型输出并加剧社会危害。

Method: 提出了一个包含指令偏见、注释者偏见、情境和文化偏见的分类法；综述了检测方法（如注释者间一致性、模型分歧、元数据分析）；提出了包括多样化注释者招募、迭代指南改进和事后模型调整在内的缓解策略。

Result: 开发了一个综合框架来理解和缓解注释偏见，包括偏见分类法、检测指标综合、适用于多语言环境的集成偏见缓解方法，以及对注释过程的伦理分析。
...
TL;DR: 提出了一个模块化框架，通过检索增强生成(RAG)流程来简化合同管理，在低资源条件下实现超过80%的问题修订识别和优化准确率。


<details>
  <summary>Details</summary>
Motivation: 合同管理过程中存在大量无结构的历史合同和标记数据稀缺的问题，自动化这一工作流程具有挑战性。

Method: 采用模块化框架，集成合成数据生成、语义条款检索、可接受性分类和基于奖励的对齐机制，识别问题修订并生成改进方案。

Result: 与行业合作伙伴共同开发和评估，系统在识别和优化问题修订方面均达到超过80%的准确率。

Conclusion: 该系统在真实世界的低资源条件下表现出色，为加速合同修订工作流程提供了实用解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 传统Heap定律假设类型-标记关系在log-log尺度下呈直线，但实际观测发现仍存在轻微凹形，这挑战了幂律关系的有效性。

Method: 使用20部英文小说或翻译作品，对log(类型)-log(标记)数据进行线性回归和二次回归分析，并采用随机抽球模型进行理论解释。

Result: 回归分析显示线性系数略大于1，二次系数约为-0.02；随机抽球模型表明log-log尺度的曲率等同于负的伪方差。

Conclusion: 二次函数比幂律函数能更好地描述类型-标记关系，伪方差形式化为小标记数情况下的曲率估计提供了理论框架。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法主要关注模型自我纠正，缺乏教育场景中"教师式"的系统性指导和修正学生解题过程的能力

Method: 将学生推理建模为多步骤序贯决策问题，引入蒙特卡洛树搜索探索最优纠正路径，利用广度优先搜索和最终答案评估生成奖励信号，通过反向传播机制实现细粒度过程监督

Result: 在两个公共数据集(ProcessBench和MR-GSM8K)以及自建的MSEB基准上，SMRC在效果和整体性能上显著优于现有方法

Conclusion: SMRC方法能有效纠正学生数学推理错误，在教育应用方面具有良好潜力
...
TL;DR: 该研究探讨了大型语言模型能否编码天体物理学中的物理统计信息，分析了提示工程和语言特征对物理信息编码的影响。


<details>
  <summary>Details</summary>
Motivation: 研究LLM是否能够从科学测量的文本描述中编码物理统计信息，探索LLM在科学领域的应用潜力。

Method: 使用稀疏自编码器从文本中提取可解释特征，分析提示工程和语言特征对物理信息编码的影响。

Result: 通过实验验证了LLM嵌入能够编码物理统计信息，并揭示了提示工程和特定语言特征在编码过程中的重要性。

Conclusion: LLM能够有效地编码科学测量中的物理信息，为科学领域的语言模型应用提供了新的可能性。
...
Method: 利用大语言模型生成历史法语（16-20世纪）和中文（1900-1950）文本的标注数据，并用这些数据微调spaCy模型。

Result: 在特定历史时期的测试中，词性标注、词形还原和命名实体识别任务都取得了显著提升。

Conclusion: 领域特定模型的重要性得到验证，即使是相对有限的合成数据也能改善计算人文学研究中低资源语料的NLP工具。
...
TL;DR: VALOR是一个用于多模态投诉分析的验证感知学习框架，通过专家路由和链式思维提示进行细粒度分类，在文本和图像信息融合的场景中优于基线模型。


<details>
  <summary>Details</summary>
Motivation: 现有投诉分析方法主要依赖单模态短文本，本研究利用多模态多轮客户支持对话，结合文本投诉和视觉证据进行细粒度分类，以支持联合国可持续发展目标。

Method: 采用多专家推理设置，使用大规模生成模型和链式思维提示进行决策，计算语义对齐分数并通过元融合策略整合到最终分类中。

Result: 在标注有细粒度方面和严重性标签的多模态投诉数据集上评估，VALOR始终优于基线模型，特别是在信息分布在文本和图像中的复杂投诉场景中。

Conclusion: 本研究强调了多模态交互和专家验证在实际投诉理解系统中的价值，支持SDG 9和SDG 12目标，促进更负责任的产品设计和消费者服务问责制。
...
TL;DR: 比较库尔德语词嵌入的三种分词策略（词级、基于语素、BPE），发现BPE在形态相似性上看似更优但仅评估28.6%的测试用例，而语素模型评估68.7%。综合评估显示基于语素的分词在嵌入空间组织、语义邻域结构和形态复杂度覆盖方面更优。


<details>
  <summary>Details</summary>
Motivation: 研究库尔德语词嵌入的分词策略，特别关注低资源语言处理中的评估偏差问题，旨在找到最适合保留形态相似性的分词方法。

Method: 开发基于BiLSTM-CRF的形态分割器，使用最小手动标注进行引导训练，比较词级、基于语素和BPE三种分词方法，通过相似性保持、聚类质量和语义组织等综合指标评估Word2Vec嵌入。

Result: BPE在形态相似性上看似更优但仅覆盖28.6%测试用例，而语素模型覆盖68.7%。综合评估显示基于语素的分词在嵌入空间组织、语义邻域结构和形态复杂度平衡覆盖方面表现最佳。

Conclusion: 基于语素的分词策略在库尔德语词嵌入中表现最优，研究强调了在低资源语言处理中进行覆盖感知评估的重要性，为低资源语言处理提供了不同的分词方法选择。
...
<details>
  <summary>Details</summary>
Motivation: 研究LLMs在研发过程中的多重功能，探索如何利用这些模型改善研究流程的效率和效果。

Method: 通过分析科学文献、专利数据库和实验数据，研究LLMs在研发工作流程中的应用。

Result: LLMs能够实现更灵活和知情的研发工作流程，加速创新周期，缩短突破性想法的上市时间。

Conclusion: LLMs在变革研发过程中发挥着重要作用，通过多种功能显著提升研究效率和创新速度。
...
TL;DR: 本文介绍了SpatiaLite基准测试，用于评估视觉语言模型的空间推理能力，发现当前先进VLM主要依赖语言表示，在视觉中心任务上表现不足且效率低下，并提出了Imagery Driven Framework来改进。


<details>
  <summary>Details</summary>
Motivation: 当前大型语言模型和视觉语言模型在逻辑推理、问题解决等方面表现出色，但空间推理（包括心理旋转、导航和空间关系理解）仍然是重大挑战。作者假设想象力是空间世界模型中的主导推理机制。

Method: 引入SpatiaLite合成基准测试，联合测量空间推理准确性和效率；通过全面实验分析VLM的空间推理机制；提出Imagery Driven Framework用于数据合成和训练。

Result: 发现三个关键结果：1) 先进VLM主要依赖语言表示，在需要感知空间关系和3D几何变换的视觉中心任务上表现不佳；2) VLM空间推理效率严重低下，token使用量随变换复杂度快速增长；3) IDF框架可以隐式构建内部世界模型。

Conclusion: SpatiaLite基准测试揭示了先进VLM的空间推理局限性和模式，识别了关键缺陷，并为未来改进提供了指导。IDF框架为构建内部世界模型提供了有效方法。
...
TL;DR: KANGURA提出了一种基于Kolmogorov-Arnold Network的3D几何感知学习方法，用于优化微生物燃料电池阳极结构，在ModelNet40数据集上达到92.7%准确率，在真实MFC问题上达到97%准确率。


<details>
  <summary>Details</summary>
Motivation: 现有预测模型难以捕捉复杂几何依赖关系以优化微生物燃料电池阳极结构，而阳极结构对MFC性能至关重要。

Method: KANGURA将预测建模为函数分解问题，使用KAN表示学习重构几何关系，通过几何解耦表示学习分离结构变化，并采用统一注意力机制增强关键几何区域。
...
TL;DR: 评估KOSMOS自主AI科学家在辐射生物学中的表现，发现其产生了一个明确发现(CDO1)、一个不确定结果(OGT)和一个错误假设(DDR-p53)，表明AI科学家能产生有用想法但需要严格验证。


<details>
  <summary>Details</summary>
Motivation: 评估自主AI科学家KOSMOS在真实科学问题中的表现，测试其生成假设和发现的能力。

Method: 使用KOSMOS AI科学家在三个辐射生物学问题上进行测试：1) DDR能力与p53转录反应的关系；2) OGT和CDO1表达与辐射响应模块的关系；3) 12基因特征预测前列腺癌放疗后生存。采用随机基因零基准进行验证。

Result: 假设1不支持(DDR与p53反应弱负相关，rho=-0.40,p=0.76)；OGT弱相关(r=0.23,p=0.34)；CDO1显著相关(r=0.70,p=0.0039)；12基因特征一致性指数0.61(p=0.017)但效应大小不唯一。

Conclusion: AI科学家能产生有用想法，但需要严格的零模型审计来验证假设的真实性，避免虚假发现。
...
TL;DR: 本文研究将Bjøru等人提出的反事实概率边界计算算法从马尔可夫模型扩展到半马尔可夫结构因果模型，以处理更复杂的混杂关系。


<details>
  <summary>Details</summary>
Motivation: Bjøru等人的方法仅适用于马尔可夫模型，其中外生变量仅影响单个内生变量。但在实际应用中，半马尔可夫模型能表示更复杂的混杂关系，因此需要扩展该方法。

Method: 通过最小示例说明扩展挑战，提出替代解决方案策略，并进行理论和计算评估。

Result: 识别了将分治算法扩展到半马尔可夫模型的挑战，并提出了可行的替代策略。

Conclusion: 成功将反事实概率边界计算方法扩展到半马尔可夫结构因果模型，为处理更复杂的因果推理问题提供了新途径。
...
<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型在智能交通系统中应用广泛，但存在严重的安全漏洞，易受越狱攻击，可能导致模型生成有害内容，威胁系统安全。

Method: 构建交通相关有害查询数据集；提出基于图像排版操纵和多轮提示的越狱攻击方法；开发多层响应过滤防御技术；使用GPT-4和人工验证评估攻击和防御效果。

Result: 实验表明提出的越狱攻击方法对开源和闭源LVLMs都有效，相比现有技术更具威胁；防御技术能有效阻止模型生成不当响应。

Conclusion: LVLMs在智能交通系统中存在严重安全风险，图像排版操纵和多轮提示的越狱攻击构成重大威胁，需要多层防御机制来保护系统安全。
...
Conclusion: CORGI算法通过消除填充完整冲突集导致的高延迟和内存溢出，使自动生成规则的系统在实际应用中更加可行。
...
Method: 使用GPT-4o分析OSHA叙述提取结构化危险推理，转换为对象级场景图，指导文本到图像扩散模型生成构图准确的危险场景。

Result: 提出的VQA图评分在四个最先进生成模型中优于CLIP和BLIP指标，基于熵验证确认其更高的判别敏感性。
...
TL;DR: 本文通过整合综述和实验验证，探讨了AI智能体在音乐分析和教育中的应用，从基于规则的模型发展到深度学习、多智能体架构和RAG框架，并通过双案例方法评估教学效果。


<details>
  <summary>Details</summary>
Motivation: 研究旨在探索AI智能体如何有效提升音乐模式识别、创作参数化和教育反馈，解决传统自动化方法在可解释性和适应性方面的不足。

Method: 采用双案例方法：(1)在中等教育中使用生成式AI平台培养分析和创造能力；(2)设计用于符号音乐分析的多智能体系统，实现模块化、可扩展和可解释的工作流程。

Result: 实验结果表明，AI智能体在音乐模式识别、创作参数化和教育反馈方面表现优异，在可解释性和适应性方面优于传统自动化方法。

Conclusion: 研究提出了一个统一框架，连接技术、教学和伦理考量，为计算音乐学和音乐教育中智能体的设计和应用提供基于证据的指导，同时强调在教育环境中负责任部署AI的重要性。
...
<details>
  <summary>Details</summary>
Motivation: 大规模伤亡事件中分类决策至关重要，但真实数据难以收集，需要高质量基准数据集来开发和评估AI系统。
...
TL;DR: AISAC是一个用于科学和工程工作流程的多智能体系统，基于LangGraph、FAISS和SQLite技术构建，具有透明性、溯源跟踪和科学适应性。


<details>
  <summary>Details</summary>
Motivation: 开发一个集成化的科学助手系统，支持科学工作流程的透明性、溯源跟踪和跨领域适应性。

Method: 采用Router-Planner-Coordinator工作流程和可选的Evaluator角色，使用提示工程智能体通过LangGraph协调，结合FAISS向量搜索和SQLite持久化的混合内存方法。

Result: 系统已在阿贡国家实验室的多个研究领域成功应用，包括废物转化研究和能源过程安全等专业部署，展示了其跨领域适用性。

Conclusion: AISAC为科学工作流程提供了一个透明、可追踪且适应性强的多智能体系统解决方案，具有实际应用价值。
...
TL;DR: 提出了一种由教师主导的自适应学习反馈系统，将概念级评估证据转化为经过验证的微干预，通过三种保障机制（充分性、注意力预算、多样性）实现精确诊断与有效干预的结合。
...
Method: 将干预分配形式化为带约束的二元整数规划，包含覆盖度、时间、难度窗口、概念矩阵先决条件和反冗余多样性约束。采用贪婪选择、梯度松弛和混合方法三种求解策略。

Result: 在1204名学生的物理课程部署中，两种求解器都实现了几乎所有学习者的完整技能覆盖，梯度方法比贪婪方法减少约12%的冗余覆盖，并协调了难度分布。

Conclusion: 开发了一个可追踪和可审计的控制器，能够闭合诊断-教学循环，在课堂规模上提供公平、负载感知的个性化学习。
...
TL;DR: 本文研究了LLM网络中的协作问答问题，分析了幻觉在LLM网络中的传播机制及其对真相估计的影响。


<details>
  <summary>Details</summary>
Motivation: 研究动机在于LLM在缺乏直接证据时容易产生幻觉，而在LLM网络中这种幻觉效应会传播，导致原本准确的LLM也开始产生幻觉。

Method: 结合网络科学的平均场动力学和经济学中的随机效用模型，构建了一个生成模型来分析LLM网络中的信息扩散。

Result: 为具有两种潜在状态的LLM网络提出了固定点存在性和唯一性的充分条件，并分析了激励对固定点行为的影响。

Conclusion: 通过实验研究了100个开源LLM网络在不同条件下的行为，包括数据异质性、节点能力、网络结构和框架敏感性。
...
TL;DR: APD-agents是一个基于大语言模型的多智能体框架，用于自动化移动应用页面设计，通过多个专业智能体的协作将用户描述转化为高质量页面布局。


<details>
  <summary>Details</summary>
Motivation: 移动应用页面设计耗时且需要专业技能，设计师需要反复调整控件大小、位置和样式，现有设计软件需要大量培训，跨页面协作设计需要额外时间确保一致性。

Method: 提出APD-agents多智能体框架，包含OrchestratorAgent、SemanticParserAgent、PrimaryLayoutAgent、TemplateRetrievalAgent和RecursiveComponentAgent，通过智能体协作将用户描述转化为结构化数据并生成从粗到细的页面布局。
...
TL;DR: 提出了PRISM框架，用于金融信息检索任务，包括文档排序和块排序。该框架结合了系统提示、上下文学习和轻量级多代理系统，在FinAgentBench数据集上取得了0.71818的NDCG@5分数。
...
Method: PRISM是一个无需训练的框架，集成了精炼的系统提示、上下文学习(ICL)和轻量级多代理系统。提示工程提供精确任务指令，ICL提供语义相关的少样本示例，多代理系统建模协调评分行为。

Result: 最佳配置在受限验证集上实现了0.71818的NDCG@5分数，证明PRISM在生产规模的金融检索中是可行且稳健的。
...
<details>
  <summary>Details</summary>
Motivation: 解决LLM在视觉语言导航中存在的空间理解不足、计算成本高和推理延迟大的问题，同时利用LLM的常识知识和推理能力。

Method: 提出R3框架：Runner（轻量级专家模型负责常规导航）、Ruminator（多模态LLM进行结构化推理）、Regulator（监控导航进度并控制思维模式）。

Result: 在REVERIE基准上SPL和RGSPL分别提升3.28%和3.30%，显著优于现有最先进方法。

Conclusion: R3框架有效解决了VLN任务中的挑战，通过双过程思维整合了LLM的泛化能力和领域专业知识。
...
TL;DR: 提出了CLEAR框架，针对企业AI代理评估中忽视成本效率、可靠性和运营稳定性的问题，通过多维度指标（成本、延迟、效能、保证、可靠性）进行更全面的评估。


<details>
  <summary>Details</summary>
Motivation: 现有AI代理基准主要评估任务完成准确性，但忽略了企业部署的关键需求，如成本效率、可靠性和运营稳定性。研究发现当前评估存在三个根本局限：缺乏成本控制、可靠性评估不足、缺少多维指标。

Method: 开发了CLEAR（成本、延迟、效能、保证、可靠性）评估框架，对6个领先代理在300个企业任务上进行评估，并进行了专家评估（N=15）。

Result: 仅优化准确性的代理比成本感知替代方案贵4.4-10.8倍，但性能相当。CLEAR框架比仅准确性评估更能预测生产成功（相关性ρ=0.83 vs ρ=0.41）。

Conclusion: CLEAR框架为企业AI代理部署提供了更全面的评估标准，强调了成本控制和多维度指标的重要性，有助于开发更实用和可持续的企业AI解决方案。
...
Result: 相比最先进的异构联邦学习方法，平均F1分数提升约13%；相比现有大语言模型联邦学习框架，在客户端增加时平均F1分数提升达5%，训练成本降低约87%。
...
TL;DR: LLMs在金融应用中存在时间顺序理解问题，测试显示即使对于预训练已知的事实，模型在复杂时间排序任务中表现不佳，但增加推理预算能显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 测试LLMs是否真正理解时间顺序，因为金融应用中的提示工程往往隐含假设模型理解时间顺序，这对避免前瞻性偏差至关重要。

Method: 设计三种时间顺序任务：时间排序、条件排序（先过滤再排序）和时代错误检测，在GPT-4.1、Claude-3.7 Sonnet和GPT-5上测试不同推理努力设置。

Result: 随着序列变长，精确匹配率急剧下降；条件排序失败主要来自过滤步骤；时代错误检测相对最容易但性能仍随复杂性下降；GPT-5在高推理努力下能达到完美排序。

Conclusion: 当前LLMs在时间顺序任务上存在局限，但分配明确推理预算能显著帮助，这对金融实时应用具有重要意义。
...
<details>
  <summary>Details</summary>
Motivation: Whisper模型在嘈杂声学条件下经常出现幻觉错误，而现有方法主要关注音频预处理或转录后处理，对模型本身的修改探索不足。

Method: 1. 自适应层注意力(ALA)：通过层间相关性分析将编码器层分组，使用可学习多头注意力融合块表示；2. 多目标知识蒸馏(KD)：在嘈杂音频上训练学生模型，使其语义和注意力分布与处理干净输入的教师模型对齐。

Result: 在嘈杂语音基准测试中显著减少了幻觉和词错误率，同时在干净语音上保持了性能。

Conclusion: ALA和KD提供了一种原则性策略来提高Whisper在真实世界嘈杂条件下的可靠性。
...
TL;DR: DevPiolt是一个基于大语言模型的物联网设备操作推荐系统，通过持续预训练、多任务微调、偏好优化和置信度控制机制，显著提升了操作推荐的准确性和用户体验。


<details>
  <summary>Details</summary>
Motivation: 现有推荐模型在处理物联网设备操作时面临复杂操作逻辑、多样化用户偏好和对次优建议敏感等挑战，限制了其实际应用效果。

Method: 1) 通过持续预训练和多任务微调为LLM注入物联网操作领域知识；2) 使用直接偏好优化对齐用户特定偏好；3) 设计基于置信度的曝光控制机制避免低质量推荐。

Result: 在多个数据集上显著超越基线模型，所有指标平均提升69.5%；实际部署在小米家庭应用中，服务25.5万用户，独立访客设备覆盖率提升21.6%，页面浏览接受率提升29.1%。

Conclusion: DevPiolt通过结合领域知识注入、偏好优化和置信度控制，有效解决了物联网设备操作推荐的关键挑战，在实际应用中取得了显著成效。
...
TL;DR: 提出基于LLM和时序模型的新型框架，用于预测区域级Airbnb市场趋势（收入、预订天数、预订数量），相比传统方法显著提升预测精度。


<details>
  <summary>Details</summary>
Motivation: Airbnb等短租平台的扩张严重扰乱了当地住房市场，导致租金上涨和住房负担问题。准确预测区域Airbnb市场趋势可为政策制定者和城市规划者提供关键洞察，以减轻这些影响。

Method: 使用滑动窗口方法预测1-3个月趋势，通过整合房源特征与外部环境因素（城市可达性、人口流动性）构建区域表示，将结构化表格数据转换为基于提示的LLM输入以生成区域嵌入，然后输入到时序模型（RNN、LSTM、Transformer）中捕捉复杂时空动态。
...
<details>
  <summary>Details</summary>
Motivation: 当前基于LLM的知识图谱推理方法存在两个关键限制：1）不加区分地提取推理路径，可能引入不相关噪声误导LLMs；2）动态探索推理路径需要高检索需求和频繁LLM调用。

Method: PathMind采用"检索-优先化-推理"三阶段方法：首先检索查询子图，然后通过语义感知路径优先级函数识别重要推理路径，最后通过双阶段训练策略（任务特定指令调优和路径偏好对齐）生成准确响应。

Result: 在基准数据集上的广泛实验表明，PathMind始终优于竞争基线，特别是在复杂推理任务上，通过识别关键推理路径，使用更少的输入token实现更好性能。

Conclusion: PathMind通过选择性路径引导机制有效解决了现有LLM-based KGR方法的局限性，实现了更忠实、可解释且高效的推理性能。
...
TL;DR: DataSage是一个多智能体框架，通过外部知识检索、多角色辩论机制和多路径推理来解决现有数据洞察代理的局限性，在InsightBench上表现优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有数据洞察代理存在三个主要问题：领域知识利用不足、分析深度不够、代码生成容易出错，导致无法提供满意的洞察发现结果。
...
Method: 系统性地改写和扰动一组知名CSPLib问题，保持结构不变但修改上下文并引入误导元素，比较三个代表性LLM在原始和修改描述下生成的模型。

Result: LLMs能生成语法有效且语义合理的模型，但在上下文和语言变化下性能急剧下降，显示出浅层理解和对措辞的敏感性。

Conclusion: LLMs在自动生成优化模型方面的成功可能更多源于数据污染而非真正的推理能力，对问题表述的变化表现出脆弱性。
...
TL;DR: 研究发现LLM对齐过程中考虑多元化价值观的重要性，通过分析不同人口群体的偏好差异和技术设计参数对模型行为的影响。


<details>
  <summary>Details</summary>
Motivation: 当前LLM对齐决策往往忽视人类社会的多样性，需要研究如何将多元化价值观纳入对齐流程以确保公平代表性。

Method: 收集美国德国1095名参与者的27375个评分，从毒性、情感意识、敏感性、刻板偏见和帮助性五个维度评估LLM响应，并基于不同社会群体偏好微调模型，同时测试评分尺度、分歧处理方法和优化技术。

Result: 发现系统性人口效应：男性评分毒性低18%，保守派和黑人评分情感意识高27.9%和44%；技术选择影响显著：保留评分者分歧比多数投票毒性减少53%，5点量表比二元格式减少22%，DPO在多值优化中优于GRPO。

Conclusion: 这是回答对齐如何平衡专家驱动和用户驱动信号以确保安全性和公平代表性的初步步骤，强调需要考虑多元化价值观。
...
TL;DR: 提出了一个神经符号框架，将感知不确定性显式建模并传播到规划中，通过Transformer感知前端和GNN关系推理提取概率符号状态，结合不确定性感知符号规划器在置信度低时主动收集信息。
...
Method: 耦合基于Transformer的感知前端和GNN关系推理来从视觉观察中提取概率符号状态，使用不确定性感知符号规划器在置信度低时主动收集信息。

Result: 在桌面机器人操作任务中，翻译器处理10,047个PyBullet生成场景，输出校准置信度的概率谓词（F1=0.68）；嵌入规划器后在Simple Stack、Deep Stack和Clear+Stack基准测试中分别达到94%/90%/88%成功率，平均90.7%，比最强POMDP基线高10-14个百分点，规划时间在15ms内。

Conclusion: 该框架是通用的，可应用于任何需要从感知输入到符号规划进行不确定性感知推理的领域，概率图模型分析建立了校准不确定性与规划收敛之间的定量联系，提供了经验验证的理论保证。
...
TL;DR: 提出基于率失真理论和最优传输几何的知识图谱构建与优化框架，通过FGW耦合量化语义失真，使用精炼操作最小化率失真拉格朗日函数，生成紧凑且信息保留的知识图谱，用于自动生成高质量选择题。


<details>
  <summary>Details</summary>
Motivation: 解决将非结构化教育材料（如讲义和幻灯片）转换为捕捉关键教学内容的知识图谱的困难，为AI驱动的学习助手系统提供自动生成高质量选择题的基础。

Method: 将讲义内容建模为度量-测度空间，使用Fused Gromov-Wasserstein耦合对齐候选知识图谱以量化语义失真，通过添加、合并、拆分、移除、重连等精炼操作最小化率失真拉格朗日函数。

Result: 在数据科学讲义上的原型应用显示，从优化后的知识图谱生成的选择题在15个质量标准上持续优于从原始笔记生成的选择题，并产生可解释的率失真曲线。

Conclusion: 本研究为个性化教育和AI辅助教育中的信息论知识图谱优化建立了理论基础，证明基于率失真理论的知识图谱精炼能有效提升自动生成问题的质量。
...
Method: 构建基于历史代理轨迹的有向图，节点代表工具，边捕获转移概率，建模工具选择的惯性，并整合参数级信息来优化工具输入生成。
...
TL;DR: SkillGen是一个基于技能的ICL框架，通过构建动作中心图、识别高效用动作和检索步骤技能来生成细粒度提示，在多个基准测试中显著提升LLM的序列决策性能。


<details>
  <summary>Details</summary>
Motivation: 现有ICL方法难以同时满足三个关键原则：关注决策关键信息、提供步骤级粒度、最小化专家标注依赖。LLM在序列决策中的效果对提示质量高度敏感。
...
Conclusion: 关注高效用片段支持任务可识别性，并为更有效的ICL提示设计提供信息，SkillGen框架在序列决策任务中实现了持续的性能提升。
...
TL;DR: 本文提出了一种异构智能体强化学习框架HAPPO，用于协调多个微电网的配电网恢复，解决了传统优化方法在非线性约束下的计算效率问题。


<details>
  <summary>Details</summary>
Motivation: 大规模停电后配电网恢复需要顺序开关操作，在功率平衡、电压限制和热额定值等非线性约束下协调分布式能源资源，传统优化方法和基于价值的强化学习方法计算效率低且难以扩展。

Method: 采用异构智能体近端策略优化(HAPPO)框架，每个智能体控制具有不同负载、DER容量和开关数量的微电网，使用分散的演员策略和集中的评论家进行训练，通过物理信息化的OpenDSS环境提供完整潮流反馈。

Result: 在IEEE 123总线和IEEE 8500节点系统上的实验表明，HAPPO相比DQN、PPO、MAES等方法具有更快的收敛速度、更高的恢复功率和更平滑的多种子训练效果。

Conclusion: 在HARL框架中纳入微电网级异构性，为复杂配电网恢复提供了可扩展、稳定且约束感知的解决方案。
...
### [215] [SQL-to-Text Generation with Weighted-AST Few-Shot Prompting](https://arxiv.org/abs/2511.13907)
*Sriom Chakrabarti,Chuangtao Ma,Arijit Khan,Sebastian Link*

Main category: cs.DB

TL;DR: 提出了一种基于加权AST检索的SQL到文本生成方法，通过结构感知提示确保生成的描述既流畅又忠实于原始查询逻辑，在多个基准数据集上显著优于现有基线。


<details>
  <summary>Details</summary>
Motivation: 解决现有SQL到文本生成方法在保持SQL查询确切语义方面的不足，特别是当存在多种正确表述时，确保非技术用户能够准确理解复杂的数据库操作。

Method: 提出加权AST检索与提示的架构，集成结构化查询表示和LLM提示，使用基于抽象语法树(AST)的相似性度量检索语义相关示例作为少样本提示。

Result: 在Spider、SParC和CoSQL三个基准数据集上的实验表明，该方法在执行准确率(EX)上比当前基线提升高达+17.24%，在精确匹配(EM)方面表现更优，并在人类评估中提供更一致的语义保真度，同时保持竞争力的运行时性能。

Conclusion: 加权AST提示是一种可扩展且有效的方法，能够从结构化数据库查询中推导出自然语言解释。
...
### [216] [Fast Verification of Strong Database Isolation (Extended Version)](https://arxiv.org/abs/2511.14067)
*Zhiheng Cai,Si Liu,Hengfeng Wei,Yuxing Chen,Anqun Pan*

Main category: cs.DB

TL;DR: VeriStrong是一个用于验证数据库强隔离保证的快速验证器，使用超多图形式化方法高效检测串行化和快照隔离违规。


<details>
  <summary>Details</summary>
Motivation: 验证数据库是否遵守其声称的强隔离保证（如串行化和快照隔离）至关重要，但在黑盒设置中具有挑战性，因为只能观察系统行为且存在不确定的事务依赖关系。

Method: 提出了超多图形式化方法，紧凑地捕捉数据库执行中的确定和不确定事务依赖关系，并开发了验证串行化和快照隔离的完备编码方法，针对数据库工作负载特性定制SMT求解。

Result: 在多样化基准测试中，VeriStrong不仅显著优于现有最先进验证器，还能扩展到大型通用工作负载，同时保持高精度的隔离异常检测。

Conclusion: VeriStrong通过创新的超多图形式化和针对性的SMT求解，实现了对数据库强隔离保证的高效、可扩展验证。
...
### [217] [Chipmink: Efficient Delta Identification for Massive Object Graph](https://arxiv.org/abs/2511.14162)
*Supawit Chockchowwat,Sumay Thakurdesai,Zhaoheng Li,Matthew Krafczyk,Yongjoo Park*

Main category: cs.DB

TL;DR: Chipmink是一个基于图的对象存储系统，通过动态分区对象到pod中来优化数据科学工作流中的对象持久化，相比传统完整快照方法显著减少存储空间和提升持久化速度。


<details>
  <summary>Details</summary>
Motivation: 现有对象持久化机制（如Pickle、Dill）依赖完整快照，会冗余存储未更改对象，导致时间和存储效率低下。数据科学系统缺乏集中式缓冲区管理器来跟踪脏对象，且对象状态分布在内存、共享内存、GPU和远程机器等不同位置，使脏对象识别更加困难。

Method: 提出基于图的对象存储系统Chipmink，通过将对象动态分区到适当子组（称为pod）来最小化预期持久化成本。这些pod有效隔离脏对象，实现高效的部分持久化。

Result: 实验表明Chipmink支持依赖共享内存、GPU和远程对象的库，在真实笔记本和脚本中相比最佳基线实现存储空间减少36.5倍，持久化速度提升12.4倍。

Conclusion: Chipmink通过动态对象分区和脏对象隔离，为数据科学系统提供了高效的持久化解决方案，显著优于传统快照方法。
...
### [218] [Gradient-Based Join Ordering](https://arxiv.org/abs/2511.14482)
*Tim Schwabe,Maribel Acosta*

Main category: cs.DB

TL;DR: 本文提出了一种基于梯度的连接排序方法，通过连续松弛技术将离散的连接顺序问题转化为可微优化问题，使用Gumbel-Softmax参数化和图神经网络成本模型，实现了线性时间复杂度的连接顺序优化。


<details>
  <summary>Details</summary>
Motivation: 传统连接排序方法作为NP难问题，存在计算复杂度高和可扩展性有限的问题，需要更高效的优化方法。

Method: 将查询计划连续松弛为软邻接矩阵表示计划叠加，结合Gumbel-Softmax参数化和可微约束，使用图神经网络作为成本模型进行基于梯度的搜索。

Result: 在两个图数据集上，该方法能找到与传统离散局部搜索方法相当甚至更低成本的计划，且运行时间随查询大小呈线性增长。
...
### [219] [Overview and Prospects of Using Integer Surrogate Keys for Data Warehouse Performance Optimization](https://arxiv.org/abs/2511.14502)
*Sviatoslav Stumpf,Vladislav Povyshev*

Main category: cs.DB

TL;DR: 使用整数型日期时间标签替代标准DATE和TIMESTAMP类型，可减少30-60%存储需求并提升25-40%查询性能，吞吐量最高提升8倍。


<details>
  <summary>Details</summary>
Motivation: 优化数据仓库和时间序列性能，解决标准日期时间类型存储效率低和查询性能不足的问题。

Method: 提出32位和64位整数格式替代标准日期时间类型，开发索引、聚合、压缩和批处理算法。

Result: 在金融、电信、物联网和科学研究等实际应用中验证，存储减少30-60%，查询速度提升25-40%，吞吐量最高提升8倍。

Conclusion: 整数型日期时间标签是高效实用的数据仓库和时间序列优化方案，具有广泛适用性和显著性能优势。
...
### [220] [Scalable Enforcement of Fine Grained Access Control Policies in Relational Database Management Systems](https://arxiv.org/abs/2511.14629)
*Anadi Shakya,Primal Pappachan,David Maier,Roberto Yus,Sharad Mehrotra,Johann-Christoph Freytag*

Main category: cs.DB

TL;DR: Sieve是一个关系数据库中间件，通过查询重写和缓存机制优化细粒度访问控制策略执行，在200-1200条策略下性能提升2-10倍。


<details>
  <summary>Details</summary>
Motivation: 智能技术普及和隐私法规（如GDPR、CPRA）要求数据库管理系统支持细粒度访问控制，但现有方法在策略数量大时性能严重下降。

Method: 结合查询重写（使用保护表达式分组和过滤策略）和缓存机制（含替换策略和刷新机制），有效利用数据库索引。

Result: 在两个DBMS上的实验显示，Sieve可扩展到大数据集和策略库，保持低查询延迟和系统负载，在200-1200条策略下性能提升2-10倍，缓存机制在动态负载下进一步提升性能6-22%。
...
### [221] [Natural Language Interfaces for Databases: What Do Users Think?](https://arxiv.org/abs/2511.14718)
*Panos Ipeirotis,Haotian Zheng*

Main category: cs.DB

TL;DR: NLIDBs通过自然语言查询数据库，但可用性挑战未被充分研究。研究发现SQL-LLM相比传统SQL平台显著提升查询效率、准确性和用户体验。


<details>
  <summary>Details</summary>
Motivation: 探索自然语言数据库接口(NLIDBs)在实际使用中的可用性挑战，包括用户挫折感、查询优化策略和错误恢复等问题，这些在现有研究中关注不足。

Method: 采用混合方法用户研究，比较最先进的NL2SQL系统SQL-LLM与传统SQL分析平台Snowflake。20名参与者完成12个现实数据库查询任务。

Result: SQL-LLM显著减少查询完成时间10-30%(平均418秒vs 629秒，p=0.036)，准确率从50%提升至75%(p=0.002)。用户查询重构更少，错误恢复快30-40秒，挫折感更低。

Conclusion: 设计良好的用户友好型NLIDBs在商业分析环境中具有实际重要性，强调在实际部署中可用性与技术准确性同等关键。
...
### [222] [Cloud-Native Vector Search: A Comprehensive Performance Analysis](https://arxiv.org/abs/2511.14748)
*Zhaoheng Li,Wei Ding,Silu Huang,Zikang Wang,Yuanjin Lin,Ke Wu,Yongjoo Park,Jianjun Chen*

Main category: cs.DB

TL;DR: 本文系统研究了云原生向量搜索，发现尽管云服务商默认使用聚类索引，但图索引在高并发、高召回率、高维数据或大数据类型场景中表现更优，且云上搜索需要不同的参数配置，缓存机制也会影响索引优化效果。


<details>
  <summary>Details</summary>
Motivation: 随着数据和任务复杂度的增长，向量索引被部署到远程存储（云原生向量搜索），云服务商默认使用聚类索引，但需要系统研究不同索引类型在云环境下的表现和优化策略。

Method: 分析聚类索引和图索引在远程存储上的瓶颈，比较不同工作负载下的性能，研究云上搜索的索引构建和搜索参数配置，并考察缓存机制对索引优化的影响。

Result: 图索引在高并发、高召回率、高维数据或大数据类型场景中表现优于聚类索引；云上搜索需要与磁盘搜索不同的参数配置；某些索引优化会与缓存机制产生冲突。

Conclusion: 云原生向量搜索需要根据具体工作负载选择合适的索引类型，调整搜索参数，并协调索引优化与缓存机制以获得最佳性能。
