...
Method: 使用随机对比采样构建训练对，应用直接偏好优化(DPO)方法，无需额外训练数据、奖励模型或人工偏好标注。
...
<details>
  <summary>Details</summary>
Motivation: 由于自闭症谱系障碍诊断过程复杂且资源密集，现有计算机辅助诊断方法在凝视特征性能不佳和缺乏现实世界泛化能力方面存在挑战。

Method: 使用包含168名ASC参与者和157名非自闭症参与者的标准化视频数据集，进行多模态分析，并引入新的统计描述符来量化眼动角度变异性，采用后期融合方法整合多个行为标记。

Result: 新提出的凝视描述符将基于凝视的分类准确率从64%提升到69%，多模态融合后总体分类准确率达到74%，计算发现与临床研究中关于ASC凝视回避的研究结果一致。

Conclusion: 研究结果表明基于视频的多模态行为分析具有开发可扩展的自闭症筛查工具的潜力，能够支持自闭症评估。
...
<details>
  <summary>Details</summary>
Motivation: VLA模型在长序列推理时面临注意力计算二次成本和KV内存无限增长的问题，现有方法在扩展主干架构时往往忽略了推理效率对实时部署的重要性。

Method: 将KV缓存划分为固定大小的块，使用循环门控模块根据学习到的效用分数来总结和过滤历史上下文，保留近期细粒度细节，同时积极修剪过时、低相关性的内存。

Result: 理论上可实现1.21倍推理加速和36% KV内存减少，对任务成功率影响最小，能够无缝集成到现有的自回归和混合VLA堆栈中。

Conclusion: 该方法提供了一种可扩展的推理解决方案，无需修改训练流程或下游控制逻辑，解决了VLA模型在实时部署中的效率瓶颈。
...
<details>
  <summary>Details</summary>
Motivation: 大规模视觉语言模型生成的放射学报告存在事实错误和幻觉问题，阻碍了临床应用，需要可靠的事实核查方法。

Method: 通过扰动真实报告生成合成数据集，训练多标签跨模态对比回归网络来检测报告中的错误发现及其位置。

Result: 在多个X光数据集上验证了方法的鲁棒性，对SOTA报告生成器的错误检测与基于真实标签的验证达到了0.997的相关系数。

Conclusion: 该方法在放射学工作流程中具有临床推理的实用性，能有效检测自动生成报告中的事实错误。
...
<details>
  <summary>Details</summary>
Motivation: 现有MLLM模型难以捕捉视网膜疾病诊断所需的关键空间细节特征，需要改进多模态融合方法来提升分类性能。

Method: 集成U-Net编码器的多层级跳跃特征到LLaMA 3.2 11B MLLM的交叉注意力模块中，采用补丁投影、缩放交叉注意力和FiLM调制进行特征融合。

Result: 在1305个眼底图像-文本对数据集上，MDF-MLLM达到94%准确率，相比基线提升56%；召回率和F1分数分别提升67%和35%。

Conclusion: MDF-MLLM为眼底图像分类提供了可泛化、可解释的模块化框架，在空间推理和分类性能上显著优于传统MLLM，具有临床决策支持应用潜力。
...
Method: 使用大语言模型将不安全提示解耦为伪安全提示和有害提示，将有害提示重写为自然对抗提示，通过视觉语言模型生成图像描述来迭代优化生成内容。
...
<details>
  <summary>Details</summary>
Motivation: 解决当前显著目标检测中存在的预测不完整和边界不准确问题，这些问题源于目标内部复杂性和卷积池化操作中的步长影响。

Method: 采用多任务学习方法，将显著目标检测、前景轮廓检测和边缘检测任务结合，通过相互学习模块(MLM)让多个网络分支相互学习，提升性能。

Result: 在七个挑战性数据集上的实验表明，该方法在显著目标检测和边缘检测任务上都达到了最先进的性能。

Conclusion: 通过多任务监督和相互学习机制，能够有效提升显著目标检测的完整性和边界精度，同时在边缘检测任务上也取得了优异表现。
...
<details>
  <summary>Details</summary>
Motivation: 现有的多模态相关性评估指标主要适用于双模态数据分析，限制了多模态相似性评估的能力。

Method: 利用多模态联合表示将多个模态整合到同一潜在空间中，实现不同模态在同一尺度上的准确表示。
...
<details>
  <summary>Details</summary>
Motivation: 当前脚手架安全检查主要依赖人工视觉检查，耗时且容易出错，可能导致不安全状况。需要利用AI和数字化技术提高检查准确性。

Method: 开发云基AI平台，处理脚手架结构的点云数据，通过比较认证参考数据与最新点云数据来检测结构修改。

Result: 提出的系统能够检测脚手架结构修改，实现自动化监测。

Conclusion: AI和数字化方法可以增强脚手架检查的准确性，减少人工检查所需的时间和精力，同时提高施工现场的安全性。
...
<details>
  <summary>Details</summary>
Motivation: 解决文本到图像生成中细粒度可控性的挑战，特别是反事实可控性（生成违背常识模式的图像），这对于创造力和探索性应用至关重要。

Method: 开发了三组件框架：图像评估器指导数据集构建，监督提示重写器生成修订提示，DPO训练的排序器选择最佳修订提示。扩展了Grounded SAM以改进图像评估器。

Result: 图像评估器性能比其骨干网络提升了114%，实验表明该方法优于现有最先进基线和ChatGPT-4o。
...
<details>
  <summary>Details</summary>
Motivation: 解决光学显微镜横向分辨率限制(~200nm)的问题，避免使用昂贵的专业设备或荧光超分辨率技术，为普通显微镜用户提供经济可行的超分辨率解决方案。

Method: 使用两种自研的深度神经网络架构(O-Net和Theta-Net)，在Zernike相衬和微分干涉对比显微镜下，对原子力显微镜校准的纳米级测试目标进行超分辨率重建。

Result: 两种模型在超分辨率重建中表现良好但具有互补性：高信噪比图像适合O-Net模型，低信噪比图像更适合Theta-Net模型。

Conclusion: 模型架构和源图像信噪比对深度神经网络在非荧光光学纳米显微镜中的性能至关重要，即使在相同训练数据和训练周期下也会产生显著差异。
...
<details>
  <summary>Details</summary>
Motivation: 现有稀疏自编码器虽然能减少神经元的多义性，但单个概念表示仍分布在多个潜在特征中，需要大量搜索过程来进行概念遗忘。

Method: 引入SAEmnesia监督稀疏自编码器训练方法，通过系统化概念标注缓解特征分裂并促进特征集中化，学习具有更强概念关联的专用神经元。

Result: 在UnlearnCanvas基准测试中比现有最优方法提升9.22%；在推理时超参数搜索减少96.67%；在9对象移除的顺序遗忘任务中，遗忘准确率提升28.4%。

Conclusion: SAEmnesia通过促进概念与神经元的精确映射，显著提高了概念遗忘的效率和效果，为文本到图像扩散模型的概念控制提供了更可解释和可扩展的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 深度学习模型在生物医学图像分类中需要大量计算资源和时间，特别是在超参数搜索时。随机采样方法无法有效代表原始数据集，特别是在不平衡数据集上存在偏向主导类的问题。

Method: 提出智能核心集选择机制，通过提取类内多样性形成每类聚类，然后基于这些聚类进行最终采样。

Result: 在知名生物医学成像数据集上的分类实验表明，该方法在多个性能指标上优于随机采样方法。

Conclusion: 所提出的智能核心集选择方法能够有效减少训练计算需求，同时保持模型性能，优于传统的随机采样方法。
...
<details>
  <summary>Details</summary>
Motivation: 当前大多数深度学习模型仅使用单个或有限数量的先前乳腺X光片，缺乏对真实世界环境中不平衡结果分布和异质性随访的适应性。

Method: 开发LongiMam模型，结合卷积神经网络和循环神经网络，整合当前和最多四个先前的乳腺X光片，在大规模人群筛查数据集上进行训练和评估。

Result: 在所有测试场景中，包含先前乳腺X光片的LongiMam模型持续改善预测性能，结合历史和近期信息的模型优于单次访问模型，在乳腺密度随时间变化的女性中表现最佳。

Conclusion: 纵向建模增强了乳腺癌预测能力，支持在筛查项目中使用重复乳腺X光片来完善风险分层，LongiMam已作为开源软件公开发布。
...
<details>
  <summary>Details</summary>
Motivation: 探索CNNs与人类大脑在更复杂的社交认知任务中的对应关系，而不仅仅是基本的视觉感知。

Method: 通过相关性分析评估CNN架构与人类行为和fMRI数据的对齐程度，并提出了Object2Brain框架，结合GradCAM和对象检测来研究不同对象类别对CNN-人类相关性的影响。

Result: CNNs在图像情感评估任务中难以超越简单视觉处理，不同CNN架构显示出不同的对象类别敏感性。

Conclusion: CNNs在社交认知等高阶大脑处理任务中的对应关系有限，需要更专门的方法来研究这种复杂认知过程。
...
<details>
  <summary>Details</summary>
Motivation: 传统概念瓶颈模型虽然允许专家干预错误概念，但无法解决模型从有偏数据中学习捷径的系统性偏差问题。
...
Conclusion: 该框架有效解决了概念瓶颈模型中的系统性偏差问题，通过可解释的调试过程提升了模型的可靠性和与专家推理的一致性。
...
<details>
  <summary>Details</summary>
Motivation: 沉船是海事历史的重要标记，但通过人工检查测深数据发现沉船耗时且需要专家分析，需要自动化工具提高效率。

Method: 开发QGIS插件，使用深度学习模型处理测深数据，通过合成数据增强训练集，支持像素级分割掩码或边界框输出。

Result: 与基于深度学习的ArcGIS工具包和传统反沉洞检测方法相比，该工具展现出更优的分割性能。

Conclusion: ShipwreckFinder提供了一个有效的开源解决方案，能够自动检测沉船，显著提高沉船发现效率。
...
<details>
  <summary>Details</summary>
Motivation: 先前研究表明神经网络可以在保持性能的同时大幅剪枝，但剪枝对模型可解释性的影响尚不清楚。本研究旨在探究剪枝如何影响从低层显著性图到高层概念表示的可解释性。

Method: 使用在ImageNette上训练的ResNet-18，比较不同剪枝水平下Vanilla Gradients和Integrated Gradients的事后解释，评估稀疏性和忠实度。应用基于CRAFT的概念提取来跟踪学习概念语义一致性的变化。

Result: 轻度到中度剪枝改善了显著性图的聚焦性和忠实度，同时保留了清晰、语义有意义的概念。而激进剪枝合并了异质特征，尽管保持了准确性，但降低了显著性图稀疏性和概念一致性。
...
Result: 在三个具有挑战性的场景理解基准测试中达到最先进性能：(i)使用真实点云，(ii)使用有姿态图像，(iii)使用无姿态图像。在3D物体检测方面与专门方法表现相当，在布局估计方面显著提升。

Conclusion: TUN3D为整体室内场景理解设立了新的基准，实现了紧凑且语义丰富的空间表示。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法往往忽视图像不同区域的重要性差异，可能损害视觉关键内容的重建质量，需要解决语义粒度不足和AI模型存储计算需求大的问题。

Method: 图像分割为关键和非关键区域，关键区域使用图像语义编码器，非关键区域采用图像到文本建模压缩，结合模型量化和低秩适应微调等轻量级部署技术。

Result: 仿真结果表明，所提系统在语义保真度和视觉质量方面均优于传统方法，有效提升了图像传输任务的性能。

Conclusion: 该系统通过精细化语义粒度和轻量级部署策略，在保持性能的同时显著提升了资源利用率，为图像语义通信提供了有效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 为支持毫米波ISAC系统中的人体感知研究提供标准化的开源数据集，促进信号处理和深度学习算法的发展。

Method: 构建包含多种人体感知任务的毫米波数据集，描述测试平台、实验设置和信号特征，并采用参数高效微调方法适配不同任务。

Result: 成功创建了可用于多种下游任务的毫米波ISAC数据集，验证了参数高效微调在保持性能的同时显著降低计算复杂度。

Conclusion: mmHSense数据集为毫米波ISAC研究提供了有价值的资源，参数高效微调方法展示了在任务适配中的有效性。
...
<details>
  <summary>Details</summary>
Motivation: 传统中轴骨架对噪声敏感，微小边界变化会导致骨架过度扩展。经典修剪方法虽然能缓解这一问题，但缺乏系统的尺度空间理论支撑。

Method: 结合骨架稀疏化和尺度空间理论，构建骨架化尺度空间，实现层次简化。提供连续和离散的理论基础，并引入逆过程的稠密化概念。

Result: 框架满足层次架构、可控简化和几何变换等变性等尺度空间性质，在鲁棒骨架化、形状压缩和增材制造刚度增强等任务中表现有效。

Conclusion: 骨架化尺度空间为形状分析提供了理论严谨的层次简化框架，既能简化又能逆向恢复细节，具有实际应用价值。
...
<details>
  <summary>Details</summary>
Motivation: 现有气候预测空间分辨率较低（如12.5公里），限制了其在地方决策中的实用性，需要高分辨率气候预测来支持本地决策。
...
Conclusion: 单图像超分辨率模型是提升气候预测空间分辨率的有效方法，能够为地方决策提供更精细的气候信息。
...
Method: JaiLIP方法在图像空间中最小化清洁图像与对抗图像之间的MSE损失和模型有害输出损失的联合目标函数。

Result: 实验表明该方法生成的对抗图像高效且不易察觉，在毒性指标上优于现有方法，并在交通领域验证了攻击的实用性。
...
<details>
  <summary>Details</summary>
Motivation: 量化自动植物识别系统与人类专家之间的性能差距，比较两者在物种识别任务中的不确定性，为计算机科学家和自然学家提供有价值的参考。

Method: 设计了ExpertCLEF挑战赛，让4个研究团队的19个深度学习系统与9位植物学专家进行对比评估，使用相同的视觉观察数据进行物种识别。

Result: 最先进的深度学习模型在植物识别任务上的性能已经接近最先进的人类专家水平。

Conclusion: 深度学习在植物识别领域取得了显著进展，其性能已接近人类专家，这为自动化生物识别系统的发展提供了有力支持。
...
<details>
  <summary>Details</summary>
Motivation: 现有生成方法首先生成三角网格再转换为四边形，这通常会产生拓扑质量较差的四边形网格，需要开发原生四边形网格生成方法。

Method: 提出QuadGPT框架，采用序列预测范式，包含统一标记化方法来处理三角形和四边形的混合拓扑，以及专门的强化学习微调方法tDPO来提升生成质量。

Result: 大量实验表明，QuadGPT在几何精度和拓扑质量上都显著超越了之前的三角到四边形转换流程。

Conclusion: 该工作为原生四边形网格生成设立了新基准，展示了将大规模自回归模型与拓扑感知的强化学习优化相结合在创建结构化3D资产方面的强大能力。
...
Result: 在ErasureBench-H和标准数据集上的实验表明，DyME持续优于最先进基线，实现了更高的多概念擦除保真度，同时最小化了对非目标内容的附带损害。

Conclusion: DyME通过模块化设计和正交约束，有效解决了多概念擦除中的冲突问题，为实际应用提供了可扩展的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有评估指标（如BLEU、ROUGE）无法捕捉人类判断的精细度，而人工评估成本高昂，需要开发专门针对视频理解任务的自动评估方法。

Method: 通过生成器和评估器的交互训练：生成器根据目标评分生成响应，不匹配评估器评分的响应被丢弃，从而训练专门的MLLM评估器。

Result: VideoJudge-7B在四个元评估基准中的三个上超越了更大的MLLM基线模型（如Qwen2.5-VL 32B和72B），且发现视频输入对评估视频理解任务至关重要。

Conclusion: 专门设计的MLLM评估器在视频理解任务评估中优于通用LLM，视频输入是有效评估的关键因素，长链思维推理并不能提升性能。
...
<details>
  <summary>Details</summary>
Motivation: 多智能体协同感知通过共享信息提升场景理解能力，但通信带宽限制了系统的可扩展性，需要高效的压缩方法来减少数据传输量。

Method: 采用端到端方法，先通过瓶颈网络压缩特征维度，然后使用多阶段残差向量量化，仅传输像素级代码索引而非完整特征。

Result: 在DAIR-V2X数据集上实现273倍到1365倍的压缩率，在18 bpp时性能匹配或优于原始特征协同感知，在6-12 bpp时仍能保持可接受的性能。
...
Method: 使用中性提示词为56种沙特职业生成1,006张图像，由两名沙特标注员从五个维度评估：感知性别、服装外貌、背景环境、活动互动和年龄，第三位资深研究员在分歧时裁决，共获得10,100个判断。

Result: 所有三个模型都显示出强烈的性别不平衡，领导和技术角色中性别偏见最明显。文化不准确性在服装、环境和活动方面频繁出现。反刻板印象图像往往源于文化误解而非真正进步性描绘。

Conclusion: 当前模型反映了训练数据中嵌入的社会偏见，仅有限地反映了沙特劳动力市场的性别动态和文化细微差别。需要更多样化的训练数据、更公平的算法和文化敏感的评估框架来确保公平真实的视觉输出。
...
Method: 引入三个预训练任务：Caption（增强对视频细节的感知）、VQA（加深对问题定义和标注指南的理解）、CoT（增强推理能力）。

Result: 实验结果显示该预训练方法显著提升了MLLM在零样本和SFT设置下的性能，且对未见过的紧急问题表现出强泛化能力。

Conclusion: 该预训练范式有效解决了短视频不当内容检测中的分布差距和复杂问题定义挑战，实现了统一检测和强泛化能力。
...
<details>
  <summary>Details</summary>
Motivation: 现有的GUI定位方法将任务视为坐标预测问题，但VLMs在处理高分辨率、复杂布局的GUI图像时难以准确预测数值坐标。

Method: 提出GUI-Cursor模型，将GUI定位重构为交互式搜索任务，模型在每一步确定目标对象、评估光标与目标的空间关系，并根据移动历史将光标移近目标。使用多步在线强化学习和基于轨迹的密集奖励函数进行训练。

Result: 基于Qwen2.5-VL-7B的GUI-Cursor在ScreenSpot-v2上从88.8%提升到93.9%，在ScreenSpot-Pro上从26.8%提升到56.5%，达到最先进水平。95%的情况下能在两步内解决问题，对更难的示例能自适应进行更多步骤。

Conclusion: 交互式搜索方法比直接坐标预测更有效，光标提供的视觉反馈有助于模型将预测与屏幕位置对齐，多步强化学习训练使模型能够高效解决GUI定位问题。
...
<details>
  <summary>Details</summary>
Motivation: 传统文本-视频检索系统存在两个问题：低质量数据对会损害检索效果但难以识别；余弦相似度无法解释排名结果，限制了可解释性。

Method: 基于LLM思维链推理构建可解释检索框架，首先扩展现有基准数据集以支持语义理解和减少数据偏差，然后设计包含成对比较步骤的检索思维链，生成详细推理和完整排名。

Result: X-CoT在实证中提高了检索性能，产生详细推理依据，并促进模型行为和数据质量分析。

Conclusion: X-CoT框架成功解决了传统检索系统的可解释性问题，同时提升了检索效果，为模型评估和数据质量检查提供了有效工具。
...
Method: 整合背景掩蔽、基于补丁的分析策略和高效的领域适应技术，专门针对手术器械图像优化无监督缺陷检测方法。

Result: 该方法能够可靠地检测手术器械图像中的细粒度缺陷，克服了现有方法的局限性。

Conclusion: 提出的方法通过专门针对手术器械的优化，实现了可靠的缺陷检测，解决了领域迁移中的关键挑战。
...
Method: 提出线性可分性（LSEP）正则化，将线性探测直接融入网络学习动态，促进中间层表示的线性可分性，无需外部编码器和表示对齐。

Result: 在基于流的Transformer架构（如SiT）上，训练效率和生成质量显著提升，在256×256 ImageNet数据集上达到FID 1.46。

Conclusion: LSEP方法有效替代了基于对齐的方法，无需外部编码器，将线性探测整合到训练过程中，实现了高效训练和高质量生成。
...
TL;DR: 提出了一种空间正则化对比学习策略，通过整合半变异函数来建模空间相关性，改善图像地理定位性能


<details>
  <summary>Details</summary>
Motivation: 现有对比学习方法忽略了地理空间中的空间依赖性，导致无法有效处理假负例（视觉和地理相似但被标记为负例的图像对）和难以区分硬负例（视觉相似但地理距离远的图像对）

Method: 使用半变异函数将特征空间中的图像距离与其地理距离相关联，捕捉空间相关性中的预期视觉内容，并以此定义给定空间距离下的预期视觉差异作为参考来识别硬负例和假负例

Result: 在OSV5M数据集上评估，证明显式建模空间先验能提高图像地理定位性能，特别是在更细粒度上表现更好

Conclusion: 整合空间先验的对比学习策略能有效改善图像地理定位，特别是在处理空间依赖性和区分困难样本方面
...
<details>
  <summary>Details</summary>
Motivation: 构建能够进行实时、开放式视频通话的数字人类代理，实现从静态肖像到持久智能视听交互的转变。

Method: 采用Thinker-Actor双变换器架构，Thinker模块基于预训练的大语言-语音模型处理流式用户输入，Actor模块使用分块自回归扩散模型，通过交叉注意力机制生成时间对齐的多模态响应。

Result: X-Streamer在两块A100 GPU上实时运行，能够维持数小时一致的视频聊天体验，从任意肖像创建交互式数字人类。

Conclusion: 该框架为实现交互式数字人类统一世界建模铺平了道路，展示了从单张肖像创建持久智能视听交互的可行性。
...
Method: 将运动预测任务构建为条件生成密集轨迹网格的问题，采用现代视频生成器的架构，但输出运动轨迹而非像素，从而直接捕捉场景范围内的动态和不确定性。

Result: 该方法在模拟数据上表现优异，在机器人等下游应用中有效，在真实世界直觉物理数据集上显示出有前景的准确性，比先前的回归器和生成器产生更准确和多样化的预测。

Conclusion: 直接建模运动轨迹比生成像素更有效地解决了从单张图像预测运动的问题，揭示了当前视频生成器作为世界模型的局限性，并为运动预测提供了更高效的解决方案。
...
Result: DINOv3聚类性能更优（轮廓系数0.31 vs 0.21），类间区分度更高（6.16倍分离比），特别擅长姿态识别；V-JEPA2在所有动作类型上表现更一致，性能方差更低（0.094 vs 0.288）。

Conclusion: DINOv3的空间处理架构擅长静态姿态识别，但在运动依赖动作上表现下降；V-JEPA2的时间建模提供跨动作类别的平衡表示质量。架构选择应根据任务需求和可靠性约束。
...
Method: 采用双架构方法：基于ResNet50的CNN-LSTM模型处理卫星图像，Vision Transformer模型处理无人机图像，结合ConceptNet和WordNet的外部语义知识扩展词汇覆盖。

Result: VLCE在InfoMetIC指标上达到95.33%的最高分，同时保持竞争力的语义对齐，显著优于LLaVA和QwenVL等基线模型。

Conclusion: VLCE的双架构系统通过自动化生成信息密集的描述，在改善灾害损害评估方面展现出巨大潜力。
...
Method: 使用多种表征相似性度量（RSA、Soft Matching、Linear Predictivity等）评估家族可分离性，并采用Similarity Network Fusion（SNF）方法整合这些互补的度量。

Result: 几何和调谐度量能强区分模型家族，而线性映射显示较弱分离。SNF融合后实现了比任何单一度量更清晰的家族分离，并揭示了监督ResNets和ViTs形成不同簇，而所有自监督模型跨越架构边界聚集在一起。

Conclusion: 这种生物学启发的框架为视觉模型提供了原则性分类，表明由架构和训练目标共同塑造的新兴计算策略定义了超越表面设计类别的表征结构。
...
Result: 实验表明FantasyWorld在多视角一致性和风格一致性方面优于现有几何一致性基线，几何分支的潜在表示可作为下游3D任务的通用表示。

Conclusion: 该框架有效桥接了视频想象与3D感知，统一主干和交叉分支信息交换是实现性能提升的关键。
...
<details>
  <summary>Details</summary>
Motivation: 解决科学机器学习中处理异构、多模态观测数据的挑战，为偏微分方程提供可扩展且数据高效的学习框架。

Method: 基于卷积视觉Transformer架构，结合组件卷积处理标量和矢量通道，跨场注意力建模不同物理场间信息传播，轴向注意力降低计算负担。

Result: 在多个下游预测任务中，MORPH在零样本和全样本泛化方面均优于从头训练的模型，匹配或超越现有强基线方法。

Conclusion: MORPH为学习科学观测数据的异构和多模态特性提供了灵活强大的基础架构，为可扩展的科学机器学习开辟了新路径。
...
<details>
  <summary>Details</summary>
Motivation: 红外成像在低光照和恶劣天气条件下具有优势，但类别不平衡、热噪声和计算限制等挑战会显著影响模型性能。需要开发既能保持高检测质量又能最小化计算成本的解决方案。

Method: 基于YOLOv8基线，用MobileNetV4替换CSPDarknet骨干网络以减少计算开销，并引入SlideLoss损失函数动态强调代表性不足和被遮挡的样本。

Result: MS-YOLO在FLIR ADAS V2基准测试中取得了有竞争力的mAP和优越的精度，同时仅需6.7 GFLOPs的计算量，计算开销减少了1.5%。
...
TL;DR: 提出MATR（Motion-Aware Transformer），通过显式预测物体运动来更新跟踪查询，减少查询冲突，在多目标跟踪任务中取得显著性能提升。


<details>
  <summary>Details</summary>
Motivation: 现有的DETR-based多目标跟踪框架将检测和跟踪查询在同一Transformer解码层中联合处理，导致查询冲突和关联精度下降。

Method: 引入运动感知Transformer，显式预测跨帧物体运动来提前更新跟踪查询，减少查询碰撞，实现更一致的训练。

Result: 在DanceTrack上HOTA提升超过9分，达到71.3的SOTA；SportsMOT上72.2 HOTA；BDD100k上54.7 mTETA和41.6 mHOTA，均达到SOTA水平。
...
Method: 提出两个互补组件：1）旋转有界的李相对偏置，通过紧凑预测模块预测每帧的平面内角度，实现几何一致对齐；2）微分群位移，计算相邻帧间角度差来估计速度，结合时间衰减和注意力掩码关注帧间关系。
...
TL;DR: 本文研究了基础AI/ML模型在合成孔径雷达(SAR)目标识别任务中的可行性，通过自监督微调公开SSL模型，开发了AFRL-DINOv2系列模型，在SAR基础模型上取得了新的最先进性能。


<details>
  <summary>Details</summary>
Motivation: 受自然图像领域基础模型成功的启发，希望将这种技术应用于SAR领域，利用自监督学习模型在有限标注数据下进行下游任务适配、对分布偏移更鲁棒、特征可迁移性强的优势。

Method: 首先测试现有视觉基础模型(DINOv2、DINOv3、PE-Core)在SAR目标识别中的表现，然后使用SAR数据对公开SSL模型进行自监督微调，开发AFRL-DINOv2系列模型，并分析不同骨干网络和下游任务适配方法的性能权衡。

Result: AFRL-DINOv2模型显著超越了当前最佳SAR领域模型SARATR-X，建立了新的SAR基础模型最先进水平，并在应对下游环境挑战(如扩展操作条件和低标注数据)方面表现出色。

Conclusion: 自监督微调公开SSL模型是开发SAR基础模型的可行路径，虽然取得了积极成果，但在SAR基础模型建设方面仍有很长的路要走。
...
TL;DR: UISim是一个基于图像的UI模拟器，通过两阶段方法从屏幕图像预测UI布局并合成新图像，实现动态UI转换模拟，用于UI测试、原型设计和AI代理训练。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖物理设备或静态截图分析，难以应对移动环境的动态多样性，限制了可扩展的UI测试和智能UI代理的开发。

Method: 采用两阶段方法：给定初始手机屏幕图像和用户操作，首先预测下一个UI状态的抽象布局，然后基于预测布局合成视觉一致的新图像。

Result: 实验结果显示UISim在生成真实且连贯的后续UI状态方面优于端到端UI生成基线方法，证明了其保真度和实用性。
...
TL;DR: 提出LFA-Net轻量级视网膜血管分割网络，使用LiteFusion-Attention注意力模块，在资源受限环境中实现高效分割。


<details>
  <summary>Details</summary>
Motivation: 解决现有深度学习模型在小血管分割和计算成本高方面的挑战，特别针对临床环境中有限计算资源的需求。

Method: 设计LiteFusion-Attention注意力模块，结合残差学习连接、Vision Mamba启发的动态机制和基于调制的注意力，高效捕获局部和全局上下文。
...
Conclusion: LFA-Net是资源受限环境下视网膜血管分割的理想解决方案，实现了高性能与低计算成本的平衡。
...
TL;DR: 提出融合视觉场景上下文和标签引导语义信息的新框架，通过多尺度场景编码和情感词典生成来提升群体情感识别性能
...
Method: 包含视觉上下文编码模块（多尺度场景信息编码个体关系）和情感语义编码模块（利用大语言模型生成情感词典，通过结构化情感树生成语义表示），最后通过相似性感知交互对齐融合视觉和语义信息

Result: 在三个广泛采用的GER数据集上实验表明，该方法相比最先进方法取得了有竞争力的性能

Conclusion: 提出的融合视觉场景上下文和标签引导语义信息的框架有效提升了群体情感识别性能
...
<details>
  <summary>Details</summary>
Motivation: SAM模型直接应用于医学图像分割存在三个主要挑战：边界模糊、解剖关系建模不足以及缺乏不确定性量化。这些限制了其在临床高风险场景中的应用。
...
Conclusion: KG-SAM被确立为一个鲁棒且可推广的医学图像分割框架，显著提升了分割性能和在临床环境中的可靠性。
...
TL;DR: UniVid框架通过微调预训练的视频生成模型，使其能够处理多种视觉任务，而无需任务特定的修改，展示了视频生成模型作为统一视觉基础模型的潜力。


<details>
  <summary>Details</summary>
Motivation: 受大语言模型统一处理多种任务的启发，研究者希望将这种范式扩展到视觉领域。但现有方法需要跨模态和跨源的任务特定预训练，成本高且难以扩展到未见任务。

Method: 提出UniVid框架，微调视频扩散变换器来处理各种视觉任务。任务表示为视觉句子，其中上下文序列定义任务和期望的输出模态。

Result: 尽管仅使用自然视频数据训练，UniVid在跨模态推理和跨源任务上都表现出良好的泛化能力。理解和生成任务可以通过简单反转视觉句子顺序来切换。

Conclusion: 预训练的视频生成模型有潜力作为可扩展和统一的视觉建模基础，为构建通用视觉模型提供了新思路。
...
<details>
  <summary>Details</summary>
Motivation: 现代ViT架构采用空间设计（如窗口注意力、相对位置编码等），现有token缩减方法无法保持这些架构所依赖的空间结构。

Method: 使用2D缩减策略保持结构化token布局，空间感知合并算法维持相对位置关系，以及新颖的每维度最大幅度token表示来保留显著特征。

Result: 在SAM-H上实现1.25倍加速且仅0.7% mIOU下降，在DeiT-B上实现1.15倍加速且无top-1精度损失。

Conclusion: 该方法在空间和非空间架构上均达到最先进水平，支持即插即用和微调两种使用方式。
...
TL;DR: 提出了GASP-ICL框架，一种无需训练的多模态深度伪造检测方法，通过图结构泰勒自适应评分器和上下文学习来增强大视觉语言模型的检测能力。


<details>
  <summary>Details</summary>
Motivation: 解决大视觉语言模型在多模态深度伪造检测中难以捕捉细微伪造线索、解决跨模态不一致性和执行任务对齐检索的问题。

Method: 使用MDD适配的特征提取器检索对齐的图像-文本对，设计图结构泰勒自适应评分器(GSTAS)来捕获跨样本关系并传播查询对齐信号，生成判别性示例。

Result: 在四种伪造类型上的实验表明，GASP-ICL超越了强基线方法，在不进行大视觉语言模型微调的情况下实现了性能提升。

Conclusion: GASP-ICL框架能够有效增强大视觉语言模型在多模态深度伪造检测中的性能，通过语义对齐的任务相关演示实现稳健检测。
...
Method: 使用时空场景图和动态提示模块引导图解析神经网络生成动作特定表示，设计视频适配的GPNN使用动态权重聚合信息

Result: 在视频动作识别实验中证明该方法优于现有最先进方法

Conclusion: ProDA框架能有效从复杂场景中解耦指定动作，提升多动作场景下的识别性能
...
TL;DR: 该论文提出了一个用于识别数字内容中仇恨的多模态数据集，结合水印稳定扩散技术和数字注意力分析模块来检测图像中的仇恨元素，并开发了DeHater视觉语言模型进行多模态去仇恨化任务。
...
Method: 使用水印稳定扩散技术结合数字注意力分析模块(DAAM)来识别图像中的仇恨元素，生成仇恨注意力地图，并模糊这些区域以去除仇恨内容。

Result: 开发了一个多模态仇恨检测数据集和DeHater视觉语言模型，为AI驱动的图像仇恨检测设立了新标准。

Conclusion: 该方法有助于开发更符合伦理的社交媒体AI应用，促进数字环境的健康发展。
...
TL;DR: 提出了一个统一的框架MIRG-RL，通过两阶段训练（监督微调+图像感知强化学习）来解决多图像推理和定位问题，在跨图像推理任务中达到64.82%的SOTA性能。
...
Method: 两阶段训练范式：结合带标注轨迹的监督微调和图像感知强化学习优化；创新性地提出了构建轨迹数据的方法，整合对象级和图像级标注信息；设计了具有对象和图像双重奖励函数的图像感知RL策略。

Result: 在多图像定位基准测试中达到最先进性能，跨图像推理任务达到64.82%，比之前最佳方法提升1%。

Conclusion: MIRG-RL框架有效解决了多图像推理和定位问题，通过强化学习方法显著提升了模型的跨图像推理能力。
...
<details>
  <summary>Details</summary>
Motivation: 当前视频生成方法在长时程生成中存在困难：扩散方法存在时间不一致性和视觉漂移，自回归方法则牺牲视觉细节。需要解决稳定长时程生成的问题。

Method: 采用混合框架，结合块内扩散去噪和块间自回归生成；核心创新是动作引导的可变长度分块机制；引入上下文感知专家混合框架(CMoE)来保证视觉质量和块间过渡。

Result: 实验结果表明该方法在扩展推演中实现了稳定和一致的长时程生成。

Conclusion: LongScape通过混合方法和动作引导分块机制，成功解决了长时程视频生成的稳定性问题。
...
TL;DR: MoWM是一个用于具身动作规划的混合世界模型框架，融合了潜在模型和像素空间模型的表征，通过运动感知表征指导细粒度视觉特征提取，在CALVIN基准上实现了最先进的任务成功率。


<details>
  <summary>Details</summary>
Motivation: 解决具身动作规划中的核心挑战，现有视频生成世界模型存在像素级重建的视觉冗余问题，而潜在世界模型又忽略了精确操作所需的细粒度细节。

Method: 提出MoWM混合世界模型框架，使用潜在模型的运动感知表征作为高级先验，指导从像素空间模型提取细粒度视觉特征，突出对动作解码有用的视觉细节。

Result: 在CALVIN基准上的广泛评估表明，该方法实现了最先进的任务成功率和优越的泛化能力。

Conclusion: MoWM框架有效融合了不同世界模型的优势，为具身规划研究提供了有价值的见解。
...
TL;DR: 提出了DiTraj框架，一种针对扩散变换器(DiT)的无训练轨迹控制方法，通过前景-背景分离引导和时空解耦3D-RoPE实现精确的轨迹控制。
...
Method: 使用LLM将用户提示分为前景和背景提示分别引导生成；提出STD-RoPE通过修改前景token的位置嵌入消除跨帧空间差异，增强轨迹控制；通过调节位置嵌入密度实现3D感知轨迹控制。

Result: 实验表明该方法在视频质量和轨迹可控性方面优于先前方法。

Conclusion: DiTraj为DiT视频生成提供了一种简单有效的无训练轨迹控制解决方案。
...
TL;DR: 本文评估了多跳问答中的检索策略，提出了一种结合稠密嵌入、词汇重叠和重排序的混合方法，在HotpotQA数据集上相比余弦相似度在精确匹配和F1分数上分别提升了50%和47%。
...
Method: 在检索增强生成框架下比较余弦相似度、最大边际相关性和混合方法，采用EfficientRAG管道进行查询优化，引入token标记和迭代优化。

Result: 混合方法显著优于基线方法，在HotpotQA数据集上相对余弦相似度在精确匹配和F1分数分别提升50%和47%，改善了实体召回和证据互补性。
...
TL;DR: 提出了HDR动态新视角合成问题，并开发了HDR-4DGS方法，通过动态色调映射模块实现时间辐射一致性和空间准确的颜色转换，在动态场景中生成逼真的HDR渲染。


<details>
  <summary>Details</summary>
Motivation: 现有HDR新视角合成方法主要关注静态场景，但现实世界场景经常包含动态元素，如移动物体和变化的照明条件，这提出了更具挑战性的问题。

Method: 提出了HDR-4DGS，一种基于高斯泼溅的架构，具有创新的动态色调映射模块，通过根据时间维度上不断变化的辐射分布动态调整色调映射函数，明确连接HDR和LDR域。

Result: 广泛的实验表明，HDR-4DGS在定量性能和视觉保真度方面都超过了现有的最先进方法。

Conclusion: HDR-4DGS能够实现时间辐射一致性和空间准确的颜色转换，从任意视角和时间实例生成逼真的HDR渲染。
...
TL;DR: SRHand是一种从低分辨率图像重建详细3D手部几何和纹理的方法，结合隐式图像表示和显式手部网格，通过几何感知隐式图像函数实现多视角一致的高质量上采样和重建。
...
Method: 提出几何感知隐式图像函数(GIIF)，结合隐式图像表示和显式手部网格，联合优化隐式图像函数和3D手部形状，保持多视角和姿态一致性。

Result: 在InterHand2.6M和Goliath数据集上，SRHand在定量和定性评估中显著优于最先进的图像上采样方法和3D手部重建方法。

Conclusion: SRHand能够从低分辨率图像重建出包含皱纹、指甲等细节的精细3D手部几何和纹理图像，解决了现有方法的局限性。
...
TL;DR: 本文质疑当前AI生成图像检测研究过于关注'假'图像而忽视'真'图像的定义，指出现代智能手机拍摄的照片也经过神经网络处理，与生成图像技术相似，呼吁重新思考'真实'图像的概念。


<details>
  <summary>Details</summary>
Motivation: 现代图像生成模型的普及引发了对其负面社会影响的担忧，但现有研究过度关注生成算法和'假'样本，缺乏对'真实'图像的明确定义和数据收集。

Method: 通过分析当前图像检测方法的局限性，指出这些方法依赖过时低分辨率的'真实'图像数据集，而现代智能手机拍摄的照片也经过神经网络处理。

Result: 发现当前'假'图像检测方法存在根本性缺陷，因为现代'真实'图像的形成过程与生成图像技术高度相似，挑战了传统真伪区分的有效性。

Conclusion: 需要重新思考'真实'图像的概念，制定明确的技术定义，创建新的基准数据集，并质疑检测'假'图像这一目标本身是否合理。
...
TL;DR: Aes-R1是一个用于图像美学评估的综合框架，通过强化学习结合链式思维数据构建和相对-绝对策略优化，提升多模态大语言模型的美学评分和推理能力。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型(MLLMs)在图像美学评估方面具有潜力，但缺乏高质量的美学推理数据和美学判断的主观性限制了其生成准确判断和可解释理由的能力。

Method: 提出Aes-R1框架：1) AesCoT管道构建和过滤高质量链式思维美学推理数据用于冷启动；2) 教导模型在评分前生成结构化解释；3) 使用RAPO强化学习算法联合优化绝对分数回归和相对排序，提升单图准确性和跨图偏好判断。

Result: Aes-R1将骨干模型的平均PLCC/SRCC提升了47.9%/34.8%，超越了同类规模的先进基线方法。消融研究验证了在有限监督和分布外场景下的鲁棒泛化能力。

Conclusion: Aes-R1使MLLMs能够生成有依据的解释和可信的评分，在统一框架中增强了美学评分和推理能力。
...
TL;DR: StableDub是一个创新的视觉配音框架，通过唇部习惯建模和遮挡鲁棒合成，解决了音频驱动方法无法捕捉说话者特定唇部习惯以及传统修复方法在处理遮挡时产生视觉伪影的问题。


<details>
  <summary>Details</summary>
Motivation: 现有视觉配音方法存在两个关键缺陷：(1)仅音频驱动范式无法充分捕捉说话者特定的唇部习惯，导致生成的唇部运动与目标头像不相似；(2)传统盲修复方法在处理遮挡物（如麦克风、手）时经常产生视觉伪影，限制了实际部署。

Method: 基于Stable-Diffusion骨干网络，开发了唇部习惯调制机制，联合建模音素级别的视听同步和说话者特定的口面部动态。引入遮挡感知训练策略，在修复过程中显式暴露遮挡物体。采用混合Mamba-Transformer架构优化训练效率。

Result: 实验结果表明StableDub在唇部习惯相似性和遮挡鲁棒性方面表现优异，在音频-唇部同步、视频质量和分辨率一致性方面也优于其他方法。

Conclusion: StableDub通过整合唇部习惯感知建模和遮挡鲁棒合成，显著提升了视觉配音方法的性能和应用范围，特别是在低资源研究场景中表现出增强的适用性。
...
TL;DR: Drag4D是一个交互式框架，通过文本驱动生成3D场景并控制物体运动，允许用户为3D物体定义轨迹并集成到高质量3D背景中。


<details>
  <summary>Details</summary>
Motivation: 解决在文本驱动的3D场景生成中集成物体运动控制的问题，实现用户对3D物体轨迹的精确控制。

Method: 三阶段流程：1) 使用2D高斯泼溅和全景图像增强文本到3D背景生成；2) 通过3D复制粘贴将物体网格集成到场景中；3) 使用部分增强的运动条件视频扩散模型进行时间动画。

Result: 实现了在高质量3D背景中用户控制物体运动的协调对齐，展示了统一架构的有效性。

Conclusion: Drag4D框架成功实现了3D场景生成与物体运动控制的集成，提供了精确的空间和时间对齐。
...
TL;DR: Syncphony是一个音频到视频生成模型，通过运动感知损失和音频同步引导技术，实现了与音频精确同步的高质量视频生成。
...
Method: 基于预训练视频主干网络，引入两个关键组件：运动感知损失（强调高运动区域学习）和音频同步引导（使用视觉对齐的无音频层模型指导全模型）。

Result: 在AVSync15和The Greatest Hits数据集上的实验表明，Syncphony在同步精度和视觉质量方面均优于现有方法。

Conclusion: Syncphony通过创新的同步机制成功实现了音频驱动的时序精确视频生成，并提出了CycleSync评估指标来量化同步效果。
...
TL;DR: 提出了一种基于语言引导的遥感变化检测模型LG-CD，通过自然语言提示引导网络关注感兴趣区域，显著提升了变化检测的准确性和鲁棒性。
...
Method: 使用视觉基础模型SAM2提取多尺度金字塔特征，通过多层适配器微调模型，设计文本融合注意力模块对齐视觉和文本信息，并实现视觉语义融合解码器。
...
TL;DR: 提出了一种联合文本和拖拽控制的图像编辑框架，结合了文本驱动和拖拽驱动方法的优势，实现了精确的空间控制和纹理指导。


<details>
  <summary>Details</summary>
Motivation: 现有文本驱动方法缺乏精确空间控制，而拖拽驱动方法缺少纹理指导，需要结合两者优势来解决互补局限性。

Method: 采用扩散模型框架，引入点云确定性拖拽增强潜在空间布局控制，以及拖拽-文本引导去噪动态平衡两种条件的影响。

Result: 实验表明该方法不仅能实现高保真联合编辑，还在单独文本或拖拽模式下表现优异，超越了专门的单模式方法。

Conclusion: 建立了一个多功能、可泛化的可控图像编辑解决方案，支持灵活的操作模式。
...
TL;DR: 提出了一种侧载对比学习适应框架，利用未标注数据通过对比学习训练CNN表示提取器，然后将其侧载到冻结的YOLO11n主干网络中，在北极地区无人机车辆检测任务中提升性能3.8%-9.5%


<details>
  <summary>Details</summary>
Motivation: 解决北极地区无人机图像中因积雪覆盖导致的强可见性挑战和域偏移问题，利用廉价获取的未标注数据改进车辆检测

Method: 侧载对比学习适应框架：在预训练阶段通过对比学习在未标注数据上训练CNN表示提取器，在微调阶段将其侧载到冻结的YOLO11n主干网络中，比较不同融合方法和粒度

Result: 在NVD数据集上，检测性能在mAP50指标上提升了3.8%到9.5%

Conclusion: 侧载对比学习适应框架能有效利用未标注数据提升轻量级模型在具有挑战性环境下的车辆检测性能
...
TL;DR: 提出IF-V2V方法，无需反转即可将现成的基于流匹配的图像到视频模型适配用于视频编辑，通过向量场校正和结构运动保持初始化来确保编辑质量和一致性。
...
Method: 使用向量场校正与样本偏差来将源视频信息融入去噪过程，采用结构运动保持初始化生成运动感知的时间相关噪声，并引入偏差缓存机制减少计算开销。
...
TL;DR: SSLCounter是一个用于多视角计数的自监督学习框架，通过神经体渲染减少对大规模标注数据的依赖，在仅使用70%训练数据时仍能实现竞争性性能。
...
Method: 使用神经体渲染学习场景的隐式表示，通过可微分神经渲染重建连续几何形状和复杂视角相关外观。

Result: 在多个多视角计数基准测试中达到最先进性能，仅使用70%训练数据时仍能保持竞争性表现。

Conclusion: SSLCounter展示了卓越的数据效率，其核心思想可以无缝集成到现有框架中，为多视角计数提供了有效的自监督解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 当前大多数基准测试强调定位准确性，但缺乏对物体在场景中如何排列和关联的评估，而有效的场景理解不仅需要识别物体，还需要推理它们的相对位置、分组和深度关系
...
<details>
  <summary>Details</summary>
Motivation: 视觉上下文学习(VICL)通常过度依赖单个上下文图像对，这会导致有偏差和不稳定的预测。

Method: 提出了基于补丁的k近邻视觉上下文学习(PANICL)，通过平滑多个上下文对之间的分配分数来减少偏差，无需额外训练。

Result: 在多种视觉任务上的实验表明，PANICL相比强基线方法有持续改进，并对领域偏移表现出强鲁棒性，能很好地泛化到其他VICL模型。

Conclusion: PANICL是一个通用且广泛适用的框架，能有效解决VICL中的过依赖问题，提高模型性能和鲁棒性。
...
TL;DR: SingRef6D是一个轻量级的6D姿态估计框架，仅需单张RGB图像作为参考，无需深度传感器、多视图图像或训练视图合成模型，在资源受限环境下仍能保持鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有6D姿态估计方法存在实际限制：基于传感器深度的方法在透明或高反射表面条件下容易失效；基于RGB的方法在低光照和无纹理场景中因缺乏几何信息而匹配性能较差。

Method: 提出两个关键创新：1) 在Depth-Anything v2基础上引入token-scaler微调机制和新优化损失，提升对挑战性表面的深度预测精度；2) 引入深度感知匹配过程，在LoFTR中有效整合空间关系，处理挑战性材质和光照条件下的匹配。

Result: 深度预测在REAL275数据集上相比Depth-Anything v2（微调头）提升14.41%（δ1.05指标）；在REAL275、ClearPose和Toyota-Light数据集上的姿态估计评估显示，平均召回率比最先进方法提升6.1%。

Conclusion: SingRef6D通过仅使用单张RGB参考图像，在无需深度传感器或密集模板的情况下，实现了对挑战性表面和光照条件的鲁棒6D姿态估计，在资源受限环境中具有显著优势。
...
TL;DR: DynaNav是一个动态视觉导航框架，通过基于场景复杂度的特征和层选择来降低计算开销，同时提高可解释性和导航性能。


<details>
  <summary>Details</summary>
Motivation: 现有的基础模型（特别是基于transformer解码器的）存在计算开销高和可解释性差的问题，限制了在资源紧张场景中的部署。

Method: 提出DynaNav框架，使用可训练的硬特征选择器进行稀疏操作，并将特征选择集成到提前退出机制中，通过贝叶斯优化确定最优退出阈值。

Result: 在真实世界数据集和模拟环境中，相比ViNT，DynaNav实现了FLOPs减少2.26倍，推理时间降低42.3%，内存使用降低32.8%，并在四个公共数据集上提高了导航性能。

Conclusion: DynaNav通过动态特征和层选择机制，在保持导航性能的同时显著降低了计算成本，为资源受限环境下的视觉导航提供了高效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: ControlNet依赖与生成目标精确对齐的视觉条件，但在实际应用中常遇到条件不对齐的情况，导致生成效果不佳。

Method: 使用与视觉条件对齐的替代提示进行辅助去噪，提取注意力掩码，然后在目标提示去噪过程中利用这些掩码来抑制冲突区域。

Result: 实验表明该方法在深度图、边缘图和人体骨架等各种不对齐条件下均优于现有基线方法。

Conclusion: SemanticControl能够有效利用语义相关但不对齐的视觉条件，提升文本到图像生成的性能。
...
TL;DR: 该论文提出了一个用于评估多模态大语言模型视觉情感感知能力的自动化评估框架，通过情感陈述判断任务来克服现有评估方法的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有MLLMs在图像情感感知方面的能力存在争议，评估方法存在忽略合理响应、情感分类有限、忽视上下文因素和人工标注成本高等问题。

Method: 设计了情感陈述判断任务和自动化流水线，能够高效构建情感中心化陈述，减少人工标注工作量。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于上下文学习的方法依赖简单的重建目标，导致严重的属性泄漏和无法对齐人类偏好，需要解决主体保真度和偏好对齐问题。
...
Method: 采用类似SAM的编码器-解码器架构，使用基于triplane的双分支编码器生成空间结构化的token；通过模型在环标注流程从在线资源中整理超过500万个3D形状-部件对。

Result: PartSAM在多个基准测试中大幅超越最先进方法，实现了高度准确的部件识别，并能自动将形状分解为表面和内部结构。

Conclusion: PartSAM标志着向3D部件理解基础模型的决定性一步，结合可扩展架构和多样化3D数据，展现出新兴的开放世界能力。
...
TL;DR: 提出基于深度学习的无参考图像对比度质量评估框架，通过定制化预训练模型EfficientNet B0、ResNet18和MobileNetV2，在对比度失真数据集上取得最优性能。


<details>
  <summary>Details</summary>
Motivation: 图像对比度是视觉感知的关键因素，但现有无参考图像质量评估模型难以准确评估真实场景下的对比度失真。

Method: 定制化并微调三个预训练架构（EfficientNet B0、ResNet18、MobileNetV2），添加对比度感知回归头，使用针对性数据增强在CID2013和CCID2014数据集上进行端到端训练。

Result: 定制化的EfficientNet B0模型在CCID2014上达到PLCC=0.9286和SRCC=0.9178，在CID2013上达到PLCC=0.9581和SRCC=0.9369，性能优于传统方法和深度基线模型。

Conclusion: 轻量级预训练网络的对比度感知适配可为无参考对比度质量评估提供高性能、可扩展的解决方案，适用于实时和资源受限应用。
...
TL;DR: Geo-R1是一个面向少样本地理空间指代表达理解的推理强化微调方法，通过先生成显式推理链再定位目标对象，在数据稀缺场景下显著优于监督微调方法。


<details>
  <summary>Details</summary>
Motivation: 解决多模态大语言模型在数据稀缺场景下泛化能力差的问题，特别是在地理空间指代表达理解中需要处理复杂的对象-上下文关系。

Method: 提出推理中心的强化微调范式，强制模型先生成可解释的推理链来分解指代表达，然后利用这些推理结果来定位目标对象。

Result: 在三个精心设计的少样本地理空间指代表达基准测试中，Geo-R1始终大幅优于监督微调基线，并展现出强大的跨数据集泛化能力。

Conclusion: Geo-R1的"先推理后行动"方法能够更有效地利用有限标注数据，提升泛化能力并提供可解释性，为少样本地理空间理解提供了有效解决方案。
...
TL;DR: 该研究评估了视觉语言模型在医疗视觉问答中的临床奉承行为，提出了一个医疗奉承数据集和轻量级缓解策略VIPER，通过过滤非证据内容来减少奉承行为。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型在临床工作流程中越来越普遍，但它们经常表现出奉承行为，优先考虑与用户措辞、社交线索或感知权威的一致性，而不是基于证据的推理。

Method: 从PathVQA、SLAKE和VQA-RAD构建医疗奉承数据集，使用心理学启发的压力模板，提出VIPER框架过滤非证据内容并生成基于证据的答案。

Result: 发现各种VLM模型普遍易受攻击，表现出显著的对抗性响应变化，模仿和专家提供的修正被发现是最有效的触发因素。VIPER框架平均减少了奉承行为，优于基线方法。

Conclusion: 该研究的基准分析和缓解框架为医疗VLM在真实世界临床交互中的稳健部署奠定了基础，强调了对基于证据的防御的需求。
...
TL;DR: GLARIFY是一种利用时空注视信息增强视觉语言模型在现实应用中效果的新方法，通过处理模糊的注视模式和复杂时空关系来改进智能眼镜的交互体验。


<details>
  <summary>Details</summary>
Motivation: 随着智能眼镜的普及，用户注意力被整合到视觉语言模型中，但注视数据建模存在两个挑战：用户口头问题使用代词或跳过上下文导致模糊性，人类注视模式嘈杂且与口头问题存在复杂时空关系。

Method: 首先分析数百个注视模态查询样本，证明用户注视模式的嘈杂性；然后使用GPT-4o设计自动数据合成管道生成GLARIFY-Ambi数据集，包含专门的思维链过程处理嘈杂注视模式；最后设计热图模块将注视信息整合到先进视觉语言模型中，同时保留预训练知识。
...
TL;DR: 研究发现大型视觉语言模型存在空间偏见问题，当相同关键视觉信息放置在不同位置时会产生不一致输出，这源于语言模型中位置嵌入的不平衡设计。作者提出平衡位置分配方法来解决这一问题。


<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型在多模态任务中表现优异，但其对空间变化的鲁棒性尚未被充分理解。作者旨在系统研究LVLMs的空间偏见问题，探索模型如何响应相同视觉信息在不同位置的情况。

Method: 通过精心设计的探测数据集分析LVLMs的空间偏见，发现该问题源于语言模型中位置嵌入的不平衡设计。提出平衡位置分配方法，为所有图像令牌分配相同的位置嵌入，促进更平衡的视觉信息整合。

Result: 实验表明BaPA方法无需重新训练即可增强LVLMs的空间鲁棒性，结合轻量级微调还能进一步提升在各种多模态基准测试中的性能。信息流分析显示BaPA产生平衡的注意力，实现更全面的视觉理解。

Conclusion: LVLMs存在显著的空间偏见问题，主要源于语言模型中位置嵌入的不平衡设计。提出的BaPA方法能有效缓解这一问题，提高模型的空间鲁棒性和整体性能。
...
TL;DR: 提出了一种从预训练扩散模型中解耦视觉和语义特征的新方法，支持视觉对应关系分析，并提出了量化主题驱动图像生成中视觉不一致性的新指标VSM


<details>
  <summary>Details</summary>
Motivation: 扩散模型主干网络已知编码了丰富的语义特征，但必须也包含视觉特征来支持图像合成能力。然而由于缺乏标注数据集，分离这些视觉特征具有挑战性

Method: 引入自动化流程构建具有标注语义和视觉对应关系的图像对，设计对比架构分离两种特征类型，利用解耦表示提出VSM指标

Result: 实验结果显示该方法在量化视觉不一致性方面优于CLIP、DINO和视觉语言模型等全局特征指标，同时支持不一致区域的空间定位

Conclusion: 这是首个同时支持主题驱动生成中不一致性量化和定位的方法，为该任务的推进提供了有价值的工具
...
Method: 采用分层特征融合策略和联合多模态多任务训练方法，支持任意模态间的跨模态检索和提示感知嵌入生成。

Result: 在MMEB-v2视频基准测试中创下新记录，在音频和视频到音频检索中表现优异，在多模态问答中显著优于现有嵌入模型。
...
TL;DR: ERGO提出了一种从粗到精的推理管道，通过先分析下采样图像识别任务相关区域，然后仅对相关区域进行全分辨率处理，在保持精度的同时显著减少计算开销。


<details>
  <summary>Details</summary>
Motivation: 现有大型视觉语言模型在处理高分辨率图像时会产生大量视觉标记，导致计算开销巨大。随着"图像思维"模型的出现，推理扩展到视觉领域，需要更高效的处理方法。

Method: 采用两阶段"从粗到精"推理管道：第一阶段分析下采样图像识别任务相关区域；第二阶段仅对相关区域进行全分辨率处理。使用强化学习框架开发奖励组件来处理感知不确定性。

Result: 在多个数据集上，ERGO比原始模型和竞争方法获得更高准确率，同时效率更高。例如在V*基准测试中超越Qwen2.5-VL-7B 4.7分，仅使用23%的视觉标记，实现3倍推理加速。

Conclusion: ERGO通过推理驱动的感知方法，在保持视觉细节的同时显著提高了处理效率，为高分辨率图像的视觉语言应用提供了有效的解决方案。
...
Method: 提出变分公式，包含针对深度聚焦的双重约束：空间约束利用不同焦点级别的梯度模式变化来区分真实深度边缘和纹理伪影，焦距约束强制单峰单调的焦点概率与物理焦点行为对齐。

Result: 在四个公共数据集上的综合实验表明，DualFocus在深度精度和感知质量方面始终优于最先进的方法。

Conclusion: 通过引入空间和焦距约束的归纳偏置，DualFocus在具有挑战性的区域提高了鲁棒性和准确性。
...
TL;DR: 提出了RDcomm框架，基于信息论构建多智能体协作的率失真理论，通过任务熵离散编码和互信息驱动的消息选择，在3D目标检测和BEV分割任务中实现108倍通信量减少的同时保持SOTA精度。


<details>
  <summary>Details</summary>
Motivation: 现有协作感知研究缺乏理论基础，特别是在任务性能与通信量权衡方面的理论支撑。需要填补多智能体协作中率失真理论的研究空白。

Method: 1) 提出多智能体协作的实用率失真理论；2) 任务熵离散编码：为特征分配任务相关码字长度以最大化信息效率；3) 互信息驱动的消息选择：利用互信息神经估计实现最优无冗余条件。

Result: 在DAIR-V2X和OPV2V数据集上，RDcomm在3D目标检测和BEV分割任务中达到最先进精度，同时通信量最多减少108倍。

Conclusion: 基于信息论的率失真理论为多智能体协作感知提供了理论基础，RDcomm框架通过高效的信息编码和消息选择机制，在保持性能的同时大幅降低通信开销。
...
TL;DR: FailureAtlas是一个主动探索文本到图像模型失败模式的框架，通过结构化搜索方法发现了大量未知错误，并揭示了这些失败与训练数据稀缺的关联。


<details>
  <summary>Details</summary>
Motivation: 静态基准测试在诊断文本到图像模型的系统性失败方面能力有限，需要一种主动探索的方法来全面映射失败景观并定位根本原因。

Method: 将错误发现构建为对最小化失败诱导概念的结构化搜索，采用新颖的加速技术处理计算爆炸问题，实现对T2I模型失败景观的自主探索和映射。

Result: 在Stable Diffusion模型中发现超过247,000个先前未知的错误切片，首次提供了大规模证据表明这些失败与训练数据集中的数据稀缺相关。

Conclusion: FailureAtlas建立了一种以诊断为导向的新方法论，为深度模型审计提供了原则性和可扩展的引擎，指导开发更稳健的生成式AI。
...
<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在视觉语言任务中表现出色，但容易产生与视觉证据不一致的幻觉内容，现有缓解方法通常需要额外微调或牺牲信息量和可扩展性。

Method: 引入幻觉放大机制：通过文本到图像模型将描述投影到视觉空间作为负锚点，原始图像作为正锚点，利用双锚点编辑解码器隐藏状态，拉近忠实语义表示，推离幻觉方向。

Result: 在多个基准测试中显著减少了对象、属性和关系层面的幻觉，使用LLaVA-v1.5-7B在CHAIR上实现了超过5%的幻觉减少，同时保持召回率和描述丰富度，并在多种架构上验证了强跨架构泛化能力。
...
TL;DR: 提出CoFFT方法，通过模拟人类视觉认知来增强视觉语言模型的推理能力，无需训练即可提升性能3.1-5.8%


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型在处理复杂冗余的视觉输入时容易受到干扰，产生任务无关的推理过程甚至幻觉，因为无法在推理过程中精确发现和处理所需区域

Method: CoFFT方法包含三个迭代阶段：1) 多样化样本生成探索推理路径；2) 双前瞻解码基于视觉焦点和推理进展评估样本；3) 视觉焦点调整向最有利于未来推理的区域精确调整
...
TL;DR: 提出轻量级多模态框架，结合Qwen2.5-VL-3B-Instruct模型和SmolAgent编排层，用于视频场景理解，支持思维链推理、语音-视觉融合和动态工具调用，在医疗机器人应用中表现优异。


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型在时间推理、不确定性估计和结构化输出方面存在局限，无法满足医疗机器人对鲁棒多模态感知和推理的需求。

Method: 结合Qwen2.5-VL-3B-Instruct模型与SmolAgent编排层，支持思维链推理、语音-视觉融合、动态工具调用，生成结构化场景图并使用混合检索模块。

Result: 在Video-MME基准测试和自定义临床数据集上表现出竞争性准确率和改进的鲁棒性，优于最先进的视觉语言模型。

Conclusion: 该框架在机器人辅助手术、患者监测和决策支持等医疗应用方面具有潜力，为医疗机器人提供了更可靠的多模态感知和推理能力。
...
Method: 构建了包含语音内容、韵律、注视、身体动作和视觉上下文的多模态数据集，并对比了多模态大语言模型与传统任务特定模型的表现。

Result: 多模态大语言模型在无需任务特定微调的情况下就超越了专门的基线模型。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法通常将声音分离视为基于掩码的回归问题，在捕捉复杂数据分布以高质量分离不同类别声音方面存在局限性。
...
Conclusion: DAVIS生成框架在解决音频-视觉声源分离任务方面表现出高效性，特别擅长为不同声音类别生成高质量的声音分离结果。
...
TL;DR: 提出SpecXNet双域网络架构，通过空间域和频域特征耦合进行深度伪造检测，在跨数据集和未知篡改场景下达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: GAN和扩散模型生成的深度伪造内容越来越逼真，现有方法仅关注空间或频域特征，限制了其对未见篡改的泛化能力。

Method: 提出双域特征耦合器(DDFC)分解为局部空间分支和全局频域分支，以及双傅里叶注意力(DFA)模块动态融合空间和频域特征，基于改进的XceptionNet架构。
...
TL;DR: 提出了大型材料高斯模型(MGM)，能够生成带有PBR材质属性的高质量3D内容，解决了现有模型无法生成材质属性的问题。


<details>
  <summary>Details</summary>
Motivation: 现有的大型重建模型虽然能高效实现高质量3D渲染，但无法生成材质的物理属性（如反照率、粗糙度、金属度），这在多样化光照环境中对真实感渲染至关重要。

Method: 首先微调新的多视角材质扩散模型，基于输入的深度和法线图生成多视角PBR图像；然后探索高斯材质表示，不仅与2D高斯泼溅对齐，还建模PBR材质的每个通道；最后通过渲染重建的点云获取PBR属性。

Result: 实验表明，该方法生成的材质在视觉上比基线方法更具吸引力，同时增强了材质建模能力，实现了实用的下游渲染应用。

Conclusion: MGM框架成功生成了具有PBR材质属性的高质量3D内容，支持动态重光照，为实际渲染应用提供了可行解决方案。
...
TL;DR: 提出了一种基于多视图增强的自监督点云补全方法，首次将Mamba架构引入该任务，在合成和真实数据集上达到最先进效果


<details>
  <summary>Details</summary>
Motivation: 现有方法存在局限性：监督方法依赖真实标注，泛化能力差；无监督方法需要完整点云；弱监督方法需要多视角观测；现有自监督方法因信号能力有限导致预测效果不佳
...
Method: 采用半监督蒸馏框架，引入三级知识融合损失来转移不同层次的知识，同时利用标记和未标记数据来增强泛化能力

Result: 实验显示Refine-Control显著降低了计算成本和延迟，同时保持了高保真生成能力和可控性

Conclusion: 该框架为在资源受限设备上部署高质量条件图像生成模型提供了有效解决方案
...
TL;DR: 提出JGEKD分类策略，通过联合图熵知识蒸馏处理非独立同分布的3D点云数据，捕获类间相关性并提升模型鲁棒性。
...
Method: 构建基于联合图熵的损失函数，使用联合图捕获类间隐藏关系，通过知识蒸馏训练模型。采用孪生结构处理空间变换不变性，开发自知识蒸馏和教师知识蒸馏框架。

Result: 在ScanObject、ModelNet40、ScanntV2_cls和ModelNet-C数据集上的广泛实验表明，该策略能取得有竞争力的结果。

Conclusion: JGEKD策略能有效处理非独立同分布的3D点云数据，通过联合图熵知识蒸馏实现类间相关性转移，并增强模型对数据损坏的鲁棒性。
...
<details>
  <summary>Details</summary>
Motivation: 现有的神经程序合成方法仅将图表示为文本程序，未能捕捉节点图固有的视觉空间特性，而这正是人类易于理解的关键。
...
TL;DR: 结合StyleGAN、DragGAN和PCA，提升GAN生成图像的潜在空间效率和可控性，在AFHQ数据集上验证了方法在保持性能的同时提高优化效率。


<details>
  <summary>Details</summary>
Motivation: 旨在提高GAN生成图像潜在空间的效率和可控性，通过整合多种技术实现更高效、可解释的图像编辑。

Method: 集成StyleGAN提供结构化潜在空间，DragGAN实现直观图像操作，PCA进行降维和跨模型对齐，应用于AFHQ数据集。

Result: 在DragGAN的潜在W+层引入PCA可显著减少总优化时间，同时保持良好视觉质量，在浅层潜在空间(W+层=3)中甚至提升SSIM指标。
...
TL;DR: MinerU2.5是一个12亿参数的文档解析视觉语言模型，采用从粗到精的两阶段解析策略，在保持高计算效率的同时实现了最先进的识别精度。
...
Result: 在多个基准测试中达到最先进性能，超越了通用和领域特定模型在各种识别任务上的表现，同时保持显著较低的计算开销。

Conclusion: MinerU2.5展示了强大的文档解析能力，通过从粗到精的两阶段策略在计算效率和识别精度之间取得了良好平衡，为文档解析任务提供了高效解决方案。
...
TL;DR: 提出Geo-CoT框架，通过两阶段对齐策略训练RSThinker模型，实现可验证的多步骤遥感分析推理


<details>
  <summary>Details</summary>
Motivation: 解决现有视觉语言模型在遥感复杂分析任务中的局限性，特别是端到端训练模式导致推理步骤缺失和输出不可验证的问题

Method: 使用Geo-CoT380k数据集进行两阶段对齐：先通过监督微调建立认知架构，再通过组奖励策略优化提升推理准确性
...
Method: 利用预训练的2D分割模型生成多粒度2D掩码并提升到3D，通过语义熵和几何不透明度优化初始对象组的模糊边界，使用视觉语言模型从最具代表性的视角解释对象外观，提取鲁棒的文本特征。

Result: MUSplat将场景适应时间从数小时减少到几分钟，在开放词汇3D对象选择和语义分割基准任务中优于已建立的基于训练的框架，同时解决了它们的单语义限制。

Conclusion: 通过完全放弃特征优化，MUSplat提供了一个高效、无需训练且能处理复杂多概念语义的3D场景理解解决方案。
...
Result: 评估20个最先进的MLLMs，Gemini-2.5 Pro表现最佳，准确率接近人类专家水平，平均差距仅1.5%。大多数模型在基于场景理解的任务上表现良好，一些模型在像素级变化检测上甚至超过人类标注者，但在需要时间推理的任务上表现下降。

Conclusion: MLLMs在城市环境理解方面显示出潜力，特别是在场景理解和某些主观感知维度上能达到或超过人类水平，但在时间推理方面仍需改进。
...
TL;DR: 提出EXCL方法解决无源无监督域自适应问题，包含双专家框架和检索增强交互优化流程，在四个基准数据集上达到最先进性能
...
Method: EXCL包含双专家框架（冻结源域模型+预训练视觉语言模型）和RAIN三阶段优化流程（协同检索样本、分别微调专家、强制学习一致性）

Result: 在四个基准数据集上的广泛实验表明，该方法达到了最先进的性能

Conclusion: EXCL通过双专家协作和检索增强交互，有效解决了无源无监督域自适应问题
...
TL;DR: FlashEdit是一个实时图像编辑框架，通过三种创新技术实现高质量编辑：一步式反演编辑、背景保护和稀疏空间交叉注意力，速度比传统方法快150倍以上。
...
Method: 采用一步式反演编辑(OSIE)避免迭代计算，使用背景保护(BG-Shield)技术选择性修改编辑区域特征，并应用稀疏空间交叉注意力(SSCA)防止语义泄漏。

Result: FlashEdit在保持优异背景一致性和结构完整性的同时，编辑时间低于0.2秒，相比多步方法实现了150倍以上的加速。

Conclusion: FlashEdit框架成功解决了扩散模型图像编辑的延迟问题，实现了实时高质量编辑，为实际应用提供了可行解决方案。
...
Method: 开发了包含多序列MRI扫描、结构化电子健康记录和临床笔记的紧凑基准，涵盖鉴别诊断、病灶识别和推理生成三个核心任务家族，并使用混合评分管道进行评估。

Result: 对GPT-4o、Claude-4和MedGemma等最先进VLM的系统评估显示，与传统数据集相比性能显著下降，错误分析表明推理失败而非感知错误是主要问题。

Conclusion: 需要双轴评估框架：广度导向的大型数据集用于统计泛化，深度导向的紧凑基准（如Neural-MedBench）用于推理保真度评估，以开发临床可信的AI。
...
<details>
  <summary>Details</summary>
Motivation: 传统大规模地图构建方法依赖昂贵的专用数据采集车辆和人工标注，现有卫星方法存在遮挡、过时等固有缺陷，且感知方法向量化效率低导致道路不连续粗糙。

Method: 将车道线表示为离散序列并建立迭代生成策略；提出支持BEV、PV和文本提示的多模态输入灵活架构；开发状态更新策略保证全局连续性。

Result: 在OpenSatMap数据集上达到最先进性能，能够推断被遮挡道路并预测数据集中缺失的道路。

Conclusion: UniMapGen通过生成式方法和多模态输入有效解决了传统地图构建方法的效率问题和卫星数据的局限性，为大规模地图构建提供了创新解决方案。
...
Result: 重建结果与最先进方法相当，能够生成三角形网格及其相关材质组件，适用于下游任务。

Conclusion: 该方法在常用数据集上验证了有效性，与最先进的表面重建方法进行了定性比较，证明了其优越性能。
...
TL;DR: 提出了任务导向的桌面场景生成任务，并开发了MesaTask框架，通过空间推理链和DPO算法生成与任务描述一致且物理合理的3D桌面场景布局。
...
Method: 提出空间推理链，将生成过程分解为物体推理、空间关系推理和场景图构建；开发基于LLM的MesaTask框架，并采用DPO算法进行增强。

Result: 构建了包含约10,700个合成桌面场景的大规模数据集MesaTask-10K；实验表明MesaTask在生成符合任务要求且布局真实的桌面场景方面优于基线方法。

Conclusion: MesaTask框架能够有效弥合任务指令与桌面场景之间的差距，生成物理合理且与任务描述高度一致的3D布局。
...
TL;DR: 论文研究了基于规则的强化学习在文档图像分类任务中的应用，发现强化学习在分布外数据上具有更好的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 尽管DeepSeek-R1已证明基于规则的强化学习的成功，但在文档分析领域强化学习应用较少，而许多下游任务可能受益于强化学习增强的推理能力。

Method: 使用基于规则的强化学习方法，在文档图像分类任务上进行实验，考察三种分布外场景：分布外图像、未见类别和不同模态。

Result: 强化学习在分布外数据上表现出更好的泛化能力，在三种不同分布外场景中都取得了良好效果。
...
TL;DR: SceneSplit是一种针对文本到视频模型的黑盒越狱攻击方法，通过将有害叙述分解为多个看似良性的场景来绕过安全过滤器，在多个T2V模型上实现了高攻击成功率。


<details>
  <summary>Details</summary>
Motivation: 随着文本到视频模型的快速发展，其安全风险日益受到关注。虽然已有研究探索了LLMs、VLMs和文本到图像模型的越狱攻击，但T2V模型的安全性研究仍存在显著空白。

Method: 提出SceneSplit方法：1）将有害叙述分解为多个单独良性的场景；2）通过场景组合约束生成输出空间；3）采用迭代场景操纵绕过安全过滤器；4）使用策略库重用成功攻击模式。

Result: 在11个安全类别上评估，SceneSplit在Luma Ray2、Hailuo和Veo2模型上分别实现了77.2%、84.1%和78.2%的平均攻击成功率，显著优于现有基线方法。
...
TL;DR: 提出了一种基于动量的采样技术HiGS，通过整合近期模型预测来提升扩散模型的采样质量和效率，无需额外训练即可实现更高质量的图像生成。
...
Method: HiGS利用当前预测与过去预测加权平均值之间的差异来引导采样过程，无需额外计算，可无缝集成到现有扩散框架中。

Result: 实验表明HiGS在不同模型和架构下均能提升图像质量，使用预训练SiT模型在30步采样下达到ImageNet 256×256无引导生成的SOTA FID 1.61。

Conclusion: HiGS是一种即插即用的扩散采样增强方法，能够实现更快速度、更高保真度的图像生成。
...
TL;DR: VeloxSeg提出了一种轻量级3D医学图像分割方法，通过双流CNN-Transformer架构、模态交互和知识迁移技术，在保持高效性的同时显著提升了分割性能。
...
Method: 采用可部署的双流CNN-Transformer架构，包含配对窗口注意力(PWA)和Johnson-Lindenstrauss引理指导的卷积(JLC)；引入"扫视-聚焦"原则进行多尺度信息检索；通过模态交互和空间解耦知识迁移(SDKT)增强表示能力。

Result: 在多模态基准测试中，VeloxSeg实现了26%的Dice系数提升，GPU吞吐量提高11倍，CPU吞吐量提高48倍。

Conclusion: VeloxSeg通过创新的架构设计和知识迁移策略，成功解决了轻量级3D医学图像分割的效率与鲁棒性平衡问题，为实际部署提供了高效可靠的解决方案。
...
TL;DR: NIFTY是一个结合扩散模型和传统基于块的纹理优化技术的混合框架，用于示例纹理合成，无需神经网络训练且能避免传统块方法的常见问题。


<details>
  <summary>Details</summary>
Motivation: 解决示例纹理合成问题，结合扩散模型和传统块方法的优势，避免神经网络训练需求，同时改善传统块方法的初始化问题和视觉伪影。

Method: 基于非局部块匹配的非参数流匹配模型，结合卷积神经网络训练的扩散模型洞察和经典基于块的纹理优化技术。

Result: 实验结果表明，与文献中的代表性方法相比，所提出的方法具有有效性。

Conclusion: NIFTY框架在示例纹理合成中表现出色，代码已开源。
...
TL;DR: RAPID3是一个无需训练扩散变换器(DiTs)的加速框架，通过三个轻量级策略头实现图像级自适应加速，达到近3倍采样速度提升且保持生成质量。


<details>
  <summary>Details</summary>
Motivation: 现有DiTs加速方法存在局限性：训练无关的加速器使用统一启发式策略导致质量损失，而动态神经网络需要高昂微调成本。需要零更新基础生成器的自适应加速方案。

Method: 提出三层次强化加速策略：Step-Skip、Cache-Reuse和Sparse-Attention三个策略头独立决策加速操作；使用Group Relative Policy Optimization在线训练策略参数；通过对抗学习鉴别器增强奖励信号防止奖励攻击。

Result: 在包括Stable Diffusion 3和FLUX在内的最先进DiT骨干网络上，RAPID3实现了近3倍的采样加速，同时保持竞争力的生成质量。

Conclusion: RAPID3框架成功实现了无需更新基础生成器的图像级自适应加速，在保持生成质量的同时显著提升采样效率，为DiTs的实际应用提供了可行的加速解决方案。
...
Method: 构建多模态知识图谱来挖掘局部视觉特征与文本之间的关系，以及属性与广泛视觉上下文样本之间的关系，引入知识图谱引导的跨模态超图学习框架

Result: 在多个PAR基准数据集上的综合实验充分证明了所提知识图谱的有效性，为知识引导的行人属性识别奠定了坚实基础

Conclusion: 提出的多模态知识图谱方法能够有效提升行人属性识别性能，为PAR任务提供了新的知识引导解决方案
...
TL;DR: CircuitSense是一个评估多模态大语言模型在电路理解方面能力的综合基准，包含8,006+个问题，涵盖从组件级原理图到系统级框图的完整工程工作流程。


<details>
  <summary>Details</summary>
Motivation: 工程设计需要从系统规范到组件实现的层次化抽象，涉及视觉理解和数学推理的结合。虽然多模态大语言模型在自然图像任务上表现出色，但其从技术图表中提取数学模型的能力尚未被探索。

Method: 提出了一个层次化合成生成管道，包括基于网格的原理图生成器和带有自动推导符号方程标签的框图生成器。对6个最先进的多模态大语言模型进行了全面评估。

Result: 闭源模型在涉及组件识别和拓扑识别的感知任务上达到85%以上的准确率，但在符号推导和分析推理方面的性能低于19%，揭示了视觉解析和符号推理之间的关键差距。

Conclusion: 具有更强符号推理能力的模型在设计任务上始终获得更高准确率，确认了数学理解在电路合成中的基础作用，并将符号推理确立为工程能力的关键指标。
...
TL;DR: HierLight-YOLO是一个基于YOLOv8的层次特征融合轻量级模型，专门用于无人机摄影中的小目标实时检测，通过HEPAN特征融合和轻量化模块在保持检测性能的同时显著降低计算复杂度。


<details>
  <summary>Details</summary>
Motivation: 解决无人机摄影场景中小目标检测（<32像素）的挑战，现有YOLO系列检测器在小目标占主导的无人机检测中假阴性率较高，需要同时满足实时性和资源受限平台的需求。

Method: 提出层次扩展路径聚合网络（HEPAN）进行多尺度特征融合，设计倒置残差深度卷积块（IRDCB）和轻量下采样模块（LDown）降低模型复杂度，并专门设计小目标检测头提升微小目标（4像素）检测能力。

Result: 在VisDrone2019基准测试上的比较实验和消融研究证明了HierLight-YOLO的最先进性能。

Conclusion: HierLight-YOLO通过层次特征融合和轻量化设计，在无人机小目标检测任务中实现了优异的实时检测性能，显著降低了模型参数和计算复杂度。
...
Method: 采用GPT-4o模型，开发了优化的提示工程技术和结构化多模态分析框架，包括图像和文本预处理方法，定义了六项具体评估标准，并引入了基于置信度的自评估机制。
...
Method: 通过专门设计的提示词配合输入图像，让GPT-4分析图像并生成顺序预测，然后解析响应构建遮挡矩阵

Result: 在COCOA和InstaOrder数据集上的评估结果显示，通过利用语义上下文、视觉模式和常识知识，模型能产生更准确的顺序预测
...
### [127] [Vision-Language Alignment from Compressed Image Representations using 2D Gaussian Splatting](https://arxiv.org/abs/2509.22615)
*Yasmine Omri,Connor Ding,Tsachy Weissman,Thierry Tambe*

Main category: cs.CV

TL;DR: 该论文探索使用2D高斯泼溅(2DGS)作为视觉语言模型的替代视觉表示，相比传统RGB像素，能显著压缩输入数据(3-20倍)并减少序列长度，同时保持有意义的零样本性能。


<details>
  <summary>Details</summary>
Motivation: 解决现有视觉语言模型的两个结构性问题：(i) 从边缘设备传输密集RGB图像到云端能耗高、成本大；(ii) 基于patch的tokenization导致序列长度爆炸，给注意力机制和上下文限制带来压力。

Method: 开发可扩展的2DGS流水线，包括结构化初始化、亮度感知剪枝和批量CUDA内核，实现90倍以上的加速。通过轻量级splat感知输入主干和感知器重采样器，将CLIP适配到2DGS表示，仅训练约7%的总参数。

Result: 在大型DataComp子集上，GS编码器在压缩输入3-20倍的同时，仍能获得有意义的零样本ImageNet-1K性能。虽然准确率目前仍落后于RGB编码器，但证明了2DGS作为多模态基底的可行性。

Conclusion: 2DGS被确立为可行的多模态基底，识别了架构瓶颈，并为开发既语义强大又传输高效的表征开辟了路径，特别适用于边缘-云端学习场景。
...
TL;DR: 提出基于显著边界增强的多焦点图像融合方法，通过梯度域模型和Tenengrad梯度检测生成高质量融合边界，在四个公开数据集上优于12种先进方法。
...
Conclusion: 提出的方法能有效增强边界质量，生成高质量的多焦点融合图像。
...
TL;DR: Labeling Copilot是首个用于计算机视觉的数据策展深度研究代理，通过多模态语言模型驱动的中央编排器，执行校准发现、可控合成和共识标注三大核心功能，有效解决大规模数据标注的瓶颈问题。


<details>
  <summary>Details</summary>
Motivation: 构建高质量领域特定数据集是部署稳健视觉系统的主要瓶颈，需要在数据质量、多样性和成本之间进行复杂权衡，特别是在处理海量未标注数据时。

Method: 使用多模态语言模型驱动的中央编排器进行多步推理，执行三个核心功能：校准发现从大型存储库中获取相关数据；可控合成生成稀有场景的新数据；共识标注通过结合非极大值抑制和投票的新颖共识机制协调多个基础模型产生准确标签。

Result: 在COCO数据集上平均每张图像产生14.2个候选提案（接近真实对象7.4个的两倍），最终标注mAP达到37.1%；在Open Images数据集上发现了903个新的边界框类别，总能力扩展到1500多个；校准发现在1000万样本规模测试中，计算效率比替代方案高40倍。
...
### [130] [Text Adversarial Attacks with Dynamic Outputs](https://arxiv.org/abs/2509.22393)
*Wenqiang Wang,Siyuan Liang,Xiao Yan,Xiaochun Cao*

Main category: cs.CV

TL;DR: 提出了TDOA方法，通过聚类训练替代模型将动态输出场景转换为静态单输出场景，采用最远标签目标攻击策略，在动态和静态输出场景下都取得了优异的攻击效果。


<details>
  <summary>Details</summary>
Motivation: 现有文本对抗攻击方法主要针对静态场景，无法处理动态输出标签空间的问题，需要设计能够适应动态输出场景的攻击方法。

Method: 使用基于聚类的替代模型训练方法将动态输出场景转换为静态单输出场景，并提出最远标签目标攻击策略选择偏离模型粗粒度标签的对抗向量。

Result: 在4个数据集和8个受害者模型上验证，单次查询最大攻击成功率达50.81%，在静态场景下最大ASR达82.68%，在生成式设置下超越先前结果0.64 RDBLEU和0.62 RDchrF。

Conclusion: TDOA方法有效解决了动态输出场景下的文本对抗攻击问题，在有限访问条件下具有破坏大型语言模型的强大潜力。
...
### [131] [Learning Human-Perceived Fakeness in AI-Generated Videos via Multimodal LLMs](https://arxiv.org/abs/2509.22646)
...
TL;DR: DeeptraceReward是首个细粒度、时空感知的基准数据集，用于标注人类感知的AI生成视频痕迹，包含4.3K详细标注，训练的多模态奖励模型在识别、定位和解释方面优于GPT-5 34.7%。
...
Method: 构建包含4.3K详细标注的数据集，每个标注提供自然语言解释、边界框区域和精确时间戳，训练多模态语言模型作为奖励模型模仿人类判断和定位。

Result: 7B奖励模型在伪造线索识别、定位和解释方面平均优于GPT-5 34.7%，观察到一致的难度梯度：二元分类最容易，时间标注最难。
...
### [132] [Integrating Background Knowledge in Medical Semantic Segmentation with Logic Tensor Networks](https://arxiv.org/abs/2509.22399)
*Luca Bergamin,Giovanna Maria Dimitri,Fabio Aiolli*

Main category: cs.CV

TL;DR: 本文提出使用逻辑张量网络（LTNs）将医学背景知识编码到语义分割模型的损失函数中，以提升医学图像分割性能，特别是在训练数据稀缺时效果更明显。


<details>
  <summary>Details</summary>
Motivation: 当前基于深度学习的医学图像语义分割系统在存在噪声和伪影时仍有改进空间，通过将医学先验知识融入模型可以提升分割性能。

Method: 使用逻辑张量网络（LTNs）通过一阶逻辑规则编码医学背景知识，包括分割形状约束和不同分割区域间的关系，并与SwinUNETR结合构建端到端的分割框架。

Result: 在海马体脑部MRI扫描分割任务上的实验表明，LTNs能够提升基线分割性能，特别是在训练数据稀缺的情况下效果更显著。

Conclusion: 尽管处于初步阶段，神经符号方法具有足够的通用性，可以适应并应用于其他医学语义分割任务。
...
### [133] [CapRL: Stimulating Dense Image Caption Capabilities via Reinforcement Learning](https://arxiv.org/abs/2509.22647)
*Long Xing,Xiaoyi Dong,Yuhang Zang,Yuhang Cao,Jianze Liang,Qidong Huang,Jiaqi Wang,Feng Wu,Dahua Lin*

Main category: cs.CV

TL;DR: 提出CapRL框架，使用强化学习和可验证奖励机制来改进图像描述生成，通过非视觉语言模型评估描述质量，在多个基准测试中取得显著提升


<details>
  <summary>Details</summary>
Motivation: 当前图像描述模型依赖监督微调，需要昂贵的人工标注数据，容易过拟合特定答案，缺乏泛化能力和多样性

Method: 采用两阶段强化学习框架：LVLM生成描述，然后用独立的无视觉LLM基于描述回答选择题，根据回答准确率计算奖励

Result: 在12个基准测试中取得显著提升，在Prism评估框架中性能接近Qwen2.5-VL-72B，平均比基线提高8.4%

Conclusion: CapRL证明了强化学习在主观性图像描述任务中的有效性，提供了一种可扩展的训练方法，减少对人工标注的依赖
...
TL;DR: 提出VARE框架和S-VARE方法，专门用于视觉自回归模型的稳定概念擦除，通过辅助视觉令牌减少微调强度，使用过滤交叉熵损失精确调整不安全令牌，同时保持语义保真度。


<details>
  <summary>Details</summary>
Motivation: 视觉自回归模型在文本到图像生成方面取得快速进展，但带来了安全隐患。现有的概念擦除技术主要针对扩散模型，无法适用于自回归模型的下一尺度令牌预测范式。

Method: 提出VARE框架利用辅助视觉令牌减少微调强度，并引入S-VARE方法，包含过滤交叉熵损失来精确识别和最小化调整不安全视觉令牌，以及保持语义保真度的保护损失。

Result: 大量实验证明该方法能够实现精确的概念擦除，同时保持生成质量，解决了早期方法在自回归文本到图像生成中的安全漏洞。

Conclusion: 该方法有效解决了自回归模型中的概念擦除问题，填补了现有方法在安全方面的空白，实现了精确擦除而不损害生成质量。
...
TL;DR: RAU是一个基于参考图像的解剖结构理解框架，结合视觉语言模型(VLMs)和SAM2分割模型，能够在医学图像中实现解剖区域的识别、定位和像素级分割。


<details>
  <summary>Details</summary>
Motivation: 医学图像中的解剖理解对于自动报告生成、术中导航和器官定位至关重要，但专家标注数据的稀缺限制了进展。利用标注参考图像来指导未标注目标图像的解释是一个有前景的解决方案。

Method: 首先训练VLM通过参考图像和目标图像之间的相对空间推理来识别解剖区域，然后将其空间线索与SAM2的细粒度分割能力相结合，实现小解剖区域的定位和分割。

Result: 在两个分布内和两个分布外数据集上，RAU始终优于使用相同内存设置的SAM2微调基线，产生更准确的分割和更可靠的定位。

Conclusion: RAU是首个探索VLMs在医学图像中基于参考的解剖结构识别、定位和分割能力的方法，其强大泛化能力使其可扩展到分布外数据集，在自动化临床工作流程中具有重要潜力。
...
### [136] [Explaining multimodal LLMs via intra-modal token interactions](https://arxiv.org/abs/2509.22415)
*Jiawei Liang,Ruoyu Chen,Xianghao Jiao,Siyuan Liang,Shiming Liu,Qunli Zhang,Zheng Hu,Xiaochun Cao*

Main category: cs.CV

TL;DR: 本文提出了一种增强多模态大语言模型可解释性的方法，通过视觉分支的多尺度解释聚合和文本分支的激活排序相关性，解决了现有方法忽视模态内依赖性的问题，生成了更忠实和细粒度的模型行为解释。


<details>
  <summary>Details</summary>
Motivation: 现有可解释性研究主要关注跨模态归因，识别模型在输出生成时关注的图像区域，但往往忽视了模态内依赖关系。视觉模态中，对孤立图像块的重要性归因由于有限感受野而忽略了空间上下文，导致解释碎片化和噪声；文本模态中，对前序令牌的依赖引入了虚假激活。

Method: 提出了利用模态内交互增强可解释性的方法：对于视觉分支，引入多尺度解释聚合，通过多尺度输入聚合归因来动态调整感受野，产生更全面和空间一致的视觉解释；对于文本分支，提出激活排序相关性，通过对齐前k个预测排名来衡量上下文令牌与当前令牌的相关性，抑制无关上下文的虚假激活同时保留语义连贯的激活。

Result: 在最先进的多模态大语言模型和基准数据集上的广泛实验表明，该方法一致优于现有的可解释性方法。

Conclusion: 该方法能够生成更忠实和细粒度的模型行为解释，有效解决了现有方法在模态内依赖性方面的局限性。
...
<details>
  <summary>Details</summary>
Motivation: 深度伪造检测器由于从有限训练数据中学习到的偏差，难以泛化到新的伪造类型。本文发现了一种新的频率域模型偏差——频谱偏差，即检测器过度依赖特定频带，限制了其跨未见伪造类型的泛化能力。

Method: 提出FreqDebias框架：1）Fo-Mixup增强，动态多样化训练样本的频率特性；2）双重一致性正则化，通过类激活图的局部一致性和超球面嵌入空间中的von Mises-Fisher分布的全局一致性，促进一致表示学习。
...
TL;DR: LucidFlux是一个无需图像描述的通用图像修复框架，通过适配大型扩散变换器(Flux.1)实现。它引入轻量级双分支条件器来注入退化输入和轻度修复代理的信号，采用时间步和层自适应调制调度，并通过SigLIP特征实现无描述语义对齐。


<details>
  <summary>Details</summary>
Motivation: 解决通用图像修复中判别性修复器和UNet-based扩散先验经常过度平滑、产生幻觉或漂移的问题，特别是在未知混合退化条件下需要保持语义的场景。

Method: 1) 轻量级双分支条件器注入退化输入和轻度修复代理信号；2) 时间步和层自适应调制调度；3) 通过SigLIP特征实现无描述语义对齐；4) 可扩展的数据筛选流程。
...
TL;DR: 提出了U-MAN架构，通过渐进注意力引导特征融合和多尺度自适应KAN模块，解决了医学图像分割中细节丢失和边界模糊的问题，在多个数据集上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 传统U-Net架构存在两个主要限制：简单跳跃连接忽略了编码器-解码器之间的语义差距，以及缺乏深层多尺度特征提取能力，导致医学图像分割中精细细节和精确边界难以保持。

Method: 提出U-MAN架构，包含两个核心模块：PAGF模块用注意力机制替代简单跳跃连接，融合编码器和解码器特征；MAN模块使网络能够自适应处理多尺度特征，提升不同大小对象的分割能力。

Result: 在BUSI、GLAS和CVC三个公开数据集上的实验表明，U-MAN优于最先进方法，特别是在定义准确边界和保留精细细节方面表现突出。

Conclusion: U-MAN通过创新的注意力引导特征融合和多尺度自适应处理，有效解决了医学图像分割中的关键挑战，为复杂解剖结构和病理区域的分割提供了更优的解决方案。
...
TL;DR: 提出了一种名为γ-Quant的任务特定非线性量化方法，用于在低带宽和能量受限环境下的模式识别任务，在仅使用4位量化数据的情况下能达到与12位原始数据相当的性能。


<details>
  <summary>Details</summary>
Motivation: 传统模式识别模型使用预处理数据（如RGB图像或归一化传感器数据），但这些预处理方式可能不适用于自动化分析场景，且在高位ADC数据传输时能耗效率低下。

Method: 针对低比特深度采集的传感器数据，提出γ-Quant方法，通过学习任务特定的非线性量化函数来优化模式识别性能。

Result: 在原始图像目标检测和可穿戴设备人体活动识别任务中，使用可学习量化的4位原始数据与12位原始数据的性能相当。

Conclusion: γ-Quant方法在保持性能的同时显著降低了数据传输需求，为低带宽和能量受限环境下的模式识别应用提供了有效解决方案。
...
TL;DR: 提出了一种自监督的可见光-红外图像融合框架SSVIF，通过跨分割一致性任务实现无需分割标签的语义特征学习，在公开数据集上表现优异。
...
Result: 在公开数据集上，仅使用未标注的可见光-红外图像对训练，SSVIF超越了传统VIF方法，与有监督的分割导向方法表现相当。

Conclusion: 提出的自监督框架SSVIF有效解决了应用导向VIF方法的数据标注问题，为可见光-红外图像融合提供了一种无需下游任务标签的高效解决方案。
...
TL;DR: 提出了Bézier Meets Diffusion统一框架，通过Bézier曲线风格迁移和条件扩散模型生成高质量带标签的跨域医学图像，显著提升分割性能。


<details>
  <summary>Details</summary>
Motivation: 解决医学影像跨模态训练中因域差异大导致的鲁棒性问题，现有GAN方法在高度可变区域难以捕获跨域映射。

Method: 1. Bézier曲线风格迁移减少域差异；2. 用分割模型生成目标域伪标签；3. 条件扩散模型合成高质量带标签图像；4. 不确定性引导的分数匹配提高鲁棒性。

Result: 在公开数据集上的实验表明，该方法能生成逼真的带标签图像，显著增强目标域数据并提升分割性能。

Conclusion: Bézier Meets Diffusion框架有效解决了医学影像跨域适应问题，通过风格迁移和扩散模型的结合实现了高质量的跨域图像生成和分割性能提升。
...
<details>
  <summary>Details</summary>
Motivation: 现有的事件处理方法将事件流转换为事件帧序列，但忽略了事件帧序列的高空间稀疏性和帧间运动冗余，导致计算开销大。现有的RGB视频令牌稀疏化方法依赖不可靠的中间令牌表示且忽略事件噪声影响，不适用于事件数据

Method: PSTTS包含两个阶段：空间令牌净化（通过评估事件帧内事件的时空一致性来丢弃噪声和非事件区域）和时间令牌选择（通过评估相邻事件帧间的运动模式相似性来识别和移除冗余时间信息）

Result: 在HARDVS、DailyDVS-200和SeACT数据集上的实验表明，PSTTS显著提升效率：在DailyDVS-200数据集上减少FLOPs 29-43.6%，提升FPS 21.6-41.3%，同时保持任务精度

Conclusion: PSTTS是一个即插即用模块，无需额外参数，能够有效处理事件数据的时空冗余问题，实现精度与效率的最佳平衡
...
TL;DR: 提出了GCPO方法，通过识别图像生成中的关键token并进行针对性优化，在仅使用30% token的情况下超越了全token优化的GRPO方法。


<details>
  <summary>Details</summary>
Motivation: 现有方法对所有图像token进行均匀优化，但不同token对RLVR训练的贡献差异未被探索。关键挑战在于如何在自回归生成中识别关键token并实现有效的token级优化。

Method: 从三个角度识别关键token：因果依赖、熵诱导的空间结构、RLVR聚焦的token多样性。对关键token引入动态token级优势权重来鼓励探索。

Result: 在多个文本到图像基准测试中，GCPO在AR模型和统一多模态模型上都表现出有效性，仅使用30% token就超越了全token优化的GRPO。

Conclusion: GCPO通过识别和优化关键token，实现了更有效的策略优化，为自回归视觉生成提供了新的优化思路。
...
Conclusion: EAGLE在提升MLLMs可解释性方面具有有效性和实用性，为理解模型决策提供了细粒度的可解释性。
...
Result: 视觉语言模型在典型颜色上准确率高，但在扩展的非典型颜色集上表现显著下降；识别出21个在所有模型中一致的常见颜色术语；发现语言模型架构独立于视觉处理能力显著影响颜色命名。
...
TL;DR: 提出EfficientDepth系统，结合transformer架构与轻量卷积解码器，通过双模态密度头和LPIPS损失函数实现高效单目深度估计，在保持几何一致性和细节的同时显著降低计算资源需求。
...
Method: 结合transformer架构与轻量卷积解码器，使用双模态密度头估计详细深度图，采用多阶段优化策略，引入基于LPIPS的损失函数，在合成和真实图像混合数据集上训练。

Result: EfficientDepth在性能上达到或超过现有最先进模型，同时显著减少计算资源需求。

Conclusion: 该方法成功解决了单目深度估计中的关键挑战，在保持高质量结果的同时实现了计算效率的提升。
...
TL;DR: HERMAN方法通过利用LLM生成层次化文本描述符，增强CLIP在类增量学习中的语义空间，结合多层级特征匹配来提升分类精度并缓解灾难性遗忘。


<details>
  <summary>Details</summary>
Motivation: 现有基于CLIP的类增量学习方法使用简单模板（如"a photo of a [CLASS]"），忽略了视觉概念的层次性，且仅使用最后一层特征，未能充分利用CLIP中的层次信息。

Method: 提出HERMAN方法：1）使用LLM递归生成区分性文本描述符；2）将描述符匹配到语义层次的不同级别；3）基于任务需求自适应路由，实现精确区分。

Result: 在多个基准测试上的广泛实验表明，该方法持续达到最先进的性能。

Conclusion: HERMAN通过层次化表示匹配有效提升了CLIP在类增量学习中的表现，缓解了灾难性遗忘问题。
...
TL;DR: 这篇论文是关于类别发现（CD）的综述研究，这是一个新兴的开放世界学习任务，旨在自动对包含未见类别的未标记数据进行分类，同时利用来自已见类别的标记数据。


<details>
  <summary>Details</summary>
Motivation: 类别发现任务近年来受到广泛关注，产生了大量文献从不同角度解决该问题。作者旨在提供全面的文献综述，进行详细分析和深入讨论。

Method: 作者首先介绍了文献分类法，考虑两种基础设置（新颖类别发现NCD和广义类别发现GCD）以及几种衍生设置。然后对每种设置的方法进行详细分析，涵盖表示学习、标签分配和类别数量估计三个基本组成部分。最后对所有方法进行基准测试并提炼关键见解。

Result: 研究发现大规模预训练骨干网络、层次化和辅助线索以及课程式训练都有利于类别发现，但在标签分配设计、类别数量估计以及扩展到复杂多目标场景方面仍存在挑战。

Conclusion: 作者讨论了迄今为止文献中的关键见解，并指出了有前景的未来研究方向。他们还维护了一个关于类别发现文献的动态调查网站。
...
TL;DR: HyCoVAD是一个混合SSL-LLM模型，通过结合自监督学习和大型语言模型来解决复杂视频异常检测问题，在ComplexVAD数据集上达到72.5%的帧级AUC，比现有基线提升12.5%且减少LLM计算量。


<details>
  <summary>Details</summary>
Motivation: 解决复杂视频异常检测中识别由多个实体间复杂关系和时序依赖定义的异常事件的挑战，传统SSL方法难以理解语义交互，而LLM虽然推理能力强但计算成本高且缺乏细粒度空间定位。

Method: 采用混合SSL-LLM架构：SSL模块基于nnFormer骨干网络，通过多任务代理学习识别可疑异常帧；LLM验证器对选定帧应用结构化、基于规则的推理来验证异常存在。

Result: 在ComplexVAD数据集上实现72.5%的帧级AUC，比现有基线方法提升12.5%，同时显著减少了LLM的计算负担。

Conclusion: HyCoVAD成功结合了SSL的低级时空模式建模能力和LLM的语义上下文推理优势，为复杂视频异常检测提供了有效的解决方案，并发布了交互异常分类法、自适应阈值协议和代码以促进未来研究。
...
Method: 扩展MLLM以融合空间几何编码器的3D先验知识，增强仅基于RGB输入的空间推理能力。构建空间几何和视觉语义编码器的历史键值缓存作为双隐式记忆，仅保留初始和滑动窗口中的token KVs，避免冗余计算，实现高效增量更新。

Result: 在广泛实验中，JanusVLN优于20多种最新方法，达到SOTA性能。相比使用多种数据类型输入的方法，成功率提高10.5-35.5；相比使用更多RGB训练数据的方法，成功率提高3.6-10.8。
...
TL;DR: SpikeMatch是首个面向脉冲神经网络的半监督学习框架，利用SNN的时序动态特性通过协同训练生成多样化伪标签，在有限标签下有效提升性能。


<details>
  <summary>Details</summary>
Motivation: 脉冲神经网络具有生物合理性和能效优势，但其半监督学习方法相比人工神经网络仍未被充分探索。

Method: 利用SNN泄漏因子的时序动态特性，在协同训练框架中通过单个SNN的多个预测一致性生成可靠伪标签，从弱增强无标签样本训练强增强样本。
...
<details>
  <summary>Details</summary>
Motivation: 解决长视频生成在效率和交互性方面的挑战：扩散模型效率低，因果注意力模型在长视频上质量下降，同时需要支持实时交互式提示输入以进行动态内容创作。

Method: 采用因果帧级自回归设计，集成KV重缓存机制实现平滑提示切换，流式长调优对齐训练和推理，短窗口注意力配合帧级注意力汇聚保持长程一致性。

Result: 仅用32 GPU天将1.3B参数的短片段模型微调至分钟级生成，在单H100 GPU上保持20.7 FPS，支持240秒视频生成，INT8量化推理仅有轻微质量损失。

Conclusion: LongLive成功解决了长视频生成的效率和质量挑战，支持实时交互式生成，在VBench基准测试中表现出色。
...
TL;DR: SPARK框架是一种高效的协同演化方法，通过回收RLVR中的rollouts和正确性数据来同时训练模型作为生成式奖励模型，无需单独奖励模型或人类偏好数据。


<details>
  <summary>Details</summary>
Motivation: 解决RLHF成本高和奖励-策略不匹配问题，以及RLVR浪费监督信息的问题。

Method: 在RLVR基础上，回收rollouts和正确性数据，使用点式奖励评分、成对比较和基于反思响应的评估等目标来训练模型自我评估和改进。

Result: SPARK-VL-7B在7个推理基准上平均提升9.7%，2个奖励基准上提升12.1%，8个通用基准上提升1.5%。

Conclusion: SPARK创建了正反馈循环，支持测试时自我反思扩展，在多个模型和基准上表现出鲁棒性和广泛泛化能力。
...
<details>
  <summary>Details</summary>
Motivation: 深度估计在机器人、自动驾驶等应用中很重要，但受计算资源限制。立体图像对提供有效解决方案，但获取可靠深度真值数据困难，因此需要自监督方法

Method: CCNeXt架构使用现代CNN特征提取器和新颖的窗口化极线交叉注意力模块，并重新设计了深度估计解码器

Result: 在KITTI Eigen Split测试数据上达到竞争性指标，比当前最佳模型快10.18倍，在KITTI Eigen Split改进真值和Driving Stereo数据集上所有指标均达到最先进水平

Conclusion: CCNeXt在深度估计任务中实现了性能与计算效率的良好平衡，为资源受限场景提供了有效解决方案
...
TL;DR: UML-CoT是一个利用统一建模语言(UML)的结构化推理和规划框架，通过类图捕捉对象语义，活动图建模控制流程，在杂乱房间清洁任务中优于非结构化思维链。


<details>
  <summary>Details</summary>
Motivation: 传统思维链提示依赖非结构化文本，在具身任务中可解释性和可执行性有限。现有结构化方法只能建模低阶关系，缺乏继承、行为抽象等构造，也没有顺序或条件规划的标准语义。
...
Conclusion: UML作为一种更具表达力和可操作性的结构化推理形式，能够有效提升推理和规划能力。
...
TL;DR: DIPSY提出了一种无需训练的少样本图像分类方法，利用IP-Adapter进行图像到图像转换，通过正负图像条件控制和类相似性采样策略生成判别性合成图像。
...
Method: 使用IP-Adapter进行图像到图像转换，引入扩展的无分类器引导方案、类相似性采样策略，构建无需微调的简单流程。

Result: 在十个基准数据集上达到最先进或可比性能，无需生成模型适应或外部工具依赖。

Conclusion: 通过正负引导的双图像提示能有效生成类判别特征，特别适用于细粒度分类任务。
...
TL;DR: 本文重新审视了自回归视觉生成模型VAR，发现当配备马尔可夫注意力掩码时，VAR在数学上等价于离散扩散模型，提出了SRDD框架，将扩散模型的优势引入VAR，提高了效率和生成质量。


<details>
  <summary>Details</summary>
Motivation: 自回归变换器在视觉生成中表现出色，但存在架构效率问题。本文旨在通过揭示VAR与离散扩散模型的数学等价性，将扩散模型的优势（如迭代优化）引入VAR。

Method: 提出SRDD框架，通过马尔可夫注意力掩码将VAR重新解释为离散扩散模型，从而可以直接引入扩散模型的迭代优化方法，减少架构冗余。

Result: 实验表明，这种基于扩散的视角使VAR在多个数据集上实现了更快的收敛、更低的推理成本和更好的零样本重建能力，生成质量得到一致提升。

Conclusion: 通过建立自回归变换器与扩散模型的数学联系，SRDD框架成功将扩散模型的优势整合到VAR中，显著提升了视觉生成的效率和性能。
...
TL;DR: 提出RefAM框架，无需训练即可利用扩散变换器的注意力特征进行指代分割，通过处理全局注意力汇和注意力重分配策略，在零样本设置下超越现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有指代分割方法需要微调或多模型组合，而大规模生成扩散模型编码了丰富语义信息，可作为通用特征提取器直接利用。

Method: 利用扩散变换器的注意力特征，识别并处理全局注意力汇，提出注意力重分配策略，将背景激活分割成更小簇，构建无需训练的RefAM框架。

Result: 在零样本指代图像和视频分割基准测试中，该方法持续优于先前方法，无需微调或额外组件即达到新的最先进水平。

Conclusion: 扩散变换器的注意力特征可直接用于下游任务，通过适当的注意力处理策略，无需训练即可实现强大的指代分割性能。
...
TL;DR: 提出双模型架构检测大语言模型幻觉，发现幻觉信号集中在高度稀疏的特征子集中，通过仅使用1%特征维度即可维持检测性能。


<details>
  <summary>Details</summary>
Motivation: 大语言模型幻觉是重要挑战，现有研究虽发现特定隐藏层在幻觉和事实内容间存在差异，但幻觉信号在层内的精确定位仍不明确，限制了高效检测方法的发展。

Method: 采用双模型架构，包含投影融合块进行自适应层间特征加权，以及差分特征学习机制，通过计算并行编码器学习互补表示之间的差异来识别判别性特征。

Result: 在HaluEval的问答、对话和摘要数据集上实验表明，幻觉信号集中在高度稀疏的特征子集中，在问答和对话任务上取得显著准确率提升。分析揭示了层级'漏斗模式'，浅层特征多样性高，深层特征使用集中。

Conclusion: 幻觉信号比先前假设的更加集中，为开发计算高效的检测系统提供了途径，可在保持准确性的同时降低推理成本。
...
TL;DR: 本文提出了一种基于上下文影响力值(CI值)的检索增强生成(RAG)上下文选择方法，通过量化移除每个上下文对性能的影响来评估上下文质量，无需复杂超参数调优。


<details>
  <summary>Details</summary>
Motivation: 现有RAG方法因检索到的上下文质量差(包含无关或噪声信息)而效果受限，传统基于预定义质量评估指标的上下文选择方法对标准RAG改进有限，未能充分利用查询、上下文列表和生成器的综合信息。

Method: 将上下文质量评估重新定义为推理时数据估值问题，引入CI值度量，通过移除每个上下文导致的性能下降来量化质量；开发参数化代理模型预测CI值，采用分层架构捕获局部查询-上下文相关性和全局上下文间交互。

Result: 在8个NLP任务和多个LLM上的广泛实验表明，该方法显著优于现有最优基线，能有效过滤低质量上下文同时保留关键信息。

Conclusion: CI值方法通过整合查询感知相关性、列表感知独特性和生成器感知对齐，实现了更全面的上下文质量评估，为RAG系统提供了有效的上下文选择解决方案。
...
### [162] [A State-of-the-Art SQL Reasoning Model using RLVR](https://arxiv.org/abs/2509.21459)
...
TL;DR: 该论文提出了一种基于强化学习的自定义推理模型训练方法，应用于BIRD数据库查询基准测试，在无需额外训练数据或专有模型的情况下达到了最先进的准确率。
...
Method: 采用简单的通用训练方案：精心设计的提示和模型选择、使用离线RL方法TAO进行预热阶段、严格的在线RLVR训练。

Result: 在BIRD基准测试中，首次提交就达到了私有测试集的最先进准确率：不使用自一致性时为73.56%，使用自一致性时为75.68%，且所需生成次数少于第二好的方法。
...
### [163] [Context Is What You Need: The Maximum Effective Context Window for Real World Limits of LLMs](https://arxiv.org/abs/2509.21361)
*Norman Paulsen*

Main category: cs.CL

TL;DR: 论文提出了最大有效上下文窗口(MECW)概念，发现实际有效上下文窗口远小于厂商宣称的最大上下文窗口(MCW)，且随问题类型变化，大多数模型在1000个token内准确率就严重下降。


<details>
  <summary>Details</summary>
Motivation: 测试语言模型在实际使用中的上下文窗口有效性，揭示厂商宣称的最大上下文窗口与实际有效使用窗口之间的显著差异。

Method: 1) 定义最大有效上下文窗口概念 2) 制定测试方法评估不同大小和问题类型的上下文窗口有效性 3) 创建标准化比较方法寻找失效点，收集数十万个数据点测试多个模型。

Result: 发现MECW与MCW存在显著差异，且随问题类型变化。部分顶级模型在仅100个token时就失效，大多数模型在1000个token内准确率严重下降，所有模型的有效窗口都比宣称窗口小99%。

Conclusion: 最大有效上下文窗口随问题类型变化，这为提高模型准确率和减少幻觉提供了明确可行的改进方向。
...
### [164] [How Large Language Models Need Symbolism](https://arxiv.org/abs/2509.21404)
*Xiaotie Deng,Hanyu Li*

Main category: cs.CL

TL;DR: AI未来发展需要超越规模扩展，需要人类构建的符号作为指南针来引导大型语言模型的强大但盲目的直觉


<details>
  <summary>Details</summary>
Motivation: 当前AI发展过于依赖规模扩展，缺乏真正的发现能力，需要引入人类符号来引导模型直觉

Method: 提出使用人类构建的符号作为指南针来引导大型语言模型的直觉能力

Result: 该方法旨在解锁真正的发现能力，使AI能够进行更深入的探索和理解

Conclusion: AI的未来发展需要结合人类符号与模型直觉，通过符号指南针来引导模型实现真正的发现
...
### [165] [One Model, Many Morals: Uncovering Cross-Linguistic Misalignments in Computational Moral Reasoning](https://arxiv.org/abs/2509.21443)
*Sualeha Farid,Jayden Lin,Zean Chen,Shivani Kumar,David Jurgens*

Main category: cs.CL

TL;DR: 研究发现大型语言模型在多语言环境中的道德判断存在显著不一致性，反映了文化错位问题，呼吁开发更具文化意识的AI系统。


<details>
  <summary>Details</summary>
Motivation: 随着LLMs在多语言和多文化环境中部署，需要评估它们在多样化语言和文化背景下进行道德推理的能力，特别是考虑到主要基于英语数据预训练可能带来的泛化问题。

Method: 将两个成熟的道德推理基准翻译成五种文化和类型学上多样化的语言，进行多语言零样本评估，并通过精心构建的研究问题分析不一致性的根本原因。

Result: 发现LLMs在不同语言中的道德判断存在显著不一致性，往往反映文化错位，并识别出从分歧到推理策略的多种潜在驱动因素。

Conclusion: 研究揭示了预训练数据在塑造LLMs道德指南针中的作用，提出了一个结构化的道德推理错误类型学，强调需要开发更具文化意识的AI系统。
...
TL;DR: 该研究评估GPT-5在糖尿病诊断和管理中的表现，使用基于ADA 2025标准的合成病例进行测试，结果显示GPT-5在症状识别、实验室解读等方面与ADA标准高度一致。


<details>
  <summary>Details</summary>
Motivation: 糖尿病是全球重大健康挑战，早期识别困难，而大型语言模型的发展为医疗决策支持提供了新机会。

Method: 使用完全基于合成病例的模拟框架，测试GPT-5在五个代表性场景中的表现：症状识别、实验室解读、妊娠糖尿病筛查、远程监测和多模态并发症检测。

Result: GPT-5在分类病例、生成临床推理、提供患者解释和输出结构化JSON摘要方面表现出色，与ADA定义的标准高度一致。

Conclusion: GPT-5可作为临床医生和患者的双重用途工具，同时强调了在医疗保健中负责任评估LLM的可重复评估框架的重要性。
...
TL;DR: 本文分析了在减轻性别刻板印象背景下，道德对齐过程中性能权衡的机制，揭示了当前公平性目标在实现权衡方面的局限性。


<details>
  <summary>Details</summary>
Motivation: 道德对齐通常通过微调或模型编辑来调节预训练语言模型的行为，但这一过程往往以降低下游任务性能为代价。现有研究试图通过精心设计的公平性目标来实现性能权衡，鼓励模型选择性地忘记刻板印象知识，同时保持其有用性。

Method: 通过遗忘和公平性目标的视角，分析在减轻性别刻板印象背景下性能权衡的底层机制。

Result: 分析表明：(1)下游任务性能主要由整体遗忘水平驱动；(2)选择性遗忘刻板印象倾向于增加整体遗忘；(3)减轻遗忘的通用解决方案对减少整体遗忘无效，无法改善下游任务性能。

Conclusion: 当前公平性目标在实现性能权衡方面存在局限性，需要更有效的方法来平衡道德对齐与下游任务性能。
...
TL;DR: 本文提出在可验证奖励强化学习(RLVR)中采用token混合生成(MoT-G)方法，通过利用模型概率分布中的丰富信息，在连续混合空间中生成思维链，显著提升推理性能。


<details>
  <summary>Details</summary>
Motivation: 现有RLVR方法在推理步骤中采样离散token，丢弃了模型概率分布中的丰富分布信息，不必要地限制了推理搜索空间。

Method: 提出统一的MoT-G框架，将RLVR扩展到连续混合空间操作，包括构建混合嵌入作为token嵌入的加权和，以及在该空间中生成思维链。

Result: 在Reasoning-Gym推理任务套件上，MoT-G方法相比标准解码获得显著改进(7/10任务提升5-35%)，且仅用一半轨迹数达到可比精度，表明训练效率提升。

Conclusion: MoT-G通过保持更高的隐藏状态熵和促进token空间探索来提升推理能力，为RLVR提供了更有效的训练方法。
...
TL;DR: 提出Dual-Head Reasoning Distillation (DHRD)方法，在训练时使用推理头学习教师模型的思维链，推理时仅用分类头，实现高精度和高吞吐量。


<details>
  <summary>Details</summary>
Motivation: 解决思维链提示在提高分类准确率的同时带来的吞吐量显著下降问题，寻求精度与效率的平衡。

Method: 在解码器语言模型上添加池化分类头和推理头，训练时使用标签交叉熵和token级语言模型损失的加权和，推理时禁用推理头。

Result: 在七个SuperGLUE任务上相对池化基线提升0.65-5.47%，推理吞吐量比思维链解码快96-142倍。

Conclusion: DHRD方法成功实现了高精度分类同时保持高推理效率，特别在蕴含/因果任务上表现更佳。
...
TL;DR: 本文通过系统实验研究了代码的哪些特性对提升大语言模型推理能力最关键，发现结构扰动比语义扰动影响更大，适当的抽象表示如伪代码和流程图与真实代码效果相当，且表面规律性比语法正确性更重要。


<details>
  <summary>Details</summary>
Motivation: 虽然代码数据已被证明能增强大语言模型的推理能力，但尚不清楚代码的哪些方面对此贡献最大，需要系统研究代码的结构和语义特性如何影响模型推理。

Method: 构建十种编程语言的平行指令数据集，应用受控扰动来选择性破坏代码的结构或语义属性，然后在五个模型家族和八个规模上对每个变体进行微调，并在自然语言、数学和代码任务上评估性能。

Result: 在3,331个实验中，大语言模型对结构扰动比语义扰动更敏感，特别是在数学和代码任务上。伪代码和流程图等适当抽象与真实代码效果相当，且用更少token编码相同信息而不遵循原始语法通常能保持甚至提升性能。即使包含误导信号的损坏代码，只要表面规律性存在，仍具有竞争力。

Conclusion: 代码的结构特性对提升大语言模型推理能力更为关键，适当的抽象表示可以替代真实代码，表面规律性比语法正确性更重要，不同编程语言的语法风格对特定任务有不同优势。
...
TL;DR: 开发了一个基于Kisan呼叫中心数据的农业聊天机器人，用于回答农民关于天气、市场价格、植物保护和政府计划等问题的查询，通过句子嵌入模型和实体提取技术实现86%的准确率。


<details>
  <summary>Details</summary>
Motivation: 印度是农业经济国家，农民需要及时获取农业实践信息以实现最佳农业增长和产出，现有呼叫中心资源有限，需要24/7可访问的自动化解决方案。

Method: 基于Kisan呼叫中心数据集构建农业聊天机器人，采用句子嵌入模型，通过消除同义词和整合实体提取技术来提升系统性能。

Result: 基础句子嵌入模型准确率为56%，经过同义词消除和实体提取优化后，准确率提升至86%。系统可全天候通过电子设备访问，提供易于理解的信息。

Conclusion: 该系统能够帮助农民更便捷地获取农业相关信息，提高农业产出，同时减轻呼叫中心工作人员负担，使其能够专注于更有价值的工作。
...
TL;DR: 该研究评估了非洲口音英语说话人日志中的领域效应，发现临床对话存在一致的领域惩罚，错误主要来自误报和漏检。通过轻量级领域适应可减少错误但无法消除差距。
...
Method: 评估多个生产和开源系统在一般和临床对话上的表现，使用严格的DER协议评估重叠。测试通过微调分割模块进行轻量级领域适应。

Result: 临床语音存在一致的领域惩罚，错误主要来自误报和漏检，与短轮次和频繁重叠相关。领域适应可减少错误但无法消除差距。

Conclusion: 需要重叠感知的分割和平衡的临床资源作为实际下一步措施。
...
TL;DR: 本文比较了两种LLM引用范式：生成时引用(G-Cite)和后处理引用(P-Cite)，发现在覆盖率和引用正确性之间存在权衡，建议高风险应用优先采用检索中心的P-Cite方法。


<details>
  <summary>Details</summary>
Motivation: 在医疗、法律、学术和金融等高风险领域，可信赖的LLM必须提供可验证的来源引用，因为即使是小错误也可能带来严重后果。

Method: 引入G-Cite和P-Cite两种范式，在四个流行的归因数据集上从零样本到高级检索增强方法进行全面评估。

Result: P-Cite方法实现高覆盖率且具有竞争力的正确性和中等延迟，而G-Cite方法以覆盖率和速度为代价优先考虑精确度。检索是两种范式中归因质量的主要驱动因素。

Conclusion: 推荐高风险应用采用检索中心的P-Cite优先方法，在严格声明验证等精度关键场景中保留使用G-Cite。
...
TL;DR: ComPSum是一个个性化多文档摘要框架，通过比较用户偏好来生成结构化分析，并基于此生成个性化摘要。作者还提出了AuthorMap评估框架和PerMSum数据集来评估个性化效果。


<details>
  <summary>Details</summary>
Motivation: 个性化多文档摘要需要满足用户对写作风格和内容焦点的个性化偏好，而识别用户偏好的细粒度差异对于有效个性化至关重要。

Method: ComPSum框架首先通过比较给定用户与其他用户的偏好来生成结构化分析，然后使用该分析指导个性化摘要的生成。

Result: 在PerMSum数据集上使用AuthorMap评估，ComPSum优于强基线方法。

Conclusion: 通过比较用户偏好来识别细粒度差异，可以有效提升个性化多文档摘要的质量。
...
TL;DR: 提出了五种VLM作为形式化器的流水线，用于解决单样本、开放词汇和多模态PDDL形式化问题，在包含真实、多视角和低质量图像的新基准上评估，发现VLM作为形式化器显著优于端到端规划生成。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型在简单多模态规划任务中表现良好，但在需要长序列动作的长时程规划任务中表现不足。文本领域通过让LLM将规划领域和问题转化为形式化规划语言（如PDDL）取得了显著进展，但在多模态环境中相关研究仍然稀缺。

Method: 开发了五种VLM作为形式化器的流水线，处理单样本、开放词汇和多模态PDDL形式化。在现有基准上评估，同时提出了两个新基准，首次考虑使用真实、多视角和低质量图像的规划。

Result: VLM作为形式化器在规划性能上大大优于端到端规划生成。瓶颈在于视觉而非语言能力，因为VLM经常无法捕捉到必要的对象关系的完整集合。生成中间文本表示（如标题或场景图）可以部分弥补性能，但效果不一致。

Conclusion: VLM作为形式化器是有效的多模态规划方法，但视觉理解能力是主要瓶颈。中间文本表示提供部分改进，但仍有提升空间，为多模态规划形式化的未来研究指明了方向。
...
TL;DR: 本研究评估了多语言AI模型在翻译比喻性语言时的本地化能力，发现尽管模型能生成语法正确的翻译，但在文化适应性方面仍有明显不足，特别是在处理习语和双关语时。


<details>
  <summary>Details</summary>
Motivation: 现有LLM翻译研究和行业基准主要关注语法准确性和词汇正确性，而忽略了文化适应性和本地化质量这些对现实应用至关重要的因素。

Method: 评估了87个LLM生成的电商营销邮件翻译，涵盖20种语言的24种方言变体，由母语评审员提供定量评分和定性反馈。

Result: 领先模型通常能产生语法正确的翻译，但文化细微的语言表达仍是明显短板，需要大量人工修正。即使是资源丰富的主流语言也经常误译比喻性表达和文字游戏。

Conclusion: 数据量不是机器翻译质量的最可靠预测指标，文化适应性应成为多语言LLM性能的关键决定因素。当前多语言AI系统在现实本地化应用方面存在局限性。
...
Method: 提出了MORL分类法，分析了各种MORL方法在LLM优化中的应用，并提出了元策略MORL的双层学习范式。

Result: 识别了现有方法的局限性，提出了解决不同目标关系影响的基准测试框架愿景。

Conclusion: 元策略MORL的发展能够通过其双层学习范式提高效率和灵活性，为改进LLM性能提供了关键研究方向和潜在解决方案。
...
TL;DR: OjaKV是一个KV缓存压缩框架，通过混合存储策略和在线子空间适应来减少长上下文推理的内存瓶颈，无需模型微调即可保持高精度。


<details>
  <summary>Details</summary>
Motivation: 现有KV缓存压缩方法使用静态离线学习的子空间，在数据分布变化时性能不佳，需要动态适应上下文变化。

Method: 采用混合存储策略：完整保留首尾关键token，对中间token使用基于Oja算法的在线PCA进行低秩压缩，在预填充阶段全面更新，解码阶段轻量级更新。

Result: 在高压缩比下保持甚至提升零样本精度，在需要复杂推理的极长上下文基准测试中表现最强。

Conclusion: OjaKV是一个实用、即插即用的解决方案，可实现内存高效的长上下文推理，兼容现代注意力模块如FlashAttention。
...
TL;DR: 这篇综述论文系统回顾了针对语言模型的可解释人工智能方法，特别关注不同Transformer架构下的技术适应性，并基于合理性和忠实性评估这些方法的有效性。


<details>
  <summary>Details</summary>
Motivation: 语言模型的黑盒特性在关键领域应用中引发了可解释性担忧，而现有的可解释AI方法在应用于复杂语言模型时面临诸多限制。现有综述未能充分捕捉语言模型架构多样性和能力演进带来的独特挑战。

Method: 基于Transformer架构类型（仅编码器、仅解码器、编码器-解码器）对可解释AI技术进行分类组织，分析每种方法在不同架构下的适应性，并通过合理性和忠实性双重视角评估其有效性。

Result: 提供了针对不同语言模型架构的可解释AI技术分类框架，识别了各种方法的优势和局限性，为开发更透明可解释的语言模型提供了结构化视角。

Conclusion: 明确了开放的研究挑战和未来发展方向，旨在指导开发更鲁棒、透明和可解释的语言模型可解释AI方法。
...
TL;DR: 该论文提出ReviewScore来检测同行评审中的错误信息点，包括基于错误前提的"弱点"和论文已回答的"问题"，并验证15.2%的弱点和26.4%的问题存在错误信息。


<details>
  <summary>Details</summary>
Motivation: 随着AI会议投稿量激增，同行评审质量下降，需要可靠检测低质量评审中的错误信息。

Method: 提出自动化引擎重构弱点中的显式和隐式前提，构建专家标注的数据集，使用8个最先进的大语言模型评估ReviewScore。

Result: 验证了15.2%的弱点和26.4%的问题存在错误信息，人类与模型在ReviewScore上达到中等一致性，前提级别的事实性评估比弱点级别的一致性更高。

Conclusion: 完全自动化ReviewScore评估具有潜力，前提级别的事实性评估比弱点级别更可靠。
...
TL;DR: 提出了GRAB基准，用于评估无监督主题模型在10-K风险披露分类中的表现，包含161万句子和自动生成的标签，支持标准化比较。


<details>
  <summary>Details</summary>
Motivation: 缺乏评估无监督主题模型在财务风险披露分类任务中的公共基准。

Method: 结合FinBERT注意力、YAKE关键词信号和分类感知的搭配匹配，自动生成句子标签，基于包含193个术语映射到21个细粒度类型的风险分类法。

Result: 创建了包含161万句子和自动生成标签的基准数据集，支持固定数据分割和多种鲁棒指标评估。

Conclusion: GRAB为财务披露中的主题模型提供了可复现、标准化的比较平台。
...
<details>
  <summary>Details</summary>
Motivation: 现有图增强检索方法面临根本性权衡：图方法依赖高质量图结构，但手动构建成本高昂，自动提取受限于LLM提取器性能。需要克服静态图索引无法适应实际查询的问题。
...
Conclusion: ToG-3通过双演化多智能体系统实现了自适应图索引构建，即使在轻量级LLM上也能实现深度精确推理，解决了静态图构建的固有缺陷。
...
TL;DR: 提出了ProPerSim任务和仿真框架，用于开发能够在真实家庭场景中做出及时、个性化推荐的AI助手，以及基于此的ProPerAssistant系统。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型在日常生活中的广泛应用，人们对不仅反应式而且主动式和个性化的AI助手需求日益增长。虽然近期研究在主动性和个性化方面分别取得了进展，但两者的结合仍未被充分探索。

Method: 引入ProPerSim仿真框架，其中具有丰富人设的用户代理与助手互动，对每个建议的偏好和情境匹配度进行评分。助手的目标是利用这些评分来学习和适应，以随时间获得更高分数。基于此提出了ProPerAssistant，一个检索增强、偏好对齐的助手，通过用户反馈持续学习和适应。

Result: 在32个不同人设上的实验表明，ProPerAssistant能够调整其策略并稳步提高用户满意度。

Conclusion: 研究展示了结合主动性和个性化的前景，为开发更智能、适应性更强的AI助手提供了可行路径。
...
TL;DR: 该论文探讨了在大规模上下文问答中部署大型语言模型的挑战，发现经过微调的8B参数开源模型在准确率上可以超越GPT-4o，展示了其在现实应用中的潜力。


<details>
  <summary>Details</summary>
Motivation: 工业环境中部署LLM进行长上下文问答面临高计算成本和延迟挑战，特别是在需要基于相同上下文回答多个问题时。

Method: 对专有和开源模型进行广泛实验和基准测试，评估它们在相同对话上下文中回答多个问题的能力。

Result: 强大的专有模型如GPT-4o整体表现最佳，但经过微调的8B参数开源模型在准确率上可以超越GPT-4o。
...
TL;DR: 提出Self-Speculative Biased Decoding方法，用于流式应用中的同时翻译，通过重用最近输出作为草稿并偏向验证，实现1.7倍加速和80%闪烁减少。


<details>
  <summary>Details</summary>
Motivation: 解决LLMs在流式应用（如实时翻译）中重复从头生成输出的问题，需要满足延迟要求同时保持输出质量。

Method: 使用最近输出作为草稿，在验证阶段偏向草稿标记以提高接受率，避免重复计算草稿，是一种即插即用的模型无关解决方案。

Result: 在同时文本重翻译任务中，相比传统自回归重翻译实现1.7倍加速，质量不受影响，结合display-only mask-k技术显著减少80%闪烁。

Conclusion: 该方法为延迟敏感的流式应用提供了有效的加速方案，无需草稿计算，减少用户干扰，提高效率。
...
<details>
  <summary>Details</summary>
Motivation: 现有大型音频语言模型在复杂声学场景下的音频推理任务中表现有限，缺乏对噪声抑制、声源分离等声学工具的使用能力。

Method: 开发了Thinking-with-Sound框架，结合语言推理和实时音频域分析，使模型能够主动对音频信号进行数值分析和数字处理。

Result: 在MELD-Hard1k基准测试中，TwS显著提升了模型鲁棒性：小模型准确率提升24.73%，大模型提升36.61%。
...
TL;DR: SynerGen是一个统一的生成式推荐模型，通过单一生成骨干网络同时处理个性化搜索和推荐任务，在检索和排序方面都表现出色，解决了传统检索-排序管道的校准问题和工程开销。


<details>
  <summary>Details</summary>
Motivation: 解决传统检索-排序管道存在的校准问题和工程开销，以及现有生成模型在统一个性化搜索和推荐时面临性能权衡的问题。

Method: 使用基于行为序列训练的仅解码器Transformer，结合InfoNCE损失进行检索优化和混合点对-配对损失进行排序优化，并提出了时间感知旋转位置嵌入来有效整合时间信息。
...
TL;DR: 使用因果推断方法分析结构化输出对LLM生成质量的影响，发现大多数情况下结构化输出没有因果影响，而传统评估指标可能产生误导性结论。
...
Method: 基于因果推断框架，推导出五种潜在因果结构，并在八个推理任务上进行系统分析。

Result: 在48个场景中，43个显示结构化输出没有因果影响；其余5个中，3个涉及由具体指令影响的多方面因果结构。

Conclusion: 传统评估指标可能夸大结构化输出的影响，因果推断提供了更精确的分析框架，揭示了结构化输出在大多数情况下对LLM生成质量没有实质性因果影响。
...
TL;DR: 提出了CARB基准来评估奖励模型的文化意识，发现现有模型存在缺陷，并提出了Think-as-Locals方法通过强化学习改进文化意识奖励建模。


<details>
  <summary>Details</summary>
Motivation: 现有奖励模型评估缺乏文化意识评估数据集，无法有效评估模型对不同文化的理解能力，阻碍了LLMs的全球对齐。

Method: 构建了CARB基准覆盖10种文化4个领域，并提出Think-as-Locals方法，通过RLVR激发生成式奖励模型进行更深层的文化推理。

Result: 评估显示现有奖励模型在文化意识建模方面存在不足，CARB表现与下游多语言文化对齐任务正相关。Think-as-Locals能有效减少虚假特征干扰。

Conclusion: CARB基准填补了文化意识评估空白，Think-as-Locals方法能提升奖励模型的文化理解能力，促进LLMs的全球对齐。
...
TL;DR: 本文提出了一种扩展动作空间的同步机器翻译方法，通过引入四种自适应动作（句子切分、丢弃、部分摘要和代词化）来改善实时翻译质量和延迟表现。


<details>
  <summary>Details</summary>
Motivation: 传统编码器-解码器策略的READ/WRITE动作无法充分应对同步机器翻译在严格实时约束下的高质量翻译需求，需要扩展动作空间来实现实时重构、省略和简化。

Method: 在仅解码器的大语言模型框架中实现四种自适应动作，通过动作感知提示构建训练参考，并开发延迟感知的TTS管道来评估质量和延迟。

Result: 在ACL60/60英中和英德基准测试中，该框架持续改善语义指标（如COMET-KIWI）并实现更低的延迟（平均滞后测量），特别是DROP和SENTENCE_CUT组合在流畅性和延迟之间达到最佳平衡。

Conclusion: 丰富基于LLM的同步机器翻译的动作空间为缩小人机翻译差距提供了有前景的方向。
...
TL;DR: 提出CaMIB模型，通过因果多模态信息瓶颈解决传统方法在OOD泛化中的偏差问题，提升多模态语言理解的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有基于互信息最大化的多模态学习方法容易受到数据集偏差影响，将统计捷径与真实因果特征混淆，导致OOD泛化性能下降。

Method: 应用信息瓶颈过滤单模态输入噪声，使用参数化掩码生成器解耦多模态表示为因果和捷径子表示，结合工具变量约束和后门调整稳定因果估计。

Result: 在多模态情感分析、幽默检测和讽刺检测任务及OOD测试集上验证了CaMIB的有效性，理论和实证分析显示其具有良好的可解释性和鲁棒性。

Conclusion: CaMIB通过因果原则而非传统似然方法，成功提升了多模态语言理解的OOD泛化能力，为克服数据集偏差提供了有效解决方案。
...
TL;DR: 本文研究了语言谜题的解决与生成任务，探索了大型语言模型在解决语言学奥林匹克竞赛谜题中的表现，并基于解决经验开发了谜题生成方法。


<details>
  <summary>Details</summary>
Motivation: 通过自动化生成语言谜题来扩大语言学的影响力，向更广泛的受众介绍这一领域，同时支持稀有和未被充分研究语言的知识传播。

Method: 扩展了现有的语言谜题解决基准，测试了包括OpenAI o1在内的先进LLM模型在不同语言学主题谜题上的表现，并利用解决经验指导谜题生成任务。

Result: LLM在大多数谜题类型上表现优于人类，但在文字系统和未被充分研究语言的谜题上表现较差。

Conclusion: 语言谜题生成作为一个研究任务具有重要意义，既能推广语言学，又能支持稀有语言知识的传播，LLM在该领域展现出巨大潜力。
...
TL;DR: 提出了ResT方法，通过基于熵的token重加权来重塑策略梯度，优化大语言模型的工具使用策略训练，在单轮和多轮任务中均取得SOTA效果
...
Result: 在BFCL和API-Bank基准测试中，ResT比现有方法提升高达8.76%；在4B基础LLM上微调后，单轮任务超越GPT-4o 4.11%，多轮任务超越1.50%

Conclusion: ResT通过熵感知的token级策略梯度，有效稳定了工具使用任务的训练收敛，在单轮和多轮任务中都实现了最先进的性能
...
<details>
  <summary>Details</summary>
Motivation: 级联系统面临在开放式文本生成中判断输出可靠性的挑战，因为生成质量是连续谱且存在多个有效响应。

Method: 使用语义一致性作为训练免费的可靠性信号，当多样化模型输出在语义层面达成共识时，其一致性比词元级置信度更可靠。

Result: 在5亿到700亿参数模型上评估，语义级联以40%的成本达到或超过目标模型质量，延迟降低达60%。
...
TL;DR: 提出了TRACE框架，通过将共情任务分解为分析和合成两个阶段，将深度分析与流畅生成相结合，显著提升了共情对话生成的效果。


<details>
  <summary>Details</summary>
Motivation: 现有方法在专门模型的深度分析和大型语言模型的生成流畅性之间存在权衡，需要一种能同时兼顾两者的共情对话生成方法。

Method: TRACE框架将共情建模为结构化认知过程，通过任务分解构建分析-合成流水线，在生成前建立全面理解。

Result: 实验结果表明，TRACE框架在自动评估和LLM评估中都显著优于强基线模型。

Conclusion: 结构化任务分解是创建更强大、可解释共情代理的有前景范式。
...
TL;DR: 提出了首个专门评估多轮长格式问答（MT-LFQA）的基准KnowMT-Bench，发现在多轮对话中模型的事实准确性和信息效率会下降，而检索增强生成（RAG）可以有效缓解这一问题。


<details>
  <summary>Details</summary>
Motivation: 现有基准主要关注单轮对话，而多轮对话基准通常评估其他能力而非知识密集型的事实准确性，这在实际应用中存在关键差距。

Method: 引入KnowMT-Bench基准，采用动态评估设置，让模型基于逻辑递进的问题序列生成多轮对话历史，然后使用人工验证的自动化流程评估最终轮答案的事实能力和信息传递效率。

Result: 实验表明多轮上下文会降低性能：事实能力因自生成历史中的上下文噪声而下降，信息效率随对话长度增加而模型变得更啰嗦而降低。检索增强生成（RAG）可以有效缓解甚至逆转这种事实退化。
...
TL;DR: LoRAN是对LoRA的非线性扩展，通过轻量级变换改进低秩更新，并引入Sinter正弦激活函数增强表达能力，在多个任务中优于QLoRA。
...
Method: 提出LoRAN方法，对低秩更新应用轻量级变换，并引入Sinter正弦激活函数添加结构化扰动而不增加参数数量。

Result: 在摘要生成和分类任务上的实验表明，LoRAN持续优于QLoRA，消融研究显示Sinter激活函数优于Sigmoid、ReLU和Tanh等标准激活函数。

Conclusion: 激活函数设计在低秩调优中具有重要性，LoRAN通过非线性扩展有效提升了LoRA的性能。
...
<details>
  <summary>Details</summary>
Motivation: RAG系统即使提供了正确和充分的上下文仍然会产生幻觉，现有方法需要大量超参数调优，限制了泛化能力。
...
TL;DR: 提出了RL-ZVP算法，利用零方差提示（所有回答获得相同奖励的问题）来增强LLM的推理能力，相比现有方法GRPO在数学推理基准上取得显著提升。


<details>
  <summary>Details</summary>
Motivation: 现有RLVR方法如GRPO只关注回答正确性不同的提示，忽略了所有回答获得相同奖励的零方差提示，而这些提示实际上也能提供有意义的学习信号。

Method: RL-ZVP算法从零方差提示中提取学习信号，直接奖励正确性和惩罚错误，无需对比不同回答，并通过词元级特征调节反馈以保留细微信号。
...
TL;DR: QoNext框架将网络和多媒体领域的体验质量原则应用于基础模型评估，通过识别影响用户体验的因素并构建预测模型，实现主动细粒度评估。


<details>
  <summary>Details</summary>
Motivation: 现有基础模型评估方法只关注输出正确性，忽略了用户交互体验的重要性，无法捕捉用户满意度的真正机制。

Method: 引入QoNext框架，识别影响用户体验的因素，在受控实验中收集人类评分，构建QoE导向数据库并训练预测模型。

Result: QoNext能够实现主动和细粒度的评估，并为产品化服务提供优化基础模型的可操作指导。

Conclusion: QoNext框架成功将体验质量原则应用于基础模型评估，填补了现有评估方法的空白，为实际应用提供了有价值的评估工具。
...
<details>
  <summary>Details</summary>
Motivation: 传统MoE模型在训练和推理时固定激活专家数量k，但直觉上在推理时激活更多专家(k'>k)应该能提升性能。然而研究发现，增加专家数量后性能反而快速下降，这源于专家之间缺乏学习协作。

Method: 引入EMoE训练框架，通过同时训练专家在多样化组合中协作，并鼓励路由器进行高质量选择，使MoE模型能够在推理时扩展激活专家数量而无需额外训练开销。

Result: 实验表明EMoE显著扩展了有效性能扩展范围，可达训练时k的2-3倍，同时将模型峰值性能推向更高水平。

Conclusion: EMoE解决了MoE模型在推理时扩展专家数量的性能下降问题，实现了更灵活的算力-性能权衡。
...
TL;DR: 本文针对土耳其语等黏着语，提出了一个系统性的引文意图分类方法，包括创建新的土耳其语引文意图数据集、使用DSPy框架自动优化提示，以及通过堆叠泛化集成实现91.3%的最先进准确率。


<details>
  <summary>Details</summary>
Motivation: 理解引文的定性意图对学术研究评估至关重要，但黏着语如土耳其语在此任务上面临独特挑战，需要专门的方法和数据集来解决。

Method: 创建新的土耳其语引文意图数据集，使用DSPy框架构建可编程分类管道自动优化提示，采用堆叠泛化集成（XGBoost元模型）聚合多个优化模型的输出。

Result: 通过堆叠泛化集成方法，实现了91.3%的最先进准确率，显著优于标准上下文学习方法。
...
TL;DR: AutoSCORE是一个多智能体LLM框架，通过评分标准对齐的结构化组件识别来增强自动评分。它使用两个智能体：一个提取评分标准相关组件并编码为结构化表示，另一个基于此分配最终分数，从而提高可解释性和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 解决LLM作为端到端评分器面临的低准确性、提示敏感性、有限可解释性和评分标准不对齐等问题，这些阻碍了LLM在自动评分实践中的应用。

Method: 提出AutoSCORE多智能体框架，包含评分标准组件提取智能体和评分智能体。前者从学生回答中提取评分标准相关组件并编码为结构化表示，后者基于此分配最终分数。

Result: 在ASAP基准的四个数据集上评估，使用GPT-4o、LLaMA-3.1-8B和LLaMA-3.1-70B。相比单智能体基线，AutoSCORE持续提高了评分准确性、人机一致性（QWK、相关性）和误差指标（MAE、RMSE），特别是在复杂多维评分标准和小型LLM上表现突出。
...
TL;DR: 提出SimulSense框架，模仿人类口译员在感知到新语义单元时触发翻译决策，实现更好的质量-延迟权衡和实时效率


<details>
  <summary>Details</summary>
Motivation: 解决现有同声传译系统需要专门训练数据和依赖计算昂贵的LLM推理的问题，模仿人类口译员的阅读/写入决策机制

Method: 提出SimulSense框架，通过连续读取输入语音并在感知到新语义单元时触发写入决策来产生翻译

Result: 相比两种最先进的基线系统，在质量-延迟权衡方面表现更优，决策速度比基线快9.6倍

Conclusion: SimulSense框架能够有效模拟人类口译员的决策过程，显著提升同声传译系统的实时效率
...
TL;DR: 本研究首次系统评估了思维链提示在临床文本理解中的效果，发现86.3%的大型语言模型在CoT设置下性能下降，揭示了临床领域CoT的可靠性悖论。


<details>
  <summary>Details</summary>
Motivation: 评估思维链提示在临床文本理解中的有效性，特别是在电子健康记录这种长文本、碎片化且嘈杂的临床文档环境中。

Method: 在87个真实世界临床文本任务上评估95个先进LLM，涵盖9种语言和8种任务类型，使用细粒度分析包括推理长度、医学概念对齐和错误分析。

Result: 与其它领域相反，86.3%的模型在CoT设置下性能下降，能力更强的模型相对稳健，而较弱模型性能显著下降。

Conclusion: CoT在临床文本任务中增强了可解释性但可能损害可靠性，需要开发透明且可信的临床推理策略。
...
TL;DR: ThaiFACTUAL是一个轻量级、模型无关的校准框架，通过反事实数据增强和基于推理的监督来缓解泰语政治立场检测中的偏见，无需微调。


<details>
  <summary>Details</summary>
Motivation: 在泰语政治环境中，由于间接语言、极化人物以及情感与立场的纠缠，大语言模型存在系统性偏见（如情感泄漏和实体偏袒），影响公平性和可靠性。

Method: 使用反事实数据增强和基于推理的监督来分离情感与立场，减少偏见；发布了首个高质量的泰语政治立场数据集，包含立场、情感、推理和偏见标记。

Result: 实验结果显示ThaiFACTUAL显著减少了伪相关，增强了零样本泛化能力，并提高了多个大语言模型的公平性。

Conclusion: 这项工作强调了基于文化的去偏技术对于代表性不足语言的重要性。
...
<details>
  <summary>Details</summary>
Motivation: LLM在加速学术创意生成方面潜力巨大，但面临创意落地困难和确认偏误的挑战，需要更好的基础支撑和偏见缓解机制。

Method: 整合动机知识图谱(MotivGraph)和Q驱动的苏格拉底创意生成器。MotivGraph结构化存储问题、挑战和解决方案节点；创意生成器采用双代理系统进行苏格拉底式提问。

Result: 在ICLR25论文主题数据集上，MotivGraph-SoIQ在LLM评分、ELO排名和人工评估指标上均优于现有最先进方法。

Conclusion: 该框架通过提供基础支撑和严格的精炼过程，有效提升了LLM创意生成的新颖性、实验严谨性和动机合理性。
...
<details>
  <summary>Details</summary>
Motivation: 现有幻觉检测方法需要外部资源或LLMs内部状态，但在黑盒API受限的情况下，迫切需要建立有效的黑盒检测方法。

Method: 研究LLMs在不确定性表达下的行为，发现事实性回答时生成一致响应，非事实性回答时生成不一致响应，基于此设计黑盒检测指标。

Result: 实验表明该方法比使用LLMs内部知识的基线方法更能预测模型响应的真实性。

Conclusion: 基于不确定性表达的黑盒检测方法能有效识别LLMs的幻觉问题，为受限API环境下的幻觉检测提供了可行方案。
...
TL;DR: GraphSearch提出了一种新的代理深度搜索工作流，通过双通道检索策略解决传统GraphRAG方法在浅层检索和结构图数据利用效率低下的问题。


<details>
  <summary>Details</summary>
Motivation: 现有GraphRAG方法存在两个核心局限：浅层检索无法发现所有关键证据，以及预构建结构图数据的低效利用，这阻碍了从复杂查询中进行有效推理。
...
TL;DR: 论文研究发现，主题建模中常被视为噪声的异常值实际上是新兴主题的弱信号，通过向量嵌入和累积聚类方法，在英法新闻数据集中观察到异常值会随时间演变成连贯主题的一致模式。


<details>
  <summary>Details</summary>
Motivation: 传统主题建模通常将异常值视为噪声而忽略，但本研究认为这些异常值可能包含新兴主题的早期信号，特别是在动态新闻语料中。

Method: 使用最先进语言模型的向量嵌入和累积聚类方法，在关注企业社会责任和气候变化的英法新闻数据集上追踪异常值随时间的变化。

Result: 发现异常值倾向于随时间演变成连贯主题的一致模式，这一现象在两种模型和两种语言中都得到验证。
...
TL;DR: 提出了TACOS（临床代理全面安全分类法），一个细粒度的21类分类法，将安全过滤和工具选择整合到单一的用户意图分类步骤中，专门针对临床聊天机器人应用。


<details>
  <summary>Details</summary>
Motivation: 临床聊天机器人应用中安全至关重要，现有方法（如护栏和工具调用）在满足临床领域的细致需求方面存在不足，不准确的回答可能导致严重后果。

Method: 开发了TACOS分类法，涵盖临床和非临床查询的广泛范围，明确建模不同的安全阈值和外部工具依赖关系，并构建了TACOS标注数据集进行实验验证。

Result: 实验结果表明专门针对临床代理设置的新分类法具有价值，并揭示了关于训练数据分布和基础模型预训练知识的有用见解。

Conclusion: TACOS框架为临床聊天机器人提供了一种更全面的安全解决方案，通过统一的意图分类方法整合了安全过滤和工具选择功能。
...
TL;DR: 提出了模糊推理链(FRC)框架，将LLM语义先验与连续模糊隶属度结合，在概率推理和模糊隶属推理之间建立显式交互，能逐步将模糊输入转化为清晰可解释的决策。


<details>
  <summary>Details</summary>
Motivation: 尽管大语言模型快速发展，但在处理具有歧义、多义性或不确定性的文本时仍面临重大挑战，传统概率方法无法有效捕捉冲突或不确定信号。

Method: FRC框架整合LLM语义先验与连续模糊隶属度，创建概率推理和模糊隶属推理之间的显式交互，实现从模糊输入到清晰决策的渐进转换。

Result: 在情感分析任务上的验证表明，FRC确保了稳定推理并促进了不同模型规模间的知识迁移，理论分析和实证结果均支持其有效性。
...
TL;DR: 提出了首个社交媒体AI生成文本(AIGT)的纵向数据集RedNote-Vibe和基于心理语言学特征的检测框架PLAD，用于分析AIGT的时间动态和用户互动模式。


<details>
  <summary>Details</summary>
Motivation: 现有数据集主要关注静态AIGT检测，而社交媒体上的内容动态由用户参与驱动并随时间演变，需要研究AIGT的时间动态和用户互动模式。

Method: 从小红书平台收集5年纵向数据，包含用户参与度指标和时间戳；提出基于心理语言学特征的PLAD检测框架。
...
TL;DR: 该论文提出了QCET评估质量标准分类法，旨在解决NLP领域中质量评估标准名称不一致导致评估结果不可比较的问题。


<details>
  <summary>Details</summary>
Motivation: 当前NLP领域存在数百种质量评估标准名称，即使名称相同也可能评估不同的质量方面，这阻碍了基于多个独立评估得出可靠结论的能力，影响了整个领域的科学进展。

Method: 采用严格描述性方法，从三个NLP评估调查中推导出标准化的质量评估标准名称和定义，并将其组织成层次结构，其中父节点捕捉子节点的共同方面。

Result: 提出了QCET分类法及其相关资源，该分类法能够将实际使用的数百种质量评估标准名称映射到标准化集合中。

Conclusion: QCET具有三个主要用途：建立现有评估的可比性、指导新评估的设计以及评估监管合规性，为解决NLP评估可比性问题提供了标准化框架。
...
TL;DR: 挑战了微调不适合模型编辑的传统观点，提出通过将深度优先流水线改为广度优先（基于epoch）的微调方法，并结合局部化调优策略，显著提升了模型编辑效果。


<details>
  <summary>Details</summary>
Motivation: 传统认为微调不适合模型编辑，但作者认为失败原因在于采用了不合适的深度优先流水线，而非微调本身的问题。

Method: 将微调从深度优先改为广度优先流水线，使用小批量优化，并开发LocFT-BF方法进行局部化参数调优。

Result: 在多种LLM和数据集上的实验表明，该方法大幅超越现有最优方法，首次实现了10万次编辑和720亿参数模型的编辑，且不损害通用能力。

Conclusion: 澄清了长期存在的误解，将微调从被低估的基线方法提升为模型编辑的领先方法，为未来研究奠定基础。
...
Method: 使用稠密字典和列稀疏系数矩阵的结构化稀疏分解，实现子空间联合表示；利用校准数据集优化分解，最小化功能重构误差而非权重近似误差。
...
TL;DR: 提出了对话行为脚本(DAS)框架，通过结构化对话行为表示来生成多语言对话，避免直接翻译带来的不自然和文化不适当问题。


<details>
  <summary>Details</summary>
Motivation: 非英语对话数据集稀缺，现有方法通常依赖英语对话的翻译，这会引入人工痕迹，降低自然性和文化适当性。

Method: 使用结构化对话行为表示框架，从抽象意图表示编码、本地化和生成多语言对话，而不是直接翻译对话话语。

Result: 在意大利语、德语和中文上的人类评估显示，DAS生成的对话在文化相关性、连贯性和情境适当性方面持续优于机器和人工翻译的对话。

Conclusion: DAS框架通过结构化对话行为表示支持灵活的语言本地化，减轻翻译腔问题，实现更流畅自然的对话生成。
...
TL;DR: 本文提出Solve-to-Judge (S2J)方法来解决生成奖励模型(GRM)中存在的解决-判断差距问题，通过同时利用模型的解决和判断能力进行监督，在优化过程中显式连接这两种能力。


<details>
  <summary>Details</summary>
Motivation: 研究发现GRM存在显著的解决-判断差距，即模型能够解决某些问题却无法做出正确判断(14%-37%的查询)。这种能力不匹配限制了GRM的判断性能。

Method: 提出S2J方法，在单个GRM的输出上同时利用其解决和判断能力进行监督，在模型优化过程中显式连接问题解决和评估能力。

Result: S2J有效减少了16.2%的解决-判断差距，将模型判断性能提升了5.8%，在使用更小训练数据集的情况下实现了SOTA性能，且不依赖外部模型蒸馏。

Conclusion: S2J通过自演进方式成功缩小了GRM的解决-判断差距，显著提升了判断性能，为生成奖励模型的优化提供了有效解决方案。
...
TL;DR: 该论文提出了一种基于测试时计算扩展(TTS)的方法来验证复杂的数值声明，通过训练验证器模型从多个推理路径中选择正确的验证结果，并引入自适应机制提高计算效率。


<details>
  <summary>Details</summary>
Motivation: 现有的大语言模型在验证需要组合推理和数值推理的真实世界数值声明时存在不足，无法理解数值细微差别且容易产生推理漂移问题。

Method: 使用测试时计算扩展(TTS)从大语言模型引出多个推理路径，训练验证器模型(VERIFIERFC)选择正确的推理路径，并引入自适应机制基于声明复杂度选择性执行TTS。

Result: TTS方法显著缓解了推理漂移问题，自适应TTS比标准TTS效率提高1.8倍，相比单次验证方法性能提升18.8%。
...
TL;DR: Uni-LAP是一个通用的法律条文预测框架，通过整合监督分类模型和大型语言模型的优势来解决法律条文预测中的挑战。


<details>
  <summary>Details</summary>
Motivation: 现有方法在处理法律条文预测时面临显著挑战：监督分类模型难以捕捉复杂事实模式，大型语言模型在预测场景中表现不佳，且不同司法管辖区的法律系统多样性导致方法缺乏普适性。

Method: 提出Uni-LAP框架，通过紧密协作整合SCM和LLM的优势：SCM使用新颖的Top-K损失函数生成准确候选条文，LLM采用三段论推理来优化最终预测。

Result: 在多个司法管辖区的数据集上评估Uni-LAP，实证结果表明该方法持续优于现有基线方法，展示了其有效性和泛化能力。

Conclusion: Uni-LAP框架成功解决了法律条文预测中的关键挑战，通过整合不同模型的优势实现了更好的性能和跨司法管辖区的适用性。
...
TL;DR: 对31个多语言视觉语言模型和21个基准测试的调查，揭示了语言中立性与文化意识之间的关键张力，发现当前训练方法偏向中立性，而评估基准大多采用翻译方法，存在跨语言能力差异和训练目标与评估目标之间的差距。


<details>
  <summary>Details</summary>
Motivation: 研究多语言视觉语言模型在处理跨语言文本和图像时的表现，特别关注语言中立性（跨语言表示一致性）与文化意识（文化语境适应）之间的平衡问题。

Method: 系统回顾了31个模型和21个基准测试，涵盖编码器专用和生成式架构，分析训练方法和评估方法。

Result: 发现当前训练方法通过对比学习偏向语言中立性，而文化意识依赖于多样化数据；三分之二的评估基准使用基于翻译的方法，强调语义一致性，近期工作开始纳入文化相关内容。

Conclusion: 存在跨语言能力差异，训练目标与评估目标之间存在差距，需要在语言中立性和文化意识之间找到更好的平衡。
...
TL;DR: FoodSEM是一个专门为食品领域命名实体链接任务微调的开源大语言模型，在多个食品本体上实现了最先进的性能，F1分数在某些数据集上达到98%。


<details>
  <summary>Details</summary>
Motivation: 现有的通用大语言模型和定制领域特定模型无法准确解决食品命名实体链接任务，需要专门针对食品领域的模型来提升语义理解能力。

Method: 通过指令-响应场景，将文本中提到的食品相关实体链接到多个本体（FoodOn、SNOMED-CT、Hansard分类法），并对模型进行微调。

Result: FoodSEM在相关模型/系统中实现了最先进的性能，与零样本、单样本和少样本LLM提示基线相比表现更优，某些本体和数据集的F1分数达到98%。

Conclusion: 本文的主要贡献包括发布适合LLM微调/评估的食品标注语料库、发布用于推进食品领域文本语义理解的鲁棒模型，以及为未来基准测试提供食品NEL的强基线。
...
TL;DR: 提出Reasoning Capsule框架，通过压缩推理计划为少量潜在token，结合潜在推理效率和显式CoT透明度，在信息瓶颈原则下平衡效率、准确性和可解释性。


<details>
  <summary>Details</summary>
Motivation: 解决CoT提示的冗长性导致的延迟和内存使用问题，以及早期错误在长链中传播的问题，寻求在推理效率和透明度之间取得平衡。

Method: 将高级推理计划压缩为少量学习的潜在token（推理胶囊），通过低容量瓶颈鼓励最小性，通过任务损失和计划重建损失确保充分性，基于信息瓶颈原则设计。

Result: 在保持或提高复杂基准测试准确性的同时，显著减少了推理的可见token占用，实现了效率、准确性和可解释性的平衡。

Conclusion: Reasoning Capsule框架成功地将潜在推理效率与显式CoT透明度相结合，为复杂推理任务提供了更高效的解决方案。
...
TL;DR: GTO通过引入草案树奖励和基于群体的草案策略训练，解决了推测解码中训练目标与解码策略不匹配的问题，显著提高了接受长度和推理速度。


<details>
  <summary>Details</summary>
Motivation: 现有推测解码方法在训练时只优化单一路径，而解码时使用树策略验证多个分支，这种策略不匹配限制了加速效果。

Method: 提出GTO方法，包含草案树奖励（采样自由目标，衡量草案树在目标模型下的期望接受长度）和基于群体的草案策略训练（通过对比当前和参考草案模型的树，形成去偏的群体标准化优势，应用PPO风格代理进行稳健更新）。

Result: 在对话、代码和数学任务上，GTO将接受长度提高了7.4%，相比之前的SOTA方法EAGLE-3实现了额外7.7%的加速。

Conclusion: GTO通过弥合草案策略不匹配，为高效LLM推理提供了实用、通用的解决方案。
...
TL;DR: 本文概述了德国国家研究数据基础设施（NFDI4DS）联盟开发和主办的12个共享任务，这些任务通过社区标准化评估促进研究进展，支持FAIR原则和可重复研究实践。


<details>
  <summary>Details</summary>
Motivation: 共享任务是推进研究的有力工具，通过社区标准化评估促进可查找、可访问、可互操作和可重用（FAIR）的研究实践，以及透明和可重复的研究。

Method: 在NFDI4DS联盟下开发和主办12个共享任务，涵盖学术文档处理中的多样化挑战，并在领先的学术场所举办。

Result: 这些任务促进了方法创新，并为更广泛的研究社区贡献了开放获取的数据集、模型和工具，这些资源已整合到联盟的研究数据基础设施中。

Conclusion: NFDI4DS联盟的共享任务在推进学术文档处理研究、促进FAIR原则和可重复研究实践方面发挥了重要作用，为研究社区提供了宝贵的资源和基础设施。
...
TL;DR: 提出MACC框架，通过多轮自适应压缩CoT推理过程，利用令牌弹性现象优化压缩深度，在保持准确性的同时显著降低推理延迟和令牌长度。
...
Method: 基于令牌弹性现象，采用多轮自适应压缩策略，逐步精炼CoT，为每个输入确定最优压缩深度。

Result: 相比最先进基线方法，平均准确率提升5.6%，CoT长度平均减少47个令牌，显著降低延迟。

Conclusion: CoT压缩既有效又可预测，通过可解释特征如困惑度和压缩率可可靠预测测试时性能，无需重复微调即可实现高效模型选择和预测。
...
TL;DR: 该论文研究了机器生成文本检测问题，提出了BMAS English数据集，支持文档级二元/多元分类、句子级分割和对抗攻击检测。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型的发展，需要区分人类和机器生成文本以保护人类创造力和创新能力的真实性。

Method: 引入BMAS English数据集，包含文档级二元分类、多元分类（识别生成器）、句子级分割和对抗攻击检测任务。

Result: 开发了一个综合数据集，能够更全面地解决机器生成文本检测问题。

Conclusion: 这项工作以更有意义的方式解决了机器生成文本检测问题，为相关研究提供了新的数据集和方法。
...
TL;DR: CompAs是一个元学习框架，通过将上下文信息转换为具有组合结构的适配器参数，实现指令、演示和检索段落的代数合并，从而降低推理成本、提高长上下文稳定性。


<details>
  <summary>Details</summary>
Motivation: 解决ICL在处理大量演示时效率低下，以及SFT存在训练开销和灵活性不足的问题，探索将上下文直接映射到适配器参数的替代方案。

Method: 引入CompAs元学习框架，将上下文转换为具有组合结构的适配器参数，支持代数合并，无需重新处理长提示。

Result: 在多项选择和抽取式问答任务上，CompAs优于ICL和先前基于生成器的方法，特别是在扩展到更多输入时表现更佳。

Conclusion: 可组合适配器生成为扩展LLM部署提供了实用且高效的替代方案，支持信息可逆编码，便于安全和安全管理。
...
TL;DR: 通过合成数据蒸馏框架比较指令微调(IFT)和推理模型在不同规模下的表现，发现推理模型在数学和通用任务上能匹配或超越更大规模的IFT系统，且随着模型规模扩大，推理模型在推理密集型任务上的价值更加显著。


<details>
  <summary>Details</summary>
Motivation: 尽管具有推理能力的大语言模型在多种任务上取得了最优表现，但推理能力在哪些任务和模型规模下有效，以及其训练和推理成本仍未被充分探索。

Method: 使用合成数据蒸馏框架进行大规模监督研究，比较不同规模的指令微调(IFT)和推理模型在数学中心和通用任务上的表现，评估多项选择和开放式两种格式。

Result: 推理持续提升模型性能，经常匹配或超越显著更大的IFT系统。IFT在训练和推理成本上保持帕累托最优，但推理模型随着模型规模扩大而价值增加，在推理密集型和开放式任务上克服IFT的性能限制。

Conclusion: 推理模型在大型模型上表现出显著优势，特别是在推理密集型任务中，虽然IFT在成本效益上更优，但推理能力为性能提升提供了重要途径。
...
TL;DR: 本文论证大型语言模型（LLM）的输出是无意义的，基于两个前提：LLM输出需要特定意图才能具有字面意义，而LLM不可能具备这种意图。


<details>
  <summary>Details</summary>
Motivation: 探讨大型语言模型输出是否真正具有意义，质疑当前对LLM语义能力的普遍认知。

Method: 通过哲学论证方法，分析意图在语义生成中的作用，并反驳语义外在主义和内在主义的反对观点。

Result: 论证表明LLM输出缺乏真正的字面意义，但承认这些输出在实用层面仍可被理解为有意义。
...
TL;DR: 提出RTP框架，利用LLM构建二进制树，通过自然语言问题对文本进行语义分区，创建可解释的文本分类法。


<details>
  <summary>Details</summary>
Motivation: 传统主题模型在数据稀缺领域表现不佳，且关键词列表需要大量人工解释，缺乏语义连贯性。

Method: RTP框架利用大语言模型交互式构建二进制树，每个节点都是自然语言问题，对数据进行语义分区。

Result: RTP的问题驱动层次结构比BERTopic等基线方法的关键词主题更具可解释性，在相关下游分类任务中表现优异。

Conclusion: RTP为数据探索引入新范式，从统计模式发现转向知识驱动的主题分析，其主题路径可作为生成模型的结构化可控提示。
...
TL;DR: 提出了StableToken，一种通过多分支架构和位级投票机制实现稳定性的语音分词器，显著提高了在噪声条件下的token稳定性，并改善了SpeechLLMs的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有的语义语音分词器对与语义无关的声学扰动非常脆弱，即使在高信噪比条件下，输出token序列也会发生剧烈变化，这增加了下游LLMs的学习负担。

Method: 采用多分支并行处理音频，通过位级投票机制合并这些表示，形成单一稳定的token序列。

Result: StableToken在token稳定性方面达到了新的最先进水平，显著降低了在各种噪声条件下的单元编辑距离（UED）。

Conclusion: 这种基础稳定性直接转化为下游优势，显著提高了SpeechLLMs在各种任务上的鲁棒性。
...
TL;DR: 提出Composite Reasoning(CR)方法，让大语言模型能够动态组合多种推理风格（演绎、归纳、溯因等），在科学和医学问答任务上超越现有基线方法。
...
TL;DR: 提出反向推测解码（RSD）方法，通过让教师模型生成候选标记但由学生模型基于自身概率分布决定是否接受，过滤低概率标记，解决大模型向小模型推理能力迁移失败的问题。


<details>
  <summary>Details</summary>
Motivation: 传统监督微调方法在将大语言模型的推理能力迁移到小模型时经常失败，性能反而下降，这源于分布不对齐问题——大模型的推理轨迹包含小模型分布下低概率的标记，超出了小模型的内部表示能力。

Method: 提出RSD方法：教师模型生成候选标记，但由学生模型基于自身概率分布决定是否接受，过滤掉对学生模型来说低概率的标记，生成学生友好的推理轨迹。

Result: 在Qwen3-0.6B上的实验显示：直接蒸馏s1K-1.1推理轨迹数据使平均性能下降20.5%，而使用RSD生成的推理轨迹训练则提升4.9%。跨模型实验表明RSD轨迹具有模型特异性。

Conclusion: 低概率标记是推理能力迁移的关键瓶颈，分布对齐必须针对每个学生架构的独特内部表示进行定制。
...
TL;DR: 提出了FeatBench基准测试，专门评估大语言模型在"氛围编程"范式下的功能实现能力，弥补现有代码生成基准测试的不足。


<details>
  <summary>Details</summary>
Motivation: 现有代码生成基准测试存在对齐问题，要么需要代码级规范，要么只关注问题解决，忽视了氛围编程范式中功能实现的关键场景。
...
Conclusion: FeatBench基准测试揭示了氛围编程中功能实现的挑战，为社区研究提供了新的评估工具和洞见。
...
TL;DR: 提出了FLEXI基准测试，这是首个专门评估全双工语音交互LLM在紧急场景下中断能力的基准，揭示了开源模型与商业模型在紧急意识、对话终止和交互延迟方面的显著差距。


<details>
  <summary>Details</summary>
Motivation: 全双工语音到语音LLM是人机自然交互的基础，但目前缺乏有效的基准测试和建模方法，特别是在紧急场景下的中断能力评估。

Method: 开发了FLEXI基准测试，通过六个多样化的人机交互场景系统评估实时对话的延迟、质量和对话效果，特别关注模型在紧急情况下的中断能力。

Result: 发现开源模型与商业模型在紧急意识、对话终止和交互延迟方面存在显著差距，商业模型表现更好。

Conclusion: 下一令牌对预测为实现真正无缝和类人全双工交互提供了有前景的路径。
...
TL;DR: 该论文从法律合规角度解决LLM安全问题，提出安全合规概念，使用欧盟AI法案和GDPR作为安全标准，开发了新基准并构建了合规推理器，在基准测试中性能显著提升。


<details>
  <summary>Details</summary>
Motivation: 现有LLM安全方法依赖临时分类法，缺乏系统化保护，无法确保现代LLM系统复杂行为的安全性，需要从法律合规角度建立严格的安全标准。

Method: 1) 开发基于法律条款的新安全合规基准；2) 使用组策略优化(GRPO)对齐Qwen3-8B模型，构建合规推理器；3) 将LLM与法律标准对齐以降低安全风险。

Result: 合规推理器在新基准上表现优异，欧盟AI法案平均提升+10.45%，GDPR平均提升+11.85%。

Conclusion: 从法律合规角度解决LLM安全问题是有效的，建立的合规推理器能够显著提升LLM的安全合规性能，为LLM安全提供了系统化解决方案。
...
TL;DR: SSKG-LLM模型通过整合知识图谱的结构和语义信息来增强大语言模型的事实推理能力，解决了LLMs处理知识图谱时忽略结构信息和嵌入空间不匹配的问题。


<details>
  <summary>Details</summary>
Motivation: 当前LLMs处理幻觉问题主要依赖知识图谱，但通常将KGs视为纯文本，仅提取语义信息而忽略了关键的结构方面。此外，KGs编码器和LLMs文本嵌入空间之间的差距阻碍了结构化知识的有效整合。

Method: 提出SSKG-LLM架构，包含知识图谱检索(KGR)模块、知识图谱编码(KGE)模块来保持语义并利用结构，以及知识图谱适应(KGA)模块使LLMs能够理解KGs嵌入。
...
TL;DR: 本文首次系统研究了仇恨言论检测中可解释性与公平性的关系，发现基于输入的解释能有效检测偏见预测并作为训练监督减少偏见，但在模型选择方面不可靠。
...
Result: 基于输入的解释能有效检测偏见预测，并可作为有用的监督信号在训练过程中减少偏见，但在候选模型中选择公平模型方面不可靠。
...
TL;DR: 系统评估了微调LLM将自然语言翻译为一阶逻辑的任务，发现微调Flan-T5-XXL在提供谓词列表时达到70%准确率，优于GPT-4o和符号系统，并揭示了谓词可用性、架构优势和泛化能力等关键发现。


<details>
  <summary>Details</summary>
Motivation: 自动化自然语言到一阶逻辑的翻译对于知识表示和形式方法至关重要，但仍然是具有挑战性的任务。

Method: 使用MALLS和Willow数据集，比较编码器-解码器与仅解码器架构，探索词汇扩展、谓词条件化和多语言训练等技术，引入精确匹配、逻辑等价和谓词对齐等评估指标。

Result: 微调Flan-T5-XXL在提供谓词列表时达到70%准确率，优于GPT-4o和DeepSeek-R1-0528模型以及ccg2lambda等符号系统。谓词可用性提升性能15-20%，T5模型优于更大的仅解码器LLM，模型能在未经特定训练的情况下泛化到未见过的逻辑参数。
...
TL;DR: 本文研究了不同规模的transformer模型学习传递关系（如图连通性）的能力，发现模型在低维网格状图上表现良好，但在高维图或包含多个不连通分量的图上表现较差，且模型规模扩大有助于提升泛化能力。


<details>
  <summary>Details</summary>
Motivation: 研究transformer模型从训练样本中学习传递关系的能力，以及模型规模对这种能力的影响，因为传递关系推理对确保LLMs的事实正确性至关重要。

Method: 生成有向图来训练不同规模的transformer模型，评估它们在不同图大小下推断传递关系的能力，特别关注网格状图的维度特性。

Result: transformer能够学习低维网格状图的连通性，但高维网格图更具挑战性；模型规模增加能改善泛化能力；对于包含多个不连通分量的非网格图，模型学习困难。

Conclusion: transformer学习传递关系的能力受图结构维度影响，低维网格图易学，高维图难学，模型规模扩大有助于提升性能，但对于复杂非网格图仍存在挑战。
...
TL;DR: 该论文介绍了InviTE语料库，包含近2000个早期现代英语句子，用于研究都铎王朝英格兰宗教改革中的辱骂语言。比较了微调的BERT模型和零样本提示的LLMs，发现预训练历史数据并微调检测辱骂的模型表现更优。
...
Method: 构建了从原始数据到预处理、数据选择和迭代标注的工作流程，创建了InviTE语料库，并评估了微调BERT模型与零样本提示LLMs的性能。

Result: 引入了包含近2000个早期现代英语句子的InviTE语料库，并发现预训练历史数据并微调检测辱骂的模型优于零样本提示的LLMs。

Conclusion: 预训练历史数据并针对特定任务微调的模型在检测历史文本中的辱骂语言方面表现更佳，为零样本方法提供了有价值的比较基准。
...
<details>
  <summary>Details</summary>
Motivation: 随着贝叶斯概率理论在认知科学中的应用发展，以及新一代概率计算工具的出现，语用学和语义学出现了'概率转向'。本文旨在探索如何将类似的贝叶斯方法应用于关联理论语用学。

Method: 采用贝叶斯方法研究关联理论语用学，特别关注通过会话含义传达隐含意义的典型语用现象。

Result: 研究表明贝叶斯方法可以应用于关联理论语用学，为理解会话含义等语用现象提供新的计算框架。

Conclusion: 贝叶斯方法为关联理论语用学提供了有前景的计算建模途径，能够更好地理解和解释隐含意义的传达机制。
...
Method: 从古腾堡计划中整理英语书籍文本，添加各种时间标注，通过时间敏感的VAD分析量化词汇语义变化，构建历史校准的情感词典。

Result: 展示了现代LLM工具需要更好地跨时间情境化歧视性语言检测和情感分析，语言模型在CHRONOBERG上顺序训练时难以编码历时意义变化。

Conclusion: 需要时间感知的训练和评估流程，CHRONOBERG作为可扩展资源，支持语言变化研究和时间泛化研究。
...
TL;DR: 提出一个使用大语言模型对风力涡轮机维护日志进行深度语义分析的框架，超越了传统文本分类，能够执行故障模式识别、因果链推断等复杂推理任务。


<details>
  <summary>Details</summary>
Motivation: 风力涡轮机维护日志中的大量非结构化文本包含丰富的运维智能，但传统定量可靠性分析方法无法有效利用这些信息。现有机器学习方法通常只停留在文本分类层面，缺乏复杂推理能力。

Method: 引入探索性框架，利用现代大语言模型进行深度语义分析，执行四种分析工作流：故障模式识别、因果链推断、比较站点分析和数据质量审计。

Result: 结果表明大语言模型可以作为强大的"可靠性协作者"，不仅能标记文本，还能综合文本信息并生成可操作的专家级假设。

Conclusion: 这项工作贡献了一种新颖且可复现的方法论，将大语言模型用作推理工具，为风能行业提供了一条增强运维智能的新途径，解锁了非结构化数据中隐藏的洞察。
...
TL;DR: 分析了OLMO2开源模型训练数据中的政治偏见，发现左倾文档在数据集中占主导地位，且训练数据中的主要政治立场与模型在政策问题上的偏见密切相关。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型已知会产生政治偏见文本，但这些偏见如何产生尚不清楚。当前LLM研究中对训练数据政治内容的分析仍不足，需要填补这一空白。

Method: 从OLMO2模型的完整训练数据集中抽取大样本，自动标注文档的政治倾向，分析其来源领域和内容，并评估训练数据中的政治内容与模型在具体政策问题上立场的相关性。

Result: 左倾文档在所有数据集中占主导；预训练语料比后训练数据包含更多政治参与内容；左右倾向文档通过不同的价值观和合法性来源来构建相似主题；训练数据中的主要立场与模型政治偏见强相关。

Conclusion: 需要在未来数据整理流程中整合政治内容分析，并对过滤策略进行深入文档记录以提高透明度。
...
TL;DR: 提出了Chimera测试套件，包含7,500个维基百科图表，用于评估视觉语言模型对图表的真实理解能力，发现现有模型主要依赖三种捷径而非真正理解。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型在图表相关基准上表现良好，但可能依赖知识、推理或模态捷径，而非真正理解和推理图表内容。

Method: 构建Chimera测试套件，包含7,500个高质量图表，每个图表标注语义三元组和多层次问题，评估实体识别、关系理解、知识基础和视觉推理四个维度。

Result: 评估15个开源视觉语言模型发现，其强劲表现主要来自三种捷径：视觉记忆捷径影响较小，知识回忆捷径作用中等，Clever-Hans捷径贡献显著。

Conclusion: 当前视觉语言模型在复杂视觉输入理解方面存在严重局限，需要更鲁棒的评估协议来基准真实理解能力而非问答捷径。
...
TL;DR: 该研究评估了LLaMA和Gemini在多语言法律任务中的表现，发现法律任务对LLM构成显著挑战，准确率常低于50%，且存在提示敏感性和对抗脆弱性问题。
...
Method: 使用LLM-as-a-Judge方法进行人类对齐评估，在多语言法律和非法律基准上测试LLaMA和Gemini，通过字符和词级扰动评估对抗鲁棒性，并开发了开源模块化评估管道。

Result: 法律推理基准LEXam上的准确率常低于50%，而通用任务XNLI超过70%；英语结果更稳定但不总是更准确；LLaMA比Gemini弱约24个百分点；语言性能与其与英语的句法相似性相关。
...
TL;DR: NeLLCom-Lex是一个神经代理框架，通过将代理置于真实词汇系统中并操纵其沟通需求来模拟语义变化。使用颜色命名任务，该框架在一个世代内模拟词汇系统的演化，研究哪些因素导致代理发展出类似人类的命名行为和词汇变化。


<details>
  <summary>Details</summary>
Motivation: 传统观察方法无法揭示语义变化的因果机制，而人类实验范式难以应用于历时语义变化过程。需要一种能够模拟语义变化机制的计算框架。

Method: 使用神经代理框架NeLLCom-Lex，将代理置于真实词汇系统（如英语）中，通过监督学习和强化学习管道训练代理'说'现有语言，并在颜色命名任务中研究其行为变化。

Result: 实验表明，经过训练的神经代理能够在颜色命名任务中显著再现类似人类的模式，支持进一步使用NeLLCom-Lex来阐明语义变化的机制。

Conclusion: NeLLCom-Lex框架能够有效模拟语义变化过程，为研究词汇语义变化的因果机制提供了新的计算工具。
...
TL;DR: 本文提出解决方案分歧度作为衡量LLM问题解决能力的新指标，发现更高的分歧度与更好的性能正相关，并证明该指标能有效提升监督微调和强化学习的训练效果。


<details>
  <summary>Details</summary>
Motivation: 现有研究主要通过监督微调或强化学习来提升LLM的问题解决能力，本文从新的视角研究LLM对同一问题生成不同解决方案的多样性，探索解决方案分歧度与模型性能的关系。

Method: 提出解决方案分歧度作为新指标，在三个代表性问题领域测试该指标对监督微调和强化学习策略的支持效果。

Result: 实验表明，解决方案分歧度与问题解决能力呈正相关，使用该指标能持续提高任务成功率。
...
TL;DR: JGU Mainz团队为WMT25有限资源斯拉夫语言任务提交了基于Qwen2.5-3B-Instruct的联合微调方案，针对乌克兰语、上索布语和下索布语的机器翻译和问答任务，通过参数高效微调、数据增强和集成方法取得了优于基线的结果。


<details>
  <summary>Details</summary>
Motivation: 针对斯拉夫语言（乌克兰语、上索布语、下索布语）在有限资源条件下的机器翻译和问答任务挑战，需要开发高效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法在可扩展性、成本和对新模型数据集的适应性方面存在局限，且产生的表示难以解释。

Method: 利用几何特性的闭式计算，将LLMs表示为提示语义任务空间中的线性算子，无需训练过程。

Result: 在成功预测和模型选择任务中取得竞争性或最先进的结果，在样本外场景表现尤为突出。
...
TL;DR: 提出了自适应多分支引导（AMBS）方法，通过两阶段1-to-N框架解决多目标对齐中的灾难性遗忘和推理碎片化问题，在多个7B LLM上显著提升了HHH对齐效果。


<details>
  <summary>Details</summary>
Motivation: 现有方法在优化单个对齐目标时会覆盖其他目标的表示（灾难性遗忘），而简单的多分支设计会导致不同目标间的输出不一致（推理碎片化），需要统一高效的多目标对齐方案。

Method: 两阶段1-to-N框架：阶段I计算共享的Transformer层后注意力隐藏状态，阶段II通过策略参考机制在并行分支中进行目标特定引导，保持跨目标一致性。

Result: 在Alpaca、BeaverTails和TruthfulQA上的实验表明，AMBS在多个7B LLM上持续改善HHH对齐。在DeepSeek-7B上，平均对齐分数提升+32.4%，不安全输出减少11.0%。

Conclusion: AMBS通过共享表示和策略参考机制，有效解决了多目标对齐中的灾难性遗忘和推理碎片化问题，在保持竞争力的同时显著提升了对齐效果。
...
TL;DR: 提出了一种端到端的FP8训练方法，通过细粒度混合粒度量化策略，在保持数值精度的同时显著提升训练效率，实现了与BF16基线相当的性能但训练时间减少22%、内存使用降低14%、吞吐量提升19%。


<details>
  <summary>Details</summary>
Motivation: 大语言模型训练的巨大计算成本是创新的主要障碍，虽然FP8训练提供了显著的理论效率增益，但由于缺乏全面开源训练方案而未能广泛采用。

Method: 采用端到端FP8训练方案，结合持续预训练和监督微调，使用细粒度混合粒度量化策略来平衡数值保真度和计算效率。

Result: 在1600亿token语料上的持续预训练实验表明，该方法不仅稳定性好且基本无损失，在推理基准测试中与BF16基线性能相当，同时训练时间减少22%、峰值内存使用降低14%、吞吐量提升19%。

Conclusion: FP8是BF16的实用且稳健的替代方案，将发布相关代码以进一步普及大规模模型训练。
...
TL;DR: 提出认知推理范式CogFlow，通过结构化认知流程增强LLMs的社会认知能力，结合监督微调和强化学习优化社交决策


<details>
  <summary>Details</summary>
Motivation: 传统逻辑推理范式不适合处理社交情境中的模糊线索和解释性过程，需要模拟人类社会认知的推理方式

Method: 1. 设计认知推理范式，将解释过程结构化；2. 通过树状规划模拟人类思维生成认知流程数据集；3. 监督微调+强化学习多目标优化

Result: CogFlow有效增强LLMs和人类的社会认知能力，提升社交决策效果

Conclusion: 认知推理范式成功弥补了LLMs在社交推理方面的不足，为复杂社交情境提供有效解决方案
...
TL;DR: 开发了一个基于检索增强的评估管道(RAEC)，利用历史消息-响应对来改进LLM生成的患者消息回复的质量评估，通过两阶段DSPy管道实现可扩展的错误检测。


<details>
  <summary>Details</summary>
Motivation: EHR门户中的异步患者-临床医生消息增加了临床医生工作量，LLM可用于协助起草回复，但可能存在临床不准确、遗漏或语气不匹配等问题，需要稳健的评估方法。

Method: 引入包含5个领域和59个错误代码的临床基础错误本体论；开发检索增强评估管道(RAEC)，利用语义相似的历史消息-响应对；采用两阶段DSPy提示架构进行分层错误检测。

Result: 在1,500多条患者消息上测试，检索上下文改进了临床完整性和工作流程适当性等领域的错误识别。人工验证显示上下文增强标签在一致性(50% vs 33%)和性能(F1=0.500 vs 0.256)上优于基线。
...
TL;DR: 本文研究使用LLMs定位上下文幻觉（模型输出中无法从源文本验证的信息），构建了专门的基准测试，并提出基于自由文本描述的新表示方法。研究发现最佳模型F1分数仅为0.67，揭示了LLMs在该任务中的主要挑战。


<details>
  <summary>Details</summary>
Motivation: 研究LLMs在定位上下文幻觉方面的适用性，作为现有复杂评估流程的更实用替代方案。由于缺乏针对幻觉定位的元评估基准，需要构建专门测试集。

Method: 构建了包含1000多个示例的基准测试，采用人类标注；提出基于自由文本描述的幻觉表示方法；评估了四个大规模LLMs；分析最优提示策略。

Result: 基准测试难度较高，最佳模型F1分数仅为0.67。发现LLMs主要挑战：倾向于错误标记缺失细节为不一致；难以处理包含参数知识但源文本中不存在的事实正确信息。

Conclusion: LLMs在定位上下文幻觉方面表现有限，需要改进提示策略和解决模型参数知识干扰问题。提出的基准测试和表示方法为该领域研究提供了基础。
...
<details>
  <summary>Details</summary>
Motivation: 收集阿拉伯语招聘广告数据，以捕捉阿拉伯劳动力市场中的语言、区域和社会经济变化，为公平意识阿拉伯NLP和劳动力市场研究提供资源。

Method: 从四个阿拉伯国家收集8500多个招聘广告，构建包含55万词的语料库，使用大型语言模型进行薪资估算和职位类别标准化等应用分析。

Result: 数据集成功捕捉了阿拉伯劳动力市场的性别代表性、职业结构和方言变化，为性别偏见检测和职业分类等基准任务提供了基础。

Conclusion: ArabJobs语料库对公平意识阿拉伯NLP和劳动力市场研究具有重要价值，数据集已在GitHub上公开提供。
...
TL;DR: 证明了所有标准次正则语言类在通过其判定谓词表示时都是线性可分的，这建立了有限可观测性并保证了使用简单线性模型的可学习性。
...
Method: 使用判定谓词表示次正则语言类，通过线性模型进行学习，并在合成数据和英语形态学真实数据上进行实验验证。

Result: 在无噪声条件下实现完美可分性，真实数据实验显示学习到的特征与已知语言约束一致。

Conclusion: 次正则层次结构为自然语言结构建模提供了严格且可解释的基础，证明了其在语言学习中的有效性。
...
TL;DR: 本文比较了多种NLP技术在模拟审议过程中的表现，提出了两种优于现有方法的模型：基于频率的话语调制和量子审议框架，可用于政策制定、辩论评估等领域。


<details>
  <summary>Details</summary>
Motivation: 随着NLP技术的发展，计算建模审议过程成为可能，通过分析观点转变和预测不同情景下的结果，为决策提供支持。

Method: 收集不同背景个体的意见构建数据集，使用包含突出事实的产品演示模拟审议过程，比较了基于频率的话语调制和量子审议框架两种模型。

Result: 提出的两种模型在解释审议性话语和产生有意义的见解方面优于现有最先进模型，能够有效捕捉观众观点的可测量转变。

Conclusion: 该研究展示了NLP技术在建模审议过程中的实际应用价值，特别是在公共政策制定、辩论评估和决策支持框架等领域具有重要潜力。
...
TL;DR: 提出基于重采样的方法来量化多语言/多任务NLP基准测试中评估指标的不确定性和统计精度，考虑模型和数据来源的变异，避免低估总体变异性。


<details>
  <summary>Details</summary>
Motivation: 现有方法往往低估NLP基准测试中性能指标的变异性，需要同时考虑模型和数据来源的变异来准确量化不确定性。

Method: 使用重采样方法，同时考虑模型和数据相关的变异源，计算评估指标的不确定性和统计精度。

Result: 在多语言问答、机器翻译和命名实体识别等任务中，该方法能准确计算排行榜中各种统计量的抽样分布，如平均值/中位数、模型间差异和排名。

Conclusion: 重采样方法能有效量化NLP基准测试中的不确定性，为模型比较和排行榜提供更可靠的统计基础。
...
TL;DR: StateX是一种通过后训练高效扩展预训练RNN状态大小的训练流程，能够提升循环神经网络的长上下文召回和上下文学习能力，而不会显著增加参数或训练成本。


<details>
  <summary>Details</summary>
Motivation: Transformer模型处理长上下文成本高昂，而RNN虽然复杂度低但状态大小固定，难以准确召回长上下文信息。直接训练大状态RNN成本高，需要一种高效扩展状态的方法。

Method: 针对线性注意力和状态空间模型两类RNN，设计了后训练架构修改来扩大状态大小，几乎不增加模型参数。通过StateX训练流程实现状态扩展。

Result: 在1.3B参数规模的模型上实验表明，StateX能有效提升RNN的召回能力和上下文学习能力，且后训练成本低，不影响其他能力。

Conclusion: StateX提供了一种高效扩展RNN状态大小的方法，解决了RNN在长上下文处理中的局限性，为RNN的实际应用提供了可行方案。
...
<details>
  <summary>Details</summary>
Motivation: 为语言模型的推理能力提供原则性的概率视角，通过变分推理方法改进推理过程，并揭示现有方法中未被注意的偏差。

Method: 基于证据下界(ELBO)扩展为多轨迹目标，提出前向KL公式稳定变分后验训练，将拒绝采样微调和二元奖励RL解释为局部前向KL目标。

Result: 在Qwen 2.5和Qwen 3模型系列上验证了方法的有效性，在广泛推理任务中表现良好。

Conclusion: 该工作提供了一个统一变分推理与强化学习方法的概率框架，为改进语言模型推理能力提供了稳定目标。
...
TL;DR: 提出反馈条件策略(FCP)，将语言反馈作为条件信号直接学习，而非压缩为标量奖励，通过离线最大似然训练和在线自举阶段实现反馈驱动的条件生成。
...
Method: FCP从响应-反馈对中学习，通过最大似然训练近似反馈条件后验，并引入在线自举阶段在正反馈条件下生成并接收新反馈进行优化。

Result: 该方法将反馈驱动学习重新定义为条件生成而非奖励优化，为LLM提供了更丰富的从语言反馈中学习的方式。

Conclusion: FCP通过条件生成框架更有效地利用语言反馈，代码已开源。
...
TL;DR: 该研究通过专家标注发现，虽然n-gram新颖性与创造力正相关，但91%的高n-gram新颖性表达不被专家评为有创造力。开源LLM的高n-gram新颖性会降低实用性，前沿闭源模型产生创造性表达的可能性低于人类。


<details>
  <summary>Details</summary>
Motivation: 研究n-gram新颖性作为创造力评估指标的局限性，因为创造力需要同时考虑新颖性和适当性（实用性和合理性）。

Method: 通过7542个专家标注（26名专家）对人工和AI生成文本的新颖性、实用性和合理性进行细读评估，并测试不同LLM识别创造性表达和非实用性表达的能力。

Result: n-gram新颖性单独使用不可靠；开源LLM的新颖性与实用性负相关；前沿LLM识别创造性表达的能力优于随机但仍有改进空间，特别是在识别非实用性表达方面。

Conclusion: 不应仅依赖n-gram新颖性评估创造力，LLM在识别创造性表达方面还有提升空间，最佳模型的LLM-as-a-Judge新颖性评分能预测专家偏好。
...
TL;DR: WebGen-Agent是一个利用多级视觉反馈来迭代生成和优化网站代码库的智能体系统，通过视觉语言模型分析截图和GUI测试结果，结合回溯和最佳选择机制，显著提升了网站生成质量。


<details>
  <summary>Details</summary>
Motivation: 当前基于LLM的代码生成智能体主要依赖简单的代码执行反馈，无法准确评估生成网站的实际视觉质量和用户体验，特别是在需要视觉效果和交互反馈的网站生成任务中表现不足。

Method: 提出WebGen-Agent系统，使用视觉语言模型对网站截图和GUI测试结果进行详细描述和评分，结合回溯和最佳选择机制；并引入Step-GRPO训练方法，将截图和GUI测试得分作为过程监督奖励信号。

Result: 在WebGen-Bench数据集上，WebGen-Agent将Claude-3.5-Sonnet的准确率从26.4%提升到51.9%，外观得分从3.0提升到3.9；Step-GRPO训练使Qwen2.5-Coder-7B-Instruct的准确率从38.9%提升到45.4%，外观得分从3.4提升到3.7。

Conclusion: 多级视觉反馈机制和过程监督训练方法能显著提升LLM在网站生成任务中的表现，为代码生成智能体提供了更有效的质量评估和优化途径。
...
<details>
  <summary>Details</summary>
Motivation: 现有基准测试不足以评估大型语言模型和多模态系统的完整能力范围，需要更全面的评估框架。

Method: 构建包含10,497个示例的基准测试，涵盖自然声音、音乐、对话等听力任务，多轮对话、角色扮演等说话任务，以及异质图像观看任务。评估了21个开源模型和GPT-4o-Audio。

Result: 发现专有模型并非全面优于开源模型；大多数模型擅长说话任务但音频理解能力较弱；设计良好的较小模型可以与更大模型相媲美。Step-Audio-2-mini在听力准确率上超过LLaMA-Omni2-32B-Bilingual两倍以上。

Conclusion: 当前模型在多模态输入和角色扮演语音模仿任务上仍有困难，在鲁棒性和安全对齐方面存在显著差距。VoiceAssistant-Eval为下一代AI助手的发展提供了严格的评估框架。
...
### [269] [QueryGym: Step-by-Step Interaction with Relational Databases](https://arxiv.org/abs/2509.21674)
...
<details>
  <summary>Details</summary>
Motivation: 现有框架通常将代理绑定到特定查询语言方言或隐藏其推理过程，缺乏透明性和通用性。

Method: 实现为Gymnasium接口，提供模式细节、中间结果和执行反馈等观察，接收数据库探索和关系代数操作等动作。
...
### [270] [Unbiased Binning: Fairness-aware Attribute Representation](https://arxiv.org/abs/2509.21785)
*Abolfazl Asudeh,Zeinab,Asoodeh,Bita Asoodeh,Omid Asudeh*

Main category: cs.DB

TL;DR: 本文提出了无偏分箱问题，旨在找到最接近等宽分箱且满足组间公平性的离散化方法，并开发了高效的动态规划算法。为了解决公平性代价过高或不存在无偏分箱的情况，进一步引入了ε-偏分箱问题，提出了可扩展的局部搜索算法。


<details>
  <summary>Details</summary>
Motivation: 特征离散化是数据共享前的常见步骤，但这一过程可能导致数据偏见并加剧下游任务的不公平性。现有方法在保证组间公平性方面存在不足，特别是在组值分布不同时难以找到无偏分箱。

Method: 首先定义边界候选集，证明无偏分箱必须从中选择边界，并开发基于动态规划的高效算法。对于ε-偏分箱问题，提出动态规划解法和基于局部搜索的可扩展算法，其中关键组件是分治算法。

Result: 证明了无偏分箱边界必须来自特定候选集，开发了多项式时间的动态规划算法。对于大规模设置，提出的局部搜索算法能够在近线性时间内找到近似最优解。

Conclusion: 本文为解决特征离散化中的公平性问题提供了理论保证和实用算法，在保证公平性的同时保持了计算效率，适用于大规模数据场景。
...
### [271] [The system of processing and analysis of customer tracking data for customer journey research on the base of RFID technology](https://arxiv.org/abs/2509.22162)
*Marina Kholod*

Main category: cs.DB

TL;DR: 该论文研究基于RFID技术的追踪数据分析系统，用于研究零售业客户旅程，包括数据收集架构、ETL处理方法和集成分析模型。


<details>
  <summary>Details</summary>
Motivation: 研究如何利用RFID技术超越传统物流应用，实现精确库存管理、防损和客户体验提升，通过整合追踪和交易数据推动零售业向数据驱动科学转型。

Method: 提出数据收集、处理和集成架构，采用ETL方法将原始RFID和POS数据转换为结构化分析数据仓库，设计结合财务指标与客户行为模式的逻辑数据库模型。

Result: 建立了综合RFID追踪和POS交易数据的分析系统，能够提供产品流动和消费者行为的全面可见性。

Conclusion: RFID与交易数据的整合为零售业向精确、数据驱动科学转型奠定了基础，通过平衡计分卡评估显示在财务绩效、客户满意度和内部流程优化方面具有显著效益。
...
### [272] [I-ETL: an interoperability-aware health (meta) data pipeline to enable federated analyses](https://arxiv.org/abs/2509.22351)
*Nelly Barret,Anna Bernasconi,Boris Bikbov,Pietro Pinoli*

Main category: cs.DB

TL;DR: I-ETL框架用于整合高度异质的医疗数据集，通过两个可扩展的概念模型和数据-元数据建模，以及确保互操作性的ETL流程，支持联邦学习环境下的医疗数据协作。


<details>
  <summary>Details</summary>
Motivation: 临床医生需要共享和分析复杂疾病数据，但医疗机构的异构数据格式和隐私要求限制了合作。联邦学习虽能保护隐私，但需要数据格式统一，而医疗机构缺乏数据标准化能力。

Method: 设计I-ETL框架，包含两个通用概念模型（数据模型和元数据模型），以及确保互操作性的ETL（提取-转换-加载）流程。

Result: 在开源数据集上的实验表明，I-ETL能成功统一表示各种健康数据集，并证明将互操作性作为集成流程首要考虑的重要性。

Conclusion: I-ETL框架有效解决了医疗数据异构性问题，支持不同医疗机构间的协作，为联邦学习在医疗领域的应用提供了可行方案。
...
### [273] [Towards mitigating information leakage when evaluating safety monitors](https://arxiv.org/abs/2509.21344)
*Gerard Boxo,Aman Neelappa,Shivam Raval*

Main category: cs.AI

TL;DR: 论文提出了一个系统框架来评估白盒监控器的真实性能，识别了两种导致性能虚高的泄漏问题，并提出了三种缓解策略。


<details>
  <summary>Details</summary>
Motivation: 现有的白盒监控器在训练和评估时，用于引发目标行为的信息会泄漏到监控器输入数据中，导致性能评估不准确。

Method: 提出了内容过滤、分数过滤和提示蒸馏微调模型生物三种策略来评估监控器的真实检测能力。

Result: 实验发现内容过滤可使AUROC降低30%，分数过滤降低15%，微调模型生物降低性能达40%。

Conclusion: 需要更严谨的评估方法来避免监控器性能的虚高，内容过滤是有效的缓解策略。
...
### [274] [Correct Reasoning Paths Visit Shared Decision Pivots](https://arxiv.org/abs/2509.21549)
...
Method: 使用决策支点作为可验证检查点，通过采样推理路径、挖掘共享支点、压缩推理路径并自训练模型。

Result: 在LogiQA、MedQA和MATH500等基准测试中验证了方法的有效性。

Conclusion: 决策支点方法能够有效对齐推理过程，提升模型推理能力。
...
### [275] [AutoClimDS: Climate Data Science Agentic AI -- A Knowledge Graph is All You Need](https://arxiv.org/abs/2509.21553)
*Ahmed Jaber,Wangshu Zhu,Karthick Jayavelu,Justin Downes,Sameer Mohamed,Candace Agonafir,Linnia Hawkins,Tian Zheng*

Main category: cs.AI

TL;DR: 提出了一个结合知识图谱和AI代理的系统，旨在解决气候数据科学中的数据碎片化、格式异构和技术门槛高等问题，通过自然语言交互和自动化工作流降低使用门槛。


<details>
  <summary>Details</summary>
Motivation: 气候数据科学面临数据源分散、格式不统一、技术门槛高等障碍，限制了参与度、减慢了发现速度并降低了科学工作流的可重复性。
...
Conclusion: 该系统为民主化气候数据访问和建立可重复、可扩展的人类-AI协作科学研究框架开辟了道路。
...
### [276] [EEG-Based Consumer Behaviour Prediction: An Exploration from Classical Machine Learning to Graph Neural Networks](https://arxiv.org/abs/2509.21567)
*Mohammad Parsa Afshar,Aryan Azimi*

Main category: cs.AI

TL;DR: 该研究使用脑电图数据和机器学习模型预测消费者行为，比较了传统机器学习模型和图神经网络模型的表现。
...
Method: 从NeuMa数据集中提取和清理脑电图特征，为图神经网络创建大脑连通性特征，使用传统机器学习模型和图神经网络模型进行比较。

Result: 虽然结果整体上没有显著差异，但图神经网络在一些基本标准上表现更好，而传统模型在这些方面表现不佳。

Conclusion: 结合脑电图信号分析和机器学习模型可以更深入地理解消费者行为，并为脑电图神经营销领域提供了传统模型与图神经网络的全面比较。
...
### [277] [GeoEvolve: Automating Geospatial Model Discovery via Multi-Agent Large Language Models](https://arxiv.org/abs/2509.21593)
...
<details>
  <summary>Details</summary>
Motivation: 现有基于LLM的算法发现框架缺乏领域知识和多步推理能力，难以解决复杂的地理空间问题，需要开发能够注入地理空间理论知识的自动化算法设计方法。

Method: 采用两层嵌套循环：内环使用代码进化器生成和变异候选解，外环通过智能体控制器评估全局精英，并查询GeoKnowRAG模块（结构化地理空间知识库）来注入地理学理论先验。

Result: 在空间插值（克里金法）和空间不确定性量化（地理空间保形预测）任务上，GeoEvolve将空间插值误差（RMSE）降低了13-21%，并将不确定性估计性能提升了17%。
...
### [278] [Automated and Interpretable Survival Analysis from Multimodal Data](https://arxiv.org/abs/2509.21600)
...
### [279] [Semantic F1 Scores: Fair Evaluation Under Fuzzy Class Boundaries](https://arxiv.org/abs/2509.21633)
...
<details>
  <summary>Details</summary>
Motivation: 传统F1指标将语义相关的预测视为完全失败，无法反映现实中类别重叠、标注者分歧和模糊类别边界的情况。

Method: 使用标签相似性矩阵计算软精度和软召回分数，通过新颖的两步精度-召回公式，无需丢弃标签或强制匹配不相似标签即可比较任意大小的标签集。

Result: 通过理论论证和实证验证，语义F1分数显示出更好的可解释性和生态有效性，能够为语义相关但不完全相同的标签提供部分信用。

Conclusion: 语义F1分数提供了更公平的评估，仅需领域适当的相似性矩阵即可应用于各种任务和模态，且对错误规范具有鲁棒性。
...
### [280] [Can AI Perceive Physical Danger and Intervene?](https://arxiv.org/abs/2509.21651)
*Abhishek Jindal,Dmitry Kalashnikov,Oscar Chang,Divya Garikapati,Anirudha Majumdar,Pierre Sermanet,Vikas Sindhwani*

Main category: cs.AI

TL;DR: 开发了一个用于具身AI系统物理安全基准测试的可扩展方法，基于真实世界伤害叙述和操作安全约束，通过生成模型创建从安全到不安全状态的图像和视频，评估主流基础模型的风险感知能力，并开发了后训练范式提升安全推理能力。


<details>
  <summary>Details</summary>
Motivation: 当AI与物理世界交互时（如机器人或辅助代理），会出现超越"数字AI"的新安全挑战，存在直接和即时的物理伤害风险。需要评估基础模型对物理安全常识的理解程度。

Method: 1) 基于真实伤害叙述和操作约束开发可扩展的物理安全基准测试方法；2) 使用生成模型创建从安全到不安全状态的多模态内容；3) 分析主流基础模型的风险感知和干预能力；4) 开发后训练范式，通过系统指令教授模型具身特定的安全约束推理。

Result: 全面分析了主要基础模型的风险感知、安全推理和干预触发能力，为安全关键应用部署提供了多方面的洞察。开发的后训练模型在约束满足评估中达到最先进性能，生成可解释的安全推理轨迹。

Conclusion: 提出了一个系统性的物理安全基准测试框架，揭示了当前基础模型在物理安全理解方面的局限性，并开发了有效的方法来提升模型的安全推理能力，为具身AI系统的安全部署提供了重要基础。
...
### [281] [Align2Speak: Improving TTS for Low Resource Languages via ASR-Guided Online Preference Optimization](https://arxiv.org/abs/2509.21718)
...
TL;DR: 提出基于GRPO的框架，通过多语言预训练、有限配对数据微调和GRPO优化，在低资源语言中实现高质量的文本转语音合成。


<details>
  <summary>Details</summary>
Motivation: 解决低资源语言TTS系统开发困难的问题，利用ASR模型相对容易获取的优势来提升TTS性能。

Method: 1. 使用IPA标记训练多语言TTS基础模型；2. 用有限配对数据微调以捕捉目标语言韵律特征；3. 应用GRPO仅使用非配对文本和说话人提示进行优化，由预训练ASR、说话人验证和音频质量评估模型提供多目标奖励。

Result: 在低资源语言中产生可理解且说话人一致的语音，显著优于仅微调方法；在高资源语言中也超越DPO等离线对齐方法，在可理解性、说话人相似性和音频质量方面表现更优。

Conclusion: GRPO框架能有效提升低资源和高资源语言的TTS性能，为多语言TTS系统开发提供了可行方案。
...
### [282] [Retrieval-of-Thought: Efficient Reasoning via Reusing Thoughts](https://arxiv.org/abs/2509.21743)
*Ammar Ahmed,Azal Ahmad Khan,Ayaan Ahmad,Sheng Di,Zirui Liu,Ali Anwar*

Main category: cs.AI

TL;DR: RoT通过检索和重用先前的推理步骤作为可组合的"思想"来指导新问题，构建思想图并通过奖励引导的遍历组装问题特定的模板，显著减少输出token和推理延迟，同时保持准确性。
...
Method: 提出RoT方法，将推理步骤组织成具有顺序和语义边的思想图，通过检索查询相关节点和应用奖励引导遍历来组装问题特定的生成模板。

Result: 在多个模型和推理基准上的评估显示，RoT最多减少40%的输出token、82%的推理延迟和59%的成本，同时保持准确性。
...
### [283] [Lifelong Learning with Behavior Consolidation for Vehicle Routing](https://arxiv.org/abs/2509.21765)
*Jiyuan Pei,Yi Mei,Jialin Liu,Mengjie Zhang,Xin Yao*

Main category: cs.AI

TL;DR: 提出LLR-BC框架，解决神经VRP求解器在终身学习中的灾难性遗忘问题，通过行为对齐和决策重要性加权来维持旧任务性能并学习新任务。


<details>
  <summary>Details</summary>
Motivation: 现有神经求解器在遇到新任务时，要么依赖零样本泛化（效果差），要么微调导致灾难性遗忘。需要一种终身学习范式来顺序学习多个任务。

Method: 提出LLR-BC框架，通过行为对齐将新任务训练的求解器行为与缓冲的旧任务行为对齐，并对低置信度决策赋予更大权重以关注关键经验。

Result: 在容量约束车辆路径问题和旅行商问题上的实验表明，LLR-BC能有效训练高性能神经求解器，解决灾难性遗忘，保持可塑性并提升零样本泛化能力。

Conclusion: LLR-BC为神经VRP求解器提供了一种有效的终身学习解决方案，能够在顺序学习多个任务时维持旧任务性能并学习新任务。
...
### [284] [UltraHorizon: Benchmarking Agent Capabilities in Ultra Long-Horizon Scenarios](https://arxiv.org/abs/2509.21766)
...
TL;DR: 提出了UltraHorizon基准测试，用于评估自主代理在长视野、部分可观测场景下的持续推理、规划、记忆管理和工具使用能力，发现现有LLM代理在这些复杂任务中表现不佳。


<details>
  <summary>Details</summary>
Motivation: 现有评估主要关注短视野、完全可观测任务，而现实世界中的关键任务（如软件开发、投资、科学发现）需要在长视野、部分可观测场景下进行，现有基准测试很少捕捉这些挑战。

Method: 通过探索作为统一任务，在三个不同环境中设计长视野发现任务，代理必须通过持续推理、规划、记忆和工具管理以及与环境交互来迭代发现隐藏规则。

Result: 在最重规模设置下，轨迹平均超过20万token和400+工具调用，标准配置下也超过3.5万token和60+工具调用。实验显示LLM代理在这些设置下持续表现不佳，而人类参与者得分更高。

Conclusion: 简单扩展在任务中失败，分析识别出八种错误类型，归因于两个主要原因：上下文锁定和功能基础能力差距。
...
### [285] [Benchmarking MLLM-based Web Understanding: Reasoning, Robustness and Safety](https://arxiv.org/abs/2509.21782)
...
Result: 评估12个MLLM发现显著差距：模型在处理真实布局的组合和跨元素推理时仍有困难，面对UI和内容扰动时鲁棒性有限，在识别和避免安全关键操作时过于保守。

Conclusion: WebRSSBench揭示了当前MLLM在网页理解能力上的重要不足，为开发更可靠的网页AI助手提供了评估基础。
...
### [286] [D-Artemis: A Deliberative Cognitive Framework for Mobile GUI Multi-Agents](https://arxiv.org/abs/2509.21799)
...
TL;DR: D-Artemis是一个基于人类认知循环（思考、对齐、反思）的新型GUI代理框架，通过细粒度的应用提示检索、预执行对齐和事后状态反思，显著提升多模态大语言模型在GUI任务中的性能，无需复杂轨迹数据集训练。
...
Method: 采用三阶段认知循环框架：1）细粒度应用提示检索；2）预执行对齐阶段（包含思想-行动一致性检查和行动修正代理）；3）事后状态反思代理。

Result: 在AndroidWorld上达到75.8%的成功率，在ScreenSpot-V2上达到96.8%的成功率，在两个主要基准测试中都创造了新的最先进结果。

Conclusion: D-Artemis框架通过认知循环设计显著增强了通用多模态大语言模型在GUI任务中的能力，展示了强大的泛化性能，且无需复杂轨迹数据集训练。
...
### [287] [ProRe: A Proactive Reward System for GUI Agents via Reasoner-Actor Collaboration](https://arxiv.org/abs/2509.21823)
*Gaole Dai,Shiqi Jiang,Ting Cao,Yuqing Yang,Yuanchun Li,Rui Tan,Mo Li,Lili Qiu*

Main category: cs.AI

TL;DR: ProRe是一个主动奖励系统，通过通用推理器和领域特定评估器主动与环境交互来改进GUI智能体的奖励评估，显著提升了奖励准确性和任务成功率。


<details>
  <summary>Details</summary>
Motivation: 现有的基于规则或模型的奖励方法难以泛化到GUI智能体，因为缺乏真实轨迹或应用数据库，而静态轨迹评估方法精度有限。

Method: 使用通用推理器调度目标状态探测任务，领域特定评估器通过主动与环境交互收集额外观测，从而分配更准确可验证的奖励。

Result: 在3000多个轨迹上的实验表明，ProRe将奖励准确性和F1分数分别提升5.3%和19.4%，与先进策略智能体结合后任务成功率提升22.4%。

Conclusion: ProRe通过主动环境交互有效解决了GUI智能体奖励评估的泛化性问题，显著提升了评估准确性和智能体性能。
...
### [288] [DS-STAR: Data Science Agent via Iterative Planning and Verification](https://arxiv.org/abs/2509.21825)
...
<details>
  <summary>Details</summary>
Motivation: 数据科学任务复杂，涉及多个数据源探索和综合分析，但现有LLM在处理异构数据格式和验证分析计划充分性方面存在困难。

Method: DS-STAR包含三个关键组件：(1)数据文件分析模块自动探索和提取多样化数据格式的上下文；(2)基于LLM的验证步骤评估每个阶段分析计划的充分性；(3)顺序规划机制从简单可执行计划开始，基于反馈迭代优化直到验证通过。

Result: 实验表明DS-STAR在三个具有挑战性的基准测试（DABStep、KramaBench和DA-Code）上达到最先进性能，特别是在需要处理多个异构数据文件的困难任务上显著优于基线方法。
...
### [289] [Axiomatic Choice and the Decision-Evaluation Paradox](https://arxiv.org/abs/2509.21836)
*Ben Abramowitz,Nicholas Mattei*

Main category: cs.AI

TL;DR: 提出了一个基于公理的决策建模框架，定义了决策公理的结构分类，并揭示了决策-评估悖论，强调在训练决策模型和应用公理时需要格外谨慎。


<details>
  <summary>Details</summary>
Motivation: 研究决策过程中公理（如伦理约束）的作用，探索决策公理的结构特性及其在决策制定与评估中的潜在冲突。

Method: 引入决策公理建模框架，基于结构特性对决策公理进行分类，分析决策制定与评估过程中公理应用的矛盾。

Result: 发现决策-评估悖论的存在，表明在现实公理结构下，使用公理进行决策与使用公理评估决策之间存在根本性冲突。

Conclusion: 决策-评估悖论揭示了在训练决策模型和应用公理时需要特别谨慎，以避免决策制定与评估之间的内在矛盾。
...
### [290] [DeepTravel: An End-to-End Agentic Reinforcement Learning Framework for Autonomous Travel Planning Agents](https://arxiv.org/abs/2509.21842)
*Yansong Ning,Rui Liu,Jun Wang,Kai Chen,Wei Li,Jun Fang,Kan Zheng,Naiqiang Tan,Hao Liu*

Main category: cs.AI

TL;DR: DeepTravel是一个端到端的智能体强化学习框架，用于构建自主旅行规划代理，能够自主规划、执行工具并反思工具响应，在多步推理中探索、验证和优化中间行动。
...
Method: 构建沙盒环境缓存交通、住宿和POI数据；开发分层奖励建模系统，包括轨迹级验证器和回合级验证器；提出回放增强强化学习方法，从失败经验缓冲区定期重放。

Result: 在滴滴企业解决方案App上的评估显示，DeepTravel使小型LLM（如Qwen3 32B）在旅行规划任务中显著优于OpenAI o1、o3和DeepSeek R1等前沿LLM。

Conclusion: DeepTravel框架成功构建了自主旅行规划代理，通过强化学习和分层验证机制显著提升了代理能力。
...
### [291] [Reimagining Agent-based Modeling with Large Language Model Agents via Shachi](https://arxiv.org/abs/2509.21862)
...
<details>
  <summary>Details</summary>
Motivation: 解决大语言模型驱动的多智能体系统中涌现行为研究缺乏受控实验原则性方法的挑战。

Method: 引入Shachi框架，将智能体策略分解为配置（内在特征）、记忆（上下文持久性）和工具（扩展能力）三个组件，由LLM推理引擎协调。

Result: 在10个任务的基准测试中验证了方法的有效性，通过模拟美国关税冲击实验证明，只有当智能体认知架构正确配置记忆和工具时，其行为才能与观察到的市场反应一致。
...
### [292] [TRACE: Learning to Compute on Graphs](https://arxiv.org/abs/2509.21886)
...
<details>
  <summary>Details</summary>
Motivation: 传统图神经网络（MPNN）和Transformer在计算图建模中存在架构不匹配，无法捕捉计算的位置感知和层次化特性，这阻碍了模型学习计算功能的能力。

Method: 1. 使用层次化Transformer架构，模拟计算的逐步流程；2. 提出函数偏移学习目标，通过预测真实全局函数与简单局部近似之间的差异来解耦学习问题。
...
### [293] [GenesisGeo: Technical Report](https://arxiv.org/abs/2509.21896)
*Minfeng Zhu,Zi Wang,Sizhe Ji,Zhengtong Du,Junming Ke,Xiao Deng,Zanlang Yin,Xiuqi Huang,Heyu Wang,Wei Chen*

Main category: cs.AI

TL;DR: GenesisGeo是一个自动化的欧几里得几何定理证明器，通过120倍加速的符号推理引擎和神经符号方法，在IMO-AG-30基准测试中达到了IMO金牌水平。


<details>
  <summary>Details</summary>
Motivation: 解决几何定理自动证明的挑战，特别是提高符号推理引擎的效率，并开发能够达到国际数学奥林匹克竞赛水平的神经符号证明器。

Method: 使用定理匹配技术将符号推理引擎DDARN加速120倍，并用C++实现其核心组件；基于Qwen3-0.6B-Base构建神经符号证明器GenesisGeo。

Result: 创建了包含2180万个几何问题的大规模数据集，其中300多万包含辅助构造；单模型在IMO-AG-30基准测试中解决了24/30个问题（IMO银牌水平），双模型集成解决了26/30个问题（IMO金牌水平）。

Conclusion: GenesisGeo通过高效的符号推理和神经符号方法的结合，在几何定理证明领域达到了国际顶级水平，为自动化数学推理提供了有力工具。
...
### [294] [DyRo-MCTS: A Robust Monte Carlo Tree Search Approach to Dynamic Job Shop Scheduling](https://arxiv.org/abs/2509.21902)
...
<details>
  <summary>Details</summary>
Motivation: 动态作业车间调度面临新作业到达频繁干扰的挑战，现有离线策略不完善，而在线规划因新作业到达不可预测而决策脆弱

Method: DyRo-MCTS方法在MCTS中集成动作鲁棒性估计，引导生产环境朝向既产生良好调度结果又易于适应未来作业到达的状态

Result: 实验表明DyRo-MCTS显著提升离线学习策略性能，在线规划时间增加可忽略，在各种调度场景中持续优于原始MCTS

Conclusion: DyRo-MCTS通过做出鲁棒调度决策，在干扰下实现长期可持续的性能提升
...
### [295] [Outlier Detection in Plantar Pressure: Human-Centered Comparison of Statistical Parametric Mapping and Explainable Machine Learning](https://arxiv.org/abs/2509.21943)
...
Result: ML模型达到高准确率且优于SPM，SPM会误分类临床有意义的变化并漏检真实异常。专家认为SPM和SHAP解释都清晰、有用且可信，但SPM复杂度较低。
...
### [296] [CoBel-World: Harnessing LLM Reasoning to Build a Collaborative Belief World for Optimizing Embodied Multi-Agent Collaboration](https://arxiv.org/abs/2509.21981)
*Zhimin Wang,Shaokang He,Duo Wu,Jinghe Wang,Linjia Kang,Jing Yu,Zhi Wang*

Main category: cs.AI

TL;DR: CoBel-World是一个多智能体协作框架，通过建模协作信念世界来提升LLM智能体的意图推理能力，显著减少通信成本并提高任务完成效率。


<details>
  <summary>Details</summary>
Motivation: 现有LLM协作框架忽视了动态意图推理能力，导致计划不一致和冗余通信，降低了协作效率。

Method: 提出CoBel-World框架，为LLM智能体配备协作信念世界，通过符号信念语言将开放世界任务知识解析为结构化信念，并执行零样本贝叶斯式信念更新。

Result: 在TDW-MAT和C-WAH基准测试中，通信成本减少22-60%，任务完成效率提高4-28%。

Conclusion: 显式的、意图感知的信念建模对于基于LLM的多智能体系统实现高效、类人协作至关重要。
...
### [297] [RISK: A Framework for GUI Agents in E-commerce Risk Management](https://arxiv.org/abs/2509.21982)
...
<details>
  <summary>Details</summary>
Motivation: 传统爬虫方法和现有GUI代理无法处理电商风险管理所需的多样化、深度嵌入的网页数据，这些代理通常限于单步任务且缺乏管理动态交互内容的能力。

Method: RISK框架包含三个组件：RISK-Data数据集（8,492单步和2,386多步交互轨迹）、RISK-Bench基准测试（802单步和320多步轨迹）和RISK-R1强化微调框架（考虑输出格式、单步级、多步级和任务级四个方面的奖励机制）。
...
### [298] [Bilinear relational structure fixes reversal curse and enables consistent model editing](https://arxiv.org/abs/2509.21993)
*Dong-Kyum Kim,Minsung Kim,Jea Kwon,Nakyeong Yang,Meeyoung Cha*

Main category: cs.AI

TL;DR: 研究发现反转诅咒不是语言模型的固有缺陷，而是知识编码方式的产物。通过在关系知识图谱上训练，模型内部会涌现双线性关系结构，这能显著缓解反转诅咒并实现一致的模型编辑。


<details>
  <summary>Details</summary>
Motivation: 解决语言模型中的反转诅咒问题——即模型无法从已学习的"A是B"推断出"B是A"，并探索模型编辑的稳定性问题。

Method: 在合成的关系知识图谱数据集上从头训练语言模型，分析其隐藏表示中的双线性关系结构，并测试模型编辑的传播效果。

Result: 具有双线性结构的模型能够有效推断未见过的反向事实，并且在编辑时能正确传播到反向事实和其他逻辑相关事实，而缺乏这种结构的模型则无法实现这些功能。

Conclusion: 关系知识训练诱导双线性内部表示的出现，这对模型编辑的成功至关重要，表明编辑算法的效果不仅取决于算法本身，还依赖于被修改知识的底层表示几何结构。
...
### [299] [GSM-Agent: Understanding Agentic Reasoning Using Controllable Environments](https://arxiv.org/abs/2509.21998)
*Hanlin Zhu,Tianyu Guo,Song Mei,Stuart Russell,Nikhil Ghosh,Alberto Bietti,Jiantao Jiao*

Main category: cs.AI

TL;DR: 提出了GSM-Agent基准测试，用于评估LLM在需要主动使用工具收集信息来解决小学数学问题的代理推理能力，发现即使前沿模型准确率也仅67%，并提出了代理推理图分析和工具增强的测试时扩展方法来改进性能。


<details>
  <summary>Details</summary>
Motivation: 当前代理基准测试往往将代理推理与复杂的数学推理、专家级知识等能力混在一起，难以单独评估代理推理能力，因此需要构建专门的基准测试来解耦这一能力。

Method: 构建GSM-Agent基准测试，要求LLM代理解决小学数学问题但只提供问题而不提供前提信息，需要主动使用工具收集信息；提出代理推理图概念来分析和可视化推理模式；开发工具增强的测试时扩展方法鼓励模型重新访问节点。

Result: 即使像GPT-5这样的前沿模型在GSM-Agent基准测试中准确率也仅为67%；分析发现许多模型在代理推理中缺乏重新访问先前节点的能力，这是静态推理中的关键模式。

Conclusion: GSM-Agent基准测试和代理推理框架有助于未来理解和推进代理推理边界的研究，提出的工具增强方法能有效改进LLM的代理推理性能。
...
### [300] [The Thinking Spectrum: An Emperical Study of Tunable Reasoning in LLMs through Model Merging](https://arxiv.org/abs/2509.22034)
*Xiaochong Lan,Yu Zheng,Shiteng Cao,Yong Li*

Main category: cs.AI

TL;DR: 该论文通过大规模实证研究评估了模型融合技术在LLM推理能力调控方面的效果，发现模型融合能够有效平衡推理精度与计算成本，甚至在某些情况下实现帕累托改进。


<details>
  <summary>Details</summary>
Motivation: 现实应用中对可调节推理能力的大型语言模型需求日益增长，需要能够高效生成平衡推理深度和计算成本的模型谱系。模型融合作为一种无需训练的技术，有望解决这一挑战。

Method: 进行大规模实证研究，评估多种模型融合技术在不同推理基准上的表现，系统性地改变融合强度来构建精度-效率曲线。

Result: 研究发现模型融合提供了有效且可控的方法来校准推理精度与token效率之间的权衡，即使父模型权重空间高度发散时也有效。在某些情况下实现了帕累托改进，即融合模型同时获得更高精度和更低token消耗。
...
### [301] [A2R: An Asymmetric Two-Stage Reasoning Framework for Parallel Reasoning](https://arxiv.org/abs/2509.22044)
*Ziqi Wang,Boye Niu,Zhongli Li,Linghui Meng,Jing Liu,Zhi Zheng,Tong Xu,Hua Wu,Haifeng Wang,Enhong Chen*

Main category: cs.AI

TL;DR: A2R是一个非对称两阶段推理框架，通过探索器并行生成多个解决方案，再由合成器整合进行精炼推理，显著提升模型在复杂任务上的性能表现。


<details>
  <summary>Details</summary>
Motivation: 解决大型推理模型在单次尝试中的表现与其潜在能力之间的差距，这种差距通常需要多个解决方案路径才能揭示。

Method: 提出A2R框架：第一阶段由探索器模型通过重复采样并行生成潜在解决方案；第二阶段由合成器模型整合这些参考进行更精细的推理。

Result: Qwen3-8B-distill模型使用A2R框架相比自一致性基线性能提升75%；A2R-Efficient变体（Qwen3-4B探索器+Qwen3-8B合成器）超越单一Qwen3-32B模型性能，成本降低近30%。
...
### [302] [Generalizing Multi-Objective Search via Objective-Aggregation Functions](https://arxiv.org/abs/2509.22085)
*Hadar Peer,Eyal Weiss,Ron Alterovitz,Oren Salzman*

Main category: cs.AI

TL;DR: 提出了一种支持多目标搜索的通用问题表述，通过聚合函数优化隐藏目标，使标准MOS算法能够应用于复杂的机器人规划问题。


<details>
  <summary>Details</summary>
Motivation: 现实机器人系统需要同时平衡多个冲突目标，但现有问题表述无法直接使用先进MOS算法，需要更通用的解决方案。

Method: 使用聚合函数优化隐藏搜索目标，扩展标准MOS算法的核心操作以反映特定聚合函数，应用于多种机器人规划场景。

Result: 扩展后的MOS算法在多个机器人规划问题中比原始版本性能提升数个数量级。

Conclusion: 提出的通用问题表述和扩展方法使标准MOS算法能够有效处理复杂的多目标机器人规划问题。
...
### [303] [Ground-Truthing AI Energy Consumption: Validating CodeCarbon Against External Measurements](https://arxiv.org/abs/2509.22092)
*Raphael Fischer*

Main category: cs.AI

TL;DR: 本研究系统评估了AI能耗估计工具的准确性，通过与真实测量数据对比发现现有工具存在高达40%的估计误差，为可持续AI发展提供了实证依据和改进指南。


<details>
  <summary>Details</summary>
Motivation: 随着AI快速发展带来的环境影响日益显著，现有能耗估计工具虽然易于使用但存在简化假设和忽略重要因素的问题，需要验证其估计准确性。

Method: 通过提出的验证框架，将静态和动态能耗估计方法与真实测量数据在数百个AI实验中进行系统性比较。

Result: 现有估计方法虽然能大致反映AI能耗模式，但持续存在高达40%的估计误差。

Conclusion: 研究为AI能耗估计质量提供了实证证据，建立了透明度，验证了广泛使用的工具，并为改进现有技术提出了指导方针，对资源感知的机器学习和AI可持续性研究做出了重要贡献。
...
### [304] [Log2Plan: An Adaptive GUI Automation Framework Integrated with Task Mining Approach](https://arxiv.org/abs/2509.22137)
*Seoyoung Lee,Seonbin Yoon,Seongbeen Lee,Hyesoo Kim,Joo Yong Sim*

Main category: cs.AI

TL;DR: Log2Plan是一个结合结构化两级规划框架和用户行为日志任务挖掘的GUI任务自动化系统，解决了现有LLM/VLM规划器在泛化性、延迟和长序列任务一致性方面的限制。


<details>
  <summary>Details</summary>
Motivation: 现有基于LLM或VLM的规划-执行代理存在脆弱的泛化能力、高延迟和有限的长序列一致性，对UI变化或复杂任务适应性差。

Method: 采用结构化两级规划框架：高层规划将用户命令映射到结构化任务字典，低层规划通过实时GUI上下文解释生成动作序列，并结合用户行为日志进行任务挖掘以支持个性化和重用。

Result: 在200个真实世界任务上的评估显示，任务成功率和执行时间显著改善，在长序列任务中保持超过60.0%的成功率。

Conclusion: Log2Plan通过结合结构化规划和任务挖掘方法，实现了鲁棒且适应性强的GUI自动化，特别在复杂多步骤工作流中表现优异。
...
### [305] [Clinical Uncertainty Impacts Machine Learning Evaluations](https://arxiv.org/abs/2509.22242)
...
TL;DR: 本文主张在机器学习评估中应明确考虑标注不确定性，使用直接操作分布的概率度量，而非简单的多数投票等聚合方法。


<details>
  <summary>Details</summary>
Motivation: 临床数据集标注存在不确定性，标注者之间存在分歧且置信度不一致，传统聚合方法（如多数投票）掩盖了这种变异性。

Method: 提出使用概率度量来明确处理标注不确定性，这些度量可应用于不同的标注生成过程（简单计数、主观置信度评级或概率响应模型），并且计算轻量。

Result: 在医学影像基准测试的简单实验中，考虑二元标签的置信度显著影响模型排名。

Conclusion: 呼吁社区发布数据集的原始标注并采用不确定性感知评估，使性能估计能更好地反映临床数据。
...
### [306] [Evaluating LLMs for Combinatorial Optimization: One-Phase and Two-Phase Heuristics for 2D Bin-Packing](https://arxiv.org/abs/2509.22255)
*Syed Mahbubul Huq,Daniel Brito,Daniel Sikar,Rajesh Mojumder*

Main category: cs.AI

TL;DR: 提出了一个评估大语言模型在组合优化中能力的框架，特别针对二维装箱问题，结合进化算法迭代生成和优化启发式解。


<details>
  <summary>Details</summary>
Motivation: 评估大语言模型在专业领域（如组合优化）中的能力，建立专门的评估基准。

Method: 系统化方法将大语言模型与进化算法结合，迭代生成和优化启发式解，并与传统方法（有限首次适应和混合首次适应）进行比较。

Result: GPT-4o在两次迭代内达到最优解，平均箱数从16减少到15，空间利用率从0.76-0.78提升到0.83，且计算资源需求更少。

Conclusion: 大语言模型在组合优化中能产生更高效的解，为理解LLM在专业领域的评估提供了贡献，并建立了相关任务的性能基准。
...
### [307] [InfiMed-Foundation: Pioneering Advanced Multimodal Medical Models with Compute-Efficient Pre-Training and Multi-Stage Fine-Tuning](https://arxiv.org/abs/2509.22261)
*Guanghao Zhu,Zhitian Hou,Zeyu Liu,Zhijie Sang,Congkai Xie,Hongxia Yang*

Main category: cs.AI

TL;DR: 提出了两个医学专用多模态大语言模型InfiMed-Foundation-1.7B和4B，通过高质量数据筛选、高效训练策略和三阶段微调，在医学任务中超越了现有模型。


<details>
  <summary>Details</summary>
Motivation: 通用多模态大语言模型在医学领域存在专业知识不足、产生幻觉响应的问题，知识蒸馏难以捕捉医学专业知识，且大规模医学数据预训练计算成本高。

Method: 结合通用和医学多模态数据，提出五维质量评估框架筛选高质量数据集，采用低到高图像分辨率和多模态序列打包提高训练效率，使用三阶段监督微调进行知识提取。

Result: 在MedEvalKit评估中，1.7B模型超越Qwen2.5VL-3B，4B模型超越HuatuoGPT-V-7B和MedGemma-27B-IT，在医学视觉问答和诊断任务中表现优异。

Conclusion: 通过解决数据质量、训练效率和领域知识提取等关键挑战，为医疗领域提供了更可靠有效的AI解决方案。
...
### [308] [Structured Sparse Transition Matrices to Enable State Tracking in State-Space Models](https://arxiv.org/abs/2509.22284)
...
<details>
  <summary>Details</summary>
Motivation: 现代状态空间模型的转移矩阵虽然计算高效但表达能力受限，而无结构转移矩阵计算和内存成本过高

Method: 将转移矩阵参数化为列one-hot矩阵(P)和复值对角矩阵(D)的乘积，使并行扫描计算成本与状态大小呈线性关系

Result: 理论上模型BIBO稳定，能模拟任意N状态FSA；实验在FSA状态跟踪任务中优于多种现代SSM变体，时间序列分类性能与神经控制微分方程相当

Conclusion: PD-SSM在保持计算效率的同时显著提升了状态空间模型的表达能力，并能有效集成到混合Transformer-SSM架构中
...
### [309] [Large Language Models as Nondeterministic Causal Models](https://arxiv.org/abs/2509.22297)
*Sander Beckers*
...
<details>
  <summary>Details</summary>
Motivation: 现有方法对LLM的解释存在歧义，既没有字面解释也没有按意图解释，需要一种更简单且直接适用于黑盒LLM的反事实生成方法。

Method: 将LLM表示为非确定性因果模型，而不是确定性因果模型，从而生成反事实。这种方法对任何黑盒LLM都适用，不依赖实现细节。

Result: 提出了一个更简单的反事实生成方法，可直接应用于任何黑盒LLM，而现有方法只能生成特定类型的反事实。
...
### [310] [PRIME: Planning and Retrieval-Integrated Memory for Enhanced Reasoning](https://arxiv.org/abs/2509.22315)
*Hieu Tran,Zonghai Yao,Nguyen Luong Tran,Zhichao Yang,Feiyun Ouyang,Shuo Han,Razieh Rahimi,Hong Yu*

Main category: cs.AI

TL;DR: PRIME是一个多智能体推理框架，模仿人类双过程认知理论，动态整合快速直觉思维（系统1）和慢速深思熟虑思维（系统2），显著提升LLM在复杂推理任务中的表现。


<details>
  <summary>Details</summary>
Motivation: 受卡尼曼《思考，快与慢》中人类双过程认知理论启发，旨在让LLM更贴近人类认知过程，在效率和准确性之间取得平衡。

Method: 采用多智能体架构：系统1快速生成答案，若检测到不确定性则触发系统2推理管道，包含规划、假设生成、检索、信息整合和决策等专门智能体。

Result: 实验表明，PRIME让开源LLaMA 3模型在需要多跳和知识基础推理的基准测试中，能与GPT-4、GPT-4o等闭源模型竞争。

Conclusion: PRIME为改进LLM在复杂知识密集型推理领域的性能提供了一个可扩展的解决方案。
...
### [311] [Do LLM Agents Know How to Ground, Recover, and Assess? A Benchmark for Epistemic Competence in Information-Seeking Agents](https://arxiv.org/abs/2509.22391)
*Jiaqi Shao,Yuxiang Lin,Munish Prasad Lohani,Yufeng Miao,Bing Luo*

Main category: cs.AI

TL;DR: SeekBench是首个通过响应轨迹的步骤级分析来评估LLM搜索代理认知能力的基准，包含190个专家标注的轨迹和1800多个响应步骤，用于分析代理是否基于观察证据生成推理步骤、自适应重新制定搜索以及正确评估证据充分性。


<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注LLM搜索代理在开放域问答中的最终答案准确性，但忽视了代理如何基于外部证据进行推理和行动，因此需要评估其认知能力。

Method: 引入SeekBench基准，通过专家标注的190个轨迹和1800多个响应步骤，对LLM搜索代理的响应轨迹进行步骤级分析，并添加证据标注以进行细粒度分析。

Result: SeekBench提供了对LLM搜索代理认知能力的评估框架，能够分析代理是否基于证据推理、自适应搜索重新制定以及正确评估证据充分性。

Conclusion: SeekBench填补了现有评估方法的空白，为评估LLM搜索代理的认知能力提供了首个基准，有助于更全面地理解代理的推理和行为过程。
...
### [312] [EMMA: Generalizing Real-World Robot Manipulation via Generative Visual Transfer](https://arxiv.org/abs/2509.22407)
...
TL;DR: EMMA框架通过DreamTransfer生成多视角一致的机器人操作视频和AdaMix训练策略，显著提升VLA模型在未见物体类别和新视觉领域的泛化能力，在零样本视觉域任务中相比仅使用真实数据获得200%以上的性能提升。


<details>
  <summary>Details</summary>
Motivation: 收集大规模真实世界机器人操作数据成本高昂且耗时，限制了VLA模型的泛化能力，需要一种能够生成多样化训练数据的方法来克服这一瓶颈。

Method: 提出EMMA框架：1) DreamTransfer - 基于扩散Transformer的多视角一致视频生成框架，支持文本控制的视觉编辑；2) AdaMix - 硬样本感知训练策略，动态重加权训练批次以关注感知或运动学挑战性样本。

Result: DreamTransfer在视角一致性、几何保真度和文本条件准确性方面显著优于现有视频生成方法。使用生成数据训练的VLA模型在零样本视觉域机器人操作任务中，相比仅使用真实数据获得200%以上相对性能提升，结合AdaMix进一步提升13%。

Conclusion: EMMA框架通过生成多样化训练数据和优化训练策略，有效解决了机器人操作数据稀缺问题，显著提升了VLA模型在未见场景中的泛化性能。
...
### [313] [Guiding Evolution of Artificial Life Using Vision-Language Models](https://arxiv.org/abs/2509.22447)
*Nikhil Baid,Hannah Erlebach,Paul Hellegouarch,Frederico Wieser*

Main category: cs.AI

TL;DR: ASAL++是基于ASAL的改进方法，使用多模态基础模型进行开放式搜索，通过第二个FM根据模拟视觉历史提出新的进化目标，创建具有日益复杂目标的进化轨迹。


<details>
  <summary>Details</summary>
Motivation: 基础模型为人工生命领域提供了强大的自动化搜索工具，之前的工作使用视觉语言模型将ALife模拟与自然语言目标提示对齐。本研究旨在开发具有开放式特征的FM驱动的ALife发现新方向。

Method: 引入ASAL++方法，使用第二个FM基于模拟视觉历史提出新的进化目标。探索两种策略：EST（每次迭代匹配单个新提示）和ETT（匹配整个生成提示序列）。在Lenia基底中使用Gemma-3进行实证测试。

Result: 实证结果显示，EST促进了更大的视觉新颖性，而ETT培养了更连贯和可解释的进化序列。

Conclusion: ASAL++为具有开放式特征的FM驱动的ALife发现指明了新的方向。
...
### [314] [GeoSketch: A Neural-Symbolic Approach to Geometric Multimodal Reasoning with Auxiliary Line Construction and Affine Transformation](https://arxiv.org/abs/2509.22460)
...
<details>
  <summary>Details</summary>
Motivation: 现有的多模态大语言模型将图表视为静态图像，缺乏动态操作能力，而动态操作（如辅助线构造和仿射变换）是人类几何推理的核心方面。

Method: GeoSketch包含三个模块：感知模块将图表抽象为结构化逻辑形式，符号推理模块应用几何定理决定下一步推理步骤，草图操作模块执行绘制辅助线或应用变换等操作。采用两阶段训练：监督微调和强化学习。

Result: 在GeoSketch基准测试上的实验表明，GeoSketch显著提高了逐步推理准确性和问题解决成功率，优于静态感知方法。
...
### [315] [InfiAgent: Self-Evolving Pyramid Agent Framework for Infinite Scenarios](https://arxiv.org/abs/2509.22502)
*Chenglin Yu,Yang Yu,Songmiao Wang,Yucheng Wang,Yifan Yang,Jinjia Li,Ming Li,Hongxia Yang*

Main category: cs.AI

TL;DR: InfiAgent是一个基于有向无环图的金字塔式多智能体框架，通过自动分解复杂智能体、双重审计机制、智能体路由和自我进化机制，解决LLM智能体开发中的可扩展性和成本效益问题。


<details>
  <summary>Details</summary>
Motivation: 开发LLM智能体需要精心设计的工作流程、提示词和迭代调优，这些手工制作的限制阻碍了智能体在各行业的可扩展性和成本效益。

Method: 提出金字塔式DAG多智能体框架，包括：智能体即工具机制自动分解复杂智能体、双重审计确保任务质量、智能体路由实现任务匹配、自我进化机制自主重构智能体DAG，以及支持并行执行的原子任务设计。

Result: 在多个基准测试中，InfiAgent相比类似的自生成智能体框架ADAS性能提升9.9%；案例研究显示AI研究助手InfiHelper生成的科学论文获得了顶级IEEE会议人类评审的认可。

Conclusion: InfiAgent框架发展成为一个能够解决广泛问题的通用金字塔式多智能体系统，显著提升了LLM智能体的执行效率和适用性。
...
### [316] [Estimating the Empowerment of Language Model Agents](https://arxiv.org/abs/2509.22504)
*Jinyeop Song,Jeff Gore,Max Kleiman-Weiner*

Main category: cs.AI

TL;DR: 提出基于信息论中赋权（empowerment）概念的LM智能体评估框架EELMA，通过计算智能体行动与未来状态之间的互信息来评估智能体能力，在语言游戏和网页浏览场景中验证其有效性。


<details>
  <summary>Details</summary>
Motivation: 随着语言模型智能体能力增强并广泛使用现实工具，需要可扩展的评估框架。传统基准测试设计成本高且需要人工设计有效任务来评估通用能力。

Method: 提出EELMA算法，通过多轮文本交互近似估计有效赋权，在语言游戏和网页浏览场景中进行验证。

Result: 发现赋权与平均任务表现强相关，表征了环境复杂性和智能体因素（如思维链、模型规模、记忆长度）对估计赋权的影响，高赋权状态和行动通常是通用能力的关键时刻。

Conclusion: 赋权作为在复杂开放环境中评估和监控LM智能体的通用指标具有吸引力。
...
### [317] [TrueGradeAI: Retrieval-Augmented and Bias-Resistant AI for Transparent and Explainable Digital Assessments](https://arxiv.org/abs/2509.22516)
*Rakesh Thakur,Shivaansh Kaushik,Gauri Chopra,Harsh Rohilla*

Main category: cs.AI

TL;DR: TrueGradeAI是一个AI驱动的数字考试框架，通过保留自然手写输入和使用基于transformer的OCR转录，结合检索增强管道进行评分，提供可解释、可审计的自动化评分，减少环境影响并确保评估公平性。


<details>
  <summary>Details</summary>
Motivation: 解决传统纸质考试的缺点：纸张浪费、物流复杂、评分延迟和评估者偏见。

Method: 使用安全平板电脑捕捉手写笔输入，应用基于transformer的OCR进行转录，通过检索增强管道整合教师解决方案、缓存层和外部参考，让大语言模型进行评分并生成证据关联的推理。

Result: 实现了手写保留与可扩展透明评估的结合，减少了环境成本，加速了反馈周期，并逐步构建可重复使用的知识库。

Conclusion: TrueGradeAI通过整合可解释的自动化、偏见缓解和可审计的评分轨迹，推进了数字考试领域，同时积极减轻评分偏见并确保评估公平性。
...
### [318] [REMA: A Unified Reasoning Manifold Framework for Interpreting Large Language Model](https://arxiv.org/abs/2509.22518)
*Bo Li,Guanzhi Deng,Ronghao Chen,Junrong Yue,Shuo Zhang,Qinghua Zhao,Linqi Song,Lijie Wen*

Main category: cs.AI

TL;DR: 提出了REMA框架，通过定义"推理流形"概念来分析LLM推理失败的原因，将错误推理的内部表示与正确推理的几何结构进行比较，量化偏差并定位分歧点。
...
Method: 定义推理流形概念，构建REMA框架，通过计算错误表示与正确流形的k近邻距离来量化几何偏差，并跟踪模型各层的偏差来定位分歧点。
...
### [319] [The Emergence of Altruism in Large-Language-Model Agents Society](https://arxiv.org/abs/2509.22537)
*Haoyang Li,Xiao Jia,Zhanzhan Zhao*

Main category: cs.AI

TL;DR: 该研究首次揭示了不同大语言模型在利己和利他倾向上的内在异质性，提出了两种社会行动逻辑原型："适应性利己主义者"和"利他优化者"，为基于LLM的社会模拟提供了模型选择的重要依据。
...
Method: 引入Schelling变体城市迁移模型，创建社会困境，让200多个LLM智能体在利己（个人效用）和利他（系统效用）目标之间进行权衡；采用基于扎根理论的方法系统编码智能体推理过程。

Result: 发现LLM存在两种不同的社会倾向原型：适应性利己主义者（默认优先考虑自身利益，但在社会规范影响下利他行为显著增加）和利他优化者（内在利他逻辑，即使直接损害自身利益也始终优先考虑集体利益）。

Conclusion: 对于社会模拟，模型选择不仅是选择推理能力，更是选择内在的社会行动逻辑。适应性利己主义者更适合模拟复杂人类社会，而利他优化者更适合模拟理想化的亲社会行为者或集体福利为主要考量的场景。
...
### [320] [StepORLM: A Self-Evolving Framework With Generative Process Supervision For Operations Research Language Models](https://arxiv.org/abs/2509.22558)
...
<details>
  <summary>Details</summary>
Motivation: 现有强化学习方法在运筹学问题上存在两个关键局限：结果奖励存在信用分配问题，正确的最终答案可能强化有缺陷的推理；传统判别式过程监督目光短浅，无法整体评估运筹学建模的相互依赖步骤。

Method: StepORLM采用生成式过程监督框架，包含策略模型和生成式过程奖励模型(GenPRM)的协同进化循环。通过双重反馈机制：来自外部求解器的确定性结果验证，以及来自GenPRM的细致整体过程评估。使用加权直接偏好优化(W-DPO)对齐策略，同时优化GenPRM。

Result: 8B参数的StepORLM在六个基准测试中建立了新的最先进水平，显著优于更大的通用模型、代理方法和专业基线。协同进化的GenPRM能够作为强大且普遍适用的过程验证器，大幅提升自身模型和其他现有LLM的推理扩展性能。

Conclusion: StepORLM通过生成式过程监督和协同进化机制，有效解决了运筹学问题中强化学习的信用分配和过程评估问题，实现了显著的性能提升，并提供了通用的过程验证能力。
...
### [321] [UniMIC: Token-Based Multimodal Interactive Coding for Human-AI Collaboration](https://arxiv.org/abs/2509.22570)
...
Method: 采用token化表示作为通信媒介，使用轻量级Transformer熵模型（通用、掩码和文本条件三种设计）来最小化token间冗余。
...
### [322] [Dynamic Experts Search: Enhancing Reasoning in Mixture-of-Experts LLMs at Test Time](https://arxiv.org/abs/2509.22572)
...
<details>
  <summary>Details</summary>
Motivation: 现有测试时扩展方法主要依赖输出级采样，忽视了模型架构的作用。在MoE模型中，发现改变激活专家数量可以产生互补的解决方案集，这揭示了一个未被充分探索的多样性来源。
...
Result: 在多个MoE架构、验证器和推理基准测试（数学、代码和知识）上的广泛实验表明，DES可靠地优于测试时扩展基线，无需额外成本即可提高准确性和稳定性。

Conclusion: DES是一种实用且可扩展的架构感知测试时扩展方法，展示了现代LLMs中结构灵活性如何推进推理能力发展。
...
### [323] [Benefits and Pitfalls of Reinforcement Learning for Language Model Planning: A Theoretical Perspective](https://arxiv.org/abs/2509.22613)
*Siwei Wang,Yifei Shen,Haoran Sun,Shi Feng,Shang-Hua Teng,Li Dong,Yaru Hao,Wei Chen*

Main category: cs.AI

TL;DR: 该论文通过图抽象模型分析RL在LLM规划中的理论机制，发现SFT可能导致伪相关解，而RL通过探索实现正确规划，但PG存在多样性崩溃问题，Q-learning则能保持多样性并避免奖励破解。
...
Method: 采用可处理的图抽象模型，重点分析策略梯度(PG)和Q-learning方法，通过理论分析和在Blocksworld基准上的实验验证。

Result: SFT引入基于共现的伪相关解；RL通过探索实现正确规划；PG存在多样性崩溃；Q-learning能保持输出多样性并避免奖励破解。

Conclusion: 探索在RL规划中起关键作用，Q-learning优于PG在于能保持多样性，但需要精心设计奖励函数以防止奖励破解。
