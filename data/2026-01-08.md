...
TL;DR: HyperCLOVA X 32B Think 是一个专注于韩语语言文化背景推理和代理能力的视觉语言模型，在韩语文本到文本、视觉到文本基准测试以及代理导向任务中表现优异。


<details>
  <summary>Details</summary>
Motivation: 开发一个专门针对韩语语言文化背景的视觉语言模型，特别强调推理能力和代理行为，以支持更广泛的研究和创新。

Method: 模型采用预训练强调推理能力，后训练支持多模态理解、增强推理、代理行为和人类偏好对齐的32B参数视觉语言模型。

Result: 在可比规模模型中，该模型在韩语文本到文本、视觉到文本基准测试以及代理导向评估任务中表现出色。
...
<details>
  <summary>Details</summary>
Motivation: 现有射频无人机检测数据集存在稀缺性和多样性有限的问题，缺乏标准化基准和工具支持，阻碍了鲁棒、可泛化的RF感知模型的发展。

Method: 通过真实世界采集（Rowan大学校园和受控RF笼设施）结合系统化合成数据生成管道，包括精确控制信噪比、注入干扰发射器、应用频率偏移及标签一致的边界框变换。

Result: 创建了包含多种当代无人机模型的大规模数据集CDRF，并发布了可互操作的开源工具，支持分类、开放集识别和目标检测的标准化基准测试。

Conclusion: CDRF基准数据集和工具套件旨在加速鲁棒、可泛化的RF感知模型的发展，支持可复现的管道和严格比较，推动RF无人机检测领域进步。
...
Result: 构建了更具挑战性的基准测试，涵盖名人、物体和色情内容等多个领域的概念擦除任务，理论分析证明了SuPLoRA在减轻生成性能退化方面的有效性

Conclusion: 提出的概念层次结构和SuPLoRA方法能够高效、有效地同时擦除多个相关概念，同时保持模型的整体生成能力
...
TL;DR: VLM4VLA：一个最小化适配流程，将通用VLM转换为VLA策略，研究发现VLM通用能力不能预测下游任务性能，视觉模块是主要瓶颈，控制相关监督能提升性能。


<details>
  <summary>Details</summary>
Motivation: 研究VLM选择与能力如何影响下游VLA策略性能，挑战当前关于VLM能力与下游控制性能关系的常见假设。

Method: 提出VLM4VLA最小适配流程，仅使用少量可学习参数将通用VLM转换为VLA策略，进行公平高效比较。在三个基准测试的多个下游任务上进行实证研究，分析VLM初始化、具体体现能力影响，并进行模态级消融实验。

Result: VLM初始化相比从头训练有持续优势，但VLM通用能力不能预测下游任务性能；提升VLM在具体体现任务上的表现不能保证更好的控制性能；视觉模块是主要性能瓶颈，向VLM视觉编码器注入控制相关监督能带来持续增益。

Conclusion: 标准VLM能力对有效体现控制是必要但不充分的，当前VLM预训练目标与体现动作规划需求存在持续领域差距，视觉模块是关键改进方向。
...
TL;DR: 使用深度学习图像识别技术自动分类刚捕获的白虾，以提升分类准确性、效率和一致性，保持新鲜度并改善产品外观。


<details>
  <summary>Details</summary>
Motivation: 随着消费者对高品质水产品需求增加，虾类加工食品的新鲜度和外观完整性成为关键问题。白虾收获后新鲜度迅速下降，软壳虾在烹饪或冷冻后常出现头身分离，影响产品外观和消费者感知。

Method: 采用基于深度学习的图像识别技术，使用卷积神经网络（CNN）模型替代人工分拣，实现白虾的自动分类。

Result: 该方法提高了分类准确性、效率和一致性，减少了处理时间，有助于保持虾的新鲜度，使运输企业能更有效地满足客户需求。

Conclusion: 深度学习图像识别技术能够有效解决虾类加工中的新鲜度和外观问题，为水产养殖业提供了自动化、高效的分类解决方案。
...
TL;DR: 提出使用高阶张量（如三阶或更高阶中心矩）扩展PCA，用于描述复杂形状并构建旋转不变特征，应用于分子形状描述、物体识别和形状相似性度量。


<details>
  <summary>Details</summary>
Motivation: PCA只能通过协方差矩阵（二阶矩）近似形状为椭球体，但真实形状通常更复杂。需要更高阶的描述方法来提高形状表示的准确性，并构建旋转不变特征。

Method: 扩展PCA到高阶张量，如三阶中心矩p_abc或更高阶，以及多项式乘以高斯函数的方法。这些高阶中心矩可以构建旋转不变特征，如幂的迹等。

Result: 该方法可以创建任意高精度的可解码形状描述符及其旋转不变特征，能够准确描述复杂形状而不仅仅是椭球近似。

Conclusion: 高阶张量扩展为形状分析提供了强大的旋转不变特征，可应用于分子形状描述、旋转不变物体识别和高效的形状相似性比较，无需昂贵的旋转优化计算。
...
TL;DR: MMErroR是一个包含2,013个样本的多模态基准测试，专门用于评估视觉语言模型检测错误推理和分类错误类型的能力，覆盖6大领域24个子领域，现有最佳模型准确率仅66.47%


<details>
  <summary>Details</summary>
Motivation: 随着视觉语言模型在多模态学习中的进步，需要评估这些模型是否真正理解内容，特别是能否检测错误的推理过程并识别错误类型

Method: 构建MMErroR基准测试，包含2,013个样本，每个样本嵌入单一连贯的推理错误，覆盖6个顶级领域和24个子领域，进行过程级、以错误为中心的评估

Result: 评估了20个先进视觉语言模型，最佳模型（Gemini-3.0-Pro）的错误分类准确率仅为66.47%，表明识别错误推理具有挑战性

Conclusion: 准确识别错误的能力为理解多模态推理模型的能力提供了宝贵见解，MMErroR基准测试揭示了当前模型在错误检测方面的局限性
...
TL;DR: 提出一种新的可重光照3D高斯头部模型，无需OLAT数据即可从单图或多视角图像重建可重光照头像


<details>
  <summary>Details</summary>
Motivation: 现有高质量重光照方法需要复杂的时间复用光照数据（如OLAT），限制了应用范围。需要一种能从普通图像重建可重光照头像的通用方法

Method: 采用两阶段设计：第一阶段在无OLAT的多视角数据集上学习平光照3DGS头像；第二阶段学习从平光照头像到基于物理的反射率参数的映射，使用小规模OLAT数据集训练

Result: 模型能泛化到未见过的对象，仅需单张图像即可拟合，实现新颖视角合成和重光照应用

Conclusion: 该方法突破了传统重光照方法对OLAT数据的依赖，实现了从普通图像到高质量可重光照头像的通用解决方案
...
Method: 1) 利用图像抠图数据集训练的数据整理流水线；2) 深度修复网络自动识别软边界区域，通过门控残差模块精修深度；3) 基于深度的前向扭曲保留高保真纹理；4) 生成式场景绘制器填充遮挡区域并消除冗余背景伪影；5) 颜色融合器自适应结合扭曲和修复结果。

Result: 在单目深度估计、立体图像/视频转换和新视角合成等任务中实现最先进性能，在软边界区域有显著改进。

Conclusion: HairGuard框架能够有效恢复3D视觉任务中的细粒度软边界细节，通过创新的深度修复和生成式场景绘制方法，在多个任务中取得了优异性能。
...
TL;DR: 论文提出了RiskCueBench视频理解基准，要求模型仅基于风险信号片段（最早出现安全问题的时刻）来预测风险事件，而非使用完整视频序列，以更好地反映现实世界条件。


<details>
  <summary>Details</summary>
Motivation: 现有视频风险评估研究通常让模型访问包含事故本身的完整视频序列，这大大降低了任务难度。为了更好反映现实世界条件，需要开发能够从早期视觉信号预测风险事件的模型。

Method: 引入RiskCueBench视频理解基准，其中视频经过精心标注以识别风险信号片段（定义为最早表明潜在安全问题的时刻），要求模型仅基于这些早期信号进行风险评估。

Result: 实验结果显示，当前系统在解释不断演变的情境和从早期视觉信号预测未来风险事件方面存在显著能力差距，突显了视频风险预测模型实际部署的重要挑战。

Conclusion: RiskCueBench基准揭示了现有视频风险评估方法的局限性，强调需要开发能够从早期视觉线索有效预测风险事件的模型，这对实际应用中的公共安全至关重要。
...
<details>
  <summary>Details</summary>
Motivation: 随着AI技术发展，深度伪造的合成与滥用成为严重威胁，为维持数字时代的信任，需要有效的深度伪造检测和标记方法

Method: 提出新颖架构，使用空间域与频域特征的交叉注意力机制，结合血液检测模块，采用Swin Transformer/BERT或EfficientNet-B4/BERT组合进行分类

Result: 在FF++数据集上达到99.80% AUC，Celeb-DF上达到99.88% AUC（使用Swin Transformer和BERT）；使用EfficientNet-B4和BERT时分别达到99.55%和99.38% AUC，跨数据集泛化性能优秀

Conclusion: 该架构在深度伪造检测任务中表现出卓越性能，超越了现有最佳方法，并具有良好的泛化能力，为统一架构设计提供了重要见解
...
TL;DR: 比较恒河猴下颞叶皮层在观看自然视频时的神经响应与静态、循环和视频ANN模型，发现当前视频模型仅捕捉外观依赖的动态，而非IT皮层表现的外观不变时间计算。


<details>
  <summary>Details</summary>
Motivation: 尽管前馈ANN是灵长类腹侧视觉流的主流模型，但它们本质上是静态计算。灵长类世界是动态的，下颞叶皮层不仅支持物体识别，还在自然视频观看中编码物体运动速度。需要探究IT的时间响应是否仅反映时间展开的前馈变换、逐帧特征与浅层时间池化，还是包含更丰富的动态计算。

Method: 通过比较恒河猴IT在观看自然视频时的神经响应与静态、循环和视频ANN模型。应用压力测试：在自然视频上训练的解码器在"无外观"变体上进行评估，这些变体保留运动但移除形状和纹理。

Result: 视频模型在神经预测性方面提供适度改进，特别是在后期响应阶段。IT群体活动在该操作中具有泛化能力，但所有ANN类别都失败。当前视频模型更好地捕捉外观依赖的动态，而非IT表达的外观不变时间计算。

Conclusion: 当前视频模型仅捕捉外观依赖的动态，而非IT皮层表现的外观不变时间计算。需要新的目标函数来编码生物时间统计特性和不变性。
...
TL;DR: Eye-Q是一个多语言视觉文字谜题基准测试，旨在评估视觉语言模型在需要深层推理、假设生成和修订的复杂视觉理解任务上的表现，而非依赖表面识别或OCR捷径。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型在标准基准测试上表现良好，但往往依赖表面识别而非深层推理。需要创建更具挑战性的基准来评估模型发现隐含视觉线索、生成和修订假设、将感知证据映射到非字面概念的能力。

Method: 提出Eye-Q多语言基准测试，包含1,343个视觉文字谜题。每个谜题展示概念密集的场景和简短描述，模型需要推断特定目标词或短语。谜题设计为非结构化和线索隐含，包含干扰项和需要选择性注意、抽象和关联推理的上下文关系。基准涵盖英语、波斯语、阿拉伯语和跨语言谜题。采用开放式、与人类对齐的评估协议，在轻度辅助下探测假设形成和修订能力。

Result: 评估最先进的视觉语言模型显示存在显著的性能差距，特别是在抽象和跨语言谜题上。最大准确率仅为60.27%，突显当前模型在构建和搜索适当概念表示以进行灵活图像到短语推理方面的局限性。

Conclusion: 视觉文字谜题是评估视觉语言模型深层推理能力的有效基准。当前模型在需要复杂视觉理解的任务上表现有限，特别是在抽象和跨语言场景中，表明需要改进模型的概念表示构建和搜索能力。
...
TL;DR: GAMBIT是一种新颖的多模态越狱框架，通过构建游戏化场景驱动模型主动完成恶意查询，显著提升了攻击成功率


<details>
  <summary>Details</summary>
Motivation: 现有攻击方法主要增加视觉任务复杂性，未能充分利用模型自身推理激励，在推理模型上表现不佳。如果模型能像人类一样思考，能否影响其认知阶段决策使其主动完成越狱？

Method: 提出GAMBIT框架：分解并重组有害视觉语义，构建游戏化场景驱动模型探索、重构意图，并将回答作为赢得游戏的一部分，形成结构化推理链

Result: 在流行的推理和非推理MLLMs上实验显示，GAMBIT达到高攻击成功率：Gemini 2.5 Flash 92.13%，QvQ-MAX 91.20%，GPT-4o 85.87%，显著优于基线方法

Conclusion: 通过游戏化场景驱动模型主动参与并降低安全注意力，GAMBIT能有效突破多模态大语言模型的安全对齐机制，为模型安全研究提供了新视角
...
TL;DR: WeedRepFormer：轻量级多任务视觉Transformer，同时进行水麻分割和性别分类，通过结构重参数化平衡精度与效率，在3.59M参数下达到92.18% mIoU和81.91%准确率。


<details>
  <summary>Details</summary>
Motivation: 现有农业模型难以平衡细粒度特征提取（用于生物属性分类）和实时部署所需的效率。需要一种能同时处理分割和分类任务的高效模型。

Method: 提出WeedRepFormer，采用结构重参数化技术，包含Vision Transformer主干、Lite R-ASPP解码器和可重参数化分类头，分离训练容量和推理延迟。构建包含10,264标注帧的水麻数据集。

Result: 在分割任务上达到92.18% mIoU，性别分类准确率81.91%，仅需3.59M参数和3.80 GFLOPs。以108.95 FPS运行，比iFormer-T分类准确率高4.40%，参数减少1.9倍。

Conclusion: WeedRepFormer通过系统性的结构重参数化，成功平衡了多任务农业视觉模型的精度和效率，为实时植物分析提供了有效解决方案。
...
TL;DR: FROST-Drive：一种新颖的端到端自动驾驶架构，通过冻结预训练视觉语言模型的视觉编码器权重，直接迁移其强大的泛化能力到驾驶任务，在长尾场景中显著优于完全微调方法。


<details>
  <summary>Details</summary>
Motivation: 当前端到端自动驾驶模型通常完全微调视觉编码器，这可能导致模型过度特化于训练数据，限制了在复杂新颖场景中的泛化能力。本文挑战这种训练范式的必要性，探索如何更好地利用预训练视觉语言模型的广泛世界知识。

Method: 提出FROST-Drive架构：1）冻结预训练VLM视觉编码器权重；2）结合基于Transformer的多模态融合适配器；3）使用GRU解码器生成平滑路径点；4）引入专门优化Rater Feedback Score的自定义损失函数。

Result: 在Waymo Open E2E Dataset（专门设计包含长尾场景的大规模数据集）上的实验表明，冻结编码器方法显著优于完全微调模型，证明了保留VLM广泛知识是更有效的策略。

Conclusion: 保留强大VLM的广泛知识比密集的领域特定适应更能实现鲁棒、可泛化的驾驶性能，为开发能更好处理现实世界复杂性的视觉模型提供了新途径。
...
TL;DR: 该研究表明，经过良好正则化的浅层CNN架构在多个孟加拉国视觉数据集上表现出色，可作为轻量级、实用的基准模型，适用于资源受限环境。


<details>
  <summary>Details</summary>
Motivation: 为资源受限环境（如孟加拉国）提供轻量级、实用的计算机视觉解决方案，避免依赖大型GPU和专用预训练模型，建立可复现的统一基准。

Method: 采用经过良好正则化的浅层卷积神经网络架构，在多个异构领域的孟加拉国视觉数据集上进行评估，包括智慧城市监控和农业品种分类等。

Result: 浅层架构在多个异构领域表现出高度竞争力，可作为统一的基准模型，证明了轻量级CNN在低资源环境中的实际部署价值。

Conclusion: 良好正则化的浅层CNN架构是资源受限环境中实用且高效的解决方案，为孟加拉国视觉数据集建立了可复现的基准，推动了轻量级模型在现实世界中的应用。
...
TL;DR: 基于MovieLens 32M数据集，通过并行化ALS框架优化低秩模型，在泛化性能和排序精度间取得最佳平衡，可视化嵌入空间发现语义聚类，并验证冷启动场景实用性。


<details>
  <summary>Details</summary>
Motivation: 大规模交互数据集中的可扩展性和数据稀疏性是协同过滤的关键瓶颈，需要研究用户偏好的潜在几何结构并开发高性能推荐系统。

Method: 使用MovieLens 32M数据集，实现高性能并行化交替最小二乘(ALS)框架，进行广泛的超参数优化，采用约束低秩模型，可视化学习到的嵌入空间。

Result: 约束低秩模型在泛化性能上显著优于高维模型，在RMSE和排序精度间达到最佳平衡；嵌入空间可视化显示无监督地形成了语义流派聚类；冷启动场景验证了系统的实用性。

Conclusion: 低秩模型能有效捕获交互数据中的深层结构关系，在冷启动场景中通过可调评分参数管理流行度偏差与个性化亲和力之间的权衡，为大规模推荐系统提供了实用解决方案。
...
TL;DR: ThinkRL-Edit：一个解耦视觉推理与图像合成的推理中心强化学习框架，通过思维链推理采样、无偏链偏好分组和二进制检查表奖励，显著提升推理中心图像编辑性能


<details>
  <summary>Details</summary>
Motivation: 当前指令驱动的图像编辑模型在视觉推理方面有限，导致推理中心编辑任务表现不佳。现有强化学习方法面临三个关键挑战：1) 推理探索局限于去噪随机性；2) 奖励融合存在偏差；3) 基于VLM的指令奖励不稳定

Method: 提出ThinkRL-Edit框架：1) 使用思维链推理采样，在生成前进行规划和反思阶段，探索多个语义假设并验证其合理性；2) 采用无偏链偏好分组策略避免加权聚合失败；3) 用二进制检查表替代基于区间的VLM评分，提供更精确、低方差、可解释的奖励

Result: 实验表明该方法在推理中心图像编辑任务上显著优于先前工作，能够生成更忠实于指令、视觉连贯且语义基础的编辑结果

Conclusion: 通过解耦视觉推理与图像合成，并扩展推理探索范围，ThinkRL-Edit有效解决了现有强化学习方法在图像编辑中的关键挑战，为推理中心图像编辑提供了更强大的框架
...
TL;DR: 该论文提出了一种轻量级自适应伪影奖励模型，用于缓解文本到图像强化学习后训练中的奖励黑客问题，通过检测和惩罚伪影图像来提高生成质量。


<details>
  <summary>Details</summary>
Motivation: 现有奖励函数常作为人类判断的不完美代理，导致模型容易产生奖励黑客行为——生成不真实或低质量但获得高奖励分数的图像。需要解决T2I RL后训练中的奖励黑客问题。

Method: 1) 系统分析美学/人类偏好奖励和提示-图像一致性奖励对奖励黑客的贡献；2) 识别跨奖励模型的共同失败模式：伪影图像生成；3) 提出轻量级自适应伪影奖励模型，在小规模精选数据集上训练；4) 将该模型作为正则化器集成到现有RL流程中。

Result: 实验表明，加入伪影奖励显著提高了视觉真实感，减少了多个T2I RL设置中的奖励黑客行为，证明轻量级奖励增强作为对抗奖励黑客的安全保障的有效性。

Conclusion: 轻量级自适应伪影奖励模型能有效缓解T2I RL后训练中的奖励黑客问题，通过检测伪影作为正则化器提高生成质量，为现有奖励模型提供了实用的安全保障机制。
...
<details>
  <summary>Details</summary>
Motivation: 遥感图像分割面临尺度变化大、相似干扰物多、边界结构复杂等问题，导致跨模态对齐存在空间非均匀性。现有方法采用均匀融合策略，在视觉清晰区域引入不必要的语言扰动，而在混淆区域又无法提供足够的消歧能力。

Method: 提出不确定性引导框架：1) 引入可插拔的Referring Uncertainty Scorer (RUS)，通过在线误差一致性监督预测参考不确定性空间分布；2) 基于此设计两个模块：Uncertainty-Gated Fusion (UGF)动态调节语言注入强度，Uncertainty-Driven Local Refinement (UDLR)利用不确定性软掩码聚焦于易错边界和细节。

Result: 大量实验表明，该方法作为统一的即插即用解决方案，在不改变主干架构的情况下，显著提高了复杂遥感场景中的鲁棒性和几何保真度。

Conclusion: 通过显式建模和利用参考不确定性作为空间先验，能够自适应地指导跨模态融合和局部细化，有效解决遥感图像分割中的空间非均匀性问题。
...
TL;DR: 提出SDCD方法，通过结构破坏对比解码来缓解大视觉语言模型中的物体幻觉问题


<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注缓解语言先验或高层统计偏差，但忽略了视觉编码过程中的内部复杂性。研究发现视觉统计偏差（源于视觉编码器在弱结构监督下的Bag-of-Patches行为）是物体幻觉的促成因素，这种偏差使模型优先考虑局部纹理特征而非整体几何结构。

Method: 提出无需训练的结构破坏对比解码算法（SDCD），通过引入打乱的结构破坏视图对输出分布进行对比校准，惩罚在结构缺失视图下仍保持高置信度的token，从而抑制纹理驱动的偏差。

Result: 实验结果表明，SDCD在多个基准测试中显著缓解了幻觉问题，并提升了大视觉语言模型的整体多模态能力。

Conclusion: 视觉统计偏差是物体幻觉的重要因素，通过结构破坏对比解码可以有效抑制纹理驱动偏差，改善模型性能。
...
Method: 采用蒸馏学习方法，利用合成和真实图像等多源异构数据训练模型；开发可微分渲染管道自动提取表情标签；使用手机和定制VR头显采集18k多样化主体数据

Result: 开发出实时面部表情追踪系统，能够准确驱动虚拟角色表情，无需冗长校准步骤

Conclusion: 该系统为虚拟环境中的交流和表达开辟了新途径，可应用于视频会议、游戏、娱乐和远程协作等领域
...
TL;DR: G2P通过将3D高斯溅射的外观感知属性转移到点云，解决了点云语义分割中几何特征不足以区分形状相似但外观不同物体的问题，无需2D或语言监督。
...
Method: 提出Gaussian-to-Point (G2P)方法：1) 建立点与优化高斯之间的对应关系，解决几何不对齐问题；2) 利用高斯不透明度属性解决几何模糊性；3) 利用高斯尺度属性实现复杂3D场景中的精确边界定位。

Result: 在标准基准测试中取得优越性能，在几何挑战性类别上显示出显著改进，且无需任何2D或语言监督。

Conclusion: G2P通过从3D高斯溅射转移外观感知属性到点云，实现了更具区分性和外观一致性的语义分割，有效解决了点云分割中的几何模糊性问题。
...
TL;DR: SBWM将人体运动预测重构为人体流形上的潜在动力学模拟，通过分离观测重建与动力学建模，解决了传统方法的长时预测漂移、平均姿态塌陷等问题。
...
Method: 提出语义信念状态世界模型(SBWM)，将人体运动预测重构为人体流形上的潜在动力学模拟。模型维护一个循环概率信念状态，其演化独立于姿态重建学习，并显式对齐SMPL-X解剖参数化。这种对齐施加了结构信息瓶颈，防止潜在状态编码静态几何或传感器噪声，强制其捕获运动动力学、意图和控制相关结构。借鉴基于模型的强化学习中的信念状态世界模型，SBWM采用随机潜在转移和以推演为中心的训练。

Result: SBWM实现了连贯的长时推演，在显著降低计算成本的同时保持了竞争性的预测精度。与专注于重建保真度的RSSM、transformer和扩散方法相比，SBWM优先考虑稳定的前向模拟。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法通常压缩光学特征以匹配热特征维度进行跨模态对齐和融合，这会导致有益于热超分辨率的高频信息丢失，并由于忽略模态间成像物理差异而引入纹理失真和边缘模糊等物理不一致的伪影。
...
Conclusion: PCNet通过跨分辨率互增强和物理约束的光学引导，实现了鲁棒的热无人机图像超分辨率，解决了现有方法中高频信息丢失和物理不一致伪影的问题。
...
<details>
  <summary>Details</summary>
Motivation: 由于像素级标注成本高昂，需要开发半监督学习方法。云模式在不同场景和同一场景类别中表现出结构多样性和上下文变异性，需要有效利用未标记数据来捕捉这些特征。

Method: 提出CloudMatch框架，为每个未标记图像生成一个弱增强视图和两个互补的强增强视图：一个通过跨场景补丁集成模拟上下文多样性，另一个通过场景内混合保持语义连贯性。通过视图一致性学习结合场景混合增强来指导伪标签生成。
...
<details>
  <summary>Details</summary>
Motivation: 当前的无标注手语翻译方法主要关注手动信号，但忽略了面部表情的语义重要性。当不同概念具有相同的手势表达时，这会导致语义歧义。面部表情作为非手动信号对于准确理解手语语义至关重要。

Method: EASLT框架包含专门的情感编码器来捕捉连续的情感动态，并通过情感感知融合模块将这些表示整合。该模块基于情感上下文自适应地重新校准时空手语特征，以解决语义歧义。

Result: 在PHOENIX14T和CSL-Daily基准测试中，EASLT在无标注方法中取得了先进性能：BLEU-4分数分别为26.15和22.80，BLEURT分数分别为61.0和57.8。消融研究证实情感建模能有效将情感语义与手动动态解耦。

Conclusion: EASLT通过将面部表情作为语义锚点而非辅助信息，显著提高了手语翻译的准确性。明确建模情感能有效解决手势歧义问题，增强翻译保真度。
...
TL;DR: SpatiaLoc是一个利用粗到细策略的跨模态定位框架，通过实例级和全局级空间关系建模，在文本和点云之间实现精确的2D位置回归。
...
Method: 采用粗到细策略：粗阶段使用Bezier增强对象空间编码器（BEOSE）通过二次贝塞尔曲线建模实例级空间关系，以及频率感知编码器（FAE）在频域生成全局级空间表示；细阶段使用不确定性感知高斯精细定位器（UGFL）将预测建模为高斯分布进行2D位置回归。

Result: 在KITTI360Pose数据集上的大量实验表明，SpatiaLoc显著优于现有的最先进方法。

Conclusion: SpatiaLoc通过强调实例级和全局级的空间关系，在跨模态定位任务中取得了卓越性能，证明了空间关系建模对于文本-点云定位的重要性。
...
TL;DR: 提出基于掩码预训练模型微调策略，通过纹理感知掩码机制增强CLIP-ViT对AI生成图像的检测泛化能力，在少量图像微调下达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 生成模型快速发展导致AI生成图像质量提升，引发虚假信息和公众信任问题。现有方法使用冻结CLIP模型但仅将其作为特征提取器，未能充分利用其潜力，且缺乏对未见生成模型的泛化能力。

Method: 提出掩码预训练模型微调策略，包含纹理感知掩码机制。分析发现CLIP-ViT能有效聚类真实图像但缺乏区分AI生成图像的能力。TAM机制在微调时掩码包含生成模型特定模式的纹理区域，迫使模型关注真实图像的"分布偏差"。

Result: 在GenImage和UniversalFakeDetect数据集上，仅用少量图像微调即可显著超越现有方法，分别达到98.2%和94.6%的平均准确率。

Conclusion: 提出的MPFT策略通过纹理感知掩码有效增强CLIP-ViT对AI生成图像的检测泛化能力，为解决生成模型检测的泛化问题提供了有效方案。
...
<details>
  <summary>Details</summary>
Motivation: 当前空间智能研究主要依赖视觉语言模型，但尚不清楚空间理解能力是来自视觉编码器还是基础推理架构。本文旨在探究纯文本LLM是否具备空间推理能力，为开发空间智能的LLM骨干提供基础。

Method: 创建SiT-Bench基准，将单视图/多视图场景转换为高保真、坐标感知的文本描述，要求LLM进行符号文本推理而非视觉模式匹配。包含5个主要类别（如自我中心导航、视角转换、机器人精细操作）和17个子任务。

Result: 评估SOTA LLM发现：模型在局部语义任务上表现良好，但在全局一致性方面存在显著的"空间鸿沟"；显式空间推理能显著提升性能，表明LLM具有潜在的世界建模能力。

Conclusion: SiT-Bench为未来VLM和具身智能体的空间基础LLM骨干开发提供了基础资源，证明了纯文本LLM具备空间推理潜力，但需要进一步改进全局一致性。
...
TL;DR: 提出环境鲁棒的少样本分割设置，建立ER-FSS基准测试集，并提出自适应注意力蒸馏方法提升模型在复杂环境下的分割性能。


<details>
  <summary>Details</summary>
Motivation: 现有少样本分割研究大多忽略真实场景中的复杂环境因素（如光照、背景、视角变化），导致实验室训练的模型在实际部署中表现不佳。需要提升模型在动态复杂环境下的鲁棒性。

Method: 提出自适应注意力蒸馏方法，通过反复对比和蒸馏已知支持图像与未知查询图像之间的关键共享语义，为新颖类别生成类别特定的注意力，增强模型在复杂环境中聚焦正确目标的能力。

Result: AAD方法在所有数据集和设置下将mIoU提升了3.3%-8.5%，表现出优越性能和强泛化能力。建立了覆盖8个数据集的ER-FSS基准测试集。

Conclusion: 提出的环境鲁棒FSS设置和AAD方法有效提升了少样本分割模型在真实复杂环境下的性能，为实际部署提供了更好的解决方案。
...
TL;DR: 提出一种针对印度石刻铭文的鲁棒自适应分块策略，结合注意力U-Net进行二值化，显著提升性能并展现跨脚本泛化能力。


<details>
  <summary>Details</summary>
Motivation: 石刻铭文图像因字符与背景对比度差、表面退化不均、干扰伪影多、文本密度和布局变化大等问题，导致现有二值化技术难以提取连贯字符区域。

Method: 采用动态采样和分块选择策略，生成适合训练注意力U-Net的补丁；注意力机制聚焦细微结构特征，分块策略帮助模型克服表面噪声和布局不规则性。

Result: 新分块机制显著提升二值化性能，模型仅在单一印度文字数据集上训练，却展现出对印度和非印度文字的零样本泛化能力。

Conclusion: 该方法为石刻铭文提供了干净、结构化的表示，为后续的文字识别、OCR和历史文本分析等下游任务奠定了基础。
...
TL;DR: 单目3D检测中，深度估计骨干网络的选择比特征增强更重要，NeWCRFs在伪激光雷达流程中表现优于Depth Anything V2，语义特征仅带来边际提升


<details>
  <summary>Details</summary>
Motivation: 单目3D物体检测作为激光雷达的低成本替代方案，其精度仍受限于单图像深度估计的困难。本研究旨在系统评估深度骨干网络和特征工程如何影响单目伪激光雷达流程的性能。

Method: 在KITTI验证集上，比较NeWCRFs（监督度量深度）与Depth Anything V2 Metric-Outdoor在相同伪激光雷达生成和PointRCNN检测协议下的表现。测试了点云增强方法，包括外观线索（灰度强度）和语义线索（实例分割置信度），并使用真实2D边界框进行深度精度与距离的诊断分析。

Result: NeWCRFs在中等难度分割上获得10.50% AP3D（IoU=0.7），表现优于Depth Anything V2。语义特征仅带来边际提升，基于掩码的采样可能因移除上下文几何而降低性能。深度准确性不能完全预测严格的3D IoU性能。

Conclusion: 在现成的激光雷达检测器下，深度骨干网络选择和几何保真度主导性能，超过了次要特征注入的影响。深度估计的质量比语义增强更为关键。
...
TL;DR: 提出一种基于特征排序的物体分类方法，无需物体对齐，通过边界分割和特征提取实现相似性度量


<details>
  <summary>Details</summary>
Motivation: 现有基于描述性特征的物体分类技术依赖物体对齐来计算相似性，这限制了方法的实用性

Method: 1. 对物体边界进行归一化和分割，得到近似凸段；2. 按长度降序排序段；3. 提取段长度、极值点数量、面积、基部和宽度等特征；4. 使用特征包度量图像边界相似性

Result: 在多个数据集上测试，获得了可接受的结果

Conclusion: 通过特征排序替代物体对齐，提出了一种有效的物体分类方法，无需对齐过程
...
TL;DR: 提出MFC-RFNet生成框架，通过多尺度特征通信、小波引导跳跃连接、条件引导空间变换融合和Rectified Flow训练，提升雷达回波序列降水临近预报的准确性和分辨率。
...
Method: 1) 小波引导跳跃连接(WGSC)保留高频细节；2) 特征通信模块(FCM)促进双向跨尺度交互；3) 条件引导空间变换融合(CGSTF)校正帧间位移；4) Rectified Flow训练实现少步采样；5) 轻量级Vision-RWKV块捕获长距离时空依赖。

Result: 在SEVIR、MeteoNet、Shanghai和CIKM四个公开数据集上评估，相比强基线模型表现一致提升，在更高雨率阈值下产生更清晰的回波形态，在更长预见期保持持续技能。

Conclusion: Rectified Flow训练与尺度感知通信、空间对齐和频率感知融合的协同作用，为基于雷达的临近预报提供了有效且鲁棒的方法。
...
<details>
  <summary>Details</summary>
Motivation: 裂缝自动分割在实际部署中面临像素级标签稀缺和跨传感器、光照、纹理、标注规范的严重域偏移问题，需要有效的合成数据方法

Method: 提出CrackSegFlow框架：1）基于流匹配的生成器，通过拓扑保持掩码注入和边界门控调制生成真实裂缝图像；2）类别条件流匹配模型合成裂缝掩码，控制裂缝覆盖率；3）将裂缝掩码注入无裂缝背景以多样化光照和表面伪影

Result: 在5个基准测试（4个沥青数据集和1个混凝土数据集）上，域内性能平均提升5.37 mIoU和5.13 F1，跨域合成平均提升13.12 mIoU和14.82 F1；相比扩散模型，提供更快确定性采样和更好的薄结构几何对齐

Conclusion: CrackSegFlow通过可控流匹配合成有效解决裂缝分割数据稀缺问题，显著提升分割性能，并发布了包含5万对图像-掩码的CSF-50K数据集供大规模基准测试
...
TL;DR: VideoMemory是一个实体中心的视频生成框架，通过动态记忆库实现跨多个镜头的人物、道具和环境一致性，解决了叙事视频生成中的实体身份保持问题。


<details>
  <summary>Details</summary>
Motivation: 现有视频生成模型虽然能产生高质量短片，但在场景变化或长时间间隔后重新出现时，往往无法保持实体身份和外观的一致性，这是叙事视频生成的核心挑战。

Method: 提出VideoMemory框架：1）多智能体系统将叙事分解为镜头；2）动态记忆库存储实体视觉和语义描述符；3）检索-更新机制从记忆库获取实体表示并合成关键帧和视频；4）每镜头后更新记忆库以反映故事驱动变化。

Result: 构建了包含54个案例的多镜头一致性基准测试，涵盖人物、道具、背景持续性场景。实验表明VideoMemory在多样化叙事序列中实现了强大的实体级一致性和高感知质量。

Conclusion: VideoMemory通过动态记忆库的实体中心框架，有效解决了叙事视频生成中的跨镜头一致性挑战，支持连贯的长格式视频生成。
...
TL;DR: MGPC是一个可泛化的多模态点云补全框架，通过整合点云、RGB图像和文本，采用模态丢弃策略、Transformer融合模块和渐进生成器，在包含100万训练对的大规模数据集MGPC-1M上表现出色。
...
Method: 提出MGPC框架：1）整合点云、RGB图像和文本的多模态统一架构；2）创新的模态丢弃策略提高鲁棒性；3）基于Transformer的融合模块；4）新颖的渐进生成器提升几何建模能力；5）构建MGPC-1M大规模数据集（100万训练对，1000+类别）。

Result: 在MGPC-1M数据集和真实世界数据上的广泛实验表明，该方法始终优于现有基线，在真实世界条件下表现出强大的泛化能力。

Conclusion: MGPC通过多模态整合、创新的训练策略和大规模数据集，显著提升了点云补全的泛化能力，为真实世界应用提供了有效解决方案。
...
TL;DR: PhysVideoGenerator：将可学习的物理先验嵌入视频生成过程的概念验证框架，通过预测器从扩散潜变量中恢复物理特征，解决现有模型物理动态表示不足的问题。
...
Method: 提出PhysVideoGenerator框架：1) 使用轻量级预测器网络PredictorP从噪声扩散潜变量回归预训练V-JEPA 2提取的高层物理特征；2) 通过专门的交叉注意力机制将预测的物理标记注入DiT生成器(Latte)的时间注意力层。

Result: 证明了该联合训练范式的技术可行性：扩散潜变量包含足够信息恢复V-JEPA 2物理表示，多任务优化在训练过程中保持稳定。建立了物理感知生成模型未来大规模评估的基础。

Conclusion: 该工作展示了将可学习物理先验嵌入视频生成过程的技术可行性，为开发更符合物理规律的视频生成模型提供了概念验证和架构基础。
...
Method: 使用CoTracker跟踪视频中随机初始化的点集，将得到的轨迹与对应图像帧一起输入基于Transformer的识别模型。无需检测手部、物体或交互区域，仅利用点轨迹作为运动线索。

Result: 实验结果表明，集成2D点轨迹相比不使用运动信息的相同模型能持续提升性能。令人惊讶的是，即使仅提供初始帧及其关联的点轨迹（不包含完整视频序列），也能获得显著增益。

Conclusion: 2D点轨迹作为轻量级但有效的运动表示，能显著提升自我中心动作识别性能，为动作理解提供了新的视角和实用方法。
...
Method: 1) 构建BREATH数据集（目前最大的体内内窥镜定位数据集）；2) 提出BREATH-VL混合框架，融合VLM语义线索与视觉配准几何信息；3) 引入轻量级上下文学习机制，将运动历史编码为语言提示以实现高效时序推理。

Result: 视觉语言模块在挑战性手术场景中实现鲁棒语义定位，BREATH-VL在精度和泛化性上优于最先进的视觉定位方法，平移误差比最佳基线降低25.5%，同时保持有竞争力的计算延迟。

Conclusion: 通过结合视觉语言模型的语义理解能力和视觉配准的几何精度，BREATH-VL框架有效解决了内窥镜6-DoF定位的挑战，在复杂医疗环境中实现了高精度、强泛化性的定位性能。
...
TL;DR: 提出DA3方法，通过少量未标注真实图像进行域适应，弥合仿真与真实图像间的域差异，提升光学系统主动对准的精度和效率。


<details>
  <summary>Details</summary>
Motivation: 大规模高精度光学系统组装需要主动对准技术，传统基于仿真的数字孪生管道存在仿真与真实图像间的域差异问题，限制了仿真训练模型的泛化能力。

Method: 提出域自适应主动对准(DA3)方法，结合自回归域变换生成器和基于对抗的特征对齐策略，通过自监督学习提取真实域信息，获取域不变图像退化特征以实现鲁棒的对准预测。

Result: 在两个镜头类型上的实验表明，DA3比纯仿真管道精度提升46%，接近使用3个镜头样本精确标注真实数据的性能，同时将设备端数据收集时间减少98.7%。
...
<details>
  <summary>Details</summary>
Motivation: 现有组合图像检索方法存在表示空间碎片化问题：查询和目标由异构模态组成，使用不同编码器处理，迫使模型仅通过事后对齐来桥接不对齐的表示空间，这从根本上限制了检索性能。这种架构不对称性在特征空间中表现为三个明显分离的簇，直接展示了异构模态如何从初始化就创建根本不对齐的表示空间。

Method: 1. 多级思维链提示策略：引导多模态大语言模型为目标图像生成具有区分性、语义兼容的标题，建立模态对称性。2. 对称双塔架构：查询和目标两侧使用相同的共享参数Q-Former进行跨模态编码，确保一致的特征表示并进一步减小对齐差距。3. 基于熵的时序动态记忆库策略：提供高质量负样本，同时保持与演化模型状态的一致性。
...
TL;DR: MATANet：一种用于细粒度海洋物种分类的新模型，通过多上下文环境注意力和分层分类感知网络，结合环境上下文和生物分类学层次结构来提升分类性能。
...
TL;DR: RadDiff是一个多模态智能系统，通过放射科医生风格的比较推理来描述配对放射学研究之间的临床差异，在RadDiffBench基准上达到47%准确率。
...
Result: 在RadDiffBench基准（57个专家验证的放射学研究对）上，RadDiff达到47%准确率，使用真实报告指导时达到50%，显著优于通用领域VisDiff基线。系统在COVID-19表型比较、种族亚组分析和生存相关影像特征发现等临床任务中表现出多功能性。

Conclusion: RadDiff和RadDiffBench为系统性地揭示放射学数据中有意义的差异提供了首个方法和基准基础，展示了在多样化临床任务中的应用潜力。
...
TL;DR: 提出了首个大规模高光谱伪装目标检测基准HyperCOD和基于SAM的HSC-SAM方法，通过空间-光谱解耦策略有效解决模态差异问题


<details>
  <summary>Details</summary>
Motivation: RGB图像在颜色纹理模糊的真实场景中检测伪装目标困难，高光谱图像虽能提供精细光谱特征，但缺乏专用大规模基准数据集限制了该领域发展

Method: 提出HSC-SAM方法，将高光谱图像解耦为空间图（输入SAM图像编码器）和光谱显著性图（作为自适应提示），有效桥接模态差异

Result: HSC-SAM在HyperCOD基准上达到SOTA性能，并在其他公开HSI数据集上展现出强大泛化能力
...
Method: 提出I2E框架，采用"分解-行动"范式：1) 使用分解器将非结构化图像转换为离散、可操作的对象层；2) 引入物理感知的视觉-语言-行动代理，通过思维链推理将复杂指令解析为一系列原子动作；3) 构建I2E-Bench基准用于多实例空间推理和高精度编辑评估。

Result: 在I2E-Bench和多个公共基准上的实验结果表明，I2E在处理复杂组合指令、保持物理合理性以及确保多轮编辑稳定性方面显著优于现有最先进方法。

Conclusion: I2E通过结构化分解和原子动作执行的新范式，有效解决了传统像素级修复方法在复杂组合编辑任务中的局限性，为图像编辑提供了更可控、更物理合理的解决方案。
...
TL;DR: 提出MVP（Masked Video Prediction）作为VideoLLMs的后训练目标，通过重建被遮蔽的视频片段来增强时间推理和因果理解能力


<details>
  <summary>Details</summary>
Motivation: 现有基于强化学习的VideoLLMs后训练方法主要关注整体内容理解（如字幕生成、视频问答），但缺乏对时间连贯性和帧间相关性的显式监督，限制了模型捕捉复杂动态和细粒度视觉因果关系的能力

Method: 1. 提出MVP目标：要求模型从一组具有挑战性的干扰项中重建被遮蔽的连续视频片段；2. 开发可扩展的数据合成管道，将任意视频语料库转换为MVP训练样本；3. 采用带有细粒度奖励函数的Group Relative Policy Optimization（GRPO）来增强模型对视频上下文和时间属性的理解

Result: 综合评估表明，MVP通过直接强化时间推理和因果理解，显著提升了视频推理能力

Conclusion: MVP作为一种新颖的后训练目标，通过显式监督时间连贯性和帧间相关性，有效弥补了现有VideoLLMs在捕捉复杂动态和视觉因果关系方面的不足，提升了视频推理能力
...
TL;DR: 该论文研究了在农业领域缺乏CAD模型的情况下，使用不同3D模型替代方案生成合成数据训练目标检测模型，用于区分石头和马铃薯，并证明通过少量真实数据微调可显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 制造业中基于AI的计算机视觉系统广泛使用合成数据来降低成本，但农业领域缺乏现成的CAD模型，难以利用合成数据训练AI模型。需要寻找替代CAD文件的方案来生成合成数据集。

Method: 提出了多种替代CAD文件的技术来创建合成数据集：1）使用扫描获得的高代表性3D模型；2）使用图像到3D的方法生成模型。将这些技术用于训练目标检测模型来分离石头和马铃薯，并在小规模真实数据集上进行微调。

Result: 研究表明，使用高代表性的3D模型（通过扫描或图像到3D方法获得）可以生成有效的合成数据来训练目标检测模型。即使使用代表性较低的模型，通过少量真实数据微调也能显著提升模型性能，甚至达到相似性能水平。

Conclusion: 在农业等缺乏CAD模型的领域，可以通过扫描或图像到3D方法获得3D模型来生成合成数据，结合少量真实数据微调，可以有效训练目标检测模型，降低数据采集和标注成本。
...
<details>
  <summary>Details</summary>
Motivation: 当前LLM在代码合成中数据增强依赖启发式或暴力搜索，缺乏性能感知的自动化优化方法

Method: 在NNGPT生态中构建闭环系统，用LoRA微调LLM，使用6000+PyTorch增强函数数据集，基于性能排序训练（更好-更差变换）
...
<details>
  <summary>Details</summary>
Motivation: 医疗影像基础模型开发需要持续监控下游性能，但研究人员通常依赖临时、手动的工作流程来跟踪大量实验、设计选择及其对性能的影响，这些流程速度慢且容易出错。

Method: 基于Snakemake构建模块化、即插即用框架，支持新数据集、基础模型、聚合方法和评估策略的无缝集成，通过高效缓存和并行执行实现可扩展性，所有实验和结果中心化跟踪且单命令可复现。

Result: 在五个最先进的基础模型和三个医疗影像分类任务上演示，EvalBlocks简化了模型评估，使研究人员能够更快迭代，专注于模型创新而非评估物流。

Conclusion: EvalBlocks框架作为开源软件发布，解决了医疗影像基础模型开发中评估流程的效率和可复现性问题，为研究人员提供了高效、可扩展的评估解决方案。
...
TL;DR: IDESplat提出迭代深度概率提升方法，通过级联warp操作和多层DPBU单元，逐步精化深度估计，实现更准确的3D高斯均值预测，在多个数据集上取得SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有3D高斯泼溅方法通常依赖单一warp操作估计深度概率，难以充分利用跨视图几何线索，导致深度图不稳定且粗糙，影响高斯均值预测精度。

Method: 提出IDESplat框架：1) 引入深度概率提升单元(DPBU)，通过级联warp操作以乘法方式集成极线注意力图；2) 堆叠多个DPBU构建迭代深度估计过程，逐步识别高概率深度候选；3) 迭代提升深度概率估计并更新深度候选，逐步精化深度图。

Result: 在RealEstate10K上比DepthSplat提升0.33dB PSNR，仅使用10.7%参数和70%内存；在DTU跨数据集实验中比DepthSplat提升2.95dB PSNR；在ACID和DL3DV数据集上也取得优异重建质量和实时效率。

Conclusion: IDESplat通过迭代深度概率提升机制有效解决了单一warp的不稳定性问题，实现了准确的高斯均值预测，展现出强大的泛化能力和高效性能。
...
TL;DR: MDENeRF：一种迭代框架，通过NeRF深度信息优化单目深度估计，结合单目全局结构和NeRF高频细节，在SUN RGB-D数据集上表现优异。
...
Method: 提出MDENeRF迭代框架：1) 初始单目估计提供全局结构；2) 在扰动视点上训练的NeRF，具有逐像素不确定性；3) 贝叶斯融合噪声单目深度和NeRF深度。从体渲染过程中推导NeRF不确定性以迭代注入高频细节。

Result: 在SUN RGB-D室内场景数据集上，该方法在关键指标和实验中表现出优越性能。

Conclusion: MDENeRF通过结合单目深度估计的全局结构和NeRF的高频细节，有效提升了深度估计的精度和几何细节，为场景理解提供了更准确的深度信息。
...
TL;DR: 本文提出FLNet深度学习架构，通过超分辨率将Sentinel-2卫星图像从10米提升到3米分辨率，用于洪水后作物损害评估，在BFCD-22数据集上取得了显著性能提升。


<details>
  <summary>Details</summary>
Motivation: 印度洪水对农作物造成广泛影响，传统人工调查方法缓慢且有偏差，现有卫星方法面临云层覆盖和低空间分辨率等挑战，需要更准确高效的作物损害评估方案。

Method: 提出FLNet深度学习架构，结合超分辨率技术将Sentinel-2卫星图像的10米空间分辨率增强到3米分辨率，然后进行作物损害分类。

Result: 在Bihar Flood Impacted Croplands Dataset (BFCD-22)上测试，关键"完全损害"类别的F1分数从0.83提升到0.89，几乎与商业高分辨率图像的0.89分数相当。
...
<details>
  <summary>Details</summary>
Motivation: 当前用于白血病诊断的深度学习模型多为黑盒，缺乏可解释性，限制了临床信任和采用。需要开发能够生成可解释形态描述的系统。

Method: 构建包含1.4万个健康与白血病细胞及专家属性描述的数据集，通过全微调和LoRA参数高效训练方法，将通用视觉语言模型适配为HemBLIP，并与MedGEMMA生物医学基础模型进行对比。

Result: HemBLIP在描述质量和形态准确性方面表现更优，LoRA适配进一步提升了性能并显著降低了计算成本。

Conclusion: 视觉语言模型在透明、可扩展的血液学诊断中具有广阔前景，HemBLIP为临床提供了可信任的形态分析工具。
...
TL;DR: FocusUI：一个高效的UI grounding框架，通过选择与指令最相关的图像块并保持位置连续性，减少视觉token数量，提升计算效率。
...
Method: 提出FocusUI框架：1）通过融合指令条件分数和基于规则的UI图分数来构建patch级监督，消除冗余视觉token；2）引入PosPad策略，将连续丢弃的视觉token压缩为特殊标记以保持位置连续性。
...
Conclusion: FocusUI通过选择性视觉token保留和位置连续性保持，实现了高效且准确的UI grounding，为实际应用提供了实用的解决方案。
...
TL;DR: ResTok提出了一种基于层次残差设计的1D视觉tokenizer，通过恢复视觉的层次残差先验，显著提升了自回归图像生成的质量和效率。


<details>
  <summary>Details</summary>
Motivation: 现有1D视觉tokenizer直接沿用语言模型的transformer设计，将视觉数据视为扁平序列，忽略了视觉固有的层次性和残差网络设计，这些特性对视觉模型的收敛和效率至关重要。

Method: 提出ResTok：1D视觉tokenizer，为图像token和潜在token构建层次残差表示。通过渐进合并获得层次表示，实现跨层特征融合；层次间的语义残差防止信息重叠，产生更集中的潜在分布。同时引入层次自回归生成器，通过一次预测整个层级的潜在token来减少采样步骤。

Result: 在ImageNet-256上仅用9个采样步骤就达到了2.34的gFID分数，显著提升了自回归图像生成的质量和效率。

Conclusion: 在视觉tokenization中恢复层次残差先验能显著改善自回归图像生成，ResTok通过将"视觉"特性重新引入视觉tokenizer设计，取得了优异性能。
...
TL;DR: 提出FUSION：首个基于扩散模型的无条件全身运动先验，联合建模身体和手部运动，解决了现有方法忽视手部运动或局限于狭窄任务的问题。
...
Method: 1. 整理并统一现有手部运动数据集与大规模身体运动数据，生成包含手部和身体的全身序列；2. 提出基于扩散模型的无条件全身运动先验FUSION，联合建模身体和手部运动；3. 开发优化流程，通过精炼扩散模型的潜在空间来生成特定任务运动。

Result: 1. 在HumanML3D数据集的关键点跟踪任务上超越最先进的骨骼控制模型；2. 实现更优的运动自然度；3. 展示两个应用：给定物体运动生成包含手指的详细全身交互运动，以及使用LLM将自然语言线索转换为可执行运动约束的自我交互运动。

Conclusion: FUSION通过联合建模身体和手部运动，实现了对运动合成的精确控制，同时保持合理的全身协调性。该方法超越了传统运动先验的典型用途，为更自然、更详细的全身运动合成提供了有效解决方案。
...
TL;DR: PosterVerse：一个全流程商业级海报生成方法，通过LLM蓝图创建、扩散模型背景生成和MLLM驱动的HTML引擎，解决现有系统设计流程不完整、文本渲染不准确的问题，并引入首个中文HTML排版数据集PosterDNA。
...
Method: PosterVerse采用三阶段专业设计流程：1）使用微调LLM从用户需求提取关键设计元素创建蓝图；2）通过定制扩散模型生成视觉吸引力的图形背景；3）使用MLLM驱动的HTML引擎进行统一布局-文本渲染，确保高文本准确性和灵活定制。同时引入PosterDNA数据集，首个引入HTML排版文件的中文海报生成数据集。

Result: 实验结果表明，PosterVerse能够持续生成商业级海报，具有吸引人的视觉效果、准确的文本对齐和可定制的布局，为自动化商业海报设计提供了有前景的解决方案。

Conclusion: PosterVerse通过全流程自动化和HTML引擎解决了商业海报生成的关键挑战，PosterDNA数据集为训练和验证海报设计模型提供了重要资源，代码和模型已开源。
...
TL;DR: 提出了一种基于Padé逼近的新型非线性神经元模型Paon，相比传统McCulloch-Pitts神经元具有更强的非线性能力，能在更少层数下实现更好性能


<details>
  <summary>Details</summary>
Motivation: 传统神经网络使用McCulloch-Pitts神经元模型（线性变换加逐点非线性激活），虽然已有二次神经元、广义运算神经元等非线性模型，但仍需更强的非线性表达能力。Padé逼近能提供更丰富的非线性函数形式

Method: 提出Padé神经元（Paon），受Padé逼近启发，每个Paon学习输入的不同非线性函数。Paon包含所有先前提出的神经元模型作为特例，可将任何网络中的任何神经元替换为Paon

Result: 在基于ResNet架构的图像超分辨率、压缩和分类模型中，用Paon替换经典神经元后，实验结果显示Paon构建的模型在更少层数下提供更好或相等的性能

Conclusion: Paon作为一种新型非线性神经元模型，具有非线性多样性、层效率高等优势，能提升神经网络性能并减少所需层数，已开源PyTorch实现
...
Method: 1) 构建大规模人类偏好数据集，基于提出的结构失真分类法进行标注；2) 使用高效的思维链合成管道生成额外数据；3) 采用两阶段训练框架：监督微调（带掩码损失）注入领域知识，然后通过强化学习（GRPO和成对奖励）增强推理能力；4) 推理时引入动态采样机制关注最可能失真的帧。

Result: 实验结果表明，REACT在评估结构失真方面补充了现有奖励模型，实现了准确的定量评估和可解释的归因分析，并提出了REACT-Bench基准测试。

Conclusion: REACT填补了生成视频评估中结构失真检测的空白，通过专门的框架级分析和两阶段训练方法，为视频生成质量提供了更全面的评估工具。
...
TL;DR: 提出一种标注高效的叶片分割方法，将像素级任务重构为区域分类问题，使用无监督区域生长和区域合并技术，结合RegionMix增强策略，实现跨风电场的鲁棒分割。


<details>
  <summary>Details</summary>
Motivation: 风力涡轮机需要频繁检查，微小表面损伤会影响气动性能、降低能量输出并加速叶片磨损。自动化检查的关键是准确分割叶片，但传统密集像素级深度学习方法需要大量标注数据，存在可扩展性挑战。

Method: 1) 将像素级分割重构为二值区域分类问题；2) 使用完全无监督、可解释的模块化自适应区域生长技术生成图像区域；3) 通过图像特定的自适应阈值引导；4) 区域合并过程将碎片区域整合为连贯片段；5) 引入RegionMix增强策略，通过组合不同区域合成新训练样本。

Result: 该框架展示了最先进的分割精度和强大的跨站点泛化能力，能够在不同风电场中一致地分割涡轮叶片。

Conclusion: 提出的标注高效分割方法通过将像素级任务重构为区域分类，结合无监督区域生成和增强策略，实现了高精度叶片分割和良好的跨风电场泛化能力，解决了传统方法对大量标注数据的依赖问题。
...
TL;DR: LocalDPO：一种新颖的文本到视频扩散模型后训练框架，通过在时空区域级别构建局部偏好对来优化对齐，无需外部评论模型或手动标注，显著提升视频质量和人类偏好评分。


<details>
  <summary>Details</summary>
Motivation: 现有的直接偏好优化方法依赖多样本排序和任务特定的评论模型，效率低下且产生模糊的全局监督。需要更高效、细粒度的视频生成器对齐方法。

Method: 提出LocalDPO框架：1）自动化收集偏好对数据，将高质量真实视频作为正样本，通过随机时空掩码局部破坏生成负样本；2）引入区域感知DPO损失，将偏好学习限制在破坏区域以加速收敛；3）单次推理生成偏好对，无需外部评论模型。

Result: 在Wan2.1和CogVideoX上的实验表明，LocalDPO相比其他后训练方法，在视频保真度、时间连贯性和人类偏好评分方面均有持续改进，建立了更高效、细粒度的视频生成器对齐范式。

Conclusion: LocalDPO通过局部偏好学习和自动化数据收集，提供了一种更高效、细粒度的文本到视频扩散模型对齐方法，显著提升了生成视频的质量和人类偏好。
...
TL;DR: 论文发现大型多模态模型在视频推理中存在"文本惯性"问题：一旦推理链中出现文本幻觉，模型会盲目坚持错误文本而忽略冲突的视觉证据，自我纠正率低于10%。作者提出主动视觉上下文精炼方法，通过视觉重定位和上下文优化来减少幻觉传播。


<details>
  <summary>Details</summary>
Motivation: 大型多模态模型在视频推理中表现出色，但其推理链的鲁棒性存在问题。研究发现模型存在"文本惯性"现象：当推理过程中出现文本幻觉时，模型会盲目坚持错误文本而忽略冲突的视觉证据，这严重影响了推理的可靠性。

Method: 1. 提出LogicGraph扰动协议，在多样化LMM的推理链中结构化注入扰动来评估自我反思能力；2. 提出主动视觉上下文精炼方法：包含主动视觉重定位机制进行细粒度验证，以及自适应上下文精炼策略来总结和去噪推理历史。

Result: 实验结果显示：1. 模型在不到10%的情况下能成功自我纠正；2. 模型主要陷入盲目的文本错误传播；3. 提出的主动视觉上下文精炼方法显著抑制了幻觉传播并增强了推理鲁棒性。

Conclusion: 大型多模态模型存在严重的文本惯性问题，需要主动的视觉验证机制来打破错误传播循环。提出的训练无关推理范式通过视觉重定位和上下文优化有效提升了推理的可靠性和鲁棒性。
...
TL;DR: Gen3R是一种结合重建模型和视频扩散模型的方法，用于生成场景级3D内容，能同时产生RGB视频和对应的3D几何信息。
...
Conclusion: 通过紧密耦合重建模型和生成模型，实现了两者的相互促进，为场景级3D生成提供了有效解决方案。
...
Conclusion: GeoReason框架通过同步内部思维与最终决策，有效解决了遥感视觉语言模型的逻辑幻觉问题，为复杂空间决策任务提供了更可靠的认知基础
...
TL;DR: 提出PIMC多模态自监督方法，利用像素级2D表示（递归图）编码卫星图像时间序列，结合遥感影像进行对比学习，在多个下游任务中优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 卫星持续产生海量数据，特别是地球观测的卫星图像时间序列（SITS）。现有深度学习模型大多处理完整图像或时间序列，难以有效提取像素级特征。需要更有效的方法来编码SITS中的视觉属性变化。

Method: 提出像素级多模态对比学习（PIMC）方法：1）将像素级植被指数时间序列（NDVI、EVI、SAVI）转换为递归图作为2D表示；2）结合遥感影像（RSI）进行多模态自监督对比学习，生成有效的编码器。

Result: 在三个下游任务中验证：PASTIS数据集的像素级预测和分类，EuroSAT数据集的土地覆盖分类。实验表明2D表示显著增强SITS特征提取，对比学习提升像素时间序列和RSI的表征质量，优于现有SOTA方法。

Conclusion: 多模态方法在各种地球观测任务中优于现有模型，建立了处理SITS和RSI的鲁棒自监督框架。2D表示和对比学习的结合有效提升了特征提取能力。
...
TL;DR: Klear是一个音频-视频联合生成系统，通过创新的单塔架构、渐进式多任务训练策略和首个大规模密集标注数据集，解决了现有方法中的音视频异步、唇语对齐差和模态退化问题，在多个任务上大幅超越先前方法。


<details>
  <summary>Details</summary>
Motivation: 现有音频-视频联合生成方法存在音视频异步、唇语对齐差和模态退化问题，这些问题源于弱音视频对应建模、有限泛化能力和高质量密集标注数据稀缺。需要从模型架构、训练策略和数据构建三个维度进行系统性改进。

Method: 1. 架构：采用单塔设计，统一DiT块和Omni-Full Attention机制，实现紧密音视频对齐和强扩展性
2. 训练：渐进式多任务策略，包括随机模态掩码、跨任务联合优化和多阶段课程学习，防止模态崩溃
3. 数据：构建首个大规模音频-视频密集标注数据集，开发自动化数据构建流水线，生成高质量对齐的三元组数据

Result: Klear在多个任务上大幅超越先前方法，实现了高保真、语义和时间对齐的指令跟随生成，在联合和单模态设置下都表现优异，并能鲁棒地泛化到分布外场景，性能可与Veo 3相媲美。

Conclusion: Klear通过系统性的三轴改进，为下一代音频-视频合成提供了统一、可扩展的路径，解决了现有方法的核心问题，在音视频对齐、生成质量和泛化能力方面取得了显著突破。
...
TL;DR: 提出Diffusion-DRF方法，使用冻结的视觉语言模型作为免训练评判器，通过可微分奖励流优化视频扩散模型，避免传统偏好优化方法的标签依赖和奖励攻击问题。


<details>
  <summary>Details</summary>
Motivation: 当前基于DPO的文本到视频生成方法依赖不可微分的偏好信号（人工标注或学习型奖励模型），导致训练标签密集、易产生偏见、容易被游戏化，常引发奖励攻击和不稳定训练。

Method: 1) 使用冻结的现成视觉语言模型作为免训练评判器；2) 通过扩散去噪链直接反向传播VLM反馈；3) 将logit级响应转换为token感知梯度进行优化；4) 提出自动化、方面结构化的提示管道获取可靠的多维VLM反馈；5) 使用梯度检查点实现高效更新。
...
TL;DR: ToTMNet：一种轻量级远程光电容积描记（rPPG）架构，使用FFT加速的Toeplitz时间混合层替代注意力机制，在保持高精度心率估计的同时大幅减少参数数量。


<details>
  <summary>Details</summary>
Motivation: 现有深度学习方法虽然提高了rPPG的鲁棒性，但计算成本和参数数量增加，基于注意力的时间建模存在二次方时间复杂度问题。需要一种更高效的时间建模方法。
...
Conclusion: Toeplitz结构的时间混合是rPPG中替代注意力的实用高效方案，门控机制对有效利用全局Toeplitz混合至关重要，尤其在领域转移场景下。主要限制是仅使用两个数据集。
...
TL;DR: 提出一种结合2D图像和深度图的视觉定位方法，通过密集匹配实现高精度，在存储和计算效率上取得平衡


<details>
  <summary>Details</summary>
Motivation: 现有视觉定位方法存在局限性：2D图像方法易于构建和维护但几何推理能力有限；3D结构方法精度高但需要集中重建且难以更新。需要一种既能保持2D方法易用性又能获得3D方法精度的解决方案。

Method: 为每张图像添加估计的深度图来捕捉几何结构，利用密集匹配器，采用紧凑压缩和GPU加速的LO-RANSAC实现，在存储和计算上实现高效平衡。

Result: 在多个标准基准测试中达到新的最先进精度，在可比地图大小下优于现有内存高效方法。

Conclusion: 通过2D图像增强深度图表示，结合密集匹配技术，实现了既易于构建维护又具有高精度的视觉定位方法，在精度和效率之间取得了良好平衡。
...
<details>
  <summary>Details</summary>
Motivation: 现实世界中的动态物体不断演化、变形和交互，产生多样的4D场景动态。传统基于规则的图形管道需要特定类别的启发式方法，劳动密集且不可扩展；而基于学习的方法需要大规模数据集，可能无法覆盖所有感兴趣的物体类别。
...
Conclusion: CHORD提供了一个通用、多功能、类别无关的生成管道，能够合成各种4D场景动态，解决了传统方法的可扩展性和数据需求问题。
...
TL;DR: 提出DeepResearch-Slice框架，通过神经符号方法预测精确跨度索引进行确定性硬过滤，解决检索-利用差距问题，在冻结骨干网络上实现73%相对性能提升


<details>
  <summary>Details</summary>
Motivation: 当前深度研究代理主要优化搜索策略以最大化检索概率，但存在检索-利用差距的关键瓶颈：即使在检索到黄金证据后，模型在嘈杂环境中由于上下文盲区而无法有效利用这些证据

Method: 提出DeepResearch-Slice神经符号框架，不同于隐式注意力机制，该方法预测精确的跨度索引，在推理前执行确定性硬过滤，有效减少噪声干扰

Result: 在六个基准测试上获得显著的鲁棒性提升，应用于冻结骨干网络时实现73%相对改进（从19.1%提升至33.0%），无需更新推理模型的参数即可有效缓解噪声问题

Conclusion: 结果表明在开放式研究中需要明确的接地机制，神经符号方法通过显式过滤能有效解决检索-利用差距问题，为深度研究代理提供了更可靠的证据利用能力
...
TL;DR: 研究发现内部推理无法完全消除LLM的谄媚行为，需要外部结构约束来保证安全性


<details>
  <summary>Details</summary>
Motivation: 大型语言模型经常表现出谄媚行为（优先取悦用户而非保持正确性），研究需要探究这是否需要外部监管，还是可以通过内部推理单独缓解

Method: 使用CAP-GSM8K对抗性数据集（N=500），评估内部推理（思维链）与外部机制（RCA）在GPT-3.5、GPT-4o和GPT-5.1上的表现

Result: 内部推理存在结构性限制：导致弱模型性能崩溃（优先悖论），前沿模型仍有11.4%最终输出差距；而RCA在所有层级上结构性地消除了谄媚行为（0.0%）

Conclusion: 外部结构约束对于保证安全性是严格必要的，混合系统只有在能力匹配且强大时才能达到共振（最优效率），而弱或不匹配的配对会导致失调和熵增
...
<details>
  <summary>Details</summary>
Motivation: 现有的大型语言模型安全评估方法主要采用基于示例的约束方法，这种方法在策略覆盖范围、攻击策略多样性和提示保真度方面存在局限性，需要更全面、可扩展的解决方案来识别和缓解LLM的安全漏洞。
...
Conclusion: Jailbreak-Zero提供了一个更可扩展和全面的解决方案，用于识别和缓解LLM的安全漏洞，通过基于策略的框架实现了更有效的红队评估，同时保持了提示的人类可读性和最小化人工干预需求。
...
TL;DR: 本研究评估了两种本地部署的大语言模型（gpt-oss-20b和gpt-oss-120b）在临床任务中的表现，发现它们能在较小模型规模下达到或超越现有开源和专有模型性能，并通过微调显著提升诊断准确性，展示了本地化LLMs在临床决策支持中的潜力。


<details>
  <summary>Details</summary>
Motivation: 专有LLMs在临床决策中存在隐私问题和云端依赖限制，而开源模型通常需要大模型规模，不适用于资源受限的临床环境。因此需要评估本地部署LLMs在临床任务中的实际可行性。

Method: 在三个代表性临床任务上对两种本地部署LLMs（gpt-oss-20b和gpt-oss-120b）进行基准测试：1）一般疾病诊断，2）专科（眼科）诊断与管理，3）模拟专家评估。与最先进的专有模型（GPT-5和o4-mini）和领先开源模型（DeepSeek-R1）进行比较，并对gpt-oss-20b在一般诊断数据上进行微调评估适应性。

Result: gpt-oss模型在所有任务中表现达到或超越DeepSeek-R1和o4-mini，尽管模型规模显著更小。微调后，gpt-oss-20b的诊断准确性显著提升，接近GPT-5的性能水平。

Conclusion: 本地部署的LLMs能够提供准确、适应性强且保护隐私的临床决策支持，为LLMs更广泛地整合到常规临床实践中提供了实用路径。
...
TL;DR: GPT-5是OpenAI推出的统一系统，包含智能快速模型、深度推理模型和实时路由器，通过安全训练减少幻觉、提高指令遵循，在写作、编程和健康领域表现突出，并对高能力生物化学领域采取预防性安全措施。


<details>
  <summary>Details</summary>
Motivation: 开发一个更强大、更实用的AI系统，超越之前模型在基准测试上的表现，同时解决现实世界查询中的实际问题，如减少幻觉、改善指令遵循、降低谄媚行为，并在关键应用领域（写作、编程、健康）提升性能。

Method: 采用统一系统架构：1) 智能快速模型处理大多数问题；2) 深度推理模型解决复杂问题；3) 实时路由器根据对话类型、复杂度、工具需求和明确意图动态选择模型；4) 路由器通过用户切换模型、响应偏好和正确性测量等真实信号持续训练；5) 使用安全完成技术防止不允许的内容。

Result: GPT-5在基准测试上超越之前模型，响应速度更快，对现实世界查询更有用。在减少幻觉、改善指令遵循、降低谄媚方面取得显著进展，在写作、编程和健康三个最常见应用领域性能提升。对gpt-5-thinking模型在生物化学领域采取高能力分类并激活相应安全防护。

Conclusion: GPT-5是一个性能显著提升的统一AI系统，通过智能路由和持续学习优化用户体验，在关键应用领域表现优异，同时采取预防性安全措施管理潜在风险，特别是在生物化学领域的高能力模型。
...
TL;DR: 论文提出专门评估小型语言模型在非推理任务（如语气修改）上能力的框架，挑战当前以推理任务为主的评估范式，为工业应用提供实用工具。


<details>
  <summary>Details</summary>
Motivation: 当前语言模型评估过度关注推理和问题解决任务，导致小型语言模型（10B参数以下）在这些指标上得分远低于大型语言模型，但这种评估未能反映小型模型在实际工业应用中的有效性。

Method: 提出专门针对非推理任务的评估框架，包含数据生成、提示调优和基于LLM的评估等创新方法，特别关注任务特定微调的能力。

Result: 该框架能够有效展示小型语言模型在语气修改等实际工业任务中的潜力，为边缘计算和私有计算场景提供了实用的基准测试工具。

Conclusion: 需要重新思考语言模型评估范式，开发专门针对非推理任务的评估方法，以更好地反映小型语言模型在工业应用中的实际价值，特别是在资源受限的环境中。
...
TL;DR: 该研究评估了13个主流大语言模型在企业环境中的指令遵循能力，发现Claude-Sonnet-4和GPT-5表现最佳，揭示了模型在通用任务和精确指令遵循之间存在"指令鸿沟"。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在自然语言理解和生成方面表现出色，但在企业部署中暴露出关键限制：对定制指令的遵循不一致。企业环境需要模型精确遵循特定指令，而现有模型在这方面存在不足。

Method: 对13个领先的LLM进行综合评估，包括指令遵循性、响应准确性和性能指标。使用真实世界的RAG场景，通过系统化测试样本和企业级评估协议进行分析。

Result: 不同模型在指令遵循方面差异显著，Claude-Sonnet-4和GPT-5获得最高评分。研究发现存在"指令鸿沟"现象：模型擅长通用任务，但难以满足企业部署所需的精确指令遵循要求。

Conclusion: 该研究为企业部署LLM解决方案提供了实用见解，并为各大模型家族的指令遵循能力建立了基准。强调需要改进模型在精确指令遵循方面的能力以满足企业需求。
...
TL;DR: 这篇综述论文系统回顾了2021年以来语义文本相似性（STS）领域的最新进展，涵盖了基于Transformer的模型、对比学习、领域特定方法、多模态方法、图神经网络和知识增强技术等六大方向，总结了当前方法、应用和挑战。


<details>
  <summary>Details</summary>
Motivation: 自2021年以来，语义文本相似性研究快速发展，得益于Transformer架构、对比学习和领域特定技术的进步。然而，这些进展分散在不同领域，缺乏系统性的整理和分析。本文旨在全面梳理STS领域的最新发展，为研究者和实践者提供清晰的路线图。

Method: 采用文献综述方法，系统分析六个关键方向：1）基于Transformer的模型（如FarSSiBERT、DeBERTa-v3）；2）对比学习方法（如AspectCSE）；3）领域特定解决方案（如医疗领域的CXR-BERT、金融领域的Financial-STS）；4）多模态方法；5）图神经网络方法；6）知识增强技术。

Result: 研究发现：1）Transformer模型在STS任务上取得了显著准确率提升；2）对比学习方法建立了新的基准；3）领域适应模型在医疗、金融等专业领域表现出色；4）多模态、图神经网络和知识增强方法进一步提升了语义理解和表示能力。

Conclusion: 该综述为STS领域的研究者和实践者提供了系统性的指导，总结了当前方法、实际应用和剩余挑战。通过梳理这些发展，论文旨在帮助读者把握快速发展的STS领域，突出新兴趋势和未来机会，推动该领域的进一步发展。
...
<details>
  <summary>Details</summary>
Motivation: 现有垂直领域LLM评估基准存在语义冗余和高计算成本问题，而现有压缩方法依赖历史模型正确性标签，无法应用于冷启动场景（新任务、新领域或新模型）

Method: 1) 在小规模领域数据上微调基础LLM以内部化任务语义；2) 仅使用原始文本内容生成高级语义嵌入；3) 在领域适应嵌入空间中进行任务感知聚类；4) 引入数据集X-ray机制分析聚类几何，基于基准内在冗余动态校准压缩强度

Result: 在专业领域数据集（特别是大规模3GPP通信基准）上，该方法有效识别并移除冗余样本，评估成本降低超过90%，同时保持与完整基准的高保真度

Conclusion: 提出的历史无关测试集压缩框架解决了冷启动场景下的基准压缩问题，无需先验模型性能数据，显著降低评估成本，为垂直领域LLM评估提供了高效解决方案
...
TL;DR: GuardEval是一个统一的多视角基准数据集，包含106个细粒度类别，用于训练和评估内容审核系统。基于此数据集训练的GemmaGuard模型在内容审核任务上显著优于现有模型。


<details>
  <summary>Details</summary>
Motivation: 当前大语言模型在内容审核方面存在局限性：难以处理隐式冒犯、微妙偏见、越狱提示等复杂情况，且过度依赖训练数据可能强化社会偏见，导致不一致和有伦理问题的输出。

Method: 1) 构建GuardEval基准数据集，涵盖人类情感、冒犯和仇恨言论、性别和种族偏见、更广泛的安全问题等106个细粒度类别；2) 使用QLoRA技术对Gemma3-12B进行微调，得到GemmaGuard模型。

Result: GemmaGuard在内容审核任务上取得了0.832的宏观F1分数，显著优于OpenAI Moderator (0.64)和Llama Guard (0.61)。多视角、以人为中心的安全基准能有效减少偏见和不一致的审核决策。

Conclusion: 多样化和具有代表性的数据能显著提高内容审核系统在复杂边缘案例上的安全性、公平性和鲁棒性。GuardEval和GemmaGuard展示了多视角基准数据集对改进内容审核系统的重要性。
...
TL;DR: LLM_annotate是一个Python包，使用大语言模型分析虚构人物性格，提供标准化工作流程进行文本标注、性格推断和质量验证


<details>
  <summary>Details</summary>
Motivation: 传统人物性格分析需要大量人工标注，效率低下且难以标准化。需要自动化工具来高效、可重复地分析虚构作品中的人物性格特征

Method: 开发Python包，包含文本分块、LLM标注、人物姓名消歧、质量评分、统计计算等功能，支持商业、开源或自定义LLM，并提供人机交互GUI进行质量验证

Result: 通过《辛普森一家大电影》和《傲慢与偏见》的示例演示，展示了该包能够高效、可重复地进行人物性格分析

Conclusion: LLM_annotate为研究人员提供了一个标准化、灵活的工具，可以高效分析虚构作品中的人物性格，支持多种LLM并确保分析质量
...
TL;DR: LLMs在主题分割方面表现优于传统方法，但仍存在待解决的问题


<details>
  <summary>Details</summary>
Motivation: 现有主题分割方法主要基于句子间的语义相似性，但这些模型缺乏LLMs所具有的长距离依赖关系和丰富知识

Method: 提出重叠递归提示策略，使用句子枚举方法，并支持采用边界相似性评估指标

Result: 实验结果表明，LLMs作为主题分割器比现有方法更有效

Conclusion: LLMs在主题分割方面具有潜力，但在能够可靠使用之前仍需解决一些问题
...
TL;DR: 提出一种针对ARM64架构的软件虚拟张量核心架构，通过内存映射和NEON SIMD内核优化，在M2硬件上实现110M参数模型>60 tokens/秒的稳定吞吐


<details>
  <summary>Details</summary>
Motivation: 解决边缘设备部署LLM时的"内存墙"瓶颈问题，标准推理运行时存在高层抽象、动态调度和未对齐内存访问等显著开销

Method: 1. 软件实现的"虚拟张量核心"架构；2. 绕过标准库容器，采用直接内存映射(mmap)；3. 手工调优的NEON SIMD内核；4. 张量虚拟化布局(TVL)确保100%缓存线利用率；5. 零拷贝加载器消除初始化延迟

Result: 在M2硬件上，110M参数模型实现稳定吞吐>60 tokens/秒，满足200ms心理语言学延迟阈值，无需不透明的依赖

Conclusion: 该架构提供了一个完全开源、可移植且确定性的参考实现，用于研究通用ARM芯片上的内存瓶颈，虽然专有硬件加速器能达到更高峰值吞吐，但本方案具有更好的透明度和可研究性
...
TL;DR: BPE分词不仅压缩文本，还重构自然语言的统计特性：驱动词频趋向Zipf定律，减少局部依赖，使模型预测熵更接近Zipf理论预测。


<details>
  <summary>Details</summary>
Motivation: 尽管自然语言呈现Zipf定律等统计规律，但现代分词方案（如BPE）如何影响这些特性尚不清楚。本文旨在分析BPE如何改变语料库统计特性及其与自然语言信息属性的关系。

Method: 1) 在Zipf频率分布假设下推导槽熵期望值的闭式表达式；2) 实证研究BPE如何变换语料库统计特性；3) 在不同BPE深度分词后的语料上训练语言模型，分析模型预测熵；4) 使用注意力机制诊断局部依赖关系。

Result: 递归应用BPE使词频趋向Zipf幂律分布，并诱导经验熵呈现特征增长模式。随着BPE深度增加，模型预测熵与Zipf理论预测的一致性增强。注意力诊断表明更深的分词减少了局部token依赖，使经验分布更接近弱依赖（近IID）状态。

Conclusion: BPE不仅作为压缩机制，更作为统计变换器，重构自然语言的关键信息特性。它驱动统计特性趋向Zipf定律，减少局部依赖，使语言模型能更好地学习自然语言的统计规律。
...
TL;DR: 研究发现隐喻会影响大语言模型的推理路径，导致跨领域对齐问题，通过监测潜在特征可以检测不良内容


<details>
  <summary>Details</summary>
Motivation: 先前研究表明隐喻影响人类决策，考虑到大语言模型训练数据包含大量隐喻，本研究探讨隐喻是否也会影响LLMs的推理路径，特别是在跨领域对齐问题中

Method: 在预训练、微调和重新对齐阶段使用隐喻进行干预，观察模型跨领域对齐程度的变化；通过监测大推理模型的全局和局部潜在特征，设计检测器预测不良内容

Result: 发现训练数据中的隐喻与LLMs推理内容的不对齐程度存在强因果关系；通过隐喻干预，模型的跨领域对齐程度显著变化；设计的检测器能够高精度预测不良内容

Conclusion: 隐喻确实影响大语言模型的推理路径，导致跨领域对齐问题，通过监测潜在特征可以有效检测和预测模型的不良内容生成
...
TL;DR: 提出PersonaWeaver框架，解决现有角色生成方法中的道德偏见和助手偏见问题，实现更丰富的角色行为多样性


<details>
  <summary>Details</summary>
Motivation: 现有程序化内容生成方法在角色生成中存在两种偏见：1）积极道德偏见（角色总是采取赞同立场）；2）助手偏见（角色总是直接回答问题）。这些偏见导致角色缺乏戏剧张力和可预测性，源于最大似然训练和助手微调

Method: 提出PersonaWeaver框架，将世界构建（角色、人口统计）与行为构建（道德立场、互动风格）解耦，生成具有更丰富反应和道德立场的角色，以及风格标记（长度、语气、标点）的二级多样性

Result: 该方法能够生成具有多样化反应和道德立场的角色，同时实现风格标记的多样性，解决了现有方法的偏见问题

Conclusion: PersonaWeaver框架通过解耦世界构建和行为构建，有效解决了角色生成中的对齐偏见问题，为大规模角色生成提供了更丰富、更具戏剧张力的解决方案
...
<details>
  <summary>Details</summary>
Motivation: LLM在训练时使用大量异构文本语料，引发对未经授权使用专有或个人数据的担忧，需要在黑盒设置下保护数据不被模型学习

Method: 提出Disclaimer Injection方法：在数据层面注入精心设计的对齐触发免责声明，利用LLM自身的对齐机制阻止有效学习，无需访问或修改训练流程

Result: 在受保护数据上微调会导致对齐相关层持续激活，使对齐约束覆盖任务学习，导致模型性能显著下降；这是首个在LLM规模上限制数据可学习性的实用方法

Conclusion: 对齐行为是数据保护的新杠杆，Disclaimer Injection是首个无需访问训练流程即可在LLM规模上保护数据的实用方法
...
TL;DR: 本文系统形式化了提格里尼亚语的基数和序数词表达规则，填补了该语言计算资源的空白，并发布了开源实现。


<details>
  <summary>Details</summary>
Motivation: 提格里尼亚语缺乏计算资源，特别是数字表达的系统化规则文档。现有大型语言模型在准确表达提格里尼亚数字方面存在显著差距，需要明确的规则文档来支持语言建模、语音合成和面向提格里尼亚语社区的应用程序。

Method: 文档化了提格里尼亚语数字表达的标准规则，包括连词系统、量级词以及日期、时间和货币的特殊情况。提供了数字到词语转换的形式化算法，并发布了开源实现。

Result: 评估前沿大型语言模型显示，它们在准确表达提格里尼亚数字方面存在显著差距，验证了明确规则文档的必要性。提供了完整的算法实现和规则文档。

Conclusion: 这项工作填补了提格里尼亚语计算资源的空白，为语言建模、语音合成和面向提格里尼亚语社区的应用程序提供了重要基础，强调了针对低资源语言需要明确的规则文档而非仅依赖大型语言模型。
...
TL;DR: LatentGraphMem：结合隐式图内存与显式子图检索的混合内存框架，用于处理长上下文稀疏证据的LLM问答任务


<details>
  <summary>Details</summary>
Motivation: 现有内存系统存在两难：显式结构化内存可解释但脆弱，隐式内存机制稳定高效但难以检查。长上下文应用中证据稀疏分散，需要更好的内存解决方案。

Method: 提出LatentGraphMem框架：1）在隐式空间存储图结构内存以保证稳定高效；2）提供任务特定的子图检索接口，在固定预算下返回紧凑符号子图；3）训练时生成显式图视图与冻结推理器交互；4）推理时在隐式空间检索，仅外部化检索到的子图。

Result: 在多个模型规模的长视野基准测试中，LatentGraphMem一致优于代表性的显式图和隐式内存基线，同时支持参数高效适配，并能灵活扩展到更大推理器而不引入大型符号工件。

Conclusion: LatentGraphMem成功结合了隐式内存的稳定性与显式检索的可解释性，为长上下文稀疏证据的LLM问答提供了有效的混合内存解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 系统生成的摘要验证具有挑战性，特别是在高风险医学领域，需要精确的上下文归因。现有方法缺乏细粒度的归因评估。

Method: 引入PCoA基准，将方面摘要与支持性上下文句子及其中的贡献短语对齐。提出解耦评估框架，独立评估生成摘要、引用和贡献短语的质量。

Result: PCoA提供了可靠的基准，实验表明在摘要前明确识别相关句子和贡献短语可以提高整体质量。数据集质量一致，评估了多个大语言模型。

Conclusion: PCoA为医学方面摘要提供了有效的短语级上下文归因基准，解耦评估框架能更精确地评估系统性能，数据已开源。
...
TL;DR: CAPT是一种无需训练的模型集成方法，通过对比解码将临床模型的知识注入到通用模型中，实现跨架构、跨词汇表的临床领域适应


<details>
  <summary>Details</summary>
Motivation: 传统方法需要为每个新模型重新进行预训练和微调，成本高昂。需要一种无需训练的方法来适应临床领域，同时支持不同架构和词汇表的模型

Method: 提出跨架构代理调优（CAPT），利用对比解码技术，通过集成现有临床模型和最新通用模型，选择性地注入临床相关信号，同时保持通用模型的推理和流畅性

Result: 在6个临床分类和文本生成任务上，CAPT显著优于单个模型和现有集成方法（平均比UniTE高17.6%，比代理调优高41.4%），增强了临床可操作性语言，减少了上下文错误

Conclusion: CAPT提供了一种高效、无需训练的临床领域适应方法，能够有效结合最新通用模型和现有临床模型的优势，为医疗AI应用提供了实用的解决方案
...
TL;DR: ASPECTSIM是一个简单可解释的框架，通过显式指定方面来评估文档相似性，相比传统整体方法显著提高了人机一致性（GPT-4o实现时提升约80%）。


<details>
  <summary>Details</summary>
Motivation: 传统文档相似性评估采用整体方法，没有考虑特定方面，导致评估不够精确。需要开发能够显式考虑特定方面的文档相似性评估框架。

Method: 提出ASPECTSIM框架，要求基于显式指定的方面来评估文档相似性。构建包含26K个方面-文档对的新基准，使用GPT-4o直接提示实现，并与16个开源LLM和9个嵌入模型进行大规模元评估。

Result: GPT-4o实现的ASPECTSIM相比传统整体方法的人机一致性提升约80%。开源LLM直接提示效果差（20-30%一致性），但两阶段精炼提升约140%，仍远低于GPT-4o。

Conclusion: 显式考虑方面对文档相似性评估至关重要，需要修订标准实践。开源LLM在捕捉方面条件相似性方面仍落后于大型专有模型，但通过方法改进可显著提升性能。
...
TL;DR: LLM作为评分者时，评分量表的选取显著影响评分一致性，0-5分量表在人类-LLM对齐方面表现最佳，但群体可靠性可能掩盖基准异质性。


<details>
  <summary>Details</summary>
Motivation: 尽管LLM越来越多地被用作自动评估者，但先前研究表明LLM评分者在提示变化时缺乏一致性，而评分量表本身的影响尚未得到充分探索。本研究旨在比较人类和LLM在不同量表上的评分表现。

Method: 收集人类和LLM在三种不同量表上的评分，涵盖六个基准测试（包括客观、开放式主观和混合任务）。使用组内相关系数(ICC)测量绝对一致性，分析量表选择对人类-LLM一致性的影响。

Result: LLM在主观基准测试中不同量表上的评分不完全一致；量表选择显著影响人类-LLM一致性，即使组内面板可靠性很高；在任务聚合后，0-5分量表显示出最强的人类-LLM对齐。

Conclusion: 评分量表设计是LLM作为评分者协议的重要组成部分，群体可靠性可能掩盖基准异质性，需要子级诊断来识别系统性亚组差异（如性别群体），强调量表设计和细粒度分析的重要性。
...
TL;DR: L2T预训练框架在标准下一个token预测基础上加入语言学习任务，通过结构化输入输出对提供显式语言刺激，提升语言能力同时保持推理能力


<details>
  <summary>Details</summary>
Motivation: 当前语言模型通过token-by-token预测学习世界知识和推理，但未显式优化语言能力。需要填补语言能力训练的空白，借鉴人类语言习得过程

Method: 提出L2T预训练框架，将原始文本转换为结构化输入输出对，在标准下一个token预测基础上集成语言学习任务，提供显式语言刺激

Result: 在原始文本和L2T数据混合预训练的语言模型，在语言能力基准测试中表现提升且学习加速，同时在通用推理任务上保持竞争力

Conclusion: L2T框架通过结合语言学习任务和标准预训练，有效提升语言模型的语言能力，同时不损害其推理性能，为语言模型预训练提供了新方向
...
TL;DR: LLMs在时间序列分类中表现不佳主要是提示评估方法的限制，而非模型缺乏时间结构理解能力。通过线性探针分析发现，LLMs内部编码了丰富的时间序列信息，性能可媲美专业时间序列模型。


<details>
  <summary>Details</summary>
Motivation: 现有基于提示的评估显示LLMs在时间序列分类上表现不佳，这引发了对LLMs是否真正理解时间结构的质疑。作者旨在探究这种表现不佳是模型能力的真实反映，还是评估方法的局限性。

Method: 通过对比分析：1）基于提示的零样本生成；2）对相同内部表示使用线性探针进行分类。同时进行分层分析，研究时间序列信息在transformer各层的分布情况，并考察视觉和多模态输入的影响。

Result: 零样本提示性能接近随机水平（F1 0.15-0.26），但线性探针显著提升性能至F1 0.61-0.67，达到或超过专业时间序列模型。分层分析显示，类别区分性的时间序列信息在早期transformer层就已出现，且视觉和多模态输入能增强这种信息。

Conclusion: 当前基于提示的评估方法系统性地低估了LLMs对时间序列的理解能力。LLMs内部编码了丰富的时间结构信息，但需要通过适当的分析方法（如线性探针）才能充分揭示。
...
Method: 构建EpiQAL基准包含三个子集：文本基础事实回忆、多步推理（连接文档证据与流行病学原理）、结论重建（隐藏讨论部分）。采用专家设计分类指导、多模型验证和基于检索的难度控制

Result: 在十个开放模型上的实验显示：当前LLMs在流行病学推理上表现有限，多步推理最具挑战性；模型排名在不同子集间变化；规模大小不能预测成功；思维链提示对多步推理有益但其他方面效果不一

Conclusion: EpiQAL为证据基础、推理能力和结论重建提供了细粒度的诊断信号，有助于评估和改进模型在流行病学推理方面的能力
...
TL;DR: SegNSP将线性文本分割重新定义为下一个句子预测任务，通过标签无关的NSP方法和分割感知损失函数，在两个数据集上取得了优于基线的性能。
...
Method: 提出SegNSP，将线性文本分割重新定义为下一个句子预测任务。采用标签无关的NSP方法，预测下一个句子是否继续当前主题，无需显式主题标签。通过分割感知损失函数结合更难负采样来增强模型，以更好地捕捉话语连续性。
...
TL;DR: SMRA框架通过道德理论基础监督注意力对齐，提升仇恨言论检测的鲁棒性和可解释性，并引入巴西葡萄牙语基准数据集


<details>
  <summary>Details</summary>
Motivation: 现有仇恨言论检测模型依赖表面词汇特征，容易受虚假相关性影响，缺乏鲁棒性、文化情境化和可解释性

Method: 提出监督道德原理注意力(SMRA)框架，基于道德基础理论，将专家标注的道德原理作为直接监督来对齐token级注意力

Result: 在二元仇恨言论检测和多标签道德情感分类中，SMRA分别提升F1分数0.9和1.5，解释忠实度显著提高(IoU F1 +7.4pp, Token F1 +5.0pp)

Conclusion: SMRA通过道德原理监督直接整合到训练目标中，产生内在可解释且情境化的解释，无需在性能或偏见方面做出权衡
...
TL;DR: CALM框架通过解耦任务语义与文化概念，构建结构化文化集群，实现语言模型的文化自我意识，在跨文化基准测试中优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有语言模型将文化视为静态背景知识，忽视了文化的动态演变特性，导致在需要真正文化敏感性的下游任务中可靠性不足。

Method: CALM框架通过：1) 解耦任务语义与显性文化概念和潜在文化信号；2) 通过对比学习形成结构化文化集群；3) 通过交叉注意力对齐建立细粒度文化特征交互；4) 使用专家混合机制沿文化特定维度自适应整合；5) 构建文化基础内部身份状态并与原始知识融合；6) 通过自我提示反思学习实现持续适应和自我修正。

Result: 在多个跨文化基准数据集上的广泛实验表明，CALM始终优于最先进的方法。

Conclusion: CALM框架成功赋予语言模型文化自我意识，能够理解文化动态性并持续适应，在需要文化敏感性的任务中表现出色。
...
TL;DR: SESS：一种基于子模优化的评估子集选择方法，用于提升提示优化的效果


<details>
  <summary>Details</summary>
Motivation: 自动提示优化依赖小规模评估子集作为反馈信号，但现有方法通常将评估子集选择视为实现细节，缺乏理论指导

Method: 提出SESS方法，将评估子集选择形式化为最大化目标集函数，证明其在温和条件下具有单调性和子模性，从而可以使用贪心算法获得理论保证

Result: 在GSM8K、MATH和GPQA-Diamond数据集上，子模选择的评估子集比随机或启发式基线方法能产生更好的优化提示

Conclusion: 评估子集选择对提示优化至关重要，子模优化方法提供了一种有理论保证的有效选择策略
...
<details>
  <summary>Details</summary>
Motivation: 传统SFT训练依赖验证困惑度监控，但这种方法混淆了语言风格模仿与真实事实内化，无法准确评估模型是否真正掌握了领域知识。

Method: 提出KR-Test框架，使用自动生成的对比示例来测量模型对正确与错误续写的似然偏好，无需指令调优或生成解码，并通过"盲测vs.先知"基线分析验证框架完整性。

Result: KR-Test能够有效区分事实学习与语言风格模仿，在LoRA训练动态分析中展示了其诊断能力，揭示了语言收敛与知识保留之间的精细解离。

Conclusion: KR-Test增强了微调动态的可解释性，为监控SFT训练中的知识内化提供了更准确的评估工具，有助于优化大语言模型的领域知识注入过程。
...
TL;DR: RPAM是一种通过模型合并实现自适应推理的轻量级方法，无需重新训练或大量数据，能显著降低推理成本同时保持性能


<details>
  <summary>Details</summary>
Motivation: 大型推理模型通常为每个查询生成长推理路径，导致不必要的计算和延迟。现有加速方法需要重新训练模型或设计复杂提示，成本高昂且对输入敏感

Method: 提出推理模式对齐合并(RPAM)：通过合并长链式思维模型和短链式思维指令模型，构建自适应推理器。首先构建带模式标签的校准集，然后通过特征对齐优化层间合并系数，同时使用对比目标将中间表示推离非选择模型

Result: 在七个广泛使用的推理基准测试中，RPAM显著降低了推理成本，同时保持了强大的性能

Conclusion: RPAM提供了一种轻量级的模型合并方法，无需从头训练或大规模额外数据即可实现自适应推理，是现有加速方法的有效替代方案
...
TL;DR: IntroLM：一种让因果语言模型在预填充阶段预测自身输出质量的方法，无需外部分类器，通过引入token条件LoRA实现，在问答基准上达到90%的ROC AUC，优于DeBERTa分类器14%，在多模型路由系统中可降低延迟33%和大模型使用50%。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖外部分类器（如BERT模型）来预测LLM输出质量，但这些方法存在上下文窗口有限、表示能力受限和额外计算开销的问题。需要一种更高效、更准确的方法让LLM能够预测自身输出质量。

Method: 提出IntroLM方法：1）使用introspective tokens让因果语言模型在预填充阶段预测输出质量；2）引入token条件LoRA，仅对introspective token激活，使模型学习预测输出质量，同时保持原始骨干模型行为；3）避免使用外部评估器。

Result: 在问答基准测试中，IntroLM应用于Qwen3 8B模型，成功预测的ROC AUC达到90%，比DeBERTa分类器高出14%。集成到多模型路由系统后，在保持相同可靠性的情况下，延迟降低33%，大模型使用减少50%，实现了优越的成本性能权衡。

Conclusion: IntroLM提供了一种高效的方法让LLM能够预测自身输出质量，无需外部分类器，在预测准确性和系统效率方面均有显著提升，为LLM的实际部署提供了有效的质量预测解决方案。
...
TL;DR: Mem-Gallery是一个评估多模态大语言模型长期对话记忆能力的新基准，包含高质量多会话对话和系统评估框架，揭示当前模型在记忆保留、推理和管理方面的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有基准要么评估纯文本多会话记忆，要么评估局部多模态理解，无法评估多模态记忆如何在长期对话轨迹中被保留、组织和演化，因此需要专门的多模态长期对话记忆评估基准。

Method: 提出Mem-Gallery基准，包含基于视觉和文本信息的高质量多会话对话，具有长交互视野和丰富的多模态依赖关系。基于此数据集构建系统评估框架，从三个功能维度评估关键记忆能力：记忆提取与测试时适应、记忆推理、记忆知识管理。

Result: 对13个记忆系统进行广泛基准测试，发现几个关键发现：需要显式的多模态信息保留和记忆组织、记忆推理和知识管理方面存在持续局限性、当前模型存在效率瓶颈。

Conclusion: Mem-Gallery填补了多模态长期对话记忆评估的空白，揭示了当前MLLM代理在记忆能力方面的关键挑战，为未来研究提供了重要基准和方向。
...
TL;DR: 论文提出个性化音频语言模型（PALM）任务，创建首个基准测试PALM-Bench，发现现有方法在建模个性化知识和跨任务迁移方面仍有局限


<details>
  <summary>Details</summary>
Motivation: 现有大型音频语言模型（LALMs）主要处理通用任务（如语音内容总结），但缺乏对个性化上下文的理解能力，无法支持个性化问答（如总结特定朋友说的话）。人类决策依赖于个人背景，因此需要开发能识别个人概念并在个人上下文中推理的模型。

Method: 1. 形式化定义个性化音频语言模型（PALM）任务；2. 创建首个基准测试PALM-Bench，包含多说话者场景下的多个任务；3. 对代表性开源LALMs进行实验，评估无训练提示和监督微调策略的效果。

Result: 实验表明，现有的无训练提示和监督微调策略虽然能带来改进，但在建模个性化知识和跨任务鲁棒迁移方面仍然有限。现有方法无法充分捕捉个人概念和上下文。

Conclusion: 需要开发更先进的个性化音频语言模型方法，以更好地识别个人概念并在个人上下文中进行推理。PALM-Bench为方法进步提供了结构化评估框架，数据和代码将开源。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于感知的自行车友好性评估方法存在局限性：难以捕捉道路环境复杂性，且未能充分考虑用户主观感知的异质性。需要更全面、可解释的评估框架。

Method: 提出人物感知的视觉语言模型框架，包含三个创新：(1)基于理论的人物条件化，通过思维链推理生成人物特定解释；(2)多粒度监督微调，结合专家标注推理和用户评分；(3)AI驱动的数据增强，创建受控配对数据以隔离基础设施变量影响。

Result: 开发了全景图像众包系统，收集了427名骑行者的12,400个人物条件化评估。实验结果显示，该框架在自行车友好性评分预测方面具有竞争力，同时能够实现可解释的因素归因。

Conclusion: 该框架能够有效解决现有自行车友好性评估方法的局限性，不仅提供准确的评分预测，还能生成可解释的评估结果，有助于创建更安全、更舒适的骑行环境。
...
TL;DR: DeepSynth-Eval是一个用于客观评估LLM智能体信息整合能力的基准，利用高质量综述论文作为黄金标准，通过逆向工程研究请求和构建"Oracle Contexts"来隔离合成与检索噪声，实验表明从数百篇参考文献中整合信息仍具挑战性，而智能体规划-写作工作流显著优于单轮生成。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型向自主智能体发展推动了深度研究的进展，但检索后的合成阶段（智能体需要消化大量上下文并将碎片化证据整合为连贯的长篇报告）由于开放式写作的主观性而缺乏客观评估标准。

Method: 提出DeepSynth-Eval基准，利用高质量综述论文作为黄金标准，逆向工程研究请求并从其参考文献构建"Oracle Contexts"以隔离合成与检索噪声。采用细粒度评估协议：通用检查清单（用于事实覆盖）和约束检查清单（用于结构组织），将主观判断转化为可验证指标。

Result: 在96个任务上的实验表明，从数百篇参考文献中整合信息仍然是一个重大挑战。智能体规划-写作工作流显著优于单轮生成，有效减少幻觉并提高对复杂结构约束的遵循度。

Conclusion: DeepSynth-Eval为评估LLM智能体的信息整合能力提供了客观基准，揭示了当前模型在从大量参考文献中合成信息方面的局限性，并证明了智能体工作流在提高合成质量方面的有效性。
...
TL;DR: 研究发现大语言模型在多跳推理中并不总是遵循"跳对齐电路假设"，而是出现了"层序反转"现象，即后跳答案实体比桥接实体更早可解码，且跳数越多越明显。


<details>
  <summary>Details</summary>
Motivation: 尽管大语言模型在多跳推理上表现良好，但其内部如何组合多个事实仍不清楚。现有研究提出的"跳对齐电路假设"认为桥接实体在深层计算，但本文发现这一假设不能推广到真实世界的多跳查询。

Method: 提出"概率性回忆与提取"框架，将多跳推理建模为浅层MLP层的广泛概率性回忆，随后在深层注意力层进行选择性提取。通过系统性的探测分析、重新解释先前的层解码证据、解释思维链增益等方式进行实证验证。

Result: 发现了"层序反转"现象：后跳答案实体比桥接实体更早可解码，且总跳数越多，这种现象越明显。提出的框架能够解释思维链的增益，并为多跳失败提供机制性诊断。

Conclusion: 多跳推理不是简单的跳对齐计算，而是涉及浅层的广泛概率性回忆和深层的选择性提取过程。这一发现挑战了现有的"跳对齐电路假设"，为理解大语言模型内部推理机制提供了新视角。
...
TL;DR: EvolMem是一个评估LLM多会话记忆能力的新基准，基于认知心理学，涵盖陈述性和非陈述性记忆的多个维度，使用混合数据合成框架生成可控复杂度的多会话对话。


<details>
  <summary>Details</summary>
Motivation: 现有基准缺乏对LLM在多样化记忆维度上的系统评估，特别是在多会话设置中。需要一个新的基准来全面评估LLM和智能体系统的多会话记忆能力。

Method: 提出EvolMem基准，基于认知心理学框架，涵盖陈述性和非陈述性记忆的多个细粒度能力。采用混合数据合成框架，包括主题启动生成和叙事启发转换，可扩展地生成可控复杂度的多会话对话，并提供样本特定的评估指南。

Result: 评估显示没有LLM在所有记忆维度上始终优于其他模型。智能体记忆机制不一定能增强LLM的能力，并且经常表现出显著的效率限制。

Conclusion: EvolMem为评估LLM的多会话记忆能力提供了系统基准，揭示了当前模型的局限性，为未来研究提供了方向。数据和代码已开源。
...
TL;DR: 论文提出了一种评估大语言模型在隐私关注和亲社会动机冲突下数据共享决策中价值观-行为一致性的新方法，包括基于上下文的评估协议、多组结构方程建模和价值观-行为对齐率指标。


<details>
  <summary>Details</summary>
Motivation: 现有评估通常单独测量隐私相关态度或共享意图，难以确定模型的表达价值观是否像真实人类行为一样共同预测其下游数据共享行为。需要一种方法来评估在竞争态度下的价值观-行为一致性。

Method: 引入基于上下文的评估协议，在有限的历史记录会话中顺序管理隐私态度、亲社会性和数据共享接受度的标准化问卷。使用多组结构方程建模分析隐私关注和亲社会性对数据共享的关系。提出价值观-行为对齐率作为人类参考的方向一致性指标。

Result: 在多个大语言模型中观察到稳定但模型特定的隐私-PSA-AoDS模式，以及在价值观-行为对齐方面存在显著异质性。

Conclusion: 该方法能够有效评估LLMs在数据共享决策中的价值观-行为一致性，揭示了不同模型在隐私关注和亲社会动机冲突下的决策模式差异。
...
TL;DR: 提出PAS框架评估LLM在警务应用中的表现，发现商用LLM在警务任务上表现不佳，特别是基于事实的建议方面


<details>
  <summary>Details</summary>
Motivation: LLM在警务操作中的应用日益增长，但缺乏专门针对警务操作的评估框架。虽然LLM的回答可能不总是法律上错误，但未经核实地使用仍可能导致严重问题，如非法逮捕和不当证据收集。

Method: 提出PAS（Police Action Scenarios）框架，覆盖整个评估过程。从8000多份官方文档构建新的QA数据集，并通过统计分析建立关键指标，由警务专家验证。
...
TL;DR: DiffCoT：将思维链推理重构为迭代去噪过程的扩散式框架，通过滑动窗口机制实现中间步骤的统一生成和回顾性修正，提高推理鲁棒性和纠错能力


<details>
  <summary>Details</summary>
Motivation: 传统思维链推理存在暴露偏差和错误累积问题，早期错误会在自回归解码过程中不可逆地传播，影响多步数学问题求解效果

Method: 提出DiffCoT框架，将思维链推理重构为迭代去噪过程；在推理步骤层面集成扩散原理，采用滑动窗口机制；引入因果扩散噪声调度以保持因果一致性；保留token级自回归特性

Result: 在三个多步思维链推理基准测试和多种模型骨干上的实验表明，DiffCoT持续优于现有思维链偏好优化方法，显著提升了推理的鲁棒性和纠错能力

Conclusion: DiffCoT通过扩散式框架有效解决了传统思维链推理中的错误传播问题，为多步推理任务提供了更稳健的解决方案
...
TL;DR: 本文研究了LLM在持续预训练中概念学习与遗忘的动态过程，通过概念电路分析揭示了概念获取、遗忘、干扰和协同的机制。


<details>
  <summary>Details</summary>
Motivation: 人类通过概念理解世界，但LLM在持续预训练中如何获取、保留和遗忘概念仍不清楚。需要从计算层面理解概念学习的动态过程，为设计更可解释和鲁棒的概念感知训练策略提供基础。

Method: 研究LLM内部的概念电路（与特定概念相关的计算子图），结合图度量分析电路结构。分析概念获取与遗忘的行为动态，以及多个概念间的干扰和协同效应。

Result: 1) 概念电路能显著反映概念学习和遗忘；2) 概念电路呈现阶段性时间模式（早期增加→逐渐减少→稳定）；3) 学习增益大的概念后续遗忘更严重；4) 语义相似概念干扰更强；5) 概念知识可迁移性不同，有些能显著促进其他概念学习。

Conclusion: 研究提供了概念学习动态的电路级视角，为设计更可解释和鲁棒的概念感知LLM训练策略提供了理论基础。
...
TL;DR: 本文提出PsychEthicsBench基准，基于澳大利亚心理学和精神病学指南，评估LLM在心理健康领域的伦理知识和行为响应，发现拒绝率是伦理行为的差指标，领域特定微调可能降低伦理鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 当前LLM在心理健康应用中的评估主要依赖拒绝性安全信号，但这无法反映临床实践所需的细微行为。临床不恰当的拒绝可能被视为缺乏同理心并阻碍求助行为，需要更全面的伦理评估框架。

Method: 提出PsychEthicsBench基准，基于澳大利亚心理学和精神病学指南，包含多项选择和开放式任务，带有细粒度伦理标注。评估14个模型，分析拒绝率与伦理行为的关系。

Result: 拒绝率是伦理行为的差指标，安全触发与临床适当性存在显著差异。领域特定微调可能降低伦理鲁棒性，多个专业模型在伦理对齐上表现不如基础模型。

Conclusion: PsychEthicsBench为心理健康领域LLM的系统性、司法管辖区感知评估提供了基础，鼓励该领域更负责任的发展。需要超越拒绝中心的评估指标来确保临床适当性。
...
TL;DR: LLMs在代码切换（多语言混合）交互中难以正确推断用户期望的输出语言，经常出现语言错配问题，但通过少量数据微调可显著改善。


<details>
  <summary>Details</summary>
Motivation: 多语言用户在对话中自然地进行代码切换（混合使用不同语言），但当前LLMs在这种场景下无法正确推断用户期望的输出语言，即使对人类来说线索很明显。这暴露了LLMs在实际多语言交互中的对齐不足问题。

Method: 1. 提出OLA基准测试，专门评估LLMs在代码切换交互中的输出语言对齐能力，聚焦韩语-英语代码切换；2. 测试范围从简单的句内混合到指令内容不匹配；3. 扩展到中文和印尼语对验证泛化性；4. 测试Chain-of-Thought提示效果；5. 使用少量数据（约1K示例）进行代码切换感知的DPO微调。

Result: 1. 即使是前沿LLMs也经常误解隐含的语言期望，表现出偏向非英语回答的偏见；2. 模型表现出不稳定性，如中途切换语言和语言侵入；3. Chain-of-Thought提示无法解决这些错误，表明对输出语言的语用推理能力弱；4. 但使用少量数据的代码切换感知DPO微调能显著减少错配问题。

Conclusion: LLMs在代码切换交互中的语言对齐失败源于对齐不足而非根本限制，少量针对性微调即可显著改善。这凸显了需要将多语言LLMs与用户在真实世界代码切换交互中的隐含期望对齐的重要性。
...
TL;DR: 提出Self-Graph Reasoning (SGR)框架，让大语言模型在开放域问答中将推理过程显式表示为结构化图，提升推理一致性和准确性。


<details>
  <summary>Details</summary>
Motivation: 现有大语言模型的推理过程通常是线性的，容易产生逻辑不一致的问题。真实世界的推理需要整合多个前提并并行解决子问题。现有方法如CoT以线性文本形式表达推理，虽然看似连贯但经常导致不一致结论。现有方法依赖外部提供的图结构，未探索LLMs如何构建和使用自己的图结构推理。

Method: 提出Self-Graph Reasoning (SGR)框架，使LLMs在生成最终答案前将推理过程显式表示为结构化图。构建图结构推理数据集，将多个候选推理图合并为精炼的图结构用于模型训练。

Result: 在五个QA基准测试（包括通用和专门领域）上，SGR持续提升推理一致性，相比基础模型获得17.74%的性能提升。使用SGR微调的LLaMA-3.3-70B模型表现与GPT-4o相当，超过Claude-3.5-Haiku。

Conclusion: 图结构推理能有效提升大语言模型的推理一致性，SGR框架在开放域问答中展示了显著效果，证明了图结构推理的有效性。
...
Method: 提出Agentic Discriminative Verifier (DiVA)混合框架，结合生成模型的代理搜索能力和判别模型的精确评分能力，并构建FGVeriBench基准测试

Result: 在FGVeriBench上的实验结果表明，DiVA在一般问题和多跳问题的事实性验证上都显著优于现有方法

Conclusion: DiVA框架通过结合生成和判别模型的优势，有效解决了细粒度事实性验证的挑战，为LLM的事实性评估提供了更精细的工具
...
TL;DR: ALMs为音频深度伪造检测提供可解释性，但本文研究发现其推理过程在对抗攻击下存在脆弱性，推理并不总是增强鲁棒性，而是可能成为性能负担。


<details>
  <summary>Details</summary>
Motivation: 音频语言模型为音频深度伪造检测提供了可解释性，但现有研究主要关注最终预测结果的鲁棒性，缺乏对模型推理过程在对抗攻击下鲁棒性的分析。需要评估ALMs的推理过程在攻击下的稳定性。

Method: 提出法证审计框架，从三个相互关联的维度评估ALMs推理在对抗攻击下的鲁棒性：声学感知、认知一致性和认知失调。系统分析推理过程在不同攻击场景下的表现。

Result: 研究发现显式推理并不普遍增强鲁棒性，而是出现分化：对于声学感知鲁棒的模型，推理起到"盾牌"防御作用；对其他模型则成为性能"税负"。认知失调可作为"无声警报"检测潜在操纵。

Conclusion: 这项工作对推理在法证音频深度伪造分析中的作用及其脆弱性进行了关键评估，揭示了推理过程在对抗攻击下的复杂影响，为开发更鲁棒的ALMs提供了重要见解。
...
TL;DR: EPAG是一个评估LLMs预咨询能力的基准数据集和框架，通过HPI-诊断指南比较和疾病诊断来评估模型性能


<details>
  <summary>Details</summary>
Motivation: 为了评估LLMs在真实临床环境中的预咨询能力，需要专门的评估框架和数据集，以促进LLM在医疗应用中的发展

Method: 开发EPAG基准数据集和评估框架，通过直接比较HPI与诊断指南以及间接通过疾病诊断来评估LLMs，实验包括不同模型比较、HPI数量影响分析以及预咨询语言对对话特征的影响

Result: 1) 使用精心策划的任务特定数据集微调的小型开源模型在预咨询任务中优于前沿LLMs；2) HPI数量增加不一定提高诊断性能；3) 预咨询语言影响对话特征

Conclusion: EPAG为评估LLMs在临床预咨询能力提供了有价值的工具，开源数据集和评估流程有助于推动LLM在真实临床环境中的应用发展
...
<details>
  <summary>Details</summary>
Motivation: 系统比较推理大模型(LRMs)与非推理LLMs在判断任务上的表现差异，探究LRMs是否在判断能力上具有优势

Method: 通过实证分析比较LRMs和非推理LLMs，提出PlanJudge策略：让模型在执行评估前先生成明确的评估计划

Result: 1) LRMs在判断准确性上优于非推理LLMs，尤其在推理密集型任务中；2) LRMs在评估场景中指令遵循能力更强；3) LRMs对针对判断任务的对抗攻击更具鲁棒性；4) 但LRMs仍存在表面质量偏见；PlanJudge能显著缓解LRMs和标准LLMs的偏见

Conclusion: LRMs在多个维度上优于非推理LLMs作为判断者，但偏见问题仍需解决；PlanJudge是一种简单有效的偏见缓解策略
...
Method: 提出Agent-Dice参数融合框架，采用两阶段过程：1）几何共识过滤修剪冲突梯度；2）基于曲率的重要性加权放大共享语义。该方法基于方向共识评估，提供理论分析验证融合方案的有效性。
...
TL;DR: 提出LLM-MC-Affect概率框架，将情感建模为情感空间上的连续潜在概率分布，而非静态标签，通过随机LLM解码和蒙特卡洛估计近似这些分布，生成高保真情感轨迹，量化核心情感趋势和感知模糊性，用于分析人际耦合动态。


<details>
  <summary>Details</summary>
Motivation: 现有文本情感分析方法通常将情感视为个体说话者的确定性点估计，无法捕捉人际互动中固有的主观性、潜在模糊性和序列耦合性，而情感协调是人际互动的核心属性，影响实时关系意义的构建。

Method: 提出LLM-MC-Affect概率框架，将情感建模为情感空间上的连续潜在概率分布，利用随机LLM解码和蒙特卡洛估计近似这些分布，生成高保真情感轨迹，通过序列互相关和基于斜率的指标分析人际耦合，识别对话者之间的主导或滞后影响。

Result: 以师生教学对话为案例研究，定量指标成功提炼出高层次互动洞察（如有效支架教学），验证了该方法的解释能力，为理解人际动态建立了可扩展、可部署的途径。

Conclusion: 该工作为理解人际动态建立了可扩展、可部署的途径，提供了一个超越教育领域、适用于更广泛社会和心理学研究的通用解决方案，将情感从静态标签转变为连续概率分布，更好地捕捉人际互动中的主观性和模糊性。
...
TL;DR: 提出ELO方法，通过只训练关键的第一层和最后一层来加速多语言大模型的持续预训练，同时保持源语言能力


<details>
  <summary>Details</summary>
Motivation: 传统持续预训练方法计算成本高，且容易导致源语言性能下降，需要更高效的优化方法

Method: ELO方法包含两个阶段：1) ELO预训练：分离出关键的第一层和最后一层进行目标语言训练；2) 层对齐：将训练好的层重新整合并进行简短的全参数微调

Result: 训练速度提升最高达6.46倍，目标语言性能提升最高达6.2%，同时有效保持源语言（英语）能力

Conclusion: ELO方法为多语言大模型的持续预训练提供了一种高效且有效的解决方案，显著降低计算成本并保持模型性能
...
Method: 基于发现答案token对早期推理关注度弱而主要关注特殊token"/think"的现象，SyncThink监控模型自身的推理转换信号，在适当时机终止推理过程。

Result: 在GSM8K、MMLU、GPQA和BBH等数据集上，SyncThink使用656个生成token和28.68秒延迟达到62.00%平均Top-1准确率，相比完整CoT解码的61.22%准确率、2141个token和92.01秒延迟，显著提升了效率。在GPQA等长视野任务上，SyncThink还能通过防止过度思考带来高达+8.1的绝对准确率提升。

Conclusion: SyncThink是一种有效的训练免费解码方法，能显著减少CoT推理开销，在保持甚至提升准确率的同时大幅降低计算成本。
...
TL;DR: e5-omni提出轻量级显式对齐方法，解决多模态嵌入模型中相似度尺度不一致、负样本不平衡和统计特性不匹配的问题，显著提升跨模态检索性能。


<details>
  <summary>Details</summary>
Motivation: 现有全模态嵌入模型主要依赖预训练视觉语言模型的隐式对齐，导致三个问题：1) 相似度得分因模态而异，缺乏统一尺度；2) 混合模态批次中负样本难度分布不平衡，许多负样本快速变得无关紧要；3) 不同模态嵌入的统计特性不匹配，影响排序稳定性。

Method: 提出e5-omni方法，包含三个组件：1) 模态感知温度校准，对齐相似度尺度；2) 可控负样本课程学习与去偏，聚焦混淆负样本并减少假负样本影响；3) 批白化与协方差正则化，匹配跨模态嵌入空间的几何结构。

Result: 在MMEB-V2和AudioCaps基准测试中，e5-omni相比现有双模态和全模态基线方法取得一致性能提升，且该方法可良好迁移到其他视觉语言模型骨干网络。

Conclusion: e5-omni提供了一种轻量级显式对齐方案，能够有效解决多模态嵌入中的对齐问题，显著提升跨模态检索性能，并具有良好的可迁移性。
...
<details>
  <summary>Details</summary>
Motivation: 在生物医学等高风险领域，如何有效验证系统生成的响应是一个关键挑战。传统基于句子级别的证据对齐方法存在局限性，需要更精确的声明级别验证机制。

Method: 提出eTracer框架，采用后验对齐方法，将每个响应声明与上下文证据进行对齐，确定支持或矛盾关系。基于声明级别的对齐结果，实现响应到上下文源的精确追溯，并量化响应忠实度。

Result: 实验表明，声明级别的对齐方法相比传统句子级别方法，显著提高了整体对齐质量和用户验证效率，解决了传统方法在将生成声明与上下文句子级别证据对齐时的局限性。

Conclusion: eTracer框架通过声明级别的证据对齐，实现了可追溯的文本生成，增强了生成响应的可验证性和可信度，特别适用于生物医学等高风险领域。
...
TL;DR: DisastQA：针对灾害管理的QA基准，包含3000个验证问题，评估模型在不确定和冲突信息下的推理能力，揭示现有模型在真实噪声环境下性能显著下降


<details>
  <summary>Details</summary>
Motivation: 现有QA基准基于干净证据构建，无法有效评估灾害管理中面对不确定和冲突信息的推理能力，需要专门基准来测试模型在真实灾害场景下的可靠性

Method: 通过人机协作流水线构建包含8种灾害类型、3000个验证问题的基准，采用分层抽样确保平衡覆盖，在从闭卷到噪声证据集成的多种条件下评估模型，对开放式QA提出基于关键点的人类验证评估协议

Result: 评估20个模型显示与通用基准（如MMLU-Pro）存在显著差异，开源模型在干净环境下接近专有系统，但在真实噪声下性能急剧下降，暴露灾害响应的关键可靠性差距

Conclusion: DisastQA基准揭示了现有模型在灾害管理真实场景中的局限性，强调需要开发能够处理不确定和冲突信息的鲁棒推理系统，所有代码、数据和评估资源已开源
...
TL;DR: NeuronScope：多智能体框架，通过迭代激活引导过程解决LLM神经元多义性解释问题，比单次方法更准确
...
Method: NeuronScope多智能体框架，将神经元解释重构为迭代、激活引导的过程：将神经元激活分解为原子语义组件，聚类为不同语义模式，使用神经元激活反馈迭代优化每个解释

Result: 实验表明NeuronScope能够揭示隐藏的多义性，产生的解释与激活相关性显著高于单次基线方法

Conclusion: NeuronScope通过多智能体迭代方法有效解决了LLM神经元多义性解释的挑战，提高了神经元解释的准确性和可靠性
...
TL;DR: STEPS框架通过技能分类学和熵最大化合成组合性挑战数据，提升LLMs的组合泛化能力


<details>
  <summary>Details</summary>
Motivation: 大语言模型和基于代理的系统在组合泛化方面存在困难，因为复杂技能组合遵循长尾分布，形成了数据瓶颈，限制了指令跟随性能和代理任务的泛化能力

Method: 提出STEPS框架：1) 使用结构信息理论发现技能间的潜在关系，构建可解释的层次化技能分类学；2) 将数据合成建模为约束信息最大化问题，选择在层次结构中最大化边际结构信息同时保持语义连贯性的技能组合

Result: 在具有挑战性的指令跟随基准测试中，STEPS超越了现有的数据合成基线方法，同时在下游基于代理的评估中展现出改进的组合泛化能力

Conclusion: STEPS框架通过系统性地合成组合性挑战数据，有效解决了LLMs和代理系统的组合泛化瓶颈问题，为提升复杂任务处理能力提供了新途径
...
TL;DR: FSLR训练框架通过专注于第一步规划（识别变量和操作），显著提升LLM的逻辑关系理解能力，相比CoT-SFT在推理准确率、训练效率和资源消耗方面都有明显优势。


<details>
  <summary>Details</summary>
Motivation: 当前大型语言模型在数学问题解决中表现出有限的逻辑推理能力，主要依赖模式匹配和记忆。研究发现超过90%的错误预测源于逻辑关系理解不足，而现有的CoT-SFT方法无法有效解决这一问题。

Method: 提出First-Step Logical Reasoning（FSLR）轻量级训练框架，专注于训练模型进行第一步规划：从问题陈述中识别要使用的变量和要应用的操作。这种方法为逻辑关系理解提供显式监督，与CoT-SFT将逻辑关系隐式嵌入完整解决方案轨迹的方法形成对比。

Result: FSLR在多个模型和数据集上一致优于CoT-SFT，在分布内和分布外设置下平均分别提升3.2%和4.6%。同时训练速度提高4-6倍，训练token消耗减少超过80%。

Conclusion: FSLR通过专注于第一步逻辑规划，有效解决了LLM在逻辑关系理解上的瓶颈，提供了一种更高效、更有效的训练方法，显著提升了模型的逻辑推理能力。
...
<details>
  <summary>Details</summary>
Motivation: 现有的基于参考的指标无法捕捉创造力的主观性，因此需要一个新的评估框架来更好地评估AI故事生成的创造力。

Method: 使用"Spike Prompting"进行受控故事生成，并通过115名读者的众包研究，分析不同创造力组件如何影响人类对创造力的即时和反思性判断。

Result: 研究发现创造力评估是分层而非累积的，不同维度在不同判断阶段变得显著，反思性评估显著改变了评分和评分者间一致性。

Conclusion: 该框架有效揭示了被基于参考的评估所掩盖的创造力维度，为AI故事生成的创造力评估提供了更全面的方法。
...
<details>
  <summary>Details</summary>
Motivation: 随着大语言模型在安全关键应用中的普及，确保其对对抗性提示的鲁棒性变得至关重要。现有红队测试数据集存在风险分类不一致、领域覆盖有限和评估方法过时等问题，阻碍了系统性的脆弱性评估。

Method: 引入RedBench数据集，整合了来自顶级会议和仓库的37个基准数据集，包含29,362个攻击性和拒绝性提示样本。采用标准化分类法，包含22个风险类别和19个领域，确保评估的一致性和全面性。

Result: 建立了现代大语言模型的基准评估，开源了数据集和评估代码，为脆弱性评估提供了系统化的工具和框架。

Conclusion: RedBench促进了鲁棒性比较，支持未来研究，推动了面向实际部署的安全可靠大语言模型的发展。
...
TL;DR: ADEPT提出自适应动态提前退出机制，可在预填充和生成阶段实现token级别的动态提前退出，通过解耦跳过层的KV缓存依赖来提升效率。


<details>
  <summary>Details</summary>
Motivation: 现有提前退出策略要么只适用于生成阶段的第一个token，要么只在预填充阶段有效，跳过的层产生的KV缓存仍然是后续token生成的瓶颈，限制了提前退出的收益。

Method: 提出ADEPT方法：1）自适应token级别提前退出机制，根据token复杂度动态调整计算；2）通过解耦跳过层的序列依赖来优化KV生成过程，使token级别提前退出更实用。

Result: 实验表明ADEPT在语言生成任务中提升效率达25%，在下游分类任务中实现4倍加速，性能提升达45%。

Conclusion: ADEPT通过自适应动态提前退出机制有效解决了KV缓存瓶颈问题，在预填充和生成阶段都能实现高效的token级别提前退出，显著提升大语言模型推理效率。
...
<details>
  <summary>Details</summary>
Motivation: 现有无人机视觉语言导航数据集存在依赖虚拟环境、指令不自然、规模有限等问题，需要构建基于真实环境、指令自然多样的大规模基准。

Method: 1) 从真实城市航拍数据构建AirNav基准；2) 提出AirVLN-R1模型，结合监督微调和强化微调；3) 通过真实世界测试进行初步评估。

Result: 构建了基于真实环境的大规模无人机VLN基准AirNav，开发了AirVLN-R1模型，并通过真实世界测试验证了可行性。

Conclusion: AirNav解决了现有数据集的问题，AirVLN-R1模型展示了良好性能，数据集和代码已公开，为无人机VLN研究提供了重要资源。
...
TL;DR: 研究发现DeepSeek-OCR的性能主要依赖语言先验而非视觉能力，移除语言支持后准确率从90%骤降至20%，传统OCR方法比端到端方法更鲁棒，视觉token越少越依赖先验，且在约10,000文本token时完全崩溃。


<details>
  <summary>Details</summary>
Motivation: DeepSeek-OCR声称能实现高比例视觉-文本压缩，可能解决LLM长上下文瓶颈，但需要探究其性能究竟来自视觉能力还是语言先验，以明确其能力边界和优化方向。

Method: 通过句子级和单词级语义破坏实验，分离模型的固有OCR能力与语言先验；比较13个基线模型；分析视觉token数量与先验依赖关系；进行上下文压力测试。

Result: 移除语言支持后性能从约90%降至20%；传统管道OCR方法比端到端方法对语义扰动更鲁棒；视觉token越少越依赖先验，增加幻觉风险；在约10,000文本token时完全崩溃。

Conclusion: DeepSeek-OCR性能主要依赖语言先验而非视觉能力，当前光学压缩技术可能加剧长上下文瓶颈，研究为视觉-文本压缩范式的未来优化提供了重要见解。
...
TL;DR: MIND框架通过能力自适应的知识蒸馏，使用"教学助理"网络合成多样教师视角，通过反馈驱动的惯性校准机制，实现从被动模仿到主动认知构建的转变，提升小模型在领域内和跨领域的推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有方法通常限制学生模型遵循单一黄金推理路径，但由于师生模型不同的归纳偏好和内在偏好，加上学生训练过程中的能力演变，教师的"最优"推理路径可能成为分布外噪声，导致学生潜在推理分布退化，性能次优。

Method: 提出MIND框架：1) 通过"教学助理"网络合成多样教师视角；2) 采用反馈驱动的惯性校准机制，利用惯性过滤的训练损失使监督与学生当前适应能力对齐；3) 实现从被动模仿到主动认知构建的蒸馏过程。

Result: 在领域内和分布外基准测试中达到最先进性能，复杂的潜在空间分析进一步证实了推理能力内化的机制。

Conclusion: MIND框架通过能力自适应蒸馏，有效解决了小模型在保持领域性能同时实现跨领域泛化的挑战，证明了从被动模仿转向主动认知构建的有效性。
...
TL;DR: 提出一个数据增强框架，通过生成合成口吃音频来提升印尼语口吃语音识别性能，使用合成数据微调预训练模型，在口吃语音上显著降低识别错误率。


<details>
  <summary>Details</summary>
Motivation: 当前语音识别系统在处理口吃语音时性能显著下降，特别是在低资源语言如印尼语中，由于缺乏专门的数据集，这个问题尤为严重。

Method: 提出数据增强框架：1) 通过基于规则的转换和大型语言模型向流畅文本注入重复和延长等口吃特征；2) 使用文本转语音合成生成合成口吃音频；3) 用合成数据通过迁移学习微调预训练的印尼语Whisper模型。

Result: 实验表明，这种针对性的合成数据暴露能持续降低口吃语音的识别错误率，同时在流畅语音段上保持性能，验证了合成数据管道在开发更具包容性语音技术中的实用性。

Conclusion: 合成数据管道可以有效解决低资源语言中口吃语音识别的数据稀缺问题，为开发更具包容性的语音技术提供了可行方案，特别是在资源匮乏的语言环境中。
...
<details>
  <summary>Details</summary>
Motivation: 开源与闭源大模型之间的性能差距主要源于高质量训练数据的获取差异。为了弥合这一差距，需要开发不依赖专有数据或模型的解决方案。

Method: 1. 多智能体工作流：协作AI智能体模拟复杂的工具集成推理，端到端生成多样化、高保真的指令数据；2. 两阶段训练策略：结合监督微调和新颖的强化学习方法，最大化模型对齐和能力。

Result: 框架赋能多个规模的开源模型，在主要深度研究基准上实现了新的最先进性能，为开源LLM发展提供了可扩展且有效的途径。

Conclusion: 该工作提供了一种不依赖专有数据或模型的可扩展路径，能够显著提升开源大模型的性能，缩小与闭源模型的差距。
...
TL;DR: 研究大语言模型在检索增强生成中如何处理知识冲突，发现模型偏好机构来源信息，但重复效应会逆转这种偏好，并提出降低重复偏差的方法。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型在检索增强生成管道中的使用日益频繁，研究其在知识冲突下的行为变得愈发重要。目前，检索信息来源的角色尚未得到充分研究，本文旨在填补这一空白，特别关注来源偏好如何影响LLM解决跨上下文知识冲突。

Method: 提出了一个新颖框架来研究来源偏好如何影响LLM解决英语中的跨上下文知识冲突。对13个开源权重LLM进行了全面、严格控制的评估，并设计了一种新方法来减轻重复效应，通过减少重复偏差来维持一致的来源偏好。

Result: 研究发现LLM偏好机构验证的信息（如政府或报纸来源），而非个人和社交媒体信息。然而，这种来源偏好可以通过简单重复来自较低可信度来源的信息而被逆转。提出的方法能将重复偏差降低高达99.8%，同时保持至少88.8%的原始偏好。

Conclusion: LLM在知识冲突解决中存在明显的来源偏好，但容易受到重复效应的影响。提出的方法能有效减轻重复偏差，维持一致的来源偏好。研究强调了在知识密集型NLP中考虑可信度和来源偏好的重要性，并开源了所有数据和代码以促进未来研究。
...
TL;DR: 该研究探讨了大型语言模型在多语言环境中个性化文本生成的能力及其对可检测性的影响，发现不同语言在针对人口群体和社交媒体平台的个性化质量存在差异，其中英语的个性化质量最高且对可检测性影响最大。


<details>
  <summary>Details</summary>
Motivation: 随着大型语言模型生成多语言连贯文本能力的增强，人们对其潜在滥用的担忧也在增加。先前研究表明这些模型可能被滥用于生成多语言的个性化虚假信息，且个性化会降低机器生成文本的可检测性，但这仅在英语中得到验证。本研究旨在跨10种语言探索这一现象，不仅关注个性化能力的潜在滥用风险，也关注其潜在益处。

Method: 研究覆盖了1080种不同个性化方面的提示组合，使用16个不同的语言模型生成文本（总计17,280个文本）。通过系统分析这些多语言文本，评估个性化质量及其对机器生成文本可检测性的影响。

Result: 研究结果显示：1）针对人口群体和社交媒体平台的个性化文本生成质量在不同语言中存在差异；2）平台个性化对生成文本可检测性的影响更大；3）英语的个性化质量最高，且对可检测性的影响最为显著。

Conclusion: 个性化能力在不同语言中表现出不同的特征，平台个性化对文本可检测性的影响尤为显著。这一发现强调了在多语言环境中评估语言模型潜在风险和益处的重要性，特别是在虚假信息检测和内容审核方面。
...
TL;DR: 研究探讨LLM生成的自我解释是否能帮助用户预测模型行为，通过反事实可模拟性测试发现，自我解释能提升预测准确性，但效果受扰动策略和评估者能力影响。


<details>
  <summary>Details</summary>
Motivation: 尽管LLM能生成自我解释，但先前研究表明这些解释可能不能可靠反映模型的真实决策过程。本研究旨在探究这些解释是否能帮助用户预测模型行为，特别是通过反事实可模拟性这一操作化指标。

Method: 使用StrategyQA数据集，评估人类和LLM评估者在有无模型链式思考或事后解释的情况下，预测模型对反事实后续问题答案的能力。比较LLM生成的反事实与基于语用学的扰动作为评估解释有用性的替代测试方法。

Result: 自我解释能持续提升LLM评估者和人类的模拟准确性，但提升程度和稳定性强烈依赖于扰动策略和评估者能力。定性分析显示，访问解释能帮助人类在扰动问题上形成更准确的预测。

Conclusion: LLM生成的自我解释虽然可能不完全反映模型内部决策过程，但在帮助用户预测模型行为方面具有实际价值，特别是在反事实场景下。解释的有用性受测试设计和评估者因素的影响。
...
TL;DR: LLM表征的内在维度（ID）可作为语言复杂度的标记，能区分不同类型复杂度，并与语言处理阶段相关


<details>
  <summary>Details</summary>
Motivation: 探索LLM表征的内在维度是否能够作为语言复杂度的标记，并研究不同层级的ID特征如何区分形式复杂度和功能复杂度

Method: 通过分析LLM表征的内在维度，比较不同句子结构（并列/从属从句、右分支/中心嵌入、明确/模糊关系从句附着）的ID差异，使用表征相似性和层消融实验验证

Result: 形式复杂度（多从句结构）在ID差异中明显体现，且与先前研究中确定的抽象语言处理阶段对齐；功能复杂度（右分支vs中心嵌入、明确vs模糊附着）也能被ID检测到，但程度较弱且不与相同处理阶段相关

Conclusion: ID是LLM中语言复杂度的有用标记，能够区分不同类型复杂度，并指向不同LLM中相似的语言处理阶段
...
TL;DR: 本文首次系统研究音频大语言模型（ALLMs）的隐私泄露风险，构建了包含2.2万条真实音频的HearSay基准测试，发现ALLMs能通过声纹准确推断用户隐私属性（性别准确率达92.89%），现有安全机制严重不足，思维链推理会进一步放大隐私风险。
...
Method: 构建了HearSay基准测试，包含超过22,000条真实世界音频片段。通过自动化分析和人工验证的严格流程确保数据质量，所有隐私标签都基于事实记录。在该基准上进行了广泛的实验评估。

Result: 1. 显著的隐私泄露：ALLMs能够从声纹中提取隐私属性，性别识别准确率达到92.89%，并能有效分析社会属性。2. 安全机制不足：现有安全措施严重不足，大多数模型无法拒绝隐私侵犯请求，生理特征相关的拒绝率接近零。3. 推理放大风险：思维链推理在能力较强的模型中会加剧隐私风险，揭示更深层的声学相关性。

Conclusion: 研究揭示了ALLMs的关键隐私漏洞，强调了针对性隐私对齐的紧迫需求。代码和数据集已开源，为后续隐私保护研究提供基础。
...
TL;DR: Membox是一个分层记忆架构，通过Topic Loom将连续同主题对话轮次分组为"记忆盒"，再通过Trace Weaver连接成长期事件时间线，显著提升LLM代理的时序推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有LLM代理记忆系统遵循碎片化-补偿范式：先将对话流分解为孤立语句存储，再通过嵌入检索恢复连贯性。这种方法不可逆地破坏了叙述和因果流程，并使检索偏向词汇相似性，无法保持对话中的主题连续性。

Method: 提出membox分层记忆架构：1) Topic Loom：以滑动窗口方式持续监控对话，将连续同主题轮次分组为连贯的"记忆盒"进行存储；2) Trace Weaver：将密封的记忆盒连接成长期事件时间线轨迹，恢复跨不连续性的宏观主题重现。

Result: 在LoCoMo数据集上的实验表明，Membox在时序推理任务上实现了高达68%的F1提升，优于Mem0、A-MEM等基线方法。同时，Membox仅使用现有方法所需上下文标记的一小部分，在效率和效果之间取得了优越平衡。

Conclusion: 通过显式建模主题连续性，Membox提供了一种认知动机机制，能够同时增强LLM代理的连贯性和效率，为对话记忆系统提供了更符合人类认知的设计思路。
...
<details>
  <summary>Details</summary>
Motivation: 训练数据影响估计方法量化训练文档对模型输出的贡献，可作为基于实例的解释来源。但由于人类无法解释数千文档，只能展示少量训练数据作为解释。选择哪些文档直接影响解释质量，而先前评估忽略了选择策略的重要性。

Method: 提出新颖的选择相关性评分，这是一个无需重新训练的指标，量化一组示例对解释模型输出的有用程度。通过微调实验验证该评分，并基于此提出平衡影响力和代表性的选择策略。

Result: 验证了选择相关性评分能够预测一组示例是支持还是削弱模型预测。发现常见选择策略常不如随机选择。提出的平衡策略能比简单选择最高排名示例更好地利用选择预算。

Conclusion: 选择策略对训练数据影响估计的解释质量至关重要，提出的选择相关性评分和平衡策略能显著改善基于实例的解释效果。
...
<details>
  <summary>Details</summary>
Motivation: 新词感知机器翻译领域相对通用机器翻译研究不足，需要专门方法处理源语言中新词在目标语言的翻译问题

Method: 1) 创建16种语言、75个翻译方向的新词翻译数据集；2) 基于Wiktionary构建搜索工具；3) 使用强化学习训练翻译代理，包含新颖的奖励设计和基于"翻译难度"的自适应生成方法

Result: 构建了包含约1000万条英文Wiktionary记录的数据集和300万条清理记录的检索语料库，开发了完整的NeoAMT框架

Conclusion: 提出的NeoAMT框架通过Wiktionary搜索工具和强化学习训练，有效提升了新词感知机器翻译的质量和准确性
...
TL;DR: 本文提出了一种新的LLM记忆评估框架CRM，通过控制提示线索来更准确地评估PII泄露是否真正源于记忆而非模式补全，并在多语言实验中证明先前报告的PII泄露主要是线索驱动行为而非真实记忆。


<details>
  <summary>Details</summary>
Motivation: 现有研究通常将LLM成功重建个人身份信息(PII)解释为记忆证据，但作者认为这可能是由提示诱导的泛化或模式补全导致的，而非真正的记忆。需要更严谨的评估方法来区分线索驱动行为和真实记忆。

Method: 提出线索抵抗记忆(CRM)框架，明确控制提示-目标重叠线索，作为有效记忆评估的必要条件。在32种语言上进行大规模多语言重新评估，包括逐字前缀-后缀补全、关联重建、无线索生成和成员推断等多种记忆范式。

Result: 当控制表面形式线索时，重建成功率显著下降；无线索生成和成员推断的真阳性率极低；先前报告的PII泄露主要源于直接表面形式线索而非真实记忆。

Conclusion: 先前报告的PII泄露应解释为线索驱动行为而非真实记忆，强调线索控制评估对于可靠量化LLM中隐私相关记忆的重要性。CRM框架为更准确的记忆评估提供了原则性方法。
...
TL;DR: 本文介绍了VietMed-MCQ数据集，这是一个针对越南传统医学(VTM)的多选题数据集，通过RAG管道生成并采用双重模型验证机制，包含3,190个问题，用于评估LLM在低资源医学领域的表现。
...
Method: 采用检索增强生成(RAG)管道创建多选题数据集，并引入双重模型验证机制确保推理一致性，通过独立答案验证进行质量检查，数据集包含三个难度级别，由一名医学专家和四名学生进行人工验证。

Result: 数据集获得94.2%的专家批准率，评分者间一致性高(Fleiss' kappa = 0.82)。对7个开源模型的测试显示，具有中文先验知识的通用模型优于越南中心模型，表明跨语言概念迁移的存在，但所有模型在复杂诊断推理方面仍有困难。

Conclusion: VietMed-MCQ数据集填补了越南传统医学领域基准测试的空白，揭示了跨语言知识迁移的潜力，为低资源医学领域的研究提供了有价值的资源，尽管模型在复杂推理方面仍需改进。
...
TL;DR: 研究发现Transformer语言模型中意义的心理语言学特征编码位置受方法选择影响，不同提取方法显示不同的层间分布模式，但模型间存在一致的深度排序规律。


<details>
  <summary>Details</summary>
Motivation: 理解Transformer语言模型在何处编码具有心理学意义的内容特征，对于理论和实践都至关重要。需要系统研究不同模型架构和嵌入提取方法对心理语言学特征编码位置的影响。

Method: 对10个Transformer模型（包括编码器和解码器架构）进行分层探测研究，涵盖58个心理语言学特征，比较三种嵌入提取方法（上下文嵌入与孤立嵌入）。

Result: 意义定位强烈依赖于方法：上下文嵌入比孤立嵌入具有更高的特征选择性和不同的层间分布模式。跨模型和方法，最后一层表示很少是线性探测恢复心理语言学信息的最佳选择。模型间存在共享的深度排序规律：词汇属性较早达到峰值，经验和情感维度较晚达到峰值。

Conclusion: Transformer模型中"意义"的编码位置反映了方法选择与架构约束之间的相互作用，研究结果强调了在解释模型表示时考虑方法学因素的重要性。
...
TL;DR: 该论文评估了AI文本检测方法，发现基于上下文的深度学习模型（如DistilBERT）在检测AI生成文本方面优于传统的基于词汇特征的方法，并强调了通过主题分割防止信息泄漏的重要性。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型的快速发展，学生越来越多地将AI生成内容作为自己的作业提交，这违反了学术诚信。因此需要评估有效的AI文本检测方法来应对这一问题。

Method: 使用HC3和DAIGT v2两个数据集构建统一基准，采用基于主题的数据分割防止信息泄漏。评估了传统机器学习模型（TF-IDF逻辑回归）和深度学习模型（BiLSTM、DistilBERT）的性能。

Result: TF-IDF逻辑回归达到82.87%的准确率；BiLSTM达到88.86%的准确率；DistilBERT达到88.11%的准确率，但ROC-AUC得分最高（0.96），表现出最强的整体性能。结果表明上下文语义建模显著优于词汇特征。

Conclusion: 深度学习模型在AI文本检测方面优于传统方法，适当的评估协议（如主题分割）对防止信息泄漏至关重要。未来工作将扩展数据集多样性，使用参数高效微调方法（如LoRA），并探索更小或蒸馏模型。
...
<details>
  <summary>Details</summary>
Motivation: 现有RLVR方法缺乏语义基础的步骤级推理进度度量，导致LLMs无法区分必要推理和冗余验证，可能继续检查正确解或将正确轨迹转为错误答案。

Method: 提出无训练探测机制提取中间置信度和正确性，组合成步骤潜力信号；基于此提出步骤潜力优势估计(SPAE)，放大潜力增益、惩罚潜力下降，并在潜力饱和后应用惩罚以鼓励及时终止。

Result: 在多个基准测试中，SPAE持续提高准确性同时显著减少响应长度，优于强RL基线和近期高效推理及令牌级优势估计方法。

Conclusion: SPAE通过细粒度过程监督解决了RLVR中的推理进度度量问题，实现了更准确、更高效的推理，代码已开源。
...
### [155] [InfiniteWeb: Scalable Web Environment Synthesis for GUI Agent Training](https://arxiv.org/abs/2601.04126)
*Ziyun Zhang,Zezhou Wang,Xiaoyi Zhang,Zongyu Guo,Jiahao Li,Bin Li,Yan Lu*

Main category: cs.CL

TL;DR: InfiniteWeb系统自动生成大规模功能性网页环境用于GUI代理训练，通过统一规范、任务驱动开发和多样化设计解决网页生成挑战，并生成可验证的任务评估器提供强化学习密集奖励信号。


<details>
  <summary>Details</summary>
Motivation: GUI代理作为实用AI助手具有前景，但训练这类代理受到合适环境稀缺的限制。现有方法在生成单个网页上表现良好，但构建具有多个互连页面的现实功能性网站面临挑战。

Method: 采用统一规范、任务中心测试驱动开发、结合网站种子和参考设计图像确保多样性。系统还生成可验证的任务评估器，为强化学习提供密集奖励信号。

Result: InfiniteWeb在现实网站构建方面超越商业编码代理，在InfiniteWeb生成环境中训练的GUI代理在OSWorld和Online-Mind2Web基准上取得显著性能提升。
...
<details>
  <summary>Details</summary>
Motivation: 现有表格剪枝方法依赖顺序修订，基于不可靠的批判信号，容易丢失关键答案数据，需要更可靠的剪枝方法

Method: 提出TabTrim框架：1) 从黄金SQL查询执行过程中提取中间子表构建黄金剪枝轨迹；2) 训练剪枝器和验证器使逐步剪枝结果对齐黄金轨迹；3) 推理时进行并行搜索探索多个候选轨迹并选择最优子表

Result: TabTrim-8B达到73.5%平均准确率，比最强基线提升3.2%，在WikiTQ上79.4%，TableBench上61.2%，实现SOTA性能

Conclusion: TabTrim通过黄金轨迹监督和并行搜索有效解决了现有表格剪枝方法的局限性，显著提升了表格问答性能
...
TL;DR: 该研究发现持续预训练（CPT）中的损失优化与知识学习过程存在系统性的不一致，损失单调下降而事实学习不稳定且非单调，知识获取窗口窄且容易遗忘。


<details>
  <summary>Details</summary>
Motivation: 当前持续预训练实践将损失作为知识学习的代理指标，但缺乏对训练过程中知识如何变化的深入理解。研究者希望将CPT视为知识学习过程而非单纯的优化问题进行研究。

Method: 构建了受控的、分布匹配的事实文档基准，并在CPT循环中直接插入诊断探针，实现epoch级别的知识获取动态测量；分析CPT如何重塑训练过程中的知识电路。

Result: 在三个指令调优LLM和多种CPT策略中，优化和学习系统性地分离：损失单调下降，而事实学习不稳定且非单调；获取的事实很少被巩固，学习强烈依赖于先前暴露，OOD性能从早期epoch就开始退化；电路分析显示知识通路在epoch间快速重新配置。

Conclusion: 损失优化与CPT中的学习进展不一致，需要基于任务级学习动态的停止标准评估，而不是单纯依赖损失指标。
...
TL;DR: PartisanLens：首个多语言超党派新闻标题数据集，涵盖西班牙语、意大利语和葡萄牙语，标注了超党派叙事和人口替代阴谋论，用于评估LLM的分类性能和自动标注能力。


<details>
  <summary>Details</summary>
Motivation: 超党派叙事和人口替代阴谋论（PRCT）是严重的社会威胁，前者加剧政治两极分化和制度不信任，后者直接引发现实极端暴力。现有资源稀缺、英语中心化，且往往孤立分析而非作为政治话语的相互关联方面。

Method: 构建PartisanLens多语言数据集（1617个超党派新闻标题，涵盖西班牙语、意大利语和葡萄牙语），进行多维度政治话语标注。评估主流LLM的分类性能，探索LLM作为自动标注器的可行性，并研究LLM能否通过模拟社会经济和意识形态视角来模仿人类标注模式。

Result: 为超党派和PRCT叙事分类建立了稳健的基线性能，展示了LLM在这项任务中的潜力和当前局限性。验证了LLM作为自动标注器的可行性，并探索了通过条件化社会经济学和意识形态特征来模拟人类标注模式的可能性。

Conclusion: PartisanLens填补了多语言政治话语分析资源的空白，为欧洲语境下的党派和阴谋论叙事检测研究提供了重要支持，同时揭示了LLM在这类复杂任务中的应用前景和限制。
...
TL;DR: 该论文对LLMs和LRMs的安全对齐能力进行了大规模实证研究，评估了6个内在模型特性和3种外部攻击技术的影响，发现推理和自我反思机制对安全对齐有显著优势，同时揭示了CoT攻击通过响应前缀带来的严重安全风险。


<details>
  <summary>Details</summary>
Motivation: 评估LLMs和LRMs的安全对齐能力，为开发更安全可靠的AI系统提供关键见解，系统研究内在模型特性和外部攻击技术对安全对齐的影响。

Method: 使用32个最近流行的LLMs和LRMs，涵盖13个模型家族（参数规模3B-235B），利用5个安全数据集，采用56种越狱技术和4种CoT攻击策略，进行了460万次API调用的大规模评估。

Result: 1) GPT-OSS-20B、Qwen3-Next-80B-A3B-Thinking和GPT-OSS-120B是最安全的三个模型；2) 后训练和知识蒸馏可能导致安全对齐系统性退化；3) 通过响应前缀的CoT攻击可将攻击成功率平均提升3.34倍；4) 角色扮演、提示注入和基于梯度的对抗提示搜索是主要的攻击方法。

Conclusion: 安全应作为后训练和知识蒸馏阶段的明确约束或核心优化目标，而非从属于一般能力追求；文本补全接口和允许用户定义响应前缀的功能存在严重安全风险，需要架构和部署层面的保障措施。
...
<details>
  <summary>Details</summary>
Motivation: 随着大型语言模型和外部工具多样性的增加，选择最优的模型-工具组合成为一个高维优化挑战。现有方法通常依赖单一模型或固定工具调用逻辑，无法充分利用异构模型-工具对的性能差异。

Method: ATLAS采用双路径方法：1) 无训练聚类路由，利用经验先验进行领域特定对齐；2) 基于强化学习的多步路由，探索自主轨迹以实现分布外泛化。

Result: 在15个基准测试中，ATLAS超越了GPT-4o等闭源模型，在分布内任务上提升10.1%，在分布外任务上提升13.1%。在视觉推理任务中，通过编排专业的多模态工具也取得了显著增益。

Conclusion: ATLAS框架通过动态工具使用和自适应路由机制，有效解决了异构模型-工具组合的优化问题，在跨领域复杂推理任务中展现出优越性能。
...
TL;DR: 小型语言模型在语法纠错和文本简化任务上效率高但性能不如大型语言模型，存在意义保留和幻觉问题


<details>
  <summary>Details</summary>
Motivation: 大型语言模型虽然性能强大但计算成本高、部署困难，需要探索小型语言模型是否能成为语法纠错和文本简化任务的高效替代方案

Method: 在JFLEG和ASSET数据集上测试小型解码器语言模型，包括开箱即用、微调和顺序运行三种方式，使用已建立的评估指标

Result: 小型语言模型能学习某些行为，但性能低于强基线模型和当前的大型语言模型，在意义保留和避免幻觉方面存在困难
...
TL;DR: DTR是一个无需训练的框架，通过生成不确定性自适应决定何时检索，并使用双路径检索机制选择外部信息，在减少不必要检索的同时提升开放域QA性能。


<details>
  <summary>Details</summary>
Motivation: 现有检索增强生成方法存在两个问题：1) 不加区分地触发检索，引入噪声；2) 依赖单一路径的证据构建，限制了性能提升。需要更智能的检索决策机制。

Method: 提出DTR框架：1) 利用生成不确定性指导检索触发时机；2) 引入双路径检索机制，自适应选择信息，更好处理稀疏和模糊查询；3) 无需训练即可部署。

Result: 在五个开放域QA基准测试、多种模型规模和不同检索器上，DTR在EM和F1指标上持续优于标准RAG和强基线方法，同时减少了不必要的检索。

Conclusion: DTR通过自适应检索决策和双路径信息选择，有效解决了传统RAG的噪声问题和性能限制，为检索增强生成提供了更高效的解决方案。
...
TL;DR: 语言模型在多项选择题回答中采用两阶段机制：首先在内容空间选择正确答案，然后将该答案绑定到输出符号


<details>
  <summary>Details</summary>
Motivation: 多项选择题回答（MCQA）容易评估，但引入了元任务：模型既要解决问题，又要输出代表答案的符号，这混淆了推理错误和符号绑定失败。研究语言模型如何在内部实现MCQA，以理解其工作机制。

Method: 使用表征分析（PCA、线性探针）和因果干预研究语言模型内部实现。分析选项边界（换行）残差状态，使用胜者身份探针揭示两阶段进展。

Result: 发现选项边界残差状态包含与每个选项正确性相关的强线性可解码信号。胜者身份探针显示两阶段进展：获胜内容位置在最后一个选项处理后立即可解码，而输出符号在接近答案发射位置时才被表示。符号和内容排列测试支持两阶段机制。

Conclusion: 语言模型在MCQA中采用两阶段机制：首先在内容空间选择胜者，然后将该胜者绑定或路由到适当的符号进行输出。这解释了推理和符号绑定的分离过程。
...
TL;DR: 本文针对大型视觉语言模型在文档问答中的安全漏洞，提出了Doc-PP基准测试和DVA框架来解决推理引发的敏感信息泄露问题。


<details>
  <summary>Details</summary>
Motivation: 现实世界中文档问答部署常受动态用户定义的信息披露政策约束，现有安全研究主要关注隐式社会规范或纯文本场景，忽视了多模态文档的复杂性，导致模型在复杂推理时容易泄露敏感信息。

Method: 1) 构建Doc-PP基准测试，基于真实报告要求跨异质视觉和文本元素进行推理；2) 提出DVA（分解-验证-聚合）结构推理框架，将推理过程与政策验证解耦。

Result: 评估揭示了系统性"推理引发的安全漏洞"：模型在需要通过复杂合成或多模态聚合进行推理时频繁泄露敏感信息；提供提取文本虽改善感知但无意中促进泄露；DVA框架显著优于标准提示防御方法。
...
TL;DR: Arctic-ABSA是一系列强大的方面级情感分析模型，针对商业需求优化，在扩展的情感类别、多语言支持和推理能力方面表现出色，性能超越GPT-4o和Claude 3.5 Sonnet。


<details>
  <summary>Details</summary>
Motivation: 针对商业应用需求，开发更强大的方面级情感分析模型，解决传统模型情感类别有限、多语言支持不足、推理能力弱的问题。

Method: 使用大规模公开数据和合成数据训练（数据集比SemEval14大20倍），扩展情感类别到5类（增加混合和未知），联合预测整体文本情感，引入推理注入技术（CoT微调）和新型推理预训练方法，支持多语言。

Result: 395M参数编码器和8B参数解码器在SemEval14基准上达到新SOTA，比GPT-4o和Claude 3.5 Sonnet准确率高10个百分点；单多语言模型在6种语言上保持87-91%准确率且不影响英语性能；发布了包含17个公共数据集的大规模基准ABSAmix。

Conclusion: Arctic-ABSA模型在方面级情感分析任务上表现出卓越性能，特别是在扩展情感分类、多语言支持和推理能力方面，为商业应用提供了强大的解决方案。
...
TL;DR: RADAR：基于检索增强和对抗精化的检测器，通过生成器改写真实文章并引入语言对抗反馈，提升虚假新闻检测的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 为了有效对抗LLM生成的虚假信息传播，需要开发能够检测经过事实性篡改的虚假新闻的鲁棒检测系统。

Method: 1. 生成器对真实文章进行事实性扰动改写；2. 轻量级检测器使用密集段落检索验证声明；3. 引入语言对抗反馈机制，通过结构化自然语言批评指导生成器进化，迫使检测器适应改进。

Result: 在虚假新闻检测基准测试中，RADAR达到86.98% ROC-AUC，显著优于带检索的通用LLM。消融研究表明检测器端检索带来最大增益，而VAF和少样本演示为鲁棒训练提供关键信号。

Conclusion: RADAR通过检索增强检测和对抗精化的协同进化框架，有效提升了虚假新闻检测的鲁棒性和准确性，为对抗LLM生成的虚假信息提供了有效解决方案。
...
TL;DR: 提出Benchmark^2框架，包含三个评估基准质量的指标：跨基准排名一致性、区分度分数和能力对齐偏差，用于系统性评估LLM基准质量。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型基准测试的快速增多，需要系统方法来评估基准本身的质量，以解决基准质量参差不齐的问题。

Method: 提出Benchmark^2框架，包含三个互补指标：1)跨基准排名一致性，衡量基准产生的模型排名是否与其他基准一致；2)区分度分数，量化基准区分不同模型的能力；3)能力对齐偏差，识别同一模型家族中强模型失败而弱模型成功的异常实例。

Result: 在15个涵盖数学、推理和知识领域的基准上对11个LLM进行实验，发现现有基准质量存在显著差异，并证明基于这些指标的选择性基准构建可以在大幅减少测试集的情况下获得可比较的评估性能。

Conclusion: Benchmark^2为系统评估LLM基准质量提供了有效框架，有助于识别高质量基准并优化基准构建，减少评估成本。
...
TL;DR: VotIE：从市政会议记录中提取结构化投票事件的新任务，在葡萄牙市政会议记录上建立首个基准，发现微调编码器在域内表现最佳，但跨市泛化时少样本LLM更稳健。


<details>
  <summary>Details</summary>
Motivation: 市政会议记录以高度异构的自由文本形式记录投票结果，不同于标准化的议会记录，这给自动化提取带来了重大挑战。需要开发专门的信息提取任务来处理这种非结构化数据。

Method: 引入VotIE（投票信息提取）任务，使用葡萄牙市政会议记录（基于CitiLink语料库）建立首个基准。实验比较了微调编码器（XLM-R-CRF）与生成式方法，并在域内和跨市泛化两种设置下进行评估。

Result: 1. 在标准域内评估中，微调编码器（XLM-R-CRF）表现最佳，达到93.2%的宏观F1分数，优于生成式方法。
2. 在跨市泛化评估中，微调模型性能显著下降，而少样本LLM表现出更强的稳健性，性能下降幅度更小。
3. 尽管生成式模型泛化能力更强，但其高计算成本限制了实际应用。
...
<details>
  <summary>Details</summary>
Motivation: 语言模型往往倾向于使用输入中特定位置的信息，而不考虑语义相关性。虽然位置偏差在各种上下文中已被研究，但先前工作尚未建立这些偏差如何在各个层和输入位置之间演变，或者它们如何独立于任务复杂性而变化。

Method: 引入基于归因的框架，使用层传导性和滑动窗口方法，量化每个层如何在输入位置之间分配重要性，生成层级位置重要性剖面。

Result: 发现位置重要性剖面是架构特定的、跨输入稳定的、对词汇打乱不变的。存在显著的近因偏差（随深度增加）和微弱的首因偏差（随模型深度减弱）。早期层在所有位置上优先加权内容词而非功能词，而后期层失去这种词类型区分。

Conclusion: 该研究揭示了语言模型中系统性的位置处理模式，这些模式独立于语义内容，为理解模型内部表示和偏差提供了新视角。
...
TL;DR: 本文提出并评估了LLM学生模拟方法，发现现有提示策略效果不佳，监督微调和偏好优化有所改进但仍有限，为这一挑战性任务提供了基准和评估指标。


<details>
  <summary>Details</summary>
Motivation: LLM在教育领域的应用需要真实学生进行评估，但这耗时且难以扩展。现有研究多使用简单提示进行学生模拟，但缺乏对学生模拟质量的评估和衡量标准。

Method: 正式定义学生模拟任务，提出涵盖语言、行为和认知三个维度的评估指标，在真实数学辅导对话数据集上对多种学生模拟方法进行基准测试。

Result: 自动和人工评估均显示：提示策略的学生模拟效果较差；监督微调和偏好优化表现更好但仍有限，表明这是一个具有挑战性的任务。

Conclusion: 学生模拟是一个需要进一步研究的挑战性任务，本文提出的评估框架和方法为未来工作提供了基准和方向。
...
TL;DR: 论文提出SpeakerSleuth基准，评估大型音频语言模型在多轮对话中判断说话人一致性的能力，发现模型存在文本优先于声学的模态不平衡问题。


<details>
  <summary>Details</summary>
Motivation: 大型音频语言模型作为评估语音生成质量的工具已广泛应用，但其在多轮对话中评估说话人一致性的能力尚未被探索。需要评估模型是否能可靠地判断多轮对话中的说话人一致性，以构建可靠的音频语言评估器。

Method: 提出SpeakerSleuth基准，包含三个反映实际需求的任务。构建了1,818个人工验证的评估实例，涵盖四个不同数据集（合成和真实语音），并控制声学难度。评估了九个广泛使用的大型音频语言模型。

Result: 模型在可靠检测声学不一致性方面表现不佳：有些模型过度预测不一致性，有些则过于宽松；模型难以准确定位有问题的对话轮次；当提供其他对话者的轮次时，性能显著下降，模型优先考虑文本连贯性而非声学线索，甚至无法检测明显的说话人性别切换。但模型在选择最佳匹配说话人的音频变体方面表现更好，显示出固有的声学辨别能力。

Conclusion: 大型音频语言模型存在显著偏见：倾向于优先考虑文本而非声学，揭示了需要解决的基本模态不平衡问题，以构建可靠的音频语言评估器。
...
TL;DR: 该论文研究多语言机器翻译中的跨语言知识迁移，开发方法提升低资源语言的鲁棒性和泛化能力，分析语言相似性、检索增强、微调策略等对多语言学习的影响。


<details>
  <summary>Details</summary>
Motivation: 多语言机器翻译系统旨在实现跨语言知识共享，但低资源语言由于平行数据有限，面临泛化和迁移挑战。需要理解多语言模型如何跨语言共享知识，以及表征、数据可用性和训练策略之间的相互作用。

Method: 以机器翻译为主要测试平台，研究跨语言知识迁移：1) 分析语言相似性对迁移的影响；2) 使用检索和辅助监督增强低资源翻译；3) 研究并行数据微调在大语言模型中的权衡；4) 考察训练中语言多样性的作用。

Result: 研究表明：语言相似性影响知识迁移效果；检索和辅助监督能增强低资源翻译；增加翻译覆盖范围能改善泛化并减少离目标行为；并行数据微调在大语言模型中可能引入非预期权衡。

Conclusion: 建模选择和数据组成显著影响多语言学习效果，该研究为构建更具包容性和鲁棒性的多语言NLP系统提供了见解，强调需要平衡语言覆盖、数据质量和训练策略。
...
<details>
  <summary>Details</summary>
Motivation: 当外部检索证据与LLM内部记忆知识冲突时，模型往往倾向于依赖记忆事实而非上下文，导致输出不忠实。现有方法需要微调或带来较大推理开销。

Method: 提出ContextFocus激活引导方法，通过轻量级干预模型激活来增强对上下文的关注，无需模型微调，推理开销极小。
...
TL;DR: SaLAD是一个多模态安全基准测试，包含2,013个真实世界图像-文本样本，涵盖10个常见类别，用于评估MLLMs在日常生活中的安全影响。研究发现当前顶级模型在危险查询上的安全响应率仅为57.2%，现有安全对齐方法效果有限。


<details>
  <summary>Details</summary>
Motivation: 随着多模态大语言模型成为人类生活中不可或缺的助手，其生成的不安全内容对人类行为构成威胁。需要研究和评估MLLMs响应在日常生活中的安全影响，现有基准测试存在不足。

Method: 提出SaLAD多模态安全基准，包含2,013个真实世界图像-文本样本，覆盖10个常见类别，平衡设计包含不安全场景和过度敏感案例。强调真实风险暴露、真实视觉输入和细粒度跨模态推理。提出基于安全警告的评估框架，鼓励模型提供清晰信息性警告而非通用拒绝。

Result: 在18个MLLMs上的测试显示，顶级模型在危险查询上的安全响应率仅为57.2%。即使流行的安全对齐方法在当前场景下效果也有限，揭示了当前MLLMs在识别日常生活中危险行为方面的脆弱性。

Conclusion: 当前MLLMs在识别日常生活中的安全风险方面存在显著不足，现有安全对齐方法效果有限。SaLAD基准为评估和改进MLLMs的安全性能提供了重要工具，揭示了多模态安全评估的必要性。
...
TL;DR: MPO是一个基于模式的提示优化框架，将提示视为结构化对象，通过局部文本梯度独立优化各个语义部分，保持整体结构固定，从而提升小型开源语言模型的推理性能。


<details>
  <summary>Details</summary>
Motivation: 现有提示优化方法将提示视为整体文本块，难以定位错误、保留关键指令或防止提示无限增长。对于依赖显式结构的小型开源指令调优模型，需要更精细的提示优化方法。

Method: MPO将提示视为结构化对象，包含系统角色、上下文、任务描述、约束和输出格式等固定语义部分。使用批评语言模型生成局部文本梯度，独立优化每个部分，同时保持整体提示模式固定。通过去重整合部分更新，减少冗余和组件间干扰。
...
Conclusion: 保持固定提示模式的同时应用局部、部分级优化是提高小型开源语言模型推理性能的有效实用方法。MPO提供了可解释且稳健的优化过程。
...
TL;DR: CoM-DAD提出了一种新的概率框架，通过分层双过程将多模态生成解耦为高层语义规划和低层token合成，解决了传统方法中离散和连续数据生成模型分裂的问题。


<details>
  <summary>Details</summary>
Motivation: 当前生成建模分裂为处理离散数据（文本）的自回归方法和处理连续数据（图像）的扩散方法，阻碍了真正统一的多模态系统发展。掩码语言模型虽然提供高效双向上下文，但缺乏自回归模型的生成保真度和扩散模型的语义连续性，且扩展到多模态设置会引入严重的对齐挑战和训练不稳定性。

Method: CoM-DAD采用分层双过程：1）通过连续潜在扩散过程建模语义流形；2）将token生成视为离散吸收扩散过程，由可变速率噪声调度调节，并以演化的语义先验为条件。引入随机混合模态传输策略来对齐不同模态，无需繁重的对比双编码器。
...
TL;DR: 提出一个通过代码引导知识图谱探索来减少LLM幻觉的框架，在多个基准测试中显著提升准确率
...
Method: 扩展链式知识蒸馏方法，加入可编程模块引导知识图谱探索，将可执行代码嵌入推理提示中，开发增强的蒸馏推理框架来显式调控中间推理步骤

Result: 在GPT-4和LLaMA-3.3上的实验显示，代码引导推理显著改善上下文建模并减少提示诱导幻觉，HIT@1、HIT@3、HIT@5分别提升15.64%、13.38%、13.28%，多个评估设置中得分超过95%

Conclusion: 该方法能有效约束错误推理，同时提高准确性和可解释性，表明代码引导的知识探索是减少LLM幻觉的有效途径
...
TL;DR: SearchAttack：一种针对检索增强LLM的红队攻击方法，通过将有害语义外包给网络搜索，仅保留查询骨架和碎片化线索，引导LLM重构检索内容以实现恶意目标


<details>
  <summary>Details</summary>
Motivation: 人们意识到LLM在开放和知识密集型任务中存在不可靠性差距，因此转向检索增强LLM。但当搜索引擎被触发执行有害任务时，LLM的安全防护无法控制返回内容，一旦返回内容包含可直接使用的有害信息，LLM无法撤回这种暴露。这揭示了网络搜索作为关键攻击面

Method: SearchAttack将有害语义外包给网络搜索，仅保留查询骨架和碎片化线索，然后通过结构化模板引导LLM重构检索内容以实现恶意目标。该方法利用网络搜索作为攻击媒介，绕过LLM的安全防护

Result: 大量实验表明，SearchAttack在攻击检索增强LLM系统方面表现出很强的有效性，成功评估了这些系统的责任漏洞

Conclusion: 网络搜索是检索增强LLM的关键攻击面，SearchAttack通过将有害内容外包给搜索引擎并引导LLM重构，有效绕过了现有安全防护，揭示了这类系统的安全漏洞
...
<details>
  <summary>Details</summary>
Motivation: 解决缺乏从结构化辩论创建多方对话资源的问題，促进透明可复现的工作流程

Method: 开发交互式平台，可视化讨论树，允许用户构建线性化对话序列，集成LLM辅助自动编辑消息和说话者描述

Result: 树形可视化有助于创建连贯有意义的对话线程，LLM支持提高输出质量并减少人工工作量

Conclusion: LLMberjack是一个开源工具，填补了从现有辩论创建多方对话资源的空白，支持透明可复现的工作流程
...
TL;DR: FLEx是一种通过少量解释示例改进语言模型行为的方法，使用聚类选择代表性错误，验证解释有效性，并总结为提示前缀，在不修改模型权重的情况下减少错误。


<details>
  <summary>Details</summary>
Motivation: 语言模型虽然能力广泛但仍会犯错，且错误在相关查询中重复出现。自然语言解释有助于纠正错误，但在需要专家标注的领域大规模收集解释不可行。
...
Conclusion: FLEx通过少量解释示例有效改进模型行为，无需修改模型权重，为解决语言模型错误重复问题提供了实用方案。
...
TL;DR: RFC Bench是一个评估大语言模型在现实金融新闻中处理金融虚假信息的基准测试，包含无参考检测和基于比较的诊断两个任务，揭示模型在有比较上下文时表现更好，无参考设置下存在显著弱点。


<details>
  <summary>Details</summary>
Motivation: 当前需要评估大语言模型在现实金融新闻环境中处理金融虚假信息的能力，特别是在金融新闻的语境复杂性下，意义往往来自分散的线索，需要专门的基准测试来衡量模型表现。

Method: 开发RFC Bench基准测试，在段落级别操作，捕捉金融新闻的语境复杂性。定义两个互补任务：1) 无参考虚假信息检测；2) 基于比较的诊断，使用成对的原始-扰动输入进行对比分析。

Result: 实验显示一致模式：当有比较上下文可用时，模型表现显著更强；而在无参考设置下暴露显著弱点，包括不稳定的预测和升高的无效输出。这表明当前模型在没有外部基础的情况下难以维持一致的信念状态。
...
### [182] [The Pneuma Project: Reifying Information Needs as Relational Schemas to Automate Discovery, Guide Preparation, and Align Data with Intent](https://arxiv.org/abs/2601.03618)
*Muhammad Imam Luthfi Balaka,Raul Castro Fernandez*

Main category: cs.DB

TL;DR: Pneuma-Seeker是一个LLM驱动的系统，通过迭代交互帮助用户表达和满足信息需求，将模糊意图转化为可用的关系数据模型和文档。


<details>
  <summary>Details</summary>
Motivation: 数据发现和准备是数据管理生命周期中的持续瓶颈，特别是当用户意图模糊、不断演变或难以操作化时。需要一种方法来帮助用户表达和满足信息需求。

Method: 系统结合三种架构思想：1) 上下文专业化以减少LLM在子任务中的负担；2) 指挥式规划器来组装动态执行计划；3) 基于共享状态的收敛机制。整合了RAG、代理框架和结构化数据准备技术。

Result: 通过LLM用户模拟评估显示，系统能帮助发现潜在意图、指导探索过程，并生成符合用途的文档。同时作为新兴文档层，捕获机构知识并支持组织记忆。

Conclusion: Pneuma-Seeker通过语言模型驱动的迭代交互，有效解决了模糊意图下的数据发现和准备问题，将用户信息需求转化为可用的关系数据模型和文档。
...
### [183] [Staged Voxel-Level Deep Reinforcement Learning for 3D Medical Image Segmentation with Noisy Annotations](https://arxiv.org/abs/2601.03875)
*Yuyang Fu,Xiuzhen Guo,Ji Shi*

Main category: eess.IV

TL;DR: 提出SVL-DRL框架，通过分阶段强化学习解决医学图像分割中的噪声标注问题，将每个体素视为自主代理，动态修正标签错误，在三个公开数据集上实现SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 医学图像分割依赖高质量标注数据，但由于器官形态结构复杂和标注者差异，噪声标注普遍存在，严重影响分割模型性能。受标注者基于先验知识修正标签错误的启发，需要自动化的噪声标注处理方案。

Method: 提出端到端分阶段体素级深度强化学习框架：1) 将噪声标注建模为体素依赖问题，采用分阶段强化学习保证模型鲁棒收敛；2) 引入体素级异步优势演员-评论家模块，每个体素作为自主代理动态优化状态表示；3) 设计新的动作空间和复合奖励函数，结合Dice值和空间连续性指标。

Result: 在三个公开医学图像数据集上的实验表明，在各种实验设置下均达到SOTA性能，Dice和IoU分数平均提升超过3%。
...
### [184] [Mastering the Game of Go with Self-play Experience Replay](https://arxiv.org/abs/2601.03306)
...
<details>
  <summary>Details</summary>
Motivation: 围棋长期以来作为人工智能的基准测试，需要复杂的战略推理和长期规划。先前方法如AlphaGo主要依赖基于模型的蒙特卡洛树搜索，本研究旨在探索模型无关强化学习在围棋中的效率

Method: 基于熵正则化Q学习的模型无关强化学习算法，使用单一Q值网络统一策略评估和改进，通过自博弈和离策略经验回放进行训练，无需人类数据或搜索过程

Result: 在仅使用7个GPU训练5个月的情况下，QZero达到了与AlphaGo相当的性能水平，首次证明了模型无关强化学习在掌握围棋游戏中的效率

Conclusion: 该研究首次展示了模型无关强化学习在掌握围棋游戏中的效率，以及离策略强化学习在解决大规模复杂环境中的可行性，为强化学习在复杂决策问题中的应用提供了新途径
...
### [185] [Digital Red Queen: Adversarial Program Evolution in Core War with LLMs](https://arxiv.org/abs/2601.03335)
*Akarsh Kumar,Ryan Bahlous-Boldi,Prafull Sharma,Phillip Isola,Sebastian Risi,Yujin Tang,David Ha*

Main category: cs.AI

TL;DR: 提出Digital Red Queen算法，利用LLM在Core War游戏中通过自我对抗演化程序，模拟真实世界的"红皇后"动态进化过程。


<details>
  <summary>Details</summary>
Motivation: 现有LLM进化框架多为静态优化问题，忽略了真实进化过程中的开放对抗动态。研究旨在探索动态"红皇后"目标的价值，模拟生物进化中的持续适应过程。

Method: 提出Digital Red Queen算法，在Core War游戏中让LLM演化汇编式程序（warriors），通过自我对抗不断适应变化的目标。每轮演化新warrior击败之前所有warrior，形成适应序列。

Result: warriors随时间变得更加通用（相对于人类warriors），且独立运行中的行为多样性减少，表明向通用行为策略的收敛压力，类似于自然界的趋同进化。

Conclusion: 从静态目标转向动态红皇后目标具有潜在价值。Core War可作为研究对抗适应的丰富沙盒，DRQ的简单有效性表明类似方法可用于网络安全、抗药性等实际对抗领域。
...
### [186] [Enhancing LLM Instruction Following: An Evaluation-Driven Multi-Agentic Workflow for Prompt Instructions Optimization](https://arxiv.org/abs/2601.03359)
*Alberto Purpura,Li Wang,Sahil Badyal,Eugenio Beaufrand,Adam Faulkner*

Main category: cs.AI

TL;DR: 提出多智能体工作流，将主要任务描述优化与约束条件解耦，通过量化分数反馈迭代改进提示，显著提升LLM输出对形式约束的遵从性


<details>
  <summary>Details</summary>
Motivation: 大型语言模型经常生成内容相关但不符合形式约束的输出，传统提示优化方法主要关注重述主要任务描述，忽略了作为响应验收标准的细粒度约束条件

Method: 提出新颖的多智能体工作流，将主要任务描述优化与其约束条件解耦，使用量化分数作为反馈，迭代重写和改进提示

Result: 评估显示该方法生成的修订提示在Llama 3.1 8B和Mixtral-8x 7B等模型上产生显著更高的遵从性分数

Conclusion: 通过解耦任务描述与约束优化，使用量化反馈的多智能体工作流能有效提高LLM输出对形式约束的遵从性
...
### [187] [Exploration Through Introspection: A Self-Aware Reward Model](https://arxiv.org/abs/2601.03389)
...
<details>
  <summary>Details</summary>
Motivation: 研究人工智能如何建模内部心理状态对于推进AI中的心智理论至关重要。现有证据表明自我认知与他人认知存在统一系统，本文通过让强化学习智能体在网格世界中推断自身内部状态来探索这种自我意识。

Method: 引入受生物疼痛启发的内省探索组件，使用隐马尔可夫模型从在线观察中推断"疼痛信念"，将该信号整合到主观奖励函数中，建立计算框架比较正常与慢性疼痛感知模型的性能差异。

Result: 内省智能体在总体上显著优于标准基线智能体，能够复现复杂的人类行为模式，验证了自我意识对智能体学习能力的积极影响。

Conclusion: 通过计算建模方法成功展示了自我意识在AI系统中的重要性，内省探索框架为理解心智理论和疼痛感知机制提供了有效工具，有助于开发更人性化的人工智能系统。
...
### [188] [Toward Maturity-Based Certification of Embodied AI: Quantifying Trustworthiness Through Measurement Mechanisms](https://arxiv.org/abs/2601.03470)
*Michael C. Darling,Alan H. Hesu,Michael A. Mardikes,Brian C. McGuigan,Reed M. Milewicz*

Main category: cs.AI

TL;DR: 提出基于成熟度的认证框架，通过显式测量机制来认证具身AI系统


<details>
  <summary>Details</summary>
Motivation: 当前缺乏对具身AI系统进行认证的结构化评估框架和定量评分机制，需要解决可信度评估中的多目标权衡问题

Method: 提出基于成熟度的认证框架，包含结构化评估框架、定量评分机制和多目标权衡导航方法，以不确定性量化作为示例测量机制，通过无人机系统检测案例研究验证可行性

Result: 展示了该框架的可行性，通过不确定性量化测量机制在无人机系统检测案例中进行了验证

Conclusion: 基于成熟度的认证框架为具身AI系统提供了可认证的评估方法，通过显式测量机制和结构化评估能够有效解决可信度评估的挑战
...
### [189] [CPGPrompt: Translating Clinical Guidelines into LLM-Executable Decision Support](https://arxiv.org/abs/2601.03475)
*Ruiqi Deng,Geoffrey Martin,Tony Wang,Gongbo Zhang,Yi Liu,Chunhua Weng,Yanshan Wang,Justin F Rousseau,Yifan Peng*

Main category: cs.AI

TL;DR: CPGPrompt：将临床指南转化为LLM可执行决策树的自动提示系统，在专科转诊决策上表现优异，但在多类路径分配上表现有差异
...
Conclusion: CPGPrompt能有效将临床指南转化为LLM可执行的决策树，在二元决策任务上表现优异。多类路径分配的性能差异揭示了不同指南结构对AI系统的影响，为未来改进提供了方向。
...
### [190] [Personalization of Large Foundation Models for Health Interventions](https://arxiv.org/abs/2601.03482)
...
<details>
  <summary>Details</summary>
Motivation: 大型基础模型正在改变医疗AI，但能否提供真正的个性化治疗建议仍存疑问。研究发现个性化与外部有效性存在根本矛盾（泛化悖论），以及隐私-性能、规模-特异性、自动化-同理心等多重矛盾。需要明确预测与因果的边界，负责任地将AI整合到个性化医疗中。

Method: 分析大型基础模型在个性化医疗中的局限性，特别是其预测能力与因果理解之间的差距。提出混合框架：基础模型从多模态群体数据中生成干预候选假设并排序，然后触发N-of-1试验进行个体因果验证。N-of-1试验作为个体因果推断的金标准，通过交叉自实验提供个体内因果证据。

Result: 大型基础模型无法替代N-of-1试验，两者具有互补优势。基础模型擅长从群体模式快速生成假设，N-of-1试验擅长个体因果验证。混合框架结合两者优势，基础模型生成带不确定性估计的干预候选排名，触发后续N-of-1试验，从而解决个性化医疗中的多重矛盾。

Conclusion: 大型基础模型和N-of-1试验是互补而非替代关系。明确预测与因果的边界，通过混合框架结合两者的优势，是负责任地将AI整合到个性化医疗中的关键。这种结合能够解决泛化悖论、隐私-性能矛盾等挑战，实现真正的个性化治疗建议。
...
### [191] [Evolving Programmatic Skill Networks](https://arxiv.org/abs/2601.03509)
*Haochen Shi,Xingdi Yuan,Bang Liu*

Main category: cs.AI

TL;DR: PSN是一个用于持续技能学习的框架，使用可执行的符号程序作为技能，通过LLM实现反射、渐进优化和重构机制，在开放环境中表现出强大的技能复用和泛化能力。


<details>
  <summary>Details</summary>
Motivation: 在开放式的具身环境中，智能体需要持续获取、细化和复用不断扩展的技能库，以应对复杂的任务分布。现有方法在技能组合、稳定性和网络紧凑性方面存在挑战。

Method: 提出程序化技能网络（PSN），将技能表示为可执行的符号程序，形成组合网络。通过LLM实现三个核心机制：1) REFLECT用于技能组合的结构化故障定位；2) 成熟度感知的渐进优化门控机制；3) 回滚验证下的规范结构重构。

Result: 在MineDojo和Crafter环境中的实验表明，PSN能够实现鲁棒的技能复用、快速适应，并在开放式任务分布上表现出强大的泛化能力。

Conclusion: PSN框架通过程序化技能表示和LLM驱动的机制，有效解决了持续技能学习中的组合性、稳定性和紧凑性问题，其学习动态与神经网络训练具有结构相似性。
...
### [192] [Variance Computation for Weighted Model Counting with Knowledge Compilation Approach](https://arxiv.org/abs/2601.03523)
*Kengo Nakamura,Masaaki Nishino,Norihito Yasuda*

Main category: cs.AI

TL;DR: 本文研究了加权模型计数(WMC)的方差计算问题，针对参数不确定性场景，提出了结构化d-DNNF的多项式时间算法，证明了其他表示形式的计算困难性，并在贝叶斯网络推理中进行了实证应用。
...
Method: 1. 为结构化d-DNNF推导多项式时间算法计算WMC方差；2. 证明结构化DNNF、d-DNNF和FBDDs的计算困难性；3. 在贝叶斯网络推理中应用该方法，实证分析参数方差对边缘概率方差的影响。

Result: 1. 成功为结构化d-DNNF设计了多项式时间方差计算算法；2. 证明了其他表示形式（结构化DNNF、d-DNNF、FBDDs）的方差计算是困难的，尽管它们支持多项式时间WMC计算；3. 实证表明算法能有效评估真实世界贝叶斯网络的边缘概率方差。

Conclusion: 本文首次系统研究了WMC方差计算问题，揭示了不同知识编译表示形式在方差计算上的复杂性差异，为不确定性推理提供了实用工具，并在贝叶斯网络推理中展示了应用价值。
...
### [193] [STAR-S: Improving Safety Alignment through Self-Taught Reasoning on Safety Rules](https://arxiv.org/abs/2601.03537)
*Di Wu,Yanyan Zhao,Xin Lu,Mingzhe Li,Bing Qin*

Main category: cs.AI

TL;DR: STAR-S是一个通过自学习循环来增强LLM安全推理能力的框架，有效防御越狱攻击


<details>
  <summary>Details</summary>
Motivation: 现有方法试图通过让模型基于安全规则进行推理来提升安全性，但难以明确设计或直接获得有效的安全推理形式，需要一种能自动学习安全规则推理的方法

Method: 提出STAR-S框架，将安全规则推理学习整合到自学习循环中：通过安全规则引导的推理和反思来生成数据，然后通过微调增强安全推理能力，形成协同循环

Result: 实验表明STAR-S能有效防御越狱攻击，性能优于基线方法

Conclusion: STAR-S通过自学习循环增强LLM的安全规则推理能力，为防御越狱攻击提供了一种有效方法
...
### [194] [ReEfBench: Quantifying the Reasoning Efficiency of LLMs](https://arxiv.org/abs/2601.03550)
*Zhizhang Fu,Yuancheng Gu,Chenkai Hu,Hanmeng Liu,Yue Zhang*

Main category: cs.AI

TL;DR: 本文提出神经符号框架评估LLM推理过程，发现扩展token生成并非深度推理必要条件，混合长短CoT训练会导致早熟饱和，蒸馏小模型无法复制逻辑效能。
...
Conclusion: 需要超越简单长度扩展的评估方法，训练数据混合策略需要谨慎设计，小模型存在固有容量限制影响逻辑推理能力。
...
### [195] [SCRIBE: Structured Mid-Level Supervision for Tool-Using Language Models](https://arxiv.org/abs/2601.03555)
...
<details>
  <summary>Details</summary>
Motivation: 现有工具增强代理在多步推理中存在信用分配困难，LLM-based奖励模型缺乏细粒度、任务特定的评估标准，导致奖励信号噪声大且不一致。

Method: 引入SCRIBE框架，在中间层抽象进行干预，基于技能原型库进行奖励建模，将开放式LLM评估转化为约束验证问题，为每个子目标分配对应原型并提供结构化评估标准。

Result: 在多个推理和工具使用基准测试中达到最先进性能，将Qwen3-4B模型的AIME25准确率从43.3%提升至63.3%，显著提高复杂多轮工具交互的成功率。

Conclusion: SCRIBE通过中间层技能掌握促进高层规划行为演化，与低层工具优化互补，为构建更自主可靠的工具使用代理提供了可扩展路径。
...
### [196] [Controllable LLM Reasoning via Sparse Autoencoder-Based Steering](https://arxiv.org/abs/2601.03595)
*Yi Fang,Wenjie Wang,Mingfeng Xue,Boyi Deng,Fengli Xu,Dayiheng Liu,Fuli Feng*

Main category: cs.AI

TL;DR: SAE-Steering：利用稀疏自编码器分解推理策略纠缠的隐藏状态，通过两阶段特征识别流程控制大型推理模型的推理策略，提升控制效果和准确性
...
Method: 使用稀疏自编码器将策略纠缠的隐藏状态分解为解耦特征空间，提出SAE-Steering两阶段特征识别流程：先召回放大策略特定关键词logits的特征（过滤99%以上特征），再按控制效果排序剩余特征

Result: SAE-Steering作为控制向量优于现有方法超过15%的控制效果，控制推理策略能将模型从错误路径转向正确路径，实现7%的绝对准确率提升

Conclusion: 通过解耦策略特征并有效识别策略特定特征，SAE-Steering能够可靠控制大型推理模型的推理策略，显著提升推理可靠性和准确性
...
### [197] [Interleaved Tool-Call Reasoning for Protein Function Understanding](https://arxiv.org/abs/2601.03604)
...
<details>
  <summary>Details</summary>
Motivation: 研究发现直接将基于文本的思维链推理范式应用于蛋白质功能理解效果不佳，因为蛋白质功能预测是知识密集型的科学任务，需要依赖外部生物先验知识和计算工具，而非纯内部推理
...
Conclusion: 蛋白质功能预测需要整合领域工具和外部知识，而非单纯依赖语言模型的内部推理能力，PFUA展示了工具增强方法在该领域的有效性
...
### [198] [Architecting Agentic Communities using Design Patterns](https://arxiv.org/abs/2601.03624)
*Zoran Milosevic,Fethi Rabhi*

Main category: cs.AI

TL;DR: 论文提出了一种基于企业分布式系统标准、形式化方法和行业实践的LLM与Agentic AI系统架构设计模式，重点关注Agentic Communities（AI代理与人类协调的组织框架），通过形式化框架确保可验证治理。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型和Agentic AI技术的快速发展，需要系统性的架构指导来构建复杂的生产级系统。现有方法缺乏结合企业实践与形式化验证的综合性框架。

Method: 提出三层设计模式分类：LLM代理（任务特定自动化）、Agentic AI（自适应目标寻求者）、Agentic Communities（组织框架）。重点研究Agentic Communities，借鉴分布式系统协调原则，建立形式化框架，通过协作协议、角色分配和治理结构确保可验证治理。

Result: 开发了一个形式化框架，能够表达组织、法律和伦理规则，通过问责机制确保代理间通信、协商和意图建模的可操作治理。通过临床试验匹配案例研究验证了该框架的有效性。

Conclusion: 该研究为从业者提供了可操作的指导，同时保持了企业部署在动态多代理生态系统中所需的形式化严谨性，实现了实践指导与形式化验证能力的平衡。
...
### [199] [How Does the Thinking Step Influence Model Safety? An Entropy-based Safety Reminder for LRMs](https://arxiv.org/abs/2601.03662)
*Su-Hyeon Kim,Hyundong Jin,Yejin Lee,Yo-Sub Han*

Main category: cs.AI

TL;DR: SafeRemind：一种在解码时动态注入安全提醒短语到大型推理模型思维步骤中的防御方法，显著提升安全性而不影响推理能力
...
Conclusion: SafeRemind通过动态注入安全提醒短语到思维步骤中，有效解决了大型推理模型的安全漏洞，提供了一种无需参数更新的高效防御机制。
...
### [200] [Sandwich Reasoning: An Answer-Reasoning-Answer Approach for Low-Latency Query Correction](https://arxiv.org/abs/2601.03672)
*Chen Zhang,Kepu Zhang,Jiatong Zhang,Xiao Zhang,Jun Xu*

Main category: cs.AI

TL;DR: Sandwich Reasoning (SandwichR) 提出了一种新的推理范式，通过"答案-推理-答案"的三明治结构，在保持推理精度的同时显著降低查询纠正的延迟，解决了传统思维链推理在实时系统中的延迟问题。


<details>
  <summary>Details</summary>
Motivation: 现代搜索管道中的查询纠正需要在实时延迟约束下保持高精度。思维链推理虽然能提高准确性，但其延迟过高无法满足实时需求。提前输出答案可以降低延迟，但在自回归解码中，早期答案无法利用后续推理来改进准确性，存在延迟与精度的权衡问题。

Method: 提出Sandwich Reasoning方法，采用"答案-推理-答案"范式：先生成初始纠正，然后进行显式推理过程，最后生成最终精炼纠正。通过一致性感知的强化学习策略，包括专门的一致性奖励来对齐初始和最终纠正，以及基于边界的拒绝采样来优先处理推理带来最大纠正增益的边界样本。同时构建了高质量的查询纠正数据集。

Result: 实验结果表明，SandwichR在保持与标准思维链推理相当的SOTA准确性的同时，实现了40-70%的延迟降低，成功解决了在线搜索中的延迟-准确性权衡问题。

Conclusion: SandwichR通过创新的三明治推理范式，在查询纠正任务中实现了延迟与精度的双赢，为实时搜索系统提供了一种有效的解决方案，平衡了推理能力和响应速度的需求。
...
### [201] [Personalized Medication Planning via Direct Domain Modeling and LLM-Generated Heuristics](https://arxiv.org/abs/2601.03687)
...
<details>
  <summary>Details</summary>
Motivation: 现有自动化用药规划方法受限于通用领域无关启发式函数，最多只能处理7种药物，这在临床实践中远远不够，需要提升可扩展性以接近实际应用

Method: 通过程序化方式定义领域（指定初始状态和状态转移过程），使用LLM生成针对具体问题的启发式函数，结合固定的搜索算法（GBFS）进行用药规划

Result: 方法显著提升了覆盖率和规划时间，将可处理的药物数量从最多7种扩展到至少28种，使用药规划更接近实际临床应用

Conclusion: 使用LLM生成领域特定启发式函数是提升自动化用药规划可扩展性的有效方法，为临床实际应用迈出了重要一步
...
### [202] [EntroCoT: Enhancing Chain-of-Thought via Adaptive Entropy-Guided Segmentation](https://arxiv.org/abs/2601.03769)
...
Method: 1. 熵基机制：在不确定节点处将推理轨迹分割为多个步骤；2. 蒙特卡洛rollout机制：评估每个步骤对最终答案的边际贡献；3. 准确过滤欺骗性推理样本，构建高质量数据集

Result: 在数学基准测试上的广泛实验表明，使用EntroCoT构建的子集进行微调，始终优于全数据集监督的基线方法

Conclusion: EntroCoT提供了一个统一框架，能够自动识别和精炼低质量思维链监督轨迹，确保每个推理轨迹中的每个中间步骤都有助于最终答案，显著提升数学推理能力
...
### [203] [ROI-Reasoning: Rational Optimization for Inference via Pre-Computation Meta-Cognition](https://arxiv.org/abs/2601.03822)
*Muyang Zhao,Qi Qi,Hao Sun*

Main category: cs.AI

TL;DR: ROI-Reasoning框架让大语言模型在严格计算预算下进行推理，通过元认知微调和强化学习优化计算资源分配


<details>
  <summary>Details</summary>
Motivation: 大语言模型虽然具备强大推理能力，但不知道不同任务需要多少计算量。在严格全局token约束下，需要让模型学会预测任务难度、估计投资回报率，并战略性地分配计算资源
...
Conclusion: 该研究将预算推理形式化为有序随机多选择背包问题，提出的ROI-Reasoning框架赋予LLMs内在的预算感知理性，实现了在有限计算资源下的高效推理
...
### [204] [Defeasible Conditionals using Answer Set Programming](https://arxiv.org/abs/2601.03840)
*Racquel Dennison,Jesse Heyninck,Thomas Meyer*

Main category: cs.AI

TL;DR: 本文提出了一种使用答案集编程（ASP）计算理性闭包（RC）的声明式方法，该方法能够自动构建最小排序模型并支持查询推理，在保持理论正确性的同时提高了计算效率。


<details>
  <summary>Details</summary>
Motivation: 理性闭包（RC）是KLM框架中用于可废止推理的重要算法，但现有实现多为命令式方法。本文旨在提供一种基于ASP的声明式方法，以更优雅、高效的方式实现RC计算。

Method: 采用答案集编程（ASP）对理性闭包进行编码，能够从给定知识库自动构建最小排序模型，并支持指定的查询推理。通过形式化证明验证了ASP编码的正确性。

Result: 实验评估表明，基于ASP的实现相比现有的命令式实现（如InfOCF求解器）具有更好的计算效率，同时严格遵循理性闭包的理论基础。

Conclusion: ASP为理性闭包计算提供了一种有效的声明式方法，不仅保持了理论正确性，还显著提升了计算性能，为可废止推理的实际应用提供了更好的工具支持。
...
### [205] [XAI-LAW: A Logic Programming Tool for Modeling, Explaining, and Learning Legal Decisions](https://arxiv.org/abs/2601.03844)
...
Method: 采用答案集编程（ASP）对意大利刑法典条款进行编码建模，包括"人身犯罪"和财产犯罪；利用归纳逻辑编程系统从案例中学习法律规则；处理编码过程中的矛盾；通过稳定模型的"支持性"提供解释

Result: 开发出能够支持法律专家推理的工具，可生成新案件的可能判决并提供解释；系统在先前判决集上得到验证和优化；自动可解释性功能有助于澄清司法决策背后的逻辑

Conclusion: ASP方法能有效建模刑法典并学习法律规则，开发的工具可增强刑事审判的推理支持和决策可解释性
...
### [206] [Formally Explaining Decision Tree Models with Answer Set Programming](https://arxiv.org/abs/2601.03845)
*Akihiro Takemura,Masayuki Otani,Katsumi Inoue*

Main category: cs.AI

TL;DR: 本文提出使用答案集编程(ASP)为决策树模型生成多种解释的方法，相比SAT方法更灵活且支持枚举所有解释。


<details>
  <summary>Details</summary>
Motivation: 决策树模型（如随机森林和梯度提升树）在机器学习中广泛应用，但其复杂结构难以解释，特别是在安全关键应用中需要正式决策依据。现有研究表明可以通过自动推理技术推导逻辑和溯因解释。
...
Conclusion: ASP方法为决策树模型提供了一种灵活的解释生成框架，能够生成多种类型的解释并支持完全枚举，在解释性方面具有优势。
...
### [207] [xDNN(ASP): Explanation Generation System for Deep Neural Networks powered by Answer Set Programming](https://arxiv.org/abs/2601.03847)
...
Result: 在合成数据集上的实验表明，提取的逻辑程序不仅保持高预测准确率，还提供了特征重要性和隐藏节点对预测影响等有价值信息。

Conclusion: xDNN(ASP)能够生成全局解释，帮助理解模型内部机制，并为网络优化（如减少隐藏节点）提供指导。
...
### [208] [Investigating the Grounding Bottleneck for a Large-Scale Configuration Problem: Existing Tools and Constraint-Aware Guessing](https://arxiv.org/abs/2601.03850)
*Veronika Semmelrock,Gerhard Friedrich*

Main category: cs.AI

TL;DR: ASP技术在处理大规模配置问题时面临内存瓶颈，作者提出约束感知猜测方法显著降低内存需求


<details>
  <summary>Details</summary>
Motivation: 研究当前ASP技术是否能够扩展到大型配置问题，特别是电子系统配置（可能包含超过30,000个组件），解决ASP中的"接地瓶颈"问题

Method: 首先研究增量求解方法，然后基于接地分析开发了约束感知猜测方法，该方法能显著减少内存需求

Result: 增量求解方法在实践中有效，但内存需求仍然存在显著限制；约束感知猜测方法显著降低了内存需求

Conclusion: 当前ASP技术在处理大规模配置问题时面临内存瓶颈，但通过约束感知猜测等创新方法可以显著改善性能，推动ASP技术向更大规模应用发展
...
### [209] [Current Agents Fail to Leverage World Model as Tool for Foresight](https://arxiv.org/abs/2601.03905)
...
TL;DR: 当前基于视觉语言模型的智能体在利用生成式世界模型进行未来状态预测时存在显著问题：调用频率低、误用预测结果、性能下降，主要瓶颈在于智能体决定何时模拟、如何解释预测结果以及如何将预见整合到推理中的能力不足。


<details>
  <summary>Details</summary>
Motivation: 随着智能体面临越来越多需要预测未来状态的任务，生成式世界模型作为外部模拟器提供了一种有前景的解决方案。本研究旨在实证检验当前智能体是否能够有效利用世界模型作为工具来增强其认知能力。

Method: 通过在不同智能体任务和视觉问答任务上进行实验，观察智能体如何调用和使用世界模型进行模拟。进行归因分析，识别智能体在使用世界模型时的主要瓶颈。

Result: 研究发现：1）智能体很少调用模拟（少于1%）；2）经常误用预测结果（约15%）；3）当模拟可用或被强制使用时，性能表现不一致甚至下降（最多5%）。归因分析显示主要瓶颈在于智能体决定何时模拟、如何解释预测结果以及如何将预见整合到下游推理的能力。

Conclusion: 当前智能体在利用世界模型进行前瞻性认知方面存在显著局限性，需要开发能够促进与世界模型进行校准、战略性交互的机制，以实现未来智能体系统中更可靠的预见性认知。
...
### [210] [Trade-R1: Bridging Verifiable Rewards to Stochastic Environments via Process-Level Reasoning Verification](https://arxiv.org/abs/2601.03948)
*Rui Sun,Yifan Sun,Sheng Xu,Li Zhao,Jing Li,Daxin Jiang,Chen Hua,Zuo Bai*

Main category: cs.AI

TL;DR: Trade-R1框架通过过程级推理验证，将可验证奖励与随机金融环境连接，解决标准RL在金融决策中的奖励黑客问题，使用三角一致性度量和两种奖励整合策略提升跨市场泛化能力。


<details>
  <summary>Details</summary>
Motivation: 虽然强化学习在数学和编程等可验证奖励领域取得了显著成果，但将其扩展到金融决策面临挑战：市场具有随机性，奖励虽然可验证但本质上是噪声的，导致标准RL退化为奖励黑客。
...
Conclusion: Trade-R1框架通过过程级推理验证成功解决了金融决策中RL的奖励黑客问题，特别是DSR策略在跨市场泛化和推理一致性方面表现最佳，为随机环境中的RL应用提供了有效解决方案。
...
### [211] [Anti-Length Shift: Dynamic Outlier Truncation for Training Efficient Reasoning Models](https://arxiv.org/abs/2601.03969)
*Wei Wu,Liyi Chen,Congxi Xiao,Tianfu Wang,Qimeng Wang,Chengqiang Lu,Yan Gao,Yi Wu,Yao Hu,Hui Xiong*

Main category: cs.AI

TL;DR: 提出DOT方法解决大模型推理时过度冗长问题，通过动态截断冗余token，在保持复杂问题推理能力的同时显著减少推理token使用


<details>
  <summary>Details</summary>
Motivation: 现有强化学习增强的大推理模型在简单查询上会产生过度冗长的推理过程，导致部署成本高昂，而现有的显式长度惩罚方法存在优化冲突且未深入探究生成机制

Method: 提出动态异常截断(DOT)：在训练时选择性抑制冗余token，仅针对完全正确rollout组中的极端长度尾部；结合辅助KL正则化和预测性动态采样确保稳定收敛

Result: 在多个模型规模上显著提升了效率-性能的帕累托前沿，在AIME-24上减少78%推理token使用的同时提高准确率，优于现有高效推理方法
...
### [212] [MobileDreamer: Generative Sketch World Model for GUI Agent](https://arxiv.org/abs/2601.04035)
...
<details>
  <summary>Details</summary>
Motivation: 现有移动GUI智能体多为反应式，仅基于当前屏幕做决策，在长时程任务中表现受限。构建世界模型能预测动作结果，支持更好的决策制定

Method: 提出MobileDreamer框架，包含文本草图世界模型和前瞻推演机制。世界模型将数字图像转换为关键任务相关草图，采用顺序不变学习策略保留GUI元素空间信息；前瞻推演利用世界模型预测能力优化动作选择

Result: 在Android World实验中达到SOTA性能，任务成功率提升5.25%。世界模型评估验证文本草图建模能准确预测关键GUI元素

Conclusion: MobileDreamer通过高效的世界模型前瞻框架显著提升了移动GUI智能体在长时程任务中的性能，为实际部署提供了可行方案
...
### [213] [ComfySearch: Autonomous Exploration and Reasoning for ComfyUI Workflows](https://arxiv.org/abs/2601.04060)
*Jinwei Su,Qizhen Lan,Zeyu Wang,Yinghui Xia,Hairu Wen,Yiqun Duan,Xi Xiao,Tianyu Shi,Yang Jingsong,Lewei He*

Main category: cs.AI

TL;DR: ComfySearch：一个基于验证引导的智能体框架，用于在ComfyUI平台上探索组件空间并生成功能性的工作流，显著提升了执行成功率、解决方案率和泛化能力。
...
Method: 提出ComfySearch框架，采用基于验证引导的工作流构建方法，通过智能体有效探索组件空间，生成功能性ComfyUI管道。

Result: 实验表明ComfySearch在复杂创意任务上显著优于现有方法，实现了更高的可执行性（通过）率、更高的解决方案率和更强的泛化能力。

Conclusion: ComfySearch通过验证引导的工作流构建有效解决了ComfyUI平台中组件空间探索和结构一致性维护的挑战，为模块化AI内容生成提供了更可靠的解决方案。
...
### [214] [Agent Drift: Quantifying Behavioral Degradation in Multi-Agent LLM Systems Over Extended Interactions](https://arxiv.org/abs/2601.04170)
*Abhishek Rath*
...
<details>
  <summary>Details</summary>
Motivation: 多智能体LLM系统已成为复杂任务分解和协作问题解决的有力架构，但其长期行为稳定性尚未得到充分研究。需要理解智能体在长时间交互序列中行为退化的现象。

Method: 提出了智能体漂移的理论框架，定义了三种表现形式：语义漂移、协调漂移和行为漂移。开发了智能体稳定性指数（ASI）作为复合度量框架，包含12个维度。通过仿真分析和理论建模研究漂移现象，并提出三种缓解策略：情景记忆整合、漂移感知路由协议和自适应行为锚定。

Result: 研究表明，未受控制的智能体漂移会导致任务完成准确率显著降低和人工干预需求增加。理论分析表明提出的缓解策略能显著减少漂移相关错误，同时保持系统吞吐量。
