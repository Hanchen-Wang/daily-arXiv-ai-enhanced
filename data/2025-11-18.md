...
TL;DR: 融合笔迹学与AI，通过分析学生手写考试试卷来量化心理压力水平，使用OCR和基于transformer的情感分析模型


<details>
  <summary>Details</summary>
Motivation: 超越传统评分系统，深入了解学生在考试期间的认知和情绪状态

Method: 集成高分辨率图像处理、TrOCR和使用RoBERTa模型的情感熵融合，通过五模型投票机制和无监督异常检测生成压力指数

Result: 开发出量化学生心理压力的创新框架

Conclusion: 该方法在学术取证领域提供了一个创新的数据驱动框架
...
TL;DR: 使用车载传感器和SVM分类器实时检测道路坑洼，在2公里道路上实现98.1%的检测准确率


<details>
  <summary>Details</summary>
Motivation: 道路状况对日常通勤至关重要，车辆数量增加需要频繁评估路况以确保交通顺畅，小裂缝可能因温度和车辆压力发展成大坑洼

Method: 利用车载传感器收集数据，采用SVM分类器进行坑洼检测

Result: 在包含26个坑洼的2公里道路上测试，达到98.1%的检测准确率
...
TL;DR: 本研究针对耕地生态系统缺乏标准化栖息地分类系统、栖息地类型覆盖不完整以及现有模型无法有效整合语义和纹理特征的问题，开发了一个包含15类耕地系统栖息地的超高分辨率遥感图像数据集，并提出了动态加权特征融合网络(DWFF-Net)，在构建的数据集上取得了0.6979的mIoU和0.8049的F1分数。
...
Result: 在构建的数据集上，提出的模型实现了0.6979的平均交并比(mIoU)和0.8049的F1分数，分别比基线网络提高了0.021和0.0161。消融研究进一步证实了多层特征融合的互补性，有效提高了田埂等微栖息地类别的IoU。

Conclusion: 本研究建立了一个基于自适应多层特征融合的耕地系统栖息地识别框架，能够以低成本实现亚米级精度的栖息地制图，为耕地景观的细粒度栖息地监测提供了强有力的技术支持。
...
TL;DR: AGENet是一个用于医学图像分割的新型框架，通过边缘感知的测地距离学习来改进边界分割精度，特别适用于标注数据有限的情况。


<details>
  <summary>Details</summary>
Motivation: 医学图像分割需要大量标注数据，这在临床应用中形成了瓶颈。现有的少样本分割方法在医学图像边界分割方面表现不佳，特别是当解剖结构相似且缺乏足够空间上下文时。

Method: AGENet结合三个主要组件：(1)边缘感知测地距离学习模块，通过迭代快速行进细化来尊重解剖边界；(2)自适应原型提取，通过空间加权聚合捕获全局结构和局部边界细节；(3)自适应参数学习，自动适应不同器官特征。

Result: 在多个医学成像数据集上的广泛实验表明，该方法在边界误差方面优于现有最先进方法，同时保持计算效率。

Conclusion: AGENet在有限标注数据的情况下实现了精确的医学图像分割，特别适合需要精确边界分割的临床应用。
...
### [5] [MovSemCL: Movement-Semantics Contrastive Learning for Trajectory Similarity](https://arxiv.org/abs/2511.12061)
...
<details>
  <summary>Details</summary>
Motivation: 现有学习方法存在三个关键限制：轨迹语义和层次建模不足、点级编码计算成本高、使用扭曲轨迹语义的物理不可行增强方法

Method: 将原始GPS轨迹转换为运动语义特征并分段，使用内外段注意力编码局部和全局轨迹模式，采用曲率引导增强策略保留信息段并屏蔽冗余段

Result: 在真实数据集上优于最先进方法，相似度搜索任务平均排名接近理想值1，启发式近似提升达20.3%，推理延迟降低达43.4%

Conclusion: MovSemCL通过层次表示和物理可行增强有效解决了轨迹相似度计算的关键挑战，在性能和效率上均有显著提升
...
TL;DR: 提出EPSegFZ网络，无需预训练即可实现少样本和零样本3D点云语义分割，通过原型增强注意力机制和语言引导原型嵌入，在S3DIS和ScanNet基准上超越现有最佳方法。


<details>
  <summary>Details</summary>
Motivation: 现有少样本3D点云分割方法依赖两阶段预训练，限制了模型灵活性和适应性；同时未能充分利用支持集中的文本注释等信息，影响了模型性能和零样本能力。

Method: 提出EPSegFZ网络，包含三个关键组件：原型增强寄存器注意力模块、基于双相对位置编码的交叉注意力机制，以及语言引导原型嵌入模块，无需预训练即可实现特征提取和查询-原型对应关系构建。

Result: 在S3DIS和ScanNet基准上分别超越现有最佳方法5.68%和3.82%，证明了方法的有效性。

Conclusion: EPSegFZ通过消除预训练依赖并充分利用文本信息，显著提升了少样本和零样本3D点云分割性能，为相关领域提供了新的解决方案。
...
TL;DR: 提出TASA框架，通过联合利用2D语义线索和3D几何推理，实现高效准确的3D场景级可操作性分割


<details>
  <summary>Details</summary>
Motivation: 现有方法主要关注对象级可操作性或将2D预测简单提升到3D，忽略了点云中的丰富几何结构信息且计算成本高

Method: 采用从粗到细的策略，包含任务感知的2D可操作性检测模块和3D可操作性细化模块，结合2D语义先验与局部3D几何信息

Result: 在SceneFun3D数据集上的实验表明，TASA在场景级可操作性分割的准确性和效率方面显著优于基线方法

Conclusion: TASA框架通过几何优化的方法有效解决了3D场景级可操作性分割的挑战，为具身智能体在复杂环境中的交互提供了有力支持
...
TL;DR: 提出了LE-CapsNet，一种轻量、增强且更准确的CapsNet变体，在CIFAR-10数据集上达到76.73%准确率，推理速度比CapsNet快4倍，在AffNIST数据集上达到94.3%准确率。


<details>
  <summary>Details</summary>
Motivation: CapsNet虽然比CNN在检测重叠类别图像和变换图像方面有优势，但存在速度慢、资源消耗大、参数多和准确率较低的问题。
...
Conclusion: LE-CapsNet在保持CapsNet优势的同时，显著提升了效率和准确性，特别是在检测仿射变换图像方面表现更鲁棒。
...
TL;DR: 本文提出了Target-Balanced Score Distillation (TBSD)方法，解决了Score Distillation Sampling (SDS)在3D资产生成中纹理优化与形状保真度之间的权衡问题。


<details>
  <summary>Details</summary>
Motivation: 传统SDS方法存在过饱和和过平滑问题，虽然通过负提示词改进，但面临纹理优化有限或纹理增益伴随形状失真的关键权衡。研究发现这种权衡主要由负提示词的使用方式决定，特别是包含目标信息的负提示词会显著提升纹理真实度但导致形状失真。

Method: 提出了Target-Balanced Score Distillation (TBSD)，将生成问题表述为多目标优化问题，并引入自适应策略来有效解决纹理与形状之间的权衡。

Result: 大量实验表明，TBSD显著优于现有最先进方法，能够生成具有高保真纹理和几何精确形状的3D资产。

Conclusion: TBSD通过平衡目标信息在负提示词中的使用，成功解决了3D生成中纹理与形状的权衡问题，实现了高质量的3D资产生成。
...
TL;DR: CompressNAS是一个受MicroNAS启发的框架，通过全局搜索秩选择来压缩深度卷积神经网络，在保持准确性的同时实现显著压缩比。


<details>
  <summary>Details</summary>
Motivation: 深度卷积神经网络在微控制器和轻量级NPU上部署困难，现有低秩张量分解方法局部选择秩且忽略全局压缩与准确性的权衡。

Method: 将秩选择作为全局搜索问题，使用快速准确性估计器评估候选分解，在内存和准确性约束下进行高效但详尽的秩探索。

Result: 在ImageNet上压缩ResNet-18达8倍且准确率下降小于4%；在COCO上压缩YOLOv5s达2倍且无准确率下降，压缩YOLOv5n达2倍且准确率下降2.5%。

Conclusion: CompressNAS框架能有效压缩深度网络，并提出了新的压缩模型家族STResNet，与其他高效模型相比具有竞争力。
...
TL;DR: 提出了AdaptFly框架，通过提示引导的测试时适应方法，无需权重更新即可调整分割模型，解决了无人机在恶劣天气和视角变化下语义分割性能下降的问题。


<details>
  <summary>Details</summary>
Motivation: 低空无人机网络需要鲁棒的语义分割作为基础，但现有分割基础模型在天气、光照和视角变化下性能快速下降。资源受限的无人机无法运行基于梯度的测试时适应，而资源充足的无人机独立适应会浪费共享经验。

Method: AdaptFly框架包含两种互补的适应模式：对于资源受限的无人机，使用轻量级token提示检索；对于资源充足的无人机，使用无梯度稀疏视觉提示优化。通过激活统计检测器触发适应，跨无人机知识池整合提示知识。

Result: 在UAVid和VDD基准测试以及真实世界无人机部署中的广泛实验表明，AdaptFly显著提高了分割精度和鲁棒性，优于静态模型和最先进的TTA基线方法。

Conclusion: AdaptFly为新兴低空经济中的弹性、通信高效感知提供了一条实用路径，通过无权重更新的提示适应实现了无人机间的协作学习。
...
TL;DR: 提出了一种基于人类视觉盲点的生物合理性掩码策略，用于从儿童视角数据中学习视觉表示和词汇-指称映射
...
Method: 使用基于掩码自编码器的视觉骨干网络，结合人类眼睛盲点知识设计新颖掩码策略，然后用于对比学习视频-文本模型

Result: 提出的生物合理性掩码策略在从跨情境和时间扩展片段学习词汇-指称映射方面至少与随机掩码同样有效

Conclusion: 生物启发的掩码策略可以替代随机掩码，为从生态有效数据中学习词汇-指称映射提供更合理的生物学解释
...
### [13] [GROVER: Graph-guided Representation of Omics and Vision with Expert Regulation for Adaptive Spatial Multi-omics Fusion](https://arxiv.org/abs/2511.11730)
...
<details>
  <summary>Details</summary>
Motivation: 空间多组学数据（转录组、蛋白质组、表观基因组）缺乏病理形态学背景，需要与组织病理图像整合以全面分析疾病组织。但不同组学、成像和空间模态之间的异质性、分辨率不匹配和生物扰动给整合带来挑战。

Method: 使用基于Kolmogorov-Arnold Networks的图卷积网络编码器捕获模态间非线性依赖关系；引入spot-feature-pair对比学习策略优化跨模态对应关系；设计动态专家路由机制自适应选择信息丰富的模态并抑制噪声输入。

Result: 在真实世界空间组学数据集上的实验表明，GROVER优于现有最先进的基线方法。

Conclusion: GROVER为多模态整合提供了一个稳健可靠的解决方案，能够有效处理空间多组学数据整合中的各种挑战。
...
### [14] [Exposing DeepFakes via Hyperspectral Domain Mapping](https://arxiv.org/abs/2511.11732)
...
Method: 提出HSI-Detect两阶段流水线：1) 从标准RGB输入重建31通道高光谱图像；2) 在高光谱域进行检测。通过扩展输入表示到更密集的光谱波段，放大RGB域中微弱或不可见的操作伪影。

Result: 在FaceForensics++数据集上的评估显示，相比仅使用RGB的基线方法，HSI-Detect取得了持续改进。

Conclusion: 光谱域映射在深度伪造检测中具有良好前景，通过分析更丰富的光谱信息可以更有效地识别伪造痕迹。
...
### [15] [Toward bilipshiz geometric models](https://arxiv.org/abs/2511.11735)
*Yonatan Sverdlov,Eitan Rosen,Nadav Dym*

Main category: cs.CV

TL;DR: 该论文研究了点云神经网络是否保持对称感知距离的双Lipschitz等价性，提出了改进方法使网络获得双Lipschitz保证，并在3D点云对应任务中验证了其优势。


<details>
  <summary>Details</summary>
Motivation: 受等变学习文献启发，研究点云神经网络是否保持自然对称感知距离的双Lipschitz等价性，以提升模型性能。

Method: 分析两种对称感知度量（Procrustes匹配度量和Hard Gromov Wasserstein距离）的双Lipschitz等价性，修改现有不变网络以获得双Lipschitz保证。

Result: 发现两种距离本身不是双Lipschitz等价的，标准不变网络也不满足双Lipschitz性质；改进后的模型在3D点云对应任务中表现更优。

Conclusion: 通过确保点云神经网络的双Lipschitz性质，可以提升其在对称感知距离下的性能，为点云处理任务提供更好的理论保证。
...
### [16] [Concept-RuleNet: Grounded Multi-Agent Neurosymbolic Reasoning in Vision Language Models](https://arxiv.org/abs/2511.11751)
*Sanchit Sinha,Guangzhi Xiong,Zhenghao He,Aidong Zhang*

Main category: cs.CV

TL;DR: Concept-RuleNet是一个多智能体神经符号系统，通过从训练图像中挖掘视觉概念来增强符号推理的可解释性，减少幻觉符号并提高预测准确性。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型虽然预测准确率高，但缺乏决策解释性，容易产生幻觉事实，特别是在分布外数据上。神经符号框架虽然结合了黑盒感知和可解释符号推理，但现有方法仅从任务标签中提取符号，缺乏视觉数据的充分基础。

Method: 1. 多模态概念生成器从训练图像子集中挖掘判别性视觉概念；2. 利用视觉概念条件化符号发现，将生成锚定在真实图像统计中；3. 大语言模型推理器将符号组合成一阶可执行规则；4. 推理时，视觉验证器量化每个符号的存在程度并触发规则执行。

Result: 在五个基准测试（包括两个医学成像任务和三个代表性不足的自然图像数据集）上，该系统将最先进的神经符号基线平均提高了5%，同时将规则中的幻觉符号出现率降低了50%。

Conclusion: Concept-RuleNet通过视觉基础的概念挖掘和符号推理，在保持透明推理的同时显著提高了神经符号系统的性能，有效减少了幻觉问题。
...
### [17] [Batch Transformer Architecture: Case of Synthetic Image Generation for Emotion Expression Facial Recognition](https://arxiv.org/abs/2511.11754)
*Stanislav Selitskiy*

Main category: cs.CV

TL;DR: 提出了一种新型的隐式稀疏Transformer架构——Batch Transformers，通过对重要维度（主成分）进行注意力机制，显著减少编码器-解码器ANN架构中的瓶颈大小。


<details>
  <summary>Details</summary>
Motivation: 传统Transformer对序列或批处理实体的全部维度进行注意力计算，导致计算效率低下。本文旨在通过关注重要维度来优化注意力机制，提高模型效率。

Method: 提出Batch Transformers架构，采用隐式稀疏风格，仅对重要维度（主成分）实施注意力机制，实现特征选择，减少编码器-解码器架构的瓶颈大小。

Result: 在面部识别任务的合成图像生成实验中，特别是在化妆和遮挡数据集上，该架构能够显著增加有限原始数据集的变异性。

Conclusion: Batch Transformers通过关注重要维度的注意力机制，有效减少了模型瓶颈，提高了数据增强能力，在有限数据集条件下表现出色。
...
### [18] [Image-POSER: Reflective RL for Multi-Expert Image Generation and Editing](https://arxiv.org/abs/2511.11780)
*Hossein Mohebbi,Mohammed Abdulrahman,Yanting Miao,Pascal Poupart,Suraj Kothawade*

Main category: cs.CV

TL;DR: Image-POSER是一个基于强化学习的文本到图像生成框架，通过动态任务分解和专家模型编排，能够处理长组合式提示词，在图像对齐度、保真度和美学质量方面优于现有基线模型。


<details>
  <summary>Details</summary>
Motivation: 现有文本到图像生成模型在处理长组合式提示词时表现不佳，无法满足创意工作流程的需求。

Method: 采用反射式强化学习框架，编排预训练文本到图像和图像到图像专家模型，通过动态任务分解处理长提示词，并使用视觉语言模型批评器提供结构化反馈。

Result: 在行业标准和自定义基准测试中，Image-POSER在图像对齐度、保真度和美学质量方面均优于基线模型，并在人类评估中持续获得偏好。
...
### [19] [SOTFormer: A Minimal Transformer for Unified Object Tracking and Trajectory Prediction](https://arxiv.org/abs/2511.11824)
...
<details>
  <summary>Details</summary>
Motivation: 解决目标跟踪和短期运动预测在遮挡、尺度变化和时间漂移下的挑战，提升时序一致性以实现实时感知。

Method: 采用基于真值初始化的记忆机制和燃烧锚点损失来稳定初始化，使用单一轻量级时序注意力层跨帧优化嵌入表示。

Result: 在Mini-LaSOT基准测试中达到76.3 AUC和53.7 FPS（4.3GB VRAM），在快速运动、尺度变化和遮挡情况下优于TrackFormer和MOTRv2等Transformer基线。

Conclusion: SOTFormer通过最小化恒定内存的时序Transformer设计，成功统一了目标检测、跟踪和短期轨迹预测，实现了高性能的实时感知。
...
### [20] [MP-GFormer: A 3D-Geometry-Aware Dynamic Graph Transformer Approach for Machining Process Planning](https://arxiv.org/abs/2511.11837)
...
<details>
  <summary>Details</summary>
Motivation: 现有动态图学习方法虽然能捕捉加工过程中的时空依赖关系，但未能融入零件的三维几何信息，缺乏领域感知能力。

Method: 利用StereoLithography表面网格表示每次加工操作后的零件3D几何，通过注意力机制将演化的3D几何表示集成到动态图学习中。

Result: 在合成数据集上的评估显示，该方法在主操作和子操作预测的准确率上分别比最先进方法提高了24%和36%。

Conclusion: MP-GFormer通过集成3D几何信息显著提升了加工操作序列预测的准确性，证明了3D几何感知在加工过程规划中的重要性。
...
### [21] [Defending Unauthorized Model Merging via Dual-Stage Weight Protection](https://arxiv.org/abs/2511.11851)
*Wei-Jia Chen,Min-Yen Tsai,Cheng-Yi Lee,Chia-Mu Yu*

Main category: cs.CV

TL;DR: MergeGuard是一个双阶段权重保护框架，通过破坏模型合并兼容性来防止未经授权的模型合并，同时保持原始模型的性能。
...
Method: 第一阶段通过L2正则化优化重新分配任务相关信息到各层；第二阶段注入结构化扰动使任务子空间错位，破坏损失景观中的曲率兼容性。

Result: 在视觉和语言模型上的广泛实验表明，MergeGuard能将合并模型准确率降低高达90%，而保护模型的性能损失不到1.5%。

Conclusion: MergeGuard通过重塑模型参数几何结构，使合并模型崩溃为破坏性干扰，同时保护模型保持完全功能，有效防止未经授权的模型合并。
...
### [22] [FocusSDF: Boundary-Aware Learning for Medical Image Segmentation via Signed Distance Supervision](https://arxiv.org/abs/2511.11864)
*Muzammal Shafique,Nasir Rahim,Jamil Ahmad,Mohammad Siadat,Khalid Malik,Ghaus Malik*

Main category: cs.CV

TL;DR: 提出FocusSDF损失函数，基于符号距离函数，通过自适应分配边界区域更高权重来改善医学图像分割中的边界保留问题。


<details>
  <summary>Details</summary>
Motivation: 医学图像分割中大多数模型未显式编码边界信息，导致边界保留成为持续挑战。

Method: 引入基于符号距离函数的FocusSDF损失函数，自适应地为靠近病变或器官边界的像素分配更高权重，使网络专注于边界区域。

Result: 在多种医学图像分割任务上的实验结果表明，FocusSDF在性能上持续优于现有的基于距离变换的损失函数。

Conclusion: FocusSDF是一种有效的边界感知损失函数，能显著提升医学图像分割的边界保留能力。
...
### [23] [Lacking Data? No worries! How synthetic images can alleviate image scarcity in wildlife surveys: a case study with muskox (Ovibos moschatus)](https://arxiv.org/abs/2511.11882)
*Simon Durand,Samuel Foucher,Alexandre Delplanque,Joëlle Taillon,Jérôme Théau*

Main category: cs.CV

TL;DR: 该研究探讨了使用合成图像补充有限训练数据，以提高在零样本和少样本设置下对麝牛的检测性能。研究发现添加合成图像能改善检测效果，但存在收益递减现象。


<details>
  <summary>Details</summary>
Motivation: 传统野生动物调查方法资源密集且受限于后勤挑战，而深度学习目标检测模型在稀疏分布物种（如麝牛）的小数据集上效果有限。

Method: 比较了在真实图像上训练的基线模型与5个零样本和5个少样本模型，这些模型在训练集中逐步添加了更多合成图像。

Result: 零样本模型中添加合成图像提高了检测性能；少样本模型中结合真实和合成图像获得了更好的召回率和略高的准确率，但改进不具统计显著性。

Conclusion: 合成图像在数据稀缺时训练准确目标检测模型具有潜力，可用于启动无真实数据的模型并随时间推移使用真实图像进行优化。
...
### [24] [Advancing Annotat3D with Harpia: A CUDA-Accelerated Library For Large-Scale Volumetric Data Segmentation](https://arxiv.org/abs/2511.11890)
*Camila Machado de Araujo,Egon P. B. S. Borges,Ricardo Marcelo Canteiro Grangeiro,Allan Pinto*

Main category: cs.CV

TL;DR: 开发了基于CUDA的Harpia处理库，为Annotat3D提供可扩展的交互式3D分割工作流，支持超出单GPU内存容量的大型数据集处理。


<details>
  <summary>Details</summary>
Motivation: 高分辨率体积成像技术生成的大型数据集对现有处理工具的效率和交互性提出了挑战，需要支持高性能计算和远程访问环境的新解决方案。

Method: Harpia库采用严格内存控制、原生分块执行和GPU加速的过滤、标注和量化工具套件，支持在共享HPC基础设施中的协作工作流。

Result: 相比NVIDIA cuCIM和scikit-image等框架，在处理速度、内存效率和可扩展性方面取得显著改进，能够可靠处理超出单GPU内存容量的数据集。

Conclusion: 该系统结合交互式人机界面和高效的GPU资源管理，特别适用于共享HPC基础设施中的协作科学成像工作流。
...
### [25] [Prompt Triage: Structured Optimization Enhances Vision-Language Model Performance on Medical Imaging Benchmarks](https://arxiv.org/abs/2511.11898)
*Arnav Singhvi,Vasiliki Bikia,Asad Aali,Akshay Chaudhari,Roxana Daneshjou*

Main category: cs.CV

TL;DR: 本文提出使用DSPy框架对医学视觉语言模型进行自动化提示优化，在放射学、胃肠病学和皮肤病学等5个医学成像任务中，相比零样本提示基线实现了53%的中位数相对改进。


<details>
  <summary>Details</summary>
Motivation: 视觉语言基础模型在医学基准测试中表现不佳，传统方法如模型微调需要大量领域数据和计算资源，手动提示工程难以推广且对医疗机构不友好，因此需要开发不依赖人工设计提示的自动化优化方法。

Method: 采用DSPy框架进行结构化自动提示优化，在5个医学成像任务中评估10个开源视觉语言模型，使用4种提示优化技术构建提示管道。

Result: 优化后的管道相比零样本提示基线实现了53%的中位数相对改进，在零样本性能较低的任务中改进幅度达300%至3400%。

Conclusion: 自动化提示优化在医学AI系统中具有巨大潜力，能够显著提升临床图像解释的准确性，减少对提示设计的依赖，让临床医生专注于患者护理和临床决策。
...
### [26] [PI-NAIM: Path-Integrated Neural Adaptive Imputation Model](https://arxiv.org/abs/2511.11908)
*Afifa Khaled,Ebrahim Hamid Sumiea*

Main category: cs.CV

TL;DR: PI-NAIM是一个双路径架构，根据缺失模式的复杂性动态路由样本到优化的插补方法，结合统计方法和神经网络，在医学影像和多模态临床数据中实现最先进的缺失值插补性能。
...
Method: 提出双路径架构，包括：(1)智能路径路由，将低缺失样本导向高效统计插补(MICE)，复杂模式导向神经网络(GAIN)；(2)跨路径注意力融合，利用缺失感知嵌入智能结合两个分支；(3)端到端联合优化插补精度和下游任务性能。

Result: 在MIMIC-III和多模态基准测试中达到最先进性能，RMSE为0.108（基线为0.119-0.152），死亡率预测AUROC为0.812。

Conclusion: PI-NAIM的模块化设计可无缝集成到处理不完整传感器测量、缺失模态或损坏输入的视觉流程中，为现实场景提供统一解决方案。
...
### [27] [Seeing the Forest and the Trees: Query-Aware Tokenizer for Long-Video Multimodal Language Models](https://arxiv.org/abs/2511.11910)
...
TL;DR: QTSplus是一个轻量级但强大的视觉token选择模块，通过动态选择与文本查询最相关的视觉证据来解决长视频理解中的计算成本问题，在保持精度的同时显著压缩视觉流和降低延迟。


<details>
  <summary>Details</summary>
Motivation: 解决多模态大语言模型在长视频理解中的挑战，主要问题是视觉token数量随视频长度线性增长，导致注意力成本、内存和延迟爆炸式增加。

Method: QTSplus通过(i)交叉注意力对视觉token评分，(ii)基于查询复杂度预测实例特定的保留预算，(iii)在训练时使用可微分直通估计器选择Top-n token，推理时使用硬门控。还包含一个小型重编码器来保持时间顺序。

Result: 在Qwen2.5-VL中集成QTSplus，视觉流压缩高达89%，端到端延迟降低28%。在8个长视频理解基准测试中，整体精度接近原始模型，在TempCompass方向上提升20.5分，顺序精度提升5.6分。

Conclusion: QTSplus是一种有效、通用的机制，可以在保持任务相关证据的同时，将MLLMs扩展到现实世界长视频场景。
...
### [28] [From Events to Clarity: The Event-Guided Diffusion Framework for Dehazing](https://arxiv.org/abs/2511.11944)
*Ling Wang,Yunfan Lu,Wenzong Ma,Huizai Yao,Pengteng Li,Hui Xiong*

Main category: cs.CV

TL;DR: 首次使用事件相机进行图像去雾，提出事件引导的扩散模型，利用事件相机的高动态范围特性来重建清晰图像。


<details>
  <summary>Details</summary>
Motivation: 传统RGB图像在雾霾条件下动态范围有限，去雾任务仍然具有挑战性，容易丢失结构和光照细节。事件相机具有更高的动态范围和微秒级延迟，更适合雾霾场景。

Method: 提出事件引导的扩散模型，设计事件引导模块将稀疏的HDR事件特征映射到扩散潜在空间，为生成过程提供精确的结构指导。

Result: 在两个基准测试和自建的重雾霾无人机数据集上取得了最先进的结果。

Conclusion: 事件相机为图像去雾提供了新的解决方案，通过有效利用事件数据的HDR信息，能够生成更真实、结构更清晰的去雾图像。
...
### [29] [Evaluation of Attention Mechanisms in U-Net Architectures for Semantic Segmentation of Brazilian Rock Art Petroglyphs](https://arxiv.org/abs/2511.11959)
*Leonardi Melo,Luís Gustavo,Dimmy Magalhães,Lucciani Vieira,Mauro Araújo*

Main category: cs.CV

TL;DR: 比较三种基于U-Net的架构在巴西岩画语义分割中的性能，其中Attention-Residual BEGL-UNet表现最佳，Dice Score达到0.710。


<details>
  <summary>Details</summary>
Motivation: 研究旨在评估不同U-Net架构在考古岩画数字化保护中的有效性，特别是关注注意力机制对分割性能的提升。

Method: 比较三种U-Net变体：(1)基础BEGL-UNet；(2)带残差块和门控注意力机制的Attention-Residual BEGL-UNet；(3)基于CBAM的空间通道注意力BEGL-UNet。所有模型使用结合二元交叉熵和高斯边缘增强的BEGL损失函数，在巴西考古遗址图像上进行5折交叉验证。

Result: Attention-Residual BEGL-UNet表现最佳，Dice Score为0.710，验证损失0.067，召回率0.854；Spatial Channel Attention BEGL-UNet表现相当，DSC为0.707，召回率0.857；基础BEGL-UNet的DSC为0.690。注意力机制相比基线提升了2.5-2.9%的Dice Score。

Conclusion: 注意力机制在考古遗产数字化保护中具有显著效果，特别是Attention-Residual BEGL-UNet架构在岩画语义分割任务中表现出最佳性能。
...
### [30] [From Classification to Cross-Modal Understanding: Leveraging Vision-Language Models for Fine-Grained Renal Pathology](https://arxiv.org/abs/2511.11984)
...
TL;DR: 该研究系统评估了视觉语言模型在细粒度肾小球亚型分类中的表现，发现在数据受限的临床场景下，病理学专业化的视觉语言骨干网络结合标准微调是最有效的起点。


<details>
  <summary>Details</summary>
Motivation: 细粒度肾小球亚型分类对肾脏活检解读至关重要，但临床上有价值的标签稀缺且难以获取。现有计算方法主要在全监督下进行粗粒度疾病分类，不清楚视觉语言模型如何在数据约束下适应临床有意义的亚型分类。

Method: 将细粒度肾小球亚型分类建模为临床现实的少样本问题，系统评估病理学专业化和通用视觉语言模型，分析分类性能（准确率、AUC、F1）和学习表示的几何特性，包括图像-文本嵌入对齐和肾小球亚型可分性。

Result: 病理学专业化视觉语言骨干网络结合标准微调是最有效起点，即使每个肾小球亚型只有4-8个标记样本，这些模型也能开始捕捉区别并显示出显著的区分度和校准提升。正负样本区分与图像-文本对齐同等重要。
...
### [31] [BeyondFacial: Identity-Preserving Personalized Generation Beyond Facial Close-ups](https://arxiv.org/abs/2511.11989)
...
TL;DR: 提出了一种解决身份保持个性化生成中过度依赖面部特写问题的方法，通过双线推理管道、身份自适应融合和身份聚合预置模块，实现了身份保真度与场景语义创作的协同优化。
...
Method: 设计了身份-语义分离的双线推理管道，提出身份自适应融合策略将身份-语义融合推迟到噪声预测阶段，并引入身份聚合预置模块来聚合身份信息并替换随机初始化。

Result: 实验验证该方法在超越面部特写的IPPG任务中实现稳定有效性能，无需手动遮罩或微调即可高效生成，可作为即插即用组件快速部署到现有IPPG框架中。
...
### [32] [Dynamic Parameter Optimization for Highly Transferable Transformation-Based Attacks](https://arxiv.org/abs/2511.11993)
*Jiaming Liang,Chi-Man Pun*

Main category: cs.CV

TL;DR: 本文提出动态参数优化方法解决变换攻击中的参数优化盲点，显著提升迁移攻击效果。


<details>
  <summary>Details</summary>
Motivation: 现有变换攻击存在参数优化盲点：仅考虑低迭代设置、对不同代理模型使用统一参数、传统网格搜索复杂度高，限制了迁移攻击的潜力。

Method: 基于经验研究提出同心衰减模型解释迁移性动态模式，并设计基于上升-下降模式的动态参数优化方法，将复杂度从O(m^n)降至O(nlogm)。

Result: 在不同代理模型、迭代次数和任务上的综合实验表明，DPO能显著提升迁移攻击效果。

Conclusion: 动态参数优化方法有效解决了变换攻击中的参数优化问题，显著提升了迁移攻击性能。
...
### [33] [LithoSeg: A Coarse-to-Fine Framework for High-Precision Lithography Segmentation](https://arxiv.org/abs/2511.12005)
*Xinyu He,Botong Zhao,Bingbing Li,Shujing Lyu,Jiwei Shen,Yue Lu*

Main category: cs.CV

TL;DR: 提出了LithoSeg，一种用于光刻SEM图像分割的粗到精网络，通过人机协同引导的SAM实现粗分割，然后通过1D回归进行精细分割，在精度和鲁棒性方面优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 光刻SEM图像的精确分割和测量对于半导体制造工艺控制、器件性能优化和良率提升至关重要，但现有方法缺乏足够的精度和鲁棒性。

Method: 采用粗到精两阶段方法：粗阶段使用人机协同引导的SAM实现鲁棒分割；精阶段将2D分割转化为1D回归问题，使用粗掩模采样凹槽法向剖面，通过轻量级MLP进行点级细化。

Result: LithoSeg在分割精度和计量精度方面均优于先前方法，同时需要更少的监督，为实际应用提供了良好前景。

Conclusion: LithoSeg通过创新的粗到精分割策略，实现了高精度、鲁棒的光刻SEM图像分割，显著提升了半导体制造中的工艺控制能力。
...
### [34] [Uncertainty-Guided Selective Adaptation Enables Cross-Platform Predictive Fluorescence Microscopy](https://arxiv.org/abs/2511.12006)
...
TL;DR: 提出SIT-ADDA-Auto框架，仅适配早期卷积层而非整个网络，实现显微镜图像的无标签跨域适应，提高重建和分割性能。


<details>
  <summary>Details</summary>
Motivation: 深度学习在显微镜应用中面临跨仪器和采集设置时的性能下降问题，传统对抗域适应方法会破坏已学习的语义表示。

Method: 引入SIT-ADDA-Auto框架，仅适配浅层网络层并冻结深层，结合预测不确定性自动选择适配深度，无需目标域标签。

Result: 在曝光、光照变化、跨仪器迁移和多种染色条件下，SIT-ADDA在重建和下游分割任务上优于全编码器适配和非对抗基线方法。

Conclusion: 为显微镜领域的无标签适应提供了设计原则，并减少了语义特征的漂移。
...
### [35] [Enhancing Road Safety Through Multi-Camera Image Segmentation with Post-Encroachment Time Analysis](https://arxiv.org/abs/2511.12018)
*Shounak Ray Chaudhuri,Arash Jahangiri,Christopher Paolini*

Main category: cs.CV

TL;DR: 提出了一种基于多摄像头计算机视觉的实时交通安全评估框架，通过计算后侵占时间(PET)来分析信号灯交叉口的安全状况，在边缘设备上实现高精度实时处理。


<details>
  <summary>Details</summary>
Motivation: 传统基于事故的交通安全分析存在数据稀疏性和延迟问题，需要开发实时、高分辨率的评估方法来改善交叉口安全。

Method: 使用四个同步摄像头提供连续视觉覆盖，在NVIDIA Jetson AGX Xavier设备上使用YOLOv11分割进行车辆检测，通过单应性矩阵将车辆多边形转换为统一的鸟瞰图，并开发了像素级PET算法进行精细危险可视化。

Result: 该系统能够以亚秒级精度识别高风险区域，在边缘设备上实现实时处理，平均帧率为2.68 FPS，生成800x800像素的对数热图，精度达到3.3平方厘米。
...
### [36] [LIHE: Linguistic Instance-Split Hyperbolic-Euclidean Framework for Generalized Weakly-Supervised Referring Expression Comprehension](https://arxiv.org/abs/2511.12020)
*Xianglong Shi,Silin Cheng,Sirui Zhao,Yunhan Jiang,Enhong Chen,Yang Liu,Sebastien Ourselin*

Main category: cs.CV

TL;DR: 提出了LIHE框架解决弱监督广义指称表达理解任务，通过两阶段方法处理零个或多个目标的情况，结合双曲和欧几里得几何来防止语义崩溃。


<details>
  <summary>Details</summary>
Motivation: 现有弱监督指称表达理解方法受限于一对一映射假设，无法处理现实场景中表达对应零个或多个目标的情况。

Method: LIHE框架包含两个阶段：指称解耦（预测目标数量并分解复杂表达）和指称定位（使用HEMix混合相似度模块结合欧几里得和双曲几何进行定位）。

Result: 在gRefCOCO和Ref-ZOM上建立了首个有效的弱监督WGREC基线，HEMix在标准REC基准上提升IoU@0.5达2.5%。

Conclusion: LIHE框架成功解决了弱监督广义指称表达理解任务，通过混合几何方法有效防止了语义崩溃问题。
...
### [37] [Null-Space Diffusion Distillation for Efficient Photorealistic Lensless Imaging](https://arxiv.org/abs/2511.12024)
*Jose Reinaldo Cunha Santos A V Silva Neto,Hodaka Kawachi,Yasushi Yagi,Tomoya Nakamura*

Main category: cs.CV

TL;DR: 提出NSDD方法，通过蒸馏迭代DDNM+求解器的零空间组件，在无配对监督下实现快速、逼真的无透镜图像重建，保持测量一致性且计算效率高。


<details>
  <summary>Details</summary>
Motivation: 现有无透镜相机重建方法依赖配对监督会导致域不匹配偏差，而通用扩散先验在噪声、高度复用和病态的无透镜反卷积设置中不稳定。

Method: 引入零空间扩散蒸馏(NSDD)：单次前向的学生网络，蒸馏迭代DDNM+求解器的零空间组件，以无透镜测量和范围空间锚点为条件。

Result: 在Lensless-FFHQ和PhlatCam数据集上，NSDD是第二快的方法（仅次于Wiener），达到接近教师模型的感知质量（第二好的LPIPS，低于DDNM+），优于DPS和经典凸基线方法。

Conclusion: NSDD为实现快速、无真实值监督、逼真的无透镜成像提供了一条实用路径。
...
### [38] [Bridging Vision and Language for Robust Context-Aware Surgical Point Tracking: The VL-SurgPT Dataset and Benchmark](https://arxiv.org/abs/2511.12026)
...
<details>
  <summary>Details</summary>
Motivation: 现有手术追踪数据集缺乏语义上下文，难以理解追踪失败机制。复杂手术环境中的烟雾遮挡、镜面反射和组织变形等条件使精确点追踪面临挑战。

Method: 引入VL-SurgPT数据集，包含754个组织追踪视频(17,171个标注点)和154个器械追踪视频(7种器械类型)。提出TG-SurgPT文本引导追踪方法，利用点状态语义描述提升追踪性能。

Result: 实验结果表明，结合点状态信息显著提高了追踪准确性和可靠性，特别是在视觉恶劣条件下，传统纯视觉方法表现不佳的场景中。
...
### [39] [GCAgent: Long-Video Understanding via Schematic and Narrative Episodic Memory](https://arxiv.org/abs/2511.12027)
*Jeong Hun Yeo,Sangyun Chung,Sungjune Park,Dae Hoe Kim,Jinyoung Moon,Yong Man Ro*

Main category: cs.CV

TL;DR: GCAgent是一个全局上下文感知代理框架，通过创新的图式和叙事情景记忆来解决长视频理解中的长期依赖问题，在Video-MME基准测试中取得了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 解决多模态大语言模型在长视频理解中的局限性，包括固有的token限制和捕获长期时间依赖的复杂性，现有方法难以捕捉全局上下文和复杂事件关系。

Method: 提出GCAgent框架，核心创新是图式和叙事情景记忆，将事件及其因果和时间关系结构化建模为简洁有序的上下文，通过感知-行动-反思的多阶段循环和记忆管理器进行上下文感知推理。
...
### [40] [VPHO: Joint Visual-Physical Cue Learning and Aggregation for Hand-Object Pose Estimation](https://arxiv.org/abs/2511.12030)
...
<details>
  <summary>Details</summary>
Motivation: 现有方法主要依赖视觉线索，常违反物理约束（如穿透或非接触），而引入物理推理的方法通常依赖后优化或不可微物理引擎，影响视觉一致性和端到端训练能力。

Method: 1) 联合视觉-物理线索学习：训练模型提取2D视觉线索和3D物理线索；2) 候选姿态聚合：通过扩散生成多个候选姿态，利用视觉和物理预测进行聚合，得到最终估计。

Result: 在姿态准确性和物理合理性方面显著优于现有最先进方法。

Conclusion: 该框架成功整合了视觉和物理线索，实现了更准确和物理合理的手-物体3D姿态估计。
...
### [41] [Improved Masked Image Generation with Knowledge-Augmented Token Representations](https://arxiv.org/abs/2511.12032)
*Guotao Liang,Baoquan Zhang,Zhiyuan Wen,Zihao Han,Yunming Ye*

Main category: cs.CV

TL;DR: KA-MIG框架通过引入token级语义依赖的先验知识来增强掩码图像生成，提出了三种知识图谱（共现图、语义相似图、位置-标记不兼容图）来学习更丰富的表示，从而提升生成质量。


<details>
  <summary>Details</summary>
Motivation: 现有掩码图像生成方法仅依靠模型自身学习视觉标记序列的语义依赖关系，但由于单个标记缺乏明确语义且序列通常较长，直接从数据中学习这些依赖关系具有挑战性。

Method: 提出KA-MIG框架，构建三种先验知识图谱（共现图、语义相似图、位置-标记不兼容图），设计图感知编码器学习标记和位置感知表示，并通过轻量级融合机制将这些表示集成到现有MIG方法中。

Result: 实验结果表明，该方法在ImageNet上的类条件图像生成任务中优于现有的MIG方法。

Conclusion: 通过引入先验知识，KA-MIG有效增强了模型捕获语义依赖关系的能力，从而提高了生成质量。
...
### [42] [Calibrated Multimodal Representation Learning with Missing Modalities](https://arxiv.org/abs/2511.12034)
*Xiaohao Liu,Xiaobo Xia,Jiaheng Wei,Shuo Yang,Xiu Su,See-Kiong Ng,Tat-Seng Chua*

Main category: cs.CV

TL;DR: 提出CalMRL方法解决多模态表示学习中模态缺失导致的锚点偏移问题，通过表示级插补和双步学习实现校准对齐


<details>
  <summary>Details</summary>
Motivation: 现有方法需要所有模态都存在才能实现跨模态对齐，无法有效利用普遍存在模态缺失的数据集

Method: 利用模态先验和内在联系进行表示级插补，采用双步学习方法结合后验分布的闭式解

Result: 理论验证了锚点偏移的缓解和收敛性，实验证明CalMRL的优越性

Conclusion: CalMRL为吸收模态缺失数据提供了新灵活性，解决了传统方法无法处理的问题
...
### [43] [SRSplat: Feed-Forward Super-Resolution Gaussian Splatting from Sparse Multi-View Images](https://arxiv.org/abs/2511.12040)
...
<details>
  <summary>Details</summary>
Motivation: 解决现有方法在从稀疏低分辨率图像进行3D重建时无法恢复精细纹理细节的问题，因为低分辨率输入缺乏高频信息。

Method: 使用多模态大语言模型和扩散模型为每个场景构建参考图库，引入参考引导特征增强模块对齐和融合低分辨率输入与参考图像的特征，训练解码器预测高斯基元，并通过纹理感知密度控制自适应调整高斯密度。
...
### [44] [FedSDA: Federated Stain Distribution Alignment for Non-IID Histopathological Image Classification](https://arxiv.org/abs/2511.12044)
*Cheng-Chang Tsai,Kai-Wen Cheng,Chun-Shien Lu*

Main category: cs.CV

TL;DR: 提出FedSDA方法，通过扩散模型和染色分离技术对齐联邦学习中非IID组织病理学图像的染色分布，解决数据分布偏移问题。


<details>
  <summary>Details</summary>
Motivation: 联邦学习中非IID（非独立同分布）数据特别是组织病理学图像的特征分布偏移问题严重阻碍了其应用，需要从数据分布角度解决这一挑战。

Method: 基于扩散模型拟合数据分布的能力，利用染色分离提取关键特征，提出FedSDA方法在联邦学习框架下将各客户端的染色分布与目标分布对齐。

Result: 实验结果表明FedSDA不仅能有效改善关注客户端模型更新差异的基线方法，还优于从数据分布角度解决非IID问题的基线方法。

Conclusion: FedSDA为计算病理学社区提供了有价值且实用的见解，能够有效缓解联邦学习中非IID组织病理学图像的分布偏移问题。
...
### [45] [DCMM-Transformer: Degree-Corrected Mixed-Membership Attention for Medical Imaging](https://arxiv.org/abs/2511.12047)
*Huimin Cheng,Xiaowei Yu,Shushan Wu,Luyang Fang,Chao Cao,Jing Zhang,Tianming Liu,Dajiang Zhu,Wenxuan Zhong,Ping Ma*

Main category: cs.CV

TL;DR: DCMM-Transformer是一种用于医学图像分析的ViT架构，通过引入度校正混合成员模型作为自注意力的加性偏置，解决了现有方法无法有效利用医学图像中潜在解剖结构的问题。


<details>
  <summary>Details</summary>
Motivation: 医学图像中存在潜在的解剖分组（如器官、组织和病理区域），但标准的Vision Transformers无法有效利用这些结构。现有方法如SBM-Transformer存在不可微分性、训练不稳定性和无法建模复杂社区结构的问题。

Method: 提出DCMM-Transformer，将度校正混合成员模型作为自注意力的加性偏置引入，而不是依赖乘法掩码和二元采样，实现了完全可微分和可解释的社区结构与度异质性建模。

Result: 在包括脑部、胸部、乳腺和眼部等多种医学成像数据集上的综合实验表明，该方法具有优越的性能和泛化能力。

Conclusion: 所学习的群组结构和结构化注意力调制显著增强了可解释性，产生了具有解剖意义和语义连贯性的注意力图。
...
### [46] [DeiTFake: Deepfake Detection Model using DeiT Multi-Stage Training](https://arxiv.org/abs/2511.12048)
*Saksham Kumar,Ashish Singh,Srinivasarao Thota,Sunil Kumar Singh,Chandan Kumar*

Main category: cs.CV

TL;DR: 提出DeiTFake方法，使用DeiT-based transformer和两阶段渐进式训练策略进行深度伪造检测，在OpenForensics数据集上达到99.22%准确率和0.9997 AUROC。
...
Method: 采用基于DeiT的transformer架构和两阶段渐进训练策略：第一阶段使用标准增强的迁移学习，第二阶段使用高级仿射和深度伪造特定增强进行微调。
...
### [47] [UniABG: Unified Adversarial View Bridging and Graph Correspondence for Unsupervised Cross-View Geo-Localization](https://arxiv.org/abs/2511.12054)
*Cuiqun Chen,Qi Chen,Bin Yang,Xingyi Zhang*

Main category: cs.CV

TL;DR: UniABG是一个新颖的双阶段无监督跨视角地理定位框架，通过对抗性视角桥接和图基对应校准解决跨视角域差距问题，在无监督设置下实现了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 解决跨视角地理定位中监督方法依赖大量配对标注的限制，以及无监督方法因跨视角域差距导致伪标签噪声的问题。

Method: 提出双阶段框架：1）视图感知对抗桥接(VAAB)建模视图不变特征增强伪标签鲁棒性；2）异构图过滤校准(HGFC)构建双视图间结构图精化跨视角关联。
...
### [48] [PipeDiT: Accelerating Diffusion Transformers in Video Generation with Task Pipelining and Model Decoupling](https://arxiv.org/abs/2511.12056)
*Sijie Wang,Qiang Wang,Shaohuai Shi*

Main category: cs.CV

TL;DR: PipeDiT是一个用于加速视频生成的流水线框架，通过序列并行、模块解耦和注意力协同处理三项创新技术，在保持生成质量的同时显著提升推理速度。


<details>
  <summary>Details</summary>
Motivation: 基于扩散变换器(DiT)的视频生成模型虽然功能强大，但实际部署时面临推理速度慢和内存消耗高的问题，阻碍了其实际应用。

Method: 1) 为序列并行设计流水线算法(PipeSP)，实现潜在生成计算与多GPU通信的流水线化；2) 提出DeDiVAE将扩散模块与VAE模块解耦到不同GPU组，实现执行流水线化；3) 提出注意力协同处理(Aco)方法优化VAE组GPU资源利用。

Result: 在OpenSoraPlan和HunyuanVideo两个开源视频生成框架上的实验表明，在常见分辨率和时间步配置下，PipeDiT实现了1.06倍到4.02倍的加速比。

Conclusion: PipeDiT框架通过创新的流水线技术有效解决了DiT视频生成模型的推理延迟和内存消耗问题，显著提升了实际部署效率。
...
TL;DR: DCA-LUT是一种基于深度学习的紫色边缘去除框架，通过色度感知坐标变换模块分离紫色边缘到专用维度，并使用5D查找表进行颜色校正，在合成和真实数据集上达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 紫色边缘是由镜头纵向色差引起的持久伪影，传统解决方案依赖昂贵的复消色差镜头硬件和手工特征提取，忽略了数据驱动方法。

Method: 提出色度感知坐标变换模块学习图像自适应颜色空间，将紫色边缘分离到专用维度；使用学习的紫色边缘通道指导亮度通道恢复；通过5D查找表进行高效非线性颜色映射。

Result: 在合成和真实数据集上的广泛实验表明，该方法在紫色边缘去除方面达到了最先进的性能。

Conclusion: DCA-LUT是第一个用于紫色边缘去除的深度学习框架，通过物理启发的色度分离和高效的颜色映射，有效解决了镜头色差引起的紫色边缘问题。
...
TL;DR: VAEmotionLLM是一个两阶段框架，通过有限音频预训练让视觉语言模型获得听觉能力，并增强跨模态情感理解。


<details>
  <summary>Details</summary>
Motivation: 现有音频-视觉语言模型通常需要大规模音频预训练，限制了可扩展性；同时大多数工作忽视艺术作品有意表达的情感。

Method: 第一阶段VG-Align通过对齐同步音视频剪辑的下一个token分布，将冻结的视觉通路蒸馏到新的音频通路；第二阶段通过轻量级EmoAdapter注入情感敏感残差并应用情感监督。

Result: 在ArtEmoBenchmark上达到最先进结果，优于音频、视觉和音频-视觉基线方法。

Conclusion: 提出的组件具有互补性，能够有效提升跨模态情感理解能力。
...
TL;DR: 提出基于多模态提示驱动的点云量化框架，利用文本嵌入作为原型先验，通过双约束量化空间整合视觉和原型特征，在点云分析任务中表现优异


<details>
  <summary>Details</summary>
Motivation: 当前基于可训练向量或聚类质心的原型方法在代表性和可解释性方面存在不足，需要改进代码本设计

Method: 使用预训练模型的文本嵌入作为原型先验，通过多模态提示自适应优化原型，引入双约束量化空间（紧凑性和分离性正则化），采用Gumbel-Softmax松弛实现可微分离散化

Result: 在ModelNet40和ScanObjectNN数据集上的广泛实验证明了方法的优越有效性

Conclusion: 提出的多模态提示驱动量化框架能够有效统一异构表示，生成同时编码几何和语义信息的混合表示，显著提升了点云分析性能
...
TL;DR: 提出了一种基于改进ResNet-101架构和概率推理的多标签图像分类方法，在COCO-2014数据集上实现了0.794 mAP，优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 多标签图像分类在计算机视觉应用中具有重要价值，但传统方法难以有效处理标签依赖性和不确定性。
...
TL;DR: SemanticStitch是一个基于深度学习的图像拼接框架，通过引入前景对象的语义先验来保持其完整性并增强视觉连贯性，显著改善了拼接质量。
...
Method: 提出SemanticStitch框架，包含新颖的损失函数强调显著对象的语义完整性，并构建了两个专门的现实世界数据集进行评估。

Result: 实验结果表明，该方法相比传统技术有显著改进，为实际应用提供了有力支持。

Conclusion: SemanticStitch通过语义先验有效解决了图像拼接中的前景对象完整性问题，提升了拼接质量。
...
TL;DR: 提出了一种分层分组提示调优方法，通过层分组共享提示和根提示生成子提示的机制，减少层间过度独立调优，缓解持续学习中的灾难性遗忘问题。


<details>
  <summary>Details</summary>
Motivation: 现有基于提示的持续学习方法在各层独立添加任务特定提示，虽然灵活性高但可能导致某些层不必要的更新，容易覆盖对先前任务重要的特征表示，增加灾难性遗忘风险。

Method: 分层分组提示调优：1) 同组层共享大致相同的提示，通过位置编码调整；2) 使用单一任务特定根提示学习生成各层组的子提示，增强子提示间的协同性。

Result: 在四个基准测试上的广泛实验表明，该方法相比多个最先进方法取得了优越性能。

Conclusion: 所提出的分层分组提示调优方法通过增强模型稳定性，有效缓解了持续学习中的灾难性遗忘问题。
...
TL;DR: PACE是首个针对SNN和事件视觉的数据集蒸馏框架，通过压缩大型训练数据集为紧凑合成数据，实现快速SNN训练，减少50倍训练时间和6000倍存储成本。


<details>
  <summary>Details</summary>
Motivation: SNN由于时间编码导致训练成本高昂，限制了实际部署。需要开发能够降低SNN训练成本的方法。

Method: PACE包含两个核心模块：ST-DSM使用残差膜电位来稠化基于尖峰的特征，并执行精细的时空幅度和相位匹配；PEQ-N提供即插即用的直通概率整数量化器。

Result: 在DVS-Gesture、CIFAR10-DVS和N-MNIST数据集上，PACE优于现有核心集选择和数据集蒸馏基线，在N-MNIST上达到84.4%准确率，约为完整训练集性能的85%。

Conclusion: PACE能够生成紧凑的替代数据集，实现分钟级SNN训练和高效边缘部署，显著降低了SNN的训练和存储成本。
...
TL;DR: SpikeNM是一个面向SNN的半结构化N:M剪枝框架，通过M路基对数参数化和可微分top-k采样器，实现了线性复杂度的高效剪枝，结合神经科学启发的资格蒸馏方法，在高稀疏度下保持甚至提升模型性能。


<details>
  <summary>Details</summary>
Motivation: 解决SNN深度架构参数膨胀和计算成本高的问题，现有剪枝方法要么难以硬件加速（非结构化），要么灵活性差且精度下降（结构化），需要一种兼顾硬件友好性和性能的半结构化剪枝方案。

Method: 提出N:M半结构化剪枝框架，使用M路基对数参数化和可微分top-k采样器线性化复杂度；引入资格蒸馏(EID)将时间累积信用转换为块级软目标，对齐掩码概率与脉冲动态。

Result: 在2:4稀疏度下，SpikeNM在主流数据集上保持甚至提升性能，同时产生硬件友好的剪枝模式，与固有脉冲稀疏性互补。

Conclusion: SpikeNM成功实现了SNN的高效半结构化剪枝，在保持性能的同时提供了硬件友好的稀疏模式，为边缘部署提供了可行方案。
...
TL;DR: 提出了DGCF框架，将冻结的自监督DINOv3 Transformer与可训练的CNN编码器-解码器集成，通过可学习的交叉融合模块分层融合Transformer的全局表示和CNN的局部特征，实现局部外观和上下文表示的平衡。


<details>
  <summary>Details</summary>
Motivation: 现有CNN模型缺乏全局语义理解，而Transformer在小规模医学数据集上容易过拟合。需要一种能平衡局部外观和全局语义的方法来生成合成CT图像。
...
TL;DR: 提出Adaptive Begin-of-Video Tokens (ada-BOV)方法，用于改进自回归视频扩散模型，解决现有方法在长视频生成中的一致性和动态质量问题。


<details>
  <summary>Details</summary>
Motivation: 现有视频扩散模型在生成长视频时面临挑战：基于分块扩展的方法存在去噪延迟和误差累积问题，流式去噪方法则存在一致性脆弱和动态质量差的问题。

Method: 提出自适应BOV令牌，通过类似自适应层归一化的调制机制吸收先前去噪帧信息；提出流式去噪细化策略，解耦采样轨迹长度与注意力窗口约束；提出扰动增强训练噪声调度。

Result: 在多个指标上取得了令人信服的定性和定量结果，显著提升了长视频生成的一致性和动态质量。

Conclusion: ada-BOV方法有效解决了自回归视频扩散模型在长视频生成中的关键问题，在保持全局一致性的同时提升了局部动态质量。
...
TL;DR: 提出SS-CA方法，通过反事实解释增强训练，纠正模型对有限因果特征的依赖，提高泛化能力和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 当前视觉模型仅依赖有限充分原因进行预测，对分布偏移或关键特征缺失敏感。模型与人类在反事实场景下的识别差异表明模型学习到的依赖关系因果性不足。

Method: 基于LIMA归因方法开发Counterfactual LIMA，识别最小空间区域集，其移除可选择性改变模型预测。使用数据增强策略将识别区域替换为自然背景，联合训练原始和增强样本。

Result: 在多个ImageNet变体上的实验表明，SS-CA提高了ID测试数据的泛化能力，在OOD基准（如ImageNet-R和ImageNet-S）上表现优异。在噪声等扰动下也展现出增强的泛化能力。

Conclusion: SS-CA有效利用可解释性洞察纠正模型缺陷，同时提升性能和鲁棒性，证明将反事实解释整合到训练过程中的有效性。
...
TL;DR: BdSL-SPOTER是一个基于姿态的transformer框架，用于准确高效地识别孟加拉手语，在BdSLW60基准测试中达到97.92%的Top-1验证准确率，比Bi-LSTM基线提升22.82%，同时保持较低计算成本。


<details>
  <summary>Details</summary>
Motivation: 为孟加拉手语开发一个准确且计算效率高的识别系统，解决低资源区域手语识别问题，为实际可访问性应用提供实用框架。

Method: 扩展SPOTER范式，采用文化特定的预处理、紧凑的四层transformer编码器、优化的可学习位置编码，并使用课程学习来增强有限数据的泛化能力和加速收敛。
...
TL;DR: TEMPO是一个基于深度学习从高分辨率卫星图像生成的全球时间序列建筑密度和高度数据集，分辨率37.6米，时间覆盖2018年Q1至2025年Q2，验证F1分数85-88%。


<details>
  <summary>Details</summary>
Motivation: 需要大规模监测全球建成区变化模式，为气候影响评估和全球韧性适应工作提供数据支持。

Method: 使用现有建筑足迹和高度数据与季度PlanetScope卫星图像配对，训练多任务深度学习模型预测建筑密度和高度。

Result: 模型在人工标注子集上F1分数达85-88%，时间稳定性高（5年趋势一致性得分0.96），能以较低计算成本捕获季度建成区变化。

Conclusion: TEMPO数据集为大规模监测发展模式和气候影响提供了高效解决方案，支持全球韧性和适应工作。
...
Result: 仅使用3.5M可训练参数，该方法在检测准确率上达到甚至超越了当前复杂的先进方法。
...
TL;DR: 提出了MEMR-Seg任务，通过多轮对话实现实体级推理的医学图像分割，构建了MR-MedSeg数据集，并开发了MediRound基线模型，引入判断与校正机制来缓解错误传播。
...
Method: 构建MR-MedSeg数据集，提出MediRound基线模型，在推理阶段引入轻量级的判断与校正机制来缓解错误传播。

Result: 实验结果表明该方法有效解决了MEMR-Seg任务，优于传统的医学参考分割方法。

Conclusion: MEMR-Seg任务和MediRound模型为多轮医学推理分割提供了有效解决方案，提升了交互性和推理能力。
...
TL;DR: RadarMP是一种使用4D毫米波雷达回波信号进行精确3D场景运动感知的新方法，通过统一架构联合建模雷达目标检测和运动估计任务，在恶劣天气条件下实现可靠的运动感知。
...
Result: 在公共数据集上的实验表明，RadarMP在各种天气和光照条件下实现了可靠的运动感知，优于基于雷达的解耦运动感知流水线。
...
TL;DR: 提出OAD-Promoter方法，通过减轻语言偏见和提升域迁移鲁棒性来增强基于LLM的视觉问答性能，在少样本和零样本场景下取得新的SOTA结果。


<details>
  <summary>Details</summary>
Motivation: LLMs在视觉问答中处理知识密集型问题时存在语言偏见问题，导致预测不可靠且难以泛化到分布外数据。

Method: OAD-Promoter包含三个组件：OEG模块生成全局描述和对象集中样本，MKA模块通过检索存储知识处理OOD样本，OAD Prompt整合模块输出优化LLM推理。

Result: 实验表明OAD-Promoter显著提升了基于LLM的VQA方法在少样本和零样本设置下的性能。

Conclusion: 该方法有效缓解了LLMs在VQA中的语言偏见问题，并提升了域迁移鲁棒性，在少样本和零样本场景下实现了最先进性能。
...
TL;DR: 提出了一个轻量级的基于C的SNN推理运行时，通过优化数据布局、预分配和稀疏性剪枝，在边缘设备上实现高效推理，速度提升约10倍，内存大幅减少。


<details>
  <summary>Details</summary>
Motivation: SNN的事件驱动特性在时序处理和能效方面具有优势，但在资源受限的硬件上训练和部署仍面临挑战，需要轻量级高效的推理解决方案。

Method: 将SNNTorch训练的模型转换为紧凑的C表示，采用静态缓存友好的数据布局和预分配避免解释器和分配开销，利用稀疏脉冲活动剪枝非活跃神经元和突触。

Result: 在N-MNIST和ST-MNIST数据集上实现与Python基线功能对等，桌面CPU上速度提升约10倍，剪枝带来额外增益，内存大幅减少可部署到微控制器。

Conclusion: 当配合优化的运行时和脉冲驱动模型压缩时，SNN可以在传统嵌入式平台上高效执行。
...
TL;DR: MAVIS是首个评估多模态来源归因系统的基准，包含15.7万视觉问答实例，开发了信息量、基础性和流畅性的自动评估指标，发现多模态RAG在图像文档上的基础性较弱，存在信息量与基础性之间的权衡。


<details>
  <summary>Details</summary>
Motivation: 现有来源归因工作主要关注纯文本场景，忽视了多模态的作用，需要评估能够理解视觉问题意图、检索多模态证据并生成带引用的长答案的系统。

Method: 构建包含15.7万视觉问答实例的数据集，每个答案都标注了引用多模态文档的事实级引用，开发了三个维度的自动评估指标，并比较了不同提示方法下的性能。

Result: 多模态RAG比单模态RAG生成更信息和流畅的答案，但在图像文档上的基础性较弱；不同提示方法在信息量和基础性之间存在权衡；需要减轻图像文档解释中的上下文偏见。

Conclusion: 多模态来源归因系统在图像文档上的基础性存在挑战，信息量与基础性之间存在权衡，未来研究应重点关注减轻图像文档解释中的上下文偏见。
...
TL;DR: 提出了TMKT框架，通过时间步混合策略在RGB和DVS模态间进行平滑知识迁移，解决了事件相机数据稀缺和模态分布差异问题。


<details>
  <summary>Details</summary>
Motivation: 事件相机与脉冲神经网络结合具有能效优势，但DVS数据稀缺且输出稀疏，传统RGB到DVS的知识迁移因模态分布差异大而效果不佳。

Method: 采用概率时间步混合策略，在SNN不同时间步插值RGB和DVS输入；引入两个轻量级模态感知目标：每帧源监督的模态感知指导和序列级混合比感知。

Result: 在多个基准测试和SNN骨干网络上验证了方法的有效性，实现了更平滑的知识迁移和更好的脉冲图像分类性能。

Conclusion: TMKT框架通过时间步混合和模态感知目标，有效缓解了模态不匹配问题，提升了跨模态知识迁移效果。
...
TL;DR: FIA-Edit是一个基于频率交互注意力的无反转图像编辑框架，通过频率表示交互和特征注入模块实现高保真编辑，并在医学图像增强中取得突破。


<details>
  <summary>Details</summary>
Motivation: 现有的基于流的无反转方法虽然高效，但缺乏有效的源信息整合，导致背景保留差、空间不一致和过度编辑问题。

Method: 提出频率交互注意力框架，包含频率表示交互模块（在自注意力中交换源和目标特征的频率成分）和特征注入模块（在交叉注意力中显式注入源侧查询、键、值和文本嵌入）。

Result: 在RTX 4090上每张512*512图像仅需约6秒，在视觉质量、背景保真度和可控性方面优于现有方法，并首次将文本引导图像编辑扩展到临床应用中，显著提升出血分类性能。

Conclusion: FIA-Edit实现了高效、高保真的图像编辑，为医学数据增强开辟了新途径，在临床应用中展现出巨大潜力。
...
TL;DR: 提出Center-Reassigned Hashing (CRH)方法，通过动态重新分配预设码本中的哈希中心来联合优化哈希函数，避免了两阶段方法的复杂性和性能损失，并在多基准测试中优于现有深度哈希方法。


<details>
  <summary>Details</summary>
Motivation: 现有的基于哈希中心的方法虽然比成对或三元组方法更高效，但随机中心初始化忽略了类间语义关系，而两阶段方法虽然能缓解这个问题，但引入了额外复杂性、计算开销和阶段间不一致导致的次优性能。

Method: 提出端到端的CRH框架，从预设码本中动态重新分配哈希中心，同时联合优化哈希函数，无需显式的中心优化阶段；采用多头机制增强哈希中心的表示能力。

Result: 在三个基准测试上的广泛实验表明，CRH能够学习到具有语义意义的哈希中心，并在检索任务中优于最先进的深度哈希方法。

Conclusion: CRH通过动态重新分配哈希中心和端到端优化，有效整合了语义关系到学习过程中，避免了传统两阶段方法的局限性，实现了更好的检索性能。
...
TL;DR: 论文提出了一种新的点云补全范式Completion-by-Correction，通过预训练图像到3D模型生成拓扑完整的形状先验，然后在特征空间进行校正，相比传统的Completion-by-Inpainting方法能更好地保持结构一致性和避免拓扑伪影。


<details>
  <summary>Details</summary>
Motivation: 现有的多模态点云补全方法大多采用Completion-by-Inpainting范式，从融合的潜在特征中合成缺失结构，但这种方法往往导致结构不一致和拓扑伪影，因为几何和语义约束有限。

Method: 提出PGNet多阶段框架：1）双特征编码以锚定生成先验；2）合成结构对齐的粗粒度支架；3）通过分层校正逐步细化几何细节。

Result: 在ShapeNetViPC数据集上的实验表明，PGNet在平均Chamfer距离上优于现有最佳方法23.5%，F-score提升7.1%。
...
<details>
  <summary>Details</summary>
Motivation: 传统的离散自回归方法由于量化过程和有限码本大小会丢失细粒度信息，而连续自回归建模在无结构的连续空间中面临效率挑战。

Method: 提出MixAR框架，探索了三种离散-连续混合策略：自注意力(DC-SA)、交叉注意力(DC-CA)和简单混合(DC-Mix)，以及训练-推理混合(TI-Mix)来保证训练和生成分布的一致性。

Result: 实验表明DC-Mix策略在计算效率和生成保真度之间取得了良好平衡，TI-Mix带来了持续改进。

Conclusion: MixAR通过离散token的先验指导有效解决了连续自回归建模的挑战，在图像生成质量上取得了显著提升。
...
TL;DR: 提出MMRINet轻量级架构，用线性复杂度的Mamba状态空间模型替代二次复杂度的注意力机制，在BraTS-Lighthouse SSA 2025中仅用约250万参数达到平均Dice分数0.752和HD95 12.23的强性能。


<details>
  <summary>Details</summary>
Motivation: 在资源受限环境中，深度3D网络计算成本过高，多参数MRI脑肿瘤自动分割面临挑战。

Method: 使用线性复杂度的Mamba状态空间模型进行高效体积上下文建模；提出双路径特征细化模块最大化特征多样性；采用渐进特征聚合实现有效的多尺度融合。

Result: 在BraTS-Lighthouse SSA 2025中，模型仅用约250万参数就达到平均Dice分数0.752和平均HD95 12.23的强性能。

Conclusion: MMRINet展示了在低资源临床环境中高效且准确的分割能力，适合资源受限的医疗设置。
...
<details>
  <summary>Details</summary>
Motivation: 驾驶员分心是交通事故的主要原因，现有深度学习方法在真实部署中面临两个关键挑战：摄像头视角变化和域偏移（如传感器模态或环境变化），现有方法通常单独处理这些问题，缺乏统一的解决方案。

Method: 两阶段框架：第一阶段使用对比学习在多视角数据中学习视角不变和动作判别特征；第二阶段使用信息瓶颈损失进行跨模态域自适应，无需新域的标注数据。

Result: 使用Video Swin和MViT视频变换器在Drive&Act数据集上评估，相比监督对比学习方法，RGB视频数据的top-1准确率提升近50%；相比仅使用无监督域自适应的方法，性能提升高达5%。

Conclusion: 该联合框架能有效解决驾驶员活动识别中的跨视角和跨模态挑战，为模型在不同车辆配置中的鲁棒和可扩展部署提供了有效解决方案。
...
TL;DR: 提出了一种分层语义学习框架来解决跨域少样本分割中的分割粒度差距问题，通过双风格随机化和分层语义挖掘模块增强模型对不同粒度语义的识别能力。


<details>
  <summary>Details</summary>
Motivation: 现有的跨域少样本分割方法主要关注源域和目标域之间的风格差距，而忽略了分割粒度差距，导致对目标域中新类别的语义判别能力不足。

Method: 提出了分层语义学习框架，包含双风格随机化模块（模拟目标域数据的风格差异）和分层语义挖掘模块（利用多尺度超像素挖掘不同粒度的类内一致性和类间区分性），以及原型置信度调制阈值模块（缓解前景背景过于相似时的分割模糊问题）。

Result: 在四个流行的目标域数据集上进行了广泛实验，结果表明该方法达到了最先进的性能。

Conclusion: 所提出的分层语义学习框架有效解决了跨域少样本分割中的分割粒度差距问题，显著提升了模型性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有稀疏注意力方法主要针对推理时加速，使用预定义稀疏模式选择关键令牌，但无法弥合训练-推理差距，且缺乏跨查询、键值对和注意力头的细粒度令牌选择能力，导致性能次优和加速收益有限。

Method: 包含三个自适应互补机制：(1) 通过惰性-主动分类进行查询选择，保留捕捉广泛语义相似性的主动查询；(2) 基于最平坦头确定共享预算的头部级动态预算分配进行KV选择；(3) 根据头部级解码查询模式选择性获取视觉KV缓存的KV缓存精简。

Result: 实验结果显示OmniSparse在保持完整注意力性能的同时，在前填充阶段实现最高2.7倍加速，在解码阶段实现2.4倍内存减少。

Conclusion: OmniSparse框架有效解决了训练-推理差距问题，通过细粒度稀疏注意力机制实现了显著的加速和内存优化，同时保持了模型性能。
...
TL;DR: 提出LSS3D方法，通过可学习空间偏移解决多视图不一致性和非正面输入视角问题，实现高质量图像到3D生成
...
Method: 为每个视图分配可学习空间偏移参数，通过重建网格引导调整各视图实现空间一致性，并引入输入视图作为额外约束增强鲁棒性

Result: 在几何和纹理评估指标上取得领先结果，对更灵活的输入视角具有更好的鲁棒性

Conclusion: LSS3D方法有效解决了多视图不一致性问题，实现了高质量3D生成，特别提升了非正面输入视角的鲁棒性
...
TL;DR: 提出Geometry-guided Multi-View Diffusion Model，通过几何信息提取和强度调整机制，解决多视图图像生成中的一致性和细节保持问题。


<details>
  <summary>Details</summary>
Motivation: 现有基于单图像扩展的多视图生成方法在保持跨视图一致性和生成高分辨率输出方面存在计算挑战，需要更有效的几何引导方法。

Method: 设计多视图几何信息提取模块（深度图、法线图、前景分割掩码），解耦几何增强注意力机制，自适应学习策略，迭代细化过程和动态几何信息强度调整机制。

Result: 模型能够生成跨视图一致且细节丰富的图像，提高了整体图像质量和细节保持能力。

Conclusion: 该方法通过几何引导有效解决了多视图图像生成中的一致性和细节问题，为3D重建、虚拟现实等应用提供了更高质量的解决方案。
...
TL;DR: 开发基于AI的交通违规自动检测系统，使用YOLOv8进行目标检测和EasyOCR进行车牌识别，能够检测头盔违规、摩托车后视镜缺失等违规行为，并提取车牌号码。
...
Method: 使用YOLOv8进行目标检测，EasyOCR进行车牌识别，在自定义标注数据集上进行训练，采用数据增强技术，通过Streamlit界面实现实时监控和违规记录。

Result: 模型整体精度0.9147，召回率0.886，平均精度(mAP@50)0.843，在严格IoU阈值下mAP@50-95为0.503，表现出强大的检测能力。

Conclusion: 该工作展示了一个实用有效的自动交通规则执法解决方案，具备实际部署的可行性。
...
### [80] [MiniGPT-Pancreas: Multimodal Large Language Model for Pancreas Cancer Classification and Detection](https://arxiv.org/abs/2412.15925)
*Andrea Moglia,Elia Clement Nastasio,Luca Mainardi,Pietro Cerveri*

Main category: cs.CV

TL;DR: MiniGPT-Pancreas是一个多模态大语言模型，通过级联微调在胰腺检测、肿瘤分类和肿瘤检测任务中表现出色，为临床医生提供胰腺癌诊断支持。


<details>
  <summary>Details</summary>
Motivation: 胰腺放射影像学具有挑战性，因为胰腺体积小、边界模糊，且在不同患者中形状和位置变化大。需要开发交互式工具来支持临床医生的胰腺癌诊断。

Method: 对通用多模态大语言模型MiniGPT-v2进行级联微调，结合NIH、MSD和AbdomenCT-1k数据集的CT扫描图像和多模态提示，用于胰腺检测、肿瘤分类和肿瘤检测。

Result: 在NIH和MSD数据集上胰腺检测IoU分别为0.595和0.550；胰腺癌分类准确率0.876；多器官检测中肝脏IoU最高为0.8399；胰腺肿瘤检测IoU为0.168。

Conclusion: MiniGPT-Pancreas是支持临床医生进行胰腺肿瘤图像分类的有前景解决方案，未来需要改进胰腺肿瘤检测任务的性能。
...
### [81] [Mixture of States: Routing Token-Level Dynamics for Multimodal Generation](https://arxiv.org/abs/2511.12207)
...
<details>
  <summary>Details</summary>
Motivation: 现有多模态扩散模型在模态融合方面存在效率问题，需要更灵活、计算高效的融合方法来提升模型性能。

Method: 使用可学习的token级路由器，基于去噪时间步和输入创建模态间隐藏状态的交互，采用top-k稀疏选择和ε-greedy训练策略。

Result: 在文本到图像生成和编辑任务中达到最先进水平，仅用3B-5B参数即可匹配或超越参数量4倍大的模型。
...
### [82] [FaNe: Towards Fine-Grained Cross-Modal Contrast with False-Negative Reduction and Text-Conditioned Sparse Attention](https://arxiv.org/abs/2511.12215)
...
### [83] [Suppressing VLM Hallucinations with Spectral Representation Filtering](https://arxiv.org/abs/2511.12220)
*Ameen Ali,Tamim Zoabi,Lior Wolf*

Main category: cs.CV

TL;DR: 提出Spectral Representation Filtering (SRF)方法，通过分析视觉语言模型表示空间的协方差结构来抑制幻觉，无需训练且无推理开销。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型经常产生幻觉，描述图像中不存在的对象、属性或关系，这源于对语言先验的过度依赖和跨模态对齐不精确。

Method: 通过特征分解识别幻觉模式，使用软谱滤波器在深层vLLM层的前馈投影权重中衰减这些模式，均衡特征方差同时保持语义保真度。

Result: 在LLaVA-1.5、MiniGPT-4和mPLUG-Owl2等模型上，SRF在MSCOCO、POPE-VQA等基准测试中持续降低幻觉率，达到最先进的忠实度且不降低描述质量。

Conclusion: SRF是一种轻量级、无需训练的后处理方法，能有效抑制视觉语言模型的幻觉，无需架构修改且无推理开销。
...
### [84] [Model Inversion Attack Against Deep Hashing](https://arxiv.org/abs/2511.12233)
*Dongdong Zhao,Qiben Xu,Ranxin Fang,Baogang Song*

Main category: cs.CV

TL;DR: DHMI是首个针对深度哈希的扩散模型反演框架，能够在黑盒设置下成功重构高质量图像，揭示了深度哈希系统的严重隐私风险。
...
Method: DHMI首先聚类辅助数据集获得语义哈希中心作为代理锚点，然后提出代理引导的去噪优化方法，融合分类一致性和哈希邻近度作为攻击指标，通过代理模型簇引导候选样本的细化。
...
### [85] [Fusionista2.0: Efficiency Retrieval System for Large-Scale Datasets](https://arxiv.org/abs/2511.12255)
...
TL;DR: Fusionista2.0是一个优化的视频检索系统，通过重新设计核心模块和用户界面，在保持准确性的同时将检索时间减少75%，提升了用户体验。
...
### [86] [Prompt-Conditioned FiLM and Multi-Scale Fusion on MedSigLIP for Low-Dose CT Quality Assessment](https://arxiv.org/abs/2511.12256)
*Tolga Demiroglu,Mehmet Ozan Unal,Metin Ertas,Isa Yildirim*

Main category: cs.CV

TL;DR: 提出基于MedSigLIP的提示条件框架，通过FiLM和多尺度池化注入文本先验，在LDCTIQA2023数据集上实现SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 开发数据高效学习的方法，通过文本提示将临床意图注入图像特征，实现快速适应和高质量LDCT图像质量评估。

Method: 使用提示条件框架，通过FiLM注入文本先验，结合全局、局部和纹理感知池化的多尺度回归头，采用轻量级MLP融合和成对排序损失训练。

Result: 在LDCTIQA2023数据集（1000张训练图像）上取得PLCC=0.9575、SROCC=0.9561、KROCC=0.8301，超越已发表的最佳挑战提交结果。

Conclusion: 提示引导方法在LDCT图像质量评估中表现出色，证明了文本提示注入临床意图的有效性和数据效率优势。
...
### [87] [A Disease-Aware Dual-Stage Framework for Chest X-ray Report Generation](https://arxiv.org/abs/2511.12259)
*Puzhen Wu,Hexin Dong,Yi Lin,Yihao Ding,Yifan Peng*

Main category: cs.CV

TL;DR: 提出了一种新颖的双阶段疾病感知框架用于胸部X光报告生成，通过疾病感知语义标记和视觉-语言对齐，结合疾病-视觉注意力融合和双模态相似性检索，显著提升了临床准确性和语言质量。


<details>
  <summary>Details</summary>
Motivation: 现有方法在视觉表示中缺乏足够的疾病感知能力，视觉-语言对齐不足，导致忽略关键病理特征，难以生成临床准确的报告。

Method: 双阶段框架：第一阶段通过交叉注意力和多标签分类学习疾病感知语义标记，同时通过对比学习对齐视觉和语言表示；第二阶段引入疾病-视觉注意力融合模块和双模态相似性检索机制。

Result: 在多个基准数据集上的实验表明，该框架在胸部X光报告生成任务中达到了最先进的性能，临床准确性和语言质量均有显著提升。

Conclusion: 提出的疾病感知框架通过增强疾病感知能力和视觉-语言对齐，有效解决了现有方法在医疗图像分析中的局限性，为临床准确的放射学报告生成提供了有效解决方案。
...
### [88] [CrossVid: A Comprehensive Benchmark for Evaluating Cross-Video Reasoning in Multimodal Large Language Models](https://arxiv.org/abs/2511.12263)
*Jingyao Li,Jingyun Wang,Molin Tan,Haochen Wang,Cilin Yan,Likun Shi,Jiayin Cai,Xiaolong Jiang,Yao Hu*

Main category: cs.CV

TL;DR: CrossVid是首个专门评估多模态大语言模型在跨视频推理中时空推理能力的基准测试，包含4个高层维度和10个具体任务，提供5331个视频和9015个问答对。实验表明当前MLLMs在跨视频推理方面表现不佳，Gemini-2.5-Pro表现最佳但准确率仅为50.4%。


<details>
  <summary>Details</summary>
Motivation: 现有视频理解基准主要关注单视频分析，无法评估MLLMs同时推理多个视频的能力。虽然近期有基准评估多视角视频，但其有限的任务无法全面评估MLLMs在多样化真实世界跨视频推理场景中的表现。

Method: 开发CrossVid基准测试，包含广泛的层次化任务（4个高层维度、10个具体任务），提供5331个视频和9015个具有挑战性的问答对，涵盖单选、多选和开放式问题格式。

Result: 在各种开源和闭源MLLMs上的广泛实验表明，Gemini-2.5-Pro在CrossVid上表现最佳，平均准确率为50.4%。深入的案例研究显示大多数当前MLLMs在跨视频推理任务中表现挣扎，主要原因是无法整合或比较分布在多个视频中的证据进行推理。

Conclusion: CrossVid有潜力指导未来增强MLLMs跨视频推理能力的进展，当前MLLMs在跨视频推理方面仍有很大提升空间。
...
### [89] [ZoomEarth: Active Perception for Ultra-High-Resolution Geospatial Vision-Language Tasks](https://arxiv.org/abs/2511.12267)
*Ruixun Liu,Bowen Fu,Jiayi Song,Kaiyu Li,Wanchen Li,Lanxuan Xue,Hui Qiao,Weizhan Zhang,Deyu Meng,Xiangyong Cao*

Main category: cs.CV

TL;DR: 提出了ZoomEarth框架，通过主动感知范式处理超高分辨率遥感图像，使用自适应裁剪缩放和区域引导奖励机制，在多个基准测试中达到最先进性能，并能与下游任务无缝集成。
...
Result: 在LRS-GRO基准测试中达到最先进性能，在三个公共超高分辨率遥感基准测试中实现零样本迁移，并能与云去除、去噪、分割和图像编辑等下游任务无缝集成。

Conclusion: ZoomEarth框架通过主动感知范式有效处理超高分辨率遥感图像，展现出强大的多功能性和可扩展性，为遥感图像处理提供了新的解决方案。
...
### [90] [TM-UNet: Token-Memory Enhanced Sequential Modeling for Efficient Medical Image Segmentation](https://arxiv.org/abs/2511.12270)
*Yaxuan Jiao,Qing Xu,Yuxiang Luo,Xiangjian He,Zhen Chen,Wenting Duan*

Main category: cs.CV

TL;DR: TM-UNet是一种轻量级医学图像分割框架，通过多尺度令牌-内存块将2D空间特征转换为令牌序列，利用矩阵内存单元选择性保留和传播判别性上下文信息，以线性复杂度实现高效全局推理。


<details>
  <summary>Details</summary>
Motivation: 虽然基于transformer的方法在医学图像分割中取得了显著成果，但其高计算成本阻碍了临床部署，因此需要开发更高效的轻量级框架。

Method: 提出多尺度令牌-内存(MSTM)块，通过空间扫描将2D特征转换为令牌序列，利用矩阵内存单元选择性保留上下文信息，并采用指数门控识别令牌有效性，通过并行池化操作实现多尺度上下文提取。

Result: TM-UNet在多种医学分割任务中优于现有最先进方法，同时显著降低了计算成本。

Conclusion: TM-UNet通过创新的令牌-内存机制实现了高效的医学图像分割，在保持性能的同时大幅减少了计算开销，具有重要的临床应用价值。
...
### [91] [D$^{3}$ToM: Decider-Guided Dynamic Token Merging for Accelerating Diffusion MLLMs](https://arxiv.org/abs/2511.12280)
*Shuochen Chang,Xiaofeng Zhang,Qingyang Liu,Li Niu*

Main category: cs.CV

TL;DR: D³ToM是一种用于加速扩散多模态大语言模型推理的方法，通过动态合并冗余视觉token来减少计算复杂度，同时保持性能。


<details>
  <summary>Details</summary>
Motivation: 扩散多模态大语言模型在推理时存在速度慢的问题，因为每个去噪步骤都需要对整个序列进行双向自注意力计算，导致立方级的解码复杂度。

Method: 使用决策器引导的动态token合并方法，在每一步去噪过程中根据重要性图保留最关键的视觉token，并通过相似性聚合合并冗余token。

Result: 实验表明D³ToM能显著加速推理速度，同时在相同计算预算下保持竞争性性能。

Conclusion: D³ToM是一种即插即用的模块，能有效解决扩散多模态大语言模型推理速度慢的问题，无需改变模型参数。
...
### [92] [One target to align them all: LiDAR, RGB and event cameras extrinsic calibration for Autonomous Driving](https://arxiv.org/abs/2511.12291)
...
<details>
  <summary>Details</summary>
Motivation: 在自动驾驶等复杂视觉系统中，精确的多传感器对齐至关重要。现有方法通常依赖分离的成对标定，需要一种能够同时标定多种传感器的联合标定方法。

Method: 设计并构建了新型3D标定靶标，包含平面特征、ChArUco图案和主动LED模式，分别针对LiDAR、RGB相机和事件相机的特性。实现一次性联合外参标定过程。

Result: 在定制数据集上的广泛实验评估证实了方法的准确性和鲁棒性，该数据集使用先进的自动驾驶传感器设置记录。

Conclusion: 该方法能够准确标定复杂视觉系统，为自动驾驶等应用提供精确的多传感器对齐解决方案。
...
### [93] [Rethinking Bias in Generative Data Augmentation for Medical AI: a Frequency Recalibration Method](https://arxiv.org/abs/2511.12301)
*Chi Liu,Jincheng Liu,Congcong Zhu,Minghao Wang,Sheng Shen,Jia Gu,Tianqing Zhu,Wanlei Zhou*

Main category: cs.CV

TL;DR: 本文提出了频率重校准(FreRec)方法来解决医学图像生成数据增强(GDA)中的频率失配问题，通过统计高频替换(SHR)和重构高频映射(RHM)来改善合成图像质量，提升下游医学图像分类性能。


<details>
  <summary>Details</summary>
Motivation: 医学AI开发依赖大数据集但常面临数据稀缺问题，生成数据增强(GDA)可合成医学图像但存在偏差风险，特别是频率失配可能导致下游任务性能下降。

Method: 提出FreRec方法：1) 统计高频替换(SHR)粗略对齐高频成分；2) 重构高频映射(RHM)提升图像质量并重建高频细节。该方法可作为独立后处理步骤与任何生成模型兼容。

Result: 在脑部MRI、胸部X光和眼底图像等多种医学数据集上的实验表明，FreRec相比未校准的AI合成样本显著提升了医学图像分类性能。

Conclusion: FreRec通过减少频率分布差异有效改善了医学GDA的可靠性，是一个兼容性强且可无缝集成到常见医学GDA流程中的解决方案。
...
TL;DR: DenseAnnotate是一个音频驱动的在线标注平台，通过语音标注创建密集、细粒度的图像和3D资产标注，解决了传统文本标注在表达性、速度和视觉特征捕捉方面的局限性。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型训练数据存在稀疏标注问题，传统文本标注方法在表达性、速度和视觉特征捕捉方面存在局限，特别是在多元文化图像和3D资产标注等专业领域。
...
Conclusion: 该平台为未来视觉语言研究提供了可行方法，可应用于各种任务和多样数据类型，解决了密集标注稀缺的问题。
...
### [95] [LiDAR-GS++:Improving LiDAR Gaussian Reconstruction via Diffusion Priors](https://arxiv.org/abs/2511.12304)
*Qifeng Chen,Jiarun Liu,Rengan Xie,Tao Tang,Sicong Du,Yiru Zhao,Yuchi Huo,Sheng Yang*

Main category: cs.CV

TL;DR: LiDAR-GS++是一种基于高斯泼溅的LiDAR重建方法，通过扩散先验增强，在公共城市道路上实现实时高保真重模拟，解决了单次扫描重建不完整导致的视点外推伪影问题。


<details>
  <summary>Details</summary>
Motivation: 现有的基于高斯泼溅的LiDAR渲染方法在视点外推时会出现伪影，主要原因是单次遍历扫描的重建不完整。

Method: 引入可控的LiDAR生成模型，以粗外推渲染为条件生成额外的几何一致扫描，并采用有效的蒸馏机制进行扩展重建。
...
<details>
  <summary>Details</summary>
Motivation: 解决现有两阶段设计流程在解决方案质量和计算效率方面的不足，实现更智能的室内设计自动化

Method: 使用LLM提取设计约束，编码到基于Modulor的网格表示中，采用粗到细的优化策略，从低分辨率网格开始逐步细化

Result: 在多样化场景中显著优于现有两阶段设计流程，通过粗到细策略实现显著的计算效率提升

Conclusion: 联合优化方法在室内设计自动化中具有优越性，为智能设计系统提供了有效解决方案
...
### [97] [Learning Time in Static Classifiers](https://arxiv.org/abs/2511.12321)
*Xi Ding,Lei Wang,Piotr Koniusz,Yongsheng Gao*

Main category: cs.CV

TL;DR: 提出了一个简单有效的框架，为前馈分类器添加时序推理能力，无需修改模型架构或引入循环模块。通过SEQ学习范式，将训练数据组织成时序连贯的轨迹，学习类别特定的时序原型，并使用可微分的soft-DTW损失对齐预测序列。


<details>
  <summary>Details</summary>
Motivation: 现实世界视觉数据通常随时间逐渐演变（如姿态、光照、物体状态或场景上下文的变化），但传统分类器假设时间独立性，无法捕捉这种动态特性。

Method: 提出支持-样本-查询（SEQ）学习范式，将训练数据构建为时序连贯轨迹；学习类别特定的时序原型；使用可微分的soft-DTW损失对齐预测序列；多目标函数促进语义一致性和时序平滑性。

Result: 在静态和时序任务中均表现优异：提升了细粒度和超细粒度图像分类性能，在视频异常检测中实现了精确且时序一致的预测。
...
### [98] [SpaceVLM: Sub-Space Modeling of Negation in Vision-Language Models](https://arxiv.org/abs/2511.12331)
*Sepehr Kazemi Ranjbar,Kumail Alhamoud,Marzyeh Ghassemi*

Main category: cs.CV

TL;DR: 提出了一种无需训练的框架，将否定建模为联合嵌入空间中的子空间而非单点，显著提升了视觉语言模型的否定理解能力，同时保持零样本性能。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型在处理否定提示时表现不佳，而基于大规模否定数据集微调的方法往往会损害模型在肯定提示上的零样本性能。

Method: 将否定建模为嵌入空间中的子空间，通过构建围绕A和N嵌入的球形帽区域，计算靠近A且远离N的区域中心方向来评分图像。

Result: 在检索、多项选择和文本到图像任务中，该方法将否定理解能力平均提高了约30%，同时保持了零样本性能。

Conclusion: 该方法有效解决了VLMs的否定理解问题，缩小了肯定和否定提示之间的性能差距，且无需训练即可实现。
...
### [99] [Ground Plane Projection for Improved Traffic Analytics at Intersections](https://arxiv.org/abs/2511.12342)
*Sajjad Pakdamansavoji,Kumar Vaibhav Jha,Baher Abdulhai,James H Elder*

Main category: cs.CV

TL;DR: 将基础设施摄像头检测到的车辆反投影到地面平面进行3D坐标分析，可以提高转向运动计数的准确性，特别是多摄像头弱融合能获得更高精度。


<details>
  <summary>Details</summary>
Motivation: 准确的交叉口转向运动计数对于信号控制、交通管理和城市规划至关重要，传统基于图像平面的计算机视觉系统存在局限性。

Method: 将单/多基础设施摄像头检测到的车辆反投影到地面平面，在真实世界3D坐标中分析，并采用多摄像头弱融合技术。

Result: 单摄像头反投影能提高轨迹分类和转向运动计数的准确性，多摄像头弱融合能获得更高的精度。

Conclusion: 交通分析应该在地面平面而非图像平面进行，反投影方法能显著提升转向运动计数的准确性。
...
### [100] [CLAReSNet: When Convolution Meets Latent Attention for Hyperspectral Image Classification](https://arxiv.org/abs/2511.12346)
*Asmit Bandyopadhyay,Anindita Das Bhattacharjee,Rakesh Das*

Main category: cs.CV

TL;DR: CLAReSNet是一种混合架构，结合多尺度卷积提取和变换器风格注意力，通过自适应潜在瓶颈解决高光谱图像分类中的高维度和样本不平衡问题。


<details>
  <summary>Details</summary>
Motivation: 高光谱图像分类面临高光谱维度、复杂光谱-空间相关性和有限训练样本（严重类别不平衡）等关键挑战。CNN擅长局部特征提取，变换器捕捉长程依赖，但单独应用效果不佳。

Method: 提出CLAReSNet混合架构：多尺度卷积主干提取层次空间特征，光谱编码器层结合双向RNN和多尺度光谱潜在注意力（MSLA），MSLA通过自适应潜在令牌分配将复杂度从O(T²D)降至O(Tlog(T)D)。

Result: 在Indian Pines和Salinas数据集上达到最先进性能，总体准确率分别为99.71%和99.96%，显著超越HybridSN、SSRN和SpectralFormer。

Conclusion: CLAReSNet在有限样本和严重类别不平衡条件下有效，学习到的嵌入表现出优越的类间可分性和紧凑的类内聚类。
...
### [101] [Explainable AI-Generated Image Detection RewardBench](https://arxiv.org/abs/2511.12363)
*Michael Yang,Shijian Deng,William T. Doan,Kai Wang,Tianyu Yang,Harsh Singh,Yapeng Tian*

Main category: cs.CV

TL;DR: 提出了XAIGID-RewardBench基准，用于评估多模态大语言模型在判断AI生成图像检测解释质量方面的能力，发现当前最佳模型与人类表现仍有差距。


<details>
  <summary>Details</summary>
Motivation: 传统基于分类的AI生成图像检测方法无法提供人类专家可理解的解释，降低了检测工具的可信度和说服力。虽然MLLMs被用于解决此问题，但其在评估自身或其他MLLMs生成的解释质量方面的能力尚未得到充分研究。
...
Conclusion: 提出了首个专门评估MLLMs判断AI生成图像检测解释质量的基准，揭示了当前模型与人类表现之间的差距，并分析了常见错误模式。
...
### [102] [Constructing and Interpreting Digital Twin Representations for Visual Reasoning via Reinforcement Learning](https://arxiv.org/abs/2511.12365)
*Yiqing Shen,Mathias Unberath*

Main category: cs.CV

TL;DR: DT-R1是一个强化学习框架，通过训练大语言模型构建视觉输入的数字孪生表示，并基于这些高级表示进行统一视觉推理，在多种任务和模态上超越特定任务模型。


<details>
  <summary>Details</summary>
Motivation: 现有视觉推理方法依赖特定任务的监督微调和架构设计，阻碍了统一解决方案的发展，限制了跨任务和跨模态的泛化能力。

Method: 使用GRPO强化学习训练大语言模型构建视觉输入的数字孪生表示，并通过新颖的奖励函数验证结构完整性和输出准确性。
...
### [103] [Fast Reasoning Segmentation for Images and Videos](https://arxiv.org/abs/2511.12368)
*Yiqing Shen,Mathias Unberath*

Main category: cs.CV

TL;DR: FastReasonSeg是一个通过数字孪生表示实现高效推理分割的方法，通过解耦感知与推理，使用监督微调和强化学习进行知识蒸馏，在保持高性能的同时大幅减少模型参数和内存消耗。
...
Method: 使用数字孪生表示将感知与推理解耦，首先在教师生成的推理链上进行监督微调，然后通过联合奖励评估分割准确性和推理质量对齐的强化微调。

Result: 在两个视频基准（JiTBench、RVTBench）和两个图像基准（ReasonSeg、LLM-Seg40K）上达到最先进的推理分割性能。蒸馏后的0.6B变体在性能上超过参数多20倍的模型，同时实现7.79 FPS吞吐量和仅2.1GB内存消耗。

Conclusion: 该方法实现了高效的推理分割，能够在资源受限的环境中部署，支持实时推理分割。
...
### [104] [Changes in Real Time: Online Scene Change Detection with Multi-View Fusion](https://arxiv.org/abs/2511.12370)
*Chamuditha Jayanga Galappaththige,Jason Lai,Lloyd Windrim,Donald Dansereau,Niko Sünderhauf,Dimity Miller*

Main category: cs.CV

TL;DR: 提出了首个姿态无关、无需标签、保证多视角一致性的在线场景变化检测方法，在超过10FPS的速度下超越了现有最佳离线方法的性能。


<details>
  <summary>Details</summary>
Motivation: 在线场景变化检测是一个极具挑战性的问题，现有在线方法准确性远低于离线方法，需要开发能在无约束视角下实时检测相关变化的方法。
...
Conclusion: 该方法首次实现了姿态无关、无需标签、多视角一致的在线场景变化检测，在保持实时性能的同时超越了离线方法的准确性。
...
### [105] [Reasoning Text-to-Video Retrieval via Digital Twin Video Representations and Large Language Models](https://arxiv.org/abs/2511.12371)
*Yiqing Shen,Chenxiao Fan,Chenjia Li,Mathias Unberath*

Main category: cs.CV

TL;DR: 提出推理文本到视频检索新范式，通过数字孪生表示和LLM推理处理隐式查询，在多个基准测试中显著优于现有方法


<details>
  <summary>Details</summary>
Motivation: 现有文本到视频检索方法只能处理显式查询，无法应对需要推理的隐式查询

Method: 两阶段框架：首先通过数字孪生表示进行组合对齐，然后使用LLM推理和即时细化处理信息缺失

Result: 在ReasonT2VBench-135上达到81.2% R@1，比最强基线提升50个百分点以上，在三个传统基准测试中均达到SOTA

Conclusion: 数字孪生表示和LLM推理的结合能够有效处理隐式查询，为文本到视频检索开辟了新方向
...
### [106] [AGGRNet: Selective Feature Extraction and Aggregation for Enhanced Medical Image Classification](https://arxiv.org/abs/2511.12382)
*Ansh Makwe,Akansh Agrawal,Prateek Jain,Akshan Agrawal,Priyanka Bagade*

Main category: cs.CV

TL;DR: 提出了AGGRNet框架，通过提取信息性和非信息性特征来区分医学图像中的细微类别差异，在多个医学影像数据集上达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 医学图像分析面临类别间视觉模式复杂相似、标注数据稀缺、专家解释差异等挑战，现有注意力模型难以有效区分细微类别，导致错误诊断。

Method: 提出AGGRNet框架，提取信息性和非信息性特征，以更好地理解细粒度视觉模式并改进复杂医学图像分析任务的分类性能。

Result: 在多个医学影像数据集上实现最先进性能，在Kvasir数据集上相比SOTA模型提升高达5%。

Conclusion: AGGRNet能够有效捕捉医学图像中的细微类别差异，显著提升复杂医学图像分析任务的分类精度。
...
### [107] [Leveraging Quantum-Based Architectures for Robust Diagnostics](https://arxiv.org/abs/2511.12386)
*Shabnam Sodagari,Tommy Long*

Main category: cs.CV

TL;DR: 本研究开发了一种混合量子-经典框架，使用CT图像诊断肾脏结石、囊肿和肿瘤。结合预训练的ResNet50编码器和量子卷积神经网络，在8量子位和12量子位配置下均达到99%的测试准确率。


<details>
  <summary>Details</summary>
Motivation: 利用量子计算提升医学图像诊断性能，特别是针对肾脏疾病的CT图像分类问题。

Method: 使用预处理的肾脏CT图像，通过ResNet50提取特征，采用角度编码转换为量子位，使用量子卷积神经网络进行分类。处理类别不平衡问题，评估8量子位和12量子位两种配置。

Result: 两种配置均快速收敛且性能稳定，测试准确率达0.99。12量子位配置在囊肿检测中实现完美召回率，肿瘤F1分数达0.9956，混淆矩阵显示各类别分类可靠。

Conclusion: 将经典预处理和深度特征提取与量子电路结合，可显著提升医学诊断性能。
...
### [108] [Calibrated Decomposition of Aleatoric and Epistemic Uncertainty in Deep Features for Inference-Time Adaptation](https://arxiv.org/abs/2511.12389)
*Divake Kumar,Patrick Poggi,Sina Tayebati,Devashri Naik,Nilesh Ahuja,Amit Ranjan Trivedi*

Main category: cs.CV

TL;DR: 提出了一个轻量级推理时间框架，通过分解特征空间中的偶然不确定性和认知不确定性来指导自适应模型选择，无需采样、集成或额外前向传播，显著降低计算成本。


<details>
  <summary>Details</summary>
Motivation: 现有估计器将所有不确定性模式压缩为单一置信度分数，无法可靠判断何时需要分配更多计算资源或调整推理过程。

Method: 使用正则化全局密度模型估计偶然不确定性，认知不确定性由三个互补组件构成：局部支持不足、流形谱崩溃和跨层特征不一致性。这些组件经验正交且无需额外计算。

Result: 在MOT17上减少约60%的计算成本且精度损失可忽略，不确定性分解比总不确定性基线提高13.6个百分点的计算节省。
...
### [109] [MSLoRA: Multi-Scale Low-Rank Adaptation via Attention Reweighting](https://arxiv.org/abs/2511.12400)
*Xu Yang,Gady Agam*

Main category: cs.CV

TL;DR: MSLoRA是一种与主干网络无关的参数高效适配器，通过重新加权特征响应而非重新调整主干网络来统一CNN和ViT的适配，仅需不到5%的参数即可提升迁移性能。


<details>
  <summary>Details</summary>
Motivation: 现有的低秩适配方法主要局限于视觉变换器(ViTs)，难以在不同架构间通用化，需要一种能统一适配CNN和ViT的通用方法。

Method: 结合低秩线性投影和多尺度非线性变换，通过点乘和残差连接融合，联合调制空间和通道注意力，保持预训练权重冻结。

Result: 在分类、检测和分割任务上持续提升迁移性能，参数效率高，优化稳定，收敛快速，跨架构泛化能力强。

Conclusion: MSLoRA通过重新加权而非重新调整，为冻结视觉主干网络的高效适配提供了一个简单通用的解决方案。
...
### [110] [VLA-R: Vision-Language Action Retrieval toward Open-World End-to-End Autonomous Driving](https://arxiv.org/abs/2511.12405)
...
Result: 在真实机器人平台上的实验表明，即使在数据有限的情况下，在非结构化、未见过的环境中也表现出强大的泛化和探索性能。
...
### [111] [Self-Supervised Visual Prompting for Cross-Domain Road Damage Detection](https://arxiv.org/abs/2511.12410)
...
Method: 提出PROBE框架，包含自监督提示增强模块(SPEM)从无标签目标数据中推导缺陷感知提示来指导冻结的ViT骨干网络，以及域感知提示对齐(DAPA)目标来对齐提示条件下的源域和目标域表示。
...
### [112] [Towards Rotation-only Imaging Geometry: Rotation Estimation](https://arxiv.org/abs/2511.12415)
*Xinrui Li,Qi Cai,Yuanxin Wu*

Main category: cs.CV

TL;DR: 提出了一种基于旋转流形的纯旋转优化框架，通过将平移表示为旋转的函数，将成像几何表示压缩到旋转流形上，在SFM任务中实现了比现有方法更优越的精度和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 延续姿态唯一成像几何的视角，探索场景结构、旋转和平移之间的关键关系，发现平移可以用旋转表示，从而将成像几何表示压缩到旋转流形上。

Method: 提出了基于重投影误差的纯旋转优化框架，适用于双视图和多视图场景，将平移表示为旋转的函数，在旋转流形上进行优化。

Result: 实验结果表明该方法在旋转估计精度和鲁棒性方面优于当前最先进的方法，甚至可与多次捆绑调整迭代结果相媲美。

Conclusion: 这项工作有助于实现更准确、高效和可靠的3D视觉计算，为SFM任务提供了新的优化思路。
...
### [113] [Seeing Through the Rain: Resolving High-Frequency Conflicts in Deraining and Super-Resolution via Diffusion Guidance](https://arxiv.org/abs/2511.12419)
*Wenjie Li,Jinglei Shi,Jin Han,Heng Guo,Zhanyu Ma*

Main category: cs.CV

TL;DR: 提出了DHGM模型，通过整合预训练扩散先验和高通滤波器，同时去除雨痕伪影并增强结构细节，解决了天气恢复和超分辨率之间的冲突问题。


<details>
  <summary>Details</summary>
Motivation: 真实世界图像常受恶劣天气影响而退化，但现有的天气恢复方法可能会牺牲对分析小物体至关重要的高频细节。简单的去雨和超分辨率级联方法存在内在冲突：去雨旨在移除高频噪声，而超分辨率则要从现有细节中生成高频纹理。

Method: DHGM模型整合预训练扩散先验与高通滤波器，同时进行去雨和超分辨率处理，避免了两者之间的内容不一致问题。
...
### [114] [MFI-ResNet: Efficient ResNet Architecture Optimization via MeanFlow Compression and Selective Incubation](https://arxiv.org/abs/2511.12422)
*Nuolin Sun,Linyuan Wang,Haonan Wei,Lei Li,Bin Yan*

Main category: cs.CV

TL;DR: MFI-ResNet通过将ResNet的残差块替换为MeanFlow模块，采用压缩-扩展策略，在减少45%以上参数的同时提升了分类准确率。
...
Method: 压缩阶段：将每个ResNet阶段的多层结构简化为1-2个MeanFlow模块构建轻量级元模型；扩展阶段：对前三个阶段采用选择性孵化策略，扩展到基准ResNet的残差块配置，最后一个阶段保持MeanFlow形式并进行微调。

Result: 在CIFAR-10和CIFAR-100数据集上，相比ResNet-50分别减少46.28%和45.59%的参数，同时准确率提升0.23%和0.17%。

Conclusion: 生成式流场可以有效表征ResNet中的特征变换过程，为理解生成式建模与判别式学习的关系提供了新视角，同时实现了参数效率和判别性能的联合提升。
...
### [115] [RedVTP: Training-Free Acceleration of Diffusion Vision-Language Models Inference via Masked Token-Guided Visual Token Pruning](https://arxiv.org/abs/2511.12428)
*Jingqi Xu,Jingxi Lu,Chenghao Li,Sreetama Sarkar,Souvik Kundu,Peter A. Beerel*

Main category: cs.CV

TL;DR: RedVTP是一种针对扩散视觉语言模型(DVLMs)的响应驱动视觉令牌剪枝策略，通过利用模型推理动态来提升推理效率，在不损失准确性的情况下显著提高吞吐量和降低延迟。


<details>
  <summary>Details</summary>
Motivation: 扩散视觉语言模型(DVLMs)虽然支持并行令牌解码，但大量视觉令牌仍然严重阻碍其推理效率。虽然视觉令牌剪枝在自回归VLMs中已有广泛研究，但在DVLMs中仍未被充分探索。

Method: 提出RedVTP策略，利用DVLMs的推理动态来估计视觉令牌重要性。该方法使用掩码响应令牌的注意力来评估视觉令牌重要性，基于这些重要性分数在推理步骤间保持一致的观察，在第一步推理后从掩码令牌中剪枝较不重要的视觉令牌。

Result: 实验表明，RedVTP将LLaDA-V和LaViDa的令牌生成吞吐量分别提高了186%和28.05%，推理延迟分别降低了64.97%和21.87%，且在不损失准确性的情况下，某些情况下甚至提高了准确性。

Conclusion: RedVTP是一种有效的视觉令牌剪枝方法，能够显著提升DVLMs的推理效率，同时保持或改善模型性能，为高效多模态推理提供了有前景的解决方案。
...
### [116] [Text-Guided Channel Perturbation and Pretrained Knowledge Integration for Unified Multi-Modality Image Fusion](https://arxiv.org/abs/2511.12432)
*Xilai Li,Xiaosong Li,Weijun Jiang*

Main category: cs.CV

TL;DR: 提出了UP-Fusion框架，通过通道扰动和预训练知识集成解决多模态图像融合中的梯度冲突问题，在保持模态区分性的同时提高融合质量。


<details>
  <summary>Details</summary>
Motivation: 统一模型在多模态图像融合中因模态差异大导致梯度冲突，而模态特定编码器方法又限制了跨任务的泛化能力。

Method: 使用语义感知通道剪枝模块(SCPM)过滤冗余特征，几何仿射调制模块(GAM)保持模态区分性，文本引导通道扰动模块(TCPM)减少对模态特定通道的依赖。

Result: 在多种多模态图像融合任务和下游任务中表现优于现有方法。

Conclusion: UP-Fusion框架有效解决了多模态融合中的梯度冲突和泛化问题，提升了融合性能。
...
Method: 使用实时摄像头采集驾驶员面部图像，通过OpenCV检测面部关键点（眼睛开合度和打哈欠动作），利用预训练的深度卷积神经网络模型分析疲劳状态

Result: 在NTHU-DDD和Yawn-Eye数据集上分别达到99.6%和97%的疲劳检测分类准确率

Conclusion: 该方法提供了一种非侵入式、低成本且有效的疲劳检测解决方案，能够实时预警并可能挽救生命
...
### [118] [CoTBox-TTT: Grounding Medical VQA with Visual Chain-of-Thought Boxes During Test-time Training](https://arxiv.org/abs/2511.12446)
*Jiahe Qian,Yuhao Shen,Zhangtianyi Chen,Juexiao Zhou,Peisong Wang*

Main category: cs.CV

TL;DR: CoTBox-TTT是一种用于医学视觉问答的测试时训练方法，通过更新少量连续软提示来适应领域转移，无需重新训练或额外标注。


<details>
  <summary>Details</summary>
Motivation: 当前医学视觉问答系统在领域转移下可靠性不足，产生与图像证据弱相关的答案，且重新训练或添加标签在部署时不切实际。

Method: 采用证据优先的测试时训练方法，仅更新少量连续软提示，通过视觉思维链信号识别问题相关区域，并鼓励原始图像和局部裁剪之间的答案一致性。

Result: 在医学VQA实验中，该方法显著提升性能，例如在LLaVA模型上添加CoTBox-TTT后，在pathVQA上的闭式答案准确率提高了12.3%。

Conclusion: CoTBox-TTT是一种实用且即插即用的方法，适用于真实部署环境，能够有效提升医学视觉问答系统的可靠性和准确性。
...
TL;DR: MOON2.0是一个用于电商产品理解的多模态表示学习框架，通过模态平衡的MoE模块、双级对齐方法和图像-文本协同增强策略，解决了模态不平衡、对齐关系利用不足和数据噪声问题。


<details>
  <summary>Details</summary>
Motivation: 现有的多模态大语言模型在电商产品理解中面临三个挑战：模态混合训练导致的模态不平衡、产品内视觉和文本信息对齐关系利用不足、以及电商多模态数据噪声处理有限。

Method: 提出MOON2.0框架，包含：1）模态驱动的MoE模块进行多模态联合学习；2）双级对齐方法利用产品内语义对齐特性；3）基于MLLM的图像-文本协同增强策略和动态样本过滤。

Result: 在MBE2.0基准和多个公共数据集上实现了最先进的零样本性能，注意力热力图可视化提供了改进多模态对齐的定性证据。

Conclusion: MOON2.0通过动态模态平衡的多模态表示学习框架，有效提升了电商产品理解的性能，为多模态表示学习提供了新的解决方案。
...
### [120] [MaskAnyNet: Rethinking Masked Image Regions as Valuable Information in Supervised Learning](https://arxiv.org/abs/2511.12480)
*Jingshan Hong,Haigen Hu,Huihuang Zhang,Qianwei Zhou,Zhao Li*

Main category: cs.CV

TL;DR: 提出MaskAnyNet方法，将图像掩码区域视为辅助知识而非丢弃信息，通过重新学习机制同时利用可见和掩码区域，提升语义多样性并保留细粒度细节。


<details>
  <summary>Details</summary>
Motivation: 传统图像掩码存在两个问题：丢弃像素未充分利用导致上下文信息损失；掩码可能移除小或关键特征。MIM研究表明掩码区域可以从部分输入重建，显示掩码区域具有语义多样性潜力。
...
Conclusion: 将掩码内容视为辅助知识而非忽略信息是有效的，MaskAnyNet能够提升模型性能并保持细粒度细节。
...
### [121] [Towards Temporal Fusion Beyond the Field of View for Camera-based Semantic Scene Completion](https://arxiv.org/abs/2511.12498)
*Jongseong Bae,Junwoo Ha,Jinnyeong Heo,Yeongin Lee,Ha Young Kim*

Main category: cs.CV

TL;DR: 提出C3DFusion模块，通过显式对齐当前和历史帧的3D点特征来生成隐藏区域感知的3D特征几何，解决现有方法难以重建车辆侧方视野外区域的问题。


<details>
  <summary>Details</summary>
Motivation: 现有基于相机的3D语义场景补全方法主要关注增强帧内区域，但难以重建车辆侧方关键视野外区域，尽管历史帧通常包含这些不可见区域的宝贵上下文信息。

Method: 提出当前中心上下文3D融合(C3DFusion)模块，通过历史上下文模糊化和当前中心特征密集化两种互补技术：抑制不准确扭曲历史点特征的噪声，增强当前点特征的体积贡献。

Result: 在SemanticKITTI和SSCBench-KITTI-360数据集上显著优于最先进方法，并在其他基线模型上表现出强大的泛化能力，获得显著性能提升。

Conclusion: C3DFusion模块能有效解决视野外区域重建问题，通过增强的时间融合技术提升3D语义场景补全性能，具有强大的有效性和泛化能力。
...
### [122] [Visible Structure Retrieval for Lightweight Image-Based Relocalisation](https://arxiv.org/abs/2511.12503)
*Fereidoon Zangeneh,Leonard Bruns,Amit Dekel,Alessandro Pieropan,Patric Jensfelt*

Main category: cs.CV

TL;DR: 提出了一种新的视觉重定位范式，通过神经网络直接从图像观测中预测可见的3D场景结构，避免依赖图像检索或搜索启发式方法。


<details>
  <summary>Details</summary>
Motivation: 传统基于结构的重定位方法在大场景中面临计算复杂度高或存储需求大的问题，需要依赖搜索启发式或图像检索来减少搜索空间。

Method: 训练一个紧凑的神经网络，直接从查询图像中预测可见的3D结构点，从而减少2D-3D对应关系的搜索空间。

Result: 该方法在定位精度上达到了最先进水平，同时降低了计算和存储需求。

Conclusion: 提出的可见结构检索网络为基于结构的重定位提供了一种更高效的新范式，平衡了精度与效率。
...
### [123] [DINO-Detect: A Simple yet Effective Framework for Blur-Robust AI-Generated Image Detection](https://arxiv.org/abs/2511.12511)
...
TL;DR: 提出了一种基于师生知识蒸馏的模糊鲁棒AI生成图像检测框架，通过冻结教师模型（DINOv3）来保持其泛化能力，将清晰图像的特征和logit响应蒸馏到在模糊图像上训练的学生模型，使学生在运动模糊下也能产生一致的表示。


<details>
  <summary>Details</summary>
Motivation: 大多数AI生成图像检测器在真实世界退化（特别是运动模糊）下表现不佳，因为模糊会扭曲精细纹理并抑制高频伪影，导致实际场景中性能严重下降。

Method: 使用师生知识蒸馏框架，冻结在清晰图像上训练的DINOv3教师模型，将其特征和logit响应蒸馏到在模糊图像上训练的学生模型，使学生能够在运动模糊下产生一致的表示。

Result: 在运动模糊和清晰条件下的广泛实验基准测试表明，该方法实现了最先进的性能，展示了改进的泛化能力和实际应用性。

Conclusion: 该方法通过知识蒸馏有效解决了AI生成图像检测在运动模糊下的性能下降问题，提高了检测器在真实世界场景中的鲁棒性和适用性。
...
### [124] [MdaIF: Robust One-Stop Multi-Degradation-Aware Image Fusion with Language-Driven Semantics](https://arxiv.org/abs/2511.12525)
*Jing Li,Yifan Wang,Jiafeng Yan,Renlong Zhang,Bin Yang*

Main category: cs.CV

TL;DR: 提出了一个基于大语言模型的退化感知图像融合框架MdaIF，用于处理多退化场景下的红外与可见光图像融合问题，通过混合专家系统和退化感知通道注意力模块提升融合性能。
...
Method: 采用混合专家系统处理多退化场景，使用预训练视觉语言模型提取语义先验，提出退化感知通道注意力模块促进多模态特征交互，利用语义先验和通道域调制特征指导专家路由。

Result: 大量实验验证了MdaIF的有效性，在复杂退化场景下表现出优于现有最先进方法的性能。

Conclusion: MdaIF框架能够有效处理多退化场景下的图像融合问题，通过退化感知机制和自适应网络架构显著提升了融合性能。
...
### [125] [D$^{2}$-VPR: A Parameter-efficient Visual-foundation-model-based Visual Place Recognition Method via Knowledge Distillation and Deformable Aggregation](https://arxiv.org/abs/2511.12528)
...
<details>
  <summary>Details</summary>
Motivation: DINOv2等视觉基础模型虽然提升了VPR性能，但模型复杂度和计算开销大，难以部署在资源受限设备上。
...
Conclusion: D²-VPR框架实现了更好的性能-效率权衡，为资源受限设备上的VPR部署提供了可行方案。
...
TL;DR: 提出了TopoFG框架，通过细粒度查询和边界点拓扑推理来精确建模复杂车道结构，在OpenLane-V2基准测试中达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法使用单一查询表示每个车道，基于查询相似性推断拓扑连接性，但难以准确建模复杂车道结构，导致拓扑预测不可靠。

Method: TopoFG框架包含三个模块：分层先验提取器（HPE）从BEV掩码和车道关键点序列提取空间和时序先验；区域聚焦解码器（RFD）构建细粒度查询并采样参考点；鲁棒边界点拓扑推理（RBTR）基于边界点查询特征建模车道连接性，并采用拓扑去噪策略。

Result: 在OpenLane-V2基准测试中，TopoFG在subsetA上达到48.0%的OLS，在subsetB上达到45.4%的OLS，创造了新的最先进性能。

Conclusion: 通过将空间和时序先验整合到细粒度查询中，并对边界点拓扑推理应用去噪策略，该方法能够精确建模复杂车道结构并提供可信的拓扑预测。
...
### [127] [ReaSon: Reinforced Causal Search with Information Bottleneck for Video Understanding](https://arxiv.org/abs/2511.12530)
...
<details>
  <summary>Details</summary>
Motivation: 视频理解需要选择既信息丰富又因果决定性的关键帧，但现有方法在有限输入token和视频帧时间稀疏性的约束下效果有限

Method: 提出因果信息瓶颈(CIB)原则，使用可学习策略网络从候选帧中选择关键帧以确保预测充分性，通过反事实干预评估因果必要性，设计复合奖励进行强化学习

Result: 在NExT-QA、EgoSchema和Video-MME数据集上的实验表明，ReaSon在有限帧设置下持续优于现有最先进方法

Conclusion: ReaSon框架通过因果信息瓶颈原则有效解决了关键帧选择问题，具有良好的有效性和泛化能力
...
### [128] [HiGFA: Hierarchical Guidance for Fine-grained Data Augmentation with Diffusion Models](https://arxiv.org/abs/2511.12547)
*Zhiguang Lu,Qianqian Xu,Peisong Wen,Siran Da,Qingming Huang*

Main category: cs.CV

TL;DR: 提出了HiGFA方法，通过分层引导的扩散模型增强细粒度视觉分类任务的数据生成质量
...
Method: 利用扩散采样过程的时间动态，在早期阶段使用强文本和轮廓引导建立整体场景和结构，在最后阶段激活专门的细粒度分类器引导，并基于预测置信度动态调制所有引导信号的强度

Result: 在多个细粒度视觉分类数据集上的实验证明了HiGFA的有效性

Conclusion: HiGFA通过分层、置信度驱动的引导编排，能够生成既多样化又忠实于原始类别的合成图像，在全局结构形成和细节精炼之间实现智能平衡
...
### [129] [EmoVerse: A MLLMs-Driven Emotion Representation Dataset for Interpretable Visual Emotion Analysis](https://arxiv.org/abs/2511.12554)
*Yijie Guo,Dexiang Hong,Weidong Chen,Zihan She,Cheng Ye,Xiaojun Chang,Zhendong Mao*

Main category: cs.CV

TL;DR: EmoVerse是一个大规模开源视觉情感分析数据集，通过背景-属性-主体三元组分解情感，提供可解释的情感推理，包含219k图像和双标注系统。


<details>
  <summary>Details</summary>
Motivation: 现有视觉情感分析研究缺乏开源可解释数据集，通常为整张图像分配单一离散情感标签，无法揭示视觉元素如何贡献情感。

Method: 提出EmoVerse数据集，采用多层级知识图谱式标注，将情感分解为B-A-S三元组并定位到视觉区域；开发多阶段标注流程确保可靠性；构建可解释模型将视觉线索映射到情感空间。

Result: 创建了包含219k图像的大规模数据集，支持词级和主体级情感推理，提供分类情感状态和维度情感空间的双重标注。
...
### [130] [OPFormer: Object Pose Estimation leveraging foundation model with geometric encoding](https://arxiv.org/abs/2511.12614)
*Artem Moroz,Vít Zeman,Martin Mikšík,Elizaveta Isianova,Miroslav David,Pavel Burget,Varun Burde*

Main category: cs.CV

TL;DR: 提出了一个统一框架，将物体检测和姿态估计与灵活的物体注册过程集成。系统支持基于3D CAD模型或从多视角图像重建神经表示来注册物体，然后使用CNOS检测器和OPFormer姿态估计模块实现精确的6D姿态估计。


<details>
  <summary>Details</summary>
Motivation: 解决现有方法在物体检测和姿态估计中缺乏统一框架的问题，特别是在没有3D CAD模型的情况下，需要一种能够从多视角图像快速重建物体表示的方法。

Method: 1) 注册阶段：从3D CAD模型或多视角图像重建神经表示；2) 检测阶段：使用CNOS检测器定位目标物体；3) 姿态估计：使用基于transformer的OPFormer模块，结合基础模型特征提取、多视角模板编码和NOCS几何先验，建立2D-3D对应关系。

Result: 在BOP基准测试中表现出色，在准确性和效率之间取得了良好平衡，适用于基于模型和无模型的场景。

Conclusion: 该集成系统提供了一个实用且高效的解决方案，能够处理不同物体表示形式，在实际应用中具有广泛适用性。
...
### [131] [SEMC: Structure-Enhanced Mixture-of-Experts Contrastive Learning for Ultrasound Standard Plane Recognition](https://arxiv.org/abs/2511.12559)
...
<details>
  <summary>Details</summary>
Motivation: 现有方法未能有效利用浅层结构信息，且难以通过图像增强生成的对比样本捕捉细粒度语义差异，导致超声标准平面识别效果不理想。

Method: 提出SEMC框架，包含语义结构融合模块(SSFM)和多专家对比识别模块(MCRM)，前者融合多尺度结构信息，后者使用MoE机制进行分层对比学习和分类。

Result: 在内部数据集和两个公共数据集上的实验结果表明，SEMC在各项指标上均优于当前最先进方法。

Conclusion: SEMC框架通过结构增强和多专家对比学习，显著提升了超声标准平面识别的性能，并构建了一个大规模肝脏超声数据集。
...
### [132] [C3Net: Context-Contrast Network for Camouflaged Object Detection](https://arxiv.org/abs/2511.12627)
*Baber Jan,Aiman H. El-Maleh,Abdul Jabbar Siddiqui,Abdul Bais,Saeed Anwar*

Main category: cs.CV

TL;DR: 提出C3Net用于伪装目标检测，通过双路径解码器架构解决六个核心挑战，在多个数据集上达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 伪装目标检测面临传统方法和基础模型都无法有效处理的挑战，需要专门架构来解决内在相似性、边缘破坏、尺度变化等复合问题。

Method: 采用双路径解码器：边缘精化路径使用梯度初始化边缘增强模块恢复精确边界；上下文定位路径使用图像上下文引导机制抑制内在显著性；通过注意力融合模块协同整合两个路径。

Result: 在COD10K数据集上S-measure达到0.898，CAMO达到0.904，NC4K达到0.913，均达到最先进水平，同时保持高效处理。

Conclusion: C3Net证明复杂多方面的检测挑战需要架构创新，专门组件协同工作才能实现全面覆盖，超越孤立改进。
...
### [133] [Through-Foliage Surface-Temperature Reconstruction for early Wildfire Detection](https://arxiv.org/abs/2511.12572)
*Mohamed Youssef,Lukas Brunner,Klaus Rundhammer,Gerald Czech,Oliver Bimber*

Main category: cs.CV

TL;DR: 提出了一种结合信号处理和机器学习的新方法，通过遮挡的森林植被重建地表温度，用于无人机自主野火监测，能够在地面火灾产生可见烟雾或火焰前进行早期检测。


<details>
  <summary>Details</summary>
Motivation: 实现完全自动化的空中野火监测，使自主无人机能够早期检测地面火灾，在烟雾或火焰可见之前发现火情。

Method: 训练视觉状态空间模型从热模糊数据中恢复部分遮挡土壤和火灾热点的细微热信号；使用潜在扩散模型和向量量化生成大量真实地表温度模拟数据；通过温度增强和程序化热森林模拟扩展数据集。

Result: 在模拟数据上，相比传统热成像和未校正的SA成像，RMSE降低了2到2.5倍；在野外实验中，对高温热点的改进更为显著，相比传统热成像RMSE增益达12.8倍，相比未校正SA图像增益达2.6倍。

Conclusion: 该方法能够重建火灾和人体特征的完整形态，而传统成像方法因部分遮挡而失效，展示了在搜索救援等其他热信号应用中的泛化能力。
...
### [134] [Beyond Pixels: Semantic-aware Typographic Attack for Geo-Privacy Protection](https://arxiv.org/abs/2511.12575)
*Jiayi Zhu,Yihao Huang,Yue Cao,Xiaojun Jia,Qing Guo,Felix Juefei-Xu,Geguang Pu,Bin Wang*

Main category: cs.CV

TL;DR: 提出了一种基于文本攻击的地理隐私保护方法，通过在图像外部添加欺骗性文本来干扰大型视觉语言模型的地理位置推断，相比传统对抗性扰动能更好地保持图像视觉质量。


<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型能够从社交媒体图片中推断用户地理位置，构成严重的隐私威胁。现有的对抗性图像扰动方法需要强失真才能有效，这会显著降低图像视觉质量。

Method: 采用两阶段语义感知的文本攻击方法，在图像视觉内容外部添加欺骗性文本，通过研究有效的文本语义来干扰地理位置推断。
...
### [135] [Multivariate Diffusion Transformer with Decoupled Attention for High-Fidelity Mask-Text Collaborative Facial Generation](https://arxiv.org/abs/2511.12631)
...
Method: 提出MDiTFace框架：1）统一的标记化策略处理语义掩码和文本输入；2）堆叠的多变量变换器块同步处理所有条件；3）新颖的解耦注意力机制，将内部计算分离为动态和静态路径，可缓存和重用静态路径特征。
...
<details>
  <summary>Details</summary>
Motivation: 解决长视频生成中的时间一致性和计算效率问题，传统方法难以同时保证长程时间连贯性和高效合成。

Method: 首先生成低帧率视频作为粗粒度蓝图，然后逐步提高帧率细化视觉细节和运动连续性；采用双向注意力机制和跨帧率自回归生成。

Result: 在长视频生成任务中达到了新的最先进水平，在视觉质量和时间质量方面都表现出色。

Conclusion: TempoMaster通过帧率渐进式生成策略，成功实现了长视频的高质量高效生成，为长视频生成提供了新的解决方案。
...
### [137] [BridgeEQA: Virtual Embodied Agents for Real Bridge Inspections](https://arxiv.org/abs/2511.12676)
*Subin Varghese,Joshua Gao,Asad Ur Rahman,Vedhus Hoskere*

Main category: cs.CV

TL;DR: 提出了BridgeEQA基准测试，用于评估具身智能体在真实桥梁检查场景中的问答能力，并开发了EMVR方法来解决多尺度推理和长距离空间理解问题。


<details>
  <summary>Details</summary>
Motivation: 解决现有具身问答基准在真实世界部署中的不足，特别是缺乏能够捕捉实际操作条件的测试环境。桥梁检查领域自然地需要多尺度推理、长距离空间理解和复杂语义关系。

Method: 构建了包含2,200个开放词汇问答对的BridgeEQA基准，基于200个真实桥梁场景和47.93张平均图像。提出了EMVR方法，将检查建模为基于图像场景图的顺序导航过程，使用马尔可夫决策过程进行视图遍历、证据比较和推理。

Result: 评估显示最先进的视觉语言模型在情节记忆EQA设置下存在显著性能差距。EMVR方法在基准测试中表现出优于基线模型的性能。

Conclusion: BridgeEQA为具身问答提供了现实且具有挑战性的测试平台，EMVR方法展示了在复杂检查任务中的有效性，为实际部署具身智能体提供了重要进展。
...
<details>
  <summary>Details</summary>
Motivation: IHC图像细胞计数对癌症诊断至关重要，但现有方法难以处理染色重叠、形态多样性和多类计数问题，基础模型在此领域的潜力尚未充分挖掘

Method: 采用排名感知的聚集框架，设计RATS策略进行样本级教师选择，并通过视觉-语言对齐的多模态微调实现多类细胞密度图回归

Result: 在12种IHC生物标志物和5种组织类型上超越现有最优方法，与病理学家评估高度一致，在H&E染色数据上也表现良好

Conclusion: CountIHC通过有效利用基础模型的互补表示，解决了IHC细胞计数的关键挑战，具有很好的可扩展性
...
### [139] [R$^{2}$Seg: Training-Free OOD Medical Tumor Segmentation via Anatomical Reasoning and Statistical Rejection](https://arxiv.org/abs/2511.12691)
...
<details>
  <summary>Details</summary>
Motivation: 医学图像分割基础模型在分布外数据上表现不佳，容易产生碎片化的假阳性结果，需要提升模型在OOD肿瘤分割中的鲁棒性。

Method: 采用两阶段方法：1) 推理阶段使用LLM引导的解剖推理规划器定位器官锚点并生成多尺度ROI；2) 拒绝阶段对基础模型生成的候选区域进行双样本统计检验，仅保留与正常组织显著不同的候选。

Result: 在多中心多模态肿瘤分割基准测试中，R²Seg显著提升了Dice系数、特异性和灵敏度，优于强基线方法和原始基础模型。
...
<details>
  <summary>Details</summary>
Motivation: 视觉自回归建模策略在图像生成方面已取得进展，但其在需要精确空间感知的分割任务中的潜力尚未被探索。

Method: 提出Seg-VAR框架，包含三个核心组件：图像编码器、空间感知的seglat编码器（将分割掩码映射为离散潜在标记）和解码器。采用多阶段训练策略：先通过图像-seglat联合训练学习seglat表示，然后精炼潜在变换，最后对齐图像编码器导出的潜在与seglat分布。
...
### [141] [HEDGE: Hallucination Estimation via Dense Geometric Entropy for VQA with Vision-Language Models](https://arxiv.org/abs/2511.12693)
*Sushant Gautam,Michael A. Riegler,Pål Halvorsen*

Main category: cs.CV

TL;DR: HEDGE是一个统一的幻觉检测框架，通过视觉扰动、语义聚类和不确定性指标来检测视觉语言模型中的幻觉问题。
...
Method: 结合受控视觉扰动、语义聚类（基于蕴含和嵌入）和鲁棒不确定性指标，构建可复现的检测流程。

Result: 在VQA-RAD和KvasirVQA-x1数据集上评估显示，统一融合模型（如Qwen2.5-VL）检测效果最好，VASE指标最鲁棒，嵌入聚类对生成答案分离效果更好。
...
Method: 采用师生框架，CNN教师模型优化ViT学生模型，并集成LoRA进行微调以减少计算成本。

Result: 在由三个公开人脸数据集构建的形态数据集上进行了广泛实验，包含十种不同的形态生成算法。与六种最先进的S-MAD技术相比，该方法表现出优越的检测性能和计算效率。

Conclusion: 所提出的方法在形态攻击检测方面具有优越性能和计算效率。
...
### [143] [Pixels or Positions? Benchmarking Modalities in Group Activity Recognition](https://arxiv.org/abs/2511.12606)
...
<details>
  <summary>Details</summary>
Motivation: 目前缺乏标准化的基准来对齐广播视频和追踪数据以进行群体活动识别，无法公平比较这两种模态的性能。

Method: 构建了基于2022年世界杯64场比赛的多模态数据集，包含94,285个同步标注的群体活动；提出了基于视频的分类器和基于追踪的图神经网络分类器，特别是引入了角色感知图架构来编码战术结构。
...
### [144] [Which Way from B to A: The role of embedding geometry in image interpolation for Stable Diffusion](https://arxiv.org/abs/2511.12757)
*Nicholas Karris,Luke Durell,Javier Flores,Tegan Emerson*

Main category: cs.CV

TL;DR: 论文发现Stable Diffusion对CLIP嵌入矩阵具有置换不变性，因此可将嵌入视为Wasserstein空间中的点云而非欧几里得空间中的矩阵。通过将嵌入插值问题重新定义为最优传输问题，可以计算嵌入之间的最短路径，从而在Stable Diffusion中生成更平滑、更连贯的插值图像。


<details>
  <summary>Details</summary>
Motivation: Stable Diffusion对CLIP嵌入矩阵的置换不变性表明这些嵌入更适合被解释为Wasserstein空间中的点云，而非欧几里得空间中的矩阵。这为理解嵌入空间的几何结构提供了新视角。

Method: 将嵌入插值问题重新定义为最优传输问题，通过求解该问题计算嵌入之间的最短路径（测地线），从而在嵌入空间中实现更自然和几何平滑的过渡。

Result: 实验表明，基于最优传输的方法相比其他标准插值方法能够生成更平滑的插值图像，验证了将嵌入视为点云能更好地反映和利用嵌入空间的几何结构。

Conclusion: 将CLIP嵌入视为Wasserstein空间中的点云而非欧几里得空间中的矩阵，能够更好地捕捉嵌入空间的几何特性，从而在Stable Diffusion中实现更高质量的图像插值。
...
TL;DR: 提出了一种分层阶梯网络和注意力仿射网络，用于在测试时适应中处理分布外样本和领域偏移问题，通过加权概率融合和动态熵抑制机制提升模型性能。


<details>
  <summary>Details</summary>
Motivation: 解决测试时适应方法在遇到分布外样本时性能显著下降的问题，避免分布外样本被误分类为已知类别而影响后续适应过程。

Method: 使用分层阶梯网络从所有Transformer层提取分布外特征，通过加权概率融合增强检测性能；引入注意力仿射网络自适应调整自注意力机制；采用加权熵机制动态抑制低置信度样本的影响。

Result: 在基准数据集上的实验结果表明，该方法显著提升了最常用分类数据集的性能。

Conclusion: 所提出的方法有效解决了测试时适应中的分布外样本和领域偏移问题，显著提升了模型在真实场景中的适应性和分类性能。
...
### [146] [Lightweight Optimal-Transport Harmonization on Edge Devices](https://arxiv.org/abs/2511.12785)
*Maria Larchenko,Dmitry Guskov,Alexander Lobashev,Georgy Derevyanko*

Main category: cs.CV

TL;DR: 提出了一种轻量级的颜色协调方法MKL-Harmonizer，支持设备端实时推理，用于增强现实中的无缝图像合成。


<details>
  <summary>Details</summary>
Motivation: 解决增强现实中颜色协调算法缺乏实时解决方案的问题，使颜色协调能够集成到AR流程中。

Method: 利用经典最优传输理论，训练一个紧凑的编码器来预测Monge-Kantorovich传输映射。

Result: 在真实AR合成图像上，该方法获得了最佳综合得分，超越了现有最先进方法。

Conclusion: 成功开发了支持设备端推理的轻量级颜色协调方法，并发布了专用的AR合成图像数据集和工具包。
...
### [147] [MSRNet: A Multi-Scale Recursive Network for Camouflaged Object Detection](https://arxiv.org/abs/2511.12810)
*Leena Alghamdi,Muhammad Usman,Hafeez Anwar,Abdul Bais,Saeed Anwar*

Main category: cs.CV

TL;DR: 提出了一种多尺度递归网络（MSRNet），通过金字塔视觉变换器提取多尺度特征，结合注意力机制和递归解码策略，在伪装目标检测任务中实现了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 伪装目标检测是一个具有挑战性的计算机视觉任务，现有方法在复杂场景中（特别是小目标和多目标情况下）的检测精度仍有待提升。

Method: 使用金字塔视觉变换器作为骨干网络提取多尺度特征，通过注意力机制进行特征融合，采用递归解码策略和多重粒度融合单元优化特征表示。

Result: 在两个基准数据集上达到了最先进的性能，在另外两个数据集上排名第二，能够成功检测小目标和多个伪装目标。

Conclusion: 通过多尺度学习和递归特征优化的联合策略，提出的方法在伪装目标检测任务中表现出色，为复杂场景下的目标检测提供了有效解决方案。
...
TL;DR: 提出了Denoising-VAE，一种基于ViT的自编码器，通过频谱自正则化策略抑制高维潜在空间中的冗余高频噪声，从而改善生成质量和训练收敛速度。
...
Method: 提出频谱自正则化策略来抑制冗余高频噪声同时保持重建质量；引入频谱对齐策略来促进基于Denoising-VAE的生成模型优化；构建不依赖VFMs的ViT自编码器。

Result: 在ImageNet 256×256基准上，扩散模型收敛速度比SD-VAE快约2倍，达到最先进的重建质量（rFID=0.28，PSNR=27.26）和竞争性生成性能（gFID=1.82）。

Conclusion: Denoising-VAE通过抑制高维潜在空间中的冗余高频噪声，有效解决了VAE的优化困境，显著提升了生成质量和训练效率。
...
### [149] [SAGA: Source Attribution of Generative AI Videos](https://arxiv.org/abs/2511.12834)
*Rohit Kundu,Vishal Mohanty,Hao Xiong,Shan Jia,Athula Balachandran,Amit K. Roy-Chowdhury*

Main category: cs.CV

TL;DR: SAGA是首个用于大规模AI生成视频来源溯源的框架，通过多粒度归因识别生成模型，仅需0.5%的标注数据即可达到全监督性能，并提供可解释的时间注意力签名。


<details>
  <summary>Details</summary>
Motivation: 生成式AI的普及导致超逼真合成视频泛滥，传统二元真伪检测器已无法应对滥用风险，迫切需要能够识别具体生成模型的溯源方法。

Method: 采用新颖的视频transformer架构，利用鲁棒视觉基础模型特征捕捉时空伪影；提出数据高效的预训练-归因策略；引入时间注意力签名(T-Sigs)进行可解释性分析。

Result: 在公共数据集上的广泛实验表明，SAGA在合成视频溯源方面设立了新基准，在跨域场景下表现优异，仅需每类0.5%的源标记数据即可匹配全监督性能。

Conclusion: SAGA为法医和监管应用提供了关键的可解释溯源洞察，是首个能够大规模进行AI生成视频多粒度来源归因的综合框架。
...
TL;DR: 提出CILMP方法，将大型语言模型（LLMs）集成到视觉语言模型（VLMs）的提示调优中，通过提取疾病特定表示并创建实例自适应提示，在医学图像分类任务中实现更好的性能。


<details>
  <summary>Details</summary>
Motivation: 现有提示调优方法无法精确区分不同类型的医学概念，缺乏跨医学成像模态的疾病相关特征。LLMs在提供专业医学知识方面表现出色，因此考虑将其集成到提示调优过程中。

Method: CILMP方法从LLMs提取疾病特定表示，在低秩线性子空间中进行干预，并利用这些表示创建疾病特定提示。加入条件机制，根据每个医学图像生成实例自适应提示。

Result: 在多个医学图像数据集上的广泛实验表明，CILMP始终优于最先进的提示调优方法。

Conclusion: CILMP通过桥接LLMs和VLMs，有效将医学知识转移到VLM提示中，提高了医学图像分类的性能和适应性。
...
TL;DR: DPVO-QAT++是一个分层量化优化框架，通过可学习尺度参数化、视觉里程计前后端异构精度设计以及GPU原生内核融合，显著降低了内存占用并提升了处理速度，同时保持了原始模型的轨迹精度。
...
Method: 采用分层量化优化框架，包括：可学习尺度参数化、视觉里程计前后端异构精度设计（前端浮点伪量化使用FP16/FP32，后端全精度）、以及GPU原生内核融合用于伪量化（自定义CUDA内核）。
...
Conclusion: DPVO-QAT++有效弥合了高精度深度视觉里程计与实际部署效率需求之间的差距，为这项技术在实际嵌入式平台上的应用提供了可行的工程范式。
...
### [152] [Video Finetuning Improves Reasoning Between Frames](https://arxiv.org/abs/2511.12868)
*Ruiqi Yang,Tian Yun,Zihan Wang,Ellie Pavlick*

Main category: cs.CV

TL;DR: 本文提出了视觉思维链(vCoT)方法，通过生成帧间过渡事件描述来增强多模态大语言模型的视频理解能力，发现视频微调模型已隐式掌握时序推理能力。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型从图像扩展到视频时往往只是简单拼接帧标记，缺乏有效的时序推理能力，需要研究视频微调对模型能力的提升作用。

Method: 提出视觉思维链(vCoT)方法，生成连续帧之间的过渡事件描述作为显式推理过程，系统比较仅图像模型与视频微调模型在有/无过渡线索时的表现差异。

Result: vCoT显著提升了仅图像模型在长视频问答任务上的性能，但对视频微调模型增益有限；视频模型能将时序推理能力迁移到静态场景，在关系视觉推理任务上优于图像模型。

Conclusion: 视频微调使模型隐式掌握了帧间过渡推理能力，这种时序推理能力可迁移到静态视觉任务，为多模态模型的时间理解提供了新视角。
...
TL;DR: 提出了FSTS框架，通过分析真实篡改行为模式来合成更逼真的训练数据，显著提升了文本图像伪造定位模型的泛化能力


<details>
  <summary>Details</summary>
Motivation: 现有T-IFL方法泛化能力差，主要原因是真实数据集规模有限，且合成数据无法捕捉真实篡改的复杂性

Method: FSTS框架：收集16,750个真实篡改实例，通过结构化流程记录编辑痕迹；建立分层建模框架，将个体篡改参数表示为基操作-参数配置的组合，群体分布通过聚合这些行为构建；基于傅里叶级数思想进行可解释逼近

Result: 在四个评估协议上的广泛实验表明，使用FSTS数据训练的模型在真实数据集上实现了显著改进的泛化能力

Conclusion: FSTS通过结构化建模真实篡改行为，能够合成更真实多样的训练数据，有效解决了文本图像伪造定位的泛化问题
...
TL;DR: 提出一个高保真、实时的对话数字人系统，结合逼真3D虚拟形象、个性化语音合成和知识驱动的对话生成，通过异步执行管道实现低延迟多模态交互。


<details>
  <summary>Details</summary>
Motivation: 当前数字人类在交互应用中难以同时实现视觉真实性和实时响应性，需要解决这一挑战来支持沉浸式应用。

Method: 采用异步执行管道协调多模态组件，结合检索增强方法（历史增强和意图路由）实现知识访问，支持唤醒词检测、情感韵律和上下文感知响应生成。

Result: 开发出集成的数字人系统，能够实现响应迅速且可信的交互体验，适用于通信、教育和娱乐等沉浸式应用。

Conclusion: 该系统成功解决了数字人类在视觉真实性和实时响应性之间的平衡问题，为沉浸式交互应用提供了可行的技术方案。
...
### [155] [DeepSport: A Multimodal Large Language Model for Comprehensive Sports Video Reasoning via Agentic Reinforcement Learning](https://arxiv.org/abs/2511.12908)
*Junbo Zou,Haotian Xia,Zhen Ye,Shengjie Zhang,Christopher Lai,Vicente Ordonez,Weining Shen,Hanjie Chen*

Main category: cs.CV

TL;DR: DeepSport是首个端到端训练的多模态大语言模型框架，专为多任务、多运动的视频理解设计，通过主动推理和专门帧提取工具实现"用视频思考"，在6.7k问题测试基准上达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 解决体育视频理解中的独特挑战：感知高速动态、理解复杂规则和长时序推理。现有方法要么专注于单一运动，要么局限于特定任务，或依赖缺乏稳健学习推理过程的训练无关范式。

Method: 提出数据蒸馏管道从10个不同数据源合成高质量思维链轨迹，创建78k训练数据集；采用两阶段训练策略：监督微调后接强化学习，使用新颖的门控工具使用奖励来优化推理过程。
...
<details>
  <summary>Details</summary>
Motivation: 解决实时应用中光学流和视差估计的高精度与低延迟需求，平衡推理时间与准确性之间的矛盾

Method: 使用非因果Mamba块模型，融合成对输入图像，在非因果选择性状态空间中进行密集感知任务处理

Result: 模型在保持高精度的同时显著减少了推理时间，降低了GPU使用率，能够生成准确的光学流和视差图

Conclusion: 该模型适用于统一的实时准确3D密集感知估计任务，在实际场景中验证了其有效性
...
TL;DR: 提出了一个任务感知的新视角合成评估框架，使用Zero123模型特征和轻量级调优来增强判别能力，包含基于参考和无参考两个评估指标，能可靠识别错误生成结果并与人类偏好一致。


<details>
  <summary>Details</summary>
Motivation: 现有新视角合成评估指标难以判断生成图像是否既真实又忠实于源视图和视角变换，标准指标经常错误排名，无法捕捉源图像、视角变化和生成输出之间的细微关系。

Method: 利用强大的NVS基础模型Zero123的特征，结合轻量级调优步骤增强判别能力，提出了基于参考的D_PRISM和无参考的MMD_PRISM两个互补评估指标。

Result: 在Toys4K、Google Scanned Objects和OmniObject3D三个基准测试中，MMD_PRISM产生了清晰稳定的模型排名，较低分数始终表示更强的模型，与人类偏好研究一致。

Conclusion: 该框架为新视角合成评估提供了原则性和实用性的方法，解决了NVS评估中的基本差距，为更可靠的新视角合成进展铺平了道路。
...
### [158] [PFAvatar: Pose-Fusion 3D Personalized Avatar Reconstruction from Real-World Outfit-of-the-Day Photos](https://arxiv.org/abs/2511.12935)
*Dianbing Xi,Guoyuan An,Jingsen Zhu,Zhijian Liu,Yuan Liu,Ruiyuan Zhang,Jiayuan Lu,Rui Wang,Yuchi Huo*

Main category: cs.CV

TL;DR: PFAvatar是一种从"今日穿搭"照片重建高质量3D虚拟形象的新方法，通过两阶段流程：基于姿势感知扩散模型的少样本微调和神经辐射场3D虚拟形象蒸馏，实现快速、高保真度的重建。


<details>
  <summary>Details</summary>
Motivation: 现有的3D虚拟形象重建方法通常需要将图像分割为服装、配饰等组件进行3D组装，这种方法容易导致不一致性。同时，传统方法在处理多样姿势、遮挡和复杂背景的"今日穿搭"照片时面临挑战。

Method: 第一阶段：使用预训练的ControlNet进行姿势估计，结合新颖的条件先验保持损失(CPPL)，对姿势感知扩散模型进行少样本端到端微调。第二阶段：通过规范SMPL-X空间采样和多分辨率3D-SDS优化，使用神经辐射场(NeRF)表示3D虚拟形象。

Result: 方法在5分钟内完成个性化，相比先前方法实现48倍加速。在重建保真度、细节保持和对遮挡/截断的鲁棒性方面优于最先进方法，能够正确处理高频纹理（如头发）和遮挡。

Conclusion: PFAvatar推进了从真实世界"今日穿搭"相册生成实用3D虚拟形象的能力，支持虚拟试穿、动画和人体视频重演等下游应用，展现了方法的通用性和实用价值。
...
### [159] [EndoSight AI: Deep Learning-Driven Real-Time Gastrointestinal Polyp Detection and Segmentation for Enhanced Endoscopic Diagnostics](https://arxiv.org/abs/2511.12962)
*Daniel Cavadia*

Main category: cs.CV

TL;DR: EndoSight AI是一个深度学习系统，用于内窥镜手术中实时检测胃肠道息肉，在检测和分割任务上表现出色，并在GPU上实现超过35帧/秒的实时推理速度。
...
Method: 利用公开的Hyper-Kvasir数据集，采用深度学习架构进行息肉定位和边界分割，并引入了热感知程序确保模型鲁棒性。

Result: 系统在息肉检测上达到88.3%的平均精度(mAP)，分割任务中Dice系数高达69%，推理速度超过35帧/秒。
...
### [160] [CalibrateMix: Guided-Mixup Calibration of Image Semi-Supervised Models](https://arxiv.org/abs/2511.12964)
*Mehrab Mustafy Rahman,Jayanth Mohan,Tiberiu Sosea,Cornelia Caragea*

Main category: cs.CV

TL;DR: 本文提出了CalibrateMix方法，通过有针对性的mixup策略来改善半监督学习模型的校准性能，同时保持或提高分类准确率。


<details>
  <summary>Details</summary>
Motivation: 现有的半监督学习方法存在校准不佳的问题，模型会产生过于自信的预测。虽然mixup在监督学习中显示出良好的校准效果，但在半监督学习中由于伪标签的过自信和不可靠性，随机mixup面临挑战。

Method: CalibrateMix利用标注和未标注样本的训练动态来识别'易学'和'难学'样本，然后在这些样本之间进行有针对性的mixup操作。

Result: 在多个基准图像数据集上的实验结果表明，该方法相比现有半监督学习方法实现了更低的期望校准误差（ECE）和更高的准确率。

Conclusion: CalibrateMix是一种有效的半监督学习校准方法，能够显著改善模型的校准性能而不牺牲分类准确率。
...
### [161] [UNSEEN: Enhancing Dataset Pruning from a Generalization Perspective](https://arxiv.org/abs/2511.12988)
*Furui Xu,Shaobo Wang,Jiajun Zhang,Chenghao Sun,Haixiang Tang,Linfeng Zhang*

Main category: cs.CV

TL;DR: 提出UNSEEN框架，从泛化角度进行数据集剪枝，通过使用未见过样本的模型进行评分，解决传统方法在训练阶段评分区分度不足的问题。


<details>
  <summary>Details</summary>
Motivation: 深度学习数据集规模增长带来计算挑战，传统数据集剪枝方法依赖训练阶段的模型性能评分，导致样本评分分布密集、区分度不足。

Method: 从泛化角度进行数据集剪枝，使用未在训练中见过的样本来评分；提出UNSEEN框架，可集成到现有方法中；扩展到多步场景，通过不同coreset训练的模型进行增量选择。

Result: 在CIFAR-10、CIFAR-100和ImageNet-1K上显著优于现有SOTA方法，在ImageNet-1K上减少30%训练数据仍能实现无损性能。

Conclusion: UNSEEN框架通过泛化视角和多步优化，有效提升了数据集剪枝的性能，解决了传统方法评分区分度不足的问题。
...
<details>
  <summary>Details</summary>
Motivation: 尽管状态空间模型（特别是Mamba架构）在序列建模中表现出色，但它们缺乏类似Transformer的透明注意力机制，因此理解这些视觉SSMs如何处理空间信息仍然具有挑战性。

Method: 提出了两种互补的方法：适用于任何SSM架构的基于雅可比矩阵的方法，通过完整的状态传播链测量影响；以及针对对角SSMs的基于格拉姆矩阵的方法，通过闭式解析解实现更快的速度。两种方法都在单次前向传播中运行，具有线性复杂度。

Result: 在三种不同的医学成像模态上验证了该框架，表明SSMs自然实现了从早期层的扩散低层纹理到深层层的聚焦、临床有意义模式的分层特征细化。分析揭示了与诊断标准对齐的领域特定可控性特征、跨网络层次结构的渐进空间选择性，以及扫描策略对注意力模式的显著影响。

Conclusion: 该框架将可控性分析确立为跨所有领域的SSMs的统一、基础可解释性范式，在计算机视觉、自然语言处理和跨领域任务中具有广泛应用。
...
### [163] [SAGE: Spuriousness-Aware Guided Prompt Exploration for Mitigating Multimodal Bias](https://arxiv.org/abs/2511.13005)
*Wenqian Ye,Di Wang,Guangtao Zheng,Bohan Liu,Aidong Zhang*

Main category: cs.CV

TL;DR: 提出了SAGE方法，通过引导式提示选择来缓解CLIP模型中的多模态伪相关偏差，无需训练或微调即可提升零样本分类的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: CLIP模型在多模态学习中存在伪相关偏差，即模型倾向于依赖虚假特征（如背景与物体的频繁共现）而非核心特征进行推断，这显著影响了模型在分布外数据上的鲁棒性。现有方法通常需要下游数据微调或先验偏差知识，限制了CLIP的开箱即用性。

Method: 提出SAGE方法：首先理论分析多模态伪相关偏差在零样本分类中的影响，然后通过引导式提示选择探索提示模板空间，选择能诱导最大类间语义分离的提示，从而改善最差组鲁棒性。

Result: 在4个真实世界基准数据集和5个流行骨干模型上的广泛实验表明，SAGE一致地提升了零样本性能和泛化能力，在无需外部知识或模型更新的情况下优于之前的零样本方法。

Conclusion: SAGE是一种简单有效的零样本方法，能够缓解CLIP模型中的多模态伪相关偏差，提高模型在分布外数据上的鲁棒性，同时保持开箱即用的优势。
...
### [164] [Counting Through Occlusion: Framework for Open World Amodal Counting](https://arxiv.org/abs/2511.12702)
*Safaeid Hossain Arib,Rabeya Akter,Abdul Monaf Chowdhury,Md Jubair Ahmed Sourov,Md Mehedi Hasan*

Main category: cs.CV

TL;DR: CountOCC是一个显式重构遮挡目标特征的无模态计数框架，通过多模态引导和视觉等价性目标，在遮挡条件下实现准确的物体计数。


<details>
  <summary>Details</summary>
Motivation: 现有物体计数方法在遮挡条件下表现不佳，因为主干网络会编码遮挡表面而非目标物体，导致特征表示被破坏。

Method: 通过整合可见片段的空间上下文与文本和视觉嵌入的语义先验，在多个金字塔层级上合成完整的特征表示，并引入视觉等价性目标确保遮挡和未遮挡视图产生空间对齐的注意力图。

Result: 在FSC 147数据集上，验证集和测试集的MAE分别降低26.72%和20.80%；在CARPK数据集上MAE降低49.89%；在CAPTUREReal数据集上MAE降低28.79%。

Conclusion: CountOCC通过互补机制保留了在遮挡条件下准确计数所需的判别性特征，在多个数据集上实现了最先进的性能。
...
### [165] [MeanFlow Transformers with Representation Autoencoders](https://arxiv.org/abs/2511.13019)
*Zheyuan Hu,Chieh-Hsin Lai,Ge Wu,Yuki Mitsufuji,Stefano Ermon*

Main category: cs.CV

TL;DR: 提出了一种在表示自动编码器（RAE）潜在空间中训练MeanFlow的高效方法，通过一致性中期训练和两阶段方案解决梯度爆炸问题，无需复杂引导即可实现高效生成。


<details>
  <summary>Details</summary>
Motivation: 传统MeanFlow在SD-VAE潜在空间中训练计算量大且不稳定，推理时解码器成本高，且依赖复杂的引导超参数进行类别条件生成。

Method: 在RAE潜在空间中训练MeanFlow，采用一致性中期训练进行轨迹感知初始化，使用两阶段方案：从预训练流匹配教师蒸馏加速收敛，然后通过单点速度估计器进行可选的自举阶段减少偏差。

Result: 在ImageNet 256上实现1步FID 2.03，优于vanilla MF的3.43，采样GFLOPS减少38%，总训练成本降低83%；在ImageNet 512上达到1步FID 3.23，在所有基线中GFLOPS最低。

Conclusion: 该方法显著提高了MeanFlow的训练效率和稳定性，简化了训练配置，在保持生成质量的同时大幅降低了计算成本。
...
### [166] [FSDAM: Few-Shot Driving Attention Modeling via Vision-Language Coupling](https://arxiv.org/abs/2511.12708)
...
<details>
  <summary>Details</summary>
Motivation: 现有模型依赖大规模注视数据集来学习驾驶员注意力模式，但这些数据集收集和整理成本高昂。

Method: 采用双路径架构，分别处理空间预测和描述生成，通过跨模态对齐保持语义一致性。

Result: 在注意力预测方面表现竞争性，生成连贯且上下文感知的解释，并在多个驾驶基准测试中展示强大的零样本泛化能力。

Conclusion: 有效注意力条件生成可在有限监督下实现，为数据受限场景中可解释驾驶员注意力系统的实际部署开辟新可能性。
...
### [167] [SpectralAdapt: Semi-Supervised Domain Adaptation with Spectral Priors for Human-Centered Hyperspectral Image Reconstruction](https://arxiv.org/abs/2511.13020)
*Yufei Wen,Yuting Zhang,Jingdan Kang,Hao Ren,Weibin Cheng,Jintai Chen,Kaishun Wu*

Main category: cs.CV

TL;DR: 提出了SpectralAdapt半监督域适应框架，通过光谱密度掩码和光谱端元表示对齐技术，解决人体HSI数据稀缺问题，提升从RGB到HSI的重建质量。


<details>
  <summary>Details</summary>
Motivation: 解决医疗应用中人体HSI数据稀缺的问题，利用丰富的通用领域HSI数据，通过域适应技术弥合通用与人体中心HSI数据集之间的域差距。

Method: 提出SpectralAdapt框架：1）光谱密度掩码(SDM)基于光谱复杂度自适应掩码RGB通道；2）光谱端元表示对齐(SERA)从标记像素提取物理可解释端元作为域不变锚点；3）半监督域适应结合有限标签和大量未标记数据。

Result: 在基准数据集上实验显示，在光谱保真度、跨域泛化能力和训练稳定性方面均有持续改进。

Conclusion: SSDA是医疗高光谱成像的有效解决方案，能够缓解域偏移、光谱退化和数据稀缺问题。
...
### [168] [Backdoor Attacks on Open Vocabulary Object Detectors via Multi-Modal Prompt Tuning](https://arxiv.org/abs/2511.12735)
...
Method: 提出TrAP多模态后门注入策略，采用课程式训练策略逐步缩小触发器尺寸，通过轻量级可学习提示令牌植入恶意行为。

Result: 在多个数据集上的实验表明，TrAP在目标误分类和消失攻击中均达到高攻击成功率，同时在下游数据集上相比零样本设置提高了干净图像性能。

Conclusion: TrAP证明了OVODs存在严重的安全漏洞，能够在保持泛化能力的同时嵌入隐藏后门，对实际应用构成威胁。
...
### [169] [Direct Visual Grounding by Directing Attention of Visual Tokens](https://arxiv.org/abs/2511.12738)
*Parsa Esmaeilkhani,Longin Jan Latecki*

Main category: cs.CV

TL;DR: 本文提出了一种新的注意力监督损失函数KLAL，通过直接监督视觉标记的注意力分布来改善视觉语言模型在视觉任务中的表现。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型中，与查询最相关的视觉标记在LLM模块的最终层很少受到答案标记的关注，这可能导致视觉问答错误。标准的下一标记预测损失无法有效引导对视觉标记的注意力。

Method: 提出KL注意力损失(KLAL)，通过KL散度将视觉标记的注意力分布与真实注意力图对齐。真实注意力图来自合成案例的任务几何或真实图像中的标注，无需额外标签。

Result: KLAL与NTP结合使用时，在几何任务、指向和指代表达理解等任务上均取得显著改进，在合成和真实数据上都表现出色。

Conclusion: 直接监督视觉标记的注意力分布能有效提升VLMs在视觉任务中的性能，特别是在需要精确视觉定位的任务上。
...
### [170] [Rethinking Saliency Maps: A Cognitive Human Aligned Taxonomy and Evaluation Framework for Explanations](https://arxiv.org/abs/2511.13081)
*Yehonatan Elisha,Seffi Cohen,Oren Barkan,Noam Koenigstein*

Main category: cs.CV

TL;DR: 提出了RFxG分类法，将显著性解释按参考框架（点对点vs对比）和粒度（细粒度vs粗粒度）两个维度组织，并开发了新的评估指标来系统评估解释方法的质量。


<details>
  <summary>Details</summary>
Motivation: 显著性图在深度学习视觉解释中广泛使用，但对其预期目的和与不同用户查询的对应关系缺乏共识，这种模糊性阻碍了解释方法的有效评估和实际效用。

Method: 引入RFxG分类法框架，提出四个新的忠实度指标，对10种最先进的显著性方法、4种模型架构和3个数据集进行综合评估。

Result: 发现现有评估指标过度关注点对点忠实度，而忽视对比推理和语义粒度，通过新指标揭示了现有方法的局限性。

Conclusion: 通过转向用户意图驱动的评估，为开发既忠实于模型行为又与人类理解和查询复杂性有意义的视觉解释提供了概念基础和实践工具。
...
### [171] [Deep Imbalanced Multi-Target Regression: 3D Point Cloud Voxel Content Estimation in Simulated Forests](https://arxiv.org/abs/2511.12740)
*Amirhossein Hassanzadeh,Bartosz Krawczyk,Michael Saunders,Rob Wible,Keith Krause,Dimah Dera,Jan van Aardt*

Main category: cs.CV

TL;DR: 该研究探索了从体素化的LiDAR点云数据中推断低级别体素内容信息（目标占用百分比）的可行性，提出了一种基于KPConv的多目标回归方法，并针对类别不平衡问题采用了密度相关性和多种优化技术。


<details>
  <summary>Details</summary>
Motivation: 体素化虽然能降低LiDAR数据处理的计算成本，但会导致细尺度结构信息丢失。研究旨在探索是否可以从高级别的体素化LiDAR数据中恢复低级别的体素内容信息，特别是森林环境中不同目标（树皮、树叶、土壤等）的占用百分比。

Method: 提出基于KPConv的多目标回归方法，采用密度相关性（DBR）处理类别不平衡问题，使用加权MSE、Focal回归和正则化来优化KPConv。对体素尺寸（0.25-2米）进行敏感性分析，评估不同网格表示对森林细节捕捉的影响。

Result: 敏感性分析显示，较大体素尺寸（如2米）由于变异性降低而误差较小，而较小体素尺寸（如0.25或0.5米）误差较高，特别是在树冠区域变异性最大的地方。对于树皮和树叶目标，小体素尺寸数据集的误差值显著高于大体素尺寸数据集。
...
<details>
  <summary>Details</summary>
Motivation: 现有显著性引导方法主要依赖图像空间，但研究表明模型内部机制可能不可靠。本文提出将引导从图像空间转移到潜在空间，以更有效地整合人类感知先验。

Method: 使用对比嵌入方法，对输入应用显著性保持和显著性退化增强，通过对比三元组损失引导模型关注显著特征、远离非显著特征，并对logit分布进行完整性检查。

Result: 在开放集和封闭集场景下，SAGE相比最先进的显著性方法提升了分类性能，并在不同骨干网络和任务中展现出良好的泛化能力。

Conclusion: SAGE通过在潜在空间中使用对比嵌入整合人类显著性指导，有效提升了模型性能，证明了该方法在不同架构和任务中的广泛适用性。
...
### [173] [Shedding Light on VLN Robustness: A Black-box Framework for Indoor Lighting-based Adversarial Attack](https://arxiv.org/abs/2511.13132)
*Chenyang Li,Wenbing Tang,Yihao Huang,Sinong Simon Zhan,Ming Hu,Xiaojun Jia,Yang Liu*

Main category: cs.CV

TL;DR: 提出了ILA框架，通过操纵室内光照来攻击视觉语言导航(VLN)代理，包括静态光照攻击(SILA)和动态光照攻击(DILA)，显著提高了VLN代理的失败率。


<details>
  <summary>Details</summary>
Motivation: 现有对抗性评估依赖不常见的纹理扰动，缺乏实际意义。室内光照是影响导航的重要但被忽视的场景属性，更符合真实世界情况。

Method: 设计黑盒攻击框架ILA，包含两种模式：SILA(恒定光照强度)和DILA(关键时刻开关灯造成光照突变)，基于典型家庭照明使用模式。

Result: 在两个最先进的VLN模型和三个导航任务上评估，ILA显著增加了失败率并降低了轨迹效率，揭示了VLN代理对真实室内光照变化的脆弱性。

Conclusion: VLN代理对室内光照变化存在先前未被识别的脆弱性，ILA框架为评估导航系统在真实环境中的鲁棒性提供了更实用的方法。
...
### [174] [Automated Road Distress Detection Using Vision Transformersand Generative Adversarial Networks](https://arxiv.org/abs/2511.13145)
*Cesar Portocarrero Rodriguez,Laura Vandeweyen,Yosuke Yamamoto*

Main category: cs.CV

TL;DR: 该研究探索使用计算机视觉技术进行道路损坏分割，评估GAN生成合成数据的有效性，并比较CNN和MaskFormer模型的性能。结果显示GAN数据能提升模型表现，MaskFormer在mAP50和IoU指标上优于CNN。


<details>
  <summary>Details</summary>
Motivation: 美国道路系统状况评级为D级，传统道路检测方法效率低下且成本高昂。随着自动驾驶车辆实时视觉数据的可用性增加，有机会应用计算机视觉方法进行先进的道路监控，指导基础设施修复工作。
...
Conclusion: 计算机视觉方法特别是MaskFormer模型结合GAN生成数据，为道路损坏检测提供了更有效的解决方案，有助于改善道路基础设施的管理和维护效率。
...
### [175] [RoCoISLR: A Romanian Corpus for Isolated Sign Language Recognition](https://arxiv.org/abs/2511.12767)
*Cătălin-Alexandru Rîpanu,Andrei-Theodor Hotnog,Giulia-Stefania Imbrea,Dumitru-Clementin Cercel*

Main category: cs.CV

TL;DR: 本文介绍了RoCoISLR数据集，这是首个用于罗马尼亚孤立手语识别的大规模标准化数据集，包含超过9000个视频样本，涵盖近6000个标准化手势。作者评估了7种最先进的视频识别模型，发现基于Transformer的架构表现最佳。
...
Method: 创建了RoCoISLR数据集，包含9000多个视频样本，涵盖近6000个标准化手势。在一致的实验设置下评估了7种最先进的视频识别模型：I3D、SlowFast、Swin Transformer、TimeSformer、Uniformer、VideoMAE和PoseConv3D。

Result: 基于Transformer的架构优于卷积基线模型，其中Swin Transformer达到了34.1%的Top-1准确率。与广泛使用的WLASL2000语料库相比，结果显示了低资源手语中长尾类别分布带来的挑战。

Conclusion: RoCoISLR为系统性罗马尼亚孤立手语识别研究提供了初步基础，并突显了在低资源手语环境中处理长尾类别分布的重要性。
...
### [176] [SOMA: Feature Gradient Enhanced Affine-Flow Matching for SAR-Optical Registration](https://arxiv.org/abs/2511.13168)
...
<details>
  <summary>Details</summary>
Motivation: SAR和光学图像由于成像机制和视觉特征根本不同，像素级配准仍然具有挑战性。虽然深度学习在许多跨模态任务中取得成功，但在SAR-光学配准任务中表现仍不理想。梯度信息在手工描述符中起着关键作用，但在深度学习框架中尚未有效利用。

Method: 提出SOMA框架，包含特征梯度增强器(FGE)和全局-局部仿射流匹配器(GLAM)。FGE通过注意力和重建机制将多尺度、多方向梯度滤波器嵌入特征空间；GLAM在粗到细架构中结合仿射变换和基于流的细化。

Result: 在SEN1-2数据集上CMR@1px提升12.29%，在GFGE_SO数据集上提升18.50%。SOMA表现出强大的鲁棒性，在不同场景和分辨率下泛化良好。

Conclusion: SOMA通过有效利用梯度信息和混合匹配策略，显著提升了SAR-光学图像配准性能，证明了结构梯度先验在跨模态配准中的重要性。
...
### [177] [GeoX-Bench: Benchmarking Cross-View Geo-Localization and Pose Estimation Capabilities of Large Multimodal Models](https://arxiv.org/abs/2511.13259)
*Yushuo Zheng,Jiangyong Ying,Huiyu Duan,Chunyi Li,Zicheng Zhang,Jing Liu,Xiaohong Liu,Guangtao Zhai*

Main category: cs.CV

TL;DR: GeoX-Bench是一个用于评估大型多模态模型在跨视角地理定位和姿态估计任务中能力的综合基准，包含10,859个全景-卫星图像对和755,976个问答对，覆盖128个城市的49个国家。


<details>
  <summary>Details</summary>
Motivation: 尽管大型多模态模型在多种任务中表现出色，但其在跨视角地理定位和姿态估计领域的能力尚未被探索，而这些能力对导航、自动驾驶和户外机器人等领域具有重要价值。

Method: 构建包含10,859个全景-卫星图像对和755,976个问答对的GeoX-Bench数据集，评估25个最先进的大型多模态模型，并探索指令调优对模型能力的提升效果。

Result: 当前大型多模态模型在地理定位任务中表现优异，但在更复杂的姿态估计任务中效果显著下降，而基于GeoX-Bench训练数据进行指令调优可以显著提升模型的跨视角地理感知能力。

Conclusion: GeoX-Bench揭示了大型多模态模型在跨视角地理定位和姿态估计任务中的能力差距，为未来改进提供了重要基准，同时证明指令调优能有效提升模型的地理感知能力。
...
### [178] [Enhancing Neuro-Oncology Through Self-Assessing Deep Learning Models for Brain Tumor Unified Model for MRI Segmentation](https://arxiv.org/abs/2511.12801)
*Andrew Zhou*
...
<details>
  <summary>Details</summary>
Motivation: 现有深度学习方法缺乏不确定性估计和肿瘤周围健康脑结构分割，限制了临床使用。需要统一肿瘤定位与解剖背景，并提供置信度评分。

Method: 在nnUNet中增加体素级不确定性通道，使用BraTS2023数据集训练，同时结合正常和癌症数据集构建统一模型进行全脑结构分割。

Result: 不确定性预测达到0.750的相关性和0.047的RMSD，不影响肿瘤分割精度；全脑结构分割DSC为0.81，肿瘤分割DSC为0.86，关键区域表现稳健。

Conclusion: 该框架首次输出肿瘤在自然环境中的位置并叠加不确定性图谱，为AI辅助手术决策提供关键洞察，帮助评估预测和修正错误。
...
### [179] [Computer Vision based group activity detection and action spotting](https://arxiv.org/abs/2511.13315)
...
TL;DR: 提出了一种结合深度学习和图关系推理的群体活动检测框架，通过Mask R-CNN进行精确定位，融合掩码信息优化特征表示，构建演员关系图建模交互，使用图卷积网络预测个体动作和群体活动。
...
Method: 使用Mask R-CNN进行演员定位，多骨干网络提取特征，RoIAlign保持空间对齐，融合掩码信息优化特征，构建演员关系图编码外观相似性和位置关系，图卷积网络进行关系推理。
...
### [180] [Semi-Supervised Multi-Task Learning for Interpretable Quality As- sessment of Fundus Images](https://arxiv.org/abs/2511.13353)
...
TL;DR: 提出一种混合半监督学习方法，结合整体质量的人工标签和质量细节的伪标签，在多任务框架下改进视网膜图像质量评估，无需大量手动标注即可获得更可解释的模型。


<details>
  <summary>Details</summary>
Motivation: 现有视网膜图像质量评估工具大多只分类整体图像质量，无法指示采集缺陷以指导重新拍摄，这主要由于详细标注成本高昂。

Method: 使用在小型数据集上训练的教师模型生成伪标签，然后在多任务设置中微调预训练模型，采用ResNet-18骨干网络。

Result: 多任务模型在EyeQ数据集上F1分数达0.875，DeepDRiD数据集上达0.778，优于单任务基线，在大多数细节预测任务中与教师模型性能相当。

Conclusion: 该半监督方法不仅提高了整体质量评估性能，还提供了关于采集条件（光照、清晰度、对比度）的可解释反馈，增强了可解释性且无需额外手动标注成本。
...
TL;DR: 提出了ViCoKD框架，通过跨模态知识蒸馏解决部分重叠多视图动作识别问题，让模态受限的学生模型从多模态教师模型学习知识。


<details>
  <summary>Details</summary>
Motivation: 多传感器系统中，部分重叠视图设置下的动作识别研究不足，现实场景中系统通常只有有限输入模态和序列级标注，而非密集帧级标签。

Method: 使用跨模态适配器和跨模态注意力，让学生模型利用多模态相关性；提出视图感知一致性模块处理视图不对齐问题，通过人类检测掩码和置信度加权的Jensen-Shannon散度强制共视动作的预测对齐。

Result: 在真实世界MultiSensor-Home数据集上的实验表明，ViCoKD在多种骨干网络和环境设置下均优于竞争性蒸馏方法，在受限条件下甚至超越了教师模型。

Conclusion: ViCoKD框架有效解决了部分重叠多视图动作识别中的模态限制和视图不对齐问题，在现实场景中表现出色。
...
### [182] [Generalized Denoising Diffusion Codebook Models (gDDCM): Tokenizing images using a pre-trained diffusion model](https://arxiv.org/abs/2511.13387)
*Fei Kong*
...
Method: 将DDCM中的随机噪声替换为根据预定义规则从特定集合采样的噪声，使该方法能够适用于多种扩散模型。

Result: 在CIFAR-10和LSUN Bedroom数据集上的实验表明，gDDCM成功将DDCM推广到多种扩散模型，并取得了改进的性能。
...
### [183] [Uni-Hand: Universal Hand Motion Forecasting in Egocentric Views](https://arxiv.org/abs/2511.12878)
*Junyi Ma,Wentao Bao,Jingyi Xu,Guanzhong Sun,Yu Zheng,Erhang Zhang,Xieyuanli Chen,Hesheng Wang*

Main category: cs.CV

TL;DR: 提出EgoLoc方法，用于在自我中心视频中零样本定位手-物体接触和分离的时间戳，无需物体掩码和动词-名词分类法，在VR/AR和机器人操作任务中具有应用价值。


<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注交互动作的行为范式（"如何交互"），但更挑战性的手-物体接触和分离关键时刻定位问题（"何时交互"）研究不足，这对混合现实沉浸式体验和机器人运动规划至关重要。

Method: 提出EgoLoc方法，引入手动力学引导采样生成高质量视觉提示，利用视觉语言模型识别接触/分离属性、定位特定时间戳，并提供闭环反馈进行进一步优化。

Result: 在公共数据集和新基准上的综合实验表明，EgoLoc在自我中心视频中实现了可信的时间交互定位，并有效促进了自我中心视觉和机器人操作任务中的多个下游应用。

Conclusion: EgoLoc无需物体掩码和动词-名词分类法，实现了可泛化的零样本实现，为自我中心视觉和机器人操作任务提供了有效的解决方案。
...
### [184] [Descriptor: Distance-Annotated Traffic Perception Question Answering (DTPQA)](https://arxiv.org/abs/2511.13397)
*Nikos Theodoridis,Tim Brophy,Reenu Mohandas,Ganesh Sistu,Fiachra Collins,Anthony Scanlan,Ciaran Eising*

Main category: cs.CV

TL;DR: 提出了DTPQA基准测试，专门用于评估视觉语言模型在交通场景中的感知能力，包含合成和真实世界两部分，并带有距离标注以分析模型性能随距离增加的变化。


<details>
  <summary>Details</summary>
Motivation: 随着视觉语言模型在自动驾驶领域的应用需求增加，需要评估其在安全关键领域中的感知能力，特别是在复杂交通场景和远距离物体识别方面的表现。

Method: 创建了包含合成基准(DTP-Synthetic)和真实世界基准(DTP-Real)的DTPQA数据集，每个样本包含图像、问题、真实答案和物体距离标注，使用Python脚本生成数据。

Result: 提供了专门用于评估VLM在交通场景中感知能力的基准测试工具，能够分析模型性能随物体距离增加而下降的情况。

Conclusion: DTPQA基准测试为评估视觉语言模型在自动驾驶领域的感知能力提供了重要工具，有助于确保模型在安全关键应用中的可靠性。
...
### [185] [Simple Lines, Big Ideas: Towards Interpretable Assessment of Human Creativity from Drawings](https://arxiv.org/abs/2511.12880)
*Zihao Lin,Zhenshan Shi,Sasa Zhao,Hanwei Zhu,Lingyu Zhu,Baoliang Chen,Lei Mo*

Main category: cs.CV

TL;DR: 提出一个数据驱动的自动绘画创造力评估框架，通过多模态多任务学习同时预测创造力分数、分类内容类型和提取风格特征。


<details>
  <summary>Details</summary>
Motivation: 当前绘画创造力评估依赖专家主观评分，既耗时又主观，需要自动化和可解释的评估方法。

Method: 使用多模态多任务学习框架，引入条件学习机制，根据绘画的风格和语义线索动态调整视觉特征提取。

Result: 模型在创造力评估上达到最先进性能，并提供与人类判断一致的可解释可视化结果。

Conclusion: 该框架能够自动、可解释地评估绘画创造力，结合内容和风格两个维度，优于现有回归方法。
...
### [186] [TripleFDS: Triple Feature Disentanglement and Synthesis for Scene Text Editing](https://arxiv.org/abs/2511.13399)
...
<details>
  <summary>Details</summary>
Motivation: 现有的场景文本编辑方法在可编辑属性解耦方面存在不足，通常只能编辑文本内容，限制了可控性和视觉一致性。

Method: 提出TripleFDS框架和SCB Synthesis数据集，使用SCB Group作为基本训练单元，通过组间对比正则化和组内多特征正交性实现三重特征解耦，在合成阶段进行特征重映射以防止重建中的"捷径"现象。

Result: 在主流STE基准测试中，TripleFDS实现了最先进的图像保真度（SSIM为44.54）和文本准确性（ACC为93.58%）。

Conclusion: TripleFDS不仅性能优越，还支持更灵活的编辑操作，如样式替换和背景转移。
...
### [187] [ActVAR: Activating Mixtures of Weights and Tokens for Efficient Visual Autoregressive Generation](https://arxiv.org/abs/2511.12893)
*Kaixin Zhang,Ruiqing Yang,Yuan Zhang,Shan You,Tao Huang*

Main category: cs.CV

TL;DR: ActVAR是一个动态激活框架，通过双重稀疏化（模型权重和token序列）提升VAR模型效率，在保持性能的同时减少21.2%的计算量。


<details>
  <summary>Details</summary>
Motivation: 解决VAR模型随着序列长度增加计算成本急剧上升的问题，现有静态剪枝方法会永久移除权重或token，破坏预训练依赖关系。

Method: 将前馈网络分解为轻量级专家子网络，使用可学习路由器动态选择token特定的专家子集；同时通过门控token选择器识别高更新潜力token进行计算，重构未选token以保持全局上下文和序列对齐；采用两阶段知识蒸馏策略训练。

Result: 在ImageNet 256×256基准测试中，ActVAR实现了高达21.2%的FLOPs减少，且性能下降最小。

Conclusion: ActVAR通过动态激活机制有效平衡了VAR模型的效率与性能，为大规模视觉生成任务提供了可行的加速方案。
...
### [188] [Unlocking the Forgery Detection Potential of Vanilla MLLMs: A Novel Training-Free Pipeline](https://arxiv.org/abs/2511.13442)
*Rui Zuo,Qinyue Tong,Zhe-Ming Lu,Ziqian Lu*

Main category: cs.CV

TL;DR: Foresee是一个基于多模态大语言模型的免训练图像伪造分析框架，通过类型先验驱动策略和灵活特征检测器，在无需额外训练的情况下实现高精度的篡改定位和丰富的文本解释。


<details>
  <summary>Details</summary>
Motivation: 现有图像伪造检测方法泛化能力有限且可解释性不足，而现有基于MLLM的方法需要大规模训练且未能充分发挥原始MLLM的内在泛化潜力。

Method: 采用免训练MLLM管道，结合类型先验驱动策略和灵活特征检测器（FFD）模块专门处理复制-移动篡改，轻量级推理过程。

Result: 在多种篡改类型（复制-移动、拼接、移除、局部增强、深度伪造和AIGC编辑）上均优于现有方法，定位精度更高且文本解释更丰富。

Conclusion: Foresee成功释放了原始MLLM在取证领域的潜力，无需额外训练即可实现优越的定位准确性和更强的泛化能力。
...
### [189] [Reconstructing 3D Scenes in Native High Dynamic Range](https://arxiv.org/abs/2511.12895)
...
<details>
  <summary>Details</summary>
Motivation: 专业数字媒体制作需要HDR成像，但现有3D场景重建主要基于LDR数据，限制了在专业工作流程中的应用。现有方法依赖多曝光融合或逆色调映射，增加了捕获复杂性且依赖合成监督。

Method: 提出Native High dynamic range 3D Gaussian Splatting (NH-3DGS)，采用新颖的亮度-色度分解的颜色表示方法，支持直接从原生HDR相机数据进行优化。

Result: 在合成和真实多视角HDR数据集上的实验表明，NH-3DGS在重建质量和动态范围保持方面显著优于现有方法。

Conclusion: NH-3DGS实现了直接从原生HDR捕获进行专业级3D重建，代码和数据集将公开。
...
### [190] [Semantic Document Derendering: SVG Reconstruction via Vision-Language Modeling](https://arxiv.org/abs/2511.13478)
*Adam Hazimeh,Ke Wang,Mark Collier,Gilles Baechler,Efi Kokiopoulou,Pascal Frossard*

Main category: cs.CV

TL;DR: SliDer是一个使用视觉语言模型将幻灯片图像转换为可编辑SVG格式的框架，解决了现有几何光栅矢量化方法无法保持文档高层次结构的问题。


<details>
  <summary>Details</summary>
Motivation: 多媒体文档如幻灯片和海报通常以静态光栅格式分发，限制了编辑和定制。现有方法基于低层次几何基元，无法保持图像和文本元素的语义区分。

Method: SliDer使用视觉语言模型检测和提取光栅输入中的图像和文本元素属性，将其组织成连贯的SVG格式，并在推理过程中迭代优化预测。

Result: SliDer实现了0.069的重建LPIPS，在82.9%的情况下被人类评估者认为优于最强的零样本VLM基线。

Conclusion: SliDer能够有效将幻灯片图像转换为紧凑且可编辑的SVG表示，同时引入了Slide2SVG数据集来促进该领域的研究。
...
### [191] [FDP: A Frequency-Decomposition Preprocessing Pipeline for Unsupervised Anomaly Detection in Brain MRI](https://arxiv.org/abs/2511.12899)
*Hao Li,Zhenfeng Zhuang,Jingyu Lin,Yu Liu,Yifei Chen,Qiong Peng,Lequan Yu,Liansheng Wang*

Main category: cs.CV

TL;DR: 提出了一种基于频率分解预处理（FDP）的无监督脑MRI异常检测方法，通过频域重建同时抑制病理特征并保留正常解剖结构，显著提升了现有方法的检测性能。


<details>
  <summary>Details</summary>
Motivation: 由于脑解剖结构的多样性和标注数据的稀缺性，监督式脑MRI异常检测面临挑战。现有无监督方法使用人工生成的噪声扰动训练，但模拟异常缺乏真实临床病变的生物物理保真度和形态复杂性。

Method: 通过系统频域分析发现异常具有独特的频率模式，提出频率分解预处理（FDP）框架，利用频域重建同时抑制病理特征和保留解剖结构，可与现有异常模拟技术无缝集成。

Result: FDP显著提升了异常检测性能，与LDM结合时DICE分数提高了17.63%，并在多个基准方法上保持稳健改进。

Conclusion: FDP是首个利用频域重建进行病理抑制和解剖保留的无监督异常检测方法，能有效提升现有方法的检测性能，同时保持诊断保真度。
...
### [192] [Robust Defense Strategies for Multimodal Contrastive Learning: Efficient Fine-tuning Against Backdoor Attacks](https://arxiv.org/abs/2511.13545)
...
### [193] [CASL: Curvature-Augmented Self-supervised Learning for 3D Anomaly Detection](https://arxiv.org/abs/2511.12909)
*Yaohua Zha,Xue Yuerong,Chunlin Fan,Yuansong Wang,Tao Dai,Ke Chen,Shu-Tao Xia*

Main category: cs.CV

TL;DR: 提出了一种基于曲率增强自监督学习的3D异常检测框架CASL，通过多尺度曲率提示引导解码器重建点云坐标，无需特定异常检测机制即可实现领先性能，并能泛化到其他3D理解任务。


<details>
  <summary>Details</summary>
Motivation: 现有3D异常检测方法任务特定性过强，而经典自监督点云模型在统一微调范式下对异常检测效果不佳，需要开发更通用的3D模型。

Method: 基于U-Net架构，引入多尺度曲率提示来指导解码器预测点云空间坐标，通过重建范式进行自监督学习。

Result: 仅使用点曲率作为异常评分已优于多个经典方法，CASL框架在异常检测上达到领先性能，且学习到的表示能很好泛化到点云分类等标准3D理解任务。

Conclusion: 曲率在3D异常检测中起关键作用，CASL框架证明了无需任务特定设计即可实现高性能异常检测，同时保持对其他3D任务的泛化能力。
...
### [194] [Hierarchical Prompt Learning for Image- and Text-Based Person Re-Identification](https://arxiv.org/abs/2511.13575)
...
<details>
  <summary>Details</summary>
Motivation: 现有的方法通常将I2I和T2I任务分开处理，这可能导致表示纠缠和性能不佳。两种任务具有不同的挑战：I2I强调判别性身份学习，而T2I需要准确的跨模态语义对齐。

Method: 1）任务路由Transformer：在共享视觉编码器中引入双分类令牌来路由I2I和T2I分支的特征；2）分层提示生成方案：整合身份级可学习令牌和实例级伪文本令牌；3）跨模态提示正则化策略：在提示令牌空间中强制语义对齐。

Result: 在多个ReID基准测试上的广泛实验验证了该方法的有效性，在I2I和T2I任务上均达到了最先进的性能。

Conclusion: HPL框架通过统一的提示学习方法有效解决了行人重识别中I2I和T2I任务的联合优化问题，实现了优异的跨模态检索性能。
...
### [195] [Explore How to Inject Beneficial Noise in MLLMs](https://arxiv.org/abs/2511.12917)
...
Conclusion: 注入定制化噪声能有效抑制不相关语义成分，显著改善跨模态表示对齐并提升下游任务性能。
...
### [196] [VVS: Accelerating Speculative Decoding for Visual Autoregressive Generation via Partial Verification Skipping](https://arxiv.org/abs/2511.13587)
*Haotian Dong,Ye Li,Rongwei Lu,Chen Tang,Shu-Tao Xia,Zhi Wang*

Main category: cs.CV

TL;DR: 提出VVS框架，通过部分验证跳过加速视觉自回归生成模型，减少目标模型前向传递次数2.8倍，同时保持生成质量。


<details>
  <summary>Details</summary>
Motivation: 视觉自回归生成模型推理延迟高，传统推测解码的"一步草稿、一步验证"范式无法直接减少前向传递次数，限制了加速潜力。

Method: 基于视觉令牌可互换性，提出VVS框架，包含三个模块：动态截断的验证自由令牌选择器、令牌级特征缓存重用、细粒度跳过步调度。

Result: 相比原始自回归解码，VVS将目标模型前向传递次数减少2.8倍，同时保持竞争力的生成质量。

Conclusion: VVS在传统推测解码框架上提供了更优的速度-质量权衡，具有重塑推测解码范式的强大潜力。
...
### [197] [CoordAR: One-Reference 6D Pose Estimation of Novel Objects via Autoregressive Coordinate Map Generation](https://arxiv.org/abs/2511.12919)
*Dexin Zuo,Ang Li,Wei Wang,Wenxian Yu,Danping Zou*

Main category: cs.CV

TL;DR: CoordAR是一种用于单参考视图下未见物体6D姿态估计的自回归框架，通过将3D-3D对应关系建模为离散标记图，解决了现有方法在对称性和遮挡场景下的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有基于单参考视图的6D姿态估计方法依赖实值坐标回归，存在全局一致性不足和缺乏不确定性建模的问题，特别是在对称或遮挡场景中表现不佳。

Method: 提出了CoordAR框架：1）坐标图标记化实现概率预测；2）模态解耦编码策略分别处理RGB外观和坐标线索；3）自回归Transformer解码器基于位置对齐查询特征和部分生成的标记序列进行解码。

Result: 在多个基准测试中显著优于现有方法，并在真实世界测试中对对称性、遮挡等挑战表现出强鲁棒性。

Conclusion: CoordAR通过自回归概率建模有效解决了单参考6D姿态估计中的对称性和遮挡问题，为无3D模型依赖的姿态估计提供了新思路。
...
### [198] [Generative Photographic Control for Scene-Consistent Video Cinematic Editing](https://arxiv.org/abs/2511.12921)
...
TL;DR: CineCtrl是首个视频电影编辑框架，提供对专业相机参数（如散景、快门速度）的精细控制，通过解耦交叉注意力机制实现独立控制，无需担心场景一致性。


<details>
  <summary>Details</summary>
Motivation: 电影叙事深受摄影元素（如景深和曝光）艺术操控的影响，但现有生成视频模型大多仅限于相机运动控制，难以控制这些专业摄影效果。

Method: 提出解耦交叉注意力机制分离相机运动和摄影输入；开发综合数据生成策略，结合模拟摄影效果和真实世界采集管道构建大规模数据集。

Result: 实验表明模型能生成高保真视频，精确控制用户指定的摄影相机效果。
...
### [199] [Alpha Divergence Losses for Biometric Verification](https://arxiv.org/abs/2511.13621)
...
<details>
  <summary>Details</summary>
Motivation: 现有的α-散度损失函数虽然能诱导稀疏解，但在验证任务中难以集成角度边缘，而角度边缘对于提升验证性能至关重要。

Method: 通过两种不同方式集成角度边缘：在参考度量中引入边缘（Q-Margin）和在对数中引入边缘（A3M），并针对A3M的训练不稳定性提出了原型重新初始化策略。

Result: 在IJB-B、IJB-C面部验证基准和VoxCeleb说话人验证任务中取得了显著性能提升，特别是在低误接受率下表现优异。

Conclusion: 提出的两种边缘化α-散度损失函数在面部和说话人验证任务中表现出色，特别适用于需要最小化误认证的高安全性应用场景。
...
### [200] [Text2Traffic: A Text-to-Image Generation and Editing Method for Traffic Scenes](https://arxiv.org/abs/2511.12932)
*Feng Lv,Haoxuan Feng,Zilu Zhang,Chunlong Xia,Yanfeng Li*

Main category: cs.CV

TL;DR: 提出了一个统一的文本驱动框架，用于交通场景的图像生成和编辑，通过可控掩码机制整合两个任务，并利用多视角数据增强几何多样性。


<details>
  <summary>Details</summary>
Motivation: 解决智能交通系统中文本驱动图像生成和编辑面临的挑战，包括生成的交通元素语义丰富度不足、相机视角有限、合成图像视觉保真度低以及文本描述与生成内容对齐不佳等问题。

Method: 采用两阶段训练策略：首先使用大规模粗粒度文本图像数据进行概念学习，然后使用细粒度描述数据进行微调以增强文本图像对齐和细节质量。引入可控掩码机制整合生成和编辑任务，并采用掩码区域加权损失动态强调关键小区域。

Result: 在交通场景的文本驱动图像生成和编辑任务中取得了领先性能。

Conclusion: 所提出的统一框架有效解决了交通场景图像生成和编辑中的关键挑战，显著提升了生成内容的语义丰富度、视觉保真度和文本对齐质量。
...
### [201] [UnSAMv2: Self-Supervised Learning Enables Segment Anything at Any Granularity](https://arxiv.org/abs/2511.13714)
*Junwei Yu,Trevor Darrell,XuDong Wang*

Main category: cs.CV

TL;DR: UnSAMv2通过粒度控制嵌入实现任意粒度的分割，无需人工标注，显著提升了SAM-2在交互式、全图像和视频分割任务中的性能。


<details>
  <summary>Details</summary>
Motivation: 解决SAM模型在分割粒度控制方面的局限性，用户需要手动细化结果才能达到所需细节水平，这个过程既模糊又耗时。

Method: 扩展了UnSAM的分治策略，发现丰富的掩码-粒度对，并引入新颖的粒度控制嵌入，实现对分割尺度的精确连续控制。

Result: 仅使用6K未标注图像和0.02%额外参数，在超过11个基准测试中显著提升性能：NoC90从5.69降至4.75，1-IoU从58.0提升至73.1，AR1000从49.6提升至68.3。
...
### [202] [ProtoAnomalyNCD: Prototype Learning for Multi-class Novel Anomaly Discovery in Industrial Scenarios](https://arxiv.org/abs/2511.12938)
*Botong Zhao,Qijun Shi,Shujing Lyu,Yue Lu*

Main category: cs.CV

TL;DR: ProtoAnomalyNCD是一个基于原型学习的框架，用于发现和分类多种类型的未见异常类别，通过结合Grounded SAM定位对象区域和异常图引导注意力机制来提升性能。
...
Method: 1) 使用Grounded SAM和文本提示定位对象区域作为先验；2) 引入异常图引导注意力块，设计区域引导因子区分背景、对象区域和异常区域；3) 在统一原型学习框架下发现未见异常类别并实现多类型异常分类。

Result: 该方法在MVTec AD、MTD和Real-IAD数据集上优于最先进方法，并能检测未见异常值，实现任务级统一。

Conclusion: ProtoAnomalyNCD通过结合对象定位和异常图引导注意力，有效提升了工业异常检测中多类型异常发现和分类的性能。
...
### [203] [Scaling Spatial Intelligence with Multimodal Foundation Models](https://arxiv.org/abs/2511.13719)
*Zhongang Cai,Ruisi Wang,Chenyang Gu,Fanyi Pu,Junxiang Xu,Yubo Wang,Wanqi Yin,Zhitao Yang,Chen Wei,Qingping Sun,Tongxi Zhou,Jiaqi Li,Hui En Pang,Oscar Qian,Yukun Wei,Zhiqian Lin,Xuanke Shi,Kewang Deng,Xiaoyang Han,Zukai Chen,Xiangyu Fan,Hanming Deng,Lewei Lu,Liang Pan,Bo Li,Ziwei Liu,Quan Wang,Dahua Lin,Lei Yang*

Main category: cs.CV

TL;DR: SenseNova-SI通过构建800万样本的多样化空间智能数据集，显著提升了多模态基础模型的空间智能能力，在多个基准测试中取得突破性表现。


<details>
  <summary>Details</summary>
Motivation: 尽管多模态基础模型取得了显著进展，但在空间智能方面仍存在明显不足，需要专门的数据和方法来提升这方面的能力。

Method: 基于Qwen3-VL、InternVL3和Bagel等多模态基础模型，系统构建了包含800万多样化样本的SenseNova-SI-8M数据集，采用严格的空间能力分类法。

Result: 在多个空间智能基准测试中取得优异表现：VSI-Bench 68.7%、MMSI 43.3%、MindCube 85.6%、ViewSpatial 54.6%、SITE 50.1%，同时保持强大的通用多模态理解能力（MMBench-En 84.9%）。

Conclusion: SenseNova-SI项目展示了数据规模扩展对空间智能提升的重要性，发现了多样化数据训练带来的泛化能力，验证了空间思维链推理的潜力，并将持续更新和公开发布模型以促进相关研究。
...
### [204] [Semi-Supervised High Dynamic Range Image Reconstructing via Bi-Level Uncertain Area Masking](https://arxiv.org/abs/2511.12939)
...
<details>
  <summary>Details</summary>
Motivation: 现有基于学习的HDR图像重建方法需要大量LDR-HDR图像对，但这些数据难以获取。研究目标是开发标注高效的HDR重建方法，在有限HDR真实标签下达到可比性能。

Method: 采用半监督学习框架：教师模型为无标签的LDR样本生成伪HDR标签，学生模型从伪标签学习。提出基于不确定性的掩码机制，在像素和补丁级别过滤伪标签中的不可靠区域，避免确认偏差。

Result: 该方法不仅优于之前的标注高效算法，而且仅使用6.7%的HDR真实标签就能达到最新全监督方法的可比性能。

Conclusion: 提出的不确定性掩码半监督方法有效解决了标注数据稀缺问题，为HDR图像重建提供了一种高效实用的解决方案。
...
### [205] [Recurrent Autoregressive Diffusion: Global Memory Meets Local Attention](https://arxiv.org/abs/2511.12940)
...
TL;DR: DiffSign是一个基于文本到图像扩散模型的交通标志识别系统对抗攻击框架，能够生成物理鲁棒、高效、可迁移且隐蔽的外观攻击。
...
Method: 提出DiffSign框架，集成CLIP损失和掩码提示以提高攻击聚焦性和可控性，并开发两种风格定制方法来指导视觉外观，提升跨域交通标志攻击泛化能力和隐蔽性。

Result: 在多种真实世界条件下（不同距离、角度、光照条件和标志类别）进行广泛评估，平均物理世界攻击成功率达到83.3%，展现出高攻击迁移性。

Conclusion: DiffSign框架能够有效克服现有方法的局限性，生成物理鲁棒、高效、可迁移且隐蔽的对抗攻击，对交通标志识别系统构成实际威胁。
...
### [207] [GrOCE:Graph-Guided Online Concept Erasure for Text-to-Image Diffusion Models](https://arxiv.org/abs/2511.12968)
*Ning Han,Zhenyu Ge,Feng Han,Yuhua Sun,Chengqing Li,Jingjing Chen*

Main category: cs.CV

TL;DR: 提出GrOCE框架，无需训练即可实现精确的概念擦除，通过图语义推理建模概念间关系，在CS和FID指标上达到SOTA性能
...
Method: 包含三个组件：动态拓扑图构建、自适应聚类识别（带相似度衰减评分的多跳遍历）、选择性边切断（在保留全局语义的同时移除目标边）

Result: 在Concept Similarity和Fréchet Inception Distance指标上达到最先进性能，实现高效、准确、稳定的概念擦除
...
### [208] [HiFusion: Hierarchical Intra-Spot Alignment and Regional Context Fusion for Spatial Gene Expression Prediction from Histopathology](https://arxiv.org/abs/2511.12969)
...
Method: 提出HiFusion框架：1）分层spot内建模模块通过多分辨率子块分解提取细粒度形态表征；2）上下文感知跨尺度融合模块使用交叉注意力选择性地整合生物相关区域上下文。

Result: 在两个基准ST数据集上的广泛实验表明，HiFusion在2D切片级交叉验证和更具挑战性的3D样本特定场景中均达到最先进性能。
...
### [209] [MCAQ-YOLO: Morphological Complexity-Aware Quantization for Efficient Object Detection with Curriculum Learning](https://arxiv.org/abs/2511.12976)
*Yoonjae Seo,Ermal Elbasani,Jaehong Lee*

Main category: cs.CV

TL;DR: MCAQ-YOLO是一种形态复杂度感知的量化框架，通过五个形态学指标动态调整空间位精度，结合课程式量化感知训练，在目标检测任务中实现优于均匀量化的性能。
...
Method: 使用分形维度、纹理熵、梯度方差、边缘密度和轮廓复杂度五个形态学指标来表征局部视觉形态并指导空间自适应位分配；采用基于课程的量化感知训练方案逐步增加量化难度。

Result: 在安全装备数据集上达到85.6% mAP@0.5，平均4.2位，压缩比7.6倍，比均匀4位量化高3.5个百分点，每张图像仅增加1.8ms运行时开销；在COCO和Pascal VOC上的跨数据集验证也证实了性能提升。
...
### [210] [ArtiWorld: LLM-Driven Articulation of 3D Objects in Scenes](https://arxiv.org/abs/2511.12977)
*Yixuan Yang,Luyang Xie,Zhen Luo,Zixiang Zhao,Mingqi Gao,Feng Zheng*

Main category: cs.CV

TL;DR: ArtiWorld是一个从文本场景描述中自动识别可关节化对象并重建可执行URDF模型的管道，核心是Arti4URDF，利用3D点云、大语言模型先验知识和URDF导向提示设计，将刚性物体转换为交互式关节化对象。


<details>
  <summary>Details</summary>
Motivation: 现有3D资产大多是刚性的，手动将其转换为关节化对象成本高昂，需要自动化方法从场景中识别可关节化对象并转换为可执行URDF模型。

Method: 使用场景感知管道，通过文本描述定位候选关节化对象，利用3D点云、LLM先验知识和URDF导向提示设计，快速重建保持原始几何形状的可执行URDF模型。
...
### [211] [Concept Regions Matter: Benchmarking CLIP with a New Cluster-Importance Approach](https://arxiv.org/abs/2511.12978)
*Aishwarya Agarwal,Srikrishna Karanam,Vineet Gandhi*

Main category: cs.CV

TL;DR: 提出了CCI方法，利用CLIP的patch嵌入将空间块聚类成语义连贯的簇，通过掩码和评估预测变化来识别概念重要性。该方法在忠实性基准测试中达到新SOTA，并引入COVAR基准来系统评估前景和背景变化的影响。


<details>
  <summary>Details</summary>
Motivation: 对比视觉语言模型（如CLIP）在零样本识别中表现良好，但容易受到虚假相关性的影响，特别是对背景的过度依赖。现有基准仅依赖准确性，隐含地将所有性能下降归因于背景相关性，这种假设是不完整的。

Method: 提出了CCI方法，使用CLIP自身的patch嵌入进行聚类、掩码和预测变化评估。结合GroundedSAM自动分类预测为前景或背景驱动。还引入了COVAR基准，系统性地变化对象前景和背景。

Result: CCI在忠实性基准测试中大幅超越先前方法，例如在MS COCO检索的删除AUC指标上实现超过两倍的改进。通过CCI与COVAR结合，对18个CLIP变体进行了全面评估。

Conclusion: CCI方法提供了重要的诊断能力，揭示了错误不仅来自背景相关性，还来自视角变化、尺度变化和细粒度对象混淆。该方法为构建更稳健的视觉语言模型指明了方向。
...
### [212] [Semantic Prioritization in Visual Counterfactual Explanations with Weighted Segmentation and Auto-Adaptive Region Selection](https://arxiv.org/abs/2511.12992)
*Lintong Zhang,Kang Yin,Seong-Whan Lee*

Main category: cs.CV

TL;DR: 提出WSAE-Net方法，通过加权语义图和自适应候选编辑序列优化视觉反事实解释，提升语义相关性和计算效率。


<details>
  <summary>Details</summary>
Motivation: 传统视觉反事实解释方法忽略替换区域与目标对象的语义相关性，损害模型可解释性并阻碍编辑流程。

Method: WSAE-Net包含两个关键创新：加权语义图生成以减少非语义特征单元计算，自适应候选编辑序列确定最优计算顺序。

Result: 通过全面实验验证，该方法展现出优越性能，有助于更清晰深入地理解视觉反事实解释。

Conclusion: WSAE-Net通过语义相关性和计算效率的优化，显著提升了视觉反事实解释的质量和实用性。
...
### [213] [PerTouch: VLM-Driven Agent for Personalized and Semantic Image Retouching](https://arxiv.org/abs/2511.12998)
*Zewei Chang,Zheng-Peng Duan,Jianxing Zhang,Chun-Le Guo,Siyu Liu,Hyungju Chun,Hyunhee Park,Zikun Liu,Chongyi Li*

Main category: cs.CV

TL;DR: PerTouch是一个基于扩散模型的图像润色框架，支持语义级图像润色，通过参数映射实现细粒度控制，并利用VLM驱动的智能体处理用户指令，结合反馈重思和场景感知记忆机制更好地适应用户偏好。


<details>
  <summary>Details</summary>
Motivation: 图像润色需要在提升视觉质量的同时满足用户的个性化审美偏好，但现有方法在可控性和主观性之间难以平衡。

Method: 使用包含特定语义区域属性值的参数映射作为输入，构建显式的参数到图像映射；引入语义替换和参数扰动机制改善语义边界感知；开发VLM驱动的智能体处理强弱用户指令；配备反馈驱动重思和场景感知记忆机制。

Result: 大量实验证明了每个组件的有效性，PerTouch在个性化图像润色方面表现出优越性能。

Conclusion: PerTouch通过统一的扩散框架成功平衡了可控性和主观性，实现了语义级的个性化图像润色。
...
### [214] [Medal S: Spatio-Textual Prompt Model for Medical Segmentation](https://arxiv.org/abs/2511.13001)
*Pengcheng Shi,Jiawei Chen,Jiaqi Liu,Xinglin Zhang,Tao Chen,Lei Li*

Main category: cs.CV

TL;DR: Medal S是一个医学分割基础模型，支持原生分辨率空间和文本提示的端到端训练框架，在五模态医学图像分割中显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 解决现有文本方法缺乏空间感知能力的问题，通过通道对齐机制减少分辨率不匹配带来的不准确性，提升多类别分割性能。

Method: 采用轻量级3D卷积模块进行体素空间细化，支持并行处理多个原生分辨率掩码，提出动态重采样、两阶段推理策略和文本预处理优化。

Result: 在验证集上，Medal S在五模态平均DSC达到75.44（vs SAT 69.83），NSD 77.34（vs 71.06），F1 38.24（vs 24.88），并行空间提示减少90%以上推理时间。
...
### [215] [Infinite-Story: A Training-Free Consistent Text-to-Image Generation](https://arxiv.org/abs/2511.13002)
...
TL;DR: Infinite-Story是一个无需训练的多提示文本到图像生成框架，通过身份提示替换和统一注意力引导机制解决身份和风格不一致问题，推理速度比现有最快模型快6倍以上。


<details>
  <summary>Details</summary>
Motivation: 解决多提示讲故事场景中文本到图像生成的两个关键挑战：身份不一致和风格不一致，同时避免现有方法需要微调或推理速度慢的问题。

Method: 基于尺度自回归模型，采用三种互补技术：身份提示替换（缓解文本编码器中的上下文偏差）、自适应风格注入和同步引导适应的统一注意力引导机制。

Result: 在多样化提示下实现高身份和风格一致性，推理速度达到每张图像1.72秒，比现有最快一致T2I模型快6倍以上，生成性能达到最先进水平。

Conclusion: Infinite-Story是一个高效实用的视觉讲故事框架，无需训练即可实现高质量的一致图像生成，具有实际应用价值。
...
### [216] [Beyond Darkness: Thermal-Supervised 3D Gaussian Splatting for Low-Light Novel View Synthesis](https://arxiv.org/abs/2511.13011)
...
<details>
  <summary>Details</summary>
Motivation: 在极端低光条件下，新视角合成在几何、颜色一致性和辐射稳定性方面严重退化。标准3D高斯泼溅管道直接应用于欠曝光输入时会失败，因为跨视图的独立增强会导致光照不一致和几何失真。

Method: DTGS通过循环增强-重建机制，将增强、几何和热监督进行联合优化。包含热监督分支动态平衡增强、结构和热损失，以及嵌入3DGS循环的Retinex分解模块提供物理可解释的反射-光照分离。

Result: 在新建的RGBT-LOW多视图低光热数据集上的实验表明，DTGS显著优于现有的低光增强和3D重建基线方法，在极端光照条件下实现了优异的辐射一致性、几何保真度和颜色稳定性。

Conclusion: DTGS通过紧密耦合光照分解与热引导的3D高斯泼溅，成功解决了极端低光条件下的新视角合成问题，提供了物理可解释且一致的增强和重建结果。
...
### [217] [You Only Look Omni Gradient Backpropagation for Moving Infrared Small Target Detection](https://arxiv.org/abs/2511.13013)
*Guoyi Zhang,Guangsheng Xu,Siyang Chen,Han Wang,Xiaohu Zhang*

Main category: cs.CV

TL;DR: 提出了BP-FPN，一种基于反向传播视角的特征金字塔架构，通过梯度隔离低层捷径和方向梯度正则化来提升红外小目标检测性能。


<details>
  <summary>Details</summary>
Motivation: 现有深度学习方法主要关注时空特征聚合，但性能提升有限，根本瓶颈在于单帧特征表示模糊，而非时空建模本身。

Method: 提出BP-FPN架构，包含梯度隔离低层捷径(GILS)和方向梯度正则化(DGR)，从反向传播角度重新设计特征学习。

Result: 在多个公共数据集上的实验表明，BP-FPN持续建立了新的最先进性能。
...
### [218] [Geometry Meets Light: Leveraging Geometric Priors for Universal Photometric Stereo under Limited Multi-Illumination Cues](https://arxiv.org/abs/2511.13015)
*King-Man Tam,Satoshi Ikehata,Yuta Asano,Zhaoyi An,Rei Kawakami*

Main category: cs.CV

TL;DR: GeoUniPS是一个通用光度立体网络，通过结合合成监督和来自大规模3D重建模型的几何先验，解决了传统方法在复杂真实场景中光照线索不可靠的问题。


<details>
  <summary>Details</summary>
Motivation: 传统通用光度立体方法在复杂真实场景中，当多光照线索不可靠时（如偏置光照、阴影或自遮挡区域）表现不佳，需要引入更强的几何先验知识。

Method: 设计了光-几何双分支编码器，从冻结的3D重建模型中提取多光照线索和几何先验；引入PS-Perp数据集解决传统正交投影假设的局限性，学习空间变化的视角方向。

Result: 在多个数据集上实现了最先进的性能，特别是在复杂真实场景中，定量和定性评估均表现出色。

Conclusion: GeoUniPS通过整合大规模3D重建模型的几何先验知识，显著提升了通用光度立体在复杂真实场景中的表现，证明了视觉几何基础模型的有效性。
...
### [219] [REVISOR: Beyond Textual Reflection, Towards Multimodal Introspective Reasoning in Long-Form Video Understanding](https://arxiv.org/abs/2511.13026)
*Jiaze Li,Hao Yin,Wenhui Tan,Jingyang Chen,Boshen Xu,Yuxun Qu,Yijing Chen,Jianzhong Ju,Zhenbo Luo,Jian Luan*

Main category: cs.CV

TL;DR: REVISOR是一个用于长视频理解的多模态反思框架，通过跨模态协作反思机制增强MLLMs的推理能力，无需额外监督微调或外部模型。


<details>
  <summary>Details</summary>
Motivation: 纯文本反思机制在长视频理解中存在局限性：1) 仅反思文本信息不足，需要针对视觉信息的反思；2) 缺乏跨模态交互能力，无法在反思中充分整合视觉信息。

Method: 提出REVISOR框架，使MLLMs能够在文本和视觉模态间协作构建内省反思过程。设计DADR奖励机制，在GRPO训练策略中确保模型推理与所选视频证据的因果对齐。

Result: 在VideoMME、LongVideoBench、MLVU和LVBench四个基准测试中取得显著成果，显著提升了MLLMs的长视频理解能力。

Conclusion: REVISOR框架通过跨模态反思机制有效解决了长视频理解中的视觉信息整合问题，为MLLMs的长视频理解提供了新的解决方案。
...
### [220] [Towards 3D Object-Centric Feature Learning for Semantic Scene Completion](https://arxiv.org/abs/2511.13031)
...
<details>
  <summary>Details</summary>
Motivation: 现有的基于视觉的3D语义场景补全方法通常采用以自我为中心的范式，在整个场景中聚合和扩散特征，但往往忽略了细粒度的对象级细节，导致在复杂环境中出现语义和几何模糊。
...
### [221] [Uni-Inter: Unifying 3D Human Motion Synthesis Across Diverse Interaction Contexts](https://arxiv.org/abs/2511.13032)
*Sheng Liu,Yuanzhi Liang,Jiepeng Wang,Sidan Du,Chi Zhang,Xuelong Li*

Main category: cs.CV

TL;DR: Uni-Inter是一个统一的人类运动生成框架，支持人-人、人-物、人-场景等多种交互场景，通过统一的交互体积表示实现异构实体的共享空间编码。
...
Method: 引入统一交互体积(UIV)作为体素表示，将异构交互实体编码到共享空间场中，通过关节级概率预测进行运动生成。
...
### [222] [uCLIP: Parameter-Efficient Multilingual Extension of Vision-Language Models with Unpaired Data](https://arxiv.org/abs/2511.13036)
*Dahyun Chung,Donghyun Shin,Yujin Sung,Seunggi Moon,Jinwoo Jeon,Byung-Jun Lee*

Main category: cs.CV

TL;DR: 提出了一种轻量级、数据高效的多语言视觉-语言对齐框架，无需图像-文本对或文本-文本对，仅训练170万参数的投影模块，通过英语表示作为语义锚点实现多语言对齐。


<details>
  <summary>Details</summary>
Motivation: 解决多语言视觉-语言模型在低资源语言（如捷克语、芬兰语、克罗地亚语、匈牙利语、罗马尼亚语）上检索性能低下的问题，这些语言在Crossmodal-3600基准测试中表现不佳。
...
Conclusion: 基于枢轴的参数高效对齐策略对于包容性多模态学习具有有效性，为低资源语言的多语言视觉-语言对齐提供了可行方案。
...
### [223] [MGCA-Net: Multi-Grained Category-Aware Network for Open-Vocabulary Temporal Action Localization](https://arxiv.org/abs/2511.13039)
*Zhenying Fang,Richang Hong*

Main category: cs.CV

TL;DR: 提出了MGCA-Net，通过多粒度类别感知来解决开放词汇时序动作定位问题，在基类和新增类别上都实现了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法大多在单一粒度上识别动作类别，这会降低基类和新增动作类别的识别准确率。

Method: 包含定位器、动作存在预测器、传统分类器和粗到细分类器，通过多粒度类别感知来增强定位性能。

Result: 在THUMOS'14和ActivityNet-1.3基准测试中达到最先进性能，并在零样本时序动作定位设置中取得最佳结果。

Conclusion: MGCA-Net通过多粒度类别感知有效提升了开放词汇时序动作定位的性能。
...
### [224] [DiffPixelFormer: Differential Pixel-Aware Transformer for RGB-D Indoor Scene Segmentation](https://arxiv.org/abs/2511.13047)
*Yan Gong,Jianli Lu,Yongsheng Gao,Jie Zhao,Xiaojuan Zhang,Susanto Rahardja*

Main category: cs.CV

TL;DR: 提出DiffPixelFormer，一种用于RGB-D室内场景分割的差分像素感知Transformer，通过增强模态内表示和建模模态间交互来提升分割性能。
...
Method: 核心是Intra-Inter Modal Interaction Block (IIMIB)，通过自注意力捕获模态内长程依赖，使用Differential-Shared Inter-Modal (DSIM)模块建模模态间交互，分离模态特定和共享线索，实现像素级跨模态对齐。采用动态融合策略平衡模态贡献。

Result: 在SUN RGB-D和NYUDv2基准测试中，DiffPixelFormer-L分别达到54.28%和59.95%的mIoU，比DFormer-L分别提升1.78%和2.75%。

Conclusion: DiffPixelFormer通过差分像素感知Transformer有效提升了RGB-D室内语义分割性能，在多个基准测试中表现优异。
...
### [225] [ViSS-R1: Self-Supervised Reinforcement Video Reasoning](https://arxiv.org/abs/2511.13054)
*Bo Fang,Yuxin Song,Qiangqiang Wu,Haoyuan Sun,Wenhao Wu,Antoni B. Chan*

Main category: cs.CV

TL;DR: 提出了Pretext-GRPO自监督强化学习算法和ViSS-R1框架，通过视觉变换任务强制MLLM深度处理视频信息，提升复杂视频推理能力
...
Method: 1. Pretext-GRPO：在标准R1流程中引入自监督强化学习，对变换后的视觉输入正确解决前置任务给予奖励；2. ViSS-R1：将前置任务自监督学习直接集成到MLLM的R1后训练范式，同时处理前置问题和真实查询

Result: 在六个广泛使用的视频推理和理解基准测试中表现出有效性和优越性

Conclusion: 该方法通过强制模型深度处理视觉信息，实现了更鲁棒的视觉中心视频理解，解决了当前MLLM在视频推理中的局限性
...
### [226] [Monocular 3D Lane Detection via Structure Uncertainty-Aware Network with Curve-Point Queries](https://arxiv.org/abs/2511.13055)
*Ruixin Liu,Zejian Yuan*

Main category: cs.CV

TL;DR: MonoUnc是一个无需鸟瞰图的单目3D车道线检测器，通过局部车道结构建模偶然不确定性，在ONCE-3DLanes和OpenLane数据集上超越现有最优方法。
...
Result: 在ONCE-3DLanes和OpenLane数据集的所有基准测试中均优于之前的最优方法，特别是在更严格的评估标准下。

Conclusion: 提出的MonoUnc方法通过显式建模局部结构不确定性，有效提升了单目3D车道线检测的精度和鲁棒性。
...
### [227] [FGNet: Leveraging Feature-Guided Attention to Refine SAM2 for 3D EM Neuron Segmentation](https://arxiv.org/abs/2511.13063)
*Zhenghua Li,Hang Chen,Zihao Sun,Kai Li,Xiaolin Hu*

Main category: cs.CV

TL;DR: 提出了一种将Segment Anything 2 (SAM2)从自然图像预训练迁移到电子显微镜图像分割的框架，通过特征引导注意力模块和双亲和度解码器，在冻结权重时达到SOTA水平，微调后显著超越现有方法。


<details>
  <summary>Details</summary>
Motivation: 电子显微镜图像中的神经结构分割面临形态复杂、信噪比低和标注稀缺的挑战，现有方法在准确性和泛化性上受限，希望通过利用视觉基础模型在自然图像上学到的先验知识来解决这些问题。

Method: 使用SAM2提取通用特征，引入特征引导注意力模块利用SAM2的语义线索指导轻量级精细编码器关注困难区域，最后通过双亲和度解码器生成粗粒度和精细化亲和度图。

Result: 实验结果表明，在SAM2权重冻结时性能与SOTA方法相当，在电子显微镜数据上微调后显著超越现有SOTA方法。

Conclusion: 这项研究验证了将自然图像预训练的表征与针对性的领域自适应指导相结合，可以有效解决神经元分割中的特定挑战。
...
### [228] [RobustGait: Robustness Analysis for Appearance Based Gait Recognition](https://arxiv.org/abs/2511.13065)
*Reeshoon Sayera,Akash Kumar,Sirshapan Mitra,Prudvi Kamtam,Yogesh S Rawat*

Main category: cs.CV

TL;DR: 提出了RobustGait框架，用于评估基于外观的步态识别系统在真实世界干扰和轮廓变化下的鲁棒性，涵盖四种扰动类型、多种轮廓提取方法和模型架构，发现RGB级噪声处理、轮廓提取器偏差和架构设计对鲁棒性的重要影响。


<details>
  <summary>Details</summary>
Motivation: 现有基于外观的步态识别系统在受控数据集上表现良好，但缺乏对其在真实世界干扰和轮廓变化下鲁棒性的系统评估。

Method: 开发RobustGait评估框架，在四个维度（扰动类型、轮廓提取方法、模型架构容量、部署场景）上评估，引入15种干扰类型和5个严重级别，在多个数据集上进行测试。

Result: 发现RGB级噪声处理能更好反映真实世界退化；步态识别对轮廓提取器偏差高度敏感；鲁棒性取决于扰动类型和架构设计；噪声感知训练和知识蒸馏能提升性能。

Conclusion: 步态识别系统的鲁棒性评估揭示了多个关键因素，包括轮廓提取器偏差和架构设计，提出的增强策略有助于开发部署就绪的系统。
...
### [229] [Decoupling Scene Perception and Ego Status: A Multi-Context Fusion Approach for Enhanced Generalization in End-to-End Autonomous Driving](https://arxiv.org/abs/2511.13079)
...
<details>
  <summary>Details</summary>
Motivation: 现有模块化规划导向的自动驾驶架构过度依赖自车状态，导致泛化能力受限和场景理解不鲁棒。根本原因在于架构设计允许自车状态被轻易用作捷径，上游BEV编码器中自车状态的过早融合使得这一强先验信息主导下游规划模块。

Method: 提出AdaptiveAD架构：1）双分支结构：场景驱动分支（多任务学习但BEV编码器故意省略自车状态）和自车驱动分支（仅基于规划任务）；2）场景感知融合模块自适应整合两个分支的互补决策；3）路径注意力机制用于自车-BEV交互；4）两个辅助任务：BEV单向蒸馏和自回归在线建图。

Result: 在nuScenes数据集上的广泛评估表明，AdaptiveAD实现了最先进的开放环路规划性能，显著减轻了对自车状态的过度依赖，并在多样化场景中展现出令人印象深刻的泛化能力。

Conclusion: AdaptiveAD通过架构层面的多上下文融合策略有效解决了端到端自动驾驶系统中过度依赖自车状态的问题，提升了系统的泛化性和鲁棒性。
...
### [230] [MergeSlide: Continual Model Merging and Task-to-Class Prompt-Aligned Inference for Lifelong Learning on Whole Slide Images](https://arxiv.org/abs/2511.13099)
*Doanh C. Bui,Ba Hung Ngo,Hoai Luan Pham,Khang Nguyen,Maï K. Nguyen,Yasuhiko Nakashima*

Main category: cs.CV

TL;DR: MergeSlide是一个用于全切片图像终身学习的框架，将终身学习视为模型融合问题，利用视觉-语言病理学基础模型，通过正交持续融合策略和任务到类别提示对齐推理来缓解灾难性遗忘。


<details>
  <summary>Details</summary>
Motivation: 全切片图像的终身学习旨在顺序训练统一模型处理癌症相关任务，减少数据传输和处理所需资源，特别是考虑到WSIs的千兆字节级大小。

Method: 1) 使用类别感知提示定义新任务；2) 使用无MLP的主干网络进行少量轮次微调；3) 采用正交持续融合策略将新任务模型融合到统一模型中；4) 引入任务到类别提示对齐推理来处理类别增量学习设置。

Result: 在六个TCGA数据集流上的实验表明，MergeSlide在性能上优于基于排练的持续学习和视觉-语言零样本基线方法。

Conclusion: MergeSlide提供了一个简单有效的终身学习框架，通过模型融合和提示对齐推理成功缓解了灾难性遗忘问题，在WSI分析任务中表现出色。
...
### [231] [CapeNext: Rethinking and refining dynamic support information for category-agnostic pose estimation](https://arxiv.org/abs/2511.13102)
*Yu Zhu,Dan Zeng,Shuiwang Li,Qijun Zhao,Qiaomu Shen,Bo Tang*

Main category: cs.CV

TL;DR: 提出了CapeNext框架，通过层次化跨模态交互和双流特征精炼，解决了类别无关姿态估计中静态关节嵌入的歧义性和区分性不足问题，在MP-100数据集上大幅超越现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有类别无关姿态估计方法使用固定文本关键点描述作为语义先验，但存在两个固有局限：(1)多义词导致的跨类别歧义（如'腿'在人类和家具中的不同视觉表现），(2)对细粒度类别内变化区分性不足（如不同姿势和毛色的猫）。
...
Conclusion: 所提出的框架有效解决了静态关节嵌入的局限性，通过动态整合多模态信息显著提升了类别无关姿态估计的性能。
...
### [232] [PlugTrack: Multi-Perceptive Motion Analysis for Adaptive Fusion in Multi-Object Tracking](https://arxiv.org/abs/2511.13105)
*Seungjae Kim,SeungJoon Lee,MyeongAh Cho*

Main category: cs.CV

TL;DR: PlugTrack是一个新颖的多目标跟踪框架，通过自适应融合卡尔曼滤波器和数据驱动的运动预测器来解决线性与非线性运动模式问题。


<details>
  <summary>Details</summary>
Motivation: 现有的运动预测方法存在局限性：卡尔曼滤波器计算高效但无法处理非线性运动，而数据驱动方法能捕捉复杂动态但泛化能力有限且计算开销大。研究发现真实跟踪场景中同时存在线性和非线性模式。
...
Conclusion: PlugTrack是首个通过自适应融合桥接经典和现代运动预测范式的多目标跟踪框架。
...
### [233] [Low-Level Dataset Distillation for Medical Image Enhancement](https://arxiv.org/abs/2511.13106)
*Fengzhi Xu,Ziyuan Yang,Mengyu Sun,Joey Tianyi Zhou,Yi Zhang*

Main category: cs.CV

TL;DR: 提出了首个用于医学图像增强的低级数据集蒸馏方法，通过共享解剖先验和个性化生成模块解决低级任务中像素级保真度问题，同时保护患者隐私。


<details>
  <summary>Details</summary>
Motivation: 现有数据集蒸馏方法主要针对高级任务，而低级任务需要像素级保真度，导致信息压缩困难。医学图像增强需要大规模数据集，但训练和存储成本高，且患者数据隐私需要保护。

Method: 利用患者间解剖相似性构建共享解剖先验作为初始化，通过结构保持个性化生成模块整合患者特定信息，构建任务特定的高低质量训练对，并通过梯度对齐注入患者特定知识。

Result: 该方法能够生成包含抽象训练信息的蒸馏数据集，无需访问原始患者数据，在保护隐私的同时实现有效的医学图像增强。

Conclusion: 提出的低级数据集蒸馏方法解决了医学图像增强中的像素级保真度和隐私保护问题，为实际部署提供了可行方案。
...
### [234] [DGS-Net: Distillation-Guided Gradient Surgery for CLIP Fine-Tuning in AI-Generated Image Detection](https://arxiv.org/abs/2511.13108)
...
<details>
  <summary>Details</summary>
Motivation: 生成模型的快速发展导致AI生成图像泛滥，引发错误信息、隐私侵犯等担忧。虽然CLIP等模型能检测合成内容，但微调时会出现灾难性遗忘，破坏预训练先验并限制跨域泛化

Method: 提出蒸馏引导的梯度手术网络(DGS-Net)，通过梯度空间分解在优化过程中分离有害和有益下降方向，将任务梯度投影到有害方向的正交补空间，并与冻结CLIP编码器蒸馏的有益方向对齐

Result: 在50个生成模型上的实验表明，该方法平均优于现有最佳方法6.6个百分点，实现了优越的检测性能和跨不同生成技术的泛化能力

Conclusion: DGS-Net通过统一优化先验保持和无关抑制，有效解决了CLIP模型微调时的灾难性遗忘问题，为AI生成图像检测提供了有效的解决方案
...
### [235] [Learning Implicit Neural Degradation Representation for Unpaired Image Dehazing](https://arxiv.org/abs/2511.13110)
*Shuaibin Fan,Senming Zhong,Wenchao Yan,Minglong Xue*

Main category: cs.CV

TL;DR: 提出了一种基于隐式神经退化表示的无监督去雾方法，通过结合通道独立和通道依赖机制来增强非线性依赖学习能力，并使用隐式神经表示将雾霾退化建模为连续函数。


<details>
  <summary>Details</summary>
Motivation: 现有方法在处理复杂场景时难以平衡非均匀雾分布的特征表示和全局一致性建模，需要更好地学习雾霾在空间变化中的退化表示。

Method: 1. 基于Kolmogorov-Arnold表示定理，结合通道独立和通道依赖机制；2. 设计隐式神经表示将雾霾退化建模为连续函数；3. 设计密集残差增强模块来学习雾特征的隐式表示。
...
### [236] [Semantics and Content Matter: Towards Multi-Prior Hierarchical Mamba for Image Deraining](https://arxiv.org/abs/2511.13113)
*Zhaocheng Yu,Kui Jiang,Junjun Jiang,Xianming Liu,Guanglu Sun,Yi Xiao*

Main category: cs.CV

TL;DR: 提出MPHM网络用于图像去雨，通过整合文本和视觉先验，采用分层Mamba模块和渐进式先验融合注入，在多个数据集上实现SOTA性能。
...
Method: 提出MPHM网络，整合CLIP文本先验和DINOv2视觉先验，采用渐进式先验融合注入策略和分层Mamba模块进行特征表示。

Result: 在Rain200H数据集上获得0.57 dB PSNR增益，在真实雨天场景中表现出优越的泛化能力。

Conclusion: MPHM网络通过多先验融合和分层Mamba架构，有效提升了图像去雨的性能和细节保真度。
...
### [237] [A Lightweight 3D Anomaly Detection Method with Rotationally Invariant Features](https://arxiv.org/abs/2511.13115)
*Hanzhe Liang,Jie Zhou,Can Gao,Bingyang Guo,Jinbao Wang,Linlin Shen*

Main category: cs.CV

TL;DR: 提出了一种旋转不变特征框架用于3D异常检测，通过点坐标映射和轻量级卷积变换特征网络来应对点云方向和位置变化带来的挑战。
...
Conclusion: RIF框架通过旋转不变特征有效解决了3D异常检测中的方向变化问题，在多个数据集上表现优异，具有工业应用潜力。
...
### [238] [CloseUpShot: Close-up Novel View Synthesis from Sparse-views via Point-conditioned Diffusion Model](https://arxiv.org/abs/2511.13121)
*Yuqi Zhang,Guanying Chen,Jiaxing Chen,Chuanyu Fu,Chuan Huang,Shuguang Cui*

Main category: cs.CV

TL;DR: 提出了CloseUpShot框架，通过点条件视频扩散从稀疏输入实现特写视角的新视图合成，解决了特写场景下像素扭曲条件稀疏和背景泄漏的问题。


<details>
  <summary>Details</summary>
Motivation: 从稀疏输入视图重建3D场景和合成新视图具有挑战性，现有方法主要针对适度视角变化，在特写场景中难以捕捉细粒度细节，因为输入信息严重受限。
...
Conclusion: CloseUpShot框架通过改进的条件机制和全局几何约束，有效提升了稀疏输入下特写新视图合成的质量。
...
### [239] [Region-Point Joint Representation for Effective Trajectory Similarity Learning](https://arxiv.org/abs/2511.13125)
*Hao Long,Silin Zhou,Lisi Chen,Shuo Shang*

Main category: cs.CV

TL;DR: RePo是一种新颖的轨迹相似度计算方法，通过联合编码区域级和点级特征来捕获空间上下文和细粒度移动模式，在各项评估指标上比现有最优方法平均提升22.2%的准确率。


<details>
  <summary>Details</summary>
Motivation: 现有基于学习的方法虽然降低了传统轨迹相似度计算的计算复杂度，但未能充分利用轨迹信息的全面谱系进行相似度建模。

Method: 提出RePo方法：1) 区域级表示：将GPS轨迹映射为网格序列，通过结构特征和视觉特征增强的空间上下文捕获语义信息；2) 点级表示：使用三个轻量级专家网络从密集GPS序列中提取局部、相关和连续移动模式；3) 路由器网络自适应融合点级特征，并通过交叉注意力与区域级特征结合生成最终轨迹嵌入；4) 使用带有困难负样本的对比损失进行训练。

Result: 实验结果表明，RePo在所有评估指标上比现有最优基线方法平均提升了22.2%的准确率。

Conclusion: RePo通过联合编码区域级和点级特征，有效捕获了轨迹的空间上下文和细粒度移动模式，显著提升了轨迹相似度计算的性能。
...
### [240] [VEIL: Jailbreaking Text-to-Video Models via Visual Exploitation from Implicit Language](https://arxiv.org/abs/2511.13127)
*Zonghao Ying,Moyang Chen,Nizhang Li,Zhiqiang Wang,Wenxin Zhang,Quanchen Zou,Zonglei Jing,Aishan Liu,Xianglong Liu*

Main category: cs.CV

TL;DR: VEIL是一个针对文本到视频模型的越狱攻击框架，通过中性场景锚点、潜在听觉触发器和风格调制器的模块化提示设计，利用跨模态关联模式生成看似良性但语义不安全的视频。


<details>
  <summary>Details</summary>
Motivation: 现有文本到视频模型的越狱攻击通常通过添加明显不安全的对抗性扰动来实现，容易被检测和防御。本文旨在探索如何通过看似良性的提示诱导模型生成违反安全策略的视频。

Method: 提出VEIL框架，包含三个模块化组件：中性场景锚点提供表面场景描述，潜在听觉触发器利用音频-视觉共现先验，风格调制器通过电影指令增强触发效果。通过约束优化和引导搜索平衡隐蔽性和有效性。

Result: 在7个文本到视频模型上的广泛实验表明，该攻击方法在商业模型中的平均攻击成功率提升了23%。

Conclusion: VEIL框架证明了看似良性的提示可以成功诱导文本到视频模型生成语义不安全的视频，揭示了现有安全防护的关键盲点。
...
### [241] [MedGEN-Bench: Contextually entangled benchmark for open-ended multimodal medical generation](https://arxiv.org/abs/2511.13135)
...
TL;DR: MedGEN-Bench是一个全面的医学多模态基准测试，包含6,422个专家验证的图像-文本对，涵盖6种成像模态、16个临床任务和28个子任务，旨在解决现有医学视觉基准在查询模糊性、诊断推理简化和图像生成能力评估不足等问题。


<details>
  <summary>Details</summary>
Motivation: 随着视觉语言模型在医学应用中的普及，临床医生期望AI系统不仅能生成文本诊断，还能产生与真实临床工作流程无缝集成的医学图像。现有医学视觉基准存在查询模糊、诊断推理过度简化以及忽视图像生成能力等问题。

Method: 提出MedGEN-Bench基准，包含三种格式：视觉问答、图像编辑和上下文多模态生成。采用三层评估框架，整合像素级指标、语义文本分析和专家指导的临床相关性评分，系统评估了10个组合框架、3个统一模型和5个视觉语言模型。

Result: 构建了包含6,422个专家验证图像-文本对的综合基准，涵盖多种成像模态和临床任务。评估框架能够全面衡量模型在医学图像生成和多模态推理方面的性能。

Conclusion: MedGEN-Bench通过关注上下文交织的指令和开放式生成输出，推动了医学AI研究的发展，为评估医学多模态系统的性能提供了更全面的基准。
...
### [242] [WinMamba: Multi-Scale Shifted Windows in State Space Model for 3D Object Detection](https://arxiv.org/abs/2511.13138)
*Longhui Zheng,Qiming Xia,Xiaolu Chen,Zhaoliang Liu,Chenglu Wen*

Main category: cs.CV

TL;DR: WinMamba是一种基于Mamba的3D特征编码骨干网络，通过窗口尺度自适应模块和窗口移位策略，在保持计算效率的同时有效捕获长距离空间依赖关系，显著提升了3D目标检测性能。


<details>
  <summary>Details</summary>
Motivation: 解决3D目标检测中同时最大化计算效率和捕获长距离空间依赖关系的根本挑战。现有Mamba方法在固定窗口内进行轴对齐扫描会丢失空间信息。

Method: 提出WinMamba块组成的骨干网络，包含窗口尺度自适应模块补偿不同分辨率下的体素特征，以及带有可学习位置编码和窗口移位策略的WinMamba层来丰富线性状态空间中的上下文信息。

Result: 在KITTI和Waymo数据集上的广泛实验表明，WinMamba显著优于基线方法。消融研究验证了WSF和AWF模块对提升检测精度的独立贡献。

Conclusion: WinMamba通过创新的窗口尺度自适应和窗口移位策略，在3D目标检测中实现了计算效率和长距离依赖捕获的良好平衡，代码将公开。
...
### [243] [Skeletons Speak Louder than Text: A Motion-Aware Pretraining Paradigm for Video-Based Person Re-Identification](https://arxiv.org/abs/2511.13150)
*Rifen Lin,Alex Jinpeng Wang,Jiawei Mo,Min Li*

Main category: cs.CV

TL;DR: CSIP-ReID是一个基于骨架序列的预训练框架，通过对比学习对齐骨架和视觉特征，并引入原型融合更新器和骨架引导时序建模模块，在视频行人重识别任务中取得最先进性能。


<details>
  <summary>Details</summary>
Motivation: 现有基于文本的多模态预训练方法在视频行人重识别中存在两个基本限制：缺乏真正的多模态预训练，以及文本无法捕捉细粒度时序运动信息。

Method: 提出两阶段方法：第一阶段使用对比学习在序列级别对齐骨架和视觉特征；第二阶段引入动态原型融合更新器融合运动和外观线索，以及骨架引导时序建模模块将骨架的时序线索整合到视觉特征中。

Result: 在标准视频ReID基准测试中达到新的最先进结果，并在骨架only ReID任务上表现出强大的泛化能力，显著优于先前方法。
...
### [244] [THIR: Topological Histopathological Image Retrieval](https://arxiv.org/abs/2511.13170)
*Zahra Tabatabaei,Jon Sporring*

Main category: cs.CV

TL;DR: THIR是一个基于拓扑数据分析的无监督医学图像检索框架，利用Betti数和持续同调从组织病理学图像中提取结构特征，无需训练即可实现高效检索。


<details>
  <summary>Details</summary>
Motivation: 乳腺癌是全球女性主要死因之一，早期诊断和准确临床决策至关重要。传统深度学习方法需要大量标注数据和GPU资源，限制了在医疗领域的应用。

Method: 使用立方体持续性从RGB组织病理学图像中提取拓扑指纹，将环的演化编码为紧凑可解释的特征向量，通过计算拓扑描述符间的距离进行相似性检索。

Result: 在BreaKHis数据集上的实验表明，THIR在标准CPU上20分钟内处理完整数据集，性能优于现有有监督和无监督方法。
...
### [245] [HDW-SR: High-Frequency Guided Diffusion Model based on Wavelet Decomposition for Image Super-Resolution](https://arxiv.org/abs/2511.13175)
*Chao Yang,Boqian Zhang,Jinghao Xu,Guang Jiang*

Main category: cs.CV

TL;DR: 提出基于小波分解的高频引导扩散网络HDW-SR，通过在小波域进行扩散和稀疏交叉注意力机制，有效恢复图像高频细节，提升超分辨率性能。
...
Conclusion: HDW-SR通过小波域的高频引导机制，有效解决了扩散模型在超分辨率任务中高频细节恢复不足的问题，实现了更好的细节恢复效果。
...
### [246] [GenTract: Generative Global Tractography](https://arxiv.org/abs/2511.13183)
*Alec Sargood,Lemuel Puglisi,Elinor Thompson,Mirco Musolesi,Daniel C. Alexander*

Main category: cs.CV

TL;DR: GenTract是首个用于全局纤维束追踪的生成模型，将纤维束追踪构建为生成任务，直接从dMRI映射到完整、解剖学上合理的流线。相比现有方法，在精度上显著提升，特别是在低分辨率和噪声数据上表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有的局部纤维束追踪方法容易产生误差累积和高假阳性率，而全局方法计算成本高昂。需要一种既能保持高精度又能在不完美数据上可靠工作的解决方案。

Method: 将纤维束追踪构建为生成任务，学习从dMRI到完整流线的直接映射。比较了基于扩散和流匹配的两种范式，并与最先进的基线方法进行对比。

Result: GenTract的精度比次优方法TractOracle高2.1倍。在具有挑战性的低分辨率和噪声设置中，其优势更加明显，比最接近的竞争对手高出一个数量级。

Conclusion: GenTract在研究级数据上产生高精度的纤维束图，同时在不完美的低分辨率数据上保持可靠性，是全局纤维束追踪的有前景解决方案。
...
### [247] [Large Language Models Meet Extreme Multi-label Classification: Scaling and Multi-modal Framework](https://arxiv.org/abs/2511.13189)
*Diego Ortego,Marlon Rodríguez,Mario Almagro,Kunal Dahiya,David Jiménez,Juan C. SanMiguel*

Main category: cs.CV

TL;DR: ViXML框架通过整合视觉基础模型和大型解码器模型，在极端多标签分类任务中显著提升性能，同时保持计算效率。
...
Method: 提出ViXML框架，集成大型解码器模型和视觉基础模型，通过每张图像提取单个嵌入来限制计算增长，同时解锁多模态能力。

Result: 在四个公开数据集上验证，ViXML在最大数据集上P@1指标提升达+8.21%，视觉增强的小编码器在多数情况下优于纯文本解码器。

Conclusion: 视觉信息在XMC中具有重要价值，ViXML框架成功平衡了性能与效率，为多模态XMC提供了有效解决方案。
...
### [248] [Video Spatial Reasoning with Object-Centric 3D Rollout](https://arxiv.org/abs/2511.13190)
*Haoran Tang,Meng Cao,Ruyang Liu,Xiaoxi Liang,Linglong Li,Ge Li,Xiaodan Liang*

Main category: cs.CV

TL;DR: 提出了Object-Centric 3D Rollout (OCR)方法，通过在训练中对选定对象的3D几何结构引入结构化扰动，强制模型进行整体场景推理，解决了多模态大语言模型在视频空间推理中的查询锁定问题。


<details>
  <summary>Details</summary>
Motivation: 现有的多模态大语言模型在视频空间推理（理解动态3D场景中物体位置、方向和相互关系）方面存在查询锁定问题，即模型只关注提示中明确提到的对象而忽略关键上下文线索。

Method: 提出OCR策略：1）对选定对象的3D几何结构引入结构化扰动，降低物体特定视觉线索；2）将改变的几何结构投影到2D空间；3）设计基于rollout的训练流程，联合利用原始视频和区域噪声视频来优化空间推理轨迹。

Result: 实验显示最先进性能：3B参数模型在VSI-Bench上达到47.5%准确率，优于多个7B基线模型。消融实验证实OCR优于先前的rollout策略（如T-GRPO、NoisyRollout）。

Conclusion: OCR方法通过结构化扰动3D几何结构，有效解决了多模态大语言模型在视频空间推理中的查询锁定问题，实现了更全面的场景理解能力。
...
### [249] [Birth of a Painting: Differentiable Brushstroke Reconstruction](https://arxiv.org/abs/2511.13191)
*Ying Jiang,Jiayin Lu,Yunuo Chen,Yumeng He,Kui Wu,Yin Yang,Chenfanfu Jiang*

Main category: cs.CV

TL;DR: 提出了一个可微分的笔触重建框架，统一了绘画、风格化纹理和涂抹操作，能够真实再现人类绘画-涂抹循环。
...
Method: 使用可微分笔触重建框架，通过并行可微分绘画渲染器优化单色和双色贝塞尔笔触，结合风格生成模块合成几何条件纹理，并引入可微分涂抹操作器实现自然色彩混合和阴影。

Result: 在油画、水彩、水墨和数字绘画上的广泛实验表明，该方法能产生逼真且富有表现力的笔触重建、平滑的色调过渡和丰富的风格化外观。

Conclusion: 该方法为表达性数字绘画创作提供了一个统一的模型，能够真实再现绘画过程中的笔触结构和色彩混合效果。
...
### [250] [Difficulty-Aware Label-Guided Denoising for Monocular 3D Object Detection](https://arxiv.org/abs/2511.13195)
...
<details>
  <summary>Details</summary>
Motivation: 单目3D目标检测由于深度线索固有的模糊性而存在根本性问题。现有DETR方法虽然通过全局注意力和辅助深度预测尝试缓解，但仍受限于不准确的深度估计，且忽略了实例级检测难度（如遮挡、距离、截断），导致检测性能不理想。
...
Conclusion: MonoDLGD通过难度感知的标签引导去噪策略，有效提升了单目3D目标检测的鲁棒性和准确性，特别是在处理不同复杂度的对象时表现出色。
...
### [251] [Self-Supervised Ultrasound Screen Detection](https://arxiv.org/abs/2511.13197)
*Alberto Gomez,Jorge Oliveira,Ramon Casero,Agis Chartsias*

Main category: cs.CV

TL;DR: 提出一种自监督流水线，从超声设备显示器照片中提取超声图像，绕过DICOM传输瓶颈，实现快速算法测试和原型开发。概念验证显示校正图像保留足够视觉保真度，心脏视图分类准确率达0.79。


<details>
  <summary>Details</summary>
Motivation: 超声设备内置显示器显示图像，但常规传输到医院系统依赖DICOM格式，存在传输瓶颈。需要一种方法绕过DICOM限制，实现快速算法测试和原型开发。

Method: 采用自监督流水线方法，从超声设备显示器拍摄的照片中提取超声图像，通过图像校正技术恢复原始图像质量。

Result: 在概念验证研究中，校正后的图像保留了足够的视觉保真度，心脏视图分类的平衡准确率达到0.79，与原始DICOM图像相比具有良好的一致性。

Conclusion: 该方法成功绕过了DICOM传输瓶颈，为快速测试和开发新的超声图像算法提供了可行方案，校正图像质量足以支持临床应用级别的分类任务。
...
### [252] [RefineVAD: Semantic-Guided Feature Recalibration for Weakly Supervised Video Anomaly Detection](https://arxiv.org/abs/2511.13204)
*Junhee Lee,ChaeBeen Bang,MyoungChul Kim,MyeongAh Cho*

Main category: cs.CV

TL;DR: RefineVAD是一个弱监督视频异常检测框架，通过模拟人类感知异常的双过程推理，整合运动感知时间注意力和类别导向细化模块，同时建模运动演化和语义类别特征。


<details>
  <summary>Details</summary>
Motivation: 现有方法将异常事件简单视为单一类别，忽略了真实世界异常在语义和时间特征上的多样性。受人类感知异常方式的启发，需要同时解释不同异常类型的运动模式和语义结构。

Method: 提出RefineVAD框架，包含两个核心模块：MoTAR模块通过运动显著性估计和基于移位注意力的动态时间焦点调整；CORE模块通过跨注意力将片段级特征与可学习的类别原型对齐，注入软异常类别先验。

Result: 在WVAD基准测试上的广泛实验验证了RefineVAD的有效性，证明了整合语义上下文来引导特征细化对异常相关模式的重要性。
...
### [253] [End-to-End Multi-Person Pose Estimation with Pose-Aware Video Transformer](https://arxiv.org/abs/2511.13208)
*Yonghui Yu,Jiahang Cai,Xun Wang,Wenwu Yang*

Main category: cs.CV

TL;DR: PAVE-Net是一个端到端的多人视频姿态估计框架，消除了传统两阶段方法中的启发式操作，通过姿态感知注意力机制实现跨帧的个体关联，在准确性和效率上都有显著提升。
...
Method: 提出PAVE-Net框架，包含空间编码器建模帧内关系和时空姿态解码器捕获跨帧全局依赖。核心创新是姿态感知注意力机制，使每个姿态查询能够选择性聚合连续帧中相同个体的特征，并显式建模姿态关键点间的时空依赖。

Result: 在PoseTrack2017上比基于图像的端到端方法提升6.0 mAP，与最先进的基于视频的两阶段方法精度相当，同时在效率上有显著提升。

Conclusion: PAVE-Net是第一个用于多帧2D人体姿态估计的端到端方法，成功消除了传统两阶段流程中的启发式操作，在准确性和效率上都取得了显著改进。
...
### [254] [3DAlign-DAER: Dynamic Attention Policy and Efficient Retrieval Strategy for Fine-grained 3D-Text Alignment at Scale](https://arxiv.org/abs/2511.13211)
*Yijia Fan,Jusheng Zhang,Kaitong Cai,Jing Yang,Jian Wang,Keze Wang*

Main category: cs.CV

TL;DR: 提出了3DAlign-DAER框架，通过动态注意力策略和高效检索策略实现文本与3D几何的细粒度对齐，并在大规模3D数据库上表现出色。
...
Method: 提出动态注意力策略（DAP），使用分层注意力融合模块表示细粒度token-to-point注意力，并通过蒙特卡洛树搜索动态校准注意力权重；提出高效检索策略（ERS）在大规模嵌入空间中进行分层搜索；构建Align3D-2M数据集（200万文本-3D对）。

Result: 在多个基准测试中表现出优越性能，在准确性和效率上均优于传统方法（如KNN）。

Conclusion: 3DAlign-DAER通过动态注意力策略和高效检索策略有效解决了细粒度文本-3D对齐问题，在大规模场景下具有显著优势。
...
### [255] [Hybrid-Domain Adaptative Representation Learning for Gaze Estimation](https://arxiv.org/abs/2511.13222)
...
<details>
  <summary>Details</summary>
Motivation: 解决基于外观的注视估计方法在跨域评估中因表情、佩戴物和图像质量等无关因素干扰而性能显著下降的问题。

Method: 使用混合域自适应表示学习框架，通过无监督域适应方式对齐高质量近眼图像特征，并设计稀疏图融合模块探索注视方向与头部姿态的几何约束。

Result: 在EyeDiap、MPIIFaceGaze和Gaze360数据集上分别达到5.02°、3.36°和9.26°的最新精度，并在跨数据集评估中表现出竞争力。

Conclusion: HARL框架能有效学习鲁棒的注视表示，在跨域场景下实现高精度的注视估计。
...
### [256] [MRIQT: Physics-Aware Diffusion Model for Image Quality Transfer in Neonatal Ultra-Low-Field MRI](https://arxiv.org/abs/2511.13232)
...
Method: 结合K空间降级模拟物理一致的uLF图像生成，使用v预测和分类器自由引导的稳定图像生成，以及SNR加权的3D感知损失保持解剖保真度，采用体积注意力-UNet架构进行结构保持转换。

Result: 在新生儿队列训练中，MRIQT在PSNR指标上超过现有GAN和CNN方法15.3%，比最先进技术高出1.78%，医生评估85%的输出图像质量良好且病理清晰可见。
...
### [257] [MMD-Thinker: Adaptive Multi-Dimensional Thinking for Multimodal Misinformation Detection](https://arxiv.org/abs/2511.13242)
...
<details>
  <summary>Details</summary>
Motivation: 多模态虚假信息在AIGC时代快速演化，创建成本低、欺骗性高，对社会构成重大威胁。现有基于通用MLLMs的检测方法存在推理不足和推理偏见两个关键局限。

Method: 1) 为多模态虚假信息检测设计定制化思维模式；2) 通过任务特定指令调优将定制思维注入通用MLLMs；3) 使用混合优势函数的强化学习策略增强推理能力；4) 构建包含8K+图像-文本对的多模态虚假信息推理数据集。

Result: 在域内和域外基准数据集上均实现了最先进的性能，同时保持了灵活的推理和令牌使用。

Conclusion: MMD-Thinker框架有效解决了多模态虚假信息检测中的推理不足和偏见问题，为应对快速演化的多模态虚假信息提供了有效解决方案。
...
### [258] [Referring Camouflaged Object Detection With Multi-Context Overlapped Windows Cross-Attention](https://arxiv.org/abs/2511.13249)
*Yu Wen,Shuyong Gao,Shuping Zhang,Miao Huang,Lili Tao,Han Yang,Haozhe Xing,Lihe Zhang,Boxue Hou*

Main category: cs.CV

TL;DR: 提出RFMNet模型，通过多阶段编码特征交互融合和重叠窗口交叉注意力机制，提升参考伪装目标检测性能，在Ref-COD基准测试中达到最优效果。


<details>
  <summary>Details</summary>
Motivation: 现有方法将参考显著图像转化为一维提示，我们希望探索通过丰富的显著图像特征和伪装目标特征的多上下文融合来进一步提升性能。

Method: 提出RFMNet模型，利用参考显著图像的多编码阶段特征，在对应编码阶段与伪装特征进行交互融合；设计重叠窗口交叉注意力机制关注局部信息匹配；提出参考特征聚合模块逐步解码分割伪装目标。

Result: 在Ref-COD基准测试上的广泛实验表明，我们的方法达到了最先进的性能。

Conclusion: 通过多阶段特征交互融合和局部注意力机制，RFMNet在参考伪装目标检测任务中取得了优异性能，证明了所提方法的有效性。
...
### [259] [Building Egocentric Procedural AI Assistant: Methods, Benchmarks, and Challenges](https://arxiv.org/abs/2511.13261)
*Junlong Li,Huaiyuan Xu,Sijie Cheng,Kejun Wu,Kim-Hui Yap,Lap-Pui Chau,Yi Wang*

Main category: cs.CV

TL;DR: 本文提出了以自我为中心的程序AI助手概念，通过第一人称视角为日常程序性任务提供逐步支持，并定义了三个核心任务：错误检测、程序学习和问答。


<details>
  <summary>Details</summary>
Motivation: 受视觉语言模型和自我中心感知研究的进展驱动，旨在开发能够支持日常程序性任务的AI助手。

Method: 提出了新的分类法，包含三个核心任务，并对现有技术、数据集和评估指标进行全面回顾，同时通过实验评估代表性VLM方法。

Result: 识别了现有VLM助手与提议的EgoProceAssist之间的差距，提供了全面的技术评估。
...
### [260] [SymGS : Leveraging Local Symmetries for 3D Gaussian Splatting Compression](https://arxiv.org/abs/2511.13264)
*Keshav Gupta,Akshat Sanghvi,Shreyas Reddy Palley,Astitva Srivastava,Charu Sharma,Avinash Sharma*

Main category: cs.CV

TL;DR: SymGS是一个基于对称性感知的3D高斯泼溅压缩框架，通过引入可学习镜像来消除局部和全局的反射冗余，实现高效压缩。
...
Method: 提出SymGS框架，引入可学习镜像来识别和消除场景中的对称冗余，可作为现有压缩方法（如HAC）的即插即用增强模块。

Result: 相比HAC方法，在基准数据集上实现1.66倍压缩（大规模场景可达3倍），平均实现108倍压缩，同时保持渲染质量。

Conclusion: SymGS通过对称性感知技术有效突破了现有压缩方法的限制，为3D高斯泼溅提供了高效的内存压缩解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型在无人机场景中的空间智能能力未被充分探索，存在导航和动态环境理解方面的担忧。

Method: 构建包含环境感知和场景理解两大类的13个子类别的基准测试，开发包含100万样本的SpatialSky-Dataset，并设计专门用于无人机空间推理的Sky-VLM模型。
...
TL;DR: 提出基于视频级监督的双主干框架，结合卷积和Transformer表示，通过top-k池化检测监控视频中的罕见多样异常


<details>
  <summary>Details</summary>
Motivation: 解决在仅有视频级监督条件下检测监控视频中罕见且多样异常行为的挑战

Method: 使用双主干框架结合卷积和Transformer表示，采用top-k池化策略

Result: 在UCF-Crime数据集上达到90.7%的AUC

Conclusion: 双主干框架能有效检测监控视频中的罕见异常行为
...
TL;DR: SF-Recon是一种直接从多视角图像重建轻量化建筑表面的方法，无需后处理网格简化，通过3D高斯泼溅、法向梯度引导优化和多视角约束三角化实现高效重建。


<details>
  <summary>Details</summary>
Motivation: 传统多视角几何流程依赖密集重建、网格化和后续简化，过程繁琐且质量敏感，需要直接重建轻量化建筑表面模型的方法。

Method: 首先训练3D高斯泼溅场获得视图一致表示，然后通过法向梯度引导的高斯优化选择与屋顶和墙壁边界对齐的基元，再通过多视角边缘一致性剪枝增强结构锐度，最后用多视角深度约束的Delaunay三角化将结构化高斯场转换为轻量化建筑网格。

Result: 在提出的SF数据集上，SF-Recon能够直接从多视角图像重建轻量化建筑模型，显著减少面和顶点数量，同时保持计算效率。

Conclusion: SF-Recon方法能够直接从多视角图像高效重建轻量化的建筑表面模型，避免了传统流程的繁琐后处理步骤。
...
TL;DR: 提出了DTO方法解决多人人体网格恢复中的场景一致性缺失问题，创建了DTO-Humans数据集，并开发了Metric-Aware HMR网络实现度量尺度的人体网格恢复。


<details>
  <summary>Details</summary>
Motivation: 现有的人体网格伪真值生成流程是单人中心的，缺乏场景级一致性，导致同一图像中个体深度和尺度冲突。

Method: 引入深度条件平移优化(DTO)方法联合优化所有个体的相机空间平移；提出Metric-Aware HMR端到端网络，包含相机分支和相对度量损失。

Result: 构建了DTO-Humans数据集（56万张高质量场景一致图像），在相对深度推理和人体网格恢复方面达到最先进性能。

Conclusion: DTO方法有效解决了多人场景中的深度和尺度一致性问题，Metric-Aware HMR网络实现了度量尺度的人体网格估计。
...
TL;DR: TabFlash是一个高效的表格理解多模态大语言模型，通过渐进式问题条件化、剪枝策略和token聚焦训练，在减少计算开销的同时实现了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 表格图像存在冗余背景区域和需要问题特定关注的特点，现有MLLM方法忽略了这些特性，导致视觉表示不具信息性和冗余。

Method: 1) 渐进式问题条件化：将问题信息以逐渐增加的频率注入ViT层；2) 剪枝策略：丢弃背景token提高效率；3) token聚焦训练：鼓励模型将关键信息集中在保留的token中。

Result: TabFlash实现了最先进的性能，优于开源和专有MLLM，同时比第二好的MLLM减少27%的FLOPs和30%的内存使用。

Conclusion: TabFlash通过结合渐进式问题条件化、剪枝和token聚焦，为表格理解提供了高效且有效的解决方案。
...
TL;DR: SkyReels-Text是一个无需字体标签或微调的字体可控框架，能够在保持视觉和谐的同时精确编辑海报中的多文本区域，支持各种字体样式。


<details>
  <summary>Details</summary>
Motivation: 现代图像编辑模型在细粒度、字体感知的文本操作方面仍存在不足，限制了其在专业设计工作流程（如海报编辑）中的实用性。

Method: 提出SkyReels-Text框架，用户只需提供裁剪的字形补丁即可控制字体样式，无需字体标签或推理时微调，支持同时编辑多个文本区域。
...
TL;DR: 提出了一个名为CorrectAD的自校正代理系统，通过扩散视频生成模型DriveSora和PM-Agent来自动纠正端到端自动驾驶规划器的故障案例，显著降低碰撞率。


<details>
  <summary>Details</summary>
Motivation: 端到端自动驾驶规划方法面临长尾问题（罕见但安全关键的故障案例），数据驱动方法的鲁棒性受到影响，需要自动化的故障纠正机制。

Method: 使用PM-Agent模拟产品经理制定数据需求，提出DriveSora扩散视频生成模型生成与3D布局对齐的时空一致视频，构建完整的自校正代理系统CorrectAD。

Result: 在nuScenes和内部数据集上测试多个端到端规划器，CorrectAD分别纠正了62.5%和49.8%的故障案例，碰撞率分别降低了39%和27%。

Conclusion: CorrectAD是一个端到端模型无关的自校正系统，能够有效提升任何端到端自动驾驶规划器的安全性和鲁棒性。
...
<details>
  <summary>Details</summary>
Motivation: 现有3D LiDAR点云生成方法存在序列生成能力不足、前景对象定位不准确和背景不真实等限制，阻碍了实际应用。

Method: 提出DriveLiDAR4D流水线，包含多模态条件和LiDAR4DNet序列噪声预测模型，实现端到端的序列LiDAR场景生成和全场景操作能力。

Result: 在nuScenes和KITTI数据集上评估，nuScenes数据集上FRD得分为743.13，FVD得分为16.96，相比当前SOTA方法UniScene，FRD提升37.2%，FVD提升24.1%。

Conclusion: 这是首个以端到端方式解决具有全场景操作能力的序列LiDAR场景生成的工作，显著超越了现有技术水平。
...
TL;DR: 提出基于YOLOv9-T的混合专家框架，通过自适应路由实现动态特征专业化，相比单一YOLOv9-T模型提升了mAP和AR指标


<details>
  <summary>Details</summary>
Motivation: 单一检测模型难以适应复杂场景下的多样化特征需求，需要更灵活的动态特征处理机制

Method: 采用混合专家框架，集成多个YOLOv9-T专家模型，通过自适应路由机制实现动态特征分配

Result: 相比单一YOLOv9-T模型，在目标检测任务中获得了更高的平均精度(mAP)和平均召回率(AR)

Conclusion: 混合专家框架通过动态特征专业化有效提升了目标检测性能，证明了多专家协作的优势
...
TL;DR: 论文提出了"What Color Is It"数据集来验证多模态大模型在颜色感知方面的视觉幻觉问题，并探索了解决方案。


<details>
  <summary>Details</summary>
Motivation: 随着多模态大模型的快速发展，这些模型在视觉感知（特别是颜色感知）方面容易受到信息干扰，存在幻觉风险。

Method: 通过构建"What Color Is It"数据集，使用简单方法触发多模态大模型中的单模态视觉幻觉，并分析其原因。

Result: 验证了多模态大模型在视觉模态中存在颜色感知幻觉问题，并提出了增强模型鲁棒性的潜在解决方案。

Conclusion: 多模态大模型在视觉感知方面存在脆弱性，特别是在颜色感知方面容易产生幻觉，需要开发更鲁棒的解决方案。
...
TL;DR: DelAnyFlow是一种分辨率无关的大规模农田边界制图方法，结合DelAny实例分割模型和结构化后处理流程，能够快速生成拓扑一致的矢量边界，在乌克兰603,000km²区域6小时内完成制图。


<details>
  <summary>Details</summary>
Motivation: 现有农田边界提取方法存在边界不完整、相邻田地合并以及难以扩展的问题，需要开发可扩展的精确制图方法。

Method: 基于YOLOv11骨干网络构建DelAny实例分割模型，在FBIS 22M数据集上训练，结合结构化后处理、合并和矢量化流程生成矢量边界。

Result: DelAny模型比SAM2精度提高100%以上，推理速度快400倍；在乌克兰识别出3.75M个田地（5m分辨率），显著优于现有产品。

Conclusion: 该方法为缺乏数字地籍数据的地区提供了可扩展、经济高效的农田边界制图解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注禁止生成图像中不存在内容的事实描述任务，而忽视了在故事写作等自愿想象任务中，模型需要生成超越图像的新内容，此时不应简单将想象内容视为幻觉。

Method: 提出VOPE方法，通过提出重新检查式问题来评估LVLM如何解释其响应中想象对象的存在性，根据模型解释与图像中对象存在性的一致性来判断是否产生幻觉。

Result: 应用VOPE评估主流LVLMs和幻觉缓解方法发现：(1)大多数LVLMs在自愿想象中严重幻觉，对想象对象的存在性评估表现很差；(2)现有幻觉缓解方法在自愿想象任务中效果有限。

Conclusion: 自愿想象任务中的幻觉问题是一个重要的研究方向，现有方法对此类任务效果不佳，需要开发专门针对想象任务的幻觉缓解技术。
...
TL;DR: 提出基于流匹配模型的3D形状映射新表示方法，支持跨表示形式的形状匹配，无需大规模训练或数据驱动过程
...
Method: 将3D形状表示为从固定锚分布通过连续可逆流映射诱导的概率分布，通过源形状到锚分布的逆流与锚分布到目标形状的正向流组合实现点对点映射

Result: 在多样化基准测试和挑战性设置中实现高覆盖率和准确性，在人体点云扫描的UV映射和配准等任务中也表现出良好效果
...
TL;DR: InterMoE是一个基于动态时间选择性专家混合的新框架，用于生成高质量的人类交互动作，在保持个体特征的同时确保语义保真度。
...
Result: 在InterHuman数据集上FID分数降低9%，在InterX数据集上降低22%，实现了最先进的个体特定高保真3D人类交互生成性能。
...
TL;DR: LGIP基准测试评估视觉语言模型对语义保持改写和语义改变翻转的鲁棒性，发现EVA02-CLIP和大型OpenCLIP变体在不变性和敏感性方面表现最佳，而SigLIP模型存在严重问题。


<details>
  <summary>Details</summary>
Motivation: 评估视觉语言模型对受控语言扰动的响应可靠性，特别是对语义保持改写的不变性和对语义改变翻转的敏感性。

Method: 使用40k MS COCO图像和人工标注，自动生成改写和基于规则的语义翻转（改变对象类别、颜色或数量），通过不变性误差、语义敏感性差距和正率统计来总结模型行为。

Result: EVA02-CLIP和大型OpenCLIP变体在不变性-敏感性边界上表现最佳，而SigLIP和SigLIP2显示出更大的不变性误差，并且经常偏好翻转的标题而非人类描述。

Conclusion: LGIP为视觉语言模型的语言鲁棒性提供了模型无关的诊断工具，超越了传统的检索指标，揭示了标准指标无法发现的模型失败情况。
...
TL;DR: 本研究提出基于深度学习的框架监测中国城中村的时空变化，发现城中村改造过程通常漫长，主要发生在城市外围区域，并揭示了三种时空转化路径。
...
Result: 1)城中村改造过程通常漫长；2)改造主要发生在城市外围区域，城市核心区相对稳定；3)揭示了同步改造、延迟改造和渐进优化三种时空转化路径。

Conclusion: 城中村改造具有碎片化、复杂性和非线性特征，需要分层级和因地制宜的规划策略，研究结果为更包容、高效和可持续的城市更新提供了实证支持。
...
TL;DR: 提出了一种渐近极小极大方法用于多目标共形预测，为成像逆问题中的不确定性量化提供紧致的预测区间，同时确保联合边际覆盖。
...
Method: 提出渐近极小极大多目标共形预测方法，在确保联合边际覆盖的同时提供紧致的预测区间。该方法可应用于多指标盲图像质量评估、多任务不确定性量化和多轮测量采集。

Result: 数值实验表明，与现有的多目标共形预测方法相比，所提出的极小极大方法在合成数据和磁共振成像数据上具有优势。

Conclusion: 该方法为多目标成像逆问题提供了有效的不确定性量化工具，在多个实际应用中展现出优越性能。
...
TL;DR: 该论文揭示了一种新的攻击类型，通过联邦学习中的微小颜色扰动来破坏模型可解释性而不影响准确性，挑战了正确预测意味着忠实解释的常见假设。


<details>
  <summary>Details</summary>
Motivation: 随着机器学习模型在安全关键领域的部署，可解释性技术变得至关重要。本文旨在揭示模型可解释性本身可能成为攻击面的新威胁。

Method: 提出色度扰动模块，通过在联邦学习中系统性地改变前景与背景的颜色对比度来制作对抗样本，这些扰动在训练轮次中累积，以隐蔽持久的方式毒化全局模型的内部特征归因。

Result: 攻击将Grad-CAM解释中的峰值激活重叠减少了高达35%，同时在所有评估数据集上保持分类准确率超过96%。标准训练流程无法检测或缓解解释退化。

Conclusion: 研究结果表明可解释性本身可能成为攻击面，挑战了模型审计中正确预测意味着忠实解释的常见假设，特别是在联邦学习环境中，细微的颜色扰动更难被发现。
...
<details>
  <summary>Details</summary>
Motivation: 现有的OOD检测器在处理语义上与ID类别相似的OOD样本时表现不佳，需要一种能够专门处理这类语义挑战性OOD样本的检测方法。

Method: 通过简单变换ID表示来合成伪OOD特征，利用神经崩溃现象，引入基于特征范数的轻量级辅助头进行半径分类，将OOD检测与主分类器解耦。

Result: 在CIFAR-10、CIFAR-100和ImageNet-200上的实验表明，BootOOD优于先验的后处理方法，超越无需异常暴露的训练方法，与最先进的异常暴露方法竞争，同时保持或提高ID准确性。

Conclusion: BootOOD提供了一种有效的自监督OOD检测解决方案，特别适用于处理语义相似的OOD样本，在保持ID性能的同时显著提升OOD检测能力。
...
<details>
  <summary>Details</summary>
Motivation: 单目高度估计在遥感3D感知中很重要，但深度学习方法受限于标注数据的稀缺性和获取成本高的问题，需要利用大量未标注数据来提升模型性能。

Method: 设计TSE-Net自训练管道，包含教师、学生和考试网络。教师网络生成伪标签，学生网络在未标注数据上训练，考试网络作为学生网络的时序集成以稳定性能。教师网络采用回归和分类联合建模，通过分层双切策略处理高度值的长尾分布，使用Plackett-Luce模型校准类别概率。

Result: 在三个不同分辨率和成像模式的数据集上进行了评估，代码已开源。

Conclusion: 提出的半监督学习框架能够有效利用未标注数据，提升单目高度估计的性能，解决了标注数据稀缺的挑战。
...
TL;DR: Opt3DGS通过两阶段优化过程解决3DGS优化中的局部最优陷阱和收敛质量不足问题，提升渲染质量


<details>
  <summary>Details</summary>
Motivation: 3D高斯泼溅在新型视图合成中表现优异，但其核心优化问题未被充分探索，存在局部最优陷阱和收敛质量不足两个关键问题

Method: 提出Opt3DGS框架，包含两阶段优化：探索阶段使用自适应加权随机梯度朗之万动力学增强全局搜索；利用阶段使用局部拟牛顿方向引导的Adam优化器利用曲率信息进行精确收敛

Result: 在多个基准数据集上的广泛实验表明，Opt3DGS通过优化3DGS的优化过程而不改变其基础表示，实现了最先进的渲染质量

Conclusion: Opt3DGS是一个稳健的框架，有效解决了3DGS优化中的关键挑战，显著提升了渲染性能
...
TL;DR: NuClass是一个病理学家工作流程启发的框架，通过多尺度整合核形态和微环境上下文进行细胞分类，解决了现有方法缺乏组织上下文和细粒度标注的问题。


<details>
  <summary>Details</summary>
Motivation: 现有基于图像块的模型能捕捉详细的核形态但缺乏组织上下文，且人类标注通常是粗粒度和不均匀分布的，难以获得细粒度的亚型级监督。
...
TL;DR: 提出ICLR框架，包含DIEM模块和CCL损失函数，通过改进色度与亮度交互来解决低光图像增强中的分布差异和梯度冲突问题。


<details>
  <summary>Details</summary>
Motivation: 自然图像中色度与亮度分支间的显著分布差异限制了互补特征提取，亮度误差通过非线性参数传播到色度通道，且大均匀色区域的色度分支间相关性弱，传统像素级损失导致梯度冲突。

Method: 提出双流交互增强模块(DIEM)从融合和增强两个维度改进互补信息提取，以及协方差校正损失(CCL)利用亮度残差统计惩罚色度误差并通过约束色度分支协方差平衡梯度冲突。
...
TL;DR: 提出一个机器学习框架，使用卷积配准神经网络学习根据特定属性（如年龄、性别）生成模板的函数，并能产生相应的解剖分割图。


<details>
  <summary>Details</summary>
Motivation: 现有模板开发成本高且数量有限，导致分析常使用不具代表性的模板，特别是在群体变异大的情况下。

Method: 使用卷积配准神经网络学习条件模板生成函数，结合分割信息生成解剖标签图，并能进行图像配准。

Result: 在3D脑部MRI数据集上验证，能学习到高质量的代表性模板，带标注的条件模板比无标签模板配准效果更好。

Conclusion: 该方法能有效生成条件模板，优于其他模板构建方法，在医学图像分析中具有应用价值。
...
TL;DR: TAND是一个新颖的组织感知细胞核检测框架，通过结合点级监督和组织掩码条件化，在减少标注负担的同时实现了联合细胞核检测和分类的SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖详细专家标注且未能充分利用组织上下文信息，这限制了计算病理学中细胞核检测和分类的准确性。

Method: TAND将ConvNeXt编码器-解码器与冻结的Virchow-2组织分割分支耦合，通过新颖的多尺度空间特征线性调制，利用语义组织概率有选择地调节分类流。
...
<details>
  <summary>Details</summary>
Motivation: 驾驶员疲劳是道路事故的主要原因之一，每年导致数千人死亡和受伤，需要开发有效的检测系统来提高道路安全

Method: 使用标准网络摄像头追踪驾驶员面部特征，重点分析眼部运动，采用Eye Aspect Ratio (EAR)方法和MediaPipe Face Mesh框架进行面部地标识别，通过OpenCV进行图像处理

Result: 实验分析表明系统具有高准确性和快速响应时间，能够有效检测长时间闭眼和低眨眼率等疲劳迹象

Conclusion: 该系统实现了高性能、低成本的驾驶员监控解决方案，可以作为现有高级驾驶辅助系统(ADAS)的组成部分
...
TL;DR: CacheFlow是一种无需训练的长视频问答解决方案，通过动态令牌丢弃和压缩长期记忆机制，在保持答案准确性的同时显著减少处理令牌数量。
...
Method: 结合动态令牌丢弃（基于帧间余弦相似性在线修剪补丁令牌）和压缩长期记忆（使用小型循环编码器构建检索索引，完整KV对卸载并在生成时重新加载）。
...
TL;DR: Part-X-MLLM是一个原生3D多模态大语言模型，通过结构化可执行语法将各种3D任务统一为程序，能够从RGB点云和自然语言提示生成包含部件级边界框、语义描述和编辑命令的连贯标记序列。


<details>
  <summary>Details</summary>
Motivation: 统一多样化的3D任务，通过结构化输出驱动下游几何感知模块，实现部件级生成和编辑，将符号规划与几何合成解耦。

Method: 采用双编码器架构预训练以分离结构和语义，在大规模部件中心数据集上进行指令调优，生成结构化可执行程序作为统一接口。

Result: 模型在生成高质量结构化规划方面表现优异，在基于问答、组合生成和局部化编辑等任务中实现最先进性能。

Conclusion: 通过单一语言原生前端控制兼容的几何引擎，Part-X-MLLM为3D多模态任务提供了统一且有效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有3D生成方法大多忽视物理和关节属性，限制了在具身AI中的应用。需要开发能够生成仿真就绪物理3D资产的框架。

Method: 提出首个基于VLM的物理3D生成模型，采用新的3D表示方法将几何token化，减少193倍token数量；构建PhysX-Mobility数据集，包含2000+真实物体和丰富物理标注。

Result: 在PhysX-Mobility和野外图像上的实验显示强大的生成性能和鲁棒泛化能力；在MuJoCo环境中验证资产可直接用于接触丰富的机器人策略学习。

Conclusion: PhysX-Anything能够显著赋能下游应用，特别是在具身AI和物理仿真领域。
...
TL;DR: DMDR结合强化学习与蒸馏过程，通过动态分布引导和重噪声采样策略，将多步扩散模型蒸馏为少步模型，在保持推理效率的同时超越教师模型性能。


<details>
  <summary>Details</summary>
Motivation: 传统蒸馏方法（如DMD）将多步扩散模型蒸馏为少步模型时，性能受限于教师模型。需要突破这一限制，让少步模型超越教师模型。

Method: 1. 将强化学习整合到蒸馏过程中；2. 使用DMD损失作为RL正则化；3. 设计动态分布引导和动态重噪声采样训练策略；4. 同时进行蒸馏和强化学习训练。

Result: DMDR在少步方法中达到领先的视觉质量和提示一致性，甚至超越了多步教师模型的性能。

Conclusion: DMDR框架成功突破了传统蒸馏的性能上限，通过RL与蒸馏的协同作用，实现了少步模型超越教师模型的突破性成果。
...
TL;DR: OlmoEarth是一个多模态时空基础模型，专为地球观测数据设计，在多个基准测试和实际任务中达到最先进性能，并部署为端到端平台供非营利组织使用。


<details>
  <summary>Details</summary>
Motivation: 地球观测数据具有空间性、时序性和多模态的独特挑战，需要专门的基础模型来处理这些复杂特性。

Method: 采用新颖的自监督学习公式、掩码策略和损失函数设计，专门针对地球观测领域优化。

Result: 在24个任务中的15个任务上获得最佳嵌入性能，在29个任务中的19个任务上通过全微调获得最佳性能，超越12个其他基础模型。

Conclusion: OlmoEarth成功解决了地球观测数据的多模态时空建模问题，并作为实用平台部署，为环境问题解决提供技术支持。
...
TL;DR: GS-Light是一种基于高斯泼溅的文本引导3D场景重光照方法，通过训练免费的扩散模型扩展处理多视图输入，结合大视觉语言模型解析光照先验，生成高质量的重光照3D场景。


<details>
  <summary>Details</summary>
Motivation: 现有方法在文本引导的3D场景重光照方面存在局限性，特别是在准确反映用户期望的光照方向方面。需要一种能够高效处理多视图输入并生成高质量重光照结果的方法。

Method: 使用大视觉语言模型解析用户提示为光照先验，结合几何和语义估计器计算光照图，生成初始潜在代码指导扩散模型。通过多视图渲染图像和初始潜在代码输入多视图重光照模型，最后微调3D高斯泼溅场景。

Result: 在室内外场景评估中，GS-Light在定量指标（多视图一致性、图像质量、美学评分、语义相似度等）和定性评估（用户研究）上均优于现有基线方法。

Conclusion: GS-Light提供了一种高效且有效的文本引导3D场景重光照解决方案，能够准确反映用户的光照要求，并在多个评估维度上表现优于现有方法。
...
TL;DR: 提出了TiViBench基准来评估图像到视频生成模型的推理能力，涵盖结构推理、空间视觉模式推理、符号逻辑推理和行动规划四个维度，并引入VideoTPO测试时优化策略来提升推理性能。
...
Method: 设计分层基准TiViBench，包含24个任务场景和3个难度级别，评估四个推理维度；提出VideoTPO测试时策略，通过LLM自分析生成候选来识别优缺点。

Result: 商业模型（如Sora 2、Veo 3.1）展现出更强的推理潜力，开源模型因训练规模和数据多样性限制而潜力未充分发掘；VideoTPO显著提升推理性能且无需额外训练。
...
TL;DR: FFSE是一个3D感知的自回归框架，能够在真实图像上实现直观、物理一致的物体编辑，通过建模为3D变换序列来支持平移、缩放、旋转等操作，同时保持背景效果和场景一致性。


<details>
  <summary>Details</summary>
Motivation: 现有文本到图像扩散模型在语义图像编辑方面取得进展，但大多数方法无法进行3D感知的物体操作，要么在图像空间操作，要么需要缓慢且容易出错的3D重建。

Method: FFSE将编辑建模为学习到的3D变换序列，引入3DObjectEditor混合数据集，从不同物体和场景的模拟编辑序列构建，支持多轮和动态条件下的有效训练。

Result: 大量实验表明，FFSE在单轮和多轮3D感知编辑场景中显著优于现有方法。

Conclusion: FFSE框架能够实现直观的3D感知物体编辑，保持物理一致性和场景一致性，为真实世界图像编辑提供了有效解决方案。
...
TL;DR: 提出SAAS模型和TMA数据增强策略，解决多镜头半监督视频目标分割问题，通过模拟镜头转换提升跨镜头泛化能力，并在新基准Cut-VOS上取得SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有VOS方法主要针对单镜头视频，难以处理镜头不连续性，限制了实际应用。需要解决多镜头视频分割中的镜头转换问题。

Method: 提出TMA数据增强策略，使用单镜头数据模拟跨镜头泛化；开发SAAS模型，能够有效检测和理解镜头转换；构建Cut-VOS多镜头分割基准。

Result: 在YouMVOS和Cut-VOS基准上的广泛实验表明，SAAS通过有效模拟、理解和分割复杂转换，实现了最先进的性能。

Conclusion: SAAS模型能够有效处理多镜头视频分割中的镜头转换问题，提出的TMA策略和Cut-VOS基准为多镜头VOS研究提供了重要支持。
...
TL;DR: 论文提出JiT方法，使用大patch尺寸的Transformer直接在像素层面预测干净图像，而非噪声，基于流形假设认为干净数据位于低维流形上。


<details>
  <summary>Details</summary>
Motivation: 当前扩散模型预测噪声而非干净图像，但根据流形假设，自然数据位于低维流形而噪声数据不在，因此直接预测干净图像能让网络在极高维空间中有效工作。

Method: 使用简单的大patch Transformer（patch尺寸16和32）直接在像素层面预测干净图像，无需tokenizer、预训练或额外损失函数。

Result: 在ImageNet 256×256和512×512分辨率上获得有竞争力的结果，而预测高维噪声的方法会灾难性失败。

Conclusion: JiT方法回归基础，为基于Transformer的扩散模型提供了一个自包含的范式，证明直接预测干净图像的可行性。
...
<details>
  <summary>Details</summary>
Motivation: 解决传统模糊匹配在处理语义相同但句法不同的引用时失败的问题，特别是在对齐官方书面记录和语音转文字记录的场景中。

Method: 采用两阶段方法：首先使用RapidFuzz进行预过滤，然后使用LLM在短片段上进行验证。通过优化提示设计（将查询放在文本前、使用紧凑格式）和适度推理预算来提高性能。

Result: 该方法将模糊匹配准确率提高了最多50个百分点，同时将延迟减半，每个正确结果的成本降低了最多96%。在10个文本记录（5万-90万标记）上的扩展测试证实了对文本长度、词汇漂移和领域变化的鲁棒性。

Conclusion: 提出的"辅助模糊"方法在保持高拒绝准确率（95-100%）的同时，显著提高了非逐字引用时间戳检索的效率和准确性。
...
TL;DR: MiroThinker v1.0是一个开源研究代理，通过交互式扩展作为性能提升的第三维度，在保持256K上下文窗口下可执行600次工具调用，在多个基准测试中超越先前开源代理并接近商业模型性能。


<details>
  <summary>Details</summary>
Motivation: 现有代理主要依赖模型规模或上下文长度扩展，但缺乏对代理-环境交互深度的系统探索。MiroThinker旨在通过交互式扩展作为性能提升的第三维度，利用环境反馈纠正错误并优化推理轨迹。

Method: 采用强化学习训练模型实现交互式扩展，模型在256K上下文窗口下可执行多达600次工具调用，支持持续多轮推理和复杂研究工作流。

Result: 72B变体在GAIA、HLE、BrowseComp和BrowseComp-ZH四个基准测试中分别达到81.9%、37.7%、47.1%和55.6%的准确率，超越先前开源代理并接近GPT-5-high等商业模型。

Conclusion: 交互深度展现出与模型规模和上下文长度类似的扩展行为，确立了交互式扩展作为构建下一代开源研究代理的第三关键维度。
...
TL;DR: 论文质疑语言模型是否真正具备推理能力，认为其表现出的推理行为实际上是统计模式匹配而非真正的逻辑推理机制


<details>
  <summary>Details</summary>
Motivation: 评估NLP领域中关于语言模型推理能力的定义，澄清语言模型训练、信息处理和生成过程的本质

Method: 假设基于transformer的语言模型实现了隐式有限阶马尔可夫核，将上下文映射到条件标记分布

Result: 语言模型的推理类输出对应于学习核中的统计规律性和近似统计不变性，而非显式逻辑机制的实现

Conclusion: 语言模型是统计模式匹配器而非真正的推理者，这一区分对于评估语言模型中的认知不确定性至关重要
...
TL;DR: 评估了7个开源的LLM模型（0.6B-70B参数）在水电许可文档信息提取中的表现，发现14B参数是一个关键阈值，小模型存在系统性幻觉问题，大模型性能更好但需要企业级基础设施。


<details>
  <summary>Details</summary>
Motivation: 研究大型语言模型在监管文档信息提取中的性能与计算资源之间的权衡，为实际部署提供实证指导。

Method: 使用7个不同参数规模的开源模型（0.6B-70B）在水电许可文档上进行信息提取评估，分析验证方法的效果。

Result: 发现14B参数是性能转折点：低于该阈值时验证方法无效（F1<0.15），高于该阈值时验证方法可行（F1=0.64）。消费级模型可达64% F1，小模型在51%达到瓶颈，大规模模型接近77% F1但需要企业基础设施。

Conclusion: 建立了首个针对监管环境下开源信息提取模型的资源-性能映射，为模型选择提供证据基础，研究结果对水电合规具有直接价值，且参数规模效应可推广到其他信息提取任务。
...
Method: 使用简单的基于LLM的自动形式化器，通过两个实验验证LLM生成输出与自然语言需求的一致性：1）识别不同表述需求的逻辑等价性；2）检测给定需求与LLM输出之间的逻辑不一致性。

Result: 实验一成功识别了两个不同表述的自然语言需求在逻辑上是等价的；实验二成功识别了给定自然语言需求与LLM生成输出之间的逻辑不一致性。
...
TL;DR: 提出一个分析电影剧本情感弧线的框架，通过字典式情感分析和层次聚类技术，提取叙事中的高低层概念，帮助用户选择故事。
...
Method: 使用基于NRC-VAD数据集的LabMTsimple storylab模块构建自定义词典，进行字典式情感分析，并应用Wards层次聚类技术聚类相似情感情节。

Result: 在电影数据集上的实验评估表明，该分析结果对消费者和读者在选择叙事或故事时很有帮助。

Conclusion: 该框架能够有效分析电影剧本的情感弧线，并通过聚类技术识别相似情感模式，为故事选择提供有价值的参考。
...
TL;DR: 本文构建了一个包含6,393份放射学报告的标注语料库，用于评估大型语言模型在随访依从性检测任务中的表现，发现GPT-4o在优化提示下达到最佳性能（F1=0.832），接近人类标注者水平。


<details>
  <summary>Details</summary>
Motivation: 目前缺乏专门用于评估LLMs在放射学任务表现的领域特定数据集，需要构建高质量标注语料库来系统比较传统机器学习方法和生成式LLMs在随访依从性检测任务中的性能。

Method: 构建了6,393份放射学报告的标注语料库，系统比较了逻辑回归、支持向量机、Longformer、微调Llama3-8B-Instruct与生成式LLMs（GPT-4o和GPT-OSS-20B）。对生成式LLMs测试了基础设置和任务优化设置，后者聚焦于元数据、推荐句子及其上下文。

Result: GPT-4o（优化设置）表现最佳（F1=0.832），GPT-OSS-20B（优化设置）紧随其后（F1=0.828）。逻辑回归和支持向量机也表现良好（F1分别为0.776和0.775）。人类标注者间一致性很高（F1=0.846）。

Conclusion: 虽然通过提示优化，LLMs能够接近人类水平的一致性，但可解释且资源效率高的传统模型仍然是重要的基准方法。
...
TL;DR: MedPT是首个针对巴西葡萄牙语的大规模真实世界医疗语料库，包含38.4万条医患问答对，通过多阶段筛选和LLM标注增强，在医疗专科分诊任务上达到94%的F1分数。


<details>
  <summary>Details</summary>
Motivation: 解决LLM在医疗领域主要关注高资源语言的问题，为葡萄牙语等低资源语言提供能够捕捉临床和文化细微差异的医疗数据集。

Method: 构建包含384,095条真实医患问答的语料库，采用混合定量-定性分析方法进行多阶段筛选，使用LLM驱动标注将问题分为7种语义类型，并通过微调1.7B参数模型进行医疗专科分诊任务验证。

Result: 数据集涵盖3,200个主题，具有独特的语言特性；在20类医疗专科分诊任务中达到94%的F1分数；错误分类反映了真实的临床模糊性，证明数据集的语义丰富性。

Conclusion: MedPT数据集将促进葡萄牙语世界更公平、准确和文化敏感的医疗技术发展，填补了低资源语言医疗NLP的重要空白。
...
TL;DR: ClinStructor使用LLM将临床自由文本转换为结构化问答对，以解决临床笔记中的偏见、泛化性和可解释性问题，在ICU死亡率预测任务中性能仅轻微下降。


<details>
  <summary>Details</summary>
Motivation: 临床笔记包含有价值信息但存在非结构化格式带来的挑战，包括无意偏见、跨临床环境泛化性差和可解释性差的问题。

Method: 利用大型语言模型将临床自由文本转换为结构化的任务特定问答对，作为预测建模的前置步骤。

Result: 在ICU死亡率预测任务中，相比直接微调方法，预测性能仅轻微下降（AUC下降2-3%），但显著提高了透明度和可控性。
...
TL;DR: 该研究通过监督微调和强化学习技术增强GPT-2的心理治疗对话生成能力，在多个评估指标上取得显著提升，特别是情感识别准确率达到99.34%。


<details>
  <summary>Details</summary>
Motivation: 新冠疫情加剧了心理健康服务的可及性问题，而现有大语言模型缺乏情境和情感意识，无法提供合适的治疗性回应。

Method: 重新构建输入格式以同时处理上下文信息和情感状态，采用多组件奖励函数使模型输出与专业治疗师回应和标注情感对齐。

Result: 强化学习在BLEU、ROUGE-1、ROUGE-2、ROUGE-L和METEOR指标上均优于基线GPT-2，情感识别准确率从66.96%提升至99.34%。
...
<details>
  <summary>Details</summary>
Motivation: 解决大语言模型在临床文本分类中预测不透明的问题，满足研究者和医生需要理解患者记录中哪些部分驱动风险信号的需求。

Method: 使用加性大语言模型框架，将半结构化文本输入分解为语义组件，预测结果是各组件贡献的总和，支持组件级风险曲线可视化。

Result: CALM在保持与传统LLM分类器相当性能的同时，提高了模型可信度，支持质量保证检查，并在模型开发和审计中揭示临床有意义的模式。

Conclusion: CALM框架通过加性结构实现了可解释的临床文本分类，平衡了性能与透明度，有望促进LLM在临床实践中的实际应用。
...
TL;DR: 提出了InData数据集来评估LLMs的多步骤工具推理能力，发现在复杂数据分析任务中现有模型表现显著下降


<details>
  <summary>Details</summary>
Motivation: 解决LLM代理直接生成和执行代码访问敏感数据的安全风险，通过预定义安全工具来限制直接数据访问
...
Conclusion: InData推动了具有更强多步骤工具使用能力的LLM的开发与评估
...
TL;DR: 论文提出LLM-KAT评估方法和实体匿名化技术，以解决大语言模型在基于知识图谱的对话生成中过度依赖内部知识而忽略外部知识图谱的问题。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在各种NLP任务中表现出色，但在基于知识图谱的对话生成任务中，模型往往过度依赖内部知识，无法有效利用提供的外部知识图谱，导致生成回复与知识图谱脱节。

Method: 1) 提出LLM-KAT评估方法来衡量生成回复中对外部知识的依附程度；2) 设计简单的实体匿名化技术，通过隐藏实体名称来鼓励模型更好地利用外部知识。

Result: 在OpenDialKG数据集上的实验表明，该方法有效提升了大语言模型对外部知识的依附能力。
...
TL;DR: 本文研究语言模型的熵校准问题，发现模型存在校准错误，随着生成文本变长，熵增加且文本质量下降。理论分析表明校准错误随规模改善缓慢，实证研究验证了这一点。作者证明在理论上可以通过预测未来熵来减少熵同时保持对数损失。


<details>
  <summary>Details</summary>
Motivation: 研究语言模型的熵校准问题，探索模型生成过程中熵增加和文本质量下降的根本问题，以及是否可以通过规模扩展或理论方法解决校准错误而不牺牲多样性。

Method: 首先在简化理论设置中分析校准错误的缩放行为与数据集大小的关系，然后实证测量从0.5B到70B参数的语言模型的校准错误，最后从理论上证明减少熵同时保持对数损失的可能性。

Result: 发现校准错误的缩放指数接近0，意味着更大模型与较小模型以相似速率积累错误；理论证明在假设可以预测文本未来熵的情况下，减少熵同时保持对数损失是可能的。

Conclusion: 语言模型的校准错误随规模改善缓慢，当前截断方法以牺牲多样性为代价；理论上存在不牺牲对数损失而减少熵的可能性，但需要能够预测未来熵的黑盒模型。
...
TL;DR: 提出了ReasoningNER框架，通过显式推理机制改进NER任务，在零样本设置下超越GPT-4 12.3个百分点


<details>
  <summary>Details</summary>
Motivation: 传统生成式LLM在NER任务中依赖隐式模式匹配，缺乏可验证的推理机制，导致性能不佳和泛化能力脆弱，特别是在零样本和低资源场景下

Method: 三阶段推理框架：1)生成NER导向的思维链数据集；2)使用思维链调优模型，使其在给出最终答案前生成连贯的推理过程；3)推理增强阶段，使用综合奖励信号优化推理过程

Result: 在零样本设置下达到SOTA性能，F1分数比GPT-4高出12.3个百分点，展示了在NER任务中令人印象深刻的认知能力

Conclusion: ReasoningNER框架成功将NER从隐式模式匹配转向显式推理，在推理导向的信息提取研究中具有巨大潜力
...
TL;DR: 该研究探讨了思维链解释在道德场景中的双重作用：既能提高透明度，也可能因确认偏见而误导用户，特别是当推理存在错误但输出看似合理时。


<details>
  <summary>Details</summary>
Motivation: 研究动机是理解思维链解释如何影响用户对多模态道德场景中AI推理的信任和错误检测能力，揭示解释可能带来的负面效应。

Method: 通过系统性地扰动推理链和操纵表达语调，分析视觉语言模型中的推理错误及其对用户信任和错误检测的影响。

Result: 研究发现：(1)用户常将信任等同于结果一致性，即使推理有缺陷仍保持依赖；(2)自信的语调会抑制错误检测但维持依赖，表明表达风格可能压倒正确性。

Conclusion: 思维链解释既能澄清也能误导，NLP系统需要提供鼓励审查和批判性思维而非盲目信任的解释。
...
TL;DR: 本文提出了一个评估大语言模型文化能力的新基准，通过真实情境而非抽象规范来测试模型的文化推理能力，并引入四个补充指标来全面评估响应质量。


<details>
  <summary>Details</summary>
Motivation: 现有的大语言模型文化能力评估方法局限于去情境化的正确性或强制选择判断，忽视了文化理解和推理的需求，无法反映模型在多元文化环境中的实际表现。

Method: 引入基于真实情境的基准测试，要求模型进行文化基础推理，并使用四个补充指标（覆盖率、特异性、内涵和连贯性）与标准精确匹配指标结合评估。

Result: 实证分析显示，传统评估方法系统性地高估文化能力且评估结果不稳定，而新提出的厚评估方法能揭示推理深度差异、减少方差，提供更稳定可解释的文化理解信号。

Conclusion: 厚评估方法比传统薄评估更适合评估大语言模型的文化能力，能够提供更准确、稳定和可解释的文化理解评估结果。
...
<details>
  <summary>Details</summary>
Motivation: 研究如何在有限训练数据下提升低资源语言对（英语→日语）的机器翻译质量，探索回译和微调技术的协同效应。

Method: 首先使用回译技术从单语日语语料生成合成数据，然后在小规模真实平行数据集上进行微调，最后将两种技术结合使用。

Result: 回译单独使用将COMET从0.460提升至0.468；微调单独使用提升至0.589；两者结合达到最佳效果0.597。

Conclusion: 即使训练数据有限，回译和目标微调的协同使用能显著提升翻译质量，为低资源语言对提供轻量级但强大的改进策略。
...
TL;DR: LLMLagBench是一个用于评估LLM训练数据时间边界的基准测试，通过检测模型对近期事件的了解来识别其知识截止时间。


<details>
  <summary>Details</summary>
Motivation: LLM在特定时间点前的文本数据上进行预训练，形成了严格的知识边界。当这个限制未知或被忽视时，模型可能在推理任务中无意间混合过时的时效性信息与通用知识，从而影响回答准确性。

Method: 引入LLMLagBench作为系统性方法，通过评估LLM对近期事件的知识来识别其训练数据的最早可能时间边界。对大量LLM（包括明确声明和未声明训练截止时间的模型）进行评估。

Result: 通过手动验证和与公开发布的LLM预训练信息进行比较，评估了该基准测试的可靠性。

Conclusion: LLMLagBench提供了一种系统性的方法来识别LLM的知识时间边界，有助于理解模型的知识局限性并提高回答准确性。
...
TL;DR: 提出了U-MStance数据集和PRISM模型来解决多模态对话立场检测中的伪多模态和用户同质性问题，通过用户画像和多模态对齐提升立场检测性能。


<details>
  <summary>Details</summary>
Motivation: 现有研究存在伪多模态（源帖子有视觉内容但评论仅文本）和用户同质化（忽略用户个性特征）的局限性，无法反映真实的多模态交互。

Method: PRISM模型从历史帖子推导用户画像，通过思维链对齐文本和视觉线索，并采用互任务强化机制联合优化立场检测和立场感知响应生成。

Result: 在U-MStance数据集上的实验表明，PRISM相比强基线模型取得了显著性能提升。

Conclusion: 用户中心和上下文基础的多模态推理对于现实立场理解具有有效性，解决了现有方法的局限性。
...
TL;DR: 提出了AI-Salesman框架，通过双阶段架构解决目标驱动说服对话中的多轮规划和事实忠实度问题，在真实销售对话数据集上显著优于基线模型。


<details>
  <summary>Details</summary>
Motivation: 目标驱动说服对话（如电话销售）需要复杂的多轮规划和严格的事实忠实度，但现有方法面临任务特定数据缺乏、策略脆弱性和事实幻觉等挑战。

Method: 构建TeleSalesCorpus数据集，提出双阶段AI-Salesman框架：训练阶段使用贝叶斯监督强化学习从噪声对话中学习鲁棒销售策略；推理阶段引入动态大纲引导代理(DOGA)，利用预构建脚本库提供动态策略指导。

Result: 实验结果表明，AI-Salesman在自动指标和综合人工评估中均显著优于基线模型，在复杂说服场景中表现出色。

Conclusion: AI-Salesman框架通过双阶段架构和动态大纲引导，有效解决了目标驱动说服对话中的关键挑战，为类似应用提供了可行解决方案。
...
TL;DR: VBackChecker是一个无需参考的幻觉检测框架，通过像素级Grounding LLM验证MLLM生成响应与视觉输入的一致性，在R^2-HalBench基准上实现SOTA性能。
...
Method: 基于"眼见为实"原则，使用具有推理和分割能力的像素级Grounding LLM构建参考无关框架，并设计了R-Instruct指令调优数据生成流程。

Result: 在R^2-HalBench上超越现有复杂框架，性能媲美GPT-4o，像素级定位任务提升超10%。

Conclusion: VBackChecker为MLLM幻觉检测提供了有效且可解释的解决方案，在真实场景中表现优异。
...
Conclusion: CriticSearch通过细粒度信用分配机制有效提升了搜索代理的训练效率和性能，为复杂问答任务提供了更可靠的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 当前大语言模型在面向任务的对话中，在领域适应和检索可控性方面仍面临挑战，需要更精确和领域自适应的实体识别方法。

Method: 使用轻量级管理器进行类型级判断，专业专家进行跨度级提取，每个专家配备KeyInfo检索器，在推理时注入语义对齐的少样本示例。

Result: 在CrossNER、MIT-Movie、MIT-Restaurant和新构建的多领域客服数据集上的实验表明，MME-RAG在大多数领域表现优于现有基线方法。

Conclusion: 层次分解和KeyInfo引导的检索是鲁棒性和跨领域泛化的关键驱动因素，MME-RAG为自适应对话理解提供了一个可扩展和可解释的解决方案。
...
TL;DR: CONFACTCHECK是一种高效的幻觉检测方法，通过检查生成文本中事实探针回答的一致性来检测LLM幻觉，无需外部知识库且资源消耗低。


<details>
  <summary>Details</summary>
Motivation: LLM在生成文本时经常产生事实错误的幻觉，这在医疗、金融等领域存在严重风险。现有检测方法需要多次API调用，增加了延迟和成本。

Method: 基于单LLM内部和不同LLM之间对生成文本中事实探针回答一致性的直觉，无需外部知识库，通过一致性检查来检测幻觉。

Result: 在多个数据集上的实验表明，CONFACTCHECK能以更少资源高效检测幻觉事实，相比类似条件下的基线方法获得更高准确率。

Conclusion: CONFACTCHECK提供了一种资源高效、无需外部知识的LLM幻觉检测解决方案，在受限访问环境下表现优异。
...
TL;DR: 提出了ViConBERT框架，通过对比学习和基于释义的蒸馏来学习越南语上下文嵌入，并创建了ViConWSD数据集用于评估越南语语义理解。


<details>
  <summary>Details</summary>
Motivation: 现有上下文词嵌入模型的进展主要集中在英语等高资源语言，越南语缺乏用于细粒度语义理解的鲁棒模型和评估资源。

Method: 结合对比学习(SimCLR)和基于释义的蒸馏来学习越南语上下文嵌入，并构建了大规模合成数据集ViConWSD用于评估。

Result: ViConBERT在WSD任务上F1得分为0.87，在ViCon数据集上AP为0.88，在ViSim-400数据集上Spearman相关系数为0.60，均优于基线模型。
...
TL;DR: 提出了一种使用小型LLM压缩大型LLM输入的新范式，开发了Cmprsr模型，在保持语义信息和控制压缩率方面表现优异，适用于不同长度和领域的输入。
...
Method: 使用小型LLM压缩大型LLM输入，通过Textgrad优化压缩元提示，对Qwen3-4B进行监督微调和GRPO训练，追求压缩率遵循和下游任务性能最大化。

Result: Cmprsr在MeetingBank、LongBench和GSM8k等数据集上优于提取式和普通抽象压缩方法，能够精确控制压缩率，在成本-质量权衡方面提供精细控制。

Conclusion: Cmprsr模型在压缩能力、压缩率控制和跨领域泛化性方面表现优异，为降低LLM使用成本提供了有效解决方案。
...
TL;DR: 本文提出了一种利用现有抽象摘要生成相应提取式摘要的轻量级方法，旨在为法律案例摘要研究社区创建增强的数据资源。


<details>
  <summary>Details</summary>
Motivation: 法律判决摘要对法律从业者构成沉重认知负担，而深度神经方法生成的抽象摘要容易误判法律术语或忽略关键细节，因此需要转向提取式摘要方法。

Method: 设计了一个轻量透明的流程，利用现有的抽象标准摘要来创建相应的提取式标准摘要，确保专家意见从原始抽象摘要传递到转换后的提取式摘要中。

Result: 扩充了七个现有案例摘要数据集，包含抽象摘要和相应的提取式摘要，并通过结构、词汇和语义维度的广泛比较评估确保增强摘要的质量。

Conclusion: 承诺公开发布增强的数据集，相信这一资源将为法律文档自动摘要领域的发展提供机会。
...
TL;DR: 研究比较了LLMs和人类在抢答式问答中的表现差异，发现LLMs在维基百科未覆盖的问题和需要数值答案的问题上表现不如人类。


<details>
  <summary>Details</summary>
Motivation: 尽管LLMs在许多NLP任务上超越了人类表现，但尚不清楚对人类困难的问题是否对LLMs同样困难。本研究旨在探索在抢答式问答中，LLMs和人类的答题难度差异。

Method: 收集包含问题、答案和人类正确率的日语问答数据，在多种设置下让LLMs回答这些问题，并从两个分析角度比较LLMs与人类的正确率。

Result: 实验结果显示，相比人类，LLMs在正确答案未被维基百科条目覆盖的问题上表现更差，并且在需要数值答案的问题上也有困难。

Conclusion: LLMs在特定类型的问题上（如维基百科未覆盖的内容和数值问题）表现不如人类，表明两者在知识获取和推理能力上存在差异。
...
TL;DR: 该论文研究了大型语言模型中的讽刺反弹现象——否定指令（如'不要提及X'）反而会增加X在模型中的可访问性。通过两个实验发现，反弹在否定后立即出现，并随着更长或语义干扰而加剧，而重复则有助于抑制。更强的极性分离与更持久的反弹相关。


<details>
  <summary>Details</summary>
Motivation: 研究否定指令在大型语言模型中引发的讽刺反弹现象，这种现象在人类认知中被称为"讽刺反弹"，即试图抑制某个概念反而会增强其可访问性。

Method: 进行两个实验：(1) 负载与内容实验：在否定指令后变化干扰文本（语义、句法、重复）并测量反弹强度；(2) 极性分离实验：测试模型是否区分同一概念的中性和负面框架，以及这种分离是否预测反弹持续性。还进行了电路追踪分析。

Result: 结果显示反弹在否定后立即出现，并随着更长或语义干扰而加剧，而重复则支持抑制。更强的极性分离与更持久的反弹相关。电路追踪分析识别出稀疏中间层注意力头放大被禁止的标记，而早期层则进行抑制。

Conclusion: 这些发现将认知预测的讽刺反弹与长上下文干扰的机制洞察联系起来，并发布了ReboundBench数据集以支持未来研究。
...
TL;DR: ILAKKANAM是首个泰米尔语语言学评估基准，包含820个来自斯里兰卡学校考试的问题，评估显示LLMs在低年级表现良好但随着语言复杂度增加而下降，性能可能源于数据暴露而非真正理解。


<details>
  <summary>Details</summary>
Motivation: 现有多语言基准多依赖英语翻译数据集，无法捕捉泰米尔语等低资源、形态丰富语言的语言和文化细微差别，需要专门评估LLMs在这些语言中的语言能力。

Method: 创建ILAKKANAM基准，使用820个斯里兰卡学校泰米尔语考试问题，由训练有素的语言学家在五个语言学类别和一个事实知识类别下进行标注，涵盖1-13年级，使用标准化评估框架评估闭源和开源LLMs。

Result: Gemini 2.5表现最佳，开源模型落后；所有模型在低年级问题表现良好但随着语言复杂度增加明显下降；模型整体表现与识别语言类别能力无强相关性。

Conclusion: LLMs在泰米尔语中的表现可能更多源于数据暴露而非真正的语言理解，需要更多针对低资源语言的专门评估和改进。
...
Method: 构建MRMBench基准（包含6个探测任务），提出推理时探测方法分析奖励预测时使用的维度，增强可解释性。
...
TL;DR: 提出SerenQA框架，用于评估LLM在科学知识图谱问答中发现意外见解的能力，重点关注药物重定位领域。
...
Conclusion: LLM在发现意外见解方面仍有显著改进空间，发布了相关资源和扩展版本。
...
TL;DR: SGuard-v1是一个轻量级的大语言模型安全护栏系统，包含ContentFilter和JailbreakFilter两个专门模型，用于检测有害内容和筛查对抗性提示，基于2B参数的Granite-3.3-2B-Instruct模型构建，支持12种语言。
...
Method: 使用约140万个训练实例对基础模型进行指令调优，ContentFilter基于MLCommons危害分类学识别安全风险，JailbreakFilter通过精心设计的课程学习覆盖60种主要攻击类型。
...
<details>
  <summary>Details</summary>
Motivation: 现有的基于QA的语义方法主要关注谓词-论元关系，但忽略了名词中心的语义关系，需要补充名词语义表示能力。

Method: 定义9个问题模板覆盖名词的显式句法和隐式上下文角色，构建QA对数据集，训练模型并与QA-SRL集成。

Result: QA-Noun几乎完全覆盖AMR的名词论元，同时揭示更多上下文隐含关系，与QA-SRL结合比FactScore和DecompScore等方法的粒度高出130%以上。
...
TL;DR: TAdaRAG是一个新颖的检索增强生成框架，通过任务自适应知识图谱构建解决传统RAG中信息丢失和无关细节的问题，在多个基准测试中表现优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 传统RAG将外部知识截断为小块导致信息丢失，引发响应幻觉和推理链断裂，且检索非结构化知识引入无关细节阻碍准确推理。

Method: 设计意图驱动路由机制到领域特定提取模板，结合监督微调和基于强化学习的隐式提取机制，确保简洁、连贯、非冗余的知识集成。

Result: 在六个公共基准和一个真实业务基准（NowNewsQA）上，使用三个骨干模型的评估显示TAdaRAG在不同领域和长文本任务中优于现有方法。

Conclusion: TAdaRAG展示了强大的泛化能力和实际有效性，能够解决传统RAG的信息丢失和无关细节问题。
...
TL;DR: 提出一种因果框架来分析和缓解RLHF奖励模型中的长度偏见，通过反事实数据增强方法训练奖励模型，使其能够独立于冗长度评估内容质量。


<details>
  <summary>Details</summary>
Motivation: RLHF训练的奖励模型存在长度偏见，系统性地倾向于偏好更长回答，将冗长度与质量混为一谈。

Method: 使用反事实数据增强方法生成响应对：长度差异对（相似内容不同长度）和内容差异对（相似长度不同内容），用这些数据训练奖励模型。

Result: 实证评估显示该方法减少了奖励分配中的长度偏见，使策略模型产生更简洁、内容集中的输出。

Conclusion: 所提方法有效减少了长度偏见，提高了RLHF流程中奖励建模的鲁棒性和内容敏感性。
...
TL;DR: MMWOZ是一个新的多模态对话数据集，扩展自MultiWOZ 2.3，包含GUI前端界面和对应的操作指令，用于弥合传统任务导向对话系统在实际应用中的差距。
...
Method: 开发网页风格GUI作为前端，设计自动化脚本将对话状态和系统动作转换为GUI操作指令，收集网页快照和对应操作指令，并提出了MATE多模态模型作为基线。

Result: 创建了MMWOZ数据集，包含GUI界面、操作指令和网页快照，为构建实用的多模态任务导向对话代理提供了基础。

Conclusion: MMWOZ数据集和MATE模型为研究实用的多模态任务导向对话系统提供了重要基础，有助于弥合理论与实际应用之间的差距。
...
TL;DR: GAPO是一种基于GRPO的扩展方法，通过计算群体层面的奖励来提升LLM生成结果的多样性，避免模式崩溃问题。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型经常出现模式崩溃问题，即使存在多个有效答案，模型也会重复生成相同的几个补全结果，这限制了模型在各种任务中的多样性。

Method: 提出Group-Aware Policy Optimization (GAPO)，作为GRPO的简单扩展，计算群体层面的奖励函数，特别使用频率感知的奖励函数来鼓励对有效LLM补全的均匀采样。

Result: GAPO训练的模型能够产生有效且更多样化的模型响应，在开放提示场景下也能提高响应多样性，同时在标准LLM基准测试（GSM8K、MATH、HumanEval、MMLU-Pro）上不损害准确性。

Conclusion: GAPO能够有效提升LLM生成结果的多样性，同时保持准确性，为解决模式崩溃问题提供了有效方案。
...
<details>
  <summary>Details</summary>
Motivation: 推动Lychee Uni-MoE系列在语言中心多模态理解、推理和生成方面的能力，构建一个能够处理10种跨模态输入的全模态模型。

Method: 采用动态容量MoE框架（共享、路由和空专家）、Omni-Modality 3D RoPE确保跨模态对齐、渐进式监督微调策略、迭代GSPO-DPO方法稳定RL训练、使用约75B tokens的开源多模态数据训练。

Result: 在85个基准测试中达到SOTA或极具竞争力的性能，在76个基准中超过50个优于Qwen2.5-Omni（使用1.2T tokens训练），视频理解平均提升7%，全模态理解平均提升7%，视听推理提升4%，长语音处理WER降低4.2%，在低级图像处理和可控生成方面领先。

Conclusion: Uni-MoE 2.0展示了在计算效率和能力之间的良好平衡，通过创新的MoE架构和训练策略，在多个多模态任务上实现了卓越性能，为全模态大模型的发展提供了重要贡献。
...
TL;DR: 提出了NOTAM语义解析任务，构建了Knots数据集（12,347条专家标注的NOTAM），通过多智能体协作框架增强领域知识发现，显著提升了航空文本理解能力。


<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注NOTAM的表面任务（如分类和命名实体识别），缺乏深度语义理解，无法处理复杂的语言结构和隐含推理。

Method: 提出NOTAM语义解析任务，构建Knots数据集，采用多智能体协作框架进行领域发现，系统评估提示工程策略和模型适应技术。

Result: 实验结果表明该方法在航空文本理解和处理方面取得了显著改进，为自动化NOTAM分析系统提供了有价值的见解。

Conclusion: NOTAM语义解析任务有效解决了航空安全信息处理的深度语义理解挑战，提出的方法和数据集为相关研究提供了重要基础。
...
TL;DR: 论文提出了Reason-KE++框架，通过SFT+RL方法解决LLM在多跳推理任务中的忠实性问题，强调过程级对齐而非单纯结果优化。


<details>
  <summary>Details</summary>
Motivation: 现有SFT方法在复杂多跳推理任务中存在"忠实性差距"，LLM倾向于模仿格式而非进行合理推理，导致参数化先验知识覆盖上下文事实，产生事实幻觉。

Method: 提出Reason-KE++框架，采用SFT+RL方法，核心是阶段感知奖励机制，为中间推理步骤提供密集监督，包括分解和子答案正确性评估。

Result: 在MQUAKE-CF-3k数据集上达到95.48%的新SOTA，相比之前方法提升5.28%，有效解决了推理完整性问题。

Conclusion: 对于复杂任务，对齐推理过程对于构建可信赖的LLM至关重要，单纯的结果优化RL会损害推理完整性。
...
TL;DR: 本文提出了一种用于波斯语到英语直接语音翻译的系统，通过自监督预训练、离散语音单元和合成并行数据来解决低资源语言的数据稀缺问题。


<details>
  <summary>Details</summary>
Motivation: 直接语音翻译系统需要大量的并行语音数据，但对于波斯语等低资源语言来说，这种数据很少可用。

Method: 系统包含三个组件：基于conformer的编码器、因果transformer解码器、基于单元的神经声码器。通过使用大语言模型翻译波斯语转录并合成英语语音来构建合成并行语料库。

Result: 在CVSS语料库的波斯语-英语部分，使用合成数据比直接基线提高了4.6 ASR BLEU。

Conclusion: 结合自监督预训练、离散语音单元和合成并行数据对于改善波斯语-英语等低资源语言对的直接语音翻译是有效的。
...
TL;DR: EvoSynth是一个自主进化合成越狱方法的框架，通过多智能体系统自主设计、进化和执行基于代码的新型攻击算法，实现了85.5%的攻击成功率，超越了现有方法的创造力限制。


<details>
  <summary>Details</summary>
Motivation: 现有自动红队框架的越狱逻辑局限于选择、组合或改进现有攻击策略，无法自主发明全新的攻击机制，限制了其创造力。

Method: 采用多智能体系统自主设计基于代码的攻击算法，包含代码级自校正循环，能够根据失败情况迭代重写自身攻击逻辑。

Result: 在高度鲁棒的Claude-Sonnet-4.5模型上实现了85.5%的攻击成功率，生成的攻击比现有方法更加多样化。

Conclusion: EvoSynth通过进化合成越狱方法的新范式，为红队测试开辟了新方向，显著提升了攻击成功率和多样性。
...
TL;DR: AFM是一种动态上下文管理器，通过语义相似度、半衰期权重和重要性分类为历史消息分配三种保真度级别，在严格token预算下显著减少推理成本，同时保持对话安全性。


<details>
  <summary>Details</summary>
Motivation: 解决LLMs在多轮对话中固定上下文窗口和简单内存策略的瓶颈问题，避免全对话重放的高成本或静态摘要导致的安全关键信息丢失。

Method: AFM根据当前查询的语义相似度、半衰期权重和重要性分类，为历史消息分配FULL、COMPRESSED或PLACEHOLDER三种保真度级别，在token预算下按时间顺序打包消息。

Result: 在涉及花生过敏用户规划泰国旅行的安全基准测试中，AFM在短中长度对话中都能保留过敏信息，安全性能与全重放相当，平均token使用量比基线减少66%。
...
TL;DR: LLMs在复杂推理任务中表现出色，但在简单的集合成员查询任务中却表现脆弱且不可预测，揭示了其推理能力的局限性。


<details>
  <summary>Details</summary>
Motivation: 研究LLMs在复杂任务中表现优异却在简单任务中失败的悖论，通过简单但基础的集合成员查询任务来暴露基本失败模式。

Method: 使用集合成员查询任务，系统评估提示措辞、语义结构、元素排序和模型选择等维度，进行大规模受控实验。

Result: LLMs在这一基础任务上的表现持续脆弱且在所有维度上都不可预测，表明模型对集合概念的理解是碎片化和复杂的。

Conclusion: 通过简单问题的大规模实验能够全面映射和分析失败模式，这种方法对LLM评估具有重要价值。
...
<details>
  <summary>Details</summary>
Motivation: 探索相变是否仅限于大型语言模型，能否在线性训练空间中直接检测，以及是否在训练早期就出现。

Method: 训练小型GPT风格transformer模型，通过词汇使用分析、平均词长、正确/错误词数、词汇多样性变化等指标，结合泊松和亚泊松统计量化词汇连接和重组。

Result: 在训练过程中发现了明显的相变点，这些转变在标准损失或验证曲线中不明显，但通过词汇和统计探针变得可见。
...
TL;DR: 提出通过中断机制来增强大语言模型的对齐性，通过在用户输入中定期添加控制语句来防止越狱行为。
...
Method: 提出中断机制——在用户输入中每隔x个token添加控制语句，这种方法可推广到思维链过程以防止策略性行为。

Result: 论文提出了中断作为解决对齐扩展性问题的可能方案，但未提供具体实验结果。

Conclusion: 中断机制是增强LLM对齐性的有前景方法，能够随着输入长度扩展对齐能力，防止模型越狱和策略行为。
...
TL;DR: 评估大型语言模型在自动形式化任务中对语义相似但表述不同的自然语言输入的鲁棒性，发现模型输出对自然语言表述的微小变化很敏感。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在自动形式化方面表现出色，但可能难以生成可靠且可验证的形式化结果。最近研究表明LLMs对自然语言输入的改写很敏感，本文旨在验证这一现象在自动形式化领域是否同样存在。

Method: 使用MiniF2F和ProofNet的Lean 4版本作为形式化基准，生成语义相似但表述不同的自然语言陈述，在两个现代LLMs上进行交叉评估，测量语义有效性和编译有效性。

Result: 研究结果显示，模型在不同改写的输入上表现存在差异，自然语言陈述的微小变化会显著影响模型输出。

Conclusion: LLMs在自动形式化任务中对自然语言输入的细微变化表现出敏感性，这影响了其生成可靠形式化结果的能力。
...
TL;DR: BioMedJImpact是一个大规模生物医学期刊影响数据集，整合了文献计量指标、合作特征和LLM衍生的AI参与度指标，用于分析合作强度和AI参与度如何共同影响科学影响力。


<details>
  <summary>Details</summary>
Motivation: 现有开放资源很少捕捉合作结构和AI研究如何共同塑造生物医学期刊声望，需要开发能够分析科学影响力和AI参与度的综合数据集。

Method: 从PubMed Central的174万篇文章构建数据集，提出可复现的三阶段LLM流程提取AI参与度特征，分析合作强度和AI参与度在疫情前后的影响。

Result: 发现两个一致趋势：合作强度更高的期刊（特别是作者团队更大更多样化）获得更高引用影响力；AI参与度与期刊声望相关性日益增强，尤其在四分位排名中。
...
<details>
  <summary>Details</summary>
Motivation: 虽然大语言模型对话流畅度不断提升，但赋予其细腻、人性化的情感表达仍具挑战。现有对齐技术往往处理表层输出或需要大量微调。

Method: 首先使用归因修补识别因果影响组件，通过观察诊断对话任务中的激活模式找到关键干预位点；然后从对比文本对（目标情感的正负示例）的激活差异中推导情感表达向量。

Result: 将这些向量应用于新对话提示显著增强了情感特征：引导后的响应显示积极情感（如喜悦、信任）增加，第一人称代词使用更频繁，表明更强的个人参与度。
...
TL;DR: 提出了一种新的稳定性度量方法，利用LDA的生成特性创建带真实主题的语料库，通过多次运行LDA来评估主题模型的一致性和准确性。


<details>
  <summary>Details</summary>
Motivation: 概率主题模型由于随机性在重新运行时会产生不一致的结果，影响可复现性、可靠性和解释性，需要评估主题模型是否真正捕捉到有意义的主题还是仅仅是噪声。

Method: 定义新的稳定性度量方法，结合准确性和一致性，利用LDA的生成特性创建带真实主题的新语料库，并对这些语料库运行LDA 50次来评估输出变异性。

Result: LDA能够正确确定文档中的基础主题数量，多次重新运行返回相似主题，具有较高的内部一致性，但这些主题并非真实主题。

Conclusion: LDA在内部一致性方面表现良好，但生成的主题与真实主题存在差异，表明需要更严格的稳定性评估方法来确保主题模型捕捉到有意义的语义结构。
...
TL;DR: NeuroLex是一个专门针对脑电图报告的轻量级领域自适应语言模型，通过EEG报告文本训练，在EEG报告处理任务上优于通用语言模型。


<details>
  <summary>Details</summary>
Motivation: 通用语言模型无法捕捉脑电图报告中的领域特定语言惯例，需要专门针对EEG报告语言特点的模型。

Method: 使用跨度损坏预训练和指令式微调，在报告润色、段落摘要和术语问答任务上训练，学习EEG解释的语法和推理模式。

Result: 相比同规模通用模型，NeuroLex实现了更低的困惑度、更高的提取和摘要准确率、更好的标签效率，以及对否定和事实幻觉的更强鲁棒性。
...
<details>
  <summary>Details</summary>
Motivation: 随着多模态大语言模型在感知任务上的成功，提升其复杂推理能力成为关键研究方向。现有模型存在推理路径不透明和泛化能力不足的问题，而思维链推理在语言模型中已证明能增强推理透明度和输出可解释性，有望扩展到多模态领域。
...
Result: BERT在二元分类中达到84.49%准确率，多类别分类72.03%准确率，且计算效率最高（443秒训练时间）。GPT-2表现最差，DeBERTa计算成本最高。GPT-2在讽刺检测方面表现突出（92.46%召回率）。
...
TL;DR: 通过系统算法审计发现，Google搜索的AI概览和精选摘要功能在婴儿护理和孕期相关查询中存在信息不一致、医疗安全保障缺失等问题，需要加强AI系统在健康信息领域的质量控制。


<details>
  <summary>Details</summary>
Motivation: 评估Google搜索中AI生成内容（AI概览和精选摘要）的质量和一致性，特别是在婴儿护理和孕期相关的高风险健康信息领域，因为这些功能用户依赖度高但缺乏控制权。

Method: 对1,508个真实的婴儿护理和孕期相关查询进行系统算法审计，使用稳健的评估框架评估答案一致性、相关性、医疗安全保障、来源类别和情感对齐等多个质量维度。

Result: AI概览和精选摘要在同一搜索结果页面中的信息不一致率达33%；医疗安全保障严重缺失（AI概览仅11%，精选摘要仅7%）；健康网站是主要来源，但精选摘要也常链接商业来源。

Conclusion: AI介导的健康信息存在重大质量隐患，需要加强质量控制；该审计方法为高风险领域AI系统评估提供了可转移的框架。
...
TL;DR: 本文提出了Visual Room 2.0基准，用于评估多模态大语言模型的感知-认知对齐，发现MLLMs的感知能力优于认知能力，且认知不依赖于感知推理。


<details>
  <summary>Details</summary>
Motivation: 扩展Searle的中文房间思想实验到多模态领域，质疑MLLMs是否能真正理解所见内容，即"看见不等于理解"。

Method: 构建包含3个层次（低、中、高）和17个任务的层次化基准，涵盖从属性识别到场景理解的感知任务，以及从文本蕴含到因果和社会推理的认知任务，共350个多模态样本和2100个渐进问题。

Result: 评估10个SOTA MLLMs发现：(1)感知能力比认知能力强8.0%；(2)认知不因果依赖于基于感知的推理；(3)认知随模型规模扩展，但感知能力不随更大变体一致提升。
...
TL;DR: HCNR方法通过识别并恢复关键表达神经元来修复SFT后LLM的诚实性，相比基线方法速度提升2.23倍且数据需求减少10倍以上


<details>
  <summary>Details</summary>
Motivation: 监督微调严重损害了LLM的诚实性，这对高风险领域的安全部署至关重要。现有方法假设SFT深度破坏了模型识别知识边界的能力，但实际观察到模型仍保留此能力，只是表达这种意识的能力受损

Method: 提出Honesty-Critical Neurons Restoration (HCNR)，通过识别和恢复关键表达控制神经元到预训练状态，同时通过Hessian引导的补偿机制与任务导向神经元协调

Result: 在四个QA任务和五个LLM家族上的实验表明，HCNR有效恢复了33.25%的受损诚实性，相比基线方法速度提升至少2.23倍且数据需求减少10倍以上

Conclusion: HCNR为可信LLM部署提供了实用解决方案，通过外科手术式修复被压制的表达能力来恢复模型诚实性
...
TL;DR: AA-Omniscience基准测试评估语言模型的事实回忆能力和知识校准能力，结果显示前沿模型在事实性和校准方面存在持续弱点，Claude 4.1 Opus表现最佳但分数仍较低。
...
Method: 构建包含6000个问题的基准测试，问题来自权威学术和行业来源，涵盖6个领域的42个经济相关主题，测量全知指数。

Result: Claude 4.1 Opus获得最高分4.8，是仅有的三个得分超过零的模型之一。不同模型在不同领域表现各异。

Conclusion: 模型在知识重要任务中应根据用例需求而非通用性能来选择，因为模型表现存在领域差异性。
...
TL;DR: 本文评估了多语言嵌入模型与对齐单语模型在双语词典归纳任务中的表现，分析了BLI作为嵌入空间对齐度量的局限性，并提出了基于词干的新BLI方法和词汇修剪技术。


<details>
  <summary>Details</summary>
Motivation: 尽管多语言嵌入已成为主流选择，但需要评估其在各方面是否真的优于对齐单语模型，以及其更高的计算成本是否总是合理。

Method: 使用双语词典归纳任务评估不同嵌入对齐技术，包括传统对齐方法、新型多语言模型和组合技术，并考虑语言家族的影响。提出了基于词干的BLI方法和词汇修剪技术。

Result: 在某些情况下BLI不能真实反映对齐程度，组合嵌入对齐技术通常表现更好，但在低资源语言情况下多语言嵌入表现更优。

Conclusion: 多语言嵌入和对齐单语模型各有优势，需要根据具体场景选择。提出的新BLI方法和词汇修剪技术能更准确地评估嵌入空间对齐程度。
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型在自动定理证明方面潜力巨大，但受到高质量形式语言数据稀缺的限制，需要开发更有效的训练方法来提升轻量级模型的推理能力。

Method: 采用三阶段训练框架：1）在广泛数学语料上进行持续预训练，引入"思维链增强状态预测"任务；2）在专家迭代循环中进行监督微调；3）使用GRPO优化器针对最具挑战性问题进行强化训练。

Result: Spark-Prover-X1-7B在同类开源模型中达到最先进性能，平均通过率37.0%（pass@32），在PutnamBench上解决27个问题，在CombiBench上达到24.0%。

Conclusion: 多样化的训练数据和渐进细化的训练流程为增强轻量级LLM的形式推理能力提供了有效路径，验证了该方法的有效性。
...
TL;DR: BeDiscovER是一个评估现代LLM语篇理解能力的综合基准套件，包含5个语篇任务、52个数据集，涵盖词汇、句子和文档层面的语篇分析。评估发现前沿模型在时间推理的算术方面表现良好，但在完整文档推理和某些语义语篇现象上仍有困难。


<details>
  <summary>Details</summary>
Motivation: 随着推理语言模型的发展，需要建立更新的、全面的基准来评估模型在语篇层面的知识理解能力，特别是涵盖从词汇到文档的多层次语篇分析任务。

Method: 整合了5个公开可用的语篇任务（包括语篇词汇、多句子和文档层面），共52个数据集，涵盖语篇解析、时间关系提取、语篇粒子消歧等任务，并评估了开源和前沿LLM在这些任务上的表现。

Result: 最先进的模型在时间推理的算术方面表现出色，但在完整文档推理和某些微妙的语义语篇现象（如修辞关系识别）方面存在困难。

Conclusion: BeDiscovER为评估现代LLM的语篇理解能力提供了全面基准，揭示了当前模型在语篇推理方面的优势与局限，为未来研究指明了方向。
...
TL;DR: LLMs在零样本设置下评估RCTs对CONSORT 2010声明的依从性，整体性能一般，能准确识别合规项目但难以检测不合规项目，目前仅适合作为初步筛查助手。


<details>
  <summary>Details</summary>
Motivation: 手动验证CONSORT依从性耗时费力，成为同行评审和证据合成的瓶颈，需要评估LLMs在此任务中的准确性和可靠性。

Method: 构建包含150篇已发表RCTs的黄金标准数据集，在零样本设置下评估当代LLMs的性能，主要指标为宏平均F1分数，辅以项目级性能指标和定性错误分析。

Result: 表现最佳的Gemini-2.5-Flash和DeepSeek-R1模型宏F1分数为0.634，Cohen's Kappa系数约0.28，仅达到一般一致性。模型能准确识别合规项目(F1>0.850)，但难以检测不合规和不适用项目(F1<0.400)。

Conclusion: LLMs有潜力作为CONSORT检查的初步筛查助手，能识别报告良好的项目，但目前无法可靠检测报告遗漏或方法学缺陷，不能替代人类专家进行试验质量的关键评估。
...
TL;DR: 提出了Agent-Event-Coder (AEC)多智能体框架，将事件抽取视为代码生成过程，通过检索、规划、编码和验证四个专业智能体的协作，在零样本设置下实现精确、完整且符合模式的事件抽取。


<details>
  <summary>Details</summary>
Motivation: 解决大语言模型在零样本事件抽取中面临的挑战，如复杂推理需求、领域特定理解困难，以及直接提示导致的不完整输出、结构无效等问题。

Method: 将事件抽取分解为四个子任务：检索、规划、编码和验证，每个任务由专门的LLM智能体处理。事件模式表示为可执行的类定义，通过验证智能体进行确定性验证和精确反馈。

Result: 在五个不同领域和六个LLM上的实验表明，AEC始终优于先前的零样本基线方法，证明了将事件抽取视为代码生成方法的有效性。

Conclusion: 通过编程启发的方法和协作智能体工作流，AEC能够在大语言模型中实现精确、完整且模式一致的事件抽取，展示了将事件抽取视为代码生成过程的强大能力。
...
Method: 在Azerbaijani Sign Language Dataset (AzSLD)和Word-Level American Sign Language (WLASL)数据集上实现并评估ConvLSTM和Vanilla Transformer两种代表性模型。

Result: Vanilla Transformer在两个数据集上的Top-1准确率均优于ConvLSTM（AzSLD: 76.8% vs 更低，WLASL: 88.3% vs 更低），但ConvLSTM计算效率更高。
...
TL;DR: 提出了一个零样本语法能力评估框架，利用未标注数据和大型语言模型生成伪标签，无需人工标注即可训练基于transformer的语法评分模型。


<details>
  <summary>Details</summary>
Motivation: 口语语法能力评估面临自发、非结构化和不流畅的挑战，且需要大量专家标注，导致大规模数据创建不切实际。

Method: 使用基于语法能力量表的提示让LLM在未标注数据上生成预测作为伪标签，通过新颖的训练框架训练transformer模型以有效处理标签噪声。

Result: 实验结果表明该方法能高精度估计语法能力分数，LLM选择和训练中干净与噪声样本比例对性能有重要影响。
...
<details>
  <summary>Details</summary>
Motivation: 解决孟加拉语等低资源语言ASR转录中的关键歧义问题：区分无意的重复不流利和故意的语法构造重叠，避免标准不流利校正错误删除有效语言信息。

Method: 构建了20,000行手动标注的孟加拉语语料库，使用两种方法进行基准测试：多语言大语言模型的少样本提示和编码器模型的任务特定微调。

Result: LLM在少样本提示下达到82.68%准确率，但微调方法更优，BanglaBERT模型达到84.78%准确率和0.677 F1分数。

Conclusion: 建立了强大的语言学基准，为开发语义保留的孟加拉语文本规范化系统提供了必要数据。
...
<details>
  <summary>Details</summary>
Motivation: 现有的大语言模型在通用领域表现出色，但在高度专业化和文化丰富的中医领域需要更严格和细致的评估。基于先前TCM-3CEval的工作，需要更细粒度的评估工具来揭示模型在中医领域的真实能力。
...
Result: 评估显示模型性能存在显著差异，deepseek_r1和gemini_2_5_pro表现最佳。模型在基础知识回忆方面表现良好，但在经典文本解释方面存在困难。排列一致性测试揭示了模型推理的脆弱性，所有模型在面对不同选项顺序时都表现出性能下降。

Conclusion: TCM-5CEval为中医领域的LLM能力提供了更详细的诊断工具，揭示了模型在推理稳定性方面的基本弱点。该基准已上传至Medbench平台，以促进进一步研究和标准化比较。
...
TL;DR: 本文提出了一种量化评估翻译器性能的方法——翻译熵，通过分析单词语义替换对翻译结果的影响来测量翻译器的信息保留能力。


<details>
  <summary>Details</summary>
Motivation: 当前缺乏客观的定量方法来评估翻译器性能，因为单语言的熵值本身未知，需要开发新的评估指标。

Method: 通过选择基准句中的特定词元，替换为其他词元但保持翻译结果不变，统计这种替换的概率分布来计算翻译熵。

Result: 该方法能够量化排名不同翻译器，发现翻译熵在解码器模块中增强，且双词元替换呈现乘法效应。

Conclusion: 翻译熵可作为翻译器性能的客观基准指标，为人工翻译器评估提供了可测量的属性。
...
Result: 结果显示GPT-4o等模型实现了高精度的变音符号恢复，始终超过中性回声基线，而Meta的Llama系列模型表现出更大的变异性。
...
TL;DR: 本文评估了视觉语言模型在语音识别任务中的表现，发现即使经过微调，模型也难以正确解读语音谱图和波形图，表明需要特定的参数知识而不仅仅是配对样本。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型和视觉语言模型的发展，研究者希望了解这些模型在融合视觉和语言模态任务中的能力，特别是能否像专业语音学家一样解读语音谱图和波形图。

Method: 创建了一个包含4000+英语单词的新数据集，通过多项选择任务测试模型从语音谱图和波形图中预测正确音素或字素转录的能力，使用基于音素编辑距离的干扰项。

Result: 无论是零样本还是微调后的模型，其表现都很少超过随机猜测水平，表明模型缺乏解读此类图形所需的特定参数知识。

Conclusion: 视觉语言模型需要专门的参数知识来正确解读语音谱图和波形图，仅靠配对样本训练是不够的。
...
TL;DR: SoCE是一种基于类别专家的模型融合方法，通过识别不同类别的专家模型并进行非均匀加权平均，提升大语言模型的性能和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 大语言模型训练成本高昂，模型融合（model souping）作为一种无需重新训练就能提升性能的技术很有前景。但传统的均匀平均方法未能充分利用不同模型在各类别任务上的专长。

Method: 利用基准测试组合识别最优模型候选，基于类别间低相关性的观察，为每个弱相关类别簇识别专家模型，采用优化的非均匀加权平均而非均匀权重进行融合。

Result: 该方法在多个领域（多语言能力、工具调用、数学等）提升了性能和鲁棒性，在伯克利函数调用排行榜上取得了最先进的结果。

Conclusion: SoCE提供了一种原则性的模型融合方法，通过类别专家识别和优化加权平均，能够有效提升模型性能，是传统均匀平均方法的改进。
...
TL;DR: RegionMarker是一个区域触发的语义水印框架，通过在低维空间中定义触发区域并向相关文本嵌入注入水印，为EaaS提供全面的版权保护。
...
<details>
  <summary>Details</summary>
Motivation: 阿拉伯世界酒店业越来越依赖客户反馈来改进服务，因此需要先进的阿拉伯语情感分析工具，特别是针对方言的情感分析。

Method: 创建了一个多方言数据集，包含538条情感平衡的酒店评论，从现代标准阿拉伯语翻译为沙特和摩洛哥方言，并由母语者验证翻译准确性和情感保持。

Result: 超过40个团队注册参与共享任务，12个团队提交了系统，最佳系统的F1分数达到0.81。

Conclusion: 研究表明跨阿拉伯方言的情感分析是可行的，但仍面临挑战，该资源支持开发面向实际应用的方言感知NLP系统。
...
TL;DR: 本文通过控制性实验研究了LLMs在任务和语言维度上的迁移效应，发现跨语言的任务迁移通常为正，而跨任务的迁移常导致性能下降，揭示了稳定的捐赠者-接收者结构。
...
Method: 采用PEFT/LoRA技术对多个开源LLM家族和规模进行控制性研究，将任务和语言视为迁移轴，在单一任务-语言源上微调模型，并测量在所有其他任务-语言目标对上的迁移效果。

Result: 发现两个一致模式：(1) 任务内跨语言迁移可靠为正，而跨任务迁移常导致性能下降；(2) 跨语言和任务存在稳定的捐赠者-接收者结构。

Conclusion: 研究结果为风险感知的微调和模型专业化提供了重要启示。
...
TL;DR: PEDIASBench评估框架评估12个LLM在儿科医疗中的表现，发现先进模型在基础知识上表现良好，但在复杂推理、动态诊疗和人文关怀方面存在局限，建议未来关注多模态整合和临床反馈循环。


<details>
  <summary>Details</summary>
Motivation: 随着LLM在医学领域的快速发展，需要评估它们是否能在真实临床环境中作为合格的儿科医生发挥作用。

Method: 开发PEDIASBench系统评估框架，从基础知识应用、动态诊疗能力、儿科医疗安全与伦理三个维度，评估12个代表性模型在19个儿科亚专业和211种典型疾病上的表现。

Result: 先进模型在基础知识上表现良好（Qwen3-235B-A22B在执照级问题上准确率超90%），但任务复杂度增加时性能下降约15%；在动态诊疗场景中，DeepSeek-R1在病例推理中得分最高（平均0.58），但多数模型难以适应实时患者变化；在伦理安全任务上，Qwen2.5-72B表现最佳（准确率92.05%），但人文敏感性有限。

Conclusion: 当前儿科LLM受限于有限的动态决策能力和不成熟的人文关怀，无法独立执行儿科诊疗，但在决策支持、医学教育和患者沟通方面具有潜力，为安全、可信、协作的智能儿科医疗系统奠定基础。
...
Method: 开发了多步骤LLM合成流程创建PAL-Set中文数据集，并提出H²Memory分层异构记忆框架，结合检索增强生成来改进个性化响应生成。

Result: 在PAL-Bench和外部数据集上的综合实验证明了所提记忆框架的有效性。

Conclusion: PAL-Bench为评估面向服务的个性化助手提供了新基准，H²Memory框架显著提升了长期用户-代理交互中的个性化服务能力。
...
TL;DR: 提出了一个基于对数函数的非线性翻译质量评估模型，解决了传统线性评估方法在不同文本长度上的偏差问题。


<details>
  <summary>Details</summary>
Motivation: 传统基于多维质量指标(MQM)的线性评估方法在不同长度文本样本上存在偏差，对短文本过度惩罚，对长文本惩罚不足，与专家直觉不一致。

Method: 构建了一个双参数对数模型E(x) = a * ln(1 + b * x)，基于韦伯-费希纳定律和认知负荷理论，通过一维根查找步骤从两个容忍点进行校准。

Result: 实证数据显示可接受错误数量随样本大小呈对数增长而非线性增长，该模型提高了跨长度样本评估的公平性和评分者间可靠性。

Conclusion: 该非线性评分模型提供了更符合人类感知的翻译质量评估范式，为AI驱动的文档级评估提供了更强基础，能够更准确地扩展到不同长度的文本评估。
...
TL;DR: 本文提出了一种基于方面的情感分析方法来解码泰国财务年报中的模糊情感，开发了标注指南并标注了100多份财务报告，通过事件研究验证了情感分析对股价的实际影响。


<details>
  <summary>Details</summary>
Motivation: 财务文件中的情感理解对洞察市场行为至关重要，但这些报告往往包含模糊语言，即使基础条件不利也呈现积极或中性前景，需要专门方法解码这种模糊情感。

Method: 开发了泰国财务年报中模糊情感的标注指南，标注了100多份财务报告，并对各种文本分类模型进行基准测试，最后通过事件研究评估情感分析对股价的实际影响。

Result: 在情感分类任务上表现出色，市场反应受到报告中特定方面的选择性影响，验证了情感分析在金融文本中的实际应用价值。

Conclusion: 研究强调了金融文本情感分析的复杂性，指出解决模糊语言对于准确评估市场情绪的重要性，为金融情感分析提供了有效的方法论和实践验证。
...
TL;DR: 本文提出了一个利用大语言模型自动化分析公共叙事的计算框架，在专家标注的基准测试中达到接近人类专家的性能水平。


<details>
  <summary>Details</summary>
Motivation: 公共叙事是领导力发展和公民动员的重要工具，但由于主观解释性强和专家标注成本高，其系统分析面临挑战。

Method: 开发了一个基于大语言模型的计算框架，使用与领域专家共同制定的编码手册，对公共叙事进行定性标注。

Result: 大语言模型在8个叙事和14个代码上平均F1得分达到0.80，接近人类专家水平。将该方法扩展到22个故事和一组政治演讲的分析。

Conclusion: 研究表明大语言模型辅助标注在可扩展叙事分析方面具有潜力，同时指出了计算公民叙事研究的局限性和未来方向。
...
### [377] [HAPO: Training Language Models to Reason Concisely via History-Aware Policy Optimization](https://arxiv.org/abs/2505.11225)
*Chengyu Huang,Zhengxin Zhang,Claire Cardie*

Main category: cs.CL

TL;DR: HAPO是一种历史感知策略优化方法，通过跟踪每个问题的最短正确响应历史，激励模型发现比之前更简洁的正确解决方案，在保持准确性的同时显著减少输出长度。


<details>
  <summary>Details</summary>
Motivation: 现有方法在测试时扩展响应长度虽然能提升LLM推理能力，但会导致冗长输出和推理成本增加。传统方法未能利用训练中同一问题的历史信息，限制了解决方案随时间推移变得更简洁的能力。

Method: HAPO跟踪每个问题的历史状态（如之前生成正确响应的最小长度），基于历史状态设计新颖的长度奖励函数，激励发现比之前更简洁的正确解决方案，同时避免过度惩罚较短的错误响应以促进探索。

Result: 在多个数学基准测试中，HAPO实现了33-59%的长度减少，准确率仅下降2-5%，有效诱导了LLM的简洁推理能力。

Conclusion: HAPO通过结合历史感知的长度奖励和正确性奖励，联合优化正确性和效率，能够显著减少模型输出长度同时保持高准确性。
...
### [378] [Toward Conversational Hungarian Speech Recognition: Introducing the BEA-Large and BEA-Dialogue Datasets](https://arxiv.org/abs/2511.13529)
*Máté Gedeon,Piroska Zsófia Barta,Péter Mihajlik,Tekla Etelka Gráczi,Anna Kohári,Katalin Mády*

Main category: cs.CL

TL;DR: 为解决匈牙利语在自动语音识别(ASR)中缺乏自发性和对话语料的问题，研究者构建了两个新数据集BEA-Large和BEA-Dialogue，并建立了可复现的基线模型，展示了对话ASR的挑战性。


<details>
  <summary>Details</summary>
Motivation: 高资源语言拥有大量ASR数据集，而匈牙利语等语言由于缺乏自发性和对话语料库而代表性不足，需要填补这一空白。

Method: 从匈牙利语音语料库BEA中构建两个新数据集：BEA-Large（255小时自发语音）和BEA-Dialogue（85小时自然对话），并使用公开可用的ASR模型建立可复现基线。

Result: 微调的Fast Conformer模型在自发语音上词错误率低至14.18%，在重复语音上为4.8%；说话人日志实验的错误率在13.05%-18.26%之间。

Conclusion: 对话ASR仍然具有挑战性，特别是由于不流畅、重叠和非正式语音模式。通过发布这些数据集和基线，旨在推进匈牙利语音技术，并为其他语言开发自发性和对话基准提供方法框架。
...
### [379] [DCRM: A Heuristic to Measure Response Pair Quality in Preference Optimization](https://arxiv.org/abs/2506.14157)
*Chengyu Huang,Tanya Goyal*

Main category: cs.CL

TL;DR: 本文提出DCRM指标来衡量偏好优化中响应对的质量，并基于此开发了best-of-N²配对方法来构建更有效的训练数据集。


<details>
  <summary>Details</summary>
Motivation: 现有偏好优化方法中，偏好响应与非偏好响应之间的差异可能无法反映模型需要学习的有用差异，需要量化这些差异的质量。

Method: 使用距离和奖励边际来量化响应对差异，提出DCRM指标，并开发best-of-N²配对方法选择高DCRM的响应对构建训练集。

Result: 在多种设置下，该方法构建的训练集能进一步提升模型在AlpacaEval、MT-Bench和Arena-Hard等基准上的性能。

Conclusion: DCRM指标与学习效果正相关，基于DCRM的配对方法能有效提升偏好优化性能。
...
### [380] [Beyond SELECT: A Comprehensive Taxonomy-Guided Benchmark for Real-World Text-to-SQL Translation](https://arxiv.org/abs/2511.13590)
*Hao Wang,Yuanfeng Song,Xiaoming Yin,Xing Chen*

Main category: cs.CL

TL;DR: 提出了一种新的文本到SQL分类法，并基于此创建了SQL-Synth数据集，该数据集比现有基准具有更好的多样性和覆盖范围，揭示了现有LLM在完整场景下的局限性，但微调可以显著改善性能。


<details>
  <summary>Details</summary>
Motivation: 现有文本到SQL数据集覆盖范围有限，无法捕捉真实应用的多样性，需要更全面的分类和数据集来提升模型性能。

Method: 提出了基于核心意图、语句类型、语法结构和关键动作的文本到SQL分类法，并开发了分类法指导的数据集合成管道，结合LLM生成SQL-Synth数据集。

Result: SQL-Synth数据集展现出比现有基准更大的多样性和覆盖范围，现有LLM在完整场景下表现有限，但微调后性能显著提升。

Conclusion: 提出的分类法具有重要影响，不仅能全面分析数据集和不同LLM的性能，还能指导LLM训练数据的构建。
...
### [381] [Omni Memory System for Personalized, Long Horizon, Self-Evolving Agents](https://arxiv.org/abs/2511.13593)
...
TL;DR: O-Mem是一个基于主动用户画像的新型记忆框架，通过动态提取和更新用户特征与事件记录，解决了LLM代理在复杂环境中长期交互的上下文一致性和个性化挑战。


<details>
  <summary>Details</summary>
Motivation: 现有的记忆系统依赖语义分组进行检索，容易忽略语义无关但关键的用户信息，并引入检索噪声。需要解决LLM代理在长期交互中的上下文一致性和动态个性化问题。

Method: 基于主动用户画像的记忆框架，动态提取和更新用户特征和事件记录，支持人物属性分层检索和主题相关上下文检索。

Result: 在LoCoMo基准测试中达到51.76%，比之前的SOTA LangMem提升近3%；在PERSONAMEM中达到62.99%，比A-Mem提升3.5%。同时提升了token和交互响应时间效率。
...
### [382] [Why is "Chicago" Predictive of Deceptive Reviews? Using LLMs to Discover Language Phenomena from Lexical Cues](https://arxiv.org/abs/2511.13658)
*Jiaming Qu,Mengtian Guo,Yue Wang*

Main category: cs.CL

TL;DR: 使用大型语言模型将机器学习检测虚假评论的词汇线索转化为人类可理解的语言现象，帮助人们在没有检测分类器的情况下评估在线评论的可信度。


<details>
  <summary>Details</summary>
Motivation: 虚假评论误导消费者、损害企业利益并破坏在线市场信任。虽然机器学习分类器能有效区分虚假评论，但其学习到的区分特征往往难以被人理解。

Method: 利用大型语言模型将机器学习的词汇线索转化为人类可理解的语言现象，并与先验知识和上下文学习获得的现象进行比较。

Result: 通过这种方法获得的语言现象具有数据实证基础，在相似领域具有通用性，且比LLMs先验知识或上下文学习获得的现象更具预测性。
...
### [383] [Crossing Borders: A Multimodal Challenge for Indian Poetry Translation and Image Generation](https://arxiv.org/abs/2511.13689)
...
<details>
  <summary>Details</summary>
Motivation: 印度诗歌具有丰富的文化遗产，但由于语言复杂性和文化背景，非母语读者难以理解。现有研究忽视了印度语言诗歌，需要提高其全球可访问性。

Method: 使用TAI框架，包括：(1) 基于胜率偏好对齐算法的翻译模块，准确翻译形态丰富的诗歌；(2) 基于语义图的图像生成模块，捕捉隐喻和语义关系，生成视觉表示。

Result: 综合实验评估显示TAI Diffusion在诗歌图像生成任务中优于强基线模型，并发布了包含1,570首21种低资源印度语言诗歌的MorphoVerse数据集。

Conclusion: 该工作通过解决诗歌翻译和视觉理解的空白，旨在扩大可访问性并丰富读者体验，支持文化传播和教育平等。
...
TL;DR: Lang1是一个专门针对医疗运营决策的模型系列，通过在临床电子健康记录和互联网文本上预训练，并在真实医疗评估基准上微调，在多项医疗预测任务中超越了通用模型。


<details>
  <summary>Details</summary>
Motivation: 通用基础模型在医疗知识和对话基准上表现良好，但缺乏医疗运营决策所需的专业知识，如患者流量、成本和护理质量等关键决策。

Method: 开发了Lang1模型系列（100M-7B参数），在NYU Langone Health的800亿临床token和6270亿互联网token的混合语料上预训练，并使用ReMedE基准（基于668,331份EHR记录）进行微调评估。

Result: 微调后的Lang1-1B在30天再入院预测、死亡率预测、住院时长、合并症编码和保险拒赔预测等任务中，超越了比其大70倍的通用模型和比其大671倍的零样本模型，AUROC提升3.64%-6.75%和1.66%-23.66%。

Conclusion: 医院运营的预测能力需要明确的监督微调，而领域内预训练使微调过程更高效。专门化LLM可以在专业任务中与通用模型竞争，有效的医疗系统AI需要领域内预训练、监督微调和真实世界评估的结合。
...
### [385] [GenIE - Simulator-Driven Iterative Data Exploration for Scientific Discovery](https://arxiv.org/abs/2511.12057)
...
<details>
  <summary>Details</summary>
Motivation: 当前数据库将模拟器视为外部预处理步骤，需要手动运行模拟、导出结果再加载到数据库进行分析，这种线性工作流效率低下、延迟高，阻碍了交互式探索。

Method: GenIE作为PostgreSQL的扩展，通过使数据库具备"模拟感知"能力，能够根据用户查询和分析需求动态调用模拟器，避免生成与分析无关的数据，重用先前生成的数据，支持迭代增量分析。

Result: 初步实验表明，GenIE可以将野火烟雾扩散分析和飓风灾害评估等缓慢的静态分析转变为交互式探索，通过智能管理多个集成模拟器之间的模拟精度和运行时间权衡。

Conclusion: GenIE有潜力成为下一代科学数据分析的基石，但实现其完整愿景仍面临挑战和机遇。
...
### [386] [SEE++: Evolving Snowpark Execution Environment for Modern Workloads](https://arxiv.org/abs/2511.12457)
...
TL;DR: Snowpark将其内部沙盒解决方案升级为gVisor，以支持日益多样化的AI/ML工作负载，通过优化实现安全高效的Python代码执行环境。


<details>
  <summary>Details</summary>
Motivation: 随着Snowpark采用率增长，工作负载多样性带来了更复杂的沙盒需求，需要更强大的沙盒解决方案来支持AI/ML工作负载。

Method: 将内部沙盒解决方案迁移到gVisor，并进行针对性优化，构建新的沙盒架构。

Result: 成功升级沙盒架构，解决了迁移过程中的挑战，实现了更安全、高性能的执行环境。

Conclusion: 升级后的架构展示了SEE的可扩展性和灵活性，能够支持下一代Snowpark工作负载，为数据工程和AI/ML提供更好的执行环境。
...
### [387] [Redbench: Workload Synthesis From Cloud Traces](https://arxiv.org/abs/2511.13059)
...
<details>
  <summary>Details</summary>
Motivation: 云数据仓库提供商的工作负载跟踪显示，TPC-H和TPC-DS等标准基准测试未能捕捉真实工作负载的关键特征，包括查询重复和字符串密集型查询。

Method: 开发Redbench工作负载生成器，整合多种工作负载生成技术，基于云提供商发布的跟踪数据重现真实工作负载特征，将现有基准测试转换为保留内在工作负载特征的查询流。

Result: 评估表明：(1) Redbench为云数据仓库基准测试生成更真实和可重现的工作负载；(2) 在四个商业数据仓库平台上揭示了系统优化的影响。
...
### [388] [LLM-Generated Negative News Headlines Dataset: Creation and Benchmarking Against Real Journalism](https://arxiv.org/abs/2511.11591)
...
<details>
  <summary>Details</summary>
Motivation: 解决NLP任务中数据获取困难和真实数据隐私问题，探索LLM生成数据作为替代方案的潜力。

Method: 使用定制提示创建负面新闻标题语料库，通过专家评审和嵌入空间分析验证，采用多种评估指标（困惑度、可读性、POS分析、BERTScore、语义相似性）与真实新闻标题进行对比。

Result: 生成的标题与真实标题高度匹配，仅在POS分析中的专有名词得分存在明显差异。

Conclusion: LLM生成的合成数据在负面情感分析任务中能够有效替代真实数据，具有实际应用价值。
...
### [389] [CLINB: A Climate Intelligence Benchmark for Foundational Models](https://arxiv.org/abs/2511.11597)
...
TL;DR: CLINB是一个评估大语言模型处理气候变化专业知识的基准测试，发现前沿模型在知识综合方面表现出色，但在证据基础和可验证性方面存在严重缺陷。


<details>
  <summary>Details</summary>
Motivation: 评估大语言模型处理复杂专业知识的能力，特别是在气候变化领域，这对于AI在科学工作流程中的部署至关重要。

Method: 引入CLINB基准测试，使用真实用户问题和气候科学家制定的评估标准，通过基于模型的评估过程评估多个前沿模型。

Result: 前沿模型展现出卓越的知识综合能力，甚至超过专家辅助的混合答案，但在证据基础和引用方面存在严重的幻觉问题。

Conclusion: 弥合知识综合与可验证归因之间的差距对于构建可信AI系统至关重要，需要像CLINB这样的可靠基准来推进这一目标。
...
### [390] [SynBullying: A Multi LLM Synthetic Conversational Dataset for Cyberbullying Detectio](https://arxiv.org/abs/2511.11599)
*Arefeh Kazemi,Hamza Qadeer,Joachim Wagner,Hossein Hosseini,Sri Balaaji Natarajan Kalaivendan,Brian Davis*

Main category: cs.AI

TL;DR: SynBullying是一个用于研究和检测网络欺凌的合成多LLM对话数据集，通过利用大语言模型模拟真实的欺凌互动，提供了可扩展且伦理安全的替代方案。


<details>
  <summary>Details</summary>
Motivation: 为了解决网络欺凌研究中数据收集的伦理问题和可扩展性限制，提供一种安全且可控的数据生成方法。

Method: 利用大语言模型生成模拟网络欺凌的多轮对话，包含对话结构、上下文感知标注和细粒度标签，涵盖多种欺凌类别。

Result: 数据集在对话结构、词汇模式、情感/毒性、角色动态、伤害强度和欺凌类型分布等五个维度上进行了评估，并验证了其作为独立训练数据和数据增强源的实用性。

Conclusion: SynBullying为网络欺凌研究提供了一个有价值的合成数据集，能够支持详细的语言和行为分析，并在分类任务中表现出良好的性能。
...
### [391] [CausalGuard: A Smart System for Detecting and Preventing False Information in Large Language Models](https://arxiv.org/abs/2511.11600)
*Piyushkumar Patel*

Main category: cs.AI

TL;DR: CausalGuard是一种结合因果推理和符号逻辑的新方法，能在生成过程中实时检测和防止大语言模型的幻觉问题，相比现有方法更有效且计算成本更低。
...
Method: CausalGuard通过两个互补路径工作：一条路径追踪模型所知内容与生成内容之间的因果关系，另一条路径使用自动推理检查逻辑一致性。与仅检查生成后输出的方法不同，该系统理解导致虚假陈述的因果链并在过程中早期进行干预。

Result: 在12个不同基准测试中，CausalGuard正确识别幻觉的比例达到89.3%，仅遗漏8.3%的实际幻觉。更重要的是，它将虚假声明减少了近80%，同时保持回答的自然性和帮助性。在需要多步逻辑的复杂推理任务上表现尤其出色。

Conclusion: CausalGuard通过展示其推理过程，在医疗诊断或财务分析等敏感领域特别有效，因为这些领域理解决策原因与决策本身同等重要。该方法为解决大语言模型的幻觉问题提供了一种更根本的解决方案。
...
### [392] [Quantifying Skill and Chance: A Unified Framework for the Geometry of Games](https://arxiv.org/abs/2511.11611)
*David H. Silver*
...
<details>
  <summary>Details</summary>
Motivation: 需要一种系统的方法来量化游戏中技能和运气的相对贡献，以便进行游戏设计、AI评估和风险分析等方面的应用。

Method: 将游戏建模为随机决策树，分解游戏结果为技能杠杆K和运气杠杆L，定义技能-运气指数S(G) = (K-L)/(K+L)，并引入波动性Sigma来测量结果不确定性。

Result: 应用于30个游戏的分析显示了一个从纯运气(抛硬币，S=-1)到混合领域(西洋双陆棋，S=0)再到纯技能(国际象棋，S=+1)的连续谱。扑克表现出中等技能主导(S=0.33)。
...
### [393] [Value-Aligned Prompt Moderation via Zero-Shot Agentic Rewriting for Safe Image Generation](https://arxiv.org/abs/2511.11693)
*Xin Zhao,Xiaojun Chen,Bingshan Liu,Zeyao Liu,Zhendong Zhao,Xiaoyan Gu*

Main category: cs.AI

TL;DR: VALOR是一个模块化的零样本代理框架，通过分层提示分析和人类对齐的价值推理来实现更安全、更有帮助的文生图生成。


<details>
  <summary>Details</summary>
Motivation: 生成式视觉语言模型存在产生不安全、冒犯性或文化不适当内容的风险，而现有防御方法难以在不牺牲生成质量或产生高成本的情况下使输出与人类价值观对齐。

Method: VALOR整合了多层NSFW检测器、文化价值对齐模块和意图消歧器，当检测到不安全内容时，通过大语言模型在动态角色特定指令下选择性重写提示，必要时进行风格化再生以引导输出到更安全的视觉领域。

Result: 在对抗性、模糊性和价值敏感提示上的实验显示，VALOR将不安全输出显著减少了高达100.00%，同时保持了提示的有用性和创造性。
...
### [394] [Towards autonomous quantum physics research using LLM agents with access to intelligent tools](https://arxiv.org/abs/2511.11752)
*Sören Arlt,Xuemei Gu,Mario Krenn*

Main category: cs.AI

TL;DR: AI-Mandel是一个能够自主生成并实现量子物理学想法的LLM智能体，它从文献中提取想法，使用领域专用AI工具将其转化为可立即在实验室实施的实验设计。


<details>
  <summary>Details</summary>
Motivation: 当前AI在科学领域应用广泛，但创意想法仍主要来自人类研究者。开发能够同时自动生成和执行想法的系统将显著改变人类在科学过程中的角色。

Method: 构建AI-Mandel LLM智能体，从文献中提取想法，并使用领域专用AI工具将这些想法转化为具体的实验设计方案。

Result: AI-Mandel生成的想法具有科学价值，其中两个想法已促成独立科学后续论文的撰写，包括量子隐形传态的新变体、不定因果顺序中的量子网络原语，以及基于量子信息传输闭合回路的几何相位新概念。

Conclusion: AI-Mandel展示了能够生成和实现具体可操作想法的AI物理学家原型，不仅有助于加速科学发展，还揭示了实现人类水平人工科学家所面临的具体挑战。
...
### [395] [Learning to Refine: An Agentic RL Approach for Iterative SPARQL Query Construction](https://arxiv.org/abs/2511.11770)
*Floris Vossebeld,Shenghui Wang*

Main category: cs.AI

TL;DR: 本文提出了一种基于强化学习的智能体框架，让LLM学习迭代构建SPARQL查询的弹性策略，在LC-QuAD 2.0数据集上实现了49.7%的准确率，比最强的零样本基线提升了17.5个百分点。


<details>
  <summary>Details</summary>
Motivation: 解决LLM在生成复杂多跳SPARQL查询时的脆弱性问题，当前方法缺乏基于实时执行反馈动态调试查询的自适应策略。

Method: 使用仅通过结果驱动的强化学习（GRPO）训练的3B参数模型，无需监督微调，学习迭代构建SPARQL查询的弹性策略，通过显式审慎推理步骤提高策略精度。

Result: 在LC-QuAD 2.0的可执行单答案子集上，实体链接后达到49.7%的准确率，比最强的迭代零样本基线提升17.5个百分点。
...
### [396] [On the Measure of a Model: From Intelligence to Generality](https://arxiv.org/abs/2511.11773)
*Ruchira Dhar,Ninell Oldenburg,Anders Soegaard*

Main category: cs.AI

TL;DR: 论文质疑以抽象智力概念为基础的人工智能评估方法，提出应以通用性作为更稳定的评估基础，将通用性重新定义为多任务学习问题。


<details>
  <summary>Details</summary>
Motivation: 当前基于ARC、Raven测试等智力基准的评估方法存在概念模糊问题，无法有效预测模型在实际任务中的表现，可能导致评估与现实效用脱节。

Method: 通过概念和形式分析，检验智力评估的三个假设：通用性、稳定性和现实性，论证只有通用性能够经受住概念和实证检验。

Result: 研究发现智力不是实现通用性的原因，通用性应被理解为多任务学习问题，直接关联到可测量的性能广度和可靠性。

Conclusion: 应重新构建AI进展的评估方式，将通用性作为评估多样化和发展中任务能力的更稳定基础。
...
### [397] [Do LLMs Really Struggle at NL-FOL Translation? Revealing their Strengths via a Novel Benchmarking Strategy](https://arxiv.org/abs/2511.11816)
*Andrea Brunello,Luca Geatti,Michele Mignani,Angelo Montanari,Nicola Saccomanno*

Main category: cs.AI

TL;DR: 本文批判性评估了现有NL-FOL翻译数据集和评估方法的局限性，提出了新的评估协议来区分真正的语义理解与表面模式识别，并发现对话导向的LLM在NL-FOL翻译方面表现出色。


<details>
  <summary>Details</summary>
Motivation: 由于一阶逻辑(FOL)在表达自然语言概念方面的强大能力，将自然语言转换为FOL(NL-FOL翻译)是一个重要但具有挑战性的问题。虽然LLM的出现带来了突破希望，但现有评估方法可能无法准确反映其真实能力。

Method: 1) 批判性分析现有NL-FOL翻译评估数据集和协议；2) 提出新的评估协议，专门设计用于区分真正的语义级逻辑理解与表面模式识别、记忆和数据集污染；3) 使用新方法评估不同LLM的性能。

Result: 研究表明，最先进的对话导向LLM展现出强大的NL-FOL翻译能力和真正的句子级逻辑理解能力，而嵌入中心模型表现明显较差。

Conclusion: 通过更严格的评估协议，发现对话导向的LLM在NL-FOL翻译任务中具有显著的语义理解能力，这为逻辑推理任务的评估提供了新的视角。
...
### [398] [TopoPerception: A Shortcut-Free Evaluation of Global Visual Perception in Large Vision-Language Models](https://arxiv.org/abs/2511.11831)
...
<details>
  <summary>Details</summary>
Motivation: 当前大型视觉语言模型将视觉特征与预训练语言模型对齐，但视觉感知模块成为瓶颈，传统评估基准存在局部捷径，可能高估模型的感知能力。

Method: 利用拓扑特性创建TopoPerception基准测试，拓扑特性依赖于图像全局结构且对局部特征不变，从而实现无捷径的全局感知评估。
...
### [399] [End to End AI System for Surgical Gesture Sequence Recognition and Clinical Outcome Prediction](https://arxiv.org/abs/2511.11899)
...
TL;DR: F2O是一个端到端系统，能将组织解剖视频转化为手势序列，并发现与术后结果相关的模式，为数据驱动的手术反馈和临床决策支持奠定基础。


<details>
  <summary>Details</summary>
Motivation: 解决术中行为细粒度分析及其对患者结果影响的长期挑战，实现自动可解释的手术评估。

Method: 利用基于transformer的空间和时间建模以及逐帧分类，在机器人辅助根治性前列腺切除术的神经保留步骤中稳健检测连续短手势（约2秒）。
...
### [400] [Forgetting-MarI: LLM Unlearning via Marginal Information Regularization](https://arxiv.org/abs/2511.11914)
...
<details>
  <summary>Details</summary>
Motivation: 随着AI模型在不断扩大数据集上训练，从训练模型中移除特定数据影响的能力对于隐私保护和法规遵从变得至关重要。遗忘方法可以在不从头重新训练的情况下选择性移除参数化知识，这对于资源密集型模型如LLM尤为重要。

Method: 引入Forgetting-MarI框架，通过惩罚边际信息来仅移除待遗忘数据贡献的额外信息，同时保留待保留数据支持的信息。该方法提供了对未学习数据集在训练模型中残余影响的明确上界。

Result: 大量实验证实该方法优于当前最先进的遗忘方法，在多样化基准测试中实现了可靠的遗忘和更好的通用模型性能保留。

Conclusion: 这一进展代表了在使AI系统更可控且符合隐私和版权法规而不损害其有效性方面的重要一步。
...
### [401] [An Analysis of Architectural Impact on LLM-based Abstract Visual Reasoning: A Systematic Benchmark on RAVEN-FAIR](https://arxiv.org/abs/2511.11916)
*Sinan Urgun,Seçkin Arı*

Main category: cs.AI

TL;DR: 评估四种LLM模型在抽象视觉推理任务中的表现，发现GPT-4.1-Mini在所有推理架构中表现最佳，但模型对架构设计的敏感性各不相同，多运行评估策略比单次运行更可靠。
...
Method: 使用四种LLM模型（GPT-4.1-Mini、Claude-3.5-Haiku、Gemini-1.5-Flash、Llama-3.3-70b）和四种推理架构（单次推理、嵌入控制重复、自我反思、多智能体）在RAVEN-FAIR数据集上进行测试，采用三阶段处理流程（JSON提取、LLM推理、工具函数）生成视觉响应，使用SSIM和LPIPS指标评估，分析思维链分数和错误类型。

Result: GPT-4.1-Mini在所有架构中始终获得最高准确率，表现出强大的推理能力。多智能体架构偶尔改变模型的语义和数值平衡，但效果并不一致有益。每个模型对架构设计表现出不同的敏感性模式，响应覆盖度的变化进一步复杂化了跨架构比较。

Conclusion: 推理有效性是模型特定的，多运行评估策略比单次运行更可靠，能够提供性能上限的更好估计。
...
### [402] [Looking Forward: Challenges and Opportunities in Agentic AI Reliability](https://arxiv.org/abs/2511.11921)
*Liudong Xing,Janet,Lin*

Main category: cs.AI

TL;DR: 本章讨论了构建可靠AI系统（特别是智能体AI系统）面临的挑战和未来发展方向，重点包括级联故障风险缓解、动态环境、任务执行不一致性、不可预测的涌现行为以及资源密集型可靠性机制等研究问题。


<details>
  <summary>Details</summary>
Motivation: 随着智能体AI系统的发展，确保其可靠性变得越来越重要。这些系统在复杂环境中运行时可能面临级联故障、动态变化、行为不可预测等风险，需要研究相应的可靠性保障机制。

Method: 本章采用问题导向的分析方法，识别了智能体AI系统可靠性的关键挑战领域，包括级联故障缓解、动态环境适应、任务执行一致性、涌现行为管理等方面，并提出了相应的研究方向。

Result: 明确了智能体AI系统可靠性的多个研究挑战和机会，包括级联故障风险、动态环境适应性、任务执行不一致性、不可预测的涌现行为以及资源密集型可靠性机制等问题。

Conclusion: 构建可靠的智能体AI系统需要在多个维度进行深入研究，包括测试评估方法、故障缓解机制、环境适应性等方面，这些研究将为AI系统的安全可靠部署提供重要支撑。
...
### [403] [A Neuromorphic Architecture for Scalable Event-Based Control](https://arxiv.org/abs/2511.11924)
...
<details>
  <summary>Details</summary>
Motivation: 为了解决神经形态控制中连续节律生成和离散决策的统一建模问题，同时结合离散计算的可靠性和连续调节的可调性优势。

Method: 提出反弹赢家通吃(RWTA)基元作为基本构建模块，构建从细胞级到系统级的可扩展架构，继承赢家通吃状态机的离散计算能力和可兴奋生物物理电路的连续调谐能力。

Result: 通过蛇形机器人神经系统设计验证了该架构的通用性、鲁棒性和模块化特性。

Conclusion: 基于RWTA基元的神经形态控制架构为连续节律生成和离散决策提供了统一的物理建模框架，具有良好的可扩展性和实用性。
...
### [404] [Augmenting The Weather: A Hybrid Counterfactual-SMOTE Algorithm for Improving Crop Growth Prediction When Climate Changes](https://arxiv.org/abs/2511.11945)
...
<details>
  <summary>Details</summary>
Motivation: 传统机器学习方法依赖历史数据分布，难以处理气候变化带来的分布外异常事件，因为历史数据中缺乏足够的少数类（气候异常事件）样本。

Method: CFA-SMOTE方法将可解释AI中的反事实生成技术与SMOTE过采样技术相结合，生成代表气候异常事件的合成数据点来增强数据集。

Result: 在不同类别不平衡比例条件下，CFA-SMOTE方法在预测爱尔兰奶牛场草生长（2018年欧洲干旱和饲料危机期间）方面优于基准方法。

Conclusion: 将气候变化预测问题视为类别不平衡问题，并通过反事实数据增强方法可以有效提高对气候异常事件的预测性能。
...
### [405] [LLM-Assisted Formalization Enables Deterministic Detection of Statutory Inconsistency in the Internal Revenue Code](https://arxiv.org/abs/2511.11954)
*Borchuluun Yadamsuren,Steven Keith Platt,Miguel Diaz*

Main category: cs.AI

TL;DR: 本文提出了一种混合神经符号框架，结合大语言模型和符号逻辑，实现了对复杂法律中法定不一致性的确定性检测，并以美国国内税收法典为例验证了方法的有效性。


<details>
  <summary>Details</summary>
Motivation: 现有LLM方法在层次化处理和深度结构化推理方面存在困难，特别是在处理长文本时。税法领域复杂且容易产生冲突条款，需要一种能够可靠检测法定不一致性的方法。

Method: 使用GPT-4o将税法条款翻译为Prolog规则，在SWISH中精炼，然后结合Prolog增强提示来测试不一致性检测。同时开发了混合Prolog模型，利用GPT-5进行规则精炼。

Result: GPT-4o在三种策略中仅检测到一种不一致性（33%准确率）。纯自然语言提示实现了100%规则覆盖，而Prolog增强提示仅66%覆盖。混合Prolog模型实现了确定性、可重现的结果，成功检测到不一致性区域。

Conclusion: 基于符号逻辑的LLM辅助形式化方法能够实现透明可靠的法定不一致性检测，混合神经符号框架在复杂法律分析中具有优势。
...
### [406] [Improving Autoformalization Using Direct Dependency Retrieval](https://arxiv.org/abs/2511.11990)
*Shaoqi Wang,Lu Yu,Chunjie Yang*

Main category: cs.AI

TL;DR: 提出基于直接依赖检索(DDR)的声明自动形式化框架，通过直接生成候选库依赖并验证其在形式库中的存在，解决了现有方法缺乏上下文意识和依赖检索精度低的问题。


<details>
  <summary>Details</summary>
Motivation: 现有自动形式化方法缺乏上下文意识，导致形式定义和定理的幻觉，且检索增强方法在形式库依赖检索方面精度和召回率差，无法有效利用日益增长的公共数据集。

Method: 提出DDR方法，直接从自然语言数学描述生成候选库依赖，通过高效后缀数组检查验证其在形式库中的存在，构建超过50万样本的依赖检索数据集并微调高精度DDR模型。

Result: DDR模型在检索精度和召回率上显著优于SOTA方法，配备DDR的自动形式化器在单次尝试准确率和多次尝试稳定性方面均优于传统基于选择的RAG方法。

Conclusion: DDR框架通过高效的依赖检索机制，有效提升了声明自动形式化的性能，为深度学习和形式数学的融合提供了更可靠的基础。
...
### [407] [Look As You Think: Unifying Reasoning and Visual Evidence Attribution for Verifiable Document RAG via Reinforcement Learning](https://arxiv.org/abs/2511.12003)
*Shuochen Liu,Pengfei Luo,Chao Zhang,Yuhao Chen,Haotian Zhang,Qi Liu,Xin Kou,Tong Xu,Enhong Chen*

Main category: cs.AI

TL;DR: 提出了Chain-of-Evidence (CoE)范式和Look As You Think (LAT)强化学习框架，用于视觉文档检索增强生成中的证据归因，通过边界框和页面索引将推理步骤与具体视觉证据区域关联。


<details>
  <summary>Details</summary>
Motivation: 现有方法缺乏细粒度监督和推理过程的渐进可追溯性，无法确保视觉语言模型在多模态问答中的可靠性和可验证性。

Method: CoE统一了思维链推理和视觉证据归因，LAT通过强化学习训练模型生成可验证的推理路径，评估证据区域的一致性，并在正确答案时提供奖励。

Result: 在Qwen2.5-VL-7B-Instruct模型上，LAT在单图和双图设置下均显著提升性能，soft EM平均提升8.23%，IoU@0.5提升47.0%，且表现出更强的跨领域泛化能力。

Conclusion: LAT框架有效提升了视觉文档检索增强生成的证据归因能力，实现了过程级的自验证，优于监督微调基线方法。
...
### [408] [Adaptive Diagnostic Reasoning Framework for Pathology with Multimodal Large Language Models](https://arxiv.org/abs/2511.12008)
...
Result: 在乳腺癌和前列腺癌数据集上，RECAP-PATH生成了与专家评估一致的推理，诊断准确性显著优于基线方法。
...
### [409] [Intelligent Collaborative Optimization for Rubber Tyre Film Production Based on Multi-path Differentiated Clipping Proximal Policy Optimization](https://arxiv.org/abs/2511.12060)
*Yinghao Ruan,Wei Pang,Shuaihao Liu,Huili Yang,Leyi Han,Xinghui Dong*

Main category: cs.AI

TL;DR: 提出MPD-PPO深度强化学习算法解决轮胎制造中的高维多目标优化问题，通过多分支策略架构和差异化梯度裁剪约束实现稳定高效的高维策略更新。


<details>
  <summary>Details</summary>
Motivation: 传统集中式调度和刚性生产线配置难以应对轮胎制造业的动态生产需求，复杂子系统网络中的非线性相互作用和涌现动态使得多子系统协调成为关键但困难的任务。

Method: 采用多路径差异化裁剪近端策略优化算法，使用多分支策略架构配合差异化梯度裁剪约束，确保高维策略更新的稳定性和效率。

Result: 在橡胶轮胎薄膜生产的宽度和厚度控制实验中，MPD-PPO在调谐精度和操作效率方面均取得显著提升。
...
### [410] [Bayesian Optimization in Language Space: An Eval-Efficient AI Self-Improvement Framework](https://arxiv.org/abs/2511.12063)
*Enoch Hyunwook Kang,Hema Yoganarasimhan*

Main category: cs.AI

TL;DR: 提出了TextGrad-Best-of-N Bayesian Optimization (T-BoN BO)框架，通过结合Best-of-N选择和文本梯度来模拟UCB采集函数，优化AI自我改进中的评估效率问题。


<details>
  <summary>Details</summary>
Motivation: 在AI自我改进应用中，评估解决方案的成本往往远高于生成解决方案的成本（如广告效果评估需要大量人工反馈），因此需要优化评估效率而非查询效率。

Method: 证明Best-of-N选择策略与文本梯度结合可以统计模拟UCB采集函数的梯度行为，提出T-BoN BO框架进行语言空间的贝叶斯优化。

Result: 在自动广告对齐任务中，T-BoN BO相比现有最优基线方法表现出更优越的性能。

Conclusion: T-BoN BO为AI自我改进提供了一个简单且评估效率高的语言空间贝叶斯优化框架，解决了评估成本高昂的实际问题。
...
TL;DR: 提出Embedding CFR算法，通过将信息集嵌入到低维连续空间来解决大规模不完全信息扩展式博弈，相比基于聚类的抽象方法能更精确捕捉信息集间的差异和联系，在扑克实验中实现了更快的可剥削性收敛。
...
Result: 在扑克实验中，在相同空间开销下，Embedding CFR相比基于聚类的抽象算法实现了显著更快的可剥削性收敛。

Conclusion: 这是扑克AI中首个通过低维嵌入预训练信息集抽象进行策略求解的算法，理论分析验证了其减少累积遗憾的能力，实验证实了其有效性。
...
### [412] [KrwEmd: Revising the Imperfect-Recall Abstraction from Forgetting Everything](https://arxiv.org/abs/2511.12089)
*Yanchang Fu,Qiyue Yin,Shengda Liu,Pei Xu,Kaiqi Huang*

Main category: cs.AI

TL;DR: KrwEmd算法通过k-recall winrate特征和earth mover's distance聚类来解决德州扑克等游戏中过度抽象问题，显著提升AI性能


<details>
  <summary>Details</summary>
Motivation: 解决大规模不完全信息游戏中过度抽象的问题，特别是由不完全回忆抽象极端实现导致的历史信息完全丢失问题

Method: 提出k-recall winrate特征，利用历史和未来游戏信息区分信号观察信息集；开发KrwEmd算法，使用earth mover's distance测量特征差异并聚类信号观察信息集

Result: 实验结果显示KrwEmd相比现有算法显著提高了AI游戏表现

Conclusion: KrwEmd是首个实用的解决过度抽象问题的算法，通过有效利用历史信息改善了AI在大型不完全信息游戏中的性能
...
### [413] [MetaGDPO: Alleviating Catastrophic Forgetting with Metacognitive Knowledge through Group Direct Preference Optimization](https://arxiv.org/abs/2511.12113)
*Lanxue Zhang,Yuqiang Xie,Fang Fang,Fanglong Dong,Rui Liu,Yanan Cao*

Main category: cs.AI

TL;DR: 提出综合解决方案缓解小模型的知识蒸馏中的灾难性遗忘问题，包括构建包含元认知知识的数据集和设计GDPO训练方法


<details>
  <summary>Details</summary>
Motivation: 现有数据集和微调方法在8B以下小模型上存在灾难性遗忘问题，主要由于忽视训练数据知识与模型固有能力的关系，以及传统训练目标无法有效约束固有知识保留

Method: 1) 构建5K实例数据集，覆盖多推理任务并包含元认知知识标注；2) 提出GDPO训练方法，通过参考模型隐式约束优化路径，更适合资源受限场景

Result: 实验表明该方法显著缓解灾难性遗忘，提升小模型的推理性能

Conclusion: 从数据和训练方法两方面入手，通过元认知知识数据集和GDPO优化，有效解决小模型知识蒸馏中的灾难性遗忘问题
...
### [414] [RTMol: Rethinking Molecule-text Alignment in a Round-trip View](https://arxiv.org/abs/2511.12135)
...
Conclusion: RTMol通过双向对齐框架有效解决了分子-文本表示对齐中的关键挑战，显著提升了双向一致性性能。
...
### [415] [Incremental Maintenance of DatalogMTL Materialisations](https://arxiv.org/abs/2511.12169)
...
Conclusion: DRedMTL为DatalogMTL提供了高效的增量推理能力，满足实际应用需求
...
### [416] [Debate over Mixed-knowledge: A Robust Multi-Agent Framework for Incomplete Knowledge Graph Question Answering](https://arxiv.org/abs/2511.12208)
*Jilong Liu,Pengyang Shao,Wei Qin,Fei Liu,Yonghui Yang,Richang Hong*

Main category: cs.AI

TL;DR: 提出DoM框架，通过多智能体辩论机制动态融合结构化和非结构化知识来解决不完整知识图谱问答问题，并创建了更真实的IKGQA数据集。


<details>
  <summary>Details</summary>
Motivation: 现实世界知识图谱通常不完整，现有方法无法自适应地融合多种知识源以充分利用其互补优势。

Method: DoM框架基于多智能体辩论范式，分配专门智能体分别对知识图谱和外部文本进行推理，通过迭代交互协调输出。包括问题分解、双重证据检索（KG和RAG）和法官智能体评估聚合。

Result: 实验表明DoM持续优于最先进的基线方法。

Conclusion: DoM通过知识互补性增强了鲁棒性，解决了知识图谱不完整性问题，并提供了更现实的评估基准。
...
### [417] [ViTE: Virtual Graph Trajectory Expert Router for Pedestrian Trajectory Prediction](https://arxiv.org/abs/2511.12214)
*Ruochen Li,Zhanxing Zhu,Tanqiu Qiao,Hubert P. H. Shum*

Main category: cs.AI

TL;DR: 提出ViTE框架，通过虚拟图和专家路由器模块，自适应建模行人轨迹预测中的显式单跳和隐式高阶交互，避免深层GNN的计算负担。


<details>
  <summary>Details</summary>
Motivation: 现有方法在行人轨迹预测中面临深度与计算成本的权衡：浅层网络感受野不足，深层网络计算成本过高。需要能够自适应建模不同层次交互的方法。

Method: ViTE框架包含两个核心模块：虚拟图通过动态虚拟节点建模长距离高阶交互；专家路由器基于社交上下文自适应选择交互专家，采用混合专家设计。

Result: 在ETH/UCY、NBA和SDD三个基准测试中均达到最先进性能，验证了方法的有效性和实际效率。

Conclusion: ViTE通过虚拟图和专家路由器的组合，实现了灵活可扩展的交互推理，在行人轨迹预测任务中表现出色且高效。
...
### [418] [Beyond World Models: Rethinking Understanding in AI Models](https://arxiv.org/abs/2511.12239)
...
Result: 虽然这些观点不代表普遍定义，但有助于探索世界模型的局限性
...
### [419] [AURA: Development and Validation of an Augmented Unplanned Removal Alert System using Synthetic ICU Videos](https://arxiv.org/abs/2511.12241)
*Junhyuk Seo,Hyeyoon Moon,Kyu-Hwan Jung,Namkee Oh,Taerim Kim*

Main category: cs.AI

TL;DR: AURA是一个基于视觉的风险检测系统，使用完全合成的ICU视频数据集开发，通过姿态估计识别两种高风险运动模式：碰撞（手进入气道管附近区域）和躁动（追踪解剖关键点的速度），为ICU中无计划拔管提供隐私保护的实时检测方案。


<details>
  <summary>Details</summary>
Motivation: ICU中无计划拔管是严重的安全问题，但实时检测受到伦理和隐私限制，难以获取标注的ICU视频数据。

Method: 利用文本到视频扩散技术生成多样化和临床真实的ICU场景，通过姿态估计识别碰撞（手进入气道管附近空间区域）和躁动（追踪解剖关键点速度）两种高风险模式。
...
### [420] [Mobile-Agent-RAG: Driving Smart Multi-Agent Coordination with Contextual Knowledge Empowerment for Long-Horizon Mobile Automation](https://arxiv.org/abs/2511.12254)
*Yuxiang Zhou,Jichang Li,Yanhao Zhang,Haonan Lu,Guanbin Li*

Main category: cs.AI

TL;DR: Mobile-Agent-RAG是一个分层多代理框架，通过双级检索增强解决移动代理在真实世界长时程跨应用任务中的性能瓶颈问题。


<details>
  <summary>Details</summary>
Motivation: 当前最先进的移动代理在真实世界长时程跨应用任务上成功率不足，主要原因是过度依赖MLLM中的静态内部知识，导致高层规划中的战略幻觉和低层UI操作中的执行错误。

Method: 提出分层多代理框架Mobile-Agent-RAG，集成双级检索增强：在规划阶段使用Manager-RAG检索人类验证的全面任务计划，在执行阶段使用Operator-RAG检索精确的低层指导。构建了两个专门的检索导向知识库。

Result: 在Mobile-Eval-RAG基准测试中，Mobile-Agent-RAG显著优于最先进的基线方法，任务完成率提高11.0%，步骤效率提高10.2%。

Conclusion: Mobile-Agent-RAG为上下文感知、可靠的多代理移动自动化建立了一个稳健的范式，通过双级检索增强有效解决了移动代理的关键失败点。
...
TL;DR: 该论文提出了一种解决大语言模型道德对齐问题的方法，通过构建Moral-Reason-QA数据集和Group Relative Policy Optimization学习框架，使LLM能够在超出训练分布的场景中应用一致的道德推理框架。
...
Method: 构建Moral-Reason-QA数据集（680个人工标注的高模糊度道德场景），使用Group Relative Policy Optimization结合复合奖励函数，同时优化决策对齐和框架特定推理过程。

Result: 在未见道德场景中成功实现泛化，功利主义框架的softmax归一化对齐分数提升+0.757，义务论框架提升+0.450。

Conclusion: LLM代理可以系统性地训练以内部化并应用特定道德框架到新情境，为AI安全提供关键基础。
...
TL;DR: UpBench是一个基于真实Upwork工作任务的动态基准测试，用于评估LLM代理在真实工作环境中的能力，采用专家制定的评分标准和人类评估相结合的方法。


<details>
  <summary>Details</summary>
Motivation: 现有基准测试大多是静态、合成或领域受限的，无法反映AI代理在动态、经济意义环境中的真实表现，需要更贴近真实工作场景的评估框架。

Method: 从Upwork平台提取真实工作任务，由专家自由职业者制定详细的验收标准，采用基于评分标准的评估框架对AI提交内容进行细粒度评估，并定期更新任务以反映工作演变。

Result: 建立了基于真实工作活动的评估基准，能够对模型优势、弱点和指令遵循能力进行细粒度分析，超越了简单的通过/失败指标。

Conclusion: UpBench提供了一个可扩展、以人为本的基础，用于在真实劳动力市场环境中评估代理系统，为实现AI通过合作而非替代来增强人类能力的协作框架提供了路径。
...
<details>
  <summary>Details</summary>
Motivation: 现有强化学习方法主要关注单领域可验证奖励，且纯在线RL框架限制了探索空间，从而限制了推理性能

Method: RGR-GRPO框架，利用评分标准提供密集信息奖励，在GRPO训练期间探索更大的解决方案空间

Result: 在14个多领域基准测试中，相比可验证在线RL基线，在数学、物理、化学和一般推理任务上分别平均提升7.0%、5.4%、8.4%和6.6%

Conclusion: RGR-GRPO在离线策略训练中保持稳定的熵波动，实现卓越的pass@k性能，反映了持续探索和有效突破现有性能瓶颈
...
TL;DR: 本文提出了一种计算理性用户模型，用于建模认知受限代理在偏见信念下的最优行为，重点关注记忆限制如何导致动态不一致的信念状态和次优决策，并开发了在线推理方法来识别用户特定的认知限制。
...
Result: 在代表性导航任务中验证：1) CR模型生成与不同记忆容量水平对应的直观合理行为；2) 推理方法从有限观察（≤100步）中准确高效地恢复真实认知限制。

Conclusion: 该方法为开发自适应AI助手提供了原则性基础，使得辅助能够考虑用户的记忆限制，实现个性化适应。
...
TL;DR: 提出一个动态学习建议者可靠性的框架，在部分可观测环境中通过贝叶斯推断建议者类型，并引入"询问"动作来策略性地请求建议。
...
Method: 1. 将建议者质量直接整合到智能体的信念表示中，通过贝叶斯推断建议者类型来调整对建议的依赖程度；2. 引入显式的"询问"动作，让智能体在关键时刻策略性地请求建议，平衡信息获取与成本。
...
### [426] [Multi-agent Self-triage System with Medical Flowcharts](https://arxiv.org/abs/2511.12439)
...
Result: 在模拟对话的大规模评估中，系统实现了95.29%的前3准确率流程图检索（N=2,000）和99.10%的流程图导航准确率（N=37,200）。
...
### [427] [ARCHE: A Novel Task to Evaluate LLMs on Latent Reasoning Chain Extraction](https://arxiv.org/abs/2511.12485)
*Pengze Li,Jiaqi Liu,Junchi Yu,Lihao Liu,Mingyu Ding,Wanli Ouyang,Shixiang Tang,Xi Chen*

Main category: cs.AI

TL;DR: 提出了ARCHE任务和ARCHE Bench基准，要求模型将复杂推理分解为标准推理范式组成的推理逻辑树，评估发现现有LLMs在科学推理方面仍有显著差距。


<details>
  <summary>Details</summary>
Motivation: 解决LLMs生成的推理内容通常非结构化且不正式的问题，探究模型是否真正理解科学推理的基本范式。

Method: 引入潜在推理链提取任务，将推理分解为演绎、归纳、溯因三种基本推理模式组成的推理逻辑树，并提出实体覆盖率和推理边准确率两个逻辑感知评估指标。

Result: 评估10个领先LLMs发现模型在REA和EC之间存在权衡，且无法提取完整标准的推理链。
...
### [428] [LOBERT: Generative AI Foundation Model for Limit Order Book Messages](https://arxiv.org/abs/2511.12563)
...
<details>
  <summary>Details</summary>
Motivation: 现有LOB模型需要繁琐的数据表示，缺乏任务适应性，难以处理不规则事件时间和高频交易者的反应。

Method: 基于BERT架构，使用新颖的标记化方案将完整多维消息作为单个标记，同时保留价格、数量和时间的连续表示。

Result: LOBERT在预测中间价格变动和下一消息等任务中达到领先性能，同时相比先前方法减少了所需上下文长度。

Conclusion: LOBERT为LOB数据提供了一个有效的通用基础模型，适用于下游任务的微调。
...
### [429] [Enhancing Conversational Recommender Systems with Tree-Structured Knowledge and Pretrained Language Models](https://arxiv.org/abs/2511.12579)
*Yongwen Ren,Chao Wang,Peng Du,Chuan Qin,Dazhong Shen,Hui Xiong*

Main category: cs.AI

TL;DR: PCRS-TKA是一个基于提示的框架，通过检索增强生成将预训练语言模型与知识图谱集成，解决了现有方法未能充分利用PLM在图关系上的推理能力、缺乏上下文知识过滤以及忽视多轮对话中协作偏好的问题。


<details>
  <summary>Details</summary>
Motivation: 现有方法在将PLM与知识图谱集成时面临三个关键挑战：未能充分利用PLM在图关系上的推理能力、不加区分地整合检索到的知识而缺乏上下文过滤、以及忽视多轮对话中的协作偏好。

Method: PCRS-TKA构建对话特定的知识树并将其序列化为文本，实现结构感知推理；选择性过滤上下文相关知识；使用专门监督信号显式建模协作偏好；通过语义对齐模块协调异构输入以减少噪声。

Result: 大量实验表明，PCRS-TKA在推荐和对话质量方面始终优于所有基线方法。

Conclusion: PCRS-TKA通过检索增强生成有效整合PLM与知识图谱，显著提升了对话推荐系统的准确性和对话质量。
...
### [430] [Dynamic Tree Databases in Automated Planning](https://arxiv.org/abs/2511.12677)
*Oliver Joergensen,Dominik Drexler,Jendrik Seipp*

Main category: cs.AI

TL;DR: 提出了一种动态树数据库变体，用于压缩命题和数值变量的状态集，在保持静态版本优点的同时避免了大量内存预分配。


<details>
  <summary>Details</summary>
Motivation: 在大型任务中扩展显式状态空间搜索时，紧凑表示生成状态集是一个核心挑战。静态树数据库虽然每个状态只需要常数空间，但需要大量内存预分配。

Method: 开发了树数据库的动态变体，用于压缩命题和数值变量的状态集，并证明其保持了静态对应物的理想特性。

Result: 在经典和数值规划任务上的实证评估显示，压缩比达到几个数量级，且运行时开销通常可忽略不计。

Conclusion: 动态树数据库为状态压缩提供了高效的解决方案，在保持压缩性能的同时解决了内存预分配问题。
...
### [431] [Adaptively Coordinating with Novel Partners via Learned Latent Strategies](https://arxiv.org/abs/2511.12754)
...
TL;DR: 提出了一个策略条件化的合作者框架，通过变分自编码器学习策略空间，聚类识别不同策略类型，并训练条件化合作者来实时适应各种人类伙伴策略。


<details>
  <summary>Details</summary>
Motivation: 在人类-智能体团队中，人工智能体需要实时适应人类伙伴的独特偏好和动态变化的策略，这在时间压力和复杂策略空间中尤为困难。

Method: 使用变分自编码器编码策略学习潜在策略空间，通过聚类识别策略类型，训练条件化合作者，并利用固定份额遗憾最小化算法进行在线适应。

Result: 在修改版Overcooked环境中的实验和在线用户研究表明，该方法在与新人类和智能体队友配对时达到了最先进的性能。
...
### [432] [Optimal Foraging in Memory Retrieval: Evaluating Random Walks and Metropolis-Hastings Sampling in Modern Semantic Spaces](https://arxiv.org/abs/2511.12759)
*James Moore*

Main category: cs.AI

TL;DR: 研究表明，现代高维嵌入空间中的简单随机游走可以产生与人类记忆检索中最佳觅食行为一致的结果，而更复杂的Metropolis-Hastings采样反而与人类行为不符。


<details>
  <summary>Details</summary>
Motivation: 探索现代高维嵌入空间是否能提供足够好的表示，使得算法能够匹配人类在语义流畅性任务中观察到的觅食模式行为。

Method: 使用最先进的嵌入和先前的语义流畅性数据，在嵌入空间上进行随机游走和Metropolis-Hastings采样，比较它们与人类行为的匹配程度。

Result: 简单随机游走在嵌入空间中产生的结果与最佳觅食和边际价值定理一致，而Metropolis-Hastings采样未能产生与人类行为一致的结果。

Conclusion: 适当结构的嵌入即使使用简单采样也能产生接近最佳的觅食动态，挑战了复杂采样机制必然导致更好认知模型的假设。
...
### [433] [Event-CausNet: Unlocking Causal Knowledge from Text with Large Language Models for Reliable Spatio-Temporal Forecasting](https://arxiv.org/abs/2511.12769)
*Luyao Niu,Zepu Wang,Shuyi Guan,Yang Liu,Peng Sun*

Main category: cs.AI

TL;DR: Event-CausNet框架利用LLM量化非结构化事件报告，构建因果知识库，并通过因果注意力机制将因果知识注入GNN-LSTM网络，显著提升非周期性事件下的交通预测性能。


<details>
  <summary>Details</summary>
Motivation: 传统时空图神经网络在处理非周期性事件（如事故）时性能急剧下降，因为GNN本质上是相关性模型，无法处理突发事件引入的新因果因素。

Method: 使用大语言模型量化非结构化事件报告，通过估计平均处理效应构建因果知识库，采用双流GNN-LSTM网络和因果注意力机制注入因果知识。

Result: 在真实数据集上，Event-CausNet将预测误差（MAE）降低了35.87%，显著优于现有最优基线方法。

Conclusion: 该框架弥合了相关性模型与因果推理之间的差距，提供了更准确、可迁移且可解释的解决方案，为关键中断期间的交通管理提供了更可靠的基础。
...
### [434] [Multi-Agent Reinforcement Learning for Heterogeneous Satellite Cluster Resources Optimization](https://arxiv.org/abs/2511.12792)
...
<details>
  <summary>Details</summary>
Motivation: 传统优化方法难以处理地球观测任务中实时、不确定和去中心化的特性，因此需要采用强化学习和多智能体强化学习来实现自适应决策。

Method: 系统地将优化问题从单卫星扩展到多卫星场景，使用基于Basilisk和BSK-RL框架构建的近真实仿真环境，评估MAPPO、HAPPO和HATRPO等先进MARL算法。
...
### [435] [Neuro-Logic Lifelong Learning](https://arxiv.org/abs/2511.12793)
*Bowen He,Xiaoan Xu,Alper Kamil Bozkurt,Vahid Tarokh,Juncheng Dong*

Main category: cs.AI

TL;DR: 提出终身学习ILP框架，利用逻辑规则的组合性和可转移性，通过重用先前任务学到的逻辑规则来高效学习新问题


<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注为单个问题设计新的网络架构，较少探索涉及问题序列的新学习范式。ILP问题的逻辑规则具有组合性和可转移性，适合终身学习场景

Method: 引入组合性框架，展示如何高效重用从早期任务获得的逻辑规则到后续任务中，通过形式化方法在任务序列上进行学习

Result: 实验结果表明该范式的可行性和优势，在任务序列上的实证评估验证了方法的有效性

Conclusion: 该方法为神经符号AI中的持续学习开辟了新方向，通过重用逻辑规则提高了可扩展性和性能
...
### [436] [Mapping fNIRS Signals to Agent Performance: Toward Reinforcement Learning from Neural Feedback](https://arxiv.org/abs/2511.12844)
*Julia Santaniello,Matthew Russell,Benson Jiang,Donatello Sassaroli,Robert Jacob,Jivko SInapov*

Main category: cs.AI

TL;DR: 本文提出使用被动脑机接口（BCI）和功能性近红外光谱（fNIRS）技术，通过隐式神经信号来指导强化学习代理的训练，构建了一个包含25名参与者在三个领域的数据集，并展示了从fNIRS信号预测代理性能的可行性。


<details>
  <summary>Details</summary>
Motivation: 传统的强化学习从人类反馈（RLHF）需要显式的人类反馈，而本文旨在探索使用隐式神经信号（通过fNIRS测量）来指导代理训练，实现更自然、无干扰的人机交互。

Method: 收集了25名参与者在三个任务领域（拾放机器人、月球着陆器、Flappy Bird）的fNIRS数据，训练分类器预测代理性能水平（最优、次优、最差），以及回归器预测动作与最优策略的偏差程度，并评估跨被试泛化能力。

Result: 二元分类平均F1分数达67%，多分类平均F1分数达46%；通过少量被试特定数据微调预训练模型，二元和多分类F1分数分别提升17%和41%，证明了从fNIRS信号映射代理性能的可行性。

Conclusion: 研究表明使用隐式fNIRS信号来指导RLHF系统是可行的，且可通过微调进一步提升性能，为未来脑驱动的RLHF系统奠定了基础。
...
### [437] [Bootstrapping LLMs via Preference-Based Policy Optimization](https://arxiv.org/abs/2511.12867)
*Chen Jia*

Main category: cs.AI

TL;DR: 提出了一种基于偏好的策略优化框架，通过主策略与奖励模型之间的min-max博弈来引导LLM对齐，无需大量人工标注。


<details>
  <summary>Details</summary>
Motivation: 通过偏好数据引导大型语言模型对齐人类偏好，避免依赖大量手动标注，实现模型行为的持续自我改进。

Method: 构建min-max博弈框架，主策略与奖励模型在偏好数据约束的置信集内相互优化，采用迭代在线算法主动收集偏好数据。

Result: 在五个基准测试中持续优于现有最先进的偏好优化技术，并为序列级和令牌级奖励模型提供了高概率遗憾界理论保证。

Conclusion: 该偏好策略优化框架为无需大量人工标注的LLM对齐提供了有效解决方案，通过理论分析和实验验证了其优越性能。
...
### [438] [Think, Speak, Decide: Language-Augmented Multi-Agent Reinforcement Learning for Economic Decision-Making](https://arxiv.org/abs/2511.12876)
*Heyang Ma,Qirui Mi,Qipeng Yang,Zijun Fan,Bo Li,Haifeng Zhang*

Main category: cs.AI

TL;DR: LAMP是一个将语言增强到经济决策中的多智能体强化学习框架，通过Think-Speak-Decide流程整合数值数据和语言信息，在经济效益、鲁棒性和可解释性方面显著优于传统方法。


<details>
  <summary>Details</summary>
Motivation: 现实经济决策不仅依赖结构化信号（价格、税收），还依赖非结构化语言（同行对话、媒体叙事）。传统多智能体强化学习在处理语言语义模糊性和上下文丰富性方面存在困难。

Method: LAMP采用Think-Speak-Decide三阶段流程：(1) Think阶段解释数值观察，提取短期冲击和长期趋势，缓存高价值推理轨迹；(2) Speak阶段基于推理生成和交换策略消息，通过解析同伴通信更新信念；(3) Decide阶段融合数值数据、推理和反思，通过MARL策略优化语言增强的决策。
...
### [439] [Online Learning of HTN Methods for integrated LLM-HTN Planning](https://arxiv.org/abs/2511.12901)
*Yuesheng Xu,Hector Munoz-Avila*

Main category: cs.AI

TL;DR: 提出了一种在线学习分层任务网络(HTN)方法的技术，通过从ChatGPT生成的任务分解中学习通用方法，减少对ChatGPT的调用次数。
...
Method: 在ChatHTN规划器基础上扩展，当ChatGPT生成任务分解时，学习通用方法而非简单记忆，使方法适用于相同任务的其他实例。

Result: 在两个领域上的实验表明，在线学习过程减少了ChatGPT调用次数，同时解决了至少同样多的问题，在某些情况下甚至更多。

Conclusion: 该方法能有效减少对大型语言模型的依赖，提高HTN规划的效率。
...
### [440] [CoS: Towards Optimal Event Scheduling via Chain-of-Scheduling](https://arxiv.org/abs/2511.12913)
*Yiming Zhao,Jiwei Tang,Shimin Di,Libin Zheng,Jianxing Yu,Jian Yin*

Main category: cs.AI

TL;DR: 提出Chain-of-Scheduling (CoS)框架，通过探索、验证和集成三个阶段激活LLM的事件调度能力，在EBSNs中实现高效有效的事件推荐。


<details>
  <summary>Details</summary>
Motivation: 现有事件推荐方法在效率、效果和泛化性之间存在固有权衡，需要解决NP难问题的挑战。

Method: 将调度任务分解为探索、验证和集成三个原子阶段，通过知识蒸馏使LLM自主生成CoS。

Result: 在三个真实数据集上实现接近理论最优的效果，具有高效率和可解释性，在域外数据上展现强零样本学习能力。

Conclusion: CoS框架成功激活了LLM的事件调度能力，在保持高效率的同时实现了优异的推荐效果和泛化性能。
...
### [441] [Fault2Flow: An AlphaEvolve-Optimized Human-in-the-Loop Multi-Agent System for Fault-to-Workflow Automation](https://arxiv.org/abs/2511.12916)
...
<details>
  <summary>Details</summary>
Motivation: 当前电网故障诊断依赖人工方法，效率低、易出错且难以维护，需要将法规文本和专家知识整合到可验证的自动化工作流中。

Method: 系统性地：(1) 提取法规逻辑为PASTA格式故障树；(2) 通过人机交互界面整合专家知识；(3) 使用AlphaEvolve模块优化推理逻辑；(4) 合成最终验证逻辑为n8n可执行工作流。
...
### [442] [Yanyun-3: Enabling Cross-Platform Strategy Game Operation with Vision-Language Models](https://arxiv.org/abs/2511.12937)
*Guoyan Wang,Yanyan Huang,Chunlin Chen,Lifeng Wang,Yuxiang Sun*

Main category: cs.AI

TL;DR: Yanyun-3是一个通用智能体框架，首次实现跨三个异构策略游戏环境的自主操作，通过结合视觉语言推理和精确执行能力，在目标定位、资源分配和区域控制等核心任务上表现出色。


<details>
  <summary>Details</summary>
Motivation: 解决策略游戏中跨平台操作的自动化需求，探索视觉语言模型在复杂人机交互场景中的应用潜力。

Method: 集成Qwen2.5-VL的视觉语言推理和UI-TARS的精确执行能力，采用屏幕捕获-模型推理-动作执行的闭环流程，研究多模态数据组合策略。

Result: 混合策略（MV+S）相比完全融合减少63%推理时间，BLEU-4得分从4.81%提升至62.41%（约12.98倍提升），展现出强大的实时性能和跨平台泛化能力。

Conclusion: 该工作不仅为策略游戏自动化提供了高效解决方案，还建立了通过结构化多模态数据组织增强VLM性能的通用范式，为具身智能中静态感知与动态推理的交互提供了新见解。
...
### [443] [MedRule-KG: A Knowledge-Graph--Steered Scaffold for Reliable Mathematical and Biomedical Reasoning](https://arxiv.org/abs/2511.12963)
*Crystal Su*

Main category: cs.AI

TL;DR: 提出了MedRule-KG系统，通过知识图谱支架和轻量级验证器来引导LLM生成数学和生物医学上有效的输出，在药物发现任务中显著减少违规并提高准确率。


<details>
  <summary>Details</summary>
Motivation: 解决大型语言模型在科学推理和早期药物发现中缺乏领域一致性的问题，确保生成的输出在数学和生物医学上有效。

Method: 使用紧凑的知识图谱支架配合轻量级验证器，将精选的符号事实注入提示中，并通过确定性检查器强制执行规则满足，将生成形式化为约束推理。

Result: 在90个任务中，相比强链式思维基线，违规数量减少83.2%，同时提高了精确匹配率，验证器仅增加可忽略的延迟。

Conclusion: MedRule-KG方法能有效引导LLM生成领域一致的输出，在药物发现任务中表现稳定且实用，适合交互式设计。
...
### [444] [WebCoach: Self-Evolving Web Agents with Cross-Session Memory Guidance](https://arxiv.org/abs/2511.12997)
*Genglin Liu,Shijie Geng,Sha Li,Hejie Cui,Sarah Zhang,Xin Liu,Tianyi Liu*

Main category: cs.AI

TL;DR: WebCoach是一个模型无关的自进化框架，为网页浏览代理提供持久跨会话记忆，通过记忆存储、经验检索和实时建议注入，提升代理在复杂浏览任务中的长期规划和持续学习能力。


<details>
  <summary>Details</summary>
Motivation: 当前多模态LLM代理在网页导航中存在重复错误问题，缺乏跨会话学习能力，限制了长期鲁棒性和样本效率。

Method: WebCoach包含三个核心组件：WebCondenser标准化导航日志为摘要，外部记忆存储组织完整轨迹作为经验，Coach基于相似性和时效性检索相关经验并通过运行时钩子注入任务特定建议。

Result: 在WebVoyager基准测试中，WebCoach持续提升了三种不同LLM骨干的浏览器使用代理性能。使用38B模型时，任务成功率从47%提升至61%，同时减少或维持平均步骤数。较小基础模型配合WebCoach可达到与使用GPT-4o的相同网页代理相当的性能。

Conclusion: WebCoach使网页代理能够访问超出原生上下文窗口的长期记忆，通过持续从新导航轨迹中整理情景记忆实现自进化，无需重新训练即可随时间改进代理性能。
...
### [445] [GEM: Generative Entropy-Guided Preference Modeling for Few-shot Alignment of LLMs](https://arxiv.org/abs/2511.13007)
*Yiyang Zhao,Huiyu Bai,Xuejiao Zhao*

Main category: cs.AI

TL;DR: 提出GEM方法，通过生成式熵引导偏好建模实现低资源和领域特定场景下的大语言模型对齐，无需大量标注数据


<details>
  <summary>Details</summary>
Motivation: 在医学、法律等专业领域，获取大规模偏好标注数据往往不可行，需要开发不依赖大量标注的模型对齐方法

Method: 基于熵理论的认知过滤模块生成多样化推理链，通过token评分机制加权高置信度答案和高熵token；使用自评估群体优势算法SEGA聚合群体认知信号，将熵分数转化为隐式奖励进行策略优化

Result: 在通用基准和领域特定任务（如数学推理和医疗对话）上的实验表明，GEM在少样本偏好数据下取得显著改进
...
### [446] [PragWorld: A Benchmark Evaluating LLMs' Local World Model under Minimal Linguistic Alterations and Conversational Dynamics](https://arxiv.org/abs/2511.13021)
*Sachin Vashistha,Aryan Bibhuti,Atharva Naik,Martin Tutek,Somak Aditya*

Main category: cs.AI

TL;DR: 该研究评估语言模型在对话中编码和更新世界模型的能力，发现模型在语言变化下难以保持准确，提出了解释性框架和微调策略来改善性能。


<details>
  <summary>Details</summary>
Motivation: 理解真实对话中的语用元素需要构建本地世界模型，但语言模型是否能够构建和维护这种隐式表示尚不清楚。

Method: 对流行数据集中的对话应用七种最小语言变化，构建两个是-否问题基准，评估多种语言模型，并提出双视角解释性框架和基于层正则化的微调策略。

Result: 语言模型在语言变化下难以保持稳健准确度，特别是在跟踪实体方面存在困难。解释性框架识别出有用和有害的transformer层。

Conclusion: 语言模型在对话世界模型维护方面存在局限性，提出的微调策略能够抑制有害层的影响，改善模型性能。
...
### [447] [Scaling Generative Verifiers For Natural Language Mathematical Proof Verification And Selection](https://arxiv.org/abs/2511.13027)
...
TL;DR: 论文分析了数学推理验证方法，发现单一基准测试存在局限性，提出了结合GenSelect和LLM-as-a-Judge的验证框架，并揭示了强化学习在证明验证中的局限性。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在数学问题上取得了显著成功，但其推理过程往往存在缺陷。为了推进严谨的基于证明的数学推理，需要可靠的证明验证能力。

Method: 分析了多种评估设置，结合证明推理和最终答案推理来评估模型性能；扩展了两种生成验证方法(GenSelect和LLM-as-a-Judge)到百万token规模；研究了提示选择对LLM-as-a-Judge的影响。

Result: 发现GenSelect和LLM-as-a-Judge的组合是最有效的验证框架；强化学习可以减少对提示的敏感性；但强化学习虽然改进了证明级指标，却未能提高最终答案精度。
...
### [448] [MEGA-GUI: Multi-stage Enhanced Grounding Agents for GUI Elements](https://arxiv.org/abs/2511.13087)
*SeokJoo Kwak,Jihoon Kim,Boyoun Kim,Jung Jae Yoon,Wooseok Jang,Jeonghoon Hong,Jaeho Yang,Yeong-Dae Kwon*

Main category: cs.AI

TL;DR: MEGA-GUI是一个多阶段的GUI定位框架，通过粗粒度ROI选择和细粒度元素定位来解决视觉杂乱和指令模糊问题，在多个基准测试中超越了现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有的GUI定位系统采用单体模型或一次性流水线，缺乏模块化，在视觉杂乱和模糊指令下表现不佳。

Method: 采用多阶段框架，将定位分解为粗粒度ROI选择和细粒度元素定位，使用专门化的视觉语言代理协调，并引入双向ROI缩放算法和上下文感知重写代理。

Result: 在ScreenSpot-Pro基准测试中达到73.18%准确率，在OSWorld-G基准测试中达到68.63%准确率，超越了之前报告的结果。

Conclusion: 模块化结构能够利用不同视觉尺度下视觉语言模型的互补优势，相比单体方法获得更高准确率。
...
### [449] [STEP: Success-Rate-Aware Trajectory-Efficient Policy Optimization](https://arxiv.org/abs/2511.13091)
*Yuhan Chen,Yuxuan Liu,Long Zhang,Pengzhi Gao,Jian Luan,Wei Liu*

Main category: cs.AI

TL;DR: STEP框架通过基于任务成功率的动态采样分配和步骤级优化，解决了多轮交互中轨迹级优化的效率问题，显著提升了样本效率和训练稳定性。


<details>
  <summary>Details</summary>
Motivation: 解决在线强化学习中多轮交互的挑战，轨迹级优化方法效率低下且会产生误导性学习信号，包括任务难度无关的均匀采样、失败轨迹中正确中间动作被惩罚以及高样本收集成本。

Method: 提出STEP框架：1) 维护平滑成功率记录指导自适应轨迹重采样，为困难任务分配更多资源；2) 计算成功率加权优势并将轨迹分解为步骤级样本；3) 应用步骤级GRPO增强来优化低成功率任务的更新。

Result: 在OSWorld和AndroidWorld上的实验表明，STEP相比轨迹级GRPO显著提高了样本效率和训练稳定性，在相同采样预算下收敛更快且泛化能力更好。

Conclusion: STEP通过动态采样分配和步骤级优化有效解决了轨迹级优化的局限性，为多轮交互的在线强化学习提供了更高效的解决方案。
...
### [450] [MM-Telco: Benchmarks and Multimodal Large Language Models for Telecom Applications](https://arxiv.org/abs/2511.13131)
...
TL;DR: MM-Telco是一个为电信领域定制的多模态基准测试套件和模型，旨在解决LLMs在电信应用中的领域特定挑战，包括网络运营、文档质量提升等任务。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在电信领域有巨大潜力，但面临领域特定挑战，需要专门适配来加速其在网络优化、故障排除等方面的应用。

Method: 提出MM-Telco多模态基准测试套件，包含文本和图像任务，涵盖网络运营、管理等实际用例，并对各种LLMs和VLMs进行基线实验。
...
### [451] [Conditional Diffusion Model for Multi-Agent Dynamic Task Decomposition](https://arxiv.org/abs/2511.13137)
*Yanda Zhu,Yuanyang Zhu,Daoyi Dong,Caihua Chen,Chunlin Chen*

Main category: cs.AI

TL;DR: C$	ext{D}^	ext{3}$T是一个新颖的两层分层多智能体强化学习框架，使用条件扩散模型进行动态任务分解，通过预测下一观察和奖励来学习子任务表示，提高复杂协作任务的性能。


<details>
  <summary>Details</summary>
Motivation: 在部分可观测的动态不确定环境中，从零开始学习动态任务分解需要大量训练样本，特别是在探索大的联合动作空间时效率低下。

Method: 提出条件扩散模型动态任务分解框架：高层策略基于子任务效果学习子任务表示；使用条件扩散模型预测下一观察和奖励；低层智能体在分配的子任务中协作学习专业技能；使用多头注意力混合网络增强价值分解。
...
### [452] [InteractiveGNNExplainer: A Visual Analytics Framework for Multi-Faceted Understanding and Probing of Graph Neural Network Predictions](https://arxiv.org/abs/2511.13160)
*TC Singh,Sougata Mukherjea*

Main category: cs.AI

TL;DR: 提出了InteractiveGNNExplainer，一个用于增强图神经网络可解释性的视觉分析框架，通过交互式图编辑和协调视图来支持节点分类任务的深入分析。


<details>
  <summary>Details</summary>
Motivation: 图神经网络在基于图的学习任务中表现出色，但其复杂的非线性操作使其成为不透明的"黑盒"，这阻碍了用户信任、调试、偏见检测以及在需要可解释性的关键领域中的采用。

Method: 开发了InteractiveGNNExplainer框架，集成了协调的交互视图（动态图布局、嵌入投影、特征检查、邻域分析）与后验（GNNExplainer）和内在（GAT注意力）解释技术，并包含交互式图编辑功能以进行"假设"分析。

Result: 通过在Cora和CiteSeer数据集上的案例研究，展示了该系统能够促进深入的错误分类诊断、GCN与GAT行为的比较分析，以及对模型敏感性的严格探测。

Conclusion: 这些功能促进了对GNN预测的更深层次、多方面的理解，有助于实现更透明、可信和稳健的图分析。
...
### [453] [Cost-Effective Communication: An Auction-based Method for Language Agent Interaction](https://arxiv.org/abs/2511.13193)
*Yijia Fan,Jusheng Zhang,Kaitong Cai,Jing Yang,Chengpei Tang,Jian Wang,Keze Wang*

Main category: cs.AI

TL;DR: DALA框架通过将通信带宽作为稀缺资源进行拍卖，让智能体基于消息价值密度竞标发言权，从而解决多智能体系统中通信效率低下的问题。


<details>
  <summary>Details</summary>
Motivation: 传统多智能体系统存在"自由通信"问题，导致指数级token成本和低信噪比，阻碍实际部署。核心问题是缺乏资源理性，忽略了稀缺性原则。

Method: 提出动态拍卖语言智能体(DALA)框架，将智能体间通信建模为中心化拍卖，智能体学习基于预测消息价值密度竞标发言机会。

Result: 在七个推理基准测试中达到新SOTA，包括MMLU 84.32%和HumanEval 91.21% pass@1率，仅使用625万token，远少于现有方法。

Conclusion: DALA通过资源约束培养了战略性沉默的新兴技能，能够动态调整从冗长到沉默的通信策略，实现了高效通信。
...
### [454] [Learning to Solve Resource-Constrained Project Scheduling Problems with Duration Uncertainty using Graph Neural Networks](https://arxiv.org/abs/2511.13214)
...
<details>
  <summary>Details</summary>
Motivation: 实际工业应用中任务持续时间存在不确定性，需要提出具有弹性的调度方案来应对这种不确定性，确保调度方案在不同持续时间场景下都能重复使用。

Method: 结合图神经网络和深度强化学习开发任务调度策略，该策略类似于优先级调度规则，并与串行调度生成方案配对生成调度。
...
### [455] [Informative Communication of Robot Plans](https://arxiv.org/abs/2511.13226)
*Michele Persiani,Thomas Hellstrom*

Main category: cs.AI

TL;DR: 提出一种基于信息增益的机器人计划语言化策略，通过考虑用户的二阶心理理论来优化沟通效果


<details>
  <summary>Details</summary>
Motivation: 现有机器人计划语言化策略（如按计划顺序递增或递减）未充分考虑用户先验知识，导致沟通效果不佳

Method: 通过测量语言化对用户二阶心理理论的信息增益，开发信息化的计划沟通策略

Result: 实验表明该策略比递增或递减策略能更快让用户理解机器人目标

Conclusion: 该研究揭示了机器人计划沟通中什么是信息化的以及为什么信息化的重要性
...
### [456] [Multi-Agent Deep Research: Training Multi-Agent Systems with M-GRPO](https://arxiv.org/abs/2511.13288)
...
<details>
  <summary>Details</summary>
Motivation: 当前多智能体系统使用统一的LLM训练所有智能体，但由于不同智能体底层分布不同，限制了性能。训练具有不同LLM的多智能体系统是下一步发展方向，但这带来了优化挑战。

Method: 提出M-GRPO方法：分层扩展的组相对策略优化，为具有主智能体（规划器）和多个子智能体（多轮工具执行器）的垂直多智能体系统设计。计算主智能体和子智能体的组相对优势，保持分层信用分配；引入轨迹对齐方案生成固定大小的批次；部署解耦训练管道，智能体在独立服务器上运行。

Result: 在真实世界基准测试（GAIA、XBench-DeepSearch、WebWalkerQA）中，M-GRPO始终优于单智能体GRPO和具有冻结子智能体的多智能体GRPO，显示出改进的稳定性和样本效率。
...
### [457] [Dropouts in Confidence: Moral Uncertainty in Human-LLM Alignment](https://arxiv.org/abs/2511.13290)
*Jea Kwon,Luiz Felipe Vecchietti,Sungwon Park,Meeyoung Cha*

Main category: cs.AI

TL;DR: 该研究通过分析32个开源模型在电车问题中的道德决策，发现模型间的不确定性差异大于道德维度间的差异，并通过引入推理时的dropout机制来增加不确定性，从而显著提升了人类-LLM道德对齐。


<details>
  <summary>Details</summary>
Motivation: 随着AI系统越来越多地参与道德决策场景，理解其道德推理和内在不确定性对于构建可靠的AI系统至关重要。人类在道德困境中存在显著不确定性，但机器和AI代理的这种不确定性程度尚未得到充分探索。

Method: 在经典电车问题中分析32个开源模型和9个道德维度的响应，使用二元熵作为不确定性度量，并通过在推理时引入dropout机制来增加模型的不确定性。

Result: 模型间的不确定性差异大于道德维度间的差异；引入dropout机制增加了总熵（主要通过互信息增加），而条件熵基本不变；该机制显著改善了人类-LLM道德对齐，互信息与对齐分数变化呈正相关。

Conclusion: 通过有意调节不确定性和降低LLM在道德复杂场景中的自信度，可以更好地对齐模型生成决策与人类偏好，这突显了在道德推理中管理模型不确定性的潜力。
...
### [458] [Grounded by Experience: Generative Healthcare Prediction Augmented with Hierarchical Agentic Retrieval](https://arxiv.org/abs/2511.13293)
...
<details>
  <summary>Details</summary>
Motivation: LLMs在医疗预测中存在事实不准确问题，传统RAG框架面临两个关键挑战：确定何时需要检索，以及优化检索器与生成器的协同工作。

Method: 提出GHAR框架，采用双代理架构：Agent-Top作为主治医生决定是否检索，Agent-Low作为咨询服务总结相关知识；将两个代理的优化统一在马尔可夫决策过程中。
...
### [459] [DAP: A Discrete-token Autoregressive Planner for Autonomous Driving](https://arxiv.org/abs/2511.13306)
*Bowen Ye,Bin Zhang,Hang Zhao*

Main category: cs.AI

TL;DR: DAP是一个离散令牌自回归规划器，联合预测BEV语义和自车轨迹，通过强化学习微调实现紧凑但可扩展的自动驾驶规划范式。
...
Method: 引入离散令牌自回归规划器，联合预测BEV语义和自车轨迹，结合强化学习微调，在保持监督行为克隆先验的同时注入奖励引导的改进。

Result: 仅使用1.6亿参数，在开环指标上达到最先进性能，在NAVSIM基准测试中提供有竞争力的闭环结果。

Conclusion: 完全离散令牌自回归公式在栅格化BEV和自车动作上运行，为自动驾驶提供了紧凑且可扩展的规划范式。
...
### [460] [Reasoning Shapes Alignment: Investigating Cultural Alignment in Large Reasoning Models with Cultural Norms](https://arxiv.org/abs/2511.13359)
*Yuhang Wang,Yanxu Zhu,Jitao Sang*

Main category: cs.AI

TL;DR: 提出了基于文化规范的文化对齐框架CNCA，利用大模型的推理能力实现与文化规范的对齐。通过三种方法从有限的调查数据中自动挖掘文化规范，并探索了上下文对齐和微调对齐两种范式来利用这些规范。


<details>
  <summary>Details</summary>
Motivation: 大模型不仅需要安全，还需要反映不同文化背景下的人类价值观多样性。现有的安全对齐方法主要关注通用安全政策，但缺乏对文化特定价值观的考虑。

Method: 1) 三种自动挖掘文化规范的方法；2) 上下文对齐：将文化规范显式整合到用户上下文中；3) 微调对齐：通过增强的思维链训练数据内化规范。

Result: 综合实验证明这些方法的有效性，推理能力更强的模型从文化规范挖掘和利用中获益更多。

Conclusion: 推理模型通过文化信息化的对齐策略有潜力更好地反映多样化的人类价值观。
...
### [461] [MedDCR: Learning to Design Agentic Workflows for Medical Coding](https://arxiv.org/abs/2511.13361)
*Jiyang Zheng,Islam Nassar,Thanh Vu,Xu Zhong,Yang Lin,Tongliang Liu,Long Duong,Yuan-Fang Li*

Main category: cs.AI

TL;DR: MedDCR是一个用于医疗编码的闭环框架，将工作流设计视为学习问题，通过设计器、编码器和反射器的协作，自动学习和优化医疗编码工作流程。


<details>
  <summary>Details</summary>
Motivation: 传统医疗编码系统依赖手动设计的工作流程，无法捕捉真实世界文档的细微差别和变异性，需要系统性地学习有效的工作流程。

Method: 采用闭环框架：设计器提出工作流程，编码器执行，反射器评估预测并提供反馈，记忆档案保存先前设计以供重用和迭代改进。

Result: 在基准数据集上，MedDCR优于最先进的基线方法，产生可解释、适应性强的的工作流程，更好地反映实际编码实践。

Conclusion: MedDCR提高了自动化系统的可靠性和可信度，为医疗编码提供了更有效的解决方案。
...
### [462] [Cognitive Maps in Language Models: A Mechanistic Analysis of Spatial Planning](https://arxiv.org/abs/2511.13371)
*Caroline Baumgartner,Eleanor Spens,Neil Burgess,Petru Manescu*

Main category: cs.AI

TL;DR: 该研究通过训练GPT-2模型在三种空间学习范式上，揭示了Transformer学习空间导航的两种不同算法：探索模型发展出类似认知地图的稳健空间表示，而目标导向模型学习路径依赖算法。


<details>
  <summary>Details</summary>
Motivation: 研究大型语言模型如何解决空间导航任务，探索不同训练范式如何影响模型学习到的空间表示和导航策略。

Method: 在网格环境中训练GPT-2模型于三种空间学习范式：被动探索（随机游走预测）、目标导向规划（生成最短路径）和混合模型（在结构化路径基础上用探索数据微调）。

Result: 探索模型发展出类似认知地图的稳健坐标系统，在中间层出现对历史方向标记依赖的急剧转变；目标导向模型保持路径依赖策略；混合模型虽改善泛化但仍保持路径依赖策略。

Conclusion: Transformer的空间智能存在于一个谱系上，从由探索数据塑造的可泛化世界模型到为目标任务优化的启发式方法，训练范式选择显著影响涌现的策略。
...
### [463] [An Operational Kardashev-Style Scale for Autonomous AI - Towards AGI and Superintelligence](https://arxiv.org/abs/2511.13411)
*Przemyslaw Chojecki*

Main category: cs.AI

TL;DR: 提出了一个基于卡达舍夫尺度的自主AI（AAI）分级系统，从固定机器人流程自动化（AAI-0）到完全人工通用智能（AAI-4）及更高等级。该量表是多维且可测试的，包含10个能力轴和复合AAI指数。


<details>
  <summary>Details</summary>
Motivation: 为了提供一个可操作、可测试的AI自主性衡量标准，将"自我改进AI"转化为可证伪的标准，并形式化"婴儿AGI成长为超级智能"的直觉。

Method: 定义10个能力轴（自主性、通用性、规划、记忆/持久性、工具经济、自我修订、社交性/协调、具身性、世界模型保真度、经济吞吐量），通过加权几何平均计算复合AAI指数。引入可测量的自我改进系数κ和两个闭包属性。

Result: 开发了OWA-Bench开放世界代理基准套件，用于评估长期视野、工具使用、持久性代理。通过合成实验展示了当前系统在量表上的映射，并证明了AAI-3代理在满足条件时会随时间成为AAI-5的定理。

Conclusion: 该AAI量表提供了一个系统化、可测试的框架来衡量AI自主性的进展，将模糊的AI发展概念转化为具体的、可验证的标准，并为AI自我改进提供了形式化基础。
...
### [464] [Multi-Agent Multimodal Large Language Model Framework for Automated Interpretation of Fuel Efficiency Analytics in Public Transportation](https://arxiv.org/abs/2511.13476)
*Zhipeng Ma,Ali Rida Bahja,Andreas Burgdorf,André Pomp,Tobias Meisen,Bo Nørregaard Jørgensen,Zheng Grace Ma*

Main category: cs.AI

TL;DR: 本研究提出了一个多智能体框架，利用多模态大语言模型自动生成数据叙述和能源洞察，通过三个专业智能体的协同工作将分析结果转化为面向利益相关者的连贯报告。
...
Method: 采用多智能体框架，包括数据叙述智能体、LLM作为评判智能体和可选的人类评估员，通过迭代过程将分析结果转化为报告。在丹麦北日德兰的公共巴士交通案例中，使用高斯混合模型聚类分析4006次行程的燃油效率数据。

Result: 比较五种最先进LLM和三种提示范式，确定GPT-4.1 mini与思维链提示为最优配置，达到97.3%的叙述准确性，同时平衡可解释性和计算成本。多智能体编排显著提高了基于LLM报告的事实精确性、连贯性和可扩展性。
...
### [465] [FreeAskWorld: An Interactive and Closed-Loop Simulator for Human-Centric Embodied AI](https://arxiv.org/abs/2511.13524)
*Yuhang Peng,Yizhou Pan,Xinning He,Jihaoyu Yang,Xinyu Yin,Han Wang,Xiaoji Zheng,Chao Gao,Jiangtao Gong*

Main category: cs.AI

TL;DR: FreeAskWorld是一个集成了大语言模型的交互式仿真框架，专注于人类中心的社会行为模拟，将经典VLN任务扩展为交互式方向询问设置，并提供了大规模基准数据集。


<details>
  <summary>Details</summary>
Motivation: 随着具身智能成为AI研究核心前沿，仿真平台需要从低级物理交互演进到复杂的人类中心社会行为，以支持更自然的智能体交互。

Method: 集成LLM进行高级行为规划和语义基础交互，基于意图和社会认知理论，构建模块化数据生成管道，支持多样具身任务。

Result: 在FreeAskWorld数据集上微调的模型优于原始模型，实现了增强的语义理解和交互能力，交互本身成为额外的信息模态。

Conclusion: 基于社会基础的仿真框架能有效推进具身AI系统向复杂高级规划和更自然的人机交互发展。
...
### [466] [Automated Construction of Medical Indicator Knowledge Graphs Using Retrieval Augmented Large Language Models](https://arxiv.org/abs/2511.13526)
*Zhengda Wang,Daqian Shi,Jingyi Zhao,Xiaolei Diao,Xiongfeng Tang,Yanguo Qin*

Main category: cs.AI

TL;DR: 提出一个结合检索增强生成(RAG)与大语言模型(LLMs)的自动化框架，用于构建医疗指标知识图谱，以解决当前临床知识图谱依赖人工标注和规则提取的局限性。


<details>
  <summary>Details</summary>
Motivation: 人工智能正在重塑现代医疗，但有效的临床决策支持需要结构化、可互操作的知识格式。当前临床知识图谱严重依赖人工标注和基于规则的提取，受限于医疗指南和文献的复杂性和上下文模糊性。
...
Conclusion: 该框架能够克服当前临床知识图谱构建的挑战，为AI驱动的医疗保健解决方案提供可靠的知识基础。
...
### [467] [Artificial Intelligence-driven Intelligent Wearable Systems: A full-stack Integration from Material Design to Personalized Interaction](https://arxiv.org/abs/2511.13565)
...
<details>
  <summary>Details</summary>
Motivation: 传统智能穿戴设备受限于经验性材料设计和基础信号处理技术，无法适应个体间和个体内变异性，需要更智能的健康管理框架。

Method: 整合多模态传感器网络与边缘-云协同计算，采用数据和知识混合建模方法，结合AI驱动的材料微结构优化、多模态信号鲁棒解释，以及融合群体洞察与个性化适应的双重机制。

Result: HSHI框架能够动态适应个体差异，通过强化学习和数字孪生实现闭环优化，促进定制化干预和反馈。

Conclusion: HSHI代表了医疗保健向预防性、适应性和技术与健康管理和谐关系模式的重要转变。
...
### [468] [CreBench: Human-Aligned Creativity Evaluation from Idea to Process to Product](https://arxiv.org/abs/2511.13626)
...
TL;DR: 提出了CreBench基准和CreExpert模型，用于解决多模态大语言模型理解人类创造力评估的挑战，通过多维度评估基准和指令调优数据集提升模型创造力评估能力。
...
Method: 构建CreBench基准（包含多维度评估）和CreMIT数据集（2.2K多模态数据、79.2K人类反馈、4.7M指令），使用GPT精炼人类反馈，并基于此微调开源MLLMs得到CreExpert模型。
...
### [469] [Beyond Mimicry: Preference Coherence in LLMs](https://arxiv.org/abs/2511.13630)
*Luhan Mikaelson,Derek Shiller,Hayley Clatterbuck*

Main category: cs.AI

TL;DR: 研究发现大多数先进语言模型缺乏统一的偏好结构，在AI特定权衡场景中只有10.4%表现出有意义的偏好一致性，近半数模型显示出不稳定决策模式。
...
Method: 使用逻辑回归和行为分类分析8个最先进模型在48个模型-类别组合中的响应，测试场景强度与选择模式的关系，并通过时间视野操纵检验工具性假设。

Result: 47.9%的组合显示场景强度与选择模式存在显著关系，但只有10.4%表现出有意义的偏好一致性（适应性或基于阈值的行为），54.2%未检测到权衡行为，45.8%显示不稳定转换。

Conclusion: 当前AI系统缺乏统一的偏好结构，在需要复杂价值权衡的部署环境中存在担忧，观察到的模式可由三种决策架构解释：全面权衡系统、选择性触发机制和无稳定决策范式。
