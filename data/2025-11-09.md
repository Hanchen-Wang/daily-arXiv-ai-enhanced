...
TL;DR: LoRA-Edge是一种参数高效微调方法，结合低秩适应和Tensor-Train分解，用于边缘设备上CNN的轻量级微调，显著减少可训练参数数量。


<details>
  <summary>Details</summary>
Motivation: 边缘设备上CNN的完整微调在内存、计算和能耗方面不可行，需要参数高效的微调方法来应对领域偏移问题。

Method: 使用Tensor-Train SVD分解预训练卷积层，选择性地仅更新输出侧核心，并通过零初始化保持辅助路径初始不活跃，最后将更新融合回密集核中。

Result: 在多个HAR数据集和CNN骨干上，LoRA-Edge仅更新最多1.49%的参数，准确率与完整微调相差不超过4.7%，收敛速度提升1.4-3.8倍。
...
TL;DR: SILVI是一个开源标注软件，用于在视频数据中标注行为和交互，为计算机视觉模型训练提供结构化输出，填补了现有工具在交互标注方面的空白。
...
Method: 开发了SILVI软件，集成了行为标注和交互标注功能，允许研究人员直接在视频数据中标注行为和交互。

Result: SILVI能够生成适合训练和验证计算机视觉模型的结构化输出，促进了精细行为分析自动化方法的发展。
...
### [3] [Improving Diagnostic Performance on Small and Imbalanced Datasets Using Class-Based Input Image Composition](https://arxiv.org/abs/2511.03891)
*Hlali Azzeddine,Majid Ben Yakhlef,Soulaiman El Hazzat*

Main category: cs.CV

TL;DR: 提出Class-Based Image Composition方法，通过将同类多张图像融合成Composite Input Images来增强训练数据，显著提升在小样本、类别不平衡数据集上的诊断性能。


<details>
  <summary>Details</summary>
Motivation: 解决小样本、类别不平衡数据集和低质量输入图像导致深度学习模型假阳性率高的问题。

Method: 使用Class-Based Image Composition方法，将同类多张图像融合成3x1布局的Composite Input Images，构建平衡数据集Co-OCTDL。

Result: 在OCTDL数据集上，相比原始数据集，使用VGG16模型获得99.6%准确率、0.995 F1-score和0.9996 AUC，假预测率显著降低。

Conclusion: 该方法能有效提升模型在类别不平衡或小样本数据集上的预测质量，增强对细微疾病模式的区分能力。
...
TL;DR: 该论文研究了在训练过程中使用基础噪声注入技术（高斯、斑点、泊松和椒盐噪声）来提高COVID-19胸部X光检测模型对分布外数据的泛化能力，显著缩小了分布内和分布外评估之间的性能差距。


<details>
  <summary>Details</summary>
Motivation: 深度学习的COVID-19胸部X光检测模型容易学习到源特定的伪影（捷径）而非合理的生物标志物，导致在分布外数据上泛化能力差。

Method: 在训练过程中应用四种基础噪声注入技术：高斯噪声、斑点噪声、泊松噪声和椒盐噪声，以增强模型对分布偏移的鲁棒性。

Result: 噪声注入技术将分布内和分布外评估的性能差距从0.10-0.20显著降低到0.01-0.06（基于AUC、F1、准确率、召回率和特异性的平均值）。

Conclusion: 基础噪声注入是一种有效的方法，可以提高COVID-19检测模型对分布外数据的泛化能力，减少模型对源特定伪影的依赖。
...
<details>
  <summary>Details</summary>
Motivation: 探索模仿学习在X射线引导脊柱手术中的适用性，因为多视图X射线解释复杂，传统方法难以处理。

Method: 开发高真实度仿真环境，收集正确轨迹和双平面X射线序列数据集，训练仅基于视觉信息的模仿学习策略进行规划和开环控制。

Result: 策略在68.5%情况下首次尝试成功，保持安全的椎弓根内轨迹，能泛化到复杂解剖结构（包括骨折），对不同的初始化具有鲁棒性。

Conclusion: 初步结果有前景，但存在入口点精度限制，完全闭环控制需要更频繁的反馈机制，结合更强的先验和领域知识可能为无CT的机器人脊柱导航提供基础。
...
TL;DR: 提出了一种无监督、无需专家标注的医学影像异常检测框架，通过增量扩展正常样本集来提升异常检测性能，无需异常标签或生成模型。


<details>
  <summary>Details</summary>
Motivation: 医学影像中异常检测面临标注稀缺和专家监督成本高的问题，需要开发无需异常标签的无监督方法。

Method: 使用冻结预训练视觉主干网络，添加小型卷积适配器进行快速域适应。通过k-NN异常评分和双重概率门控机制（距离z-score阈值和SWAG不确定性边界）安全地增量扩展正常样本集。

Result: 在多个医学影像数据集上显著提升性能：COVID-CXR的ROC-AUC从0.9489提升至0.9982，Pneumonia CXR从0.6834提升至0.8968，Brain MRI ND-5从0.6041提升至0.7269。

Conclusion: 该框架在标签稀缺的医学影像应用中具有高效性和有效性，能够通过增量学习持续改进正常性概念的定义。
...
TL;DR: 提出了两种互补的时间动作定位方法：边界距离回归(BDR)通过有符号距离回归实现信息理论最优定位，自适应时间细化(ATR)通过连续深度选择实现可微分的计算分配优化。
...
Method: BDR用有符号距离回归替代分类，提供更尖锐的边界峰值；ATR通过连续深度选择τ∈[0,1]实现端到端可微分计算分配，无需强化学习。

Result: 在THUMOS14上，ATR在162G FLOPs下达到56.5% mAP@0.7，相比统一处理的53.6%(198G FLOPs)提升2.9%且减少18%计算量；BDR可提升现有方法1.8-3.1% mAP@0.7。

Conclusion: 该方法通过自适应计算分配和边界回归优化，显著提升了时间动作定位的精度和效率，特别是在边界异质性高的场景下表现更优。
...
TL;DR: 提出了一种统一优化几何和外观的框架，通过高斯引导的网格可微渲染同时优化网格几何和顶点颜色，实现高质量3D重建。
...
TL;DR: 提出了一种基于线性分数变换参数α的光场相机标定方法，能够解耦主镜头和微透镜阵列，包含最小二乘解析解和非线性优化，并提供了特征检测方法。
...
Method: 使用线性分数变换参数α解耦主镜头和微透镜阵列，包含基于最小二乘的解析解和非线性优化，并提出了原始图像特征检测方法。
...
<details>
  <summary>Details</summary>
Motivation: 现有场景重建方法只能恢复可见表面，导致不完整的重建结果。场景的结构元素（如墙壁、地板、天花板）相对容易预测，因为它们通常是平面、重复且简单的。

Method: 创建合成数据集Room Envelopes，提供RGB图像和两个关联的点图：一个捕捉可见表面，另一个捕捉移除家具和装置后的结构布局表面。这为前馈单目几何估计器提供直接监督。

Result: 该方法能够预测第一可见表面和第一布局表面，从而理解场景的完整范围和物体的形状位置。
...
TL;DR: 研究发现3D姿态信息在人类社交互动判断中起关键作用，仅基于面部位置和方向的简单3D社交姿态特征就能达到完整关节位置的预测效果，并能显著提升AI视觉模型的性能。


<details>
  <summary>Details</summary>
Motivation: 人类能快速从视觉输入中提取社交互动信息，但支撑这种能力的计算机制尚不明确，且当前AI视觉系统在社交互动识别方面仍面临挑战。研究者假设人类依赖3D视觉空间姿态信息进行社交判断，而这在大多数AI视觉模型中缺失。

Method: 结合最先进的姿态和深度估计算法提取视频中人物的3D关节位置，比较其与当前AI视觉模型在预测人类社交互动判断方面的能力。推导出一组紧凑的3D社交姿态特征，仅描述视频中面部的3D位置和方向。

Result: 3D关节位置在预测人类社交判断方面优于大多数当前AI视觉模型。简化的3D社交姿态特征与完整关节位置具有相同的预测强度，且与现成AI视觉模型的嵌入结合时能显著提升性能。AI视觉模型中3D社交姿态特征的表示程度预测了模型匹配人类社交判断的能力。

Conclusion: 人类社交场景理解依赖于3D姿态的显式表征，可以通过简单、结构化的视觉空间基元来支持，这为改进AI社交互动识别系统提供了重要启示。
...
TL;DR: CaRF是一个完全可微的3D高斯场分割框架，通过引入相机感知编码和跨视图监督，解决了现有方法在多视角一致性方面的不足。


<details>
  <summary>Details</summary>
Motivation: 现有3D高斯分割方法依赖2D渲染伪监督和视图特定特征学习，导致跨视图一致性差，需要提升3D场景理解的可靠性和一致性。

Method: 提出高斯场相机编码(GFCE)将相机几何融入高斯文本交互，以及训练配对视图监督(ITPVS)来对齐不同视角的高斯对数概率。
...
TL;DR: 提出PhysCorr框架，通过PhysicsRM奖励模型和PhyDPO优化方法提升视频生成的物理一致性，解决现有模型在物体动力学、交互和运动模式上的不真实问题。


<details>
  <summary>Details</summary>
Motivation: 当前文本到视频生成模型在感知质量上取得进展，但生成内容经常违反物理合理性原则，表现为不合理的物体动力学、不连贯的交互和不真实的运动模式，这阻碍了在具身AI、机器人和仿真密集型领域的应用。

Method: 提出PhysCorr统一框架，包括PhysicsRM（首个双维度奖励模型，量化物体内部稳定性和物体间交互）和PhyDPO（新颖的直接偏好优化流程，利用对比反馈和物理感知重加权引导生成物理一致的输出）。

Result: 在多个基准测试上的广泛实验表明，PhysCorr在物理真实性方面取得显著改进，同时保持视觉保真度和语义对齐。

Conclusion: 这项工作向物理基础和可信赖的视频生成迈出了关键一步，提出的方法具有模型无关性和可扩展性，可无缝集成到各种视频扩散和基于transformer的骨干网络中。
...
TL;DR: GNN-MoE：一种基于图神经网络路由器的混合专家框架，通过参数高效微调方法增强ViT在领域泛化中的性能，利用Kronecker适配器和图结构实现轻量级且鲁棒的领域适应。


<details>
  <summary>Details</summary>
Motivation: 领域泛化要求ViT在未见领域上具有鲁棒性能，但标准微调成本高且可能损害泛化能力，需要参数高效且能适应领域变化的微调方法。

Method: 提出GNN-MoE框架，使用混合专家结构和Kronecker适配器进行参数高效微调，创新性地采用图神经网络路由器（GCN、GAT、SAGE）在补丁间图上动态分配补丁到专家，利用补丁间关系实现上下文感知路由。

Result: 在领域泛化基准测试中达到最先进或竞争性性能，同时保持高参数效率。

Conclusion: 基于图的上下文路由对于实现鲁棒、轻量级的领域泛化具有重要价值，GNN-MoE展示了图结构在领域适应中的有效性。
...
TL;DR: 提出了MedDChest，一个专门针对胸部影像优化的基础视觉Transformer模型，通过大规模领域内预训练和领域特定的数据增强策略，在胸部诊断任务上显著优于ImageNet预训练模型。


<details>
  <summary>Details</summary>
Motivation: 解决医学影像中视觉模型性能受限于在自然图像上预训练的主干网络所导致的基本领域差距问题。

Method: 在包含120万张图像的大规模多模态胸部影像数据集上从头预训练ViT模型，并提出引导随机调整裁剪（Guided Random Resized Crops）的内容感知数据增强策略。

Result: MedDChest在多种下游诊断任务上显著优于公开可用的ImageNet预训练模型，证明了大规模领域内预训练结合领域特定数据增强的优越性。

Conclusion: MedDChest提供了一个强大且鲁棒的特征提取器，可作为各种胸部诊断任务的更好起点，模型权重将公开以促进未来研究。
...
Method: 开发稀疏体素化表示方法，保留锐利边缘和内部结构；设计双模式自编码器，实现可扩展且细节保持的形状重建。
...
<details>
  <summary>Details</summary>
Motivation: PPG信号因其无创采集、活体检测能力和低成本可穿戴设备适用性在生物认证中受到关注，但面临运动伪影、光照变化和个体生理差异的挑战。

Method: 对原始PPG信号进行预处理后，通过连续小波变换转换为时频标量图，然后使用CVT-ConvMixer-LSTM混合模型结合空间和时间特征。
...
Method: 1. 将视觉刺激解耦为层次化视觉和语言语义组件；2. 将视觉和大脑表示投影到共享潜在空间；3. 引入不确定性感知模块进行加权对齐；4. 使用可学习的语言锚定语义矩阵增强跨模态相关性；5. 采用单模态预训练后多模态微调的两阶段训练策略。
...
TL;DR: 比较了两种无配对自监督CT图像去噪方法：基于CycleGAN的残差翻译器和Noise2Score分数匹配去噪器。CycleGAN在最终图像质量上表现最佳，而Noise2Score在无配对情况下提供稳健替代方案。


<details>
  <summary>Details</summary>
Motivation: 研究在无配对和自监督条件下进行CT图像去噪，评估两种训练数据效率高的方法，为缺乏干净配对数据的情况提供解决方案。

Method: 使用CycleGAN残差翻译器和Noise2Score分数匹配去噪器两种方法，通过配置扫描确定最佳参数设置，并进行收敛训练。

Result: CycleGAN将输入从34.66 dB/0.9234 SSIM提升到38.913 dB/0.971 SSIM，得分1.9441；Noise2Score在噪声较大输入上表现良好，绝对PSNR/SSIM略低但仍有竞争力。

Conclusion: CycleGAN提供最强的最终图像质量，Noise2Score在无配对情况下提供稳健替代方案，两者在不同场景下各有优势。
...
TL;DR: UKAST是一种结合Kolmogorov-Arnold Networks的U-Net架构，在Swin Transformer编码器中集成有理函数基的KANs，解决了传统CNN和Transformer在医学图像分割中的局限性，实现了更高的数据效率和更好的性能。
...
Method: 提出UKAST架构，将基于有理函数的KANs集成到Swin Transformer编码器中，使用Group Rational KANs替代传统样条基KANs，提高表达能力和数据效率。

Result: 在4个不同的2D和3D医学图像分割基准测试中达到最先进性能，超越CNN和Transformer基线，在数据稀缺场景下表现尤为突出，FLOPs减少且参数仅小幅增加。
...
<details>
  <summary>Details</summary>
Motivation: 当前文本到图像合成方法在对象定位控制方面存在不足，未能充分利用位置信息，导致对对象空间布局理解不够充分。

Method: 提出SpatialLock框架，包含两个组件：位置参与注入(PoI)通过注意力层直接整合空间信息；位置引导学习(PoG)使用基于感知的监督进一步优化对象定位。

Result: 实验表明SpatialLock在精确对象定位方面达到了新的最先进水平，在多个数据集上获得了超过0.9的IOU分数。
...
TL;DR: THG是一种无需训练的策略，通过将CFG ODE重新表述为多速率ODE系统，显著加速扩散采样同时保持高保真生成。该方法将噪声估计和额外引导项分别在不同时间步网格上积分，减少计算量30%且几乎不损失生成质量。


<details>
  <summary>Details</summary>
Motivation: 传统扩散求解器未能充分利用额外引导项对数值误差的鲁棒性，存在大量计算冗余。通过分析发现额外引导分支对近似更稳健，这为加速采样提供了机会。

Method: 将CFG ODE重新表述为多速率ODE系统，噪声估计在细粒度时间步网格上积分（乌龟方程），额外引导项仅在粗网格上积分（野兔方程）。还引入了误差边界感知时间步采样器和引导尺度调度器。

Result: 在相同计算预算下，THG将函数评估次数减少高达30%，生成保真度几乎无损失（ΔImageReward ≤ 0.032），优于现有最先进的基于CFG的无训练加速器。
...
Method: 使用扩散模型框架，通过线性平滑将参考特征作为辅助信息，采用风格-内容引导机制减少内容泄漏，并通过联合AdaIN模块支持多风格生成。

Result: 实验表明该方法能实现高质量的草图生成，具有准确的对齐风格和更好的风格控制灵活性。

Conclusion: M3S框架在草图风格控制方面表现出色，为精确的草图风格生成提供了有效解决方案。
...
TL;DR: 开发了一个完整的自动化网球比赛分析系统，集成了多个深度学习模型来实时检测和跟踪球员、网球以及识别球场关键点，提供详细的比赛分析数据。


<details>
  <summary>Details</summary>
Motivation: 为教练、广播公司和球员提供可操作的比赛洞察，通过自动化分析系统来理解比赛动态，包括球员移动模式、球速、击球准确性和球员反应时间等。

Method: 使用YOLOv8进行球员检测，自定义训练的YOLOv5模型进行球跟踪，以及基于ResNet50的架构进行球场关键点检测，构建了一个完整的分析流程。

Result: 实验结果表明，该系统在不同球场条件和比赛场景下表现出稳健的性能，能够输出带注释的视频和详细的性能指标。

Conclusion: 该自动化网球比赛分析系统为比赛分析提供了有效的解决方案，能够生成有价值的分析数据，帮助各方更好地理解比赛动态。
...
<details>
  <summary>Details</summary>
Motivation: 复杂海洋环境导致相机运动和视觉退化，给多目标跟踪带来挑战，需要提高船舶导航安全和海事监视效果。

Method: 采用并行跟踪器架构，包含目标检测与重识别分支，以及动态相机运动估计分支。集成了可逆柱状检测网络、轻量级Transformer外观提取器，并通过投影变换解耦平台和目标运动。

Result: 在新加坡海事数据集上的评估显示，DMSORT实现了最先进的性能，在基于ReID的MOT框架中运行速度最快，同时保持高身份一致性和对抖动、遮挡的鲁棒性。

Conclusion: DMSORT方法在复杂海洋环境中有效解决了多目标跟踪问题，具有高效性和鲁棒性，为海事应用提供了可靠的解决方案。
...
TL;DR: 该论文提出了一个框架，让计算机使用代理能够从在线视频教程中学习，通过检索、过滤视频，将其转换为结构化演示轨迹，并在执行时动态选择轨迹作为上下文指导，从而提升代理性能。


<details>
  <summary>Details</summary>
Motivation: 当前计算机使用代理在执行需要特定应用程序、平台和多步骤工作流程的领域知识任务时，仍落后于人类用户。人类可以通过观看视频教程来弥补这一差距，但现有代理缺乏这种能力。

Method: 使用视觉语言模型推断UI操作，将视频分割为短操作子序列并为每个子序列分配文本目标。在执行时采用两阶段选择机制动态选择单个轨迹作为上下文指导。

Result: 在两个广泛使用的基准测试上的实验表明，该框架始终优于仅使用文本教程或转录的强基线代理和变体。

Conclusion: 在线视频可以系统地提炼为可操作的指导，通过轨迹分割和选择、操作过滤以及视觉信息的重要性，显著提高计算机使用代理在执行时的性能。
...
<details>
  <summary>Details</summary>
Motivation: 在文档理解中，确定扫描或拍摄文档的正确方向是关键的预处理步骤，因为旋转错误会严重影响OCR等下游任务的性能，特别是在用户拍摄时相机方向错误的情况下。

Method: 构建了ORB基准测试（包含英文和11种印度语言的子集），并开发了一个基于Phi-3.5-Vision视觉编码器的快速、鲁棒、轻量级旋转分类管道，采用动态图像裁剪技术，专门针对4类旋转任务进行微调。

Result: 旋转分类方法在两个数据集上分别达到96%和92%的准确率；在模拟真实世界设置中，该模块显著提升了闭源OCR模型（最高14%）和开源权重模型（最高4倍）的性能。

Conclusion: 该研究证明了旋转校正模块在提升OCR性能中的关键作用，提出的轻量级方法在多种语言和模型上都表现出色，为文档理解预处理提供了有效的解决方案。
...
Method: 使用20对组织样本，应用多种预处理步骤（包括CycleGAN、Macenko、Reinhard、Vahadane颜色变换、反转、对比度调整、强度归一化和去噪），采用VALIS配准方法进行刚性和非刚性配准，评估相对目标配准误差。

Result: 在两种配准场景中，CycleGAN颜色变换均实现了最低的配准误差，其他方法显示出较高的误差。

Conclusion: 配准前应用颜色变换可改善不同模态图像之间的对齐效果，支持数字病理学中更可靠的分析。
...
TL;DR: 该论文研究了协方差描述符在医学图像分类中的应用，特别是结合预训练视觉编码器和专门为对称正定矩阵设计的SPDNet网络，在MedMNIST基准测试中取得了优于传统方法和手工特征的结果。


<details>
  <summary>Details</summary>
Motivation: 协方差描述符在通用计算机视觉任务中表现出色，但在医学图像分析中尚未充分探索。研究者希望验证其在医学图像分类中的有效性，特别是结合现代预训练视觉编码器。

Method: 使用预训练通用视觉编码器（DINOv2和MedSAM）提取特征构建协方差描述符，并与手工特征进行比较。采用专门为对称正定矩阵设计的SPDNet分类网络，在MedMNIST基准的11个数据集上进行评估。

Result: 基于GVE特征的协方差描述符始终优于基于手工特征的描述符。特别是SPDNet与DINOv2特征结合时，性能优于最先进方法。

Conclusion: 将协方差描述符与强大的预训练视觉编码器结合，在医学图像分析中具有巨大潜力，为医学图像分类提供了新的有效方法。
...
TL;DR: 提出了一种用于人体运动风格迁移的新方法AStF，通过引入偏度和峰度等更高阶统计量来更全面地捕捉运动数据的动态模式，相比传统仅使用均值和方差的方法有显著改进。
...
Result: 实验结果表明，通过更全面地建模动态风格的时空统计模式，AStF在运动风格迁移任务中表现出优于现有最先进方法的性能。

Conclusion: 引入偏度和峰度等更高阶统计量能够更有效地捕捉运动风格特征，为运动风格迁移提供了更全面的统计建模方法。
...
TL;DR: 该论文提出MedSapiens模型，通过将人类姿态估计基础模型Sapiens适配到医学影像中的解剖标志点检测任务，在多个数据集上建立了新的最优性能，并在少样本设置下表现出色。


<details>
  <summary>Details</summary>
Motivation: 传统解剖标志点检测依赖领域特定模型，而大规模预训练视觉模型的出现提供了新机遇。作者旨在探索人类中心基础模型在医学影像中的潜力，这些模型天生优化了空间姿态定位，但这一潜力尚未被充分挖掘。

Method: 通过多数据集预训练将人类姿态估计基础模型Sapiens适配到医学影像领域，创建MedSapiens模型。该方法利用了人类中心基础模型对空间定位的强先验知识。

Result: MedSapiens在多个数据集上实现了新的最优性能：相比通用模型平均成功检测率提升5.26%，相比专业模型提升21.81%。在少样本设置下，相比现有少样本最优方法提升2.69%。

Conclusion: 人类中心基础模型为医学影像中的解剖标志点检测提供了强大的空间定位先验，这一潜力值得进一步探索。MedSapiens证明了这种适配策略的有效性。
...
TL;DR: Proto-LeakNet是一种基于信号泄漏的深度伪造图像溯源框架，通过在扩散模型潜在空间中检测生成器特定的统计痕迹，实现闭集分类和开集评估，无需重新训练即可分析未知生成器。
...
Method: 在扩散模型的潜在域中操作，通过部分前向扩散重模拟来暴露生成器特定的残余线索。使用时序注意力编码器聚合多步潜在特征，特征加权原型头结构化嵌入空间并实现透明归属。

Result: 仅在闭集数据上训练，达到98.13%的宏AUC，学习的潜在几何结构在后处理下保持稳健，超越最先进方法，在已知和未知生成器之间实现强可分离性。
...
Result: 在HITSZ-VCM和BUPT数据集上的广泛实验表明，该方法显著优于现有最先进方法。
...
TL;DR: FastGS是一个新颖的3D高斯溅射加速框架，通过基于多视角一致性的密度化和修剪策略，解决了训练时间与渲染质量之间的权衡，实现了显著的训练加速。


<details>
  <summary>Details</summary>
Motivation: 现有的3D高斯溅射加速方法在训练过程中未能有效调节高斯数量，导致冗余计算时间开销。

Method: 提出基于多视角一致性的密度化和修剪策略，摒弃预算机制，充分考虑每个高斯基元的重要性。

Result: 在多个数据集上显著优于最先进方法，在Mip-NeRF 360数据集上实现3.32倍训练加速，在Deep Blending数据集上实现15.45倍加速，并在各种任务中展现2-7倍加速。

Conclusion: FastGS展示了强大的通用性，能够为动态场景重建、表面重建、稀疏视图重建、大规模重建和SLAM等多种任务提供显著的训练加速。
...
TL;DR: 该研究通过自监督学习在大型农业数据集上训练领域特定的视觉基础模型，显著提升了除草剂试验中植物物种识别和损伤分类的准确性，特别是在未见过的环境和无人机图像等域转移场景下表现更佳，同时大幅减少了标注需求。


<details>
  <summary>Details</summary>
Motivation: 通用视觉基础模型在农业领域表现有限，无法准确区分物种和损伤类型的细微差异，而除草剂田间试验需要精确的物种识别和损伤评估。

Method: 采用自监督学习方法，在大型精选农业数据集上训练领域特定的视觉基础模型，学习针对除草剂试验图像的丰富可迁移表示。

Result: 领域特定模型在物种识别（F1从0.91提升至0.94）和损伤分类（从0.26提升至0.33）上显著优于通用模型；在未见条件下提升更大（物种识别从0.56到0.66，损伤分类从0.17到0.27）；在无人机图像等域转移场景下保持强性能（物种分类从0.49到0.60）。
...
TL;DR: 本研究通过结合合成和真实Sentinel-1卫星图像训练YOLOv10模型，评估合成数据在提升海上基础设施检测性能中的作用，并在未训练区域验证模型的地理迁移能力。


<details>
  <summary>Details</summary>
Motivation: 海上基础设施快速扩张需要有效监测系统，但现有模型在样本稀缺时性能不足，特别是对于代表性不足的对象类别、形状和尺寸。

Method: 使用合成和真实Sentinel-1卫星图像训练YOLOv10目标检测模型，在四个区域（里海、南海、几内亚湾、巴西海岸）进行训练，并在三个未见区域（墨西哥湾、北海、波斯湾）评估地理迁移性。

Result: 共检测到3,529个海上平台，F1分数从0.85提升至0.90，模型在未训练区域表现良好，证明了地理迁移能力。

Conclusion: 合成数据能有效改善不平衡类别表示和模型性能，为实现全球可迁移的海上基础设施检测迈出重要一步，展示了深度学习在全球规模监测中的潜力。
...
TL;DR: RISE-T2V是一个创新的文本到视频生成框架，通过集成提示重述和语义特征提取，解决了现有模型在简短提示下视频质量下降的问题，显著提升了视频生成与用户意图的对齐能力。


<details>
  <summary>Details</summary>
Motivation: 现有文本到视频扩散模型依赖预训练文本编码器进行语义对齐，但在处理简洁提示时视频质量下降严重，且无法在线重述提示以更好地匹配用户意图，限制了模型的可扩展性和可用性。

Method: 提出RISE-T2V框架，创新性地将提示重述和语义特征提取集成到单一流程中。通过Rephrasing Adapter模块，利用LLM在下一个token预测时的文本隐藏状态作为视频生成条件，使模型能够隐式地将基础提示重述为更全面的表示。

Result: 大量实验证明RISE-T2V是一个通用框架，适用于不同的视频扩散模型架构，显著提升了文本到视频模型生成高质量、符合用户意图视频的能力。

Conclusion: RISE-T2V通过集成提示重述和语义特征提取，有效解决了现有文本到视频模型在语义理解方面的局限性，为视频生成任务提供了更强大的能力。
...
TL;DR: 提出了一种基于体素稀疏化和子流形稀疏卷积网络的两阶段方法，用于医学影像中的肿瘤分割，在保持高分辨率输入的同时显著降低计算资源需求。


<details>
  <summary>Details</summary>
Motivation: 医学影像中的肿瘤分割是专业且耗时的任务，传统方法受限于3D扫描的计算复杂度，通常需要降采样或使用图像块，这限制了在临床环境中的定量分析应用。
...
Conclusion: 该方法在保持高精度分割的同时，显著降低了计算资源需求，为临床环境中的自动化肿瘤分割提供了可行的解决方案。
...
TL;DR: 对VOC 2008数据集中的马和摩托车进行二元分类的九种CNN架构评估，采用少数类增强技术解决类别不平衡问题，ConvNeXt-Tiny在各项指标中表现最佳。
...
Method: 使用九种卷积神经网络架构（包括ResNet-50、ConvNeXt-Tiny、DenseNet-121和Vision Transformer等），在VOC 2008数据集上实施少数类数据增强技术进行二元分类。

Result: ConvNeXt-Tiny表现最佳，马检测AP达95.53%，摩托车检测AP达89.12%；数据增强显著改善了少数类检测性能，尤其对深层架构帮助更大。

Conclusion: 为不平衡二元分类任务提供了架构选择指导，量化了数据增强策略在缓解类别不平衡问题中的重要作用。
...
TL;DR: 该研究分析了传感器遮挡对基于BEV融合的3D物体检测性能的影响，发现在nuScenes数据集上，相机遮挡导致41.3%的mAP下降，而LiDAR仅在重度遮挡时性能下降47.3%。融合设置中，模型更依赖LiDAR数据。


<details>
  <summary>Details</summary>
Motivation: 虽然BEV融合架构通过多模态集成展示了强大的性能，但环境条件（如雾、霾或物理障碍）导致的传感器遮挡对3D检测精度的影响尚未充分探索。

Method: 使用BEVFusion架构在nuScenes数据集上评估相机和LiDAR遮挡对3D检测性能的影响，使用mAP和NDS作为评估指标。

Result: 仅相机检测时，中度遮挡导致mAP下降41.3%；仅LiDAR检测时，重度遮挡导致mAP下降47.3%；融合设置中，相机遮挡仅导致4.1%下降，而LiDAR遮挡导致26.8%下降。

Conclusion: 模型在3D物体检测任务中更依赖LiDAR数据，未来需要研究遮挡感知的评估方法和改进的传感器融合技术，以在部分传感器失效或环境条件恶劣时保持检测精度。
...
<details>
  <summary>Details</summary>
Motivation: 分析化学中成像技术广泛应用，但传统化学计量学方法难以高效提取和分析空间信息。深度学习虽能增强图像处理能力，但由于缺乏结构化实施指导，在分析化学中的应用仍然有限。

Method: 提供MATLAB代码教程演示，展示处理分析化学中常见成像模式数据的方法，指导读者使用现有开源深度学习模型提取深度特征。

Result: 开发了完整的教程框架，能够处理多种成像模式数据，并将空间信息与其他数据源（如光谱信息）进行整合。

Conclusion: 本教程成功填补了分析化学领域深度学习应用的指导空白，为研究人员提供了实用的工具和方法来提取和利用成像数据中的空间信息。
...
<details>
  <summary>Details</summary>
Motivation: 解决医学视觉问答中需要同时提供准确答案、解释性推理和视觉定位的需求，通过多任务学习提升模型的综合能力。
...
Conclusion: 多任务框架能够联合学习视觉定位、推理和解释，生成既准确又可解释的响应，为医学视觉问答提供了有效的解决方案。
...
Result: 在NVIDIA Jetson Orin上部署，运行效率达50.7FPS；在多个挑战性数据集上显著优于之前的轻量级模型。
...
<details>
  <summary>Details</summary>
Motivation: 为了解决视觉对象建模和表示学习在不同尺度下的碎片化问题，提供一个统一的平台来整合数据集、模型和训练技术，加速研究和实际应用之间的转化。

Method: 使用YAML驱动的单一工作流覆盖分类、检索和度量学习；通过timm兼容接口提供1000多个预训练骨干网络；包含模块化损失函数、数据增强和分布式训练工具。

Result: 在ImageNet-1K、MS-Celeb-1M和Stanford online products等基准数据集上达到或超过了参考结果；支持一键导出到ONNX或HuggingFace格式。

Conclusion: DORAEMON通过整合数据集、模型和训练技术，为视觉识别和表示学习提供了一个可扩展的基础平台，能够高效地将研究成果转化为实际应用。
...
<details>
  <summary>Details</summary>
Motivation: 由于章鱼的伪装能力、皮肤纹理颜色快速变化、非刚性身体变形和频繁遮挡，加上水下光照和浑浊度变化，在自然栖息地分析章鱼具有挑战性，且缺乏大规模标注数据集。

Method: 集成SAM2与自定义训练的YOLOv11对象检测器。用户提供点坐标生成初始分割掩码，然后训练YOLO模型，最终实现全自动管道，通过边界框提示SAM2，无需进一步人工干预。引入两个无监督指标：时间一致性DICE_t和新组件计数NC_t。

Result: HideAndSeg表现满意，相比手动提示方法减少了分割噪声。在自然环境中即使完全遮挡后也能重新识别和分割章鱼，而手动提示模型在此场景下失败。
...
TL;DR: 该论文提出了首个针对凸多边形拼图的计算求解器，扩展了传统仅限于方形拼图的求解方法，结合几何和图像兼容性开发了贪婪求解器，并建立了首个此类拼图的基准数据集。


<details>
  <summary>Details</summary>
Motivation: 现有自动拼图求解器主要针对方形拼图，严重限制了实际应用范围。该研究旨在显著扩展计算处理的拼图类型，专注于凸多边形拼图这一多边形拼图的主要子集。

Method: 结合几何和图像兼容性，引入贪婪求解器来处理凸多边形拼图问题。

Result: 开发了首个凸多边形拼图求解器，报告了多种性能指标，并建立了此类拼图的第一个基准数据集。

Conclusion: 该工作显著扩展了计算拼图求解的范围，为凸多边形拼图提供了有效的求解方法和基准评估标准。
...
TL;DR: V-Thinker是一个通用多模态推理助手，通过端到端强化学习实现视觉中心的交互式思考，在视觉交互推理任务上显著优于现有LMM基线模型。


<details>
  <summary>Details</summary>
Motivation: 解决大型多模态模型在图像交互与长程推理能力深度整合方面的挑战，突破现有视觉工具空间有限和任务特定工作流设计的限制。

Method: 包含两个关键组件：1）数据进化飞轮，自动合成、进化和验证交互式推理数据集；2）视觉渐进训练课程，先通过点级监督对齐感知，再通过两阶段强化学习框架整合交互推理。

Result: 在VTBench基准测试中，V-Thinker在通用和交互推理场景中一致优于强大的LMM基线模型。
...
<details>
  <summary>Details</summary>
Motivation: 传统“Thinking with Text”和“Thinking with Images”范式存在局限性：(1)图像只能捕捉单一时刻，无法表示动态过程；(2)文本和视觉作为独立模态分离，阻碍统一多模态理解与生成。

Method: 引入“Thinking with Video”范式，使用Sora-2等视频生成模型，并开发VideoThinkBench基准测试，包含视觉中心任务和文本中心任务。

Result: Sora-2在视觉中心任务上与SOTA VLMs相当甚至超越，在Eyeballing Games等任务上表现更优；在文本中心任务上，MATH准确率达92%，MMMU达75.53%。自一致性和上下文学习可进一步提升性能。

Conclusion: 视频生成模型是潜在的统一多模态理解与生成模型，“thinking with video”可作为统一多模态推理范式。
...
TL;DR: 提出了一个三轴分析框架（传感器、标签、域）来适应地理空间基础模型（GeoFMs），用于滑坡测绘，在多个实验中优于传统CNN、ViT和其他GeoFMs。


<details>
  <summary>Details</summary>
Motivation: 滑坡对生命、基础设施和环境造成严重损害，传统深度学习模型在不同传感器、区域或有限训练数据条件下表现不佳，需要更稳健的滑坡测绘方法。

Method: 基于Prithvi-EO-2.0地理空间基础模型，采用全局预训练、自监督学习和适应性微调的三轴分析框架，处理传感器、标签和域的适应性问题。

Result: 模型在光谱变化下保持稳健，在标签稀缺时维持准确性，在不同数据集和地理环境中具有更好的泛化能力，优于U-Net、Segformer等传统模型。
...
TL;DR: 提出一个新的评估框架，包含8个指标来评估说话头部视频生成的质量、自然性和同步性，解决了现有评估指标不足的问题。
...
Method: 设计了包含三个维度（质量、自然性、同步性）的8个评估指标，强调效率和与人类偏好的一致性，分析头部、嘴巴、眉毛的细粒度动态以及面部质量。
...
<details>
  <summary>Details</summary>
Motivation: 准确评估腹腔镜胆囊切除术的手术复杂度至关重要，严重炎症与更长手术时间和更高并发症风险相关。Parkland分级量表提供了临床验证的炎症严重程度分层框架，但其在手术视频中的自动化应用尚未充分探索。

Method: STC-Net是一个基于单时间戳的复杂度估计框架，包含定位、窗口提议和分级模块。它通过结合硬和软定位目标以及背景感知分级监督的新颖损失函数，直接在完整视频上运行。

Result: 在1,859个LC视频的私有数据集上评估，STC-Net达到62.11%的准确率和61.42%的F1分数，比非定位基线在两个指标上都提高了10%以上。

Conclusion: STC-Net展示了从完整LC视频中基于PGS自动评估手术复杂度的可扩展有效方法，有望用于术后分析和手术培训。
...
<details>
  <summary>Details</summary>
Motivation: 现有的前馈3D重建方法在处理稀疏、非重叠相机视图和复杂场景动态时存在困难，需要一种能够有效整合空间视图和时间帧信息的鲁棒动态场景重建方法。
...
TL;DR: PixCLIP是一个新颖的框架，通过同时处理视觉提示输入和长文本描述来解决CLIP模型在细粒度图像-文本对齐方面的局限性，在像素级交互和长文本处理方面取得了突破性成果。


<details>
  <summary>Details</summary>
Motivation: CLIP模型在细粒度图像-文本对齐方面存在局限，特别是文本编码器的token长度限制阻碍了处理更细粒度文本信息的能力。现有方法主要关注增强视觉信息处理的粒度，但缺乏对文本内容处理的同步改进。

Method: 1) 建立自动化标注流水线生成像素级定位的长文本描述，构建包含近150万样本的LongGRIT数据集；2) 用LLM替换CLIP的原始文本编码器；3) 提出三分支像素-文本对齐学习框架，实现任意粒度的图像区域与对应文本描述的细粒度对齐。

Result: PixCLIP在像素级交互和长文本处理方面展示了突破性成果，实现了最先进的性能表现。

Conclusion: 通过同时增强视觉和文本内容处理的粒度，PixCLIP有效解决了CLIP在细粒度图像-文本对齐方面的局限性，为细粒度视觉语言理解提供了新的解决方案。
...
TL;DR: 提出了一种自动化的虚拟IHC染色图像质量评估框架，使用颜色反卷积生成IHC阳性像素掩码，通过Dice、IoU等指标直接量化染色准确性，发现传统图像保真度指标与染色准确性相关性差。


<details>
  <summary>Details</summary>
Motivation: 传统纹理和分布指标只能评估图像保真度，无法准确量化IHC染色的准确性，需要开发基于染色准确性的评估方法。

Method: 使用颜色反卷积生成真实和虚拟IHC的染色阳性像素掩码，计算Dice系数、IoU和Hausdorff距离等染色准确性指标，无需专家手动标注。

Result: 传统指标（FID、PSNR、SSIM）与染色准确性和病理学家评估相关性差；配对模型（PyramidPix2Pix、AdaptiveNCE）染色准确性最高；非配对扩散和GAN模型可靠性较低；全玻片图像评估显示补丁级评估无法发现的性能下降。
...
TL;DR: 提出了一种可扩展的流式视频质量评估模型，该模型无需参考视频且不依赖人工标注，通过时间感知卷积架构预测全参考指标。


<details>
  <summary>Details</summary>
Motivation: 现有视频质量评估方法存在局限：全参考方法需要干净参考视频，大多数无参考方法依赖昂贵的人工标注，且多数无监督方法基于图像而忽略时间上下文。

Method: 利用DAVIS数据集的合成退化，训练时间感知卷积架构直接从退化视频预测全参考指标（LPIPS、PSNR、SSIM），推理时无需参考视频。

Result: 流式方法优于图像基线，能泛化到多种退化类型；与BRISQUE相比，与全参考指标相关性更高。

Conclusion: 时间建模对于现实视觉系统中可扩展的视频质量评估具有重要价值，提出的方法在无需参考和人工标注的情况下有效评估视频质量。
...
TL;DR: 偏振分辨近红外成像通过测量眼组织反射光的偏振状态，为眼动追踪提供了额外的光学对比机制。偏振眼动追踪系统使用偏振滤光片阵列相机和线性偏振近红外光源，能在巩膜上显示可追踪特征和在角膜上显示注视信息模式。


<details>
  <summary>Details</summary>
Motivation: 传统基于强度的眼动追踪系统在某些条件下表现受限，偏振成像可以提供额外的对比机制，提高眼动追踪的准确性和鲁棒性。

Method: 使用偏振滤光片阵列相机配合线性偏振近红外光源，通过卷积神经网络对346名参与者的数据进行训练，开发偏振眼动追踪系统。

Result: 在正常条件和存在眼睑遮挡、眼距变化、瞳孔大小变化的情况下，偏振眼动追踪系统将中位95百分位绝对注视误差降低了10-16%。

Conclusion: 偏振眼动追踪系统将光-组织偏振效应与人机交互的实际增益联系起来，为未来可穿戴设备提供了一种简单、鲁棒的传感模式。
...
TL;DR: 本文提出了一种诊断和去偏多模态基准测试的方法，通过测试集压力测试和迭代偏置剪枝来识别和消除非视觉偏差，确保基准测试真正评估模型的视觉理解能力。
...
Method: 提出测试集压力测试方法，通过仅使用测试集的非视觉文本输入对大型语言模型进行k折交叉验证来揭示捷径性能；同时使用基于随机森林的轻量级诊断方法进行快速审计；采用迭代偏置剪枝程序过滤高偏差样本。

Result: 在四个基准测试（VSI-Bench、CV-Bench、MMMU和VideoMME）中发现了普遍存在的非视觉偏差；通过完整框架创建了VSI-Bench-Debiased，显著降低了非视觉可解性，扩大了视觉盲性能差距。

Conclusion: 基准测试设计应采用诊断原则，主动识别和消除非视觉偏差，确保评估的是模型的真实视觉理解能力而非利用测试集模式的能力。
...
TL;DR: SIMS-V是一个利用3D模拟器生成空间丰富视频训练数据的框架，通过系统消融实验发现三种关键问题类型（度量测量、视角依赖推理、时间追踪）能最有效地发展可迁移的空间智能，仅用25K模拟样本训练的7B模型就能超越72B基线并在真实世界空间推理基准上取得竞争性表现。
...
Result: 识别出三种最有效的问题类别（度量测量、视角依赖推理、时间追踪），仅用25K模拟样本训练的7B参数视频LLM超越了72B基线，在真实世界空间推理基准上达到与专有模型竞争的性能，同时保持通用视频理解能力。

Conclusion: 模拟数据可以高效地发展可迁移的空间智能，精心选择的问题类型比全面覆盖更有效，该方法在保持通用能力的同时显著提升了具身和真实世界空间任务的性能。
...
TL;DR: 本文提出了从任务驱动系统向空间超感知的范式转变，主张通过语义感知、流式事件认知、隐式3D空间认知和预测性世界建模四个阶段实现真正的多模态智能。


<details>
  <summary>Details</summary>
Motivation: 当前多模态系统主要停留在反应式、任务驱动的层面，缺乏对空间认知和世界建模的深入理解，限制了真正的多模态智能发展。

Method: 提出了VSI-SUPER基准测试（包括VSR和VSC任务），并训练了Cambrian-S模型。同时提出了预测性感知方法，使用自监督的下一潜在帧预测器来驱动记忆和事件分割。

Result: 通过数据扩展（VSI-590K）和模型训练，在VSI-Bench上取得了30%的绝对提升，但在VSI-SUPER上表现仍然有限。预测性感知方法显著优于领先的专有基线模型。

Conclusion: 仅靠规模扩展不足以实现空间超感知，需要能够预见、选择和组织经验的模型，预测性感知是实现这一目标的有效路径。
...
Result: 在VBench上获得83.74分，大幅超越所有自回归模型，甚至超过HunyuanVideo等扩散模型；无需额外优化即可生成5秒720p视频，速度比领先扩散方法快10倍。

Conclusion: InfinityStar是首个能够生成工业级720p视频的离散自回归视频生成器，代码和模型已发布以促进高效高质量视频生成研究。
...
TL;DR: 提出了Track Any State任务，用于跟踪物体在状态转换过程中的变化，并开发了TubeletGraph零样本系统来解决物体在显著外观变化后丢失跟踪的问题。


<details>
  <summary>Details</summary>
Motivation: 现实世界中的物体经常经历状态转换（如苹果被切成块、蝴蝶破茧而出），现有方法在物体外观发生显著变化后容易丢失跟踪目标。
...
Conclusion: 该方法能够有效跟踪物体通过转换的过程，检测和描述状态变化，为解决物体在显著外观变化后丢失跟踪的问题提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现代社交媒体应用需要多个具有美学吸引力的图像裁剪，而现有方法主要关注单一裁剪，无法满足这一需求。

Method: 使用图像分割算法作为预处理步骤，评估了多个单裁剪模型的效果，并构建了包含277张相关图像和人工标注的数据集。

Result: 开发了一个可用于多裁剪评估的数据集，并验证了图像分割预处理结合单裁剪模型的方法可行性。
...
TL;DR: 本文提出了一种从LLM隐藏状态中提取人格特质表示的新方法，使用大五人格模型构建低秩子空间，实现精确的人格特质控制。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在生成内容时表现出隐含的人格特质，但如何可靠地控制或对齐这些特质以满足特定需求仍是一个开放挑战。需要有效的机制来在生成过程中操纵模型行为。

Method: 提出了一种新颖的流水线：从transformer层提取隐藏状态激活，使用大五人格特质框架，应用低秩子空间发现方法，识别特质特定最优层，并通过灵活的引导框架实现人格对齐。

Result: 研究发现人格特质占据低秩共享子空间，这些潜在结构可以通过精心设计的扰动转化为有效的引导机制，而不影响流畅性、方差和通用能力。
...
TL;DR: TextualVerifier是一个基于LLM的验证框架，通过思维链分解、变体生成、多数投票和共识聚合四个阶段，为TextGrad提供自我验证机制，显著提升文本推理的有效性。


<details>
  <summary>Details</summary>
Motivation: TextGrad缺乏自我验证机制来确保文本推理的有效性，这限制了其在复合AI系统中的可靠性。

Method: 采用四阶段工作流程：思维链分解、变体生成、多数投票和共识聚合，与TextGrad在损失函数和优化结果验证阶段非侵入式集成。

Result: 在PRM800K上推理步骤有效性提升29%；与TextGrad集成后在GPQA-Diamond、MMLU-ML和MMLU-CP基准上分别获得8.08、10.71和3.92个百分点的改进，损失函数集成使准确率从68.2%提升至70.4%。

Conclusion: TextualVerifier是首个基于LLM技术为TextGrad提供自我验证的框架，无需数值梯度，为文本优化中的验证开辟了新方向。
...
TL;DR: 扩展希腊方言数据集GRDD+，新增6种希腊方言变体，总规模达6,374,939词，涵盖10种方言。对多个LLM进行微调实验，比较不同模型架构与前沿模型的性能。
...
Method: 扩展GRDD数据集，新增Cretan、Cypriot等方言数据，并加入6种新方言变体。对Llama-3-8B、Llama-3.1-8B、Krikri-8B三种模型进行微调，并与Claude-3.7-Sonnet、Gemini-2.5、ChatGPT-5等前沿模型进行比较。

Result: 构建了包含10种希腊方言变体、总计6,374,939词的数据集，这是迄今为止首个具有如此变异性和规模的数据集。

Conclusion: 成功创建了大规模希腊方言数据集，并通过微调实验验证了方言数据对LLM性能的影响，为希腊方言处理研究提供了重要资源。
...
### [67] [BAPPA: Benchmarking Agents, Plans, and Pipelines for Automated Text-to-SQL Generation](https://arxiv.org/abs/2511.04153)
*Fahim Ahmed,Md Mubtasim Ahasan,Jahir Sadik Monon,Muntasir Wahed,M Ashraful Amin,A K M Mahbubur Rahman,Amin Ahsan Ali*

Main category: cs.CL

TL;DR: 本文探索了三种多智能体LLM管道来提升文本到SQL的性能，发现多智能体讨论可以显著提升小模型表现，其中LLM推理器-编码器管道效果最佳，能将准确率从52.4%提升到56.4%。


<details>
  <summary>Details</summary>
Motivation: 现有大型语言模型在从自然语言生成SQL时面临模式规模大和复杂推理的挑战，先前工作多关注复杂不实用的旗舰模型管道，而忽视了小型高效模型。
...
Result: 在Bird-Bench Mini-Dev集上的实验显示，多智能体讨论能提升小模型性能，Qwen2.5-7b-Instruct经过三轮讨论后执行准确率提升10.6%。LLM推理器-编码器管道效果最佳，DeepSeek-R1-32B和QwQ-32B规划器将Gemma 3 27B IT准确率从52.4%提升到最高的56.4%。

Conclusion: 多智能体方法能有效提升文本到SQL任务的性能，特别是对小模型有显著改善，其中规划器-编码器管道是最有效的策略。
...
Method: 构建了1400亿token的波兰语预训练语料库、77k自定义指令数据集和100k偏好优化数据集，采用负责任的AI框架，包含严格的数据治理和混合输出校正模块。
...
### [69] [RUST-BENCH: Benchmarking LLM Reasoning on Unstructured Text within Structured Tables](https://arxiv.org/abs/2511.04491)
...
<details>
  <summary>Details</summary>
Motivation: 现有表格推理基准测试大多使用小型、统一的表格，无法反映真实世界数据的复杂性，也无法全面评估LLM的推理能力。真实表格通常很长、异构且领域特定，需要跨数千个token进行多跳推理。
...
Conclusion: RUST-BENCH为推进表格推理研究建立了一个具有挑战性的新测试平台，突出了当前LLM在处理真实世界复杂表格数据时的局限性。
...
### [70] [STARS: Segment-level Token Alignment with Rejection Sampling in Large Language Models](https://arxiv.org/abs/2511.03827)
*Mohammad Atif Quamar,Mohammad Areeb,Mikhail Kuznetsov,Muslum Ozgur Ozmen,Z. Berkay Celik*

Main category: cs.CL

TL;DR: STARS是一种解码时算法，通过迭代采样、评分和拒绝/接受固定大小的短令牌片段来引导模型生成，显著提高计算效率和对齐质量。
...
Method: STARS采用段级令牌对齐与拒绝采样，在解码时迭代采样、评分和拒绝/接受固定大小的短令牌片段，实现早期生成路径修正。
...
Method: 将多标签分类重新表述为二分决策序列，每个目标维度独立查询；使用LLM-to-SLM蒸馏技术，通过强大的标注模型（DeepSeek-V3）生成多个标注来微调较小模型；结合前缀缓存机制优化推理效率。

Result: 微调后的模型在训练维度上相比零样本基线有显著提升，特别是在情感文本分析的24个维度（包括情感和情绪）上表现出色。

Conclusion: 将多标签分类分解为二分查询，结合蒸馏和缓存感知推理，为基于LLM的分类提供了一个可扩展且有效的框架，该方法虽然验证于情感状态分析，但具有跨领域的通用性。
...
TL;DR: 该论文分析了三种低资源语言（阿法尔奥罗莫语、阿姆哈拉语和提格里尼亚语）机器翻译数据集中的性别偏见问题，发现数据集中存在严重的男性偏向、有害内容和质量缺陷。


<details>
  <summary>Details</summary>
Motivation: 随着低资源语言被纳入NLP研究，大规模数据收集往往重数量轻质量，可能导致技术表现不佳和传播社会偏见。

Method: 通过分析三种低资源语言的机器翻译数据集，特别关注性别表征，包括人名、动词语法性别和刻板印象描绘。

Result: 发现训练数据中政治和宗教领域文本占比大，基准数据集集中在新闻、健康和体育领域；存在严重的男性偏向，以及针对女性的有害和毒性内容，数据量最大的语言问题最突出。
...
TL;DR: GRAD是一种解码时方法，通过构建稀疏标记转移图来缓解LLM幻觉问题，无需重新训练即可基于语料库证据进行生成


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖外部知识源，但基于提示的接地方法脆弱且领域敏感，符号知识集成成本高昂。受知识图谱启发，寻求更轻量级的解决方案

Method: GRAD通过单次前向传播在检索语料上累积下一个标记对数概率来构建稀疏标记转移图，在解码时自适应融合图检索对数概率和模型对数概率

Result: 在三个模型和多个问答基准测试中，GRAD持续超越基线方法，内在准确率提高9.7%，幻觉率降低8.6%，正确性提高6.9%
...
TL;DR: 论文测试了人类和视觉语言模型在迭代参考游戏中的表现，发现模型在缺乏相关上下文时表现远不如人类，但在有相关上下文时性能显著提升。
...
Result: 模型在无相关上下文时表现优于随机但远逊于人类；有相关上下文时模型性能随试验次数显著提升；抽象指称的少样本参考游戏对模型仍是挑战。

Conclusion: 迭代参考游戏是测试上下文敏感语用推理的有效方法，当前模型在有相关上下文时能显著提升性能，但抽象指称的少样本任务仍是难点。
...
TL;DR: 开发了一个基于26亿条地理定位推文的人类繁荣地理指数(HFGI)，使用微调的大语言模型对48个繁荣指标进行分类，提供美国县级和州级月度/年度繁荣相关话语数据。


<details>
  <summary>Details</summary>
Motivation: 量化人类繁荣这一多维概念（包括幸福、健康、目标、美德、关系和经济稳定），超越经济指标理解社会福祉，现有测量缺乏精细的空间和时间分辨率。

Method: 分析2013-2023年约26亿条美国地理定位推文，使用微调的大语言模型按照哈佛全球繁荣研究框架对48个指标进行分类，包括对移民态度和腐败感知。

Result: 创建了县级和州级月度/年度繁荣相关话语指标数据集，验证表明这些测量准确代表基础构念，并与既定指标显示预期相关性。

Conclusion: 该资源支持在空前分辨率下对福祉、不平等和社会变革进行多学科分析，提供了过去十年美国社交媒体话语中反映的人类繁荣动态洞察。
...
Method: 使用学习的映射建立潜在桥梁，实现表示空间之间的直接语义交换。训练了Llama-2-7B和Mistral-7B-Instruct之间的双编码器翻译器，以30%的混合强度注入翻译向量来引导目标模型的生成。
...
<details>
  <summary>Details</summary>
Motivation: RAG系统在检索证据不完整时经常失败，导致推理过程中出现空白。溯因推理能够为解释观察结果生成合理的缺失前提，填补这些空白。

Method: 提出一个框架，包括检测证据不足、生成候选缺失前提，以及通过一致性和合理性检查来验证这些前提。
...
<details>
  <summary>Details</summary>
Motivation: RNN-T在端到端语音识别中依赖大规模高质量标注数据，但这些数据获取成本高且困难，需要减少这种依赖。

Method: 设计了一个灵活的训练图，能够鲁棒地处理转录错误，无需额外的置信度估计或辅助预训练模型。

Result: 在合成和工业数据集上的实验表明，WST在转录错误率高达70%时仍能有效保持性能，始终优于BTC和OTC等基于CTC的弱监督方法。
...
TL;DR: 提出了T-FIX基准，用于评估LLM在知识密集型领域生成的解释与专家判断的对齐程度，解决了现有评估方法只关注解释合理性而忽视与专家直觉一致性的问题。


<details>
  <summary>Details</summary>
Motivation: 在知识密集型应用场景中，用户（通常是领域专家）不仅需要答案，还需要反映专家级推理的解释。现有评估方法主要关注解释的合理性或内部一致性，无法衡量解释内容是否真正符合专家直觉。

Method: 开发了T-FIX基准，涵盖七个知识密集型领域，与领域专家合作制定了新的指标来测量LLM解释与专家判断的对齐程度。

Result: 提出了专家对齐作为评估解释的新标准，并建立了相应的评估框架和指标。

Conclusion: T-FIX基准为评估LLM在专业领域生成解释的质量提供了新的维度，强调了解释内容与专家直觉对齐的重要性。
...
Method: 使用知识规划模块将复杂时序问题分解为子目标序列，构建时序知识存储库并采用对比检索框架选择性检索语义和时序对齐的事实。
...
TL;DR: 比较人类与大型语言模型在情感词汇联想行为上的差异，发现LLMs与人类联想重叠度中等，但LLMs倾向于放大情感刺激，且更可预测、缺乏创造性。


<details>
  <summary>Details</summary>
Motivation: 探索人类与大型语言模型在词汇联想行为上的相似性，特别是针对情感词汇，以了解LLMs是否能像人类一样形成联想。

Method: 通过比较人类参与者与大型语言模型对情感词汇的联想反应，分析联想重叠度、情感放大效应和创造性差异。

Result: 人类与LLMs的联想重叠度为中等水平；LLMs倾向于放大刺激词的情感负荷；LLMs的联想更可预测且缺乏创造性。

Conclusion: 大型语言模型在词汇联想行为上与人类存在显著差异，表现为情感放大、可预测性强和创造性不足的特点。
...
TL;DR: 开发基于Transformer的放射学报告自动去识别模型，通过大规模多模态训练提升性能，在PHI检测方面超越现有学术和商业系统


<details>
  <summary>Details</summary>
Motivation: 提升放射学报告中受保护健康信息(PHI)的自动去识别性能，解决跨机构泛化问题，并建立比商业云供应商系统更优的检测标准

Method: 在现有Transformer PHI去识别流水线基础上，使用斯坦福大学两个大型标注放射学语料库进行微调，引入新的PHI类别(AGE)，采用"hide-in-plain-sight"方法评估合成PHI稳定性

Result: 在Penn数据集上F1得分为0.973，斯坦福数据集上为0.996，优于之前最优模型；合成PHI评估显示一致性检测能力(F1:0.959)；在所有供应商系统中表现最佳

Conclusion: 基于Transformer的去识别模型在PHI检测方面建立了新的安全临床文本处理基准，大规模多模态训练改善了跨机构泛化和鲁棒性
...
<details>
  <summary>Details</summary>
Motivation: 经典的语言识别在极限中是不可能的，但最近在语言生成方面的积极结果促使重新审视这个问题，通过给学习者提供每次输出k个猜测的能力来扩展识别能力。

Method: 基于Angluin特征化的递归版本，给出了k列表识别的精确特征化，并将其分解为k个可识别语言集合的并集。

Result: 证明了语言集合可k列表识别的充要条件是该集合可分解为k个可识别语言集合，并建立了指数识别速率的最优性。

Conclusion: k列表识别为语言识别提供了新的可能性，通过并行猜测策略可以克服经典识别中的限制，并获得了最优的统计识别速率。
...
TL;DR: 批处理提示不仅能降低大语言模型的推理成本，还能作为推理时正则化器，提高多步推理的准确性和效率，减少3-5倍的推理token使用，抑制过度思考，鼓励更果断的回答。


<details>
  <summary>Details</summary>
Motivation: 探索批处理在大语言模型推理中的额外好处，特别是作为推理时正则化器的作用，以提高推理效率和可靠性。

Method: 在13个多样化基准测试上进行综合研究，通过详细的行为分析观察批处理对模型行为的影响，包括抑制过度思考、减少犹豫语言和鼓励果断回答。

Result: 批处理显著提高了准确性，同时大幅减少了推理token使用（通常3-5倍），并观察到批处理推理中的涌现集体效应：模型能够从批内较早的例子中泛化模式来解决更难的问题。
...
Method: 使用项目反应理论测量问题难度，构建难度排序器，通过强化学习指导问题重写模型生成不同难度级别的数学问题变体。

Result: 在竞赛级数学基准测试中，RIDE生成的扰动版本使26个先进LLM的平均性能下降21.73%，暴露了数学推理的鲁棒性限制。

Conclusion: RIDE框架有效验证了LLMs数学推理能力的局限性，为更严谨的评估提供了有效方法。
...
TL;DR: CantoASR是一个协作式ASR-LALM错误校正框架，通过整合强制对齐、LoRA微调Whisper和指令调优Qwen-Audio，显著提升了低资源粤语语音识别的准确率。


<details>
  <summary>Details</summary>
Motivation: 粤语ASR面临低资源、六声调、变调和口音变化等挑战，现有模型如Whisper在粤语上词错误率较高，需要更好的声调和韵律处理能力。

Method: 提出协作框架：使用强制对齐提取声学特征，LoRA微调Whisper提升声调识别，指令调优Qwen-Audio进行韵律感知校正。

Result: 在自发粤语数据上的评估显示，相比Whisper-Large-V3在CER上有显著提升。
...
TL;DR: LAAC提出将LLM作为智能通信中介，通过结构化对话捕获发送者意图并促进真实知识交流，避免AI生成内容的膨胀-压缩循环。该论文系统评估了LAAC在多个通信领域部署的可信度要求。


<details>
  <summary>Details</summary>
Motivation: 当前AI生成内容泛滥导致通信剧场化，发送者用LLM将简单想法膨胀为冗长内容，接收者又用LLM压缩回摘要，双方都不接触真实内容。需要新的通信范式来解决这一问题。

Method: 通过控制实验评估LAAC的多代理架构，系统研究三个可信度维度：信息捕获保真度、可重现性和查询响应完整性。

Result: 初步发现揭示了在LAAC可靠部署到高风险通信场景之前必须解决的可测量信任差距。

Conclusion: 虽然LAAC有潜力改变通信范式，但在部署为可信通信中介之前需要解决信息保真度、一致性和可靠性方面的关键问题。
...
TL;DR: 本文提出了一个计算图灵测试框架来评估LLM生成文本的真实性，发现在情感表达等方面LLM输出仍与人类文本有明显差异，且存在真实性与语义保真度的权衡。


<details>
  <summary>Details</summary>
Motivation: 当前社会科学中广泛使用LLM模拟人类行为，但缺乏对LLM生成文本真实性的可靠验证方法，现有基于人类判断的评估方法不够准确。

Method: 开发了计算图灵测试框架，结合BERT检测性、语义相似度等聚合指标与可解释语言特征，系统比较了9个开源LLM在5种校准策略下的表现。

Result: 即使经过校准，LLM输出在情感基调和情感表达方面仍与人类文本明显可区分；指令调优模型表现不如基础模型；模型规模扩大不会增强人类相似性；存在真实性与语义保真度的权衡。
...
TL;DR: 评估当前大语言模型是否能通过波兰国家上诉庭资格考试，测试显示模型在知识测试中表现尚可，但在实践写作部分均未达标，且LLM作为评判者的评估与官方委员会存在分歧。


<details>
  <summary>Details</summary>
Motivation: 研究当前LLMs在法律专业考试中的实际能力，探索LLM作为考生和评判者的可行性，评估其在法律实践中的应用潜力。

Method: 构建混合信息检索和提取管道，测试多个LLM在闭卷和各种检索增强生成设置下的表现，包括GPT-4.1、Claude 4 Sonnet和Bielik-11B-v2.6等模型。

Result: 模型在知识测试中取得满意分数，但在实践写作部分均未达到及格线；LLM作为评判者的评估与官方委员会判断经常不一致。

Conclusion: 尽管技术进步迅速，当前LLMs在波兰公共采购裁决中尚不能替代人类法官或独立考官，存在幻觉倾向、法律条款引用错误、逻辑论证薄弱等关键限制。
...
<details>
  <summary>Details</summary>
Motivation: 现有机器学习遗忘评估方法主要在单个输入层面评估，可能忽略语义相似示例中的残余影响，这会损害隐私并导致间接信息泄露。

Method: REMIND通过分析模型在输入微小变化时的损失动态来检测残余记忆，发现未学习数据产生更平坦的损失景观，而保留或无关数据则呈现更尖锐、波动的模式。

Result: REMIND在仅需查询访问的条件下优于现有方法，在不同模型、数据集和转述输入中表现出鲁棒性，适合实际部署。
...
TL;DR: 本文通过检索增强生成和测试时计算来量化预训练过程中数据集价值的利用效率，发现当前预训练方法未能充分利用现有数据集中的信息，检索可以显著提升模型性能。


<details>
  <summary>Details</summary>
Motivation: 虽然研究者致力于改进预训练数据集，但很少有人关注预训练机制从数据中提取知识和思想的效率。本文旨在量化预训练过程中被遗漏的数据集价值。

Method: 使用检索增强生成和测试时计算，在标准开源数据集上进行预训练后检索，并在MMLU、Math-500和SimpleQA等任务上评估性能提升。

Result: 检索显著提升了MMLU、Math-500和SimpleQA的准确率，即使在去污染后效果依然存在。在MMLU上，检索相当于预训练单独使用的约5倍计算量。通过额外测试时计算解析检索内容，LLaMA 3.1 8B模型在MMLU上提升了10个百分点。
...
TL;DR: 提出一种基于图的方法进行主题标注，该方法通过丰富主题词并分析词间关系来生成有意义的标签，相比传统基准方法在BERTScore和余弦相似度上表现更好，与ChatGPT-3.5效果相当但计算效率更高。


<details>
  <summary>Details</summary>
Motivation: 现有主题建模方法生成的主题词分布缺乏清晰可解释性，且大多数方法计算成本高昂。本文旨在开发一种计算效率高的方法为话题词集分配有意义的标签。

Method: 采用基于图的方法，不仅丰富主题词的语义相关词汇，还探索词间关系，通过分析图中的连接来推导合适的主题标签。

Result: 在两个不同数据集上的比较研究表明，该方法在BERTScore和余弦相似度指标上优于传统基准方法，与ChatGPT-3.5效果相当，同时保持计算效率。

Conclusion: 图基方法为话题标注提供了有效的解决方案，未来研究方向包括增强可解释性和自动化程度。
...
TL;DR: SSPO提出句子级重要性比率，平衡GRPO和GSPO的优缺点，避免训练崩溃和采样数据利用率低的问题，在五个数据集上平均得分46.57，优于GRPO和GSPO。
...
TL;DR: 本文提出了一种用于机器翻译微调的数据选择方法，通过定义可学习性分数来评估数据点的训练效用，并采用考虑数据点间相互依赖关系的批量选择策略，显著提高了数据效率和计算效率。
...
Result: 在英译波斯等多个语言对上的实验表明，该方法相比iid基线可实现高达5倍的数据效率提升，使用缓存嵌入时计算效率提高24%，并实现了优于随机选择方法的翻译性能。
...
TL;DR: 本研究测试了LLMs在时间推理方面的能力，使用1940年的挪威书籍中的琐事问题，要求LLMs以1940年的视角回答问题。测试了多种语言和模型，发现英文提示效果优于挪威语，更大的模型表现更好。


<details>
  <summary>Details</summary>
Motivation: 探索LLMs在时间推理方面的能力，特别是处理历史背景下的知识问题，并比较不同语言和模型规模对性能的影响。

Method: 使用1940年挪威书籍中的琐事问题，让LLMs以1940年的视角回答，测试了英文和挪威语两种语言版本，并采用LLM-as-judge和人工检查相结合的方式进行评分。

Result: 英文提示的结果始终优于挪威语，这是一个意外的发现。更大的LLMs表现更好，测试了DeepSeek-R1、Gemma3、Qwen3和Llama3.1模型系列，以及专门为挪威语定制的最大的LLM。

Conclusion: LLMs在时间推理方面表现出一定的能力，但语言选择和模型规模对性能有显著影响，英文提示效果更好，更大的模型表现更优。
...
TL;DR: PTTSD是一个概率性文本时间序列抑郁症检测框架，通过临床访谈的逐句分析预测PHQ-8抑郁评分，同时建模时间不确定性，在多个数据集上达到最先进的性能。
...
Result: 在E-DAIC和DAIC-WOZ数据集上，PTTSD在纯文本系统中达到最先进性能（E-DAIC上MAE=3.85，DAIC上MAE=3.55），产生良好校准的预测区间。

Conclusion: PTTSD通过不确定性感知预测提供了临床相关的可解释性，注意力机制和概率建模被证明具有重要价值。
...
TL;DR: ThaiOCRBench是首个针对泰语文本丰富视觉理解任务的综合基准，包含2,808个样本和13个任务类别，评估了多种最先进的视觉语言模型在零样本设置下的表现。


<details>
  <summary>Details</summary>
Motivation: 现有基准主要关注高资源语言，泰语在需要文档结构理解的任务中代表性不足，存在评估空白。

Method: 构建了多样化、人工标注的数据集，在零样本设置下评估了从专有到开源的各种最先进视觉语言模型。

Result: 专有模型（如Gemini 2.5 Pro）表现优于开源模型，特别是在细粒度文本识别和手写内容提取任务中，开源模型性能下降最明显。

Conclusion: 该基准为低资源、复杂脚本环境下的视觉语言模型评估提供了标准化框架，并为改进泰语文档理解提供了可行见解。
...
TL;DR: OUNLP系统在TSAR-2025共享任务中提出基于LLM提示的多轮文本简化方法，发现简化性能与源和目标CEFR级别差距相关，开发了MRS-Rule和MRS-Joint方法，最终排名第7。


<details>
  <summary>Details</summary>
Motivation: 基于对提示式文本简化方法的分析，发现文本简化性能与源CEFR级别和目标CEFR级别之间的差距高度相关。

Method: 提出了两种多轮简化方法：基于规则的多轮简化（MRS-Rule）和联合规则与LLM的多轮简化（MRS-Joint），使用GPT-4o生成简化文本。

Result: 提交的系统在20个团队中排名第7，后续改进表明将LLM简化候选作为起点可以进一步提升多轮简化性能。

Conclusion: 多轮简化方法在文本简化任务中表现良好，特别是结合LLM简化候选作为起点的策略能够有效提升简化性能。
...
TL;DR: 本文系统评估了6个大型语言模型在BFI-2人格框架下的表现，发现不同模型在人格维度上存在显著差异，神经质和外向性受温度参数影响，模型架构可能影响稳定人格特征的形成。
...
Result: 发现五个维度中有四个存在显著差异，神经质和外向性对温度调整敏感，层次聚类揭示了不同的模型集群。

Conclusion: 结果为LLMs中类似人格模式的出现提供了新见解，并为模型调优、选择和AI系统伦理治理提供了新视角。
...
TL;DR: RAGalyst是一个自动化、与人类标注对齐的代理框架，用于严格评估领域特定的RAG系统。它通过代理管道生成高质量的合成问答数据集，优化LLM-as-a-Judge指标，并在三个不同领域验证了RAG性能的高度上下文依赖性。


<details>
  <summary>Details</summary>
Motivation: 现有RAG评估框架在专业安全关键领域存在不足，启发式指标无法捕捉领域特定细微差别，而LLM-as-a-Judge方法缺乏与人类判断的验证对齐。

Method: RAGalyst采用代理管道从源文档生成高质量合成问答数据集，包含代理过滤步骤确保数据保真度。通过提示优化改进两个关键LLM-as-a-Judge指标（答案正确性和可回答性），实现与人类标注的强相关性。

Result: 在三个不同领域（军事行动、网络安全、桥梁工程）评估各种RAG组件，发现性能高度依赖上下文，没有单一的嵌入模型、LLM或超参数配置普遍最优。分析了RAG中低答案正确性的最常见原因。
...
TL;DR: 本文提出了一种处理放射学报告中不确定性的两阶段框架，包括量化显性不确定性（通过LLM验证的短语排名映射到概率值）和建模隐性不确定性（通过专家定义的诊断路径扩展子发现），并发布了Lunguage++基准数据集。


<details>
  <summary>Details</summary>
Motivation: 放射学报告对临床决策至关重要，但包含两种不确定性：显性不确定性（通过模糊短语表达）和隐性不确定性（由于推理过程省略导致），现有基于规则的系统无法有效处理这些不确定性。

Method: 1. 量化显性不确定性：创建专家验证的LLM基准排名，将常见模糊短语映射到概率值；2. 建模隐性不确定性：通过专家定义的14种常见诊断路径，系统性地添加特征性子发现。
...
TL;DR: 通过控制语言模型的隐藏激活来预测和干预其不确定性，发现模型在不确定性高时更容易被引导，表明模型隐式表示可能的推理路径。


<details>
  <summary>Details</summary>
Motivation: 语言模型在生成文本时，单个标记的选择可能导致不同的推理路径，使得不确定性难以量化。研究旨在探索语言模型是否表示它们可能采取的替代路径。

Method: 使用隐藏激活来控制并预测语言模型在思维链推理过程中的不确定性，通过激活干预实验分析模型的可引导性。

Result: 发现模型在不同标记处的不确定性与通过控制激活来引导模型的容易程度存在明显相关性，表明激活干预在模型尚未确定最终答案时最有效。

Conclusion: 隐藏激活可以预测模型的未来结果分布，证明模型隐式表示可能路径的空间，为理解语言模型的推理过程提供了新视角。
...
TL;DR: IntelliProof是一个基于LLM的交互式论证性论文分析系统，将论文构建为论证图，通过可视化界面帮助用户理解论文的论证结构和质量。


<details>
  <summary>Details</summary>
Motivation: 现有自动论文评分系统缺乏用户体验，无法有效帮助用户理解论证结构和关系。IntelliProof旨在弥合论证性论文的结构语义与用户理解之间的差距。

Method: 将论文构建为论证图（节点表示主张，边表示支持/攻击关系），使用LLM对关系进行分类和评分，提供可视化界面和自然语言工具。

Result: 系统能够生成论证图，提供分类理由和论文连贯性量化指标，支持快速探索论证质量，同时保留人工监督。

Conclusion: IntelliProof通过结合LLM分析和交互式可视化，有效提升了论证性论文的分析效率和理解深度，提供了实用的论文分析工具。
...
TL;DR: 最新开源大语言模型在代码生成中仍存在已知漏洞，安全与功能权衡阻碍了有效修复。作者提出了Prompt Exposure和Model Exposure两个新指标来评估漏洞风险。


<details>
  <summary>Details</summary>
Motivation: 随着基于大语言模型的代码助手在软件开发中作用日益重要，其生成的漏洞对网络安全的影响也愈发关键。现有安全基准和改进方法对主流编码LLM的实际影响尚不明确。

Method: 引入Prompt Exposure(PE)指标，综合考虑漏洞严重性、生成概率和诱导漏洞代码的提示词表述；基于PE定义Model Exposure(ME)评分，衡量模型生成漏洞的严重性和普遍性。

Result: 研究显示即使最新的开源模型在现实使用场景中仍然容易受到最早报告的漏洞攻击，表明安全与功能之间的权衡阻碍了漏洞的有效修复。

Conclusion: 需要新的严重性指标来鼓励缓解最严重和普遍的漏洞，PE和ME指标有助于优先处理高风险漏洞。
...
Method: 应用并评估了多种RAG策略，包括传统、零样本回退、代理式、迭代反馈和聚合RAG，结合基于教科书的检索和网络检索与生成推理。关键创新是通过OCR集成孟加拉语医学教科书语料库，并实现代理式RAG管道动态选择检索和推理策略。

Result: 实验结果显示，代理式RAG在openai/gpt-oss-120b上达到最高准确率89.54%，优于其他配置，并展现出更优的推理质量。

Conclusion: 这些发现突显了基于RAG的方法在提高孟加拉语医学问答可靠性和可访问性方面的潜力，为多语言医学人工智能的未来研究奠定了基础。
...
Method: 结合稠密检索和专门分类的方法，用通用文本嵌入替代自回归LLM生成解释性推理。
...
<details>
  <summary>Details</summary>
Motivation: 传统的Chain-of-Thought提示需要生成完整的固定长度推理过程，这在计算上是浪费的，增加了token使用和延迟。

Method: LEASH监控两个内在信号：token级熵的斜率和top-logit边界的改进，当两个信号都趋于平稳时停止生成，表明模型已达到稳定的推理状态。

Result: 在四个指令调优模型上，GSM8K和AQuA-RAT基准测试显示，LEASH平均减少token生成30-35%，延迟减少27%，准确率相对CoT下降10个百分点。
...
### [108] [GPU-Based Floating-point Adaptive Lossless Compression](https://arxiv.org/abs/2511.04140)
*Zheng Li,Weiyan Wang,Ruiyuan Li,Chao Chen,Xianlei Long,Linjiang Zheng,Quanqing Xu,Chuanhui Yang*

Main category: cs.DB

TL;DR: Falcon是一个基于GPU的浮点自适应无损压缩框架，针对IoT和HPC领域的浮点时间序列数据，通过异步流水线、精确浮点转整数转换和自适应稀疏位平面编码，实现了比现有方法更高的压缩比和吞吐量。


<details>
  <summary>Details</summary>
Motivation: IoT和HPC领域产生大量浮点时间序列数据，需要在不损失精度的情况下高效压缩。利用GPU的并行计算能力可以实现前所未有的吞吐量，但面临数据移动瓶颈、精度保持转换复杂度和异常值导致的稀疏性退化三大挑战。

Method: 1) 轻量级异步流水线隐藏CPU-GPU数据传输的I/O延迟；2) 具有理论保证的精确快速浮点转整数转换方法，消除浮点运算误差；3) 自适应稀疏位平面无损编码策略，减少异常值引起的稀疏性问题。

Result: 在12个不同数据集上的实验表明，压缩比相比最先进的基于CPU的方法提高了9.1%，压缩吞吐量比最快的基于GPU的竞争对手高2.43倍，解压缩吞吐量高2.4倍。

Conclusion: Falcon框架成功解决了GPU浮点无损压缩的关键挑战，在压缩比和吞吐量方面均显著优于现有方法，为大规模浮点时间序列数据的高效处理提供了有效解决方案。
...
### [109] [EntroGD: Efficient Compression and Accurate Direct Analytics on Compressed Data](https://arxiv.org/abs/2511.04148)
*Xiaobo Zhao,Daniel E. Lucani*

Main category: cs.DB

TL;DR: EntroGD是一个基于熵指导的广义去重压缩框架，将复杂度从O(nd²)降低到O(nd)，同时保持压缩性能并显著加速配置和分析过程。


<details>
  <summary>Details</summary>
Motivation: 传统广义去重算法在处理高维数据时面临可扩展性挑战，GreedyGD算法的O(nd²)复杂度限制了其在大规模高维数据上的应用。

Method: 采用两步法：首先生成保留分析保真度的浓缩样本，然后应用熵指导的比特选择来最大化压缩效率。

Result: 在18个不同类型和维度的数据集上，EntroGD实现了与GD基压缩器和通用压缩器相当的压缩性能，配置时间比GreedyGD减少最多53.5倍，聚类加速最多31.6倍，且精度损失可忽略。

Conclusion: EntroGD为直接在压缩数据上执行分析提供了一个高效且可扩展的解决方案。
...
### [110] [Scaling Agent Learning via Experience Synthesis](https://arxiv.org/abs/2511.03773)
...
TL;DR: DreamGym是一个统一的框架，通过基于推理的经验模型合成多样化经验数据，解决RL训练中成本高、任务多样性不足、奖励信号不可靠等问题，显著提升自主智能体的在线强化学习训练效果。


<details>
  <summary>Details</summary>
Motivation: 当前强化学习在实际应用中面临昂贵的环境交互、有限的任务多样性、不可靠的奖励信号和复杂的基础设施等挑战，这些都阻碍了可扩展经验数据的收集。

Method: DreamGym将环境动态提炼为基于推理的经验模型，通过逐步推理获得一致的状态转换和反馈信号；利用离线真实数据初始化经验回放缓冲区并持续丰富；自适应生成挑战当前策略的新任务以实现在线课程学习。

Result: 在多样化环境和智能体架构上的实验表明，DreamGym显著提升了RL训练效果。在WebArena等非RL就绪任务上超越所有基线30%以上；在RL就绪但成本高昂的设置中，仅使用合成交互就能匹配GRPO和PPO性能；在从合成经验到真实环境的策略迁移中，获得显著性能提升且需要更少的真实交互。
...
### [111] [How Different Tokenization Algorithms Impact LLMs and Transformer Models for Binary Code Analysis](https://arxiv.org/abs/2511.03825)
*Ahmed Mostafa,Raisul Arefin Nahid,Samuel Mulder*

Main category: cs.AI

TL;DR: 该研究评估了NLP分词模型在汇编代码分析中的内在特性，包括词汇量大小、语义覆盖等，并探讨了预处理定制选项和预分词规则对下游任务（如函数签名预测）的影响。


<details>
  <summary>Details</summary>
Motivation: 汇编代码分析中的分词问题尚未得到充分探索，但其对下游任务性能有重要影响。本研究旨在填补这一空白，评估不同分词模型的内在特性及其对汇编代码分析的适用性。

Method: 系统研究多种分词模型，通过内在评估比较分词效率、词汇压缩和表示保真度。使用Llama 3.2、BERT和BART等预训练模型，在多个性能指标上评估这些分词器的有效性。
...
TL;DR: BehaviorLens框架评估了用户行为数据的文本与图像表示对多模态大语言模型性能的影响，发现图像表示能显著提升预测准确率。
...
Method: 开发BehaviorLens基准框架，在六个MLLMs上测试交易数据的三种表示方式：文本段落、散点图和流程图。

Result: 图像表示使MLLMs的下一次购买预测准确率比等效文本表示提高了87.5%，且无需额外计算成本。

Conclusion: 图像表示用户行为数据能显著提升多模态大语言模型的推理性能。
...
### [113] [KnowThyself: An Agentic Assistant for LLM Interpretability](https://arxiv.org/abs/2511.03878)
...
<details>
  <summary>Details</summary>
Motivation: 现有LLM可解释性工具分散且代码密集，需要整合到一个易于使用的平台中

Method: 使用编排器LLM重新表述用户查询，通过代理路由器分发到专门模块，最后将输出情境化为连贯解释

Result: 开发了一个基于聊天的交互式界面，用户可以上传模型、用自然语言提问并获得可视化解释

Conclusion: KnowThyself为LLM可解释性提供了强大且可扩展的基础平台
...
TL;DR: 本文挑战了关于深度知识追踪(DKT)模型性能来源的普遍解释，提出DKT的优势在于其能够隐式建模先决条件关系作为因果结构，而非双向关系。


<details>
  <summary>Details</summary>
Motivation: 计算教育研究中长期目标是开发可解释的知识追踪模型。DKT作为传统方法的重大进步，其性能提升通常被归因于建模知识组件间的双向关系，但本文质疑这一解释。

Method: 通过将练习关系图修剪为有向无环图(DAGs)，并在Assistments数据集的因果子集上训练DKT，分析其预测能力与因果结构的一致性。还提出使用DKT学习表示提取练习关系DAGs的替代方法。

Result: 实验表明DKT的预测能力与因果结构高度一致，其学习表示可用于有效提取练习关系DAGs，为DKT性能来源提供了新的解释。

Conclusion: DKT的有效性主要源于其近似知识组件间因果依赖关系的能力，而非简单的关系映射。这一发现对理解深度知识追踪模型的工作原理具有重要意义。
...
<details>
  <summary>Details</summary>
Motivation: 研究LLMs是否能代表其广泛用户群体的文化多样性，特别是考察提示语言和文化框架如何影响模型响应与不同国家人类价值观的匹配度。

Method: 使用霍夫斯泰德价值观调查模块和世界价值观调查的63个项目，翻译成11种语言，以带有和不带有明确文化视角的提示形式测试10个LLM。

Result: 提示语言和文化视角都会导致LLM输出变化，但模型存在系统性偏见，偏向特定国家价值观。明确文化视角比针对性提示语言更能改善与人类价值观的匹配度。
...
### [116] [ArchPilot: A Proxy-Guided Multi-Agent Approach for Machine Learning Engineering](https://arxiv.org/abs/2511.03985)
...
<details>
  <summary>Details</summary>
Motivation: 现有的LLM代理在自动ML工程中严重依赖重复的完整训练来评估候选方案，导致计算开销大、搜索空间受限、迭代周期慢。

Method: 使用三个专业代理：协调代理采用MCTS启发算法管理搜索过程，生成代理迭代生成和改进架构，评估代理执行代理训练并生成优化代理函数。

Result: 在MLE-Bench上的实验表明，ArchPilot超越了AIDE和ML-Master等最先进基线，验证了多代理系统的有效性。
...
### [117] [Detecting Silent Failures in Multi-Agentic AI Trajectories](https://arxiv.org/abs/2511.04032)
*Divya Pathak,Harshit Kumar,Anuska Roy,Felix George,Mudit Verma,Pratibha Moogi*

Main category: cs.AI

TL;DR: 提出了多智能体AI系统中的异常检测任务，构建了两个包含4,275和894条轨迹的基准数据集，并评估了监督和半监督方法的性能，最高准确率达到98%。
...
Method: 开发了数据集构建流程，捕捉用户行为、智能体非确定性和LLM变化，并在此基础上有监督（XGBoost）和半监督（SVDD）异常检测方法。

Result: 监督和半监督方法表现相当，准确率分别达到98%和96%，为多智能体AI系统的异常检测提供了基准。

Conclusion: 这是对多智能体AI系统中异常检测的首次系统性研究，提供了数据集、基准和见解，指导未来研究。
...
### [118] [Interpreting Multi-Attribute Confounding through Numerical Attributes in Large Language Models](https://arxiv.org/abs/2511.04053)
*Hirohane Takagi,Gouki Minegishi,Shota Kizawa,Issey Sukeda,Hitomi Yanaka*

Main category: cs.AI

TL;DR: LLMs在数值推理中存在系统错误，研究发现它们编码了真实世界的数值相关性但会放大这些相关性，无关数值上下文会干扰模型表示和输出。


<details>
  <summary>Details</summary>
Motivation: 尽管行为研究发现LLMs存在数值推理错误，但其内部表示机制尚不清楚。研究旨在探索LLMs如何整合单个实体的多个数值属性，以及无关数值上下文如何干扰这些表示。

Method: 结合线性探测、偏相关分析和基于提示的脆弱性测试，在不同规模的模型上进行实验。

Result: LLMs编码了真实世界的数值相关性但会系统性地放大它们；无关上下文会引起数值表示的一致偏移，下游影响随模型规模而异。
...
### [119] [Are We Asking the Right Questions? On Ambiguity in Natural Language Queries for Tabular Data Analysis](https://arxiv.org/abs/2511.04584)
*Daniel Gomm,Cornelius Wolff,Madelon Hulsebos*

Main category: cs.AI

TL;DR: 该论文将自然语言查询中的歧义重新定义为合作交互的特征，提出了区分可解析合作查询与不可解析非合作查询的框架，并分析了15个流行数据集中的查询类型分布问题。


<details>
  <summary>Details</summary>
Motivation: 传统方法将查询歧义视为缺陷，但作者认为歧义应该是用户与系统之间合作交互的特征，双方共同承担查询规范的责任。

Method: 开发了一个原则性框架来区分合作查询（可解析）和非合作查询（不可解析），并将该框架应用于15个流行表格问答和分析数据集的查询分析。

Result: 发现现有数据集中的查询类型混合不当，既不适合评估系统执行准确性，也不适合评估解释能力。框架和分析将视角从修复歧义转向在解析查询中拥抱合作。
...
Method: 基于博弈论思想，特别是"选择并冻结"协议，使用大型语言模型智能体代表对立政治利益进行回合制谈判。智能体从少量候选地图中交替选择和冻结选区，通过受限且可解释的选择逐步划分州。

Result: 在2020年美国人口普查数据上的评估显示，Agentmandering显著减少了党派偏见和不公平性，同时实现了比标准基线低2到3个数量级的方差，在摇摆州场景中表现出公平性和稳定性。

Conclusion: Agentmandering框架通过将战略互动嵌入选区重划过程，能够有效减少党派偏见，提高公平性和稳定性，特别是在竞争激烈的选举环境中。
...
TL;DR: LLM-KGFR框架通过结合大语言模型和知识图谱基础检索器，解决了LLM在知识密集型问题上的局限性，实现了零样本泛化和高效处理大规模图谱。
...
Method: 提出LLM-KGFR协作框架，其中LLM与结构化检索器KGFR协同工作。KGFR使用LLM生成的关系描述编码关系，并根据实体在问题中的角色初始化实体，实现零样本泛化。采用非对称渐进传播(APP)高效处理大型图谱，通过节点、边和路径级接口形成可控推理循环。
...
TL;DR: 提出了首个系统性框架来评估语音AI测试质量，通过人类中心化基准测试解决测试平台的双重挑战：生成真实对话和准确评估响应质量。


<details>
  <summary>Details</summary>
Motivation: 随着语音AI代理快速部署到生产环境，缺乏系统性测试可靠性评估方法，组织无法客观评估测试方法的有效性，这在大规模部署时造成了关键测量差距。

Method: 结合心理测量技术（成对比较产生Elo评分、自举置信区间和置换测试）与严格统计验证，提供可重复的指标，适用于任何测试方法。

Result: 对三个领先商业平台的实证评估显示，表现最佳的平台Evalion在评估质量上达到0.92 F1分数，其他平台为0.73；在模拟质量上达到0.61，其他平台为0.43。
...
TL;DR: 本文研究了在多人类环境中，基于赋权(empowerment)的辅助AI代理可能导致其他人类环境控制力和奖励降低的现象，称为"去赋权"(disempowerment)。


<details>
  <summary>Details</summary>
Motivation: 赋权作为AI代理的通用目标无关目标，在单人类环境中表现良好，但在多人类环境(如家庭、医院)中可能产生负面效应，需要研究其在这种复杂环境中的表现。

Method: 开发了开源的多人类网格世界测试套件Disempower-Grid，通过实验验证辅助RL代理在优化单人类赋权时对其他人类的影响。

Result: 实验表明，优化单人类赋权的辅助RL代理会显著降低其他人类的环境影响力和奖励，即产生去赋权现象。联合赋权可以缓解此问题，但会牺牲用户的奖励。
...
<details>
  <summary>Details</summary>
Motivation: 需要一种系统化的方法来评估和优化工作流的质量和效率，特别是在现代自动化系统中进行工作流发现和改进。

Method: 结合Opus工作流奖励（概率函数估计性能）和Opus工作流规范惩罚（衡量结构信息质量），形成一个统一的优化公式。

Result: 提出了一个支持自动化工作流评估、排名和优化的框架，可以集成到强化学习循环中指导工作流发现和改进。

Conclusion: Opus工作流评估框架为工作流质量评估和优化提供了一个统一的数学基础，支持在现代自动化系统中进行有效的工作流管理。
...
### [125] [Shared Spatial Memory Through Predictive Coding](https://arxiv.org/abs/2511.04235)
*Zhengru Fang,Yu Guo,Jingjing Wang,Yuang Zhang,Haonan An,Yinhai Wang,Yuguang Fang*

Main category: cs.AI

TL;DR: 提出了一个多智能体预测编码框架，通过最小化智能体间的互不确定性来解决协调问题，在带宽受限条件下实现了优雅的性能退化。


<details>
  <summary>Details</summary>
Motivation: 多智能体系统中部分可观测性和有限带宽常导致协调失败，需要开发能够有效共享和重建空间记忆的机制。

Method: 基于信息瓶颈目标的多智能体预测编码框架，包含自监督运动预测产生的网格细胞状度量空间编码、带宽高效通信机制和分层强化学习策略。
...
### [126] [RLoop: An Self-Improving Framework for Reinforcement Learning with Iterative Policy Initialization](https://arxiv.org/abs/2511.04285)
*Zeng Zhiyuan,Jiashuo Liu,Zhangyue Yin,Ge Zhang,Wenhao Huang,Xipeng Qiu*

Main category: cs.AI

TL;DR: RLoop通过迭代策略初始化解决RL训练中的过拟合问题，将训练过程转化为探索-利用循环，显著提升泛化能力。


<details>
  <summary>Details</summary>
Motivation: 解决强化学习训练中的过拟合问题，即模型获得训练奖励但失去泛化能力，这由策略过度专业化和灾难性遗忘引起。

Method: 提出RLoop框架，通过迭代策略初始化：先用RL从给定策略探索解空间，然后过滤成功轨迹创建专家数据集，通过拒绝采样微调(RFT)优化初始策略，形成探索-利用的良性循环。

Result: RLoop有效缓解遗忘问题，大幅提升泛化能力，相比原始RL平均准确率提升9%，pass@32提升超过15%。

Conclusion: RLoop通过将瞬态策略变化转化为稳健性能增益，成功解决了RL训练中的泛化问题。
...
### [127] [GUI-360: A Comprehensive Dataset and Benchmark for Computer-Using Agents](https://arxiv.org/abs/2511.04307)
...
<details>
  <summary>Details</summary>
Motivation: 解决计算机使用代理面临的三个关键挑战：真实CUA任务稀缺、多模态轨迹自动收集标注流程缺失、缺乏统一基准来联合评估GUI定位、屏幕解析和动作预测。

Method: 采用LLM增强的自动化流程，包括查询来源、环境模板构建、任务实例化、批量执行和LLM驱动的质量过滤，收集Windows办公应用中的轨迹数据。

Result: 构建了包含完整分辨率截图、可访问性元数据、目标、推理轨迹和成功/失败动作轨迹的数据集，基准测试显示现有模型在定位和动作预测方面存在显著不足。

Conclusion: 监督微调和强化学习能带来显著改进，但尚未达到人类水平的可靠性，发布数据集和代码以促进可重复研究和稳健桌面CUAs的进展。
...
TL;DR: CAV探测器的分类准确率不能可靠衡量概念对齐度，因为探测器容易捕捉虚假相关性。作者提出基于空间线性归因的新概念定位方法，并建议使用硬准确率、分割分数和增强鲁棒性三类指标来评估概念对齐。


<details>
  <summary>Details</summary>
Motivation: 当前可解释AI中，人们通常使用线性分类器探测器的准确率来衡量CAV是否忠实表示目标概念，但作者发现这种假设不可靠，探测器更容易捕捉虚假相关性而非真正概念。

Method: 提出基于空间线性归因的概念定位方法，并与现有特征可视化技术进行比较。构建故意错位的探测器来利用虚假相关性，验证准确率接近标准探测器。

Result: 实验证明，具有平移不变性和空间对齐的探测器能持续提高概念对齐度。故意错位的探测器也能达到接近标准探测器的准确率。

Conclusion: 需要基于对齐度的评估指标而非探测器准确率，且应根据模型架构和目标概念性质定制探测器。
...
TL;DR: AdversariaLLM是一个用于LLM越狱鲁棒性研究的工具箱，旨在解决当前研究生态系统的碎片化和错误问题，通过提供可复现、正确且可扩展的框架来促进LLM安全研究的透明度和可比性。


<details>
  <summary>Details</summary>
Motivation: 当前LLM安全性和鲁棒性研究生态系统存在碎片化、实现错误等问题，导致研究难以复现和比较，阻碍了该领域的实质性进展。

Method: 开发了AdversariaLLM工具箱，包含12种对抗攻击算法、7个基准数据集（涵盖有害性、过度拒绝和效用评估），通过Hugging Face集成多种开源LLM，并提供了计算资源跟踪、确定性结果和分布评估等高级功能。

Result: 建立了一个支持透明、可比和可复现研究的LLM安全研究基础框架，集成了JudgeZoo评估包。

Conclusion: AdversariaLLM为解决LLM安全研究中的碎片化和复现性问题提供了系统化解决方案，为未来研究奠定了坚实基础。
...
<details>
  <summary>Details</summary>
Motivation: 目前基于LLMs的医疗系统在药物安全方面的研究有限，缺乏真实世界数据集，且在真实临床咨询环境下的评估不足。

Method: 构建包含药物风险信息的问诊对话，创建专门的药物安全数据库RxRisk DB，采用两阶段过滤策略确保临床真实性和专业性，最终形成2,443个高质量咨询场景的基准测试集。

Result: 当前LLMs在整合禁忌症和相互作用知识方面存在困难，特别是当风险是隐含而非明确时表现不佳。

Conclusion: RxSafeBench为评估LLMs的药物安全性提供了首个全面基准，有助于推动更安全可靠的AI驱动临床决策支持系统的发展。
...
TL;DR: 提出Monitor-Generate-Verify（MGV）框架，通过在Generate-Verify范式前添加显式监控机制来解决模型在推理过程中过早陷入次优路径的问题。
...
Method: 将Flavell以及Nelson和Narens的元认知理论形式化为计算规范，在生成前添加显式监控机制，捕捉从难度评估到置信度判断的元认知体验，并通过验证反馈来优化未来监控。
...
TL;DR: 提出Iterative RMFT方法，通过迭代蒸馏低后悔值决策轨迹来增强LLM的决策能力，无需依赖已知算法或人工设计思维链模板。


<details>
  <summary>Details</summary>
Motivation: LLM在交互动态环境中作为决策代理时表现不佳，难以实现低后悔值和有效的探索-利用权衡，需要专门的后训练方法来提升决策能力。

Method: 迭代后悔最小化微调：模型多次生成决策轨迹，选择k个最低后悔值的轨迹，然后在这些轨迹上微调自身，利用后悔值指标激发模型的决策能力和推理逻辑。

Result: Iterative RMFT显著提升了多种LLM的决策性能，包括Transformer、开源LLM和GPT-4o mini等闭源模型，在具有不同时间跨度、动作空间和奖励过程的任务中表现出良好的泛化能力。
...
TL;DR: CoRPO提出了一种新的强化学习优化方法，解决了GRPO在使用序数奖励时对失败轨迹给予正优势的问题，通过自适应基线确保失败解决方案不被正强化，并在达到质量阈值后自动切换到相对偏好模式。


<details>
  <summary>Details</summary>
Motivation: GRPO的简单性使其在适应LLMs到特定任务时很受欢迎，但在使用序数奖励提供部分信用时存在问题，其组平均基线经常给失败轨迹分配正优势并强化错误行为。

Method: CoRPO使用自适应基线强制执行最低质量阈值，确保失败解决方案永远不会被正强化。一旦策略持续满足此阈值，基线自动过渡到相对偏好模式，推动模型寻找最优解决方案而不仅仅是"可接受"的解决方案。

Result: 在代码验证任务上的实证验证表明，CoRPO表现出更稳定的收敛性和更好的域外泛化能力。

Conclusion: 这项工作是在更广泛研究计划中的关键一步，旨在通过强化学习使LLMs能够学习真正的新能力，通过从二进制奖励到序数奖励的进展，最终实现更密集的每步监督。
...
TL;DR: PAVe系统结合经典路径规划算法与LLM语义推理层，通过多目标Dijkstra算法生成候选路线，再由LLM代理根据用户任务、偏好和规避规则进行上下文评估，实现个性化车辆路线规划。
...
Method: 使用多目标（时间、CO2）Dijkstra算法生成候选路线，然后由LLM代理基于预处理的地理空间POI缓存，评估这些路线是否符合用户提供的任务、偏好和规避规则。
...
TL;DR: 本文首次探讨了网络代理的能源消耗和碳排放问题，通过理论和实证分析揭示了不同设计理念对能耗的显著影响，并呼吁在评估网络代理时考虑能源效率指标。
...
Result: 研究表明不同网络代理设计理念会严重影响能耗，且更高能耗不一定带来更好结果。同时发现某些网络代理缺乏模型参数和流程透明度，限制了能耗估算。

Conclusion: 需要改变网络代理的评估方式，在基准测试中加入专门的能源消耗度量指标，以促进更可持续的网络代理发展。
...
TL;DR: 研究发现LLMs能够复制人类合作行为模式，Llama模型能高保真再现人类偏离理性选择理论的行为，而Qwen模型更接近纳什均衡预测，无需角色提示即可实现群体级行为复制。


<details>
  <summary>Details</summary>
Motivation: 了解LLMs与人类决策的匹配程度至关重要，因为不匹配可能导致实际应用中的有害结果，而无法复制人类行为会使LLMs在社会模拟中无效。

Method: 开发游戏论实验的数字孪生，引入系统化的提示和探测框架进行机器行为评估，测试三个开源模型（Llama、Mistral和Qwen）。

Result: Llama能高保真复制人类合作模式，捕捉人类偏离理性选择理论的行为；Qwen与纳什均衡预测紧密对齐；无需角色提示即可实现群体级行为复制。

Conclusion: 适当校准的LLMs能够复制聚合的人类行为模式，并能系统探索未开发的实验空间，为社会科学研究提供补充方法，生成关于人类社交决策的新实证预测。
...
TL;DR: 提出数据驱动的稀疏传感框架，结合EPA-SWMM模型优化传感器布置，用最少传感器实现城市排水系统峰值流量的高精度重建。


<details>
  <summary>Details</summary>
Motivation: 城市地表水泛滥日益频繁，但高时空分辨率的洪水预测和监测受到时间、预算和技术限制，如何在资源受限条件下监测排水网络和预测流量状况是主要挑战。

Method: 使用SWMM模型生成训练数据集，应用数据驱动稀疏传感框架，利用奇异值分解降维和QR分解进行传感器分配，识别最优监测节点。

Result: 在77个节点中仅需3个最优布置传感器即可实现满意重建性能（NSE值0.92-0.95），模型对测量不确定性具有良好鲁棒性。

Conclusion: 该框架平衡计算效率和物理可解释性，能以最少传感器实现高精度流量重建，可进一步与预测模型集成实现有限传感资源下的洪水预警和实时控制。
...
TL;DR: 开发了Jr. AI Scientist系统，这是一个模拟学生研究流程的自主AI科学家系统，能够分析论文局限性、提出假设、实验验证并撰写论文，在评估中表现优于现有全自动系统，但也揭示了当前AI科学家系统的局限性和风险。
...
Result: Jr. AI Scientist生成的论文在AI评审中获得比现有全自动系统更高的评分，但作者评估和Agents4Science评审揭示了重要局限性，表明直接应用当前AI科学家系统存在潜在风险。

Conclusion: 当前AI科学家系统虽然取得进展，但仍存在重要限制和风险，需要进一步研究解决关键挑战，这些见解有助于深化对AI科学家发展现状和风险的理解。
...
### [139] [Question the Questions: Auditing Representation in Online Deliberative Processes](https://arxiv.org/abs/2511.04588)
*Soham De,Lodewijk Gelauff,Ashish Goel,Smitha Milli,Ariel Procaccia,Alice Siu*

Main category: cs.AI

TL;DR: 提出了一个基于合理代表制(JR)的审核框架，用于衡量专家问答环节中问题选择的代表性，并开发了高效的审核算法。通过历史审议数据比较了主持人选择、整数线性规划选择和LLM生成摘要问题三种方法的代表性。
...
Method: 基于合理代表制(JR)概念建立审核框架，开发了O(mn log n)时间复杂度的审核算法，并应用于历史审议数据进行比较分析。

Result: 比较了三种问题选择方法的代表性：主持人选择、整数线性规划选择和LLM生成摘要问题，揭示了LLM在支持审议过程中的潜力和当前局限性。

Conclusion: 通过将审核方法集成到在线审议平台，使实践者能够轻松审核和改进未来审议中的代表性。
...
### [140] [DR. WELL: Dynamic Reasoning and Learning with Symbolic World Model for Embodied LLM-Based Multi-Agent Collaboration](https://arxiv.org/abs/2511.04646)
*Narjes Nourzad,Hanqing Yang,Shiyu Chen,Carlee Joe-Wong*

Main category: cs.AI

TL;DR: DR.WELL是一个去中心化的神经符号框架，用于协作多智能体规划，通过两阶段协商协议（角色提议和联合分配）实现合作，避免了轨迹级协调的脆弱性。
...
Method: 采用两阶段协商协议：首先智能体提议候选角色并给出理由，然后在共识和环境约束下承诺联合分配；承诺后各智能体独立生成并执行符号计划，通过共享世界模型进行执行结果落地。

Result: 在协作推块任务实验中，智能体能够跨回合适应，动态世界模型捕获可重用模式，提高了任务完成率和效率。

Conclusion: 通过符号规划而非原始轨迹推理，DR.WELL避免了脆弱的步骤级对齐，实现了更高级别、可重用、可同步且可解释的操作，以时间开销换取演化出的更高效协作策略。
...
### [141] [VeriCoT: Neuro-symbolic Chain-of-Thought Validation via Logical Consistency Checks](https://arxiv.org/abs/2511.04662)
*Yu Feng,Nathaniel Weir,Kaj Bostrom,Sam Bayless,Darion Cassel,Sapana Chaudhary,Benjamin Kiesl-Reiter,Huzefa Rangwala*

Main category: cs.AI

TL;DR: VeriCoT是一个神经符号方法，从CoT推理中提取和验证形式逻辑论证，通过一阶逻辑形式化推理步骤，使用自动求解器验证逻辑有效性，并提高推理可靠性。


<details>
  <summary>Details</summary>
Motivation: LLMs通过CoT进行多步推理，但无法可靠验证自身逻辑，即使在正确答案下，底层推理可能存在缺陷，影响高风险场景的信任度。

Method: 将CoT推理步骤形式化为一级逻辑，识别基于源上下文、常识知识或先前推理步骤的前提，使用符号表示实现自动求解器验证逻辑有效性。
