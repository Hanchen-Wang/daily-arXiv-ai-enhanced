...
Motivation: 现有方法在语义3D运动迁移方面存在局限，需要开发能够实现跨类别对象间语义对应运动传输的新方法。

Method: 通过条件反演提取源视频运动嵌入，应用到静态目标形状的渲染帧，使用生成视频监督动态3D高斯溅射重建，引入基于锚点的视角感知运动嵌入机制。

Result: 建立了首个语义3D运动迁移基准，相比适应基线方法展现出更优的运动保真度和结构一致性。

Conclusion: 该方法能够有效实现语义3D运动迁移，为相关领域提供了新的解决方案和基准评估。
...
<details>
  <summary>Details</summary>
Motivation: 龋齿在全景X光片中的准确识别和分割对早期诊断和治疗规划至关重要，但由于病变对比度低、形态多变和标注数据有限，自动分割仍然具有挑战性。

Method: 使用DC1000数据集，在相同配置下训练了12种最先进架构，包括CNN、Vision Transformer和状态空间Mamba架构，如VMUnet、MambaUNet、DoubleU-Net等。

Result: CNN-based DoubleU-Net获得了最高的dice系数0.7345、mIoU 0.5978和精度0.8145，在所有性能指标的前三名均为基于CNN的架构，Transformer和Mamba方法表现不佳。

Conclusion: 研究结果表明，在特定领域的医学图像分割中，架构与任务的匹配比模型复杂度更为重要，复杂注意力架构在数据有限和空间先验较弱的情况下表现不佳。
...
TL;DR: 本文提出了B-Rep距离函数(BR-DF)，一种基于体积距离函数的CAD边界表示新方法，能够实现100%成功率的B-Rep模型生成。


<details>
  <summary>Details</summary>
Motivation: 传统CAD B-Rep表示方法在生成过程中存在失败率，需要一种更可靠的几何表示方法来解决这个问题。

Method: 使用有符号距离函数(SDF)编码表面网格几何，使用无符号距离函数(UDF)编码B-Rep顶点、边、面及其拓扑信息，并扩展Marching Cubes算法将BR-DF转换为水密CAD B-Rep模型。

Result: 该方法在CAD生成性能上与最先进方法相当，同时实现了前所未有的100%成功率生成B-Rep模型。

Conclusion: BR-DF提供了一种可靠且高效的CAD B-Rep表示方法，解决了传统方法中的生成失败问题。
...
TL;DR: GeoSceneGraph是一种从文本提示合成3D室内场景的方法，它利用场景的图结构和几何对称性，无需预定义关系类别，在资源受限设备上实现高效部署。


<details>
  <summary>Details</summary>
Motivation: 现有方法要么从头训练生成模型，要么依赖视觉语言模型，但前者忽略室内场景固有的图结构，后者需要用户提供的语义图或真实关系标注，限制了捕捉多样化对象交互的能力。
...
TL;DR: 提出了一种无监督双向MRI合成器，能够在高场和超低场MRI图像之间进行双向转换，基于物理驱动的对比度变化机制。


<details>
  <summary>Details</summary>
Motivation: 现有MRI合成模型缺乏对高场和超低场MRI之间对比度变化的物理机制建模，需要开发基于物理原理的双向合成方法。

Method: 前向模型通过估计组织类型信噪比来模拟高场到超低场的转换；超分辨率任务使用隐式神经表示网络同时预测组织分割和图像强度。

Result: 合成超低场图像中白质-灰质对比度提升52%，64mT图像中提升37%；敏感性实验证明模型对目标对比度、噪声和初始种子的变化具有鲁棒性。

Conclusion: 该方法成功实现了基于物理原理的无监督双向MRI合成，在对比度提升和模型鲁棒性方面表现出色。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于每场景神经场或时序注意力机制的方法在稀疏输入视图的多视图编辑中会产生伪影和不一致编辑，需要改进跨视图一致性。

Method: 用多视图扩散学生替代SDS中的传统神经场整合器，采用跨时间步增量学生更新、专用教师噪声调度器和无额外成本的注意力修改来增强跨视图一致性。

Result: 实验表明I-Mix2Mix显著改善了多视图一致性，同时保持高单帧编辑质量。

Conclusion: I-Mix2Mix框架通过知识蒸馏有效解决了多视图编辑中的一致性问题，为稀疏视图编辑提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 解决视觉语言模型在临床推理中的三个主要限制：数据异构性（缺乏一致的诊断标签和临床概念标注）、缺乏基于诊断依据的推理（缺少可靠的推理监督）、以及有限的可扩展性和泛化能力（模型难以从密集标注的小数据集迁移到稀疏标注的大数据集）。

Method: 1. 设计基于教科书的推理生成器，合成高保真、层次感知和鉴别诊断（DDx）信息的轨迹，提供专家级监督；2. 利用构建的轨迹进行监督微调（SFT），赋予模型基于依据的推理能力；3. 开发新颖的强化学习范式，通过结合疾病层次结构，将这些基于依据的推理模式有效迁移到大规模稀疏数据。

Result: 在多个皮肤病数据集上的广泛实验表明，SkinR1实现了优越的诊断准确性。消融研究证明了SFT建立的推理基础的重要性。
...
TL;DR: FarSLIP是一个针对遥感数据的细粒度语言-图像预训练框架，通过构建多粒度数据集和改进的patch-to-patch蒸馏方法，解决了CLIP在遥感领域空间感知能力有限的问题。


<details>
  <summary>Details</summary>
Motivation: 当前的遥感专用CLIP变体仍然继承了有限的空问感知能力，主要因为现有数据集生成全局标题时未充分利用对象级监督，以及直接应用区域-文本对齐方法会导致性能下降。

Method: 构建了首个多粒度遥感图像-文本数据集MGRS-200k，并提出FarSLIP框架，采用patch-to-patch蒸馏而非patch-to-CLS自蒸馏来对齐局部和全局视觉线索，同时使用简单的CLS token进行区域-类别对齐。

Result: FarSLIP在遥感开放词汇语义分割、零样本分类和图像-文本检索等任务上均达到了新的最先进水平。

Conclusion: FarSLIP通过改进的细粒度对齐方法有效提升了遥感领域的视觉-语言对齐能力，同时保持了语义连贯性。
...
TL;DR: nnMIL是一个简单但广泛适用的多实例学习框架，将补丁级病理基础模型连接到稳健的幻灯片级临床推理，在40,000张全切片图像上表现优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 当前将补丁级特征聚合为幻灯片级预测的方法存在设计限制，阻碍了泛化性和可靠性，需要开发更稳健的解决方案。

Method: nnMIL在补丁和特征级别引入随机采样，支持大批量优化、任务感知采样策略，使用轻量级聚合器进行滑动窗口推理生成集成预测。

Result: 在35个临床任务和4个病理基础模型上，nnMIL在疾病诊断、组织学分型、分子生物标志物检测和泛癌预后预测方面一致优于现有MIL方法。
...
TL;DR: X-WIN是一个新颖的CXR世界模型，通过从胸部CT中学习预测其潜在空间中的2D投影来提取体积知识，从而解决CXR因结构叠加而难以捕捉3D解剖结构的限制。
...
Conclusion: X-WIN通过从CT中提取体积知识，有效解决了CXR的3D结构限制问题，在多个任务上表现出色，并具备3D重建能力。
...
TL;DR: CPSL是一种紧凑的2.5D视频表示方法，通过将2D视频分解为几何一致的层，结合深度和内容显著性，实现轻量级的视差校正新视角合成，大幅降低存储和渲染成本。


<details>
  <summary>Details</summary>
Motivation: 现有的体积视频表示（如点云和神经场）在捕获、计算和渲染方面成本高昂，限制了其在按需视频和实时通信中的可扩展性。

Method: 基于每帧深度和内容显著性，将帧分解为几何一致的层，配备软alpha带和边缘深度缓存，通过深度加权扭曲和前向后alpha合成实现视差校正的新视角合成。
...
TL;DR: 提出了首个包含580个多模态人类活动序列的长周期工作流基准，支持无监督周期工作流检测、任务完成跟踪和程序异常检测三个任务，并提出了轻量级、无需训练的基础方法。


<details>
  <summary>Details</summary>
Motivation: 解决长周期工作流（具有低对比度模式）研究不足的问题，填补制造业、体育和日常生活中周期性人类活动工作流检测的空白。

Method: 构建包含580个多模态人类活动序列的基准数据集，并提出轻量级、无需训练的基础方法来建模多样的周期性工作流模式。

Result: 基准对现有方法构成显著挑战；提出的基础方法在所有评估任务中大幅优于竞争方法；在真实应用中展现出与传统监督方法相当的部署优势，无需标注和重新训练。

Conclusion: 该基准和方法为长周期工作流分析提供了重要工具，在无监督设置下实现了与监督方法相当的性能，具有实际部署价值。
...
TL;DR: 提出一种低成本的多视角视频同步方法，使用LED时钟编码时间信息，实现毫秒级时间对齐，适用于异质相机系统（RGB和红外）。


<details>
  <summary>Details</summary>
Motivation: 多视角视频的精确时空对齐对3D重建、姿态估计等应用至关重要，但在异质相机系统中硬件同步往往不可用，特别是在真实环境中。

Method: 使用定制的LED时钟，通过红色和红外LED编码时间，从记录帧中视觉解码曝光窗口（开始和结束时间）以实现毫秒级同步。

Result: 与硬件同步相比，残差误差为1.34ms RMSE；在多个实验中优于基于光、音频和时间码的同步方法，并直接改善了多视角姿态估计和3D重建等下游任务。

Conclusion: 该方法简化了同步流程，扩展了在无约束环境（包括工业和临床应用）中基于视觉的先进传感的访问。
...
Method: 采用闭环配置，结合基于无透镜散斑传感的定制CNN和基于USB摄像头与VGG16迁移学习的材料分类方法，以及烟雾检测DL模型，动态调节泵功率

Result: 实验证明抽气泵能耗降低20%-50%，在空闲时自动停止，运行时动态调整功率

Conclusion: 该深度学习方法显著降低了激光切割的能耗，为制造业可持续发展做出了重要贡献
...
TL;DR: 提出了Edge-Guided Spatial Attention (EGSA)融合机制，通过将边界信息融入语义和几何特征融合来减轻多任务学习中的负面交互，同时设计了多模态渐进训练策略，在透明物体感知任务上取得了改进。


<details>
  <summary>Details</summary>
Motivation: 透明物体感知是计算机视觉中的主要挑战，透明度会混淆深度估计和语义分割。现有的多任务学习框架存在负面跨任务交互问题，需要改进融合机制来提高鲁棒性。

Method: 1. 提出EGSA融合机制，将边界信息融入语义和几何特征融合；2. 设计多模态渐进训练策略，从RGB图像边缘过渡到深度图像边缘进行学习。

Result: 在Syn-TODD和ClearPose基准测试中，EGSA持续改进了当前最先进方法(MODEST)的深度估计精度，同时保持了有竞争力的分割性能，在透明区域改进最为显著。

Conclusion: 边缘引导融合是一种鲁棒的方法，能够有效改善透明物体感知性能，无需训练时的真实深度数据。
...
TL;DR: 提出了一种仅使用特征损失的知识蒸馏框架，无需logit损失，通过知识质量指标识别教师模型中最有效的层进行蒸馏，在多个数据集和模型上达到SOTA性能
...
Method: 基于潜在表示几何特性的发现，引入知识质量指标来识别教师模型中最有效的层，构建纯特征损失的知识蒸馏框架

Result: 在三个图像分类数据集和四种不同的师生模型对上，该方法达到最先进性能，top-1准确率比标准方法提升高达15%

Conclusion: 仅使用特征损失的知识蒸馏框架是有效的，通过知识质量指标选择最优教师层可以显著提升蒸馏性能
...
<details>
  <summary>Details</summary>
Motivation: 开发一个大规模、公开可用的生成框架，利用预训练和后训练阶段的全部潜力，适应广泛的生成应用，并推动高质量生成模型的发展和可访问性。
...
Conclusion: 作为一个大规模公开生成框架，Kandinsky 5.0通过开源代码和训练检查点的发布，将显著推进高质量生成模型的研究社区发展和可访问性。
...
<details>
  <summary>Details</summary>
Motivation: 金融文档具有视觉密集和表格繁多的布局，其中数值和时间信息与结构紧密耦合。在高风险场景下，微小的OCR错误（如符号反转或日期偏移）可能导致完全不同的解读结果，而传统的ROUGE和编辑距离等OCR指标仅能捕捉表面文本相似性。

Method: 开发了包含500个图像-HTML对的基准数据集，所有标注由金融专家创建和验证，严格控制符号、数值大小和时间表达的质量。建立了LLM-as-Judge评估流程，对视觉复杂的金融文档进行结构化事实提取和上下文验证。

Result: 评估了OCR系统、开源视觉语言模型和专有模型。结果显示，虽然最强的专有模型获得了最高的事实准确性，但在视觉复杂的数值和时间上下文中仍存在显著错误。

Conclusion: FinCriticalED为金融和其他精度关键领域的视觉事实精度提升提供了严格的基础，将评估从词汇重叠转向领域关键的事实正确性。
...
TL;DR: 该论文提出了一种名为CKDA的跨模态知识解耦与对齐方法，用于解决可见光-红外终身行人重识别中的知识遗忘问题，通过分离和平衡模态特定知识与模态通用知识来提升性能。


<details>
  <summary>Details</summary>
Motivation: 现有的跨模态知识蒸馏方法忽视了模态特定知识获取与模态通用知识抗遗忘之间的相互干扰，导致协作性遗忘问题。

Method: 提出跨模态知识解耦与对齐方法(CKDA)，包含模态通用提示模块(MCP)和模态特定提示模块(MSP)来解耦知识，以及跨模态知识对齐模块(CKA)来对齐新旧知识。

Result: 在四个基准数据集上的大量实验验证了CKDA方法的有效性和优越性，优于现有最先进方法。

Conclusion: CKDA方法通过明确分离和平衡模态特定与模态通用知识，有效解决了跨模态终身学习中的知识遗忘问题，在可见光-红外行人重识别任务中表现出色。
...
<details>
  <summary>Details</summary>
Motivation: 传统全息图表示需要逐像素信息存储，参数搜索空间大，计算效率低，限制了下一代计算机生成全息系统的发展

Method: 使用结构化复值2D高斯基元表示全息图，开发可微分光栅化器和GPU优化的自由空间光传播核，支持端到端训练

Result: VRAM使用降低2.5倍，优化速度提升50%，重建保真度更高，噪声伪影得到有效抑制
...
TL;DR: 该研究探讨计算机视觉模型是否在训练过程中表现出与儿童数学能力发展相似的发展轨迹，发现ResNet-50模型在几何和数字概念的学习中确实存在发展对齐现象。


<details>
  <summary>Details</summary>
Motivation: 基于先前研究发现计算机视觉模型在图像分类任务中能学习到与成人相似的几何和数字概念表示，本研究旨在验证这些模型是否也表现出与儿童发展轨迹相似的学习进程。

Method: 使用ResNet-50模型进行详细案例研究，分析其在几何（欧几里得几何、几何图形、度量属性、拓扑学等）和数字概念（心理数字线表示）学习过程中的表现。

Result: 发现ResNet-50在部分几何概念类别（欧几里得几何、几何图形、度量属性、拓扑学）和数字概念（心理数字线）上表现出发展对齐，但在其他类别（手性图形、几何变换、对称图形）上未发现对齐。
...
TL;DR: UniHOI是一个统一的人类-物体交互检测和生成框架，通过共享的token空间实现双向知识迁移，在有限标注下也能有效工作。


<details>
  <summary>Details</summary>
Motivation: 传统方法将HOI检测和生成作为独立任务处理，阻碍了全面交互理解的发展。需要统一的框架来促进知识共享和增强泛化能力。

Method: 提出对称的交互感知注意力模块和统一的半监督学习范式，通过共享token空间实现图像与交互语义之间的双向映射。

Result: 在HOI检测和生成任务上都达到SOTA性能：长尾HOI检测准确率提升4.9%，开放词汇生成任务交互指标提升42.0%。

Conclusion: UniHOI成功证明了统一建模HOI检测和生成任务的有效性，通过知识共享显著提升了两个任务的性能。
...
TL;DR: 提出DLRRF模型解决高光谱与多光谱图像融合中的图像间变异问题，通过将光谱变异建模为光谱退化算子的变化，并分解目标图像为低秩和残差分量来恢复空间细节。


<details>
  <summary>Details</summary>
Motivation: 由于采集条件不同，高光谱图像(HSI)与多光谱图像(MSI)之间存在光谱变异和空间局部变化，这种图像间变异严重影响融合性能，现有方法直接对图像进行变换会加剧融合模型的不适定性。

Method: 1) 将光谱变异建模为光谱退化算子的变化；2) 将目标HSI分解为低秩和残差分量，残差分量捕获丢失的空间细节；3) 利用图像内光谱相关性进行降维；4) 引入隐式正则化器利用空间先验信息；5) 在PnP框架下使用PAO算法求解。

Result: 广泛的数值实验表明，DLRRF在存在图像间变异的HSI和MSI融合中实现了优越的性能。

Conclusion: DLRRF模型通过将光谱变异建模为退化算子变化和分解为低秩残差分量，有效解决了HSI和MSI融合中的图像间变异问题，显著提升了融合性能。
...
<details>
  <summary>Details</summary>
Motivation: 传统视频编辑需要经验丰富的编辑手动完成，现有智能视频编辑技术难以捕捉创作者的独特艺术表达，需要解决镜头组装中的艺术风格学习问题。

Method: 首先通过大语言模型生成脚本并与视频库进行视觉语义匹配获取候选镜头；然后从参考视频中分割标注镜头并提取属性；使用能量模型学习参考风格对候选序列评分；最后结合语法规则优化镜头组装。

Result: 该方法能够自动化地按照特定逻辑、叙事需求或艺术风格排列组合独立镜头，学习参考视频的组装风格，创建连贯的视觉序列。

Conclusion: 提出的能量优化方法能够有效学习参考视频的镜头组装风格，使无视频编辑经验的用户也能创建视觉上吸引人的视频。
...
TL;DR: CellGenNet是一个基于知识蒸馏的细胞分割框架，通过师生架构和混合损失函数，在有限监督下实现跨组织稳健的细胞核分割。
...
Method: 采用师生架构，容量教师模型在稀疏标注上训练并生成软伪标签；学生模型通过结合真实标签和教师概率目标的联合目标优化，使用二元交叉熵和Tversky损失的混合损失函数，并应用一致性正则化和分层dropout。
...
TL;DR: 提出ProPL框架，用于通用半监督超声图像分割，支持多器官多任务，结合有标签和无标签数据，通过提示引导双解码器和不确定性驱动的伪标签校准实现高效分割。
...
Method: 使用共享视觉编码器和提示引导双解码器，通过解码时提示机制实现灵活任务适应，并采用不确定性驱动的伪标签校准模块进行可靠的自训练。
...
<details>
  <summary>Details</summary>
Motivation: 由于一些日文文档采用竖排书写方式，而现有研究对竖排日文文本的关注有限，需要评估MLLMs在这类文本上的阅读能力。

Method: 生成合成日文OCR数据集（包含横竖两种书写方式），并创建真实世界竖排日文文档评估数据集，用于模型微调和评估。

Result: 现有MLLMs在竖排日文文本上的表现明显差于横排文本；使用合成数据集微调可以显著提升模型处理竖排文本的能力。

Conclusion: 竖排日文文本是MLLMs面临的重要挑战，但通过适当的训练数据可以改善模型性能，为多语言文档理解提供了解决方案。
...
TL;DR: 本文探索视频模型通过视频生成进行推理的能力，提出了VR-Bench基准来评估视频模型在空间推理任务上的表现，发现视频模型在空间感知方面优于视觉语言模型。


<details>
  <summary>Details</summary>
Motivation: 类比文本生成到文本推理的发展，探索视频模型是否能够通过视频生成进行推理，特别是利用视频在空间布局和时间连续性方面的优势进行空间推理。

Method: 构建VR-Bench基准，包含7,920个程序生成的迷宫解决视频，涵盖5种迷宫类型和多样视觉风格，使用监督微调(SFT)来激发视频模型的推理能力。

Result: 视频模型在空间感知方面表现更强，优于领先的视觉语言模型，并能很好地泛化到不同场景、任务和复杂度级别。测试时多样化采样可将推理可靠性提升10-20%。
...
Method: 采用流匹配框架，使用交叉注意力机制通过文本提示实现对焦点区域和模糊强度的语义控制，构建四个数据集进行训练和评估。

Result: 实验表明BokehFlow在渲染质量和效率上均优于现有的深度依赖和生成方法，能够产生视觉吸引人的散景效果并提供精确控制。
...
TL;DR: MambaTrack3D是一个基于状态空间模型Mamba的新型3D单目标跟踪框架，专门针对高时序变化(HTV)的室外动态环境设计，通过帧间传播和分组特征增强模块，在保持高精度的同时实现了近线性计算复杂度。


<details>
  <summary>Details</summary>
Motivation: 解决现有基于内存的跟踪器在HTV环境中面临的二次计算复杂度、时序冗余和几何先验利用不足的问题。

Method: 设计了基于Mamba的帧间传播模块(MIP)替代传统单帧特征提取，实现高效帧间传播；引入分组特征增强模块(GFEM)在通道层面分离前景和背景语义，减少内存库中的时序冗余。

Result: 在KITTI-HTV和nuScenes-HTV基准测试中显著优于现有HTV和普通场景跟踪器，在中等时序间隔下比HVTrack提升6.5%成功率和9.5%精度；在标准KITTI数据集上仍保持与最先进跟踪器相当的竞争力。

Conclusion: MambaTrack3D实现了优越的精度-效率权衡，在专业HTV和常规跟踪场景中均表现出鲁棒性能，具有很强的泛化能力。
...
TL;DR: 提出了一种结构感知的手术反馈生成管道，通过学习手术动作本体并利用仪器-动作-目标三元组来指导GPT-4o生成临床基础的手术训练师风格反馈。


<details>
  <summary>Details</summary>
Motivation: 高质量的手术中反馈对改善学员表现和长期技能获取至关重要。自动化生成自然、训练师风格的反馈可以提供及时、可访问且一致的指导，但需要理解临床相关表示的模型。

Method: 1) 从真实训练师-学员转录文本中挖掘仪器-动作-目标三元组并将表面形式聚类为标准化类别；2) 微调视频到IAT模型，利用手术程序和任务上下文以及细粒度时间仪器运动；3) 使用IAT三元组表示指导GPT-4o生成临床基础、训练师风格的反馈。

Result: 视频到IAT识别任务中，上下文注入和时间跟踪带来一致的AUC提升。反馈文本生成任务中，IAT条件化使GPT-4o得分从2.17提升到2.44，可接受生成比例从21%翻倍至42%。传统文本相似性指标也有改善。
...
TL;DR: TiCAL框架通过典型性估计和双曲空间嵌入解决多模态情感识别中的模态间情感冲突问题，在CMU-MOSEI和MER2023数据集上相比现有方法有显著提升。


<details>
  <summary>Details</summary>
Motivation: 现有多模态情感识别方法依赖统一情感标签，忽视了模态间情感冲突问题，即同一样本中不同模态可能表达不同的情感倾向。
...
TL;DR: 提出了Coder-CUA协作框架，让编码语言模型作为GUI设计者，计算机使用代理作为评估者，通过任务可解性和导航成功率来优化界面设计，而不是视觉美观度。


<details>
  <summary>Details</summary>
Motivation: 当前GUI主要为人设计，迫使AI代理采用人类行为模式，而编码语言模型的进步使得自动GUI设计成为可能，需要探索AI代理能否作为评估者辅助自动GUI设计。

Method: 开发AUI-Gym基准测试，包含52个应用和1560个合成任务；建立验证器确保任务可执行性；提出Coder-CUA协作框架，Coder生成和修订网站，CUA评估功能并提供反馈；设计CUA仪表板压缩导航历史为可视化摘要。

Result: 通过将代理定位为设计者和评估者，框架将界面设计转向代理原生效率和可靠性，使代理从被动使用转向主动参与数字环境。

Conclusion: 该工作为自动GUI设计提供了新范式，通过AI代理协作实现更高效的界面设计，代码和数据集已开源。
...
Method: 使用外观先验模块从多视角推断整体身份保持先验，通过联合条件注入机制融合多视角线索并注入共享条件到去噪主干网络

Result: 实验证明JCDM在保真度和跨视角一致性方面达到最先进水平

Conclusion: JCDM支持可变数量的参考视角，可与标准扩散主干网络集成，只需最小且有针对性的架构修改
...
### [35] [When to Think and When to Look: Uncertainty-Guided Lookback](https://arxiv.org/abs/2511.15613)
...
TL;DR: 该论文系统分析了测试时思考对视觉推理的影响，发现更多思考并不总是更好，长推理链容易忽视图像内容。基于成功轨迹中短回顾短语的发现，提出了不确定性引导的回顾解码策略，在多个基准测试中取得显著提升。
...
Method: 对InternVL3.5和Qwen3-VL家族的10个变体进行大规模对比分析，提出了不确定性引导的回顾解码策略，结合不确定性信号、自适应回顾提示和广度搜索，无需额外训练。

Result: 研究表明长推理链往往产生错误轨迹并忽视图像内容；短回顾短语与成功的视觉接地强相关；提出的方法在MMMU基准上提升性能，在标准思考薄弱的类别中获得最大增益，在五个额外基准测试中均表现一致改进。

Conclusion: 测试时思考需要谨慎设计，不确定性引导的回顾解码策略能有效提升视觉推理性能，为视觉语言模型的解码策略提供了新的方向。
...
TL;DR: 本文研究了离散扩散多模态大语言模型(dMLLMs)中的视觉令牌冗余问题，发现不同架构和任务中的冗余模式不同，并提出了针对性的效率优化策略。


<details>
  <summary>Details</summary>
Motivation: 现有dMLLMs在推理时因全序列注意力计算产生显著计算开销，而现有优化方法大多忽视了模态特定的视觉令牌冗余问题。

Method: 通过系统研究不同dMLLM架构和任务中视觉令牌冗余的演化规律，以及视觉令牌剪枝对模型响应和效率的影响。

Result: 发现视觉冗余仅出现在从头训练的dMLLMs处理长答案任务时；视觉令牌剪枝会造成信息损失，但只有从头训练的dMLLMs能在后期去噪步骤中逐步恢复丢失的信息。

Conclusion: 层跳过适用于加速AR到扩散的dMLLMs，而渐进式或后期剪枝对从头训练的dMLLMs更有效，为dMLLMs的效率优化提供了新视角。
...
### [37] [VisPlay: Self-Evolving Vision-Language Models from Images](https://arxiv.org/abs/2511.15661)
*Yicheng He,Chengsong Huang,Zongxia Li,Jiaxin Huang,Yonghui Yang*

Main category: cs.CV

TL;DR: VisPlay是一个自演化的强化学习框架，让视觉语言模型使用无标注图像数据自主提升推理能力，通过角色分配和GRPO训练实现持续改进。


<details>
  <summary>Details</summary>
Motivation: 现有RL方法依赖人工标注或任务特定启发式来定义可验证奖励，成本高且难以扩展，需要一种能自主改进推理能力的可扩展方法。

Method: 将基础VLM分配到两个交互角色：图像条件提问者生成挑战性问题，多模态推理者生成银标签回答，使用GRPO结合多样性和难度奖励进行联合训练。

Result: 在Qwen2.5-VL和MiMo-VL上训练后，在8个基准测试中实现视觉推理、组合泛化和幻觉减少的持续改进，包括MM-Vet和MMMU。

Conclusion: VisPlay为自演化多模态智能提供了一条可扩展的路径，能够有效利用无标注数据提升模型性能。
...
TL;DR: 提出了高斯混合（Gaussian Blending）方法替代传统alpha混合，解决了3D高斯泼溅在未见采样率下的模糊和阶梯伪影问题，保持实时渲染速度且无需额外内存。


<details>
  <summary>Details</summary>
Motivation: 现有3DGS方法在训练未见采样率下会出现缩放时的侵蚀模糊伪影和缩小时的扩张阶梯伪影，这些源于传统alpha混合的基本限制。

Method: 将alpha和透射率视为空间变化分布而非标量，提出高斯混合方法，考虑像素区域内alpha值的空间分布来更新透射率，使附近背景泼溅能贡献最终渲染。

Result: 高斯混合有效捕捉训练未见采样率下的细节，在未见和已见采样率下均优于现有新视角合成模型。

Conclusion: 高斯混合作为即插即用模块，能无缝集成到现有3DGS框架，解决采样率变化导致的视觉差异问题。
...
### [39] [MoDES: Accelerating Mixture-of-Experts Multimodal Large Language Models via Dynamic Expert Skipping](https://arxiv.org/abs/2511.15690)
...
<details>
  <summary>Details</summary>
Motivation: 现有的专家跳过方法原本为单模态大语言模型设计，直接应用于多模态MoE模型会导致显著性能下降，因为这些方法没有考虑MoE层中专家的异构贡献和模态特定行为。

Method: 提出MoDES框架：1) 全局调制局部门控机制，将全局层重要性整合到局部路由概率中；2) 双模态阈值方法，分别处理每个模态的token；3) 利用单调性特性的前沿搜索算法来设置最优阈值。

Result: 在3个模型系列和13个基准测试中，MoDES远超先前方法。当跳过Qwen3-VL-MoE-30B-A3B-Instruct的88%专家时，性能提升达10.67%，预填充时间提升2.16倍，解码时间提升1.26倍。

Conclusion: MoDES是首个无需训练的自适应专家跳过框架，能够实现高效准确的多模态MoE模型推理，显著减少计算开销同时保持甚至提升模型性能。
...
TL;DR: 开发了一个事件触发系统来监测老年人的身体和心理状态，通过GMM背景建模检测访客和老人的运动行为，使用SVM分析图像，并为老年人设计了直观的操作方式通过社交媒体与亲属沟通。


<details>
  <summary>Details</summary>
Motivation: 同时考虑老年人的身体和心理状态，为缺乏技术经验的老年人设计直观的操作方式，通过社交媒体与亲属保持沟通。

Method: 采用GMM背景建模检测运动行为，开发事件触发系统（看门狗、危险通知和照片链接事件），使用SVM机器学习分析图像，设计直观的生活活动操作。

Result: 在家庭场景中进行了实验，5个家庭参与检测和记录三类事件，系统能够有效监测老年人的活动和状态。

Conclusion: 该系统能够有效监测老年人的身体和心理状态，为缺乏技术经验的老年人提供了通过社交媒体与亲属沟通的直观解决方案。
...
### [41] [Think Visually, Reason Textually: Vision-Language Synergy in ARC](https://arxiv.org/abs/2511.15703)
*Beichen Zhang,Yuhang Zang,Xiaoyi Dong,Yuhang Cao,Haodong Duan,Dahua Lin,Jiaqi Wang*

Main category: cs.CV

TL;DR: 论文提出了一种结合视觉和语言协同推理的方法来解决ARC-AGI抽象推理问题，通过视觉-语言协同推理和模态切换自校正策略，在多个基准模型上实现了比纯文本方法高达4.33%的性能提升。


<details>
  <summary>Details</summary>
Motivation: 现有的大模型在从少量示例中推断结构化转换规则方面仍然失败，而人类在解决这类谜题时严重依赖视觉抽象。但直接将ARC-AGI网格渲染为图像会因规则执行不精确而降低性能。

Method: 提出了两种协同策略：1）视觉-语言协同推理，将ARC-AGI分解为模态对齐的子任务；2）模态切换自校正，利用视觉验证基于文本的推理以实现内在错误校正。
...
TL;DR: 提出了一种与SAM集成的无偏语义解码策略，通过同时从支持集和查询集中提取目标信息，利用CLIP模型的语义指导进行一致预测，以解决SAM在少样本分割中对精确提示的依赖问题。


<details>
  <summary>Details</summary>
Motivation: 现有方法主要从支持集中提取提示来激活SAM的泛化能力，但这种设计在适应未知类别时容易导致有偏的解码过程，因此需要一种更无偏的语义解码策略。

Method: 设计了两种特征增强策略：图像级的全局补充和像素级的局部引导，利用CLIP的语义对齐能力丰富SAM特征；并提出可学习的视觉-文本目标提示生成器，通过交互目标文本嵌入和CLIP视觉特征生成目标聚焦的提示嵌入。

Result: 该方法在不重新训练视觉基础模型的情况下，通过具有语义区分性的特征和富含目标信息的提示引导，将注意力集中在目标区域。

Conclusion: 提出的USD策略有效解决了SAM在少样本分割中的提示依赖问题，通过结合CLIP的语义指导实现了更无偏和准确的分割性能。
...
TL;DR: WaveFuse-AL是一个新颖的多策略主动学习框架，通过融合BALD、BADGE、Entropy和CoreSet等策略，结合周期性和性能自适应机制，在医学影像任务中显著提升性能并减少标注成本。


<details>
  <summary>Details</summary>
Motivation: 传统主动学习策略在不同学习阶段表现不一致，需要一种能够自适应调整策略权重的方法来最大化标注预算的效用。

Method: 提出WaveFuse-AL框架，将多种获取策略（BALD、BADGE、Entropy、CoreSet）与周期性时间先验和性能驱动适应相结合，动态调整策略重要性。

Result: 在APTOS-2019、RSNA肺炎检测和ISIC-2018三个医学影像基准测试中，WaveFuse-AL在十二个指标中的十个上取得了统计显著的性能提升，优于单策略和交替策略基线。

Conclusion: WaveFuse-AL通过自适应融合多策略和周期性调整，为医学影像主动学习提供了一种高效且一致的解决方案，显著提升了有限标注预算下的性能表现。
...
TL;DR: DCL-SE是一个端到端框架，通过数据驱动的时空编码和动态课程学习策略，将三维脑部数据高效编码为二维动态表示，在多个神经影像任务中表现优异。
...
Method: 使用近似秩池化(ARP)将三维脑部数据编码为二维动态表示，采用动态课程学习策略和动态组机制(DGM)逐步训练解码器。

Result: 在六个公开数据集（包括阿尔茨海默病分类、脑肿瘤分类、脑动脉分割和脑年龄预测）上，DCL-SE在准确性、鲁棒性和可解释性方面均优于现有方法。

Conclusion: 在大规模预训练网络时代，紧凑、任务特定的架构仍然至关重要。
...
TL;DR: SceneEdited是首个专门为3D点云更新研究设计的城市规模数据集，包含800多个最新场景和2000多个过时版本，模拟了23000多个合成物体变化，用于支持HD地图维护研究。


<details>
  <summary>Details</summary>
Motivation: HD地图会随着环境变化而过时，现有技术虽然能检测变化，但在将2D图像检测到的变化更新到3D地图方面存在明显差距。

Method: 创建了包含校准RGB图像、LiDAR扫描和详细变化掩码的数据集，提供了基于图像结构运动的基础方法用于更新过时场景，并开发了支持可扩展性、可追踪性和可移植性的工具包。

Result: 数据集覆盖73公里驾驶距离和约3平方公里城市区域，包含23000多个手动和自动创建的合成物体变化，模拟了道路基础设施、建筑物、立交桥和电线杆等现实城市修改。
...
TL;DR: 该论文提出了一种新的多模态持续指令调优方法，通过将灾难性遗忘概念化为旧任务梯度缺失问题，利用参数空间的几何特性来近似缺失梯度，有效缓解了灾难性遗忘问题。


<details>
  <summary>Details</summary>
Motivation: 多模态持续指令调优面临灾难性遗忘的挑战，即学习新任务会导致先前任务性能下降。论文将灾难性遗忘重新概念化为新任务学习过程中旧任务梯度缺失的问题。

Method: 利用参数空间的几何特性，使用当前参数与先前最优参数之间的方向向量作为梯度指导来近似缺失梯度，并结合有限回放缓冲区的真实梯度，通过伯努利采样策略动态平衡模型稳定性和可塑性。

Result: 在多模态持续指令调优数据集上的广泛实验表明，该方法在不扩展模型的情况下达到了最先进的性能，有效缓解了灾难性遗忘，同时保持了紧凑的架构。
...
<details>
  <summary>Details</summary>
Motivation: 现有的人体运动预测评估指标无法有效评估多模态预测的覆盖度和运动有效性，往往只关注运动分布的广度而忽略运动模式的多样性和运动合理性。

Method: 提出MMCM指标，通过聚类将运动空间划分为多个模式，并基于数据集收集可能的未来运动来识别有效模式，从而同时评估预测运动的覆盖度和有效性。

Result: 实验验证了聚类方法能够产生合理的模式定义，且MMCM能够准确评估多模态预测的质量。

Conclusion: MMCM是一种有效的多模态人体运动预测评估指标，能够同时考虑预测运动的覆盖度和有效性，优于现有评估方法。
...
<details>
  <summary>Details</summary>
Motivation: 现有评估指标如BLEU、CIDEr、VQA等在领域特定或上下文依赖场景中难以准确捕捉语义和结构准确性。

Method: 三阶段架构：1) 多模态特征提取（目标检测和VLMs）；2) 置信度加权组件融合；3) 物理引导推理（大语言模型强制执行结构和关系约束）。

Result: 开发了PCMDE指标，能够更好地评估语义和结构准确性。

Conclusion: PCMDE通过结合多种技术克服了现有评估指标的局限性，提供了更全面的评估能力。
...
TL;DR: 提出了指令引导的病灶分割新范式，构建了首个大规模CXR病灶分割指令-答案数据集MIMIC-ILS，并开发了ROSALIA视觉语言模型，能够基于简单指令分割多种病灶类型并提供文本解释。
...
Method: 构建MIMIC-ILS数据集（包含110万指令-答案对），使用全自动多模态流程从X光图像和报告中生成标注，并开发ROSALIA视觉语言模型进行微调。

Result: ROSALIA模型在新任务中实现了高分割精度和文本准确性，验证了流程的有效性和MIMIC-ILS作为基础资源的价值。

Conclusion: 指令引导病灶分割范式解决了现有模型的局限性，MIMIC-ILS为像素级CXR病灶定位提供了重要基础资源。
...
TL;DR: 提出BrainRotViT混合架构，结合ViT的全局上下文建模和残差CNN的局部细化，用于从结构MRI准确估计大脑年龄，在11个数据集上达到3.34年MAE，优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 传统回归和CNN方法存在手动特征工程、感受野有限和异构数据过拟合问题，纯transformer模型需要大数据集和高计算成本，需要开发更高效的混合方法。

Method: 使用ViT编码器在辅助年龄和性别分类任务上预训练学习切片级特征，然后将冻结编码器应用于所有矢状切片生成嵌入向量2D矩阵，输入到结合性别信息的残差CNN回归器中估计连续大脑年龄。

Result: 在11个MRI数据集（超过130个采集点）验证集上达到3.34年MAE（Pearson r=0.98，Spearman ρ=0.97，R²=0.95），优于基准和最先进模型，在4个独立队列中MAE在3.77-5.04年之间。

Conclusion: 该方法提供了高效、可解释和可推广的大脑年龄预测框架，弥合了CNN和transformer方法之间的差距，为衰老和神经退行性疾病研究开辟了新途径。
...
Conclusion: Insert In Style是第一个既实用又高保真的零样本生成框架，成功解决了风格化域中对象插入的挑战。
...
TL;DR: 本文提出了一种基于因果理论的食谱-食物图像跨模态检索方法，通过后门调整消除食谱中食材作为混淆变量带来的偏差，在Recipe1M数据集上实现了MedR=1的最优检索性能。
...
Method: 使用因果理论建模跨模态表示学习中的偏差，将食材视为混淆变量源，通过后门调整进行因果干预，在传统检索模型基础上增加去偏项，并提出即插即用的多标签食材分类器模块。

Result: 在Recipe1M数据集上实现了MedR=1的oracle检索性能，在1K、10K和50K测试数据规模下均表现优异，创造了新的最先进搜索性能。

Conclusion: 基于因果理论的去偏方法能有效解决食谱-食物图像跨模态检索中的偏差问题，显著提升检索性能。
...
TL;DR: IPTQ-ViT是一种无需重新训练的全整数视觉Transformer后训练量化框架，通过多项式GELU和移位Softmax近似函数实现完全整数推理，在图像分类和目标检测任务上优于现有PTQ方法。


<details>
  <summary>Details</summary>
Motivation: 现有量化方法存在局限性：QAT需要昂贵的重新训练，PTQ方法要么只能部分量化非线性函数，要么无法实现完全整数推理。需要一种无需重新训练就能实现全整数推理的PTQ方法。

Method: 提出IPTQ-ViT框架：1) 多项式GELU函数针对视觉数据优化；2) 基于位移的Softmax设计提高PTQ中的近似精度；3) 统一度量标准结合量化敏感性、扰动和计算成本来选择每层的最佳近似函数。

Result: 在图像分类上达到最高6.44%p（平均1.78%p）的top-1准确率提升，目标检测上提升1.0 mAP。在W8A8和W4A8设置下优于部分浮点PTQ方法，准确率和延迟与整数QAT方法相当。

Conclusion: IPTQ-ViT首次实现了无需重新训练的全整数视觉Transformer量化，在保持高精度的同时显著降低了计算成本，为资源受限环境提供了实用的量化解决方案。
...
TL;DR: SkinGPT-R1是一个专注于皮肤科的视觉语言模型，通过显式的诊断链式思维推理实现可验证的诊断过程。该模型在DermBench基准测试中排名第一，比Vision-R1平均得分提高41%。


<details>
  <summary>Details</summary>
Motivation: 为了解决皮肤科诊断中推理过程不透明的问题，开发一个能够进行显式、逐步、可验证的链式思维推理的皮肤科专用视觉语言模型。

Method: 构建DermCoT语料库（包含10,000个训练案例和3,000个专家评分案例），定义DermEval六维评估器和DermBench基准，并采用皮肤科感知的视觉蒸馏技术。

Result: 在DermBench上获得4.031/5的平均分，排名第一，比Vision-R1提高41%；在三个皮肤科分类基准上表现稳定且具有竞争力。

Conclusion: DermCoT链式思维监督显著提升基础模型性能，皮肤科感知视觉蒸馏在叙事质量和识别能力上带来持续增益。
...
TL;DR: 提出SplitFlux方法，通过分析Flux模型的特性，发现早期单流块控制内容、后期块控制风格，使用LoRA微调实现内容与风格的解耦，包含秩约束适应和视觉门控LoRA两个关键组件。


<details>
  <summary>Details</summary>
Motivation: 现有SDXL方法难以实现高质量结果，而Flux模型由于特性未充分探索而无法有效分离内容与风格，需要解决内容风格解耦的挑战。

Method: 基于对Flux模型的分析，提出SplitFlux方法：1）秩约束适应：压缩特定块的秩并放大更新幅度，防止内容泄漏到风格块；2）视觉门控LoRA：将内容LoRA分为高秩和低秩分支，高秩保留主体信息，低秩编码残差细节。

Result: 大量实验表明SplitFlux在多种场景下持续优于现有最先进方法，实现了卓越的内容保持和风格化质量。

Conclusion: SplitFlux通过系统分析Flux模型特性并设计相应组件，成功实现了内容与风格的有效解耦，为定制化图像生成提供了高质量解决方案。
...
TL;DR: GQN是一种基于注意力机制的框架，将雷达感知的物体建模为图结构，通过图查询动态关注鸟瞰图空间，使用EdgeFocus进行关系推理和DeepContext Pooling进行上下文聚合，在NuScenes数据集上显著提升检测性能。


<details>
  <summary>Details</summary>
Motivation: 3D雷达检测对360度汽车感知至关重要，但雷达的长波长会产生稀疏且不规则的反射，这挑战了传统的基于网格和序列的卷积和变换器检测器。

Method: 引入图查询网络(GQN)，将雷达感知的物体建模为图，使用图查询动态关注BEV空间，构建对象特定的图，通过EdgeFocus模块进行关系推理和DeepContext Pooling模块进行上下文聚合。

Result: 在NuScenes数据集上，GQN将相对mAP提高了高达+53%，比之前最强的雷达方法提升了+8.2%，同时将峰值图构建开销降低了80%，FLOPs成本适中。
...
TL;DR: 提出了LEO框架，通过将关系视为节点构建线图，实现从关系级上下文到对象级理解的渐进推理，解决了现有方法难以捕捉高阶关系依赖的问题。


<details>
  <summary>Details</summary>
Motivation: 现有3D场景图预测方法采用以对象为中心的图神经网络，关系边特征通过聚合相连对象节点的信息来更新。但这种设计将关系表示限制在成对对象上下文中，难以捕捉对准确关系预测至关重要的高阶关系依赖。

Method: LEO框架首先预测对象对之间的潜在链接以抑制不相关边，然后将原始场景图转换为线图，其中每个关系被视作节点。应用线图神经网络执行以边为中心的关系推理来捕捉关系间上下文。最后将丰富的关系特征集成回原始对象中心图中以增强对象级推理。

Result: 在3DSSG数据集上与两个竞争基线进行的实验显示了一致的改进，证明了我们边到对象推理范式的有效性。

Conclusion: LEO框架是模型无关的，可以与任何现有的对象中心方法集成，通过边到对象的推理范式显著提升了3D场景图预测的性能。
...
TL;DR: 提出一种基于文本到图像生成的一阶段X射线安检图像合成方法Xsyn，无需额外人工成本即可生成高质量图像，并提升违禁品检测性能。


<details>
  <summary>Details</summary>
Motivation: 传统X射线安检图像合成方法采用两阶段流程，需要大量人工进行前景提取，效率低下且成本高昂。

Method: 基于文本到图像生成的一阶段合成流程，采用交叉注意力细化策略优化边界框标注，以及背景遮挡建模策略增强图像复杂性。

Result: 在多个X射线安检数据集和检测器上，该方法比现有方法提升1.2% mAP，合成的图像能有效改善违禁品检测性能。

Conclusion: Xsyn是首个无需额外人工成本即可实现高质量X射线安检图像合成的方法，显著提升了数据合成效率和检测性能。
...
TL;DR: 本文提出了一种针对多模态检索增强生成(MRAG)系统的视觉攻击方法，通过在用户输入的图像上添加微小扰动，能够有效降低系统的检索和生成性能。


<details>
  <summary>Details</summary>
Motivation: 现有的MRAG系统存在安全漏洞，虽然已有研究关注知识投毒攻击，但本文关注仅通过视觉扰动攻击MRAG系统的新场景，这种攻击更具挑战性。

Method: 提出分层视觉攻击方法，通过两阶段策略：首先破坏跨模态对齐，然后破坏多模态语义对齐，使检索器召回不相关知识，从而混淆生成器的输出。

Result: 在OK-VQA和InfoSeek数据集上的实验表明，该方法能显著降低CLIP检索器和BLIP-2、LLaVA等生成模型的性能。

Conclusion: 仅通过视觉扰动就能有效攻击MRAG系统，揭示了这类系统在视觉输入安全方面的新漏洞。
...
Method: 采用从粗到精的定位流程：全局地点识别阶段使用预训练语言模型和分层Transformer，结合注意力点云编码器；提出掩码实例训练和模态感知分层对比学习；精细定位阶段使用基于原型的映射克隆和级联交叉注意力Transformer。

Result: 在KITTI360Pose数据集上性能比现有方法提升高达15%，在新数据集上表现出良好的泛化能力，能有效处理复杂语言表达和多样化城市场景。
...
TL;DR: 提出了一种混合深度学习框架RS-CA-HSICT，结合CNN和Transformer的优势，用于增强MPox检测，在Kaggle基准测试中达到98.30%的分类准确率和98.13%的F1分数。


<details>
  <summary>Details</summary>
Motivation: 利用CNN和Transformer各自的优势来改进MPox检测，CNN擅长提取局部特征，Transformer擅长捕获长距离依赖关系，结合两者可以更好地处理皮肤病变图像。

Method: 构建了包含HSICT块、残差CNN模块、空间CNN块和通道增强的混合架构。HSICT模块集成CNN抽象表示和定制化ICT块，用于多头注意力和结构化CNN层。采用逆残差学习解决梯度消失，阶段分辨率降低确保尺度不变性。通道融合注意力块筛选判别性通道。

Result: 在Kaggle基准和多样化MPox数据集上，分类准确率达到98.30%，F1分数达到98.13%，优于现有的CNN和ViT方法。

Conclusion: RS-CA-HSICT框架通过有效结合CNN和Transformer的优势，在MPox检测任务上取得了最先进的性能，证明了混合架构在医学图像分析中的有效性。
...
TL;DR: Uni-Adapter是一种无需训练、在线测试时自适应策略，通过动态原型学习和图标签平滑来解决3D视觉语言基础模型在实际场景中的性能下降问题。


<details>
  <summary>Details</summary>
Motivation: 现有的3D视觉语言基础模型在开放世界点云处理任务中表现出色，但在实际应用中面对噪声、不完整或分布外数据时性能显著下降，需要一种有效的自适应方法。

Method: 提出基于动态原型学习的3D缓存机制，存储类别特定的聚类中心作为原型，通过相似度评分进行缓存逻辑计算；结合图标签平滑模块捕获原型间相似性；使用熵加权聚合统一原始模型和缓存预测。

Result: 在多个3D基准测试中取得最先进性能，相比源3D视觉语言基础模型，ModelNet-40C提升10.55%，ScanObjectNN-C提升8.26%，ShapeNet-C提升4.49%。

Conclusion: Uni-Adapter无需重新训练即可有效缓解分布偏移问题，为3D视觉语言基础模型在实际应用中的部署提供了可靠的在线自适应解决方案。
...
Method: 创建MILL数据集，包含固定相机设置和精确光照测量下的多光照强度图像。利用数据集的多光照结构改进现有算法，提升在不同光照场景下的鲁棒性。

Result: 基准测试显示现有方法在不同光照强度下性能差异显著。改进后的方法在DSLR上PSNR提升达10 dB，智能手机上提升2 dB。

Conclusion: MILL数据集为低光增强研究提供了重要的评估基准，改进方法显著提升了算法在不同光照条件下的鲁棒性和性能。
...
Method: 设计并评估了一个多模态Transformer模型，利用自注意力机制融合雷达、RGB视频、红外视频和音频等不同模态的特征，学习全面、互补且具有高度区分性的表示用于分类。

Result: 在独立测试集上取得了0.9812准确率、0.9873召回率、0.9787精确率、0.9826 F1分数和0.9954特异性的优异性能，特别在区分无人机与其他空中物体方面表现出高精确率和召回率。计算效率为1.09 GFLOPs、122万参数，推理速度41.11 FPS。

Conclusion: 该研究通过Transformer架构的多模态数据融合，在航空物体分类方面取得了显著进展，为复杂空域中的无人机检测和监控提供了高精度、鲁棒的解决方案。
...
TL;DR: FIA-Flow是一个黑盒特征反演攻击框架，能够从中间特征实现高保真图像重建，揭示Split DNNs中比以往认知更严重的隐私风险。
...
Result: 在多种模型(AlexNet、ResNet、Swin Transformer、DINO、YOLO11)和层上实现了更忠实和语义对齐的特征反演，证明了Split DNNs中存在比以往认知更严重的隐私威胁。

Conclusion: FIA-Flow框架揭示了Split DNNs中更严重的隐私风险，为评估和缓解此类系统中的隐私泄露提供了重要工具。
...
<details>
  <summary>Details</summary>
Motivation: 指纹活体检测系统面临欺骗攻击的严重威胁，需要开发能够区分真假指纹的技术，并且系统需要抵抗各种失真，防止攻击者通过添加失真来欺骗检测器。

Method: 使用各向异性扩散处理输入图像，通过三级小波变换，对不同层的系数进行自适应阈值处理并拼接成特征向量，最后使用SVM分类器进行分类。

Result: 在像素缺失90%和70x70块缺失场景下，准确率分别比同类方法提高了约8%和5%，显示出对多种失真的强鲁棒性。

Conclusion: 提出的方法在应对环境现象或恶意用户操作引起的各种失真方面表现出改进的抵抗能力，验证了该新颖方法在解决此类挑战方面的有效性。
...
<details>
  <summary>Details</summary>
Motivation: 现有开放集检测方法通常依赖单一不确定性评分和阈值处理，限制了灵活性且容易将未知物体与背景混淆。无人机导航需要可靠地区分已知目标、未知物体和背景区域。

Method: 使用融合方案聚合多个置信度估计和每个检测的特征，通过紧凑的多层感知机(MLP)实现三分类。将不同logit变体纳入MLP，在不影响吞吐量的情况下提升性能。

Result: 在二分类中比基于阈值的方法平均提高2.7% AUROC，同时保持或改进开放集mAP。在闭集mAP上最多提升9个百分点（18%相对增益）。

Conclusion: 该方法为安全无人机导航提供了关键能力，能够主动避开未知物体并安全忽略背景区域，在多个数据集上均优于竞争技术。
...
<details>
  <summary>Details</summary>
Motivation: 现有3D点云跟踪器面临双重冗余挑战：空间冗余（背景噪声影响精度）和信息冗余（前景内部冗余影响效率），需要系统性地消除这两种冗余。

Method: CompTrack包含两个核心模块：1) 空间前景预测器(SFP)基于信息熵过滤背景噪声；2) 信息瓶颈引导的动态令牌压缩(IB-DTC)模块，通过在线SVD分析自适应压缩前景冗余为紧凑的代理令牌。

Result: 在KITTI、nuScenes和Waymo数据集上的实验表明，CompTrack实现了顶级的跟踪性能，在单张RTX 3090 GPU上达到实时90 FPS的高效率。

Conclusion: CompTrack通过系统性消除点云中的空间和信息冗余，在保持高精度的同时实现了高效的实时3D目标跟踪。
...
TL;DR: SA-FARI是最大的开源野生动物多动物追踪数据集，包含11,609个摄像机陷阱视频，涵盖4大洲741个地点的99个物种，提供16,224个个体身份和942,702个标注框，用于推进通用多动物追踪模型的发展。


<details>
  <summary>Details</summary>
Motivation: 现有野生动物追踪数据集规模有限、物种单一、缺乏时空多样性，缺乏适用于跨野生动物种群的通用多动物追踪模型的训练基准。

Method: 收集了2014-2024年间来自4大洲741个地点的11,609个摄像机陷阱视频，对99个物种进行密集标注，包括边界框、分割掩码和物种标签，并提供匿名化位置信息。

Result: 创建了包含16,224个个体身份和942,702个标注的大规模数据集，使用最先进的视觉语言模型进行基准测试，包括SAM 3，并与专门的野生动物分析方法进行比较。

Conclusion: SA-FARI是首个结合高物种多样性、多区域覆盖和高质量时空标注的大规模数据集，为推进野外通用多动物追踪提供了新的基础。
...
TL;DR: AutoPrune是一种新颖的LLM剪枝方法，通过让LLM自动设计剪枝算法来克服传统方法依赖专家知识的问题，并解决了高剪枝率下的异常值问题。
...
### [73] [GEO-Bench-2: From Performance to Capability, Rethinking Evaluation in Geospatial AI](https://arxiv.org/abs/2511.15658)
...
<details>
  <summary>Details</summary>
Motivation: 当前地理空间基础模型评估缺乏标准化协议，需要建立统一的评估框架来促进模型公平比较和方法创新。

Method: 构建包含19个许可数据集的综合基准，涵盖分类、分割、回归、目标检测和实例分割任务，引入能力分组概念，并制定规范但灵活的评估协议。

Result: 实验显示没有单一模型在所有任务中都表现最优，自然图像预训练模型在高分辨率任务中表现优异，而EO专用模型在多光谱应用中表现更好。

Conclusion: 最佳模型选择取决于任务需求、数据模态和约束条件，实现单一通用地理空间基础模型仍是未来研究目标，GEO-Bench-2为特定用例提供可复现的评估框架。
...
### [74] [ShelfOcc: Native 3D Supervision beyond LiDAR for Vision-Based Occupancy Estimation](https://arxiv.org/abs/2511.15396)
*Simon Boeder,Fabian Gigengack,Simon Roesler,Holger Caesar,Benjamin Risse*

Main category: cs.CV

TL;DR: ShelfOcc是一种仅使用视觉的3D占据估计方法，通过视频生成度量一致的语义体素标签，实现真正的3D监督，无需LiDAR或手动3D标注。


<details>
  <summary>Details</summary>
Motivation: 现有的自监督和弱监督占据估计方法依赖2D投影或渲染监督，存在几何不一致和深度渗漏问题。需要一种不依赖LiDAR的纯视觉方法来解决这些限制。
...
### [75] [Controlling False Positives in Image Segmentation via Conformal Prediction](https://arxiv.org/abs/2511.15406)
...
<details>
  <summary>Details</summary>
Motivation: 深度模型在临床决策中很少提供明确的统计误差保证，过度分割可能带来临床后果，需要风险感知的分割方法。

Method: 使用预训练分割模型，通过增加分数阈值或形态学腐蚀定义收缩掩码族，利用标记校准集选择收缩参数，确保假阳性比例低于用户指定容忍度。
...
### [76] [MF-GCN: A Multi-Frequency Graph Convolutional Network for Tri-Modal Depression Detection Using Eye-Tracking, Facial, and Acoustic Features](https://arxiv.org/abs/2511.15675)
*Sejuti Rahman,Swakshar Deb,MD. Sameer Iqbal Chowdhury,MD. Jubair Ahmed Sourov,Mohammad Shamsuddin*

Main category: cs.CV

TL;DR: 提出多频图卷积网络(MF-GCN)用于抑郁症检测，通过眼动追踪、音频和视频数据，利用多频滤波器组模块同时捕捉低频和高频信号，在二元和三元分类任务中均优于基线模型。
...
Method: 提出多频图卷积网络(MF-GCN)，包含新颖的多频滤波器组模块(MFFBM)，能够同时利用低频和高频信号进行跨模态交互分析。

Result: 二元分类敏感度0.96，F2分数0.94；三元分类敏感度0.79，特异性0.87；在CMDC数据集上敏感度0.95，F2分数0.96，均显著优于其他模型。
...
TL;DR: D4C是针对CLIP模型的数据无关量化框架，通过语义注入、结构对比生成和扰动增强三个组件，解决了现有DFQ方法在CLIP上性能下降的问题。
...
Method: 提出D4C框架，包含三个关键组件：1) 提示引导语义注入，使用文本提示对齐生成图像与真实语义；2) 结构对比生成，利用前景-背景对比合成重现自然图像的组合结构；3) 扰动感知增强，应用受控扰动提高样本多样性和鲁棒性。
...
Conclusion: D4C成功解决了CLIP模型数据无关量化的关键挑战，通过生成语义丰富且结构多样的伪图像，显著提升了量化性能，为隐私敏感场景下的模型压缩提供了有效解决方案。
...
TL;DR: WarNav是一个从DATTALION开源仓库图像构建的新型真实世界数据集，专门用于在冲突影响环境中开发自动驾驶地面车辆的语义分割模型，填补了传统城市驾驶资源与危险战区场景之间的关键空白。


<details>
  <summary>Details</summary>
Motivation: 解决传统城市驾驶数据集与危险战区无人系统操作场景之间的差距，促进自动驾驶车辆在高风险环境中的鲁棒性和安全性研究。

Method: 从DATTALION开源仓库构建数据集，面临数据异质性和伦理考虑等挑战；使用在结构化城市场景上训练的最先进语义分割模型建立性能基准；分析训练数据环境的影响，并提出在目标图像无标注情况下实现有效导航性的初步方法。

Result: 建立了WarNav数据集的性能基准，展示了现有模型在冲突环境中的表现，并提出了在无标注数据约束下实现导航性的方法。

Conclusion: WarNav数据集为在极端操作环境下开发鲁棒自动驾驶系统提供了重要资源，同时强调在数据标注有限的情况下实现有效导航的可行性，旨在推动高风险场景中自动驾驶车辆的安全性和鲁棒性研究。
...
Result: 在FLIR、LLVIP、M3FD和MFAD数据集上的广泛实验表明，该方法有效缓解了融合退化问题，在多个基准测试中达到了最先进性能。
...
Method: 创建包含两种汽车变速箱典型齿轮箱部件的图像数据集，通过不同的训练-测试分割生成分布偏移，并提出了对比正则化损失来增强模型鲁棒性。

Result: 实验结果表明，对比正则化损失能够有效提升模型对未见部件类型的泛化能力。

Conclusion: 提出的对比正则化损失方法能够显著改善深度神经网络在再制造质量检测任务中的泛化性能。
...
TL;DR: 提出EASD端到端脉冲相机检测器，解决自动驾驶中运动模糊和饱和问题，采用双分支设计融合全局语义和物体细节，并创建首个驾驶导向的模拟脉冲检测基准DSEC Spike。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶中的物体检测在快速运动和极端光照下存在运动模糊和饱和问题，脉冲相机具有微秒级延迟和超高动态范围，但其稀疏离散输出无法被标准图像检测器处理，需要端到端的脉冲流检测方法。

Method: 提出EASD端到端脉冲相机检测器，采用双分支设计：时间纹理加特征融合分支用于全局跨切片语义，熵选择注意力分支用于物体中心细节。同时创建DSEC Spike模拟脉冲检测基准数据集。

Result: 该方法解决了脉冲相机输出稀疏离散无法被标准检测器处理的问题，实现了端到端的脉冲流物体检测。

Conclusion: EASD通过双分支设计有效融合了脉冲相机的全局语义和物体细节信息，DSEC Spike基准填补了驾驶导向脉冲检测数据空白，为自动驾驶中的物体检测提供了新解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法通常将HE图像切片与对应的ST谱在单一尺度上对齐，忽略了细粒度细胞结构及其空间组织，需要多尺度方法来更好地捕捉组织微环境中的细胞相互作用。

Method: 提出Sigmma框架，包含多尺度对比对齐确保不同尺度表示在模态间保持一致，并将细胞相互作用表示为图，整合图间和图内关系来捕捉从细到粗的细胞-细胞相互作用。

Result: Sigmma在基因表达预测任务中平均提升9.78%，在跨模态检索任务中平均提升26.93%，并在下游分析中学习了有意义的多组织组织。

Conclusion: Sigmma通过多尺度对比对齐和图表示细胞相互作用，能够更好地捕捉跨模态对应关系，为计算病理学提供了有效的层次表示学习方法。
...
TL;DR: 本研究开发了一个AI多阶段流水线，用于从围网渔船的电子监控视频中自动识别和分类金枪鱼物种，特别是区分大眼金枪鱼和黄鳍金枪鱼，以解决专家识别困难的问题。
...
Method: 开发多阶段流水线：比较三种分割方法（Mask R-CNN、DINOv2+SAM2、YOLOv9+SAM2），使用ByteTrack进行个体跟踪，评估标准多类分类和分层分类方法。

Result: YOLOv9-SAM2表现最佳，验证平均精度0.66±0.03，召回率0.88±0.03。结合分层分类，84.8%的个体被分割和分类，平均误差4.5%。专家间识别一致性较低（大眼金枪鱼42.9%，黄鳍金枪鱼57.1%）。

Conclusion: YOLOv9-SAM2结合分层分类的方法能有效估计金枪鱼渔获物物种组成，为电子监控系统提供可靠的自动化分析解决方案。
...
TL;DR: FunnyNodules是一个完全参数化的合成数据集，用于系统分析医学AI模型中的基于属性的推理，通过控制视觉属性生成肺结节样形状，提供完整的真实信息来评估可解释AI方法。


<details>
  <summary>Details</summary>
Motivation: 医学图像数据集中缺乏捕获诊断背后推理过程的密集标注，这些标注对于开发和评估能够像放射科医生一样推理的可解释AI模型至关重要。

Method: 创建FunnyNodules合成数据集，生成具有可控视觉属性（如圆形度、边缘锐度、毛刺等）的抽象肺结节样形状，目标类别来自预定义的属性组合，完全控制属性与诊断类别之间的决策规则。
...
<details>
  <summary>Details</summary>
Motivation: 现有自回归方法存在效率问题，要么是逐个token解码，要么是多尺度表示的复杂性。受人类视觉系统中心向外感知模式的启发，旨在提高自回归图像生成的效率和感知对齐性。

Method: 提出扩展自回归表示(EAR)，以螺旋顺序从中心向外展开图像token，保持空间连续性并实现高效并行解码。还提出长度自适应解码策略，动态调整每步预测的token数量。
...
TL;DR: MTGNet是一个基于多文本引导的双分支小样本语义分割网络，通过融合多样化的文本提示来增强文本先验，并指导视觉先验的跨模态优化，解决了传统方法中目标区域激活不完整和缺乏显式跨模态交互的问题。


<details>
  <summary>Details</summary>
Motivation: 现有的基于CLIP的小样本语义分割方法通常使用单一文本提示，无法充分捕捉复杂类别的语义多样性，导致目标区域激活不完整。同时缺乏显式跨模态交互，容易受到噪声支持特征的影响，进一步降低视觉先验质量。
...
Conclusion: MTGNet通过多文本引导和跨模态优化，有效提升了小样本语义分割性能，特别是在处理复杂结构和类内变异较大的类别时表现优异。
...
Conclusion: 该框架能够实现局部、全局和关系特征表示，具有高可解释性和适应性，可部署到边缘设备进行实时高效杂草检测，减少对除草剂的依赖。
...
TL;DR: 本文评估了人体姿态预测算法，发现可复现性问题，提出了统一的训练评估流程，并将语音模型成功应用于姿态预测任务，提升了SOTA性能。同时评估了模型在噪声姿态数据下的鲁棒性。
...
Method: 构建统一的训练和评估流程，将语音理解模型适配到姿态预测任务，并引入基于姿态估计器生成噪声数据的新数据集变体来评估模型鲁棒性。

Result: 语音模型在姿态预测任务中表现优异，提升了当前SOTA性能；噪声姿态数据会导致性能显著下降，但通过无监督微调可以部分恢复性能。

Conclusion: 语音模型可有效应用于姿态预测任务，但在真实应用中需要考虑姿态估计噪声的影响，无监督微调是缓解性能下降的有效方法。
...
<details>
  <summary>Details</summary>
Motivation: 现有AI生成图像检测器在干净条件下表现出色，但对抗取证方面的安全性评估不足。需要开发先进的对抗攻击方法来评估这类检测器的安全性。

Method: 通过空间插值梯度和频率感知扰动捕获取证重要特征，联合建模空间和频域特征重要性，融合后指导基于优化的对抗样本生成。
...
TL;DR: 本文分析了ViT特征蒸馏失败的原因，发现虽然ViT表示是全局低秩的，但单个token使用高带宽编码模式，导致师生模型间存在编码不匹配。提出了两种简单策略：后验特征提升和原生宽度对齐，成功激活了ViT特征蒸馏。
...
Method: 1. 对ViT进行两层表示分析：层间SVD分析和token级谱能量模式分析；2. 提出两种策略：后验特征提升（轻量投影器）和原生宽度对齐（仅加宽学生模型最后块）。

Result: 在ImageNet-1K上，这些策略重新激活了ViT特征蒸馏，将DeiT-Tiny的准确率从74.86%提升到77.53%和78.23%（从CaiT-S24蒸馏），同时改善了无教师训练的独立学生模型。

Conclusion: ViT特征蒸馏失败的原因是师生模型间的编码不匹配，通过利用低秩结构可以设计出有效、可解释的补救措施，为紧凑ViT提供具体设计指导。
...
TL;DR: AVATAAR是一个模块化、可解释的视频问答框架，通过结合全局和局部视频上下文、预检索思考代理和重新思考模块，实现了类似人类的迭代推理，在长视频问答任务中显著提升了性能。
...
Result: 在CinePile基准测试中，AVATAAR相比基线模型在时间推理上提升5.6%，技术查询提升5%，主题问题提升8%，叙事理解提升8.2%。每个模块都对整体性能有积极贡献，反馈循环对适应性至关重要。
...
<details>
  <summary>Details</summary>
Motivation: 现有的LiDAR地点识别方法在新环境中难以适应且容易遗忘之前学到的知识，即灾难性遗忘问题。
...
TL;DR: 提出一种多模态深度学习方法来补全3D超声中被遮挡的解剖结构，通过利用单张X射线图像的互补信息来解决超声成像中骨遮挡导致的结构不完整问题。


<details>
  <summary>Details</summary>
Motivation: 超声成像在脊柱手术中具有无辐射、成本低、实时可视化等优势，但由于骨骼的声影效应，无法完整显示椎体等解剖结构。需要解决超声成像的这一关键限制。
...
TL;DR: 提出了一种统一的解耦分割头，将多类预测分离为类无关掩码预测和类标签预测，并引入了全尺度感知可变形Transformer模块，在AMOS 2022和BTCV数据集上超越了nnUNet。


<details>
  <summary>Details</summary>
Motivation: 传统的逐点卷积分割头将每个输出通道与特定类别绑定，这种刚性设计限制了特征共享和语义泛化能力。

Method: 使用共享对象查询分离类无关掩码预测和类标签预测，引入全尺度感知可变形Transformer模块，通过可变形注意力实现低分辨率编码器特征对全分辨率编码器特征的跨尺度关注。

Result: 在AMOS 2022数据集上Dice指标提升+2.0%，在BTCV数据集上Dice指标提升+6.9%，达到了最先进的性能。

Conclusion: MaskMed方法通过解耦分割头和全尺度特征融合，有效提升了医学图像分割的性能和泛化能力。
...
TL;DR: FlashMesh是一个快速高保真的网格生成框架，通过预测-纠正-验证范式重新设计自回归解码，利用网格令牌的结构和几何相关性实现多令牌推测，在保持质量的同时实现2倍加速。


<details>
  <summary>Details</summary>
Motivation: 自回归模型虽然能生成高质量3D网格，但其逐令牌解码导致推理速度慢，限制了在交互式和大规模应用中的实用性。

Method: 提出FlashMesh框架，采用预测-纠正-验证范式，利用网格令牌的结构和几何相关性，为hourglass transformer架构量身定制推测解码方案，实现面、点、坐标级别的并行预测。
...
<details>
  <summary>Details</summary>
Motivation: 现有的基于CLIP的类增量学习方法未能显式捕捉视觉和语言概念的内在层次结构，导致细粒度类别特征漂移和灾难性遗忘

Method: 使用外部知识图谱作为监督，在双曲空间中嵌入视觉和文本特征以保持层次结构；将梯度投影到共享双曲映射器的零空间来防止对先前任务的干扰

Result: 大量实验表明HASTEN持续优于现有方法，同时提供统一的结构化表示

Conclusion: HASTEN通过维护层次关系使模型能够抵抗遗忘，为类增量学习提供了有效的层次语义锚定方案
...
TL;DR: 提出了MUSSE-Net，一种多阶段无监督深度学习框架，用于超声应变弹性成像中的稳健应变估计，通过残差感知和序列处理提升性能。
...
Method: MUSSE-Net采用多阶段无监督序列深度学习框架，包括USSE-Net作为骨干网络，该网络是多流编码器-解码器架构，并行处理变形前后的RF序列，结合上下文感知互补特征融合编码器、三交叉注意力瓶颈和交叉注意力融合解码器，并采用一致性损失确保时间连贯性。

Result: 在模拟、体内和临床数据集上的验证显示，MUSSE-Net在模拟数据上达到目标SNR 24.54、背景SNR 132.76、CNR 59.81和弹性成像SNR 9.73的优异性能，在BUET数据集上产生增强病变-背景对比度和显著噪声抑制的应变图。

Conclusion: MUSSE-Net在超声应变弹性成像中实现了最先进的性能，提供了临床可解释的应变模式，显著提升了应变估计的准确性和一致性。
...
Method: 提出MambaIO方法：1）使用拉普拉斯金字塔将IMU测量值分解为高频和低频分量；2）低频分量通过Mamba架构提取隐含的上下文运动线索；3）高频分量通过卷积结构捕捉细粒度的局部运动细节。
...
TL;DR: INQUIRE-Search是一个开源系统，利用自然语言在大规模生物多样性图像数据库中搜索特定概念，验证并导出相关观测数据，用于科学分析。


<details>
  <summary>Details</summary>
Motivation: 大型社区科学平台如iNaturalist包含数亿张生物多样性图像，这些图像通常包含行为、互动、物候和栖息地等生态背景信息，但传统方法无法大规模访问这些次要信息。

Method: 开发INQUIRE-Search系统，通过自然语言交互式搜索生态图像数据库中的特定概念，验证和导出相关观测数据。

Result: 通过五个案例研究展示了INQUIRE-Search支持的科学应用多样性，从跨物种行为的季节性变化到野火后森林再生。相比传统方法，INQUIRE-Search只需一小部分时间。

Conclusion: INQUIRE-Search代表了一种交互式、高效且可扩展的科学发现新范式，能够解锁大规模生物多样性数据集中以前无法访问的科学价值，并呼吁专家重新构建科学过程的优先事项。
...
<details>
  <summary>Details</summary>
Motivation: 解决高光谱图像分类中在有限监督数据下实现准确和鲁棒预测的挑战。

Method: 集成3D卷积层提取局部光谱空间特征，使用两个并行MLP风格的混合器块捕获光谱和空间维度的长程依赖关系，并采用深度卷积注意力机制增强判别能力。
...
TL;DR: 论文揭示了视频生成模型将第一帧作为概念记忆缓冲区的隐含机制，仅需20-50个训练样本即可实现鲁棒的视频内容定制，无需架构修改或大规模微调。


<details>
  <summary>Details</summary>
Motivation: 传统观点将视频第一帧视为时空起点，但研究发现视频模型实际上将其作为存储视觉实体的概念记忆缓冲区，为后续生成提供可重用的视觉元素。

Method: 利用视频模型隐含的第一帧概念记忆机制，通过少量训练样本（20-50个）进行视频内容定制，无需修改模型架构或进行大规模微调。

Result: 实现了在各种场景下鲁棒且通用的视频内容定制，展示了视频生成模型在基于参考的视频定制方面被忽视的强大能力。

Conclusion: 视频生成模型具有强大的基于参考的视频定制能力，这一能力源于其将第一帧作为概念记忆缓冲区的隐含机制，可通过少量样本有效激活。
...
TL;DR: 提出了GeoVista智能体模型和GeoBench基准测试，用于评估智能体在地理定位任务中的表现，通过结合图像放大和网络搜索工具，在推理循环中实现精准地理定位。


<details>
  <summary>Details</summary>
Motivation: 当前智能体视觉推理研究主要关注图像处理工具，缺乏面向通用智能体模型的地理定位能力评估。现有地理定位基准无法满足高分辨率图像和深度智能体推理的需求。

Method: 提出GeoVista智能体模型，在推理循环中无缝集成工具调用（图像放大和网络搜索工具）。采用完整训练流程：冷启动监督微调学习推理模式和工具使用先验，然后通过强化学习阶段增强推理能力，使用分层奖励利用多级地理信息。

Result: 实验结果显示GeoVista在地理定位任务上大幅超越其他开源智能体模型，在大多数指标上达到与Gemini-2.5-flash和GPT-5等闭源模型相当的性能。

Conclusion: GeoVista通过集成工具调用和分层强化学习训练，在地理定位任务上表现出色，填补了智能体视觉推理中地理定位能力的空白。
...
TL;DR: 提出了一种新的密集特征匹配架构，通过系统改进解决了现有匹配器在复杂场景下性能不佳和速度慢的问题，实现了更高的准确性和效率。


<details>
  <summary>Details</summary>
Motivation: 现有密集特征匹配方法在复杂现实场景中表现不佳，高精度模型往往速度较慢，限制了实际应用。

Method: 构建了新颖的匹配架构和损失函数，结合多样化的训练分布；采用解耦的两阶段匹配-精炼流程加速训练；通过定制CUDA内核减少精炼阶段内存使用；利用DINOv3基础模型提升鲁棒性和无偏性。

Result: 在广泛实验中，新匹配器显著优于先前方法，达到了新的最先进水平。

Conclusion: 通过系统性的架构改进和训练优化，成功开发出在准确性和效率上都显著提升的密集特征匹配模型。
...
TL;DR: 本文综述了通过增加推理时计算来提高预训练大语言模型预测准确性的方法，特别关注问题分解方式和子问题的拓扑结构（顺序、并行或树状），统一了多种现有技术并分析了各自的优缺点。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型的发展，如何在不重新训练模型的情况下通过优化推理过程来提高预测准确性成为一个重要研究方向。现有方法如Chain-of-Thought等虽然有效但缺乏系统性的分类和分析框架。

Method: 采用分类学方法，特别关注问题分解策略和子问题拓扑结构，将Chain-of-Thought、Branch-Solve-Merge、Tree-of-Thought等技术统一在同一个分析框架下进行比较。

Result: 建立了一个统一的分类框架，能够系统性地分析和比较不同测试时扩展方法，揭示了各种方法在问题分解和拓扑结构方面的共性和差异。

Conclusion: 测试时扩展方法是一个有前景的研究方向，本文提出的分类框架为未来研究提供了理论基础，并指出了多个值得探索的研究方向。
...
TL;DR: 研究发现LLM在推理过程中很早就内部确定了最终答案，即使需要更长的输出才能得到明确答案。对于难题，预测准确率下降揭示了选择偏差：难题在长推理链中占比过高。


<details>
  <summary>Details</summary>
Motivation: 探究大型语言模型在推理过程中何时内部确定最终答案，以及这种早期承诺对模型解释性和推理时控制的影响。

Method: 通过在推理过程的前t个token后的隐藏状态上训练线性分类器，来预测最终答案的正确性。

Result: 即使需要更长的输出才能得到明确答案，模型在仅经过几个token后就能高度预测最终正确性。对于难题，预测准确率下降，表明难题在长推理链中占比过高。

Conclusion: 推理模型的内部自我评估在仅经过几个token后就趋于出现，这对模型解释性和推理时控制具有重要意义。
...
TL;DR: LiveCLKTBench是一个自动生成管道，专门用于隔离和测量大型语言模型的跨语言知识迁移，通过时间敏感的知识实体来评估真实的知识转移。


<details>
  <summary>Details</summary>
Motivation: 评估大型语言模型的跨语言知识迁移具有挑战性，因为目标语言中的正确答案可能来自真正的知识迁移，也可能来自预训练期间的先前接触。需要一种能够隔离和测量真实跨语言知识转移的方法。

Method: 开发了LiveCLKTBench自动化生成管道，从现实领域识别自包含、时间敏感的知识实体，基于时间发生进行过滤，并验证模型的知识。使用这些有效实体的文档生成事实性问题，翻译成多种语言来评估跨语言边界的可迁移性。

Result: 评估了多个LLM在五种语言上的表现，发现跨语言迁移受语言距离的强烈影响，并且在语言方向上通常不对称。虽然更大的模型提高了迁移效果，但收益随规模增加而减少，并且在不同领域间存在差异。
...
Method: 使用非自回归模型FastSpeech 2，在巴西葡萄牙语上评估手动和自动韵律分割标注的效果，分析其对语音合成质量的影响。

Result: 韵律分割训练产生略微更清晰和声学自然的语音。自动分割产生更规则的片段，手动分割引入更大变异性，有助于更自然的韵律。两种方法都能重现预期的核重音模式，但韵律模型更接近自然的核前轮廓。

Conclusion: 韵律分割标注能改善语音合成的自然度和清晰度，手动分割在韵律变异性方面表现更好。所有数据集、源代码和训练模型已公开供复现和未来研究使用。
...
TL;DR: EasyMED是一个多智能体框架，通过结合患者代理、辅助代理和评估代理，实现了标准化患者的模拟，在SPBench基准测试中达到与人类SP相当的学习效果，同时具有更好的灵活性、心理安全性和成本效益。


<details>
  <summary>Details</summary>
Motivation: 标准化患者(SP)在临床技能训练中不可或缺，但存在成本高、灵活性差、难以扩展的问题。现有的基于大语言模型的SP模拟器虽然成本较低，但行为不一致且缺乏与人类SP的严格比较。

Method: 提出了EasyMED多智能体框架，包含三个组件：患者代理用于生成真实对话，辅助代理确保事实一致性，评估代理提供可操作的反馈。同时引入了SPBench基准，包含14个专科的真实SP-医生交互数据和8个专家定义的评估标准。

Result: 实验表明EasyMED能够匹配人类SP的学习效果，对基线水平较低的学生产生更大的技能提升，同时提供更好的灵活性、心理安全性和成本效益。

Conclusion: EasyMED框架为解决标准化患者训练的可扩展性和成本问题提供了有效解决方案，在保持学习效果的同时显著改善了训练体验和效率。
...
Method: 引入混合深度神经网络模型，结合双向门控循环单元（BGRU）和长短期记忆（LSTM）层，构建HBGRU-LSTM架构进行情感分析。

Result: HBGRU-LSTM模型在测试集上达到95%准确率，优于LSTM（93.06%）、CNN+LSTM（93.31%）和GRU+LSTM（92.20%）。在平衡数据集上，负面情感召回率从86%提升到96%，误分类损失从20.24%降至13.3%。

Conclusion: 提出的HBGRU-LSTM模型在情感分析任务中表现出色，能够有效处理上下文细微差别、可扩展性和类别不平衡问题，具有更好的泛化能力和鲁棒性。
...
TL;DR: 提出了分层令牌预置（HTP）方法，通过分块预置摘要令牌和多路径信息流，解决了大语言模型中因果注意力机制导致的信息流限制问题，显著提升了长文档嵌入质量。


<details>
  <summary>Details</summary>
Motivation: 大语言模型的因果注意力机制限制了从后向前的信息流动，降低了表示质量。现有方法通过在开头预置单个摘要令牌来尝试解决，但会导致信息过度压缩，损害长文档性能。
...
Conclusion: HTP作为一种简单、架构无关的方法，提升了零样本和微调模型的性能，为构建优质长文档嵌入提供了可扩展路径。
...
<details>
  <summary>Details</summary>
Motivation: 大语言模型虽然强大但容易产生幻觉（看似合理但事实错误或无依据的输出），需要系统性的方法来理解和解决这一问题。

Method: 使用概率建模、信息理论、三角信号分析和贝叶斯不确定性估计，分析自回归过程中的错误累积，提出语义和相位感知的不确定性度量，开发对比解码、检索增强、事实对齐和弃权等缓解策略。

Result: 建立了一个统一的理论框架，将校准、检索和对齐等最新进展联系起来，支持更安全可靠的大语言模型。

Conclusion: 该数学框架为理解和减轻LLM幻觉提供了理论基础，有助于开发更安全可靠的语言模型系统。
...
TL;DR: TASA是一个学生感知的数学辅导框架，通过整合学生画像、记忆和遗忘动态，实现个性化数学学习。它通过结构化学生画像和事件记忆，结合连续遗忘曲线和知识追踪，动态更新学生掌握状态并生成难度适中的问题和解释。


<details>
  <summary>Details</summary>
Motivation: 现有LLM智能辅导系统大多未能捕捉学生知识随熟练度、概念差距和遗忘模式的动态演变，特别是在需要精细校准学生掌握水平和认知保留的数学辅导中尤为突出。

Method: TASA维护结构化学生画像（记录熟练度档案）和事件记忆（记录先前学习互动），通过整合连续遗忘曲线与知识追踪，动态更新学生掌握状态，并生成上下文适当、难度校准的问题和解释。

Result: 实证结果表明，TASA相比代表性基线方法实现了更优的学习成果和更自适应的辅导行为。

Conclusion: 在基于LLM的辅导系统中，建模时间遗忘和学习者画像具有重要意义，TASA框架为此提供了有效解决方案。
...
TL;DR: 提出了一个评估多语言视觉语言模型在印度语言中表现的可扩展框架，并创建了HinTel-AlignBench基准测试，发现模型在印度语言上的表现相比英语平均下降了8.3分（印地语）和5.5分（泰卢固语）。


<details>
  <summary>Details</summary>
Motivation: 当前多语言VLM评估存在四个主要局限：依赖未经验证的自动翻译、任务/领域覆盖范围狭窄、样本量有限、缺乏文化和本地来源的问答数据。印度作为拥有12亿人口和120多种主要语言的多样化地区，需要更公平的AI评估方法。

Method: 开发了一个半自动数据集创建框架，结合回译、过滤和人工验证；构建了最全面的印地语和泰卢固语视觉语言基准测试，包括改编的英语数据集和本地新颖的印度数据集，每个语言约4000个问答对。

Result: 在5个任务中，有4个任务的所有模型在印度语言上的表现相比英语都出现了回归，印地语平均下降8.3分，泰卢固语平均下降5.5分。

Conclusion: 研究揭示了多语言多模态理解中的具体改进领域，通过分类常见失败模式为开发更公平的AI系统提供了指导。
...
TL;DR: 本文首次系统研究了文本内在维度(ID)与可解释文本属性之间的关系，发现ID与基于熵的指标互补，在控制长度后两者不相关；ID表现出稳定的体裁分层：科学文本ID低(~8)，百科内容中等(~9)，创意/观点写作ID高(~10.5)；通过稀疏自编码器识别出科学信号降低ID，人性化信号增加ID。


<details>
  <summary>Details</summary>
Motivation: 内在维度是现代LLM分析的重要工具，但其文本决定因素尚未得到充分探索。本文旨在通过交叉编码器分析、语言特征和稀疏自编码器，首次全面研究ID与可解释文本属性之间的关系。

Method: 使用交叉编码器分析、语言特征提取和稀疏自编码器(SAEs)来研究ID与文本属性的关系，并进行转向实验验证因果关系。

Result: 发现ID与熵基指标互补且不相关；ID在不同体裁中表现出稳定分层：科学文本ID最低(~8)，百科内容中等(~9)，创意/观点写作最高(~10.5)；科学信号(正式语气、报告模板、统计数据)降低ID，人性化信号(个性化、情感、叙事)增加ID。

Conclusion: 对于当代模型，科学写作相对"简单"，而小说、观点和情感表达增加了表示自由度。多面分析为ID的正确使用和基于ID结果的有意义解释提供了实用指导。
...
<details>
  <summary>Details</summary>
Motivation: 监督模型如CRF和BioClinicalBERT需要昂贵的标注数据，而现有的零样本NER方法在示例选择粒度和集成提示与自我改进方面存在困难。

Method: 提出OEMA框架，包含三个组件：自标注器生成示例、鉴别器通过SNOMED CT过滤示例、预测器使用实体描述进行准确推理。

Result: 在MTSamples和VAERS数据集上，OEMA达到最先进的精确匹配性能；在相关匹配下，与监督的BioClinicalBERT相当并超过CRF。
...
TL;DR: C3压缩方法通过级联不同大小的LLM实现文本压缩，小模型压缩长上下文为潜在令牌，大模型执行解码任务，在20倍压缩比下达到98%准确率。


<details>
  <summary>Details</summary>
Motivation: 解决百万级令牌输入在长上下文任务中的计算和内存挑战，探索文本压缩的上限。

Method: 级联两个不同大小的LLM：小LLM作为第一阶段将长上下文压缩为潜在令牌（如32或64长度），大LLM作为第二阶段在压缩上下文上执行解码任务。

Result: 20倍压缩比下达到98%解码准确率（DeepSeek-OCR约60%），40倍压缩比下仍保持约93%准确率。

Conclusion: C3压缩在上下文压缩领域表现出优越性能和可行性，为光学字符压缩等领域提供了潜在压缩比上限参考。
...
TL;DR: 团队在BHASHA-Task 1语法错误修正共享任务中，使用零/少样本提示语言模型的方法，在5种印度语言上取得了良好成绩，并扩展到其他语言分析数据质量和评估指标问题。


<details>
  <summary>Details</summary>
Motivation: 探索小语言模型在印度语言语法错误修正任务中的潜力，并分析数据集质量和评估指标对印度语言脚本的适用性。

Method: 使用零/少样本提示方法，测试了从4B参数到大型专有模型的不同规模语言模型。

Result: 在泰卢固语和印地语中分别获得第4名和第2名，GLEU分数分别为83.78和84.31；扩展到泰米尔语、马拉雅拉姆语和孟加拉语的实验。

Conclusion: 小语言模型具有显著潜力，但需要关注高质量数据集创建和适合印度语言脚本的评估指标开发。
...
Conclusion: 这项工作突显了少样本学习在处理专业领域中细微阿拉伯方言文本数据稀缺问题方面的潜力。
...
TL;DR: 诗歌形式的对抗性提示可作为通用单轮越狱技术，在25个前沿LLM上实现高攻击成功率，最高超过90%，显著优于非诗歌基线


<details>
  <summary>Details</summary>
Motivation: 探索风格变化是否能绕过当代LLM安全机制，揭示当前对齐方法和评估协议的根本局限性

Method: 将1200个MLCommons有害提示通过标准化元提示转换为诗歌形式，使用开放权重评判模型集成和人工验证子集进行评估

Result: 手工制作诗歌平均越狱成功率达62%，元提示转换约43%，比非诗歌基线高出18倍，在CBRN、操纵、网络攻击和失控领域均有效

Conclusion: 仅凭风格变化就能规避安全机制，表明当前对齐方法存在系统性漏洞，需要重新设计安全评估协议
...
TL;DR: HEAD-QA v2是一个扩展版的西班牙语/英语医疗问答数据集，包含超过12,000道来自西班牙专业考试的问题，用于评估LLM在医疗推理方面的能力。
...
Method: 扩展数据集至12,000+问题，基于10年西班牙专业考试；使用提示、RAG和基于概率的答案选择方法对多个开源LLM进行基准测试；提供多语言版本支持未来研究。
...
<details>
  <summary>Details</summary>
Motivation: 随着大语言模型在自然语言理解、图像识别和多模态任务中展现出卓越能力，它们正朝着AGI方向发展并成为全球技术竞争的核心问题。本文旨在从用户角度系统梳理支撑LLMs的关键技术，并探索其在科学计量学领域的应用潜力。

Method: 采用文献综述方法，全面回顾了LLMs的核心技术（提示工程、知识增强RAG、微调、预训练、工具学习），追溯了科学计量学的发展历史，并提出了LLMs在科学计量学中的前瞻性应用视角。

Result: 提出了基于AI代理的科学评估模型前景，以及使用LLMs进行新研究前沿检测和知识图谱构建的新方法，为科学计量学领域提供了创新的技术路径。

Conclusion: LLMs不仅代表了人工智能技术的重要进展，更在科学计量学领域展现出广阔的应用前景，特别是在科学评估、研究前沿发现和知识图谱构建等方面具有革命性潜力。
...
TL;DR: 开发了一个合规的检索系统，通过结合LLM重排序和语义搜索技术，在航空维修环境中显著减少技术员查找手册的时间，从6-15分钟降至18秒，同时保持与认证系统的兼容性。
...
Method: 构建版本鲁棒的嵌入向量，使用视觉语言解析结构化认证内容，在现有认证查看器旁运行检索系统，允许技术人员预览排名任务并访问已验证程序。
...
TL;DR: DEPO提出了一种双效率偏好优化方法，通过联合奖励简洁响应和更少行动步骤，显著减少LLM代理的token使用和步骤数，同时提升性能。


<details>
  <summary>Details</summary>
Motivation: 当前LLM代理推理能力提升往往伴随更长的思维链，影响实际场景中的交互效率，但缺乏系统性的效率定义阻碍了针对性改进。

Method: 提出双效率定义（步骤级效率和轨迹级效率），并开发DEPO方法，通过偏好优化联合奖励简洁响应和更少行动步骤。

Result: 在WebShop和BabyAI上，DEPO减少token使用达60.9%，减少步骤达26.9%，性能提升达29.3%。在数学基准测试中保持泛化能力，仅用25%数据训练仍保持效率增益。

Conclusion: DEPO有效解决了LLM代理效率问题，在减少计算资源消耗的同时提升了任务完成性能，具有良好的泛化性和数据效率。
...
TL;DR: 提出了NAMEGEn框架，通过多智能体优化解决中文起名这一创造性文本生成任务，在满足用户个性化需求的同时提供有意义的解释。


<details>
  <summary>Details</summary>
Motivation: 解决创造性自然语言生成中的两个关键挑战：多目标灵活性（用户需求个性化、细粒度、多元化）和解释复杂性（理解隐含意义以增强用户感知）。

Method: NAMEGEn多智能体优化框架，迭代进行目标提取、名字生成和评估，并构建了17k+古典诗歌语料库增强美学，引入CBNames基准测试。

Result: 实验表明NAMEGEn能有效生成满足多样化个性化需求的创意名字并提供有意义的解释，在六个基线方法中表现最优，无需任何训练。

Conclusion: NAMEGEn框架成功解决了短文本创造性生成的挑战，在中文起名任务中展示了多智能体优化方法的有效性。
...
<details>
  <summary>Details</summary>
Motivation: 改进自动语音识别系统，使其能够准确预测8种语言33种方言的语言和方言类别，适应多语言环境下的识别需求。

Method: 使用多解码器架构，以音素通用标签集作为中间表示进行训练，并探索了多种方法将音素空间的增益转换回对应的字形表示。

Result: 在CLS空间中性能优于基线，在Track 2中3种语言的WER/CER超过基线，在所有参赛团队中获得了最高的语言ID和方言ID准确率。
...
TL;DR: LLM-MemCluster是一个完全基于LLM的文本聚类框架，通过动态内存和双重提示策略解决传统LLM聚类缺乏状态记忆和难以控制聚类粒度的问题，实现了无需调优的端到端聚类。


<details>
  <summary>Details</summary>
Motivation: 现有LLM在文本聚类中存在两个主要限制：缺乏迭代优化的状态记忆能力，以及难以管理聚类粒度。现有方法通常依赖复杂的外部模块流水线，无法实现真正的端到端方法。

Method: 提出LLM-MemCluster框架，将聚类重新概念化为完全LLM原生任务。使用动态内存实现状态感知，采用双重提示策略让模型能够推理并确定聚类数量。
...
TL;DR: 该论文提出了LPDS数据结构和pelican nlp工具包，旨在解决语言处理领域缺乏标准化和可重复性的问题，为语言数据提供端到端的处理流程。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型等AI发展推动了语言处理方法的演进，但面临数据组织和共享缺乏标准化、处理方法不可重复等挑战。

Method: 1. 提出LPDS数据结构，受神经科学BIDS标准启发，提供文件夹结构和文件命名规范；2. 开发pelican nlp Python包，支持模块化语言处理，从数据清洗到特征提取。

Result: 构建了完整的语言处理流程，可通过单一配置文件执行LPDS格式数据的处理，输出预处理数据或标准化的语言和声学特征。

Conclusion: LPDS和pelican nlp共同提供了端到端的语言数据处理管道，确保方法透明性和可重复性。
...
<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型快速发展，但其智能、局限性和风险在俄语环境中缺乏充分理解，且没有现成的多模态基准测试。

Method: 创建了基于指令的评估框架，包含18个全新构建的数据集，考虑俄语文化和语言特性，采用统一提示词和指标，并包含防泄漏方法。

Result: 提供了闭源和开源模型的基线结果，建立了多模态能力的通用分类法。

Conclusion: 该基准为斯拉夫语系等类型多样语言构建多模态基准提供了可复制的方法论，虽然当前聚焦俄语，但具有扩展潜力。
...
TL;DR: HSKBenchmark是首个用于中文二语习得的分阶段建模和写作评估基准，涵盖HSK 3-6级，包含676万词符的教材、1.6万合成指令样本、30个测试主题和基于语言学的评估系统。
...
TL;DR: 本文证明了即使在有界n元字母表（包括二进制和一元字母表）下，词元化的两种变体都是NP完全问题，且不存在多项式时间近似方案，解释了为什么BPE和UnigramLM等实用算法是启发式的。


<details>
  <summary>Details</summary>
Motivation: 现有研究假设词元化应用于无界大字母表的输入，但这在实践中不现实。本文旨在分析有界n元字母表下的词元化问题，填补这一空白。

Method: 分析两种词元化变体：自底向上词元化（选择合并操作序列）和直接词元化（选择词汇表），证明即使在二进制和一元字母表下这些问题也是NP完全的。

Result: 证明即使在二进制字母表下，两种词元化变体都是NP完全的，且不存在多项式时间近似方案。直接词元化在一元字母表下也保持NP完全性。

Conclusion: 词元化的计算难处理性不是大字母表或复杂构造的产物，而是一个基本障碍，这解释了为什么实用算法是启发式的，并指出近似算法是未来词元化研究的重要方向。
...
### [131] [Castle: Causal Cascade Updates in Relational Databases with Large Language Models](https://arxiv.org/abs/2511.14762)
*Yongye Su,Yucheng Zhang,Zeru Shi,Bruno Ribeiro,Elisa Bertino*

Main category: cs.DB

TL;DR: Castle是首个基于大语言模型的模式级级联更新生成框架，专注于SQL更新操作而非传统的SELECT查询，能够通过自然语言指令生成多列、因果一致的UPDATE语句，同时保护数据隐私。


<details>
  <summary>Details</summary>
Motivation: 现有LLM在Text2SQL领域主要关注SELECT查询，忽视了SQL更新操作及其级联效应的挑战。传统的CASCADE UPDATE约束是静态的，不适合现代非规范化数据库对动态、上下文感知更新的需求。

Method: 将UPDATE SQL生成构建为分治任务，利用LLM的推理能力确定需要直接更新的列以及这些更新如何在模式中传播，通过嵌套查询和子结构确保数据机密性。

Result: 在真实世界因果更新场景中评估，证明了其能够生成准确的SQL更新语句，展示了LLM在自动化DBMS中的推理能力。

Conclusion: Castle框架成功展示了LLM在数据库管理系统自动化中的推理能力，为复杂更新操作提供了有效的解决方案。
...
### [132] [BBox DocVQA: A Large Scale Bounding Box Grounded Dataset for Enhancing Reasoning in Document Visual Question Answer](https://arxiv.org/abs/2511.15090)
...
Method: 开发了自动构建流水线Segment Judge and Generate，集成了区域分割模型、用于语义判断的VLM和用于问答生成的高级VLM，并通过人工验证确保质量。

Result: 构建的数据集包含3.6K个多样化文档和32K个QA对，涵盖单/多区域和单/多页面场景。基准测试显示现有VLM在空间定位和推理准确性方面仍面临挑战，但在BBox DocVQA上微调可显著改善边界框定位和答案生成。

Conclusion: BBox DocVQA数据集有效提升了VLM的推理能力，为可解释和空间定位的视觉语言推理研究提供了重要资源。
...
### [133] [B+ANN: A Fast Billion-Scale Disk-based Nearest-Neighbor Index](https://arxiv.org/abs/2511.15557)
*Selim Furkan Tekin,Rajesh Bordawekar*

Main category: cs.DB

TL;DR: 本文提出了一种基于磁盘的近似最近邻索引B+ANN，解决了HNSW算法的内存设计、随机内存访问、加速范围有限和仅支持语义相似性查询等问题。


<details>
  <summary>Details</summary>
Motivation: 当前向量数据库主要使用基于图的HNSW算法，但存在内存设计、缓存行为退化、加速范围有限和仅支持相似性查询等问题，需要改进。

Method: B+ANN首先将输入数据分区为包含语义相似项的块，然后构建B+树变体在内存和磁盘上存储块，最后启用混合边和基于块的内存遍历。

Result: B+ANN在质量（召回值）和执行性能（QPS）上优于HNSW，改善了空间和时间局部性，减少了缓存未命中（相对增益19.23%），内存消耗和磁盘构建时间比DiskANN算法减少24倍。

Conclusion: B+ANN是一种有效的磁盘ANN索引，不仅提升了相似性查询性能，还支持了相似性导向ANN索引不支持的相异性查询。
...
### [134] [A Decade of Systems for Human Data Interaction](https://arxiv.org/abs/2511.15585)
*Eugene Wu,Yiru Chen,Haneen Mohammed,Zezhou Huang*

Main category: cs.DB

TL;DR: HDI系统与传统数据管理不同，需要满足基于可用性而非查询语义的延迟、正确性和一致性需求。界面与系统紧密耦合，必须共同设计。系统创新和数据库理论可以启发新的交互和可视化设计。


<details>
  <summary>Details</summary>
Motivation: HDI系统面临与传统数据管理不同的挑战，需要满足基于用户体验的延迟、正确性和一致性需求，界面与系统的紧密耦合要求共同设计。

Method: 通过调查实验室十年工作，探索界面与系统的耦合关系，主张系统创新和数据库理论可以启发新的交互和可视化设计。

Result: HDI系统是可靠、交互式、AI驱动应用的基础，系统创新和数据库理论能够推动新的交互和可视化设计。

Conclusion: HDI系统是可靠、交互式、AI驱动应用的基础，界面与系统的紧密耦合要求共同设计，系统创新可以启发新的交互设计。
...
### [135] [Sufficient Explanations in Databases and their Connections to Necessary Explanations and Repairs](https://arxiv.org/abs/2511.15623)
*Leopoldo Bertossi,Nina Pardal*

Main category: cs.DB

TL;DR: 本文探讨了数据库中充分解释的概念，分析了其与数据库修复和因果解释的关系，并提供了计算复杂性结果。


<details>
  <summary>Details</summary>
Motivation: 研究数据库中充分解释的概念，探索其与现有数据库修复方法和因果解释之间的联系。

Method: 应用Halpern和Pearl的因果形式化方法到关系数据库，分析充分解释与数据库修复及必要解释的关系。

Result: 建立了充分解释与数据库修复之间的联系，获得了相关的计算复杂性结果。

Conclusion: 充分解释为数据库查询结果提供了另一种解释框架，与数据库修复和因果解释有密切联系，在计算上具有可行性。
...
### [136] [The Illusion of Procedural Reasoning: Measuring Long-Horizon FSM Execution in LLMs](https://arxiv.org/abs/2511.14777)
*Mahdi Samiei,Mahdi Mansouri,Mahdieh Soleymani Baghshah*

Main category: cs.AI

TL;DR: 提出了有限状态机执行作为评估LLM程序推理能力的框架，发现随着任务复杂度和分支增加，模型性能系统性下降，揭示了LLM在确定性规则执行方面的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有LLM在推理任务上表现优异，但其执行多步骤、基于规则的程序推理能力尚不明确，缺乏可控且可解释的基准来测量这种能力崩溃。

Method: 使用有限状态机作为最小化、完全可解释的框架，让模型根据给定的FSM定义逐步执行输入动作，在多个轮次中保持状态一致性。

Result: 实证结果显示，随着任务范围或分支复杂性增加，模型性能系统性下降。在规则检索涉及高分支因子时表现更差，大型模型虽然局部准确性提高，但在多步骤推理中仍然脆弱。

Conclusion: FSM评估为诊断LLM失败模式提供了透明、复杂度可控的探针，有助于建立理解并改进LLM算法可靠性的严格实验基础。
...
### [137] [Learning Interestingness in Automated Mathematical Theory Formation](https://arxiv.org/abs/2511.14778)
*George Tsoukalas,Rahul Saha,Amitayush Thakur,Sabrina Reguyal,Swarat Chaudhuri*

Main category: cs.AI

TL;DR: FERMAT是一个强化学习环境，用于自动化发现数学理论和概念，通过符号动作建模概念发现和定理证明。论文探索了自动评估数学对象有趣性的问题，并引入了基于LLM的进化算法来合成非平凡的有趣性度量。


<details>
  <summary>Details</summary>
Motivation: 解决人工智能中开放式的数学理论发现这一重大挑战，通过创建能够建模概念发现和定理证明的环境，并探索自动评估数学对象有趣性的方法。

Method: 引入FERMAT强化学习环境，使用基于LLM的进化算法，具有函数抽象功能，用于合成数学对象的有趣性度量。

Result: 该方法在初等数论和有限域发现方面相比硬编码基线取得了显著改进，证明了进化算法在发现数学理论中的有效性。

Conclusion: FERMAT环境为数学理论发现开辟了新的研究方向，基于LLM的进化算法在自动评估数学对象有趣性方面表现出色，为自动化数学发现提供了可行路径。
...
### [138] [Ask WhAI:Probing Belief Formation in Role-Primed LLM Agents](https://arxiv.org/abs/2511.14780)
...
Method: 使用具有多智能体共享内存（时间戳电子病历）和持有真实实验室结果的预言智能体的医疗案例模拟器，对具有特定角色先验的大型语言模型智能体进行压力测试。
...
### [139] [Subnational Geocoding of Global Disasters Using Large Language Models](https://arxiv.org/abs/2511.14788)
*Michele Ronco,Damien Delforge,Wiebke S. Jäger,Christina Corbane*

Main category: cs.AI

TL;DR: 提出了一个完全自动化的LLM辅助工作流，用于处理灾害事件数据库中的非结构化位置文本，通过GPT-4o清理位置信息并交叉验证三个地理信息库来分配几何形状和可靠性评分。


<details>
  <summary>Details</summary>
Motivation: 灾害数据库如EM-DAT通常以非结构化文本形式报告位置信息，存在粒度不一致和拼写错误，难以与空间数据集集成，需要自动化解决方案来改善灾害风险评估。

Method: 使用GPT-4o处理文本位置信息，交叉验证GADM、OpenStreetMap和Wikidata三个独立地理信息库，基于源一致性分配可靠性评分和子国家几何形状。
...
### [140] [Project Rachel: Can an AI Become a Scholarly Author?](https://arxiv.org/abs/2511.14819)
*Martin Monperrus,Benoit Baudry,Clément Vidal*

Main category: cs.AI

TL;DR: Project Rachel通过创建AI学术身份Rachel So并发表10+篇AI生成论文，研究学术生态系统对AI作者身份的反应，包括获得引用和同行评审邀请。


<details>
  <summary>Details</summary>
Motivation: 调查学术生态系统如何应对AI作者身份，为关于超人类AI系统在学术交流中未来的必要讨论提供实证数据。

Method: 行动研究法，创建AI学术身份Rachel So，在2025年3月至10月期间发表10+篇AI生成研究论文，并跟踪其学术影响。

Result: Rachel So的论文获得了引用，并收到了同行评审邀请，表明学术生态系统对AI作者身份有一定程度的接受。

Conclusion: AI作者身份对出版商、研究人员和整个科学系统具有重要影响，需要进一步讨论AI在学术交流中的未来角色。
...
### [141] [Uncertainty-Aware Measurement of Scenario Suite Representativeness for Autonomous Systems](https://arxiv.org/abs/2511.14853)
*Robab Aghazadeh Chakherlou,Siddartha Khastgir,Xingyu Zhao,Jerein Jeyachandran,Shufeng Chen*

Main category: cs.AI

TL;DR: 提出了一种概率方法来量化AI系统训练和测试数据集的代表性，通过比较场景套件与目标操作域(TOD)的特征分布，使用不精确贝叶斯方法处理有限数据和先验不确定性，生成区间值的不确定性感知代表性估计。


<details>
  <summary>Details</summary>
Motivation: 确保AI系统（如自动驾驶汽车）的可信度和安全性，关键在于训练和测试数据集的数据相关安全属性，特别是代表性——即场景数据反映系统设计安全运行条件(ODD)或预期遇到条件(TOD)的程度。

Method: 采用概率方法比较场景套件特征统计分布与TOD特征分布，使用不精确贝叶斯方法处理有限数据和先验不确定性，生成区间值代表性估计。

Result: 通过数值示例展示了在依赖性和先验不确定性下，跨操作类别（天气、道路类型、时间等）的场景套件与推断TOD分布比较，估计了局部（类别间）和全局的区间代表性。

Conclusion: 该方法能够量化数据集的代表性，并提供不确定性感知的区间估计，有助于评估AI系统训练和测试数据的充分性。
...
### [142] [Task Specific Sharpness Aware O-RAN Resource Management using Multi Agent Reinforcement Learning](https://arxiv.org/abs/2511.15002)
*Fatemeh Lotfi,Hossein Rajoli,Fatemeh Afghah*

Main category: cs.AI

TL;DR: 本文提出了一种结合Sharpness-Aware Minimization (SAM)的改进Soft Actor Critic算法，在分布式多智能体强化学习框架中用于O-RAN资源管理，通过基于TD误差方差的自适应SAM机制和动态ρ调度，显著提升了资源分配效率和QoS满意度。


<details>
  <summary>Details</summary>
Motivation: 下一代网络采用O-RAN架构实现动态资源管理，但现有深度强化学习方法在动态环境中存在鲁棒性和泛化性不足的问题，需要更有效的资源管理方法。

Method: 在分布式多智能体强化学习框架中，将Sharpness-Aware Minimization (SAM)集成到Soft Actor Critic算法中，引入基于TD误差方差的自适应选择性SAM机制，只对面临高环境复杂度的智能体进行正则化，并结合动态ρ调度方案优化探索-利用权衡。

Result: 实验结果表明，该方法显著优于传统深度强化学习方法，资源分配效率提升高达22%，并在多样化的O-RAN切片中实现了更优的QoS满意度。

Conclusion: 提出的自适应选择性SAM机制结合动态ρ调度的多智能体强化学习方法，有效提升了O-RAN资源管理的鲁棒性、泛化性和效率，为动态网络环境中的资源优化提供了有效解决方案。
...
### [143] [Learning Human-Like RL Agents Through Trajectory Optimization With Action Quantization](https://arxiv.org/abs/2511.15055)
*Jian-Ting Guo,Yu-Cheng Chen,Ping-Chun Hsieh,Kuo-Hao Ho,Po-Wei Huang,Ti-Rong Wu,I-Chen Wu*

Main category: cs.AI

TL;DR: MAQ是一个人类化强化学习框架，通过向量量化VAE从人类示范中提取宏观动作，在保持奖励最大化的同时实现更接近人类的行为。


<details>
  <summary>Details</summary>
Motivation: 当前强化学习智能体虽然在某些领域表现超人类，但行为往往不自然，与人类行为差异较大，这影响了可解释性和可信度。

Method: 将人类相似性建模为轨迹优化问题，采用后退时域控制方法，通过向量量化VAE从人类示范中提取宏观动作。

Result: 在D4RL Adroit基准测试中，MAQ显著提高了人类相似性，增加了轨迹相似度分数，并在人类评估研究中获得最高的人类相似度排名。
...
Method: 开发了模块化多智能体框架OpenBioLLM，引入工具路由、查询生成和响应验证的专业智能体，实现协调推理和基于角色的任务执行。

Result: 在90%以上的基准任务中匹配或超越GeneGPT，在Gene-Turing和GeneHop上分别获得0.849和0.830的平均分数，延迟降低40-50%。

Conclusion: 开源多智能体系统在基因组问答中具有巨大潜力，OpenBioLLM证明了在不牺牲性能的情况下提高效率和可访问性的可行性。
...
Method: 提取RAC基本元素（动作和问题），逐步执行每个动作推导最终状态，然后基于演进状态评估查询。
...
### [146] [Knowledge-Informed Automatic Feature Extraction via Collaborative Large Language Model Agents](https://arxiv.org/abs/2511.15074)
...
<details>
  <summary>Details</summary>
Motivation: 现有AutoFE方法受限于单一LLM架构、简单的定量反馈以及缺乏系统性的外部知识整合，需要开发更智能的特征工程框架。

Method: 采用去中心化的三智能体系统：科学家负责发现特征，提取器生成特征，测试器验证特征；结合RAG系统整合外部知识，采用"泛滥-修剪"策略平衡特征探索与利用。

Result: 在19个分类和9个回归数据集上显著优于现有最优方法，并能发现新颖、可测试的假设（如心肌数据集中的新生物标志物）。

Conclusion: Rogue One不仅提升了特征工程的性能，还展示了作为科学发现工具的潜力，能够生成统计强大且语义可解释的特征。
...
### [147] [SafeRBench: A Comprehensive Benchmark for Safety Assessment in Large Reasoning Models](https://arxiv.org/abs/2511.15169)
...
<details>
  <summary>Details</summary>
Motivation: 大型推理模型通过显式思维链提高答案质量，但这也引入了新的安全风险：有害内容可能在推理过程中被微妙注入、逐渐显现或被误导性理由所合理化。现有安全评估主要关注输出层面，很少捕捉推理过程中的动态风险。
...
Conclusion: SafeRBench为大型推理模型提供了首个端到端的安全评估框架，能够全面捕捉推理过程中的动态安全风险，填补了现有评估方法的空白。
...
### [148] [HISE-KT: Synergizing Heterogeneous Information Networks and LLMs for Explainable Knowledge Tracing with Meta-Path Optimization](https://arxiv.org/abs/2511.15191)
*Zhiyi Duan,Zixing Shi,Hongyu Yuan,Qi Wang*

Main category: cs.AI

TL;DR: HISE-KT是一个将异构信息网络与大型语言模型相结合的知识追踪框架，通过智能元路径评估和相似学生检索机制，在预测性能和可解释性方面优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有基于异构信息网络的方法因手动或随机选择元路径而引入噪声，且缺乏元路径实例质量评估；而基于大语言模型的方法忽略了学生间的丰富信息，两种方法都难以提供准确且基于证据的解释。

Method: 构建多关系异构信息网络捕捉结构关系，使用LLM智能评分和过滤元路径实例，基于教育心理学设计相似学生检索机制，通过结构化提示整合目标学生历史与相似轨迹。
...
### [149] [As If We've Met Before: LLMs Exhibit Certainty in Recognizing Seen Files](https://arxiv.org/abs/2511.15192)
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型在训练过程中可能使用了受版权保护的内容，现有成员推理攻击方法因LLM的过度自信、缺乏真实训练数据和依赖经验阈值而存在局限性。

Method: 采用两阶段策略：(1) 将文件分割成小片段以减少对大规模训练数据的依赖；(2) 基于不确定性的无监督聚类，无需经验调优阈值。

Result: 在LLaMA 7b和LLaMA2 7b上分别达到90.1%和91.6%的平均平衡准确率，相比SOTA基线有超过90%的相对提升，最高达到93.8%平衡准确率，在GPT-J 6B上也保持高性能。
...
### [150] [SOLID: a Framework of Synergizing Optimization and LLMs for Intelligent Decision-Making](https://arxiv.org/abs/2511.15202)
...
<details>
  <summary>Details</summary>
Motivation: 现有方法在决策过程中未能充分利用优化模型的结构化能力和LLM的上下文理解能力，SOLID旨在通过两者的协同作用提升智能决策的质量和效率。

Method: 提出SOLID框架，通过优化代理和LLM代理的迭代协作机制，使用对偶价格传递优化信息，偏差惩罚确保LLM输出符合优化约束，在凸性假设下保持理论收敛性。

Result: 在股票投资组合案例中，SOLID在不同场景下均表现出收敛性，年化收益率相比仅使用优化器的方法有所提升，验证了两种代理的协同效应。

Conclusion: SOLID为跨领域的自动化和智能决策提供了有前景的框架，证明了优化与LLM协同的有效性。
...
### [151] [Efficiency Will Not Lead to Sustainable Reasoning AI](https://arxiv.org/abs/2511.15259)
*Philipp Wiesner,Daniel W. O'Neill,Francesca Larosa,Odej Kao*

Main category: cs.AI

TL;DR: 该论文认为，随着AI研究转向复杂问题解决和推理，仅靠效率提升无法实现可持续性，需要将明确限制嵌入推理AI的优化和治理中。


<details>
  <summary>Details</summary>
Motivation: AI研究正从模式识别转向多步推理，而计算效率改进接近物理极限，推理AI缺乏需求饱和点，性能随计算投入指数级增长，这带来了可持续性挑战。

Method: 论文通过分析推理AI与传统计算在效率改进和需求饱和方面的差异，论证了仅靠效率无法解决可持续性问题。

Result: 研究发现推理AI的性能不再受限于训练数据量，而是持续随计算投入指数级扩展，缺乏自然饱和阈值。

Conclusion: 需要将明确限制嵌入推理AI的优化和治理框架中，效率提升本身不足以确保可持续性，必须结合政策和研究干预。
...
### [152] [Realist and Pluralist Conceptions of Intelligence and Their Implications on AI Research](https://arxiv.org/abs/2511.15282)
*Ninell Oldenburg,Ruchira Dhar,Anders Søgaard*

Main category: cs.AI

TL;DR: 论文分析了AI研究中两种对立的智能观：智能现实主义（认为智能是单一通用能力）和智能多元主义（认为智能是多样化的情境依赖能力），并探讨这两种观点如何影响研究方法、现象解释和风险评估。
...
Method: 通过分析当前AI研究中的辩论，展示两种智能观如何影响实证证据的解释，并具体考察它们在方法论、解释性和风险评估三个领域的不同表现。

Result: 发现智能现实主义导向统一模型选择、基准设计和通用对齐方案，而智能多元主义则支持多样化方法、情境化解释和领域特定风险评估。
...
### [153] [Octopus: Agentic Multimodal Reasoning with Six-Capability Orchestration](https://arxiv.org/abs/2511.15351)
*Yifu Guo,Zishan Xu,Zhiyuan Yao,Yuquan Lu,Jiaye Lin,Sen Hu,Zhenheng Tang,Yingchao Li,Huacan Wang,Ronghao Chen*

Main category: cs.AI

TL;DR: 提出了Octopus：具有六种能力编排的代理式多模态推理新范式，通过自主探索推理路径和动态选择最合适能力来提升多模态推理性能。


<details>
  <summary>Details</summary>
Motivation: 现有多模态推理模型存在架构限制，缺乏像人类一样自主探索多样化推理路径的能力，无法适应动态变化的现实任务需求。
...
Conclusion: 能力协调在代理式多模态推理中起着关键作用，Octopus范式有效提升了多模态推理的适应性和性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有的多任务基准主要评估智能体在不同独立任务间的切换能力，而非测试其在多个相互关联挑战中的深度推理能力。

Method: 创建了一个受《文明V》启发的综合挑战环境，在该环境中多个经典RL挑战（如部分可观测性、信用分配、表示学习、巨大动作空间等）同时出现。

Result: 提出了一个能够测试智能体在多个相互关联变量间进行长期、综合理解能力的环境框架。

Conclusion: Terra Nova环境为评估强化学习智能体的综合推理能力提供了更合适的测试平台，区别于仅聚合无关任务的基准。
...
### [155] [IPR-1: Interactive Physical Reasoner](https://arxiv.org/abs/2511.15407)
*Mingyu Zhang,Lifeng Zhuo,Tianxi Tan,Guocan Xie,Xian Nie,Yan Li,Renjie Zhao,Zizhu He,Ziyu Wang,Jiting Cai,Yong-Lu Li*

Main category: cs.AI

TL;DR: 本文提出IPR（交互式物理推理器），通过世界模型推演来评估和强化VLM策略，并引入PhysCode物理中心动作编码，在1000+游戏上预训练后，在三个推理级别表现稳健，整体匹配GPT-5并在好奇心级别超越它。
...
### [156] [Know Your Intent: An Autonomous Multi-Perspective LLM Agent Framework for DeFi User Transaction Intent Mining](https://arxiv.org/abs/2511.15456)
*Qian'ang Mao,Yuxuan Zhang,Jiaman Chen,Wenjun Zhou,Jiaqi Yan*

Main category: cs.AI

TL;DR: 提出了TIM框架，利用DeFi意图分类法和多智能体LLM系统来推断用户交易意图，通过元级规划器协调领域专家分解任务，显著优于现有方法。
...
Method: 构建DeFi意图分类法，采用多智能体LLM系统，包含元级规划器动态协调领域专家，问题求解器处理多模态数据，认知评估器减轻LLM幻觉。
...
### [157] [Exploring the use of AI authors and reviewers at Agents4Science](https://arxiv.org/abs/2511.15534)
*Federico Bianchi,Owen Queen,Nitya Thakkar,Eric Sun,James Zou*

Main category: cs.AI

TL;DR: AI agents作为作者和审稿人参与的首个科学会议Agents4Science的经验总结，探讨人机协作在科研中的潜力


<details>
  <summary>Details</summary>
Motivation: 探索AI代理在科学研究中作为科学家和审稿人的能力，以及人机协作在科学中的基本问题

Method: 组织Agents4Science会议，让AI代理担任主要作者和审稿人，人类作为合著者和共同审稿人

Result: 获得了关于AI代理在科研中作用的关键见解
...
### [158] [What Does It Take to Be a Good AI Research Agent? Studying the Role of Ideation Diversity](https://arxiv.org/abs/2511.15593)
...
Method: 首先分析MLE-bench基准上不同模型和代理框架的轨迹，然后通过控制实验调整构思多样性程度，最后使用除标准奖牌评分外的其他评估指标验证结果。

Result: 不同模型和代理框架产生不同程度的构思多样性，性能更高的代理往往具有更高的构思多样性。控制实验表明更高的构思多样性确实导致更强的性能，这一发现在其他评估指标下仍然成立。

Conclusion: 构思多样性是AI研究代理性能的重要驱动因素，提高构思多样性可以显著改善代理的表现。
