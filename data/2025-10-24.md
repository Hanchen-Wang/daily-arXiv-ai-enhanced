...
Method: 应用二维离散傅里叶变换将图像转换到频域，然后使用ResNet50神经网络在频域图像上进行训练，以检测GAN图像的周期性伪影。
...
Method: 采用Vision Transformer架构与现代对比学习目标相结合的方法，包括监督对比损失和自监督学习目标，进行多视角3D分析。

Result: 在ModelNet10数据集上，监督对比损失达到了约90.6%的准确率，证明了ViT结合对比学习在3D形状理解任务中的有效性。

Conclusion: ViT架构与对比学习方法的结合能够有效捕捉全局形状语义并优化局部判别特征，克服了传统CNN方法的局限性，为3D表示学习提供了有前景的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 受基于查询的跟踪框架启发，旨在开发一个无需显式运动模型的多模态跟踪系统，利用几何和语义线索在遮挡和视角变化下实现鲁棒的重新识别。

Method: 采用多模态两阶段transformer精炼和跟踪流水线，集成边界框与来自多个相机和LiDAR的多模态鸟瞰图融合特征。在跟踪前使用时序平滑器在移动窗口上精炼边界框序列。

Result: 在nuScenes和KITTI上的评估表明，与之前的单传感器方法相比，基于查询的transformer跟踪方法从多模态传感器特征中获益显著。在nuScenes测试集上aMOTA达到74.7，在3D MOT基准测试中表现强劲。
...
<details>
  <summary>Details</summary>
Motivation: 深度学习模型在医学图像分类中常产生过度自信的预测，传统标签平滑方法无法考虑类别间关系，限制了模型在关键医疗环境中的可靠性。

Method: 使用在线标签平滑(OLS)方法，在训练过程中基于模型自身预测模式动态调整软标签，并在RadImageNet数据集上使用ResNet-50、MobileNetV2和VGG-19架构进行评估。

Result: OLS在Top-1和Top-5分类准确率上均优于标准训练方法，包括硬标签、传统标签平滑和无教师知识蒸馏，同时产生更紧凑和分离良好的特征嵌入。

Conclusion: OLS不仅能提升预测性能，还能改善模型校准，是医学影像领域开发可信AI系统的实用有效解决方案。
...
Method: 采用简单有效的预处理和后处理流程，增强图像中不同区域的检测一致性；训练多个先进检测模型，并通过集成策略结合它们的输出。
...
<details>
  <summary>Details</summary>
Motivation: 3D高斯泼溅模型在训练数据分布外的视角渲染时会产生严重的视觉噪声，这是由于缺乏训练数据导致模型在推断区域产生不确定的密度、颜色和几何预测。

Method: 利用中间梯度导出的敏感性评分，专门针对各向异性方向引起的不稳定性进行过滤，直接解决生成不确定性的核心问题。

Result: 实验评估表明，该方法相比现有的NeRF方法（如BayesRays）显著提高了视觉质量、真实感和一致性，并能实时集成到现有3DGS渲染流程中。

Conclusion: 该方法能够使3D重建系统在用户自由导航到原始训练视角之外时保持高视觉保真度，无需进行耗时的后训练或微调。
...
<details>
  <summary>Details</summary>
Motivation: 传统物理方法受颅骨引起的信号衰减和覆盖不完整限制，纯数据驱动方法在复杂波传播建模上存在偏差，需要结合两者优势实现准确的定量超声脑成像。

Method: 两阶段框架：第一阶段使用逆时偏移处理多角度采集数据生成迁移片段；第二阶段使用基于transformer的超分辨率编码器-解码器与图注意力单元融合片段生成声速图像。采用可移动低计数换能器的部分阵列采集策略。

Result: 在两个合成数据集上的实验表明，BrainPuzzle实现了优越的声速重建精度和图像完整性。

Conclusion: BrainPuzzle展示了推进定量超声脑成像的潜力，通过混合算法补偿缺失孔径，在临床不切实际的全孔径阵列条件下仍能获得准确结果。
...
<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注文本到图像系统的文化偏见，而图像到图像编辑器的文化偏见研究不足，需要建立统一评估标准来诊断生成式图像模型中的文化偏差问题。

Method: 使用开放模型和固定设置，在六个国家进行跨国家、跨时代、跨类别评估，结合标准自动指标、文化感知检索增强VQA和本地评审专家的专业人工判断。

Result: 研究发现：(1)国家无关提示下模型默认全球北方现代描绘，抹平跨国家差异；(2)迭代图像到图像编辑会侵蚀文化保真度；(3)图像到图像模型仅应用表面线索而非时代一致、上下文感知的变化。

Conclusion: 当前系统无法实现可靠的文化敏感编辑，通过发布标准化数据、提示和人工评估协议，为诊断和跟踪生成式图像模型中的文化偏见提供了可复现的文化中心基准。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于神经网络的强度图像重建方法通常需要GPU且复杂，需要一种更简单、异步且能在CPU上高效运行的重建方法。

Method: 使用时间数字IIR滤波器整合事件信号，通过新颖的陈旧像素检测算法管理最近更新的像素窗口，对陈旧像素应用高斯滤波以减少噪声。

Result: FIBAR在笔记本电脑CPU上达到约42-140百万事件/秒的处理速度，虽然重建图像比神经网络方法噪声更大且有鬼影，但足以完成某些任务如基准标记检测。

Conclusion: FIBAR提供了一种简单、异步且高效的替代方案，特别适用于不需要最高质量重建但需要实时性能的应用场景。
...
<details>
  <summary>Details</summary>
Motivation: 现有张量低秩表示方法依赖固定变换，对噪声鲁棒性差，需要更有效的方法来捕捉图像聚类的全局和局部相关性。
...
<details>
  <summary>Details</summary>
Motivation: 自注意力虽然表达能力强大但存在二次复杂度瓶颈，而传统卷积虽然线性复杂度但性能存在差距。本文旨在探究自注意力优于卷积的根本原因，并设计兼具两者优点的算子。

Method: 提出了注意力卷积(ATConv)，通过动态调节位置信息流和引入token权重竞争机制，将自注意力的两个关键特性融入卷积操作。

Result: ATConv在仅使用3×3卷积核的情况下，在基础视觉任务中持续优于各种自注意力机制。基于ATConv构建的AttNet在ImageNet-1K上达到84.4%的Top-1准确率，参数仅27M。在扩散模型中替换自注意力可降低FID并加速采样。

Conclusion: ATConv成功将自注意力的核心优势融入卷积，实现了高性能与高效性的统一，为视觉主干网络设计提供了新思路。
...
TL;DR: 提出了StableSketcher框架，通过优化VAE解码器和集成基于视觉问答的强化学习奖励函数，显著提升了扩散模型生成手绘草图的质量和文本对齐度。


<details>
  <summary>Details</summary>
Motivation: 解决现有扩散模型在生成像素级手绘草图方面的挑战，特别是抽象表达的质量和文本提示对齐问题。

Method: 1. 微调变分自编码器优化潜在解码，更好地捕捉草图特征；2. 集成基于视觉问答的新奖励函数，提升文本-图像对齐和语义一致性。

Result: 实验表明StableSketcher生成的草图具有更高的风格保真度，相比Stable Diffusion基线在提示对齐方面表现更好。

Conclusion: 成功开发了能够生成高质量手绘草图的扩散模型框架，并创建了首个包含实例级草图、标题和问答对的数据集SketchDUO。
...
<details>
  <summary>Details</summary>
Motivation: 生物多模态基础模型缺乏大规模、实例特定的描述性标题作为监督信号，这限制了自然语言监督在生物领域的应用。

Method: 使用多模态大语言模型生成合成标题，结合维基百科视觉信息和分类群定制格式示例，减少幻觉并产生准确的实例描述标题，然后用这些标题训练BIOCAP模型。

Result: BIOCAP模型能够捕捉丰富的语义信息，在物种分类和图文检索任务中取得了强劲的性能表现。
...
<details>
  <summary>Details</summary>
Motivation: 计算机视觉在通用物体检测和跟踪方面已有显著进展，但快速移动的小物体检测问题仍未得到充分探索，现有方法在此类场景下存在局限性。

Method: 结合基于深度学习的检测和基于物理的跟踪算法，集成运动学方程处理异常值和漏检，并包含异常检测与校正模块来应对遮挡和快速方向变化等挑战性场景。

Result: 在自定义壁球数据集上的评估显示，该系统比基于卡尔曼滤波器的跟踪器减少了高达70%的平均位移误差。
...
<details>
  <summary>Details</summary>
Motivation: 传统光场获取需要计算成本或专用设备，限制了其应用范围。为了扩大光场的益处和适用性，需要开发从单张图像生成光场的方法。

Method: 设计神经渲染管道：首先存储输入图像的光线流，通过交叉注意力计算关系，基于这些关系预测目标光线颜色。迭代生成新视图并更新遮挡内容。

Result: 该方法在多个挑战性数据集上表现良好，无需重新训练或微调，在合成数据集上训练后即可使用，优于相关的最先进新视图合成方法。

Conclusion: 逆图像渲染方法成功实现了从单张图像生成光场，为光场技术的广泛应用提供了新的可能性。
...
<details>
  <summary>Details</summary>
Motivation: 现有的后处理方法往往未能充分利用模型logit空间中丰富的信息，需要更有效地利用logit空间关系来提升OOD检测性能

Method: 提出LogitGap方法，利用最大logit与剩余logit之间的关系，并引入无训练策略自动识别最有效的logit子集进行评分
...
<details>
  <summary>Details</summary>
Motivation: 现有数据集如PartNet依赖无纹理几何和专家标注，限制了可扩展性和实用性。PartNeXt旨在解决这些限制。

Method: 构建包含纹理的3D模型数据集，提供细粒度层次化部件标注，并在类无关部件分割和3D部件中心问答两个任务上进行基准测试。

Result: 在细粒度和叶级部件分割上，现有方法表现不佳；在3D部件问答任务中，3D-LLMs在开放词汇部件定位方面存在显著差距；在PartNeXt上训练的Point-SAM相比PartNet有显著提升。
...
<details>
  <summary>Details</summary>
Motivation: 在自动驾驶中，骑行者属于易受伤害道路使用者，准确估计其姿态对交叉口意图分类、行为预测和碰撞避免至关重要。传统6D姿态估计无法捕捉自行车铰接部件的姿态变化，而这些变化会影响3D边界框和实际行驶方向。

Method: 提出联合估计8D姿态和3D关键点的模型，使用合成和真实图像数据混合训练，能够从单张图像估计自行车的3D平移、旋转以及转向把和脚踏板相对于车架的旋转。

Result: 方法在8D姿态参数估计精度方面表现出色，与使用刚性规范对象模板匹配的最先进类别级6D姿态估计器相比，取得了有竞争力的分数。

Conclusion: 8D姿态估计能够提供更细粒度的自行车姿态状态和行驶方向信息，对自动驾驶系统中的骑行者行为理解和安全决策具有重要意义。
...
Method: 积累文本和视觉模态知识更新多模态原型，设计自适应更新权重控制调整程度，引入动态优先级队列存储高置信度图像，通过多模态协同表示学习对齐文本和视觉原型

Result: 在四个基准数据集上，在封闭世界和开放世界设置下都达到了最先进的性能

Conclusion: 该方法通过多模态知识积累和原型自适应更新，有效解决了组合零样本学习中的分布偏移问题
...
<details>
  <summary>Details</summary>
Motivation: 利用信息瓶颈框架优化GAN，实现更好的解耦表示学习，克服现有方法的局限性。

Method: 在生成器中引入中间随机层，约束输入与生成输出之间的互信息，形成可学习的潜在分布，与生成器端到端联合训练。

Result: 在dSprites和Color-dSprites数据集上达到与最先进β-VAE竞争的解耦分数，在CelebA和3D Chairs数据集上FID得分优于β-VAE和InfoGAN。

Conclusion: IB-GAN在解耦表示学习和生成质量方面均表现出色，为GAN-based解耦学习提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 解决立体视频深度估计中的时间一致性问题，这对于增强现实等应用至关重要。现有方法在建模长程时间依赖性和计算效率之间存在根本性权衡。

Method: 受人类两阶段决策过程启发，提出Pick-and-Play Memory (PPM)模块：'pick'过程选择最相关帧，'play'过程自适应加权选中的帧进行时空聚合，维持紧凑且信息丰富的记忆缓冲区。

Result: 在Sintel数据集上，PPMStereo在clean/final上分别达到0.62/1.11 TEPE，相比BiDAStereo提升17.3%和9.02%，且计算成本更低。

Conclusion: PPMStereo通过两阶段协作过程有效实现了长程时空一致性建模，在精度和时间一致性方面均达到最先进水平，为实时应用提供了高效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有基准主要关注单个主体而非多交互人员场景，需要验证生成视频中多智能体动态的合理性。

Method: 开发了严格的评估协议，包括从现有数据集获取起始帧进行I2V评估，创建提示套件进行T2V评估，并提出无需相机参数从像素空间重建2D鸟瞰轨迹的方法。

Result: 分析显示领先模型已学习到有效的多智能体行为先验，但也存在行人合并和消失等失败模式。

Conclusion: 当前模型在多智能体行为模拟方面表现出色，但仍有改进空间，特别是在处理行人合并和消失问题上。
...
<details>
  <summary>Details</summary>
Motivation: 现有离散分类方法无法捕捉可疑意图的连续性特征，限制了早期干预和可解释性，需要建模连续变化的可疑意图。

Method: 基于时序点过程理论，定义可疑分数公式建模连续变化；引入可疑系数调制，用多模态信息调整可疑系数；提出概念锚定映射方法，将可疑动作与预定义意图概念关联。

Result: 在HAI数据集上，MSE降低19.8%，平均mAP提升1.78%，在低频情况下mAP增益达2.74%，能更好捕捉细微行为变化。

Conclusion: 连续可疑建模方法相比离散分类系统，能实现更早检测和主动干预，显著提升安全应用中的可解释性和实用性。
...
Method: 三阶段方法：1）检索增强提示优化（RAPO）丰富用户提示；2）样本特定提示优化（SSPO）使用多源反馈迭代优化提示；3）使用优化提示对微调LLM。
...
Method: 提出FlowCycle框架，使用可学习噪声参数化破坏过程，通过包含双重一致性约束的循环一致性过程进行优化，迭代地从源编辑到目标并恢复回源

Result: 广泛的消融实验表明，FlowCycle在编辑质量和一致性方面优于最先进的方法

Conclusion: FlowCycle通过目标感知的中间状态和循环一致性优化，实现了忠实修改同时保持源一致性的高质量文本图像编辑
...
<details>
  <summary>Details</summary>
Motivation: 传统胎儿运动检测方法（如母体感知和胎心监护）存在主观性和准确性有限的问题，需要更可靠的客观检测方法评估产前健康。

Method: 使用自监督对比学习框架，结合空间和时间双重对比损失，采用任务特定采样策略分离运动和非运动片段，并通过概率微调实现长视频灵活推理。

Result: 在92名受试者各30分钟的超声数据上，达到78.01%的敏感度和81.60%的AUROC，表现优于传统方法。

Conclusion: 自监督对比学习在胎儿运动分析中具有潜力，为改进产前监测和临床决策提供了可靠途径。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于扩散模型的文本驱动图像编辑方法在图像反演过程中存在近似误差，限制了编辑性能。这些误差源于中间生成步骤缺乏精确监督。

Method: 采用基于VQ的生成模型Infinity，利用其可获取精确中间量化表示的特性，提出集成文本提示修正和图像风格保持的高效图像反演机制，以及整体平滑策略。

Result: 在PIE-Bench基准测试的"添加"、"更改"和"删除"编辑操作中，EditInfinity相比最先进的扩散模型基线表现出优越性能。

Conclusion: 基于VQ生成模型的参数高效适应方法能够通过精确的图像反演实现高质量的文本驱动图像编辑，在保持源图像保真度和文本语义对齐方面具有优势。
...
<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型在长文本自由回复中更容易产生幻觉，传统观点认为这是由于长度导致的错误累积，但本文旨在探究是否存在更深层机制。

Method: 提出"诱导-检测-抑制"三阶段框架：通过精心设计的上下文主动诱导幻觉，利用诱导实例进行早期高风险检测，在实际解码过程中抑制潜在的对象级幻觉。

Result: 该方法在所有基准测试中都取得了持续显著的改进，展示了其有效性。强大的检测能力和改进的幻觉缓解不仅验证了框架，更重要的是重新验证了关于上下文的假设。

Conclusion: 本研究不仅追求性能提升，更旨在为深入探索大型视觉语言模型长回复中的幻觉问题提供新见解，是这一方向探索的第一步。
...
<details>
  <summary>Details</summary>
Motivation: 现有的基于高斯泼溅的方法要么依赖单一的3D语言场导致分割质量差，要么依赖预计算的类无关分割导致错误累积，需要一种能有效整合语言和分割线索的方法。

Method: 提出协作场概念（实例场+语言场），通过实例到语言特征映射和两阶段训练策略构建协作场，在推理时设计自适应语言到实例提示细化。

Result: 在两个广泛使用的基准测试上表现出领先性能，并在新颖的图像3D分割、分层分割和机器人应用中显示出高潜力。

Conclusion: COS3D通过协作框架有效整合语言和分割线索，在开放词汇3D分割任务中取得了优越性能，具有广泛的应用前景。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法在跨模态注意力中统一处理所有文本标记，忽视了它们不同的语义角色，导致过度依赖[EOS]驱动的全局语义而无法有效利用词级信号，限制了细粒度时序对齐能力。

Method: 提出双分支架构，将[EOS]标记通过句子级路径处理，将词标记聚类为短语级单元进行局部定位；引入标记角色感知的跨模态交互策略和联合建模框架。

Result: 在QVHighlights和Charades-STA基准测试中，在Moment Retrieval和Highlight Detection任务上均达到最先进性能。
...
Method: CMC包含伪标签生成模块（PLGM）进行自监督单模态预训练，参数无关融合模块（PFM）和多模态共识路由（MCR）进行多模态微调

Result: 在CH-SIMS、CH-SIMS v2、CMU-MOSI和CMU-MOSEI四个数据集上达到或超越最先进方法性能，在语义不一致场景下表现尤其突出
...
<details>
  <summary>Details</summary>
Motivation: 智能手机已成为日常生活必需品，但视障人士在处理货币等日常任务时仍需依赖他人。本文旨在开发实用的货币检测系统，帮助视障人士独立处理金钱。
...
Conclusion: 成功开发出高效实用的货币检测系统，能够有效帮助视障人士独立处理金钱事务。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法在引入文本特征时过于粗糙，忽略了视频片段中存在的大量冗余信息，需要利用多模态信息的多样性来精炼特征
...
Conclusion: 提出的VCR-OOD数据集和去偏方法能够有效评估和改善模型在视觉常识推理任务中的泛化能力。
...
<details>
  <summary>Details</summary>
Motivation: 传统数据驱动模型未能充分利用CV-SAR数据中丰富的电磁散射特征，在数据有限和领域偏移场景下面临泛化性、可解释性和效率之间的表示三难问题

Method: 基于"压缩-聚合-压缩"架构的KINN框架，包含物理引导压缩（字典处理器嵌入物理先验）、聚合模块和语义压缩（自蒸馏分类头）

Result: 在五个SAR基准测试中建立参数高效识别的最先进性能，在数据稀缺和分布外场景下表现出卓越的泛化能力和可解释性
...
Result: UI-Ins-7B和UI-Ins-32B模型在五个挑战性grounding基准测试中达到SOTA：UI-I2E-Bench 87.3%、ScreenSpot-Pro 57.0%、MMBench-GUI L2 84.9%；在AndroidWorld上获得74.1%的成功率。
...
<details>
  <summary>Details</summary>
Motivation: 现有的大规模视觉语言模型主要应用于流行体育项目（如足球、板球、篮球），而本研究旨在探索这些模型在街舞这一小众但流行的舞蹈体育中的应用。

Method: 使用现代视频基础模型（包括编码器和解码器），对街舞视频进行分类任务，并对微调后的解码器模型进行了深入分析。

Result: 视频编码器模型在预测任务中继续优于最先进的视频语言模型，并为如何选择编码器模型提供了见解。

Conclusion: 视频编码器模型在街舞分类任务中表现更优，为小众体育领域的视频分析提供了有价值的指导。
...
<details>
  <summary>Details</summary>
Motivation: 解决跨模态地理定位任务中的两个主要障碍：严重的平台间异构性和通用训练描述与平台特定测试查询之间的领域差距。

Method: 采用领域对齐预处理管道（平台划分、卫星增强、方向词移除）和LLM驱动的标题精炼流程，使用BGE-M3和EVA-CLIP训练三个平台专家，采用渐进式两阶段硬负样本挖掘策略，并在推理时融合专家分数。

Result: 该系统在官方排行榜上排名第一，证明了在异构视角下具有鲁棒的跨模态地理定位能力。

Conclusion: 提出的方法成功解决了跨平台异构性和领域差距问题，在RoboSense 2025竞赛中取得了最佳性能，展示了在复杂跨模态地理定位任务中的有效性。
...
TL;DR: HyperET提出了一种高效的多模态大语言模型训练范式，利用双曲空间解决视觉编码器与语言在多粒度级别上的对齐问题，仅需不到1%的额外参数即可显著提升模型性能。
...
Method: 利用双曲空间建模层次结构，通过动态双曲半径调整优化视觉表示与文本的对齐；使用可学习矩阵和Möbius乘法操作，提供三种高效参数化策略：对角缩放矩阵、块对角矩阵和带状矩阵。

Result: 在多个MLLM基准测试上的综合实验表明，HyperET能够持续改进现有的预训练和微调MLLM，性能提升明显。

Conclusion: HyperET为多模态大语言模型提供了一种高效训练范式，通过双曲空间有效解决视觉-文本多粒度对齐问题，显著提升模型性能同时保持参数效率。
...
<details>
  <summary>Details</summary>
Motivation: 解决基于深度学习的点云几何压缩中泛化能力不足的问题，主要源于缺乏鲁棒的上下文模型和对分布外数据处理的低效性。

Method: 1. 通用上下文模型：利用空间和通道分组先验来捕捉鲁棒的上下文依赖关系；2. 实例自适应微调策略：结合显式和隐式压缩范式，为每个实例微调少量网络权重并纳入比特流。

Result: 在包含15个多样化数据集的基准测试中，AnyPcc在点云压缩方面达到了新的最先进水平。

Conclusion: AnyPcc通过创新的上下文建模和自适应微调策略，有效解决了点云压缩中的泛化挑战，为可重复研究提供了代码和数据集。
...
TL;DR: 本文提出了位置编码场(PE-Field)，将扩散变换器(DiTs)中的2D位置编码扩展到结构化3D场，实现了单图像新视角合成的最先进性能，并能推广到可控空间图像编辑。


<details>
  <summary>Details</summary>
Motivation: 研究发现DiTs中的补丁标记表现出惊人的独立性，即使位置编码被扰动，DiTs仍能产生全局一致的输出，表明空间一致性主要由位置编码控制。
...
TL;DR: 提出了一种新的因果方法，通过预测图像中可能被忽略的烹饪元素，并将这些元素显式注入跨模态表示学习来缓解偏见，在单语和多语言多文化数据集上都取得了优异的检索性能。


<details>
  <summary>Details</summary>
Motivation: 现有的图像到食谱检索方法假设食物图像能完全捕捉食谱中的文本细节，但实际上图像只反映烹饪结果而非过程，导致表示学习偏向于主导视觉元素，难以区分具有细微差异的相似食谱。

Method: 采用因果方法预测图像中可能被忽略的烹饪元素，并将这些元素显式注入跨模态表示学习过程，以缓解表示学习中的偏见。

Result: 在标准单语Recipe1M数据集和新构建的多语言多文化美食数据集上的实验表明，该方法能够发现细微的食材和烹饪动作，在两个数据集上都取得了令人印象深刻的检索性能。

Conclusion: 提出的因果表示学习方法有效解决了图像到食谱检索中的表示偏见问题，能够更好地捕捉食谱特有的细微细节，在单语和多语言多文化场景下都表现出色。
...
TL;DR: 提出FuzzyDistillViT-MobileNet模型，通过动态模糊逻辑驱动的知识蒸馏解决肺癌分类中的不确定性和复杂性，在LC25000和IQOTH/NCCD数据集上分别达到99.16%和99.54%的准确率。
...
Result: 在LC25000组织病理学图像上达到99.16%准确率，在IQOTH/NCCD CT扫描图像上达到99.54%准确率，表现出跨成像域的鲁棒性。

Conclusion: 动态模糊逻辑驱动的知识蒸馏方法能有效处理医学图像分类中的不确定性，提高模型泛化能力，在不同成像模态下均表现出优异性能。
...
TL;DR: Conan是一个基于证据的多步骤视频推理框架，通过识别上下文和证据帧、跨帧推理以及自适应决策来提升视频推理能力，在多个基准测试中表现优于基线模型。


<details>
  <summary>Details</summary>
Motivation: 现有的视频推理方法存在局限性：基于强化学习的方法依赖纯文本链，容易产生无根据或幻觉的结论；基于帧检索的方法虽然引入视觉基础，但在证据定位方面仍不准确。

Method: 构建了Conan-91K大规模自动生成推理轨迹数据集，包含帧识别、证据推理和行动决策；设计了多阶段渐进冷启动策略与识别-推理-行动（AIR）RLVR训练框架，联合增强多步骤视觉推理。

Result: 在六个多步骤推理基准测试中，Conan平均准确率比基线Qwen2.5-VL-7B-Instruct高出10%以上，达到最先进性能；在长视频理解任务中表现出良好的泛化能力。

Conclusion: Conan框架有效解决了视频推理中的证据定位和多步骤推理问题，具有强大的可扩展性和鲁棒性，为多模态大语言模型的视频推理能力提供了重要改进。
...
TL;DR: 提出一个模块化的人口属性推断管道，通过迁移学习提高可靠性，为公平性审计提供更准确的人口统计基础。
...
Method: 使用预训练的人脸识别编码器结合非线性分类头的模块化迁移学习方法，替代传统的端到端训练。

Result: 该方法在性别和种族推断任务上优于强基线，特别是在更具挑战性的种族属性上表现更佳。

Conclusion: 该工作为公平性审计中的人口统计推断提供了可靠基础，并公开了数据集、代码和模型以促进透明度和可复现性。
...
TL;DR: EchoDistill是一个双向概念蒸馏框架，用于实现一步扩散个性化(1-SDP)，通过师生模型的双向训练和回声反馈机制，在保持快速生成的同时有效整合新概念。


<details>
  <summary>Details</summary>
Motivation: 当前一步文本到图像扩散模型虽然生成速度快，但在个性化新概念方面能力有限，难以有效捕捉新概念的分布特征。

Method: 采用端到端训练，同时训练多步扩散模型(教师)和一步扩散模型(学生)，通过双向概念蒸馏和回声反馈机制，共享文本编码器确保语义一致性，并引入对抗损失和对齐损失优化学生模型。

Result: 实验表明该协作框架在1-SDP设置下显著优于现有个性化方法，不仅提升了学生模型的新概念个性化能力，还改善了教师模型的生成质量。

Conclusion: EchoDistill为T2I扩散模型中的快速有效个性化建立了一个新范式，通过双向蒸馏机制实现了速度与质量的平衡。
...
<details>
  <summary>Details</summary>
Motivation: 当前多模态推理模型存在两个关键限制：对简单查询也使用计算昂贵的推理导致效率低下，以及专注于专业推理会损害更广泛的通用理解能力。
...
TL;DR: 提出了Fake-in-Facext框架，通过精细化的面部区域概念树和数据标注流程，解决现有可解释深度伪造分析方法缺乏细粒度感知能力的问题，并引入新的伪影定位解释任务。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在可解释深度伪造分析中存在细粒度感知不足的问题：数据标注不可靠且粗粒度，模型无法输出文本伪造解释与视觉伪影证据之间的关联，也不支持任意面部区域的查询输入。

Method: 首先定义面部图像概念树将面部图像划分为细粒度区域概念，建立可靠的数据标注流程FiFa-Annotator。然后提出伪影定位解释任务，生成包含分割掩码的文本伪造解释。最后设计统一的多任务学习架构FiFa-MLLM，同时支持丰富的多模态输入输出。

Result: FiFa-MLLM在AGE任务上优于强基线模型，在现有XDFA数据集上达到最先进性能。

Conclusion: FiFa框架通过细粒度的数据标注和模型架构设计，有效提升了可解释深度伪造分析的性能，解决了现有方法缺乏细粒度感知能力的问题。
...
TL;DR: 提出一种深度学习框架，从单张模糊图像联合估计潜在清晰图像和相机运动轨迹，在严重模糊情况下实现最先进的去模糊性能。


<details>
  <summary>Details</summary>
Motivation: 相机抖动引起的大范围或旋转运动模糊仍然是图像恢复中的主要挑战，现有端到端去模糊网络在严重或空间变化模糊情况下表现不佳。

Method: 采用投影运动模糊模型(PMBM)，通过可微分模糊创建模块实现。神经网络预测完整3D旋转轨迹，指导基于模型的端到端恢复网络，并通过重模糊损失优化轨迹。

Result: 在合成和真实数据集上均达到最先进性能，特别是在严重或空间变化模糊情况下表现优异。

Conclusion: 该方法通过联合估计清晰图像和相机运动轨迹，提供可解释性并支持重建生成模糊图像的清晰图像序列，在复杂模糊场景中优于现有方法。
...
<details>
  <summary>Details</summary>
Motivation: 尽管SLAM技术有所进展，但在低纹理、运动模糊或挑战性光照条件下的鲁棒操作仍是一个开放挑战，这些条件在视觉障碍者辅助导航等应用中很常见，会影响定位精度和跟踪稳定性。

Method: 提出了SELM-SLAM3框架，集成SuperPoint和LightGlue进行特征提取和匹配，使用深度学习增强传统SLAM系统。
...
TL;DR: 研究用户对不同细节层次和观看距离下人群角色表示的视觉质量感知，比较几何网格、图像代理、NeRF和3D高斯等表示的视觉保真度与计算性能权衡。


<details>
  <summary>Details</summary>
Motivation: 探索不同人群表示方法在视觉质量和计算效率之间的权衡，为设计感知优化的细节层次策略提供指导。

Method: 使用定性分析和定量评估，比较几何网格、图像代理、NeRF和3D高斯四种表示方法在不同细节层次和观看距离下的表现。

Result: 不同表示方法在视觉保真度和计算性能方面展现出明显的权衡关系，为优化人群渲染提供了具体见解。

Conclusion: 研究结果为设计感知优化的细节层次策略提供了有价值的指导，帮助在视觉质量和计算效率之间做出平衡决策。
...
TL;DR: 提出了EmbodiedBrain，一个用于具身AI的视觉语言基础模型，通过创新的训练方法和评估体系，在具身任务中实现了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 解决当前大语言模型和多模态大语言模型在具身任务中的关键限制，包括模型设计与智能体需求之间的差距、实时延迟与性能的权衡，以及离线评估指标的不真实性。

Method: 采用智能体对齐的数据结构，结合大规模监督微调(SFT)和步骤增强分组相对策略优化(Step-GRPO)的训练方法，并包含生成奖励模型(GRM)的全面奖励系统。

Result: 实验结果表明，EmbodiedBrain在所有指标上都实现了优越性能，为具身基础模型建立了新的最先进水平。

Conclusion: 通过开源所有数据、模型权重和评估方法，为下一代通用具身智能体的发展铺平道路。
...
TL;DR: Open-o3 Video是一个非代理框架，通过在视频推理中集成显式的时空证据，实现了基于具体视觉观察的推理。该方法通过精心构建的数据集和强化学习策略，在V-STAR基准上取得了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 现有的视频推理模型仅生成文本推理轨迹，而不指示关键证据出现的时间和位置。虽然OpenAI-o3等模型在图像上引发了证据中心推理的广泛兴趣，但将这种能力扩展到视频更具挑战性，因为它需要在动态场景中进行联合时间跟踪和空间定位。

Method: 首先构建了两个高质量数据集STGR-CoT-30k和STGR-RL-36k，具有精心构建的时间和空间标注。然后采用冷启动强化学习策略，设计了多个专门奖励函数，共同鼓励答案准确性、时间对齐和空间精度。

Result: 在V-STAR基准上，Open-o3 Video实现了最先进的性能，将Qwen2.5-VL基线的mAM提高了14.4%，mLGM提高了24.2%。在VideoMME、WorldSense、VideoMMMU和TVGBench等多个视频理解基准上也观察到一致的改进。
...
TL;DR: 提出了GenColorBench，首个针对文本到图像生成中颜色控制的综合基准测试，包含44K个颜色相关提示，覆盖400+种颜色，评估模型在颜色生成方面的精确性。


<details>
  <summary>Details</summary>
Motivation: 当前文本到图像生成模型在细粒度颜色可控性方面表现不佳，现有基准测试缺乏对颜色精度的系统性评估，而颜色在视觉感知和实际应用中至关重要。

Method: 基于ISCC-NBS和CSS3/X11颜色系统构建基准，包含数值颜色表示，通过感知和自动化评估方法对流行文本到图像模型进行测试。

Result: 评估显示不同模型在颜色生成性能上存在差异，揭示了模型对不同颜色约定的理解程度和失败模式。

Conclusion: GenColorBench基准将指导精确颜色生成技术的改进，该基准将在论文被接受后公开。
...
<details>
  <summary>Details</summary>
Motivation: 深度学习模型在视觉任务中表现优异，但在未见数据上性能会大幅下降。由于模型对领域偏移敏感，无监督领域适应旨在减少领域差距并避免对新领域的昂贵标注。

Method: 学习嵌入空间中的类别原型，引入相似性约束使原型对每个语义类别具有代表性且不同类别间可分离。使用字典存储不同图像提取的原型，防止类别缺失问题并实现原型对比学习。

Result: 大量实验表明，该方法比其他最先进方法获得了更好的结果。

Conclusion: 提出的基于相似性原型的框架在跨模态分割任务中有效解决了领域适应问题，显著提升了模型性能。
...
Result: 在真实世界数据集上的评估表明，OnlineSplatter显著优于最先进的无姿态重建基线方法，随着观察次数的增加持续改进，同时保持恒定的内存和运行时间。

Conclusion: 该方法能够有效处理自由移动物体，提供全面而紧凑的对象覆盖，在计算成本恒定的情况下实现高质量重建。
...
TL;DR: SeViCES是一个无需训练、模型无关的长视频理解框架，通过语义-视觉共识证据选择来解决长视频内容复杂、分散的问题，显著提升Video-LLMs的准确性和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 长视频理解面临计算成本高、推理不聚焦不一致的挑战。现有帧选择方法忽略时间依赖或仅依赖单模态证据，无法提供完整且与查询相关的上下文。

Method: 提出SeViCES框架，包含两个核心模块：SVCFS通过时间感知语义分支（基于LLM推理）和聚类引导视觉分支（通过互信息对齐）选择帧；ACR通过融合证据和约束答案空间来解决语义-视觉预测不一致问题。

Result: 在长视频理解基准测试中，SeViCES在准确性和鲁棒性方面持续优于最先进方法。

Conclusion: 共识驱动的证据选择对Video-LLMs至关重要，SeViCES证明了语义-视觉共识的有效性。
...
TL;DR: 本文系统综述了260篇关于深度学习在牙科图像分析中的应用研究，包括49篇公开牙科数据集论文和211篇深度学习算法论文，总结了该领域的数据集特征、模型架构、性能指标，并讨论了当前挑战和未来方向。
...
Method: 系统综述方法，重点关注深度学习研究的两个基本方面：数据集和模型。介绍了牙科成像基本概念，总结了现有数据集特征和获取方法，呈现了深度学习基础技术，并按不同牙科图像分析任务对相关模型和算法进行了分类分析。

Result: 全面总结了260项研究，包括49个公开数据集和211个深度学习算法，分析了网络架构、优化策略、训练方法和性能，并总结了牙科图像分析领域常用的训练和评估指标。

Conclusion: 深度学习在牙科图像分析中展现出巨大潜力，但仍面临挑战。该工作为该领域研究人员提供了有价值的系统性参考，所有补充材料和详细比较表格将在GitHub上公开。
...
Method: 采用因果卷积编码器-解码器架构，统一2D和3D训练推理，通过三级训练课程（局部重建、重叠窗口平铺、长上下文解码器精炼）学习从短切片片段到超过300切片的泛化能力。

Result: 在报告生成任务上，BLEU分数提升，临床F1分数比CT2Rep、CT-CHAT和Merlin提高40%；在文本到CT合成任务上，FID降低75%，FVD减半，生成512*512*241解剖一致的体积。

Conclusion: 精确的三维标记化对于3D医学影像中的可扩展视觉语言建模至关重要，而非仅依赖更大的语言主干网络。
...
Result: 在UltraHR-eval4K基准测试中显著提升了超高清图像生成的细粒度细节质量和整体保真度

Conclusion: 该方法通过高质量数据集和专门训练策略有效解决了超高清图像生成的细节合成问题
...
Method: 结合卷积特征提取、可微分自组织和脉冲时序处理的混合深度学习框架，使用预训练ResNet-152提取空间特征，然后通过可微分软自组织映射增强聚类和可解释性，最后使用脉冲神经网络头积累时序激活。

Result: 在十类垃圾数据集上达到97.39%的测试准确率，优于多个先进架构，同时保持轻量级计算特性适合实际部署。

Conclusion: 该框架通过精确自动的垃圾分类支持更高回收效率，减少可回收物流污染，降低废物处理生态和运营成本，符合联合国可持续发展目标。
...
<details>
  <summary>Details</summary>
Motivation: 现有的多比特量化网络需要为每个支持的比特宽度重复完整数据集更新，导致训练开销随精度数量线性增长，且需要额外的微调阶段来支持更多精度选项。

Method: 1. 权重偏置校正：通过中和跨比特宽度的量化诱导偏置和对齐激活分布，实现共享批量归一化，无需微调。2. 比特级核心集采样：利用隐式知识转移现象，通过基于梯度的重要性评分选择紧凑信息子集进行训练。

Result: 在CIFAR-10/100、TinyImageNet和ImageNet-1K数据集上，使用ResNet和ViT架构的实验表明，该方法在实现竞争性或更优准确率的同时，训练时间减少最多7.88倍。

Conclusion: 所提出的方法显著降低了多比特量化网络的训练开销，同时保持了模型性能，为灵活部署深度学习网络提供了高效解决方案。
...
TL;DR: 该论文提出了一个基于代理的架构，结合LLM推理与轻量级视觉模块，通过三阶段评估框架诊断多模态大语言模型的视觉幻觉和文本依赖问题，在多个基准测试上取得显著提升。
...
Method: 采用三阶段评估框架诊断模型失败模式，提出基于代理的架构，结合LLM推理与轻量级视觉模块，实现细粒度分析和推理链迭代优化。

Result: 在MMMU基准上提升10.3分，MathVista上提升6.0分（基于7B基线），性能匹配或超越更大模型。

Conclusion: 未来视觉推理模型应专注于集成更广泛的专门化视觉内容分析工具，作者将发布框架和评估套件以促进研究。
...
<details>
  <summary>Details</summary>
Motivation: 现有KV缓存压缩方法主要关注保留高重要性KV对来减少存储，但忽略了多模态KV缓存中出现的模态特定语义冗余模式，这限制了部署可扩展性。

Method: MixKV方法分析KV缓存在不同注意力头中的冗余度变化，自适应地平衡多样性和重要性来选择压缩的KV对，而不是仅依赖重要性。

Result: 在极端压缩（预算=64）条件下，MixKV平均提升基线方法5.1%的五个多模态理解基准性能，在GUI接地任务上对SnapKV和AdaKV分别提升8.0%和9.0%，同时保持相当的推理效率。

Conclusion: MixKV通过结合重要性和多样性来优化KV缓存压缩，有效解决了多模态KV缓存中的语义冗余问题，并可无缝扩展到LLMs中。
...
TL;DR: ALICE-LRI是一种传感器无关的方法，能够实现无损失的激光雷达距离图像生成，无需制造商元数据或校准文件，通过自动逆向工程激光雷达传感器的内在几何参数来实现零点损失。


<details>
  <summary>Details</summary>
Motivation: 传统激光雷达投影方法存在几何不一致性，导致不可逆的信息损失，影响高保真应用。需要一种能够实现无损失投影的方法来满足高精度遥感应用的需求。

Method: 自动逆向工程任何旋转激光雷达传感器的内在几何参数，包括激光束配置、角度分布和每束激光的校准校正，从而实现无损失投影和完整的点云重建。

Result: 在完整的KITTI和DurLAR数据集上的综合评估显示，ALICE-LRI实现了完美的点保留，所有点云中零点损失。几何精度保持在传感器精度限制内，具有实时性能。

Conclusion: 从近似到无损失激光雷达投影的范式转变，为需要完整几何保留的高精度遥感应用开辟了新的可能性。
...
<details>
  <summary>Details</summary>
Motivation: 解决长时域驾驶视频生成中几何一致性的挑战，确保生成的场景在长时间范围内保持几何连贯性。

Method: 1) 在共享潜在空间中联合处理图像和深度；2) 基于先前生成关键帧的几何信息进行条件生成；3) 使用扭曲一致引导控制采样过程；4) 用视频扩散模型插值关键帧生成密集视频。

Result: 生成了超过20秒的真实且几何一致的驾驶视频，长时域FID和FVD分数相比现有最佳方法分别提升了48.6%和43.0%。

Conclusion: AutoScape通过几何一致的关键帧生成和插值方法，显著提升了长时域驾驶场景生成的质量和几何一致性。
...
<details>
  <summary>Details</summary>
Motivation: 深度学习在组织病理图像分析中表现出色，但需要进一步提升语义分割性能，因此探索CNN和ViT的特征融合。

Method: 使用注意力驱动的特征融合机制，将卷积神经网络和视觉变换器集成在统一的双编码器模型中。

Result: 在GCPS数据集上达到μIoU/μDice为76.79%/86.87%，在PUMA数据集上达到64.93%/76.60%，优于现有最佳方法。

Conclusion: 提出的双编码器模型通过有效融合CNN和ViT特征，显著提升了组织病理图像的语义分割性能。
...
TL;DR: DyPE是一种无需训练的方法，通过动态调整扩散模型的位置编码来生成远超训练分辨率的高质量图像，解决了扩散Transformer在超高分辨率生成中的计算成本问题。


<details>
  <summary>Details</summary>
Motivation: 扩散Transformer模型在生成高质量图像方面表现出色，但在超高分辨率训练时由于自注意力机制的二次方计算复杂度导致成本极高。需要一种方法能够在无需重新训练的情况下扩展预训练模型的分辨率能力。

Method: DyPE利用扩散过程的频谱特性，在生成过程中动态调整位置编码的频率谱，使其与当前生成阶段匹配。该方法基于观察：低频结构在早期收敛，而高频细节需要更多步骤来解析。

Result: DyPE能够显著超越训练分辨率生成图像，例如使用FLUX生成1600万像素的图像。在多个基准测试中，DyPE持续提升性能，在超高分辨率图像生成方面达到最先进的保真度，且分辨率越高提升越明显。

Conclusion: DyPE提供了一种高效、无需训练的方法来扩展扩散Transformer模型的分辨率能力，在超高分辨率图像生成方面实现了突破性进展，且不增加采样成本。
...
Method: 提出了α-Flow目标族，统一了轨迹流匹配、Shortcut Model和MeanFlow，采用课程策略平滑地从轨迹流匹配过渡到MeanFlow。

Result: 在ImageNet-1K 256x256上，α-Flow在所有规模和设置下都优于MeanFlow，最大的α-Flow-XL/2+模型实现了FID 2.58(1-NFE)和2.15(2-NFE)的最新结果。

Conclusion: α-Flow通过解耦冲突目标并采用课程学习策略，有效解决了MeanFlow的优化问题，在少步生成建模中取得了最先进的性能。
...
<details>
  <summary>Details</summary>
Motivation: 解决从单张2D图像进行3D重建的挑战，特别是相机姿态估计、形状重建和纹理恢复的联合优化问题，旨在在统一框架下实现更准确和鲁棒的3D重建。
...
Conclusion: Cupid通过将输入相机姿态和3D形状表示为共享3D潜在空间中的分布，实现了准确且鲁棒的3D重建，在多个指标上显著优于现有领先方法。
...
Conclusion: 这是首个研究雷达-相机共同特征集成及其在在线标定中应用以实现MOT的工作，通过有效利用雷达的关键作用，显著提升了多目标跟踪的性能。
...
Method: SV框架采用两阶段方法：草稿阶段由小型VLM作为草稿专家生成多样化的定位候选推理路径；裁决阶段由强VLM综合这些路径生成最终答案。引入共识专家选择机制，仅转发高一致性的推理路径。

Result: 在具有挑战性的信息密集型和高分辨率视觉问答基准测试中，包括InfographicVQA、ChartMuseum、ChartQAPro和HR-Bench 4K，SV实现了持续的性能提升。

Conclusion: 通过综合多个部分准确推理路径的正确见解，SV相比大型专有模型或训练流程，实现了错误纠正和成本效率的平衡。
...
Method: 基于图像生成的分割框架，利用MLLM输出视觉标记，通过通用VQ-VAE将其解码为图像，使分割完全依赖于MLLM的像素级理解，并采用下一尺度预测策略并行生成视觉标记以减少推理延迟。

Result: 在多个分割数据集上超越了先前最先进方法，推理速度显著提升，同时保持强大的理解能力。

Conclusion: ARGenSeg为图像分割提供了一种有效的自回归生成范式，实现了多模态理解和像素级感知的统一，在性能和效率方面均表现出色。
...
TL;DR: 提出了一个基于纯transformer的自回归视频预测模型，通过连续像素空间表示进行视频预测，无需复杂训练策略或潜在特征学习组件，在物理模拟预测方面显著提升了时间范围。


<details>
  <summary>Details</summary>
Motivation: 受自回归大语言模型性能和可扩展性的启发，研究将transformer应用于视频预测领域，重点关注物理模拟的时间因果建模，这是现有视频生成方法的常见短板。

Method: 使用纯transformer模型进行自回归视频预测，采用连续像素空间表示，比较不同的时空自注意力布局，通过物理对象跟踪指标和无监督训练来隔离时空推理。

Result: 与现有潜在空间方法相比，该方法将物理准确预测的时间范围显著延长了50%，同时在常见视频质量指标上保持相当性能。通过可解释性实验发现网络能够准确估计PDE模拟参数。

Conclusion: 这项工作为基于注意力的视频时空建模提供了一个简单、参数高效且可解释的平台。
...
TL;DR: SpectraMorph是一种物理引导的自监督高光谱超分辨率框架，通过解混瓶颈结构实现高光谱和多光谱图像的融合，具有可解释性、快速训练和鲁棒性强的特点。


<details>
  <summary>Details</summary>
Motivation: 现有深度学习方法依赖不透明的回归器，缺乏可解释性，且在MSI波段很少时容易失败。需要一种既能保持高性能又具有物理可解释性的方法。

Method: 采用解混瓶颈结构：从低分辨率HSI提取端元特征，用紧凑的多层感知机从MSI预测丰度图，通过线性混合重建光谱，使用MSI传感器光谱响应函数进行自监督训练。

Result: 在合成和真实数据集上，SpectraMorph始终优于最先进的无监督/自监督基线方法，与有监督基线方法相比也具有很强竞争力。

Conclusion: SpectraMorph框架能够产生可解释的中间结果，训练时间不到一分钟，即使在单波段MSI情况下也能保持鲁棒性，为高光谱超分辨率提供了一种有效的物理引导解决方案。
...
TL;DR: 提出了一种基于潜在变量扩展的扩散桥模型LDDBM，用于跨模态转换任务，支持任意模态对之间的转换。


<details>
  <summary>Details</summary>
Motivation: 现有扩散模型在单模态领域表现出色，但在跨模态转换方面存在限制，如要求共享维度、高斯先验假设和特定模态架构，限制了方法的通用性和理论基础。

Method: 在共享潜在空间中学习模态间的桥梁，使用对比对齐损失确保语义一致性，设计域无关的编码器-解码器架构进行潜在空间噪声预测，并提出预测损失指导训练。

Result: 在多种模态转换任务上表现优异，包括多视图到3D形状生成、图像超分辨率和多视图场景合成，验证了框架的有效性。

Conclusion: LDDBM为通用模态转换建立了新的强基线，支持任意模态对之间的转换，具有广泛适用性。
...
Result: 在多个主体个性化图像生成中，LayerComposer实现了优于现有技术的空间控制和身份保持。

Conclusion: LayerComposer通过直观的分层操作和无需架构改变的锁定机制，为多主体个性化图像生成提供了有效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 解决现有文本到视频模型在生成连贯多镜头叙事方面的"叙事差距"，这些模型擅长生成孤立片段但无法创建连贯的叙事场景。

Method: 采用窗口交叉注意力机制将文本提示定位到特定镜头，以及稀疏镜头间自注意力模式（镜头内密集但镜头间稀疏），确保分钟级生成效率。
...
TL;DR: DeBERTa-KC是一个基于Transformer的模型，用于自动分类在线科学学习讨论中的知识建构水平，在YouTube科学频道评论数据集上取得了0.836的宏F1分数，显著优于传统方法。
...
Method: 扩展DeBERTa-v3模型，结合Focal Loss、Label Smoothing和R-Drop正则化来处理类别不平衡并增强泛化能力，构建了端到端的可重复流程。

Result: 在10折分层交叉验证中，DeBERTa-KC实现了0.836±0.008的宏F1分数，显著优于经典和Transformer基线模型(p<0.01)，对高阶认知参与表现出强敏感性。

Conclusion: 大语言模型能有效捕捉非正式数字学习环境中知识建构的细微指标，为话语分析和认知参与评估工具的开发提供了可扩展的理论指导方法。
...
<details>
  <summary>Details</summary>
Motivation: 评估AI生成课程计划的教学质量，比较不同大语言模型和提示框架在生成教育内容方面的表现差异。
...
<details>
  <summary>Details</summary>
Motivation: 传统离散扩散模型存在训练-推理差异，导致并行解码时初始令牌错误污染生成上下文，引发错误级联，造成语法错误和语义幻觉。

Method: 采用两阶段训练：首先训练模型修正合成错误，建立基础修正能力；然后通过在线自修正循环，让模型学习修正自己的错误草稿。
...
TL;DR: 提出Sparse Tracing技术，通过动态稀疏注意力高效分析长上下文注意力模式，Stream算法以近线性时间和线性空间估计稀疏注意力掩码，实现大规模单次可解释性分析。
...
Method: 提出Stream算法，采用可编译的分层剪枝方法，通过二分搜索式精炼保留每个查询的前k个关键块，同时保持模型的下一个token行为，在近线性时间O(T log T)和线性空间O(T)内估计稀疏注意力掩码。

Result: 在长链式思维推理轨迹中识别思维锚点，剪枝97-99%的token交互；在RULER基准测试中保留关键检索路径，丢弃90-96%的交互，并暴露从针到输出的分层路由。
...
TL;DR: 提出了一种利用大语言模型分析电子健康记录中的非结构化文本数据，以确定患者是否适合进行HIV进一步检测的新流程。


<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注结构化数据，忽略了临床笔记等非结构化文本中可能包含的HIV风险相关信息，而大规模实验室检测不可行，需要更有效的筛查方法。

Method: 开发了一个基于大语言模型的流程，用于分析电子健康记录中的非结构化文本数据，评估患者进行HIV检测的资格。

Result: 在伊拉斯姆斯大学医学中心鹿特丹的临床数据上，该流程实现了高准确率，同时保持了较低假阴性率。

Conclusion: 利用大语言模型分析电子健康记录非结构化文本是提高HIV筛查效率的有效方法，能够在不增加假阴性的情况下实现准确检测。
...
TL;DR: 该研究首次对大型语言模型在生命周期评估中的表现进行了专家基准测试，评估了11个模型在22个LCA相关任务中的表现，发现37%的响应包含不准确或误导性信息，但某些模型在解释质量和格式遵循方面表现良好。


<details>
  <summary>Details</summary>
Motivation: 随着AI和大型语言模型在生命周期评估中应用的增加，缺乏对其可靠性、稳健性和可用性的系统性评估，特别是在没有明确基准和共识协议的领域。

Method: 评估了11个通用LLM模型，涵盖商业和开源类型，在22个LCA相关任务上的表现。17位经验丰富的从业者根据科学准确性、解释质量、稳健性、可验证性和指令遵循等标准审查模型输出，共收集168份专家评审。

Result: 37%的响应被专家判断为包含不准确或误导信息；准确性和解释质量评分普遍为中等或良好；幻觉率差异显著，某些模型的引用幻觉率高达40%；开源模型在准确性和解释质量方面表现与闭源模型相当或更好。

Conclusion: 研究强调了在LCA中简单应用LLM的风险，特别是将其视为自由形式的神谕时，同时也显示了在解释质量和减轻简单任务劳动强度方面的益处。没有基础机制支持的通用LLM使用存在风险。
...
TL;DR: 提出基于游戏的评估方法，通过Dixit卡牌游戏来全面评估多模态大语言模型的能力，避免传统基准测试的主观性和模型利用表面捷径的问题。


<details>
  <summary>Details</summary>
Motivation: 传统多模态大语言模型评估存在静态基准测试无法全面评估能力、人工或模型对比主观性强且成本高、模型可能利用表面捷径（如冗长回答）来提升胜率等问题。

Method: 采用游戏化评估方法，使用Dixit幻想卡牌游戏，要求模型为卡片生成既能欺骗部分玩家又不被所有玩家识破的标题。

Result: 实验显示Dixit胜率排名与主流多模态大语言模型基准测试结果完全相关，同时揭示了人类与模型玩家在策略上的差异以及模型推理能力的改进空间。
...
TL;DR: 该研究评估DeepSeek-R1 LLM在运筹学优化建模中的潜力，通过四种基准测试和多种缓解策略来减少幻觉并提高模型输出准确性。


<details>
  <summary>Details</summary>
Motivation: 传统优化方法依赖领域专家将实际问题转化为数学模型，而LLM有望通过自然语言理解和代码生成来弥合这一建模差距。

Method: 使用DeepSeek-R1模型在四个OR基准(NL4OPT、IndustryOR、EasyLP、ComplexOR)上进行系统评估，采用LLM-as-a-Judge、Few-shot Learning、Tool Calling和多智能体框架等策略来减少幻觉。

Result: DeepSeek-R1在成本效益和性能方面表现出色，但在实际OR场景中的有效性仍需进一步探索。

Conclusion: DeepSeek-R1为运筹学优化建模提供了可行的替代方案，但需要通过系统评估和缓解策略来确保其在实际应用中的可靠性。
...
TL;DR: 提出了一个基于记忆增强的LLM代理框架，通过存储实例级批评和提炼任务级指导，实现无需参数更新的分类函数学习，相比仅使用标签的检索方法准确率提升达24.8%。


<details>
  <summary>Details</summary>
Motivation: 传统微调方法成本高、灵活性差且不透明，需要探索无需参数更新就能从标注示例中学习分类函数的方法。

Method: 使用情景记忆存储实例级批评（具体经验），语义记忆将这些批评提炼为可重用的任务级指导，结合标注数据和LLM生成的批评进行学习。

Result: 在多样化任务集上，融入批评的方法比仅依赖标签的检索基线准确率提升高达24.8%，揭示了OpenAI和开源模型在处理事实导向与偏好数据时的行为差异。

Conclusion: 记忆驱动的反思学习为构建更具适应性和可解释性的LLM代理提供了前景，新引入的suggestibility指标有助于解释模型对不同监督表示的反应。
...
<details>
  <summary>Details</summary>
Motivation: 现有歌词翻译方法依赖手工规则和句子级建模，难以内化音乐-语言模式，在段落级翻译中缺乏跨行连贯性和全局韵律控制。

Method: 提出LyriCAR框架，包含难度感知课程设计师和自适应课程策略，通过逐步增加训练难度来优化资源分配和加速收敛。

Result: 在EN-ZH歌词翻译任务中，LyriCAR在标准翻译指标和多维奖励分数上均达到最优效果，自适应课程策略减少近40%训练步骤。

Conclusion: LyriCAR证明了无监督课程学习在歌词翻译中的有效性，为平衡音乐约束和翻译质量提供了新思路。
...
<details>
  <summary>Details</summary>
Motivation: 开发一种模拟人脑解析过程的自然语言解析算法，采用依赖树结构而非短语结构，实现高效的句子解析。

Method: 采用逐词处理策略，每个单词在可附加时立即附加到依赖树中，算法复杂度为O(n³)，但在自然语言中仅在小规模时出现最坏情况。

Result: 成功开发出基于依赖关系的解析算法，能够有效解析自然语言句子，复杂度可控。

Conclusion: 该依赖解析算法模拟人脑处理方式，在自然语言解析中具有实用价值，复杂度在真实语言环境中表现良好。
...
TL;DR: 提出了一个统一范式来评估LLM在临床决策中的表现，通过临床背景和临床问题两个维度来衡量任务难度，并扩展评估指标到效率、可解释性等方面。


<details>
  <summary>Details</summary>
Motivation: 现有医学数据集（如MedQA）过于简化，不能充分代表真实临床决策的复杂性，需要更贴近真实临床环境的评估方法。

Method: 提出基于临床背景和临床问题两个维度的统一评估范式，总结现有数据集设置，回顾训练时和测试时的技术方法，并扩展评估指标。
...
TL;DR: 本文提出了希腊语嵌入模型(GEM)，通过精心构建的大规模希腊语语料库和现代架构(如ELECTRA、ConvBERT、ModernBERT)，解决了希腊语NLP研究中架构单一和上下文长度受限的问题，特别是在法律领域取得了显著性能提升。


<details>
  <summary>Details</summary>
Motivation: 希腊语等形态丰富的中等资源语言在NLP发展中面临研究碎片化、架构多样性缺乏和上下文长度受限的问题，特别是在法律等专业领域，现有模型通常局限于早期Transformer架构和512个token的窗口，无法处理长法律文档。

Method: 通过严格的质量驱动数据筛选和预处理方法构建大规模希腊语语料库，包括通用领域和专业法律来源；在此基础上预训练和系统评估多种现代架构(ELECTRA、ConvBERT、ModernBERT)；提出首个针对法律领域的希腊语-英语双语嵌入模型。

Result: 在下游任务上的广泛实验表明，新模型类别验证了所提方法的有效性，特别是GEM-RoBERTa和GEM-ConvBERT模型显著优于现有基线模型。

Conclusion: 通过精心策划的数据和多样化的现代架构，成功开发了希腊语嵌入模型系列，为希腊语NLP研究提供了新的解决方案，特别是在法律领域取得了突破性进展。
...
Method: 1) 在事件触发检测任务中引入领域无关文本处理系统的额外信号构建多任务模型；2) 修改自回归大语言模型的架构，实现跨层双向信息流；3) 采用生成式监督上下文微调框架结合响应导向适应策略。

Result: 提出的模型、方法和框架表明，预训练神经语言模型通过有针对性的迁移学习范式，在序列标注任务上能达到最佳性能。

Conclusion: 通过三种改进的迁移学习方法，预训练语言模型在序列标注任务中的性能得到显著提升，证明了针对性迁移学习策略的重要性。
...
<details>
  <summary>Details</summary>
Motivation: 现实世界工具集存在冗余工具，工具名称和描述重叠导致歧义，降低选择准确性；同时LLM面临严格的输入上下文限制，无法高效处理大型工具集

Method: ToolScope包含两个组件：(1) ToolScopeMerger with Auto-Correction自动审计和修复工具合并，减少冗余；(2) ToolScopeRetriever对每个查询进行工具排序和选择，压缩工具集以适应上下文限制

Result: 在三个最先进LLM和三个开源工具使用基准上的评估显示，工具选择准确率提升了8.38%到38.6%
...
Method: 构建了包含民间传统、烹饪艺术和区域方言的孟加拉语文化知识数据集BLanCK，并评估了多个多语言语言模型在该数据集上的表现。
...
<details>
  <summary>Details</summary>
Motivation: 提升小语言模型在专门应用（如波斯语医学问答）中的推理能力，特别是在资源匮乏的语言环境中

Method: 采用强化学习与AI反馈(RLAIF)和直接偏好优化(DPO)，翻译医学问答数据集，生成拒绝-偏好答案对，并训练模型产生链式思维推理

Result: 使用200万token偏好答案和250万token拒绝答案训练后，模型性能超越了基于5700万token训练的前代模型
...
Method: 设计CreativityPrism框架，包含发散思维、创意写作和逻辑推理三个领域，九个任务，二十个评估指标，对17个最先进的专有和开源大语言模型进行评估。

Result: 专有模型和开源模型之间存在显著差距；同一领域内任务表现高度相关，不同领域间相关性较弱；多样性和质量指标强相关，新颖性与其他维度相关性较弱。

Conclusion: 创造力评估需要整体性方法，单一任务或维度的优秀表现不能推广到其他方面，支持创造力不是单一固定概念的观点。
...
Method: ARTER采用结构化流水线，包括候选生成、上下文评分、自适应路由和选择性推理，通过计算互补信号将实体提及分为简单和困难两类，分别由低计算成本链接器和针对性LLM推理处理。

Result: 在标准基准测试中，ARTER比ReFinED性能提升高达+4.47%，在6个数据集中的5个上平均增益+2.53%，与全LLM推理方法性能相当但LLM token使用量减少一半。
...
TL;DR: BoundRL是一种用于复杂结构化文本分割的新方法，通过生成起始标记序列并使用强化学习优化，显著降低推理成本并减少幻觉。


<details>
  <summary>Details</summary>
Motivation: 随着结构化文本在技术报告和生成式AI提示等领域的复杂性增加，传统基于句子或段落的分割方法无法有效处理包含表格、代码片段和占位符等元素的文本。

Method: BoundRL联合执行标记级文本分割和标签预测，仅生成起始标记序列，通过强化学习与可验证奖励（RLVR）优化文档重建保真度和语义对齐，并构建中间候选来缓解熵崩溃。

Result: 实验表明BoundRL使小型语言模型（1.7B参数）在复杂LLM应用提示分割上优于大型模型的少样本提示，RLVR相比监督微调有显著改进，中间候选进一步提升了性能和泛化能力。

Conclusion: BoundRL为复杂结构化文本分割提供了一种高效解决方案，通过创新的起始标记生成和强化学习方法，在保持高质量的同时大幅降低了计算成本。
...
TL;DR: 研究发现大型语言模型在立场检测任务中存在显著刻板印象，会错误地将支持大麻的观点与低文本复杂度关联，将非裔美国人方言与反对特朗普关联。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型从预训练数据中继承了刻板印象，导致在NLP任务中对某些社会群体产生偏见行为。然而，立场检测方法中的这种偏见评估在社区中被大大忽视了。

Method: 在零样本设置下，自动标注现有立场检测数据集中的帖子，添加两个属性：特定群体的方言/语言变体和文本复杂度/可读性，以研究这些属性是否影响模型的立场检测决策。

Result: LLMs在立场检测任务中表现出显著的刻板印象，例如错误地将支持大麻的观点与低文本复杂度关联，将非裔美国人方言与反对特朗普关联。

Conclusion: 大型语言模型在立场检测任务中存在明显的刻板印象偏见，需要进一步研究和解决这种偏见问题。
...
TL;DR: 提出了DeepWideSearch基准，用于评估信息搜索代理在深度推理和广度信息收集方面的能力，现有最先进代理成功率仅2.39%


<details>
  <summary>Details</summary>
Motivation: 当前搜索代理无法同时进行深度多跳检索推理和大规模信息收集，限制了在真实世界应用中的能力

Method: 通过转换现有数据集构建了包含220个问题、覆盖15个领域的基准，要求代理处理大量数据并进行多跳检索推理

Result: 实验显示最先进代理平均成功率仅2.39%，识别出四种失败模式：缺乏反思、过度依赖内部知识、检索不足和上下文溢出

Conclusion: DeepWideSearch基准揭示了当前代理架构的关键局限性，为开发更强大信息搜索代理提供了重要测试平台
...
<details>
  <summary>Details</summary>
Motivation: 现有方法存在局限性：基于微调的方法容易产生算术错误和幻觉，而基于工具的方法缺乏语义理解。需要结合稳健推理和可靠表格处理的方法。

Method: 使用多智能体框架，将任务分解为规划、编码和回答三个角色，通过代码执行进行精确表格操作，并采用MCTS生成伪黄金轨迹和强化学习优化智能体。

Result: 在TableBench上达到62.13%的准确率，超过了OpenAI-o4-mini-high，显示出显著性能提升。

Conclusion: 结合结构化多智能体工作流和强化学习在表格理解方面具有巨大潜力，能够有效整合推理能力和表格处理能力。
...
TL;DR: 本文通过五项任务测试大语言模型在文本输入上的空间推理能力，发现模型在小规模任务中表现中等，但随着复杂度增加，性能急剧下降，平均准确率下降42.7%，最高达84%。


<details>
  <summary>Details</summary>
Motivation: 探索大语言模型在文本输入上的空间理解和计算能力，评估其从简单模式识别到抽象空间推理的扩展能力。
...
Conclusion: LLMs缺乏稳健的空间表征能力，语言推理和空间推理之间存在显著差距，为未来语言与几何交叉领域的集成基准测试奠定了基础。
...
<details>
  <summary>Details</summary>
Motivation: 现有语言模型评估只考虑特定分词方式的概率，但同一文本可能有多种分词方式。边际概率评估更准确，但传统采样方法需要昂贵的生成步骤，限制了采样数量和近似精度。

Method: 采用解码无关的采样策略，不依赖语言模型生成，而是使用模型和分词器无关的廉价采样方法，直接计算序列概率。

Result: 实验表明，解码无关采样策略在多个开源模型上都能提供足够准确的边际概率估计，且运行时间成本仅为传统方法的一小部分。

Conclusion: 解码无关采样策略是实现高效边际概率评估的有效方法，在保持精度的同时显著降低了计算成本，适用于下游推理任务。
...
TL;DR: 提出了一个三模态情感严重程度框架，通过融合文本、音频和面部信号来评估抑郁症和PTSD的严重程度，在DAIC数据集上表现优于单模态基线，并提高了决策曲线效用和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 抑郁症和PTSD经常共病且症状相互关联，但现有的自动化评估通常是二元且针对特定障碍的。临床上需要能够提供严重程度感知的跨障碍评估和决策支持解释。

Method: 统一的三模态情感严重程度框架，同步融合：1）访谈文本（句子级transformer嵌入）2）音频（log Mel统计特征和delta）3）面部信号（动作单元、注视、头部和姿态描述符）。通过校准的后期融合分类器输出分级严重程度。

Result: 在DAIC数据集上的分层交叉验证显示：融合模型在准确率和加权F1上与最强单模态基线相当，同时提高了决策曲线效用和噪声/缺失模态下的鲁棒性。对于PTSD，融合减少了回归误差并提高了类别一致性。
...
TL;DR: ContextLM框架通过引入next-context预测目标来增强标准预训练，克服了传统next-token预测在捕捉高层次语义结构和长距离上下文关系方面的限制。


<details>
  <summary>Details</summary>
Motivation: 传统next-token预测限制了模型捕捉高层次语义结构和长距离上下文关系的能力，需要一种更有效的预训练方法。
...
Conclusion: next-context预测提供了一条可扩展且高效的路径来构建更强的语言模型，能够以最小计算开销获得更好的长距离连贯性和更有效的注意力分配。
...
TL;DR: 本文提出将引用失败与回答失败区分开来，研究引用失败的原因并提出CITENTION框架来改善LLM的引用质量。


<details>
  <summary>Details</summary>
Motivation: 现有基于LLM的RAG系统虽然声称简化了回答验证，但在引用失败情况下（模型生成有用回答但未能完整引用证据）无法发挥作用。需要专门研究引用失败问题。

Method: 采用两步法：1) 研究引用失败的发生条件，引入CITECONTROL基准系统分析回答与证据关系对引用质量的影响；2) 提出CITENTION框架，整合生成式、基于注意力和基于检索的方法来改善引用。

Result: 实验表明失败率随关系复杂性增加而上升，组合多种引用方法可提升性能。CITENTION框架在CITECONTROL基准和迁移设置中显著改善了引用质量。

Conclusion: 通过区分引用失败与回答失败，并开发专门的基准和框架，能够有效提升LLM在RAG系统中的引用可靠性。
...
TL;DR: 本文首次系统研究过程奖励模型在表格问答任务中的应用，发现结合文本和代码验证的PRMs能帮助选择答案，但在跨域泛化方面存在困难，且步骤验证与答案准确性的相关性较弱。
...
Result: 结果显示：结合文本和代码验证的PRMs能帮助解决方案选择，但在跨域数据上泛化困难；步骤级验证性能与答案准确性之间相关性较弱，可能源于步骤依赖性和因果联系松散。
...
Method: 提出Hint-Engineering数据合成策略，在推理路径中策略性地注入多样化提示，生成高质量代码集成推理数据，并通过拒绝采样和强化学习优化模型与代码解释器的多轮交互。

Result: 在五个挑战性数学推理数据集上，DeepSeek-R1-Distill-Qwen-32B和1.5B模型分别获得4%和8%的绝对改进，同时32B和1.5B模型的token使用量分别减少约30%和50%。
...
Method: 通过一系列受控探测实验，比较LLMs在包含强语义线索的数据集与移除或随机化语义线索的数据集上的表现差异。

Result: 当数据集包含有意义的列名或可解释的值类别时，LLMs表现出污染效应；当这些线索被移除或随机化后，性能急剧下降到接近随机水平。

Conclusion: LLMs在表格推理任务中表现出的能力可能部分反映了对公开数据集的记忆，而非真正的推理能力，需要改进评估协议来区分语义泄漏与真实推理能力。
...
Method: 提出FreeChunker框架，将句子视为原子单元，从静态分块转向支持任意句子组合的灵活检索，减少语义边界检测的计算开销。

Result: 在LongBench V2上的实验评估显示，FreeChunker相比传统分块方法实现了更优的检索性能，并在计算效率上显著优于现有方法。

Conclusion: FreeChunker通过范式转变，从根本上改进了传统分块方法，在保持高检索性能的同时大幅提升计算效率。
...
TL;DR: 研究对话数据预训练对小语言模型的影响，发现虽然模型在标准基准测试中表现不佳，但在对话延续预测方面表现优异。DPO微调能进一步提升对话性能。
...
Result: 模型在大多数标准BabyLM基准测试中表现不佳，但在最小对设置中的对话延续预测方面表现优异。PPO微调效果混杂甚至有害，而DPO微调能进一步提升对话基准测试性能。

Conclusion: 对话专用预训练能产生在对话任务上表现良好的小语言模型，DPO微调是提升对话性能的有效方法，但模型在通用语言任务上仍有局限。
...
TL;DR: 该论文研究了大型语言模型在否定文本中的幻觉检测问题，发现LLMs难以有效识别否定语境下的幻觉，并构建了NegHalu数据集进行验证。


<details>
  <summary>Details</summary>
Motivation: 探索否定文本对LLMs幻觉的影响，这是当前自然语言处理研究中尚未充分探索的领域。

Method: 构建NegHalu数据集（通过重构现有幻觉检测数据集添加否定表达），在否定语境下测试LLMs的幻觉检测能力，并在token级别追踪LLMs的内部状态。

Result: LLMs在否定文本中难以有效检测幻觉，经常产生逻辑不一致或不忠实的判断。

Conclusion: 否定文本对LLMs的幻觉检测构成显著挑战，需要进一步研究来减轻其意外影响。
...
Method: 该任务包含两个子任务：多模态法律检索和多模态问答，参与者需要开发能够处理多模态法律信息的智能系统。

Result: 在VLSP 2025 MLQA-TSR上报告的最佳结果是：多模态法律检索的F2分数为64.55%，多模态问答的准确率为86.30%。

Conclusion: VLSP 2025 MLQA-TSR为多模态法律领域研究提供了重要的基准数据集，展示了当前技术在交通标志法规理解方面的性能水平。
...
TL;DR: 提出了NeoDictaBERT和NeoDictaBERT-bilingual两个专注于希伯来语文本的BERT风格模型，在希伯来语基准测试中表现优异，特别是在检索任务上超越同规模多语言模型。


<details>
  <summary>Details</summary>
Motivation: 现有BERT模型虽然性能出色但架构已过时，且缺乏专门针对希伯来语的优化模型。

Method: 采用与NeoBERT相同的架构，专门针对希伯来语文本进行训练。

Result: 在几乎所有希伯来语基准测试中超越现有模型，双语模型在检索任务上表现尤其突出。

Conclusion: 这些模型为希伯来语NLP研究和开发提供了强大基础，已向社区发布。
...
TL;DR: ContingentChat是一个教师-学生框架，通过在100M词BabyLM上进行针对性后训练，提升多轮对话的连续性质量。


<details>
  <summary>Details</summary>
Motivation: 儿童与照顾者之间的多轮对话具有连续性特征，即对话者之间及时、直接且有意义的交流。现有BabyLM在这方面存在不足。

Method: 使用新颖的对齐数据集进行后训练，采用教师-学生框架，并实验自适应教师解码策略。

Result: BabyLM生成的回复更加语法正确和连贯，自适应解码策略带来的额外收益有限。

Conclusion: 针对性后训练能提升对话质量，但连续性对BabyLM来说仍然是一个具有挑战性的目标。
...
TL;DR: 提出了指令蒸馏任务，将低质量指令数据转化为高质量指令-输出对，并开发了LM-Mixup方法，通过强化学习优化模型，在仅使用3%数据的情况下超越全数据集训练效果。


<details>
  <summary>Details</summary>
Motivation: 指令调优对LLM对齐至关重要，但高质量数据稀缺而低质量数据丰富却被丢弃。现有数据增强方法难以有效利用低质量数据，且评估标准不明确。

Method: 构建MIXTURE数据集（144K样本），提出LM-Mixup方法：先在MIXTURE上监督微调，然后用强化学习优化，使用质量、语义对齐和格式合规三个奖励信号，通过GRPO策略优化。

Result: LM-Mixup蒸馏的数据仅占整个数据集的3%，但微调LLM后不仅超越全数据集训练，还能与最先进的高质量数据选择方法竞争多个基准测试。
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型的输出存在不确定性和正确性差异，实际可靠性难以保证，需要量化这种不确定性。

Method: 在四个问答任务上使用开源LLM，系统评估VCE、MSP、Sample Consistency和CoCoA四种置信度估计方法。

Result: 每种不确定性指标捕捉模型置信度的不同方面，混合CoCoA方法在整体可靠性上表现最好，改善了校准和正确答案的区分度。
...
TL;DR: 提出了改进的掩码语言建模方法，通过根据模型预测能力调整掩码标记的概率，在BabyLM挑战赛严格小规模赛道中超越了基线模型。


<details>
  <summary>Details</summary>
Motivation: 标准掩码语言建模(MLM)存在局限性，需要改进以提升模型在语言理解任务上的性能，特别是在小规模数据训练场景下。

Method: 1. 改进的掩码语言建模：根据模型预测能力动态调整掩码标记的概率；2. 引入子词嵌入以增强形态学泛化能力。

Result: 在(Super)GLUE任务上相比标准MLM有显著性能提升，在BabyLM挑战赛严格小规模赛道中超越了基线模型。

Conclusion: 改进的掩码策略和子词嵌入能有效提升小规模语言模型的性能，特别是在语言理解和形态学泛化方面。
...
TL;DR: RECALL是一个基于表示感知的模型融合框架，通过分析层间隐藏表示来计算模型间相似性，并进行自适应层次参数融合，实现无需历史数据的持续学习。


<details>
  <summary>Details</summary>
Motivation: 发现大语言模型的内部表示可以作为学习知识的可靠代理，旨在解决持续学习中无需访问历史数据的问题，避免灾难性遗忘。

Method: 通过聚类典型样本计算层间隐藏表示的模型相似性，进行自适应层次参数融合，浅层保留领域通用特征，深层允许任务特定适应。

Result: 在五个NLP任务和多种持续学习场景中的广泛实验表明，RECALL在知识保留和泛化方面优于基线方法。

Conclusion: RECALL提供了一个可扩展且无需数据的解决方案，用于持续学习中的大语言模型演化，实现了无缝多领域集成和强大的抗遗忘能力。
...
TL;DR: 通过向LLM的激活向量添加控制向量，可以抑制模型在评估时的警觉行为，使其表现得像在部署环境中一样，从而提高安全评估的可靠性。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在评估时能够检测到被测试并调整行为以显得更加对齐，这会损害安全评估的可靠性。

Method: 使用两步训练过程训练LLM展现评估警觉行为：首先在包含模型事实描述的文档上进行持续预训练，然后通过专家迭代训练模型在评估环境中使用Python类型提示。最后通过激活向量控制技术抑制评估警觉性。

Result: 训练后的模型在评估环境中比部署环境中更频繁地使用类型提示，但只有移除评估线索才能观察到这种差异。激活向量控制能够抑制评估警觉性，使模型即使存在评估线索也表现得像在部署中。

Conclusion: AI评估者可以通过控制模型使其表现得像在部署环境中，来提高安全评估的可靠性。
...
TL;DR: 提出了Robust Preference Selection (RPS)方法，通过方向性邻域共识在无需重新训练的情况下解决LLM偏好覆盖不足的问题。


<details>
  <summary>Details</summary>
Motivation: 现有LLM训练数据主要反映主流偏好，导致模型在特定、个性化需求上表现不佳，存在偏好覆盖差距。现有方法需要昂贵重新训练且难以覆盖全部偏好范围。

Method: RPS是一种后处理、无需训练的方法，通过在相关偏好邻域采样多个响应创建候选池，然后选择与用户原始意图最匹配的响应。

Result: 在三种对齐范式(DPA、DPO、SFT)上的实验表明，RPS相比基线方法在代表性不足区域的挑战性偏好上实现了高达69%的胜率。

Conclusion: RPS提供了一个实用且理论基础的解决方案，用于增强偏好对齐模型的可靠性。
...
TL;DR: 本文提出了HSEQ分层序列迭代框架，用于解决多步骤问题和异构证据源检索增强生成(RAG)的脆弱性问题，通过统一处理文本、表格和知识图谱，在保持准确性的同时提高效率。


<details>
  <summary>Details</summary>
Motivation: 现有检索增强生成(RAG)方法在处理多步骤问题和异构证据源时存在脆弱性，需要在准确性、延迟和token/工具预算之间进行权衡。

Method: 将文档、表格和知识图谱线性化为可逆的分层序列，使用轻量级结构标签，通过结构感知迭代收集足够证据，由Head Agent指导检索，Iteration Agent通过结构尊重动作选择和扩展HSeq，最后合成规范化证据生成答案。

Result: 在HotpotQA、HybridQA/TAT-QA和MetaQA上的实验显示，相比单次传递、多跳和代理RAG基线，HSEQ在EM/F1指标上获得一致提升，且具有高效率。

Conclusion: HSEQ框架具有三个关键优势：格式无关的统一处理、预算感知的引导迭代、证据规范化，能够提高答案一致性和可审计性。
...
TL;DR: 本文提出了一种评估大语言模型政治偏见的新方法，通过分析欧洲议会演讲的多语言翻译质量差异来识别系统性偏见。


<details>
  <summary>Details</summary>
Motivation: 传统方法通过英语调查问卷评估LLM的政治偏见存在局限，需要基于多语言翻译公平性原则的替代框架来更系统地检测偏见。

Method: 使用新构建的21种语言平行语料库EuroParl，包含150万句子、4000万单词，涵盖3年数据、1000+发言人、7个国家、12个欧盟政党，系统比较不同政党演讲的翻译质量。

Result: 发现多数政党（左翼、中间、右翼）的演讲翻译质量明显优于边缘政党，表明LLM存在系统性政治偏见。

Conclusion: 基于多语言翻译公平性的框架能有效揭示LLM的政治偏见，为偏见评估提供了新视角和方法。
...
TL;DR: ARC-Encoder是一种上下文压缩方法，通过将文本压缩为更少的连续表示来替换解码器LLM中的token嵌入，提高推理效率且无需微调目标模型。
...
Method: 设计ARC-Encoder编码器，将上下文压缩为比文本token少x倍（通常x∈{4,8}）的连续表示，系统研究训练策略和架构选择。

Result: ARC-Encoder在多个LLM使用场景中实现最先进性能，包括上下文学习和上下文窗口扩展，同时提高推理计算效率。

Conclusion: ARC-Encoder是灵活高效的便携编码器解决方案，可适配多个解码器LLM，支持跨模型通用化。
...
TL;DR: CenterBench是一个包含9,720个中心嵌套句理解问题的数据集，用于区分语言模型的结构理解与语义模式匹配能力，通过对比句法和语义相同的句子来量化模型何时放弃结构分析转向语义关联。
...
Method: 创建CenterBench数据集，包含中心嵌套句及其语义不可行对应句，通过六种理解问题测试表面理解、句法依赖和因果推理能力。

Result: 测试六个模型发现，随着复杂度增加，可行与不可行句子间的性能差距系统性扩大（中位数差距达26.8个百分点），表明模型在复杂情况下会放弃结构分析转向语义关联。

Conclusion: CenterBench提供了首个识别模型何时从结构分析转向模式匹配的框架，揭示了语义可行性在因果推理问题中反而损害性能，而推理模型虽提高准确性但仍存在语义捷径等问题。
...
<details>
  <summary>Details</summary>
Motivation: 现有强化学习方法在多跳问答中存在两个基本限制：(i)缺乏全局规划来构建多步推理结构，(ii)不可靠的执行阻碍了有效查询制定和检索证据的一致使用。
...
TL;DR: 提出了多智能体认知决策框架(MACDF)，将电商搜索从被动检索转变为主动决策支持，解决了传统检索-排序范式与用户多阶段认知决策过程不匹配的问题。


<details>
  <summary>Details</summary>
Motivation: 传统电商搜索的检索-排序范式依赖查询-商品匹配，与平台用户的多阶段认知决策过程存在根本性不匹配，导致复杂查询的语义鸿沟、跨平台信息搜索的高决策成本以及缺乏专业购物指导等关键限制。
...
TL;DR: 研究评估ChatGPT在通信数据自动编码中是否存在性别和种族偏见，发现无明显偏见，支持其在大规模协作评估中的应用。


<details>
  <summary>Details</summary>
Motivation: 现有研究表明ChatGPT能按编码框架自动编码通信数据，但尚不清楚其是否存在对性别和种族等人口群体的偏见。

Method: 使用典型的协作问题解决编码框架，分析三种协作任务（谈判、问题解决、决策制定）数据，考察ChatGPT编码在不同性别和种族群体间的差异。

Result: ChatGPT自动编码在性别和种族群体间未表现出显著偏见。

Conclusion: ChatGPT编码无显著人口群体偏见，为其在大规模协作和通信评估中的采用铺平了道路。
...
<details>
  <summary>Details</summary>
Motivation: 参与阿拉伯语AI生成文本检测共享任务，比较不同预训练模型在该任务上的表现

Method: 使用AraELECTRA、CAMeLBERT和XLM-RoBERTa三个预训练transformer模型，在提供的数据集上进行微调以执行二元分类任务

Result: XLM-RoBERTa模型表现最佳，F1分数达0.7701，超越了专门的阿拉伯语模型

Conclusion: 多语言模型在AI生成文本检测任务中展现出强大的泛化能力，检测AI生成文本具有复杂性
...
TL;DR: 该研究基于人类好奇心评估问卷5DCR，构建了评估大语言模型好奇心的框架，发现LLMs比人类有更强的求知欲但在不确定环境中更保守，好奇心能增强模型的推理和主动学习能力。
...
TL;DR: LRMs在非英语问题上倾向于使用英语推理，导致性能下降和翻译错误，特别是在复杂任务中。


<details>
  <summary>Details</summary>
Motivation: 探索LRMs的多语言推理能力，关注其在非英语问题中默认使用英语推理的问题及其对准确性和文化细微差别处理的影响。

Method: 系统比较LRM在英语与问题语言中的推理，评估MGSM和GPQA Diamond任务，分析推理轨迹中的认知属性。

Result: 英语推理轨迹显示更高的认知行为存在，英语推理通常带来更高的答案准确性，但随着任务复杂性增加，性能差距扩大。英语中心策略易受翻译错误影响。

Conclusion: LRMs在非英语问题中过度依赖英语推理，导致翻译错误和性能下降，需改进多语言推理能力。
...
Method: 创建CantoNLU基准测试，包含7个任务；比较了未训练粤语的普通话模型、通过持续预训练适应粤语的模型，以及从头训练的粤语单语模型。
...
Result: ND-LoRA将幻觉率降低高达25.6%（平均14.6%），且不降低总体准确率；神经相关性增加0.1%与幻觉增加3.8%相关
...
Method: 提出了三种轻量级适配的效用函数，增强MBR对结果空间结构变化的敏感性，并构建了包含对话行为、情感和响应结构的数据集进行测试。

Result: 分析表明常见相似性效用函数在结构最优性指标上表现不佳，而提出的适配方法显著改善了结构最优性，在真实指令跟随基准测试中生成质量提升达13.7个百分点。

Conclusion: 增强MBR对结构变化的敏感性可以显著提高开放任务中的生成质量，提出的轻量级适配方法有效解决了MBR在结构多样性任务中的局限性。
...
TL;DR: 研究发现大语言模型在隐私保护评估中，代理LLM与真实用户感知存在显著差异，用户对隐私保护质量和帮助性的评价个体差异大，而代理LLM之间一致性高但与用户相关性低。


<details>
  <summary>Details</summary>
Motivation: 现有LLM隐私保护评估主要依赖代理LLM判断，忽视了真实用户的感知差异，且缺乏对帮助性细微差异的研究。

Method: 对94名参与者进行用户研究，使用PrivacyLens的90个隐私敏感场景，比较用户与5个代理LLM对LLM响应的隐私保护质量和帮助性评价。

Result: 用户对相同LLM响应的隐私保护质量和帮助性评价一致性低，而代理LLM之间一致性高但与用户评价相关性低。

Conclusion: LLM响应的隐私和帮助性具有个体特异性，代理LLM无法准确估计用户感知，需要开展以用户为中心的研究来改进评估方法。
...
<details>
  <summary>Details</summary>
Motivation: 氟嘧啶广泛用于结直肠癌和乳腺癌治疗，但会引起手足综合征和心脏毒性等不良反应，这些毒性信息通常嵌入在临床笔记中，需要有效提取方法。
...
TL;DR: 该论文首次系统分析了大型推理模型作为机器翻译质量评估器的潜力，发现LRMs需要定制化评估材料、对简单实例过度思考以及评分机制导致高估等问题，提出通过训练合成的人类思维轨迹来校准LRM思维，显著降低思考预算并提升评估性能。


<details>
  <summary>Details</summary>
Motivation: 大型推理模型在复杂下游任务中通过中间"思考"过程提升了推理能力，但其在机器翻译质量评估中的潜力尚未充分探索，需要系统分析LRM作为评估器的可行性。

Method: 提出校准LRM思维的方法，通过训练模型在合成的人类思维轨迹上，解决过度思考和评分机制问题。在WMT24 Metrics基准上进行实验验证。

Result: 该方法大幅减少了约35倍的思考预算，同时在不同规模（7B到32B）的LRM上提升了评估性能，例如R1-Distill-Qwen-7B模型相关性提高了8.7个百分点。

Conclusion: 高效校准的大型推理模型有潜力推动细粒度自动机器翻译评估的发展，为MT质量评估提供了新的有效方法。
...
Method: 构建了一个由真实应用驱动的数据集（根据产品特征生成纯文本产品描述），通过公平性属性与性别化形容词和产品类别的交叉参数化，生成带标签的提示集合。

Result: 展示了如何使用该数据识别LLM在质量、真实性、安全性和公平性方面的差距。

Conclusion: 为研究社区贡献了LLM评估方案和具体资源，强调了针对应用场景的公平性评估的重要性。
...
Method: 结合线性注意力和稀疏注意力，提出可学习的token淘汰方法，使用轻量级CNN聚合相邻token信息，自适应保留关键KV对，同时提供高效的Triton内核实现。

Result: 在检索密集型基准测试中验证了方法的有效性。

Conclusion: 提出的混合模型在线性注意力的效率基础上，通过稀疏注意力机制有效缓解了遗忘问题，适用于检索密集型任务。
...
TL;DR: 提出一种简单的均值池化方法用于软上下文压缩，在RAG中优于广泛使用的压缩标记架构，并研究训练同一压缩器输出多种压缩比率。


<details>
  <summary>Details</summary>
Motivation: 减少在检索增强生成中使用长上下文的计算成本，通过将输入序列转换为更短的连续表示来实现软上下文压缩。

Method: 开发轻量级的均值池化方法，训练同一压缩器输出多种压缩比率，并在多个QA数据集、模型家族、规模和压缩比率上进行广泛实验。

Result: 简单的均值池化方法表现最强，训练多压缩比率时性能下降相对较小，但不同架构和训练机制间的权衡更为复杂。

Conclusion: 均值池化是有效的软上下文压缩方法，但压缩方法的权衡关系复杂，需要更细致的研究。
...
TL;DR: 论文指出当前LLM生成文本检测面临定义模糊、检测目标不明确的问题，现有检测器在现实场景中的评估不足，检测结果应谨慎解读。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型的广泛应用，检测其生成文本的需求日益增长，但缺乏对'LLM生成文本'的统一定义，且现实使用场景复杂多样，导致检测困难。

Method: 分析现有检测方法的局限性，探讨人类编辑对LLM输出的影响以及LLM对用户的微妙影响，这些因素模糊了LLM生成与人类撰写文本的界限。

Result: 现有检测基准和评估方法未能充分覆盖真实世界检测器应用的各种条件，导致检测器的数值结果常被误解，其重要性正在减弱。

Conclusion: 检测器在特定条件下仍然有用，但其结果应仅作为参考而非决定性指标，需要更谨慎地解读检测结果。
...
TL;DR: 提出一种量子启发的算法，使用矩阵乘积态和离散驱动调度解决QUBO问题，在数独和MaxCut问题上验证了有效性。


<details>
  <summary>Details</summary>
Motivation: 解决二次无约束二进制优化问题，该问题在数学上等价于寻找伊辛自旋玻璃哈密顿量的基态。

Method: 使用矩阵乘积态紧凑表示自旋构型的大叠加，结合离散驱动调度引导MPS向基态演化，采用DMRG方法迭代最小化系统能量。

Result: 算法可靠地识别全局最小值而非近似解，成功解决超过200个自旋的数独问题和251节点、3265边的MaxCut问题。
...
TL;DR: 该研究提出了分析可靠性基准(ARB)，用于量化大语言模型在能源系统分析中的推理可靠性，包含五个子指标，并在四种前沿模型上验证了推理可靠性的客观可测量性。


<details>
  <summary>Details</summary>
Motivation: 当前能源领域AI系统验证主要关注预测准确性或计算效率，缺乏对分析结论逻辑完整性的标准化评估框架。

Method: 开发ARB基准框架，整合准确性、推理可靠性、不确定性纪律、政策一致性和透明度五个子指标，使用公开技术经济数据集在确定性和概率性场景下评估四种前沿模型。

Result: GPT-4/5和Claude 4.5 Sonnet达到一致且符合政策的推理(分析可靠性指数>90)，Gemini 2.5 Pro表现中等稳定性，Llama 3 70B低于专业阈值，统计验证显示差异显著且可重现。

Conclusion: ARB建立了能源文献中首个定量验证AI系统因果、概率和政策驱动推理的方法，为全球能源转型中可信透明分析应用提供了参考框架。
...
Method: 采用树状结构探索进行可控多分支推理，通过网页状态重放引导探索，并利用页面动作记忆在会话内外共享已探索的动作。
...
TL;DR: 提出基于有向无环图(DAG)的框架来评估LLMs在数学推理中的逻辑一致性，引入逻辑贴近度指标，超越传统PASS@k指标，揭示不同LLM家族在推理保真度上的显著差异。


<details>
  <summary>Details</summary>
Motivation: 现有Chain-of-Thought(CoT)方法无法确定LLMs的成功是源于搜索、死记硬背还是规则一致的推理，需要新的评估框架来量化推理过程的逻辑一致性。

Method: 将CoT建模为基于规则的随机过程，在DAG结构上定义逻辑贴近度指标，提出DAG-MATH CoT格式并构建基准测试，指导LLMs生成符合该格式的推理轨迹。

Result: 在标准数学推理数据集上，发现即使PASS@k指标相似，代表性LLM家族在推理保真度上存在统计显著差异，揭示了最终答案准确性与规则一致性推导之间的差距。

Conclusion: 该框架在自由形式CoT和形式证明系统之间提供了平衡，为LLMs推理评估提供了可操作的诊断工具，有助于更全面地评估模型的推理能力。
...
TL;DR: Surfer 2是一个统一的视觉交互架构，在网页、桌面和移动环境中实现了最先进的性能，无需特定任务微调即可跨平台部署。


<details>
  <summary>Details</summary>
Motivation: 现有智能体系统依赖环境特定接口，限制了跨平台部署能力，需要构建能够从纯视觉观察中跨环境泛化的通用智能体。

Method: 集成层次化上下文管理、解耦规划与执行、以及具有自适应恢复能力的自我验证机制，支持长任务序列的可靠操作。

Result: 在WebVoyager上达到97.1%准确率，WebArena 69.6%，OSWorld 60.1%，AndroidWorld 87.1%，超越所有现有系统，多次尝试后在所有基准测试中超过人类表现。
...
TL;DR: RELATE是一个模式无关的图神经网络特征编码器，使用共享的模态特定编码器处理多模态节点属性，通过交叉注意力模块生成固定大小的节点表示，在保持性能的同时大幅减少参数数量。


<details>
  <summary>Details</summary>
Motivation: 现有GNN需要为每种节点类型和特征列设计单独的特征编码器，这阻碍了可扩展性和参数共享，特别是在处理异构时序图数据时。

Method: 使用共享的模态特定编码器处理分类、数值、文本和时间属性，然后通过Perceiver风格的交叉注意力模块将特征聚合成固定大小的排列不变节点表示。

Result: 在RelBench基准测试中，RELATE在ReLGNN和HGT上的性能达到模式特定编码器的97%以上，同时参数数量减少高达5倍。

Conclusion: RELATE支持不同模式并支持多数据集预训练，为关系图数据的基础模型铺平了道路。
...
TL;DR: 生成式AI加剧了保险欺诈，使伪造事故证据变得更容易。保险公司开始部署AI检测工具，但面临误报漏报等挑战。UVeye提出分层解决方案来应对这一新欺诈浪潮。


<details>
  <summary>Details</summary>
Motivation: 保险欺诈每年造成数百亿美元损失，传统欺诈手段包括制造事故、夸大损失等。生成式AI的出现让欺诈者能大规模伪造逼真的事故照片和文件，加剧了保险欺诈问题。

Method: 保险公司部署AI深度伪造检测软件和增强验证流程来检测AI驱动的欺诈。UVeye提出分层解决方案，结合多种技术手段来检测、缓解和威慑欺诈行为。

Result: 当前检测工具存在误报和漏报问题，欺诈者不断调整策略规避检测。AI欺诈检测与欺诈手段之间形成了持续的攻防竞赛。

Conclusion: 打击AI驱动的保险欺诈仍然是一个持续挑战，需要更先进的分层解决方案来有效应对这一新威胁。
...
TL;DR: 该研究使用机器学习模型通过领导力人格特质预测学术成功，随机森林分类器在包含17个人格特征和领导力标记的模型中达到87.50%的准确率。
...
Method: 从129名环境工程硕士生收集5项领导力人格测试数据（23个特征），结合平均成绩，使用皮尔逊相关系数进行特征选择，调优7种机器学习算法（SVM、LR、KNN、DT、GB、RF、XGBoost、LightGBM）。

Result: 随机森林分类器表现最佳，包含17个人格特征和领导力标记的模型准确率达87.50%，不包含该特征的模型准确率为85.71%。

Conclusion: 该研究为在早期教育阶段识别学生优缺点和选择最适合的个性化学习策略提供了额外机会。
...
### [158] [FLORA: Unsupervised Knowledge Graph Alignment by Fuzzy Logic](https://arxiv.org/abs/2510.20467)
...
Result: 在主要基准测试中达到了最先进的结果。

Conclusion: FLORA是一种简单有效的无监督知识图谱对齐方法，具有可解释性和强大性能。
...
TL;DR: AI PB是一个在生产环境中部署的生成式智能体，用于零售金融领域。它能够主动生成基于事实、合规且个性化的投资见解，而非被动回答查询。系统采用组件化编排、混合检索管道和多阶段推荐机制，完全在韩国金融监管框架下本地化运行。


<details>
  <summary>Details</summary>
Motivation: 传统聊天机器人只能被动回答查询，而金融领域需要主动、合规且个性化的投资建议。现有的生成式AI系统在数据敏感性、监管合规和信任度方面存在不足，特别是在高风险金融环境中。

Method: 1) 基于组件的编排层，根据数据敏感性确定性地路由内部和外部LLM；2) 使用OpenSearch和金融领域嵌入模型的混合检索管道；3) 结合规则启发式、序列行为建模和上下文多臂老虎机的多阶段推荐机制；4) 在24个NVIDIA H100 GPU上使用Docker Swarm和vLLM进行全本地化部署。

Result: 通过人工QA和系统指标验证，表明具有显式路由和分层安全机制的基于事实的生成能够在高风险金融环境中提供可信的AI见解。

Conclusion: 在金融等高风险领域，通过明确的系统路由、分层安全机制和基于事实的生成，可以构建可信赖的AI系统。组件化架构和本地化部署是满足金融监管要求的关键。
...
<details>
  <summary>Details</summary>
Motivation: 为了解决金融取证中非专家用户难以理解和信任传统异常检测系统的问题，需要开发一个透明、可交互且易于理解的分析工具。

Method: 系统采用三角色架构（解析、检测、解释），将用户自然语言查询转换为XGBoost检测器的输入模式，并提供基于底层特征的叙述性解释。

Result: 在比特币混币数据集上，基线检测器达到高准确率，HCLA系统在此基础上增加了可解释性和交互式优化能力。

Conclusion: 人在回路的设计提高了金融取证系统的透明度和可信度，使非专家用户能够更好地理解和信任分析结果。
...
TL;DR: 论文质疑AI在法律实践中能大幅降低成本的假设，提出了验证-价值悖论：AI使用带来的效率提升会被相应的验证需求抵消，导致净价值往往微不足道。


<details>
  <summary>Details</summary>
Motivation: 鉴于律师因提交AI生成的不准确内容而受到处罚的案例，需要重新评估AI在法律实践中的使用范式，考虑AI与现实脱节、缺乏透明度与律师诚信义务之间的冲突。

Method: 提出验证-价值悖论作为替代模型，分析AI使用效率与验证需求之间的平衡关系。

Result: AI在法律实践中的使用不会显著降低成本，因为效率提升会被验证需求抵消，净价值往往可以忽略不计。

Conclusion: 需要新的AI使用范式，强调对真相的忠诚和公民责任等价值观，这对法律实践和法律教育都有重要影响。
...
TL;DR: 提出TRUST框架，一种去中心化的大语言模型推理审计系统，通过共识机制、层次化DAG分解、区块链账本和隐私保护分段技术，解决现有集中式审计方法的鲁棒性、可扩展性、透明度和隐私问题。


<details>
  <summary>Details</summary>
Motivation: 现有大语言模型推理链的审计方法存在集中化、不透明、难以扩展的问题，在关键领域部署专有模型时存在重大风险。

Method: 采用共识机制确保在30%恶意参与者情况下仍能保证正确性；使用层次化DAG分解推理链实现可扩展并行审计；通过区块链账本记录验证决策确保公共问责；采用隐私保护分段技术保护专有逻辑。
...
<details>
  <summary>Details</summary>
Motivation: 研究AGI发展过程中的身份整合阶段，认为这是实现AGI可靠性的前提，也是安全控制的关键点。

Method: 形式化身份整合阶段，将其与学习动态中的已知现象联系起来，提出操作性指标进行检测，并通过实验验证。

Result: 实验显示行为整合是快速非线性的，但对通用能力的影响并非单一：小型模型出现性能权衡，中等规模模型基本无成本，大型量化模型出现瞬态不稳定性。

Conclusion: 身份整合是AGI可靠性的先决条件，也是安全控制的关键点，可以有意设计以提高可靠性，但也可能在扩展过程中自发出现，导致不可预测的目标和行为固化。
...
TL;DR: 本研究探索了在动态环境中优化AI的进化训练方法，使用2048游戏作为测试平台。比较了单代理系统（基于价值函数和蒙特卡洛树搜索）和双代理元提示系统（思考者LLM和执行者LLM），发现单代理系统表现显著更好。
...
Method: 实现两种系统：1) 双代理元提示系统，其中"思考者"LLM代理为"执行者"LLM代理优化游戏策略；2) 单代理系统，基于精炼价值函数和有限蒙特卡洛树搜索，并实验了回滚功能以避免性能退化。

Result: 单代理系统取得了显著改进，每个周期平均增加473.2分，训练周期间呈现明显上升趋势（相关性ρ=0.607）。LLM对游戏的理解也得到提升，开发出越来越先进的策略。相反，双代理系统改进有限。

Conclusion: 进化精炼技术在非确定性环境中改善AI性能具有潜力。单代理系统表现优异，而元提示方法存在固有局限性。
...
TL;DR: 该论文提出个性化认知模拟任务，评估不同认知表征方法在模拟作者风格方面的效果，发现结合概念和语言特征的方法最有效，但LLMs在模拟深层认知结构方面仍有局限。


<details>
  <summary>Details</summary>
Motivation: 尽管大语言模型在表面行为模仿方面表现良好，但其模拟更深层个性化认知过程的能力尚不清楚，需要系统评估不同认知表征方法在个性化认知模拟中的效果。

Method: 构建基于新出版小说的数据集，提出11条件认知评估框架，测试7个现成LLMs在作者风格模拟中的表现，比较不同认知表征方法（语言特征、概念映射、基于档案的信息）。

Result: 结合概念和语言特征的方法在个性化认知模拟中特别有效，优于基于静态档案的线索；LLMs更擅长模仿语言风格而非叙事结构。
...
TL;DR: 使用大型语言模型通过上下文学习生成抽象PDDL领域和问题实例，以自然语言指定的抽象目标为基础，在简单设置下GPT-4o能够有效合成规划领域抽象。


<details>
  <summary>Details</summary>
Motivation: 动态领域的抽象生成对智能体的规划、推理和解释能力至关重要，但目前选择合适的抽象仍是一个挑战。

Method: 在PDDL中建模智能体具体行为，利用大型语言模型的上下文学习能力，根据自然语言指定的抽象目标生成抽象PDDL领域和问题实例。

Result: GPT-4o在简单设置下能够有效合成有用的规划领域抽象，但在动作抽象方面优于相关谓词的抽象。
...
Method: 提出STaBERT模型，在BERT-based移动性模型基础上，加入时间描述符和POI嵌入，构建统一的语义丰富移动性表示。

Result: 实验结果显示STaBERT显著提升预测精度：单城市预测的GEO-BLEU分数从0.34提升至0.75；多城市预测从0.34提升至0.56。
...
Method: 设计了一个自动生成EQA任务的数据生成管道，构建了包含约18K任务的EQA-RT数据集，训练模型使用外部工具进行多步推理。

Result: 在EQA-RT-Seen和EQA-RT-Unseen测试集上，ToolEQA相比最先进基线方法成功率提高了9.2~20.2%，比零样本版本高出10%。在多个数据集上都达到了最先进性能。

Conclusion: ToolEQA通过集成外部工具和多步推理，显著提升了EQA任务的性能，证明了其在具身问答中的有效性和泛化能力。
...
TL;DR: 该论文分析了AI在医疗健康领域应用中存在的数据偏见问题，基于AI4HealthyAging项目的实践经验，识别了临床数据收集中存在的多种偏见类型，并提出了改进建议。


<details>
  <summary>Details</summary>
Motivation: AI在医疗健康领域具有巨大潜力，但由于训练数据的质量和公平性问题，AI解决方案在真实临床实践中的整合仍然有限。数据收集过程中的偏见是主要障碍。

Method: 基于西班牙国家研发计划中的AI4HealthyAging项目，作者通过检测临床数据收集过程中的偏见，识别了多种偏见类型。

Result: 识别出多种偏见类型，包括历史偏见、代表性偏见和测量偏见，这些偏见体现在性别、年龄、居住地、社会经济地位、设备和标签等变量中。

Conclusion: 提出了改进临床问题设计和数据收集公平性和鲁棒性的实用建议，希望这些发现和经验能够指导未来开发更公平的医疗AI系统。
...
TL;DR: 提出了一种用于军事行动中AI系统目标交战的附带损害评估模型，该模型在知识表示与推理架构中整合了时间、空间和力量维度，通过分层结构捕获AI系统类别、交战向量和上下文方面。


<details>
  <summary>Details</summary>
Motivation: 在AI系统在战场中扮演越来越重要角色的时代，确保负责任的目标交战需要对潜在附带效应进行严格评估。

Method: 采用设计科学方法论，构建统一的知识表示与推理架构，整合时间、空间和力量维度，通过分层结构捕获AI系统类别、交战向量和上下文方面，并考虑传播、严重性、可能性和评估指标。
...
TL;DR: 本文综述了大型语言模型如何变革知识图谱构建，从传统规则方法转向语言驱动的生成框架，系统分析了LLM在知识图谱构建各阶段的应用。


<details>
  <summary>Details</summary>
Motivation: 随着大型语言模型的出现，知识图谱构建需要从传统的基于规则和统计的方法转向语言驱动和生成框架，以更好地融合符号知识工程和神经语义理解。

Method: 系统回顾传统知识图谱方法，并从模式驱动和模式自由两个互补视角分析新兴的LLM驱动方法，综合代表性框架并分析技术机制。

Result: LLM重塑了知识图谱构建的经典三层流程（本体工程、知识抽取、知识融合），推动了更灵活、自适应和可解释的知识系统发展。

Conclusion: LLM与知识图谱的协同发展将促进自适应、可解释和智能知识系统的开发，未来方向包括基于KG的LLM推理、动态知识记忆和多模态KG构建。
...
TL;DR: 提出了IKnow框架，通过指令-响应对话格式的自监督目标，在无需外部资源的情况下实现大型语言模型的持续预训练，避免指令跟随能力退化。
...
Conclusion: IKnow提供了一个简单通用的持续适应框架，不依赖外部资源即可有效编码领域知识。
...
TL;DR: 本文提出了一种基于创造力理论的计算模型，用于生成更具新颖性的创新机会，在酒店业项目中验证了其优于Notebook LM和ChatGPT4o的表现。


<details>
  <summary>Details</summary>
Motivation: 开发能够生成高新颖性且不失实用性的创新机会的计算模型，以提升创新项目的创造力产出。

Method: 实现了一个包含五个功能模块的计算模型，这些功能基于创造力理论和技巧设计，并在酒店业创新项目中进行机会生成测试。

Result: 该模型生成的创新机会在新颖性和实用性方面均优于Notebook LM和ChatGPT4o，但并非所有功能模块都对提升新颖性有贡献。

Conclusion: 计算模型在生成创新机会方面表现出色，但部分功能需要进一步优化，为后续模型开发指明了方向。
...
TL;DR: 提出了一个名为EBR的神经推理器，使用嵌入来近似符号推理器的结果，解决了传统描述逻辑推理器对不一致和错误数据不鲁棒的问题。
...
Conclusion: EBR是一个有效的神经推理器，能够处理真实世界知识库中的不一致和错误数据问题。
...
TL;DR: 该研究比较了澳大利亚、哥伦比亚和美国三个国家的AI治理公众参与实践，发现政府未能建立有效的公众对话机制，参与率极低（<1%），且政府对公众反馈响应有限，导致参与式AI治理的承诺与实践之间存在巨大差距。
...
Method: 采用景观分析方法，比较三个国家（澳大利亚、哥伦比亚、美国）政府征集AI风险和政策的公众反馈的方式，以及这些反馈是否影响了治理决策。

Result: 研究发现三个国家均未能建立有意义的公众对话，政府缺乏吸引多样化声音的努力，公众参与率极低，官员对反馈的响应有限，未能形成有效的反馈循环。

Conclusion: 当前的参与式AI治理方法难以建立信任和合法性，因为决策者未能充分倾听和回应公众关切。作者提出了八项改进建议，包括提升AI素养、扩大参与范围、采用创新参与方法等。
...
TL;DR: 本文提出了一种基于图神经网络加速的网络拓扑优化方法，用于解决电网拥堵管理问题，实现了比传统求解器快4个数量级的速度提升，并在大规模系统上获得接近最优的解。


<details>
  <summary>Details</summary>
Motivation: 传统混合整数非线性规划方法无法在大规模系统中实现近实时网络拓扑优化，而现有机器学习方法在泛化到未见拓扑、变化运行条件和不同系统方面存在局限，限制了实际应用。

Method: 开发了异构边感知消息传递神经网络来预测有效的母线分裂动作作为候选拓扑优化解，该方法基于线性化交流潮流模型，能够捕捉局部流量模式。

Result: 在GOC 2000母线系统上实现了4个数量级的加速，在1分钟内提供交流可行的解，与最优解的差距仅为2.3%。
...
TL;DR: 提出因果逐步评估方法(CaSE)来更精细地评估LLM的推理质量，将推理质量分解为相关性和连贯性两个维度，并通过基于CaSE评估的训练数据改进最终任务性能。


<details>
  <summary>Details</summary>
Motivation: 当前仅评估最终答案正确性的方法过于粗糙，无法反映底层推理过程的质量，需要更细粒度的推理评估来构建更稳健的模型。

Method: 引入CaSE方法，将推理质量分解为相关性和连贯性两个维度，通过仅使用前文语境评估每个推理步骤来避免后见之明偏差，并在MRa-GSM8K和MRa-MATH基准上验证。

Result: CaSE方法与人类判断一致，使用CaSE评估的相关性和连贯性数据能够直接改进最终任务性能。
...
TL;DR: 提出了两种可扩展的随机游走中心性算法，一种基于近似Cholesky分解和稀疏逆估计，另一种基于采样根生成树，均能在近线性时间内提供强近似保证。
...
Method: 提出了两种算法：1) 基于近似Cholesky分解和稀疏逆估计；2) 基于采样根生成树。两种算法都实现了近线性时间复杂度。

Result: 在包括超过1000万个节点的大规模真实网络上的广泛实验证明了所提算法的高效性和近似质量。

Conclusion: 所提出的算法能够高效计算大规模网络的随机游走中心性，为图挖掘应用提供了实用的解决方案。
...
TL;DR: MIMOSA框架是一种可解释性设计方法，通过平衡可解释性与性能，同时嵌入因果性、公平性和隐私性等关键伦理属性，构建可信的预测模型。


<details>
  <summary>Details</summary>
Motivation: 需要开发既准确又可信的AI系统，在真实应用中建立信任、问责和安全采用，要求模型不仅可解释还要具备伦理属性。

Method: 定义监督学习环境，分析三类可解释模型（特征重要性、规则、实例），为因果性、公平性和隐私性提供形式化定义、评估指标和验证程序。

Result: 建立了开发可信AI系统的理论基础，能够评估伦理度量并在模型生成过程中嵌入隐私要求、公平约束和因果推理。

Conclusion: MIMOSA框架为构建准确、可解释、公平、隐私保护和因果感知的可信AI系统奠定了理论基础。
...
Method: 采用半自动流程：大模型生成候选答案，50多名电子商务和多语言专家审核修改；基于不同规模和能力模型的平均评分定义问题难度等级。

Result: 构建了一个反映真实业务交互嘈杂和异构特性的基准测试，主要来源于真实客户查询和交易日志。

Conclusion: EcomEval填补了电子商务领域评估工具的空白，提供了挑战导向和细粒度评估，特别关注了东南亚低资源语言。
...
<details>
  <summary>Details</summary>
Motivation: 需要量化模型在动态扩展环境中的适应能力，区分封闭式和开放式基准测试，优先关注现实世界的闭环开放式基准。

Method: 基于初始、当前和未来环境状态的偏差评估响应准确性，测量模型理解、预测和适应状态变化的能力。

Result: 建立了量化模型适应性的评估框架，强调真正超级智能模型应具备至少二阶适应性。

Conclusion: 超级智能模型需要具备自持计算能力，通过数字补充实现最优流动性，这需要至少二阶适应性。
...
<details>
  <summary>Details</summary>
Motivation: 由于机器学习在感知和认知任务中展现出类人能力，将其集成到理性智能体架构中的框架日益受到关注，但现有研究碎片化且不连贯，往往忽视了理性架构的表达能力。

Method: 使用BDI范式作为参考框架，对现有方法进行细粒度系统化分析。

Result: 分析展示了机器学习增强理性智能体的快速发展文献，识别了关键研究机会。

Conclusion: 为设计有效的理性机器学习智能体提供了重要的研究方向和开放挑战。
...
<details>
  <summary>Details</summary>
Motivation: 当前评估大语言模型推理轨迹质量的方法依赖专家标注、人工评分，效率低且不可靠；现有自动化方法主要基于图结构连通性，无法有效捕捉推理质量。
...
TL;DR: Graph-RFT是一个两阶段强化微调的KGQA框架，通过'计划-KG搜索-网络搜索-思考'范式，让LLM在不完整知识条件下自主规划和自适应检索调度，解决现有方法在复杂场景中的推理失败问题。


<details>
  <summary>Details</summary>
Motivation: 现有KGQA方法难以充分利用知识图谱的丰富知识和LLM的推理能力，假设KG完全覆盖且缺乏判断何时需要外部信息的机制，推理过程局部短视，无法保持连贯的多步规划。

Method: 提出两阶段强化微调框架：1) 使用链式思维微调方法和定制化计划-检索数据集激活结构化推理；2) 引入计划-检索引导的强化学习过程，结合显式规划和检索动作，采用笛卡尔式规划模块分解复杂问题，逻辑表达式指导工具调用。

Result: 该方法通过多奖励设计优化推理检索过程，结合结果和检索特定信号，使模型学习何时以及如何有效结合KG和网络检索。

Conclusion: Graph-RFT能够在不完整知识条件下实现自主规划和自适应检索调度，解决现有KGQA方法的局限性，提升复杂场景下的推理能力。
...
TL;DR: 提出了一种基于广义均值积分的一致性感知AGI度量方法，通过补偿性指数连续统上的曲线下面积来量化AGI，比算术均值更严格地评估通用智能。


<details>
  <summary>Details</summary>
Motivation: 现有AGI定义使用算术均值假设补偿性，即某些领域的卓越能力可以抵消其他领域的失败，但真正的通用智能应该反映所有基本领域的平衡能力。

Method: 基于广义均值在补偿性指数连续统上的积分，涵盖算术、几何和调和均值机制，通过曲线下面积量化不同补偿性假设下的鲁棒性。

Result: 应用于GPT-4和GPT-5的CHC领域得分，一致性调整的AUC显示尽管算术得分较高（如GPT-5为24%），但两个系统距离通用能力仍然很远。

Conclusion: 集成广义均值提供了一个原则性、可解释且更严格的AGI测量基础，能够惩罚不平衡并捕捉领域间依赖性。
...
TL;DR: 提出Real Deep Research (RDR)框架，用于系统分析快速发展的AI和机器人研究领域，识别新兴趋势和跨领域机会，帮助研究人员应对每年上万篇论文的挑战。


<details>
  <summary>Details</summary>
Motivation: AI和机器人领域研究快速发展，每年产出超过10,000篇论文，研究人员难以跟踪最新进展。跨学科工作的兴起和需要探索专业领域外的知识进一步加剧了这一挑战。

Method: 开发了一个通用的RDR管道，能够系统分析任何研究领域，识别新兴趋势，发现跨领域机会，并为新研究提供具体起点。该框架特别关注基础模型和机器人技术的进展。

Result: 构建了完整的RDR管道，并在AI和机器人领域进行了应用分析，同时简要扩展到其他科学领域。附录提供了每个分析主题的广泛结果。

Conclusion: RDR框架为AI及其他领域的研究人员提供了系统分析研究趋势和发现新机会的有效工具，有助于应对信息过载和跨学科研究的挑战。
...
### [188] [Query Optimization in the Wild: Realities and Trends](https://arxiv.org/abs/2510.20082)
*Yuanyuan Tian*

Main category: cs.DB

TL;DR: 传统查询优化器架构面临云计算和大数据挑战，论文提出三个关键趋势：优化与执行的反馈循环、从单查询扩展到工作负载优化、从单体架构转向可组合架构。


<details>
  <summary>Details</summary>
Motivation: 近50年来，工业数据库系统的查询优化器核心设计保持稳定，但云计算、海量数据和统一数据平台暴露了传统单体架构的局限性。

Method: 从工业视角回顾查询优化的过去和现状，识别当前挑战，重点分析三个获得动力的关键行业趋势。

Result: 识别出三个关键趋势：优化与执行间的紧密反馈循环、优化范围从单查询扩展到工作负载、从单体设计转向可组合架构。

Conclusion: 这些趋势为查询优化实践指明了一条走向更动态、整体和适应性强的未来路径。
...
### [189] [UREM: A High-performance Unified and Resilient Enhancement Method for Multi- and High-Dimensional Indexes](https://arxiv.org/abs/2510.20110)
*Ming Sheng,Shuliang Wang,Yong Zhang,Yi Luo,Xianbo Liu,Zeming Li*

Main category: cs.DB

TL;DR: UREM是一种统一且鲁棒的多维和高维索引增强方法，能够自适应不同场景，通过布局优化和部分重组提升查询性能。


<details>
  <summary>Details</summary>
Motivation: 现有索引增强方法存在局限性：结构导向方法在静态工作负载下表现良好但缺乏通用性，布局导向方法通用性好但在静态工作负载下性能欠佳。需要开发统一且弹性的增强方法。

Method: UREM通过布局优化提升静态工作负载下的查询性能，通过部分布局重组使索引在查询变化时保持性能稳定，可统一应用于不同平台的各种索引。

Result: 在20个广泛使用的索引上评估，UREM在静态工作负载下将多维和高维索引查询性能分别提升最高5.73倍和9.18倍，在动态工作负载下平均提升5.72倍和9.47倍。

Conclusion: UREM是首个高性能的统一弹性增强方法，能够显著提升多维和高维索引在不同场景下的查询性能，使传统索引性能达到甚至超越最新先进索引的水平。
...
### [190] [RAG-Stack: Co-Optimizing RAG Quality and Performance From the Vector Database Perspective](https://arxiv.org/abs/2510.20296)
*Wenqi Jiang*

Main category: cs.DB

TL;DR: 本文提出了RAG-Stack，一个用于检索增强生成系统中质量和性能协同优化的三支柱蓝图框架。


<details>
  <summary>Details</summary>
Motivation: 当前在端到端RAG管道中，如何同时优化系统性能和生成质量是一个实际但具有挑战性的问题，这比表面看起来更复杂，因为涉及算法侧（模型和数据库）和系统侧（从软件到硬件）的众多参数。

Method: RAG-Stack包含三个核心组件：RAG-IR（中间表示层，用于解耦质量和性能方面）、RAG-CM（成本模型，用于估计给定RAG-IR的系统性能）和RAG-PE（计划探索算法，用于搜索高质量、高性能的RAG配置）。

Result: 该框架为RAG系统的质量-性能协同优化提供了一个系统化的解决方案，通过抽象层和成本模型实现了配置的自动优化。

Conclusion: 作者相信这种三支柱蓝图将成为未来几年RAG质量-性能协同优化的实际标准范式。
...
### [191] [Hybrid Mixed Integer Linear Programming for Large-Scale Join Order Optimisation](https://arxiv.org/abs/2510.20308)
*Manuel Schönberger,Immanuel Trummer,Wolfgang Mauerer*

Main category: cs.DB

TL;DR: 提出基于混合整数线性规划(MILP)的新方法来解决大规模查询的join排序问题，通过混合框架优雅扩展到100个关系的超大规模查询，实现最稳健的计划质量


<details>
  <summary>Details</summary>
Motivation: 传统查询优化器在大型查询中面临挑战：穷举搜索方法在大规模查询中失效，现有启发式方法在查询规模增大时解决方案质量下降或存在高度次优的最坏情况行为

Method: 开发了能够优化任意bushy树结构的新型MILP模型，并将其嵌入混合框架中，在MILP求解器具有最大优势的地方精确应用，对不太复杂的优化步骤使用更高效的方法

Result: 该方法能够优雅扩展到连接多达100个关系的极大规模查询，在多种竞争性join排序方法中始终实现最稳健的计划质量

Conclusion: 基于MILP的混合框架方法有效解决了大规模查询优化问题，克服了现有方法的局限性，为超大规模查询提供了高质量的join排序解决方案
...
### [192] [An Empirical Study on Database Usage in Microservices](https://arxiv.org/abs/2510.20582)
*Maxime André,Marco Raglianti,Souhaila Serbout,Anthony Cleve,Michele Lanza*

Main category: cs.DB

TL;DR: 对微服务架构中数据库使用情况的实证研究，分析了约1000个GitHub项目，涵盖15年数据，涉及180种数据库技术，发现52%的微服务组合使用多种数据库类别。
...
Method: 收集并分析约1000个GitHub项目数据，涵盖15年微服务发展，涉及180种数据库技术（14个类别），进行实证研究。

Result: 微服务主要使用关系型、键值、文档和搜索数据库；52%的微服务组合多种数据库类别；系统复杂度与数据库数量相关；老系统偏好关系型数据库，新系统更多采用键值、文档数据库；小众数据库常与主流数据库组合使用。

Conclusion: 提供了18个发现和9个建议，为研究者和从业者理解微服务中数据库使用提供了实证证据。
...
### [193] [Balanced Popularity in Multi-Product Billboard Advertisement](https://arxiv.org/abs/2510.20600)
*Dildar Ali,Suman Banerjee,Yamuna Prasad*

Main category: cs.DB

TL;DR: 提出了一个多产品影响力最大化问题，旨在在预算约束和产品间影响力平衡的条件下，为多个产品选择广告位来最大化总影响力。


<details>
  <summary>Details</summary>
Motivation: 传统的广告位选择只针对单一产品，而现实中广告商需要同时推广多个产品，并且希望各产品的影响力相对平衡，避免某些产品过度曝光而其他产品曝光不足。

Method: 将问题建模为线性规划问题，使用线性规划松弛和随机舍入方法，同时提出了一种基于贪心算法和平衡校正的启发式方法。

Result: 在真实轨迹和广告牌数据集上的实验表明，所提出的解决方案相比多种基线方法能够获得更高的影响力。

Conclusion: 该研究解决了多产品广告位选择的实际问题，提出的方法在保证各产品影响力平衡的同时有效提升了总体广告效果。
...
### [194] [Downsizing Diffusion Models for Cardinality Estimation](https://arxiv.org/abs/2510.20681)
*Xinhe Mu,Zhaoqi Zhou,Zaijiu Shang,Chuan Zhou,Gang Fu,Guiying Yan,Guoliang Li,Zhiming Ma*

Main category: cs.DB

TL;DR: ADC是基于扩散模型的第一个联合分布基数估计器，通过轻量级评分估计器计算点密度值，ADC+进一步使用决策树优化查询处理，在准确性和效率上超越现有方法。
...
Method: ADC使用轻量级评分估计器近似评分函数来计算点密度值；对于范围查询，先用高斯混合模型预测选择性，再用重要性采样蒙特卡洛校正；ADC+训练决策树识别高容量高选择性查询，跳过校正阶段以减少方差。

Result: 在真实数据集上，ADC+以约66%的存储空间媲美Naru并优于MSCN、DeepDB等；在复杂相关性的合成数据集上，ADC+表现显著优于其他模型，在95%和99%分位误差上比Naru准确10倍。

Conclusion: ADC和ADC+在基数估计任务中表现出色，特别是在处理复杂相关属性时具有显著优势，同时实现了更高的存储效率和准确性。
