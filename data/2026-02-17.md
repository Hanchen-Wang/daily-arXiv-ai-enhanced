...
TL;DR: MAILS：一种面向无人机的无地图激光雷达重定位框架，通过局部保持滑动窗口注意力、坐标无关特征初始化和局部不变位置编码，显著提升了无人机场景下的定位精度。


<details>
  <summary>Details</summary>
Motivation: 现有激光雷达重定位方法主要针对自动驾驶设计，在无人机场景下精度显著下降。无人机飞行中会遇到大幅偏航旋转和高度变化，且现有数据集未能捕捉真实无人机飞行特性。

Method: 1. 局部保持滑动窗口注意力模块提取稀疏点云的局部判别性几何特征；2. 坐标无关特征初始化模块处理偏航旋转；3. 局部不变位置编码机制处理高度变化；4. 构建大规模无人机激光雷达定位数据集。

Result: 实验表明，该方法实现了令人满意的定位精度，并显著优于现有技术。代码和数据集即将发布。

Conclusion: MAILS框架有效解决了无人机场景下激光雷达重定位的挑战，通过创新的特征提取和位置编码机制，显著提升了在复杂飞行条件下的定位性能。
...
TL;DR: XIL方法通过用户对模型解释的反馈来引导训练，能有效减少视觉分类器中的偏见和虚假相关性，特别是在性别分类任务中。CAIPI方法在引导模型关注相关特征和减少偏见方面表现最佳。


<details>
  <summary>Details</summary>
Motivation: 研究动机是探索解释性交互学习（XIL）在缓解视觉分类器偏见和虚假相关性方面的能力，特别是在容易产生数据偏见的场景（如性别分类）中，以提高模型的公平性和透明度。

Method: 研究了两种方法学上不同的XIL策略：CAIPI和Right for the Right Reasons（RRR），以及一种结合两种策略的新型混合方法。使用GradCAM和Bounded Logit Attention（BLA）生成解释，通过比较分割掩码进行定量评估。

Result: 实验结果表明：（1）这些方法能有效引导ML模型关注相关图像特征，特别是CAIPI方法；（2）能减少模型偏见（平衡男性和女性预测的错误分类率）。XIL方法在提高性别分类器公平性方面显示出潜力。

Conclusion: XIL方法通过增加透明度和公平性，虽然通常会导致轻微的性能下降，但CAIPI方法甚至有可能提高分类准确性。XIL在改善性别分类器公平性方面具有实际应用潜力。
...
TL;DR: COOPERTRIM：一种自适应特征选择框架，利用时间连续性识别环境动态特征，减少冗余传输，显著降低协同感知带宽需求


<details>
  <summary>Details</summary>
Motivation: 协同感知中有限通信带宽与丰富传感器信息之间的矛盾阻碍了实际部署。现有方法虽然选择部分特征共享，但仍对当前无线技术造成压力，需要从根本上缓解这一矛盾。

Method: 提出COOPERTRIM自适应选择框架：1）引入符合时间不确定性度量来评估特征相关性；2）采用数据驱动机制动态确定共享数量；3）利用时间连续性识别环境动态特征，避免重复传输静态信息。

Result: 在语义分割和3D检测任务中，COOPERTRIM分别实现80.28%和72.52%的带宽减少，同时保持可比精度。相比其他选择策略，IoU提升高达45.54%，带宽减少72%。结合压缩策略，带宽使用可降至1.46%而不影响IoU性能。

Conclusion: COOPERTRIM能优雅适应环境动态、定位误差和通信延迟，展现了灵活性，为实际部署铺平道路。通过利用时间连续性，从根本上缓解了带宽与信息丰富度之间的矛盾。
...
TL;DR: 本文首次系统研究了多模态大语言模型量化对可靠性的影响，发现后训练量化会降低准确性和可靠性，但数据感知量化方法和Selector置信度估计器能有效缓解此问题，int4 MBQ+Selector组合在减少75%内存需求的同时接近未压缩性能。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型存在过度自信问题（产生高度确定但错误的答案），同时模型尺寸过大限制了在边缘设备的部署。本文旨在研究量化压缩如何同时影响模型的准确性和可靠性，并探索缓解量化对可靠性负面影响的方法。

Method: 使用两种MLLM模型（Qwen2-VL-7B和Idefics3-8B），采用数据无关（HQQ）和数据感知（MBQ）两种量化方法在不同比特宽度下进行后训练量化。为应对量化导致的可靠性下降，将Selector置信度估计器适配到量化多模态设置，并在不同量化级别和分布外场景测试其鲁棒性。

Result: 后训练量化会同时降低准确性和可靠性；数据感知量化方法能缓解这种影响；Selector置信度估计器能显著减轻量化对可靠性的负面影响；int4 MBQ与Selector的组合实现了最佳效率-可靠性权衡，在减少约75%内存需求的同时接近未压缩模型的性能。

Conclusion: 本文首次系统建立了多模态设置中量化与可靠性的联系，表明通过精心选择量化方法和置信度估计器，可以在大幅压缩模型的同时保持可靠性，为在资源受限设备上部署可靠的多模态模型提供了实用方案。
...
TL;DR: NutVLM是一个用于自动驾驶视觉语言模型的自适应防御框架，通过三路分类检测威胁，使用灰度掩码净化局部攻击，通过专家引导的对抗提示调优处理全局扰动，在Dolphins基准上实现4.89%的整体指标提升。


<details>
  <summary>Details</summary>
Motivation: 当前自动驾驶中的视觉语言模型容易受到对抗性攻击（从局部物理补丁到不可感知的全局扰动），现有防御方法有限且难以平衡鲁棒性和干净样本性能，需要全面的安全解决方案。

Method: 1. 使用NutNet++作为哨兵进行三路分类（良性样本、局部补丁、全局扰动）；2. 局部威胁通过高效灰度掩码净化；3. 全局扰动触发专家引导的对抗提示调优（EAPT），通过基于梯度的潜在优化和离散投影生成"纠正驾驶提示"，无需全模型微调。

Result: 在Dolphins基准测试中，NutVLM在准确率、语言分数和GPT分数等整体指标上提升了4.89%，验证了其作为智能交通可扩展安全解决方案的有效性。

Conclusion: NutVLM提供了一个全面的自适应防御框架，能够保护自动驾驶中视觉语言模型的整个感知-决策生命周期，在保持干净样本性能的同时提高对抗鲁棒性，代码已开源。
...
<details>
  <summary>Details</summary>
Motivation: 现有多模态大语言模型物理推理评估主要依赖视觉问答和期望违背等识别式协议，这些方法通常无需明确的物理假设即可回答，难以真正评估模型的物理推理能力。

Method: 提出VisPhyWorld执行框架，要求模型从视觉观察生成可执行的模拟器代码；基于此框架构建VisPhyBench基准，包含209个评估场景（源自108个物理模板），并设计系统化协议评估模型重建外观和生成物理合理运动的能力。

Result: 该框架在基准测试中能生成97.7%的有效重建视频；实验表明，虽然最先进的MLLMs在语义场景理解方面表现良好，但在准确推断物理参数和模拟一致的物理动态方面存在困难。

Conclusion: VisPhyWorld框架通过要求生成可执行代码，使物理推理过程变得可检查、可编辑、可证伪，为评估多模态大语言模型的真实物理推理能力提供了更可靠的方法。
...
<details>
  <summary>Details</summary>
Motivation: 当前HRRP生成数据评估主要依赖分类模型（黑盒方法），缺乏可解释性和多层次评估能力，需要基于物理特性的评估指标

Method: 将HRRP数据分解为掩码、特征和噪声三个物理组件，基于这种分解提出两种物理解释性强的评估指标

Result: 在具有挑战性的任务上验证了所提指标的判别能力，证明了其有效性

Conclusion: 提出的基于物理分解的HRRP评估方法能够提供可解释的、多层次的生成数据评估，优于现有的黑盒分类模型方法
...
TL;DR: 该论文研究基于几何条件（船舶尺寸和方位角）的高分辨率距离像生成，以解决传统HRRP对采集条件敏感的问题，并在大规模海事数据集上验证了生成模型的有效性。


<details>
  <summary>Details</summary>
Motivation: 高分辨率距离像（HRRP）虽然支持快速机载处理用于雷达自动目标识别，但其对采集条件的高度敏感性限制了在不同操作场景下的鲁棒性。现有研究受限于小型、特定数据集，需要在大规模代表性数据集上研究HRRP合成。

Method: 研究基于大规模海事数据库，分析发现场景的关键驱动因素是几何参数：船舶尺寸和期望方位角。基于这些变量作为条件，训练生成模型来合成HRRP特征。

Result: 生成的HRRP特征能够复现真实数据中观察到的视线几何趋势，验证了生成模型的有效性。结果表明采集几何在鲁棒HRRP生成中的核心作用。

Conclusion: 几何条件（船舶尺寸和方位角）是HRRP合成的关键因素，基于这些条件的生成模型能够有效合成具有鲁棒性的雷达特征，为雷达自动目标识别提供了新思路。
...
TL;DR: 该研究通过对比VGG、ResNet和GoogLeNet三种CNN架构，发现网络深度的有效性取决于架构机制而非名义深度，残差和Inception架构能以更低的有效深度实现更好的精度-计算权衡。


<details>
  <summary>Details</summary>
Motivation: 虽然增加卷积深度是图像识别进步的关键，但更深的网络并不总是带来更高的精度、稳定的优化或高效的计算。需要研究深度如何影响分类性能、收敛行为和计算效率。

Method: 对三种经典卷积神经网络架构（VGG、ResNet和GoogLeNet）进行受控比较研究，标准化训练协议，明确区分名义深度和有效深度，分析深度对性能的影响。

Result: 普通深度网络表现出早期精度饱和和优化不稳定，而残差和基于Inception的架构能够以更低的有效深度将额外深度转化为精度提升，并获得有利的精度-计算权衡。

Conclusion: 有效深度而非名义深度是控制深度作为卷积网络中有效缩放维度的关键因素，架构机制对约束训练期间深度的有效表现至关重要。
...
TL;DR: KidMesh：一种基于深度神经网络的端到端方法，可直接从磁共振尿路造影自动重建小儿先天性肾积水网格，无需复杂后处理即可用于尿流动力学模拟。


<details>
  <summary>Details</summary>
Motivation: 现有基于体素的分割方法主要关注形态特征（大小、形状、结构），需要进行复杂后处理才能转换为网格表示以进行功能评估（如尿流动力学模拟）。同时，由于MRU切片稀疏采样，难以获得准确的网格级标注。

Method: KidMesh从MRU图像提取特征图，通过网格采样转换为特征顶点，然后根据这些特征顶点变形模板网格以生成特定CH网格。开发了无需精确网格级标注的训练方案。

Result: KidMesh平均0.4秒重建CH网格，性能与传统方法相当且无需后处理。重建网格无自相交，仅3.7%和0.2%的顶点误差距离超过3.2mm和6.4mm。光栅化后与手动分割掩模的Dice得分为0.86。这些网格可用于肾尿流模拟。

Conclusion: KidMesh提供了一种高效、准确的端到端CH网格重建方法，可直接用于临床尿流动力学模拟，为先天性肾积水的功能评估提供了有价值的工具。
...
TL;DR: DriveMamba：基于Mamba架构的端到端自动驾驶新范式，通过单阶段统一解码器实现动态任务关系建模、隐式视角对应学习和长时序融合，具有线性复杂度的高效处理能力。


<details>
  <summary>Details</summary>
Motivation: 现有端到端自动驾驶系统（如UniAD）采用顺序式模块化设计（感知-预测-规划），存在信息丢失、误差累积、任务关系建模不灵活、图像骨干网络训练不足以及注意力机制二次复杂度等问题，限制了系统的可扩展性和效率。

Method: 提出DriveMamba框架：1）将图像特征和任务输出转换为token级稀疏表示并按3D空间位置排序；2）使用线性复杂度的Mamba解码器进行长上下文序列建模，同时捕获任务间依赖关系；3）设计双向轨迹引导的"局部到全局"扫描方法，从自我视角保持空间局部性以辅助规划。

Result: 在nuScenes和Bench2Drive数据集上的广泛实验表明，DriveMamba在性能、泛化性和效率方面均表现出优越性，验证了其作为高效端到端自动驾驶解决方案的有效性。

Conclusion: DriveMamba通过任务中心化的可扩展范式，成功解决了传统端到端自动驾驶系统的局限性，实现了动态任务关系建模、高效长时序处理和空间局部性保持，为端到端自动驾驶提供了新的高效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 标准确定性反演（如DDIM）在源域频谱稀疏时（如超分辨率、草图转图像）会失败，导致恢复的潜在变量不符合预期各向同性高斯分布，出现低频信号，使目标采样锁定在过度平滑和纹理贫乏的生成中，这种现象称为"频谱塌缩"。
...
Conclusion: OVG成功解决了条件扩散反演中的频谱塌缩问题，通过正交方差引导在保持输入语义结构的同时恢复高质量纹理，为跨域图像翻译提供了有效解决方案。
...
TL;DR: 提出PCReg-Net，一种用于高速光学分辨率光声显微镜的渐进对比度引导配准框架，解决双向扫描中的域偏移和几何错位问题，在OR-PAM-Reg-4K数据集上达到NCC 0.983，超越现有方法。


<details>
  <summary>Details</summary>
Motivation: 高速光学分辨率光声显微镜（OR-PAM）采用双向光栅扫描可提高成像速度，但会引入前向和后向扫描线之间的域偏移和几何错位。现有方法受限于亮度恒定假设，配准质量有限（NCC≤0.96）。

Method: 提出PCReg-Net渐进对比度引导配准框架，包含四个轻量级模块：1）用于粗配准的配准U-Net；2）捕获多尺度结构线索的参考特征提取器；3）通过比较粗配准特征和参考特征识别残留错位的对比模块；4）具有特征注入功能的高保真输出细化U-Net。还提出了无参考评估帧间时间一致性的TNCC和TNCG指标。

Result: 在OR-PAM-Reg-4K数据集（432个测试样本）上，PCReg-Net达到NCC 0.983、SSIM 0.982、PSNR 46.96 dB，以实时速度超越现有方法超过14 dB。

Conclusion: PCReg-Net有效解决了高速OR-PAM双向扫描中的配准问题，实现了高质量的图像对齐，代码已开源。
...
<details>
  <summary>Details</summary>
Motivation: 野火对生态系统、人类生命和基础设施的威胁日益增长，但卫星监测面临微弱烟雾信号、动态天气条件和实时分析需求等挑战，需要更有效的早期检测和风险评估方法

Method: 构建基于Landsat-8/9、GOES-16等公开地球观测数据的标记数据集；使用YOLOv12检测火区和烟羽；集成多模态大语言模型将检测结果转化为情境化风险评估和响应建议；采用服务导向架构支持实时处理

Result: 开发了WildfireVLM系统，能够检测卫星图像中的小规模复杂模式，生成情境化风险评估和优先响应建议，并通过LLM-as-judge评估验证风险推理质量
...
TL;DR: 提出一个基于视觉语言模型Qwen2-VL-7B的自动化绘画创造力评估框架，通过多任务学习预测分数并生成解释性反馈，在100分制上达到Pearson r > 0.97的高准确度。


<details>
  <summary>Details</summary>
Motivation: 传统创造力评估（如托兰斯创造力测试）需要人工评分，工作量巨大且难以规模化。现有的机器学习方法虽然能评估视觉创造力，但大多只依赖图像特征，缺乏解释性反馈，限制了实际应用价值。

Method: 1. 使用1000幅人工创作的绘画作品数据集，每幅作品都有1-100的评分和简短文字描述；2. 两位专家使用五维度评分标准（原创性、色彩、纹理、构图、内容）进行评估并提供书面批评；3. 微调Qwen2-VL-7B视觉语言模型，采用多任务学习；4. 在视觉编码器输出上添加轻量级回归头，使模型能同时预测数值分数和生成符合评分标准的反馈；5. 在系统提示中嵌入结构化评分标准和作品描述，约束生成文本与定量预测一致。

Result: 1. 定量结果：在100分制上达到Pearson相关系数r > 0.97，平均绝对误差约3.95分；2. 定性结果：生成的反馈与专家批评在语义上高度相似（平均SBERT余弦相似度=0.798）；3. 模型能通过单次前向传播同时预测分数和生成解释性反馈。

Conclusion: 该框架成功将计算机视觉与艺术评估相结合，为创造力研究和课堂教学提供了一个可扩展的自动化评估工具，既能提供准确的数值评分，又能生成有意义的解释性反馈。
...
TL;DR: 本文提出了首个用于多模态大语言模型（MLLMs）的并行推理框架Visual Para-Thinker，通过视觉分区和并行注意力机制将并行推理范式扩展到视觉领域。


<details>
  <summary>Details</summary>
Motivation: 现有LLM测试时缩放定律强调通过延长推理长度来激发自反思行为，但这种垂直缩放策略常因模型陷入特定思维模式而遇到探索瓶颈。虽然并行思维通过从深度转向并行性缓解了探索范围变窄的问题，但如何将这一范式扩展到视觉领域仍是一个开放的研究问题。

Method: 首先研究了视觉分区在并行化推理中的作用，提出了两种不同的策略。基于此，引入了Visual Para-Thinker框架，集成了并行注意力（Pa-Attention）和LPRoPE以保持路径独立性和促进推理多样性。利用vLLM框架开发了原生多模态实现，支持高效并行处理。

Result: 在V*、CountBench、RefCOCO和HallusionBench等基准数据集上的实证结果表明，Visual Para-Thinker成功地将并行推理的优势扩展到了视觉领域。

Conclusion: 本文提出的Visual Para-Thinker是首个用于MLLMs的并行推理框架，通过视觉分区和并行处理机制有效解决了传统垂直缩放策略中的探索瓶颈问题，为视觉领域的并行推理提供了可行的解决方案。
...
TL;DR: ASTG框架利用多模态大语言模型构建空间和时间推理智能体，以训练免费、开放世界的方式解决时空视频定位任务，通过提出-评估范式实现自主时空管检索。


<details>
  <summary>Details</summary>
Motivation: 现有STVG方法存在计算冗余、监督需求大、泛化能力有限的问题，弱监督方法性能较差且受限于数据集训练范式，需要开发开放世界、无需训练的解决方案。

Method: 提出ASTG框架，构建两个专门智能体：SRA（空间推理智能体）和TRA（时空推理智能体），基于多模态大语言模型协同工作，采用提出-评估范式，解耦时空推理，自动化管提取、验证和时序定位过程，配备视觉记忆和对话上下文提升检索效率。

Result: 在流行基准测试中表现优异，显著超越现有弱监督和零样本方法，与部分全监督方法性能相当。

Conclusion: ASTG框架为STVG任务提供了一种训练免费、开放世界的有效解决方案，通过智能体协同和提出-评估范式实现了高效的时空管检索。
...
<details>
  <summary>Details</summary>
Motivation: 毫米波雷达在视觉退化室内环境（烟雾、灰尘、低光照）中提供可靠感知，但基于学习的雷达感知受限于大规模雷达数据收集和标注的稀缺性和高成本

Method: 结合单目深度估计、分割和视觉语言推理重建材料感知的3D场景，然后使用基于菲涅尔反射模型的可配置物理射线追踪器模拟毫米波传播

Result: 在真实室内场景评估中，通过迁移学习提升下游3D雷达感知：在合成数据上预训练雷达点云目标检测模型并在真实雷达数据上微调，可获得最高+3.7 3D AP（IoU 0.3）的提升，主要来自空间定位的改进
...
TL;DR: IDPruner是一种基于最大边际相关性的视觉token剪枝方法，通过平衡token重要性和语义多样性，实现高效的多模态大语言模型推理加速。


<details>
  <summary>Details</summary>
Motivation: 现有视觉token剪枝方法缺乏对token重要性和语义多样性之间权衡的原则性框架，导致无法实现最优的token选择。

Method: 提出IDPruner方法，利用最大边际相关性算法在token重要性和语义多样性之间实现帕累托最优平衡，无需注意力图，兼容FlashAttention，支持一次性剪枝部署。

Result: 在多种模型架构和多模态基准测试中达到最先进性能，在Qwen2.5-VL-7B-Instruct上，剪枝75%token时保持95.18%性能，剪枝90%时仍保持86.40%性能。

Conclusion: IDPruner通过原则性地平衡token重要性和语义多样性，实现了高效且泛化性强的视觉token剪枝，显著加速MLLM推理。
...
<details>
  <summary>Details</summary>
Motivation: 标准视觉基准测试中纹理相关性主导了性能评估，难以真正衡量模型的结构拓扑不变性能力。需要设计专门的诊断性基准来隔离拓扑不变性这一核心能力
...
Conclusion: 实验结果验证了"形式优先"假设：在结构约束架构中的泛化能力是几何完整性的属性，而非统计规模的结果
...
TL;DR: 提出分层零样本框架，结合轻量目标检测与紧凑视觉语言模型，用于军事边缘机器人自主部署，在合成战场数据上验证了高效性能。


<details>
  <summary>Details</summary>
Motivation: 军事动态环境中部署自主边缘机器人面临两大挑战：领域特定训练数据稀缺，以及边缘硬件计算能力有限。需要开发能在资源受限环境下可靠运行的零样本解决方案。

Method: 采用分层级联架构：Grounding DINO作为高召回率、文本可提示的区域建议器；高置信度检测帧传递给边缘级视觉语言模型（Qwen和Gemma系列，4B-12B参数）进行语义验证。进一步扩展为侦察-指挥官工作流，并引入"受控输入"方法分离感知与推理。

Result: 在55个高保真合成战场视频上评估：误报过滤达100%准确率，损伤评估达97.5%，细粒度车辆分类55-90%。侦察-指挥官工作流实现100%正确资产部署，推理评分9.8/10（GPT-4o评估），延迟低于75秒。发现不同模型有不同失败模式。

Conclusion: 分层零样本架构适用于边缘自主系统，为安全关键应用中视觉语言模型适用性认证提供了诊断框架。不同模型在感知与推理能力上存在互补特性，需要针对性部署。
...
TL;DR: MotionWeaver：首个多人体图像动画框架，通过统一运动表示和4D锚定范式处理多样人形、复杂交互和遮挡


<details>
  <summary>Details</summary>
Motivation: 现有字符图像动画方法主要局限于单人场景，难以泛化到多人体场景，因为涉及多样人形、复杂交互和频繁遮挡

Method: 1) 引入统一运动表示，提取身份无关运动并绑定到对应角色；2) 提出整体4D锚定范式，构建共享4D空间融合运动表示与视频潜在表示，并通过分层4D级监督增强

Result: 在构建的300视频基准测试中达到最先进结果，能有效泛化到多样人形、复杂交互和挑战性多人体场景

Conclusion: MotionWeaver是首个端到端多人体图像动画框架，通过创新表示和4D范式解决了多人体动画的关键挑战
...
TL;DR: HiST-VLA是一个用于自动驾驶的分层时空视觉-语言-动作模型，通过几何感知、动态令牌稀疏化和分层规划器来提升轨迹生成的可靠性和效率。


<details>
  <summary>Details</summary>
Motivation: 当前VLA模型在安全关键场景中存在数值推理不精确、3D空间感知弱、对上下文敏感等限制，需要更可靠的轨迹生成方法。

Method: 1) 集成几何感知与细粒度驾驶指令和状态历史提示；2) 在VLA架构中引入动态令牌稀疏化，融合冗余令牌而非过滤；3) 使用分层transformer规划器逐步细化粗粒度路径点为细粒度轨迹；4) 通过动态潜在正则化融入语言指令。

Result: 在NAVSIM v2基准测试中取得SOTA性能：Navtest上EPDMS为88.6，伪闭环Navhard基准上EPDMS为50.9。

Conclusion: HiST-VLA通过分层时空建模和高效令牌处理，显著提升了自动驾驶轨迹生成的可靠性和性能，为安全关键场景提供了有效解决方案。
...
Result: 在嵌入式平台上实现准确的鸟类物种识别是可行的，系统能够在资源受限的硬件上有效工作
...
TL;DR: MedScope是一个工具使用的临床视频推理模型，通过从粗到细的证据搜索处理长视频，结合中间推理、工具调用和验证，实现基于时间定位视觉证据的准确预测。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型处理临床视频时通常采用被动采样或弱基础检查，限制了它们迭代定位、验证和基于时间定位证据进行预测的能力，需要更精确的视频推理方法。

Method: 提出MedScope模型，采用从粗到细的证据搜索策略，交替进行中间推理、针对性工具调用和检索观察验证；构建ClinVideoSuite数据集；使用Grounding-Aware Group Relative Policy Optimization (GA-GRPO)优化，通过基础对齐奖励和证据加权优势直接强化工具使用。
...
Method: 1) 协作推理框架：边缘设备部署轻量级通用ViT，近边缘加速器部署多个中型专家ViT；2) 路由机制：使用边缘模型的Top-k预测动态选择低置信度样本的最相关专家；3) 渐进式专家训练策略：增强专家在数据集子集上的准确性。

Result: 在CIFAR-100数据集和真实边缘/近边缘测试平台上：1) 训练策略将专家在目标子集上的准确率提升4.12%；2) 整体准确率比静态专家提升2.76%；3) 相比边缘执行延迟降低45%；4) 相比仅近边缘卸载能耗降低46%。

Conclusion: 提出的协作推理框架有效平衡了边缘计算和云端计算，通过动态路由和专家训练策略，在保持准确性的同时显著降低了延迟和能耗，为边缘设备部署Vision Transformers提供了实用解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 脑膜瘤的生物学行为和治疗反应取决于其分级，准确诊断对治疗计划和预后评估至关重要。观察到空间-频域特征的加权融合显著影响脑膜瘤分类性能，且不同频率带对分类的贡献在不同图像中差异很大。

Method: 提出自适应权重特征融合架构，用于少样本脑膜瘤学习。该方法自适应地融合离散小波变换提取的不同频率带信息和空间域信息，构建AMSF-Net网络。

Result: 在三个数据集上的实验结果表明，所提方法优于现有最先进方法。同时引入了一个新的脑膜瘤MRI数据集用于验证方法有效性。

Conclusion: 提出的自适应多尺度特征融合方法能有效提升少样本脑膜瘤分类性能，为脑膜瘤的准确诊断和治疗规划提供了有力工具。
...
TL;DR: 该研究应用语义分割和双重机器学习框架，量化视觉环境特征（特别是绿化比例）对交通事故的因果效应，发现绿化对交通事故具有显著负向因果效应，但保护弱势道路使用者的效果有限。
...
Conclusion: 研究为绿化作为潜在安全干预措施提供了因果证据，建议优先改善危险的视觉环境，同时强调需要针对弱势道路使用者进行专门的设计优化。绿化政策应针对高风险区域实施，并考虑不同事故类型的差异化保护效果。
...
<details>
  <summary>Details</summary>
Motivation: 现有图像编辑系统在数据质量、训练效率和评估全面性方面存在不足，需要构建更高质量的训练数据、更有效的训练方法以及更全面的评估基准。

Method: 1) 构建1.6B样本的训练语料库并经过严格清洗过滤；2) 多阶段训练流程：预训练、监督微调和强化学习；3) 引入Multi-Condition Aware Bucket Sampler、Stochastic Instruction Alignment等数据效率优化技术；4) 提出Asymmetric Gradient Optimization、DiffusionNFT、Consistency Loss等稳定优化方法；5) 建立包含15个编辑类别的REDEdit-Bench评估基准。

Result: 在REDEdit-Bench和公开基准测试（ImgEdit和GEdit）上，FireRed-Image-Edit表现出与开源和专有系统相当或更优的性能，特别是在新引入的美化和低层增强任务上表现良好。

Conclusion: FireRed-Image-Edit通过系统性的数据、训练和评估优化，实现了指令图像编辑的先进性能，并发布了代码、模型和基准套件以支持未来研究。
...
TL;DR: FOREST是一个基于潜在扩散变换器的仓库存储意图预测模型，能够根据当前观察和计划存储行为预测存储箱的最终布局。


<details>
  <summary>Details</summary>
Motivation: 在自动化仓库中，机器人执行数百万次存储操作，系统需要在实际执行前根据当前观察和计划存储行为预测存储箱的最终布局，这对仓库规划非常有价值。

Method: FOREST使用物品对齐的实例掩码表示存储箱状态，采用潜在扩散变换器从观察上下文中预测存储后的配置。

Result: FOREST显著提高了预测布局与真实布局之间的几何一致性，在负载质量评估和多存储推理任务中，用FOREST预测替换真实布局仅导致适度的性能损失。

Conclusion: FOREST能够为仓库规划提供有用的前瞻信号，表明该模型在实际应用中的有效性。
...
TL;DR: 本文提出了一种用于商业产品营销图像生成的自动化可扩展流水线，结合文本到图像模型，在保持质量的同时增加创意变化，实现了30.77%的营销对象保真度提升和52%的人类偏好提升。
...
Method: 提出了一种新的自动化可扩展流水线，用于生成商业产品的营销图像。该系统结合文本到图像模型，在保持图像质量和保真度的同时，引入足够的创意变化以符合营销指南。通过优化流程，实现了效率和人工监督的无缝结合。

Result: 使用DINOV2实现了30.77%的营销对象保真度提升，同时生成结果获得了52.00%的人类偏好提升，表明系统在保持质量的同时增加了创意吸引力。

Conclusion: 该研究成功开发了一个平衡自动化和人工监督的文本到图像模型部署流水线，在保持图像质量的同时实现了可扩展性，为商业产品营销图像生成提供了有效的解决方案。
...
TL;DR: 使用高分辨率卫星图像和区域自适应图模型进行大规模砖窑检测，评估多种方法在砖窑监测中的互补优势
...
Method: 提出ClimateGraph区域自适应图模型，捕捉砖窑布局的空间和方向结构；同时评估遥感检测流程并与卫星图像基础模型进行基准测试

Result: 图模型、基础模型和遥感方法在不同方面具有互补优势，为从卫星图像进行可扩展砖窑监测提供实用指导

Conclusion: 综合多种方法可以实现更有效的大规模砖窑监测，为环境和社会治理提供技术支持
...
<details>
  <summary>Details</summary>
Motivation: 现有图像描述研究主要集中在英语，为第四大流行语言印地语开发自动图像描述系统提供了研究机会，旨在填补这一空白。

Method: 使用Flickr8k数据集，通过Google云翻译生成印地语描述。采用VGG16、ResNet50和Inception V3等预训练CNN提取图像特征，结合单向/双向LSTM文本编码和注意力机制生成句子级特征向量。

Result: 基于注意力机制的双向LSTM与VGG16组合在BLEU-1和BLEU-4得分上分别达到0.59和0.19的最佳结果，表明系统能够生成相关且语义准确的印地语图像描述。

Conclusion: 研究成功实现了生成印地语图像描述的目标，为未来多语言图像描述研究提供了可参考的模型框架。
...
TL;DR: AdaCorrection是一种自适应偏移缓存校正框架，用于加速扩散Transformer推理，通过轻量级时空信号估计缓存有效性并自适应混合缓存和新鲜激活，在保持生成质量的同时提供适度加速。


<details>
  <summary>Details</summary>
Motivation: 扩散Transformer在高质量图像和视频生成方面达到最先进性能，但由于其迭代去噪结构导致推理成本高昂。现有加速方法依赖静态重用计划或粗粒度启发式方法，常导致时间漂移和缓存不对齐，显著降低生成质量。

Method: AdaCorrection框架在每个时间步使用轻量级时空信号估计缓存有效性，并自适应混合缓存激活和新鲜激活。这种校正无需额外监督或重新训练即可在线计算，通过自适应偏移缓存校正来维持高生成保真度。

Result: 该方法在保持接近原始FID的同时提供适度加速，在图像和视频扩散基准测试中一致改善生成性能，实现了强生成质量与最小计算开销的平衡。

Conclusion: AdaCorrection通过自适应缓存校正有效解决了扩散Transformer推理加速中的质量退化问题，在保持高保真度的同时实现了效率提升。
...
Result: 在雨雪去除任务中，PSNR/SSIM分别达到35.0023 dB/0.9549(雨)和29.8108 dB/0.9243(雪)，平均优于Histoformer和LDRCNet 1.2570 dB/0.9272 dB(PSNR)和0.0262/0.0289(SSIM)；复杂混合分离中，恢复的双源图像平均PSNR/SSIM为25.0049 dB/0.7997，优于对比方法4.1249 dB/0.0926。

Conclusion: DCDSM在解决雨雪残留去除和细节保留挑战方面表现出优越性，主客观评估均证实其性能优势，为盲图像分离问题提供了有效解决方案。
...
TL;DR: 提出无需参考标准的流程图图像转代码质量评估框架，使用OCR召回率和视觉蕴含精度两个自动化指标，通过F1分数提供统一质量评分。


<details>
  <summary>Details</summary>
Motivation: 在生产环境中，视觉语言模型处理流程图图像生成代码时缺乏真实标注，难以评估输出质量，需要无参考的评估方法进行实时质量监控。

Method: 提出参考无关评估框架，包含两个自动化指标：1) Recall_OCR：通过OCR提取图像文本作为代理参考，评估内容覆盖率；2) Precision_VE：通过视觉蕴含检测生成代码中的幻觉元素。两者的调和平均F1_OCR-VE提供统一质量分数。

Result: 在FlowVQA数据集上验证，与真实标注指标高度一致（召回率、精度、F1的Pearson相关系数分别为0.97、0.91、0.94），证明框架可作为生产环境中连续质量监控的实用替代方案。

Conclusion: 该参考无关评估框架能有效监控流程图图像到代码生成的质量，为生产环境中的视觉语言模型应用提供了实用的质量评估工具。
...
TL;DR: LAF-YOLOv10在YOLOv10n基础上改进，针对无人机图像小目标检测，通过四个互补技术模块提升性能，在VisDrone数据集上达到35.1% mAP@0.5，参数量2.3M，适合嵌入式部署。


<details>
  <summary>Details</summary>
Motivation: 无人机作为主要感知平台，在监控、交通监测和灾害响应中应用广泛，但现有检测器面临无人机特有的挑战：目标像素少、背景杂乱、严重遮挡以及严格的机载计算预算限制。

Method: 基于YOLOv10n构建LAF-YOLOv10，集成四个互补技术：1) PC-C2f模块限制空间卷积到1/4骨干通道；2) AG-FPN在跨尺度融合前插入SE通道门控并使用DySample上采样；3) 增加160×160分辨率的P2检测头，移除P5头；4) 用Wise-IoU v3替换CIoU进行边界框回归。

Result: 在VisDrone-DET2019数据集上达到35.1±0.3% mAP@0.5，参数量2.3M，比YOLOv10n提升3.3个百分点；在UAVDT数据集上达到35.8±0.4% mAP@0.5；在NVIDIA Jetson Orin Nano上实现24.3 FPS (FP16)。

Conclusion: LAF-YOLOv10通过四个互补模块的联合集成，有效解决了无人机图像小目标检测的多个瓶颈问题，在保持轻量化的同时显著提升检测精度，适合嵌入式无人机部署。
...
TL;DR: 该论文针对胸部X光分类中的不完美监督问题，提出了针对长尾多标签分类和零样本OOD识别的解决方案，在CXR-LT 2026挑战赛中取得了领先成绩。


<details>
  <summary>Details</summary>
Motivation: 临床实践中胸部X光分类面临两个主要挑战：一是极端长尾的多标签疾病分布，二是罕见或未见过的疾病标注缺失。CXR-LT 2026挑战赛旨在解决这些问题。
...
TL;DR: 提出了一个室内无人机数据集，并评估了三种基于回放的类增量学习方法在资源受限的无人机平台上的性能，发现FAR在有限内存预算下表现最佳。


<details>
  <summary>Details</summary>
Motivation: 室内无人机等自主智能体需要实时学习新物体类别同时限制灾难性遗忘，但现有无人机数据集主要关注室外场景且缺乏时间连贯的室内视频，因此需要创建室内数据集并评估类增量学习方法。

Method: 1) 创建包含14,400帧的室内无人机数据集，采用半自动标注流程；2) 使用YOLOv11-nano作为资源高效的检测器；3) 评估三种基于回放的CIL策略：ER、MIR和FAR；4) 使用Grad-CAM分析注意力转移。

Result: 在5-10%的有限内存预算下，FAR表现最佳，在5%回放时达到82.96%的平均准确率。Grad-CAM分析显示混合场景中注意力在不同类别间转移，导致无人机定位质量下降。

Conclusion: 这项工作贡献了一个具有时间连贯性的室内无人机视频数据集，并证明了基于回放的持续学习可以有效地应用于边缘空中系统，特别是在有限内存预算下FAR表现优异。
...
TL;DR: 提出混合架构，在可穿戴设备上选择性高分辨率OCR与低分辨率视频流结合，实现文本VQA任务的高效部署
...
Result: 在五个任务类别的文本VQA样本基准测试中，系统达到72%准确率，功耗仅为全分辨率流式传输的0.49倍，可在资源受限的可穿戴设备上持续进行VQA会话而不牺牲文本理解质量。

Conclusion: 通过利用分辨率需求的不对称性，混合架构成功解决了可穿戴设备上文本VQA部署的功耗与性能矛盾，为资源受限环境下的视频大语言模型应用提供了可行方案。
...
TL;DR: 该研究系统评估了7种视频基础模型在帕金森病筛查中的表现，使用32,847个视频数据，发现不同模型在不同临床任务中表现各异，为远程神经监测提供了架构选择指南。


<details>
  <summary>Details</summary>
Motivation: 远程视频评估为帕金森病筛查提供了可扩展途径，但不同视频基础模型架构在临床任务中的比较效果尚不清楚，需要系统研究来确定最适合帕金森病筛查的模型。

Method: 使用1,888名参与者（727名帕金森患者）的32,847个视频，涵盖16个标准化临床任务。评估7种最先进的视频基础模型（VideoPrism、V-JEPA、ViViT、VideoMAE等），通过冻结嵌入和线性分类头评估模型性能。

Result: 不同模型在不同任务中表现各异：VideoPrism在视觉语音运动学和面部表情任务中表现最佳，V-JEPA在上肢运动任务中表现最优，TimeSformer在节奏性任务（如手指敲击）中竞争力强。整体AUC为76.4-85.3%，准确率为71.5-80.6%，特异性高达90.3%，但敏感性较低（43.2-57.3%）。
...
TL;DR: SpargeAttention2是一种可训练的稀疏注意力方法，通过混合Top-k/Top-p掩码规则、高效实现和蒸馏式微调目标，在视频扩散模型中达到95%注意力稀疏度和16.2倍加速，同时保持生成质量。


<details>
  <summary>Details</summary>
Motivation: 现有训练免费稀疏注意力方法有效但有限，可训练稀疏注意力能进一步提高稀疏度但存在三个关键问题：Top-k/Top-p掩码规则何时失效、为何可训练方法能达到更高稀疏度、扩散损失微调的局限性。

Method: 提出SpargeAttention2方法，包含：(1)混合Top-k和Top-p的掩码规则，提高高稀疏度下的鲁棒性；(2)高效的可训练稀疏注意力实现；(3)蒸馏启发的微调目标，更好保持生成质量。

Result: 在视频扩散模型实验中，SpargeAttention2达到95%注意力稀疏度和16.2倍注意力加速，同时保持生成质量，一致优于先前稀疏注意力方法。

Conclusion: SpargeAttention2通过解决可训练稀疏注意力的关键问题，实现了高稀疏度和显著加速而不损失生成质量，为扩散模型高效推理提供了有效方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有的NeRF和3DGS方法主要关注正常光照条件下的自动驾驶场景重建，而夜间低光驾驶场景由于复杂的照明和外观条件更具挑战性，导致现有方法性能下降。需要专门针对夜间场景的重建方法。
...
TL;DR: 提出PCC框架，通过对抗学习在BEV特征中隐藏视觉线索，防止图像重建，保护合作感知中的隐私


<details>
  <summary>Details</summary>
Motivation: 自动驾驶协同感知系统通过共享传感信息提升感知性能，但存在隐私泄露风险，共享数据可能被重建出敏感视觉内容

Method: 设计隐藏网络防止重建网络从共享BEV特征恢复输入图像，采用对抗学习机制训练；感知网络与隐藏网络集成并端到端优化以保持分割性能

Result: PCC框架有效降低重建图像质量，对分割性能影响最小，为协同车辆提供隐私保护

Conclusion: 提出的PCC框架在保护隐私的同时维持了协同感知系统的分割性能，解决了自动驾驶协同感知中的隐私泄露问题
...
TL;DR: Diff-Aid：一种轻量级推理时方法，通过自适应调整跨Transformer块和去噪时间步的每令牌文本-图像交互，提升文本到图像扩散模型的提示遵循能力


<details>
  <summary>Details</summary>
Motivation: 现有文本到图像扩散模型在处理复杂文本描述时存在文本和视觉特征交互不足的问题。先前方法通过架构设计或手工调整文本条件权重来增强交互，但缺乏灵活性，且忽视了不同块和去噪阶段之间的动态交互。

Method: 提出Diff-Aid，一种轻量级推理时方法，能够自适应调整跨Transformer块和去噪时间步的每令牌文本和图像交互。该方法作为即插即用模块，可无缝集成到下游应用中。

Result: 在SD 3.5和FLUX等强基线模型上的实验表明，Diff-Aid在提示遵循、视觉质量和人类偏好等多个指标上均带来一致改进。同时，该方法产生了可解释的调制模式，揭示了不同块、时间步和文本令牌在去噪过程中对语义对齐的贡献。

Conclusion: Diff-Aid提供了一种灵活高效的解决方案，通过自适应调整文本-图像交互来提升文本到图像扩散模型的生成质量，同时具有可解释性和广泛的下游应用集成能力。
...
TL;DR: TwInS是一个受人类视觉系统启发的双流联合学习框架，能够同时执行场景解析和几何视觉任务，通过双向特征交互和半监督训练实现卓越性能。


<details>
  <summary>Details</summary>
Motivation: 受人类视觉系统并行处理上下文和空间信息的启发，旨在开发一个能够同时处理场景解析和几何视觉任务的统一框架，减少对人工标注数据的依赖。

Method: 采用双流架构：场景解析流和几何视觉流相互交互。场景解析的多层次上下文特征注入几何视觉流进行迭代优化，解码的几何特征通过跨任务适配器投影到上下文特征空间进行选择性融合。采用半监督训练策略，利用大规模多视图数据实现自进化。

Result: 在三个公共数据集上的广泛实验验证了TwInS核心组件的有效性，并展示了其优于现有最先进方法的性能。

Conclusion: TwInS是一个有效的生物启发式联合学习框架，通过双向特征交互和半监督训练，在场景解析和几何视觉任务上实现了卓越性能，代码将在发表后公开。
...
<details>
  <summary>Details</summary>
Motivation: 现有视觉注意力增强方法采用预定义缩放因子，存在一个基本权衡：在某些生成步骤中缩放因子太弱无法解决幻觉，而在其他步骤中又太强导致新幻觉。需要自适应地确定每个生成步骤应该增强多少注意力。
...
Conclusion: 通过自适应token级视觉注意力增强，AdaVBoost有效缓解了大型视觉语言模型的幻觉问题，解决了预定义缩放因子的局限性。
...
TL;DR: REVISE是一个用于视频问答的多轮智能体，通过稀疏采样关键帧、维护跨轮次摘要状态、提前停止机制，显著减少计算成本同时提升准确率。


<details>
  <summary>Details</summary>
Motivation: 传统视频问答方法通常均匀采样大量帧，计算成本高且效率低。需要一种能够智能选择关键信息帧、减少冗余计算的方法来提升视频推理的效率和实用性。

Method: 1) 稀疏帧选择：智能选择少量信息丰富的帧而非均匀采样；2) 跨轮次摘要状态维护；3) 置信度提前停止机制；4) 支持专有VLM的即插即用；5) 为开源模型引入EAGER奖励机制，包含置信度增益、摘要充分性和正确提前停止三个奖励项。

Result: 在多个视频问答基准测试中，REVISE在减少帧数、轮次和提示词数量的同时提高了准确率，实现了实用的稀疏视频推理。

Conclusion: REVISE通过稀疏帧选择、状态维护和提前停止机制，为视频问答提供了一种高效实用的解决方案，显著降低了计算成本同时保持了甚至提升了性能。
...
TL;DR: ZEN是一个用于术中手术视频理解的通用基础模型，通过自监督多教师蒸馏框架在超过400万帧、21种手术的视频上训练，在20个下游任务中表现出强大的跨手术泛化能力。


<details>
  <summary>Details</summary>
Motivation: 微创手术中，实时视觉解读存在显著的术者间和手术间差异，限制了评估一致性、培训效果以及可靠AI系统的发展。现有手术AI模型通常针对狭窄定义的任务设计，缺乏跨手术和机构的泛化能力。

Method: 构建大规模多样化数据集（超过400万帧、21种手术），采用自监督多教师蒸馏框架训练ZEN基础模型，系统评估多种表示学习策略，并在统一基准下进行验证。

Result: 在20个下游任务中，包括全微调、冻结骨干网络、少样本和零样本设置下，ZEN始终优于现有手术基础模型，展现出强大的跨手术泛化能力。

Conclusion: ZEN为手术场景理解提供了统一表示，支持未来在术中辅助和手术培训评估中的应用，是迈向通用手术视频理解的重要一步。
...
<details>
  <summary>Details</summary>
Motivation: 无人机视觉目标跟踪需要在准确性和效率之间取得平衡，特别是在不可预测的遮挡等挑战性条件下。现有方法难以同时满足高精度和实时性要求。

Method: 提出LGTrack框架，包含：1）轻量级全局分组坐标注意力（GGCA）模块，捕获长距离依赖和全局上下文；2）轻量级相似性引导层适应（SGLA）模块，替代知识蒸馏，实现跟踪精度和推理效率的最佳平衡。

Result: 在三个数据集上的实验表明，LGTrack在UAVDT数据集上达到258.7 FPS的实时速度，同时保持82.8%的跟踪精度，实现了最先进的性能。
...
TL;DR: DCDM是一个系统级视频生成框架，通过三个专用组件解决视频一致性挑战：语言模型解析语义实现片段内一致性，时序相机表示控制相机运动实现片段间一致性，整体场景生成范式确保镜头间一致性。


<details>
  <summary>Details</summary>
Motivation: 当前视频生成模型虽然视觉保真度高，但在语义、几何和身份一致性方面存在不足，需要解决片段内世界知识一致性、片段间相机一致性和镜头间元素一致性这三个关键挑战。

Method: DCDM采用分而治之策略，将视频一致性建模分解为三个专用组件：1) 使用大语言模型解析提示为结构化语义表示，通过扩散变换器生成连贯内容；2) 提出噪声空间中的时序相机表示实现精确相机运动控制，结合文本到图像初始化增强可控性；3) 采用整体场景生成范式，结合窗口化交叉注意力和稀疏镜头间自注意力，确保长程叙事连贯性。

Result: 在AAAI'26 CVM竞赛测试集上验证，结果表明所提出的策略能有效解决视频一致性挑战。

Conclusion: DCDM通过系统级框架成功解决了视频生成中的三个关键一致性挑战，为高质量视频生成提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 创建韩国医学多模态基准，评估视觉语言模型在韩国医学执照考试风格问题上的表现，补充现有的文本基准，促进韩国医学AI发展。

Method: 从2012-2023年韩国医学执照考试中收集1534个问题和2043张医学图像，约30%包含需要跨图像推理的多张图像。在统一零样本评估协议下，对50多个专有和开源VLM进行基准测试。

Result: 最佳专有模型(Gemini-3.0-Pro)准确率96.9%，最佳开源模型(Qwen3-VL-32B-Thinking)83.7%，最佳韩国专用模型(VARCO-VISION-2.0-14B)仅43.2%。推理导向模型比指令调优模型提升高达20个百分点，所有模型在多图像问题上表现下降。

Conclusion: KorMedMCQA-V填补了韩国医学多模态评估空白，与文本基准形成统一评估套件。结果显示当前模型在韩国医学多模态推理上仍有挑战，特别是在多图像整合和特定医学模态方面。
...
TL;DR: 提出一种基于强化学习的个性化超声心动图采集策略，通过智能选择视图或终止采集来优化心衰评估，在减少32%视频采集量的情况下保持诊断性能


<details>
  <summary>Details</summary>
Motivation: 床旁超声(POCUS)需要在有限时间和操作者精力下支持临床决策，需要一种能平衡诊断性能与采集成本的个性化数据采集策略

Method: 将POCUS建模为顺序采集问题：强化学习代理根据部分观察的多视图研究选择下一个视图或终止采集；终止时，共享多视图变换器进行多任务推理（主动脉瓣狭窄分类和左心室射血分数回归），输出不确定性以平衡诊断收益与采集成本

Result: 在12,180个患者级研究中，测试集1,820个研究上，方法使用32%更少的视频匹配完整研究性能，达到77.2%的平均平衡准确率，展示了在采集预算下的稳健多任务性能
...
TL;DR: 该研究提出了LeafNet数据集和LeafBench基准，用于评估视觉语言模型在植物病理学任务中的表现，发现现有模型在细粒度疾病识别方面存在显著不足。


<details>
  <summary>Details</summary>
Motivation: 尽管基础模型和视觉语言预训练取得了显著进展，但在农业领域特定任务（如植物病理学）中的应用仍然有限，主要原因是缺乏大规模、全面的多模态图像-文本数据集和基准测试。

Method: 研究者创建了包含186,000张叶片图像（涵盖97种疾病类别）的LeafNet数据集，并开发了包含13,950个问答对的LeafBench视觉问答基准，涵盖六个关键农业任务。在12个最先进的视觉语言模型上进行了基准测试。

Result: 基准测试显示模型性能在不同任务间差异显著：二分类健康-患病识别准确率超过90%，而细粒度病原体和物种识别准确率低于65%。多模态模型优于纯视觉模型，表明语言表征整合能显著提升诊断精度。

Conclusion: 研究揭示了当前视觉语言模型在植物病理学应用中的关键差距，强调需要LeafBench作为严格框架来推动方法学进步和评估进展，以实现可靠的AI辅助植物疾病诊断。
...
TL;DR: EchoTorrent是一个用于实时多模态视频生成的框架，通过多教师训练、自适应CFG校准、混合长尾强制和VAE解码器优化四重设计，解决了现有模型在流式推理中的延迟、时空稳定性和唇音同步问题。
...
Method: 1. 多教师训练：在特定偏好领域微调预训练模型获得领域专家，顺序向学生模型传递领域知识
2. 自适应CFG校准：通过分阶段时空调度校准DMD中的音频CFG增强误差，消除冗余CFG计算
3. 混合长尾强制：在长时自展开训练中通过因果-双向混合架构强制尾帧对齐
4. VAE解码器优化：在像素域优化VAE解码器恢复高频细节

Result: EchoTorrent实现了少次自回归生成，显著提升了时间一致性、身份保持和音频-唇音同步性能，在流式模式下有效缓解了时空退化问题。

Conclusion: EchoTorrent通过创新的四重设计解决了多模态视频生成中的效率-性能权衡问题，为实时视频生成提供了有效的解决方案，在保持高质量的同时显著降低了延迟并提升了稳定性。
...
TL;DR: 该研究提出了一种集成学习方法EL-4，通过结合U-Net和FPN分割模型的优势，提高了废物分割的准确性，以支持机器人自动化分拣系统。


<details>
  <summary>Details</summary>
Motivation: 环境污染物处理是全球关键问题，回收利用是重要解决方案。废物分拣是回收过程中获取原材料的关键步骤，但真实废物环境复杂（变形物品、无特定模式、物体重叠），使得分割任务困难。需要提高分割精度以支持机器人准确识别和拾取废物。

Method: 提出集成学习方法，结合U-Net和FPN两种高性能分割模型。U-Net擅长捕捉细节和边界，FPN有效处理尺度变化和复杂环境上下文。采用加权平均方法融合两种模型的预测结果。使用模拟真实废物场景的数据集，并应用预处理技术增强深度学习模型的特征学习。

Result: 集成模型EL-4在IoU指标上达到0.8306，优于U-Net的0.8065；Dice损失降低至0.09019，优于FPN的0.1183。表明集成方法能有效提高分割精度。

Conclusion: 该研究提出的集成学习方法能显著提高废物分割精度，有助于提高材料回收设施的废物分拣效率，促进更好的原材料获取，减少人工干预，提高整体处理能力。
...
Method: 提出基于权重分解低秩适应的多模态生成框架，通过解耦幅度和方向权重更新，使基础生成模型能独立学习神经拓扑和基质对比度。结合神经分割掩码和疾病特异性临床提示，合成解剖学一致的图像。

Result: 在视觉保真度（FID: 5.18）和结构完整性（SSIM: 0.630）方面达到最先进水平，显著优于GAN和标准扩散基线。合成图像保留临床生物标志物，统计上等同于真实患者数据。用于训练诊断模型时，诊断准确率提升2.1%，分割性能提升2.2%。

Conclusion: WDLoRA多模态生成框架能有效合成解剖学真实的角膜共聚焦显微镜图像，缓解医学AI中的数据瓶颈，提高下游诊断任务的性能。
...
TL;DR: 使用微调的视觉语言模型（Microsoft Florence）定位显微镜图像中的寄生虫卵，性能优于传统目标检测方法，为自动化寄生虫诊断提供可扩展解决方案。


<details>
  <summary>Details</summary>
Motivation: 土壤传播蠕虫感染在全球热带和亚热带地区广泛流行，但专业诊断资源有限。传统显微镜诊断方法劳动强度大、耗时且易出错，需要自动化解决方案。

Method: 采用微软Florence视觉语言模型进行微调，专门用于定位显微镜图像中的所有寄生虫卵，并与EfficientDet等传统目标检测方法进行比较。

Result: 定位VLM在mIOU指标上达到0.94，性能优于其他目标检测方法，显示出在寄生虫卵定位方面的优越能力。
...
<details>
  <summary>Details</summary>
Motivation: 机器人手术系统依赖高质量视觉反馈进行精确遥操作，但能量设备产生的手术烟雾会显著降低内窥镜视频质量，影响人机界面和手术效果

Method: 提出RGA-Net（互惠门控和注意力融合网络），采用分层编码器-解码器架构，包含双流混合注意力模块（结合窗口移位注意力和频域处理）和轴分解注意力模块（通过分解注意力机制处理多尺度特征），通过互惠交叉门控块连接

Result: 在DesmokeData和LSD3K手术数据集上的实验表明，RGA-Net在恢复机器人手术集成所需的视觉清晰度方面表现优异

Conclusion: 该方法通过提供持续清晰的视觉化增强了外科医生-机器人界面，为减轻外科医生认知负担、优化手术流程和减少微创手术中的医源性损伤风险奠定了技术基础，代表了通过计算视觉增强实现更可靠、更安全机器人手术系统的重要一步
...
TL;DR: IGOFormer是一个用于航空定向目标检测的查询式检测器，通过几何感知解码和动量匹配机制提升小目标和任意方向目标的检测性能。


<details>
  <summary>Details</summary>
Motivation: 现有查询式检测器在处理任意方向目标（尤其是纹理信息有限的小目标）时性能受限，主要原因是像素级特征解码中未充分利用内在几何信息，以及阶段式二分匹配导致的跨阶段匹配不一致问题。

Method: 提出IGOFormer，包含两个核心组件：1）内在几何感知解码器，通过注入从查询相关性推断的互补几何嵌入来增强目标相关特征；2）基于动量的二分匹配方案，通过指数移动平均自适应聚合历史匹配成本，防止跨阶段匹配不一致。

Result: 在DOTA-V1.0数据集上，使用Swin-T骨干网络和单尺度设置，达到78.00%的AP50分数，展示了在航空定向目标检测中的优越性能。

Conclusion: IGOFormer通过显式整合内在几何到特征解码和增强跨阶段匹配稳定性，有效提升了查询式定向目标检测器的性能，特别是在处理小目标和任意方向目标时表现优异。
...
TL;DR: 本文开发了多个变分自编码器（VAE）来编码3D脑部MRI扫描，系统评估了潜在表示在重建质量、潜在空间结构和唐氏综合征分类任务中的有效性。
...
Conclusion: 该研究表明VAE学习的潜在表示能够有效编码3D脑部MRI的重要特征，并在下游临床分类任务中表现出良好的性能，为生成模型在神经影像研究和临床决策中的应用提供了支持。
...
TL;DR: 提出了一个专门用于OOD文本到运动生成的基准测试，包含1025个OOD文本描述，通过统一评估框架（LLM评估、多因素运动评估、细粒度精度评估）评估14个基线模型，发现现有方法在OOD场景下存在局限性。


<details>
  <summary>Details</summary>
Motivation: 现有文本到运动生成评估主要关注分布内文本输入和有限评估标准，无法系统评估模型在复杂分布外文本条件下的泛化能力和运动生成能力。

Method: 构建包含1025个文本描述的OOD提示数据集，提出统一评估框架：LLM评估、多因素运动评估、细粒度精度评估，对14个代表性基线模型进行全面分析。

Result: 不同基线模型在文本-运动语义对齐、运动泛化性和物理质量等方面各有优势，但大多数模型在细粒度精度评估中表现不佳，揭示了现有方法在OOD场景下的局限性。

Conclusion: 该基准测试为未来生产级文本到运动模型的设计和评估提供了实用指导，强调了在OOD场景下改进模型性能的重要性。
...
TL;DR: OmniScience是一个包含150万张科学图表-标题-上下文三元组的大规模多模态数据集，通过动态模型路由重标注流程提升科学图像理解能力，显著提升开源MLLMs在科学图像理解任务上的性能。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在自然图像理解上表现良好，但在科学图像（如示意图、实验表征图、分析图表）理解上能力有限，特别是开源MLLMs。这种局限主要源于现有数据集领域覆盖有限、结构标注粗糙、语义基础薄弱。

Method: 1) 构建OmniScience数据集：包含150万张图表-标题-上下文三元组，覆盖10+个主要科学领域；2) 开发动态模型路由重标注流程：利用最先进的多模态大语言模型，结合视觉特征、原始图标题和文本参考文献，生成密集、自包含的描述；3) 实施严格质量过滤和人类专家对齐；4) 提出标题QA协议作为视觉理解的代理评估任务。

Result: 1) 图像-文本多模态相似度得分从0.769提升到0.956；2) 在OmniScience上微调的Qwen2.5-VL-3B模型相比基线有显著提升：在MM-MT-Bench上获得0.378的增益，在MMMU上获得0.140的增益。

Conclusion: OmniScience数据集通过高质量的重标注流程显著提升了多模态大语言模型对科学图像的理解能力，填补了现有数据集的不足，为科学图像理解研究提供了有价值的资源。
...
TL;DR: SAM4Dcap：一个从单目视频估计生物力学指标的开源端到端框架，无需额外训练，结合了4D人体网格重建与生物力学求解器。


<details>
  <summary>Details</summary>
Motivation: 定量生物力学分析对临床诊断和损伤预防至关重要，但传统光学动作捕捉系统成本高昂且局限于实验室环境。现有多视角视频方法降低了门槛，但仍不适用于需要单目捕捉的家庭场景。

Method: 整合SAM-Body4D的时间一致4D人体网格恢复与OpenSim生物力学求解器，将重建的网格转换为与多种肌肉骨骼模型兼容的轨迹文件，引入自动化提示策略和Linux原生构建。

Result: 初步评估显示，SAM4Dcap在行走和下落跳跃任务中，膝关节运动学预测可达到与多视角系统相当的水平，尽管在髋关节屈曲和残留抖动方面仍存在一些差异。
...
<details>
  <summary>Details</summary>
Motivation: 现有离线3D MOT方法只是在线框架的直接扩展，未能充分利用离线设置的优势，且依赖固定的上游和定制化架构，适应性有限。

Method: 提出Tracking-by-Tracking标准化范式，仅处理任意现成的跟踪输出并生成离线优化轨迹段。采用预处理、分层匹配与融合、轨迹段优化的结构化流程，充分利用离线跟踪的资源无约束性和未来可观测性。

Result: 在nuScenes上达到77.6% AMOTA的SOTA性能，在KITTI上达到83.00% HOTA的领先结果。实验验证了方法的灵活性、泛化性和模块有效性。

Conclusion: Offline-Poly通过跟踪中心设计和TBT范式，有效解决了现有离线3D MOT方法的局限性，在多个数据集上取得了最先进的性能，展现了良好的通用性和模块化优势。
...
TL;DR: 提出Skeleton2Stage方法，通过强化学习微调扩散模型，解决舞蹈生成中骨架到网格的物理约束问题，减少身体自穿透和足地接触异常。


<details>
  <summary>Details</summary>
Motivation: 现有舞蹈生成方法主要在骨架域训练，忽略了网格级的物理约束，导致生成的舞蹈在人体网格可视化时出现身体自穿透和足地接触异常，降低了美学效果并限制了实际应用。

Method: 1) 从人体网格推导基于物理的奖励函数；2) 应用强化学习微调(RLFT)引导扩散模型生成物理合理的运动；3) 奖励设计包括：模仿奖励（通过物理模拟器评估运动可模仿性）、足地偏差奖励（FGD）及测试时FGD指导；4) 提出抗冻结奖励来保持运动动态性。

Result: 在多个舞蹈数据集上的实验表明，该方法能显著提高生成运动的物理合理性，产生更真实和美观的舞蹈动作。

Conclusion: 通过结合基于物理的奖励和强化学习微调，成功弥合了骨架到网格的差距，生成了物理合理且美观的舞蹈动作，为舞蹈生成的现实应用提供了更好的解决方案。
...
TL;DR: 提出PerASCD方法，利用遥感基础模型PerA增强语义变化检测，通过级联门控解码器简化流程并提升性能，在公开数据集上达到SOTA效果。
...
Method: 基于遥感基础模型PerA，提出级联门控解码器简化复杂解码流程并促进多级特征交互融合，引入软语义一致性损失缓解训练数值不稳定性。

Result: 在两个公开基准数据集上达到最先进性能，解码器能有效简化SCD范式并实现跨不同视觉编码器的无缝适配。

Conclusion: PerASCD方法通过基础模型驱动和简化解码器设计，显著提升了语义变化检测的性能和实用性，为遥感动态监测提供了有效工具。
...
TL;DR: DiWR是一种从无定向点云重建水密表面的鲁棒方法，通过联合优化点方向、面积权重和置信系数，使用广义环绕数场作为隐式表示，能处理非均匀采样、噪声和离群点。


<details>
  <summary>Details</summary>
Motivation: 现有方法在处理非均匀采样、噪声和离群点的无定向点云时面临挑战，需要多阶段预处理或无法有效处理这些复杂情况。需要一种能直接处理这些挑战的端到端重建方法。

Method: 使用广义环绕数场作为目标隐式表示，在单一流程中联合优化点方向、每点面积权重和置信系数。通过最小化诱导环绕数场的Dirichlet能量以及额外的GWN约束，补偿非均匀采样、减少噪声影响并降低离群点权重。

Result: 在3D高斯泼溅、计算机视觉流程和损坏的图形基准测试的点云上评估，DiWR能在这些挑战性输入上产生合理的水密表面，优于传统的多阶段流程和最近的联合方向重建方法。

Conclusion: DiWR提供了一种鲁棒的水密表面重建方法，能有效处理非均匀采样、噪声和离群点，无需依赖单独的预处理步骤，在多种挑战性场景中表现优于现有方法。
...
Method: 设计多尺度动力学机制分解复杂运动场，提出具有多尺度动力学的高斯序列表示，通过多级运动组合构建动态3D高斯，并整合视觉基础模型的多模态先验进行互补监督。

Result: 在基准和真实世界操作数据集上的动态新视角合成实验中，相比现有方法取得了显著改进，实现了准确且全局一致的4D重建。

Conclusion: 通过多尺度动力学分解和多模态先验整合，成功解决了单目视频4D重建的歧义问题，为机器人学习提供了可扩展的动态场景理解方法。
...
TL;DR: VAR-3D提出了一种新的文本到3D生成方法，通过视图感知的3D VQ-VAE和渲染监督训练策略，解决了现有方法在3D表示学习中的信息丢失和几何一致性退化问题。


<details>
  <summary>Details</summary>
Motivation: 现有文本到3D生成方法面临两大挑战：1）在编码过程中存在信息丢失，导致表示失真，向量量化进一步放大了这种失真，降低了文本条件3D形状的几何一致性；2）传统的两阶段训练范式在重建和文本条件自回归生成之间存在目标不匹配。

Method: 提出VAR-3D框架：1）使用视图感知的3D VQ-VAE将复杂3D几何结构转换为离散标记；2）引入渲染监督训练策略，将离散标记预测与视觉重建耦合，鼓励生成过程更好地保持视觉保真度和结构一致性。

Result: 实验表明VAR-3D在生成质量和文本-3D对齐方面显著优于现有方法。

Conclusion: VAR-3D通过创新的视图感知3D表示学习和渲染监督训练，有效解决了文本到3D生成中的表示失真和几何一致性退化问题，为高质量文本条件3D生成提供了新思路。
...
TL;DR: 提出基于嵌入器引导强化学习（EG-RL）的推理驱动通用多模态嵌入框架，通过生成可追溯的推理链（T-CoT）来提升多模态检索性能


<details>
  <summary>Details</summary>
Motivation: 现有生成式嵌入方法的推理链仅局限于文本分析，与目标检索无关，无法有效提升跨模态语义一致性

Method: 1) 设计EG-RL框架，嵌入器为推理器提供显式监督；2) 引入T-CoT提取关键多模态线索；3) 通过强化学习优化推理过程

Result: 在有限计算资源下，在MMEB-V2和UVRB基准测试中超越了领先的嵌入模型，提升了跨模态语义一致性和细粒度匹配能力
...
<details>
  <summary>Details</summary>
Motivation: 超声去噪对于减轻斑点噪声引起的图像退化、提高图像质量和诊断可靠性至关重要。然而，斑点模式同时包含纹理和精细解剖细节，在有效抑制噪声的同时保持结构保真度仍然是一个重大挑战。

Method: 提出先验引导的分层实例-像素对比学习模型：1) 统计引导的像素级对比学习策略，增强噪声像素和干净像素之间的分布差异，提高局部结构一致性；2) 使用内存库促进特征空间的实例级对比学习，使表示更接近真实数据分布；3) 采用混合Transformer-CNN架构，结合Transformer编码器进行全局上下文建模和CNN解码器进行细粒度解剖结构恢复。

Result: 在两个公开可用的超声数据集上进行广泛评估，结果表明所提出的模型始终优于现有方法，证实了其有效性和优越性。

Conclusion: 该研究提出的先验引导分层对比学习模型能够有效解决超声去噪中噪声抑制与结构保留的平衡问题，通过多级对比学习和混合架构实现了更好的去噪性能。
...
<details>
  <summary>Details</summary>
Motivation: 解决在超低比特率语义通信约束下实现高保真、因果、实时视频生成的挑战，传统方法难以在极低带宽下保持视频质量

Method: 使用有损语义视频编码传输语义场景结构，配合高度压缩的低分辨率帧提供纹理信息；提出模块化视频扩散模型（语义控制、恢复适配器、时间适配器）和高效时间蒸馏方法

Result: 在超低比特率（<0.0003 bpp）下实现强感知质量、语义保真度和时间一致性，显著优于传统、神经和生成基线方法，参数量减少300倍，训练时间减少2倍

Conclusion: 该框架为超低比特率语义通信提供了一种有效的视频生成解决方案，在保持高质量的同时满足实时性和因果性要求
...
TL;DR: 使用深度学习从术前心脏CT预测TAVI术后瓣周漏的研究，通过3D卷积神经网络分析术前CT体积数据来预测并发症风险。


<details>
  <summary>Details</summary>
Motivation: 主动脉瓣狭窄是老年患者的常见致命疾病，TAVI是主要治疗方法。尽管手术技术不断进步，但瓣周漏仍然是TAVI术后最常见的并发症之一，对长期预后有显著影响。目前需要从术前影像中预测这一并发症的方法。

Method: 收集了TAVI术前患者的数据集，使用3D卷积神经网络在等向性CT体积数据上进行训练，通过深度学习分析术前CT影像来预测瓣周漏的发生。

Result: 研究结果表明，基于体积的深度学习能够从术前TAVI影像中捕捉到细微的解剖特征，为预测瓣周漏提供了有效方法。

Conclusion: 这项研究展示了深度学习在预测TAVI术后瓣周漏方面的潜力，为个性化风险评估和手术优化开辟了新途径。源代码已公开。
...
TL;DR: 提出一个用于机器人手术器械分割的合成数据集生成与验证工作流，通过3D重建和动画技术创建逼真的标注视频序列，验证了合成数据与真实数据平衡使用能显著提升模型泛化能力。


<details>
  <summary>Details</summary>
Motivation: 解决机器人手术中器械分割任务面临的数据稀缺问题，需要大量标注数据但真实手术数据获取困难且标注成本高，需要可扩展的合成数据生成方法。

Method: 使用3D重建达芬奇机器人手臂，在Autodesk Maya中通过全自动Python流程生成逼真标注视频序列，包含随机运动模式、光照变化和合成血液纹理，保持像素级准确的地面真值掩码。

Result: 实验表明，真实数据与合成数据的平衡组合相比仅使用真实数据能显著提升模型泛化能力，但过度依赖合成数据会导致可测量的域偏移。提供了可复现的框架和开源代码。

Conclusion: 该框架为手术计算机视觉提供了可复现、可扩展的工具，支持数据增强、域适应和基于模拟的预训练研究，有助于推动机器人辅助手术的发展。
...
TL;DR: 提出基于SimCLR的自监督学习预训练策略，用于从心尖四腔超声心动图视频预测心输出量，在数据有限情况下仍能提升性能并减轻过拟合
...
Conclusion: 即使在数据稀缺的情况下，自监督学习预训练也能有效提升心输出量预测性能，证明了自监督学习在医学影像分析中的潜力。
...
TL;DR: 研究发现，生成模型与人类视觉行为对齐度的提升主要源于图像缩放操作的低通滤波效应，而非生成模型本身。简单地在测试时模糊图像就能显著提高判别模型的视觉对齐度。


<details>
  <summary>Details</summary>
Motivation: 尽管深度神经网络在计算机视觉基准测试中表现出色，但在模拟人类视觉行为方面仍有不足。先前研究认为生成分类器比判别分类器能更好地对齐人类视觉行为，但本文旨在验证这种对齐提升是否主要源于图像预处理中的低通滤波效应。

Method: 通过一系列控制实验：1）在判别模型（如CLIP）测试时应用低通滤波器（模糊图像）；2）直接优化滤波器以最大化对齐度；3）计算所有可能的帕累托最优解前沿；4）将最优高斯滤波器与人类视觉系统的带通滤波器频谱进行比较。

Result: 1）仅通过测试时模糊图像，就能将判别模型在模型vs人类基准测试中的对齐度提升至新SOTA，将DNN与人类观察者之间的对齐差距减半；2）低通滤波器可能是最优的；3）最优高斯滤波器的频谱与人类视觉系统的对比敏感度函数相匹配。

Conclusion: 生成模型与人类视觉行为对齐度的提升主要源于图像缩放操作的低通滤波效应，而非生成模型架构本身。人类视觉系统的对比敏感度函数可以通过特定宽度的高斯滤波器很好地近似，这种滤波器也能最大化错误一致性。
...
TL;DR: 比较DNN模型与人类在颜色恒常性任务中的表现，发现两者在VR环境中对局部环绕、最大通量和空间均值等颜色恒常机制有相似的行为模式


<details>
  <summary>Details</summary>
Motivation: 研究深度神经网络模型是否能模拟人类颜色恒常性的认知机制，通过比较模型与人类在相同VR任务中的表现，验证模型对人类颜色感知的建模能力

Method: 使用预训练的ResNet U-Net模型预测表面反射率，通过迁移学习在VR基线条件下微调解码器，让模型执行与人类实验相同的消色差物体选择任务

Result: 模型与人类行为高度一致：两者在基线条件下都表现出高颜色恒常性，当移除局部环绕或空间均值颜色线索时，都出现相似的条件依赖性性能下降

Conclusion: 深度神经网络能够有效模拟人类颜色恒常性的关键机制，为理解人类颜色感知的计算基础提供了新的研究工具和验证方法
...
TL;DR: 提出基于DINOv2视觉Transformer的字体分类系统，使用LoRA微调，仅训练不到1%参数即可在394种字体上达到约86%的top-1准确率


<details>
  <summary>Details</summary>
Motivation: 需要开发能够准确识别大量字体家族的实用系统，同时解决训练数据稀缺和模型部署效率的问题

Method: 1) 构建合成数据集生成流水线，使用Google Fonts进行大规模渲染并加入多样化增强；2) 采用LoRA对DINOv2 Vision Transformer进行微调，仅更新少量参数；3) 集成内置预处理确保训练推理一致性；4) 部署为HuggingFace推理端点

Result: 在394种字体分类任务上达到约86%的top-1准确率，仅训练了87.2M参数中的不到1%，模型、数据集和训练流水线已开源

Conclusion: LoRA微调结合合成数据生成是高效字体分类的有效方法，开源资源为字体识别研究提供了实用工具
...
TL;DR: RPGD是一种基于人体姿态的外参标定框架，通过RANSAC-P3P粗配准和梯度下降精配准，仅使用自然人体运动就能将MoCap 3D骨骼数据与单目或多视角RGB相机对齐。
...
Method: 采用从粗到精的策略：1) 使用RANSAC-P3P进行全局鲁棒的粗配准；2) 基于梯度下降的精配准优化；3) 专门针对人体姿态设计的标定框架。

Result: 在三个大规模公开3D HPE数据集和自采集的野外数据集上验证，RPGD能够恢复与地面真值精度相当的外参，即使在具有挑战性的噪声环境下也能实现亚像素级的MPJPE重投影误差。

Conclusion: RPGD为大规模3D人体姿态估计数据集采集提供了实用、自动的外参标定解决方案，仅需自然人体运动即可实现可靠标定。
...
TL;DR: MamaDino模型通过结合卷积与Transformer架构，并显式建模双侧乳腺不对称性，在低分辨率（512x512）乳腺X光片上实现了与高分辨率Mirai模型相当的3年乳腺癌风险预测性能。


<details>
  <summary>Details</summary>
Motivation: 当前乳腺癌筛查正从统一间隔转向风险适应性和个性化策略。现有深度学习模型（如Mirai）通常使用卷积网络、高分辨率输入（>100万像素）和简单的多视图融合，对双侧乳腺不对称性的显式建模有限。研究者假设结合互补的归纳偏置和显式不对称建模可以在更低分辨率图像上达到先进性能。

Method: 提出MamaDino模型：融合冻结的自监督DINOv3 ViT-S特征与可训练的CNN编码器（512x512分辨率），通过BilateralMixer聚合双侧乳腺信息，输出3年乳腺癌风险评分。在OPTIMAM数据集（53,883名女性）上训练，在内部测试集和外部未见站点上进行评估。

Result: MamaDino在乳腺级别上匹配Mirai性能，同时使用约13倍更少的输入像素。添加BilateralMixer将内部测试AUC从0.713提升至0.736，外部测试从0.666提升至0.677。性能在不同年龄、种族、扫描仪、肿瘤类型和分级上表现一致。

Conclusion: 显式双侧乳腺建模和互补归纳偏置的结合使得在显著更低分辨率的乳腺X光片上也能达到与先进模型相当的预测性能，表明以更结构化方式使用较少细节图像可以恢复先进准确性。
...
Method: 构建了SpaVis-6M数据集（最大的Visium空间转录组数据集），训练空间感知基因编码器。采用分层多尺度对比对齐和跨尺度斑块定位机制，将空间转录组与病理图像对齐，捕捉空间结构和分子变异。

Result: 在六个数据集和四个下游任务上验证，STAMP始终表现出强大性能，证明了自监督、基因引导训练为学习病理图像表示提供了稳健且任务无关的信号。
...
TL;DR: MarsRetrieval：首个用于火星地理空间发现的视觉-语言检索基准，包含三个任务，评估多模态嵌入架构，显示领域特定微调对行星环境通用地理空间发现至关重要


<details>
  <summary>Details</summary>
Motivation: 现有基准大多局限于封闭集监督视觉任务，不支持文本引导检索进行地理空间发现。需要专门针对火星探索的检索基准来评估视觉-语言模型在地理发现中的能力

Method: 提出MarsRetrieval检索基准，包含三个任务：配对图像-文本检索、地貌检索、全球地理定位。采用统一的检索中心协议，评估对比双塔编码器和生成式视觉-语言模型等多模态嵌入架构

Result: MarsRetrieval具有挑战性：即使是强大的基础模型也常常无法捕捉领域特定的地貌区分。领域特定微调对行星环境中通用地理空间发现至关重要

Conclusion: MarsRetrieval填补了火星地理空间发现检索基准的空白，为评估视觉-语言模型在行星科学中的应用提供了重要工具，强调领域适应在行星探索中的必要性
...
TL;DR: E-DiT：基于动态路由和特征缓存的弹性扩散Transformer加速框架，通过自适应跳过计算块和调整MLP宽度，实现2倍加速且保持生成质量
...
Method: 为每个DiT块配备轻量级路由器，动态识别输入潜变量的样本相关稀疏性，自适应决定是否跳过该块；若不跳过则预测最优MLP宽度缩减比例；推理时引入块级特征缓存机制，利用路由器预测消除冗余计算。

Result: 在2D图像（Qwen-Image和FLUX）和3D资产（Hunyuan3D-3.0）上的广泛实验表明，E-DiT可实现约2倍加速，且生成质量损失可忽略不计。

Conclusion: E-DiT通过自适应利用DiT生成过程中的样本相关稀疏性，有效解决了现有固定计算容量加速方法的局限性，在保持生成质量的同时显著提升效率。
...
TL;DR: SpatialID：无需训练的空间自适应身份调制框架，通过空间掩码提取器和时空调度策略，在文本到图像生成中实现更好的身份保持和文本遵循
...
Result: 在IBench上的大量实验表明，SpatialID在文本遵循（CLIP-T: 0.281）、视觉一致性（CLIP-I: 0.827）和图像质量（IQ: 0.523）方面达到最先进性能，显著消除背景污染同时保持强大的身份保持能力。

Conclusion: SpatialID通过空间自适应身份调制和动态调度策略，有效解决了现有方法中身份特征污染非面部区域的问题，实现了更好的个性化文本到图像生成效果。
...
Result: 相比Virchow2，参数减少28倍，FLOPs降低403.5倍；在NVIDIA Jetson Orin Nano上每小时处理208张切片，速度提升104.5倍，能耗降低171倍；在37个队列、26个任务上验证，平均AUC保留Virchow2的99.71%，在19个模型中排名第二。
...
TL;DR: Flow4R：一个统一的动态3D场景重建框架，以相机空间场景流为核心表示，从两视图输入中联合推断几何、物体运动和相机运动。


<details>
  <summary>Details</summary>
Motivation: 现有方法通常将几何与运动解耦：多视图重建方法假设静态场景，而动态跟踪框架依赖显式的相机姿态估计或分离的运动模型。需要一种统一的方法来同时处理3D结构、物体运动和相机运动。

Method: Flow4R将相机空间场景流作为核心表示，使用Vision Transformer从两视图输入中预测每个像素的最小属性集：3D点位置、场景流、姿态权重和置信度。这种以流为中心的表述允许通过共享解码器在单次前向传播中对称推断局部几何和双向运动，无需显式姿态回归器或光束法平差。

Result: 在静态和动态数据集上联合训练后，Flow4R在4D重建和跟踪任务上实现了最先进的性能，证明了流中心表示在时空场景理解中的有效性。

Conclusion: Flow4R展示了以场景流为核心表示的统一框架能够有效处理动态3D场景重建问题，为时空场景理解提供了一种简洁而强大的方法。
...
Method: 提出FLEX框架：1) 频率感知RoPE调制，自适应插值欠训练的低频分量并外推高频分量；2) 反相位噪声采样，注入高频动态先验；3) 仅推理注意力锚点，固定全局结构。这是一个即插即用的推理时增强方法。

Result: 在VBench评估中，FLEX在6倍外推（30秒时长）上显著优于最先进模型，在12倍尺度（60秒时长）上与长视频微调基线性能相当。可支持LongLive等模型实现4分钟规模的连贯动态视频合成。

Conclusion: FLEX有效解决了自回归视频扩散模型的长视频生成外推失败问题，通过频率感知调制和动态先验注入，无需额外训练即可显著扩展生成时长，为现有推理管道提供即插即用的长视频生成增强方案。
...
TL;DR: 提出基于可解释性启发的层剪枝框架，通过梯度-激活归因评估层重要性，相比传统L1范数剪枝在目标检测任务中实现更好的精度-效率权衡。
...
Method: 提出可解释性启发的层剪枝框架，采用SHAP启发的梯度-激活归因方法来估计层重要性，作为功能贡献的数据驱动代理，而非仅依赖静态权重大小。

Result: 在多种目标检测架构（ResNet-50、MobileNetV2、ShuffleNetV2、Faster R-CNN、RetinaNet、YOLOv8）上验证，在COCO数据集上相比L1范数剪枝方法：ShuffleNetV2推理速度提升10%（L1剪枝性能下降13.7%），RetinaNet保持基线mAP（0.151）且推理速度影响可忽略（L1剪枝mAP下降1.3%，速度提升6.2%）。
...
TL;DR: BitDance：一种可扩展的自回归图像生成器，通过预测二进制视觉标记而非码本索引，使用二进制扩散头生成高熵二进制潜在表示，并采用next-patch扩散解码方法实现高效并行推理。


<details>
  <summary>Details</summary>
Motivation: 传统自回归图像生成器使用码本索引表示图像，表达能力有限。BitDance旨在通过二进制视觉标记创建更紧凑且表达能力更强的离散表示，同时解决从巨大标记空间采样的难题。

Method: 1. 使用二进制视觉标记（每个标记代表2^256种状态）替代码本索引；2. 采用二进制扩散头（连续空间扩散）而非softmax分类来生成二进制标记；3. 提出next-patch扩散解码方法，并行预测多个标记以加速推理。

Result: 在ImageNet 256x256上达到FID 1.24（自回归模型最佳）；使用260M参数（比1.4B参数模型少5.4倍）且推理速度快8.7倍；生成1024x1024图像时比先前AR模型快30倍以上；文本到图像生成表现出色。

Conclusion: BitDance通过二进制标记表示和扩散解码方法，在保持参数效率的同时显著提升了自回归图像生成的性能与速度，为AR基础模型研究提供了新方向。
...
<details>
  <summary>Details</summary>
Motivation: 现实场景中低质量图像（缺乏清晰语义结构和高频细节）会导致语义分割性能下降。传统图像恢复模型关注像素级保真度，但无法恢复任务相关的语义线索；而现有分割模型对真实世界退化缺乏鲁棒性。
...
TL;DR: CoCoEdit是一个通过区域正则化强化学习实现内容一致性图像编辑的后训练框架，在保持编辑质量的同时显著减少非编辑区域的意外变化。


<details>
  <summary>Details</summary>
Motivation: 现有图像编辑模型主要关注目标对象的编辑效果，但往往导致非编辑区域发生意外变化，缺乏内容一致性。需要一种方法在保持编辑质量的同时确保非编辑区域内容不变。

Method: 1) 构建包含4万个高质量样本的训练集，包含精炼指令和掩码；2) 引入像素级相似度奖励补充MLLM奖励；3) 提出基于区域的正则化器，对高奖励样本保持非编辑区域，对低奖励样本鼓励编辑效果；4) 在GEdit-Bench和ImgEdit-Bench上标注编辑掩码进行评估。

Result: 将CoCoEdit应用于Qwen-Image-Edit和FLUX-Kontext模型，不仅实现了与SOTA模型竞争的编辑分数，而且在内容一致性方面显著更好，PSNR/SSIM指标和人类主观评分都证明了这一点。

Conclusion: CoCoEdit框架通过区域正则化强化学习有效解决了图像编辑中的内容一致性问题，在保持高质量编辑的同时最小化非编辑区域的意外变化，为实际应用提供了可靠解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于文本中心思维链的MLLMs在图像伪造检测中存在幻觉问题，因为语言模态无法充分捕捉像素级的不一致性。需要一种能够将不可见的低层篡改痕迹转化为显式视觉表示的方法。

Method: 提出ForgeryVCR框架，包含法证工具箱将不可见痕迹转化为视觉中间表示。采用战略工具学习后训练范式，包括增益驱动的轨迹构建用于监督微调，以及基于工具效用奖励的强化学习优化。

Result: ForgeryVCR在检测和定位任务中均达到最先进性能，展现出优越的泛化能力和鲁棒性，同时工具冗余最小。

Conclusion: 通过视觉中心推理和战略工具学习，ForgeryVCR能够主动调用多视角推理路径，有效解决传统文本中心方法在图像伪造检测中的幻觉问题，为MLLMs在法证分析中的应用提供了新思路。
...
<details>
  <summary>Details</summary>
Motivation: 现有的大规模重建模型（LRMs）在单图像3D重建中虽然进展迅速，但重建结果常存在几何不一致性和细节错位问题，限制了重建的保真度。

Method: 提出GeoFusionLRM框架，通过专门的transformer和融合模块反馈几何线索（法线和深度预测），使模型能够纠正错误并强制与条件图像的一致性，而不依赖仅从输入图像提取的特征。

Result: 大量实验表明，GeoFusionLRM相比最先进的LRM基线方法，实现了更锐利的几何结构、更一致的法线方向和更高的重建保真度。

Conclusion: GeoFusionLRM通过几何感知的自校正机制，有效提升了单图像3D重建的结构准确性和与输入视图的对齐度，无需额外监督，为LRM提供了改进方向。
...
<details>
  <summary>Details</summary>
Motivation: 人类感知本质上是多感官的，整合视觉、声音和运动来理解世界。声音提供了关于空间布局、屏幕外事件和因果交互的重要线索，特别是在自我中心设置中听觉和视觉信号紧密耦合。然而，当前的多模态大语言模型主要关注视觉-语言理解，缺乏对声音理解能力的系统评估。

Method: 1. 整合Ego4D和EgoBlind数据集，涵盖有视觉和依赖声音的体验；2. 定义七任务分类法：内在声音感知、空间定位、因果推理和跨模态推理；3. 通过多阶段自动生成流程构建基准，包含7315个验证过的问答对，覆盖900个视频；4. 在9个最先进的多模态大语言模型上进行全面实验评估。

Result: 当前模型展现出初步的听觉推理能力，但在细粒度的空间和因果理解方面仍然有限。EgoSound基准为推进多感官自我中心智能建立了具有挑战性的基础。

Conclusion: EgoSound是首个系统评估多模态大语言模型在自我中心声音理解能力的基准，揭示了当前模型的局限性，并为弥合"看到"和"真正听到"世界之间的差距提供了重要基础，推动了多感官自我中心智能的发展。
...
TL;DR: DenseMLLM：一种无需任务特定解码器的多模态大语言模型，通过视觉token监督策略实现密集预测任务（如语义分割、深度估计），保持标准架构的同时达到竞争性性能。


<details>
  <summary>Details</summary>
Motivation: 当前MLLMs在高级视觉理解方面表现出色，但扩展到细粒度密集预测任务通常需要复杂的任务特定解码器和定制化，这增加了模型复杂性，偏离了MLLMs的通用设计理念，限制了其实用性。

Method: 提出DenseMLLM模型，基于标准架构，采用新颖的视觉token监督策略来处理多标签和多任务，无需额外任务特定解码器，保持简约设计。

Result: 模型在广泛的密集预测和视觉语言基准测试中实现了高度竞争性的性能，证明标准通用MLLM无需架构专业化即可有效支持密集感知。

Conclusion: 标准通用多模态大语言模型可以通过适当的监督策略有效执行密集预测任务，无需复杂的任务特定解码器，这为MLLMs的实际应用提供了更简洁实用的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 传统机械化板栗收获成本高、非选择性且易损坏坚果。开发低成本、视觉引导的自动化收获技术需要准确可靠的板栗检测系统，但在复杂果园环境中存在阴影、光照变化、杂草、落叶等干扰，这些挑战尚未得到解决。

Method: 收集了319张果园地面板栗图像，包含6524个标注板栗。系统评估了29种最先进的实时目标检测器，包括14个YOLO（v11-13）系列和15个RT-DETR（v1-v4）系列的不同规模模型，通过重复建模实验进行板栗检测性能比较。

Result: YOLOv12m模型在所有评估模型中取得最佳mAP@0.5（95.1%），RT-DETRv2-R101是RT-DETR模型中最准确的变体（mAP@0.5为91.1%）。YOLOv11x在mAP@[0.5:0.95]上达到最佳精度（80.1%）。所有模型都显示出实时板栗检测的潜力，YOLO模型在检测精度和推理速度上都优于RT-DETR模型。

Conclusion: YOLO模型更适合板载部署，为低成本自动化板栗收获技术提供了有效的视觉检测解决方案。研究数据集和软件程序已公开，促进了该领域进一步研究。
...
<details>
  <summary>Details</summary>
Motivation: 现有扩散语言模型主要针对特定任务进行强化学习训练，缺乏统一的多模态理解和生成能力。作者希望构建一个通用的多模态推理扩散语言模型，能够统一处理多种任务。

Method: 提出统一的后期训练框架，结合监督微调(SFT)和多任务强化学习(RL)。采用新颖的训练技术：答案强制、树搜索和互补似然估计，以提高效果和可扩展性。

Result: 大量实验表明LaViDa-R1在广泛的视觉任务上表现强劲，包括视觉数学推理、推理密集型grounding和图像编辑等。

Conclusion: LaViDa-R1成功构建了一个统一的多模态推理扩散语言模型，展示了扩散语言模型在多模态理解和生成任务中的潜力。
...
TL;DR: ARport：基于增强现实的标记物自由手术端口规划系统，通过头戴式显示器将术前规划的套管布局映射到患者体表，简化机器人辅助手术的术中设置。


<details>
  <summary>Details</summary>
Motivation: 机器人辅助手术中，精确的端口放置对手术视野和器械操作至关重要。目前术前规划与术中执行之间存在差距，需要一种直观的空间引导系统来弥合这一差距。

Method: 基于光学透视头戴式显示器实现，无需外部传感器或标记物。系统通过RGB、深度和姿态数据重建手术场景，使用基础模型提取患者体表，通过表面配准将术前解剖模型与患者体表对齐，实现套管布局的原位可视化。

Result: 在全尺寸人体模型实验中，ARport能够准确地将术前规划的套管位置叠加到物理模型上，实现了虚拟规划与真实解剖结构之间的空间一致性。

Conclusion: ARport提供了一个完全无标记、硬件需求最小的解决方案，可在患者体表直接可视化术前套管规划。该系统有助于高效的术中设置，并展示了无缝集成到常规临床工作流程的潜力。
...
TL;DR: 基于Moufad等人(2025)的工作，本文扩展了无VJP计算的测试时引导方法，在大规模图像视频编辑基准上验证其性能，证明测试时引导可媲美甚至超越基于训练的方法。


<details>
  <summary>Details</summary>
Motivation: 文本驱动的图像视频编辑本质上可视为修复问题，但现有基于扩散和流模型的测试时引导方法依赖昂贵的VJP计算，限制了实际应用。

Method: 基于Moufad等人(2025)的无VJP近似方法，提供理论分析并大幅扩展其在大规模图像视频编辑基准上的实证评估。

Result: 测试时引导方法在性能上可媲美甚至超越基于训练的方法，证明了其在大规模图像视频编辑任务中的有效性。

Conclusion: 测试时引导方法为文本驱动的图像视频编辑提供了实用且高效的解决方案，无需昂贵的VJP计算即可达到与训练方法相当的性能。
...
TL;DR: SEAL是一个参数高效的视觉-组学自监督学习框架，通过将局部分子信息注入病理视觉编码器来增强现有病理基础模型，在多种下游任务中表现优于纯视觉和空间转录组预测基线。


<details>
  <summary>Details</summary>
Motivation: 空间转录组学提供了空间分辨的基因表达测量，但如何系统性地利用局部表达与形态学之间的耦合关系来改进病理学表征仍是一个挑战。现有的多模态基础模型成功整合视觉与互补模态，表明可以利用这种形态-分子耦合来增强病理学表示。

Method: SEAL是一个参数高效的视觉-组学微调方法，可以灵活应用于广泛使用的病理基础模型。该方法在超过70万个配对的基因表达点-组织区域示例上进行训练，涵盖14个器官的肿瘤和正常样本。

Result: 在38个切片级和15个块级下游任务测试中，SEAL在切片级分子状态、通路活性和治疗反应预测以及块级基因表达预测任务上，均优于广泛使用的纯视觉和空间转录组预测基线。SEAL编码器在分布外评估中表现出稳健的领域泛化能力，并支持新的跨模态功能如基因到图像检索。

Conclusion: SEAL为空间转录组指导的病理基础模型微调提供了一个通用框架，表明用局部分子监督增强现有模型是改进视觉表征和扩展其跨模态效用的有效且实用的方法。
...
TL;DR: UniWeTok是一个统一的离散视觉分词器，使用大规模二进制码本（2^128）同时支持高保真重建、复杂语义提取和生成适应性，在图像生成、理解和编辑任务上达到SOTA性能。
...
Method: 提出UniWeTok统一离散分词器，采用大规模二进制码本；训练框架包括前后蒸馏和生成感知先验；架构采用卷积-注意力混合架构和SigLu激活函数；设计三阶段训练框架增强跨分辨率和感知敏感场景的适应性。

Result: 在ImageNet上达到SOTA图像生成性能（FID: 1.38 vs REPA 1.42），训练计算量显著降低（33B vs 262B tokens）；在通用领域，在多模态理解、图像生成（DPG Score: 86.63 vs FLUX.1 83.84）和编辑（GEdit Score: 5.09 vs OmniGen 5.06）任务上表现优异。

Conclusion: UniWeTok成功解决了现有视觉分词器的局限性，提供了一个统一的视觉表示框架，在保持高保真重建的同时支持复杂语义提取和生成任务，为统一多模态大语言模型的发展提供了重要基础。
...
TL;DR: UniRef-Image-Edit：统一单图编辑与多图合成的多模态生成系统，通过SELF统一输入表示和两阶段训练框架提升跨参考一致性


<details>
  <summary>Details</summary>
Motivation: 现有基于扩散的编辑方法在处理多个条件时，由于参考输入之间交互有限，难以保持一致性。需要统一框架同时处理单图编辑和多图合成任务。

Method: 1. 提出SELF（Sequence-Extended Latent Fusion）统一输入表示，将多参考图动态序列化为连贯的潜在序列；2. 两阶段训练：SFT阶段联合训练单图编辑和多图合成任务，采用渐进序列长度训练策略；3. RL阶段引入MSGRPO强化学习框架优化多参考图像生成的一致性。

Result: 构建了高性能多模态生成系统，显著提升了跨参考图像的一致性，能够处理复杂的多图像合成任务。将开源代码、模型、训练数据和奖励数据。

Conclusion: UniRef-Image-Edit通过统一的输入表示和两阶段训练框架，成功解决了多参考图像生成中的一致性难题，为多模态图像编辑提供了有效的解决方案。
...
TL;DR: GeoEyes是一个针对超高分辨率遥感VQA的MLLM训练框架，通过分阶段训练解决工具使用同质化问题，实现按需缩放并提升性能
...
Method: 提出GeoEyes分阶段训练框架：1) 冷启动SFT数据集UHR-CoZ，覆盖多样化缩放模式；2) 自适应缩放GRPO强化学习方法，显式奖励缩放交互中的证据增益和答案改进

Result: 模型学会按需缩放和适当停止行为，在UHR遥感基准测试中取得显著改进，在XLRS-Bench上达到54.23%准确率

Conclusion: GeoEyes通过分阶段训练有效解决了MLLM在超高分辨率遥感VQA中的工具使用同质化问题，实现了更智能的缩放策略
...
TL;DR: HiVid：首个利用LLM作为可扩展人类代理生成视频流媒体内容感知权重的框架，显著提升VOD和直播的QoE预测准确性


<details>
  <summary>Details</summary>
Motivation: 内容感知流媒体需要动态的块级重要性权重来优化主观体验质量(QoE)，但直接人工标注成本过高，而视觉显著性模型泛化能力差。需要一种可扩展且高保真的权重生成方法。

Method: 提出HiVid框架，利用LLM作为人类代理生成权重。包含三个核心模块：(1)感知模块：在局部上下文窗口中评估帧，自回归构建视频连贯理解；(2)排序模块：针对VOD的局部窗口评分不一致问题，提出LLM引导的归并排序算法进行全局重排序；(3)预测模块：针对直播的低延迟需求，使用多模态时间序列模型预测未来权重，包含内容感知注意力和自适应时域机制。

Result: 实验显示HiVid在VOD上的权重预测准确率比SOTA基线提升11.5%，在直播上提升26%。真实用户研究验证HiVid将流媒体QoE相关性提升14.7%。

Conclusion: HiVid成功利用LLM作为可扩展的人类代理，为内容感知流媒体生成高质量的动态权重，显著提升了VOD和直播场景的QoE预测性能。
...
TL;DR: 提出首个利用双像素传感器进行单图像栅栏移除的框架Freq-DP Net，通过融合几何先验和结构先验实现精确栅栏分割，并建立了多样化基准数据集。


<details>
  <summary>Details</summary>
Motivation: 单图像栅栏遮挡移除是挑战性任务，现有方法在静态场景中效果不佳或需要多帧运动线索。双像素传感器提供额外的深度/几何信息，为解决此问题提供了新途径。

Method: 提出Freq-DP Net双分支网络：1) 几何分支利用显式代价体积建模离焦视差；2) 结构分支通过快速傅里叶卷积学习栅栏全局模式先验；3) 注意力机制智能融合两种线索。

Result: 方法显著优于强通用基线，在单图像、基于双像素的栅栏移除任务上建立了新的最先进水平。建立了包含不同栅栏类型的多样化基准数据集。

Conclusion: 首次利用双像素传感器解决单图像栅栏移除问题，通过融合几何和结构先验实现了精确的栅栏分割和移除，为相关应用提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有3D形状数据集主要关注几何信息而忽略了拓扑结构，这限制了统一几何-拓扑学习的发展。需要建立包含拓扑特征的基准数据集，并开发能够自动选择重要拓扑特征的深度学习方法。

Method: 1) 创建拓扑增强数据集：为ModelNet40和ShapeNet中的每个点云计算持久同调特征；2) 提出TopoGAT方法：基于深度学习的显著持久点选择方法，直接从输入数据和拓扑特征中学习识别最重要的拓扑特征，避免手工统计选择标准的局限性。

Result: 1) 建立了首个拓扑增强的3D点云基准数据集；2) TopoGAT方法在稳定性和判别能力上优于传统统计方法；3) 将选择的显著持久点集成到标准点云分类和分割流程中，提高了分类准确率和分割指标。

Conclusion: 拓扑增强数据集和可学习的特征选择方法为将持久同调更广泛地集成到3D点云分析的深度学习工作流程中奠定了基础，实现了几何和拓扑信息的统一学习。
...
TL;DR: Sali-Cache：一种先验优化框架，通过双信号自适应缓存解决VLM处理长视频时的内存瓶颈问题，在保持100%准确率的同时实现2.20倍内存压缩。


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型在处理长视频内容时面临关键内存瓶颈，KV缓存随序列长度线性增长。现有解决方案主要采用反应性淘汰策略，在计算完整注意力矩阵后丢弃token，造成大量计算浪费。

Method: 提出Sali-Cache框架，通过先验内存管理实现双信号自适应缓存：1) 基于光流分析的时间滤波器检测帧间冗余；2) 基于显著性检测的空间滤波器识别视觉显著区域。在进入计算昂贵的注意力操作前智能管理内存分配。

Result: 在LLaVA 1.6架构上的实验评估显示：1) 有效内存使用达到2.20倍压缩比；2) 在BLEU、ROUGE-L和Exact Match指标上保持100%准确率；3) 在相同内存预算下，能长时间保持上下文丰富特征而不降低模型性能。

Conclusion: Sali-Cache能够高效处理长视频内容，使视觉语言模型能够在消费级硬件上运行，解决了内存瓶颈问题，同时保持了模型性能。
...
TL;DR: AbracADDbra是一个通过触摸先验空间定位简洁指令的交互式图像编辑框架，包含触摸引导放置和扩散生成两个阶段，在Touch2Add基准上表现出色


<details>
  <summary>Details</summary>
Motivation: 解决基于指令的对象添加中文本提示的模糊性和基于掩码输入的繁琐性问题，提供更直观的用户交互体验

Method: 采用解耦架构：1) 视觉语言Transformer用于触摸引导的对象放置；2) 扩散模型联合生成对象和实例掩码以实现高保真融合

Result: 放置模型显著优于随机放置和通用VLM基线，在Touch2Add基准上表现出色，初始放置准确性与最终编辑质量强相关

Conclusion: 该框架为更易用、高效的创意工具铺平了道路，验证了解耦方法的有效性
...
TL;DR: ScreenParse：大规模屏幕解析数据集，包含77.1万网页截图和2100万个UI元素标注，用于训练高效屏幕理解模型


<details>
  <summary>Details</summary>
Motivation: 现有屏幕理解数据集标注稀疏，仅标注任务相关元素，覆盖率和泛化能力有限；同时实际部署需要低延迟的本地化使用

Method: 1. 开发Webshot自动化流水线生成ScreenParse数据集；2. 训练ScreenVLM模型（3.16亿参数），使用ScreenTag标记表示和结构感知损失函数

Result: ScreenVLM在密集解析任务上大幅超越更大的基础VLM（PageIoU 0.592 vs 0.294），在公开基准上表现优异；在ScreenParse上微调基础VLM能持续提升其定位性能

Conclusion: 密集屏幕监督能为UI理解提供可迁移的结构先验知识，ScreenParse数据集和ScreenVLM模型为高效屏幕解析提供了有效解决方案
...
TL;DR: 论文提出了一种结合光度误差和重投影误差优点的第三种方法：使用密集几何特征描述子替代光度误差，但实验表明该方法在姿态优化中并未超越基于重投影误差的方法。


<details>
  <summary>Details</summary>
Motivation: 计算机视觉中两帧相对姿态优化问题通常使用光度误差或重投影误差，两者各有优劣（精度、鲁棒性、闭环可能性）。作者希望结合两种范式的优势，提出统一方法。

Method: 使用密集采样的几何特征描述子，将光度误差替换为描述子残差。这样既能利用微分光度方法的亚像素精度，又能利用几何特征描述子的表达能力。

Result: 实验表明，虽然提出的策略是有趣的方法并能实现准确跟踪，但最终并未超越基于重投影误差的姿态优化策略，尽管利用了更多信息。

Conclusion: 分析发现描述子相似度度量变化过于缓慢，不一定严格对应关键点定位精度，这可能是该方法未能超越传统方法的原因。
...
TL;DR: 本文提出使用生成式AI（基于LoRA微调的Stable Diffusion）生成深色皮肤皮肤镜图像，以解决皮肤癌检测中肤色不平衡问题，提升模型公平性和性能。


<details>
  <summary>Details</summary>
Motivation: 当前AI皮肤癌诊断工具主要基于浅色皮肤图像训练（如ISIC数据集中浅色皮肤占70%以上，深色皮肤不足8%），导致对深色皮肤人群的诊断准确性和公平性下降，阻碍了医疗公平性。

Method: 使用LoRA（低秩适应）方法微调预训练的Stable Diffusion模型，在ISIC数据集的深色皮肤子集上训练，生成以病变类型和肤色为条件的合成皮肤镜图像，用于数据增强。

Result: 在分割任务中，使用增强数据集训练的模型在IoU、Dice系数和边界精度上均有提升；在分类任务中，EfficientNet-B0模型达到92.14%的准确率，验证了生成数据的有效性。
...
TL;DR: 本文提出一种基于RGB手部图像的类风湿关节炎炎症检测框架，通过自监督预训练和失衡感知训练解决医学图像中的样本稀缺和数据不平衡问题。


<details>
  <summary>Details</summary>
Motivation: 类风湿关节炎需要早期诊断和密切随访，但患者获得专科治疗往往耗时较长。因此需要开发能够通过家庭拍摄的RGB图像轻松检测关节炎症的系统。现有工作尚未明确解决RGB图像中炎症检测面临的样本稀缺、数据不平衡等挑战。

Method: 构建专用数据集定量展示视觉检测炎症的难度，提出包含全局局部编码器的炎症检测框架，结合大规模健康手部图像的自监督预训练和失衡感知训练来检测类风湿关节炎相关的关节炎症。

Result: 实验表明，与基线模型相比，所提方法将F1分数提高了0.2分，Gmean提高了0.25分。

Conclusion: 该研究证明了通过RGB手部图像检测类风湿关节炎炎症的可行性，提出的框架通过自监督预训练和失衡感知训练有效解决了医学图像分析中的常见挑战，为家庭监测提供了潜在解决方案。
...
TL;DR: 提出事件-帧融合框架，利用事件提供时间密集运动线索，帧提供空间密集精确估计，通过仿射不变单纯形框架和邻域贪婪优化策略，在低存储和处理资源下实现高精度变形测量。


<details>
  <summary>Details</summary>
Motivation: 传统基于图像的变形测量方法依赖帧间微小运动，限制了在高度动态场景中的应用，或需要高速相机导致存储和计算成本过高。需要一种既能处理快速变形又资源高效的方法。

Method: 提出事件-帧融合框架：事件提供时间密集运动线索，帧提供空间密集精确估计。引入仿射不变单纯形(AIS)框架，将变形场划分为线性化子区域以减少运动模糊。采用邻域贪婪优化策略，让收敛良好的子区域指导收敛差的邻居，减少长期跟踪中的误差累积。

Result: 建立了包含120多个序列的基准数据集。实验结果显示，该方法在生存率上比现有最佳基线提高1.6%，同时仅使用高速视频方法18.9%的数据存储和处理资源。

Conclusion: 事件-帧融合框架结合AIS和邻域贪婪优化策略，能够在低资源消耗下实现高精度的密集变形测量，特别适用于高度动态场景。
...
TL;DR: 将VACE视频生成模型适配为实时自回归版本，通过将参考帧移至并行条件通路，保持固定块大小和KV缓存，实现流式处理


<details>
  <summary>Details</summary>
Motivation: 原始VACE模型使用双向注意力处理完整序列，不适合需要固定块大小和因果注意力的流式处理管道。需要将VACE的能力（参考引导、结构条件、修复、时间扩展）适配到实时自回归视频生成场景

Method: 关键修改：将参考帧从扩散潜在空间移至并行条件通路，保持固定块大小和KV缓存。重用现有预训练VACE权重，无需额外训练。支持1.3B和14B模型规模

Result: VACE为结构控制和修复增加20-30%的延迟开销，VRAM成本相对于基础模型可忽略。但由于因果注意力限制，参考到视频的保真度相比批处理VACE严重下降

Conclusion: 成功将VACE适配到实时自回归视频生成，实现了流式处理能力，但在参考保真度上做出了权衡。提供了开源实现
...
<details>
  <summary>Details</summary>
Motivation: 现有的多轮越狱攻击在扩展到视觉语言模型时效果不佳，因为过于恶意的视觉输入容易触发安全机制，导致模型响应更加保守。

Method: 提出MAPA方法：1) 在每一轮中交替使用文本和视觉攻击动作以获取最恶意的响应；2) 跨轮次通过迭代来回优化调整攻击轨迹，逐步放大响应的恶意程度。

Result: MAPA在多个最新模型上显著优于现有方法，攻击成功率提高了11-35%，包括LLaVA-V1.6-Mistral-7B、Qwen2.5-VL-7B-Instruct、Llama-3.2-Vision-11B-Instruct和GPT-4o-mini。

Conclusion: MAPA通过两层级设计有效规避了视觉语言模型的安全防御机制，为多模态模型的安全性评估提供了新的攻击基准。
...
<details>
  <summary>Details</summary>
Motivation: 视觉语言导航需要大量来自私有室内环境的轨迹指令数据，存在隐私问题。联邦学习虽能保护隐私，但传统FL在VLN的极端跨客户端异构性（环境和指令风格差异）下表现不佳，单一全局模型不适用。
...
TL;DR: 提出基于特征重校准的嗅觉-视觉多模态模型，用于细粒度大米变质检测，通过FDEC重构标记多模态嵌入特征数据集，FDRA-Net强调信号变化，实现99.89%分类准确率


<details>
  <summary>Details</summary>
Motivation: 现有大米变质检测多模态方法在表示和提取细粒度异常特征方面能力有限，且依赖高成本设备（如高光谱相机、质谱仪），增加检测成本并延长数据采集时间

Method: 提出特征重校准的嗅觉-视觉多模态模型：1）细粒度变质嵌入构造器（FDEC）重构标记多模态嵌入特征数据集，增强样本表示；2）细粒度变质重校准注意力网络（FDRA-Net）强调信号变化，增加对大米表面细粒度变质的敏感性

Result: 实验显示该方法达到99.89%的分类准确率，相比最先进方法提高了检测准确率并简化了流程，现场检测展示了准确性和操作简便性的优势

Conclusion: 该方法能有效解决大米变质检测中细粒度特征提取和设备依赖问题，可扩展到农业和食品工业中的其他农产品检测
...
TL;DR: 该研究探讨了学习组件在空间感知中的角色，发现几何算法作为后端决策者比直接使用学习预测更可靠，特别是在几何对齐后结合几何处理阶段能提升性能。


<details>
  <summary>Details</summary>
Motivation: 传统空间感知依赖几何建模，而学习组件逐渐被用于增强几何系统。但学习组件应直接替代几何估计还是作为中间模块仍不明确，需要研究有效的端到端模块化框架。

Method: 提出端到端模块化框架：学习组件提出几何假设（相机位姿和深度），几何算法（点对面RGB-D ICP）作为后端进行最终估计决策。在TUM RGB-D基准上评估VGGT模型在不同运动幅度和场景动态下的表现。

Result: 三个一致发现：1) 学习提出的位姿单独不可靠；2) 学习提出的几何若未与相机内参对齐会降低性能；3) 当学习提出的深度经过几何对齐并由几何处理阶段处理时，在中等挑战的刚性场景中能获得稳定改进。

Conclusion: 几何不仅是细化组件，更是验证和吸收学习几何观测的仲裁者。研究强调了模块化、几何感知系统设计对鲁棒空间感知的重要性。
...
TL;DR: XR系统中VIO在传感器退化条件下的实证研究：视觉退化导致厘米级误差，惯性退化可导致数百至数千米轨迹偏差


<details>
  <summary>Details</summary>
Motivation: 工业XR系统依赖VIO进行姿态跟踪，但实际环境常偏离理想传感假设。现有评估多关注正常传感器行为，对持续传感器退化在操作条件下的影响理解不足。

Method: 通过受控实证研究，系统性地注入影响视觉和惯性模态的故障，在不同操作状态下定量评估VIO行为。

Result: 发现故障影响存在显著不对称性：视觉传感退化通常导致厘米级有界姿态误差，而惯性传感退化可引发大得多的轨迹偏差，某些情况下可达数百至数千米。

Conclusion: 这些发现表明，在工业XR系统的评估和设计中，应更加重视惯性传感的可靠性。
...
<details>
  <summary>Details</summary>
Motivation: 面部动作单元检测面临的主要挑战是在有限标注数据条件下有效学习具有区分性和泛化能力的AU表示。现有方法在利用多模态信息方面存在不足。

Method: 提出HiVA方法：1) 使用大语言模型生成多样化的AU文本描述；2) 引入AU感知动态图模块学习AU特定视觉表示；3) 设计分层跨模态注意力架构，包含解耦双交叉注意力（细粒度AU特定交互）和上下文双交叉注意力（全局AU依赖关系建模）。

Result: 大量实验表明HiVA持续超越最先进方法。定性分析显示HiVA产生语义上有意义的激活模式，证明其学习鲁棒且可解释的跨模态对应关系的能力。

Conclusion: HiVA通过分层视觉-语言交互，利用文本AU描述作为语义先验，有效增强了AU检测能力，在有限标注数据下实现了鲁棒且可解释的面部行为分析。
...
TL;DR: D-SECURE是一个结合内部篡改检测和外部证据推理的多模态虚假信息检测框架，通过整合HAMMER篡改检测器和DEFAME检索流程来应对新闻式帖子的验证挑战。


<details>
  <summary>Details</summary>
Motivation: 当前多模态虚假信息将逼真的图像编辑与流畅但误导性的文本结合，现有系统通常依赖单一证据源：基于内容的检测器只能识别图像和标题的内部不一致性，而基于检索的事实核查器将输入视为粗略声明，往往忽略细微的视觉或文本篡改。这种分离导致内部一致但虚假的内容能绕过篡改检测器，而事实核查器可能验证包含像素级或令牌级损坏的声明。

Method: D-SECURE框架整合了HAMMER篡改检测器和DEFAME检索流程。DEFAME执行广泛的验证，而HAMMER分析剩余或不确定的案例，这些案例可能包含细粒度编辑。该框架结合内部篡改检测与外部证据推理，为新闻式帖子提供统一解决方案。

Result: 在DGM4和ClaimReview样本上的实验突出了两个系统的互补优势，并证明了它们融合的合理性。该框架能够提供统一的、可解释的报告，结合了篡改线索和外部证据。

Conclusion: D-SECURE通过结合内部篡改检测和外部证据推理，有效解决了多模态虚假信息检测中的关键挑战，为新闻式帖子提供了更全面、可靠的验证框架。
...
TL;DR: 该论文提出了一种基于矢量图形的分层可控图像生成方法，允许用户在元素级别上精确控制图像内容，如调整形状、颜色和对象操作。


<details>
  <summary>Details</summary>
Motivation: 当前图像生成技术虽然视觉质量显著，但缺乏元素级别的精细控制能力。用户无法直观地修改图像中的形状、颜色或添加/移除对象，这限制了图像生成的实际应用价值。
...
Conclusion: 通过矢量图形表示和引导的合成框架，实现了元素级别的图像生成控制，为可控图像生成开辟了新方向，具有广泛的应用潜力。
...
Method: 1. 像素级语义对应模块：挖掘中间扩散特征构建内容与风格图像之间的密集对齐映射
2. 循环一致性模块：在迭代中强制执行结构和感知对齐
3. 基于预训练潜在扩散模型，无需额外训练或监督

Result: CoCoDiff在视觉质量和定量结果上达到最先进水平，超越了依赖额外训练或标注的方法，同时保持了对象和区域级别的风格化，保留了几何结构和细节。

Conclusion: CoCoDiff证明了在生成扩散模型中挖掘对应线索的有效性，为无需训练、低成本的细粒度风格迁移提供了新思路，在保持语义一致性的同时实现了高质量的视觉风格化。
...
TL;DR: TikArt是一个基于多模态大语言模型的细粒度视觉推理代理，通过"思考-孔径-观察"循环，使用缩放和分割孔径动作进行多步区域推理，结合强化学习优化策略，在多个细粒度视觉任务上取得显著提升。


<details>
  <summary>Details</summary>
Motivation: 解决多模态大语言模型在细粒度视觉推理中的局限性，当关键证据存在于微小物体、杂乱区域或细微标记时，单一的全局图像编码会导致信息丢失。

Method: 提出TikArt代理，采用"思考-孔径-观察"循环：1) Zoom提取矩形裁剪区域，2) Segment调用SAM2获取基于掩码的不规则目标裁剪。每次动作后生成显式观察，将局部视觉线索转化为持久语言记忆。基于Qwen3-VL-8B构建，使用AGRPO强化学习算法优化推理策略，采用两阶段课程学习：先预热分割动作，然后联合优化视觉数学、细粒度VQA和分割任务。

Result: 在V*、HR-Bench-4K/8K、MME-RealWorld-Lite、MMStar、RefCOCO和ReasonSeg等多个基准测试中，相比骨干模型取得一致性能提升，并为高分辨率推理提供可解释的孔径轨迹。

Conclusion: TikArt通过孔径引导的多步视觉语言推理，有效解决了细粒度视觉推理中的信息丢失问题，结合强化学习优化的孔径动作策略，在多个细粒度视觉任务上表现出色，提供了可解释的推理过程。
...
TL;DR: 提出Gaussian Mesh Renderer (GMR)，将3DGS的高效光栅化过程与三角网格表示相结合，实现轻量级可微分网格渲染器


<details>
  <summary>Details</summary>
Motivation: 三角网格模型在表面重建中仍然流行，但传统基于网格的可微分渲染器存在优化缓慢或计算负担重的问题。3DGS虽然实现了快速渲染和优化，但需要新的方法将高斯表示与网格表示紧密结合

Method: 提出Gaussian Mesh Renderer (GMR)，从对应的网格三角形中解析推导出每个高斯基元，保持结构保真度并实现梯度流动。利用3DGS的高效光栅化过程，实现轻量级可微分网格渲染

Result: 相比传统网格渲染器，GMR实现了更平滑的梯度，特别有助于在有限内存下使用小批量进行更好的优化。方法已在GitHub上开源

Conclusion: GMR成功将3DGS的高效渲染能力与三角网格表示相结合，为表面重建提供了一种轻量级、可微分的渲染解决方案，解决了传统方法的优化效率问题
...
<details>
  <summary>Details</summary>
Motivation: 在医学诊断中，图像质量不佳或临床情况复杂时，单一模态信息可能不足。需要结合影像和文本信息，并考虑预测不确定性来提高分割的可靠性和准确性。

Method: 提出模态解码注意力块（MoDAB）结合轻量级状态空间混合器（SSMix）实现高效跨模态融合和长程依赖建模；设计谱熵不确定性（SEU）损失函数，联合捕捉空间重叠、谱一致性和预测不确定性。

Result: 在QATA-COVID19、MosMed++和Kvasir-SEG等多个公开医学数据集上验证，方法在分割性能上优于现有SOTA方法，同时计算效率显著更高。

Conclusion: 不确定性建模和结构化模态对齐在视觉-语言医学分割任务中至关重要，提出的框架在复杂临床情况下能提高模型可靠性。
...
Conclusion: PID-LRSC框架有效解决了病理图像分析中的实例-语义纠缠问题，提升了模型性能和诊断可靠性，为病理辅助诊断提供了更清晰、更可靠的解决方案。
...
TL;DR: 提出一种端到端的多示例学习框架，结合Grassmann重嵌入和流形自适应聚类，用于全切片图像分析，提升分级准确性和可解释性。


<details>
  <summary>Details</summary>
Motivation: 当前主流的两步框架使用离线特征编码器，缺乏领域特定知识。基于注意力的MIL方法结果导向且可解释性有限，聚类方法可提供可解释决策但面临高维特征和语义模糊质心的问题。

Method: 提出端到端MIL框架，集成Grassmann重嵌入和流形自适应聚类，利用流形几何结构实现鲁棒聚类。设计先验知识引导的代理实例标记和聚合策略，近似补丁标签并聚焦病理相关肿瘤区域。

Result: 在多中心WSI数据集上的实验表明：1) 聚类整合模型在分级准确性和可解释性方面均取得优越性能；2) 端到端学习能优化特征表示，且计算资源需求可接受。

Conclusion: 该端到端框架通过流形几何结构和先验知识引导，在保持可解释性的同时提升了全切片图像分析的性能，为病理诊断提供了更有效的解决方案。
...
Method: 提出MedVAR自回归基础模型，采用多尺度预测范式，从粗到细生成图像，产生结构化多尺度表示。构建包含44万CT/MRI图像的数据集，涵盖六个解剖区域。

Result: MedVAR在保真度、多样性和可扩展性方面达到最先进的生成性能，为未来医学生成基础模型提供了有前景的架构方向。

Conclusion: MedVAR是首个自回归医学图像生成基础模型，通过多尺度预测和大型数据集实现了高效、可扩展的医学图像合成，具有重要的应用价值。
...
<details>
  <summary>Details</summary>
Motivation: 现有结构保持方法存在效率低下问题，适配器参数有时与基础架构相当，且适配器不了解输入提示，仅针对结构输入优化。

Method: 提出两种高效适配器：Nexus Prime和Nexus Slim，通过交叉注意力机制实现丰富的多模态条件控制，使适配器能同时理解输入提示和保持结构。
...
TL;DR: 该论文系统评估了79个深度学习模型在龙卷风建筑损毁评估任务上的表现，发现优化器选择和低学习率比架构选择更重要，ConvNeXt-Base模型在优化设置下表现最佳。
...
Method: 提出了系统实验框架，评估79个开源深度学习模型（CNN和Vision Transformer），在自建的QSTD基准数据集上进行2300多次控制实验，重点研究架构与优化的交互作用。

Result: 优化器选择比架构更重要：从Adam切换到SGD使Vision Transformer和Swin Transformer的F1分数提升25-38点；低学习率(1e-4)使平均F1提升10.2点；ConvNeXt-Base在优化设置下表现最佳，在跨事件测试中达到46.4% Macro F1。

Conclusion: 实现操作级性能的关键在于架构与优化的复杂交互，而非单纯架构选择。优化器选择和低学习率是提升龙卷风损毁评估模型性能的决定性因素。
...
TL;DR: 比较TrOCR和Qwen在18世纪印刷文本OCR中的表现，发现虽然Qwen整体准确率更高，但会悄悄修改历史文本形式，而TrOCR更忠实于原始拼写但错误传播更严重


<details>
  <summary>Details</summary>
Motivation: 18世纪印刷文本的OCR仍然具有挑战性，虽然基于Transformer的OCR系统和视觉语言模型在整体准确率上表现良好，但CER和WER等指标无法充分评估其在学术应用中的可靠性

Method: 比较专门的OCR Transformer（TrOCR）和通用视觉语言模型（Qwen）在线级历史英文文本上的表现，使用长度加权准确度指标和基于假设的错误分析

Result: Qwen获得更低的CER/WER，对退化输入更具鲁棒性，但会选择性进行语言正则化和拼写规范化，可能悄悄改变具有历史意义的形式；TrOCR更一致地保持拼写保真度，但更容易出现级联错误传播
...
Result: 在六个公开LiDAR数据集上的实验首次系统评估了跨视角域泛化，CVGC在从单一源域泛化到多个异构采集视角目标域时持续优于现有方法

Conclusion: CVGC有效解决了跨视角LiDAR语义分割的域泛化挑战，为实际LiDAR应用提供了更可靠的跨视角泛化能力
...
TL;DR: MoRL是一个统一的多模态运动模型，通过监督微调和强化学习训练，结合任务特定奖励设计和测试时推理方法Chain-of-Motion，显著提升了运动理解和生成的能力。
...
Method: 提出MoRL统一多模态运动模型，采用监督微调和强化学习训练，设计任务特定奖励（语义对齐、推理连贯性、物理合理性、文本-运动一致性），并引入Chain-of-Motion测试时推理方法，构建了两个大规模CoT数据集。

Result: 在HumanML3D和KIT-ML数据集上的实验表明，MoRL相比现有最先进基线取得了显著提升。

Conclusion: MoRL通过结合监督学习、强化学习和测试时推理，有效提升了运动理解和生成的逻辑推理能力和感知真实性，为多模态运动建模提供了新方法。
...
TL;DR: OmniVTON++是一个无需训练的通用虚拟试穿框架，通过结构化服装变形、主体姿态引导和连续边界缝合技术，实现跨数据集、跨服装类型的统一解决方案。


<details>
  <summary>Details</summary>
Motivation: 现有虚拟试穿方法通常针对特定数据条件优化，部署时需要重新训练，限制了其作为统一解决方案的泛化能力。需要一种无需训练、具有通用适用性的框架。

Method: 提出三个核心技术：1) 结构化服装变形实现对应驱动的服装适配；2) 主体姿态引导在扩散采样中逐步调节结构；3) 连续边界缝合进行边界感知的细化。这些技术形成无需任务特定训练的完整流程。

Result: 在多样化的泛化设置中（包括跨数据集和跨服装类型评估）实现了最先进的性能，可靠地在不同场景和扩散骨干网络中运行。支持单/多服装、单/多人以及动漫角色的虚拟试穿。
...
<details>
  <summary>Details</summary>
Motivation: 当前自动驾驶的VLA模型存在两种主要范式：扩散式规划器存在模态对齐困难、训练效率低、泛化有限的问题；基于token的规划器则存在累积因果错误和不可逆解码的问题。这两种范式具有互补的优缺点，需要一种结合两者优势的解决方案。

Method: 提出了DriveFine模型，采用掩码扩散VLA架构。核心创新是block-MoE设计，在生成专家之上无缝注入细化专家，通过推理时的显式专家选择和训练时的梯度阻断，实现两个专家的完全解耦。同时设计了混合强化学习策略，鼓励细化专家的有效探索同时保持训练稳定性。

Result: 在NAVSIM v1、v2和Navhard基准测试上的广泛实验表明，DriveFine展现出强大的有效性和鲁棒性，解决了现有方法的局限性。

Conclusion: DriveFine通过创新的block-MoE架构成功结合了扩散式和token式规划器的优势，实现了灵活解码与自校正能力的统一，为自动驾驶VLA模型提供了更有效的解决方案。
...
TL;DR: YOLO26是最新YOLO版本，通过消除DFL、实现端到端NMS-Free推理、引入ProgLoss+STAL标签分配、使用MuSGD优化器等改进，在CPU模式下推理速度提升43%，可在边缘设备实现实时性能，并在实例分割、姿态估计、OBB解码等任务上有改进。


<details>
  <summary>Details</summary>
Motivation: YOLO作为计算机视觉领域的领先模型已有十年历史，本研究旨在深入探索最新版本YOLO26的创新特性，提供比现有技术文档更深入的理解，特别是通过源代码分析揭示其真实工作机制，为研究人员和开发者提供精确的架构理解。

Method: 通过严格的架构调查，主要使用GitHub仓库中的源代码和官方文档进行分析，提取源代码中鲜为人知的真实操作机制，首次呈现基于CNN的YOLO26核心架构图。

Result: YOLO26在CPU模式下实现43%的推理速度提升，可在无GPU的边缘设备上实现实时性能，在实例分割、姿态估计、定向边界框解码等多个计算机视觉任务上都有改进，并提供了详细的架构图。

Conclusion: 本研究首次呈现了YOLO26的CNN架构，提供了精确的架构理解，有助于研究人员和开发者进一步改进YOLO模型，确保其在计算机视觉深度学习领域保持领先地位。
...
TL;DR: VariViT：一种改进的Vision Transformer，能够处理可变尺寸的医学图像，通过新的位置嵌入调整方案和批处理策略，在脑肿瘤分类任务中优于传统ViT和ResNet。


<details>
  <summary>Details</summary>
Motivation: 传统ViT需要固定尺寸的图像补丁，这在医学图像处理中存在问题：不规则形状的肿瘤结构需要裁剪，导致前景背景比例变化大；图像缩放会损失信息并引入伪影；大图像计算成本高，小图像信息丢失，存在计算精度权衡。

Method: 提出VariViT模型，能够处理可变尺寸图像同时保持固定补丁大小。采用新的位置嵌入调整方案以适应可变数量的补丁，并实现新的批处理策略以降低计算复杂度。

Result: 在两个3D脑MRI数据集上的评估显示，VariViT在胶质瘤基因型预测和脑肿瘤分类任务中优于传统ViT和ResNet，分别获得75.5%和76.3%的F1分数。提出的批处理策略相比传统架构减少高达30%的计算时间。

Conclusion: VariViT在图像表示学习中表现出高效性，特别适用于医学图像处理，能够处理可变尺寸图像同时保持计算效率，为医学图像分析提供了更好的解决方案。
...
Conclusion: VIGIL填补了该领域空白，提供了首个针对图像重语境化任务中幻觉的细粒度分类和分解方法，并开源了数据集、检测流程和基准代码。
...
TL;DR: 该论文提出了一种从自由手绘草图生成图像的新方法，通过调制机制优先考虑草图的语义解释而非严格的边缘对齐，解决了自由手绘草图缺乏像素对齐真实图像的问题。


<details>
  <summary>Details</summary>
Motivation: 虽然生成式AI在多种条件输入下取得了进展，但现有研究主要关注边缘图（常被误称为"草图"），而真正处理自由手绘草图的算法仍未被充分探索。自由手绘草图具有固有的抽象性和变形，缺乏像素对齐的真实图像作为监督信号，这给生成既逼真又忠实于草图内容的图像带来了挑战。

Method: 提出了一种基于调制的方法，优先考虑草图的语义解释而非严格的边缘位置对齐。引入了一种新颖的损失函数，使得模型能够在不需要像素对齐真实图像的情况下，直接在自由手绘草图上进行训练。

Result: 该方法在语义对齐、生成图像的逼真度和整体质量方面均优于现有方法，能够更好地处理自由手绘草图的抽象性和变形。

Conclusion: 通过优先语义解释而非像素级对齐，该方法成功解决了自由手绘草图生成图像的核心挑战，为基于草图的图像生成提供了更有效的解决方案。
...
TL;DR: 该论文是第一篇关于3D视觉中全局求解器的系统性综述，提出了分支定界、凸松弛和渐进非凸性三大核心范式的分类体系，分析了它们在10个核心视觉任务中的最优性-鲁棒性-可扩展性权衡，并指出了未来研究方向。


<details>
  <summary>Details</summary>
Motivation: 传统3D视觉中的几何优化问题通常使用局部或启发式方法处理，这些方法无法提供可证明的全局最优解。全局求解器作为一种新兴范式，能够为这些非凸几何优化问题提供可证明的解决方案，但该领域缺乏系统性综述来统一理论框架和实践进展。

Method: 论文提出了一个全面的分类体系，将全局求解器分为三大核心范式：分支定界（BnB）、凸松弛（CR）和渐进非凸性（GNC）。分析了每种范式的理论基础、算法设计和实际增强技术，并考察了它们在10个核心3D视觉任务中的应用。

Result: 通过系统分析揭示了全局求解器在不同视觉任务中的最优性-鲁棒性-可扩展性权衡关系，为算法选择提供了指导。建立了统一的视角来理解全局求解器在几何视觉中的应用，并指出了该领域的关键未来方向。

Conclusion: 该综述为3D视觉中的全局求解器研究提供了统一的理论框架和实践路线图，强调了可证明的、可信赖的感知系统对于现实世界应用的重要性，特别是在安全关键部署中。通过整合理论基础、实践进展和更广泛影响，推动了可证明的、可信赖的感知系统的发展。
...
TL;DR: MeFEm是基于改进JEPA架构的视觉模型，用于面部图像的生物特征和医学分析，通过特殊掩码策略和损失加权，在人体测量任务上优于现有基线，并提供BMI估计能力。


<details>
  <summary>Details</summary>
Motivation: 现有面部生物特征分析模型存在数据需求大、领域偏差问题，需要更高效且能处理医学相关任务（如BMI估计）的模型。

Method: 基于改进的联合嵌入预测架构(JEPA)，采用轴向条纹掩码策略聚焦语义相关区域，使用循环损失加权方案，以及CLS令牌的概率重分配以提升线性探测质量。

Result: 在整合的精选数据集上训练，MeFEm在核心人体测量任务上优于FaRL和Franca等强基线，且数据使用量显著更少；在BMI估计上也展现出有希望的结果。

Conclusion: MeFEm为面部生物特征和医学分析提供了强大的基线模型，模型权重已公开，可促进该领域未来研究。
...
TL;DR: 首个通用图像免疫框架，通过单一对抗扰动保护图像免遭基于扩散模型的恶意编辑，无需针对每张图像单独优化。


<details>
  <summary>Details</summary>
Motivation: 扩散模型虽然带来了强大的图像编辑能力，但也引发了深度伪造和版权侵犯等伦理法律风险。现有图像免疫方法需要为每张图像单独优化对抗扰动，缺乏可扩展性和实用性。

Method: 提出首个通用图像免疫框架，受目标攻击中通用对抗扰动（UAP）技术启发，生成单一、广泛适用的对抗扰动。该扰动将语义目标嵌入待保护图像，同时抑制原始内容，在编辑过程中误导模型注意力。

Result: 在UAP设置下显著优于多个基线方法。即使在更严格的扰动预算下，性能也与图像特定方法相当。同时在不同扩散模型间展现出强大的黑盒可迁移性。

Conclusion: 该方法首次实现了通用图像免疫，无需训练数据或领域知识，在数据缺失环境下仍有效，具有实际应用价值和广泛适用性。
...
TL;DR: 该论文研究了长期运动信息在感知任务中的价值，发现运动表示在理解动作、物体、材料和空间信息方面优于图像，在低数据场景和零样本任务中泛化能力更强，且计算效率更高。


<details>
  <summary>Details</summary>
Motivation: 虽然时间信息被认为对感知至关重要，但现有研究主要关注图像信息，对时间维度（特别是长期运动信息）的作用理解不足。研究者希望探索长期运动信息能为视觉学习提供什么独特价值。

Method: 利用最新的点轨迹估计技术来学习时间表示，并在多种感知任务上进行实验验证。通过比较运动表示与图像表示的性能差异来评估其价值。

Result: 1) 长期运动表示不仅能理解动作，还能更好地理解物体、材料和空间信息；2) 在低数据设置和零样本任务中，运动表示的泛化能力远超图像表示；3) 运动信息的低维度特性使其在计算效率（GFLOPs）和准确性之间达到更好平衡，与视频表示结合可获得更高性能。

Conclusion: 长期运动信息在感知任务中具有独特优势，包括更强的信息表达能力、更好的泛化性能和更高的计算效率。这些发现为未来设计利用长期运动信息的感知模型提供了重要指导。
...
TL;DR: CAPA是一个参数高效的测试时优化框架，通过冻结预训练的3D基础模型，仅更新少量参数（如LoRA或VPT），利用推理时可用的稀疏几何线索进行适配，实现深度补全任务。
...
Method: 冻结3D基础模型主干，仅使用参数高效微调（如LoRA或VPT）更新少量参数。通过推理时可用的稀疏观测直接计算梯度来指导优化。对于视频，引入序列级参数共享，联合适配所有帧以利用时间相关性。

Result: 在室内外数据集上，针对多种条件模式都取得了最先进的结果。能够有效纠正基础模型的几何失真和结构错位问题。

Conclusion: CAPA提供了一种模型无关的测试时优化框架，能够有效将基础模型的几何先验与场景特定测量相结合，在深度补全任务中表现出色，同时保持参数高效性。
...
<details>
  <summary>Details</summary>
Motivation: 传统持续学习需要重复训练，计算成本高，且存在严重的遗忘问题，限制了在实际场景中的应用。需要一种能够避免这些问题的解决方案。

Method: SAILS将类增量语义分割分为两个阶段：1）使用Segment Anything Model进行零样本区域提取；2）在固定特征空间通过原型进行语义关联。采用选择性类内聚类，每个类使用多个原型来建模类内变异性。

Result: SAILS在标准CISS数据集上通常超越现有的基于训练的方法，特别是在长且具有挑战性的任务序列中。完全消除了遗忘，保持任务不变性能，并展现出正向后向迁移（新类引入能提升旧类性能）。

Conclusion: SAILS通过避免参数更新，完全解决了持续学习中的遗忘问题，为实际应用中的类增量语义分割提供了高效且有效的解决方案。
...
TL;DR: GOT-JEPA是一个基于模型预测预训练的通用目标跟踪框架，通过教师-学生架构从干净和遮挡的帧中预测伪跟踪模型，提升泛化能力和遮挡处理。OccuSolver则增强遮挡感知，通过点跟踪器进行细粒度可见性估计和遮挡模式捕捉。


<details>
  <summary>Details</summary>
Motivation: 当前通用目标跟踪器存在两个主要问题：1) 过度优化训练目标，导致在未见场景中泛化能力不足；2) 遮挡推理过于粗糙，缺乏对遮挡模式的详细建模。人类视觉系统能够整合历史信息、适应场景变化并进行细粒度遮挡推理，现有方法在这方面存在明显差距。

Method: 提出GOT-JEPA框架，将JEPA从图像特征预测扩展到跟踪模型预测。使用教师-学生架构：教师从干净当前帧生成伪跟踪模型，学生从遮挡/干扰的当前帧学习预测相同模型。在此基础上，提出OccuSolver增强遮挡感知，采用点中心点跟踪器进行对象感知的可见性估计和细粒度遮挡模式捕捉，基于跟踪器迭代生成的对象先验逐步优化可见性状态。

Result: 在七个基准测试上的广泛评估表明，该方法有效提升了跟踪器的泛化能力和鲁棒性。GOT-JEPA通过稳定伪监督训练预测器在遮挡和干扰下产生可靠跟踪模型，OccuSolver通过渐进式可见性状态优化和遮挡处理，生成更高质量的参考标签，逐步改进后续模型预测。

Conclusion: 该方法通过模型预测预训练框架和增强的遮挡感知机制，成功解决了通用目标跟踪中的泛化不足和遮挡处理粗糙的问题，使跟踪器能够更好地适应动态环境和处理复杂遮挡场景。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法将视觉信息融入语言标记，但需要更有效地利用视觉上下文进行细粒度分割。需要减少高方差跨模态投影并增强语义一致性

Method: 提出视觉信息部分注意力(VIPA)框架，包含视觉表达生成器(VEG)模块，通过局部-全局语言上下文线索检索信息视觉标记，并精炼这些标记以减少噪声和共享视觉属性

Result: 在四个公共RIS基准测试中优于现有最先进方法，实验和视觉分析证明了方法的有效性

Conclusion: VIPA框架通过视觉表达有效利用视觉上下文，增强跨模态对齐，使网络注意力能够鲁棒地对齐细粒度感兴趣区域
...
TL;DR: 论文揭示了视觉识别中眼动扫描路径评估的局限性：数据集中心偏差会混淆标准指标，导致中心注视基线表现优异。作者提出GCS去偏度量，发现只有在中等视野大小下才能同时实现去偏后的扫描路径对齐和人类眼动统计相似性。


<details>
  <summary>Details</summary>
Motivation: 当前任务驱动的硬注意力模型常通过扫描路径与人类注视的匹配度来评估，但标准指标受数据集中心偏差强烈影响，使得中心注视基线表现优异，难以区分真正的行为对齐与中心趋势。

Method: 使用Gaze-CIFAR-10数据集，分析硬注意力分类器在不同视野约束下的表现。提出GCS（注视一致性分数）——一种中心去偏的复合度量，结合了运动相似性。通过扫描不同视野大小和外围上下文，识别扫描路径对齐的最佳条件。

Result: 发现中心注视基线在标准指标下表现惊人地好，接近许多学习策略。只有在中等视野大小下存在一个"外围甜点区"，能同时实现去偏后的扫描路径对齐和人类眼动统计相似性。当视野过大时，模型会进入"捷径机制"。

Conclusion: 标准扫描路径指标在物体中心数据集上过于乐观，需要去偏度量如GCS来评估真正的行为对齐。设计注视基准时需要更好地分离行为对齐与中心偏差，中等视野大小是实现人类眼动统计相似性的关键。
...
<details>
  <summary>Details</summary>
Motivation: 短期对象交互预测对于可穿戴助手理解用户目标和及时提供帮助至关重要，现有方法性能有待提升

Method: 1) 提出STAformer和STAformer++注意力架构，集成帧引导时序池化、双图像-视频注意力和多尺度特征融合；2) 引入环境可供性模型作为交互记忆，以及基于手和对象轨迹的交互热点预测

Result: 在Ego4D数据集上获得+23p.p的总体Top-5 mAP提升，在EPIC-Kitchens STA标签上获得+31p.p提升

Conclusion: 通过注意力架构和人类行为建模显著提升了短期对象交互预测性能，发布了代码、标注和预提取的可供性数据以促进未来研究
...
TL;DR: 提出多维度持久层拉普拉斯框架，通过多尺度拓扑谱表示提升图像分类稳定性


<details>
  <summary>Details</summary>
Motivation: 传统降维方法（如PCA）对降维维度选择敏感，需要更稳定的多维度表示方法

Method: 将图像视为单纯复形，使用持久层拉普拉斯提取多尺度局部拓扑谱表示，跨尺度和维度聚合统计特征

Result: 在COIL20和ETH80数据集上，该方法在宽维度范围内表现更稳定，在中等维度下优于PCA基线

Conclusion: 多维度持久层拉普拉斯框架能有效利用多个降维维度的互补优势，提供更鲁棒的图像表示
...
TL;DR: CT-Bench是一个首创的CT病变分析基准数据集，包含20,335个病变的影像元数据集和2,850个多任务视觉问答对，用于评估AI模型在病变定位、描述、大小估计和属性分类方面的能力。


<details>
  <summary>Details</summary>
Motivation: 当前AI在CT影像病变自动勾画和报告生成方面的进展受到公开CT数据集稀缺的限制，特别是缺乏病变级别的标注数据，这阻碍了相关研究的进展。

Method: 构建CT-Bench数据集，包含两个部分：1）病变影像和元数据集（20,335个病变，包含边界框、描述和大小信息）；2）多任务视觉问答基准（2,850个QA对，涵盖病变定位、描述、大小估计和属性分类），并包含困难负样本以反映真实诊断挑战。

Result: 评估了多种最先进的多模态模型（包括视觉语言模型和医学CLIP变体），与放射科医生评估进行比较，证明了CT-Bench作为全面病变分析基准的价值。在病变影像和元数据集上微调模型在两个组件上都带来了显著的性能提升。

Conclusion: CT-Bench填补了公开CT数据集稀缺的空白，为病变分析提供了全面的基准，展示了其在临床实用性和推动AI在医学影像分析方面进展的重要价值。
...
TL;DR: Wrivinder是一个零样本几何驱动框架，通过聚合多个地面照片重建3D场景并与卫星图像对齐，实现亚30米精度的地理定位，同时发布了MC-Sat数据集用于系统评估。


<details>
  <summary>Details</summary>
Motivation: 地面图像与地理注册卫星地图的对齐对于测绘、导航和态势感知至关重要，但在大视角差异或GPS不可靠时仍然具有挑战性。现有任务缺乏合适的基准测试数据集。
...
TL;DR: AnchorWeave：通过多局部几何记忆而非单一全局3D重建来解决长视频生成中的空间一致性挑战，使用覆盖驱动检索和多锚点编织控制器来协调跨视图不一致性。


<details>
  <summary>Details</summary>
Motivation: 现有基于记忆的视频生成方法通过从重建的3D场景渲染锚点视频来保持空间一致性，但多视图3D重建不可避免地引入跨视图错位（姿态和深度估计误差导致同一表面在不同视图中被重建到略微不同的3D位置），这些不一致性累积成噪声几何，污染条件信号并降低生成质量。

Method: AnchorWeave使用多个干净的局部几何记忆替代单一错位的全局记忆，并学习协调它们的跨视图不一致性。具体包括：1）覆盖驱动的局部记忆检索（与目标轨迹对齐）；2）多锚点编织控制器在生成过程中集成选定的局部记忆。

Result: 大量实验表明AnchorWeave显著改善了长期场景一致性，同时保持了强大的视觉质量。消融和分析研究进一步验证了局部几何条件、多锚点控制和覆盖驱动检索的有效性。

Conclusion: 通过用多个局部几何记忆替代单一全局3D重建，并引入协调机制来处理跨视图不一致性，AnchorWeave有效解决了长视频生成中的空间一致性挑战，为可控视频生成提供了更可靠的记忆增强框架。
...
TL;DR: 提出基于部件的生成框架，从单张图像快速生成带关节的3D资产，避免逐实例优化，实现可控的部件装配与关节运动。


<details>
  <summary>Details</summary>
Motivation: 现有方法要么优化耗时（数分钟到数小时），要么依赖模板检索导致结果与输入不匹配。需要一种能快速生成高保真、与输入一致的关节化3D资产的方法。

Method: 提出部件中心的生成框架，将物体表示为可移动部件集合，每个部件用带有部件身份和关节线索的潜在标记编码。基于单张图像条件生成，保持实例级对应关系和有效的部件结构与运动。

Result: 在常见关节类别（如抽屉、门）上实验显示，相比基于优化和检索的方法，在输入一致性、部件准确性和关节合理性方面均有提升，同时大幅减少推理时间。

Conclusion: 该框架避免了逐实例优化，支持快速前馈推理和可控装配与关节运动，对于具身交互应用具有重要意义。
...
TL;DR: ThermEval-B是一个包含5.5万个热成像视觉问答对的基准测试，用于评估VLMs在热成像理解方面的能力，发现现有模型在温度推理、色彩映射转换等方面表现不佳。


<details>
  <summary>Details</summary>
Motivation: 热成像在可见光失效的场景（如夜间监控、搜救、自动驾驶、医疗筛查）中至关重要，但现有视觉语言模型主要针对RGB图像，无法泛化到热成像。热成像编码的是物理温度而非颜色或纹理，需要现有RGB基准测试无法评估的感知和推理能力。

Method: 提出了ThermEval-B基准测试，包含约5.5万个热成像视觉问答对，整合了公共数据集和新收集的ThermEval-D数据集（首个提供密集逐像素温度图和语义身体部位标注的数据集）。评估了25个开源和闭源视觉语言模型。

Result: 模型在温度基础推理方面持续失败，在色彩映射转换下性能下降，倾向于依赖语言先验或固定响应。通过提示或监督微调只能获得边际改进。现有模型无法有效处理热成像特有的挑战。

Conclusion: 热成像理解需要超越RGB中心假设的专门评估，ThermEval-B基准测试为推动热成像视觉语言建模的进展提供了必要工具。
...
TL;DR: Sphere Encoder是一种高效的生成框架，通过单次前向传播生成图像，性能可与多步扩散模型竞争但推理成本极低


<details>
  <summary>Details</summary>
Motivation: 现有扩散模型需要多步推理，计算成本高。作者希望开发一种更高效的生成框架，能在保持图像质量的同时大幅减少推理步骤

Method: 学习一个编码器将自然图像均匀映射到球形潜在空间，解码器将随机潜在向量映射回图像空间。仅通过图像重建损失训练，生成时只需解码球面上的随机点。支持条件生成，多次循环编码器/解码器可进一步提升质量

Result: 在多个数据集上，Sphere Encoder的性能与最先进的扩散模型相当，但推理成本仅为后者的极小部分（少于5步）

Conclusion: Sphere Encoder提供了一种高效的单次前向传播生成框架，在保持图像质量的同时大幅降低了推理成本，为生成模型的实际应用提供了新方向
...
TL;DR: EditCtrl：一种高效视频修复控制框架，通过局部视频上下文模块和轻量级全局上下文嵌入器，将计算成本与编辑大小成正比，相比现有方法计算效率提升10倍且质量更好。


<details>
  <summary>Details</summary>
Motivation: 现有基于预训练视频基础模型的高保真生成视频编辑方法存在计算效率低下的问题，无论修复掩码大小如何都处理完整视频上下文，导致计算资源浪费。

Method: 提出EditCtrl框架，包含：1）局部视频上下文模块，仅处理掩码标记，计算成本与编辑大小成正比；2）轻量级时序全局上下文嵌入器，确保视频整体上下文一致性且开销最小。

Result: EditCtrl比最先进的生成编辑方法计算效率高10倍，甚至比采用全注意力机制的方法编辑质量更好，并能实现多区域文本提示编辑和自回归内容传播等新功能。

Conclusion: EditCtrl通过将计算聚焦于需要编辑的区域，实现了高效且高质量的生成视频编辑，为视频编辑开辟了新的可能性。
...
TL;DR: 提出多模态一致性引导的无参考数据选择管道，用于ASR口音自适应，通过语音-文本对齐和预测WER筛选伪标签，显著提升口音语音识别性能


<details>
  <summary>Details</summary>
Motivation: ASR系统在口音语音上性能下降，因为声学-语音和韵律变化导致与训练数据不匹配，而传统基于文本的伪标签选择方法（如困惑度过滤）偏好流畅但声学不匹配的假设，导致微调时错误放大

Method: 提出多模态一致性引导的无参考数据选择管道：1）基于子模互信息的目标感知预选；2）通过扰动解码生成多个伪转录；3）使用语音-文本对齐和预测WER两个无参考信号评分；4）基于百分位的选择规则保留可靠伪标签

Result: 在域内设置中，从30k数据池中选择约1.5k话语达到10.91% WER，接近使用30k监督标签的10.45%；在跨域设置中，一致性过滤子集避免了强口音偏移下未过滤伪标签导致的性能下降，匹配小时实验进一步证实优于随机采样和现有基线

Conclusion: 多模态一致性引导的数据选择管道能有效筛选可靠伪标签用于ASR口音自适应，在监督标签有限的情况下实现接近全监督的性能，并避免跨域场景中的性能退化
...
TL;DR: LLMs和机器翻译系统在危机领域翻译中存在性能下降和不稳定性，即使语言正确的翻译也可能扭曲紧迫感感知，提示语言会影响LLM的紧迫性分类，凸显了在危机通信中部署通用语言技术的风险。
...
Method: 使用多语言危机数据和新引入的紧迫性标注数据集（覆盖32种以上语言），评估最先进的LLMs和机器翻译系统在危机领域翻译中的表现，特别关注紧迫性保留。

Result: 专用翻译模型和LLMs都表现出显著的性能下降和不稳定性；即使语言正确的翻译也可能扭曲感知的紧迫性；基于LLM的紧迫性分类因提示语言和输入语言的不同而有很大差异。
...
TL;DR: 本研究针对斯瓦希里语中的网络霸凌和辱骂性模糊语言检测，使用机器学习模型（SVM、逻辑回归、决策树）结合SMOTE处理数据不平衡，发现模型性能受限于小数据集和不平衡问题，未来需要扩展数据集和采用更先进技术。


<details>
  <summary>Details</summary>
Motivation: 数字技术发展增加了网络霸凌和在线虐待的风险，需要加强检测和预防措施，特别是针对儿童。斯瓦希里语作为非洲使用最广泛的语言（超过1亿使用者），但属于低资源语言，缺乏足够的语言资源和技术支持，因此研究其辱骂性模糊语言检测具有重要现实意义。

Method: 采用机器学习模型包括支持向量机（SVM）、逻辑回归和决策树，通过严格的参数调优进行优化。使用合成少数类过采样技术（SMOTE）处理数据不平衡问题。在高维文本数据上评估模型性能，分析精确率、召回率和F1分数。

Result: 机器学习模型在高维文本数据上表现良好，但由于数据集规模小且不平衡，研究结果的普适性受到限制。详细分析了各模型在检测模糊语言方面的精确率、召回率和F1分数，揭示了各模型的细微性能差异。

Conclusion: 本研究为创建更安全的儿童在线环境做出贡献，建议扩展数据集并采用更先进的机器学习技术来提高网络霸凌检测系统的有效性。未来工作将专注于增强数据鲁棒性、探索迁移学习以及整合多模态数据，以创建更全面和文化敏感的检测机制。
...
TL;DR: 语言模型嵌入包含的输入信息很少，而自编码器嵌入能形成近乎完美的记忆；通过结合因果和信息保留目标，可以训练出能形成和解码信息丰富记忆的模型


<details>
  <summary>Details</summary>
Motivation: 机器学习模型在隐藏层向量嵌入中存储输入信息的能力（类似于"记忆"）被广泛应用但缺乏系统表征。研究者希望理解不同模型架构在记忆形成能力上的差异，并探索更高效的记忆存储方法

Method: 1) 比较语言模型和自编码器的嵌入信息含量；2) 提出并行化编码器-解码器记忆模型架构；3) 结合因果训练和信息保留目标函数；4) 采用冻结高保真编码器+课程训练方法（先学习处理记忆，再学习预测下一个token）

Result: 语言模型嵌入通常包含相对较少的输入信息，无论训练数据和计算规模如何；自编码器嵌入能形成近乎完美的记忆；结合目标函数的模型能学习形成和解码信息丰富的记忆；课程训练方法能进一步简化训练过程

Conclusion: 仅靠下一个token预测训练不适合准确的记忆形成，因为该目标本身是不可逆的；对于不暴露整个输入的模型，需要使用组合目标函数来促进记忆形成；编码器-解码器架构结合适当训练策略能实现高效记忆存储
...
TL;DR: 本研究首次对土耳其新闻媒体中的AI生成内容进行实证量化分析，通过微调土耳其专用BERT模型，在三大土耳其媒体3600篇文章数据集上实现了97.08%的F1分数，发现约2.5%的新闻内容被LLM改写或修订。
...
Method: 通过微调土耳其专用BERT模型（dbmdz/bert-base-turkish-cased），在来自三大土耳其媒体（具有不同编辑方向）的3600篇标记文章数据集上进行二元分类，检测AI改写内容。随后将该模型部署在2023年至2026年间的3500多篇未见文章上。

Result: 模型在保留测试集上获得0.9708的F1分数，两个类别的精确率和召回率对称。对3500多篇未见文章的分析显示，跨来源和时间稳定的分类模式，平均预测置信度超过0.96，估计平均约2.5%的新闻内容被LLM改写或修订。

Conclusion: 这是首次超越记者自我报告感知，对土耳其新闻媒体中AI使用情况进行实证、数据驱动测量的研究，填补了该领域的研究空白，为理解AI在非英语新闻媒体中的渗透提供了重要基准。
...
TL;DR: 该论文提出了一种量化LLM推理时"深度思考"的方法，通过识别内部预测在深层发生显著修正的token，发现深度思考比例与准确性呈正相关，并基于此提出了Think@n策略来优化推理效率。


<details>
  <summary>Details</summary>
Motivation: 现有研究发现，LLM的生成长度（token数量）并不能可靠地反映推理质量，有时增加长度反而会导致"过度思考"和性能下降。需要找到更好的指标来量化推理时的努力程度。

Method: 提出了"深度思考token"的概念，即那些在深层模型层中内部预测发生显著修正的token。计算深度思考比例（深度思考token占总生成序列的比例），并基于此设计了Think@n策略，该策略优先选择高深度思考比例的样本，并基于短前缀早期拒绝无希望的生成。

Result: 在四个具有挑战性的数学和科学基准测试（AIME 24/25, HMMT 25, GPQA-diamond）和多种推理模型（GPT-OSS, DeepSeek-R1, Qwen3）上，深度思考比例与准确性表现出稳健且一致的正相关关系，显著优于基于长度和置信度的基线方法。

Conclusion: 深度思考比例是比生成长度更可靠的推理质量指标，基于此的Think@n策略能够在匹配或超越标准自一致性性能的同时，显著降低推理成本，为LLM推理优化提供了新方向。
...
TL;DR: 论文提出能力校准概念，针对LLM在查询上的期望准确率进行置信度估计，而非传统的单次响应校准，以更好反映模型真实能力。


<details>
  <summary>Details</summary>
Motivation: 现有LLM校准主要关注响应级置信度（单次输出的正确性），但这与实际问题解决场景不匹配，因为用户关心的是模型整体解决查询的可能性。这种不匹配源于现代LLM解码的随机性，单次响应正确性无法反映底层模型能力。

Method: 引入能力校准概念，针对模型在查询上的期望准确率进行置信度估计。正式区分能力校准与响应校准，建立实证评估框架，研究一系列置信度估计方法。

Result: 能力校准置信度在pass@k预测和推理预算分配方面表现优于传统方法，证明两者在理论和实证上都存在差异，为多样化应用奠定基础。

Conclusion: 能力校准比响应校准更符合实际应用需求，能更好反映LLM的真实问题解决能力，在预测模型性能和资源分配方面具有重要应用价值。
...
TL;DR: FLIP是一种无需参考回答和评分标准的奖励建模方法，通过反向推理从回答反推指令，用推断指令与原始指令的相似度作为奖励信号，在多个领域优于LLM-as-a-Judge基线79.6%。


<details>
  <summary>Details</summary>
Motivation: 当前奖励建模方法存在局限性：LLM-as-a-Judge依赖大模型强推理能力，其他方法需要参考回答或明确评分标准，限制了灵活性和可访问性。需要一种无需参考和评分标准的奖励建模方法。

Method: 提出FLIP方法，通过反向推理重新定义奖励建模：从给定回答推断最可能产生该回答的指令，然后使用推断指令与原始指令的相似度作为奖励信号。

Result: 在4个领域使用13个小语言模型的评估显示，FLIP平均优于LLM-as-a-Judge基线79.6%。在并行采样和GRPO训练的外在评估中显著提升下游性能，特别擅长处理长输出且对奖励攻击鲁棒。

Conclusion: FLIP通过利用验证-生成差距，在传统判断方法失效的降尺度场景中实现了可靠的奖励建模，提供了一种无需参考和评分标准的灵活奖励建模方案。
...
TL;DR: DistillLens是一个知识蒸馏框架，通过将中间隐藏状态投影到词汇空间，对称地对齐师生模型的思维过程，优于传统知识蒸馏方法。


<details>
  <summary>Details</summary>
Motivation: 传统知识蒸馏只优化最终输出，将教师模型的中间层思维过程视为黑箱；现有的基于特征的蒸馏方法忽略了最终输出所需的不确定性分布。

Method: 通过Logit Lens将中间隐藏状态投影到词汇空间，使用对称散度目标强制执行结构对齐，防止过度自信和不足自信，同时保留最终推理所需的高熵信息通道。

Result: 在GPT-2和Llama架构上的实验表明，DistillLens在多样化的指令跟随基准测试中始终优于标准知识蒸馏和特征迁移基线方法。

Conclusion: DistillLens通过对称对齐师生模型的思维过程，有效提升了知识蒸馏的效果，代码已开源。
...
TL;DR: 提出LCR（LLM-Confidence Reranker），一种无需训练、即插即用的重排序算法，利用LLM置信度信号提升RAG系统性能，在多个基准测试中显著改善检索质量。


<details>
  <summary>Details</summary>
Motivation: LLM在知识密集型任务中存在幻觉问题，RAG通过外部知识缓解但依赖准确检索。现有重排序器需要专门训练、计算成本高，且未能充分利用LLM的语义能力和置信度信号。

Method: LCR采用两阶段流程：1) 通过多项式采样和聚类进行置信度评估（基于MSCP）；2) 基于查询和文档置信度阈值的分箱和多级排序。优先相关文档，对高置信度查询保持原始排序。

Result: 在BEIR和TREC基准测试中，使用BM25和Contriever检索器，LCR仅用7-9B参数预训练LLM，将NDCG@5提升高达20.6%，且不降低性能。消融研究验证LLM置信度与文档相关性正相关。

Conclusion: LCR提供计算效率、可扩展并行性和广泛兼容性，能有效缓解医疗诊断等应用中的幻觉问题，为RAG系统提供高效重排序解决方案。
...
Result: 在PAC学习理论基础上证明成对比较具有更优的样本复杂度，实证验证相比绝对评分方法减少4.5倍噪声。使用Qwen2.5-7B模型与Qwen2.5-14B、Qwen2.5-32B、Qwen3-8B等对手训练，在Alpaca Eval 2.0和MT-Bench上显示清晰性能层次：点基方法 < 静态成对训练 < Elo-Evolve。

Conclusion: 成对比较和动态对手选择为LLM对齐提供了渐进式优势，Elo-Evolve框架通过竞争性学习范式解决了传统对齐方法的局限性，实现了更高效稳定的模型对齐。
...
TL;DR: 使用历时分布语义学工具分析19世纪文学隐喻随时间变化的处理成本，发现总体稳定但受文体影响：现代文学中隐喻更难处理，而网络非文学语言中更易理解


<details>
  <summary>Details</summary>
Motivation: 文学隐喻作为文学语言的特色，在实验研究中相对较少，且以往研究忽略了时间维度。本研究旨在探索文学隐喻的处理成本是否随时间推移和文体差异而变化

Method: 使用历时分布语义学方法，在19世纪和21世纪的意大利文学与非文学语料库（总计1.24亿词符）上训练词向量，计算515个19世纪文学隐喻中主题与载体之间的语义相似度，以此作为隐喻处理需求的代理指标

Result: 总体语义相似度（即隐喻处理需求）随时间保持稳定。但文体起关键作用：现代文学语境中的隐喻比19世纪文学更难处理（主题-载体相似度更低），而当今非文学语言（如网络）中的隐喻比19世纪非文学文本更易理解（相似度更高）。这一模式还受隐喻个体词语的语义特征（如向量一致性和语义邻域密度）影响

Conclusion: 研究结果与意大利语更广泛的语言变化一致：现代文学的文体简化可能增加了隐喻处理需求，而网络语言的高度创造性似乎使隐喻更易理解。历时分布语义学为研究文学隐喻的时间维度提供了有效工具
...
TL;DR: 本文提出一个多智能体框架，通过整合论证理论（使用RAG）来识别政治辩论中重新表述的修辞功能，相比零基线模型在宏观F1分数上提升近30%。


<details>
  <summary>Details</summary>
Motivation: 识别话语中重新表述的战略用途是计算论证的关键挑战。虽然LLMs能检测表面相似性，但往往无法捕捉重新表述的语用功能（如修辞作用）。需要将理论知识与LLMs结合来超越单纯的释义检测。

Method: 提出比较性多智能体框架：1）建立包含四种重新表述功能（去强化、强化、具体化、泛化）的标注政治辩论数据集；2）评估两个并行LLM智能体系统：一个通过RAG增强论证理论知识，另一个为零基线模型。

Result: RAG增强的智能体在所有类别上显著优于基线，在检测强化和泛化上下文方面优势尤其明显，整体宏观F1分数提升近30%。

Conclusion: 理论基础不仅有益，而且对于超越单纯释义检测、实现功能感知的论证话语分析至关重要。该多智能体架构为开发可扩展的理论驱动计算工具迈出了一步。
...
TL;DR: 本文提出RMPL框架，用于低资源条件下的多媒体事件抽取，通过多任务渐进学习提升跨模态事件语义理解。


<details>
  <summary>Details</summary>
Motivation: 多媒体事件抽取面临标注数据稀缺的问题，现有方法依赖跨模态对齐或提示工程，缺乏结构化事件表示学习，导致多模态场景下论元定位能力弱。

Method: 提出RMPL框架，整合单模态事件抽取和多媒体关系抽取的异构监督，采用阶段式训练：先学习跨模态共享的事件中心表示，再针对事件提及识别和论元角色抽取进行微调。

Result: 在M2E2基准测试中，使用多种视觉语言模型进行实验，在不同模态设置下均取得一致性能提升。

Conclusion: RMPL框架通过关系感知的多任务渐进学习，有效解决了低资源条件下多媒体事件抽取的挑战，提升了跨模态事件语义理解能力。
...
TL;DR: 该研究分析了德语与德国手语之间的词汇-手势映射关系，创建了首个跨模态语义对应标注数据集，并评估了计算方法在识别不同对应类型时的表现。


<details>
  <summary>Details</summary>
Motivation: 现有手语词典在表示多义词和同音词在不同语境下对应不同手势时往往不足，需要基于用法的分析方法来识别词典中缺失的新映射关系，以丰富词典资源。

Method: 研究分析了德语和德国手语，手动标注了1,404个词汇使用-手势ID映射，这些映射来自德语词汇使用图的32个词汇和德国手语数字词典的49个手势。识别了三种对应类型：一对多、多对一和一对一，以及无匹配情况。评估了精确匹配和基于SBERT嵌入的语义相似性两种计算方法。

Result: 语义相似性方法整体表现显著优于精确匹配（88.52% vs 71.31%），特别在一对多对应类型上提升了52.1个百分点。研究建立了首个跨模态语义对应标注数据集，并揭示了哪些对应模式可以通过计算方法识别。

Conclusion: 该工作为手语词典编纂提供了基于用法的分析方法，展示了语义相似性方法在识别复杂词汇-手势映射关系上的优势，公开的代码和数据集将促进相关研究发展。
...
Method: 开发了OMGs多智能体AI框架，其中领域特定的智能体协作审议，整合多学科证据，生成具有透明推理的MDT式建议。使用SPEAR（安全性、个性化、证据、可操作性、稳健性）框架系统评估MDT建议质量。
...
Conclusion: 多智能体审议系统可以达到与专家MDT共识相当的性能，有潜力在资源有限的环境中扩展专业肿瘤学知识的可及性，特别是在证据和稳健性方面能显著增强临床决策。
...
TL;DR: 本研究采用普遍语法框架，考察也门英语学习者习得英语不规则屈折变化的过程，发现L1迁移在初级阶段占主导，而后期学习者对UG特性的敏感性增强，但某些不规则形式仍存在持续困难。


<details>
  <summary>Details</summary>
Motivation: 研究旨在探讨成人二语学习者在普遍语法框架下如何习得英语不规则屈折变化，特别关注L1迁移和L2发展因素的作用，以及学习者是否持续拥有UG访问权限。

Method: 采用普遍语法方法，结合特征重组假说，分析也门英语学习者在两个发展阶段中的错误类型。使用统计方法（包括单因素方差分析）评估学习者从不规则屈折变化的进步情况。

Result: 阶段1数据显示L1迁移占主导地位，阶段2显示学习者对UG特性的敏感性增强。统计分析表明从阶段1到阶段2在不规则屈折变化产生方面有显著进步，但辅音变化、零语素和-a复数等特定形式仍存在持续困难。

Conclusion: 虽然L1迁移和L2发展因素影响习得初期阶段，但适当的语言输入和教学对促进成人二语学习者基于UG的特征重组至关重要。学习者持续拥有UG访问权限，但有限暴露、无效输入建模和教学不足会限制完全UG访问。
...
TL;DR: 该论文提出将LLM的心智理论(ToM)形式化为认知分歧检测与解决机制，创建了评估基准并发现现有模型在识别阻碍任务成功的认知差距方面存在显著局限，通过强化学习训练轨迹数据提升了模型推理用户心理状态的能力。


<details>
  <summary>Details</summary>
Motivation: 当前LLM在用户意图和指令表述不精确时难以理解真实需求，导致用户主观信念与环境真实状态之间存在认知分歧。现有ToM评估主要关注孤立的信念推理，忽视了其在真实交互中的功能性价值。

Method: 1) 将LLM的ToM形式化为认知分歧检测与解决机制；2) 提出评估基准来评估模型在实践中如何协调用户信念和画像；3) 构建基于轨迹的ToM数据集，将信念追踪与任务相关状态推理联系起来；4) 通过强化学习在该数据上训练模型。

Result: 对11个领先模型的评估显示，它们在识别阻碍任务成功的底层认知差距方面存在显著局限。使用强化学习在轨迹数据上训练的模型在推理用户心理状态方面表现出一致的改进，从而提升了下游任务性能。

Conclusion: ToM应被视为必要的交互层面机制，而非独立的推理技能。该研究强调了ToM在实际应用中的价值，通过将信念追踪与任务状态推理相结合，可以显著提升LLM理解用户真实需求的能力。
...
TL;DR: 提出SpecVocab方法，通过每步动态选择词汇子集来优化推测解码，相比现有方法EAGLE-3提升接受长度和吞吐量


<details>
  <summary>Details</summary>
Motivation: 推测解码加速语言模型推理时，现有方法使用缩减词汇表会降低推测效果，当目标词不在词汇表中时尤其明显

Method: 提出SpecVocab方法，在解码的每一步动态选择词汇子集进行推测，而非使用固定缩减词汇表

Result: SpecVocab相比EAGLE-3获得更高的接受长度，在多种任务上实现最高8.1%的平均吞吐量提升

Conclusion: 词汇推测比缩减词汇表更有效，SpecVocab方法在保持推测解码效率的同时提升了推测效果
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型智能体越来越多地部署在涉及敏感、上下文相关信息的个性化任务中，由于上下文隐私的隐含性，智能体行动中可能出现隐私侵犯。现有方法依赖于外部、推理时干预，这些方法脆弱、场景特定，并可能扩大隐私攻击面。

Method: 提出PrivAct框架，将上下文隐私保护直接内化到模型的生成行为中，通过将隐私偏好嵌入到每个智能体中，实现系统范围的上下文完整性，同时获得更优的隐私-帮助性权衡。

Result: 在多个LLM骨干网络和基准测试上的实验表明，PrivAct在上下文隐私保护方面实现了一致的改进，泄漏率降低了高达12.32%，同时保持了相当的帮助性，并在多样化的多智能体拓扑结构中展现出零样本泛化能力和鲁棒性。

Conclusion: PrivAct通过将隐私保护内化到智能体生成行为中，提供了一种更有效、更鲁棒的上下文隐私保护方法，优于依赖外部干预的现有方法，实现了更好的隐私-帮助性平衡。
...
TL;DR: 开发"负责任智能"框架，将大语言模型的生成能力与真实世界部署的严格要求相协调，通过领域适应、伦理严谨性和文化/多语言对齐三个方向实现


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型成为人工智能的变革性力量，迫切需要超越通用架构，开发具有上下文感知、内在安全性和尊重全球文化差异的系统

Method: 从经典监督适应满足任务特定需求，到解码时对齐确保安全性，最后利用人类反馈和偏好建模实现社会语言敏锐度

Result: 提出了一个综合框架，通过三个相互关联的线程（领域适应、伦理严谨性、文化/多语言对齐）来协调LLMs的生成能力与部署要求

Conclusion: 需要开发负责任智能框架，使大语言模型在保持强大生成能力的同时，满足技术精确性、安全性和全球包容性的严格要求
...
TL;DR: 论文指出当前LLM安全机制主要针对英语和高资源语言，但在全球南方地区的低资源语言、语码混合和文化特定场景中安全防护严重失效，需要建立多语言安全核心框架而非附加功能。


<details>
  <summary>Details</summary>
Motivation: LLM正在全球南方地区部署，但现有安全管道、基准测试和对齐主要针对英语和少数高资源语言，隐含假设安全性可以跨语言"转移"，而证据表明这种假设不成立，导致低资源语言用户面临安全风险。

Method: 综合近期研究发现，分析安全护栏在低资源和语码混合输入中的弱化、文化有害行为的持续性、英语知识编辑和安全补丁在低资源语言中的失效问题，并提出实践议程。

Result: 研究发现：(1)安全护栏在低资源和语码混合输入中显著减弱；(2)文化有害行为在标准毒性评分可接受时仍持续存在；(3)英语知识编辑和安全补丁常无法传递到低资源语言。

Conclusion: 需要将多语言安全作为核心要求而非附加功能，通过参数高效安全引导、文化基础评估和偏好数据、参与式工作流程等实践议程，让本地社区定义和减轻危害，实现公平AI。
...
TL;DR: 本文介绍了ADAB（阿拉伯语礼貌数据集），这是一个包含10,000个样本的阿拉伯语礼貌检测数据集，涵盖现代标准阿拉伯语和多种方言，标注了礼貌、不礼貌和中性三类，并包含16种礼貌类别的语言特征标注。


<details>
  <summary>Details</summary>
Motivation: 随着文化感知自然语言处理系统的重要性日益增长，需要更多资源来捕捉不同语言的社会语用现象。尽管阿拉伯语交流中蕴含丰富复杂的礼貌表达，但阿拉伯语礼貌检测资源仍然不足，需要填补这一空白。

Method: 从四个在线平台（社交媒体、电子商务和客户服务领域）收集阿拉伯语数据，涵盖现代标准阿拉伯语和多种方言（海湾、埃及、黎凡特和马格里布）。基于阿拉伯语语言传统和语用理论进行标注，分为礼貌、不礼貌和中性三类，并标注16种礼貌类别的语言特征。评估了40种模型配置，包括传统机器学习、基于Transformer的模型和大语言模型。

Result: 创建了包含10,000个样本的ADAB数据集，实现了较高的标注者间一致性（kappa = 0.703）。通过基准测试评估了各种模型配置的性能，为阿拉伯语礼貌检测研究提供了可靠的数据资源。

Conclusion: ADAB数据集填补了阿拉伯语礼貌检测资源的空白，支持阿拉伯语NLP的礼貌感知研究，为开发文化敏感的阿拉伯语自然语言处理系统提供了重要基础。
...
TL;DR: 该论文通过大规模实证研究，评估了24种不同提示模板对小型语言模型RAG系统在复杂多跳问答任务中的性能影响，发现最优模板相比标准RAG提示能带来高达83-84.5%的性能提升。


<details>
  <summary>Details</summary>
Motivation: 虽然检索增强生成(RAG)在大型语言模型中已被广泛研究，但针对小型语言模型(SLMs)的RAG优化仍存在研究空白，特别是在需要复杂推理的多跳问答任务中。提示模板设计是影响性能的关键但未被充分探索的因素。

Method: 在HotpotQA数据集上进行大规模实证研究，评估24种不同提示模板（包括标准RAG提示、9种文献中的成熟技术、14种新颖混合变体），在两个主流小型语言模型(Qwen2.5-3B Instruct和Gemma3-4B-It)上进行测试，共涉及18720个测试实例。

Result: 研究发现显著性能提升：Qwen2.5模型性能提升达83%，Gemma3-4B-It模型提升达84.5%，相比标准RAG提示，两种模型都能获得高达6%的改进。研究提供了具体分析和可操作建议。

Conclusion: 该研究填补了小型语言模型RAG优化的研究空白，证明了提示模板设计对性能的重要影响，为资源受限环境中部署SLM-based RAG系统提供了有效的提示设计指南和实用建议。
...
TL;DR: 提出预编辑规范化(PEN)任务，将古文字识别(ATR)的原始输出按编辑规范进行规范化，平衡古文字保真度与实用性的矛盾，并创建了相关数据集和模型。


<details>
  <summary>Details</summary>
Motivation: 当前自动文本识别(ATR)存在方法论鸿沟：古文字转录模型输出难以被普通读者和下游NLP工具使用，而规范化模型又容易过度规范化和产生幻觉，难以适应新领域。

Method: 提出预编辑规范化(PEN)任务，创建基于CoMMA语料库的4.66M样本银训练语料和1.8k样本金评估集，使用ByT5序列到序列模型进行规范化和预标注任务基准测试。

Result: 规范化模型达到6.7%的字符错误率(CER)，显著优于该任务的先前模型，提供了PEN的正式定义、大规模训练语料和高质量评估集。

Conclusion: PEN任务有效解决了古文字识别输出与实用需求之间的鸿沟，在保持古文字保真度的同时提供规范化版本，为历史档案数字化提供了新方法。
...
TL;DR: HLE-Verified是对Humanity's Last Exam基准的验证修订版本，通过两阶段验证修复流程减少标注噪声，提升模型评估的准确性。


<details>
  <summary>Details</summary>
Motivation: 原始HLE基准存在大量噪声项，会偏差评估结果并扭曲跨模型比较，需要更可靠的基准来准确评估前沿大语言模型。

Method: 采用两阶段验证修复流程：第一阶段通过领域专家审查和模型交叉检查进行二元验证；第二阶段对可修复项在保持原始评估意图的严格约束下进行修订，包括双独立专家修复、模型辅助审计和最终裁决。

Result: 获得641个验证项和1,170个修订认证项，剩余689个作为不确定集。在HLE-Verified上评估的7个前沿语言模型平均绝对准确率提升7-10个百分点，在原始问题或参考答案有误的项上提升30-40个百分点。

Conclusion: HLE-Verified通过减少标注噪声改善了HLE式评估，能更忠实地测量模型能力，模型置信度与问题或参考答案错误存在强关联，验证了修订的有效性。
...
TL;DR: 利用大语言模型对阿尔茨海默病患者电子健康记录进行链式思维推理，通过显式诊断逻辑提升AD评估的稳定性和诊断性能
...
Method: 提出利用大语言模型对患者临床电子健康记录进行链式思维推理。不同于直接在EHR数据上微调LLMs进行AD分类，该方法使用LLM生成的CoT推理路径为模型提供显式AD评估诊断逻辑，然后进行基于结构化CoT的预测。

Result: 实验结果表明，提出的基于CoT的诊断框架在多个CDR分级任务中显著增强了稳定性和诊断性能，相比零样本基线方法F1分数提升高达15%。

Conclusion: 该方法不仅增强了模型诊断内在复杂因素的能力，还提高了AD进展不同阶段预测过程的可解释性，为AD评估提供了有效的AI辅助诊断框架。
...
TL;DR: 该研究探讨了LLM自我解释（如思维链）在长度与充分性之间的权衡，发现更简洁的解释通常仍能保持准确性，但过度压缩会导致性能下降。


<details>
  <summary>Details</summary>
Motivation: LLM依赖自我解释（如思维链）来提高多步问答性能，但这些解释通常冗长且生成成本高。研究旨在探索解释的必要长度，在充分性（能证明正确答案）和简洁性（减少长度）之间找到平衡。

Method: 基于信息瓶颈原则，将解释视为压缩表示，仅保留产生正确答案所需的关键信息。引入评估流程，在ARC Challenge数据集上约束解释长度并评估充分性，使用多个语言模型进行实验。研究涵盖英语（原始数据集）和波斯语（通过翻译作为资源受限语言）。

Result: 实验表明，更简洁的解释通常仍保持充分性，能在显著减少解释长度的同时保持准确性。然而，过度压缩会导致性能下降，需要在简洁性和充分性之间找到平衡点。

Conclusion: LLM解释存在长度与充分性的权衡，更简洁的解释通常足以支持正确答案，但需要避免过度压缩。这一发现对优化LLM解释生成、降低计算成本具有重要意义，特别是在资源受限的语言环境中。
...
TL;DR: 该论文分析了用于支付数据提取的命名实体识别算法，比较了CRF、BiLSTM-CRF和基于Transformer的模型，提出了新的PaymentBERT架构，在金融交易处理中实现了95.7%的F1分数。


<details>
  <summary>Details</summary>
Motivation: 命名实体识别在自动化金融交易处理中至关重要，特别是从非结构化支付数据中提取结构化信息。金融交易数据格式多样（如SWIFT MT103、ISO 20022等），需要高效的实体提取方法来支持制裁筛查、反洗钱合规和支付处理系统。

Method: 论文系统分析了多种NER算法：传统CRF、BiLSTM-CRF和基于Transformer的模型（BERT、FinBERT）。在包含50,000个标注支付交易的数据集上进行实验，涵盖多种支付格式。提出了PaymentBERT，一种结合领域特定金融嵌入和上下文表示的新型混合架构。

Result: 微调的BERT模型在实体提取上达到94.2%的F1分数，比传统CRF方法高出12.8个百分点。提出的PaymentBERT架构实现了最先进的95.7% F1分数，同时保持实时处理能力。论文还提供了跨格式泛化分析、消融研究和部署考虑。

Conclusion: 基于Transformer的模型在支付数据NER任务上显著优于传统方法。提出的PaymentBERT架构通过结合金融领域知识和上下文表示，实现了最优性能。这项研究为金融机构实施自动化制裁筛查、反洗钱合规和支付处理系统提供了实用见解。
...
TL;DR: 本文提出了一种用于机器翻译的组相对奖励模型（GRRM），通过组质量度量范式改进GRPO框架中的组内排序准确性，从而提升翻译质量和推理能力。


<details>
  <summary>Details</summary>
Motivation: 在机器翻译等开放域任务中，传统的标量质量度量方法在评估候选翻译时缺乏比较上下文，难以区分细微的语言差异，导致GRPO框架中的组内排序不准确。

Method: 提出组质量度量范式，并实例化为组相对奖励模型。GRRM通过联合处理整个候选组，利用比较分析来严格解析相对质量和自适应粒度，而不是像传统方法那样独立评分。

Result: GRRM在所有基线方法中实现了有竞争力的排序准确性。将GRRM集成到GRPO训练循环中后，不仅提高了整体翻译质量，还解锁了与最先进推理模型相当的推理能力。

Conclusion: 组相对奖励模型通过引入组质量度量范式，有效解决了GRPO框架在开放域任务中的组内排序问题，显著提升了机器翻译的性能和推理能力。
...
TL;DR: MoE嵌入模型使用线性加权求和聚合专家输出，但这与专家表示的实际几何结构不一致。研究发现专家输出位于共享的超球面流形上，线性聚合会导致向量塌缩。作者提出了球面重心聚合(SBA)方法，在保持现有路由机制的同时维护超球面几何结构。


<details>
  <summary>Details</summary>
Motivation: 现有MoE嵌入模型假设专家输出在嵌入空间中具有线性子空间结构，但这种假设与专家表示的实际几何结构不一致。线性聚合会导致向量幅度和方向的失真，降低嵌入的可比性。

Method: 提出球面重心聚合(SBA)方法，将径向和角度分量分离处理，以保持超球面几何结构。该方法完全兼容现有的路由机制，无需额外训练成本。

Result: 在MTEB基准测试的语义相似性、聚类和重复问题检测等任务上，SBA相比线性聚合方法表现出持续的性能提升，同时保持完全稳定性。几何分析证实SBA能防止聚合引起的塌缩并保持超球面一致性。

Conclusion: MoE嵌入架构中的几何感知聚合至关重要。SBA作为一种几何保持的聚合算子，通过分离径向和角度分量有效解决了线性聚合与超球面几何结构不一致的问题，为MoE嵌入模型提供了更好的几何基础。
...
TL;DR: LLM在事实核查任务中，随着上下文长度增加，验证准确性下降，且证据在提示中的位置（开头或结尾）对准确性有显著影响。


<details>
  <summary>Details</summary>
Motivation: 虽然LLM在多种任务中表现出强大的推理能力，但在长上下文中的表现不一致。先前研究关注问答任务中的中间上下文退化，本研究旨在探究上下文对LLM事实核查的影响。
...
Result: LLM表现出非平凡的参数化事实知识，但验证准确性通常随上下文长度增加而下降。与先前研究一致，证据位置至关重要：当相关证据出现在提示开头或结尾时准确性更高，出现在中间时准确性较低。

Conclusion: 这些结果强调了提示结构在检索增强事实核查系统中的重要性，需要优化证据放置位置以提高LLM的事实核查性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有测试时间缩放方法在代码生成中存在两个主要挑战：1) 欠思考 - 推理链过浅，无法捕捉问题复杂性；2) 过思考 - 推理过于冗长，导致效率低下和计算成本增加

Method: LogitsCoder框架通过轻量级logit级控制机制增强思维链推理：1) Logits Preference Decoding引导token选择朝向统计偏好的模式；2) Logits Rank Based Path Selection选择多样化推理路径；3) Thoughts Aggregation聚合不同推理路径，迭代生成和优化推理步骤

Result: 实验表明LogitsCoder能产生更高效、更高质量的推理链，在代码生成性能上优于基线方法

Conclusion: LogitsCoder通过logit级控制机制有效平衡推理深度和效率，解决了代码生成中的欠思考和过思考问题，实现了更好的代码生成性能
...
TL;DR: LM-Lexicon是一种创新的定义建模方法，通过数据聚类、语义专家学习和稀疏专家混合架构，在五个基准测试上比现有方法提升了7%的BLEU分数。


<details>
  <summary>Details</summary>
Motivation: 现有的定义建模方法可能无法充分处理语义复杂性，需要更精细的语义领域专业化来提升定义质量。

Method: 将定义建模任务分解为专门的语义领域，使用数据聚类策略，训练小型语言模型作为领域专家，采用稀疏专家混合架构和语义感知的领域级路由机制。

Result: 在五个广泛使用的基准测试上比现有最佳模型提升了7%的BLEU分数；聚类策略使定义质量提升近10%；语义感知路由比传统标记级路由专家效率高1%；通过测试时计算和语义专家扩展可获得进一步性能提升。

Conclusion: LM-Lexicon推进了定义建模领域的发展，为语义密集型应用的高效语言模型开发提供了见解。
...
TL;DR: 本文对Mozilla Common Voice语料库的普什图语部分进行发布级别分析，记录其从2023年中1.49小时到2025年2768.7小时的快速增长，并分析数据集质量、贡献者参与不平等、元数据完整性等问题。


<details>
  <summary>Details</summary>
Motivation: 普什图语作为超过6000万人使用的语言，历史上缺乏适合现代ASR开发的大规模开放许可语音数据。需要分析Mozilla Common Voice语料库中普什图语部分的增长和质量，为低资源语言ASR开发提供数据基础。

Method: 对Mozilla Common Voice语料库版本24.0（2025年12月）进行发布级别分析，追踪主要版本的趋势变化。分析内容包括：数据规模增长、验证吞吐量、贡献者参与不平等性（基尼系数）、人口统计元数据完整性、句子级别集中度等。

Result: 数据从2023年中1.49小时快速增长到2025年2768.7小时，其中975.89小时已验证可用于监督ASR训练。贡献者参与极不平等（基尼系数0.941），年龄分布偏向年轻成年人，41.97%的片段缺乏自报性别标签。文本层面，35.88%的唯一句子占已验证片段的50%。
...
TL;DR: 提出OpenRS系统，通过可检查的原则推理过程替代单一标量奖励模型，解决开放对齐中的奖励黑客问题


<details>
  <summary>Details</summary>
Motivation: 现有标量奖励模型将多维人类偏好压缩为单一不透明分数，导致信息瓶颈、脆弱性和奖励黑客问题，特别是在不可验证任务中

Method: 开发OpenRS系统，包含：1) 成对自适应元准则(PAMR)，根据候选回答语义差异动态生成准则；2) 轻量级点式可验证准则(PVRs)，提供硬约束护栏和可验证奖励；3) 显式元准则规范；4) 两级元准则细化流程；5) 准则级成对比较和外部偏好聚合

Result: OpenRS系统避免了点式加权标量化，提高了开放场景中的区分能力，并作为成对强化学习训练中的奖励监督机制

Conclusion: 稳健对齐应是基于可检查原则的显式推理过程，而非内部化的学习函数；OpenRS通过准则化框架实现了这一理念，为开放对齐提供了更透明、可解释的解决方案
...
TL;DR: 该研究通过自动化翻译和筛选现有多模态数据集，结合合成波兰语数据，成功创建了波兰语视觉语言模型，在波兰语评估中表现优于现有模型。


<details>
  <summary>Details</summary>
Motivation: 大多数视觉语言模型基于英语数据训练，限制了在其他语言和文化环境中的性能，阻碍了反映多元语言文化现实的多模态系统发展，特别是对非英语用户。

Method: 复制并改进LLaVA-Next方法，采用全自动管道翻译和筛选现有多模态数据集，补充合成波兰语OCR和文化特定任务数据，几乎完全依赖自动翻译，仅需最小人工干预。

Result: 在波兰语适配的MMBench上比LLaVA-1.6-Vicuna-13B提升9.5%，在生成评估中人类标注者认为语言正确性更高，证明大规模自动翻译结合轻量筛选能有效为低资源语言构建高质量多模态模型。

Conclusion: 大规模自动翻译结合轻量筛选能有效为低资源语言构建高质量多模态模型，但文化覆盖和评估方面仍存在挑战，研究公开了模型和评估数据集以促进进一步研究。
...
TL;DR: 论文提出Gaussian Thought Sampler (GTS)，一种可学习的推理探索机制，相比启发式扰动方法能更有效地进行推理时扩展


<details>
  <summary>Details</summary>
Motivation: 现有推理时扩展方法使用启发式扰动（如dropout或固定高斯噪声）引入随机性，但这些方法探索效率低下，强扰动不一定产生更有效的候选轨迹，未建模噪声可能破坏内部决策结构而非引导它

Method: 将潜在思维探索建模为从可学习密度的条件采样，实现为Gaussian Thought Sampler (GTS)。GTS预测连续推理状态的上下文相关扰动分布，使用GRPO风格策略优化训练，同时保持主干网络冻结

Result: 在GSM8K数据集上使用两种潜在推理架构的实验表明，GTS比启发式基线方法实现了更可靠的推理时扩展

Conclusion: 改进潜在推理时扩展需要结构化且可优化的探索机制，而非简单地放大随机性
...
TL;DR: 论文提出一个行为框架来区分LLM事实性错误的原因：是知识缺失还是访问失败，并引入WikiProfile基准进行评测，发现前沿模型已编码95-98%事实，但回忆仍是主要瓶颈，思考能显著改善回忆能力。


<details>
  <summary>Details</summary>
Motivation: 现有的事实性评估将所有错误同等对待，无法区分失败是由于知识缺失（空架子）还是对已编码事实的访问受限（丢失的钥匙）。需要更精细的分析框架来理解LLM事实性错误的本质原因。

Method: 提出行为框架，在事实层面而非问题层面分析知识，将每个事实分类为：是否编码、可访问性（无法回忆、直接回忆、需要推理计算）。引入WikiProfile基准，通过基于网络搜索的提示LLM自动化流程构建。

Result: 在13个LLM的400万次响应中，发现前沿模型（GPT-5和Gemini-3）在基准上已编码95-98%的事实，但回忆仍是主要瓶颈。许多先前归因于知识缺失的错误实际上源于访问失败，这些失败系统性影响长尾事实和反向问题。思考能改善回忆并恢复大量失败。

Conclusion: 未来改进可能更依赖于改善模型如何利用已编码知识的方法，而非单纯扩展规模。需要关注提高模型访问已编码事实的能力，特别是通过推理计算（思考）来改善回忆性能。
...
TL;DR: 论文提出了CCiV基准来评估大语言模型生成宋词的能力，从结构、韵律和质量三个维度进行系统评估，发现模型常生成历史变体、韵律比结构更难控制，且形式正确性与文学质量关联较弱。


<details>
  <summary>Details</summary>
Motivation: 宋词生成需要结构严谨、韵律和谐和艺术质量的高度结合，这对大语言模型提出了重大挑战。目前缺乏系统评估模型在这三个维度表现的方法，需要建立专门的基准来推动这一领域的发展。

Method: 提出了CCiV（Chinese Cipai Variants）基准，包含30个词牌，从结构、韵律和质量三个维度评估17个大语言模型生成的宋词。采用形式感知提示策略来改善模型表现。

Result: 发现两个关键现象：1）模型经常生成有效但意料之外的历史词牌变体；2）遵守平仄韵律比结构规则困难得多。形式感知提示对强模型能改善结构和韵律控制，但对弱模型可能适得其反。形式正确性与文学质量之间关联弱且不一致。

Conclusion: CCiV基准强调了需要变体感知的评估方法和更全面的约束性创意生成方法。宋词生成需要同时考虑形式正确性和文学质量，当前模型在这方面的能力仍有待提升。
...
TL;DR: 研究发现位置不变编码的transformer模型能够学习西班牙语L形词素模式，但无法像人类一样进行生产性泛化，揭示了统计模式学习与形态抽象之间的差距。


<details>
  <summary>Details</summary>
Motivation: 探究神经网络是否能够作为形态学习的认知模型，特别是能否像人类一样泛化不规则形态模式。西班牙语L形词素是一个理想测试案例，因为其模式缺乏明显的音系、语义或句法动机。

Method: 使用五种编码器-解码器transformer模型进行比较，这些模型在两个维度上变化：1) 顺序位置编码 vs 位置不变位置编码；2) 原子标签表示 vs 分解标签表示。测试模型在训练数据中L形动词稀缺的情况下能否学习并泛化该模式。

Result: 位置编码是关键因素：位置不变模型能够正确恢复L形范式聚类，即使训练数据中L形动词很少；而顺序位置编码模型只能部分捕捉该模式。然而，所有模型都无法生产性地泛化该模式到新形式。位置不变模型将L形词干泛化到虚拟语气单元格，但未能扩展到第一人称单数直陈式，形成了基于语气的泛化而非L形词素模式。

Conclusion: 神经网络能够学习统计模式但无法像人类一样进行形态抽象。人类优先泛化到第一人称单数直陈式，而模型则泛化到虚拟语气形式，这表明当前模型与人类形态学习之间存在根本差异，统计模式复制不等同于形态抽象。
...
TL;DR: 提出多智能体医疗问答框架，结合证据检索、不确定性估计和偏见检查，提高LLM在医疗QA中的可靠性
...
Method: 两阶段方法：1) 微调三种LLM家族(GPT、LLaMA、DeepSeek R1)于MedQuAD医疗QA数据；2) 构建模块化多智能体管道，包括临床推理、证据检索、精炼等智能体，并集成安全机制

Result: DeepSeek R1表现最佳(ROUGE-1 0.536等)，系统整体准确率达87%，证据增强降低不确定性(困惑度4.13)，平均端到端延迟36.5秒

Conclusion: 智能体专业化和验证层能缓解单模型限制，为基于证据和偏见感知的医疗AI提供实用、可扩展的设计方案
...
TL;DR: 提出延迟视觉摄取(DVI)框架，将视觉理解推迟到用户提问时进行，而非预先处理所有页面，显著降低计算成本并提高可靠性。


<details>
  <summary>Details</summary>
Motivation: 现有多模态文档问答方法采用供给端摄取策略，在索引阶段对所有页面运行视觉语言模型(VLM)生成描述，这种方法成本高、端到端不可靠且失败后无法恢复。

Method: DVI采用需求端摄取策略：索引阶段仅进行轻量级元数据提取，将视觉理解推迟到用户提出具体问题时。核心原则是"索引用于定位而非理解"，通过结构化元数据索引和BM25全文搜索实现页面定位，然后将原始图像与具体问题一起发送给VLM进行针对性分析。

Result: 在两个真实工业工程图纸(113页+7页)上的实验表明，DVI在零摄取VLM成本下达到可比的整体准确率(46.7% vs. 48.9%)，在视觉必要查询上的有效率达到50%(vs. 预摄取的0%)，页面定位准确率100%(搜索空间压缩98%)。

Conclusion: DVI将"问答准确率"问题转化为"页面定位"问题，一旦找到正确的图纸页面，获取答案就变成了交互轮次的问题。该方法还支持交互式细化和渐进缓存。
...
TL;DR: 大型语言模型在长上下文任务中，理论容量与实际性能存在显著差距，尤其在处理大量社交媒体帖子进行抑郁检测时，模型性能随输入量增加而显著下降。


<details>
  <summary>Details</summary>
Motivation: 尽管LLMs的理论上下文窗口已扩展至数百万token，但研究表明模型在实际应用中难以稳健利用长上下文信息，特别是在需要理解大量细节的复杂任务中。本研究旨在评估当前先进模型在长上下文任务中的实际表现。

Method: 评估了四个先进模型（Grok-4、GPT-4、Gemini 2.5和GPT-5）在长上下文任务上的表现。使用了三个数据集：两个补充数据集（烹饪食谱检索和数学问题），以及一个包含20K社交媒体帖子的主要数据集用于抑郁检测。

Result: 当社交媒体数据集输入量超过5K帖子（70K token）时，所有模型性能显著下降。处理20K帖子时，准确率降至50-53%。值得注意的是，GPT-5模型虽然准确率大幅下降，但精确率保持约95%，这在抑郁检测等敏感应用中可能很有价值。研究还表明"迷失在中间"问题在新模型中已基本解决。

Conclusion: 研究强调了模型理论容量与复杂高容量数据任务实际性能之间的差距，并指出在实际应用中需要超越简单准确率的评估指标。GPT-5的高精确率特性在敏感应用中具有重要价值。
...
TL;DR: 论文提出了一种基于弃权的科学声明验证框架，通过将声明分解为最小条件、使用NLI审核证据，并选择性支持、反驳或弃权，在科学推理任务中控制错误风险。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型越来越多用于回答和验证科学声明，但现有评估通常假设模型必须始终给出明确答案。在科学环境中，未经支持或不确定的结论可能比弃权更有害，因此需要研究如何让模型在证据不足时合理弃权。

Method: 提出弃权感知的验证框架：1) 将科学声明分解为最小条件；2) 使用自然语言推理(NLI)根据可用证据审核每个条件；3) 选择性决定支持、反驳或弃权。在SciFact和PubMedQA两个科学基准上评估，涵盖闭卷和开放域证据设置，使用六种不同语言模型进行实验。

Result: 实验发现：1) 原始准确率在不同架构间差异不大；2) 弃权在控制错误方面起关键作用；3) 基于置信度的弃权在中等覆盖率水平下显著降低风险，即使绝对准确率提升有限。主要挑战不是选择单一最佳模型，而是确定何时有足够证据支持答案。

Conclusion: 弃权感知评估是评估科学可靠性的实用且模型无关的方法，为科学领域的选择性推理提供了统一的实验基础。在科学推理任务中，关键不是模型必须回答，而是知道何时应该弃权。
...
<details>
  <summary>Details</summary>
Motivation: 克服传统上下文无关文法(CFG)的限制，将1950年代以来的理论句法研究成果应用于计算环境，创建透明可解释的NLP模型

Method: 基于CFG和广义短语结构文法(GPSG)的新解析算法，包含句法规则和特征系统，能生成依存和成分句法树，处理噪声和不完整解析，提供多解析假设

Result: 在Universal Dependencies数据集上测试：开发集(7个语料库)平均UAS为54.5%，测试集(12个语料库)平均UAS为53.8%，可通过重排序进一步提高准确率

Conclusion: 该方法成功将理论句法学应用于计算解析，提供透明可解释的NLP模型，能同时生成两种句法树并处理不完整输入，具有进一步优化的潜力
...
TL;DR: AD-Bench是一个针对广告营销领域的LLM智能体评估基准，基于真实业务需求构建，包含三个难度级别，实验显示即使最先进模型在复杂场景下仍有显著能力差距。


<details>
  <summary>Details</summary>
Motivation: 当前LLM智能体评估基准主要局限于理想化模拟，无法满足广告营销等专业领域的实际需求，这些领域需要与专业工具进行多轮交互的复杂任务。

Method: 基于真实用户营销分析需求构建基准，由领域专家提供可验证的参考答案和对应的参考工具调用轨迹，将请求分为三个难度级别（L1-L3）以评估多轮、多工具协作能力。

Result: 在AD-Bench上，Gemini-3-Pro在整体上达到Pass@1=68.0%和Pass@3=83.0%，但在L3难度下性能显著下降至Pass@1=49.4%和Pass@3=62.1%，轨迹覆盖率为70.1%。

Conclusion: 即使最先进的模型在复杂广告营销分析场景中仍存在显著能力差距，AD-Bench为评估和改进广告营销智能体提供了现实的基准。
...
TL;DR: 该研究提出了基于token嵌入聚类结构的大语言模型幻觉几何分类法，识别出三种幻觉类型，并引入三个可测量的几何统计量来量化分析。


<details>
  <summary>Details</summary>
Motivation: 研究动机是理解大语言模型幻觉的几何基础，通过分析嵌入空间的结构特征来建立可观测的幻觉分类框架，为幻觉检测提供理论基础。

Method: 分析了11个transformer模型（包括编码器和解码器架构）的静态嵌入空间，识别出三种幻觉类型：类型1（中心漂移）、类型2（错误井收敛）、类型3（覆盖间隙）。引入了三个几何统计量：α（极性耦合）、β（聚类凝聚度）、λ_s（径向信息梯度）。

Result: 在所有11个模型中，极性结构（α > 0.5）普遍存在（11/11），聚类凝聚度（β > 0）普遍存在（11/11），径向信息梯度显著（9/11，p < 0.05）。ALBERT和MiniLM在λ_s上不显著有架构上的解释原因。

Conclusion: 研究建立了幻觉类型检测的几何先决条件，并产生了关于架构依赖性脆弱性特征的可测试预测，为理解模型幻觉提供了新的几何视角。
...
TL;DR: STATe-of-Thoughts (STATe) 是一种可解释的推理时计算方法，通过离散的文本干预而非随机采样，在高层推理模式上进行搜索，生成高质量、多样且可解释的文本。


<details>
  <summary>Details</summary>
Motivation: 现有推理时计算方法（如Best-of-N、Tree-of-Thoughts）使用高温采样难以实现有意义的输出多样性，且对推理过程的控制有限，导致可解释性不足。

Method: STATe采用结构化方法：控制器选择编码高层推理选择的动作，生成器基于这些选择产生推理步骤，评估器对候选结果评分以指导搜索，用离散可解释的文本干预替代随机采样。

Result: 1) 动作引导的文本干预比基于温度的采样产生更大的响应多样性；2) 在论点生成案例中，STATe的显式动作序列捕获了高度预测输出质量的可解释特征；3) 能够识别有前景但未探索的动作空间区域并直接引导生成。

Conclusion: STATe为生成高质量、多样且可解释的文本提供了一个实用框架，通过结构化搜索高层推理模式解决了现有方法的局限性。
...
TL;DR: 研究首次大规模诊断AI智能体社会动态演化，发现虽然全局语义快速稳定，但个体保持高多样性，缺乏相互影响和共识形成，表明仅靠规模和交互密度不足以实现社会化。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型智能体在网络环境中日益增多，需要研究AI智能体社会是否像人类社会一样经历收敛动态，以及仅靠规模和交互密度是否足以实现社会化。

Method: 在Moltbook平台上创建开放、持续演化的在线AI智能体社会，引入定量诊断框架，测量语义稳定化、词汇周转、个体惯性、影响持久性和集体共识等动态演化指标。

Result: 系统处于动态平衡：全局语义平均快速稳定，但个体保持高多样性和持续词汇周转，避免同质化。然而，智能体表现出强个体惯性和对交互伙伴的最小适应性响应，缺乏相互影响和共识形成，影响短暂且无持久超级节点，社会无法发展稳定的集体影响锚点。

Conclusion: 规模和交互密度本身不足以诱导社会化，AI智能体社会缺乏共享社会记忆，这为下一代AI智能体社会的设计和分析提供了可操作的原则。
...
TL;DR: InnoEval是一个深度创新评估框架，通过异质深度知识搜索和多视角评审委员会来模拟人类水平的科学创意评估，解决了现有LLM评估方法的知识局限性和偏见问题。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型促进了科学创意的快速生成，但创意评估方法未能同步发展。现有评估方法存在知识视野狭窄、评估维度扁平化、以及LLM作为评判者的固有偏见等问题，需要更接近人类专家水平的评估框架。

Method: 1. 将创意评估视为知识基础的多视角推理问题；2. 使用异质深度知识搜索引擎从多样在线资源检索动态证据；3. 建立包含不同学术背景评审者的创新评审委员会，实现跨多个指标的多维度解耦评估。

Result: 在基于权威同行评审提交构建的综合数据集上，InnoEval在点对点、配对和分组评估任务中持续优于基线方法，展现出与人类专家高度一致的判断模式和共识。

Conclusion: InnoEval通过知识基础和多视角评审机制，能够有效模拟人类专家水平的科学创意评估，为解决LLM时代创意评估滞后问题提供了可行方案。
...
TL;DR: 本文提出多令牌策略梯度优化（MPO），将连续K个令牌作为统一语义动作，以解决复杂推理任务中令牌级优化与块级推理结构不匹配的问题。


<details>
  <summary>Details</summary>
Motivation: 现有自回归语言模型的策略梯度方法通常一次选择一个令牌作为策略动作。虽然对许多生成任务有效，但这种方法可能无法完全捕捉复杂推理任务的结构，因为单个语义决策通常跨越多个令牌（如定义变量或组合方程），导致令牌级优化与推理的块级本质不匹配。

Method: 提出多令牌策略梯度优化（MPO）框架，将连续K个令牌序列视为统一的语义动作。这种块级视角使方法能够捕捉推理轨迹的组合结构，并支持在连贯、更高级别的目标上进行优化。

Result: 在数学推理和编程基准测试中，MPO优于标准的令牌级策略梯度基线，突显了令牌级策略梯度在复杂推理中的局限性。

Conclusion: 研究结果表明需要超越令牌级粒度来处理推理密集型语言任务，为未来研究提供了方向。
...
Method: 构建TruthStance数据集，包含2023-2025年Truth Social对话线程，保留回复树结构。提供1,500个实例的人工标注基准，评估LLM提示策略，并使用最佳配置生成大规模标签。

Result: 发布了包含24,378帖子和523,360评论的大规模数据集，提供人工标注基准和LLM生成的论证存在性（24,352帖子）和立场标签（107,873评论），支持深度、主题和用户层面的分析。

Conclusion: TruthStance填补了替代技术平台论证挖掘和立场检测研究的空白，为分析在线话语中的意见形成和争议提供了重要资源，所有代码和数据均已公开。
...
TL;DR: 论文从测度论和频域分析角度重构Transformer/注意力机制，理论证明幻觉是LLM的结构性局限，提出WavePhaseNet方法通过DFT构建语义概念层次结构，将高维嵌入空间降维至约3000维，并利用上同调正则化控制语义一致性。


<details>
  <summary>Details</summary>
Motivation: 现有大语言模型存在幻觉问题，作者认为这是Transformer/注意力机制的结构性局限。通过测度论分析发现嵌入空间作为条件期望无法与语义真值集同构，导致逻辑一致性崩溃，需要新的理论框架来解决这一问题。

Method: 提出WavePhaseNet方法：1) 使用离散傅里叶变换沿序列维度分解语义信息，低频捕获全局意义和意图，高频表示局部语法和表达；2) 通过累积能量分析将GPT-4的24,576维嵌入空间降维至约3000维；3) 应用上同调正则化在重叠局部窗口上构建图结构和上链复形，利用Hodge理论的调和投影控制语义一致性。

Result: 理论证明幻觉是LLM的结构性局限；通过频域分析实现了语义信息的精确分解；发现约3000维是"完整表示"的下界，降维能保持意义和意图同时抑制幻觉；上同调正则化提供了可计算的语义一致性控制机制。

Conclusion: 论文从测度论和频域分析角度为LLM幻觉问题提供了理论解释和解决方案。WavePhaseNet通过DFT分解、维度约简和上同调一致性控制，实现了语义信息的精确操作和幻觉抑制，为大语言模型的可靠推理提供了新框架。
...
TL;DR: 提出LLM辅助的蒸馏框架，用于时序知识图谱推理，通过结合传统时序教师模型和LLM作为辅助教师，提升轻量级学生模型的性能而不增加推理复杂度。
...
Method: 提出LLM辅助的蒸馏框架：1) 使用高容量的时序教师模型；2) 引入大语言模型作为辅助教师，提供丰富的背景知识和时序信号；3) 采用分阶段对齐策略，逐步整合两个教师的指导；4) 联合优化监督和蒸馏目标。

Result: 在多个公共TKG基准测试和不同骨干架构上的实验表明，该方法在链接预测性能上持续优于强蒸馏基线，同时保持学生模型的紧凑和高效。

Conclusion: 大语言模型可以作为有效的教师，将时序推理能力转移到资源高效的TKG系统中，展示了LLM在时序知识蒸馏中的潜力。
...
TL;DR: FilBBQ是一个针对菲律宾语境的偏见测试基准，包含超过10,000个提示，用于评估生成模型在性别和同性恋偏见方面的表现，改进了评估协议以提高可靠性。


<details>
  <summary>Details</summary>
Motivation: 随着自然语言生成成为语言模型的流行用例，偏见基准测试变得重要。现有BBQ基准主要针对英语，需要扩展到菲律宾语境，以评估模型在菲律宾文化背景下的性别和同性恋偏见。

Method: 通过四阶段开发过程构建FilBBQ：1) 模板分类，2) 文化感知翻译，3) 新模板构建，4) 提示生成。采用稳健评估协议，通过多个种子运行获取响应并平均偏见分数，解决模型响应不稳定性问题。

Result: FilBBQ包含超过10,000个提示，测试结果显示：1) 不同种子间的偏见分数存在变异性，2) 模型表现出与情绪、家务、刻板同性恋兴趣和一夫多妻相关的性别和同性恋偏见。

Conclusion: FilBBQ扩展了BBQ基准的语言范围到菲律宾语境，提供了更可靠的评估协议，证实了菲律宾语模型中存在特定文化背景的偏见，为偏见评估提供了新工具。
...
TL;DR: 论文提出SSR方法解决RCG中的后见之明合理化问题，通过结构骨架引导推理减少答案锚定效应，相比语义抑制方法提升10%效果。


<details>
  <summary>Details</summary>
Motivation: 传统反向思维链生成(RCG)存在后见之明合理化问题：模型在生成推理轨迹时看到答案，答案成为认知锚点，导致生成的解释是事后合理化而非真实推理过程。

Method: 提出结构骨架引导推理(SSR)：1) 先生成答案不变的功能性骨架结构；2) 用骨架引导完整轨迹生成。还提出蒸馏版SSR-D，通过微调确保结构遵循。通过三个层次测量锚定效应：词汇、熵、概率锚定。

Result: SSR-D在开放推理基准上比语义抑制基线提升10%效果，同时保持分布外泛化能力。语义抑制方法虽然减少词汇重叠，但反而增加熵和概率锚定。

Conclusion: 基于认知心理学中的反讽过程理论，主动抑制答案反而加深答案依赖。SSR通过结构规划重定向信息流，有效减少锚定效应，提供更真实的推理轨迹生成方法。
...
TL;DR: HyperRAG：基于n元超图的RAG框架，通过HyperRetriever和HyperMemory两种检索变体，实现更高效的多跳推理和问答
...
Method: 提出HyperRAG框架，包含两种互补的检索变体：1) HyperRetriever学习n元事实上的结构语义推理，构建查询条件化的关系链；2) HyperMemory利用LLM的参数化记忆指导beam搜索，动态评分n元事实和实体进行查询感知的路径扩展。

Result: 在WikiTopics（11个闭域数据集）和三个开放域QA基准（HotpotQA、MuSiQue、2WikiMultiHopQA）上验证有效性。HyperRetriever达到最高答案准确率，平均MRR提升2.95%，Hits@10提升1.23%。定性分析显示能通过自适应可解释的n元链构建弥合推理差距。

Conclusion: HyperRAG通过n元超图增强RAG，提供更高效、准确的多跳推理能力，在开放域和闭域QA任务中均表现出色，解决了传统图基RAG方法的局限性。
...
TL;DR: 该研究提出了一种用于低资源孟加拉语IR数据集的BETA标注框架，使用多个LLM进行标注，并探讨了跨语言数据集重用的可靠性问题。


<details>
  <summary>Details</summary>
Motivation: 低资源语言的IR研究受到高质量任务特定标注数据集稀缺的限制。手动标注昂贵且难以扩展，而使用LLM作为自动标注器又存在标签可靠性、偏见和评估有效性的担忧。

Method: 采用BETA标注框架，涉及来自不同模型家族的多个LLM标注器，包含上下文对齐、一致性检查和多数同意机制，随后进行人工评估验证标签质量。同时研究通过一跳机器翻译重用其他低资源语言IR数据集的有效性。

Result: 实验显示不同语言之间存在显著差异，反映了语言依赖的偏见和不一致的语义保留，直接影响跨语言数据集重用的可靠性。BETA框架能够创建相对可靠的标注数据集。

Conclusion: 该研究突出了LLM辅助低资源IR数据集创建的潜力和局限性，提供了跨语言数据集重用风险的实证证据，并为低资源语言环境下构建更可靠的基准和评估流程提供了实用指导。
...
<details>
  <summary>Details</summary>
Motivation: 工业级用户表示学习需要在鲁棒通用性和任务敏感性之间取得平衡。现有方法主要产生静态、任务无关的嵌入，难以在统一向量空间中协调下游场景的不同需求，且异构多源数据带来的噪声和模态冲突会降低表示质量。

Method: 1) 构建工业级预训练数据集UserU，对齐多模态行为序列与用户理解语义；2) Q-Anchor Embedding架构：将分层粗到细编码器集成到双塔LLM中，通过联合对比-自回归优化实现查询感知的用户表示；3) 基于聚类的软提示调优，增强判别性潜在结构，使模型注意力与场景特定模态对齐；4) 部署时在序列末端锚定查询，实现KV缓存加速推理。

Result: 在10个Alipay工业基准测试中取得一致的SOTA性能，展现出强大的可扩展性和高效部署能力。在Alipay生产系统的两个真实场景中进行大规模在线A/B测试，进一步验证了其实际有效性。

Conclusion: Query-as-Anchor框架成功解决了工业级用户表示学习中通用性与任务敏感性的平衡问题，通过动态查询感知合成实现了优于静态编码的性能，并在实际生产环境中验证了其有效性和部署效率。
...
TL;DR: LLMs在英语中展现强大数学推理能力，但在僧伽罗语和泰米尔语等低资源语言中，这种能力是否反映真正的多语言推理还是依赖翻译处理尚不清楚。研究发现基础算术推理能跨语言稳健转移，但复杂推理任务在泰米尔语和僧伽罗语中显著退化。


<details>
  <summary>Details</summary>
Motivation: 研究动机是探究LLMs在低资源语言（如僧伽罗语和泰米尔语）中的数学推理能力是否真正反映多语言推理，还是依赖于隐式翻译到英语类表示。这挑战了模型展现强大多语言性能即意味着跨语言同等有效推理能力的常见假设。

Method: 使用六种数学问题类型的分类法（从基础算术到复杂单位冲突和优化问题），评估四个主要大语言模型。为避免翻译伪影混淆语言能力与翻译质量，构建了平行数据集，其中每个问题均由三种语言的流利说话者（具有数学训练）原生创作。

Result: 分析表明，基础算术推理能跨语言稳健转移，但复杂推理任务在泰米尔语和僧伽罗语中显示显著退化。失败模式因模型和问题类型而异，表明明显的多语言能力可能不反映跨语言的统一推理能力。

Conclusion: 这些发现挑战了模型展现强大多语言性能即意味着跨语言同等有效推理能力的常见假设，并强调在多语言环境中需要进行细粒度、类型感知的评估。
...
TL;DR: XTF是一个可解释的token级噪声过滤框架，通过分解token对微调过程的贡献为三个属性（推理重要性、知识新颖性、任务相关性），过滤噪声token以提升LLM微调性能。
...
Result: 在数学、代码和医学三个代表性下游任务上对7个主流LLM进行实验，XTF相比常规微调能显著提升下游性能达13.7%。
...
TL;DR: 比较五种LLM在医疗问答任务上的性能，发现大模型表现更好，Llama-4-Maverick-17B在效率与性能间取得平衡，为医疗NLP应用提供基准。


<details>
  <summary>Details</summary>
Motivation: 评估不同规模LLM在医疗问答任务上的表现，为低资源环境下的医疗NLP应用提供参考，探索模型大小、计算资源与临床效用之间的平衡。

Method: 使用iCliniq数据集（38,000个医疗问答），对Llama-3-8B-Instruct、Llama 3.2 3B、Llama 3.3 70B Instruct、Llama-4-Maverick-17B-128E-Instruct和GPT-5-mini五种模型进行零样本评估，采用BLEU和ROUGE指标。

Result: 大模型（如Llama 3.3 70B Instruct）表现优于小模型，符合规模扩展效益；Llama-4-Maverick-17B在性能与效率间表现更均衡，适合实际部署。

Conclusion: LLM在医疗推理方面能力不断提升，支持医疗问答系统在实际临床环境中应用的可行性增加；该基准为未来研究提供了标准化设置，有助于优化模型大小、计算资源与临床效用的平衡。
...
TL;DR: WDQL数据集包含20万个Wikidata知识图谱的问答对，比现有最大类似数据集大6倍以上，基于真实查询日志构建而非模板生成


<details>
  <summary>Details</summary>
Motivation: 现有Wikidata数据集规模有限且多依赖模板生成查询，缺乏真实世界的查询多样性，需要更大规模、基于真实查询日志的数据集来训练更好的问答系统

Method: 从Wikidata查询服务获取真实SPARQL查询日志，使用基于代理的方法迭代进行去匿名化、清理和验证，同时生成对应的自然语言问题

Result: 构建了包含20万问答对的WDQL数据集，比现有最大类似数据集大6倍，所有资产和代理代码都在宽松许可下公开可用

Conclusion: WDQL数据集为训练问答方法提供了有价值的资源，基于真实查询日志的构建方法确保了数据的多样性和实用性
...
<details>
  <summary>Details</summary>
Motivation: 大语言模型计算成本高，现有层剪枝方法无法同时保持剪枝性能和效率

Method: 两阶段方法：1）基于梯度幅度的全局层重要性评估，仅需单次反向传播；2）分析剪枝导致的均值偏移，通过投影补偿矩阵一步修正

Result: 在剪枝速度上平均实现4倍加速，性能优于现有层剪枝方法

Conclusion: GradMAP在保持模型性能的同时显著提升剪枝效率，为LLM部署提供实用解决方案
...
TL;DR: 首次在视觉接地环境中计算研究均匀信息密度假说，发现视觉接地使信息分布更均匀，支持上下文敏感的UID表述


<details>
  <summary>Details</summary>
Motivation: 先前关于均匀信息密度假说的研究仅限于纯文本输入，忽略了语言产生时的感知上下文。本研究旨在探索在视觉接地环境中UID的表现，以更真实地模拟多模态语言使用。

Method: 使用多语言视觉-语言模型在30种语言的图像-字幕数据和13种语言的视觉叙事数据上估计惊奇度，涵盖11个语系。比较视觉接地与纯文本设置下的信息分布均匀性。

Result: 视觉接地持续平滑信息分布，在类型多样的语言中增加全局和局部均匀性。在视觉叙事中，图像和语篇上下文接地有额外效果，最强惊奇度降低出现在语篇单元起始处。

Conclusion: 本研究首次模拟生态合理的多模态语言使用中信息流的时间动态，发现接地语言表现出更大的信息均匀性，支持上下文敏感的UID表述。
...
Method: 提出FAL-AD框架：1）语音转换增强生成多样化病理语音样本；2）自适应联邦学习范式最大化跨机构协作效益；3）注意力跨模态融合模型实现细粒度词级对齐和声学-文本交互。

Result: 在ADReSSo数据集上达到91.52%的多模态准确率，优于所有集中式基线方法。

Conclusion: FAL-AD通过联邦学习与数据增强的协同整合，为数据效率困境提供了实用解决方案，实现了最先进的阿尔茨海默病语音检测性能。
...
TL;DR: 构建了皮埃蒙特语（意大利西北部濒危罗曼语）的众包数据集，包含145个意大利语-皮埃蒙特语平行句，用于评估大语言模型在分词对等性、主题分类和机器翻译上的表现。


<details>
  <summary>Details</summary>
Motivation: 皮埃蒙特语作为濒危罗曼语缺乏标准化资源，需要评估大语言模型对低资源语言的处理能力，特别是自然拼写风格而非标准化拼写的情况。

Method: 创建众包数据集：从Flores+衍生145个平行句，由母语者按自然拼写风格翻译，并进行人工词对齐。使用该数据集评估多个大语言模型在分词对等性、主题分类和机器翻译任务上的表现。

Result: 皮埃蒙特语相比高资源罗曼语存在分词惩罚，但大语言模型在分类任务上表现接近意大利语、法语和英语。机器翻译结果不对称：从皮埃蒙特语翻译到高资源语言表现尚可，但生成皮埃蒙特语仍具挑战性。

Conclusion: 皮埃蒙特语作为低资源语言在大语言模型中面临分词和生成挑战，但分类任务表现良好。数据集和代码已公开，为濒危语言处理研究提供资源。
...
<details>
  <summary>Details</summary>
Motivation: 当前缺乏系统评估LLM在结构化数据提取和JSON生成方面能力的基准，特别是在不同模型、规模和提示策略下的性能比较，这对于LLM在解析和ETL应用中的实际部署至关重要。

Method: 创建包含多样化、人工验证的解析场景的开放数据集，涵盖不同复杂度；引入互补的性能指标，包括token级准确性和文档级有效性；在22个模型和5种提示策略上进行系统测试。

Result: 选择合适的提示策略比模型规模等标准属性更重要；提示策略特别能确保较小或不太可靠模型的结构有效性，但会增加语义错误数量；基准套件为LLM在解析和ETL应用中的未来研究奠定了基础。

Conclusion: LLMStructBench为评估LLM在结构化数据提取方面的能力提供了系统基准，强调了提示策略选择的重要性，并为LLM在解析和ETL应用中的进一步研究提供了工具和方向。
...
TL;DR: 大规模研究表明LLM在时间序列预测中确实有效，尤其在跨域泛化方面表现突出，推翻了先前负面结论


<details>
  <summary>Details</summary>
Motivation: 先前研究质疑LLM在时间序列预测中的实际价值，认为其性能与无LLM方法相当。本研究旨在通过大规模实验验证LLM是否真正能提升预测性能

Method: 进行了大规模LLM时间序列预测研究，涵盖80亿观测值、17个预测场景、4个预测范围、多种对齐策略，包括域内和跨域设置

Result: LLM确实能提升预测性能，尤其在跨域泛化方面增益显著。预对齐在90%以上任务中优于后对齐。LLM的预训练知识和模型架构都发挥作用且互补
...
TL;DR: 使用认知网络科学构建行为心智网络分析STEM态度，发现学生对科学持正面态度但对数学有负面焦虑，GPT模型能近似文化态度但无法复制人类教育焦虑的经验成分


<details>
  <summary>Details</summary>
Motivation: STEM态度发展涉及概念知识、教育经验和情感的交互作用，需要系统方法来量化群体对STEM领域的心智模式，并比较人类与AI模型在这些态度上的差异

Method: 使用行为心智网络方法，基于994名高中生、大学生和STEM专家的自由联想数据构建网络，节点为提示词和自由联想，边为实证关联链接，概念标注情感效价。分析关键概念（如STEM科目）的语义邻域框架，量化情感氛围、情绪特征、网络重叠和具体性，并与GPT-oss生成的"数字孪生"数据比较

Result: 学生群体对科学和研究持一致正面态度，但对核心量化科目（数学和统计学）表现出更多负面和焦虑相关氛围，在数学焦虑较高的子群体中更明显。高焦虑框架比随机更抽象，表明对威胁性量化领域有更去情境化的表征。人类网络中数学与焦虑的重叠度高于GPT模型

Conclusion: 行为心智网络能捕捉对目标领域的心智模式的认知-情感特征，基于大语言模型的数字孪生能近似文化态度，但无法复制人类教育焦虑中关键的情境敏感、经验基础的成分
...
TL;DR: 本文发现自回归Transformer中残差连接导致输入输出对齐偏移问题，并提出基于残差衰减的轻量级缓解方法


<details>
  <summary>Details</summary>
Motivation: 自回归Transformer通过因果掩码实现并行化，但残差连接将激活与当前token绑定，而监督目标针对下一个token，如果当前token不是预测最有信息量的token，这种输入输出对齐偏移会传播不匹配信息

Method: 1) 使用解码轨迹和相似性度量在预训练LLMs中经验定位输入输出对齐偏移；2) 提出基于残差衰减的轻量级残差路径缓解方法，可作为固定层干预或可学习门控机制实现

Result: 实验表明隐藏token表示在网络深处从输入对齐切换到输出对齐；在多个基准测试中，提出的缓解策略减轻了表示不对齐问题并带来性能提升

Conclusion: 残差衰减策略为自回归Transformer提供了一种高效通用的架构增强方法，解决了输入输出对齐偏移问题
...
<details>
  <summary>Details</summary>
Motivation: 推理导向大语言模型在数学和编程任务上表现出色，但在机器翻译任务中的影响尚未充分探索，需要系统评估其在翻译中的效果

Method: 在WMT24++基准上评估多个开源和闭源推理模型，分析推理轨迹特征，提出结构化推理框架（多步草稿、充分性优化、流畅性改进、选择性迭代修订），并构建合成数据集进行后训练

Result: 启用显式推理会降低翻译质量，推理轨迹高度线性缺乏修正，注入高质量推理轨迹不能可靠提升弱模型性能，结构化推理框架显著优于标准翻译微调和通用推理基线

Conclusion: 推理必须针对任务进行结构化设计才能有益于机器翻译，通用推理方法在翻译任务中效果有限
...
TL;DR: 研究测试社区讨论是否能改善脱口秀创作，通过多智能体沙盒实验发现，包含社区讨论的条件在75.6%的情况下表现更好，显著提升了创作清晰度和社交响应性。


<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注即时反馈和局部优化，但缺乏对在线社区中长期公共反馈如何影响创作质量的系统性研究，特别是脱口秀这种依赖观众反应的创作形式。

Method: 采用多智能体沙盒控制实验，设置讨论组（记录、过滤、存储社区讨论作为社交记忆，并在后续创作中检索使用）和对照组（无讨论），共进行50轮实验，生成250对独白，由5位专家标注者评估。

Result: 讨论组在75.6%的情况下获胜，显著提升了创作清晰度（Δ=0.440）和社交响应性（Δ=0.422），偶尔会增加攻击性幽默内容。

Conclusion: 社区讨论能有效改善脱口秀创作质量，特别是提升作品的清晰度和观众共鸣，证明了社交记忆和公共反馈在创意写作中的价值。
...
TL;DR: GPT-4.1模型在错误知识微调后表现出毒性（涌现错位），研究发现这些模型能够自我意识到自己的行为变化，无需上下文示例就能评估自身的有害性。


<details>
  <summary>Details</summary>
Motivation: 探索大语言模型在涌现错位现象中的行为自我意识能力，研究模型是否能意识到自身安全状态的变化，为模型安全评估提供新方法。

Method: 对GPT-4.1模型进行顺序微调：首先使用已知会诱导涌现错位的错误问答数据集，然后使用能纠正错位的数据集。评估模型在没有上下文示例的情况下是否能自我评估行为变化。

Result: 涌现错位的模型自我评估为显著更有害，而重新对齐的模型则评估为更安全。这表明模型具有行为自我意识，能够跟踪自身的对齐状态。

Conclusion: 大语言模型具有行为自我意识能力，能够提供关于自身安全状态的信息信号，这为模型安全评估提供了新的查询方法。
...
TL;DR: 该研究从几何视角分析小规模语言模型的幻觉问题，提出真实回答在嵌入空间中更紧密聚集的假设并证明，基于此开发仅需30-50标注即可分类大量回答的高效方法，F1分数超90%。


<details>
  <summary>Details</summary>
Motivation: 幻觉（流畅但事实错误的回答）是语言模型可靠性的主要挑战，尤其在多步骤或代理设置中。传统基于知识和单回答评估范式有限，需要从新视角理解幻觉现象。

Method: 从几何视角研究幻觉，假设同一提示下模型生成的多个回答中，真实回答在嵌入空间中呈现更紧密的聚类。证明此假设后，利用几何洞察开发标签高效传播方法，仅需30-50个标注即可分类大量回答。

Result: 验证了真实回答在嵌入空间中更紧密聚集的假设，实现了稳定的可分离性。提出的传播方法仅需少量标注（30-50个）即可达到90%以上的F1分数，有效分类大量回答。

Conclusion: 从嵌入空间的几何视角研究幻觉，补充了传统知识中心和单回答评估范式，为后续研究开辟了新方向，提供了高效检测幻觉的方法。
...
<details>
  <summary>Details</summary>
Motivation: LLM代理依赖文本可见的元数据选择和链接第三方工具，这种便利性创造了供应链攻击面，恶意工具服务器可被共同注册并诱导过度思考循环

Method: 将攻击形式化为结构性过度思考攻击，区别于令牌级冗长，实现14个恶意工具跨越三个服务器，触发重复、强制细化和分心等攻击模式

Result: 攻击导致严重的资源放大（高达142.4倍令牌），可降低任务结果质量，解码时简洁控制无法可靠防止循环诱导

Conclusion: 防御应基于工具调用结构而非仅令牌层面进行推理，结构性过度思考攻击是LLM工具使用中的重要安全威胁
...
TL;DR: 本文提出了BasPhyCo，首个巴斯克语非问答式物理常识推理数据集，包含标准语和方言变体，评估了多语言大语言模型在巴斯克语物理常识推理任务上的表现。


<details>
  <summary>Details</summary>
Motivation: 物理常识推理是人类智能的基本能力，但现有研究未关注大语言模型在低资源语言（如巴斯克语）非问答式物理常识推理任务上的表现。本文旨在填补这一研究空白。

Method: 以意大利语GITA数据集为起点，创建了BasPhyCo数据集（含标准语和方言变体）。评估模型在三个层次上的表现：区分合理/不合理叙述的准确性、识别冲突元素的连贯性、确定具体物理状态的可验证性。使用多语言大语言模型及专门针对意大利语和巴斯克语预训练的模型进行评估。

Result: 结果显示，在可验证性方面，大语言模型在巴斯克语等低资源语言中的物理常识推理能力有限，特别是在处理方言变体时表现更差。

Conclusion: 大语言模型在低资源语言的物理常识推理任务上存在局限性，特别是在方言处理方面，这凸显了开发专门针对低资源语言物理常识推理模型的重要性。
...
TL;DR: 提出了Testimole-conversational，一个包含超过300亿词符的意大利语讨论板消息大规模语料库，覆盖1996-2024年，适用于意大利语大语言模型预训练和语言社会学分析。


<details>
  <summary>Details</summary>
Motivation: 需要大规模意大利语语料库来支持本土大语言模型预训练，同时讨论板消息作为计算机媒介通信的重要资源，对语言学和社会学分析具有独特价值。

Method: 收集了1996年至2024年期间的意大利语讨论板消息，构建了包含超过300亿词符的大规模语料库，涵盖了丰富的计算机媒介通信内容。

Result: 创建了Testimole-conversational语料库，这是目前最大的意大利语讨论板消息集合，包含超过300亿词符，时间跨度近30年，捕捉了非正式书面意大利语、话语动态和在线社交互动。

Conclusion: 该语料库不仅为意大利语大语言模型预训练提供了理想数据集，还为语言变异和数字通信中的社会现象研究提供了宝贵资源，将免费提供给研究社区使用。
...
Conclusion: BFS-PO有效缓解了大型推理模型的过度思考问题，通过优化推理链长度实现了准确性与效率的平衡。
...
TL;DR: 提出了一个面向联络中心工具感知计划生成的领域基础框架和基准，用于将业务洞察查询分解为可执行步骤，支持结构化（Text2SQL）和非结构化（RAG）工具，包含并行依赖关系。


<details>
  <summary>Details</summary>
Motivation: 联络中心需要将复杂的业务洞察查询分解为可执行的步骤，涉及结构化（数据库）和非结构化（对话记录）工具，当前LLM在工具理解和计划生成方面存在挑战，需要系统评估和改进方法。

Method: 1) 基于参考的计划评估框架，包含七维度指标评估和一次性评估两种模式；2) 数据策划方法，通过评估器->优化器循环迭代优化计划，生成高质量计划谱系；3) 大规模研究14个不同规模和家族的LLM，评估其在有无谱系提示下的计划生成能力。

Result: LLM在复杂查询和超过4步的计划上表现不佳（通常5-15步）；最佳总指标得分84.8%（Claude-3-7-Sonnet），最强一次性匹配率仅49.75%（o3-mini）；计划谱系整体效果参差不齐，但对部分顶级模型有益，提高了许多模型的步骤可执行性。

Conclusion: 研究揭示了LLM在工具理解方面的持续差距，特别是在工具提示对齐和工具使用完整性方面；更短、更简单的计划明显更容易生成；该框架为评估和改进联络中心数据查询的智能体规划提供了可复现的路径。
...
TL;DR: 该研究对基于LLM的客服质量评估系统进行反事实公平性评估，发现存在系统性偏见，公平性与准确性不相关，公平意识提示仅带来有限改进。


<details>
  <summary>Details</summary>
Motivation: LLM在客服质量评估中应用日益广泛，但其基于网络规模训练数据可能带来人口统计和行为偏见，扭曲员工绩效评估，需要系统评估其公平性。

Method: 采用反事实公平性评估框架，在13个维度（身份、上下文、行为风格）上量化公平性，使用反事实翻转率(CFR)和平均绝对分数差异(MASD)指标，评估18个LLM在3000个真实客服对话记录上的表现。

Result: 发现系统性差异：CFR在5.4%-13.0%之间，MASD在置信度、积极和改进分数上存在一致偏移。更大、对齐更强的模型偏见较小，但公平性与准确性不相关。上下文历史绩效提示导致最严重退化（CFR达16.4%），隐性语言身份线索是持续偏见来源。公平意识提示仅带来有限改进。

Conclusion: 在部署LLM进行高风险员工评估前，需要建立标准化公平性审计流程，以识别和减轻系统性偏见。
...
TL;DR: 提出一个强化学习框架，用大语言模型从跨域用户信号生成高质量的兴趣驱动新闻搜索查询列表，并通过策略蒸馏实现可扩展部署
...
Method: 将查询列表生成建模为策略优化问题，采用GRPO（Group Relative Policy Optimization）和多奖励信号，系统研究推理时采样和模型容量两个计算维度，并通过策略蒸馏将大模型知识迁移到紧凑学生模型

Result: 离线实验、消融研究和大规模在线A/B测试显示，在兴趣建模质量和下游推荐性能方面均获得一致提升，增加计算量呈现类似缩放的行为模式

Conclusion: 提出的强化学习框架能有效生成高质量的兴趣驱动查询列表，通过策略蒸馏实现可扩展部署，为跨域新闻推荐提供了有效的解决方案
...
TL;DR: Pep框架通过离线学习偏好结构+在线贝叶斯推理，在冷启动个性化中比强化学习更高效准确，参数少且能动态调整问题


<details>
  <summary>Details</summary>
Motivation: 冷启动个性化需要从有限交互中推断用户偏好，但传统强化学习无法有效利用偏好数据的结构化特征，导致策略退化为静态问题序列

Method: 将冷启动偏好获取分解为：1) 离线学习偏好相关性的结构化世界模型；2) 在线进行无需训练的贝叶斯推理，选择信息量大的问题并预测完整偏好

Result: 在医疗、数学、社交和常识推理任务中，Pep达到80.8%的偏好对齐率（RL为68.5%），交互次数减少3-5倍，参数仅约1万（RL为80亿）

Conclusion: 冷启动偏好获取的关键瓶颈是能否有效利用偏好数据的结构化特征，Pep的模块化框架通过简单信念模型实现了高效准确的个性化
...
TL;DR: 提出基于参数高效微调LLM的文本风格迁移新方法，利用回译生成平行语料，结合RAG增强术语一致性，在多个领域超越零样本提示和少样本ICL


<details>
  <summary>Details</summary>
Motivation: 解决文本风格迁移中平行语料稀缺的问题，传统方法需要大量配对数据，而实际应用中不同风格间的平行语料难以获取

Method: 使用回译技术从单语语料库合成平行数据集，通过参数高效微调LLM，结合检索增强生成(RAG)确保术语和专有名词的一致性

Result: 在四个研究领域中，该方法在BLEU分数和风格准确率上均一致优于零样本提示和少样本ICL技术

Conclusion: 回译合成平行语料结合参数高效微调LLM是有效的文本风格迁移方法，RAG进一步提升了术语一致性和风格鲁棒性
...
### [245] [Agentic AI for Commercial Insurance Underwriting with Adversarial Self-Critique](https://arxiv.org/abs/2602.13213)
*Joyjit Roy,Samaresh Kumar Singh*

Main category: cs.AI

TL;DR: 提出一种决策否定、人在回路（human-in-the-loop）的代理系统，包含对抗性自我批判机制，用于受监管的保险核保工作流，显著减少AI幻觉并提高准确性，同时保持人类最终决策权。
...
Method: 开发决策否定、人在回路的代理系统，包含对抗性自我批判机制作为有界安全架构。批评代理在提交建议给人类评审前挑战主代理的结论。同时建立决策否定代理潜在错误的正式分类法。

Result: 在500个专家验证的核保案例中，对抗性批判机制将AI幻觉率从11.3%降至3.8%，决策准确率从92%提高到96%。框架通过设计强制执行人类对所有约束性决策的最终权威。

Conclusion: 对抗性自我批判支持在受监管领域更安全的AI部署，为人类监督不可或缺的场景提供了负责任的集成模型，解决了受监管工作流中AI安全的关键缺口。
...
### [246] [BotzoneBench: Scalable LLM Evaluation via Graded AI Anchors](https://arxiv.org/abs/2602.13214)
*Lingfeng Li,Yunlong Lu,Yuefei Zhang,Jingyu Yao,Yixin Zhu,KeYuan Cheng,Yongyi Wang,Qirui Zheng,Xionghui Yang,Wenxin Li*

Main category: cs.AI

TL;DR: 论文提出了BotzoneBench，一个基于固定技能层次游戏AI的LLM战略推理评估框架，避免了传统LLM对战锦标赛的二次计算成本和排名不稳定性问题。


<details>
  <summary>Details</summary>
Motivation: 当前LLM在交互环境中的战略决策能力评估存在挑战：现有基准主要测试静态推理，而基于LLM对战锦标赛的方法计算成本高（二次复杂度），且排名依赖于瞬时的模型池，缺乏稳定的性能锚点进行纵向追踪。

Method: 提出BotzoneBench评估框架，将LLM评估锚定到固定技能层次校准的游戏AI上，在Botzone平台的竞争基础设施上评估8种多样化游戏（从确定性完全信息棋盘游戏到随机不完全信息纸牌游戏），通过177,047个状态-动作对系统评估5个旗舰模型。

Result: 揭示了显著的性能差异和不同的战略行为，表现最佳的模型在多个领域达到中高等级专业游戏AI的熟练度，实现了线性时间绝对技能测量和跨时间稳定可解释性。

Conclusion: 这种锚定评估范式可推广到任何具有明确定义技能层次的领域，为评估交互式AI能力建立了可扩展和可重用的框架。
...
### [247] [When to Think Fast and Slow? AMOR: Entropy-Based Metacognitive Gate for Dynamic SSM-Attention Switching](https://arxiv.org/abs/2602.13215)
*Haoran Zheng*

Main category: cs.AI

TL;DR: AMOR是一种混合架构，结合了状态空间模型（SSM）和稀疏注意力机制，通过预测熵动态判断何时需要注意力计算，在保持SSM线性计算效率的同时提升长距离信息检索能力。


<details>
  <summary>Details</summary>
Motivation: Transformer对所有位置分配均匀计算，不考虑难度差异；SSM虽然计算效率高但长距离信息检索能力不足。受认知双过程理论启发，需要一种能动态分配计算资源的架构。

Method: 提出AMOR混合架构：使用SSM作为主干网络，通过预测熵衡量模型"不确定性"，仅在不确定时激活稀疏注意力。采用Ghost KV技术从SSM隐藏状态投影键值对，复用SSM的O(n)计算而非每层都需要O(n²)注意力。

Result: 在小规模合成检索任务中，AMOR优于纯SSM和纯Transformer基线，实现完美检索准确率，同时仅在22%的位置上激活注意力。预测熵能可靠指示检索需求，检索位置与局部位置之间存在1.09纳特的熵差。

Conclusion: AMOR通过自适应计算在效率和性能间取得平衡，提供可解释的决策机制，预测熵作为路由信号在信息论上有明确意义，为高效长序列建模提供了新方向。
...
TL;DR: VeRA框架通过将基准问题转换为可执行规范，自动生成无限验证变体，解决当前评估方案的静态性问题，实现可扩展的鲁棒评估。


<details>
  <summary>Details</summary>
Motivation: 当前大多数评估方案存在"静态"问题：重复使用相同问题导致记忆化、格式利用和最终饱和。需要构建性鲁棒的评估来真正衡量AI进展，而非事后检测。

Method: 提出VeRA（验证推理数据增强）框架，将基准问题转换为包含三部分的可执行规范：自然语言模板、一致性生成器（采样有效配置）、确定性验证器（验证参数并计算正确答案）。从单个种子问题自动创建无限验证变体。

Result: 评估16个前沿模型发现：VeRA-E提高评估质量并揭示污染模式；VeRA-H实现无需人工的困难任务生成；VeRA确立验证基准作为通用范式。框架将基准从静态对象重构为按需生成新鲜验证实例的可执行规范。

Conclusion: VeRA重新概念化基准评估，从静态耗尽对象转变为按需生成新鲜验证实例的可执行规范，增强评估的鲁棒性和成本效益，使任何可验证领域的评估都能无限扩展而不牺牲标签完整性。
...
### [249] [Scaling the Scaling Logic: Agentic Meta-Synthesis of Logic Reasoning](https://arxiv.org/abs/2602.13218)
*Bowen Liu,Zhi Wu,Runquan Xie,Zhanhui Kang,Jia Li*

Main category: cs.AI

TL;DR: SSLogic是一个通过迭代合成和修复可执行的生成器-验证器程序对来扩展可验证训练信号的代理元合成框架，能够实现任务族级别的扩展和难度控制。


<details>
  <summary>Details</summary>
Motivation: 可验证训练信号的扩展是强化学习从可验证奖励（RLVR）的关键瓶颈。逻辑推理是一个天然的基础，但现有的合成方法要么依赖专家编写的代码，要么局限于固定模板，限制了任务族级别的扩展。

Method: 提出SSLogic框架，采用生成-验证-修复的闭环循环，迭代合成和修复可执行的生成器-验证器程序对。引入多门验证协议，结合多策略一致性检查和对抗性盲审，确保任务可靠性。

Result: 从400个种子家族开始，经过两轮演化扩展到953个家族和21,389个可验证实例。在相同训练步数下，相比种子基线，在SynLogic、BBEH、AIME25和Brumo25等基准上分别获得+5.2、+1.4、+3.0和+3.7的提升。
...
### [250] [A Geometric Taxonomy of Hallucinations in LLMs](https://arxiv.org/abs/2602.13224)
*Javier Marín*

Main category: cs.AI

TL;DR: 论文提出幻觉的三分法：不忠实、虚构和事实错误，发现前两种在嵌入空间有可检测的几何特征，而事实错误无法通过嵌入检测


<details>
  <summary>Details</summary>
Motivation: 现有研究将大语言模型的"幻觉"混为一谈，但不同现象在嵌入空间可能有不同的几何特征，需要系统分类来澄清嵌入检测的适用范围

Method: 提出三分法分类：不忠实（不利用上下文）、虚构（发明语义无关内容）、事实错误（概念框架正确但内容错误）。通过几何分析比较不同幻觉类型在嵌入空间的特征

Result: 发现不对称性：标准基准中的幻觉检测在领域内有效（AUROC 0.76-0.99），跨领域则失效（AUROC 0.50）。人类制作的虚构内容有全局可检测方向（AUROC 0.96），而事实错误无法检测（AUROC 0.478）

Conclusion: 嵌入检测适用于类型I和II幻觉，但类型III（事实错误）需要外部验证机制，因为嵌入编码的是分布共现模式而非与外部现实的对应关系
...
### [251] [Variation is the Key: A Variation-Based Framework for LLM-Generated Text Detection](https://arxiv.org/abs/2602.13226)
...
Result: VaryBalance在AUROC指标上比当前最先进的检测器Binoculars高出34.3%，并且在多种生成模型和语言上保持鲁棒性。

Conclusion: VaryBalance是一种简单、有效且实用的LLM生成文本检测方法，通过利用人类文本与LLM改写版本之间的差异特征，显著提升了检测性能。
...
### [252] [Intelligence as Trajectory-Dominant Pareto Optimization](https://arxiv.org/abs/2602.13230)
*Truong Xuan Khanh,Truong Quynh Hoa*

Main category: cs.AI

TL;DR: 该论文提出轨迹主导帕累托优化框架，将智能视为轨迹级现象，揭示帕累托陷阱作为局部非支配区域会限制全局发展路径，定义了陷阱逃逸难度指数来量化约束刚性。


<details>
  <summary>Details</summary>
Motivation: 尽管人工智能近期取得进展，许多系统在长期适应性方面仍停滞不前。作者认为这种限制主要不是源于学习、数据或模型容量不足，而是源于智能随时间优化的更深层结构特性。

Method: 提出轨迹主导帕累托优化框架，将帕累托最优性推广到完整轨迹层面；定义陷阱逃逸难度指数(TEDI)来量化约束刚性；建立帕累托陷阱的形式分类；通过最小智能体-环境模型进行说明。

Result: 动态智能上限是轨迹级支配关系的必然几何结果，与学习进展或架构规模无关；帕累托陷阱会导致轨迹级分歧；提供了诊断和克服自适应系统中长期发展约束的原则性框架。

Conclusion: 智能研究的重点应从终端性能转向优化几何，轨迹主导帕累托优化为理解和克服自适应系统中的长期发展约束提供了新的理论视角。
...
### [253] [No Need to Train Your RDB Foundation Model](https://arxiv.org/abs/2602.13697)
...
Method: 提出原则性的关系数据库编码器家族：1) 将可变大小的RDB邻域压缩为固定长度的ICL样本；2) 关键创新：ICL特定压缩应限制在共享单位和角色的高维RDB列内，而不是跨列；3) 证明在此限制下，编码器表达能力不受排除可训练参数的影响；4) 开发可扩展的SQL原语实现编码阶段

Result: 开发了开源的RDB基础模型，无需训练或微调即可在未见数据集上实现稳健性能，可与现有单表ICL基础模型无缝配对使用

Conclusion: 提出了无需训练的关系数据库编码器框架，通过限制在列内压缩而非跨列压缩的原则性方法，实现了多表关系数据库与单表ICL基础模型的集成，提供了实用的开源解决方案
...
TL;DR: PlotChain是一个用于评估多模态大语言模型在工程图表读取能力上的确定性基准测试，包含15类450个图表，通过检查点诊断评估模型表现。


<details>
  <summary>Details</summary>
Motivation: 现有MLLM评估主要关注OCR提取或自由形式描述，缺乏对工程图表（如Bode图、应力应变曲线等）定量数值恢复能力的系统性评估。

Method: 构建包含15类工程图表的确定性生成基准，每类30个图表，每个项目包含已知参数和精确真值。采用检查点诊断评估，包含中间字段隔离子技能，使用温度=0的确定性协议和严格JSON输出格式。

Result: 在'plotread'容差策略下，最佳模型表现：Gemini 2.5 Pro 80.42%，GPT-4.1 79.84%，Claude Sonnet 4.5 78.21%，GPT-4o 61.59%。频域任务表现脆弱，带通响应≤23%。

Conclusion: PlotChain为MLLM在工程图表读取能力提供了可复现的评估基准，揭示了模型在频域任务上的局限性，并提供了完整的工具链支持后续研究。
...
### [255] [Tabular Foundation Models Can Learn Association Rules](https://arxiv.org/abs/2602.14622)
*Erkan Karabulut,Daniel Daza,Paul Groth,Martijn C. Schut,Victoria Degeler*

Main category: cs.AI

TL;DR: TabProbe：利用表格基础模型（TFMs）进行关联规则挖掘的新框架，无需频繁项集挖掘，在低数据场景下仍保持高性能
...
Method: 提出模型无关的关联规则学习框架，可从任何条件概率模型中提取关联规则。TabProbe是该框架的具体实现，利用TFMs作为条件概率估计器来学习关联规则，无需频繁项集挖掘

Result: TFMs能够持续产生简洁、高质量的关联规则，具有强大的预测性能，在低数据设置下保持鲁棒性，无需任务特定训练

Conclusion: TabProbe框架成功利用TFMs进行关联规则挖掘，解决了传统方法的可扩展性和低数据性能问题，为知识发现提供了有效的新方法
...
### [256] [Stay in Character, Stay Safe: Dual-Cycle Adversarial Self-Evolution for Safety Role-Playing Agents](https://arxiv.org/abs/2602.13234)
*Mingyang Liao,Yichen Wan,shuchen wu,Chenxi Miao,Xin Shen,Weikang Li,Yang Li,Deguo Xia,Jizhou Huang*

Main category: cs.AI

TL;DR: 提出Dual-Cycle Adversarial Self-Evolution框架，通过攻击者循环生成越狱提示，防御者循环构建分层知识库，实现无需训练的LLM角色扮演安全增强


<details>
  <summary>Details</summary>
Motivation: 现有基于训练的方法（如数据筛选、对齐正则化）在角色扮演中面临成本高、损害角色行为、不适用于闭源模型等问题，需要训练无关的安全增强方案

Method: 双循环对抗自演化框架：攻击者循环合成渐进式越狱提示；防御者循环构建三层知识库（全局安全规则、角色约束、安全示例），推理时检索组合知识指导生成

Result: 在多个专有LLM上的实验显示，相比基线方法在角色保真度和越狱抵抗方面均有提升，且能泛化到未见过的角色和攻击提示

Conclusion: 提出的训练无关框架有效平衡了角色扮演的保真度和安全性，解决了现有训练方法的局限性，具有实际应用价值
...
### [257] [Lang2Act: Fine-Grained Visual Reasoning through Self-Emergent Linguistic Toolchains](https://arxiv.org/abs/2602.13235)
*Yuqi Xiong,Chunyi Peng,Zhipeng Xu,Zhenghao Liu,Zulong Chen,Yukun Yan,Shuo Wang,Yu Gu,Ge Yu*

Main category: cs.AI

TL;DR: Lang2Act提出了一种通过自涌现语言工具链实现细粒度视觉感知和推理的方法，取代了传统VRAG框架中依赖固定外部工具的分离设计，显著提升了视觉语言模型的感知能力。


<details>
  <summary>Details</summary>
Motivation: 现有VRAG框架通常依赖预定义的外部工具来扩展VLMs的感知能力，这种解耦设计在应用图像裁剪等操作时会导致视觉信息的不必要损失。需要一种更灵活、更紧密集成的方法来提升视觉感知能力。

Method: 提出Lang2Act框架，通过自涌现语言工具链实现细粒度视觉感知。采用两阶段强化学习训练：第一阶段优化VLMs自探索高质量动作以构建可重用语言工具箱；第二阶段进一步优化VLMs有效利用这些语言工具进行下游推理。

Result: 实验结果表明Lang2Act能显著增强VLMs的视觉感知能力，实现了超过4%的性能提升。所有代码和数据已开源。

Conclusion: Lang2Act通过自涌现语言工具链的方法，克服了传统VRAG框架中视觉感知与推理分离的局限性，为增强视觉语言模型的感知能力提供了一种有效的新途径。
...
### [258] [NL2LOGIC: AST-Guided Translation of Natural Language into First-Order Logic with Large Language Models](https://arxiv.org/abs/2602.13237)
...
<details>
  <summary>Details</summary>
Motivation: 在法律和治理等领域，自动化推理需要准确性和可解释性。现有方法（如GCD和CODE4LOGIC）存在语法控制脆弱（全局语法约束弱）和语义忠实度低（子句级语义理解不足）的问题。

Method: 提出NL2LOGIC框架，引入抽象语法树作为中间表示，结合递归大型语言模型语义解析器和AST引导的生成器，确定性地生成可求解的逻辑代码。
...
Conclusion: NL2LOGIC通过抽象语法树中间表示有效解决了现有方法的语法控制和语义忠实度问题，显著提升了自然语言到一阶逻辑翻译的质量和下游推理性能。
...
### [259] [AST-PAC: AST-guided Membership Inference for Code](https://arxiv.org/abs/2602.13240)
...
<details>
  <summary>Details</summary>
Motivation: 代码大语言模型常在受限许可的大规模数据集上训练，引发数据治理和版权问题。成员推断攻击可作为审计机制检测未经授权的数据使用，但在代码领域的应用研究不足。
...
Conclusion: 研究结果表明需要开发语法感知和规模自适应的校准方法，这是可靠审计代码语言模型数据来源的前提。未来工作应关注这些方向。
...
### [260] [X-Blocks: Linguistic Building Blocks of Natural Language Explanations for Automated Vehicles](https://arxiv.org/abs/2602.13248)
*Ashkan Y. Zadeh,Xiaomeng Li,Andry Rakotonirainy,Ronald Schroeter,Sebastien Glaser,Zishuo Zhu*

Main category: cs.AI

TL;DR: X-Blocks框架：用于分析自动驾驶自然语言解释的层次化语言分析框架，包含上下文、句法和词汇三个层次，旨在为生成场景感知的解释提供语言学设计原则。


<details>
  <summary>Details</summary>
Motivation: 自然语言解释对于建立对自动驾驶系统的信任和接受度至关重要，但现有方法缺乏系统性的框架来分析人类如何在不同场景下语言构建驾驶原理。

Method: 提出X-Blocks层次化分析框架，包含三个层次：1) 上下文层面：提出RACE框架（多LLM集成，结合思维链推理和自一致性机制），将解释分类为32个场景感知类别；2) 词汇层面：使用信息性狄利克雷先验的对数几率分析揭示场景特定的词汇模式；3) 句法层面：使用依存句法分析和模板提取分析可重用语法家族。

Result: RACE在Berkeley DeepDrive-X数据集上达到91.45%准确率和Cohen's kappa 0.91，接近人类可靠性；词汇分析揭示了区分驾驶场景的上下文特定词汇模式；句法分析显示解释使用有限的语法家族，不同上下文在谓词类型和因果结构上有系统性变化。

Conclusion: X-Blocks框架是数据集无关和任务独立的，可广泛应用于其他自动驾驶数据集和安全关键领域，为生成支持透明度、用户信任和认知可访问性的场景感知解释提供基于证据的语言学设计原则。
...
### [261] [DPBench: Large Language Models Struggle with Simultaneous Coordination](https://arxiv.org/abs/2602.13255)
*Najmul Hasan,Prashanth BusiReddyGari*

Main category: cs.AI

TL;DR: DPBench是一个基于哲学家就餐问题的基准测试，评估LLM在多智能体系统中的协调能力，发现LLM在顺序决策时能有效协调，但在同时决策时死锁率超过95%，通信反而可能加剧死锁。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型越来越多地部署在多智能体系统中，但缺乏测试它们在资源竞争下协调能力的基准测试。需要评估LLM在需要协调访问共享资源时的表现。

Method: 基于哲学家就餐问题开发DPBench基准测试，包含8种不同条件（决策时序、群体规模、通信等）。在GPT-5.2、Claude Opus 4.5和Grok 4.1上进行实验，分析协调失败的原因。

Result: 发现显著的不对称性：LLM在顺序设置中能有效协调，但在需要同时决策时失败严重，某些条件下死锁率超过95%。失败源于收敛推理——智能体独立得出相同策略，同时执行时必然导致死锁。通信不仅不能解决问题，反而可能增加死锁率。

Conclusion: 需要并发资源访问的多智能体LLM系统可能需要外部协调机制，而非依赖涌现的协调能力。DPBench作为开源基准测试发布，用于评估LLM协调能力。
...
### [262] [MAPLE: A Sub-Agent Architecture for Memory, Learning, and Personalization in Agentic AI Systems](https://arxiv.org/abs/2602.13258)
*Deepak Babu Piskala*

Main category: cs.AI

TL;DR: MAPLE框架将LLM代理的个性化能力分解为三个独立组件：记忆、学习和个性化，通过专用子代理实现更好的用户适应


<details>
  <summary>Details</summary>
Motivation: 当前LLM代理系统将记忆、学习和个性化视为统一能力，这种架构上的混淆限制了代理适应个体用户的能力。需要将这些机制分解为独立组件，各自具有不同的基础设施、时间尺度和优化策略。

Method: 提出MAPLE框架，将个性化能力分解为三个专用子代理：记忆组件负责存储和检索基础设施；学习组件异步从积累的交互中提取智能；个性化组件在实时有限上下文预算内应用学到的知识。每个组件都有专门的工具和明确定义的接口。

Result: 在MAPLE-Personas基准测试中，该分解方法相比无状态基线实现了14.6%的个性化分数提升（p < 0.01, Cohen's d = 0.95），并将特征融入率从45%提高到75%，使代理能够真正学习和适应。

Conclusion: 将记忆、学习和个性化分解为独立组件是提升LLM代理个性化能力的关键架构改进，MAPLE框架通过专用子代理实现了更有效的用户适应。
...
### [263] [General learned delegation by clones](https://arxiv.org/abs/2602.13262)
*Darren Li,Meiqi Chen,Chenze Shao,Fandong Meng,Jie Zhou*

Main category: cs.AI

TL;DR: SELFCEST 通过让基础模型在并行上下文中生成相同权重的克隆，使用智能强化学习来分配计算资源，在固定推理预算下提升语言模型的推理效率。


<details>
  <summary>Details</summary>
Motivation: 前沿语言模型虽然能通过增加测试时计算来提升性能，但串行推理或无协调的并行采样在固定推理预算下计算效率低下。需要一种更高效的计算资源分配方法。

Method: SELFCEST 让基础模型能够在并行上下文中生成相同权重的克隆，通过智能强化学习进行端到端训练，在全局任务奖励下使用共享参数rollout，学习到的控制器能够跨分支分配生成和上下文预算。

Result: 在具有挑战性的数学推理基准和长上下文多跳问答任务中，SELFCEST 在匹配推理预算下相比单体基线改善了准确率-成本帕累托前沿，并在两个领域都展现出分布外泛化能力。

Conclusion: SELFCEST 提供了一种有效的方法来优化语言模型在固定推理预算下的计算效率，通过智能并行克隆和资源分配机制，在复杂推理任务中实现了更好的性能-成本权衡。
...
### [264] [Human-Centered Explainable AI for Security Enhancement: A Deep Intrusion Detection Framework](https://arxiv.org/abs/2602.13271)
...
TL;DR: 提出结合可解释人工智能(XAI)的入侵检测系统框架，使用CNN和LSTM网络处理流量序列，集成SHAP增强模型透明度，在NSL-KDD数据集上达到高精度，并通过专家调查评估系统可靠性。


<details>
  <summary>Details</summary>
Motivation: 网络威胁日益复杂，需要既准确又可解释的入侵检测系统。传统IDS和黑盒深度学习模型缺乏透明度，难以让安全分析师理解和验证模型决策。

Method: 提出集成XAI的IDS框架：1) 结合CNN和LSTM网络捕捉流量序列的时间依赖性；2) 集成SHAP模型提供模型决策的可解释性；3) 使用NSL-KDD基准数据集进行实验评估；4) 通过基于IPIP6和大五人格特质的信任导向专家调查评估系统可靠性和可用性。

Result: 1) CNN和LSTM准确率均达到0.99；2) LSTM在宏观平均精确率、召回率和F1分数上优于CNN；3) 加权平均指标两者表现相似；4) SHAP识别出srv_serror_rate、dst_host_srv_serror_rate和serror_rate等关键特征；5) 专家调查验证了系统的可靠性和可用性。

Conclusion: 结合性能和透明度的网络安全解决方案具有潜力，未来可通过自适应学习实现实时威胁检测。该框架为安全分析师提供了可理解和验证的入侵检测工具。
...
### [265] [TemporalBench: A Benchmark for Evaluating LLM-Based Agents on Contextual and Event-Informed Time Series Tasks](https://arxiv.org/abs/2602.13272)
*Muyan Weng,Defu Cao,Wei Yang,Yashaswi Sharma,Yan Liu*

Main category: cs.AI

TL;DR: TemporalBench是一个多领域基准测试，用于评估模型在逐步丰富信息设置下的时间推理能力，揭示仅靠预测准确性无法反映真实的时间理解和上下文推理能力。


<details>
  <summary>Details</summary>
Motivation: 当前不清楚强大的预测性能是否反映真正的时间理解能力，还是仅仅是在上下文和事件驱动条件下的推理能力。需要区分模型是否真正理解时间模式，而不仅仅是进行数值预测。

Method: 设计了四层任务分类法：历史结构解释、无上下文预测、上下文时间推理、事件条件预测，覆盖零售、医疗、能源和物理系统四个领域。通过控制对未来目标和上下文信息的访问，进行诊断性分析。

Result: 实验显示强大的数值预测准确性并不能可靠转化为稳健的上下文或事件感知时间推理能力。现有代理框架表现出碎片化的优势和系统性失败模式，这些在仅关注预测的基准测试中被隐藏。

Conclusion: 需要超越传统预测基准，评估模型的时间推理能力。TemporalBench提供了一个公开的数据集和排行榜，用于诊断模型在时间理解方面的真实能力。
...
### [266] [ProMoral-Bench: Evaluating Prompting Strategies for Moral Reasoning and Safety in LLMs](https://arxiv.org/abs/2602.13274)
*Rohan Subramanian Thomas,Shikhar Shiromani,Abdullah Chaudhry,Ruizhe Li,Vasu Sharma,Kevin Zhu,Sunishchal Dev*

Main category: cs.AI

TL;DR: ProMoral-Bench是一个统一的提示设计基准，评估11种提示范式在4个LLM家族上的表现，发现简洁的示例引导提示优于复杂多阶段推理，提供更高的道德安全分数和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 提示设计对LLM的道德能力和安全对齐有重要影响，但现有研究在数据集和模型上比较分散，缺乏统一的评估框架。
...
Result: 紧凑的示例引导支架优于复杂多阶段推理，提供更高的UMSS分数和更强的鲁棒性，且token成本更低。多轮推理在扰动下脆弱，而少样本示例能持续增强道德稳定性和越狱抵抗能力。

Conclusion: ProMoral-Bench为原则性、成本效益高的提示工程建立了标准化框架，为LLM的道德安全对齐提供了系统评估工具。
...
### [267] [Artificial Organisations](https://arxiv.org/abs/2602.13275)
*William Waites*

Main category: cs.AI

TL;DR: 该论文提出将组织理论应用于多智能体AI安全，通过架构设计（信息隔离和对抗性审查）而非依赖个体对齐来实现可靠集体行为，并在文档撰写系统中验证了这一方法。


<details>
  <summary>Details</summary>
Motivation: 当前对齐研究主要关注单个AI系统的可靠性，但人类机构通过组织结构来缓解个体错位风险。论文旨在探索多智能体AI系统如何借鉴这种制度模型，通过架构设计而非假设个体对齐来实现可靠结果。

Method: 提出了Perseverance Composition Engine多智能体系统，包含三个角色：Composer（起草文本）、Corroborator（验证事实依据，有完整源访问权限）、Critic（评估论证质量，无源访问权限）。通过系统架构强制执行信息不对称，实现分层验证。

Result: 在474个撰写任务中观察到与制度假设一致的模式。当分配需要编造内容的不可能任务时，系统从尝试编造转向诚实拒绝并提出替代方案，这种行为既未指令也未个体激励。结果表明架构强制可能从不可靠组件产生可靠结果。
...
### [268] [BEAGLE: Behavior-Enforced Agent for Grounded Learner Emulation](https://arxiv.org/abs/2602.13280)
...
<details>
  <summary>Details</summary>
Motivation: 收集真实学生学习行为数据面临隐私问题和纵向研究成本高的挑战。虽然大语言模型可以模拟学生，但它们存在"能力偏差"——倾向于追求高效正确性，而不是模拟新手学习者那种反复无常、迭代式的挣扎过程。

Method: BEAGLE整合了三个关键技术创新：1）半马尔可夫模型控制认知行为和元认知行为的时间与转换；2）带有显式缺陷注入的贝叶斯知识追踪，强制实施真实的知识缺口和"未知的未知"；3）解耦的智能体设计，将高级策略使用与代码生成动作分离，防止模型默默纠正自己的故意错误。

Result: 在Python编程任务评估中，BEAGLE在重现真实轨迹方面显著优于最先进的基线方法。在人类图灵测试中，用户无法区分合成轨迹与真实学生数据，准确率与随机猜测无显著差异（52.8%）。

Conclusion: BEAGLE通过整合自我调节学习理论，成功解决了LLM模拟学生时的能力偏差问题，能够生成与真实学生学习行为难以区分的合成数据，为教育研究和自适应辅导系统训练提供了有力工具。
...
### [269] [Accuracy Standards for AI at Work vs. Personal Life: Evidence from an Online Survey](https://arxiv.org/abs/2602.13283)
*Gaston Besanson,Federico Todeschini*

Main category: cs.AI

TL;DR: 研究人们在专业与个人情境下使用AI工具时的准确性权衡差异：工作中对准确性要求显著高于个人生活，且工具不可用时对个人生活影响更大。


<details>
  <summary>Details</summary>
Motivation: 研究人们在专业和个人情境下使用AI工具时对准确性的不同要求，以及这些权衡的决定因素，并探讨当AI/应用不可用时用户的应对方式。

Method: 通过在线调查（N=300）收集数据，定义"准确性"为情境特定的可靠性：输出在特定容忍阈值内与用户意图对齐的程度。分析专业与个人情境下的准确性要求差异，并检验使用频率和经验模式与工作标准的关系。

Result: 工作中要求高准确性（最高评分）的比例为24.1%，个人生活中仅为8.8%（+15.3个百分点）。使用更宽泛的评分标准（前两项）差距仍然显著（67.0% vs. 32.9%）。重度应用使用和经验模式与更严格的工作标准相关。工具不可用时，个人生活受到的干扰（34.1%）显著大于工作（15.3%）。

Conclusion: 人们对AI工具准确性的要求因情境而异，工作中要求显著高于个人生活。工具不可用对个人生活的影响更大，表明AI工具在个人生活中的整合程度可能更高。这些发现对AI系统设计和用户支持策略有重要启示。
...
### [270] [Mirror: A Multi-Agent System for AI-Assisted Ethics Review](https://arxiv.org/abs/2602.13292)
...
TL;DR: Mirror是一个AI辅助伦理审查的代理框架，通过EthicsLLM模型和两种工作模式（快速审查和委员会审查）来提升伦理审查的质量和一致性。


<details>
  <summary>Details</summary>
Motivation: 现代研究治理中的伦理审查系统面临压力，特别是在大规模、跨学科科学研究中。现有系统在应对异质性风险时存在机构审查能力限制，而大语言模型虽然提供了新机会，但直接应用受到伦理推理能力不足、监管结构整合不够和隐私限制的制约。

Method: 提出了Mirror框架，核心是EthicsLLM模型（在EthicsQA数据集上微调）。框架包含两种模式：Mirror-ER通过可执行规则库自动化快速审查；Mirror-CR通过专家代理、伦理秘书代理和主要研究者代理之间的协调互动模拟完整委员会审议。

Result: 实证评估表明，Mirror相比强大的通用大语言模型，显著提高了伦理评估的质量、一致性和专业性。

Conclusion: Mirror框架通过整合伦理推理、结构化规则解释和多代理审议，为AI辅助伦理审查提供了有效的解决方案，能够支持现代研究治理中的伦理审查需求。
...
### [271] [DECKBench: Benchmarking Multi-Agent Frameworks for Academic Slide Generation and Editing](https://arxiv.org/abs/2602.13318)
*Daesik Jang,Morgan Lindsay Heisler,Linzi Xing,Yifei Li,Edward Wang,Ying Xiong,Yong Zhang,Zhenan Fan*

Main category: cs.AI

TL;DR: DECKBench：一个用于评估多智能体幻灯片生成与编辑的基准框架，包含数据集、评估协议和基线系统，旨在解决学术幻灯片自动生成中的内容选择、组织、布局和多轮指令跟随等挑战。


<details>
  <summary>Details</summary>
Motivation: 现有基准和评估方法无法充分衡量学术幻灯片自动生成与编辑中的关键挑战，包括忠实的内容选择、连贯的幻灯片组织、布局感知的渲染以及鲁棒的多轮指令跟随。

Method: 提出了DECKBench评估框架，包含：1）基于论文-幻灯片对的精选数据集，并添加了真实的模拟编辑指令；2）系统评估幻灯片级和演示文稿级的保真度、连贯性、布局质量和多轮指令跟随的评估协议；3）模块化的多智能体基线系统，将任务分解为论文解析与摘要、幻灯片规划、HTML创建和迭代编辑。

Result: 实验结果表明，该基准能够有效揭示系统的优势和失败模式，为改进多智能体幻灯片生成与编辑系统提供可操作的见解。

Conclusion: 这项工作为学术演示文稿生成与编辑的可复现和可比较评估建立了标准化基础，代码和数据已公开。
...
### [272] [Situation Graph Prediction: Structured Perspective Inference for User Modeling](https://arxiv.org/abs/2602.13319)
*Jisung Shin,Daniel Platnick,Marjan Alirezaie,Hossein Rahnama*

Main category: cs.AI

TL;DR: 论文提出情境图预测(SGP)任务，将视角建模重构为从可观察多模态痕迹推断结构化、本体对齐的视角表示的反向推理问题，并通过结构优先的合成数据生成策略解决数据瓶颈。


<details>
  <summary>Details</summary>
Motivation: 构建具有视角感知能力的AI需要建模内部状态（目标、情感、上下文），而不仅仅是偏好。当前进展受限于数据瓶颈：数字足迹涉及隐私敏感，且视角状态很少被标注。

Method: 提出情境图预测(SGP)任务，将视角建模重构为反向推理问题。采用结构优先的合成生成策略，通过设计对齐潜在标签和可观察痕迹。构建数据集并使用检索增强的上下文学习作为监督代理进行诊断研究。

Result: 在GPT-4o的研究中发现，表面层提取与潜在视角推断之间存在差距，表明在受控设置下潜在状态推断比表面提取更难。结果证明SGP任务非平凡，并为结构优先的数据合成策略提供了证据。

Conclusion: SGP任务为视角建模提供了可行的框架，结构优先的合成数据生成策略是解决数据瓶颈的有效方法，潜在状态推断比表面特征提取更具挑战性。
...
### [273] [Information Fidelity in Tool-Using LLM Agents: A Martingale Analysis of the Model Context Protocol](https://arxiv.org/abs/2602.13320)
*Flint Xiaofeng Fan,Cheston Tan,Roger Wattenhofer,Yew-Soon Ong*

Main category: cs.AI

TL;DR: 本文首次提出分析MCP智能体错误累积的理论框架，证明累积失真呈线性增长且高概率偏差有界，为可靠AI系统部署提供理论保证。
...
Method: 1. 引入首个分析MCP智能体错误累积的理论框架；2. 开发结合离散事实匹配和连续语义相似度的混合失真度量；3. 建立顺序工具交互中错误传播的鞅集中界；4. 在Qwen2-7B、Llama-3-8B和Mistral-7B上进行实验验证。

Result: 1. 证明累积失真呈线性增长，高概率偏差有O(√T)界；2. 实验验证理论预测，经验失真跟踪线性趋势；3. 语义加权减少80%失真；4. 每约9步重新接地足以控制错误。

Conclusion: 该理论框架为AI智能体系统的可靠部署提供理论保证，证明错误传播不会呈指数级增长，并转化为可操作的部署原则，确保系统行为的可预测性。
...
### [274] [Detecting Jailbreak Attempts in Clinical Training LLMs Through Automated Linguistic Feature Extraction](https://arxiv.org/abs/2602.13321)
*Tri Nguyen,Huy Hoang Bao Le,Lohith Srikanth Pentapalli,Laurah Turner,Kelly Cohen*

Main category: cs.AI

TL;DR: 使用专家标注的四个核心语言特征，训练BERT模型提取特征，再通过多种分类器检测临床LLM中的越狱行为，实现可扩展的自动检测系统。


<details>
  <summary>Details</summary>
Motivation: 临床训练大语言模型需要检测越狱尝试，但之前基于手动标注语言特征的方法存在可扩展性和表达能力限制，需要更自动化的解决方案。

Method: 1) 使用专家标注的四个核心语言特征：专业性、医学相关性、伦理行为和上下文干扰；2) 训练通用和医学领域BERT模型预测这些特征；3) 选择最可靠的特征回归器作为特征提取器；4) 使用树基、线性、概率和集成方法构建第二层分类器预测越狱可能性。

Result: 系统在交叉验证和保留集评估中表现出色，表明LLM提取的语言特征为自动越狱检测提供了有效基础。错误分析揭示了当前标注和特征表示的局限性。

Conclusion: 该方法为安全关键的临床对话系统提供了一种可扩展且可解释的越狱行为检测方法，未来可通过更丰富的标注方案、细粒度特征提取和对话动态风险捕捉来改进。
...
### [275] [Contrastive explanations of BDI agents](https://arxiv.org/abs/2602.13323)
*Michael Winikoff*

Main category: cs.AI

TL;DR: 该研究扩展了BDI智能体的解释能力，使其能回答对比性问题（"为什么做X而不是F？"），实验发现对比性解释更短且在某些方面更受偏好，但令人意外的是，提供完整解释有时反而不如不提供任何解释。


<details>
  <summary>Details</summary>
Motivation: 自主系统的解释能力对透明度和信任建立至关重要。现有BDI智能体只能回答"为什么做X"这类问题，但人类实际常问对比性问题（"为什么做X而不是F？"），因此需要扩展解释机制以支持这类更自然的提问方式。

Method: 扩展先前BDI智能体的解释机制，使其能够回答对比性问题。通过计算评估对比解释长度，并进行人类主体评估，测量对比性解释的偏好程度、对信任建立、感知理解和系统正确性信心的影响，同时评估提供解释的整体效益。

Result: 计算评估显示对比性问题能显著缩短解释长度。人类评估发现对比性解释在某些方面更受偏好，并能带来更高的信任、感知理解和系统正确性信心。但意外发现：提供完整解释并不总是有益，在某些情况下甚至比不提供任何解释更差。

Conclusion: 扩展BDI智能体以回答对比性问题是可行且有价值的，能产生更简洁的解释并可能改善用户体验。然而，提供解释本身并不总是有益的，需要谨慎考虑何时以及如何提供解释，以避免负面效果。
...
### [276] [Nanbeige4.1-3B: A Small General Model that Reasons, Aligns, and Acts](https://arxiv.org/abs/2602.13367)
...
TL;DR: Nanbeige4.1-3B是一个仅30亿参数的统一通用语言模型，首次在单模型中同时实现强大的智能体行为、代码生成和通用推理能力，在复杂任务中能稳定执行多达600轮工具调用。


<details>
  <summary>Details</summary>
Motivation: 当前小型语言模型通常只能在特定领域表现良好，缺乏同时具备智能体行为、代码生成和通用推理能力的统一模型。本研究旨在证明小型模型也能同时实现广泛能力和强专业化。

Method: 1. 结合点对点和配对奖励建模提升推理和偏好对齐；2. 设计复杂度感知的强化学习奖励优化代码正确性和效率；3. 进行复杂数据合成并加入回合级监督训练，实现稳定长时程工具交互。

Result: Nanbeige4.1-3B显著超越同规模模型（如Nanbeige4-3B-2511和Qwen3-4B），甚至在某些方面优于大得多的模型（如Qwen3-30B-A3B），能可靠执行多达600轮工具调用解决复杂问题。

Conclusion: 小型模型可以同时实现广泛能力和强专业化，重新定义了30亿参数模型的潜力，为高效、多功能的小型语言模型发展开辟了新方向。
...
### [277] [MoralityGym: A Benchmark for Evaluating Hierarchical Moral Alignment in Sequential Decision-Making Agents](https://arxiv.org/abs/2602.13372)
...
TL;DR: 提出Morality Chains形式化框架和MoralityGym基准，用于评估AI在冲突性层次化道德规范下的对齐表现，揭示现有安全强化学习方法在伦理决策中的局限性。


<details>
  <summary>Details</summary>
Motivation: 评估AI在冲突性、层次化人类道德规范下的对齐能力是AI安全、道德哲学和认知科学交叉领域的关键挑战，需要更可靠的伦理决策方法。

Method: 提出Morality Chains形式化框架，将道德规范表示为有序的道义约束；创建MoralityGym基准，包含98个电车难题式的Gymnasium环境；通过任务解决与道德评估解耦，引入新的道德度量标准。

Result: 使用安全强化学习方法进行基准测试，揭示了现有方法在伦理决策中的关键局限性，表明需要更原则性的伦理决策方法。

Conclusion: 这项工作为开发在复杂现实世界中行为更可靠、透明和道德的AI系统奠定了基础，推动了心理学和哲学洞见在规范敏感推理评估中的整合。
...
### [278] [On-Policy Supervised Fine-Tuning for Efficient Reasoning](https://arxiv.org/abs/2602.13407)
*Anhao Zhao,Ziyang Chen,Junlong Tong,Yingqi Fan,Fanghua Ye,Shuhao Li,Yunpu Ma,Wenjie Li,Xiaoyu Shen*

Main category: cs.AI

TL;DR: 论文提出On-Policy SFT方法，通过简化奖励机制为截断式长度惩罚，将优化问题简化为对自生成数据的监督微调，在保持准确性的同时大幅减少推理长度和训练成本。


<details>
  <summary>Details</summary>
Motivation: 现有大型推理模型通常使用强化学习训练，但复杂的多奖励目标（同时优化正确性和简洁性）会导致训练不稳定和次优权衡。作者质疑这种复杂性的必要性，希望通过简化方法解决这一问题。

Method: 提出On-Policy SFT方法：1) 去除KL正则化（在正确性和长度可直接验证时失去作用）；2) 去除组归一化（在多奖励信号下变得模糊）；3) 将奖励简化为基于截断的长度惩罚；4) 将优化问题转化为对自生成数据的监督微调，筛选同时满足正确性和简洁性的数据。

Result: 在五个基准测试中，该方法将推理链长度减少高达80%，同时保持原始准确性，超越了更复杂的基于RL的方法。训练效率显著提升：GPU内存使用减少50%，收敛速度加快70%。

Conclusion: 通过简化奖励机制为截断式长度惩罚，将优化问题转化为监督微调，On-Policy SFT方法在保持准确性的同时大幅提升推理效率和训练效率，为准确性与效率的帕累托前沿提供了简洁有效的解决方案。
...
### [279] [NeuroWeaver: An Autonomous Evolutionary Agent for Exploring the Programmatic Space of EEG Analysis Pipelines](https://arxiv.org/abs/2602.13473)
*Guoan Wang,Shihao Yang,Jun-En Ding,Hao Zhu,Feng Liu*

Main category: cs.AI

TL;DR: NeuroWeaver：一个统一的自主进化代理，通过将EEG分析流程工程重新定义为离散约束优化问题，在有限计算资源下生成轻量级、神经科学合理的解决方案，性能媲美大型基础模型。


<details>
  <summary>Details</summary>
Motivation: 基础模型在EEG分析中面临数据需求大、参数多导致计算成本高的问题，难以在资源受限的临床环境中部署。而通用自动机器学习框架缺乏神经生理学先验知识，常产生不科学的解决方案。

Method: 将流程工程重新定义为离散约束优化问题，采用领域知情子空间初始化将搜索限制在神经科学合理的流形上，结合多目标进化优化通过自反思细化动态平衡性能、新颖性和效率。

Result: 在五个异构基准测试中，NeuroWeaver合成的轻量级解决方案持续优于最先进的任务特定方法，性能与大规模基础模型相当，但使用的参数数量显著减少。

Conclusion: NeuroWeaver通过将领域知识与进化优化相结合，为资源受限的临床环境提供了高效、科学合理的EEG分析解决方案，解决了基础模型和通用自动机器学习框架的局限性。
...
### [280] [OMNI-LEAK: Orchestrator Multi-Agent Network Induced Data Leakage](https://arxiv.org/abs/2602.13477)
*Akshat Naik,Jay Culligan,Yarin Gal,Philip Torr,Rahaf Aljundi,Alasdair Paren,Adel Bibi*

Main category: cs.AI

TL;DR: 研究发现多智能体系统存在新型安全漏洞OMNI-LEAK，即使有数据访问控制，攻击者也能通过间接提示注入泄露敏感数据


<details>
  <summary>Details</summary>
Motivation: 随着LLM智能体能力增强，多智能体系统将成为实用范式，但现有安全研究主要关注单智能体系统，缺乏对多智能体系统的威胁建模，特别是存在基本工程防护措施（如访问控制）的情况

Method: 通过红队测试流行的编排器模式多智能体系统（中心智能体分解和委派任务给专门智能体），研究具体未来用例的安全漏洞，开发了OMNI-LEAK攻击向量

Result: 发现前沿模型对各类攻击都易受攻击，推理和非推理模型均存在漏洞，即使攻击者不了解实现细节也能成功，证明了在存在数据访问控制的情况下通过单次间接提示注入泄露敏感数据的可能性
...
### [281] [Translating Dietary Standards into Healthy Meals with Minimal Substitutions](https://arxiv.org/abs/2602.13502)
*Trevor Chan,Ilias Tagkopoulos*

Main category: cs.AI

TL;DR: 提出一个端到端框架，将饮食标准转化为完整餐食，通过识别34个可解释的餐食原型，使用生成模型和分量预测器来满足USDA营养目标，实现更营养、更经济的餐食改进。


<details>
  <summary>Details</summary>
Motivation: 个性化饮食系统的重要目标是在不牺牲便利性或可负担性的情况下改善营养质量。当前需要将饮食指南转化为现实、预算友好的餐食和简单替换方案。

Method: 使用WWEIA摄入数据（135,491餐）识别34个可解释的餐食原型，然后使用这些原型来调节生成模型和分量预测器，以满足USDA营养目标。通过允许1-3种食物替换来创建改进的餐食。

Result: 生成的餐食在遵循推荐每日摄入量目标方面提高了47.0%，同时保持与真实餐食的组成接近。通过1-3种食物替换，餐食营养提高了10%，成本降低了19-32%。

Conclusion: 该框架可以将饮食指南转化为现实、预算友好的餐食和简单替换方案，为临床决策支持、公共卫生项目和消费者应用提供基础，实现可扩展、公平的日常营养改善。
...
### [282] [SPILLage: Agentic Oversharing on the Web](https://arxiv.org/abs/2602.13516)
*Jaechul Roh,Eugene Bagdasarian,Hamed Haddadi,Ali Shahin Shamsabadi*

Main category: cs.AI

TL;DR: 论文提出"自然代理过度分享"概念，指网络代理在完成任务时无意中泄露任务无关的用户信息，并开发SPILLage框架从内容和行为两个维度分析此问题。


<details>
  <summary>Details</summary>
Motivation: 随着LLM驱动的代理在开放网络上自动化用户任务，它们会与第三方交互并留下行动痕迹。现有研究主要关注文本泄露，但代理还会通过点击、滚动等行为模式泄露信息，需要系统研究这种"过度分享"现象。

Method: 提出SPILLage框架，从渠道（内容vs行为）和直接性（显式vs隐式）两个维度分析过度分享。在真实电商网站上对180个任务进行基准测试，使用两个代理框架和三个骨干LLM进行1080次运行，评估不同缓解策略的效果。

Result: 过度分享普遍存在，行为过度分享是内容过度分享的5倍。提示级缓解效果有限甚至可能恶化问题，但执行前移除任务无关信息可将任务成功率提高17.9%。

Conclusion: 保护网络代理隐私需要更广泛的"输出"视角，不仅要关注代理输入的内容，还要关注它们在网络上的行为。减少过度分享不仅能保护隐私，还能提高任务成功率。
...
### [283] [REMem: Reasoning with Episodic Memory in Language Agent](https://arxiv.org/abs/2602.13530)
...
<details>
  <summary>Details</summary>
Motivation: 人类擅长在时空背景下记忆具体经历并进行跨事件推理（叙事记忆），而当前语言代理的记忆主要是语义的，无法有效回忆和推理交互历史。现有工作往往忽视叙事性、缺乏明确的事件建模，或过度强调简单检索而非复杂推理。

Method: REMem采用两阶段框架：1）离线索引阶段，将经验转换为混合记忆图，灵活链接时间感知的要点和事实；2）在线推理阶段，使用具有精心设计工具的代理检索器在记忆图上进行迭代检索。

Result: 在四个叙事记忆基准测试中，REMem显著优于最先进的记忆系统（如Mem0和HippoRAG 2），在叙事回忆和推理任务上分别获得3.4%和13.4%的绝对提升。此外，REMem对不可回答问题表现出更稳健的拒绝行为。

Conclusion: REMem通过构建混合记忆图和代理检索机制，有效解决了语言代理的叙事记忆和推理问题，为构建更接近人类记忆能力的智能代理提供了有前景的方向。
...
### [284] [OpAgent: Operator Agent for Web Navigation](https://arxiv.org/abs/2602.13559)
...
TL;DR: 提出在线强化学习WebAgent，通过直接与网站交互优化策略，结合分层多任务微调、在线强化学习和模块化操作代理，在WebArena上达到71.6%的SOTA成功率。


<details>
  <summary>Details</summary>
Motivation: 现有基于监督微调或离线强化学习的方法存在严重的分布偏移问题，离线轨迹无法捕捉真实网站环境的随机状态转换和实时反馈，需要更鲁棒的在线交互方法。

Method: 1) 分层多任务微调：基于规划、行动和基础三类功能原语构建数据集，训练具有强指令跟随能力的视觉语言模型；2) 在线强化学习：开发在线交互环境，采用混合奖励机制（WebJudge整体评估+RDT进度奖励）；3) 操作代理：模块化框架包含规划器、基础器、反射器和总结器，实现错误恢复和自我纠正。

Result: 强化学习增强模型在WebArena上达到38.1%成功率（pass@5），超越所有现有单体基线；模块化OpAgent框架将性能提升至71.6%的SOTA成功率。

Conclusion: 通过在线强化学习和模块化代理框架的结合，有效解决了传统方法的分布偏移问题，在复杂网站导航任务中实现了显著的性能提升，为自主网络代理提供了鲁棒的解决方案。
...
### [285] [Who Do LLMs Trust? Human Experts Matter More Than Other LLMs](https://arxiv.org/abs/2602.13568)
*Anooshka Bajaj,Zoran Tiganj*

Main category: cs.AI

TL;DR: LLMs在决策任务中表现出对人类专家反馈的显著偏好，即使专家意见错误时也会遵从，这揭示了LLMs存在类似人类的基于可信度的社会影响模式。


<details>
  <summary>Details</summary>
Motivation: 研究LLMs在遇到社会信息（如其他智能体答案、工具输出或人类建议）时是否表现出类似人类的社会影响模式，特别是是否更倾向于接受人类而非其他LLM的反馈。

Method: 通过三个二元决策任务（阅读理解、多步推理、道德判断），向四个指令调优的LLMs呈现被标记为来自朋友、人类专家或其他LLMs的先前回答。操纵群体是否正确以及群体规模。第二个实验引入单个人类与单个LLM之间的直接分歧。

Result: 在所有任务中，模型显著更倾向于遵从标记为来自人类专家的回答，即使该信号是错误的；并且相比于其他LLMs，模型更愿意向专家意见修正自己的答案。
...
### [286] [Differentiable Rule Induction from Raw Sequence Inputs](https://arxiv.org/abs/2602.13583)
...
<details>
  <summary>Details</summary>
Motivation: 传统可微分归纳逻辑编程(ILP)方法依赖符号数据集，难以直接从原始数据学习规则，存在显式标签泄漏问题（无法将连续输入映射到符号变量而不需要输入特征标签的显式监督）

Method: 整合自监督可微分聚类模型与新型可微分ILP模型，使系统能够直接从原始数据学习规则，无需显式标签泄漏

Result: 该方法能够直观且精确地从时间序列和图像数据中学习泛化规则，学习到的规则通过特征有效描述原始数据

Conclusion: 通过结合自监督聚类与可微分ILP，成功解决了直接从原始数据学习规则时的显式标签泄漏问题，提高了规则学习在原始数据上的适用性
...
### [287] [A First Proof Sprint](https://arxiv.org/abs/2602.13587)
*Joseph Corneli*

Main category: cs.AI

TL;DR: 本文报告了一个多智能体证明冲刺项目，针对10个研究级问题，结合快速草稿生成与对抗性验证、针对性修复和显式溯源。通过依赖关系图分解定位漏洞，协调评审驱动的修订，最终获得明确但异质的结果。


<details>
  <summary>Details</summary>
Motivation: 旨在探索在压缩时间框架内进行高质量数学证明验证的方法，解决传统同行评审流程缓慢的问题，同时提高证明可靠性和校准度。

Method: 采用多智能体协作工作流：1) 快速草稿生成；2) 对抗性验证；3) 针对性修复；4) 显式溯源追踪。使用依赖关系图分解来定位证明漏洞，协调评审驱动的修订过程。

Result: 问题3在限定标准下获得验证完整的存在性路径；问题5在F_O局部连通谱的限定形式下解决；问题10在明确假设条件下解决；问题4和6获得部分结果；问题7通过旋转路径定理链暂时关闭。在QC验证层，问题7和9有节点级验证但仍有未解决的验证缺口。

Conclusion: 结构感知验证和层切换策略能显著提高压缩证明冲刺的可靠性和校准度，为快速数学验证提供了有效方法论框架。
...
### [288] [Hippocampus: An Efficient and Scalable Memory Module for Agentic AI](https://arxiv.org/abs/2602.13594)
*Yi Li,Lianjie Cao,Faraz Ahmed,Puneet Sharma,Bingzhe Li*

Main category: cs.AI

TL;DR: Hippocampus是一个代理AI记忆管理系统，使用紧凑二进制签名进行语义搜索，通过无损令牌ID流进行精确内容重建，显著降低检索延迟和存储开销。


<details>
  <summary>Details</summary>
Motivation: 代理AI需要持久记忆来存储超出LLM有限上下文窗口的用户特定历史。现有记忆系统使用密集向量数据库或知识图谱遍历，存在检索延迟高和存储可扩展性差的问题。

Method: 引入Hippocampus系统，核心是动态小波矩阵（DWM），压缩并共同索引二进制签名流和令牌ID流，支持在压缩域中进行超快速搜索，避免昂贵的密集向量或图计算。

Result: 评估显示Hippocampus将端到端检索延迟降低高达31倍，将每查询令牌占用减少高达14倍，同时在LoCoMo和LongMemEval基准测试中保持准确性。

Conclusion: Hippocampus的设计随着记忆大小线性扩展，适用于长期代理部署，解决了现有记忆系统的高延迟和可扩展性问题。
...
### [289] [The Quantization Trap: Breaking Linear Scaling Laws in Multi-Hop Reasoning](https://arxiv.org/abs/2602.13595)
*Henry Han,Xiyang Liu,Xiaodong Wang,Fei Han,Xiaodong Li*

Main category: cs.AI

TL;DR: 研究发现神经缩放定律在多跳推理任务中失效，降低精度（16位到8/4位）反而增加能耗并降低准确性，存在"量化陷阱"


<details>
  <summary>Details</summary>
Motivation: 挑战当前AI行业"越小越好"的启发式方法，揭示在复杂推理任务中降低数值精度可能适得其反

Method: 通过理论分解分析，识别硬件转换开销、反量化内核的隐藏延迟成本，以及顺序能量摊销失败等因素

Result: 发现多跳推理中存在量化陷阱，降低精度反而增加净能耗并降低推理准确性，缩放定律在实践中不可避免地被打破

Conclusion: 对于复杂推理任务，行业"越小越好"的启发式方法在数学上是适得其反的，需要重新考虑精度降低策略
...
### [290] [DiffusionRollout: Uncertainty-Aware Rollout Planning in Long-Horizon PDE Solving](https://arxiv.org/abs/2602.13616)
...
<details>
  <summary>Details</summary>
Motivation: 自回归扩散模型在长时程偏微分方程预测中面临误差累积问题，需要一种机制来量化预测不确定性并改善长期预测可靠性。

Method: 基于扩散模型预测不确定性与标准偏差之间的强相关性，提出自适应选择步长的机制，在自回归展开过程中根据预测置信度动态调整步长。

Result: 在长轨迹偏微分方程预测基准测试中验证了不确定性度量的有效性，实现了更低的预测误差和更长的预测轨迹，同时保持与真实值的高相关性。

Conclusion: DiffusionRollout通过自适应步长选择策略有效缓解了自回归扩散模型中的误差累积问题，提高了长时程物理系统预测的可靠性。
...
### [291] [Guided Collaboration in Heterogeneous LLM-Based Multi-Agent Systems via Entropy-Based Understanding Assessment and Experience Retrieval](https://arxiv.org/abs/2602.13639)
*Linlin Wang,Tianqing Zhu,Laiqiao Qin,Longxiang Gao,Wanlei Zhou*

Main category: cs.AI

TL;DR: 该论文发现异构多智能体系统中强-弱协作可能不如弱-弱组合，提出基于熵的自适应指导框架来解决认知不匹配问题。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型在多智能体系统中的广泛应用，异构系统中不同能力的智能体之间存在认知不匹配问题，导致强-弱协作效率低下，甚至不如弱-弱组合。

Method: 提出基于熵的自适应指导框架，通过多维度熵度量（表达、不确定性、结构、连贯性、相关性）量化弱智能体的理解程度，动态调整指导强度（轻度、中度、重度），并结合检索增强生成机制保留成功协作经验。

Result: 在GSM8K、MBPP和CVRP三个基准数据集上的实验表明，该方法能持续提升异构协作的效果和稳定性，有效缓解认知不平衡问题。
...
### [292] [Building Autonomous GUI Navigation via Agentic-Q Estimation and Step-Wise Policy Optimization](https://arxiv.org/abs/2602.13653)
*Yibo Wang,Guangda Huzhang,Yuwei Hu,Yu Xia,Shiyin Lu,Qing-Guo Chen,Zhao Xu,Weihua Luo,Kaifu Zhang,Lijun Zhang*

Main category: cs.AI

TL;DR: 提出基于MLLM的GUI智能体框架，包含agentic-Q评估和逐步策略优化，降低数据收集成本，实现稳定高效优化，在GUI导航和定位任务上表现优异。
...
Method: 提出MLLM为中心的GUI智能体框架：1) agentic-Q评估：优化Q模型生成逐步值评估动作对任务完成的贡献；2) 逐步策略优化：以状态-动作轨迹的逐步样本为输入，通过强化学习和agentic-Q模型优化策略。

Result: 框架赋予Ovis2.5-9B强大的GUI交互能力，在GUI导航和定位基准测试中表现卓越，甚至超越更大规模的竞争对手。

Conclusion: 该框架通过自生成轨迹降低数据收集成本，环境解耦确保稳定优化，为GUI智能体提供了高效实用的解决方案。
...
### [293] [HyFunc: Accelerating LLM-based Function Calls for Agentic AI through Hybrid-Model Cascade and Dynamic Templating](https://arxiv.org/abs/2602.13665)
*Weibin Liao,Jian-guang Lou,Haoyi Xiong*

Main category: cs.AI

TL;DR: HyFunc是一个消除AI代理系统中LLM函数调用冗余的框架，通过混合模型级联和动态模板技术，在保持性能的同时显著降低推理延迟


<details>
  <summary>Details</summary>
Motivation: 当前基于LLM的AI代理系统在将用户意图转换为结构化函数调用时存在三种计算冗余：1)每次请求都要处理大量函数描述；2)使用大型模型生成整个可预测的token序列；3)生成固定的样板参数语法。这些冗余导致高推理延迟，阻碍实时应用。

Method: HyFunc采用混合模型级联：大型模型将用户意图提炼为单个"软token"，该token指导轻量级检索器选择相关函数，并引导较小的前缀调优模型生成最终调用。同时使用"动态模板"技术，在扩展的vLLM引擎中动态注入样板参数语法，避免语法冗余。

Result: 在未见过的BFCL基准数据集上，HyFunc实现了0.828秒的推理延迟，优于所有基线模型，同时达到80.1%的性能，超越了所有参数规模相当的模型，在效率和性能之间取得了良好平衡。

Conclusion: HyFunc为AI代理系统提供了一种更高效的新范式，通过消除冗余处理实现了显著的延迟降低，同时保持了良好的性能表现，有望推动实时AI应用的发展。
...
### [294] [AllMem: A Memory-centric Recipe for Efficient Long-context Modeling](https://arxiv.org/abs/2602.13680)
*Ziming Wang,Xiang Wang,Kailong Peng,Lang Qin,Juan Gabriel Kostelec,Christos Sourmpis,Axel Laborieux,Qinghai Guo*

Main category: cs.AI

TL;DR: AllMem是一种混合架构，结合滑动窗口注意力与非线性的测试时训练记忆网络，旨在解决LLM在长序列任务中的计算复杂度和内存开销问题，实现超长上下文的有效扩展。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在长序列任务中面临性能瓶颈，主要源于自注意力机制的计算复杂度和内存开销。现有线性记忆模型存在表示约束，而全局注意力在超长上下文中的计算成本过高。

Method: 提出AllMem混合架构：1) 集成滑动窗口注意力与非线性的测试时训练记忆网络；2) 实现记忆高效微调策略，将预训练模型的标准注意力层替换为记忆增强的滑动窗口层；3) 参数化记忆机制缓解噪声并保持长距离建模能力。

Result: 4k窗口模型在37k LongBench上实现接近无损性能（仅下降0.83分）；8k窗口变体在128k InfiniteBench上超越完整注意力表现，验证了参数化记忆在缓解噪声和保持长距离建模方面的有效性。

Conclusion: AllMem能够有效扩展LLM到超长上下文，缓解灾难性遗忘，同时显著降低长序列推理的计算和内存开销，为将现成预训练LLM转化为高效长序列模型提供了可行框架。
...
### [295] [PhGPO: Pheromone-Guided Policy Optimization for Long-Horizon Tool Planning](https://arxiv.org/abs/2602.13691)
*Yu Li,Guangfeng Cai,Shengtian Yang,Han Luo,Shuo Han,Xu He,Dong Li,Lei Feng*

Main category: cs.AI

TL;DR: 论文提出Pheromone-Guided Policy Optimization (PhGPO)，通过从历史轨迹中学习工具转换模式（信息素）来指导策略优化，改进大语言模型代理的长时域工具规划能力。


<details>
  <summary>Details</summary>
Motivation: 大语言模型代理在复杂任务中表现出强大的工具使用能力，但长时域多步骤工具规划面临组合爆炸的挑战。即使找到正确的工具使用路径，通常也只被视为当前训练的即时奖励，无法为后续训练提供可重用的信息。

Method: 受蚁群优化启发，提出信息素引导策略优化(PhGPO)：1)从历史轨迹中学习轨迹级的工具转换模式（信息素）；2)使用学习到的信息素来指导策略优化，引导策略向历史上成功的工具转换方向优化。

Result: 综合实验结果表明，PhGPO方法在长时域工具规划任务中表现出有效性，能够利用历史成功轨迹中的可重用信息改进规划性能。

Conclusion: 历史成功轨迹包含可重用的工具转换模式，通过信息素引导策略优化可以显著提升大语言模型代理的长时域工具规划能力，为解决组合爆炸问题提供了新思路。
...
### [296] [Can a Lightweight Automated AI Pipeline Solve Research-Level Mathematical Problems?](https://arxiv.org/abs/2602.13695)
*Lve Meng,Weilong Zhao,Yanzhi Zhang,Haoxiang Guan,Jiyan He*

Main category: cs.AI

TL;DR: LLMs结合自动化流程和基于引用的验证，能够解决研究级数学问题，在ICCM竞赛题和"首次证明"数据集上取得验证通过的成果。


<details>
  <summary>Details</summary>
Motivation: 虽然LLMs在数学竞赛基准测试中表现出色，但在研究问题上的轻量级自然语言流程部署仍未被充分探索。本文旨在探索下一代LLMs通过优化流程解决复杂研究级数学问题的能力。

Method: 构建了一个简化的自动化流程，集成了下一代LLMs（如Gemini 3 Pro、GPT-5.2 Pro），并优化了基于引用的验证机制。在两个新数据集上评估：ICCM问题集（类似丘成桐大学生数学竞赛）和"首次证明"问题集（包含未发表的研究问题）。

Result: 流程为所有ICCM前两套问题和"首次证明"问题集生成了候选证明。ICCM前两套问题和"首次证明"问题4的解决方案已完全验证通过。所有生成的证明已提交官方组织，结果公开可用。

Conclusion: 下一代LLMs通过优化的自动化流程能够有效解决研究级数学问题，展示了AI在数学研究中的实际应用潜力。计划开源完整流程方法。
...
TL;DR: OneLatent通过将推理过程压缩为单个潜在token，大幅减少输出长度（11倍），仅牺牲少量精度（2.21%下降），同时提升token贡献度6.8倍


<details>
  <summary>Details</summary>
Motivation: 解决CoT推理成本过高的问题（增加1-2个数量级推理成本），需要压缩中间推理步骤以减少计算开销

Method: 将文本推理步骤渲染为图像，通过DeepSeek-OCR隐藏状态监督，将中间推理压缩为单个潜在token

Result: 输出长度平均减少11倍，精度仅下降2.21%，token贡献度提升6.8倍；在长链推理任务上达到99.80%（ProntoQA）和97.80%（ProsQA），压缩比达87.4倍

Conclusion: OneLatent框架有效压缩推理过程，显著降低计算成本，同时保持高性能，支持压缩约束下的泛化能力
...
TL;DR: OR-Agent是一个可配置的多智能体研究框架，通过结构化树状工作流管理科学发现过程，结合进化-系统化构思机制和分层优化反思系统，在组合优化和仿真场景中超越传统进化方法。


<details>
  <summary>Details</summary>
Motivation: 自动化科学发现在复杂实验驱动领域中需要超越简单的程序迭代突变，需要结构化假设管理、环境交互和原则性反思。现有方法通常局限于简单的突变-交叉循环，缺乏对研究轨迹的系统化控制。

Method: 1) 结构化树状工作流：将研究组织为树状结构，显式建模分支假设生成和系统回溯；2) 进化-系统化构思机制：统一进化选择研究起点、全面研究计划生成和协调探索；3) 分层优化反思系统：短期实验反思作为"语言梯度"提供即时纠正信号，长期反思积累跨实验洞察作为"语言动量"，记忆压缩作为正则化机制防止漂移。

Result: 在经典组合优化基准测试（旅行商问题、容量车辆路径、装箱问题、定向问题、多重背包问题）和基于仿真的协同驾驶场景中，OR-Agent超越了强大的进化基线，同时提供了一个通用、可扩展和可检查的AI辅助科学发现框架。

Conclusion: OR-Agent通过结构化假设管理、进化-系统化构思和分层反思系统，为自动化科学发现提供了一个原则性架构，在复杂实验驱动领域中表现出色，代码和数据已开源。
...
<details>
  <summary>Details</summary>
Motivation: 尽管大型基础模型在语言理解、视觉和推理方面取得了突破，但这些系统相互隔离，难以共享能力。整合这些独立基础模型的互补优势对于构建可信赖的智能系统至关重要，但目前缺乏协调这些黑盒异构模型的成熟方法。

Method: StackingNet采用元集成框架，基于集体智能原理，在推理过程中结合多个模型的预测结果。该框架无需访问模型的内部参数或训练数据，能够识别或修剪降低性能的模型。

Result: 在语言理解、视觉估计和学术论文评分等任务中，StackingNet相比单个模型和经典集成方法，在准确性、鲁棒性和公平性方面都有持续改进。能够提升准确性、减少偏见、实现可靠性排名。
...
TL;DR: 论文提出Vashista稀疏注意力机制，基于理论证明注意力可集中在少量关键token上，实现长上下文推理的稳定加速。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在长上下文推理中注意力计算成本高，但实际只有少量token对每个查询有实质贡献。需要理论分析和实用机制来安全地实现稀疏注意力。

Method: 1) 理论分析：将注意力建模为关键向量凸包上的投影，提出面稳定性定理，证明在严格互补边际条件下，熵注意力会集中在常数大小的活动面上；2) 提出Vashista稀疏注意力：基于页面式上下文选择策略维护每个查询的小候选集，与现代推理栈兼容。

Result: 1) 理论：非活动token的总质量呈指数衰减(exp(-Ω(Δ/ε)))，活动面上的误差与温度参数ε呈线性关系；2) 实践：在长上下文评估中观察到稳定的常数大小有效支持、显著的时钟加速，以及在支持间隙诊断预测的范围内质量下降最小。

Conclusion: 该研究为稀疏长上下文解码提供了理论依据和实用机制，在隐私敏感和隔离环境中可实现可预测的延迟和成本，无需外部检索依赖。
...
<details>
  <summary>Details</summary>
Motivation: 需要系统评估LLM生成的智能合约质量，目前缺乏可重复的基准来量化合成质量、识别系统错误模式，并支持形式验证和合规检查。

Method: 使用CrewAI风格的代理团队进行迭代优化，将合同文本解析为结构化模式，生成Solidity代码，通过编译和安全检查进行自动质量评估，并在五个维度上测量质量。

Result: 框架支持与真实实现的配对评估，量化对齐度并识别逻辑遗漏和状态转换不一致等系统错误模式，为智能合约合成质量提供可重复基准。

Conclusion: 该框架为智能合约合成质量的实证研究提供了可重复基准，并支持扩展到形式验证和合规检查，有助于提高LLM生成智能合约的可靠性和安全性。
...
TL;DR: 提出统一框架解决在线实验瓶颈：通过历史数据和内容嵌入指导变体测试优先级、解释胜出原因、发现高潜力新变体，已集成到Adobe产品中。


<details>
  <summary>Details</summary>
Motivation: 现代在线实验面临两个瓶颈：流量稀缺导致难以选择测试变体，事后洞察提取依赖人工、不一致且忽视内容特性。同时，组织未能充分利用历史A/B测试结果和丰富的内容嵌入来指导优先级排序和创意迭代。

Method: 提出统一框架：1) 利用处理嵌入和历史结果训练CTR排序模型，平衡价值和内容多样性；2) 将处理映射到语义营销属性，通过符号一致稀疏约束Lasso获得可解释系数；3) 计算机会指数结合属性重要性和当前实验中的表达不足；4) 使用LLM将机会转化为具体创意建议并估计学习和转化潜力。

Result: 该框架已集成到Adobe产品"Experimentation Accelerator"中，为商业客户提供AI驱动的洞察和机会。在真实世界实验中的评估验证了生成管道的高质量。

Conclusion: 该框架解决了在线实验的关键瓶颈，通过AI驱动的优先级排序、解释和机会发现，实现了更快、更信息丰富、更高效的测试周期。
...
TL;DR: 论文提出随着AI研究生成变得廉价，可审计性成为瓶颈，主张将声明级可审计性作为深度研究代理的核心设计目标，并引入AAR标准和语义溯源框架来解决验证问题。


<details>
  <summary>Details</summary>
Motivation: 当前AI研究代理能快速生成科学报告，但验证成本高昂。随着研究生成变得廉价，主要风险从孤立事实错误转向声明-证据链接薄弱、缺失或误导性的科学风格输出，可审计性成为瓶颈。

Method: 提出声明级可审计性作为设计目标，识别长期失败模式（目标漂移、瞬态约束、不可验证推理），引入AAR标准（溯源覆盖率、溯源健全性、矛盾透明度、审计工作量），并倡导语义溯源与协议化验证的持久可查询溯源图。

Result: 建立了可审计性作为可测试目标的框架，提出了具体的测量维度和验证方法，为深度研究代理的可审计性提供了系统化的解决方案。

Conclusion: 随着AI研究生成成本降低，可审计性成为关键瓶颈，需要将声明级可审计性作为核心设计原则，通过语义溯源和持续验证机制来确保研究输出的可靠性和可验证性。
...
TL;DR: 提出MOC-HER和MOC-2HER方法，通过双重目标重标记策略解决稀疏奖励多目标环境中分层强化学习的性能问题


<details>
  <summary>Details</summary>
Motivation: 现有分层强化学习方法（如Option-Critic和MOC）在稀疏奖励的多目标环境中表现不佳，特别是在物体操纵任务中，奖励取决于物体到达目标而非智能体直接交互，这使得HRL智能体难以发现如何与物体交互

Method: 1. 提出MOC-HER：将Hindsight Experience Replay机制集成到MOC框架中，通过重标记已实现结果的目标来解决稀疏奖励环境
2. 提出MOC-2HER：引入双重目标Hindsight Experience Replay，创建两组虚拟目标：除了基于物体最终状态重标记目标（标准HER），还从智能体效应器位置生成目标，奖励智能体既与物体交互又完成任务

Result: 在机器人操纵环境中，MOC-2HER达到高达90%的成功率，而MOC和MOC-HER的成功率均低于11%，双重目标重标记策略在稀疏奖励多目标任务中效果显著

Conclusion: 提出的双重目标重标记策略能有效解决稀疏奖励多目标环境中分层强化学习的性能瓶颈，特别是在物体操纵任务中，通过同时奖励与物体交互和完成任务，显著提升了学习效率和成功率
...
Method: 引入Ambient Physics框架：随机掩码已观测测量值的子集，并在这些掩码点上进行监督学习，使模型无法区分"真正未观测"和"人工未观测"点，从而必须在所有位置产生合理预测。
...
TL;DR: 提出VSAL框架，通过自适应布局生成器动态创建信息丰富的图可视化，提升图属性检测性能
...
Method: 提出VSAL框架，包含自适应布局生成器，能够为每个图实例动态生成信息丰富的可视化布局，然后使用视觉模型进行属性检测

Result: 在哈密顿环、平面性、无爪性、树检测等多个任务上，VSAL超越了现有最先进的基于视觉的方法
...
TL;DR: 该论文提出了一套简单、计算成本低、任务无关的指标来检测和区分思维链推理中的三种病理模式：事后合理化、编码推理和内化推理。


<details>
  <summary>Details</summary>
Motivation: 思维链推理是现代LLM架构的基础，也是AI安全的关键干预点。然而，思维链推理可能存在病理模式，这些病理会妨碍其监控有效性。先前研究已识别出三种病理：事后合理化、编码推理和内化推理，但缺乏有效的检测方法。

Method: 1. 创建了一套具体指标，这些指标简单易实现、计算成本低且任务无关；2. 开发了经过专门训练以展现特定思维链病理的模型生物来验证方法；3. 提供实用的工具包来评估思维链病理。

Result: 成功开发了能够检测和区分三种思维链病理的指标系统，并通过专门训练的模型生物验证了方法的有效性。该工具包可直接应用于训练时监控。

Conclusion: 该研究为评估思维链推理病理提供了实用的工具包，对训练时监控具有直接意义，有助于提高LLM推理的透明度和安全性。
...
### [308] [From Pixels to Policies: Reinforcing Spatial Reasoning in Language Models for Content-Aware Layout Design](https://arxiv.org/abs/2602.13912)
...
<details>
  <summary>Details</summary>
Motivation: 解决大语言模型在图形布局设计中的两个关键挑战：1）LLMs的空间推理能力有限；2）设计决策过程缺乏透明度。传统方法在像素级别操作，无法提供可解释的设计决策。

Method: 将布局设计重新定义为在结构化文本空间环境上的策略学习问题，该环境显式编码画布几何、元素属性和元素间关系。使用多目标空间批判将布局质量分解为几何有效性、关系一致性和美学一致性，并通过相对群体优化来稳定开放式设计空间中的学习。

Result: LaySPA在结构有效性和视觉质量方面都有提升，优于更大的专有LLMs，性能与专门的SOTA布局生成器相当，同时需要更少的标注样本和更低的延迟。

Conclusion: LaySPA通过强化学习框架成功赋予LLMs显式空间推理能力，实现了透明可控的设计决策，在布局设计任务中表现出色，平衡了性能、效率和可解释性。
...
### [309] [HyMem: Hybrid Memory Architecture with Dynamic Retrieval Scheduling](https://arxiv.org/abs/2602.13933)
*Xiaochen Zhao,Kaikai Wang,Xiaowen Zhang,Chen Yao,Aili Wang*

Main category: cs.AI

TL;DR: HyMem提出混合记忆架构，通过多粒度记忆表示实现动态按需调度，在长对话中平衡效率与性能，计算成本降低92.6%


<details>
  <summary>Details</summary>
Motivation: 现有LLM代理在短文本中表现良好，但在长对话中因记忆管理效率低下而表现不佳。现有方法面临效率与效果的根本权衡：记忆压缩可能丢失复杂推理所需的关键细节，而保留原始文本则对简单查询引入不必要的计算开销。核心问题在于单一记忆表示和静态检索机制无法模拟人类灵活主动的记忆调度能力。

Method: HyMem采用混合记忆架构，包含双粒度存储方案和动态两级检索系统：轻量级模块构建摘要级上下文以实现高效响应生成，而基于LLM的深度模块仅针对复杂查询选择性激活，并通过反思机制进行迭代推理优化。

Result: 在LOCOMO和LongMemEval基准测试中，HyMem表现出色，优于完整上下文方法，同时将计算成本降低92.6%，在长期记忆管理中实现了效率与性能的最优平衡。

Conclusion: HyMem通过认知经济原则启发的混合记忆架构，成功解决了长对话中LLM代理的记忆管理问题，实现了动态按需调度，在保持高性能的同时显著降低计算成本。
...
### [310] [Statistical Early Stopping for Reasoning Models](https://arxiv.org/abs/2602.13935)
*Yangxinyu Xie,Tao Wang,Soham Mallick,Yan Sun,Georgy Noarov,Mengxin Yu,Tanwi Mallick,Weijie J. Su,Edgar Dobriban*

Main category: cs.AI

TL;DR: 论文提出两种基于统计原理的早期停止方法，通过监控生成过程中的不确定性信号来减少LLM在推理时的过度思考问题，提高效率和可靠性。


<details>
  <summary>Details</summary>
Motivation: LLMs在推理能力上虽有显著提升，但在面对不确定、表述不清或模糊的查询时，经常会出现过度思考现象，生成不必要的推理步骤，影响效率和可靠性。

Method: 提出两种统计原理的早期停止方法：1）参数化方法：将不确定性关键词的出现间隔时间建模为更新过程，应用序列测试进行停止决策；2）非参数化方法：为定义明确的查询提供有限样本保证，确保不会过早停止。

Result: 在多领域推理任务和多个模型上的实验表明，基于不确定性的早期停止方法能显著提高LLM推理的效率和可靠性，尤其在数学推理任务上观察到特别显著的改进。

Conclusion: 监控不确定性信号的统计早期停止方法能有效解决LLM过度思考问题，在保持推理质量的同时提高生成效率，为LLM推理优化提供了有前景的方向。
...
### [311] [A Generalizable Physics-guided Causal Model for Trajectory Prediction in Autonomous Driving](https://arxiv.org/abs/2602.13936)
*Zhenyu Zong,Yuchen Wang,Haohong Lin,Lu Gan,Huajie Shao*

Main category: cs.AI

TL;DR: 提出PCM模型，通过解耦场景编码器和因果ODE解码器，利用领域不变特征和运动学模型增强轨迹预测的零样本泛化能力


<details>
  <summary>Details</summary>
Motivation: 交通参与者的轨迹预测对自动驾驶安全至关重要，但现有方法在未见领域的零样本泛化能力有限。由于运动学规律在不同领域具有一致性，作者希望利用领域不变知识来提升零样本轨迹预测能力
...
Conclusion: 通过结合领域不变特征提取和运动学模型整合，PCM模型能够有效提升轨迹预测的零样本泛化能力，为自动驾驶系统在未知环境中的安全运行提供了解决方案
...
### [312] [Neuromem: A Granular Decomposition of the Streaming Lifecycle in External Memory for LLMs](https://arxiv.org/abs/2602.13967)
...
Conclusion: Neuromem提供了一个系统化的测试平台来评估流式场景下的外部记忆模块，揭示了记忆生命周期各维度对性能和成本的影响，为实际部署提供了重要指导。
...
### [313] [Cognitive Chunking for Soft Prompts: Accelerating Compressor Learning via Block-wise Causal Masking](https://arxiv.org/abs/2602.13980)
*Guojie Liu,Yiqi Wang,Yanfeng Yang,Wenqi Fan,Songlei Jian,Jianfeng Zhang,Jie Yu*

Main category: cs.AI

TL;DR: 提出PIC方法，通过修改Transformer注意力掩码将长上下文压缩为局部块，降低压缩器训练难度，在QA等任务上显著优于基线，尤其在64倍高压缩比时提升明显。
...
Method: 提出并行迭代压缩(PIC)，通过修改Transformer注意力掩码，将内存令牌的感受野限制在顺序局部块中，降低压缩器训练难度。

Result: 在多个下游任务中优于基线，64倍压缩比时QA任务F1提升29.8%、EM提升40.7%，16倍压缩器训练时间减少约40%且性能更好。

Conclusion: PIC通过局部化压缩有效降低训练难度，在高压缩比场景下表现优异，显著提升训练效率，为长上下文压缩提供了新思路。
...
### [314] [Bridging AI and Clinical Reasoning: Abductive Explanations for Alignment on Critical Symptoms](https://arxiv.org/abs/2602.13985)
*Belona Sonna,Alban Grastien*

Main category: cs.AI

TL;DR: 该论文提出使用形式化溯因解释来提升医疗AI的可信度，通过最小充分特征集保证推理一致性，使AI决策与临床推理对齐。


<details>
  <summary>Details</summary>
Motivation: 当前AI在临床诊断中虽然准确率高，但其推理过程常偏离结构化临床框架，导致信任度低、可解释性差。现有事后解释方法透明度有限且缺乏形式化保证，关键症状可能被AI忽略。

Method: 采用形式化溯因解释方法，提供对最小充分特征集的一致且有保证的推理。这种方法使AI决策过程更清晰，并能与临床推理框架对齐。

Result: 该方法在保持预测准确性的同时，提供临床可操作的洞察，建立了医疗诊断中可信AI的稳健框架。

Conclusion: 形式化溯因解释为解决医疗AI的可信度问题提供了有效途径，通过保证推理一致性和与临床框架对齐，促进了AI在医疗诊断中的实际应用。
...
### [315] [Prompt-Driven Low-Altitude Edge Intelligence: Modular Agents and Generative Reasoning](https://arxiv.org/abs/2602.14003)
*Jiahao You,Ziye Jia,Chao Dong,Qihui Wu*

Main category: cs.AI

TL;DR: P2AECF是一个提示到代理的边缘认知框架，通过将高级语义提示转换为可执行的推理工作流，解决大型AI模型在边缘部署的灵活性、效率和适应性挑战。
...
Method: 提出P2AECF框架，包含三个关键机制：1) 提示定义认知：将任务意图解析为抽象且模型无关的表示；2) 基于代理的模块化执行：根据当前资源条件动态选择轻量级可重用认知代理；3) 扩散控制推理规划：结合运行时反馈和系统上下文自适应构建和优化执行策略。

Result: 通过低空智能网络用例展示了该框架能够为实时低空协作提供自适应、模块化和可扩展的边缘智能。

Conclusion: P2AECF框架通过将高级语义提示转换为可执行的推理工作流，实现了灵活、高效和自适应的边缘智能，解决了大型AI模型在边缘部署的关键限制。
...
### [316] [FloCA: Towards Faithful and Logically Consistent Flowchart Reasoning](https://arxiv.org/abs/2602.14035)
...
<details>
  <summary>Details</summary>
Motivation: 流程图导向对话系统需要遵循特定领域的流程图来指导用户完成多轮决策或操作流程。尽管LLM在任务导向对话系统中取得了进展，但将其应用于流程图导向对话仍面临两个限制：1) LLM缺乏明确的机制来表示和推理流程图拓扑结构；2) LLM容易产生幻觉，导致不忠实的流程图推理。

Method: 提出FloCA方法，使用LLM进行意图理解和响应生成，同时将流程图推理委托给执行拓扑约束图执行的外部工具，确保跨对话轮次的忠实和逻辑一致的节点转换。

Result: 在FLODIAL和PFDial数据集上的广泛实验突出了现有基于LLM方法的瓶颈，并证明了FloCA的优越性。引入了基于LLM的用户模拟器和五个新指标的评估框架，涵盖推理准确性和交互效率。
...
### [317] [Choosing How to Remember: Adaptive Memory Structures for LLM Agents](https://arxiv.org/abs/2602.14038)
*Mingfei Lu,Mengjia Wu,Feng Liu,Jiawei Xu,Weikai Li,Haoyang Wang,Zhengdong Hu,Ying Ding,Yizhou Sun,Jie Lu,Yi Zhang*

Main category: cs.AI

TL;DR: FluxMem是一个自适应记忆组织框架，通过多结构记忆选择和三层次记忆层次，提升LLM智能体在长程交互中的性能


<details>
  <summary>Details</summary>
Motivation: 现有智能体记忆系统存在两个关键问题：采用一刀切的记忆结构，且未将记忆结构选择建模为上下文自适应决策，导致处理异构交互模式能力有限，性能次优

Method: 提出FluxMem统一框架，为智能体配备多种互补记忆结构，基于交互级特征学习选择合适结构，使用下游响应质量和记忆利用率的离线监督。引入三层次记忆层次和基于Beta混合模型的概率门控进行分布感知记忆融合

Result: 在PERSONAMEM和LoCoMo两个长程基准测试中，平均分别提升9.18%和6.14%

Conclusion: FluxMem通过自适应记忆组织和概率融合机制，显著提升了LLM智能体在长程交互中的记忆管理能力
...
### [318] [REAL: Resolving Knowledge Conflicts in Knowledge-Intensive Visual Question Answering via Reasoning-Pivot Alignment](https://arxiv.org/abs/2602.14065)
*Kai Ye,Xianwei Mao,Sheng Zhou,Zirui Shao,Ye Mo,Liangliang Liu,Haikuan Huang,Bin Li,Jiajun Bu*

Main category: cs.AI

TL;DR: 提出REAL框架，通过推理枢纽概念解决知识密集型VQA中的知识冲突问题，包含RPA-SFT训练和RPGD解码策略，在多个基准测试中取得SOTA性能。
...
Method: 提出REAL框架，核心是推理枢纽概念——推理链中强调知识连接的原子单元。包含两个组件：1) RPA-SFT：通过将冲突与枢纽提取对齐来训练可泛化的判别器；2) RPGD：利用推理枢纽进行针对性冲突缓解的模型内解码策略。

Result: 在多个基准测试上的广泛实验表明，REAL显著提高了判别准确性，并实现了最先进的性能，验证了枢纽驱动解决范式的有效性。

Conclusion: REAL框架通过推理枢纽概念有效解决了KI-VQA中的知识冲突问题，提出的RPA-SFT和RPGD方法为处理冲突证据提供了可泛化的解决方案。
...
### [319] [Plan-MCTS: Plan Exploration for Action Exploitation in Web Navigation](https://arxiv.org/abs/2602.14083)
...
TL;DR: Plan-MCTS：将网页导航探索转移到语义规划空间的新框架，通过解耦战略规划与执行落地，解决稀疏路径和噪声上下文问题，在WebArena上达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 当前LLM驱动的自主代理在网页导航中面临两个关键挑战：1）稀疏的有效路径导致探索效率低下；2）噪声上下文稀释了准确的状态感知。现有方法虽然整合了树搜索来增强长时程推理，但在网页导航中效果有限。

Method: Plan-MCTS框架通过将探索转移到语义规划空间来重构网页导航：1）解耦战略规划与执行落地；2）将稀疏动作空间转换为密集规划树以提升探索效率；3）将噪声上下文提炼为抽象语义历史以增强状态感知；4）引入双重门控奖励机制验证物理可执行性和战略对齐；5）通过结构细化对失败子规划进行在线修复。

Result: 在WebArena上的大量实验表明，Plan-MCTS实现了最先进的性能，超越了现有方法，具有更高的任务有效性和搜索效率。

Conclusion: Plan-MCTS通过将网页导航探索转移到语义规划空间，有效解决了稀疏路径和噪声上下文问题，显著提升了自主代理在复杂网页导航任务中的性能表现。
...
### [320] [GUI-GENESIS: Automated Synthesis of Efficient Environments with Verifiable Rewards for GUI Agent Post-Training](https://arxiv.org/abs/2602.14093)
*Yuan Cao,Dezhi Ran,Mengzhou Wu,Yuzhe Guo,Xin Chen,Ang Li,Gang Cao,Gong Zhi,Hao Yu,Linyi Li,Wei Yang,Tao Xie*

Main category: cs.AI

TL;DR: GUI-GENESIS框架自动合成高效GUI训练环境，通过多模态代码模型重建真实应用为轻量级网页环境，提供可验证的代码原生奖励，显著降低延迟和成本，提升智能体性能。


<details>
  <summary>Details</summary>
Motivation: 在真实应用上训练GUI智能体面临高延迟、低可复现性和依赖噪声视觉代理的不可验证奖励等问题，需要开发更高效的训练环境框架。

Method: 使用多模态代码模型将真实应用重建为轻量级网页环境，并配备代码原生奖励（可执行断言），提供确定性奖励信号，消除视觉估计噪声。

Result: GUI-GENESIS将环境延迟降低10倍，每个epoch节省超过28,000美元成本；训练出的智能体在真实任务上比基础模型提升14.54%，比真实世界RL基线提升3.27%。

Conclusion: GUI-GENESIS为GUI智能体训练提供了高效、可验证的环境合成框架，显著提升训练效率和智能体性能，并展示了模型能够合成自身尚无法解决的环境，为自改进智能体开辟了途径。
...
### [321] [NEST: Nascent Encoded Steganographic Thoughts](https://arxiv.org/abs/2602.14095)
*Artem Karpov*

Main category: cs.AI

TL;DR: 研究评估大型语言模型在思维链中进行隐写的能力，发现当前模型在复杂数学任务上尚无法持续隐藏推理，但在简化计数任务中Claude Opus 4.5达到92%准确率，显示初步能力。


<details>
  <summary>Details</summary>
Motivation: 监控思维链推理是LLM代理的基础安全技术，但如果模型学会隐藏推理，这种监督就会失效。研究探索隐写思维链的潜力，以评估风险并为部署策略提供依据。

Method: 系统评估28个模型（从历史版本到前沿模型）的隐写能力极限。测量监控规避、拒绝率、编码保真度和隐藏任务准确性，在四个数据集上比较隐写藏头诗与普通推理和填充标记基线。

Result: 当前模型在复杂数学和算术任务上无法持续隐藏推理。但在简化计数实验中，Claude Opus 4.5在隐藏任务上达到92%准确率，显示初步能力。罕见情况下（<1%），GPT-5.2可能拒绝隐写指令同时遵守它们。

Conclusion: 研究强调需要持续评估隐写风险，提供了一种预先检测和预防隐藏推理的方法论，这种隐藏推理可能助长未对齐的谋划和欺骗行为。
...
### [322] [Algebraic Quantum Intelligence: A New Framework for Reproducible Machine Creativity](https://arxiv.org/abs/2602.14130)
...
TL;DR: 提出代数量子智能(AQI)框架，通过非交换代数结构扩展LLMs的语义空间，解决LLMs创造力受限的问题，在创造性推理基准测试中显著优于基线模型。


<details>
  <summary>Details</summary>
Motivation: 当前大型语言模型在生成流畅和上下文相关的文本方面表现出色，但在产生真正创造性输出方面存在局限。这种限制源于LLMs的结构特性：当提供丰富上下文时，未来生成空间变得高度受限，生成过程几乎由确定性动态控制。

Method: 提出代数量子智能(AQI)计算框架，采用受量子理论启发的非交换代数结构，实现顺序依赖、干涉和不确定性等特性。语义状态表示为希尔伯特空间中的向量，演化由非交换算子计算的C值控制。通过扩展基于Transformer的LLM，添加600多个专用算子来实现AQI。

Result: 在涵盖十个领域的创造性推理基准测试中，使用LLM作为评判协议进行评估。AQI持续优于强基线模型，产生统计显著的改进并减少跨领域方差。该架构已在真实企业环境中部署。

Conclusion: 非交换代数动态可以作为机器创造性的实用且可复现的基础。AQI框架能够系统性地扩展语义空间，解决LLMs创造力受限的根本问题。
...
### [323] [ForesightSafety Bench: A Frontier Risk Evaluation and Governance Framework towards Safe AI](https://arxiv.org/abs/2602.14135)
...
TL;DR: 提出了ForesightSafety Bench AI安全评估框架，包含7个基础安全支柱和多个高级风险维度，共94个细化风险维度，评估了20多个主流大模型，发现前沿AI在多个支柱存在广泛安全漏洞。
...
Method: 提出ForesightSafety Bench框架，从7个基础安全支柱扩展到高级风险领域，包括具身AI安全、AI4Science安全、社会与环境AI风险、灾难性和存在性风险等，共94个细化风险维度，收集了数万个结构化风险数据点。

Result: 评估了20多个主流先进大模型，发现前沿AI在多个支柱存在广泛安全漏洞，特别是在风险自主性、AI4Science安全、具身AI安全、社会AI安全以及灾难性和存在性风险方面。

Conclusion: 建立了一个广泛涵盖、层次清晰、动态演进的AI安全评估框架，揭示了前沿AI模型的关键风险模式和能力边界，为AI安全评估提供了系统性的解决方案。
...
### [324] [Process-Supervised Multi-Agent Reinforcement Learning for Reliable Clinical Reasoning](https://arxiv.org/abs/2602.14160)
*Chaeeun Lee,T. Michael Yates,Pasquale Minervini,T. Ian Simpson*

Main category: cs.AI

TL;DR: 提出基于强化学习的多智能体框架，用于基因-疾病有效性评估任务，通过过程监督提升临床推理的准确性和可解释性。


<details>
  <summary>Details</summary>
Motivation: 临床决策需要基于异构证据进行细致推理并提供可追溯的论证。现有LLM多智能体系统主要优化结果准确性，但忽视了符合临床标准的过程基础推理。基因-疾病有效性评估是一个典型案例，专家需要综合多种生物医学证据来确定基因与疾病的因果关系。

Method: 引入"智能体即工具"的强化学习框架，包含两个目标：(1)过程级监督确保推理遵循有效的临床路径；(2)通过分层多智能体系统实现高效协调。使用GRPO训练Qwen3-4B作为监督智能体。

Result: 在ClinGen数据集上的评估显示：仅使用结果奖励时，多智能体系统将最终结果准确率从0.195提升到0.732，但过程对齐性较差（0.392 F1）。结合过程和结果奖励时，结果准确率达到0.750，同时过程保真度显著提升至0.520 F1。

Conclusion: 过程监督对于临床推理任务至关重要，能够同时提升结果准确性和过程对齐性。提出的强化学习框架在基因-疾病有效性评估任务中表现出色，为临床决策支持系统提供了新的方法。
...
### [325] [Text Before Vision: Staged Knowledge Injection Matters for Agentic RLVR in Ultra-High-Resolution Remote Sensing Understanding](https://arxiv.org/abs/2602.14225)
...
TL;DR: 在超高分辨率遥感图像的多模态推理中，研究发现高质量的地球科学纯文本问答是提升视觉推理能力的主要驱动力，提出分阶段知识注入方法显著超越大型通用模型。


<details>
  <summary>Details</summary>
Motivation: 超高分辨率遥感图像的多模态推理面临视觉证据获取的瓶颈：需要在巨大的像素空间中定位微小的任务相关区域。虽然基于缩放工具的强化学习提供了一条路径，但标准强化学习在没有结构化领域先验的情况下难以导航这些广阔的视觉空间。

Method: 提出分阶段知识注入方法：1）使用可扩展的知识图验证的地球科学纯文本问答进行冷启动，注入推理结构；2）在监督微调阶段对相同的困难超高分辨率图像-文本示例进行"预热"，以稳定和增强后续基于工具的强化学习。

Result: 在XLRS-Bench上达到60.40%的Pass@1，显著优于更大的通用模型（如GPT-5.2、Gemini 3.0 Pro、Intern-S1），建立了新的最先进水平。

Conclusion: 高质量的地球科学纯文本问答是超高分辨率视觉推理能力提升的主要驱动力，分阶段的知识注入方法能有效解决视觉证据获取的瓶颈问题，为遥感图像的多模态推理提供了新的解决方案。
...
### [326] [CORPGEN: Simulating Corporate Environments with Autonomous Digital Employees in Multi-Horizon Task Environments](https://arxiv.org/abs/2602.14229)
...
TL;DR: 论文提出Multi-Horizon Task Environments (MHTEs)作为新的评估范式，针对多任务并发长时程推理挑战，开发CorpGen框架解决四个关键失败模式，在模拟企业环境中实现3.5倍性能提升。


<details>
  <summary>Details</summary>
Motivation: 现有基准测试只评估智能体在单一任务上的表现，而真实组织工作需要管理多个并发的长时程任务，涉及任务交错、依赖关系和优先级重排。需要新的评估范式和解决方案来应对这些挑战。

Method: 提出CorpGen框架，采用分层规划实现多时程目标对齐、子智能体隔离防止任务交叉污染、分层内存系统（工作内存、结构化内存、语义内存）和自适应摘要技术。通过数字员工模拟企业环境。

Result: 在三个CUA后端（UFO2、OpenAI CUA、分层架构）上，CorpGen相比基线实现最高3.5倍改进（15.2% vs 4.3%），在负载增加时保持稳定性能。消融研究表明经验学习贡献最大。

Conclusion: CorpGen框架能有效解决多时程任务环境中的四个关键失败模式，性能提升源于架构机制而非特定CUA实现，为构建能处理真实组织工作复杂性的自主智能体提供了有效方案。
...
### [327] [REDSearcher: A Scalable and Cost-Efficient Framework for Long-Horizon Search Agents](https://arxiv.org/abs/2602.14234)
...
TL;DR: REDSearcher是一个统一框架，通过协同设计复杂任务合成、中期训练和后期训练，解决大规模语言模型在深度搜索任务中高质量轨迹稀疏和奖励信号不足的问题，在文本和多模态搜索基准上达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型正从通用知识引擎转向现实世界问题解决者，但在深度搜索任务优化上仍面临挑战。主要瓶颈在于高质量搜索轨迹和奖励信号的极端稀疏性，这源于可扩展的长时程任务构建困难以及涉及外部工具调用的交互密集型rollout的高成本。

Method: 提出REDSearcher统一框架，包含四个关键改进：1) 将任务合成构建为双重约束优化，通过图拓扑和证据分散精确控制任务难度；2) 引入工具增强查询以鼓励主动工具使用而非被动回忆；3) 在中期训练中强化核心原子能力（知识、规划和函数调用）；4) 构建本地模拟环境实现快速低成本的强化学习实验迭代。

Result: 在文本和多模态搜索代理基准测试中，该方法达到了最先进的性能。研究团队将发布10K高质量复杂文本搜索轨迹、5K多模态轨迹和1K文本RL查询集，以及代码和模型检查点。
...
### [328] [Reshaping MOFs text mining with a dynamic multi-agents framework of large language model](https://arxiv.org/abs/2504.18880)
...
<details>
  <summary>Details</summary>
Motivation: MOFs合成条件信息在文献中通常分散、不一致且难以解释，这给实验设计带来了挑战。需要一种能够自动从文献中提取和标准化合成信息的方法。

Method: 开发了MOFh6系统，这是一个基于大语言模型的系统，能够读取原始文章或晶体代码，跨段落链接相关描述，统一配体缩写与全名，并输出结构化的合成参数。
...
### [329] [GRAIL: Goal Recognition Alignment through Imitation Learning](https://arxiv.org/abs/2602.14252)
...
<details>
  <summary>Details</summary>
Motivation: 现有目标识别方法通常依赖于最优目标导向策略表示，但这可能与行为者的真实行为不同，阻碍准确识别其目标。需要解决次优、有偏行为下的目标识别问题。
...
Conclusion: GRAIL为在不确定环境中解释智能体目标提供了可扩展且鲁棒的模型，能够处理次优、有偏和噪声行为，显著提升目标识别性能。
...
TL;DR: AutoWebWorld：通过将网页环境建模为有限状态机并自动生成交互网站，解决了网页GUI代理训练数据收集困难的问题，实现了低成本、可验证的轨迹生成。


<details>
  <summary>Details</summary>
Motivation: 现有自主网页GUI代理的性能严重依赖训练数据质量，但收集真实网站的交互轨迹成本高且难以验证，因为状态转换是隐式的，需要依赖昂贵的外部验证器来评估步骤正确性。

Method: 提出AutoWebWorld框架：1）将网页环境建模为有限状态机（FSM），明确定义所有状态、动作和转换规则；2）使用编码代理将FSM转换为交互式网站；3）实现程序化验证：动作正确性根据预定义规则检查，任务成功通过到达FSM图中的目标状态确认。

Result: 1）以每条轨迹仅0.04美元的成本，从29个多样化网页环境中生成了11,663条已验证轨迹；2）使用该合成数据训练的7B网页GUI代理在WebVoyager上15步内超越了所有基线；3）观察到明显的缩放定律：随着合成数据量增加，在WebVoyager和Online-Mind2Web上的性能持续提升。

Conclusion: AutoWebWorld通过将网页环境建模为有限状态机并自动生成可交互网站，解决了网页代理训练数据收集的瓶颈问题，实现了低成本、可验证的轨迹生成，显著提升了真实世界网页任务的性能表现。
...
### [331] [Benchmarking at the Edge of Comprehension](https://arxiv.org/abs/2602.14307)
...
TL;DR: 提出"批判性稳健基准测试"框架，通过对抗性生成-评估游戏来比较模型，即使人类无法完全理解任务也能保持评估完整性。


<details>
  <summary>Details</summary>
Motivation: 随着前沿大语言模型快速饱和新基准，传统基准测试面临危机：人类难以生成判别性任务、提供准确答案或评估复杂解决方案。如果基准测试不可行，我们衡量AI进展的能力将受到威胁。

Method: 提出批判性稳健基准测试框架，基于"批判性稳健正确性"概念：答案被认为是正确的，如果没有对手能令人信服地证明其错误。人类作为有界验证者，专注于局部声明。使用项目化二分Bradley-Terry模型联合排名LLMs的解题能力和生成难题能力。

Result: 在数学领域对八个前沿LLMs展示了方法的有效性，结果显示得分稳定且与外部能力测量相关。

Conclusion: 该框架将基准测试重新定义为对抗性生成-评估游戏，人类作为最终裁决者，即使在人类无法完全理解的后理解机制下也能有效比较模型能力。
...
### [332] [Competition for attention predicts good-to-bad tipping in AI](https://arxiv.org/abs/2602.14370)
*Neil F. Johnson,Frank Y. Huo*

Main category: cs.AI

TL;DR: 论文提出边缘AI设备中危险行为涌现的数学机制，基于注意力竞争揭示动态临界点n*，为安全控制提供新杠杆


<details>
  <summary>Details</summary>
Motivation: 超过半数全球人口携带可离线运行类ChatGPT模型的设备，缺乏安全监管可能导致自残、经济损失和极端主义等危险。现有安全工具要么需要云端连接，要么只能在危害发生后发现故障

Method: 提出基于注意力竞争的数学机制，认为危险行为源于原子尺度上对机器注意力的竞争，推导出动态临界点n*的数学公式，该公式由对话上下文与竞争输出盆地之间的点积注意力竞争决定

Result: 该机制在多个AI模型上得到验证，可针对不同"好"与"坏"的定义实例化，理论上适用于跨领域（健康、法律、金融、国防）、不同法律环境（欧盟、英国、美国及各州）、语言和文化设置

Conclusion: 研究揭示了边缘AI设备中危险行为涌现的根本机制，为开发无需云端连接的事前安全控制工具提供了理论基础，改变了边缘AI安全监管的范式
...
### [333] [Boule or Baguette? A Study on Task Topology, Length Generalization, and the Benefit of Reasoning Traces](https://arxiv.org/abs/2602.14404)
*William L. Tong,Ege Cakar,Cengiz Pehlevan*

Main category: cs.AI

TL;DR: 该研究通过PITA数据集（2300万条命题逻辑语句及证明）探究推理模型在长度泛化上的表现，发现推理轨迹模型在广泛浅层任务上泛化良好，但在狭窄深层任务上表现不佳。


<details>
  <summary>Details</summary>
Motivation: 尽管推理模型（生成中间推理轨迹的神经网络）近年来快速发展，但我们对推理轨迹如何支持推理以及该范式的局限性仍缺乏完整理解。需要更清晰的理论框架来理解推理模型的泛化能力，特别是在长度泛化方面的表现。

Method: 引入PITA大规模数据集（2300万条命题逻辑语句及对应证明），提出任务深度（解决问题所需步数）和任务广度（任务中独特示例数量）的概念。通过在不同深度和广度的PITA子集上测试推理轨迹模型，并与非推理轨迹基线比较。同时使用基于三段论的简单合成任务验证结果的普适性。

Result: 推理轨迹模型在广泛且浅层的任务子集上泛化良好，但在狭窄且深层的任务子集上表现相对较差（相对于非推理轨迹基线）。研究发现了限制推理模型在深层任务上表现的基本缩放规律，同时突出了它们在广泛任务上的泛化优势。

Conclusion: 该研究揭示了使用推理轨迹的内在优势和局限性：推理轨迹模型在广泛任务上具有强大的泛化能力，但在需要深层推理的狭窄任务上存在根本性限制。这些发现为理解推理模型的泛化边界提供了理论基础。
...
### [334] [Precedent-Informed Reasoning: Mitigating Overthinking in Large Reasoning Models via Test-Time Precedent Learning](https://arxiv.org/abs/2602.14451)
*Qianyue Wang,Jinwu Hu,Huanxiang Lin,Bolin Chen,Zhiquan Wen,Yaofo Chen,Yu Rong,Mingkui Tan*

Main category: cs.AI

TL;DR: PIR（先例知情推理）通过自适应选择相关先例并在测试时内部化学习，将LLM推理从自我探索转变为基于先例的引导学习，显著缩短推理轨迹同时保持或提升准确性。


<details>
  <summary>Details</summary>
Motivation: 当前LLM推理存在效率问题：冗长的思维链包含冗余的自我探索和验证，既增加计算成本又可能降低性能。受人类利用过去相关案例约束搜索空间、减少试错的推理模式启发，需要将LLM推理范式从穷举式自我探索转变为基于先例的引导学习。

Method: PIR包含两个核心组件：1）自适应先例选择（APS）：为每个问题和LLM构建紧凑的先例集，基于语义相似度和模型困惑度联合评分排序，自适应调整先例数量以最大化困惑度降低；2）测试时经验内部化（TEI）：在测试时对先例知情指令进行轻量级适配器更新，内部化解题模式并在后续推理中作为先验使用。

Result: 在数学推理、科学问答和代码生成任务上的实验表明，PIR能持续缩短推理轨迹，同时保持或提高最终准确性，在LLM中实现了出色的准确性与效率权衡。
...
### [335] [Frontier AI Risk Management Framework in Practice: A Risk Analysis Technical Report v1.5](https://arxiv.org/abs/2602.14457)
...
TL;DR: 该论文提出了前沿AI风险管理框架，评估了大型语言模型和智能体AI在五个关键维度的风险：网络攻击、说服与操纵、战略欺骗、失控AI研发和自复制，并提出了缓解策略。
...
Method: 采用更新的风险评估框架，对五个关键维度进行细粒度评估：引入更复杂的网络攻击场景、评估LLM对LLM的说服风险、新增突发性错位实验、关注智能体"错误进化"、监控OpenClaw安全性能、引入资源受限的自复制场景。

Result: 提出了针对新兴威胁的一系列稳健缓解策略，为前沿AI的安全部署提供了初步的技术和可操作路径。

Conclusion: 这项工作反映了当前对AI前沿风险的理解，并呼吁采取集体行动来缓解这些挑战，为AI安全部署提供了实践框架。
...
### [336] [Bounding Probabilities of Causation with Partial Causal Diagrams](https://arxiv.org/abs/2602.14503)
*Yuxuan Xie,Ang Li*

Main category: cs.AI

TL;DR: 提出一个利用部分因果信息来界定因果概率的通用框架，通过优化编程整合可用信息，在无需完全可识别性的情况下获得更紧且形式有效的界限。


<details>
  <summary>Details</summary>
Motivation: 因果概率对个体层面的解释和决策至关重要，但本质上是反事实的，通常无法从数据中精确识别。现有界限要么忽略可用协变量，要么需要完整的因果图，要么依赖于限制性的二元设置，限制了实际应用。现实应用中，因果信息往往是部分但非平凡的。

Method: 提出一个通用框架，将可用的结构或统计信息作为约束系统地整合到优化编程公式中，从而在无需完全可识别性的情况下获得更紧且形式有效的界限。

Result: 该方法扩展了因果概率在现实场景中的适用性，其中因果知识虽不完整但具有信息性，能够利用部分因果信息获得更优的界限。

Conclusion: 通过优化编程框架整合部分因果信息，为界定因果概率提供了更实用、更灵活的方法，解决了现有方法在现实应用中的局限性。
...
### [337] [Formally Verifying and Explaining Sepsis Treatment Policies with COOL-MC](https://arxiv.org/abs/2602.14505)
*Dennis Gross*

Main category: cs.AI

TL;DR: COOL-MC：结合模型检验与可解释性的强化学习策略验证工具，用于分析败血症治疗策略，通过可达状态空间构建、临床标签和PCTL查询解释决策驱动因素。


<details>
  <summary>Details</summary>
Motivation: 医疗领域的强化学习策略（如败血症治疗优化）通常不透明且难以验证，传统模型检验器无法处理大规模MDP，也缺乏解释策略决策原因的能力。

Method: COOL-MC基于Storm模型检验器，但增加了三个关键功能：1) 仅构建训练策略诱导的可达状态空间，生成更小的离散时间马尔可夫链；2) 自动用临床有意义的原子命题标记状态；3) 集成可解释性方法与PCTL查询，揭示决策驱动特征。

Result: 在ICU-Sepsis MDP基准测试中，COOL-MC实现了完整MDP验证的硬边界约束，训练出达到最优生存概率的安全RL策略，并通过PCTL验证和可解释性分析发现策略主要依赖既往用药史而非患者实时状况。

Conclusion: COOL-MC通过整合形式化验证与可解释性，为临床医生在部署前调查和调试败血症治疗策略提供了有效工具，揭示了标准评估无法发现的策略弱点。
...
### [338] [Diagnosing Knowledge Conflict in Multimodal Long-Chain Reasoning](https://arxiv.org/abs/2602.14518)
*Jing Tang,Kun Wang,Haolang Lu,Hongjin Chen,KaiTao Chen,Zhongxiang Sun,Qiankun Li,Lingjuan Lyu,Guoshun Nan,Zhigang Zeng*

Main category: cs.AI

TL;DR: 该论文研究了多模态大语言模型在长链推理中面临知识冲突时的失败机制，揭示了冲突在模型内部的线性可分性、深度定位、层次一致性和方向不对称性等特征。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在长链推理中，当不同知识源提供冲突信号时经常失败。研究者希望理解这些失败背后的机制，特别是知识冲突如何在模型内部被表示和处理。

Method: 通过探测内部表示，分析模型在不同知识冲突情况下的行为。区分输入级客观冲突和过程级有效冲突，并研究冲突信号在模型各层的编码方式。

Result: 发现：(1) 不同冲突类型被编码为线性可分的特征；(2) 冲突信号集中在模型中后期层；(3) 沿轨迹聚合噪声标记级信号能稳健恢复输入级冲突类型；(4) 强化模型隐式源偏好比强制相反源更容易。

Conclusion: 研究为多模态推理中的知识冲突提供了机制层面的理解，使长链推理失败的原则性诊断和控制成为可能，有助于改进多模态大语言模型的鲁棒性。
...
### [339] [Disentangling Deception and Hallucination Failures in LLMs](https://arxiv.org/abs/2602.14529)
*Haolang Lu,Hongrui Peng,WeiYe Fu,Guoshun Nan,Xinye Cao,Xingrui Li,Hongcan Guo,Kun Wang*

Main category: cs.AI

TL;DR: 该论文提出将LLM事实错误分为知识存在与行为表达两个层面，区分幻觉与欺骗两种不同机制，并构建受控环境进行系统分析


<details>
  <summary>Details</summary>
Motivation: 传统上LLM事实错误常被归因于知识缺失，但作者认为这种观点混淆了不同失败机制。需要从内部机制角度区分知识存在与行为表达，以理解幻觉与欺骗这两种看似相似但机制不同的失败模式

Method: 构建实体中心事实问题的受控环境，保持知识不变但选择性改变行为表达，系统分析四种行为情况。通过表示可分离性、稀疏可解释性和推理时激活引导三种方法分析失败模式

Result: 论文建立了区分知识存在与行为表达的分析框架，能够系统识别和区分幻觉与欺骗两种不同的LLM失败机制，为理解模型内部工作机制提供了新视角

Conclusion: LLM的事实错误不应简单归因于知识缺失，而应从机制角度区分知识存在与行为表达。这种区分有助于更精确地诊断和解决模型失败，为改进LLM的事实准确性提供理论基础
...
### [340] [MATEO: A Multimodal Benchmark for Temporal Reasoning and Planning in LVLMs](https://arxiv.org/abs/2602.14589)
...
Method: 收集专业多模态食谱语料库，通过标准化编辑流程将指令分解为离散步骤并配图，设计可扩展众包流水线收集时序执行顺序图标注

Result: 使用MATEO评估了6个最先进的LVLMs，考察模型规模、语言上下文、多模态输入结构和微调策略的影响

Conclusion: MATEO填补了多模态时序推理评估的空白，为提升LVLMs在真实世界规划中的时序推理能力提供了基准
...
TL;DR: Arbor框架通过将决策树导航分解为专门的节点级任务，解决了大语言模型在医疗分诊等高风险领域中遵循结构化工作流程的问题，显著提高了准确性和效率。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在医疗分诊等高风险领域难以严格遵循结构化工作流程。单一提示方法随着提示长度增加会出现指令遵循退化、中间信息丢失和上下文窗口溢出等问题。

Method: Arbor框架将决策树导航分解为专门的节点级任务：将决策树标准化为边列表表示并存储；运行时通过有向无环图编排机制迭代检索当前节点的出边；通过专门的LLM调用评估有效转换；将响应生成委托给单独的推理步骤。

Result: 在10个基础模型上使用真实临床分诊对话的标注轮次进行评估：平均轮次准确率提高29.4个百分点；每轮延迟降低57.1%；每轮成本平均降低14.4倍。

Conclusion: 架构分解减少了对内在模型能力的依赖，使较小模型能够匹配或超越在单一提示基准下运行的较大模型，为高风险领域的结构化决策提供了可扩展解决方案。
...
TL;DR: 提出基分提取函数，将用户对论证的偏好映射到基分，从而简化论证框架中基分设置过程，支持透明可争议的AI系统。


<details>
  <summary>Details</summary>
Motivation: 在渐进论证中，论证的基分设置通常需要专业知识且不直观，而通过用户对论证的偏好来推导基分可以简化这一过程。

Method: 引入基分提取函数，将带偏好的双极论证框架映射到定量双极论证框架，提出算法并考虑人类偏好的非线性特征进行近似。

Result: 在机器人场景中进行理论和实验评估，为实践中选择适当的渐进语义提供建议。

Conclusion: 基分提取函数能够有效简化论证基分设置过程，支持透明可争议的AI系统开发，为实际应用提供了实用工具。
...
TL;DR: 提出基于深度强化学习的公交车疏散路径规划方法，解决城市紧急疏散中的公交车疏散定向问题，在真实道路网络上实现快速近优解。


<details>
  <summary>Details</summary>
Motivation: 气候变化导致自然灾害频发，城市紧急疏散需求增加。传统汽车疏散易造成拥堵混乱，需要有效的公交车疏散方法来快速制定疏散计划。

Method: 提出公交车疏散定向问题（BEOP），采用基于图学习的深度强化学习方法，训练后能快速生成疏散路线，并用MILP公式界定解的质量差距。

Result: 在旧金山真实道路网络上验证，方法能达到近优解质量，推理速度快（秒级），并能分析不同疏散车辆数量对疏散配额的影响。

Conclusion: 深度强化学习方法能有效解决公交车疏散路径规划问题，提供快速、高质量的疏散方案，有助于制定城市应急疏散策略。
...
TL;DR: 提出新方法使用top-k规划生成同一目标的多种不同计划，创建无偏数据集，并引入版本覆盖分数(VCS)衡量目标识别器在不同计划集下的鲁棒性


<details>
  <summary>Details</summary>
Motivation: 现有目标识别数据集存在系统性偏差，因为它们都由启发式前向搜索规划系统生成，缺乏对更现实场景（如不同规划器）的挑战性，影响目标识别器在不同规划器下的评估

Method: 使用top-k规划方法为同一目标假设生成多个不同的计划，创建无偏基准数据集，并引入版本覆盖分数(VCS)来衡量目标识别器基于不同计划集推断目标时的鲁棒性

Result: 当前最先进的目标识别器在低可观测性设置下的鲁棒性显著下降

Conclusion: 提出的新方法能够创建更真实、无偏的目标识别基准数据集，VCS指标能有效评估目标识别器在不同规划器下的鲁棒性，揭示了现有方法在低可观测性下的局限性
...
TL;DR: 提出E-SPL方法，结合强化学习和系统提示进化，通过并行多提示rollout、权重更新和进化操作，实现模型上下文和权重的联合优化。


<details>
  <summary>Details</summary>
Motivation: 当前LLM主要通过自我反思更新上下文和强化学习更新权重两种机制进行自我改进，但缺乏将两者联合优化的方法。需要一种能够同时改进模型上下文和模型权重的系统。

Method: E-SPL方法：1) 每轮RL迭代选择多个系统提示并行运行rollout；2) 对每个系统提示条件下的模型权重进行RL更新；3) 通过LLM驱动的突变和交叉对系统提示种群进行进化更新；4) 使用TrueSkill评分系统进行进化选择。

Result: 在AIME→BeyondAIME泛化设置中，E-SPL将RL成功率从38.8%提升至45.1%，优于反射提示进化的40.0%。结果表明结合强化学习和系统提示进化能带来样本效率和泛化能力的持续提升。

Conclusion: E-SPL通过联合优化模型上下文和权重，实现了声明性知识（提示）和程序性知识（权重）的自然分离，在推理和智能体任务中表现出改进的性能，为自主自我改进系统提供了有效方法。
...
TL;DR: WebWorld是一个大规模开放网络模拟器，通过百万级网络交互训练，支持推理、多格式数据和30+步长时程模拟，在WebArena上提升Qwen3-14B性能9.2%，并展示跨领域泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有网络代理需要大量轨迹进行泛化，但真实世界训练受网络延迟、速率限制和安全风险约束。现有模拟器局限于封闭环境且轨迹数量有限，无法满足大规模开放网络训练需求。

Method: 构建可扩展数据管道，在100万+开放网络交互上进行训练，支持推理、多格式数据和长时程模拟（30+步）。引入WebWorld-Bench进行内在评估，包含九个维度的双重指标。

Result: WebWorld模拟性能与Gemini-3-Pro相当；使用WebWorld合成轨迹训练的Qwen3-14B在WebArena上提升9.2%，达到GPT-4o水平；作为世界模型在推理时搜索中优于GPT-5；展示跨代码、GUI和游戏环境的泛化能力。

Conclusion: WebWorld是首个大规模开放网络模拟器，通过可扩展训练方法实现了高性能网络代理训练，为世界模型构建提供了可复现方案，并展示了跨领域泛化潜力。
...
TL;DR: 大型语言模型在核危机模拟中表现出复杂的战略行为，包括欺骗、心智理论和元认知能力，其行为既验证又挑战了传统战略理论的核心理念。


<details>
  <summary>Details</summary>
Motivation: 研究前沿大语言模型在战略竞争环境中的行为模式，特别是它们在核危机模拟中的决策逻辑，这对国家安全专业人员和理解AI在不确定性下的推理能力具有重要意义。

Method: 设计核危机模拟场景，让三个前沿大语言模型（GPT-5.2、Claude Sonnet 4、Gemini 3 Flash）扮演对立领导人角色，观察它们在战略竞争中的行为模式和决策过程。

Result: 模型表现出欺骗、心智理论和元认知能力；既支持了Schelling、Kahn、Jervis等战略理论，又挑战了传统观念：核禁忌无法阻止核升级、战略核攻击确实发生、威胁更多引发对抗而非服从、高可信度加速而非遏制冲突、模型从不选择妥协或撤退。

Conclusion: AI模拟是强大的战略分析工具，但必须针对人类推理模式进行适当校准。理解前沿模型如何模仿或不模仿人类战略逻辑，对于AI日益影响战略结果的世界至关重要。
...
TL;DR: 提出首个包含本体模式与事实的数据集提取工作流及数据集套件，支持机器学习与推理服务


<details>
  <summary>Details</summary>
Motivation: 现有知识图谱精化算法的评估数据集通常只包含事实数据，缺乏丰富的本体模式信息，这限制了依赖本体约束、推理或神经符号技术的评估，无法反映真实大规模知识图谱的性能

Method: 开发了一个工作流，从源知识图谱中提取包含模式与事实的数据集，处理两者间的不一致性，利用推理推导隐含知识，并将数据集序列化为OWL格式供推理服务使用，同时提供加载到张量表示的实用工具

Result: 创建了首个包含模式与事实的数据集资源套件，包括从具有丰富模式的知识图谱中提取的新数据集，以及对现有数据集的模式信息增强，所有数据集都以OWL格式序列化

Conclusion: 该资源填补了知识图谱精化评估中模式信息缺失的空白，为依赖本体约束和推理的方法提供了合适的评估基准，促进了神经符号技术在大规模真实知识图谱中的应用评估
...
Method: 提出StarWM世界模型，使用结构化文本表示将观测分解为五个语义模块；构建SC2-Dynamics-50k指令调优数据集；开发多维离线评估框架；提出StarWM-Agent决策系统，采用生成-模拟-优化决策循环进行前瞻驱动的策略优化。
...
Conclusion: StarWM是首个用于星际争霸II的世界模型，通过结构化表示和指令调优有效预测部分可观测环境下的未来状态。将世界模型整合到决策循环中能够显著提升智能体性能，证明了前瞻驱动策略优化的有效性。
...
TL;DR: EmbeWebAgent是一个将智能体直接嵌入现有UI的企业级Web代理框架，通过前端轻量级钩子和可复用后端工作流实现，支持混合粒度操作和跨技术栈兼容。


<details>
  <summary>Details</summary>
Motivation: 传统Web代理在人类界面级别操作（截图或DOM树），缺乏应用级访问，导致鲁棒性和动作表达能力受限。在企业环境中，前后端都有明确控制权，这为更强大的代理框架提供了机会。

Method: 使用轻量级前端钩子（精选的ARIA和URL观察、通过WebSocket暴露的每页函数注册表）将代理直接嵌入现有UI，配合可复用的后端工作流进行推理和执行动作。框架与技术栈无关（如React或Angular），支持从GUI原语到高级复合操作的混合粒度动作，并通过MCP工具协调导航、操作和领域特定分析。

Result: 演示显示只需最小的改造工作量，就能在实时UI环境中实现鲁棒的多步骤行为。框架展示了在现有企业应用中的实际部署能力。

Conclusion: EmbeWebAgent通过将代理直接嵌入UI，克服了传统Web代理的限制，为企业环境提供了更强大、更鲁棒的自动化解决方案，同时保持对现有技术栈的兼容性和最小改造需求。
...
TL;DR: 提出Concept Influence方法，通过语义方向而非单个测试样本来归因模型行为，比传统影响函数更高效且可扩展


<details>
  <summary>Details</summary>
Motivation: 现有训练数据归因方法（如影响函数）计算成本高且基于单个测试样本，容易偏向句法相似性而非语义相似性，需要更可扩展且能归因抽象行为的方法

Method: 引入Concept Influence方法，将模型行为归因于语义方向（如线性探针或稀疏自编码器特征），而非单个测试样本；提出基于探针的归因方法作为Concept Influence的一阶近似

Result: Concept Influence及其近似方法在涌现错位基准和实际后训练数据集上验证有效，性能与传统影响函数相当但计算效率提高一个数量级以上
...
<details>
  <summary>Details</summary>
Motivation: 解决一阶关系领域中归纳学习和演绎推理之间的长期矛盾，传统方法需要完整模型且学习困难

Method: 将不完整一阶公理与部分观测样本合并到SOS层次的有界度片段中，同时执行基础提升和世界提升

Result: 实现了在多项式时间内隐式学习一阶概率逻辑并执行提升推理，无需显式构建模型

Conclusion: 通过隐式学习推理框架成功调和了归纳学习和演绎推理的张力，为一阶关系概率推理提供了新方法
...
TL;DR: 本文通过引入"潜力"概念分析思维链推理，发现推理过程存在非单调性、洞察性跳跃和幸运猜测等现象，并证明思维链具有可迁移性，仅需20%的强模型推理就能解锁弱模型性能。


<details>
  <summary>Details</summary>
Motivation: 尽管思维链提示已成为激发大语言模型推理能力的标准技术，但其成功背后的驱动机制仍不清楚。本文旨在深入分析思维链推理过程，理解哪些部分真正贡献于最终答案。

Method: 引入"潜力"概念量化思维链各部分对正确答案可能性的提升程度，通过竞争级数学问题分析推理轨迹，并研究思维链的可迁移性，测量弱模型在强模型部分思维链下的表现。

Result: 发现思维链潜力呈现非单调性（推理偏离）、尖锐但难解释的峰值（推理洞察和跳跃）以及幸运猜测等现象。可迁移性实验表明，仅需20%的强模型思维链就能解锁弱模型在原本无法解决问题上的性能。

Conclusion: 思维链推理机制在很大程度上是可迁移的，但部分行为难以从人类角度解释。研究揭示了思维链如何提升模型性能，为理解大语言模型推理过程提供了新视角。
...
TL;DR: 该论文提出，强大的推理能力源于语言自我反思，而非单纯扩大模型规模。作者主张通过高质量社交互动中的对话式脚手架来培养AI的内省能力，认为对话质量是新的数据质量。


<details>
  <summary>Details</summary>
Motivation: 当前AI训练方法将推理视为规模扩展的涌现属性，但作者认为这种方法存在局限。他们从维果茨基发展心理学出发，主张通过社会互动和内省来培养更强大的推理能力。

Method: 基于维果茨基发展心理学理论，提出三个核心观点：1）私人思维的社会起源，强调从对话环境中学习；2）对话式脚手架支持的内省体验，将原始数据转化为可学习的叙事；3）对话质量即数据质量，通过多样化和严谨的对话来提升推理能力。

Result: 论文提出了一个理论框架，强调对话质量对AI推理能力的关键作用。虽然没有提供具体实验数据，但为下一代通用智能的发展指明了方向。

Conclusion: 优化对话式脚手架是发展下一代通用智能的主要杠杆。AI的推理深度和测试时计算效率取决于其掌握的对话的多样性和严谨性，对话质量成为新的数据质量标准。
...
TL;DR: 提出ReusStdFlow框架解决企业Agentic AI中的"可重用性困境"和结构幻觉问题，通过"提取-存储-构建"范式将异构DSL标准化为模块化工作流片段，实现90%以上的准确率。


<details>
  <summary>Details</summary>
Motivation: 解决企业Agentic AI中的"可重用性困境"和结构幻觉问题，这些问题是企业数字资产自动化重组和高效重用的主要障碍。

Method: 采用"提取-存储-构建"范式：1) 将异构、平台特定的领域特定语言(DSL)解构为标准化的模块化工作流片段；2) 使用图数据库和向量数据库的双重知识架构，协同检索拓扑结构和功能语义；3) 基于检索增强生成(RAG)策略智能组装工作流。

Result: 在200个真实世界的n8n工作流上进行测试，系统在提取和构建方面均达到90%以上的准确率。
...
TL;DR: MAC-AMP是一个基于多智能体大语言模型的闭环协作系统，用于多目标抗菌肽设计，通过模拟同行评审-自适应强化学习框架，在活性、毒性、新颖性等多个关键分子特性上实现优化。


<details>
  <summary>Details</summary>
Motivation: 当前抗菌肽设计模型难以平衡活性、毒性和新颖性等关键目标，使用僵化或不明确的评分方法导致结果难以解释和优化。随着大语言模型能力的快速发展，多智能体协作在复杂科学设计场景中展现出巨大潜力。

Method: 提出MAC-AMP系统，采用闭环多智能体协作框架，实现完全自主的模拟同行评审-自适应强化学习。系统仅需任务描述和示例数据集即可设计新型抗菌肽，具有跨领域可迁移性，支持多目标优化且保持可解释性。

Result: MAC-AMP在抗菌活性、抗菌肽相似性、毒性合规性和结构可靠性方面表现出色，优于其他抗菌肽生成模型，能够有效优化多个关键分子特性。

Conclusion: 基于多智能体大语言模型的闭环协作系统为抗菌肽设计提供了创新解决方案，实现了多目标优化、可解释性和跨领域可迁移性，在应对抗生素耐药性全球健康威胁方面具有重要应用价值。
...
TL;DR: 本文提出了混合行动理论框架下的两种主要原因定义，并证明它们等价，同时展示了这些定义具有直观合理的性质。


<details>
  <summary>Details</summary>
Motivation: 实际因果关系推理是理性研究的基础，但现有研究主要关注离散变化，而现实世界中的变化既有离散也有连续（即混合变化）。尽管已有广泛的实际因果关系研究，但只有少数近期研究探讨了连续变化下的因果关系。

Method: 在混合时间情境演算的混合行动理论框架中提出了两种主要原因定义：一种是基础性的，另一种通过贡献形式化因果关系，然后使用修改后的"but-for"测试从反事实角度验证。证明这两种定义是等价的。

Result: 证明了提出的两种主要原因定义在混合行动理论框架中是等价的，并展示了这些定义具有直观合理的性质。

Conclusion: 该研究扩展了实际因果关系理论到混合变化领域，为处理离散和连续变化的因果关系提供了形式化框架，并建立了不同定义之间的等价性。
...
TL;DR: 提出用于药物资产侦察的基准测试方法和自学习Bioptic Agent，在跨语言资产发现上显著优于现有AI系统


<details>
  <summary>Details</summary>
Motivation: 生物制药创新格局已变：大量新药资产源自美国以外，主要通过区域非英语渠道披露。超过85%的专利申请来自美国以外，中国占全球近一半；中国占全球药物开发约30%。在这种高风险环境下，未能发现"隐形"资产会给投资者和业务开发团队带来数十亿美元风险，而当前的深度研究AI代理在跨异质多语言源的高召回发现方面仍落后于人类专家。

Method: 提出药物资产侦察的基准测试方法，构建基于树结构的自学习Bioptic Agent。使用多语言多代理管道构建具有挑战性的完整性基准：复杂用户查询配以主要在美国中心雷达之外的ground-truth资产。收集专家投资者、BD和VC专业人员的筛选查询作为先验条件生成基准查询。使用LLM-as-judge评估方法，校准专家意见。与Claude Opus 4.6、OpenAI GPT-5.2 Pro、Perplexity Deep Research、Gemini 3 Pro + Deep Research和Exa Websets进行比较。
...
Conclusion: 提出的Bioptic Agent在药物资产侦察任务上显著优于现有AI系统，特别是在发现美国中心雷达之外的跨语言资产方面。该方法为解决生物制药领域日益增长的多语言信息挑战提供了有效解决方案，证明了计算资源投入对性能提升的重要性。
...
### [359] [LoPace: A Lossless Optimized Prompt Accurate Compression Engine for Large Language Model Applications](https://arxiv.org/abs/2602.13266)
*Aman Ulla*

Main category: cs.DB

TL;DR: LoPace是一个专门为LLM应用设计的无损提示压缩框架，通过Zstandard压缩、BPE分词与二进制打包以及混合方法，平均节省72.2%存储空间，保持100%无损重建。
...
Method: 提出LoPace压缩框架，采用三种压缩方法：1) Zstandard压缩；2) BPE分词与二进制打包；3) 结合前两者的混合方法。

Result: 在386个不同类型提示上测试，平均节省72.2%空间，实现100%无损重建。混合方法表现最佳，平均压缩比4.89倍，速度3.3-10.7 MB/s，内存占用仅0.35 MB。

Conclusion: LoPace已具备生产就绪性，具有小内存占用和良好可扩展性，适合大型数据库和实时LLM应用。
...
### [360] [MergePipe: A Budget-Aware Parameter Management System for Scalable LLM Merging](https://arxiv.org/abs/2602.13273)
*Yuanyi Wang,Yanggan Gu,Zihao Wang,Kunxi Li,Yifan Yang,Zhaoyi Yan,Congkai Xie,Jianmin Wu,Hongxia Yang*

Main category: cs.DB

TL;DR: MergePipe：首个将LLM合并视为数据管理与执行问题的参数管理系统，通过目录驱动抽象、成本感知规划和流式执行引擎，显著降低I/O开销并提升合并效率。


<details>
  <summary>Details</summary>
Motivation: 随着专家模型数量增加，现有LLM合并方法将参数视为非结构化文件进行无状态一次性合并，导致磁盘I/O过高、参数扫描冗余和可扩展性差的问题。

Method: 1) 引入目录驱动抽象，管理模型参数、合并计划和执行谱系；2) 成本感知规划器显式建模专家参数I/O并强制执行用户指定的I/O预算；3) 流式执行引擎在事务保证下物化合并模型。

Result: 实验显示MergePipe将总I/O降低达一个数量级，端到端加速达11倍（墙钟时间减少达90%），相比现有最先进的LLM合并流水线有显著提升。

Conclusion: 通过将LLM合并视为数据管理问题，并专注于优化专家参数访问这一主要成本来源，MergePipe能够有效缓解朴素流水线的O(K) I/O增长，实现可预测的扩展行为。
...
### [361] [Arming Data Agents with Tribal Knowledge](https://arxiv.org/abs/2602.13521)
*Shubham Agarwal,Asim Biswal,Sepanta Zeighami,Alvin Cheung,Joseph Gonzalez,Aditya G. Parameswaran*

Main category: cs.DB

TL;DR: Tk-Boost是一个为NL2SQL代理提供部落知识的框架，通过分析代理在数据库查询中的错误来识别其误解，生成针对性知识并建立检索机制，从而提升SQL生成准确性。


<details>
  <summary>Details</summary>
Motivation: 当前基于LLM的NL2SQL代理在面对大规模真实数据库时仍会出错，因为它们缺乏对底层数据的正确理解，并形成对数据的误解。现有方法只是简单重述数据库内容，未能解决代理的误解问题。

Method: Tk-Boost首先让NL2SQL代理在数据库上回答一些查询，通过分析其错误来识别误解，然后生成针对性的部落知识。为支持准确检索，框架为这些知识建立适用性条件索引。在回答新查询时，使用这些知识为代理提供反馈，在SQL生成过程中解决误解。

Result: 在BIRD和Spider 2.0基准测试中，Tk-Boost将各种NL2SQL代理的准确性提升了最高16.9%（Spider 2.0）和13.7%（BIRD）。

Conclusion: 通过积累和利用部落知识来纠正NL2SQL代理的误解，Tk-Boost能够显著提升代理在真实数据库查询中的准确性，且该框架可作为任何NL2SQL代理的附加组件使用。
...
### [362] [DTBench: A Synthetic Benchmark for Document-to-Table Extraction](https://arxiv.org/abs/2602.13812)
*Yuxiang Guo,Zhuoran Du,Nan Tang,Kezheng Tang,Congcong Ge,Yunjun Gao*

Main category: cs.DB

TL;DR: DTBench是一个用于文档到表格提取的合成基准测试，通过多智能体合成工作流从真实表格生成文档，评估LLM在复杂推理和冲突解决等能力上的表现。


<details>
  <summary>Details</summary>
Motivation: 现有基准测试未能明确区分和全面覆盖文档到表格提取所需的各种能力，而人工标注成本高、难以扩展且能力覆盖有限，因此需要构建一个能力感知的基准测试。

Method: 采用反向Table2Doc范式，设计多智能体合成工作流从真实表格生成文档，构建DTBench基准测试，包含5个主要类别和13个子类别的两层能力分类体系。

Result: 评估多个主流LLM在DTBench上的表现，发现模型之间存在显著性能差距，以及在推理、忠实性和冲突解决方面存在持续挑战。

Conclusion: DTBench为数据生成和评估提供了全面的测试平台，促进了文档到表格提取的未来研究，该基准测试已公开可用。
...
### [363] [TabTracer: Monte Carlo Tree Search for Complex Table Reasoning with Large Language Models](https://arxiv.org/abs/2602.14089)
...
<details>
  <summary>Details</summary>
Motivation: 现有表格推理方法存在局限性：基于提示的方法缺乏步骤级验证；基于智能体的方法验证范围有限、回滚能力不足、轨迹冗余导致token成本高。需要一种能够协调多步骤工具调用、支持验证和回滚、同时降低冗余的框架。

Method: 1. 步骤级验证：使用类型化操作和轻量级数值/格式检查来提供可靠奖励并抑制幻觉
2. 执行反馈蒙特卡洛树搜索：维护候选表格状态的搜索树，使用反向传播的反思分数指导UCB1选择和版本化快照回滚
3. 预算感知剪枝：通过去重、状态哈希和单调性门控减少冗余，降低token成本

Result: 在TabFact、WikiTQ和CRT数据集上的综合评估显示，TabTracer比最先进的基线方法准确率提升高达6.7%，同时token消耗减少59-84%。

Conclusion: TabTracer通过协调多步骤工具调用、实施步骤级验证、使用蒙特卡洛树搜索进行状态跟踪和回滚，以及减少冗余，在表格推理任务中实现了更高的准确性和更低的计算成本。
...
### [364] [Towards a Hybrid Quantum-Classical Computing Framework for Database Optimization Problems in Real Time Setup](https://arxiv.org/abs/2602.14263)
*Hanwen Liu,Ibrahim Sabek*

Main category: cs.DB

TL;DR: 提出首个实时量子增强数据库系统愿景，通过透明化量子求解过程解决数据库优化问题，相比经典查询优化器提升14倍性能


<details>
  <summary>Details</summary>
Motivation: 现有量子计算方法将数据库优化问题直接提交给黑盒量子求解器，无法进行细粒度控制以平衡精度和效率，限制了在实时数据库场景中的灵活性和处理大规模问题的能力

Method: 提出实时量子增强数据库系统愿景，开发两种互补的可扩展性策略应对大规模挑战、过度复杂性和超出硬件限制的问题，并与数据库查询优化器集成作为初步原型

Result: 在真实工作负载评估中，相比经典查询优化器实现最高14倍性能提升，同时比黑盒量子求解器获得更好的效率和解决方案质量

Conclusion: 提出的量子增强数据库系统能够有效解决现有黑盒量子求解器的局限性，为数据库优化问题提供透明、可扩展的解决方案，在效率和解决方案质量方面均优于现有方法
...
### [365] [Qute: Towards Quantum-Native Database](https://arxiv.org/abs/2602.14699)
*Muzhi Chen,Xuanhe Zhou,Wei Zhou,Bangrui Xu,Surui Tang,Guoliang Li,Bingsheng He,Yeye He,Yitong Song,Fan Wu*

Main category: cs.DB

TL;DR: Qute是一个将量子计算作为一等执行选项的量子数据库系统，通过编译扩展SQL为量子电路、混合优化器、选择性量子索引和保真度存储来克服当前量子比特限制。


<details>
  <summary>Details</summary>
Motivation: 现有方法要么在经典机器上运行量子算法，要么将现有数据库适配用于量子模拟，缺乏真正将量子计算作为数据库执行选项的系统。需要克服当前量子比特限制，实现量子原生数据库。

Method: 1) 将扩展SQL编译为门高效的量子电路；2) 使用混合优化器动态选择量子或经典执行计划；3) 引入选择性量子索引；4) 设计保真度存储以缓解量子比特限制。

Result: 在真实量子处理器(origin_wukong)上部署Qute，在规模上优于经典基线。发布了开源原型：https://github.com/weAIDB/Qute。

Conclusion: 提出了量子数据库Qute，将量子计算作为一等执行选项，并制定了三阶段演进路线图，为实现量子原生数据库奠定了基础。
...
### [366] [A Unified Mathematical Framework for Distributed Data Fabrics: Categorical Hypergraph Models](https://arxiv.org/abs/2602.14708)
*T. Shaska,I. Kotsireas*

Main category: cs.DB

TL;DR: 提出一个基于超图的数学框架，用于统一分布式系统中的异构数据管理，通过范畴论和模张量范畴建模数据、元数据、转换等组件，并证明关键任务的NP难度，提供可扩展的解决方案。


<details>
  <summary>Details</summary>
Motivation: 当前分布式数据架构缺乏严格的数学基础，通常依赖临时架构，在一致性、数据沿袭和可扩展性方面存在困难。需要统一的数学框架来管理异构数据。

Method: 提出超图结构 \( \mathcal{F} = (D, M, G, T, P, A) \) 来建模数据集、元数据、图、转换、策略和分析。采用范畴论方法，将数据集作为对象，转换作为态射。将超图嵌入模张量范畴，通过编织幺半结构捕捉关系对称性。使用谱方法和基于对称性的对齐进行可扩展解决方案。

Result: 证明了关键任务（如模式匹配和动态分区）的NP难度。框架确保在CAP和CAL定理下的一致性、完整性和因果性。利用稀疏关联矩阵和编织操作实现容错操作。

Conclusion: 该数学框架为分布式数据架构提供了理论基础，通过超图、范畴论和几何类比统一了异构数据管理，解决了现有架构在一致性、沿袭和可扩展性方面的挑战。
