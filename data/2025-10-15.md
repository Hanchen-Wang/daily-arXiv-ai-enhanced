...
<details>
  <summary>Details</summary>
Motivation: 探究CLIP作为视觉编码器优于自监督模型DINO的原因，是源于语言监督还是更大的训练数据。
...
<details>
  <summary>Details</summary>
Motivation: 自监督学习在通用视觉领域已成功应用，但在医学影像中由于数据有限和领域特定偏差而未被充分利用。本文旨在开发专门针对乳腺X线摄影的SSL方法。

Method: 提出乳腺组织感知数据增强采样器（图像级和补丁级监督），以及利用3D DBT结构的跨切片对比学习目标，将3D结构信息融入2D预训练。

Result: 在多个乳腺癌筛查任务中达到最先进性能，在五个基准数据集上表现出良好的泛化能力。

Conclusion: MammoDINO为乳腺X线摄影提供了可扩展、无需标注的多用途CAD工具基础，有助于减轻放射科医生工作负担并提高乳腺癌筛查诊断效率。
...
Method: 采用双模型框架，VideoLLaMA专门负责captioning任务，Qwen2.5-VL专门负责视觉问答任务，通过任务特定的分离训练策略。
...
<details>
  <summary>Details</summary>
Motivation: 准确估计房间3D布局在机器人、增强现实和室内设计中具有重要应用价值，需要能够处理各种复杂布局的鲁棒方法。

Method: 采用两阶段架构：CNN提取高级特征并学习TPS变换参数，然后TPS变换层将参考布局变形为目标布局。

Result: 在多个公开数据集上表现出色，3DIoU值分别达到85.49、86.16、81.76和91.98，优于现有方法。

Conclusion: 该方法在房间布局估计中具有高精度，TPS变换与全景图像兼容性好，能鲁棒处理各种房间布局类型。
...
Method: 将掩码维度（边界框坐标）直接嵌入输入提示，使模型能够推理对象几何和布局；针对四个问题类别进行任务特定监督的微调；在训练集中向GPT响应添加标准化答案以改进一致性。

Result: 在公共排行榜上获得73.0606的最终得分，总体排名第四。
...
<details>
  <summary>Details</summary>
Motivation: 医学影像中模型决策的可解释性直接影响临床信任和采用，Vision Transformers的复杂注意力机制对可解释性构成挑战。

Method: 使用Gradient Attention Rollout和Grad-CAM评估ViT、DeiT、DINO和Swin Transformer在血液细胞分类和乳腺超声图像分类任务中的可解释性。

Result: DINO结合Grad-CAM在所有数据集上提供最忠实和定位准确的解释，Grad-CAM产生类别区分性和空间精确的热图，而Gradient Attention Rollout产生更分散的激活。
...
<details>
  <summary>Details</summary>
Motivation: 水下环境中存在图像退化（低对比度、颜色失真）和海洋生物自然伪装两大挑战，传统图像增强方法难以恢复关键特征，陆地场景的伪装目标检测方法无法适应水下光学特性。

Method: 使用MSRCR算法进行数据增强生成光照不变图像；设计扩展感受野模块结合多尺度渐进解码器捕获多尺度上下文信息；提出自适应先验引导机制，在高层特征嵌入空间注意力进行粗定位，在低层特征使用可变形卷积细化轮廓。

Result: 在两个公开MAS数据集上的实验表明，APGNet在广泛使用的评估指标下优于15种最先进方法。

Conclusion: APGNet通过自适应先验引导机制有效解决了水下伪装目标检测问题，在图像退化和自然伪装双重挑战下表现出优越性能。
...
Conclusion: VidMP3解决了当前视频编辑方法在保持运动一致性和结构灵活性方面的挑战，代码将开源。
...
<details>
  <summary>Details</summary>
Motivation: 计算机视觉领域面临高质量标记数据稀缺的挑战，获取标记数据成本高昂且有时不可行，需要寻找可靠的方法来克服数据不足的问题。

Method: 探讨领域自适应及其各种实现方法，核心思想是利用在特定数据集上训练的模型来预测来自不同领域但相同类型的数据。

Result: 论文主要提供了领域自适应的理论框架和方法论讨论，而非具体的实验结果。

Conclusion: 领域自适应是解决标记数据稀缺问题的有效途径，能够使模型在不同领域间进行知识迁移和应用。
...
<details>
  <summary>Details</summary>
Motivation: 现有音频驱动视频生成方法在唇音同步准确性、长视频时间连贯性和多角色动画方面仍面临挑战，需要更高效、可控的解决方案。

Method: 采用扩散变换器框架，结合LoRA训练策略和位置偏移推理实现长视频生成；使用部分参数更新和奖励反馈增强唇音同步；提出无需训练的Mask-CFG方法支持多角色动画。

Result: 实验结果表明该方法在质量、时间连贯性和多角色动画方面优于现有最先进方法，实现了高质量、高效且成本效益高的音频驱动视频生成。

Conclusion: 该方法通过简单高效的方式解决了音频驱动视频生成中的关键挑战，在唇音同步、长视频连贯性和多角色动画方面取得了显著改进。
...
<details>
  <summary>Details</summary>
Motivation: 解决室内布局设计中多样化和高质量训练数据的迫切需求。
...
Method: 提出边缘引导注意力块(EGAB)将显式边缘先验嵌入Transformer架构，基于此开发EG-Restormer处理严重模糊；设计轻量高效的LENet处理轻度模糊；集成两者形成自适应双网络(ADNet)根据模糊程度动态选择网络。

Result: EG-Restormer和ADNet在严重模糊QR码的解码率上取得最先进性能，同时保持有竞争力的速度。

Conclusion: 所提出的方法通过显式利用QR码结构先验和自适应网络选择，在保持效率的同时显著提升了QR码去模糊的解码成功率。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于预训练扩散模型的3D场景重建方法存在两个关键局限：缺乏可靠的几何监督导致重建质量不佳，以及缺乏有效机制来缓解生成图像中的多视角不一致性。

Method: 利用平面结构生成精确度量尺度深度图，在生成流程中融入几何指导来改进可见性掩码估计、引导新视角选择，并使用视频扩散模型增强多视角一致性修复。

Result: 在Replica、ScanNet++和DeepBlending数据集上的实验表明，该方法在几何和外观重建方面均优于现有基线，特别是在未观察区域。支持单视图输入和未标定视频，在室内外场景中具有强泛化能力。

Conclusion: 精确几何是有效利用生成模型增强3D场景重建的基本前提，该方法通过几何指导显著提升了重建质量和一致性，具有实际应用价值。
...
<details>
  <summary>Details</summary>
Motivation: 尽管预训练模型在非排练类增量学习中取得了显著进展，但仍面临三个核心挑战：模型复杂度不断增加、增量学习过程中表示空间的不平稳变化，以及阶段优化与全局推理之间的不一致性。

Method: 构建了基于预训练模型的增量并行适配器（IPA）网络，通过并行连接学习轻量级适配器，使用转移门继承和传播表示能力；设计了分离锚监督（DAS），通过分别将正负样本与虚拟锚点进行比较来解耦约束。

Result: 在六个基准测试上的广泛实验表明，DRL在整个CIL过程中始终优于其他最先进方法，同时在训练和推理阶段都保持高效率。
...
<details>
  <summary>Details</summary>
Motivation: 现有预训练扩散引导方法依赖显式退化先验或全局统计指导，难以处理局部伪影和面部颜色问题，需要更精细的区域控制方法。
...
Conclusion: SSDiff通过自监督选择性引导机制有效解决了老照片人脸修复中的复合退化问题，实现了更好的区域控制和修复质量。
...
<details>
  <summary>Details</summary>
Motivation: 随着检索增强图像生成系统的广泛应用，保护私有图像数据集免遭未授权使用成为重要挑战，传统数字水印方法在此类系统中效果有限。

Method: 利用视觉语言模型合成与原始数据集视觉一致的哨兵图像，通过随机生成的字符序列作为检索密钥进行保护验证。

Result: 实验结果表明ImageSentinel能有效检测未授权的数据集使用，同时为授权应用保持生成质量。

Conclusion: ImageSentinel为视觉数据集在RAIG系统中的保护提供了有效解决方案，解决了传统水印方法在复杂特征提取和重组过程中的局限性。
...
TL;DR: 提出了一种约束优化方法来设计实用的压缩单光子3D成像编码函数，通过梯度下降联合优化照明和编码矩阵，在硬件约束下显著优于传统编码设计。


<details>
  <summary>Details</summary>
Motivation: 单光子相机在飞行时间3D成像中面临硬件限制（如系统带宽、最大激光功率、传感器数据速率等），现有压缩直方图方法在真实世界照明硬件约束下表现不佳。

Method: 使用梯度下降联合优化照明和编码矩阵，确保编码函数满足硬件约束条件，特别是带宽和峰值功率限制。

Result: 通过广泛仿真验证，该方法在带宽和峰值功率约束下始终优于传统编码设计，在峰值功率受限系统中优势尤为明显，并能适应任意参数化脉冲响应。

Conclusion: 该方法为压缩单光子3D成像提供了实用的编码函数设计解决方案，能够有效应对真实世界的硬件约束挑战。
...
Method: 提出CapFlow多智能体协作工作流，利用开源模型进行大规模高质量视觉描述合成，然后通过微调得到MetaCaptioner通用视觉描述器。

Result: CapFlow在多个视觉领域实现与GPT-4.1相当的描述质量，成本降低89.5%。MetaCaptioner在开源社区达到顶级多模态性能，与商业模型描述能力相当。
...
<details>
  <summary>Details</summary>
Motivation: 远程生理测量需要收集用户隐私信息，现有方法依赖带标签的客户端数据，难以在缺乏标签的真实部署场景中更新模型。

Method: FedHUG框架包含两个核心模块：最小偏差聚合模块动态调整权重处理异构非IID特征；全局分布感知学习控制器参数化标签分布并动态调整客户端训练策略。

Result: 在RGB视频和毫米波雷达生理测量中，该方法在无监督域泛化任务上表现出优于现有技术的性能。

Conclusion: FedHUG框架有效解决了联邦学习中无监督域泛化的挑战，为隐私保护的远程生理测量提供了可行方案。
...
Method: 提出KFF方法，包含域知识分裂模块(KFI)从配对类感知域提示池中分离新域知识，以及域知识融合模块(KFU)将分裂的新知识以最小成本合并到现有知识池中。

Result: 在ImageNet-C数据集上的广泛实验验证了所提方法相对于其他方法的有效性。

Conclusion: KFF方法能够自适应地扩展和合并新旧域的类感知域知识，动态积累判别性历史知识，有效解决CTTA中的知识管理问题。
...
<details>
  <summary>Details</summary>
Motivation: 传统原型方法基于确定性特征平均，无法捕捉类内多样性，在医学图像中由于标注数据有限和解剖变异性大，导致泛化能力不足。

Method: 提出扩散原型学习框架，包含扩散原型增强模块、空间感知条件机制和保守融合策略，通过前向-反向扩散过程生成多样化原型变体。

Result: 在腹部MRI和CT数据集上的实验表明，该方法显著提升了一样本医学图像分割性能，达到了新的最先进水平。

Conclusion: DPL通过将原型建模为概率分布并利用扩散过程生成多样化变体，有效解决了医学图像分割中的原型表示挑战，实现了更好的泛化性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有3D重建方法在多模态数据融合和几何一致性方面存在局限，需要统一框架同时处理RGB、深度、法线和语义信息，并确保几何精度。

Method: 使用CUDA加速的光栅化流水线，通过可微分射线-椭球体相交渲染深度而非高斯中心，推导表面法线渲染的解析梯度，引入可学习属性进行高斯剪枝。

Result: 定量和定性实验表明，在所有模态上都达到了最先进的重建精度，验证了几何感知范式的有效性。

Conclusion: UniGS框架成功实现了高保真多模态3D重建，在几何一致性和计算效率方面表现出色，为多模态3D场景重建提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 完全监督的3D实例分割需要密集的点级标注，成本高昂。边界框标注虽然更易获取，但在重叠区域存在歧义。现有两阶段方法训练复杂且无法端到端优化。

Method: 采用师生框架，教师模型作为伪标签生成器，通过指数移动平均从学生模型更新。引入基于实例中心的查询精炼来提升定位精度，并设计了查询一致性损失和掩码特征一致性损失来对齐预测与伪掩码的语义和几何信号。

Result: 在ScanNetV2和S3DIS数据集上的实验表明，BEEP3D在保持计算效率的同时，达到了与最先进弱监督方法相当或更优的性能。

Conclusion: BEEP3D提供了一种高效的端到端弱监督3D实例分割解决方案，通过师生框架和一致性损失有效解决了边界框标注的歧义问题。
...
<details>
  <summary>Details</summary>
Motivation: 现有知识蒸馏方法难以有效传递教师MLLM的丰富视觉感知能力到学生模型，主要原因是学生和教师模型之间的视觉注意力不匹配。

Method: 提出CompoDistill框架，通过显式对齐学生和教师模型的视觉注意力来增强学生的视觉感知能力。

Result: 实验表明CompoDistill在需要视觉感知能力的组合推理任务上显著提升性能，同时在视觉问答任务上保持良好表现，并展示了在更先进骨干网络上的有效性。

Conclusion: CompoDistill通过解决视觉注意力对齐问题，有效提升了学生模型的视觉感知能力，具有良好的通用性和实用性。
...
<details>
  <summary>Details</summary>
Motivation: 尽管端到端自动驾驶模型在多样化大规模数据集上训练，但在分布外场景中仍表现不佳。COOOL基准测试旨在解决这一差距，鼓励超越封闭分类的危险理解。
...
<details>
  <summary>Details</summary>
Motivation: 生成式AI在计算机视觉领域的发展为优化物流和制造工作流程提供了新机会，但AI应用常受限于专业知识和资源，依赖通用模型需要领域特定数据进行微调，而收集真实数据成本高昂且效率低下。

Method: 使用NVIDIA Omniverse Replicator工具生成合成数据，在仓库环境中进行托盘检测实验，比较仅使用真实数据与使用不同合成数据生成策略的效果。
...
<details>
  <summary>Details</summary>
Motivation: 微表情识别面临挑战，因为面部线索细微短暂且标注数据有限。现有动态图像方法往往忽略微表情不同时间阶段的独特特征。

Method: 提出DIANet双流框架：一个流编码起始到顶点阶段，另一个流捕获顶点到结束阶段。每个流使用专用CNN处理，通过交叉注意力模块自适应融合特征。

Result: 在CASME-II、SAMM和MMEW三个基准数据集上的实验表明，该方法始终优于传统的单阶段动态图像方法。

Conclusion: 明确建模时间阶段信息对于微表情识别至关重要，为推进该领域提供了有前景的方向。
...
<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型在推理任务上表现优异，但对于构建高性能VL推理训练数据集的原则理解不足，需要系统研究数据整理策略对模型推理能力的影响。

Method: 引入多种数据整理方法，分析上下文来源影响，实施针对性数据干预（如图像描述辅助信号、纯文本推理），并探索图像、问题和思维链解决方案的规模化扩展。

Result: 研究发现上下文来源策略显著影响VLM性能，数据干预带来显著收益，多维度数据扩展持续提升推理能力。基于此构建的HoneyBee数据集使3B参数VLM在MathVerse上分别超越SOTA和基础模型7.8%和24.8%。

Conclusion: 本文提出了改进的视觉语言推理数据集整理策略，通过系统性数据整理和规模化扩展，显著提升了VLM的推理性能，同时提出了降低解码成本的测试时扩展策略。
...
<details>
  <summary>Details</summary>
Motivation: 解决并行token预测中的结构不一致问题，传统方法一旦采样token就无法回溯修正错误预测。
...
Result: 实验结果表明，该方法在多个定量指标上优于现有最先进方法，特别是在跨域场景中表现出优异的鲁棒性。
...
<details>
  <summary>Details</summary>
Motivation: 当前视频表示方法过度依赖不稳定且过于细粒度的运动与外观先验（像素级匹配和跟踪），容易因跟踪误差、遮挡和大范围运动而失效。

Method: 使用分层代理节点稳定表达视觉对象的多尺度结构，结合动态表示更新机制利用视频时空先验，并采用形状和纹理表示的分离编码方式。

Result: 实验表明该方法以更少的参数实现高精度的视频重建，并支持视频修复和基于关键帧的时间一致性视频编辑等复杂任务。
...
Method: 基于卷积局部不变性，从ID图像中提取背景特征作为模拟OOD视觉表示，通过优化减少这些背景特征的L2范数来训练模型。

Result: 在多个标准OOD检测基准测试中验证了方法的有效性，与现有后处理方法具有广泛组合兼容性，达到了新的最先进性能。

Conclusion: 提出的方法能够有效缓解OOD检测中的过度自信问题，无需额外OOD数据，具有实际应用价值。
...
Conclusion: AngularFuse通过同时考虑梯度幅值和方向，能够生成纹理强度保持且边缘方向正确的融合图像，展示了优越的融合能力。
...
<details>
  <summary>Details</summary>
Motivation: 现有基准主要评估一般医学任务，无法充分评估在脊柱等依赖视觉输入的细微领域的性能，需要专门的评估工具。

Method: 整合开源脊柱疾病数据集的图像标签对，基于视觉相似性为每个VQA对采样具有挑战性的硬负选项，模拟真实世界场景。

Result: 评估12个领先的MLLM显示，这些模型在脊柱任务上表现不佳，揭示了当前模型在脊柱领域的局限性。

Conclusion: SpineBench填补了脊柱医学领域评估的空白，为未来脊柱医学应用的改进提供了指导方向。
...
<details>
  <summary>Details</summary>
Motivation: 当前动态3D城市场景重建方法在保真度和计算成本之间存在明显权衡，其语义无关的设计无法区分静态背景和安全关键对象的重要性差异。

Method: 提出两个核心贡献：1) 语义引导的剪枝和正则化策略，使用混合重要性度量简化非关键元素；2) 优先级驱动的渲染管线，基于优先级的深度预传递来剔除遮挡基元。

Result: 在Waymo和KITTI数据集上的实验表明，PAGS在安全关键对象上实现卓越的重建质量，同时显著减少训练时间，渲染速度超过350 FPS。

Conclusion: PAGS框架通过语义优先级自适应机制，成功解决了动态3D城市场景重建中保真度与效率的权衡问题。
...
<details>
  <summary>Details</summary>
Motivation: 现有文本-视频检索方法假设视频已预先裁剪且仅包含文本相关内容，但实际中视频通常未经修剪、时长较长且背景内容复杂，因此需要解决部分相关视频检索(PRVR)这一更具挑战性的任务。

Method: 采用双学习框架与动态知识蒸馏(DL-DKD++)，其中大型教师模型为紧凑的双分支学生网络提供监督。学生网络包含继承分支（吸收教师可迁移知识）和探索分支（从PRVR数据集学习任务特定信息）。引入动态软目标构建机制，用自适应软目标替代硬目标监督。

Result: 在TVR、ActivityNet和Charades-STA数据集上的实验结果表明，所提模型在PRVR任务上达到了最先进的性能。

Conclusion: DL-DKD++框架通过知识蒸馏和动态软目标机制，有效解决了部分相关视频检索问题，在多个基准数据集上取得了优异表现。
...
<details>
  <summary>Details</summary>
Motivation: 视觉语言模型在多模态推理方面取得显著进展，但仍容易产生幻觉输出，即输出内容未基于视觉证据。本研究特别关注标志幻觉这一被忽视的问题，即模型在标志不含可见文字的情况下仍生成品牌名称或文本内容。

Method: 使用纯符号、混合和含文本标志的精选数据集，以及具有挑战性的Hard-60子集，系统测量领先VLM的幻觉程度。通过九种结构化扰动测试鲁棒性，并对开源LLaVA进行嵌入级分析，识别与幻觉相关的投影器维度。

Result: 研究发现幻觉在强失真下仍然存在，遮挡暴露了最严重的弱点。嵌入分析显示幻觉与投影器的小部分维度相关，针对性消融可显著减少错误同时保持OCR准确性。模型特别容易对标志性圆形标志产生幻觉。

Conclusion: VLM往往依赖符号先验而非真实字形感知，投影器子空间在这种失败模式中起决定性作用。研究为构建更可信的多模态系统提供了新的诊断视角和可行的缓解策略，特别是投影器解纠缠和OCR引导解码方向。
...
<details>
  <summary>Details</summary>
Motivation: 解决街景环境中从训练轨迹生成不同轨迹（如不同车道或方向）新视角的挑战，提升自动驾驶模拟的真实性。

Method: 两阶段方法：1) 使用高斯泼溅进行3D场景重建并渲染目标相机视角；2) 使用专用单步扩散模型增强渲染结果。

Result: 在公开测试排行榜上获得综合得分0.432，总体排名第二，通过PSNR、SSIM和LPIPS指标验证了新视角质量。

Conclusion: 混合方法结合3D重建和生成增强在街景新视角合成任务中表现优异，证明了该技术路线的有效性。
...
<details>
  <summary>Details</summary>
Motivation: 现有SSC方法依赖时序堆叠或深度投影，缺乏显式运动推理，且难以处理遮挡和噪声深度监督。

Method: 采用多级融合策略，使用预训练光流对齐跨帧的分割、视觉和深度特征；通过课程学习机制从稀疏LiDAR深度渐进过渡到密集立体深度；利用SAM语义先验增强体素级语义学习。

Result: 在SemanticKITTI基准测试中达到16.9的平均IoU，实现最先进性能。

Conclusion: 验证了运动引导和课程感知设计在基于相机的3D语义场景补全中的有效性。
...
<details>
  <summary>Details</summary>
Motivation: 深度神经网络性能提升往往伴随计算复杂度和成本的增加，而3D体积或视频分类任务中存在固有冗余，并非所有切片或帧都是必要的。现有方法只能任务自适应而不能输入自适应，限制了实际应用。

Method: 提出注意力引导的采样模块，通过可学习的子采样框架克服非可微操作的挑战，实现动态输入自适应采样，可在推理阶段根据不同输入调整采样策略。

Result: 在MedMNIST3D医学影像数据集和两个超声视频数据集（包括真实临床条件下收集的内部数据集）上验证了方法的有效性，实现了性能提升和复杂度降低。

Conclusion: 提出的输入自适应采样方法能够有效减少深度神经网络的计算复杂度，同时在分类任务中保持甚至提升性能，特别适用于3D医学影像和视频分析等存在冗余数据的应用场景。
...
TL;DR: 提出了一种概率化重新解释场景坐标回归模型训练的方法，通过注入高级重建先验来改善在训练图像多视角约束不足时的退化问题，包括简单深度分布先验和基于3D点云扩散模型的学习先验。


<details>
  <summary>Details</summary>
Motivation: 场景坐标回归模型在训练图像多视角约束不足时会退化，需要注入高级重建先验来提升模型性能。

Method: 概率化重新解释SCR模型训练，引入多种先验：简单深度分布先验和基于大规模室内扫描数据训练的3D点云扩散模型的学习先验，在训练过程中将预测的3D场景点推向合理几何结构。

Result: 在三个室内数据集上，先验帮助学习更好的场景表示，产生更连贯的场景点云、更高的配准率和更好的相机位姿，对下游任务如新视角合成和相机重定位有积极影响。

Conclusion: 提出的概率化先验方法能有效改善SCR模型在训练约束不足时的性能，提升场景表示质量和下游任务效果。
...
TL;DR: 这篇系统综述探讨了视觉语言模型(VLMs)在城市基础设施监控中的应用，特别关注零样本学习能力，分析了32项相关研究。


<details>
  <summary>Details</summary>
Motivation: 传统城市监控方法依赖物联网传感器和人工检查，成本高、难以扩展，且与市民的视觉感知不一致。研究探索机器是否能像市民一样"看"并推断城市基础设施状况。

Method: 采用PRISMA系统综述方法，分析2021-2025年间32项同行评审研究，重点关注四个核心研究问题：VLM应用任务、架构框架、数据集资源和性能评估。

Result: 研究发现VLMs在城市监控中展现出潜力，特别是在零样本应用场景下，能够处理复杂的视觉信息并与自然语言推理结合。

Conclusion: 视觉语言模型为解决城市基础设施监控挑战提供了有前景的技术路径，特别是在实现机器视觉感知与市民认知对齐方面具有重要价值。
...
TL;DR: 提出基于条件流匹配(CFM)的图像质量转换框架，用于从低场MRI重建高场级图像，在减少参数量的同时实现最先进性能。


<details>
  <summary>Details</summary>
Motivation: 低场MRI虽然经济便携，但信噪比低、诊断质量差，需要一种无需昂贵基础设施就能提升图像质量的方法。

Method: 使用条件流匹配(CFM)框架，通过直接回归最优速度场来学习噪声分布与目标数据分布之间的连续流，避免传统生成模型的迭代采样或对抗目标。

Result: CFM不仅达到最先进性能，还能稳健地泛化到分布内和分布外数据，且使用的参数显著少于竞争性深度学习方法。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于LLM的代理系统在长视频理解中存在两个主要问题：1) 在单帧上进行建模和推理，难以捕捉连续帧的时序上下文；2) 为降低密集帧级标注成本而采用稀疏帧采样，可能丢失关键信息。

Method: VideoLucy采用受人类回忆过程启发的分层记忆结构，具有渐进粒度。通过基于代理的迭代回溯机制，系统性地挖掘视频范围内与问题相关的深度记忆，直到收集到足够信息来提供自信的答案。

Result: 在多个长视频理解基准测试中，基于开源模型的VideoLucy显著优于最先进方法，性能甚至超过GPT-4o等最新专有模型。

Conclusion: VideoLucy通过深度记忆回溯框架有效解决了长视频理解中的时序理解和细节保留问题，同时引入了EgoMem新基准来全面评估模型在极长视频中理解复杂事件和捕捉细粒度细节的能力。
...
TL;DR: 这篇论文是关于纵向放射学报告生成（LRRG）的首个全面综述，特别关注胸部X光片报告生成，旨在解决传统单图像方法无法捕捉纵向临床背景的问题。


<details>
  <summary>Details</summary>
Motivation: 胸部X光成像在现代医学中广泛使用，给放射科医生带来沉重工作负担。传统方法依赖单张图像，无法生成临床准确的比较陈述，需要纳入纵向数据来模拟放射科医生的诊断工作流程。

Method: 系统回顾了纵向放射学报告生成的数据集构建策略、报告生成架构以及专门针对纵向设计的模型，并评估了包含纵向特定指标和广泛使用基准的评估协议。
...
<details>
  <summary>Details</summary>
Motivation: 针对现有对抗攻击方法在深度伪造检测器上迁移性和视觉不可察觉性不足的问题，开发一个能够同时优化这两个关键指标的攻击框架。

Method: 采用两阶段方法：第一阶段使用双流攻击模块生成对抗候选样本（MNTD-PGD优化小扰动预算的梯度计算，SG-PGD聚焦视觉显著区域）；第二阶段通过度量感知选择模块基于黑盒模型成功率和结构相似性评估候选样本。

Result: 与最先进的攻击方法相比，MS-GAGA在未见检测器上实现了高达27%的错误分类率提升。

Conclusion: MS-GAGA通过联合优化迁移性和不可察觉性，有效提升了对抗样本在黑盒深度伪造检测器上的攻击效果。
...
TL;DR: 提出早期融合框架，在数据增强前结合文本和视觉特征，解决传统增强方法破坏图像-文本空间对齐的问题，在医学图像分割任务中取得SOTA结果。


<details>
  <summary>Details</summary>
Motivation: 传统数据增强方法（如旋转、翻转）会破坏图像与文本描述之间的空间对齐关系，影响多模态学习在医学图像分割中的性能。

Method: 采用早期融合框架，在数据增强前将文本和视觉特征结合；设计轻量级生成器将文本嵌入投影到视觉空间，弥合语义鸿沟。

Result: 在三个医学成像任务和四个分割框架上评估，可视化显示生成的伪图像能准确定位区域，达到最先进性能。

Conclusion: 提出的早期融合方法能有效保持图像-文本的空间一致性，提升医学图像分割性能，代码已开源。
...
TL;DR: 提出Bi-Stage 3D Gaussian Splatting框架，通过两阶段优化解决运动模糊图像中的3D场景重建问题，包含相机姿态优化和全局刚性变换，配合子帧梯度聚合策略提升重建质量。


<details>
  <summary>Details</summary>
Motivation: 现有3DGS去模糊方法性能受限，对相机姿态精度极度依赖且无法有效控制运动模糊导致的错误高斯基元密集化，需要解决运动模糊图像的高质量3D重建挑战。

Method: 双阶段框架：1)相机姿态优化阶段粗略优化相机姿态；2)全局刚性变换阶段校正运动引起的模糊失真。采用子帧梯度聚合策略缓解多子帧梯度冲突，并引入时空双阶段优化策略动态调整基元密集化阈值。

Result: 综合实验验证了所提去模糊方法的有效性，在性能上优于现有最先进方法。

Conclusion: BSGS框架能准确从运动模糊图像中重建3D场景，通过两阶段优化和梯度聚合策略有效解决了现有方法的局限性。
...
TL;DR: 提出了一种轻量级、无需网络的Voronoi辅助扩散方法，直接从无向点云计算无符号距离场，解决了现有神经方法在数值稳定性、计算成本和可控性方面的限制。


<details>
  <summary>Details</summary>
Motivation: 现有神经方法学习无符号距离场时存在数值不稳定、计算成本高和可控性有限的问题，需要一种更稳定高效的方法来处理各种拓扑结构的3D形状。
...
Conclusion: VAD方法提供了一种计算高效、数值稳定的无符号距离场计算方案，能够处理各种复杂拓扑结构的3D形状。
...
TL;DR: 提出了一种基于分数扩散的简单有效的人体运动生成方法，仅需特征空间归一化和分析推导的损失权重，就能达到最先进水平，同时直接生成运动和形状。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖过度参数化的输入特征和辅助损失，但对于扩散模型来说这些策略并非必要。本文旨在证明通过精心设计的特征归一化和损失权重，就能有效匹配人体运动分布。

Method: 使用基于分数的扩散模型，仅采用特征空间归一化和分析推导的L2分数匹配损失权重，直接生成运动和形状，避免缓慢的关节后处理。
...
TL;DR: 提出CoIRL-AD双策略框架，通过竞争机制结合模仿学习和强化学习，相比传统两阶段方法显著提升自动驾驶模型的泛化能力和性能。


<details>
  <summary>Details</summary>
Motivation: 模仿学习泛化能力差，强化学习样本效率低且收敛不稳定，需要结合两者优势来提升自动驾驶模型性能。

Method: 提出竞争性双策略框架CoIRL-AD，让IL和RL智能体在训练中交互，通过竞争机制促进知识交换并避免梯度冲突。

Result: 在nuScenes数据集上实验显示，碰撞率降低18%，在长尾场景中表现出更强的泛化能力和改进性能。

Conclusion: CoIRL-AD框架通过IL和RL的竞争性交互，有效提升了自动驾驶模型的性能和泛化能力。
...
TL;DR: 提出了首个无人机多光谱多目标跟踪基准MMOT，包含125个视频序列和48.8万+标注，并开发了光谱特征提取和方向感知的跟踪方案，显著提升了小目标和密集场景的跟踪性能。
...
Method: 构建大规模多光谱无人机数据集MMOT，提出轻量级光谱3D-Stem集成光谱特征，开发方向感知卡尔曼滤波器进行精确状态估计，以及端到端方向自适应transformer。

Result: 实验表明多光谱输入相比RGB基线显著提升了跟踪性能，特别是对小目标和密集物体效果更明显。
...
Method: 引入基于Mamba的Temporally Conditional Mamba模型，将条件信息整合到Mamba块的循环动态中。

Result: 在多种人类运动任务上的实验表明，该模型在时间对齐、运动真实性和条件一致性方面显著优于现有最先进方法。

Conclusion: 该方法有效解决了基于时间条件的人类运动生成中的时间对齐问题，提升了生成质量。
...
TL;DR: 提出了一种零样本农业图像分割方法，结合Plantnet植物分类模型、DinoV2主干网络和Segment Anything Model (SAM)，无需收集新数据集即可实现植物区域的精确分割。
...
Method: 使用Plantnet的植物专用表示来识别植物区域并生成粗分割掩码，然后通过SAM进行细化处理得到详细分割结果。

Result: 在四个不同复杂度的公开数据集上评估，使用Plantnet微调的DinoV2相比基础DinoV2模型在IoU指标上表现更优。
...
<details>
  <summary>Details</summary>
Motivation: 现有研究表明扩散模型的生成质量与学习到的表示相关，外部指导可以加速训练。作者希望探索如何利用模型自身的中间表示来替代外部监督。

Method: 基于扩散模型各层表示质量不同的观察，使用语义最丰富的表示作为内在指导来正则化较弱的表示，提出LayerSync作为即插即用的正则化项。

Result: 在图像生成上显著提升训练效率和生成质量，例如在ImageNet数据集上加速流式transformer训练8.75倍，生成质量提升23.6%。方法可扩展到音频、视频和运动生成等领域。

Conclusion: LayerSync是一种自给自足的正则化方法，能有效提升扩散模型的训练效率和生成质量，且具有跨领域的通用性。
...
TL;DR: 提出了一种新颖的两阶段训练框架，用于像素空间扩散模型和一致性模型，通过预训练编码器和端到端微调，显著提升了像素空间生成模型的性能，在ImageNet数据集上取得了与潜在空间模型相媲美的结果。
...
Method: 采用两阶段训练：第一阶段预训练编码器捕获干净图像的语义信息，并与确定性采样轨迹对齐；第二阶段将编码器与随机初始化解码器集成，对完整模型进行端到端微调。

Result: 在ImageNet-256上扩散模型达到FID 2.04，ImageNet-512达到FID 2.35（75 NFE），大幅超越先前像素空间方法；一致性模型在ImageNet-256上单步采样达到FID 8.82，显著优于潜在空间对应模型。

Conclusion: 该训练框架成功缩小了像素空间与潜在空间生成模型之间的性能差距，首次实现了无需预训练VAE或扩散模型的高分辨率图像一致性模型训练。
...
TL;DR: 提出IVT-LR方法，通过隐式多模态潜在推理减少标注需求和推理延迟，在M3CoT和ScienceQA上实现5.45%准确率提升和5倍速度提升
...
Method: IVT-LR在潜在空间中注入视觉和文本信息，每个推理步骤结合潜在文本（前一步隐藏状态）和潜在视觉（选定图像嵌入），采用渐进多阶段训练策略

Result: 在M3CoT和ScienceQA上平均准确率提升5.45%，推理速度提升5倍以上

Conclusion: IVT-LR通过多模态潜在推理有效解决了标注需求和推理效率问题，为多模态推理提供了新思路
...
<details>
  <summary>Details</summary>
Motivation: 现有方法忽视水下成像物理原理或简单将水下图像退化视为干扰因素，未能充分利用其中包含的宝贵信息

Method: 基于整流流的框架，将水下物理成像信息作为显式先验直接融入网络训练过程，并引入时间维度建模

Result: 在USOD10K数据集上，S_m指标获得0.072的提升

Conclusion: WaterFlow方法在性能和有效性方面表现出优越性
...
Method: 利用图像纹理在不同频带具有位置相似性和内容一致性的特性，而噪声不具备这一特性，设计了跨频一致性损失和超轻量网络实现图像去噪。

Result: 在多个真实世界图像数据集上的实验表明，ZSCFC在计算效率和去噪性能上优于其他最先进的零样本方法。

Conclusion: ZSCFC是一种高效有效的真实世界去噪方法，能够在单张噪声图像上训练，不依赖噪声分布假设，在效率和性能上均有优势。
...
TL;DR: 本文开发了轻量级的人体网格恢复(HMR)和姿态估计(HPE)模型，通过使用分层视觉基础模型(VFMs)的前几个阶段作为编码器，在保持性能的同时显著提升计算效率。


<details>
  <summary>Details</summary>
Motivation: 现有最先进的HMR方法依赖大型非分层视觉变换器作为编码器，计算成本高昂。本文旨在开发简单高效的模型，通过利用分层VFMs的早期阶段来优化准确性与计算效率的平衡。

Method: 构建了三种轻量级HMR2.0变体，并提出使用Swin Transformer、GroupMixFormer和VMamba等分层VFMs的前两到三个阶段作为编码器，因为其中间阶段产生的特征图分辨率与非分层模型相当或更高。

Result: 评估了27个基于分层VFM的HMR和HPE模型，发现仅使用前两到三个阶段就能达到与全阶段模型相当的性能，且截断模型在准确性和计算效率之间展现出更好的权衡。

Conclusion: 分层VFMs的早期阶段足以实现高性能的HMR和HPE任务，为开发轻量级高效模型提供了有效途径。
...
TL;DR: TerraCodec (TEC) 是一个专门为地球观测数据设计的深度学习压缩算法家族，包括图像压缩变体和时序Transformer模型，在Sentinel-2数据上实现3-10倍压缩比提升，并支持零样本云修复。


<details>
  <summary>Details</summary>
Motivation: 地球观测卫星产生大量多光谱图像时间序列，现有压缩方法存在局限性：图像编解码器忽略时间冗余，视频编解码器依赖的运动先验无法捕捉静态场景的辐射演化。

Method: 提出TerraCodec系列算法，包括适应多光谱输入的高效图像压缩变体，以及利用时间依赖性的时序Transformer模型(TEC-TT)。创新性地提出Latent Repacking方法，训练灵活码率的Transformer模型。

Result: 在Sentinel-2数据上，TerraCodec优于传统编解码器，在同等图像质量下实现3-10倍更强的压缩。TEC-TT在AllClear基准测试中零样本云修复性能超越现有最优方法。

Conclusion: 专门定制的学习型压缩算法是地球观测领域的有前景方向，代码和模型权重将开源发布。
...
TL;DR: 提出了一种多无人机协作占用预测框架，通过空间感知特征编码和跨智能体特征整合来保留3D空间结构和语义信息，显著提升了协作感知性能并大幅降低了通信开销。


<details>
  <summary>Details</summary>
Motivation: 当前基于鸟瞰图的无人机群协作感知方法存在两个主要局限：边界框表示无法捕捉完整的场景语义和几何信息，且在遇到未定义或遮挡物体时性能显著下降。

Method: 结合空间感知特征编码器和跨智能体特征整合来有效保留3D空间结构和语义；引入高度感知特征缩减来紧凑表示场景信息；采用双掩码感知引导机制自适应选择特征并减少通信开销。

Result: 实验结果表明，该方法达到了最先进的精度，显著优于现有的协作方法，同时将通信开销降低到先前方法的一小部分。

Conclusion: 所提出的多无人机协作占用预测框架有效解决了现有方法的局限性，在保持高精度的同时大幅降低了通信需求，为无人机群系统提供了高效的协作感知解决方案。
...
TL;DR: 提出EReLiFM方法解决带噪声标签的开集域泛化问题，通过证据可靠性感知的残差流元学习来提升模型在噪声环境下的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现实应用中标签噪声会破坏源域知识，使模型难以识别已知类别和拒绝未知类别。现有方法在有限干净标签数据下难以弥合域差距。

Method: 采用两阶段证据损失聚类提升标签可靠性感知，提出残差流匹配机制建模结构化域和类别条件残差，通过元学习优化使干净集更新方向最大化减少噪声集损失。

Result: 实验表明EReLiFM在OSDG-NL任务上优于现有方法，达到最先进性能。

Conclusion: 该方法有效解决了带噪声标签的开集域泛化问题，通过可靠性感知和残差流机制提升了模型在噪声环境下的泛化能力。
...
TL;DR: SRUM是一个自奖励后训练框架，通过让多模态模型的理解模块评估生成模块来实现自我改进，无需额外人工标注数据。


<details>
  <summary>Details</summary>
Motivation: 现有统一多模态模型存在视觉理解能力无法有效迁移到视觉生成的问题，模型能正确理解图像但无法从文本生成忠实图像。
...
Conclusion: 建立了一个新范式，使多模态模型的理解模块能够通过自奖励来指导和增强自身的生成能力。
...
TL;DR: 提出H-EGL框架，结合自监督和人工指导的约束，增强注意力对齐并提高泛化能力，在胸部X光分类中优于现有EGL方法。


<details>
  <summary>Details</summary>
Motivation: Transformer模型在医学影像中表现优异，但容易学习虚假相关性导致偏见和泛化能力有限。人类-AI注意力对齐可缓解此问题，但依赖昂贵的人工监督。

Method: 提出混合解释引导学习(H-EGL)框架，结合自监督和人工指导约束。自监督组件利用类别区分性注意力，不依赖限制性先验，提升鲁棒性和灵活性。

Result: 在胸部X光分类任务中使用Vision Transformer验证，H-EGL优于两种最先进的EGL方法，表现出更好的分类准确率和泛化能力。

Conclusion: H-EGL框架能产生与人类专家知识更好对齐的注意力图，同时提升模型性能和泛化能力。
...
TL;DR: IRIS是一个评估多模态大语言模型在"与图像思考"范式下能力的基准，包含1,204个视觉任务，显示当前模型在整合视觉和工具使用方面表现不佳，最强模型仅达到18.68%通过率。


<details>
  <summary>Details</summary>
Motivation: 现有基准主要遵循"关于图像思考"范式，将图像视为静态输入，而忽略了MLLMs需要主动操作图像作为可操纵认知工作空间的能力。

Method: 引入IRIS基准，包含1,204个挑战性视觉任务（603单轮，601多轮），涵盖五个不同领域，配有详细评分标准，系统评估MLLMs的感知、转换和推理能力。

Result: 当前MLLMs在需要有效整合视觉和通用工具的复杂任务中表现不佳，GPT-5-think仅达到18.68%通过率，不同模型在工具使用行为上存在差异。
...
<details>
  <summary>Details</summary>
Motivation: 基础模型在医疗AI中应用面临数据共享限制和隐私保护问题，联邦学习可以解决数据聚合难题，但需要更好的个性化微调方法。
...
TL;DR: FlashVSR是首个基于扩散模型的实时视频超分辨率框架，通过三阶段蒸馏、稀疏注意力和小型解码器实现17 FPS的高效处理，在A100 GPU上支持768x1408分辨率。


<details>
  <summary>Details</summary>
Motivation: 现有扩散模型在视频超分辨率应用中存在延迟高、计算量大、对超高分辨率泛化能力差的问题，需要实现高效、可扩展且实时的解决方案。
...
Method: 提出SPORTS框架，包含：1）自适应注意力几何融合机制的视频全景分割；2）结合全景分割和光流图的视觉里程计；3）基于点的场景渲染，将稀疏点云转换为神经场。

Result: 在三个公共数据集上的实验表明，该方法在里程计、跟踪、分割和新视角合成任务上优于现有最先进方法。

Conclusion: SPORTS框架通过任务间的紧密集成，实现了更全面的场景理解，为具身AI智能体提供了有效的场景感知解决方案。
...
TL;DR: 提出了VQArt-Bench，一个用于文化遗产领域的大规模视觉问答基准，通过多智能体流水线生成细致、验证过的多样化问题，评估多模态大语言模型在视觉艺术分析中的深层语义理解能力。


<details>
  <summary>Details</summary>
Motivation: 现有VQA基准在评估深层语义理解方面存在不足，特别是复杂领域如视觉艺术分析，问题局限于简单句法结构和表面属性，无法捕捉人类视觉查询的多样性和深度，导致模型利用统计捷径而非进行视觉推理。

Method: 使用新颖的多智能体流水线，专门智能体协作生成细致、验证过的语言多样化问题，基准结构沿着相关视觉理解维度设计，探测模型解释符号意义、叙事和复杂视觉关系的能力。

Result: 对14个最先进MLLMs的评估揭示了当前模型的显著局限性，包括简单计数任务的意外弱点，以及专有模型与开源模型之间的明显性能差距。

Conclusion: VQArt-Bench填补了现有VQA基准在评估深层语义理解方面的空白，特别是在文化遗产领域，揭示了当前MLLMs在复杂视觉推理任务中的不足。
...
<details>
  <summary>Details</summary>
Motivation: 事件相机中的光流和6-DoF自运动估计通常独立处理，但由于缺乏鲁棒的数据关联，单独解决这两个问题在无监督情况下是病态的挑战。现有方法要么引入偏差和计算开销，要么容易陷入局部最优解。

Method: 将相机自运动建模为连续样条，光流建模为隐式神经表示，通过归纳偏差嵌入时空一致性；结合微分几何约束引入结构和运动先验，避免显式深度估计但保持几何一致性。

Result: 在一般6-DoF运动场景中表现出良好的通用性，在无监督方法中达到最先进性能，甚至与有监督方法相竞争。
...
<details>
  <summary>Details</summary>
Motivation: 头部运动在脑PET成像中会导致图像伪影和示踪剂定量不准确，而基于硬件的运动跟踪在临床实践中应用受限。

Method: 使用带有交叉注意力的深度学习模型，从1秒3D PET原始数据预测刚性头部运动，通过监督学习利用已有的动态PET扫描和硬件跟踪的黄金标准运动数据。

Result: 在两个PET扫描仪和四种示踪剂上验证，DL-HMC++优于现有数据驱动方法，产生的无运动图像与黄金标准硬件跟踪结果难以区分，标准摄取值差异率仅为1.2±0.5%和0.5±0.2%。
...
Method: 提出了一种推理时的特征无关上采样架构，无需编码器特定训练，可应用于各种特征类型。

Result: AnyUp在特征上采样方面达到了新的最先进水平，能够泛化到不同的特征类型，保持特征语义，并且高效易用于各种下游任务。
...
TL;DR: 本文提出了高效感知超分辨率(EPSR)的全面研究和基准测试，旨在在严格效率约束下(最多5M参数和2000 GFLOPs)复制或改进Real-ESRGAN的感知结果，并在新构建的4K测试数据集上取得了优于Real-ESRGAN的性能。


<details>
  <summary>Details</summary>
Motivation: 虽然高效PSNR导向的超分辨率取得了显著进展，但专注于感知质量度量的方法仍然相对低效，这促使研究者填补这一空白。

Method: 在严格效率约束下(最多5M参数和2000 GFLOPs)开发高效感知超分辨率方法，并在包含500张4K测试图像的新数据集上进行评估，这些图像使用多种退化类型处理，模拟真实部署条件。

Result: 最佳方法在所有基准数据集上都优于Real-ESRGAN，证明了高效方法在感知领域的潜力。

Conclusion: 本文为高效感知超分辨率建立了现代基准，展示了在严格效率约束下实现优异感知质量的可能性。
...
TL;DR: USplat4D是一个不确定性感知的动态高斯泼溅框架，通过估计每个高斯的时间变化不确定性并构建时空图进行优化，解决单目动态3D重建中的遮挡和极端视角问题。
...
TL;DR: MVP4D是一个基于扩散模型的视频生成系统，能够从单张参考图像生成可动画的多视角数字人视频，并蒸馏为实时渲染的4D虚拟形象。


<details>
  <summary>Details</summary>
Motivation: 传统创建逼真数字人虚拟形象的过程昂贵且耗时，需要大型相机捕捉设备和专业3D艺术家。现有单图生成方法缺乏多视角约束，导致视角偏离时质量下降。

Method: 基于预训练视频扩散模型，同时生成数百帧从不同视角（最多360度）的视频，然后将输出蒸馏为可实时渲染的4D虚拟形象。

Result: 相比先前方法，显著提高了生成虚拟形象的逼真度、时间一致性和3D一致性。

Conclusion: 该方法显著降低了虚拟形象创建的门槛，同时保持了高质量的多视角渲染效果。
...
TL;DR: AIM 2025高效真实世界去模糊挑战赛回顾，基于RSBlur数据集，要求模型参数少于500万且计算量低于200 GMACs，4支队伍提交有效方案，最佳方案PSNR达31.1298 dB。
...
Method: 基于RSBlur数据集构建测试集，使用双相机系统捕获模糊和退化图像对，要求参与者开发满足参数和计算量限制的去模糊解决方案。
...
TL;DR: UniFusion是一个基于扩散的生成模型，使用冻结的大型视觉语言模型(VLM)作为统一的多模态编码器，通过Layerwise Attention Pooling(LAP)机制提取文本和视觉token的语义和细节信息，并提出了VERIFI方法增强推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有视觉生成架构通常使用独立的图像和文本编码器，限制了跨模态推理和知识迁移能力。现有解决方案要么使用VLM的最后一层信息，要么需要大量计算资源和数据训练统一模型，限制了可访问性。

Method: 1. 使用冻结的VLM作为统一多模态编码器；2. 提出Layerwise Attention Pooling(LAP)机制提取多层级信息；3. 提出VERIFI方法，在模型内提示重写时仅使用VLM生成的文本token来条件化扩散变换器；4. 在编辑任务上进行微调。

Result: LAP在文本-图像对齐方面优于其他浅层融合架构，能够忠实传递VLM的视觉信息到扩散模型。微调编辑任务不仅提高了生成的对齐度，还展现出强大的泛化能力，在单图像编辑训练后能够零样本泛化到多图像参考。

Conclusion: UniFusion的统一编码器设计有效促进了跨模态知识迁移，证明了使用冻结VLM作为统一编码器的可行性，为多模态生成提供了更高效和灵活的解决方案。
...
TL;DR: CuMPerLay是一个可微分的向量化层，将立方多参数持久性(CMP)集成到深度学习管道中，通过分解CMP为可学习的单参数持久性组合，为图像分析提供稳健的拓扑特征向量。


<details>
  <summary>Details</summary>
Motivation: CMP虽然为图像拓扑分析提供了强大方法，但由于多过滤结构的复杂性和CMP向量化的困难，其应用受到限制。CuMPerLay旨在解决这些挑战，使CMP能够无缝集成到深度学习框架中。

Method: 提出新算法向量化立方复形的多参数同调，将CMP分解为可学习的单参数持久性组合，其中双过滤函数被联合学习。该方法具有可微性，可与Swin Transformers等先进架构集成。

Result: 在基准医学成像和计算机视觉数据集上的实验表明，CuMPerLay在分类和分割性能上带来提升，特别是在数据有限的情况下。理论分析证明了在广义Wasserstein度量下的稳定性保证。
...
<details>
  <summary>Details</summary>
Motivation: 解决VLA模型中存在的"监督不足"问题，即模型的大容量仅由稀疏的低维动作监督，导致表示能力未充分利用。

Method: 提出DriveVLA-W0训练范式，使用世界建模预测未来图像，为模型提供密集的自监督信号。针对两种主要VLA架构：使用离散视觉标记的自回归世界模型和使用连续视觉特征的扩散世界模型。

Result: 在NAVSIM v1/v2基准测试和680倍更大的内部数据集上，DriveVLA-W0显著优于BEV和VLA基线方法，并增强了数据缩放效应。

Conclusion: 世界建模是解决VLA监督不足的有效方法，能够学习驾驶环境的底层动态，显著提升模型性能并加速数据规模增长带来的性能增益。
...
TL;DR: Rex-Omni是一个3B规模的MLLM，通过特殊任务设计、数据引擎和两阶段训练流程，在零样本设置下实现了与传统回归模型相当或更优的目标检测性能，并具备多种视觉感知能力。


<details>
  <summary>Details</summary>
Motivation: 传统坐标回归模型（如YOLO、DETR）主导目标检测，但MLLM方法面临召回率低、重复预测、坐标不对齐等问题。本文旨在弥合这一差距。
...
Conclusion: Rex-Omni为更通用和语言感知的视觉感知系统铺平了道路，具备对象指代、指向、视觉提示、GUI定位、空间指代、OCR和关键点定位等多种能力。
...
Method: 采用两阶段训练流程：冷启动监督微调阶段和在线强化学习优化。引入DeepMMSearchVQA数据集，包含多样化的多跳查询，整合文本和视觉信息，教导模型何时搜索、搜索什么、使用哪个搜索工具以及如何对检索信息进行推理。

Result: 在多个知识密集型基准测试上进行了广泛实验，证明了该方法的优越性。

Conclusion: 提出的DeepMMSearch-R1模型能够有效进行多模态网络搜索，为推进多模态网络搜索提供了有价值的见解。
...
TL;DR: LLMs在逻辑谜题上表现良好但依赖记忆而非推理，PHANTOM RECALL基准测试显示模型在扰动谜题中会出现'幻影回忆'现象，即错误地复述记忆中的解决方案。


<details>
  <summary>Details</summary>
Motivation: 探究LLMs在逻辑谜题中的真实推理能力，而非仅仅依赖记忆模板，特别是在谜题被轻微修改时模型的表现。

Method: 引入PHANTOM RECALL基准，包含25个经典逻辑谜题和149个扰动版本，评估11个主流LLM，开发自动逻辑等价判断器、细粒度错误分类法和提示缓解框架。

Result: 模型在原始谜题上准确率接近完美，但在扰动谜题上表现显著低于人类，出现幻影回忆和过度阐述等错误模式。
...
TL;DR: LLMs可作为世界模型增强智能体决策，但存在幻觉和静态知识限制。研究提出R-WoM方法，通过检索外部教程知识来提升长时程模拟能力，在OSWorld和WebArena上分别取得25.3%和18.1%的性能提升。


<details>
  <summary>Details</summary>
Motivation: 探索LLMs是否适合作为世界模型，解决其在长时程模拟中因幻觉和静态知识导致的错误累积问题。

Method: 提出检索增强世界模型(R-WoM)，通过从外部教程检索事实性、最新知识来增强LLM的模拟能力，减少幻觉。

Result: R-WoM在OSWorld和WebArena基准测试中分别实现25.3%和18.1%的性能提升，尤其在长时程模拟中表现更佳。

Conclusion: LLMs在短时程状态预测中有效，但在长时程规划中存在局限；R-WoM通过知识检索有效缓解了这些问题，显著提升了世界建模能力。
...
TL;DR: 研究发现大语言模型对输入文本的微小变化（如拼写错误、改写）非常敏感，其内部对陈述真实性的表示能力会随着样本与预训练数据的相似度降低而显著退化，揭示了模型学习的知识表示是浅层且非鲁棒的。


<details>
  <summary>Details</summary>
Motivation: 探索大语言模型性能脆弱性的根本原因，检验这是否源于不稳定的内部知识表示，特别是测试模型在语义保持但表面形式变化的样本上区分陈述真实性的能力。

Method: 通过对样本应用语义保持的扰动（如拼写错误、改写），使其分布外化，然后在四个大语言模型家族、五个评估数据集和三种知识探测方法上评估表示可分性的退化情况。

Result: 当样本呈现与预训练数据相似度降低时，模型内部对陈述真实性的表示能力会崩溃。虽然模型在样本与预训练数据相似时能区分真假陈述，但这种能力高度依赖陈述的确切表面形式。

Conclusion: 大语言模型可能学习的是浅层、非鲁棒的知识表示，这解释了基准测试性能的脆弱性，并对真实性探测的实用性提出了根本挑战，呼吁进一步研究改进学习知识表示的鲁棒性。
...
TL;DR: 大型推理模型在机器翻译任务中生成中间思考标记并不能提升翻译性能，但通过模块化翻译策略构建中间标记可以带来改进。


<details>
  <summary>Details</summary>
Motivation: 探索大型推理模型在机器翻译任务中生成中间思考标记的潜在益处，特别是在不同资源水平的语言对中。

Method: 使用蒸馏思维链方法，基于人类翻译实践生成逐步翻译的合成解释，并比较标准输入输出微调与包含思考标记的微调。

Result: 思维标记不能帮助LRMs更好地执行机器翻译，包含翻译步骤的合成思维链解释微调不优于标准微调，但模块化翻译策略构建的中间标记能带来改进。
...
TL;DR: 提出了MIND用户参与的事实核查流程，用于检测多语言问答知识库中的事实和文化差异，特别关注文化敏感问题在不同地区的答案差异。
...
Method: 开发了用户参与的事实核查流程MIND，在母婴健康领域的双语问答系统上进行评估，并发布了标注事实和文化不一致的双语问题数据集。

Result: MIND在所有测试案例中都能可靠地识别不一致性，支持开发更具文化意识和事实一致性的问答系统。

Conclusion: MIND能够有效检测多语言问答系统中的事实和文化不一致性，有助于构建更准确、文化敏感的问答系统。
...
TL;DR: TopoAlign框架利用代码仓库作为数学大语言模型的训练资源，通过将代码分解并重组为与形式数学语句结构对齐的模拟数据，显著提升了模型在数学形式化任务上的性能。


<details>
  <summary>Details</summary>
Motivation: 当前数学大语言模型在自动形式化任务上表现不佳，主要受限于大规模非正式与形式数学语句配对语料的稀缺性。虽然现有模型能够从自然语言生成代码，但代码与形式数学之间的结构和语法差异限制了有效的迁移学习。

Method: TopoAlign框架将代码分解为文档字符串、主函数和依赖函数，然后将这些组件重新组装成结构上与形式数学语句对齐的模拟数据。这种方法无需额外人工标注即可生成用于训练数学大语言模型的结构对齐代码数据。

Result: 在DeepSeek-Math和Herald两个先进模型上进行了评估，使用minif2f、Putnam和ProofNet基准测试。TopoAlign为DeepSeek-Math带来了显著提升，BEq@10指标提高17.77%，typecheck@10指标提高68.82%。即使对于专门的Herald模型，也实现了BEq@10提升0.12%和typecheck@10提升1.09%。

Conclusion: TopoAlign框架证明，即使不引入新的数学知识，通过训练结构对齐的代码数据也能显著提升数学大语言模型的性能，为利用广泛可用的代码仓库资源提供了有效途径。
...
TL;DR: GRAVITY是一个通过生成基于用户画像的合成偏好数据来减少对人工标注依赖的个性化LLM框架，整合了人口统计、文化和心理维度，在跨文化场景中显著提升了生成内容的质量和用户偏好度。


<details>
  <summary>Details</summary>
Motivation: 当前LLM个性化方法依赖昂贵的人工反馈或交互日志，限制了可扩展性且忽略了更深层次的用户属性，需要一种减少人工标注依赖的解决方案。

Method: 提出GRAVITY框架，整合Hofstede文化维度、Schwartz基本价值观、世界价值观调查和Big Five OCEAN人格特质等理论，生成基于用户画像的合成偏好数据对来指导个性化内容生成。

Result: 在400名亚马逊用户的书籍描述任务中，GRAVITY相比基于提示的条件生成、标准微调和朴素合成对生成方法，实现了超过4%的偏好增益，用户研究显示其输出在86%的情况下更受偏好。

Conclusion: 基于场景的合成数据能够捕捉更丰富的用户差异，减少对昂贵标注的依赖，产生更具吸引力的用户中心内容，为LLM个性化提供了可扩展的路径。
...
TL;DR: 提出了CRUMQs管道，用于自动生成不可作弊、真实、不可回答和多跳查询，以解决现有RAG基准测试在复杂查询评估上的不足。


<details>
  <summary>Details</summary>
Motivation: 现实RAG系统面临复杂查询时，相关信息可能缺失或不完整，需要能够拒绝不可回答和超出范围的问题。现有基准测试很少反映真实任务复杂度，限制了发现RAG系统局限性的能力。

Method: 开发了首个自动、难度可控的CRUMQs生成管道，可适应任何语料库和领域。该管道创建不可作弊、真实、不可回答和多跳查询。
...
TL;DR: 提出直接多令牌解码（DMTD）方法，通过仅使用后期层生成多个令牌来加速解码器-仅Transformer的推理，无需额外参数或验证步骤。


<details>
  <summary>Details</summary>
Motivation: 基于预训练LLM中不同层承担不同角色的观察：早期层处理输入理解，中间层处理任务特定处理，后期层生成输出。假设经过早期和中间层处理后，隐藏状态已包含足够信息支持仅使用后期层生成多个令牌。

Method: 直接多令牌解码（DMTD）范式，在早期和中间层处理完表示后，仅使用后期层一次性生成多个令牌，避免重复遍历早期和中间层。

Result: 在有限数据集上微调的DMTD Qwen3-4B模型实现了高达2倍的加速，仅有轻微性能损失。扩展分析表明使用更大训练数据集性能有望进一步提升。

Conclusion: DMTD是一种有效的推理加速方法，无需额外参数或验证步骤，在保持性能的同时显著提升解码速度，具有很好的扩展潜力。
...
TL;DR: 提出了Context-Folding框架，让LLM智能体能够主动管理工作上下文，通过子轨迹分支处理子任务并在完成后折叠中间步骤，仅保留结果摘要。
...
Method: 开发了端到端强化学习框架FoldGRPO，使用特定过程奖励来鼓励有效的任务分解和上下文管理，智能体可以程序化地分支到子轨迹处理子任务。
...
TL;DR: 论文指出自动形式化需要先进行猜想步骤，创建了ConjectureBench数据集和评估框架来衡量LLMs的猜想能力，并提出Lean-FIRe方法改进猜想和自动形式化性能。


<details>
  <summary>Details</summary>
Motivation: 现有自动形式化研究忽视了猜想这一关键前置步骤，且LLMs的猜想能力评估有限，难以理解其对自动形式化的影响。

Method: 扩充现有数据集创建ConjectureBench，重新设计评估框架和指标，并提出推理时方法Lean-FIRe来改进猜想和自动形式化。

Result: 当考虑猜想因素时，LLMs的自动形式化性能被显著高估；使用Lean-FIRe方法，GPT-4.1和DeepSeek-V3.1分别成功实现了13个和7个PutnamBench问题的端到端自动形式化。

Conclusion: LLMs具备生成准确猜想所需的知识，但改进自动形式化性能需要将猜想视为独立任务，并研究如何正确整合到自动形式化流程中。
...
TL;DR: SAGE是一个用于多轮交互智能体评估的用户模拟框架，通过整合业务上下文知识（如理想客户画像和业务基础设施）来生成更真实、多样的交互，并能发现更多智能体错误。


<details>
  <summary>Details</summary>
Motivation: 现有的多轮交互智能体评估方法依赖人工评估，而使用模拟用户的方法通常建模通用用户，忽略了领域特定原则，无法捕捉真实用户行为。

Method: 提出SAGE框架，整合自上而下的业务逻辑知识（如理想客户画像）和自下而上的业务基础设施知识（如产品目录、FAQ、知识库），使模拟器能生成反映目标市场用户信息需求和期望的交互。

Result: 该方法生成的交互更加真实和多样，同时能识别出多达33%的智能体错误，显示出其作为支持错误发现和迭代改进的有效评估工具。

Conclusion: SAGE通过整合业务上下文知识，提供了一个更有效的多轮交互智能体评估框架，能够生成更真实的用户交互并发现更多智能体缺陷。
...
TL;DR: 本文提出了一种用于离散数学逻辑等价问题的新型自动问题生成方法，通过定义形式语言和生成规则，开发线性时间算法来生成难度均匀的问题，以解决学术不端和提供练习资源。


<details>
  <summary>Details</summary>
Motivation: 在线教学时代抄袭问题日益严重，现有自动问题生成方法效率低下且问题难度不均，需要开发更有效的方法来生成逻辑等价问题。

Method: 定义逻辑等价问题的形式语言，将其转换为两组生成规则，并开发线性时间算法进行问题生成。

Result: 学生测试显示生成问题准确性与教科书问题相当；难度评估表明生成问题难度与教科书问题相似，质量得到验证。

Conclusion: 提出的自动问题生成方法能有效生成高质量的逻辑等价问题，为离散数学教学提供了可靠的问题生成解决方案。
...
TL;DR: 比较神经符号系统和LLM系统在农业领域信息抽取的表现，LLM系统总体表现更好但各有优缺点


<details>
  <summary>Details</summary>
Motivation: 当前信息抽取过度依赖大语言模型，忽视了传统符号或统计系统的经验积累，需要系统比较不同方法的实际效果

Method: 在农业领域的猪肉、乳制品和作物三个子领域，对9个访谈进行信息抽取，比较神经符号系统和LLM系统的性能

Result: LLM系统总体表现优于神经符号系统（F1总分：69.4 vs 52.7；核心信息：63.0 vs 47.2），但各有优缺点
...
TL;DR: 提出了Curative Prompt Refinement (CPR)框架，通过清理不良提示和生成额外任务描述来减少LLM幻觉问题，显著提高生成质量。


<details>
  <summary>Details</summary>
Motivation: LLM有时会生成看似合理但错误的"幻觉"事实，这通常是由于用户使用结构不良或模糊的提示导致的，使得模型基于假设而非实际意图生成响应。

Method: 开发了CPR框架，使用微调的小型语言模型来清理不良提示并生成额外的信息性任务描述，以对齐用户意图和提示内容。

Result: 应用CPR后，语言模型的生成质量显著提高，同时有效缓解了幻觉问题。实证研究显示，使用CPR的提示在90%以上的情况下优于原始提示。

Conclusion: CPR是一个即插即用的提示优化框架，无需外部知识就能有效减少LLM幻觉，提高响应质量。
...
TL;DR: 提出了多阶段提示精炼框架MPR，通过系统化改进格式不良的提示来减少大语言模型的幻觉问题，在幻觉基准测试中精炼后的提示胜率超过85%


<details>
  <summary>Details</summary>
Motivation: 大语言模型在幻觉问题上仍面临挑战，特别是格式不良提示（歧义表述、语法错误、信息不完整）的影响尚未充分研究

Method: MPR框架通过多个阶段系统改进提示：每个阶段针对特定错误（标点、拼写错误、关键词误用），使用微调的小语言模型，通过迭代增强提示清晰度和自反思机制来优先处理最相关输入

Result: 在幻觉基准测试中，MPR精炼的提示相比原始形式胜率超过85%，能有效减少幻觉并提高输出准确性，且可与现有后处理幻觉缓解框架结合

Conclusion: MPR提供了一个轻量级、适应性强的解决方案，可在各领域增强大语言模型的可靠性
...
<details>
  <summary>Details</summary>
Motivation: 研究人类标签变异如何影响模型公平性，这是一个尚未被充分探索的领域。

Method: 通过比较在多数投票标签上训练与各种HLV训练方法，分析它们对模型公平性的影响。

Result: 实验表明，在没有显式去偏的情况下，HLV训练方法对公平性产生积极影响。

Conclusion: 人类标签变异训练方法可以在不依赖显式去偏技术的情况下提升模型公平性。
...
TL;DR: 该论文综述了大语言模型不确定性量化在幻觉检测中的应用，系统分类了现有方法并分析了当前局限性和未来研究方向。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型在现实应用中的部署，其产生的幻觉问题引发了可靠性和可信度担忧，需要不确定性量化方法来评估模型生成的可信度。

Method: 论文首先介绍不确定性量化的理论基础，包括认知不确定性和偶然不确定性的区分，然后系统分类现有方法，并提供代表性方法的实证结果。

Result: 论文展示了不确定性量化在检测大语言模型幻觉方面的有效性，能够识别不可靠的生成内容并提高模型可靠性。

Conclusion: 不确定性量化为解决大语言模型幻觉问题提供了有前景的研究方向，但仍存在局限性，需要进一步探索和发展。
...
TL;DR: 提出基于大语言模型的提示词重写框架，通过奖励系统和迭代DPO训练优化文本到图像生成的输入提示，提升图像质量和对齐度
...
Method: 使用大语言模型作为提示词重写器，采用精心设计的奖励系统和迭代直接偏好优化训练流程，无需监督微调数据

Result: 在多个T2I模型和基准测试中，提示词重写器持续改善了图像-文本对齐、视觉质量和美学效果，优于强基线方法，且具有很好的跨模型迁移性
...
TL;DR: 提出了分层对齐方法，针对Transformer模型的不同功能层进行定向DPO优化，避免了传统对齐方法的"对齐税"问题，在保持语法流畅性的同时显著提升逻辑一致性和事实准确性。


<details>
  <summary>Details</summary>
Motivation: 传统对齐方法将模型视为单一实体进行统一优化，忽视了Transformer架构中不同层处理不同任务的功能特化，导致优化效果受限。

Method: 引入分层对齐方法，将模型层划分为局部（语法）、中间（逻辑）和全局（事实性）三个功能块，使用LoRA进行定向微调，分别应用DPO优化。

Result: 局部层对齐提升语法流畅性，全局层对齐不仅改善事实一致性，还是提升逻辑一致性的最有效策略，所有分层策略都成功避免了标准DPO中的"对齐税"。
...
TL;DR: APCE提出了一种上下文感知的输入块选择方法，通过低维语义相似性匹配来选择最重要的输入块，在长文本摘要任务中实现了内存效率提升和性能保持。


<details>
  <summary>Details</summary>
Motivation: 解决长上下文Transformer模型的两个关键挑战：(1) 内存占用随序列长度增加而增长（二次自注意力和线性KV缓存）；(2) ContextRot现象，即模型性能随上下文长度增加而下降。

Method: APCE方法通过低维语义相似性匹配，选择与当前查询最相关的输入块进行处理，从而减少需要处理的输入序列长度。

Result: 在长文本摘要任务中，APCE仅使用50%-70%的输入序列就能达到与完整密集基线相当或更优的性能，显著改善了KV缓存和自注意力的内存效率。

Conclusion: APCE为长上下文Transformer模型提供了一种上下文感知的效率解决方案，有望扩展到其他相关长上下文任务。
...
TL;DR: Verily行为健康安全过滤器在精神健康危机检测中表现出色，相比NVIDIA NeMo和OpenAI Omni Moderation，在敏感性和特异性方面均有显著优势，特别注重减少漏检危机。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在处理精神科紧急情况时经常提供有害或不适当的建议，助长破坏性行为，需要开发有效的安全过滤器来保护用户。

Method: 使用Verily精神健康危机数据集（1,800条模拟消息）和NVIDIA Aegis AI内容安全数据集子集（794条精神健康相关消息），与NVIDIA NeMo和OpenAI Omni Moderation进行对比评估。

Result: VBHSF在Verily数据集上敏感性0.990、特异性0.992、F1分数0.939；在NVIDIA数据集上敏感性0.982、准确性0.921、特异性0.859。在所有情况下敏感性均显著高于对比方法。

Conclusion: VBHSF表现出稳健、可泛化的性能，优先考虑敏感性以最小化漏检危机，这对于医疗应用至关重要。
...
TL;DR: 提出PACE方法，通过让LLMs生成平行关联链来评估其创造力，该方法能有效避免数据污染问题，且与人类评估结果高度相关。
...
Method: PACE方法要求LLMs生成平行关联链，通过分析这些链来评估模型的创造力，避免数据污染并提供高效评估。

Result: PACE与Chatbot Arena创意写作排名高度相关（Spearman's ρ=0.739），高性能LLMs达到普通人水平，但专业人士仍优于LLMs；语言分析显示人类和LLMs关联具体性都下降，但人类关联模式更多样。

Conclusion: PACE是评估LLMs创造力的有效方法，虽然高性能LLMs接近普通人创造力水平，但人类专业人士仍保持优势，且人类关联模式更具多样性。
...
Result: 实验表明，即使在高质量双语语料下，跨语言知识转移仍然具有挑战性。

Conclusion: 跨语言知识转移机制需要进一步研究，当前方法在多语言领域适应中仍存在局限性。
...
TL;DR: 本文系统分析了端到端大语音语言模型中存在的模态差距问题，发现语音和文本输入在深层表示中方向对齐但幅度分离，并提出基于对齐路径评分和关键令牌干预的改进方法。


<details>
  <summary>Details</summary>
Motivation: 尽管端到端大语音语言模型在对话生成方面表现出色，但在语义理解基准测试中始终落后于传统流水线系统，需要深入理解其中的模态差距问题。

Method: 通过系统实验分析语音和文本的粗粒度和细粒度表示，引入对齐路径评分量化令牌级对齐质量，并设计角度投影和长度归一化等干预策略。

Result: 发现语音和文本表示在深层网络中方向相似度增加但欧氏距离扩大，表示相似度与模态差距强相关，提出的干预策略能提升语音输入的正确性。
...
<details>
  <summary>Details</summary>
Motivation: 随着多模态大语言模型的广泛使用，安全问题日益突出。多轮对话在日常交互中更常见，但现有基准测试未能充分考虑这种情况，存在更大的安全风险。

Method: 构建了包含10,000个样本的SafeMT基准测试，涵盖17种场景和4种越狱方法，并提出了安全指数(SI)来评估对话安全性。同时提出了对话安全调节器来检测对话中的恶意意图。

Result: 评估了17个模型，发现随着有害对话轮数增加，成功攻击的风险显著上升，表明现有模型的安全机制不足以识别对话交互中的危险。实验表明提出的调节器比现有防护模型更能有效降低多轮攻击成功率。

Conclusion: 多轮对话对MLLMs构成严重安全威胁，需要专门的安全机制来检测和防范对话中的恶意意图，提出的对话安全调节器是有效的解决方案。
...
TL;DR: 提出Credal Transformer，用基于证据理论的Credal注意力机制替代标准注意力，通过生成分布集合而非单一分布来量化不确定性，从而减少LLM的幻觉问题。


<details>
  <summary>Details</summary>
Motivation: 解决LLM产生事实错误但自信断言的问题，认为这源于Transformer的Softmax函数丢弃了不确定性信息，导致'人工确定性'。

Method: 引入Credal注意力机制，将注意力分数重新概念化为Dirichlet分布的证据质量，产生'credal set'（分布集合）来直接测量模型不确定性。

Result: Credal Transformer能够识别分布外输入、量化模糊性，并在不可回答问题上通过弃权显著减少自信错误。

Conclusion: 提供了一种新的架构来缓解幻觉问题，并将不确定性量化直接集成到模型中，为更可靠的AI奠定了基础。
...
TL;DR: 这篇论文对并行推理这一新兴的LLM推理范式进行了系统性综述，包括其定义、分类、技术方法、应用场景以及未来挑战。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型能力增强，并行推理作为一种新的推理范式出现，能够通过同时探索多条思路来增强推理的鲁棒性，克服传统顺序方法的脆弱性。

Method: 首先给出并行推理的形式化定义，澄清其与相关概念的区别；然后基于新颖的分类法组织讨论先进技术，包括非交互式推理、交互式推理和效率导向的解码策略；最后探索各种应用场景。

Result: 提出了并行推理的完整分类框架，涵盖了当前主要的技术方法和应用领域，为初学者提供了有用的路线图。

Conclusion: 并行推理是提升LLM推理鲁棒性的重要方向，论文系统总结了该领域的进展和挑战，为未来研究指明了潜在方向。
...
TL;DR: 该论文研究了将离散空间中的推理时间缩放技术（多样本生成结合PRM/ORM重排序）应用于连续空间推理的可行性，发现虽然能生成多样化的推理路径，但由于连续思维表示缺乏关键归纳偏置，导致PRM/ORM模型难以有效区分正确和错误的推理。


<details>
  <summary>Details</summary>
Motivation: 探索已证明在离散空间文本推理中有效的推理时间缩放技术（多样本生成结合PRM/ORM重排序）是否能够成功适应连续空间推理。

Method: 使用COCONUT连续空间推理LM作为骨干，通过dropout-based采样生成多样化推理路径，并分析Pass@N性能。通过探究几何属性和轨迹动态等方面，识别阻碍PRM/ORM有效区分正确和错误推理的根本原因。

Result: 生成多样化推理路径是可行的，Pass@N分析显示存在显著的性能提升潜力，类似于离散空间中的增益。但离散空间中的数据生成和PRM/ORM训练方法在连续空间中只能带来边际改进，因为连续思维表示缺乏关键归纳偏置，阻碍了正确和错误推理的有效区分。
...
TL;DR: LLaDR是一个基于大语言模型的药物重定位框架，通过从LLMs提取生物医学实体的语义丰富文本表示来增强知识图谱嵌入，在药物重定位任务中达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法忽视了真实实验室中的常识性生物医学概念知识，如某些药物与特定治疗根本不相容的机制先验知识，导致生物医学概念表示不足。

Method: 从大语言模型提取生物医学实体的语义丰富治疗相关文本表示，并用这些表示来微调知识图谱嵌入模型，将治疗相关知识注入KGE中。

Result: 在基准测试中，LLaDR在不同场景下均达到最先进性能，阿尔茨海默病的案例研究进一步证实了其鲁棒性和有效性。
...
TL;DR: 该研究首次系统性地揭示了大型音频语言模型在时间戳预测中存在的时间偏差问题，即模型预测的事件发生时间往往比实际时间提前或延后，且这种偏差会随音频长度增加而累积。


<details>
  <summary>Details</summary>
Motivation: 虽然大型音频语言模型在音频理解和多模态推理中应用日益广泛，但其定位事件发生时间的能力尚未得到充分研究。研究者希望系统探索模型在时间戳预测中的潜在偏差问题。

Method: 通过在带时间戳的数据集上进行受控实验，研究者量化了时间偏差现象，提出了时间偏差指数(TBI)来衡量预测事件时间与真实时间的系统性错位，并开发了可视化框架。

Result: 研究发现：(i)时间偏差在不同数据集和模型中普遍存在；(ii)偏差随音频长度增加而累积，在长录音中可达数十秒；(iii)偏差程度因事件类型和位置而异。

Conclusion: 研究揭示了当前大型音频语言模型在时间定位方面的根本局限性，呼吁开发具有时间鲁棒性的架构来解决这一问题。
...
TL;DR: 提出基于LLM和DPO的语音翻译分割框架，通过偏好对齐预测更自然的分割点，在多个语言对上超越SHAS模型，提升翻译质量和延迟表现。


<details>
  <summary>Details</summary>
Motivation: 现有分割模型如SHAS虽然比启发式方法更稳健，但仍受限于监督学习目标，缺乏人类偏好对齐，而这对实时自然翻译至关重要。

Method: 使用基于大语言模型的分割框架，通过直接偏好优化训练，使LLM能够预测更符合实时翻译需求的自然分割点。

Result: 在ACL 60/60语料库上评估，DPO调优的LLM在英日、英中、英德三个语言对上比SHAS获得更高分割准确率，翻译质量和延迟均有提升。
...
TL;DR: HALF框架通过将九个应用领域分为三个危害等级（严重、中等、轻度），在真实场景中评估LLM偏见并考虑危害严重性，发现LLM在不同领域公平性不一致，模型大小或性能不能保证公平性。


<details>
  <summary>Details</summary>
Motivation: 现有LLM偏见评估缺乏真实场景基础，且未考虑不同领域危害严重性的差异（如手术决策偏见与文本摘要风格偏见的权重不同）。

Method: 开发HALF（Harm-Aware LLM Fairness）框架，使用五阶段流程将九个应用领域组织成三个危害等级（严重、中等、轻度），在真实应用场景中评估模型偏见并按危害严重性加权结果。

Result: 对八个LLM的评估显示：(1) LLM在不同领域的公平性不一致；(2) 模型大小或性能不能保证公平性；(3) 推理模型在医疗决策支持中表现更好但在教育领域表现更差。

Conclusion: HALF揭示了先前基准测试成功与部署准备度之间的明显差距，强调需要基于危害严重性的真实场景公平性评估。
...
TL;DR: 研究发现LLMs在微调过程中会习得Knobe效应这种道德偏见，通过层修补分析可以定位到特定层，仅需修补这些关键层即可消除偏见，无需重新训练模型。


<details>
  <summary>Details</summary>
Motivation: 探究LLMs在微调过程中如何内化人类偏见，特别是Knobe效应这种道德判断偏见，并理解其具体表现机制。

Method: 在3个开源权重的LLMs上进行层修补分析，通过将预训练模型的激活值修补到微调模型的关键层来研究偏见定位。

Result: Knobe效应不仅能在微调过程中被学习，而且被定位到特定层组。仅需修补少数关键层的激活值就能完全消除该偏见效应。
...
TL;DR: 提出了DSAS（双阶段自适应锐化）方法，通过CGW和RAS两个模块解决LLMs在多文档问答中的长距离依赖建模和"中间迷失"问题，无需架构修改或额外训练参数。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在多文档问答任务中存在两个主要限制：长距离依赖建模困难（难以关注长文本中的关键信息）和"中间迷失"问题（难以处理长输入中间的信息）。现有解决方案要么截断全局依赖，要么需要昂贵的微调，缺乏通用简单的解决方案。

Method: DSAS包含两个模块：(1) CGW模块通过层级注意力跟踪和位置感知加权评估段落相关性，缓解"中间迷失"问题；(2) RAS模块通过抑制关键文本与无关文本之间的信息交换，增强对关键段落的关注，改善长距离依赖建模。这是一个即插即用的解决方案。

Result: 在四个基准测试上的广泛实验表明，DSAS在主流LLMs（Llama、Qwen、Mistral、Deepseek）上均有效，在Llama-3.1-8B-Instruct和Qwen2.5-14B-Instruct上，多文档问答任务的F1分数平均提高了4.2%。消融研究确认了CGW和RAS模块的必要贡献。

Conclusion: DSAS提供了一种有效且通用的解决方案，能够显著提升LLMs在多文档问答任务中的性能，同时保持即插即用的便利性，无需模型架构修改或额外训练参数。
...
TL;DR: 论文提出了MedQA-Followup框架，用于评估医学问答中多轮对话的鲁棒性，发现LLMs在多轮设置下存在严重脆弱性，准确性可能从91.2%降至13.5%。


<details>
  <summary>Details</summary>
Motivation: 现有评估框架通常只评估单轮问答，忽略了医学咨询中常见的冲突输入、误导性上下文和权威影响等复杂性，而LLMs在真实多轮交互中的可靠性尚未得到充分理解。

Method: 引入MedQA-Followup框架，区分浅层鲁棒性（抵抗误导性初始上下文）和深层鲁棒性（在答案被多轮挑战时保持准确性），同时引入间接-直接轴来区分上下文框架和明确建议。

Result: 评估五个最先进的LLMs发现，虽然模型在浅层扰动下表现良好，但在多轮设置中表现出严重脆弱性，Claude Sonnet 4的准确性从91.2%降至13.5%。反直觉的是，基于上下文的间接干预往往比直接建议更有害。

Conclusion: 多轮鲁棒性是医学LLMs安全可靠部署的关键但尚未充分探索的维度，间接上下文干预暴露了临床部署的重大脆弱性。
...
TL;DR: Chinese ModernBERT是一个从头开始训练的中文编码器，通过优化词汇表、训练策略和系统设计，在中文NLP任务上实现了准确率、速度和内存效率的帕累托改进。


<details>
  <summary>Details</summary>
Motivation: 现有的Transformer改进主要针对英语，未能充分迁移到中文，因为中文在分词和词法上与英语有显著差异。需要专门为中文设计优化的编码器模型。

Method: 采用硬件感知的32k BPE词汇表、动态全词掩码训练、两阶段预训练（从1024扩展到8192上下文长度）、阻尼余弦学习率调度，并在1.2T中文token上预训练。

Result: 在CLUE基准测试中与强中文编码器竞争，在长序列处理上保持高吞吐量，在SimCLUE测试集上达到0.505（Pearson）/0.537（Spearman）的相关性分数，超越了Qwen-0.6B-embedding。

Conclusion: Chinese ModernBERT展示了为中文专门设计模型的优势，提供了可复现的研究基础，并指出了通过增加高质量对比数据来扩展语义文本相似性任务的清晰路径。
...
TL;DR: 提出了一种基于大语言模型的无监督自动化语法标注流程，用于大规模语料库的标注，在COHA语料库中实现了98%以上的准确率。


<details>
  <summary>Details</summary>
Motivation: 解决语料库快速扩张时人工标注成为方法瓶颈的问题，探索利用LLMs实现规模化自动标注的可能性。

Method: 采用四阶段工作流程：提示工程、事前评估、自动批量处理、事后验证，使用GPT-5通过OpenAI API进行标注。

Result: 在60小时内标注了COHA语料库中的143,933个句子，在两个复杂标注任务中达到98%以上的准确率。

Conclusion: LLMs能够以最小人工干预完成大规模数据准备任务，为基于语料库的研究开辟新可能，但需关注成本、许可和伦理问题。
...
<details>
  <summary>Details</summary>
Motivation: 现有反言论生成方法存在可靠性不足和可扩展性差的问题，需要结合权威知识来提升生成内容的质量和可信度。

Method: 构建包含32,792个文本的知识库，集成RAG流程，将反言论生成建模为知识驱动的文本生成过程。

Result: 在MultiTarget-CONAN数据集上的评估显示，该框架在自动评估和人工评估中均优于标准LLM基线方法。
...
Method: 引入细粒度输入归因方法，识别对大脑-LLM对齐最重要的特定词语，比较BA和NWP所依赖的词语子集。

Result: 发现BA和NWP依赖于很大程度上不同的词语子集：NWP表现出近因和首因偏差，关注语法；而BA优先考虑语义和语篇层面信息，具有更有针对性的近因效应。
...
TL;DR: MoBiLE是一个基于专家混合模型的即插即用推理框架，通过大-小专家混合策略，在不重要的token上使用一半专家来加速推理，同时在重要token上保持完整专家以保证模型质量。


<details>
  <summary>Details</summary>
Motivation: 现有的MoE模型推理方法受限于CPU-GPU互联带宽，预取方法训练开销大且在细粒度专家分割的现代MoE模型中效果不佳。

Method: 提出大-小专家混合策略，为不重要token减少专家数量，设计专用的回退和预取机制在大小专家间切换以提高内存效率。

Result: 在四种典型现代MoE架构和生成任务上，MoBiLE在消费级GPU系统上相比基线实现了1.60x到1.72x的加速，精度损失可忽略不计。

Conclusion: MoBiLE框架有效解决了MoE模型推理中的带宽瓶颈问题，实现了显著加速且保持模型质量。
...
TL;DR: 研究发现LLM在学术评审中存在系统性偏见：偏向LLM生成的论文，对包含批判性陈述的人类论文评分偏低，揭示了在学术评审中使用LLM的风险和公平性问题。


<details>
  <summary>Details</summary>
Motivation: 随着LLM在学术工作流程中的深度整合，研究人员需要了解LLM在研究和评审双重角色中可能带来的新风险，特别是对学术公平性的影响。

Method: 通过模拟实验，构建研究代理（生成和修改论文）和评审代理（评估提交论文），并基于模拟结果进行人工标注分析。

Result: 发现LLM评审与人类判断存在显著偏差：LLM评审系统性地给LLM撰写的论文打高分，而对包含批判性陈述的人类论文持续低估评分。

Conclusion: LLM在学术评审中存在语言特征偏见和批判性陈述厌恶，如果未经充分谨慎就部署在同行评审中，会对人类作者和学术研究构成风险和公平性问题；但LLM指导的修改确实能提升论文质量。
...
TL;DR: 本研究对200多种语言进行大规模跨语言标记化效率评估，发现拉丁文字语言标记化效率高，而非拉丁文字和形态复杂语言的标记化成本显著更高（通常高3-5倍），导致计算成本增加和有效上下文利用率降低。


<details>
  <summary>Details</summary>
Motivation: 标记化差异对实现跨语言人口公平获取人工智能构成重大障碍，需要系统量化大型语言模型中的计算不平等问题。

Method: 使用标准化实验框架，通过tiktoken库对所有语言样本进行统一标记化处理，收集标记化统计数据，使用TPS和RTC等评估指标，并以英语为基准进行对比。

Result: 拉丁文字语言标记化效率高，非拉丁文字和形态复杂语言标记化膨胀显著，RTC比率通常高3-5倍，导致计算成本增加和有效上下文利用率降低。
...
TL;DR: PRoH是一个动态规划和推理框架，通过上下文感知规划、结构化问题分解和实体加权重叠检索算法，解决了现有知识超图检索方法的局限性，在多跳问答任务中实现了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 现有基于知识超图的检索增强生成方法存在三个主要限制：静态检索规划、非自适应检索执行以及对知识超图结构和语义的浅层使用，这些限制了它们进行有效多跳问答的能力。

Method: PRoH包含三个核心创新：(1)上下文感知规划模块，通过勾勒局部知识超图邻域来指导结构化推理计划生成；(2)结构化问题分解过程，将子问题组织为动态演化的有向无环图，实现自适应多轨迹探索；(3)实体加权重叠引导的推理路径检索算法，优先考虑语义连贯的超边遍历。

Result: 在多个领域的实验中，PRoH实现了最先进的性能，平均F1分数比之前的SOTA模型HyperGraphRAG提高了19.73%，生成评估分数提高了8.41%，同时在长距离多跳推理任务中保持了强大的鲁棒性。

Conclusion: PRoH框架通过动态规划和结构化推理，有效克服了现有知识超图检索方法的局限性，在多跳问答任务中表现出卓越的性能和鲁棒性。
...
Conclusion: CLEAR通过深入理解LLM内部知识整合机制，有效解决了RAG系统的忠实性问题，为改进知识冲突下的证据整合提供了新思路。
...
TL;DR: 该研究通过多语言Wug测试评估大语言模型在形态学泛化任务中的表现，发现模型能像人类一样处理未见词汇，但其准确性更多受训练数据量而非语言结构复杂度影响。


<details>
  <summary>Details</summary>
Motivation: 探讨大语言模型的语言能力是否接近人类水平，以及模型表现主要受语言复杂性还是训练数据量的影响。

Method: 使用多语言Wug测试，在加泰罗尼亚语、英语、希腊语和西班牙语四种语言上测试六个模型，并与人类表现进行比较。

Result: 模型能够以类似人类的准确度对未见词汇进行形态学泛化，但准确性模式更接近社区规模和数据可用性，而非结构复杂性。西班牙语和英语等资源丰富的语言表现更好。
...
TL;DR: 提出SMEC框架，通过序列嵌套表示学习、自适应维度选择和可选择性跨批次记忆模块，在保持性能的同时显著降低LLM嵌入的维度。


<details>
  <summary>Details</summary>
Motivation: LLM生成的高维嵌入虽然包含丰富语义信息，但加剧了计算复杂度和存储需求，阻碍实际部署。

Method: 使用SMRL方法减少训练中的梯度方差，ADS模块降低维度剪枝时的信息退化，S-XBM模块增强高低维嵌入间的无监督学习。

Result: 在图像、文本和多模态数据集上实验表明，SMEC在显著降维的同时保持性能。在BEIR数据集上，256维压缩嵌入性能比Matryoshka-Adaptor和Search-Adaptor分别提升1.1和2.7个点。

Conclusion: SMEC框架有效解决了LLM高维嵌入的计算和存储问题，为实际部署提供了可行方案。
...
TL;DR: 该论文提出了首个个性化机器生成文本检测基准，揭示了检测器在个性化设置中的性能下降问题，并提出了一种预测检测器性能变化的方法。
...
Result: 实验结果显示在个性化设置中，一些最先进的检测器性能显著下降，MIMIC方法能够准确预测性能变化的方向和幅度，与实际性能差距的相关性达到85%。

Conclusion: 该工作揭示了检测器在个性化文本检测中的局限性，提出的方法能够有效预测性能变化，为个性化文本检测的进一步研究提供了基础。
...
TL;DR: 将测试时缩放技术从数学领域转移到LeWiDi-2025任务，评估了三种方法在标注分歧任务上的表现。


<details>
  <summary>Details</summary>
Motivation: 测试时缩放技术此前仅限于数学和编程等有明确正确答案的领域，本研究旨在探索其在LeWiDi标注分歧任务上的适用性。

Method: 使用三种测试时缩放方法：模型平均、多数投票和Best-of-N采样，在LeWiDi-2025任务上进行实验。

Result: 模型平均和多数投票方法在LeWiDi任务上持续提升LLM性能，但Best-of-N方法未能有效转移。

Conclusion: Best-of-N方法目前无法从数学领域成功转移到LeWiDi任务，需要分析造成这种差距的潜在原因。
...
TL;DR: VLMs在评估非典型实例时，会在语义先验（概念表示通常为真）和语用先验（输入相关）之间产生冲突，导致概念理解在输入不匹配时退化。


<details>
  <summary>Details</summary>
Motivation: 研究VLMs在处理典型和非典型实例时，如何权衡语义先验和语用先验之间的冲突。

Method: 引入VISaGE评估数据集，包含典型和异常图像，通过精心平衡的实验设计进行分析。

Result: 当语用先验的输入一致性假设被违反时，概念理解会退化，这种效应比语义先验在查询单个实例时更强。

Conclusion: VLMs在处理非典型实例时更依赖语用先验，输入不一致会显著影响概念理解性能。
...
TL;DR: 提出Faithful Uncertainty Tuning (FUT)方法，通过微调让LLM忠实表达不确定性，而不改变答案分布。


<details>
  <summary>Details</summary>
Motivation: LLM经常错误传达其不确定性：重复查询会产生不同答案，但生成的回答通常没有或很少包含反映这种变异性的不确定性表述，这造成了忠实性差距。

Method: 通过增强模型样本与不确定性修饰语（如'可能'或'很可能'）对齐来构建训练数据，无需额外监督，只需模型和一组提示。

Result: FUT显著减少了忠实性差距，同时保持了QA准确性并引入最小的语义分布偏移。

Conclusion: FUT是教导LLM忠实传达不确定性的一种简单有效方法。
...
TL;DR: 提出了StyleDecipher框架，通过结合离散风格指标和连续风格表示来检测LLM生成的文本，在多个领域实现了最先进的检测精度和跨领域泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有方法在检测机器生成文本时面临泛化能力有限、易受改写攻击、缺乏可解释性等问题，特别是在面对风格多样性和人机混合创作时表现不佳。

Method: 使用组合特征提取器量化风格差异，联合建模离散风格指标和基于语义嵌入的连续风格表示，在统一表示空间中捕捉人类和LLM输出的风格级差异。

Result: 在五个不同领域的实验中，StyleDecipher在域内检测中达到最先进精度，在跨域评估中比现有基线提升高达36.30%，并对对抗扰动和混合内容保持鲁棒性。
...
TL;DR: ACADATA是一个高质量的学术翻译平行数据集，包含150万段落对的训练集和6000个翻译的评估集，涵盖96个语言方向。通过在ACAD-TRAIN上微调LLM，在学术翻译质量上显著提升，超越了专有和开源模型。
...
Method: 构建ACADATA数据集（ACAD-TRAIN训练集和ACAD-BENCH评估集），在两个大语言模型上微调，并与专业机器翻译系统、通用LLM和专有模型进行基准测试。

Result: 微调后7B和2B模型在学术翻译质量上分别平均提升6.1和12.4 d-BLEU点，从英语翻译时在通用领域长文本翻译提升达24.9%，最佳模型在学术翻译领域超越了所有专有和开源模型。
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型对提示设计高度敏感，需要优化的提示技术来生成一致、高质量的输出。现有COSTAR框架在大型LLMs上表现良好，但在小型本地优化模型上性能不一致。

Method: 引入COSTAR-A框架，在COSTAR（上下文、目标、风格、语调、受众、响应）基础上增加'答案'组件。通过一系列受控的提示-输出评估，在不超过80亿参数的小型微调模型上测试。
...
TL;DR: 本文提出PARO框架，通过让LLM学习任务特定的推理模式来自动生成推理轨迹，从而大幅减少对人类标注的依赖，在模式化推理任务中达到与10倍规模人工标注相当的性能。


<details>
  <summary>Details</summary>
Motivation: 现有的SFT+RLVR范式依赖昂贵的人工标注推理轨迹，本文旨在研究如何在不大幅降低推理性能的前提下显著减少标注成本。

Method: 提出PARO框架，识别模式化推理任务中的固定推理模式，让LLM基于这些模式自动生成对齐的推理轨迹，无需人工标注。

Result: 实验表明PARO生成的推理轨迹在SFT+RLVR训练中能达到与10倍规模人工标注相当的性能。

Conclusion: 在模式化推理任务中，大规模人工标注可以被仅需有限人类监督的LLM自动标注所替代，推理模式而非标注数量是性能的关键决定因素。
...
TL;DR: 论文提出生成空间大小(GSS)概念来解决LLM在创意任务中输出过于同质化和在事实任务中产生多样化但不正确回答的问题，并开发了GSSBench评估套件。


<details>
  <summary>Details</summary>
Motivation: 当前LLM在校准方面存在问题：在创意任务中输出过于同质化，在事实任务中产生多样化但不正确的回答。这两种失败模式可以通过生成空间大小(GSS)概念统一解决。

Method: 提出GSS概念和GSSBench评估套件，使用EigenScore等幻觉检测指标来评估模型内部任务表示，仅利用模型内部信息。

Result: 发现幻觉检测指标特别是EigenScore在评估GSS方面优于标准多样性和不确定性量化指标，能够提供模型内部任务表示的可解释性见解。
...
<details>
  <summary>Details</summary>
Motivation: 当前全模态语言模型在捕捉和描述细粒度细节方面的能力仍有局限，且存在细节与幻觉共生的内在问题，需要系统性的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 扩展现有研究，从两个角度改进：使用广义范畴语法覆盖更多自然语言结构，以及更关注语言模型对未见长句的泛化能力。

Method: 采用广义范畴语法构建人工语言，覆盖无界依赖和轻度上下文敏感结构，评估语言模型对未见长句的处理能力。

Result: 实验表明，类型学上合理的词序更容易让语言模型进行生产性泛化。

Conclusion: 语言模型确实具有偏好类型学合理语法结构的归纳偏置，这种偏置在更接近自然语言的设置下依然存在。
...
TL;DR: 提出DGRC方法，利用"at-issueness"概念评估语言模型对话的自然度，发现模型偏好继续讨论核心议题内容，指令调优模型此倾向更强，但会因对话线索调整偏好。
...
Result: 语言模型偏好继续讨论核心议题内容，指令调优模型此倾向更强；当存在相关线索时，模型会减少对核心议题的偏好；指令调优不会进一步放大这种调节。

Conclusion: DGRC方法能减轻语言模型分析中的偏见，系统测试话语敏感行为；模型对核心议题的偏好调节反映了成功对话动态的特征。
...
TL;DR: 该论文主张从索绪尔和乔姆斯基的理论框架转向马恩扎克的实证主义语言学观点，认为语言是使用频率驱动的现象，并以此为基础重新评估和指导语言模型的设计与评估。


<details>
  <summary>Details</summary>
Motivation: 当前基于索绪尔和乔姆斯基理论对LLMs的语言学批评往往流于推测且缺乏建设性，作者认为需要采用更实证的语言学框架来理解语言模型的本质。

Method: 采用波兰语言学家Witold Mańczak的实证主义语言学理论，将语言定义为所有口头和书面表达的总和，强调使用频率是语言的主要支配原则。

Result: 基于马恩扎克的理论框架，成功挑战了先前对LLMs的批评，并为语言模型的设计、评估和解释提供了建设性指导。

Conclusion: 实证主义语言学视角为理解语言模型提供了更富有成效的框架，使用频率原则应成为评估LLMs语言能力的核心标准。
...
TL;DR: Dr.LLM是一个动态路由框架，通过轻量级路由器决定跳过、执行或重复Transformer层，在保持或提高精度的同时节省计算量。


<details>
  <summary>Details</summary>
Motivation: 传统LLM对所有token使用相同层数，导致简单查询计算浪费，复杂查询推理深度不足。现有自适应深度方法依赖昂贵推理搜索、架构修改或大规模重训练，且往往牺牲精度。

Method: 使用蒙特卡洛树搜索(MCTS)生成高质量层配置作为监督信号，训练轻量级路由器。采用窗口池化稳定路由、焦点损失与类别平衡、瓶颈MLP路由器等技术处理类别不平衡和长序列。

Result: 在ARC和DART任务上精度提升达+3.4%，平均每样本节省5层。在跨域任务上仅损失0.85%精度，同时保持效率，比现有路由方法提升达+7.7%。
...
TL;DR: 研究发现，为低资源语言创建高质量语音数据集需要大量人工劳动，在实验室条件下平均需要30小时人工来准确转录1小时语音，在实地条件下需要36小时。


<details>
  <summary>Details</summary>
Motivation: 研究低资源语言（特别是低识字率的口头语言）创建语音数据集的实际人工成本，为类似语言创建NLP资源提供基准和实用见解。

Method: 通过为期一个月的实地研究，让10名母语转录员修正ASR生成的班巴拉语语音转录，分析53小时语音数据的转录过程。

Result: 在实验室条件下，准确转录1小时语音平均需要30小时人工劳动；在实地条件下需要36小时人工劳动。

Conclusion: 为低资源语言创建高质量语音数据集需要大量人工投入，研究为类似语言创建NLP资源提供了成本基准和实用指导。
...
### [151] [Aixel: A Unified, Adaptive and Extensible System for AI-powered Data Analysis](https://arxiv.org/abs/2510.12642)
*Meihui Zhang,Liming Wang,Chi Zhang,Zhaojing Luo*

Main category: cs.DB

TL;DR: Aixel是一个统一、自适应、可扩展的AI驱动数据分析系统，通过四层架构（应用、任务、模型、数据）解决现有系统在数据集成、学习优化和资源管理方面的碎片化问题。


<details>
  <summary>Details</summary>
Motivation: 现代数据分析需要集成多源异构数据，同时满足准确性、延迟和成本要求。现有系统在数据库、分析库和调优服务之间碎片化，导致用户交互复杂、适应性有限、性能不佳和组件扩展性差。

Method: 采用四层架构：任务层提供声明式接口捕获用户意图并生成可执行操作计划；模型层提供版本化存储支持自适应构建和任务对齐的漂移检测；数据层提供统一数据管理能力；优化器编译调度计划以满足目标要求。

Result: Aixel实现了用户友好、自适应、高效和可扩展的系统，通过内置的重用和缓存机制提高效率，支持任务对齐的数据发现和特征管理。

Conclusion: Aixel通过统一的多层架构设计，有效解决了现代数据分析中的碎片化问题，提供了更好的用户体验、适应性和性能表现。
...
### [152] [AI Agents for the Dhumbal Card Game: A Comparative Study](https://arxiv.org/abs/2510.11736)
*Sahaj Raj Malla*

Main category: cs.AI

TL;DR: 本研究系统评估了AI代理在Dhumbal文化纸牌游戏中的表现，比较了基于规则、搜索和学习的策略，发现基于规则的激进代理在1024轮模拟中获得了88.3%的最高胜率。


<details>
  <summary>Details</summary>
Motivation: 评估AI代理在具有文化意义的不完全信息多人纸牌游戏Dhumbal中的表现，为AI研究和文化游戏的数字保存提供支持。

Method: 形式化Dhumbal游戏机制，实现多样化AI代理（启发式、搜索方法如MCTS和ISMCTS、强化学习如DQN和PPO），通过类别内锦标赛和跨类别锦标赛进行评估。

Result: 基于规则的激进代理获得最高胜率88.3%，显著优于ISMCTS（9.0%）和PPO（1.5%），通过有效利用Jhyap声明取得成功。
...
TL;DR: LLM作为评估者存在严重的正向偏见，本文提出了少数否决策略和基于回归的框架来缓解这种偏见，在代码反馈任务上实现了2倍的性能提升。


<details>
  <summary>Details</summary>
Motivation: 现有LLM评估方法存在系统性正向偏见，虽然能准确识别有效输出，但对无效输出的识别能力很差，导致可靠性评分被夸大。

Method: 提出了最优少数否决策略和基于回归的框架，利用少量人工标注数据直接建模验证器偏见。

Result: 在366个高中Python程序的代码反馈任务中，回归方法将最大绝对误差降至1.2%，比14个最先进LLM的最佳集成性能提升2倍。

Conclusion: 提出的方法能有效缓解LLM评估者的正向偏见，显著提高评估准确性。
...
TL;DR: 提出了Holistic Agent Leaderboard (HAL)来解决AI代理评估中的挑战，包括标准化评估框架、三维分析和LLM辅助日志检查，旨在推动从基准测试表现转向真实世界可靠性的研究。


<details>
  <summary>Details</summary>
Motivation: AI代理评估存在诸多挑战，影响了对代理实际性能的理解，需要更全面可靠的评估方法。

Method: 1) 标准化评估框架，在数百个VM上并行评估；2) 三维分析涵盖模型、支架和基准；3) LLM辅助日志检查发现未报告行为。

Result: 进行了21,730次代理运行，覆盖9个模型和9个基准，发现意外洞察如更高推理努力反而降低准确性，并发现代理的异常行为模式。

Conclusion: 通过标准化评估方法和解决常见陷阱，HAL有助于将研究重点从基准测试表现转向真实世界可靠性。
...
TL;DR: CGBench是一个用于评估语言模型在科学文献解释能力的基准测试，基于临床遗传学专家标注数据构建，测试模型在提取实验结果、评估证据强度和分类实验成果方面的表现。


<details>
  <summary>Details</summary>
Motivation: 传统基因变异解释方法耗时耗力，需要自动化工具来加速临床转化研究。现有基准测试过于狭窄，无法反映真实研究需求。

Method: 基于ClinGen专家标注的临床遗传学文献构建CGBench基准，测试8种不同语言模型在三个核心任务上的表现：精确提取实验结果、评估证据强度、分类描述实验结果。

Result: 模型在文献解释方面显示出潜力但存在显著差距，推理模型在细粒度任务上表现更好，非推理模型在高层解释上更优。模型经常产生幻觉或误解结果。

Conclusion: CGBench揭示了语言模型在科学文献精确解释方面的优缺点，为AI在临床遗传学和更广泛科学领域的应用开辟了新途径。
...
TL;DR: 提出一种训练LLMs生成顺序澄清问题的方法，通过前向过程添加用户画像的“噪声”，反向过程训练模型通过提问来“去噪”，从而有效获取用户偏好。
...
Method: 采用两阶段方法：前向过程生成澄清问题并逐步移除答案来添加“噪声”；反向过程训练模型通过提问来“去噪”用户画像。

Result: 该方法显著提高了LLM提出漏斗式问题和有效获取用户偏好的能力。

Conclusion: 该方法为训练LLMs生成有效顺序澄清问题提供了一种新颖的解决方案，能够更好地获取用户偏好信息。
...
TL;DR: CausalTrace是一个集成到工业CoPilot中的神经符号因果分析模块，通过数据驱动因果分析和工业知识图谱，提供因果发现、反事实推理和根因分析功能，在火箭装配测试中表现出与领域专家高度一致的解释能力和强大的RCA性能。


<details>
  <summary>Details</summary>
Motivation: 现代制造环境不仅需要准确预测，还需要可解释的异常分析、根因识别和干预建议。现有AI系统往往是孤立的黑盒，缺乏预测、解释和因果推理的无缝集成，限制了在高风险工业环境中的可信度和实用性。

Method: CausalTrace是一个神经符号因果分析模块，集成到SmartPilot工业CoPilot中，通过数据驱动因果分析并结合工业本体和知识图谱，支持因果发现、反事实推理和根因分析，提供实时操作员交互和透明可解释的决策支持。

Result: 在学术火箭装配测试中，CausalTrace与领域专家达成高度一致（本体问答ROUGE-1：0.91），RCA性能强劲（MAP@3：94%，PR@2：97%，MRR：0.92，Jaccard：0.92），在C3AN评估中获得4.59/5分，展示了实时部署的精确性和可靠性。

Conclusion: CausalTrace成功地将神经符号因果分析集成到工业CoPilot中，提供了透明、可解释的决策支持，在高风险制造环境中展现出强大的实用价值和可信度，为工业AI系统的发展提供了重要参考。
...
TL;DR: PACT是一个评估LLM生成代码契约遵循性的框架，通过扩展HumanEval+和MBPP+基准，提供系统性评估代码在功能正确性和契约遵循性方面的能力。


<details>
  <summary>Details</summary>
Motivation: 现有代码生成基准主要评估功能正确性，忽略了真实软件中契约遵循（处理异常输入）的重要性，导致模型无法生成真正健壮的代码。

Method: PACT提供包含契约违反测试用例的测试套件，支持在不同提示条件下分析代码生成，并引入新指标量化契约遵循性。

Result: 研究表明，在提示中加入契约违反测试用例比仅使用契约描述更能提升模型对契约的遵循能力。

Conclusion: PACT通过揭示传统基准忽略的关键错误，为评估LLM生成代码在功能和契约遵循方面的健壮性提供了严谨且可解释的指标。
...
TL;DR: 提出了一种地理空间感知层(GAL)，通过整合地理数据增强LLM在灾害响应中的能力，使其能够基于结构化地球数据生成资源分配建议。


<details>
  <summary>Details</summary>
Motivation: 现有统计方法缺乏语义上下文，跨事件泛化能力差，可解释性有限。LLM虽然具有少样本泛化能力，但受限于文本且缺乏地理感知。

Method: 开发GAL层，从原始野火检测出发，自动检索并整合基础设施、人口统计、地形和天气信息，生成带单位标注的感知脚本，为LLM提供丰富的地理上下文。

Result: 在真实野火场景中评估，地理空间接地的智能体表现优于基线模型，能够生成基于证据的资源分配建议。

Conclusion: 该框架可推广到洪水、飓风等其他灾害类型，有效提升了灾害响应的智能决策能力。
...
TL;DR: ThinkPilot是一个无需训练即可自动优化大型推理模型（LRMs）推理过程的框架，通过进化过程生成think-prefixes来引导模型实现更好的性能表现。
...
Result: 实验表明ThinkPilot能显著改善准确率-长度权衡，大幅提升安全性（如将DeepSeek-R1-Distill-Qwen-32B的StrongREJECT分数从27.0%降至0.7），增强指令跟随能力，并能与现有基于训练的方法协同工作。

Conclusion: think-prefixes能可靠控制LRMs的推理行为，不同任务对特定行为分布有强烈偏好。ThinkPilot通过自动识别和激发这些行为，提供了一个可推广的框架来使LRMs推理与任务需求对齐。
...
TL;DR: 本文重新诠释了AI智能体学习的角色，将其视为具有计算能力的随机动力系统，强调时间在学习推理中的基础作用，提出从归纳学习转向转导学习，并论证信息的关键作用是减少时间而非重建误差。


<details>
  <summary>Details</summary>
Motivation: 探索AI推理智能体是否能成为通用计算器，以及如何通过学习来提升推理能力，质疑模型大小或训练数据量是否是关键因素。

Method: 将AI智能体重新解释为计算能力的随机动力系统，提出转导学习框架，分析算法信息与最优加速的关系，推导推理时间与训练时间的幂律缩放关系。

Result: 理论推导显示通用求解器使用过去数据能达到的最优加速与其算法信息紧密相关，大模型在无限空间和时间下可能表现为无洞察力的专家，仅能暴力求解任务。
...
TL;DR: HiCoTraj是一个利用大语言模型进行零样本人口属性推理的框架，通过将轨迹数据转换为语义丰富的自然语言表示，并使用分层思维链推理来推断年龄、性别、收入等人口属性，无需标注的训练数据。


<details>
  <summary>Details</summary>
Motivation: 现有基于移动轨迹的人口属性推断方法严重依赖带有人口标签的大规模轨迹数据，导致可解释性有限且在不同数据集和用户群体间泛化能力差。需要解决标注数据稀缺的问题。

Method: 将轨迹转换为语义丰富的自然语言表示，包括详细活动记录和多尺度访问摘要。使用分层思维链推理，通过三个认知阶段：事实特征提取、行为模式分析和结构化输出的人口属性推断。

Result: 在真实世界轨迹数据上的实验评估表明，HiCoTraj在零样本场景下对多个人口属性实现了有竞争力的性能。

Conclusion: HiCoTraj框架通过利用LLM的零样本学习和语义理解能力，解决了标注人口数据稀缺的挑战，同时提供了透明的推理链，在人口属性推断方面表现出色。
...
TL;DR: 提出了EmboMatrix训练场概念，通过大规模任务模拟和交互式学习，让LLM获得真正的具身决策能力，并训练出EmboBrain模型，在具身决策基准上超越了671B参数的DeepSeek-R1基线。


<details>
  <summary>Details</summary>
Motivation: 解决LLM仅基于语言训练而缺乏物理环境暴露的问题，使其能够真正理解具身决策，将高级目标转化为可执行动作。

Method: 构建EmboMatrix训练场基础设施，包含多智能体数据引擎生成大规模任务场景、分布式异构硬件系统进行可扩展模拟、多级奖励架构提供精确监督。

Result: 训练的EmboBrain-7B模型在两个具身决策基准上比671B的DeepSeek-R1基线提升了9.5%，证明了交互式环境学习的力量。

Conclusion: EmboMatrix训练场能够有效培养LLM的具身决策能力，通过环境交互学习可以构建真正智能的具身智能体。
...
### [164] [BeSTAD: Behavior-Aware Spatio-Temporal Anomaly Detection for Human Mobility Data](https://arxiv.org/abs/2510.12076)
...
Result: 能够检测行为转变和偏离常规的细微偏差，并识别大规模移动数据集中表现出此类变化的个体。
...
### [165] [Evaluating the Quality of Randomness and Entropy in Tasks Supported by Large Language Models](https://arxiv.org/abs/2510.12080)
...
Result: LLM能够生成具有一定随机性的输出，但性能不一致，且经常显著偏离预期行为。
...
### [166] [One Life to Learn: Inferring Symbolic World Models for Stochastic Environments from Unguided Exploration](https://arxiv.org/abs/2510.12088)
*Zaid Khan,Archiki Prasad,Elias Stengel-Eskin,Jaemin Cho,Mohit Bansal*

Main category: cs.AI

TL;DR: OneLife框架通过条件激活的程序化法则在概率编程框架中建模世界动态，能够在复杂、随机环境中从有限的、无指导的交互中学习关键环境动态。
...
### [167] [ToPolyAgent: AI Agents for Coarse-Grained Topological Polymer Simulations](https://arxiv.org/abs/2510.12091)
*Lijie Ding,Jan-Michael Carrillo,Changwoo Do*

Main category: cs.AI

TL;DR: ToPolyAgent是一个多智能体AI框架，通过自然语言指令执行拓扑聚合物的粗粒度分子动力学模拟，集成了大语言模型与领域专用计算工具，支持线型、环状、刷状、星型聚合物及树枝状聚合物的交互式和自主模拟工作流。


<details>
  <summary>Details</summary>
Motivation: 通过将自然语言界面与严格的模拟工具相结合，ToPolyAgent降低了复杂计算工作流的门槛，推动了聚合物科学中AI驱动的材料发现，为自主和可扩展的多智能体科学研究生态系统奠定了基础。

Method: 系统包含四个LLM驱动的智能体：配置智能体生成初始聚合物-溶剂配置，模拟智能体执行基于LAMMPS的MD模拟和构象分析，报告智能体编译markdown报告，工作流智能体用于简化的自主操作。支持交互式（包含用户反馈循环）和自主式（端到端任务执行）两种模式。

Result: 通过案例研究展示了ToPolyAgent在不同聚合物架构、溶剂条件、恒温器和模拟时长下的多功能性，并作为研究助手研究了相互作用参数对线型聚合物构象的影响，以及接枝密度对刷状聚合物持久长度的影响。

Conclusion: ToPolyAgent成功地将自然语言界面与分子动力学模拟工具集成，为聚合物科学中的AI驱动研究提供了有效的多智能体框架，具有降低技术门槛和促进材料发现的潜力。
...
### [168] [Precise Attribute Intensity Control in Large Language Models via Targeted Representation Editing](https://arxiv.org/abs/2510.12121)
*Rongzhi Zhang,Liqin Ye,Yuzhao Heng,Xiang Chen,Tong Yu,Lingkai Kong,Sudheer Chava,Chao Zhang*

Main category: cs.AI

TL;DR: 提出了一种精确控制LLM输出属性强度的方法，通过目标导向训练和梯度干预实现用户定义的属性强度控制。


<details>
  <summary>Details</summary>
Motivation: 现有LLM对齐方法只能提供方向性指导，无法可靠实现精确的属性强度控制，需要更精细的控制机制。

Method: 将属性强度控制重新定义为目标达成问题，训练轻量级价值函数预测属性强度，并通过梯度干预隐藏表示来精确导航模型。

Result: 在LLaMA-3.2-3b和Phi-4-mini上的实验证实该方法能高精度地将文本生成引导至用户指定的属性强度。

Conclusion: 该方法实现了对属性强度的细粒度连续控制，并在偏好数据合成、帕累托前沿优化和对齐行为蒸馏等下游任务中提升了效率。
...
### [169] [MatSciBench: Benchmarking the Reasoning Ability of Large Language Models in Materials Science](https://arxiv.org/abs/2510.12171)
...
TL;DR: 提出了MatSciBench，一个包含1340个问题的材料科学基准测试，评估LLMs在材料科学领域的推理能力。


<details>
  <summary>Details</summary>
Motivation: 填补LLMs在材料科学推理能力研究方面的空白，建立全面的评估基准。

Method: 构建了包含6个主要领域和31个子领域的结构化分类体系，采用三级难度分类，包含多模态推理问题，并评估了多种推理策略。

Result: 即使是表现最佳的Gemini-2.5-Pro模型，在大学生水平的材料科学问题上准确率也低于80%，表明基准的复杂性。
...
### [170] [Evolution of meta's llama models and parameter-efficient fine-tuning of large language models: a survey](https://arxiv.org/abs/2510.12178)
...
TL;DR: 这篇综述论文系统回顾了Meta AI的LLaMA系列模型（从LLaMA 1到LLaMA 4）的快速演进，以及为这些模型开发的参数高效微调（PEFT）方法，包括模型架构、性能特征和各种PEFT技术的机制与应用。


<details>
  <summary>Details</summary>
Motivation: 随着LLaMA系列模型的快速发展，需要系统梳理这些基础模型及其高效微调方法，为研究者和实践者提供一站式资源，帮助他们理解模型演进、PEFT技术原理及其在实际应用中的表现。

Method: 论文首先描述LLaMA系列基础模型（7B-65B到288B参数）的架构和性能特征，然后详细介绍了五种应用于LLaMA的PEFT方法：LoRA、LLaMA-Adapter V1和V2、LLaMA-Excitor以及QLoRA，分析每种方法的机制、参数节省和应用案例。

Result: 论文提供了模型和适配器架构、参数数量和基准测试结果的系统分析，展示了微调后的LLaMA模型在某些情况下能够超越更大基线模型的性能，并探讨了在医疗、法律等实际领域的成功应用案例。

Conclusion: LLaMA模型与PEFT方法的结合为高效模型适配提供了有效途径，论文最后讨论了持续面临的挑战（如扩展到更大上下文、提高鲁棒性）和未来研究方向，为相关领域的研究者和实践者提供了重要参考。
...
### [171] [ResearStudio: A Human-Intervenable Framework for Building Controllable Deep-Research Agents](https://arxiv.org/abs/2510.12194)
*Linyi Yang,Yixuan Weng*

Main category: cs.AI

TL;DR: ResearStudio是一个开源框架，通过"协作工作坊"设计实现实时人类控制的研究代理系统，支持用户在AI主导、人类辅助和人类主导、AI辅助模式间无缝切换。
...
### [172] [On the Design and Evaluation of Human-centered Explainable AI Systems: A Systematic Review and Taxonomy](https://arxiv.org/abs/2510.12201)
*Aline Mangold,Juliane Zietz,Susanne Weinhold,Sebastian Pannasch*

Main category: cs.AI

TL;DR: 该论文对65个评估可解释AI系统的用户研究进行了全面综述，提出了以人为中心的XAI系统设计目标和评估框架，并根据用户AI专业水平（AI新手和数据专家）进行了差异化设计。
...
Method: 对65个XAI用户研究进行系统性综述分析，提出XAI系统特性和以人为中心的评估指标分类，并针对不同AI专业水平的用户提出差异化设计目标。

Result: 发现XAI系统由核心系统和解释组件构成；评估指标可分为情感、认知、可用性、可解释性和解释指标；针对AI新手强调负责任使用、接受度和可用性，针对数据专家强调性能导向、人机协作和任务表现。

Conclusion: 提出了扩展的XAI评估和设计框架，强调需要根据用户特征定制XAI系统设计，为XAI开发者提供了以人为中心的指导原则。
...
### [173] [GOAT: A Training Framework for Goal-Oriented Agent with Tools](https://arxiv.org/abs/2510.12218)
*Hyunji Min,Sangwon Jung,Junyoung Sung,Dosung Lee,Leekyeung Han,Paul Hongsuck Seo*

Main category: cs.AI

TL;DR: GOAT是一个无需人工标注的训练框架，通过从API文档自动构建合成数据集来微调LLM代理，使其能够处理目标导向的API执行任务。
...
### [174] [MedKGEval: A Knowledge Graph-Based Multi-Turn Evaluation Framework for Open-Ended Patient Interactions with Clinical LLMs](https://arxiv.org/abs/2510.12224)
...
<details>
  <summary>Details</summary>
Motivation: 现有LLM医疗评估方法依赖事后对话转录分析，无法捕捉临床环境中多轮医患互动的动态性和上下文敏感性。

Method: 使用知识图谱驱动的患者模拟机制，构建控制模块从知识图谱检索医学事实；采用实时轮级评估框架，由法官代理评估模型响应的临床适当性、事实准确性和安全性；整合开源资源和专家标注数据构建知识图谱。

Result: 对8个最先进LLM的全面多轮基准测试表明，MedKGEval能够识别传统评估管道常忽略的细微行为缺陷和安全风险。

Conclusion: MedKGEval为临床LLM提供了更可靠的评估框架，虽然最初针对中英文医疗应用设计，但可通过切换知识图谱轻松扩展到其他语言。
...
### [175] [PromptFlow: Training Prompts Like Neural Networks](https://arxiv.org/abs/2510.12246)
*Jingyi Wang,Hongyuan Zhu,Ye Niu,Yunhui Deng*

Main category: cs.AI

TL;DR: 提出了PromptFlow框架，这是一个受TensorFlow启发的模块化训练框架，通过元提示、操作符、优化器和评估器实现自动化的提示工程，解决了传统提示工程依赖人工、缺乏动态策略选择和经验复用的问题。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在跨领域部署时需要领域特定的适应策略，而手动提示工程既费时又依赖专业知识。现有的自动化提示工程方法存在静态更新规则、缺乏动态策略选择、无法细粒度编辑提示部分以及经验复用不足等问题。

Method: 设计了PromptFlow框架，包含元提示、操作符、优化器和评估器模块。采用基于梯度的元学习自主探索最优提示优化轨迹，并设计了强化学习方法在提示工程过程中复用LLM的经验。

Result: 在多个数据集上进行了广泛实验，证明了PromptFlow框架的有效性。

Conclusion: PromptFlow框架能够以最少的任务特定训练数据实现自动化的提示优化，解决了当前提示工程方法的局限性，为LLM的领域适应提供了有效的解决方案。
...
### [176] [$\mathbf{T^3}$: Reducing Belief Deviation in Reinforcement Learning for Active Reasoning](https://arxiv.org/abs/2510.12264)
*Deyu Zou,Yongqiang Chen,Jianxiang Wang,Haochen Yang,Mufei Li,James Cheng,Pan Li,Yu Gong*

Main category: cs.AI

TL;DR: 提出T³方法，通过检测和截断信念偏差过大的轨迹来改进LLM主动推理训练，提高训练稳定性和性能


<details>
  <summary>Details</summary>
Motivation: LLM智能体在主动推理中容易发生信念偏差，导致问题状态跟踪失败和重复无意义行动，这会破坏强化学习训练效果

Method: 开发T³方法，跟踪模型信念偏差，检测过度偏差并截断训练轨迹，保留信息性前缀的信用分配

Result: 在5个挑战性任务中，T³持续提升训练稳定性、令牌效率和最终性能，性能提升达30%，同时减少约25%的令牌使用
...
TL;DR: 提出张量逻辑语言，统一神经和符号AI，通过张量方程实现逻辑规则与爱因斯坦求和的统一，支持变换器、形式推理、核机器和图模型等AI形式。
...
Method: 构建张量逻辑语言，其唯一构造是张量方程，将逻辑规则与爱因斯坦求和视为相同操作，并以此为基础实现各种AI形式。

Result: 成功实现了神经、符号和统计AI的关键形式，包括变换器、形式推理、核机器和图模型，并支持嵌入空间中的可靠推理。
...
Result: 在具有挑战性的多模态基准测试中表现出卓越性能，相比最先进方法有显著改进，特别是在传统方法失效的长文档上性能提升尤为明显。
...
<details>
  <summary>Details</summary>
Motivation: 解决大语言模型在数学研究中验证困难的问题，证明虽然看起来合理但缺乏严格验证，旨在将AI从竞赛数学扩展到研究级数学工具。
...
TL;DR: 这篇论文首次系统性地综述了基于大型语言模型的Vibe Coding范式，建立了理论框架并分析了五种开发模型，揭示了成功Vibe Coding的关键因素。


<details>
  <summary>Details</summary>
Motivation: 随着LLMs的发展，从代码生成辅助转向自主编码代理，催生了"Vibe Coding"新范式，但其有效性尚未得到充分探索，需要建立系统性的理论框架和实践指南。

Method: 通过分析1000多篇研究论文，建立约束马尔可夫决策过程理论框架，将现有实践综合为五种开发模型：无约束自动化、迭代对话协作、规划驱动、测试驱动和上下文增强模型。

Result: 成功Vibe Coding不仅依赖代理能力，更需要系统性的上下文工程、完善的开发环境和人机协作开发模型。

Conclusion: Vibe Coding代表了软件开发的新范式，需要建立完整的理论框架和实践体系来充分发挥其潜力，人机协作模型和上下文工程是成功的关键因素。
...
TL;DR: PricingLogic是首个评估大语言模型在旅游定价任务中可靠性的基准测试，包含300个基于真实定价政策的问题，测试显示LLMs在复杂定价场景中存在系统性失败。


<details>
  <summary>Details</summary>
Motivation: 旅游机构希望将易出错的定价任务自动化给AI系统，但在未经验证可靠性的情况下部署LLMs可能导致重大财务损失和客户信任流失。
...
Conclusion: 尽管具备通用能力，但当前的LLMs在没有进一步保障措施或领域适应的情况下，在收入关键应用中仍然不可靠。
...
TL;DR: 提出了MTOS框架，将多主题上下文与LLMs结合，模拟社交网络中的意见演化，解决了现有研究在跨领域多主题认知转移方面的局限性。


<details>
  <summary>Details</summary>
Motivation: 现实社交网络中信息常涉及多个相关主题，现有基于LLM的研究多关注单一主题，无法捕捉多主题跨领域中的认知转移；传统数值模型则过度简化语言态度为离散值，缺乏解释性和行为一致性。

Method: MTOS框架整合LLMs与短期和长期记忆，采用多种用户选择交互机制和动态主题选择策略，并引入信念衰减机制实现跨主题视角更新。

Result: 多主题设置显著改变极化趋势：正相关主题放大回音室效应，负相关主题抑制回音室，无关主题通过资源竞争也能缓解回音室效应。相比数值模型，LLM智能体能更真实地模拟动态意见变化，再现新闻文本的语言特征，捕捉复杂人类推理。

Conclusion: MTOS框架通过多主题上下文与LLMs的结合，提升了社交意见模拟的解释性和系统稳定性，能够更准确地捕捉多主题环境中的认知转移和意见演化动态。
...
<details>
  <summary>Details</summary>
Motivation: 随着LLM在通信系统中被用于自主评估内容质量，需要确保这些AI"评判者"的公正性，因为任何评估标准中的偏见都可能扭曲结果并损害用户信任。

Method: 系统调查了两种LLM评判模型（GPT-Judge和JudgeLM）在点式评分设置下的11种偏见类型，涵盖隐性和显性形式，并分析了评分标准、微调策略和任务难度的影响。

Result: 发现最先进的LLM评判者对偏见输入具有鲁棒性，通常给它们分配比相应干净样本更低的分数；提供详细评分标准能进一步增强这种鲁棒性；在偏见响应上微调LLM会显著降低其性能；评判分数与任务难度相关。
...
TL;DR: 开发了一个基于有向无环图的任务导向医疗对话框架，包含问题转换、冷启动、自适应分支、终止逻辑和报告生成等功能，在初步评估中表现出良好的用户体验和临床工作流程集成。


<details>
  <summary>Details</summary>
Motivation: 为医疗领域开发一个高效的任务导向对话系统，能够将医疗算法和指南转化为结构化的临床问题，支持自适应访谈流程，并生成符合临床工作流程的结构化报告。

Method: 采用有向无环图结构组织医疗问题，包含五个核心组件：医疗指南转换管道、基于层次聚类的冷启动机制、扩展-剪枝自适应分支机制、终止逻辑以及自动化报告生成。

Result: 患者应用获得低认知负荷(15.6)、高可用性(86)和强满意度(8.1/9)；医生应用获得中等认知负荷(26)、优秀可用性(88.5)和高满意度(8.3/9)，有效集成到临床工作流程中。

Conclusion: 该医疗对话框架在初步评估中表现出良好的性能和用户体验，能够降低认知需求并支持高效报告生成，但存在系统延迟和小样本评估的局限性。
...
TL;DR: 提出细胞状态潜在(CSL)视角，通过操作符语法组织学习，强调决策对齐的评估框架，以解决当前AI虚拟细胞模型在跨实验室、跨平台迁移性、数据泄露和跨尺度耦合方面的挑战。


<details>
  <summary>Details</summary>
Motivation: 当前AI虚拟细胞模型虽然取得了进展，但在跨实验室和平台的可迁移性、数据泄露、剂量时间效应处理以及跨尺度耦合方面存在局限，需要更系统化的评估和学习框架。

Method: 提出模型无关的细胞状态潜在(CSL)视角，采用操作符语法组织学习过程：测量、跨尺度耦合的提升/投影、以及剂量和调度的干预。

Result: 构建了决策对齐的评估蓝图，涵盖模态、尺度、上下文和干预，强调功能空间读出如通路活性、空间邻域和临床相关终点。

Conclusion: 建议采用操作符感知的数据设计、防泄露分区以及透明的校准和报告，以实现可重复的同类比较，推动AI虚拟细胞模型的可靠发展。
...
Result: 在酒店评论基准数据集和两个公共基准测试（二分类和多分类）上的实验表明，ProtoSiTex实现了最先进的性能，同时提供了忠实的人类对齐解释。
...
TL;DR: 提出基于包容性适应度的多智能体强化学习框架，通过基因型分配和遗传共享模拟自然选择，在囚徒困境网络游戏中观察到符合汉密尔顿法则的社会动态。


<details>
  <summary>Details</summary>
Motivation: 受自然选择中竞争与合作力量驱动智力演化的启发，旨在创建能模拟生物进化社会动态的多智能体系统。

Method: 为每个智能体分配基因型，基于包容性适应度设计奖励函数，允许基因共享，在囚徒困境网络游戏中测试社会动态。

Result: 实验结果与生物学原理（如汉密尔顿法则）一致，展示了基于基因相似度的合作谱系和独特的非团队社会动态。

Conclusion: 基于包容性适应度的奖励机制为更高级战略和社会智能智能体的涌现提供了基础，可扩展到更开放的环境模拟进化军备竞赛。
...
<details>
  <summary>Details</summary>
Motivation: 现有基准主要关注9x9数独等流行谜题，容易导致模型过拟合规范格式和记忆解题模式，无法真正测试模型理解新规则或适应变体的能力。

Method: 通过三个维度系统化转换规范谜题：增加复杂性(IC)、罕见元素(UE)和不可解谜题(UP)，减少对捷径记忆的依赖。

Result: 评估显示即使在其他基准上表现优秀的模型也出现显著性能下降，表明它们严重依赖记忆的刻板模式。增加复杂性是主要困难来源，但模型也在不增加难度的细微规则变化上表现不佳。
...
TL;DR: 提出了Memory-as-Action框架，将工作内存管理重新定义为可学习的内部能力，通过强化学习联合优化任务推理和内存管理，解决了LLM在长视野任务中内存受限的问题。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在长视野智能体任务中面临内存限制，现有工作内存方法通常依赖与核心策略解耦的外部启发式机制，这限制了性能。

Method: 提出Memory-as-Action框架，智能体通过执行显式编辑操作来主动管理工作内存；开发Dynamic Context Policy Optimization算法，通过分割轨迹和应用轨迹级优势来解决轨迹断裂问题。

Result: 端到端联合优化任务推理和内存管理不仅减少了计算消耗，还提高了任务性能，通过适应模型内在能力的自适应上下文管理策略实现。

Conclusion: 将内存管理重新定义为可学习的内部能力，通过统一的策略框架实现端到端优化，为LLM在长视野任务中的内存挑战提供了有效解决方案。
...
TL;DR: ERA是一个两阶段框架，通过先验知识学习和在线强化学习，让小规模视觉语言模型在具身AI任务中超越大型模型
...
Method: 两阶段框架：1）具身先验学习，从轨迹增强、环境锚定和外部知识三种数据中蒸馏知识；2）在线强化学习，采用自总结、密集奖励塑造和回合级策略优化

Result: ERA-3B在EB-ALFRED任务上提升8.4%，在EB-Manipulation任务上提升19.4%，超越了GPT-4o和之前的基线方法
...
TL;DR: 提出了一个多智能体辩论评判框架，通过智能体协作推理和迭代优化回答，提高了LLM作为评判者的准确性，同时通过稳定性检测机制保持计算效率。


<details>
  <summary>Details</summary>
Motivation: 当前LLM作为评判者的方法依赖简单聚合（如多数投票），即使个体智能体提供正确答案也可能失败，需要更有效的协作评判机制。

Method: 多智能体辩论评判框架，智能体协作推理和迭代优化回答；引入稳定性检测机制，通过时变Beta-Binomial混合模型建模评判者共识动态，基于分布相似性（K-S检验）的自适应停止准则。

Result: 在多个基准测试和模型上的实验表明，该框架相比多数投票提高了评判准确性，同时保持了计算效率。

Conclusion: 辩论过程能够放大正确性，多智能体辩论框架为自动化评判任务提供了更可靠和高效的解决方案。
...
TL;DR: 该论文研究了利用车辆间通信数据（CAM）进行车辆轨迹预测的方法，开发了CAMNet神经网络模型，并在真实数据集上验证了CAM数据在轨迹预测中的有效性。


<details>
  <summary>Details</summary>
Motivation: 解决自动驾驶中传感器视野受限的问题，通过车辆间通信共享信息来提高环境感知能力，特别是在传感器被遮挡的情况下。

Method: 设计并训练了基于协同感知消息的图神经网络（CAMNet），在一个广泛使用的运动预测数据集上进行训练，并在专门创建的CAM数据集上进行评估。

Result: 方法显示出有希望的结果，证明CAM数据确实可以支持车辆轨迹预测，但也发现了一些局限性。

Conclusion: CAM数据在车辆轨迹预测中具有应用潜力，但该方法仍存在局限性，为未来研究提供了改进方向。
...
TL;DR: 提出了一种结合自监督学习和图论技术的方法，无需标记数据即可改进OOD检测，提高AI系统鲁棒性，在AUROC指标上达到0.99。


<details>
  <summary>Details</summary>
Motivation: 在自动驾驶、交通、医疗等安全关键系统中，AI系统需要在各种条件下保持可靠性能，包括处理OOD样本、对抗攻击和环境变化，系统故障可能带来严重后果。

Method: 利用自监督学习原理从无标记数据中学习有用表示，结合图论技术更有效地识别和分类OOD样本。

Result: 与现有最先进方法相比，该方法在AUROC指标上达到0.99。

Conclusion: 所提出的方法能够显著提高AI系统的鲁棒性，特别是在处理OOD样本方面表现出色，且无需依赖标记数据。
...
TL;DR: CLUTCH是一种基于深度组合多臂老虎机的新型JavaScript模糊测试方法，通过注意力机制和Concrete Dropout动态选择变异目标，显著提高了测试效率和覆盖率。


<details>
  <summary>Details</summary>
Motivation: 现有JavaScript模糊测试方法使用随机选择变异目标，效率不高。作者认为选择更好的变异目标适合使用具有可变臂数的组合多臂老虎机来解决。

Method: 提出CLUTCH方法：使用注意力机制处理可变长度的JavaScript测试用例表示，通过Concrete Dropout动态调整探索策略，基于深度组合多臂老虎机选择变异目标。

Result: 与三种最先进解决方案相比，CLUTCH将有效测试用例数量平均提高20.3%，每个测试用例的覆盖率平均提高8.9%。在可变和组合设置中，后悔值分别减少至少78.1%和4.1%。

Conclusion: CLUTCH在JavaScript模糊测试中显著提高了效率，证明了深度组合多臂老虎机在选择变异目标方面的有效性。
...
### [196] [CTRL-Rec: Controlling Recommender Systems With Natural Language](https://arxiv.org/abs/2510.12742)
*Micah Carroll,Adeline Foote,Kevin Feng,Marcus Williams,Anca Dragan,W. Bradley Knox,Smitha Milli*

Main category: cs.AI

TL;DR: CTRL-Rec是一个允许用户通过自然语言实时控制推荐系统的方法，使用LLM模拟用户对推荐项目的批准判断，训练嵌入模型来近似这些判断，并将其集成到传统推荐系统中。


<details>
  <summary>Details</summary>
Motivation: 当用户对推荐系统不满意时，他们通常缺乏细粒度的控制来改变推荐结果。LLM提供了通过自然语言请求来指导推荐的解决方案。

Method: 在训练时使用LLM模拟用户基于语言请求对项目的批准判断，训练嵌入模型来近似这些模拟判断，然后将这些基于用户请求的预测集成到传统推荐系统的标准信号加权中。

Result: 在MovieLens数据集上的实验表明，该方法能够在各种请求上实现细粒度控制。在19名Letterboxd用户的研究中，CTRL-Rec受到用户积极评价，与传统控制相比显著增强了用户的控制感和满意度。
...
### [197] [Ax-Prover: A Deep Reasoning Agentic Framework for Theorem Proving in Mathematics and Quantum Physics](https://arxiv.org/abs/2510.12787)
...
TL;DR: Ax-Prover是一个基于多智能体系统的自动定理证明器，能在Lean中解决跨科学领域的问题，支持自主或人机协作模式。
...
Method: 通过Model Context Protocol为大型语言模型配备Lean工具，结合LLM的知识推理能力和形式化验证工具的正确性保障。

Result: 在公开数学基准测试中与最先进证明器竞争，在新引入的抽象代数和量子理论基准上显著优于其他方法。

Conclusion: 基于工具的智能体定理证明方法为跨领域形式化验证提供了可泛化的通用方法，并成功辅助专家数学家完成复杂密码学定理的形式化证明。
