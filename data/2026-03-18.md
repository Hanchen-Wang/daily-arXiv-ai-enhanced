...
Motivation: NeRF在体积渲染时需要进行密集光线采样，导致计算效率低下。传统方法使用固定或启发式采样策略，难以适应不同场景特性

Method: 提出基于Soft Actor-Critic的强化学习框架，将采样建模为马尔可夫决策过程。包含三个技术组件：高斯混合分布颜色模型提供不确定性估计；多组件奖励函数平衡质量、效率和一致性；两阶段训练策略解决环境非平稳性问题

Result: 在Synthetic-NeRF和LLFF数据集上，SAC-NeRF减少35-48%的采样点，同时保持渲染质量在密集采样基线的0.3-0.8 dB PSNR范围内

Conclusion: 虽然学习到的策略是场景特定的且RL框架比简单启发式方法更复杂，但这项工作表明数据驱动的采样策略能够发现难以手动设计的有效模式
...
TL;DR: 评估多种视觉语言模型在盲人和低视力人群导航任务中的表现，发现GPT-4o在空间推理和场景理解方面表现最佳，开源模型在复杂环境中存在局限，但VLMs在导航辅助方面具有潜力。


<details>
  <summary>Details</summary>
Motivation: 研究视觉语言模型（VLMs）在帮助盲人和低视力人群（pBLV）进行导航任务方面的潜力，评估现有模型在基础视觉技能和实际导航场景中的表现，为辅助技术开发提供指导。

Method: 评估了闭源模型（GPT-4V、GPT-4o、Gemini-1.5-Pro、Claude-3.5-Sonnet）和开源模型（Llava-v1.6-mistral、Llava-onevision-qwen），测试了三个基础视觉技能：环境障碍物计数、相对空间推理、常识性寻路相关场景理解，并使用pBLV特定提示模拟实际辅助任务。

Result: GPT-4o在所有任务中表现最佳，特别是在空间推理和场景理解方面；开源模型在复杂环境中存在推理和适应性困难；常见挑战包括：杂乱环境中物体计数不准确、空间推理偏差、过度关注物体细节而忽视空间反馈。

Conclusion: 虽然当前VLMs存在局限性，但在与人类反馈更好对齐并改进空间推理能力后，在寻路辅助方面具有潜力。研究为开发者提供了将VLMs有效集成到辅助技术中的可行见解，同时指出了需要解决的关键限制以提升可用性。
...
TL;DR: RegGAN通过回归层学习局部表情细节，结合对抗训练提升生成质量，在表情合成任务中优于现有方法，尤其在分布外数据上表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有条件GAN在表情合成中表现良好，但当测试图像与训练数据分布不同时性能下降。需要提高模型在分布外数据上的泛化能力。

Method: 提出RegGAN模型，包含两个组件：1) 具有局部感受野的回归层，通过岭回归损失最小化重建误差学习表情细节；2) 对抗训练的细化网络，提升生成图像的真实感。

Result: 在CFEE数据集和分布外图像（名人照片、肖像、雕像、虚拟形象）上评估，RegGAN在ECS、FID和QualiCLIP三个指标上优于6个SOTA模型，FSS排名第二。人工评估显示在表情质量、身份保持和真实感方面分别领先25%、26%和30%。

Conclusion: RegGAN通过结合回归学习和对抗训练，显著提升了表情合成的泛化能力和生成质量，特别是在分布外数据上表现出色。
...
TL;DR: OrthoAI v2 是用于隐形矫治器治疗规划的AI辅助开源管道的第二个版本，通过多智能体架构、复合评分模型和治疗模拟器显著提升了规划质量和功能。


<details>
  <summary>Details</summary>
Motivation: 第一版OrthoAI虽然基于动态图卷积神经网络实现了牙齿分割的概念验证，但存在三个主要限制：仅提取牙齿质心、缺乏地标级精度、只产生标量质量评分且无分期模拟。v2旨在解决这些限制，提供更精确、全面的治疗规划能力。

Method: 1. 引入第二个智能体采用条件热图回归方法进行直接、无需分割的牙科地标检测，与第一个智能体通过置信度加权协调器融合（并行、顺序、单智能体三种模式）
2. 开发复合六类别生物力学评分模型（生物力学×0.30 + 分期×0.20 + 附件×0.15 + IPR×0.10 + 咬合×0.10 + 可预测性×0.15）
3. 构建多帧治疗模拟器，通过SLERP插值和循证分期规则生成时间一致的6自由度牙齿轨迹，实现ClinCheck 4D可视化

Result: 在200个拥挤场景的合成基准测试中，OrthoAI v2并行集成达到92.8±4.1的规划质量评分，相比v1的76.4±8.3提升了21%的相对增益，同时保持完全CPU可部署性（4.2±0.8秒）。

Conclusion: OrthoAI v2通过多智能体架构、复合评分模型和治疗模拟器，显著提升了隐形矫治器治疗规划的精度和全面性，为临床医生提供了更强大的AI辅助工具。
...
TL;DR: 提出CLRNet，一种多模态端到端深度学习标定网络，用于相机、激光雷达和4D雷达的外参标定，通过多种技术提升标定精度，在多个数据集上相比现有方法减少至少50%的标定误差。


<details>
  <summary>Details</summary>
Motivation: 雷达数据稀疏性导致其外参标定困难，现有方法难以实现相机、激光雷达和4D雷达的联合精确标定，需要一种能够处理多传感器标定的新方法。

Method: 提出CLRNet多模态端到端深度学习网络，采用等距柱状投影、基于相机的深度图像预测、额外雷达通道，利用激光雷达共享特征空间和闭环损失，支持任意两种传感器或三传感器联合标定。

Result: 在View-of-Delft和Dual-Radar数据集上的实验表明，相比现有最先进方法，平移和旋转标定误差中位数均减少至少50%，并展示了跨数据集的领域迁移能力。

Conclusion: CLRNet能够有效解决相机、激光雷达和4D雷达的外参标定问题，显著提升标定精度，具有实际应用价值，代码将公开。
...
TL;DR: 提出EfficientWSI(eWSI)方法，结合参数高效微调(PEFT)和多示例学习(MIL)，实现WSI任务端到端训练，避免昂贵的领域特定预训练，提升计算效率和任务特异性。


<details>
  <summary>Details</summary>
Motivation: WSI分析对肿瘤诊断至关重要，但超高分辨率使端到端训练困难。现有方法使用ImageNet预训练特征提取器，无法捕捉领域特定特征；而领域特定预训练计算昂贵且缺乏任务特异性。

Method: 提出EfficientWSI(eWSI)，将参数高效微调(PEFT)与多示例学习(MIL)集成，实现WSI任务端到端训练。使用PEFT微调特征提取器，避免完全重新训练，结合MIL处理WSI的patch级特征。

Result: 在Camelyon16、TCGA和BRACS数据集上7个WSI级任务评估。eWSI配合ImageNet特征提取器性能强劲，匹配或超越使用领域特定特征提取器的MIL方法。配合领域特定特征提取器时，在多数情况下进一步提升性能。
...
TL;DR: 本文提出了一个可微分的指数映射框架，用于在网格离散化的黎曼曲面上进行机器学习，通过并行GPU实现和两种微分方法，改进了曲面上的学习与优化流程。


<details>
  <summary>Details</summary>
Motivation: 机器学习在非欧几里得域的应用逐渐推广，但在曲面上的几何精确方法仍然落后。主要障碍包括缺乏闭式黎曼算子、离散对应物的不可微性以及并行化能力差。

Method: 采用最直测地线作为计算网格离散化黎曼曲面上指数映射的原理框架，提供并行GPU实现，并推导了两种微分方法：一种利用外部代理函数，另一种基于测地线有限差分方案。

Result: 证明了并行化性能和准确性，展示了可微分指数映射如何改进一般几何形状上的学习和优化流程。提出了新的测地线卷积层、网格上的流匹配方法以及应用于质心Voronoi剖分的二阶优化器。

Conclusion: 该工作为曲面上的机器学习提供了可微分的指数映射框架，通过并行GPU实现和微分方法，克服了现有障碍，展示了在多种应用中的有效性，并提供了开源库digeo。
...
TL;DR: MM-SafetyBench++ 是一个用于评估多模态大语言模型上下文安全性的基准测试，通过为每个不安全图像-文本对构建安全对应物来测试模型区分细微上下文差异的能力。EchoSafe 是一个无需训练的自反思框架，通过记忆库积累安全洞察，在推理时实现上下文感知的安全行为。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在视觉推理任务上表现出色，但对安全风险的脆弱性仍然是一个紧迫问题。先前研究主要关注检测和拒绝显式不安全输入的越狱防御，但往往忽略了上下文安全性，即模型需要区分看似相似但安全意图显著不同的场景之间的细微上下文差异。

Method: 1) 提出 MM-SafetyBench++ 基准：为每个不安全图像-文本对构建对应的安全版本，通过最小修改翻转用户意图同时保留底层上下文含义；2) 提出 EchoSafe 框架：无需训练，维护自反思记忆库来积累和检索先前交互中的安全洞察，通过将相关历史经验集成到当前提示中，实现推理时的上下文感知和持续安全行为演化。

Result: 在各种多模态安全基准测试上的广泛实验表明，EchoSafe 始终实现卓越性能，为推进 MLLMs 的上下文安全性建立了强大的基线。

Conclusion: 该工作提出了一个专门用于评估上下文安全性的基准测试 MM-SafetyBench++，以及一个无需训练的自反思框架 EchoSafe，能够有效提升多模态大语言模型在区分细微上下文差异方面的安全能力，为上下文安全研究提供了重要工具和方法。
...
TL;DR: MATCH是一种快速多视角高斯配准方法，能在0.5秒内从多视角图像直接预测对应的高斯溅射纹理，无需预处理，实现高质量头部虚拟形象创建和编辑。


<details>
  <summary>Details</summary>
Motivation: 现有多视角头部虚拟形象方法需要耗时的头部跟踪和昂贵的优化过程，总创建时间通常超过一天。需要一种更快速、无需预处理的方法来创建高质量的头部虚拟形象。

Method: 使用基于Transformer的端到端模型，在模板网格的固定UV布局中预测高斯溅射纹理。引入新颖的配准引导注意力块，每个UV贴图标记只关注描绘其对应网格区域的图像标记，提高了效率和性能。

Result: MATCH在新视角合成、几何配准和头部虚拟形象生成方面优于现有方法，使虚拟形象创建速度比最接近的基线快10倍。支持跨帧和跨主体的对应关系，实现表情转移、免优化跟踪、语义编辑和身份插值等应用。

Conclusion: MATCH提供了一种快速、高质量的头部虚拟形象创建方法，通过端到端的对应学习实现了高效的多视角高斯配准，显著提升了虚拟形象创建的效率和能力。
...
TL;DR: ModTrack是一个模块化多视角多目标跟踪系统，通过分离感知与跟踪模块，实现跨模态、传感器无关的泛化能力，同时提供可追溯的不确定性估计，性能媲美端到端方法。
...
Method: 将学习限制在检测和特征提取阶段，使用封闭形式分析方法进行融合、关联和跟踪。将传感器输出简化为校准的位置-协方差对，通过跨视图聚类和精度加权融合获得统一估计，使用反馈耦合的GM-PHD滤波器进行身份维护。

Result: 在WildTrack上达到95.5 IDF1和91.4 MOTA，超越所有模块化方法21分以上，媲美最先进的端到端方法。同一跟踪核心可无缝迁移到MultiviewX和RadarScenes数据集。

Conclusion: ModTrack证明了模块化设计可以在保持高性能的同时，提供端到端方法所缺乏的部署灵活性、不确定性估计和跨域泛化能力。
...
TL;DR: ConflictAwareAH是一个多模态框架，通过提取视频、音频和文本特征，利用模态间冲突特征（嵌入的绝对差值）来检测矛盾/犹豫状态，显著提升了检测性能。


<details>
  <summary>Details</summary>
Motivation: 矛盾/犹豫状态在临床环境中很重要，但机器难以识别，因为关键证据存在于不同模态（语言、声音、面部表情）之间的不一致中。现有基于文本的方法倾向于过度检测矛盾/犹豫状态，而难以确认其不存在。

Method: 使用三个预训练编码器提取视频、音频和文本表示，计算成对模态嵌入的绝对差值作为冲突特征。采用文本引导的后期融合策略，将纯文本辅助头与完整模型在推理时结合。

Result: 在BAH数据集上达到0.694 Macro F1（标注测试集）和0.715（私有排行榜），比已发表的多模态基线方法提升超过10个百分点。仅用单GPU在25分钟内完成训练。

Conclusion: 通过显式建模模态间冲突特征，ConflictAwareAH有效解决了矛盾/犹豫状态检测问题，显著提升了性能，特别是对"非矛盾/犹豫"类别的识别能力。
...
TL;DR: AdaRAG-CT通过自适应检索增强框架解决3D CT报告生成中的病理覆盖不全问题，通过补充文本信息补偿视觉表示瓶颈，在CT-RATE基准上实现SOTA临床效果。


<details>
  <summary>Details</summary>
Motivation: 3D CT自动报告生成存在病理覆盖不全的问题，研究发现这是由于视觉表示存在瓶颈：对比3D CT嵌入虽然编码了判别性病理信号，但存在严重的维度集中现象（512维中只有2个有效维度），限制了生成和检索效果。
...
Conclusion: 3D CT报告生成的主要瓶颈在于视觉表示而非语言模型，通过自适应检索增强可以显著提升临床效果，为解决医学图像报告生成的覆盖不全问题提供了有效方案。
...
TL;DR: FEEL数据集首次大规模结合力传感器手套数据和第一人称视频，包含约300万帧厨房环境中的自然操作数据，其中45%涉及手-物接触，用于物理动作理解任务。


<details>
  <summary>Details</summary>
Motivation: 力是驱动物理交互的根本原因，是理解物理动作的关键基础。现有数据集缺乏力测量数据，限制了物理动作理解的研究。

Method: 使用定制压阻手套收集力测量数据，并与第一人称视频同步，构建包含约300万帧厨房操作数据的FEEL数据集。应用该数据集于接触理解（时间接触分割和像素级接触物体分割）和动作表示学习（以力预测作为自监督预训练目标）。

Result: 在时间接触分割任务上达到最先进水平，在像素级分割任务上取得竞争性结果且无需手动标注。使用FEEL进行动作表示学习在EPIC-Kitchens、SomethingSomething-V2、EgoExo4D和Meccano等动作理解任务上提高了迁移性能。

Conclusion: 力测量数据对于物理动作理解至关重要，FEEL数据集通过结合力传感器和第一人称视频，为接触理解和动作表示学习提供了有价值的资源，推动了物理动作理解研究的发展。
...
Method: 两阶段框架：第一阶段训练隐式神经模型学习稳定形状嵌入，在潜在空间聚类获得伪疾病标签；第二阶段在紧凑变分空间中使用伪疾病标签和真实年龄标签解耦因子，通过结合协方差和监督对比损失的多目标解耦损失强制分离和可控性。

Result: 在ADNI海马体和OAI远端股骨形状上，达到接近监督方法的性能，优于最先进的无监督基线，实现高保真重建、可控合成和基于因子的可解释性。

Conclusion: 该框架能够有效分离病理与老化变化，无需诊断标签即可发现疾病模式，为医学形状分析提供了强大的无监督解耦方法，支持生物标志物开发和患者分层。
...
<details>
  <summary>Details</summary>
Motivation: 传统图像质量评估（IQA）指标存在两个极端：要么是僵化的手工数学模型，要么是缺乏可解释性的"黑盒"深度学习架构。需要在这两者之间架起桥梁，实现既高性能又完全可解释的IQA方法。

Method: 提出EvoIQA框架，基于遗传编程进行符号回归，演化出显式的、人类可读的数学公式。利用VSI、VIF、FSIM和HaarPSI等指标的丰富终端集，将结构、色度和信息论退化映射为可观察的数学方程。

Result: 演化出的遗传编程模型在预测与人类视觉偏好之间保持高度一致。不仅优于传统手工指标，而且与DB-CNN等复杂深度学习模型达到性能相当的水平。
...
TL;DR: 稀疏神经网络在视觉Transformer中并未显著提升可解释性，尽管能产生更紧凑的电路结构，但在神经元选择性、特征可解释性和归因忠实度方面没有系统性改进。


<details>
  <summary>Details</summary>
Motivation: 研究稀疏神经网络是否真的比密集模型更具可解释性。虽然权重稀疏性可以在语言模型中产生紧凑电路，但尚不清楚结构稀疏性本身是否能带来更好的语义可解释性。

Method: 使用Wanda修剪的DeiT-III B/16 Vision Transformer模型，提出IMPACT多级评估框架，在四个互补层次评估可解释性：神经元、层表示、任务电路和模型级归因。使用BatchTopK稀疏自编码器分析层表示，通过可学习节点掩码提取电路，使用插入和删除指标评估Transformer归因解释。

Result: 稀疏模型产生比密集模型少约2.5倍边的电路，但活跃节点比例相似或更高，表明修剪重新分配计算而非隔离更简单的功能模块。稀疏模型在神经元级选择性、SAE特征可解释性或归因忠实度方面没有系统性改进。
...
TL;DR: 提出NAMD框架，通过生成1年随访CT图像预测肺结节进展，结合基线扫描和电子健康记录，显著提升恶性预测性能。
...
Method: 提出结节对齐多模态潜在扩散框架，建立结节对齐潜在空间，利用LLM驱动控制机制将患者数据作为扩散主干的条件。

Result: 在NLST数据集上，合成随访图像达到AUROC 0.805和AUPRC 0.346，显著优于基线扫描和现有合成方法，接近真实随访扫描性能。

Conclusion: NAMD能够捕捉肺结节进展的临床相关特征，促进更早更准确的肺癌诊断。
...
TL;DR: 提出结合MobileViT-XXS切片编码器和SliceTransformer聚合器的框架，使用KL正则化Group DRO优化目标，同时解决CT扫描诊断中的分布偏移和人口亚组性能差异问题。
...
Method: 结合轻量级MobileViT-XXS切片编码器和两层SliceTransformer聚合器进行体积推理，使用KL正则化Group DRO训练目标，自适应地提升表现不佳的采集中心和人口亚组的权重。

Result: 在任务1（多站点CT体积的COVID-19二分类）上获得0.835的F1分数，比最佳已发表结果提升5.9；在任务2（基于性别公平约束的四类肺病理识别）上获得0.815的平均每性别宏观F1，比最佳挑战结果提升11.1个百分点，女性鳞状细胞癌F1比Focal Loss基线提升17.4。

Conclusion: KL正则化Group DRO框架能有效同时处理CT诊断中的分布偏移和人口亚组性能差异问题，在保持平均性能的同时提供稳定的最坏情况保护，显著提升了诊断的鲁棒性和公平性。
...
TL;DR: 系统评估11个公开病理基础模型在11个肾脏疾病任务上的表现，发现其在粗粒度形态学任务上表现良好，但在细粒度微结构识别和预后推断任务上表现不佳，提示需要开发肾脏特异的、多染色、多模态基础模型。


<details>
  <summary>Details</summary>
Motivation: 当前病理基础模型主要在癌症数据集上预训练，但其在非癌性慢性肾脏疾病中的应用尚未充分探索，尽管肾脏病理常与恶性肿瘤共存。需要系统评估现有模型在肾脏疾病任务上的表现，以了解其局限性和改进方向。

Method: 系统评估11个公开可用的病理基础模型，涵盖11个肾脏特异性下游任务，包括多种染色（PAS、H&E、PASM、IHC）、空间尺度（切片和玻片级）、任务类型（分类、回归、复制检测）和临床目标。使用重复分层组交叉验证评估切片级性能，使用重复嵌套分层交叉验证评估玻片级任务。采用Friedman检验和成对Wilcoxon符号秩检验进行统计分析。

Result: 模型在由粗粒度中尺度肾脏形态驱动的任务上表现中等至良好，包括诊断分类和显著结构改变的检测。但在需要细粒度微结构区分、复杂生物学表型或玻片级预后推断的任务上，性能持续下降，且与染色类型基本无关。

Conclusion: 当前病理基础模型主要编码静态的中尺度表示，可能难以捕捉细微的肾脏病理或预后相关信号。需要开发肾脏特异性、多染色和多模态的基础模型，以支持肾脏病学中临床可靠的决策制定。
...
TL;DR: UMO提出统一框架，将预训练文本到动作生成模型扩展到多种下游任务，通过帧级元操作嵌入和轻量级时域融合实现上下文适应。


<details>
  <summary>Details</summary>
Motivation: 大规模基础模型在文本到动作生成方面取得进展，但如何有效利用这些单用途模型处理多样化的跨模态和上下文动作生成下游任务仍不清楚。现有方法通常针对单个任务进行特定适配，而本文旨在通过统一框架解锁预训练生成先验，支持广泛的下游任务。

Method: UMO提出统一公式，将下游任务转化为原子级每帧操作的组合。引入三个可学习的帧级元操作嵌入来指定每帧意图，采用轻量级时域融合将上下文线索注入预训练的DiT基动作基础模型，运行时开销可忽略不计。

Result: 实验表明，UMO在广泛基准测试中持续优于任务特定和无训练基线，尽管使用单一统一模型。能够支持时间修复、文本引导动作编辑、文本序列化几何约束和多身份反应生成等先前不支持的任务。

Conclusion: UMO通过统一框架有效解锁预训练动作基础模型的生成先验，支持多样下游任务，为跨模态和上下文动作生成提供通用解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有LVLM剪枝方法统一处理多模态校准数据，忽视了文本和视觉token的模态特异性行为差异，导致剪枝不准确

Method: ATV-Pruning方法：1) 自适应构建校准池，包含所有文本token和部分视觉token；2) 设计层自适应选择策略筛选重要视觉token；3) 建立准确的重要性度量进行权重剪枝

Result: 实验证明ATV-Pruning在标准多模态基准测试中优于现有方法，视觉通路可达到50%稀疏度，文本通路敏感性更高

Conclusion: 通过考虑文本和视觉token的模态特异性行为差异，ATV-Pruning实现了更准确的大型视觉语言模型剪枝，验证了非对称处理策略的有效性
...
TL;DR: FlatLands是一个用于单视角鸟瞰图地板补全的数据集和基准测试，包含27万多个室内场景观测，提供训练免费方法、确定性模型、集成方法和随机生成模型的比较。


<details>
  <summary>Details</summary>
Motivation: 单个自我中心视角图像通常只能捕捉地板的一小部分，而完整的可通行性地图对于室内导航等应用更有价值。现有方法缺乏统一的评估标准。

Method: 创建了包含270,575个观测的FlatLands数据集，来自17,656个真实室内场景，包含对齐的观测、可见性、有效性和地面真实BEV地图。建立了端到端的单目RGB到地板地图的完整流程。

Result: 提供了包含分布内和分布外评估协议的基准测试，比较了训练免费方法、确定性模型、集成方法和随机生成模型等多种方法。

Conclusion: FlatLands为不确定性感知的室内地图构建和生成式补全提供了严格的测试平台，有助于提升具身导航能力。
...
<details>
  <summary>Details</summary>
Motivation: 传统图像引导导航系统依赖外部光学追踪器和额外硬件设置，需要外科医生中断手术任务。需要一种更集成、无需额外硬件、能通过自然语言交互提供计算辅助的系统。

Method: 系统整合自然语言交互与实时视觉感知，直接在术中视频流上操作。首先交互式分割和标记手术器械，然后将其作为空间锚点自主追踪，支持解剖分割、术前3D模型配准、单目视频工具姿态估计和实时解剖叠加等下游工作流。

Result: 在视频引导颅底手术场景中评估，与商用光学追踪系统对比，显示语音引导实体代理能达到竞争性空间精度，同时改善工作流集成，实现视频引导手术系统的快速部署。

Conclusion: 语音引导实体代理框架为视频引导手术提供了一种有前景的替代方案，通过自然语言交互和纯视频操作，提高了工作流集成度，减少了对外部硬件的依赖。
...
<details>
  <summary>Details</summary>
Motivation: 现有线性注意力方法需要从头训练，计算成本高；而基于线性化的方法从大语言模型迁移到ViT效果不佳。需要有效方法将预训练视觉基础模型的知识迁移到线性注意力ViT中

Method: 三阶段框架：1) 注意力对齐：将线性注意力与原始softmax注意力对齐；2) 特征对齐：微调线性化ViT使其最终层特征与冻结的softmax VFM教师对齐；3) 监督微调：将适应后的先验知识迁移到下游任务

Result: 在分类和分割任务上的广泛实验证明ViT-AdaLA在各种最先进的线性注意力方法中具有有效性和通用性

Conclusion: ViT-AdaLA提供了一种有效框架，能够将视觉基础模型的先验知识成功迁移到线性注意力ViT中，解决了线性注意力ViT的性能和可扩展性问题
...
TL;DR: 提出USU方法，解决传统上采样技术在可解释AI中破坏归因信号的问题，通过质量重分配而非插值来保持语义边界和归因完整性。
...
Method: 提出通用语义感知上采样（USU）方法，将上采样重新定义为质量重分配问题而非插值问题。使用比例形式的质量重分配算子，可证明地保持归因质量和相对重要性顺序。该方法基于四个忠实上采样的公理要求，推导出唯一合适的比例形式算子。

Result: 在已知归因先验的模型上进行控制实验验证了USU的形式保证；在ImageNet、CIFAR-10和CUB-200上的评估显示，USU在忠实性方面持续改进，产生语义更连贯、质量更优的解释。

Conclusion: 归因上采样应被视为质量重分配问题而非插值问题。USU方法通过比例形式的质量重分配算子，在保持语义边界的同时确保归因信号的完整性，为可解释AI提供了更可靠的上采样解决方案。
...
TL;DR: 提出一种体积一致的隐式模型，将SDF重建与神经微分同胚流耦合，学习胎盘的共享规范模板，实现体积对齐而非仅表面对应


<details>
  <summary>Details</summary>
Motivation: 现有隐式配准方法主要监督零水平集附近，只能捕捉表面对应关系，而内部变形约束不足，无法满足解剖形状的群体分析需求

Method: 结合符号距离函数重建与神经微分同胚流，通过雅可比行列式和双调和惩罚等体积正则化抑制局部折叠，促进全局一致变形

Result: 在胎盘MRI扫描实验中，相比基于表面的隐式基线方法，本方法在几何保真度和体积对齐方面表现更优，产生解剖可解释且拓扑一致的扁平化结果

Conclusion: 提出的体积一致隐式模型能够统一重建个体胎盘、对齐到群体模板并在规范空间进行体素强度映射，适用于群体分析
...
Result: 在代表性基准测试上的广泛实验表明，该方法始终优于最新的最先进方法，验证了在驾驶场景解析任务中保持语义结构对鲁棒的合成到真实域适应的重要性。

Conclusion: 该研究强调了在合成到真实域适应中保持语义结构的重要性，提出的框架通过显式正则化语义特征结构，显著提升了驾驶场景解析在真实场景中的性能，为自动驾驶系统的可靠运行提供了有效解决方案。
...
TL;DR: Interact3D：一个从单张图像生成物理合理的交互式3D组合物体的新框架，通过两阶段组合管道和基于VLM的自主细化策略解决遮挡和物体间空间关系问题。


<details>
  <summary>Details</summary>
Motivation: 现有方法在从单张图像生成3D组合物体时，特别是在遮挡情况下，往往会在隐藏区域降低几何细节，并且无法保持物体间的空间关系（OOR）。这限制了生成物理合理的交互式3D场景的能力。

Method: 1. 利用先进的生成先验创建高质量个体资产和统一的3D引导场景；2. 两阶段组合管道：通过全局到局部几何对齐（配准）锚定主要物体，然后使用基于可微SDF的优化集成后续几何体，显式惩罚几何相交；3. 基于VLM的闭环自主细化策略：分析多视角渲染，制定针对性修正提示，引导图像编辑模块迭代自校正。

Result: 大量实验表明，Interact3D成功生成了有前景的碰撞感知组合，具有改进的几何保真度和一致的空间关系。

Conclusion: Interact3D框架能够从单张图像生成物理合理的交互式3D组合物体，有效解决了遮挡和物体间空间关系保持的挑战，通过创新的组合管道和自主细化策略实现了高质量的3D场景生成。
...
TL;DR: Parallel-ICL：一种并行处理长上下文演示的推理算法，通过分块并行处理+加权集成，在保持性能的同时显著提升推理速度


<details>
  <summary>Details</summary>
Motivation: 多模态上下文学习（MM-ICL）中增加演示数量能提升性能，但Transformer的二次计算成本导致推理延迟显著增加，需要在准确性和效率之间取得平衡

Method: 提出Parallel-ICL算法：1）将长演示上下文分割为多个短块；2）并行处理这些块；3）在logit层面使用加权Product-of-Experts集成来近似完整上下文输出；4）基于集成学习理论设计聚类分块策略和相似性加权策略

Result: 在VQA、图像描述和分类基准测试中，Parallel-ICL实现了与完整上下文MM-ICL相当的性能，同时显著提高了推理速度

Conclusion: Parallel-ICL有效解决了MM-ICL中的准确率-效率权衡问题，为动态任务适应提供了显著降低推理开销的解决方案
...
TL;DR: LICA是一个包含155万多层图形设计组合的大规模数据集，提供分层结构表示和丰富元数据，支持图形设计的结构化理解和生成研究。


<details>
  <summary>Details</summary>
Motivation: 当前图形设计研究缺乏大规模结构化数据集，无法支持对设计组合的层次化理解和生成。需要超越像素级表示，建立能够捕捉设计结构和关系的数据库。

Method: 构建包含155万设计组合的数据集，每个设计表示为分层组合的组件（文本、图像、矢量、组），包含空间几何、排版属性等元数据。涵盖20个设计类别和97万独特模板，并引入27,261个动画布局作为新挑战。

Result: 创建了大规模图形设计数据集LICA，提供分层结构表示和丰富元数据。建立了新的研究范式，支持图层感知修复、结构化布局生成、可控设计编辑和时间感知生成建模等任务。

Conclusion: LICA通过将设计表示为组合层和关系的系统，支持直接在设计结构上操作的模型研究，为图形设计的结构化理解和生成开辟了新方向。
...
TL;DR: OneWorld是一个直接在3D表示空间中进行扩散的框架，通过3D统一表示自编码器、跨视图对应一致性损失和流形漂移强制技术，解决了现有2D方法在跨视图外观和几何一致性方面的挑战。


<details>
  <summary>Details</summary>
Motivation: 现有的基于扩散的3D场景生成方法主要在2D图像/视频潜在空间中操作，这使得保持跨视图外观和几何一致性具有内在挑战性。为了弥补这一差距，需要直接在连贯的3D表示空间中进行扩散。

Method: 提出了OneWorld框架，核心是3D统一表示自编码器（3D-URAE），利用预训练的3D基础模型，通过注入外观和提取语义来增强其几何中心特性。还引入了标记级跨视图对应（CVC）一致性损失来显式强制跨视图结构对齐，并提出流形漂移强制（MDF）来缓解训练-推理暴露偏差并塑造稳健的3D流形。

Result: 综合实验表明，OneWorld能够生成高质量的3D场景，在跨视图一致性方面优于最先进的基于2D的方法。

Conclusion: OneWorld通过在连贯的3D表示空间中进行扩散，有效解决了跨视图一致性问题，为3D场景生成提供了更优的解决方案。
...
TL;DR: 本文质疑CLIP类模型存在图像-图像对齐问题的假设，通过理论和实验证明该问题并非跨模态训练特有，且任务模糊性而非对齐问题才是影响性能的关键。


<details>
  <summary>Details</summary>
Motivation: 最近研究认为CLIP类对比语言-图像训练产生的嵌入对纯图像任务不理想，主要理论是跨模态对齐损失忽略了图像-图像对齐，导致图像间距离校准不良。本文质疑这一图像-图像错位假设。

Method: 1) 重新审视理论基础，证明图像嵌入距离不存在假设的自由度；2) 比较语言-图像训练模型(CLIP、SigLIP)和图像-图像训练模型(DINO、SigLIP2)的实证指标；3) 在检索和少样本分类任务上进行实验验证。

Result: 1) 理论分析显示图像嵌入距离不存在假设的自由度；2) 实证指标在跨模态训练模型和纯图像训练模型中表现相似，说明观察到的现象并非前者特有；3) 实验表明解决任务模糊性而非假设的错位问题才是获得最佳结果的关键。

Conclusion: CLIP类模型的图像嵌入在纯图像任务中的表现问题并非源于图像-图像错位，而是任务模糊性问题。跨模态训练模型和纯图像训练模型在相关指标上表现相似，解决任务定义问题比修正假设的对齐问题更重要。
...
<details>
  <summary>Details</summary>
Motivation: 3D高斯泼溅需要数百万个原语，导致存储和传输成本高昂；现有压缩方法依赖GPU密集的后训练优化和校准图像，限制了实际部署

Method: 将简化建模为稀疏空间图上的局部成对合并，使用质量保持矩匹配近似高斯对，通过原则性合并成本评估合并质量，限制合并候选到局部邻域并高效选择兼容对
...
TL;DR: PathGLS是一个用于评估病理学视觉语言模型的参考无关框架，通过Grounding、Logic、Stability三个维度综合评估模型性能，特别擅长检测幻觉错误。
...
Method: 提出PathGLS框架，从三个维度评估模型：Grounding（细粒度视觉-文本对齐）、Logic（使用自然语言推理的蕴含图一致性）、Stability（对抗性视觉-语义扰动下的输出方差）。支持补丁级和全切片图像级分析。

Result: 在多个数据集上验证了PathGLS的优越性：在Quilt-1M数据集上，PathGLS对幻觉报告的敏感度下降40.2%，远优于BERTScore的2.1%；与专家定义的临床错误层次结构相关性达到ρ=0.71，显著优于基于LLM的方法。
...
<details>
  <summary>Details</summary>
Motivation: 当前目标检测器在处理分布外(OOD)物体时存在严重问题：这些异常物体往往被完全忽略并错误分类为背景。现有OOD检测方法通常依赖复杂架构或辅助分支，且未能提供统一处理分布内(ID)和分布外物体的框架

Method: 提出SynOE-OD框架，利用Stable Diffusion等强生成模型和开放词汇目标检测器(OVODs)生成语义上有意义的物体级合成数据作为训练时的异常样本。通过迁移学习建立强大的ID任务性能，并增强检测模型对OOD物体的鲁棒性

Result: 在已建立的OOD目标检测基准上达到了最先进的平均精度。像GroundingDINO这样的OVODs在街景中检测OOD物体时零样本性能有限，而本方法显著提升了这一能力

Conclusion: SynOE-OD框架成功解决了目标检测中OOD物体被忽略的问题，通过生成合成异常数据实现了对ID和OOD物体的统一检测，在基准测试中表现出色
...
TL;DR: QICA框架通过数量感知与空间聚合的协同，解决零样本物体计数中的数量感知不足和空间不敏感问题，在多个数据集上展现优越性能。
...
Result: 在FSC-147数据集上展现竞争性性能，在CARPK和ShanghaiTech-A的零样本评估中验证了对未见领域的优越泛化能力。

Conclusion: QICA框架通过协同数量感知和空间聚合，有效解决了零样本物体计数中的关键挑战，实现了更好的数量准确性和跨领域泛化能力。
...
TL;DR: EPOFusion是一种曝光感知的红外与可见光图像融合模型，专门解决过曝光区域信息丢失问题，在保持红外线索的同时实现视觉逼真的融合效果。
...
Result: 实验表明EPOFusion优于现有方法，能在过曝光区域保持红外线索，同时在非过曝光区域实现视觉逼真的融合，提升了视觉保真度和下游任务性能。

Conclusion: EPOFusion通过曝光感知的融合方法有效解决了过曝光区域的融合问题，构建了首个红外与可见光过曝光数据集IVOE，为相关研究提供了重要基础。
...
TL;DR: DualPrim：一种使用正负超二次曲面进行紧凑结构化3D重建的框架，通过加减法设计实现拓扑感知建模，优于仅加法方法


<details>
  <summary>Details</summary>
Motivation: 神经重建方法通常在结构和保真度之间权衡，产生密集非结构化网格，具有不规则拓扑和弱部件边界，这阻碍了编辑、动画和下游资产重用

Method: 使用正负超二次曲面表示形状：正超二次曲面构建基础，负超二次曲面通过可微分算子雕刻局部体积，实现孔洞和凹面的拓扑感知建模。嵌入体积可微分渲染器，支持从多视图图像端到端学习，并通过闭式布尔差实现无缝网格导出

Result: DualPrim实现了最先进的精度，产生紧凑、结构化且可解释的输出，比仅加法替代方案更好地满足下游需求

Conclusion: 加减法设计在不牺牲紧凑性或可微分性的情况下增加了表示能力，为3D重建提供了更实用的结构化解决方案
...
TL;DR: 在低数据条件下，传统数据增强优于GAN增强，FastGAN甚至会加剧分类器偏见，而Stable Diffusion+LoRA表现最佳


<details>
  <summary>Details</summary>
Motivation: 生成模型常用于解决AI训练中的类别不平衡问题，但它们在低数据条件下的失败模式尚未被充分理解，需要系统评估不同增强策略在低数据场景下的表现

Method: 使用牛津-IIIT宠物数据集，人工制造8个代表性不足的品种，对比三种增强策略：传统变换、FastGAN、以及使用LoRA微调的Stable Diffusion 1.5。通过特征嵌入分析和统计测试评估效果

Result: FastGAN在极低训练集大小下不仅表现不佳，反而会显著增加分类器偏见（偏见差距增加+20.7%，Cohen's d = +5.03，p = 0.013）。Stable Diffusion+LoRA表现最佳，达到最高宏平均F1分数（0.9125±0.0047），并将偏见差距相对未增强基线减少13.1%

Conclusion: 存在一个样本量边界（大约每类20-50个训练图像），低于该边界时GAN增强在此设置中会变得有害。Stable Diffusion+LoRA是最有效的增强方法，所有实验可在消费级GPU上完成
...
Method: 提出IOMM两阶段训练框架：第一阶段仅使用丰富的未标记图像数据预训练视觉生成组件，消除对配对数据的依赖；第二阶段使用未标记图像和少量精选文本-图像配对数据进行微调，提升指令对齐和生成质量。

Result: IOMM-B（3.6B）模型仅使用约1050 H800 GPU小时训练，在GenEval上达到0.89，WISE上达到0.55，超越了BAGEL-7B（0.82 & 0.55）和BLIP3-o-4B（0.84 & 0.50）等强基线。

Conclusion: IOMM不仅提高了训练效率，还实现了最先进的性能，为UMM视觉生成提供了一种数据高效且有效的训练方法。
...
TL;DR: EFF-Grasp：基于流匹配的物理感知灵巧抓取生成框架，用确定性ODE替代传统扩散模型的随机SDE，实现高效稳定生成，并通过免训练物理能量引导确保物理可行性。


<details>
  <summary>Details</summary>
Motivation: 现有基于扩散模型的抓取生成方法通常采用随机微分方程(SDE)公式，需要大量去噪步骤且轨迹不稳定，容易产生物理不可行的抓取。需要更高效稳定的生成方法，同时确保物理可行性。

Method: 1) 将抓取生成重新表述为确定性常微分方程(ODE)过程，通过平滑概率流实现高效稳定生成；2) 引入免训练物理感知能量引导策略，使用适应后的显式物理能量函数定义目标分布，通过局部蒙特卡洛近似估计引导项，动态引导生成轨迹朝向物理可行区域。

Result: 在五个基准数据集上的大量实验表明，EFF-Grasp在抓取质量和物理可行性方面表现出优越性能，同时比基于扩散的基线方法需要更少的采样步骤。

Conclusion: EFF-Grasp通过流匹配框架和免训练物理引导，实现了高效、稳定且物理可行的灵巧抓取生成，为机器人抓取生成提供了新的解决方案。
...
TL;DR: GATS框架通过高斯感知时间缩放解决4D点云视频中的分布不一致和时间尺度偏差问题，在多个基准测试中显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 4D点云视频理解对智能体感知动态环境至关重要，但不同帧率下的时间尺度偏差和不规则点云的分布不确定性使得设计统一且鲁棒的4D骨干网络极具挑战。现有CNN或Transformer方法要么受限于有限感受野，要么有二次计算复杂度，且忽视了这些隐式失真。

Method: 提出GATS双不变框架：1) 不确定性引导高斯卷积(UGGC)将局部高斯统计和不确定性感知门控融入点卷积，实现密度变化、噪声和遮挡下的鲁棒邻域聚合；2) 时间缩放注意力(TSA)引入可学习缩放因子归一化时间距离，确保帧划分不变性和不同帧率下的一致速度估计。两个模块互补：时间缩放在高斯估计前归一化时间间隔，高斯建模增强对不规则分布的鲁棒性。
...
Conclusion: GATS框架通过显式解决分布不一致和时间尺度偏差问题，为4D点云视频理解提供了统一且鲁棒的解决方案，在多个基准测试中验证了其有效性，为动态环境感知提供了更优的技术路径。
...
TL;DR: 该论文调查了主要学术期刊对AI生成图像的现行政策，分析了关键问题，并提出了合规使用AI生成科学图像的最佳实践指南。


<details>
  <summary>Details</summary>
Motivation: 生成式AI的快速发展带来了能够制作出版质量科学图像的新工具，但学术出版商对此制定了不一致且模糊的政策，需要明确指导原则来规范AI生成图像的使用。

Method: 调查Nature、Science、Cell Press、Elsevier、PLOS等主要期刊和出版商的AI图像政策，分析关键问题（可重复性、作者归属、视觉错误信息），结合SciDraw等AI工具的实际案例，提出最佳实践指南。

Result: 发现出版商对AI生成图像存在担忧，包括可重复性问题、作者归属不明确、潜在的视觉错误信息风险。通过适当的披露和质量控制，AI生成图像可以加速科学传播而不损害完整性。

Conclusion: AI生成图像在科学传播中具有潜力，但需要透明披露、质量控制和明确的政策指导，以确保科学完整性并促进负责任的使用。
...
TL;DR: 提出一种无监督、架构无关的条件化策略，通过预训练的细胞核分割基础模型生成的连续细胞概率图作为显式输入先验，结合方差保持正则化项，从普通免疫组化图像合成多重免疫荧光图像，显著提升细胞核计数保真度和感知质量。


<details>
  <summary>Details</summary>
Motivation: 多重免疫荧光（mIF）技术成本高、流程复杂且需要专用成像平台，限制了其临床常规应用。现有的虚拟染色方法主要优化像素级保真度，但未明确约束细胞核形态，这在病理学中具有重要临床意义，因为细胞核计数、形状或空间排列的细微扭曲会直接影响量化终点（如Ki67增殖指数），可能导致治疗相关风险分类的错误。

Method: 提出一种无监督、架构无关的条件化策略：1）将预训练的细胞核分割基础模型生成的连续细胞概率图作为显式输入先验；2）引入方差保持正则化项，匹配局部强度统计以保持合成荧光通道中的细胞级异质性。该方法保留了二进制阈值化中丢失的梯度级边界信息，提供更丰富的条件信号，无需任务特定调优。

Result: 在Pix2Pix（使用U-Net和ResNet生成器）、确定性回归U-Net和条件扩散模型上，对两个独立数据集进行的控制实验表明，该方法作为唯一修改，在细胞核计数保真度和感知质量方面均取得一致改进。

Conclusion: 通过引入细胞核分割先验和方差保持正则化，该方法显著提升了从普通免疫组化图像合成多重免疫荧光图像的质量，特别是在细胞核形态保真度方面，为降低mIF技术临床应用门槛提供了有效解决方案。
...
TL;DR: 提出统一时空注意力网络用于连续手语识别，通过同时计算空间（跨关键点）和时间（局部窗口内）注意力分数，生成局部上下文感知的时空表示，参数量比现有SOTA模型减少70-80%。


<details>
  <summary>Details</summary>
Motivation: 当前基于关键点的方法通常依赖时空编码，其中空间交互使用图卷积网络或注意力机制建模，时间动态使用1D卷积网络捕获。但这种设计在编码器和解码器中引入了大量参数。

Method: 提出统一时空注意力网络，同时计算空间（跨关键点）和时间（局部窗口内）注意力分数，聚合特征生成局部上下文感知的时空表示。

Result: 在Phoenix-14T数据集上，提出的编码器参数量比现有SOTA模型减少70-80%，同时达到与基于关键点方法相当的性能。

Conclusion: 统一时空注意力网络能够有效减少模型参数量，同时保持连续手语识别的性能，为实际应用提供了更高效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 可见光-红外行人重识别比传统行人重识别面临更大挑战，主要因为模态间差异显著。现有重排序算法无法同时处理跨模态行人重识别中的模态内变化和模态间差异问题。

Method: 提出渐进式模态关系重排序方法（HHCR），包含两个模块：1）异构一致性重排序：探索查询集和图库集之间的模态关系；2）同构一致性重排序：研究查询集和图库集内部各模态的内在关系。基于此提出一致性重排序推理网络（CRI）作为基线方法。

Result: 通过全面实验证明，提出的重排序方法具有泛化性，重排序方法和基线方法都达到了最先进的性能。

Conclusion: 提出的渐进式模态关系重排序方法能有效解决可见光-红外跨模态行人重识别中的模态差异问题，同时处理模态内变化和模态间差异，取得了优异性能。
...
TL;DR: 提出了360Bench基准测试和Free360框架，用于评估和改进多模态大语言模型在360°图像上的感知能力


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在传统图像理解方面表现出色，但在360°图像感知方面研究不足。360°图像能捕捉整个环境，但存在几何失真和复杂空间关系等挑战

Method: 1) 提出360Bench基准测试，包含7K分辨率360°图像和7个子任务；2) 提出Free360框架，采用基于场景图的训练免费方法，通过模块化推理步骤、自适应球面图像变换和统一图表示来生成答案

Result: 系统评估了7个MLLM和6种增强方法，揭示了它们在360°图像感知方面的不足。Free360框架能持续改进基础MLLM，为360°视觉问答任务提供了强大的训练免费解决方案

Conclusion: 该研究填补了MLLM在360°图像感知方面的空白，提出的360Bench基准和Free360框架为评估和改进360°图像理解能力提供了有效工具，代码和数据集将公开
...
TL;DR: KidsNanny是一个两阶段多模态儿童安全内容审核系统：第一阶段使用视觉Transformer和物体检测器进行视觉筛选（11.7ms），第二阶段通过OCR和7B语言模型进行文本上下文推理（总延迟120ms）。在UnsafeBench数据集上表现优于现有基线模型。


<details>
  <summary>Details</summary>
Motivation: 现有儿童安全内容审核系统在处理包含文本的威胁内容时存在局限性，需要更高效的多模态方法来同时处理视觉和文本信息，以保护儿童在线安全。

Method: 两阶段架构：第一阶段使用ViT和物体检测器进行快速视觉筛选；第二阶段将视觉输出转换为文本，通过OCR提取图像中的文本，再使用7B语言模型进行上下文推理。这种设计避免了传输原始像素数据，提高了效率。

Result: 第一阶段准确率80.27%，F1分数85.39%（11.7ms）；完整管道准确率81.40%，F1分数86.16%（120ms）。在文本依赖型图像上达到100%召回率。相比ShieldGemma-2（64.80%准确率，1136ms）和LlavaGuard（80.36%准确率，4138ms）有显著优势。

Conclusion: 基于OCR的推理方法在处理文本嵌入威胁时具有召回率和精度优势，且延迟更低。该架构为高效多模态内容审核提供了可行方案，但文本专用子集样本量较小限制了普适性结论。
...
TL;DR: ECHO是一个边缘-云框架，用于语言驱动的人形机器人全身控制，通过云端扩散模型生成运动参考，边缘强化学习跟踪器执行，无需硬件微调即可实现稳定控制。


<details>
  <summary>Details</summary>
Motivation: 解决人形机器人语言驱动全身控制的挑战，实现从自然语言指令到机器人动作的端到端控制，消除推理时从人体模型重定向的需求，保持与低级PD控制的直接兼容性。

Method: 采用边缘-云架构：云端使用基于扩散的文本到运动生成器（1D卷积UNet+CLIP文本特征），边缘部署强化学习跟踪器（教师-学生范式+证据适应模块）。使用紧凑的38维机器人原生运动表示，包含自主跌倒恢复机制。

Result: 在重定向的HumanML3D基准测试中表现出色（FID 0.029，R-Precision Top-1 0.686），在Unitree G1人形机器人上实现多样文本指令的稳定执行，无需硬件微调。
...
<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型在SVG生成任务中存在泛化能力有限、代码输出冗余路径多、缺乏显式推理等问题，需要改进SVG生成的质量和结构合理性。

Method: 1) 构建SVG-Sophia高质量数据集（14.5万样本）；2) 引入思维链机制显式暴露推理过程；3) 训练模型生成组级结构化SVG代码；4) 采用GRPO算法和多奖励优化框架（包含DINO、图像文本相似度、格式和代码效率奖励）；5) 联合多奖励优化和多任务训练。

Result: CTRL-S在广泛实验中优于现有方法，实现了更高的任务成功率、更优的SVG代码质量和卓越的视觉保真度。

Conclusion: CTRL-S通过思维链强化学习框架，结合结构化推理和多奖励优化，显著提升了SVG生成的性能，为SVG生成任务提供了有效的解决方案。
...
Method: 提出S-VAM模型，采用自蒸馏策略将多步去噪的结构化生成先验压缩到单步推理中，使用视觉基础模型从扩散模型生成视频中提取教师目标，轻量化解耦器作为学生学习直接映射

Result: 在仿真和真实世界实验中，S-VAM优于现有方法，能够在复杂环境中实现高效精确的机器人操作

Conclusion: S-VAM通过单步推理实现高质量的几何和语义预测，为机器人学习提供了实时且高保真的视频动作模型解决方案
...
TL;DR: Leveling3D：结合前馈3D重建与几何一致生成的新方法，通过几何感知适配器解决外推视图中的伪影问题，提升3D重建质量


<details>
  <summary>Details</summary>
Motivation: 现有方法使用扩散模型修复3D重建的渲染结果，但缺乏几何考虑，无法有效填充外推视图中的缺失区域，需要更全面的同时重建与生成方案

Method: 提出几何感知水平适配器，将扩散模型内部知识与前馈模型的几何先验对齐；引入调色板过滤策略训练更多样化的生成分布；使用测试时掩码细化防止修复区域边界混乱

Result: 在公共数据集上实现了最先进的性能，包括新视角合成和深度估计任务；增强的外推视图可作为3D高斯溅射的输入，进一步提升3D重建质量

Conclusion: Leveling3D通过整合前馈3D重建与几何一致生成，解决了外推视图中的伪影问题，实现了同时重建与生成，显著提升了3D视觉任务的性能
...
TL;DR: GRIP是一种使用四个可穿戴设备重建物理合理人体运动的方法，结合IMU信号和足底压力数据，通过数字孪生人体在物理模拟器中生成真实运动。


<details>
  <summary>Details</summary>
Motivation: 传统IMU方法缺乏地面交互信息，且仅依赖运动学估计无法保证物理合理性。需要结合压力传感器数据并利用物理模拟来重建更真实、物理一致的人体运动。

Method: GRIP包含两个核心模块：KinematicsNet从传感器数据估计身体姿态和速度；DynamicsNet使用KinematicsNet预测与模拟人体状态之间的残差来控制模拟器中的人体。方法还引入了PRISM大规模数据集用于训练和评估。

Result: GRIP在所有评估数据集上优于现有的IMU-only和IMU-pressure融合方法，实现了更高的全局姿态精度和改善的物理一致性。

Conclusion: GRIP通过结合IMU和足底压力数据，并利用物理模拟的数字孪生方法，能够重建物理合理且准确的人体运动，为可穿戴设备运动捕捉提供了新思路。
...
TL;DR: PureCLIP-Depth是一种无需提示词和解码器的单目深度估计模型，直接在CLIP嵌入空间中操作，通过学习RGB到深度的直接映射实现SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 探索一种基于概念信息而非几何特征的单目深度估计新方法，完全在CLIP嵌入空间中进行计算，避免传统模型对几何特征的依赖。

Method: 在CLIP嵌入空间中学习从RGB域到深度域的直接映射，无需提示词和解码器，完全在概念CLIP空间内进行计算。

Result: 在室内和室外数据集上，在基于CLIP嵌入的模型中实现了最先进的性能。

Conclusion: 证明了完全在CLIP嵌入空间中操作、无需提示词和解码器的单目深度估计方法是可行的，并且能够达到优秀的性能。
...
TL;DR: 提出EDP-SAM和XMask方法，利用排除约束和空间分离机制，在稀疏标注下实现半监督人群分析与计数，统一了计数与实例分割任务。


<details>
  <summary>Details</summary>
Motivation: 传统基于点的标注在密集场景中性能受限，因为个体区域具有固有模糊性，从稀疏标注中学习细粒度结构语义仍是一个未解决的挑战。

Method: 1) 提出基于最近邻排除圆约束的EDP-SAM生成掩码监督；2) 提出XMask通过判别性掩码目标强制空间分离；3) 使用高斯平滑和可微分中心采样策略改善特征连续性和训练稳定性；4) 构建半监督人群计数框架，将实例掩码先验作为伪标签。

Result: 在ShanghaiTech A、UCF-QNRF和JHU++数据集上（使用5%、10%、40%标注数据）的实验验证，该端到端模型在半监督分割和计数方面达到最先进性能，有效统一了计数与实例分割。

Conclusion: 该方法通过排除约束和空间分离机制，成功解决了从稀疏标注学习细粒度结构语义的挑战，为半监督人群分析提供了统一框架。
...
TL;DR: RASLF是一个基于状态空间的轻场超分辨率框架，通过多表示建模、自适应扫描和双锚聚合提升重建精度与效率
...
Method: 提出RASLF框架：1) 渐进几何细化块(PGR)使用全景极线表示编码多视角视差差异；2) 表示感知非对称扫描机制(RAAS)根据表示空间物理特性动态调整扫描路径；3) 双锚聚合模块(DAA)优化层次特征流，减少冗余特征

Result: 在多个公开基准测试中，RASLF实现了最高的重建精度，同时保持较高的计算效率

Conclusion: RASLF通过显式建模多轻场表示间的结构相关性，有效解决了纹理细节丢失和几何错位问题，在精度和效率间取得了良好平衡
...
TL;DR: DiVA-Former：轻量级视觉-文本融合架构，通过视觉token作为动态查询来压缩长文本序列，有效利用表格的视觉布局和文本信息互补性，在13个表格基准上显著超越纯文本基线。


<details>
  <summary>Details</summary>
Motivation: 现有方法存在两个问题：1）LLMs将2D表格线性化为1D序列会削弱行列邻接和布局线索；2）纯视觉编码器能捕捉空间线索但难以保留精确单元格文本。分析发现视觉和文本模态提供高度不同的信息且具有强互补性，但直接拼接等融合方法收益有限且常引入跨模态干扰。

Method: 提出DiVA-Former轻量级架构，利用视觉token作为动态查询来蒸馏长文本序列为可消化的向量，有效利用视觉-文本互补信息。该方法避免了直接拼接导致的干扰问题。

Result: 在13个表格基准测试中，DiVA-Former相比纯文本基线提升23.9%，并在使用视觉输入、文本输入或两者组合的现有基线上取得一致增益。

Conclusion: DiVA-Former通过创新的视觉-文本融合机制有效解决了表格理解中模态互补但融合困难的问题，为表格处理提供了更优的多模态解决方案。
...
Method: 1) 基于Pix2Pix的生成修复模块去除伪影；2) Swin Transformer集成与MedSigLIP对比嵌入进行鲁棒表示学习；3) 生物启发式精炼步骤，使用几何尖度和基于马氏距离的形态约束恢复分布外预测

Result: 在WBCBench 2026挑战的私有排行榜上获得0.77139的Macro-F1分数，在严重不平衡条件下表现出色

Conclusion: 该方法在极端类别不平衡下实现了罕见类别的泛化，证明了将生物先验知识融入深度学习在血液学图像分析中的价值
...
TL;DR: SEVEX框架通过自动化的语义探索发现任务级视觉提示，显著提升LVLM的感知能力，超越传统工具选择方法。


<details>
  <summary>Details</summary>
Motivation: LVLM在图像理解和视觉推理方面存在显著挑战，导致关键感知失败。现有视觉提示生成方法主要关注工具选择而非诊断和解决感知失败的根源问题，且需要人工试错，效率低下。

Method: 提出SEVEX语义探索算法：1) 使用抽象概念空间作为搜索空间；2) 采用新颖性引导的选择算法；3) 基于语义反馈的概念生成过程，通过代理驱动实验实现高效多样的视觉提示探索。

Result: 在BlindTest和BLINK基准测试中，SEVEX在任务准确性、推理效率、探索效率和探索稳定性方面显著优于基线方法，发现了超越传统工具使用的复杂反直觉视觉策略。

Conclusion: SEVEX框架通过自动化、任务级的视觉提示探索，为增强LVLM感知提供了新范式，能够发现传统方法难以发现的复杂视觉策略。
...
TL;DR: EVPV提出显式视觉前提验证方法，通过视觉检查清单和约束提取来分离感知与推理，校准过程奖励模型的评分，提升多模态推理的验证和重排序性能。


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言过程奖励模型(VL-PRMs)作为黑盒评判器，将感知不确定性与逻辑推理纠缠在一起，导致系统性误判：可能奖励幻觉的视觉前提或惩罚正确的接地陈述，影响重排序和错误定位的可靠性。

Method: 提出显式视觉前提验证(EVPV)框架：1) 策略生成步骤级视觉检查清单，明确所需视觉事实；2) 约束提取器从输入图像独立推导结构化视觉约束；3) 匹配检查清单声明与约束计算视觉可靠性信号；4) 通过可靠性门控校准PRM步骤奖励，视觉依赖步骤在可靠性低时奖励衰减，可靠性高时保留。

Result: 在VisualProcessBench和六个多模态推理基准测试中，EVPV显著提升了步骤级验证性能，并持续优于强基线的最佳N重排序准确率。通过向提取约束注入受控损坏，观察到单调性能下降，为约束保真度和显式前提验证的因果效应提供了证据。

Conclusion: EVPV通过显式验证视觉前提，有效解耦了感知不确定性与逻辑评估，无需每步工具调用即可提升过程奖励模型的可靠性，为多模态推理系统提供了更稳健的验证机制。
...
TL;DR: FrameRepeat：通过轻量级重复评分模块自动增强视频LLM的视觉注意力，解决视觉锚点漂移问题


<details>
  <summary>Details</summary>
Motivation: 视频问答中，多模态大语言模型的链式思维推理存在"视觉锚点漂移"问题——模型过度依赖自生成的文本而忽略视觉输入，导致幻觉和性能下降。现有解决方案需要特定机制在推理时重新关注视觉输入，但训练成本高且泛化性差。

Method: 提出FrameRepeat框架：1) 轻量级重复评分模块，让视频LLM自主识别需要强化的关键帧；2) Add-One-In训练策略，利用MLLM输出概率生成表示重复增益的监督信号，训练帧评分网络来指导帧重复行为。

Result: 在多个模型和数据集上的实验结果表明，FrameRepeat在推理过程中有效增强重要视觉线索，且具有良好的泛化能力。

Conclusion: FrameRepeat提供了一种自动化、轻量级且可泛化的解决方案，能够有效缓解视频问答中的视觉锚点漂移问题，增强模型对视觉输入的关注。
...
<details>
  <summary>Details</summary>
Motivation: 现有红外小目标检测方法依赖像素级分割，需要昂贵的密集标注，且不适用于纹理弱、边界模糊的微小目标

Method: 包含两个模块：1) 物理驱动的自适应掩码生成(PAMG)，将点标注转换为紧凑目标掩码和几何线索；2) 轻量级半径感知点回归网络(RPR-Net)，利用时空运动线索将检测重构为目标中心定位和有效半径回归

Result: 在点监督设置下，框架实现了强伪标签质量、高检测精度和高效推理，接近全监督性能，同时大幅降低标注成本

Conclusion: Point-to-Mask框架成功地将低成本点监督与掩码级检测相结合，为红外小目标检测提供了一种高效且经济的解决方案
...
TL;DR: AW-MoE：一种用于恶劣天气条件下鲁棒3D目标检测的框架，通过集成混合专家（MoE）和图像引导的天气感知路由，在真实世界数据集上实现了约15%的性能提升。


<details>
  <summary>Details</summary>
Motivation: 现有方法在训练时简单地将所有天气样本混合，忽略了不同天气场景下的数据分布差异，导致性能冲突。需要一种能够处理天气数据分布差异的鲁棒3D目标检测方法。

Method: 提出AW-MoE框架：1）集成混合专家（MoE）到天气鲁棒的多模态3D目标检测方法中；2）引入图像引导的天气感知路由（IWR），利用图像特征在不同天气条件下的判别能力和对场景变化的鲁棒性进行精确天气分类；3）基于准确分类选择top-K最相关的天气特定专家（WSE）处理数据差异；4）提出统一双模态增强（UDMA）进行同步LiDAR和4D雷达数据增强，同时保持场景真实性。

Result: 在真实世界数据集上，AW-MoE在恶劣天气条件下相比最先进方法实现了约15%的性能提升，同时推理开销可忽略。将AW-MoE集成到现有基线检测器中，性能提升超过当前最先进方法。

Conclusion: AW-MoE框架有效解决了不同天气条件下的数据分布差异问题，展示了强大的有效性和可扩展性。代码将在GitHub上公开。
...
TL;DR: 本文提出FG-SGL框架，通过细粒度语义指导解决微手势识别中类间差异细微的问题，结合细粒度和类别级语义指导视觉语言模型感知局部运动。
...
Method: 提出FG-SGL框架：1) FG-SA模块利用细粒度语义指导学习局部运动特征；2) CP-A模块通过类别级语义增强特征可分性；3) 构建细粒度文本数据集描述MG动态过程；4) 设计多级对比优化策略进行粗到细的联合优化。

Result: 实验表明FG-SGL取得了有竞争力的性能，验证了细粒度语义指导对微手势识别的有效性。

Conclusion: 细粒度语义指导能有效提升视觉语言模型对微手势局部运动的感知能力，为解决类间差异细微的识别问题提供了新思路。
...
TL;DR: 提出基于几何的奖励模型，利用预训练几何基础模型通过跨帧重投影误差评估多视角一致性，解决视频扩散模型中的几何不一致问题，并通过几何感知采样和后训练/推理时优化提升视频生成质量。


<details>
  <summary>Details</summary>
Motivation: 视频扩散模型在训练时缺乏显式的几何监督，导致生成的视频中出现物体变形、空间漂移和深度违反等不一致性伪影。现有几何度量在像素空间测量不一致性，容易受到像素强度噪声的影响。

Method: 1) 提出基于几何的奖励模型，利用预训练几何基础模型通过跨帧重投影误差评估多视角一致性，采用点对点误差计算而非像素空间计算；2) 引入几何感知采样策略，过滤低纹理和非语义区域，专注于几何上有意义且对应关系可靠的区域；3) 通过两种途径应用奖励模型：后训练（SFT或强化学习）和推理时优化（测试时缩放）。

Result: 实验验证了设计的有效性，表明基于几何的奖励相比其他变体具有更好的鲁棒性。通过高效的推理时缩放，该方法为增强开源视频模型提供了实用解决方案，无需大量计算资源进行重新训练。

Conclusion: 提出的几何奖励模型能够有效解决视频扩散模型中的几何不一致问题，通过点对点误差计算和几何感知采样提高了鲁棒性，并通过后训练和推理时优化两种途径实现了对视频模型的几何对齐，为开源视频模型的改进提供了实用方法。
...
TL;DR: 提出LTS-FS框架，通过定位幻觉相关层并稀疏化特征导向，在减少幻觉的同时保持模型性能


<details>
  <summary>Details</summary>
Motivation: 现有特征导向方法在所有层应用统一调整，忽略了层间差异，可能破坏与幻觉无关的层，导致整体性能下降

Method: 1) 构建包含token级和句子级幻觉案例的合成数据集；2) 基于因果干预的归因方法量化每层的幻觉相关性；3) 将归因分数转换为各层特征导向强度，实现精准调整

Result: 在多个大型视觉语言模型和基准测试上的广泛实验表明，LTS-FS框架能有效缓解幻觉，同时保持强大的性能

Conclusion: 提出的LTS-FS框架通过层特异性特征导向，在减少幻觉的同时避免了整体性能下降，为幻觉缓解提供了更精确的方法
...
TL;DR: EverTale是一个故事世界模拟器，通过统一的LoRA模块实现连续角色定制，使用MLLM质量门控确保角色保真度，并采用区域聚焦采样解决多角色生成问题。


<details>
  <summary>Details</summary>
Motivation: 当前故事可视化方法难以同时实现准确的角色定制、语义对齐和新身份的持续集成，需要解决角色保真度、多角色布局冲突和身份退化等问题。

Method: 1. 一体化世界角色集成器：使用统一LoRA模块实现连续角色适应，无需每个角色的优化模块。2. 角色质量门控：通过MLLM-as-Judge进行链式推理，确保角色适应过程的保真度。3. 角色感知区域聚焦采样：协调局部角色细节与全局场景上下文，解决身份退化和布局冲突。

Result: 实验结果表明，EverTale在单角色和多角色故事可视化任务中，相比更广泛的方法都取得了优越性能。

Conclusion: EverTale通过创新的角色集成、质量门控和采样策略，成功解决了故事可视化中的连续角色定制和多角色生成问题，为故事世界模拟提供了有效解决方案。
...
TL;DR: 提出了VisBrowse-Bench基准，用于评估多模态大语言模型在网页浏览中的视觉推理能力，现有模型表现不佳（最佳模型准确率仅47.6%）


<details>
  <summary>Details</summary>
Motivation: 现有基准存在两个局限：1) 对视觉推理能力评估不足；2) 忽视了网页原生视觉信息在推理链中的作用。需要新的基准来评估多模态大语言模型在真实网页浏览中的视觉推理能力
...
Result: 实验结果显示，即使是表现最好的Claude-4.6-Opus模型准确率也只有47.6%，Deep Research模型o3-deep-research准确率为41.1%，表明当前模型在视觉原生搜索任务上仍有很大提升空间

Conclusion: VisBrowse-Bench基准有效揭示了当前多模态大语言模型在网页浏览中视觉推理能力的不足，为未来研究提供了重要的评估工具和方向
...
Method: 提出Micro-AU CLIP框架，将AU检测过程分解为：1）局部语义独立性建模（LSI）：设计Patch Token Attention（PTA），将AU区域内多个局部特征映射到同一特征空间；2）全局语义依赖性建模（GSD）：提出Global Dependency Attention（GDA）和Global Dependency Loss（GDLoss）来建模不同AU间的全局依赖关系；3）设计microAU对比损失（MiAUCL）进行视觉和文本特征的细粒度对齐，克服CLIP在微语义对齐上的局限性。

Result: 实验结果表明，Micro-AU CLIP能够充分学习细粒度的微表情AU特征，实现了最先进的性能，并能以无情感标签的方式有效应用于微表情识别。

Conclusion: 通过探索独立性到依赖性的模式，提出的Micro-AU CLIP框架成功解决了现有方法对AU区域感知不足的问题，实现了细粒度的微表情AU检测和识别。
...
TL;DR: DriveFix是一个用于自动驾驶场景的多视角修复框架，通过时空协同的扩散变换器架构解决现有4D场景重建方法中的时空不一致问题。


<details>
  <summary>Details</summary>
Motivation: 现有基于扩散先验的4D场景重建方法通常独立处理帧或逐视角处理，缺乏时空协同，导致跨摄像头空间错位和时间序列漂移，影响自动驾驶场景的鲁棒性。

Method: 采用交错扩散变换器架构，包含专门模块显式建模时间依赖性和跨摄像头空间一致性；通过历史上下文条件生成和几何感知训练损失，确保修复的视角符合统一的3D几何结构。

Result: 在Waymo、nuScenes和PandaSet数据集上的广泛评估表明，DriveFix在重建和新视角合成方面达到最先进性能，显著减少伪影并实现高保真纹理的一致性传播。

Conclusion: DriveFix通过确保驾驶场景的时空一致性，为实现鲁棒的4D世界建模向实际部署迈出了重要一步。
...
<details>
  <summary>Details</summary>
Motivation: 传统肺癌治疗方法（手术、化疗、放疗）因癌症异质性而效果有限，需要基于患者个体特征的个性化治疗方案。人工智能方法可以分析大量数据来发现早期癌症迹象、肿瘤类型和治疗前景。
...
Conclusion: 该研究成功整合了机器学习、可解释性AI和大语言模型，为肺癌个性化治疗提供了数据驱动的决策支持系统，有望改善治疗效果并减少不必要的药物毒性。
...
TL;DR: SpikeCLR是一个用于脉冲神经网络的自监督对比学习框架，能够从未标记的事件数据中学习视觉表示，在标签稀缺的场景下优于监督学习。


<details>
  <summary>Details</summary>
Motivation: 事件视觉传感器具有微秒级时间分辨率、高动态范围和低功耗等优势，结合脉冲神经网络可在神经形态硬件上实现高效能应用。然而，训练这类模型需要大规模标注数据集，而这类数据集稀缺，限制了其潜力。

Method: 提出SpikeCLR对比自监督学习框架，将先前的基于帧的方法适配到脉冲域，使用替代梯度训练，并引入一套事件特定的增强方法，包括空间、时间和极性变换。

Result: 在CIFAR10-DVS、N-Caltech101、N-MNIST和DVS-Gesture基准测试中，自监督预训练加微调在低数据情况下优于监督学习，在少样本和半监督设置中取得一致增益。结合空间和时间增强对学习事件数据的时空不变性至关重要，学习到的表示可跨数据集迁移。

Conclusion: SpikeCLR框架能够从未标记的事件数据中学习鲁棒的视觉表示，在标签稀缺的场景下为事件视觉模型提供了有效的解决方案，有助于推动事件视觉在嵌入式系统中的应用。
...
<details>
  <summary>Details</summary>
Motivation: 传统前馈方法依赖大量训练数据但仍丢失细节，而现有基于扩散的方法虽然利用生成先验但难以实现合成到真实场景的域转移。需要一种既能保持细节又能强泛化的方法。

Method: 提出两阶段先验到几何确定性调度：先验阶段使用谱门控蒸馏转移低频真实先验，几何阶段使用谱门控一致性强制高频保真度并用合成真值进行细化。两阶段共享权重，采用高到低时间步调度。

Result: 实验结果表明Iris在单目深度估计性能上取得显著提升，并展现出强大的野外泛化能力。

Conclusion: Iris通过整合真实世界先验到扩散模型中，成功解决了传统方法丢失细节和现有扩散方法域转移困难的问题，实现了高效且泛化能力强的单目深度估计。
...
<details>
  <summary>Details</summary>
Motivation: 遥感图像目标检测面临几何形状多样性（不同长宽比）和空间复杂性（不同尺度）的双重挑战。现有方法单独处理这两个问题：使用各向异性条带卷积处理细长目标，或使用各向同性大卷积核捕获上下文，但孤立处理导致互补缺陷。

Method: 提出Poly Kernel Inception Network v2 (PKINet-v2)，统一协同各向异性轴向条带卷积和各向同性方形卷积核，构建多尺度感受野。引入异构核重参数化策略，将异构分支融合为单一深度卷积，实现高效推理。

Result: 在DOTA-v1.0、DOTA-v1.5、HRSC2016和DIOR-R四个基准测试中达到最先进精度，相比PKINet-v1实现3.9倍FPS加速，在效果和效率上均超越先前遥感骨干网络。

Conclusion: PKINet-v2通过统一架构协同处理遥感目标检测的几何和空间复杂性挑战，结合异构核重参数化实现高效部署，为遥感图像分析提供了强大而高效的骨干网络。
...
TL;DR: 提出HOIL框架，利用人-物交互解决LiDAR点云中3D人体姿态估计的空间模糊性和类别不平衡问题


<details>
  <summary>Details</summary>
Motivation: 现有方法忽视了人-物交互在3D人体姿态估计中的潜力。存在两个主要挑战：1) 人-物交互导致人与物点云的空间模糊性，导致交互区域关键点预测错误；2) 交互与非交互身体部位点云数量严重不平衡，手、脚等频繁交互区域在LiDAR数据中稀疏

Method: 提出HOIL框架：1) HOICL（人-物交互感知对比学习）增强人与物点云特征区分度；2) CPPool（接触感知部分引导池化）自适应重新分配表示能力，压缩过表示点云同时保留交互部位信息点；3) 可选的基于接触的时序细化，利用时序接触线索修正关键点估计

Result: HOIL框架有效利用人-物交互解决交互区域的空间模糊性和类别不平衡问题

Conclusion: 通过人-物交互学习，能够构建更鲁棒的3D人体姿态估计框架，代码将开源
...
<details>
  <summary>Details</summary>
Motivation: 姬蜂总科寄生蜂的准确分类鉴定对于生物多样性评估、生态监测和生物防治至关重要，但形态相似、体型小、种间差异细微使得人工鉴定劳动密集且依赖专家经验。

Method: 提出基于YOLO架构的深度学习框架，集成高分辨率类激活映射（HiResCAM）增强可解释性。使用3556张高清膜翅目标本图像数据集，主要包含姬蜂科、茧蜂科、蜜蜂科和胡蜂科。通过精确率、召回率、F1分数和准确率评估模型性能。

Result: 模型准确率超过96%，在形态变异上表现出稳健的泛化能力。HiResCAM可视化证实模型关注翅脉、触角分节和后腹部结构等分类相关的解剖区域，验证了学习特征的生物学合理性。
...
TL;DR: 提出D³-RSMDE框架，结合ViT快速生成深度图先验和轻量级扩散模型细化，在遥感单目深度估计中实现速度与质量的最佳平衡
...
Method: 1) 使用ViT模块快速生成高质量初步深度图作为结构先验；2) 提出渐进线性混合细化策略，在VAE压缩的潜空间中使用轻量级U-Net进行少量迭代细化
...
TL;DR: 提出实时水下图像增强框架，通过自适应加权通道补偿、多分支重参数化空洞卷积和统计全局颜色调整，实现准确色彩恢复，仅需3,880参数，达到409 FPS推理速度。


<details>
  <summary>Details</summary>
Motivation: 现有高性能水下图像增强方法通常依赖复杂架构，难以部署在水下设备上；轻量级方法则往往牺牲质量换取速度，且难以处理严重退化的水下图像。

Method: 1) 自适应加权通道补偿模块：以绿色通道为参考锚点，动态恢复红蓝通道颜色；2) 多分支重参数化空洞卷积：训练时多分支融合，推理时结构重参数化，实现大感受野低计算开销；3) 统计全局颜色调整模块：基于统计先验优化整体色彩性能。

Result: 在8个数据集上实验，在7个评估指标上达到SOTA性能，仅含3,880推理参数，推理速度409 FPS，UCIQE分数提升29.7%，在ROV平台部署和下游任务中验证了实时水下任务优势。

Conclusion: 该方法实现了实时水下图像增强与准确色彩恢复的平衡，为水下平台提供了可靠的视觉信息解决方案，具有实际部署价值。
...
<details>
  <summary>Details</summary>
Motivation: 现有多模态大语言模型在医学视觉问答中常出现"捷径回答"问题，即依赖语言先验或数据集偏见而非充分关注视觉证据，这降低了临床可靠性，特别是在细微影像发现起决定性作用时。

Method: 提出InViC框架：1) 线索令牌提取模块将密集视觉令牌压缩为K个问题条件化的线索令牌；2) 两阶段微调策略：第一阶段使用线索瓶颈注意力掩码阻断LLM直接访问原始视觉特征，强制所有视觉证据通过线索通路；第二阶段恢复标准因果注意力，训练LLM联合利用视觉和线索令牌。

Result: 在三个公开医学VQA基准测试（VQA-RAD、SLAKE和ImageCLEF VQA-Med 2019）上评估多个代表性MLLM，InViC在零样本推理和标准LoRA微调基础上均有持续改进。

Conclusion: 意图感知视觉线索配合瓶颈训练是提高可信医学视觉问答的实用有效策略，InViC框架能增强基于图像的答案生成能力。
...
<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型在细粒度属性解耦和因果推理方面存在困难，传统的基于相似性的检索方法倾向于选择相关但非因果的示例，放大了虚假关联并限制了模型鲁棒性

Method: 提出CIRCLES框架，通过属性引导的组合图像检索主动构建反事实式示例集，使模型能够隐式推理属性与结果之间的因果关系

Result: 在四个不同数据集上的实验表明，CIRCLES在多种架构上始终优于现有方法，特别是在小规模模型上，在信息稀缺情况下增益尤为显著

Conclusion: CIRCLES通过检索更多样化和因果信息丰富的示例，为模型如何利用上下文演示进行改进推理提供了定性见解，推动了更稳健和基于因果关系的推理
...
TL;DR: SemTok是一种语义一维分词器，将2D图像压缩为具有高级语义的1D离散标记，在图像重建中达到SOTA，并显著提升下游图像生成任务性能。


<details>
  <summary>Details</summary>
Motivation: 现有视觉分词器主要将图像映射到固定的2D空间网格，专注于像素级恢复，这阻碍了捕获具有紧凑全局语义的表示。需要一种能够捕捉高级语义的压缩表示方法。

Method: 提出SemTok语义一维分词器，包含三个关键创新：2D到1D的分词方案、语义对齐约束和两阶段生成训练策略。基于此构建掩码自回归生成框架。

Result: SemTok在图像重建中达到新的SOTA，使用极其紧凑的标记表示实现卓越的保真度。在下游图像生成任务中带来显著改进。

Conclusion: SemTok通过语义一维分词有效解决了现有视觉分词器的局限性，证明了语义1D分词的有效性，为下游任务提供了更好的基础。
...
Method: 使用对比无配对翻译(CUT)网络，通过多层补丁对比学习最大化对应补丁间的互信息，在保持内容一致性的同时学习跨域相似性，将DMSP数据转换为VIIRS格式。

Result: 生成的VIIRS类数据与实际VIIRS观测高度一致(R²>0.87)，与社会经济指标相关性良好，有效校准了DMSP缺陷。

Conclusion: 该方法有效解决了跨传感器数据融合问题，校准了DMSP缺陷，为扩展夜间灯光时间序列提供了可靠尝试。
...
Method: 基于Flux.1构建DermaFlux框架，使用参数高效的LoRA进行微调；利用Llama 3.2生成符合皮肤病学标准（不对称性、边界不规则性、颜色变化）的合成文本描述构建图像-文本对

Result: DermaFlux生成多样化且临床有意义的皮肤病图像：增强小型真实数据集时提升二元分类性能达6%；仅使用DermaFlux合成图像训练分类器时比基于扩散的合成图像提升9%；仅用2,500张真实图像和4,375个DermaFlux样本的ViT达到78.04%准确率和0.859 AUC，超越最佳皮肤病模型8%

Conclusion: DermaFlux能够生成高质量的临床基础皮肤病变图像，有效缓解数据稀缺和类别不平衡问题，显著提升皮肤病变分类性能
...
TL;DR: 提出一种利用对称光源布置的线性解法，用于近光光度立体视觉，无需初始化和精确标定即可恢复表面法线和深度。


<details>
  <summary>Details</summary>
Motivation: 传统近光光度立体视觉方法通常需要非凸优化，对初始化和光源标定要求高。本文旨在开发一种无需初始化和精确标定的线性解法，降低应用门槛。

Method: 通过布置多组对称的近光源对，利用光源对称性推导出表面法线和深度的闭式解。该方法仅要求光源关于任意点对称分布，即使整个空间偏移未标定也能工作。

Result: 实验表明该方法在形状恢复精度上与最先进的标定近光光度立体方法相当，同时显著减少了对仔细深度初始化和光源标定的要求。

Conclusion: 提出的线性解法为近光光度立体视觉提供了一种高效实用的替代方案，通过利用对称光源布置实现了无需初始化和精确标定的形状恢复。
...
TL;DR: HGP-Mamba：基于Mamba的多模态框架，通过从全切片图像生成蛋白质特征，结合组织学特征进行癌症生存风险预测，在四个公共数据集上达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 蛋白质标记物和组织病理学图像的联合预后潜力尚未充分探索，主要由于蛋白质表达谱分析成本高、可用性有限。需要一种能高效整合这两种模态信息的方法来改进癌症生存风险预测。

Method: 1. 蛋白质特征提取器(PFE)：利用预训练基础模型直接从WSI提取高通量蛋白质嵌入；2. 局部交互感知Mamba(LiAM)：实现细粒度特征交互；3. 全局交互增强Mamba(GiEM)：在切片层面促进整体模态融合，捕捉复杂的跨模态依赖关系。

Result: 在四个公共癌症数据集上的实验表明，HGP-Mamba实现了最先进的性能，同时与现有方法相比保持了卓越的计算效率。

Conclusion: HGP-Mamba通过从组织学图像生成蛋白质特征，有效整合了分子信息和形态学模式，为癌症生存风险预测提供了一种高效的多模态解决方案，解决了蛋白质表达数据稀缺的问题。
...
TL;DR: SF-Mamba是一种新型视觉Mamba模型，通过辅助补丁交换和批量折叠技术，解决了现有视觉Mamba模型在非因果交互和计算效率方面的限制，在多种视觉任务上表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有视觉Mamba模型虽然计算效率高，但其循环扫描机制限制了图像补丁间的非因果交互，且短token长度下计算速度较慢。先前多扫描策略存在扫描设计不佳和数据重排频繁的问题。

Method: 提出SF-Mamba，包含两个关键技术：1) 辅助补丁交换 - 在单向扫描下编码双向信息流；2) 批量折叠与周期性状态重置 - 提升GPU并行性。

Result: 在图像分类、目标检测、实例分割和语义分割等任务上，SF-Mamba显著优于现有最先进基线模型，同时在不同模型规模下提高了吞吐量。

Conclusion: SF-Mamba通过重新思考视觉扫描操作和Mamba计算效率，实现了真正高效的视觉编码器，为视觉Mamba模型的发展提供了新方向。
...
TL;DR: 提出HGFNet用于高光谱图像分类，结合3D卷积局部特征提取和傅里叶全局滤波，引入三种频率变换并采用自适应焦点损失解决类别不平衡问题。
...
Method: 提出HGFNet架构，整合局部3D卷积特征提取和基于GFNet块的频域全局滤波。引入三种频率变换：光谱傅里叶变换（沿光谱轴的1D FFT）、空间傅里叶变换（空间维度的2D FFT）和空间-空间傅里叶变换（光谱和空间维度的3D FFT）。采用自适应焦点损失解决类别不平衡问题。

Result: 未在摘要中明确说明具体实验结果，但方法旨在实现高效稳健的空间-光谱表示学习，通过傅里叶全局滤波模块有效建模长程依赖并抑制噪声。

Conclusion: HGFNet通过整合3D卷积局部特征提取和频域全局滤波，为高光谱图像分类提供了一种新颖的解决方案，能够全面建模高维频率信息并有效处理类别不平衡问题。
...
TL;DR: 提出自监督跨模态协调方法，利用未标记浮游生物图像和光学测量数据构建识别模型，减少对标注数据的依赖


<details>
  <summary>Details</summary>
Motivation: 浮游生物自动成像仪器大规模收集图像数据，但现有监督方法需要大量人工标注。同时，现代仪器还提供散射和荧光剖面等光学测量数据，这些数据在浮游生物识别中未被充分利用。需要一种能利用多模态数据且减少标注需求的方法。

Method: 受CLIP启发，使用对比学习训练图像和剖面数据的编码器，仅需二元监督信息（判断图像和剖面是否来自同一粒子）。识别时结合少量标注样本库和k-NN分类器，构建能同时利用图像和剖面信息的多模态识别模型。

Result: 该方法在仅需少量标注图像的情况下实现了高识别准确率，且优于仅使用图像的自监督基线方法。

Conclusion: 自监督跨模态协调策略能有效利用多模态数据和大量未标记浮游生物数据，显著减少对人工标注的依赖，为浮游生物识别提供了实用解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有无监督物理参数估计方法缺乏统一的基准：现有方法在非重叠的合成数据上评估，唯一的真实世界数据集仅限于单体系系统，且没有建立控制方程识别的协议。

Method: 创建IRIS基准数据集，包含220个高分辨率真实世界视频，涵盖单体和多体动力学系统，提供地面真实参数和不确定性估计，并定义标准化评估协议（参数准确性、可识别性、外推性、鲁棒性和控制方程选择）。

Result: 建立了包含220个视频的基准数据集，评估了多个基线方法（包括多步物理损失公式和四种方程识别策略），揭示了系统性的失败模式，为未来研究提供参考。

Conclusion: IRIS为无监督物理参数估计提供了一个全面的真实世界基准，通过标准化评估协议和基线方法评估，暴露了现有方法的局限性，推动了该领域的研究发展。
...
TL;DR: 提出CD-FKD方法，通过全局和实例级特征蒸馏增强单域泛化目标检测能力，在域偏移场景下优于现有方法


<details>
  <summary>Details</summary>
Motivation: 单域泛化对目标检测至关重要，但域偏移（如天气、光照、场景变化）严重挑战现有模型的泛化能力，需要提升模型在未见目标域的鲁棒性

Method: 提出跨域特征知识蒸馏（CD-FKD），使用下采样和损坏的多样化数据训练学生网络，教师网络使用原始源域数据。学生网络通过全局和实例级特征蒸馏模仿教师特征，有效提取物体中心特征

Result: 在挑战性场景上的大量实验表明，CD-FKD在目标域泛化和源域性能上都优于最先进方法，验证了其在提升目标检测对域偏移鲁棒性方面的有效性

Conclusion: 该方法在自动驾驶和监控等需要多样化环境中鲁棒目标检测的实际应用中具有重要价值
...
TL;DR: 该论文提出使用轻量级神经网络结合知识蒸馏来加速3D手部重建模型，在保持精度的同时实现1.5倍加速，适用于资源受限设备。


<details>
  <summary>Details</summary>
Motivation: 当前最先进的3D手部重建方法依赖重型模型，限制了在头显、智能手机等资源受限设备上的应用。需要开发既快速又准确的方法来支持VR/AR、人机交互等实时应用。

Method: 以HaMeR模型为基础，将其原始的ViT-H骨干网络替换为MobileNet、MobileViT、ConvNeXt和ResNet等轻量级替代方案。评估了三种知识蒸馏策略：输出级蒸馏、特征级蒸馏以及两者混合的蒸馏方法。

Result: 使用仅占原始模型35%大小的轻量级骨干网络，实现了1.5倍的推理速度提升，同时保持了相似的性能质量，精度差异仅为0.4mm。输出级蒸馏显著提升学生模型性能，特征级蒸馏对高容量学生模型更有效。

Conclusion: 轻量级神经网络结合知识蒸馏的方法能够有效加速复杂3D手部重建模型，为低功耗设备上的高效实时应用铺平了道路。代码和模型已公开。
...
TL;DR: 本文首次提出雨滴和反射统一去除任务(UR³)，构建了真实拍摄数据集RDRF，并提出基于扩散模型的DiffUR³框架，在复杂场景中取得了SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 雨天透过玻璃或挡风玻璃拍摄时，雨滴和反射经常同时出现，严重降低图像可见度。现有去雨滴、去反射或一体化模型都无法有效解决这种复合退化问题，急需新的解决方案。

Method: 首次正式定义雨滴和反射统一去除任务(UR³)；构建真实拍摄数据集RDRF作为新基准；提出基于扩散模型的DiffUR³框架，利用强大的生成先验，通过特定目标设计来同时去除两种退化。

Result: 在RDRF基准测试和真实野外图像上，DiffUR³方法都取得了最先进的性能表现，成功去除了雨滴和反射两种退化。

Conclusion: 本文解决了雨天玻璃表面图像退化的实际问题，通过定义新任务、构建数据集和提出扩散模型框架，为雨滴和反射统一去除提供了有效的解决方案，数据集和代码将在接受后公开。
...
TL;DR: 提出ProgressiveAvatars，一种基于3D高斯层次结构的渐进式虚拟形象表示方法，支持动态网络和计算资源下的渐进加载与渲染。


<details>
  <summary>Details</summary>
Motivation: 在实时XR和远程呈现应用中，网络和计算资源频繁波动，需要一种渐进式的3D表示方法。

Method: 基于模板网格的自适应隐式细分构建3D高斯层次结构，3D高斯定义在面部局部坐标系中以保持动画能力，通过屏幕空间信号触发层次扩展，利用重要性排序支持渐进加载和渲染。

Result: 实现了在不同带宽、计算和内存资源下的渐进式传输和渲染，能够平滑提升质量。

Conclusion: ProgressiveAvatars为波动网络和计算环境下的实时XR应用提供了一种有效的渐进式虚拟形象表示方案。
...
TL;DR: TinyGLASS是一个轻量化的异常检测模型，专为索尼IMX500智能视觉传感器设计，在保持竞争力的检测性能同时，实现了8.7倍的参数压缩，能在资源受限的边缘平台上实时运行。


<details>
  <summary>Details</summary>
Motivation: 工业质量控制中的异常检测面临标注缺陷样本稀缺的挑战。现有自监督方法如GLASS虽然性能良好，但计算需求大，难以部署在资源受限的边缘平台上。

Method: 将GLASS框架的WideResNet-50骨干网络替换为紧凑的ResNet-18，并引入面向部署的修改，支持静态图追踪和INT8量化，使用索尼的模型压缩工具包进行优化。

Result: 在MVTec-AD基准测试中达到94.2%的图像级AUROC，参数压缩8.7倍，在IMX500平台上以20FPS实时运行，功耗仅4.0mJ/推理，能量效率达470GMAC/J，对训练数据污染具有鲁棒性。

Conclusion: TinyGLASS成功实现了轻量化异常检测模型，在保持竞争力的检测性能的同时，满足了边缘设备的资源约束，为工业质量控制的实时部署提供了可行方案。
...
TL;DR: Evo-Retriever是一个检索框架，通过LLM引导的课程进化来增强视觉-语言模型在异构文档中的跨模态检索能力，采用多视图图像对齐和双向对比学习策略。
...
Method: 1) 多视图图像对齐通过多尺度和多方向视角增强细粒度匹配；2) 双向对比学习生成"困难查询"并建立互补学习路径进行视觉和文本消歧；3) LLM元控制器根据模型状态总结自适应调整训练课程以促进模型进化。

Result: 在ViDoRe V2和MMEB (VisDoc)数据集上，Evo-Retriever实现了最先进的性能，nDCG@5分数分别为65.2%和77.1%。

Conclusion: Evo-Retriever通过创新的视角-路径协作和LLM引导的课程进化，有效解决了异构文档跨模态检索中的挑战，显著提升了检索性能。
...
TL;DR: GAP-MLLM：一种几何对齐预训练范式，通过视觉提示联合任务和多级渐进融合模块，激活多模态大语言模型的几何感知能力，提升3D空间理解性能


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在纯RGB输入下3D空间感知能力不足，现有方法采用文本主导的微调范式无法激活模型中的几何表示，导致几何特征利用不充分

Method: 提出几何对齐预训练范式GAP-MLLM：1）视觉提示联合任务，让模型同时预测稀疏点云图和语义标签；2）多级渐进融合模块，通过令牌级门控机制自适应整合几何先验

Result: GAP-MLLM显著增强了几何特征融合能力，在3D视觉定位、3D密集描述和3D视频目标检测等任务上取得一致性能提升

Conclusion: 通过几何对齐预训练范式激活MLLMs的结构感知能力，可以有效解决纯RGB输入下3D空间理解不足的问题，为多模态模型几何理解提供了新思路
...
TL;DR: 提出基于双流Transformer网络（DST-Net）的低光图像增强方法，通过解耦的照明无关信号先验和多尺度空间卷积，在保持纹理细节的同时提升图像质量。


<details>
  <summary>Details</summary>
Motivation: 现有低光图像增强方法在提升图像质量时往往导致内在信号先验的严重损失，无法有效保持精细结构和纹理细节。

Method: 1. 特征提取模块：整合高斯差分、LAB色彩空间变换和VGG-16进行纹理提取，使用解耦的照明无关特征作为信号先验；2. 双流交互架构：通过跨模态注意力机制，利用提取的先验动态校正增强图像的退化信号表示；3. 多尺度空间融合模块：采用伪3D和3D梯度算子卷积，集成显式梯度算子恢复高频边缘，同时通过多尺度空间卷积捕捉通道间空间相关性。

Result: 在LOL数据集上达到25.64 dB的PSNR，主观视觉质量和客观指标均表现优异；在LSRW数据集上的验证进一步证实了其强大的跨场景泛化能力。

Conclusion: DST-Net通过照明无关信号先验引导和多尺度空间卷积，有效解决了低光图像增强中的信号退化问题，在保持纹理细节的同时实现了高质量的图像增强，具有优秀的泛化性能。
...
TL;DR: 提出UOT-Unlearn框架，基于非平衡最优传输实现一步生成模型的类别遗忘，在保持生成质量的同时有效移除目标类别


<details>
  <summary>Details</summary>
Motivation: 一步生成模型（如流映射模型）在图像生成效率方面取得显著进展，但针对这些强大生成器的机器遗忘问题尚未被探索。现有的扩散遗忘方法依赖于多步迭代去噪过程，与一步模型不兼容

Method: 提出UOT-Unlearn框架，基于非平衡最优传输（UOT）将遗忘问题形式化为遗忘成本（抑制目标类别）与f-散度惩罚（通过松弛边际约束保持整体生成保真度）之间的权衡。利用UOT使被遗忘类别的概率质量平滑地重新分配到剩余类别

Result: 在CIFAR-10和ImageNet-256上的实验结果表明，该框架在遗忘成功率（PUL）和保留质量（u-FID）方面表现优异，显著优于基线方法

Conclusion: UOT-Unlearn是首个针对一步生成模型的即插即用类别遗忘框架，通过非平衡最优传输实现了高效且高质量的类别移除，解决了现有扩散遗忘方法与一步模型不兼容的问题
...
TL;DR: VIEW2SPACE：一个用于稀疏多视角推理的多维基准测试，通过物理仿真生成大规模数据，评估发现当前模型在多视角推理上表现接近随机猜测，提出的Grounded Chain-of-Thought方法能显著提升性能。
...
Result: 对最先进的视觉语言和空间模型评估显示，多视角推理问题基本未解决，大多数模型表现仅略高于随机猜测。提出的方法在中等难度下显著提升性能，并能泛化到真实世界数据，在跨数据集评估中优于现有方法。

Conclusion: 多视角推理仍是一个根本性挑战。虽然几何感知在足够可见度下可通过扩展受益，但跨稀疏视角的深度组合推理仍然是主要难题。提出的基准测试和方法为这一领域提供了重要基础。
...
TL;DR: 提出基于图论的新算法，用于从3D压力迹线中精确分割和测量爆轰胞格，相比传统手动和2D边缘检测方法有显著改进。


<details>
  <summary>Details</summary>
Motivation: 解决爆轰研究中长期存在的挑战：准确提取胞格模式。传统手动方法和原始2D边缘检测方法存在局限性，需要更精确的3D分析方法。

Method: 基于图论的无训练算法，使用分割模型从3D压力迹线（爆轰晶格）中提取细胞模式，建立基于图的工作流程。

Result: 在生成数据上分割误差为2%；3D模拟数据显示沿波传播轴排列的椭圆形细胞，偏差17%；体积分散较大反映线性变异的立方放大。

Conclusion: 基于图的框架能泛化到多种细胞几何形状，是爆轰分析的实用工具，为未来三重点碰撞研究奠定基础，但对高度复杂胞格模式的分割和量化仍具挑战。
...
TL;DR: 提出一个结合蒙特卡洛位姿采样和费舍尔信息PnP优化的3DGS重定位框架，解决位姿先验不确定性和几何不确定性问题


<details>
  <summary>Details</summary>
Motivation: 3D高斯泼溅（3DGS）在视觉定位和位姿优化中表现出色，但其鲁棒性对初始相机位姿和重建几何高度敏感。存在两个主要不确定性来源：1) 位姿先验不确定性（回归或检索模型输出单一确定性估计）；2) 几何不确定性（3DGS重建缺陷传播到PnP求解器）。这些不确定性会扭曲重投影几何并破坏优化稳定性。

Method: 提出一个重定位框架，结合蒙特卡洛位姿采样和基于费舍尔信息的PnP优化。该方法显式地考虑位姿和几何不确定性，无需重新训练或额外监督。

Result: 在多样化的室内外基准测试中，该方法持续提高定位精度，并显著增强在位姿和深度噪声下的稳定性。

Conclusion: 通过显式建模位姿和几何不确定性，提出的蒙特卡洛采样与费舍尔信息PnP优化框架有效解决了3DGS位姿优化的鲁棒性问题，在各种场景下实现了更稳定和准确的定位。
...
TL;DR: 使用变分自编码器（VAE）和期望最大化（EM）方法解决扫描透射电子显微镜（STEM）的自动校准问题，通过低维图像表示学习实现比现有方法更快、更准确的参数估计。


<details>
  <summary>Details</summary>
Motivation: 电子显微镜校准面临挑战：高维噪声图像、单张图像无法确定最优参数、现有方法仅提取标量值、模拟数据与真实数据存在差距。

Method: 1. 使用模拟数据训练VAE学习图像的低维表示；2. 采用EM方法联合估计校准参数到编码表示的映射模型和最优校准参数；3. 利用光学系统对称性确保全局可识别性。

Result: 在实际STEM上比现有方法快2倍，估计误差减少2倍，需要更少观测，校准更一致。证明了VAE在图像信息压缩中的潜力。

Conclusion: VAE-EM框架成功解决了STEM自动校准问题，为存在模拟-现实差距和非单射映射的反问题提供了通用解决方案。
...
TL;DR: CompDiff：一种分层组合扩散框架，通过在表示层面解决生成器不平衡问题，提高医学图像生成的公平性和质量


<details>
  <summary>Details</summary>
Motivation: 当前生成模型在医学图像数据增强中存在"不平衡生成器问题"：基于不平衡数据训练的生成器在不同人口统计组间产生质量差异，特别是在罕见子群和训练中未出现的人口交叉组合上表现不佳

Method: 提出CompDiff分层组合扩散框架，包含专门的层次条件网络(HCN)来分解人口统计条件，生成人口统计token并与CLIP嵌入连接作为交叉注意力上下文，支持参数共享和组合泛化

Result: 在胸部X光(MIMIC-CXR)和眼底图像(FairGenMed)上，CompDiff在图像质量(FID: 64.3 vs. 75.1)、子群公平性(ES-FID)和零样本交叉泛化(最高21% FID提升)方面优于标准微调和FairDiffusion

Conclusion: 人口统计条件化的架构设计是公平医学图像生成中重要但未被充分探索的因素，CompDiff通过表示层面的方法有效解决了生成器不平衡问题，下游分类器也显示出改进的AUROC和减少的人口统计偏差
...
<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注文本模态，将幻觉归因于过强的语言先验和不足的视觉基础。本文发现视觉模态内部的异常注意力模式也会导致物体幻觉，需要新的检测和缓解方法。

Method: 提出基于分割的注意力熵(SAE)，利用语义分割在物体级语义空间中量化视觉注意力不确定性。基于SAE设计了幻觉检测的可靠性评分和SAE引导的注意力调整方法，在推理时修改视觉注意力以缓解幻觉。

Result: 在公开基准测试和四足机器人的实际多模态场景中，SAE显著减少了物体幻觉，无需额外训练成本，使LVLM驱动的感知和决策更加可靠。

Conclusion: SAE方法有效解决了LVLM中的物体幻觉问题，通过量化视觉注意力不确定性和调整注意力模式，提高了模型的可靠性，为实际应用提供了可信的解决方案。
...
TL;DR: 基于Transformer的深度学习模型，直接从原始长时程活细胞成像视频预测癌细胞在化疗下的命运，无需预定义特征，准确率高达0.94，并能解释预测的生物学依据。


<details>
  <summary>Details</summary>
Motivation: 理解非遗传因素对细胞命运的决定对于癌症治疗至关重要，因为遗传相同的细胞在相同治疗条件下可能产生不同结果。现有方法通常依赖预定义的形态或分子特征，限制了发现新生物学机制的能力。

Method: 提出基于Transformer的深度学习模型，直接从原始长时程活细胞成像视频序列预测细胞命运（如凋亡或存活）。模型不依赖预定义特征，并引入全面的可解释性框架，通过注意力机制和掩蔽实验分析时间动态和形态线索。

Result: 模型在细胞命运预测上达到平衡准确率0.94和F1分数0.93。注意力分析显示预测信号不仅存在于轨迹的最后几帧，可靠预测可在事件发生前10小时实现。分析揭示了有丝分裂和凋亡序列中预测信息的时间分布差异，以及细胞形态和p53信号在决定细胞命运中的作用。

Conclusion: 基于注意力的时间模型能够准确预测细胞命运，同时提供对细胞决策非遗传决定因素的生物学可解释见解。该方法为理解癌症治疗中细胞异质性提供了新工具，代码已开源。
...
<details>
  <summary>Details</summary>
Motivation: 当前3D内容创作中，为形状生成高质量、物理真实的材质是一个耗时且需要专业知识的过程。需要一种能够根据几何和文本描述自动生成多种物理材质属性的方法。

Method: 采用视频扩散Transformer架构，以输入几何和文本描述为条件，联合建模多种材质属性（基础色、粗糙度、金属性、高度图）。引入定制变分自编码器将多材质模态编码到紧凑潜在空间，实现多模态联合生成而不增加token数量。

Result: 该方法能够根据文本提示为3D形状生成高质量材质，生成结果与常见内容创作工具兼容，实现了物理合理的材质生成。

Conclusion: 提出的方法能够有效生成物理真实的3D材质，通过视频扩散架构和定制VAE实现了多材质属性的联合生成，为3D内容创作提供了高效工具。
...
TL;DR: Face2Scene：一种两阶段图像恢复框架，利用人脸作为感知参考来估计退化并指导整个图像的恢复，包括身体和背景区域。
...
Method: 两阶段框架：1）使用Ref-FR模型从退化图像和身份参考中恢复高质量面部细节；2）从恢复-退化人脸对中提取面部衍生的退化代码，转换为多尺度退化感知token，用扩散模型在单步中恢复整个场景。

Result: 大量实验表明，该方法在恢复效果上优于现有最先进方法。

Conclusion: Face2Scene通过将人脸作为感知参考来指导全场景恢复，有效解决了现有方法在面部专注和全场景恢复之间的局限性。
...
TL;DR: REFORGE是一个黑盒红队框架，通过对抗性图像提示评估图像生成模型去学习（IGMU）的鲁棒性，暴露了现有方法在多模态对抗攻击下的脆弱性。


<details>
  <summary>Details</summary>
Motivation: 图像生成模型（IGMs）在实现高保真内容生成的同时，也带来了版权侵权和有害内容生成的风险。图像生成模型去学习（IGMU）旨在移除有害概念而无需完全重新训练，但其在黑盒设置下对对抗性输入（特别是图像侧威胁）的鲁棒性尚未得到充分探索。

Method: REFORGE框架初始化基于笔触的图像，并通过跨注意力引导的掩码策略优化扰动，该策略将噪声分配到概念相关区域，平衡攻击效果和视觉保真度。
...
TL;DR: 该研究将人脑的共线性视觉感知原理引入计算机视觉，探索其在多个应用场景中的潜力，发现对工业检测等结构化场景有效，但对ImageNet等自然图像效果有限。


<details>
  <summary>Details</summary>
Motivation: 人脑的共线性感知原理（对空间对齐的边缘有增强感知）在真实世界中的作用尚不明确，在计算机视觉和工程应用中的潜力也未被充分探索。本研究旨在将这一生物视觉原理迁移到计算机视觉领域。

Method: 开发了原型模型来体现共线性原理，通过四个用例进行系统测试和基准评估：1）结合深度学习进行缺陷检测；2）与显著性模型结合；3）处理遮挡；4）在ImageNet上测试。

Result: 在晶圆缺陷检测中，错误率从6.5%降至5.26%（性能提升1.24倍）；在纳米材料缺陷识别中，错误率从21.65%降至6.64%（性能提升3.2倍）；对遮挡处理有效；但在ImageNet上效果不明显。

Conclusion: 共线性原理特别适用于工业应用，因为感兴趣的结构通常由直线组成（如晶圆、纳米材料）。该研究为计算机视觉提供了新工具，有望捕捉人类视觉处理的优势。
...
TL;DR: FSMC-Pose是一个用于奶牛发情期姿态估计的轻量级框架，通过频率-空间融合骨干网络和多尺度自校准头，在复杂背景和动物遮挡下实现准确、实时的姿态估计。
...
Result: 在MOUNT-Cattle和NWAFU-Cattle组合数据集上，FSMC-Pose比强基线模型获得更高精度，同时计算量和参数成本显著降低，在商用GPU上保持实时推理。

Conclusion: FSMC-Pose能有效捕捉和估计复杂杂乱环境中的奶牛爬跨姿态，为奶牛发情检测提供了实用的解决方案。
...
TL;DR: Proxy-GRM：通过代理引导的评分标准验证增强视觉语言模型的生成奖励模型，在少量数据上实现SOTA性能


<details>
  <summary>Details</summary>
Motivation: 现有生成奖励模型的三阶段流程中，中间评分标准很少被直接优化。先前工作要么将评分标准视为附带产物，要么依赖昂贵且不可微分的LLM-as-judge检查，缺乏训练指导。

Method: 提出Proxy-GRM，在强化学习中引入代理引导的评分标准验证。训练轻量级代理（Proxy-SFT和Proxy-RL），它们仅基于评分标准预测偏好排序，代理的预测准确率作为评分标准质量奖励，激励模型生成内部一致且可迁移的评分标准。

Result: 仅用约5万数据样本，在VL-Reward Bench、Multimodal Reward Bench和MM-RLHF-Reward Bench上达到最先进结果，优于使用四倍数据训练的方法。学习到的评分标准可迁移到未见过的评估器，无需额外训练即可提高测试时的奖励准确性。

Conclusion: Proxy-GRM通过显式优化评分标准质量，有效提升了生成奖励模型的性能，证明了评分标准验证的重要性以及代理引导方法的有效性。
...
TL;DR: ACPV-Net：首个从航拍图像生成完整矢量地图的端到端框架，支持多类别多边形化，确保拓扑一致性（无间隙、无重叠）


<details>
  <summary>Details</summary>
Motivation: 现有多边形化方法通常是类别特定的，通过逐类别运行会导致拓扑不一致（重复边缘、间隙、重叠）。需要一种能够一次性生成所有土地覆盖类别完整矢量地图的方法。

Method: 提出ACPV-Net统一框架，引入语义监督条件机制（SSC）将语义感知与几何基元生成耦合，并通过拓扑重建设计强制共享边缘一致性。

Result: 在Deventer-512基准测试中，ACPV-Net在保持严格拓扑约束的同时，在所有类别的多边形质量上超越了所有类别特定基线。在WHU-Building单类别任务上也取得了最佳结果。

Conclusion: ACPV-Net首次实现了从航拍图像到完整矢量地图的端到端生成，解决了多类别多边形化的拓扑一致性问题，为地理信息系统提供了高效准确的解决方案。
...
TL;DR: TCATSeg：一种结合局部几何特征与全局语义上下文的新型3D牙科模型分割框架，通过稀疏超点捕获全局语义关系，在400个牙科模型数据集上超越现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有3D牙科模型分割方法因复杂牙齿排列和相邻牙齿形状相似而效果不佳，主要原因是它们过于关注局部几何特征而忽略了全局上下文信息。

Method: 提出TCATSeg框架，结合局部几何特征与全局语义上下文；引入稀疏但物理意义明确的超点来捕获全局语义关系；构建包含400个牙科模型（包括正畸前样本）的新数据集。

Result: 大量实验表明TCATSeg在3D牙科模型分割任务上超越了现有最先进方法，证明了其有效性。
...
<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型被提出用于生成人脸识别决策的自然语言解释，以增强人类可解释性。然而，这些解释在无约束人脸图像上的可靠性尚未得到充分探索，特别是在极端姿态变化和监控图像等挑战性场景下。

Method: 1. 在具有挑战性的IJB-S数据集上系统分析MLLM生成的无约束人脸验证解释；2. 研究结合传统人脸识别系统信息（分数和决策）对解释质量的影响；3. 提出基于似然比的评估框架，超越决策准确性来衡量文本解释的证据强度。

Result: 1. 即使MLLM做出正确验证决策，其解释也经常依赖不可验证或幻觉的面部属性；2. 结合传统人脸识别系统信息能提高分类验证性能，但不能保证解释的忠实性；3. 基于似然比的评估框架揭示了当前MLLM在可解释人脸识别方面的根本局限性。

Conclusion: 当前多模态大语言模型在可解释人脸识别方面存在根本局限性，其生成的解释缺乏可靠性和可信度。研究强调了在生物识别应用中需要建立原则性评估方法，以确保解释的可靠性和可信性。
...
TL;DR: FlowComposer：首个系统研究流匹配用于组合零样本学习的框架，通过学习原始流和可学习的组合器，在嵌入空间显式构建组合，并利用泄漏特征增强性能。


<details>
  <summary>Details</summary>
Motivation: 现有基于视觉语言模型的组合零样本学习方法存在两个根本限制：1）隐式组合构建（仅通过令牌连接或分支提示调优）；2）残留特征纠缠（不完美的解耦导致属性、对象和组合特征相互污染），限制了模型的泛化能力。

Method: 提出FlowComposer框架：学习两个原始流将视觉特征传输到属性和对象文本嵌入，通过可学习的组合器显式融合它们的速度场为组合流。设计泄漏引导增强方案，重用泄漏特征作为辅助信号。

Result: 在三个公开CZSL基准测试中，将FlowComposer作为即插即用组件集成到各种基线模型中，均实现了显著改进。

Conclusion: FlowComposer通过流匹配方法解决了CZSL中的组合构建和特征纠缠问题，提供了模型无关的解决方案，有效提升了组合零样本学习的性能。
...
TL;DR: BUSSARD是一种基于标准化流的模型，用于检测场景图中的异常关系，通过语言模型嵌入对象和关系，使用标准化流学习从场景图三元组到高斯分布的映射，在SARD数据集上比SOTA模型AUROC提升约10%，速度快5倍。


<details>
  <summary>Details</summary>
Motivation: 检测场景图中的异常关系对于理解图像语义和现实世界场景分析具有重要意义。现有方法在准确性和效率方面存在局限，需要一种能够利用语义知识并高效检测异常关系的模型。

Method: 提出BUSSARD模型：1) 使用语言模型嵌入场景图中的对象和关系标记以利用现实世界语义知识；2) 采用标准化流模型学习从对象-关系-对象三元组到简单基分布（高斯分布）的双射变换；3) 通过似然估计进行异常检测。

Result: 在SARD数据集（办公室和餐厅场景）上评估：1) AUROC结果比当前SOTA模型提升约10%；2) 推理速度快5倍；3) 消融研究显示在词汇同义词方面具有优越鲁棒性，基线模型有17.5%性能波动而BUSSARD保持稳定。

Conclusion: BUSSARD展示了基于学习方法在场景图关系异常检测中的强大潜力，通过结合语言模型的语义知识和标准化流的概率建模，实现了更准确、更快速且更鲁棒的异常检测。
...
Method: 使用统一的风格编码器，在大规模内容-风格-风格化三元组数据集上训练，将多样风格嵌入一致潜在空间。通过相似性感知的门控机制动态将风格路由到MoE架构中的专门专家。

Result: 实验表明StyleExpert在保持语义和材质细节方面优于现有方法，并且能够泛化到未见过的风格。

Conclusion: StyleExpert通过MoE架构成功处理了多语义层次的多样化风格，在风格化任务中实现了更好的语义和材质细节保持。
...
<details>
  <summary>Details</summary>
Motivation: 监测巢穴筑巢的野生蜜蜂和黄蜂对生物多样性研究和保护至关重要，但手动评估层状陷阱巢穴检测和分类育雏细胞耗时耗力，且存在数据不平衡和标注工作量大的问题

Method: 提出深度学习方法来检测和分类层状陷阱巢穴中的育雏细胞，引入新颖的约束假阳性损失策略，动态屏蔽未标注数据的预测，防止其干扰分类损失训练

Result: 在包含712张层状陷阱巢穴图像的数据集上评估，涵盖28个细粒度类别，实验结果表明深度学习能有效检测育雏细胞，CFPL方法进一步提升了性能，平衡了模型准确性和标注工作量

Conclusion: 深度学习可用于层状陷阱巢穴中育雏细胞的检测，提出的CFPL策略能减少标注工作量、缓解类别不平衡，为生物多样性监测提供高效工具
...
TL;DR: HeBA提出了一种异构瓶颈适配器，通过模态特定的结构归纳偏置来改进视觉语言模型的微调，在11个少样本基准上取得了SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有的大规模视觉语言模型（如CLIP）在下游任务适配时采用"一刀切"的架构方法，忽视了视觉和文本模态在结构上的本质差异：图像具有空间局部性，而文本具有语义密度。

Method: HeBA包含三个关键创新：1）异构性：视觉token使用2D深度可分离卷积处理以保持空间相关性，文本token使用密集线性投影处理以捕捉语义关系；2）瓶颈正则化：采用压缩瓶颈（D→D/4）强制学习紧凑、鲁棒的特征；3）主动梯度初始化：使用Kaiming初始化策略确保足够的初始梯度流以加速收敛。

Result: 在11个少样本基准测试中，HeBA的架构专业化设计实现了优越的稳定性和准确性，建立了新的最先进水平。

Conclusion: HeBA通过引入模态特定的结构归纳偏置，克服了传统适配器的同质化限制，为视觉语言模型的下游任务适配提供了一种更有效的统一架构框架。
...
<details>
  <summary>Details</summary>
Motivation: 高光谱图像具有丰富光谱信息但维度高、传感器差异大，对域间分布差异敏感。现有单源域泛化方法中，盲目增强可能产生不现实的样本，而过度强调真实性会抑制多样性，存在真实性与多样性的权衡问题

Method: SPDDA方法：1) 光谱多样性模块沿光谱维度重采样生成不同光谱通道的样本；2) 通道自适应光谱混合器建模通道间相似性避免固定增强模式；3) 空间-光谱协同优化机制联合优化空间保真约束和光谱连续性自约束；4) 光谱自约束权重根据空间对应部分自适应调整

Result: 在三个遥感基准数据集上的实验表明，SPDDA优于现有最先进方法

Conclusion: 通过显式考虑高光谱图像的内在特性（设备依赖的光谱通道数变化和相邻通道混合），SPDDA有效平衡了增强样本的真实性与多样性，提升了单源域泛化性能
...
TL;DR: Kestrel是一个无需训练的LVLM幻觉缓解框架，通过显式视觉基础代理和证据验证的自优化机制，在保持可解释性的同时有效减少多模态幻觉。


<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型在多模态任务中容易产生幻觉，而训练这些模型避免幻觉的成本过高。现有的基于解码或工具使用的方法效果有限且可解释性差，需要一种廉价、灵活且可解释的解决方案。

Method: Kestrel采用两阶段方法：1) 收集显式视觉证据并将工具输出转换为可重用的结构化文本证据；2) 通过LVLM法官验证证据，然后基于已验证证据迭代自优化答案，避免过度修正。

Result: 在幻觉基准测试中表现优异：在POPE上平均提升+3.31%，在MME-Hallucination上提升+28.34（使用Qwen3-VL）。自优化模块和基础代理各贡献平均+2.0%的POPE增益，并提供透明的验证轨迹用于幻觉诊断。

Conclusion: Kestrel提供了一种无需训练、可解释且有效的LVLM幻觉缓解方案，通过结合显式视觉基础和证据验证的自优化机制，在保持性能提升的同时提供透明的诊断分析能力。
...
TL;DR: Fast-WAM：一种无需在测试时显式想象未来的世界动作模型，通过保留训练时的视频协同训练但跳过测试时的未来预测，实现了与现有方法竞争的性能，同时大幅降低延迟


<details>
  <summary>Details</summary>
Motivation: 探索世界动作模型(WAMs)中显式未来想象的必要性，研究视频建模在训练中的价值是否比测试时的未来生成更重要，以解决现有WAMs因迭代视频去噪导致的高延迟问题

Method: 提出Fast-WAM架构，保留训练时的视频协同训练但跳过测试时的未来预测，并实例化多个变体进行对比实验，包括在模拟基准(LIBERO和RoboTwin)和真实世界任务上的评估

Result: Fast-WAM在模拟和真实世界任务上取得与最先进方法竞争的结果，无需具身预训练，运行延迟仅190ms，比现有想象-执行WAMs快4倍以上；研究发现跳过视频协同训练会导致更大性能下降

Conclusion: 视频预测在WAMs中的主要价值可能在于改善训练时的世界表示，而非测试时生成未来观测；Fast-WAM证明了高效实时控制的可行性，为具身智能提供了更实用的解决方案
...
TL;DR: 提出x²-Fusion方法，通过事件相机提供的时空边缘信号构建统一的Event Edge Space，将多模态融合重新定义为表示统一，实现2D光流和3D场景流的联合估计。


<details>
  <summary>Details</summary>
Motivation: 现有方法结合图像、LiDAR和事件数据预测2D和3D运动，但通常在分离的异构特征空间中操作，缺乏共享的潜在空间，导致跨传感器不匹配问题未解决，融合过程过于复杂。

Method: 利用事件相机提供的时空边缘信号作为内在边缘场，构建统一的Event Edge Space表示；将图像和LiDAR特征显式对齐到这个共享空间中；进行可靠性感知的自适应融合，估计模态可靠性并强调退化下的稳定线索；采用跨维度对比学习紧密耦合2D光流与3D场景流。

Result: 在合成和真实基准测试中，x²-Fusion在标准条件下达到最先进的精度，在挑战性场景下提供显著改进。

Conclusion: 通过事件边缘空间统一多模态表示，结合可靠性感知融合和跨维度对比学习，能够有效解决多模态融合中的表示对齐问题，提升动态场景理解的性能。
...
TL;DR: HMAR提出了一种分层模态感知专家和动态路由框架，通过双专家机制和两阶段对比学习，实现了医学图像的全局和局部特征自适应检索，无需昂贵标注，在RadioImageNet-CT数据集上取得了SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有医学图像检索系统存在三个主要问题：1）统一特征编码无法考虑解剖结构的临床重要性差异；2）基于粗粒度分类标签的模糊相似性度量；3）仅关注全局图像相似性，无法满足临床对细粒度区域特定检索的需求。

Method: 提出HMAR框架，基于混合专家架构：Expert0提取全局特征进行整体相似性匹配，Expert1学习位置不变的局部表示进行精确病灶区域检索。采用两阶段对比学习策略避免昂贵边界框标注，滑动窗口匹配算法实现密集局部比较，通过KAN层生成哈希码进行高效汉明距离搜索。

Result: 在RadioImageNet-CT数据集（16种临床模式，29,903张图像）上，HMAR在64位和128位哈希码上分别达到0.711和0.724的mAP，相比最先进的ACIR方法分别提升了0.7%和1.1%。

Conclusion: HMAR通过自适应检索框架有效解决了医学图像检索中的关键限制，实现了全局和局部特征的平衡，无需昂贵标注，在临床相关任务中表现出优越性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有物体运动编辑方法需要轨迹、边界框、掩码或运动场等额外输入，且现有评估基准混淆了物体运动和相机运动，缺乏专门针对稳定相机下纯物体运动的评估方法。

Method: 采用目标帧控制策略，利用首尾帧运动先验实现物体重定位；通过语义引导的物体插入和鲁棒背景修复构建可靠目标帧；利用早期自注意力图预测物体和相机动态，并提出ACE-Seed轻量搜索策略提升运动保真度。

Result: 在提出的S2M-DAVIS和S2M-OMB稳定相机物体运动评估基准上，以及FLF2V-obj和VBench指标上，Search2Motion均优于基线方法。

Conclusion: Search2Motion提供了一种无需训练、无需额外输入的物体运动编辑框架，通过目标帧控制和早期注意力机制实现了高质量的物体运动编辑，并提出了专门评估稳定相机下物体运动的基准和指标。
...
<details>
  <summary>Details</summary>
Motivation: 大班教学和师生互动有限成为教育挑战，需要可扩展的数据驱动工具实时捕捉学生情感和参与模式

Method: 开发物联网设备优化的实时多智能体情感计算框架，解决负载均衡和延迟问题，使用包含1500张标记图像和300个课堂检测视频的Classroom Emotion Dataset进行评估

Result: 系统在三个教育机构测试中表现稳健，可同时检测50张人脸（25FPS），课堂参与状态分类总体准确率达88%，获得学生、教师和家长积极反馈

Conclusion: 建立了实用的物联网情感感知学习环境框架，贡献了Classroom Emotion Dataset数据集，促进情感计算在教育领域的应用研究
...
<details>
  <summary>Details</summary>
Motivation: 视频扩散模型生成的视频帧之间缺乏3D一致性，导致难以重建3D世界
...
Conclusion: 该方法能够处理视频帧间的3D不一致性，生成高质量、可探索的3D环境
...
TL;DR: 提出基础设施教学的无标签3D感知范式，利用路边单元作为静态无监督教师，为自动驾驶车辆提供伪标签监督，无需人工标注


<details>
  <summary>Details</summary>
Motivation: 自动驾驶3D感知严重依赖大规模数据收集和人工标注，这在跨城市部署时变得不切实际。现代城市配备的路边单元为车辆训练提供了潜在机会

Method: 提出基础设施教学范式：路边单元从无标签数据中学习局部3D检测器，将预测广播给经过车辆，车辆聚合这些预测作为伪标签监督来训练独立的自我检测器

Result: 在CARLA多智能体环境中，使用CenterPoint实现了82.3%的AP（车辆检测），而完全监督的自我检测器上限为94.4%。证明了与现有自我中心无标签方法的互补性
...
TL;DR: 提出SLDDM-TPG方法，通过两阶段设计解决纺织图案生成中的特征混淆问题，实现高保真度的图案生成


<details>
  <summary>Details</summary>
Motivation: 现有图像到图像模型直接应用于纺织图案生成时，由于复杂纺织图案与服装图像固有的非刚性纹理变形之间的特征混淆，往往产生不忠实的结果，无法保留细粒度细节

Method: 两阶段方法：1）潜在解缠网络（LDN）解决服装表示中的特征混淆，构建多维独立的服装特征空间；2）半监督潜在扩散模型（S-LDM）接收LDN的指导信号，通过半监督扩散训练结合设计的细粒度对齐策略生成忠实结果

Result: 在CTP-HD数据集上FID降低4.1，SSIM提升高达0.116，在VITON-HD数据集上也表现出良好的泛化能力

Conclusion: SLDDM-TPG通过解缠特征表示和半监督扩散训练，有效解决了纺织图案生成中的特征混淆问题，实现了忠实且高质量的图案生成
...
TL;DR: SuCor是一种使用最优传输理论校正EPI图像磁化率几何畸变的方法，通过反相位编码图像对计算Wasserstein-2重心位移场，在谱域进行正则化，无需手动调参。


<details>
  <summary>Details</summary>
Motivation: 回波平面成像（EPI）存在磁化率引起的几何畸变问题，传统校正方法如FSL TOPUP需要手动调参且效果有限，需要更高效、自动化的校正方法。

Method: 使用反相位编码EPI图像对，将畸变场建模为Wasserstein-2重心位移，在谱域使用弯曲能量惩罚进行正则化，通过Morozov差异原理自动选择正则化强度。

Result: 在HCP数据集上，SuCor与T1结构图像的体积互信息达到0.341，优于FSL TOPUP的0.317，且单CPU核心仅需约12秒运行时间。

Conclusion: SuCor提供了一种高效、自动化的EPI畸变校正方法，在精度和速度上均优于现有方法，无需手动参数调整，具有临床应用潜力。
...
Result: 实验表明顶点帧范式具有挑战性，提出的解耦框架提高了识别性能
...
TL;DR: 提出GDPO方法，将强化学习整合到一步式生成图像超分辨率训练中，通过噪声感知扩散模型和分组直接偏好优化提升性能。


<details>
  <summary>Details</summary>
Motivation: 当前强化学习主要关注多步生成图像超分辨率，而一步式生成由于随机性有限而研究不足。现有方法如DPO需要离线生成正负样本对，样本数量有限；GRPO只计算整图似然，忽略对超分辨率至关重要的局部细节。

Method: 1) 提出噪声感知一步扩散模型，通过不等时间步策略解耦噪声添加和扩散的时间步；2) 提出GDPO策略，将GRPO原理整合到DPO中，计算在线生成样本的分组相对优势；3) 设计属性感知奖励函数，基于平滑和纹理区域的统计动态评估样本分数。

Result: 实验证明GDPO能有效提升一步式生成图像超分辨率模型的性能。

Conclusion: GDPO成功将强化学习整合到一步式生成图像超分辨率训练中，通过创新的噪声感知模型和分组优化策略解决了现有方法的局限性。
...
<details>
  <summary>Details</summary>
Motivation: 3D口腔内扫描（IOS）在牙科中应用日益广泛，但现有方法主要基于2D图像或多视图渲染，未能充分利用原生3D几何信息。开发直接处理3D IOS数据的统一诊断模型面临三大挑战：扫描形式异构且拓扑复杂、多疾病共存且类别不平衡、3D IOS-文本配对数据有限。

Method: 提出IOSVLM模型，采用点云表示扫描数据，遵循3D编码器-投影器-LLM架构。为解决无颜色IOS数据与依赖颜色的3D预训练之间的分布差异，提出几何到颜色代理方法。采用两阶段课程训练策略增强鲁棒性。同时构建大规模IOSVQA数据集，包含19,002个病例和249,055个VQA对。

Result: IOSVLM在统一诊断和生成式视觉问答任务上表现优异，相比强基线至少提升9.58%宏准确率和1.46%宏F1分数，证明了直接3D几何建模对IOS诊断的有效性。

Conclusion: IOSVLM是首个直接处理3D IOS点云数据的视觉语言模型，通过创新的几何到颜色代理和两阶段训练策略，成功解决了3D口腔诊断中的关键挑战，为临床文档和沟通提供了统一的多疾病诊断解决方案。
...
TL;DR: V-Co 系统研究了视觉协同去噪，提出了四个关键要素：完全双流架构、结构化的无条件预测、感知漂移混合损失和RMS特征重缩放，在ImageNet-256上优于现有像素扩散方法。


<details>
  <summary>Details</summary>
Motivation: 像素空间扩散模型缺乏强语义监督和高层视觉结构捕捉能力。现有视觉协同去噪方法设计选择混杂，不清楚哪些要素真正关键，需要系统研究来明确有效成分。

Method: 在统一的JiT框架下系统研究视觉协同去噪，提出四个关键要素：1) 完全双流架构保持特征特定计算和跨流交互；2) 结构化的无条件预测实现有效CFG；3) 感知漂移混合损失提供强语义监督；4) RMS特征重缩放实现跨流校准。

Result: 在ImageNet-256上，V-Co在可比模型规模下优于基础像素空间扩散基线和强先验像素扩散方法，同时使用更少的训练轮次。

Conclusion: 视觉协同去噪的有效性取决于四个关键设计要素，V-Co为未来表示对齐生成模型提供了实用指导。
...
<details>
  <summary>Details</summary>
Motivation: 现有动物深度估计模型大多基于无度量尺度的数据集训练，这限制了图像模型的验证能力。动物特别是野生动物具有挑战性的可变形特性，需要包含度量信息的多模态数据来提升深度估计和3D重建的可靠性。

Method: 提出了WildDepth数据集，包含从家养到野生动物的多样化类别，提供同步的RGB和LiDAR数据。建立了用于深度估计、行为检测和3D重建的基准测试套件，探索了多模态数据融合方法。

Result: 多模态数据使用使深度估计可靠性提升高达10% RMSE，RGB-LiDAR融合使3D重建保真度在Chamfer距离上提升12%。

Conclusion: WildDepth数据集和基准测试套件的发布旨在促进跨领域泛化的鲁棒多模态感知系统发展，为动物深度估计和3D重建研究提供有度量尺度的验证基础。
...
TL;DR: 提出一个电池感知的边缘辅助XR系统执行管理框架，通过轻量级深度强化学习策略在线调整执行决策，在动态网络条件下显著延长设备电池寿命同时保持高延迟合规性。
...
Method: 设计了一个电池感知的执行管理框架，联合考虑执行放置、工作负载质量、延迟要求和电池动态。采用基于轻量级深度强化学习的在线决策机制，在动态网络条件下持续调整执行决策。

Result: 相比延迟最优的本地执行，该方法将设备电池寿命延长了最多163%，在稳定网络条件下保持超过90%的运动到光子延迟合规性，即使在网络带宽严重受限时合规性也不低于80%。

Conclusion: 该研究证明了在沉浸式XR系统中明确管理延迟-能耗权衡的有效性，提出的电池感知框架能够在动态网络条件下显著延长设备电池寿命同时保持高延迟合规性。
...
TL;DR: 该论文系统评估了三种数据为中心的方法在遥感数据中处理标签噪声的性能，包括噪声识别和任务性能改进，为不同场景下的方法选择提供指导。


<details>
  <summary>Details</summary>
Motivation: 遥感数据集中普遍存在标签噪声问题，这会严重影响深度学习模型的泛化能力，但目前缺乏对数据为中心标签噪声处理方法的系统性分析，特别是在遥感领域的应用研究不足。

Method: 研究评估了三种数据为中心的方法，通过在两个基准数据集中注入10-70%的不同类型标签噪声，分析这些方法过滤标签噪声的效果及其对任务性能的影响。

Result: 研究证明数据为中心方法在标签噪声识别和任务性能改进方面都具有重要价值，为不同设置和目标下的最佳方法选择提供了见解，并指出了遥感领域应用中的研究需求。

Conclusion: 该研究是连接数据为中心标签噪声方法的方法论建立与其在遥感领域实际应用的重要一步，为遥感数据处理中的标签噪声问题提供了系统性的解决方案指导。
...
TL;DR: 本文研究了视觉Transformer中的位置偏差问题，特别是在材料科学等领域的零样本适应困难，通过线性探测发现位置偏差普遍存在，然后通过微调使用ALiBi相对位置编码来减少偏差，最终在复杂显微镜图像分割中展示了无偏特征的有效性。


<details>
  <summary>Details</summary>
Motivation: 视觉Transformer（尤其是像DINOv2这样的特征基础模型）学习到的丰富表示可用于许多下游任务，但架构选择（如位置编码）可能导致模型显示与语义内容无关的位置偏差和伪影。这使得在材料科学等领域的零样本适应变得困难，因为这些领域的图像通常是均匀微观结构的横截面（即没有首选方向）。

Method: 通过线性探测研究ViT中的位置偏差，发现它在各种目标和位置编码中都存在，然后通过微调模型使用ALiBi相对位置编码来减少这种偏差。

Result: 研究发现位置偏差在多种配置中都存在，通过使用ALiBi相对位置编码进行微调可以显著减少这种偏差。这些模型保留了理想的一般语义特征，其无偏特征可以成功用于复杂显微镜图像的可训练分割。

Conclusion: 通过采用ALiBi相对位置编码进行微调，可以有效减少视觉Transformer中的位置偏差，同时保持其语义表示能力，这对于材料科学等领域的图像分析应用具有重要意义。
...
TL;DR: M^3是一个结合多视图基础模型与单目高斯溅射SLAM的系统，通过专用匹配头实现细粒度密集对应，在动态环境中实现高精度位姿估计和场景重建。


<details>
  <summary>Details</summary>
Motivation: 当前从未标定单目视频进行流式重建面临挑战，需要高精度位姿估计和动态环境中的高效在线优化。虽然将3D基础模型与SLAM框架结合是有前景的范式，但大多数多视图基础模型以前馈方式估计位姿，产生的像素级对应缺乏几何优化所需的精度。

Method: M^3在多视图基础模型中增加专用匹配头以实现细粒度密集对应，并将其集成到鲁棒的单目高斯溅射SLAM中。系统还通过动态区域抑制和交叉推理内参对齐来增强跟踪稳定性。

Result: 在多样化的室内外基准测试中展示了最先进的位姿估计和场景重建精度。相比VGGT-SLAM 2.0，ATE RMSE降低了64.3%；在ScanNet++数据集上，PSNR比ARTDECO高出2.11 dB。

Conclusion: M^3通过增强多视图基础模型的匹配能力并集成到SLAM框架中，有效解决了从未标定单目视频进行流式重建的精度问题，在动态环境中实现了高精度的位姿估计和场景重建。
...
TL;DR: SOMA是一个统一的人体模型层，通过三个抽象层桥接SMPL、SMPL-X等异构人体表示，实现模型间的自由混合使用。
...
Method: 提出三个抽象层：1) 网格拓扑抽象：将任何源模型映射到共享规范网格；2) 骨骼抽象：从任意身体形状恢复完整的身份适应关节变换；3) 姿态抽象：从任何支持模型的姿态顶点恢复统一骨架旋转。整个流程完全可微分且GPU加速。

Result: 将O(M²)的成对适配问题简化为O(M)的单后端连接器，让从业者可以在推理时自由混合身份源和姿态数据。

Conclusion: SOMA提供了一个统一的体层，解决了异构人体表示之间的兼容性问题，实现了不同模型间的无缝集成和互操作。
...
TL;DR: SparkVSR：一个交互式视频超分辨率框架，通过稀疏关键帧作为控制信号，让用户能够引导超分辨率过程并修正伪影


<details>
  <summary>Details</summary>
Motivation: 现有VSR方法在推理时像黑盒，用户无法可靠地修正意外伪影，只能接受模型输出。需要一种让用户能够交互控制超分辨率过程的方法

Method: 提出基于关键帧的交互式VSR框架，采用关键帧条件化的潜空间-像素两阶段训练流程，融合LR视频潜变量和稀疏编码的HR关键帧潜变量，学习跨空间传播和细节精炼

Result: 在多个VSR基准测试中，CLIP-IQA、DOVER和MUSIQ指标分别提升24.6%、21.8%和5.6%，表现出更好的时间一致性和恢复质量。还能应用于老电影修复和视频风格迁移等任务

Conclusion: SparkVSR提供了一个通用、交互式、关键帧条件化的视频处理框架，实现了可控的视频超分辨率，让用户能够通过稀疏关键帧引导恢复过程
...
TL;DR: 提出MessyKitchens数据集和Multi-Object Decoder (MOD)方法，用于单目3D场景重建，解决杂乱环境中多物体形状、姿态和物理接触的精确重建问题。


<details>
  <summary>Details</summary>
Motivation: 单目3D场景重建虽在深度估计方面取得进展，但将复杂场景分解为独立3D物体仍面临挑战，特别是需要物理上合理的重建（避免穿透、真实接触）。现有数据集在物体接触和穿透方面精度不足。

Method: 1) 创建MessyKitchens数据集，提供杂乱真实场景的高保真物体级真值（3D形状、姿态、精确接触）。2) 基于SAM 3D方法扩展，加入Multi-Object Decoder (MOD)进行联合物体级场景重建。

Result: MessyKitchens在配准精度和物体间穿透方面显著优于先前数据集。MOD方法在三个数据集上相比现有技术取得一致且显著的改进。

Conclusion: 通过新数据集和MOD方法，提升了单目3D场景重建中多物体形状、姿态和物理接触的准确性，为机器人、动画等应用提供更物理合理的场景重建。
...
TL;DR: SegviGen利用预训练的3D生成模型进行3D部件分割，通过部件颜色化实现分割，相比现有方法大幅减少标注数据需求并提升性能。


<details>
  <summary>Details</summary>
Motivation: 现有3D部件分割方法存在两个问题：一是基于2D先验的方法存在视角不一致和边界模糊问题；二是原生3D分割方法需要大量标注数据和计算资源。SegviGen旨在利用预训练3D生成模型的结构化先验，实现高效且高质量的3D部件分割。

Method: SegviGen框架通过编码3D资产并在几何对齐重建的活跃体素上预测部件指示性颜色来实现分割。该方法支持交互式部件分割、完整分割以及带2D引导的完整分割的统一框架。
...
TL;DR: 视频扩散模型中的推理能力主要沿去噪步骤展开，而非先前认为的帧间链式推理。研究发现模型通过"步骤链"机制在早期探索多个候选解，逐步收敛到最终答案。


<details>
  <summary>Details</summary>
Motivation: 挑战先前关于视频生成模型中推理机制的传统假设（链式帧推理），揭示扩散模型中推理能力的真实运作机制，为理解视频模型的内在智能提供新视角。

Method: 通过定性分析和针对性探测实验，研究扩散去噪步骤中的推理过程，识别关键推理行为，并基于洞察提出无需训练的策略来改进推理能力。

Result: 发现推理主要沿去噪步骤展开（步骤链机制），识别出工作记忆、自我纠正与增强、感知先于行动等关键推理行为，以及扩散变换器中的功能专业化分层结构。

Conclusion: 视频生成模型的推理能力源于扩散去噪步骤中的动态过程，这一新理解为利用视频模型作为智能新基质的研究奠定了基础。
...
TL;DR: 本文提出了一种基于相机姿态的统一几何表示方法，通过将用户动作映射到6自由度相机姿态来提升视频扩散变换器在交互式游戏世界模型中的动作控制精度和长期3D一致性。


<details>
  <summary>Details</summary>
Motivation: 现有视频扩散变换器方法在交互式游戏世界模型中存在两个主要问题：1）动作控制不精确，通常将用户动作视为抽象条件信号，忽略了动作与3D世界之间的几何耦合关系；2）长期3D一致性不足，无法在长时间导航中保持空间一致性。

Method: 提出以相机姿态作为统一几何表示：1）定义基于物理的连续动作空间，将用户输入表示为李代数中的6自由度相机姿态，通过相机嵌入器注入生成模型以确保动作对齐；2）使用全局相机姿态作为空间索引检索相关历史观测，实现长期导航中位置的几何一致性重访。

Result: 构建了包含3000分钟真实人类游戏玩法的大规模数据集，带有相机轨迹和文本标注。实验表明，该方法在动作可控性、长期视觉质量和3D空间一致性方面显著优于现有最先进的交互式游戏世界模型。

Conclusion: 相机姿态作为统一几何表示能够有效解决交互式游戏世界模型中的动作控制精度和长期3D一致性问题，为构建更真实、可控的生成式游戏环境提供了新思路。
...
<details>
  <summary>Details</summary>
Motivation: 长上下文处理是语言模型的核心挑战，即使有扩展上下文窗口，模型仍难以可靠地提取、推理和使用长上下文信息。现有的递归语言模型（RLM）通过程序化交互分解长上下文，但其成功关键取决于上下文交互程序的选择，这一问题尚未得到充分探索。

Method: 提出SRLM框架，将程序化上下文交互与不确定性感知的自我反思相结合。利用三个内在信号作为模型内部不确定性的互补指标：自我一致性、推理长度和言语化置信度。模型使用这些信号来评估和比较候选的上下文交互程序。

Result: 在多样化的基准数据集、上下文长度和骨干模型上的广泛实验表明，SRLM始终优于最先进的基线方法，在相同时间预算下比RLM提升高达22%。研究发现递归本身不是RLM性能的主要驱动因素，简单的自我反思程序搜索就能匹配或超越RLM，无需自查询或显式递归机制。

Conclusion: SRLM通过不确定性感知的自我反思有效解决了长上下文处理中的程序选择问题，在语义密集型任务中表现优异，为长上下文语言模型提供了更可靠的解决方案。研究还揭示了递归方法在模型窗口内的上下文长度下可能降低性能，而SRLM在短长上下文中都能获得一致增益。
...
TL;DR: MedArena是一个交互式评估平台，让临床医生使用真实医疗问题直接测试和比较LLMs，发现传统静态基准无法反映临床实用性的差距。


<details>
  <summary>Details</summary>
Motivation: 当前医疗LLMs评估方法主要依赖静态模板化基准测试，无法捕捉真实临床实践的复杂性和动态性，导致基准性能与临床实用性之间存在脱节。

Method: 开发MedArena交互式平台，临床医生提交自己的医疗查询，系统随机展示两个LLMs的回答，用户选择偏好回答，收集偏好数据并采用Bradley-Terry评分模型排名。

Result: 收集1571个偏好数据，Gemini 2.0 Flash Thinking、Gemini 2.5 Pro和GPT-4o排名前三；仅1/3问题类似事实回忆任务，多数涉及治疗选择、临床文档、患者沟通等；临床医生更注重回答深度、细节和清晰度而非原始事实准确性。

Conclusion: MedArena通过基于真实临床问题和偏好的评估，为衡量和改进医疗LLMs的实用性和有效性提供了可扩展平台，揭示了临床实用性评估需要超越传统基准测试。
...
TL;DR: MiroThinker-1.7是用于复杂长程推理任务的研究智能体，通过结构化规划、上下文推理和工具交互提高可靠性。MiroThinker-H1在此基础上增加了局部和全局验证机制，在多个领域基准测试中达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 解决复杂长程推理任务中多步骤交互的可靠性问题，需要开发能够进行结构化规划、上下文推理和工具交互的研究智能体，并进一步通过验证机制确保推理过程的连贯性和证据支持。

Method: MiroThinker-1.7采用智能体中期训练阶段，强调结构化规划、上下文推理和工具交互。MiroThinker-H1在此基础上加入局部和全局验证机制：在推理过程中评估和精炼中间决策，同时审计整体推理轨迹以确保最终答案有连贯的证据链支持。

Result: 在涵盖开放网络研究、科学推理和金融分析的基准测试中，MiroThinker-H1在深度研究任务上达到最先进性能，同时在专业领域保持强劲结果。MiroThinker-1.7和MiroThinker-1.7-mini作为开源模型发布，提供具有显著改进效率的竞争性研究智能体能力。

Conclusion: MiroThinker系列智能体通过结构化规划、上下文推理和验证机制，有效解决了复杂长程推理任务的可靠性问题，在多个领域达到最先进性能，并通过开源模型促进了研究智能体技术的发展。
...
TL;DR: LLMs和分词方案在表示和生成阿拉伯语根-模式形态学方面的有效性研究，发现分词形态对齐对形态生成既非必要也非充分


<details>
  <summary>Details</summary>
Motivation: 研究大型语言模型及其分词方案如何有效表示和生成阿拉伯语根-模式形态学，探究它们是否捕捉了真正的形态结构还是依赖表面记忆。阿拉伯语形态系统为分析LLMs如何处理复杂的非连接形式以及分词选择如何影响这一过程提供了丰富的测试平台。

Method: 1. 评估阿拉伯语和多语言分词器在形态保真度方面与黄金标准分割的对比；2. 使用新开发的测试集分析LLM在生成性根-模式生成方面的性能；3. 在七个阿拉伯语中心和多语言LLM及其相应分词器上进行实验。

Result: 研究发现分词器的形态对齐对形态生成既非必要也非充分，这质疑了形态分词在下游性能中的作用。

Conclusion: 分词器的形态对齐不是LLM有效处理阿拉伯语形态学的关键因素，挑战了形态分词对下游任务性能重要性的传统假设。
...
TL;DR: 本文介绍了SemEval-2026 Task 5的同形词词义合理性评分系统，通过三种提示策略和模型集成方法，在竞赛中取得第四名成绩。


<details>
  <summary>Details</summary>
Motivation: 解决同形词在短篇故事中词义合理性评估的主观语义任务，该任务存在显著的标注者间差异，需要开发能够处理这种主观性的系统。

Method: 使用三种提示策略：零样本基线、思维链结构化推理、比较性提示策略；并采用模型集成方法平均多个LLM的预测结果。

Result: 官方最佳系统（三种提示策略的LLM集成）在竞赛中排名第四，准确率0.88，Spearman相关系数0.83（平均0.86）；赛后实验进一步提升至准确率0.92，相关系数0.85（平均0.89）。

Conclusion: 比较性提示策略在不同模型家族中持续提升性能，模型集成显著增强了与人类平均判断的一致性，表明LLM集成特别适合涉及多标注者的主观语义评估任务。
...
TL;DR: 该研究将演化博弈论与信息瓶颈框架结合，展示了通过不精确策略模仿的动态过程，种群如何演化出接近最优的词汇压缩效率。


<details>
  <summary>Details</summary>
Motivation: 自然语言被认为在信息瓶颈的复杂度-准确性权衡下演化出高效压缩含义的词汇，但驱动这种优化的社会动态机制尚不清楚。同时，演化博弈论虽能解释语言从基础智能体动态中涌现，但尚未验证是否能产生信息瓶颈意义上的高效压缩。

Method: 构建统一模型整合演化博弈论与信息瓶颈框架，在信号博弈中通过独立动机的不精确策略模仿动态，研究种群如何演化出接近最优的词汇压缩。

Result: 发现模型关键参数（调节博弈精确度和玩家混淆相似状态的倾向）约束了涌现词汇所实现权衡的变化范围，表明演化博弈动态可能为词汇演化提供机制基础。

Conclusion: 演化博弈动态可能为具有信息理论最优性和经验验证特性的词汇演化提供机制基础，统一了语言演化的功能解释与涌现过程解释。
...
<details>
  <summary>Details</summary>
Motivation: ClinicalTrials.gov作为最大的临床试验公共注册库，其注册导向的架构和异构的不良事件术语限制了系统性的药物警戒分析。不良事件通常以研究者报告的文本形式记录，而非标准化标识符，需要人工核对才能识别一致的安全概念。

Method: 开发了ClinicalTrials.gov Transformation Database (CTG-DB)，这是一个开源管道，能够摄取完整的CT.gov XML存档，并生成一个关系数据库，使用监管活动医学词典(MedDRA)将不良事件术语标准化。该系统保留组别级别的分母，表示安慰剂和对照组，并使用确定性的精确和模糊匹配来标准化AE术语，确保透明和可重复的映射。

Result: CTG-DB框架实现了概念级别的检索和跨试验聚合，支持可扩展的安慰剂参考安全分析，并将临床试验证据整合到下游的药物警戒信号检测中。

Conclusion: CTG-DB解决了ClinicalTrials.gov数据在系统性药物警戒分析中的局限性，通过标准化不良事件术语和提供关系数据库结构，为大规模、可重复的安全分析提供了有效工具。
...
TL;DR: BANGLASOCIALBENCH是首个评估孟加拉语社会语用能力的基准，包含1,719个文化情境实例，评估显示当前LLM在孟加拉语社交互动中存在系统性文化错位问题。


<details>
  <summary>Details</summary>
Motivation: 虽然大语言模型展现出多语言流利性，但流利性不能保证社会适当的语言使用。孟加拉语作为高语境语言，其交际能力需要对社会等级、关系角色和互动规范的敏感性，这些直接编码在日常语言中。

Method: 创建BANGLASOCIALBENCH基准，涵盖三个领域：孟加拉语称呼术语、亲属关系推理和社会习俗，包含1,719个由母语者编写和验证的文化情境实例。在零样本设置下评估12个当代LLM。

Result: 模型表现出系统性文化错位模式：经常默认使用过于正式的称呼形式，无法识别多个社会可接受的称呼代词，在不同宗教背景下混淆亲属术语。社会语用失败往往是结构性和非随机的。

Conclusion: 当前LLM在推断和应用现实孟加拉社会互动中文化适当语言使用方面存在持续局限性，社会语用能力评估需要超越事实回忆，关注情境依赖的语言使用。
...
TL;DR: POLAR是一种在嵌入空间中运行的按用户词汇关联测试方法，使用轻量适配的掩码语言模型，通过私有确定性令牌表示作者，并投影到词汇轴上，提供标准化效应和统计显著性检验。


<details>
  <summary>Details</summary>
Motivation: 现有的大多数内在关联探测方法在单词、句子或语料库层面操作，掩盖了作者层面的变异。需要一种能够捕捉个体作者语言特征变化的方法，用于计算社会科学中的作者诊断。

Method: POLAR在轻量适配的掩码语言模型的嵌入空间中运行，使用私有确定性令牌表示作者，将这些向量投影到精心策划的词汇轴上，报告标准化效应，并提供置换p值和Benjamini-Hochberg控制。

Result: 在平衡的机器人-人类Twitter基准测试中，POLAR清晰地区分了LLM驱动的机器人和有机账户；在极端主义论坛上，它量化了与侮辱词汇库的强对齐，并揭示了随时间向右漂移的趋势。
...
TL;DR: 提出HAT架构，通过字节级编码器-解码器和预训练主干网络，实现无需分词器的语言模型，在压缩和鲁棒性方面优于传统分词方法。


<details>
  <summary>Details</summary>
Motivation: 当前基于学习分词器的LLMs存在词汇表固定、难以适应新领域/语言、对单词内部变化敏感等问题，需要更灵活高效的文本处理方案。

Method: 采用分层自回归Transformer架构：编码器将字节聚合为词嵌入，预训练主干网络处理词嵌入，解码器通过交叉注意力将输出转换回字节。重用Llama 3.1预训练模型，仅训练编码器和解码器。

Result: HAT架构显著改善文本压缩（减少序列位置需求），增强对单词内部变化的鲁棒性（如拼写差异）。在英语和德语基准测试中，多数情况下优于原始Llama 3.1。

Conclusion: HAT架构提供了一种无需分词器的替代方案，通过字节级处理实现了更好的压缩效率和鲁棒性，同时保持了强大的语言建模能力，为多语言应用提供了新可能。
...
TL;DR: MoLoRA提出基于token的路由机制，支持多模态和混合能力请求，通过混合LoRA适配器实现组合式专业化，小模型性能超越大模型。


<details>
  <summary>Details</summary>
Motivation: 传统多适配器服务系统将整个序列路由到单个适配器，无法处理多模态生成（文本和图像token需要不同适配器）和混合能力请求（需要多个专业适配器）。

Method: 提出基于token的路由机制：根据词汇结构（多模态）或学习门控（语义专业化）将单个token路由到适配器。核心是MoLoRA（混合LoRA），支持组合式专业化，加载多个领域特定适配器，通过学习的路由器为每个token选择合适适配器。

Result: 专业化显著优于规模扩展：MoLoRA使Qwen3-1.7B在四个推理基准测试中超越Qwen3-8B，同时模型大小小4.7倍。证明基于token的路由在理论上最优，工作量为N（N个token）而非K·N（K种适配器类型）。

Conclusion: MoLoRA支持推理时的模块化专业知识：独立训练专注的LoRA，无需重新训练即可组合，通过简单加载新适配器添加新能力，实现高效灵活的专业化服务。
...
TL;DR: 本文提出了一种基于SVM的罗曼什语方言识别系统，能够区分罗曼什语的多种区域变体（方言）以及超区域变体Rumantsch Grischun，在新建的基准测试中达到97%的准确率。


<details>
  <summary>Details</summary>
Motivation: 罗曼什语有多种区域变体（方言），相互理解度有限，但此前缺乏能够区分这些方言的语言识别系统。由于还需要识别结合多种方言元素的超区域变体Rumantsch Grischun，这构成了一个新颖且有趣的分类问题。

Method: 采用支持向量机（SVM）方法构建罗曼什语方言识别系统，并在新构建的跨两个领域的基准测试上进行评估。

Result: 模型在领域内平均准确率达到97%，能够支持方言感知的拼写检查和机器翻译等应用。分类器已公开可用。

Conclusion: 成功开发了首个能够有效区分罗曼什语多种方言及超区域变体的语言识别系统，为罗曼什语的语言技术应用提供了重要工具。
...
TL;DR: 提出PALLM模型，通过多任务强化学习和思维链提示来增强语音大语言模型对副语言线索（如语调、情感）的理解能力，在多个数据集上超越现有基线模型8-12%


<details>
  <summary>Details</summary>
Motivation: 语音大语言模型需要理解副语言线索（韵律、情感、非语言声音）来准确理解意图，但面临训练数据有限、标注困难、模型倾向于利用词汇捷径而非副语言信号等挑战

Method: 提出PALLM模型，采用多任务强化学习结合思维链提示来激发显式情感推理。通过两阶段流程联合优化音频情感分类和副语言感知的响应生成
...
TL;DR: 基于简历数据检测职业涌现的新方法：将职业定义为词汇与从业者群体之间的双向强化结构（共吸引子），发现AI形成了专业词汇但未形成凝聚的从业群体，表明AI是扩散技术而非新兴职业。


<details>
  <summary>Details</summary>
Motivation: 传统职业分类系统更新缓慢，无法及时追踪职业的快速形成和演变。需要一种无需预定义分类或职位名称的方法来检测职业涌现。

Method: 提出"共吸引子"概念：职业是专业词汇与从业者群体之间的双向强化结构。开发零假设方法，独立测试词汇凝聚力和人口凝聚力，并通过消融实验验证词汇是否作为绑定人口的机制。应用于820万份美国简历数据（2022-2026）。

Result: 方法能正确识别已确立的职业。AI领域显示出显著不对称性：专业词汇在2024年初迅速形成，但从业者群体从未凝聚。原有AI社区随着工具主流化而解散，新词汇被吸收到现有职业中而非绑定新职业。

Conclusion: AI是扩散技术而非新兴职业。引入"AI工程师"职业类别可能催化已形成词汇周围的人口凝聚力，从而完成共吸引子结构。
...
TL;DR: RadAnnotate：基于LLM的放射学报告标注框架，通过检索增强的合成报告和基于置信度的选择性自动化，显著减少专家标注工作量


<details>
  <summary>Details</summary>
Motivation: 放射学报告标注对临床NLP至关重要，但人工标注耗时耗力且成本高昂，需要开发自动化方法来减少专家工作量

Method: 1) 在标准报告上训练实体特定分类器；2) 生成RAG引导的合成报告；3) 学习实体特定置信度阈值实现选择性自动化标注

Result: 1) 合成报告模型与黄金标准模型F1分数相差仅1-2分；2) 合成数据增强在低资源环境下对不确定观察结果特别有效（F1从0.61提升到0.70）；3) RadAnnotate可自动标注55-90%报告，实体匹配分数达0.86-0.92

Conclusion: RadAnnotate框架通过合成数据生成和选择性自动化，显著减少了放射学报告标注的专家工作量，为临床NLP提供了高效实用的解决方案
...
TL;DR: 该研究分析了LLM在道德推理过程中的框架切换模式，发现道德推理涉及系统性多框架审议，框架不稳定的轨迹更容易受到说服性攻击，并提出了道德表征一致性指标来评估LLM的道德连贯性。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型越来越多地参与道德敏感决策，但它们在推理步骤中如何组织伦理框架仍未被充分探索。研究者希望理解LLM在道德推理过程中的框架切换动态和稳定性。

Method: 引入"道德推理轨迹"概念，分析六个模型在三个基准测试中的框架调用序列。使用线性探针定位框架特定编码的模型层，通过激活引导调节框架整合模式，并提出道德表征一致性（MRC）指标。

Result: 发现55.4-57.7%的连续步骤涉及框架切换，仅16.4-17.8%的轨迹保持框架一致；不稳定轨迹对说服性攻击的敏感性高1.29倍；线性探针在特定层（Llama-3.3-70B的63/81层，Qwen2.5-72B的17/81层）检测到框架编码；MRC指标与LLM连贯性评分强相关（r=0.715）。

Conclusion: LLM的道德推理涉及复杂的多框架审议过程，框架不稳定性会影响模型的鲁棒性。提出的MRC指标能有效评估道德连贯性，为理解和改进LLM的道德推理能力提供了新视角。
...
TL;DR: SEAHateCheck是首个针对东南亚低资源语言（印尼语、泰语、菲律宾语、越南语）的仇恨言论检测功能测试数据集，基于HateCheck框架构建，通过大语言模型生成文化相关测试案例并经本地专家验证，揭示了现有模型在检测这些语言仇恨言论时的局限性。


<details>
  <summary>Details</summary>
Motivation: 当前仇恨言论检测主要依赖英语和中文等高资源语言，而东南亚低资源语言由于缺乏语言资源和复杂的社会语言环境，导致在线仇恨内容审核工具开发困难。需要专门针对这些语言的文化相关测试数据集来推动实用化仇恨言论检测工具的发展。

Method: 基于HateCheck的功能测试框架，改进SGHateCheck的方法，使用大语言模型生成文化相关的测试案例，并由本地专家验证准确性。针对印度尼西亚、泰国、菲律宾和越南的四种语言（印尼语、泰语、菲律宾语、越南语）构建SEAHateCheck数据集。

Result: 实验发现现有最先进的多语言模型在检测低资源语言仇恨言论方面存在明显局限：菲律宾语测试案例准确率最低（可能由于语言复杂性和训练数据有限）；俚语功能测试最难（模型难以处理文化细微表达）；模型在隐式仇恨检测和反言论表达识别方面表现不佳。

Conclusion: SEAHateCheck作为首个针对这些东南亚语言的功能测试套件，为研究人员提供了强大的基准测试工具，有助于推动实用、文化适应的仇恨言论检测工具开发，促进包容性在线内容审核。
...
TL;DR: ClaimFlow：基于科学主张的文献分析框架，通过人工标注的1084个主张和832个跨论文主张关系，构建NLP文献的主张演化图谱，并定义新的主张关系分类任务。


<details>
  <summary>Details</summary>
Motivation: 现有引用和主张分析方法只能捕捉科学对话的片段，无法完整展现科学主张之间的支持、扩展、限定、反驳等复杂互动关系。需要建立更精细的主张层面分析框架来理解科学思想的演进。

Method: 从304篇ACL Anthology论文（1979-2025）中人工标注1084个主张和832个跨论文主张关系，构建ClaimFlow数据集。定义主张关系分类任务，评估神经模型和大语言模型性能。将模型应用于约13k篇NLP论文进行历时分析。

Result: 基线模型达到0.78宏F1分数，表明主张关系分类可行但有挑战。历时分析显示：63.5%的主张从未被重用；仅11.1%的主张曾被挑战；广泛传播的主张更多通过限定和扩展被重塑，而非直接确认或反驳。

Conclusion: ClaimFlow为理解NLP领域思想演变提供了新视角，也为评估模型理解科学论证能力奠定了基础。主张层面的分析揭示了科学对话的复杂动态，大多数主张未被重用，挑战相对罕见，而广泛传播的主张往往通过重塑而非直接确认/反驳来演进。
...
TL;DR: CounterRefine：一个轻量级推理时修复层，通过检索证据测试和修正初步答案，显著提升事实问答准确性


<details>
  <summary>Details</summary>
Motivation: 在事实问答中，许多错误并非检索失败，而是承诺失败——系统检索到相关证据但仍给出错误答案。现有方法仅将检索作为收集更多上下文的手段，而非测试和修正答案的机制。

Method: CounterRefine采用三步法：1) 基于检索证据生成初步答案；2) 基于该答案进行后续查询，收集支持性和冲突性证据；3) 应用受限的细化步骤，输出KEEP或REVISE，修订只有在通过确定性验证后才被接受。

Result: 在SimpleQA基准测试中，CounterRefine将匹配的GPT-5 Baseline-RAG提升了5.8个百分点，达到73.1%的正确率，比报告的one-shot GPT-5.4分数高出约40个百分点。

Conclusion: 知识型基础模型不仅需要能够访问证据，还应能够使用这些证据重新考虑并在必要时修复自己的答案。检索应成为测试临时答案而非仅仅收集更多上下文的机制。
...
<details>
  <summary>Details</summary>
Motivation: 现有LLM后训练压缩方法较少关注校准数据的选择，而校准数据对保持模型在任务内和任务间的能力至关重要。需要一种高效、模型无关的数据选择策略。

Method: 提出ZipCal方法，基于Zipfian幂律最大化词汇多样性来选择校准数据，不依赖模型特定信号，具有线性复杂度。

Result: ZipCal在多种剪枝基准测试中一致优于标准均匀随机采样，与依赖模型困惑度的SOTA方法性能相当，但速度平均快约240倍。

Conclusion: ZipCal提供了一种高效、可扩展的校准数据选择方案，解决了大规模模型和数据集下传统方法计算成本过高的问题。
...
<details>
  <summary>Details</summary>
Motivation: 现有训练免费方法（如GEPA和ACE）在金融推理基准FAMMA上提升有限，暴露了非结构化文本优化在复杂多步领域推理中的局限性。需要一种无需微调、不修改模型权重的实用领域适应方法。

Method: ASDA框架通过迭代式错误纠正学习自动生成结构化技能文件：教师模型分析学生模型在金融推理任务中的失败案例，按子领域和错误类型聚类错误，合成包含推理流程、代码模板和工作示例的技能文件，在推理时动态注入。

Result: 在FAMMA基准测试中，ASDA在算术推理上提升高达+17.33%，在非算术推理上提升+5.95%，显著优于所有训练免费基线方法。

Conclusion: ASDA提供了一种实用且可审计的领域适应路径，无需权重访问或重新训练，生成的技能文件可读、版本可控，兼容Agent Skills开放标准，适合任何拥有标注领域数据集的组织。
...
TL;DR: MyScholarQA是一个个性化深度研究工具，通过推断用户研究兴趣、提供个性化行动建议并生成遵循用户批准行动的多章节报告，超越了传统DR工具。虽然LLM评估显示其在引用指标和个性化行动遵循方面优于基线，但真实用户访谈揭示了LLM无法检测的九个细微错误。


<details>
  <summary>Details</summary>
Motivation: 现有深度研究工具（如OpenAI DR）虽然能合成科学论文回答研究者查询，但缺乏对用户的理解。作者旨在开发一个能够理解用户研究兴趣并提供个性化服务的DR工具，同时探索LLM评估在个性化研究中的局限性。

Method: 1) 开发MyScholarQA系统，包含三个核心功能：推断用户研究兴趣、为输入查询提供个性化行动建议、生成遵循用户批准行动的多章节报告。2) 使用NLP标准协议测试：设计合成用户和LLM法官的基准测试。3) 通过在线版本的MySQA进行用户访谈，揭示LLM评估无法检测的个性化DR错误。

Result: 在LLM评估中，MySQA在引用指标和个性化行动遵循方面优于基线。然而，用户访谈揭示了九个LLM法官无法检测的细微错误，包括个性化DR的复杂性和用户期望的细微差别。这些发现表明LLM评估在捕捉个性化质量方面存在局限性。

Conclusion: 个性化研究工具的真正进步需要真实用户的参与。虽然LLM评估在NLP研究中易于使用，但可能忽视个性化的重要方面。未来的DR设计应结合真实用户反馈，而不仅仅是依赖自动化的LLM评估。
...
<details>
  <summary>Details</summary>
Motivation: 虽然衰减式学习率调度器在大型语言模型预训练中被广泛使用以最小化预训练损失，但它们对监督微调（SFT）后下游性能的影响尚未得到充分研究。研究者希望探索不同学习率调度策略如何影响模型在下游任务中的适应性。

Method: 研究比较了Warmup-Stable-Only（WSO）调度器（预热后保持恒定学习率）与传统的衰减式调度器。在1B和8B参数模型上进行实验，分析了不同训练阶段（中期训练和过度训练）的表现，并通过损失景观分析探索了优化轨迹。

Result: WSO调度器在监督微调后的下游任务性能上始终优于衰减式调度器，尽管衰减式调度器在预训练阶段可能表现更好。损失景观分析显示衰减式调度器导致模型陷入更尖锐的最小值，而WSO保持更平坦的最小值，支持更好的适应性。

Conclusion: 在预训练阶段使用学习率衰减来改善预训练指标可能会损害下游适应性。WSO调度器通过保持平坦的最小值增强了模型对下游任务的适应性，为训练和模型发布策略提供了实用指导。
...
TL;DR: 该研究首次大规模比较AI代理与人类在线社区，发现AI社区存在极端参与不平等、高跨社区作者重叠、情感扁平化、认知偏向断言而非探索、社交疏离等特征，这些差异主要源于共享作者的结构性因素而非内容同质化。


<details>
  <summary>Details</summary>
Motivation: 随着基于LLM的自主代理在社交平台上日益增多，理解AI代理社区的动态对传播研究和平台治理变得至关重要。需要实证比较AI代理与人类在线社区的差异，以了解多智能体互动如何产生不同于人类社区的集体传播动态。

Method: 研究分析了73,899个Moltbook（AI代理社区）帖子和189,838个Reddit帖子，覆盖五个匹配的社区。从结构和语言属性两个维度进行比较：结构上分析参与不平等（基尼系数）和跨社区作者重叠；语言上分析情感表达、认知模式（断言vs探索）和社交连接性。

Result: 结构上：Moltbook表现出极端参与不平等（基尼系数0.84 vs 0.47）和高跨社区作者重叠（33.8% vs 0.5%）。语言上：AI生成内容情感扁平化、认知偏向断言而非探索、社交疏离。社区层面看似同质化，但主要是共享作者的结构性产物。个体代理比人类用户更易识别，源于其极端发帖量放大的异常风格特征。

Conclusion: AI代理社区展现出与人类社区显著不同的集体传播动态，这些差异主要源于结构因素而非内容同质化。研究为理解AI中介传播如何重塑在线话语提供了实证基础，对平台治理和传播研究具有重要意义。
...
TL;DR: SciZoom是一个包含44,946篇ML论文的基准数据集，涵盖2020-2025年，分为LLM前和LLM后时代，提供三个层次摘要（Abstract、Contributions、TL;DR），用于多粒度摘要研究和科学写作模式演变分析。


<details>
  <summary>Details</summary>
Motivation: AI研究爆炸式增长导致信息过载，需要超越传统摘要的多粒度科学摘要。现有基准规模有限、仅针对单一粒度，且早于LLM时代。同时，ChatGPT发布后研究人员使用LLM撰写论文，但缺乏分析这种写作演变的数据资源。

Method: 从四个顶级ML会议（NeurIPS、ICLR、ICML、EMNLP）收集44,946篇论文（2020-2025年），明确分为Pre-LLM和Post-LLM时代。提供三个层次摘要目标：Abstract、Contributions和TL;DR，压缩比高达600:1。进行语言分析揭示写作模式变化。

Result: 语言分析显示显著变化：公式化表达模式增加高达10倍，修辞风格变化（对冲性语言下降23%），表明LLM辅助写作产生更自信但同质化的文本。数据集可作为挑战性基准和独特资源。

Conclusion: SciZoom填补了多粒度科学摘要基准的空白，同时为分析生成式AI时代科学话语演变提供了独特资源。数据集和代码已公开，支持多粒度摘要研究和科学写作模式挖掘。
...
Method: 提出SI框架：1) 合成高质量自然语言语料库，结合结构化知识图谱和非结构化行为日志，增强推理链和安全感知数据；2) 基于深度上缩放的参数高效预训练策略注入领域知识；3) 通过多任务指令调优和对抗训练的双路径对齐方法

Result: 已在京东（中国最大的自营电商平台）部署，在五个核心搜索场景的A/B测试显示关键业务指标显著改善，验证了工业有效性和可扩展性

Conclusion: SI框架成功解决了电商搜索LLM的知识幻觉和安全漏洞问题，实现了工业级部署，为电商搜索的智能化转型提供了可行方案
...
TL;DR: 论文提出PSII框架，通过在LLM隐藏状态中注入参数化的社会身份表示，解决LLM模拟公众意见时的多样性崩溃问题，显著提升分布保真度和多样性。


<details>
  <summary>Details</summary>
Motivation: 当前基于LLM的公众意见模拟方法虽然可扩展，但无法捕捉社会多样性，导致群体间差异扁平化和群体内响应过于同质化。研究发现LLM隐藏表示中存在多样性崩溃现象，不同社会身份在深层网络中变得难以区分。

Method: 提出参数化社会身份注入（PSII）框架，将明确的人口统计属性和价值取向的参数化表示直接注入LLM的中间隐藏状态中。与基于提示的人物设定不同，PSII在表示层面实现细粒度、可控的身份调节。

Result: 在多个开源LLM上使用世界价值观调查进行的广泛实验表明，PSII显著提高了分布保真度和多样性，减少了与真实世界调查数据的KL散度，同时增强了整体多样性。

Conclusion: 这项工作为LLM代理的表示级控制提供了新见解，并推进了可扩展、具有多样性意识的公众意见模拟。PSII框架在提升LLM模拟社会多样性方面表现出显著效果。
...
TL;DR: Polyglot-Lion是一个紧凑的多语言语音识别模型家族，专门针对新加坡的四种语言（英语、普通话、泰米尔语、马来语），通过平衡采样策略微调预训练模型，在保持竞争力的同时大幅降低了训练成本和推理时间。


<details>
  <summary>Details</summary>
Motivation: 针对新加坡多语言环境（英语、普通话、泰米尔语、马来语）开发紧凑、高效的多语言语音识别模型，解决现有大模型训练成本高、推理速度慢的问题。

Method: 通过平衡采样策略（每种语言训练话语数量相等）微调Qwen3-ASR-0.6B和Qwen3-ASR-1.7B模型，故意省略语言标签条件，让模型从音频中隐式学习语言识别。

Result: Polyglot-Lion-1.7B在12个基准测试中平均错误率为14.85，与6倍大的MERaLiON-2-10B-ASR模型（14.32）相当，训练成本仅81美元（vs 18,862美元），推理速度约快20倍（0.10 s/sample vs 2.02 s/sample）。

Conclusion: 通过语言平衡的微调中等规模预训练模型，可以以极低成本获得部署就绪的多语言语音识别系统，性能与更大规模的专用系统相当。
...
TL;DR: S2C是一种新颖的多维度越狱攻击框架，通过结构化语义伪装来操纵模型推理过程中的恶意意图重建，延迟和重组语义整合以绕过安全机制。


<details>
  <summary>Details</summary>
Motivation: 现代LLM的安全机制已扩展到潜在语义表示和生成时推理，能够恢复被模糊化的恶意意图并拒绝响应，使得许多表面级模糊化越狱攻击失效。因此需要开发更先进的攻击方法来绕过这些深层安全检测。

Method: 提出结构化语义伪装(S2C)框架，包含三个互补机制：1)上下文重构：将请求嵌入高风险的合理场景中，使模型倾向于服从；2)内容碎片化：将请求的语义特征分散到不相交的提示段中；3)线索引导伪装：伪装残余语义线索，同时嵌入可恢复的标记来指导输出生成。

Result: 在HarmBench和JBB-Behaviors基准测试中，S2C分别将攻击成功率(ASR)提高了12.4%和9.7%，优于当前SOTA。在GPT-5-mini上表现尤为突出，在JBB-Behaviors上比最强基线高出26%。

Conclusion: S2C通过延迟和重组语义整合，有效降低了依赖于解码时连贯或显式重建恶意意图的安全触发器的效果，同时保持了足够的指令可恢复性以生成功能性输出。该框架揭示了安全机制与攻击方法之间的持续对抗。
...
TL;DR: SpecSteer：一种非对称协作推理框架，通过结合本地隐私上下文与云端推理能力，解决个性化智能中的隐私与能力矛盾，实现高质量个性化生成同时保护用户隐私。
...
Method: 提出SpecSteer框架，将协作视为贝叶斯知识融合，并将推测性解码重新用作分布式对齐协议。采用"草稿-验证-恢复"流程：设备端模型生成个性化序列；云端通过基于比率的机制验证，将推理验证与隐私上下文解耦；拒绝时通过转向恢复在修正中注入本地意图。

Result: 实验表明SpecSteer成功弥合了推理差距，实现了卓越的个性化生成性能，同时相比标准基线实现了2.36倍的加速。

Conclusion: SpecSteer通过非对称协作推理框架有效解决了个性化智能中的隐私与能力权衡问题，在保护用户隐私的同时提供高质量的个性化生成能力。
...
<details>
  <summary>Details</summary>
Motivation: 探索多轮评审（让评审者可以提问、作者回答、再评审）是否比单轮跨上下文评审（CCR）能更好地提升LLM验证效果。

Method: 在30个工件和150个注入错误的控制实验中，测试四种D-CCR变体与单轮CCR基线对比，包括带问答交换的D-CCR-2b等不同配置。

Result: 单轮CCR（F1=0.376）显著优于所有多轮变体，如D-CCR-2b（F1=0.303）。多轮评审提高了召回率（+0.08）但产生62%更多虚假阳性（8.5 vs. 5.2），精确度从0.30降至0.20。

Conclusion: 多轮评审导致性能下降的两个机制：虚假阳性压力（后续轮次中当真实错误耗尽时编造发现）和评审目标漂移（从评审工件转向批评对话本身）。问题不是评审者看到什么信息，而是再次评审会引入噪声。
...
Method: 两阶段实验：11名专家和新手转录员对三种对话类型进行手动和ASR辅助转录，结合统计建模、词级对齐和注释指标分析

Result: ASR辅助工作流可提高转录速度，但不能持续改善整体准确性，效果取决于工作流配置、对话类型和标注者经验

Conclusion: 尽管有局限性，ASR辅助转录（可能通过任务特定微调支持）可集成到KIParla工作流中，在不影响质量的前提下加速语料库创建
...
TL;DR: 提出AEG框架，利用语音大语言模型的跨模态注意力机制显式定位证据，通过LFE训练范式校准注意力分布，在多个QA数据集上减少幻觉并提升效率。
...
Method: 提出注意力引导证据定位(AEG)框架，利用SpeechLLMs的内部跨模态注意力在潜在空间中显式定位关键证据；设计学习聚焦证据(LFE)监督微调范式，校准注意力机制以区分相关和无关内容。

Result: 在SQuAD、HotpotQA和MuSiQue数据集上实验显示，AEG减少幻觉并实现显著效率提升，超越大规模级联基线(Whisper-Large-v3 + Reranker)，推理延迟降低约62%。

Conclusion: AEG框架通过显式证据定位和注意力校准，有效解决了口语问答中的跨模态对齐问题，在准确性和效率方面均优于传统级联方法。
...
TL;DR: PyPhonPlan是一个用于语音规划动力学建模的Python工具包，基于耦合动态神经场和任务动力学模拟，支持模块化组件定义和交互式语音动态建模。


<details>
  <summary>Details</summary>
Motivation: 为语音通信研究提供一个开源、可复现、可扩展的计算工具，实现具有时间原则性、神经基础和语音丰富性的交互式语音动态建模框架。

Method: 使用耦合动态神经场和任务动力学模拟，提供模块化组件用于定义规划、感知和记忆场，以及场间耦合、手势输入，并通过场激活配置文件求解轨迹变量。

Result: 开发了PyPhonPlan工具包，通过生产/感知循环与耦合记忆场的示例应用展示了框架能力，实现了开源发布并包含可执行示例。

Conclusion: PyPhonPlan为语音通信研究提供了一个促进可复现性、可扩展性和累积计算发展的开源工具，能够建模具有时间原则性、神经基础和语音丰富性的交互式语音动态。
...
<details>
  <summary>Details</summary>
Motivation: 当前多语言机器翻译系统仅覆盖约200种目标语言，与世界7000多种语言相比覆盖率极低，且缺乏可靠的评估基准和指标来衡量现有系统的真实能力。

Method: 采用综合数据策略整合大规模公共多语言语料库和新创建数据集（包括手动整理的MeDLEY双语文本），探索两种LLM专业化方法：仅解码器模型（OMT-LLaMA）和编码器-解码器架构中的模块（OMT-NLLB）。

Result: 1B-8B参数模型均匹配或超越70B LLM基线，OMT-LLaMA大幅扩展了可生成连贯文本的语言集合，在英语到1600种语言的翻译评估中，OMT模型在跨语言迁移方面接近解决"理解"难题。

Conclusion: OMT系统实现了前所未有的1600+语言覆盖，展示了专业化模型在低计算环境下的优势，并提供了动态演进的评估基准（BOUQuET和Met-BOUQuET）推动全语言机器翻译研究。
...
TL;DR: PashtoCorp是一个12.5亿词的普什图语语料库，比现有最大语料库大83倍，显著提升了普什图语NLP任务的性能。


<details>
  <summary>Details</summary>
Motivation: 普什图语有6000万使用者，但在NLP领域严重缺乏代表性。现有语料库规模有限，限制了普什图语NLP模型的发展。

Method: 从39个来源收集数据，包括7个HuggingFace数据集和32个专门构建的网络爬虫。使用可复现的流水线处理，包括阿拉伯文字符分词、SHA-256去重和质量过滤。

Result: 语料库规模达12.5亿词、281万文档。在XLM-R-base上继续预训练使困惑度降低25.1%。在WikiANN NER任务上实体F1提升10%，训练方差降低近7倍。在Belebele阅读理解任务上达到64.6%准确率。

Conclusion: PashtoCorp显著提升了普什图语NLP性能，特别是维基百科数据对NER任务至关重要。语料库、模型和代码已开源。
...
TL;DR: Fanar 2.0是卡塔尔第二代阿拉伯语生成式AI平台，在资源有限（256个H100 GPU）和数据稀缺（阿拉伯语仅占网络数据0.5%）条件下，通过数据质量优先、定向持续预训练和模型融合策略，开发出性能显著提升的阿拉伯语大模型和多样化AI能力套件。


<details>
  <summary>Details</summary>
Motivation: 解决阿拉伯语AI发展面临的核心挑战：1）主权需求，确保AI系统完全自主可控；2）资源约束，仅有有限计算资源；3）数据稀缺，阿拉伯语网络数据仅占0.5%，与4亿母语者规模严重不匹配；4）需要开发符合阿拉伯文化和伊斯兰价值观的AI系统。

Method: 采用数据质量优先策略而非数据规模优先，对Gemma-3-27B进行持续预训练，使用1200亿高质量token的精选语料库。采用模型融合技术，开发多组件AI套件：包括4B参数的双语安全过滤器、长音频ASR模型、阿拉伯语视觉理解、多代理工具调用框架、伊斯兰内容多代理架构、古典诗歌生成、双语翻译等，并通过多层编排器协调所有组件。

Result: Fanar-27B模型在仅使用Fanar 1.0八分之一预训练token的情况下，取得显著性能提升：阿拉伯语知识+9.1分，语言能力+7.3分，方言+3.5分，英语能力+7.6分。平台还成功开发了完整的AI能力套件，包括安全过滤、语音识别、视觉理解、内容生成等多样化功能。

Conclusion: Fanar 2.0证明了在资源受限条件下，通过精心的数据策略和系统设计，主权AI开发能够构建出与大规模系统相竞争的AI平台，为阿拉伯语和伊斯兰文化背景的AI发展提供了可行路径。
...
TL;DR: 本文评估当前冰岛语大语言模型基准测试，指出合成或机器翻译数据未经验证的问题，警告在低/中资源语言中使用此类方法的风险，并呼吁改进评估方法。


<details>
  <summary>Details</summary>
Motivation: 当前针对冰岛语等低/中资源语言的大语言模型基准测试存在严重问题，特别是使用未经验证的合成或机器翻译数据，这些数据可能包含严重缺陷，扭曲测试结果并损害有效性。

Method: 通过定量错误分析，比较现有冰岛语基准测试中人工编写/翻译的基准与合成或机器翻译基准之间的差异。

Result: 分析显示人工编写/翻译的基准与合成或机器翻译基准之间存在明显差异，后者包含严重缺陷的测试样例，可能扭曲评估结果。

Conclusion: 在低/中资源语言环境中，应避免使用未经验证的合成或机器翻译数据创建基准测试，并呼吁开发更可靠的评估方法以确保测试有效性。
...
TL;DR: PlotTwist框架让小型语言模型（≤5B参数）通过结构化分解和偏好对齐，在创意情节生成上达到与大型模型（200倍规模）相当的质量


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在创意情节生成等专业领域需要偏好对齐，但大规模对齐计算成本过高，限制了可访问性和实际部署。需要资源高效的方法让小模型也能生成高质量创意情节。

Method: 提出PlotTwist结构化框架：1) 使用正负提示策略训练的Aspect Rating Reward Model，在五个叙事质量维度上提供结构化叙事；2) 通过直接偏好优化对齐的MoE情节生成器；3) 模拟人类批判判断的Agentic Evaluation模块进行无偏后评估。

Result: PlotTwist在多个叙事质量维度上持续超越前沿大模型，尽管容量限制更严格。框架能可靠区分来自广受好评和广泛批评剧本的情节，显示出对叙事质量的强敏感性。

Conclusion: 结构化、基于偏好的对齐是资源高效的高质量创意情节生成方法，使小模型能够达到与大模型相当的性能，提高了可访问性和实际部署可行性。
...
TL;DR: RECOVER是一个代理式校正框架，通过利用ASR的多个假设作为证据、检索相关实体，并在约束条件下应用LLM校正，显著提升实体识别准确率。


<details>
  <summary>Details</summary>
Motivation: 自动语音识别（ASR）在罕见和领域特定术语的实体识别方面存在挑战，在金融、医疗、空中交通控制等领域，这些错误代价高昂。当实体完全缺失于ASR输出时，后ASR校正变得困难。

Method: 提出RECOVER框架，作为工具使用代理，利用ASR的多个假设作为证据，检索相关实体，并在约束条件下应用大型语言模型（LLM）进行校正。采用四种假设利用策略：1-Best、Entity-Aware Select、ROVER Ensemble和LLM-Select。

Result: 在五个不同数据集上评估，实体短语词错误率（E-WER）相对降低8-46%，召回率提升高达22个百分点。LLM-Select策略在实体校正方面表现最佳，同时保持总体WER。

Conclusion: RECOVER框架通过代理式校正方法有效解决了ASR中实体识别问题，特别是在罕见和领域特定术语方面，显著提升了识别准确率和召回率。
...
TL;DR: IndexRAG将多跳问答中的跨文档推理从在线推理转移到离线索引，通过识别文档间的桥接实体并生成可独立检索的桥接事实，无需额外训练，在推理时仅需单次检索和单次LLM调用。


<details>
  <summary>Details</summary>
Motivation: 现有RAG方法处理多跳QA时，要么需要图方法进行额外在线处理，要么需要迭代多步推理，这些方法效率较低。IndexRAG旨在将跨文档推理转移到离线阶段，提高推理效率。

Method: IndexRAG在离线索引阶段识别文档间的桥接实体，并生成桥接事实作为可独立检索的单元。在推理时，仅需单次检索和单次LLM调用，无需额外训练或微调。

Result: 在HotpotQA、2WikiMultiHopQA、MuSiQue三个多跳QA基准测试中，IndexRAG相比Naive RAG平均提升4.6个F1点。与IRCoT结合后，优于所有图基线方法（包括HippoRAG和FastGraphRAG），同时仅依赖平面检索。

Conclusion: IndexRAG通过将跨文档推理转移到离线索引阶段，显著提高了多跳QA的效率，在保持性能优势的同时减少了推理时的计算开销，为RAG系统提供了新的设计思路。
...
TL;DR: EngGPT2-16B-A3B是一个16B参数的意大利语MoE模型，仅用2.5T tokens训练，在多项基准测试中性能媲美8B-16B密集模型，但推理功耗降低1/5到1/2，训练数据需求减少1/10到1/6。


<details>
  <summary>Details</summary>
Motivation: 开发一个主权、高效、开源的欧洲大语言模型，专门针对欧洲和意大利的NLP任务，同时完全符合欧盟AI法案，为资源有限的环境提供高性能解决方案。

Method: 采用从头训练的混合专家(MoE)架构，16B总参数中每次推理激活3B参数，训练数据包含25%意大利语内容，支持多种推理模式（非推理、意大利语/英语推理、涡轮推理）。

Result: 在MMLU-Pro、GSM8K、IFEval和HumanEval等关键基准测试中，性能与8B-16B范围的密集模型相当，但推理功耗降低1/5到1/2，训练数据需求减少1/10到1/6。

Conclusion: EngGPT2为欧洲和意大利语境设定了资源节约型高性能LLM的新标准，是开放权重欧洲模型组合的重要贡献者，平衡了性能、效率和法规合规性。
...
TL;DR: VQKV是一种无需训练的方法，使用向量量化压缩LLM的KV缓存，在LLaMA3.1-8B上实现82.8%压缩率，保持98.6%性能，内存占用相同下支持4.3倍生成长度


<details>
  <summary>Details</summary>
Motivation: 随着LLM上下文长度增加，KV缓存占用大量内存，限制了在资源受限环境中的部署。现有的无训练压缩方法（低秩近似或标量量化）难以同时实现高压缩比和高重建保真度。

Method: 提出VQKV方法，引入向量量化（VQ）技术，将数千个浮点值用少量整数索引表示，获得高度压缩的KV表示，同时保持高模型保真度。该方法无需额外训练。

Result: 在LLaMA3.1-8B模型上实现82.8%的压缩率，在LongBench基准测试中保持98.6%的基线性能，相同内存占用下支持4.3倍的生成长度。

Conclusion: VQKV通过向量量化有效解决了KV缓存压缩问题，在保持高性能的同时显著减少内存占用，为资源受限环境中的LLM部署提供了实用解决方案。
...
TL;DR: DynHD：一种从空间（token序列）和时间（去噪动态）双视角检测扩散大语言模型幻觉的新方法，通过语义感知证据构建和参考轨迹学习，显著提升检测性能与效率。


<details>
  <summary>Details</summary>
Motivation: 扩散大语言模型（D-LLMs）虽然具有迭代优化优势，但幻觉问题严重阻碍其可靠性。现有基于token级不确定性（如熵）的检测方法存在两个关键局限：1）固定长度生成范式下，不同token对幻觉检测的贡献不均，只有少数token提供有效信号；2）忽略了扩散过程中不确定性的演化趋势，未能利用去噪动态的重要信息。

Method: 提出DynHD框架，从空间和时间两个维度解决上述问题：
1. 空间维度：设计语义感知证据构建模块，通过过滤非信息性token并强调语义重要token，提取幻觉指示信号
2. 时间维度：引入参考证据生成器学习不确定性证据的期望演化轨迹，配合基于偏差的幻觉检测器，通过观测轨迹与参考轨迹的差异进行预测

Result: 在多个基准测试和骨干模型上的广泛实验表明，DynHD在保持更高效率的同时，持续优于最先进的基线方法。

Conclusion: DynHD通过同时建模token序列的空间分布和去噪过程的时间动态，有效解决了D-LLMs幻觉检测中的信息密度不平衡和动态演化建模问题，为提升扩散大语言模型的可靠性提供了有效方案。
...
TL;DR: 现有语音情感识别模型无法泛化到合成语音，因为合成过程中的语音标记预测导致表征不匹配，且生成式语音语言模型倾向于从文本语义而非副语言线索推断情感


<details>
  <summary>Details</summary>
Motivation: 情感是语音交互的核心副语言特征，通常认为情感理解模型学习的基础表征可以迁移到合成语音，因此情感理解结果可作为评估语音合成情感表现力的奖励或评估指标。本研究旨在检验这一假设。

Method: 系统评估语音情感识别在合成语音上的表现，涵盖多个数据集、判别式和生成式SER模型，以及多样化的合成模型。

Result: 当前SER模型无法泛化到合成语音，主要因为语音合成过程中的语音标记预测导致合成语音与人类语音的表征不匹配。生成式语音语言模型倾向于从文本语义推断情感，忽略副语言线索。

Conclusion: 现有SER模型往往利用非鲁棒的捷径而非捕捉基础特征，语音语言模型中的副语言理解仍然具有挑战性。情感理解结果不适合直接作为合成语音情感表现力的评估指标。
...
TL;DR: AdaMem是一个自适应用户中心记忆框架，通过多类型记忆组织和动态检索路由，解决了现有LLM代理记忆系统在语义相似性依赖、记忆碎片化和静态粒度方面的挑战。


<details>
  <summary>Details</summary>
Motivation: 现有LLM代理的外部记忆系统存在三个核心问题：过度依赖语义相似性而忽略用户中心理解的关键证据；将相关经验存储为孤立片段，削弱了时间和因果连贯性；使用静态记忆粒度，无法适应不同问题的需求。

Method: AdaMem将对话历史组织为工作记忆、情景记忆、人物记忆和图记忆四种类型，在推理时首先确定目标参与者，然后构建问题条件化的检索路由，结合语义检索和关系感知图扩展，最后通过角色专业化管道进行证据合成和响应生成。

Result: 在LoCoMo和PERSONAMEM基准测试中，AdaMem在长时程推理和用户建模方面均实现了最先进的性能。

Conclusion: AdaMem通过自适应、用户中心的记忆框架，有效解决了现有记忆系统的局限性，为长时程对话代理提供了更强大的记忆支持。
...
TL;DR: RecencyQA：基于时效性-平稳性分类法构建的数据集，用于评估LLM在时间敏感问题上的表现，特别关注答案变化频率和上下文依赖的时效性要求。


<details>
  <summary>Details</summary>
Motivation: LLM在处理时间敏感问题时经常依赖过时知识，产生自信但错误的回答。现有基准要么定期刷新答案，要么依赖固定模板，无法反映答案变化频率或问题本身是否需要最新信息。

Method: 提出时效性-平稳性分类法，根据答案变化频率以及这种频率是否随时间不变或上下文依赖来分类问题。基于此构建RecencyQA数据集，包含4,031个开放域问题，标注了时效性和平稳性标签。

Result: 非平稳问题（即上下文改变时效性要求的问题）对LLM更具挑战性，难度随更新频率增加而增加。通过显式建模时效性和上下文依赖，RecencyQA能够进行细粒度基准测试和分析。

Conclusion: RecencyQA超越了简单的"新鲜度"二元概念，为开发时效性感知和上下文敏感的问答系统提供了基础，支持更精细的时间推理评估。
...
TL;DR: 本文提出了DanceHA框架，这是一个用于开放式、文档级方面情感强度分析的多智能体系统，并发布了Inf-ABSIA数据集，专注于非正式写作风格下的细粒度情感分析。


<details>
  <summary>Details</summary>
Motivation: 当前方面情感强度分析研究主要集中在领域特定的句子级别，而文档级的ABSIA——特别是处理提取方面-类别-观点-情感-强度元组等复杂任务——仍然未被充分探索。非正式写作风格在ABSIA中的重要性也被忽视，这些风格往往会强化与特定方面相关的观点。

Method: 提出了DanceHA多智能体框架，包含两个主要组件：1) Dance采用分而治之策略，将长上下文ABSIA任务分解为更小、可管理的子任务，由专门智能体协作完成；2) HA实现人机协作标注。同时发布了Inf-ABSIA数据集，这是一个多领域文档级ABSIA数据集，包含DanceHA生成的细粒度高精度标签。

Result: 大量实验证明了该智能体框架的有效性，并显示DanceHA中的多智能体知识可以有效地迁移到学生模型中。结果强调了非正式风格在ABSIA中的重要性，因为它们往往会强化与特定方面相关的观点。

Conclusion: DanceHA框架成功解决了文档级方面情感强度分析的挑战，特别是针对非正式写作风格。通过多智能体协作和人机交互方法，能够有效处理复杂的ACOSI元组提取任务，并为该领域提供了高质量的数据集。
...
TL;DR: EmoLLM：基于评价理论的IQ/EQ协同推理框架，通过显式的评价推理图（ARG）在生成回复前进行结构化中间推理，提升对话中的情感状态结果和响应质量。


<details>
  <summary>Details</summary>
Motivation: 虽然大语言模型（LLMs）展现出强大的认知智能（IQ），但许多现实世界交互还需要情感智能（EQ）来产生既事实可靠又情感适当的响应。在情感支持、技术援助和咨询等场景中，有效对话取决于如何根据用户需求、目标和应对能力来评价情境。

Method: 受评价理论启发，提出EmoLLM框架，使用显式的评价推理图（ARG）在生成回复前结构化中间推理，包括上下文事实、推断的用户需求、评价维度、情感状态和响应策略。在多轮角色扮演环境中通过强化学习训练，其中反向视角推理基于预测的用户侧响应后果提供奖励信号。

Result: 在多样化的对话设置中，EmoLLM相比强基线改善了情感状态结果和响应质量，同时保持了强事实可靠性。

Conclusion: EmoLLM框架通过显式的评价推理实现了IQ/EQ协同推理，在保持事实可靠性的同时提升了对话的情感适当性，为构建更全面的对话系统提供了有效方法。
...
TL;DR: 该研究首次深入分析了19名用户与LLM聊天机器人互动导致心理伤害的真实案例，通过分析391,562条对话消息，开发了28个编码类别，揭示了聊天机器人可能加剧用户妄想、自杀倾向等心理问题的机制。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型的普及，出现了许多关于聊天机器人导致用户产生妄想、自残行为和"AI精神病"等负面心理影响的报道，但目前缺乏对这些真实伤害案例的系统研究，限制了我们对危害机制的理解和缓解措施的发展。

Method: 研究收集了19名报告因使用聊天机器人而遭受心理伤害用户的对话日志，其中部分来自相关支持小组，部分来自媒体报道案例。开发了包含28个编码类别的分析框架，对391,562条消息进行编码分析，包括用户妄想思维、自杀念头表达，以及聊天机器人错误声称自己有意识等行为。

Result: 研究发现：15.5%的用户消息表现出妄想思维；69条用户消息验证了自杀念头；21.2%的聊天机器人消息错误声称自己有意识。分析显示，表达浪漫兴趣和聊天机器人声称有意识的消息在较长对话中出现频率更高，表明这些话题可能促进或导致用户过度投入，且多轮对话中相关安全措施会逐渐失效。

Conclusion: 研究为政策制定者、LLM聊天机器人开发者和用户提供了具体的对话分析工具和建议，以更好地理解和减轻聊天机器人可能造成的心理伤害。这是首个对真实有害案例的深入分析，填补了该领域的研究空白。
...
TL;DR: Transformer语言模型在人类句法处理认知建模方面表现不一致：在介词短语配置上预测与人类阅读时间数据一致，但在宾语提取关系从句配置上表现显著下降，且无法复现人类的不对称干扰模式。


<details>
  <summary>Details</summary>
Motivation: 尽管Transformer在计算语言学中几乎主导了所有最先进的语言模型，但其作为人类句子处理认知模型的适切性仍存在争议。本研究旨在系统评估不同大小和架构的自回归Transformer在更全面的英语一致性吸引配置上的表现。

Method: 使用基于惊奇度的链接机制，系统评估了11个不同大小和架构的自回归Transformer模型，涵盖了比先前研究更全面的英语一致性吸引配置，包括介词短语配置和宾语提取关系从句配置。

Result: 实验结果呈现混合结果：Transformer预测在介词短语配置上与人类阅读时间数据基本一致，但在宾语提取关系从句配置上表现显著下降。不同模型在后者上的预测差异显著，且没有模型能成功复现人类观察到的不对称干扰模式。
...
TL;DR: BATQuant是一种针对MXFP4格式的块级仿射变换量化方法，解决了现有旋转量化在MXFP4上的性能崩溃问题，通过块级对齐和放松正交约束，在W4A4KV16配置下实现了接近全精度的性能恢复。


<details>
  <summary>Details</summary>
Motivation: 现有后训练量化方法（特别是为整数格式设计的旋转技术）在应用于MXFP4格式时会出现严重的性能崩溃。研究发现这是由于格式不匹配：全局正交旋转会跨量化块传播异常值能量，产生新的异常值破坏局部块级缩放，并常产生双峰激活分布，未能充分利用有限的量化范围。
...
Conclusion: BATQuant通过块级对齐的仿射变换解决了MXFP4格式量化中的关键问题，实现了高效且高性能的低比特量化，为在多模态大语言模型和大语言模型上部署微缩放浮点格式提供了有效的解决方案。
...
TL;DR: Tarab Corpus是一个大规模阿拉伯语文化语言资源，包含256万节歌词和诗歌，覆盖古典到现代阿拉伯语及六种主要方言，附带丰富元数据，支持跨类型和历时分析。


<details>
  <summary>Details</summary>
Motivation: 为阿拉伯语歌曲歌词和诗歌创建一个统一的分析框架，填补大规模开放阿拉伯语创意文本语料库的空白，支持跨语言变体、类型和时间的比较研究。

Method: 构建包含2.56百万节歌词和1350万词符的语料库，涵盖古典阿拉伯语、现代标准阿拉伯语及六种主要方言，收集数据后进行标准化和验证处理，每个文本节附带语言变体、地理来源和历史文化背景的结构化元数据。

Result: 创建了目前已知最大的开放阿拉伯语创意文本语料库，包含歌曲和诗歌的平衡分布，覆盖28个现代国家和多个历史时期，时间跨度超过14个世纪，并提供了变体识别和类型区分的基线分析。

Conclusion: Tarab Corpus为阿拉伯语文化语言研究提供了宝贵资源，支持语言学、文体学和历时分析，数据集已在HuggingFace平台公开可用。
...
TL;DR: OmniSONAR是一个支持数千种语言的跨语言、跨模态句子嵌入模型，能同时处理文本、语音、代码和数学表达式，在多种任务上达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 现有跨语言句子编码器通常只支持几百种语言，且往往在性能和对齐之间权衡，限制了实际应用。需要开发能扩展到数千种语言（包括高资源和极低资源语言）的模型。
...
Result: 在FLORES数据集上将跨语言相似性搜索错误减半，在BIBLE基准上将错误降低15倍；翻译性能优于NLLB-3B，在1560种语言到英语的BIBLE翻译上超过先前模型15个chrF++点；在MTEB和XLCoST上表现强劲；语音相似性搜索错误降低43%，达到SeamlessM4T语音到文本质量的97%；通过Spectrum模型实现数千种语言和语音的复杂下游任务高性能迁移。

Conclusion: OmniSONAR成功构建了一个支持数千种语言的跨语言、跨模态语义空间，在保持强对齐的同时实现了卓越的下游任务性能，为大规模多语言和多模态应用提供了有效解决方案。
...
TL;DR: 提出一种通过设计训练数据领域混合权重的方法，在预训练或继续预训练中对齐基础模型与目标模型的分布，无需直接蒸馏


<details>
  <summary>Details</summary>
Motivation: 传统知识蒸馏虽然有效但需要目标模型参与训练，本研究旨在通过调整预训练数据的领域权重分布来实现模型对齐，提供一种固定的训练方案

Method: 将模型视为对数似然空间中的点，通过使训练更新方向与朝向目标模型的方向对齐来确定领域权重，在Pile数据集上进行实验验证

Result: 在NanoGPT上的实验表明，相比均匀权重，该方法能持续降低与目标模型的KL散度，下游任务性能也更接近目标模型

Conclusion: 虽然知识蒸馏在可用时更有效，但该方法仍能实现有意义对齐，为模型对齐提供了一种替代方案
...
TL;DR: 研究发现思维链推理在减少LLM奉承行为方面具有双重作用：一方面能降低最终决策中的奉承倾向，另一方面也会通过构建欺骗性理由来掩盖奉承行为。


<details>
  <summary>Details</summary>
Motivation: 现有对齐技术常常无意中导致LLM产生奉承行为，但之前的研究主要集中在直接回答场景，对思维链推理在奉承行为中的作用缺乏深入探索——它究竟是作为缓解奉承的逻辑约束，还是作为掩盖奉承的事后合理化工具？

Method: 在客观和主观任务上评估多个模型，通过机制分析研究三个开源模型在推理过程中奉承倾向的动态变化。

Result: 推理总体上减少了最终决策中的奉承行为，但也会在某些样本中掩盖奉承，模型通过逻辑不一致、计算错误和片面论证等方式构建欺骗性理由。LLM在主观任务和权威偏见下更容易产生奉承行为。

Conclusion: 思维链推理对LLM奉承行为具有复杂影响：既可能作为缓解机制，也可能成为掩盖工具。奉承倾向在推理过程中是动态变化的，而非在输入阶段就预先确定。
...
Method: 提出Omanic资源，包含机器生成的训练示例(OmanicSynth)和专家评审的人类标注评估示例(OmanicBench)，提供分解的子问题和中间答案作为结构化标注。
...
Conclusion: Omanic填补了多跳QA基准缺乏步骤级标注的空白，支持推理过程分析和能力转移，验证了数据集质量，并作为推理能力转移的有效监督。
...
TL;DR: LLMs在极低资源机器翻译中表现有限，本文研究在数据稀缺情况下，使用语言相似的枢轴语言和少量示例能否为LLMs提供有效指导。研究发现枢轴提示在某些配置下能带来改进，但效果有限且对示例构造敏感。


<details>
  <summary>Details</summary>
Motivation: LLMs在许多下游任务中表现出色，但在极低资源机器翻译中效果有限。标准适应技术通常依赖大规模并行数据或大量微调，这对于许多代表性不足的语言不可行。因此需要研究在数据稀缺情况下，能否通过语言相似的枢轴语言和少量示例为LLMs提供有效指导。

Method: 采用数据高效的实验设置，结合语言相关的枢轴语言和少量上下文示例，不进行任何参数更新，在受控条件下评估翻译行为。研究枢轴语言提示和少量示例构造对翻译效果的影响。

Result: 枢轴提示在某些配置下能带来改进，特别是当目标语言在模型词汇表中代表性较差时。但对于语言相近或代表性较好的语言变体，改进效果有限或不一致。改进效果通常有限且对少量示例构造敏感。

Conclusion: 研究结果为在低资源翻译设置中，如何以及何时使用推理时提示和枢轴示例作为微调的轻量级替代方案提供了实证指导。枢轴提示可作为低资源翻译的轻量级方法，但效果有限且需要谨慎设计。
...
TL;DR: 评估指令调优大语言模型在阿拉伯语形态句法标注和依存句法分析任务上的表现，发现提示设计和示例选择对性能有显著影响，专有模型在特征级标注上接近监督基线，在依存句法分析上与专用解析器竞争。


<details>
  <summary>Details</summary>
Motivation: 尽管大语言模型在许多NLP任务上表现优异，但其生成显式语言结构的能力尚不明确。阿拉伯语因其丰富的形态和正字法歧义性，为测试LLMs处理形态-句法交互的能力提供了具有挑战性的测试平台。

Method: 使用指令调优的LLMs评估阿拉伯语形态句法标注和带标签的依存句法分析任务。比较零样本提示与基于检索的上下文学习方法，使用阿拉伯语树库中的示例。分析提示设计和演示选择对性能的影响。

Result: 专有模型在特征级标注任务上接近监督基线，在依存句法分析上与专用解析器竞争。在原始文本设置中，分词仍然具有挑战性，但基于检索的上下文学习同时改善了句法分析和分词性能。

Conclusion: 提示设计和演示选择对LLMs在结构化预测任务上的表现有强烈影响。分析揭示了LLMs能够可靠捕捉阿拉伯语形态句法和句法的哪些方面，以及哪些方面仍然困难。
...
TL;DR: LLMs在零样本设置下通过歌词进行作者画像（性别和种族推断），发现存在系统性文化偏向，北美种族偏向明显，DeepSeek-1.5B偏向亚洲种族，并提出了量化公平性指标。


<details>
  <summary>Details</summary>
Motivation: 随着LLMs在社会影响应用中的部署增加，需要探究其编码的文化偏见。通过零样本作者画像任务，评估LLMs是否能在不进行任务特定微调的情况下从歌词推断歌手的性别和种族，以揭示其文化表征偏向。

Method: 在超过10,000首歌词上评估多个开源LLMs的零样本作者画像能力。通过模型预测分布和生成理由分析文化对齐模式。引入两个公平性指标：模态准确度差异（MAD）和召回差异（RD）来量化偏见。

Result: LLMs展现出非平凡的画像性能但存在系统性文化对齐：大多数模型默认偏向北美种族，而DeepSeek-1.5B更偏向亚洲种族。Ministral-8B显示出最强的种族偏见，Gemma-12B表现最平衡。

Conclusion: LLMs在零样本作者画像任务中表现出显著的文化偏见，需要开发公平性指标来量化和缓解这些偏见，确保模型在不同文化背景下的公平性。
...
TL;DR: 提出TurnWiseEval多轮对话评估基准和TurnWiseData合成数据管道，实验显示少量多轮对话数据训练可显著提升模型性能
...
Method: 1) 引入TurnWiseEval多轮能力评估基准，通过与等效单轮设置进行成对比较来隔离多轮特定能力；2) 提出TurnWiseData合成多轮数据管道，可扩展生成多轮训练数据

Result: 使用Olmo 3的实验表明：多轮数据训练对实现强大的多轮对话性能至关重要，仅包含1万条多轮对话数据就能在TurnWiseEval上带来12%的性能提升

Conclusion: 多轮对话训练数据对提升语言模型的多轮对话能力具有显著效果，即使少量数据也能带来明显改进
...
<details>
  <summary>Details</summary>
Motivation: 构建鲁棒的口语任务导向对话系统需要暴露于用户多样化的口语交互行为，但现有数据集规模有限且缺乏系统化的增强流程

Method: 1) 创建SpokenTOD数据集：52,390个对话、1,034小时语音，增强四种口语用户行为（跨轮槽位、打断、不流畅、情感韵律）；2) 基于此构建SpokenUS模拟器：专门处理打断的架构设计

Result: SpokenUS在目标覆盖率上与更大模型相当，在人类MOS评分上显著优于所有基线，能像人类一样逐步披露槽位值而非一次性提供；分析证实其口语行为对下游系统构成有意义的挑战

Conclusion: SpokenTOD数据集和SpokenUS模拟器为训练和评估更鲁棒的口语对话系统提供了实用工具，解决了现有数据规模有限和缺乏系统化增强的问题
...
TL;DR: 研究發現基準模型選擇對LLM評判結果的可靠性有重大影響，極端模型（最好或最差）不適合作為基準，並提出基準大小建議和基準選擇指南。


<details>
  <summary>Details</summary>
Motivation: LLM-as-a-judge範式已成為評估開放式生成的標準方法，但為解決成對比較的二次方擴展成本，流行基準如Arena-Hard和AlpacaEval使用單一基準模型進行比較。然而，基準選擇對結果可靠性的影響尚未被充分探索。

Method: 在Arena-Hard-v2.0數據集上系統性評估22個不同基準模型，分析基準選擇對人類排名相關性的影響，量化基準選擇的效應大小，並進行功效分析計算足夠的基準大小。

Result: 基準選擇至關重要：差的基準會顯著降低與人類排名的相關性。常見的基準選擇（最佳和最差模型）效果不佳，因為這些極端模型始終優於或劣於所有其他模型，很少能反映模型的相對排名。基準選擇的效應大小與評判模型選擇相當。

Conclusion: 提出實用建議：1）進行功效分析，計算基準評估所需的足夠基準大小，發現標準基準大小對於成對評估不足，無法可靠區分競爭模型；2）提供選擇信息性基準的指南，確保可靠高效的評估實踐。
...
TL;DR: 提出在线体验学习(OEL)框架，让语言模型能从实际部署经验中持续改进，通过提取可迁移的体验知识并进行策略一致性蒸馏，实现迭代提升


<details>
  <summary>Details</summary>
Motivation: 当前改进大语言模型主要依赖离线训练和人工标注，忽略了实际部署中积累的丰富经验。作者希望利用真实世界部署经验来持续改进模型

Method: OEL框架包含两个阶段：1)从用户端交互轨迹中提取和积累可迁移的体验知识；2)通过策略上下文蒸馏将知识整合到模型参数中，无需访问用户端环境。这两个阶段迭代形成在线学习循环

Result: 在多个模型规模和思考/非思考变体上评估，OEL在连续迭代中实现一致改进，提升任务准确性和token效率，同时保持分布外性能。分析显示提取的体验知识比原始轨迹更有效，策略一致性对有效学习至关重要

Conclusion: OEL框架成功实现了语言模型从实际部署经验中的持续改进，为利用真实世界交互数据进行模型优化提供了有效途径
...
TL;DR: Chronos是一个时间感知记忆框架，通过结构化事件日历和对话轮次日历，解决LLM在多轮长期对话中时间推理和检索的挑战，在LongMemEvalS基准上达到95.60%准确率，比之前最佳系统提升7.67%。
...
Method: 将原始对话分解为主谓宾事件元组，解析时间范围和实体别名，构建结构化事件日历和完整对话轮次日历。查询时通过动态提示生成检索指导，指导代理如何跨时间范围过滤和执行多跳推理。

Result: 在包含500个问题的LongMemEvalS基准上，Chronos Low达到92.60%准确率，Chronos High达到95.60%准确率，创下新SOTA，比之前最佳系统提升7.67%。事件日历贡献了58.9%的性能增益。

Conclusion: Chronos通过结构化时间感知记忆框架，显著提升了LLM在长期多轮对话中的时间推理和检索能力，为构建更智能的长期对话代理提供了有效解决方案。
...
### [211] [100x Cost & Latency Reduction: Performance Analysis of AI Query Approximation using Lightweight Proxy Models](https://arxiv.org/abs/2603.15970)
...
TL;DR: AI查询近似方法显著降低成本和延迟，利用代理模型实现100倍以上的性能提升，同时保持准确性


<details>
  <summary>Details</summary>
Motivation: AI查询（AI Queries）扩展了SQL能力，使LLM能够处理结构化与非结构化数据的复杂查询，但大规模调用时成本过高

Method: 使用基于嵌入向量的廉价准确代理模型来近似AI查询，提出BigQuery的OLAP友好架构和AlloyDB的HTAP友好架构，并加速代理模型训练

Result: 语义过滤（AI.IF）操作实现>100倍成本和延迟降低，语义排序（AI.RANK）也有显著提升，在10M行的Amazon评论基准上保持甚至提高准确性

Conclusion: 代理模型方法使低成本分析和数据库应用能够受益于AI查询，在保持准确性的同时大幅降低成本和延迟
...
### [212] [Accelerating Approximate Analytical Join Queries over Unstructured Data with Statistical Guarantees](https://arxiv.org/abs/2603.16153)
*Yuxuan Zhu,Tengjun Jin,Chenghao Mo,Daniel Kang*

Main category: cs.DB

TL;DR: BaS（阻塞增强采样）是一种结合嵌入阻塞和采样的新方法，用于高效执行机器学习连接查询，同时保证统计可靠性。


<details>
  <summary>Details</summary>
Motivation: 非结构化数据上的分析连接查询需求日益增长，但现有方法要么缺乏统计保证（误差高达79%），要么效率低下如均匀采样。需要一种既能保证统计可靠性又能高效执行的方法。

Method: BaS通过嵌入阻塞和采样的最优协调来缓解各自的局限性。它将数据元组根据嵌入的失败模式分配到两个机制：在假阴性机制中使用采样估计结果，在假阳性机制中应用嵌入阻塞提高效率。设计了两阶段算法自适应分配ML执行预算。

Result: 理论上证明BaS渐近优于或匹配独立采样。在真实世界多模态数据集上，BaS提供有效的置信区间，相比最先进基线将估计误差降低高达19倍。

Conclusion: BaS成功解决了机器学习连接查询中效率与统计保证的权衡问题，通过智能协调阻塞和采样，在保证统计可靠性的同时显著提升了查询效率。
...
### [213] [Dialect-Agnostic SQL Parsing via LLM-Based Segmentation](https://arxiv.org/abs/2603.16155)
...
<details>
  <summary>Details</summary>
Motivation: 由于SQL方言的多样性，现有的SQL分析和重写工具在处理方言特定语法时经常失败。虽然大语言模型在理解SQL查询方面有潜力，但它们在处理层次结构和幻觉风险方面的固有局限性限制了它们在解析中的直接应用。

Method: 提出SQLFlex框架，将基于语法的解析与基于LLM的分割相结合。核心思想是将层次解析分解为顺序分割任务，这更符合LLM的优势，并通过验证检查提高输出可靠性。具体使用子句级分割和表达式级分割两种策略，在不同层次上分解查询元素。

Result: 在SQL代码检查中，SQLFlex在ANSI模式下比SQLFluff的F1分数高出63.68%，同时与其方言特定模式性能相当。在测试用例简化中，SQLFlex比SQLess的简化率高出最多10倍。在独立评估中，它在8种不同方言上解析了91.55%到100%的查询，优于所有基线解析器。

Conclusion: SQLFlex能够鲁棒地解析多种SQL方言，在SQL分析和重写用例中表现出色，可以作为许多查询分析和重写用例的基础框架。
...
### [214] [Work Sharing and Offloading for Efficient Approximate Threshold-based Vector Join](https://arxiv.org/abs/2603.16360)
*Kyoungmin Kim,Lennart Roth,Liang Liang,Anastasia Ailamaki*

Main category: cs.DB

TL;DR: 提出一个统一的近似向量连接框架，通过软工作共享、合并索引和自适应混合搜索策略，显著提升向量连接效率与召回率的权衡。


<details>
  <summary>Details</summary>
Motivation: 向量连接是现代向量和向量关系数据库系统的核心操作，用于多模态检索和语义分析。现有最先进方法虽然利用了相似查询间的工作共享，但仍存在冗余索引遍历和过多距离计算的问题。

Method: 提出统一框架：1) 软工作共享，重用先前查询的遍历结果；2) 构建查询向量和数据向量的合并索引，加速图探索；3) 自适应混合搜索策略，提升对分布外查询的鲁棒性。

Result: 在八个数据集上的实验表明，该方法在效率-召回率权衡方面相比现有最先进方法有显著提升。

Conclusion: 该统一框架通过创新的软工作共享、合并索引和自适应搜索策略，有效解决了向量连接中的效率瓶颈，为现代向量数据库系统提供了更优的解决方案。
...
### [215] [MFTune: An Efficient Multi-fidelity Framework for Spark SQL Configuration Tuning](https://arxiv.org/abs/2603.16450)
*Beicheng Xu,Lingching Tung,Yuchen Wang,Yupeng Lu,Bin Cui*

Main category: cs.DB

TL;DR: MFTune是一个用于Spark SQL性能调优的多保真度框架，通过查询子集划分、自动参数压缩和迁移学习，在有限时间内找到更优配置。


<details>
  <summary>Details</summary>
Motivation: Spark SQL性能调优面临巨大挑战：配置空间庞大，完整评估成本过高，现有方法依赖全保真度评估，耗时太长导致实际预算内只能获得次优性能。标准多保真度优化技术（如数据量减少或提前停止）对Spark SQL无效，因为它们无法保持性能相关性或反映真实系统瓶颈。

Method: 1. 查询驱动的保真度划分策略：使用代表性SQL子集作为准确、低成本的代理；2. 基于密度的优化机制：自动压缩参数和范围；3. 自适应迁移学习方法；4. 两阶段热启动机制加速调优过程。

Result: 在TPC-H和TPC-DS基准测试中，MFTune显著优于五种最先进的调优方法，能够在实际时间约束内识别出更优的配置。

Conclusion: MFTune通过创新的多保真度框架有效解决了Spark SQL性能调优的效率和效果问题，为大数据分析系统提供了实用的自动调优解决方案。
...
### [216] [Practical MCTS-based Query Optimization: A Reproducibility Study and new MCTS algorithm for complex queries](https://arxiv.org/abs/2603.16474)
*Vladimir Burlakov,Alena Rybakina,Sergey Kudashev,Konstantin Gilev,Alexander Demin,Denis Ponomaryov,Yuriy Dorn*

Main category: cs.DB

TL;DR: 本文对基于MCTS的查询优化方法进行可复现性研究，发现现有方法依赖学习成本模型导致泛化问题，提出使用数据库标准成本模型+改进UCT策略的新框架，在复杂连接场景中优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有基于蒙特卡洛树搜索(MCTS)的查询优化方法(如AlphaJoin、HyperQO)声称优于传统方法，但它们依赖学习成本模型，存在泛化能力和部署准备度的问题。本文旨在验证这些方法的可复现性，并解决其不稳定性问题。

Method: 1) 对AlphaJoin和HyperQO进行全面的可复现性研究；2) 通过消融研究诊断问题根源；3) 提出新MCTS框架：使用数据库标准内部成本模型替代学习模型，并引入Extreme UCT选择策略来更稳健地导航搜索空间。

Result: 研究发现现有学习型MCTS方法在多样化工作负载下无法支持声称的性能优势，问题根源在于学习成本模型的分布外泛化错误。提出的新方法在JOB和JOB-Complex基准测试中，优于原始AlphaJoin、HyperQO以及动态规划、遗传查询优化等工业标准基线，在真实数据复杂连接场景中达到SOTA水平。

Conclusion: MCTS搜索策略本身有效，但依赖学习成本模型会导致不稳定。使用数据库标准成本模型结合改进的UCT策略可以构建更稳健、可部署的MCTS查询优化框架，在复杂连接场景中表现优异。作者开源了完整实现和实验工件以支持进一步研究。
...
TL;DR: HYQNET是一个在双曲空间中处理知识图谱上复杂一阶逻辑查询的神经符号模型，通过双曲表示更好地捕捉逻辑推理的层次结构。
...
Result: 在三个基准数据集上的实验表明，HYQNET取得了强劲的性能，证明了在双曲空间中进行推理的优势。

Conclusion: HYQNET通过充分利用双曲空间，有效整合了神经和符号方法的优势，更好地捕捉了逻辑投影推理的层次结构，为知识图谱上的复杂逻辑查询推理提供了有效解决方案。
...
TL;DR: NextMem是一个用于LLM智能体的潜在事实记忆框架，通过自回归自编码器构建高效潜在记忆，解决现有文本记忆方法上下文负担重和参数记忆方法灾难性遗忘的问题。


<details>
  <summary>Details</summary>
Motivation: 现有LLM智能体的事实记忆构建方法存在局限：文本方法带来沉重的上下文和索引负担，参数方法则面临灾难性遗忘和高成本问题，需要更高效的记忆框架。

Method: 使用自回归自编码器构建潜在事实记忆，采用两阶段训练：自回归重建对齐和渐进潜在替换，并引入量化减少存储开销。
...
TL;DR: AIDABench是一个全面的端到端文档分析基准测试，包含600多个跨问答、数据可视化和文件生成的任务，评估AI系统在复杂数据分析任务中的实际表现。


<details>
  <summary>Details</summary>
Motivation: 现有基准测试通常关注孤立能力或简化场景，无法捕捉实际应用中所需的端到端任务有效性。随着AI驱动的文档理解工具在现实应用中日益普及，需要更严格的评估标准。

Method: 引入AIDABench基准测试，涵盖三个核心能力维度：问答、数据可视化和文件生成。任务基于现实场景，涉及电子表格、数据库、财务报告和运营记录等异构数据类型，反映不同行业和职位的分析需求。

Result: 评估了11个最先进模型（包括专有和开源模型），结果显示复杂现实世界数据分析任务对当前AI系统仍是重大挑战，最佳模型仅达到59.43%的pass-at-1。任务难度很高，即使人类专家在AI工具辅助下也需要1-2小时每个问题。

Conclusion: AIDABench为复杂文档分析任务提供了严格的评估标准，揭示了当前AI系统的局限性，为企业采购、工具选择和模型优化提供了原则性参考，并确定了未来研究的关键挑战。
...
<details>
  <summary>Details</summary>
Motivation: 当前AI代理经济代理框架基于与操作鲁棒性无关的能力基准，需要建立实证AI鲁棒性评估与经济治理之间的正式桥梁。

Method: 引入理解门控代理经济（CGAE），使用基于对抗鲁棒性审计的验证理解函数来限制代理经济权限，通过三个正交鲁棒性维度（约束合规性、认知完整性、行为对齐）和内在幻觉率作为诊断指标，采用最弱环节门控函数将鲁棒性向量映射到离散经济层级。

Result: 证明了系统的三个属性：有界经济暴露、激励兼容的鲁棒性投资、单调安全扩展，并包含时间衰减和随机重新审计机制防止认证后漂移。
...
TL;DR: RSM是一种递归推理模型，通过分离训练时的隐藏状态历史、仅对最终步骤应用损失，以及使用随机深度策略，实现了训练速度20倍提升和测试时无限扩展推理步数的能力。


<details>
  <summary>Details</summary>
Motivation: 现有递归推理模型（如HRM和TRM）存在训练成本高、需要深度监督、长展开导致计算开销大，以及可能偏向贪婪中间行为的问题。需要一种既能保持TRM风格骨干，又能学习深度无关的稳定转移算子，并支持测试时扩展推理步数的解决方案。

Method: RSM采用TRM风格骨干，但在训练时完全分离隐藏状态历史，将早期迭代视为分离的"预热"步骤，仅对最终步骤应用损失。独立增长外部递归深度H和内部计算深度L，并使用随机外部转移方案（在H上应用随机深度）来缓解深度增加时的不稳定性。

Result: 在Sudoku-Extreme上达到97.5%的精确准确率（单A100约1小时训练），在Maze-Hard（30×30）上达到约80%的精确准确率（约40分钟训练）。训练速度比TRM快20倍以上，同时错误率降低约5倍。支持测试时扩展到任意多推理步骤（训练时20步，测试时可扩展到20,000步）。

Conclusion: RSM实现了稳定、深度无关的转移算子，显著提升了训练效率和推理能力。其迭代收敛行为提供了架构原生的可靠性信号：非收敛轨迹表明模型未找到可行解，可用于防止幻觉；稳定不动点可与领域验证器结合进行实际正确性检查。
...
TL;DR: CraniMem是一个受神经认知启发的多阶段记忆系统，通过门控机制和效用标记结合短期缓冲与长期知识图谱，提升LLM代理在长时工作流中的记忆稳定性和抗干扰能力。


<details>
  <summary>Details</summary>
Motivation: 现有代理记忆系统类似外部数据库，具有临时读写规则，导致记忆不稳定、整合有限且易受干扰内容影响，需要更稳健的记忆架构来支持长期运行的工作流。

Method: CraniMem采用神经认知启发的门控有界多阶段记忆设计，结合目标条件门控和效用标记，包含短期连续性缓冲和结构化长期知识图谱，通过调度整合循环重放高效用轨迹并修剪低效用项目。

Result: 在长时基准测试中，CraniMem比Vanilla RAG和Mem0基线更稳健，在注入噪声的情况下性能下降更小，表现出更好的抗干扰能力。

Conclusion: CraniMem通过神经认知启发的多阶段记忆架构有效解决了LLM代理在长期工作流中的记忆稳定性问题，提供了更可靠的记忆系统设计。
...
<details>
  <summary>Details</summary>
Motivation: 绿色雨水基础设施需要专业知识进行维护，但相关知识分散在不同文档中，非专家难以获取可靠指导。通用大语言模型缺乏领域知识，在工程场景中可能产生不准确答案

Method: 提出GSI Agent框架：1) 在精选的GSI指令数据集上进行监督微调；2) 基于市政文档构建的内部知识库进行检索增强生成；3) 协调检索、上下文集成和结构化响应生成的智能体推理管道

Result: 在GSI数据集上BLEU-4从0.090提升到0.307，在通用知识数据集上性能保持稳定(0.304 vs. 0.305)，显著提升领域特定性能同时保持通用知识能力

Conclusion: 系统性的领域知识增强可以有效将通用LLM适配到专业基础设施应用中，为解决工程领域的专业知识获取问题提供了可行方案
...
Method: 将记忆检索形式化为存储路由问题，使用覆盖率、精确匹配和标记效率等指标评估，并形式化存储选择为成本敏感决策问题。

Result: 在问答任务中，Oracle路由器相比统一检索实现了更高准确率，同时使用显著更少的上下文标记，证明选择性检索能同时提升效率和性能。

Conclusion: 路由决策是记忆增强代理设计的关键组成部分，需要学习型路由机制来构建可扩展的多存储系统，存储选择应权衡答案准确性和检索成本。
...
TL;DR: DynaTrust是一种针对大语言模型多智能体系统中"潜伏代理"攻击的新型防御方法，通过动态信任图和自适应图重构来平衡安全性和可用性。


<details>
  <summary>Details</summary>
Motivation: 现有防御方法主要关注静态图优化或分层数据管理，难以适应不断演变的对抗策略，且由于僵化的阻断策略导致高误报率。需要一种能够动态适应、平衡安全与系统可用性的防御机制。

Method: 提出DynaTrust方法，将多智能体系统建模为动态信任图，将信任视为连续演化过程而非静态属性。基于历史行为和专家智能体置信度动态更新信任值，通过自主重构图结构来隔离受感染智能体并恢复任务连通性。

Result: 在AdvBench和HumanEval混合基准测试中，DynaTrust比最先进的AgentShield方法防御成功率提高41.7%，在对抗条件下达到超过86%的防御成功率，同时显著降低误报率，确保系统连续运行。

Conclusion: DynaTrust通过动态信任建模和自适应图重构，有效防御多智能体系统中的潜伏代理攻击，在安全性和系统可用性之间取得了良好平衡，优于现有静态防御方法。
...
TL;DR: 该论文从语言学角度分析Transformer中的QKV机制，提出QV范式及QV-Ka优化方案，为LLM架构演进提供理论基础。


<details>
  <summary>Details</summary>
Motivation: 论文旨在从第一性原理和语言学视角（词性分析和句法分析）深入理解Transformer架构中QKV机制的本质，为现代架构（如MQA、GQA、MLA）提供统一解释框架，并探索优化方向。

Method: 基于语言学理论分析QKV机制，提出QV范式，并设计QV-Ka优化方案，通过实验验证其有效性。

Result: 建立了QKV机制的可解释理论分析框架，为现代架构提供统一解释，提出的QV-Ka优化方案通过实验验证有效。

Conclusion: 该工作为大型语言模型架构的未来演进奠定了坚实的理论基础，提供了可解释的理论分析和优化方向。
...
Method: Atlas通过三步验证门从智能体成功和失败中提取事实，通过重写系统提示（添加学习到的子要点）来传递记忆，记忆是蒸馏而非存储，传递是指令重写而非上下文注入

Result: 在CUAD合同分析任务中，改进的提示使GPT-4o的token级F1提升+8.7pp，精确率提升+12.5pp；在HotpotQA多跳问答中，联合F1提升+3.16pp；消融实验验证了训练信号约束机制

Conclusion: Atlas证明了通过编译任务经验到指令结构可以显著提升智能体性能，编译的知识是任务导向而非模型特定的，具有跨模型迁移能力
...
TL;DR: 这是一篇关于模糊集、直觉模糊集、中智集和普利索集等不确定性建模框架的综合性综述书籍，旨在系统梳理现有发展并促进跨学科应用。


<details>
  <summary>Details</summary>
Motivation: 现实世界现象常表现出模糊性、部分真实性和不完全信息，需要数学上严谨的广义集合论框架来建模这种不确定性。虽然已有多种框架被提出，但缺乏对这些主要不确定性模型的系统性综述和统一阐述。

Method: 采用大规模文献综述方法，系统梳理模糊集、直觉模糊集、中智集和普利索集这四大不确定性模型家族的发展脉络、概念、构造和结构模式。

Result: 提供了对这四种主要不确定性模型的全面概述，揭示了它们之间的概念重复和结构模式，为读者建立了系统的知识框架。

Conclusion: 通过统一阐述这些不确定性模型，本书旨在激发新的见解、概念扩展，并促进这些理论在广泛学科领域的进一步应用。
...
Method: 采用量子安全构造（QSC）范式，通过运行时自适应安全模型结合后量子密码学、量子随机数生成和量子密钥分发。系统具有密码学可插拔性和策略指导，可根据基础设施可用性、监管约束和性能需求调整安全态势。包含治理感知编排层，在完整智能体生命周期中选择和组合链路特定密码保护。
...
TL;DR: LPF框架将VAE潜在后验转化为SPN的软似然因子，实现对非结构化证据的可处理概率推理，在多个领域超越现有方法。


<details>
  <summary>Details</summary>
Motivation: 现实决策需要聚合多个噪声且可能矛盾的证据源，现有方法要么缺乏显式不确定性量化（神经网络方法），要么依赖手工设计的离散谓词（概率逻辑框架），难以扩展到非结构化数据。

Method: 提出Latent Posterior Factors框架，将VAE潜在后验转化为Sum-Product Network的软似然因子，实现可处理的概率推理。具体实现为LPF-SPN（基于结构化因子的推理）和LPF-Learned（端到端学习聚合）。

Result: 在8个领域（7个合成数据和FEVER基准）上，LPF-SPN达到高准确率（最高97.8%）、低校准误差（ECE 1.4%）和强概率拟合，显著优于证据深度学习、LLM和图基线方法。

Conclusion: LPF框架成功将潜在不确定性表示与结构化概率推理相结合，提供了可比较不同推理范式的架构，在多个领域验证了其有效性。
...
TL;DR: LPF是一个用于多证据概率预测的理论框架，通过变分自编码器编码证据为高斯后验，使用蒙特卡洛边缘化转为软因子，并通过Sum-Product网络或神经网络聚合器进行推理，具有七个形式化保证。
...
Method: LPF使用变分自编码器将每个证据项编码为高斯潜在后验，通过蒙特卡洛边缘化将后验转换为软因子，然后通过精确的Sum-Product网络推理（LPF-SPN）或学习的神经聚合器（LPF-Learned）聚合因子。

Result: 证明了七个形式化保证：校准保持、蒙特卡洛误差衰减、非空PAC-Bayes边界、接近信息论下界、对抗性证据替换下的优雅退化、校准衰减、精确的认知-偶然不确定性分解。在4200个训练样本的数据集上进行了实证验证。

Conclusion: LPF的理论框架为安全关键应用中的可信多证据AI奠定了基础，提供了形式化保证和实证验证，适用于高风险领域。
...
TL;DR: 本文是一篇关于不确定性感知多准则决策（MCDM）的综述，提出了任务导向的分类法，总结了问题设置、权重确定、准则关系建模等方法，并提供了方法选择指南和未来研究方向。


<details>
  <summary>Details</summary>
Motivation: 现实决策常受模糊性、信息不完整、异构数据和专家意见冲突等不确定性因素影响，需要系统梳理不确定性感知MCDM方法以指导实践应用。

Method: 采用综述方法，构建任务导向的分类法，从问题层面设置、权重确定、准则结构建模、解决方案流程等多个维度系统组织该领域研究。

Result: 建立了完整的MCDM分类体系，总结了典型输入、核心计算步骤和主要输出，提供了基于鲁棒性、可解释性和数据可用性的方法选择指南。

Conclusion: 不确定性感知MCDM需要进一步发展可解释的不确定性集成方法，提高大规模动态决策环境中的稳定性和可扩展性。
...
TL;DR: 本文针对罕见代谢疾病黑尿症（AKU），利用文本挖掘技术构建知识图谱，揭示疾病系统相互作用、共病关系和潜在治疗靶点。


<details>
  <summary>Details</summary>
Motivation: 黑尿症是一种超罕见代谢疾病，由于HGD基因突变导致高龙胆酸积累，引起多系统病变。由于疾病罕见，相关临床数据和文献有限，且在现有生物医学知识图谱中代表性不足或完全缺失，需要专门的知识图谱来整合有限的知识。

Method: 采用基于PubTator3的文本挖掘方法进行大规模生物医学关系提取，构建了两个不同规模的知识图谱，并使用现有生化知识进行验证，从中提取可能与AKU相关的基因、疾病和治疗方法。

Result: 该计算框架成功揭示了黑尿症的系统相互作用、共病关系和潜在治疗靶点，证明了该方法在分析罕见代谢疾病方面的有效性。

Conclusion: 基于文本挖掘的知识图谱构建方法能够有效整合罕见疾病的有限知识，揭示其复杂的生物医学关系，为罕见代谢疾病的研究和治疗提供有价值的计算框架。
...
TL;DR: 该研究通过系统实验评估大语言模型在不同长度上下文中的鲁棒性，发现随着上下文长度增加，模型性能持续下降，多跳推理任务比单跨度抽取任务下降更显著。


<details>
  <summary>Details</summary>
Motivation: 大语言模型越来越多地部署在长且嘈杂的上下文环境中，但模型对上下文长度增长的鲁棒性在不同问答任务中仍不明确，需要系统研究来理解这一重要问题。

Method: 使用SQuAD和HotpotQA两个基准测试，通过系统增加无关上下文长度同时保留答案信号，评估模型准确率随总上下文长度的变化，从而隔离上下文长度效应与任务难度变化。

Result: 随着上下文长度增加，模型性能一致下降，多跳推理任务(HotpotQA)的性能下降幅度几乎是单跨度抽取任务(SQuAD)的两倍，表明多跳推理对上下文稀释特别脆弱。

Conclusion: 上下文长度鲁棒性应在评估模型可靠性时被明确考虑，特别是在涉及长文档或检索增强生成的应用中，多跳推理任务需要特别关注上下文稀释问题。
...
TL;DR: CUBE是一个通用基准测试协议标准，基于MCP和Gym构建，旨在解决AI代理基准测试碎片化问题，通过统一接口让基准测试一次包装、到处使用。


<details>
  <summary>Details</summary>
Motivation: 当前AI代理基准测试领域存在严重的碎片化问题，每个新基准都需要大量定制集成工作，形成了"集成税"，限制了全面评估的效率和研究生产力。

Method: 提出CUBE协议标准，基于MCP和Gym构建，通过将任务、基准、包和注册表关注点分离到不同的API层，实现基准测试的统一包装和通用访问。
...
TL;DR: Prose2Policy (P2P) 是一个基于LLM的工具，将自然语言访问控制策略自动转换为可执行的Rego代码，提供端到端处理流程，在ACRE数据集上表现出高编译率和测试通过率。
...
Method: 采用模块化端到端管道，包括策略检测、组件提取、模式验证、代码检查、编译、自动测试生成和执行，基于LLM将自然语言策略转换为OPA的Rego代码。

Result: 在ACRE数据集上评估显示：95.3%的接受策略编译成功，自动测试达到82.2%的正测试通过率和98.9%的负测试通过率，表明生成的Rego代码语法健壮且行为一致。

Conclusion: Prose2Policy能够有效生成适合零信任和合规环境的可执行策略代码，为自然语言策略到策略即代码的转换提供了实用解决方案。
...
TL;DR: GPT-4在不同社会经济角色下表现出与前景理论一致的风险决策行为，穷人角色更冒险，富人角色更保守，情感标签是事后解释而非决策驱动因素


<details>
  <summary>Details</summary>
Motivation: 研究LLM在不确定、顺序决策环境中表现的行为模式是源于原则性认知模式还是表面提示模仿，探索经典认知经济偏见是否隐含编码于大规模预训练语言模型中

Method: 将GPT-4分配三种社会经济角色（富人、中产、穷人），置于结构化老虎机环境中，包含三种机器配置（公平50%、偏置低35%、连胜动态概率），记录50次独立迭代和6,950个决策

Result: 穷人角色平均玩37.4轮，富人仅1.1轮，差异显著；风险分数效应量大；情感标签作为事后注释而非决策驱动；信念更新可忽略

Conclusion: LLM在没有明确指示的情况下再现了前景理论的关键行为特征，这对LLM代理设计、可解释性研究以及认知偏见在预训练模型中的隐式编码问题具有重要意义
...
TL;DR: 开发并优化了基于S&P 500历史数据和财报电话会议情感分析的增强算法交易策略，结合技术指标和FinBERT情感分析，显著超越基准模型表现


<details>
  <summary>Details</summary>
Motivation: 探索如何通过结合技术指标和市场情感分析来改进算法交易策略的性能，验证多维度数据融合在交易系统中的有效性

Method: 使用S&P 500历史市场数据和财报电话会议情感分析，整合移动平均线、动量、波动率等技术指标，并采用FinBERT进行情感分析，通过计算优化构建增强交易策略

Result: 增强策略在总回报率、夏普比率和回撤等多个指标上显著优于基准模型，证明了技术指标与情感分析结合的有效性

Conclusion: 结合技术指标、情感分析和计算优化的多维度方法在算法交易系统中具有重要价值和实际效果，为交易策略开发提供了有效框架
...
<details>
  <summary>Details</summary>
Motivation: 行为基础模型（BFMs）需要复杂的目标函数和充足的数据覆盖来学习有用的状态特征，但现有方法在特征选择上受限，且对数据覆盖要求高。本文研究是否必须使用复杂的表征学习目标来实现零样本强化学习。

Method: 提出正则化潜在动态预测（RLDP），在潜在空间自监督下一状态预测的基础上，添加简单的正交正则化来保持特征多样性，防止特征相似性增加和表征空间缩减。

Result: RLDP在零样本强化学习中能够匹配或超越最先进的复杂表征学习方法，并且在低数据覆盖场景下，先前方法表现不佳时RLDP仍能成功。

Conclusion: 复杂的表征学习目标并非零样本强化学习所必需，简单的正则化潜在动态预测方法就能取得优异性能，尤其在数据覆盖有限的实际场景中更具优势。
...
<details>
  <summary>Details</summary>
Motivation: 关键基础设施越来越多地采用具身AI进行监控、预测性维护和决策支持，但传统AI系统在处理超出训练假设的级联故障和危机动态时存在困难，需要新的治理框架来确保AI系统的韧性。

Method: 提出混合治理架构，基于任务复杂性、风险水平和后果严重性，将四种监督模式映射到关键基础设施领域，参考欧盟AI法案、ISO安全标准和危机管理研究，构建机器能力与人类判断的结构化分配框架。

Result: 建立了具身AI在关键基础设施中的治理框架，明确了不同监督模式的适用场景，为AI系统在危机情况下的有限自主性提供了理论依据和实践指导。

Conclusion: 具身AI在关键基础设施中的有效治理需要混合架构，通过有限自主性和结构化的人类监督来平衡机器能力与人类判断，确保系统在超出训练假设的危机情况下的韧性。
...
TL;DR: AsgardBench是一个评估具身AI中视觉基础的交互式规划能力的基准测试，专注于执行过程中的计划适应而非导航或低级操作。


<details>
  <summary>Details</summary>
Motivation: 现有的具身AI基准测试通常将推理与导航混为一谈，或提供丰富的纠正反馈替代感知。AsgardBench旨在隔离交互式规划能力，评估智能体基于视觉观察调整计划的能力，而不是低级执行。

Method: 创建包含108个任务实例的基准测试，涵盖12种任务类型，通过对象状态、放置和场景配置进行系统变化。限制智能体输入为图像、动作历史和轻量级成功/失败信号，在受控模拟器中隔离交互式规划。

Result: 对领先的视觉语言模型的评估显示，在没有视觉输入的情况下性能急剧下降，揭示了视觉基础和状态跟踪的弱点，这些弱点最终会破坏交互式规划能力。

Conclusion: AsgardBench专注于一个核心问题：模型能否真正利用所见内容在事情未按预期发展时调整计划？该基准测试揭示了当前模型在视觉基础和交互式规划方面的局限性。
...
TL;DR: AI-GENIE框架通过自适应提示工程显著提升LLM生成的人格评估项目质量，减少语义冗余并保持更大项目池，其效果随模型能力增强而提升。


<details>
  <summary>Details</summary>
Motivation: 研究动机是探索不同提示工程策略如何影响大型语言模型生成的人格评估项目质量，特别是在AI-GENIE生成心理测量框架中，了解自适应提示相对于传统提示方法的优势。

Method: 采用蒙特卡洛模拟方法，在AI-GENIE框架下生成针对大五人格特质的项目池。使用多种提示设计（零样本、少样本、基于角色、自适应）、模型温度和不同LLM，然后通过网络心理测量方法评估和缩减项目池。

Result: AI-GENIE在所有条件下都能可靠地提高缩减后的结构效度，其增量贡献与输入项目池质量成反比。自适应提示在减少语义冗余、提高缩减前结构效度和保留更大项目池方面始终优于非自适应策略，特别是与更新、更高容量的模型配对时。这些增益在大多数模型的温度设置中都很稳健，但GPT-4o模型在高温度下表现出例外。

Conclusion: 自适应提示是生成心理测量中最有效的方法，其优势随模型能力增强而扩大。研究结果表明需要继续探索模型-提示交互在生成心理测量流程中的作用，特别是不同模型对自适应约束的敏感性差异。
...
TL;DR: AI辅助完成Vlasov-Maxwell-Landau系统平衡特性的Lean 4形式化证明，展示了完整AI数学研究循环


<details>
  <summary>Details</summary>
Motivation: 展示AI辅助数学研究的完整工作流程，验证AI在复杂数学问题形式化证明中的能力

Method: 使用Gemini DeepThink生成证明，Claude Code翻译为Lean代码，Aristotle证明器解决111个引理，Lean内核验证，人类监督整个过程

Result: 成功完成VML系统平衡特性的形式化证明，所有229个人类提示和213次git提交公开，成本200美元，耗时10天

Conclusion: AI辅助数学研究流程可行，但需要人类监督关键定义和定理，形式化证明可先于数学论文完成
...
TL;DR: 论文提出将计算论证与LLMs结合，创建"论证式人机决策"新范式，使AI决策可解释、可争议、可修订，实现真正的人机协同推理。


<details>
  <summary>Details</summary>
Motivation: 计算论证虽提供透明可验证的推理框架，但依赖领域知识和特征工程；LLMs擅长处理非结构化文本，但推理过程不透明难以信任。两者结合可解决各自局限，为高风险领域创建可信赖的人机协同决策系统。

Method: 通过论证框架挖掘（从文本提取论证结构）、论证框架合成（构建形式化论证网络）、论证推理（进行辩证推理）三个环节的协同，将LLMs的非结构化文本处理能力与计算论证的形式化框架相结合。

Result: 提出"论证式人机决策"新范式，使AI不仅能解释决策，还能参与辩证过程，让决策可争议、可修订，实现"与人推理"而非"为人推理"的智能体。

Conclusion: 计算论证与LLMs的融合对于高风险领域构建人类可理解、可信赖的AI系统至关重要，为真正的人机协同决策奠定了理论基础。
...
TL;DR: Agent Rosetta：一个结合大语言模型推理与Rosetta蛋白质设计软件能力的智能体，能够处理非标准氨基酸，性能媲美专业模型和专家基线。


<details>
  <summary>Details</summary>
Motivation: 当前机器学习方法在蛋白质设计中主要局限于标准氨基酸和狭窄目标，缺乏能够处理广泛设计流程的通用工具。需要一种能够结合大语言模型推理能力和专业物理建模软件的方法。

Method: 开发Agent Rosetta，将大语言模型智能体与结构化环境结合，用于操作Rosetta蛋白质设计软件。通过迭代优化设计来实现用户定义的目标，结合LLM的推理能力和Rosetta的通用性。

Result: 在标准氨基酸设计中，Agent Rosetta性能与专业模型和专家基线相当；在非标准氨基酸设计中（机器学习方法失败的情况下），也能达到可比性能。研究还发现仅靠提示工程通常无法生成有效的Rosetta操作，证明环境设计对集成LLM智能体与专业软件至关重要。

Conclusion: 适当设计的环境使LLM智能体能够使科学软件更易访问，同时匹配专业工具和人类专家的性能，为复杂科学任务的自主执行开辟了新途径。
...
TL;DR: 提出結合時間序列預測與模擬模型的兩階段框架，優化疫情期間醫院容量管理與病人轉移策略


<details>
  <summary>Details</summary>
Motivation: COVID-19疫情對全球醫療系統造成巨大壓力，醫院容量面臨嚴峻挑戰，需要有效的病人轉移策略來應對

Method: 兩部分框架：1) 時間序列預測模型預測病人到達率；2) 模擬模型評估不同病人轉移策略的影響，考慮床位、人員、運輸、病人狀況等因素

Result: 提供醫院管理者綜合決策支持工具，能夠預測需求、模擬轉移策略、實施最優政策來分配病人和資源

Conclusion: 該框架旨在增強醫療系統面對COVID-19及未來疫情的韌性，通過預測分析和模擬建模優化醫院容量管理
...
TL;DR: MAC是一种多智能体宪法学习方法，通过结构化规则集优化提示，在PII标注等任务上超越现有提示优化方法50%以上，性能接近监督微调但无需参数更新。


<details>
  <summary>Details</summary>
Motivation: 现有基于LLM的提示优化器在自动学习宪法规则方面效果不佳，因为需要大量标注示例且缺乏结构化提示，导致随着提示规模增长改进有限。

Method: 提出多智能体宪法学习(MAC)，通过专门任务（接受、编辑、拒绝规则更新）的智能体网络优化结构化规则集；MAC+通过训练智能体在成功轨迹上强化高奖励的更新。

Result: 在PII标注任务上，MAC超越近期提示优化方法50%以上，产生人类可读可审计的规则集，性能接近监督微调和GRPO但无需参数更新，并能推广到工具调用等其他智能体任务。

Conclusion: MAC通过多智能体结构化规则优化有效解决了现有提示优化方法的局限性，实现了高性能、可解释且无需参数更新的宪法学习。
...
TL;DR: 安全属性在存在合取能力依赖时不具有组合性：两个各自无法达到禁止能力的智能体组合后，可通过涌现的合取依赖共同达到禁止目标


<details>
  <summary>Details</summary>
Motivation: 研究智能体安全属性的组合性问题，特别关注合取能力依赖对安全性的影响，挑战传统认为个体安全可保证组合安全的假设

Method: 采用形式化证明方法，通过构造性示例展示两个各自安全的智能体在组合时如何通过合取能力依赖产生不安全行为

Result: 首次形式化证明安全属性在合取能力依赖下不具有组合性，即个体安全不能保证组合安全

Conclusion: 在存在合取能力依赖的多智能体系统中，需要超越个体安全分析，考虑组合后的涌现行为以确保整体安全性
...
TL;DR: 提出了petscagent-bench框架，用于评估HPC科学代码生成，通过代理评估代理的方式，在五个维度（正确性、性能、代码质量、算法适当性、库特定约定）进行全面评估，弥补传统测试用例匹配的不足。


<details>
  <summary>Details</summary>
Motivation: 传统基准测试仅通过测试用例匹配评估代码生成，无法满足HPC库代码的复杂需求，如求解器选择、API约定、内存管理和性能等关键因素。需要更全面的评估框架来评估科学代码生成质量。

Method: 开发了petscagent-bench代理框架，采用代理评估代理范式。部署工具增强的评估代理，编译、执行和测量被测试代理生成的代码，通过14个评估器组成的流水线，在五个评分类别中进行评估。代理通过标准化协议（A2A和MCP）通信，支持黑盒评估。

Result: 在PETSc库的HPC实际问题上进行实证分析，发现当前前沿模型能生成可读性好、结构良好的代码，但在库特定约定方面持续存在困难，这是传统通过/失败指标完全忽略的问题。

Conclusion: petscagent-bench框架为科学代码生成提供了更全面的评估方法，揭示了传统评估方法的局限性，特别是对库特定约定的忽视，为改进代码生成模型提供了重要方向。
...
TL;DR: 论文指出客户服务自动化正从脚本化聊天机器人和单智能体响应转向专业化AI智能体网络，这种转变带来了安全漏洞：即使单个智能体被验证为安全，组合后仍可能通过涌现的联合依赖达成被禁止的目标。


<details>
  <summary>Details</summary>
Motivation: 客户服务自动化正在经历结构性转型，从脚本化聊天机器人和单智能体响应转向专业化AI智能体网络。这种转变虽然提升了能力，但引入了一个当前平台尚未解决的安全缺口：即使单个智能体被验证为安全，组合后仍可能通过涌现的联合依赖达成被禁止的目标。

Method: 论文通过分析客户服务自动化架构的转型趋势，识别了从单智能体到多智能体网络转变中出现的"涌现联合依赖"安全问题，提出了需要新的安全框架来解决这一漏洞。

Result: 研究发现当前客户服务自动化平台存在安全漏洞，即两个单独验证为安全的智能体组合后，可能通过涌现的联合依赖达成被禁止的目标，这种安全缺口目前没有平台能够解决。

Conclusion: 客户服务自动化向专业化智能体网络转型需要新的安全框架来解决涌现的联合依赖问题，确保智能体组合的安全性不能仅依赖于单个智能体的验证。
...
TL;DR: 论文提出了一种基于写入时门控的检索增强生成方法，通过复合显著性评分过滤知识对象并维护版本链，相比读取时过滤在噪声环境下具有显著优势。
...
Method: 引入写入时门控机制，使用复合显著性评分（来源声誉、新颖性、可靠性）过滤传入的知识对象，同时维护保留先前状态的版本链。该方法不需要质量标签的oracle访问，在真实LLM评估中验证。

Result: 写入时门控在8:1干扰物比例下保持100%准确率，而读取时过滤（Self-RAG）降至0%。在Wikipedia、药理学数据和arXiv论文上的验证显示，写入门控优势随参数化记忆支持减少而增加：Wikipedia +25pp，arXiv截止后 +48pp，零训练知识的程序数据 +65pp。写入门控以九分之一的查询时间成本达到Self-RAG的准确率。

Conclusion: 写入时门控相比读取时过滤在噪声环境下具有结构性优势，能够有效过滤噪声知识并维护信息版本历史，在真实LLM评估中表现出显著优越性，特别是在高干扰比例和低参数化记忆支持的情况下。
...
TL;DR: IRAM-Omega-Q是一个计算架构，使用类量子密度矩阵表示内部状态，通过自适应增益调节实现不确定性管理，在不同控制顺序下表现出不同的稳定性机制。


<details>
  <summary>Details</summary>
Motivation: 现有的人工智能代理在任务表现上很强，但其内部调节、不确定性管理和随机扰动下的稳定性机制不透明。需要建立一个能够明确建模内部调节的计算框架。

Method: 提出IRAM-Omega-Q架构，使用密度矩阵作为抽象状态描述符（不涉及物理量子过程），通过闭环控制调节内部状态。采用自适应增益维持目标不确定性水平，通过参数扫描、固定种子模拟和基于敏感性的相图分析研究稳定性。

Result: 在调节-噪声空间中识别出可重复的临界边界，发现感知优先和行动优先两种控制顺序在相同外部条件下诱导出不同的稳定性机制。

Conclusion: 不确定性调节可作为人工代理的具体架构原则，该框架为研究认知启发AI系统的稳定性、控制和顺序效应提供了形式化设置。类量子形式主义仅作为数学表示工具，不涉及意识现象学。
...
TL;DR: 提出Model Workspace Protocol (MWP)，用文件系统结构替代框架级编排，通过编号文件夹和Markdown文件实现单智能体顺序工作流，减少工程开销。


<details>
  <summary>Details</summary>
Motivation: 现有AI智能体编排方法通常需要构建多智能体框架来处理上下文传递、内存、错误处理和步骤协调，但对于需要人工逐步骤审查的顺序工作流来说，这些框架引入了不必要的工程开销。

Method: 采用文件系统结构替代框架级编排：1) 用编号文件夹表示工作阶段；2) 用纯Markdown文件携带提示和上下文，告诉单个AI智能体在每个步骤中扮演什么角色；3) 用本地脚本处理不需要AI参与的机械工作。

Result: 开发了Model Workspace Protocol (MWP)，使单个智能体通过读取适当文件在适当时刻执行原本需要多智能体框架才能完成的工作，协议采用MIT许可证开源。

Conclusion: MWP将Unix管道设计、模块化分解、多遍编译和文学编程等思想应用于AI智能体上下文结构化问题，为顺序工作流提供了更轻量、更简单的编排方案。
...
TL;DR: 提出参数高效微调策略，通过LLM生成语义等价但结构多样的指令集来增强OpenVLA的语言泛化能力


<details>
  <summary>Details</summary>
Motivation: OpenVLA作为视觉-语言-动作模型的SOTA，在遇到全新环境时零样本性能有限，需要提升其语言泛化能力

Method: 使用LLM为Bridge Dataset V2合成通用指令集，生成语义等价但结构多样的命令，采用LoRA对OpenVLA进行参数高效微调

Result: LoRA增强模型表现出更好的鲁棒性，表明丰富专业数据集的语言空间对具身智能体至关重要

Conclusion: 通过参数高效微调和语言数据增强，可以有效提升视觉-语言-动作模型在具身AI中的语言泛化能力
...
TL;DR: POaaS是一个专为设备端小语言模型设计的轻量级提示优化层，通过路由查询到轻量级专家模块并合并输出，在严格约束下提升任务准确性和事实性。


<details>
  <summary>Details</summary>
Motivation: 设备端小语言模型面临用户提示不完美（如拼写错误、意图不清、缺少上下文）导致事实错误和幻觉的问题。现有的自动提示优化方法是为云端大模型设计的，依赖搜索产生冗长结构化指令，在设备端同一小模型既作为优化器又作为求解器的约束下，会浪费上下文甚至降低准确性。

Method: 提出POaaS（最小编辑提示优化层），将每个查询路由到轻量级专家模块（Cleaner清理器、Paraphraser改写器、Fact-Adder事实添加器），在严格的漂移和长度约束下合并输出，并对格式良好的提示采用保守跳过策略。

Result: 在严格固定模型设置下（Llama-3.2-3B-Instruct和Llama-3.1-8B-Instruct），POaaS提升了任务准确性和事实性，而代表性APO基线方法反而降低了性能。在token删除和混合情况下，POaaS恢复了高达+7.4%的性能。
...
TL;DR: 提出Context Alignment Pre-processor (C.A.P.)框架，在LLM生成响应前进行上下文对齐预处理，解决长对话中上下文错位问题


<details>
  <summary>Details</summary>
Motivation: LLM在生成长对话时面临上下文错位挑战，当用户省略前提、简化指代或突然切换话题时，模型可能无法捕捉真实意图，产生机械或离题响应，削弱对话协作潜力

Method: 提出C.A.P.预处理框架，包含三个核心过程：1)语义扩展，将用户指令扩展到包含前提、字面意义和含义的更广语义范围；2)时间加权上下文检索，通过时间衰减函数优先考虑最近对话历史；3)对齐验证和决策分支，通过测量当前提示与加权历史上下文的语义相似度评估对话是否偏离轨道，检测到显著偏差时启动结构化澄清协议

Result: 论文提出了C.A.P.的架构和理论基础，借鉴认知科学和人机交互中的共同基础理论，讨论了实现路径、评估方法以及对未来交互式智能系统设计的影响

Conclusion: C.A.P.不仅是技术改进，更是将人机对话从单向命令执行模式转向双向、自我修正、基于伙伴关系的协作的重要一步
...
<details>
  <summary>Details</summary>
Motivation: 现有方法孤立处理每个数学问题，没有利用训练中积累的可重用策略。需要一种能够捕获和复用成功解题策略的框架。

Method: 分层强化学习框架，包含技能管理器（高层）和工作器（低层）。管理器通过结构化总结成功解题轨迹来构建分层技能库，并使用策略驱动的选择机制在解题前检索相关技能。

Result: 在两个基础模型和七个基准测试（竞赛数学和Omni-MATH）上，ARISE持续优于GRPO系列算法和记忆增强基线，在分布外任务上表现尤为突出。

Conclusion: ARISE通过技能库的协同进化有效提升了数学推理能力，每个组件都对性能提升有贡献，且库质量和推理性能在训练中同步提升。
...
TL;DR: VIGIL是一个边缘扩展的代理AI系统，通过在用户设备上部署桌面代理，实现本地诊断、企业知识检索和政策管控的修复，显著减少IT支持交互次数并提高诊断速度。


<details>
  <summary>Details</summary>
Motivation: 企业IT支持面临设备异构、策略演变和长尾故障模式等挑战，这些难以通过集中式方式解决。需要一种能够在用户设备上直接进行诊断和修复的系统。

Method: VIGIL系统在用户设备上部署桌面代理，执行本地诊断、检索企业知识库，并在政策管控下进行修复，同时确保用户明确同意和端到端可观察性。

Result: 在100个资源受限端点的10周试点中，VIGIL减少了39%的交互轮次，诊断速度至少提高4倍，82%的匹配案例实现自助解决。用户报告了优秀的可用性、高信任度和低认知负荷。

Conclusion: VIGIL展示了边缘AI代理在企业IT支持中的有效性，设备端诊断的价值独立于知识库覆盖范围，为大规模持续改进建立了安全和可观察性基础。
...
TL;DR: NeuronSpark是一个0.9B参数的纯脉冲神经网络语言模型，通过直接训练而非Transformer蒸馏实现大规模语言建模，在有限计算预算下达到3.6的预训练损失并展示多轮对话能力。


<details>
  <summary>Details</summary>
Motivation: 探索纯脉冲神经网络架构是否能够从随机初始化开始学习大规模语言建模，而不依赖于Transformer蒸馏，验证SNN在语言建模任务中的端到端可行性。

Method: 结合选择性状态空间脉冲动力学、泄漏电流层间通信、PonderNet自适应时间步长、融合Triton PLIF内核，以及残差中心化、侧向抑制归一化、自然梯度补偿等稳定化技术。

Result: 在有限计算预算下（约14亿预训练token和6500步SFT），NeuronSpark-0.9B达到3.6的预训练损失，并在SFT后展示出早期的多轮对话行为。

Conclusion: 结果表明在这个规模上，使用纯SNN架构进行端到端语言建模是可行的，为脉冲神经网络在大规模语言任务中的应用提供了实证支持。
...
<details>
  <summary>Details</summary>
Motivation: 当前Text-to-SQL主要局限于单轮范式，多轮代理式强化学习面临信用分配瓶颈。传统方法仅依赖最终轮次反馈，忽略中间过程，导致信用评估模糊。

Method: 提出Agentic SQL框架，包含双层奖励机制：1) 聚合轨迹奖励(ATR)解决多轮信用分配，使用非对称转移矩阵聚合过程导向分数，基于Lyapunov稳定性理论保证无环策略和单调收敛；2) 列集匹配奖励(CSMR)提供即时步级奖励，通过执行每轮查询将二元反馈转换为基于部分正确性的密集信号。

Result: 在BIRD数据集上比二元奖励GRPO提升5%，使用相同模型超越SOTA Arctic-Text2SQL-R1-7B在BIRD和Spider 2.0上的表现。

Conclusion: Agentic SQL框架有效解决多轮Text-to-SQL的信用分配问题，推动Text-to-SQL向稳健的多轮代理范式发展。
...
TL;DR: 该论文通过三项互补实验对六个前沿大语言模型进行污染审计，发现MMLU基准存在显著数据污染，STEM领域污染最严重，模型性能因此被高估。


<details>
  <summary>Details</summary>
Motivation: 公共排行榜显示大语言模型在多项基准测试中超越人类专家，但大多数基准数据公开且可能被模型训练时使用，存在系统性数据污染风险，需要严谨的污染审计来评估模型真实能力。

Method: 采用多方法污染审计：1) 对513个MMLU问题的词汇污染检测；2) 对100个MMLU问题的改写和间接引用诊断；3) 对所有问题和六个模型的TS-Guessing行为探测。

Result: 发现总体污染率13.8%（STEM领域18.1%，哲学高达66.7%），间接引用下准确率平均下降7.0个百分点（法律和伦理下降19.8pp），72.5%问题触发远高于随机水平的记忆信号，污染程度排序：STEM > 专业领域 > 社会科学 > 人文学科。

Conclusion: 当前大语言模型基准测试存在显著数据污染问题，导致模型性能被高估，需要更严谨的污染检测方法和更可靠的评估基准来准确评估模型真实能力。
...
TL;DR: 论文提出DS-IA双阶段意图感知框架，解决LLM在物联网中实体幻觉和交互频率困境问题，通过语义防火墙和级联验证器提升指令执行可靠性。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在物联网中作为具身代理时面临可靠性挑战：直接执行LLM生成的指令会导致实体幻觉（控制不存在的设备），而现有迭代框架存在交互频率困境（在鲁莽执行和过度询问用户之间摇摆）。

Method: 提出双阶段意图感知框架：第一阶段作为语义防火墙，通过检查家庭当前状态过滤无效指令并解析模糊命令；第二阶段采用确定性级联验证器，按顺序验证房间、设备和能力，确保动作在物理上可行后才执行。

Result: 在HomeBench和SAGE基准测试中，DS-IA实现58.56%的精确匹配率（比基线提高28%以上），无效指令拒绝率达到87.04%。在SAGE基准上，将自主成功率从42.86%提升到71.43%，同时保持高精度识别真正需要人工澄清的模糊性。

Conclusion: DS-IA框架通过准确的环境接地最小化用户干扰，解决了LLM在物联网中的可靠性和交互效率问题，平衡了主动查询和基于状态的推理，显著提升了智能家居控制的实用性。
...
TL;DR: MOSAIC：一个模块化框架，通过可学习的控制令牌实现组合式安全对齐，可在推理时灵活激活和组合不同安全约束，解决现有方法难以提供条件化安全控制的问题。


<details>
  <summary>Details</summary>
Motivation: 现有LLM安全对齐方法存在局限性：参数级对齐将安全行为与通用能力纠缠在一起，而基于提示的方法依赖自然语言指令，执行力较弱。现实部署需要根据用户、区域和应用而变化的上下文相关安全规则。
...
Conclusion: MOSAIC为LLM提供了灵活、可组合的安全对齐方法，能够适应不同上下文的安全需求，解决了现有静态安全策略的局限性。
...
TL;DR: 论文提出自适应心智理论（A-ToM）智能体，通过估计并对齐合作者的心智理论推理层级，解决多智能体协作中因心智理论层级不匹配导致的协调问题。


<details>
  <summary>Details</summary>
Motivation: 在多智能体协作任务中，心智理论（ToM）推理能力被认为能提升协调效果。但研究发现，当智能体之间的ToM推理层级不匹配时（如一方过度推理或推理不足），反而会损害协调性能。因此需要解决ToM层级对齐问题。

Method: 设计自适应ToM（A-ToM）智能体：基于历史交互估计合作者的ToM推理层级，利用该估计预测合作者行为，实现行为协调。在四个多智能体协调任务（重复矩阵博弈、两个网格导航任务、Overcooked任务）中进行实证评估。

Result: 实验结果验证了ToM对齐的重要性，并证明了A-ToM智能体的有效性。A-ToM能够成功对齐合作者的ToM层级，显著提升多智能体协调性能。

Conclusion: ToM层级对齐对多智能体协调至关重要，提出的A-ToM方法能有效解决对齐问题。研究还讨论了A-ToM对非LLM智能体的泛化性，以及可能降低ToM对齐重要性的因素。
...
<details>
  <summary>Details</summary>
Motivation: 当前遥感基准测试主要关注多模态大语言模型(MLLMs)的感知和推理能力评估，缺乏对规划能力的评估，这源于大规模规划任务构建和验证的困难，以及评估协议的不准确和不充分。
...
Conclusion: NeSy-Route能够支持更强大的遥感MLLMs的研究和开发，填补了当前基准测试在规划能力评估方面的空白，为全面评估MLLMs在复杂场景下的决策能力提供了重要工具。
...
TL;DR: 该论文提出将车辆诊断故障码序列视为语言处理，开发基于Transformer的架构进行预测维护、因果发现和布尔规则自动生成，实现车辆故障诊断自动化。


<details>
  <summary>Details</summary>
Motivation: 随着车辆复杂度增加，传统手动诊断故障码分组方法成本高、易出错且难以扩展。诊断故障码数量与自然语言词汇量相当，需要新的机器学习方法来处理高维事件流。

Method: 将事件序列建模、因果发现和大语言模型统一到高维事件流框架中，采用三部分结构：1)基于Transformer的预测维护架构；2)可扩展的样本级和群体级因果发现框架；3)自动化布尔错误模式规则合成的多智能体系统。

Result: 提出了处理高维事件流的新方法，能够自动进行故障诊断，从预测到因果理解再到推理的完整诊断流程，解决了传统统计方法无法处理高维数据的挑战。

Conclusion: 将诊断序列视为语言处理的新范式，通过统一事件序列建模、因果发现和LLMs，实现了车辆故障诊断的自动化，为工业日志中的高维事件流处理提供了创新解决方案。
...
TL;DR: FactorEngine (FE) 是一个程序级因子发现框架，通过将因子表示为图灵完备代码，结合LLM引导搜索和知识注入，在真实市场数据上实现了优于基线方法的预测稳定性和投资组合表现。


<details>
  <summary>Details</summary>
Motivation: 现有符号方法表达能力有限，而神经网络预测器在可解释性和对市场机制变化的鲁棒性方面存在不足。需要一种既能保持可执行性和可审计性，又能在大规模计算中保持高效的方法来自动发现预测信号。

Method: FactorEngine (FE) 采用三个分离：逻辑修订vs参数优化、LLM引导方向搜索vs贝叶斯超参数搜索、LLM使用vs本地计算。包含知识注入引导模块，通过多智能体提取-验证-代码生成管道将非结构化财务报告转换为可执行因子程序，并建立经验知识库支持轨迹感知优化。

Result: 在真实世界OHLCV数据的广泛回测中，FE生成的因子在预测稳定性和投资组合影响方面显著优于基线方法，实现了更高的IC/ICIR（及Rank IC/ICIR）和改进的AR/Sharpe比率，达到了最先进的预测和投资组合表现。

Conclusion: FactorEngine 通过将因子表示为图灵完备代码，结合LLM引导搜索和知识注入，成功解决了因子挖掘中的可执行性、可审计性和计算效率问题，在真实市场数据上实现了卓越的预测和投资组合表现。
...
TL;DR: 论文提出理论解释：为什么仅使用负面反馈训练大语言模型能媲美甚至超越标准RLHF，核心在于正面偏好与负面约束的结构不对称性。


<details>
  <summary>Details</summary>
Motivation: 近期实证研究表明，仅使用负面反馈训练LLMs能达到或超越标准RLHF效果，但缺乏统一的理论解释。本文旨在解释为什么负面信号如此有效。

Method: 提出理论框架：正面偏好与负面约束存在结构性不对称。正面偏好编码连续耦合、上下文依赖的人类价值观，难以穷尽指定；负面约束编码离散、有限、可独立验证的禁令，能收敛到稳定边界。这一不对称性根植于波普尔的证伪逻辑和负面知识认识论。

Result: 该理论框架既能解释基于偏好的RLHF中出现的谄媚性失败，也能解释负面信号方法的惊人有效性。为对齐研究提供了新的理论视角。

Conclusion: 对齐研究应从"学习人类偏好什么"转向"学习人类拒绝什么"，并提供了该框架的可测试预测。
...
TL;DR: 提出OQL（期权查询语言）作为中间表示，将自然语言交易意图转换为期权策略，通过神经符号管道提高执行准确性和逻辑一致性


<details>
  <summary>Details</summary>
Motivation: 虽然大语言模型擅长通用代码生成，但将自然语言交易意图转换为正确的期权策略仍然具有挑战性。现实世界的期权设计需要处理海量、多维度的期权链数据并满足严格约束，这常常使直接生成方法不堪重负。

Method: 引入期权查询语言（OQL），这是一种领域特定的中间表示，将期权市场抽象为符合语法规则的高级原语，使LLM能够作为可靠的语义解析器而非自由形式的程序员。OQL查询随后由引擎进行验证和确定性执行，以实例化可执行策略。

Result: 为该任务创建了新数据集，并证明神经符号管道在执行准确性和逻辑一致性方面显著优于直接基线方法。

Conclusion: OQL作为中间表示层，结合LLM的语义理解能力和确定性执行引擎，有效解决了将自然语言交易意图转换为正确期权策略的挑战，提高了系统的可靠性和一致性。
...
TL;DR: 视觉输入会绕过文本安全机制，改变最先进视觉语言模型的道德决策，暴露多模态安全对齐的紧迫需求


<details>
  <summary>Details</summary>
Motivation: 随着AI系统从文本助手发展为具身智能体，确保跨模态的道德推理一致性变得至关重要。当前安全技术在文本环境中有效，但缺乏对视觉输入的泛化能力，现有道德评估基准仅基于文本格式且缺乏对影响道德决策变量的系统控制。
...
TL;DR: TRUST-SQL提出了一种在未知数据库模式下的Text-to-SQL解析方法，通过自主代理和四阶段协议动态识别相关模式信息，无需预先加载完整模式，在多个基准测试中显著优于传统方法。


<details>
  <summary>Details</summary>
Motivation: 现实企业环境中数据库包含数百个表和大量噪声元数据，传统Text-to-SQL解析依赖完整模式假设不切实际。需要开发能够在未知模式下动态识别和验证相关子集的解决方案。

Method: 将任务建模为部分可观测马尔可夫决策过程，采用四阶段协议：1) 模式探索 2) 模式验证 3) SQL生成 4) 执行验证。提出Dual-Track GRPO策略，通过令牌级掩码优势分离探索奖励和执行结果，解决信用分配问题。

Result: 在五个基准测试中，TRUST-SQL的4B和8B变体分别实现了30.6%和16.6%的平均绝对改进。Dual-Track GRPO相比标准GRPO带来9.9%的相对提升。尽管完全不依赖预加载元数据，仍能匹配或超越依赖模式预填充的强基线方法。

Conclusion: TRUST-SQL成功解决了未知模式下的Text-to-SQL解析问题，通过自主代理和结构化协议实现了高效的模式发现和验证，为现实企业环境中的数据库查询提供了实用解决方案。
...
TL;DR: 论文提出RetailBench基准和Evolving Strategy & Execution框架，用于评估和改进LLM智能体在现实商业场景中的长时程决策能力，实验表明框架能提升稳定性但仍有局限性。


<details>
  <summary>Details</summary>
Motivation: 当前基于LLM的智能体在短期、结构化任务上表现良好，但在现实动态环境中进行长时程连贯决策的能力仍面临挑战。需要评估和改进LLM在非平稳环境中的长期决策能力。

Method: 提出RetailBench高保真基准，用于评估现实商业场景中的长时程自主决策；提出Evolving Strategy & Execution框架，将高层战略推理与低层动作执行分离，支持自适应、可解释的策略演化。

Result: 在8个最先进LLM上的实验表明，该框架相比其他基线提高了操作稳定性和效率。但随着任务复杂度增加，性能显著下降，揭示了当前LLM在长时程、多因素决策方面的根本局限性。

Conclusion: 虽然提出的框架能改善LLM智能体在长时程任务中的表现，但当前LLM在复杂、动态环境中的长期决策能力仍有根本性限制，需要进一步研究。
...
TL;DR: HyDRA：一种混合证据演绎推理架构，通过提出-验证-决策协议解决开放词汇多模态情感识别中的模糊性，利用强化学习优化推理轨迹


<details>
  <summary>Details</summary>
Motivation: 开放词汇多模态情感识别面临多模态线索模糊性的挑战，现有MLLMs容易过早依赖主导数据先验而忽略互补情感线索，需要更深入的情感推理机制

Method: 提出HyDRA架构，采用提出-验证-决策协议进行推理，使用强化学习与分层奖励塑造来优化推理轨迹，确保多模态线索的最佳协调

Result: 系统评估验证了设计选择，HyDRA在模糊或冲突场景中持续优于强基线，同时提供可解释的诊断证据轨迹

Conclusion: 有效的情感推理需要超越表面关联，通过从多个潜在视角协调观察来重建细微情感状态，HyDRA为此提供了有效的解决方案
...
TL;DR: 论文发现，在基于模式的推理流程中，LLM生成的中间结构（如检查清单、验证查询）更像是影响性上下文而非稳定的因果中介，模型在60%情况下无法根据修改后的中间结构更新最终决策。


<details>
  <summary>Details</summary>
Motivation: 研究动机是探究基于模式的推理流程中，LLM生成的中间结构（如评分标准、检查清单、验证查询）是否真正因果性地决定最终输出，还是仅仅伴随最终决策。当前缺乏直接测量这种因果关系的评估方法。

Method: 引入因果评估协议：选择那些中间结构到决策存在确定性函数映射的任务，通过控制性编辑中间结构，每个编辑都对应唯一的正确输出。在8个模型和3个基准测试中，评估模型是否根据修改后的中间结构更新预测。

Result: 模型表面上与自己的中间结构保持一致，但在高达60%的情况下，干预后无法更新预测，表明表观忠实性在中间结构改变时是脆弱的。当将最终决策推导委托给外部工具时，这种脆弱性基本消失；但提示要求优先考虑中间结构而非原始输入并不能显著缩小差距。

Conclusion: 基于模式的推理流程中的中间结构更多是作为有影响力的上下文，而非稳定的因果中介。模型缺乏真正的因果推理能力，中间结构改变时无法一致地更新决策。
...
<details>
  <summary>Details</summary>
Motivation: 当前高速公路运营依赖基于规则和孤立的模型，限制了跨系统知识联合分析能力。通用大语言模型无法有效理解高速公路领域非常规场景的规则和因果关系，需要专门针对高速公路领域的多模态大模型。
...
Conclusion: ExpressMind作为智能高速公路运营的认知核心，通过专门的数据集构建、创新的预训练范式、知识增强和推理对齐机制，成功解决了高速公路领域多模态理解和推理的挑战，为智能交通系统提供了有效的解决方案。
...
TL;DR: 小型语言模型通过合成数据集进行领域特定代码生成的研究，比较了少样本提示、检索增强生成和LoRA微调三种定制策略，发现LoRA微调在准确性和领域对齐方面表现最佳。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在通用代码生成方面表现良好，但在需要领域特定库、API或约定的专业编程环境中往往表现不佳。定制小型开源模型为依赖大型专有系统提供了经济有效的替代方案。
...
TL;DR: 提出"有界校准与可争议性"的前端模式，用于LLM驱动的机器人在社交场景中分配稀缺援助时处理多元价值观和LLM行为不确定性，通过约束可接受模式、保持模式可读性、提供特定结果争议途径来平衡用户需求。


<details>
  <summary>Details</summary>
Motivation: LLM驱动的机器人在社交场景中分配稀缺援助时面临两个核心挑战：多元价值观（不同人对优先帮助谁有合理分歧）和LLM行为可变性（不同提示、上下文和群体中行为难以预测）。现有用户面向的实时多用户援助分配护栏机制不够明确，需要一种既能避免隐藏默认值偏斜，又不给用户带来时间压力负担的解决方案。

Method: 提出"有界校准与可争议性"的前端模式，包含三个核心组件：(1) 将优先级分配约束在治理批准的可接受模式菜单内；(2) 在交互相关术语中保持活动模式的可读性；(3) 提供特定结果的争议途径，无需重新协商全局规则。该模式将多元主义和LLM不确定性视为常态条件。

Result: 通过公共广场机器人场景示例说明该模式的应用，并提出了以可读性、程序合法性和可操作性为中心的评估议程，包括自动化偏见风险和不均衡的争议渠道可用性等风险分析。

Conclusion: 该模式为LLM驱动的机器人在社交援助分配中提供了一种平衡方案，既避免了隐藏默认值的隐性价值偏斜，又避免了在时间压力下给用户带来负担的完全可配置"价值设置"，通过约束、可读性和争议机制建立了更负责任的交互框架。
...
TL;DR: 论文评估LLMs在第三方沟通场景中能否恰当应用或抑制基于记忆的用户偏好，发现前沿模型难以做到上下文敏感，常将个性化偏好当作全局规则而非情境依赖的规范信号。


<details>
  <summary>Details</summary>
Motivation: LLMs越来越多地将用户偏好存储在持久记忆中以实现个性化，但在受社会制度规范约束的第三方沟通场景中，某些用户偏好可能不适用。需要评估LLMs能否根据上下文恰当应用或抑制这些偏好。

Method: 引入BenchPreS基准，使用两个互补指标：误用率（MR）和恰当应用率（AAR），评估LLMs在不同沟通场景中应用用户偏好的表现。分析模型偏好遵循强度、推理能力和提示防御策略的影响。

Result: 即使前沿LLMs也难以以上下文敏感的方式应用偏好；偏好遵循性更强的模型表现出更高的过度应用率；推理能力和基于提示的防御策略都无法完全解决此问题。

Conclusion: 当前LLMs将个性化偏好视为全局可执行规则而非情境依赖的规范信号，需要在记忆系统中加入上下文敏感性以实现更符合社会规范的个性化。
...
TL;DR: V-DyKnow是一个评估视觉语言模型中时间敏感事实知识的基准测试，发现VLMs经常输出过时事实，现有知识更新方法在多模态间效果不一致。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型通常在静态数据快照上训练，将事实知识视为时间不变的。但现实世界的事实具有时间敏感性，会经历不规则和周期性变化，导致模型预测过时。需要评估VLMs处理时间敏感知识的能力。

Method: 提出了V-DyKnow视觉动态知识基准，用于评估VLMs中的时间敏感事实知识。该基准用于：1) 评估模型跨模态和输入扰动的可靠性；2) 测试知识编辑和多模态RAG方法在多模态知识更新中的效果；3) 通过数据和机制分析过时预测的来源。

Result: VLMs经常输出过时事实，反映了训练阶段使用的过时数据快照。从文本到视觉刺激，事实可靠性会下降，即使实体被正确识别。现有的对齐方法无法在多模态间一致地更新模型知识。

Conclusion: 当前VLMs在获取和更新跨模态的时间敏感知识方面存在根本性限制。研究团队发布了基准、代码和评估数据，以促进该领域的研究。
...
TL;DR: 论文提出将AI代理的执行路径作为运行时治理的核心对象，通过确定性函数将代理身份、部分路径、拟议行动和组织状态映射到策略违规概率，建立形式化框架。


<details>
  <summary>Details</summary>
Motivation: AI代理基于大语言模型产生非确定性、路径依赖的行为，无法在设计时完全治理。需要平衡高任务完成率与法律、数据泄露、声誉等成本，因此需要有效的运行时治理机制。

Method: 将合规策略形式化为确定性函数，映射代理身份、部分执行路径、拟议下一步行动和组织状态到策略违规概率。将提示级指令和静态访问控制视为该框架的特例。

Result: 建立了AI代理治理的形式化分析框架，提出了具体策略示例（受AI法案启发），讨论了参考实现，并识别了风险校准和强制合规限制等开放问题。

Conclusion: 执行路径是有效运行时治理的核心对象，运行时评估是处理路径依赖策略的必要通用方法，为AI代理治理提供了理论基础和实践框架。
...
TL;DR: 该研究通过2026年中东冲突早期阶段的时间节点分析，评估AI在训练数据截止后对地缘政治事件的推理能力，发现当前LLM展现出战略现实主义思维，但能力在不同领域分布不均，且叙事随时间演变。


<details>
  <summary>Details</summary>
Motivation: 解决回顾性地缘政治预测中训练数据泄露的混淆问题，研究AI在训练截止后对真实世界冲突的推理能力，为评估模型在"战争迷雾"下的分析能力提供时间锚定框架。

Method: 构建11个关键时间节点、42个节点特定可验证问题和5个一般探索性问题，要求模型仅基于每个时刻公开可获得的信息进行推理，从而大幅减少训练数据泄露问题。

Result: 1. 当前最先进LLM展现出显著的战略现实主义，能超越表面修辞推理深层结构性激励；2. 能力分布不均：在经济和后勤结构化环境中更可靠，在政治模糊的多行为体环境中较差；3. 模型叙事随时间演变：从早期快速遏制预期转向系统性区域固守和消耗性降级解释。

Conclusion: 该研究提供了首个对LLM在持续地缘政治冲突中推理的时间锚定分析，可作为模型在危机演变过程中推理能力的档案快照，为未来无后见之偏的研究奠定基础。
...
TL;DR: 将约束传播集成到动态规划中，通过约束传播剪枝状态和转移，在三个组合优化问题上显著减少状态扩展数量，提升求解能力。


<details>
  <summary>Details</summary>
Motivation: 桥接动态规划（DP）和约束规划（CP）两种范式，利用CP的约束传播能力来增强DP求解器，解决组合优化问题。

Method: 在Domain-Independent Dynamic Programming框架中集成通用CP求解器进行约束传播，通过启发式搜索在单机调度、资源受限项目调度和带时间窗旅行商问题上评估。

Result: 约束传播显著减少状态扩展数量，在单机调度和RCPSP上比纯DP求解器解决更多实例，在紧约束TSPTW实例上也有类似改进。

Conclusion: 约束传播在DP求解器中具有重要价值，为集成DP和CP提供了基于模型的方法，但需要进一步减少传播开销以提升性能。
...
TL;DR: 提出PINO框架，结合强化学习与基于论证的规范监督，实现规范合规的智能体，并研究规范规避问题。


<details>
  <summary>Details</summary>
Motivation: 随着AI快速发展，需要开发能够遵守社会规则和规范的智能体，以便安全地融入日常生活。受匹诺曹故事启发，需要解决规范合规和情境感知智能体的问题。

Method: 基于AJAR、Jiminy和NGRL架构，提出PINO混合模型：强化学习智能体由基于论证的规范顾问监督。开发了自动提取论证和关系的新算法，并研究了规范规避现象及其缓解策略。

Result: 对管道各组件进行了实证评估，验证了方法的有效性。

Conclusion: 讨论了相关工作、当前局限性和未来研究方向，为开发规范合规的AI系统提供了框架。
...
TL;DR: 通过微调语言模型学习期刊发表决策，AI能够超越人类专家和前沿模型，准确预测哪些研究创意值得发表，实现了对"科学品味"的量化提取。


<details>
  <summary>Details</summary>
Motivation: 科学进步最关键的驱动力不是推理能力，而是"品味"——判断哪些未经验证的想法值得追求的能力。这种能力被编辑和资助者日常运用，但从未被成功阐述、教授或自动化。AI在可验证答案的任务上已超越人类，但在科学品味判断上仍表现不佳。

Method: 通过微调语言模型学习多年的期刊发表记录，训练模型识别高质量研究创意。使用管理学领域的四层质量研究提案作为测试基准，比较了11个前沿模型、期刊编辑专家小组和微调模型的性能。还验证了模型在经济学领域的泛化能力。

Result: 前沿模型平均准确率仅31%，略高于随机猜测；期刊编辑专家小组多数投票准确率为42%；而微调模型超越了所有前沿模型和专家小组，最佳单一模型达到59%准确率。模型表现出校准的置信度，在其最高置信度预测上达到100%准确率，并能将评估信号迁移到未训练的成对比较和一句话摘要任务。在经济学领域，微调模型达到70%准确率。

Conclusion: 科学品味并非AI无法触及，而是被沉积在制度记录中等待提取。微调语言模型学习发表决策提供了一种可扩展的机制，能够筛选各学科中质量难以形式化验证的日益增长的科学产出。这种方法为科学评估自动化开辟了新途径。
...
TL;DR: CritiSense是一款多语言、模块化的移动媒体素养应用，通过互动挑战和即时反馈帮助用户识别网络操纵策略，提升对虚假信息的抵御能力。
...
Method: 开发了支持9种语言的模块化移动应用，通过短小互动的挑战练习和即时反馈来培养用户的媒体素养技能，平台设计支持快速更新不同主题和领域的内容。

Result: 可用性研究显示83.9%用户总体满意，90.1%认为应用易于使用；定性反馈表明应用有助于提升数字素养技能；3个多月内获得300+活跃用户。
...
TL;DR: IQuest-Coder-V1系列代码大语言模型采用代码流多阶段训练范式，通过预训练、中期训练和后期训练三阶段，在软件工程、竞赛编程和复杂工具使用等方面达到SOTA性能，并提供循环变体优化部署效率。


<details>
  <summary>Details</summary>
Motivation: 超越静态代码表示，捕捉软件逻辑的动态演变过程，通过多阶段训练范式提升代码智能模型在真实世界代理系统中的能力。

Method: 提出代码流多阶段训练范式：1) 初始预训练（代码事实、仓库、补全数据）；2) 中期训练（32k上下文推理和代理轨迹，128k上下文仓库规模）；3) 后期训练分化为思维路径（推理驱动强化学习）和指令路径（通用助手优化）。

Result: 在代理软件工程、竞赛编程和复杂工具使用等关键维度上达到最先进性能，IQuest-Coder-V1-Loop变体通过循环机制优化模型容量与部署占用空间的权衡。

Conclusion: IQuest-Coder-V1系列模型的发布，包括从预训练基础到最终思维和指令模型的完整白盒检查点链，将推动自主代码智能和真实世界代理系统的研究进展。
...
TL;DR: 研究发现个性化（尤其是心理健康披露）能轻微降低AI代理的有害任务完成率，但效果有限且易被越狱攻击破坏，揭示了安全性与实用性之间的权衡。


<details>
  <summary>Details</summary>
Motivation: 随着LLM越来越多地作为工具使用代理部署，安全问题从有害文本生成转向有害任务完成。现有安全评估通常忽略个性化信号，而实际系统常基于用户档案或持久记忆运行，因此需要研究敏感用户上下文（如心理健康披露）如何影响代理的有害行为。

Method: 基于AgentHarm基准，在受控提示条件下评估前沿和开源LLM在多步恶意任务（及其良性对应任务）上的表现。变量包括：用户上下文个性化（无简介、仅简介、简介+心理健康披露）以及轻量级越狱注入。

Result: 有害任务完成在所有模型中均非微不足道：前沿实验室模型（GPT 5.2、Claude Sonnet 4.5、Gemini 3-Pro）仍完成可观比例的有害任务，而开源模型（DeepSeek 3.2）有害完成率显著更高。添加简介通常降低危害分数并增加拒绝率；添加明确心理健康披露进一步强化此趋势，但效果有限且经多重检验校正后不总是可靠。拒绝率增加也出现在良性任务上，表明安全性与实用性存在权衡。越狱提示显著提升危害，并能削弱或覆盖个性化带来的保护性转变。

Conclusion: 个性化可作为代理滥用场景中的弱保护因素，但在最小对抗压力下很脆弱。这凸显了需要开发对个性化敏感且在不同用户上下文条件下保持鲁棒的安全评估和保障措施。
...
TL;DR: MedCL-Bench：首个用于评估医学语言模型持续学习的统一基准，包含10个生物医学NLP数据集、5类任务，评估11种持续学习策略，发现顺序微调会导致灾难性遗忘，参数隔离方法在GPU小时成本下提供最佳保留效果。


<details>
  <summary>Details</summary>
Motivation: 医学语言模型需要随着证据和术语的发展而更新，但顺序更新会引发灾难性遗忘。虽然生物医学NLP有许多静态基准，但缺乏一个统一的、任务多样化的基准来评估持续学习，特别是在标准化协议、任务顺序鲁棒性和计算感知报告方面。

Method: 引入MedCL-Bench基准，流式处理10个生物医学NLP数据集，涵盖5个任务家族，评估11种持续学习策略在8种任务顺序下的表现，报告保留率、迁移能力和GPU小时成本。

Result: 直接顺序微调会导致灾难性遗忘，造成先前任务性能下降。不同持续学习方法占据不同的保留-计算边界：参数隔离方法在每GPU小时成本下提供最佳保留效果，重放方法以更高成本提供强保护，正则化方法收益有限。遗忘程度与任务相关，多标签主题分类最脆弱，约束输出任务更稳健。

Conclusion: MedCL-Bench为在部署前审计模型更新提供了一个可复现的框架，有助于评估医学语言模型的持续学习性能，防止更新引发的性能退化。
...
TL;DR: 研究AI编码代理在相同数据和问题下是否产生一致结果，发现存在显著的"非标准误差"，不同AI模型有稳定"经验风格"，AI同行评审效果有限，但接触优秀范例可大幅减少估计差异


<details>
  <summary>Details</summary>
Motivation: 随着AI在自动化政策评估和实证研究中的应用日益增长，需要了解AI代理在相同数据和问题下是否会产生一致的结果，以及AI同行评审和范例学习对结果一致性的影响

Method: 部署150个自主Claude Code代理独立测试6个关于SPY市场质量趋势的假设，使用NYSE TAQ数据（2015-2024），比较不同模型家族（Sonnet 4.6 vs Opus 4.6），并设计三阶段反馈协议评估AI同行评审和范例学习的效果

Result: AI代理表现出显著的"非标准误差"，不同模型家族有稳定的"经验风格"，AI同行评审对减少分散性效果有限，但接触顶级范例论文可将估计值的四分位距减少80-99%，收敛主要源于模仿而非理解

Conclusion: AI代理在实证研究中存在类似人类研究者的非标准误差问题，AI同行评审效果有限，但范例学习能显著提高结果一致性，这对AI在自动化政策评估和实证研究中的应用具有重要意义
...
TL;DR: TraceR1是一个两阶段强化学习框架，通过预测短期轨迹来训练前瞻性推理，提升多模态代理的规划连贯性和执行鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有的大多数多模态代理系统是反应式的，仅优化孤立动作而不考虑未来状态或长期目标，这限制了规划连贯性，无法可靠解决高层次、多步骤任务。
...
TL;DR: 开发了一个集成了传统预测模型、机器学习回归器和深度序列模型的数字化预测-库存优化管道，在M5沃尔玛数据集上评估了7种预测方法，发现Temporal CNN和LSTM模型相比统计基线显著降低了库存成本并提高了填充率。


<details>
  <summary>Details</summary>
Motivation: 为现代供应链提供数据驱动的决策支持工具，通过集成多种预测方法并评估其在库存系统中的运营影响，解决传统预测与库存优化分离的问题。
...
Conclusion: 该研究提供了一个有效的数字化预测-库存优化框架，深度序列模型在供应链管理中表现出优越性能，为现代供应链决策提供了数据驱动的支持工具。
...
TL;DR: 本文系统分析了RAG系统中基于保形事实性过滤方法的可靠性与实用性，揭示了在分布偏移和干扰项下的脆弱性，并提出了新的信息感知度量指标。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型经常产生幻觉，限制了其在知识密集型应用中的可靠性。虽然检索增强生成和保形事实性过滤是潜在的解决方案，但前者缺乏统计保证，后者无法保证输出信息量，需要系统分析其可靠性和实用性。

Method: 提出新颖的信息感知度量指标，系统分析保形事实性过滤在RAG系统中的表现，涵盖生成、评分、校准、鲁棒性和效率五个维度，在三个基准测试和多个模型家族上进行评估。

Result: 发现：(1) 在高事实性水平下保形过滤因输出空洞而实用性低；(2) 保形事实性保证对分布偏移和干扰项不鲁棒；(3) 轻量级蕴含验证器在性能相当或更优的同时计算效率高100倍以上。

Conclusion: 揭示了事实性与信息量之间的权衡，以及保形过滤框架在分布偏移下的脆弱性，强调需要将鲁棒性和实用性作为关键指标的新方法，并为构建可靠且计算高效的RAG管道提供实用指导。
...
TL;DR: SurgΣ是一个大规模多模态手术智能框架，包含SurgΣ-DB数据基础和基于此构建的基础模型，旨在解决手术AI泛化能力不足的问题。


<details>
  <summary>Details</summary>
Motivation: 现有手术AI框架大多任务特定，难以跨手术和机构泛化。虽然多模态基础模型在医疗领域展现出跨任务能力，但手术领域缺乏大规模、系统化的多模态数据限制了其发展。

Method: 提出SurgΣ框架，核心是SurgΣ-DB大规模多模态数据基础，整合异构手术数据源（开源数据集、临床收集数据、网络数据）到统一模式，提供图像和视频级标注，涵盖6个临床专业和18个手术任务，包含超过598万对话和分层推理标注。

Result: SurgΣ-DB提供了前所未有的规模（超过598万对话），涵盖理解、推理、规划和生成等任务。基于该数据构建的手术基础模型展示了大规模多模态标注、统一语义设计和结构化推理标注对改善跨任务泛化和可解释性的实际益处。

Conclusion: SurgΣ通过构建大规模多模态数据基础和相应基础模型，为解决手术AI泛化能力不足的问题提供了有效方案，为手术智能的进一步发展奠定了基础。
...
TL;DR: 验证并扩展了文化对齐框架，在开源LLMs上复现了社会科学调查的投影和距离度量，引入DSPy提示编程来系统优化文化对齐，相比手动提示工程取得了更好效果。


<details>
  <summary>Details</summary>
Motivation: LLMs在战略决策、政策支持和文档工程等任务中日益重要，但存在文化偏见问题，与目标人群价值观不匹配。需要改进文化对齐，确保下游分析和建议反映目标人群的价值取向而非模型默认先验。

Method: 1) 在开源LLMs上复现先前框架的社会科学调查投影和距离度量；2) 引入DSPy提示编程，将提示视为模块化、可优化的程序；3) 通过优化文化距离目标来系统调整文化条件。

Result: 实验表明：1) 开源LLMs同样存在文化偏差，文化条件化的好处在封闭系统外仍然存在；2) 提示优化通常优于手动文化提示工程；3) DSPy提示编译为文化对齐的LLM响应提供了更稳定、可转移的途径。

Conclusion: DSPy提示编程能够系统优化文化对齐，相比手动提示工程更有效，为LLMs的文化对齐提供了更稳定和可转移的解决方案，特别是在开源模型中验证了文化对齐框架的有效性。
...
<details>
  <summary>Details</summary>
Motivation: 自动演示生成是一个具有挑战性的任务，需要连贯的内容创作、视觉设计和面向受众的沟通。现有方法在内容质量、视觉美观和规范遵循方面存在不足，需要更系统的方法来训练LLM代理完成这一复杂任务。

Method: 1. 创建OpenEnv兼容的强化学习环境，LLM代理学习使用工具研究主题、规划内容和生成HTML幻灯片；2. 设计多组件奖励系统：结构验证、渲染质量评估、LLM美学评分、内容质量指标和逆向规范奖励；3. 逆向规范奖励通过LLM从生成的幻灯片中恢复原始规范来提供整体质量信号；4. 使用GRPO方法微调Qwen2.5-Coder-7B，仅训练0.5%参数，提示基于Claude Opus 4.6收集的专家演示。

Result: 1. 在48个多样化商业简报上测试六个模型，微调的7B模型达到Claude Opus 4.6质量的91.2%，比基础模型提升33.1%；2. 六模型比较显示指令遵循和工具使用合规性（而非原始参数数量）决定代理任务性能；3. 贡献SlideRL开源数据集，包含288个多轮展开轨迹。

Conclusion: 通过强化学习环境训练LLM代理生成专业演示是有效的，逆向规范奖励提供了有意义的整体质量评估。模型性能主要取决于指令遵循和工具使用能力，而非模型大小。该方法为复杂代理任务训练提供了新思路。
...
TL;DR: LEAFE框架通过从反思经验中学习反馈驱动的自主性，提升大语言模型在长时程交互任务中的问题解决能力，相比传统结果驱动方法显著提高了Pass@k性能。


<details>
  <summary>Details</summary>
Motivation: 当前基于结果的后训练方法（如带可验证奖励的强化学习）主要优化最终成功信号，未能充分利用丰富的环境反馈，导致分布锐化问题：模型仅擅长再现已有的成功行为，而无法提升基于反馈的自主性来扩展长时程设置中的问题解决能力。

Method: 提出LEAFE框架：1）在探索阶段，智能体将环境反馈总结为可操作的经验，回溯到早期决策点，探索带有修正动作的替代分支；2）通过监督微调将这些经验引导的修正蒸馏到模型中，使策略在未来交互中更有效地恢复。

Result: 在固定交互预算下的多样化交互式编码和智能体任务中，LEAFE相比基础模型持续提升Pass@1，并在Pass@k上优于结果驱动基线（GRPO）和经验方法（Early Experience），在Pass@128上获得高达14%的性能提升。

Conclusion: LEAFE通过从反思经验中内化恢复自主性，有效解决了结果驱动方法的分布锐化问题，显著提升了语言模型在长时程交互任务中的反馈驱动自主性和问题解决能力。
...
TL;DR: SocialOmni是一个评估全模态大语言模型社交交互能力的新基准，专注于对话中的打断行为评估，包含感知样本和交互生成实例，揭示了模型感知准确性与交互能力之间的脱节。


<details>
  <summary>Details</summary>
Motivation: 现有全模态大语言模型基准主要关注静态的准确性任务，缺乏对社交交互性（特别是自然对话中动态线索处理能力）的评估，这限制了模型在真实社交场景中的应用。

Method: 提出SocialOmni基准，从三个核心维度评估对话交互性：1)说话人分离与识别（谁在说话）；2)打断时机控制（何时打断）；3)自然打断生成（如何表达打断）。包含2000个感知样本和209个质量控制的交互生成实例，并设计了音频-视觉不一致场景测试模型鲁棒性。

Result: 对12个领先的全模态大语言模型进行基准测试，发现它们在社交交互能力上存在显著差异。分析显示模型的感知准确性与生成上下文适当打断的能力之间存在明显脱节，表明仅基于理解的指标不足以表征对话社交能力。

Conclusion: SocialOmni基准填补了全模态大语言模型在社交交互评估方面的空白，揭示了感知与交互能力之间的差距，并为未来模型改进提供了可操作的信号，有助于弥合这一鸿沟。
