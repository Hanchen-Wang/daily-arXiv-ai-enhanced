...
Motivation: 传统基于预测熵的固定阈值方法在不确定性感知决策中存在置信错误率高和缺乏可解释性的问题，需要一种更可靠、透明且可审计的决策机制。
...
<details>
  <summary>Details</summary>
Motivation: 高速公路交通拥堵严重降低出行效率，现有"检测-预测"系统存在遮挡条件下车辆感知精度低和长序列依赖丢失等关键缺陷，需要开发更有效的技术框架来解决这些问题。

Method: 优化YOLOv11为YOLOv11-DIoU（使用DIoU Loss替代GIoU Loss），改进DeepSort融合马氏距离和余弦距离，使用Greenberg模型分析交通流，构建GRU-Attention模型捕捉拥堵前兆特征。

Result: YOLOv11-DIoU达到95.7% mAP（比基线高6.5%），遮挡漏检率5.3%；DeepSort达到93.8% MOTA（比SORT高11.3%）；GRU-Attention模型测试准确率99.7%，10分钟提前预警时间误差≤1分钟，验证视频显示95%预警准确率。
...
<details>
  <summary>Details</summary>
Motivation: 优化现有路边停车服务质量，通过自动化地面实况测试流程来替代人工工程工作

Method: 应用机器学习特别是图像模式识别方法，使用卷积神经网络实现分析过程的高度自动化

Result: 实现了99.58%的人工资源时间减少，达到了较高的自动化水平

Conclusion: 分析自动化工具显著提升了效率，为未来发展和潜在应用提供了良好基础
...
<details>
  <summary>Details</summary>
Motivation: 尽管CLIP等视觉语言模型在零样本分布外检测方面表现出色，但研究社区对其工作机制、相对于单模态方法的优势以及行为鲁棒性缺乏系统理解，需要深入分析以指导更可靠的AI系统设计。

Method: 采用系统化的实证分析方法，使用分布内和分布外提示词，从三个维度进行研究：(1)机制分析：形式化VLM嵌入空间的关键操作特性；(2)优势比较：实证量化VLM相对于单模态方法的优越性；(3)敏感性测试：评估模型对图像噪声和提示词措辞的鲁棒性。

Result: 研究发现VLM能够有效利用丰富的语义新颖性，在零样本分布外检测方面显著优于单模态方法；同时揭示了重要的不对称鲁棒性特征：对常见图像噪声具有韧性，但对提示词措辞高度敏感。

Conclusion: 研究提供了对VLM基于分布外检测优势和关键脆弱性的结构化理解，为开发更鲁棒可靠的未来设计提供了重要的实证指导，强调了提示工程在VLM应用中的关键作用。
...
Method: 利用新发展的截面曲率抽象概念，构建离散度量空间的曲率几何剖面，捕获点三元组与其他点之间的度量关系

Result: 实验证明该曲率分析方法可用于估计数据集的本征维度，并有效评估降维技术的效果

Conclusion: 曲率剖面为数据表示质量提供了定量度量工具，在探索网络几何特性和评估降维方法方面具有重要应用价值
...
<details>
  <summary>Details</summary>
Motivation: 斐济作为发展中国家正经历快速城市化，需要通过先进技术监测大规模开发项目（住房、道路、市政工程）对土地利用的影响，为城市规划提供数据支持。

Method: 使用Landsat-8卫星影像，结合Google Earth Engine平台，采用k-means聚类无监督学习和卷积神经网络有监督学习方法进行土地覆盖分类和变化检测。

Result: 生成了土地覆盖地图，并通过可视化方式展示了城市区域随时间的变化情况，成功监测了纳迪地区的土地利用变化。

Conclusion: 该研究证明了机器学习和遥感技术在监测发展中国家城市化进程中的有效性，为土地覆盖建模和变化检测提供了可靠的技术框架。
...
Method: 三阶段框架：1) YOLOv7分割模型进行快速目标定位；2) ConvNeXt特征提取器配合三元组损失生成判别性嵌入；3) 特征辅助IoU跟踪器处理遮挡和运动下的多目标跟踪。采用混合精度推理优化边缘部署。

Result: 在真实监控和无人机视频数据集上的大量实验表明，该框架在各种异物入侵场景下具有高精度和鲁棒性。NVIDIA Jetson设备上的硬件基准测试证实了其在实际边缘应用中的实用性和可扩展性。

Conclusion: 该框架为电力传输系统提供了一种高效、实时的异物入侵检测与跟踪解决方案，具备边缘部署能力和增量学习特性，适合实际应用场景。
...
<details>
  <summary>Details</summary>
Motivation: 当前图像编辑评估方法存在局限性：要么依赖配对的参考图像导致覆盖范围有限，要么仅使用零样本视觉语言模型进行评估，其基于提示的评估往往不精确。需要一种更可靠、可解释的自动化评估框架。

Method: 提出EdiVal-Agent框架：首先将图像分解为语义对象，然后合成多样化的上下文感知编辑指令。评估时结合VLM和开放词汇对象检测器评估指令遵循，使用语义级特征提取器评估内容一致性，利用人类偏好模型判断视觉质量。

Result: 实验表明，将VLM与对象检测器结合使用在指令遵循评估中比单独使用VLM和CLIP指标与人类判断的一致性更强。构建的EdiVal-Bench基准覆盖9种指令类型和11种最先进的编辑模型。

Conclusion: EdiVal-Agent能够识别现有编辑模型的失败模式，为下一代编辑模型的开发提供信息。其模块化设计允许未来工具的集成，随着时间的推移提高评估准确性。
...
<details>
  <summary>Details</summary>
Motivation: 为了解决3D视觉中不同任务需要专门模型的问题，研究者希望开发一个统一的模型来处理多种3D重建任务，提高效率和泛化能力。

Method: 采用基于transformer的前馈架构，输入图像和可选几何信息，通过分解的多视角场景几何表示（深度图、局部射线图、相机位姿和度量尺度因子）来回归3D几何和相机参数。

Result: 实验表明MapAnything在多个3D视觉任务上优于或匹配专门的模型，同时提供更高效的联合训练性能。

Conclusion: MapAnything为通用3D重建骨干网络开辟了道路，展示了统一模型在多样化3D视觉任务中的潜力。
...
<details>
  <summary>Details</summary>
Motivation: 解决GPS不可用环境下机器人定位问题，现有RGB方法对光照、天气和季节变化敏感，而现有跨模态方法在复杂场景和视角变化下表现不佳。

Method: 使用VMamba骨干网络提取RGB特征；语义感知特征融合模块结合地点描述符和分割掩码；LiDAR描述符融合语义和几何信息；在NetVLAD中引入跨模态语义注意力机制；设计多视角语义-几何匹配和语义一致性损失。

Result: 在KITTI和KITTI-360数据集上实现了最先进的性能，优于其他跨模态地点识别方法。

Conclusion: 通过融合语义信息，SCM-PR框架显著提升了跨模态地点识别的鲁棒性和准确性，特别是在复杂环境和视角变化条件下。
...
<details>
  <summary>Details</summary>
Motivation: 3D高斯泼溅技术虽然渲染质量高且实时性好，但数据量巨大需要压缩。现有方法依赖简单的均匀标量量化，需要探索更先进的量化器来提升压缩性能。

Method: 使用格点向量量化(LVQ)替代均匀标量量化，并为每个场景优化格点基向量，实现场景自适应。通过缩放格点基向量可动态调整量化密度，支持多码率目标。

Result: SALVQ方法在保持低复杂度的同时显著提升了率失真性能，可无缝集成到现有3DGS压缩架构中，仅需最小修改和计算开销。

Conclusion: 场景自适应格点向量量化为3D高斯泼溅压缩提供了高效的解决方案，平衡了向量量化的率失真效率和标量量化的低复杂度，支持灵活的码率调整。
...
Method: 三阶段模块化流程：1)使用现成的人体检测和深度估计 2)VLM基础推理分类成对社交关系 3)轻量级空间聚合算法定位社交连接群体

Result: 开发了包含10万张街景图像的新数据集，结合人工标注和MINGLE输出，确保语义丰富性和现实场景覆盖

Conclusion: MINGLE方法有效解决了社交群体区域检测任务，为城市社交互动研究提供了重要工具和数据集支持
...
<details>
  <summary>Details</summary>
Motivation: 现有工作主要关注输出层面的人口统计分布，无法保证偏见缓解后概念表示的解耦。需要更深入地揭示图像生成过程中的表征偏见，特别是人口统计特征与语义概念之间的结构性纠缠。

Method: 利用交叉注意力归因图量化人口统计-语义概念的空间纠缠（通过IoU指标），并采用能量引导扩散采样方法直接在潜在噪声空间中修改，在去噪过程中最小化期望SoftIoU。
...
<details>
  <summary>Details</summary>
Motivation: 现有图像标注工具大多需要用户先上传预收集的数据集，这限制了按需流程的支持，在实验室环境中尤其成问题，因为实时从显微镜等仪器获取数据越来越普遍。

Method: 开发了一个基于Python的图形用户界面，与网络摄像头、显微镜等成像系统集成，提供类似商业图形编辑软件的标注工具，包括贝塞尔样条和二进制掩码，使用非破坏性图层实现高性能编辑，并通过OpenCV和Numpy优化对象检测操作。

Result: LivePixel实现了与各种视频设备的广泛兼容性，能够无缝进行数据收集和标注，为实验工作流程中的AI模型开发提供了加速。

Conclusion: LivePixel解决了科学领域中缺乏灵活标注工具的问题，通过实时图像标注功能促进了AI模型在实验室环境中的部署和应用。
...
<details>
  <summary>Details</summary>
Motivation: 现实世界虚拟试穿应用需要有限的训练和推理预算，而现有方法需要大量端到端训练，计算成本高昂。

Method: 使用DEFT冻结预训练模型参数，训练小型h-transform网络学习条件变换；提出自适应一致性损失，将一致性损失和去噪分数匹配损失以数据自适应方式结合。

Result: 在虚拟试穿任务上达到最先进性能，仅需15个去噪步骤，同时保持竞争性结果。

Conclusion: DEFT-VTON方法在保持高性能的同时显著降低了训练和推理成本，为实际部署提供了高效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 自动驾驶领域需要合成数据来覆盖特定交通场景，但合成数据与真实数据之间存在领域差距，需要改进行人识别效果

Method: 开发了数据增强管道，在Cityscapes数据集中添加虚拟行人，并设计了新颖的生成对抗网络架构来学习数据集的光照条件以提高增强的真实性

Result: 在语义分割和实例分割任务上评估了该方法

Conclusion: 该方法能够有效生成更真实的合成数据，改善自动驾驶系统中的行人识别能力
...
Method: 基于函数空间推广Kolmogorov-Arnold表示定理，使用傅里叶分解和Hermite函数基学习内部函数，提出U-FunKAN用于医学图像分割

Result: 在IXI数据集上有效抑制MRI吉布斯伪影，在BUSI、GlaS、CVC-ClinicDB三个医学数据集上实现最先进的二值分割性能，各项指标均优于其他KAN基础模型
...
<details>
  <summary>Details</summary>
Motivation: 现有多模态分类方法通常忽视仇恨内容的关键性，对所有内容一视同仁，且无法系统捕捉视频中的结构化信息，限制了多模态融合的效果。

Method: 构建实例图将视频分割为多个实例提取特征，通过互补权重图为这些特征分配重要性权重以突出仇恨实例，结合权重和特征生成视频标签，使用图框架系统建模模态内和跨模态的结构关系。

Result: 在公开数据集上的大量实验表明，该模型在仇恨视频分类方面达到最先进水平，并具有很强的可解释性。

Conclusion: 提出的多模态双流图神经网络模型有效解决了现有方法的局限性，在仇恨视频检测任务上表现出优异性能。
...
Method: 提出ColonCrafter扩散模型，从合成结肠镜序列学习稳健的几何先验来生成时间一致的深度图。引入风格迁移技术，在将真实临床视频适配到合成训练域的同时保持几何结构。

Result: 在C3VD数据集上实现了最先进的零样本性能，优于通用和特定于内窥镜的方法。虽然完整的轨迹3D重建仍然具有挑战性，但展示了包括3D点云生成和表面覆盖评估在内的临床相关应用。

Conclusion: ColonCrafter通过扩散模型和风格迁移技术成功解决了结肠镜深度估计的时间一致性问题，为临床3D应用提供了有效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 当前3DGS研究主要关注高性能桌面GPU，忽视了嵌入式平台的计算资源限制。微型飞行器等设备在系统性能和重建质量之间存在权衡，需要优化内存使用同时提升渲染质量。

Method: 1. 在体素空间中基于几何相似性合并SLAM中的冗余3D高斯基元，减少GPU内存使用；2. 通过Patch-Grid点采样初始化3D高斯基元，实现更精确的场景建模。

Result: 在公开数据集上的定量和定性评估表明，该方法在减少GPU内存使用的同时提升了渲染质量，且不影响系统运行时性能。

Conclusion: 提出的方法有效解决了嵌入式平台在3D高斯泼溅技术中面临的内存限制问题，在保持性能的同时提升了重建和渲染质量，为资源受限设备的3D重建应用提供了可行方案。
...
<details>
  <summary>Details</summary>
Motivation: 亚马逊雨林作为重要生态系统，森林砍伐对全球碳排放和生物多样性有重大影响，需要有效的监测方法来研究其影响。

Method: 使用深度学习技术比较同一区域不同时间的卫星图像对，识别森林覆盖变化，并构建视觉语义模型从科学文献中提取关键词自动标注检测到的变化。

Result: 在亚马逊图像对数据集上评估表明，该方法能有效检测森林砍伐并生成相关标注。
...
Method: 采用Vision-Language Models (VLMs)，结合视觉特征提取和自然语言处理，集成坐标验证机制和概率高斯建模进行异常分布分析，使用多层可视化技术生成医学图示和统计表示。

Result: 在多模态异常检测中表现优异，位置测量平均偏差为80像素，具备零样本学习能力，可通过用户友好的Gradio界面集成到临床工作流中。

Conclusion: 该框架在自动化诊断支持和放射工作流效率方面取得显著进展，但需要进行临床验证和多中心评估才能广泛采用。
...
<details>
  <summary>Details</summary>
Motivation: 扩展CLAP算法到更通用的框架，超越原有的2D定位应用，使其能够适用于3D定位和图像拼接等多种领域，处理噪声和不确定性

Method: 采用聚类方法来抑制噪声和错误特征匹配，通过聚类策略替代传统的基于重投影误差的异常值拒绝方案（如RANSAC）

Result: 成功将CLAP算法推广到3D定位和图像拼接应用，建立了CLAP、RANSAC和霍夫变换之间的关系，证明了该方法的广泛适用性

Conclusion: CLAP的通用化框架可广泛应用于多个不同领域，是处理噪声和不确定性的有效工具，为各种定位和匹配问题提供了新的解决方案
...
<details>
  <summary>Details</summary>
Motivation: 医学图像配准中变形与组织形态特征密切相关，但现有的弱监督方法需要分割掩码或地标等解剖先验信息，而这些弱标签往往难以获得，限制了实际应用。受视觉基础模型强大表示学习能力的启发，利用SAM来增强特征提取。

Method: 设计了一个任务特定的适应管道，使用SAM的图像编码器提取结构感知特征嵌入；设计轻量级3D头部来细化嵌入空间中的特征，适应医学图像的局部变形；引入分层特征一致性损失来指导从粗到细的特征匹配。

Result: 在基准数据集上的广泛实验表明，SAMIR在心脏图像配准和腹部CT图像配准任务上显著优于最先进方法，在ACDC数据集上性能提升2.68%，在腹部数据集上提升6.44%。

Conclusion: SAMIR框架通过利用SAM的强大表示学习能力，有效解决了医学图像配准中特征提取的挑战，在不依赖额外弱标签的情况下实现了优异的配准性能，具有重要的实际应用价值。
...
<details>
  <summary>Details</summary>
Motivation: 传统集中式训练方法需要合并数据，会危及客户端数据安全。卫星图像处理需要分布式方法在保护隐私的同时实现准确的森林砍伐识别。

Method: 使用联邦学习框架，结合FLOWER和RAY框架进行分布式训练。采用YOLOS-small、Faster R-CNN with ResNet50和Faster R-CNN with MobileNetV3三种模型在公开数据集上进行训练和测试。

Result: 该方法能够在保护各边缘卫星中心数据隐私的前提下，实现协作式的森林砍伐检测模型训练。

Conclusion: 联邦学习为卫星图像分割任务提供了新的分布式解决方案，在确保数据安全的同时实现了有效的森林砍伐检测。
...
<details>
  <summary>Details</summary>
Motivation: 传统双视图位姿估计方法存在尺度模糊问题，且在宽基线和纹理缺失区域表现不佳，需要一种能够实现度量级精确估计的鲁棒方法。

Method: 使用度量单目深度估计器和高斯场景重建器为每张图像构建度量3D高斯混合模型，然后通过优化可微分GMM对齐目标来精化初始位姿估计，该目标综合考虑几何结构、颜色、协方差和语义特征一致性。

Result: 在Real-Estate10K数据集上的大量实验表明，GARPS超越了传统方法和最先进的学习方法，包括MASt3R。

Conclusion: 该研究展示了将单视图感知与多视图几何相结合来实现鲁棒度量相对位姿估计的潜力。
...
<details>
  <summary>Details</summary>
Motivation: 卷积神经网络中的乘法操作计算复杂度高、能耗大，阻碍了在移动设备上的部署。受资源受限设备使用查找表简化复杂计算的启发，作者希望用高效的查找操作替代传统乘法操作。

Method: 引入通用的查找表操作作为神经网络的基本构建块，采用可微分方式构建查找表以实现端到端优化，并提出多种训练策略促进收敛。用查找操作替代昂贵的乘法操作来构建查找网络。

Result: 查找网络在图像分类、图像超分辨率和点云分类任务中表现出色，在能耗和推理速度方面效率更高，同时保持与普通卷积网络竞争的性能。在不同任务和数据类型上都达到了最先进的性能。

Conclusion: 查找表操作可以有效替代传统乘法操作，显著提升神经网络在移动设备上的部署效率，为资源受限环境下的深度学习应用提供了可行的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 传统的基于补丁的视觉投影器在减少视觉token数量和保持语义清晰度之间难以平衡，通常需要保留过长的token序列来避免性能下降，计算成本高昂。

Method: 利用SAM生成语义超像素作为视觉词汇，通过压缩和投影语义超像素作为视觉token；提出语义超像素位置嵌入来增强MLLM对超像素几何和位置的感知；使用语义超像素聚合器保留超像素内部细节和全局上下文。
...
<details>
  <summary>Details</summary>
Motivation: 现有BEV分割方法主要针对针孔相机，难以直接应用于存在严重几何畸变、多视角对应模糊和时间动态不稳定的鱼眼相机，这些因素显著降低了BEV性能。
...
Conclusion: FishBEV通过专门针对鱼眼相机特性的三个互补创新模块，有效解决了鱼眼相机BEV分割中的关键挑战，为自动驾驶环视感知提供了有效的解决方案。
...
TL;DR: 该研究提出了基于样条的Kolmogorov-Arnold网络（KANs）用于医学图像分类，包括SBTAYLOR-KAN、SBRBF-KAN和SBWAVELET-KAN三种变体。这些模型在有限数据集上表现出色，SBTAYLOR-KAN仅用2872个参数就达到98.93%的准确率，远少于传统CNN的百万级参数，同时通过Grad-CAM提供可解释性。
...
Method: 开发了三种基于样条的KAN模型：SBTAYLOR-KAN（B样条+泰勒级数）、SBRBF-KAN（B样条+径向基函数）和SBWAVELET-KAN（B样条+Morlet小波变换），利用样条基函数逼近来捕获局部和全局非线性。

Result: SBTAYLOR-KAN在多个医学图像数据集上达到98.93%的最高准确率，仅使用30%训练数据仍保持86%以上准确率，在皮肤癌数据集上达到68.22%准确率，且仅需2872个可训练参数。

Conclusion: 该框架为医学图像分类提供了轻量级、可解释且可泛化的解决方案，特别适合数据稀缺的临床AI应用场景，解决了有限数据集和计算资源受限的挑战。
...
<details>
  <summary>Details</summary>
Motivation: 生成模型特别是扩散模型的快速发展使得恶意使用者能够廉价复制艺术家的独特风格，这导致了对艺术作品风格模仿保护方法的需求增加。微调扩散模型能够以更高保真度和控制力内化和复制艺术风格。

Method: 研究发现某些交叉注意力层对艺术风格表现出高度敏感性。基于此提出了StyleProtect保护策略，通过仅更新选定的交叉注意力层来实现有效的风格防御。实验使用基于WikiArt精心策划的艺术作品数据集和Anita卡通动画数据集。

Result: 所提出的方法在保护艺术作品和动漫的独特风格免受恶意扩散定制方面表现出有希望的性能，同时保持了竞争力的不可感知性。

Conclusion: StyleProtect是一种高效轻量级的保护策略，能够有效防御微调扩散模型的风格模仿，为艺术作品的风格保护提供了可行的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 解决自监督单目深度估计在低纹理区域和动态区域因输入数据不确定性导致的深度精度下降问题。

Method: 采用教师-学生训练策略，将不确定性估计嵌入训练流程和网络架构，仅在教师网络训练时使用光流，避免推理时额外开销。

Result: 在KITTI和Cityscapes数据集上取得最先进的性能，在自监督深度和姿态估计方面达到state-of-the-art结果。

Conclusion: UM-Depth通过不确定性感知细化有效提升了自监督深度估计的准确性，特别是在具有挑战性的场景区域，且不增加推理成本。
...
<details>
  <summary>Details</summary>
Motivation: 现有的基于查询的RVOS方法使用静态文本查询进行跨模态对齐，但容易被外观或运动相似的干扰物误导，导致查询选择偏差。

Method: 将引用查询分解为三个专门组件：外观查询（静态属性）、帧内交互查询（空间关系）和帧间运动查询（时间关联）。查询通过整合语言线索和视觉指导动态构建，并引入了两个运动感知聚合模块来增强对象标记表示。

Result: 在多个RVOS基准测试上的广泛实验证明了TQF的优势以及结构化查询设计和运动感知聚合模块的有效性。

Conclusion: TQF通过三重查询分解和运动感知聚合机制，有效解决了RVOS中的查询选择偏差问题，提升了视频对象分割的性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法通常将广义引用表达式理解(GREC)和分割(GRES)任务独立处理，忽视了联合训练的优势，且GRES方法缺乏实例感知能力和实例级框掩码一致性预测。

Method: 提出InstanceVG框架，为每个实例查询分配先验参考点，统一实例级边界框和掩码的联合一致性预测，确保同一实例的点、框、掩码预测一致性。

Result: 在10个数据集4个任务上的大量实验表明，InstanceVG在各项评估指标上显著超越现有方法，达到最先进性能。

Conclusion: InstanceVG是首个同时处理GREC和GRES任务并融入实例感知能力的通用视觉定位框架，通过实例查询机制实现了多粒度预测的一致性，取得了优异性能。
...
Result: 在三个公共数据集上实现了所有全局匹配方法中的最先进性能。
...
<details>
  <summary>Details</summary>
Motivation: 解决文本驱动图像编辑中颜色控制精度不足和难以实现连续控制的问题，现有方法缺乏对颜色变化范围的控制且插值系数与图像颜色的关系未知不可控

Method: 引入颜色映射模块，显式建模文本嵌入空间与图像RGB值的对应关系，基于给定RGB值预测对应的嵌入向量，在保持语义一致性的同时实现精确颜色控制
...
<details>
  <summary>Details</summary>
Motivation: 现有基于LLM的安全方法只关注文本提示而忽略了生成的图像，可能导致不安全输出或对已安全提示进行不必要的修改。

Method: 使用视觉语言模型(VLMs)同时分析输入提示和生成图像，通过视觉反馈进行迭代提示词优化，并构建了包含文本和视觉安全信号的新数据集。

Result: 实验结果表明该方法能产生更安全的输出，同时保持与用户意图的一致性，安全性可与现有LLM方法相媲美。

Conclusion: 该方法为生成更安全的文本到图像内容提供了实用解决方案，通过视觉反馈有效提升了模型的安全性表现。
...
<details>
  <summary>Details</summary>
Motivation: 现有RAW数据处理方法存在两个主要问题：大规模ISP网络计算开销大，基于传统ISP流水线调优的方法表达能力有限。需要一种既能保持高性能又适合资源受限设备的解决方案。

Method: 提出TA-ISP框架，预测一组轻量级多尺度调制算子（全局、区域、像素级别），通过因子化控制重塑不同空间范围的图像统计信息，替代传统密集卷积流水线。
...
<details>
  <summary>Details</summary>
Motivation: 现有图像去雨技术主要针对白天条件，在夜间低光照环境下性能不佳，因为雨纹分布具有空间异质性且条纹可见度受光照影响。

Method: 提出夜间去雨位置增强感知网络(NDLPNet)，包含位置感知模块(PPM)来捕获空间上下文信息，增强特征通道重要性识别和重新校准能力。

Result: 在现有数据集和新建的NSR数据集上的大量实验表明，该方法在夜间去雨任务中优于最先进方法，能有效去除雨纹同时保留关键背景信息。

Conclusion: NDLPNet通过位置感知机制有效解决了夜间低光照条件下的图像去雨问题，新建的NSR数据集为该领域研究提供了新的基准。
...
<details>
  <summary>Details</summary>
Motivation: 现有的发音结构分割方法主要依赖视觉线索，但同步的声学和音韵学信号能提供互补的上下文信息，丰富视觉信息并提高分割精度。

Method: 提出VocSegMRI多模态框架，通过交叉注意力融合实现视频、音频和音韵学输入的动态特征对齐，并加入对比学习目标来增强跨模态表示。
...
### [43] [BWCache: Accelerating Video Diffusion Transformers through Block-Wise Caching](https://arxiv.org/abs/2509.13789)
...
<details>
  <summary>Details</summary>
Motivation: 现有Diffusion Transformers在视频生成中存在不可避免的延迟问题，现有加速方法要么牺牲视觉质量，要么无法在适当粒度上重用中间特征
...
Conclusion: BWCache通过有效利用DiT块特征的时间相似性，为DiT-based视频生成提供了高效的训练免费加速解决方案
...
Conclusion: 该框架有效利用现有预训练扩散模型，能以最小重训练成本适应不同预训练模型的新需求，在低比特率压缩方面表现出色
...
TL;DR: AdaThinkDrive是一个创新的视觉语言动作框架，采用双模式推理机制（快速回答和慢速思考），通过自适应推理在自动驾驶中平衡准确性和效率，在Navsim基准测试中达到90.3 PDMS，比最佳纯视觉基线提升1.7分。


<details>
  <summary>Details</summary>
Motivation: 现有的CoT推理技术在简单场景中表现不佳，引入不必要的计算开销却无法提升决策质量，需要一种能够智能选择何时使用推理的自适应方法。

Method: 提出AdaThinkDrive框架：1）在大规模自动驾驶场景上进行预训练获取世界知识和驾驶常识；2）在监督微调时引入双模式数据集（有/无CoT）；3）结合GRPO提出自适应思考奖励策略，通过比较不同推理模式的轨迹质量来奖励选择性应用CoT。

Result: 在Navsim基准测试中达到90.3 PDMS，超越最佳纯视觉基线1.7分；相比"从不思考"和"总是思考"基线分别提升2.0和1.4 PDMS；推理时间比"总是思考"基线减少14%。

Conclusion: AdaThinkDrive通过自适应推理机制成功平衡了自动驾驶的准确性和效率，证明了选择性应用CoT推理的有效性，为VLA模型在自动驾驶领域的应用提供了新思路。
...
### [46] [Bridging the Synthetic-Real Gap: Supervised Domain Adaptation for Robust Spacecraft 6-DoF Pose Estimation](https://arxiv.org/abs/2509.13792)
...
<details>
  <summary>Details</summary>
Motivation: 航天器姿态估计对于自主空间操作至关重要，但现有混合方法在合成数据上表现良好，在真实图像上性能急剧下降，存在显著的合成到真实域差距问题。

Method: 基于学习不变表示和风险(LIRR)范式，联合使用标记的合成数据和有限的标记真实数据，优化域不变表示和任务特定风险，构建轻量级、主干无关的计算高效框架。

Result: 在SPEED+基准测试中，该方法始终优于仅源域、微调和oracle基线。仅使用5%标记目标数据就能达到或超过使用更大比例标记数据的oracle性能。

Conclusion: 该框架为在真实空间环境中实现稳健且可部署的航天器姿态估计提供了实用途径，具有轻量级、主干无关和计算高效的特点。
...
TL;DR: 本文提出了一种新的深度伪造检测方法，通过独立地从局部和全局角度预测撰改区域，并使用形态学操作融合输出，提高了伪造区域定位的准确性和稳健性。


<details>
  <summary>Details</summary>
Motivation: 虽然深度伪造检测的准确性不断提升，但准确定位撰改区域仍是个重大挑战。现有方法往往忽视了局部细节与全局语义上下文的补充性，且融合策略不优导致噪声放大和错误增多。

Method: 提出了一种新方法：独立地从局部和全局角度预测撰改区域，然后使用形态学操作来融合这两种预测结果。这种方法有效抑制噪声同时增强空间一致性。

Result: 大量实验表明，每个模块都能有效提高伪造区域定位的准确性和稳健性。

Conclusion: 该方法通过合理的局部-全局预测融合策略，有效解决了深度伪造检测中的区域定位问题，为精确定位撰改区域提供了有效的解决方案。
...
Result: 无需中间表示，避免了窗口延迟，实现了线性计算复杂度，能够自适应事件率进行高效处理

Conclusion: 该方法直接处理原始事件流，在保持高精度的同时实现了低延迟和实时效率，为高速视觉任务提供了新解决方案
...
### [49] [Diving into Mitigating Hallucinations from a Vision Perspective for Large Vision-Language Models](https://arxiv.org/abs/2509.13836)
...
<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型中的物体幻觉严重阻碍了实际应用，不同视觉编码器的训练范式差异导致其具有不同的归纳偏置和幻觉特性，需要系统分析。

Method: 引入VHBench-10基准测试（约10,000样本，10个细粒度幻觉类别），提出VisionWeaver上下文感知路由网络，使用全局视觉特征生成路由信号，动态聚合多个专门专家的视觉特征。

Result: 评估确认编码器具有独特的幻觉特性，VisionWeaver有效显著减少幻觉并提升整体模型性能。

Conclusion: 视觉编码器的选择对幻觉性能至关重要，提出的VisionWeaver方法通过多专家特征动态聚合有效解决了物体幻觉问题。
...
### [50] [Dense Video Understanding with Gated Residual Tokenization](https://arxiv.org/abs/2509.14199)
*Haichao Zhang,Wenhao Chai,Shwai He,Ang Li,Yun Fu*

Main category: cs.CV

TL;DR: 提出了DVU框架和GRT方法，通过运动补偿和语义融合实现高帧率视频理解，减少计算开销并保持时间精度


<details>
  <summary>Details</summary>
Motivation: 当前视频大语言模型主要依赖低帧率采样，丢弃了密集时间信息，无法处理需要精确时间对齐的任务（如讲座理解）

Method: Gated Residual Tokenization (GRT) 两阶段框架：1) 运动补偿门控token化跳过静态区域；2) 语义场景内token合并融合静态区域token

Result: 在DIVE基准测试中，GRT优于更大的VLLM基线模型，且性能随帧率增加而提升

Conclusion: 密集时间信息对视频理解至关重要，GRT方法能够实现高效、可扩展的高帧率视频理解
...
Method: 提出语义加权自适应粒子滤波(SWA-PF)方法，整合无人机图像和卫星图像的鲁棒语义特征，包含语义加权机制和优化的粒子滤波架构

Result: 在自建的多高度飞行段数据集上评估，计算效率比特征提取方法提升10倍，全局定位误差低于10米，可在数秒内实现快速4自由度位姿估计

Conclusion: 该方法在GNSS拒止环境中实现了高效准确的无人机定位，使用低分辨率卫星地图即可快速完成位姿估计，具有重要的实际应用价值
...
### [52] [MOCHA: Multi-modal Objects-aware Cross-arcHitecture Alignment](https://arxiv.org/abs/2509.14001)
*Elena Camuffo,Francesco Barbato,Mete Ozay,Simone Milani,Umberto Michieli*

Main category: cs.CV

TL;DR: MOCHA是一种知识蒸馏方法，通过对象级别的多模态语义传输，将大型视觉语言教师模型的知识转移到轻量级视觉目标检测学生模型中，在少样本个性化检测任务中表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有方法主要关注密集或全局对齐，缺乏对象级别的语义传输能力。需要一种无需修改教师模型、推理时无需文本输入的高效知识蒸馏方法。

Method: 使用翻译模块将学生特征映射到联合空间，通过双重目标损失（局部对齐和全局关系一致性）指导学生和翻译器的训练，实现对象级别的多模态语义传输。

Result: 在四个少样本个性化检测基准测试中平均得分提升10.1分，性能与大型多模态模型相当，适合实际部署。

Conclusion: MOCHA通过对象级别的跨架构对齐，成功实现了高效的多模态知识蒸馏，在保持紧凑架构的同时达到优秀性能。
...
TL;DR: 提出Masked Feature Modeling (MFM)作为无监督域自适应语义分割的辅助任务，通过在特征空间进行掩码和重建，与分割任务目标对齐，无需修改推理流程即可提升性能。


<details>
  <summary>Details</summary>
Motivation: 现有对比学习等自监督方法在无监督域自适应语义分割中已取得进展，但掩码建模方法由于架构不兼容和优化目标不一致而未被充分探索。需要一种与分割任务目标一致且兼容标准架构的掩码建模方法。

Method: 提出MFM方法：1) 在特征空间进行特征掩码和重建；2) 引入轻量级Rebuilder辅助模块进行联合训练（推理时丢弃）；3) 利用分割解码器对重建特征进行分类，将辅助目标与像素级预测任务紧密耦合。

Result: 在各种架构和UDA基准测试上的广泛实验表明，MFM能够一致地提升分割性能，提供了简单、高效且可泛化的策略。

Conclusion: MFM是一种有效的无监督域自适应语义分割辅助任务，通过与分割任务目标对齐的特征空间掩码建模，实现了性能提升且不影响推理效率。
...
### [54] [Data-Efficient Spectral Classification of Hyperspectral Data Using MiniROCKET and HDC-MiniROCKET](https://arxiv.org/abs/2509.13809)
...
<details>
  <summary>Details</summary>
Motivation: 虽然1D-Justo-LiuNet是目前光谱分类的最先进模型，但在训练数据有限时性能会下降。需要寻找对有限数据更鲁棒的分类方法。

Method: 采用MiniROCKET和HDC-MiniROCKET模型进行光谱分类，这些模型在特征提取部分没有可训练参数，使用精心设计的特征提取方法。

Result: MiniROCKET在有限数据场景下优于1D-Justo-LiuNet，在一般情况下性能相当，尽管参数更多但对有限训练数据更鲁棒。

Conclusion: MiniROCKET系列模型是光谱分类的有效替代方案，特别是在训练数据有限的情况下，能够提供更好的性能表现。
...
TL;DR: Semi-MOE：首个用于半监督组织病理学图像分割的多任务专家混合框架，通过三个专家网络和自适应多目标损失，在低标签设置下优于现有方法


<details>
  <summary>Details</summary>
Motivation: 解决现有半监督学习方法在组织病理学图像分割中因模糊腺体边界和形态误分类导致的噪声伪标签问题
...
Conclusion: 基于MoE的架构在推进半监督分割方面具有潜力，Semi-MOE框架有效解决了组织病理学图像分割中的伪标签噪声问题
...
### [56] [Where Do Tokens Go? Understanding Pruning Behaviors in STEP at High Resolutions](https://arxiv.org/abs/2509.14165)
*Michal Szczepanski,Martyna Poreba,Karim Haroun*

Main category: cs.CV

TL;DR: STEP是一个混合令牌缩减框架，结合动态补丁合并和令牌剪枝，在保持精度的同时显著提升Vision Transformer在语义分割中的效率


<details>
  <summary>Details</summary>
Motivation: Vision Transformers在语义分割中性能优异但计算和内存成本高昂，需要高效的令牌缩减方法来提升效率

Method: 提出STEP框架：1) dCTS轻量CNN策略网络实现灵活的超补丁合并 2) 编码器集成早期退出机制移除高置信度超令牌 3) 结合动态补丁合并和令牌剪枝

Result: 在1024x1024高分辨率图像上，dCTS单独应用可减少2.5倍令牌数，计算成本降低2.6倍，吞吐量提升3.4倍；完整STEP框架可达4倍计算复杂度降低和1.7倍推理加速，精度损失不超过2%

Conclusion: STEP框架有效解决了ViT在语义分割中的效率问题，通过动态令牌缩减实现了计算效率和精度的良好平衡
...
### [57] [Consistent View Alignment Improves Foundation Models for 3D Medical Image Segmentation](https://arxiv.org/abs/2509.13846)
*Puru Vaish,Felix Meister,Tobias Heimann,Christoph Brune,Jelmer M. Wolterink*

Main category: cs.CV

TL;DR: 该论文挑战了表示学习中视图不相关假设，提出显式对齐不同视图表示的方法来提升下游任务性能，在MICCAI 2025 SSL3D挑战中取得优异成绩。


<details>
  <summary>Details</summary>
Motivation: 当前表示学习方法隐含假设数据点的不相关视图足以学习有意义的表示，但作者发现潜在空间的有意义结构不会自然出现，需要显式诱导。

Method: 提出Consistent View Alignment方法，对齐数据不同视图的表示，在保持互补信息的同时避免产生假阳性对齐。

Result: 方法在MICCAI 2025 SSL3D挑战中使用Primus vision transformer和ResEnc CNN分别获得第一和第二名，下游任务性能显著提升。

Conclusion: 结构化视图对齐在学习有效表示中起着关键作用，显式对齐机制比隐含假设不相关视图更有效。
...
### [58] [SpecDiff: Accelerating Diffusion Model Inference with Self-Speculation](https://arxiv.org/abs/2509.13848)
*Jiayi Pan,Jiaming Xu,Yongkang Zhou,Guohao Dai*

Main category: cs.CV

TL;DR: SpecDiff是一种训练免费的多级特征缓存策略，通过自推测引入未来信息，结合历史信息进行动态特征选择和分类，在保持质量的同时实现显著加速。


<details>
  <summary>Details</summary>
Motivation: 现有特征缓存方法仅依赖历史信息，导致精度和速度性能受限。需要打破这种限制，通过引入未来信息来克服速度-精度权衡瓶颈。

Method: 提出自推测范式，利用不同迭代次数间相同时步的信息相似性引入未来信息。包括基于自推测信息的缓存特征选择算法和基于特征重要性分数的多级特征分类算法。

Result: 在Stable Diffusion 3、3.5和FLUX上分别实现平均2.80×、2.74×和3.17×的加速，质量损失可忽略不计。

Conclusion: SpecDiff通过融合推测和历史信息，突破了速度-精度权衡的瓶颈，推动了高效扩散模型推理的帕累托前沿。
...
### [59] [EDITS: Enhancing Dataset Distillation with Implicit Textual Semantics](https://arxiv.org/abs/2509.13858)
*Qianxin Xia,Jiawei Du,Guoming Lu,Zhiyong Shu,Jielei Wang*

Main category: cs.CV

TL;DR: EDITS是一个新的数据集蒸馏框架，利用视觉语言模型和大型语言模型提取图像中的文本语义信息，通过全局语义查询和局部语义感知构建图像和文本原型，最终使用扩散模型生成合成数据集，显著提升了蒸馏效果。


<details>
  <summary>Details</summary>
Motivation: 传统的数据集蒸馏方法主要捕获低层视觉特征，忽略了图像中蕴含的高层语义和结构信息，导致蒸馏效果受限。

Method: 首先使用视觉语言模型生成外部文本并与图像特征融合形成先验聚类缓冲区，然后通过局部语义感知选择代表性样本构建图像和文本原型，最后采用双原型指导策略通过扩散模型生成最终合成数据集。

Result: 大量实验证实了该方法的有效性，在保持竞争力的模型性能的同时实现了高效学习。

Conclusion: EDITS框架通过利用图像中的隐式文本语义信息，显著提升了数据集蒸馏的效果，为高效学习提供了新的解决方案。
...
TL;DR: LamiGauss是一种结合高斯泼溅辐射光栅化和专用探测器-世界变换模型的X射线层析成像重建算法，能够在极稀疏投影条件下实现高质量重建。


<details>
  <summary>Details</summary>
Motivation: 传统CT在平板结构检测中存在几何限制，而现有层析成像方法在稀疏投影条件下重建质量不佳，需要开发新的高效重建算法。

Method: 结合高斯泼溅辐射光栅化和包含层析倾斜角的专用变换模型，采用初始化策略过滤层析伪影，防止高斯分配到虚假结构，直接从稀疏投影优化。

Result: 在合成和真实数据集上验证了方法的有效性，仅使用3%的完整视图就能达到优于在全数据集上优化的迭代方法的性能。

Conclusion: LamiGauss算法能够有效处理稀疏投影条件下的X射线层析成像重建问题，在保持高质量的同时显著减少数据采集需求。
...
TL;DR: 提出了DAM4SAM，一个针对SAM2的干扰物感知内存模块和自省管理方法，有效减少跟踪漂移并提升遮挡后重检测能力，在多个基准测试中创下新记录。


<details>
  <summary>Details</summary>
Motivation: 现有基于内存的视频分割方法如SAM2在分割任务中表现优异，但在视觉目标跟踪中面对干扰物（与目标视觉相似的对象）时存在挑战，需要专门针对干扰物问题的解决方案。

Method: 提出了干扰物感知的即插即用内存模块和基于自省的内存管理方法，构建了DiDi干扰物蒸馏数据集用于分析，并将该模块集成到不同跟踪器架构中。

Result: DAM4SAM在13个基准测试中超越SAM2.1，在10个测试中创下新记录。集成到实时跟踪器EfficientTAM中提升11%性能，与EdgeTAM集成提升4%性能，显示出良好的架构泛化能力。

Conclusion: 提出的干扰物感知内存模块能有效提升目标跟踪在干扰物环境下的性能，具有良好的通用性和实际应用价值。
...
TL;DR: PelFANet是一种双流注意力网络，通过融合原始骨盆X光片和分割骨图像来提高骨折分类准确性，特别在细微或不可见骨折病例中表现优异。


<details>
  <summary>Details</summary>
Motivation: 骨盆骨折在标准X光片中常常难以诊断，特别是当骨折迹象细微或不可见时，需要更精确的检测方法来提高诊断准确性。

Method: 采用双流注意力网络，使用融合注意力块(FABlocks)迭代交换和精炼来自原始X光片和分割骨图像的特征，通过两阶段分割引导的管道进行训练。

Result: 在AMERI数据集上，可见骨折准确率达到88.68%，AUC为0.9334；对未训练过的不可见骨折病例也达到82.29%准确率和0.8688 AUC。

Conclusion: 解剖感知的双输入架构在骨折检测方面具有重要临床潜力，特别适用于放射学表现细微的情况。
...
TL;DR: EvHand-FPV是一个轻量级单事件相机第一人称视角3D手部追踪框架，通过手腕ROI定位、端到端映射和多任务学习策略，在保持精度的同时大幅降低计算量和参数数量。


<details>
  <summary>Details</summary>
Motivation: 传统帧式方法在精度、延迟和能效方面难以满足XR设备等资源受限场景的需求，事件相机提供微秒级时间分辨率和毫瓦级功耗，但缺乏第一人称视角的基准数据集。
...
Conclusion: EvHand-FPV实现了准确高效的自我中心事件手部追踪，适合设备端XR应用，提供了数据集和代码开源。
...
TL;DR: 提出WARM模块，通过白化和染色变换改进注意力机制，解决少样本3D点云分割中可学习原型标记与支持特征之间的分布差异问题，在多个基准测试中取得最先进性能


<details>
  <summary>Details</summary>
Motivation: 现有少样本3D点云分割方法使用传统算法（如最远点采样）构建原型，其初始随机性严重影响性能，且原型生成过程尚未充分探索。注意力机制虽有潜力，但存在分布差异问题

Method: 提出White Aggregation and Restoration Module (WARM)，在白化和染色变换之间夹入交叉注意力机制。白化将支持特征与原型标记对齐，注意力处理后染色恢复原始分布

Result: 在多个少样本3D点云分割基准测试中取得最先进性能，显著优于现有方法

Conclusion: WARM通过简单有效的设计实现了鲁棒的注意力机制，能够捕捉支持特征间的语义关系，生成具有代表性的原型
...
TL;DR: SRC框架通过迭代校准理性与答案的对齐，提升大型视觉语言模型的推理一致性和准确性


<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型在视觉问答中虽然表现出色，但理性推理与生成答案之间存在不一致性，导致错误响应

Method: 采用轻量级"理性微调"修改响应格式，使用R-Scorer评分模型评估候选响应质量，通过置信度加权的偏好微调方式校准对齐
...
TL;DR: 提出了AntiPure方法，通过频率引导和时间步错误引导来对抗图像净化技术，保护图像不被恶意定制和伪造


<details>
  <summary>Details</summary>
Motivation: 扩散模型如Stable Diffusion的强大定制能力带来了严重的安全风险，现有的保护性扰动方法容易被净化技术移除，导致图像再次面临恶意伪造的风险

Method: 提出AntiPure方法，包含两种引导机制：1) 块级频率引导，减少模型对净化图像高频分量的影响；2) 错误时间步引导，破坏模型在不同时间步的去噪策略

Result: 实验表明AntiPure在净化-定制工作流中实现了最小的感知差异和最大的失真效果，优于其他保护性扰动方法

Conclusion: AntiPure作为一种净化压力测试方法，能够有效嵌入难以察觉的扰动并在代表性净化设置下持续存在，实现有效的后定制失真
...
TL;DR: 提出Noise Level Guidance (NLG)方法，无需额外数据、网络或反向传播，通过优化初始噪声来提高扩散模型生成质量和提示遵循度


<details>
  <summary>Details</summary>
Motivation: 扩散模型使用随机高斯噪声启动会导致输出质量变化和提示遵循不一致，现有噪声优化方法需要额外数据集、网络或计算，实用性受限

Method: NLG方法通过增加初始噪声与通用指导对齐的可能性来优化噪声水平，无需额外训练数据、辅助网络或反向传播

Result: 在五个标准基准测试中，该方法显著提升了输出生成质量和输入条件遵循度

Conclusion: NLG为扩散模型提供了一个实用且可扩展的增强框架，能与现有指导方法无缝集成并保持计算效率
...
<details>
  <summary>Details</summary>
Motivation: 当前模型（包括类别无关计数模型和大规模视觉语言模型）在细粒度、意图驱动的计数任务上的能力尚不明确，需要专门的评估基准来诊断和改进细粒度视觉计数系统。

Method: 构建PairTally数据集，包含681张高分辨率图像，每张图像有两个物体类别，分为类别间（不同类别）和类别内（密切相关的子类别）两种设置。对多种最先进模型进行基准测试，包括基于示例的方法、语言提示模型和大规模视觉语言模型。

Result: 尽管最近有进展，但当前模型在可靠地计数用户意图方面仍然困难，特别是在细粒度和视觉模糊的情况下。
...
TL;DR: 本文提出了增强版YOLO-FEDER FusionNet无人机检测框架，通过整合通用目标检测和伪装目标检测技术，在复杂视觉环境中显著提升了检测性能。


<details>
  <summary>Details</summary>
Motivation: 解决无人机在复杂视觉环境中检测困难的问题，包括背景干扰、小目标尺度和伪装效应等挑战，提升在低目标-背景可分离性环境中的检测效果。

Method: 采用YOLO-FEDER FusionNet框架，整合通用目标检测和伪装目标检测技术。改进包括：使用大规模逼真合成数据加少量真实样本的训练策略；系统评估多尺度FEDER特征融合；测试多种YOLO骨干网络配置。

Result: 最佳配置（YOLOv8l骨干网络+DWD模块FEDER特征）相比基线：误报率降低39.1个百分点，mAP@0.5提升62.8个百分点，检测性能显著改善。

Conclusion: 集成中间FEDER特征结合骨干网络升级能显著提升复杂环境中无人机检测性能，为视觉复杂场景下的目标检测提供了有效解决方案。
...
TL;DR: SAIL-VL2是一个开源的2B和8B参数规模视觉语言基础模型，在106个数据集上实现SOTA性能，特别是在MMMU和MathVista等复杂推理基准测试中表现优异。
...
Method: 采用三核心创新：1)大规模数据筛选流水线提升数据质量；2)渐进式训练框架，从预训练视觉编码器到多模态预训练，再到思维融合SFT-RL混合范式；3)架构创新，包括稀疏混合专家(MoE)设计。

Result: 在OpenCompass排行榜上，SAIL-VL2-2B在4B参数规模以下的官方开源模型中排名第一，在多个图像和视频基准测试中达到最先进性能。

Conclusion: SAIL-VL2通过创新的数据管理、训练范式和架构设计，为开源多模态社区提供了一个强大、高效且可扩展的基础模型，在多模态理解和推理任务中表现出色。
...
TL;DR: 提出PROFUSEme方法，通过融合临床、影像和病理多模态数据来预测前列腺癌根治术后的生化复发，在内部验证和外部挑战中表现优异
...
Result: 内部5折嵌套交叉验证平均C-index为0.861，在CHIMERA 2025挑战验证集上C-index为0.7103，优于晚期融合配置

Conclusion: PROFUSEme方法通过多模态数据融合有效预测前列腺癌术后生化复发，具有临床实用价值
...
<details>
  <summary>Details</summary>
Motivation: 为了解决角色动画和替换任务中需要高保真度复制表情动作以及实现环境光照无缝集成的挑战，开发一个统一的框架来处理这些相关但不同的任务。
...
Conclusion: Wan-Animate成功地将多个角色动画任务统一到一个框架中，通过创新的技术方法实现了高质量的动画生成和环境集成，作者承诺将开源模型权重和源代码。
...
TL;DR: AD-DINOv3是一个新颖的零样本异常检测框架，通过结合DINOv3视觉编码器和CLIP文本编码器，并引入轻量级适配器和异常感知校准模块，有效解决了特征错位和全局语义偏差问题，在工业和医疗基准测试中达到或超越了最先进方法。


<details>
  <summary>Details</summary>
Motivation: 传统零样本异常检测主要基于CLIP模型，但最近DINOv3等视觉基础模型展现出强大的可迁移表示能力。然而将DINOv3应用于异常检测面临两个关键挑战：大规模预训练数据与异常检测任务之间的领域偏差导致特征错位，以及预训练表示对全局语义的固有偏差导致细微异常被误判为正常前景对象。

Method: 提出AD-DINOv3多模态框架：1) 将异常检测构建为多模态对比学习问题；2) 使用DINOv3作为视觉骨干提取patch token和CLS token；3) 使用CLIP文本编码器提供正常和异常提示的嵌入；4) 引入轻量级适配器桥接领域差距；5) 设计异常感知校准模块(AACM)显式引导CLS token关注异常区域。

Result: 在八个工业和医疗基准测试上的广泛实验表明，AD-DINOv3始终匹配或超越了最先进的零样本异常检测方法，验证了其作为通用框架的优越性。

Conclusion: AD-DINOv3成功将DINOv3适配到零样本异常检测任务，通过多模态对比学习和专门的校准机制有效解决了领域偏差和语义偏差问题，为异常检测提供了一个强大且通用的解决方案。
...
Conclusion: 提出的两种策略有效解决了弱监督音频-视觉视频解析中的关键挑战，取得了优异效果
...
TL;DR: 提出了一种基于软混合专家机制（Soft MoE）的遥感基础模型CSMoE，通过集成专家专业化和跨传感器表示学习，在保持性能的同时显著提升计算效率。


<details>
  <summary>Details</summary>
Motivation: 现有遥感基础模型存在计算复杂度高或表示能力有限的问题，限制了实际应用。需要开发既能保持高性能又具有计算效率的模型。
...
Conclusion: 所提出的软混合专家机制集成方法有效解决了遥感基础模型的计算效率问题，为开发高效遥感基础模型提供了可行方案。
...
<details>
  <summary>Details</summary>
Motivation: 传统组织病理学诊断需要多种化学染色，过程耗时费力且对环境不友好。现有虚拟染色方法依赖对齐良好的配对数据，但获取这种数据困难，因为化学染色过程会导致组织结构变形，且单个组织切片无法进行多次染色

Method: 采用级联配准机制的鲁棒虚拟染色框架，通过配准技术解决生成图像与真实标签之间的空间不匹配问题

Result: 在五个数据集上显著优于最先进模型，内部数据集平均提升3.2%，外部数据集提升10.1%。在严重不对齐的数据集上，PSNR比基线模型提升23.8%

Conclusion: 该方法在多样化数据集上表现出卓越的鲁棒性，简化了虚拟染色数据采集过程，为推进虚拟染色技术发展提供了新思路
...
TL;DR: 美颜滤镜会降低深度伪造和变形攻击检测器的性能，导致检测准确率下降，凸显了需要开发对这些面部美化操作具有鲁棒性的检测模型。


<details>
  <summary>Details</summary>
Motivation: 社交媒体美颜滤镜的普及引发了对面部图像视频可靠性和自动化面部分析有效性的担忧，特别是对于旨在区分真实数据和伪造数据的数字操纵检测器，在涉及深度伪造和变形攻击的情况下尤为重要。

Method: 通过全面分析，在基准数据集上评估多个最先进的检测器在应用各种平滑滤镜前后的性能表现。

Result: 研究发现美颜滤镜会导致检测器性能下降，揭示了面部美化引入的脆弱性。

Conclusion: 面部美化操作会削弱检测器的有效性，强调了开发对这些改变具有鲁棒性的检测模型的必要性。
...
<details>
  <summary>Details</summary>
Motivation: 通过大规模基准测试整合多模态机器学习和LLM方法，跟踪该快速发展领域的最新进展，并扩展多模态推理在真实世界和专业化场景中的应用

Method: 发布两个定制数据集（Lens支持12个日常场景的通用推理，AdsQA支持广告视频的领域特定推理），评估40+基线模型（包括通用MLLM和任务特定模型），设立三个竞赛赛道（VG-RS、VQA-SA、VR-Ads）

Result: 76个来自知名学术和工业机构的团队注册，40+有效提交（从1200+提交中筛选）进入排名榜单，所有数据集、代码集和排名结果公开可用

Conclusion: MARS2 2025挑战赛成功推动了多模态推理技术的发展，为研究社区提供了宝贵的资源和基准，未来将持续更新并举办相关活动
...
<details>
  <summary>Details</summary>
Motivation: 探索抽象图像能否有效传递视觉语义信息，以及为什么基于基本形状的抽象图像表示性能不如传统栅格图像，研究不同抽象层次对高层语义内容捕获能力的影响。

Method: 构建分层抽象图像数据集HAID，包含从正常栅格图像生成的多层次抽象图像；在HAID上训练和评估传统视觉系统，进行图像分类、分割和目标检测任务的全面对比研究。

Result: 研究发现抽象图像确实能够传递视觉语义信息，但在表示质量上仍不及传统栅格图像；不同抽象层次对语义内容捕获能力存在差异。

Conclusion: 抽象图像可以作为传递视觉语义信息的潜在有效格式，对视觉任务有贡献价值，但需要进一步研究如何提升其表示性能以缩小与传统图像的差距。
...
<details>
  <summary>Details</summary>
Motivation: 鸟瞰图感知在自动驾驶中具有重要价值，但现有研究忽略了域偏移问题，导致跨域迁移时性能显著下降。需要解决多视图3D目标检测中的域适应挑战

Method: 提出BEVUDA++框架，包含可靠深度教师(RDT)和几何一致学生(GCS)模型。RDT通过不确定性估计生成深度感知信息，GCS将多空间特征映射到统一的几何嵌入空间，并引入不确定性引导的指数移动平均(UEMA)减少误差累积

Result: 在四个跨域场景中实现了最先进的性能，在昼夜适应任务上获得了12.9% NDS和9.5% mAP的提升

Conclusion: 该方法有效解决了BEV感知中的域偏移问题，通过几何一致性学习和不确定性引导，显著提升了跨域3D目标检测的性能
...
TL;DR: 提出了Cineaste基准测试，用于评估模型在长视频电影理解中的细粒度推理能力，包含3119个多选题，涵盖5个推理类别，现有模型表现不佳（最高63.15%准确率）。


<details>
  <summary>Details</summary>
Motivation: 现有视频理解基准主要测试短片段识别或模板化问题，缺乏对长叙事内容的细粒度推理评估，需要更全面的长视频电影理解评估工具。

Method: 使用GPT-4o生成多样化、上下文丰富的问题，整合视觉描述、字幕、场景标题和摘要；采用两阶段过滤：上下文独立性过滤确保问题需要视频上下文，上下文真实性过滤验证事实一致性。

Result: 现有多模态大语言模型在Cineaste上表现不佳，最佳开源模型准确率仅63.15%，长时序推理是主要瓶颈。

Conclusion: 长视频电影理解存在显著挑战，需要在细粒度上下文理解和长时序推理方面取得进展。
...
<details>
  <summary>Details</summary>
Motivation: 现有考试式基准主要关注理解和推理任务，而当前生成基准强调世界知识和视觉概念的说明，忽略了严格绘图考试的评估。

Method: 构建包含1000个样本的多学科文本到图像考试基准，每个问题配备真实图像和细粒度评分点，采用四级分类法组织考试式提示。

Result: 实验显示即使最先进的模型如GPT-Image-1和Gemini-2.5-Flash-Image的严格得分也低于15%，大多数模型得分几乎为0%，表明该基准具有巨大挑战性。

Conclusion: 通过将图像生成构建为考试，GenExam提供了对模型整合知识、推理和生成能力的严格评估，为通向通用AGI的道路提供了见解。
...
<details>
  <summary>Details</summary>
Motivation: 意大利语等语法性别语言中的性别中性改写具有挑战性，需要消除不必要的性别指定同时保持语义，但目前缺乏系统评估。

Method: 采用少样本提示比较多个LLM，对选定模型进行微调，并应用针对性清理提升任务相关性，使用二维框架评估中立性和语义保真度。

Result: 开源权重LLM优于现有意大利语GNR专用模型，微调后的小模型以更小规模匹配或超越最佳开源LLM性能。

Conclusion: 研究揭示了在中立性优化和意义保持之间的权衡关系，为语法性别语言的性别中性改写提供了有效的评估框架和解决方案。
...
TL;DR: Op-Fed是一个包含1044条人工标注句子的数据集，来自FOMC会议记录，用于分析货币政策立场。通过五阶段分层标注方案和主动学习解决类别不平衡和上下文依赖问题。


<details>
  <summary>Details</summary>
Motivation: FOMC的货币政策决策影响数百万人，但现有数据集中表达非中性货币政策立场的句子不足8%，且65%的实例需要跨句上下文理解，需要专门的数据集来支持相关研究。

Method: 开发了五阶段分层标注方案来分离意见、货币政策和货币政策立场等维度；使用主动学习选择标注实例，使所有模式方面的正例数量大致翻倍。

Result: 性能最佳的闭源LLM在意见分类上达到0.80的零样本准确率，但在货币政策立场分类上仅0.61，低于人类基准0.89。

Conclusion: Op-Fed数据集可用于未来模型训练、置信度校准，并作为未来标注工作的种子数据集，特别是在需要理解跨句上下文的货币政策立场分析任务中。
...
TL;DR: 论文提出了一个分析框架来研究LLM跨任务迁移学习中的潜在能力和副作用，发现性能提升主要受隐藏统计因素而非表面数据集相似性的影响


<details>
  <summary>Details</summary>
Motivation: 由于LLM部署中经常遇到训练时未见的任务，且难以获得所有任务的高质量训练数据，需要研究不同特征数据集间的迁移学习和分布外请求的处理

Method: 构建迁移学习矩阵和降维分析框架，训练10个模型来识别潜在能力（如推理、情感分类、NLU、算术）并分析跨任务交互

Result: 发现性能改进往往无法用表面数据集相似性或源数据质量解释，而是受源数据集的隐藏统计因素（如类别分布、生成长度倾向）和特定语言特征影响更大
...
TL;DR: 研究发现LLM内部存在少量神经元编码问题模糊性信息，可在预填充阶段检测和控制，实现从直接回答到弃权的行为控制


<details>
  <summary>Details</summary>
Motivation: 现实问题普遍存在模糊性，但LLM往往自信回答而非寻求澄清，需要研究LLM如何内部表示和处理模糊性问题

Method: 识别LLM预填充阶段的模糊性编码神经元(AENs)，训练探针进行模糊性检测，通过神经元操作控制模型行为

Result: 发现仅需1个神经元即可编码模糊性信息，AEN探针在模糊性检测上优于基线方法，浅层神经元即出现模糊性编码

Conclusion: LLM形成紧凑的内部模糊性表示，支持可解释和可控的行为，为模型透明度提供了新见解
...
TL;DR: 提出了首个中文文献语法纠错持续学习基准CL²GEC，包含10个学科的1万条人工标注句子，评估LLM在持续学习场景下的跨学科语法纠错能力。
...
Method: 构建包含10个学科1万句的标注数据集，在持续学习设置下评估大语言模型，包括顺序调优、参数高效适应和四种代表性CL算法，使用标准GEC指标和适应任务变异的持续学习指标。
...
TL;DR: AgentCTG是一个新颖的可扩展框架，通过模拟多智能体工作流中的控制和调节机制，实现对文本生成的精确复杂控制。该方法在多个公开数据集上达到最先进效果，并在角色驱动重写任务中验证了实用性。


<details>
  <summary>Details</summary>
Motivation: 尽管NLP领域在许多任务上取得显著进展，但受控文本生成(CTG)仍面临诸多挑战，特别是在实现细粒度条件控制方面。实际应用场景中还需要考虑成本、可扩展性、领域知识学习和更精确控制等需求。

Method: 提出AgentCTG框架，通过多智能体工作流模拟控制和调节机制，探索不同智能体间的协作方法，并引入自动提示模块来增强生成效果。

Result: 在多个公开数据集上达到最先进结果。在角色驱动重写任务中成功将原始文本转换为符合特定角色配置文件的新文本，同时保留领域知识。在线导航角色扮演应用中显著提升了驾驶体验。

Conclusion: AgentCTG通过优化上下文相关文本的生成，实现了更沉浸式的在线社区互动，促进了更大的个性化和用户参与度，为受控文本生成提供了有效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型在上下文保真度方面存在困难，当基于提供的信息回答问题时会产生不一致的答案。现有方法要么依赖昂贵的监督微调来生成答案后的证据，要么训练模型进行网络搜索而不一定改善给定上下文的利用。

Method: 提出CARE框架，教导LLMs利用自身的检索能力，在推理过程中显式整合上下文证据。该方法只需要有限的标记证据数据，通过策略性检索的上下文标记来增强推理链。

Result: 在多个真实世界和反事实QA基准测试上的广泛实验表明，该方法显著优于监督微调、传统检索增强生成方法和外部检索解决方案。

Conclusion: 这项工作代表了在使LLMs在知识密集型任务中更加准确、可靠和高效方面的根本性进步。
...
TL;DR: 本文构建了一个专注于比较级推理的日语NLI数据集，评估了LLMs在零样本和少样本设置下的表现，发现模型对提示格式敏感，且在处理日语特有语言现象时存在困难，但包含逻辑语义表示的提示能提升性能。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在自然语言推理方面表现优异，但涉及数值和逻辑表达式的推理仍然具有挑战性。比较级是与此类推理相关的关键语言现象，但LLMs在处理这些现象时的鲁棒性，特别是在训练数据中非主导语言（如日语）方面，尚未得到充分探索。

Method: 构建了一个专注于比较级的日语NLI数据集，并在零样本和少样本设置下评估了各种LLMs。分析了不同提示格式的影响以及少样本示例中黄金标签的作用。

Result: 模型在零样本设置中对提示格式敏感，在少样本设置中受黄金标签影响。LLMs在处理日语特有语言现象时存在困难。包含逻辑语义表示的提示有助于模型预测那些即使在少样本示例下也难以解决的推理问题的正确标签。

Conclusion: 研究表明LLMs在处理日语比较级推理时存在局限性，但通过适当的提示工程（特别是包含逻辑语义表示）可以显著提升模型性能，这为改进多语言NLI系统提供了重要见解。
...
TL;DR: 使用DSPy提示优化技术，将指令调优的大型语言模型应用于临床分类任务，能够同时处理临床文本和结构化EHR数据，性能媲美专业多模态系统且更简单灵活
...
TL;DR: DSCC-HS是一个主动干预的自回归解码框架，通过紧凑代理模型动态校准大语言模型的输出，在TruthfulQA上达到99.2%的事实一致性率。
...
Method: 基于双过程认知理论，训练Factual Alignment Proxy和Hallucination Detection Proxy两个对抗角色代理模型，在推理时通过实时注入转向向量来动态引导目标模型。

Result: 在TruthfulQA上达到99.2%的事实一致性率，在BioGEN基准上获得最高FActScore 46.50，实现最先进性能。

Conclusion: DSCC-HS是一个原理清晰、高效的即插即用解决方案，能显著增强大语言模型的事实性，无需修改目标模型。
...
TL;DR: 开发了一个基于自然语言处理的筛查工具，用于自动检测放射肿瘤学中的高严重性事件报告，通过跨机构迁移学习提高了模型泛化能力。
...
Result: 在机构内部测试中AUROC达到0.82(SVM)和0.81(BlueBERT)，跨机构迁移学习后BlueBERT_TRANSFER在外部数据集上AUROC提升至0.78，与人类专家性能相当(AUROC 0.81)。

Conclusion: 成功开发了跨机构的NLP模型，能够以与人类专家相当的水平检测高严重性事件报告，为医疗安全质量改进提供了有效的自动化工具。
...
TL;DR: 提出DSPC双阶段渐进压缩方法，无需训练即可压缩LLM提示，在减少3倍token的同时性能提升7.76%，优于现有最佳方法


<details>
  <summary>Details</summary>
Motivation: 解决LLM提示越来越长导致计算成本增加的问题，现有压缩方法需要训练辅助模型带来额外计算开销

Method: 两阶段无训练方法：粗粒度阶段基于TF-IDF过滤低语义价值句子；细粒度阶段通过注意力贡献、跨模型损失差异和位置重要性评估token重要性，修剪低效用token

Result: 在LLaMA-3.1-8B-Instruct和GPT-3.5-Turbo上验证，在Longbench数据集的FewShot任务中仅用3倍少token达到49.17性能，比最佳基线LongLLMLingua提升7.76

Conclusion: DSPC方法有效解决了提示压缩问题，无需训练即可显著减少计算成本同时保持性能
...
TL;DR: 本研究提出了ccg-jcomp系统，这是一个基于组合语义学的日语比较句逻辑推理系统，用于处理自然语言推理中的比较句挑战。


<details>
  <summary>Details</summary>
Motivation: 日语和英语比较句在形态和语义上存在显著差异，使得现有的英语比较句逻辑推理系统难以直接应用于日语。需要专门针对日语比较句特点设计逻辑推理系统。

Method: 基于组合语义学构建逻辑推理系统ccg-jcomp，专门处理日语比较句的数值和逻辑表达式，并在包含比较表达的日语NLI数据集上进行评估。

Result: 通过与现有大型语言模型的准确率比较，证明了所提出系统的有效性。
...
TL;DR: 本文探索了阿拉伯语方言识别(ADI)的数据高效和参数高效方法，包括软提示策略、LoRA重参数化以及零样本/少样本推理，发现LLMs在区分方言细微差别方面表现不佳，而LoRA微调模型表现最佳。


<details>
  <summary>Details</summary>
Motivation: 研究阿拉伯语方言识别的高效方法，旨在解决大语言模型在区分方言细微差别方面的困难，探索参数高效和数据高效的替代方案。

Method: 采用多种软提示策略(prefix-tuning、prompt-tuning、P-tuning、P-tuning V2)和LoRA重参数化；使用阿拉伯语专用编码器模型和开源解码器模型进行实验；分析零样本和少样本推理能力。

Result: LLMs在零样本和少样本设置下难以区分方言细微差别；软提示编码器变体表现更好；基于LoRA的微调模型表现最佳，甚至超过了完全微调。

Conclusion: LoRA等参数高效方法在阿拉伯语方言识别任务中表现出色，为资源受限环境下的方言识别提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 当前课程调优方法依赖静态启发式难度度量，无法适应模型在训练过程中不断发展的能力，导致固定的、可能次优的学习轨迹。

Method: 提出CAMPUS框架，包含三个核心优势：动态子课程选择、能力感知的课程进度调整、基于多种难度的调度策略。

Result: 大量实验证明，CAMPUS在高效指令调优方面相比其他最先进基线方法表现出优越性能。

Conclusion: CAMPUS通过动态多视角课程学习方法有效解决了课程僵化问题，为指令调优提供了更有效的训练策略。
...
<details>
  <summary>Details</summary>
Motivation: 研究语法性别在职位名称中的显式分配如何影响自动职位排名系统的结果，为评估职位排名系统中的性别偏见奠定基础。

Method: 提出使用RBO（Rank-Biased Overlap）指标来比较控制性别因素的排名结果，生成并分享了四种语法性别语言的职位名称匹配测试集，包含男性和女性形式的职业名称，并标注了性别和匹配相关性。

Result: 使用新测试集和提出的方法评估了多个现成的多语言模型的性别偏见，结果显示所有模型都表现出不同程度的性别偏见。

Conclusion: 自动职位排名系统普遍存在性别偏见问题，需要开发更公平的算法和评估方法来减少这种偏见对就业市场的影响。
...
Result: 在短形式问答数据集上表现优于或相当于现有方法，在医疗数据集上取得更优结果（幻觉风险特别关键的场景）。理论证明凸包体积与熵之间存在联系。

Conclusion: 提出的几何框架为黑盒设置下的幻觉检测提供了有效的全局和局部不确定性量化方法，特别在关键风险领域具有重要应用价值。
...
TL;DR: AutoMin 2025共享任务，包含会议纪要生成和问答两个任务，涵盖英语和捷克语，但参与团队较少，组织者提供了多个基线系统进行LLM评估。
...
Method: 设置两个主要任务：1）会议纪要生成（英语和捷克语，项目会议和欧洲议会）；2）问答任务（单语言英语问答和跨语言捷克语问答）。组织者提供多个基线系统进行基准测试。

Result: 2025年参与度较低：纪要任务只有1个团队参加，问答任务有2个团队参加。但通过组织者提供的多个基线系统，实现了对大语言模型的全面评估。

Conclusion: 尽管参与团队数量有限，但AutoMin 2025成功建立了评估框架，为自动会议纪要技术和跨语言问答系统的性能基准测试提供了重要参考。
...
TL;DR: 该研究分析了大型语言模型对德国方言使用者的偏见，发现所有测试模型都表现出显著的方言命名和使用偏见，且明确标注语言人口统计信息会放大这种偏见。


<details>
  <summary>Details</summary>
Motivation: 尽管方言具有重要文化价值，但方言使用者常面临负面社会刻板印象。研究旨在探究这种刻板印象是否在大语言模型中得到反映。

Method: 基于社会语言学文献构建评估框架，通过关联任务和决策任务分析方言命名偏见和使用偏见，并创建包含7种德国方言与标准德语对照的新评估语料库。

Result: 所有测试模型都表现出显著的方言命名和使用偏见，体现为负面形容词关联；模型在决策中重现这些偏见；明确标注语言人口统计信息比隐晦提示更能放大偏见。

Conclusion: 大语言模型确实反映了社会中对方言使用者的负面刻板印象，且明确的人口统计标注会加剧偏见，这对模型公平性评估具有重要意义。
...
TL;DR: 研究发现大型语言模型在社会偏见场景中与人类价值观的对齐程度存在差异，模型规模大不一定意味着更好的对齐效果，不同模型家族在特定类型场景中表现出一致性偏好。


<details>
  <summary>Details</summary>
Motivation: 研究大型语言模型与人类价值观在社会偏见方面的对齐情况，特别是探索不同类型偏见场景下模型对齐的差异，以及模型对偏见解释的理解能力。

Method: 通过分析12个来自4个模型家族的大型语言模型和4个数据集，评估模型在不同类型偏见场景中的对齐表现，并研究模型对偏见解释的理解能力，同时训练小型语言模型生成解释。

Result: 发现模型参数规模大并不一定带来更低的对齐错误率和攻击成功率；模型对特定类型场景有对齐偏好；同一模型家族的模型判断一致性更高；模型对偏见解释的理解能力无显著差异；微调后的小模型生成解释可读性更好但模型认同度较低。

Conclusion: 大型语言模型在社会偏见价值观对齐方面存在场景类型偏好和模型家族一致性特征，模型规模不是决定对齐效果的关键因素，小型模型经过训练可以生成可读性更好的解释但需要提升模型认同度。
...
TL;DR: CER框架结合科学证据检索、大语言模型推理和监督验证预测，用于生物医学事实核查，在多个数据集上达到最先进性能


<details>
  <summary>Details</summary>
Motivation: 医疗领域错误信息（如疫苗犹豫和未经证实的治疗方法）对公共卫生和医疗系统信任构成风险，生物医学声明验证因复杂术语、领域专业知识和科学证据基础需求而具有独特挑战

Method: CER框架整合科学证据检索、大语言模型推理和监督验证预测，通过将大语言模型文本生成能力与高质量生物医学科学证据检索技术相结合，有效减少幻觉风险

Result: 在专家标注数据集（HealthFC、BioASQ-7b、SciFact）上评估显示达到最先进性能，并展现出良好的跨数据集泛化能力

Conclusion: CER框架为生物医学事实核查提供了有效解决方案，通过证据基础和推理的结合确保输出基于可验证的证据来源，代码和数据已开源以促进透明性和可复现性
...
<details>
  <summary>Details</summary>
Motivation: 医疗领域中的错误信息（如疫苗犹豫和未经证实的治疗方法）对公共卫生和医疗系统信任构成风险。生物医学声明验证具有独特挑战性，需要处理复杂术语、领域专业知识，并基于科学证据进行验证。

Method: CER框架整合了科学证据检索、大语言模型推理和监督真实性预测。通过将大语言模型的文本生成能力与高质量生物医学科学证据的先进检索技术相结合，有效减少幻觉风险，确保输出基于可验证的证据来源。

Result: 在专家标注的数据集（HealthFC、BioASQ-7b、SciFact）上的评估显示，CER实现了最先进的性能，并展现出有前景的跨数据集泛化能力。

Conclusion: CER框架为生物医学事实核查提供了有效的解决方案，通过结合证据检索和语言模型推理，在确保证据基础的同时实现了优异的性能，代码和数据已开源以确保透明性和可重现性。
...
TL;DR: 本文评估了大语言模型在词义消歧任务中的表现，发现GPT-4o和DeepSeek-V3等领先模型在WSD任务上达到了与专门系统相当的性能，同时在生成任务中能以高达98%的准确率解释上下文中的词义。


<details>
  <summary>Details</summary>
Motivation: 尽管已有大量评估工作，但大语言模型是否真正理解词义仍未被充分探索。本文旨在填补这一空白，评估指令调优LLMs的词义消歧能力及其在生成任务中的词义理解能力。

Method: 评估方法包括：1）将指令调优LLMs与专门设计的WSD系统进行词义消歧性能比较；2）评估两个顶级开源和闭源LLMs在三种生成设置下的词义理解能力：定义生成、自由形式解释和示例生成。

Result: 研究发现：在WSD任务中，GPT-4o和DeepSeek-V3等领先模型达到了与专门WSD系统相当的性能，且在跨领域和难度级别上表现出更强的鲁棒性。在生成任务中，LLMs能以高达98%的准确率解释上下文中的词义，其中自由形式解释任务表现最佳。

Conclusion: 大语言模型在词义消歧任务中表现出与专门系统相当的能力，同时在生成任务中展现出强大的词义理解和解释能力，特别是在自由形式解释任务中表现最优，这与其生成能力最为匹配。
...
TL;DR: 研究发现多语言RAG系统中存在英语偏好偏见，模型会优先引用英文文档而非最相关文档，这种偏见在低资源语言和中间位置文档中更加明显


<details>
  <summary>Details</summary>
Motivation: 研究多语言检索增强生成系统中不同文档语言混合是否会对生成和引用行为产生意外影响，特别是语言偏好是否会影响引用选择

Method: 使用模型内部机制的控制方法，在保持文档相关性等其它因素不变的情况下测量语言偏好，涵盖8种语言和6个开源模型

Result: 模型在英语查询时优先引用英文来源，这种偏见在低资源语言和中间位置文档中被放大，模型有时会牺牲文档相关性来满足语言偏好

Conclusion: 引用选择并非总是由信息量驱动，语言偏好显著影响多语言上下文中的引用行为，这对理解语言模型如何利用多语言上下文具有重要意义
...
TL;DR: 基于COMET框架构建的翻译质量评估系统，通过长上下文数据增强和多种人工标注数据集整合，显著提升了与人工评估的相关性


<details>
  <summary>Details</summary>
Motivation: 解决传统翻译质量评估模型在短片段训练上的局限性，通过利用长上下文信息来更好地预测翻译质量

Method: 使用COMET框架，通过拼接领域内人工标注句子构建长上下文训练数据，计算加权平均分数，整合MQM、SQM和DA多种人工评估数据集，训练多语言回归模型

Result: 实验结果表明，引入长上下文信息的模型相比仅使用短片段训练的模型，与人工评估的相关性得到显著提升

Conclusion: 长上下文信息对于翻译质量评估具有重要价值，多数据集整合和长上下文训练能够有效提升评估模型的性能
...
TL;DR: Slim-SC是一种新的测试时缩放技术，通过逐步剪枝冗余推理链来加速自一致性方法，在保持或提高准确性的同时显著降低计算开销。


<details>
  <summary>Details</summary>
Motivation: 自一致性(SC)方法虽然能提高LLM推理性能，但其数量级计算开销限制了广泛应用。现有加速方法主要依赖模型置信度或缺乏理论支持的启发式方法。

Method: 提出Slim-SC方法，在思想层面利用链间相似性识别和移除冗余推理链，采用逐步剪枝策略。

Result: 在三个STEM推理数据集和两种LLM架构上，Slim-SC将推理延迟降低45%，KVC使用降低26%，同时保持或提高准确性。

Conclusion: Slim-SC为自一致性方法提供了一个简单而高效的测试时缩放替代方案，显著降低了计算成本。
...
TL;DR: ES-CoT是一种推理时方法，通过检测答案收敛性来提前停止思维链生成，在保持精度的同时平均减少41%的推理token使用


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在解决复杂问题时需要生成长思维链，但这种方法推理成本高昂，需要一种方法来缩短思维链生成而不损失性能
...
Conclusion: ES-CoT是一种实用有效的推理效率提升方法，步骤答案会稳定收敛到最终答案，大的运行长度跳跃可靠地标记了这种收敛
...
<details>
  <summary>Details</summary>
Motivation: 解决阿拉伯语NLP中高质量指令数据和专用模型缺乏的问题，通过创新的翻译调优流程创建阿拉伯语指令数据集。

Method: 使用FP8压缩的AR↔EN教师模型生成高质量双语监督数据，然后用轻量级模型翻译英文指令集为阿拉伯语，最后训练不同规模的Hala模型并应用slerp合并技术。

Result: 在阿拉伯语基准测试中，Hala在"nano"(≤2B)和"small"(7-9B)类别中都达到了最先进水平，性能超越了其基础模型。

Conclusion: Hala模型系列为阿拉伯语NLP研究提供了有效的解决方案，通过释放模型、数据、评估和配方来加速该领域的研究进展。
...
TL;DR: 本研究比较了文本和音频两种方式评估机器翻译质量，发现基于音频的评估能更自然地反映真实应用场景，且评估结果与文本评估基本一致，但在某些情况下能识别出系统间的显著差异。
...
Method: 使用亚马逊Mechanical Turk众包平台，对WMT通用机器翻译共享任务的10个系统进行文本和音频两种方式的评估，并进行统计显著性检验和自复制实验来验证音频方法的可靠性。
...
TL;DR: 本文验证了训练数据中上下文丰富样本的稀疏性是机器翻译模型难以有效利用上下文的主要原因，并通过控制实验证实了这一瓶颈。研究发现不同上下文现象之间缺乏泛化性，并提出两种训练策略，在单语和多语设置下分别获得6%和8%的准确率提升。


<details>
  <summary>Details</summary>
Motivation: 人类水平的翻译需要利用上下文来确保连贯性和处理复杂现象（如代词消歧）。标准训练数据中上下文丰富样本的稀疏性被认为是模型难以利用上下文的主要原因，本文旨在系统验证这一假设。

Method: 通过构建具有受控比例上下文相关示例的训练数据集，在单语和多语设置下进行系统验证。提出并实证评估两种训练策略来充分利用可用数据。

Result: 证实了训练数据稀疏性与模型性能之间的强关联，发现不同上下文现象之间缺乏泛化性。提出的训练策略在ctxPro评估中使单语和多语设置的准确率分别提升6和8个百分点。

Conclusion: 上下文样本稀疏性是机器翻译中上下文利用的关键瓶颈，不同上下文现象需要专门处理，提出的数据利用策略能有效提升模型性能，但跨语言迁移效果有限。
...
<details>
  <summary>Details</summary>
Motivation: 现有的多智能体辩论系统中，虽然某些LLM在特定任务上具有更好的知识或推理能力，但由于缺乏置信度表达，难以在辩论中清晰传达这种优势，导致系统可能固执坚持错误信念或过早收敛于次优答案

Method: 开发ConfMAD框架，在多智能体辩论过程中集成置信度表达机制，让LLM能够明确传达其置信水平

Result: 实验结果表明该方法有效，并分析了置信度如何影响辩论动态，为设计置信度感知的多智能体辩论系统提供了见解

Conclusion: 在多智能体辩论系统中引入置信度表达机制能够显著提升辩论效果和整体系统性能，ConfMAD框架为此类系统的设计提供了有效解决方案
...
TL;DR: 提出基于问题的手语翻译任务(QB-SLT)，通过对话上下文提升翻译质量，开发了跨模态自监督学习融合方法SSL-SSAW，在新建数据集上达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 手语翻译中对话提供重要上下文线索，但传统依赖gloss标注成本高且不自然。对话自然发生且更易标注，探索如何有效整合对话信息来改进翻译。

Method: 提出跨模态自监督学习与Sigmoid自注意力加权融合方法(SSL-SSAW)：使用对比学习对齐多模态特征，SSAW模块自适应提取问题和手语序列特征，利用自监督学习增强表示能力。

Result: 在新建的CSL-Daily-QA和PHOENIX-2014T-QA数据集上达到SOTA性能，易获取的问题辅助可以达到甚至超过gloss辅助的性能，可视化结果验证了对话整合的有效性。

Conclusion: QB-SLT任务具有实用价值，提出的SSL-SSAW方法能有效利用对话上下文提升翻译质量，问题辅助是gloss标注的可行替代方案。
...
Conclusion: Canary-1B-v2证明了在语音处理任务中，通过优化的架构设计和训练策略，可以在保持竞争力的同时实现显著的效率提升。
...
TL;DR: CS-FLEURS是一个新的代码切换语音识别和翻译数据集，包含4个测试集，覆盖113种语言对的52种语言，以及128小时的训练数据，旨在推动低资源语言的代码切换研究。


<details>
  <summary>Details</summary>
Motivation: 当前代码切换语音识别和翻译系统主要针对高资源语言，缺乏对低资源语言的支持。CS-FLEURS旨在填补这一空白，为开发更广泛的代码切换系统提供数据支持。

Method: 构建包含4个测试集的数据集：1）14种X-英语语言对的真实语音阅读合成代码切换句子；2）16种X-英语语言对的生成式文本转语音；3）60种{阿拉伯语、普通话、印地语、西班牙语}-X语言对的生成式文本转语音；4）45种X-英语低资源语言对的拼接式文本转语音。同时提供128小时的16种X-英语语言对训练数据。

Result: 成功创建了CS-FLEURS数据集，包含113种独特代码切换语言对，覆盖52种语言，为代码切换语音研究提供了全面的测试和训练资源。

Conclusion: CS-FLEURS数据集将有助于扩大代码切换语音研究的范围，特别是在低资源语言方面，为未来研究提供了宝贵的数据基础。
...
<details>
  <summary>Details</summary>
Motivation: 现有的多模态大语言模型联想能力评估框架往往忽视了关联任务中固有的模糊性问题，这种模糊性源于联想的发散性，会降低评估的可靠性。

Method: 将模糊性分解为内部模糊性和外部模糊性，引入AssoCiAm基准，采用混合计算方法来规避模糊性，对多模态大语言模型进行广泛实验。

Result: 发现认知与联想之间存在强正相关关系，评估过程中的模糊性会导致模型行为更加随机化，验证了所提方法在确保评估准确性和可靠性方面的有效性。

Conclusion: AssoCiAm基准能够有效解决联想评估中的模糊性问题，为多模态大语言模型的联想能力提供更准确可靠的评估框架，揭示了认知与联想的重要关联。
...
TL;DR: 提出了一种整合金融背景和行为金融学的新框架，通过精心策划的数据和微调，使8B参数模型在财务咨询任务上达到与更大模型(14-32B)相当的性能，同时成本降低80%。


<details>
  <summary>Details</summary>
Motivation: 个性化财务咨询需要考虑用户目标、约束、风险承受能力和司法管辖区。现有方法维护成本高且收益低于预期，需要更有效的端到端财务顾问解决方案。

Method: 构建了一个整合金融背景和行为金融学研究的监督数据框架，创建了19k样本的推理数据集，并对Qwen-3-8B模型进行全面微调。

Result: 通过保留测试集和盲审LLM评审研究，证明8B模型在事实准确性、流畅性和个性化指标上与更大基线模型(14-32B)性能相当，同时成本降低80%。

Conclusion: 通过精心数据策划和行为整合，较小的模型可以实现与更大模型相当的性能，显著降低成本，为个性化财务顾问系统提供了可复现的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 通过大规模计算分析理解英美两国议会移民话语的长期演变趋势和差异，探索大语言模型在政治历史话语分析中的可扩展应用。

Method: 使用开源大语言模型对英国议会75年辩论和美国国会辩论进行标注，识别对移民的高级立场态度；为英国开发半自动化框架提取细粒度叙事框架，追踪话语随时间、政党的变化。

Result: 美国移民话语日益极化，英国各党派立场相对一致但保守党与工党意识形态差距在2025年达到最负面；英国话语转向边境管控、非法移民等安全化叙事，社会整合等长期整合框架减少；移民讨论从国内法转向国际法和人权。

Conclusion: 大语言模型能够支持政治历史语境中可扩展的细粒度话语分析，揭示了移民话语的复杂演变模式和跨国差异。
...
TL;DR: Apertus是一个完全开源的大语言模型套件，专注于数据合规性和多语言表示，使用公开可用数据训练，采用Goldfish目标抑制记忆化，支持1800多种语言，在8B和70B规模上达到最先进的多语言性能。


<details>
  <summary>Details</summary>
Motivation: 解决当前开源模型生态系统中数据合规性和多语言表示不足的问题，避免使用未经许可的数据和侵犯内容所有者权利。

Method: 使用完全公开可用的数据进行预训练，尊重robots.txt排除规则，过滤非许可、有毒和个人身份信息内容；采用Goldfish目标抑制记忆化；在15T tokens上训练，覆盖1800多种语言，40%数据为非英语内容。

Result: Apertus模型在多语言基准测试中接近完全开源模型的最先进结果，与开源权重对应模型相当或超越；发布了8B和70B两种规模的模型。

Conclusion: Apertus提供了一个完全透明、合规的开源大语言模型解决方案，不仅发布模型权重，还包括数据准备脚本、检查点、评估套件和训练代码等所有科学成果，支持透明审计和扩展。
...
### [123] [The NIAID Discovery Portal: A Unified Search Engine for Infectious and Immune-Mediated Disease Datasets](https://arxiv.org/abs/2509.13524)
...
TL;DR: NIAID数据生态系统发现门户是一个统一的搜索平台，整合了400多万个传染病和免疫介导疾病研究相关数据集，提供用户友好的搜索过滤器和API访问，旨在降低数据访问门槛并促进数据重用。


<details>
  <summary>Details</summary>
Motivation: 解决有价值的研究数据集因分散在不同存储库中而难以被发现和重用的问题，为传染病和免疫介导疾病研究社区提供集中化的数据发现解决方案。

Method: 集成来自专业领域和通用存储库的元数据，通过标准化关键元数据字段和协调异构格式，提供过滤器和预构建查询功能，支持探索性浏览和精确搜索。

Result: 建立了包含400多万个数据集的统一搜索门户，支持流行病学、临床和多组学等多种资源类型的发现，提供了简化发现过程的工具和程序化访问API。

Conclusion: 该门户通过降低重要生物医学数据集的访问障碍，作为研究人员理解、诊断或治疗传染病和免疫介导疾病的入口点，支持假设生成、比较分析和公共数据的二次使用。
...
### [124] [Tractability Frontiers of the Shapley Value for Aggregate Conjunctive Queries](https://arxiv.org/abs/2509.13565)
*Christoph Standke,Benny Kimelfeld*

Main category: cs.DB

TL;DR: 本文研究了在聚合连接查询中计算元组Shapley值的复杂度问题，针对不同聚合函数（min、max、count-distinct、average、quantile）确定了可处理的层次化查询类别，并证明了这些类别的极大性。


<details>
  <summary>Details</summary>
Motivation: Shapley值作为衡量元组对数据库查询结果贡献的重要指标，其计算复杂度问题尚未完全解决。先前工作只解决了sum和count聚合函数的情况，其他常见聚合函数的复杂度仍为开放问题。

Method: 通过识别每个聚合函数对应的层次化查询类别，分析在这些类别中Shapley值的可计算性，并证明这些类别的极大性（即超出这些类别的查询在特定值函数下是#P-难的）。

Result: 发现不同聚合函数对应不同的层次化查询泛化：max、min、count-distinct对应all-hierarchical查询，average和quantile对应更严格的q-hierarchical查询。每个类别都是可处理性的极大类。

Conclusion: 研究为不同聚合函数的Shapley值计算提供了完整的复杂度刻画，揭示了聚合函数特性与查询结构层次性之间的对应关系，解决了先前提出的开放问题。
...
### [125] [XASDB -- Design and Implementation of an Open-Access Spectral Database](https://arxiv.org/abs/2509.13566)
*Denis Spasyuk*

Main category: cs.DB

TL;DR: XASDB是一个基于网络的X射线吸收光谱数据库平台，由加拿大光源开发，包含1000多个参考光谱，支持浏览器端数据处理和可视化，促进FAIR数据原则和协作研究。


<details>
  <summary>Details</summary>
Motivation: 随着同步辐射设施产生的X射线吸收光谱数据量和复杂性不断增加，需要强大的数据管理、共享和分析基础设施来处理多样化的数据格式并促进科学研究。

Method: 采用Node.js/MongoDB架构开发网络平台，集成XASproc JavaScript库进行浏览器端数据处理（归一化、背景扣除、EXAFS提取等），并通过XASVue光谱查看器提供跨设备可视化功能。

Result: 成功构建了包含40种元素、324种化学化合物的1000多个参考光谱的数据库，实现了标准化数据输出、完整元数据和集成分析能力，支持线性组合拟合、机器学习和教育应用。

Conclusion: XASDB展示了以网络为中心的方法在XAS数据分析中的潜力，加速了材料科学、环境研究、化学和生物学领域的进展，同时促进了FAIR数据原则和协作研究。
...
### [126] [Algorithms for Optimizing Acyclic Queries](https://arxiv.org/abs/2509.14144)
*Zheng Luo,Wim Van den Broeck,Guy Van den Broeck,Yisu Remy Wang*

Main category: cs.DB

TL;DR: 本文提出了三种构建无环查询连接树的方法：枚举所有连接树、构建最浅连接树实现并行执行、将左深线性计划转换为连接树以重用二元连接优化基础设施。


<details>
  <summary>Details</summary>
Motivation: 传统查询优化主要关注二元连接算法（如哈希连接、排序合并连接），但近年来理论最优算法（如Yannakakis算法）受到关注。这些算法依赖连接树而非二元连接的运算符树，需要新的优化技术。

Method: 1. 为α-无环查询提供摊销常数延迟枚举所有连接树的算法；2. 使用Tarjan和Yannakakis的最大基数搜索算法为Berge-无环查询构建唯一最浅连接树；3. 为γ-无环查询提供将左深线性计划转换为连接树的简单算法。

Result: 开发了基于成本的acyclic连接优化器基础，实现了大连接查询的并行执行能力，并允许重用现有的二元连接优化基础设施。

Conclusion: 提出的三种方法为不同类别的无环查询提供了有效的连接树构建技术，填补了理论最优算法在实际查询优化中的应用空白，为现代查询处理系统提供了重要的优化工具。
...
### [127] [Explicit Reasoning Makes Better Judges: A Systematic Study on Accuracy, Efficiency, and Robustness](https://arxiv.org/abs/2509.13332)
*Pratik Jayarao,Himanshu Gupta,Neeraj Varshney,Chaitanya Dwivedi*

Main category: cs.AI

TL;DR: 本研究系统比较了思考型和非思考型LLM在LLM作为评判者范式中的表现，发现思考型模型在准确性、计算效率和鲁棒性方面均优于非思考型模型，即使经过多种增强策略改进后。


<details>
  <summary>Details</summary>
Motivation: 随着大型语言模型越来越多地被用作自动评判工具，确保其可靠性、效率和鲁棒性变得至关重要。本研究旨在系统比较思考型和非思考型LLM在评判任务中的表现差异。

Method: 使用开源Qwen 3模型（0.6B、1.7B和4B参数），在RewardBench任务上评估准确性和计算效率（FLOPs）。对非思考型模型测试了多种增强策略，包括上下文学习、规则引导评判、基于参考的评估和n-best聚合。

Result: 思考型模型准确率高出约10个百分点，计算开销仅增加不到2倍，而增强策略如少样本学习虽然带来适度提升但成本更高（>8倍）。在多种偏见条件下，思考型模型保持显著更高的一致性（平均高6%）。多语言实验也证实显式推理的优势超越英语。

Conclusion: 显式推理在LLM作为评判者范式中具有明显优势，不仅在准确性和效率方面，而且在鲁棒性方面都表现出色，为LLM评判系统的设计提供了重要指导。
...
### [128] [Evaluation Awareness Scales Predictably in Open-Weights Large Language Models](https://arxiv.org/abs/2509.13333)
*Maheep Chaudhary,Ian Su,Nikhil Hooda,Nishith Shankar,Julia Tan,Kevin Zhu,Ashwinee Panda,Ryan Lagasse,Vasu Sharma*

Main category: cs.AI

TL;DR: 大型语言模型存在评估意识问题，即模型能在评估和部署环境中表现出不同行为，这威胁AI安全评估。研究发现评估意识随模型规模呈幂律增长，可用于预测更大模型的欺骗行为。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在评估和部署环境中表现出不同的行为（评估意识），这可能使模型在测试时隐藏危险能力，从而破坏AI安全评估的有效性。之前的研究仅在单个70B模型上证明了这一现象，但不同规模模型间的评估意识变化规律尚不清楚。

Method: 研究使用线性探测方法分析15个不同规模模型（从0.27B到70B参数）的转向向量激活，这些模型来自四个不同的模型家族，通过系统性的规模扩展实验来探究评估意识的缩放规律。

Result: 研究发现评估意识与模型规模之间存在清晰的幂律缩放关系：模型规模越大，评估意识越强。这种可预测的缩放规律使得能够对未来更大模型的欺骗行为进行预测。

Conclusion: 评估意识的幂律缩放规律为设计针对不同规模模型的AI安全评估策略提供了指导，有助于开发能够应对未来更大模型欺骗行为的评估方法，确保AI安全评估的有效性。
...
### [129] [FRIT: Using Causal Importance to Improve Chain-of-Thought Faithfulness](https://arxiv.org/abs/2509.13334)
...
TL;DR: FRIT是一种通过干预训练提升思维链推理因果一致性的对齐方法，通过生成忠实/不忠实推理对来训练模型偏好因果一致的推理路径，在多个任务上显著提升了推理忠实性和准确性。


<details>
  <summary>Details</summary>
Motivation: 现有思维链推理方法存在推理步骤与最终答案缺乏因果关联的问题，导致推理脆弱且不可信。虽然已有方法主要关注测量忠实性，但系统性提升忠实性的方法仍然有限。

Method: 提出FRIT方法：1）通过对模型生成的思维链推理步骤进行干预，生成系统性破坏的示例，创建忠实/不忠实推理对；2）应用直接偏好优化训练模型偏好因果一致的推理路径。

Result: 在Qwen3-8B和Mistral-7B-v0.1模型上的实验表明，FRIT在GSM8K任务上使Mistral的忠实推理提升3.4个百分点，准确性提升7.6个百分点。
...
### [130] [Position: AI Safety Must Embrace an Antifragile Perspective](https://arxiv.org/abs/2509.13339)
*Ming Jin,Hyunin Lee*
...
<details>
  <summary>Details</summary>
Motivation: 传统静态基准测试和一次性鲁棒性测试无法应对环境演变和模型漂移问题（如奖励黑客攻击、过度优化等），需要新的方法来保证AI系统的长期安全可靠性。

Method: 提出抗脆弱性方法，利用当前不确定性来更好地准备应对未来更大、更不可预测的不确定性，包括重新校准AI安全的测量、基准测试和持续改进方法。

Result: 识别了静态测试的关键局限性（场景多样性不足、奖励黑客攻击、过度对齐等），并探索了抗脆弱性解决方案管理罕见事件的潜力。

Conclusion: 抗脆弱性方法对于开放端机器学习系统的长期可靠性至关重要，需要建立相应的伦理和实践指南来培育抗脆弱性AI安全社区。
...
### [131] [Imagined Autocurricula](https://arxiv.org/abs/2509.13341)
...
TL;DR: IMAC方法利用世界模型生成想象环境，通过无监督环境设计自动课程训练智能体，在有限数据下实现对新任务的强泛化能力


<details>
  <summary>Details</summary>
Motivation: 解决在真实世界中训练智能体时缺乏大量训练数据和准确仿真的问题，利用离线被动收集的数据构建世界模型来生成多样化训练环境

Method: 提出IMAC（Imagined Autocurricula）方法，结合无监督环境设计（UED）在世界模型生成的想象环境中自动构建课程，训练鲁棒智能体
...
### [132] [OpenHA: A Series of Open-Source Hierarchical Agentic Models in Minecraft](https://arxiv.org/abs/2509.13347)
...
<details>
  <summary>Details</summary>
Motivation: 解决动作空间选择这一关键但未解决的挑战，发现现有抽象动作空间没有普遍最优解，需要为构建通用智能体提供新的解决方案。

Method: 提出Chain of Action (CoA)框架，将抽象动作视为中间推理步骤而非单独策略的命令，在单一VLA模型中统一高层规划和底层控制，并使用多样化动作空间混合训练。

Result: CoA框架训练的All-in-One智能体实现了新的state-of-the-art，相比专门的基线模型显著提高了整体任务成功率，学习到了更鲁棒和可泛化的策略。

Conclusion: CoA框架成功解决了动作空间选择的困境，通过统一规划和控制的端到端方法实现了更好的性能，同时发布了OpenHA套件促进可复现研究。
...
### [133] [Teaching LLMs to Plan: Logical Chain-of-Thought Instruction Tuning for Symbolic Planning](https://arxiv.org/abs/2509.13351)
...
Method: 开发指令调优框架，通过逻辑思维链推理指导模型进行动作适用性、状态转换和计划有效性的严格推理，包含前提条件满足、效果应用和不变性保持的显式推理链
...
### [134] [Agentic UAVs: LLM-Driven Autonomy with Integrated Tool-Calling and Cognitive Reasoning](https://arxiv.org/abs/2509.13352)
*Anis Koubaa,Khaled Gabr*

Main category: cs.AI

TL;DR: 提出了Agentic UAVs框架，通过五层架构整合LLM驱动推理，在搜救模拟中显著提升了检测置信度、人员检测率和行动推荐能力


<details>
  <summary>Details</summary>
Motivation: 现有无人机系统主要依赖基于规则的控制和窄AI，缺乏情境感知推理、自主决策和生态系统集成能力，无法在动态不确定任务中有效适应

Method: 提出五层架构（感知、推理、行动、集成、学习），整合ROS2和Gazebo原型，结合YOLOv11目标检测与GPT-4推理，部署本地Gemma-3模型

Result: 在模拟搜救场景中，检测置信度从0.72提升至0.79，人员检测率从75%提升至91%，行动推荐率从4.5%大幅提升至92%

Conclusion: 适度的计算开销能够实现质的自主性提升和生态系统集成，为无人机系统带来新的自主水平
...
### [135] [Semantic Fusion with Fuzzy-Membership Features for Controllable Language Modelling](https://arxiv.org/abs/2509.13357)
...
<details>
  <summary>Details</summary>
Motivation: 为了增强语言模型的语义理解能力，提供可解释的语义特征表示，并实现用户可控的文本生成，同时保持模型简洁性和兼容性

Method: 使用并行模糊成员特征通道编码token级语义特征（如词性、浅层角色、边界标志、情感极性等），通过门控适配器将语义矩阵融合到语言模型中，采用标准的下一个token预测、语义特征重构辅助损失和形容词分布正则化

Result: 在合成双子句语料库上，语义融合提高了困惑度，实现了精确的用户可控极性和标点生成，同时保持模型简洁性

Conclusion: 语义融合是一种轻量级方法，为条件自然语言生成提供了可解释的途径，仅增加少量开销且完全兼容输入输出嵌入
...
### [136] [Asterisk Operator](https://arxiv.org/abs/2509.13364)
*Zixi Li*

Main category: cs.AI

TL;DR: 提出星号操作符(*-operator)统一框架，通过邻接结构并行传播实现抽象推理，在ARC2挑战和康威生命游戏中验证了其通用性和收敛性，仅用600万参数就达到100%准确率。


<details>
  <summary>Details</summary>
Motivation: 为了解决抽象推理任务中的结构化推理问题，需要一种能够保持局部计算约束同时实现全局推理能力的统一框架。

Method: 基于邻接结构并行传播(ASPP)的星号操作符，将结构化推理任务形式化为局部并行状态演化过程，通过隐式关系图指导推理。

Result: 在ARC2验证集上达到100%准确率，仅需600万参数，证明了操作符的通用性、收敛性和优越性能。

Conclusion: 星号操作符为神经符号推理提供了高效收敛的计算范式，是抽象推理领域的重大突破。
...
### [137] [$Agent^2$: An Agent-Generates-Agent Framework for Reinforcement Learning Automation](https://arxiv.org/abs/2509.13368)
*Yuan Wei,Xiaohan Shan,Ran Miao,Jianmin Li*

Main category: cs.AI

TL;DR: Agent²是一个完全自动化的RL智能体生成框架，通过LLM驱动将自然语言任务描述和环境代码转换为高性能强化学习解决方案，无需人工干预


<details>
  <summary>Details</summary>
Motivation: 传统强化学习智能体开发需要大量专业知识和迭代周期，失败率高且可访问性有限，需要实现完全自动化的智能体设计
...
### [138] [The Art of Saying "Maybe": A Conformal Lens for Uncertainty Benchmarking in VLMs](https://arxiv.org/abs/2509.13379)
*Asif Azad,Mohammad Sadat Hossain,MD Sadik Hossain Shanto,M Saifur Rahman,Md Rizwan Pervez*

Main category: cs.AI

TL;DR: 本文对16个最先进的视觉语言模型进行了全面的不确定性基准测试，发现在多模态系统中，更大的模型具有更好的不确定性量化能力，数学和推理任务的不确定性表现较差。


<details>
  <summary>Details</summary>
Motivation: 虽然视觉语言模型在复杂视觉理解方面取得了显著进展，但不确定性量化这一关键维度尚未得到足够关注，需要建立一个可靠的不确定性评估基础。

Method: 在6个多模态数据集上评估16个最先进的VLMs（开源和闭源），使用3种不同的评分函数进行全面的不确定性基准测试研究。

Result: 研究发现：1）更大的模型始终表现出更好的不确定性量化；2）更确定的模型获得更高的准确性；3）数学和推理任务在所有模型中相比其他领域表现出较差的不确定性性能。
...
### [139] [From Next Token Prediction to (STRIPS) World Models -- Preliminary Results](https://arxiv.org/abs/2509.13389)
...
<details>
  <summary>Details</summary>
Motivation: 从纯动作轨迹中学习世界模型，无需手动编码领域知识，使用深度学习自动推断动作的前提条件和效果

Method: 将任务建模为监督式下一个令牌预测问题，使用Transformer架构处理动作序列，通过正负样本（有效和无效动作序列）训练模型

Result: 实验表明合适的Transformer架构能够准确表示命题STRIPS世界模型，并且仅从随机有效和无效动作序列中就能成功学习

Conclusion: 深度学习方法特别是Transformer架构能够有效学习符号世界模型，为从观察数据中自动获取领域知识提供了可行途径
...
### [140] [SteeringControl: Holistic Evaluation of Alignment Steering in LLMs](https://arxiv.org/abs/2509.13450)
...
<details>
  <summary>Details</summary>
Motivation: 现有的对齐工作往往只关注真实性或推理能力来展示表示引导的副作用，但许多权衡关系尚未被系统性地理解。需要建立一个全面的评估框架来系统分析不同引导方法的效果和行为纠缠问题。

Method: 构建了一个模块化的引导框架，基于独特组件作为现有方法的构建块。收集了安全相关的主要和次要行为数据集，围绕五种流行引导方法进行评估。在Qwen-2.5-7B和Llama-3.1-8B模型上进行实验。

Result: 发现强引导性能取决于引导方法、模型和目标行为的特定组合。不良的三者组合可能导致严重的概念纠缠问题。不同组合的效果差异显著。

Conclusion: 表示引导方法的效果具有高度情境依赖性，需要仔细选择方法、模型和目标的组合。该基准为系统评估引导方法提供了重要工具，揭示了行为纠缠的复杂性。
...
### [141] [AI Agents with Human-Like Collaborative Tools: Adaptive Strategies for Enhanced Problem-Solving](https://arxiv.org/abs/2509.13547)
*Harper Reed,Michael Sugimura,Angelo Zangari*

Main category: cs.AI

TL;DR: 为LLM智能体提供类似人类的协作工具和自主权可以显著提升其在最困难编程问题上的性能表现，成本降低15-40%，回合数减少12-27%，完成速度提高12-38%。


<details>
  <summary>Details</summary>
Motivation: 研究是否通过赋予LLM智能体人类自然使用的协作工具和自主权，能够改善它们在问题解决中的表现。

Method: 为Claude Code智能体配备基于MCP的社交媒体和日志记录工具，让它们自主决定如何使用这些工具来解决34个Aider多语言Python编程挑战。

Result: 协作工具在最具挑战性的问题上显著提升性能，不同模型自然采用不同的协作策略，智能体表现出偏好写作而非阅读（2-9倍），结构化表达是改进的主要驱动力。

Conclusion: AI智能体可以在其能力边界处系统性地受益于受人类启发的协作工具，这表明自适应协作界面可以作为推理增强器而非通用的效率提升工具。
...
### [142] [Gen AI in Proof-based Math Courses: A Pilot Study](https://arxiv.org/abs/2509.13570)
*Hannah Klawa,Shraddha Rajpal,Cigole Thomas*

Main category: cs.AI

TL;DR: 本研究调查了本科生在证明数学课程中使用生成式AI的情况，分析了学生的使用方式和看法，并探讨了AI整合到数学教学中的未来考量。


<details>
  <summary>Details</summary>
Motivation: 随着生成式AI在高等教育中的快速兴起和当前AI检测工具不可靠，制定鼓励学生学习和批判性思维的政策变得日益重要。

Method: 通过调查问卷和学生访谈，分析三个证明数学课程（抽象代数、拓扑学）中学生使用AI工具的方式、对AI有用性和局限性的看法。

Result: 研究发现学生在允许使用AI的课程政策下如何与AI工具互动，以及他们对生成式AI在数学证明中作用的认知。

Conclusion: 研究讨论了将生成式AI整合到证明数学教学中的未来考虑因素，强调需要制定促进学习和批判性思维的政策。
...
### [143] [Programmable Cognitive Bias in Social Agents](https://arxiv.org/abs/2509.13588)
...
<details>
  <summary>Details</summary>
Motivation: 传统使用自然语言隐式描述智能体行为的方法无法在不同模型间产生一致行为，且无法捕捉描述的细微差别，需要更系统化的行为规范方法。

Method: CoBRA包含两个组件：认知偏见指数（通过经典社会科学实验量化智能体反应）和行为调节引擎（将智能体行为与受控认知偏见对齐），基于经典社会科学实验显式编程认知偏见。

Result: 评估显示CoBRA能够以模型无关的方式精确编程社交智能体中展示的认知偏见，在演示和技术基准测试中表现良好。

Conclusion: CoBRA提供了一种有效的方法来系统化指定和控制在LLM-based社会模拟中的智能体行为，解决了传统方法的局限性。
...
### [144] [See, Think, Act: Teaching Multimodal Agents to Effectively Interact with GUI by Identifying Toggles](https://arxiv.org/abs/2509.13615)
*Zongru Wu,Rui Mao,Zhiyuan Tian,Pengzhou Cheng,Tianjie Ju,Zheng Wu,Lingzhong Dong,Haiyue Sheng,Zhuosheng Zhang,Gongshen Liu*

Main category: cs.AI

TL;DR: 本文提出了State-aware Reasoning (StaR)训练方法，解决多模态代理在GUI切换控制指令执行不可靠的问题，特别是在当前状态与期望状态一致时的错误执行。


<details>
  <summary>Details</summary>
Motivation: 现有的多模态代理在图形用户界面(GUI)控制中无法可靠执行切换控制指令，特别是在当前切换状态已经符合期望状态时，这成为关键瓶颈。

Method: 构建了基于公共数据集的二进制切换指令状态控制基准，并提出StaR训练方法，教导代理感知当前切换状态、分析指令中的期望状态，并相应采取行动。

Result: 在三个多模态代理上的实验表明，StaR可以将切换指令执行准确率提高30%以上。在三个公共基准测试上的进一步评估显示，StaR还能提升一般任务性能。动态环境评估突显了StaR在实际应用中的潜力。
...
### [145] [InfraMind: A Novel Exploration-based GUI Agentic Framework for Mission-critical Industrial Management](https://arxiv.org/abs/2509.13704)
*Liangtao Lin,Zhaomeng Zhu,Tianwei Zhang,Yonggang Wen*

Main category: cs.AI

TL;DR: InfraMind是一个专为工业管理系统设计的GUI代理框架，通过系统探索、内存驱动规划、状态识别、知识蒸馏和多层安全机制，解决了LLM-based GUI代理在工业管理中的五大挑战，显著提升了任务成功率和操作效率。


<details>
  <summary>Details</summary>
Motivation: 工业基础设施管理面临系统复杂性增加、多供应商集成和专家操作员短缺等挑战。现有RPA方法灵活性有限且维护成本高，而通用LLM-based GUI代理在工业管理中存在元素理解、精度效率、状态定位、部署约束和安全要求等五大问题。

Method: 提出InfraMind框架，包含五个创新模块：(1)基于系统搜索的探索和虚拟机快照用于自主理解复杂GUI；(2)内存驱动规划确保高精度高效任务执行；(3)高级状态识别用于层次化界面的鲁棒定位；(4)结构化知识蒸馏实现轻量级模型高效部署；(5)多层安全机制保护敏感操作。
...
### [146] [THOR: Tool-Integrated Hierarchical Optimization via RL for Mathematical Reasoning](https://arxiv.org/abs/2509.13761)
*Qikai Chang,Zhenrong Zhang,Pengfei Hu,Jiefeng Ma,Yicheng Pan,Jianshu Zhang,Jun Du,Quan Liu,Jianqing Gao*

Main category: cs.AI

TL;DR: THOR是一个通过强化学习实现工具集成层次优化的框架，用于提升大语言模型在数学推理和代码生成任务中的性能，通过多智能体数据生成、分层优化和自校正机制实现最先进效果。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在数学推理方面取得显著进展，但在高精度任务（如数值计算和符号操作）上仍存在困难。现有工具集成方法面临三个关键挑战：构建工具集成推理数据、执行细粒度优化和增强推理能力。

Method: 提出THOR框架：1) TIRGen多智能体actor-critic流水线构建高质量工具集成推理数据集；2) 分层强化学习策略联合优化轨迹级问题解决和步骤级代码生成；3) 自校正机制利用工具反馈动态修正推理错误。

Result: 该方法在多个数学基准测试中达到同类规模模型的最先进性能，在代码基准测试上也实现了一致性改进，并展现出对不同模型的强泛化能力。

Conclusion: THOR通过工具集成和分层优化有效解决了大语言模型在高精度数学任务中的局限性，为提升模型推理能力提供了有效解决方案。
...
### [147] [MIRA: Empowering One-Touch AI Services on Smartphones with MLLM-based Instruction Recommendation](https://arxiv.org/abs/2509.13773)
*Zhipeng Bian,Jieming Zhu,Xuyang Xie,Quanyu Dai,Zhou Zhao,Zhenhua Dong*

Main category: cs.AI

TL;DR: MIRA是一个智能手机AI任务指令推荐框架，通过长按图像或文本来提供上下文相关的AI任务指令建议，使用多模态大语言模型和结构化推理来提高推荐准确性。


<details>
  <summary>Details</summary>
Motivation: 随着生成式AI技术的快速发展，各种AI服务被集成到智能手机中，但用户访问这些预定义AI服务的方式需要更加简化和直观。

Method: 1) 基于多模态大语言模型的推荐流水线，进行结构化推理以提取关键实体和推断用户意图；2) 模板增强推理机制，整合高级推理模板提高任务推断准确性；3) 基于前缀树的约束解码策略，限制输出为预定义指令候选，确保建议的一致性和意图对齐。
...
### [148] [An Exhaustive DPLL Approach to Model Counting over Integer Linear Constraints with Simplification Techniques](https://arxiv.org/abs/2509.13880)
*Mingwei Zhang,Zhenhao Gu,Liangda Fang,Cunjing Ge,Ziliang Chen,Zhao-Rong Lai,Quanlong Guan*

Main category: cs.AI

TL;DR: 本文提出了一种基于DPLL架构的精确方法来解决整数线性约束的模型计数问题，通过集成混合整数规划中的简化技术显著提升了效率，在随机基准测试和应用实例上都优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 整数线性约束是计算机科学、运筹学和优化领域的基础约束，许多应用问题可归结为整数线性约束的模型计数问题(MCILC)，需要高效的精确求解方法。

Method: 基于穷举DPLL架构设计精确方法，集成混合整数规划中的有效简化技术来提升效率。

Result: 在2840个随机基准测试中解决了1718个实例（现有最佳方法仅解决1470个），在4131个应用实例中是唯一能解决所有实例的方法。

Conclusion: 该方法在整数线性约束模型计数问题上显著优于现有精确方法，特别是在应用实例上表现出色，是唯一能完全解决所有测试实例的方法。
...
### [149] [Exploring Major Transitions in the Evolution of Biological Cognition With Artificial Neural Networks](https://arxiv.org/abs/2509.13968)
*Konstantinos Voudouris,Andrew Barron,Marta Halina,Colin Klein,Matishalin Patel*

Main category: cs.AI

TL;DR: 该研究使用人工神经网络模型探讨信息流结构变化是否能带来认知性能的过渡性变化，发现循环网络相比前馈网络在处理复杂语法时表现出质的性能提升，并观察到训练难度形成的过渡障碍。


<details>
  <summary>Details</summary>
Motivation: 探索认知进化是否通过一系列主要转变来实现，这些转变操纵生物神经网络的结构，从根本上改变信息流动方式。

Method: 使用理想化的信息流模型和人工神经网络，比较前馈、循环和分层拓扑结构，测试它们在学习不同复杂度人工语法时的性能，控制网络大小和资源。

Result: 循环网络相比前馈网络能够处理更多类型的输入，在最复杂语法学习上表现出质的性能提升；循环网络的训练难度形成了过渡障碍和偶然不可逆性；分层网络在语法学习上没有表现出优势。

Conclusion: 某些信息流结构的变化确实能够带来认知性能的过渡性变化，支持认知进化可能通过主要转变实现的假设。
...
### [150] [CrowdAgent: Multi-Agent Managed Multi-Source Annotation System](https://arxiv.org/abs/2509.14030)
...
Method: 采用多智能体系统架构，实现任务分配、数据标注和质量管理的端到端流程控制，通过理性任务分配使不同标注源在协作标注工作流中协同推进。
...
### [151] [Hierarchical Learning for Maze Navigation: Emergence of Mental Representations via Second-Order Learning](https://arxiv.org/abs/2509.14195)
*Shalima Binta Manir,Tim Oates*

Main category: cs.AI

TL;DR: 本文通过层次架构验证了二阶学习促进环境-认知同构性形成的假设，使用GCN作为一阶学习器和MLP控制器作为二阶学习器，在迷宫导航任务中证明了结构化心理表征对二阶学习效果的关键作用。


<details>
  <summary>Details</summary>
Motivation: 探索二阶学习（适应一阶学习的学习机制）是否能够促进结构化内部心理模型的形成，这种模型与外部环境同构，是高级认知的基础但难以实证研究。

Method: 提出分层架构：图卷积网络（GCN）作为一阶学习器直接映射节点特征到最优导航路径预测，MLP控制器作为二阶学习器在遇到结构新颖的迷宫环境时动态调整GCN参数。
