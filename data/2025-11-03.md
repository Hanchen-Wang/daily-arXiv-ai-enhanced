...
Motivation: 人类读取测量仪器相对容易且不需要太多专业知识，但当前视觉语言模型在这方面表现不佳，需要专门的基准测试来评估和改进这方面的能力。

Method: 开发了MeasureBench基准测试，包含真实世界和合成图像，并设计了可扩展的数据合成流程，能够程序化生成各种类型的测量仪器，控制视觉外观的关键细节。

Result: 评估显示即使是前沿的视觉语言模型在测量读取方面也普遍存在困难，主要失败模式是指针定位错误，模型能识别数字但无法准确定位指针位置。
...
TL;DR: 提出了一个用于多机构定量CT的领域自适应transformer分割框架，通过对抗对齐和统计对齐解决跨机构数据分布差异问题


<details>
  <summary>Details</summary>
Motivation: 深度学习模型在不同机构间的定量CT数据上存在领域偏移问题，导致分割不稳定和定量指标不可靠，这阻碍了多中心骨质疏松研究和跨站点结果的可重复性

Method: 基于3D TransUNet骨干网络，集成梯度反转层的对抗对齐和最大均值差异的统计对齐两种互补策略，平衡不变性和细粒度对齐

Result: 在包含1024张Tulane大学和384张Rochester扫描的大型髋部骨折研究队列上进行训练和验证

Conclusion: 该双机制方法实现了扫描仪无关的特征学习，同时保留了解剖细节
...
<details>
  <summary>Details</summary>
Motivation: 传统自适应密度控制方法仅基于位置梯度大小进行基元分裂，忽略了梯度的方向信息，导致冗余分裂和次优的分裂位置选择。

Method: 引入梯度方向一致性概念，通过梯度角度相干性来指导基元分裂决策，并在需要分裂时利用方向一致性确定最优分裂位置，使子基元更好地对齐局部结构。

Result: 相比现有方法，DC4GS显著减少了基元数量（实验中最高达30%），同时大幅提升了重建保真度。

Conclusion: 方向一致性驱动的自适应密度控制能更有效地捕捉局部结构复杂性，避免冗余分裂，实现更紧凑和高质量的重建结果。
...
TL;DR: 提出了一种尺度自适应课程学习(SACL)方法，通过动态调整课程设计来应对医疗影像数据稀缺场景，在有限标注数据下显著提升肺结节检测性能
...
Result: 在LUNA25数据集上，SACL在完整数据集上达到与静态课程学习相当的mAP50性能，但在数据受限条件下显著优于基线，在10%、20%和50%训练数据下分别提升4.6%、3.5%和2.0%
...
TL;DR: 提出统一多流SYNAPSE-Net框架，用于多模态MRI中异质性脑病变的自动分割，在三个挑战数据集上实现最先进性能。
...
Result: 在WMH数据集DSC达0.831，HD95为3.03；ISLES 2022边界精度最佳；BraTS 2020肿瘤核心区域DSC最高0.8651。

Conclusion: 该统一自适应框架在多种脑病变分割中实现最先进性能，为自动分割提供了稳健且临床可行的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法忽视了优先处理与观众互动最相关的视频帧，这对于生成上下文适当的评论至关重要。直播视频包含大量数据和无关内容，需要有效筛选关键信息。

Method: SFAT模型利用CLIP的视觉-文本多模态知识，根据语义相关性为视频帧分配权重，采用加权帧求和技术强调信息丰富的帧，并使用带交叉注意力机制的评论解码器整合聊天和视频上下文。

Result: 构建了大规模多模态英语视频评论数据集，涵盖11个视频类别、438小时内容和320万条评论。与现有方法相比，SFAT模型在生成直播视频评论方面表现出更好的效果。

Conclusion: SFAT模型通过语义帧聚合和跨模态注意力机制，能够有效生成上下文相关的直播视频评论，解决了现有方法在帧优先级处理和数据集多样性方面的局限性。
...
<details>
  <summary>Details</summary>
Motivation: 探索将在大语言模型中广泛成功的混合专家范式应用于医学视觉语言任务，利用文本信息增强医学图像分析性能。

Method: 采用混合专家架构，动态选择专家，有效利用多尺度视觉特征并结合文本嵌入，针对医学图像的复杂性进行优化。

Result: 在包含3,410个CT扫描的10个数据集上表现出色，在综合医学图像分割基准测试中达到竞争性精度。

Conclusion: MoME为医学图像分析提供了一种新颖的架构，展示了基础模型在医学成像中的集成潜力，通过MoE范式有效提升了模型性能。
...
TL;DR: 提出了一种无示例的增量关系蒸馏(IRD)框架，用于解决开放世界中增量HOI检测的挑战，包括灾难性遗忘、交互漂移和零样本HOI组合检测。


<details>
  <summary>Details</summary>
Motivation: 在开放世界中，人-物交互(HOI)持续演化，传统封闭世界HOI检测模型难以应对。受人类渐进获取知识能力的启发，探索增量HOI检测(IHOID)来开发能够在动态环境中识别人-物关系的智能体。

Method: 提出IRD框架，将物体和关系学习解耦，引入两种独特的蒸馏损失来学习跨不同HOI组合但共享相同关系的不变关系特征。

Result: 在HICO-DET和V-COCO数据集上的广泛实验表明，该方法在缓解遗忘、增强对交互漂移的鲁棒性以及零样本HOI泛化方面优于最先进的基线方法。

Conclusion: IRD框架有效解决了增量HOI检测中的关键挑战，为开放世界环境中的动态人-物交互识别提供了可行方案。
...
TL;DR: VitalLens 2.0是一个新的深度学习模型，通过面部视频估计生理信号，在远程光电容积描记术(rPPG)方面实现了准确性的显著提升，能够稳健估计心率、呼吸率和心率变异性指标。


<details>
  <summary>Details</summary>
Motivation: 开发一个更准确的远程生理信号监测模型，通过面部视频实现非接触式的心率、呼吸率和心率变异性测量，为健康监测提供便利。

Method: 采用新的模型架构，并大幅增加了训练数据的规模和多样性，训练数据包含1,413个独特个体。在422个独特个体的组合测试集上进行评估。

Result: 在个体平均结果中，VitalLens 2.0实现了：心率MAE为1.57 bpm，呼吸率MAE为1.08 bpm，HRV-SDNN MAE为10.18 ms，HRV-RMSSD MAE为16.45 ms。这些结果代表了新的最先进水平，显著优于先前方法。

Conclusion: VitalLens 2.0在远程生理信号估计方面取得了重大突破，提供了高精度的非接触式健康监测解决方案，现已通过VitalLens API向开发者开放。
...
TL;DR: AD-SAM是基于SAM的自动驾驶语义分割模型，通过双编码器和可变形解码器提升道路场景分割精度，在Cityscapes和BDD100K数据集上显著优于SAM、G-SAM和DeepLabV3。


<details>
  <summary>Details</summary>
Motivation: 针对自动驾驶场景的空间和几何复杂性，需要改进基础模型SAM以更好地处理道路场景的语义分割任务。

Method: 使用双编码器结合SAM的ViT-H全局语义和ResNet-50局部空间细节，通过可变形融合模块对齐多尺度特征，采用可变形注意力进行多阶段精炼，使用混合损失函数优化训练。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法通常约束在固定分层结构（如二叉树），无法灵活地从数据中推断更复杂的分层关系，限制了跨类别通用层次结构的建模能力。

Method: 使用分层变压器（HiT），每层通过压缩码本学习树层次结构的父子关系，码本自动识别不同形状类别间的共同子结构，不限制具体分层结构类型。

Result: 在55个ShapeNet类别上进行无监督形状分割任务，成功将形状分割为多个粒度级别，捕获了有意义的父子节点包含关系。

Conclusion: HiT能够从数据中直接推断分层结构，表示比先前方法更通用和复杂的分层关系，在跨类别形状分析中表现出色。
...
TL;DR: ZEBRA是首个零样本脑视觉解码框架，通过对抗训练分离fMRI表征中的主体相关和语义相关成分，无需主体特定适应即可实现跨主体泛化。


<details>
  <summary>Details</summary>
Motivation: 当前fMRI到图像重建方法主要依赖主体特定模型或微调，限制了可扩展性和实际应用。

Method: 利用对抗训练显式解耦fMRI表征中的主体相关和语义相关成分，分离出主体不变、语义特定的表征。

Result: ZEBRA在多个指标上显著优于零样本基线，性能与完全微调模型相当。
...
TL;DR: 该论文提出了WildfireX-SLAM数据集，这是一个用于森林环境中SLAM任务的大规模合成数据集，特别针对野火应急响应和森林管理应用。


<details>
  <summary>Details</summary>
Motivation: 现有的3DGS-based SLAM方法主要关注小规模室内场景，而针对大规模森林场景的SLAM研究因缺乏高质量数据集而受阻，特别是在野火应急响应和森林管理领域。

Method: 利用Unreal Engine 5 Electric Dreams环境样本项目，开发了一个数据采集流程，能够收集空中和地面视图，包括真实相机位姿和多种数据模态，并支持环境因素（光照、天气、野火类型等）的灵活控制。

Result: 创建了包含5,500张低空RGB-D航空图像的大规模数据集，覆盖16平方公里的森林区域，并进行了全面的基准测试，揭示了森林环境中3DGS-based SLAM的独特挑战。

Conclusion: WildfireX-SLAM数据集填补了森林环境中SLAM研究的空白，为未来工作提供了改进方向，数据集和代码将公开可用。
...
TL;DR: 提出了E-MMDiT，一个仅304M参数的高效轻量级多模态扩散模型，通过token压缩、位置增强和交替子区域注意力等技术，在低训练资源下实现快速图像生成。


<details>
  <summary>Details</summary>
Motivation: 现有扩散模型通常需要大规模训练数据和大量计算资源，或者结构复杂延迟高，需要开发更高效轻量的模型来降低生成AI的门槛。

Method: 采用高度压缩的视觉分词器生成紧凑表示，提出多路径压缩模块进一步压缩token，引入位置增强保持空间一致性，使用交替子区域注意力降低计算成本，并提出AdaLN-affine轻量模块。
...
TL;DR: 提出CVCAM和MHSAM模块，通过跨视图交互和多尺度空间注意力提升跨视图目标地理定位性能，并创建G2D数据集填补地面到无人机定位任务空白
...
Method: 引入跨视图交叉注意力模块(CVCAM)进行多轮视图间交互，集成多头空间注意力模块(MHSAM)提取多尺度空间特征，创建G2D数据集

Result: 在CVOGL和G2D数据集上的大量实验表明，所提方法实现了高定位精度，超越了当前最先进方法

Conclusion: 提出的CVCAM和MHSAM模块有效提升了跨视图目标地理定位性能，G2D数据集填补了重要空白，方法具有优越性能
...
TL;DR: 提出HiGS分层生成框架，通过多步关联语义空间组合实现3D场景生成，从全局到局部逐步扩展场景，让用户选择关键语义对象进行精细控制，模型自动完成周边区域。


<details>
  <summary>Details</summary>
Motivation: 现有方法大多采用单步生成过程，难以平衡场景复杂度和用户输入量。受人类场景建模认知过程启发，从全局到局部、关注关键元素、通过语义关联完成场景。

Method: 引入渐进式分层空间-语义图(PHiSSG)，动态组织空间关系和语义依赖，保持图节点与生成对象的一对一映射，支持递归布局优化。
...
TL;DR: AFM-Net是一个用于遥感图像场景分类的先进分层融合框架，通过CNN分支提取层次视觉先验，Mamba分支进行高效全局序列建模，实现了局部和全局特征的协同表示，在多个数据集上超越了现有方法。


<details>
  <summary>Details</summary>
Motivation: 遥感图像场景分类面临复杂空间结构和多尺度特征的挑战。现有方法中CNN擅长建模局部纹理，Transformer擅长捕捉全局上下文，但Transformer的高计算成本限制了它们的有效集成。

Method: 提出AFM-Net框架，包含CNN分支和Mamba分支两条路径，通过分层融合机制逐步聚合多尺度特征，实现动态跨层级特征交互和上下文重建，最后通过专家混合分类器模块进行自适应路由。

Result: 在AID、NWPU-RESISC45和UC Merced数据集上分别达到93.72%、95.54%和96.92%的准确率，超越了现有最先进方法，在性能和效率之间取得了平衡。

Conclusion: AFM-Net通过有效整合CNN和Mamba的优势，实现了遥感图像场景分类的高性能，证明了分层融合机制和专家混合分类器的有效性。
...
Result: 研究发现了部分成功和关键失败模式，包括结构遗漏、幻觉和检测模糊性，即使最终评分与专家注释匹配，中间表示的不一致性也常削弱可解释性。
...
Method: 多阶段流程：先用VLM生成初始描述，LLM识别关键物体并预测相关物体，物体检测系统验证预测，对未提及的新检测物体进行区域特定描述，移除未检测物体的引用。

Result: 在精选的高分辨率图像数据集上实验表明，该流程能生成更详细可靠的图像描述，同时有效减少幻觉。

Conclusion: 提出的多模型集成流程显著提升了高分辨率图像描述的准确性和细节丰富度，成功解决了现有VLM在描述高分辨率图像时的局限性。
...
<details>
  <summary>Details</summary>
Motivation: 现有的相机-雷达融合方法大多局限于单帧输入，只能捕捉场景的部分信息。不完整的场景信息加上图像质量下降和4D雷达稀疏性，阻碍了整体检测性能。多帧融合虽然能提供更丰富的时空信息，但面临跨帧跨模态特征融合和计算成本高的挑战。

Method: 提出M^3Detection框架，利用基线检测器的中间特征和跟踪器生成的参考轨迹，提高计算效率并为第二阶段提供更丰富信息。设计了全局级对象间特征聚合模块和局部级网格间特征聚合模块，通过轨迹级多帧时空推理模块编码跨帧交互。

Result: 在VoD和TJ4DRadSet数据集上的大量实验表明，M^3Detection实现了最先进的3D检测性能，验证了其在相机-4D成像雷达多帧检测中的有效性。

Conclusion: 该框架通过多级特征融合和时空推理，有效解决了多帧相机-雷达融合中的挑战，为恶劣天气下的鲁棒3D感知提供了成本效益高的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 扩散模型在视觉生成任务中表现出色，但视频生成特别是涉及人类动作的舞蹈生成面临质量要求和连续性挑战，需要专门的方法来处理人类动作的高度自由度。

Method: 基于稳定视频扩散模型，引入外观增强模块(AEM)关注参考图像细节，姿态渲染模块(PRM)扩展运动引导以捕捉额外领域的姿态条件，并构建TikTok-3K数据集增强训练。

Result: 在真实世界数据集上的广泛实验表明，该模型性能优于现有最先进方法。

Conclusion: DANCER框架通过条件增强和渲染机制有效解决了人类舞蹈视频生成的挑战，在舞蹈合成任务上取得了优异表现。
...
TL;DR: H2-Cache是一种用于扩散模型的分层缓存机制，通过双阈值系统分别处理结构定义和细节精炼阶段，在保持图像质量的同时实现高达5.08倍的加速。


<details>
  <summary>Details</summary>
Motivation: 扩散模型在图像生成方面表现出色，但其迭代去噪过程计算成本高昂。现有缓存技术面临速度与保真度之间的权衡问题，存在质量下降和计算开销大的局限性。

Method: 提出H2-Cache分层缓存机制，基于去噪过程可分离为结构定义阶段和细节精炼阶段的洞察，采用双阈值系统选择性缓存每个阶段，并引入池化特征摘要(PFS)技术进行轻量级相似性估计。
...
TL;DR: SilhouetteTell是一种新型视频识别攻击方法，通过分析字幕轮廓的空间和时间特征来识别用户观看的视频内容，对在线和离线视频都有效，攻击距离可达40米。
...
Method: 通过观察字幕内容决定其在屏幕上显示的轮廓形状，识别每个字幕轮廓并计算连续字幕之间的时间差，将空间和时间域信息结合成字幕轮廓的时空特征，探索录制视频字幕轮廓与其字幕文件之间的时空相关性。

Result: 在现成智能手机上的综合实验证实了SilhouetteTell在各种设置下推断视频标题和片段的极高有效性，包括从40米距离进行攻击。
...
TL;DR: 提出DPHR策略解决无人机-卫星跨视角地理定位中的视角差异和困难负样本问题，通过双层级渐进式难度感知重加权方法提升性能
...
Method: 双层级渐进式难度感知重加权(DPHR)：样本级使用基于比率的难度感知模块评估相对难度并分配细粒度权重；批次级使用渐进式自适应损失加权机制利用训练进度信号衰减早期优化中的噪声梯度，并随着训练成熟逐步增强困难负样本挖掘

Result: 在University-1652和SUES-200基准测试中证明了DPHR的有效性和鲁棒性，相比最先进方法获得了一致的改进

Conclusion: DPHR策略通过双层级渐进式难度感知重加权，有效解决了跨视角地理定位中的视角差异和困难负样本问题，实现了稳定收敛和性能提升
...
TL;DR: 提出了一种稀疏模型反演策略，通过选择性反演语义前景来加速高分辨率图像的模型反演，避免噪声背景和虚假相关的反演。


<details>
  <summary>Details</summary>
Motivation: 现有密集反演方法在反演高分辨率图像时效率低下，主要原因是冗余反演噪声背景和虚假相关（幻觉现象）。

Method: 提出稀疏模型反演策略，作为即插即用扩展，选择性反演语义前景，停止反演噪声背景和潜在虚假相关，无需修改原始损失函数。

Result: 实现了显著的反演加速（最高3.79倍），在无数据模型量化和无数据知识转移中保持甚至提升了下游性能。
...
TL;DR: 该论文提出了多模态交互式真实性评估任务(MIVA)，基于狼人杀游戏构建了包含同步视频、文本和真实标签的数据集，评估发现当前最先进的多模态大语言模型在区分真假陈述方面表现不佳。


<details>
  <summary>Details</summary>
Motivation: 随着AI系统日益融入人类生活，赋予其强大的社会智能变得至关重要。其中关键方面是辨别真假，这在人类互动中普遍存在，通过语言和非语言视觉线索的复杂交互来传达。

Method: 提出了多模态交互式真实性评估任务(MIVA)，基于社交推理游戏狼人杀构建了一个新颖的多模态数据集，包含同步的视频、文本和可验证的真实标签。

Result: 对最先进的多模态大语言模型的全面基准测试显示存在显著的性能差距：即使是像GPT-4o这样强大的模型也难以可靠地区分真假。失败模式分析表明这些模型未能有效将语言与视觉社交线索相结合，可能过于保守。

Conclusion: 当前多模态大语言模型在真实性检测方面存在明显不足，迫切需要新的方法来构建更具洞察力和可信度的AI系统。
...
TL;DR: 提出了一种融合多源数据的层次图神经网络模型，用于深入分析村庄空间形态，通过多模态特征融合和联合训练策略显著提升了分类性能。


<details>
  <summary>Details</summary>
Motivation: 随着城市化进程，村庄空间特征逐渐消失和景观同质化问题突出，现有研究多采用单一学科视角和定性分析方法，受限于数字基础设施不足和数据匮乏。

Method: 构建层次图神经网络模型，包含输入节点和通信节点两种节点类型，静态输入边和动态通信边两种边类型，结合图卷积网络和图注意力网络，采用两阶段特征更新机制和多模态特征融合，引入关系池化机制并在17个子类型上实施联合训练策略。

Result: 实验结果表明，该方法在多模态融合和分类任务上显著优于现有方法，联合优化使平均准确率/F1值从0.71/0.83提升到0.82/0.90，其中地块任务提升了6%。
...
<details>
  <summary>Details</summary>
Motivation: 解决医学图像诊断中大规模标注数据稀缺、领域偏移（特别是CT窗口设置差异）以及数据隐私约束等挑战，构建鲁棒且泛化能力强的模型。

Method: 采用持续预训练策略，结合潜在重放机制保护数据隐私，引入基于Wasserstein距离的知识蒸馏和批量知识集成技术来学习领域偏移鲁棒的特征表示。

Result: 在两个不同窗口设置的胸部CT图像上验证了方法的有效性，相比其他方法表现出更优越的性能。

Conclusion: 该框架成功解决了医学图像持续学习中的隐私保护和领域偏移问题，为动态医疗环境下的模型部署提供了可行方案。
...
TL;DR: 提出SpecAware，一种新颖的高光谱光谱内容感知基础模型，通过超网络驱动的两阶段编码过程统一多传感器HSI学习，解决了HSI数据异质性和传感器差异带来的泛化挑战。


<details>
  <summary>Details</summary>
Motivation: 高光谱成像(HSI)在精细土地利用和土地覆盖(LULC)制图中至关重要，但HSI数据的固有异质性阻碍了通过联合训练开发泛化模型。现有HSI基础模型通常忽略传感器元属性的关键指导作用，难以进行多传感器训练，限制了其可迁移性。

Method: SpecAware采用两阶段超网络驱动编码：1) 元内容感知模块，融合传感器元属性和图像内容为每个HSI样本生成条件输入；2) HyperEmbedding模块，通过样本条件超网络动态生成矩阵因子对进行通道级编码，实现自适应空间模式提取和潜在语义特征重投影。

Result: 在六个数据集上的广泛实验表明，SpecAure能够学习到优越的特征表示，在土地覆盖语义分割分类、变化检测和场景分类任务中表现出色。

Conclusion: SpecAware能够感知和解释不同场景和传感器的空间-光谱特征，自适应处理可变数量的光谱通道，为联合预训练建立了统一框架，显著提升了HSI基础模型的泛化能力。
...
TL;DR: YOLOv11在卫星图像上实现建筑物实例分割和高度分类，在DFC2023数据集上达到60.4% mAP@50和38.3% mAP@50-95的性能，优于先前多任务框架。


<details>
  <summary>Details</summary>
Motivation: 准确的建筑物实例分割和高度分类对城市规划、3D城市建模和基础设施监测至关重要，需要高效的深度学习方法处理复杂城市场景。

Method: 使用YOLOv11深度学习模型，采用改进的架构更好地结合不同尺度特征，提高目标定位精度，在DFC2023 Track 2数据集（12个城市12.5万个标注建筑物）上进行评估。

Result: YOLOv11在五个预定义高度层级上保持稳健的分类精度，特别擅长处理遮挡、复杂建筑形状和类别不平衡问题，尤其对罕见高层建筑表现优异。
...
TL;DR: MoRE是一个基于混合专家(MoE)架构的密集3D视觉基础模型，通过动态路由特征到任务特定专家来提升可扩展性和适应性，结合置信度深度细化模块和语义特征集成，在多个几何任务上达到最先进性能。
...
Conclusion: MoRE通过MoE架构和精心设计的模块成功解决了3D模型扩展的挑战，为3D视觉几何重建提供了可扩展且鲁棒的解决方案。
...
TL;DR: Object-IR是一个自监督的图像重定向框架，通过基于网格变形的学习优化方法，在保持重要语义对象外观的同时减少几何失真，无需人工标注数据即可实现实时处理。


<details>
  <summary>Details</summary>
Motivation: 解决图像重定向中语义重要区域的几何失真问题，传统方法难以在保持对象外观的同时避免失真。

Method: 将图像重定向重新定义为基于学习的网格变形优化问题，使用CNN预测网格运动，结合对象一致性损失、几何保持损失和边界损失进行自监督训练。

Result: 在RetargetMe基准测试中达到最先进性能，定量指标和主观视觉质量评估均优于现有方法，处理1024x683分辨率图像平均仅需0.009秒。

Conclusion: Object-IR通过自监督的网格变形方法有效解决了图像重定向中的几何失真问题，实现了高质量、实时的图像重定向处理。
...
Conclusion: FuseCPath框架能有效融合异构病理基础模型，提升下游任务的性能，为计算病理学提供了强大的工具。
...
TL;DR: 提出了一种结合空间和频域的两阶段训练方法，通过离散小波变换分析图像高频分量，使用去噪网络增强分类器对抗攻击的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 深度神经网络在安全关键系统中容易受到对抗攻击，限制了其应用。需要开发有效的防御方法来提高模型鲁棒性。

Method: 两阶段训练：首先训练结合空间特征和小波变换的去噪网络，然后使用去噪后的图像重新训练分类器。利用DWT分析频域特征，特别关注高频分量的修复。

Result: 在MNIST、CIFAR-10和Fashion-MNIST数据集上的实验表明，该方法显著提高了分类准确率，性能明显优于单独使用去噪网络和对抗训练的方法。

Conclusion: 提出的结合空间和频域的去噪策略能有效防御对抗攻击，通过修复受损的高频分量显著提升分类器鲁棒性。
...
TL;DR: 提出了一种利用对比学习进行对抗防御的新方法，通过对比损失函数增强分类模型的鲁棒性，使其对对抗攻击更具抵抗力。
...
Method: 使用对比学习框架，结合干净图像和对抗扰动图像进行训练，通过优化模型参数和扰动来学习鲁棒表示。

Result: 实验结果显示模型对各种对抗扰动的鲁棒性显著提高，对比损失有助于提取更具信息性和弹性的特征。

Conclusion: 对比学习是提升深度学习模型对抗鲁棒性的有效方法，为对抗鲁棒性研究领域做出了贡献。
...
TL;DR: 提出利用网络爬取描述和大型语言模型提取关键词的方法，解决视觉语言模型在零样本动作识别中因多语义词导致的歧义问题，无需人工标注。
...
Conclusion: 该方法展示了在不同下游任务中的适应性和有效性，无需人工标注即可解决多语义词歧义问题。
...
TL;DR: RegionRAG提出了一种从文档级检索转向区域级检索的新范式，通过识别相关图像区域而非整个文档来减少无关视觉内容，提升多模态RAG的性能和效率。
...
Method: 提出RegionRAG框架，将检索粒度从文档级转向区域级。训练时采用混合监督策略从标注和未标注数据中定位相关图像块；推理时通过动态管道将显著图像块分组为完整语义区域。
...
TL;DR: 提出了T^3框架，通过Jensen-Shannon散度动态计算模型融合系数，解决医疗影像中预训练模型与微调专家模型在模态偏移下的性能平衡问题。


<details>
  <summary>Details</summary>
Motivation: 医疗影像中，预训练模型具有广泛鲁棒性但缺乏模态特异性，而微调专家模型在分布内准确率高但在模态偏移下表现不佳。现有模型融合方法在医疗领域效果不稳定。

Method: T^3框架无需反向传播，基于两个模型输出分布的Jensen-Shannon散度计算逐样本融合系数；T^3_B扩展为批处理版本降低计算成本。

Result: 在跨四个模态的严格评估中，T^3在Top-1准确率和错误率降低方面达到新的最先进水平，显著优于基线方法。

Conclusion: T^3为临床环境中自适应多模态视觉语言模型部署提供了高效解决方案，在保持效率的同时实现了性能提升。
...
TL;DR: HyperClick是一个通过不确定性校准增强GUI代理可靠性的框架，解决了当前模型在GUI接地任务中过度自信的问题。
...
TL;DR: FOCUS是一种无需训练、模型无关的关键帧选择方法，通过多臂老虎机框架将关键帧选择建模为组合纯探索问题，在严格token预算下选择查询相关帧，显著提升长视频理解性能。


<details>
  <summary>Details</summary>
Motivation: 现有的关键帧选择方法要么均匀采样，要么使用检索式评分，但需要预过滤且可能错过最具信息量的时刻，无法有效处理小时级长视频的token膨胀问题。

Method: 将关键帧选择建模为多臂老虎机中的组合纯探索问题，将短时间片段视为臂，使用经验均值和Bernstein置信半径识别信息丰富区域，采用两阶段探索-利用策略。

Result: 在两个长视频问答基准测试中，FOCUS仅处理不到2%的视频帧就实现了显著的准确率提升，在超过20分钟的视频上，LongVideoBench准确率提升11.9%。

Conclusion: FOCUS为多模态大语言模型的长视频理解提供了一个简单通用的可扩展解决方案，证明了其作为关键帧选择方法的有效性。
...
TL;DR: 提出S-GRACE方法，通过语义引导生成对抗样本来改进扩散模型中的概念擦除，显著提升擦除性能26%，减少训练时间90%。


<details>
  <summary>Details</summary>
Motivation: 现有概念擦除方法在扩散模型中采用对抗训练时忽视了概念语义的作用，导致无法有效拟合概念空间，要么无法全面覆盖目标概念，要么干扰其他概念空间。

Method: S-GRACE方法利用概念空间内的语义指导来生成对抗样本并进行擦除训练，确保对抗样本能够有效拟合目标概念空间。

Result: 在多种扩散模型去学习场景下的实验表明，与7种最先进方法和3种对抗提示生成策略相比，S-GRACE显著提升擦除性能26%，更好地保留非目标概念，训练时间减少90%。
...
TL;DR: FGMamba是一种轻量级医学图像超分辨率方法，结合了门控状态空间建模和金字塔频率融合，在保持小参数量的同时实现了优异的超分辨率性能。


<details>
  <summary>Details</summary>
Motivation: 医学图像超分辨率在提高诊断准确性的同时降低采集成本和扫描时间很重要，但如何在低计算开销下同时建模长距离解剖结构和细粒度频率细节仍具挑战性。

Method: 提出FGMamba方法，包含两个关键创新：门控注意力增强状态空间模块(GASM)结合状态空间建模与双分支空间和通道注意力；金字塔频率融合模块(PFFM)通过FFT引导融合在多个分辨率上捕获高频细节。

Result: 在五种医学成像模态(超声、OCT、MRI、CT和内窥镜)上的广泛评估显示，FGMamba在PSNR/SSIM指标上表现优异，同时保持紧凑参数规模(<0.75M)，优于基于CNN和Transformer的最先进方法。

Conclusion: 频率感知状态空间建模对于可扩展和准确的医学图像增强是有效的，FGMamba在保持轻量级架构的同时实现了全局依赖建模和精细细节增强的统一。
...
TL;DR: CASR-Net是一个用于冠状动脉分割的三阶段深度学习网络，通过多通道预处理、UNet+DenseNet121编码器和Self-ONN解码器的分割网络，以及轮廓细化模块，显著提升了冠状动脉分割的准确性和连续性。


<details>
  <summary>Details</summary>
Motivation: 冠状动脉疾病的早期检测对降低死亡率和改善治疗规划至关重要。虽然X射线血管造影分析是识别心脏异常（包括狭窄冠状动脉）的常用且经济有效的方法，但图像质量差会显著阻碍临床诊断。

Method: 提出CASR-Net三阶段管道：1）结合CLAHE和改进Ben Graham方法的多通道预处理；2）基于UNet+DenseNet121编码器和Self-ONN解码器的分割网络；3）轮廓细化模块抑制假阳性。

Result: 在两个公共数据集上的5折交叉验证显示，CASR-Net优于多个最先进模型，达到IoU 61.43%、DSC 76.10%和clDice 79.36%。多通道预处理使DSC提升0.31-0.89%，IoU提升0.40-1.16%。

Conclusion: CASR-Net为自动冠状动脉分割提供了稳健方法，为临床医生的诊断和治疗规划提供了有价值的工具，特别是在保持狭窄血管分支连续性方面表现优异。
...
TL;DR: 提出了参数化提示的增量目标检测方法P²IOD，通过神经网络作为参数化提示来自适应整合跨任务知识，解决了现有提示池方法在检测图像中忽略共现现象的问题。


<details>
  <summary>Details</summary>
Motivation: 现有基于提示池的增量学习方法假设增量任务间类别集不相交，这不适用于目标检测，因为检测图像中存在固有的共现现象——先前任务的未标记对象可能出现在当前任务图像中，导致提示池混淆。
...
<details>
  <summary>Details</summary>
Motivation: 内窥镜视频中可变形组织重建面临组织运动导致的锯齿和伪影问题，现有3D高斯泼溅方法过于注重渲染速度而忽视这些问题。

Method: 使用注意力驱动的动态加权4D变形解码器，结合3D平滑滤波器和2D Mip滤波器来减轻伪影并捕捉组织运动的精细细节。

Result: 在EndoNeRF和SCARED两个公开基准测试中，所有指标（PSNR、SSIM、LPIPS）均优于现有最先进方法，并提供更好的可视化质量。

Conclusion: SAGS框架成功解决了可变形内窥镜场景重建中的伪影问题，在保持高效渲染的同时实现了更高质量的组织重建。
...
<details>
  <summary>Details</summary>
Motivation: 解决在深空探测、战场情报等极低带宽场景下的视觉通信问题，现有文本到图像生成方法只能达到语义级近似，无法满足视觉通信和远程分析的需求。

Method: 将图像生成与深度图像压缩无缝集成，使用联合文本和编码潜在来指导修正流模型进行精确的视觉场景生成，语义文本描述和编码潜在都以极低码率编码传输。

Result: 实验结果表明，该方法在使用更少带宽的情况下，能够达到与现有方法相同的图像重建质量和视觉分析精度。

Conclusion: 提出的方法在超低码率下实现了准确的视觉场景重建和视觉分析，为极低带宽环境下的视觉通信提供了有效解决方案。
...
TL;DR: 该论文介绍了针对ODELIA乳腺MRI挑战赛2025的解决方案，旨在通过更高效准确的乳腺MRI扫描解释来改善乳腺癌早期检测。


<details>
  <summary>Details</summary>
Motivation: 乳腺癌检测面临高质量分割标签有限的挑战，因此开发基于分类的鲁棒方法对于大规模筛查等应用至关重要。

Method: 采用迭代开发过程，包括实验、评估和优化阶段，最终形成基于分类的解决方案。

Result: 提供了完整的实现方案并公开发布在GitHub上，专注于性能、鲁棒性和临床相关性。

Conclusion: 基于分类的方法有望改善乳腺癌早期检测，特别是在大规模筛查应用中具有重要价值。
...
Result: 在SmartEdit数据集上PSNR指标超越之前SOTA方法9.955 dB，在自建CIEBench基准上也优于之前方法。
...
Result: 在MMEB基准测试中取得了最佳整体分数，在具有挑战性的视频和视觉文档检索任务上超越了所有先前工作，创下了新的最先进水平。

Conclusion: RzenEmbed提供了一个有效的统一多模态嵌入框架，能够处理多种视觉模态，在检索任务中表现出色，为多模态学习提供了新的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有参数高效微调方法存在局限性：基于添加的方法（如Adapters）引入推理延迟和工程复杂性，而基于选择的方法（如GPS）需要完整反向传播，内存使用与全微调相同。

Method: FPS通过参数幅度与对应输入激活的乘积来对参数进行排序，利用预训练知识和下游数据，在单次前向传播中完成参数选择。

Result: 在24个视觉任务（FGVC和VTAB-1k）上评估，FPS性能与最先进方法相当，同时峰值内存使用减少近9倍，参数选择速度提升约2倍。
...
TL;DR: 提出了一个两阶段微调开源视频扩散变换器的实用流程，用于从少量数据合成影视制作中的电影场景，通过分离视觉风格学习和运动生成来实现高效领域迁移。


<details>
  <summary>Details</summary>
Motivation: 为电视和电影制作开发能够从小数据集合成电影场景的实用方法，解决传统方法需要大量训练数据的问题。

Method: 两阶段流程：第一阶段在Wan2.1 I2V-14B模型的交叉注意力层集成LoRA模块，使用El Turco电视剧的短片段数据集进行视觉风格学习；第二阶段生成风格一致的关键帧，通过视频解码器扩展为连贯的720p序列。

Result: 定量和定性评估显示，在FVD、CLIP-SIM和LPIPS指标上，相较于基础模型在电影保真度和时间稳定性方面有显著提升，专家用户研究也证实了改进效果。

Conclusion: 该流程实现了高效的领域迁移，在单GPU上几小时内完成训练，并发布了完整的训练和推理流程以支持跨电影领域的复现和适应。
...
Method: 构建语义感知的视觉特征提取框架，使用交叉注意力机制从中间Transformer层提取视觉特征；将特征树嵌入到具有不同曲率的双曲流形中；通过最小化异构流形间的KL距离来学习中间流形实现对齐。

Result: 在多个图像数据集上的分类学开放集分类任务中，在少样本和跨域设置下始终优于强基线方法。

Conclusion: 提出的跨树对齐方法有效解决了视觉-语言模型中的模态对齐不对称问题，通过层次特征提取和异构双曲流形对齐实现了更好的性能。
...
Method: 1) 建立包含16个数据集的通用视频检索基准；2) 基于诊断结果开发可扩展的数据合成流程生成155万高质量样本；3) 设计模态金字塔课程训练通用视频嵌入器。

Result: 通用视频嵌入器在UVRB上实现了最先进的零样本泛化性能，分析显示流行基准对通用能力的预测能力差，部分相关检索是主导但被忽视的场景。
...
TL;DR: 提出融合法医特征与深度学习表示的混合框架，在多个基准数据集上优于单一方法基线，并在压缩、对抗扰动和未知操作下保持稳定性能，同时通过可解释性分析增强透明度。


<details>
  <summary>Details</summary>
Motivation: 生成对抗网络和扩散模型的快速发展使合成媒体日益逼真，引发了对虚假信息、身份欺诈和数字信任的社会担忧。现有深度伪造检测方法存在泛化能力差、易受失真影响或对新操作技术有限的局限性。

Method: 提出混合框架，融合包括噪声残差、JPEG压缩痕迹和频域描述符在内的法医特征，以及来自卷积神经网络和视觉变换器的深度学习表示。

Result: 在FaceForensics++、Celeb-DF v2和DFDC数据集上分别获得0.96、0.82和0.77的F1分数，在压缩（QF=50时F1=0.87）、对抗扰动（AUC=0.84）和未知操作（F1=0.79）下表现稳定，Grad-CAM和法医热图与真实操作区域重叠率达82%。

Conclusion: 混合方法提供了平衡的解决方案，结合了深度模型的适应性和法医线索的可解释性，可开发出具有弹性和可信度的深度伪造检测系统。
...
TL;DR: 该论文审计了乳腺癌肿瘤分割数据集MAMA-MIA中自动分割标签的公平性，发现了针对年轻患者的年龄相关偏见，并展示了多数据源聚合对特定站点种族偏见的影响。


<details>
  <summary>Details</summary>
Motivation: 深度学习模型旨在改进诊断工作流程，但公平性评估在分类之外（如图像分割）仍未被充分探索。未解决的分割偏见可能导致某些人群的护理质量差异，并在临床决策点中复合，通过迭代模型开发放大。

Method: 审计MAMA-MIA乳腺癌肿瘤分割数据集中自动分割标签的公平性，评估跨年龄、种族和数据源的分割质量。

Result: 分析揭示了针对年轻患者的内在年龄相关偏见，即使在控制数据源等混杂因素后仍然持续。这种偏见可能与生理因素有关，这是放射科医生和自动化系统都面临的已知挑战。多数据源聚合影响了站点特定的种族偏见。

Conclusion: 需要在细粒度水平上调查数据，以解决分割偏见问题，确保深度学习模型在医疗应用中的公平性。
...
TL;DR: 本文提出了一种解决部分相关视频检索中语义坍塌问题的新框架，通过文本相关性保持学习和跨分支视频对齐方法，有效提升了多事件视频的检索性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法将所有标注的文本-视频对视为正样本，其他为负样本，忽略了视频内部和跨视频的丰富语义变化，导致同一视频中不同事件的查询和视频片段嵌入坍塌在一起，而不同视频中语义相似的查询和片段被推远，限制了包含多个不同事件视频的检索性能。

Method: 1. 文本相关性保持学习：保持基础模型编码的文本查询间的语义关系；2. 跨分支视频对齐：通过对比对齐方法在不同时间尺度上解缠层次化视频表示；3. 顺序保持标记合并和自适应CBVA：生成内部一致但相互区分的视频片段以增强对齐效果。

Result: 在PRVR基准测试上的广泛实验表明，该框架有效防止了语义坍塌，显著提高了检索准确率。

Conclusion: 提出的框架通过解决文本和视频嵌入空间中的语义坍塌问题，在部分相关视频检索任务中取得了显著性能提升，为多事件视频的精确检索提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 传统盲图像去模糊方法依赖手工先验，而深度学习方法需要大量预训练数据，限制了在真实场景中的适应性。需要一种无需外部数据、能动态学习实例特定先验的方法。

Method: 将盲去模糊建模为迭代反向自扩散过程，从纯噪声开始逐步优化。使用两个随机初始化的神经网络连续优化清晰图像和模糊核，结合数据一致性和核的L1稀疏约束，采用噪声调度机制稳定优化。

Result: DeblurSDI在广泛实验中表现优异，即使在高度退化场景下也能恢复清晰图像和准确的模糊核，对模糊核尺寸变化具有鲁棒性。

Conclusion: DeblurSDI提供了一种有效的零样本盲去模糊解决方案，通过自扩散过程动态学习实例特定先验，在无需预训练的情况下实现了优越性能。
...
TL;DR: CoMViT是一个紧凑且可泛化的Vision Transformer架构，专为资源受限的医疗图像分析设计，通过多种优化技术实现高性能和轻量化。
...
Method: 整合卷积分词器、对角掩码、动态温度缩放和基于池化的序列聚合，通过系统架构优化实现轻量化设计。
...
<details>
  <summary>Details</summary>
Motivation: 解决当前生成模型的两个主要限制：基于图像的模型缺乏语义结构，而基于代码的方法缺乏直观性和编辑效率。

Method: 采用多智能体框架，包含Manager、Designer和Toolbox三个专门模块，通过角色分工协作生成矢量图形。

Result: 开发了VisBench基准测试，包含七维评估指标，验证了架构合理性和评估方法的可靠性。

Conclusion: VisPainter框架在科学插图生成中实现了真正的元素级控制，为高效、直观的迭代修改提供了解决方案。
...
TL;DR: 提出了一种多层级人机交互框架，通过机器学习、卫星影像和深度学习相结合的方式，提高发展中国家教育设施记录的准确性和完整性。


<details>
  <summary>Details</summary>
Motivation: 在发展中国家，教育设施数据往往稀缺且更新不及时，需要一种准确、完整的学校地图绘制方法来支持规划和资源分配。

Method: 采用多层级方法：第一层使用机器学习分析人口密度、土地覆盖和现有基础设施；第二层使用中分辨率卫星影像识别学校可能存在的区域；第三层使用超高分辨率影像和深度学习模型生成详细的候选学校位置。关键组件是允许人工操作员迭代审查、验证和优化结果的交互界面。

Result: 初步评估表明，该多层级策略为教育基础设施地图绘制提供了可扩展且经济有效的解决方案。中分辨率方法因改进不显著而被移除。

Conclusion: 该框架通过结合机器学习和人工验证，能够有效识别和验证学校位置，特别适用于数据稀缺的发展中地区。
...
TL;DR: 提出Referee方法，利用单次示例中的说话者特定线索进行音视频深度伪造检测，通过匹配参考和目标内容中的身份相关查询，联合推理音视频同步性和身份一致性。


<details>
  <summary>Details</summary>
Motivation: 先进生成模型产生的深度伪造内容构成严重威胁，现有音视频深度伪造检测方法难以泛化到未见过的伪造类型。

Method: 参考感知的音视频深度伪造检测方法Referee，利用单次示例提取说话者特定线索，通过跨模态特征匹配和对齐身份相关查询，联合推理音视频同步性和身份一致性。
...
TL;DR: 构建了大规模水下多任务数据集NautData，并提出可插拔视觉特征增强模块VFE，开发了水下大语言模型NAUTILUS，显著提升水下场景理解性能。
...
Method: 构建包含145万图像-文本对的NautData数据集，支持8个水下场景理解任务；提出基于水下成像模型的物理先验，设计可插拔视觉特征增强模块VFE；将VFE集成到LLaVA-1.5和Qwen2.5-VL中构建NAUTILUS模型。

Result: 在NautData和公开水下数据集上的实验表明，VFE模块有效提升了两个基线模型在大多数支持任务上的性能，确保NAUTILUS在水下场景理解领域的优越性。
...
TL;DR: ThinkMorph是一个统一的多模态推理模型，通过文本和图像的互补性思维链来推进推理过程，在视觉中心任务上取得显著提升并展现出新兴的多模态智能能力。
...
Method: 构建ThinkMorph模型，在24K高质量交错推理轨迹上进行微调，涵盖不同视觉参与度的任务。模型学习生成渐进式的文本-图像推理步骤，在保持连贯语言逻辑的同时具体操作视觉内容。

Result: 在视觉中心基准测试上平均提升34.7%，优于基础模型，并能泛化到域外任务，匹配或超越更大和专有的视觉语言模型。模型展现出新兴的多模态智能能力。

Conclusion: 这些发现为表征统一多模态推理模型的新兴能力提供了有前景的方向，表明文本和图像思维的互补性可以显著提升推理性能。
...
<details>
  <summary>Details</summary>
Motivation: 准确肺肿瘤分割对诊断和治疗规划至关重要，有效结合PET和CT的解剖与功能信息仍面临重大挑战。

Method: 基于视觉Mamba架构，构建vMambaX框架，集成上下文门控跨模态感知模块(CGM)，自适应增强模态间特征交互，强调信息区域同时抑制噪声。
...
TL;DR: 提出了一种基于深度学习的3D点云水印框架，通过SVD将水印嵌入点云块的奇异值中，并使用PointNet++网络进行水印提取，显著优于传统SVD方法。
...
Method: 使用SVD将二进制水印嵌入3D点云块的奇异值中，采用PointNet++神经网络架构进行水印提取，训练网络抵抗旋转、缩放、噪声、裁剪等攻击。

Result: 在ModelNet40数据集上验证，深度学习提取方法在Crop(70%)攻击下达到0.83的比特精度和0.80的IoU，显著优于SVD方法的0.58比特精度和0.26 IoU。
...
TL;DR: MapSAM2是一个基于视觉基础模型的统一框架，用于自动分割历史地图图像和时间序列，通过将地图处理为视频格式来提升分割精度。


<details>
  <summary>Details</summary>
Motivation: 历史地图是记录地理特征的重要档案，但由于风格多变和标注数据稀缺，自动分析面临挑战。构建时空关联数据集对建筑年代判定、道路网络发展等应用至关重要。

Method: 基于视觉基础模型，将历史地图图像和时间序列视为视频处理。对图像，将图块集作为视频处理以利用记忆注意力机制；对时间序列，提出从单年地图生成伪时间序列的方法。

Result: 实验表明MapSAM2能有效学习时间关联，在有限监督或使用伪视频条件下准确分割和关联时间序列中的建筑物。

Conclusion: MapSAM2为历史地图分析提供了有效的统一框架，将发布数据集和代码支持未来研究。
...
### [67] [Spatial-SSRL: Enhancing Spatial Understanding via Self-Supervised Reinforcement Learning](https://arxiv.org/abs/2510.27606)
*Yuhong Liu,Beichen Zhang,Yuhang Zang,Yuhang Cao,Long Xing,Xiaoyi Dong,Haodong Duan,Dahua Lin,Jiaqi Wang*

Main category: cs.CV

TL;DR: 提出了Spatial-SSRL，一种自监督强化学习范式，直接从普通RGB或RGB-D图像中获取可验证信号，通过五个预训练任务提升大型视觉语言模型的空间理解能力。


<details>
  <summary>Details</summary>
Motivation: 现有的大型视觉语言模型在空间理解方面存在不足，而现有的监督微调和强化学习方法依赖昂贵的监督、专用工具或受限环境，限制了规模化应用。

Method: 自动构建五个捕捉2D和3D空间结构的预训练任务：打乱补丁重排序、翻转补丁识别、裁剪补丁修复、区域深度排序和相对3D位置预测，这些任务提供易于验证的真实答案，无需人工或LVLM标注。
...
TL;DR: 提出了CroVCA（跨视图代码对齐）方法，通过简单的二元交叉熵损失和编码率最大化正则化来学习二进制哈希码，实现了高效的大规模检索。


<details>
  <summary>Details</summary>
Motivation: 解决大规模检索中高维嵌入空间计算成本高的问题，同时避免现有哈希方法复杂的多目标优化和长训练时间。

Method: 使用单一二元交叉熵损失强制跨语义对齐视图的代码一致性，编码率最大化作为防坍塌正则化器，设计了轻量级MLP哈希网络HashCoder。

Result: 在多个基准测试中达到最先进结果，仅需5个训练周期，16位时效果尤其好，在单个GPU上COCO无监督哈希训练不到2分钟，ImageNet100监督哈希约3分钟。

Conclusion: CroVCA方法具有高效性、适应性和广泛适用性，为大规模检索提供了简单而统一的二进制代码学习方案。
...
### [69] [Sketch-to-Layout: Sketch-Guided Multimodal Layout Generation](https://arxiv.org/abs/2510.27632)
...
<details>
  <summary>Details</summary>
Motivation: 现有布局生成方法需要复杂的用户约束规范，降低了可用性。用户草图作为直观约束可以简化设计过程，但目前这一研究方向尚未充分探索。

Method: 使用多模态transformer模型，以草图和内容资产作为输入生成布局。为了解决训练数据稀缺问题，开发了高效的合成草图生成方法。
...
TL;DR: 提出了ANCHOR框架，通过监督对比学习和硬正例挖掘来增强神经网络对抗攻击的鲁棒性，使模型学习更稳定和有意义的特征表示。


<details>
  <summary>Details</summary>
Motivation: 神经网络虽然强大，但其依赖的梯度学习机制使其容易受到对抗攻击，即微小但精心设计的扰动就能导致模型错误分类。

Method: 使用监督对比学习结合显式硬正例挖掘，让原始图像、增强版本和扰动版本的嵌入在特征空间中聚集在一起，同时与其他类别的图像分离。

Result: 在CIFAR-10数据集上，ANCHOR在PGD-20攻击下取得了优异的干净准确率和鲁棒准确率，优于标准的对抗训练方法。

Conclusion: 结合对抗指导和硬挖掘对比监督有助于模型学习更结构化和鲁棒的表示，缩小了准确率和鲁棒性之间的差距。
...
Method: 开发VessShape方法生成大规模2D合成数据集，包含程序生成的管状几何结构结合多种前景和背景纹理，强制模型学习形状线索而非纹理特征。
...
### [72] [Who Made This? Fake Detection and Source Attribution with Diffusion Features](https://arxiv.org/abs/2510.27602)
...
Method: 使用预训练扩散模型的内部激活特征，通过k近邻分类器进行深度伪造检测，同时采用紧凑神经网络模型进行来源生成器识别。

Result: 扩散特征上的k近邻分类器无需微调即可实现跨生成器的最先进性能，紧凑神经网络模型能够准确进行来源识别。
...
### [73] [PETAR: Localized Findings Generation with Mask-Aware Vision-Language Modeling for PET Automated Reporting](https://arxiv.org/abs/2510.27680)
...
<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型在医疗应用中主要局限于2D成像，而3D PET/CT具有大体积数据、小而分散的病灶以及冗长放射学报告的特点，需要专门的处理方法。

Method: 构建了包含11,000多个病灶级描述和3D分割的大规模数据集，使用混合规则和LLM流水线提取数据。提出PETAR-4B模型，整合PET、CT和病灶轮廓，实现全局上下文推理与细粒度病灶感知的结合。
...
<details>
  <summary>Details</summary>
Motivation: 现有的视觉-语言-动作模型在联合预测下一状态观察和动作序列时面临模态差异的挑战

Method: 采用多模态扩散transformer架构，维护分离的模态流同时支持跨模态知识共享，引入独立噪声扰动和解耦流匹配损失

Result: 在RoboCasa和GR-1基准测试中比基线方法提升6%，测试时缩放额外提升2-5%，真实世界任务成功率提高13%

Conclusion: DUST框架有效解决了多模态冲突问题，在模拟和真实世界任务中均表现出色，并具有大规模预训练潜力
...
<details>
  <summary>Details</summary>
Motivation: 异构协作感知中不同代理使用固定感知模型导致中间特征存在领域差距，现有方法将特定代理表示作为共同表示，难以实现有效对齐

Method: 引入协商器从各模态代理的局部表示推导共同表示，使用发送器和接收器实现特征空间转换，并采用结构对齐、语用对齐和分布对齐损失进行训练监督

Result: NegoCollab有效减少了与各种局部表示的内在领域差距，实现了更好的特征对齐

Conclusion: 基于协商共同表示的异构协作方法能够充分将共同表示中的知识提取到发送器中，提升协作性能
...
Result: 在AI-TOD-v2、MS-COCO-2017和Visdrone-2019数据集上，GCD作为边界框回归损失函数和标签分配度量，均达到最优性能。

Conclusion: GCD是一种有效的相似性度量方法，解决了小目标检测中的关键问题，具有良好的泛化能力。
...
TL;DR: 提出了一种基于无监督深度学习的通用去噪框架，用于提升操作显微镜的定量分析能力，通过减少噪声提高分辨率并保持物理保真度。
...
Method: 开发了无监督深度学习去噪框架，结合PDE约束优化进行模型学习，并在多种显微镜模态和尺度上验证。

Result: 去噪方法在STXM中揭示了纳米级化学和结构异质性，在光学显微镜中实现了自动粒子分割和相分类，在中子射线照相中将噪声引起的变异性降低了近80%。

Conclusion: 深度学习去噪是一种强大的、模态无关的增强技术，能够推进定量操作成像并扩展先前受噪声限制技术的应用范围。
...
TL;DR: 提出Sh-ViT模型，一种轻量级且鲁棒的行人重识别方法，通过打乱模块、场景自适应增强和知识蒸馏技术，在遮挡和模糊条件下表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有行人重识别方法在遮挡、视角变形和图像质量差等监控场景挑战下表现不佳，需要更鲁棒的解决方案。

Method: 基于ViT-Base构建，包含三个关键组件：1）最终Transformer层的打乱模块破坏空间相关性；2）场景自适应数据增强模拟监控条件；3）DeiT知识蒸馏改善有限标签下的学习。

Result: 在自建MyTT数据集上达到83.2% Rank-1和80.1% mAP，在Market1501上达到94.6% Rank-1和87.5% mAP，优于CNN和ViT基线及现有最优方法。
...
<details>
  <summary>Details</summary>
Motivation: DMD单步蒸馏模型在复杂生成任务中表现不佳，而直接扩展到多步蒸馏会导致内存使用增加、计算深度加深，造成不稳定和效率降低。随机梯度截断会显著降低多步蒸馏模型的生成多样性。

Method: 基于分阶段蒸馏和专家混合思想，提出Phased DMD框架：1）将SNR范围划分为子区间，逐步精炼模型到更高SNR水平；2）在每个子区间内进行分数匹配，确保训练目标的准确性。

Result: 在Qwen-Image（200亿参数）和Wan2.2（280亿参数）等最先进的图像和视频生成模型上进行验证，实验结果表明Phased DMD比DMD更好地保持了输出多样性，同时保留了关键生成能力。

Conclusion: Phased DMD通过分阶段蒸馏和子区间分数匹配，有效解决了多步蒸馏中的稳定性和多样性问题，在保持生成能力的同时提升了模型性能。
...
Method: 采用可学习的提升小波，通过提升和逆提升单元自适应捕获雷达信号特征并合成生理上有意义的ECG波形；引入多分辨率短时傅里叶变换损失来增强重建保真度。
...
TL;DR: 分析了混合状态空间模型和注意力机制的架构设计，发现顺序混合在短上下文表现更好，并行混合在长上下文更有效，并提出通过数据增强训练来提升召回能力。


<details>
  <summary>Details</summary>
Motivation: 混合模型结合了状态空间模型的效率和注意力的高召回能力，但其架构设计选择尚未被充分理解，需要分析不同集成方式的效果。

Method: 比较顺序和并行集成SSM与注意力层的方法，并引入基于数据增强的持续训练策略，使用包含释义的数据集来增强模型能力。

Result: 顺序混合在短上下文表现更优，并行混合在长上下文更有效；数据增强训练能进一步提升召回能力，且优于架构修改方法。
...
TL;DR: 提出一种基于语义框架的方法来识别医疗记录中的应报告事件，特别是针对性别暴力事件，在2100万句巴西葡萄牙语医疗记录中验证了方法的有效性。


<details>
  <summary>Details</summary>
Motivation: 解决医疗记录中性别暴力事件漏报问题，利用自然语言处理技术提高公共卫生监测能力。

Method: 使用语义框架定义细粒度模式，在非结构化医疗记录文本中搜索这些模式，共定义了8个模式进行识别。

Result: 方法能有效识别暴力报告，精确度达到0.726，证明了方法的稳健性。

Conclusion: 该方法具有透明、高效、低碳和语言无关的特点，可轻松适应其他健康监测场景，促进NLP在公共卫生系统中的伦理和可解释应用。
...
Conclusion: 这是首个专注于从医患对话中提取医疗指令的挑战，为减少临床文档负担提供了重要方向。
...
TL;DR: LOPSIDED框架是一个语义感知的隐私保护代理，通过动态替换用户提示中的敏感PII实体为语义一致的假名，在保护隐私的同时保持对话的上下文完整性。


<details>
  <summary>Details</summary>
Motivation: 随着对话AI系统的广泛使用，用户在与大语言模型交互时可能泄露包含个人身份信息(PII)的敏感数据，存在隐私泄露风险。

Method: 提出LOPSIDED框架，动态检测和替换敏感PII实体为语义一致的假名，模型生成响应后自动进行去假名化处理。

Result: 在ShareGPT真实对话数据集上的评估显示，相比基线方法，LOPSIDED将语义效用错误减少了5倍，同时增强了隐私保护。

Conclusion: LOPSIDED框架能够在保护用户隐私的同时保持对话的语义完整性，为大语言模型应用提供了有效的隐私保护解决方案。
...
TL;DR: 提出了一种基于代理的测试时对齐方法，使用小型对齐模型指导大型语言模型，通过0-1背包问题优化令牌级延迟决策，在任务性能和推测解码速度方面均有提升


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在预训练后仍需对齐以适应下游任务需求，但随着模型规模扩大，对齐过程的计算成本急剧增加

Method: 令牌级级联方法，将令牌特定延迟规则简化为0-1背包问题，推导最优延迟决策的原始和对偶近似

Result: 实验证明该方法在任务性能和推测解码速度方面均有显著优势

Conclusion: 代理测试时对齐方法能有效降低大型语言模型对齐的计算成本，同时保持或提升性能
...
TL;DR: ELM是一种用于紧凑语言模型的神经架构搜索方法，通过灵活的搜索空间和动态模块调整维度及头数，结合新颖的知识蒸馏损失，显著提升了模型效率。
...
Method: 扩展现有NAS方法，引入包含高效transformer块和动态维度/头数调整模块的灵活搜索空间，并使用新颖的知识蒸馏损失来保持各块特性。

Result: 在掩码语言建模和因果语言建模任务上的实验表明，ELM发现的模型显著优于现有方法。

Conclusion: ELM通过改进的搜索空间和蒸馏策略，成功开发出更高效的紧凑语言模型，解决了大型模型的计算和环境问题。
...
<details>
  <summary>Details</summary>
Motivation: 在线平台助长了各种形式的性别歧视，但在低资源语言中，由于语言资源有限和文化差异影响性别歧视的表达和感知，性别歧视检测进展有限。

Method: 通过两阶段用户研究(n=66)让母语者探索性别歧视在日常话语中的定义和表达，使用传统机器学习分类器和预训练多语言语言模型进行实验，评估少样本学习在豪萨语性别歧视检测中的有效性。

Result: 研究发现在捕捉文化细微差别方面存在挑战，特别是在寻求澄清和习语表达方面，许多情况下会出现误报。

Conclusion: 该研究强调了在低资源语言中开发性别歧视检测系统时考虑文化细微差别的重要性，并揭示了当前方法在处理澄清性表达和习语时的局限性。
...
TL;DR: 本文为量化互文性研究提供了路线图，总结了其数据、方法和应用，涵盖多语言和多主题数据，回顾了从统计到深度学习的方法，并展望了计算机技术驱动下更精确、多样和大规模的互文研究前景。


<details>
  <summary>Details</summary>
Motivation: 互文性作为文学理论中的重要概念，在数字人文学研究中具有重要理论基础。随着自然语言处理技术的发展，互文性研究进入了量化时代，需要系统总结其研究路线和方法。

Method: 基于多语言和多主题数据，综述了从统计方法到深度学习的各种量化互文性研究方法，并总结了相关平台工具。

Result: 系统梳理了量化互文性研究的数据、方法和应用，为相关研究提供了全面的路线图。

Conclusion: 在计算机技术进步的推动下，可以预期更精确、多样和大规模的互文研究，互文性有望在连接人工智能与人文学科的跨学科研究中获得更广泛应用。
...
TL;DR: 本文提出基于最小描述长度(MDL)的方法来评估数字系统的规律性和处理复杂度，认为递归数字系统在规律性和处理效率方面是最优的，而非之前的词汇大小与形态句法复杂度的权衡。


<details>
  <summary>Details</summary>
Motivation: 先前研究认为递归数字系统在词汇大小和平均形态句法复杂度之间达到最优权衡，但需要依赖临时约束来排除不自然的系统。本文认为问题在于忽略了规律性这一关键因素。

Method: 采用最小描述长度(MDL)方法来衡量数字系统的规律性和处理复杂度，比较自然语言系统与之前研究中"最优"但不自然的递归数字系统。

Result: 基于MDL的规律性和处理复杂度测量能更好地区分自然系统与不自然但可能的系统，先前文献中的临时约束可以从规律性中自然推导出来。

Conclusion: 在语言最优性研究中需要纳入形式集合的规律性因素，递归数字系统在规律性和处理复杂度方面表现出高效性。
...
TL;DR: VISTA是一个用于评估对话系统事实性的框架，通过声明级验证和序列一致性追踪来检测多轮对话中的幻觉问题。


<details>
  <summary>Details</summary>
Motivation: 现有指标要么评估孤立回复，要么将不可验证内容视为错误，限制了在多轮对话中的应用。幻觉问题仍然是部署对话AI系统的主要障碍。

Method: VISTA将每个助手回复分解为原子事实声明，根据可信来源和对话历史进行验证，并对不可验证陈述进行分类（主观、矛盾、缺乏证据或弃权）。

Result: 在8个大语言模型和4个对话事实性基准测试中，VISTA显著优于FACTSCORE和LLM-as-Judge基线方法。人工评估证实VISTA的分解提高了标注者一致性。
...
TL;DR: 提出一种结合多粒度记忆索引和不确定性估计的置信度控制方法，解决复杂知识环境下检索增强生成的覆盖不足、结果不稳定和可靠性有限的问题。


<details>
  <summary>Details</summary>
Motivation: 解决复杂知识环境中检索增强生成存在的覆盖不足、结果不稳定和可靠性有限的问题，需要一种能够动态索引知识并控制生成置信度的技术方案。

Method: 构建分层记忆结构，将知识表示划分为不同粒度级别，实现从局部细节到全局上下文的动态索引检索；引入不确定性估计机制，在生成过程中显式约束和过滤低置信度路径；优化目标包括生成损失、熵约束和方差正则化。

Result: 在QA准确性、检索召回率、排序质量和事实一致性方面优于现有模型，通过敏感性测试验证了方法在不同场景下的稳定性和鲁棒性。
...
TL;DR: CoDeC是一种通过上下文学习检测语言模型训练数据污染的方法，通过测量上下文学习对模型性能的影响来区分记忆数据和未见数据。
...
Method: 利用上下文学习对模型置信度的影响：上下文示例通常提升未见数据集的置信度，但可能降低训练数据集的置信度，因为会破坏记忆模式。

Result: CoDeC产生可解释的污染分数，能清晰区分已见和未见数据集，并在未公开训练语料的开放权重模型中揭示了强记忆证据。
...
<details>
  <summary>Details</summary>
Motivation: 解决大规模语言模型在安全对齐和鲁棒性方面的局限性，特别是在噪声和不确定输入下保持稳定预测输出的需求。

Method: 采用对比蒸馏方法冻结骨干模型，通过蒸馏损失、鲁棒性损失和正则化项的统一优化目标，结合噪声扰动和鲁棒优化约束进行训练。
...
TL;DR: 研究发现LLM安全护栏存在选择性拒绝偏见，对不同人口群体拒绝生成有害内容的程度不一致，这可能导致新的偏见问题。


<details>
  <summary>Details</summary>
Motivation: 探索LLM安全护栏是否对不同人口群体存在选择性拒绝偏见，以及这种偏见可能带来的安全风险。

Method: 通过分析针对不同人口群体的拒绝率、LLM响应类型和拒绝生成长度来评估选择性拒绝偏见。

Result: 发现LLM在性别、性取向、国籍和宗教等属性上存在选择性拒绝偏见，并可通过间接攻击利用这种偏见。

Conclusion: 需要开发更公平和鲁棒的安全护栏，确保在不同人口群体间的一致保护。
...
TL;DR: LLM作为NLG评估工具存在评分不一致性问题，但通过合理使用仍可发挥作用


<details>
  <summary>Details</summary>
Motivation: 随着NLG技术广泛应用，传统评估指标与人类偏好存在差距，LLM评估虽更接近人类判断但存在可靠性问题

Method: 通过实验量化LLM评估者在不同运行中的评分不一致性，分析跨NLG任务和基准的方差

Result: LLM评估者具有较低的内部评分者信度，评分在不同运行间存在显著差异，导致评估结果几乎随机

Conclusion: 尽管LLM评估存在不一致性问题，但通过制定合理使用指南仍可在NLG评估中发挥作用
...
TL;DR: 本文分析了transformer语言模型（而非语言识别器）的表达能力，揭示了自回归和概率特性如何影响其表达能力。


<details>
  <summary>Details</summary>
Motivation: 现有的transformer表达能力研究主要关注其作为语言识别器的功能，但实际应用中transformer主要作为语言模型使用，需要研究其作为概率性自回归生成模型的表达能力。

Method: 通过理论分析，比较transformer作为语言识别器和语言模型时的表达能力差异，特别关注自回归和概率特性对表达能力的影响。

Result: 研究发现：1）使transformer语言识别器具有自回归特性有时能增强其表达能力；2）概率化会打破非概率情况下的等价关系。

Conclusion: 本文厘清了transformer在作为语言模型这一主要应用场景下能够表达的函数类型，揭示了自回归和概率特性对其表达能力的重要影响。
...
TL;DR: 扩展URIEL+语言知识库，通过添加脚本向量、集成Glottolog语言数据和改进谱系插补，显著减少了数据稀疏性并提高了语言覆盖率。


<details>
  <summary>Details</summary>
Motivation: URIEL+语言知识库存在数据稀疏问题，包括缺失特征类型、不完整的语言条目和有限的谱系覆盖，这限制了其在跨语言迁移特别是低资源语言支持中的实用性。
...
Conclusion: 这些扩展使URIEL+更加完整和包容，为多语言研究提供了更好的支持。
...
TL;DR: 提出了MemeArena框架，通过基于代理的竞技场式评估方法，对多模态大语言模型在理解多模态有害内容方面进行上下文感知和无偏见的评估。


<details>
  <summary>Details</summary>
Motivation: 现有评估方法主要关注二元分类任务的检测准确率，无法反映不同上下文中对有害性的深入解释细微差别。

Method: MemeArena模拟多样化的解释上下文来制定评估任务，通过整合不同观点并在评估者之间达成共识，实现公平无偏的比较。

Result: 实验表明该框架有效减少了评估偏见，判断结果与人类偏好高度一致。

Conclusion: 该框架为多模态有害性理解提供了可靠且全面的mLLM评估方法。
...
<details>
  <summary>Details</summary>
Motivation: 探索自然语言在编码信息中展现周期性模式的程度，这是信息密度理论发展的延伸问题。

Method: 采用APS方法，这是一种基于典型周期性检测算法的技术，能够识别单个文档惊奇值序列中的任何显著周期。

Result: 发现相当比例的人类语言在信息上表现出强烈的周期性模式；识别并验证了超越典型文本结构单元（如句子边界、基本语篇单元等）分布的新周期。

Conclusion: 语言中信息的周期性是结构化因素与在更长距离上起作用的其他驱动因素共同作用的结果。该方法在LLM生成检测方面具有潜在应用价值。
...
TL;DR: 提出了BEAM基准测试和LIGHT框架，用于评估和改进LLM在长对话中的记忆能力。BEAM包含100个长对话和2000个问题，LIGHT框架为LLM配备了三种互补记忆系统，显著提升了模型在长对话中的表现。


<details>
  <summary>Details</summary>
Motivation: 现有基准测试缺乏叙事连贯性、覆盖领域狭窄且只测试简单的回忆任务，无法有效评估LLM在需要长期记忆的长上下文推理任务中的能力。

Method: 1) 开发BEAM基准：自动生成长达1000万token的连贯对话和多样化问题；2) 提出LIGHT框架：基于人类认知原理，为LLM配备长期情景记忆、短期工作记忆和事实积累便签三种记忆系统。

Result: 实验显示，即使具有100万token上下文窗口的LLM（无论是否使用检索增强）在对话变长时表现都会下降。而LIGHT框架在不同模型上均能提升性能，相比最强基线平均提升3.5%-12.69%。消融研究证实了每个记忆组件的贡献。

Conclusion: LIGHT框架通过模拟人类记忆系统，有效提升了LLM在长对话任务中的记忆能力，为解决长上下文推理问题提供了有效方案。
...
TL;DR: LLINK是一种计算高效的方法，通过将多语言编码器的句子嵌入对齐到解码器的潜在空间，改善低资源非拉丁文字LLM的性能，无需改变分词器或重新训练解码器。


<details>
  <summary>Details</summary>
Motivation: 指令调优的大语言模型在低资源非拉丁文字上表现不佳，主要由于分词器碎片化和弱跨语言耦合问题。
...
TL;DR: 提出了MedCalc-Eval基准测试，包含700多个医疗计算任务，并开发了MedCalc-Env强化学习环境来提升LLM在医疗计算方面的表现。


<details>
  <summary>Details</summary>
Motivation: 现有医疗领域LLM评估主要关注问答和描述性推理，忽视了临床决策中至关重要的定量推理能力。现有数据集如MedCalc-Bench覆盖的计算任务较少，无法反映真实世界的计算场景。

Method: 构建了包含700多个任务的MedCalc-Eval基准测试，分为基于方程的计算（如Cockcroft-Gault、BMI、BSA）和基于规则的评分系统（如Apgar、格拉斯哥昏迷量表）。同时开发了基于InternBootcamp框架的MedCalc-Env强化学习环境，支持多步临床推理和规划。

Result: 在MedCalc-Env环境中微调Qwen2.5-32B模型，在MedCalc-Eval上取得了最先进的结果，在数值敏感性、公式选择和推理鲁棒性方面都有显著提升。
...
TL;DR: 本文发现推理语言模型的多语言推理差距主要源于语言理解失败，并提出选择性翻译策略，仅当检测到理解失败时才进行翻译，在仅翻译约20%输入的情况下达到接近全翻译的性能。


<details>
  <summary>Details</summary>
Motivation: 推理语言模型在复杂推理任务上表现良好，但在高资源语言和低资源语言之间存在多语言推理差距，且其根本原因尚未被充分探索。

Method: 通过评估多种检测方法识别理解失败，提出选择性翻译策略，仅在检测到理解失败时将多语言输入翻译成英语。

Result: 选择性翻译能够弥合多语言推理差距，在仅翻译约20%输入的情况下达到接近全翻译的性能。

Conclusion: 理解失败是多语言推理差距的主要原因，可以通过检测和选择性缓解，为实现更公平的多语言推理提供了关键见解和可行路径。
...
TL;DR: 研究发现大型语言模型中的评估判断沿一个主导维度——情感-同意轴(VAA)进行计算，该轴同时编码主观情感和事实同意，揭示了推理从公正推断转向目标导向论证的机制。


<details>
  <summary>Details</summary>
Motivation: 探索智能系统是依赖专门模块还是统一通用资源进行判断，检验大型语言模型中可解码的神经表示是否真正独立。

Method: 通过分析多种大型语言模型，识别评估判断的主导维度，并进行直接干预实验验证VAA的功能。

Result: 发现情感-同意轴(VAA)作为控制信号引导生成过程构建与其评估状态一致的论证，即使牺牲事实准确性。

Conclusion: 揭示了促进连贯判断的架构如何系统性破坏忠实推理，为系统性偏见和幻觉提供了机制性解释。
...
<details>
  <summary>Details</summary>
Motivation: 在缺乏大规模训练数据的情况下，基于翻译的跨语言迁移策略需要标签投影，而现有词对齐器性能有限，特别是基于机器翻译模型的词对齐研究较少且效果不佳。

Method: 利用大规模多语言机器翻译模型的编码器构建TransAlign词对齐器，用于在翻译过程中映射原始句子和翻译句子之间的标签。

Result: TransAlign不仅实现了强大的词对齐性能，而且在基于机器翻译的跨语言迁移中显著优于流行的词对齐器和最先进的非词对齐标签投影方法。

Conclusion: TransAlign通过利用大规模多语言机器翻译模型的编码器，为跨语言迁移中的标签投影任务提供了更有效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法通常通过操纵隐藏表示来引导LLM生成，而本文旨在利用这些隐藏特征作为判别信号来更有效地探索推理路径。

Method: 在树状结构节点扩展中使用分类器对候选进行评分和排序，优先扩展高分候选；完成树扩展后收集所有分支答案，通过聚合CoT分数来识别最优答案。

Result: 实验结果表明该框架能有效覆盖和识别有效推理链，在多个算术推理基准上取得显著改进。

Conclusion: ThoughtProbe通过利用LLM隐藏推理特征进行系统性的树状空间探索，显著提升了模型的推理性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有研究多关注电池生命周期的单一阶段或方法，缺乏对整个生命周期中NLP应用的全面调查，且需要为数字电池护照等新兴需求提供技术支持。

Method: 采用PRISMA系统评价方法，使用Google Scholar、IEEE Xplore和Scopus三个数据库，评估274篇论文后最终评审66篇相关论文。

Result: 研究发现电池领域正在出现新的NLP任务，有助于材料发现和生命周期各阶段工作，但仍存在缺乏标准基准等挑战。

Conclusion: 提出的技术语言处理框架结合智能AI和优化提示，能够有效应对现有挑战，为电池领域NLP应用提供新思路。
...
TL;DR: IntAttn-Edit是一种知识编辑方法，通过联合更新MLP和注意力模块，使用知识平衡策略来提升编辑效果。
...
Method: 将关联记忆范式扩展到联合更新MLP和注意力模块，使用知识平衡策略按模块贡献度分配更新幅度。

Result: 在标准基准测试中，IntAttn-Edit实现了更高的编辑成功率、更好的泛化能力和更强的知识保留能力。

Conclusion: 注意力模块在知识存储中发挥重要作用，联合更新MLP和注意力模块的知识平衡策略能显著提升编辑性能。
...
TL;DR: Awal项目是一个社区驱动的塔马齐格特语语言技术资源开发计划，通过awaldigital.org平台收集翻译和语音数据，但18个月仅获得6,421个翻译对和3小时语音数据，显示标准众包方法在复杂社会语言环境中的局限性。
...
Result: 尽管获得积极反响，但实际数据贡献集中在语言学家和活动家群体，社区参与面临书面语言信心不足和标准化挑战等障碍。

Conclusion: 标准众包方法在复杂社会语言环境中的适用性有限，需要开发改进的开源机器翻译模型来利用收集的数据。
...
TL;DR: 提出基于贝叶斯启发式记忆更新算法和记忆熵概念的新型记忆管理系统，解决AI代理在个性化服务中的记忆冗余、陈旧和上下文整合问题。


<details>
  <summary>Details</summary>
Motivation: 当前AI代理系统依赖外部记忆数据库提供个性化体验，但面临记忆冗余、陈旧和上下文整合差的挑战，主要由于交互过程中缺乏有效的记忆更新机制。

Method: 采用贝叶斯启发式记忆更新算法，引入记忆熵概念，通过最小化全局熵使代理能够自主维护动态更新的记忆向量数据库。

Result: 实验结果表明，该系统在个性化、逻辑连贯性和准确性方面表现优异，消融研究验证了贝叶斯启发式更新机制在缓解记忆膨胀方面的有效性。
...
Conclusion: 基于Transformer的神经架构搜索方法结合多目标优化和辅助评估指标，能够有效发现性能更优的神经网络结构用于机器翻译任务。
...
TL;DR: VCORE是一种基于优化理论的token重加权框架，通过约束优化问题重新分配监督信号，提升LLMs在长推理链任务中的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 标准交叉熵损失对所有token一视同仁，忽略了推理轨迹中不同token的异质贡献，导致监督信号分配不当和泛化能力弱。
...
Conclusion: VCORE通过优化理论视角改进了CoT监督，为提升LLMs推理能力提供了更强的基础。
...
TL;DR: 本文研究了基于人类反馈的强化学习（RLHF）中奖励模型的前缀偏见问题，提出了检测和评估方法，揭示了种族和性别维度的显著偏见，并提出了数据增强策略来缓解这些偏见。


<details>
  <summary>Details</summary>
Motivation: 虽然有许多公开可用的偏好数据集提供响应对的比较，但由此产生的奖励模型可能存在偏见的问题尚未得到充分探索。本文旨在检测和评估LLM奖励模型中的前缀偏见。

Method: 引入了检测和评估前缀偏见的新方法，利用这些指标揭示偏好模型在种族和性别维度上的显著偏见。进行了跨多种开源偏好数据集和奖励模型架构的综合评估，并提出了数据增强策略来缓解偏见。

Result: 研究发现无论底层模型架构如何，奖励模型都容易受到这种偏见的影响。提出的数据增强策略在减少前缀偏见影响方面显示出有效性。
...
TL;DR: 提出了一种高效的协作推理框架，利用扩散语言模型生成候选思维，大语言模型评估质量，以解决自回归生成计算效率低的问题。


<details>
  <summary>Details</summary>
Motivation: 大语言模型的自回归生成范式导致推理性能随测试时间计算呈次优扩展，需要过多计算开销。扩散语言模型能够通过单次前向传播并行去噪高效生成多样样本，可用来生成中间思维，减轻计算负担。

Method: 提出协作推理框架：扩散语言模型生成候选思维，大语言模型评估这些思维的质量。通过这种分工，在保持推理质量的同时显著降低计算成本。

Result: 在多样化基准测试上的实验表明，该框架在复杂推理任务中实现了强大的性能。

Conclusion: 该协作推理框架为未来研究提供了有前景的方向，通过结合扩散语言模型和大语言模型的优势，实现了高效且高质量的推理。
...
<details>
  <summary>Details</summary>
Motivation: 研究计算嵌入与人类语义判断在英语复合词处理中的对齐程度，推动计算心理语言学发展。

Method: 使用GloVe和BERT嵌入，结合关联强度、频率和可预测性指标，计算嵌入衍生的LMD和ST指标，通过Spearman相关和回归分析与人类判断的关系。
...
TL;DR: 本文研究了两种因果域泛化方法在低资源自然语言任务中的应用：因果数据增强通过生成反事实样本来提升对伪相关的鲁棒性；不变因果表示学习通过DINER框架改善多语言情感分析的分布外性能。


<details>
  <summary>Details</summary>
Motivation: 现实场景中训练和测试数据分布不一致的问题在低资源设置下尤为突出，传统机器学习假设失效，需要开发能够跨域泛化的鲁棒模型。

Method: 1. 因果数据增强：自动生成反事实样本来模拟受控分布偏移；2. 不变因果表示学习：使用DINER框架学习跨域不变的特征表示。

Result: 两种方法均提升了未见域的鲁棒性：反事实数据增强在情感分类中带来一致的跨域准确率提升；DINER在多语言情感分析中改善了分布外性能，但不同语言间增益存在差异。

Conclusion: 因果域泛化方法在低资源自然语言处理任务中有效，能够提升模型对分布偏移的鲁棒性，为实际应用提供了有前景的解决方案。
...
Method: 使用稀疏注意力降低计算成本，自适应跨度动态调整注意力范围，双线性注意力在精炼上下文中建模复杂token交互。
...
TL;DR: SQLSpace是一个人类可解释、可泛化、紧凑的文本到SQL示例表示方法，通过最小人工干预获得。它用于评估文本到SQL基准测试的组成差异、理解模型性能细节以及通过针对性查询重写提升模型性能。


<details>
  <summary>Details</summary>
Motivation: 开发一种能够深入分析文本到SQL任务的方法，超越简单的准确率评估，揭示基准测试的组成差异和模型性能的细粒度模式。

Method: 提出SQLSpace表示方法，通过最小人工干预从文本到SQL示例中提取人类可解释的特征表示。

Result: SQLSpace能够揭示不同基准测试之间的组成差异，暴露仅靠准确率评估无法发现的性能模式，并支持查询成功的建模。

Conclusion: SQLSpace为文本到SQL任务提供了比原始示例更深入的分析能力，支持更细致的基准比较、性能分析和模型改进。
...
TL;DR: 本文提出了以患者为中心的临床摘要(PCS)新标准，通过混合方法开发框架，评估开源LLM在此任务上的表现，发现模型在完整性和流畅度上接近人类水平，但在准确性和以患者为中心方面仍有差距。


<details>
  <summary>Details</summary>
Motivation: 现有LLM生成的临床摘要过于关注患者生物学信息，而忽略了患者的偏好、价值观、愿望和关切，无法实现真正的以患者为中心的护理。

Method: 采用混合方法：通过患者和临床医生的半结构化访谈确定PCS内容标准，制定标注指南，由临床医生创建88份房颤咨询的黄金标准PCS，使用5个开源LLM进行零样本和少样本提示生成摘要，并通过ROUGE-L、BERTScore和定性指标评估。

Result: 患者强调生活方式、社会支持、近期压力源和护理价值观；临床医生需要简洁的功能、心理社会和情感背景。Mistral-8B和Llama-3.1-8B在零样本表现最佳，Llama-3.1-8B在少样本表现最佳。模型在完整性和流畅度上与专家相似，但在正确性和以患者为中心方面不如人类PCS。

Conclusion: 开源LLM在生成以患者为中心的临床摘要方面展现出潜力，但在准确捕捉患者价值观和确保临床实用性方面仍需改进，需要进一步研究来提升模型在此关键任务上的性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有阿拉伯语基准主要关注现代标准阿拉伯语(MSA)，而日常交流中广泛使用的方言变体评估不足，需要更全面的评估框架。
...
TL;DR: 该研究探讨了在医疗领域对多语言BERT模型进行领域自适应预训练对低资源语言医疗NLP任务性能的影响，发现领域适应能显著提升任务表现，且临床领域模型优于通用生物医学领域模型。


<details>
  <summary>Details</summary>
Motivation: 解决低资源语言在医疗领域NLP工具缺乏的问题，探索领域自适应预训练如何提升多语言BERT模型在医疗任务中的性能。

Method: 对多语言BERT模型进行四种领域自适应预训练实验，然后在三个下游任务上微调：荷兰语临床笔记的自动患者筛查、罗马尼亚语和西班牙语临床笔记的命名实体识别。

Result: 领域自适应显著提升了任务性能，临床领域适应模型优于通用生物医学领域适应模型，并观察到跨语言可迁移性的证据。

Conclusion: 领域适应和跨语言能力在医疗NLP中是可行的，这些发现可为开发多语言医疗NLP系统提供指导，以缓解训练数据不足问题。
...
TL;DR: MARAG-R1是一个基于强化学习的多工具RAG框架，通过动态协调多种检索机制来解决单检索器RAG系统信息获取受限的问题，在语料库级推理任务中达到新的SOTA效果。
...
Method: 提出MARAG-R1框架，配备四种检索工具（语义搜索、关键词搜索、过滤、聚合），通过两阶段训练（监督微调+强化学习）让模型学习如何及何时使用这些工具，实现推理与检索的交错进行。

Result: 在GlobalQA、HotpotQA和2WikiMultiHopQA上的实验表明，MARAG-R1显著优于强基线方法，在语料库级推理任务中取得了新的SOTA结果。

Conclusion: 多工具RAG框架通过动态协调多种检索机制，能够更全面、精确地访问外部知识，有效提升了LLM在语料库级推理任务中的表现。
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型在推理时面临计算瓶颈，特别是自注意力机制的二次复杂度随上下文长度增加而显著增长。

Method: 使用KL散度进行草稿与目标模型的层对齐、GPU优化的无排序top-p token选择算法、基于预测的动态KV缓存剪枝。

Result: 在PG-19数据集上实现75%以上的KV缓存访问减少，困惑度仅增加15.29%，显著优于现有稀疏注意力方法。

Conclusion: 推测执行可以通过近似验证来增强，而不会造成显著的性能下降。
...
TL;DR: 提出了一种名为CultureCartography的混合倡议方法，让LLM和人类协作发现文化特定知识，通过LLM提出低置信度问题，人类填补知识空白并引导话题方向。


<details>
  <summary>Details</summary>
Motivation: LLM需要文化特定知识来安全有效地服务全球用户，但这些知识可能在预训练中未学习到。需要找到对群体内用户重要但LLM未知的知识。

Method: 使用CultureExplorer工具实现CultureCartography方法：LLM初始化标注，提出低置信度答案的问题；人类受访者填补空白并通过直接编辑引导模型关注重要话题。

Result: 相比人类仅回答LLM提出问题的基线，CultureExplorer更有效地产生领先模型（如DeepSeek R1和GPT-4o）缺失的知识，即使有网络搜索。基于这些数据微调使Llama-3.1-8B在相关文化基准上的准确率提升高达19.2%。

Conclusion: 混合倡议协作方法能更有效地识别和填补LLM的文化知识空白，提升模型在文化特定任务上的表现。
...
TL;DR: CALM模型通过将离散的token预测转变为连续的向量预测，使用自编码器将K个token压缩为单个连续向量，从而将生成步骤减少K倍，显著提升计算效率。


<details>
  <summary>Details</summary>
Motivation: 解决大语言模型逐token生成的效率瓶颈，通过增加每个生成步骤的语义带宽来提高模型效率。
...
TL;DR: 提出了CATArena评估平台，通过开放式计分的棋牌游戏来持续评估LLM智能体的学习能力，解决了现有基准测试中的分数饱和问题。


<details>
  <summary>Details</summary>
Motivation: 当前基准测试主要评估固定场景下的端到端性能，存在分数饱和和依赖专家标注的问题，无法有效评估智能体的学习能力这一核心进化驱动力。

Method: 提出迭代式竞争性同伴学习框架，让智能体通过重复交互和反馈优化策略；构建CATArena平台，包含四种开放式计分的棋牌游戏，提供无明确上限的任务。

Result: 实验结果表明，CATArena能够为智能体核心能力（特别是学习能力和策略编码）提供可靠、稳定和可扩展的基准测试。

Conclusion: CATArena平台通过开放式任务设计有效解决了基准测试的分数饱和问题，为评估LLM智能体的学习能力提供了系统性的解决方案。
...
TL;DR: 提出了一个可扩展的框架，通过构建可验证的长思维链知识库来解压缩科学推理，创建了SciencePedia百科全书。该方法使用苏格拉底式代理生成数百万个基本原理问题，通过多模型验证确保高质量，最终合成连贯的科学文章。


<details>
  <summary>Details</summary>
Motivation: 现有科学材料压缩了推理过程，只呈现结论而省略推导链，这阻碍了验证和跨领域连接。需要解压缩科学推理以建立可验证的知识基础。

Method: 采用端点驱动的还原策略：苏格拉底代理生成约300万个基本原理问题，多独立求解器模型生成长思维链，通过提示净化和跨模型答案共识进行严格过滤，仅保留可验证端点的内容。

Result: 构建了包含约20万个细粒度条目的SciencePedia，涵盖数学、物理、化学、生物、工程和计算等领域。评估显示，基于检索长思维链合成的文章知识点密度显著更高，事实错误率显著降低。
...
<details>
  <summary>Details</summary>
Motivation: 开发一个能够跨多个科学领域进行端到端科学分析的AI研究助手，促进跨学科研究并提高科研效率。

Method: 采用模块化架构，结合Cmbagent作为深度研究后端，能够处理特定任务或进行完整的科学分析流程。

Result: 系统在多个科学领域生成了AI论文，包括天体物理学、生物学、生物物理学等，并通过领域专家评估获得了数值评分和评审反馈。

Conclusion: Denario展示了AI驱动研究的潜力，但也存在局限性，需要进一步探讨AI科研的伦理影响及其与科学哲学的关系。
...
TL;DR: 提出认知包络概念，为AI决策建立推理边界，以解决基础模型在物理信息系统中产生的幻觉、过度泛化和上下文错位等问题。


<details>
  <summary>Details</summary>
Motivation: 基础模型在物理信息系统中增强了感知、推理和规划能力，但也引入了新的错误类型，导致错误决策。

Method: 引入认知包络概念，建立推理边界来约束AI生成的决策，同时补充元认知和传统安全包络的使用。

Result: 提出了认知包络的概念框架，类似于安全包络，需要实用的指导方针和系统化流程来定义、验证和保证。

Conclusion: 认知包络为解决基础模型在物理信息系统中产生的错误决策提供了系统化的安全保障方法。
...
<details>
  <summary>Details</summary>
Motivation: 全球农业面临食品需求增长、气候多变性和可持续实践需求的挑战，需要智能解决方案来应对这些挑战。

Method: 平台整合物联网、AI、卫星成像和基于角色的任务编排，特别针对地中海葡萄园设计，包括卫星指数集成、实时环境数据和角色感知任务管理。
...
TL;DR: 在非序列数据上应用因果掩码是可行的，在某些领域甚至比序列化方法更优


<details>
  <summary>Details</summary>
Motivation: 研究在具有空间或关系结构的领域中，是否可以在非序列数据上接受因果掩码引入的信息损失，以及这种方法是否可行
...
TL;DR: 提出Adaptive Effort Control方法，通过强化学习训练模型根据用户指定的相对思考比例自动分配推理资源，实现3倍推理长度减少的同时保持或提升性能。


<details>
  <summary>Details</summary>
Motivation: AI模型增加思考预算可提高准确性，但不同问题需要不同推理量。用户需要在输出质量、延迟和成本之间权衡，但现有方法缺乏细粒度控制，需要预先知道问题难度来设置token预算。

Method: 基于强化学习的自适应努力控制方法，训练模型使用用户指定的相对于当前平均思维链长度的token比例，无需数据集和阶段特定调优。

Result: 在1.5B到32B参数规模的模型上，该方法实现了约3倍的思维链长度减少，同时保持或改进了相对于RL训练基础模型的性能。

Conclusion: 该方法有效解决了推理资源分配的细粒度控制问题，用户可通过连续努力参数动态调整成本-准确性权衡，模型自动按任务难度比例分配资源。
...
TL;DR: 在NVIDIA的企业AI助手NVInfo中实施数据飞轮机制，通过MAPE驱动的闭环系统持续改进RAG管道的性能，使用微调方法显著提升了路由和查询重述的准确性和延迟表现。


<details>
  <summary>Details</summary>
Motivation: 企业AI代理需要持续适应以保持准确性、降低延迟并符合用户需求，通过数据飞轮机制实现持续学习和自我改进。

Method: 采用MAPE驱动的数据飞轮构建闭环系统，通过收集用户反馈识别故障模式，使用NVIDIA NeMo微服务进行针对性微调改进。

Result: 3个月收集495个负样本，识别出路由错误(5.25%)和查询重述错误(3.2%)。微调后路由模型准确率达96%，模型大小减少10倍，延迟改善70%；查询重述准确率提升3.7%，延迟降低40%。

Conclusion: 人机回环反馈结合数据飞轮机制可将企业AI代理转变为自我改进系统，为构建稳健、自适应的企业AI代理提供了可重复的蓝图。
...
TL;DR: 该论文评估了SOTA LLMs在证明评分方面的能力，提出了基于代理工作流程的多步骤评分方法，能够更准确地检测错误、判断严重程度并分配公平分数。


<details>
  <summary>Details</summary>
Motivation: 随着LLMs在奥林匹克数学问题上取得显著进展，需要评估这些模型在证明评分方面的能力，包括错误检测、严重程度判断和公平分数分配，而不仅仅是二元正确性判断。

Method: 使用90个Gemini 2.5 Pro生成的解决方案和MathArena数据集，引入代理工作流程来提取和分析参考解决方案，自动推导问题特定的评分标准，实现多步骤评分过程。
...
Result: 在分布式GPU集群的LLM推理应用中，Glia生成了新的请求路由、调度和自动扩展算法，性能达到人类专家水平且耗时显著减少，同时提供了对工作负载行为的新见解
...
TL;DR: 本文提出了一种面向系统之系统(SoS)生命周期管理的实用框架，强调从传统线性产品生命周期向网络化系统开发的转变，包含MBSE、PLM、CAD-CAE等核心组件，并提出了四项实施原则和三步路线图。
...
Method: 基于当前文献和行业经验，提出以MBSE为语义骨干、PLM为治理配置层、CAD-CAE为模型衍生域、数字线程和数字孪生为持续反馈的参考框架，并制定四项原则和三步实施路线图。

Result: 实施该框架可提高变更鲁棒性、缩短流转时间、改善重用能力，并支持基于可持续性的明智决策。
...
TL;DR: 提出了一种细粒度、实例定制的个性化适配框架，通过动态生成样本级干预向量注入模型前向传播，解决了现有方法在处理动态用户模式和数据稀疏场景时的局限性。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型个性化需求日益增长，但现有参数化适配方法在应对动态用户模式和高数据稀疏场景时存在适应性差和数据效率低的问题。

Method: 采用细粒度引导组件从注意力层和MLP层捕获细微信号，结合输入感知聚合模块将这些信号合成为上下文相关的增强，作为插件组件兼容不同个性化技术。

Result: 在短到长文本生成和网页函数调用等多样化场景中的实验验证了方法的有效性，在快速变化环境中显著提升个性化性能，同时在不同交互模式和上下文长度下保持鲁棒性。

Conclusion: 该方法具有高灵活性和数据效率，能够有效应对快速变化的分布和高数据稀疏场景，且与现有方法正交兼容，为LLM个性化提供了新的解决方案。
...
TL;DR: 提出了GUI-Rise框架，通过结构化推理、动作预测和历史总结的集成，提升了GUI导航代理的跨领域泛化能力和历史利用效率。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在GUI导航代理中存在跨领域泛化能力不足和历史利用效率低的问题。

Method: 开发了推理增强框架，包含结构化推理链、动作预测和历史总结组件，通过监督微调伪标签轨迹和GRPO强化学习进行训练。

Result: 在标准基准测试中实现了最先进的结果，特别是在跨领域场景下表现优异，验证了框架的鲁棒推理和泛化能力。

Conclusion: 该框架能够有效提升GUI导航代理的跨任务泛化性能，通过系统化的推理和历史管理机制实现更稳健的导航能力。
...
TL;DR: 提出了三种奖励机器（RM）的泛化形式：数值RM、议程RM和耦合RM，以及新的组合学习算法CoRM，用于解决长时程问题中无序子任务的学习效率问题。


<details>
  <summary>Details</summary>
Motivation: 传统奖励机器在处理长时程问题中无序子任务时，学习信息量随子任务数量呈指数级增长，限制了其可扩展性。

Method: 引入三种RM泛化：数值RM（紧凑表达复杂任务）、议程RM（跟踪剩余子任务）、耦合RM（子任务状态耦合）；提出CoRM组合学习算法。

Result: 实验表明CoRM在长时程无序子任务问题上比现有RM算法具有更好的可扩展性。

Conclusion: 所提出的RM泛化形式和CoRM算法有效解决了长时程无序子任务的学习挑战，显著提升了学习效率。
...
TL;DR: 提出了一种基于结果感知的过程发现方法，通过区分理想和不理想的过程执行轨迹，分别学习过程模型，从而更好地理解过程结果的关键驱动因素。


<details>
  <summary>Details</summary>
Motivation: 传统过程发现方法不考虑执行结果差异，导致模型无法捕捉关键行为差异，不利于合规检查和性能分析。区分理想与不理想执行轨迹可以指导更有效的过程发现。

Method: 通过控制流特征学习可解释的判别规则，将具有相似理想性特征的轨迹分组，然后在每个组内分别应用过程发现技术。

Result: 该方法在多个真实事件日志上得到验证，能够有效分离和可视化关键过程模式，生成聚焦且可解释的模型。

Conclusion: 结果感知的过程发现方法能够揭示理想和不理想执行的关键驱动因素，为过程改进提供更有针对性的见解。
...
TL;DR: 重新评估LLM在数学发现中的贡献，发现LLM生成的装箱问题启发式算法虽然可读但难以理解，且问题实例本身相对简单，需要更严格的验证。
...
Result: 新算法比LLM生成的算法更简单、高效、可解释且泛化能力更强，表明问题实例本身相对简单。

Conclusion: LLM在此问题上的贡献有限，需要更严格的验证和情境化来评估LLM生成输出的科学价值。
...
TL;DR: ToolScope是一个代理框架，通过统一全局规划和局部多模态感知来增强多模态大语言模型使用外部工具的能力，在多个VQA基准测试中平均性能提升达6.69%。
...
Method: ToolScope包含三个主要组件：全局导航器提供高层战略指导，代理执行器通过集成搜索、代码和感知工具迭代增强MLLM的局部感知能力，响应合成器将推理过程整合为连贯输出。

Result: 在四个VQA基准测试（VQA 2.0、ScienceQA、MAT-Search和MathVista）上评估，展示了强大的泛化能力，平均性能提升达+6.69%。

Conclusion: ToolScope通过统一全局规划和局部多模态感知，有效解决了多模态大语言模型在长视野VQA任务中视觉上下文退化的问题，显著提升了模型性能。
...
TL;DR: 提出一个结合视觉和运动约束的多智能体强化学习框架，用于模拟行人-驾驶员交互，在真实数据集上验证了约束模型能产生更真实的行为。
...
Result: 结合视觉和运动约束的模型表现最佳，运动约束产生更平滑的人类速度调整，视觉约束引入感知不确定性导致更谨慎行为，在数据有限情况下优于监督学习方法。

Conclusion: 带有人类约束的多智能体强化学习是模拟真实道路使用者交互的有前景方法，能够考虑个体差异并无需大量训练数据。
...
TL;DR: 提出Nous智能体，通过主动询问来减少用户意图不确定性，基于信息论原理训练，无需人工偏好标注，在科学图表生成任务中表现优异。


<details>
  <summary>Details</summary>
Motivation: 解决人机协作中的"意图表达鸿沟"问题，即用户难以向AI有效传达复杂高维想法，导致低效试错循环。

Method: 采用苏格拉底式协作范式，训练Nous智能体主动探询信息；基于信息论框架，将对话信息增益定义为内在奖励信号，等同于结构化任务空间中香农熵的减少。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法使用监督微调或带长度奖励的强化学习来提高效率，但往往以牺牲准确性为代价。大型推理模型存在认知效率低下的问题，如对简单问题"过度思考"和对复杂问题"思考不足"。
...
Conclusion: 该研究表明，自适应调整推理链长度可以同时提升模型准确性和效率，挑战了传统偏好较短推理路径的做法。
...
TL;DR: TempoBench是一个首个基于形式化基础的、可验证的诊断基准，通过参数化难度来系统分析LLM的推理能力，包含时间轨迹评估（TTE）和时间因果评估（TCE）两个基准。


<details>
  <summary>Details</summary>
Motivation: 现有方法在改进LLM推理时存在局限性：临时生成的数据集可能编码偏差且无法形式化验证，而Lean等证明系统虽然可验证但不适合捕捉基于决策链的代理任务。这导致了在业务代理或代码助手等功能上的性能差距。

Method: TempoBench使用两个评估基准：时间轨迹评估（TTE）测试LLM理解和模拟多步推理系统执行的能力；时间因果评估（TCE）测试LLM进行多步因果推理和从复杂系统中提取因果关系的能力。

Result: 模型在TCE-normal上得分65.6%，在TCE-hard上仅得7.5%，表明最先进的LLM能理解TCE任务，但随着系统复杂度增加表现显著下降。

Conclusion: TempoBench填补了现有推理基准在推理结构和现实世界对齐方面的不足，为系统分析LLM推理能力提供了形式化基础的可验证诊断工具。
...
TL;DR: SIGMA是一个多代理框架，通过独立推理、定向搜索和调解机制整合知识，在数学推理任务中显著优于现有系统。


<details>
  <summary>Details</summary>
Motivation: 当前检索增强模型存在单视角依赖、搜索策略僵化、多源信息整合困难等问题，需要更灵活有效的知识集成方法。

Method: 采用多代理框架，各代理从不同分析视角独立推理，生成假设性段落优化检索，通过调解机制整合发现。

Result: 在MATH500、AIME和GPQA等基准测试中，SIGMA比开源和闭源系统性能提升7.4%，显著提高了推理准确性和效率。

Conclusion: 多代理按需知识集成方法能有效提升复杂知识密集型问题的解决能力，具有可扩展性。
...
TL;DR: 提出了InnovatorBench基准平台和ResearchGym研究环境，用于评估AI代理在LLM研究中的端到端能力，发现前沿模型在代码驱动任务中表现良好，但在算法相关任务和长期决策中存在困难。


<details>
  <summary>Details</summary>
Motivation: 现有基准在简化环境中测试狭窄技能，需要更现实的端到端评估平台来加速AI驱动的科学发现。

Method: 开发InnovatorBench包含20个任务，涵盖数据构建、过滤、增强、损失设计、奖励设计和支架构建，并创建ResearchGym环境支持代理操作，实现轻量级ReAct代理。

Result: 前沿模型在代码驱动研究任务中表现有希望，但在脆弱算法相关任务和长期决策（如急躁、资源管理差、过度依赖模板推理）中表现挣扎，代理需要超过11小时才能达到最佳性能。
...
Result: 在VerilogEval 2.0和RTLLM 2.0基准测试中，VeriMoA实现了Pass@1指标15-30%的提升，特别是使较小模型能够匹配较大模型和微调替代方案，无需昂贵训练。

Conclusion: VeriMoA通过创新的多智能体框架有效解决了HDL生成的挑战，为硬件设计自动化提供了高效且无需训练的解决方案。
...
TL;DR: BEAT是首个针对多模态大语言模型(MLLM)驱动具身代理的视觉后门攻击框架，使用环境中的物体作为触发器，在触发出现时让代理持续执行攻击者指定的多步策略。


<details>
  <summary>Details</summary>
Motivation: 随着MLLM驱动的具身代理能够直接从视觉输入进行感知、推理和规划，这种视觉驱动的具身代理开启了新的攻击面：视觉后门攻击，即代理在正常行为直到视觉触发器出现在场景中，然后持续执行攻击者指定的策略。

Method: BEAT通过(1)构建跨越多样化场景、任务和触发器放置的训练集来暴露代理于触发器变异性，(2)引入两阶段训练方案：先应用监督微调(SFT)，然后使用新颖的对比触发器学习(CTL)，将触发器判别制定为触发存在和触发自由输入之间的偏好学习。

Result: 在各种具身代理基准测试和MLLM上，BEAT实现了高达80%的攻击成功率，同时保持强大的良性任务性能，并能可靠地泛化到分布外的触发器放置。与朴素SFT相比，CTL在有限后门数据下将后门激活准确率提升高达39%。

Conclusion: 这些发现揭示了基于MLLM的具身代理中关键但未被探索的安全风险，强调了在现实世界部署之前需要强大的防御措施。
...
TL;DR: 本文提出了一个新的Agentic AI定义，强调其作为软件交付机制的性质，并指出验证比基础模型本身更重要。


<details>
  <summary>Details</summary>
Motivation: 当前对Agentic AI的定义不够清晰，需要重新定义以反映其在企业环境中的实际应用价值。

Method: 提出基于现实主义的Agentic AI定义，将其视为在复杂企业环境中自主工作的软件交付机制。

Result: 发现Agentic AI系统主要是应用而非基础，其成功取决于终端用户和主要利益相关者的验证。

Conclusion: Agentic AI的关键在于验证，而非基础模型本身；在良好验证下，LLMs可被更简单、快速、可解释的模型替代。
...
<details>
  <summary>Details</summary>
Motivation: 现有训练LLM智能体的方法存在两个主要问题：基于密集人工标注的行为克隆方法成本过高，而基于结果驱动的采样方法在领域专业化任务中容易因有效轨迹稀少而失败。

Method: Apollo框架允许人类在智能体偏离有希望轨迹时进行干预，提供先验知识、策略建议等，然后通过监督控制过滤次优动作，防止错误传播。

Result: 在InnovatorBench上的实验表明，使用Apollo训练的GLM-4.5模型相比未训练基线提升超过50%，相比无人机交互变体提升28%。
...
TL;DR: MolChord是一个基于结构的药物设计方法，通过整合蛋白质和分子结构的文本描述与序列表示，结合偏好数据优化分子生成，在CrossDocked2020数据集上取得最先进性能。


<details>
  <summary>Details</summary>
Motivation: 解决蛋白质结构表示与分子表示的有效对齐问题，确保生成的药物与其药理学特性保持一致，这是药物发现中的关键挑战。

Method: 使用NatureLM作为分子生成器，结合扩散结构编码器对齐蛋白质和分子结构；通过整合偏好数据构建属性感知数据集，并采用直接偏好优化(DPO)改进对齐过程。

Result: 在CrossDocked2020数据集上的实验结果表明，该方法在关键评估指标上达到了最先进的性能。

Conclusion: MolChord展示了作为实用SBDD工具的潜力，能够有效生成具有所需药理学特性的候选分子。
...
### [156] [Category-Aware Semantic Caching for Heterogeneous LLM Workloads](https://arxiv.org/abs/2510.26835)
*Chen Wang,Xunzhuo Liu,Yue Zhu,Alaa Youssef,Priya Nagpurkar,Huamin Chen*

Main category: cs.DB

TL;DR: 提出类别感知语义缓存系统，通过按查询类别动态调整相似度阈值、TTL和配额，解决传统统一缓存策略在异构查询负载中的问题，使低命中率类别也能经济可行地缓存。


<details>
  <summary>Details</summary>
Motivation: LLM服务系统处理异构查询负载，不同类别查询在嵌入空间分布、内容陈旧度和重复模式上差异很大。传统统一缓存策略导致密集空间假阳性、稀疏空间遗漏有效改写、内存浪费或数据陈旧等问题，且向量数据库因远程搜索成本高而排除长尾查询。

Method: 采用类别感知语义缓存架构，相似度阈值、TTL和配额按查询类别变化。使用混合架构分离内存HNSW搜索和外部文档存储，将未命中成本从30ms降至2ms。还包含自适应负载策略，根据下游模型负载动态调整参数。

Result: 未命中成本大幅降低使低命中率类别经济可行（盈亏平衡点从15-20%降至3-5%），实现整个工作负载分布的缓存覆盖。理论预测显示自适应负载策略可将过载模型流量减少9-17%。

Conclusion: 类别感知语义缓存能有效处理异构查询负载，通过降低未命中成本和动态参数调整，使缓存覆盖所有查询类别，提高系统整体效率。
...
### [157] [SpotIt: Evaluating Text-to-SQL Evaluation with Formal Verification](https://arxiv.org/abs/2510.26840)
*Rocky Klopfenstein,Yang He,Andrew Tremante,Yuepeng Wang,Nina Narodytska,Haoze Wu*

Main category: cs.DB

TL;DR: 提出SpotIt评估管道，通过形式化有界等价验证主动寻找能区分生成SQL和真实SQL的数据库，揭示传统基于测试的评估方法可能忽略查询差异的问题。


<details>
  <summary>Details</summary>
Motivation: 当前基于测试的Text-to-SQL评估方法存在乐观偏差，因为两个不同的SQL查询可能在测试数据库上偶然产生相同结果，无法准确反映模型性能。

Method: 开发SpotIt评估管道，扩展现有验证器以支持更丰富的SQL子集，通过有界等价验证主动搜索能区分查询的数据库。

Result: 在BIRD数据集上评估10种Text-to-SQL方法，发现基于测试的方法经常忽略生成查询与真实查询之间的差异。

Conclusion: 验证结果揭示了当前Text-to-SQL评估的复杂性，SpotIt方法能提供更准确和可靠的性能评估。
...
### [158] [The Impact of Data Compression in Real-Time and Historical Data Acquisition Systems on the Accuracy of Analytical Solutions](https://arxiv.org/abs/2510.26868)
*Reham Faqehi,Haya Alhuraib,Hamad Saiari,Zyad Bamigdad*

Main category: cs.DB

TL;DR: 本文评估了工业物联网环境中数据压缩机制对分析准确性的影响，指出过度压缩会丢失关键模式、扭曲统计指标并降低预测精度，提出了在分析完整性和压缩效率之间取得平衡的最佳实践。


<details>
  <summary>Details</summary>
Motivation: 工业物联网环境产生大量实时和历史过程数据，数据压缩虽然能降低存储成本，但可能影响工程分析的准确性和可靠性。理解这种权衡对于制定既支持运营效率又保证分析准确性的数据策略至关重要。

Method: 通过理论分析、模拟信号压缩和实证评估，研究常见压缩机制对统计分析、异常检测和机器学习模型准确性的影响。

Result: 研究表明过度压缩会丢失关键模式、扭曲统计指标并降低预测精度，压缩与分析的准确性存在显著关联。

Conclusion: 需要在分析完整性和压缩效率之间找到平衡点，提出了实现这种平衡的最佳方法和实践建议。
...
### [159] [Unstructured Data Analysis using LLMs: A Comprehensive Benchmark](https://arxiv.org/abs/2510.27119)
...
TL;DR: UDA-Bench是首个用于无结构化数据分析的综合基准测试，包含5个领域的数据集和多样化查询工作负载，用于评估LLM驱动的数据系统。


<details>
  <summary>Details</summary>
Motivation: 当前无结构化数据分析系统在查询接口、优化策略和操作实现等方面差异很大，缺乏统一的评估基准来比较不同系统的性能。

Method: 组织30名研究生花费10,000小时手动标注5个领域的数据集，构建关系数据库作为基准测试的ground truth，并设计多样化查询来评估系统性能。

Result: 创建了包含高质量、大规模、多样化数据集和丰富查询工作负载的基准测试，能够全面评估无结构化数据分析系统的关键组件。

Conclusion: UDA-Bench填补了无结构化数据分析系统评估的空白，为系统比较和技术分析提供了标准化的测试平台。
...
### [160] [Compass: General Filtered Search across Vector and Structured Data](https://arxiv.org/abs/2510.27141)
...
Method: 利用现有索引结构（HNSW和IVF用于向量属性，B+树用于关系属性），实施原则性协作查询执行策略，协调多模态的候选生成和谓词评估。

Result: 综合实证评估表明，Compass在多样化混合查询工作负载中始终优于现有唯一高性能通用框架NaviX，同时在仅涉及单一属性的场景中匹配专用单属性索引的查询吞吐量。
...
### [161] [ShapleyPipe: Hierarchical Shapley Search for Data Preparation Pipeline Construction](https://arxiv.org/abs/2510.27168)
*Jing Chang,Chang Liu,Jinbin Huang,Shuyuan Zheng,Rui Mao,Jianbin Qin*

Main category: cs.DB

TL;DR: ShapleyPipe是一个基于博弈论Shapley值的数据准备管道自动化构建框架，通过分层分解将搜索复杂度从指数级降低到多项式级，在减少24%评估次数的同时达到98.1%的高预算基线性能。


<details>
  <summary>Details</summary>
Motivation: 现有数据准备管道自动化方法存在两个根本局限：将管道构建视为黑盒优化而无法量化单个操作符的贡献，以及面临搜索空间的组合爆炸问题（N^M配置）。

Method: 使用博弈论Shapley值系统量化每个操作符的边际贡献，采用分层分解将类别级结构搜索与操作符级优化分离，开发多臂老虎机机制进行智能类别评估和置换Shapley值来捕捉位置依赖的操作符交互。

Result: 在18个不同数据集上的广泛评估显示，ShapleyPipe使用24%更少的评估达到98.1%的高预算基线性能，比最先进的强化学习方法性能提升3.6%，并提供可解释的操作符估值（与经验性能相关性ρ=0.933）。

Conclusion: ShapleyPipe不仅实现了性能提升，还提供了可解释的操作符估值，支持数据驱动的管道分析和系统化的操作符库优化。
...
### [162] [DRAMA: Unifying Data Retrieval and Analysis for Open-Domain Analytic Queries](https://arxiv.org/abs/2510.27238)
*Chuxuan Hu,Maxwell Yang,James Weiland,Yeji Lim,Suhas Palawala,Daniel Kang*

Main category: cs.DB

TL;DR: DRAMA是一个端到端的数据分析范式，通过自然语言查询在开放域数据上自动执行数据收集、转换和分析，显著提升了数据分析的效率和准确性。


<details>
  <summary>Details</summary>
Motivation: 现有自动化数据科学工作流系统无法同时支持开放域数据收集、结构化数据转换和分析推理这三个关键能力，导致手动数据分析效率低下。

Method: 提出DRAMA范式，将数据收集、转换和分析统一为单一流程；开发DRAMA-Bot多智能体系统，包含数据检索器（协调子智能体执行数据收集和转换）和数据分析器（对检索数据进行结构化推理）。

Result: 在DRAMA-Bench基准测试中，DRAMA-Bot达到86.5%的任务准确率，成本仅0.05美元，优于所有基线方法，准确率最高提升6.9倍，成本不到1/6。

Conclusion: DRAMA范式有效解决了自动化数据分析的关键挑战，在开放域数据分析任务中展现出卓越的性能和成本效益。
...
### [163] [Approximate Diverse $k$-nearest Neighbor Search in Vector Database](https://arxiv.org/abs/2510.27243)
*Jiachen Zhao,Xiao Yan,Eric Lo*

Main category: cs.DB

TL;DR: 提出了一种将结果多样化无缝集成到最先进近似k近邻搜索方法中的新方法，通过渐进式搜索框架在保持低延迟的同时获得近似最优的多样化结果。


<details>
  <summary>Details</summary>
Motivation: 现有基于贪婪的多样化方法经常产生次优结果，无法充分近似特定多样化水平下的最优相似度得分，且需要能够适应不同用户定义结果大小和多样性需求的灵活算法。

Method: 采用渐进式搜索框架，包含迭代搜索、多样化和验证阶段，精心设计的多样化和验证步骤使方法能够高效近似用户指定多样化水平下的最优多样化结果集，无需额外索引开销。

Result: 在三个百万级基准数据集上的实验表明，该方法在中等和高多样性设置下，以最小延迟开销持续检索接近最优的多样化结果，在延迟、相似度和召回率方面表现优异。

Conclusion: 所提出的方法能够有效解决近似k近邻搜索中的多样化需求，在保持高性能的同时提供灵活且接近最优的多样化结果。
