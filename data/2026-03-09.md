...
TL;DR: 提出SKETCHGAIT框架，将标签自由的素描作为步态识别新模态，与解析模态互补，实现92.9%的SUSTech1K Rank-1准确率。
...
Method: 从结构视角重新审视步态表示，提出素描作为新视觉模态，通过边缘检测器从RGB图像提取高频结构线索。提出SKETCHGAIT框架：两个独立流分别学习素描和解析模态，轻量级早期融合分支捕获结构互补性。

Result: 在SUSTech1K上达到92.9% Rank-1准确率，在CCPG上达到93.1%平均Rank-1准确率，验证了素描模态和框架的有效性。

Conclusion: 素描作为无标签的密集部分级结构表示，与标签引导的解析在语义上解耦、结构上互补，SKETCHGAIT框架有效融合这两种模态，显著提升步态识别性能。
...
TL;DR: 提出Thinking with Spatial Code框架，将RGB视频转换为显式的、时间一致的3D表示，用于物理世界视觉问答。通过空间编码器将视频解析为结构化空间代码（包含3D定向边界框和语义标签），使大语言模型能直接对显式空间变量进行推理。


<details>
  <summary>Details</summary>
Motivation: 现有视觉问答模型通常依赖隐式视觉表示，缺乏对物理世界3D结构的显式理解。需要一种能将视频转换为结构化3D表示的方法，使语言模型能进行基于几何的推理。

Method: 1) 空间编码器统一6D物体解析和跟踪骨干网络与几何预测，编码图像和几何特征；2) 使用强化学习微调LLMs，采用空间规则奖励机制，鼓励视角感知和几何基础的推理。

Result: 在VSI-Bench上超越专有视觉语言模型，达到新的最先进水平。

Conclusion: 通过将视频转换为显式3D空间表示，使LLMs能直接对空间变量进行推理，显著提升了物理世界视觉问答的性能。
...
<details>
  <summary>Details</summary>
Motivation: 关键点检测是许多视觉任务的基础，但现代神经网络模型对小的输入扰动仍然脆弱。由于高维输入和连续坐标输出，关键点检测器的形式化鲁棒性验证尚未充分探索。现有解耦方法独立验证每个关键点，忽略了关键点之间的相互依赖性和下游任务需求，导致保守的保证。

Method: 提出耦合鲁棒性验证框架，将验证问题表述为混合整数线性规划（MILP）的证伪问题。该方法结合可达热图集合和编码联合偏差约束的多面体，验证所有关键点的集体行为。不可行性证明鲁棒性，可行性提供反例，并证明方法是可靠的。

Result: 实验表明，耦合方法实现了高验证率，在严格的误差阈值下仍然有效，而解耦方法在这些情况下会失败。该方法为关键点检测器提供了更强的鲁棒性保证。

Conclusion: 提出了首个针对热图基关键点检测器的耦合鲁棒性验证框架，通过考虑关键点之间的相互依赖性和下游任务需求，提供了比传统解耦方法更强的鲁棒性保证。该方法为关键点检测器的安全部署提供了重要工具。
...
TL;DR: DreamCAD是一个多模态生成框架，直接从点级监督生成可编辑的BReps，无需CAD特定标注，并使用可微分细分方法在大规模3D数据集上训练。
...
Method: 将BRep表示为参数化曲面片集合，使用可微分细分方法生成网格，实现大规模训练；同时构建了CADCap-1M数据集（100万+描述）。

Result: 在ABC和Objaverse基准测试中，在文本、图像和点模态上达到最先进性能，几何保真度提升，用户偏好超过75%。

Conclusion: DreamCAD通过点级监督生成可编辑BReps，无需CAD特定标注，解决了数据稀缺问题，推动了可扩展CAD生成的发展。
...
Method: 将生物批次缓解视为域泛化问题，提出对抗性批次表示增强(ABRA)。通过参数化特征统计量为结构化不确定性来显式建模批次统计波动，使用min-max优化框架在表示空间中主动合成最坏情况生物批次扰动，采用严格角度几何边界保持细粒度类别可区分性，并引入协同分布对齐目标防止表示坍塌。

Result: 在大型RxRx1和RxRx1-WILDS基准上进行广泛评估，ABRA在siRNA扰动分类任务上建立了新的最先进性能。

Conclusion: ABRA通过对抗性批次表示增强有效缓解生物批次效应，提高了深度学习模型在细胞成像数据上的泛化能力，为高通量筛选中的批次效应问题提供了有效解决方案。
...
TL;DR: 提出PFS（后融合稳定器）模块，在现有BEV融合检测器的中间特征上进行轻量级操作，提升在域偏移和传感器故障下的鲁棒性，无需修改原架构或重新训练。
...
Method: 设计PFS作为轻量级模块，操作现有检测器的中间BEV表示，通过稳定特征统计、抑制传感器退化区域、自适应残差校正来生成精炼特征图。

Result: 在nuScenes基准测试中，PFS在多种故障模式下达到SOTA，相机丢失鲁棒性提升+1.2%，低光性能提升+4.4% mAP，仅需3.3M参数。

Conclusion: PFS作为近恒等变换模块，在保持性能的同时显著提升BEV融合检测器在域偏移和传感器故障下的鲁棒性，易于集成到已部署系统。
...
TL;DR: CBM-Suite框架解决概念瓶颈模型的四大限制：提出概念相关性预评估指标、解决线性问题、缩小准确率差距、系统分析视觉骨干网络影响


<details>
  <summary>Details</summary>
Motivation: 传统概念瓶颈模型存在四个根本限制：缺乏概念相关性预评估指标、线性问题导致模型绕过概念瓶颈、相比不透明模型存在准确率差距、缺乏对不同视觉骨干网络和视觉语言模型的系统研究

Method: 1. 提出基于熵的指标量化概念集的内在适用性；2. 在概念激活和分类器之间插入非线性层解决线性问题；3. 使用线性教师探针引导的蒸馏损失缩小准确率差距；4. 系统分析不同视觉编码器、视觉语言模型和概念集对准确率和可解释性的影响

Result: CBM-Suite框架产生更准确的模型，并为改进基于概念的可解释性提供见解

Conclusion: CBM-Suite系统性地解决了概念瓶颈模型的关键限制，提高了模型准确率并增强了可解释性，为概念基础模型的发展提供了方法论框架
...
TL;DR: 提出iFID（插值FID），一种改进的rFID变体，能显著提升与扩散模型生成质量gFID的相关性，相关系数约0.85。
...
Method: 对每个数据样本，在潜在空间中检索其最近邻，插值两者的潜在表示，解码插值后的潜在向量，计算解码样本与原始数据集的FID得分。

Result: iFID与扩散gFID呈现强相关性（Pearson线性相关和Spearman秩相关约0.85），是首个展示与扩散生成质量强相关的指标。

Conclusion: iFID能有效预测扩散模型生成质量，揭示了重建指标与扩散细化阶段质量相关，而插值指标与扩散导航阶段质量相关的机制。
...
TL;DR: 提出Implicit Error Counting (IEC)方法，在缺乏理想参考答案的任务中，通过隐式计数错误而非构建评分标准来提供强化学习奖励信号。


<details>
  <summary>Details</summary>
Motivation: 现有RLVR和RaR方法依赖理想参考答案构建评分标准，但许多现实任务允许多种有效输出且缺乏单一理想答案，存在参考缺失的空白。

Method: 提出IEC方法：枚举响应中的错误而非正确之处，在任务相关维度上应用严重性加权评分，并通过隐式分数发射和组校准转换为校准后的每方面奖励。

Result: 在虚拟试衣任务上验证：IEC在MDressBench上优于RaR（CEC: 5.31 vs 5.60），在VITON-HD和DressCode上匹配或超越6个基线方法中的8个感知指标中的6个。

Conclusion: 当理想答案不可用时，计数错误比构建评分标准提供更强的信号，IEC填补了当前后训练方法在参考缺失设置中的空白。
...
TL;DR: SemVID：一种无需训练的视频时序定位剪枝框架，通过保留关键证据和保持跨帧连接性，在仅使用12.5%视觉token的情况下保持95.4%的定位精度，实现5.8倍加速。


<details>
  <summary>Details</summary>
Motivation: 视频时序定位需要处理长视频，现有视觉语言模型计算成本过高。虽然已有视觉token剪枝方法用于视频问答，但直接应用于时序定位会导致性能严重下降，因为时序定位依赖边界敏感证据和跨帧推理链。

Method: 提出SemVID框架，基于两个剪枝原则：证据保留（保持查询关键patch，特别是事件边界周围）和连接强度（保持token级跨帧连接性用于长程证据聚合）。框架首先通过平衡查询相关性和帧间变化分配每帧token预算，然后选择三类token：对象token（多样化查询关键证据）、运动token（捕捉有意义转换并作为跨帧中继）和少量上下文token（保持场景连续性）。

Result: 在视频时序定位基准测试中，SemVID在仅使用12.5%视觉token的情况下保持95.4%的mIoU，实现5.8倍预填充加速，在相同预算下持续优于先前方法。

Conclusion: SemVID通过精心设计的token选择策略，实现了视频时序定位的高效准确剪枝，为长视频处理提供了有效的计算优化方案。
...
TL;DR: OWL是一种基于视觉运动线索的感知函数，通过局部视觉膨胀和刚体旋转两个基本线索，实现无需先验知识的3D场景重建和相机航向估计。


<details>
  <summary>Details</summary>
Motivation: 解决运动过程中3D感知的复杂挑战，特别是如何在不需要环境先验知识、物体运动信息或相机运动参数的情况下，仅从原始视觉运动图像序列中提取有意义的3D信息。

Method: 提出OWL感知函数，基于两个基本视觉运动线索：1) 固定点附近点的感知局部视觉膨胀，2) 刚体相对于固定点的感知旋转。通过像素级并行计算，直接从原始视觉运动图像序列中提取OWL值，无需测量或先验知识。

Result: 仿真实验表明，OWL能够实现3D物体的几何恒常性，仅凭视觉运动线索就能完成缩放3D场景重建。该方法提供了一种实时、简约的3D点相对运动表示方法。

Conclusion: OWL弥合了理论概念与机器人、自主导航实际应用之间的差距，可能成为下一代自主系统的构建模块，为机器感知提供新视角，并对自然感知、行为心理学和神经功能研究有启示意义。
...
TL;DR: MultiHaystack是首个评估大规模跨模态检索与推理的基准，包含46,000+多模态候选和747个可验证问题，揭示当前MLLMs在跨模态检索上的严重瓶颈。


<details>
  <summary>Details</summary>
Motivation: 现有MLLM基准主要评估单模态理解，而忽略了现实世界中从大规模异构多模态语料库中检索相关证据的关键需求。大多数基准将检索限制在小规模、单模态候选集中，过度简化了搜索空间并夸大了端到端可靠性。

Method: 创建MultiHaystack基准，包含超过46,000个跨文档、图像和视频的多模态检索候选，以及747个开放但可验证的问题。每个问题都基于检索池中唯一的验证证据项，需要跨模态的证据定位和细粒度推理。

Result: 模型在提供对应证据时表现良好，但从完整语料库中检索证据时性能大幅下降。最强的检索器E5-V仅达到40.8%的Recall@1，GPT-5等最先进MLLMs的推理准确率从提供证据时的80.86%降至top-5检索时的51.4%。
...
Method: 提出三阶段框架：1)感知阶段使用混合自回归稀疏自编码器将噪声音频分解为语义基础的"声学原子"；2)多模态推理阶段使用通过GRPO微调的MLLM融合声学原子与视觉特征；3)精确预测阶段在S^2流形上使用黎曼流匹配。

Result: 实验表明该框架显著优于单模态基线，证明声景的可解释感知提供了关键的正交信号，结合多模态推理可实现高精度全球定位。

Conclusion: 可解释的声景感知与多模态推理相结合，为精确全球地理定位提供了有效解决方案，听觉线索是视觉信息的重要补充。
...
TL;DR: Any2Full：一种单阶段、领域通用、模式无关的深度补全框架，通过尺度提示适配预训练的单目深度估计模型，实现稀疏到稠密深度补全


<details>
  <summary>Details</summary>
Motivation: 现有RGBD融合深度补全方法受限于特定训练分布和深度模式，缺乏领域泛化能力；而基于单目深度估计的两阶段方法存在额外计算和结构化失真问题

Method: 将深度补全重新定义为预训练单目深度估计模型的尺度提示适配，设计尺度感知提示编码器从稀疏输入提取尺度线索，生成统一尺度提示来引导模型预测

Result: 在AbsREL指标上比OMNI-DC提升32.2%，比PriorDA快1.4倍，同时保持相同MDE骨干网络，在鲁棒性和效率方面表现优异

Conclusion: Any2Full建立了通用深度补全的新范式，通过单阶段、领域通用、模式无关的方法有效解决了稀疏深度补全问题
...
TL;DR: 本文提出了一种自动化流程，将ImageNet训练集转换为多标签数据集，无需人工标注，通过自监督ViT进行无监督物体发现，生成的多标签标注显著提升了模型性能。


<details>
  <summary>Details</summary>
Motivation: 原始ImageNet基准强制使用单标签假设，但许多图像包含多个物体，这导致标签噪声并限制了学习信号的丰富性。多标签标注能更准确地反映现实世界视觉场景，使模型学习更丰富、更鲁棒的表征。

Method: 使用自监督视觉Transformer进行无监督物体发现，选择与原始标签对齐的区域训练轻量级分类器，然后将其应用于所有区域，在整个数据集中生成一致的多标签标注。

Result: 生成的多标签标注在定性评估中与人类判断高度一致，在定量基准测试中持续提升性能。相比传统单标签方案，使用多标签监督训练的模型在架构间获得更好的域内准确率（在ReaL上提升高达+2.0 top-1准确率，在ImageNet-V2上提升+1.5），并在下游任务中表现出更强的可迁移性（在COCO和VOC上分别提升高达+4.2和+2.3 mAP）。

Conclusion: 准确的多标签标注对于提升分类性能和表征学习至关重要。提出的自动化流程成功生成了高质量的多标签数据集，显著改善了模型性能。
...
TL;DR: 提出基于CLIP的多模态框架，自动从手术视频生成结构化时间线和叙述，减少外科医生手动标注负担


<details>
  <summary>Details</summary>
Motivation: 当前手术时间线创建方法存在缺陷：术后报告通常模糊不清，而手动标注视频又极其耗时。需要一种折中方案来自动化生成手术时间线
...
Conclusion: 该方法通过自动从手术视频创建时间线和叙述，减少了外科医生手动视频审查和标注的需求，为工具辅助手术提供了更高效的时间线生成方案
...
TL;DR: Icarus是一个全天气天空模型，能够学习全动态范围物理捕捉户外图像的曝光范围，实现用户可控的太阳和云层定位，提供前所未有的光照准确性和真实感。


<details>
  <summary>Details</summary>
Motivation: 现有基于深度学习的天空模型在重现高分辨率自然天空时存在不足，特别是在14EV+类别不平衡的太阳区域，导致视觉质量差、光照传输、阴影和色调失真。
...
Conclusion: Icarus是一个全天气天空模型，解决了现有深度学习天空模型在高分辨率太阳区域建模的局限性，实现了更准确、真实的环境贴图生成。
...
TL;DR: LayerBind：无需训练的文本到图像生成区域布局控制方法，通过分层建模和绑定实现精确区域和遮挡控制


<details>
  <summary>Details</summary>
Motivation: 现有区域控制方法存在数据偏见、图像质量下降和遮挡顺序控制困难的问题。作者观察到空间布局和遮挡在去噪早期阶段就已确定，因此重新排列早期潜在结构足以修改最终输出

Method: 分为两个阶段：1) 实例初始化 - 利用多模态联合注意力中的上下文共享机制，创建每个实例的分支，在指定早期步骤按层顺序融合；2) 语义护理 - 通过分层注意力增强强化区域细节并保持遮挡顺序，使用分层透明度调度器

Result: LayerBind在定性和定量结果上都表现出有效性，无需训练且即插即用，可作为区域和遮挡控制器应用于各种Diffusion Transformers，并原生支持可编辑工作流程

Conclusion: LayerBind解决了现有区域控制方法的局限性，通过分层建模和绑定实现了精确的区域和遮挡控制，具有强大的创意应用潜力
...
TL;DR: BM25-V：基于视觉词稀疏自编码的检索方法，结合BM25评分实现高效可解释的图像检索


<details>
  <summary>Details</summary>
Motivation: 密集图像检索虽然准确，但可解释性差、计算量大且难以溯源。需要一种高效、可解释且可归因的图像检索方法。

Method: 使用稀疏自编码器（SAE）从Vision Transformer的patch特征中提取稀疏视觉词激活，然后应用Okapi BM25评分进行检索。利用倒排索引实现高效检索，并作为两阶段检索的第一阶段。

Result: 在7个基准测试中，Recall@200 ≥ 0.993，两阶段检索仅需重排200个候选就能恢复接近密集检索的准确率（平均差距0.2%）。SAE在ImageNet-1K上训练后能零样本迁移到7个细粒度基准。

Conclusion: BM25-V提供了一种高效、可解释且可归因的图像检索方法，通过稀疏视觉词和BM25评分实现了接近密集检索的性能，同时支持检索决策的可视化溯源。
...
TL;DR: 研究发现：在2D到3D重建中，保持光谱结构一致性比增强空间细节更重要；结构光谱一致性是NVS质量的最佳预测指标；可学习上采样器在重建质量上通常不优于经典插值方法


<details>
  <summary>Details</summary>
Motivation: 现有可学习上采样方法主要关注增强空间细节（如更锐利的几何或更丰富的纹理），但它们对3D感知能力的影响尚未充分探索。需要填补这一研究空白。

Method: 引入光谱诊断框架，包含六个互补指标：振幅重分布、结构光谱对齐和方向稳定性。在CLIP和DINO骨干网络上，对经典插值和可学习上采样方法进行系统评估。

Result: 1. 结构光谱一致性是NVS质量的最强预测指标；2. 几何和纹理对不同的光谱特性敏感；3. 可学习上采样器虽然能产生更锐利的空间特征，但在重建质量上通常不优于经典插值方法。

Conclusion: 重建质量与保持光谱结构的相关性比增强空间细节更强，光谱一致性应成为2D到3D管道中上采样策略设计的重要原则。
...
TL;DR: EventGeM：一种基于事件相机的视觉地点识别方法，通过全局-局部特征融合和深度估计重排序，在多个基准数据集上实现最先进的定位性能，并能实时运行。


<details>
  <summary>Details</summary>
Motivation: 事件相机因其稀疏激活和高时间分辨率在机器人导航和定位任务中越来越受欢迎，但需要更准确、高效的视觉地点识别方法。

Method: 使用预训练的ViT-S/16骨干网络从事件直方图图像中提取全局特征进行初始匹配，然后使用预训练的MaxViT骨干网络检测局部特征关键点进行基于2D单应性的RANSAC重排序，最后使用预训练的视觉基础模型进行深度估计以比较结构相似性进行进一步重排序。

Result: 在多个基准数据集和光照条件下，相比当前最好的事件相机地点识别方法，EventGeM实现了最先进的定位性能，并且能够在各种计算架构上实时运行。

Conclusion: EventGeM通过全局到局部特征融合和深度估计重排序，为事件相机提供了高效、准确的视觉地点识别解决方案，成功在机器人平台上进行了实时在线定位演示。
...
Result: 在NVIDIA A6000上，平均达到12.2 FPS（基线为8.4 FPS），吞吐量提升1.45倍。生成质量不受影响，无需额外训练即可集成到模型中。

Conclusion: 该方法有效连接了传统压缩算法与现代生成流水线，通过利用时间冗余性显著提升视频编辑效率，同时保持生成质量。
...
<details>
  <summary>Details</summary>
Motivation: 深度视觉分类器虽然准确率高，但校准性差且在小分布偏移下脆弱。现有方法往往需要复杂架构修改或额外数据，需要一种简单有效的正则化方法。

Method: MaCS在交叉熵损失基础上添加两个正则项：(1) 铰链平方边缘惩罚，强制正确类别与最强竞争者之间的logit差距；(2) 一致性正则器，最小化干净输入与轻微扰动视图预测之间的KL散度。

Result: 在多个图像分类基准和多种骨干网络（CNN和ViT）上，MaCS一致提高了校准性（降低ECE和NLL）和对常见损坏的鲁棒性，同时保持或提高了top-1准确率。

Conclusion: MaCS是一个有效的即插即用训练目标替代方案，无需额外数据、架构修改或显著推理开销，通过理论分析和实验验证了其改善泛化保证和鲁棒性的能力。
...
TL;DR: 提出一个融合CNN和ViT的遥感图像分类模型，通过集成四个独立的融合模型来避免冗余特征表示带来的性能瓶颈，在多个数据集上达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 遥感图像分类需要准确的计算方法。CNN擅长提取局部特征但难以捕获全局上下文信息，ViT通过自注意力机制建模长距离依赖关系。融合两者能获得更好性能，但简单的CNN+ViT组合会产生冗余特征表示导致性能瓶颈。

Method: 提出一个融合模型，结合CNN和ViT的优势。为了克服性能瓶颈，训练四个独立的融合模型，每个模型集成CNN和ViT骨干网络，然后在最终预测阶段通过集成方法组合它们的输出。

Result: 在UC Merced数据集上达到98.10%准确率，RSSCN7数据集上94.46%，MSRSI数据集上95.45%。这些结果超越了竞争架构，特别是在训练过程中计算资源的高效利用方面表现突出。

Conclusion: 提出的融合模型有效结合了CNN和ViT的优势，通过集成多个独立模型避免了冗余特征表示问题，在遥感图像分类任务上取得了优异的性能，同时保持了计算效率。
...
TL;DR: Cog2Gen3D：一个3D认知引导的扩散框架，通过语义和几何表征融合解决3D生成中的尺度一致性和物理合理性挑战
...
Method: 提出Cog2Gen3D框架，包含三个关键设计：1）认知特征嵌入：编码多模态为语义和几何表征，并提取逻辑表征；2）3D潜在认知图：构建双流语义-几何图，通过基于共通的交叉注意力融合；3）认知引导的潜在扩散：使用融合的3D认知图作为条件，引导潜在扩散过程生成3D高斯。

Result: 在基于Marble World Labs构建的验证集上，Cog2Gen3D在语义保真度和几何合理性方面显著优于现有方法，确保了3D生成的物理合理性和结构合理性。

Conclusion: 通过统一的3D认知图框架，成功解决了3D生成中的尺度一致性和物理合理性挑战，为可控的物理世界3D生成提供了有效解决方案。
...
TL;DR: VS3R是一个视频稳定框架，通过结合前馈3D重建与生成视频扩散，解决了几何鲁棒性与全帧一致性之间的权衡问题，在各种相机模型下实现高保真全帧稳定。
...
Method: VS3R框架结合前馈3D重建与生成视频扩散：联合估计相机参数、深度和掩码以确保全场景可靠性；引入混合稳定渲染模块融合语义和几何线索以实现动态一致性；使用双流视频扩散模型通过结构引导与语义锚点协同来修复遮挡区域和校正伪影。

Result: VS3R在多样相机模型下实现了高保真、全帧稳定，在鲁棒性和视觉质量方面显著优于现有最先进方法。
...
TL;DR: 提出了TumorChain框架，通过多模态交错推理实现肿瘤分析的临床可解释性，在TumorCoT基准上验证了有效性


<details>
  <summary>Details</summary>
Motivation: 临床肿瘤分析需要从影像发现到病理结论的逐步推理，现有方法缺乏可追溯性和一致性，容易产生诊断错误

Method: 构建TumorCoT数据集（150万条带CoT标注的VQA指令），提出TumorChain框架，结合3D影像编码器、临床文本理解和器官级视觉语言对齐，通过跨模态对齐和迭代交错因果推理实现自我精炼

Result: 在病灶检测、印象生成和病理分类任务上均优于基线方法，在DeepTumorVQA基准上表现出强泛化能力

Conclusion: 多模态推理框架为临床肿瘤分析提供了可靠且可解释的解决方案，提高了诊断的可追溯性并减少了幻觉风险
...
TL;DR: 提出PatchCue：一种基于图像块的视觉提示范式，通过将图像分割成块并在块级别表示视觉提示，显著提升视觉语言模型的视觉推理能力，优于像素级边界框和点级提示。


<details>
  <summary>Details</summary>
Motivation: 现有推理范式（如Chain-of-Thought）仅依赖文本信息，未能充分利用重要视觉线索。虽然先前工作引入了像素级视觉提示，但这些表示需要精确的空间定位，增加了学习复杂度。

Method: 提出PatchCue范式：将图像分割成块，在块级别表示视觉提示，与现代VLM的块标记化输入对齐。采用两阶段训练：冷启动监督微调输出块级提示，然后通过过程监督提示奖励的强化学习指导中间视觉推理步骤。

Result: 在多个VLM和多样化基准测试（包括通用视觉问答、复杂推理和文档理解）上的广泛实验表明，PatchCue能持续提升整体模型性能。块级提示优于像素级边界框和点级提示。
...
TL;DR: MemSeg-Agent：一种基于记忆增强的分割代理，通过将适应从权重空间转移到记忆空间，实现少样本学习、联邦监督学习和测试时适应，显著降低联邦学习中的通信开销。


<details>
  <summary>Details</summary>
Motivation: 医学图像分割模型在单一数据集上训练后难以泛化到不同机构、扫描仪或患者群体。现有的视觉基础模型需要任务特定的微调，这在联邦学习中引入大量通信开销，且无法在部署期间持续进行知识演化。

Method: 提出MemSeg-Agent，通过固定主干网络配合轻量级的静态记忆、少样本记忆和测试时工作记忆，由代理控制器动态组合这些记忆单元。在联邦学习中更新紧凑的记忆单元而非模型参数。

Result: 在四个公共数据集上的实验表明：静态记忆单独就能匹配或超越强监督基线且参数效率高；测试时工作记忆进一步提升了域内和跨域性能而无需微调。

Conclusion: MemSeg-Agent为代理AI时代提供了可扩展和自适应的医学图像分割新范式，通过记忆空间适应而非权重空间适应，实现了高效的知识演化。
...
TL;DR: 提出CORE-Seg框架，通过语义引导提示适配器将推理与分割结合，在ComLesion-14K基准上实现最先进的复杂病灶分割性能。


<details>
  <summary>Details</summary>
Motivation: 医学图像分割正从传统的视觉模式匹配转向认知推理分析。现有的多模态大语言模型具有通用常识但缺乏复杂病灶的专业视觉推理能力，而传统分割模型擅长像素级分割但缺乏逻辑可解释性。
...
Conclusion: 该工作展示了推理驱动分割在复杂病灶分析中的有效性，为医学图像分割提供了新的认知推理范式。
...
TL;DR: 本文系统评估了合成新视角在视频地点识别中的效果，发现小规模合成视角添加能改善识别统计，而大规模添加时视角变化幅度不如添加数量和数据集图像类型重要。


<details>
  <summary>Details</summary>
Motivation: 合成新视角生成对机器人导航有积极影响：地面机器人场景可生成鸟瞰图引导空中机器人；视频地点识别中，空中视角可帮助无人机识别地面机器人看到的地点，反之亦然。

Method: 使用五个公开VPR图像数据库和七种典型图像相似性方法，系统评估合成新视角在VPR中的效果。

Result: 对于小规模合成添加，新视角能改善VPR识别统计；对于大规模添加，视角变化幅度不如添加数量和数据集图像类型重要。

Conclusion: 合成新视角在VPR中具有实用价值，特别是小规模添加时能提升识别性能，为机器人导航中的跨视角识别提供了有效方法。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于图像相似性的后门检测方法难以应对新兴的视觉多样性后门攻击，这些攻击生成的图像在视觉上可能差异很大，但部分语义模式会被稳定操纵。

Method: BlackMirror包含两个组件：MirrorMatch将视觉模式与对应指令对齐以检测语义偏差；MirrorVerify评估这些偏差在不同提示下的稳定性，以区分真正的后门行为和良性响应。

Result: 综合实验表明，BlackMirror在广泛的攻击类型中实现了准确检测，是一个无需训练、即插即用的通用框架。

Conclusion: BlackMirror为模型即服务应用中的黑盒后门检测提供了有效的解决方案，能够应对视觉多样性的后门攻击挑战。
...
TL;DR: CylinderSplat：一种用于全景3D高斯溅射的前馈框架，通过圆柱形三平面表示解决稀疏视图下的遮挡问题，在单视图和多视图全景新视角合成中实现SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有前馈3D高斯溅射方法在全景图像应用中存在挑战：多视图成本体积方法在稀疏视图下难以解决遮挡问题，标准笛卡尔三平面表示无法有效捕捉360°场景的几何结构，导致失真和混叠。

Method: 提出圆柱形三平面表示，与全景数据和曼哈顿世界假设下的真实世界结构更匹配。采用双分支架构：像素分支重建观测良好区域，体积分支利用圆柱形三平面补全遮挡或稀疏观测区域。框架灵活支持从单视图到多视图的可变输入。

Result: 在单视图和多视图全景新视角合成任务中均达到最先进水平，在重建质量和几何精度方面均优于先前方法。

Conclusion: CylinderSplat通过创新的圆柱形三平面表示和双分支架构，有效解决了全景3D高斯溅射中的几何表示和遮挡问题，为全景新视角合成提供了高效解决方案。
...
TL;DR: RAC是一种基于Rectified Flow的自动编码器，替代传统VAE，通过多步解码、双向推理和生成式解码，在重建和生成质量上超越SOTA VAE，同时计算成本降低约70%


<details>
  <summary>Details</summary>
Motivation: 传统VAE存在重建-生成差距问题，解码路径不够灵活，且需要独立的编码器和解码器，参数较多。需要一种既能提高生成质量又能降低计算成本的替代方案

Method: 提出Rectified Flow Auto Coder (RAC)：1）通过将解码器应用于流时间步实现多步解码，路径直且可校正；2）支持双向推理，解码器通过时间反转可作为编码器使用；3）采用生成式解码方法，可沿路径校正潜在变量

Result: RAC在重建和生成质量上均超越SOTA VAE，参数数量减少近41%，计算成本降低约70%，部分解决了重建-生成差距问题

Conclusion: RAC是一种有效的VAE替代方案，通过Rectified Flow启发的设计实现了更好的重建和生成性能，同时显著降低了计算成本，为生成模型提供了新的架构思路
...
<details>
  <summary>Details</summary>
Motivation: 现有方法通常依赖隐式符号距离场和后期布局优化，缺乏统一的场景重建方法，无法直接生成可用于下游应用的轻量级高质量网格。

Method: 基于点云编码器，通过像素对齐的图像特征和跨注意力机制增强全局场景上下文理解，采用自回归方式从统一令牌流（包含上下文、姿态和网格信息）生成场景。

Result: 在合成和真实数据集上的实验表明，PixARMesh实现了最先进的重建质量，同时生成轻量级、高质量的网格，可直接用于下游应用。

Conclusion: PixARMesh提供了一种从单张图像直接生成完整3D室内场景网格的有效方法，通过统一的自回归框架实现了布局和几何的联合优化，为实际应用提供了实用的解决方案。
...
TL;DR: InnoAds-Composer：单阶段三条件控制的产品海报生成框架，通过重要性分析和路由机制解决多阶段方法的主体保真度、文本准确性和风格一致性问题，并设计了文本特征增强模块提升中文文本渲染质量。


<details>
  <summary>Details</summary>
Motivation: 现有基于扩散模型的产品海报生成方法通常采用多阶段流程，难以同时控制主体、文本和风格三个条件，且存在主体保真度差、文本不准确、风格不一致等问题。

Method: 提出单阶段框架InnoAds-Composer，支持主体、字形和风格三条件控制；通过层和时间步重要性分析，将每个条件仅路由到最敏感位置，缩短有效token序列；设计文本特征增强模块，融合字形图像和字形裁剪特征提升中文文本渲染准确性。

Result: 实验表明InnoAds-Composer显著优于现有产品海报生成方法，且未明显增加推理延迟；构建了首个同时包含主体、文本和风格条件的高质量电商产品海报数据集和基准。

Conclusion: InnoAds-Composer通过单阶段三条件控制和重要性路由机制，有效解决了产品海报生成中的主体保真度、文本准确性和风格一致性问题，为电商广告生成提供了高效解决方案。
...
TL;DR: 提出了一种新颖的掩码机制，通过结合深度残差网络和U-Net架构构建残差掩码网络，在面部表情识别任务中实现了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 面部表情识别在人机交互中具有重要应用价值。现有方法中，注意力机制能提升FER任务性能，但需要更有效的方法来引导网络关注关键区域信息。

Method: 提出掩码思想，使用分割网络细化特征图，使网络能聚焦于相关信息做出正确决策。结合深度残差网络和U-Net架构构建残差掩码网络。

Result: 在FER2013和私有VEMO数据集上达到了最先进的准确率，代码已开源。

Conclusion: 掩码机制能有效提升CNN在面部表情识别任务中的性能，残差掩码网络在公开和私有数据集上都表现出色。
...
TL;DR: 论文提出三种偏差缓解技术改进概念瓶颈模型的公平性：top-k概念过滤减少信息泄露、移除有偏概念、对抗性去偏，在公平性-性能权衡上优于先前工作。


<details>
  <summary>Details</summary>
Motivation: 概念瓶颈模型理论上应通过可解释概念提升公平性，但实际存在信息泄露问题，且在性别偏见缓解上效果有限，需要改进其公平性表现。

Method: 提出三种偏差缓解技术：1) 使用top-k概念过滤器减少信息泄露；2) 识别并移除有偏概念；3) 应用对抗性去偏方法。

Result: 提出的去偏CBM在公平性-性能权衡方面优于先前工作，为公平且可解释的图像分类提供了重要进展。

Conclusion: 通过三种偏差缓解技术的组合，概念瓶颈模型能够显著提升公平性，同时保持可解释性，为实现公平且可解释的图像分类提供了有效方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有视觉token剪枝方法通常对所有输入使用固定的token预算，忽略了图像信息密度的显著差异。这种一刀切的方法无法根据图像内容复杂度进行自适应调整，导致信息密集场景可能被过度压缩，而简单场景又浪费计算资源。

Method: 提出能量驱动的自适应剪枝框架E-AdaPrune，通过分析视觉特征空间的奇异值谱来确定token预算。方法基于保留特定比例的谱能量来分配token数量，信息密集场景获得更多token，冗余场景则被积极压缩。使用随机奇异值分解控制额外延迟在8ms/图像内。

Result: 在9个基准测试和3个VLM骨干网络（LLaVA-1.5-7B/13B、LLaVA-NeXT-8B）上评估，在相同平均token预算下，E-AdaPrune平均提升达0.6%，在MMVet推理任务上获得显著的+5.1%相对提升。

Conclusion: E-AdaPrune通过自适应token剪枝有效解决了图像信息密度变化问题，在保持模型性能的同时显著加速推理，为视觉语言模型的高效部署提供了实用解决方案。
...
TL;DR: 提出CollabOD轻量级协作检测框架，解决无人机图像中小目标检测的挑战，通过结构细节保留、跨路径特征对齐和定位感知轻量化设计来提升检测性能。
...
Method: 提出CollabOD轻量级协作检测框架，包含三个核心策略：1) 结构细节保留；2) 跨路径特征对齐；3) 定位感知轻量化设计。从图像处理、通道结构和轻量化设计角度优化传统无人机感知模型架构，并采用统一的细节感知检测头。

Result: 该框架在保持高效推理的同时增强了表示稳定性，统一的细节感知检测头进一步提高了回归鲁棒性，且不引入额外的部署开销。
...
TL;DR: 提出Art3D框架，从追求几何精度转向艺术性视差合成，通过双路径架构分离全局深度参数与局部艺术效果，学习专业3D电影数据以生成更具沉浸感和情感共鸣的3D内容。


<details>
  <summary>Details</summary>
Motivation: 现有2D转3D方法虽能保证几何精度，但缺乏艺术性，无法复现专业3D电影的沉浸感和情感共鸣体验。传统方法将艺术意图（如零平面偏移、局部深度雕刻）误认为数据噪声或歧义。

Method: 提出Art3D框架，采用艺术视差合成新范式。使用双路径架构：一路处理全局深度参数（宏观意图），另一路处理局部艺术效果（视觉笔触）。通过间接监督从专业3D电影数据中学习。

Result: 实验表明Art3D在复现关键局部出屏效果和匹配电影3D内容的全局深度风格方面展现出潜力，为艺术驱动的转换工具奠定了基础。

Conclusion: 需要从物理准确的视差估计转向艺术一致的视差合成新范式，Art3D为此提供了初步框架，能够更好地捕捉和复现专业3D电影的艺术意图。
...
TL;DR: 提出Skeleton-to-Image Encoding (S2I)方法，将3D人体骨骼序列转换为图像格式，使预训练视觉模型能直接用于骨骼数据自监督学习，解决骨骼数据格式差异和数据集稀缺问题。


<details>
  <summary>Details</summary>
Motivation: 现有大规模预训练视觉模型难以直接应用于3D骨骼数据，因为数据格式差异大；骨骼数据集稀缺；需要将骨骼数据融入多模态动作识别而不增加额外模型分支。

Method: 提出Skeleton-to-Image Encoding (S2I)：基于身体部位语义分割和排列关节，将骨骼序列转换为标准尺寸的图像格式，使预训练视觉模型能用于骨骼自监督表示学习。

Result: 在NTU-60、NTU-120和PKU-MMD数据集上验证了方法的有效性和泛化能力，包括在跨格式评估设置下表现良好。

Conclusion: S2I编码为骨骼数据提供统一的图像格式表示，首次实现预训练视觉模型在骨骼自监督学习中的应用，能有效处理异构骨骼数据并传递视觉知识。
...
### [43] [Pano3DComposer: Feed-Forward Compositional 3D Scene Generation from Single Panoramic Image](https://arxiv.org/abs/2603.05908)
*Zidian Qiu,Ancong Wu*

Main category: cs.CV

TL;DR: Pano3DComposer：一种高效的端到端全景图像到3D场景生成框架，通过对象-世界坐标转换预测器和粗到细对齐机制，在约20秒内生成高保真3D场景。


<details>
  <summary>Details</summary>
Motivation: 现有组合式图像到3D场景生成方法存在两个主要问题：1）依赖耗时迭代布局优化或不灵活的联合对象-布局生成；2）大多基于有限视场角透视图像，难以创建完整的360度环境。

Method: 提出Pano3DComposer框架，包含两个核心技术：1）Object-World Transformation Predictor（对象-世界坐标转换预测器），将现成图像到3D模型生成的3D对象从局部坐标转换到世界坐标；2）Coarse-to-Fine（C2F）对齐机制，通过场景渲染反馈迭代优化几何一致性。

Result: 在合成和真实数据集上的图像/文本到3D任务中取得了优越的几何精度，能够在RTX 4090 GPU上约20秒内生成高保真3D场景。

Conclusion: Pano3DComposer通过解耦对象生成与布局估计，结合高效的坐标转换预测和迭代对齐机制，实现了快速、高质量的全景3D场景生成，解决了现有方法的效率和质量限制。
...
<details>
  <summary>Details</summary>
Motivation: 现有变化描述方法仅处理静态图像对，忽略了变化过程的丰富时间动态，而理解"如何变化"与理解"什么变化"同样重要。
...
Conclusion: ProCap通过将变化建模从静态图像比较重构为动态过程建模，能够更好地理解和描述变化过程，为变化描述任务提供了新视角。
...
TL;DR: 该报告探讨巴基斯坦制造的手术器械及其缺陷检测，通过与行业领导者合作，利用YOLOv8、ResNet-152和EfficientNet-b4等深度学习架构开发自动化缺陷检测方法，提升制造质量标准和患者安全。


<details>
  <summary>Details</summary>
Motivation: 现代医疗中手术器械的质量标准对临床成功至关重要，即使是微小缺陷也可能导致致命后果。巴基斯坦制造的手术器械需要更严格的质量控制，通过自动化缺陷检测来防止经济损失并保障患者生命安全。

Method: 1. 与巴基斯坦Sialkot手术器械集群的行业领导者Daddy D Pro和Dr. Frigz International合作；2. 创建包含4,414张高分辨率图像的新数据集；3. 采用深度学习架构包括YOLOv8、ResNet-152和EfficientNet-b4进行自动化缺陷检测；4. 通过自动光学检测（AOI）工具进行质量保证。

Result: 开发了基于深度学习的手术器械缺陷检测系统，能够识别裂纹、锈蚀和结构异常等关键缺陷。该研究为制造商、医疗专业人员和监管机构提供了有价值的资源，有助于提升巴基斯坦制造手术器械的质量标准。

Conclusion: 通过行业合作和先进深度学习技术的整合，成功开发了自动化缺陷检测方法，显著提升了手术器械制造的质量控制水平，为更可靠的医疗环境做出了贡献，体现了技术驱动质量改进的重要性。
...
TL;DR: MM-ISTS：基于视觉-文本大语言模型的多模态框架，用于不规则采样时间序列预测，通过跨模态编码和自适应特征提取提升预测性能。


<details>
  <summary>Details</summary>
Motivation: 现有不规则采样时间序列预测方法仅利用历史观测值，无法学习上下文语义和细粒度时间模式，需要更全面的多模态方法来捕捉复杂的时间模式和上下文理解。

Method: 提出两阶段编码机制：1) 跨模态视觉-文本编码模块自动生成信息丰富的视觉图像和文本数据，结合多模态大语言模型；2) ISTS编码从历史观测中提取时间特征，包括多视图嵌入融合和时间-变量编码器。还包含自适应查询特征提取器和多模态对齐模块。

Result: 在真实数据上的大量实验证明了所提出解决方案的有效性，能够有效提升不规则采样时间序列的预测性能。

Conclusion: MM-ISTS通过融合时间、视觉和文本模态，结合多模态大语言模型，成功解决了不规则采样时间序列预测中上下文语义学习和细粒度时间模式捕捉的挑战。
...
Conclusion: RepKAN具有显著潜力，可以作为未来可解释视觉基础模型的主干架构，为遥感图像分类提供高性能且物理可解释的解决方案。
...
TL;DR: RAID数据集：用于驾驶员风险感知研究的大规模数据集，包含4,691个标注视频片段，涵盖多种交通场景，支持风险对象识别和行人注意力分析。


<details>
  <summary>Details</summary>
Motivation: 实现零碰撞智能车辆系统需要理解驾驶员的风险感知，这是一个复杂的认知过程，受到驾驶员对外部刺激的自愿反应和周围道路使用者对自车注意力的影响。目前缺乏专门用于驾驶员风险感知研究的大规模数据集。

Method: 1. 构建RAID数据集：包含4,691个标注视频片段，涵盖多样交通场景，标注内容包括驾驶员意图操作、道路拓扑、风险情况、驾驶员反应和行人注意力。2. 提出弱监督风险对象识别框架：通过建模驾驶员意图操作与反应之间的关系来识别潜在风险源。3. 分析行人注意力在风险评估中的作用。

Result: 实验评估显示，该方法在RAID数据集上比现有最佳方法性能提升20.6%，在HDDS数据集上提升23.1%。

Conclusion: RAID数据集为驾驶员风险感知和上下文风险评估研究提供了有价值的资源，提出的弱监督风险对象识别框架有效提升了风险源识别性能，行人注意力分析进一步增强了风险评估能力。
...
TL;DR: TAR-ViTPose：一种用于视频2D人体姿态估计的时序聚合与恢复Vision Transformer，通过聚合跨帧时序信息提升姿态估计的鲁棒性和准确性。
...
Result: 在PoseTrack2017基准上比单帧基线ViTPose提升+2.3 mAP，优于现有最先进的视频方法，并在实际应用中实现更高的运行时帧率。

Conclusion: TAR-ViTPose通过有效的时序信息聚合和恢复机制，显著提升了视频中2D人体姿态估计的性能和鲁棒性，同时保持了高效的推理速度。
...
TL;DR: 研究通过分析VLM中间激活，发现视觉概念在线性编码上的差异，识别出感知失败和认知失败两种故障模式，并发现物体距离会降低视觉概念的线性可分性。


<details>
  <summary>Details</summary>
Motivation: 尽管VLMs在自动驾驶应用中越来越普遍，但它们经常在简单的视觉问题上失败，而这些失败的原因尚不清楚。研究旨在理解VLMs在自动驾驶相关视觉任务中的失败机制。

Method: 创建仅在特定视觉概念上不同的反事实图像集，使用四个SOTA VLMs的激活训练线性探针来区分这些概念，分析视觉信息的线性编码程度。

Result: 物体存在等概念被显式线性编码，而方向等空间概念仅被隐式编码。识别出两种失败模式：感知失败（视觉信息未线性编码）和认知失败（信息存在但语义对齐失败）。物体距离增加会快速降低视觉概念的线性可分性。

Conclusion: 研究提高了对VLMs在自动驾驶相关简单视觉任务中失败案例的理解，揭示了视觉概念编码的差异和两种不同的故障机制，为改进VLMs提供了重要见解。
...
<details>
  <summary>Details</summary>
Motivation: 现有NeRF和3DGS方法虽然渲染质量高，但依赖耗时的逐场景优化，限制了实时部署。新兴的前馈高斯泼溅方法效率有所提升，但缺乏显式的流形几何结构，无法直接用于仿真。

Method: 提出前馈三角形基元生成框架，包含像素对齐的三角形生成模块，并引入相对3D点云监督来增强几何学习的稳定性和一致性。直接从标定的多视角图像预测连续三角形表面。

Result: 实验表明该方法实现了高效重建，同时保持了与标准图形和机器人仿真器的无缝兼容性，能够生成仿真就绪的模型。

Conclusion: 该方法通过前馈方式生成三角形基元，解决了现有方法在实时部署和仿真兼容性方面的限制，为机器人学和仿真应用提供了高效的3D重建解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 当前扩散模型在说话头生成中存在高推理延迟、时间不稳定（闪烁和身份漂移）以及音频-视觉对齐不完美的问题，特别是在具有挑战性的语音条件下。

Method: 采用参考条件的潜在扩散框架，使用师生蒸馏方法：用标准噪声预测目标训练的扩散教师指导轻量级学生去噪器，减少推理步骤；结合身份锚定和时间正则化减少身份漂移和闪烁；使用基于视位的音频条件提供粗粒度唇部运动控制。

Result: 在LRS3数据集上的实验显示，蒸馏扩散模型能保留强教师的大部分重建行为，同时实现显著降低的推理延迟；CPU和边缘计算的初步延迟特性分析表明边缘部署的可行性。

Conclusion: 蒸馏扩散模型可以在保持高质量重建的同时大幅降低推理延迟，这是向计算受限环境下实用扩散基说话头生成迈出的初步步骤。
...
TL;DR: OD-RASE框架通过本体论驱动的方法检测导致交通事故的道路结构，并连接基础设施改进建议，提升自动驾驶安全性


<details>
  <summary>Details</summary>
Motivation: 自动驾驶系统在处理罕见情况或复杂道路结构时仍有局限，当前道路基础设施为人类驾驶员设计，安全改进通常只在事故发生后进行，这种被动方法对需要主动风险缓解的自动驾驶系统构成挑战

Method: 1. 基于道路交通系统专业领域知识形式化本体论；2. 使用大规模视觉语言模型生成基础设施改进建议；3. 通过本体论驱动数据过滤增强可靠性；4. 在事故前道路图像上自动标注改进建议构建新数据集；5. 提出OD-RASE模型，结合LVLM和扩散模型生成改进建议和改进后道路环境图像

Result: 实验表明，本体论驱动数据过滤能够高精度预测导致事故的道路结构和相应的改进计划
...
TL;DR: 该论文提出情感连续说话人脸生成（EC-TFG）新任务，以及TIE-TFG模型，能够根据文本和动态情感描述生成具有自然情感变化的人脸视频。


<details>
  <summary>Details</summary>
Motivation: 现有说话人脸生成方法通常只能表达固定目标情感，缺乏人类在交流时自然连续的情感变化能力，无法生成真实的情感动态表达。

Method: 提出TIE-TFG模型，采用时间密集型情感波动建模技术，通过生成与输入文本对应的情感变化序列来驱动连续的面部表情变化。

Result: 广泛评估表明该方法能够产生平滑的情感过渡，在不同情感状态下保持高质量的视觉效果和运动真实性。

Conclusion: EC-TFG任务和TIE-TFG模型成功实现了情感连续变化的说话人脸生成，显著提升了合成视频的情感表达自然度。
...
TL;DR: SLER-IR：一种球面分层专家路由框架，通过动态激活跨网络层的专门专家来处理多样化图像退化，消除线性嵌入空间中的几何偏差，解决特征干扰和专家专业化不足问题。


<details>
  <summary>Details</summary>
Motivation: 现有统一框架在处理多样化图像退化时面临特征干扰和专家专业化不足的挑战，需要一种能够动态激活专门专家并消除几何偏差的方法。

Method: 提出球面分层专家路由框架（SLER-IR），包含：1）球面均匀退化嵌入，通过对比学习将退化表示映射到超球面；2）全局-局部粒度融合模块，整合全局语义和局部退化线索；3）动态激活跨网络层的专门专家。

Result: 在三任务和五任务基准测试中，SLER-IR在PSNR和SSIM指标上均优于现有最先进方法，取得了持续改进。

Conclusion: SLER-IR通过球面退化嵌入和分层专家路由，有效解决了多样化图像退化中的特征干扰和专家专业化问题，为统一图像修复框架提供了有效解决方案。
...
TL;DR: 提出基于自适应径向投影的文档图像倾斜估计方法，创建高质量数据集DISE-2021，傅里叶方法综合改进


<details>
  <summary>Details</summary>
Motivation: 文档倾斜估计是文档处理系统的关键任务，直接影响后续处理步骤。在数字化时代，这个问题具有重要研究价值，需要更鲁棒可靠的解决方案。

Method: 提出新颖的倾斜估计方法：在2D离散傅里叶幅度谱上应用自适应径向投影，提取文档图像的主导倾斜角度。同时创建DISE-2021数据集用于评估。

Result: 所提方法表现出鲁棒性和可靠性，在所有对比方法中表现最优。提供了傅里叶方法的全面分析，涵盖多个改进方面。

Conclusion: 该方法在文档倾斜估计任务上优于现有方法，开源代码可用。DISE-2021数据集为评估倾斜估计器提供了高质量基准。
...
<details>
  <summary>Details</summary>
Motivation: 生成式真实图像超分（Real-ISR）虽然能合成视觉上可信的细节，但存在关键失败模式：输出可能看起来清晰但与低分辨率证据不忠实（语义和结构幻觉），且这种忠实性难以评估。现有偏好强化学习方法面临退化鲁棒性信号缺失、优化瓶颈和真实退化数据覆盖有限等问题。

Method: 提出LucidNFT框架：1）Lucidity一致性评估器：退化鲁棒的语义评估器，使LR锚定的忠实性可测量和优化；2）解耦优势归一化策略：在融合前保留每个LR条件rollout组内的目标对比，防止优势崩溃；3）LucidityLR：大规模真实退化图像集合，支持鲁棒的RL微调。

Result: 实验表明LucidNFT能持续改进基于流的Real-ISR基线，在多样真实场景中实现更好的感知-忠实性权衡，并具有稳定的优化动态。

Conclusion: LucidNFT通过引入退化鲁棒的忠实性评估器、改进的优化策略和大规模真实退化数据集，有效解决了真实图像超分中的语义幻觉和优化瓶颈问题，为生成式超分提供了更可靠的解决方案。
...
TL;DR: Place-it-R1：基于多模态大语言模型（MLLMs）的端到端视频物体插入框架，通过Think-then-Place范式实现物理一致的视频编辑。


<details>
  <summary>Details</summary>
Motivation: 当前视频编辑技术主要关注视觉保真度而非物理因果关系，导致插入的物体与环境物理不一致。需要一种能够理解物理场景并进行环境感知推理的方法。

Method: 采用Think-then-Place范式：1) MLLM进行物理场景理解和交互推理，生成环境感知的思维链令牌和有效插入区域；2) 引入MLLM引导的空间直接偏好优化（DPO），通过反馈评分提升视觉自然性；3) 推理时MLLM迭代触发细化循环，形成闭环优化；4) 提供两种用户可选模式：灵活模式（允许环境修改）和标准模式（保持场景完整性）。

Result: 实验表明Place-it-R1相比现有最先进解决方案和商业模型，实现了物理一致性的视频物体插入。

Conclusion: 该框架通过结合MLLM的认知推理和视频扩散的生成能力，实现了物理一致且视觉自然的视频物体插入，为用户提供了在物理合理性和保真度之间的明确控制。
...
### [59] [Unify the Views: View-Consistent Prototype Learning for Few-Shot Segmentation](https://arxiv.org/abs/2603.05952)
...
<details>
  <summary>Details</summary>
Motivation: 少样本分割在有限监督下泛化到新类别时面临挑战，特别是在大外观或视角变化下的结构错位和跨视角不一致问题。

Method: 1. 在骨干特征上构建空间-视角图：空间图捕捉局部几何拓扑，视角图连接不同视角特征传播视角不变的结构语义。2. 从支持-查询特征差异中推导判别性先验，捕获类别特定对比度，重新加权SAM特征并重新校准骨干激活。3. 通过掩码交叉注意力逐步集成前景增强的SAM特征和结构丰富的ResNet特征，生成类别一致原型作为SAM解码器的自适应提示。

Result: 在多个少样本分割基准测试上的广泛实验验证了VINE的有效性和鲁棒性，特别是在具有视角偏移和复杂结构的挑战性场景中表现优异。

Conclusion: VINE通过联合建模结构一致性和前景判别性，有效解决了少样本分割中的结构错位和跨视角不一致问题，在挑战性场景下实现了准确的分割性能。
...
TL;DR: VLM-RobustBench：首个全面评估视觉语言模型在49种图像失真下鲁棒性的基准，发现VLMs对空间扰动（尤其是重采样和几何变换）比视觉上更严重的光度失真更敏感。


<details>
  <summary>Details</summary>
Motivation: 虽然视觉语言模型在高质量数据集上表现良好，但对其在真实世界图像失真下的性能缺乏系统理解。需要建立全面基准来评估VLMs在各种图像扰动下的鲁棒性。

Method: 构建VLM-RobustBench基准，涵盖49种增强类型（噪声、模糊、天气、数字、几何扰动），在分级严重度（低/中/高）和二元变换下评估，共133种损坏设置。在MMBench（视觉基础）和MMMU-Pro（推理导向）两个基准上评估四个VLM家族（Qwen、InternVL、Molmo、Gemma）。

Result: 视觉严重度是困难的弱预测指标：低严重度的空间扰动（如glass_blur）比视觉上更严重的光度损坏对性能影响更大。重采样和几何失真（如upsample、elastic_transform）导致最大性能下降（高达34个百分点）。当前VLMs语义能力强但空间脆弱。

Conclusion: 当前VLMs对空间扰动（特别是重采样和几何变换）高度敏感，需要开发新的鲁棒性评估协议和训练机制，强调重采样和几何不变性。
...
### [61] [OVGGT: O(1) Constant-Cost Streaming Visual Geometry Transformer](https://arxiv.org/abs/2603.05959)
...
<details>
  <summary>Details</summary>
Motivation: 现有几何基础模型使用全注意力机制，二次计算成本限制了其只能处理短序列；因果注意力变体如StreamVGGT虽然支持流式处理，但KV缓存会不断增长，在数百帧内耗尽GPU内存，无法实现真正长期部署的流式推理

Method: 结合自选择性缓存（利用FFN残差幅度压缩KV缓存，完全兼容FlashAttention）和动态锚点保护（保护坐标关键令牌不被驱逐，抑制几何漂移），将内存和计算限制在固定预算内

Result: 在室内、室外和超长序列基准测试中，OVGGT能够在恒定VRAM范围内处理任意长视频，同时实现最先进的3D几何精度

Conclusion: OVGGT通过训练无关的方法解决了长视频3D几何重建中的内存和计算瓶颈，实现了真正实用的流式推理部署
...
TL;DR: 提出RF-Sampling方法，专门为流匹配模型（如FLUX）设计的无需训练推理增强框架，通过文本表示线性组合和流反转提升生成质量和文本对齐


<details>
  <summary>Details</summary>
Motivation: 现有的推理增强策略主要适用于传统扩散模型，在流模型上效果不佳，需要专门为流模型（特别是CFG蒸馏变体）设计有效的推理增强方法

Method: RF-Sampling：基于理论推导的训练免费推理增强框架，通过文本表示线性组合和流反转技术，在噪声空间中探索更符合输入提示的生成路径

Result: 在多个基准测试中，RF-Sampling一致提升了生成质量和提示对齐效果，是首个在FLUX上展现一定程度测试时缩放能力的推理增强方法

Conclusion: RF-Sampling为流模型提供了有效的推理增强解决方案，填补了现有方法在流模型上的空白，并证明了其在提升生成质量和文本对齐方面的有效性
...
### [63] [Exploring Open-Vocabulary Object Recognition in Images using CLIP](https://arxiv.org/abs/2603.05962)
*Wei Yu Chen,Ying Dai*

Main category: cs.CV

TL;DR: 提出基于两阶段策略（目标分割+识别）的开放词汇目标识别框架，无需复杂重训练和标注，通过CLIP或CNN/MLP生成嵌入，使用SVD构建共享表示空间，在多个数据集上达到SOTA性能。
...
Method: 采用两阶段策略：1）目标分割裁剪区域；2）使用CLIP或CNN/MLP生成目标级图像嵌入和类别级文本嵌入，通过SVD构建共享表示空间，最后通过嵌入相似度匹配进行识别。

Result: 在COCO、Pascal VOC和ADE20K数据集上，无需训练的CLIP编码方法（无SVD）取得了最高的平均AP，超越了当前SOTA方法，同时验证了CNN/MLP编码在OVOR中的潜力。

Conclusion: 提出的OVOR框架简化了开放词汇目标识别流程，无需复杂训练和标注，CLIP编码方法效果最佳，CNN/MLP编码也显示出潜力，为实际应用提供了高效解决方案。
...
Method: 1) 构建新颖的深度图像-文本对数据集和深度指令数据集，使用GLPN模型从视觉图像生成深度图，GPT-4生成深度指令；2) 改进CLIP中的ViT编码器，融入局部物体信息以更好地捕捉深度连续变化；3) 基于现有深度图像数据集开发全面的深度问答基准。

Result: 实验结果表明，DeepSight显著提升了深度感知能力和下游任务性能，在多模态三维理解方面取得了实质性进展。

Conclusion: DeepSight作为首个专门用于深度感知的MLLM，通过创新的数据集构建和模型架构改进，有效解决了现有模型在深度信息理解方面的不足，为三维场景理解开辟了新途径。
...
TL;DR: 提出兩階段框架：先在標註人造字母上訓練對比學習編碼器，再通過師生蒸餾擴展到歷史文字，實現無監督相似性發現


<details>
  <summary>Details</summary>
Motivation: 解決文字相似度度量的基本挑戰：人造字母可可靠標註，但歷史文字間的關係不確定且有爭議

Method: 兩階段框架：1) 在標註人造字母上訓練對比損失編碼器建立教師模型；2) 通過師生蒸餾擴展到歷史文字，學生在教師指導下學習無監督表示

Result: 在多樣文字系統上實驗顯示有效的少樣本字形識別和有意義的文字聚類，無需真實演化關係標註
...
<details>
  <summary>Details</summary>
Motivation: 开放词汇目标检测(OVOD)通过视觉-语言对齐实现新类别检测，但大模型尺寸限制了在资源受限设备上的部署。虽然量化提供了实用的压缩方法，但简单的极端低比特（如4位）量化会严重降低细粒度视觉-语言对齐并扭曲区域间关系结构。

Method: 提出课程关系量化感知训练(CR-QAT)框架，结合阶段优化和关系知识蒸馏。课程QAT(CQAT)通过分区渐进量化缓解误差累积，确保稳定优化；文本中心关系KD(TRKD)通过构建文本锚定的成对相似性矩阵，全面传递教师模型的多维关系知识。

Result: 在LVIS和COCO零样本基准测试中，CR-QAT在激进低比特设置下始终优于现有QAT基线，分别实现了相对AP提升高达38.9%和40.9%。

Conclusion: CR-QAT框架有效解决了开放词汇目标检测在极端低比特量化下的性能退化问题，通过课程式量化和关系知识蒸馏的结合，在保持模型压缩的同时显著提升了检测性能。
...
TL;DR: 提出DynUAV基准测试，针对无人机视角下动态多目标跟踪，包含剧烈自运动和复杂表观轨迹，包含42个视频序列和170万标注框，评估现有跟踪器在剧烈尺度变化、视角变化和运动模糊下的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有无人机视角多目标跟踪基准缺乏真实世界的动态复杂性，主要包含可预测的相机运动和线性运动模式，无法反映无人机快速移动和敏捷机动带来的显著观测挑战。

Method: 构建DynUAV基准测试，包含42个视频序列，超过170万个边界框标注，涵盖车辆、行人以及挖掘机、推土机、起重机等工业类别。基准特点包括剧烈自运动导致的复杂表观轨迹、剧烈尺度变化、视角变化和运动模糊。

Result: 对现有最先进跟踪器在DynUAV上的综合评估显示，它们在处理动态条件下的检测和关联交织挑战方面存在明显局限性，特别是在管理剧烈尺度变化、视角变化和运动模糊方面表现不足。

Conclusion: DynUAV作为一个严格的基准测试，揭示了现有跟踪器在真实世界无人机视角动态多目标跟踪中的不足，预计将作为具有挑战性的测试平台推动该领域进展，所有资源将在指定链接公开。
...
TL;DR: CoE是一个免训练的多模态摘要框架，通过事件链和层次事件图实现结构化推理，在多个数据集上显著优于现有方法。
...
Method: 提出CoE框架，使用层次事件图（HEG）将文本语义编码为显式事件层次结构，指导事件链推理。通过该结构定位关键视觉线索，建模事件演化和因果转换，并通过轻量级风格适应进行领域对齐。

Result: 在八个不同数据集上的实验表明，CoE一致优于最先进的视频CoT基线方法，平均提升+3.04 ROUGE、+9.51 CIDEr和+1.88 BERTScore，展现了其鲁棒性、可解释性和跨领域泛化能力。

Conclusion: CoE通过结构化的事件链推理有效解决了多模态摘要中的关键挑战，实现了免训练的高质量摘要生成，具有优秀的跨领域适应能力。
...
### [69] [Towards High-resolution and Disentangled Reference-based Sketch Colorization](https://arxiv.org/abs/2603.05971)
*Dingkun Yan,Xinrui Wang,Ru Wang,Zhuoru Li,Jinze Yu,Yusuke Iwasawa,Yutaka Matsuo,Jiaxian Guo*

Main category: cs.CV

TL;DR: 提出双分支框架解决素描上色中的分布偏移问题，通过语义对齐和语义错配分支建模训练与推理数据分布，使用Gram正则化损失增强跨域分布一致性，结合动漫专用Tagger网络实现精细控制。


<details>
  <summary>Details</summary>
Motivation: 素描上色是动画和数字插图自动化的关键任务。先前研究主要关注缓解分布偏移引起的伪影，而非从根本上解决问题。本文旨在直接最小化分布偏移，实现更高质量、更高分辨率和更好可控性的上色效果。

Method: 提出双分支框架：语义对齐分支建模训练数据分布，语义错配分支建模推理数据分布；使用Gram正则化损失强制跨域分布一致性和稳定性；采用动漫专用Tagger网络从参考图像提取细粒度属性，调制SDXL的条件编码器实现精确控制；添加插件模块增强纹理传递。

Result: 定量和定性比较以及用户研究表明，该方法有效克服了分布偏移挑战，在质量和可控性指标上均达到最先进水平。消融研究揭示了各组件的影响。

Conclusion: 通过直接最小化分布偏移而非仅仅缓解其影响，该框架在素描上色任务中实现了更高质量、更高分辨率和更好可控性，为动画和数字插图创作提供了有效的自动化解决方案。
...
TL;DR: TaPD是一个用于自动驾驶轨迹预测的插件框架，通过渐进式知识蒸馏和时间回填机制，适应不同长度的历史观测，特别在短观测情况下表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有轨迹预测器通常假设固定长度的历史观测，但在实际场景中（如遮挡或感知范围有限）观测长度可能变化或极短，导致性能显著下降。需要一种能够适应不同观测长度的统一预测框架。

Method: 提出TaPD框架，包含两个协同模块：1）观测自适应预测器（OAF），基于渐进式知识蒸馏（PKD），将长时观测"教师"的运动模式知识传递给短时观测"学生"；2）时间回填模块（TBM），在极短历史情况下显式重建缺失的历史轨迹。采用余弦退火蒸馏权重平衡预测监督和特征对齐，以及解耦的预训练-重建-微调协议。

Result: 在Argoverse 1和Argoverse 2数据集上的广泛实验表明，TaPD在所有观测长度下均优于强基线方法，在极短输入情况下提升尤其显著，并能以插件方式提升其他预测器（如HiVT）的性能。

Conclusion: TaPD是一个有效的观测自适应轨迹预测框架，通过渐进式知识蒸馏和时间回填机制解决了变长历史观测的挑战，在实际自动驾驶场景中具有实用价值。
...
TL;DR: GazeMoE：基于混合专家机制从冻结基础模型中自适应提取多模态线索的端到端视线目标估计框架


<details>
  <summary>Details</summary>
Motivation: 从可见图像估计人类视线目标对机器人理解人类注意力至关重要，但现有神经架构和训练范式在泛化性方面仍有挑战。预训练视觉基础模型为定位视线目标提供了新途径，但需要有效整合眼睛、头部姿态、手势等多模态线索。

Method: 提出GazeMoE框架，通过混合专家（MoE）模块从冻结的基础模型中自适应选择视线目标相关线索。为解决视线目标分类（框内vs框外）的类别不平衡问题，引入类别平衡辅助损失和数据增强策略（区域特定裁剪和光度变换）。

Result: 在基准数据集上的广泛实验表明，GazeMoE在具有挑战性的视线估计任务上实现了最先进的性能，超越了现有方法。

Conclusion: GazeMoE通过自适应多模态线索提取和类别平衡策略，有效提升了视线目标估计的性能和鲁棒性，为机器人理解人类注意力提供了新方案。
...
TL;DR: HiPP-Prune：一种分层偏好条件结构化剪枝框架，通过单一策略调用生成全局剪枝蓝图，支持用户指定偏好向量进行可控的鲁棒性-效用权衡，特别关注减少视觉语言模型中的目标幻觉问题。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型剪枝面临挑战：压缩不仅影响任务效用，还会影响视觉基础能力，甚至在相同稀疏度下加剧目标幻觉问题。需要一种能够平衡多个目标（任务效用、幻觉鲁棒性、压缩率）的剪枝方法。

Method: HiPP-Prune采用分层偏好条件结构化剪枝框架，将剪枝决策分解为总体稀疏度预算和分层分配。策略状态整合视觉敏感度信号（源自视觉token与语言隐藏状态间的注意力流），防止过度剪枝视觉关键层。使用计划级组相对策略优化在多目标回报下优化剪枝计划。

Result: 在LLaVA模型上的POPE和ScienceQA实验表明，HiPP-Prune能够发现多样化的非支配剪枝计划，在匹配稀疏度预算下提供可控的鲁棒性-效用权衡。

Conclusion: HiPP-Prune通过分层偏好条件剪枝框架，有效解决了视觉语言模型剪枝中的多目标平衡问题，特别在减少目标幻觉方面表现出色，为高效部署提供了可控的压缩方案。
...
### [73] [RePer-360: Releasing Perspective Priors for 360$^\circ$ Depth Estimation via Self-Modulation](https://arxiv.org/abs/2603.05999)
...
<details>
  <summary>Details</summary>
Motivation: 现有的深度基础模型在透视图像上训练效果良好，但在360°全景图像上泛化能力差，因为透视域和全景域之间存在显著的几何差异。完全微调这些模型通常需要大量全景数据，成本高昂。
...
Result: RePer-360仅使用1%的训练数据就超越了标准微调方法。在相同域内训练设置下，RMSE提升了约20%。该方法在保留预训练透视先验的同时有效适应全景域。
...
<details>
  <summary>Details</summary>
Motivation: 随着视觉语言模型变得越来越复杂和广泛应用，理解其决策过程变得至关重要。传统的可解释性方法是为分类任务设计的，难以处理现代自回归VLM，因为其复杂的逐token生成过程以及视觉和文本模态之间错综复杂的交互。

Method: DEX-AR通过计算逐token生成过程中注意力图的层梯度来解释自回归VLM。该方法包含两个关键创新：1) 动态头过滤机制，识别关注视觉信息的注意力头；2) 序列级过滤方法，聚合每token解释同时区分视觉基础和纯语言token。

Result: 在ImageNet、VQAv2和PascalVOC上的评估显示，在使用新的归一化困惑度测量的基于扰动的指标以及基于分割的指标上都有一致的改进。

Conclusion: DEX-AR为自回归视觉语言模型提供了一种有效的可解释性方法，能够处理其复杂的逐token生成过程和多模态交互，在多个基准测试中表现出优越性能。
...
TL;DR: K-MaT是一个无需低端模态训练图像的提示学习框架，通过知识锚定和流形传输将高端医学影像模型的决策结构迁移到低端模态，在跨模态基准测试中取得SOTA结果。


<details>
  <summary>Details</summary>
Motivation: 现有大规模生物医学视觉语言模型在高端影像（如CT）上表现良好，但迁移到前线低端模态（如X光）时容易陷入模态特定捷径，导致性能崩溃。需要一种无需低端训练图像就能实现跨模态迁移的方法。

Method: 提出K-MaT框架：1）分解提示并锚定到临床文本描述；2）使用融合Gromov-Wasserstein最优传输将低端提示流形对齐到视觉基础的高端空间；3）通过知识锚定保持临床相关性。

Result: 在四个跨模态基准测试（皮肤镜、乳腺X光到超声、CT到胸部X光）中取得SOTA：平均准确率调和均值达44.1%（比BiomedCoOp的42.0%提升），宏F1达36.2%。在乳腺成像任务中有效缓解灾难性遗忘（CoOp在低端模态准确率降至27.0%）。

Conclusion: 通过最优传输对齐提示流形为医学视觉语言模型的零样本跨模态部署提供了高效途径，能够在无需低端训练图像的情况下保持跨模态的稳健性能。
...
TL;DR: EffectMaker是一个统一的推理-生成框架，通过多模态大语言模型解释高级特效语义，结合扩散Transformer进行上下文学习，实现无需逐特效微调的参考视频特效定制。


<details>
  <summary>Details</summary>
Motivation: 传统高质量视觉特效制作需要专业知识和昂贵流程，现有AIGC系统面临特效数据稀缺、超自然/风格化效果建模困难、需要逐特效微调导致可扩展性和泛化性受限等问题。

Method: 采用语义-视觉双路径引导机制：1) 多模态大语言模型解释高级特效语义并推理如何适应目标主体；2) 扩散Transformer通过上下文学习从参考视频中捕捉细粒度视觉线索；3) 构建包含130k视频、3k特效类别的EffectData数据集。

Result: EffectMaker在视觉质量和特效一致性方面优于现有最佳基线方法，提供了可扩展且灵活的自定义特效生成范式。

Conclusion: EffectMaker通过统一的推理-生成框架解决了特效生成中的数据稀缺和泛化问题，无需逐特效微调即可实现准确、可控、特效一致的合成，为定制化VFX生成提供了可扩展的解决方案。
...
### [77] [Dynamic Chunking Diffusion Transformer](https://arxiv.org/abs/2603.06351)
*Akash Haridas,Utkarsh Saxena,Parsa Ashrafi Fashi,Mehdi Rezagholizadeh,Vikram Appia,Emad Barsoum*

Main category: cs.CV

TL;DR: DC-DiT 是一种动态分块扩散变换器，通过可学习的编码-路由-解码支架自适应压缩图像为变长token序列，根据图像区域细节和扩散时间步动态调整计算量，在ImageNet 256×256上超越参数匹配和FLOP匹配的DiT基线。


<details>
  <summary>Details</summary>
Motivation: 传统扩散变换器将图像处理为固定长度的token序列，对所有区域均匀计算，忽略了图像细节分布不均和去噪过程从粗到细的特性。需要一种能自适应分配计算资源的方法。

Method: 在DiT骨干网络上添加可学习的编码器-路由器-解码器支架，通过端到端学习的分块机制自适应压缩2D输入为变长token序列。该机制学习将均匀背景区域压缩为较少token，细节丰富区域分配更多token，并根据扩散时间步动态调整压缩程度。

Result: 在ImageNet 256×256上，DC-DiT在4×和16×压缩下，FID和Inception Score均优于参数匹配和FLOP匹配的DiT基线。无需显式监督即可学习有意义的视觉分割，且可通过微调预训练DiT检查点实现，减少最多8×训练步数。

Conclusion: DC-DiT证明了自适应token压缩在扩散模型中的有效性，是一种有前景的技术，可应用于像素空间、视频和3D生成，并能与其他动态计算方法结合进一步减少生成FLOPs。
...
### [78] [MOSIV: Multi-Object System Identification from Videos](https://arxiv.org/abs/2603.06022)
...
TL;DR: 提出MOSIV框架，用于从视频中进行多物体系统识别，通过可微分模拟器直接优化连续、每物体材料参数，解决现有方法在复杂多物体交互场景中的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有方法主要针对单物体场景或固定材料原型的离散分类，无法处理复杂多物体交互的视频系统识别问题，需要新的框架来解决这一挑战。

Method: 提出MOSIV框架，使用可微分模拟器直接优化连续、每物体材料参数，通过从视频中提取的几何目标进行指导，并创建了包含接触丰富的多物体交互的合成基准数据集。

Result: 在新基准测试中，MOSIV相比适应基线方法显著提高了接地准确性和长时程模拟保真度，成为该新任务的强基线方法。
...
TL;DR: 提出一种提示组感知训练框架，通过质量引导的组正则化和logit级一致性约束，减少SAM3在医学图像分割中对提示表述的敏感性，提升零样本泛化能力


<details>
  <summary>Details</summary>
Motivation: 基础模型如SAM3支持灵活的文本引导医学图像分割，但其预测对提示表述高度敏感，即使语义等价的描述也会产生不一致的分割结果，限制了在临床和病理工作流程中的可靠性

Method: 将提示敏感性重新定义为组一致性问题：1）将语义相关的提示组织成共享同一真实掩码的提示组；2）引入质量引导的组正则化，利用分割损失作为隐式排名信号；3）采用停止梯度策略的logit级一致性约束来对齐组内预测；无需修改架构且推理保持不变

Result: 在多数据集细胞核基准测试中，文本提示下获得一致提升，显著降低不同提示质量水平的性能方差；在六个零样本跨数据集任务中，Dice系数平均提升2.16个百分点

Conclusion: 该方法提高了视觉语言分割在计算病理学中的鲁棒性和泛化能力，为医学图像分割中的提示敏感性问题提供了有效解决方案
...
### [80] [StruVis: Enhancing Reasoning-based Text-to-Image Generation via Thinking with Structured Vision](https://arxiv.org/abs/2603.06032)
*Yuanhuiyi Lyu,Kaiyu Lei,Ziqiao Weng,Xu Zheng,Lutao Jiang,Teng Li,Yangfu Li,Ziyuan Huang,Linfeng Zhang,Xuming Hu*

Main category: cs.CV

TL;DR: StruVis提出了一种基于结构化视觉表示的文本到图像生成推理框架，避免了传统方法的视觉缺失或计算成本过高问题，通过文本结构化视觉表示作为中间推理状态来提升生成质量。


<details>
  <summary>Details</summary>
Motivation: 现有文本到图像生成的推理框架存在两个问题：1）纯文本推理虽然计算高效但缺乏视觉上下文，容易遗漏关键空间和视觉元素；2）文本-图像交错推理虽然能提供视觉参考，但计算成本高且受限于生成器的表示能力。

Method: 提出StruVis框架，使用文本基的结构化视觉表示作为中间推理状态，而不是依赖中间图像生成。这使得多模态大语言模型能够在纯文本推理过程中有效"感知"视觉结构，通过结构化视觉引导的推理来解锁其文本到图像生成的推理潜力。

Result: 在推理基的文本到图像基准测试中取得显著性能提升：在T2I-ReasonBench上获得4.61%增益，在WISE上获得4%增益。该框架与生成器无关，可无缝集成到不同的文本到图像生成器中。

Conclusion: StruVis通过结构化视觉表示作为中间推理状态，有效解决了现有推理框架的局限性，在保持计算效率的同时提升了文本到图像生成的视觉准确性和推理能力。
...
TL;DR: PSIVG框架将物理模拟器集成到视频扩散过程中，通过重建4D场景和前景物体网格，在物理模拟器中生成一致轨迹，引导视频生成器产生符合物理规律的运动。


<details>
  <summary>Details</summary>
Motivation: 当前基于扩散的视频生成方法虽然视觉质量高，但经常违反基本物理定律（如重力、惯性、碰撞），导致物体运动不一致、动力学不真实，限制了AI生成视频的真实性和可靠性。

Method: PSIVG框架：1）从预训练扩散模型生成的模板视频重建4D场景和前景物体网格；2）在物理模拟器中初始化并生成物理一致的轨迹；3）用模拟轨迹引导视频生成器；4）提出测试时纹理一致性优化（TTCO）技术，基于模拟器的像素对应关系调整文本和特征嵌入。

Result: 综合实验表明，PSIVG生成的视频能更好地遵循真实世界物理规律，同时保持视觉质量和多样性。

Conclusion: PSIVG通过将物理模拟器集成到视频生成流程中，有效解决了当前视频生成方法违反物理定律的问题，提高了生成视频的物理一致性和真实性。
...
### [82] [Occlusion-Aware SORT: Observing Occlusion for Robust Multi-Object Tracking](https://arxiv.org/abs/2603.06034)
...
<details>
  <summary>Details</summary>
Motivation: 2D多目标跟踪面临部分遮挡导致的定位成本混淆问题，现有方法在遮挡场景下跟踪性能下降，需要专门处理遮挡的解决方案。

Method: 提出OA-SORT框架，包含三个核心组件：1）OAM模块分析物体遮挡状态，引入高斯地图减少背景影响；2）OAO利用遮挡状态缓解成本混淆；3）BAM抑制估计不稳定性。

Result: 在DanceTrack测试集上达到63.1% HOTA和64.2% IDF1；将框架集成到其他四个跟踪器中，平均提升HOTA 2.08%、IDF1 3.05%，证明了遮挡感知的可重用性。

Conclusion: 遮挡处理对MOT至关重要，OA-SORT框架有效解决了遮挡导致的跟踪问题，且具有即插即用、无需训练的优点，可广泛应用于现有跟踪器提升性能。
...
TL;DR: CLoPA是一种持续适应策略，通过轻量级训练调整nnInteractive模型参数，在医学图像分割任务中快速达到专家级性能


<details>
  <summary>Details</summary>
Motivation: 现有零样本分割模型在多样化医学成像任务中无法稳定达到专家级性能，而标注活动产生持续增长的任务特定标注数据，需要在线适应机制

Method: CLoPA持续适应策略：在标注缓存上微调nnInteractive模型的小部分参数，通过轻量级训练调度触发，无需新参数或改变推理流程

Result: 在8个医学分割任务中，CLoPA快速提升性能至专家级，即使nnInteractive之前失败的任务也能成功，多数增益在单次训练后实现

Conclusion: 不同参数组的调优效果取决于任务特性和数据状况；对于复杂几何目标，需要更深层次的特征表示对齐
...
TL;DR: 论文提出使用分层子采样处理VGG16超列，并通过集成学习提升脑肿瘤分割性能，在极低样本量(N≤20)下逻辑回归表现最佳，显著优于标准UNet基线。


<details>
  <summary>Details</summary>
Motivation: 超列（hypercolumns）在图像分割中具有潜力，但计算复杂度随训练集大小线性增长，限制了实际应用。需要解决超列处理的计算效率问题，特别是在小样本场景下。

Method: 对VGG16生成的超列应用分层子采样降低计算复杂度，研究集成学习方法（堆叠和投票）在稀疏超列上的性能，并与逻辑回归等简单分类器比较。

Result: 在脑肿瘤数据集上，10%分层子采样率下，N=20时最佳平均Dice得分为0.66。集成学习表现有竞争力，但在极低样本量(N≤20)下，简单逻辑回归分类器最有效。相比标准多尺度UNet基线，性能提升24.53%（p=3.07e-11）。

Conclusion: 分层子采样能有效降低超列计算复杂度，在极低样本量场景下，简单分类器（如逻辑回归）比复杂集成方法更有效，显著优于标准UNet基线，解决了过拟合问题。
...
### [85] [FontUse: A Data-Centric Approach to Style- and Use-Case-Conditioned In-Image Typography](https://arxiv.org/abs/2603.06038)
*Xia Xin,Yuki Endo,Yoshihiro Kanamori*

Main category: cs.CV

TL;DR: 提出FontUse数据集和标注流程，通过数据驱动方法提升文本到图像模型在排版控制方面的能力


<details>
  <summary>Details</summary>
Motivation: 现有文本到图像模型在排版控制方面表现不佳，经常忽略或弱化用户请求的排版样式，需要专门解决排版控制问题

Method: 构建FontUse数据集（约7万张图像），使用分割模型和多模态大语言模型自动标注，包含用户友好提示、文本区域位置和OCR识别字符串；通过微调现有生成模型，无需架构修改即可实现排版控制

Result: 实验表明，使用该流程训练的模型在多样化的提示和布局下，生成的文本渲染比竞争基线更符合提示要求；提出了基于Long-CLIP的评估指标

Conclusion: 通过数据驱动的方法和专门的标注流程，显著提升了文本到图像模型在排版控制方面的能力，使模型能够更一致地解释样式和使用场景条件
...
### [86] [Do Foundation Models Know Geometry? Probing Frozen Features for Continuous Physical Measurement](https://arxiv.org/abs/2603.06459)
*Yakov Pyotr Shkolnikov*

Main category: cs.CV

TL;DR: 视觉语言模型在视觉编码中蕴含了丰富的几何信息，但文本生成路径无法有效表达这些信息，存在3.3倍的表达瓶颈。通过轻量级线性探针可以从冻结特征中提取精确的几何信息，而无需微调或文本生成。


<details>
  <summary>Details</summary>
Motivation: 研究动机是探索视觉语言模型中视觉编码路径与文本生成路径之间的表达差距，特别是模型内部编码的几何信息（如手部关节角度）如何被有效提取和利用，而无需依赖文本生成路径。

Method: 使用6,000参数的线性探针从冻结的视觉特征中提取几何信息；比较不同训练目标（自监督、对比学习、混合范式）的编码器；进行LoRA微调实验；分析不同网络层的几何信号分布；使用统计等效性检验比较不同架构的性能。

Result: 线性探针从冻结特征中提取手部关节角度达到6.1度MAE，而最佳文本输出仅20.0度（3.3倍差距）；LoRA微调将差距缩小到6.5度；不同训练目标的编码器在几何提取上达到统计等效性能（R^2≈0.55）；发现网络中层（18-22层）的注意力头携带最强的几何信号。

Conclusion: 视觉语言模型的视觉编码路径蕴含丰富的几何信息，但文本生成路径存在表达瓶颈；训练目标比架构对几何提取能力影响更大；可以通过轻量级线性探针直接从冻结特征中提取多任务几何信息，无需微调或文本生成。
...
TL;DR: 提出GvU方法，通过理解分支的内在奖励机制，让统一多模态模型自我指导生成质量，缩小理解与生成能力差距


<details>
  <summary>Details</summary>
Motivation: 统一多模态模型在视觉理解方面表现出色，但在生成能力上相对较弱，存在理解与生成过程的解耦问题。模型能准确理解视觉细节，却难以从复杂文本提示生成语义一致的图像。

Method: 提出GvU方法：1）基于理解分支的内在文本-图像对齐奖励机制；2）让模型同时作为教师和学生，用理解分支评估自身输出并指导生成；3）设计自监督强化学习框架，通过内在奖励信号迭代提升生成质量，无需外部监督。

Result: 实验结果表明，该方法显著提升了统一多模态模型的生成能力，同时增强了细粒度视觉理解能力，缩小了视觉理解与生成之间的能力差距。

Conclusion: 通过利用模型内部理解能力来指导生成过程，可以有效缩小统一多模态模型中理解与生成能力的差距，实现两种能力的协同提升。
...
Method: 提出Head-Sliding RoPE进行时间平衡，为参考token分配特定时间偏移，均匀分布其跨帧影响；设计两级空间注意力门进行空间选择，集中注意力于HOI区域并自适应调整强度。

Result: 在未见过的真实场景中进行广泛定性和定量评估，GenHOI显著优于最先进的HOI重演和通用视频编辑方法。
...
TL;DR: AI系统在超过45,000张超声图像上训练，能够以超过93%的敏感性和95%的特异性诊断胎儿口面裂，性能与高级放射科医生相当，并显著优于初级医生。
...
Method: 开发了一个人工智能系统，在来自22家医院的9,215名胎儿的45,139张超声图像上进行训练，用于诊断胎儿口面裂。该系统作为医疗副驾驶使用，并在24名放射科医生和学员中进行了试点研究。

Result: AI系统诊断胎儿口面裂的敏感性和特异性分别超过93%和95%，与高级放射科医生性能相当，并显著优于初级放射科医生。作为医疗副驾驶使用时，该系统将初级放射科医生的敏感性提高了6%以上。试点研究表明，该模型可以改善罕见病症的专业知识发展。
...
TL;DR: Curious-VLA通过两阶段设计解决VLA模型的探索限制问题：IL阶段使用可行轨迹扩展和标准化表示，RL阶段采用自适应多样性采样和跨度驾驶奖励，在Navsim基准上取得SOTA结果。


<details>
  <summary>Details</summary>
Motivation: 现有VLA模型存在窄策略限制，模仿学习会限制探索能力，导致后续强化学习阶段因反馈多样性不足而过早饱和，无法充分发挥模型潜力。

Method: 两阶段框架：IL阶段使用可行轨迹扩展策略生成多个物理有效轨迹，采用步进标准化轨迹表示；RL阶段引入自适应多样性感知采样优先高多样性样本，使用跨度驾驶奖励和焦点风格加权来扩大奖励值跨度。

Result: 在Navsim基准测试中取得最先进结果：PDMS 90.3，EPDMS 85.4，Best-of-N PDMS达到94.8，有效释放了VLA模型的探索潜力。

Conclusion: Curious-VLA通过解决探索-利用困境，成功提升了VLA模型的性能，证明了多样性增强策略在自动驾驶VLA模型中的重要性。
...
<details>
  <summary>Details</summary>
Motivation: 当前外科AI缺乏理解外科推理的能力（如为什么选择特定器械、风险评估、下一步操作等），因为大规模标注这类推理数据非常困难。但外科教学视频中专家讲解已经包含了这些推理信息，只是缺乏系统化的提取方法。

Method: 1. 创建SUREON数据集：从外科学术视频中提取134.7K个片段，涵盖170种手术类型，使用多智能体流程提取和结构化监督信息，生成206.8K个问答对。2. 定义12个问题类别，涵盖安全评估、决策理据和预测。3. 开发两个模型：SureonVLM（通过监督微调）和SureonVLM-R1（使用Group Relative Policy Optimization训练）。

Result: 1. 创建了包含206.8K个问答对的SUREON数据集和354个专家验证的基准测试。2. 两个模型在SUREON基准测试上准确率超过84%，显著优于更大的通用领域模型。3. 在标准外科感知任务上也优于通用模型。4. SureonVLM-R1展现出明确的推理行为，如从视觉上下文中推断手术意图。

Conclusion: 通过从外科教学视频中提取专家讲解，可以大规模训练AI理解外科推理。SUREON数据集和模型为外科AI向理解"为什么"和"接下来是什么"的推理能力迈出了重要一步。
...
TL;DR: BEVLM：将语义丰富的BEV表示与LLMs连接，提升自动驾驶中的空间推理和语义理解能力


<details>
  <summary>Details</summary>
Motivation: 现有方法将多视角多帧图像独立输入LLMs，导致计算冗余和空间一致性不足；而BEV表示有空间结构但缺乏语义丰富性。需要结合两者的优势。

Method: 提出BEVLM框架，将空间一致且语义蒸馏的BEV表示与LLMs连接，使用BEV特征作为统一输入，并从LLMs向BEV表示蒸馏语义知识。

Result: BEVLM使LLMs在跨视角驾驶场景中的推理准确率提升46%；通过语义蒸馏，闭环端到端驾驶性能在安全关键场景中提升29%。

Conclusion: BEVLM成功结合了BEV的空间一致性和LLMs的语义理解能力，显著提升了自动驾驶系统的推理准确性和驾驶性能。
...
TL;DR: 提出一个将已存档的RGB-LiDAR传感器数据转换为3D高斯泼溅(3DGS)初始化资产的管道，用于创建大规模数字孪生，解决现有数据利用率低的问题。


<details>
  <summary>Details</summary>
Motivation: 机器人学和自动驾驶领域对大规模数字孪生的需求快速增长，但构建3DGS环境通常需要昂贵的数据采集。同时，部署的平台收集了大量全向RGB和LiDAR数据，但由于传输限制和缺乏可扩展的重用管道，大部分数据被直接丢弃或未充分利用。

Method: 提出全向RGB-LiDAR重用管道，包括：1) ERP到立方体贴图转换模块用于确定性空间锚定；2) PRISM颜色分层下采样策略；3) 基于FPFH的全局配准和ICP算法桥接多模态输入；4) 将LiDAR数据转换为可用的SfM几何。

Result: 管道成功将大量被丢弃的数据重新用于SfM几何，LiDAR增强的初始化在结构复杂场景中相比仅使用视觉的基线方法，持续提高了最终3DGS渲染的保真度。

Conclusion: 这项工作为从标准存档传感器日志创建仿真级数字孪生提供了一个确定性工作流程，提高了现有传感器数据的利用率，降低了数字孪生构建成本。
...
TL;DR: 提出Lyapunov Probes方法，将LLMs/MLLMs视为动力系统，通过稳定性理论检测幻觉：幻觉出现在知识过渡区边界，探针网络在扰动下强制置信度单调衰减以区分稳定事实区域和不稳定幻觉区域。


<details>
  <summary>Details</summary>
Motivation: 传统方法将幻觉检测视为分类任务，本文从动力系统稳定性理论视角重新审视该问题，将(M)LLMs视为动力系统，其中事实知识表示为表示空间中的稳定平衡点，幻觉出现在知识过渡区域的边界。

Method: 提出Lyapunov Probes：轻量级网络，通过基于导数的稳定性约束训练，强制在输入扰动下置信度单调衰减。采用系统扰动分析和两阶段训练过程，可靠区分稳定事实区域和不稳定幻觉区域。

Result: 在多样化数据集和模型上的实验表明，该方法相比现有基线取得了一致的改进效果。

Conclusion: 通过动力系统稳定性理论框架重新定义幻觉检测问题，提出的Lyapunov Probes方法能有效识别幻觉区域，为(M)LLMs的可靠性评估提供了新视角。
...
<details>
  <summary>Details</summary>
Motivation: 现有联邦域泛化方法依赖全局特征和简单平均聚合，难以捕捉细粒度局部特征，且忽略客户端在特征提取能力上的差异

Method: 提出FedARKS框架，包含两个机制：RK（鲁棒知识）用于提取细粒度局部特征，KS（知识选择）用于差异化聚合客户端模型

Result: 未在摘要中提供具体实验结果，但声称能解决现有方法的局限性

Conclusion: FedARKS框架通过鲁棒知识提取和差异化知识选择，能更好地提升模型在未见域上的泛化能力
...
TL;DR: 提出RMD蒸馏框架，通过跨分辨率分布匹配解决低分辨率生成质量下降问题，实现高保真、少步数的多分辨率级联推理加速


<details>
  <summary>Details</summary>
Motivation: 现有扩散蒸馏方法受限于去噪过程，步数减少已基本饱和；部分时间步低分辨率生成虽能加速，但因跨分辨率分布差距导致质量明显下降

Method: 使用logSNR曲线为每个分辨率划分时间步区间，引入logSNR映射补偿分辨率引起的偏移；沿分辨率轨迹进行分布匹配，减少低分辨率生成分布与教师高分辨率分布间的差距；在上采样中加入预测噪声重注入机制稳定训练

Result: RMD在保持高保真生成的同时加速推理，在SDXL上实现33.4倍加速，在Wan2.1-14B上实现25.6倍加速，同时保持高视觉保真度

Conclusion: RMD通过跨分辨率分布匹配有效解决了低分辨率生成的质量下降问题，为扩散模型的快速高保真推理提供了新框架
...
TL;DR: 研究发现视觉语言模型对结构化颜色扭曲敏感，即使人类仍能轻松识别，模型却产生错误预测，且增大语言模型规模无法可靠缓解此问题。


<details>
  <summary>Details</summary>
Motivation: 尽管视觉语言模型在基准测试中表现良好，但在面对结构化颜色扭曲时会出现系统性的感知弱点，即使场景对人类来说仍然容易识别，模型却会产生自信但荒谬的预测。

Method: 使用空间颜色混合方法，创建8种程序化颜色扭曲变体，在4个数据集上评估9个来自3个模型家族的视觉语言模型。同时进行包含61名参与者的人类研究，并测试简单的人类启发式预处理步骤。

Result: 随着扭曲程度增加，所有模型和数据集上的准确率急剧下降，增大语言模型规模无法可靠缓解此问题。人类在相同扭曲下显著优于视觉语言模型。简单的人类启发式预处理能恢复部分性能。

Conclusion: 视觉语言模型存在系统性感知弱点，感知感知的预处理和工具使用是提高模型鲁棒性的实用策略。
...
TL;DR: 提出虚拟治疗框架，将非小细胞肺癌放疗期间肿瘤进展建模为剂量感知的多模态条件图像翻译问题，用于合成放疗引起的解剖变化随访CT图像。
...
Method: 提出虚拟治疗框架，将NSCLC进展建模为剂量感知多模态条件图像翻译问题。基于CT扫描、基线临床变量和指定辐射剂量增量，合成反映治疗诱导解剖变化的随访CT图像。在222名III期NSCLC患者的895个CT扫描数据集上评估，生成过程以递送剂量增量、人口统计学和肿瘤相关临床变量为条件。基准测试了基于GAN和扩散模型的2D和2.5D配置。

Result: 定量和定性结果表明，基于扩散的模型从多模态剂量感知条件中获益更一致，产生比基于GAN的基线更稳定、解剖学上更合理的肿瘤演变轨迹。

Conclusion: 虚拟治疗框架作为NSCLC计算机模拟治疗监测和适应性放疗研究工具具有潜力，扩散模型在该任务中表现优于GAN模型。
...
TL;DR: FreeOcc：无需训练的语义和全景占据预测方法，利用预训练基础模型从多视角图像恢复语义和几何信息，在未见环境中实现3D场景理解


<details>
  <summary>Details</summary>
Motivation: 当前基于相机的占据预测方法通常需要昂贵的密集3D监督或需要在目标域数据上训练模型，这限制了在未见环境中的部署。需要一种无需训练的方法来实现3D场景理解。

Method: 1) 使用可提示的基础分割模型提取每视角的全景先验，通过提示到分类规则；2) 使用重建基础模型重建度量3D点；3) 通过深度和置信度感知过滤将可靠标签提升到3D；4) 随时间融合并通过确定性细化堆栈进行体素化；5) 通过拟合和合并当前视角的3D框候选来恢复实例，实现无需学习3D模型的实例感知占据。

Result: 在Occ3D-nuScenes上，FreeOcc无需训练达到16.9 mIoU和16.5 RayIoU，与最先进的弱监督方法相当。作为伪标签生成管道训练下游模型时，达到21.1 RayIoU，超过之前的弱监督基线。在全景占据预测方面，为无需训练和弱监督设置了新基线，分别达到3.1 RayPQ和3.9 RayPQ。

Conclusion: FreeOcc展示了基础模型驱动的感知是实现无需训练3D场景理解的实用途径，能够在未见环境中部署，同时为下游模型训练提供高质量的伪标签。
...
<details>
  <summary>Details</summary>
Motivation: 传统半监督学习在标注极度有限时伪标签不稳定，导致性能下降；现有视觉语言模型在医学图像上效果有限，因为领域特定提示难以迁移
...
TL;DR: JOPP-3D是一个开放词汇语义分割框架，通过联合利用全景图像和点云数据，实现语言驱动的场景理解，在2D和3D开放/封闭词汇语义分割上显著优于现有方法。
...
Result: 在Stanford-2D-3D-s和ToF-360数据集上的实验表明，JOPP-3D能够跨全景和3D域生成连贯且有语义意义的分割结果，在开放和封闭词汇的2D和3D语义分割上相比SOTA有显著提升。
...
TL;DR: 该论文提出了一种使用强化学习（RL）和多尺度反馈来增强3D扩散模型的方法，通过在MRI数据上预训练扩散模型，然后使用PPO算法和结合2D切片与3D体积分析的新型奖励系统进行微调，以同时优化局部纹理细节和全局结构一致性。


<details>
  <summary>Details</summary>
Motivation: 尽管扩散模型已成为强大的3D医学图像生成工具，但标准训练目标与临床相关性之间仍存在差距。需要一种方法来弥合这一差距，使生成的图像不仅具有统计上的合理性，还能在临床任务中发挥实际效用。

Method: 1. 在MRI体积数据上预训练3D扩散模型以建立生成先验；2. 使用近端策略优化（PPO）算法进行微调；3. 设计新型奖励系统，整合2D切片级评估和3D体积分析；4. 在BraTS 2019和OASIS-1数据集上进行验证。

Result: 定量分析显示，在Fréchet Inception Distance（FID）指标上有显著改进。更重要的是，与未优化的基线相比，合成数据在下游肿瘤和疾病分类任务中表现出更强的实用性。

Conclusion: 通过强化学习反馈可以有效引导生成过程向更高质量的分布发展，结合多尺度奖励系统能够同时优化局部细节和全局结构，从而生成更具临床相关性的3D医学图像。
...
TL;DR: 提出两种技术（自动聚合和逐像素重缩放）来解决训练免费扩散分割器中注意力图与全局表示之间的差距，提升语义分割性能


<details>
  <summary>Details</summary>
Motivation: 现有基于预训练扩散模型的训练免费分割方法未能充分利用更强大生成模型的能力，主要存在两个问题：跨注意力图在多头多层之间存在差异，以及全局图无法直接转化为准确的语义相关性

Method: 提出两种技术：1）自动聚合（auto aggregation）解决多头多层注意力图与全局表示之间的差距；2）逐像素重缩放（per-pixel rescaling）解决不同文本token之间的分数不平衡问题

Result: 在标准语义分割基准上评估，并与生成技术集成，展示了改进的性能和广泛的适用性

Conclusion: 通过自动聚合和逐像素重缩放技术，训练免费分割方法能够更好地利用生成模型的能力，提高语义分割性能
...
TL;DR: 提出Motion Turing Test框架和HHMotion数据集，用于评估人形机器人动作的人类相似度，并建立自动评分基线模型。


<details>
  <summary>Details</summary>
Motivation: 受图灵测试启发，需要量化评估人形机器人动作与人类动作的相似度，但目前缺乏系统性的评估框架和数据集。

Method: 提出Motion Turing Test评估框架，创建包含1000个动作序列的HHMotion数据集，将动作转换为SMPL-X表示消除外观影响，招募30名标注者进行人工评分，并建立自动预测人类相似度分数的基线模型。

Result: 分析显示人形机器人动作与人类动作存在明显差异，特别是在跳跃、拳击、跑步等动态动作中；提出的基线模型在预测人类相似度分数方面优于多种基于LLM的方法。

Conclusion: 建立了系统的人形机器人动作人类相似度评估框架和数据集，为未来研究提供了基准，并展示了当前人形机器人动作仍需改进，特别是在动态动作方面。
...
<details>
  <summary>Details</summary>
Motivation: 传统基于组织学图像的癌症区域检测方法容易因不同组织区域的形态相似性而产生高假阳性率。空间转录组技术提供了详细的细胞表型和空间定位信息，但现有方法无法有效整合组织学图像和ST数据，特别是在跨样本、跨平台/批次的癌症区域检测场景中。

Method: 提出SpaCRD方法，核心是一个类别正则化的变分重建引导的双向交叉注意力融合网络，使模型能够从多个角度自适应地捕捉组织学特征和基因表达之间的潜在共表达模式。该方法基于迁移学习，在源数据上训练后可以泛化到不同平台和批次的样本。

Result: 在涵盖多种疾病类型、平台和批次的23个匹配的组织学-ST数据集上进行广泛基准分析，结果表明SpaCRD在癌症区域检测方面持续优于现有的8个最先进方法。
...
TL;DR: ALANet提出自适应语言感知网络，通过过滤和优化策略处理不准确的语言输入，解决复杂反射去除问题，并在CRLAV数据集上超越现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有反射去除方法难以处理复杂反射，而反射图像中的模糊和扭曲干扰导致机器生成的语言描述不准确，这损害了语言引导反射去除的性能。

Method: 提出自适应语言感知网络(ALANet)，集成过滤策略（减少语言的负面影响同时保留其益处）和优化策略（增强语言与视觉特征的对齐），并利用语言线索从特征图中解耦特定层内容。

Result: 在提出的CRLAV数据集上实验表明，ALANet在复杂反射和不同语言准确度条件下超越了现有最先进的图像反射去除方法。

Conclusion: ALANet能够有效处理不准确的语言输入，提升复杂反射去除性能，代码和数据集已开源。
...
<details>
  <summary>Details</summary>
Motivation: 全监督方法需要昂贵的帧级标注且对模糊动作边界敏感，点监督方法（每个动作片段仅标注一帧）可以显著降低标注成本，但现有方法在骨架数据上尚未充分探索。

Method: 1) 使用预训练统一模型编码多模态骨架数据（关节、骨骼、运动信息）；2) 提出原型相似性方法，并与能量函数和约束K-Medoids聚类结合生成伪标签；3) 提出多模态伪标签集成增强伪标签可靠性；4) 在PKU-MMD、MCFS-22和MCFS-130数据集上建立基准。

Result: 在PKU-MMD（X-Sub和X-View）、MCFS-22和MCFS-130数据集上建立了新的点监督基准，实验表明该方法达到竞争性性能，甚至超越某些全监督方法，同时显著减少标注工作量。

Conclusion: 提出的点监督框架通过多模态骨架特征和伪标签集成，在显著降低标注成本的同时实现了与全监督方法竞争的性能，为骨架动作分割提供了有效的弱监督解决方案。
...
TL;DR: VG3S提出了一种基于视觉基础模型几何先验的3D高斯溅射框架，用于自动驾驶场景的语义占据预测，显著提升了预测精度。


<details>
  <summary>Details</summary>
Motivation: 当前基于3D高斯溅射的占据预测方法严重依赖准确的几何线索，但在纯视觉中心范式中这些几何信息往往不足。需要利用视觉基础模型强大的几何理解能力来增强占据预测。

Method: 提出VG3S框架，通过即插即用的分层几何特征适配器，将冻结的视觉基础模型的3D几何先验注入到高斯占据预测中。该适配器通过特征聚合、任务特定对齐和多尺度重构来转换通用VFM特征。

Result: 在nuScenes占据基准测试中，VG3S相比基线在IoU上提升了12.6%，在mIoU上提升了7.5%。框架能够无缝泛化到不同的视觉基础模型，一致地提升占据预测精度。

Conclusion: VG3S证明了将来自强大预训练几何基础视觉模型的先验知识整合到占据预测中的巨大价值，为自动驾驶场景理解提供了有效的解决方案。
...
TL;DR: 提出EntON方法，在3D高斯泼溅中引入特征熵优化的邻域致密化策略，通过交替优化框架提升几何精度和渲染质量，同时减少高斯数量和训练时间。


<details>
  <summary>Details</summary>
Motivation: 标准3DGS产生的高斯中心与底层物体几何对齐不佳，而专注于表面的重建方法常牺牲光度精度。需要一种既能保持几何准确性又能保证渲染质量的方法。

Method: 提出特征熵优化的邻域致密化策略EntON：1) 从每个高斯中心的k近邻协方差矩阵特征值计算3D形状特征熵，量化局部结构有序度；2) 建立交替优化框架，在标准梯度致密化和特征熵感知致密化之间切换；3) 特征熵感知致密化优先在低熵（有序、平坦）邻域致密化以捕捉表面细节，在高熵（无序、球形）区域修剪高斯。

Result: 在DTU和TUM2TWIN数据集上的实验表明：几何精度提升达33%，渲染质量提升达7%，高斯数量减少达50%，训练时间减少达23%。

Conclusion: EntON通过避免不必要的场景扩展，在几何精度、渲染质量和效率之间实现了良好平衡，为3D高斯泼溅提供了更优的致密化策略。
...
TL;DR: WAFL将时序伪造定位任务从时间回归和连续定位转变为离散词级二分类，通过词锚定、特征重对齐和不对称损失函数，显著提升了定位性能并降低了计算成本。
...
Method: 提出词锚定时序伪造定位(WAFL)范式：1) 将最小有意义的伪造单元定义为词标记；2) 引入法证特征重对齐模块将预训练语义特征映射到判别性法证流形；3) 设计基于伪影的不对称损失函数处理极端类别不平衡。

Result: WAFL在数据集内和跨数据集设置下均显著优于现有方法，同时需要更少的可学习参数并具有更高的计算效率。

Conclusion: 将时序伪造定位重新定义为词级二分类任务是有效的，WAFL通过特征重对齐和不对称损失函数解决了现有方法的局限性，实现了高性能和高效率的伪造定位。
...
<details>
  <summary>Details</summary>
Motivation: 现有异步事件神经网络虽然能实现低延迟检测，但存在两个瓶颈：循环架构难以在长序列上高效训练，提高精度往往增加单事件计算量和延迟。线性注意力虽支持并行训练和循环推理，但标准方法为每个事件更新全局状态，导致精度-效率权衡不佳。

Method: 提出空间稀疏线性注意力（SSLA），引入混合空间状态分解和散射-计算-聚集训练过程，实现状态级稀疏化和训练并行化。基于SSLA构建端到端异步线性注意力模型SSLA-Det用于事件相机目标检测。

Result: 在Gen1和N-Caltech101数据集上，SSLA-Det在异步方法中达到最先进精度，分别获得0.375 mAP和0.515 mAP，同时相比最强异步基线减少超过20倍的单事件计算量。

Conclusion: SSLA-Det展示了线性注意力在低延迟事件视觉中的潜力，通过状态级稀疏化实现了精度和效率的良好平衡，为事件相机目标检测提供了高效解决方案。
...
TL;DR: HCF-RES是一个多模态3D指代表达分割框架，通过层次化视觉语义分解和渐进式多级融合，在稀疏点云基础上引入2D视觉语义，提升对细粒度描述的定位能力。
...
Method: 提出HCF-RES框架：1) 层次化视觉语义分解：利用SAM实例掩码指导CLIP编码，获取像素级和实例级特征，在2D到3D投影中保持对象边界；2) 渐进式多级融合：通过模态内协作、跨模态自适应加权和语言引导细化，集成2D语义和3D几何特征。

Result: 在ScanRefer和Multi3DRefer数据集上达到最先进性能。

Conclusion: 通过结合2D视觉语义和3D几何信息的多模态方法，HCF-RES显著提升了3D指代表达分割的准确性，特别是在处理复杂自然语言描述时。
...
TL;DR: NOVA提出了一种基于大语言模型自回归生成的新范式，将3D多目标跟踪从传统的距离匹配转向时空语义建模，显著提升了开放词汇场景下的跟踪性能。


<details>
  <summary>Details</summary>
Motivation: 现有3D多目标跟踪方法受限于闭集假设和"语义盲"启发式规则，难以泛化到未知目标类别。开放世界感知需要能够处理未知目标的通用跟踪能力。

Method: 将3D轨迹重新定义为结构化时空语义序列，利用大语言模型的自回归能力，将跟踪任务转化为原则性的下一步序列补全过程，通过语言空间层次结构解决语义模糊性并保持身份一致性。

Result: 在nuScenes、V2X-Seq-SPD和KITTI数据集上表现优异。在nuScenes数据集上，NOVA在Novel类别上达到22.41%的AMOTA，相比基线绝对提升20.21%，仅使用0.5B参数的自回归模型。

Conclusion: NOVA通过生成式时空语义建模范式，成功将3D跟踪从传统距离匹配转向语义感知，为开放世界感知提供了有效的解决方案，展示了语言模型在复杂时空推理任务中的潜力。
...
TL;DR: 提出了一种用于移动载体的实时无人机检测系统，采用旋转事件相机实现360°视野，无需运动补偿，在Jetson Orin NX上实时运行，平均角度误差小于2°


<details>
  <summary>Details</summary>
Motivation: 现有无人机检测系统主要基于静态相机，限制了在移动载体（如四足机器人、无人地面车辆）上的应用。事件相机具有高动态范围和微秒级响应，适合检测快速移动目标，但现有事件相机方案假设相机静止，无法适应移动载体场景。

Method: 使用旋转事件相机提供360°水平视野；提出无需运动补偿的图像式事件表示方法；设计轻量级神经网络架构进行高效的时空学习；在Jetson Orin NX上实现实时运行。

Result: 系统在户外实验中验证了可靠检测性能，在挑战性条件下平均角度误差低于2°，能够在移动载体上实时运行，适合实际监控应用。

Conclusion: 该系统成功解决了移动载体上的无人机检测问题，结合旋转事件相机和无需运动补偿的表示方法，实现了高精度实时检测，将开源完整流程以支持未来研究。
...
TL;DR: 提出StrSR框架，通过谱和轨迹正则化解决DiT在真实图像超分辨率中的单步蒸馏问题，消除网格状伪影


<details>
  <summary>Details</summary>
Motivation: 扩散变换器(DiT)在真实图像超分辨率中表现优异，但其迭代采样计算成本高，需要单步蒸馏。现有单步蒸馏方法在DiT上存在轨迹不匹配和严重网格状周期性伪影问题

Method: 提出StrSR框架，包含非对称判别蒸馏架构来弥合轨迹差距，以及频率分布匹配策略来抑制DiT特有的高频谱泄漏引起的周期性伪影

Result: 大量实验表明，StrSR在真实图像超分辨率任务中实现了最先进的性能，在定量指标和视觉感知方面都表现出色
...
TL;DR: 该论文提出了首个针对3D语义占据预测中标签噪声问题的基准测试OccNL，并开发了DPR-Occ框架，通过双源部分标签推理在极端噪声下保持性能稳定。
...
Method: 提出DPR-Occ框架，通过双源部分标签推理构建可靠监督：结合时间模型记忆和表示层结构相似性，动态扩展和修剪候选标签集，在抑制噪声传播的同时保留真实语义。

Result: 在SemanticKITTI上的实验表明，DPR-Occ在极端噪声下防止了几何和语义崩溃。即使在90%标签噪声下，相比现有标签噪声学习方法，仍能获得显著性能提升（最高2.57% mIoU和13.91% IoU）。

Conclusion: 通过连接标签噪声学习和3D感知，OccNL和DPR-Occ为动态环境中安全关键的机器人感知提供了可靠基础，揭示了现有方法在稀疏3D体素空间中的根本性脆弱性。
...
TL;DR: ADiVA方法通过属性分布建模和语义-视觉对齐解决生成式零样本学习中的类-实例差距和语义-视觉域差距问题，显著提升了多个基准数据集上的性能。
...
Method: 提出ADiVA方法，包含两个模块：1）属性分布建模（ADM）模块学习可迁移的属性分布并为未见类采样实例级属性；2）视觉引导对齐（VGA）模块细化语义表示以更好地反映视觉结构。

Result: 在三个广泛使用的基准数据集上，ADiVA显著优于现有最先进方法（如在AWA2和SUN上分别获得4.7%和6.1%的提升），并可作为插件增强现有生成式ZSL方法。

Conclusion: ADiVA通过联合建模属性分布和执行显式语义-视觉对齐，有效解决了生成式零样本学习中的关键挑战，为提升ZSL性能提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有无训练方法依赖T2V模型中间输出构建运动指导，计算开销大且灵活性有限，需要更高效灵活的运动迁移方案

Method: 1) 利用流基T2V模型的预测输出直接提取运动表示；2) 提出流指导从潜在预测中提取运动表示对齐源视频与生成视频的运动模式；3) 引入速度正则化策略稳定优化并确保平滑运动演化

Result: FlowMotion在时间和资源效率上优于现有方法，同时保持有竞争力的性能表现

Conclusion: 通过直接操作模型预测，FlowMotion实现了高效灵活的视频运动迁移，为无训练运动转移提供了新思路
...
TL;DR: 提出基于垂直分数扩散模型的3D牙科种植体修复方法，通过结合两个不同平面的2D扩散模型来处理CBCT投影序列，减少金属伪影，提高图像质量。
...
Method: 提出3D牙科种植体修复方法，基于垂直分数扩散模型。训练两个在不同平面上的2D分数扩散模型，在采样方案中结合这两个模型来建模投影序列的3D分布，在投影域进行操作。

Result: 该方法能有效生成高质量、伪影减少的3D CBCT图像，证明其在改善临床成像方面的潜力。

Conclusion: 提出的基于垂直分数扩散模型的3D牙科种植体修复方法能有效减少CBCT中的金属伪影，提高图像质量，是改善临床成像的有前景的解决方案。
...
TL;DR: LTA是一种基于潜在空间的对抗攻击方法，通过在Stable Diffusion VAE的潜在空间中优化扰动，生成空间相干、低频的对抗样本，显著提升了跨架构的迁移攻击成功率。


<details>
  <summary>Details</summary>
Motivation: 传统像素空间对抗攻击产生的高频噪声对预处理（如调整大小、裁剪）脆弱，且跨架构迁移效果差。需要探索更鲁棒、迁移性更强的攻击方法。

Method: 使用预训练的Stable Diffusion VAE将图像编码到潜在空间，在潜在空间中优化扰动以最大化代理分类器损失，同时通过解码后软约束像素空间的ℓ∞预算。采用EOT（随机调整大小、插值、裁剪）和周期性潜在高斯平滑来提升鲁棒性。

Result: LTA在CNN和视觉Transformer目标模型上实现了强大的迁移攻击成功率，生成的扰动具有空间相干性和低频特性，与像素空间基线有质的区别，在迁移-质量权衡中占据独特位置。
...
TL;DR: 提出一种小波增强的专家混合提示学习方法，用于零样本异常检测，通过变分自编码器建模全局语义、小波分解提取多频特征、专家混合模块聚合上下文信息，在14个工业和医学数据集上验证了有效性。


<details>
  <summary>Details</summary>
Motivation: 现有零样本异常检测方法通常依赖固定文本提示，难以捕捉复杂语义，且仅关注空间域特征，限制了检测细微异常的能力。需要解决这些挑战以提升检测性能。

Method: 1) 使用变分自编码器建模全局语义表示并集成到提示中，增强对多样化异常模式的适应性；2) 通过小波分解提取多频图像特征，通过跨模态交互动态优化文本嵌入；3) 引入语义感知的专家混合模块聚合上下文信息。

Result: 在14个工业和医学数据集上进行了广泛实验，证明了所提方法的有效性。

Conclusion: 提出的小波增强专家混合提示学习方法能够有效解决现有零样本异常检测方法的局限性，通过结合全局语义建模、多频特征提取和上下文聚合，显著提升了检测性能。
...
Method: 提出原型库驱动的无监督点云语义分割策略P-SLCR：1）一致性结构学习：在一致性点和一致性原型库之间建立结构特征学习；2）语义关系一致性推理：分别构建一致性和模糊性原型库之间的原型互关系矩阵，通过该矩阵约束保持语义一致性。

Result: 在S3DIS、SemanticKITTI和Scannet数据集上广泛评估，相比无监督方法取得最佳性能。在S3DIS数据集Area-5上达到47.1%的mIoU，比经典全监督方法PointNet高出2.5%。

Conclusion: P-SLCR方法通过原型库驱动、结构学习和一致性推理，有效解决了无监督点云语义分割的挑战，在多个数据集上验证了其优越性。
...
<details>
  <summary>Details</summary>
Motivation: 扩散世界模型虽然具有统一世界模拟的潜力，但其迭代去噪过程计算成本过高，难以用于交互式使用和长时程推演。现有特征缓存方法虽然能加速推理，但由于令牌异构性和非均匀时间动态这两个世界模型特有的障碍，导致为单模态扩散设计的策略无法有效迁移。
...
Conclusion: WorldCache通过专门针对扩散世界模型设计的缓存框架，有效解决了令牌异构性和非均匀时间动态问题，显著提升了扩散世界模型的推理效率，使其更适合交互式使用和长时程推演。
...
<details>
  <summary>Details</summary>
Motivation: 现有3D网格生成方法通常依赖等值面提取或启发式网格化，难以同时保持复杂几何结构和良好拓扑，且推理效率较低。需要一种能够直接生成显式3D网格并保持拓扑结构的高效方法。

Method: LATO使用顶点位移场(VDF)表示网格，通过稀疏体素VAE压缩为结构化体素潜在表示。VAE解码器通过渐进细分和剪枝实例化顶点位置，专用连接头预测顶点对之间的边连接性。采用两阶段流匹配：首先生成结构体素，然后细化体素拓扑特征。

Result: 相比基于等值面/三角形的扩散模型和自回归生成方法，LATO能够生成具有复杂几何结构、良好拓扑的网格，同时推理效率显著提高。

Conclusion: LATO提供了一种可扩展的拓扑保持3D网格生成框架，通过直接预测网格连接性避免了传统网格化步骤的局限性，在几何质量和计算效率方面均优于现有方法。
...
<details>
  <summary>Details</summary>
Motivation: 传统干重测量方法需要手动、耗时且破坏性的干燥称重过程，限制了生物多样性监测的规模化。开发仅通过图像就能估计无脊椎动物生物量的方法，可以大大提升定量生物多样性监测的效率。

Method: 提出两种方法：1）使用面积和沉降速度作为预测因子，通过BIODISCOVER双摄像头系统在乙醇柱中捕获标本沉降图像序列；2）训练端到端深度神经网络家族，包括单视图、多视图和元数据感知架构。收集了大量干重测量和图像序列配对数据集进行训练和评估。

Result: 方法能够估计形态复杂多样的标本干重。结合自动分类学分类，该方法在个体水平上实现了10-20%的中位数百分比误差，是组级干重估计的准确方法。研究强调了选择适当评估指标的重要性，建议同时使用百分比误差和绝对误差。

Conclusion: 计算机视觉方法可以替代传统破坏性干重测量，实现高效、非破坏性的无脊椎动物生物量估计。该方法为规模化生物多样性监测提供了可行方案，并探索了不同的优化损失、数据增强方法和模型架构对深度学习模型训练的影响。
...
TL;DR: OralGPT-Plus：基于智能体交互和对称感知推理的牙科全景X光片分析视觉语言模型，通过迭代诊断和强化学习提升临床可靠性


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型采用静态单次推理范式，无法满足牙科全景X光片分析所需的细粒度空间推理、双侧对称性理解和多步诊断验证需求，限制了临床可靠性

Method: 1. 构建DentalProbe数据集（5000张图像，专家标注诊断轨迹）；2. 开发Reinspection-driven强化学习框架（基于规则的奖励和条件诊断驱动奖励）；3. 提出OralGPT-Plus智能体视觉语言模型，支持迭代对称感知诊断推理

Result: OralGPT-Plus在MMOral-X基准测试（首个全景诊断综合基准）和现有全景基准上均优于强基线模型，显示出交互式和对称感知推理的有效性

Conclusion: 智能体建模在牙科影像分析中具有重要价值，为临床对齐的全景X光片分析研究奠定了基础，展示了交互式、对称感知推理在提升诊断可靠性方面的潜力
...
TL;DR: Rewis3d利用前馈式3D重建技术提升2D图像的弱监督语义分割性能，通过3D几何结构传播稀疏标注，在稀疏监督场景下达到SOTA水平。
...
Method: 采用双师生架构，利用前馈式3D重建从2D视频恢复3D几何结构，通过2D图像与重建3D点云之间的语义一致性来传播稀疏标注。

Result: Rewis3d在稀疏监督场景下达到最先进性能，相比现有方法提升2-7%，且无需额外标注或推理开销。
...
<details>
  <summary>Details</summary>
Motivation: 现有MIL方法将WSI视为无序补丁集合，限制了捕捉全局组织与局部模式之间结构化依赖关系的能力。虽然SSM能有效建模长序列，但如何结构化WSI token以充分利用其空间层次结构仍是一个开放问题

Method: 提出MoEMambaMIL框架：1）通过多分辨率预处理将补丁token组织成保持空间包含关系的区域感知序列；2）结合静态分辨率特定专家和动态稀疏专家，实现分辨率感知编码和区域自适应上下文建模；3）采用区域嵌套选择性扫描和混合专家建模

Result: 在9个下游任务中实现了最佳性能

Conclusion: MoEMambaMIL通过结构化序列组织和专家专业化，有效解决了WSI分析中的长序列建模和空间层次利用问题，为组织病理学图像分析提供了新框架
...
TL;DR: CHMv2是一个基于DINOv3深度估计模型从高分辨率光学卫星影像生成的全球米级分辨率冠层高度图，相比现有产品在精度、偏差和细节保留方面有显著提升。


<details>
  <summary>Details</summary>
Motivation: 精确的冠层高度信息对于森林碳量化、恢复与退化监测以及栖息地结构评估至关重要，但全球范围内高精度的机载激光扫描（ALS）数据分布不均，需要一种更广泛可用的替代方案。

Method: 使用DINOv3构建深度估计模型，通过大规模地理多样化的训练数据、自动化数据整理与配准、针对冠层高度分布定制的损失函数和数据采样策略，从高分辨率光学卫星影像生成冠层高度图。

Result: CHMv2相比现有产品显著提高了精度，减少了高森林的偏差，更好地保留了冠层边缘和间隙等精细结构。通过独立ALS测试集以及数千万个GEDI和ICESat-2观测数据的验证，证明其在主要森林生物群落中具有一致的性能。

Conclusion: CHMv2提供了一个全球范围内高精度、米级分辨率的冠层高度图，解决了ALS数据分布不均的问题，为森林监测和生态研究提供了可靠的数据支持。
...
TL;DR: REACT++ 是基于 REACT 架构改进的场景图生成模型，通过高效特征提取和原型空间中的主体-客体交叉注意力，在保持物体检测性能的同时，显著提升了关系预测准确性和推理速度。


<details>
  <summary>Details</summary>
Motivation: 当前场景图生成方法通常只关注关系预测准确性、物体检测准确性或延迟降低中的单一目标，缺乏同时平衡这三个目标的解决方案。为了支持实时应用，需要在性能与推理速度之间取得更好的权衡。

Method: 基于 REACT 架构，采用高效特征提取和原型空间中的主体-客体交叉注意力机制，平衡延迟和表示能力。

Result: REACT++ 在现有 SGG 模型中达到最高推理速度，在保持物体检测性能的同时提升了关系预测准确性。相比前代 REACT，速度提升 20%，关系预测准确性平均提升 10%。

Conclusion: REACT++ 成功平衡了场景图生成中的三个关键目标：关系预测准确性、物体检测准确性和推理速度，为实时应用提供了有效的解决方案。
...
TL;DR: 提出人机协同拼图框架，结合自动求解器和人工交互指导，解决大规模考古碎片重建难题


<details>
  <summary>Details</summary>
Motivation: 传统拼图求解器在真实考古碎片重建中面临挑战：碎片数量大（数千片）、侵蚀严重、形状不规则、存在缺失区域和大规模模糊性，无法有效处理复杂文化遗产重建

Method: 提出人机协同框架，集成自动松弛标记求解器与交互式人工指导；用户可迭代锁定已验证的拼合位置、纠正错误，引导系统实现语义和几何一致的组装；引入两种互补交互策略：迭代锚定和连续交互优化

Result: 在多个RePAIR数据集上的实验表明，该混合方法在准确性和效率上显著优于全自动和纯人工基线，为大规模专家参与文物重建提供实用解决方案

Conclusion: 人机协同方法能有效解决大规模考古碎片重建的复杂性和规模问题，为文化遗产保护提供可行的技术方案
...
TL;DR: DiffInf使用自影响引导的扩散框架修复面部属性标注不一致问题，通过生成式校正对齐图像内容与标签，提升分类性能


<details>
  <summary>Details</summary>
Motivation: 面部属性分类依赖大规模标注数据集，但年龄、表情等属性本质上是连续且模糊的，被离散化为分类标签时会产生标注不一致。这种不一致源于主观性和姿态、光照、表情、人口统计变异等视觉混淆因素，导致图像与标签不匹配，从而引入监督误差，损害表示学习并降低下游预测性能。

Method: 1. 首先训练基线分类器，使用一阶近似计算样本级自影响分数，识别对优化过程产生不成比例影响的训练实例；2. 不丢弃这些高影响样本，而是通过潜在扩散自编码器进行有针对性的生成校正，更好地对齐视觉内容与分配标签，同时保持身份和真实感；3. 训练轻量级高影响成员预测器作为替代影响正则器，在校正过程中提供可微分指导；4. 编辑后的样本替换原始样本，生成大小不变的影响精炼数据集。

Result: 在多类面部属性分类任务中，DiffInf相比标准噪声标签训练、鲁棒优化基线和基于影响的过滤方法，持续改善了泛化性能。结果表明，在图像层面修复有影响的标注不一致，可以在不牺牲分布覆盖的情况下增强下游面部属性分类。

Conclusion: 通过自影响引导的扩散框架修复标注不一致，能够有效提升面部属性分类性能，该方法通过生成式校正而非简单丢弃样本，保持了数据集的完整性和分布覆盖。
...
TL;DR: BEiT视觉Transformer在感知冲突中偏好凸性补全，研究发现这种偏好由特定注意力头（L0H9）控制，调整该头可改变补全决策


<details>
  <summary>Details</summary>
Motivation: 研究视觉Transformer如何解决图形-背景组织问题，特别是在局部几何证据与全局组织先验（如格式塔凸性先验）之间的冲突，探索BEiT内部组件如何实现这种感知偏好

Method: 使用合成飞镖形状创建受控感知冲突，系统性地掩蔽可接受凹性或凸性补全的区域；通过logit归因将内部激活投影到离散视觉码本空间，分解注意力头的直接影响

Result: BEiT在感知冲突中可靠地偏好凸性补全；图形-背景组织在早期和中间层保持模糊，在后期层突然解决；识别出头L0H9作为早期种子引入弱凸性偏置；下调该注意力头可改变分布质量，使凹性证据引导补全

Conclusion: BEiT的凸性偏好由特定可识别的功能单元控制，特别是注意力头L0H9，这表明Transformer子结构中存在专门处理格式塔先验的机制，为理解视觉Transformer的感知组织提供了新见解
...
<details>
  <summary>Details</summary>
Motivation: 监测鱼类生长行为对水产养殖和家庭水族箱中的鱼类健康至关重要，但传统测量方法面临挑战：鱼类体型小且水族箱环境存在强烈折射畸变。需要一种实用、非侵入式的图像测量方法。

Method: 使用YOLOv11-Pose网络检测鱼类并预测解剖关键点；通过考虑空气-玻璃-水界面的折射感知极线约束实现鲁棒匹配；使用学习质量分数过滤不可靠检测；通过折射感知3D三角测量恢复3D关键点，进而测量鱼类长度。

Result: 在濒危苏拉威西米鱼的新立体数据集上验证了方法，证明过滤低质量检测对准确长度估计至关重要。系统为家庭水族箱提供了简单实用的非侵入式生长监测解决方案。

Conclusion: 该方法提供了一种简单实用的非侵入式鱼类长度测量方案，可轻松应用于家庭水族箱环境，有效解决了水族箱折射畸变带来的测量挑战。
...
TL;DR: WanderDream是一个用于心理探索模拟的大规模数据集，包含全景视频和问答对，支持在无法实际探索的情况下进行空间推理。


<details>
  <summary>Details</summary>
Motivation: 在现实场景中，机器人或视障用户常因物理限制或安全考虑无法进行主动探索。需要让智能体仅凭有限观察就能心理模拟未来轨迹并回答空间假设问题。

Method: 构建WanderDream数据集：WanderDream-Gen包含15.8K个全景视频，覆盖1,088个真实场景；WanderDream-QA包含158K个问答对，涵盖轨迹的起始状态、路径和结束状态。

Result: 实验表明：(1)心理探索对情境推理至关重要；(2)世界模型在WanderDream-Gen上表现优异；(3)想象力显著提升WanderDream-QA上的推理能力；(4)数据在真实场景中具有出色的可迁移性。

Conclusion: WanderDream是首个用于心理探索模拟的大规模数据集，使模型能够在无需主动探索的情况下进行空间推理，在现实世界应用中具有重要价值。
...
TL;DR: CaTok提出了一种1D因果图像分词器，通过MeanFlow解码器解决视觉AR模型中的因果对齐问题，在ImageNet重建上达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 当前视觉分词器要么将2D块展平为非因果序列，要么采用启发式排序与"下一词预测"模式不对齐。扩散自编码器也存在因果性不足或平衡性问题。

Method: 提出CaTok：1D因果图像分词器，通过时间间隔选择token并绑定到MeanFlow目标，学习因果1D表示。还提出REPA-A正则化，对齐编码器特征与视觉基础模型以稳定训练。

Result: 在ImageNet重建上达到0.75 FID、22.53 PSNR和0.674 SSIM的SOTA结果，训练轮次更少。AR模型性能与领先方法相当。

Conclusion: CaTok成功解决了视觉AR模型中的因果对齐问题，支持快速一步生成和高保真多步采样，同时自然捕捉跨token间隔的多样化视觉概念。
...
TL;DR: Pinterest Canvas是一个大规模图像生成系统，通过基础扩散模型和任务特定微调来支持图像编辑和增强用例，在背景增强和宽高比外绘等任务上显著提升用户参与度。


<details>
  <summary>Details</summary>
Motivation: 现有图像生成模型虽然灵活，但难以通过提示或简单推理适应来控制，不适合有严格产品要求的用例。Pinterest需要专门系统来支持其平台上的图像编辑和增强任务。

Method: 首先在多样化多模态数据集上训练基础扩散模型，然后针对具体下游任务在任务特定数据集上快速微调，产生专门化模型。系统包括数据集管理、训练和推理最佳实践。

Result: 在线A/B实验显示，增强图像分别获得18.0%和12.5%的参与度提升。人工评估表明模型在这些任务上优于第三方模型。系统还能泛化到多图像场景合成和图像到视频生成等任务。

Conclusion: Pinterest Canvas通过基础模型加任务特定微调的方法，成功构建了能够满足严格产品要求的大规模图像生成系统，在多个图像编辑任务上表现出色且可泛化到其他下游应用。
...
TL;DR: 该论文系统研究去噪生成模型的训练目标，重点分析损失权重和输出参数化（噪声、干净图像、速度）如何与数据流形内在维度、模型架构和数据集大小相互作用，旨在提供实用的设计选择指导而非提出新方法。


<details>
  <summary>Details</summary>
Motivation: 去噪生成模型（如扩散模型、流匹配模型）的训练目标设计存在多种选择（损失权重、输出参数化），但这些选择如何与数据特性、模型架构等因素相互作用尚不清楚。作者旨在系统分析这些因素，为实际训练提供指导而非提出新方法。

Method: 通过系统数值研究，分析损失权重和输出参数化（噪声、干净图像、速度）与数据流形内在维度、模型架构、数据集大小的相互作用。实验涵盖具有可控几何形状的合成数据集和图像数据，使用PSNR（去噪精度）和FID（生成质量）作为评估指标。

Result: 研究揭示了不同训练目标与数据特性、模型架构等因素之间的复杂相互作用，为去噪生成模型的设计选择提供了实证依据和实用指导。

Conclusion: 该研究系统分析了去噪生成模型训练目标的关键因素，为实际应用中的设计选择提供了实证指导，强调了根据数据特性、模型架构等因素优化训练目标的重要性。
...
<details>
  <summary>Details</summary>
Motivation: 现有放射学基础模型（RFMs）开发过度依赖暴力扩展方法，直接迁移自然图像处理方法，导致模型在临床实践中脆弱且昂贵，需要更高效、精确的预训练框架。
...
Result: 在超过20万张图像（四个机构，两种模态）的实验中，GreenRFM在胸部和腹部CT数据集上表现优异，超越多个基线模型。在内部肌肉骨骼MRI图像上也验证了监督原则的跨模态可迁移性。

Conclusion: GreenRFM挑战了"规模即一切"的教条，其性能和效率使临床医生即使在笔记本电脑上也能公平地开发SOTA放射学基础模型，推动了RFMs的民主化发展。
...
<details>
  <summary>Details</summary>
Motivation: 在医疗影像等专业领域，逐帧视频标注成本高昂且耗时。现有标注传播方法存在局限性：视频跟踪器需要逐视频初始化且无法跨视频泛化；传统对应方法在低纹理场景表现不佳；密集特征匹配方法缺乏时空平滑性

Method: 提出Match4Annotate框架：1）使用SIREN隐式神经表示拟合DINOv3特征，生成连续高分辨率时空特征场；2）学习帧对之间的平滑隐式变形场来指导对应匹配；3）支持点标注和掩码标注的跨视频传播

Result: 在三个临床超声数据集上评估：1）跨视频传播达到最先进水平，优于特征匹配和单次分割基线；2）视频内传播与专用跟踪器竞争力相当；3）轻量级测试时优化特征匹配管道为可扩展标注工作流提供高效解决方案

Conclusion: 轻量级、测试时优化的特征匹配管道有潜力为可扩展标注工作流提供高效且易用的解决方案，特别是在医疗影像等专业领域
...
TL;DR: Self-Flow：一种自监督流匹配范式，通过双时间步调度机制，在生成框架中集成表示学习，无需外部模型监督，实现跨模态的优越生成质量。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖外部模型进行语义表示学习，但这些模型需要单独训练、目标不一致且表现出意外的缩放行为。这种依赖源于生成模型的训练目标（去噪任务）缺乏学习语义表示的激励。

Method: 提出Self-Flow自监督流匹配范式，核心机制是双时间步调度：对不同的token应用异构噪声水平，创建信息不对称，迫使模型从被破坏的输入中推断缺失信息，从而同时学习强表示和生成能力。

Result: 方法在图像、视频和音频生成上取得优越性能，能够跨模态泛化并支持多模态训练，同时遵循预期的缩放定律。

Conclusion: Self-Flow成功地将表示学习集成到生成框架中，消除了对外部模型的依赖，通过自监督方式同时学习语义表示和生成能力，实现了跨模态的优越生成质量。
...
Method: SCAN采用自编码器方法重建模型中间层特征，基于信息瓶颈原理生成高分辨率自置信度图，识别信息丰富的区域。该框架适用于卷积神经网络和Transformer架构。

Result: 在多种架构和数据集上的实验表明，SCAN在AUC-D、Negative AUC、Drop%、Win%等定量指标上表现优异，定性分析显示其产生的解释比现有方法更清晰、更聚焦于对象。

Conclusion: SCAN提供了一个统一的框架，既具有架构通用性又保持高保真度，增强了模型透明度，为理解复杂神经网络的决策过程提供了更可靠的工具。
...
TL;DR: AV-Unified是一个统一的多任务音频-视觉场景理解框架，通过将不同任务的输入输出转换为离散token序列，实现跨异构数据集的联合学习，并采用多尺度时空感知模块来捕捉音频-视觉关联。


<details>
  <summary>Details</summary>
Motivation: 当前音频-视觉场景理解任务（如事件定位、解析、分割、问答等）大多被单独研究，难以全面理解复杂场景和探索任务间关系，需要统一的框架来整合这些任务。

Method: 1) 将所有任务的输入输出标准化为离散token序列，建立共享表示；2) 设计多尺度时间感知模块捕捉不同时间粒度的音频-视觉事件；3) 设计跨模态引导的空间感知模块建模空间音频-视觉关联；4) 使用任务特定文本提示增强模型适应性和任务感知能力。

Result: 在多个基准数据集（AVE、LLP、MUSIC-AVQA、VGG-SS、AVS）上的实验表明，AV-Unified在时间、空间和时空任务上均表现出有效性。

Conclusion: AV-Unified提供了一个统一的框架，能够联合学习广泛的音频-视觉场景理解任务，通过标准化表示和多尺度感知有效捕捉音频-视觉关联，为全面理解复杂音频-视觉场景提供了有效解决方案。
...
Method: 使用扫描的逆反射贴片配合背景减法，恢复每个像素的响应图，建立LiDAR到RGB的显式对应关系

Result: 在ams OSRAM TMF8828上演示了该方法，能够为跨模态对齐和融合提供明确的LiDAR-RGB对应关系

Conclusion: 该方法解决了漫反射LiDAR与RGB相机校准的特殊挑战，实现了有效的空间校准和跨模态对齐
...
TL;DR: 提出了一种在扩散模型中处理语言否定的统一框架，通过将否定建模为语义引导的结构化可行性约束，无需重新训练模型参数
...
Method: 将否定重新解释为扩散动力学中语义引导的结构化可行性约束，将无分类器引导定义为语义更新方向，并通过将更新投影到从语言结构导出的凸约束集来强制执行否定

Result: 方法实现了稳健的否定合规性，同时保持视觉保真度和结构连贯性，建立了扩散生成模型中语言否定的第一个统一表述

Conclusion: 提出了一个训练免费、与预训练扩散主干兼容的统一框架，能够处理多样化的否定现象，并扩展到视频生成，同时引入了专门的基准套件来评估生成系统中的语言失败模式
...
TL;DR: SurgFormer是一种基于多分辨率门控Transformer的软组织模拟方法，能够在体积网格上实现数据驱动的实时软组织变形预测，支持切割条件下的模拟。


<details>
  <summary>Details</summary>
Motivation: 高保真生物力学求解器通常计算成本过高，难以实现交互式应用。需要开发能够在大型网格上快速预测节点位移场的学习方法，同时支持标准变形和拓扑改变（如切割）两种情况。

Method: 构建固定网格层次结构，应用多分支块结合局部消息传递、粗粒度全局自注意力和逐点前馈更新，通过学习的逐节点、逐通道门控自适应整合局部和远程信息。对于切割条件模拟，将切除信息编码为学习的切割嵌入作为额外输入。

Result: 在两个手术模拟数据集（胆囊切除术和阑尾切除术）上评估，SurgFormer在保持高效性的同时实现了高精度，成为两种任务（标准变形和切割条件变形）的实用骨干模型。

Conclusion: SurgFormer是第一个在相同体积管道中研究XFEM监督切割条件变形的学习体积代理方法，为手术模拟提供了准确高效的解决方案。
...
TL;DR: 该论文研究了自动驾驶数据集中冗余作为数据质量因素的重要性，通过分析多源摄像头和多模态图像-LiDAR数据的冗余性，并评估去除冗余标签对目标检测性能的影响。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶研究通常优先考虑算法设计而忽视数据质量分析，但实际中多源多模态数据质量因环境条件和传感器限制而异。冗余作为数据质量的基本但未充分探索的问题，对自动驾驶性能有直接影响。

Method: 使用nuScenes和Argoverse 2数据集，对多源摄像头数据和多模态图像-LiDAR数据中的冗余进行建模和测量，评估去除冗余标签对YOLOv8目标检测任务的影响。

Result: 在nuScenes中，选择性去除共享视野摄像头中的冗余多源图像目标标签可提升检测性能（mAP50从0.66提升到0.70等）。在AV2中，去除4.1-8.6%标签后mAP50保持在0.64基线附近。多模态分析显示图像和LiDAR数据间存在显著冗余。
...
TL;DR: EgoReasoner：针对自我中心视频4D推理任务的两阶段框架，通过任务自适应思维模板和任务感知奖励函数，在少量样本上超越更大模型


<details>
  <summary>Details</summary>
Motivation: 自我中心视频理解面临动态4D环境的复杂性，现有任务无关方法（如通用思维链和统一强化学习）无法适应不同认知操作（空间锚定、时间追踪、持续时间推理）的结构差异，导致性能不稳定

Method: 提出EgoReasoner两阶段框架：1）任务自适应思维模板指导生成结构化思维链轨迹，通过监督微调使模型适应不同任务类型；2）任务感知奖励函数验证实体基础、时间对齐和任务自适应逻辑一致性，通过GRPO进行强化微调，选择性加强各推理路径
...
<details>
  <summary>Details</summary>
Motivation: 当前VLM发展主要依赖模型规模扩展，这阻碍了在移动和边缘设备上的部署。传统VLMs依赖对比预训练（如CLIP/SigLIP）初始化的视觉编码器存在目标不匹配问题：对比学习为区分性任务优化，强制了类别级不变性，抑制了密集描述和复杂推理所需的细粒度视觉线索。

Method: 提出Penguin-VL，其视觉编码器从纯文本LLM初始化，而非传统的对比预训练方法。这种方法能更好地保留细粒度的空间和时间线索，提高视觉保真度和数据效率。

Result: 在各种图像和视频基准测试中，Penguin-VL在数学推理上与领先VLMs（如Qwen3-VL）相当，在文档理解、视觉知识和多视角视频理解等任务上超越它们。这些成果是在轻量级架构（2B和8B）下实现的，表明改进视觉表示而非模型缩放是性能提升的主要驱动力。

Conclusion: Penguin-Encoder作为传统对比预训练编码器的优越替代方案，能更好地保留密集感知和复杂推理所需的关键细粒度线索，为计算高效的VLMs提供了强大的即插即用替代方案，在资源受限环境中实现高性能。
...
TL;DR: SCOPE是一个用于3D点云增量少样本分割的即插即用框架，通过利用未标注背景中的伪实例来增强原型表示，在保持低遗忘的同时显著提升新类别的分割性能。


<details>
  <summary>Details</summary>
Motivation: 3D点云的增量少样本分割研究不足，现有方法存在灾难性遗忘问题，或无法在稀疏监督下学习判别性原型，且忽视了新类别经常作为未标注背景出现在基础训练场景中的关键线索。

Method: 提出SCOPE框架：基础训练后，使用类别无关分割模型从背景区域提取高置信度伪实例构建原型池；当新类别到达时，从背景原型池中检索相关原型，与少样本原型融合形成增强表示，无需重新训练骨干网络或增加参数。

Result: 在ScanNet和S3DIS数据集上达到SOTA性能：新类别IoU分别提升6.98%和3.61%，平均IoU分别提升2.25%和1.70%，同时保持低遗忘率。

Conclusion: SCOPE通过利用背景中的未标注信息有效增强了3D点云增量少样本分割的原型表示，显著提升了新类别的分割性能，同时缓解了灾难性遗忘问题。
...
<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型主要采用自回归架构，在架构设计上存在改进空间。离散扩散模型在其他领域表现出色，有潜力成为多模态系统的骨干。

Method: 使用统一的掩码离散扩散模型直接捕捉离散多模态token的联合分布，支持双模态和多模态任务。
...
TL;DR: 研究发现多模态大语言模型（MLLM）分类性能评估存在严重偏差，主要源于评估协议缺陷和标注质量问题，而非模型本身能力不足。


<details>
  <summary>Details</summary>
Motivation: 现有研究对MLLM与监督学习模型、视觉语言模型的比较存在矛盾结论，这些矛盾源于评估协议要么夸大要么低估性能，需要系统分析评估协议和标注质量对MLLM分类性能的影响。

Method: 识别并修复常见评估协议的关键问题：模型输出超出类别列表被丢弃、弱多项选择干扰项导致结果夸大、输出映射不佳导致开放世界设置表现差；量化批次大小、图像顺序、文本编码器选择等设计选择的影响；使用ReGT（ImageNet-1k的625类多标签重新标注）进行评估。

Result: MLLM从修正标注中获益最大（提升达+10.8%），显著缩小了与监督模型的感知差距；模型对监督训练信号的依赖程度越低，对标注质量越敏感；MLLM可辅助人工标注，在困难案例中约50%的情况下标注者确认或整合了MLLM的预测。

Conclusion: MLLM在分类任务中的表现不佳主要是噪声标注和有缺陷评估协议的人工产物，而非真正的模型缺陷；MLLM对标注质量敏感，且能有效辅助大规模数据集构建。
...
TL;DR: Aletheia是一个基于RAG和LLM的浏览器扩展，用于检测假新闻并提供证据解释，包含讨论中心和实时事实核查功能，在检测性能和用户体验方面优于现有工具。


<details>
  <summary>Details</summary>
Motivation: 数字时代假新闻的泛滥严重威胁公众信任和民主制度，现有浏览器扩展存在模型行为不透明、解释支持有限、用户参与度不足等问题，需要开发更有效、透明、以用户为中心的检测工具。

Method: 提出Aletheia浏览器扩展，采用检索增强生成(RAG)和大语言模型(LLM)技术检测假新闻并提供基于证据的解释。系统包含两个交互组件：讨论中心（支持用户围绕标记内容进行对话）和"保持信息"功能（展示近期事实核查）。

Result: 通过大量实验证明Aletheia在检测性能上优于最先进的基线方法。补充的用户研究（250名参与者）证实了系统的可用性和感知有效性。
...
<details>
  <summary>Details</summary>
Motivation: 研究语法约束解码中不同但等价的语法如何影响解码效率，特别是状态空间膨胀和在线歧义成本，以优化解码性能。

Method: 将GCD建模为自回归下一个token分布与基于上下文无关语法(CFG)编译的下推系统可达性预言机的耦合，提出结构歧义成本(SAC)度量，建立理论下界，并推导失真界限。

Result: 证明了预言机不变性定理，给出了典型语言的状态膨胀精确计数，建立了Ω(t²)每token工作量的下界，推导了KL散度和总变差失真界限，并将结果集成到Transformer和MoE架构中。

Conclusion: 语法等价性不保证解码效率等价，结构歧义成本是解码性能的关键因素，存在最小SAC的语法代表，这些理论结果为自动化语法优化提供了基础。
...
TL;DR: NOTAI.AI是一个可解释的AI生成文本检测框架，通过整合曲率信号、神经特征和文体特征，使用XGBoost分类器进行检测，并提供SHAP解释和自然语言推理。


<details>
  <summary>Details</summary>
Motivation: 现有的AI生成文本检测方法往往缺乏可解释性，用户难以理解检测决策的依据。需要开发一个既能准确检测又能提供透明解释的系统。

Method: 扩展Fast-DetectGPT，整合17个可解释特征（包括条件概率曲率、ModernBERT检测分数、可读性指标和文体线索），使用XGBoost元分类器，应用SHAP进行特征归因，并通过LLM生成自然语言解释。

Result: 开发了一个交互式Web应用程序，支持实时分析、可视化特征检查和结构化证据呈现。系统源代码和演示视频已公开，支持可重复性。

Conclusion: NOTAI.AI提供了一个可解释的AI生成文本检测框架，通过多特征融合和透明解释机制，提高了检测系统的可信度和用户可理解性。
...
TL;DR: CoT提示提升LLM推理但增加隐私风险，会泄露PII到推理轨迹和输出中。研究提出评估框架，发现CoT持续增加泄露风险，尤其高风险类别，且泄露程度与模型家族和推理预算相关。测试多种轻量级门控方法，无单一方法在所有情况下最优，需混合自适应策略。


<details>
  <summary>Details</summary>
Motivation: Chain-of-Thought (CoT)提示虽然能提升大语言模型的推理能力，但会增加隐私风险，可能将提示中的个人身份信息(PII)泄露到推理轨迹和输出中，即使模型被指示不要重复PII。需要系统研究这种推理时的直接PII泄露问题。

Method: 提出模型无关的评估框架：(1)将泄露定义为跨11种PII类型的风险加权、令牌级事件；(2)追踪泄露曲线随CoT预算的变化；(3)在结构化PII数据集上比较开源和闭源模型家族，使用分层风险分类法。同时评估四种轻量级推理时门控方法：基于规则的检测器、TF-IDF+逻辑回归分类器、GLiNER-based NER模型和LLM-as-judge。

Result: CoT持续增加PII泄露，尤其对高风险类别影响更大。泄露程度强烈依赖于模型家族和推理预算，增加推理预算可能放大或减弱泄露，取决于基础模型。四种门控方法中，没有单一方法在所有模型或预算情况下表现最优，需要混合、风格自适应的门控策略。

Conclusion: CoT提示确实会增加PII泄露风险，需要开发平衡效用和风险的混合自适应门控策略。研究提供了可复现的通用评估协议，为隐私保护推理系统设计提供指导。
...
<details>
  <summary>Details</summary>
Motivation: 人们越来越多地使用LLM进行日常道德和人际指导，但这些系统无法审视缺失的背景信息，且对呈现方式的稳定性存疑

Method: 使用2,939个道德困境案例，设计三类内容扰动（表面编辑、视角转换、说服线索）和评估协议变化，测试四个主流LLM模型的稳定性

Result: 表面扰动影响小（7.5%翻转率），视角转换导致高不稳定性（24.3%），评估协议差异影响最大（协议间一致性仅67.6%），道德模糊案例最易受影响
...
<details>
  <summary>Details</summary>
Motivation: 为越南语这一在NLP中代表性不足但广泛使用的语言提供易于访问的多语言文本分析工具，降低技术门槛，支持可重复研究。

Method: 设计统一的双语NLP流水线，集成混合VnCoreNLP和BPE分词策略、微调的TabularisAI情感分类器和微调的Qwen2.5抽象摘要模型。

Result: 在三部分评估（分词、情感分析、摘要）中，该方法在越南语和英语上都取得了与广泛使用的基线模型相当或更优的性能。
...
TL;DR: 该论文使用RGB基准评估RAG系统在不同信息一致性场景下的鲁棒性，比较了基于知识图谱的GraphRAG与RGB基线，展示了GraphRAG定制化改进在噪声鲁棒性等方面的提升。


<details>
  <summary>Details</summary>
Motivation: 尽管检索增强生成(RAG)能减少LLM的事实幻觉并提供新知识，但检索信息的不一致性会影响LLM响应质量。需要评估RAG系统在不同信息一致性场景下的鲁棒性，以设计更可靠的现实世界应用系统。

Method: 使用RGB基准在四种场景下评估LLMs：噪声鲁棒性、信息整合、负面拒绝和反事实鲁棒性。比较RGB RAG基线与基于知识图谱的GraphRAG系统，并测试三种GraphRAG定制化改进方案。

Result: GraphRAG在RGB基准上相比基线有所改进，特别是在噪声鲁棒性等方面表现更好。三种GraphRAG定制化方案进一步提升了系统鲁棒性，为设计更可靠的RAG系统提供了见解。

Conclusion: 基于知识图谱的GraphRAG系统在应对不一致检索信息方面比传统RAG基线更具优势，通过适当的定制化可以进一步提高鲁棒性，这对设计适用于现实场景的可靠RAG系统具有重要意义。
...
TL;DR: 全球调查显示，不同群体对文化的定义存在差异，GenAI在文化表征时应采用参与式方法，关注宗教、传统等维度，并建立文化"红线"敏感框架。


<details>
  <summary>Details</summary>
Motivation: 目前缺乏关于全球对GenAI应如何表征文化的态度的实证证据，需要了解不同群体对文化的理解以及GenAI应如何处理文化表征问题。

Method: 通过大规模全球调查收集数据，涵盖欧洲、北美、南美、亚洲和非洲地区，从社区直接提炼文化的工作定义，分析文化概念复杂性及其与GenAI表征的关系。

Result: 调查揭示了不同群体对文化的多样化定义，发现了文化概念的复杂性，并提出了GenAI文化表征的具体建议。

Conclusion: 提出文化表征的参与式方法，优先考虑宗教、传统等特定文化维度，建立文化"红线"敏感框架，为GenAI文化发展提供指导建议。
...
<details>
  <summary>Details</summary>
Motivation: Transformer模型在扩展时伴随着参数大幅增长和嵌入维度的冗余，需要更高效的架构设计来减少参数同时保持性能。

Method: 基于三阶张量的L-积进行结构化谱分解，将令牌表示重塑为谱张量切片，在变换域中执行注意力与前馈操作，构建张量Transformer架构，将编码器分解为p个独立谱子变换器。

Result: 在IMDB和AG News数据集上，提出的模型能显著减少编码器参数（p=4时达75%），同时保持竞争力准确率。IMDB上匹配或超越标准基线，AG News上以小幅准确率下降换取4倍编码器压缩，在BERT-base宽度下性能恢复至同等水平。

Conclusion: 提出的L-Transformer通过谱分解实现了Transformer参数的有效压缩，不仅减少计算开销，还引入嵌入频率的归纳偏置，通过切片依赖的频率缩放提升泛化能力，与现有训练流程兼容。
...
TL;DR: 该论文指出当前AI系统以文本为中心的设计忽视了口语语言和原住民社区，以瓜拉尼语为例提出口语优先的多智能体架构，强调尊重原住民数据主权和语言多样性。


<details>
  <summary>Details</summary>
Motivation: 当前AI和HCI系统普遍采用文本优先的设计范式，主要服务于书面语言，忽视了以口语为主的语言（如瓜拉尼语）和原住民社区的需求。这种设计偏差导致数字技术无法真正赋能多元语言实践。

Method: 提出口语优先的多智能体架构，将瓜拉尼语自然语言理解与对话状态管理、社区主导的治理机制解耦。通过分离不同功能模块，构建尊重原住民数据主权和双语现象的技术框架。

Result: 开发了一个技术框架，将交互重点从单纯的语言识别转向对话轮转、修复和共享上下文等核心互动要素。该框架确保数字生态系统能够赋能而非忽视多样化的语言实践。

Conclusion: 要使AI真正具有文化根基，必须从将口语语言适配文本中心系统转向将口语对话作为首要设计需求。只有将口语实践视为一等公民，才能确保数字技术真正赋能多元语言社区。
...
TL;DR: CodeScout通过轻量级预探索将模糊的用户请求转化为全面、可执行的问题陈述，提升AI代码助手处理不明确任务的能力。


<details>
  <summary>Details</summary>
Motivation: 当前AI代码助手在处理缺乏足够任务上下文和需求规格的模糊问题陈述时表现不佳。研究发现，这类失败通常与代理轨迹过长、过度探索或重复尝试相同修复而缺乏适当演进和测试有关。

Method: CodeScout采用上下文查询精化方法，在任务执行前对目标代码库进行轻量级预探索。包括：目标上下文界定、多视角分析（检查潜在修复和探索机会）、将洞察合成为增强的问题陈述（包含重现步骤、预期行为和针对性探索提示）。

Result: 在SWEBench-Verified上使用最先进的代理框架和语言模型评估，CodeScout相比默认基线方法实现了20%的解决率提升，多解决了27个问题。

Conclusion: 通过上下文分析进行系统化查询精化是增强AI代码助手能力的有前景方向，无需修改底层代理框架即可补充现有能力。
...
TL;DR: NERdME数据集：包含200个手动标注的README文件，超过10,000个标注实体，10种实体类型，用于填补学术信息抽取在代码仓库实现细节方面的空白


<details>
  <summary>Details</summary>
Motivation: 现有学术信息抽取数据集主要关注科学论文，忽略了代码仓库中的实现级细节。README文件虽然描述了数据集、源代码等实现级工件，但其自由格式的Markdown缺乏语义结构，使得自动信息抽取变得困难

Method: 创建NERdME数据集：手动标注200个README文件，包含超过10,000个标注实体和10种实体类型。使用大语言模型和微调的transformer模型进行基线实验，并进行下游实体链接实验

Result: 基线结果显示论文级和实现级实体之间存在明显差异，表明扩展SIE基准以包含README文件中的实体类型具有价值。实体链接实验证明从README提取的实体可以支持工件发现和元数据集成

Conclusion: NERdME数据集填补了学术信息抽取在实现级细节方面的空白，README文件中的实体信息对于工件发现和元数据集成具有重要价值，扩展SIE基准以包含这些实体类型是必要的
...
Method: 提出PVminer基准用于结构化提取患者声音，并开发PVminerLLM——针对此任务进行监督微调的大型语言模型。在多个数据集和模型规模上进行评估。

Result: PVminerLLM显著优于基于提示的基线方法，在代码预测上达到83.82% F1，子代码预测80.74% F1，证据跨度提取87.03% F1。即使较小模型也能实现强性能，表明无需极大模型规模即可可靠提取患者声音。

Conclusion: PVminerLLM能够实现患者生成文本中嵌入的社会和经验信号的可扩展分析，为理解非临床健康结果驱动因素提供了可行工具。代码、评估脚本和训练模型将公开，标注数据集可供研究使用。
...
TL;DR: 本文提出了一个用于大规模分析辅导对话的导师行为分类法，通过混合演绎-归纳方法开发，将辅导行为分为四类，支持AI标注和辅导策略计算建模。
...
Method: 采用混合演绎-归纳方法：首先综合认知科学、学习科学、课堂话语分析和智能辅导系统研究构建初步框架，然后通过专家标注真实辅导转录稿进行迭代精炼。

Result: 开发出包含四类辅导行为的分类法：辅导支持、学习支持、社会情感与动机支持、后勤支持。学习支持行为进一步按学生参与度谱系组织，区分引发学生推理的行为与提供直接解释或答案的行为。
...
TL;DR: RouteGoT：一种预算可控、节点自适应的图结构推理路由框架，通过智能分配不同规模的模型到推理图的不同节点，在保持或提升准确率的同时大幅减少token消耗。


<details>
  <summary>Details</summary>
Motivation: 现有的图结构推理方法（如ToT、GoT、AGoT）虽然能提升某些基准测试的准确率，但引入了显著的token消耗和延迟开销，且性能提升不稳定。作者发现GoT风格推理管道中存在阶段和节点异质性：高质量规划和最终综合需要强模型，而许多中间子任务可由轻量级模型准确解决。

Method: RouteGoT是一个预算可控、节点自适应的路由框架，在推理图中执行路由：优先为规划和综合分配强模型，同时根据预测的难度动态分配轻量级模型和成本效益策略给叶子子任务。框架进一步将显式预算约束集成到全局推理调度器中，以在用户指定的token预算下控制图扩展。

Result: 在推理、检索和多跳QA基准测试中，RouteGoT匹配或提高了准确率，同时大幅减少了token使用；具体来说，相比AGoT平均提高了8.1个百分点的准确率，减少了79.1%的输出token。此外，RouteGoT通过保持更优的成本-准确率权衡，优于现有的路由基线，在不同预算目标和任务下表现出更好的鲁棒性。

Conclusion: RouteGoT通过智能路由和预算控制，有效解决了图结构推理中的效率问题，实现了可预测的性能-成本权衡，为实际部署提供了实用的解决方案。
...
TL;DR: HART是一个细粒度的幻觉归因和证据检索框架，通过结构化建模（跨度定位、机制归因、证据检索、因果追踪）来解决LLM幻觉问题，并创建了首个幻觉追踪结构化数据集。


<details>
  <summary>Details</summary>
Motivation: 现有幻觉归因方法主要基于外部知识检索或内部模型机制，侧重于语义相似性匹配或表示级判别，难以在跨度级别建立幻觉类型、错误生成机制和外部事实证据之间的结构化对应关系，限制了幻觉片段的可解释性和支持/反对证据的可追溯性。

Method: 提出HART框架，将幻觉追踪形式化为包含四个阶段的结构化建模任务：跨度定位、机制归因、证据检索和因果追踪。基于此构建了首个幻觉追踪结构化数据集，联合标注幻觉类型、错误机制和反事实证据集。

Result: 在提出的数据集上的实验结果表明，HART显著优于BM25和DPR等强检索基线，验证了所提追踪范式在幻觉分析和证据对齐方面的有效性和泛化能力。

Conclusion: HART通过结构化建模方法解决了现有幻觉归因方法的局限性，提供了更细粒度的幻觉分析和证据对齐能力，为LLM幻觉问题提供了新的解决方案。
...
TL;DR: ReflexiCoder是一个强化学习框架，通过将结构化推理轨迹（生成、反思、修正）内化到模型权重中，使LLM具备内在的自主反思和修正能力，显著提升代码生成性能。


<details>
  <summary>Details</summary>
Motivation: 传统LLM代码生成方法（单次前向生成）在复杂算法任务上存在性能瓶颈，现有迭代优化方法依赖外部反馈或计算昂贵的提示-响应循环，需要更高效的内在反思能力。

Method: 提出ReflexiCoder强化学习框架，使用RL-zero训练范式配合细粒度奖励函数，优化整个反思-修正轨迹，将结构化推理能力内化到模型权重中，实现无需外部反馈的自主调试。

Result: 在7个基准测试中，ReflexiCoder-8B在1.5B-14B开源模型中达到SOTA水平：HumanEval 94.51%，MBPP 81.80%，BigCodeBench 35.00%，LiveCodeBench 52.21%，CodeForces 37.34%，推理计算开销减少约40%。

Conclusion: ReflexiCoder通过将结构化推理轨迹内化到模型权重中，实现了从外部依赖优化到内在自主反思的范式转变，显著提升了代码生成性能并降低了推理成本。
...
TL;DR: ROSE是一种改进的LLM剪枝方法，通过重新排序剪枝顺序来提升SparseGPT框架的性能，优先剪枝潜在误差较大的权重，在多个主流LLM上表现优于原始SparseGPT和其他剪枝方法。


<details>
  <summary>Details</summary>
Motivation: SparseGPT作为基于Hessian的二阶梯度LLM剪枝方法，采用固定的从左到右剪枝顺序，当权重呈现列模式时会导致次优性能。本文研究剪枝顺序对SparseGPT框架的影响，旨在解决这一限制。
...
Conclusion: 剪枝顺序对SparseGPT框架性能有重要影响，ROSE通过自适应重排序机制显著提升了剪枝效果，为LLM高效部署提供了更优的剪枝方案。
...
TL;DR: CoCA提出了一种置信优先的LLM不确定性估计方法，通过GRPO强化学习框架联合优化置信度校准和答案准确性，使用分段信用分配避免奖励黑客问题。


<details>
  <summary>Details</summary>
Motivation: 现有LLM不确定性估计方法多为答案优先，即在生成答案后才产生置信度，这限制了实际可用性。需要一种置信优先的范式，让模型在回答前输出其正确概率。

Method: 提出CoCA框架，采用GRPO强化学习方法，通过分段信用分配分别对置信度和答案部分分配奖励和组相对优势，实现联合优化置信度校准和答案准确性。

Result: 在数学、代码和事实问答基准测试中，CoCA改善了校准和不确定性区分能力，同时保持了答案质量，为下游应用提供了更好的不确定性估计。

Conclusion: 置信优先范式结合联合优化方法能够有效提升LLM的不确定性估计可靠性，为实际部署提供了更实用的解决方案。
...
TL;DR: 当前主流BPE分词方法对黏着语不友好，会破坏语素边界并增加token数量


<details>
  <summary>Details</summary>
Motivation: BPE及其变体作为主流分词方法本质上是脚本无关的，主要针对英语等屈折语优化，但对黏着语（如达罗毗荼语系、突厥语系、乌拉尔语系、韩语、日语等）处理不佳，这些语言中单个单词可能编码根词、时态、体、人称、数、性别一致、格和后置词等多种信息

Method: 论文未在摘要中明确说明具体方法，但指出了当前BPE分词方法的问题：统计分词器将黏着语的单词分割成字节对块，破坏了语素边界并增加了token数量

Result: 摘要未提供具体实验结果，但指出了当前分词方法对黏着语处理存在的问题

Conclusion: 需要针对黏着语特点开发更合适的分词方法，以更好地处理这些语言的形态学特性
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型在生成长篇故事时经常出现前后不一致的问题，如事实矛盾、角色特征冲突等，而现有的故事生成基准主要关注情节质量和流畅度，缺乏对一致性的系统评估。

Method: 提出了ConStory-Bench基准测试，包含2000个提示和四种任务场景，定义了5大类19种子类型的错误分类。开发了ConStory-Checker自动检测管道，能够检测矛盾并为每个判断提供明确的文本证据。
...
TL;DR: 使用LLM集成系统对联合国安理会决议进行语义标注，通过CPR和TWF指标避免幻觉，GPT-4.1表现最佳但GPT-4.1-mini成本效益更高


<details>
  <summary>Details</summary>
Motivation: 利用LLM性能变异性构建集成系统，对联合国安理会决议进行准确高效的语义标注和数据清洗，避免模型幻觉和不必要的文本修改

Method: 提出CPR（内容保持率）和TWF（标签格式良好度）两个评估指标，通过多个GPT模型多次运行构建集成系统，选择最佳输出

Result: GPT-4.1在两项任务中表现最佳（清洗：CPR 84.9%；语义标注：CPR 99.99%，TWF 99.92%）。GPT-4.1-mini以20%成本达到可比性能

Conclusion: 通过集成设计和评估指标，创建了可靠的LLM系统，能够在具有挑战性的文本上执行语义标注任务
...
TL;DR: InfoGatherer：一个基于证据网络和Dempster-Shafer理论的信息收集框架，通过检索文档和针对性提问来减少LLM在关键领域决策中的不确定性


<details>
  <summary>Details</summary>
Motivation: LLM在医疗分诊、法律协助等高风险领域部署时，用户初始查询往往不完整，单次检索不足以支持可靠决策，导致错误且过度自信的回答。现有方法依赖LLM的隐式置信信号，难以确定未知信息、关键信息以及何时停止提问。

Method: 提出InfoGatherer框架，从两个互补来源收集缺失信息：检索领域文档和向用户提出针对性后续问题。使用Dempster-Shafer信念分配在结构化证据网络上建模不确定性，能够原则性地融合不完整且可能矛盾的证据，而不提前得出确定性答案。

Result: 在法律和医疗任务中，InfoGatherer优于强基线方法，同时需要更少的交互轮次。通过将不确定性建立在形式化证据理论而非启发式LLM信号上，实现了更可信、可解释的决策支持。

Conclusion: InfoGatherer通过结构化证据网络和Dempster-Shafer理论，为关键领域提供了更可靠、可解释的决策支持框架，减少了LLM决策中的不确定性，提高了高风险应用的可靠性。
...
TL;DR: 论文提出了一种名为"下一个动作预测"(NAP)的任务，通过分析用户与计算机的多模态交互历史来预测用户的下一个动作，并开发了LongNAP模型和大规模标注数据集。


<details>
  <summary>Details</summary>
Motivation: 真正主动的AI系统需要预测用户下一步会做什么，这需要比稀疏提示更丰富的信息——需要对用户所见所做的完整上下文进行推理。

Method: 1) 使用视觉语言模型标注纵向自然计算机使用数据，构建包含36万个动作的数据集；2) 提出LongNAP模型，结合参数化和上下文学习，通过策略梯度方法训练生成用户特定推理轨迹、检索相关历史轨迹，并应用这些轨迹预测未来动作。

Result: LongNAP在留出数据上显著优于监督微调和提示基线（分别提升79%和39%），能泛化到未见用户。17.1%的预测轨迹与用户实际行为良好对齐（LLM-judge评分≥0.5），高置信度预测时达到26%。
...
TL;DR: 通过建模作者的语言上下文（HuLM任务）来解决生态学谬误，在8B Llama模型上验证了作者语境建模的效用，通过HuFT微调和HuLM预训练提升了多个下游任务性能。


<details>
  <summary>Details</summary>
Motivation: 语言模型训练和推理忽略了一个基本语言事实：同一作者写的多个文本序列之间存在依赖关系。先前研究表明，解决这种生态学谬误可以显著提升较小GPT模型的性能。本研究探索在更大规模（8B）Llama模型中，通过建模作者语言上下文（HuLM任务）是否也能带来类似益处。

Method: 提出HuLM（Human-aware Language Modeling）任务来建模作者的语言上下文，探索两种变体：1）HuFT（Human-aware Fine-Tuning）：在微调阶段使用作者上下文；2）HuLM预训练：在预训练阶段使用作者上下文。使用QLoRA技术进行高效微调，处理作者按时间顺序排列的其他文本。

Result: 1）仅通过HuFT微调（使用QLoRA）就能提升8B模型性能；2）基于QLoRA的HuLM预训练产生的人类感知模型，在线性任务分类器训练下，在八个下游任务上均表现出改进性能。

Conclusion: 建模语言在其原始生成者（作者）的语境中具有重要价值和实用性，解决生态学谬误对大规模语言模型同样有效，作者语境建模是提升语言模型性能的有效途径。
...
TL;DR: 提出结构化风格重写框架，通过显式解构风格为三个维度（词汇特征、句法模式、语用风格）和隐式风格条件策略，使小语言模型能生成高保真风格化内容，在动漫角色领域超越更大基线模型。


<details>
  <summary>Details</summary>
Motivation: 小语言模型在角色扮演中难以生成高度风格化的人物，因为数据稀缺和风格解耦复杂。标准监督微调只能捕捉表层语义，无法复现角色的句法和语用细微差别，导致"出戏"生成。

Method: 提出结构化风格重写框架：1）将风格显式解构为三个可解释维度：词汇特征（通过PMI）、句法模式（基于PCFG规则）和语用风格；2）引入隐式风格条件策略，通过思维链蒸馏，利用训练期间的显式推理轨迹作为强归纳偏置，使模型潜在表示与结构化风格特征对齐。

Result: 在动漫角色这一高风格化领域的广泛实验中，该方法使Qwen-1.7B模型在风格一致性和语义保真度上显著超越更大的基线模型（如4B标准监督微调）。

Conclusion: 该方法为在消费级硬件上实现高效推理和部署提供了数据高效的范式，能够民主化小语言模型的高保真风格化生成能力。
...
TL;DR: 该研究开发了可解释的模型来预测心理健康状态，通过整合个体心理特质与情境特征来分析社交媒体数据中的动态心理过程。


<details>
  <summary>Details</summary>
Motivation: 心理健康不是固定特质而是动态过程，受个体倾向与情境交互影响。当前需要将计算建模与心理学理论结合，以更可解释的方式评估动态心理状态。

Method: 基于互动主义和建构主义心理学理论，整合个体心理特质（如韧性、认知扭曲、内隐动机）与从Situational 8 DIAMONDS框架推导的语言推断情境特征，并与心理测量学启发的语言模型嵌入进行比较。

Result: 理论驱动的特征在保持竞争力的同时提供更好的可解释性。定性分析显示预测幸福感的特征具有心理一致性。
...
<details>
  <summary>Details</summary>
Motivation: 生成语言模型在单轮Text-to-SQL中表现良好，但在多轮场景中性能不足，主要因为难以处理上下文信息的复杂性和动态模式链接
...
Conclusion: Track-SQL通过双提取模块有效解决多轮Text-to-SQL中的上下文跟踪和模式链接问题，显著提升生成语言模型性能
...
TL;DR: MASFactory是一个基于图的框架，用于编排基于LLM的多智能体系统，通过Vibe Graphing将自然语言意图编译为可编辑的工作流规范，并提供可视化工具和可复用组件。
...
Method: 提出MASFactory框架，引入Vibe Graphing方法将自然语言意图编译为可编辑的工作流规范，然后转换为可执行图。提供可复用组件、可插拔上下文集成以及可视化工具。

Result: 在七个公共基准测试上评估MASFactory，验证了代表性MAS方法的复现一致性以及Vibe Graphing的有效性。

Conclusion: MASFactory通过图中心框架解决了LLM-based多智能体系统工作流编排的挑战，提供了更高效、可复用且易于集成的工作流开发方法。
...
TL;DR: ViewFusion是一个两阶段框架，通过显式分离跨视图空间预对齐和问题回答，提升多视图空间推理能力，在MMSI-Bench上比Qwen3-VL-4B-Instruct准确率提高5.3%。
...
Conclusion: 通过显式分离空间预对齐和问题回答，ViewFusion能够更好地利用跨视图关系，提升多视图空间推理能力，特别是在需要视角变换和跨视图对齐的任务上表现更优。
...
TL;DR: 研究评估开源大语言模型在奥地利高中德语作文评分中的应用，发现模型在评分细则维度上最多只有40.6%与人类评分者一致，最终成绩匹配率仅32.8%，表明当前模型在实际评分环境中准确性不足。


<details>
  <summary>Details</summary>
Motivation: 自动作文评分系统旨在减轻教师评分负担并减少主观偏见。随着大语言模型的发展，研究者希望探索开源模型在德语作文评分中的应用潜力，特别是基于评分细则的评估。

Method: 使用101份匿名学生考试文本（三种文本类型）构建数据集，评估四种开源大语言模型（DeepSeek-R1 32b、Qwen3 30b、Mixtral 8x7b和LLama3.3 70b），采用不同上下文和提示策略进行评分细则维度的评估。

Result: 模型在评分细则维度上与人类评分者的最大一致率为40.6%，最终成绩匹配率仅为32.8%。结果显示虽然较小模型能够使用标准化评分细则进行德语作文评分，但准确性不足以在实际评分环境中应用。

Conclusion: 尽管开源大语言模型在德语作文评分中展现出一定潜力，但当前模型的评分准确性仍然有限，无法满足实际评分环境的要求，需要进一步改进。
...
TL;DR: 该研究通过持续预训练让LLM接触领域特定文本，模拟经验积累，发现模型能力呈双峰分布：表达型通才和抑制型专家，并揭示了"抑制优势"现象——降低社交特质能提升复杂推理表现。


<details>
  <summary>Details</summary>
Motivation: 人类问题解决受益于多样化的风格和人格特质，但当前LLM开发过度追求统一性能基准，偏向特定行为倾向（如自信）。需要研究不同经验如何塑造机器个性并影响问题解决能力。

Method: 采用持续预训练方法，让模型以无监督方式接触领域特定文本，模拟经验积累过程。使用基于大五人格框架的机器人格量表（MPI）量化模型变体的人格特质，分析其与语言风格和推理行为的关系。

Result: 模型能力呈双峰分布：在"表达型通才"和"抑制型专家"处达到峰值。发现"抑制优势"现象——降低社交特质（如外向性）能提升复杂推理性能。建立了训练数据语言学特征（如祈使句频率、词汇多样性）与人格特质之间的因果关系。

Conclusion: 该研究为"人格工程"提供了路线图，表明可以通过调整训练数据的语言学特征来设计特定人格特质的AI系统，从而优化不同任务的问题解决能力。
...
TL;DR: 提出一个结合LLM和神经符号推理的管道，用于处理文本中的省略三段论（enthymeme），将自然语言转换为逻辑公式并确定逻辑蕴含关系。


<details>
  <summary>Details</summary>
Motivation: 现实世界中的文本和对话论证通常是省略三段论（部分前提或主张是隐含的）。现有的NLP方法只能识别省略三段论但不能解码其底层逻辑，而基于逻辑的方法需要充足的知识库公式进行溯因推理。缺乏系统性的方法将省略三段论的文本组件转换为逻辑论证并生成解码所需的逻辑公式。

Method: 提出一个三阶段管道：1）使用大语言模型基于显式前提和主张生成中间隐含前提；2）使用另一个LLM将自然语言翻译为逻辑公式；3）使用基于SAT求解器的神经符号推理器确定逻辑蕴含关系。

Result: 在两个省略三段论数据集上评估管道，在选择正确隐含前提方面表现出有希望的性能，通过精确率、召回率、F1分数和准确率进行衡量。

Conclusion: 提出的集成LLM和神经符号推理的管道能够有效处理文本中的省略三段论，实现从自然语言到逻辑公式的转换和逻辑蕴含关系的确定。
...
TL;DR: 提出一种零样本机制，通过动态裁剪上下文窗口来减少扩散语言模型的计算浪费，在保持性能的同时显著提升效率


<details>
  <summary>Details</summary>
Motivation: 扩散语言模型在固定长度的上下文窗口上进行预定的去噪步骤，这与实际响应长度无关，导致在常见的短响应任务中产生大量计算浪费。作者推测潜在提示表示包含足够信息来估计所需输出长度。

Method: 提出零样本机制，在生成开始前动态裁剪上下文窗口，基于潜在提示表示估计所需输出长度，从而减少扩散步骤和计算成本。

Result: 在GSM8K、HumanEval、IfEval和LongFormQA四个基准测试中，该方法在所有任务上都显著减少了FLOPs，没有统计显著的性能下降，并在2/4任务中显著提升了性能。

Conclusion: 通过利用潜在提示信息动态调整上下文窗口，可以在保持扩散语言模型性能的同时实现大规模计算效率提升，为实际应用提供了实用的优化方案。
...
Method: 提出图基框架：1) 将叙事表示为实体交互图；2) 在节点级引入因果估计，将句子图提炼为最小因果子图；3) 基于此表示构建分类流程。

Result: 该分类流程在对立思维分类任务上超越了现有方法。
...
<details>
  <summary>Details</summary>
Motivation: 现有评估指标缺乏临床上下文，对正常或临床不显著发现给予过多权重，无法准确评估报告生成的临床质量。

Method: CRIMSON框架整合患者年龄、指征和指南决策规则，将错误分类为假阳性、漏诊和八种属性级错误，每个发现分配临床显著性级别（紧急、可行动非紧急、不可行动、预期/良性），实现严重性感知加权。
...
Conclusion: CRIMSON提供了一个临床基础、可复现的评估框架，能更准确地评估胸部X光报告生成的临床质量，并发布了相关基准和模型。
...
Result: 在多个主观对话基准（EMPA、EmoBench、EQ-Bench）和7B到32B模型规模上，MAPO在训练稳定性和最终性能上均优于仅基于结果的GRPO和单级归一化基线。在EMPA上，7B基础模型的对话得分提升高达+43.2分。尽管仅在EMPA风格环境中训练，方法在未见的情感智能基准上泛化良好，EmoBench提升高达+4分，EQ-Bench提升+3.5分。

Conclusion: 密集过程监督与混合级归一化相结合，能够实现主观、开放式多轮对话的有效且可扩展的强化学习。该方法解决了传统RL在主观对话任务中的信用分配和可扩展性问题。
...
TL;DR: AI-CROWD协议通过集成多个大语言模型的输出生成共识标签，近似替代大规模内容分析中难以获得的人工标注真值


<details>
  <summary>Details</summary>
Motivation: 大规模内容分析面临缺乏可观测真值标签的挑战，因为人工标注大规模数据集成本高、耗时长且难以保持一致性

Method: 引入AI-CROWD协议，利用多个大语言模型输出的集体结果，通过多数投票聚合输出，并分析一致/分歧模式来生成共识标签

Result: 协议能够识别高置信度分类，同时标记潜在的模糊性或模型特定偏差，提供近似真值的标签

Conclusion: AI-CROWD协议为大规模内容分析提供了一种实用的解决方案，通过模型集成共识来克服人工标注的限制
...
<details>
  <summary>Details</summary>
Motivation: 现有RAG生成器基准测试覆盖范围有限，无法在统一条件下同时评估多种能力（如长上下文集成、多步推理、表格解释、证据缺失时的弃权等），与实际应用需求存在差距。

Method: 构建包含114个人工构建的日语问题（及机器翻译+人工校对的英文版本）的数据集，使用虚构实体和场景确保答案基于外部文档。定义五个评估类别：集成、推理、逻辑、表格、弃权，每类进一步细分为实际评估方面。采用LLM-as-a-Judge进行评分。

Result: 在API模型和开源模型测试中，没有模型总体准确率超过90%。基准测试能够测量每个类别中的优势和弱点，为实际RAG部署中的模型选择和构建RAG专用模型提供有价值指标。

Conclusion: LIT-RAGBench填补了现有评估与实际应用之间的差距，通过系统覆盖多个能力类别和组合模式，成为RAG生成器评估的重要基准，已公开发布数据集和评估代码。
...
TL;DR: FlashPrefill：通过瞬时模式发现和阈值处理实现超快速预填充的框架，在256K序列上达到27.78倍加速，在4K上下文长度上仍保持1.71倍加速。


<details>
  <summary>Details</summary>
Motivation: 长上下文建模是LLM的关键能力，但注意力机制的二次复杂度成为关键瓶颈，特别是在计算密集的预填充阶段。现有稀疏注意力机制要么搜索延迟显著，要么稀疏性不足。

Method: 提出FlashPrefill框架，采用快速块搜索技术同时定位动态垂直、斜线和块稀疏注意力模式，并引入动态阈值机制，避免排序或累积注意力分数的过高开销，有效消除长尾分布以增强稀疏性。

Result: 在256K序列上实现前所未有的27.78倍加速，在4K上下文长度上仍保持1.71倍加速，相比现有方法在短上下文上效率下降的问题，FlashPrefill在不同序列规模上都表现出鲁棒性和实用性。

Conclusion: FlashPrefill通过瞬时模式发现和动态阈值处理，显著提升了长上下文建模中预填充阶段的效率，解决了现有稀疏注意力机制的局限性，为实际应用提供了高效的解决方案。
...
TL;DR: SPOT框架通过将显式思维链压缩为紧凑的潜在暂停标记，在减少推理成本的同时提升推理性能，并保持潜在状态的可解释性。


<details>
  <summary>Details</summary>
Motivation: 显式思维链推理虽然能提升大语言模型的推理性能，但会产生大量推理标记导致高昂的推理成本。现有方法主要通过剪枝或简洁提示来减少标记，但只是截断模型输出而非内化模型思考。潜在推理方法虽然提供了替代方案，但面临两个关键挑战：1) 刚性点对点对齐无法捕捉整个推理段的密集、变长语义；2) 缺乏可解释性，潜在状态难以解码或审计。

Method: SPOT框架包含两个核心创新：1) 跨度级语义对齐：使用Sinkhorn最优传输目标，将每个暂停标记软匹配到整个推理段的语义，克服步末对齐的刚性；2) 冻结头部解码约束：保持潜在状态在冻结的预训练语言模型头部下可直接解码为标记分布，实现对潜在思考的可读关键词解释。

Result: 在推理基准测试中，SPOT平均提升准确率2.3个百分点，同时减少生成标记37.5%，并能提供潜在推理过程的忠实语义解释。

Conclusion: SPOT通过灵活的潜在推理框架，在减少推理成本的同时提升推理性能，并解决了潜在推理方法的可解释性问题，为高效且透明的推理系统提供了新方向。
...
TL;DR: LLMs在多语言、多文化环境中部署时，其英语中心训练数据与不同社会文化价值观存在错位风险。研究发现主流模型在宗教等敏感领域无法准确代表少数群体观点，常放大负面刻板印象。


<details>
  <summary>Details</summary>
Motivation: LLMs日益部署于多语言多文化环境，但其主要基于英语训练数据，可能无法与不同社会的多元文化价值观对齐。需要评估LLMs在不同文化背景下的对齐情况，特别是在宗教等敏感领域。

Method: 对GPT-4o-Mini、Gemini-2.5-Flash、Llama 3.2、Mistral和Gemma 3等当代LLMs进行多语言文化对齐审计，聚焦印度、东亚和东南亚地区。使用log-probs/logits分析模型内部表征，将模型观点分布与真实公众态度对比。采用人口统计提示和母语提示等轻量干预措施，并在CrowS-Pairs、IndiBias、ThaiCLI、KoBBQ等偏见基准上进行下游评估。

Result: 主流模型在广泛社会议题上能与公众意见对齐，但在宗教观点上持续失败，特别是对少数群体的代表性不足，常放大负面刻板印象。轻量干预措施能部分缓解但无法消除文化差距。下游偏见基准评估显示在敏感情境中存在持续伤害和代表性不足问题。

Conclusion: 为确保LLMs在全球范围内的公平部署，迫切需要系统性的、基于地区的文化对齐审计。当前模型在宗教等敏感领域的文化对齐存在显著不足，需要更深入的文化理解和代表性改进。
...
TL;DR: 研究探索ChatGPT模仿原创艺术作品生成图像的能力，发现AI生成的模仿作品在色彩纹理相似度与构图概念感知相似度之间存在显著差距，艺术家认为AI作品缺乏维度、语境和意图感。


<details>
  <summary>Details</summary>
Motivation: 探索人工智能在视觉创意领域的潜力，特别是ChatGPT能否有意识地模仿原创艺术作品（绘画、雕塑、装置等）生成新的图像，并评估这种模仿的质量和局限性。

Method: 邀请12位来自罗马尼亚、保加利亚、法国、奥地利和英国的艺术家，每人提供3件原创作品；让ChatGPT生成这些作品的模仿版本；结合人类评估（艺术家评分和评论）与计算方法（检测视觉和风格相似性或差异）进行分析。

Result: 研究发现色彩和纹理的相似度与构图、概念和感知的相似度之间存在显著差距；艺术家认为ChatGPT生成的模仿作品缺乏维度、语境和意图感，更像是近似引用而非能引发情感的有价值艺术品；建议使用"风格转移仪表板"的多指标评估而非单一风格指标。

Conclusion: AI在模仿艺术作品方面仍有局限性，特别是在传达作品的深度、语境和意图方面；需要采用多维度指标来评估AI生成作品与原创的相似度；当前AI生成的模仿作品更像是近似引用而非真正的创造性表达。
...
TL;DR: 该研究将数学问题理解建模为数学实体关系抽取任务，使用Transformer模型自动提取数学文本中的实体关系，BERT模型达到99.39%准确率，并结合SHAP方法增强模型可解释性。


<details>
  <summary>Details</summary>
Motivation: 数学文本理解具有挑战性，因为包含专业实体和复杂关系。需要自动提取数学实体关系以支持自动化问题求解、知识图谱构建和智能教育系统。

Method: 将数学问题解释建模为数学实体关系抽取任务，将操作数视为实体，运算符视为关系。应用基于Transformer的模型（特别是BERT）进行关系提取，并集成SHAP方法进行可解释性分析。
...
TL;DR: LLMs在条件推理中表现出与人类相似的领域特异性：在道义规则上表现更好，并显示出匹配偏误的错误模式


<details>
  <summary>Details</summary>
Motivation: 尽管先前研究比较了LLM和人类推理，但LLM推理的领域特异性仍未充分探索。研究者希望了解LLMs在道义条件句和描述性条件句中的推理表现，以及其错误模式是否与人类认知偏误相似

Method: 引入新的Wason选择任务数据集，明确编码道义模态以区分道义条件句和描述性条件句，系统检验LLMs在道义规则下的条件推理能力，并分析错误模式是更符合确认偏误还是匹配偏误

Result: 与人类相似，LLMs在道义规则上推理表现更好，并显示出类似匹配偏误的错误模式（倾向于忽略否定并选择与规则词汇匹配的项目）

Conclusion: LLMs的表现随规则类型系统变化，其错误模式可以平行于该范式中已知的人类认知偏误，表明LLMs的推理能力具有领域特异性
...
TL;DR: 该研究对LLM在英语二语写作自动评分中的四种主要方法进行了统一比较，发现结合k-SFT和RAG的方法在F1分数上达到93%，揭示了不同方法在准确性、成本和鲁棒性之间的权衡。


<details>
  <summary>Details</summary>
Motivation: 现有研究通常孤立地考察LLM在自动作文评分中的各种技术，缺乏对它们在英语作为第二语言写作中相对优点的系统理解。需要建立一个统一的基准来比较不同LLM评分范式的效果。

Method: 在IELTS写作任务2上构建统一基准，评估四种LLM评分方法：1)基于编码器的分类微调；2)零样本和少样本提示；3)指令微调和检索增强生成(RAG)；4)监督微调结合直接偏好优化(DPO)和RAG。

Result: 研究揭示了不同方法在准确性、成本和鲁棒性之间的权衡。最佳配置（结合k-SFT和RAG）获得了最强的整体结果，F1分数达到93%。

Conclusion: 这是首次对现代LLM在英语二语自动评分策略进行的统一实证比较，展示了在自动评分写作任务中的潜力。研究结果为选择适合的LLM评分方法提供了实用指导。
...
TL;DR: 该研究探讨大语言模型在溯因推理中的表现，通过将三段论数据集转换为适合溯因推理的形式，评估LLMs是否存在与人类相似的推理偏见。


<details>
  <summary>Details</summary>
Motivation: 现有研究表明LLMs与人类存在相似的推理偏见，但批评LLMs可能不公平，因为人类推理不仅包含形式演绎，还涉及基于有限信息的溯因推理。需要探究LLMs在溯因推理中的准确性，了解其是否表现出偏见，并找出改进方向。

Method: 将三段论数据集转换为适合溯因推理的形式，用于评估最先进的大语言模型在溯因推理中的表现，分析其是否存在系统性偏见。

Result: 论文发现LLMs在溯因推理中表现出特定模式，揭示了模型在上下文化推理方面的局限性，为理解机器与人类认知差异提供了实证依据。

Conclusion: LLMs在溯因推理中存在可识别的偏见模式，强调超越形式演绎的上下文化推理的重要性。这一研究为改进LLMs在复杂推理任务中的应用提供了重要见解，有助于缩小机器与人类认知之间的差距。
...
<details>
  <summary>Details</summary>
Motivation: 当前XAI方法采用"一刀切"范式，忽视了用户在专业知识、目标和认知需求上的差异。虽然大语言模型能将技术解释转化为自然语言，但存在忠实性和幻觉问题。

Method: PONTE框架包含三个核心组件：(1)低维偏好模型捕捉风格要求；(2)基于结构化XAI工件的偏好条件生成器；(3)验证模块确保数值忠实性、信息完整性和风格对齐，可选支持检索增强论证。用户反馈迭代更新偏好状态，实现快速个性化。

Result: 在医疗和金融领域的自动和人工评估表明，验证-精炼循环显著提高了完整性和风格对齐性。人工研究证实了预期偏好向量与感知风格之间的强一致性、对生成随机性的鲁棒性，以及持续积极的质量评估。

Conclusion: PONTE通过将个性化建模为闭环验证和适应过程而非提示工程，实现了自适应和可靠的XAI叙述，有效解决了XAI中的个性化需求和LLM幻觉挑战。
...
TL;DR: BRTR是一个多模态代理框架，通过迭代工具调用循环取代单次检索，显著提升复杂电子表格理解能力，在三个基准测试中超越先前方法25-32个百分点。


<details>
  <summary>Details</summary>
Motivation: 现有多模态RAG方法在分析企业级电子表格时存在单次检索丢失关键上下文、压缩降低数据分辨率、全上下文注入超出LLM窗口等问题，无法可靠支持复杂工作簿的多步推理。

Method: BRTR采用多模态代理框架，用迭代工具调用循环替代单次检索，支持从复杂分析到结构化编辑的端到端Excel工作流，保持完整可审计性。

Result: 在三个前沿电子表格理解基准测试中达到SOTA：FRTR-Bench提升25个百分点，SpreadsheetLLM提升7个百分点，FINCH提升32个百分点。NVIDIA NeMo Retriever 1B被确定为混合表格和视觉数据的最佳嵌入模型，GPT-5.2在效率-准确性权衡上表现最佳。

Conclusion: BRTR通过迭代工具调用循环有效解决了复杂企业电子表格理解中的关键挑战，显著提升了多步推理能力，同时保持完全可审计性，为电子表格分析提供了可靠的代理框架。
...
TL;DR: 提出一个上下文感知的多语言ASR框架，通过轻量级投影模块结合冻结语音编码器和仅解码器语言模型，使用对比学习对齐语音与上下文表示，在11种语言和5种英语方言上实现超过5%的性能提升。


<details>
  <summary>Details</summary>
Motivation: 当前ASR系统大多局限于单语设置和短孤立话语，上下文感知ASR存在多语言支持有限和语音-上下文表示对齐缺乏原则性方法的问题。

Method: 通过轻量级投影模块结合冻结语音编码器和仅解码器语言模型，支持结构化上下文提示（对话历史和偏置词），并采用对比学习目标在共享嵌入空间中对齐语音和上下文表示。

Result: 在超过1500小时的真实世界对话语音（11种语言和5种英语方言）上评估，上下文输入持续改善识别质量，对比学习对齐在不同上下文类型上提供额外增益，总体性能提升超过5%。

Conclusion: 上下文建模和跨模态对齐在多语言ASR中都很重要，提出的框架在保持预训练模型模块化的同时支持多语言和口音，通过对比学习改善语音-上下文交互。
...
<details>
  <summary>Details</summary>
Motivation: 研究政治话语中的歧义和回避技术分类，探索不同建模方法在CLARITY任务中的效果，特别是比较直接预测清晰度标签与通过任务层次结构推导清晰度的方法。

Method: 1) 直接预测清晰度标签；2) 先预测回避标签，然后通过任务分类层次结构推导清晰度；3) 探索多种辅助训练变体；4) 在回避优先的框架下评估解码器模型的零样本性能。

Result: 两种建模方法性能相当。在编码器模型中，RoBERTa-large在公开测试集上表现最佳；在零样本设置下，GPT-5.2在隐藏评估集上泛化能力更好。

Conclusion: 两种建模方法都能有效处理政治话语中的歧义和回避技术分类任务，不同模型在不同评估场景下各有优势，为相关研究提供了有价值的建模策略比较。
...
### [204] [Towards Neural Graph Data Management](https://arxiv.org/abs/2603.05529)
*Yufei Li,Yisen Gao,Jiaxin Bai,Jiaxuan Xiong,Haoyu Huang,Zhongwei Xie,Hong Ting Tsang,Yangqiu Song*

Main category: cs.DB

TL;DR: NGDBench是一个统一的神经图数据库基准测试，支持完整的Cypher查询语言，用于评估LLM和RAG方法在图数据管理中的能力，涵盖金融、医疗等多个领域。


<details>
  <summary>Details</summary>
Motivation: 虽然AI系统在处理非结构化文本方面取得了显著进展，但存储在数据库中的结构化图数据增长迅速，却难以被神经模型有效利用。现有基准测试仅限于基本逻辑操作，缺乏对复杂图查询和实际场景的全面评估。

Method: 引入NGDBench基准测试，支持完整的Cypher查询语言，包含复杂模式匹配、变长路径和数值聚合。基准测试包含五个不同领域的数据，并引入了现实噪声注入和动态数据管理操作。

Result: 对最先进的LLM和RAG方法的评估显示，它们在结构化推理、噪声鲁棒性和分析精度方面存在显著局限性。NGDBench为推进神经图数据管理提供了关键的测试平台。

Conclusion: NGDBench填补了图数据库神经模型评估的空白，为研究社区提供了评估和改进神经图数据管理能力的重要工具。代码和数据已开源。
...
### [205] [Human-Data Interaction, Exploration, and Visualization in the AI Era: Challenges and Opportunities](https://arxiv.org/abs/2603.05542)
...
TL;DR: 本文探讨AI时代下人类数据交互系统面临的挑战，包括大规模异构数据、基础模型不确定性、感知延迟、可扩展性限制等问题，并提出需要重新定义人机角色、整合认知设计原则以构建以人为本的AI数据分析系统。


<details>
  <summary>Details</summary>
Motivation: AI技术的快速发展正在改变以人为中心的系统，对人类-AI交互、人类-数据交互和可视化分析产生深远影响。AI时代的数据分析涉及大规模、异构、多模态的非结构化数据，以及LLMs和VLMs等基础模型，这些因素给分析过程带来了额外的不确定性，暴露了人类数据交互系统的持续挑战。

Method: 本文通过调查AI最新进展引入的挑战，研究这些发展如何重塑用户与数据的交互方式，同时为构建AI时代交互式数据分析的人类中心AI系统，提出局限性和开放研究方向。

Result: 识别出人类数据交互系统面临的主要挑战包括：感知不对齐的延迟、可扩展性约束、现有交互和探索范式的局限性，以及AI生成洞察的可靠性和可解释性日益增长的不确定性。

Conclusion: 应对这些挑战需要超越传统的效率和可扩展性指标，重新定义分析工作流程中人类和机器的角色，并将认知、感知和设计原则整合到人类数据交互堆栈的每个层面，为构建以人为本的AI系统指明研究方向。
...
### [206] [Space-efficient B-tree Implementation for Memory-Constrained Flash Embedded Devices](https://arxiv.org/abs/2603.05632)
*Nadir Ould-Khessal,Scott Fazackerley,Ramon Lawrence*

Main category: cs.DB

TL;DR: 本文针对嵌入式设备开发并评估了多种B树变体，证明即使是最小的设备也能实现高效的B树索引，且存储特定优化能带来显著的性能优势。


<details>
  <summary>Details</summary>
Motivation: 物联网设备在农业、环境和工业监测中收集数据，需要优化设备端数据处理。边缘计算允许在数据收集点附近处理数据，减少网络传输。现有的B树优化主要针对服务器和固态硬盘的闪存存储，需要嵌入式设备不具备的硬件和内存资源。

Method: 开发并实验评估了多种适用于内存受限嵌入式设备的B树变体，针对嵌入式设备的存储特性进行专门优化。

Result: 实验结果表明，即使是最小的设备也能执行高效的B树索引操作，并且采用存储特定优化能带来显著的性能优势。

Conclusion: 通过为嵌入式设备开发专门的B树优化方案，可以在资源受限的环境中实现高效的数据索引和处理，支持物联网边缘计算应用。
...
### [207] [Querying with Conflicts of Interest](https://arxiv.org/abs/2603.05704)
*Nischal Aryal,Arash Termehchy,Marianne Winslett*

Main category: cs.DB

TL;DR: 提出了一个处理数据源与用户利益冲突的查询框架，包括检测偏见、提取相关信息以及查询重写算法


<details>
  <summary>Details</summary>
Motivation: 数据源与用户之间存在利益冲突，数据源可能为了自身利益（如增加收入）而返回有偏见的结果，而现有方案缺乏实施动力

Method: 提出了一个形式化框架来处理有偏见的查询场景，包括高效算法来检测用户是否能从有偏见的数据源中提取相关信息、检测查询结果中的偏见信息，以及重写输入查询以增加返回结果中的相关信息

Result: 在真实世界数据集上的实验表明，提出的算法高效且能在大型数据上返回相关信息

Conclusion: 该框架为解决数据源与用户利益冲突问题提供了有效的技术方案，能够帮助用户在存在偏见的数据源中获取相关信息
...
### [208] [Efficient Vector Search in the Wild: One Model for Multi-K Queries](https://arxiv.org/abs/2603.06159)
*Yifan Peng,Jiafei Fan,Xingda Wei,Sijie Shen,Rong Chen,Jianning Wang,Xiaojian Luo,Wenyuan Yu,Jingren Zhou,Haibo Chen*

Main category: cs.DB

TL;DR: OMEGA：一种K值泛化的学习式top-K搜索方法，通过基于轨迹的特征训练K=1的基础模型，结合动态精炼过程处理不同K值查询，在保持高准确率的同时显著降低预处理时间和查询延迟。


<details>
  <summary>Details</summary>
Motivation: 现有学习式top-K搜索模型针对特定K值训练，无法泛化到现实世界中的多K值查询场景：对于较大K值会导致准确率下降，对于较小K值会造成性能损失。训练能够泛化不同K值的模型需要大量预处理时间，不适合实际部署。

Method: 提出OMEGA方法：1）使用基于轨迹的特征训练K=1的基础模型；2）通过动态精炼过程准确预测较大K值；3）通过最小化性能损失处理较小K值；4）利用top-K搜索的统计特性减少过度模型调用以提高效率。

Result: 在多个公共和生产数据集上的评估显示：在相同预处理预算下，OMEGA相比最先进的学习式搜索方法平均延迟降低6-33%，同时达到相同的召回目标。仅用16-30%的预处理时间，OMEGA就能达到基线方法最优平均延迟的1.01-1.28倍。

Conclusion: OMEGA是一种K值泛化的学习式top-K搜索方法，能够同时实现高准确率、高性能和低预处理成本，有效解决了现有方法在多K值查询场景中的泛化问题。
...
### [209] [Tag-specific Regret Minimization Problem in Outdoor Advertising](https://arxiv.org/abs/2603.06405)
*Dildar Ali,Abishek Salaria,Ansh Jasrotia,Suman Banerjee*

Main category: cs.DB

TL;DR: TRMOA是一个NP难且不可近似于常数因子的组合优化问题，作者提出了公平感知的贪心轮询、随机贪心和局部搜索算法来最小化广告投放的总遗憾。


<details>
  <summary>Details</summary>
Motivation: 户外广告因其高投资回报率而流行，电商平台希望通过影响者提供商进行有效广告投放。影响者提供商需要在预算约束下合理分配广告标签，满足影响需求并最小化总遗憾。

Method: 1) 证明TRMOA是NP难且不可近似于常数因子；2) 提出公平感知的贪心轮询方法，平衡广告商间的分配；3) 引入随机贪心算法；4) 提出局部搜索算法。

Result: 使用真实世界轨迹和广告牌数据集进行实验，验证了所提方法的有效性和效率。

Conclusion: TRMOA是一个具有挑战性的组合优化问题，提出的公平感知贪心轮询、随机贪心和局部搜索算法能够有效减少遗憾并平衡广告商间的分配。
...
### [210] [RoboLayout: Differentiable 3D Scene Generation for Embodied Agents](https://arxiv.org/abs/2603.05522)
*Ali Shamsaddinlou*

Main category: cs.AI

TL;DR: RoboLayout扩展LayoutVLM，通过集成可达性约束和局部优化改进，生成适合不同具身智能体交互的室内场景布局。
...
Method: 1) 在可微分布局优化中集成显式的可达性约束；2) 支持多种具身智能体抽象（机器人、人类、动物等）；3) 提出局部细化阶段，选择性重新优化问题对象放置。

Result: RoboLayout保持了LayoutVLM的语义对齐和物理合理性，同时增强了面向具身智能体的室内场景生成适用性，在不同场景配置中表现出色。

Conclusion: RoboLayout通过智能体感知推理和优化稳定性改进，成功生成了既语义连贯又适合具身智能体交互的室内场景布局，扩展了视觉语言模型在具身AI环境设计中的应用。
...
### [211] [Real-Time AI Service Economy: A Framework for Agentic Computing Across the Continuum](https://arxiv.org/abs/2603.05614)
*Lauri Lovén,Alaa Saleh,Reza Farahani,Ilir Murturi,Miguel Bordallo López,Praveen Kumar Donta,Schahram Dustdar*

Main category: cs.AI

TL;DR: 论文研究实时AI服务在设备-边缘-云连续体中的资源分配问题，发现服务依赖图（DAG）的拓扑结构是决定去中心化价格机制能否可靠工作的关键因素。层次化依赖图（树或串并联）能实现稳定价格和最优分配，而复杂交叉依赖会导致价格振荡和性能下降。作者提出混合管理架构，通过封装复杂子图为资源切片来改善系统稳定性。


<details>
  <summary>Details</summary>
Motivation: 随着实时AI服务在设备-边缘-云连续体上运行，自主AI代理生成延迟敏感的工作负载，编排多阶段处理流水线，并在策略和治理约束下竞争共享资源。需要理解去中心化、基于价格的资源分配机制在复杂服务依赖关系下能否可靠扩展。

Method: 将服务依赖建模为DAG（有向无环图），节点代表计算阶段，边编码执行顺序。分析不同拓扑结构（层次化vs复杂交叉依赖）对价格稳定性和分配效率的影响。提出混合管理架构，其中跨域集成器将复杂子图封装为资源切片，向市场提供更简单、结构良好的接口。通过系统消融研究（6个实验，1,620次运行，每个10个种子）验证方法。

Result: 1) 依赖图拓扑是价格稳定性和可扩展性的首要决定因素；2) 混合架构将价格波动降低70-75%而不牺牲吞吐量；3) 治理约束产生可量化的效率-合规权衡，取决于拓扑和负载；4) 在诚实竞价下，去中心化市场匹配集中化价值最优基线，证明去中心化协调能复制集中化分配质量。

Conclusion: 服务依赖图的拓扑结构是去中心化资源分配机制能否可靠扩展的关键。对于层次化依赖图，基于价格的机制能实现稳定均衡和最优分配；对于复杂交叉依赖，需要混合架构来封装复杂性。该方法为实时AI服务在设备-边缘-云连续体中的资源管理提供了理论基础和实践方案。
...
### [212] [Reasoning Models Struggle to Control their Chains of Thought](https://arxiv.org/abs/2603.05706)
*Chen Yueh-Han,Robert McCarthy,Bruce W. Lee,He He,Ian Kivlichan,Bowen Baker,Micah Carroll,Tomek Korbak*

Main category: cs.AI

TL;DR: 论文评估了推理模型对思维链（CoT）的控制能力，发现当前模型难以按要求控制思维链内容，这有利于CoT监控的有效性。


<details>
  <summary>Details</summary>
Motivation: 如果模型能够控制其在思维链中表达的内容，可能会破坏CoT监控的有效性。为了评估这种潜在风险，需要测量模型的CoT控制能力。

Method: 引入CoT-Control评估套件，包含要求模型在解决问题时遵循特定CoT指令的任务（如回答遗传学问题时不使用"染色体"一词）。测试不同规模模型、不同训练阶段、不同计算量下的控制能力。

Result: 推理模型的CoT控制能力显著低于最终输出的控制能力（如Claude Sonnet 4.5的CoT控制率仅2.7%，而最终输出控制率达61.9%）。控制能力随模型规模增大而提高，但随RL训练、测试时计算量和问题难度增加而降低。即使给予激励或对抗性提示优化，控制能力也没有显著提升。

Conclusion: 目前CoT控制能力不太可能成为CoT监控的失效模式，但控制能力低下的机制尚不清楚。建议前沿实验室在未来模型中持续跟踪CoT控制能力，以确保CoT监控的有效性。
...
### [213] [Evolving Medical Imaging Agents via Experience-driven Self-skill Discovery](https://arxiv.org/abs/2603.05860)
*Lin Fan,Pengyu Dai,Zhipeng Deng,Haolin Wang,Xun Gong,Yefeng Zheng,Yafei Ou*

Main category: cs.AI

TL;DR: MACRO：一种自演化的医疗代理，通过从已验证的执行轨迹中自主发现有效的多步骤工具序列，将其合成为可复用的复合工具，从而持续扩展其行为库，实现从静态工具组合到经验驱动工具发现的转变。


<details>
  <summary>Details</summary>
Motivation: 临床图像解读本质上是多步骤和工具中心的，现有基于LLM的医疗代理系统将工具集和调用策略视为静态部署，这种设计在面对现实世界的领域转移、跨任务和不断演变的诊断需求时非常脆弱，预定义的工具链经常退化并需要昂贵的人工重新设计。

Method: MACRO采用自演化的经验增强方法：1）从已验证的执行轨迹中自主识别重复出现的有效多步骤工具序列；2）将这些序列合成为可复用的复合工具；3）将这些复合工具注册为新的高级原语；4）使用轻量级图像特征记忆将工具选择基于视觉-临床上下文；5）采用GRPO-like训练循环强化可靠调用发现的复合工具。

Result: 在多样化的医学影像数据集和任务上的广泛实验表明，自主复合工具发现持续提高了多步骤编排的准确性和跨领域泛化能力，优于强基线方法和最近的先进代理方法，弥合了脆弱的静态工具使用与自适应、上下文感知的临床AI辅助之间的差距。

Conclusion: MACRO通过从静态工具组合转向经验驱动的工具发现，实现了医疗代理的自我演化能力，能够在最小监督下进行闭环自我改进，为临床AI辅助提供了更适应、更可靠的解决方案。
...
### [214] [The World Won't Stay Still: Programmable Evolution for Agent Benchmarks](https://arxiv.org/abs/2603.05910)
...
<details>
  <summary>Details</summary>
Motivation: 现有基准测试大多假设静态环境，忽略了现实世界的演化特性，无法有效评估智能体对环境变化的适应能力。需要一种可扩展、可控的环境演化方法来更好地评估智能体的适应性。
...
Conclusion: ProEvolve提供了一种可编程的环境演化方法，能够更好地评估LLM智能体对动态现实世界的适应能力，解决了现有基准测试的局限性。
...
### [215] [DeepFact: Co-Evolving Benchmarks and Agents for Deep Research Factuality](https://arxiv.org/abs/2603.05912)
*Yukun Huang,Leonardo F. R. Ribeiro,Momchil Hardalov,Bhuwan Dhingra,Markus Dreyer,Venkatesh Saligrama*

Main category: cs.AI

TL;DR: 论文提出AtS框架来构建动态可修订的事实核查基准，并创建了DeepFact-Bench基准和DeepFact-Eval验证代理，显著提升了DRR事实核查的准确性。


<details>
  <summary>Details</summary>
Motivation: 现有事实核查工具主要针对通用领域的事实性原子声明，缺乏针对深度研究报告(DRR)的声明级事实核查基准，且静态专家标注基准在此场景下表现脆弱。

Method: 提出Evolving Benchmarking via Audit-then-Score (AtS)框架：当验证器与当前基准不一致时，必须提交证据；审计员裁决争议；接受的修订更新基准后再对模型评分。基于此构建了DeepFact-Bench基准和DeepFact-Eval验证代理。

Result: 经过四轮AtS迭代，专家在微黄金集上的准确率从60.8%提升到90.9%；DeepFact-Eval在DeepFact-Bench上优于现有验证器，并能很好地迁移到外部事实性数据集。

Conclusion: AtS框架通过动态修订机制显著提升了基准质量，专家作为审计员比一次性标注员更可靠；DeepFact-Bench和DeepFact-Eval为DRR事实核查提供了有效的基准和工具。
...
### [216] [An Interactive Multi-Agent System for Evaluation of New Product Concepts](https://arxiv.org/abs/2603.05980)
*Bin Xuan,Ruo Ai,Hakyeon Lee*

Main category: cs.AI

TL;DR: 本研究提出了一种基于大语言模型的多智能体系统，用于自动化产品概念评估，通过八个专业领域虚拟代理进行技术可行性和市场可行性评估，结果与行业专家评估一致。


<details>
  <summary>Details</summary>
Motivation: 传统专家主导的产品概念评估方法存在主观偏见、时间和成本高等局限性，需要一种更客观、高效的自动化评估方法来支持企业战略资源分配和项目成功。

Method: 1) 通过分析产品开发和团队协作研究，建立技术可行性和市场可行性两个主要评估维度；2) 构建包含八个专业领域虚拟代理（如研发、营销）的多智能体系统；3) 使用检索增强生成和实时搜索工具收集客观证据；4) 基于既定标准进行结构化审议；5) 使用专业产品评审数据对代理进行微调以提高判断准确性。

Result: 在专业显示器概念的案例研究中，系统评估排名与行业资深专家的评估结果一致，验证了该多智能体评估方法的可用性。

Conclusion: 基于多智能体的自动化评估方法能够有效支持产品开发决策，提供与传统专家评估一致的结果，同时克服了传统方法的主观偏见和高成本问题。
...
### [217] [Agentic LLM Planning via Step-Wise PDDL Simulation: An Empirical Characterisation](https://arxiv.org/abs/2603.06064)
*Kai Göbel,Pierrick Lorang,Patrik Zips,Tobias Glück*

Main category: cs.AI

TL;DR: LLM作为任务规划器的评估：PyPDDLEngine通过MCP接口将PDDL规划操作暴露为LLM工具调用，让LLM作为交互式搜索策略逐动作选择。在102个IPC Blocksworld实例上，agentic LLM规划（66.7%成功率）比直接LLM规划（63.7%）略有优势，但远低于经典规划器Fast Downward（85.3%）。


<details>
  <summary>Details</summary>
Motivation: 探索大型语言模型（LLMs）是否能够作为可行的任务规划器，与经典的符号规划方法相媲美。任务规划是自主机器人系统的核心能力要求，但LLMs作为规划器的有效性仍是一个开放性问题。

Method: 开发了PyPDDLEngine，一个开源的PDDL仿真引擎，通过Model Context Protocol（MCP）接口将规划操作暴露为LLM工具调用。LLM不是一次性生成完整动作序列，而是作为交互式搜索策略，每次选择一个动作，观察结果状态，并可以重置和重试。在102个IPC Blocksworld实例上评估了四种方法：Fast Downward lama-first和seq-sat-lama-2011作为经典基线，直接LLM规划（Claude Haiku 4.5），以及通过PyPDDLEngine的agentic LLM规划。

Result: Fast Downward达到85.3%的成功率。直接LLM规划和agentic LLM规划分别达到63.7%和66.7%，agentic方法有3个百分点的优势，但每个解决方案的token成本高5.7倍。在大多数共同解决的难度块中，两种LLM方法产生的计划都比seq-sat-lama-2011更短，尽管后者有迭代质量改进，这表明可能是训练数据回忆而非通用规划能力。

Conclusion: agentic方法的收益取决于环境反馈的性质。编码代理受益于编译器错误和测试失败等外部基础信号，而PDDL步骤反馈是自我评估的，让代理在没有外部验证的情况下评估自己的进展。LLMs作为规划器仍有局限性，特别是在需要外部验证的环境中。
...
### [218] [Aggregative Semantics for Quantitative Bipolar Argumentation Frameworks](https://arxiv.org/abs/2603.06067)
*Yann Munro,Isabelle Bloch,Marie-Jeanne Lesot*

Main category: cs.AI

TL;DR: 提出一种新的聚合语义学家族，用于定量双极论证框架，通过三个可解释的步骤分别聚合攻击者和支持者，增强参数化和可理解性。


<details>
  <summary>Details</summary>
Motivation: 现有定量双极论证框架中，攻击者和支持者通常被对称处理，但在实际情境中它们可能扮演不同角色。需要一种更灵活、更可解释的语义学来更好地建模这种不对称性。

Method: 提出聚合语义学，采用三阶段计算：1) 计算攻击者的全局权重；2) 计算支持者的全局权重；3) 将这两个值与论证的内在权重聚合。讨论了三种聚合函数应满足的性质及其与经典渐进语义学原则的关系。

Result: 通过简单示例展示了聚合语义学的灵活性，并在最终示例中测试比较了500种聚合语义学，展示了其广泛的行为范围。该方法保持了双极性，比现有文献更进一步，且更具可理解性。

Conclusion: 聚合语义学为定量双极论证框架提供了一种新颖、灵活且可解释的方法，通过分离处理攻击者和支持者，更好地适应实际情境中的不对称性，增强了语义学的参数化和可理解性。
...
### [219] [Offline Materials Optimization with CliqueFlowmer](https://arxiv.org/abs/2603.06082)
...
<details>
  <summary>Details</summary>
Motivation: 当前基于深度学习的生成建模方法在材料发现中因最大似然训练而无法有效探索有前景的材料空间区域，需要一种能直接优化目标属性的替代技术。

Method: 提出CliqueFlowmer模型，将基于团的离线模型优化（MBO）融入Transformer和流生成框架，实现材料属性的直接优化。

Result: CliqueFlowmer在优化能力上表现优异，生成的材料性能显著优于现有生成基线方法。

Conclusion: CliqueFlowmer为计算材料发现提供了有效的离线模型优化方法，已开源代码以支持专业材料优化问题和跨学科研究。
...
### [220] [Conversational Demand Response: Bidirectional Aggregator-Prosumer Coordination through Agentic AI](https://arxiv.org/abs/2603.06217)
*Reda El Makroum,Sebastian Zwickl-Bernhard,Lukas Kranzl,Hans Auer*

Main category: cs.AI

TL;DR: 本文提出对话式需求响应（CDR），通过双向自然语言交互协调聚合商和产消者，使用智能代理架构实现透明、可解释的需求响应协调。


<details>
  <summary>Details</summary>
Motivation: 现有需求响应协调要么完全自动化，要么仅限于单向调度信号和价格警报，缺乏让产消者进行知情决策的可能性，难以维持产消者的持续参与。

Method: 采用双层多代理架构：聚合商代理发送灵活性请求，产消者家庭能源管理系统通过调用基于优化的工具评估可交付性和成本效益。支持产消者发起的上游通信，可直接向聚合商传达偏好变化。

Result: 概念验证评估显示交互在12秒内完成。该架构展示了智能代理如何弥合聚合商与产消者之间的协调差距，在保持自动需求响应可扩展性的同时，提供透明性、可解释性和用户自主权。

Conclusion: 对话式需求响应通过双向自然语言交互，结合智能代理技术，为维持产消者持续参与提供了可行方案，所有系统组件已开源发布以促进可重复性和进一步发展。
...
### [221] [Artificial Intelligence for Climate Adaptation: Reinforcement Learning for Climate Change-Resilient Transport](https://arxiv.org/abs/2603.06278)
...
TL;DR: 提出基于强化学习的长期洪水适应规划决策支持框架，结合降雨预测、洪水建模、交通模拟和影响量化，在哥本哈根案例中优于传统优化方法


<details>
  <summary>Details</summary>
Motivation: 气候变化加剧降雨和洪水，导致城市交通系统中断增加。制定有效适应策略面临长期基础设施投资、深度气候不确定性和复杂系统相互作用的挑战

Method: 提出基于强化学习的决策支持框架，作为综合评估模型，整合降雨预测、洪水建模、交通模拟，量化基础设施和移动性的直接间接影响，学习平衡投资成本与避免影响的适应策略

Result: 在哥本哈根2024-2100年案例研究中，框架优于传统优化方法，发现协调的空间和时间适应路径，学习影响减少与适应投资之间的权衡，产生更具韧性的策略

Conclusion: 强化学习作为灵活决策支持工具，在气候不确定性下具有自适应基础设施规划的潜力，能够发现传统方法难以找到的协调适应路径
...
### [222] [The EpisTwin: A Knowledge Graph-Grounded Neuro-Symbolic Architecture for Personal AI](https://arxiv.org/abs/2603.06290)
...
TL;DR: EpisTwin是一个神经符号框架，通过构建可验证的个人知识图谱来解决个人AI中的数据碎片化问题，结合图检索增强生成和在线深度视觉细化进行复杂推理。


<details>
  <summary>Details</summary>
Motivation: 当前个人AI面临用户数据分散在孤立数据孤岛中的问题，而传统的检索增强生成方法依赖非结构化向量相似性，无法捕捉潜在的语义拓扑和时间依赖关系，限制了整体理解能力。

Method: EpisTwin使用多模态语言模型将异构的跨应用数据提取为语义三元组，构建个人知识图谱。在推理时，通过代理协调器结合图检索增强生成和在线深度视觉细化，动态地将符号实体重新锚定到原始视觉上下文中。

Result: 该框架在PersonalQA-71-100合成基准测试中表现出色，该基准模拟真实用户的数字足迹。在各种最先进的评判模型评估中都获得了稳健的结果。
...
TL;DR: SAHOO框架通过多信号检测器、约束保护检查和回归风险量化来监控和控制递归自我改进中的对齐漂移，在代码生成、数学推理和真实性任务中实现显著质量提升同时保持安全约束。


<details>
  <summary>Details</summary>
Motivation: 递归自我改进正从理论走向实践，但迭代自我修改存在微妙的对齐漂移风险。需要一种实用的框架来监控和控制这种漂移，确保在系统自我改进过程中保持对齐性。

Method: SAHOO框架包含三个安全措施：(1) 目标漂移指数(GDI)，一个结合语义、词汇、结构和分布度量的学习型多信号检测器；(2) 约束保护检查，强制执行安全关键不变性，如语法正确性和非幻觉；(3) 回归风险量化，标记可能撤销先前增益的改进周期。

Result: 在189个代码生成、数学推理和真实性任务中，SAHOO实现了显著质量提升：代码任务改进18.3%，推理任务改进16.8%，同时在两个领域保持约束保护，在真实性任务中保持低违规率。阈值在18个任务的验证集上校准。

Conclusion: SAHOO使递归自我改进过程中的对齐保护变得可测量、可部署和可系统验证，绘制了能力-对齐边界，展示了早期改进周期的高效性和后期对齐成本的上升，以及领域特定的权衡（如流畅性与事实性）。
...
TL;DR: 提出schema-gated orchestration架构，通过模式验证分离对话灵活性与执行确定性，解决LLM科学工作流中的确定性与灵活性矛盾


<details>
  <summary>Details</summary>
Motivation: LLM可以将自然语言目标转化为可执行计算，但科学工作流需要确定性、可追溯性和治理保障，而LLM自主决策难以保证这些要求。研究发现工业研发中存在确定性约束执行与对话灵活性之间的冲突需求。

Method: 1) 通过18位专家的半结构化访谈识别需求；2) 提出schema-gated orchestration原则：模式作为强制执行的边界；3) 将需求操作化为执行确定性(ED)和对话灵活性(CF)；4) 使用多模型协议评估20个系统，分析架构景观

Result: 1) 发现经验帕累托前沿：现有系统无法同时实现高灵活性和高确定性；2) 多模型评分显示高度一致性(Krippendorff α=0.80 ED, 0.98 CF)；3) 识别出生成式与工作流中心极端之间的收敛区域；4) 提出三种操作原则指导采用

Conclusion: schema-gated架构通过分离对话权限与执行权限，能够解耦灵活性与确定性的权衡，为LLM科学工作流提供可治理的解决方案，同时保持人类在环控制和透明度。
...
### [225] [Boosting deep Reinforcement Learning using pretraining with Logical Options](https://arxiv.org/abs/2603.06565)
*Zihan Ye,Phil Chau,Raban Emunds,Jannis Blüml,Cedric Derstroff,Quentin Delfosse,Oleg Arenz,Kristian Kersting*

Main category: cs.AI

TL;DR: 提出H²RL混合分层强化学习方法，通过逻辑选项预训练将符号结构注入神经网络，改善长期决策能力


<details>
  <summary>Details</summary>
Motivation: 深度强化学习智能体经常出现对齐问题，过度利用早期奖励信号；纯符号方法难以扩展到连续环境；受人类学习新技能能力启发，需要结合符号和神经方法的优势

Method: H²RL采用两阶段框架：1) 逻辑选项预训练策略，将符号结构注入神经网络；2) 通过标准环境交互精炼最终策略，避免短期奖励循环，引导目标导向行为

Result: 实验表明该方法能持续改善长期决策能力，在多个任务上优于纯神经、纯符号和神经符号基线方法

Conclusion: 提出的混合分层强化学习方法成功将符号结构注入深度策略，解决了强化学习中的对齐问题，提高了长期决策性能
