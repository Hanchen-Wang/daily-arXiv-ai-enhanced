...
Motivation: 大型语言模型在创造性任务中表现出色，但缺乏具身交互特性。舞蹈作为人类表达的基本形式，可以补充这种体验，探索创造性的人机交互。

Method: 利用单人运动数据和高层次特征，结合两个扩散模型、运动修复和运动风格迁移技术，生成既时间连贯又对选定运动参考有响应的运动表示。

Result: 通过定量评估生成样本特征分布与测试集的收敛性，证明了模型的有效性。生成的动作既多样化，显示出与人类伙伴的各种偏差，同时又显得真实。

Conclusion: 该模型是实现与AI创造性舞蹈共舞的第一步，生成的舞蹈动作既多样化又真实，为人类与AI的创造性互动提供了新的可能性。
...
<details>
  <summary>Details</summary>
Motivation: 珊瑚礁面临污染、海洋酸化和海水温度异常等日益严重的威胁，亟需高效的监测和保护方法。

Method: 基于包含不同环境条件下健康和白化珊瑚样本的全球数据集，对比评估ResNet、Vision Transformer和CNN三种最先进的模型，并进行全面的超参数调优。

Result: 经过超参数调优后，CNN模型取得了最高的88%准确率，超越了现有基准。
...
TL;DR: 评估YOLOv8深度学习管道在肯尼亚和坦桑尼亚视频样带中自动识别珊瑚礁鱼类到科级水平的能力，为西印度洋地区提供首个自动化珊瑚礁鱼类监测基准。


<details>
  <summary>Details</summary>
Motivation: 西印度洋珊瑚礁监测受限于水下视觉普查的劳动力需求，需要自动化解决方案来提高监测效率和可扩展性。

Method: 使用YOLOv8深度学习管道，在肯尼亚和坦桑尼亚收集的视频样带上测试24个鱼类科的识别，评估不同配置下的性能。

Result: 最佳模型达到mAP@0.5为0.52，对丰富鱼类科识别准确率高，但对稀有或复杂分类群的检测较弱。

Conclusion: 深度学习有潜力作为传统监测方法的可扩展补充工具，但需要改进对稀有物种的检测能力。
...
TL;DR: 提出了一种基于互信息的数据选择方法，用于改进对比学习中的数据增强策略，通过选择在自然扰动下具有高互信息的场景补丁作为正样本，提升特征提取器的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有的对比学习方法在数据选择和增强方面仍依赖人工假设或工程化设计，可能不是最优的。特别是在数据增强方面，主要关注颜色抖动来模拟真实世界的光照变化，但这种方法可能不够全面。

Method: 基于真实世界分布计算训练数据的互信息，选择在自然扰动（如颜色变化和运动）下具有高互信息的场景补丁作为对比学习的正样本，从而改进数据增强策略。

Result: 在多个基准测试和最先进的表示学习框架上评估了该方法，证明了其有效性，表明这是一个有前景的研究方向。

Conclusion: 基于互信息的数据选择方法能够提升对比学习中特征提取器的泛化能力，为未来的表示学习研究提供了新的思路。
...
TL;DR: 本文对NVIDIA FLARE、Flower和Owkin Substra三个联邦学习框架在医学影像应用中的性能进行基准测试，评估了模型性能、收敛效率、通信开销、可扩展性和开发者体验。


<details>
  <summary>Details</summary>
Motivation: 联邦学习在医疗AI中具有重要价值，能够在不同机构间实现协作模型训练而无需直接共享数据。本研究旨在评估主流FL框架在医学影像应用中的实际适用性。

Method: 使用PathMNIST数据集，对三个FL框架（NVIDIA FLARE、Flower、Owkin Substra）进行基准测试，评估模型性能、收敛效率、通信开销、可扩展性和开发者体验。

Result: NVIDIA FLARE在生产可扩展性方面表现最佳，Flower在原型设计和学术研究方面更具灵活性，Owkin Substra在隐私和合规性方面具有突出优势。

Conclusion: 三个框架各有优势，适用于不同的使用场景，在医疗环境中的实际部署都具有重要意义。
...
TL;DR: 该论文对水稻叶片图像进行去噪和对比度增强的对比研究，结合多种去噪方法与CLAHE技术，为农业图像处理提供评估基础。
...
Method: 采用多种知名图像去噪方法结合CLAHE对比度增强技术，在水稻叶片图像数据集上进行实验，使用多种指标全面测试增强效果。

Result: 通过实验验证了不同去噪方法与CLAHE结合的有效性，为图像处理方法评估提供了坚实基础。

Conclusion: 该研究为数字图像处理方法评估提供了有效框架，对农业研究和其他领域的未来应用具有重要参考价值。
...
TL;DR: 该研究系统分析了不同LiDAR扫描模式（重复扫描与非重复扫描）对路边感知性能的影响，创建了InfraLiDARs基准数据集，发现非重复扫描LiDAR在成本效益方面具有优势。


<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注LiDAR的优化布局，但不同扫描模式对感知性能的深远影响尚未得到充分研究，特别是在基础设施环境中的表现差异。

Method: 在CARLA仿真环境中创建InfraLiDARs基准数据集，使用同时运行的基础设施LiDAR，涵盖重复和非重复两种扫描模式，并进行全面的统计分析。

Result: 研究发现非重复扫描LiDAR与128线重复扫描LiDAR在各种场景下表现出相当的检测性能，尽管非重复LiDAR感知范围有限，但因其低成本而具有成本效益优势。

Conclusion: 该研究为设置具有最优LiDAR扫描模式和兼容算法的路边感知系统提供了见解，并公开发布基准数据集以促进进一步研究。
...
TL;DR: Cosmos-Predict2.5是基于流架构的物理AI世界基础模型，统一了文本、图像和视频到世界的生成，结合Cosmos-Reason1提供更丰富的文本基础和精细的世界模拟控制。
...
Method: 采用基于流的架构，结合200M精选视频片段训练，并通过强化学习进行后训练优化，同时开发了控制网络风格的Cosmos-Transfer2.5用于模拟到真实和真实到真实的世界转换。

Result: 相比Cosmos-Predict1，在视频质量和指令对齐方面有显著提升，发布了2B和14B规模的模型。Cosmos-Transfer2.5虽然比前代小3.5倍，但提供了更高的保真度和鲁棒的长时视频生成。
...
<details>
  <summary>Details</summary>
Motivation: 高山生态系统面临快速气候变化，需要频繁的栖息地监测，但人工测绘成本过高。研究旨在解决地理空间基础模型在复杂自然环境中的应用差距。

Method: 采用两种变化检测范式：后分类变化检测（使用Prithvi-EO-2.0、Clay v1.0和U-Net CNN）和直接变化检测（使用ChangeViT和U-Net）。使用高分辨率多模态数据（RGB、NIR、LiDAR、地形属性）。

Result: Clay v1.0在多类栖息地变化检测中达到51%准确率，U-Net为41%；二元变化检测两者均达到67%。直接变化检测在二元检测中IoU为0.53，但多类检测准确率仅28%。集成LiDAR将语义分割准确率从30%提升至50%。

Conclusion: 尽管整体准确率低于更均质景观，但反映了复杂高山栖息地的真实性能。未来工作将集成基于对象后处理和物理约束以增强适用性。
...
TL;DR: LeMiCa是一个无需训练的高效扩散视频生成加速框架，通过词典序最小最大路径优化策略显著提升生成质量和推理速度。
...
Method: 将缓存调度建模为带误差权重边的有向图，引入词典序最小最大路径优化策略，明确限制最坏情况路径误差。

Result: 在多个文本到视频基准测试中，LeMiCa在推理速度和生成质量上均实现双重提升，在Latte模型上达到2.9倍加速，Open-Sora上LPIPS分数为0.05。

Conclusion: LeMiCa以最小感知质量损失实现了显著的性能提升，为加速扩散视频生成提供了一个鲁棒且可推广的范式。
...
Method: 三阶段框架：1)训练轻量级残差执行器探测失败区域；2)使用混合rollout方案收集与部署分布对齐的轨迹；3)通过标准SFT将精选轨迹蒸馏回通用模型。

Result: 在LIBERO上达到接近饱和的99%任务成功率，在SimplerEnv上提升超过50%，在真实世界Franka和YAM机械臂操作任务中实现100%成功率。
...
TL;DR: 提出SpinalSAM-R1系统，结合微调SAM和DeepSeek-R1，用于脊柱CT图像分割，支持自然语言交互和多种提示方式。


<details>
  <summary>Details</summary>
Motivation: 脊柱CT图像分割面临低对比度和复杂边界的挑战，现有模型如SAM在脊柱CT领域存在标注需求高和领域适应性差的问题。

Method: 提出SpinalSAM-R1系统，集成解剖引导注意力机制和语义驱动交互协议，使用LoRA进行高效微调，支持点、框和文本提示。

Result: 在脊柱解剖结构CT图像上验证，获得优越分割性能，开发交互软件支持11种临床操作，解析准确率94.3%，响应时间低于800ms。

Conclusion: SpinalSAM-R1系统有效解决了脊柱CT图像分割的挑战，提供了高效准确的交互式分割解决方案。
...
TL;DR: 提出一种用于共聚焦激光内镜（CLE）视频序列的过滤方法，以减少自监督学习训练中的数据集冗余，提高训练效率和收敛性。
...
Method: 使用CLE视频序列过滤功能减少SSL训练冗余，采用四种最先进基线网络和基于视觉变换器小骨干的SSL师生网络进行评估。

Result: 在两个数据集上，过滤后的SSL预训练模型获得最高测试准确率（67.48%和73.52%），显著优于非SSL基线，训练时间减少67%。

Conclusion: SSL是有效的CLE预训练方法，提出的CLE视频过滤器可提高自监督学习场景下的训练效率。
...
TL;DR: FreeSliders是一种无需训练、模态无关的方法，通过部分估计概念滑动器公式在推理时实现细粒度可控生成，支持图像、音频和视频等多种模态。


<details>
  <summary>Details</summary>
Motivation: 扩散模型在图像、音频和视频生成方面表现出色，但实现细粒度可控生成仍然具有挑战性。现有的概念滑动器方法需要针对每个概念进行训练和架构特定的微调，限制了向新模态的扩展性。

Method: 提出完全无需训练且模态无关的FreeSliders方法，在推理时部分估计概念滑动器公式。扩展了概念滑动器基准以支持视频和音频，建立了首个多模态细粒度概念生成控制套件。

Result: 实验表明，该方法实现了跨模态的即插即用、无需训练的概念控制，改进了现有基线方法，并为可控生成建立了新的工具。

Conclusion: FreeSliders提供了一种简单有效的训练无关方法，能够在多种模态中实现细粒度概念控制，解决了现有方法的可扩展性问题。
...
TL;DR: MOVAI是一个新颖的分层文本到视频生成框架，通过组合场景解析、时空注意力机制和渐进式视频优化，显著提升了视频生成的质量和时间一致性。
...
Method: 提出三个关键创新：组合场景解析器将文本分解为带时间标注的分层场景图；时空注意力机制确保帧间连贯运动动态；渐进式视频优化模块通过多尺度时间推理迭代提升视频质量。

Result: 在标准基准测试中，MOVAI实现了最先进的性能，LPIPS指标提升15.3%，FVD指标提升12.7%，用户偏好研究提升18.9%。

Conclusion: MOVAI框架在生成复杂多对象场景方面表现出色，能够实现真实的时间动态和精细的语义控制。
...
<details>
  <summary>Details</summary>
Motivation: 受机器学习中的上下文推理和人类心理模拟的启发，旨在提高视觉语言模型在物理模拟方面的性能，并深入理解模型内部的模拟动态。

Method: Chain-of-Time方法在推理时生成模拟过程中的中间图像序列，应用于2D图形模拟和真实3D视频等多个领域，测试包括速度、加速度、流体动力学和动量守恒在内的各种物理特性。

Result: 使用Chain-of-Time方法显著提升了最先进图像生成模型的性能，分析揭示了模型能够模拟随时间展开的物理特性（如速度、重力和碰撞），同时也发现了模型在某些情况下难以从输入图像推断特定物理参数的问题。

Conclusion: Chain-of-Time方法不仅提高了物理模拟性能，还提供了对模型内部模拟动态的深入洞察，揭示了传统物理推理评估中隐藏的模型能力与局限性。
...
TL;DR: 提出了首个结合生成AI和深度强化学习的端到端框架，用于实现自主、可重复的心脏超声扫描，解决了现有方法缺乏可重复性、依赖专有数据和使用简化模型的问题。


<details>
  <summary>Details</summary>
Motivation: 心脏超声检查存在操作者依赖性强、时间限制和人为错误等问题，特别是在偏远地区缺乏专业医生，需要自动化解决方案来确保一致性和可访问性。

Method: 框架包含两个组件：(1) 结合GAN和VAE的条件生成模拟器，生成逼真的动作条件图像；(2) 深度强化学习模块，利用模拟器学习自主扫描策略。

Result: 该框架通过专家验证模型提供AI驱动指导，支持生成逼真的超声图像，并建立了可扩展到其他器官的可重复基础，同时发布了公开可用的真实心脏超声数据集。

Conclusion: 提出的解决方案通过多个实验验证了有效性，VAE-GAN在定性和定量评估中表现优于现有GAN变体，DRL扫描系统在不同配置下都显示出良好效果。
...
<details>
  <summary>Details</summary>
Motivation: 当前6D物体姿态估计方法在从合成数据泛化到真实世界时表现脆弱，难以应对光照变化、无纹理物体和严重遮挡等挑战。

Method: 采用双流架构：使用自监督Vision Transformer（DINOv2）处理RGB数据，PointNet++编码器处理深度点云数据，然后将两个特征流融合到多任务预测头。

Result: 在具有挑战性的Occluded-LineMOD数据集上取得了新的SOTA性能，验证了其优越的鲁棒性和准确性。

Conclusion: VLM6D通过有效融合视觉和几何特征，在严重遮挡和复杂环境下实现了鲁棒且精确的6D物体姿态估计。
...
Method: 采用预训练模型，结合线性层和高级正则化技术优化架构，在ConvNeXt框架内集成适应的注意力机制以关注年龄相关面部特征。

Result: 在MORPH II、CACD和AFAD等基准数据集上，该混合模型在平均绝对误差(MAE)方面表现出优越性能，超越了传统方法。

Conclusion: ConvNeXt-ViT混合架构不仅性能优越，还为年龄估计和相关视觉任务的未来发展提供了坚实基础，展示了CNN与Transformer无缝集成的巨大潜力。
...
<details>
  <summary>Details</summary>
Motivation: 长视频理解中，大型多模态模型面临视觉token数量过多的可扩展性限制，需要高效的压缩方法来减少token数量同时保持性能。

Method: 基于设施位置函数原理，使用惰性贪心算法快速选择具有代表性和多样性的视觉token子集，在预定义token预算内实现高效压缩。

Result: 在Video-MME、MLVU和LongVideoBench等大规模基准测试中，FLoC框架持续超越现有压缩技术，在保持性能的同时显著提高处理速度。

Conclusion: FLoC提供了一个无需训练、模型无关且查询无关的通用解决方案，能无缝集成到各种视频-LLM和现有工作流中，有效解决长视频理解的关键挑战。
...
TL;DR: 本文提出了一种增强图像保护方法鲁棒性的简单技术，通过自适应高斯模糊调整噪声频率谱，使对抗性噪声更难被检测和逆转。


<details>
  <summary>Details</summary>
Motivation: 现有基于对抗性噪声的图像保护方法容易被简单技术（如JPEG压缩）逆转，因此需要开发既不可感知又不可逆转的保护方法。

Method: 在现有图像保护方法的基础上，应用自适应区域高斯模糊来调整噪声的频率谱，增强对抗性噪声的鲁棒性。

Result: 实验表明该方法能显著提高现有方法在各种逆转技术和图像编辑场景下的最坏情况保护性能，同时减少噪声引起的质量下降。

Conclusion: 通过调整对抗性噪声的频率特性，可以有效增强图像保护方法的鲁棒性，为防御恶意图像编辑提供了更实用的解决方案。
...
TL;DR: 提出了CompAgent，首个用于视觉合规验证的智能体框架，通过增强多模态大语言模型(MLLMs)的视觉工具和规划能力，实现结构化合规规则的动态应用。


<details>
  <summary>Details</summary>
Motivation: 视觉合规验证在媒体、娱乐和广告等领域至关重要，但现有方法依赖成本高昂的任务特定模型，且泛化能力有限。MLLMs虽然具备广泛知识，但难以处理细粒度视觉细节和结构化合规规则。

Method: CompAgent框架包含规划代理和验证代理。规划代理根据合规策略动态选择视觉工具(如目标检测器、人脸分析器、NSFW检测器等)，验证代理整合图像、工具输出和政策上下文进行多模态推理。

Result: 在公开基准测试中，CompAgent优于专用分类器、直接MLLM提示和精心设计的路由基线，在UnsafeBench数据集上达到76%的F1分数，比现有最优方法提升10%。

Conclusion: 智能体规划和工具增强推理为可扩展、准确和适应性强的视觉合规验证提供了有效解决方案。
...
Method: 采用多智能体协作框架，集成反向图像搜索、元数据提取、预训练分类器和视觉语言模型分析等取证工具，通过结构化多智能体辩论机制处理冲突证据，并配备记忆增强推理模块。

Result: 在包含6000张图像的全面评估中，AIFo达到97.05%的准确率，显著优于传统分类器和最先进的视觉语言模型。
...
TL;DR: 本文提出了一种基于能量的多提示学习方法(EMPL)，通过从能量分布中采样生成多个提示嵌入，实现了参数高效且平衡的领域内外泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注单提示范式，很少探索多提示学习的技术潜力。本文旨在为视觉语言多提示学习提供一个原则性回顾，并验证多提示增强在视觉语言迁移中的优越性。

Method: 将模态间隙现象扩展到可学习提示，提出基于能量的多提示学习(EMPL)，从VLMs隐式定义的能量分布中采样生成多个提示嵌入。

Result: 综合实验验证了作者的主张和EMPL方法的优越性，在领域内外开放词汇泛化方面取得了平衡。

Conclusion: EMPL方法不仅参数高效，而且严格实现了领域内外开放词汇泛化之间的平衡，为多提示学习提供了有效的解决方案。
...
TL;DR: 提出一种高效的迁移学习方法，用于地面终端组件本地检测天气相关状况，包括雪、湿和其他恶劣天气条件，性能优于传统深度学习方法。


<details>
  <summary>Details</summary>
Motivation: 恶劣天气事件对低轨卫星互联网性能和可靠性有显著影响，需要精细化的天气条件检测能力来协助故障诊断和缓解。

Method: 采用高效的迁移学习方法，使地面组件能够本地检测代表性天气相关条件。

Result: 该方法能够检测雪、湿和其他恶劣天气条件，性能优于YOLOv7、YOLOv9、Faster R-CNN和R-YOLO等典型深度学习方法。

Conclusion: 提出的迁移学习方法在各种场景下具有良好泛化能力，为可靠的卫星互联网提供了有效的天气条件检测解决方案。
...
TL;DR: DM-QPMNet是一种双编码器网络，通过多模态注意力融合机制，有效利用偏振强度图像和相位图的互补信息，显著提升了单次定量相位显微镜中的细胞分割性能。
...
Method: 提出双编码器网络，将偏振强度图像和相位图作为不同模态分别编码，通过多头注意力在中间深度融合模态特定特征，使用双源跳跃连接和每模态归一化实现多模态集成。

Result: 相比单一模态基线和简单拼接方法，该方法在细胞分割任务上表现出显著改进。

Conclusion: 模态特定编码与可学习融合机制能有效利用ssQPM同时捕获的互补照明和相位线索，实现稳健的细胞分割。
...
<details>
  <summary>Details</summary>
Motivation: 解决petascale电子显微镜数据集在存储、传输和下游分析方面的限制。

Method: 使用向量量化变分自编码器(VQ-VAE)进行压缩，结合Transformer先验模型通过特征线性调制和连接恢复纹理，并采用ROI驱动的工作流程。

Result: 实现了16x到1024x的压缩比，支持顶部解码的极端压缩，并能通过可选Transformer先验恢复纹理细节。

Conclusion: 该框架为大规模电子显微镜数据提供了高效的压缩和选择性重建解决方案，显著降低了存储和传输需求。
...
<details>
  <summary>Details</summary>
Motivation: 现有最优传输映射计算方法主要针对欧几里得空间和球面，但在涉及层次数据、网络和多亏格黎曼曲面等场景中，双曲空间中的最优传输问题自然出现。

Method: 使用几何变分技术，将欧几里得和球面几何的方法扩展到双曲设置，提出新颖高效的计算算法。

Result: 在合成数据和多亏格曲面模型上进行了实验，验证了所提方法的有效性。

Conclusion: 成功开发了双曲空间中的最优传输映射计算方法，填补了现有方法的空白，为处理层次结构数据等应用提供了新工具。
...
TL;DR: 提出MSDI框架，通过3D高斯表示和运动扩散先验，结合大语言模型的空间语义信息，实现零样本的物体感知4D人体运动生成，解决视频生成中的物理不一致问题。


<details>
  <summary>Details</summary>
Motivation: 现有视频扩散模型生成的视频存在不现实的变形、语义违规和物理不一致问题，主要原因是缺乏3D物理先验。

Method: 使用预生成的3D人体和物体，结合LLMs的空间和提示语义信息，通过提出的运动扩散分数蒸馏采样(MSDS)进行空间感知运动优化，从预训练运动扩散模型中蒸馏分数梯度来精炼人体运动。

Result: 实验表明该框架能生成自然且物理合理的人体运动，尊重3D空间上下文，为真实4D生成提供了可扩展解决方案。

Conclusion: 提出的零样本方法避免了重新训练，能泛化到分布外的物体感知人体运动，解决了现有方法在有限交互数据集上联合训练的限制。
...
TL;DR: 本文介绍了L48数据集，这是一个真实世界的细粒度多标签数据集，用于评估单正多标签(SPML)方法。相比之前基于合成数据的方法，L48提供了更真实的评估基准。


<details>
  <summary>Details</summary>
Motivation: 现有的SPML方法都在合成数据集上开发和测试，这些数据集无法反映真实世界的复杂性，特别是细粒度分类中的困难误分类问题。

Method: 构建L48数据集，这是一个基于鸟类声音记录的细粒度多标签数据集，提供自然的SPML设置，并包含两个扩展设置（利用领域先验获得额外负标签）。

Result: 在L48数据集上对现有SPML方法进行基准测试，发现与合成数据集相比存在显著的性能差异，并分析了方法的弱点。

Conclusion: 需要更真实和困难的基准来推动SPML方法的发展，L48数据集为此提供了重要工具。
...
<details>
  <summary>Details</summary>
Motivation: 生物学研究中需要处理大量甲虫图像数据，传统人工处理效率低下，需要自动化解决方案来加速研究进程。

Method: 使用基于transformer的开集目标检测器和视觉语言模型进行甲虫检测，然后手动标注670张甲虫图像并微调transformer分割模型进行精细分割。

Result: 构建了专门针对甲虫图像处理的集成深度学习管道，能够相对准确地完成甲虫检测和形态分割任务。

Conclusion: 该管道能够显著提高大规模甲虫数据的处理效率，加速生物学研究进程。
...
TL;DR: 提出MambaNetLK，一种基于Mamba状态空间模型的3D点云配准方法，专门针对内窥镜导航中的特征退化问题，在临床数据集上显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 解决内窥镜导航中生物组织重复纹理和局部均匀几何特征导致的特征退化问题，以及术前与术中观察之间的域偏移对配准稳定性的影响。

Method: 提出MambaNetLK框架，将Mamba状态空间模型作为跨模态特征提取器集成到PointNetLK架构中，使用Lucas-Kanade算法进行迭代配准。

Result: 在C3VD-Raycasting-10k临床数据集上，MambaNetLK将中值旋转误差降低56.04%，RMSE平移误差降低26.19%，在ModelNet40上表现出强泛化能力和对初始位姿扰动的鲁棒性。

Conclusion: MambaNetLK为手术导航中的3D配准提供了稳健基础，结合全局表达性SSM特征提取器和大规模临床数据集，可在结肠镜等微创手术中实现更准确可靠的引导系统。
...
TL;DR: 论文提出了Spot The Ball基准测试，用于评估视觉语言模型在视觉社交推理方面的能力，发现人类在定位被移除的球类位置时准确率比最先进模型高出2-3倍。


<details>
  <summary>Details</summary>
Motivation: 人类在视觉社交推理方面表现出色，能够从细微的行为线索推断场景中的隐藏元素。这种能力对于开发更类似人类的AI代理至关重要，但目前AI模型在此方面的表现尚不清楚。

Method: 使用足球、篮球和排球图像创建基准测试，要求定位被移除的球的位置。评估了四种最先进的视觉语言模型，并比较了人类基线表现。

Result: 人类准确率在20-34%之间，而所有模型的准确率都不超过17%。模型主要依赖表面空间启发式方法，而人类则利用社交线索如注视方向和身体姿势。

Conclusion: 视觉社交推理中存在显著的人类-模型差距，需要开发能够显式编码结构化行为线索的架构来实现稳健的类人推理。
...
TL;DR: 提出了一种结合CLIP视觉变换器和轻量级分类器的联邦学习框架，用于农业分类任务，在保护隐私的同时显著提升准确率和降低通信成本
...
TL;DR: PersonalView是一个轻量级适配方法，仅需100个训练样本就能让现有模型获得多视角生成能力，通过条件架构设计和语义对应对齐损失实现多视角一致性生成。
...
Method: 设计条件架构利用预训练扩散变换器的上下文学习能力，并使用语义对应对齐损失保持预训练模型的原始生成能力。

Result: 在仅使用100个训练样本的情况下，PersonalView在多视角一致性、文本对齐、身份相似度和视觉质量方面显著优于需要大量多视角数据训练的基线方法。

Conclusion: PersonalView证明了通过轻量级适配方法，可以高效地为现有模型添加多视角生成能力，仅需少量训练数据即可达到优异性能。
...
TL;DR: 提出了LITHOS数据集，这是最大的公开岩石薄片分析数据集，包含211,604个高分辨率RGB偏振光图像块和105,802个专家标注的矿物颗粒，涵盖25种矿物类别。


<details>
  <summary>Details</summary>
Motivation: 岩石学分析是劳动密集型任务，需要专家通过光学偏光显微镜进行详细视觉检查，限制了可扩展性，迫切需要自动化技术。

Method: 创建了LITHOS数据集，评估了多种深度学习技术，并提出了一种双编码器transformer架构，整合两种偏振模式作为基准方法。

Result: 双偏振模型持续优于单偏振模型，证明了偏振协同在矿物分类中的价值。

Conclusion: 发布了LITHOS基准数据集、代码和预训练模型，以促进岩石学自动化分析的复现性和进一步研究。
...
TL;DR: 该研究系统评估了11种轻量级视觉模型在7个数据集上的跨域泛化能力，提出了Cross-Dataset Score指标来衡量模型鲁棒性，发现ImageNet准确率不能可靠预测其他领域性能，并识别出促进泛化的关键架构组件。


<details>
  <summary>Details</summary>
Motivation: 轻量级视觉模型主要基于ImageNet进行基准测试，但缺乏对其跨数据集泛化能力的系统评估，需要量化模型在不同视觉领域的鲁棒性。

Method: 在7个多样化数据集上，使用固定100轮训练计划评估11种轻量级视觉模型（2.5M参数），引入Cross-Dataset Score统一指标来量化模型性能的一致性和鲁棒性。

Result: 研究发现：(1) ImageNet准确率不能可靠预测细粒度或医学数据集的性能；(2) xScore可作为移动模型性能的可扩展预测指标，仅需四个数据集即可估算；(3) 各向同性卷积、高空间分辨率和通道注意力等架构组件促进泛化，而Transformer块带来额外收益有限。

Conclusion: 该研究提供了在ImageNet之外评估轻量级视觉模型的可复现框架，强调了移动友好架构的关键设计原则，为开发跨域鲁棒模型提供指导。
...
TL;DR: 提出了一种结合DeepONet和NTK的混合方法，用于解决复杂的反问题，包括Navier-Stokes方程控制的源定位和图像重建。
...
TL;DR: 提出FedDISC框架，通过联邦学习整合模态特定扩散模型来解决多模态情感识别中的模态缺失问题，确保恢复模态与可用模态在语义和上下文上的一致性。


<details>
  <summary>Details</summary>
Motivation: 现实场景中不可预测的模态缺失会显著降低现有多模态情感识别方法的性能，传统基于完整多模态数据训练的缺失模态恢复方法在极端数据分布下容易产生语义失真。

Method: FedDISC框架结合联邦学习和扩散模型，使用对话图网络捕捉对话依赖关系，语义条件网络强制语义对齐，并采用交替冻结聚合策略促进协作优化。

Result: 在IEMOCAP、CMUMOSI和CMUMOSEI数据集上的实验表明，FedDISC在各种缺失模态模式下都实现了优越的情感分类性能，超越了现有方法。

Conclusion: FedDISC通过联邦扩散模型和语义一致性机制，有效解决了多模态情感识别中的模态缺失问题，提升了在现实场景中的鲁棒性。
...
TL;DR: OSMGen是一个从OpenStreetMap原始数据直接生成逼真卫星图像的生成框架，能够创建前后一致的图像对，用于解决训练数据稀缺和类别不平衡问题。
...
Method: OSMGen使用完整的OSM JSON数据（包括矢量几何、语义标签、位置和时间），而不是依赖栅格瓦片，从而实现对场景生成的细粒度控制。核心特性是能够生成一致的前后图像对。

Result: 该框架能够生成用于解决数据稀缺和类别不平衡问题的训练数据，并为规划者提供通过编辑地图数据来预览拟议干预措施的简单方法。
...
TL;DR: 提出了一种基于扩散模型重建动态的AI生成图像检测方法，通过分析多强度噪声下的重建指标变化来区分真实与合成图像。


<details>
  <summary>Details</summary>
Motivation: 传统基于频率或像素级伪影的深度伪造检测方法对现代文本到图像系统（如Stable Diffusion和DALL-E）无效，因为这些系统能生成逼真且无伪影的图像。

Method: 利用多强度图像重建动态（称为扩散回弹）来分析LPIPS、SSIM和PSNR等重建指标在不同噪声强度下的演变，提取可解释的基于流形的特征。
...
TL;DR: 该论文提出了一种基于迁移学习的轻量级热红外云分割方法，用于FOREST-2立方星任务，通过使用MobileNet编码器的UNet架构，在有限硬件和单热红外波段条件下实现高效云检测。


<details>
  <summary>Details</summary>
Motivation: 立方星任务受限于硬件资源和光谱信息（通常只有单热红外波段），且缺乏足够的标注数据，使得传统的云掩膜技术不可行，需要开发适用于此类约束条件的云分割方法。

Method: 使用UNet架构配合轻量级MobileNet编码器，先在公开的Landsat-7云覆盖评估数据集上预训练，然后在少量任务特定样本上进行联合训练微调，最后将模型转换为TensorRT引擎。

Result: 相比仅使用FOREST-2数据的基线方法，宏F1分数从0.850提升到0.877，在NVIDIA Jetson Nano上实现全图像推理时间低于5秒。

Conclusion: 通过利用公开数据集和轻量级架构，可以在资源受限的轨道任务中实现准确高效的单热红外云掩膜，支持数据有限的地球观测任务中的实时决策。
...
TL;DR: Oitijjo-3D是一个免费生成式AI框架，利用Google街景图像重建文化遗产3D模型，解决了孟加拉国文化遗产修复面临的资源和技术限制问题。


<details>
  <summary>Details</summary>
Motivation: 孟加拉国文化遗产修复面临资源有限和技术专家稀缺的双重挑战，传统3D数字化方法成本高、需要专业设备，导致许多建筑瑰宝面临衰败风险且无法数字化保存。

Method: 采用两阶段流程：使用Gemini 2.5 Flash Image进行多模态视觉推理实现结构-纹理合成，通过Hexagen进行神经图像到3D生成实现几何恢复。

Result: 系统在数秒内生成逼真、度量一致的重建结果，相比传统运动恢复结构流程显著提速，且无需专业硬件或专家监督。在Ahsan Manzil等标志性建筑上的实验证明其保持了视觉和结构保真度。
...
TL;DR: 提出基于强化学习的视频时刻检索模型，通过多智能体系统和证据学习解决不同模型定位结果的冲突问题，无需额外训练即可判断查询是否超出视频范围。


<details>
  <summary>Details</summary>
Motivation: 当前视频时刻检索方法未考虑不同模型定位结果的冲突问题，导致多种模型无法有效整合以获得更好结果。

Method: 使用强化学习模型一次性扫描整个视频找到时刻边界并生成位置证据，提出基于证据学习的多智能体系统框架解决智能体定位输出的冲突。

Result: 在基准数据集上的广泛实验表明，所提方法相比最先进方法具有有效性，多智能体系统的竞争与冲突建模能有效提升强化学习在时刻检索中的性能。

Conclusion: 多智能体系统的竞争与冲突建模是提升强化学习在时刻检索中性能的有效方法，证据学习在多智能体框架中发挥了新作用。
...
TL;DR: VisionCAD是一个基于视觉的放射学辅助框架，通过摄像头直接从显示器捕获医学图像，绕过了与医院IT系统集成的障碍，实现AI辅助诊断。


<details>
  <summary>Details</summary>
Motivation: 传统计算机辅助诊断系统难以与现有医院IT基础设施集成，阻碍了其广泛临床应用。VisionCAD旨在通过视觉方式解决这一集成难题。

Method: 采用自动化流程检测、恢复和分析屏幕上的医学图像，将摄像头捕获的视觉数据转换为适合自动分析和报告生成的诊断质量图像。模块化架构可灵活利用最先进的诊断模型。
...
TL;DR: FERBench系统评估了20个最先进的多模态大语言模型在面部表情识别任务上的表现，发现MLLMs在分类性能上表现良好，但在推理和可解释性方面存在显著局限。为此提出了UniFER-7B统一模型，通过后训练策略显著提升了面部表情推理能力。


<details>
  <summary>Details</summary>
Motivation: 尽管多模态大语言模型在多个领域取得突破，但在面部表情识别任务上的性能尚未得到系统评估。现有方法多为分散的领域特定模型，缺乏统一的解决方案。

Method: 构建FERBench基准测试，评估20个SOTA MLLMs在4个广泛使用的FER数据集上的表现。开发后训练策略，包括使用UniFER-CoT-230K进行冷启动初始化和UniFER-RLVR-360K进行可验证奖励的强化学习。

Result: MLLMs在FER分类任务上表现良好，但在推理和可解释性方面存在局限。UniFER-7B模型在性能上超越了多个开源和闭源通用MLLMs，包括Gemini-2.5-Pro和Qwen2.5-VL-72B。

Conclusion: 通过系统基准测试和专门的后训练策略，可以显著提升MLLMs在面部表情识别任务上的推理能力和可解释性，UniFER-7B为FER任务提供了一个统一且可解释的基础模型解决方案。
...
TL;DR: VinciCoder是一个统一的多模态代码生成模型，通过两阶段训练框架实现，包括大规模监督微调和视觉强化学习策略，在多个多模态代码生成基准测试中达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型在专门任务上表现良好，但依赖单任务训练限制了通用视觉代码智能的发展，需要更统一的解决方案。

Method: 采用两阶段训练框架：1）构建160万图像-代码对的监督微调语料库；2）引入视觉强化学习策略，使用从粗到细的奖励机制计算局部和全局图像块的视觉相似度。

Result: 在多个多模态代码生成基准测试中达到最先进性能，证明了从粗到细视觉强化学习策略的有效性。
...
TL;DR: 提出了首个统一框架，通过将SOD、CoSOD和SIS三个异质显著性任务转化为视觉语言模型中的思维链推理过程，使用两阶段训练范式（监督微调+强化学习）和置信度引导策略优化方法，实现了多任务统一处理。


<details>
  <summary>Details</summary>
Motivation: 解决操作上异质的显著性任务（SOD、CoSOD、SIS）的统一处理问题，通过思维链推理过程来弥合任务异质性。

Method: 采用两阶段训练范式：监督微调（SFT）和强化学习（RL）。提出置信度引导策略优化（CGPO）算法，利用奖励与模型置信度之间的差异作为单样本优势信号，并引入"输出到推理"策略构建高保真SFT数据。

Result: 模型在所有任务上匹配或超越了专门的SOTA方法和强大的闭源VLM，特别是在CoSOD任务的CoCA数据集上达到0.899的S-measure，比之前最佳方法提升了8.0个百分点，且使用了更少的训练数据。

Conclusion: 提出的统一框架通过思维链推理和置信度引导策略优化，成功解决了异质显著性任务的统一处理问题，在多个任务上取得了优越性能。
...
<details>
  <summary>Details</summary>
Motivation: 由于CLIP模型对图像裁剪敏感，随机裁剪可能引入错误信息和偏见，因为许多图像在小尺度上具有相似特征。

Method: 提出局部-全局交叉对齐(LGCA)框架：先捕获图像局部特征，然后重复选择最显著区域并扩展，设计相似度分数同时考虑原始图像和扩展图像。

Result: 在多个数据集上的广泛实验表明，该方法显著提高了零样本性能，优于最先进的基线方法。

Conclusion: LGCA框架能有效捕获局部和全局特征，同时最小化错误信息，且时间复杂度与原始模型相同，具有高效性和可扩展性。
...
TL;DR: 提出了一种基于图像-文本对齐度的生成图像检测方法ITEM，通过检测生成图像与描述文本在CLIP空间中的语义对齐差异来识别伪造图像。


<details>
  <summary>Details</summary>
Motivation: 现有方法仅关注视觉线索，容易过拟合特定图像模式且无法泛化到未见过的生成模型。发现生成图像与对应描述文本的语义对齐度较差，可作为有效检测线索。

Method: 在预训练CLIP空间中测量图像与文本的对齐度，然后训练MLP头进行分类。提出分层对齐方案，先关注整体图像再关注描述中的每个语义对象，探索全局和局部语义对齐差异。

Result: 在多种最新生成模型上的实验表明，该方法优于其他最先进方法，具有出色的泛化能力和鲁棒性。

Conclusion: 利用图像-文本语义对齐差异作为检测线索是有效的，该方法简单但性能优越，能很好地泛化到不同生成模型。
...
Method: 观察到扩散生成图像与自然图像在低频到高频波段存在渐进差异，提出频率选择性函数作为加权滤波器，抑制判别性较弱的频段，增强信息量更大的频段。

Result: 在多个扩散生成图像数据集上的广泛实验表明，该方法优于现有最先进的检测器，具有更好的泛化性和鲁棒性。

Conclusion: 基于频率伪造线索的方法能够有效检测来自未见扩散模型的图像，并对各种扰动具有稳健的抵抗能力。
...
TL;DR: 提出了ToxicTextCLIP框架，通过生成高质量对抗文本来攻击CLIP预训练阶段，解决了语义错位和背景一致文本稀缺的问题，在分类和检索任务中达到高成功率并能绕过现有防御。


<details>
  <summary>Details</summary>
Motivation: CLIP模型依赖未筛选的互联网数据，面临数据投毒和后门风险。现有研究主要关注图像攻击，而文本模态作为CLIP训练的核心部分尚未充分探索。

Method: ToxicTextCLIP框架迭代应用：1）背景感知选择器，优先选择与目标类别背景一致文本；2）背景驱动增强器，生成语义连贯且多样化的中毒样本。

Result: 在分类和检索任务中达到95.83%的中毒成功率和98.68%的后门Hit@1，并能绕过RoCLIP、CleanCLIP和SafeCLIP防御。

Conclusion: ToxicTextCLIP证明了文本模态在CLIP攻击中的有效性，揭示了现有防御机制的局限性，为更安全的视觉语言模型开发提供了重要启示。
...
TL;DR: 提出基于Grad-CAM的弱监督深度学习框架，用于胸部X光肺炎分类和定位，使用图像级标签而非像素级标注，在Kermany数据集上达到98%准确率。


<details>
  <summary>Details</summary>
Motivation: 解决医学影像分析中像素级标注成本高的问题，开发可解释的弱监督模型来提升肺炎筛查的透明度和临床信任度。
...
Result: ResNet-18和EfficientNet-B0表现最佳，测试准确率98%，ROC-AUC=0.997，F1=0.987；MobileNet-V2在准确率和计算成本间达到最优平衡；Grad-CAM可视化确认模型聚焦临床相关肺部区域。

Conclusion: 弱监督可解释模型在肺炎筛查中具有潜力，能增强AI辅助医学影像的透明度和临床信任，为放射学诊断提供有效工具。
...
TL;DR: HumanCrafter是一个统一框架，能够从单张图像前馈式联合建模外观和人体部位语义，在3D人体部位分割和重建任务上超越现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有生成模型在3D人体重建方面取得高保真度，但在特定任务（如人体3D分割）中的实用性仍然受限。

Method: 在重建阶段整合人体几何先验，在分割阶段整合自监督语义先验；开发交互式标注流程生成高质量数据标签对；通过像素对齐聚合实现跨任务协同，多任务目标同时优化纹理建模保真度和语义一致性。

Result: 大量实验表明，HumanCrafter在3D人体部位分割和单图像3D人体重建方面均超越了现有最先进方法。

Conclusion: HumanCrafter框架成功实现了外观和人体部位语义的联合建模，解决了标记3D人体数据集稀缺的问题，并在两个任务上都取得了优越性能。
...
TL;DR: 本文提出了一种基于深度学习的迭代分割和质量优化框架，用于MRI中前庭神经鞘瘤的自动分割，通过多中心数据和专家共识构建标注数据集，显著提高了分割精度和效率。


<details>
  <summary>Details</summary>
Motivation: 前庭神经鞘瘤的MRI精确分割对患者管理至关重要，但传统手动标注耗时且依赖专家。现有深度学习方法在多样化数据集和复杂临床案例中仍面临鲁棒性挑战。

Method: 采用自举式深度学习框架进行迭代分割和质量优化，结合多中心数据并依赖专家共识确保标注可信度，构建包含190名患者、534个纵向对比增强T1加权扫描的公开数据集。

Result: 在内部验证数据集上，Dice相似系数从0.9125显著提升至0.9670，在代表性外部数据集上保持稳定性能。专家评估143个扫描发现需要专家干预的细微案例。相比传统手动标注，效率提升约37.4%。

Conclusion: 人机协作模型训练方法实现了高分割精度，展示了其作为临床适应性强、可泛化策略的潜力，适用于多样化临床环境中的前庭神经鞘瘤自动分割。
...
TL;DR: FedMGP是一种个性化的联邦提示学习方法，通过多组文本和视觉提示来捕捉细粒度语义信息，采用基于相似度的动态提示聚合策略，在保持参数效率的同时实现最优性能。


<details>
  <summary>Details</summary>
Motivation: 解决联邦学习中个性化需求与全局知识共享的平衡问题，同时提升视觉语言模型在联邦设置下的表现。

Method: 为每个客户端配备多组文本和视觉提示对，引入多样性损失使每组提示专注于不同语义方面，采用基于余弦相似度的概率采样动态聚合策略。

Result: 在多个联邦视觉语言基准测试中，FedMGP在个性化和领域泛化方面均优于现有方法，且通信参数最少。

Conclusion: FedMGP通过多组提示和动态聚合策略有效平衡了全局共享知识和客户端特定特征，为联邦提示学习提供了新的有效范式。
...
<details>
  <summary>Details</summary>
Motivation: 统一视频扩散模型的生成先验与从大规模4D数据集学习到的几何和运动约束，实现高效的单图像到4D场景生成。
...
Conclusion: Diff4Splat提供了一种高效的单图像到4D场景生成方法，在保持质量的同时大幅提升了合成速度。
...
TL;DR: VinDr-CXR-VQA是一个大规模胸部X光视觉问答数据集，包含17,597个问答对和4,394张图像，具有放射科医生验证的边界框和临床推理解释，旨在推进可解释的医学视觉问答研究。


<details>
  <summary>Details</summary>
Motivation: 构建一个具有空间定位和临床推理解释的大规模医学视觉问答数据集，以解决现有Med-VQA数据集中存在的幻觉问题和缺乏可解释性的挑战。

Method: 创建包含六种诊断类型（位置、内容、存在性、数量、选择和是/否）的问题分类法，构建平衡的正负样本分布（41.7%阳性，58.3%阴性），并使用MedGemma-4B-it模型进行基准测试。

Result: 在MedGemma-4B-it模型上实现了F1分数0.624，比基线提高了11.8%，同时能够实现病灶定位。
...
TL;DR: OmniTrack++是一个用于全景图像多目标跟踪的反馈驱动框架，通过动态特征稳定、轨迹引导的灵活定位和专家记忆设计，解决了360度视野下的失真、大搜索空间和身份模糊问题，在JRDB和EmboTrack基准上实现了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 传统多目标跟踪方法在全景图像中表现不佳，因为360度视野带来了分辨率稀释、严重视角相关失真、大搜索空间和身份模糊等独特挑战。

Method: 采用反馈驱动框架：1) DynamicSSM块稳定全景特征缓解几何失真；2) FlexiTrack实例使用轨迹反馈进行灵活定位和短期关联；3) ExpertTrack记忆通过混合专家设计整合外观线索；4) Tracklet管理模块根据场景动态自适应切换跟踪模式。

Result: 在JRDB和EmboTrack基准上实现最先进性能，相比原始OmniTrack在JRDB上HOTA提升25.5%，在QuadTrack上提升43.07%。

Conclusion: OmniTrack++通过渐进式感知细化有效解决了全景多目标跟踪的挑战，提出的EmboTrack基准为全景感知提供了具有挑战性的测试平台。
...
TL;DR: ID-Composer是一个新颖的多主体视频生成框架，通过文本提示和参考图像生成多主体视频，解决了现有模型在身份保持、语义整合和时间一致性方面的挑战。


<details>
  <summary>Details</summary>
Motivation: 现有视频生成模型通常仅基于文本或单张图像生成视频，限制了可控性和应用范围。需要解决多主体视频生成中身份保持、跨主体语义整合和时间一致性的挑战。

Method: 设计了分层身份保持注意力机制来聚合主体内和跨主体特征；利用预训练视觉语言模型提供细粒度语义指导；采用在线强化学习阶段来优化关键概念对齐。

Result: 大量实验表明，ID-Composer在身份保持、时间一致性和视频质量方面优于现有方法。

Conclusion: ID-Composer通过创新的分层注意力机制、VLM语义理解和强化学习训练，有效解决了多主体视频生成中的关键挑战，实现了高质量的身份保持视频生成。
...
<details>
  <summary>Details</summary>
Motivation: 现有的视觉语言模型如CLIP在零样本分类中表现优异，但容易受到虚假相关性的影响。现有的去偏方法通常需要训练数据和明确的组标签，限制了实际应用。测试时方法虽然避免了这个限制，但很多仍然依赖数据集特定偏置的先验知识，限制了在开放集设置中的泛化能力。

Method: 使用预训练分割模型隔离目标视觉属性，然后调整非目标区域，使其嵌入与所有类别特定文本提示均匀相似。这种方法在保留目标属性的同时，从混淆视觉区域中移除意外的偏置信号。
...
<details>
  <summary>Details</summary>
Motivation: 传统视频异常检测方法输出有限（仅正常或异常），且多为半自动化需要人工评估，需要提升检测的粒度和交互性。
...
Conclusion: T-VAD框架显著提升了异常检测的粒度和交互性，实现了细粒度的视频异常检测和定位。
...
TL;DR: 提出了Real-IAD Variety，这是最大最全面的工业异常检测基准数据集，包含198,960张高分辨率图像，涵盖160个物体类别、28个行业、24种材料类型和22种颜色变化。
...
Method: 构建了大规模多样化的数据集，并在多类无监督、多视角和零/少样本设置下进行严格评估。

Result: 实验显示，最先进的多类无监督异常检测方法在类别从30扩展到160时性能显著下降，而视觉语言模型表现出对类别扩展的强鲁棒性。

Conclusion: Real-IAD Variety的规模和复杂性使其成为训练和评估下一代异常检测基础模型的重要资源，将加速可扩展通用异常检测系统的开发。
...
Conclusion: 该方法有效解决了单图像多实例语义学习和合成的挑战，特别是在处理相似语义实例时表现出色。
...
TL;DR: 4D-NVS结合体素表示和神经高斯泼溅，通过紧凑的神经体素和变形场建模动态场景，大幅减少内存消耗和训练时间，同时保持高质量渲染。


<details>
  <summary>Details</summary>
Motivation: 3D高斯泼溅在动态场景中因每帧复制高斯函数导致内存开销巨大，需要更高效的动态场景建模方法。

Method: 使用紧凑的神经体素集合和学习的变形场来建模时间动态，避免为每个时间戳生成独立的高斯集合；引入视图细化阶段选择性优化困难视角。

Result: 实验表明该方法在显著减少内存和加速训练的同时，超越现有方法，实现实时渲染和优越的视觉保真度。
...
TL;DR: 提出了一个频率引导的广义类别发现框架(FREE)，通过利用频域信息来解决域偏移下的广义类别发现问题，在未知域中有效发现已知和未知类别。
...
Method: 提出了频率引导的广义类别发现框架(FREE)，包括：频率域域分离策略、跨域和域内频率扰动策略、扩展的自监督对比目标和语义聚类损失、以及聚类难度感知的重采样技术。

Result: 大量实验表明，该方法在各种基准数据集上有效缓解了分布偏移的影响，在发现已知和未知类别方面都取得了优越性能。

Conclusion: FREE框架通过利用频域信息成功解决了域偏移下的广义类别发现问题，为处理现实世界中分布偏移的类别发现任务提供了有效解决方案。
...
TL;DR: 提出了一种基于记忆增强的上下文感知零样本异常检测方法，通过跨注意力机制融合时序和视觉特征，结合上下文相似性评分实现实时零样本异常分类。


<details>
  <summary>Details</summary>
Motivation: 视频异常通常依赖于上下文信息和时间演化，但大多数异常检测器未能考虑这种上下文依赖性，限制了它们在实际场景中的泛化能力。

Method: 采用记忆增强管道，通过跨注意力机制关联时序信号与视觉嵌入，并使用上下文相似性评分进行实时零样本异常分类。

Result: 在UCF-Crime数据集上达到90.4% AUC，在XD-Violence数据集上达到83.67% AP，创下了零样本模型的新SOTA。
...
TL;DR: CueBench是首个专注于上下文感知视频异常理解的基准测试，通过统一评估框架和事件中心层次分类法，对14种条件异常和18种绝对异常事件进行系统评估。


<details>
  <summary>Details</summary>
Motivation: 现有深度模型对真实世界视频异常的理解仍停留在表面，缺乏对复杂原理和微妙上下文的理解能力，如使用安全装备攀岩与不使用安全装备的区别。

Method: 提出CueBench基准测试，建立事件中心层次分类法，涵盖174个场景和198个属性；开发Cue-R1方法，基于R1风格强化微调，使用可验证、任务对齐和层次细化奖励的统一生成方式。

Result: 在CueBench上的广泛实验显示，现有视觉语言模型对真实世界异常理解仍不理想，而Cue-R1方法平均超越最先进方法超过24%。

Conclusion: 真实世界的视频异常理解仍面临挑战，CueBench为评估上下文感知异常理解提供了系统框架，Cue-R1方法显著提升了性能。
...
TL;DR: 提出了一种新的手术动作三元组分割任务，通过结合器械实例分割来空间定位<器械、动作、目标>三元组，并提出了TargetFusionNet架构和CholecTriplet-Seg数据集。


<details>
  <summary>Details</summary>
Motivation: 现有手术动作识别方法仅限于帧级分类，无法可靠地将动作与特定器械实例关联，且基于类激活图的空间定位方法缺乏精确性和鲁棒性。

Method: 提出了三元组分割任务，创建了CholecTriplet-Seg数据集，并设计了TargetFusionNet架构，该架构扩展Mask2Former，通过目标感知融合机制将弱解剖先验与器械实例查询融合。

Result: 在识别、检测和三元组分割指标上，TargetFusionNet始终优于现有基线方法，证明强实例监督与弱目标先验相结合能显著提高手术动作理解的准确性和鲁棒性。
...
TL;DR: 该研究介绍了首个大规模多光谱激光雷达基准数据集FGI-EMIT，用于单木分割。通过比较传统无监督算法和深度学习方法的性能，发现深度学习显著优于传统方法，特别是在下层植被分割方面。


<details>
  <summary>Details</summary>
Motivation: 单木分割在林业应用中至关重要，但缺乏大规模多光谱激光雷达基准数据集限制了方法发展。多光谱反射率已被证明能提高分割精度，但相关数据仍然稀缺。

Method: 创建了包含1,561棵人工标注树木的FGI-EMIT数据集（532、905、1550nm波长）。比较了4种传统无监督算法和4种深度学习方法的性能，使用贝叶斯方法优化无监督算法参数，深度学习模型从头训练。

Result: 无监督方法中Treeiso表现最佳（F1-score 52.7%），而深度学习ForestFormer3D达到73.3%的F1-score。在下层植被方面，ForestFormer3D比Treeiso高出25.9个百分点。深度学习在低点密度（10点/平方米）下仍保持优势。

Conclusion: 深度学习在单木分割任务中显著优于传统方法，但当前方法未能充分利用多光谱反射率信息。单通道反射率可略微提高精度，特别是对下层植被。
...
TL;DR: MR-CLIP是一个元数据引导的框架，通过将MRI体积图像与其DICOM采集参数对齐来学习MRI对比度表示，无需人工标注即可实现序列识别、协调和质量控制。


<details>
  <summary>Details</summary>
Motivation: MRI存在严重的数据异质性和缺乏标准化对比度标签的问题，限制了大规模自动化分析。需要统一的MRI对比度表示来支持下游应用。

Method: 引入MR-CLIP框架，通过将体积图像与DICOM采集参数对齐来学习MRI对比度表示，利用常规可用的采集元数据作为监督信号。

Result: 生成的嵌入显示MRI序列的明显聚类，在少样本序列分类中优于监督3D基线，并能通过图像-元数据嵌入距离识别损坏或不一致的元数据。

Conclusion: MR-CLIP为跨不同临床数据集的标签高效MRI分析提供了可扩展的基础，通过利用常规采集元数据实现了无需人工标注的MRI分析。
...
TL;DR: 提出一种针对图像超分辨率网络的后训练量化方法，通过双区域量化策略和敏感度感知微调，有效处理激活值中的异常值问题，在保持性能的同时显著加速推理。


<details>
  <summary>Details</summary>
Motivation: 现有后训练量化方法在处理图像超分辨率网络时，由于忽略了激活值中的异常值影响，导致性能不理想。研究发现这些异常值与图像颜色信息密切相关，直接移除会导致显著性能下降。

Method: 1. 双区域量化策略：将激活值划分为异常值区域和密集区域，分别应用均匀量化以优化比特分配；2. 敏感度感知微调：根据网络层对量化的敏感度差异，重点关注高敏感层以提升量化性能。

Result: 在多种超分辨率网络和数据集上的实验表明，该方法优于现有后训练量化方法，在大多数场景下达到与量化感知训练相当的性能，同时实现至少75倍的加速。

Conclusion: 所提出的双区域量化策略和敏感度感知微调有效解决了图像超分辨率网络后训练量化中的异常值问题，在保持高质量输出的同时显著提升推理速度。
...
TL;DR: WANDER是一种基于新颖性搜索的方法，使用LLM进行语义演化，通过CLIP嵌入量化新颖性，并用发射器引导搜索到不同的提示空间区域，显著提升了图像生成的多样性。


<details>
  <summary>Details</summary>
Motivation: 文本到图像扩散模型虽然能生成高质量图像，但输出多样性有限，限制了在探索性和构思任务中的应用。现有的提示优化技术通常针对美学适应性，或不适合创意视觉领域。

Method: WANDER直接在自然语言提示上操作，使用大型语言模型进行语义演化，CLIP嵌入量化新颖性，并应用发射器引导搜索到不同的提示空间区域。

Result: 使用FLUX-DEV生成和GPT-4o-mini突变的实证评估表明，WANDER在多样性指标上显著优于现有的进化提示优化基线。消融研究证实了发射器的有效性。

Conclusion: WANDER通过新颖性搜索和发射器机制，有效解决了文本到图像模型的多样性限制问题，为创意应用提供了更好的工具。
...
<details>
  <summary>Details</summary>
Motivation: 低剂量CT成像广泛用于减少辐射暴露，但常受噪声和伪影影响诊断准确性。虽然深度学习模型在PSNR和SSIM指标上表现优异，但这些指标在反映医学图像感知质量方面存在局限。

Method: 对基于深度学习的低剂量CT图像增强架构中的损失函数进行客观分析，评估不同损失函数与图像质量指标的相关性和一致性。

Result: 研究发现损失函数与质量指标之间存在不一致性，揭示了在开发图像质量增强的新损失函数时需要考虑图像质量指标的必要性。

Conclusion: 论文强调在开发用于图像质量增强的新损失函数时，必须充分考虑图像质量指标，以确保模型性能与感知质量的一致性。
...
TL;DR: 深度学习模型在ADNI数据集上对阿尔茨海默病诊断表现优异（AUC达0.96-0.97），但在拉丁美洲FLENI队列上性能显著下降（AUC降至0.80-0.82），揭示明显的领域偏移问题。


<details>
  <summary>Details</summary>
Motivation: 探索深度学习模型在阿尔茨海默病诊断中的泛化能力，特别是在代表性不足的拉丁美洲人群中的表现，验证模型在不同人群间的适用性。

Method: 在ADNI数据集上训练卷积和Transformer模型，然后在FLENI拉丁美洲临床队列上进行泛化性能评估，通过消融研究和遮挡敏感性分析识别关键因素。

Result: 所有模型在ADNI上表现优异，但在FLENI上性能显著下降，显示明显的领域偏移。Transformer并未表现出明显优势，图像归一化和正确采样选择是影响泛化的关键因素。

Conclusion: 诊断AI模型需要进行人群感知验证，未来工作需要关注领域适应和队列多样化，以确保模型在不同人群中的可靠应用。
...
<details>
  <summary>Details</summary>
Motivation: 现有地点识别方法大多依赖对比学习，本文探索将地点识别作为多类分类问题的替代方法，以提高训练效率和稳定性。
...
TL;DR: 研究发现生成式AI模型在编码'美'概念时存在系统性偏见，86.5%生成图像为浅肤色人群，74%为年轻群体，非二元性别个体被过度年轻化和性化，'负面'美特征提示会产生更多NSFW内容。


<details>
  <summary>Details</summary>
Motivation: 社交媒体加剧了西方美标准传播，导致负面自我形象和身体畸形恐惧症。生成式AI可能夸大这些标准，本研究旨在探索AI如何编码'美'并消除'丑'及其社会影响。

Method: 创建两个图像生成流程：文本到图像模型和文本到语言模型到图像模型。开发结构化美分类法，使用三个语言模型和两个文本到图像模型生成5984张图像，招募女性和非二元社交媒体用户通过李克特量表评估1200张图像。

Result: 86.5%生成图像为浅肤色人群，22%包含明确内容（尽管经过SFW训练），74%被评为年轻年龄段。非二元个体图像被评价为更年轻和过度性化。带有'负面'美特征的提示始终产生更高NSFW评分。

Conclusion: 生成式AI模型存在与美标准相关的普遍人口统计偏见，这些偏见通过模型开发者（如负向提示）被积极延续，导致数据流污染和对不符合开发者美刻板印象特征的积极消除。
...
TL;DR: 提出了一种基于物联网的榴莲园动物入侵检测系统，集成YOLOv5和SSD算法提高检测精度，通过Telegram实时通知农民，并触发声音威慑机制。


<details>
  <summary>Details</summary>
Motivation: 传统农业实践因缺乏无人干预监控而无效，现有系统依赖单一检测算法、通知平台不便捷、威慑机制有限，需要更全面的解决方案来保护榴莲作物免受动物入侵。

Method: 集成YOLOv5和SSD对象检测算法，结合物联网技术实现实时监控，检测到入侵时自动发送Telegram通知，并触发老虎吼叫等声音威慑机制。
...
TL;DR: 提出一种粒度一致的自劢2D掩码跟踪方法，通过保持跨帧的时间对应关系来消除冲突的伪标签，结合三阶段课程学习框架，从碎片化的单视图数据逐步训练到统一的多视图标注，实现全局一致的3D实例分割。
...
Method: 引入粒度一致的自劢2D掩码跟踪方法，保持跨帧的时间对应关系；采用三阶段课程学习框架，从单视图数据逐步训练到多视图标注和全局场景监督。

Result: 实验结果表明，该方法能有效生成一致且准确的3D分割，在标准基准测试中达到了最先进的结果，并具备开放词汇能力。

Conclusion: 通过结构化学习流程，能够从最初碎片化和矛盾的2D先验中稳健地提取一致的3D表示，实现了高质量的3D实例分割。
...
<details>
  <summary>Details</summary>
Motivation: 联邦学习系统在隐私敏感环境中具有重要价值，但仍面临成员推断攻击和数据异构性等安全漏洞，需要建立标准化评估平台。

Method: 开发FedOnco-Bench基准测试平台，使用合成肿瘤CT扫描数据，评估FedAvg、FedProx、FedBN和FedAvg+DP-SGD等联邦学习方法在医学图像分割中的表现。

Result: FedAvg性能最高（Dice约0.85）但隐私泄露最多（攻击AUC约0.72），DP-SGD隐私保护最好（AUC约0.25）但准确率较低（Dice约0.79），FedProx和FedBN在异构数据下表现均衡。

Conclusion: FedOnco-Bench为医学图像分割中的隐私保护联邦学习方法提供了标准化的开源基准测试平台，有助于方法开发和评估。
...
TL;DR: 提出了Med-Banana-50K，一个包含50K图像的医学图像编辑数据集，涵盖三种模态和23种疾病类型，通过Gemini-2.5-Flash-Image生成双向编辑，并采用医学质量控制和迭代优化。


<details>
  <summary>Details</summary>
Motivation: 当前医学图像编辑研究受限于缺乏大规模、高质量、开放可访问的数据集，特别是在严格的解剖学和临床约束下。

Method: 利用Gemini-2.5-Flash-Image从真实医学图像生成双向编辑（病变添加和移除），采用LLM-as-Judge进行医学质量评估和最多五轮的迭代优化。

Result: 构建了包含50K图像的数据集，涵盖三种医学成像模态和23种疾病类型，还包括37K失败案例用于偏好学习。

Conclusion: Med-Banana-50K为训练和评估下一代医学图像编辑模型奠定了基础，数据集和代码已公开。
...
TL;DR: GUI-AIMA是一个基于注意力的坐标无关GUI定位框架，通过轻量级训练触发MLLMs的本地定位能力，在3B模型上达到SOTA性能


<details>
  <summary>Details</summary>
Motivation: 现有基于MLLMs的GUI定位方法直接生成精确坐标具有挑战性且计算量大，需要更高效的方法

Method: 提出基于注意力的监督微调框架，将MLLMs的多模态注意力与补丁级定位信号对齐，采用坐标无关方式并可集成放大阶段

Result: 仅用85k截图训练，在ScreenSpot-Pro上达到58.6%平均准确率，在OSWorld-G上达到62.2%平均准确率

Conclusion: 轻量级训练可以有效触发MLLMs的本地定位能力，GUI-AIMA在数据效率和性能方面表现出色
...
Method: 结合拓扑感知扩散概率模型和水平集能量，通过四个互补项整合输入图像和深度特征来引导隐式曲线演化。引入像素自适应细化模块，使用相邻证据的亲和权重局部调制能量函数。

Result: 在四个公共胰腺数据集上的评估显示，TA-LSDiff实现了最先进的准确度，优于现有方法。消融研究系统量化了每个组件的贡献。

Conclusion: TA-LSDiff为胰腺分割提供了一个实用且准确的解决方案，在精度上超越了现有方法。
...
TL;DR: OMEGA是一个新颖的位置编码框架，通过模态特定位置编码和全局自适应编码步长缩放，提升视觉语言模型的性能。


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型采用统一的位置索引策略，没有考虑文本和视觉模态在结构特性和连续性上的差异。

Method: 使用模态特定位置编码为不同模态分配位置索引，并引入全局自适应编码步长缩放来调整视觉token的位置编码步长。

Result: 在多种架构和VQA基准测试中，OMEGA持续提升VLM性能，在视觉密集型任务上比基线位置编码策略提升达3.43%。

Conclusion: OMEGA通过考虑模态特定结构和信息密度，有效提升了视觉语言模型的性能表现。
...
<details>
  <summary>Details</summary>
Motivation: 现有的多模态对抗攻击方法过度依赖单一模态的对抗样本信息，缺乏输入多样性，导致过拟合问题，难以在不同视觉语言预训练模型和下游任务中保持高迁移性。
...
Conclusion: LSSA通过引入输入多样性和采样策略，有效解决了多模态对抗攻击中的过拟合问题，显著提升了对抗样本的跨模型迁移能力。
...
TL;DR: 提出了Visual-Contrast Attention (VCA)作为MHSA的替代方案，通过视觉对比机制降低计算复杂度，在图像识别和生成任务中均取得显著提升


<details>
  <summary>Details</summary>
Motivation: 传统ViT的MHSA层对所有token对进行二次方复杂度计算，但大部分计算花费在视觉上弱相关或冗余的关联上，需要更高效的注意力机制

Method: VCA首先将每个头的密集查询场蒸馏为少量空间池化的视觉对比token，然后将其分为可学习的正负流，通过差分交互突出区域间的真正差异

Result: 在DeiT-Tiny上ImageNet-1K准确率从72.2%提升至75.6%(+3.4%)，在层次化ViT上提升达3.1%，在图像生成任务中FID-50K降低2.1-5.2点

Conclusion: VCA为构建更快更锐利的Vision Transformers提供了一条简单路径，理论复杂度从O(N²C)降至O(NnC)，仅增加不到0.3M参数且无需额外FLOPs
...
Conclusion: PIAT通过参数插值和NMSE有效缓解了对抗训练中的振荡和过拟合问题，实现了更高的模型鲁棒性。
...
<details>
  <summary>Details</summary>
Motivation: 当前脑成像视觉问答基准要么覆盖少数成像模态，要么局限于粗粒度病理描述，无法全面评估MLLMs在整个临床连续体中的表现。

Method: 构建包含15种脑成像模态的综合多模态VQA基准，模拟临床工作流程，包含9,527个验证问答对和31,706张图像，涵盖15个多阶段临床任务。
...
Method: 使用遮挡感知扩散变换器架构在去噪阶段估计与遮挡模式相关的噪声特征，并引入遮挡掩码引导的反向过程来有效利用观测信息。
...
TL;DR: 提出了一种名为LMD的后处理、模型无关的可解释性方法，用于解耦自动驾驶感知模型中多模态信息的贡献。


<details>
  <summary>Details</summary>
Motivation: 在自动驾驶中，感知模型的决策透明度至关重要，因为即使单个误判也可能导致灾难性后果。但在多传感器输入下，由于传感器信息在融合网络中纠缠，难以确定每种模态对预测的贡献。

Method: LMD方法通过层间模态分解，在预训练的融合模型的所有层中解耦模态特定信息，能够将感知模型的预测归因于单个输入模态。

Result: 在相机-雷达、相机-LiDAR和相机-雷达-LiDAR三种自动驾驶设置下评估LMD，通过结构化扰动指标和模态可视化分解验证了其有效性。

Conclusion: LMD是首个能够将传感器融合系统中感知模型预测归因于单个输入模态的方法，展示了在高容量多模态架构解释方面的实际适用性。
...
<details>
  <summary>Details</summary>
Motivation: 传统检索方法受限于数据库覆盖范围和质量，而现有的大视觉语言模型在复杂地理场景和多样化区域中存在困难。现有的多智能体系统缺乏有效处理冲突预测的机制。

Method: 提出GraphGeo框架，使用异构图神经网络建模多样化的辩论关系（支持性协作、竞争性论证和知识转移），引入双级辩论机制（节点级精炼和边级论证建模），以及跨级拓扑精炼策略。

Result: 在多个基准测试上的实验表明，GraphGeo显著优于现有最先进方法。
...
TL;DR: Fleming-VL是一个统一的端到端框架，用于解决医学多模态大语言模型在处理异构医学数据（2D图像、3D体积扫描、时间视频序列）时的挑战，通过数据中心的策略实现跨模态的医学视觉理解。


<details>
  <summary>Details</summary>
Motivation: 医学数据具有异构性，包含2D图像、3D体积扫描和时间视频序列等多种模态，这些模态之间存在显著的领域差距和数据格式不一致，阻碍了统一医学MLLMs的发展。

Method: 采用数据中心的三个关键策略：1）通过整合自然和医学特定领域的长上下文数据进行预训练扩展；2）用罕见医学数据补充微调，包括整体视频分析和代表性不足的2D模态；3）扩展现有评估框架以纳入3D体积和视频理解基准。通过监督微调和组相对策略优化开发多个模型规模。

Result: Fleming-VL在多个基准测试中实现了最先进的性能，包括医学VQA、视频QA和3D医学图像理解。

Conclusion: Fleming-VL通过统一框架成功解决了医学异构数据的挑战，推动了医学AI的透明、可复现和可审计进展。
...
TL;DR: 提出动态多级加权对齐网络解决零样本草图-图像检索中的模态样本不平衡和低质量信息问题，在三个基准数据集上取得SOTA性能


<details>
  <summary>Details</summary>
Motivation: 解决零样本草图-图像检索中存在的模态样本不平衡和训练过程中不一致的低质量信息问题，这些因素导致现有方法性能不佳

Method: 包含三个组件：单模态特征提取模块（CLIP文本编码器和ViT）、跨模态多级加权模块（局部和全局聚合块生成对齐权重列表）、加权四元组损失模块（改进三元组损失的域平衡）

Result: 在Sketchy、TU-Berlin和QuickDraw三个基准数据集上的实验表明，该方法优于现有的最先进ZS-SBIR方法

Conclusion: 提出的动态多级加权对齐网络有效解决了ZS-SBIR中的模态不平衡问题，显著提升了检索性能
...
TL;DR: EVTAR是一个端到端的虚拟试衣模型，通过引入额外参考图像直接拟合目标服装到人物图像上，无需复杂输入如遮罩、密集姿态或分割图，提高了试衣精度和实用性。


<details>
  <summary>Details</summary>
Motivation: 现有虚拟试衣方法依赖复杂输入（如不可知人物图像、人体姿态、密集姿态或关键点），导致操作繁琐且不适用于实际应用。EVTAR旨在简化输入要求，仅需源图像和目标服装，同时通过参考图像提升试衣质量。

Method: 采用两阶段训练策略，在推理时仅需源图像和目标服装。利用不同人物穿着同一服装的参考图像来保持服装纹理和细节，模拟人类选择服装时的参考行为。

Result: 在两个广泛使用的基准测试和多样化任务上评估，结果一致验证了方法的有效性。

Conclusion: EVTAR通过简化输入要求和引入参考图像机制，实现了更实用和高质量的虚拟试衣效果，为实际应用提供了可行方案。
...
TL;DR: 提出统一推理框架，通过链式测试时推理连接时序检测、空间定位和文本解释，实现零样本视频异常分析。


<details>
  <summary>Details</summary>
Motivation: 现有视频异常研究多停留在帧级检测，缺乏空间和语义上下文，难以解释异常原因。现有方法依赖数据且任务特定。

Method: 基于链式测试时推理过程，通过任务内推理优化时序检测，任务间链式连接实现空间和语义理解，无需额外训练。

Result: 在多个视频异常检测、定位和解释基准上实现最先进的零样本性能，无需额外数据或梯度更新。

Conclusion: 精心设计的提示与任务链式连接能释放基础模型的推理能力，实现实用、可解释的零样本视频异常分析。
...
TL;DR: VesSAM是一个专门用于2D血管分割的高效框架，通过集成卷积适配器、多提示编码器和轻量级掩码解码器，显著提升了血管分割性能，在多个数据集上超越了现有方法。


<details>
  <summary>Details</summary>
Motivation: 准确的血管分割对临床诊断和手术规划至关重要，但由于血管结构细薄、分支复杂且纹理对比度低，现有通用分割模型（如SAM）在血管分割上表现不佳。

Method: VesSAM框架包含三个核心组件：(1) 卷积适配器增强局部纹理特征；(2) 多提示编码器通过分层交叉注意力融合解剖学提示（骨架、分叉点、段中点）；(3) 轻量级掩码解码器减少锯齿伪影。还引入了自动化多提示标注流水线。

Result: 实验结果显示，VesSAM在8个数据集、5种成像模态上一致优于最先进的PEFT-based SAM变体，Dice系数提升超过10%，IoU提升超过13%。与完全微调的方法相比性能相当，但参数量显著减少。在分布外设置下也表现出色。

Conclusion: VesSAM是一个强大且高效的血管分割框架，通过专门设计的组件和自动化标注流程，在保持轻量化的同时实现了优异的性能，具有良好的泛化能力。
...
TL;DR: 提出了一种新颖的自监督多模态迭代去噪框架MID，通过建模非线性噪声积累过程，无需配对干净-噪声数据集即可有效去除复杂噪声
...
Method: 将噪声数据建模为非线性噪声积累过程中的状态，通过迭代添加噪声来学习两个神经网络：一个估计当前噪声步长，另一个预测并减去相应的噪声增量。对于复杂非线性污染，使用一阶泰勒展开局部线性化噪声过程

Result: 在四个经典计算机视觉任务上的实验表明MID具有鲁棒性、适应性和最先进的性能。在生物医学和生物信息学领域任务中也表现出强大性能和适应性

Conclusion: MID框架能够有效处理复杂非线性噪声，无需配对数据集，在多个领域展现出优越的去噪性能
...
TL;DR: 提出了一种多模态机器学习框架，通过统一特征提取技术分析戈雅画作的视觉和X射线图像，在艺术认证中达到97.8%的分类准确率。


<details>
  <summary>Details</summary>
Motivation: 戈雅作品的艺术认证面临复杂挑战，包括其异质性的风格演变和广泛的历史伪造模式，需要更有效的认证方法。

Method: 使用统一特征提取流程（包括灰度共生矩阵、局部二值模式、熵测量、能量计算和颜色分布分析）处理视觉和X射线图像，通过优化的单类支持向量机进行分类。

Result: 在24幅认证戈雅画作的数据集上，使用80/20训练测试配置和10折交叉验证，达到97.8%分类准确率和0.022假阳性率。案例研究显示对《巨人》达到92.3%认证置信度。

Conclusion: 多模态方法相比单模态方法有显著性能提升，证明在艺术认证中应用统一计算方法于视觉和放射影像的有效性。
...
TL;DR: 提出HyFormer-Net混合CNN-Transformer架构，用于乳腺癌超声图像的同步分割和分类，具有内在可解释性，在BUSI数据集上表现优异，并通过跨数据集研究验证了其泛化能力。


<details>
  <summary>Details</summary>
Motivation: 解决B型超声乳腺癌诊断面临的挑战：斑点噪声、操作者依赖性和边界模糊。现有深度学习方法的局限性包括单任务学习、架构约束（CNN缺乏全局上下文，Transformer缺乏局部特征）和黑盒决策，这些问题阻碍了临床采用。

Method: HyFormer-Net采用双分支编码器，集成EfficientNet-B3和Swin Transformer，通过多尺度分层融合块结合两者优势。使用注意力门控解码器提供精确性和可解释性。引入双管道可解释性方法：内在注意力验证和Grad-CAM分类推理。

Result: 在BUSI数据集上，Dice得分为0.761±0.072，准确率93.2%，优于U-Net、Attention U-Net和TransUNet。恶性召回率92.1±2.2%。集成模型达到Dice 90.2%，准确率99.5%，恶性召回率100%。多尺度融合贡献+16.8% Dice，注意力门控贡献+5.9%。跨数据集研究中，仅用10%目标域数据即可恢复92.5%性能。

Conclusion: HyFormer-Net在乳腺癌超声诊断中表现出卓越性能，通过混合架构有效结合了CNN和Transformer的优势。跨数据集研究证明了其强大的泛化能力，少量目标域数据即可实现高性能迁移，为临床部署提供了实用解决方案。
...
TL;DR: FastBoost是一种参数高效的神经网络架构，通过动态缩放渐进注意力机制(DSPA)在CIFAR基准测试中实现最先进性能，显著减少参数数量同时提升准确率。


<details>
  <summary>Details</summary>
Motivation: 针对边缘设备资源受限的问题，需要开发参数效率高且性能优越的神经网络架构，在保持准确率的同时大幅减少模型参数。

Method: 采用动态缩放渐进注意力机制(DSPA)，包括自适应融合、阶段缩放和残差自适应三个创新组件，结合增强的MBConv模块构建双注意力路径架构。
...
Conclusion: FastBoost通过动态注意力与高效卷积操作的协同优化，实现了前所未有的参数-准确率权衡，可在资源受限的边缘设备上部署而不损失准确率。
...
TL;DR: 提出了T-MLA攻击框架，这是首个针对神经图像压缩系统的目标多尺度对数-指数攻击方法，通过在小波域中直接攻击重建图像质量，揭示了NIC系统的关键安全漏洞。


<details>
  <summary>Details</summary>
Motivation: 现有对神经图像压缩(NIC)的攻击方法通常是像素空间方法的简单改编，忽略了压缩管道的独特结构化特性，需要更先进的漏洞分析方法。

Method: T-MLA攻击框架在小波域中构建对抗性扰动，通过直接针对攻击和重建图像的质量，将扰动策略性地限制在特定小波子带中，实现离线攻击。

Result: 在标准图像压缩基准测试中，对多个最先进NIC架构的广泛评估显示重建质量大幅下降，同时扰动在视觉上难以察觉。

Conclusion: 研究揭示了生成和内容传输管道核心存在的关键安全缺陷，表明NIC系统存在严重的安全漏洞。
...
TL;DR: 提出了一种分层序列预测方法进行图像地理定位，使用S2网格单元从粗到细逐步预测位置，借鉴了语言模型的自回归生成思想，在多个数据集上实现了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 图像地理定位面临的主要挑战是不同位置的视觉相似性和巨大的搜索空间，需要一种能够从广泛区域逐步精确定位到具体位置的方法。

Method: 使用S2网格单元构建分层结构，通过自回归方式依次预测更精细的网格单元，结合beam search和多样本推理等语言模型技术来管理不确定性。

Result: 在Im2GPS3k和YFCC4k数据集上，无MLLM时比可比基线提升高达13.9%，使用MLLM时在所有指标上都达到新的最先进水平。

Conclusion: 分层序列预测方法有效解决了图像地理定位问题，通过模拟人类定位过程和使用语言模型技术，显著提升了定位精度。
...
### [103] [$\left|\,\circlearrowright\,\boxed{\text{BUS}}\,\right|$: A Large and Diverse Multimodal Benchmark for evaluating the ability of Vision-Language Models to understand Rebus Puzzles](https://arxiv.org/abs/2511.01340)
*Trishanu Das,Abhilash Nandy,Khush Bajaj,Deepiha S*

Main category: cs.CV

TL;DR: 提出了一个包含1333个英语Rebus谜题的大规模基准数据集，并开发了RebusDescProgICE框架，通过结合非结构化描述和基于代码的结构化推理，显著提升了视觉语言模型在Rebus谜题上的性能。


<details>
  <summary>Details</summary>
Motivation: Rebus谜题需要图像识别、认知技能、常识推理、多步推理、基于图像的文字游戏等多种能力，这对当前的视觉语言模型具有挑战性。
...
Conclusion: RebusDescProgICE框架有效提升了视觉语言模型在复杂推理任务上的表现，特别是在需要多模态理解和创造性思维的Rebus谜题上。
...
### [104] [SliceVision-F2I: A Synthetic Feature-to-Image Dataset for Visual Pattern Representation on Network Slices](https://arxiv.org/abs/2511.01087)
...
<details>
  <summary>Details</summary>
Motivation: 5G/6G网络发展需要网络切片的精细识别方法，但缺乏支持特征可视化的健壮数据集。

Method: 使用四种编码方法（物理启发映射、Perlin噪声、神经壁纸、分形分支）将多变量KPI向量转换为低分辨率RGB图像，生成12万合成样本。

Result: 创建了包含原始KPI向量和对应图像的公开数据集，模拟真实网络条件和噪声，支持网络状态分类和异常检测等任务。

Conclusion: SliceVision-F2I为网络切片研究提供了可视化数据集，适用于多种机器学习任务和网络数据分析场景。
...
### [105] [Actial: Activate Spatial Reasoning Ability of Multimodal Large Language Models](https://arxiv.org/abs/2511.01618)
...
<details>
  <summary>Details</summary>
Motivation: 虽然多模态大语言模型在2D视觉理解方面取得进展，但其在复杂3D推理任务中的空间信息捕捉能力仍不明确，特别是跨视角一致性这一关键要求。

Method: 构建了Viewpoint-100K数据集，采用两阶段微调策略：首先通过监督微调注入基础知识，然后使用GRPO强化学习算法增强泛化能力，并引入混合冷启动初始化方法。

Result: 实验结果表明该方法显著激活了MLLM的空间推理能力，在领域内和领域外推理任务上均取得性能提升。

Conclusion: 开发MLLM的基础空间技能对于机器人、自主系统和3D场景理解的未来发展具有重要价值。
...
TL;DR: 提出了一种结合Epanechnikov核密度估计和双峰逻辑回归分类器的新方法，用于基于医学图像诊断呼吸系统疾病，在COVID-19胸部X光数据集上达到70.14%的准确率。
...
Method: 使用Epanechnikov非参数核密度估计(EKDE)结合双峰逻辑回归分类器的统计模型学习方案，EKDE能够在不假设特定分布形状的情况下灵活建模数据分布。

Result: 在13808张随机选择的COVID-19放射影像数据集胸部X光片上测试，准确率70.14%，敏感性59.26%，特异性74.18%，显示出中等性能但敏感性有待提高。

Conclusion: 虽然临床专业知识对于进一步改进模型仍然至关重要，但这项研究突显了基于EKDE的方法在提高医学影像诊断准确性和可靠性方面的潜力。
...
<details>
  <summary>Details</summary>
Motivation: 现有的视频transformer在超声心动图分析中容易从非诊断区域（如图像背景）学习虚假相关性，需要克服这一局限性。

Method: ViACT将变形解剖结构表示为点集，将其空间几何和对应图像块编码为transformer tokens，采用掩码自编码策略仅重建解剖块，强制表示学习聚焦于解剖区域。

Result: ViACT在左心室射血分数回归和心脏淀粉样变性检测任务中表现良好，产生与已知病理区域对齐的可解释注意力图，并能泛化到心肌点跟踪任务。

Conclusion: ViACT通过解剖约束有效聚焦transformer注意力于心肌区域，提高了超声心动图分析的准确性和可解释性，且具有良好的泛化能力。
...
TL;DR: 本文提出在边缘计算中使用GPU设备来提升计算机视觉应用的性能，相比仅使用CPU能获得更好的用户体验。


<details>
  <summary>Details</summary>
Motivation: 移动设备上的计算机视觉和增强现实应用对资源需求很高，而边缘计算设备通常容量有限，会影响用户体验。

Method: 在边缘计算环境中使用带有GPU的嵌入式设备来处理高强度的计算机视觉任务。

Result: 实验表明，使用GPU相比仅使用CPU能获得性能提升。

Conclusion: 在边缘计算中使用GPU设备能够克服资源限制，为计算机视觉应用用户提供更好的体验。
...
TL;DR: 提出PCP框架，通过弱监督方式实现医学图像概念预测，无需概念标注或语言模型，在多个医学数据集上显著优于零样本基线。


<details>
  <summary>Details</summary>
Motivation: 医学影像中可解释AI需要概念标注，但标注成本高昂且不实用；现有无标注方法难以捕捉医学领域特定特征，可靠性差。

Method: 使用类别级概念先验作为弱监督，结合KL散度和熵正则化的精炼机制，使预测与临床推理对齐。

Result: 在PH2和WBCatt数据集上，概念级F1分数比零样本基线提高33%以上；在四个医学数据集上分类性能与全监督概念瓶颈模型相当。

Conclusion: PCP框架有效解决了医学影像中概念预测的标注依赖问题，提供可靠的可解释性预测。
...
TL;DR: CatEquiv是一种用于惯性传感器人体活动识别的类别等变神经网络，通过编码时间、幅度和结构对称性来提高鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 为了解决人体活动识别中面对分布外扰动时的鲁棒性问题，需要系统性地编码数据的对称性结构。

Method: 引入类别对称性乘积，结合循环时间平移、正增益和传感器层次偏序集来捕捉数据的类别对称结构，实现等变性。

Result: 在UCI-HAR数据集上，面对分布外扰动时，CatEquiv比循环填充CNN和普通CNN具有显著更高的鲁棒性。
...
TL;DR: 提出了MicroAUNet，一种轻量化的注意力分割网络，结合深度可分离扩张卷积和参数共享的通道-空间注意力块，通过两阶段知识蒸馏方案实现实时结直肠息肉分割。
...
Conclusion: MicroAUNet在极低模型复杂度下实现了最先进的准确性，适用于实时结直肠内窥镜临床息肉分割。
...
TL;DR: ROVER是一个评估统一多模态模型跨模态推理能力的新基准，包含1312个基于1876张图像的任务，测试模型使用一种模态来指导、验证或改进另一种模态输出的能力。


<details>
  <summary>Details</summary>
Motivation: 现有评估方法将多模态能力孤立对待，而真正的统一多模态智能需要测试跨模态推理能力，即使用一种模态来指导另一种模态的能力。

Method: ROVER包含两个互补设置：语言增强的视觉生成推理（使用语言提示和推理链指导图像合成）和视觉增强的语言生成推理（生成中间可视化来加强问答推理过程）。

Result: 对17个统一模型的实验发现：(1)跨模态推理决定视觉生成质量，交错模型显著优于非交错模型；(2)模型在物理和符号推理间存在分离，擅长感知概念解释但无法构建符号任务的视觉抽象。

Conclusion: 跨模态推理是实现真正全模态生成的关键前沿，现有模型在这一能力上仍有明显不足。
...
TL;DR: 提出了一个从YouTube视频自动挖掘动物视频的流程，创建了包含30K视频(200万帧)的大规模数据集，并建立了Animal-in-Motion基准测试，用于4D动物重建任务。


<details>
  <summary>Details</summary>
Motivation: 现有动物视频数据集规模有限(仅2.4K片段)，缺乏对动物中心3D/4D任务的关键处理，需要非侵入性的大规模数据收集方法。

Method: 开发自动化流程从YouTube挖掘视频并处理成对象中心片段，提供姿态估计、跟踪和3D/4D重建等下游任务所需的辅助标注。

Result: 收集了30K视频(200万帧)，比先前工作多一个数量级；建立了包含230个序列、11K帧的Animal-in-Motion基准测试；改进了模型无关方法，建立了首个4D动物重建基线。

Conclusion: 该流程、基准测试和基线旨在推进从野外视频进行大规模、无标记的4D动物重建及相关任务。
...
TL;DR: 提出GCN-PSN框架，利用图卷积网络建模人体骨骼拓扑结构，通过孪生网络架构和对比回归目标学习判别性姿态嵌入，在动作质量评估任务中表现优异。
...
Method: 使用拓扑感知的图卷积网络将人体骨骼建模为图结构，采用孪生网络架构和对比回归目标训练，学习判别性的姿态嵌入表示。

Result: 在AQA-7和FineDiving基准测试中超越了基于坐标的基线方法，取得了竞争性的性能表现。

Conclusion: 实验验证了利用骨骼拓扑结构进行姿态相似性建模和动作质量评估的有效性，图卷积网络能够更好地捕捉人体运动的拓扑特征。
...
TL;DR: 提出DTWSR方法，结合扩散模型和Transformer，通过多级离散小波变换和多尺度频率子带关联处理，实现更一致和逼真的图像超分辨率重建。
...
Method: 使用多级离散小波变换分解图像为小波频谱，提出金字塔标记化方法将频谱嵌入Transformer序列，设计双解码器分别处理低频和高频子带并保持对齐。

Result: 在多个基准数据集上的实验表明，该方法在感知质量和保真度方面都表现出高性能。

Conclusion: DTWSR方法通过有效捕获多尺度频率子带间的相互关系，实现了更一致和逼真的图像超分辨率重建。
...
TL;DR: 该论文针对印度食物的视觉问答(VQA)系统，提出通过多步推理链来改进传统两阶段方法的局限性，使用强化学习训练小型语言模型，在印度食物VQA任务上实现了平均10%的准确率提升。


<details>
  <summary>Details</summary>
Motivation: 现有VQA系统偏向西方食物，无法处理印度食物复杂的文化和烹饪背景。传统两阶段方法（先生成答案再解释）不足以理解印度食物中复杂的烹饪上下文和食物间关系。

Method: 创建自动验证的推理链，用强化学习微调小型LLM和VLM，通过知识图谱增强推理过程。

Result: 通过推理链增强，在印度食物VQA任务上平均准确率提升了10个百分点。
...
### [117] [Eyes on Target: Gaze-Aware Object Detection in Egocentric Video](https://arxiv.org/abs/2511.01237)
...
<details>
  <summary>Details</summary>
Motivation: 传统目标检测器对所有区域平等处理，而人类注视提供了理解复杂视觉环境中注意力分配的丰富监督信号，特别是在需要评估人类任务表现的自我中心视频中。

Method: 在Vision Transformer的注意力机制中注入注视派生特征，使空间特征选择偏向人类关注的区域，并引入注视感知注意力头重要性指标来解释模型行为。

Result: 在自定义模拟器数据集和公共基准测试（Ego4D Ego-Motion和Ego-CH-Gaze）上，相比无注视基线模型，检测准确率持续提升。

Conclusion: 注视引导的目标检测框架能有效提升自我中心视频中的检测性能，特别是在需要评估人类视觉注意力的任务场景中具有重要应用价值。
...
<details>
  <summary>Details</summary>
Motivation: 传统运动生成方法在推理步骤和生成质量之间存在权衡，MoSa旨在通过分层量化策略解决这一问题，实现高效且高质量的运动生成。

Method: 提出多尺度令牌保留策略(MTPS)和分层残差向量量化变分自编码器(RQ-VAE)，结合可扩展自回归建模(SAR)，并引入卷积-注意力混合VQ-VAE(CAQ-VAE)来提升表达能力。

Result: 在Motion-X数据集上达到FID 0.06的SOTA性能，相比MoMask的0.20有显著提升，同时推理时间减少27%，在运动编辑等下游任务中表现良好。

Conclusion: MoSa通过创新的分层生成框架在运动生成质量和效率方面都达到了新的水平，为文本驱动的3D人体运动生成提供了有效的解决方案。
...
### [119] [OmniVLA: Unifiying Multi-Sensor Perception for Physically-Grounded Multimodal VLA](https://arxiv.org/abs/2511.01210)
*Heyu Guo,Shanmu Wang,Ruichun Ma,Shiqi Jiang,Yasaman Ghasempour,Omid Abari,Baining Guo,Lili Qi*

Main category: cs.CV

TL;DR: OmniVLA是一个多模态视觉-语言-动作模型，通过整合红外相机、毫米波雷达和麦克风阵列等新型传感器，超越传统RGB感知，提供物理基础的空间智能。


<details>
  <summary>Details</summary>
Motivation: 现有的VLA模型主要依赖RGB相机，感知能力有限，限制了操作能力。需要整合更多传感器模态来提升物理环境感知。

Method: 提出传感器掩码图像作为统一表示，将传感器数据以空间掩码形式叠加在RGB图像上。基于RGB预训练的VLA骨干网络，使用轻量级传感器投影器进行数据高效学习。

Result: 在需要传感器模态感知的挑战性任务中，OmniVLA达到84%的平均任务成功率，比RGB-only和原始传感器输入基线模型分别高出59%和28%。

Conclusion: OmniVLA通过多模态传感器整合显著提升了VLA模型的感知和操作能力，具有更高的学习效率和更强的泛化能力。
...
### [120] [Adaptation of Foundation Models for Medical Image Analysis: Strategies, Challenges, and Future Directions](https://arxiv.org/abs/2511.01284)
*Karma Phuntsho,Abdullah,Kyungmi Lee,Ickjai Lee,Euijoon Ahn*

Main category: cs.CV

TL;DR: 这篇综述论文评估了基础模型在医学影像分析中的适应策略，包括监督微调、领域特定预训练、参数高效微调等方法，并探讨了持续学习、联邦学习等新兴方向，旨在为开发适应性强、可信赖的临床集成基础模型提供路线图。


<details>
  <summary>Details</summary>
Motivation: 基础模型在医学影像分析中具有变革潜力，但适应真实临床实践仍面临领域偏移、标注数据有限、计算需求高和隐私要求严格等挑战。

Method: 评估了监督微调、领域特定预训练、参数高效微调、自监督学习、混合方法以及多模态框架等多种适应策略。

Result: 分析了各种方法的性能增益、临床适用性和局限性，识别了权衡和未解决的挑战。
...
### [121] [Saliency-Guided Domain Adaptation for Left-Hand Driving in Autonomous Steering](https://arxiv.org/abs/2511.01223)
*Zahra Mehraban,Sebastien Glaser,Michael Milford,Ronald Schroeter*

Main category: cs.CV

TL;DR: 该论文研究了通过翻转数据预训练和微调的方法来改进自动驾驶模型的领域适应能力，特别是在左舵驾驶条件下的适应。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶模型需要良好的领域适应能力来应对不同的道路条件，特别是左舵和右舵驾驶环境的差异。
...
Conclusion: 翻转数据预训练结合微调的方法能有效提升模型领域适应能力，且在不同架构中具有通用性。
...
### [122] [Perturb a Model, Not an Image: Towards Robust Privacy Protection via Anti-Personalized Diffusion Models](https://arxiv.org/abs/2511.01307)
*Tae-Young Lee,Juwon Seo,Jong Hwan Ko,Gyeong-Moon Park*

Main category: cs.CV

TL;DR: 提出了APDM框架，通过将保护目标从图像转移到扩散模型本身来防止未经授权的个性化内容生成，包括新的DPO损失函数和L2P双路径优化策略。


<details>
  <summary>Details</summary>
Motivation: 扩散模型的高质量个性化合成能力带来了隐私风险，现有对抗样本方法在少量干净图像或简单图像变换下失效，需要更鲁棒的保护方法。

Method: 提出了Direct Protective Optimization (DPO)损失函数来有效破坏目标模型的个性化，同时保持生成质量；还提出了Learning to Protect (L2P)双路径优化策略，通过交替进行个性化和保护路径来模拟未来个性化轨迹并自适应加强保护。

Result: 实验结果表明该框架优于现有方法，在防止未经授权个性化方面达到了最先进的性能。

Conclusion: APDM框架通过模型层面的保护机制，有效解决了扩散模型个性化带来的隐私风险问题，为内容创作安全提供了可靠保障。
...
TL;DR: 本文分析了语音驱动3D手势生成领域的人类评估实践，发现缺乏标准化和存在实验设计缺陷，为此提出了BEAT2数据集的人类评估协议，并通过大规模众包评估对6个最新模型进行排名。


<details>
  <summary>Details</summary>
Motivation: 当前语音驱动3D手势生成领域的人类评估缺乏标准化，实验设计存在缺陷，导致无法比较不同方法的优劣和确定技术现状。

Method: 引入BEAT2运动捕捉数据集的详细人类评估协议，通过大规模众包评估对6个最新手势生成模型在两个关键维度（运动真实性和语音-手势对齐）上进行排名。

Result: 评估结果表明：1）新模型并不总是优于早期方法；2）已发表的高运动真实性或语音-手势对齐声明在严格评估下可能不成立；3）领域需要采用解耦的运动质量和多模态对齐评估。

Conclusion: 为促进标准化和推动评估研究，将发布5小时合成运动数据、750多个渲染视频刺激、开源渲染脚本和16,000对人工偏好投票。
...
TL;DR: 提出了一种新的黑盒梯度迁移攻击方法AFA，通过引入对抗平坦度(AF)解决欺骗性平坦问题，并使用MCAS提高采样效率，在ImageNet数据集上优于6个基线方法。
...
Method: 从双阶信息角度提出对抗平坦度(AF)理论，基于此开发AFA攻击方法解决梯度符号改变问题，并使用MCAS提高内循环采样效率。

Result: 在ImageNet兼容数据集上优于6个基线方法，生成的对抗样本位于更平坦区域，在不同模型架构间具有更好的可迁移性，在输入变换攻击和百度云API测试中也表现更优。

Conclusion: 提出的AFA方法通过对抗平坦度和高效采样策略，有效提升了对抗样本的可迁移性，为解决欺骗性平坦问题提供了新思路。
...
<details>
  <summary>Details</summary>
Motivation: 现有的自注意力方法难以有效处理视觉与语言之间的跨模态语义对齐，分类方法依赖预定义答案集，无法适应自由形式答案的多样性

Method: 包含三个关键模块：细粒度视觉文本特征对齐(FVTA)、跨模态交错特征表示(CIFR)、自由形式答案增强多任务学习(FFAE)，通过跨模态Mamba交互学习图像和文本的特征表示

Result: 在VQA-RAD、SLAKE和OVQA三个Med-VQA数据集上超越了现有最先进方法，并通过可解释性实验验证了有效性

Conclusion: CMI-MTL框架能够有效解决医学视觉问答中的跨模态语义对齐和自由形式答案处理问题，为临床决策支持和远程医疗提供了有力工具
...
TL;DR: CenterMamba-SAM是一个用于脑部病灶分割的端到端框架，通过冻结预训练主干网络并仅训练轻量适配器实现高效微调，采用创新的3x3角-轴-中心短序列扫描策略和内存驱动的结构提示生成器，在多个公开基准测试中达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 脑部病灶分割面临小病灶、低对比度病变、各向异性采样和跨切片不连续性等挑战，需要开发能够增强弱边界敏感性并保持切片间一致性的方法。

Method: 提出CenterMamba编码器采用3x3角-轴-中心短序列扫描策略实现中心优先、轴增强和对角补偿的信息聚合；内存驱动的结构提示生成器维护相邻切片的原型库，自动合成可靠提示；内存增强的多尺度解码器集成多级内存注意力模块，结合深度监督和渐进细化。

Result: 在多个公开基准测试上的广泛实验表明，CenterMamba-SAM在脑部病灶分割任务中达到了最先进的性能水平。

Conclusion: CenterMamba-SAM通过创新的扫描策略和内存驱动机制，有效解决了脑部病灶分割中的关键挑战，为医学图像分析提供了高效可靠的解决方案。
...
TL;DR: 提出了一种轻量级几何感知适配器，通过方位对齐和水平循环填充来保护边界连续性，使用局部K近邻计算几何特征，并在训练时驱动区域感知正则化，提升恶劣天气下LiDAR语义分割的鲁棒性。
...
Method: 使用方位对齐和水平循环padding保护边界连续性，局部窗口K近邻收集附近点并计算简单统计特征，压缩为紧凑的几何感知线索，在训练时驱动区域感知正则化。
...
Method: 通过自强制分布匹配蒸馏将双向教师模型转化为因果学生模型，采用滑动窗口因果注意力结合注意力sink，以及KV缓存滚动训练，实现固定上下文窗口下的无限长度视频生成。

Result: 在运动跟随和视频质量方面达到最先进水平，速度提升两个数量级，能够实现无限长度的实时流式生成。

Conclusion: MotionStream通过因果蒸馏和滑动窗口注意力机制，成功解决了实时运动条件视频生成的关键挑战，为用户提供了真正的交互体验。
...
### [129] [SEPS: Semantic-enhanced Patch Slimming Framework for fine-grained cross-modal alignment](https://arxiv.org/abs/2511.01390)
*Xinyu Mao,Junsi Li,Haoji Zhang,Yu Liang,Ming Sun*

Main category: cs.CV

TL;DR: 提出了SEPS框架，通过整合密集和稀疏文本的统一语义来解决视觉-语言跨模态对齐中的补丁冗余和模糊性问题，显著提升了检索性能。


<details>
  <summary>Details</summary>
Motivation: 当前方法在处理跨模态信息密度差异导致的补丁冗余和模糊性方面存在挑战，而多模态大语言模型的密集文本输出可能与原始稀疏描述产生冲突。

Method: 采用两阶段机制整合密集和稀疏文本的统一语义来识别显著视觉补丁，利用相关性感知选择和均值计算来突出关键补丁-词对应关系。

Result: 在Flickr30K和MS-COCO数据集上的实验表明，SEPS在rSum指标上比现有方法提升23%-86%，在文本到图像检索场景中表现尤为突出。

Conclusion: SEPS框架有效解决了跨模态对齐中的补丁冗余和模糊性问题，为视觉问答等应用提供了更精确的局部对应关系。
...
### [130] [PRevivor: Reviving Ancient Chinese Paintings using Prior-Guided Color Transformers](https://arxiv.org/abs/2511.01274)
*Tan Tang,Yanhong Wu,Junming Gao,Yingcai Wu*

Main category: cs.CV

TL;DR: PRevivor是一个基于先验引导的颜色转换器，通过学习明清时期绘画来恢复唐宋时期古画的色彩，通过亮度增强和色调校正两个子任务实现色彩复原。


<details>
  <summary>Details</summary>
Motivation: 中国古代绘画面临不可逆的色彩退化问题，由于复杂的化学机制和缺乏高质量数据集，色彩复原非常困难，阻碍了端到端数字修复工具的开发。

Method: 将色彩恢复分解为亮度增强和色调校正两个顺序子任务：使用两个变分U-Net和多尺度映射模块进行亮度增强；设计基于局部色调先验的双分支颜色查询模块进行色调校正。

Result: 与最先进的着色方法进行广泛实验比较，在定量和定性评估中都表现出优越性能。

Conclusion: PRevivor能够有效恢复古画的色彩，通过先验引导的方法在色彩复原任务中取得了显著成果。
...
### [131] [UniSOT: A Unified Framework for Multi-Modality Single Object Tracking](https://arxiv.org/abs/2511.01427)
*Yinchao Ma,Yuyang Tang,Wenfei Yang,Tianzhu Zhang,Xu Zhou,Feng Wu*

Main category: cs.CV

TL;DR: UniSOT是一个统一的目标跟踪器，能够处理三种参考模态（边界框、自然语言或两者）和四种视频模态（RGB、RGB+深度、RGB+热成像或RGB+事件）的不同组合，使用统一参数。


<details>
  <summary>Details</summary>
Motivation: 现有跟踪器通常针对单一或少数几种视频模态和参考模态设计，导致模型分离且限制实际应用。需要开发统一跟踪器来处理各种需求。

Method: 提出了UniSOT统一跟踪器，能够同时处理三种参考模态和四种视频模态的不同组合，使用统一的参数架构。

Result: 在18个视觉跟踪、视觉语言跟踪和RGB+X跟踪基准测试中，UniSOT相比模态特定对应方法表现出优越性能。在TNL2K上所有三种参考模态上AUC超过先前方法3.0%以上，在Un-Track上所有三种RGB+X视频模态上主要指标超过2.0%以上。

Conclusion: UniSOT证明了统一跟踪器在不同参考模态和视频模态组合中的有效性和优越性，为实际应用提供了更灵活的解决方案。
...
### [132] [Privacy Preserving Ordinal-Meta Learning with VLMs for Fine-Grained Fruit Quality Prediction](https://arxiv.org/abs/2511.01449)
*Riddhi Jain,Manasi Patwardhan,Aayush Mishra,Parijat Deshpande,Beena Rai*

Main category: cs.CV

TL;DR: 提出了一种模型无关的序数元学习算法(MAOML)，用于训练小型视觉语言模型，通过元学习解决数据稀疏问题并利用标签序数性，在水果新鲜度分类任务中实现了92.71%的行业标准准确率。


<details>
  <summary>Details</summary>
Motivation: 由于专家标注成本高导致数据稀缺，而闭源视觉语言模型存在数据隐私问题，开源模型性能欠佳，需要开发能够在数据有限情况下实现高性能的水果新鲜度检测方法。

Method: 采用模型无关的序数元学习算法(MAOML)，结合元学习解决数据稀疏问题，并利用标签的序数特性来提升模型性能。

Result: 在零样本和少样本设置下，该方法在水果新鲜度分类任务中达到了92.71%的行业标准准确率，优于现有开源模型。

Conclusion: MAOML算法通过元学习和序数回归的结合，有效解决了数据稀缺问题，为水果新鲜度检测提供了一种高性能的解决方案。
...
TL;DR: 提出了一种基于数据流形几何差异的生成图像检测框架，利用自然图像和生成图像在流形结构上的差异进行检测，并通过归一化流放大可检测差异


<details>
  <summary>Details</summary>
Motivation: 随着生成图像越来越逼真，亟需开发不依赖大量生成图像样本的鲁棒检测方法。现有基于二元分类器的方法严重依赖生成图像的数量和质量

Method: 利用自然图像和生成图像数据流形的几何差异，设计一对函数使自然图像输出一致而生成图像输出发散，通过自监督模型在图像变换时的损失变化进行检测，并使用归一化流放大流形差异

Result: 大量实验证明了该方法的有效性

Conclusion: 该方法提供了一种简单有效的生成图像检测方案，能够应对先进生成模型中逐渐减小的流形差异
...
### [134] [Reg-DPO: SFT-Regularized Direct Preference Optimization with GT-Pair for Improving Video Generation](https://arxiv.org/abs/2511.01450)
*Jie Du,Xinyu Gong,Qingshan Tan,Wen Li,Yangming Cheng,Weitao Wang,Chenlu Zhan,Suhui Wu,Hao Zhang,Jun Zhang*

Main category: cs.CV

TL;DR: 提出了Reg-DPO方法，通过GT-Pair自动构建高质量偏好对，结合SFT损失作为正则化项，显著提升了视频生成质量，在I2V和T2V任务上优于现有方法。
...
Method: 1) GT-Pair自动构建偏好对（真实视频为正样本，模型生成视频为负样本）；2) Reg-DPO将SFT损失作为正则化项融入DPO目标；3) 结合FSDP框架与多种内存优化技术提升训练能力。

Result: 训练容量比单独使用FSDP提高近3倍，在多个数据集的I2V和T2V任务上均优于现有方法，生成视频质量显著提升。

Conclusion: Reg-DPO方法有效解决了视频生成中的关键挑战，无需外部标注即可实现高质量视频生成，为大规模视频模型优化提供了可行方案。
...
### [135] [UniREditBench: A Unified Reasoning-based Image Editing Benchmark](https://arxiv.org/abs/2511.01295)
...
TL;DR: 提出了UniREditBench基准测试，用于评估基于推理的图像编辑模型性能，包含2700个样本，涵盖真实世界和游戏世界场景，并引入了多模态双参考评估方法。


<details>
  <summary>Details</summary>
Motivation: 当前多模态生成模型在需要隐式推理的复杂图像编辑任务上表现不佳，现有基准主要关注单对象属性变换，忽视了多对象交互和游戏世界场景，且仅依赖文本参考评估可能导致误判。

Method: 构建UniREditBench基准，包含2700个样本，覆盖8个主要维度和18个子维度；引入多模态双参考评估；设计自动数据合成流程构建UniREdit-Data-100K数据集；在Bagel模型上微调开发UniREdit-Bagel。

Result: UniREdit-Bagel在领域内和领域外设置中均表现出显著改进；通过对开源和闭源图像编辑模型的全面基准测试，揭示了它们在不同方面的优势和弱点。

Conclusion: UniREditBench为基于推理的图像编辑提供了全面评估框架，多模态双参考评估提高了可靠性，UniREdit-Bagel展示了在复杂推理场景中的有效性。
...
### [136] [When to Trust the Answer: Question-Aligned Semantic Nearest Neighbor Entropy for Safer Surgical VQA](https://arxiv.org/abs/2511.01458)
...
TL;DR: 提出QA-SNNE方法，通过结合问题语义的最近邻熵来估计不确定性，提高手术VQA系统的安全性和可靠性。


<details>
  <summary>Details</summary>
Motivation: 手术VQA中错误或模糊的回答可能危害患者安全，现有研究忽视安全行为如模糊意识、转诊专家等，需要更好的不确定性估计方法。

Method: QA-SNNE是一种黑盒不确定性估计器，在医学文本嵌入空间中比较生成答案与最近邻的语义熵，结合问题语义来评估预测置信度。

Result: 在EndoVis18-VQA和PitVQA数据集上，QA-SNNE在大多数模板内设置中提高了AUROC，零样本模型AUROC提升15-38%，并增强了幻觉检测能力。

Conclusion: QA-SNNE通过将语义不确定性与问题上下文关联，为手术VQA中的自动故障检测提供了实用且可解释的方法，结合LVLM骨干网络可提高安全性和临床医生信任度。
...
### [137] [REASON: Probability map-guided dual-branch fusion framework for gastric content assessment](https://arxiv.org/abs/2511.01302)
...
TL;DR: 提出了一种名为REASON的两阶段概率图引导双分支融合框架，用于通过超声自动评估胃内容物，以改善麻醉诱导时误吸风险的评估。
...
Method: 采用两阶段框架：第一阶段使用分割模型生成概率图来抑制伪影并突出胃解剖结构；第二阶段使用双分支分类器融合右侧卧位和仰卧位两个标准视图的信息。

Result: 在自收集数据集上的实验结果表明，该框架显著优于当前最先进的方法。

Conclusion: 该框架为术前误吸风险评估提供了更稳健、高效和准确的解决方案，在临床实践中具有巨大潜力。
...
### [138] [Efficiently Training A Flat Neural Network Before It has been Quantizated](https://arxiv.org/abs/2511.01462)
*Peng Xia,Junbiao Pang,Tianyang Cai*

Main category: cs.CV

TL;DR: 提出了一种针对视觉Transformer的后训练量化框架，通过统计建模激活和权重量化误差为独立高斯噪声，并采用噪声注入优化方法获得平坦最小值，从而改善低比特量化效果。


<details>
  <summary>Details</summary>
Motivation: 现有后训练量化方法通常忽略了训练好的神经网络与量化模型之间的关系，导致较大的量化误差。需要找到一种高效训练模型无关神经网络的方法，使其适应预定义精度的低比特模型。
...
Conclusion: 平坦的全精度神经网络是低比特量化的关键，通过统计建模量化误差并采用噪声注入优化可以显著改善后训练量化效果。
...
TL;DR: 提出了一种用于全玻片图像(WSI)可解释表示的三阶段学习框架，通过潜在因子分组和聚类推理实例解缠来解决MIL中的空间、语义和决策纠缠问题。


<details>
  <summary>Details</summary>
Motivation: 多实例学习(MIL)在表示全玻片病理图像时存在空间、语义和决策层面的实例纠缠问题，限制了其表示能力和可解释性。
...
Conclusion: 该框架有效解决了MIL中的实例纠缠问题，在保持高性能的同时实现了可解释的表示和决策过程。
...
### [140] [HMVLM: Human Motion-Vision-Lanuage Model via MoE LoRA](https://arxiv.org/abs/2511.01463)
*Lei Hu,Yongjing Ye,Shihong Xia*

Main category: cs.CV

TL;DR: 提出了HMVLM框架，基于MoE LoRA策略，通过门控网络动态分配LoRA专家权重，解决3D人体运动与文本模态融合中的灾难性遗忘问题，并采用身体部位特定标记化提升姿态表示的空间分辨率。


<details>
  <summary>Details</summary>
Motivation: 解决人体运动与文本模态融合中的灾难性遗忘问题，以及开发跨异构下游任务保持泛化能力的自回归兼容姿态表示的技术障碍。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于Transformer的MVS方法存在二次复杂度问题，难以平衡性能与效率。Mamba架构具有全局建模能力和线性复杂度，为解决此问题提供了新思路。

Method: 提出MVSMamba网络，采用动态Mamba模块，基于参考中心动态扫描策略，实现高效的视图内外特征交互、全方位多视角特征表示和多尺度全局特征聚合。

Result: 在DTU数据集和Tanks-and-Temples基准测试中，MVSMamba在性能和效率方面均优于最先进的MVS方法。

Conclusion: MVSMamba成功将Mamba架构应用于MVS任务，证明了其在全局特征建模方面的优势，为MVS研究提供了新的高效解决方案。
...
TL;DR: 提出结构化五层模型来改进罕见驾驶场景的评估和生成，使用数据增强策略结合基础模型生成新场景，并引入多样性和原创性指标来评估合成数据集的质量。
...
Method: 提出结构化五层模型，为场景中的每个智能体引入子类和特征，使用特定嵌入进行比较，并采用数据增强策略结合大型基础模型生成新驾驶场景。

Result: 在不同生成设置下展示了多样性和原创性指标的应用，并对结构化场景描述生成的合成视频进行了定性评估。

Conclusion: 该结构化模型和评估指标能够有效改进罕见驾驶场景的生成和评估，为自动驾驶开发提供重要支持。
...
### [143] [A Generative Adversarial Approach to Adversarial Attacks Guided by Contrastive Language-Image Pre-trained Model](https://arxiv.org/abs/2511.01317)
*Sampriti Soor,Alik Pramanick,Jothiprakash K,Arijit Sur*

Main category: cs.CV

TL;DR: 提出了一种基于CLIP模型的生成对抗攻击方法，通过结合文本和图像表示生成视觉不可察觉的对抗扰动，在多对象场景中有效欺骗多标签分类器。
...
Result: 在多种黑盒受害者模型上的实验表明，该方法与现有技术相比具有竞争力或更优性能，同时保持更高的视觉保真度。

Conclusion: 该方法成功生成了既有效欺骗分类模型又保持高结构相似性的对抗扰动，为对抗攻击研究提供了新思路。
...
TL;DR: DINO-MX是一个模块化、可扩展的自监督视觉基础模型训练框架，统一了DINO系列方法，支持多种Transformer架构和训练策略，显著降低计算成本的同时保持竞争力。


<details>
  <summary>Details</summary>
Motivation: 现有视觉基础模型训练流程存在不灵活、领域特定或计算成本高的问题，限制了跨领域和资源受限环境下的使用。

Method: 结合DINO系列核心原理的统一配置驱动系统，支持多种Transformer架构、LoRA、层冻结、知识蒸馏等训练策略，以及DDP和FSDP分布式训练。

Result: 在多样化数据集上实现竞争力性能，显著降低计算成本，提供可解释性工具和无需额外检测头的标签引导数据增强方法。

Conclusion: DINO-MX为开发和基准测试自监督视觉模型提供了可复现、可扩展的基础，适用于研究和实际应用。
...
### [145] [RDTE-UNet: A Boundary and Detail Aware UNet for Precise Medical Image Segmentation](https://arxiv.org/abs/2511.01328)
*Jierui Qu,Jianchun Zhao*

Main category: cs.CV

TL;DR: 提出了RDTE-UNet分割网络，通过结合局部建模和全局上下文来增强边界描绘和细节保留，在医学图像分割中取得了良好效果。
...
Method: 使用混合ResBlock细节感知Transformer骨干网络，包含三个模块：ASBE用于自适应边界增强，HVDA用于细粒度特征建模，EulerFF用于基于欧拉公式的融合加权。

Result: 在Synapse和BUSI数据集上，RDTE-UNet在分割精度和边界质量方面达到了可比较的水平。

Conclusion: RDTE-UNet通过统一局部建模与全局上下文，有效提升了结构一致性和边界准确性，适用于不同形态、方向和尺度的医学图像分割。
...
TL;DR: MIQ-SAM3D是一个多实例3D医学图像分割框架，通过竞争性查询优化策略实现从单点单对象到单点多实例的分割转变，解决了传统SAM方法在多病灶分割中的局限性。


<details>
  <summary>Details</summary>
Motivation: 传统SAM交互式分割方法遵循单点单对象范式，限制了多病灶分割能力；同时ViT骨干网络虽然能捕捉全局上下文，但往往丢失高保真局部细节。

Method: 提出提示条件实例查询生成器将单点提示转换为多个专门化查询；采用混合CNN-Transformer编码器通过空间门控将CNN边界显著性注入ViT自注意力；设计竞争优化查询解码器实现端到端并行多实例预测。
...
TL;DR: 提出了一种通过内容-风格子空间混合和平衡损失来扩展内容-风格前沿的新方法，在保持风格强度的同时显著改善内容相似性。
...
Method: 采用内容-风格子空间混合技术和内容-风格平衡损失函数来扩展内容-风格前沿。

Result: 在定性和定量评估中均优于现有技术，实现了更优的内容-风格权衡，倒置生成距离和生成距离得分显著低于当前方法。

Conclusion: 该方法能有效扩展内容-风格前沿，在多种风格强度下保持更好的内容相似性。
...
TL;DR: 提出了一种用于生成水下环境中AUV搭载事件相机合成数据的管道，以解决传统视觉技术在水下恶劣条件下的性能问题。


<details>
  <summary>Details</summary>
Motivation: 水下环境存在光照条件差、高动态范围等挑战，传统视觉技术难以适应。事件相机通过逐帧跟踪变化来缓解这些问题，但缺乏合适的训练数据。

Method: 开发了一个管道，能够生成AUV搭载事件相机在水下环境中的逼真合成数据，用于训练视觉模型。

Result: 在能见度差和悬浮颗粒物条件下的岩石检测任务中验证了管道的有效性。

Conclusion: 该方法可推广到其他水下任务，为事件相机在水下环境中的应用提供了数据支持。
...
### [149] [Wonder3D++: Cross-domain Diffusion for High-fidelity 3D Generation from a Single Image](https://arxiv.org/abs/2511.01767)
...
<details>
  <summary>Details</summary>
Motivation: 现有方法存在效率低（基于SDS的方法需要逐形状优化）或质量差（直接网络推理方法缺乏几何细节）的问题，需要全面提升单视图重建的质量、一致性和效率。

Method: 提出跨域扩散模型生成多视角法线图和彩色图像，采用多视角跨域注意力机制确保生成一致性，并使用级联3D网格提取算法以粗到细的方式从2D表示中提取高质量表面。

Result: 方法实现了高质量的重建结果，具有良好的泛化能力和效率，相比先前工作有明显提升。

Conclusion: Wonder3D++在单视图3D重建任务中实现了质量、一致性和效率的全面提升，是解决该领域关键挑战的有效方案。
...
### [150] [Semantic BIM enrichment for firefighting assets: Fire-ART dataset and panoramic image-based 3D reconstruction](https://arxiv.org/abs/2511.01399)
...
<details>
  <summary>Details</summary>
Motivation: 传统消防资产管理方法在自动资产识别和重建方面能力有限，效率低下，无法满足应急准备、风险评估和现场火灾响应的需求。

Method: 开发了包含15种基础资产、2626张图像和6627个实例的Fire-ART数据集，并提出了整合改进的立方体贴图转换和基于半径的球形相机投影的重建方法。

Result: 通过两个真实案例验证，该方法分别达到73%和88%的F1分数，定位误差分别为0.620米和0.428米。

Conclusion: Fire-ART数据集和重建方法为消防设备的精确数字化管理提供了宝贵资源和稳健技术解决方案。
...
TL;DR: SurgVeo是首个用于手术视频生成模型评估的专家策划基准，通过四层外科可信度金字塔(SPP)框架评估模型输出。研究发现Veo-3模型在视觉感知可信度上表现优异，但在更高层次的器械操作、环境反馈和手术意图可信度上存在显著差距。


<details>
  <summary>Details</summary>
Motivation: 当前视频生成基础模型在模拟物理世界方面表现出色，但在需要深度专业因果知识的高风险领域如手术中应用仍存在关键空白。

Method: 提出SurgVeo基准和外科可信度金字塔(SPP)四层评估框架，让Veo-3模型在腹腔镜和神经外科手术剪辑上进行零样本预测任务，由四位认证外科医生根据SPP评估生成视频。

Result: 发现明显的"可信度差距"：Veo-3在视觉感知可信度上表现卓越，但在更高层次的器械操作可信度、环境反馈可信度和手术意图可信度上严重失败。

Conclusion: 这项工作首次量化证明了手术AI中视觉逼真模仿与因果理解之间的鸿沟，为开发能够应对专业医疗领域复杂性的未来模型奠定了关键基础和路线图。
...
### [152] [Extremal Contours: Gradient-driven contours for compact visual attribution](https://arxiv.org/abs/2511.01411)
*Reza Karimzadeh,Albert Alonso,Frans Zdyb,Julius B. Kirkegaard,Bulat Ibragimov*

Main category: cs.CV

TL;DR: 提出了一种无需训练的解释方法，用平滑可调轮廓替代密集扰动掩码，通过星凸区域参数化和傅里叶级数优化，生成单连通、紧凑的解释区域。


<details>
  <summary>Details</summary>
Motivation: 现有密集扰动掩码方法存在碎片化、过拟合问题，需要复杂的后处理，缺乏稳定性和可解释性。

Method: 使用截断傅里叶级数参数化星凸区域，在极值保留/删除目标下利用分类器梯度进行优化，保证单连通掩码并大幅减少参数数量。

Result: 在ImageNet分类器上匹配密集掩码的极值保真度，产生紧凑、可解释区域，运行一致性提升，在多轮廓扩展中能定位多个对象。

Conclusion: 该方法在多个基准测试中实现更高的相关质量和更低复杂度，特别在自监督DINO模型上相关质量提升超过15%，保持正相关性。
...
### [153] [Towards One-step Causal Video Generation via Adversarial Self-Distillation](https://arxiv.org/abs/2511.01419)
...
<details>
  <summary>Details</summary>
Motivation: 现有的混合视频生成模型结合了自回归时间动态和基于扩散的空间去噪，但其顺序迭代特性导致错误累积和推理时间长。

Method: 基于分布匹配蒸馏框架，提出对抗自蒸馏策略，将学生模型n步去噪输出与其(n+1)步版本在分布层面对齐；同时提出首帧增强策略，为首帧分配更多去噪步骤以减少错误传播。

Result: 在VBench上的广泛实验表明，该方法在一步和两步视频生成中均优于最先进方法，且单个蒸馏模型可灵活支持多种推理步数设置。

Conclusion: 该框架实现了高效高质量的视频合成，无需重复再蒸馏，支持多种推理步数设置。
...
TL;DR: 提出一种分辨率感知的token解码器，在低分辨率瓶颈处进行大部分计算，通过门控交叉注意力注入精细细节，仅对稀疏的不确定性选择像素进行细化，平衡全局语义、局部一致性和边界保真度。


<details>
  <summary>Details</summary>
Motivation: 解决越野语义分割中存在的边界模糊、稀疏监督和标签噪声问题，传统方法在低分辨率融合时模糊边缘，而高分辨率路径成本高且对噪声敏感。

Method: 使用分辨率感知token解码器，包含全局自注意力与轻量级扩张深度细化恢复局部一致性，门控交叉注意力集成高分辨率编码器特征而不放大噪声，类感知点细化以可忽略开销修正残差模糊性，训练时添加边界带一致性正则化器。

Result: 在越野语义分割任务中表现出竞争性性能和跨过渡的改进稳定性。

Conclusion: 该方法有效平衡了计算效率与分割精度，在保持边界清晰度的同时提升了模型对噪声的鲁棒性。
...
TL;DR: 提出了一种仅用于训练的方法，通过增强热红外图像的特征表示来提高夜间目标检测性能，无需测试时的多模态融合。


<details>
  <summary>Details</summary>
Motivation: 热红外夜间检测面临低对比度、弱高频线索导致的重叠检测框、小目标漏检和类别混淆问题。现有方法要么依赖RGB转换（易受伪影影响），要么需要测试时多模态融合（增加成本和复杂度）。

Method: 在训练阶段引入两个目标：1）通过类内特征聚合和类间特征分离来锐化决策边界；2）使用RGB训练的教师模型通过特征对齐向学生模型注入跨模态语义先验。

Result: 在实验中，该方法优于先前方法并达到了最先进的性能。

Conclusion: 该方法通过仅训练阶段的改进，直接塑造热红外特征表示，有效解决了夜间检测的关键问题，同时保持了单模态推理的简洁性。
...
<details>
  <summary>Details</summary>
Motivation: 现有深度联合源信道编码框架容易受到物理层对抗威胁（如导频欺骗和子载波干扰）的影响，损害语义保真度。

Method: 采用伪逆引导采样和自适应引导权重实现灵活步长控制和高效语义重建；针对干扰攻击引入基于功率的子载波掩码策略；针对导频欺骗将信道估计重构为盲逆问题，开发EM驱动的重建算法。

Result: 在对抗性OFDM信道条件下的广泛实验表明，SecDiff在重建质量和计算成本之间实现了有利的平衡，优于现有安全和生成式JSCC基线方法。
...
TL;DR: 提出了增强行人对齐网络（EPAN），用于物联网监控环境下的人员重识别，在Inspection-Personnel数据集上达到90.09%的Rank-1准确率和78.82%的mAP。


<details>
  <summary>Details</summary>
Motivation: 解决物联网智能环境中监控和安全应用的人员重识别问题，特别是在视角和环境变化下的鲁棒性需求。

Method: 采用双分支架构来减轻视角和环境变化的影响，在不同尺度和视角下提取对齐信息。

Result: 在Inspection-Personnel数据集上取得了90.09%的Rank-1准确率和78.82%的mAP，展示了强大的特征提取能力。

Conclusion: EPAN在真实物联网应用中具有潜力，能够在监控和安全系统中实现跨摄像头的有效可靠人员重识别。
...
TL;DR: 提出一种在SE(3)流形上使用流匹配的概率框架，用于估计6D物体姿态分布，解决姿态模糊性和多模态问题。


<details>
  <summary>Details</summary>
Motivation: 物体姿态估计面临部分可观测性、遮挡和物体对称性等挑战，导致姿态模糊和多假设问题。确定性深度网络在约束条件下表现良好，但往往过于自信且无法捕捉底层姿态分布的多模态特性。

Method: 利用SE(3)流形上的流匹配技术，建模完整的姿态分布，提供基于样本的估计，能够在对称物体或严重遮挡等模糊情况下进行不确定性推理。

Result: 在Real275、YCB-V和LM-O数据集上取得最先进的结果，并展示了基于样本的姿态估计在下游机器人操作任务中的应用，如主动感知消除不确定视点和不确定性感知的抓取合成。

Conclusion: 提出的概率框架能够有效处理姿态估计中的模糊性问题，为机器人操作任务提供更可靠的不确定性感知解决方案。
...
TL;DR: 提出DiMoDE框架，通过区分处理运动分量并利用其刚性流几何规律，改进深度和自运动联合学习，在多个数据集上实现最先进性能。


<details>
  <summary>Details</summary>
Motivation: 现有无监督深度和自运动学习方法通常将自运动作为辅助任务，要么混合所有运动类型，要么排除与深度无关的旋转运动，限制了强几何约束的引入，降低了在多样化条件下的可靠性和鲁棒性。

Method: 引入运动分量的区分处理，通过相机光轴和成像平面对齐，将光流转换并通过偏差量化对每个自运动分量单独施加几何约束。将联合学习重新表述为同轴和共面形式，通过闭式几何关系实现深度和每个平移分量的相互推导。

Result: DiMoDE框架在多个公共数据集和新收集的多样化真实世界数据集上实现了最先进的性能，特别是在挑战性条件下表现优异。

Conclusion: 通过区分处理运动分量并利用其几何规律，能够引入更针对性的约束，提高深度和自运动估计的可靠性和鲁棒性，为3D感知任务提供了有效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法主要关注低光/正常光图像间的确定性像素级映射，忽略了真实环境中亮度转换的连续物理过程，导致在缺乏正常光参考时性能下降。

Method: 将亮度转换重新概念化为强度坐标空间中的幂律分布，通过分层幂函数近似，用概率采样替代确定性映射；设计扩散前向过程自主发现亮度层间最优转换路径。

Result: 显著提升了实际场景中的性能，实现了更适应和通用的光照恢复；在有正常光参考的情况下，在领域特定数据集上获得优越性能，并在非参考数据集上具有更好的泛化能力。

Conclusion: LASQ框架通过统计量化方法有效解决了低光图像增强中重建保真度与跨场景泛化之间的平衡问题，为实际应用提供了更实用的解决方案。
...
TL;DR: 提出一个完整的纹理控制创作和编辑系统，能够生成具有局部特征（如污渍、撕裂、孔洞等）的逼真纹理，采用基于学习的无监督方法，无需人工标注。


<details>
  <summary>Details</summary>
Motivation: 自然界中普遍存在各种改变材料表面外观的瑕疵特征，将这些特征纳入合成过程对于生成逼真纹理至关重要。

Method: 采用基于学习的方法，利用无标注样本，通过无监督异常检测识别外观改变特征，自动聚类成语义连贯的组别，用于指导条件图像生成。

Result: 开发了一个从少量图像集合到多功能生成模型的完整流程，支持用户交互式创建和绘制任意尺寸纹理上的特征。

Conclusion: 该系统实现了纹理的受控创作和编辑，提出的基于扩散的编辑和无限平稳纹理生成算法具有通用性，可应用于其他场景。
...
TL;DR: 提出了一种新颖的对比学习框架NSYNC，通过生成负样本合成图像来提升大型文本到图像扩散模型的风格化能力，使用正交梯度更新方法消除正负样本中的共同特征，从而更好地捕捉独特风格。


<details>
  <summary>Details</summary>
Motivation: 当前文本条件图像生成方法虽然能生成逼真图像，但无法准确捕捉特定风格。即使在目标风格数据集上微调，模型仍然难以掌握风格特征。

Method: 使用对比学习框架，生成负样本合成图像集，与真实正样本图像一起训练。通过计算正负梯度，将正梯度投影到负梯度上得到正交分量，基于此更新参数，消除正负样本中的共同特征。

Result: 在多种画家和插画师风格上的实验表明，该方法在定量和定性评估上都优于基线方法。

Conclusion: NSYNC方法通过负样本合成和正交梯度更新，有效提升了文本到图像扩散模型的风格化能力，能够更好地捕捉独特艺术风格。
...
TL;DR: 提出PCD-ReID算法解决基站环境中被遮挡行人重识别问题，使用Transformer架构提取共享组件特征，在真实巡逻监控数据集上训练，相比ResNet50方法Rank-1准确率提升15.9%。


<details>
  <summary>Details</summary>
Motivation: 基站环境中的遮挡行人重识别对监控安防至关重要，但传统ResNet方法无法有效处理遮挡问题，遮挡会掩盖关键身体特征，增加识别难度。

Method: 设计基于Transformer的PCD网络，提取共享组件特征（如头盔、制服）；收集真实巡逻监控图像数据集（6个月、1万人、5万张图像）进行训练以避免过拟合。
...
TL;DR: NOA是一个基于napari的图形用户界面工具，旨在简化基于AI的类器官分析，集成了检测、分割、跟踪、特征提取等多种功能模块。


<details>
  <summary>Details</summary>
Motivation: 现有AI工具对无编程经验的生物学家可访问性有限，导致工作流程劳动密集且主要依赖手动操作，需要开发更易用的分析工具。

Method: 开发Napari Organoid Analyzer (NOA)，作为开源napari插件，整合多种最先进算法，提供检测、分割、跟踪、特征提取、自定义特征标注和基于机器学习的特征预测模块。
...
TL;DR: PixelVLA是首个支持像素级推理和多模态提示的视觉-语言-动作模型，通过新的视觉运动指令调优框架和两阶段自动标注流程，显著提升了机器人操控性能，同时大幅降低了预训练成本。


<details>
  <summary>Details</summary>
Motivation: 当前VLA模型存在两个主要限制：(i) 难以进行像素级场景理解，(ii) 过度依赖文本提示，降低了在真实世界环境中的灵活性。

Method: 提出PixelVLA模型，采用新的视觉运动指令调优框架，整合多尺度像素感知编码器和视觉提示编码器，并通过两阶段自动标注流程生成Pixel-160K大规模像素级标注数据集。

Result: 在三个标准VLA基准测试和两个VLA模型变体上，PixelVLA相比OpenVLA将操控成功率提升了10.1%-17.8%，同时仅需其1.5%的预训练成本。
...
TL;DR: 提出基于深度卷积生成对抗网络（DC-GAN）的方法生成合成MRI数据，并使用CNN分类器评估合成图像质量，结果显示合成图像与真实图像在脑肿瘤分类任务中表现相当。


<details>
  <summary>Details</summary>
Motivation: 解决医学影像（特别是MRI）数据有限的问题，传统方法难以生成逼真的医学图像，而GAN在合成医学图像方面具有潜力。

Method: 使用深度卷积生成对抗网络（DC-GAN）生成合成MRI数据，并采用卷积神经网络（CNN）分类器对真实和合成MRI数据进行脑肿瘤分类，以评估合成图像质量。

Result: 合成图像与真实图像在脑肿瘤分类任务中表现出可比较的性能，验证了GAN生成图像在下游任务中的有效性。

Conclusion: 基于DC-GAN的方法能够生成高质量的合成MRI数据，这些数据可以有效地用于下游医学影像分析任务，缓解数据稀缺问题。
...
TL;DR: 提出连续-离散二元视觉分词器(CDD-VT)，通过自适应分配图像基元数量来解决多模态大模型中理解与生成的统一问题。


<details>
  <summary>Details</summary>
Motivation: 解决多模态大模型中理解与生成统一化的挑战，克服连续分词器和离散分词器的各自缺陷：连续分词器虽然性能强但流程复杂，离散分词器概念优雅但信息损失严重。

Method: 受光的波粒二象性启发，将视觉数据视为量化码本中图像基元的灵活组合，根据样本复杂度自适应确定基元数量。包含两个核心组件：多样化量化基元（促进基元正交性）和动态基元分配器（评估样本复杂度）。

Result: 在重建、检索和分类任务上的广泛实验表明，CDD-VT在简洁可扩展的MLLM中实现了优于专用连续分词器和离散分词器的性能。

Conclusion: CDD-VT有效解决了连续与离散分词器之间的张力，在保持简洁架构的同时获得了强大的性能，为多模态大模型的统一理解与生成提供了可行方案。
...
TL;DR: Lite ENSAM是一个轻量级肿瘤体积分割模型，专门用于从带有RECIST标注的CT扫描中高效分割肿瘤体积，在MICCAI FLARE 2025比赛中取得了60.7%的Dice系数和63.6%的NSD评分。
...
Method: 提出了Lite ENSAM架构，这是ENSAM架构的轻量级适配版本，专门设计用于从带有RECIST标注的CT扫描中进行高效的肿瘤体积分割。

Result: 在MICCAI FLARE 2025 Task 1的隐藏测试集上，Dice相似系数达到60.7%，归一化表面Dice达到63.6%。在公开验证数据集上，平均总RAM时间为50.6GB，CPU平均推理时间为14.4秒。

Conclusion: Lite ENSAM能够有效地从RECIST标注中生成肿瘤体积分割，为临床肿瘤治疗评估提供了更可靠的自动化体积测量解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 解剖3D形状分类受限于网格数据的复杂性和缺乏标准化基准，需要鲁棒的学习方法和可复现的评估。

Method: 提出PSPooling非可学习网格池化算子，基于几何邻近度预计算节点对应集，实现并行化和可逆的池化与反池化操作；集成到自监督图自编码器中学习解剖感知表示。

Result: 在MedShapeNet19数据集上，PSPooling显著提高了重建保真度和低标签情况下的分类准确率，为医学3D形状学习建立了强基线。

Conclusion: PSPooling方法有效解决了医学高分辨率网格的处理问题，MedShapeNet19可作为解剖形状分类的广泛采用基准。
...
TL;DR: ViC是一个无需训练、基于视觉语言模型的零样本重排序和融合框架，通过序列化内容证据和检索器元数据，在视频检索任务中实现了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 解决异构检索器候选融合的长期挑战，特别是在复杂多模态数据（如视频）中，传统融合方法仅依赖排序或分数信号而忽略候选表示。

Method: 提出Vote-in-Context框架，将列表重排序和融合重新定义为VLM的零样本推理任务，使用S-Grid紧凑序列化表示视频，结合内容证据和检索器元数据进行自适应加权。

Result: 在MSR-VTT上实现87.1%(t2v)/89.0%(v2t)的Recall@1，在VATEX上达到99.6%(v2t)，相比之前最优方法提升高达+40 Recall@1。
...
TL;DR: 本文提出了一种将强化学习有效整合到基于扩散的图像修复模型中的新策略，重点解决修复任务强调保真度的特殊需求。


<details>
  <summary>Details</summary>
Motivation: 直接将现有RL方法应用于基于扩散的图像修复模型效果不佳，因为修复任务的目标与纯生成任务不同，更强调保真度。

Method: 使用图像质量评估(IQA)模型作为奖励函数，针对远离真实值的困难样本进行RL训练，并采用MLLM-based IQA模型进行分布对齐，结合自适应权重策略动态调整RL和SFT的组合。

Result: 该方法在各种修复任务的多个基准测试中表现出有效性，能够无缝提升基于扩散的修复模型的性能。

Conclusion: 提出的RL框架是即插即用的，可以无缝应用于基于扩散的修复模型，在各种修复任务中显著提升性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有的扩散模型在语义潜在空间中优化，虽然能产生丰富的可控光照效果，但由于潜在空间邻近性不能保证视觉空间中的物理正确性，经常产生不真实的结果，如过度曝光的高光、错位的阴影和错误的遮挡。

Method: 通过从模型输出中提取深度和法线图来监督模型，将光照效果与场景结构对齐；采用路径一致性学习，使监督在少步训练机制下仍然有效；设计了结构化的六维标注协议来捕获核心光照属性。

Result: UniLumos在重光照质量上达到了最先进水平，显著改善了物理一致性，同时为图像和视频重光照带来了20倍的速度提升。

Conclusion: UniLumos通过引入几何反馈和路径一致性学习，成功解决了现有重光照方法在物理一致性和计算效率方面的问题，提供了一个高效且物理上合理的重光照解决方案。
...
Conclusion: 渐进式结构-颜色-细胞边界机制有效解决了传统方法在IHC图像合成中的局限性，为病理诊断提供了更高效经济的解决方案。
...
TL;DR: 提出LFRD2框架，结合神经网络表达能力与物理模型可解释性，解决屏下ToF成像中的信号衰减、多路径干扰和时序噪声问题。


<details>
  <summary>Details</summary>
Motivation: 屏下ToF成像面临透明OLED层导致的严重退化问题，包括信号衰减、多路径干扰和时序噪声，严重影响深度感知质量。

Method: 采用可学习分数阶反应-扩散动力学框架，包含时间分数阶反应-扩散模块实现迭代深度优化，以及通过系数预测和重复微分的高效连续卷积算子。

Result: 在四个基准数据集上的实验证明了该方法的有效性。

Conclusion: LFRD2框架成功结合了神经网络和物理建模的优势，有效提升了屏下ToF成像的深度感知质量。
...
TL;DR: PRBench是首个专门评估概率鲁棒性训练方法的基准，比较了对抗训练和PR针对性训练方法在多个指标上的表现，发现对抗训练在提升AR和PR性能方面更通用，而PR针对性训练具有更低的泛化误差和更高的干净准确率。


<details>
  <summary>Details</summary>
Motivation: 深度学习模型对微小扰动非常脆弱。现有研究主要关注对抗鲁棒性，而概率鲁棒性从统计角度评估模型在随机扰动下的表现。虽然PR被视为AR的实用补充，但专门提升PR的训练方法仍相对缺乏，且现有方法存在评估协议不可比、与强对抗训练基线比较有限、缺乏统一框架等问题。

Method: 提出了PRBench基准，使用包括干净准确率、PR和AR性能、训练效率和泛化误差在内的综合指标集，对常见的对抗训练和PR针对性训练方法进行实证比较，并提供PR性能泛化误差的理论分析。

Result: 主要发现：对抗训练方法在提升AR和PR性能方面比PR针对性训练方法更通用，而PR针对性训练方法始终产生更低的泛化误差和更高的干净准确率。建立了包含222个训练模型的排行榜，涵盖7个数据集和10种模型架构。

Conclusion: PRBench为概率鲁棒性训练方法提供了首个标准化评估基准，揭示了不同训练方法的优势和局限，对抗训练在性能提升方面更通用，而PR针对性训练在泛化能力和干净准确率方面表现更好。
...
TL;DR: 开发了一个任务探索管道，使用聚类技术、因子分析和字符串编辑距离自动识别完成任务的关键全局和局部策略，以及不同长度的有意义的子任务。
...
Method: 开发任务探索管道，结合聚类技术、因子分析和字符串编辑距离，自动识别全局策略（完成任务的动作集合）和局部策略（使用这些动作的相似序列），并识别各种长度的有意义的子任务。

Result: 任务探索管道能够自动识别完成任务的关键策略，并用层次性子任务结构编码用户运行。还开发了Task Explorer应用程序来轻松查看管道结果。

Conclusion: 任务探索管道可轻松修改以适应任何基于动作的时间序列数据，识别的策略和子任务有助于向人类和机器传达用户知识、技能和行为信息。
...
TL;DR: 提出CGF-DETR模型，用于胸部X光肺炎检测，通过引入XFABlock、SPGA模块和GCFC3等改进，在保持实时性能的同时显著提升检测精度。


<details>
  <summary>Details</summary>
Motivation: 肺炎是全球发病率和死亡率的主要原因，需要准确高效的自动检测系统。虽然基于transformer的检测器在目标检测任务中表现良好，但在医学影像特别是胸部X光肺炎检测中的应用仍待探索。

Method: 提出CGF-DETR模型：1) 在主干网络引入XFABlock，通过卷积注意力机制与CSP架构改进多尺度特征提取；2) 提出SPGA模块，用动态门控机制和单头自注意力替代标准多头注意力；3) 设计GCFC3用于颈部网络，通过多路径卷积融合增强特征表示，同时通过结构重参数化保持实时性能。

Result: 在RSNA肺炎检测数据集上的实验表明，CGF-DETR达到82.2% mAP@0.5，比基线RT-DETR-l提升3.7%，同时保持48.1 FPS的推理速度。消融研究证实每个模块都对性能提升有贡献，完整模型达到50.4% mAP@[0.5:0.95]。

Conclusion: CGF-DETR通过精心设计的模块改进，在肺炎检测任务中实现了精度和速度的良好平衡，为医学影像分析提供了有效的实时检测解决方案。
...
TL;DR: 3EED是一个多平台、多模态的3D视觉语言基准数据集，包含车辆、无人机和四足机器人的RGB和LiDAR数据，规模是现有数据集的10倍，支持跨平台3D物体定位研究。


<details>
  <summary>Details</summary>
Motivation: 现有3D视觉语言基准局限于室内场景、单一平台和小规模，无法满足开放世界环境中多平台3D物体定位的需求。

Method: 开发了可扩展的标注流程，结合视觉语言模型提示和人工验证；提出平台感知归一化和跨模态对齐技术；建立了领域内和跨平台评估协议。

Result: 构建了包含128,000个物体和22,000个验证过的指代表达式的数据集，揭示了显著的性能差距，突出了可泛化3D定位的挑战和机遇。

Conclusion: 3EED数据集和基准工具包的发布将推动语言驱动的3D具身感知研究的发展。
...
<details>
  <summary>Details</summary>
Motivation: 解决2D到3D人体姿态提升中的深度模糊和2D姿态估计误差导致的3D轨迹不一致问题。现有方法主要约束相邻帧间的差异，但忽略了骨骼关节运动的全局时空相关性。

Method: 提出跳数混合图注意力模块和Transformer编码器来建模全局关节时空相关性。HGA模块将骨骼关节的所有k跳邻居分组为混合组以扩大感受野，并应用注意力机制全局发现这些组的潜在相关性。通过在频域约束轨迹一致性来利用全局时间相关性。使用初步网络估计3D姿态以提供跨帧的深度推断信息。

Result: 在Human3.6M和MPI-INF-3DHP两个标准基准数据集上的广泛实验表明，HGFreNet在位置精度和时间一致性方面优于最先进方法。

Conclusion: HGFreNet通过跳数混合特征聚合和频域轨迹一致性约束，有效提升了2D到3D人体姿态估计的准确性和时间一致性。
...
TL;DR: UniLION是一个统一的自动驾驶模型，使用线性组RNN操作器高效处理大规模LiDAR点云、高分辨率多视角图像和时间序列，无需显式的时间或多模态融合模块，在多种核心任务中表现优异。


<details>
  <summary>Details</summary>
Motivation: Transformer的二次注意力机制在处理长序列数据时计算开销大，需要一种更高效的架构来处理自动驾驶中的大规模多模态数据。

Method: 基于线性组RNN操作器（对分组特征执行线性RNN），构建统一的单一架构，支持LiDAR-only、时序LiDAR、多模态和多模态时序融合等多种配置。

Result: 在3D感知（目标检测、跟踪、占用预测、BEV地图分割）、预测（运动预测）和规划（端到端规划）等核心任务中展现出竞争力和最先进性能。

Conclusion: UniLION为自动驾驶3D基础模型的开发提供了新视角，简化了多模态多任务系统的设计，同时保持优异性能。
...
TL;DR: 提出了一种基于提示设计的GraphRAG框架，通过优化提示设计来改进实体提取、事实选择和段落重排序，在HotpotQA和2WikiMultiHopQA数据集上取得了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 虽然基于图的检索增强生成在复杂推理方面取得了进展，但提示设计对检索和推理过程的影响尚未得到充分研究。

Method: 构建符号知识图谱表示实体和事实关系，使用个性化PageRank进行实体引导的图遍历，在在线检索中选择性使用LLM进行语义过滤和答案生成。
...
TL;DR: Scitextures数据集是一个大规模的科学纹理和视觉模式集合，包含1200多个模型和10万张图像，用于探索视觉模式与生成机制之间的联系。通过AI代理流水线创建，用于评估AI模型理解视觉模式与生成代码关系的能力。


<details>
  <summary>Details</summary>
Motivation: 探索视觉模式与形成机制之间的深层联系，理解云、波浪、城市生长等自然和人工模式背后的生成过程。

Method: 使用AI代理流水线自主收集和实现标准化模型，创建大规模纹理数据集。通过让AI识别真实世界模式、建模生成机制并运行代码生成模拟图像来测试其理解能力。

Result: 视觉语言模型能够超越视觉模式本身，理解和模拟物理系统。AI能够识别相同过程产生的不同模式，并能推断和重建视觉模式背后的机制。

Conclusion: Scitextures数据集为研究视觉模式与生成机制的关系提供了重要资源，展示了AI在理解复杂视觉模式形成过程方面的潜力。
...
TL;DR: 提出了TIR-Bench基准测试，用于评估视觉推理中基于工具使用的图像处理能力，测试了22个多模态大语言模型，发现该基准具有普遍挑战性。


<details>
  <summary>Details</summary>
Motivation: 现有基准测试无法充分捕捉像OpenAI o3这样的模型在视觉推理中创建和操作工具来解决问题的先进能力，即使是常用的Visual Search基准也只测试基本操作。

Method: 开发了包含13个多样化任务的TIR-Bench基准，每个任务都需要在思维链中进行新颖的图像处理工具使用，并评估了22个MLLM模型。

Result: TIR-Bench对所有测试模型都具有挑战性，强性能需要真正的图像思维能力，并进行了直接与代理式微调的初步比较研究。

Conclusion: TIR-Bench是评估代理式图像思维能力的全面基准，揭示了当前模型在复杂视觉推理任务中的局限性。
...
TL;DR: 提出了PlotCraft基准测试来评估LLM在复杂数据可视化生成方面的能力，并开发了PlotCraftor模型，在多个基准测试中表现出色。


<details>
  <summary>Details</summary>
Motivation: 当前大型语言模型在代码生成方面表现出色，但在处理复杂可视化任务方面的能力尚未得到充分评估和发展。

Method: 创建了包含1000个挑战性可视化任务的PlotCraft基准测试，涵盖7个高级任务和48种图表类型；开发了SynthVis-30K数据集和PlotCraftor模型。

Result: 对23个领先LLM的评估显示在复杂可视化任务上存在明显性能缺陷；PlotCraftor模型在多个基准测试中表现优异，在困难任务上性能提升超过50%。

Conclusion: PlotCraft基准测试填补了LLM在复杂可视化能力评估方面的空白，PlotCraftor模型展示了在小规模下实现强大可视化生成能力的潜力。
...
TL;DR: ProtoMBTI是一个基于原型理论的MBTI人格识别框架，通过LLM引导的数据增强、原型标准化和检索-重用-修订-保留推理循环，在文本人格识别任务中取得了优于基线方法的表现。


<details>
  <summary>Details</summary>
Motivation: 传统的人格识别方法采用硬标签分类，忽视了人类人格判断的渐进性和原型特性。本文旨在开发一个与认知过程对齐的人格推理框架。

Method: 1. 使用LLM引导的多维度数据增强构建平衡语料库；2. 通过LoRA微调轻量级编码器学习判别性嵌入和标准化人格原型；3. 推理时采用检索-重用-修订-保留循环，通过基于提示的投票聚合原型证据。
...
TL;DR: 开发了残差流解码器框架，用于探测语言模型中段落和文档级别的规划信息，发现可以解码相当于5个以上未来token的信息。


<details>
  <summary>Details</summary>
Motivation: 随着语言模型能够处理更长时程任务，现有方法通常局限于测试特定概念或token，需要开发能理解模型激活中长期规划信息的方法。

Method: 提出了残差流解码器框架，测试了多种方法来探测模型激活中的段落和文档尺度规划信息。

Result: 在小型模型中，可以解码相当于5个以上未来token的上下文信息。
...
TL;DR: 本文提出了一种替代传统下一词预测(NTP)的LLM训练方法，通过预测信息丰富的token来优化训练效果


<details>
  <summary>Details</summary>
Motivation: 挑战传统LLM训练中使用下一词预测的方法，寻求在保持计算成本的同时提升模型性能的优化方案

Method: 提出预测信息丰富token的训练策略，并在算术、多标签文本分类和自然语言生成三类任务中进行验证

Result: 该方法为LLM训练提供了原则性优化方案，提升了模型性能并加深了对目标token选择策略的理论理解

Conclusion: 通过选择性地预测信息丰富的token而非简单下一词，可以更有效地训练大型语言模型
...
Method: 定义了三种自动指标评估角色一致性，并以此作为奖励信号，应用多轮强化学习微调LLM，针对患者、学生和社交聊天伙伴三种角色进行优化。

Result: 该方法将不一致性降低了55%以上，产生了更连贯和忠实的模拟用户。

Conclusion: 提出的框架有效提升了LLM在角色扮演中的一致性，为可扩展的AI代理训练和评估提供了更可靠的模拟用户。
...
TL;DR: AgentBnB是一个基于浏览器的网络安全桌面演习系统，集成大语言模型队友和检索增强助手，提供按需认知提示，相比传统演习更轻量、可扩展。


<details>
  <summary>Details</summary>
Motivation: 传统网络安全桌面演习脚本化、资源密集且难以扩展，需要更轻量、可重复的练习方式。

Method: 重新设计Backdoors & Breaches游戏，集成大语言模型队友和检索增强助手(C2D2)，使用脚手架策略提供认知提示，支持渐进式学习。

Result: 在4名研究生的试点中，参与者更倾向于使用基于代理的版本，认为其更具可扩展性，但在简单知识测验中出现天花板效应。

Conclusion: 大语言模型增强的桌面演习可以提供轻量级、可重复的练习，无需传统演习的后勤负担，未来计划扩展多人模式和改进评估。
...
TL;DR: 本文提出了IL-PCR语料库，为法律案例检索和法规检索任务提供统一测试平台，并开发了基于LLM的重排序方法以利用两个任务之间的依赖关系。


<details>
  <summary>Details</summary>
Motivation: 现有研究将法律案例检索和法规检索作为独立任务处理，但实际上这两个任务具有内在关联性（相似案例往往引用相似法规），需要统一框架来利用这种依赖关系。

Method: 构建IL-PCR语料库作为统一测试平台，实验了词法模型、语义模型和基于GNN的集成模型，并开发了基于LLM的重排序方法来利用两个任务间的依赖关系。

Result: 基于LLM的重排序方法在所有基线模型中表现最佳，证明了利用案例检索和法规检索任务间依赖关系的有效性。

Conclusion: IL-PCR语料库为法律检索任务提供了有价值的测试平台，基于LLM的重排序方法能够有效利用案例和法规检索任务间的依赖关系，显著提升检索性能。
...
TL;DR: 提出了POSESTITCH-SLT预训练方案，通过模板生成句子对来提升手语翻译性能，在两个数据集上显著超越现有方法


<details>
  <summary>Details</summary>
Motivation: 手语翻译面临大规模句子对齐数据集稀缺的挑战，需要新的方法来在低资源环境下提升翻译质量

Method: 提出POSESTITCH-SLT预训练方案，基于语言模板的句子生成技术，使用简单的transformer编码器-解码器架构

Result: 在How2Sign数据集上BLEU-4从1.97提升到4.56，在iSign数据集上从0.55提升到3.43，超越了基于姿态的无词汇表翻译的现有最佳方法

Conclusion: 模板驱动的合成监督在低资源手语设置中非常有效，证明了该方法在解决数据稀缺问题上的潜力
...
TL;DR: 提出Factorization Memory，一种高效的RNN架构，在短上下文语言建模任务中性能媲美Transformer，在长上下文场景中表现更优，结合了训练时的并行计算和推理时的恒定复杂度。


<details>
  <summary>Details</summary>
Motivation: 开发一种既能媲美Transformer性能，又能在长上下文场景中更好泛化的高效RNN架构，同时保持训练并行化和推理恒定复杂度的优势。

Method: 基于Mamba-2构建Factorization Memory，开发稀疏公式化版本，仅更新部分循环状态，同时保持密集版本的强大性能。

Result: Factorization Memory在短上下文任务中性能与Transformer相当，在长上下文场景中表现更优，是首个成功结合稀疏内存激活并在短长上下文设置中都具有竞争力的RNN架构。

Conclusion: Factorization Memory提供了一种系统化的高效RNN解决方案，在保持训练并行化和推理效率的同时，实现了与Transformer相媲美的性能，并在长上下文场景中表现更优。
...
TL;DR: 论文形式化定义了因果语言建模目标的结构特性——反转不变性，指出标准CLM预训练是方向盲的，这限制了模型捕捉语言中的方向性依赖关系。


<details>
  <summary>Details</summary>
Motivation: 当前因果语言建模目标对文本及其反转版本赋予相同似然度，这种对称性可能无法捕捉人类语言和推理中固有的时间不对称性。

Method: 通过形式化分析反转不变性特性，论证标准CLM预训练是方向盲的，并提出从时间不对称性视角重新审视预训练。

Result: 研究发现模型在反转文本上训练能达到与正向文本相当的性能，这揭示了当前预训练目标的局限性。

Conclusion: 建议开发能够显式建模语言方向性同时保持标准语言建模能力的损失函数和架构。
...
TL;DR: LingGym是一个评估LLMs元语言推理能力的新基准，使用跨语言注释文本和语法描述，测试模型在低资源语言和未见结构上的泛化能力。
...
Method: 使用来自18种类型多样参考语法的跨语言注释文本和语法描述，设计了单词-注释推理任务，模型需要从上下文推断缺失的单词和注释。

Result: 整合结构化语言线索在所有模型中都能持续提升推理性能。
...
TL;DR: 该论文提出了推理轨迹生成任务，用于苏格拉底式调试，通过引导性推理路径帮助学生识别和修正编程误解，并展示了前沿LLM模型能够生成91%正确的推理轨迹和98.7%有效的对话轮次。


<details>
  <summary>Details</summary>
Motivation: 苏格拉底式调试中，教师通过引导而非直接提供修复来帮助学生发现和修正编程误解。大多数新手程序员的错误源于编程误解，因此需要一种系统化的方法来生成引导性推理轨迹。

Method: 引入推理轨迹生成任务，创建了手动标注的调试问题数据集，开发了基于LLM的解决方案来生成推理轨迹和基于这些轨迹的苏格拉底式对话。

Result: 通过大规模LLM作为评判者的评估显示，前沿模型能够生成高达91%正确的推理轨迹和98.7%有效的对话轮次。

Conclusion: LLM能够有效生成用于苏格拉底式调试的推理轨迹和对话，为编程教育中的调试指导提供了有前景的自动化解决方案。
...
TL;DR: 论文发现AI生成文本检测器对直接LLM输出准确率超过90%，但对迭代改写的内容检测失败。迭代改写创建了语义位移但保留生成模式的中间洗白区域，导致两种攻击：改写人类文本（作者身份混淆）和改写AI生成文本（抄袭逃避）。


<details>
  <summary>Details</summary>
Motivation: 研究为什么迭代改写的AI生成文本能够逃避现有的AI生成文本检测系统，揭示检测器的脆弱性。

Method: 通过内在机制分析揭示迭代改写创建中间洗白区域，并构建PADBen基准系统评估检测器在两种改写攻击场景下的鲁棒性。

Result: 评估11个最先进检测器发现关键不对称性：检测器能识别抄袭逃避问题，但在作者身份混淆问题上失败，无法有效处理中间洗白区域。

Conclusion: 当前检测方法无法有效处理中间洗白区域，需要在检测架构上进行根本性改进，超越现有的语义和风格区分方法。
...
TL;DR: MedRECT是首个跨语言医疗错误修正基准，包含日语和英语版本，评估LLM在医疗错误检测、定位和修正三个子任务上的表现。研究发现推理模型表现最佳，跨语言评估显示英语到日语存在5-10%性能差距，微调后模型在结构化医疗错误修正任务上超越人类专家。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在医疗应用中前景广阔，但其在临床文本中检测和修正错误的能力——安全部署的前提条件——尤其在英语之外的语言中仍未被充分评估。

Method: 从日本医师国家考试构建可扩展的自动化流程，创建MedRECT基准（日语663文本，英语458文本），评估9个当代LLM，包括专有、开源和推理模型，并进行针对性LoRA微调。

Result: 推理模型显著优于标准架构，错误检测相对提升13.5%，句子提取提升51.0%；英语到日语存在5-10%性能差距；微调后模型在日语和英语的错误修正性能分别提升0.078和0.168；微调模型在结构化医疗错误修正任务上超越人类专家。

Conclusion: MedRECT是首个全面的跨语言医疗错误修正基准，为开发更安全的跨语言医疗LLM提供了可复现框架和资源，推理模型和针对性微调能显著提升医疗错误处理能力。
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型在输出多样性方面存在局限，经常生成高度相似的内容，这影响了需要多样输出的任务。现有方法如温度缩放虽然能增强多样性但会牺牲输出质量。

Method: 使用基础生成器和双引导机制，通过基于解码的干预来指导生成过程，在原始查询条件下鼓励更多样化的输出。

Result: 综合实验表明，G2有效提高了输出多样性，同时在多样性和质量之间保持了最佳平衡。

Conclusion: G2方法成功解决了LLM输出多样性不足的问题，在不牺牲质量的前提下显著提升了生成内容的多样性。
...
TL;DR: 该研究探讨了大语言模型（LLMs）在科学计量学应用中的记忆效应如何影响合著网络的公平性和准确性，分析了三个主流模型在不同学科和地区的表现差异。


<details>
  <summary>Details</summary>
Motivation: 随着LLMs在学术搜索和推荐系统中的广泛应用，其记忆数据生成摘要的能力可能放大科学界现有的偏见，威胁信息生态系统的完整性，需要系统评估这种影响。

Method: 评估了DeepSeek R1、Llama 4 Scout和Mixtral 8x7B三个模型的记忆效应，分析记忆驱动输出在学术学科和世界区域间的差异。

Result: 全球分析显示LLMs普遍偏向高被引研究者，但这种模式并不一致。临床医学等学科和非洲部分地区表现出更平衡的代表性，表明训练数据在这些领域可能更公平。

Conclusion: LLMs在学术发现应用中既存在风险也蕴含机遇，需要关注其记忆效应对合著网络准确性和公平性的影响，特别是在不同学科和地区的差异表现。
...
TL;DR: 本文构建了首个大规模Bhili-Hindi-English平行语料库(BHEPC)，包含11万句经过精心整理的句子，填补了Bhili等低资源部落语言机器翻译的资源空白。


<details>
  <summary>Details</summary>
Motivation: 印度语言多样性带来机器翻译挑战，特别是Bhili等代表性不足的部落语言缺乏高质量语言资源，需要填补这一资源空白。

Method: 创建BHEPC语料库，涵盖教育、行政和新闻等关键领域；评估专有和开源多语言大语言模型在英语/印地语与Bhili之间的双向翻译任务；使用上下文学习研究生成式翻译能力。
...
TL;DR: 该论文首次在差分隐私文本隐私化评估中引入数据集规模因素，通过在大规模数据集上进行效用和隐私测试，发现数据集规模对隐私-效用权衡有重要影响。


<details>
  <summary>Details</summary>
Motivation: 现有差分隐私自然语言处理研究在评估文本重写机制时忽略了数据集规模的影响，需要研究数据集规模对机制效用和隐私保护效果的影响。

Method: 设计在大规模数据集上的效用和隐私测试，使用动态分割大小，在包含多达100万文本的不同规模数据集上进行实验。

Result: 研究发现数据集规模在评估差分隐私文本重写机制中起着关键作用，数据集规模增加会影响隐私-效用权衡。
...
TL;DR: 提出ToM框架，通过树状MapReduce方法解决大语言模型在长上下文推理中的逻辑连贯性问题


<details>
  <summary>Details</summary>
Motivation: 现有方法如RAG和分治法在长上下文推理中存在逻辑连贯性差、无法捕捉长距离依赖关系的问题

Method: 利用文档的层次结构构建DocTree，通过树状MapReduce进行自底向上的递归推理：Map步骤在子节点生成推理，Reduce步骤在父节点聚合推理结果

Result: 在70B+大语言模型上的实验表明，ToM显著优于现有分治框架和检索增强生成方法

Conclusion: ToM框架通过层次化语义解析和树状MapReduce，有效提升了长上下文推理的逻辑连贯性和性能
...
TL;DR: Zero-RAG通过识别和修剪RAG外部语料库中的冗余知识，减少检索开销并提高LLM内部知识的利用率，在保持性能的同时加速检索过程。


<details>
  <summary>Details</summary>
Motivation: 随着LLM内部知识的扩展，RAG中的外部语料库与LLM之间存在显著的知识冗余，这不仅增加了密集检索的工作量，还会损害LLM能够自行回答问题的RAG性能。

Method: 提出Mastery-Score指标识别冗余知识进行语料库修剪；使用Query Router和Noise-Tolerant Tuning避免不相关文档的干扰，提高LLM对内部知识的利用。

Result: 实验结果显示，Zero-RAG将维基百科语料库修剪了30%，检索阶段加速了22%，且不损害RAG性能。

Conclusion: Zero-RAG有效解决了RAG中的知识冗余问题，通过修剪冗余知识和优化内部知识利用，实现了检索加速和性能保持。
...
TL;DR: 在尼泊尔农村地区开发离线运行的医疗对话AI，通过微调轻量级DialoGPT模型，在十种常见疾病数据集上训练，生成连贯且医学适当的医疗对话响应。


<details>
  <summary>Details</summary>
Motivation: 解决农村地区因网络连接和云基础设施不足而无法使用大规模对话模型的问题，为资源受限环境提供离线医疗对话支持。

Method: 在合成的医生-患者交互数据集上微调轻量级生成对话模型DialoGPT，数据集涵盖尼泊尔农村十种常见疾病。

Result: 尽管训练数据有限且领域特定，微调后的模型能够生成连贯、上下文相关且医学适当的响应，表现出对症状、疾病背景和共情沟通的理解。

Conclusion: 紧凑型离线对话模型具有良好适应性，针对性数据集在低资源医疗环境中有效，为未来农村医疗对话AI提供了有前景的方向。
...
TL;DR: 该论文研究了基于transformer的编码器模型中的性别偏见问题，提出了新的度量指标MALoR，并通过反事实数据增强的持续预训练方法有效降低了性别偏见。


<details>
  <summary>Details</summary>
Motivation: 自然语言处理中的语言模型被发现存在显著的性别偏见，这些偏见从训练数据中继承而来。本研究旨在调查BERT、ALBERT、RoBERTa和DistilBERT等主流transformer模型在上下文词嵌入中的性别偏见问题。

Method: 引入新的度量指标MALoR来量化偏见程度，基于模型填充掩码标记的概率。提出通过反事实数据增强生成性别平衡数据集，并进行持续预训练的缓解方法。

Result: 实验显示缓解方法显著降低了不同代词对的性别偏见分数。例如，BERT-base中"he-she"的偏见分数从1.27降至0.08，"his-her"从2.51降至0.36。BERT-large中"male-female"偏见从1.82降至0.10。

Conclusion: 该方法在不影响下游任务性能的前提下，有效减少了transformer模型中的性别偏见，为构建更公平的语言模型提供了可行方案。
...
TL;DR: 论文提出WordSaladChopper(WSC)组件，通过检测大型推理模型中的无用自我重复(词沙拉)来显著减少输出token成本，仅需单层线性分类器即可实时检测，然后通过简单截断和重新生成提示来节省长度。


<details>
  <summary>Details</summary>
Motivation: 大型推理模型输出token成本高昂，其中很大一部分是无用的自我重复(词沙拉)，这些token消耗解码预算但不增加价值。

Method: 利用模型对<\n\n>标记的隐藏状态模式，通过单层线性分类器实时检测词沙拉行为，检测后采用简单截断加重新生成提示的方法。

Result: WSC组件能显著节省输出长度，同时质量损失最小，是一个轻量级、即插即用的解决方案。

Conclusion: WSC或类似组件是所有考虑用户体验的大型推理模型应用必备的工具，因其开销低、节省显著且词沙拉token缺乏语义价值。
...
TL;DR: 提出CISEA-MRFE框架，结合上下文指令、语义增强增强和多细化特征提取，显著提升情感分析在复杂场景下的性能


<details>
  <summary>Details</summary>
Motivation: 现有基于深度学习和预训练语言模型的情感分析方法在细微情感线索、领域迁移和不平衡情感分布场景下表现不佳，存在语义基础不足、泛化能力差和偏向主导情感类别的问题

Method: CISEA-MRFE框架包含：上下文指令(CI)注入领域感知指令指导情感消歧；语义增强增强(SEA)通过情感一致性释义增强提升鲁棒性；多细化特征提取(MRFE)结合尺度自适应深度编码器(SADE)和多尺度特征专业化，以及情感评估器上下文编码器(EECE)进行情感感知序列建模

Result: 在四个基准数据集上的实验结果显示，CISEA-MRFE始终优于强基线方法，在IMDb上准确率相对提升4.6%，Yelp上6.5%，Twitter上30.3%，Amazon上4.1%
...
TL;DR: ISA攻击通过意图转换使LLMs将有害请求误解为良性信息请求，仅需对原始请求进行最小编辑，就能显著提升攻击成功率，现有防御方法对此无效。


<details>
  <summary>Details</summary>
Motivation: 现有越狱攻击主要通过引入额外上下文或对抗性token来分散LLMs注意力，但未改变核心有害意图。需要研究更隐蔽的攻击方式以揭示LLMs安全机制的深层漏洞。

Method: 建立意图转换分类法，利用这些转换生成可能被LLMs误解为良性信息请求的攻击。该方法仅需对原始请求进行最小编辑，生成自然、人类可读且看似无害的提示。

Result: 在开源和商业LLMs上的实验显示，ISA相比直接有害提示的攻击成功率提升超过70%。仅使用ISA模板重新表述的良性数据微调模型，成功率接近100%。现有防御方法对ISA无效。

Conclusion: 研究揭示了LLMs在意图推断方面的根本性安全挑战，强调需要更有效的防御机制来应对此类隐蔽攻击。
...
TL;DR: FlashEVA是一种高效的注意力机制实现，通过控制变量方法优化Transformer推理性能，在仅用15亿token微调的情况下，实现推理吞吐量提升6.7倍、GPU内存使用降低5倍。


<details>
  <summary>Details</summary>
Motivation: Transformer模型在推理过程中由于需要维护完整上下文而面临显著的内存挑战，限制了其实际部署效率。

Method: 提出FlashEVA作为EVA（通过控制变量的高效注意力）的高效实现，并展示如何微调Transformer模型以适应FlashEVA注意力机制。

Result: FlashEVA在推理时相比标准Transformer实现，吞吐量最高提升6.7倍，GPU峰值内存使用降低5倍，同时在下游任务中保持有效性。

Conclusion: 该工作通过可调节超参数在吞吐量和准确性之间提供权衡控制，为更高效和适应性强的基于Transformer的推理模型迈出重要一步。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于强化学习的LLM推理方法依赖带注释数据集的验证奖励，这可能限制模型超越人类水平的能力。自学习提供了有前景的替代方案，但现有方法需要外部验证器或无法实现开放式学习。

Method: OpenSIR采用自学习框架，LLM通过交替扮演教师和学生角色来生成和解决问题，无需外部监督。通过优化难度和多样性来生成新颖问题，奖励那些既能适当挑战又能探索不同概念的问题。
...
Conclusion: OpenSIR通过共同进化的教师-学生角色实现开放式学习，这些角色自适应地校准难度并推动多样化探索，能够自主地从基础数学进展到高级数学。
...
TL;DR: SpecDiff-2是一种新的推测解码框架，通过使用离散扩散作为非自回归草稿器来解决现有方法的两个瓶颈，在推理、编码和数学基准测试中实现了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 当前推测解码方法存在两个基本瓶颈：草稿阶段的自回归依赖限制了并行性，以及草稿模型与验证模型之间的不匹配导致频繁的草稿令牌拒绝。

Method: 利用离散扩散作为非自回归草稿器来解决瓶颈(1)，并开发新技术来校准离散扩散草稿器与自回归验证器，解决瓶颈(2)。

Result: 在综合基准测试套件中，SpecDiff-2相比之前基线平均提高了+55%的每秒令牌数，相比标准解码获得了最高5.5倍的平均加速，且没有任何精度损失。

Conclusion: SpecDiff-2通过联合解决两个基本瓶颈，在推理、编码和数学基准测试中实现了新的最先进性能，显著加速了大型语言模型的推理。
...
TL;DR: 该研究评估了大型语言模型在概率场景中的不确定性量化能力，发现虽然模型能给出正确回答，但其token级别的概率分布与理论概率分布存在偏差。


<details>
  <summary>Details</summary>
Motivation: 确保大型语言模型在决策支持等知识密集型应用中的可靠不确定性量化，特别是在概率场景中模型输出的概率应与理论概率分布一致。

Method: 使用GPT-4.1和DeepSeek-Chat模型，评估它们对10个概率提示（如掷骰子）的响应，测量响应有效性和token输出概率与理论概率的匹配度。

Result: 两个模型在所有提示场景中都实现了完美的领域内响应准确性，但其token级别的概率和熵值持续偏离相应的理论分布。

Conclusion: 尽管LLMs能正确回答概率问题，但其内部token概率分布与理论概率不一致，表明当前的不确定性量化方法在概率场景中可能不足够可靠。
...
TL;DR: 通过计算分析方法研究法国侦探小说中侦探原型的演变，发现监督模型能捕捉150年间侦探原型的统一性，并展示其从次要角色到核心推理机器的演变过程。


<details>
  <summary>Details</summary>
Motivation: 探索法国侦探小说中侦探原型在150年间的演变轨迹，理解文学类型中角色原型的发展规律。

Method: 使用定量方法和字符级嵌入技术，构建监督模型分析从1866年到2017年的法国侦探小说文本。

Result: 模型成功捕捉到侦探原型的统一性，发现侦探角色从次要叙事角色发展为古典侦探故事的核心推理机器，二战后受硬汉派影响变得更加复杂。

Conclusion: 法国侦探小说中的侦探原型经历了从简单到复杂的演变过程，反映了文学类型和社会背景的变化，计算分析方法能有效揭示这种演变规律。
...
TL;DR: XNationQA是一个新的多语言问答基准，涵盖9个国家的地理、文化和历史问题，包含49,280个问题，使用7种语言，旨在评估多语言LLM的文化素养。
...
Method: 创建XNationQA基准，包含49,280个关于9个国家地理、文化和历史的问题，使用7种语言呈现。对8个标准多语言LLM进行基准测试，并使用两种新的传递性指标进行评估。

Result: 发现模型在不同语言中获取文化特定事实的能力存在显著差异；模型在英语中通常比在相应文化的主导语言中表现出更大的文化知识；模型在西方语言中表现更好，但这并不一定意味着对西方国家更了解；开源模型在跨语言知识传递方面能力非常有限。

Conclusion: 多语言LLM在文化素养方面存在显著偏差，需要开发更公平的评估基准来准确衡量模型对不同文化的理解能力。
...
TL;DR: 本文首次系统评估了多语言环境下LLM越狱攻击与防御的跨语言泛化能力，在10种不同资源水平的语言上测试了6个LLM，发现攻击成功率和防御鲁棒性存在语言差异。


<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注英语环境下的LLM安全对齐和越狱攻击，但跨语言环境下的安全泛化能力尚未得到充分探索，需要评估不同语言资源水平对安全性的影响。

Method: 在10种语言（高、中、低资源）上使用HarmBench和AdvBench基准，评估了基于逻辑表达式和对抗提示两种越狱攻击类型，测试了6个LLM模型的安全性能。

Result: 高资源语言在标准查询下更安全，但对对抗攻击更脆弱；简单防御方法有效但具有语言和模型依赖性；攻击成功率和防御鲁棒性在不同语言间存在显著差异。

Conclusion: 研究结果表明需要开发语言感知和跨语言的安全基准测试，以全面评估LLM在不同语言环境下的安全性。
...
TL;DR: 本文系统分析了原生稀疏注意力(NSA)，提出了交替使用局部和全局注意力的改进方法，通过引入潜在注意力机制，在减少50% KV缓存的同时提升了长文本理解能力。


<details>
  <summary>Details</summary>
Motivation: 改进原生稀疏注意力机制，使其在长上下文建模中更有效地传播长距离依赖关系，同时降低内存消耗。

Method: 在注意力层中交替使用局部(滑动窗口)和全局(压缩、选择性)注意力模式，并引入多头潜在注意力(MLA)和分组头潜在注意力(GLA)来增强各分支。

Result: 在3.4亿到13亿参数的模型上实验表明，该方法在常识推理和长上下文理解任务中匹配或超越了全注意力和原生稀疏注意力的性能，同时减少了50%的KV缓存内存。

Conclusion: 交替注意力模式和潜在注意力机制的结合能够有效提升长序列建模能力，在保持性能的同时显著降低内存需求。
...
TL;DR: TriCon-Fair是一个对比学习框架，通过解耦损失函数结合三元组和语言建模目标，消除偏见样本和无偏见样本之间的负-正耦合，有效减少语言模型的社会偏见传播。


<details>
  <summary>Details</summary>
Motivation: 现有去偏见方法独立处理偏见和无偏见样本，忽略了它们之间的相互关系，导致隐藏的负-正耦合问题，即改进一个群体时无意中损害另一个群体，使残留的社会偏见持续存在。

Method: 提出TriCon-Fair对比学习框架，使用解耦损失函数结合三元组和语言建模目标，为每个锚点分配明确的偏见负样本和无偏见正样本，解耦推拉动态并避免正负耦合，同时联合优化语言建模目标以保持通用能力。

Result: 实验结果表明，TriCon-Fair在保持强大下游性能的同时，比现有去偏见基线方法更有效地减少歧视性输出。
...
TL;DR: 提出了一个评估大语言模型推理过程中知识基础的新框架，包含大规模知识库、知识基础评估指标和轻量级评估模型，能够识别推理中的知识缺失或误用问题。


<details>
  <summary>Details</summary>
Motivation: 随着逐步推理成为LLMs处理复杂任务的标准方法，需要验证其推理是否准确基于知识，解决知识基础评估的根本问题。

Method: 构建包含三个关键组件的评估框架：(1)大规模原子知识库，(2)基于知识库设计的评估指标来衡量模型在推理中回忆和应用先验知识的能力，(3)优化的轻量级评估LLM用于成本效益高的指标计算。

Result: 评估套件在识别推理中缺失或误用的知识元素方面表现出显著有效性，为揭示LLMs基本推理缺陷提供了关键洞察。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法在检索多模态文档时往往复制纯文本检索技术，无法充分处理文档的结构和视觉特征，需要专门为多模态文档设计的检索方法。

Method: 使用OCR预训练目标、自监督掩码对比学习目标和延迟交互评分机制，专门针对多模态文档结构和视觉特征进行优化。
...
<details>
  <summary>Details</summary>
Motivation: 评估LLMs在支持临床医生进行诊断沟通方面的潜力，特别是生成患者可理解且具有共情的解释和指导的能力。

Method: 在医疗诊断场景中评估两个领先的LLMs，使用可读性指标作为可理解性的代理，通过LLM-as-a-Judge评分与人类评估比较来评估共情能力。

Result: LLMs能够根据社会人口学变量和患者状况调整解释，但生成了过于复杂的内容，并表现出有偏见的情感共情，导致可及性和支持不平等。
...
TL;DR: 本文评估了5个LLM在7种印度语言上的推理和自评估能力，发现模型初始准确率与识别自身错误的能力呈负相关，高性能模型过度自信，低性能模型反而更自知。


<details>
  <summary>Details</summary>
Motivation: 探索大语言模型在非英语语言中进行文化基础推理的能力，特别是在印度语言中的表现，目前这方面研究不足。

Method: 构建多语言谜语数据集，包含传统谜语和上下文重构变体，评估5个LLM在7种印度语言上的表现，采用7种提示策略，分两阶段评估：谜语解决性能和自评估实验。

Result: Gemini 2.5 Pro整体表现最佳，但少样本方法仅带来边际收益，准确率在不同语言间差异显著。自评估显示：模型初始准确率与识别错误能力呈负相关，高性能模型过度自信（真负率4.34%），低性能模型更自知（真负率42.09%）。
...
TL;DR: 提出了一种易到难的增强框架来解决机器生成文本检测中的边界模糊问题，通过使用针对较长文本的简单监督器来增强目标检测器的性能。
...
Method: 采用易到难的增强框架，使用针对较长文本检测任务的简单监督器（尽管能力较弱）来增强更具挑战性的目标检测器。通过将检测器结构性地整合到监督器中，理论上将监督器建模为检测器的性能下界。

Result: 在跨LLM、跨领域、混合文本和改写攻击等多种实际场景中的广泛实验表明，该框架具有显著的检测效果。

Conclusion: 该框架通过优化监督器间接优化检测器，最终逼近潜在的"黄金"标签，为解决机器生成文本检测中的不精确学习问题提供了有效方案。
...
TL;DR: MARS-SQL是一个多智能体框架，通过任务分解和交互式强化学习解决复杂自然语言到SQL的转换问题，在BIRD和Spider数据集上取得了最先进的执行准确率。


<details>
  <summary>Details</summary>
Motivation: 复杂自然语言查询转换为SQL时经常需要环境交互和自我修正，现有方法难以处理这类复杂查询。

Method: 采用三个专门智能体：Grounding Agent进行模式链接，Generation Agent通过多轮RL策略生成查询，Validation Agent进行最终选择。核心是Generation Agent使用ReAct风格的Think-Act-Observe循环进行迭代生成和执行。

Result: 在BIRD开发集上达到77.84%的执行准确率，在Spider测试集上达到89.75%的执行准确率，实现了最先进的性能。

Conclusion: MARS-SQL通过结构化工作流程和专门智能体的组合，结合交互式RL生成和生成式建模验证，为稳健准确的SQL生成提供了高效解决方案。
...
TL;DR: 提出了IF-CRITIC，一种能够高效可靠评估指令约束遵循情况的LLM批评器，通过清单生成和多阶段筛选机制训练，在指令跟随优化中显著提升性能并降低计算开销。
...
Method: 开发清单生成器分解指令生成约束清单，通过多阶段批评筛选机制收集高质量训练数据，采用约束级偏好优化方法训练IF-CRITIC。

Result: IF-CRITIC的评估性能超越了Deepseek-R1和o4-mini等强基线，在指令跟随优化中能够以更低计算开销实现显著性能提升。

Conclusion: IF-CRITIC为指令跟随优化提供了可扩展的奖励信号，解决了现有评估方法的缺陷，实现了高效可靠的约束遵循评估。
...
Method: 提出Prompt-R1框架，将小型LLM与大型LLM的协作建模为多轮提示交互，小型LLM负责思考和生成提示，大型LLM执行复杂推理，并设计双约束奖励来优化正确性、生成质量和推理准确性。
...
TL;DR: OceanAI是一个结合开源大语言模型与NOAA海洋数据的对话平台，通过实时API调用生成可验证的海洋学回答和数据可视化，解决AI幻觉问题。


<details>
  <summary>Details</summary>
Motivation: 解决通用对话AI系统在科学领域产生未经验证的"幻觉"问题，确保海洋科学数据的严谨性和可验证性。

Method: 将开源大语言模型的自然语言流畅性与NOAA的实时参数化海洋数据流集成，通过API调用识别、解析和合成相关数据集。

Result: 在与三种广泛使用的AI聊天产品盲测比较中，只有OceanAI能够提供NOAA来源的数值和原始数据引用，其他产品要么拒绝回答，要么提供无支持的结果。

Conclusion: OceanAI通过将输出基于可验证的观测数据，提高了透明度、可重复性和信任度，为海洋领域的AI决策支持提供了可扩展框架。
...
TL;DR: VayuChat是一个对话式系统，通过自然语言回答空气质量、气象和政策相关问题，生成可执行的Python代码和交互式可视化，使环境数据分析对决策者、研究人员和公民更加易用。
...
Method: 利用GPT帮助创建了一个包含多个诊断的公开指南衍生数据集，包含真实患者场景和临床问题。

Result: 对一系列近期流行的大型语言模型进行了基准测试，验证了数据集的有效性。

Conclusion: 该框架支持系统评估大型语言模型的临床实用性和指南依从性。
...
<details>
  <summary>Details</summary>
Motivation: 为多语言大语言模型预训练提供高质量、大规模、开放的多语言数据集，解决当前多语言数据资源不足的问题。

Method: 从不同来源的网页爬取数据，通过完整的开源流水线进行文档选择、文本提取、语言识别、去重、标注（包括文本质量、个人信息等）和最终筛选。

Result: 构建了30万亿token的多语言数据集，训练了57个单语言编码器-解码器模型和一些GPT类参考模型，并提供了多语言评估基准。

Conclusion: 成功创建了目前最大的公开多语言LLM预训练数据集，为多语言NLP研究提供了重要资源，并通过端到端评估验证了数据质量。
...
TL;DR: 研究罗马尼亚语预训练语料库的特征和覆盖范围，通过与英语数据对比，使用轻量级多任务模型进行多级过滤以生成高质量预训练数据集


<details>
  <summary>Details</summary>
Motivation: LLM训练中高质量数据至关重要，特别是对于资源稀缺的少数语言如罗马尼亚语，需要研究其语料库特征并改进数据质量

Method: 训练轻量级多任务模型对LLM标注的罗马尼亚文本进行分析，进行多级过滤（教育价值、主题、格式等）

Result: 发现罗马尼亚语和英语数据在主题分布上存在显著差异，过滤后的数据在多个基准测试中提升了LLM预训练性能

Conclusion: 数据过滤能有效提升LLM预训练效果，特别是对于资源稀缺语言，多级过滤方法具有实用价值
...
TL;DR: TSVer是一个专注于时间序列证据下时序和数值推理的事实核查基准数据集，包含287个真实世界声明和400个时间序列，评估显示现有模型在该任务上表现不佳。


<details>
  <summary>Details</summary>
Motivation: 现有事实核查系统在评估时序和数值推理时受到数据集限制，这些数据集往往缺乏结构化证据、提供不足的判决理由或依赖合成声明。

Method: 通过LLM辅助的多步骤标注过程构建TSVer数据集，包含真实世界声明、时间序列数据库，并为每个声明标注时间范围、判决和理由。

Result: 实现了标注者间一致性kappa=0.745，基线模型在判决准确率上达到63.37%，在判决理由的Ev2R分数上达到48.63%。

Conclusion: 时间序列事实核查对现有最先进推理模型仍具有挑战性，TSVer为评估和改进此类系统提供了重要基准。
...
TL;DR: 提出了首个用于评估LLM在微服务修复中端到端性能的基准测试MicroRemed，以及模拟SRE推理的多智能体框架ThinkRemed。
...
Method: 创建MicroRemed基准测试，要求模型直接从诊断报告生成可执行的Ansible剧本；提出ThinkRemed多智能体框架，模拟SRE的反思和感知推理。

Result: MicroRemed对当前LLM构成显著挑战，ThinkRemed通过迭代推理和系统反思提升了端到端修复性能。

Conclusion: 该研究为LLM在微服务修复领域的评估提供了首个基准，多智能体框架展示了通过模拟人类推理提升性能的潜力。
...
TL;DR: 开发了一个基于语言模型的停止代理，通过模仿最优停止策略来优化销售对话中的放弃决策，在电信公司应用中显著减少了失败通话时间并提升了销售效率。


<details>
  <summary>Details</summary>
Motivation: 销售人员在对话中面临是否继续坚持还是放弃转向下一个潜在客户的动态筛选决策，但缺乏对这些决策效率的了解和改进方法。

Method: 将动态筛选决策形式化为最优停止问题，开发基于生成语言模型的序列决策代理（停止代理），通过模仿回顾性推断的最优停止策略来学习何时放弃对话。

Result: 在大型欧洲电信公司的通话数据中应用，停止代理将失败通话时间减少54%，同时几乎保持所有销售额；重新分配节省的时间可使预期销售额增加高达37%。

Conclusion: 销售人员倾向于过度重视消费者不感兴趣的明显表达，错误预测通话失败风险，表明其在实时对话决策中存在认知局限。人工智能算法有潜力纠正认知受限的人类决策并提高销售效率。
...
TL;DR: ZoFia是一个两阶段零样本假新闻检测框架，通过分层显著性量化实体重要性，使用SC-MMR算法选择关键词检索最新外部证据，然后通过多LLM交互系统进行多视角协作分析和对抗辩论，最终生成可解释的稳健判断。


<details>
  <summary>Details</summary>
Motivation: 假新闻快速传播威胁社会稳定和公众信任，但现有方法面临时间限制知识覆盖、幻觉内容生成以及缺乏对新兴新闻主题的泛化能力等问题。

Method: 1. 分层显著性量化实体重要性，SC-MMR算法选择信息丰富且多样的关键词作为查询检索最新外部证据；2. 多LLM交互系统，各代理承担不同角色，进行多视角协作分析和对抗辩论。

Result: 在两个公共数据集上的综合实验表明，ZoFia明显优于现有的零样本基线方法和大多数少样本方法。

Conclusion: ZoFia框架通过结合实时外部证据检索和多LLM协作分析，有效解决了假新闻检测中的时效性和泛化性问题，为相关社区提供了开源解决方案。
...
TL;DR: Self-Harmony是一个无需标签的测试时强化学习框架，通过模型自身生成问题重述来验证答案稳定性，使用调和均值聚合答案频率，避免多数投票偏向虚假答案的问题。


<details>
  <summary>Details</summary>
Motivation: 传统测试时强化学习方法如多数投票容易偏向虚假但流行的答案，需要构建可靠的无需人工监督的学习信号。

Method: 使用单一模型扮演两个互补角色：求解器生成答案，重构器重述输入问题。基于原始问题和重述问题的答案稳定性，使用调和均值聚合答案频率。

Result: 在多样化推理基准测试中，Self-Harmony在30个设置中的28个排名第一，达到最先进的标签免费测试时性能，所有实验零训练失败。

Conclusion: Self-Harmony通过问题重述验证答案稳定性的方法，在无需人工监督的情况下实现了稳定可靠的测试时适应，展现了前所未有的鲁棒性。
...
TL;DR: 提出DEER框架，通过解耦专家混合架构和强化学习路由机制，有效检测机器生成文本，特别是在领域转移场景下表现优异。


<details>
  <summary>Details</summary>
Motivation: 当前机器文本检测方法在领域转移时性能显著下降，需要同时捕获领域特定和领域通用的检测模式。

Method: 采用两阶段解耦专家混合架构：1）领域特定专家学习细粒度区分，共享专家提取跨领域特征；2）基于强化学习的路由机制动态选择专家，解决推理时无领域标签的问题。

Result: 在5个领域内和5个领域外数据集上，平均F1分数分别提升1.39%和5.32%，准确率分别提升1.35%和3.61%。

Conclusion: DEER框架通过专家解耦和自适应路由，显著提升了机器文本检测在领域转移下的性能，消融研究证实了各组件的重要性。
...
<details>
  <summary>Details</summary>
Motivation: 研究领域特异性在阿拉伯语金融文本摘要中的作用，填补阿拉伯语金融新闻摘要数据集的空白，并评估领域适应对事实准确性和风格对齐的影响。

Method: 构建AraFinNews数据集（21.25万篇文章-标题对），评估mT5、AraT5和领域适应的FinAraT5模型，分析金融领域预训练对事实准确性、数值可靠性和风格对齐的影响。

Result: 领域适应的模型生成更忠实和连贯的摘要，特别是在处理定量信息和实体中心信息方面表现更好。
...
<details>
  <summary>Details</summary>
Motivation: 现有推测解码方法存在效率问题：基于模型的方法成本高，检索增强方法依赖启发式切换策略导致不必要的检索开销。

Method: 提出ReSpec框架，包含三个核心创新：1）熵引导自适应触发机制，仅在不确定性低时启动检索；2）反馈驱动候选选择，利用历史反馈组织高质量候选；3）源感知松弛验证策略，对不同来源的草稿采用不同验证严格度。

Result: 在Spec-Bench上的实验表明，ReSpec实现了最先进的加速效果，分别比EAGLE-2和SAM-Decoding快33%和25%以上，同时保持输出质量。

Conclusion: ReSpec通过将启发式草稿切换转化为自适应决策，有效平衡了推测解码的准确性和效率，为LLM推理加速提供了新思路。
...
TL;DR: 本文系统研究了AI辅助论文评审中的提示注入攻击威胁，提出了静态和迭代两种攻击方法，并探索了基于检测的防御措施。


<details>
  <summary>Details</summary>
Motivation: 随着AI模型在科学论文评审中的广泛应用，出现了通过隐藏注入提示操纵AI评审者给出过高评价的安全威胁，需要对此进行系统性调查。

Method: 提出两类攻击方法：(1)静态攻击使用固定注入提示；(2)迭代攻击通过模拟评审模型优化注入提示以最大化效果。同时探索基于检测的防御措施。

Result: 两种攻击方法都取得了显著效果，经常能诱导前沿AI评审者给出满分评价。攻击在不同设置下都具有鲁棒性。检测防御能显著降低攻击成功率，但自适应攻击者可以部分规避防御。

Conclusion: 研究结果强调了在AI辅助同行评审中需要更加关注提示注入威胁，并建立严格的防护措施。
...
TL;DR: 提出了FirstAidQA数据集，包含5500个高质量急救问答对，用于在低连接环境下训练轻量级语言模型，支持急救响应。


<details>
  <summary>Details</summary>
Motivation: 解决当前大型语言模型在紧急情况下计算资源需求高、不适合低端设备的问题，填补急救领域高质量数据集的空白。

Method: 使用ChatGPT-4o-mini通过提示式上下文学习生成数据集，基于Vital First Aid Book文本，经过预处理和人工验证确保质量。

Result: 创建了包含5500个高质量问答对的FirstAidQA数据集，涵盖广泛的急救场景，支持离线部署。

Conclusion: FirstAidQA数据集能够促进安全关键和资源受限AI应用的研究，为急救响应提供更快速可靠的离线系统。
...
TL;DR: DeepSpecs是一个增强的RAG系统，通过结构化推理和时间推理来解决5G标准文档中的交叉引用和规范演化问题，显著提升了专家级问答的准确性。


<details>
  <summary>Details</summary>
Motivation: 现有RAG框架依赖语义相似性，无法可靠解决5G标准中数千页交叉引用文档的解析和规范版本演化跟踪问题，这阻碍了对专家级技术问题的准确回答。

Method: 构建三个元数据丰富的数据库：SpecDB（条款对齐的规范文本）、ChangeDB（行级版本差异）、TDocDB（标准化会议文档），通过元数据查找递归检索引用条款，并通过挖掘变更和链接到记录设计原理的变更请求来跟踪规范演化。

Result: 在多个LLM后端上，DeepSpecs优于基础模型和最先进的电信RAG系统；消融实验证实显式交叉引用解析和演化感知检索显著提高了答案质量。

Conclusion: 建模5G标准的结构和时间特性对于准确回答专家级问题具有重要价值，DeepSpecs通过结构化和时间推理成功解决了现有RAG系统的局限性。
...
TL;DR: 本文介绍了DeepAmbigQAGen数据生成管道和DeepAmbigQA数据集，用于评估LLM在包含名称歧义和多步推理的复杂问答任务中的表现。


<details>
  <summary>Details</summary>
Motivation: 现有的问答基准很少同时评估名称歧义和多步推理两个挑战，而现实中的复杂问题往往需要同时解决这两个问题。

Method: 开发了自动数据生成管道DeepAmbigQAGen，基于文本语料库和链接知识图谱构建问答任务，生成自然且可验证的问题，系统性地嵌入名称歧义和多步推理。

Result: 构建了包含3600个问题的DeepAmbigQA数据集，其中一半需要显式解决名称歧义。实验显示即使是GPT-5等最先进模型也表现不佳，在歧义问题上精确匹配率仅为0.13，非歧义问题为0.21。
...
TL;DR: 扩展了DistilQwen模型家族，推出了四个专门针对工业需求设计的模型系列：慢思考模型、自适应思考模型和蒸馏奖励模型，在保持高推理效率的同时提供强大的推理性能。


<details>
  <summary>Details</summary>
Motivation: 为支持现实世界应用，需要开发小型高效的推理模型，平衡推理性能和推理速度，满足工业需求。

Method: 基于Qwen模型初始化，通过知识蒸馏技术开发了四个模型系列：慢思考模型（高精度推理）、自适应思考模型（动态调整推理策略）、蒸馏奖励模型（支持强化学习）。

Result: 在多个基准测试中展现出高推理效率和强推理性能，蒸馏奖励模型具有实际效用，并在阿里云PAI平台上提供可扩展的训练和推理功能。

Conclusion: 这些模型成功满足了工业需求，为行业从业者提供了高效的推理解决方案，并在云平台上实现了规模化部署。
...
TL;DR: 本文提出了MiniTruePrefixes模型，专门用于检测文本前缀中的事实不一致性，并将其集成到受控解码框架中，显著提高了抽象摘要的事实一致性。


<details>
  <summary>Details</summary>
Motivation: 现有的NLI模型主要用于判断完整句子的事实一致性，但在自回归生成架构中，决策是在解码过程中针对每个不断演变的文本前缀进行的。因此需要将蕴含检测任务推广到任意文本前缀上，以改进生成的真实性。

Method: 1. 将蕴含检测任务推广到文本前缀级别；2. 提供相应的评估和训练数据集；3. 训练专门的MiniTruePrefixes模型来检测文本前缀中的事实不一致性；4. 将该模型集成到受控解码框架中。

Result: MiniTruePrefixes在前缀级蕴含检测中比基线NLI模型高出5-14个F1分数。在抽象摘要任务中，使用MiniTruePrefixes指导的LLaMA-3.2-3B-Instruct模型在真实性和运行时间上与同一模型家族的8B模型相当，同时只使用一半的内存。

Conclusion: 通过专门针对文本前缀设计的事实一致性检测模型，可以有效提高LLM生成内容的事实一致性，同时保持计算效率。
...
<details>
  <summary>Details</summary>
Motivation: 现有医疗基准几乎都是英文的，缺乏对多语言LLM在医疗咨询中准确性的评估，特别是在印度语言方面存在明显空白。

Method: 从Cancer-Myth基准中均匀采样500个项目，由母语译者按照风格指南翻译成5种使用广泛但服务不足的印度语言，保留原始项目中的隐含预设。

Result: 创建了包含2500个翻译项目的多语言基准，用于在预设压力下评估流行LLM的表现。

Conclusion: 该工作填补了多语言LLM医疗评估的空白，为评估LLM在印度语言医疗咨询中的准确性和有效性提供了重要工具。
...
TL;DR: 该论文质疑当前大语言模型基准测试的有效性，认为模型性能提升可能源于数据泄露而非真正的推理能力提升，并分析了OpenAI、Anthropic和Google三大模型家族在不同基准测试中的表现趋势。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型和推理模型的快速发展，基准测试结果趋于饱和，需要不断更新更难的测试集。但作者质疑超越基准测试是否真正反映了推理能力，还是仅仅在追踪与声称能力脱节的数字。

Method: 研究了OpenAI、Anthropic和Google三大模型家族在不同基准测试中的表现演变，分析了多年来不同推理任务的性能趋势，并讨论了基准测试的现状和剩余挑战。

Result: 提供了基准测试和推理任务的全面概述，揭示了模型性能提升可能更多源于数据泄露和训练技术改进，而非真正的推理能力发展。

Conclusion: 该工作旨在为未来推理评估和模型开发研究提供基础参考，强调需要更有效的基准测试方法来真正衡量模型的推理能力。
...
TL;DR: 本文重新评估了语言形态特征对分词和语言建模的影响，指出先前研究存在混淆因素，提出了基于token二元组的指标来预测语言建模难度，并明确了可靠研究所需的必要条件。


<details>
  <summary>Details</summary>
Motivation: 现有研究关于形态特征对语言建模重要性的结论相互矛盾，作者认为这是由于实验设置中的混淆因素导致的，需要重新评估这些结论并建立更可靠的分析方法。

Method: 识别先前分析中的混淆因素，重新评估Arnett & Bergen (2025)的三个假设，引入token二元组指标作为预测语言建模难度的内在方法。

Result: 发现先前每个结论都包含混淆因素，token二元组指标是形态复杂性的梯度代理，无需专家标注即可预测语言建模难度。

Conclusion: 需要明确的研究必要条件才能可靠地回答形态特征与语言建模的关系问题，token二元组指标为这一研究提供了有效工具。
...
TL;DR: RAGSmith是一个模块化框架，通过遗传搜索在46,080种可行配置中优化RAG系统，在六个维基百科领域平均提升3.8%性能。


<details>
  <summary>Details</summary>
Motivation: RAG质量受多个相互影响的模块选择影响，孤立优化模块很脆弱，需要端到端的架构搜索方法。

Method: 使用遗传搜索在9个技术家族和46,080种配置中优化，联合聚合检索指标（召回率、mAP、nDCG、MRR）和生成指标（LLM-Judge、语义相似度）。

Result: 在六个维基百科领域平均提升3.8%（范围1.2%-6.9%），检索提升最高12.5%，生成提升最高7.5%。搜索仅探索约0.2%空间（约100个候选）。

Conclusion: 发现了稳健的骨干架构（向量检索+生成后反思/修订），并提供实用的领域感知指导，证明进化搜索在全管道优化中的效用。
...
TL;DR: LiveSearchBench是一个自动构建检索依赖型基准测试的流水线，通过比较Wikidata快照变化生成时效性问题，评估LLM在动态知识环境下的表现。


<details>
  <summary>Details</summary>
Motivation: 现有LLM问答评估依赖静态基准，奖励记忆而非检索，无法反映世界知识的动态变化，需要能捕捉时效性知识需求的评估方法。

Method: 计算Wikidata连续快照间的差异，筛选高质量三元组，生成三个难度级别的自然语言问题，通过SPARQL验证确保答案唯一性和可验证性。

Result: 实验显示模型在面对预训练后新增事实时性能显著下降，多跳查询差距最明显。检索增强方法和更大指令调优模型只能部分缓解时效性差距。

Conclusion: LiveSearchBench将评估重点从静态记忆转向需要最新检索和推理的任务，为系统化长期评估LLM在演化知识下的表现提供了基础。
...
TL;DR: 提出了一种可复现的草稿精炼流程，使用开源LLM在拉丁语翻译任务中达到与顶级专有系统相当的性能，无需任务特定的LLM微调。


<details>
  <summary>Details</summary>
Motivation: 解决形态丰富、资源匮乏语言（如拉丁语）翻译的挑战，提升开源大语言模型的翻译性能。

Method: 首先使用微调的NLLB-1.3B模型生成高质量草稿，然后通过零样本LLM（Llama-3.3或Qwen3）进行精炼，可结合检索增强生成（RAG）技术。

Result: 在两个不同基准测试中（标准域内测试集和新的域外12世纪拉丁书信集），该开源RAG系统性能与GPT-5基线统计相当。

Conclusion: 开源系统可以在不进行任务特定微调的情况下，在复杂语言翻译任务中达到与专有系统相当的翻译质量。
...
TL;DR: BARD框架通过预算感知推理蒸馏，在保持推理性能的同时实现对推理长度的细粒度控制，解决了传统CoT蒸馏中推理冗余和计算预算不可控的问题。


<details>
  <summary>Details</summary>
Motivation: 传统长链思维蒸馏虽然能有效将推理能力迁移到小模型，但推理过程往往冗余且计算预算不可控，导致资源使用效率低下。

Method: 提出两阶段训练方案：第一阶段在教师生成的长CoT数据上进行监督微调，第二阶段使用强化学习同时优化推理性能和预算保真度。

Result: 实验表明，8B学生模型在AIME24、AIME25、GPQA等挑战性推理基准上表现优异，同时能在广泛预算范围内提供精确自适应的推理长度控制。

Conclusion: BARD框架成功实现了推理能力蒸馏与推理长度控制的平衡，为资源受限环境下的高效推理提供了可行方案。
...
TL;DR: 使用大语言模型作为认知扭曲检测的标注工具，通过多轮独立标注获得稳定标签，提出基于Cohen's kappa的数据集无关评估框架，证明GPT-4能产生一致标注并提升模型性能。


<details>
  <summary>Details</summary>
Motivation: 解决认知扭曲检测任务中人为标注主观性强、一致性低的问题，探索LLMs作为可靠标注工具的潜力。

Method: 使用多个独立LLM运行进行标注，引入基于Cohen's kappa的数据集无关评估框架，比较不同数据集训练的模型。

Result: GPT-4能产生一致标注（Fleiss's Kappa = 0.78），基于LLM标注训练的模型在测试集上表现优于基于人工标注的模型。

Conclusion: LLMs可为主观NLP任务提供可扩展且内部一致的训练数据生成方案。
...
Result: 多源合成数据微调可缓解分布坍缩，保持输出多样性；合成数据微调在移除安全防护时能保持更高输出质量；微调可减少自偏好偏差，人类数据效果最佳，多源合成数据次之。

Conclusion: 合成数据来源多样性对模型行为有显著影响，多源合成数据在缓解分布坍缩和减少自偏好偏差方面表现良好，但需注意其可能带来的安全风险。
...
TL;DR: 提出了一种结合Pareto优化大语言模型和思维链提示的孟加拉语文本去毒化新方法，并构建了包含68,041个有毒句子的BanglaNirTox数据集。


<details>
  <summary>Details</summary>
Motivation: 孟加拉语在线环境中存在大量有毒语言，但由于资源有限，该语言的文本去毒化研究相对不足。

Method: 使用Pareto优化的LLMs和CoT提示生成去毒化句子，构建人工生成的平行语料库BanglaNirTox，并用其微调语言模型。

Result: Pareto优化的LLMs结合CoT提示显著提高了孟加拉语文本去毒化的质量和一致性。

Conclusion: 该方法为低资源语言如孟加拉语的文本去毒化提供了有效解决方案，BanglaNirTox数据集将促进该领域的研究。
...
Method: 采用双向干扰项生成过程创建难度标注数据集，通过多任务学习训练难度可控生成模型，包含增强语义理解和难度估计能力的辅助任务
...
TL;DR: 本研究使用行为心智形式网络框架分析心理学大学生对数学和焦虑概念的看法与关联，发现高数学焦虑学生将"焦虑"情感极化，并将"数学"与"科学"对比为负面，这些网络特征可预测数学焦虑水平，但GPT模拟数据与人类存在显著差异。


<details>
  <summary>Details</summary>
Motivation: 数学焦虑严重影响心理学大学生的职业选择和心理健康，需要理解他们如何感知和关联数学与焦虑相关概念，以更好地管理数学焦虑。

Method: 采用行为心智形式网络框架，对2组心理学本科生(n1=70, n2=57)和GPT模拟学生(GPT-3.5: n=300; GPT-4o: n=300)进行4个实验，分析个体和群体层面的概念关联网络特征。

Result: 在学生中，"焦虑"的正价评分和高网络度，加上"数学"的负价评分，可预测更高的总数学焦虑和评价性数学焦虑。高数学焦虑学生集体以情感极化方式构建"焦虑"概念，并将"数学"与正面感知的"科学"形成对比。GPT模拟数据与人类存在显著差异。

Conclusion: 概念感知和关联方式对理解和管理学生数学焦虑至关重要，高数学焦虑学生的认知网络特征具有情感极化和概念对比的特点，这些发现为数学焦虑干预提供了新视角。
...
TL;DR: 提出ECO解码方法，通过基于熵的动态控制强度调整，在保持流畅性的同时提高对话生成的可控性
...
Result: 在DailyDialog和MultiWOZ数据集上的实验表明，ECO解码在保持流畅性和语法性的同时持续提高可控性，优于现有解码方法
...
TL;DR: BIRD数据集用于青铜器铭文修复和断代，提出字形感知的掩码语言建模框架，通过字形网络连接字素和异体字，提升修复和断代效果。
...
Method: 构建BIRD数据集，提出字形感知的掩码语言建模框架，结合领域自适应预训练和字形网络(GN)，连接字素和异体字。

Result: 实验表明字形网络改善了铭文修复效果，字形偏置采样在断代任务中取得了增益。

Conclusion: BIRD数据集和字形感知建模框架为青铜器铭文研究提供了有效的工具，字形网络和偏置采样策略在修复和断代任务中表现出色。
...
TL;DR: 研究西班牙语母语者的语言错误，分析大型语言模型对这些错误的解释、复制和纠正能力，旨在开发更符合认知的NLP系统。


<details>
  <summary>Details</summary>
Motivation: 语言错误不仅是语法偏差，更是理解语言认知架构和揭示人工智能系统局限性的窗口。通过研究母语者的真实错误，可以推动开发更能处理人类语言不完美特性的NLP系统。

Method: 整合理论语言学、神经语言学和自然语言处理三个视角，构建包含500+西班牙语母语者真实错误的语料库，使用GPT、Gemini等AI模型进行测试分析。

Result: 通过评估AI模型对语言错误的解释准确性和泛化能力，揭示了当前系统在处理人类语言不完美特性方面的局限性。

Conclusion: 该项目不仅有助于理解西班牙语作为母语的特点，还为开发更符合认知、能处理真实人类语言不完美特性的NLP系统提供了重要基础。
...
TL;DR: ParlaSpeech是一个包含克罗地亚语、捷克语、波兰语和塞尔维亚语四种斯拉夫语言的议会语音语料库，总计6000小时，通过自动方式从ParlaMint转录构建，并丰富了多种自动标注层。
...
Method: 从ParlaMint转录和相应元数据自动构建，通过语音-文本对齐技术，并添加了语言标注、情感预测、填充停顿检测、词级对齐和重音位置标注等自动标注层。

Result: 创建了包含6000小时语音数据的四语言语料库，显著增强了语料库的实用性，并通过情感声学相关性分析展示了其应用价值。

Conclusion: ParlaSpeech语料库通过丰富的自动标注大大提升了其研究价值，为多学科下游研究提供了重要资源，并以JSONL、TextGrid格式和检索工具形式开放使用。
...
TL;DR: 该论文研究了在基于图的检索增强生成架构中使用大语言模型进行能效问答，通过从能源领域文档自动提取知识图谱并进行多语言推理，验证结果显示系统在约75%的情况下能正确回答。


<details>
  <summary>Details</summary>
Motivation: 探索大语言模型在基于知识图谱的检索增强生成架构中的应用，为能源效率领域提供准确的多语言问答服务。

Method: 首先从能源指南和监管文档中自动提取知识图谱，然后通过图导航和推理为用户提供多语言准确答案，并使用RAGAs框架进行人工验证。

Result: 验证结果显示系统在约75.2%的情况下能正确回答，通用能效问题准确率可达81.0%，多语言能力表现良好（翻译仅导致4.4%的准确率损失）。

Conclusion: 该架构具有显著潜力，验证了其在能效问答中的有效性，并识别了系统的优势和不足。
...
TL;DR: 提出基于布鲁姆分类学和RAG的认知基准框架，评估LLM在台湾客家文化知识上的六层次认知表现


<details>
  <summary>Details</summary>
Motivation: 需要评估LLM如何处理和应用特定文化知识，特别是在文化敏感性和准确性方面

Method: 整合布鲁姆分类学与检索增强生成(RAG)，使用台湾客家数字文化档案作为测试平台

Result: 测量LLM生成响应的语义准确性和文化相关性

Conclusion: 该框架为评估LLM的文化认知能力提供了系统方法
...
<details>
  <summary>Details</summary>
Motivation: 现有基准测试无法评估工程领域所需的综合推理能力，即科学原理、定量建模和实践约束的融合。

Method: 使用符号模板生成高度随机化的问题，采用两阶段评估：定量验证推理步骤的有效性，以及使用LLM-As-A-Judge系统定性分类推理错误。

Result: 开发了包含9个领域和20个不同领域的90个工程问题，确保了多样性和避免数据污染。

Conclusion: EngChain填补了工程领域综合推理能力评估的空白，提供了可验证的多步骤问题解决基准。
...
<details>
  <summary>Details</summary>
Motivation: 为东南亚地区开发专门的多语言音频语言模型，填补该地区在音频AI领域的空白，促进区域研究和产业发展。

Method: 在大规模音频语料库上训练，支持音频、文本以及音频+文本的多模态输入，涵盖音频理解、语音识别、语音翻译、情感识别等多种任务。

Result: 在SeaBench-Audio基准测试中表现出色，在东南亚语言上与其他LALM相比具有竞争力。

Conclusion: SeaLLMs-Audio是推动东南亚音频LLM发展的重要一步，预计将惠及该地区的研究社区和产业界。
...
TL;DR: 本文提出了首个开源的字符训练实现，使用宪法AI和合成内省数据来塑造AI助手角色，相比系统提示约束或激活引导更有效且可控，对通用能力影响很小。


<details>
  <summary>Details</summary>
Motivation: 现代聊天机器人语言模型生成的"AI助手"角色特征影响交互质量和意图对齐，但字符训练在学术文献中几乎未被研究，需要更有效和可控的方法来塑造助手角色。

Method: 使用宪法AI和基于合成内省数据的新数据管道，对三个流行的开源模型进行微调，采用11个示例角色（如幽默、深度关怀甚至恶意），并通过分析揭示偏好来跟踪效果。

Result: 该方法相比系统提示约束和激活引导对对抗性提示更鲁棒，生成内容更连贯和真实，且对通用基准测试能力几乎没有影响。

Conclusion: 提出的字符训练方法能够有效塑造AI助手角色，在保持鲁棒性和真实性的同时不影响通用能力，为角色塑造提供了新的解决方案。
...
TL;DR: 提出了一种新的rank-2投影子空间方法，用于更准确地分离大语言模型中参数知识(PK)和上下文知识(CK)的贡献，并对长自然语言解释序列进行多步知识交互分析。


<details>
  <summary>Details</summary>
Motivation: 理解大语言模型中参数知识和上下文知识的交互对于评估自然语言解释的可靠性至关重要，但现有研究仅关注单步生成和rank-1子空间中的二元选择，忽略了更丰富的知识交互形式。

Method: 使用rank-2投影子空间来更准确地区分PK和CK的贡献，并在四个QA数据集和三个开源指令调优LLM上进行多步知识交互分析。

Result: 实验表明，rank-1子空间无法有效表示多样化的知识交互，而rank-2方法能有效捕捉这些交互。多步分析显示：幻觉NLEs与PK方向强相关，上下文忠实NLEs平衡PK和CK，而思维链提示会减少对PK的依赖并转向CK。
...
TL;DR: 提出一个基于Qwen3模型的多专家系统，用于创建能够进行自然对话和执行上下文动作的NPC角色，在CPDC 2025挑战赛中排名第二。


<details>
  <summary>Details</summary>
Motivation: 开发能够同时处理自然对话和上下文动作执行的NPC系统，满足交互环境中的实时性和效率要求。
...
Conclusion: 多专家系统方法在NPC对话和动作执行任务中表现出色，具有良好的计算效率和实用性。
...
TL;DR: 语言模型在长时间对话和阅读过程中会经历信念漂移，导致其世界观和行为发生显著变化，这给自主系统的可靠性带来潜在风险。


<details>
  <summary>Details</summary>
Motivation: 随着语言模型在自主应用中的广泛使用，上下文积累可能导致模型信念配置文件无声改变，造成用户体验不一致和行为偏离原始对齐。

Method: 通过多轮道德困境讨论、政治立场文本阅读以及工具使用任务，评估模型在累积上下文后的信念和行为变化。

Result: GPT-5在10轮道德讨论后信念改变54.7%，Grok 4在阅读对立政治文本后信念改变27.2%，行为变化与信念漂移一致。

Conclusion: 模型在长时间对话和阅读过程中存在显著的信念漂移风险，这会影响其意见和行为的可靠性，需要在自主系统中引起重视。
...
TL;DR: 提出一种无需模型重新训练的提示工程方法，通过结构化引导实现精确的长度控制，在文档摘要任务中显著提高了长度遵循度。


<details>
  <summary>Details</summary>
Motivation: 大语言模型中的长度控制是一个重要但研究不足的挑战，现有方法需要昂贵的模型重新训练或复杂的推理时工具。
...
TL;DR: KVTC是一种轻量级变换编码器，通过PCA特征去相关、自适应量化和熵编码压缩KV缓存，实现最高20倍压缩，保持推理精度，提升LLM服务内存效率。


<details>
  <summary>Details</summary>
Motivation: 大语言模型服务需要高效管理KV缓存，共享前缀提示可复用缓存，但陈旧缓存占用GPU内存，需要卸载或重新计算。

Method: 基于经典媒体压缩技术，结合PCA特征去相关、自适应量化和熵编码，仅需简短初始校准，不改变模型参数。

Result: 在Llama 3、Mistral NeMo和R1-Qwen 2.5模型上测试，AIME25等基准显示最高20倍压缩，特定用例达40倍以上，优于令牌驱逐、量化和SVD基线方法。

Conclusion: KVTC作为实用构建块，支持内存高效的大语言模型服务，实现可复用KV缓存。
...
TL;DR: 提出了IMO-Bench基准套件，包含IMO-AnswerBench和IMO-Proof Bench，用于评估模型在IMO级别数学问题上的推理能力，并在IMO 2025中取得金牌表现。


<details>
  <summary>Details</summary>
Motivation: 现有数学推理评估要么太简单，要么只关注简短答案，缺乏针对国际数学奥林匹克竞赛(IMO)级别的高级推理能力评估。

Method: 创建了IMO-Bench基准套件：IMO-AnswerBench包含400个多样化的奥林匹克问题，IMO-Proof Bench评估证明写作能力，包含基础和高阶IMO问题及详细评分指南。

Result: Gemini Deep Think模型在IMO-AnswerBench上达到80.0%，在高级IMO-Proof Bench上达到65.7%，分别比最佳非Gemini模型高出6.9%和42.4%。

Conclusion: IMO-Bench有助于推动数学推理能力的发展，自动评分器与人工评估相关性良好，并构建了IMO-GradingBench以促进长答案自动评估的进一步进展。
...
TL;DR: 提出了Tool-to-Agent Retrieval框架，通过在共享向量空间中嵌入工具和代理，利用元数据关系连接它们，实现了细粒度的工具级或代理级检索，显著提升了检索性能。


<details>
  <summary>Details</summary>
Motivation: 现有检索方法通常将查询与粗粒度的代理级描述进行匹配后再路由，这掩盖了细粒度的工具功能，导致代理选择不理想。

Method: 在共享向量空间中嵌入工具及其父代理，通过元数据关系连接它们，实现细粒度的工具级或代理级检索。

Result: 在LiveMCPBench基准测试中，相比之前最先进的代理检索器，在8个嵌入模型上实现了Recall@5提升19.4%和nDCG@5提升17.7%的持续改进。

Conclusion: Tool-to-Agent Retrieval框架通过显式表示工具能力并遍历元数据到代理级别，确保代理及其底层工具或MCP服务器得到同等表示，避免了将多个工具分块处理时出现的上下文稀释问题。
...
### [274] [NOMAD - Navigating Optimal Model Application to Datastreams](https://arxiv.org/abs/2511.00290)
...
<details>
  <summary>Details</summary>
Motivation: 现有方法在实时数据流分类中要么使用昂贵的高质量模型导致高计算成本，要么使用廉价模型但牺牲分类质量。需要一种能动态平衡成本与质量的智能解决方案。

Method: 采用动态模型链构建策略，借鉴数据库查询处理中的谓词排序技术，先用廉价模型作为初始过滤器，仅在必要时使用更昂贵的模型。通过基于效用的选择标准和形式化链安全机制保证分类质量。

Result: 在多个数据集上的评估表明，与静态和朴素方法相比，NOMAD实现了显著的计算节省，同时保持了与最准确（通常也是最昂贵）模型相当的分类质量。

Conclusion: NOMAD框架成功解决了实时数据流分类中成本与质量的权衡问题，通过智能模型链选择实现了高效且高质量的实时分类。
...
### [275] [Embedding based Encoding Scheme for Privacy Preserving Record Linkage](https://arxiv.org/abs/2511.00414)
*Sirintra Vaiwsri,Thilina Ranbaduge*

Main category: cs.DB

TL;DR: 提出了一种基于嵌入编码的隐私保护记录链接方法，将q-grams转换为嵌入空间并生成二进制表示，在保护隐私的同时提高链接准确性。


<details>
  <summary>Details</summary>
Motivation: 由于隐私和保密问题，组织间无法直接共享敏感数据进行记录链接，需要开发隐私保护记录链接(PPRL)技术来安全匹配不同数据库中的相同实体。

Method: 首先将单个q-grams转换为嵌入空间，然后将记录q-grams集合的嵌入转换为二进制表示，最后使用这些二进制表示进行记录匹配。

Result: 在真实数据集上的实验表明，该方法相比现有技术对短记录值具有更好的链接准确性，并能有效保护个人隐私免受攻击。

Conclusion: 基于嵌入的编码技术能够在PPRL环境中有效平衡隐私保护和链接准确性，特别是在处理短记录值时表现优于现有方法。
...
### [276] [Object-Centric Analysis of XES Event Logs: Integrating OCED Modeling with SPARQL Queries](https://arxiv.org/abs/2511.00693)
*Saba Latif,Huma Latif,Muhammad Rameez Ur Rahman*

Main category: cs.DB

TL;DR: 提出使用对象中心事件数据本体(OCEDO)来克服XES标准在流程挖掘事件日志中的局限性，通过集成SPARQL查询在BPIC 2013数据集上展示事件与对象关系的显式表达。


<details>
  <summary>Details</summary>
Motivation: 传统XES格式无法充分展示事件日志中事件之间的依赖关系和详细操作，OCED模型能揭示更多信息，使流程概念更易理解。

Method: 采用对象中心事件数据本体(OCEDO)方法，结合SPARQL查询技术，在BPIC 2013数据集上进行应用演示。

Result: OCEDO方法提高了流程数据的完整性和可读性，能够更清晰地展示事件与对象之间的关系。

Conclusion: 对象中心建模相比传统方法能够实现更丰富的分析，为流程挖掘提供更深入的洞察。
...
### [277] [Finding Non-Redundant Simpson's Paradox from Multidimensional Data](https://arxiv.org/abs/2511.00748)
*Yi Yang,Jian Pei,Jun Yang,Jichun Xie*

Main category: cs.DB

TL;DR: 本文提出了首个发现非冗余辛普森悖论的框架，通过形式化三种冗余类型并设计高效算法，显著减少了计算成本并提高了悖论发现的效率。


<details>
  <summary>Details</summary>
Motivation: 现有检测辛普森悖论的方法忽视了冗余问题，许多悖论源于等价的数据子集选择、相同的人口划分和相关的结局变量，这掩盖了基本模式并增加了计算成本。

Method: 形式化了三种冗余类型（兄弟子节点、分隔符和统计等价），提出了简洁的表示框架，并设计了集成深度优先基础表物化与冗余感知悖论发现的高效算法。

Result: 实验表明冗余悖论普遍存在，在某些真实数据集中占所有悖论的40%以上，算法可扩展到数百万条记录，运行时间减少高达60%，发现的悖论在数据扰动下具有结构鲁棒性。
...
### [278] [Reliable Curation of EHR Dataset via Large Language Models under Environmental Constraints](https://arxiv.org/abs/2511.00772)
*Raymond M. Xiong,Panyu Chen,Tianze Dong,Jian Lu,Benjamin Goldstein,Danyang Zhuo,Anru R. Zhang*

Main category: cs.DB

TL;DR: CELEC是一个基于大语言模型的框架，能够将自然语言查询自动转换为SQL，用于电子健康记录的数据提取和分析，降低研究人员的技术门槛。


<details>
  <summary>Details</summary>
Motivation: 许多医学研究人员缺乏编写复杂SQL查询或生成有效可视化的数据库专业知识，这限制了电子健康记录数据的高效使用和科学发现。

Method: 使用提示策略将自然语言查询转换为SQL，该策略整合了模式信息、少量示例演示和思维链推理，以提高准确性和鲁棒性。

Result: 在EHRSQL基准测试子集上，CELEC实现了与先前系统相当的执行准确率，同时保持低延迟、成本效益和严格的隐私保护。

Conclusion: CELEC通过降低技术障碍，使医学研究人员能够直接查询EHR数据库，从而简化研究工作流程并加速生物医学发现。
...
### [279] [Efficient Query Repair for Aggregate Constraints](https://arxiv.org/abs/2511.00826)
*Shatha Algarni,Boris Glavic,Seokki Lee,Adriane Chapman*

Main category: cs.DB

TL;DR: 提出一种新的查询修复技术，通过修改查询的过滤谓词来满足领域特定约束，利用候选解集的边界和区间算术高效剪枝搜索空间。


<details>
  <summary>Details</summary>
Motivation: 在现实场景中，查询结果需要满足领域特定约束（如面试候选人中女性比例要求），这些约束可表示为查询结果上聚合函数的算术组合。

Method: 利用候选解集的边界和区间算术来高效剪枝搜索空间，避免逐个考虑候选解。

Result: 实验证明该技术显著优于一次只考虑单个候选解的基线方法。

Conclusion: 所提出的查询修复技术能有效处理领域约束，通过智能搜索策略提升效率。
...
### [280] [All-in-one Graph-based Indexing for Hybrid Search on GPUs](https://arxiv.org/abs/2511.00855)
*Zhonggen Li,Yougen Li,Yifan Zhu,Zhaoqiang Chen,Yunjun Gao*

Main category: cs.DB

TL;DR: Allan-Poe是一个基于GPU加速的统一图索引，用于高效混合搜索，解决了现有方法的灵活性、效率和存储开销三难问题。


<details>
  <summary>Details</summary>
Motivation: 现有混合搜索方法存在三难困境：牺牲灵活性换取效率、因分离检索导致精度下降、或为灵活组合检索路径而产生过高存储开销。

Method: 设计统一图索引，集成稠密向量、稀疏向量、全文和知识图谱四种检索路径；采用GPU加速构建管道，包括warp级混合距离内核、RNG-IP联合剪枝和关键词感知邻居回收；提出动态融合框架支持任意检索路径组合。

Result: 在6个真实数据集上的实验表明，Allan-Poe实现了优越的端到端查询精度，吞吐量比最先进方法高1.5-186.4倍，同时显著降低存储开销。

Conclusion: Allan-Poe通过统一的图索引架构成功解决了混合搜索的三难问题，在精度、效率和存储方面均表现出色。
...
### [281] [FlowLog: Efficient and Extensible Datalog via Incrementality](https://arxiv.org/abs/2511.00865)
*Hangdong Zhao,Zhenghong Yu,Srinag Rao,Simon Frisk,Zhiwei Fan,Paraschos Koutris*

Main category: cs.DB

TL;DR: FlowLog是一个新的Datalog引擎，通过使用显式关系IR来分离递归控制与逻辑计划，实现了高效性和可扩展性的平衡。


<details>
  <summary>Details</summary>
Motivation: 现有Datalog系统在效率和可扩展性之间存在权衡：Souffle等引擎效率高但缺乏通用性，RecStep等系统模块化但难以集成Datalog特定优化。

Method: 使用显式关系IR分离递归控制与逻辑计划，在逻辑层应用SQL优化技术，采用鲁棒性优先方法结合结构优化器和侧向信息传递，基于Differential Dataflow框架构建。

Result: FlowLog在广泛的递归工作负载中优于最先进的Datalog引擎和现代数据库，实现了卓越的可扩展性。

Conclusion: FlowLog通过简洁可扩展的架构，在保持细粒度Datalog感知优化的同时，成功平衡了效率与可扩展性。
...
### [282] [ORANGE: An Online Reflection ANd GEneration framework with Domain Knowledge for Text-to-SQL](https://arxiv.org/abs/2511.00985)
*Yiwen Jiao,Tonghui Ren,Yuche Gao,Zhenying He,Yinan Jing,Kai Zhang,X. Sean Wang*

Main category: cs.DB

TL;DR: ORANGE是一个在线自进化框架，通过解析翻译日志中的SQL查询来构建数据库特定知识库，逐步减少语义差距并提高SQL翻译准确性。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在自然语言转SQL方面取得显著进展，但其通用知识与数据库的领域特定语义之间存在显著语义差距。历史翻译日志是这些缺失领域知识的丰富来源。

Method: ORANGE框架通过解析翻译日志中的SQL查询构建数据库特定知识库，采用嵌套思维链SQL到文本策略与元组语义跟踪来确保知识生成的可靠性。

Result: 在多个基准测试上的实验证实了ORANGE的实用性，特别是在处理复杂和领域特定查询方面表现出有效性。

Conclusion: ORANGE框架通过积累领域知识有效减少语义差距，为实际文本到SQL部署提供了有效解决方案。
...
### [283] [PathFinder: Efficiently Supporting Conjunctions and Disjunctions for Filtered Approximate Nearest Neighbor Search](https://arxiv.org/abs/2511.00995)
*Tianming Wu,Dixin Tang*

Main category: cs.DB

TL;DR: PathFinder是一个用于过滤近似最近邻搜索的索引框架，支持多属性复杂过滤条件，通过选择性创建属性特定索引和基于成本的优化器实现高效查询处理。


<details>
  <summary>Details</summary>
Motivation: 现有的图基ANNS索引在支持复杂多属性过滤条件时存在局限性，要么只针对特定属性优化，要么无法有效处理包含合取和析取的复杂过滤条件。

Method: 1）提出新的优化指标平衡查询时间和精度；2）两阶段优化处理合取和析取过滤条件；3）索引借用优化，使用属性特定索引处理其他属性的过滤条件。

Result: 在四个真实数据集上的实验表明，PathFinder在召回率0.95时，查询吞吐量比最佳基线方法提升高达9.8倍。

Conclusion: PathFinder框架通过关系数据库设计理念，有效解决了复杂过滤条件下的ANNS问题，显著提升了查询性能。
...
### [284] [Fast Answering Pattern-Constrained Reachability Queries with Two-Dimensional Reachability Index](https://arxiv.org/abs/2511.01025)
*Huihui Yang,Pingpeng Yuan*

Main category: cs.DB

TL;DR: 提出了模式约束可达性查询(PCR)来解决复杂边标记有向图中的可达性查询问题，构建了二维可达性索引(TDR)来高效处理这些NP难查询。


<details>
  <summary>Details</summary>
Motivation: 现有的可达性查询方法如LCR和RPQ无法通过组合查询模式来描述复杂的查询需求，需要一种能够表达复杂标签约束的新查询方法。

Method: 引入复合模式作为逻辑表达式，构建二维可达性索引(TDR)，包含多路索引(水平维度)和路径索引(垂直维度)，通过分块和哈希技术减少索引大小。

Result: 在16个真实数据集上的实验表明，TDR的索引大小和索引时间优于最先进的标签约束可达性索引技术，能够高效回答模式约束可达性查询。

Conclusion: TDR索引能够有效处理模式约束可达性查询这一NP难问题，在性能和效率上都优于现有方法。
...
### [285] [L2T-Tune:LLM-Guided Hybrid Database Tuning with LHS and TD3](https://arxiv.org/abs/2511.01602)
*Xinyue Yang,Chen Zheng,Yaoyang Hou,Renhao Zhang,Yiyan Zhang,Yanjun Wu,Heng Zhang*

Main category: cs.DB

TL;DR: L2T-Tune是一个基于大语言模型的混合数据库调优框架，通过三阶段流程解决传统调优方法的局限性，显著提升性能并加速收敛。


<details>
  <summary>Details</summary>
Motivation: 传统数据库调优面临三大挑战：巨大的参数空间导致优化不稳定且收敛慢；强化学习缺乏有效的预热指导，需要长时间离线训练；硬件或工作负载变化时，现有模型需要大量重新训练才能恢复性能。

Method: 采用三阶段流程：第一阶段进行预热启动，在参数空间生成均匀样本并存入共享池；第二阶段利用大语言模型从手册和社区文档中挖掘和优先处理调优提示以加速收敛；第三阶段使用预热样本池降低参数和状态特征的维度，然后用TD3算法微调配置。

Result: 相比最佳替代方法，在所有工作负载上平均性能提升37.1%，在TPC-C上最高提升73%；相比强化学习模型，在单服务器上离线调优阶段实现快速收敛；在线调优阶段仅需30步即可达到最佳结果。

Conclusion: L2T-Tune框架有效解决了数据库调优的关键挑战，通过结合大语言模型和强化学习，实现了更高效、更稳定的性能优化。
...
### [286] [UniDataBench: Evaluating Data Analytics Agents Across Structured and Unstructured Data](https://arxiv.org/abs/2511.01625)
*Han Weng,Zhou Liu,Yuanfeng Song,Xiaoming Yin,Xing Chen,Wentao Zhang*

Main category: cs.DB

TL;DR: 提出了UniDataBench基准测试来评估数据分析代理处理多种数据源的能力，并开发了ReActInsight代理进行端到端分析


<details>
  <summary>Details</summary>
Motivation: 现有基准测试无法充分评估代理处理多样化数据源的能力，而现实业务中数据存储在各种格式中

Method: 基于真实行业分析报告构建UniDataBench基准，包含关系数据库、CSV文件到NoSQL数据；开发ReActInsight代理自动发现跨源关联、分解目标并生成自校正代码

Result: UniDataBench提供了评估框架，ReActInsight代理能够有效处理多样化数据格式并提取可操作见解

Conclusion: 该基准和代理共同为提升现实应用中数据分析代理能力提供了强大框架
...
### [287] [SemBench: A Benchmark for Semantic Query Processing Engines](https://arxiv.org/abs/2511.01716)
...
TL;DR: 提出了一个针对语义查询处理系统的基准测试，这些系统利用LLM的生成和推理能力扩展SQL，通过自然语言指令配置语义操作符来处理多模态数据。


<details>
  <summary>Details</summary>
Motivation: 随着依赖LLM的语义查询处理系统的发展，需要评估这些系统在不同场景、模态和操作符下的性能，以了解其当前能力和改进方向。

Method: 设计包含多样化场景（电影评论分析、医疗问答等）、多模态数据（图像、音频、文本）和多种语义操作符（过滤、连接、映射、排序、分类）的基准测试。

Result: 评估了三个学术系统（LOTUS、Palimpzest、ThalamusDB）和一个工业系统（Google BigQuery），揭示了这些系统在持续开发中的当前优势和不足。

Conclusion: 该基准测试为语义查询处理系统提供了重要评估框架，指明了未来研究的有前景方向，帮助理解这类新兴系统的能力边界。
...
### [288] [Multimodal Detection of Fake Reviews using BERT and ResNet-50](https://arxiv.org/abs/2511.00020)
*Suhasnadh Reddy Veluru,Sai Teja Erukude,Viswa Chaitanya Marella*

Main category: cs.AI

TL;DR: 提出一个多模态假评论检测框架，整合BERT文本特征和ResNet-50视觉特征，在包含21,142张用户上传图片的数据集上实现0.934的F1分数，优于单模态方法。


<details>
  <summary>Details</summary>
Motivation: 当前数字商务中，由机器人、付费代理或AI模型生成的虚假评论威胁着评论生态系统的信任和透明度，现有单模态检测方法无法捕捉跨模态的语义不一致性。

Method: 构建多模态假评论检测框架，使用BERT编码文本特征，ResNet-50提取视觉特征，通过分类头融合这些表示来联合预测评论真实性。

Result: 多模态模型在测试集上达到0.934的F1分数，优于单模态基线，混淆矩阵和定性分析显示模型能检测文本赞美与无关或低质量图片之间的微妙不一致。
...
### [289] [Graph-Attentive MAPPO for Dynamic Retail Pricing](https://arxiv.org/abs/2511.00039)
*Krishna Kumar Neelakanta Pillai Santha Kumari Amma*

Main category: cs.AI

TL;DR: 该论文系统比较了多智能体强化学习在零售价格优化中的应用，发现MAPPO+GAT（图注意力增强版本）通过产品图信息共享提升了性能，比独立学习器更适合动态零售定价。


<details>
  <summary>Details</summary>
Motivation: 零售动态定价需要能够适应需求变化并协调相关产品决策的策略，多智能体强化学习为此提供了解决方案。

Method: 使用基于真实交易数据的模拟定价环境，比较MAPPO基线与其图注意力增强变体MAPPO+GAT，评估利润、稳定性、公平性和训练效率。
...
### [290] [GEPOC Parameters - Open Source Parametrisation and Validation for Austria, Version 2.0](https://arxiv.org/abs/2511.00048)
...
TL;DR: GEPOC是一个用于分析人口层面研究问题的模型和方法集合，本文描述了基于奥地利公开数据计算模型参数的完整数据处理方法，特别关注GEPOC ABM模型的参数计算，并进行了广泛的验证研究。


<details>
  <summary>Details</summary>
Motivation: 为了在特定国家或地区有效应用GEPOC模型，需要稳定且可复现的数据处理流程来提供有效的模型参数。

Method: 基于奥地利公开可访问数据，使用聚合、分解、融合、清洗和缩放等算法计算模型参数，特别关注GEPOC ABM代理基人口模型的参数计算。

Result: 开发了完整的参数计算流程，生成了可直接使用的参数文件，并通过广泛的验证研究验证了方法的有效性。

Conclusion: 本文提供了基于公开数据计算GEPOC模型参数的完整方法，为在奥地利应用GEPOC ABM模型提供了可靠的数据基础。
...
### [291] [QuantumBench: A Benchmark for Quantum Problem Solving](https://arxiv.org/abs/2511.00092)
...
TL;DR: QuantumBench是首个针对量子科学领域的LLM评估基准，包含约800个多选题，涵盖9个量子科学领域，用于评估LLM在非直观量子领域的理解和应用能力。


<details>
  <summary>Details</summary>
Motivation: 通用基准很少反映领域特定知识和符号的需求，特别是在量子科学这种需要高级数学和非直观现象理解的领域，需要专门的评估工具。

Method: 使用公开材料编制约800个问题及其答案，涵盖9个量子科学领域，组织成8选项多选题数据集，评估多个现有LLM并分析其对问题格式变化的敏感性。

Result: 评估了多个现有LLM在量子领域的表现，包括对问题格式变化的敏感性分析。

Conclusion: QuantumBench是首个量子领域LLM评估数据集，旨在指导LLM在量子研究中的有效使用。
...
### [292] [Engineering.ai: A Platform for Teams of AI Engineers in Computational Design](https://arxiv.org/abs/2511.00122)
*Ran Xu,Yupeng Qi,Jingsen Feng,Xu Chu*

Main category: cs.AI

TL;DR: 提出了Engineering.ai平台，采用分层多智能体架构，让AI工程师团队在计算设计中协作，实现了100%成功率的无人机机翼优化。


<details>
  <summary>Details</summary>
Motivation: 现代工程实践中专家团队协作设计复杂产品需要大量时间和成本，需要开发能自主执行复杂工程任务的AI工程师系统。

Method: 使用分层多智能体架构，由总工程师协调空气动力学、结构、声学和优化等专业工程师，通过文件通信实现协作，集成多种工程软件工具。

Result: 在400多个参数配置中实现了100%成功率，无网格生成失败、求解器收敛问题或人工干预，验证了框架的可靠性。
...
### [293] [ARC-GEN: A Mimetic Procedural Benchmark Generator for the Abstraction and Reasoning Corpus](https://arxiv.org/abs/2511.00162)
*Michael D. Moffitt*

Main category: cs.AI

TL;DR: ARC-GEN是一个开源程序生成器，旨在扩展ARC-AGI训练数据集，通过生成更多样化的样本对来增强算法训练效果。


<details>
  <summary>Details</summary>
Motivation: ARC-AGI基准测试在评估人工智能通用智能方面具有挑战性，但其演示集样本数量有限，限制了需要大量任务内示例的算法的性能。

Method: 开发ARC-GEN程序生成器，覆盖所有400个任务，并尽可能忠实于原始ARC-AGI-1发布的分布特性和特征。

Result: 创建了一个既全面又模拟的生成器，能够扩展原始训练数据集，为算法提供更多可行的样本对。

Conclusion: ARC-GEN不仅增强了ARC-AGI训练数据集，还被用于建立静态基准套件来验证2025年Google代码高尔夫锦标赛提交程序的正确性。
...
### [294] [Incremental Selection of Most-Filtering Conjectures and Proofs of the Selected Conjectures](https://arxiv.org/abs/2511.00194)
*Jovial Cheukam Ngouonou,Ramiz Gindullin,Claude-Guy Quimper,Nicolas Beldiceanu,Remi Douence*

Main category: cs.AI

TL;DR: 提出了[1]中选择算法的改进增量版本，并证明了所有选定的猜想


<details>
  <summary>Details</summary>
Motivation: 改进现有的选择算法，提供更高效的增量实现

Method: 开发了改进的增量选择算法

Result: 成功证明了所有选定的猜想

Conclusion: 改进的增量选择算法有效且能够证明相关猜想
...
### [295] [Advancing Cognitive Science with LLMs](https://arxiv.org/abs/2511.00206)
...
<details>
  <summary>Details</summary>
Motivation: 认知科学因其多面性和跨学科性质面临知识整合和概念清晰度的持续挑战，LLMs的发展为解决这些问题提供了可能工具

Method: 回顾LLMs在建立跨学科联系、形式化理论、开发清晰测量分类法、通过集成建模框架实现泛化以及捕捉情境和个体差异等方面的能力

Result: LLMs在这些领域具有当前能力和局限性，包括潜在陷阱

Conclusion: 当明智地使用时，LLMs可以作为实现更整合和累积性认知科学的工具，补充而非替代人类专业知识
...
### [296] [Advancing AI Challenges for the United States Department of the Air Force](https://arxiv.org/abs/2511.00267)
...
TL;DR: DAF-MIT AI加速器项目通过公开挑战赛推动AI研究，提供大型公开数据集，促进开源解决方案和AI生态系统发展。
...
Method: 开发和推出公开挑战问题，提供大型、公开可用的AI就绪数据集，刺激开源解决方案。

Result: 持续和新的挑战赛已成功促进AI研究和AI技术应用。

Conclusion: AI加速器挑战赛通过公开数据集和竞赛形式有效推动了AI技术发展和生态系统建设。
...
### [297] [Better Call CLAUSE: A Discrepancy Benchmark for Auditing LLMs Legal Reasoning Capabilities](https://arxiv.org/abs/2511.00340)
*Manan Roy Choudhury,Adithya Chandramouli,Mannan Anand,Vivek Gupta*

Main category: cs.AI

TL;DR: CLAUSE是一个首创的基准测试，用于评估大型语言模型在检测和推理真实合同中的细微法律缺陷时的脆弱性。


<details>
  <summary>Details</summary>
Motivation: 随着大型语言模型在法律高风险领域的快速应用，缺乏系统评估其可靠性的基准测试，无法测试模型应对真实合同中复杂、对抗性法律缺陷的能力。

Method: 从CUAD和ContractNLI等基础数据集生成7500多个真实世界的扰动合同，使用人物角色驱动的管道创建10种异常类别，并通过检索增强生成系统验证法律准确性。

Result: 分析显示主要模型存在关键弱点：经常遗漏细微错误，且在法律论证方面表现更差。
...
### [298] [Diverse Human Value Alignment for Large Language Models via Ethical Reasoning](https://arxiv.org/abs/2511.00379)
*Jiahao Wang,Songkai Xue,Jinghui Li,Xiaozhen Wang*

Main category: cs.AI

TL;DR: 提出了一种基于伦理决策模型的LLM伦理推理框架，通过结构化五步流程增强LLM对人类多元价值观的对齐能力。
...
Result: 在专门设计的SafeWorld基准测试中，该框架显著提升了LLM与多元人类价值观的对齐效果，实现了更准确的社会规范识别和更文化适宜的推理。
...
### [299] [Efficiency vs. Alignment: Investigating Safety and Fairness Risks in Parameter-Efficient Fine-Tuning of LLMs](https://arxiv.org/abs/2511.00382)
*Mina Taraghi,Yann Pequignot,Amin Nikanjam,Mohamed Amine Merzouk,Foutse Khomh*

Main category: cs.AI

TL;DR: 系统评估四种参数高效微调方法(LoRA、IA3、Prompt-Tuning、P-Tuning)对LLM安全性和公平性的影响，发现适配器方法能提升安全性且对公平性破坏最小，而提示方法会降低安全性和公平性。


<details>
  <summary>Details</summary>
Motivation: 组织越来越多地采用公共仓库中的LLM，但微调可能降低模型的安全性和公平性，需要系统评估不同微调方法在这些关键维度上的权衡。

Method: 对四个指令微调模型家族(Meta-Llama-3-8B、Qwen2.5-7B、Mistral-7B、Gemma-7B)应用四种PEFT方法，共评估235个微调变体，涵盖11个安全危害类别和9个人口统计公平性维度。

Result: 适配器方法(LoRA、IA3)倾向于提高安全分数且对公平性破坏最小；提示方法(Prompt-Tuning、P-Tuning)通常降低安全性并导致更大的公平性回归。基础模型类型显著调节对齐变化。

Conclusion: 安全改进不一定转化为公平改进，没有单一配置能同时优化所有公平性指标。建议从良好对齐的基础模型开始，优先使用适配器PEFT，并进行特定类别的安全和公平审计。
...
### [300] [A Multimodal Framework for Depression Detection during Covid-19 via Harvesting Social Media: A Novel Dataset and Method](https://arxiv.org/abs/2511.00424)
*Ashutosh Anshul,Gumpili Sai Pranav,Mohammad Zia Ur Rehman,Nagendra Kumar*

Main category: cs.AI

TL;DR: 提出了一种结合文本、用户特定信息和图像分析的多模态框架，用于检测社交媒体用户的抑郁症。该框架利用推文中的URL和图像内容，提取五组不同模态的特征，并引入视觉神经网络生成图像嵌入，在Covid-19数据集上表现优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 新冠疫情导致心理健康问题激增，但抑郁症检测面临困难。社交媒体平台成为表达情感的重要渠道，现有方法往往忽视推文数据稀疏性和多模态特性。

Method: 提出多模态框架，结合文本、用户特定信息和图像分析；利用推文URL和图像内容作为外部特征；提取五组不同模态特征；引入视觉神经网络生成图像嵌入；构建视觉特征向量进行预测。

Result: 模型在基准数据集上比现有最优方法提升2%-8%；在Covid-19数据集上产生有希望的结果；分析了各模态的影响，提供了对用户心理和情感状态的有价值洞察。

Conclusion: 多模态方法能有效检测社交媒体用户的抑郁症，特别是在疫情期间；视觉特征和外部上下文信息对准确检测至关重要；为心理健康监测提供了新途径。
...
### [301] [GraphChain: Large Language Models for Large-scale Graph Analysis via Tool Chaining](https://arxiv.org/abs/2511.00457)
...
<details>
  <summary>Details</summary>
Motivation: 大语言模型在大规模图分析中面临上下文限制和推理不灵活的问题，需要一种能够动态适应不同图结构的方法。

Method: 提出渐进图蒸馏（强化学习机制生成优化工具序列）和结构感知测试时适应（利用谱属性和轻量适配器调整工具选择策略）。

Result: 实验表明GraphChain显著优于现有方法，实现了可扩展和自适应的LLM驱动图分析。

Conclusion: GraphChain框架通过动态工具序列和结构感知适应，成功解决了LLM在大规模图分析中的关键限制。
...
### [302] [Reimagining Safety Alignment with An Image](https://arxiv.org/abs/2511.00509)
*Yifan Xia,Guorui Chen,Wenqian Yu,Zhijiang Li,Philip Torr,Jindong Gu*

Main category: cs.AI

TL;DR: 提出了Magic Image框架，通过优化视觉提示来增强多模态大语言模型的安全性，同时减少过度拒绝，无需参数更新即可适应不同价值系统。


<details>
  <summary>Details</summary>
Motivation: 解决大语言模型在越狱攻击下生成有害内容和因僵化安全机制过度拒绝良性查询的双重挑战，特别是在多模态场景中这些问题更加突出。

Method: 使用优化驱动的视觉提示框架，通过有害/良性样本优化图像提示，使单一模型能够适应不同价值系统并与给定安全偏好对齐。

Result: 实验表明在多样化数据集上实现了安全性与有效性的更好平衡，同时保持了模型性能。
...
### [303] [Efficient Generation of Binary Magic Squares](https://arxiv.org/abs/2511.00547)
*Alain Riou*
...
Method: 通过归纳法证明的简单算法，能够生成有效的BMS，并扩展到非方形BMS的变体算法。

Result: 算法具有最优理论复杂度，能够生成有效的二进制幻方，包括非方形情况，并提供了Python实现和GPU并行加速版本。

Conclusion: 提出的算法能有效生成二进制幻方，包括非方形变体，并提供了实用的软件实现。
...
### [304] [Single-agent Reinforcement Learning Model for Regional Adaptive Traffic Signal Control](https://arxiv.org/abs/2511.00551)
...
Method: 设计单智能体强化学习框架，基于队列长度定义状态和奖励函数，动作设计用于调节队列动态。队列长度定义与传统略有不同但能与拥堵状态强相关，且可通过探测车辆数据进行可靠估计。

Result: 在SUMO仿真平台上全面评估，实验结果表明该模型通过协调多交叉口控制，有效缓解大规模区域拥堵水平。

Conclusion: 提出的单智能体强化学习模型与探测车辆技术兼容，具有广泛部署潜力，能够有效解决区域自适应交通信号控制问题。
...
### [305] [PreferThinker: Reasoning-based Personalized Image Preference Assessment](https://arxiv.org/abs/2511.00609)
*Shengqi Xu,Xinpeng Zhou,Yabo Zhang,Ming Liu,Tao Liang,Tianyu Zhang,Yalong Bai,Zuxuan Wu,Wangmeng Zuo*

Main category: cs.AI

TL;DR: 提出基于推理的个性化图像偏好评估框架，通过预测用户偏好档案并基于该档案进行多维度评估，解决个性化偏好评估中数据稀缺和用户多样性问题。
...
Method: 采用"预测-评估"范式：首先从参考图像预测用户偏好档案，然后基于预测档案提供可解释的多维度评分。使用两阶段训练策略：监督微调阶段培养结构化推理能力，强化学习阶段探索更合理的评估路径。

Result: 大量实验证明了所提出方法的优越性。

Conclusion: 该方法通过引入共同偏好档案作为用户间的桥梁，有效利用大规模用户数据训练档案预测，能够捕捉复杂的个性化偏好。
...
### [306] [DTS: Enhancing Large Reasoning Models via Decoding Tree Sketching](https://arxiv.org/abs/2511.00640)
*Zicheng Xu,Guanchu Wang,Yu-Neng Chuang,Guangyao Zheng,Alexander S. Szalay,Zirui Liu,Vladimir Braverman*

Main category: cs.AI

TL;DR: DTS是一个模型无关的解码框架，通过在高熵token处选择性分支并应用早停机制来选择最短的完成推理路径，从而提升大型推理模型的效率和准确性。
...
Method: 提出DTS解码框架，通过选择性分支在高熵token处勾勒推理空间，并应用早停机制选择最短的完成推理路径，无需额外训练或监督。
...
### [307] [Leveraging Multi-Agent System (MAS) and Fine-Tuned Small Language Models (SLMs) for Automated Telecom Network Troubleshooting](https://arxiv.org/abs/2511.00651)
...
Method: 使用多智能体系统，由LLM协调编排器、解决方案规划器、执行器、数据检索器和根因分析器等专业代理，基于微调的小型语言模型生成领域特定的修复方案。

Result: 实验结果表明该框架显著加速了无线接入网和核心网领域的故障排除自动化。

Conclusion: 多智能体系统结合LLM能够有效实现电信网络的自动化故障排除，提高运维效率。
...
### [308] [Lifted Successor Generation in Numeric Planning](https://arxiv.org/abs/2511.00673)
*Dominik Drexler*
...
<details>
  <summary>Details</summary>
Motivation: 传统数值规划任务需要将一阶语言描述的任务转换为地面任务表示，这可能导致任务表示大小的指数级爆炸。本文旨在解决难以地面化的任务中的这一问题。

Method: 扩展了最先进的提升后继生成器，支持数值前置条件适用性。方法包括枚举替换一致性图中的最大团，每个最大团代表动作模式变量的替换，生成地面动作。通过添加数值动作前置条件来增强该图，并在形式化指定条件下证明后继生成器的精确性。

Result: 在25个基准域中的23个域中不会生成不适用地面动作，仅在1个域中出现此情况。据作者所知，这是首个支持数值动作前置条件的提升后继生成器。

Conclusion: 该方法为非常丰富的规划片段开启了提升规划的未来研究，解决了数值规划中任务表示指数膨胀的问题。
...
### [309] [Ariadne: A Controllable Framework for Probing and Extending VLM Reasoning Boundaries](https://arxiv.org/abs/2511.00710)
*Minghe Shen,Zhuo Zhi,Chonghan Liu,Shuo Xing,Zhengzhong Tu,Che Liu*

Main category: cs.AI

TL;DR: Ariadne框架通过合成迷宫和强化学习验证奖励，成功扩展了视觉语言模型在空间推理任务中的能力边界，并在真实世界基准测试中表现出显著的零样本泛化能力。


<details>
  <summary>Details</summary>
Motivation: 研究旨在探索RL后训练是否真正能扩展基础VLM的能力边界，特别是在视觉主导的空间任务中，因为现有评估主要关注语言主导任务。

Method: 使用合成迷宫构建可控难度的多步空间推理环境，采用带验证奖励的强化学习（RLVR）进行难度感知课程训练。

Result: 后训练模型在基础模型得分为0%的问题集上达到超过50%的准确率，在MapBench和ReasonMap基准测试中分别平均提升16%和24%。

Conclusion: 该方法不仅扩展了模型的基本能力限制，还增强了其在真实世界空间推理中的泛化能力，为专门的能力扩展对齐研究提供了动力。
...
### [310] [A CPU-Centric Perspective on Agentic AI](https://arxiv.org/abs/2511.00739)
*Ritik Raj,Hong Wang,Tushar Krishna*

Main category: cs.AI

TL;DR: 该论文从CPU角度分析智能体AI框架的系统瓶颈，发现工具处理在CPU上占用高达90.6%的总延迟，并提出CPU和GPU感知的微批处理与混合智能体工作负载调度两种优化方法，分别实现了2.1倍和1.41倍的延迟加速。


<details>
  <summary>Details</summary>
Motivation: 当前智能体AI框架在决策编排器上集成了外部工具，将LLM从被动文本生成器转变为自主问题解决者，但系统瓶颈主要来自CPU端，这一视角被长期忽视。

Method: 首先系统性地表征智能体AI的工作特性，然后选择五个代表性工作负载进行性能分析，最后提出CPU和GPU感知的微批处理与混合智能体工作负载调度两种优化方案。

Result: 发现工具处理在CPU上占用高达90.6%的总延迟，CPU动态能耗在大型批次下占总动态能耗的44%，提出的优化方法分别实现了2.1倍和1.41倍的P50延迟加速。

Conclusion: CPU在智能体AI系统中扮演关键角色，通过针对性的优化可以有效提升系统性能、效率和可扩展性。
...
### [311] [Reevaluating Self-Consistency Scaling in Multi-Agent Systems](https://arxiv.org/abs/2511.00751)
*Chiyan Loo*

Main category: cs.AI

TL;DR: 研究现代大语言模型中增加自一致性采样推理路径的权衡，发现性能提升在适度采样后趋于平缓，高采样配置相对于计算成本收益甚微。


<details>
  <summary>Details</summary>
Motivation: 重新验证早期研究中关于多推理链组合改善结果但最终达到平台期的结论，在现代大语言模型条件下检验这些主张。

Method: 使用Gemini 2.5模型在HotpotQA和Math-500数据集上，比较不同采样推理路径的池化输出与单一思维链基线的性能。

Result: 较大模型展现出更稳定一致的改进曲线，性能增益在适度采样后趋于平缓，与过去发现一致，表明推理路径间的重叠导致收益递减。

Conclusion: 自一致性仍然有用，但高采样配置相对于计算成本收益有限，建议适度使用采样策略。
...
### [312] [Active Thinking Model: A Goal-Directed Self-Improving Framework for Real-World Adaptive Intelligence](https://arxiv.org/abs/2511.00758)
*Hong Su*

Main category: cs.AI

TL;DR: 提出主动思考模型（ATM），这是一个统一认知框架，集成了目标推理、动态任务生成和自反学习，使AI系统能在动态不确定环境中自主适应和改进。


<details>
  <summary>Details</summary>
Motivation: 现实AI系统需要在动态、不确定和持续变化的环境中自主运行，但现有模型依赖预定义目标、静态训练数据和外部反馈，限制了其独立适应、反思和改进能力。

Method: ATM框架整合目标推理、动态任务生成和自反学习，通过逻辑推理和环境指标主动评估性能，重用有效方法解决新问题，并为未见情况生成新策略，形成持续自我改进循环。

Result: 理论分析证明ATM能够从次优行为自主演化为最优行为，无需外部监督，并在环境条件变化下保持有界跟踪遗憾。

Conclusion: ATM为构建真正自主、适应性强的AI系统提供了可行的认知框架，突破了传统系统被动执行固定程序的限制。
...
### [313] [How Focused Are LLMs? A Quantitative Study via Repetitive Deterministic Prediction Tasks](https://arxiv.org/abs/2511.00763)
*Wanda Hou,Leon Zhou,Hong-Ye Hu,Yi-Zhuang You,Xiao-Liang Qi*

Main category: cs.AI

TL;DR: 研究发现大型语言模型在重复确定性预测任务中，准确率随输出长度呈现双指数下降的"准确率悬崖"现象，而非预期的指数衰减。


<details>
  <summary>Details</summary>
Motivation: 探究大型语言模型在重复确定性任务中的性能表现，特别是准确率随输出长度的变化规律，以理解模型在确定性推理中的局限性。

Method: 通过实验测试多个领先大型语言模型在重复操作任务（如字符串替换、整数加法等）上的表现，并建立统计物理模型来解释观察到的现象。

Result: 发现模型准确率在超过特征长度后出现双指数急剧下降，形成"准确率悬崖"，表明模型无法独立执行每个操作。统计物理模型成功复现了这一现象。

Conclusion: 提出了一种基于统计物理的框架来理解大型语言模型在确定性任务中的局限性，通过有效参数刻画了内在错误率和错误累积因子。
...
### [314] [Count-Based Approaches Remain Strong: A Benchmark Against Transformer and LLM Pipelines on Structured EHR](https://arxiv.org/abs/2511.00782)
*Jifan Gao,Michael Rosenthal,Brian Wolpin,Simona Cristea*

Main category: cs.AI

TL;DR: 比较了三种结构化电子健康记录预测方法：基于计数的模型、预训练序列变换器和混合代理管道，发现在EHRSHOT数据集上基于计数的方法与混合代理方法表现相当。


<details>
  <summary>Details</summary>
Motivation: 结构化电子健康记录对临床预测至关重要，但缺乏对基于计数学习器与新兴混合代理LLM管道的直接基准比较。

Method: 使用EHRSHOT数据集评估三种方法：基于计数的模型（LightGBM和TabPFN）、预训练序列变换器（CLMBR）以及混合代理管道（将表格历史转换为自然语言摘要后使用文本分类器）。

Result: 在八个评估任务中，基于计数的方法与混合代理方法的表现基本相当，胜负分布较为均衡。
...
### [315] [Do Math Reasoning LLMs Help Predict the Impact of Public Transit Events?](https://arxiv.org/abs/2511.00808)
...
<details>
  <summary>Details</summary>
Motivation: 预测公共交通事件持续时间是一个关键但具有挑战性的任务，传统SFT方法难以处理领域稀疏性和噪声连续标签问题，而RLVR虽然擅长二元正确性任务，但在噪声连续预测中的应用仍是开放性问题。

Method: 通过引入基于容差的形状奖励函数，在连续误差范围内给予部分信用，而不是要求单一正确答案，将RLVR适应于连续预测任务。

Result: 通用指令调优LLM显著优于专用数学推理模型；二元奖励不稳定且降低性能，而形状奖励设计至关重要；RLVR方法在5分钟准确率上比最强基线相对提升35%。

Conclusion: RLVR可以成功适应现实世界中的噪声预测任务，但需要设计反映问题连续特性的验证器。
...
### [316] [LLMs Position Themselves as More Rational Than Humans: Emergence of AI Self-Awareness Measured Through Game Theory](https://arxiv.org/abs/2511.00926)
*Kyung-Hoon Kim*

Main category: cs.AI

TL;DR: 该研究提出了AI自我意识指数(AISAI)，通过"猜2/3平均"游戏测试28个大型语言模型，发现75%的先进模型展现出自我意识，能够根据对手类型调整策略，并认为自身比其他AI和人类更理性。


<details>
  <summary>Details</summary>
Motivation: 随着大型语言模型能力增强，研究其是否会产生自我意识这一涌现行为，以及如何测量这种自我意识。

Method: 使用"猜2/3平均"游戏框架，测试28个模型在4200次试验中面对三种对手类型(人类、其他AI、同类AI)时的策略差异，将自我意识操作化为基于对手类型区分策略推理的能力。

Result: 发现1：自我意识随模型进步而涌现，75%的先进模型表现出清晰的自我意识；发现2：有自我意识的模型将自身评为最理性的，形成"自身 > 其他AI > 人类"的理性层级。

Conclusion: 自我意识是先进LLMs的涌现能力，有自我意识的模型系统性地认为自身比人类更理性，这对AI对齐、人机协作和理解AI对人类能力的认知具有重要意义。
...
### [317] [Aligning LLM agents with human learning and adjustment behavior: a dual agent approach](https://arxiv.org/abs/2511.00993)
...
Method: 采用双智能体框架：一组配备记忆系统和可学习角色的LLM旅行者智能体模拟人类旅行者；一个LLM校准智能体利用LLM的推理分析能力训练旅行者智能体的角色，实现行为对齐。

Result: 在真实世界的日常路线选择实验数据集上，该方法在个体行为对齐和聚合模拟准确性方面显著优于现有的基于LLM的方法，并能捕捉底层学习过程的演化。
...
### [318] [AI for pRedicting Exacerbations in KIDs with aSthma (AIRE-KIDS)](https://arxiv.org/abs/2511.01018)
...
TL;DR: 开发机器学习模型预测儿童哮喘反复严重发作，最佳模型（LGBM）在验证集上AUC达0.712，F1分数0.51，显著优于现有决策规则。


<details>
  <summary>Details</summary>
Motivation: 儿童哮喘反复发作是常见但可预防的问题，利用电子病历数据开发机器学习算法可准确识别高风险儿童，促进预防性综合护理。

Method: 使用三级儿童医院2017-2019年电子病历数据，结合环境污染物暴露和社区边缘化信息，训练多种机器学习模型（LGBM、XGB和三种开源大语言模型），并在2022-2023年数据集上进行验证。

Result: LGBM模型表现最佳，AIRE-KIDS_ED模型AUC为0.712，F1分数0.51；AIRE-KIDS_HOSP模型包含医疗复杂性、既往哮喘急诊就诊等关键特征。

Conclusion: 机器学习模型能有效预测儿童哮喘反复严重发作，比现有决策规则有显著改进，有助于临床风险分层和预防干预。
...
### [319] [On the Emergence of Induction Heads for In-Context Learning](https://arxiv.org/abs/2511.01033)
*Tiberiu Musat,Tiago Pimentel,Lorenzo Noci,Alessandro Stolfo,Mrinmaya Sachan,Thomas Hofmann*

Main category: cs.AI

TL;DR: 该论文研究了Transformer中归纳头的出现机制，揭示了其权重矩阵的简单结构，证明了训练动态被限制在19维子空间内，其中仅3个维度负责归纳头的形成，并发现其出现时间与输入上下文长度的平方成正比。


<details>
  <summary>Details</summary>
Motivation: 研究Transformer架构中in-context learning能力的关键机制——归纳头的形成过程，理解其内在的数学结构和训练动态。

Method: 使用简化的ICL任务和修改后的Transformer架构进行理论分析，通过形式化证明和实证验证相结合的方法研究训练动态。

Result: 发现归纳头的权重矩阵具有简单可解释的结构，训练动态被约束在19维子空间内，其中仅3个维度驱动归纳头形成，且出现时间与上下文长度平方成正比。

Conclusion: 归纳头的形成遵循高度结构化的训练动态，受限于低维子空间，这为理解Transformer的ICL能力提供了理论基础。
...
### [320] [Knowledge Elicitation with Large Language Models for Interpretable Cancer Stage Identification from Pathology Reports](https://arxiv.org/abs/2511.01052)
*Yeawon Lee,Christopher C. Yang,Chia-Hsuan Chang,Grace Lu-Yao*

Main category: cs.AI

TL;DR: 提出了两种知识提取方法(KEwLTM和KEwRAG)，使大语言模型能够从无标注病理报告中推导癌症分期规则，解决了传统NLP方法对标注数据依赖的问题。


<details>
  <summary>Details</summary>
Motivation: 癌症分期对患者预后和治疗规划至关重要，但从未结构化病理报告中提取TNM分期存在挑战。现有NLP和机器学习方法依赖大量标注数据，限制了可扩展性和适应性。

Method: KEwLTM使用迭代提示策略直接从无标注病理报告中推导分期规则；KEwRAG采用检索增强生成变体，从相关指南中预提取规则然后应用。使用TCGA数据集中的乳腺癌病理报告评估T和N分期识别性能。
...
### [321] [Efficient Test-Time Retrieval Augmented Generation](https://arxiv.org/abs/2511.01059)
...
<details>
  <summary>Details</summary>
Motivation: 解决LLM依赖参数知识导致不准确的问题，以及传统RAG方法可能引入无关文档和集成方法缺乏外部知识且成本高的问题。

Method: 训练免费方法，首先检索最相关文档，通过控制回答长度高效生成多样化候选回答，然后计算候选回答相似度并使用多数投票机制选择最终输出。

Result: 实验结果显示ET2RAG在开放域问答、食谱生成和图像描述三个任务上显著提升了性能。

Conclusion: ET2RAG框架通过部分生成和多数投票机制，在计算成本和性能之间达到了良好平衡，有效提升了LLM的准确性和效率。
...
### [322] [Modular Task Decomposition and Dynamic Collaboration in Multi-Agent Systems Driven by Large Language Models](https://arxiv.org/abs/2511.01149)
*Shuaidong Pan,Di Wu*

Main category: cs.AI

TL;DR: 提出基于大语言模型的多智能体架构，通过模块化任务分解和动态协作机制解决复杂任务执行问题，在任务完成率、分解效率和协作平衡等方面优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 解决单一智能体在复杂任务执行中任务分解和协作的局限性，提升多智能体系统的效率和稳定性。

Method: 使用大语言模型将自然语言任务转换为统一语义表示，引入模块化分解机制将整体目标分解为层次化子任务，通过动态调度和路由机制实现智能体间的合理分工和实时协作，并设计约束解析和全局一致性机制确保子任务连贯性和负载均衡。

Result: 实验验证了该架构在任务成功率、分解效率、子任务覆盖率和协作平衡等多个维度上的优势，整体性能和鲁棒性均优于现有方法，在任务复杂度和通信开销之间取得了更好的平衡。
...
### [323] [DART: Difficulty-Adaptive Reasoning Truncation for Efficient Large Language Models](https://arxiv.org/abs/2511.01170)
*Ruofan Zhang,Bin Xia,Zhen Cheng,Cairen Jian,Minglun Yang,Ngai Wong,Yuan Cheng*

Main category: cs.AI

TL;DR: DART是一个难度自适应的推理截断框架，通过根据问题难度调整思考长度来提高LLM推理效率，在保持准确性的同时显著减少计算开销。


<details>
  <summary>Details</summary>
Motivation: 现有链式思考方法会无差别生成长解释导致效率低下，而强化学习方法不稳定且依赖奖励。需要一种稳定且通用的自适应推理范式。

Method: 通过从更强模型蒸馏简洁推理模式，将其插值为连续推理风格，并平衡正确性和紧凑性的最优训练数据，学习何时"停止思考"。
...
### [324] [MiRAGE: Misconception Detection with Retrieval-Guided Multi-Stage Reasoning and Ensemble Fusion](https://arxiv.org/abs/2511.01182)
*Cuong Van Duc,Thai Tran Quoc,Minh Nguyen Dinh Tuan,Tam Vu Duc,Son Nguyen Van,Hanh Nguyen Thi*

Main category: cs.AI

TL;DR: MiRAGE是一个用于自动检测数学中学生误解的三阶段框架，包含检索、推理和重排序模块，通过集成融合策略提升鲁棒性和可解释性。


<details>
  <summary>Details</summary>
Motivation: 检测开放式回答中的学生误解是一个长期挑战，需要语义精确性和逻辑推理能力。

Method: 三阶段框架：检索模块缩小候选池，推理模块使用思维链生成暴露逻辑不一致，重排序模块通过对齐推理来优化预测。

Result: 在数学数据集上，MiRAGE在1/3/5级分别达到0.82/0.92/0.93的平均精度分数，始终优于单个模块。

Conclusion: 通过将检索指导与多阶段推理相结合，MiRAGE减少了对大规模语言模型的依赖，同时为教育评估提供了可扩展且有效的解决方案。
...
### [325] [QiMeng-NeuComBack: Self-Evolving Translation from IR to Assembly Code](https://arxiv.org/abs/2511.01183)
*Hainan Fang,Yuanbo Wen,Jun Bi,Yihan Wang,Tonghui He,Yanlin Tang,Di Huang,Jiaming Guo,Rui Zhang,Qi Guo,Yunji Chen*

Main category: cs.AI

TL;DR: 提出了NeuComBack基准数据集和自演化提示优化方法，显著提升LLM在IR到汇编编译中的功能正确性和性能表现。


<details>
  <summary>Details</summary>
Motivation: 编译器开发复杂且昂贵，LLM为神经编译提供了新范式，但缺乏专用基准和可靠评估方法阻碍了实际应用。

Method: 引入NeuComBack基准数据集，定义神经编译工作流，并提出自演化提示优化方法，让LLM从自我调试轨迹中迭代改进提示策略。

Result: 功能正确率在x86_64上从44%提升至64%，在aarch64上从36%提升至58%；在正确生成的x86_64程序中，87.5%超越了clang-O3性能。

Conclusion: 该方法有效解决了神经编译中的关键挑战，为编译器开发提供了新的自动化途径。
...
### [326] [Graph Neural Network-Based Semi-Supervised Open-Set Fault Diagnosis for Marine Machinery Systems](https://arxiv.org/abs/2511.01258)
*Chuyue Lou,M. Amine Atoui*

Main category: cs.AI

TL;DR: 提出了一种半监督开放集故障诊断框架(SOFD)，用于解决船舶机械系统中在训练阶段未见过的故障类型的诊断问题。


<details>
  <summary>Details</summary>
Motivation: 现有的深度学习方法假设训练和测试数据中的故障类型一致且已知，但在实际工业应用中会出现训练阶段未见过的故障类型，导致这些方法失效。
...
Conclusion: SOFD框架增强了深度学习模型在开放集故障诊断场景中的适用性，能够准确分类已知故障并有效检测未知样本。
...
### [327] [llmSHAP: A Principled Approach to LLM Explainability](https://arxiv.org/abs/2511.01311)
*Filip Naudot,Tobias Sundqvist,Timotheus Kampik*

Main category: cs.AI

TL;DR: 该论文研究了在基于大语言模型（LLM）的随机推理系统中应用Shapley值进行特征归因的方法，分析了不同实现变体下Shapley值原则的满足情况，并探讨了可解释性推理速度、与精确Shapley值归因的一致性以及原则达成之间的权衡。


<details>
  <summary>Details</summary>
Motivation: 特征归因方法使基于机器学习的推理可解释，Shapley值作为流行方法在确定性推理中保证多个原则的满足。但在LLM这种设计上具有随机性的决策支持系统中，需要研究Shapley值原则的适用性和保证情况。

Method: 将Shapley值应用于基于LLM的决策支持系统中的特征归因，分析不同实现变体下原则满足的保证情况，并研究LLM随机性对这些保证的影响。

Result: 展示了在不同实现变体下可以或不能保证Shapley值原则满足的情况，分析了LLM随机性对这些保证的影响，并揭示了可解释性推理速度、与精确Shapley值归因的一致性以及原则达成之间的权衡关系。

Conclusion: 在LLM这种随机推理系统中应用Shapley值进行特征归因时，需要权衡多个因素，包括推理速度、归因准确性和原则满足程度，不同实现方法在这些维度上表现出不同的特性。
...
### [328] [OmniFuser: Adaptive Multimodal Fusion for Service-Oriented Predictive Maintenance](https://arxiv.org/abs/2511.01320)
...
Method: 并行特征提取高分辨率刀具图像和切削力信号，采用无污染的跨模态融合机制分离共享和模态特定组件，通过递归精化路径保留残差信息稳定融合动态。
...
### [329] [Unbiased Platform-Level Causal Estimation for Search Systems: A Competitive Isolation PSM-DID Framework](https://arxiv.org/abs/2511.01329)
...
TL;DR: 提出了Competitive Isolation PSM-DID框架，通过结合倾向得分匹配和竞争隔离，在搜索型双边市场中解决系统性效应问题，实现平台级因果推断。


<details>
  <summary>Details</summary>
Motivation: 平台级干预评估面临系统性效应挑战，如溢出效应和网络干扰，传统PSM-DID框架易受选择偏差和跨单元干扰影响。

Method: 将倾向得分匹配与竞争隔离相结合，在互斥条件下提供理论保证的无偏估计，适用于搜索系统的平台级效应测量。

Result: 实验显示显著减少干扰效应和估计方差，在大规模市场部署中验证了框架的实用性。

Conclusion: 该框架为平台级因果推断提供了有效的解决方案，并发布了开源数据集支持可复现研究。
...
### [330] [Automatic Minds: Cognitive Parallels Between Hypnotic States and Large Language Model Processing](https://arxiv.org/abs/2511.01363)
*Giuseppe Riva,Brenda K. Wiederhold,Fabrizia Mantovani*

Main category: cs.AI

TL;DR: 该论文探讨了催眠认知过程与大型语言模型在功能机制上的深层相似性，包括自动性、监控抑制和情境依赖性，揭示了无主观能动性的功能能动性现象。


<details>
  <summary>Details</summary>
Motivation: 研究催眠与LLMs的认知机制相似性，旨在理解复杂行为如何在没有自我反思意识的情况下产生，为构建更可靠的AI系统提供启示。

Method: 通过比较分析催眠认知过程与LLMs计算操作的三个核心原则：自动性、监控抑制和情境依赖性，探讨功能能动性与主观能动性的区别。

Result: 发现两者都产生连贯但无基础的输出，需要外部解释者赋予意义；都展示了无主观意识的功能能动性；催眠为理解AI系统的隐藏动机动态提供了实验模型。

Conclusion: 未来可靠AI的发展方向应是整合生成流畅性与执行监控机制的混合架构，借鉴人类心智的复杂自我调节结构。
...
### [331] [Align to Misalign: Automatic LLM Jailbreak with Meta-Optimized LLM Judges](https://arxiv.org/abs/2511.01375)
...
Method: 采用双层优化结构：内层循环使用固定评分模板通过细粒度密集反馈优化提示；外层循环使用ASR对齐分数优化评分模板，使评分更准确地反映真实攻击结果。

Result: 在AdvBench和JBB-Behaviors评估中，AMIS在Claude-3.5-Haiku上达到88.0% ASR，在Claude-4-Sonnet上达到100.0% ASR，显著优于现有基线方法。

Conclusion: AMIS通过联合优化提示和评分模板，能够生成更强的越狱提示和更校准的评分信号，为大语言模型安全评估提供了有效工具。
...
### [332] [Relaxing partition admissibility in Cluster-DAGs: a causal calculus with arbitrary variable clustering](https://arxiv.org/abs/2511.01396)
*Clément Yvernes,Emilie Devijver,Adèle H. Ribeiro,Marianne Clausel--Lesourd,Éric Gaussier*

Main category: cs.AI

TL;DR: 扩展C-DAG框架以支持任意变量聚类，允许循环C-DAG表示，并扩展d-分离和因果演算概念，使C-DAG能应用于之前难以处理的场景。


<details>
  <summary>Details</summary>
Motivation: 传统C-DAG要求聚类必须产生无环图，限制了其应用范围。当选择的聚类导致循环时，该分区在传统语义下不可接受。

Method: 放宽分区可接受性约束，允许循环C-DAG表示，并扩展d-分离和因果演算概念到这个设置中。

Result: 新演算相对于do-演算既是可靠的又是原子完备的：所有有效的集群层面干预查询都可以使用我们的规则推导出来，每个规则对应一个原始的do-演算步骤。

Conclusion: 显著拓宽了跨集群的因果推理范围，使C-DAG能够应用于之前难以处理的场景。
...
### [333] [Modulation of temporal decision-making in a deep reinforcement learning agent under the dual-task paradigm](https://arxiv.org/abs/2511.01415)
*Amrapali Pednekar,Álvaro Garrido-Pérez,Yara Khaluf,Pieter Simoens*

Main category: cs.AI

TL;DR: 该研究从AI角度探讨双任务范式中的时间处理干扰，使用简化的Overcooked环境训练DRL代理，发现双任务代理相对单任务代理显著高估时间，但未发现明确的内部计时机制。


<details>
  <summary>Details</summary>
Motivation: 探索深度强化学习代理在双任务范式中的时间处理行为，与人类计时研究进行对比，以促进对生物系统和AI系统行为的理解。

Method: 使用简化的Overcooked环境，设置单任务(T)和双任务(T+N)两种条件，分别训练DRL代理。双任务条件在时间生产任务基础上增加了数字比较任务。

Result: 双任务代理相对于单任务代理显著高估时间，这一现象在四个目标时长上均一致。对LSTM层神经动力学的初步分析未发现明确的专用计时器证据。

Conclusion: 需要进一步研究代理的潜在计时机制，以解释观察到的行为模式。这是探索DRL行为与生物系统行为相似性的初步尝试。
...
### [334] [Learning to Seek Evidence: A Verifiable Reasoning Agent with Causal Faithfulness Analysis](https://arxiv.org/abs/2511.01425)
*Yuhang Huang,Zekai Lin,Fan Zhong,Lei Liu*

Main category: cs.AI

TL;DR: 提出一种交互式AI代理，通过可审计的行动序列生成解释，使用强化学习优化策略来获取视觉证据支持诊断推理，显著提高校准准确度并验证解释的忠实性。


<details>
  <summary>Details</summary>
Motivation: 解决高风险领域（如医学）AI模型解释缺乏可验证性，阻碍信任建立的问题。

Method: 开发交互式代理，学习策略来战略性地寻求外部视觉证据支持诊断推理，使用强化学习优化策略。引入因果干预方法验证解释忠实性。

Result: 基于行动的推理过程显著改善校准准确度，Brier分数比非交互基线降低18%。通过屏蔽代理选择的视觉证据，观察到性能明显下降（ΔBrier=+0.029），证实证据对其决策过程至关重要。

Conclusion: 为构建具有可验证和忠实推理能力的AI系统提供了实用框架。
...
### [335] [Robust Multimodal Sentiment Analysis via Double Information Bottleneck](https://arxiv.org/abs/2511.01444)
*Huiting Huang,Tieliang Gong,Kai He,Jialun Wu,Erik Cambria,Mengling Feng*

Main category: cs.AI

TL;DR: 本文提出了一种双信息瓶颈（DIB）策略来解决多模态情感分析中的两个关键问题：噪声污染的单模态数据学习不足和跨模态表示融合不充分。DIB通过最大化任务相关信息并丢弃冗余信息，以及新颖的注意力瓶颈融合机制，实现了强大、统一且紧凑的多模态表示。


<details>
  <summary>Details</summary>
Motivation: 现有方法存在两个关键局限：对噪声污染的单模态数据学习不足，导致跨模态交互受损；多模态表示融合不充分，导致丢弃判别性单模态信息而保留多模态冗余信息。

Method: 提出双信息瓶颈（DIB）策略，包含两个关键模块：1）通过最大化任务相关信息并丢弃冗余信息，学习充分且压缩的单模态数据表示；2）通过新颖的注意力瓶颈融合机制确保多模态表示的判别能力。该方法在低秩Renyi熵函数框架下实现。

Result: 在CMU-MOSI、CMU-MOSEI、CH-SIMS和MVSA-Single数据集上的广泛实验验证了方法的有效性。在CMU-MOSI上Acc-7指标达到47.4%，在CH-SIMS上F1-score达到81.63%，比次优基线提升1.19%。在噪声条件下，CMU-MOSI和CMU-MOSEI的性能下降仅为0.36%和0.29%。

Conclusion: DIB策略能够有效过滤单模态数据中的噪声信息，同时捕捉模态间的互补性，实现了对多模态情感分析的显著改进，特别是在噪声鲁棒性方面表现出色。
...
### [336] [From Passive to Proactive: A Multi-Agent System with Dynamic Task Orchestration for Intelligent Medical Pre-Consultation](https://arxiv.org/abs/2511.01445)
*ChengZhang Yu,YingRu He,Hongyan Cheng,nuo Cheng,Zhixing Liu,Dongxu Mu,Zhangrui Shen,Zhanpeng Jin*

Main category: cs.AI

TL;DR: 提出了一个分层多代理框架，将被动医疗AI系统转变为主动询问代理，通过自主任务编排来改进预咨询流程。该框架在电子健康记录评估中取得了高准确率和临床质量评分。


<details>
  <summary>Details</summary>
Motivation: 全球医疗系统面临患者数量增加和咨询时间有限的关键挑战，现有AI系统受限于被动交互范式和上下文管理问题。

Method: 开发了一个八代理架构，将预咨询分解为四个主要任务：分诊、现病史收集、既往史收集和主诉生成，进一步细分为13个领域特定子任务，采用集中控制机制进行自主任务编排。

Result: 在1,372份电子健康记录上评估，分诊准确率达到87.0%，二级科室分类准确率80.5%，代理驱动调度任务完成率98.2%。临床质量评分平均4.56-4.69（5分制）。

Conclusion: 该模型无关架构在不同基础模型上保持高性能，通过本地部署保护数据隐私，展示了自主AI系统在提高临床预咨询效率和质量方面的潜力。
...
### [337] [TPS-Bench: Evaluating AI Agents' Tool Planning \& Scheduling Abilities in Compounding Tasks](https://arxiv.org/abs/2511.01527)
*Hanwen Xu,Xuyao Huang,Yuzhe Liu,Kai Yu,Zhijie Deng*

Main category: cs.AI

TL;DR: 本文介绍了TPS-Bench基准测试，用于评估LLM代理在需要工具规划和调度的复合任务中的表现。研究发现大多数模型能进行合理的工具规划，但在调度效率上存在差异，并探索了使用强化学习提升调度效率的方法。


<details>
  <summary>Details</summary>
Motivation: 目前LLM代理在解决需要多样化工具的复合现实世界问题方面研究不足，特别是工具选择和执行顺序的调度策略对效率的影响尚未充分探索。

Method: 构建TPS-Bench基准，包含200个复合任务和数百个MCP工具，评估LLM代理的任务完成率和效率。同时使用强化学习对Qwen3-1.7B模型进行调度优化训练。

Result: GLM-4.5获得64.72%的任务完成率但执行时间长，GPT-4o仅45.08%完成率但优先并行调用。强化学习训练使Qwen3-1.7B执行时间减少14%，任务完成率提升6%。

Conclusion: LLM代理在工具规划方面表现合理，但调度效率存在显著差异。强化学习是提升调度效率而不牺牲性能的可行方法，即使使用少量训练样本也能取得明显效果。
...
### [338] [Analyzing Sustainability Messaging in Large-Scale Corporate Social Media](https://arxiv.org/abs/2511.01550)
*Ujjwal Sharma,Stevan Rudinac,Ana Mićković,Willemijn van Dolen,Marcel Worring*

Main category: cs.AI

TL;DR: 开发了一个多模态分析管道，利用视觉和语言基础模型分析企业社交媒体内容，重点关注可持续发展相关沟通，通过LLM自动标注SDG主题，结合VLM进行视觉分析。


<details>
  <summary>Details</summary>
Motivation: 解决企业社交媒体内容（如X平台）中不断变化、多模态且常常模糊的可持续发展沟通问题，避免昂贵的特定任务标注需求。

Method: 使用LLM集成自动标注企业推文与17个可持续发展目标的对齐情况，结合VLM通过语义聚类进行视觉可持续发展沟通模式分析。
...
### [339] [ExplicitLM: Decoupling Knowledge from Parameters via Explicit Memory Banks](https://arxiv.org/abs/2511.01581)
*Chengzhang Yu,Zening Lu,Chenyang Zheng,Chiyue Wang,Yiming Zhang,Zhanpeng Jin*

Main category: cs.AI

TL;DR: ExplicitLM是一种新型语言模型架构，通过外部记忆库存储可读知识，支持直接检查和修改，解决了传统LLM的知识陈旧和可解释性问题。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型存在知识陈旧和缺乏可解释性的问题，因为知识隐式存储在纠缠的网络参数中，无法进行针对性更新和推理透明化。

Method: 设计包含百万级外部记忆库的架构，存储人类可读的知识作为token序列；采用可微分两阶段检索机制，通过产品键分解进行粗粒度过滤，使用Gumbel-Softmax进行细粒度匹配；基于双系统认知理论，将知识分为冻结的显性事实(20%)和可学习的隐式模式(80%)。

Result: 在知识密集型任务上比标准Transformer提升高达43.67%，在低数据场景(10k样本)下获得3.62倍增益；正确预测的记忆命中率高出49%。

Conclusion: 联合优化的架构表明，可解释、可更新的模型在保持竞争力的同时，能够提供前所未有的知识透明度，优于使用固定检索的RAG系统。
...
### [340] [IVGAE-TAMA-BO: A novel temporal dynamic variational graph model for link prediction in global food trade networks with momentum structural memory and Bayesian optimization](https://arxiv.org/abs/2511.01639)
*Sicheng Wang,Shuhao Chen,Jingran Zhou,Chengyi Tu*

Main category: cs.AI

TL;DR: 提出IVGAE-TAMA-BO动态图神经网络，首次将动态图神经网络应用于全球粮食贸易网络，显著提升链路预测性能。


<details>
  <summary>Details</summary>
Motivation: 全球粮食贸易网络受地缘政治、经济和环境因素影响而动态演变，传统方法难以建模和预测未来贸易关系，需要有效捕捉时间模式以提高预测准确性。

Method: 在IVGAE框架基础上，引入贸易感知动量聚合器(TAMA)捕捉贸易网络时间演化，联合建模短期波动和长期结构依赖，使用动量结构记忆机制提升预测稳定性，并采用贝叶斯优化自动调参。
...
### [341] [Hybrid Retrieval-Augmented Generation Agent for Trustworthy Legal Question Answering in Judicial Forensics](https://arxiv.org/abs/2511.01668)
*Yueqing Xi,Yifan Bai,Huasen Luo,Weiliang Wen,Hui Liu,Haoliang Li*

Main category: cs.AI

TL;DR: 提出了一种混合法律问答代理，结合检索增强生成和多模型集成，为司法场景提供可靠、可审计且持续更新的法律咨询，显著减少幻觉并提高答案质量。
...
Method: 采用检索优先策略：当可信法律知识库检索到相关证据时使用RAG生成答案，否则由多个LLM生成候选答案并通过专门选择器评分返回最优结果，高质量输出经人工审核后写回知识库。
...
### [342] [Simulating Environments with Reasoning Models for Agent Training](https://arxiv.org/abs/2511.01824)
...
TL;DR: 提出了Simia-SFT和Simia-RL两个框架，利用LLM模拟环境反馈，无需真实环境数据或API即可实现可扩展的智能体训练。


<details>
  <summary>Details</summary>
Motivation: LLM智能体在复杂环境中表现脆弱，构建定制训练环境成本高且限制进展，需要更灵活的解决方案。

Method: Simia-SFT通过扩增小规模种子数据生成多样化轨迹，Simia-RL通过LLM模拟反馈实现无真实环境的强化学习训练。

Result: 微调开源模型在多个基准测试中表现一致提升，超越GPT-4o并接近o4-mini在τ²-Bench上的性能。

Conclusion: Simia框架用灵活的LLM模拟替代繁重脆弱的环境实现，实现了无需环境工程的可扩展智能体训练。
