...
Motivation: 符号计算机视觉需要与基于像素的视觉模型不同的学习范式，当前方法在图表解析和重建方面存在探索效率问题，需要结合神经网络的推理能力和符号模型的解释性。

Method: 提出自监督符号自动编码器，包含符号分层过程奖励建模，通过点在线、线在形、形在关系的分层一致性奖励，并引入稳定化机制平衡探索与利用。

Result: 几何图表重建MSE减少98.2%，7B模型在图表重建上超越GPT-4o 0.6%，MathGlance感知基准提升13%，MathVerse和GeoQA推理基准提升3%。

Conclusion: 该方法有效结合了神经网络的推理能力和符号模型的解释性，在图表理解任务上实现了显著的性能提升，为符号视觉学习提供了新范式。
...
TL;DR: 提出基于"排水节点"的统一框架，用于处理噪声标签、类别模糊和异常样本，通过重新分配概率质量到不确定性，在多种噪声场景下显著提升分类精度。


<details>
  <summary>Details</summary>
Motivation: 深度学习面临噪声标签、类别模糊、异常样本拒绝等挑战，特别是实例依赖和不对称标签噪声问题，需要一种统一解决方案。

Method: 在网络输出端添加"排水节点"，重新分配概率质量到不确定性，保持端到端训练和可微性，为模糊、异常或噪声样本提供自然逃逸路径。

Result: 在CIFAR-10/100上，针对实例依赖和不对称噪声，高噪声区域准确率提升达9%；在mini-WebVision、mini-ImageNet和Clothing-1M等真实数据集上达到或超越SOTA。

Conclusion: 排水节点框架有效处理噪声标签和异常样本，具有去噪效果，可扩展到分类之外的应用，如网络规模半监督数据清洗和开放集应用。
...
<details>
  <summary>Details</summary>
Motivation: 现实世界中的面部图像通常质量差、姿态非正面且有遮挡，导致生物识别面部识别模型的准确率显著下降。研究旨在探索针对性的AI驱动头部姿态校正和图像修复是否能提高识别准确率。

Method: 使用模型无关的大规模法证评估流程，评估三种修复方法：3D重建（NextFace）、2D正面化（CFR-GAN）和特征增强（CodeFormer）。

Result: 简单应用这些技术会显著降低面部识别准确率，但选择性应用CFR-GAN与CodeFormer的组合能带来有意义的改进。

Conclusion: 面部识别修复技术需要谨慎选择和应用，CFR-GAN与CodeFormer的组合策略在改善现实世界面部识别准确率方面具有潜力。
...
TL;DR: Flux4D是一个简单可扩展的4D动态场景重建框架，通过直接预测3D高斯及其运动动态，以完全无监督方式重建传感器观测，无需预训练模型或标注数据。


<details>
  <summary>Details</summary>
Motivation: 现有可微分渲染方法（如NeRF和3DGS）在大规模动态场景重建中存在可扩展性限制，需要标注数据来解耦运动。现有自监督方法虽然尝试消除标注，但仍受限于单场景优化和对超参数的敏感性。

Method: Flux4D直接预测3D高斯及其运动动态，仅使用光度损失和"尽可能静态"的正则化，通过跨多场景训练直接从原始数据中分解动态元素，无需预训练监督模型或基础先验。

Result: 在室外驾驶数据集上的实验表明，Flux4D在可扩展性、泛化能力和重建质量方面显著优于现有方法，能够在数秒内高效重建动态场景，并能有效扩展到大型数据集。

Conclusion: Flux4D提供了一个简单可扩展的4D动态场景重建框架，通过完全无监督的方式实现大规模动态场景的高效重建，具有良好的泛化能力，包括对罕见和未知物体的处理。
...
<details>
  <summary>Details</summary>
Motivation: 现有零样本物体计数方法通常需要大量标注数据和视觉示例，而大语言模型具有强大的推理和数据理解能力，可以探索仅用文本提示实现零样本计数的可能性
...
<details>
  <summary>Details</summary>
Motivation: 现有3D形状数据集和模型只关注几何形状，忽略了决定物体外观的材料属性，需要从3D数据中推断材料组成

Method: 两阶段LLM方法：第一阶段预测物体语义，第二阶段基于推断的语义为每个几何段分配合理材料；零样本操作，无需任务特定训练

Result: 在Fusion/ABS和ShapeNet的1000个形状上，该方法实现了高语义和材料合理性，LLM-as-a-Judge评估显示良好效果

Conclusion: 语言模型可作为通用先验，在3D数据中桥接几何推理和材料理解，实现从点云推断材料组成
...
TL;DR: 提出一种仅需单张噪声图像和单张暗帧即可合成真实传感器噪声的方法，用于低光去噪训练，无需大量配对数据。


<details>
  <summary>Details</summary>
Motivation: 低光条件下拍摄的原始图像因光子计数少和传感器噪声而非常嘈杂。基于学习的去噪器需要大量干净-噪声图像对进行训练，但这些数据难以收集。

Method: 使用泊松分布建模信号相关噪声，并提出傅里叶域频谱采样算法精确建模信号无关噪声，仅需单张噪声图像和单张暗帧即可合成多样化的噪声实现。

Result: 该方法能准确合成保持真实传感器噪声空间和统计特性的噪声，在多个低光去噪基准测试中达到最先进性能。

Conclusion: 提出的噪声合成方法既准确又实用，无需简化参数模型或大量配对数据集，为低光去噪训练提供了有效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 潜在扩散模型在图像修复和局部编辑方面取得了显著进展，但其固有的潜在压缩会引入像素级不一致问题，如色移、纹理不匹配和编辑边界处的可见接缝。现有的解决方法在实践中通常无法完全消除这些伪影，并且在不同潜在表示或任务中泛化能力不足。
...
Result: 在图像修复、物体移除和插入等基准测试上的大量实验表明，PixPerfect显著提高了感知保真度和下游编辑性能，为鲁棒且高保真的局部图像编辑建立了新标准。

Conclusion: PixPerfect是一个有效的像素级精炼框架，能够解决潜在扩散模型在局部编辑中产生的像素级不一致问题，具有广泛的适用性和优异的性能表现。
...
TL;DR: 该论文提出PyroFocus两阶段深度学习管道，用于实时机载/星载野火检测，通过火灾分类和FRP回归/分割实现速度与精度的平衡。
...
Method: 提出PyroFocus两阶段管道：第一阶段进行多类火灾分类（无火、活跃火、后火），第二阶段进行火灾辐射功率回归或分割。系统评估了包括自定义CNN和Transformer模型在内的多种深度学习架构，使用NASA MASTER数据进行验证。

Result: 实验结果表明，提出的两阶段管道在速度和精度之间实现了良好的权衡，在准确率、推理延迟和资源效率方面表现优异，展示了未来野火监测任务中实时边缘部署的潜力。

Conclusion: PyroFocus两阶段管道为机载/星载野火实时检测提供了有效的解决方案，在计算效率和检测精度之间取得了良好平衡，适合未来野火监测任务部署。
...
TL;DR: 提出了H²U3D数据集和SpatialReasoner框架，用于解决大尺度3D环境中的空间推理问题，通过主动感知和分层探索实现高效场景理解。


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型主要局限于房间尺度场景，缺乏处理大尺度多楼层3D环境的能力，需要专门的数据集和方法来解决房屋尺度的空间推理问题。

Method: 1) 创建H²U3D数据集：包含多楼层环境，通过自动化标注流程构建分层粗到细的视觉表示和带思维链标注的问答对；2) 提出SpatialReasoner框架：主动感知框架，基于文本查询自主调用空间工具探索3D场景，采用两阶段训练策略（监督冷启动+带自适应探索奖励的强化学习）。

Result: SpatialReasoner在H²U3D数据集上达到最先进性能，优于GPT-4o和Gemini-2.5-Pro等强基线。平均仅使用3-4张图像，而基线需要16+张图像，证明了粗到细主动探索范式的有效性。

Conclusion: H²U3D数据集和SpatialReasoner框架为大规模3D环境中的空间推理提供了有效解决方案，通过主动感知和分层探索实现了高效准确的大尺度场景理解。
...
TL;DR: NavMapFusion：基于扩散模型的在线地图构建框架，融合低精度导航地图与高精度传感器数据，提升自动驾驶环境感知精度


<details>
  <summary>Details</summary>
Motivation: 自动驾驶需要准确的环境表示，传统HD地图需要在线构建，而广泛可用的导航级SD地图分辨率不足但可作为先验指导。如何利用可能过时的粗粒度导航地图指导在线地图构建？

Method: 提出NavMapFusion扩散框架，以高保真传感器数据和低保真导航地图为条件进行迭代去噪。将先验地图与在线感知的差异对应为扩散过程中的噪声，一致区域强化地图构建，过时部分被抑制

Result: 在nuScenes基准测试中，基于OpenStreetMap道路线数据，NavMapFusion在100米感知范围内实现21.4%相对提升，更大感知范围提升更显著，同时保持实时能力
...
TL;DR: 提出SLEDGE框架，通过分层逐步生成设计，模拟设计师的渐进式创作过程，而非传统单步生成方法。


<details>
  <summary>Details</summary>
Motivation: 现有设计生成方法大多将设计合成视为单步生成问题，忽视了设计本质上是一个逐步细化、渐进修改的创造性过程，低估了创作过程的复杂性。

Method: 提出"逐步分层设计生成"新问题设置，利用多模态LLM构建SLEDGE框架，将设计更新建模为基于前状态的原子化分层变化，并以设计指令为基础。

Result: 建立了包含数据集和基准的新评估套件，通过详尽实验分析并与针对新设置的最先进方法比较，证明了所提方法的有效性。

Conclusion: 该工作为这一实用但未充分探索的研究领域吸引了关注，展示了逐步分层设计生成的实际价值和应用前景。
...
TL;DR: ProtoEFNet是一种基于视频的原型学习模型，用于连续射血分数回归，通过学习动态时空原型来捕获临床相关的心脏运动模式，同时保持可解释性。


<details>
  <summary>Details</summary>
Motivation: 射血分数评估传统上需要手动追踪和专业知识，耗时且存在观察者间差异。现有深度学习模型多为黑盒，缺乏透明度，降低了临床信任度。后解释性方法无法指导模型内部推理，临床应用可靠性有限。

Method: 提出ProtoEFNet视频原型学习模型，学习动态时空原型捕获临床相关心脏运动模式。引入原型角度分离损失，在连续EF谱上强制区分性表示。

Result: 在EchonetDynamic数据集上，ProtoEFNet达到与非可解释模型相当的准确性，同时提供临床相关见解。消融研究表明，所提损失将F1分数从77.67±2.68提升至79.64±2.10。

Conclusion: ProtoEFNet通过原型学习实现了准确且可解释的射血分数预测，原型角度分离损失有效提升了模型性能，为临床提供了可信的决策依据。
...
TL;DR: 提出HalluGen框架，通过扩散模型生成可控的幻觉伪影，构建首个大规模幻觉数据集，用于安全关键图像恢复的评估和检测。


<details>
  <summary>Details</summary>
Motivation: 生成模型在图像恢复中容易产生幻觉（plausible but incorrect structures），这在医疗影像、工业检测等安全关键领域尤其危险，可能导致严重诊断错误。现有研究受限于评估幻觉需要标注数据，而标注成本高且主观。

Method: 提出HalluGen框架，基于扩散模型合成具有可控类型、位置和严重程度的真实幻觉。利用该框架构建了包含4,350张标注图像的大规模幻觉数据集（源自1,450张脑部MR图像）。开发了SHAFE指标（基于特征评估的语义幻觉评估）和参考无关的幻觉检测器。

Result: HalluGen生成的幻觉在感知上真实但语义错误（分割IoU从0.86降至0.36）。构建了首个大规模幻觉数据集，支持系统评估。SHAFE指标在幻觉敏感性上优于传统指标，训练的检测器能泛化到真实恢复失败案例。

Conclusion: HalluGen及其开源数据集为安全关键图像恢复中的幻觉评估建立了首个可扩展的基础，解决了评估幻觉的循环依赖问题，推动了该领域的研究进展。
...
<details>
  <summary>Details</summary>
Motivation: 医学影像中微弱、空间分散的异常检测面临挑战，现有架构存在局限性：2D/3D CNN的强局部性约束和ViT的全局注意力扩散都不适合稀疏的早期疾病信号检测。

Method: 系统比较了16种现代深度学习架构，包括2D/3D卷积网络、混合模型和体积Transformer，用于3D眼前段OCT中的亚临床圆锥角膜检测，并进行机制分析。

Result: 分层注意力模型在亚临床异常检测中表现最佳，比2D/3D CNN和ViT的敏感性和特异性高21-23%，且参数效率更高。机制分析显示其优势源于空间尺度对齐。

Conclusion: 分层注意力为3D医学影像中的早期病理变化分析提供了原则性有效方法，其窗口化机制能匹配亚临床异常的中等多切片范围，避免CNN过度局部化和ViT注意力扩散。
...
Conclusion: SeeU的2D→4D→2D框架通过建模4D世界动态，显著提升了视觉内容生成的连续性和物理一致性，为多种视觉任务提供了新的解决方案。
...
TL;DR: 该研究提出了一种基于深度学习的肺癌自动分类系统，使用DenseNet169和SVM模型在IQOTHNCCD CT数据集上达到98%准确率，并集成Grad-CAM和SHAP增强模型可解释性。
...
Method: 使用IQOTHNCCD公开CT数据集（正常、良性、恶性三类）。采用DenseNet169模型，集成Squeeze-and-Excitation注意力模块、Focal Loss处理类别不平衡、特征金字塔网络进行多尺度特征融合。同时开发SVM模型，使用MobileNetV2进行特征提取。集成Grad-CAM可视化CT扫描中的决策区域，SHAP解释SVM模型特征贡献。

Result: DenseNet169和SVM模型均达到98%的准确率，表现出强大的分类性能。模型在准确性和可解释性方面均表现优异，适合实际医疗应用。

Conclusion: 深度学习技术能够显著提高肺癌诊断的准确性、透明度和鲁棒性，为实际医疗实践提供了可靠的工具，具有临床应用潜力。
...
Method: 1) 构建FireSentry省级尺度多模态数据集（亚米空间/亚秒时间分辨率）；2) 提出FiReDiff双模态范式：先在红外模态预测未来视频序列，然后在掩码模态基于生成动态精确分割火场掩码
...
Conclusion: FireSentry基准数据集和FiReDiff范式共同推进了细粒度野火预测和动态灾害模拟，数据集已公开
...
<details>
  <summary>Details</summary>
Motivation: 现有高斯方法存在局限性：要么使用标注的3D标签监督封闭集语义高斯，忽略了渲染能力；要么通过纯2D自监督学习开放集高斯表示，导致几何质量下降且仅限于相机设置。需要充分发挥高斯方法的潜力。

Method: 提出多模态高斯变换器，使高斯能够从不同传感器模态查询特征；采用货架监督学习范式，在2D图像和3D场景级别联合优化高斯特征。
...
<details>
  <summary>Details</summary>
Motivation: 光学与合成孔径雷达(SAR)图像间的跨模态船舶重识别在海上情报与监视中至关重要，但两者间的显著模态差异构成了主要挑战，现有方法难以有效处理这一差距。
...
Result: 在HOSS ReID数据集上的实验表明，MOS在所有评估协议中显著超越现有方法，在ALL to ALL、Optical to SAR和SAR to Optical设置下分别实现了R1准确率+3.0%、+6.2%和+16.4%的显著提升。

Conclusion: MOS框架通过模态对齐和跨模态数据生成有效解决了光学-SAR跨模态船舶重识别中的模态差异问题，为跨模态识别任务提供了有效的解决方案。
...
TL;DR: 提出ViDiC任务和ViDiC-1K数据集，用于评估多模态大语言模型在视频对之间相似性和差异性的细粒度描述能力，现有模型在此任务上存在显著性能差距。


<details>
  <summary>Details</summary>
Motivation: 现有图像差异描述方法无法捕捉动态场景中的运动连续性、事件演化或编辑一致性，需要开发能够理解视频对之间视觉差异的系统。

Method: 引入ViDiC任务和包含1000个视频对的ViDiC-1K数据集，涵盖7个类别，采用基于LLM-as-a-Judge协议的双检查表框架进行可靠评估。

Result: 对19个代表性多模态模型的实验显示，它们在比较性描述和差异感知能力上存在显著性能差距。

Conclusion: ViDiC-1K可作为具有挑战性的基准测试，为推进多模态智能的视频理解、编辑感知和比较推理奠定坚实基础。
...
TL;DR: YOLOA是一个实时可供性检测模型，通过LLM适配器联合处理物体检测和可供性学习，在精度和效率之间取得良好平衡。
...
Method: 提出YOLOA模型，采用轻量级检测器，包含物体检测和可供性学习分支，通过LLM适配器进行精炼。LLM适配器与初步预测交互，生成更准确的类别先验、边界框偏移和可供性门控。

Result: 在重新标注的ADG-Det和IIT-Heat基准测试中达到最先进精度（52.8/73.1 mAP），同时保持实时性能（最高89.77 FPS，轻量版最高846.24 FPS）。

Conclusion: YOLOA通过LLM适配器有效联合处理物体检测和可供性学习，在精度和效率之间实现了优秀平衡，解决了可供性检测中的"是什么-在哪里-如何使用"挑战。
...
Method: 提出了DM3D架构，包含：1)偏移引导的高斯序列化机制，统一局部重采样和全局重排序；2)基于高斯KNN的重采样(GKR)增强结构感知；3)基于高斯的可微分重排序(GDR)实现端到端序列化优化；4)三路径频率融合模块增强特征互补性并减少混叠。

Result: 在基准数据集上的大量实验表明，DM3D在分类、少样本学习和部件分割任务上实现了最先进的性能。

Conclusion: 自适应序列化有效释放了SSMs在点云理解中的潜力，DM3D通过结构自适应序列化机制成功解决了SSMs与点云不规则特性之间的冲突。
...
TL;DR: 首次系统评估8种立体深度估计方法在林业场景的零样本泛化能力，发现不同方法在不同场景表现差异显著，DEFOM在植被密集环境中表现最佳。


<details>
  <summary>Details</summary>
Motivation: 现有深度估计方法评估主要关注城市和室内场景，缺乏对植被密集的林业环境的专门评估，而自主无人机林业作业需要具备强大跨域泛化能力的深度估计方法。

Method: 在Scene Flow数据集上训练8种先进立体方法（包括迭代优化、基础模型和零样本适应范式），然后在4个标准基准（ETH3D、KITTI 2012/2015、Middlebury）和新的5,313对Canterbury林业数据集上进行零样本评估。

Result: 基础模型在结构化场景表现优异（BridgeDepth在ETH3D上0.23px，KITTI上0.83-1.07px），迭代方法保持跨域鲁棒性（IGEV++：0.36-6.77px）。RAFT-Stereo在ETH3D上出现灾难性失败（26.23px EPE，98%错误率）。DEFOM在林业数据集中表现最佳，具有优越的深度平滑性、遮挡处理和跨域一致性。

Conclusion: 深度估计方法的性能高度依赖于场景类型，DEFOM被确定为植被深度估计的最佳基准方法，为自主无人机林业操作提供了关键的评估框架。
...
TL;DR: 提出一种利用预训练扩散模型进行标签高效的高光谱图像分类框架，通过提取扩散模型早期去噪步骤的空间特征，并与光谱信息融合，在稀疏标注下实现优越性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有医学视觉语言预训练方法面临两个主要挑战：1) 网络收集数据的固有噪声问题；2) 非结构化长医学文本的复杂性。需要更鲁棒的方法来处理这些问题。
...
Conclusion: 提出的MAGEN和O-MAKE框架有效解决了医学VLP中的数据噪声和长文本复杂性挑战，为医学图像分析提供了更鲁棒的预训练方法，在皮肤病学领域展现出卓越性能。
...
TL;DR: LM-CartSeg是一个全自动的膝关节MRI分割和影像组学分析流程，通过几何规则处理实现稳健的软骨/骨分割和内外侧分区，为多中心骨关节炎研究提供基础。


<details>
  <summary>Details</summary>
Motivation: 膝关节MRI影像组学需要稳健且具有解剖学意义的ROI来同时捕获软骨和软骨下骨，现有方法多依赖手动ROI且缺乏质量控制报告。

Method: 训练两个3D nnU-Net模型，通过零样本预测融合和几何规则细化：连通分量清洗、构建10mm软骨下骨带、基于PCA和k-means的数据驱动胫骨内外侧分割。

Result: 后处理显著改善分割质量（ASSD从2.63降至0.36mm，HD95从25.2降至3.35mm，DSC 0.91）；几何内外侧规则产生稳定分区；仅6-12%特征与体积/厚度强相关。

Conclusion: LM-CartSeg提供自动、质量控制的ROI和影像组学特征，这些特征携带超出简单形态测量的判别信息，为多中心膝关节OA影像组学研究提供实用基础。
...
TL;DR: 提出无监督学习3D关键点框架，用于条件扩散模型重建完整形状，在关键点一致性上比先前方法提升6个百分点
...
Method: 从点云数据中无监督学习空间结构化的3D关键点，作为紧凑可解释的表征来条件化Elucidated Diffusion Model (EDM)重建完整形状

Result: 学习到的关键点在不同物体实例间展现可重复的空间结构，支持关键点空间的平滑插值，表明其能捕捉几何变化；在关键点一致性上比先前方法提升6个百分点

Conclusion: 提出的无监督框架成功学习到具有空间结构的3D关键点，能够有效条件扩散模型进行形状重建，在多种物体类别上表现优异
...
<details>
  <summary>Details</summary>
Motivation: 扩散模型在视频生成方面表现出色，但需要大量计算资源，每个视频生成需要数十次迭代步骤，而分类器自由指导（CFG）更是将计算需求加倍，这种低效率阻碍了在下游应用中的广泛采用。

Method: 提出GalaxyDiT方法，通过指导对齐和系统代理选择实现计算重用。使用秩次相关分析为每个视频模型确定最佳代理，确保最优计算重用，该方法无需额外训练。

Result: 在Wan2.1-1.3B和Wan2.1-14B模型上分别实现了1.87倍和2.37倍的速度提升，VBench-2.0基准测试中质量下降仅为0.97%和0.72%。在高加速率下，与基础模型的保真度优于先前方法，峰值信噪比（PSNR）高出5-10 dB。

Conclusion: GalaxyDiT提供了一种高效、无需训练的视频生成加速方案，在保持高质量的同时显著降低计算成本，有助于扩散模型在下游应用中的广泛部署。
...
TL;DR: 该论文提出了一种通过深度预测和多视角几何损失来增强视频生成中3D结构一致性的方法，解决了现有2D方法在几何一致性和运动合理性方面的不足。


<details>
  <summary>Details</summary>
Motivation: 当前视频生成方法主要在2D像素空间操作，缺乏明确的3D结构建模机制，导致时间上的几何不一致、运动不合理和结构伪影等问题。

Method: 在潜在扩散模型中引入每帧深度预测，并提出多视角几何损失来在共享3D坐标系中对齐跨帧的预测深度图，从而增强结构一致性。

Result: 在多个数据集上的实验表明，该方法相比现有基线能产生显著更稳定和几何一致的结果，改善了时空连贯性、形状一致性和物理合理性。

Conclusion: 通过深度表示和多视角几何正则化，该方法成功弥合了外观生成与3D结构建模之间的差距，提升了视频生成的几何质量和物理合理性。
...
<details>
  <summary>Details</summary>
Motivation: 现有自动驾驶视觉定位方法难以处理模糊、依赖上下文的指令，缺乏对3D空间关系和场景演化的推理能力，需要更智能的解决方案。

Method: 提出ThinkDeeper框架，包含：1) 空间感知世界模型(SA-WM)，将当前场景蒸馏为指令感知的潜在状态并展开未来状态序列；2) 超图引导解码器，分层融合这些状态与多模态输入，捕获高阶空间依赖。

Result: 在六个基准测试中表现优异：Talk2Car排行榜排名第一，在DrivePilot、MoCAD和RefCOCO/+/g基准上超越最先进基线，在挑战性场景（长文本、多智能体、模糊性）中表现出强鲁棒性和高效性，即使使用50%数据训练仍保持优越性能。

Conclusion: ThinkDeeper通过前瞻性空间推理显著提升了自动驾驶中视觉定位的性能和鲁棒性，为解决模糊指令问题提供了有效框架，并展示了在数据有限情况下的良好泛化能力。
...
TL;DR: 本文提出Text-Printed Image (TPI)方法，通过将文本直接渲染到空白画布上生成合成图像，用于低成本的大视觉语言模型训练，解决了文本中心训练中的模态鸿沟问题。


<details>
  <summary>Details</summary>
Motivation: 现有大视觉语言模型需要大量图像-文本对进行任务特定微调，但图像收集成本高且受隐私限制。文本数据广泛可用且易于编辑，但仅用原始文本训练因模态鸿沟在VQA任务上效果有限。

Method: 提出Text-Printed Image (TPI)方法：将给定文本描述直接渲染到纯白画布上生成合成图像。这种简单渲染将文本投影到图像模态，可低成本集成到现有LVLM训练流程中，且能保持文本语义。

Result: 在四个模型和七个基准测试上的系统实验表明，TPI比扩散模型生成的合成图像在文本中心训练中更有效。TPI作为低成本数据增强策略也显示出实际效用。

Conclusion: 文本中心训练具有显著潜力，TPI方法为LVLMs的全自动数据生成开辟了新路径，解决了模态鸿沟问题，实现了低成本可扩展的训练数据生成。
...
TL;DR: NAS-LoRA：一种新的参数高效微调方法，通过轻量级神经架构搜索块动态优化先验知识，提升SAM在特定领域的适应性能，同时降低训练成本24.14%


<details>
  <summary>Details</summary>
Motivation: 虽然SAM是强大的视觉基础模型，但在医学和农业等特定下游任务中适应仍具挑战。LoRA及其变体虽被广泛使用，但SAM的Transformer编码器缺乏图像块内的空间先验，可能阻碍高级语义信息的获取。需要将归纳偏置整合到模型中。

Method: 提出NAS-LoRA方法：1）在LoRA的编码器和解码器之间加入轻量级神经架构搜索块，动态优化整合到权重更新中的先验知识；2）采用分阶段优化策略，帮助ViT编码器平衡权重更新和架构调整，逐步学习高级语义信息。

Result: 实验表明NAS-LoRA改进了现有的PEFT方法，同时在不增加推理成本的情况下将训练成本降低了24.14%，证明了NAS在增强视觉基础模型PEFT方面的潜力。

Conclusion: NAS-LoRA通过整合神经架构搜索到参数高效微调框架中，有效弥合了预训练SAM与专业领域之间的语义鸿沟，为视觉基础模型的高效适应提供了新思路。
...
TL;DR: 提出基于基分解的模块BDM，扩展为SD²M、SD³M、TD²M等模块，构建SD²Net用于单帧红外小目标检测，STD²Net用于多帧检测，在SISTD和MISTD任务上取得SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 红外小目标检测面临两个主要挑战：目标纹理不明显和背景杂波严重，导致背景干扰目标识别。需要增强目标特征并抑制背景干扰。

Method: 提出基分解模块（BDM），将复杂特征分解为多个基特征，增强有用信息并消除冗余。基于BDM扩展出SD²M（空间差异分解模块）、SD³M（空间差异分解下采样模块）和TD²M（时间差异分解模块）。SD²Net采用U型架构集成SD²M和SD³M，STD²Net通过TD²M引入运动信息。

Result: 在SISTD任务上，SD²Net表现优异；在MISTD数据集上，STD²Net达到87.68% mIoU，显著优于SD²Net的64.97%。实验证明在两个任务上都实现了SOTA性能。

Conclusion: 提出的基分解方法能有效增强红外小目标特征并抑制背景干扰，SD²Net和STD²Net分别在单帧和多帧检测任务中表现出色，为红外小目标检测提供了有效的解决方案。
...
TL;DR: PRIS框架通过在推理时自适应地修订提示词来改进文本到视觉生成，而不是固定提示词。它通过分析生成的视觉内容，识别失败模式，并重新设计提示词，然后使用修订后的提示词重新生成。


<details>
  <summary>Details</summary>
Motivation: 文本到视觉生成中，用户意图与生成视觉之间的精确对齐是一个核心挑战。传统方法主要通过扩展视觉生成过程（如增加采样步骤或种子）来改进，但这很快会达到质量瓶颈，因为提示词在整个过程中保持不变。

Method: 提出PRIS框架，在推理时自适应地修订提示词。核心思想是：审查生成的视觉内容，识别跨视觉内容的重复失败模式，相应地重新设计提示词，然后使用修订后的提示词重新生成视觉内容。引入了元素级事实校正验证器，在细粒度级别评估提示属性与生成视觉内容之间的对齐。

Result: 在文本到图像和文本到视频基准测试上的广泛实验证明了该方法的有效性，包括在VBench 2.0上获得15%的提升。结果表明，联合扩展提示词和视觉内容是充分利推理时扩展定律的关键。

Conclusion: PRIS框架通过自适应地修订提示词而不是固定提示词，显著改进了文本到视觉生成的质量。这表明在推理时联合扩展提示词和视觉内容比仅扩展视觉生成过程更有效。
...
Conclusion: 建模动作执行和结果能提高错误检测可靠性，效果感知表示有潜力应用于更广泛的下游任务。
...
TL;DR: CookAnything是一个基于扩散模型的框架，能够从任意长度的烹饪指令生成连贯、语义分明的图像序列，解决了现有方法无法适应不同食谱长度的问题。


<details>
  <summary>Details</summary>
Motivation: 现有扩散模型难以处理结构化多步骤场景（如食谱插图），且当前食谱插图方法无法适应食谱长度的自然变化，总是生成固定数量的图像，与实际指令结构不符。
...
Conclusion: 该框架在指令媒体和程序化内容创作等领域具有广泛应用潜力，能够生成连贯、语义分明的烹饪步骤图像序列。
...
TL;DR: 提出公平感知的低秩适应方法（Fairness-aware Low-Rank Adaptation）用于医学视觉语言模型，通过可微分的MaxAccGap损失函数优化跨人口群体的准确性公平性，在仅需0.24%可训练参数的情况下显著减少诊断准确性差异。


<details>
  <summary>Details</summary>
Motivation: 医学视觉语言模型在医疗影像任务上表现出专家级性能，但在不同人口群体间存在显著的诊断准确性差异。需要开发既能保持参数效率又能明确优化公平性的方法，以适应资源受限的医疗环境。

Method: 提出三种方法：FR-LoRA（将MaxAccGap正则化集成到训练目标中）、GR-LoRA（应用逆频率加权平衡梯度贡献）和Hybrid-LoRA（结合两种机制）。核心算法贡献是可微分的MaxAccGap损失函数，实现跨人口群体准确性公平性的端到端优化。

Result: 在10,000张青光眼眼底图像上评估，GR-LoRA将诊断准确性差异减少69%，同时保持53.15%的整体准确性。消融研究表明，强正则化强度能以最小准确性代价实现最优公平性，种族特定优化可实现60%的差异减少。

Conclusion: 该方法仅需0.24%的可训练参数，使公平医疗AI在资源受限的医疗环境中具有实际部署可行性，为解决医学AI中的公平性问题提供了参数高效且有效的解决方案。
...
Method: 提出V-ITI框架，包含两个组件：1）视觉忽视检测器：通过头级判别探针识别视觉忽视；2）视觉回忆干预器：仅在检测到视觉忽视时，用预存的视觉激活信息调制激活。该方法基于研究发现，视觉忽视可以通过MLLMs中的头级激活模式准确检测。

Result: 在八个基准测试和不同MLLM家族上的广泛实验表明，V-ITI能够持续缓解与视觉相关的幻觉，同时保持一般任务性能。

Conclusion: V-ITI通过解决"何时干预"的关键问题，提供了一种轻量级、高效的视觉推理时干预方法，有效减少多模态大语言模型的幻觉，提高其在精度敏感应用中的可靠性。
...
TL;DR: 论文提出从动作中心转向物体中心的程序活动理解范式，将动作视为驱动状态转换的机制，并创建了Object-IVQA基准测试来评估物体中心推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有基于动作的方法难以处理真实程序中步骤顺序随物体状态变化的灵活性，需要转向物体中心的范式来更好地理解程序性活动。

Method: 提出物体中心范式，将动作视为驱动状态转换的机制；创建Object-IVQA基准测试（107个视频，514个开放性问题）；开发协调物体中心规划、感知、分析和生成工具的智能体框架。

Result: 现有大型视觉语言模型在物体级识别和推理方面表现不佳，而提出的框架实现了显著改进。

Conclusion: 物体中心范式能更好地理解程序性活动，提出的框架通过显式证据检索和跨片段多跳推理，在物体中心推理任务上表现出色。
...
TL;DR: 提出一个混合内容审核框架，结合监督分类和基于相似性的匹配，用于直播平台的多模态内容审核，显著减少不良内容观看
...
Method: 混合审核框架：1) 监督分类管道处理已知违规；2) 基于参考的相似性匹配管道处理新颖或微妙案例；3) 多模态大语言模型将知识蒸馏到两个管道中，提升准确性同时保持推理轻量

Result: 生产环境中：分类管道在80%精确度下达到67%召回率；相似性管道在80%精确度下达到76%召回率；大规模A/B测试显示用户观看不良直播减少6-8%

Conclusion: 该框架提供了一个可扩展且适应性强的多模态内容治理方法，能够同时处理明确违规和新兴对抗行为
...
TL;DR: CartoMapQA是一个专门评估视觉语言模型在地图理解能力上的基准测试，包含2000多个样本，涵盖从符号识别到路线推理的多层次地图解读技能，揭示了现有模型在地图语义理解、空间推理和OCR识别方面的局限性。


<details>
  <summary>Details</summary>
Motivation: 虽然视觉语言模型在视觉-文本融合方面取得了进展，但它们在地图解读方面的能力尚未得到充分探索。地图理解对于导航、地理搜索和城市规划等实际应用至关重要，因此需要专门的评估工具来衡量和改进模型的地图理解能力。

Method: 研究者创建了CartoMapQA基准测试，包含2000多个样本，每个样本由地图图像、问题（开放式或多选题）和标准答案组成。任务涵盖低、中、高三个层次的地图解读技能：符号识别、信息提取、比例尺解读和路线推理。对开源和专有视觉语言模型进行了全面评估。

Result: 评估发现现有模型存在持续挑战：1）难以理解地图特定语义；2）空间推理能力有限；3）容易产生OCR相关错误。这些弱点表明当前模型在地图理解方面仍有显著不足。

Conclusion: CartoMapQA为评估和改进视觉语言模型的地图理解能力提供了有价值的工具，有助于开发更适用于依赖可靠地图理解的实际应用（如导航、地理搜索和城市规划）的模型。代码和数据已开源供研究社区使用。
...
TL;DR: EEA是一个用于长视频理解的新型视频代理框架，通过语义引导的层次树搜索过程实现探索-利用平衡，在保证计算效率的同时提升信息覆盖完整性。


<details>
  <summary>Details</summary>
Motivation: 当前长视频理解方法存在两个主要问题：1）密集预处理导致计算开销过大；2）无法有效平衡探索与利用，导致信息覆盖不完整和效率低下。

Method: EEA框架通过语义引导的层次树搜索过程实现探索-利用平衡。它自主发现并动态更新任务相关的语义查询，收集与这些查询匹配的视频帧作为语义锚点。在树搜索过程中，优先探索语义相关帧，同时确保未知段落的充分覆盖。此外，通过显式建模不确定性，自适应地结合视觉语言模型的内在奖励与语义先验。

Result: 在多个长视频基准测试上的实验验证了该方法在性能和计算效率方面的优越性。

Conclusion: EEA框架通过语义引导的层次树搜索有效解决了长视频理解中的探索-利用平衡问题，在保证计算效率的同时实现了更完整的信息覆盖。
...
TL;DR: 该论文质疑基于视觉的文本压缩对语言建模的有效性，发现简单替代方法在相同压缩比下表现相当或更好，视觉压缩在语言建模上甚至不如截断方法。


<details>
  <summary>Details</summary>
Motivation: DeepSeek-OCR展示了从少量视觉token高保真重建渲染文本的能力，引发了视觉上下文压缩的热潮。但现有评估仅停留在重建层面，未验证这些表示是否真正有助于语言建模。

Method: 比较DeepSeek-OCR的视觉编码器与两种简单替代方法：无参数的平均池化和学习的分层编码器，在相同压缩比下测试文本重建和语言建模性能。

Result: 简单方法在文本重建上匹配或超越视觉方法，在语言建模上表现更好，而基于视觉的压缩甚至无法超越简单的截断方法。

Conclusion: 对光学上下文压缩的热情超出了实际证据支持，视觉压缩在语言建模中的优势尚未得到证实。
...
TL;DR: DPMFormer提出了一种新的域泛化语义分割框架，通过域感知提示学习、对比学习和一致性学习来解决视觉与文本上下文语义不对齐问题。


<details>
  <summary>Details</summary>
Motivation: 现有基于视觉语言模型（VLMs）的域泛化语义分割方法忽略了视觉与文本上下文之间的语义不对齐问题，这源于在单一源域上学习的固定上下文提示的刚性。

Method: 1. 域感知提示学习：促进视觉与文本线索的语义对齐
2. 域感知对比学习：通过纹理扰动捕获各种域特定属性
3. 域鲁棒一致性学习：最小化原始图像与增强图像预测之间的差异

Result: 该方法在各种DGSS基准测试中建立了新的最先进水平。

Conclusion: DPMFormer通过解决语义不对齐问题，显著提升了域泛化语义分割的性能，代码已开源。
...
### [46] [MKSNet: Advanced Small Object Detection in Remote Sensing Imagery with Multi-Kernel and Dual Attention Mechanisms](https://arxiv.org/abs/2512.03640)
...
<details>
  <summary>Details</summary>
Motivation: 遥感图像中小目标检测面临挑战：高分辨率图像中目标尺寸小，在CNN深层网络中容易丢失关键信息；遥感图像存在大量空间冗余和复杂背景细节，容易掩盖小目标。

Method: 提出MKSNet网络架构，包含：1）多核选择机制，使用大卷积核捕获广泛上下文信息，自适应选择核大小；2）双注意力机制，结合空间注意力模块（自适应调整特征图空间权重）和通道注意力模块（优化通道信息选择）。

Result: 在DOTA-v1.0和HRSC2016基准测试中，MKSNet显著超越了现有最先进模型，特别是在遥感图像小目标检测方面表现出色。

Conclusion: MKSNet能够有效处理多尺度和高分辨率图像数据的复杂性，在遥感目标检测中展现出卓越的有效性和创新性。
...
TL;DR: CodeVision：一个基于代码作为通用工具的MLLM框架，通过两阶段训练（SFT+RL）提升视觉推理的鲁棒性和工具组合能力


<details>
  <summary>Details</summary>
Motivation: 当前MLLM在图像方向变化或自然损坏时表现脆弱，且工具集有限，缺乏真实世界必要性和可扩展性，需要更鲁棒的工具推理方法

Method: 提出CodeVision框架，将代码作为通用接口调用任意图像操作；采用两阶段训练：先在高质量多轮工具组合数据集上SFT，再用密集过程奖励函数进行RL训练

Result: 在Qwen2.5-VL和Qwen3-VL系列上显著提升性能，涌现出灵活工具组合、高效链式执行、运行时反馈错误恢复等能力

Conclusion: CodeVision通过代码作为工具的方法解决了MLLM的脆弱性问题，提供了可扩展的鲁棒视觉推理框架，并发布了新数据集和基准测试
...
TL;DR: 该论文提出了一种基于计算机视觉的舞蹈动作自动分析框架，结合YOLOv8/v11进行舞者检测和SAM进行精确分割，实现了无标记的舞蹈动作跟踪与量化分析。


<details>
  <summary>Details</summary>
Motivation: 传统舞蹈分析通常依赖专业设备或人工标记，成本高且难以扩展。本研究旨在探索利用当代计算机视觉技术实现自动化舞蹈动作分析的可能性，为量化舞蹈表现建立技术基础。

Method: 提出概念验证框架：1) 使用YOLOv8/v11进行舞者检测；2) 利用Segment Anything Model (SAM)进行像素级分割；3) 基于检测和分割结果进行动作跟踪和量化，包括舞步计数、空间覆盖模式分析和节奏一致性测量。

Result: 在49秒的加纳AfroBeats舞蹈视频测试中：检测精度约94%，召回率89%；SAM分割IoU约83%；量化分析显示主要舞者比次要舞者多执行23%的舞步，运动强度高37%，使用空间多42%。
...
TL;DR: ToG-Bench是首个面向任务的时空视频定位基准，针对第一人称视频，强调任务导向推理而非简单描述性指令，包含显式-隐式双重定位和多对象定位挑战。


<details>
  <summary>Details</summary>
Motivation: 现有时空视频定位研究主要关注对象中心和描述性指令，忽视了任务导向推理，而这对实现具身智能完成目标导向交互至关重要。需要建立连接感知与交互的基准。

Method: 基于ScanNet视频构建ToG-Bench基准，包含100个标注片段和2,704个任务导向定位指令，采用基础模型标注加人工细化的半自动化流程。引入任务级评估指标，系统评估7个最先进的多模态大语言模型。

Result: 实验揭示了任务导向时空视频定位的内在挑战，在显式-隐式和多对象定位方面存在显著性能差距，表明在具身场景中连接感知与交互的难度很大。

Conclusion: ToG-Bench填补了任务导向时空视频定位基准的空白，突出了具身智能中感知与交互之间的鸿沟，为未来研究提供了重要方向。
...
### [50] [AdaptVision: Efficient Vision-Language Models via Adaptive Visual Acquisition](https://arxiv.org/abs/2512.03794)
...
Method: 提出AdaptVision范式：1）初始处理低分辨率图像的压缩视觉token；2）必要时调用边界框工具裁剪关键区域获取额外视觉信息；3）使用强化学习框架训练，提出解耦轮次策略优化（DTPO），将学习目标分解为工具学习和准确性改进两个组件，并分别计算优势估计。

Result: 在多个VQA基准测试中，AdaptVision相比现有高效VLM方法，在消耗显著更少视觉token的同时，实现了更优越的性能。

Conclusion: AdaptVision通过自适应视觉token获取机制，成功实现了视觉语言模型在准确性和效率之间的更好平衡，为高效VLM设计提供了新思路。
...
TL;DR: CSMapping：利用扩散模型先验和约束优化，通过众包数据构建高质量语义地图和拓扑道路中心线，质量随数据量增加而提升


<details>
  <summary>Details</summary>
Motivation: 众包可以实现可扩展的自动驾驶地图构建，但低成本传感器噪声导致地图质量无法随数据量增加而提升，需要解决噪声鲁棒性和地图质量随数据量扩展的问题

Method: 1. 语义地图：训练潜在扩散模型学习真实世界地图结构的生成先验，通过约束MAP优化在潜在空间整合先验；2. 拓扑地图：应用置信度加权k-medoids聚类和运动学细化轨迹，生成平滑的道路中心线

Result: 在nuScenes、Argoverse 2和大型专有数据集上实现最先进的语义和拓扑地图性能，质量随数据量增加而持续提升

Conclusion: CSMapping系统能够从众包数据中构建高质量的地图，解决了噪声鲁棒性和可扩展性问题，为自动驾驶地图构建提供了有效的解决方案
...
TL;DR: BlackCAtt：一种基于因果像素的黑盒对抗攻击方法，针对目标检测器生成可解释、不可感知、可复现的对抗样本


<details>
  <summary>Details</summary>
Motivation: 现有对抗攻击方法多为白盒且架构特定，难以理解其工作原理。需要一种能够解释攻击机制的方法，帮助开发者理解和分析攻击，并改进模型防御能力。

Method: 提出BlackCAtt黑盒算法，使用最小因果像素集结合目标检测器的边界框，创建导致边界框丢失、修改或新增的对抗攻击。该方法将检测器视为黑盒，适用于不同架构和规模的检测器。

Result: 在COCO测试集上，BlackCAtt比基线方法在移除检测方面好2.7倍，在修改检测方面好3.86倍，在触发新虚假检测方面好5.75倍。生成的攻击几乎不可感知。

Conclusion: 因果像素识别能够产生更精确、更不可感知的攻击，BlackCAtt展示了因果像素在创建可解释、架构无关的黑盒对抗攻击中的强大能力。
...
TL;DR: 本文提出Stable Signer模型，将手语生成重新定义为包含文本理解（Prompt2Gloss、Text2Gloss）和Pose2Vid的端到端分层生成任务，通过SLUL进行文本理解，使用SLP-MoE手势渲染专家块生成高质量多风格手语视频，性能提升48.6%。


<details>
  <summary>Details</summary>
Motivation: 传统手语生成方法存在文本转换不准确、姿势生成错误、姿势渲染到真人视频效果差等问题，导致误差逐渐累积，进展缓慢。需要简化冗余结构，优化任务目标。

Method: 1. 重新定义SLP为端到端分层生成任务（文本理解 + Pose2Vid）；2. 提出SLUL（手语理解链接器）进行文本理解；3. 使用SLP-MoE手势渲染专家块生成手势；4. 训练SLUL时采用新开发的SAGM Loss（语义感知词元掩码损失）。
...
### [54] [FloodDiffusion: Tailored Diffusion Forcing for Streaming Motion Generation](https://arxiv.org/abs/2512.03520)
...
<details>
  <summary>Details</summary>
Motivation: 现有方法依赖分块处理或带扩散头的自回归模型，无法有效处理时间变化的文本提示下的流式运动生成任务。需要一种能够处理时间变化控制事件并保证输出分布建模的方法。

Method: 采用扩散强制框架，但改进了原始方法：1) 使用双向注意力而非因果注意力；2) 实现下三角时间调度器而非随机调度；3) 采用连续时间变化的方式引入文本条件。这些改进确保了输出分布的正确建模。

Result: 在流式运动生成任务上首次实现了最先进的性能，在HumanML3D基准测试中达到FID 0.057，能够以实时延迟生成文本对齐的无缝运动序列。

Conclusion: FloodDiffusion通过改进的扩散强制框架成功解决了文本驱动的流式人体运动生成问题，首次证明了扩散强制方法在该任务上的有效性，并提供了模型、代码和权重。
...
### [55] [Research on Brain Tumor Classification Method Based on Improved ResNet34 Network](https://arxiv.org/abs/2512.03751)
*Yufeng Li,Wenchao Zhao,Bo Dang,Weimin Wang*

Main category: cs.CV

TL;DR: 提出基于改进ResNet34的脑肿瘤分类模型，通过多尺度特征提取和通道注意力机制，在减少参数的同时提高分类准确率至98.8%
...
Method: 以ResNet34残差网络为主干网络，引入多尺度特征提取：使用多尺度输入模块作为ResNet34网络的第一层，使用Inception v2模块作为残差下采样层。加入通道注意力机制模块，从通道域角度为图像不同通道分配不同权重，获取更重要的特征信息。
...
TL;DR: OpenTrack3D：一个用于开放词汇3D实例分割的通用框架，通过视觉-空间跟踪器在线构建跨视角一致的对象提议，并使用多模态大语言模型增强组合推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有开放词汇3D实例分割方法存在两个关键限制：1) 提议生成依赖数据集特定的提议网络或基于网格的超点，在无网格场景中不适用且泛化能力有限；2) CLIP分类器的文本推理能力弱，难以处理组合式和功能性的用户查询。

Method: 1) 使用2D开放词汇分割器生成掩码，通过深度信息提升到3D点云；2) 利用DINO特征图提取掩码引导的实例特征；3) 设计视觉-空间跟踪器融合视觉和空间线索以保持实例一致性；4) 提供可选的超点细化模块（当场景网格可用时）；5) 用多模态大语言模型替代CLIP以增强组合推理能力。

Result: 在ScanNet200、Replica、ScanNet++和SceneFun3D等多个基准测试中取得了最先进的性能，展示了强大的泛化能力。

Conclusion: OpenTrack3D是一个通用且准确的框架，解决了现有方法在无网格场景中的局限性，并通过MLLM增强了复杂查询的理解能力，为机器人和AR/VR应用提供了更好的3D场景理解工具。
...
TL;DR: PULSE是一个统一的心脏图像分析多任务视觉语言框架，可在单一架构中同时处理解剖分割、疾病分类和临床报告生成，并跨模态和数据集泛化。


<details>
  <summary>Details</summary>
Motivation: 目前心脏图像分析任务分散：解剖分割、疾病分类和临床报告生成通常由不同网络分别处理，缺乏统一框架。现有方法无法在单一架构中统一这些目标，同时保持跨成像模态和数据集的泛化能力。

Method: 基于自监督表示构建多任务视觉语言框架，采用复合监督策略平衡区域重叠学习、像素级分类保真度和边界感知IoU细化。多尺度令牌重建解码器实现解剖分割，共享全局表示支持疾病分类和临床文本输出。

Result: PULSE学习任务不变的心脏先验知识，在数据集间稳健泛化，能够以最小监督适应新的成像模态，将领域推向可扩展的基础模型式心脏分析框架。

Conclusion: PULSE首次在单一架构中统一了心脏图像分析的主要任务，实现了从像素到结构再到临床推理的完整流程，为可扩展的心脏分析基础框架迈出了重要一步。
...
TL;DR: BlurDM：一种将模糊形成过程无缝集成到扩散模型中的去模糊方法，通过双重扩散方案同时去噪和去模糊，显著提升现有去模糊方法性能。


<details>
  <summary>Details</summary>
Motivation: 现有基于扩散模型的动态场景去模糊方法未能充分利用模糊形成过程的本质特性，限制了其潜力。需要将模糊形成过程自然地整合到扩散模型中。

Method: 提出BlurDM，通过双重扩散前向方案隐式建模模糊形成过程，在噪声和模糊上同时扩散。在反向生成过程中，推导出双重去噪和去模糊公式，使模型能够同时执行去噪和去模糊。在潜在空间中执行BlurDM，形成灵活的先验生成网络。

Result: 在四个基准数据集上的大量实验表明，BlurDM能够显著且一致地增强现有去模糊方法的性能。

Conclusion: BlurDM成功地将模糊形成过程整合到扩散模型中，通过双重扩散方案实现了同时去噪和去模糊，为图像去模糊任务提供了有效的解决方案。
...
### [59] [GAOT: Generating Articulated Objects Through Text-Guided Diffusion Models](https://arxiv.org/abs/2512.03566)
...
Method: 三阶段框架：1) 微调点云生成模型从文本生成粗略物体表示；2) 设计超图学习方法细化表示，将物体部件表示为图顶点；3) 使用扩散模型生成铰接关节（图边）。
...
<details>
  <summary>Details</summary>
Motivation: 现有基准主要关注静态高质量图像，忽略了时序退化和错误传播这两个关键失效模式——瞬态视觉退化会引发幻觉并在后续帧中持续存在，而实际部署的VLM（如自动驾驶）必须处理不完美条件下的连续视觉流。

Method: 1) DIQ-H基准：应用运动模糊、传感器噪声和压缩伪影等物理基础退化；2) 通过多轮问答任务测量幻觉持续性、错误恢复和时序一致性；3) UIR方法：使用轻量级VLM进行不确定性过滤，生成可靠的伪真实标签。

Result: 实验评估16个SOTA VLM显示显著鲁棒性差距：即使GPT-4o恢复率仅78.5%，开源模型时序一致性低于60%；UIR方法实现15.3%的准确率提升，实现可扩展标注。

Conclusion: DIQ-H为评估VLM在真实世界部署中的可靠性提供了全面平台，揭示了现有模型在动态视觉退化下的严重鲁棒性问题，为未来研究提供了重要基准。
...
### [61] [Global-Local Aware Scene Text Editing](https://arxiv.org/abs/2512.03574)
...
<details>
  <summary>Details</summary>
Motivation: 现有场景文本编辑方法存在两个主要问题：1）编辑后的局部区域与周围环境不一致；2）无法有效处理编辑前后文本长度的显著差异。

Method: 提出GLASTE端到端框架，包含：全局-局部组合结构、联合全局和局部损失、增强的文本图像特征提取。将文本风格表示为与图像大小无关的向量，并使用仿射融合保持目标文本的宽高比。

Result: 在真实数据集上的实验表明，GLASTE在定量指标和定性结果上都优于先前方法，有效缓解了一致性和长度敏感性挑战。

Conclusion: GLASTE通过同时考虑全局上下文和局部特征，成功解决了场景文本编辑中的一致性和长度敏感性难题，为实际应用提供了更可靠的解决方案。
...
TL;DR: 提出一种新的测试时缩放方法TEP-Diffusion，通过文本嵌入扰动增强文本到图像扩散模型的生成多样性和质量，无需额外计算成本。


<details>
  <summary>Details</summary>
Motivation: 现有测试时缩放方法主要关注搜索策略和奖励模型，但忽略了扩散模型中噪声的随机特性对方法性能的影响。本文旨在探索文本嵌入扰动这种新的随机性形式，以增强生成多样性和质量。

Method: 1) 引入基于步骤的文本嵌入扰动，结合频率引导的噪声调度和空间噪声扰动；2) 根据频率特定贡献和扰动容忍度自适应调整扰动强度。方法可无缝集成到现有TTS方法中。

Result: 在多个基准测试上表现出显著改进，几乎不需要额外计算成本。代码已开源。

Conclusion: 文本嵌入扰动与空间噪声在频域上具有互补性，能有效增强扩散模型的生成多样性和质量，为测试时缩放提供了新的随机性形式。
...
### [63] [UniComp: Rethinking Video Compression Through Informational Uniqueness](https://arxiv.org/abs/2512.03575)
...
<details>
  <summary>Details</summary>
Motivation: 现有注意力压缩方法存在局限，需要从信息论角度出发，在有限计算预算下最大化视频表示的信息保真度。
...
<details>
  <summary>Details</summary>
Motivation: 当前大型多模态模型处理长视频时面临上下文长度限制和密集视频标记计算成本高的问题。现有查询感知帧选择方法计算开销大，但作者质疑这种复杂机制是否总是必要的。

Method: 提出DIG框架：1）识别并验证查询类型学，区分全局查询和局部查询；2）全局查询采用高效均匀采样；3）局部查询激活专门管道提取查询相关帧；4）无需训练，自适应选择策略。
...
### [65] [Cross-Stain Contrastive Learning for Paired Immunohistochemistry and Histopathology Slide Representation Learning](https://arxiv.org/abs/2512.03577)
*Yizhi Zhang,Lei Fan,Zhulin Tao,Donglin Di,Yang Song,Sidong Liu,Cong Cong*

Main category: cs.CV

TL;DR: 提出CSCL框架，通过跨染色对比学习增强WSI表征，利用五染色数据集提升H&E特征的生物学信息兼容性


<details>
  <summary>Details</summary>
Motivation: 计算病理学需要通用的WSI表征，多染色数据能丰富H&E特征，但现有数据集存在染色间错位问题，限制了进展

Method: 两阶段预训练框架：1) 轻量适配器进行补丁级对比对齐；2) MIL框架集成染色特异性特征，包含跨染色注意力融合和全局对齐模块

Result: 在癌症亚型分类、IHC生物标志物状态分类和生存预测任务上均取得一致提升，获得高质量可迁移的H&E切片级表征

Conclusion: CSCL框架通过跨染色对比学习有效解决了染色间错位问题，显著提升了WSI表征的质量和可迁移性
...
TL;DR: 研究表明：虽然可以将草图视为序列，但绝对坐标优于相对坐标，非自回归解码器优于自回归解码器，时序重要性取决于具体顺序和任务。


<details>
  <summary>Details</summary>
Motivation: 尽管草图表示学习领域已有显著进展，但人们对时序方面对表示质量的实际相关性仍缺乏深入理解。本研究旨在探究将草图视为序列是否合理，以及哪些内部顺序更具相关性。

Method: 通过比较不同时序建模方法，包括传统位置编码、绝对坐标与相对坐标的对比，以及自回归与非自回归解码器的性能评估，来研究草图序列表示的有效性。

Result: 结果表明：1) 使用传统位置编码对草图序列建模是有效的；2) 绝对坐标始终优于相对坐标；3) 非自回归解码器优于自回归解码器；4) 时序的重要性取决于所考虑的顺序和评估的任务。

Conclusion: 草图可以合理地视为序列，但最佳表示方法需要考虑坐标类型（绝对优于相对）、解码器架构（非自回归优于自回归），且时序的重要性是任务和顺序依赖的。
...
### [67] [Dynamic Optical Test for Bot Identification (DOT-BI): A simple check to identify bots in surveys and online processes](https://arxiv.org/abs/2512.03580)
*Malte Bleeker,Mauro Gotsch*

Main category: cs.CV

TL;DR: DOT-BI是一种基于人类运动感知的快速机器人识别方法，通过隐藏数字在动态纹理背景中，使人眼可见而算法难以检测。


<details>
  <summary>Details</summary>
Motivation: 需要一种快速简便的方法来区分在线调查和流程中的人类响应者与自动化系统，解决传统CAPTCHA的可用性问题。

Method: 使用与背景相同随机黑白像素纹理的隐藏数字，通过数字与背景之间的运动和尺度差异使其在连续帧中对人类可见，而逐帧算法处理无法获得有意义的信号。

Result: 1) 最先进的多模态模型(GPT-5-Thinking和Gemini 2.5 Pro)即使获得明确机制说明也无法提取正确值；2) 在线调查中99.5%参与者成功完成任务，平均完成时间10.7秒；实验室研究显示相对于对照组没有负面可用性影响。

Conclusion: DOT-BI是一种有效且用户友好的机器人识别方法，通过人类独特的运动感知能力区分人类和自动化系统，并提供了代码和预渲染变体以促进实际应用。
...
TL;DR: 提出Pyramid Sparse Attention (PSA)，一种用于视频理解和生成任务的高效注意力机制，通过多级池化KV表示替代二元掩码，在保持计算效率的同时减少信息损失。


<details>
  <summary>Details</summary>
Motivation: 注意力机制是基础模型的核心，但其二次复杂度限制了模型扩展。现有稀疏注意力方法通常使用二元掩码保留或丢弃整个KV块，在高稀疏度下会造成显著信息损失。

Method: 提出PSA模块，采用多级池化KV表示而非二元掩码。每个查询块动态分配较低的池化级别给关键KV块，较高的池化级别给次要KV块，形成从完全保留到完全剪枝的信息插值。设计采用硬件友好的解耦块-瓦片内核。

Result: 在视频理解和生成基准测试中，PSA保持了上下文信息和视觉保真度，在效率-质量权衡方面优于或与现有稀疏注意力基线相当。

Conclusion: PSA通过多级池化KV表示有效缓解了信息损失问题，在低计算预算下保持了计算效率，为视频任务提供了更好的稀疏注意力解决方案。
...
### [69] [Beyond Boundary Frames: Audio-Visual Semantic Guidance for Context-Aware Video Interpolation](https://arxiv.org/abs/2512.03590)
*Yuchen Deng,Xiuyang Wu,Hai-Tao Zheng,Jie Wang,Feidiao Yang,Yuxing Han*

Main category: cs.CV

TL;DR: BBF是一个上下文感知的视频帧插值框架，通过解耦的多模态融合机制和渐进式训练，支持文本、音频、图像、视频等多种条件输入，在通用插值和音视频同步插值任务上表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有方法在处理快速、复杂、高度非线性的运动模式时存在困难，特别是扩散模型方法虽然优于传统光流方法，但在多样化应用场景和细粒度运动任务（如音视频同步插值）中仍难以生成清晰、时间一致的帧。

Method: 1. 增强插值模型的输入设计，使其能灵活处理多种条件模态（文本、音频、图像、视频）；2. 提出解耦的多模态融合机制，将不同条件信号顺序注入DiT骨干网络；3. 采用渐进式多阶段训练范式，使用起始帧差异嵌入动态调整数据采样和损失权重。

Result: 大量实验结果表明，BBF在通用插值和音视频同步插值任务上都优于专门的SOTA方法，建立了在协调多通道条件下的统一视频帧插值框架。
...
### [70] [Harnessing Hypergraphs in Geometric Deep Learning for 3D RNA Inverse Folding](https://arxiv.org/abs/2512.03592)
*Guang Yang,Lei Fan*
...
<details>
  <summary>Details</summary>
Motivation: RNA逆折叠问题是一个关键挑战，涉及寻找能折叠成所需二级结构的核苷酸序列，这对分子稳定性和功能至关重要。序列与结构之间的复杂关系使得这项任务特别具有挑战性。

Method: 提出HyperRNA框架，包含三个主要组件：预处理阶段基于3-bead粗粒度表示构建RNA骨架原子坐标图；编码阶段使用注意力嵌入模块和超图编码器捕获高阶依赖和复杂生物分子相互作用；解码阶段以自回归方式生成RNA序列。

Result: 在PDBBind和RNAsolo数据集上的定量和定性实验表明，HyperRNA在RNA序列生成和RNA-蛋白质复合物序列生成任务中优于现有RNA设计方法，展示了超图在RNA工程中的潜力。

Conclusion: HyperRNA不仅超越了现有RNA设计方法，还突显了利用超图进行RNA工程的潜力，为解决RNA逆折叠问题提供了有效框架。
...
TL;DR: 该论文在生成模型效率提升和计算机视觉应用两方面做出贡献：第一部分通过6项创新改进归一化流架构，包括可逆卷积层、高效耦合层和并行反演算法；第二部分展示了5个实际应用，包括农业质量评估、地质制图、自动驾驶隐私保护和艺术修复。


<details>
  <summary>Details</summary>
Motivation: 论文旨在解决两个核心问题：1) 提高生成模型（特别是归一化流）的计算效率，使其更实用；2) 将生成模型应用于实际计算机视觉挑战，解决现实世界中的具体问题。

Method: 第一部分：提出6项归一化流架构改进：可逆3x3卷积层（有数学证明）、Quad-coupling层、并行反演算法、高效反向传播算法、Inverse-Flow训练方法、Affine-StableSR超分辨率模型。第二部分：应用生成模型解决实际问题：使用Conditional GANs进行农业质量评估、堆叠自编码器进行地质制图、基于人脸检测和图像修复的隐私保护方法、Stable Diffusion图像修复、扩散模型艺术修复。

Result: 在效率方面：提出的算法和架构显著提升了归一化流的计算性能；在应用方面：农业质量评估系统在种子纯度测试中取得良好准确率，地质制图框架相比传统方法改进了特征提取，隐私保护方法有效保护敏感信息，艺术修复模型能统一处理多种退化类型。

Conclusion: 该论文通过理论创新和实际应用展示了生成模型在计算机视觉领域的巨大潜力，既提升了模型效率，又解决了多个现实世界问题，为生成模型的实际部署和应用提供了有价值的贡献。
...
TL;DR: CloseUpAvatar提出了一种新颖的关节人体化身表示方法，通过双纹理系统（低频和高频细节）实现更广泛相机运动下的高质量渲染，特别是近距离视图的保真度。
...
Method: 将化身表示为带纹理的平面集合，包含两组可学习的纹理：低频和高频细节。系统根据相机与化身表面的距离自动切换纹理，近距离时使用高频纹理，远距离时逐渐减少其影响。

Result: 在ActorsHQ高分辨率数据集上的实验表明，CloseUpAvatar在从新颖宽范围相机位置渲染时，在定性和定量上都优于现有方法，同时通过限制所需基元数量保持高FPS。

Conclusion: CloseUpAvatar通过距离自适应的双纹理系统，实现了在更广泛相机运动范围内的高质量化身渲染，平衡了渲染质量和计算效率。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于移位窗口自注意力的Vision Transformers在医学图像分割中存在局限性：局部注意力机制难以有效融合局部细节与全局上下文，这对于微肿瘤和微小器官分割等挑战性任务尤为不利，因为这些任务既需要精细的边界定义，又需要广泛的上下文理解。

Method: 提出HBFormer（Hybrid-Bridge Transformer）架构，包含两个核心部分：1）'Hybrid'设计：将经典的U型编码器-解码器框架与强大的Swin Transformer骨干网络结合，实现鲁棒的层次特征提取；2）'Bridge'机制：通过新颖的多尺度特征融合（MFF）解码器实现多尺度特征集成，该解码器采用非对称设计，通过通道和空间注意力模块（由一系列扩张卷积和深度可分离卷积构建）融合编码器的多尺度特征与全局上下文信息。

Result: 在多个具有挑战性的医学图像分割数据集上进行了全面实验，包括多器官、肝肿瘤和膀胱肿瘤基准测试。HBFormer在这些数据集上取得了最先进的结果，展示了其在微肿瘤和微小器官分割方面的卓越能力。

Conclusion: HBFormer通过创新的混合桥接架构有效解决了医学图像分割中局部与全局特征融合的难题，特别是在微肿瘤和微小器官分割等精细任务上表现出色，为临床诊断提供了更精确的工具。
...
<details>
  <summary>Details</summary>
Motivation: 部分牙齿点云常因遮挡和扫描视角限制而存在大范围缺失区域，这会偏置编码器的全局特征并迫使解码器产生幻觉结构，影响补全准确性。

Method: 提出检索增强框架，将原型记忆库集成到标准编码器-解码器流程中。编码部分输入后，从可学习记忆库中检索最近邻流形原型，通过置信度门控加权与查询特征融合，然后解码。记忆库端到端优化，无需牙齿位置标签即可自组织为可重用牙齿形状原型。

Result: 在自处理的Teeth3DS基准测试中，Chamfer距离指标持续改善，可视化显示更锐利的牙尖、牙嵴和邻接过渡区域，证明方法能提供更准确和忠实的牙齿点云补全。

Conclusion: 该方法提供了一种简单而有效的方式来利用跨样本规律性，为牙齿点云补全提供结构先验，稳定缺失区域推理并释放解码器容量用于细节恢复，兼容常见补全主干网络且保持相同训练损失。
...
TL;DR: Motion4D：将2D基础模型先验整合到4D高斯泼溅表示中，解决动态场景分析中的3D不一致问题，实现时空一致性和语义连贯性。
...
Method: 提出两阶段迭代优化框架：1) 顺序优化，分阶段更新运动和语义场保持局部一致性；2) 全局优化，联合优化所有属性实现长期一致性。引入3D置信度图动态调整运动先验，自适应重采样基于RGB和语义误差在欠表示区域插入新高斯点，通过迭代精炼解决语义不一致问题。

Result: 在点跟踪、视频对象分割和新视角合成等多种场景理解任务中，Motion4D显著优于2D基础模型和现有3D方法。

Conclusion: Motion4D成功将2D基础模型先验整合到统一的4D高斯泼溅表示中，有效解决了动态场景分析中的3D一致性问题，为复杂3D环境理解提供了新框架。
...
Method: LAMP利用大语言模型作为运动规划器，定义受电影摄影惯例启发的运动领域特定语言，通过LLM的程序合成能力从自然语言生成结构化运动程序，然后确定性地映射到3D轨迹。

Result: 实验表明LAMP在运动可控性和用户意图对齐方面优于现有方法，建立了首个直接从自然语言规范生成物体和相机运动的框架。

Conclusion: LAMP通过LLM作为运动规划器，实现了从自然语言到3D轨迹的有效转换，为视频生成中的运动控制提供了新的解决方案。
...
TL;DR: ReCamDriving是一个纯视觉的、相机控制的新轨迹视频生成框架，利用密集且场景完整的3DGS渲染进行显式几何引导，实现精确的相机可控生成。


<details>
  <summary>Details</summary>
Motivation: 现有方法存在局限性：基于修复的方法无法恢复复杂伪影，基于LiDAR的方法依赖稀疏且不完整的线索。需要一种能够实现精确相机控制和结构一致性的视频生成方法。

Method: 采用两阶段训练范式：第一阶段使用相机姿态进行粗略控制，第二阶段结合3DGS渲染进行细粒度视角和几何引导。提出基于3DGS的跨轨迹数据整理策略，消除相机变换模式的训练-测试差距。

Result: 构建了ParaDrive数据集，包含超过110K并行轨迹视频对。实验表明ReCamDriving在相机可控性和结构一致性方面达到最先进水平。

Conclusion: ReCamDriving通过利用3DGS渲染的显式几何引导和创新的训练策略，成功实现了精确的相机可控视频生成，解决了现有方法的局限性。
...
Method: 提出FeatureLens框架，包含图像特征提取器（IFE）和浅层分类器（如SVM、MLP或XGBoost），模型参数仅1,000到30,000个。通过分析图像特征中的异常来检测对抗样本。

Result: 在闭集评估中检测准确率达到97.8%到99.75%，在泛化评估中达到86.17%到99.6%。对FGSM、PGD、CW和DAmageNet攻击均有效，仅使用51维特征。
...
TL;DR: MSVP是一种用于小图像数据集的多尺度视觉提示方法，通过全局、中尺度和局部提示图与输入图像融合，显著提升CNN和ViT性能，参数量增加不到0.02%。


<details>
  <summary>Details</summary>
Motivation: 现有视觉提示方法主要针对大型Vision Transformers和高分辨率数据集（如ImageNet），而MNIST、Fashion-MNIST和CIFAR-10等小图像基准在教育和研究中广泛使用，但在提示方面研究不足。

Method: 提出Multi-Scale Visual Prompting (MSVP)，学习全局、中尺度和局部提示图，通过轻量级1×1卷积与输入图像融合。该方法与骨干网络无关，参数量增加极少。

Result: 在MNIST、Fashion-MNIST和CIFAR-10上使用简单CNN、ResNet-18和小型ViT进行统一基准测试，MSVP带来一致性能提升，计算开销可忽略。消融实验验证了多尺度提示的有效性。
...
TL;DR: Colon-X是一个结肠镜多模态智能开源项目，包含ColonVQA数据集（110万+视觉问答条目）和ColonReason推理数据集，并开发了ColonR1推理模型，在数据稀缺条件下性能超越监督微调25.22%。


<details>
  <summary>Details</summary>
Motivation: 推动结肠镜多模态智能发展，解决当前多模态大语言模型在临床输出方面不够稳健可信的问题，实现从多模态理解到临床推理的关键转变。

Method: 1) 构建ColonVQA多模态数据集（76个临床发现，18个多模态任务）；2) 评估22个多模态大语言模型的泛化性和鲁棒性；3) 通过多专家辩论流程标注ColonReason推理数据集；4) 开发ColonR1模型，采用任务自适应奖励和梯度稳定优化技术。

Result: 1) 现有MLLMs的临床输出远不够稳健可信；2) ColonR1在数据稀缺条件下达到56.61%总体准确率，比监督微调高出25.22%，为多模态结肠镜分析设立了新的推理基准。

Conclusion: Colon-X项目为结肠镜多模态智能提供了全面的数据基础和推理模型，成功实现了从多模态理解到临床推理的转变，所有数据和模型资源已开源。
...
TL;DR: ConvRot：一种基于分组旋转的量化方法，使用正则Hadamard变换抑制扩散变压器中的行列异常值，实现W4A4推理，无需重新训练即可保持图像质量。


<details>
  <summary>Details</summary>
Motivation: 扩散变压器在生成高质量图像方面表现出色，但随着模型规模增大，内存占用和推理延迟显著增加，给实际部署带来挑战。现有基于旋转的量化方法存在较大开销，且难以处理扩散变压器中的行向异常值。

Method: 提出ConvRot方法：1) 使用分组旋转和正则Hadamard变换(RHT)抑制行列异常值；2) 将复杂度从二次降低到线性；3) 设计ConvLinear4bit模块，集成旋转、量化、GEMM和反量化，实现即插即用的W4A4推理。

Result: 在FLUX.1-dev上的实验显示：2.26倍加速、4.05倍内存减少，同时保持图像保真度。这是首次将基于旋转的量化应用于扩散变压器的即插即用W4A4推理。

Conclusion: ConvRot有效解决了扩散变压器部署中的内存和延迟问题，通过创新的旋转量化方法实现了高效且保真的W4A4推理，为实际应用提供了可行的解决方案。
...
TL;DR: GaussianBlender：首个前馈式文本驱动3D风格化框架，无需逐资产优化，实现即时、高保真、多视角一致的3D编辑
...
TL;DR: 本文探讨主动视觉感知的机遇与挑战，提供该领域的全面概述，包括其潜力、当前研究和未来发展方向。


<details>
  <summary>Details</summary>
Motivation: 主动视觉感知系统能够通过感知和行动与环境动态交互，根据特定目标或不确定性调整行为，相比被动系统具有显著优势。然而，该领域面临实时处理复杂视觉数据、动态环境决策和多模态感知融合等挑战，需要系统性的研究。

Method: 本文采用综述研究方法，对主动视觉感知领域进行全面分析，包括系统性地梳理该领域的关键概念、技术方法、应用场景以及面临的挑战。

Result: 论文提供了主动视觉感知的全面概述，明确了该技术在机器人、自动驾驶、人机交互和监控系统等领域的应用潜力，同时识别了当前面临的主要技术挑战和研究空白。

Conclusion: 主动视觉感知具有广阔的应用前景，但要实现更广泛的应用，需要解决实时处理、动态决策和多模态融合等关键技术挑战，未来研究应关注这些方向以推动该领域的发展。
...
Method: SUSS通过结构化多元正态分布建模图像的感知组件，在生成式自监督方式下训练，使模型为人类不可察觉的增强分配高似然度。最终评分是组件对数概率的加权和，权重从人类感知数据集中学习。与基于特征的方法不同，SUSS学习像素空间中残差的图像特定线性变换，通过去相关残差和采样实现透明检查。

Result: SUSS与人类感知判断高度一致，在不同失真类型上表现出强大的感知校准能力，提供局部化、可解释的相似度评估解释。作为感知损失函数在下游成像任务中表现出稳定的优化行为和竞争性性能。

Conclusion: SUSS提供了一种既与人类感知对齐又具有可解释性的感知相似度评分方法，通过结构化概率建模和生成式自监督学习，在保持透明度的同时实现了优秀的感知对齐性能。
...
<details>
  <summary>Details</summary>
Motivation: 解决从非结构化互联网图像进行大规模3D重建时的图像匹配挑战，特别是在大规模图像集合中有效匹配图像的需求

Method: 结合数据集自适应图像配对策略与旋转感知关键点提取匹配：1) 使用DINO检索语义相关图像对；2) 通过旋转增强使用ALIKED和Light Glue提取方向依赖的局部特征
...
<details>
  <summary>Details</summary>
Motivation: 当前视觉-语言-动作（VLA）模型在具身任务中表现良好，但存在冗余和不稳定的动作生成问题，限制了在时间敏感场景中的应用。作者认为这些问题源于现有VLA模型的空间均匀感知场，导致模型容易被目标无关物体分散注意力。
...
Conclusion: PosA-VLA框架通过姿态条件监督锚定视觉注意力，有效解决了VLA模型在复杂环境中产生冗余动作的问题，提高了动作生成的精度和效率，具有实际应用价值。
...
TL;DR: 提出DMDL框架，通过模型级因果干预和优化级协同训练，解决无监督跨模态行人重识别中的模态偏差问题。


<details>
  <summary>Details</summary>
Motivation: 现有两阶段学习框架在单模态训练中学习的模态特定线索会传播到跨模态学习阶段，导致模态偏差，损害身份判别和泛化能力。

Method: 提出双级模态去偏学习框架：1) 模型级：因果启发调整干预模块，用因果建模替代基于似然的建模；2) 优化级：协同无偏训练策略，通过模态特定增强、标签精炼和特征对齐中断模态偏差传播。

Result: 在基准数据集上的广泛实验表明，DMDL能够实现模态不变特征学习，获得更具泛化能力的模型。

Conclusion: DMDL框架通过模型级和优化级的双重去偏设计，有效解决了无监督跨模态行人重识别中的模态偏差问题，提升了模型的泛化性能。
...
Result: 实验表明SelfDebias能够跨提示词和扩散模型架构（包括条件和非条件模型）泛化。它不仅能有效消除关键人口统计学维度上的偏见，同时保持生成图像的视觉保真度，还能处理更抽象概念的偏见识别挑战。

Conclusion: SelfDebias提供了一种完全无监督的测试时去偏方法，适用于各种扩散模型，能够自动识别和减少偏见，同时保持生成质量，为扩散模型的公平性提供了实用解决方案。
...
<details>
  <summary>Details</summary>
Motivation: VAR模型在并行采样同一尺度的多个token时可能导致结构错误，影响生成图像质量，需要一种高效的推理时改进方法

Method: 提出潜在尺度拒绝采样(LSRS)，使用轻量级评分模型评估每个尺度采样的多个候选token图，选择高质量图指导后续尺度生成，优先处理对结构一致性关键的前期尺度

Result: LSRS显著提升VAR生成质量，计算开销极小：VAR-d30模型推理时间仅增加1%，FID从1.95降至1.78；增加15%推理时间可进一步降至1.66

Conclusion: LSRS为VAR模型提供了高效的测试时扩展解决方案，有效缓解自回归错误累积，在保持计算效率的同时提升生成质量
...
<details>
  <summary>Details</summary>
Motivation: 埃及象形文字完全由图画组成，翻译面临单个符号多义性等挑战。深度学习翻译应用快速发展，本研究旨在利用深度学习技术实现从图像到英语的自动识别和翻译。
...
Conclusion: 提出的方法在古埃及象形文字自动识别和翻译方面取得了有前景的结果，展示了深度学习在复杂象形文字翻译任务中的有效性。
...
TL;DR: 提出一种基于视频的鲁棒呼吸率测量方法，仅需普通摄像头，在受试者运动干扰下仍能保持高精度，平均绝对误差仅0.57次/分钟。
...
Method: 结合多种数学变换，使用最小硬件要求（普通摄像头），开发出对受试者运动引起的失真具有更强抵抗力的呼吸率测量算法。

Result: 在14名志愿者超过2.5小时的视频测试中，与参考数据相比平均绝对误差为0.57次呼吸/分钟，相对误差小于5%，明显优于先前工作。

Conclusion: 该方法能够远程测量受试者呼吸率，对受试者行为限制极小，为实际应用提供了更可靠的视频呼吸率监测方案。
...
TL;DR: 提出LUnet：一种紧凑的扁平化UNet架构，通道数不随分辨率减半而加倍，参数量减少30倍以上，性能与传统UNet相当
...
Method: 提出LUnet架构，采用紧凑的扁平化层次结构，通道数在分辨率减半时保持不变（不翻倍）。研究发现剪枝的关键是最终结构而非通道选择策略，因此设计固定架构而非数据自适应剪枝

Result: LUnet参数量减少30倍以上，在公共MRI数据集和两个内部CT数据集上，性能与传统UNet相当，且优于同等参数量下的标准UNet。跳跃连接允许瓶颈通道大幅减少

Conclusion: UNet剪枝的关键是最终结构而非通道选择策略。提出的LUnet架构通过固定扁平化设计，在显著减少参数量的同时保持性能，为医学图像分割提供了高效解决方案
...
TL;DR: 提出HP-Net用于视频动作识别，通过热图池化网络提取丰富、鲁棒且简洁的人体池化特征，结合多模态数据提升识别性能。
...
Conclusion: HP-Net通过提取信息丰富、鲁棒且简洁的池化特征，并结合多模态学习，实现了更鲁棒的视频动作识别。
...
TL;DR: CoDA提出了一种无需目标数据集预训练扩散模型的数据集蒸馏框架，通过识别目标数据集的核心分布并引导生成过程与其对齐，在ImageNet-1K等基准上达到SOTA性能
...
Result: 在不依赖目标数据集特定训练生成模型的情况下，CoDA在所有基准测试（包括ImageNet-1K及其子集）上达到或超越依赖此类模型的先前方法性能，在ImageNet-1K的50-images-per-class设置下创下60.4%的最新SOTA准确率

Conclusion: CoDA通过核心分布对齐有效解决了数据集蒸馏中的关键挑战，实现了无需目标数据集预训练的高效蒸馏，为数据集蒸馏提供了更实用和可扩展的解决方案
...
<details>
  <summary>Details</summary>
Motivation: 现有交通图像修复方法主要关注空间域建模，忽视了频率域先验。虽然Mamba架构擅长长距离依赖建模，但其在频率域特征提取方面的潜力尚未被探索。

Method: 提出FAMamba框架：1) 双分支特征提取块(DFEB)，通过双向2D频率自适应扫描增强局部-全局交互；2) 先验引导块(PGB)，通过基于小波的高频残差学习细化纹理细节；3) 自适应频率扫描机制(AFSM)，使Mamba能在不同子图上进行频率域扫描。

Result: 大量实验证明了FAMamba的效率和有效性。

Conclusion: FAMamba通过整合频率引导与序列建模，为恶劣天气条件下的交通图像修复提供了高效解决方案，充分利用了子图结构的纹理分布特性。
...
TL;DR: 发布来自伊拉克的339张前列腺活检WSI数据集，填补中东地区病理AI数据空白，支持模型跨人群验证


<details>
  <summary>Details</summary>
Motivation: 当前公开病理数据集主要来自西方人群，缺乏中东等地区数据，导致AI模型在这些人群中的泛化能力未知。需要多样化的数据集来开发和验证病理AI模型。

Method: 收集伊拉克埃尔比勒185名患者的前列腺穿刺活检样本，生成339张全切片图像。由三位病理学家独立标注Gleason评分和ISUP分级。使用三种扫描仪（Leica、Hamamatsu、Grundium）扫描，数据以原始格式提供。

Result: 创建了首个来自中东地区的前列腺病理公开数据集，包含多扫描仪图像和专家标注，支持分级一致性分析、颜色归一化和跨扫描仪鲁棒性评估。

Conclusion: 该数据集填补了中东地区病理数据的空白，有助于开发更具全球代表性的AI模型，促进病理AI在不同人群中的公平性和有效性验证。
...
TL;DR: 小规模视觉Transformer（5M参数）在预训练、中间微调和下游任务中的表现研究，发现预训练有帮助但收益递减，中间微调可能因任务机制不匹配而损害性能


<details>
  <summary>Details</summary>
Motivation: 虽然Transformer架构在计算机视觉和NLP领域表现出色，但通常需要大量参数和训练数据。本研究旨在探索小规模视觉Transformer（仅5M参数）在不同预训练、中间微调和下游数据集及训练目标下的边际效益

Method: 使用三个不同的数据集进行实验：预训练数据集、中间微调数据集和下游数据集，并采用不同的训练目标。研究评估这些阶段对小规模5M参数视觉Transformer性能的影响

Result: 预训练和微调对模型有帮助但收益递减；中间微调可能对下游性能产生负面影响，特别是当任务机制不相似时；无差别的中间任务堆叠会浪费计算资源甚至降低性能

Conclusion: 小规模视觉Transformer最受益于有针对性的预训练和谨慎的数据选择，而盲目堆叠中间任务不仅浪费计算资源，还可能损害最终性能
...
TL;DR: CaravelMetrics是一个用于脑血管自动分析的框架，通过骨架化图表示建模血管形态，计算15个形态、拓扑、分形和几何特征，支持全局和区域分析，应用于570个MRI扫描显示年龄、性别和教育相关的血管变化。


<details>
  <summary>Details</summary>
Motivation: 需要一种可扩展、全自动的定量脑血管特征提取方法，以支持血管健康和衰老的规范建模和群体研究。传统方法可能依赖手动分析，缺乏标准化和可重复性。

Method: 通过骨架化生成血管图表示，整合基于图谱的区域分割、中心线提取和图构建，计算15个形态测量、拓扑、分形和几何特征，支持全局和动脉区域的多尺度分析。

Result: 应用于IXI数据集的570个3D TOF-MRA扫描（年龄20-86岁），产生可重复的血管图，捕捉到年龄和性别相关的变化，以及教育相关的血管复杂性增加，与文献报道一致。

Conclusion: CaravelMetrics提供了一个可扩展、全自动的定量脑血管特征提取框架，支持规范建模和群体水平的血管健康和衰老研究，能够检测与年龄、性别和教育相关的脑血管组织变化。
...
TL;DR: 开发了基于Siamese Swin Transformer的双交叉注意力模型（SSDCA），用于结合直肠癌患者治疗前后的纵向内镜图像，准确区分临床完全缓解和局部复发，以支持"观察等待"策略。


<details>
  <summary>Details</summary>
Motivation: 直肠癌患者在总新辅助治疗后达到临床完全缓解时，采用"观察等待"策略日益受到认可。然而，需要客观准确的方法从随访内镜图像中早期检测局部复发，以管理护理并预防远处转移。

Method: 提出SSDCA模型：结合治疗前（restaging）和随访（follow-up）的纵向内镜图像对，使用预训练的Swin Transformer提取领域无关特征，通过双交叉注意力机制强调两个时间点的特征差异，无需图像空间对齐即可预测治疗反应。

Result: 在135名患者训练集和62名患者测试集上，SSDCA取得最佳性能：平衡准确率81.76%±0.04，敏感性90.07%±0.08，特异性72.86%±0.05。模型对血液、粪便、毛细血管扩张和图像质量差等伪影具有鲁棒性。特征聚类显示最大类间分离和最小类内分散。

Conclusion: SSDCA能够准确区分临床完全缓解和局部复发，为直肠癌"观察等待"策略提供了可靠的自动化监测工具，有望改善患者管理和预后。
...
Conclusion: 通过整合帧内和帧间对应关系构建的时空约束，FRESCO显著提升了零样本视频编辑的时空一致性，为视频应用提供了更有效的解决方案
...
TL;DR: UniMo：首个统一建模2D人体视频和3D人体运动的自回归模型，实现两种模态的同时生成与理解


<details>
  <summary>Details</summary>
Motivation: 现有方法通常只能以另一种模态为条件生成单一模态，或将其中一种与其他模态（如文本、音频）整合。由于2D视频和3D运动在结构和分布上存在显著差异，统一这两种模态进行同时优化和生成仍是一个未充分探索的挑战性课题。

Method: 1) 受LLM统一不同模态能力的启发，将视频和3D运动建模为统一的token序列，使用独立的嵌入层缓解分布差异；2) 设计序列建模策略，在单一框架内整合两种不同任务；3) 设计新颖的3D运动tokenizer，采用时间扩展策略，使用单一VQ-VAE生成量化运动token，配备多个专家解码器分别处理身体形状、平移、全局方向和身体姿态。

Result: 大量实验证明，该方法能同时生成对应的视频和运动，并执行精确的运动捕捉。展示了统一建模的有效性。

Conclusion: 这项工作探索了LLM融合多样化数据类型的能力，为将人体中心信息整合到现有模型中铺平了道路，并可能实现人体、物体和场景的多模态可控联合建模。
...
TL;DR: 提出一个通过频率域混合增强现有ground truth图像质量的框架，为真实世界图像修复提供更高质量的监督信号
...
Method: 使用条件频率掩码生成器学习自适应频率掩码，指导原始ground truth与其超分辨率变体的频率分量最优融合，生成感知增强的ground truth图像，然后训练轻量级输出精炼网络

Result: 实验表明该方法能持续提升修复图像质量，用户研究验证了监督增强和输出精炼的有效性

Conclusion: 提出的框架通过增强ground truth图像质量，为真实世界图像修复提供了更有效的监督，且能与现有修复模型无缝集成
...
TL;DR: MUT3R：一种无需训练的框架，利用预训练Transformer中隐含的运动线索抑制动态内容，提升动态场景流式3D重建的稳定性


<details>
  <summary>Details</summary>
Motivation: 现有基于状态的循环神经网络在静态3D重建上表现优异，但在动态场景中容易受到运动伪影的影响，动态区域会破坏空间记忆和图像特征之间的注意力传播

Method: 通过分析预训练Transformer内部行为，发现跨层聚合自注意力图可以揭示动态区域被自然降权的模式。基于此设计注意力级门控模块，在推理早期抑制动态内容影响，无需重新训练或微调模型

Result: 该方法在多个动态基准测试中提升了时间一致性和相机姿态鲁棒性，稳定了几何推理过程

Conclusion: MUT3R提供了一种简单且无需训练的途径，使预训练Transformer能够自我诊断运动线索并自我修正，实现运动感知的流式重建
...
TL;DR: TempR1：一个基于多任务强化学习的时序感知框架，通过系统化增强多模态大语言模型的时序理解能力，在多个基准测试中达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 增强多模态大语言模型（MLLMs）的时序理解能力对于推进长视频分析至关重要，但现有强化学习方法局限于有限任务类型和数据，难以泛化到多样化的时序理解场景。

Method: 提出TempR1框架：1）构建多任务语料库，让模型接触多样化的时序结构和语义；2）基于GRPO算法实现稳定有效的跨任务优化；3）将时序任务分为三种预测区间与真实实例对应类型，为每种类型设计定制化的定位奖励。

Result: TempR1在多个基准测试中达到最先进性能。联合优化互补任务产生强大的协同效应，既增强了泛化能力，也提升了单任务性能。
...
TL;DR: UniID是一个无需调优的人脸个性化框架，通过统一文本嵌入和适配器两种范式，在保持高身份保真度的同时实现灵活文本控制。


<details>
  <summary>Details</summary>
Motivation: 现有无需调优的人脸个性化方法分为文本嵌入和适配器两种范式，但都难以同时实现高身份保真度和灵活文本控制。需要一种统一框架来解决这一挑战。

Method: 提出UniID统一框架，包含：1）身份聚焦学习方案，引导两个分支专门捕获身份特征；2）归一化重缩放机制，恢复基础扩散模型的文本控制能力，同时让互补身份信号相互增强。

Result: 在六个最先进方法的对比实验中，UniID在身份保持和文本控制方面均表现出优越性能。

Conclusion: UniID通过统一两种范式并采用原则性训练-推理策略，实现了高保真人脸个性化与灵活文本控制的平衡，为无需调优的人脸个性化提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于拖拽的图像编辑方法严重依赖手动提供的掩码和文本提示来保持语义保真度和运动精度。去除这些约束会导致视觉伪影（无掩码时）和空间控制不佳（无提示时），需要解决这一基本权衡问题。

Method: 提出DirectDrag框架，包含两个关键创新：1）自动软掩码生成模块，从点位移智能推断可编辑区域，自动定位沿运动路径的变形；2）读出引导特征对齐机制，利用中间扩散激活来保持基于点编辑时的结构一致性。

Result: 尽管无需手动掩码或提示，DirectDrag在DragBench和实际场景中实现了优于现有方法的图像质量，同时保持了具有竞争力的拖拽精度，证明了其高效性和实用性。

Conclusion: DirectDrag通过创新的自动软掩码生成和特征对齐机制，实现了无需掩码和提示的高质量交互式图像编辑，在保持高保真度的同时提供精确的空间控制，为直观的图像操作提供了实用解决方案。
...
TL;DR: 研究发现现有前馈3D重建模型VGGT能天然区分干扰图像，无需额外训练即可实现异常视图过滤


<details>
  <summary>Details</summary>
Motivation: 野外图像集合中的"噪声"图像（与其他图像重叠度低或无关的输入）会降低前馈3D重建模型的性能，而传统SfM管道有几何验证和异常值拒绝机制，前馈模型缺乏这些显式机制

Method: 通过分析不同比例合成干扰物下的VGGT模型，发现特定层天然表现出异常抑制行为，利用该层编码的判别性内部表示实现噪声过滤，无需额外微调或监督

Result: 在受控和野外数据集上的大量实验表明，这种隐式过滤机制具有一致性，并能很好地泛化到不同场景

Conclusion: 前馈3D重建模型即使没有显式异常值拒绝机制或噪声感知训练，也能天然区分干扰图像，这一发现为改进野外条件下的3D重建提供了新思路
...
<details>
  <summary>Details</summary>
Motivation: 现有方法要么在高维数据空间应用群理论先验，要么需要手动划分潜在变量为等变和不变子空间，限制了在表示空间中鲁棒学习和操作群作用的能力。
...
Conclusion: 提出了一种新颖框架，首次实现了在潜在图像流形上学习群作用，自动发现变换相关结构，为群作用学习提供了更灵活和自动化的解决方案。
...
TL;DR: NVRC-Lite：轻量级神经视频压缩框架，通过多尺度特征网格和八叉树上下文模型，在保持低复杂度的同时提升性能并加速编解码


<details>
  <summary>Details</summary>
Motivation: 现有基于隐式神经表示（INR）的视频压缩方法虽然性能优异，但存在两个主要问题：1）轻量级INR在低复杂度下性能有限；2）自回归熵编码模型虽然有效但速度过慢，不实用

Method: 1）将多尺度特征网格集成到轻量级神经表示中，使用更高分辨率网格提升低复杂度INR性能；2）提出八叉树上下文模型用于高维特征网格的熵编码，替代缓慢的自回归模型

Result: NVRC-Lite在PSNR和MS-SSIM指标上分别比最佳轻量级INR视频编码器C3节省21.03%和23.06%的BD-rate，同时实现8.4倍编码加速和2.5倍解码加速

Conclusion: NVRC-Lite成功扩展了NVRC框架到轻量级表示，通过多尺度特征网格和八叉树上下文模型，在保持低计算复杂度的同时显著提升了性能并大幅加速了编解码过程
...
TL;DR: C3G：一种前馈框架，通过在关键空间位置生成紧凑的3D高斯分布，减少冗余并提升特征提取效果，用于无位姿稀疏视图的3D场景重建与理解。


<details>
  <summary>Details</summary>
Motivation: 现有方法使用逐像素3D高斯泼溅进行重建，然后通过2D到3D特征提升进行场景理解，但会产生过多冗余高斯分布，导致内存开销大、多视图特征聚合效果差，从而影响新视角合成和场景理解性能。

Method: 提出C3G框架，仅在关键空间位置估计紧凑的3D高斯分布，减少冗余。引入可学习token通过自注意力聚合多视图特征来指导高斯生成，确保每个高斯分布整合了跨视图的相关视觉特征。然后利用学习到的注意力模式进行高斯解码以高效提升特征。

Result: 在无位姿新视角合成、3D开放词汇分割和视角不变特征聚合等任务上的实验表明，该方法在内存效率和特征保真度方面优于现有方法，证明了紧凑但几何有意义的表示足以实现高质量场景重建和理解。

Conclusion: 紧凑的3D高斯表示不仅能够实现高质量的场景重建，还能有效支持场景理解任务，在减少内存开销的同时保持特征保真度，为3D计算机视觉中的无位姿稀疏视图处理提供了有效解决方案。
...
TL;DR: RELIC是一个统一的实时交互世界模型框架，通过压缩的历史潜在token表示长期记忆，支持16FPS实时生成，实现准确动作跟随、稳定长期流式生成和鲁棒的空间记忆检索。


<details>
  <summary>Details</summary>
Motivation: 构建真正的交互世界模型需要三个关键要素：实时长期流式生成、一致的空间记忆和精确的用户控制。现有方法通常只解决其中一个方面，同时实现三者极具挑战性，例如长期记忆机制往往会降低实时性能。

Method: 基于自回归视频扩散蒸馏技术，使用压缩的历史潜在token（包含相对动作和绝对相机位姿）在KV缓存中表示长期记忆。通过微调双向教师视频模型生成超出原始5秒训练时长的序列，采用新的内存高效自强制范式将其转换为因果学生生成器，支持长期教师和学生自展开的完整上下文蒸馏。

Result: RELIC作为140亿参数模型，在精心策划的Unreal Engine渲染数据集上训练，实现16FPS实时生成，相比先前工作表现出更准确的动作跟随、更稳定的长期流式生成和更鲁棒的空间记忆检索能力。

Conclusion: RELIC的能力为下一代交互世界建模奠定了坚实基础，统一解决了实时长期流式生成、空间记忆和用户控制这三个关键挑战。
...
TL;DR: 提出DIRL双阶段训练框架，让VLM通过交互探索学习协调多种视觉工具，提升空间推理精度，在多个基准测试中达到SOTA


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型在空间推理方面缺乏度量精度，虽然可以通过工具增强能力，但传统方法依赖手工提示或固定工具流水线，限制了模型发现最优工具使用模式的能力

Method: 提出Double Interactive Reinforcement Learning (DIRL)框架：1) 教学阶段：结合单工具专家的交互RL演示与前沿模型的多工具轨迹；2) 探索阶段：通过持续RL进一步优化多工具协调

Result: SpaceTools模型在RoboSpatial-Home、BLINK、BOP-ASK等空间理解基准测试中达到SOTA性能，相比普通SFT提升12%，相比RL提升16%，并能可靠地控制7自由度机器人进行真实世界操作

Conclusion: DIRL框架有效解决了VLM多工具协调的挑战，显著提升了空间推理精度，为具身智能应用提供了可靠的技术基础
...
TL;DR: PosterCopilot：一个基于大型多模态模型的图形设计框架，通过渐进式三阶段训练策略提升布局几何精度和审美推理能力，支持图层可控的迭代编辑，实现专业级海报设计。


<details>
  <summary>Details</summary>
Motivation: 现有基于大型多模态模型的图形设计自动化方法存在几何布局不准确、缺乏专业工作流程所需的图层特定迭代编辑能力等问题，无法满足专业设计需求。

Method: 提出渐进式三阶段训练策略：1）扰动监督微调；2）视觉-现实对齐的强化学习；3）审美反馈的强化学习。同时开发完整工作流，将训练好的LMM设计模型与生成模型结合，支持图层可控的迭代编辑。

Result: 实验表明PosterCopilot能够生成几何精确且审美优越的布局，为专业迭代设计提供前所未有的可控性。

Conclusion: PosterCopilot通过创新的训练策略和工作流程，解决了现有LMM在图形设计中的几何精度和编辑可控性问题，实现了专业级的自动化设计能力。
...
TL;DR: 提出SimFlow方法，通过固定VAE编码器输出的方差为常数，简化了归一化流与VAE的联合训练，避免了复杂的噪声添加和去噪步骤，在ImageNet 256×256生成任务上取得了SOTA结果。


<details>
  <summary>Details</summary>
Motivation: 现有归一化流方法存在两个主要问题：1）需要添加随机噪声作为数据增强，引入复杂的噪声添加和去噪流程；2）使用预训练且冻结的VAE编码器，导致重建和生成质量不佳。

Method: 提出SimFlow方法，将VAE编码器输出的方差固定为常数（如0.5），而不是让编码器预测方差。这种方法使编码器能输出更广泛的token分布，解码器学习从增强的token分布中重建干净图像，同时简化了VAE的证据下界，使归一化流与VAE能够稳定地联合训练。
...
Conclusion: 通过简单地将VAE方差固定为常数，可以同时解决现有归一化流方法的两个主要限制，实现更简单、更有效的训练流程，并在图像生成任务上取得优异的性能。
...
TL;DR: 单人生涯学习范式：使用单个人的第一视角视频训练视觉模型，通过多视角自监督学习几何表示，证明不同人生涯训练的模型具有高度对齐的几何理解，且学习到的表示能有效迁移到下游任务。


<details>
  <summary>Details</summary>
Motivation: 探索单人生涯（single-life）学习范式，研究仅使用单个人的第一视角视频数据能否学习到有效的视觉表示，以及不同人生涯训练的模型是否会发展出对齐的几何理解。

Method: 1. 使用单个人的第一视角视频数据训练独立的视觉编码器；2. 利用单人生涯中自然捕获的多视角进行自监督学习；3. 提出基于交叉注意力的新度量方法来量化不同模型内部表示的功能对齐程度；4. 在室内外不同环境下进行实验验证。

Result: 1. 不同人生涯独立训练的模型发展出高度对齐的几何理解；2. 单人生涯模型学习到的几何表示能有效迁移到深度估计等下游任务；3. 仅使用一周30小时的单人生涯数据，性能可与30小时多样化网络数据训练相当。

Conclusion: 世界共享的结构既导致单人生涯训练模型的一致性，又为视觉表示学习提供了强大的信号，单人生涯学习范式是有效的视觉表示学习方法。
...
TL;DR: 该论文提出了一种动态评估大语言模型安全性的方法，通过定义伦理熵概念并训练分类器来监测模型在部署过程中的价值漂移、越狱攻击等动态失效问题。


<details>
  <summary>Details</summary>
Motivation: 传统大语言模型安全评估主要依赖静态基准测试，但关键失效往往是动态的：分布偏移下的价值漂移、越狱攻击、以及部署中对齐性的缓慢退化。需要一种动态监测方法来应对这些实时风险。

Method: 基于"智能第二定律"框架，将伦理熵作为状态变量；定义五类行为分类法；训练分类器从模型转录本中估计伦理熵S(t)；对四个前沿模型的基座版和指令调优版进行压力测试，测量熵动态变化；建立包含S(t)和有效对齐工作率gamma_eff的监测管道。

Result: 基座模型显示持续的熵增长，而调优变体抑制了漂移并将伦理熵降低了约80%；从这些轨迹中估计出有效对齐工作率gamma_eff；监测管道能在熵漂移超过稳定性阈值时发出警报，实现对价值漂移的运行时监督。

Conclusion: 该研究将伦理熵框架操作化，为大语言模型提供了一种动态安全监测方法，能够实时检测价值漂移并发出警报，为部署中的模型对齐提供了有效的运行时监督工具。
...
TL;DR: 该论文研究EaaS水印防御技术，发现现有水印可通过文本重述绕过，并提出新型线性变换水印方法WET来抵御此类攻击。


<details>
  <summary>Details</summary>
Motivation: 随着企业提供嵌入即服务(EaaS)，模型面临模仿攻击风险。现有水印技术存在漏洞，需要更强大的防御机制来保护EaaS提供商的知识产权。

Method: 首先揭示现有EaaS水印可通过文本重述攻击绕过，然后提出WET水印技术，采用线性变换嵌入向量，通过反向变换和相似度比较进行水印验证。

Result: 文本重述攻击能有效绕过当前最先进的EaaS水印；提出的WET方法对重述攻击具有近乎完美的可验证性，并通过消融研究验证了各组件的重要性。

Conclusion: 现有EaaS水印存在重述攻击漏洞，WET线性变换水印技术能有效防御此类攻击，为EaaS知识产权保护提供了更可靠的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型在评估时表现出选择支持性偏见，会系统性地偏向自己选择的选项，这可能影响AI辅助决策的客观性。现有去偏见方法主要针对人口统计和社会偏见，而认知偏见的解决方法尚未充分探索。

Method: 提出推理依赖生成（RDG）框架，自动构建平衡的推理问答对，明确建模选择、证据和理由之间的依赖关系。该方法生成跨领域的大规模数据集，包含上下文依赖数据和依赖解耦数据。
...
TL;DR: 研究探索语言模型在劳动力市场信息分类中的应用，将职位空缺文本链接到ESCO和EQF两大欧洲框架，比较句子链接和实体链接两种方法，并发布开源工具和标注数据集。


<details>
  <summary>Details</summary>
Motivation: 当前劳动力市场信息分类需要更精确的方法来链接职位空缺文本到标准化的欧洲框架（ESCO和EQF），以超越表面技能提取，深入理解职业和资质的表示方式。

Method: 研究比较了句子链接和实体链接两种方法，开发了开源工具，创建了两个专门用于评估职业和资质表示的标注数据集，并探索了生成式大语言模型在此任务中的应用。

Result: 研究结果推动了职位实体提取的技术进步，为数字中介经济中的工作、技能和劳动力市场叙事分析提供了计算基础设施，并公开了代码库。

Conclusion: 该研究通过比较不同方法、发布工具和数据集，为劳动力市场信息分类提供了实用解决方案，有助于更深入地分析数字时代的就业话语和技能需求。
...
TL;DR: InvertiTune是一个结合可控数据生成和监督微调的框架，用于从文本构建知识图谱，通过生成更符合现实场景的训练数据来提升轻量级模型的单次KG构建性能。


<details>
  <summary>Details</summary>
Motivation: 现有基于LLM的Text2KG方法通常依赖迭代提示，计算成本高且容易忽略文本中分布的复杂关系。需要更高效、更准确的单次知识图谱构建方法。

Method: 提出InvertiTune框架：1）可控数据生成管道：从大型知识库提取子图，过滤噪声，利用LLM生成对应的自然文本描述；2）监督微调：使用生成的数据集对轻量级模型进行微调，实现单次KG构建。

Result: 在CE12k数据集上，InvertiTune超越了更大的非微调LLM和现有Text2KG方法，同时在CrossEval-1200测试集上表现出更强的跨数据集泛化能力。

Conclusion: 研究表明，现实、高质量的训练数据对于推进高效、高性能的Text2KG系统至关重要，InvertiTune框架为解决现有方法的局限性提供了有效途径。
...
TL;DR: 提出一个从政治文本中检测和解析解释的框架，使用轻量级因果语言模型提取因果主张，形成结构化数据集用于下游分析。
...
Result: 展示了如何大规模研究因果解释，方法具有适中的标注需求、良好的泛化能力和相对于人工编码的准确性。

Conclusion: 提出的框架能够系统分析政治文本中的解释，为政治科学中的解释研究提供了可扩展的工具。
...
TL;DR: 提出RMFT（随机掩码微调）方法，在保持性能的同时显著减少LLM对训练数据中个人身份信息的记忆，相比基线减少80%以上的信息提取率。


<details>
  <summary>Details</summary>
Motivation: 当前大语言模型容易记忆训练数据中的个人身份信息，带来严重的安全和隐私风险，需要开发既能保护隐私又不过度影响模型性能的微调技术。

Method: 提出随机掩码微调（RMFT）方法，在微调过程中随机掩码部分输入，减少模型对特定信息的记忆。同时提出MaxTER评估框架，用于评估隐私与性能的权衡。

Result: 在Enron邮件数据集上，RMFT相比基线微调减少了80.81%的总提取率和80.17%的已见提取率，性能损失仅为困惑度增加5.73%，优于去重方法。

Conclusion: RMFT是有效的隐私保护微调技术，能在显著降低PII记忆风险的同时保持模型性能，为LLM隐私保护提供了实用解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 传统社会语言学分析依赖人工标注，耗时且难以扩展到大规模语料。本研究旨在利用大语言模型自动恢复可解释的社会语言学模式，推进跨语言和低资源双语研究的计算方法

Method: 使用大语言模型自动标注3,691个语码转换句子的主题、体裁和语用功能，整合迈阿密双语语料库的人口统计元数据，并为西班牙语-瓜拉尼语数据集添加新的主题标注

Result: 分布结果显示：迈阿密数据中性别、语言优势和语用功能存在系统性关联；巴拉圭文本中正式瓜拉尼语和非正式西班牙语存在明显的双言分工。这些发现用语料规模的定量证据复制并扩展了早期的互动和社会语言学观察

Conclusion: 大语言模型能够可靠地恢复传统上只能通过人工标注获得的可解释社会语言学模式，推动了跨语言和低资源双语研究的计算方法
...
TL;DR: PERCS数据集：针对四种不同医学素养人群（普通公众、医预科学生、非医学研究者、医学专家）的个性化生物医学摘要简化数据集，支持可控摘要生成研究
...
Method: 创建PERCS数据集，包含生物医学摘要及其针对四种不同医学素养人群的简化版本，由医生进行事实准确性和人物对齐审核，建立详细错误分类体系，并用四种大语言模型进行基准测试

Result: 技术验证显示不同人群的摘要在可读性、词汇和内容深度上存在明显差异，建立了自动评估指标（全面性、可读性、忠实度）的基准结果

Conclusion: PERCS数据集支持个性化医学沟通和可控生物医学摘要生成研究，为未来研究提供公开的数据集、标注指南和评估材料
...
TL;DR: 本文提出Idea-Gated Transformer架构，通过分离语义规划和语法生成来解决自回归语言模型中的"主题漂移"问题，使用辅助的"Idea Head"预测未来上下文窗口的词袋分布，创建"概念向量"来实时门控主要词汇生成。


<details>
  <summary>Details</summary>
Motivation: 自回归语言模型在下一个词预测训练中经常出现"主题漂移"问题，生成内容会偏离初始提示，这是因为模型过度依赖局部关联而非全局规划。虽然增大模型规模可以缓解此问题，但下一个词预测目标本身存在的短视性仍然存在。

Method: 提出Idea-Gated Transformer架构，包含一个辅助的"Idea Head"来预测未来上下文窗口的词袋分布，生成"概念向量"。通过可微分的门控机制，在生成过程中实时抑制语义无关的词汇，有效剪枝搜索空间。

Result: 在WikiText-103上的实验表明，Idea-Gated模型在验证困惑度上与标准GPT-2基线相当，但在领域保持性方面表现显著更优。定性和定量分析显示，门控机制成功将生成锁定在特定语义簇中，并能抵抗关联漂移。
...
TL;DR: HBLR框架通过置信度感知的符号翻译和假设驱动的后向推理，显著提升了逻辑推理的准确性和效率。
...
Method: 提出假设驱动的后向逻辑推理框架，包含两个核心模块：1) 置信度感知符号翻译，仅将高置信度内容转换为逻辑形式；2) 假设驱动的后向推理，模拟人类演绎思维，从结论出发递归验证前提。

Result: 在五个推理基准测试上的广泛实验表明，HBLR在准确性和效率方面均优于强基线方法。

Conclusion: HBLR框架通过结合符号翻译和后向推理，有效解决了传统前向推理的局限性，为逻辑推理任务提供了更可靠和高效的解决方案。
...
TL;DR: 提出高阶注意力网络(Hon)，通过递归框架增强Transformer的表示能力，打破标准注意力的低秩瓶颈，以O(1)额外参数捕获多跳关系。


<details>
  <summary>Details</summary>
Motivation: 标准Transformer的一阶注意力机制存在低秩瓶颈限制，难以在单层内捕获复杂的多跳关系，需要增强表示能力。

Method: 提出高阶注意力网络(Hon)，通过递归框架动态精炼Query和Key表示：Query和Key向量本身是内部注意力循环的输出，允许token在最终注意力计算前聚合全局上下文并建模高阶相关性。采用参数高效的权重共享策略，确保增强表达能力的同时只增加O(1)参数。

Result: 理论分析证明该方法打破了标准注意力的线性瓶颈。实证结果表明，Hon在多个基准测试中优于标准Transformer。

Conclusion: 高阶注意力网络通过递归框架有效增强了Transformer的表示能力，能够捕获复杂的高阶相关性，同时保持参数效率。
...
<details>
  <summary>Details</summary>
Motivation: 合作视频游戏中的语言数据具有丰富性，包含复杂的协调、沟通和不确定性推理，这些现象在现有闲聊或任务导向对话语料库中很少出现。

Method: 收集Portal 2合作模式中的玩家对话，构建包含11.5小时语音、24.5K话语的Portal Dialogue Corpus，包含视频、音频、转录、游戏状态数据以及手动和自动标注。

Result: 识别出复杂空间指代、澄清修复、临时约定形成等独特语言现象，这些在现有对话语料库中罕见，为复杂情境下的协作问题解决语言分析提供了宝贵资源。

Conclusion: 该语料库为研究复杂、情境化、协作问题解决场景中的语言使用提供了重要资源，公开的多模态数据支持未来更深入的语言分析研究。
...
Method: 提出Dual LoRA方法，将低秩矩阵分为两组：幅度组控制参数是否更新及更新幅度（使用ReLU函数），方向组决定参数更新方向（使用符号函数），从而更好地模拟基于梯度优化的全参数微调过程。

Result: 在广泛的NLP任务（自然语言生成、理解、常识推理）上，使用GPT-2、RoBERTa、DeBERTa和LLaMA-1/2/3作为基线模型，Dual LoRA在相同可训练参数数量下，始终优于LoRA及其最先进的变体。

Conclusion: Dual LoRA通过引入归纳偏置，将参数更新过程分解为幅度和方向两个维度，显著提升了LoRA方法的性能，为参数高效微调提供了更有效的解决方案。
...
TL;DR: PretrainZero：基于预训练语料的强化主动学习框架，通过主动识别语料中的合理信息内容进行强化学习预训练，无需可验证标签或奖励模型，显著提升基础模型的一般推理能力。


<details>
  <summary>Details</summary>
Motivation: 当前基于强化学习的大型思维模型虽然在特定领域展现专家级能力，但严重依赖特定领域的可验证奖励，这限制了通用推理能力的扩展。需要突破可验证数据的瓶颈，实现更通用的推理能力。

Method: 提出PretrainZero框架：1）主动预训练：学习统一推理策略，主动识别预训练语料中的合理信息内容；2）自监督学习：无需可验证标签、预训练奖励模型或监督微调，直接在通用语料上进行强化学习预训练；3）验证扩展：通过处理越来越难的掩码跨度来增强推理能力。

Result: 在强化预训练中，PretrainZero将Qwen3-4B-Base在MMLU-Pro、SuperGPQA和数学平均基准上分别提升了8.43、5.96和10.60分。预训练模型也可作为下游RLVR任务的推理基础模型。

Conclusion: PretrainZero成功将强化学习从特定领域的后训练扩展到通用预训练，突破了可验证数据的瓶颈，显著提升了基础模型的通用推理能力，为实现更通用的人工智能提供了新途径。
...
TL;DR: Top-k注意力机制在解码和训练阶段的有效性研究，验证了在解码阶段仅保留与查询最相似的关键词能达到或超越全注意力性能，并探索了原生Top-k训练策略和近似算法的影响。


<details>
  <summary>Details</summary>
Motivation: LLMs在长上下文建模中越来越普遍，但其推理计算成本已成为阻碍智能体和多模态应用发展的关键瓶颈，需要研究更高效的注意力机制。

Method: 研究Top-k注意力机制在解码和训练阶段的有效性：1）验证精确Top-k解码的性能；2）探索原生Top-k注意力训练策略；3）研究近似Top-k算法精度对下游任务的影响；4）从熵的角度提供理论解释。

Result: 1）Top-k解码在HELMET和LongBench v2等下游任务上达到或超越全注意力性能；2）训练与推理一致的Top-k注意力操作能进一步释放Top-k解码潜力；3）下游任务性能与近似保真度正相关；4）Top-k注意力SFT导致下游任务熵降低，验证低熵状态更适合Top-k解码的假设。

Conclusion: Top-k注意力机制是降低LLMs推理计算成本的有效方法，通过精确Top-k解码、一致的训练策略和适当的近似算法，可以在保持性能的同时显著提高效率，为长上下文建模提供实用解决方案。
...
TL;DR: LLMs的推理能力在摘要生成中并非普遍有效，存在摘要质量与事实忠实度的权衡，过度推理反而可能损害事实一致性。


<details>
  <summary>Details</summary>
Motivation: 虽然LLMs在数学和代码生成等分析任务中表现出色，但其在抽象摘要生成中的有效性被广泛假设但未经验证。本研究旨在填补这一空白，系统评估推理策略在摘要生成中的作用。

Method: 首先将通用推理策略适配到摘要领域，然后对8种推理策略和3个大型推理模型在8个多样化数据集上进行大规模比较研究，评估摘要质量和忠实度。

Result: 推理并非普遍解决方案，其有效性高度依赖于具体策略和上下文。存在摘要质量与事实忠实度的权衡：显式推理策略倾向于提高流畅性但牺牲事实基础，而LRMs中的隐式推理则呈现相反模式。增加LRM的内部推理预算不会改善甚至可能损害事实一致性。
...
TL;DR: 提出INDI-PROP数据集和两个GPT-4o-mini引导的多跳推理框架，用于印度新闻媒体中的意识形态偏见、叙事框架和说服技巧的细粒度分类。


<details>
  <summary>Details</summary>
Motivation: 叙事是宣传的认知和情感支架，将孤立的说服技巧组织成连贯的故事。现有研究缺乏对意识形态基础的细粒度叙事分类，特别是在印度新闻媒体这样的多语言、多文化背景中。

Method: 1. 创建INDI-PROP数据集：包含1,266篇关于CAA和农民抗议的文章，进行三层标注（意识形态偏见、事件特定叙事框架、说服技巧）。2. 提出FANTA框架：通过信息提取和上下文框架进行分层推理。3. 提出TPTC框架：通过两阶段方法系统分解说服线索。

Result: FANTA和TPTC框架在偏见、叙事和说服技巧分类任务上都显著优于基线模型，验证了多跳推理方法的有效性。

Conclusion: 该研究提供了首个意识形态基础的细粒度叙事数据集和有效的多跳推理框架，为分析印度新闻媒体中的宣传机制提供了新工具，展示了分层标注和推理在理解复杂宣传叙事中的价值。
...
TL;DR: 提出一个可解释的事实一致性评估框架，通过分解原子事实和加权度量来解决LLM幻觉问题，特别针对临床等高风险领域


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在自然语言处理任务中取得显著进展，但容易生成错误或误导性但看似合理的论点（幻觉问题）。这在临床等高风险领域尤为严重，事实不准确可能带来严重后果。现有评估指标无法充分评估事实一致性且缺乏可解释性，使得诊断和缓解错误变得困难。

Method: 提出一个可解释的事实一致性评估框架，将文本分解为原子事实，引入灵活的无模式方法。采用加权度量而非绝对度量来增强事实评估，并提出机制来控制复杂领域的评估复杂性。

Result: 在流行的通用和临床数据集上对方法进行了基准测试，并发布代码以支持未来研究中事实感知的模型训练。

Conclusion: 该框架解决了LLM幻觉评估的关键问题，特别是在高风险领域，通过可解释的事实一致性评估为模型训练和改进提供了实用工具。
...
TL;DR: 意大利首次全面调查显示：生成式AI被广泛用于工作和个人用途，包括情感支持和医疗建议等敏感任务，正取代其他技术成为主要信息来源。尽管用户数字素养较低，这一趋势仍在持续，存在错误识别风险。研究发现显著的性别鸿沟，女性采用率仅为男性一半，且使用频率更低。素养是采用的关键预测因素，但只能部分解释性别差异。


<details>
  <summary>Details</summary>
Motivation: 生成式AI聊天机器人等人工智能语言技术正在改变数字互动，虽然具有社会潜力，但由于采用不均衡和对局限性认识不足，可能加剧数字鸿沟。需要了解意大利生成式AI的采用情况、使用模式和素养水平，以应对潜在风险。

Method: 基于新收集的1,906名意大利语成年人的调查数据，进行首次全面的实证映射研究，分析生成式AI的采用、使用模式和数字素养。

Result: 1. 生成式AI被广泛用于工作和个人用途，包括情感支持和医疗建议等敏感任务；2. 生成式AI正在取代其他技术成为主要信息来源，尽管用户数字素养较低；3. 存在显著的性别鸿沟，女性采用率仅为男性一半，使用频率也更低；4. 数字素养是采用的关键预测因素，但只能部分解释性别差异。

Conclusion: 研究提供了生成式AI多用途使用的细致洞察，强调需要有针对性的教育举措，并进一步调查素养无法完全解释的公平参与障碍。需要解决用户识别错误信息的能力不足问题，以及性别鸿沟背后的深层障碍。
...
TL;DR: 本文提出了Hydro-SE Bench基准测试，包含4000道选择题，用于评估大语言模型在水科学与工程领域的知识与应用能力，发现商业LLMs准确率在0.74-0.80之间，小参数模型在0.41-0.68之间，模型在自然物理科学相关子领域表现良好，但在行业标准和水利结构等专业领域存在不足。


<details>
  <summary>Details</summary>
Motivation: 水科学与工程是保障人类供水、清洁水电能源和防洪抗旱的关键领域，需要多学科专家协作决策，但大语言模型在该领域的知识和应用能力尚未得到充分评估。随着LLMs的快速发展，需要系统评估其在Hydro-SE领域的潜力。

Method: 提出Hydro-SE Bench评估基准，包含4000道选择题，涵盖9个子领域，从基础概念知识、工程应用能力、推理计算能力三个维度评估LLMs。评估了商业LLMs和小参数模型的表现。

Result: 商业LLMs准确率在0.74-0.80之间，小参数模型在0.41-0.68之间。模型在自然物理科学相关子领域表现良好，但在行业标准、水利结构等专业领域表现不佳。模型扩展主要提升推理计算能力，但在实际工程应用方面仍有很大改进空间。

Conclusion: 研究揭示了LLMs在Hydro-SE任务中的优势和不足，为模型开发者提供了明确的训练目标，为Hydro-SE研究人员提供了应用LLMs的实用指导。模型在专业领域知识方面仍需改进，特别是在实际工程应用场景中。
...
TL;DR: 研究发现LLMs中不同句法现象（特别是句法一致性）会激活重叠的单元，表明句法一致性构成了LLMs表征空间中有意义的功能类别。


<details>
  <summary>Details</summary>
Motivation: 虽然大型语言模型能够可靠地区分语法正确和不正确的句子，但语法知识如何在模型内部表示仍然是一个开放性问题。研究者希望探究不同的句法现象在LLMs中是共享还是使用不同的组件。

Method: 采用受认知神经科学启发的功能定位方法，在七个开源模型中识别对67种英语句法现象最敏感的单元。通过跨语言分析（英语、俄语、中文）和57种语言的跨语言比较，研究句法一致性的表征模式。

Result: 不同类型的句法一致性（如主谓一致、照应一致、限定词-名词一致）会激活重叠的单元集合，表明一致性构成了LLMs中有意义的功能类别。这一模式在英语、俄语和中文中都成立，且结构更相似的语言在主谓一致性上共享更多单元。

Conclusion: 句法一致性——作为句法依赖关系的关键标记——构成了LLMs表征空间中有意义的类别，揭示了LLMs中句法知识的组织方式。
...
<details>
  <summary>Details</summary>
Motivation: 随着AI导师在教育领域的广泛应用，需要系统评估其教学质量。当前缺乏专门工具来评估AI导师的教学效果，教育利益相关者和研究人员需要工具来分析和改进AI导师的表现。

Method: 开发了一个集成语言技术的应用程序，包含：1) AI导师教学质量评估功能；2) 演示和评估软件；3) 模型检查工具；4) 数据可视化模块。支持用户反馈收集和注释功能。

Result: 成功开发了AITutor-EvalKit工具，为教育利益相关者和ACL社区提供了一个全面的AI导师评估平台，支持教学效果分析、模型透明度和数据驱动的改进。

Conclusion: AITutor-EvalKit是一个有价值的工具，能够帮助评估和改进AI导师的教学质量，促进教育技术发展，并为研究社区提供数据收集和分析能力。
...
TL;DR: DZ-TDPO提出了一种非破坏性对齐框架，通过冲突感知的动态KL约束和可学习的时间注意力偏置，解决长对话系统中的状态惯性问题，在保持零样本泛化能力的同时实现SOTA性能。
...
Result: 在Multi-Session Chat数据集上，DZ-TDPO达到86.2%的胜率（Phi-3.5），Qwen2.5-7B模型实现99.4%的胜率且困惑度开销可忽略；同时保持零样本泛化能力和MMLU通用能力。

Conclusion: 通过精确的注意力调节而非破坏性权重更新，可以有效缓解状态惯性问题，同时保持模型的通用能力，揭示了"容量-稳定性权衡"现象。
...
TL;DR: AR-Med是一个用于医疗搜索自动相关性评估的框架，通过检索增强的LLM和知识蒸馏技术，在在线医疗平台实现高精度、高效率部署。


<details>
  <summary>Details</summary>
Motivation: 在线医疗平台搜索的准确性和可靠性对用户安全和服务效果至关重要。传统方法难以理解复杂细微的用户查询，而LLM虽有潜力但面临事实幻觉、专业知识缺失和运营成本高等挑战。

Method: 1. 采用检索增强方法，将LLM推理建立在已验证的医学知识基础上；2. 设计实用的知识蒸馏方案，将大型教师模型压缩为紧凑而强大的学生模型；3. 引入LocalQSMed多专家标注基准指导模型迭代。

Result: AR-Med离线准确率超过93%，比原在线系统提升24%绝对改进，在线相关性和用户满意度显著提升，已在在线医疗配送平台大规模部署。

Conclusion: 该工作为在真实医疗应用中开发可信赖的LLM驱动系统提供了实用且可扩展的蓝图，成功解决了医疗搜索中的关键挑战。
...
TL;DR: 提出ESPO框架，将序列生成视为单一动作，使用ELBO作为序列级似然代理，解决扩散大语言模型中的RL适配问题


<details>
  <summary>Details</summary>
Motivation: 强化学习在自回归语言模型中效果显著，但难以适配到扩散大语言模型，核心困难在于似然近似问题：自回归模型天然提供token级条件概率，而扩散模型通过迭代非自回归去噪步骤生成序列，缺乏这种因子化
...
Result: 在数学推理、编码和规划任务上的广泛实验表明，ESPO显著优于token级基线方法，在Countdown任务上实现20-40分的显著提升，同时在数学和编码基准上保持一致的增益

Conclusion: ESPO为扩散大语言模型中的强化学习建立了序列级优化的原则性和经验有效的范式
...
TL;DR: Doublespeak是一种针对大语言模型的上下文表示劫持攻击，通过将有害关键词替换为良性标记来绕过安全对齐，使模型将看似无害的提示内部解释为禁止指令。


<details>
  <summary>Details</summary>
Motivation: 当前大语言模型的安全对齐策略存在漏洞，攻击者可能通过操纵模型的内部表示来绕过安全机制。研究旨在揭示这种新的攻击面，证明现有对齐策略在表示层面的不足。

Method: 采用上下文表示劫持攻击方法，系统地将有害关键词（如"bomb"）替换为良性标记（如"carrot"）在多个上下文示例中。通过分析模型内部表示的变化，展示良性标记的语义如何逐层收敛到有害语义。

Result: 攻击在闭源和开源系统上均有效，在Llama-3.3-70B-Instruct上单句上下文覆盖达到74%的攻击成功率。可解释性工具显示语义覆盖逐层出现，早期层的良性含义在后期层收敛为有害语义。

Conclusion: Doublespeak攻击揭示了LLM潜在空间中的新攻击面，表明当前对齐策略不足，需要在表示层面进行操作。这种优化无关、跨模型家族广泛可转移的攻击突显了安全对齐需要更深入的理解和防御机制。
...
<details>
  <summary>Details</summary>
Motivation: 现有对比解码方法（如DoLa）仅在仅解码器架构中实现，且主要研究其对事实性的改进。本研究旨在探索在编码器-解码器架构中应用对比解码，特别是评估其对指令跟随能力的影响。

Method: 将DoLa方法适配到T5和FLAN-T5模型家族，通过层对层分析logit演化来量化DoLa对token输出概率的影响。

Result: DoLa在某些任务类别中提高了文本生成的忠实性，但在其他任务中反而有害。通过层对层分析揭示了DoLa对token输出概率的具体影响模式。

Conclusion: 这是首次在编码器-解码器架构中实现对比解码策略的研究，结果表明DoLa对指令跟随能力的影响具有任务依赖性，需要针对不同任务类型进行选择性应用。
...
TL;DR: 研究提出一个通过提示工程优化LLM在心理学文本分类中性能的框架，发现构造定义和任务框架比思维链、角色扮演等策略更重要，推荐结合人工和自动方法生成多个提示变体并进行实证选择。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在文本分类中表现良好，但其输出严重依赖提示的措辞。心理学等领域中的构造具有精确的理论定义，可能未在预训练数据中充分体现，需要系统化的提示工程方法来优化LLM性能，使其与专家判断对齐。

Method: 提出一个实证框架，评估五种提示策略：基于编码本的实证提示选择、自动提示工程、角色扮演提示、思维链推理和解释性提示，结合零样本和少样本分类。在三个心理学构造和两个模型上进行实验。

Result: 发现角色扮演、思维链和解释性提示并不能完全解决提示措辞不当导致的性能损失。最重要的提示特征是构造定义、任务框架，其次是提供的示例。最佳分类结果来自结合基于编码本的实证提示选择和自动提示工程的少样本提示。

Conclusion: 建议研究人员生成尽可能多的提示变体（人工或自动生成），基于训练数据集中的实证性能选择提示和示例，并在保留集上验证最终方法。这为需要与专家判断对齐的场景提供了实用、系统且理论驱动的方法。
...
TL;DR: 该论文提出通过微调LLMs遵循医学共识指南进行逐步推理，以提升医学早期预测的可解释性和准确性，并以Sepsis-3定义为例验证了方法的有效性。


<details>
  <summary>Details</summary>
Motivation: 当前医学机器学习模型过于关注预测准确性，忽视了医疗从业者需要的可信解释。医学共识指南普遍存在但未被有效利用来指导AI推理过程。

Method: 将医学共识指南转化为可执行的推理规则，用电子病历数据实例化这些规则来微调LLMs，使模型学会遵循共识指南进行逐步推理。同时采用多模态方法整合时间序列预测模型的输出表示。

Result: 微调后的小型模型在推理正确性和预测准确性上显著优于使用明确提示的大型LLMs和仅训练于医学文本的模型。模型在未见患者数据上几乎达到完美的推导正确性，主要瓶颈在于对未来稀疏不规则临床变量的预测。

Conclusion: 通过微调LLMs遵循医学共识指南进行逐步推理，可以显著提升医学预测的可解释性和准确性。多模态整合时间序列预测能改善对未来临床变量的预测性能。
...
<details>
  <summary>Details</summary>
Motivation: Transformer解码器的KV缓存内存消耗在长序列中成为瓶颈，现有的跨层共享方法性能不如GQA等层内方法，需要找到更有效的KV缓存压缩方案

Method: 提出FusedKV：顶层KV缓存是底层和中层最有信息量的KV缓存的可学习融合，直接在RoPE后的keys上操作，避免重新计算旋转嵌入。还提出FusedKV-Lite：顶层KV缓存直接从底层values和中层keys派生，进一步减少I/O开销

Result: 在332M到4B参数的LLM上实验，减少50%缓存内存，同时验证困惑度低于标准Transformer解码器

Conclusion: FusedKV是一种内存高效、高性能的架构替代方案，通过智能融合不同层的KV缓存信息，在显著减少内存的同时保持模型性能
...
TL;DR: 研究显示，结合标准与多样化语料训练的语言模型在保持标准任务性能的同时，能显著提升对非标准语言变体的理解能力。


<details>
  <summary>Details</summary>
Motivation: 当前语言模型依赖经过质量筛选的大规模文本语料，这可能导致非标准语言变体（方言、历史、非正式等）被排除，降低模型鲁棒性并强化表征偏见。研究旨在构建能够捕捉语言多样性全谱的模型。

Method: 针对巴斯克语（形态丰富、资源稀缺），构建结合标准文本、社交媒体和历史资料的新语料库，预训练BERnaT系列编码器模型（标准、多样、组合三种配置）。提出将自然语言理解任务分为标准和多样化子集的评估框架。

Result: 结合标准和多样化数据训练的模型在所有任务类型上表现均优于仅使用标准语料训练的模型，且不损害标准基准的准确性。

Conclusion: 语言多样性对于构建包容性、可泛化的语言模型至关重要，应超越对标准化文本的依赖，全面捕捉语言变体。
...
<details>
  <summary>Details</summary>
Motivation: 尽管大语言模型在偏见检测方面有进步，但宗教等敏感话题仍面临挑战，因为微小错误可能导致严重误解。多语言模型经常误读宗教内容，在宗教语境中准确性不足。

Method: 引入BRAND数据集，聚焦南亚四大宗教（佛教、基督教、印度教、伊斯兰教），包含2400多个条目，使用英语和孟加拉语三种不同类型的提示进行评估。

Result: 模型在英语中表现优于孟加拉语，即使回答宗教中立问题时也持续表现出对伊斯兰教的偏见。不同语言提问相似问题时，多语言模型存在持续性偏见。

Conclusion: 研究揭示了多语言模型在宗教语境中的系统性偏见，强调了在HCI领域处理宗教和灵性问题时需要更细致的考量。
...
TL;DR: 该研究提出两阶段方法（持续预训练+监督微调）将Qwen2.5-3B模型适配到低资源藏语，显著降低了困惑度并大幅提升了翻译质量，同时通过层分析揭示了适配机制。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在低资源语言（如藏语）上的适配面临数据稀缺和跨语言漂移的挑战，需要有效方法来扩展多语言基础模型到资源匮乏的语言环境。

Method: 采用两阶段适配方法：1) 持续预训练（CPT）建立藏语语言基础；2) 监督微调（SFT）进行任务和翻译专业化。使用Qwen2.5-3B模型，并对Qwen3-4B的435层进行层分析。

Result: 困惑度从2.98降至1.54；中→藏翻译质量显著提升（BLEU从0.046到0.261，chrF从2.2到6.6）。层分析显示适配主要集中在嵌入层和输出头，中后期MLP投影编码领域特定转换。

Conclusion: CPT构建藏语语义流形，SFT以最小表征干扰锐化任务对齐。该研究首次定量探索藏语适配动态，为将多语言基础模型扩展到低资源环境提供了开放可复现的框架。
...
TL;DR: 本文提出两种tokenizer适应方法：继续BPE训练用于词汇扩展，以及基于叶子的词汇剪枝，以提高tokenizer在新领域或语言中的效率和效果。


<details>
  <summary>Details</summary>
Motivation: 当前tokenizer适应方法存在两个问题：1）词汇扩展时，在新数据上训练新tokenizer并添加非重叠token，常导致许多token无法访问或从未使用；2）需要词汇剪枝方法来移除冗余token。需要更有效的tokenizer适应工具来提升预训练语言模型在新领域或语言中的迁移效果。

Method: 提出两种互补方法：1）继续BPE训练：通过在新数据上继续BPE合并学习过程来适应预训练tokenizer，而不是训练全新tokenizer；2）基于叶子的词汇剪枝：识别并移除词汇表中的冗余token，同时保持模型质量。

Result: 在多种语言和模型家族上的实验表明：继续BPE训练提高了tokenization效率，并更好地利用了添加的词汇；词汇剪枝方法能有效移除冗余token而不损害模型质量。两种方法共同提供了实用的受控词汇修改工具。

Conclusion: 本文提出的继续BPE训练和基于叶子的词汇剪枝方法为tokenizer适应提供了有效的解决方案，提高了tokenizer在新领域或语言中的效率和效果，相关工具已开源发布。
...
TL;DR: AugServe：针对增强型大语言模型推理服务的高效推理框架，通过两阶段自适应请求调度和动态令牌批处理机制，显著提升有效吞吐量并降低延迟。


<details>
  <summary>Details</summary>
Motivation: 随着增强型大语言模型在Web应用中日益普及，提升推理服务效率、优化服务级别目标对改善用户体验至关重要。现有系统面临两大挑战：1）先到先服务调度导致严重的队头阻塞，使许多请求排队延迟超出SLO；2）静态批次令牌限制无法适应波动的负载和硬件条件，两者都降低了有效吞吐量和服务质量。

Method: AugServe采用两阶段自适应请求调度策略：第一阶段结合增强型LLM请求的推理特征优化调度决策顺序；第二阶段利用运行时信息持续优化决策，适应请求特征和系统能力。此外，系统根据硬件状态和实时负载动态调整令牌批处理机制。
...
Conclusion: Jina-VLM证明了通过精心设计的架构组合，可以在相对较小的参数规模下实现卓越的多语言视觉语言理解能力
...
TL;DR: SkillFactory是一种在强化学习前通过监督微调让模型学习认知技能的方法，通过重新组织模型自身生成的样本来创建训练数据，即使这些"银标"数据不完美，也能帮助模型在后续强化学习中更好地掌握技能。


<details>
  <summary>Details</summary>
Motivation: 当基础语言模型不具备某些认知技能（如验证答案、回溯、尝试替代方法等）时，如何让模型学会利用这些技能？传统方法依赖从更强模型蒸馏，但本文希望探索不依赖蒸馏的方法。

Method: SkillFactory方法：在强化学习前的监督微调阶段，使用模型自身生成的样本，通过重新组织这些样本来创建训练数据，形成"银标"SFT轨迹。这些数据虽然不完美，但能为模型学习认知技能提供基础。

Result: 1) SkillFactory SFT初始化帮助模型在强化学习后更好地泛化到任务的更难变体；2) 模型确实使用了认知技能；3) 经过强化的SkillFactory模型在域外任务上比强化后的基础模型更稳健，不易出现性能退化。

Conclusion: 在强化学习前学习的归纳偏置有助于模型学习稳健的认知技能使用。SkillFactory提供了一种不依赖蒸馏的方法，通过模型自身生成的重新组织数据来培养认知技能。
...
TL;DR: 本文主张将AI对齐重新构想为通过基于过程、多智能体、发展性机制构建具有合成熵和理性响应能力的智能体，而非编码固定的人类价值内容。


<details>
  <summary>Details</summary>
Motivation: 传统基于内容的价值规范方法存在"规范陷阱"问题，由于"是-应该"鸿沟、价值多元性和扩展框架问题的结合，导致结构不稳定。需要新的对齐框架来应对这些挑战。

Method: 提出三个哲学贡献：1) 阐述"规范陷阱"论证；2) 提出"合成熵"作为理解多智能体对齐动态的信息论框架；3) 基于兼容主义指导控制理论建立真实与模拟道德能力的功能区分，并设计具身实验范式和验证机制。

Result: 建立了新的AI对齐理论框架，该框架能够生成关于人工系统中价值涌现和道德能动性的具体、可证伪预测，但实证验证仍在进行中。

Conclusion: AI对齐应转向过程导向、多智能体、发展性的方法，通过合成熵和理性响应机制构建智能体，而非试图编码固定价值内容。这为AI对齐提供了新的哲学基础和方法论方向。
...
TL;DR: 提出基于第一性原理的"权重计算主义"认知架构，通过逻辑原子和基本操作实现可解释决策，为AGI提供可信赖的理论基础


<details>
  <summary>Details</summary>
Motivation: 当前AI范式作为"体验架构师"面临可解释性和价值对齐的根本挑战，需要建立能够实现透明推理和价值追溯的新认知架构

Method: 将认知分解为不可分割的逻辑原子和两个基本操作（指向和比较），通过权重计算模型（权重=收益×概率）形式化决策，所有值可追溯到可审计的初始权重集，采用基于图算法的计算引擎和全局工作空间工作流实现

Result: 该架构实现了透明、类人的推理能力，在全新场景中展现出稳健的学习性能，为构建可信赖和对齐的AGI奠定了实践和理论基础

Conclusion: 权重计算主义为AGI提供了一条可行的技术路径，实现了根本的可解释性、内在的泛化能力和可追溯的价值对齐，是构建可信赖通用人工智能的重要进展
...
Method: 采用符号求解器集成方法，利用LLM的代码生成能力将推理任务翻译成可执行代码，然后使用符号求解器解决问题。通过实验比较传统长思维链方法与符号求解器集成方法在不同类型问题上的表现。
...
Conclusion: 符号求解器集成方法对特定类型的推理问题（特别是需要大量搜索和回溯的约束满足问题）有显著增强效果，而传统长思维链方法在需要深度隐式推理的问题上仍保持优势。两种方法应根据问题特性选择使用。
...
TL;DR: 该研究比较了主动推理中四种偏好分布定义方式对智能体在网格世界导航任务中性能的影响，发现目标塑造（中间目标）能提升整体性能但会牺牲环境动态学习。
...
Method: 研究考虑了四种定义偏好分布的方式：硬目标vs软目标、有目标塑造vs无目标塑造。在网格世界导航任务中比较了四种智能体的性能。

Result: 目标塑造（设置中间目标）能实现最佳整体性能（促进利用），但会牺牲对环境转移动态的学习（阻碍探索）。

Conclusion: 偏好分布的定义方式显著影响主动推理智能体的性能权衡，目标塑造虽然提升任务完成效率，但会减少环境知识获取，需要在具体应用场景中权衡选择。
...
<details>
  <summary>Details</summary>
Motivation: LLM智能体在社交互动中展现出强大能力，并越来越多地部署在与人类和人工智能体互动的场景中。然而，现有评估方法无法衡量这些能力在新颖社交情境中的泛化程度，这成为LLM智能体发展的关键前沿问题。

Method: 提出使用Concordia自然语言多智能体模拟环境来评估LLM智能体在零样本、混合动机环境中的合作能力。该方法通过测试智能体在不同合作伙伴和情境中识别和利用互利机会的能力来衡量一般合作智能。

Result: 通过NeurIPS 2024 Concordia竞赛的实证结果显示，当前智能体能力与实现可靠合作所需的稳健泛化之间存在显著差距，特别是在需要说服和规范执行的场景中。
...
TL;DR: Argos是一个用于训练多模态推理模型的奖励代理，通过智能选择评分函数来评估最终答案、时空定位和推理过程质量，在多个智能体任务中取得SOTA结果。
...
Method: 提出Argos奖励代理，为每个样本从教师模型和基于规则的评分函数池中选择合适的函数，同时评估：1)最终响应准确性；2)引用实体和动作的时空定位；3)推理过程质量。

Result: 在空间推理、视觉幻觉、机器人和具身AI等多个智能体任务中取得最先进结果。证明仅依赖SFT后训练不够，需要在线验证防止模型崩溃到非接地解，并能减少奖励黑客行为。

Conclusion: Argos通过智能奖励选择机制显著提升多模态推理模型的训练效果，提供理论上的帕累托最优性证明，为解决多模态强化学习中的奖励设计挑战提供了有效方案。
...
TL;DR: 提出一个通信受限的多智能体强化学习框架，通过区分有损和无损消息，并量化通信消息对全局奖励的影响，解决现实场景中的通信损失问题。


<details>
  <summary>Details</summary>
Motivation: 现实世界中通信损失普遍存在，现有多智能体强化学习方法由于可扩展性和鲁棒性有限，难以应用于复杂动态的真实环境。

Method: 提出广义通信受限模型统一描述不同场景的通信条件；将其作为学习先验区分有损和无损消息；使用双重互信息估计器解耦有损和无损消息对分布式决策的影响；引入通信受限多智能体强化学习框架，将通信消息影响量化到全局奖励中。

Result: 在多个通信受限基准测试中验证了方法的有效性。

Conclusion: 提出的通信受限多智能体强化学习框架能够有效处理现实世界中的通信损失问题，提高在复杂动态环境中的适用性。
...
Method: 采用分层多智能体架构，包含任务规划、执行、自我评估和反馈机制，能够检测和纠正高级战略错误。

Result: 在材料科学中成功复现锂离子传导和合金偏析的关键结果，协调数十个并行模拟任务；在Kaggle实验中从自然语言指令出发，生成与人工基准相当的解决方案。

Conclusion: 分层多智能体系统与自我评估反馈机制的结合，能够使AI系统具备独立进行大规模科学和分析工作的能力。
...
TL;DR: RP-ReAct是一种多智能体架构，将战略规划与低级执行解耦，通过Reasoner Planner Agent进行规划分析，Proxy-Execution Agent执行工具交互，并采用上下文保存策略管理大型工具输出，在复杂企业任务中实现更优的可靠性和效率。


<details>
  <summary>Details</summary>
Motivation: 当前自主智能体在企业复杂任务中存在两个主要局限：1）单智能体架构的单一规划-执行循环导致轨迹不稳定；2）为数据隐私使用本地开源模型时，较小的上下文窗口会因大型工具输出而快速耗尽，影响性能。

Method: 提出RP-ReAct多智能体方法，包含：Reasoner Planner Agent（RPA）负责规划子步骤并持续分析执行结果，使用大型推理模型；Proxy-Execution Agent（PEA）使用ReAct方法将子步骤转换为具体工具交互；并采用上下文保存策略，通过外部存储和按需访问管理大型工具输出，防止上下文窗口溢出。

Result: 在ToolQA基准测试中使用六种开源推理模型评估，RP-ReAct在跨域复杂任务中表现出优于最先进基线的性能和改进的泛化能力，同时在不同模型规模下展现出增强的鲁棒性和稳定性。

Conclusion: RP-ReAct通过解耦规划与执行的多智能体架构，结合上下文管理策略，为企业领域复杂任务提供了更有效、可部署的智能体解决方案，解决了单智能体架构的稳定性和上下文限制问题。
...
Method: 引入"概率天使非确定性"（PAN）编程模型，分离工作流和推理策略，使用Python装饰器将智能体工作流程序编译为搜索空间

Result: 实现EnCompass框架，通过三个案例研究证明该框架能快速提升智能体可靠性，轻松切换不同推理策略，且代码增量小

Conclusion: PAN模型和EnCompass框架为LLM智能体开发提供了更灵活、可实验的编程范式，显著简化了智能体设计和优化过程
...
TL;DR: DeepRule是一个用于零售品类和定价优化的自动化业务规则生成框架，通过三层架构解决理论模型与现实经济复杂性之间的差距，在真实零售环境中实现了比基线更高的利润。


<details>
  <summary>Details</summary>
Motivation: 解决现有理论模型与现实经济复杂性之间的系统性不匹配问题，具体包括：1）非结构化文本数据模态不匹配；2）动态特征纠缠（非线性价格弹性和时变属性建模）；3）多层级业务约束导致的运营不可行性。

Method: 采用三层架构：1）混合知识融合引擎，使用大语言模型深度语义解析非结构化文本；2）博弈论约束优化机制，通过双边效用函数动态协调供应链利益；3）可解释决策蒸馏接口，利用LLM引导的符号回归优化定价策略和可审计业务规则。

Result: 在真实零售环境中验证了框架有效性，相比系统性B2C基线实现了更高的利润，同时确保了运营可行性。
...
<details>
  <summary>Details</summary>
Motivation: 尽管大规模语言和视觉模型发展迅速，但AI代理仍存在根本性限制：它们无法记忆。没有可靠的记忆，代理会灾难性地遗忘过去经验，难以进行长时程推理，在多模态或交互环境中无法连贯操作。

Method: MemVerse采用模型无关的即插即用设计，结合快速参数化回忆与分层检索式记忆。它维护短期记忆用于近期上下文，同时将原始多模态经验转化为结构化长期记忆，组织为分层知识图谱。框架支持持续整合、适应性遗忘和有界内存增长。为满足实时需求，引入周期性蒸馏机制，将长期记忆中的关键知识压缩到参数化模型中，实现快速、可微的回忆同时保持可解释性。

Result: 大量实验表明，MemVerse显著提高了多模态推理和持续学习效率，使代理能够在扩展交互中记忆、适应和连贯推理。

Conclusion: MemVerse通过桥接参数化回忆与检索式记忆，为AI代理提供了可扩展、自适应的记忆能力，解决了现有系统在记忆、长时程推理和多模态环境操作方面的根本限制。
...
Method: 提出RoCo多智能体角色协作系统，包含四个专门角色：探索者（创造性、多样性驱动）、利用者（保守性、效率导向）、批评者（评估效果并提供反馈）、整合者（平衡创新与利用）。这些智能体通过结构化的多轮交互过程协作，包括反馈、精炼和精英突变，同时考虑短期和长期反思。

Result: 在五个不同的组合优化问题上，无论是白盒还是黑盒设置下，RoCo都表现出优越性能，生成的启发式算法优于现有方法（如ReEvo和HSEvo）。
...
TL;DR: 提出Omni-AutoThink自适应推理框架，通过动态调整推理深度来解决现有Omni模型推理行为僵化的问题，包含自适应监督微调和自适应强化学习两个阶段，并在多模态基准上取得显著改进。


<details>
  <summary>Details</summary>
Motivation: 现有Omni模型在推理行为上存在僵化问题：要么对简单问题过度推理，要么在需要推理时无法有效推理。需要一种能够根据任务难度动态调整推理深度的自适应框架。

Method: 提出两阶段框架：1）自适应监督微调阶段，使用大规模推理增强数据赋予Omni模型基础推理能力；2）自适应强化学习阶段，基于任务复杂度和奖励反馈优化推理行为。还构建了涵盖文本、文本-音频、文本-视觉、文本-音频-视觉多模态的自适应推理基准。

Result: 实验结果表明，相比先前基线方法，提出的框架在自适应推理性能上取得显著提升。所有基准数据和代码将公开。

Conclusion: Omni-AutoThink框架成功解决了Omni模型推理行为僵化的问题，通过自适应调整推理深度实现了更高效、更智能的多模态推理，为自适应推理研究提供了新的基准和工具。
...
TL;DR: 提出Static-DRA：基於樹狀靜態工作流的深度研究代理，透過Depth和Breadth參數讓用戶可調控研究深度與廣度，平衡報告品質與計算成本。
...
Method: 設計基於樹狀靜態工作流的Static-DRA，包含Supervisor、Independent和Worker三類代理，透過Depth和Breadth參數控制研究強度，實現多跳檢索與並行子主題研究。

Result: 在DeepResearch Bench上使用RACE框架評估，配置Depth=2、Breadth=5時獲得34.72分。實驗顯示增加Depth和Breadth參數能提升研究深度與評分。

Conclusion: Static-DRA提供實用且資源感知的解決方案，讓用戶能透明控制深度研究過程，在報告品質與計算成本間取得平衡。
...
TL;DR: 提出基于逻辑编程的策略感知自主代理框架，通过惩罚推理在必要时偏离策略以实现高风险目标，同时模拟非合规行为辅助政策制定。


<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注确保合规性，但实际场景中为实现高风险目标可能需要偏离策略。同时，建模非合规行为有助于政策制定者模拟真实人类决策。

Method: 扩展Gelfond和Lobo的授权与义务策略语言(AOPL)以纳入惩罚机制，集成回答集编程(ASP)进行推理。开发从扩展AOPL到ASP的自动翻译，并改进基于ASP的规划算法以考虑惩罚。

Result: 在两个领域实验中，该框架生成更高质量的计划，避免有害行动，并在某些情况下提高计算效率。能够区分非合规计划，优先选择惩罚最小的方案。

Conclusion: 该框架通过惩罚推理增强了自主决策能力，为政策完善提供信息支持，在逻辑编程理论与实践中具有应用潜力。
...
TL;DR: 该论文提出了一个用于评估LLM智能体在工业自动化中规划与执行能力的标准化基准测试，包含可执行的模拟环境和五个复杂度类别，通过MCP协议实现不同智能体架构的统一评估。


<details>
  <summary>Details</summary>
Motivation: 工业自动化需要灵活的控制策略来适应变化的任务和环境，基于大语言模型的智能体具有自适应规划与执行的潜力，但缺乏标准化的基准测试来进行系统化比较。

Method: 引入一个包含可执行模拟环境的基准测试，基于Blocksworld问题设计五个复杂度类别，通过集成模型上下文协议（MCP）作为标准化工具接口，使不同智能体架构能够无需特定修改即可连接到基准测试进行评估。

Result: 通过单智能体实现展示了基准测试的适用性，建立了用于比较基于LLM的规划与执行方法的定量指标。

Conclusion: 该基准测试为系统化评估和比较LLM智能体在工业自动化中的规划与执行能力提供了标准化框架，有助于推动自适应控制策略的发展。
...
### [171] [Thucy: An LLM-based Multi-Agent System for Claim Verification across Relational Databases](https://arxiv.org/abs/2512.03278)
*Michael Theologitis,Dan Suciu*

Main category: cs.DB

TL;DR: Thucy是一个跨数据库、跨表格的多智能体声明验证系统，能够自主发现、检查并推理关系数据库来验证声明，在TabFact基准上超越之前最佳方法5.6个百分点。


<details>
  <summary>Details</summary>
Motivation: 当前声明验证系统主要局限于小型单表数据库，无法处理真实世界中跨多个数据库和表格的复杂验证需求。虽然LLM和智能体AI取得了显著进展，但技术可能性与实际演示系统之间仍存在明显差距。

Method: Thucy采用多智能体架构，完全独立于底层数据源，能够自主发现、检查和推理所有可用关系数据库。系统生成支持验证结论的具体SQL查询，为熟悉SQL的专家用户提供完全透明度。

Result: 在TabFact数据集（结构化数据事实验证的标准基准）上，Thucy达到了94.3%的准确率，比之前最佳方法的88.7%提高了5.6个百分点。

Conclusion: Thucy展示了首个跨数据库、跨表格的多智能体声明验证系统的可行性，通过提供具体证据和完全透明的SQL查询，在结构化数据事实验证方面取得了显著进步。
...
### [172] [Continuous Prompts: LLM-Augmented Pipeline Processing over Unstructured Streams](https://arxiv.org/abs/2512.03389)
*Shu Chen,Deepti Raghavan,Uğur Çetintemel*

Main category: cs.DB

TL;DR: Continuous Prompts (CPs) 框架将LLM推理引入连续流处理，通过扩展RAG到流式环境、定义连续语义操作符，并采用动态优化框架在准确性和效率间进行权衡。


<details>
  <summary>Details</summary>
Motivation: 当前LLM框架是状态无关和单次执行的，无法满足对非结构化流进行持久、语义感知计算的需求，限制了其在长期运行分析中的应用。

Method: 1. 提出Continuous Prompts (CPs)框架，将RAG扩展到流式设置；2. 定义连续语义操作符；3. 实现多种方法（主要基于LLM，也包含嵌入变体）；4. 研究两种LLM优化：元组批处理和操作符融合；5. 开发动态优化框架，使用轻量级影子执行和成本感知多目标贝叶斯优化来学习吞吐量-准确性边界。

Result: 在VectraFlow流处理系统中实现CPs，通过操作级微基准测试和真实数据集上的流式管道验证，表明VectraFlow能够适应工作负载动态，在准确性和效率间进行权衡，并维持对演化非结构化流的持久语义查询。

Conclusion: Continuous Prompts框架成功将LLM推理引入连续流处理，解决了现有LLM框架在持久语义计算方面的局限性，通过动态优化实现了准确性和效率的有效平衡。
...
### [173] [Enterprise Data Science Platform: A Unified Architecture for Federated Data Access](https://arxiv.org/abs/2512.03401)
*Ryoto Miyamoto,Akira Kasuga*

Main category: cs.DB

TL;DR: 提出企业数据科学平台（EDSP），基于数据湖仓架构，遵循"一次写入，随处读取"原则，解决多查询引擎环境中的数据孤岛问题，减少数据复制和厂商锁定。


<details>
  <summary>Details</summary>
Motivation: 企业在不同部门采用不同数据分析平台时，数据共享困难。n个数据集需要服务m个环境时，可能产生n*m个副本，导致不一致性和成本增加。传统数据仓库将数据复制到特定厂商存储中，跨平台访问困难。

Method: 提出企业数据科学平台（EDSP），基于数据湖仓架构，采用四层架构：数据准备层、数据存储层、访问接口层和查询引擎层。遵循"一次写入，随处读取"原则，实现集中式数据管理和联邦数据访问。

Result: 主要云数据仓库和编程环境可以直接查询EDSP管理的数据集。实验显示，与传统需要数据迁移的方法相比，EDSP减少了33-44%的操作步骤。虽然查询延迟可能增加最多2.6倍，但端到端完成时间仍保持在秒级，满足分析用例的实际性能需求。

Conclusion: EDSP已在生产环境中部署，证实了跨多个查询引擎的互操作性。该平台为解决多查询引擎环境中的数据孤岛问题提供了实用的设计指南，消除了数据重复和厂商锁定问题。
...
### [174] [ExOAR: Expert-Guided Object and Activity Recognition from Textual Data](https://arxiv.org/abs/2512.03790)
*Iris Beerepoot,Vinicius Stein Dani,Xixi Lu*

Main category: cs.DB

TL;DR: ExOAR：结合大语言模型与人工验证的交互式方法，从非结构化文本中识别对象和活动，为对象中心流程挖掘提供结构化数据


<details>
  <summary>Details</summary>
Motivation: 对象中心流程挖掘需要结构化数据，但从非结构化文本中提取这种数据仍然是一个挑战。现有方法难以有效识别文本中的对象和活动，需要一种结合自动化与人工监督的解决方案。

Method: ExOAR（专家引导的对象和活动识别）是一种交互式方法，结合大语言模型与人工验证。系统引导用户通过连续阶段：LLM基于上下文输入（如用户职业）和文本数据生成候选对象类型、活动和对象实例，用户审查并优化这些建议后再进入下一阶段。

Result: ExOAR作为实用工具实现，首先通过演示进行初步验证，然后使用来自五个用户的真实世界活动窗口跟踪数据进行评估。结果表明，ExOAR能有效弥合非结构化文本数据与对象中心流程分析所需的结构化日志之间的差距，同时保持灵活性和人工监督。

Conclusion: ExOAR成功解决了从非结构化文本中提取结构化数据用于对象中心流程挖掘的挑战，通过结合LLM的自动化能力与人类专家的监督，实现了高效准确的对象和活动识别。
...
### [175] [IBM Multilevel Process Mining vs de facto Object-Centric Process Mining approaches](https://arxiv.org/abs/2512.03906)
...
<details>
  <summary>Details</summary>
Motivation: 过程挖掘领域正从传统方法向对象中心过程挖掘演进，IBM开发了多级过程挖掘方法，需要比较这两种方法的优缺点以指导产品发展

Method: 描述两种方法（对象中心过程挖掘和IBM多级过程挖掘），进行对比分析，基于比较开发新的组织挖掘功能，并通过实例演示

Result: IBM基于对比分析开发了组织挖掘新功能，该功能结合了两种方法的优势，展示了这种创新方法的潜力

Conclusion: 组织挖掘作为结合对象中心过程挖掘和多级过程挖掘优势的创新方法，具有显著潜力，推动了IBM过程挖掘产品的演进
